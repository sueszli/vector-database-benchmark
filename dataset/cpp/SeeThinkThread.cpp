...
SeeThinkThread::SeeThinkThread(NUbot* nubot) : ConditionalThread(string("SeeThinkThread"), THREAD_SEETHINK_PRIORITY)
{
    ...
    m_image_stream.open( (DATA_DIR + std::string("image.strm")).c_str() );
...
void SeeThinkThread::run()
{
    ...
    while (err == 0 && errno != EINTR)
    {
        try
        {
            ...
            #ifdef USE_VISION
                m_image_stream >> img;
                m_nubot->m_vision->ProcessFrame(&img, Blackboard->Sensors, Blackboard->Actions, Blackboard->Objects);

                std::cout << "SeeThinkThread::run()  frame " << ++frame << std::endl;
            #ifdef THREAD_SEETHINK_PROFILE
                    prof.split("vision");
                #endif
            #endif
