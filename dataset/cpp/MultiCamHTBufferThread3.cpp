...
int ConfigureTrigger(INodeMap & nodeMap, bool isPrimary)
{
    ...
    try
    {
		...
		//Enabling Hardware Trigger on Primary Camera
	...
	#if 0   //Enable the 3.3V option i.e. make it true
		...
		else 
		{
			...
			//////////////////////////////////////////////
		    // Setting trigger selector to FrameStart Mode
		    //////////////////////////////////////////////
...
void AcquireImages(CameraPtr pCam, vector<Mat> &imageBuffer)
{
    ...
    try
    {
		...
		cout << "Acquiring Images from " << pCam->GetUniqueID() << endl;
		...
		for(int i=0; i<numImages; i++)
        {
			...
			else
            {
				...
				Mat imgTemp = Mat(convertedImage->GetHeight(),
		                      convertedImage->GetWidth(), CV_8UC(1), convertedImage->GetData(), rowBytes);
...
int RunMultipleCameras(CameraList camList)
{
    ...
    try
    {
        ...
        for (int i = 0; i < camList.GetSize(); i++)
        {
			...
			if(pCam->GetUniqueID() == "16276645")
				result = ConfigureTrigger(nodeMap, true);
            else
            	result = ConfigureTrigger(nodeMap, false);
		...
		vector<Mat> imageBuffer1 (bufferSize);
		vector<Mat> imageBuffer2 (bufferSize);

		pCam1 = camList.GetByIndex(0);
		pCam2 = camList.GetByIndex(1);

#if 0
		thread t1(AcquireImages, pCam1, std::ref(imageBuffer1));
		thread t2(AcquireImages, pCam2, std::ref(imageBuffer2));
		thread t3(SaveImages, 1, std::ref(imageBuffer1));
		thread t4(SaveImages, 2, std::ref(imageBuffer2));
		...
		// This will not work because, the secondary camera is configured to acquire image only on receipt of hardware trigger.
		// In the below case, the second camera starts acquiring images only after the first camera has completed, so it will not get hardware trigger from primary camera
		//AcquireImages(pCam1, imageBuffer1);
		//AcquireImages(pCam2, imageBuffer2);
		//SaveImages(1, imageBuffer1);
		//SaveImages(2, imageBuffer2);
