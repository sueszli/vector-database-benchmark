...
template <typename T>
std::vector<T*> flat_filter(Pattern& pattern, std::string pattern_type) {
	...
	{
		if (p->pattern_is_type(pattern_type))
		{
			ret.emplace_back(static_cast<T*>(p));
		}
	}
	return ret;
}
...
static std::pair<Required, std::vector<Option>> create_pattern_tree(std::string const& doc)
{
	...
	std::vector<Option const*> pattern_options = flat_filter<Option const>(pattern, "Option");
	...
	for(auto& options_shortcut : flat_filter<OptionsShortcut>(pattern, "OptionsShortcut")) {
