...
private:
    ...
    vk::AttachmentReference* getAttachmentReference() { return (s.num_refs < max_refs) ? &s.attachmentReferences[s.num_refs++] : nullptr; }
...
public:
    ...
    std::ostream& write(std::ostream& os) {
        ...
        snprintf(tmp, sizeof(tmp), "  0x%08x,0x%08x,0x%08x,0x%08x,0x%08x,\n", p[0], p[1], p[2], p[3], p[4]);
        os << tmp;
        for (int i = 5; i != s.opcodes_.size(); i += s.opcodes_[i] >> 16) {
...
public:
    ...
    void descriptorSetLayout(vk::DescriptorSetLayout layout) { setLayouts_.push_back(layout); }
...
public:
    ...
    void colorBlend(const vk::PipelineColorBlendAttachmentState& state) { colorBlendAttachments_.push_back(state); }

    void subPass(uint32_t subpass) { subpass_ = subpass; }
    ...
    /// Add a vertex attribute to the pipeline.
    void vertexAttribute(const vk::VertexInputAttributeDescription& desc) { vertexAttributeDescriptions_.push_back(desc); }
    ...
    /// Usually only one of these is needed to specify the stride.
    /// Vertices can also be delivered one per instance.
    void vertexBinding(const vk::VertexInputBindingDescription& desc) { vertexBindingDescriptions_.push_back(desc); }
...
public:
    ...
    GenericBuffer(vk::Device device,
    ...
    void barrier(vk::CommandBuffer cb,
...
public:
    ...
    template <class Type, class Allocator>
    HostVertexBuffer(const vk::Device& device, const vk::PhysicalDeviceMemoryProperties& memprops, const std::vector<Type, Allocator>& value)
        : GenericBuffer(device, memprops, vk::BufferUsageFlagBits::eVertexBuffer, value.size() * sizeof(Type), vk::MemoryPropertyFlagBits::eHostVisible) {
...
public:
    ...
    template <class Type, class Allocator>
    HostIndexBuffer(const vk::Device& device, const vk::PhysicalDeviceMemoryProperties& memprops, const std::vector<Type, Allocator>& value)
        : GenericBuffer(device, memprops, vk::BufferUsageFlagBits::eIndexBuffer, value.size() * sizeof(Type), vk::MemoryPropertyFlagBits::eHostVisible) {
        updateLocal(device, value);
    }
};
...
public:
    ...
    void beginDescriptorSet(vk::DescriptorSet dstSet) { dstSet_ = dstSet; }
    ...
    void copy(vk::DescriptorSet srcSet,
              uint32_t srcBinding,
    ...
    void update(const vk::Device& device) const { device.updateDescriptorSets(descriptorWrites_, descriptorCopies_); }
...
public:
    ...
    void layout(vk::DescriptorSetLayout layout) { s.layouts.push_back(layout); }
...
public:
    ...
    void setCurrentLayout(vk::ImageLayout oldLayout) { s.currentLayout = oldLayout; }
...
protected:
    void create(vk::Device device,
        ...
        if (hostImage)
            search = vk::MemoryPropertyFlagBits::eHostCoherent | vk::MemoryPropertyFlagBits::eHostVisible;
...
public:
    ...
    TextureImage2D(vk::Device device,
                   const vk::PhysicalDeviceMemoryProperties& memprops,
                   uint32_t width,
                   uint32_t height,
...
public:
    ...
    TextureImageCube(vk::Device device,
                     const vk::PhysicalDeviceMemoryProperties& memprops,
                     uint32_t width,
                     uint32_t height,
                     uint32_t mipLevels = 1,
...
public:
    ...
    DepthStencilImage(vk::Device device,
                      const vk::PhysicalDeviceMemoryProperties& memprops,
                      uint32_t width,
                      uint32_t height,
...
public:
    ...
    ColorAttachmentImage(vk::Device device,
                         const vk::PhysicalDeviceMemoryProperties& memprops,
                         uint32_t width,
                         uint32_t height,
                         vk::Format format = vk::Format::eR8G8B8A8Unorm) {
...
public:
    ...
    vk::UniqueSampler createUnique(vk::Device device) const { return device.createSamplerUnique(s.info); }

    /// Allocate a non self-deleting Sampler.
    vk::Sampler create(vk::Device device) const { return device.createSampler(s.info); }
...
inline vk::Format GLtoVKFormat(uint32_t glFormat) {
    switch (glFormat) {
        case 0x1907:
            return vk::Format::eR8G8B8Unorm;  // GL_RGB
        case 0x1908:
            return vk::Format::eR8G8B8A8Unorm;  // GL_RGBA
        case 0x83F0:
            return vk::Format::eBc1RgbUnormBlock;  // GL_COMPRESSED_RGB_S3TC_DXT1_EXT
        case 0x83F1:
            return vk::Format::eBc1RgbaUnormBlock;  // GL_COMPRESSED_RGBA_S3TC_DXT1_EXT
        case 0x83F2:
            return vk::Format::eBc3UnormBlock;  // GL_COMPRESSED_RGBA_S3TC_DXT3_EXT
        case 0x83F3:
            return vk::Format::eBc5UnormBlock;  // GL_COMPRESSED_RGBA_S3TC_DXT5_EXT
    }
...
public:
    ...
    KTXFileLayout(uint8_t* begin, uint8_t* end) {
        uint8_t* p = begin;
        if (p + sizeof(Header) > end)
            return;
        header = *(Header*)p;
        static const uint8_t magic[] = { 0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31, 0x31, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A };
        ...
        if (format_ == vk::Format::eUndefined)
            return;

        p += sizeof(Header);
        if (p + header.bytesOfKeyValueData > end)
            return;

        for (uint32_t i = 0; i < header.bytesOfKeyValueData;) {
            uint32_t keyAndValueByteSize = *(uint32_t*)(p + i);
            if (header.endianness != 0x04030201)
                swap(keyAndValueByteSize);
    ...
    uint32_t size(uint32_t mipLevel) { return imageSizes_[mipLevel]; }
...
private:
    static void swap(uint32_t& value) { value = value >> 24 | (value & 0xff0000) >> 8 | (value & 0xff00) << 8 | value << 24; }
