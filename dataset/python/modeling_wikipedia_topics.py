...
def build_and_save_wikipedia_model(save_path):
    # the indexed wikipedia file containing id and word mappings
    id2word = gensim.corpora.Dictionary.load_from_text('data/wiki_en_wordids.txt')

    # we have a matrix market format
    corpus = gensim.corpora.MmCorpus('data/wiki_en_output_tfidf.mm')

    # now build the LDA model
    # this might take some hours
    mm = gensim.models.LdaModel(corpus=corpus, id2word=id2word, num_topics=100, update_every=1, chunksize=10000,
                                passes=1)
    mm.save(save_path)
    return mm
