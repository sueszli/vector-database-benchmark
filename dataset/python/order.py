...
class InstallKamsERP_Order(InstallKamsERP_Configuration):
    ...
    def get_orders(self):
        ...
        for order in orders:
            customer = InstallKamsERP_Customer().create_or_update_customer(order)  # need update
        ...
        for line in fetched_order_line:
            if line:
                order_line_to_insert = {
                    ...
                    'price_unit': float(InstallKamsERP_Product.get_netto_price_for_product(line)),
                    ...
                    'product_id': InstallKamsERP_Product.get_product_id(line),
                }
                self.xml_operand.insert_order_line([order_line_to_insert])

        shipment_to_insert = {
            'order_id': customer_order.get('id'),
            'price_unit': float(InstallKamsERP_Product.calculate_netto_price(order.przesylka_koszt_brutto, 23)),
