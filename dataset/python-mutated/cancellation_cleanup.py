...
class CancellationCleanup(LoopService):
    ...
    def __init__(self, loop_seconds: float=None, **kwargs):
        if False:
            while True:
                i = 10
        super().__init__(loop_seconds=loop_seconds or PREFECT_API_SERVICES_CANCELLATION_CLEANUP_LOOP_SECONDS.value(), **kwargs)
        self.batch_size = 200
    ...
    async def clean_up_cancelled_flow_run_task_runs(self, db):
        while True:
            cancelled_flow_query = sa.select(db.FlowRun).where(db.FlowRun.state_type == states.StateType.CANCELLED, db.FlowRun.end_time.is_not(None), db.FlowRun.end_time >= pendulum.now('UTC').subtract(days=1)).limit(self.batch_size)
    ...
    async def clean_up_cancelled_subflow_runs(self, db):
        ...
        while True:
            subflow_query = sa.select(db.FlowRun).where(or_(db.FlowRun.state_type == states.StateType.PENDING, db.FlowRun.state_type == states.StateType.SCHEDULED, db.FlowRun.state_type == states.StateType.RUNNING, db.FlowRun.state_type == states.StateType.PAUSED, db.FlowRun.state_type == states.StateType.CANCELLING, db.FlowRun.id > high_water_mark), db.FlowRun.parent_task_run_id.is_not(None)).order_by(db.FlowRun.id).limit(self.batch_size)
            async with db.session_context() as session:
    ...
    async def _cancel_child_runs(self, db: PrefectDBInterface, flow_run: PrefectDBInterface.FlowRun) -> None:
        async with db.session_context() as session:
            child_task_runs = await models.task_runs.read_task_runs(session, flow_run_filter=filters.FlowRunFilter(id={'any_': [flow_run.id]}), task_run_filter=filters.TaskRunFilter(state={'type': {'any_': NON_TERMINAL_STATES}}), limit=100)
        for task_run in child_task_runs:
            async with db.session_context(begin_transaction=True) as session:
                await models.task_runs.set_task_run_state(session=session, task_run_id=task_run.id, state=states.Cancelled(message='The parent flow run was cancelled.'), force=True)

    async def _cancel_subflow(self, db: PrefectDBInterface, flow_run: PrefectDBInterface.FlowRun) -> None:
        if not flow_run.parent_task_run_id:
            return False
        if flow_run.state.type in states.TERMINAL_STATES:
            return False
        async with db.session_context() as session:
            parent_task_run = await models.task_runs.read_task_run(session, task_run_id=flow_run.parent_task_run_id)
            if not parent_task_run:
                return False
            containing_flow_run = await models.flow_runs.read_flow_run(session, flow_run_id=parent_task_run.flow_run_id)
            if containing_flow_run and containing_flow_run.state.type != states.StateType.CANCELLED:
                return False
        ...
        async with db.session_context(begin_transaction=True) as session:
            await models.flow_runs.set_flow_run_state(session=session, flow_run_id=flow_run.id, state=state)
