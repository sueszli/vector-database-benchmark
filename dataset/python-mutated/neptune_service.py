...
class Neptune(AWSService):
    ...
    def __describe_clusters__(self, regional_client):
        ...
        try:
            for cluster in regional_client.describe_db_clusters(Filters=[{'Name': 'engine', 'Values': [self.service_name]}])['DBClusters']:
                cluster_arn = cluster['DBClusterArn']
                if not self.audit_resources or is_resource_filtered(cluster_arn, self.audit_resources):
                    self.clusters[cluster_arn] = Cluster(arn=cluster_arn, name=cluster['DBClusterIdentifier'], id=cluster['DbClusterResourceId'], db_subnet_group_id=cluster['DBSubnetGroup'], region=regional_client.region)
        ...
        try:
            for cluster in self.clusters.values():
                if cluster.region == regional_client.region:
                    try:
                        subnets = []
                        db_subnet_groups = regional_client.describe_db_subnet_groups(DBSubnetGroupName=cluster.db_subnet_group_id)['DBSubnetGroups']
