...
from homeassistant.components.light import ATTR_BRIGHTNESS, ATTR_COLOR_MODE, ATTR_COLOR_TEMP, ATTR_EFFECT, ATTR_FLASH, ATTR_HS_COLOR, ATTR_RGB_COLOR, ATTR_RGBW_COLOR, ATTR_RGBWW_COLOR, ATTR_TRANSITION, ATTR_WHITE, ATTR_XY_COLOR, ENTITY_ID_FORMAT, FLASH_LONG, FLASH_SHORT, VALID_COLOR_MODES, ColorMode, LightEntity, LightEntityFeature, brightness_supported, color_supported, filter_supported_color_modes, valid_supported_color_modes
from homeassistant.const import CONF_BRIGHTNESS, CONF_COLOR_TEMP, CONF_EFFECT, CONF_HS, CONF_NAME, CONF_OPTIMISTIC, CONF_RGB, CONF_XY, STATE_ON
...
from ..const import CONF_COMMAND_TOPIC, CONF_ENCODING, CONF_QOS, CONF_RETAIN, CONF_STATE_TOPIC
from ..debug_info import log_messages
from ..mixins import MQTT_ENTITY_COMMON_SCHEMA, MqttEntity, write_state_on_attr_change
from ..models import ReceiveMessage
from ..util import valid_subscribe_topic
from .schema import MQTT_LIGHT_SCHEMA_SCHEMA
from .schema_basic import CONF_BRIGHTNESS_SCALE, CONF_WHITE_SCALE, MQTT_LIGHT_ATTRIBUTES_BLOCKED
...
def valid_color_configuration(config: ConfigType) -> ConfigType:
    ...
    return config
_PLATFORM_SCHEMA_BASE = MQTT_RW_SCHEMA.extend({vol.Optional(CONF_BRIGHTNESS, default=DEFAULT_BRIGHTNESS): cv.boolean, vol.Optional(CONF_BRIGHTNESS_SCALE, default=DEFAULT_BRIGHTNESS_SCALE): vol.All(vol.Coerce(int), vol.Range(min=1)), vol.Inclusive(CONF_COLOR_MODE, 'color_mode', default=DEFAULT_COLOR_MODE): cv.boolean, vol.Optional(CONF_COLOR_TEMP, default=DEFAULT_COLOR_TEMP): cv.boolean, vol.Optional(CONF_EFFECT, default=DEFAULT_EFFECT): cv.boolean, vol.Optional(CONF_EFFECT_LIST): vol.All(cv.ensure_list, [cv.string]), vol.Optional(CONF_FLASH_TIME_LONG, default=DEFAULT_FLASH_TIME_LONG): cv.positive_int, vol.Optional(CONF_FLASH_TIME_SHORT, default=DEFAULT_FLASH_TIME_SHORT): cv.positive_int, vol.Optional(CONF_HS, default=DEFAULT_HS): cv.boolean, vol.Optional(CONF_MAX_MIREDS): cv.positive_int, vol.Optional(CONF_MIN_MIREDS): cv.positive_int, vol.Optional(CONF_NAME): vol.Any(cv.string, None), vol.Optional(CONF_QOS, default=DEFAULT_QOS): vol.All(vol.Coerce(int), vol.In([0, 1, 2])), vol.Optional(CONF_RETAIN, default=DEFAULT_RETAIN): cv.boolean, vol.Optional(CONF_RGB, default=DEFAULT_RGB): cv.boolean, vol.Optional(CONF_STATE_TOPIC): valid_subscribe_topic, vol.Inclusive(CONF_SUPPORTED_COLOR_MODES, 'color_mode'): vol.All(cv.ensure_list, [vol.In(VALID_COLOR_MODES)], vol.Unique(), valid_supported_color_modes), vol.Optional(CONF_WHITE_SCALE, default=DEFAULT_WHITE_SCALE): vol.All(vol.Coerce(int), vol.Range(min=1)), vol.Optional(CONF_XY, default=DEFAULT_XY): cv.boolean}).extend(MQTT_ENTITY_COMMON_SCHEMA.schema).extend(MQTT_LIGHT_SCHEMA_SCHEMA.schema)
DISCOVERY_SCHEMA_JSON = vol.All(_PLATFORM_SCHEMA_BASE.extend({}, extra=vol.REMOVE_EXTRA), valid_color_configuration)
PLATFORM_SCHEMA_MODERN_JSON = vol.All(_PLATFORM_SCHEMA_BASE, valid_color_configuration)
...
class MqttLightJson(MqttEntity, LightEntity, RestoreEntity):
    ...
    async def _subscribe_topics(self) -> None:
        ...
        if self._optimistic and last_state:
            ...
            self._attr_brightness = last_attributes.get(ATTR_BRIGHTNESS, self.brightness)
            self._attr_color_mode = last_attributes.get(ATTR_COLOR_MODE, self.color_mode)
            self._attr_color_temp = last_attributes.get(ATTR_COLOR_TEMP, self.color_temp)
            ...
            self._attr_rgbw_color = last_attributes.get(ATTR_RGBW_COLOR, self.rgbw_color)
            self._attr_rgbww_color = last_attributes.get(ATTR_RGBWW_COLOR, self.rgbww_color)
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if ATTR_HS_COLOR in kwargs and (self._config[CONF_HS] or self._config[CONF_RGB] or self._config[CONF_XY]):
        ...
        if ATTR_RGBWW_COLOR in kwargs and self._supports_color_mode(ColorMode.RGBWW):
            ...
            message['color'] = {'r': rgbcw[0], 'g': rgbcw[1], 'b': rgbcw[2], 'c': rgbcw[3], 'w': rgbcw[4]}
            if self._optimistic:
                self._attr_color_mode = ColorMode.RGBWW
        ...
        if ATTR_WHITE in kwargs and self._supports_color_mode(ColorMode.WHITE):
            ...
            if self._optimistic:
                ...
                should_update = True
        await self.async_publish(str(self._topic[CONF_COMMAND_TOPIC]), json_dumps(message), self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
        if self._optimistic:
            self._attr_is_on = True
            should_update = True
        if should_update:
