...
class UKCoalEmploy(ChangePointFileDataset):
    ...
    def __init__(self):
        if False:
            i = 10
            return i + 15
        super().__init__(annotations={'6': [15, 28, 45, 60, 68, 80], '7': [18, 47, 81], '8': [], '9': [15, 27, 46, 68, 81], '13': [19, 28, 45, 68, 80]}, filename='uk_coal_employment.csv', task=datasets.base.REG, n_samples=105, n_features=1)

    def __iter__(self):
        if False:
            while True:
                i = 10
        return stream.iter_csv(self.path, target='Employment', converters={'Employment': int}, parse_dates={'Year': '%Y'})