...
my_reconciler = MyManagedElementReconciler(ManagedElementDiff().add('foo', 'bar').add('same', 'as').with_nested('nested', ManagedElementDiff().add('qwerty', 'uiop').add('new', 'field')))
...
my_nested_reconciler = MyReconcilerHolder(MyManagedElementReconciler(ManagedElementDiff().modify('foo', 'bar', 'baz')))