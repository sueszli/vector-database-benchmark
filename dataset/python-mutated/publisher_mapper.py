...
from calibre.gui2.tag_mapper import RuleEdit as RuleEditBase, RuleEditDialog as RuleEditDialogBase, RuleItem as RuleItemBase, Rules as RulesBase, RulesDialog as RulesDialogBase, Tester as TesterBase
from calibre.utils.config import JSONConfig
publisher_maps = JSONConfig('publisher-mapping-rules')

class RuleEdit(RuleEditBase):
    ACTION_MAP = OrderedDict((('replace', _('Change')), ('capitalize', _('Capitalize')), ('titlecase', _('Title-case')), ('lower', _('Lower-case')), ('upper', _('Upper-case'))))
    MATCH_TYPE_MAP = OrderedDict((('one_of', _('is one of')), ('not_one_of', _('is not one of')), ('has', _('contains')), ('matches', _('matches regex pattern')), ('not_matches', _('does not match regex pattern'))))
...
class RuleItem(RuleItemBase):
    ...
    def text_from_rule(rule, parent):
        ...
        text = _('<b>{action}</b> the publisher name, if it <i>{match_type}</i>: <b>{query}</b>').format(action=RuleEdit.ACTION_MAP[rule['action']], match_type=RuleEdit.MATCH_TYPE_MAP[rule['match_type']], query=query)
...
class Rules(RulesBase):
    ...
    MSG = _('You can specify rules to manipulate publisher names here. Click the "Add Rule" button below to get started. The rules will be processed in order for every publisher.')
...
if __name__ == '__main__':
    app = Application([])
    d = RulesDialog()
    d.rules = [{'action': 'replace', 'query': 'alice Bob', 'match_type': 'one_of', 'replace': 'Alice Bob'}]
    d.exec()
