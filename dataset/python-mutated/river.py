...
class RiverModel(state.HasState):
    ...
    batch_size = traitlets.Int(default_value=1000000, allow_none=False, help='Number of samples to be sent to the model in each batch.')
    ...
    prediction_type = traitlets.Enum(values=['predict', 'predict_proba'], default_value='predict', help='Which method to use to get the predictions.                                      Can be "predict" or "predict_proba" which correspond to                                      "predict_many" and "predict_proba_many in a River model respectively.')
