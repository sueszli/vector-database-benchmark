...
class TestASGIReceiveProxy:
    ...
    async def test_basic(self, setup_receive_proxy: Tuple[ASGIReceiveProxy, ActorHandle]):
    ...
    async def test_actor_raises_exception(self, setup_receive_proxy: Tuple[ASGIReceiveProxy, ActorHandle]):
        (asgi_receive_proxy, actor) = setup_receive_proxy
        await actor.put.remote({'type': 'foo'})
        await actor.put.remote({'type': 'bar'})
    ...
    async def test_actor_crashes(self, setup_receive_proxy: Tuple[ASGIReceiveProxy, ActorHandle]):
        (asgi_receive_proxy, actor) = setup_receive_proxy
        await actor.put.remote({'type': 'foo'})
        await actor.put.remote({'type': 'bar'})
        assert await asgi_receive_proxy() == {'type': 'foo'}
