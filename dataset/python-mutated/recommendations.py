...
ENSURE_RIGHT_NDK_MESSAGE = 'Make sure your NDK version is greater than {min_supported}. If you get build errors, download the recommended NDK {rec_version} from {ndk_url}.'
NDK_LOWER_THAN_SUPPORTED_MESSAGE = 'The minimum supported NDK version is {min_supported}. You can download it from {ndk_url}.'
UNSUPPORTED_NDK_API_FOR_ARMEABI_MESSAGE = 'Asked to build for armeabi architecture with API {req_ndk_api}, but API {max_ndk_api} or greater does not support armeabi.'
CURRENT_NDK_VERSION_MESSAGE = 'Found NDK version {ndk_version}'
RECOMMENDED_NDK_VERSION_MESSAGE = 'Maximum recommended NDK version is {recommended_ndk_version}, but newer versions may work.'
...
OLD_API_MESSAGE = 'Target APIs lower than 30 are no longer supported on Google Play, and are not recommended. Note that the Target API can be higher than your device Android version, and should usually be as high as possible.'
...
TARGET_NDK_API_GREATER_THAN_TARGET_API_MESSAGE = 'Target NDK API is {ndk_api}, higher than the target Android API {android_api}.'
...
MIN_PYTHON_VERSION = packaging.version.Version(f'{MIN_PYTHON_MAJOR_VERSION}.{MIN_PYTHON_MINOR_VERSION}')
PY2_ERROR_TEXT = 'python-for-android no longer supports running under Python 2. Either upgrade to Python {min_version} or higher (recommended), or revert to python-for-android 2019.07.08.'.format(min_version=MIN_PYTHON_VERSION)
PY_VERSION_ERROR_TEXT = 'Your Python version {user_major}.{user_minor} is not supported by python-for-android, please upgrade to {min_version} or higher.'.format(user_major=sys.version_info.major, user_minor=sys.version_info.minor, min_version=MIN_PYTHON_VERSION)
