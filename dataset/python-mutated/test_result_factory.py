...
from prefect.settings import PREFECT_LOCAL_STORAGE_PATH, PREFECT_RESULTS_DEFAULT_SERIALIZER, PREFECT_RESULTS_PERSIST_BY_DEFAULT, temporary_settings
...
async def factory(prefect_client):
    return await ResultFactory.default_factory(client=prefect_client, persist_result=True)
...
async def test_root_flow_custom_storage_by_instance_unsaved(prefect_client, tmp_path):
    ...
    assert isinstance(result_factory.storage_block_id, uuid.UUID)
    storage_block_document = await prefect_client.read_block_document(result_factory.storage_block_id)
...
async def test_child_flow_custom_storage_by_instance_unsaved(prefect_client, tmp_path):
    ...
    assert isinstance(child_factory.storage_block_id, uuid.UUID)
    storage_block_document = await prefect_client.read_block_document(child_factory.storage_block_id)
    assert LocalFileSystem._from_block_document(storage_block_document) == storage
    assert child_factory.persist_result is True
    assert child_factory.serializer == DEFAULT_SERIALIZER()
...
async def test_task_custom_storage_by_instance_unsaved(prefect_client, tmp_path):
    ...
    assert isinstance(task_factory.storage_block_id, uuid.UUID)
    storage_block_document = await prefect_client.read_block_document(task_factory.storage_block_id)
    assert LocalFileSystem._from_block_document(storage_block_document) == storage
    assert task_factory.persist_result is True
    assert task_factory.serializer == DEFAULT_SERIALIZER()

async def _verify_default_storage_creation_with_persistence(prefect_client, result_factory: prefect.results.ResultFactory):
...
async def _verify_default_storage_creation_without_persistence(result_factory: prefect.results.ResultFactory):
    assert result_factory.storage_block is not None
    assert_blocks_equal(result_factory.storage_block, DEFAULT_STORAGE())
...
@pytest.mark.parametrize('options', [{'persist_result': True}, {'cache_result_in_memory': False}])
async def test_default_storage_creation_for_flow_with_persistence_features(prefect_client, options):
    ...
    def foo():
        ...
        return get_run_context().result_factory
    result_factory = foo()
    await _verify_default_storage_creation_with_persistence(prefect_client, result_factory)
...
async def test_default_storage_creation_for_task_with_persistence_features(prefect_client):
...
@pytest.mark.parametrize('options,expected', [({'persist_result': True, 'cache_result_in_memory': False, 'result_serializer': 'json'}, {'persist_result': True, 'cache_result_in_memory': False, 'serializer': JSONSerializer()}), ({'persist_result': False, 'cache_result_in_memory': True, 'result_serializer': 'json'}, {'persist_result': False, 'cache_result_in_memory': True, 'serializer': JSONSerializer()})])
