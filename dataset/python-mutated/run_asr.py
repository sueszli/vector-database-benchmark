...
from transformers import HfArgumentParser, Trainer, TrainingArguments, Wav2Vec2CTCTokenizer, Wav2Vec2FeatureExtractor, Wav2Vec2ForCTC, Wav2Vec2Processor, is_apex_available, trainer_utils
...
class DataTrainingArguments:
    ...
    train_split_name: Optional[str] = field(default='train', metadata={'help': "The name of the training data set split to use (via the datasets library). Defaults to 'train'"})
    validation_split_name: Optional[str] = field(default='validation', metadata={'help': "The name of the validation data set split to use (via the datasets library). Defaults to 'validation'"})
    target_text_column: Optional[str] = field(default='text', metadata={'help': "Column in the dataset that contains label (target text). Defaults to 'text'"})
    speech_file_column: Optional[str] = field(default='file', metadata={'help': "Column in the dataset that contains speech file path. Defaults to 'file'"})
    target_feature_extractor_sampling_rate: Optional[bool] = field(default=False, metadata={'help': "Resample loaded audio to target feature extractor's sampling rate or not."})
    max_duration_in_seconds: Optional[float] = field(default=None, metadata={'help': 'Filters out examples longer than specified. Defaults to no filtering.'})
    orthography: Optional[str] = field(default='librispeech', metadata={'help': "Orthography used for normalization and tokenization: 'librispeech' (default), 'timit', or 'buckwalter'."})
