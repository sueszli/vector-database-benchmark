...
class PDQNPolicy(Policy):
    ...
    Config:
        ...
        == ==================== ======== ============== ======================================== =======================
    """
    config = dict(type='pdqn', cuda=False, on_policy=False, priority=False, priority_IS_weight=False, discount_factor=0.97, nstep=1, learn=dict(update_per_collect=3, batch_size=64, learning_rate=0.001, target_theta=0.005, ignore_done=False), collect=dict(unroll_len=1, noise_sigma=0.1), eval=dict(), other=dict(eps=dict(type='exp', start=0.95, end=0.1, decay=10000), replay_buffer=dict(replay_buffer_size=10000)))
