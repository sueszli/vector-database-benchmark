...
from .fixtures import app_client, app_client_with_trace, app_client_returned_rows_matches_page_size, generate_compound_rows, generate_sortable_rows, make_app_client
import json
import pytest
import urllib
...
async def test_table_json(ds_client):
    ...
    data = response.json()
    assert data['query']['sql'] == 'select id, content from simple_primary_key order by id limit 51'
    assert data['query']['params'] == {}
    assert data['rows'] == [{'id': '1', 'content': 'hello'}, {'id': '2', 'content': 'world'}, {'id': '3', 'content': ''}, {'id': '4', 'content': 'RENDER_CELL_DEMO'}, {'id': '5', 'content': 'RENDER_CELL_ASYNC'}]

@pytest.mark.asyncio
async def test_table_not_exists_json(ds_client):
    assert (await ds_client.get('/fixtures/blah.json')).json() == {'ok': False, 'error': 'Table not found: blah', 'status': 404, 'title': None}

@pytest.mark.asyncio
async def test_table_shape_arrays(ds_client):
    response = await ds_client.get('/fixtures/simple_primary_key.json?_shape=arrays')
    assert response.json()['rows'] == [['1', 'hello'], ['2', 'world'], ['3', ''], ['4', 'RENDER_CELL_DEMO'], ['5', 'RENDER_CELL_ASYNC']]

@pytest.mark.asyncio
async def test_table_shape_arrayfirst(ds_client):
    response = await ds_client.get('/fixtures.json?' + urllib.parse.urlencode({'sql': 'select content from simple_primary_key order by id', '_shape': 'arrayfirst'}))
    assert response.json() == ['hello', 'world', '', 'RENDER_CELL_DEMO', 'RENDER_CELL_ASYNC']

@pytest.mark.asyncio
async def test_table_shape_objects(ds_client):
    response = await ds_client.get('/fixtures/simple_primary_key.json?_shape=objects')
    assert response.json()['rows'] == [{'id': '1', 'content': 'hello'}, {'id': '2', 'content': 'world'}, {'id': '3', 'content': ''}, {'id': '4', 'content': 'RENDER_CELL_DEMO'}, {'id': '5', 'content': 'RENDER_CELL_ASYNC'}]

@pytest.mark.asyncio
async def test_table_shape_array(ds_client):
    response = await ds_client.get('/fixtures/simple_primary_key.json?_shape=array')
    assert response.json() == [{'id': '1', 'content': 'hello'}, {'id': '2', 'content': 'world'}, {'id': '3', 'content': ''}, {'id': '4', 'content': 'RENDER_CELL_DEMO'}, {'id': '5', 'content': 'RENDER_CELL_ASYNC'}]

@pytest.mark.asyncio
async def test_table_shape_array_nl(ds_client):
    response = await ds_client.get('/fixtures/simple_primary_key.json?_shape=array&_nl=on')
    lines = response.text.split('\n')
    results = [json.loads(line) for line in lines]
    assert [{'id': '1', 'content': 'hello'}, {'id': '2', 'content': 'world'}, {'id': '3', 'content': ''}, {'id': '4', 'content': 'RENDER_CELL_DEMO'}, {'id': '5', 'content': 'RENDER_CELL_ASYNC'}] == results

@pytest.mark.asyncio
async def test_table_shape_invalid(ds_client):
    response = await ds_client.get('/fixtures/simple_primary_key.json?_shape=invalid')
    assert response.json() == {'ok': False, 'error': 'Invalid _shape: invalid', 'status': 400, 'title': None}

@pytest.mark.asyncio
async def test_table_shape_object(ds_client):
    response = await ds_client.get('/fixtures/simple_primary_key.json?_shape=object')
    assert response.json() == {'1': {'id': '1', 'content': 'hello'}, '2': {'id': '2', 'content': 'world'}, '3': {'id': '3', 'content': ''}, '4': {'id': '4', 'content': 'RENDER_CELL_DEMO'}, '5': {'id': '5', 'content': 'RENDER_CELL_ASYNC'}}
...
async def test_table_shape_object_compound_primary_key(ds_client):
    ...
    assert response.json() == {'a,b': {'pk1': 'a', 'pk2': 'b', 'content': 'c'}, 'a~2Fb,~2Ec-d': {'pk1': 'a/b', 'pk2': '.c-d', 'content': 'c'}}

@pytest.mark.asyncio
async def test_table_with_slashes_in_name(ds_client):
    response = await ds_client.get('/fixtures/table~2Fwith~2Fslashes~2Ecsv.json?_shape=objects')
    assert response.status_code == 200
    data = response.json()
...
async def test_table_with_reserved_word_name(ds_client):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data['rows'] == [{'rowid': 1, 'group': 'group', 'having': 'having', 'and': 'and', 'json': '{"href": "http://example.com/", "label":"Example"}'}]

@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_rows,expected_pages', [('/fixtures/no_primary_key.json', 201, 5), ('/fixtures/paginated_view.json', 201, 9), ('/fixtures/no_primary_key.json?_size=25', 201, 9), ('/fixtures/paginated_view.json?_size=50', 201, 5), ('/fixtures/paginated_view.json?_size=max', 201, 3), ('/fixtures/123_starts_with_digits.json', 0, 1), ('/fixtures/compound_three_primary_keys.json?_facet=pk1', 1001, 21), ('/fixtures/roadside_attraction_characteristics.json?_size=2&_sort=attraction_id&_labels=on', 5, 3)])
async def test_paginate_tables_and_views(ds_client, path, expected_rows, expected_pages):
    fetched = []
    count = 0
    while path:
        if '?' in path:
...
@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_error', [('/fixtures/no_primary_key.json?_size=-4', '_size must be a positive integer'), ('/fixtures/no_primary_key.json?_size=dog', '_size must be a positive integer'), ('/fixtures/no_primary_key.json?_size=1001', '_size must be <= 100')])
...
async def test_page_size_zero(ds_client):
    ...
    response = await ds_client.get('/fixtures/no_primary_key.json?_size=0&_extra=count,next_url')
    assert response.status_code == 200
...
async def test_sortable_and_filtered(ds_client):
    path = '/fixtures/sortable.json?content__contains=d&_sort_desc=sortable&_shape=objects&_extra=human_description_en,count'
    response = await ds_client.get(path)
    fetched = response.json()['rows']
    assert 'where content contains "d" sorted by sortable descending' == response.json()['human_description_en']
...
async def test_sortable_argument_errors(ds_client):
    ...
    response = await ds_client.get('/fixtures/sortable.json?_sort=sortable_with_nulls&_sort_desc=sortable')
...
@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_rows', [('/fixtures/searchable.json?_shape=arrays&_search=dog', [[1, 'barry cat', 'terry dog', 'panther'], [2, 'terry dog', 'sara weasel', 'puma']]), ('/fixtures/searchable.json?_shape=arrays&_search=AND', []), ('/fixtures/searchable.json?_shape=arrays&_search=te*+AND+do*', []), ('/fixtures/searchable.json?_shape=arrays&_search=te*+AND+do*&_searchmode=raw', [[1, 'barry cat', 'terry dog', 'panther'], [2, 'terry dog', 'sara weasel', 'puma']]), ('/fixtures/searchable.json?_shape=arrays&_search_text2=te*&_searchmode=raw', [[1, 'barry cat', 'terry dog', 'panther']]), ('/fixtures/searchable.json?_shape=arrays&_search=weasel', [[2, 'terry dog', 'sara weasel', 'puma']]), ('/fixtures/searchable.json?_shape=arrays&_search_text2=dog', [[1, 'barry cat', 'terry dog', 'panther']]), ('/fixtures/searchable.json?_shape=arrays&_search_name%20with%20.%20and%20spaces=panther', [[1, 'barry cat', 'terry dog', 'panther']])])
async def test_searchable(ds_client, path, expected_rows):
    response = await ds_client.get(path)
    assert expected_rows == response.json()['rows']
_SEARCHMODE_RAW_RESULTS = [[1, 'barry cat', 'terry dog', 'panther'], [2, 'terry dog', 'sara weasel', 'puma']]
...
@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_rows', [('/fixtures/searchable_view_configured_by_metadata.json?_shape=arrays&_search=weasel', [[2, 'terry dog', 'sara weasel', 'puma']]), ('/fixtures/searchable_view.json?_shape=arrays&_search=weasel', [[1, 'barry cat', 'terry dog', 'panther'], [2, 'terry dog', 'sara weasel', 'puma']]), ('/fixtures/searchable_view.json?_shape=arrays&_search=weasel&_fts_table=searchable_fts&_fts_pk=pk', [[2, 'terry dog', 'sara weasel', 'puma']])])
...
async def test_searchable_invalid_column(ds_client):
    ...
    assert response.status_code == 400
    assert response.json() == {'ok': False, 'error': 'Cannot search by that column', 'status': 400, 'title': None}

@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_rows', [('/fixtures/simple_primary_key.json?_shape=arrays&content=hello', [['1', 'hello']]), ('/fixtures/simple_primary_key.json?_shape=arrays&content__contains=o', [['1', 'hello'], ['2', 'world'], ['4', 'RENDER_CELL_DEMO']]), ('/fixtures/simple_primary_key.json?_shape=arrays&content__exact=', [['3', '']]), ('/fixtures/simple_primary_key.json?_shape=arrays&content__not=world', [['1', 'hello'], ['3', ''], ['4', 'RENDER_CELL_DEMO'], ['5', 'RENDER_CELL_ASYNC']])])
...
async def test_table_filter_queries_multiple_of_same_type(ds_client):
    response = await ds_client.get('/fixtures/simple_primary_key.json?_shape=arrays&content__not=world&content__not=hello')
    assert [['3', ''], ['4', 'RENDER_CELL_DEMO'], ['5', 'RENDER_CELL_ASYNC']] == response.json()['rows']

@pytest.mark.skipif(not detect_json1(), reason='Requires the SQLite json1 module')
@pytest.mark.asyncio
async def test_table_filter_json_arraycontains(ds_client):
    response = await ds_client.get('/fixtures/facetable.json?_shape=arrays&tags__arraycontains=tag1')
    assert response.json()['rows'] == [[1, '2019-01-14 08:00:00', 1, 1, 'CA', 1, 'Mission', '["tag1", "tag2"]', '[{"foo": "bar"}]', 'one', 'n1'], [2, '2019-01-14 08:00:00', 1, 1, 'CA', 1, 'Dogpatch', '["tag1", "tag3"]', '[]', 'two', 'n2']]

@pytest.mark.skipif(not detect_json1(), reason='Requires the SQLite json1 module')
@pytest.mark.asyncio
async def test_table_filter_json_arraynotcontains(ds_client):
    response = await ds_client.get('/fixtures/facetable.json?_shape=arrays&tags__arraynotcontains=tag3&tags__not=[]')
    assert response.json()['rows'] == [[1, '2019-01-14 08:00:00', 1, 1, 'CA', 1, 'Mission', '["tag1", "tag2"]', '[{"foo": "bar"}]', 'one', 'n1']]

@pytest.mark.asyncio
async def test_table_filter_extra_where(ds_client):
    response = await ds_client.get("/fixtures/facetable.json?_shape=arrays&_where=_neighborhood='Dogpatch'")
    assert [[2, '2019-01-14 08:00:00', 1, 1, 'CA', 1, 'Dogpatch', '["tag1", "tag3"]', '[]', 'two', 'n2']] == response.json()['rows']

@pytest.mark.asyncio
async def test_table_filter_extra_where_invalid(ds_client):
    response = await ds_client.get("/fixtures/facetable.json?_where=_neighborhood=Dogpatch'")
    assert response.status_code == 400
...
async def test_table_through(ds_client):
    response = await ds_client.get('/fixtures/roadside_attractions.json?_shape=arrays&_through={"table":"roadside_attraction_characteristics","column":"characteristic_id","value":"1"}&_extra=human_description_en')
    assert response.json()['rows'] == [[3, 'Burlingame Museum of PEZ Memorabilia', '214 California Drive, Burlingame, CA 94010', None, 37.5793, -122.3442], [4, 'Bigfoot Discovery Museum', '5497 Highway 9, Felton, CA 95018', 'https://www.bigfootdiscoveryproject.com/', 37.0414, -122.0725]]
    assert response.json()['human_description_en'] == 'where roadside_attraction_characteristics.characteristic_id = "1"'

@pytest.mark.asyncio
async def test_max_returned_rows(ds_client):
    response = await ds_client.get('/fixtures.json?sql=select+content+from+no_primary_key')
    data = response.json()
...
async def test_view(ds_client):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data['rows'] == [{'upper_content': 'HELLO', 'content': 'hello'}, {'upper_content': 'WORLD', 'content': 'world'}, {'upper_content': '', 'content': ''}, {'upper_content': 'RENDER_CELL_DEMO', 'content': 'RENDER_CELL_DEMO'}, {'upper_content': 'RENDER_CELL_ASYNC', 'content': 'RENDER_CELL_ASYNC'}]

@pytest.mark.xfail
@pytest.mark.asyncio
async def test_unit_filters(ds_client):
    response = await ds_client.get('/fixtures/units.json?_shape=arrays&distance__lt=75km&frequency__gt=1kHz')
    assert response.status_code == 200
    data = response.json()
...
async def test_expand_labels(ds_client):
    response = await ds_client.get('/fixtures/facetable.json?_shape=object&_labels=1&_size=2&_neighborhood__contains=c')
    assert response.json() == {'2': {'pk': 2, 'created': '2019-01-14 08:00:00', 'planet_int': 1, 'on_earth': 1, 'state': 'CA', '_city_id': {'value': 1, 'label': 'San Francisco'}, '_neighborhood': 'Dogpatch', 'tags': '["tag1", "tag3"]', 'complex_array': '[]', 'distinct_some_null': 'two', 'n': 'n2'}, '13': {'pk': 13, 'created': '2019-01-17 08:00:00', 'planet_int': 1, 'on_earth': 1, 'state': 'MI', '_city_id': {'value': 3, 'label': 'Detroit'}, '_neighborhood': 'Corktown', 'tags': '[]', 'complex_array': '[]', 'distinct_some_null': None, 'n': None}}

@pytest.mark.asyncio
async def test_expand_label(ds_client):
    response = await ds_client.get('/fixtures/foreign_key_references.json?_shape=object&_label=foreign_key_with_label&_size=1')
    assert response.json() == {'1': {'pk': '1', 'foreign_key_with_label': {'value': '1', 'label': 'hello'}, 'foreign_key_with_blank_label': '3', 'foreign_key_with_no_label': '1', 'foreign_key_compound_pk1': 'a', 'foreign_key_compound_pk2': 'b'}}

@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_cache_control', [('/fixtures/facetable.json', 'max-age=5'), ('/fixtures/facetable.json?_ttl=invalid', 'max-age=5'), ('/fixtures/facetable.json?_ttl=10', 'max-age=10'), ('/fixtures/facetable.json?_ttl=0', 'no-cache')])
...
async def test_infinity_returned_as_null(ds_client):
    ...
    assert response.json() == [{'rowid': 1, 'value': None}, {'rowid': 2, 'value': None}, {'rowid': 3, 'value': 1.5}]

@pytest.mark.asyncio
async def test_infinity_returned_as_invalid_json_if_requested(ds_client):
    response = await ds_client.get('/fixtures/infinity.json?_shape=array&_json_infinity=1')
    assert response.json() == [{'rowid': 1, 'value': float('inf')}, {'rowid': 2, 'value': float('-inf')}, {'rowid': 3, 'value': 1.5}]

@pytest.mark.asyncio
async def test_custom_query_with_unicode_characters(ds_client):
    response = await ds_client.get('/fixtures/~F0~9D~90~9C~F0~9D~90~A2~F0~9D~90~AD~F0~9D~90~A2~F0~9D~90~9E~F0~9D~90~AC.json?_shape=array')
    assert response.json() == [{'id': 1, 'name': 'San Francisco'}]

@pytest.mark.asyncio
async def test_null_and_compound_foreign_keys_are_not_expanded(ds_client):
    response = await ds_client.get('/fixtures/foreign_key_references.json?_shape=array&_labels=on')
    assert response.json() == [{'pk': '1', 'foreign_key_with_label': {'value': '1', 'label': 'hello'}, 'foreign_key_with_blank_label': {'value': '3', 'label': ''}, 'foreign_key_with_no_label': {'value': '1', 'label': '1'}, 'foreign_key_compound_pk1': 'a', 'foreign_key_compound_pk2': 'b'}, {'pk': '2', 'foreign_key_with_label': None, 'foreign_key_with_blank_label': None, 'foreign_key_with_no_label': None, 'foreign_key_compound_pk1': None, 'foreign_key_compound_pk2': None}]
...
@pytest.mark.asyncio
@pytest.mark.parametrize('qs', ['', '?_shape=arrays', '?_shape=arrayfirst', '?_shape=object', '?_shape=objects', '?_shape=array', '?_shape=array&_nl=on'])
...
@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_columns', (('/fixtures/facetable.json?_col=created', ['pk', 'created']), ('/fixtures/facetable.json?_nocol=created', ['pk', 'planet_int', 'on_earth', 'state', '_city_id', '_neighborhood', 'tags', 'complex_array', 'distinct_some_null', 'n']), ('/fixtures/facetable.json?_col=state&_col=created', ['pk', 'state', 'created']), ('/fixtures/facetable.json?_col=state&_col=state', ['pk', 'state']), ('/fixtures/facetable.json?_col=state&_col=created&_nocol=created', ['pk', 'state']), ('/fixtures/facetable.json?_nocol=state&_facet=state', ['pk', 'created', 'planet_int', 'on_earth', '_city_id', '_neighborhood', 'tags', 'complex_array', 'distinct_some_null', 'n']), ('/fixtures/simple_view.json?_nocol=content', ['upper_content']), ('/fixtures/simple_view.json?_col=content', ['content'])))
...
@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_error', (('/fixtures/facetable.json?_col=bad', '_col=bad - invalid columns'), ('/fixtures/facetable.json?_nocol=bad', '_nocol=bad - invalid columns'), ('/fixtures/facetable.json?_nocol=pk', '_nocol=pk - invalid columns'), ('/fixtures/simple_view.json?_col=bad', '_col=bad - invalid columns')))
