...
class APPOTfLearner(AppoLearner, TfLearner):
    ...
    def compute_loss_for_module(self, *, module_id: ModuleID, hps: AppoLearnerHyperparameters, batch: NestedDict, fwd_out: Mapping[str, TensorType]) -> TensorType:
        ...
        (vtrace_adjusted_target_values, pg_advantages) = vtrace_tf2(target_action_log_probs=old_actions_logp_time_major, behaviour_action_log_probs=behaviour_actions_logp_time_major, discounts=discounts_time_major, rewards=rewards_time_major, values=values_time_major, bootstrap_value=bootstrap_value, clip_pg_rho_threshold=hps.vtrace_clip_pg_rho_threshold, clip_rho_threshold=hps.vtrace_clip_rho_threshold)
        is_ratio = tf.clip_by_value(tf.math.exp(behaviour_actions_logp_time_major - old_actions_logp_time_major), 0.0, 2.0)
        logp_ratio = is_ratio * tf.math.exp(target_actions_logp_time_major - behaviour_actions_logp_time_major)
        surrogate_loss = tf.math.minimum(pg_advantages * logp_ratio, pg_advantages * tf.clip_by_value(logp_ratio, 1 - hps.clip_param, 1 + hps.clip_param))
        if hps.use_kl_loss:
    ...
    def _update_module_target_networks(self, module_id: ModuleID, hps: AppoLearnerHyperparameters) -> None:
        ...
        for (target_network, current_network) in target_current_network_pairs:
            for (old_var, current_var) in zip(target_network.variables, current_network.variables):
