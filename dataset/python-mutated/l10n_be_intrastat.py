...
class AccountInvoiceLine(models.Model):
    ...
    intrastat_transaction_id = fields.Many2one('l10n_be_intrastat.transaction', string='Intrastat Transaction Type', help='Intrastat nature of transaction')

class AccountInvoice(models.Model):
    _inherit = 'account.invoice'
    incoterm_id = fields.Many2one('stock.incoterms', string='Incoterm', help='International Commercial Terms are a series of predefined commercial terms used in international transactions.')
    transport_mode_id = fields.Many2one('l10n_be_intrastat.transport_mode', string='Intrastat Transport Mode')
    intrastat_country_id = fields.Many2one('res.country', string='Intrastat Country', help='Intrastat country, delivery for sales, origin for purchases', domain=[('intrastat', '=', True)])
...
class IntrastatRegion(models.Model):
    ...
    _sql_constraints = [('l10n_be_intrastat_regioncodeunique', 'UNIQUE (code)', 'Code must be unique.')]
...
class IntrastatTransaction(models.Model):
    ...
    _sql_constraints = [('l10n_be_intrastat_trcodeunique', 'UNIQUE (code)', 'Code must be unique.')]

class IntrastatTransportMode(models.Model):
    _name = 'l10n_be_intrastat.transport_mode'
    code = fields.Char(required=True, readonly=True)
    name = fields.Char(string='Description', readonly=True)
    _sql_constraints = [('l10n_be_intrastat_trmodecodeunique', 'UNIQUE (code)', 'Code must be unique.')]
...
class ResCompany(models.Model):
    ...
    transport_mode_id = fields.Many2one('l10n_be_intrastat.transport_mode', string='Default transport mode')
    incoterm_id = fields.Many2one('stock.incoterms', string='Default incoterm for Intrastat', help='International Commercial Terms are a series of predefined commercial terms used in international transactions.')
