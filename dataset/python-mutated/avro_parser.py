...
AVRO_TYPE_TO_JSON_TYPE = {'null': 'null', 'boolean': 'boolean', 'int': 'integer', 'long': 'integer', 'float': 'number', 'double': 'string', 'bytes': 'string', 'string': 'string'}
AVRO_LOGICAL_TYPE_TO_JSON = {'decimal': {'type': 'string'}, 'uuid': {'type': 'string'}, 'date': {'type': 'string', 'format': 'date'}, 'time-millis': {'type': 'integer'}, 'time-micros': {'type': 'integer'}, 'timestamp-millis': {'type': 'string', 'format': 'date-time'}, 'timestamp-micros': {'type': 'string'}, 'local-timestamp-millis': {'type': 'string', 'format': 'date-time'}, 'local-timestamp-micros': {'type': 'string'}}
...
class AvroParser(FileTypeParser):
    ...
    async def infer_schema(self, config: FileBasedStreamConfig, file: RemoteFile, stream_reader: AbstractFileBasedStreamReader, logger: logging.Logger) -> SchemaType:
        ...
        json_schema = {field['name']: AvroParser._convert_avro_type_to_json(avro_format, field['name'], field['type']) for field in avro_schema['fields']}
        return json_schema
