...
class ScrollBar(Widget):
    ...
    async def _on_mouse_move(self, event: events.MouseMove) -> None:
        if self.grabbed and self.window_size:
            ...
            if self.vertical:
                virtual_size = self.window_virtual_size
                y = round(self.grabbed_position + (event.screen_y - self.grabbed.y) * (virtual_size / self.window_size))
            else:
                virtual_size = self.window_virtual_size
                x = round(self.grabbed_position + (event.screen_x - self.grabbed.x) * (virtual_size / self.window_size))
