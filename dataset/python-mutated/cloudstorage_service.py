            ...
            try:
                ...
                while request is not None:
                    ...
                    for bucket in response.get('items', []):
                        bucket_iam = self.client.buckets().getIamPolicy(bucket=bucket['id']).execute()['bindings']
                        public = False
                        if 'allAuthenticatedUsers' in str(bucket_iam) or 'allUsers' in str(bucket_iam):
                            public = True
                        self.buckets.append(Bucket(name=bucket['name'], id=bucket['id'], region=bucket['location'], uniform_bucket_level_access=bucket['iamConfiguration']['uniformBucketLevelAccess']['enabled'], public=public, retention_policy=bucket.get('retentionPolicy'), project_id=project_id))
