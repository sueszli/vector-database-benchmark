...
if MODEL == 'yolov3':
    ...
    model = Yolo(model)
    detector = PyTorchYolo(model=model, device_type='cpu', input_shape=(3, 640, 640), clip_values=(0, 255), attack_losses=('loss_total',))
elif MODEL == 'yolov5':
    import yolov5
...
preds = extract_predictions(dets[0], threshold)
plot_image_with_boxes(img=image_adv[0].transpose(1, 2, 0).copy(), boxes=preds[1], pred_cls=preds[0], title='Predictions on adversarial image')