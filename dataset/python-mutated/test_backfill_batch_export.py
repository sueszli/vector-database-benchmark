...
from posthog.temporal.tests.utils.models import acreate_batch_export, adelete_batch_export, afetch_batch_export_backfills
from posthog.temporal.workflows.backfill_batch_export import BackfillBatchExportInputs, BackfillBatchExportWorkflow, BackfillScheduleInputs, backfill_range, backfill_schedule, get_schedule_frequency, wait_for_schedule_backfill_in_range
...
async def temporal_schedule(temporal_client, team):
    ...
    batch_export_data = {'name': 'no-op-export', 'destination': destination_data, 'interval': interval, 'paused': True}
    batch_export = await acreate_batch_export(team_id=team.pk, name=batch_export_data['name'], destination_data=batch_export_data['destination'], interval=batch_export_data['interval'])
...
async def test_backfill_schedule_activity(activity_environment, temporal_worker, temporal_schedule):
    ...
    desc = await temporal_schedule.describe()
    inputs = BackfillScheduleInputs(schedule_id=desc.id, start_at=start_at.isoformat(), end_at=end_at.isoformat(), buffer_limit=2, wait_delay=1.0, frequency_seconds=desc.schedule.spec.intervals[0].every.total_seconds())
...
async def test_backfill_batch_export_workflow(temporal_worker, temporal_schedule, temporal_client, team):
    ...
    inputs = BackfillBatchExportInputs(team_id=team.pk, batch_export_id=desc.id, start_at=start_at.isoformat(), end_at=end_at.isoformat(), buffer_limit=2, wait_delay=1.0)
    handle = await temporal_client.start_workflow(BackfillBatchExportWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE, execution_timeout=dt.timedelta(minutes=1), retry_policy=temporalio.common.RetryPolicy(maximum_attempts=1))
    await handle.result()
    desc = await temporal_schedule.describe()
    result = desc.info.num_actions
    expected = 10
...
@pytest.mark.django_db(transaction=True)
async def test_backfill_batch_export_workflow_fails_when_schedule_deleted(temporal_worker, temporal_schedule, temporal_client, team):
    """Test BackfillBatchExportWorkflow fails when its underlying Temporal Schedule is deleted."""
    start_at = dt.datetime(2023, 1, 1, 0, 0, 0, tzinfo=dt.timezone.utc)
    end_at = dt.datetime(2023, 1, 1, 0, 10, 0, tzinfo=dt.timezone.utc)
    desc = await temporal_schedule.describe()
    workflow_id = str(uuid.uuid4())
    inputs = BackfillBatchExportInputs(team_id=team.pk, batch_export_id=desc.id, start_at=start_at.isoformat(), end_at=end_at.isoformat(), buffer_limit=1, wait_delay=2.0)
    handle = await temporal_client.start_workflow(BackfillBatchExportWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE, execution_timeout=dt.timedelta(seconds=20), retry_policy=temporalio.common.RetryPolicy(maximum_attempts=1))
...
@pytest.mark.django_db(transaction=True)
async def test_backfill_batch_export_workflow_fails_when_schedule_deleted_after_running(temporal_worker, temporal_schedule, temporal_client, team):
    """Test BackfillBatchExportWorkflow fails when its underlying Temporal Schedule is deleted.
    ...
    inputs = BackfillBatchExportInputs(team_id=team.pk, batch_export_id=desc.id, start_at=start_at.isoformat(), end_at=end_at.isoformat(), buffer_limit=1, wait_delay=2.0)
    handle = await temporal_client.start_workflow(BackfillBatchExportWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE, execution_timeout=dt.timedelta(seconds=20), retry_policy=temporalio.common.RetryPolicy(maximum_attempts=1))
    await wait_for_schedule_backfill_in_range(client=temporal_client, schedule_id=desc.id, start_at=start_at, end_at=dt.datetime(2023, 1, 1, 0, 1, 0, tzinfo=dt.timezone.utc), now=now, wait_delay=1.0)
    desc = await temporal_schedule.describe()
    result = desc.info.num_actions
    assert result >= 1
    await temporal_schedule.delete()
