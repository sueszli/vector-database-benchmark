    ...
    def execute(self):
        ...
        if shield_client.enabled:
            for elb in elb_client.loadbalancers:
                ...
                report.status = 'FAIL'
                report.status_extended = f'ELB {elb.name} is not protected by AWS Shield Advanced.'
                for protection in shield_client.protections.values():
                    if elb.arn == protection.resource_arn:
                        report.status = 'PASS'
                        report.status_extended = f'ELB {elb.name} is protected by AWS Shield Advanced.'
                        break
