...
def create_segment_to_index_task(segment_id: str, keywords: Optional[List[str]]=None):
    ...
    try:
        update_params = {DocumentSegment.status: 'indexing', DocumentSegment.indexing_at: datetime.datetime.utcnow()}
        ...
        update_params = {DocumentSegment.status: 'completed', DocumentSegment.completed_at: datetime.datetime.utcnow()}
        DocumentSegment.query.filter_by(id=segment.id).update(update_params)
        db.session.commit()
