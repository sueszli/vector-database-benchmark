...
from ..utils import determine_ext, float_or_none, int_or_none, parse_age_limit, qualities, random_birthday, unified_timestamp, urljoin

class VideoPressIE(InfoExtractor):
    _ID_REGEX = '[\\da-zA-Z]{8}'
    _PATH_REGEX = 'video(?:\\.word)?press\\.com/embed/'
    _VALID_URL = 'https?://%s(?P<id>%s)' % (_PATH_REGEX, _ID_REGEX)
    _EMBED_REGEX = [f"""<iframe[^>]+src=["\\'](?P<url>(?:https?://)?{_PATH_REGEX}{_ID_REGEX})"""]
    _TESTS = [{'url': 'https://videopress.com/embed/kUJmAcSf', 'md5': '706956a6c875873d51010921310e4bc6', 'info_dict': {'id': 'kUJmAcSf', 'ext': 'mp4', 'title': 'VideoPress Demo', 'thumbnail': 're:^https?://.*\\.jpg', 'duration': 634.6, 'timestamp': 1434983935, 'upload_date': '20150622', 'age_limit': 0}}, {'url': 'https://videopress.com/embed/iH3gstfZ', 'only_matching': True}, {'url': 'https://video.wordpress.com/embed/kUJmAcSf', 'only_matching': True}]
