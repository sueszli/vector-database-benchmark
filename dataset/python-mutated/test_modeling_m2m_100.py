...
def prepare_m2m_100_inputs_dict(config, input_ids, decoder_input_ids, attention_mask=None, decoder_attention_mask=None, head_mask=None, decoder_head_mask=None, cross_attn_head_mask=None):
    if False:
        for i in range(10):
            print('nop')
...
class M2M100ModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'conversational': M2M100ForConditionalGeneration, 'feature-extraction': M2M100Model, 'summarization': M2M100ForConditionalGeneration, 'text2text-generation': M2M100ForConditionalGeneration, 'translation': M2M100ForConditionalGeneration} if is_torch_available() else {}
...
class M2M100ModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor([[-0.778, -0.1676, 0.1038], [-6.7556, -1.3992, 0.0567], [-7.5383, -0.592, -0.2779]], device=torch_device)
    ...
    def test_inference_head(self):
        ...
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[-1.0448, -1.0411, 3.7992], [-3.2191, -3.2386, -1.3451], [-3.621, -3.5993, 0.4925]], device=torch_device)
        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=TOLERANCE))
    ...
    def test_seq_to_seq_generation(self):
        ...
        src_fr = ["L'affaire NSA souligne l'absence totale de débat sur le renseignement", 'Selon moi, il y a deux niveaux de réponse de la part du gouvernement français.', "Lorsque François Hollande téléphone à Barack Obama ou quand le ministre des affaires étrangères Laurent Fabius convoque l'ambassadeur des Etats-Unis, ils réagissent à une vraie découverte, qui est celle de l'ampleur de la surveillance américaine sur l'ensemble des communications en France."]
        dct = tokenizer(src_fr, padding=True, return_tensors='pt')
        hypotheses_batch = model.generate(input_ids=dct['input_ids'].to(torch_device), attention_mask=dct['attention_mask'].to(torch_device), num_beams=5, forced_bos_token_id=tokenizer.get_lang_id('en'))
        expected_en = ['The NSA case highlights the total absence of intelligence debate', 'I think there are two levels of response from the French government.', 'When François Hollande calls Barack Obama or when Foreign Minister Laurent Fabius calls the U.S. Ambassador, they respond to a real discovery, which is that of the scale of U.S. surveillance on all communications in France.']
