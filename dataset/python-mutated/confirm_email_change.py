...
class ConfirmEmailChange(BaseMutation):
    ...
    class Arguments:
        token = graphene.String(description='A one-time token required to change the email.', required=True)
        channel = graphene.String(description='Slug of a channel which will be used to notify users. Optional when only one channel exists.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.CUSTOMER_UPDATED, description='A customer account was updated.'), WebhookEventInfo(type=WebhookEventAsyncType.NOTIFY_USER, description='A notification that account email change was confirmed.'), WebhookEventInfo(type=WebhookEventAsyncType.ACCOUNT_EMAIL_CHANGED, description='An account email was changed.')]
