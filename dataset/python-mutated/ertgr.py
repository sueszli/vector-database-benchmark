...
from ..utils import clean_html, determine_ext, ExtractorError, dict_get, int_or_none, merge_dicts, parse_qs, parse_age_limit, parse_iso8601, str_or_none, try_get, url_or_none, variadic
...
class ERTFlixCodenameIE(ERTFlixBaseIE):
    ...
    _TESTS = [{'url': 'ertflix:monogramma-praxitelis-tzanoylinos', 'md5': '5b9c2cd171f09126167e4082fc1dd0ef', 'info_dict': {'id': 'monogramma-praxitelis-tzanoylinos', 'ext': 'mp4', 'title': 'md5:ef0b439902963d56c43ac83c3f41dd0e'}}]
...
class ERTWebtvEmbedIE(InfoExtractor):
    ...
    _VALID_URL = f'https?:{_BASE_PLAYER_URL_RE}\\?([^#]+&)?f=(?P<id>[^#&]+)'
    _EMBED_REGEX = [f"""<iframe[^>]+?src=(?P<_q1>["\\'])(?P<url>(?:https?:)?{_BASE_PLAYER_URL_RE}\\?(?:(?!(?P=_q1)).)+)(?P=_q1)"""]
    _TESTS = [{'url': 'https://www.ert.gr/webtv/live-uni/vod/dt-uni-vod.php?f=trailers/E2251_TO_DIKTYO_E09_16-01_1900.mp4&bgimg=/photos/2022/1/to_diktio_ep09_i_istoria_tou_diadiktiou_stin_Ellada_1021x576.jpg', 'md5': 'f9e9900c25c26f4ecfbddbb4b6305854', 'info_dict': {'id': 'trailers/E2251_TO_DIKTYO_E09_16-01_1900.mp4', 'title': 'md5:914f06a73cd8b62fbcd6fb90c636e497', 'ext': 'mp4', 'thumbnail': 'https://program.ert.gr/photos/2022/1/to_diktio_ep09_i_istoria_tou_diadiktiou_stin_Ellada_1021x576.jpg'}}]
