...
@click.command(name='p', help='for a folder as input')
...
@click.option('-w', '--watch', default=False, is_flag=True, show_default=True, help='watches a folder for changes')
...
@click.argument('input', type=click.Path(exists=True, path_type=pathlib.Path, file_okay=False, dir_okay=True, readable=True))
@click.argument('output', type=click.Path(exists=False, path_type=pathlib.Path, file_okay=False, dir_okay=True, writable=True))
def p_command(model: str, extras: str, input: pathlib.Path, output: pathlib.Path, watch: bool, **kwargs) -> None:
    if False:
        for i in range(10):
            print('nop')
    ...
    def process(each_input: pathlib.Path) -> None:
        ...
        try:
            ...
            if not each_output.exists():
                each_output.write_bytes(cast(bytes, remove(each_input.read_bytes(), session=session, **kwargs)))
                if watch:
                    print(f'processed: {each_input.absolute()} -> {each_output.absolute()}')
