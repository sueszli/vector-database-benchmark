...
flags.DEFINE_boolean('download_only', False, 'Only perform downloading of data; Do not proceed to session preparation, model definition or training')
...
flags.DEFINE_integer('replicas_to_aggregate', None, 'Number of replicas to aggregate before parameter updateis applied (For sync_replicas mode only; default: num_workers)')
flags.DEFINE_integer('train_steps', 20000, 'Number of (global) training steps to perform')
...
flags.DEFINE_boolean('existing_servers', False, 'Whether servers already exists. If True, will use the worker hosts via their GRPC URLs (one client process per worker host). Otherwise, will create an in-process TensorFlow server.')
