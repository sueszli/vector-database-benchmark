...
if __name__ == '__main__':
    ...
    parser.add_argument('--ops', default='dense_dense_mm,bsr_dense_mm,bsr_scatter_mm6', type=str)
    ...
    if args.star > 0:
        ...
        if 'bsr_scatter_mm6' in ops:
            if split_n_list[0] is None:
                split_n_list = [meta['SPLIT_N'] // 2, meta['SPLIT_N'], meta['SPLIT_N'] * 2][int(meta['SPLIT_N'] == 1):]
            elif split_n_list[0] == 0:
                split_n_list = [meta['SPLIT_N']]
            if tile_m_list[0] is None:
                tile_m_list = [meta['TILE_M'] // 2, meta['TILE_M'], meta['TILE_M'] * 2][int(meta['TILE_M'] == 16):]
            elif tile_m_list[0] == 0:
                tile_m_list = [meta['TILE_M']]
            if tile_n_list[0] is None:
                tile_n_list = [meta['TILE_N'] // 2, meta['TILE_N'], meta['TILE_N'] * 2][int(meta['TILE_N'] == 16):]
            elif tile_n_list[0] == 0:
                tile_n_list = [meta['TILE_N']]
            if group_size_list[0] is None:
                group_size_list = [meta['GROUP_SIZE'] - 1, meta['GROUP_SIZE'], meta['GROUP_SIZE'] + 1][int(meta['GROUP_SIZE'] == 1):]
            elif group_size_list[0] == 0:
                group_size_list = [meta['GROUP_SIZE']]
        if 'bsr_dense_mm_with_meta' in ops:
            if group_size_list[0] is None:
                group_size_list = [meta['GROUP_SIZE_ROW'] - 1, meta['GROUP_SIZE_ROW'], meta['GROUP_SIZE_ROW'] + 1][int(meta['GROUP_SIZE_ROW'] == 1):]
            elif group_size_list[0] == 0:
                group_size_list = [meta['GROUP_SIZE_ROW']]
        if num_warps_list[0] is None:
            num_warps_list = [meta['num_warps'] // 2, meta['num_warps'], meta['num_warps'] * 2][int(meta['num_warps'] == 1):]
        elif num_warps_list[0] == 0:
            num_warps_list = [meta['num_warps']]
        if num_stages_list[0] is None:
            num_stages_list = [meta['num_stages'] - 1, meta['num_stages'], meta['num_stages'] + 1][int(meta['num_stages'] == 1):]
        elif num_stages_list[0] == 0:
            num_stages_list = [meta['num_stages']]
    device = args.device
    dense_dense_mm_sizes = set()
    target_performance = None
    performance_rtol = 0.01
    best_messages = []
    ...
    def show_best_messages(best_messages=best_messages):
        ...
        for m in best_messages[-10:]:
            print(m)
        sys.stdout.flush()
    for (m, k, n, bm, bk, sparsity) in itertools.product(m_list, k_list, n_list, bm_list, bk_list, sparsity_list):
        k = k or m
        n = n or m
        bk = bk or bm
        if bm > m or bk > k:
        ...
        for op in ops:
            ...
            best_tflops = 0
            for (split_n, num_warps, num_stages, tile_m, tile_n, group_size) in itertools.product(split_n_list, num_warps_list, num_stages_list, tile_m_list, tile_n_list, group_size_list):
                if (tile_m or 0) > bm or (tile_n or 0) > n // (split_n or 1) or n % (split_n or 1) != 0 or ((split_n or 0) > n):
                    continue
                test_func = globals()['test_' + op]
                meta = dict(bsr_scatter_mm6=dict(SPLIT_N=split_n, TILE_M=tile_m, TILE_N=tile_n, GROUP_SIZE=group_size, num_stages=num_stages, num_warps=num_warps), bsr_dense_mm_with_meta=dict(GROUP_SIZE_ROW=group_size, num_stages=num_stages, num_warps=num_warps)).get(op, dict())
                ...
                for r in range(args.repeat):
                    try:
                        ...
                        print(f'op={op}[{meta_str}]({bsr_size},{k}x{n}) dtype={args.dtype} sparsity={sparsity!r}(nnz={x._nnz()}) blocksize={bm}x{bk} OutOfResources', file=outfile)
                        continue
                    except AssertionError:
                        raise
                    except Exception as msg:
                        msg = str(msg).split('\n', 1)[0]
                        print(f'op={op}[{meta_str}]({bsr_size},{k}x{n}) dtype={args.dtype} sparsity={sparsity!r}(nnz={x._nnz()}) blocksize={bm}x{bk} {msg}', file=outfile)
                        continue
                    ...
                    if best_tflops < performance_tflops:
                        ...
                        best_message = f'op={op}[{meta_str}]({bsr_size},x{n}) dtype={args.dtype} sparsity={sparsity:.4f}(nnz={x._nnz()}) blocksize={bm}x{bk} time={time_ms:.3f} ms performance={performance_tflops:.3f} TFLOPS'
                        if best_message not in best_messages:
                            best_messages.append(best_message)
                        mark += ' !!!'
                    print(f'op={op}[{meta_str}]({bsr_size},x{n}) dtype={args.dtype} sparsity={sparsity:.4f}(nnz={x._nnz()}) blocksize={bm}x{bk} time={time_ms:.3f} ms performance={performance_tflops:.3f} TFLOPS{mark}', file=outfile)
                    outfile.flush()
                if args.repeat > 1:
                    avg_time_ms = sum(time_ms_lst) / len(time_ms_lst)
                    avg_performance_tflops = sum(performance_tflops_lst) / len(performance_tflops_lst)
                    print(f'op={op}[{meta_str}]({bsr_size},{k}x{n}) dtype={args.dtype} sparsity={sparsity!r}(nnz={x._nnz()}) blocksize={bm}x{bk} time={time_ms:.3f} ms performance={performance_tflops:.3f} TFLOPS [AVERAGE]', file=outfile)
                    outfile.flush()
