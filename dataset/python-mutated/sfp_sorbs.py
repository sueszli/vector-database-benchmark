...
class sfp_sorbs(SpiderFootPlugin):
    meta = {'name': 'SORBS', 'summary': 'Query the SORBS database for open relays, open proxies, vulnerable servers, etc.', 'flags': [], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'website': 'http://www.sorbs.net/', 'model': 'FREE_NOAUTH_UNLIMITED', 'references': ['http://www.sorbs.net/information/proxy.shtml', 'http://www.sorbs.net/information/spamfo/', 'http://www.sorbs.net/general/using.shtml'], 'favIcon': 'https://www.google.com/s2/favicons?domain=http://www.sorbs.net/', 'logo': 'http://www.sorbs.net/img/pix.gif', 'description': 'The Spam and Open Relay Blocking System (SORBS) was conceived as an anti-spam project where a daemon would check "on-the-fly", all servers from which it received email to determine if that email was sent via various types of proxy and open-relay servers.\nThe SORBS (Spam and Open Relay Blocking System) provides free access to its DNS-based Block List (DNSBL) to effectively block email from more than 12 million host servers known to disseminate spam, phishing attacks and other forms of malicious email.'}}
    opts = {'netblocklookup': True, 'maxnetblock': 24, 'subnetlookup': True, 'maxsubnet': 24}
    optdescs = {'netblocklookup': 'Look up all IPs on netblocks deemed to be owned by your target for possible blacklisted hosts on the same target subdomain/domain?', 'maxnetblock': 'If looking up owned netblocks, the maximum netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'subnetlookup': 'Look up all IPs on subnets which your target is a part of for blacklisting?', 'maxsubnet': 'If looking up subnets, the maximum subnet size to look up all the IPs within (CIDR value, 24 = /24, 16 = /16, etc.)'}
    results = None
    checks = {'127.0.0.2': 'SORBS - Open HTTP Proxy', '127.0.0.3': 'SORBS - Open SOCKS Proxy', '127.0.0.4': 'SORBS - Open Proxy', '127.0.0.5': 'SORBS - Open SMTP Relay', '127.0.0.6': 'SORBS - Spammer', '127.0.0.7': 'SORBS - Vulnerability exposed to spammers', '127.0.0.8': 'SORBS - Host ignored by SORBS', '127.0.0.9': 'SORBS - Hijacked host', '127.0.0.10': 'SORBS - Dynamic IP address range', '127.0.0.11': 'SORBS - Misconfigured A or MX record', '127.0.0.12': 'SORBS - Host does not send mail', '127.0.0.14': 'SORBS - Network does not contain servers'}
    ...
    def producedEvents(self):
        if False:
            while True:
                i = 10
        return ['BLACKLISTED_IPADDR', 'BLACKLISTED_AFFILIATE_IPADDR', 'BLACKLISTED_SUBNET', 'BLACKLISTED_NETBLOCK', 'MALICIOUS_IPADDR', 'MALICIOUS_AFFILIATE_IPADDR', 'MALICIOUS_NETBLOCK', 'MALICIOUS_SUBNET', 'PROXY_HOST']
    ...
    def handleEvent(self, event):
        ...
        for addr in addrs:
            ...
            for result in res:
                ...
                if k in ['127.0.0.2', '127.0.0.3', '127.0.0.4']:
                    evt = SpiderFootEvent('PROXY_HOST', eventData, self.__name__, event)
                    self.notifyListeners(evt)
                if k not in ['127.0.0.8', '127.0.0.10', '127.0.0.11', '127.0.0.12', '127.0.0.14']:
