...
class ExplicitTeamMemberSerializer(serializers.ModelSerializer, UserPermissionsSerializerMixin):
    ...
    class Meta:
        ...
        fields = ['id', 'level', 'parent_level', 'parent_membership_id', 'joined_at', 'updated_at', 'user', 'user_uuid', 'effective_level']
...
class ExplicitTeamMemberViewSet(StructuredViewSetMixin, viewsets.ModelViewSet):
    ...
    queryset = ExplicitTeamMembership.objects.filter(parent_membership__user__is_active=True).select_related('team', 'parent_membership', 'parent_membership__user')
