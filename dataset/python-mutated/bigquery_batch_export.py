...
async def insert_into_bigquery_activity(inputs: BigQueryInsertInputs):
    ...
    async with get_client() as client:
        ...
        table_schema = [bigquery.SchemaField('uuid', 'STRING'), bigquery.SchemaField('event', 'STRING'), bigquery.SchemaField('properties', 'STRING'), bigquery.SchemaField('elements', 'STRING'), bigquery.SchemaField('set', 'STRING'), bigquery.SchemaField('set_once', 'STRING'), bigquery.SchemaField('distinct_id', 'STRING'), bigquery.SchemaField('team_id', 'INT64'), bigquery.SchemaField('ip', 'STRING'), bigquery.SchemaField('site_url', 'STRING'), bigquery.SchemaField('timestamp', 'TIMESTAMP'), bigquery.SchemaField('bq_ingested_timestamp', 'TIMESTAMP')]
        json_columns = ('properties', 'elements', 'set', 'set_once')
        with bigquery_client(inputs) as bq_client:
            bigquery_table = create_table_in_bigquery(inputs.project_id, inputs.dataset_id, inputs.table_id, table_schema, bq_client)
            with BatchExportTemporaryFile() as jsonl_file:
                ...
                for result in results_iterator:
                    row = {field.name: json.dumps(result[field.name]) if field.name in json_columns else result[field.name] for field in table_schema if field.name != 'bq_ingested_timestamp'}
...
class BigQueryBatchExportWorkflow(PostHogWorkflow):
    ...
    async def run(self, inputs: BigQueryBatchExportInputs):
        ...
        insert_inputs = BigQueryInsertInputs(team_id=inputs.team_id, table_id=inputs.table_id, dataset_id=inputs.dataset_id, project_id=inputs.project_id, private_key=inputs.private_key, private_key_id=inputs.private_key_id, token_uri=inputs.token_uri, client_email=inputs.client_email, data_interval_start=data_interval_start.isoformat(), data_interval_end=data_interval_end.isoformat(), exclude_events=inputs.exclude_events, include_events=inputs.include_events)
        await execute_batch_export_insert_activity(insert_into_bigquery_activity, insert_inputs, non_retryable_error_types=['Forbidden', 'RefreshError', 'NotFound'], update_inputs=update_inputs, heartbeat_timeout_seconds=None)