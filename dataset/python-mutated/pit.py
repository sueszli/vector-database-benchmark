...
class P(ElemOperator):
    ...
    def _load_internal(self, instrument, start_index, end_index, freq):
        ...
        for cur_index in range(start_index, end_index + 1):
            ...
            if end_ws > 0:
                raise ValueError("PIT database does not support referring to future period (e.g. expressions like `Ref('$$roewa_q', -1)` are not supported")
            try:
        ...
        resample_series = pd.Series(resample_data, index=pd.RangeIndex(start_index, end_index + 1), dtype='float32', name=str(self))
