...
class MockLNWallet(Logger, EventListener, NetworkRetryManager[LNPeerAddr]):
    ...
    MPP_SPLIT_PART_FRACTION = 1
    MPP_SPLIT_PART_MINAMT_MSAT = 5000000
    ...
    async def create_routes_from_invoice(self, amount_msat: int, decoded_invoice: LnAddr, *, full_path=None):
        paysession = PaySession(payment_hash=decoded_invoice.paymenthash, payment_secret=decoded_invoice.payment_secret, initial_trampoline_fee_level=0, invoice_features=decoded_invoice.get_features(), r_tags=decoded_invoice.get_routing_info('r'), min_final_cltv_delta=decoded_invoice.get_min_final_cltv_delta(), amount_to_pay=amount_msat, invoice_pubkey=decoded_invoice.pubkey.serialize(), uses_trampoline=False, use_two_trampolines=False)
        payment_key = decoded_invoice.paymenthash + decoded_invoice.payment_secret
        self._paysessions[payment_key] = paysession
        return [r async for r in self.create_routes_for_payment(amount_msat=amount_msat, paysession=paysession, full_path=full_path, budget=PaymentFeeBudget.default(invoice_amount_msat=amount_msat))]
...
class PeerInTests(Peer):
    DELAY_INC_MSG_PROCESSING_SLEEP = 0
high_fee_channel = {'local_balance_msat': 10 * bitcoin.COIN * 1000 // 2, 'remote_balance_msat': 10 * bitcoin.COIN * 1000 // 2, 'local_base_fee_msat': 500000, 'local_fee_rate_millionths': 500, 'remote_base_fee_msat': 500000, 'remote_fee_rate_millionths': 500}
low_fee_channel = {'local_balance_msat': 10 * bitcoin.COIN * 1000 // 2, 'remote_balance_msat': 10 * bitcoin.COIN * 1000 // 2, 'local_base_fee_msat': 1000, 'local_fee_rate_millionths': 1, 'remote_base_fee_msat': 1000, 'remote_fee_rate_millionths': 1}
depleted_channel = {'local_balance_msat': 0, 'remote_balance_msat': 10 * bitcoin.COIN * 1000, 'local_base_fee_msat': 1000, 'local_fee_rate_millionths': 1, 'remote_base_fee_msat': 1000, 'remote_fee_rate_millionths': 1}
_GRAPH_DEFINITIONS = {'square_graph': {'alice': {'channels': {'bob': high_fee_channel.copy(), 'carol': low_fee_channel.copy()}}, 'bob': {'channels': {'dave': high_fee_channel.copy()}, 'config': {SimpleConfig.EXPERIMENTAL_LN_FORWARD_PAYMENTS: True, SimpleConfig.EXPERIMENTAL_LN_FORWARD_TRAMPOLINE_PAYMENTS: True}}, 'carol': {'channels': {'dave': low_fee_channel.copy()}, 'config': {SimpleConfig.EXPERIMENTAL_LN_FORWARD_PAYMENTS: True, SimpleConfig.EXPERIMENTAL_LN_FORWARD_TRAMPOLINE_PAYMENTS: True}}, 'dave': {}}}
...
class TestPeerDirect(TestPeer):
    ...
    async def test_reestablish(self):
        ...
        async def reestablish():
            await asyncio.gather(p1.reestablish_channel(alice_channel), p2.reestablish_channel(bob_channel))
    ...
    async def test_reestablish_with_old_state(self):
        ...
        (alice_channel_0, bob_channel_0) = create_test_channels(random_seed=random_seed)
        (p1, p2, w1, w2, _q1, _q2) = self.prepare_peers(alice_channel, bob_channel)
        ...
        async def reestablish():
            await asyncio.gather(p1.reestablish_channel(alice_channel_0), p2.reestablish_channel(bob_channel))
    ...
    async def test_reestablish_replay_messages_rev_then_sig(self):
        ...
        async def f():
            ...
            async with OldTaskGroup() as group:
                ...
                p2.transport.queue.put_nowait(asyncio.Event())
            ...
            async with OldTaskGroup() as group:
                ...
                self.assertTrue(any(('alice->bob' in msg and 'replaying a revoke_and_ack first' in msg for msg in logs.output)))
                self.assertTrue(any(('alice->bob' in msg and "replayed 2 unacked messages. ['update_add_htlc', 'commitment_signed']" in msg for msg in logs.output)))
            ...
            async with OldTaskGroup() as group:
                ...
                with self.assertLogs('electrum', level='INFO') as logs:
                    async with OldTaskGroup() as group2:
                        await group2.spawn(p1.reestablish_channel(chan_AB))
                        await group2.spawn(p2.reestablish_channel(chan_BA))
                self.assertTrue(any(('alice->bob' in msg and 'replaying a revoke_and_ack last' in msg for msg in logs.output)))
                self.assertTrue(any(('alice->bob' in msg and "replayed 2 unacked messages. ['update_add_htlc', 'commitment_signed']" in msg for msg in logs.output)))
                self.assertEqual(chan_AB.peer_state, PeerState.GOOD)
                self.assertEqual(chan_BA.peer_state, PeerState.GOOD)
                raise SuccessfulTest()
    ...
    async def _test_simple_payment(self, test_trampoline: bool, test_hold_invoice=False, test_failure=False, test_bundle=False, test_bundle_timeout=False):
        ...
        if test_trampoline:
            ...
            electrum.trampoline._TRAMPOLINE_NODES_UNITTESTS = {'bob': LNPeerAddr(host='127.0.0.1', port=9735, pubkey=w2.node_keypair.pubkey)}
    ...
    async def test_check_invoice_before_payment(self):
        ...
        async def try_paying_some_invoices():
            invoice_features = w2.features.for_invoice() | 1 << 990
    ...
    async def test_payment_race(self):
        ...
        async def pay():
            ...
            shi1 = SentHtlcInfo(route=route1, payment_secret_orig=lnaddr2.payment_secret, payment_secret_bucket=lnaddr2.payment_secret, amount_msat=lnaddr2.get_amount_msat(), bucket_msat=lnaddr2.get_amount_msat(), amount_receiver_msat=lnaddr2.get_amount_msat(), trampoline_fee_level=None, trampoline_route=None)
            await w1.pay_to_route(sent_htlc_info=shi1, paysession=paysession1, min_final_cltv_delta=lnaddr2.get_min_final_cltv_delta())
            ...
            shi2 = SentHtlcInfo(route=route2, payment_secret_orig=lnaddr1.payment_secret, payment_secret_bucket=lnaddr1.payment_secret, amount_msat=lnaddr1.get_amount_msat(), bucket_msat=lnaddr1.get_amount_msat(), amount_receiver_msat=lnaddr1.get_amount_msat(), trampoline_fee_level=None, trampoline_route=None)
            await w2.pay_to_route(sent_htlc_info=shi2, paysession=paysession2, min_final_cltv_delta=lnaddr1.get_min_final_cltv_delta())
    ...
    async def test_payment_recv_mpp_confusion1(self):
        ...
        async def pay():
            ...
            p1.pay(route=route, chan=alice_channel, amount_msat=1000, total_msat=lnaddr1.get_amount_msat(), payment_hash=lnaddr1.paymenthash, min_final_cltv_delta=lnaddr1.get_min_final_cltv_delta(), payment_secret=lnaddr1.payment_secret)
            p1.pay(route=route, chan=alice_channel, amount_msat=lnaddr1.get_amount_msat() - 1000, total_msat=lnaddr1.get_amount_msat(), payment_hash=lnaddr2.paymenthash, min_final_cltv_delta=lnaddr1.get_min_final_cltv_delta(), payment_secret=lnaddr1.payment_secret)
        ...
        (lnaddr1, _pay_req) = self.prepare_invoice(w2, amount_msat=100000000)
        (lnaddr2, _pay_req) = self.prepare_invoice(w2, amount_msat=100000000)
    ...
    async def test_payment_recv_mpp_confusion2(self):
        ...
        async def pay():
            self.assertEqual(PR_UNPAID, w2.get_payment_status(lnaddr1.paymenthash))
            route = (await w1.create_routes_from_invoice(amount_msat=1000, decoded_invoice=lnaddr1))[0][0].route
            p1.pay(route=route, chan=alice_channel, amount_msat=1000, total_msat=2000, payment_hash=lnaddr1.paymenthash, min_final_cltv_delta=lnaddr1.get_min_final_cltv_delta(), payment_secret=lnaddr1.payment_secret)
            p1.pay(route=route, chan=alice_channel, amount_msat=1000, total_msat=lnaddr1.get_amount_msat(), payment_hash=lnaddr1.paymenthash, min_final_cltv_delta=lnaddr1.get_min_final_cltv_delta(), payment_secret=lnaddr1.payment_secret)
            while nhtlc_success + nhtlc_failed < 2:
                await htlc_resolved.wait()
            self.assertEqual(0, nhtlc_success)
    ...
    async def test_modern_shutdown_with_overlap(self):
        await self._test_shutdown(alice_fee=1, bob_fee=200, alice_fee_range={'min_fee_satoshis': 1, 'max_fee_satoshis': 10}, bob_fee_range={'min_fee_satoshis': 10, 'max_fee_satoshis': 300})
    ...
    async def _test_shutdown(self, alice_fee, bob_fee, alice_fee_range=None, bob_fee_range=None):
        ...
        async def pay():
            ...
            p1.pay(route=route, chan=alice_channel, amount_msat=lnaddr.get_amount_msat(), total_msat=lnaddr.get_amount_msat(), payment_hash=lnaddr.paymenthash, min_final_cltv_delta=lnaddr.get_min_final_cltv_delta(), payment_secret=lnaddr.payment_secret)
    ...
    async def test_channel_usage_after_closing(self):
        ...
        async def f():
            shi = SentHtlcInfo(route=route, payment_secret_orig=lnaddr.payment_secret, payment_secret_bucket=lnaddr.payment_secret, amount_msat=amount_msat, bucket_msat=amount_msat, amount_receiver_msat=amount_msat, trampoline_fee_level=None, trampoline_route=None)
            paysession = w1._paysessions[lnaddr.paymenthash + lnaddr.payment_secret]
            pay = w1.pay_to_route(sent_htlc_info=shi, paysession=paysession, min_final_cltv_delta=lnaddr.get_min_final_cltv_delta())
...
class TestPeerForwarding(TestPeer):
    ...
    async def test_payment_multihop_with_preselected_path(self):
        ...
        async def pay(pay_req):
            with self.subTest(msg='bad path: edges do not chain together'):
                path = [PathEdge(start_node=graph.workers['alice'].node_keypair.pubkey, end_node=graph.workers['carol'].node_keypair.pubkey, short_channel_id=graph.channels['alice', 'bob'].short_channel_id), PathEdge(start_node=graph.workers['bob'].node_keypair.pubkey, end_node=graph.workers['dave'].node_keypair.pubkey, short_channel_id=graph.channels['bob', 'dave'].short_channel_id)]
            ...
            with self.subTest(msg='bad path: last node id differs from invoice pubkey'):
                path = [PathEdge(start_node=graph.workers['alice'].node_keypair.pubkey, end_node=graph.workers['bob'].node_keypair.pubkey, short_channel_id=graph.channels['alice', 'bob'].short_channel_id)]
                with self.assertRaises(LNPathInconsistent):
                    await graph.workers['alice'].pay_invoice(pay_req, full_path=path)
            with self.subTest(msg='good path'):
                path = [PathEdge(start_node=graph.workers['alice'].node_keypair.pubkey, end_node=graph.workers['bob'].node_keypair.pubkey, short_channel_id=graph.channels['alice', 'bob'].short_channel_id), PathEdge(start_node=graph.workers['bob'].node_keypair.pubkey, end_node=graph.workers['dave'].node_keypair.pubkey, short_channel_id=graph.channels['bob', 'dave'].short_channel_id)]
                (result, log) = await graph.workers['alice'].pay_invoice(pay_req, full_path=path)
                self.assertTrue(result)
                self.assertEqual([edge.short_channel_id for edge in path], [edge.short_channel_id for edge in log[0].route])
            raise PaymentDone()
    ...
    async def test_payment_multihop_route_around_failure(self):
        ...
        async def pay(lnaddr, pay_req):
            ...
            self.assertTrue(result)
            self.assertEqual(PR_PAID, graph.workers['dave'].get_payment_status(lnaddr.paymenthash))
            self.assertEqual([graph.channels['alice', 'carol'].short_channel_id, graph.channels['carol', 'dave'].short_channel_id], [edge.short_channel_id for edge in log[0].route])
            self.assertEqual([graph.channels['alice', 'bob'].short_channel_id, graph.channels['bob', 'dave'].short_channel_id], [edge.short_channel_id for edge in log[1].route])
            self.assertEqual(OnionFailureCode.TEMPORARY_NODE_FAILURE, log[0].failure_msg.code)
    ...
    async def test_refuse_to_forward_htlc_that_corresponds_to_payreq_we_created(self):
        ...
        async def pay():
            (lnaddr1, pay_req1) = self.prepare_invoice(graph.workers['bob'], amount_msat=100000000000)
            (lnaddr2, pay_req2) = self.prepare_invoice(graph.workers['dave'], amount_msat=100000000, payment_hash=lnaddr1.paymenthash, include_routing_hints=True)
            ...
            with self.subTest(msg='try to make Bob forward in trampoline mode'):
                electrum.trampoline._TRAMPOLINE_NODES_UNITTESTS = {graph.workers['bob'].name: LNPeerAddr(host='127.0.0.1', port=9735, pubkey=graph.workers['bob'].node_keypair.pubkey)}
    ...
    async def test_payment_with_temp_channel_failure_and_liquidity_hints(self):
        ...
        graph_definition['alice']['channels']['carol']['local_balance_msat'] = 200000000
        graph_definition['alice']['channels']['carol']['remote_balance_msat'] = 200000000
        graph_definition['carol']['channels']['dave']['local_balance_msat'] = 50000000
        graph_definition['carol']['channels']['dave']['remote_balance_msat'] = 200000000
        graph_definition['alice']['channels']['bob']['local_balance_msat'] = 200000000
        graph_definition['alice']['channels']['bob']['remote_balance_msat'] = 200000000
        graph_definition['bob']['channels']['dave']['local_balance_msat'] = 200000000
        graph_definition['bob']['channels']['dave']['remote_balance_msat'] = 200000000
        graph = self.prepare_chans_and_peers_in_graph(graph_definition)
        amount_to_pay = 100000000
        peers = graph.peers.values()
    ...
    async def _run_mpp(self, graph, kwargs):
        ...
        self.assertEqual(500000000000, graph.channels['alice', 'bob'].balance(LOCAL))
        self.assertEqual(500000000000, graph.channels['alice', 'carol'].balance(LOCAL))
        amount_to_pay = 600000000000
        peers = graph.peers.values()

        async def pay(attempts=1, alice_uses_trampoline=False, bob_forwarding=True, mpp_invoice=True, disable_trampoline_receiving=False, test_hold_invoice=False, test_failure=False):
    ...
    async def test_payment_multipart_trampoline_e2e(self):
        graph = self.prepare_chans_and_peers_in_graph(self.GRAPH_DEFINITIONS['square_graph'])
        electrum.trampoline._TRAMPOLINE_NODES_UNITTESTS = {graph.workers['bob'].name: LNPeerAddr(host='127.0.0.1', port=9735, pubkey=graph.workers['bob'].node_keypair.pubkey), graph.workers['carol'].name: LNPeerAddr(host='127.0.0.1', port=9735, pubkey=graph.workers['carol'].node_keypair.pubkey)}
        with self.assertRaises(NoPathFound):
    ...
    async def _run_trampoline_payment(self, graph, *, include_routing_hints=True, test_hold_invoice=False, test_failure=False, attempts=2):
        dave_w = graph.workers['dave']
