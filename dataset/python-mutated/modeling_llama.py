...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int]=None):
    if False:
        return 10
    warnings.warn('Calling `transformers.models.llama.modeling_llama._prepare_4d_attention_mask` is deprecated and will be removed in v4.37. Use `transformers.modeling_attn_mask_utils.AttentionMaskConverter._prepare_4d_attention_mask')
...
def _make_causal_mask(input_ids_shape: torch.Size, dtype: torch.dtype, device: torch.device, past_key_values_length: int=0):
    if False:
        i = 10
        return i + 15
    warnings.warn('Calling `transformers.models.llama.modeling_llama._make_causal_mask` is deprecated and will be removed in v4.37. Use `transformers.models.llama.modeling_llama.AttentionMaskConverter._make_causal_mask')
    return AttentionMaskConverter._make_causal_mask(input_ids_shape=input_ids_shape, dtype=dtype, device=device, past_key_values_length=past_key_values_length)
...
class LlamaDecoderLayer(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention(config=config) if not getattr(config, '_flash_attn_2_enabled', False) else LlamaFlashAttention2(config=config)
