...
class Test_cloudtrail_cloudwatch_logging_enabled:
    ...
    def test_no_trails(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info):
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_cloudwatch_logging_enabled.cloudtrail_cloudwatch_logging_enabled.cloudtrail_client', new=Cloudtrail(current_audit_info)):
                from prowler.providers.aws.services.cloudtrail.cloudtrail_cloudwatch_logging_enabled.cloudtrail_cloudwatch_logging_enabled import cloudtrail_cloudwatch_logging_enabled
    ...
    def test_trails_sending_logs_during_and_not_last_day(self):
        ...
        from prowler.providers.aws.services.cloudtrail.cloudtrail_service import Cloudtrail
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()):
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_cloudwatch_logging_enabled.cloudtrail_cloudwatch_logging_enabled.cloudtrail_client', new=Cloudtrail(self.set_mocked_audit_info())) as service_client:
                from prowler.providers.aws.services.cloudtrail.cloudtrail_cloudwatch_logging_enabled.cloudtrail_cloudwatch_logging_enabled import cloudtrail_cloudwatch_logging_enabled
                for trail in service_client.trails:
                    if trail.name == trail_name_us:
                        trail.latest_cloudwatch_delivery_time = datetime.now().replace(tzinfo=timezone.utc)
                    elif trail.name == trail_name_eu:
                        trail.latest_cloudwatch_delivery_time = (datetime.now() - timedelta(days=2)).replace(tzinfo=timezone.utc)
                regions = []
                ...
                for report in result:
                    if report.resource_id == trail_name_us:
                        ...
                        assert search(report.status_extended, f'Single region trail {trail_name_us} has been logging the last 24h.')
                    ...
                    if report.resource_id == trail_name_eu:
                        ...
                        assert search(report.status_extended, f'Single region trail {trail_name_eu} is not logging in the last 24h.')
                        assert report.resource_tags == []
            ...
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_cloudwatch_logging_enabled.cloudtrail_cloudwatch_logging_enabled.cloudtrail_client', new=Cloudtrail(self.set_mocked_audit_info())) as service_client:
                ...
                for report in result:
                    if report.resource_id == trail_name_us:
                        ...
                        assert report.resource_arn == trail_us['TrailARN']
                        assert report.status == 'PASS'
                        assert search(report.status_extended, f'Multiregion trail {trail_name_us} has been logging the last 24h.')
                        assert report.resource_tags == []
            ...
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_cloudwatch_logging_enabled.cloudtrail_cloudwatch_logging_enabled.cloudtrail_client', new=Cloudtrail(self.set_mocked_audit_info())) as service_client:
                ...
                for report in result:
                    if report.resource_id == trail_name_us:
                        ...
                        assert report.resource_arn == trail_us['TrailARN']
                        assert report.status == 'PASS'
                        assert report.status_extended == f'Single region trail {trail_name_us} has been logging the last 24h.'
                        assert report.resource_tags == []
                    if report.resource_id == trail_name_eu:
                        assert report.resource_id == trail_name_eu
                        assert report.resource_arn == trail_eu['TrailARN']
                        assert report.status == 'FAIL'
                        assert report.status_extended == f'Single region trail {trail_name_eu} is not logging in the last 24h or not configured to deliver logs.'
                        assert report.resource_tags == []