"""A mock pyspiel config for testing. Copy of original config_rwneg.py.
"""
import collections
...
def get_config():
    ...
    examples_scenarios = [scenario_a, scenario_b, scenario_c]
    ...
    params = {'num_distinct_actions': num_players * num_tones, 'num_llm_seeds': 2, 'num_players': num_players, 'min_utility': min([float(p.min) for p in payoffs]), 'max_utility': max([float(p.max) for p in payoffs]), 'num_max_replies': 2}
