...
class SimpleCorridor(gym.Env):
    ...
    def step(self, action):
        ...
        return ([self.cur_pos], random.random() * 2 if done else -0.1, done, truncated, {})
if __name__ == '__main__':
    args = parser.parse_args()
    print(f'Running with following CLI options: {args}')
    ray.init(local_mode=args.local_mode)
    config = get_trainable_cls(args.run).get_default_config().environment(SimpleCorridor, env_config={'corridor_length': 5}).framework(args.framework).rollouts(num_rollout_workers=1).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    ...
    if args.no_tune:
        ...
        tuner = tune.Tuner(args.run, param_space=config.to_dict(), run_config=air.RunConfig(stop=stop))
        results = tuner.fit()
        if args.as_test:
