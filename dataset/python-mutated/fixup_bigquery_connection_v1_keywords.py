...
class bigquery_connectionCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_connection': ('parent', 'connection', 'connection_id'), 'delete_connection': ('name',), 'get_connection': ('name',), 'get_iam_policy': ('resource', 'options'), 'list_connections': ('parent', 'page_size', 'page_token'), 'set_iam_policy': ('resource', 'policy', 'update_mask'), 'test_iam_permissions': ('resource', 'permissions'), 'update_connection': ('name', 'connection', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=bigquery_connectionCallTransformer()):
    if False:
        i = 10
        return i + 15
