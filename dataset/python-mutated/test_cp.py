...
def test__render_filenames_undefined_template():
    ...
    template = 'biscuits'
    ret = (path, dest)
    pytest.raises(CommandExecutionError, cp._render_filenames, path, dest, saltenv, template)
...
def test__render_filenames_success():
    ...
    with patch.dict(templates.TEMPLATE_REGISTRY, {'jinja': mock_jinja}):
        with patch('salt.utils.files.fopen', mock_open(read_data=file_data)):
            ...
            assert cp._render_filenames(path, dest, saltenv=None, template=template) == ret
...
def test_push():
    ...
    with patch('salt.modules.cp.os.path', MagicMock(isfile=Mock(return_value=True), wraps=cp.os.path)), patch('salt.modules.cp.os.path', MagicMock(getsize=MagicMock(return_value=10), wraps=cp.os.path)), patch.multiple('salt.modules.cp', _auth=MagicMock(**{'return_value.gen_token.return_value': 'token'}), __opts__={'id': 'abc', 'file_buffer_size': 10}), patch('salt.utils.files.fopen', mock_open(read_data=b'content')) as m_open, patch('salt.channel.client.ReqChannel.factory', MagicMock()) as req_channel_factory_mock:
        ...
        req_channel_factory_mock().__enter__().send.assert_called_once_with(dict(loc=fh_.tell(), cmd='_file_recv', tok='token', path=['saltines', 'test.file'], size=10, data=b'', id='abc'))