...
if __name__ == '__main__':
    ...
    parser.add_argument('--executor_cores', type=int, default=8, help='The number of cores to use on each executor.')
    parser.add_argument('--executor_memory', type=str, default='4g', help='The amount of memory to allocate on each executor.')
    parser.add_argument('--num_executors', type=int, default=2, help='The number of executors to use in the cluster.')
    parser.add_argument('--driver_cores', type=int, default=4, help='The number of cores to use for the driver.')
    parser.add_argument('--driver_memory', type=str, default='4g', help='The amount of memory to allocate for the driver.')
    parser.add_argument('--backend', type=str, default='ray', help='The backend of TF2 Estimator, either ray or spark.')
    parser.add_argument('--lr', default=0.001, type=float, help='The learning rate to train the model.')
    parser.add_argument('--epochs', default=5, type=int, help='The number of epochs to train the model.')
    parser.add_argument('--batch_size', default=8000, type=int, help='The batch size to train the model.')
    parser.add_argument('--model_dir', default='./', type=str, help='The directory to save the trained model.')
    parser.add_argument('--data_dir', type=str, default='./movielens', help='The directory for the movielens data.')
    ...
    if args.cluster_mode == 'local':
        ...
        sc = init_orca_context('yarn-client', cores=args.executor_cores, num_nodes=args.num_executors, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, object_store_memory='10g')
    ...
    if args.backend == 'ray':
        ...
        invalidInputError(False, "backend should be either 'ray' or 'spark', but got " + args.backend)
    ...
    full = FeatureTable.from_pandas(pddf).apply('label', 'label', lambda x: x - 1, 'int')
    ...
    estimator = Estimator.from_keras(model_creator=model_creator, config=config, backend=args.backend, model_dir=args.model_dir)
    estimator.fit(train.df, batch_size=args.batch_size, epochs=args.epochs, feature_cols=['user', 'item'], label_cols=['label'], steps_per_epoch=steps_per_epoch, validation_data=test.df, validation_steps=val_steps)
    result = estimator.evaluate(test.df, batch_size=args.batch_size, feature_cols=['user', 'item'], label_cols=['label'], num_steps=val_steps)
    print('Evaluation results:', result)
    predictions = estimator.predict(test.df, batch_size=args.batch_size, feature_cols=['user', 'item'])
