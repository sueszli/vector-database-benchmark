...
for i in range(num_models):
    net = mx.mod.Module(symbol=loss, data_names=[data_desc.name], label_names=[label_desc.name], context=mx.gpu())
    net.bind(data_shapes=[data_desc], label_shapes=[label_desc])
    net.init_params(initializer=mx.init.Xavier())
    net.init_optimizer(optimizer='adam', optimizer_params={'learning_rate': learning_rate, 'rescale_grad': 1.0}, kvstore=None)
    test_net = mx.mod.Module(symbol=logits, data_names=[data_desc.name], label_names=None, context=mx.gpu())
    test_net.bind(data_shapes=[data_desc], label_shapes=None, for_training=False, grad_req='null', shared_module=net)
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        for (i, net) in enumerate(train_nets):
            net.forward(data_batch=mx.io.DataBatch(data=[nd.array(data_npy)], label=[nd.array(label_npy)]), is_train=True)
...
for i in range(total_batch):
    ...
    for (i, test_net) in enumerate(test_nets):
        test_net.forward(data_batch=mx.io.DataBatch(data=[nd.array(data_npy)], label=None), is_train=False)
