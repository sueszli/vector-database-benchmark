...
step_progress = Progress(TextColumn('  '), TimeElapsedColumn(), TextColumn('[bold purple]{task.fields[action]}'), SpinnerColumn('simpleDots'))
app_steps_progress = Progress(TextColumn('[bold blue]Progress for app {task.fields[name]}: {task.percentage:.0f}%'), BarColumn(), TextColumn('({task.completed} of {task.total} steps done)'))
overall_progress = Progress(TimeElapsedColumn(), BarColumn(), TextColumn('{task.description}'))
progress_group = Group(Panel(Group(current_app_progress, step_progress, app_steps_progress)), overall_progress)
step_actions = ('downloading', 'configuring', 'building', 'installing')
apps = [('one', (2, 1, 4, 2)), ('two', (1, 3, 8, 4)), ('three', (2, 1, 3, 2))]
