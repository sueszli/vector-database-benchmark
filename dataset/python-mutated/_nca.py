...
class NeighborhoodComponentsAnalysis(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    0.961904...
    """
    _parameter_constraints: dict = {'n_components': [Interval(Integral, 1, None, closed='left'), None], 'init': [StrOptions({'auto', 'pca', 'lda', 'identity', 'random'}), np.ndarray], 'warm_start': ['boolean'], 'max_iter': [Interval(Integral, 1, None, closed='left')], 'tol': [Interval(Real, 0, None, closed='left')], 'callback': [callable, None], 'verbose': ['verbose'], 'random_state': ['random_state']}
