...
from ...shipping_zone.utils import create_shipping_method, create_shipping_method_channel_listing, create_shipping_zone
from ...taxes.utils import get_tax_configurations, update_country_tax_rates, update_tax_configuration
from ...utils import assign_permissions
from ...warehouse.utils import create_warehouse
from ..utils import checkout_billing_address_update, checkout_complete, checkout_create, checkout_delivery_method_update, checkout_dummy_payment_create, checkout_shipping_address_update, get_checkout
...
def prepare_tax_configuration(e2e_staff_api_client, channel_slug, shipping_country_code, shipping_country_tax_rate, billing_country_code, billing_country_tax_rate, prices_entered_with_tax):
    if False:
        print('Hello World!')
...
@pytest.mark.e2e
def test_checkout_calculate_simple_tax_based_on_shipping_country_CORE_2001(e2e_staff_api_client, e2e_not_logged_api_client, permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes, permission_manage_shipping, permission_manage_taxes):
    if False:
        print('Hello World!')
    ...
    shipping_country_tax_rate = prepare_tax_configuration(e2e_staff_api_client, channel_slug, shipping_country_code='CZ', shipping_country_tax_rate=21, billing_country_code='DE', billing_country_tax_rate=19, prices_entered_with_tax=True)
    variant_price = '17.77'
    ...
    checkout_data = checkout_create(e2e_not_logged_api_client, lines, channel_slug, email='testEmail@example.com')
    checkout_id = checkout_data['id']
    assert checkout_data['isShippingRequired'] is True
    shipping_address = {'firstName': 'John', 'lastName': 'Muller', 'companyName': 'Saleor Commerce CZ', 'streetAddress1': 'Sluneční 1396', 'streetAddress2': '', 'postalCode': '74784', 'country': 'CZ', 'city': 'Melc', 'phone': '+420722274643', 'countryArea': ''}
    checkout_data = checkout_shipping_address_update(e2e_not_logged_api_client, checkout_id, shipping_address)
    assert len(checkout_data['shippingMethods']) == 1
    shipping_method_id = checkout_data['shippingMethods'][0]['id']
    billing_address = {'firstName': 'John', 'lastName': 'Muller', 'companyName': 'Saleor Commerce DE', 'streetAddress1': 'Potsdamer Platz 47', 'streetAddress2': '', 'postalCode': '85131', 'country': 'DE', 'city': 'Pollenfeld', 'phone': '+498421499469', 'countryArea': ''}
    ...
    calculated_tax = round(float(variant_price) * shipping_country_tax_rate / (100 + shipping_country_tax_rate), 2)
    assert checkout_data['totalPrice']['tax']['amount'] == calculated_tax
    assert checkout_data['totalPrice']['net']['amount'] == float(variant_price) - calculated_tax
    ...
    shipping_tax = round(float(shipping_price) * shipping_country_tax_rate / (100 + shipping_country_tax_rate), 2)
    assert checkout_data['shippingPrice']['tax']['amount'] == shipping_tax
    assert checkout_data['shippingPrice']['net']['amount'] == float(shipping_price) - shipping_tax
