...
"""
__all__ = ('SessionMRI', 'TransactionMRI', 'SpanMRI', 'MRI_SCHEMA_REGEX', 'MRI_EXPRESSION_REGEX', 'ErrorsMRI', 'parse_mri', 'get_available_operations')
import re
from dataclasses import dataclass
from enum import Enum
from typing import List, Optional
from sentry.snuba.metrics.utils import AVAILABLE_GENERIC_OPERATIONS, AVAILABLE_OPERATIONS, OP_REGEX
NAMESPACE_REGEX = '(transactions|errors|issues|sessions|alerts|custom|spans|escalating_issues)'
ENTITY_TYPE_REGEX = '(c|s|d|g|e)'
MRI_NAME_REGEX = '([a-z_]+(?:\\.[a-z_]+)*)'
MRI_SCHEMA_REGEX_STRING = f'(?P<entity>{ENTITY_TYPE_REGEX}):(?P<namespace>{NAMESPACE_REGEX})/(?P<name>{MRI_NAME_REGEX})@(?P<unit>[\\w.]*)'
MRI_SCHEMA_REGEX = re.compile(f'^{MRI_SCHEMA_REGEX_STRING}$')
MRI_EXPRESSION_REGEX = re.compile(f'^{OP_REGEX}\\(({MRI_SCHEMA_REGEX_STRING})\\)$')
...
class TransactionMRI(Enum):
    ...
    MEASUREMENTS_TIME_TO_INITIAL_DISPLAY = 'd:transactions/measurements.time_to_initial_display@millisecond'
    MEASUREMENTS_TIME_TO_FULL_DISPLAY = 'd:transactions/measurements.time_to_full_display@millisecond'
