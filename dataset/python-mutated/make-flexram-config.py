...
import re
import argparse
ocram_regex = '^\\s*ocrm_size\\s*=\\s*(?P<size>.*);'
dtcm_regex = '^\\s*dtcm_size\\s*=\\s*(?P<size>.*);'
itcm_regex = '^\\s*itcm_size\\s*=\\s*(?P<size>.*);'
gpr_base_regex = '^.*IOMUXC_GPR_BASE\\s*\\((?P<base_addr>\\w*)u\\)'
fsl_ram_bank_size_regex = '^.*FSL_FEATURE_FLEXRAM_INTERNAL_RAM_BANK_SIZE\\s*\\((?P<size>\\w*)\\)'
fsl_bank_nbr_regex = '^.*FSL_FEATURE_FLEXRAM_INTERNAL_RAM_TOTAL_BANK_NUMBERS\\s*\\((?P<number>\\w*)\\)'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='make-flexram-ld.py', usage='%(prog)s [options] [command]', description='Evaluate FlexRAM configuration and generate startup code.')
    parser.add_argument('-d', '--defines_file', dest='defines_file', help='Path to MCU defines file', default='../../../lib/nxp_driver/sdk/devices/MIMXRT1021/MIMXRT1021.h')
    parser.add_argument('-f', '--features_file', dest='features_file', help='Path to MCU features file', default='../../../lib/nxp_driver/sdk/devices/MIMXRT1021/MIMXRT1021_features.h')
    parser.add_argument('-l', '--ld_file', dest='linker_file', help='Path to the aggregated linker-script', default='MIMXRT1021.ld')
    parser.add_argument('-c', '--controller', dest='controller', help='Controller name', default='MIMXRT1021')
