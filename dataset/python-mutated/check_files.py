...
PROVIDERS_DOCKER = 'FROM ghcr.io/apache/airflow/main/ci/python3.10\nRUN rm -rf /opt/airflow/airflow/providers\n\n\n# Install providers\n{}\n'
AIRFLOW_DOCKER = 'FROM python:3.8\n\n# Upgrade\nRUN pip install "apache-airflow=={}"\n\n'
DOCKER_UPGRADE = 'FROM apache/airflow:1.10.15\n\n# Install upgrade-check\nRUN pip install "apache-airflow-upgrade-check=={}"\n\n'
AIRFLOW = 'AIRFLOW'
PROVIDERS = 'PROVIDERS'
...
path_option = click.option('--path', '-p', prompt='Path to files', type=str, help='Path to directory where are sources')
version_option = click.option('--version', '-v', prompt='Version', type=str, help='Version of package to verify. For example 1.10.15.rc1, 2021.3.17rc1')
