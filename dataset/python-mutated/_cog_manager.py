...
class CogManager:
    ...
    async def _find_ext_cog(self, name: str) -> ModuleSpec:
        ...
        if not name.isidentifier() or keyword.iskeyword(name):
            raise NoSuchCog(f"No 3rd party module by the name of '{name}' was found in any available path.", name=name)
    ...
    async def _find_core_cog(name: str) -> ModuleSpec:
        ...
        try:
            ...
            if e.name == package + real_name:
                raise NoSuchCog("No core cog by the name of '{}' could be found.".format(name), path=e.path, name=e.name) from e
            raise
        return mod.__spec__
...
class CogManagerUI(commands.Cog):
    ...
    async def paths(self, ctx: commands.Context):
        ...
        msg = _('Install Path: {install_path}\nCore Path: {core_path}\n\n').format(install_path=install_path, core_path=core_path)
        partial = []
    ...
    async def removepath(self, ctx: commands.Context, *path_numbers: positive_int):
        ...
        parts = []
        if valid:
            parts.append(_('The following paths were removed: {paths}').format(paths=humanize_list([inline(str(path)) for path in valid])))
        if invalid:
            parts.append(_('The following path numbers did not exist: {path_numbers}').format(path_numbers=humanize_list([inline(str(path)) for path in invalid])))
        for page in pagify('\n\n'.join(parts), ['\n', ' ']):
            await ctx.send(page)
    ...
    async def installpath(self, ctx: commands.Context, path: Path=None):
        ...
        await ctx.send(_('The bot will install new cogs to the `{}` directory.').format(install_path))
