...
from ipv8.messaging.anonymization.tunnel import CIRCUIT_STATE_READY, CIRCUIT_TYPE_RP_DOWNLOADER, CIRCUIT_TYPE_RP_SEEDER, PEER_FLAG_EXIT_BT
...
from tribler.core.components.bandwidth_accounting.community.bandwidth_accounting_community import BandwidthAccountingCommunity
...
class TestTriblerTunnelCommunity(TestBase):
    ...
    async def test_intro_point_slot(self):
        ...
        self.nodes[0].overlay.send_cell(circuit.peer, EstablishIntroPayload(circuit.circuit_id, int(random() * 2 ** 16), b'', b''))
        await self.deliver_messages()
    ...
    async def test_perform_http_request(self):
        ...
        response = await self.nodes[0].overlay.perform_http_request(('127.0.0.1', http_tracker.port), b'GET /scrape?info_hash=0 HTTP/1.1\r\n\r\n')
        self.assertEqual(response.split(b'\r\n')[0], b'HTTP/1.1 200 OK')
        self.assertEqual(response.split(b'\r\n\r\n')[1], (await http_tracker.handle_scrape_request(Mock(query={'info_hash': '0'}))).body)
    ...
    async def test_perform_http_request_not_allowed(self):
        ...
        with self.assertRaises(AsyncTimeoutError):
            await wait_for(self.nodes[0].overlay.perform_http_request(('127.0.0.1', http_tracker.port), b'GET /scrape?info_hash=0 HTTP/1.1\r\n\r\n'), timeout=0.3)
        await http_tracker.stop()
    ...
    async def test_perform_http_request_no_http_exits(self):
        ...
        with self.assertRaises(RuntimeError):
            await self.nodes[0].overlay.perform_http_request(('127.0.0.1', 0), b'GET /scrape?info_hash=0 HTTP/1.1\r\n\r\n')
    ...
    async def test_perform_http_request_failed(self):
        ...
        await self.introduce_nodes()
        with self.assertRaises(AsyncTimeoutError):
            await wait_for(self.nodes[0].overlay.perform_http_request(('127.0.0.1', 1234), b'GET /scrape?info_hash=0 HTTP/1.1\r\n\r\n'), timeout=0.3)
