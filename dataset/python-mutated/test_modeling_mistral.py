...
from transformers.testing_utils import backend_empty_cache, require_flash_attn, require_torch, require_torch_gpu, slow, torch_device
...
if is_torch_available():
    import torch
    from transformers import MistralForCausalLM, MistralForSequenceClassification, MistralModel
...
class MistralModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (MistralModel, MistralForCausalLM, MistralForSequenceClassification) if is_torch_available() else ()
    all_generative_model_classes = (MistralForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': MistralModel, 'text-classification': MistralForSequenceClassification, 'text-generation': MistralForCausalLM, 'zero-shot': MistralForSequenceClassification} if is_torch_available() else {}
