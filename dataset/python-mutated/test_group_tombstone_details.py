...
class GroupTombstoneDetailsTest(APITestCase):
    ...
    def test_delete(self):
        ...
        path = reverse('sentry-api-0-group-tombstone-details', kwargs={'organization_slug': self.org.slug, 'project_slug': self.project.slug, 'tombstone_id': tombstone.id})
    ...
    def test_dont_delete_from_other_proj(self):
        ...
        self.team = self.create_team(organization=self.org, name='Mariachi Band')
        self.project = self.create_project(organization=self.org, teams=[self.team], name='Bengal')
        self.other_project = self.create_project(organization=self.org, teams=[self.team], name='Snake')
        self.login_as(user=self.user)
        group = self.create_group()
        tombstone = GroupTombstone.objects.create(project_id=group.project_id, level=group.level, message=group.message, culprit=group.culprit, data=group.data, previous_group_id=group.id)
        GroupHash.objects.create(project=group.project, hash='x' * 32, group=group, group_tombstone_id=tombstone.id)
        assert GroupHash.objects.filter(group_tombstone_id=tombstone.id).exists()
        path = reverse('sentry-api-0-group-tombstone-details', kwargs={'organization_slug': self.org.slug, 'project_slug': self.other_project.slug, 'tombstone_id': tombstone.id})
        response = self.client.delete(path)
