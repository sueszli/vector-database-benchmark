...
@pluginmatcher(re.compile('https?://replay\\.gulli\\.fr/(?:Direct|.+/(?P<video_id>VOD\\d+))'))
class Gulli(Plugin):
    LIVE_PLAYER_URL = 'https://replay.gulli.fr/jwplayer/embedstreamtv'
    VOD_PLAYER_URL = 'https://replay.gulli.fr/jwplayer/embed/{0}'
    _playlist_re = re.compile('sources: (\\[.+?\\])', re.DOTALL)
    _vod_video_index_re = re.compile('jwplayer\\(idplayer\\).playlistItem\\((?P<video_index>[0-9]+)\\)')
    _mp4_bitrate_re = re.compile('.*_(?P<bitrate>[0-9]+)\\.mp4')
    _video_schema = validate.Schema(validate.all(validate.transform(lambda x: re.sub('"?file"?:\\s*[\\\'"](.+?)[\\\'"],?', '"file": "\\1"', x, flags=re.DOTALL)), validate.transform(lambda x: re.sub('"?\\w+?"?:\\s*function\\b.*?(?<={).*(?=})', '', x, flags=re.DOTALL)), validate.parse_json(), [validate.Schema({'file': validate.url()})]))
