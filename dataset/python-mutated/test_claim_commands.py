...
STREAM_TYPES = {'video': 1, 'audio': 2, 'image': 3, 'document': 4, 'binary': 5, 'model': 6}
...
class ClaimTestCase(CommandTestCase):
    ...
    image_data = unhexlify(b'89504e470d0a1a0a0000000d49484452000000050000000708020000004fc510b9000000097048597300000b1300000b1301009a9c18000000154944415408d763fcffff3f031260624005d4e603004c45030b5286e9ea0000000049454e44ae426082')
...
class ClaimSearchCommand(ClaimTestCase):
    ...
    async def create_lots_of_streams(self):
        ...
        for j in range(4):
            same_height_claims = []
            for k in range(5):
                claim_tx = await self.stream_create(f'c{j}-{k}', '0.000001', channel_id=self.channel_id, confirm=False)
                same_height_claims.append(claim_tx['outputs'][0]['name'])
                await self.on_transaction_dict(claim_tx)
            claim_tx = await self.stream_create(f'c{j}-6', '0.000001', channel_id=self.channel_id, confirm=True)
            same_height_claims.append(claim_tx['outputs'][0]['name'])
    ...
    async def assertFindsClaims(self, claims, **kwargs):
        ...
        self.assertEqual(len(claims), len(results), f"{[claim['outputs'][0]['name'] for claim in claims]} != {[result['name'] for result in results]}")
        for (claim, result) in zip(claims, results):
            self.assertEqual((claim['txid'], self.get_claim_id(claim)), (result['txid'], result['claim_id']), f"(expected {claim['outputs'][0]['name']}) != (got {result['name']})")
    ...
    async def test_disconnect_on_memory_error(self):
        claim_ids = ['0000000000000000000000000000000000000000'] * 23828
        self.assertListEqual([], await self.claim_search(claim_ids=claim_ids))
        await asyncio.wait_for(self.daemon.jsonrpc_resolve([f'0000000000000000000000000000000000000000{i}' for i in range(30000)]), 30)
        claim_ids = ['0000000000000000000000000000000000000000'] * 33829
    ...
    async def test_basic_claim_search(self):
        ...
        await self.assertFindsClaim(channel2, public_key_id=channel_txo2['value']['public_key_id'])
        ...
        signed2 = await self.stream_create('on-channel-claim', '0.0001', channel_id=channel_id2, allow_duplicate_name=True)
        ...
        await self.assertFindsClaims([], channel=f'@abc#{self.channel_id}', has_channel_signature=True, invalid_channel_signature=True)
        ...
        await self.assertFindsClaims([signed2], channel_ids=[channel_id2, self.channel_id], valid_channel_signature=True, invalid_channel_signature=False)
    ...
    async def test_search_by_channel(self):
        ...
        await match([claim5, claim4, claim3, claim2, claim1], not_channel_ids=['abc123'], valid_channel_signature=True, claim_type='stream')
        await match([claim5, claim4, claim1], not_channel_ids=[chan1_id], valid_channel_signature=True, claim_type='stream')
        await match([claim3, claim2, claim1], not_channel_ids=[chan2_id], valid_channel_signature=True, claim_type='stream')
        ...
        await match([claim5, claim4, claim3, claim2], not_channel_ids=['abc123'], has_channel_signature=True, valid_channel_signature=True)
        await match([claim5, claim4], not_channel_ids=[chan1_id], has_channel_signature=True, valid_channel_signature=True)
        await match([claim3, claim2], not_channel_ids=[chan2_id], has_channel_signature=True, valid_channel_signature=True)
    ...
    async def test_no_source_and_valid_channel_signature_and_media_type(self):
        ...
        paradox_no_source_claims = await self.claim_search(has_no_source=True, valid_channel_signature=True, media_type='video/mp4')
    ...
    async def test_limit_claims_per_channel(self):
        ...
        await match([claim6, claim5, claim4, claim3, claim1], limit_claims_per_channel=2, claim_type='stream')
        await match([claim6, claim5, claim4, claim3, claim2, claim1], limit_claims_per_channel=3, claim_type='stream')
    ...
    async def test_no_duplicates(self):
        ...
        for i in range(10):
            ...
            claims.append(await self.stream_repost(repost_id, f'claim{i}', bid='0.001', channel_id=self.get_claim_id(channel)))
        await match([first, second] + channels, remove_duplicates=True, order_by=['^height'])
        await match(list(reversed(channels)) + [second, first], remove_duplicates=True, order_by=['height'])
        await match([channels[0], claims[0], channels[1], claims[1]] + channels[2:], height='>218', remove_duplicates=True, order_by=['^height'])
        await match(channels[2:][::-1] + [claims[1], channels[1], claims[0], channels[0]], height='>218', limit_claims_per_channel=1, remove_duplicates=True, order_by=['height'])
    ...
    async def test_limit_claims_per_channel_across_sorted_pages(self):
        ...
        await match([first, second, claims[0], claims[1]], page_size=4, limit_claims_per_channel=3, claim_type='stream', order_by=['^height'])
        await match([first, claims[0], claims[1], claims[2]], page_size=4, limit_claims_per_channel=1, claim_type='stream', order_by=['^height'])
        await match([second, claims[3], claims[4], claims[5]], page_size=4, page=2, limit_claims_per_channel=1, claim_type='stream', order_by=['^height'])
        await match([claims[6], claims[7], last], page_size=4, page=3, limit_claims_per_channel=1, claim_type='stream', order_by=['^height'])
        for limit in [None, 0, -1]:
            await match([first, second] + claims + [last], limit_claims_per_channel=limit, claim_type='stream', order_by=['^height'])

    async def test_claim_type_and_media_type_search(self):
        address = await self.account.receiving.get_or_create_usable_address()
        tx = await Transaction.claim_create('unknown', b'{"sources":{"lbry_sd_hash":""}}', 1, address, [self.account], self.account)
    ...
    async def test_search_by_text(self):
        ...
        claim1 = await self.stream_create('the-real-satoshi', title='The Real Satoshi Nakamoto', description='Documentary about the real Satoshi Nakamoto, creator of bitcoin.', tags=['satoshi nakamoto', 'bitcoin', 'documentary'])
        claim2 = await self.stream_create('about-me', channel_id=chan1_id, title='Satoshi Nakamoto Autobiography', description='I am Satoshi Nakamoto and this is my autobiography.', tags=['satoshi nakamoto', 'bitcoin', 'documentary', 'autobiography'])
        claim3 = await self.stream_create('history-of-bitcoin', channel_id=chan2_id, title='History of Bitcoin', description='History of bitcoin and its creator Satoshi Nakamoto.', tags=['satoshi nakamoto', 'bitcoin', 'documentary', 'history'])
        claim4 = await self.stream_create('satoshi-conspiracies', channel_id=chan3_id, title='Satoshi Nakamoto Conspiracies', description='Documentary detailing various conspiracies surrounding Satoshi Nakamoto.', tags=['conspiracies', 'bitcoin', 'satoshi nakamoto'])
        ...
        claim2 = await self.stream_update(self.get_claim_id(claim2), clear_tags=True, tags=['cloud'], title='Satoshi Nakamoto Nography', description='I am Satoshi Nakamoto and this is my nography.')
...
class TransactionOutputCommands(ClaimTestCase):
    ...
    async def test_txo_list_my_input_output_filtering(self):
        ...
        self.assertEqual(['2.9', '5.047662', '1.0', '7.947786', '1.0', '8.973893', '10.0'], [t['amount'] for t in r])
    ...
    async def test_txo_plot(self):
        ...
        self.assertEqual([{'day': '2016-06-25', 'total': '0.6'}], plot)
        plot = await self.txo_plot(type='support', days_back=1)
        self.assertEqual([{'day': '2016-06-24', 'total': '0.9'}, {'day': '2016-06-25', 'total': '0.6'}], plot)
        plot = await self.txo_plot(type='support', days_back=2)
        self.assertEqual([{'day': '2016-06-23', 'total': '0.5'}, {'day': '2016-06-24', 'total': '0.9'}, {'day': '2016-06-25', 'total': '0.6'}], plot)
        ...
        self.assertEqual([{'day': '2016-06-23', 'total': '0.5'}, {'day': '2016-06-24', 'total': '0.9'}], plot)
...
class ClaimCommands(ClaimTestCase):
    ...
    async def test_claim_stream_channel_list_with_resolve(self):
        ...
        self.assertTrue(r[0]['meta']['is_controlling'])
        self.assertTrue(r[1]['meta']['is_controlling'])
        stream_tx = await self.daemon.jsonrpc_stream_create('foo', '1.0', file_path=self.create_upload_file(data=b'hi'))
...
class ChannelCommands(CommandTestCase):
    ...
    async def test_channel_bids(self):
        ...
        with self.assertRaisesRegex(InsufficientFundsError, 'Not enough funds to cover this transaction.'):
    ...
    async def test_setting_channel_fields(self):
        values = {'title': 'Cool Channel', 'description': 'Best channel on LBRY.', 'thumbnail_url': 'https://co.ol/thumbnail.png', 'tags': ['cool', 'awesome'], 'languages': ['en-US'], 'locations': ['US::Manchester'], 'email': 'human@email.com', 'website_url': 'https://co.ol', 'cover_url': 'https://co.ol/cover.png', 'featured': ['cafe']}
        ...
        self.assertEqual(channel, {'public_key': channel['public_key'], 'public_key_id': channel['public_key_id'], **fixed_values})
        tx = await self.out(self.channel_create('@lightchannel'))
        channel = tx['outputs'][0]['value']
        self.assertEqual(channel, {'public_key': channel['public_key'], 'public_key_id': channel['public_key_id']})
        ...
        self.assertEqual(channel, {'public_key': fixed_values['public_key'], 'public_key_id': fixed_values['public_key_id'], 'featured': ['beef']})
        tx = await self.out(self.channel_update(claim_id, **values))
        channel = tx['outputs'][0]['value']
        fixed_values['featured'].insert(0, 'beef')
        ...
        self.assertEqual(tx['outputs'][0]['value'], {'public_key': channel['public_key'], 'public_key_id': channel['public_key_id'], 'title': 'foo', 'email': 'new@email.com'})
...
class StreamCommands(ClaimTestCase):
    ...
    async def test_create_stream_names(self):
        ...
        with self.assertRaisesRegex(Exception, "You already have a stream claim published under the name 'foo'."):
            await self.stream_create('foo')
        with self.assertRaisesRegex(Exception, "Stream names cannot start with '@' symbol."):
            await self.stream_create('@foo')
        self.assertItemCount(await self.daemon.jsonrpc_claim_list(), 1)
        await self.assertBalance(self.account, '8.993893')
    ...
    async def test_publishing_checks_all_accounts_for_channel(self):
        ...
        result = await self.out(self.daemon.jsonrpc_account_send('5.0', await self.daemon.jsonrpc_address_unused(account2_id), blocking=True))
        ...
        with self.assertRaisesRegex(ValueError, "Couldn't find channel with channel_id"):
            await self.stream_create('hovercraft5', '0.1', channel_id=baz_id, channel_account_id=[account1_id])
        with self.assertRaisesRegex(ValueError, "Couldn't find channel with channel_name '@baz'"):
            await self.stream_create('hovercraft5', '0.1', channel_name='@baz', channel_account_id=[account1_id])
        await self.channel_update(baz_id, account_id=account2_id, claim_address=await self.daemon.jsonrpc_address_unused(account1_id))
        await self.stream_create('hovercraft5', '0.1', channel_id=baz_id)
    ...
    async def test_repost(self):
        ...
        tx = await self.stream_repost(claim_id, 'newstuff-again', '1.1', channel_name='@spam', title='repost title', description='repost desc', tags=['tag1', 'tag2'])
        ...
        await self.stream_update(repost_id, title='title 2', description='desc 2', tags=['tag3'])
        repost_resolve = await self.out(self.daemon.jsonrpc_resolve(repost_url))
        repost = repost_resolve[repost_url]
        self.assertEqual(goodies_claim_id, repost['reposted_claim']['signing_channel']['claim_id'])
        ...
        resolved = await self.out(self.daemon.jsonrpc_resolve(['@reposting-goodies/repost-on-channel', 'newstuff-again']))
    ...
    async def test_filtering_channels_for_removing_content(self):
        ...
        bad_content_id = self.get_claim_id(await self.stream_create('bad_content', '0.1', channel_name='@some_channel', tags=['bad']))
        filtering_channel_id = self.get_claim_id(await self.channel_create('@filtering', '0.1'))
        ...
        blocking_channel_id = self.get_claim_id(await self.channel_create('@blocking', '0.1'))
        await self.conductor.spv_node.stop(False)
        await self.conductor.spv_node.start(self.conductor.lbcwallet_node, extraconf={'blocking_channel_ids': [blocking_channel_id], 'filtering_channel_ids': [filtering_channel_id]})
        ...
        bad_channel_id = self.get_claim_id(await self.channel_create('@bad_channel', '0.1', tags=['bad-stuff']))
        worse_content_id = self.get_claim_id(await self.stream_create('worse_content', '0.1', channel_name='@bad_channel', tags=['bad-stuff']))
        ...
        self.assertTrue(filtered['channels'][0]['channel']['short_url'].startswith('lbry://@filtering#'))
        last_result = result
        result = await self.out(self.daemon.jsonrpc_claim_search(any_tags=['bad-stuff'], order_by=['height'], no_totals=True))
        self.assertEqual(result['items'], last_result['items'])
    ...
    async def test_setting_stream_fields(self):
        values = {'title': 'Cool Content', 'description': 'Best content on LBRY.', 'thumbnail_url': 'https://co.ol/thumbnail.png', 'tags': ['cool', 'awesome'], 'languages': ['en'], 'locations': ['US:NH:Manchester:03101:42.990605:-71.460989'], 'author': 'Jules Verne', 'license': 'Public Domain', 'license_url': 'https://co.ol/license', 'release_time': 123456, 'fee_currency': 'usd', 'fee_amount': '2.99', 'fee_address': 'mmCsWAiXMUVecFQ3fVzUwvpT9XFMXno2Ca'}
        fixed_values = values.copy()
        fixed_values['locations'] = [{'country': 'US', 'state': 'NH', 'city': 'Manchester', 'code': '03101', 'latitude': '42.990605', 'longitude': '-71.460989'}]
        fixed_values['thumbnail'] = {'url': fixed_values.pop('thumbnail_url')}
        fixed_values['release_time'] = str(values['release_time'])
        fixed_values['stream_type'] = 'binary'
        fixed_values['source'] = {'hash': '56bf5dbae43f77a63d075b0f2ae9c7c3e3098db93779c7f9840da0f4db9c2f8c8454f4edd1373e2b64ee2e68350d916e', 'media_type': 'application/octet-stream', 'size': '3'}
        fixed_values['fee'] = {'address': fixed_values.pop('fee_address'), 'amount': fixed_values.pop('fee_amount'), 'currency': fixed_values.pop('fee_currency').upper()}
        ...
        stream = tx['outputs'][0]['value']
        self.assertEqual(stream, {'stream_type': 'binary', 'source': {'size': '3', 'media_type': 'application/octet-stream', 'name': stream['source']['name'], 'hash': '56bf5dbae43f77a63d075b0f2ae9c7c3e3098db93779c7f9840da0f4db9c2f8c8454f4edd1373e2b64ee2e68350d916e', 'sd_hash': stream['source']['sd_hash']}})
        ...
        fixed_values['source']['name'] = stream['source']['name']
        fixed_values['source']['sd_hash'] = stream['source']['sd_hash']
        self.assertEqual(stream, {'stream_type': 'binary', 'source': {'size': '3', 'media_type': 'application/octet-stream', 'name': fixed_values['source']['name'], 'hash': '56bf5dbae43f77a63d075b0f2ae9c7c3e3098db93779c7f9840da0f4db9c2f8c8454f4edd1373e2b64ee2e68350d916e', 'sd_hash': fixed_values['source']['sd_hash']}, 'tags': ['blah'], 'languages': ['uk'], 'locations': [{'country': 'UA', 'city': 'Kyiv'}]})
        tx = await self.out(self.stream_update(claim_id, **values))
        stream = tx['outputs'][0]['value']
        fixed_values['tags'].insert(0, 'blah')
        fixed_values['languages'].insert(0, 'uk')
        fixed_values['locations'].insert(0, {'country': 'UA', 'city': 'Kyiv'})
        self.assertEqual(stream, fixed_values)
        tx = await self.out(self.stream_update(claim_id, tags='single', clear_tags=True, languages='pt', clear_languages=True, locations='BR', clear_locations=True))
        txo = tx['outputs'][0]
        ...
        tx = await self.out(self.stream_update(claim_id, file_name='changed_name', file_hash='cafebeef', file_size=42))
        ...
        self.assertEqual(3, len(await self.claim_search(release_time=['>=123097', '<123457'], order_by=['release_time'])))
        self.assertEqual(3, len(await self.claim_search(release_time=['<123457', '>0'], order_by=['release_time'])))
    ...
    async def test_setting_fee_fields(self):
        ...
        tx = await self.stream_update(claim_id, fee_amount='0.99', fee_currency='lbc')
        self.assertEqual(tx['outputs'][0]['value']['fee'], {'amount': '0.99', 'currency': 'LBC', 'address': txo['address']})
        tx = await self.stream_update(claim_id, fee_amount='0.1', fee_currency='usd', fee_address=fee_address)
        self.assertEqual(tx['outputs'][0]['value']['fee'], {'amount': '0.1', 'currency': 'USD', 'address': fee_address})
        tx = await self.stream_update(claim_id, fee_address=txo['address'])
        self.assertEqual(tx['outputs'][0]['value']['fee'], {'amount': '0.1', 'currency': 'USD', 'address': txo['address']})
        tx = await self.stream_update(claim_id, fee_amount='0.2')
        self.assertEqual(tx['outputs'][0]['value']['fee'], {'amount': '0.2', 'currency': 'USD', 'address': txo['address']})
        ...
        tx = await self.stream_update(claim_id, fee_amount='0.99', fee_currency='lbc', replace=True)
        self.assertEqual(tx['outputs'][0]['value']['fee'], {'amount': '0.99', 'currency': 'LBC', 'address': txo['address']})
        tx = await self.stream_update(claim_id, fee_amount='0.1', fee_currency='usd', fee_address=fee_address, replace=True)
        self.assertEqual(tx['outputs'][0]['value']['fee'], {'amount': '0.1', 'currency': 'USD', 'address': fee_address})
        with self.assertRaisesRegex(Exception, 'please specify a fee currency'):
    ...
    async def test_automatic_type_and_metadata_detection_for_image(self):
        ...
        self.assertEqual(txo['value'], {'source': {'size': '99', 'name': txo['value']['source']['name'], 'media_type': 'image/png', 'hash': '6c7df435d412c603390f593ef658c199817c7830ba3f16b7eadd8f99fa50e85dbd0d2b3dc61eadc33fe096e3872d1545', 'sd_hash': txo['value']['source']['sd_hash']}, 'stream_type': 'image', 'image': {'width': 5, 'height': 7}})
    ...
    async def test_automatic_type_and_metadata_detection_for_video(self):
        ...
        self.assertEqual(txo['value'], {'source': {'size': '2299653', 'name': 'ForBiggerEscapes.mp4', 'media_type': 'video/mp4', 'hash': '5f6811c83c1616df06f10bf5309ca61edb5ff949a9c1212ce784602d837bfdfc1c3db1e0580ef7bd1dadde41d8acf315', 'sd_hash': txo['value']['source']['sd_hash']}, 'stream_type': 'video', 'video': {'width': 1280, 'height': 720, 'duration': 15}})

    async def test_overriding_automatic_metadata_detection(self):
        tx = await self.out(self.daemon.jsonrpc_stream_create('chrome', '1.0', file_path=self.video_file_name, width=99, height=88, duration=9))
        txo = tx['outputs'][0]
        self.assertEqual(txo['value'], {'source': {'size': '2299653', 'name': 'ForBiggerEscapes.mp4', 'media_type': 'video/mp4', 'hash': '5f6811c83c1616df06f10bf5309ca61edb5ff949a9c1212ce784602d837bfdfc1c3db1e0580ef7bd1dadde41d8acf315', 'sd_hash': txo['value']['source']['sd_hash']}, 'stream_type': 'video', 'video': {'width': 99, 'height': 88, 'duration': 9}})
    ...
    async def test_update_file_type(self):
        ...
        self.assertEqual(video_txo['value']['video'], {'duration': 15, 'height': 720, 'width': 1280})
    ...
    async def test_replace_mode_preserves_source_and_type(self):
        expected = {'tags': ['blah'], 'languages': ['uk'], 'locations': [{'country': 'UA', 'city': 'Kyiv'}], 'source': {'size': '2299653', 'name': 'ForBiggerEscapes.mp4', 'media_type': 'video/mp4', 'hash': '5f6811c83c1616df06f10bf5309ca61edb5ff949a9c1212ce784602d837bfdfc1c3db1e0580ef7bd1dadde41d8acf315'}, 'stream_type': 'video', 'video': {'width': 1280, 'height': 720, 'duration': 15}}
        channel = await self.channel_create('@chan')
        tx = await self.out(self.daemon.jsonrpc_stream_create('chrome', '1.0', file_path=self.video_file_name, tags='blah', languages='uk', locations='UA::Kyiv', channel_id=self.get_claim_id(channel)))
        ...
        tx = await self.out(self.daemon.jsonrpc_stream_update(txo['claim_id'], title='new title', replace=True))
        txo = tx['outputs'][0]
    ...
    async def test_create_update_and_abandon_stream(self):
        ...
        tx = await self.stream_create(bid='2.5')
        claim_id = self.get_claim_id(tx)
        ...
        await self.stream_update(claim_id, bid='1.0')
        txs = await self.transaction_list()
    ...
    async def test_publish(self):
        ...
        self.assertEqual(r['claim_id'], self.get_claim_id(tx3))
        ...
        tx5 = await self.publish('future-release', bid='0.1', languages='uk-UA', tags=['Anime', 'anime '])
        self.assertItemCount(await self.daemon.jsonrpc_file_list(), 2)
        ...
        await self.publish('future-release', bid='0.1', tags=['Anime', 'anime ', 'psy-trance'], title='Psy will be over 9000!!!')
        self.assertItemCount(await self.daemon.jsonrpc_file_list(), 2)
        claim = await self.resolve('lbry://future-release')
        self.assertEqual(claim['value']['tags'], ['anime', 'psy-trance'])
        self.assertEqual(claim['value']['title'], 'Psy will be over 9000!!!')
        self.assertNotIn('source', claim['value'])
        tx6 = await self.publish('future-release', sd_hash='beef', file_hash='beef', file_name='blah.mp3', tags=['something-else'])
        claim = await self.resolve('lbry://future-release')
...
class SupportCommands(CommandTestCase):
    ...
    async def test_regular_supports_and_tip_supports(self):
        ...
        result = await self.out(self.daemon.jsonrpc_account_send('5.0', await self.daemon.jsonrpc_address_unused(wallet_id='wallet2')))
        ...
        await self.assertBalance(account2, '5.0')
        tip = await self.out(self.daemon.jsonrpc_support_create(claim_id, '1.0', True, account_id=account2.id, wallet_id='wallet2', funding_account_ids=[account2.id], blocking=True))
        ...
        support = await self.out(self.daemon.jsonrpc_support_create(claim_id, '2.0', False, account_id=account2.id, wallet_id='wallet2', funding_account_ids=[account2.id], blocking=True))
...
class CollectionCommands(CommandTestCase):
    ...
    async def test_collections(self):
        claim_ids = [self.get_claim_id(tx) for tx in [await self.stream_create('stream-one'), await self.stream_create('stream-two')]]
