...
if TYPE_CHECKING:
    from litestar.types import LifeSpanReceiveMessage, LifeSpanSendMessage, LifeSpanShutdownEvent, LifeSpanStartupEvent
...
class LifeSpanHandler(Generic[T]):
    ...
    async def wait_startup(self) -> None:
        ...
        if message['type'] not in ('lifespan.startup.complete', 'lifespan.startup.failed'):
            raise RuntimeError(f"Received unexpected ASGI message type. Expected 'lifespan.startup.complete' or 'lifespan.startup.failed'. Got {message['type']!r}")
    ...
    async def wait_shutdown(self) -> None:
        async with self.stream_send:
            ...
            if message['type'] not in ('lifespan.shutdown.complete', 'lifespan.shutdown.failed'):
                raise RuntimeError(f"Received unexpected ASGI message type. Expected 'lifespan.shutdown.complete' or 'lifespan.shutdown.failed'. Got {message['type']!r}")
