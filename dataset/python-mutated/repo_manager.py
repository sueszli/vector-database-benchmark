...
from typing import Any, AsyncContextManager, Awaitable, Dict, Generator, Iterable, List, NamedTuple, Optional, Tuple
import discord
...
DECODE_PARAMS = {'encoding': 'utf-8', 'errors': 'surrogateescape'}
...
class Repo(RepoJSONMixin):
    GIT_CLONE = 'git clone -c credential.helper= -c core.askpass= --recurse-submodules -b {branch} {url} {folder}'
    GIT_CLONE_NO_BRANCH = 'git -c credential.helper= -c core.askpass= clone --recurse-submodules {url} {folder}'
    GIT_CURRENT_BRANCH = 'git -C {path} symbolic-ref --short HEAD'
    GIT_CURRENT_COMMIT = 'git -C {path} rev-parse HEAD'
    GIT_LATEST_COMMIT = 'git -C {path} rev-parse {branch}'
    GIT_HARD_RESET = 'git -C {path} reset --hard origin/{branch} -q'
    GIT_PULL = 'git -c credential.helper= -c core.askpass= -C {path} pull --recurse-submodules -q --ff-only'
    GIT_DIFF_FILE_STATUS = "git -C {path} diff-tree --no-commit-id --name-status -r -z --line-prefix='\t' {old_rev} {new_rev}"
    ...
    GIT_IS_ANCESTOR = 'git -C {path} merge-base --is-ancestor {maybe_ancestor_rev} {descendant_rev}'
    GIT_CHECK_IF_MODULE_EXISTS = 'git -C {path} cat-file -e {rev}:{module_name}/__init__.py'
    GIT_GET_LAST_MODULE_OCCURRENCE_COMMIT = 'git -C {path} log --diff-filter=D --pretty=format:%H -n 1 {descendant_rev} -- {module_name}/__init__.py'
    PIP_INSTALL = '{python} -m pip install -U -t {target_dir} {reqs}'
    MODULE_FOLDER_REGEX = re.compile('(\\w+)\\/')
    AMBIGUOUS_ERROR_REGEX = re.compile('^hint: {3}(?P<rev>[A-Za-z0-9]+) (?P<type>commit|tag) (?P<desc>.+)$', re.MULTILINE)
    ...
    async def convert(cls, ctx: commands.Context, argument: str) -> Repo:
        ...
        if poss_repo is None:
            raise commands.BadArgument(_('Repo by the name "{repo_name}" does not exist.').format(repo_name=argument))
        return poss_repo
    ...
    async def is_ancestor(self, maybe_ancestor_rev: str, descendant_rev: str) -> bool:
        ...
        git_command = ProcessFormatter().format(self.GIT_IS_ANCESTOR, path=self.folder_path, maybe_ancestor_rev=maybe_ancestor_rev, descendant_rev=descendant_rev)
        ...
        raise errors.GitException(f'Git failed to determine if commit {maybe_ancestor_rev} is ancestor of {descendant_rev} for repo at path: {self.folder_path}', git_command)
    ...
    async def _get_file_update_statuses(self, old_rev: str, new_rev: Optional[str]=None) -> Dict[str, str]:
        """
        ...
        if new_rev is None:
            new_rev = self.branch
        git_command = ProcessFormatter().format(self.GIT_DIFF_FILE_STATUS, path=self.folder_path, old_rev=old_rev, new_rev=new_rev)
        p = await self._run(git_command)
        if p.returncode != 0:
            raise errors.GitDiffError(f'Git diff failed for repo at path: {self.folder_path}', git_command)
        stdout = p.stdout.strip(b'\t\n\x00 ').decode(**DECODE_PARAMS).split('\x00\t')
        ret = {}
        for filename in stdout:
            (status, __, filepath) = filename.partition('\x00')
            ret[filepath] = status
        return ret

    async def get_last_module_occurrence(self, module_name: str, descendant_rev: Optional[str]=None) -> Optional[Installable]:
        """
        ...
        p = await self._run(ProcessFormatter().format(self.GIT_CHECK_IF_MODULE_EXISTS, path=self.folder_path, rev=descendant_rev, module_name=module_name), debug_only=True)
        if p.returncode == 0:
            async with self.checkout(descendant_rev):
                return discord.utils.get(self.available_modules, name=module_name)
        git_command = ProcessFormatter().format(self.GIT_GET_LAST_MODULE_OCCURRENCE_COMMIT, path=self.folder_path, descendant_rev=descendant_rev, module_name=module_name)
        p = await self._run(git_command)
        if p.returncode != 0:
            raise errors.GitException(f'Git log failed for repo at path: {self.folder_path}', git_command)
    ...
    async def get_modified_modules(self, old_rev: str, new_rev: Optional[str]=None) -> Tuple[Installable, ...]:
        """
    ...
    async def _get_commit_notes(self, old_rev: str, relative_file_path: str) -> str:
        ...
        """
        git_command = ProcessFormatter().format(self.GIT_LOG, path=self.folder_path, old_rev=old_rev, relative_file_path=relative_file_path)
        p = await self._run(git_command)
        if p.returncode != 0:
            raise errors.GitException(f'An exception occurred while executing git log on this repo: {self.folder_path}', git_command)
    ...
    async def get_full_sha1(self, rev: str) -> str:
        ...
        """
        git_command = ProcessFormatter().format(self.GIT_GET_FULL_SHA1, path=self.folder_path, rev=rev)
        p = await self._run(git_command)
        if p.returncode != 0:
            stderr = p.stderr.decode(**DECODE_PARAMS).strip()
            ambiguous_errors = (f'error: short object ID {rev} is ambiguous\nhint: The candidates are:\n', f'error: short SHA1 {rev} is ambiguous\nhint: The candidates are:\n')
            ...
            if candidates:
                raise errors.AmbiguousRevision(f'Short SHA1 {rev} is ambiguous.', git_command, candidates)
            raise errors.UnknownRevision(f'Revision {rev} cannot be found.', git_command)
        return p.stdout.decode(**DECODE_PARAMS).strip()
    ...
    async def _run(self, *args: Any, valid_exit_codes: Tuple[int, ...]=(0,), debug_only: bool=False, **kwargs: Any) -> CompletedProcess:
        """
        Parameters
        ----------
        ...
        async with self._repo_lock:
            p: CompletedProcess = await asyncio.get_running_loop().run_in_executor(self._executor, functools.partial(sp_run, *args, stdout=PIPE, stderr=PIPE, **kwargs))
    ...
    async def _checkout(self, rev: Optional[str]=None, force_checkout: bool=False) -> None:
        ...
        p = await self._run(git_command)
        if p.returncode != 0:
            raise errors.UnknownRevision(f'Could not checkout to {rev}. This revision may not exist', git_command)
    ...
    async def clone(self) -> Tuple[Installable, ...]:
        ...
        if self.branch is not None:
            git_command = ProcessFormatter().format(self.GIT_CLONE, branch=self.branch, url=self.url, folder=self.folder_path)
        else:
            git_command = ProcessFormatter().format(self.GIT_CLONE_NO_BRANCH, url=self.url, folder=self.folder_path)
        p = await self._run(git_command)
        if p.returncode:
    ...
    async def current_branch(self) -> str:
        ...
        p = await self._run(git_command)
        if p.returncode != 0:
            raise errors.GitException(f'Could not determine current branch at path: {self.folder_path}', git_command)
        return p.stdout.decode(**DECODE_PARAMS).strip()
    ...
    async def latest_commit(self, branch: Optional[str]=None) -> str:
        ...
        if not exists:
            raise errors.MissingGitRepo(f'A git repo does not exist at path: {self.folder_path}')
        git_command = ProcessFormatter().format(self.GIT_LATEST_COMMIT, path=self.folder_path, branch=branch)
        p = await self._run(git_command)
        if p.returncode != 0:
    ...
    async def hard_reset(self, branch: Optional[str]=None) -> None:
        ...
        if not exists:
            raise errors.MissingGitRepo(f'A git repo does not exist at path: {self.folder_path}')
        git_command = ProcessFormatter().format(self.GIT_HARD_RESET, path=self.folder_path, branch=branch)
        p = await self._run(git_command)
        if p.returncode != 0:
            raise errors.HardResetError(f'Some error occurred when trying to execute a hard reset on the repo at the following path: {self.folder_path}', git_command)
    ...
    async def update(self) -> Tuple[str, str]:
        ...
        p = await self._run(git_command)
        if p.returncode != 0:
            raise errors.UpdateError(f'Git pull returned a non zero exit code for the repo located at path: {self.folder_path}', git_command)
        await self._setup_repo()
    ...
    async def install_libraries(self, target_dir: Path, req_target_dir: Path, libraries: Iterable[Installable]=()) -> Tuple[Tuple[InstalledModule, ...], Tuple[Installable, ...]]:
        ...
        if libraries:
            ...
            for lib in libraries:
                if not (await self.install_requirements(cog=lib, target_dir=req_target_dir) and await lib.copy_to(target_dir=target_dir)):
                    failed.append(lib)
                else:
    ...
    async def install_raw_requirements(self, requirements: Iterable[str], target_dir: Path) -> bool:
        ...
        if not requirements:
            return True
        p = await self._run(ProcessFormatter().format(self.PIP_INSTALL, python=executable, target_dir=target_dir, reqs=requirements))
        if p.returncode != 0:
            log.error('Something went wrong when installing the following requirements: {}'.format(', '.join(requirements)))
            return False
        return True
...
class RepoManager:
    ...
    TREE_URL_RE = re.compile('(?P<tree>/tree)/(?P<branch>\\S+)$')
    ...
    async def add_repo(self, url: str, name: str, branch: Optional[str]=None) -> Repo:
        ...
        if self.does_repo_exist(name):
            raise errors.ExistingGitRepo('That repo name you provided already exists. Please choose another.')
        (url, branch) = self._parse_url(url, branch)
        r = Repo(url=url, name=name, branch=branch, commit='', folder_path=self.repos_folder / name)
        await r.clone()
    ...
    async def update_repos(self, repos: Optional[Iterable[Repo]]=None) -> Tuple[Dict[Repo, Tuple[str, str]], List[str]]:
        ...
        for repo in repos:
            try:
                ...
                log.error("Repository '%s' failed to update. URL: '%s' on branch '%s'", repo.name, repo.url, repo.branch, exc_info=err)
