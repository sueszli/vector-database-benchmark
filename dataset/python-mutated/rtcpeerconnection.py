...
from .exceptions import InternalError, InvalidAccessError, InvalidStateError, OperationError
...
from .rtcicetransport import RTCIceCandidate, RTCIceGatherer, RTCIceParameters, RTCIceTransport
from .rtcrtpparameters import RTCRtpCodecCapability, RTCRtpCodecParameters, RTCRtpDecodingParameters, RTCRtpHeaderExtensionParameters, RTCRtpParameters, RTCRtpReceiveParameters, RTCRtpRtxParameters, RTCRtpSendParameters
...
class RTCPeerConnection(AsyncIOEventEmitter):
    ...
    async def createAnswer(self):
        ...
        if self.signalingState not in ['have-remote-offer', 'have-local-pranswer']:
            raise InvalidStateError(f'Cannot create answer in signaling state "{self.signalingState}"')
        ...
        for remote_m in self.__remoteDescription().media:
            if remote_m.kind in ['audio', 'video']:
                ...
                media = create_media_description_for_transceiver(transceiver, cname=self.__cname, direction=and_direction(transceiver.direction, transceiver._offerDirection), mid=transceiver.mid)
                dtlsTransport = transceiver._transport
            else:
                media = create_media_description_for_sctp(self.__sctp, legacy=self._sctpLegacySdp, mid=self.__sctp.mid)
    ...
    async def createOffer(self) -> RTCSessionDescription:
        ...
        for transceiver in self.__transceivers:
            transceiver._codecs = filter_preferred_codecs(CODECS[transceiver.kind][:], transceiver._preferred_codecs)
        ...
        for i in range(max(len(local_media), len(remote_media))):
            ...
            if media_kind in ['audio', 'video']:
                ...
                description.media.append(create_media_description_for_transceiver(transceiver, cname=self.__cname, direction=transceiver.direction, mid=mid))
            elif media_kind == 'application':
                self.__sctp_mline_index = i
                description.media.append(create_media_description_for_sctp(self.__sctp, legacy=self._sctpLegacySdp, mid=mid))
        ...
        for transceiver in filter(lambda x: x.mid is None and (not x.stopped), self.__transceivers):
            ...
            description.media.append(create_media_description_for_transceiver(transceiver, cname=self.__cname, direction=transceiver.direction, mid=allocate_mid(mids)))
        if self.__sctp and self.__sctp.mid is None:
            self.__sctp_mline_index = next_mline_index()
            description.media.append(create_media_description_for_sctp(self.__sctp, legacy=self._sctpLegacySdp, mid=allocate_mid(mids)))
        bundle = sdp.GroupDescription(semantic='BUNDLE', items=[])
        for media in description.media:
            bundle.items.append(media.rtp.muxId)
        description.group.append(bundle)
    ...
    async def getStats(self) -> RTCStatsReport:
        ...
        coros = [x.getStats() for x in self.getSenders()] + [x.getStats() for x in self.getReceivers()]
    ...
    async def setLocalDescription(self, sessionDescription: RTCSessionDescription) -> None:
        ...
        """
        self.__log_debug('setLocalDescription(%s)\n%s', sessionDescription.type, sessionDescription.sdp)
    ...
    async def setRemoteDescription(self, sessionDescription: RTCSessionDescription) -> None:
        ...
        """
        self.__log_debug('setRemoteDescription(%s)\n%s', sessionDescription.type, sessionDescription.sdp)
        description = sdp.SessionDescription.parse(sessionDescription.sdp)
        description.type = sessionDescription.type
        ...
        for (i, media) in enumerate(description.media):
            ...
            if media.kind in ['audio', 'video']:
                ...
                if transceiver.mid is None:
                    ...
                    transceiver._set_mline_index(i)
                common = filter_preferred_codecs(find_common_codecs(CODECS[media.kind], media.rtp.codecs), transceiver._preferred_codecs)
                if not len(common):
                    raise OperationError('Failed to set remote {} description send parameters'.format(media.kind))
                transceiver._codecs = common
                transceiver._headerExtensions = find_common_header_extensions(HEADER_EXTENSIONS[media.kind], media.rtp.headerExtensions)
                direction = reverse_direction(media.direction)
                if description.type in ['answer', 'pranswer']:
                    transceiver._currentDirection = direction
                else:
                    transceiver._offerDirection = direction
                if direction in ['recvonly', 'sendrecv'] and (not transceiver.receiver.track):
                    transceiver.receiver._track = RemoteStreamTrack(kind=media.kind, id=description.webrtc_track_id(media))
                    trackEvents.append(RTCTrackEvent(receiver=transceiver.receiver, track=transceiver.receiver.track, transceiver=transceiver))
                dtlsTransport = transceiver._transport
            ...
            if dtlsTransport is not None:
                ...
                if description.type == 'offer' and (not iceTransport._role_set):
                    ...
                    iceTransport._role_set = True
                if description.type == 'answer':
                    dtlsTransport._set_role(role='server' if media.dtls.role == 'client' else 'client')
        ...
        if bundle and bundle.items:
            ...
            if self.__sctp and self.__sctp.mid in slaveMids and (not self.__sctp._bundled):
            ...
            self.__updateIceConnectionState()
            self.__updateConnectionState()
        coros = [add_remote_candidates(iceTransport, media) for (iceTransport, media) in iceCandidates.items()]
        await asyncio.gather(*coros)
