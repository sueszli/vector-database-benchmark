...
only_int_re = re.compile('^\\d+$')
any_int_re = re.compile('^\\d+')
MONTH_ALPHAS = {'jan': 1, 'feb': 2, 'mar': 3, 'apr': 4, 'may': 5, 'jun': 6, 'jul': 7, 'aug': 8, 'sep': 9, 'oct': 10, 'nov': 11, 'dec': 12}
DAY_OF_WEEK_ALPHAS = {'sun': 0, 'mon': 1, 'tue': 2, 'wed': 3, 'thu': 4, 'fri': 5, 'sat': 6}
ALPHAS = {}
for i in (MONTH_ALPHAS, DAY_OF_WEEK_ALPHAS):
    ALPHAS.update(i)
del i
step_search_re = re.compile('^([^-]+)-([^-/]+)(/(\\d+))?$')
WEEKDAYS_RE_PART = '|'.join(DAY_OF_WEEK_ALPHAS.keys())
MONTHS_RE_PART = '|'.join(MONTH_ALPHAS.keys())
star_or_int_re = re.compile('^(\\d+|\\*)$')
special_day_of_week_or_month_re = re.compile('^(?P<pre>((?P<he>(({WEEKDAYS})(-({WEEKDAYS}))?)'.format(WEEKDAYS=WEEKDAYS_RE_PART) + '|(({MONTHS})(-({MONTHS}))?)|\\w+)#)|l)(?P<last>\\d+)$'.format(MONTHS=MONTHS_RE_PART))
__all__ = ('croniter',)
warn_deprecated('3.10.0', 'buildbot.util.croniter has been deprecated. Use croniter Pypi package as a replacement. Note that croniter assumes that the input times in UTC timezone whereas buildbot.util.croniter assumed that the input times are in local timezone.')

class croniter:
    MONTHS_IN_YEAR = 12
    RANGES = ((0, 59), (0, 23), (1, 31), (1, 12), (0, 6), (0, 59))
    DAYS = (31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31)
    ALPHACONV = ({}, {}, {'l': 'l'}, copy.deepcopy(MONTH_ALPHAS), copy.deepcopy(DAY_OF_WEEK_ALPHAS), {})
