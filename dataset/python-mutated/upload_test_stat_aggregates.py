...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Upload test stat aggregates to Rockset.')
    parser.add_argument('--date', type=datetime.date.fromisoformat, help='Date to upload test stat aggregates for (YYYY-MM-DD). Must be in the last 30 days', required=True)
    ...
    upload_to_s3(bucket_name='torchci-aggregated-stats', key=f'test_data_aggregates/{str(args.date)}', docs=data)