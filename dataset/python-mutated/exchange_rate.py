...
from .i18n import _
from .util import ThreadJob, make_dir, log_exceptions, OldTaskGroup, make_aiohttp_session, resource_path, EventListener, event_listener, to_decimal
from .util import NetworkRetryManager
from .network import Network
from .simple_config import SimpleConfig
from .logging import Logger
CCY_PRECISIONS = {'BHD': 3, 'BIF': 0, 'BYR': 0, 'CLF': 4, 'CLP': 0, 'CVE': 0, 'DJF': 0, 'GNF': 0, 'IQD': 3, 'ISK': 0, 'JOD': 3, 'JPY': 0, 'KMF': 0, 'KRW': 0, 'KWD': 3, 'LYD': 3, 'MGA': 1, 'MRO': 1, 'OMR': 3, 'PYG': 0, 'RWF': 0, 'TND': 3, 'UGX': 0, 'UYI': 0, 'VND': 0, 'VUV': 0, 'XAF': 0, 'XAU': 4, 'XOF': 0, 'XPF': 0, 'BTC': 8, 'LTC': 8, 'XRP': 6, 'ETH': 18}
SPOT_RATE_REFRESH_TARGET = 150
SPOT_RATE_CLOSE_TO_STALE = 450
SPOT_RATE_EXPIRY = 600
...
class ExchangeBase(Logger):
    ...
    async def update_safe(self, ccy: str) -> None:
        try:
            ...
            assert all((isinstance(rate, (Decimal, type(None))) for rate in self._quotes.values())), f'fx rate must be Decimal, got {self._quotes}'
...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(r.replace('BTC', ''), to_decimal(json[r]['last'])) for r in json if r != 'timestamp'])
...
class BitcoinVenezuela(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        rates = [(r, to_decimal(json['BTC'][r])) for r in json['BTC'] if json['BTC'][r] is not None]
        return dict(rates)
...
class CoinCap(ExchangeBase):
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coincap.io', '/v2/assets/bitcoin/history?interval=d1&limit=2000')
        return dict([(datetime.utcfromtimestamp(h['time'] / 1000).strftime('%Y-%m-%d'), str(h['priceUsd'])) for h in history['data']])
...
class CoinDesk(ExchangeBase):
    ...
    async def request_history(self, ccy):
        ...
        query = '/v1/bpi/historical/close.json?start=%s&end=%s' % (start, end)
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(ccy.upper(), to_decimal(d['value'])) for (ccy, d) in json['rates'].items()])
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com', '/api/v3/coins/bitcoin/market_chart?vs_currency=%s&days=max' % ccy)
        return dict([(datetime.utcfromtimestamp(h[0] / 1000).strftime('%Y-%m-%d'), str(h[1])) for h in history['prices']])
...
class Kraken(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict(((k[-3:], to_decimal(v['c'][0])) for (k, v) in json['result'].items()))
...
class Winkdex(ExchangeBase):
    ...
    async def request_history(self, ccy):
        ...
        return dict([(h['timestamp'][:10], str(to_decimal(h['price']) / 100)) for h in history])
...
class Walltime(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('s3.amazonaws.com', '/data-production-walltime-info/production/dynamic/walltime-info.json')
    ...
    async def run(self):
        while True:
            ...
            if not self.is_enabled():
                continue
            if manually_triggered and self.has_history():
            ...
            addr_name = 'spot-urgent' if is_urgent else 'spot'
