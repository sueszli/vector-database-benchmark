...
class ReportStream(BasicAmazonAdsStream, ABC):
    ...
    ERRORS = [(400, 'KDP authors do not have access to Sponsored Brands functionality'), (401, re.compile('^Not authorized to access scope \\d+$')), (400, re.compile('^Tactic T00020 is not supported for report API in marketplace [A-Z\\d]+\\.$')), (400, re.compile('^Tactic T00030 is not supported for report API in marketplace [A-Z\\d]+\\.$')), (406, re.compile('^Report date is too far in the past\\.'))]
