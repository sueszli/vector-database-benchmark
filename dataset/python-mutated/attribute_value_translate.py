...
class AttributeValueTranslate(BaseTranslateMutation):
    ...
    class Arguments:
        id = graphene.ID(required=True, description='AttributeValue ID or AttributeValueTranslatableContent ID.')
        language_code = graphene.Argument(LanguageCodeEnum, required=True, description='Translation language code.')
        input = AttributeValueTranslationInput(required=True, description='Fields required to update attribute value translations.')
