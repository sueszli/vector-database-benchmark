...
class test_random_gam_gridsearch_specific:
    ...
    hyper_parameters = {'scale': [[1, 1, 1], [2, 2, 2]], 'gam_columns': [['C16', 'C17', 'C18']], 'num_knots': [[5, 5, 5], [6, 6, 6]], 'bs': [[2, 3, 2], [0, 2, 3]]}
    manual_gam_models = []
    num_grid_models = 0
        ...
        for scale in self.hyper_parameters['scale']:
            for gam_columns in self.hyper_parameters['gam_columns']:
                for num_knots in self.hyper_parameters['num_knots']:
                    for bs in self.hyper_parameters['bs']:
                        if len(scale) != len(gam_columns) or len(num_knots) != len(gam_columns):
                            continue
                        self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family='gaussian', gam_columns=gam_columns, keep_gam_cols=True, scale=scale, num_knots=num_knots, bs=bs))
    ...
    def match_models(self):
        ...
        for model in self.manual_gam_models:
            ...
            for grid_search_model in self.h2o_model.models:
                if grid_search_model.actual_params['gam_columns'] == gam_columns and grid_search_model.actual_params['scale'] == scale and (bs == grid_search_model.actual_params['bs']) and (grid_search_model.actual_params['num_knots'] == num_knots):
