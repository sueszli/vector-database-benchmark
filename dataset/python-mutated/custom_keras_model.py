...
if __name__ == '__main__':
    ...
    ModelCatalog.register_custom_model('keras_model', MyVisionNetwork if args.use_vision_network else MyKerasModel)
    ModelCatalog.register_custom_model('keras_q_model', MyVisionNetwork if args.use_vision_network else MyKerasQModel)
    ...
    config = get_trainable_cls(args.run).get_default_config().environment('ALE/Breakout-v5' if args.use_vision_network else 'CartPole-v1').framework('tf').callbacks(MyCallbacks).training(model={'custom_model': 'keras_q_model' if args.run == 'DQN' else 'keras_model'}).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    if args.run == 'DQN':
        config = DQNConfig().update_from_dict(config.to_dict()).training(num_steps_sampled_before_learning_starts=0)
