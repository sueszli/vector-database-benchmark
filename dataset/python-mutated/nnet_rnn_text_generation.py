...
model = NeuralNet(layers=[rnn_layer, Dense(X.shape[2]), Activation('softmax')], loss='categorical_crossentropy', optimizer=RMSprop(learning_rate=0.01), metric='accuracy', batch_size=64, max_epochs=1, shuffle=False)
for _ in range(25):
    model.fit(X, y)
