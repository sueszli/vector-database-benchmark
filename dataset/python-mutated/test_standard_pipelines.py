...
from haystack.pipelines import Pipeline, FAQPipeline, DocumentSearchPipeline, MostSimilarDocumentsPipeline, WebQAPipeline, SearchSummarizationPipeline
...
async def test_query_and_indexing_pipeline(samples_path):
    pipeline = Pipeline.load_from_yaml(samples_path / 'pipelines' / 'test.haystack-pipeline.yml', pipeline_name='indexing_pipeline')
    await pipeline._arun(file_paths=samples_path / 'pipelines' / 'sample_pdf_1.pdf')
    pipeline = Pipeline.load_from_yaml(samples_path / 'pipelines' / 'test.haystack-pipeline.yml', pipeline_name='query_pipeline')
    prediction = await pipeline._arun(query='Who made the PDF specification?', params={'Retriever': {'top_k': 2}, 'Reader': {'top_k': 1}})
...
async def test_async_concurrent_complex_pipeline(samples_path):
    ...
    queries = ['How to test module-1?', 'How to test module-2?', 'How to test module-3?', 'How to test module-4?', 'How to test module-5?']
    futures = []
