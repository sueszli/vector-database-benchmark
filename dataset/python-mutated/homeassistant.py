    ...
    async def async_load(self) -> None:
        ...
        for user in data['users']:
            ...
            if (folded := username.casefold()) in seen:
                self.is_legacy = True
                logging.getLogger(__name__).warning("Home Assistant auth provider is running in legacy mode because we detected usernames that are case-insensitiveequivalent. Please change the username: '%s'.", username)
                break
            seen.add(folded)
            if username != username.strip():
                self.is_legacy = True
                logging.getLogger(__name__).warning("Home Assistant auth provider is running in legacy mode because we detected usernames that start or end in a space. Please change the username: '%s'.", username)
                break
        self._data = data
...
class HassAuthProvider(AuthProvider):
    ...
    async def async_validate_login(self, username: str, password: str) -> None:
        ...
        await self.hass.async_add_executor_job(self.data.validate_login, username, password)
    ...
    async def async_change_password(self, username: str, new_password: str) -> None:
        ...
        if self.data is None:
            await self.async_initialize()
            assert self.data is not None
        await self.hass.async_add_executor_job(self.data.change_password, username, new_password)
        await self.data.async_save()
...
class HassLoginFlow(LoginFlow):
    ...
    async def async_step_init(self, user_input: dict[str, str] | None=None) -> FlowResult:
        ...
        if user_input is not None:
            try:
                await cast(HassAuthProvider, self._auth_provider).async_validate_login(user_input['username'], user_input['password'])
            except InvalidAuth:
                errors['base'] = 'invalid_auth'
            if not errors:
                user_input.pop('password')
                return await self.async_finish(user_input)
        return self.async_show_form(step_id='init', data_schema=vol.Schema({vol.Required('username'): str, vol.Required('password'): str}), errors=errors)