...
from homeassistant.const import ATTR_BATTERY_LEVEL, ATTR_GPS_ACCURACY, ATTR_LATITUDE, ATTR_LONGITUDE, STATE_HOME, STATE_NOT_HOME, EntityCategory
...
from .const import ATTR_HOST_NAME, ATTR_IP, ATTR_MAC, ATTR_SOURCE_TYPE, CONNECTED_DEVICE_REGISTERED, DOMAIN, LOGGER, SourceType
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    component = hass.data[DOMAIN] = EntityComponent[BaseTrackerEntity](LOGGER, DOMAIN, hass)
...
class ScannerEntity(BaseTrackerEntity):
    ...
    async def async_internal_added_to_hass(self) -> None:
        ...
        if not self.registry_entry or not self.platform.config_entry or (not self.mac_address) or ((device_entry := self.find_device_entry()) is None) or self.device_info:
            if self.device_info:
        ...
        if self.registry_entry.device_id != device_entry.id:
            self.registry_entry = er.async_get(self.hass).async_update_entity(self.entity_id, device_id=device_entry.id)
        if self.platform.config_entry.entry_id not in device_entry.config_entries:
            dr.async_get(self.hass).async_update_device(device_entry.id, add_config_entry_id=self.platform.config_entry.entry_id)
        await super().async_internal_added_to_hass()
