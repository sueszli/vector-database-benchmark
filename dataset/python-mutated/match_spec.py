...
class MatchSpec(metaclass=MatchSpecType):
    ...
    """
    FIELD_NAMES = ('channel', 'subdir', 'name', 'version', 'build', 'build_number', 'track_features', 'features', 'url', 'md5', 'sha256', 'license', 'license_family', 'fn')
...
class CaseInsensitiveStrMatch(GlobLowerStrMatch):
    ...
    def match(self, other):
        ...
        if self._re_match:
            ...
            return self._raw_value == _other_val
_implementors = {'channel': ChannelMatch, 'name': GlobLowerStrMatch, 'version': VersionSpec, 'build': GlobStrMatch, 'build_number': BuildNumberMatch, 'track_features': FeatureMatch, 'features': FeatureMatch, 'license': CaseInsensitiveStrMatch, 'license_family': CaseInsensitiveStrMatch}