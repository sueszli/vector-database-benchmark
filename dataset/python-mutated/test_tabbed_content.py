...
async def test_tabbed_content_add_before_id():
    ...
    async with TabbedApp().run_test() as pilot:
        ...
        assert tabbed_content.tab_count == 2
        assert tabbed_content.active == 'initial-1'
        assert [tab.id for tab in tabbed_content.query(Tab).results(Tab)] == ['new-1', 'initial-1']
...
async def test_tabbed_content_add_before_pane():
    ...
    async with TabbedApp().run_test() as pilot:
        ...
        assert tabbed_content.tab_count == 1
        assert tabbed_content.active == 'initial-1'
        await tabbed_content.add_pane(TabPane('Added', id='new-1'), before=pilot.app.query_one('TabPane#initial-1', TabPane))
        assert tabbed_content.tab_count == 2
        assert tabbed_content.active == 'initial-1'
        assert [tab.id for tab in tabbed_content.query(Tab).results(Tab)] == ['new-1', 'initial-1']
...
async def test_tabbed_content_add_before_badly():
    ...
    async with TabbedApp().run_test() as pilot:
        ...
        with pytest.raises(Tabs.TabError):
            await tabbed_content.add_pane(TabPane('Added', id='new-1'), before='unknown-1')
...
async def test_tabbed_content_add_after():
    ...
    async with TabbedApp().run_test() as pilot:
        ...
        assert tabbed_content.tab_count == 2
        assert tabbed_content.active == 'initial-1'
        assert [tab.id for tab in tabbed_content.query(Tab).results(Tab)] == ['initial-1', 'new-1']
...
async def test_tabbed_content_add_after_pane():
    ...
    async with TabbedApp().run_test() as pilot:
        ...
        assert tabbed_content.tab_count == 1
        assert tabbed_content.active == 'initial-1'
        await tabbed_content.add_pane(TabPane('Added', id='new-1'), after=pilot.app.query_one('TabPane#initial-1', TabPane))
        await pilot.pause()
        assert tabbed_content.tab_count == 2
        assert tabbed_content.active == 'initial-1'
        assert [tab.id for tab in tabbed_content.query(Tab).results(Tab)] == ['initial-1', 'new-1']
...
async def test_tabbed_content_add_after_badly():
    ...
    async with TabbedApp().run_test() as pilot:
        ...
        assert tabbed_content.active == 'initial-1'
        with pytest.raises(Tabs.TabError):
            await tabbed_content.add_pane(TabPane('Added', id='new-1'), after='unknown-1')
...
async def test_tabbed_content_add_before_and_after():
    ...
    async with TabbedApp().run_test() as pilot:
        ...
        assert tabbed_content.active == 'initial-1'
        with pytest.raises(Tabs.TabError):
            await tabbed_content.add_pane(TabPane('Added', id='new-1'), before='initial-1', after='initial-1')
