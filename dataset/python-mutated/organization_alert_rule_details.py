...
from sentry.api.serializers.models.alert_rule import AlertRuleSerializer, DetailedAlertRuleSerializer
from sentry.api.serializers.rest_framework.project import ProjectField
from sentry.apidocs.constants import RESPONSE_ACCEPTED, RESPONSE_FORBIDDEN, RESPONSE_NOT_FOUND, RESPONSE_UNAUTHORIZED
...
from sentry.incidents.logic import AlreadyDeletedError, delete_alert_rule, get_slack_actions_with_async_lookups
...
class OrganizationAlertRuleDetailsPutSerializer(serializers.Serializer):
    ...
    dataset = serializers.CharField(required=False, help_text='The name of the dataset that this query will be executed on. Valid values are `events`, `transactions`, `metrics`, `sessions`, and `generic-metrics`. Defaults to `events`. See **Metric Alert Rule Types** under [Create a Metric Alert Rule](/api/alerts/create-a-metric-alert-rule-for-an-organization/#metric-alert-rule-types) for valid configurations.')
    queryType = serializers.ChoiceField(required=False, choices=((0, 'event.type:error'), (1, 'event.type:transaction'), (2, 'None')), help_text='The type of query. If no value is provided, `queryType` is set to the default for the specified `dataset.` See **Metric Alert Rule Types** under [Create a Metric Alert Rule](/api/alerts/create-a-metric-alert-rule-for-an-organization/#metric-alert-rule-types) for valid configurations.')
...
class OrganizationAlertRuleDetailsEndpoint(OrganizationAlertRuleEndpoint):
    publish_status = {'DELETE': ApiPublishStatus.PUBLIC, 'GET': ApiPublishStatus.PUBLIC, 'PUT': ApiPublishStatus.PUBLIC}
