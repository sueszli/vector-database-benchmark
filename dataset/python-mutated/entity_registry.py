...
from homeassistant.const import ATTR_DEVICE_CLASS, ATTR_FRIENDLY_NAME, ATTR_ICON, ATTR_RESTORED, ATTR_SUPPORTED_FEATURES, ATTR_UNIT_OF_MEASUREMENT, EVENT_HOMEASSISTANT_START, EVENT_HOMEASSISTANT_STOP, MAX_LENGTH_STATE_DOMAIN, MAX_LENGTH_STATE_ENTITY_ID, STATE_UNAVAILABLE, STATE_UNKNOWN, EntityCategory, Platform
from homeassistant.core import Event, HomeAssistant, callback, split_entity_id, valid_entity_id
...
ENTITY_CATEGORY_VALUE_TO_INDEX: dict[EntityCategory | None, int] = {val: idx for (idx, val) in enumerate(EntityCategory)}
...
DISLAY_DICT_OPTIONAL = (('ai', 'area_id'), ('di', 'device_id'), ('tk', 'translation_key'))
...
class RegistryEntry:
    ...
    options: ReadOnlyEntityOptionsType = attr.ib(default=None, converter=_protect_entity_options)
    ...
    async def async_load(self) -> None:
        ...
        if data is not None:
            for entity in data['entities']:
                ...
                entities[entity['entity_id']] = RegistryEntry(aliases=set(entity['aliases']), area_id=entity['area_id'], capabilities=entity['capabilities'], config_entry_id=entity['config_entry_id'], device_class=entity['device_class'], device_id=entity['device_id'], disabled_by=RegistryEntryDisabler(entity['disabled_by']) if entity['disabled_by'] else None, entity_category=EntityCategory(entity['entity_category']) if entity['entity_category'] else None, entity_id=entity['entity_id'], hidden_by=RegistryEntryHider(entity['hidden_by']) if entity['hidden_by'] else None, icon=entity['icon'], id=entity['id'], has_entity_name=entity['has_entity_name'], name=entity['name'], options=entity['options'], original_device_class=entity['original_device_class'], original_icon=entity['original_icon'], original_name=entity['original_name'], platform=entity['platform'], supported_features=entity['supported_features'], translation_key=entity['translation_key'], unique_id=entity['unique_id'], previous_unique_id=entity['previous_unique_id'], unit_of_measurement=entity['unit_of_measurement'])
            for entity in data['deleted_entities']:
                key = (split_entity_id(entity['entity_id'])[0], entity['platform'], entity['unique_id'])
                deleted_entities[key] = DeletedRegistryEntry(config_entry_id=entity['config_entry_id'], entity_id=entity['entity_id'], id=entity['id'], orphaned_timestamp=entity['orphaned_timestamp'], platform=entity['platform'], unique_id=entity['unique_id'])
