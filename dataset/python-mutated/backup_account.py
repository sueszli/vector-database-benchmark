...
class AccountBackupAutomation(PeriodTaskModelMixin, JMSOrgBaseModel):
    ...
    backup_type = models.CharField(max_length=128, choices=AccountBackupType.choices, default=AccountBackupType.email.value, verbose_name=_('Backup Type'))
    ...
    recipients_part_one = models.ManyToManyField('users.User', related_name='recipient_part_one_plans', blank=True, verbose_name=_('Recipient part one'))
    recipients_part_two = models.ManyToManyField('users.User', related_name='recipient_part_two_plans', blank=True, verbose_name=_('Recipient part two'))
    obj_recipients_part_one = models.ManyToManyField('terminal.ReplayStorage', related_name='obj_recipient_part_one_plans', blank=True, verbose_name=_('Object Storage Recipient part one'))
    obj_recipients_part_two = models.ManyToManyField('terminal.ReplayStorage', related_name='obj_recipient_part_two_plans', blank=True, verbose_name=_('Object Storage Recipient part two'))
    zip_encrypt_password = fields.EncryptCharField(max_length=4096, blank=True, null=True, verbose_name=_('Zip Encrypt Password'))
...
class AccountBackupExecution(OrgModelMixin):
    ...
    snapshot = models.JSONField(encoder=ModelJSONFieldEncoder, default=dict, blank=True, null=True, verbose_name=_('Account backup snapshot'))
    trigger = models.CharField(max_length=128, default=Trigger.manual, choices=Trigger.choices, verbose_name=_('Trigger mode'))
    reason = models.CharField(max_length=1024, blank=True, null=True, verbose_name=_('Reason'))
    is_success = models.BooleanField(default=False, verbose_name=_('Is success'))
    plan = models.ForeignKey('AccountBackupAutomation', related_name='execution', on_delete=models.CASCADE, verbose_name=_('Account backup plan'))
