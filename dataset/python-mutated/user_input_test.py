...
@plugin.command
@lightbulb.option('type', 'The type of task to request.', choices=[hikari.CommandChoice(name=task.split('.')[-1], value=task) for task in TaskRequestType], required=False, default=TaskRequestType.summarize_story, type=str)
...
async def task_thread(ctx: lightbulb.SlashContext):
    ...
    await thread.send(f"Please complete the task.\nSample Task\n\nSelf destruct {format_time(datetime.now() + timedelta(seconds=MAX_TASK_TIME), 'R')}\n")
    try:
        event = await ctx.bot.wait_for(hikari.GuildMessageCreateEvent, timeout=MAX_TASK_TIME, predicate=lambda e: e.author.id == ctx.author.id and e.channel_id == thread.id)
...
async def task_dm(ctx: lightbulb.Context):
    ...
    await ctx.author.send(f"Please complete the task.\nSample Task\n\nSelf destruct {format_time(datetime.now() + timedelta(seconds=MAX_TASK_TIME), 'R')}\n")
    try:
        event = await ctx.bot.wait_for(hikari.DMMessageCreateEvent, timeout=MAX_TASK_TIME, predicate=lambda e: e.author.id == ctx.author.id)
        await ctx.respond(f'Received message: {event.message.content}')
    except asyncio.TimeoutError:
        await ctx.respond('You took too long to respond.')
...
class TaskModal(miru.Modal):
    ...
    response = miru.TextInput(label='Response', placeholder='Enter your response!', required=True, style=hikari.TextInputStyle.PARAGRAPH, row=2)
...
async def task_modal(ctx: lightbulb.SlashContext):
    """Request a task from the backend."""
    view = ModalView(modal_title='Assistant Response', task='Please explain the moon landing to a six year old.', timeout=MAX_TASK_TIME)
    resp = await ctx.respond('Task - Respond to the prompt as if you were the Assistant:', flags=hikari.MessageFlag.EPHEMERAL, components=view)
...
async def rating_task(ctx: lightbulb.SlashContext):
    """Rate stuff."""
    await ctx.respond('List the responses in order of best to worst response (1,2,3,4,5)', flags=hikari.MessageFlag.EPHEMERAL)
    try:
        event = await ctx.bot.wait_for(hikari.MessageCreateEvent, timeout=MAX_TASK_TIME, predicate=lambda e: e.author.id == ctx.author.id)
    except asyncio.TimeoutError:
