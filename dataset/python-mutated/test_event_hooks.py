...
async def test_async_event_hooks():
    ...
    async def on_response(response):
        events.append({'event': 'response', 'headers': dict(response.headers)})
    event_hooks = {'request': [on_request], 'response': [on_response]}
    async with httpx.AsyncClient(event_hooks=event_hooks, transport=httpx.MockTransport(app)) as http:
...
async def test_async_event_hooks_with_redirect():
    ...
    async def on_response(response):
        events.append({'event': 'response', 'headers': dict(response.headers)})
    event_hooks = {'request': [on_request], 'response': [on_response]}
    async with httpx.AsyncClient(event_hooks=event_hooks, transport=httpx.MockTransport(app), follow_redirects=True) as http:
