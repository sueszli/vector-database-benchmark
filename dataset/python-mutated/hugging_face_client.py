...
class HuggingFaceClient(AIClient):
    ...
    async def inference_with_prompt(self, variable_values: Dict[str, str], prompt_template: str, is_json_response: bool=True):
        ...
        data = json.dumps({'inputs': formated_prompt, 'parameters': {'return_full_text': False, 'max_new_tokens': 800, 'num_return_sequences': 1}})
        ...
        if 'error' in response_json:
            if response_json['error'] == 'Bad Gateway':
                raise Exception('Error hugging face endpoint Bad Gateway.                                 Please start hugging face inference endpoint.')
            print(f"Unexpected error from hugging face: {response_json['error']}")
            return ''
        generated_text = response.json()[0]['generated_text'].lstrip('\n')
        if is_json_response:
            generated_text = re.sub('^```json', '', generated_text)
    ...
    async def find_block_params(self, block_description: str):
        ...
        variable_values['block_languages'] = [f'{type.name.lower()}' for type in BlockLanguage]
        variable_values['pipeline_types'] = [f'{type.name.lower()}' for type in PipelineType]
        variable_values['action_types'] = [f'{type.name.lower()}' for type in ActionType]
        variable_values['data_sources'] = [f'{type.name.lower()}' for type in DataSource]
        function_params_response = await self.inference_with_prompt(variable_values, PROMPT_FOR_FUNCTION_PARAMS, is_json_response=True)
