...
def parse_args():
    ...
    parser.add_argument('--n-atoms', type=int, default=101, help='the number of atoms')
    parser.add_argument('--v-min', type=float, default=-100, help='the return lower bound')
    parser.add_argument('--v-max', type=float, default=100, help='the return upper bound')
...
class QNetwork(nn.Module):
    ...
    def __init__(self, env, n_atoms=101, v_min=-100, v_max=100):
        ...
        self.network = nn.Sequential(nn.Linear(np.array(env.single_observation_space.shape).prod(), 120), nn.ReLU(), nn.Linear(120, 84), nn.ReLU(), nn.Linear(84, self.n * n_atoms))
