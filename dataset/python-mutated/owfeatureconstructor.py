...
import numpy as np
from AnyQt.QtWidgets import QSizePolicy, QAbstractItemView, QComboBox, QLineEdit, QHBoxLayout, QVBoxLayout, QStackedWidget, QStyledItemDelegate, QPushButton, QMenu, QListView, QFrame, QLabel, QMessageBox, QGridLayout, QWidget, QCheckBox
...
from Orange.widgets.utils import itemmodels, vartype, ftry, unique_everseen as unique
from Orange.widgets.utils.sql import check_sql_input
from Orange.widgets import report
from Orange.widgets.utils.widgetpreview import WidgetPreview
from Orange.widgets.widget import OWWidget, Msg, Input, Output
from Orange.widgets.utils.concurrent import ConcurrentWidgetMixin, TaskState
FeatureDescriptor = namedtuple('FeatureDescriptor', ['name', 'expression', 'meta'], defaults=[False])
ContinuousDescriptor = namedtuple('ContinuousDescriptor', ['name', 'expression', 'number_of_decimals', 'meta'], defaults=[False])
DateTimeDescriptor = namedtuple('DateTimeDescriptor', ['name', 'expression', 'meta'], defaults=[False])
DiscreteDescriptor = namedtuple('DiscreteDescriptor', ['name', 'expression', 'values', 'ordered', 'meta'], defaults=[False])
StringDescriptor = namedtuple('StringDescriptor', ['name', 'expression', 'meta'], defaults=[True])
...
class FeatureEditor(QFrame):
    ...
    FUNCTIONS = dict(chain([(key, val) for (key, val) in math.__dict__.items() if not key.startswith('_')], [(key, val) for (key, val) in builtins.__dict__.items() if key in {'str', 'float', 'int', 'len', 'abs', 'max', 'min'}]))
...
class DateTimeFeatureEditor(FeatureEditor):
    ExpressionTooltip = FeatureEditor.ExpressionTooltip + 'Result must be a string in ISO-8601 format (e.g. 2019-07-30T15:37:27 or a part thereof),\nor a number of seconds since Jan 1, 1970.'
...
class DiscreteFeatureEditor(FeatureEditor):
    ExpressionTooltip = FeatureEditor.ExpressionTooltip + 'Result must be a string, if values are not explicitly given\nor a zero-based integer indices into a list of values given below.'
...
_VarMap = {DiscreteDescriptor: vartype(Orange.data.DiscreteVariable('d')), ContinuousDescriptor: vartype(Orange.data.ContinuousVariable('c')), DateTimeDescriptor: vartype(Orange.data.TimeVariable('t')), StringDescriptor: vartype(Orange.data.StringVariable('s'))}
...
class OWFeatureConstructor(OWWidget, ConcurrentWidgetMixin):
    name = 'Formula'
    description = 'Construct new features (data columns) from a set of existing features in the input dataset.'
    category = 'Transform'
    ...
    settings_version = 3
    EDITORS = [(ContinuousDescriptor, ContinuousFeatureEditor), (DateTimeDescriptor, DateTimeFeatureEditor), (DiscreteDescriptor, DiscreteFeatureEditor), (StringDescriptor, StringFeatureEditor)]
    ...
    class Warning(OWWidget.Warning):
        renamed_var = Msg('Recently added variable has been renamed, to avoid duplicates.\n')
...
def make_lambda(expression, args, env=None):
    ...
    return finner
__ALLOWED = ['Ellipsis', 'False', 'None', 'True', 'abs', 'all', 'any', 'acsii', 'bin', 'bool', 'bytearray', 'bytes', 'chr', 'complex', 'dict', 'divmod', 'enumerate', 'filter', 'float', 'format', 'frozenset', 'getattr', 'hasattr', 'hash', 'hex', 'id', 'int', 'iter', 'len', 'list', 'map', 'max', 'memoryview', 'min', 'next', 'object', 'oct', 'ord', 'pow', 'range', 'repr', 'reversed', 'round', 'set', 'slice', 'sorted', 'str', 'sum', 'tuple', 'type', 'zip']
__GLOBALS = {name: getattr(builtins, name) for name in __ALLOWED if hasattr(builtins, name)}
__GLOBALS.update({name: getattr(math, name) for name in dir(math) if not name.startswith('_')})
__GLOBALS.update({'normalvariate': random.normalvariate, 'gauss': random.gauss, 'expovariate': random.expovariate, 'gammavariate': random.gammavariate, 'betavariate': random.betavariate, 'lognormvariate': random.lognormvariate, 'paretovariate': random.paretovariate, 'vonmisesvariate': random.vonmisesvariate, 'weibullvariate': random.weibullvariate, 'triangular': random.triangular, 'uniform': random.uniform, 'nanmean': lambda *args: np.nanmean(args), 'nanmin': lambda *args: np.nanmin(args), 'nanmax': lambda *args: np.nanmax(args), 'nansum': lambda *args: np.nansum(args), 'nanstd': lambda *args: np.nanstd(args), 'nanmedian': lambda *args: np.nanmedian(args), 'nancumsum': lambda *args: np.nancumsum(args), 'nancumprod': lambda *args: np.nancumprod(args), 'nanargmax': lambda *args: np.nanargmax(args), 'nanargmin': lambda *args: np.nanargmin(args), 'nanvar': lambda *args: np.nanvar(args), 'mean': lambda *args: np.mean(args), 'std': lambda *args: np.std(args), 'median': lambda *args: np.median(args), 'cumsum': lambda *args: np.cumsum(args), 'cumprod': lambda *args: np.cumprod(args), 'argmax': lambda *args: np.argmax(args), 'argmin': lambda *args: np.argmin(args), 'var': lambda *args: np.var(args)})
