...
def train_mlp(model, sharding_stage, use_pure_bf16=False, accumulate_grad=False, use_main_grad=False, test_scaler=False, use_storage=True):
    if False:
        for i in range(10):
            print('nop')
    ...
    if sharding_stage == 2:
        ...
        if use_storage:
            ...
            buffer_max_size = 0
        model = GroupShardedStage2(model, optimizer, group=group, buffer_max_size=buffer_max_size)
    else:
...
def test_stage2_bf16():
    ...
    o2_losses = train_mlp(mlp2, sharding_stage=2, use_pure_bf16=True, use_main_grad=True)
    ...
    train_mlp(mlp3, sharding_stage=2, use_pure_bf16=True, use_main_grad=True, test_scaler=True)
    mlp4 = MLP()
    mlp4.set_state_dict(state_dict)
    o2_losses_no_storage = train_mlp(mlp4, sharding_stage=2, use_pure_bf16=True, use_main_grad=True, use_storage=False)
    for i in range(len(o2_losses_no_storage)):
        o2_loss_no_storage = paddle.cast(o2_losses_no_storage[i], dtype='float32').detach()
        o2_32_loss = paddle.cast(o2_losses[i], dtype='float32').detach()
    ...
    o1_losses_grad_acc = train_mlp(mlp5, sharding_stage=2, use_pure_bf16=False, accumulate_grad=True)
    o2_losses_grad_acc = train_mlp(mlp6, sharding_stage=2, use_pure_bf16=True, use_main_grad=True, accumulate_grad=True)
