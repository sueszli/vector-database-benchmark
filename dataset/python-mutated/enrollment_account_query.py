...
def main():
    ...
    response = client.query.usage(scope='providers/Microsoft.Billing/billingAccounts/100/enrollmentAccounts/456', parameters={'dataset': {'filter': {'and': [{'or': [{'dimensions': {'name': 'ResourceLocation', 'operator': 'In', 'values': ['East US', 'West Europe']}}, {'tags': {'name': 'Environment', 'operator': 'In', 'values': ['UAT', 'Prod']}}]}, {'dimensions': {'name': 'ResourceGroup', 'operator': 'In', 'values': ['API']}}]}, 'granularity': 'Daily'}, 'timeframe': 'MonthToDate', 'type': 'Usage'})
    print(response)
