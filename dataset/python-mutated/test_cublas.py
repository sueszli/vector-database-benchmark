...
@testing.parameterize(*testing.product({'dtype': ['float32', 'float64', 'complex64', 'complex128'], 'n': [10, 33, 100], 'bs': [None, 1, 10], 'nrhs': [None, 1, 10]}))
class TestBatchedGesv:
    _tol = {'f': 5e-05, 'd': 1e-12}
...
@testing.parameterize(*testing.product({'dtype': ['float32', 'float64', 'complex64', 'complex128'], 'n': [10, 100], 'mode': [None, numpy, cupy]}))
class TestLevel1Functions:
    _tol = {'f': 1e-05, 'd': 1e-12}
...
@testing.parameterize(*testing.product({'dtype': ['float32', 'float64', 'complex64', 'complex128'], 'shape': [(10, 9), (9, 10)], 'trans': ['N', 'T', 'H'], 'order': ['C', 'F'], 'mode': [None, numpy, cupy]}))
class TestGemv:
    _tol = {'f': 1e-05, 'd': 1e-12}
...
@testing.parameterize(*testing.product({'rank': [5, 9], 'band': [0, 1, 3], 'lower': [0, 1], 'order': ['C', 'F'], 'mode': [None, numpy, cupy]}))
class TestSbmv:
    _tol = {'f': 1e-05, 'd': 1e-12}
...
@testing.parameterize(*testing.product({'dtype': ['float32', 'float64', 'complex64', 'complex128'], 'shape': [(10, 9), (9, 10)], 'order': ['C', 'F'], 'mode': [None, numpy, cupy]}))
class TestGer:
    _tol = {'f': 1e-05, 'd': 1e-12}
...
@testing.parameterize(*testing.product({'nk': [(5, 9), (9, 5)], 'transa': ['N', 'T'], 'ordera': ['F', 'C'], 'orderc': ['F', 'C'], 'lower': [0, 1], 'mode': [None, numpy, cupy]}))
class TestSyrk:
    _tol = {'f': 1e-05, 'd': 1e-12}
...
@testing.parameterize(*testing.product({'mnk': [(8, 9, 10), (10, 9, 8)], 'transa': ['N', 'T', 'H'], 'transb': ['N', 'T', 'H'], 'ordera': ['C', 'F'], 'orderb': ['C', 'F'], 'orderc': ['C', 'F'], 'mode': [None, numpy, cupy]}))
...
@testing.parameterize(*testing.product({'shape': [(9, 10), (10, 9)], 'side': ['L', 'R'], 'ordera': ['C', 'F'], 'orderc': ['C', 'F']}))
class TestDgmm:
    _tol = {'f': 1e-05, 'd': 1e-12}
    ...
    def test_dgmm_inplace(self, dtype):
        ...
        cupy.testing.assert_allclose(self.a, ref, rtol=self.tol, atol=self.tol)
    _dgmm_incx_minus_one_hip_skip_condition = [('C', 'F', (9, 10), 'R'), ('C', 'F', (10, 9), 'R'), ('F', 'F', (9, 10), 'L'), ('F', 'F', (10, 9), 'L')]
