...
cat_cols = ['present_media', 'tweet_type', 'language']
...
embedding_cols = ['engaged_with_user_id', 'hashtags', 'present_domains']
len_cols = ['len_hashtags', 'len_domains', 'len_links']
conf = {'spark.network.timeout': '10000000', 'spark.sql.broadcastTimeout': '7200', 'spark.sql.shuffle.partitions': '2000', 'spark.locality.wait': '0s', 'spark.sql.hive.filesourcePartitionFileCacheSize': '4096000000', 'spark.sql.crossJoin.enabled': 'true', 'spark.serializer': 'org.apache.spark.serializer.KryoSerializer', 'spark.kryo.unsafe': 'true', 'spark.kryoserializer.buffer.max': '1024m', 'spark.task.cpus': '1', 'spark.executor.heartbeatInterval': '200s', 'spark.driver.maxResultSize': '40G', 'spark.app.name': 'recsys-train', 'spark.driver.memoryOverhead': '100G', 'spark.executor.memoryOverhead': '100G'}
...
if __name__ == '__main__':
    ...
    parser.add_option('--backend', type=str, default='ray', help='The backend of Orca Estimator, either ray or spark.')
    parser.add_option('--master', type=str, default=None, help='The master url, only used when cluster mode is standalone.')
    parser.add_option('--executor_cores', type=int, default=44, help='The executor core number.')
    parser.add_option('--executor_memory', type=str, default='30g', help='The executor memory.')
    parser.add_option('--num_executor', type=int, default=8, help='The number of executor.')
    ...
    parser.add_option('--learning_rate', '-l', dest='learning_rate', default=0.0001, type=float)
    parser.add_option('--early_stopping', type=int, default=3, dest='early_stopping')
    parser.add_option('--hidden_units', dest='hidden_units', type=str, help='hidden units for deep mlp', default='1024, 1024')
    ...
    if options.cluster_mode == 'local':
        ...
        init_orca_context('standalone', master=options.master, cores=options.executor_cores, num_nodes=options.num_executor, memory=options.executor_memory, driver_cores=options.driver_cores, driver_memory=options.driver_memory, conf=conf)
    elif options.cluster_mode == 'yarn':
        init_orca_context('yarn-client', cores=options.executor_cores, num_nodes=options.num_executor, memory=options.executor_memory, driver_cores=options.driver_cores, driver_memory=options.driver_memory, conf=conf)
    ...
    column_info = ColumnFeatureInfo(wide_base_cols=wide_cols, wide_base_dims=wide_dims, wide_cross_cols=cross_cols, wide_cross_dims=cross_sizes, indicator_cols=indicator_cols, indicator_dims=indicator_sizes, embed_cols=embedding_cols, embed_in_dims=embedding_sizes, embed_out_dims=[8] * len(embedding_cols), continuous_cols=len_cols, label='label')
    config = {'lr': options.learning_rate, 'hidden_units': options.hidden_units, 'column_info': column_info, 'inter_op_parallelism': 1, 'intra_op_parallelism': 40}
    est = Estimator.from_keras(model_creator=model_creator, verbose=True, config=config, backend=options.backend)
    ...
    start = time()
    est.fit(data=train_tbl.df, epochs=options.epochs, batch_size=options.batch_size, callbacks=callbacks, steps_per_epoch=steps, validation_data=test_tbl.df, validation_steps=val_steps, feature_cols=column_info.feature_cols, label_cols=column_info.label_cols)
    end = time()
