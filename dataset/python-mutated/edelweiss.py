...
class Edelweiss(Source):
    ...
    touched_fields = frozenset(['title', 'authors', 'tags', 'pubdate', 'comments', 'publisher', 'identifier:isbn', 'identifier:edelweiss', 'rating'])
...
if __name__ == '__main__':
    from calibre.ebooks.metadata.sources.test import test_identify_plugin, title_test, authors_test, comments_test, pubdate_test
    tests = [({'title': "The Husband's Secret", 'authors': ['Liane Moriarty']}, [title_test("The Husband's Secret", exact=True), authors_test(['Liane Moriarty'])]), ({'identifiers': {'isbn': '9780312621360'}}, [title_test('Flame: A Sky Chasers Novel', exact=True), authors_test(['Amy Kathleen Ryan'])]), ({'identifiers': {'edelweiss': '0321180607'}}, [title_test('XQuery From the Experts:\xa0A Guide to the W3C XML Query Language', exact=True), authors_test(['Howard Katz', 'Don Chamberlin', 'Denise Draper', 'Mary Fernandez', 'Michael Kay', 'Jonathan Robie', 'Michael Rys', 'Jerome Simeon', 'Jim Tivy', 'Philip Wadler']), pubdate_test(2003, 8, 22), comments_test('Jérôme Siméon'), lambda mi: bool(mi.comments and 'No title summary' not in mi.comments)])]
