...
if __name__ == '__main__':
    ...
    Rx = np.array([[1, 0, 0, 0], [0, np.cos(theta), -np.sin(theta), 0], [0, np.sin(theta), np.cos(theta), 0], [0, 0, 0, 1]], dtype=np.float32)
    Tz = np.array([[1, 0, 0, -(0.25 / 2)], [0, 1, 0, 0], [0, 0, 1, 0.425], [0, 0, 0, 1]], dtype=np.float32)
    Rx = Rx.dot(Tz)
    DISP_Y_MAX = 15
    DISP_X_MAX = 640
        ...
        if len(left_candidates[f]) == 0 or len(right_candidates[f]) == 0:
            ...
            for (i_l, c_l) in enumerate(left_candidates[f]):
                ...
                for (j_r, c_r) in enumerate(right_candidates[f]):
                    print(j_r)
                    if abs(c_l[Y] - c_r[Y]) < DISP_Y_MAX and abs(c_l[X] - c_r[X]) < DISP_X_MAX:
