...
class RuleEdit(RuleEditBase):
    ACTION_MAP = OrderedDict((('replace', _('Change')), ('capitalize', _('Capitalize')), ('lower', _('Lower-case')), ('upper', _('Upper-case'))))
...
class Rules(RulesBase):
    ...
    MSG = _('You can specify rules to manipulate author names here. Click the "Add Rule" button below to get started. The rules will be processed in order for every author.')
...
if __name__ == '__main__':
    app = Application([])
    d = RulesDialog()
    d.rules = [{'action': 'replace', 'query': 'alice B & alice bob', 'match_type': 'one_of', 'replace': 'Alice Bob'}]
    d.exec()
