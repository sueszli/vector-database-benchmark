...
from transformers import AutoConfig, AutoTokenizer, EvalPrediction, HfArgumentParser, TFAutoModelForTokenClassification, TFTrainer, TFTrainingArguments
...
class DataTrainingArguments:
    ...
    labels: Optional[str] = field(metadata={'help': 'Path to a file containing all labels. If not specified, CoNLL-2003 labels are used.'})
...
def main():
    ...
    logger.info('n_replicas: %s, distributed training: %s, 16-bits training: %s', training_args.n_replicas, bool(training_args.n_replicas > 1), training_args.fp16)
    ...
    with training_args.strategy.scope():
        model = TFAutoModelForTokenClassification.from_pretrained(model_args.model_name_or_path, from_pt=bool('.bin' in model_args.model_name_or_path), config=config, cache_dir=model_args.cache_dir)
    ...
    def compute_metrics(p: EvalPrediction) -> Dict:
        ...
        return {'precision': precision_score(out_label_list, preds_list), 'recall': recall_score(out_label_list, preds_list), 'f1': f1_score(out_label_list, preds_list)}
    trainer = TFTrainer(model=model, args=training_args, train_dataset=train_dataset.get_dataset() if train_dataset else None, eval_dataset=eval_dataset.get_dataset() if eval_dataset else None, compute_metrics=compute_metrics)
    ...
    if training_args.do_predict:
        test_dataset = TFTokenClassificationDataset(token_classification_task=token_classification_task, data_dir=data_args.data_dir, tokenizer=tokenizer, labels=labels, model_type=config.model_type, max_seq_length=data_args.max_seq_length, overwrite_cache=data_args.overwrite_cache, mode=Split.test)
