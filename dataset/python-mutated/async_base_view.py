...
class AsyncBaseHTTPView(abc.ABC, BaseView[Request], Generic[Request, Response, SubResponse, Context, RootValue]):
    schema: BaseSchema
    ...
    async def execute_operation(self, request: Request, context: Context, root_value: Optional[RootValue]) -> ExecutionResult:
        ...
        assert self.schema
        return await self.schema.execute(request_data.query, root_value=root_value, variable_values=request_data.variables, context_value=context, operation_name=request_data.operation_name, allowed_operation_types=allowed_operation_types)
    ...
    async def parse_multipart(self, request: AsyncHTTPRequestAdapter) -> Dict[str, str]:
        ...
        try:
            return replace_placeholders_with_files(operations, files_map, form_data['files'])
        except KeyError as e:
            raise HTTPException(400, 'File(s) missing in form data') from e
    ...
    async def run(self, request: Request, context: Optional[Context]=UNSET, root_value: Optional[RootValue]=UNSET) -> Response:
        request_adapter = self.request_adapter_class(request)
        ...
        context = await self.get_context(request, response=sub_response) if context is UNSET else context
        root_value = await self.get_root_value(request) if root_value is UNSET else root_value
        assert context
        try:
            result = await self.execute_operation(request=request, context=context, root_value=root_value)
    ...
    async def parse_http_body(self, request: AsyncHTTPRequestAdapter) -> GraphQLRequestData:
