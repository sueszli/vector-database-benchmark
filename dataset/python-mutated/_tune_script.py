...
if __name__ == '__main__':
    ...
    parser.add_argument('--indicator-file', required=False, default='/tmp/tune_cloud_indicator', type=str)
    args = parser.parse_args()
    trainable = str(os.environ.get('TUNE_TRAINABLE', 'function'))
    num_cpus_per_trial = int(os.environ.get('TUNE_NUM_CPUS_PER_TRIAL', '2'))
    run_kwargs = dict(no_syncer=args.no_syncer or False, storage_path=args.storage_path or None, experiment_name=args.experiment_name or 'cloud_test', indicator_file=args.indicator_file, trainable=trainable, num_cpus_per_trial=num_cpus_per_trial)
