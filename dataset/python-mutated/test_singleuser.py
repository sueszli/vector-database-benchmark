...
async def test_singleuser_auth(app, create_user_with_scopes, access_scopes, server_name, expect_success, full_spawn):
    ...
    r.raise_for_status()
    assert urlparse(r.url).path.rstrip('/').endswith(url_path_join(f'/user/{user.name}', spawner.name, spawner.default_url or '/tree'))
    ...
    access_scopes = [s.replace('$server', f'{user.name}/{server_name}') for s in access_scopes]
    ...
    final_path = url_path_join('/user/', user.name, spawner.name, spawner.default_url or '/tree')
...
async def test_disable_user_config(request, app, tmpdir, full_spawn):
    ...
    r = await async_requests.get(url, cookies=cookies)
    r.raise_for_status()
    assert r.url.rstrip('/').endswith(url_path_join('/user/nandy', user.spawner.default_url or '/tree'))
    assert r.status_code == 200
    r = await async_requests.get(url_path_join(public_url(app, user), 'jupyterhub-test-info'), cookies=cookies)
    r.raise_for_status()
    info = r.json()
    pprint(info)
...
async def test_notebook_dir(request, app, tmpdir, user, full_spawn, extension, notebook_dir):
    if extension:
        try:
    ...
    with abs_dir.join('absfile.txt').open('w') as f:
        f.write('absfile\n')
    if notebook_dir:
        expected_root_dir = notebook_dir.replace('ABS', str(abs_dir)).replace('~', str(home_dir))
    else:
    ...
    await app.proxy.add_user(user)
    url = public_url(app, user)
    r = await async_requests.get(url_path_join(public_url(app, user), 'jupyterhub-test-info'), headers=headers)
    r.raise_for_status()
    info = r.json()
    pprint(info)
    assert info['root_dir'] == expected_root_dir
    r = await async_requests.get(url_path_join(public_url(app, user), 'api/contents/'), headers=headers)
    r.raise_for_status()
    ...
    if not notebook_dir or notebook_dir == '~':
        ...
        raise ValueError(f'No contents check for notebook_dir={notebook_dir!r}')
...
@pytest.mark.skipif(IS_JUPYVERSE, reason="jupyverse doesn't implement token authentication")
