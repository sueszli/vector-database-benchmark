...
async def test_send_binary_very_long(writer: Any) -> None:
    ...
    assert writer.transport.write.call_args_list[0][0][0] == b'\x82\x7f\x00\x00\x00\x00\x00\x01\x00\x01'
...
async def test_send_text_masked(protocol: Any, transport: Any) -> None:
    writer = WebSocketWriter(protocol, transport, use_mask=True, random=random.Random(123))
    await writer.send(b'text')
