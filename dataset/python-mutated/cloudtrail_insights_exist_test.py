...
class Test_cloudtrail_insights_exist:
    ...
    def test_no_trails(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info):
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_insights_exist.cloudtrail_insights_exist.cloudtrail_client', new=Cloudtrail(current_audit_info)):
                from prowler.providers.aws.services.cloudtrail.cloudtrail_insights_exist.cloudtrail_insights_exist import cloudtrail_insights_exist
    ...
    def test_trails_with_no_insight_selector(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info):
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_insights_exist.cloudtrail_insights_exist.cloudtrail_client', new=Cloudtrail(current_audit_info)):
                ...
                assert result[0].status_extended == f'Trail {trail_name_us} does not have insight selectors and it is logging.'
    ...
    def test_trails_with_insight_selector(self):
        ...
        cloudtrail_client_us_east_1.start_logging(Name=trail_name_us)
        cloudtrail_client_us_east_1.get_trail_status(Name=trail_name_us)
        cloudtrail_client_us_east_1.put_insight_selectors(TrailName=trail_name_us, InsightSelectors=[{'InsightType': 'ApiErrorRateInsight'}])
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info):
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_insights_exist.cloudtrail_insights_exist.cloudtrail_client', new=Cloudtrail(current_audit_info)):
                from prowler.providers.aws.services.cloudtrail.cloudtrail_insights_exist.cloudtrail_insights_exist import cloudtrail_insights_exist
                check = cloudtrail_insights_exist()
                ...
                assert result[0].status_extended == f'Trail {trail_name_us} has insight selectors and it is logging.'
                assert result[0].resource_id == trail_name_us
                assert result[0].region == 'us-east-1'
                assert result[0].resource_arn == trail_us['TrailARN']
                assert result[0].resource_tags == []