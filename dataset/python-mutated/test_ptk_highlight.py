...
import pytest
from pygments.token import Error, Keyword, Literal, Name, Number, Operator, Punctuation, String, Text
    ...
    def factory(code, tokens):
        ...
        for tk in tokens:
            while tks:
                ...
                break
    return factory
_cases = {'ls': {'ls -al': [(Name.Builtin, 'ls')]}, 'ls-bin': {'/bin/ls -al': [(Name.Builtin, '/bin/ls')]}, 'print': {'print("hello")': [(Name.Builtin, 'print'), (Punctuation, '('), (Literal.String.Double, '"'), (Literal.String.Double, 'hello'), (Literal.String.Double, '"'), (Punctuation, ')'), (Text.Whitespace, '\n')]}, 'invalid-cmd': {'non-existance-cmd -al': [(Name, 'non')], '![non-existance-cmd -al]': [(Error, 'non-existance-cmd')], 'for i in range(10):': [(Keyword, 'for')], '(1, )': [(Punctuation, '('), (Number.Integer, '1')]}, 'multi-cmd': {'cd && cd': [(Name.Builtin, 'cd'), (Operator, '&&'), (Name.Builtin, 'cd')], 'cd || non-existance-cmd': [(Name.Builtin, 'cd'), (Operator, '||'), (Error, 'non-existance-cmd')]}, 'nested': {'echo @("hello")': [(Name.Builtin, 'echo'), (Keyword, '@'), (Punctuation, '('), (String.Double, 'hello'), (Punctuation, ')')], 'print($(cd))': [(Name.Builtin, 'print'), (Punctuation, '('), (Keyword, '$'), (Punctuation, '('), (Name.Builtin, 'cd'), (Punctuation, ')'), (Punctuation, ')'), (Text.Whitespace, '\n')], 'print(![echo "])\\""])': [(Name.Builtin, 'print'), (Punctuation, '('), (Keyword, '!'), (Punctuation, '['), (Name.Builtin, 'echo'), (Text, ' '), (Literal.String.Double, '"])\\""'), (Punctuation, ']'), (Punctuation, ')'), (Text.Whitespace, '\n')]}, 'subproc-args': {'cd 192.168.0.1': [(Text, '192.168.0.1')]}, 'backtick': {'echo g`.*\\w+`': [(String.Affix, 'g'), (String.Backtick, '`'), (String.Regex, '.'), (String.Regex, '*'), (String.Escape, '\\w')]}, 'macro': {'g!(42, *, 65)': [(Name, 'g'), (Keyword, '!'), (Punctuation, '('), (Number.Integer, '42')], 'echo! hello world': [(Name.Builtin, 'echo'), (Keyword, '!'), (String, 'hello world')], 'bash -c ! export var=42; echo $var': [(Name.Builtin, 'bash'), (Text, '-c'), (Keyword, '!'), (String, 'export var=42; echo $var')]}}
