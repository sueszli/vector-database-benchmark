...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='dump mge model for resnet50', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-b', '--batch-size', help='batch size of the model', default=1, type=int)
    parser.add_argument('-d', '--dtype', help='the dtype of the model,which includes float32 and uint8', default='float32', type=str)
    parser.add_argument('--inputs', help='set the inputs data to get a model with testcase', default='', type=str)
    parser.add_argument('--dir', help='set the dir where the model to dump', default='.', type=str)
    parser.add_argument('--enable-nchw4', help='enable-nchw4 for NVIDIA CUDNN', action='store_true')
    parser.add_argument('--enable-chwn4', help='enable-chwn4 for NVIDIA CUDNN', action='store_true')
    ...
    if args.dtype == 'float32' or args.dtype == 'uint8':
        ...
        if args.dtype == 'uint8':
            data_type = np.uint8
        data = tensor((np.random.random([args.batch_size, 3, 224, 224]) * 255).astype(data_type))
        fun(data)
        if args.inputs == '':
            fun.dump(args.dir + '/resnet50_b' + str(args.batch_size) + '_' + args.dtype + '_without_data.mge', arg_names=['data'], no_assert=True, enable_nchw4=args.enable_nchw4, enable_chwn4=args.enable_chwn4)
        else:
            fun.dump(args.dir + '/resnet50_b' + str(args.batch_size) + '_' + args.dtype + '_with_data.mge', arg_names=['data'], input_data=[args.inputs], no_assert=True, enable_nchw4=args.enable_nchw4)
    else:
