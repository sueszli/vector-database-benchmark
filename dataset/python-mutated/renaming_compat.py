...
from picard.config import BoolOption, Option, TextOption, get_config
...
class RenamingCompatOptionsPage(OptionsPage):
    ...
    options = [BoolOption('setting', 'windows_compatibility', True), BoolOption('setting', 'windows_long_paths', system_supports_long_paths() if IS_WIN else False), BoolOption('setting', 'ascii_filenames', False), BoolOption('setting', 'replace_spaces_with_underscores', False), TextOption('setting', 'replace_dir_separator', DEFAULT_REPLACEMENT), Option('setting', 'win_compat_replacements', {'*': DEFAULT_REPLACEMENT, ':': DEFAULT_REPLACEMENT, '<': DEFAULT_REPLACEMENT, '>': DEFAULT_REPLACEMENT, '?': DEFAULT_REPLACEMENT, '|': DEFAULT_REPLACEMENT, '"': DEFAULT_REPLACEMENT})]
...
class WinCompatReplacementValidator(QtGui.QValidator):
    _re_valid_win_replacement = re.compile('^[^"*:<>?|/\\\\\\s]?$')
