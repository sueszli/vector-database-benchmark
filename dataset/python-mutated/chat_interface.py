...
from gradio.components import Button, Chatbot, Component, Markdown, State, Textbox, get_component_instance
...
class ChatInterface(Blocks):
    ...
    async def _submit_fn(self, message: str, history_with_input: list[list[str | None]], request: Request, *args) -> tuple[list[list[str | None]], list[list[str | None]]]:
    ...
    async def _stream_fn(self, message: str, history_with_input: list[list[str | None]], request: Request, *args) -> AsyncGenerator:
        history = history_with_input[:-1]
        (inputs, _, _) = special_args(self.fn, inputs=[message, history, *args], request=request)
        if self.is_async:
    ...
    async def _api_submit_fn(self, message: str, history: list[list[str | None]], request: Request, *args) -> tuple[str, list[list[str | None]]]:
        (inputs, _, _) = special_args(self.fn, inputs=[message, history, *args], request=request)
        if self.is_async:
            response = await self.fn(*inputs)
        else:
    ...
    async def _api_stream_fn(self, message: str, history: list[list[str | None]], request: Request, *args) -> AsyncGenerator:
        (inputs, _, _) = special_args(self.fn, inputs=[message, history, *args], request=request)
        if self.is_async:
            generator = self.fn(*inputs)
        else:
    ...
    async def _examples_stream_fn(self, message: str, *args) -> AsyncGenerator:
        (inputs, _, _) = special_args(self.fn, inputs=[message, [], *args], request=None)
        if self.is_async:
            generator = self.fn(*inputs)
        else:
