...
METRICS = [('Euclidean', distance.Euclidean), ('Manhattan', distance.Manhattan), ('Mahalanobis', distance.Mahalanobis), ('Cosine', distance.Cosine), ('Jaccard', distance.Jaccard), ('Spearman', distance.SpearmanR), ('Absolute Spearman', distance.SpearmanRAbsolute), ('Pearson', distance.PearsonR), ('Absolute Pearson', distance.PearsonRAbsolute)]
...
class OWNeighbors(OWWidget):
    ...
    class Info(OWWidget.Warning):
        removed_references = Msg('Input data includes reference instance(s).\nReference instances are excluded from the output.')

    class Warning(OWWidget.Warning):
        all_data_as_reference = Msg('Every data instance is same as some reference')
...
if __name__ == '__main__':
    iris = Table('iris.tab')
    WidgetPreview(OWNeighbors).run(set_data=iris, set_ref=iris[:1])