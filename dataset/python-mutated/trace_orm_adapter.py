...
Demos::
    ...
    python tools/trace_orm_adapter.py -m pytest         test/orm/inheritance/test_polymorphic_rel.py::PolymorphicAliasedJoinsTest::test_primary_eager_aliasing_joinedload

    python tools/trace_orm_adapter.py -m pytest         test/orm/test_eager_relations.py::LazyLoadOptSpecificityTest::test_pathed_joinedload_aliased_abs_bcs
...
You can then set a breakpoint at the end of any adapt step:
    ...
    python tools/trace_orm_adapter.py -d 10 -m pytest -s         test/orm/test_eager_relations.py::LazyLoadOptSpecificityTest::test_pathed_joinedload_aliased_abs_bcs
...
util.ORMStatementAdapter.__bases__ = (_ORMAdapterTrace,) + util.ORMStatementAdapter.__bases__
...
def _orm_adapter_trace_print(obj):
    ...
    if t_print:
        ...
        return f'{obj!r}'
_orm_adapter_trace_printers = {'table': lambda t: f'''Table("{t.name}", entity={t._annotations.get('parentmapper', None)})''', 'column': lambda c: f'''Column("{c.name}", {_orm_adapter_trace_print(c.table)} entity={c._annotations.get('parentmapper', None)})''', 'join': lambda j: f'{j.__class__.__name__}({_orm_adapter_trace_print(j.left)}, {_orm_adapter_trace_print(j.right)})', 'label': lambda l: f'Label({_orm_adapter_trace_print(l.element)})'}
