...
async def api_logs(start_time, input_data, endpoint, error_data: dict=None):
    if config['Web_API']['Allow_Logs'] == 'True':
        ...
        with open(file_name, 'a', encoding='utf-8') as f:
            data = {'time': time_now, 'endpoint': f'/{endpoint}/', 'total_time': total_time, 'input_data': input_data, 'error_data': error_data if error_data else 'No error'}
...
async def root():
    ...
    Root path info.
    """
    data = {'API_status': 'Running', 'Version': version, 'Update_time': update_time, 'Request_Rate_Limit': Rate_Limit, 'Web_APP': 'https://www.douyin.wtf/', 'API_V1_Document': 'https://api.douyin.wtf/docs', 'TikHub_API_Document': 'https://api.tikhub.io/docs', 'GitHub': 'https://github.com/Evil0ctal/Douyin_TikTok_Download_API'}
...
async def hybrid_parsing(request: Request, url: str, minimal: bool=False):
    ...
    if minimal:
        ...
        result = {'url': url, 'endpoint': '/api/', 'total_time': float(format(time.time() - start_time, '.4f'))}
        result.update(data)
...
async def get_douyin_video_data(request: Request, douyin_video_url: str=None, video_id: str=None):
    ...
    if video_id is None or video_id == '':
        ...
        if video_id is None:
            result = {'status': 'failed', 'platform': 'douyin', 'message': 'video_id获取失败/Failed to get video_id'}
            return ORJSONResponse(result)
    if video_id is not None and video_id != '':
        ...
        if video_id is not None:
            ...
            if video_data is None:
                result = {'status': 'failed', 'platform': 'douyin', 'endpoint': '/douyin_video_data/', 'message': '视频API数据获取失败/Failed to get video API data'}
                return ORJSONResponse(result)
            await api_logs(start_time=start_time, input_data={'douyin_video_url': douyin_video_url, 'video_id': video_id}, endpoint='douyin_video_data')
            total_time = float(format(time.time() - start_time, '.4f'))
            result = {'status': 'success', 'platform': 'douyin', 'endpoint': '/douyin_video_data/', 'message': '获取视频数据成功/Got video data successfully', 'total_time': total_time, 'aweme_list': [video_data]}
            return ORJSONResponse(result)
        else:
            print('获取抖音video_id失败')
            result = {'status': 'failed', 'platform': 'douyin', 'endpoint': '/douyin_video_data/', 'message': '获取视频ID失败/Failed to get video ID', 'total_time': 0, 'aweme_list': []}
            return ORJSONResponse(result)
...
async def get_douyin_live_video_data(request: Request, douyin_live_video_url: str=None, web_rid: str=None):
    ...
    if web_rid is None or web_rid == '':
        ...
        if web_rid is None:
            result = {'status': 'failed', 'platform': 'douyin', 'message': 'web_rid获取失败/Failed to get web_rid'}
            return ORJSONResponse(result)
    if web_rid is not None and web_rid != '':
        ...
        if web_rid is not None:
            ...
            if video_data is None:
                result = {'status': 'failed', 'platform': 'douyin', 'endpoint': '/douyin_live_video_data/', 'message': '直播视频API数据获取失败/Failed to get live video API data'}
                return ORJSONResponse(result)
            await api_logs(start_time=start_time, input_data={'douyin_video_url': douyin_live_video_url, 'web_rid': web_rid}, endpoint='douyin_live_video_data')
            total_time = float(format(time.time() - start_time, '.4f'))
            result = {'status': 'success', 'platform': 'douyin', 'endpoint': '/douyin_live_video_data/', 'message': '获取直播视频数据成功/Got live video data successfully', 'total_time': total_time, 'aweme_list': [video_data]}
            return ORJSONResponse(result)
        else:
            print('获取抖音video_id失败')
            result = {'status': 'failed', 'platform': 'douyin', 'endpoint': '/douyin_live_video_data/', 'message': '获取直播视频ID失败/Failed to get live video ID', 'total_time': 0, 'aweme_list': []}
            return ORJSONResponse(result)
...
async def get_tiktok_video_data(request: Request, tiktok_video_url: str=None, video_id: str=None):
    ...
    if video_id is not None and video_id != '':
        ...
        if video_data is None or video_data.get('aweme_id') != video_id:
            ...
            result = {'status': 'failed', 'platform': 'tiktok', 'endpoint': '/tiktok_video_data/', 'message': '视频数据获取失败/Failed to get video data'}
            return ORJSONResponse(result)
        ...
        total_time = float(format(time.time() - start_time, '.4f'))
        result = {'status': 'success', 'platform': 'tiktok', 'endpoint': '/tiktok_video_data/', 'message': '获取视频数据成功/Got video data successfully', 'total_time': total_time, 'aweme_list': [video_data]}
        return ORJSONResponse(result)
    else:
        print('视频链接错误/Video link error')
        result = {'status': 'failed', 'platform': 'tiktok', 'endpoint': '/tiktok_video_data/', 'message': '视频链接错误/Video link error'}
        return ORJSONResponse(result)
...
async def Get_Shortcut():
    data = {'version': config['Web_API']['iOS_Shortcut_Version'], 'update': config['Web_API']['iOS_Shortcut_Update_Time'], 'link': config['Web_API']['iOS_Shortcut_Link'], 'link_en': config['Web_API']['iOS_Shortcut_Link_EN'], 'note': config['Web_API']['iOS_Shortcut_Update_Note'], 'note_en': config['Web_API']['iOS_Shortcut_Update_Note_EN']}
    return ORJSONResponse(data)
...
async def download_file_hybrid(request: Request, url: str, prefix: bool=True, watermark: bool=False):
    ...
    if config['Web_API']['Download_Switch'] != 'True':
        return ORJSONResponse({'status': 'endpoint closed', 'message': '此端点已关闭请在配置文件中开启/This endpoint is closed, please enable it in the configuration file'})
    start_time = time.time()
    ...
    if data is None:
        ...
        if url_type == 'video':
            ...
            url = data.get('video_data').get('nwm_video_url_HQ') if not watermark else data.get('video_data').get('wm_video_url_HQ')
            print('url: ', url)
            ...
            for i in url:
                ...
                file_name = file_name_prefix + platform + '_' + aweme_id + '_' + str(index + 1) + '.' + file_format if not watermark else file_name_prefix + platform + '_' + aweme_id + '_' + str(index + 1) + '_watermark' + '.' + file_format
                file_path = root_path + '/' + file_name
...
async def batch_download_file(url_list: str, prefix: bool=True):
    ...
    return ORJSONResponse({'status': 'failed', 'message': "嘿嘿嘿，这个功能还没做呢，等我有空再做吧/Hehehe, this function hasn't been done yet, I'll do it when I have time"})
