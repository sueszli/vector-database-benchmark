...
async def multi_items_app(scope, receive, send):
    ...
    for (key, value) in data.multi_items():
        ...
        if isinstance(value, UploadFile):
            content = await value.read()
            output[key].append({'filename': value.filename, 'size': value.size, 'content': content.decode(), 'content_type': value.content_type})
        else:
...
async def app_with_headers(scope, receive, send):
    ...
    for (key, value) in data.items():
        if isinstance(value, UploadFile):
            content = await value.read()
            output[key] = {'filename': value.filename, 'size': value.size, 'content': content.decode(), 'content_type': value.content_type, 'headers': list(value.headers.items())}
        else:
            output[key] = value
    await request.close()
    response = JSONResponse(output)
