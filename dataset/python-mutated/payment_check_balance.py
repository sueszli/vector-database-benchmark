...
class CardInput(graphene.InputObjectType):
    code = graphene.String(description="Payment method nonce, a token returned by the appropriate provider's SDK.", required=True)
    cvc = graphene.String(description='Card security code.', required=False)
    money = MoneyInput(description='Information about currency and amount.', required=True)

class PaymentCheckBalanceInput(BaseInputObjectType):
    gateway_id = graphene.types.String(description='An ID of a payment gateway to check.', required=True)
    method = graphene.types.String(description='Payment method name.', required=True)
    channel = graphene.String(description='Slug of a channel for which the data should be returned.', required=True)
...
class PaymentCheckBalance(BaseMutation):
    ...
    class Arguments:
        input = PaymentCheckBalanceInput(description='Fields required to check payment balance.', required=True)
