...
class ClientSideSessionBackend(BaseSessionBackend['CookieBackendConfig']):
    ...
    async def store_in_message(self, scope_session: ScopeSession, message: Message, connection: ASGIConnection) -> None:
        ...
        for cookie_key in cookies_to_clear:
            cookie_params = dict(extract_dataclass_items(self.config, exclude_none=True, include={f for f in Cookie.__dict__ if f not in ('key', 'secret', 'max_age')}))
            headers.add('Set-Cookie', Cookie(value='null', key=cookie_key, expires=0, **cookie_params).to_header(header=''))
