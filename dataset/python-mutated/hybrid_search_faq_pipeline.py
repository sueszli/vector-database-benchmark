...
def hybrid_search_faq_pipeline():
    if False:
        while True:
            i = 10
    document_store = ElasticsearchDocumentStore(host='localhost', username='', password='', index='document', embedding_field='question_emb', embedding_dim=384, excluded_meta_data=['question_emb'], similarity='cosine')
    sparse_retriever = BM25Retriever(document_store=document_store)
    dense_retriever = EmbeddingRetriever(document_store=document_store, embedding_model='sentence-transformers/all-MiniLM-L6-v2', use_gpu=True, scale_score=False)
    ...
    prediction = pipeline.run(query='How is the virus spreading?', params={'SparseRetriever': {'top_k': 10}, 'DenseRetriever': {'top_k': 10}, 'JoinDocuments': {'top_k_join': 15}, 'ReRanker': {'top_k': 5}})
