...
class AltCLIPTextModelTester:
    ...
    def __init__(self, parent, batch_size=12, seq_length=7, is_training=True, use_input_mask=True, use_labels=True, vocab_size=99, hidden_size=32, projection_dim=32, project_dim=32, num_hidden_layers=2, num_attention_heads=4, intermediate_size=37, dropout=0.1, attention_dropout=0.1, max_position_embeddings=512, initializer_range=0.02, scope=None):
        if False:
            for i in range(10):
                print('nop')
        self.parent = parent
    ...
    def get_config(self):
        if False:
            for i in range(10):
                print('nop')
        return AltCLIPTextConfig(vocab_size=self.vocab_size, hidden_size=self.hidden_size, projection_dim=self.projection_dim, project_dim=self.project_dim, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, dropout=self.dropout, attention_dropout=self.attention_dropout, max_position_embeddings=self.max_position_embeddings, initializer_range=self.initializer_range, pad_token_id=1)
...
class AltCLIPModelTester:
    ...
    def get_config(self):
        if False:
            print('Hello World!')
        return AltCLIPConfig.from_text_vision_configs(self.text_model_tester.get_config(), self.vision_model_tester.get_config(), projection_dim=64)
...
class AltCLIPModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        expected_probs = torch.tensor([[0.99942, 0.00057805]], device=torch_device)
