...
RMSE_CUTOFF = 0.35
_DEFAULT_OPTIONS_REGRESSION = {'max_iterations': 10, 'max_depth': 6, 'min_child_weight': 0.1, 'min_loss_reduction': 0.0, 'row_subsample': 0.8, 'column_subsample': 0.8, 'random_seed': None, 'metric': 'auto', 'model_checkpoint_interval': 5, 'model_checkpoint_path': None, 'resume_from_checkpoint': None}
...
class RandomForestRegressionTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.model = tc.random_forest_regression.create(self.dtrain, target=self.target, validation_set=self.dtest, **self.param)
    ...
    def test_create(self):
        if False:
            while True:
                i = 10
        model = tc.random_forest_regression.create(self.dtrain, target='label', validation_set=self.dtest, **self.param)
        rmse = model.evaluate(self.dtest, 'rmse')['rmse']
        self.assertTrue(model is not None)
        self.assertTrue(rmse < RMSE_CUTOFF)
        dtrain = self.dtrain
        dtrain['label'] = 10
        self.assertRaises(ToolkitError, lambda : tc.random_forest_regression.create(self.dtrain, target='label_wrong', **self.param))

    def test_create_with_deprecated_num_trees(self):
        if False:
            print('Hello World!')
        num_trees = 10
        model = tc.random_forest_regression.create(self.dtrain, target='label', validation_set=self.dtest, num_trees=num_trees)
        self.assertTrue(model is not None)
    ...
    def _test_regression_model(self, train, test, rmse_threshold, target='label'):
        if False:
            for i in range(10):
                print('nop')
        model = tc.random_forest_regression.create(train, target=target, validation_set=test, **self.param)
...
def test_suite_random_forest_classifier():
    ...
    for t in testCases:
        ...
        testcase_class = type('RandomForestClassifierTest_%s' % t.__name__, (RandomForestClassifierTest,), testcase_members)
...
def binary_classification_integer_target(cls):
    ...
    cls.param = {'max_depth': 3, 'min_loss_reduction': 1, 'max_iterations': 2, 'min_child_weight': 1}
    cls.target = 'label'
    cls.unpacked_features = cls.data.column_names()
    cls.unpacked_features.remove(cls.target)
    cls.features = cls.unpacked_features[:]
    cls.model = tc.random_forest_classifier.create(cls.dtrain, target=cls.target, validation_set=cls.dtest, **cls.param)
...
class RandomForestClassifierTest(unittest.TestCase):
    ...
    def test_create(self):
        if False:
            return 10
        model = tc.random_forest_classifier.create(self.dtrain, target='label', validation_set=self.dtest, **self.param)
        self.assertTrue(model is not None)
        self.assertGreater(model.evaluate(self.dtest, 'accuracy')['accuracy'], 0.9)
        dtrain = self.dtrain[:]
        dtrain['label'] = 10
        self.assertRaises(ToolkitError, lambda : tc.random_forest_classifier.create(self.dtrain, target='label_wrong', **self.param))
    ...
    def _test_classifier_model(self, train, test, accuracy_threshold, target='label'):
        if False:
            i = 10
            return i + 15
        model = tc.random_forest_classifier.create(train, target=target, validation_set=test, **self.param)
