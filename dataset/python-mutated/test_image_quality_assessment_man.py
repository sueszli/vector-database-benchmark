...
class ImageQualityAssessmentMANTest(unittest.TestCase):
    ...
    def test_run_with_model_from_modelhub(self):
        ...
        pipeline_ins = pipeline(task=Tasks.image_quality_assessment_mos, model=model)
    ...
    def test_run_with_model_name(self):
        if False:
            return 10
        pipeline_ins = pipeline(task=Tasks.image_quality_assessment_mos, model=self.model_id)
        out_path = pipeline_ins(input=self.test_img)[OutputKeys.SCORE]
        print('pipeline: the out_path is {}'.format(out_path))
