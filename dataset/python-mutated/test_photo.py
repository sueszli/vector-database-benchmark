...
async def photolist(bot, chat_id):
    ...
    return await expect_bad_request(func, 'Type of file mismatch', 'Telegram did not accept the file.')
...
class TestPhotoWithRequest(TestPhotoBase):
    ...
    async def test_send_photo_all_args(self, bot, chat_id, photo_file):
        message = await bot.send_photo(chat_id, photo_file, caption=self.caption, disable_notification=False, protect_content=True, parse_mode='Markdown', has_spoiler=True)
    ...
    async def test_send_photo_parse_mode_markdown(self, bot, chat_id, photo_file):
        message = await bot.send_photo(chat_id, photo_file, caption=self.caption, parse_mode='Markdown')
        assert isinstance(message.photo[-2], PhotoSize)
        assert isinstance(message.photo[-2].file_id, str)
        assert isinstance(message.photo[-2].file_unique_id, str)
        assert message.photo[-2].file_id
    ...
    async def test_send_photo_parse_mode_html(self, bot, chat_id, photo_file):
        message = await bot.send_photo(chat_id, photo_file, caption=self.caption, parse_mode='HTML')
        assert isinstance(message.photo[-2], PhotoSize)
        assert isinstance(message.photo[-2].file_id, str)
        assert isinstance(message.photo[-2].file_unique_id, str)
        assert message.photo[-2].file_id
    ...
    async def test_send_photo_caption_entities(self, bot, chat_id, photo_file):
        ...
        message = await bot.send_photo(chat_id, photo_file, caption=test_string, caption_entities=entities)
    ...
    async def test_send_photo_default_parse_mode_2(self, default_bot, chat_id, photo_file):
        test_markdown_string = '_Italic_ *Bold* `Code`'
        message = await default_bot.send_photo(chat_id, photo_file, caption=test_markdown_string, parse_mode=None)
    ...
    async def test_send_photo_default_parse_mode_3(self, default_bot, chat_id, photo_file):
        test_markdown_string = '_Italic_ *Bold* `Code`'
        message = await default_bot.send_photo(chat_id, photo_file, caption=test_markdown_string, parse_mode='HTML')
        assert message.caption == test_markdown_string
        assert message.caption_markdown == escape_markdown(test_markdown_string)

    @pytest.mark.parametrize('default_bot', [{'protect_content': True}], indirect=True)
    async def test_send_photo_default_protect_content(self, chat_id, default_bot, photo):
        tasks = asyncio.gather(default_bot.send_photo(chat_id, photo), default_bot.send_photo(chat_id, photo, protect_content=False))
    ...
    async def test_send_photo_default_allow_sending_without_reply(self, default_bot, chat_id, photo_file, custom):
        reply_to_message = await default_bot.send_message(chat_id, 'test')
        await reply_to_message.delete()
        if custom is not None:
            message = await default_bot.send_photo(chat_id, photo_file, allow_sending_without_reply=custom, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        elif default_bot.defaults.allow_sending_without_reply:
            message = await default_bot.send_photo(chat_id, photo_file, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        else:
            with pytest.raises(BadRequest, match='Message to reply not found'):
                await default_bot.send_photo(chat_id, photo_file, reply_to_message_id=reply_to_message.message_id)
    ...
    async def test_send_url_png_file(self, bot, chat_id):
        message = await bot.send_photo(photo='http://dummyimage.com/600x400/000/fff.png&text=telegram', chat_id=chat_id)
