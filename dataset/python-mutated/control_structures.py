...
class If_Node(CSNodeBase):
    title = 'branch'
    init_inputs = [NodeInputType(type_='exec'), NodeInputType(label='cond')]
    init_outputs = [NodeOutputType('true', type_='exec'), NodeOutputType('false', type_='exec')]
...
class ForLoop_Node(CSNodeBase):
    ...
    title = 'for'
    init_inputs = [NodeInputType(type_='exec'), NodeInputType(label='from'), NodeInputType(label='to')]
    init_outputs = [NodeOutputType('loop', type_='exec'), NodeOutputType('i', type_='data'), NodeOutputType('finished', type_='exec')]
    GUI = guis.ForLoopGui
...
class ForEachLoop_Node(CSNodeBase):
    title = 'for each'
    init_inputs = [NodeInputType(type_='exec'), NodeInputType(label='elements')]
    init_outputs = [NodeOutputType('loop', type_='exec'), NodeOutputType('e', type_='data'), NodeOutputType('finished', type_='exec')]
...
class WhileLoop_Node(CSNodeBase):
    title = 'while'
    init_inputs = [NodeInputType(type_='exec'), NodeInputType(label='cond')]
    init_outputs = [NodeOutputType('loop', type_='exec'), NodeOutputType('finished', type_='exec')]
...
class DoWhileLoop_Node(CSNodeBase):
    ...
    def update_event(self, inp=-1):
        ...
        while self.input(0).payload:
            self.exec_output(0)
        self.exec_output(1)
nodes = [If_Node, ForLoop_Node, ForEachLoop_Node, WhileLoop_Node, DoWhileLoop_Node]