...
from tests.slack_sdk.web.mock_web_api_server import cleanup_mock_web_api_server, setup_mock_web_api_server
...
class TestWebClient_HttpRetry(unittest.TestCase):
    ...
    def test_remote_disconnected(self):
        ...
        client = WebClient(base_url='http://localhost:8888', token='xoxb-remote_disconnected', team_id='T111', retry_handlers=[retry_handler])
        try:
            client.auth_test()
    ...
    def test_ratelimited_no_retry(self):
        if False:
            print('Hello World!')
        client = WebClient(base_url='http://localhost:8888', token='xoxb-ratelimited', team_id='T111')
        try:
            client.auth_test()
            self.fail('An exception is expected')
    ...
    def test_ratelimited(self):
        if False:
            print('Hello World!')
        client = WebClient(base_url='http://localhost:8888', token='xoxb-ratelimited_only_once', team_id='T111')
    ...
    def test_fatal_error_no_retry(self):
        if False:
            print('Hello World!')
        client = WebClient(base_url='http://localhost:8888', token='xoxb-fatal_error', team_id='T111')
        try:
            client.auth_test()
            self.fail('An exception is expected')
        except SlackApiError as e:
    ...
    def test_fatal_error(self):
        if False:
            while True:
                i = 10
        client = WebClient(base_url='http://localhost:8888', token='xoxb-fatal_error_only_once', team_id='T111')
