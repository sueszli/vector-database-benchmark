...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate boilerplate for a new rule.', epilog='python scripts/add_rule.py --name PreferListBuiltin --code PIE807 --linter flake8-pie')
    parser.add_argument('--name', type=str, required=True, help="The name of the check to generate, in PascalCase (e.g., 'PreferListBuiltin').")
    parser.add_argument('--prefix', type=str, required=True, help="Prefix code for the plugin (e.g. 'PIE').")
    parser.add_argument('--code', type=str, required=True, help="The code of the check to generate (e.g., '807').")
    parser.add_argument('--linter', type=str, required=True, help="The source with which the check originated (e.g., 'flake8-pie').")
