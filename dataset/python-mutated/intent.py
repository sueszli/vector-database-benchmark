...
from homeassistant.const import ATTR_DEVICE_CLASS, ATTR_ENTITY_ID, ATTR_SUPPORTED_FEATURES
...
async def async_handle(hass: HomeAssistant, platform: str, intent_type: str, slots: _SlotsType | None=None, text_input: str | None=None, context: Context | None=None, language: str | None=None, assistant: str | None=None) -> IntentResponse:
    ...
    intent = Intent(hass, platform=platform, intent_type=intent_type, slots=slots or {}, text_input=text_input, context=context, language=language, assistant=assistant)
    try:
...
class ServiceIntentHandler(IntentHandler):
    ...
    """
    slot_schema = {vol.Any('name', 'area'): cv.string, vol.Optional('domain'): vol.All(cv.ensure_list, [cv.string]), vol.Optional('device_class'): vol.All(cv.ensure_list, [cv.string])}
    ...
    async def async_handle(self, intent_obj: Intent) -> IntentResponse:
        ...
        states = list(async_match_states(hass, name=name, area=area, domains=domains, device_classes=device_classes, assistant=intent_obj.assistant))
        if not states:
            raise IntentHandleError(f'No entities matched for: name={name}, area={area}, domains={domains}, device_classes={device_classes}')
    ...
    async def async_handle_states(self, intent_obj: Intent, states: list[State], area: area_registry.AreaEntry | None=None) -> IntentResponse:
        ...
        if area is not None:
            success_results.append(IntentResponseTarget(type=IntentResponseTargetType.AREA, name=area.name, id=area.id))
        ...
        for (state, service_coro) in zip(states, asyncio.as_completed(service_coros)):
            target = IntentResponseTarget(type=IntentResponseTargetType.ENTITY, name=state.name, id=state.entity_id)
            try:
                await service_coro
        ...
        if not success_results:
            ...
            raise IntentHandleError(f'Failed to call {self.service} for: {failed_entity_ids}')
        response.async_set_results(success_results=success_results, failed_results=failed_results)
    ...
    async def async_call_service(self, intent_obj: Intent, state: State) -> None:
        ...
        hass = intent_obj.hass
        await self._run_then_background(hass.async_create_task(hass.services.async_call(self.domain, self.service, {ATTR_ENTITY_ID: state.entity_id}, context=intent_obj.context, blocking=True), f'intent_call_service_{self.domain}_{self.service}'))
...
class Intent:
    """Hold the intent."""
    __slots__ = ['hass', 'platform', 'intent_type', 'slots', 'text_input', 'context', 'language', 'category', 'assistant']
