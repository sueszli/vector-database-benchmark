...
class SimpleSPNet(paddle.nn.Layer):
    ...
    def __init__(self, vocab_size, hidden_size, inner_size, output_size, np_fc1, np_fc2, mp_id):
        ...
        self.linear1 = spu.ColumnSequenceParallelLinear(hidden_size, inner_size, weight_attr=paddle.framework.ParamAttr(initializer=paddle.nn.initializer.Assign(init_fc1_data)), gather_output=False, has_bias=True)
        self.linear2 = spu.RowSequenceParallelLinear(inner_size, hidden_size, weight_attr=paddle.framework.ParamAttr(initializer=paddle.nn.initializer.Assign(init_fc2_data)), input_is_parallel=True, has_bias=True)
...
class TestDistSPSyncTraining(unittest.TestCase):
    ...
    def build_model_optimizer_train(self, batchs, fp16=False, amp_level='O1', mp_sync_param=False, mp_sync_grad=False, mp_sync_moment=False):
        ...
        model = SimpleSPNet(vocab_size, hidden_size, inner_size, output_size, np_fc1, np_fc2, mp_id)
    ...
    def mp_sync_base(self, mp_sync_param=False, mp_sync_grad=False, mp_sync_moment=False):
        ...
        for i in range(len(losses)):
            np.testing.assert_allclose(losses[i], losses_sync[i], rtol=1e-05, atol=1e-05)
        ...
        for i in range(len(losses_fp16)):
            np.testing.assert_allclose(losses_fp16[i], losses_sync_fp16[i], rtol=1e-05, atol=1e-05)
        ...
        for i in range(len(losses_fp16_O2)):
            np.testing.assert_allclose(losses_fp16_O2[i], losses_sync_fp16_O2[i], rtol=1e-05, atol=1e-05)
...
class TestDistSPTraining(unittest.TestCase):
    ...
    def build_model_optimizer(self):
        ...
        np_fc1 = np.random.random_sample((hidden_size, inner_size))
        np_fc2 = np.random.random_sample((inner_size, hidden_size))
        model_a = SimpleSPNet(vocab_size, hidden_size, inner_size, output_size, np_fc1, np_fc2, mp_id)
    ...
    def test_mp_model(self):
        ...
        for _ in range(5):
            ...
            np.testing.assert_allclose(loss_a.numpy(), loss_b.numpy(), rtol=1e-05, atol=1e-05)
