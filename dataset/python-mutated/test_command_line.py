...
async def test_create_new_credential(manager, provider) -> None:
    ...
    credentials = await provider.async_get_or_create_credentials({'username': 'good-user', 'password': 'good-pass'})
...
async def test_match_existing_credentials(store, provider) -> None:
    ...
    existing = auth_models.Credentials(id=uuid.uuid4(), auth_provider_type='command_line', auth_provider_id=None, data={'username': 'good-user'}, is_new=False)
    provider.async_credentials = AsyncMock(return_value=[existing])
    credentials = await provider.async_get_or_create_credentials({'username': 'good-user', 'password': 'irrelevant'})
...
async def test_utf_8_username_password(provider) -> None:
    """Test that we create a new credential."""
    credentials = await provider.async_get_or_create_credentials({'username': 'ßßß', 'password': 'äöü'})
    assert credentials.is_new is True
...
async def test_login_flow_validates(provider) -> None:
    ...
    result = await flow.async_step_init({'username': 'bad-user', 'password': 'bad-pass'})
    assert result['type'] == data_entry_flow.FlowResultType.FORM
    assert result['errors']['base'] == 'invalid_auth'
    result = await flow.async_step_init({'username': 'good-user', 'password': 'good-pass'})
