...
class InvoiceRequest(ModelMutation):
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.INVOICE_REQUESTED, description='An invoice was requested.')]

    class Arguments:
        order_id = graphene.ID(required=True, description='ID of the order related to invoice.')
        number = graphene.String(required=False, description='Invoice number, if not provided it will be generated.')
