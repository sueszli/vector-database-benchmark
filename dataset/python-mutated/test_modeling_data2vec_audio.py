...
class Data2VecAudioModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (Data2VecAudioForCTC, Data2VecAudioModel, Data2VecAudioForSequenceClassification, Data2VecAudioForAudioFrameClassification, Data2VecAudioForXVector) if is_torch_available() else ()
    pipeline_model_mapping = {'audio-classification': Data2VecAudioForSequenceClassification, 'automatic-speech-recognition': Data2VecAudioForCTC, 'feature-extraction': Data2VecAudioModel} if is_torch_available() else {}
    test_pruning = False
    ...
    def test_initialization(self):
        ...
        for model_class in self.all_model_classes:
            ...
            for (name, param) in model.named_parameters():
                uniform_init_parms = ['conv.weight', 'masked_spec_embed', 'codevectors', 'quantizer.weight_proj.weight', 'project_hid.weight', 'project_hid.bias', 'project_q.weight', 'project_q.bias', 'feature_projection.projection.weight', 'feature_projection.projection.bias', 'objective.weight']
    ...
    def test_mask_feature_prob_ctc(self):
        if False:
            i = 10
            return i + 15
        model = Data2VecAudioForCTC.from_pretrained('hf-internal-testing/tiny-random-data2vec-seq-class', mask_feature_prob=0.2, mask_feature_length=2)
    ...
    def test_mask_time_prob_ctc(self):
        if False:
            while True:
                i = 10
        model = Data2VecAudioForCTC.from_pretrained('facebook/data2vec-audio-base-960h', mask_time_prob=0.2, mask_time_length=2)
        model.to(torch_device).train()
        processor = Wav2Vec2Processor.from_pretrained('hf-internal-testing/tiny-random-wav2vec2', return_attention_mask=True)
        batch_duration_in_seconds = [1, 3, 2, 6]
        input_features = [np.random.random(16000 * s) for s in batch_duration_in_seconds]
