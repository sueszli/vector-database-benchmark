...
async def test_send_req_async_regression_62453(minion_opts):
    event_enter = MagicMock()
    event_enter.send.side_effect = lambda data, tag, cb=None, timeout=60: tornado.gen.maybe_future(True)
    event = MagicMock()
    event.__enter__.return_value = event_enter
    minion_opts['random_startup_delay'] = 0
    minion_opts['return_retry_tries'] = 30
...
async def test_master_type_failover(minion_opts):
    ...
    """
    minion_opts.update({'master_type': 'failover', 'master': ['master1', 'master2'], '__role': '', 'retry_dns': 0})
    ...
    with patch('salt.minion.resolve_dns', mock_resolve_dns), patch('salt.channel.client.AsyncPubChannel.factory', mock_channel_factory), patch('salt.loader.grains', MagicMock(return_value=[])):
...
async def test_master_type_failover_no_masters(minion_opts):
    ...
    def mock_resolve_dns(opts, fallback=False):
        ...
        assert not fallback
        raise SaltClientError('Cannot resolve {}'.format(opts['master']))
    with patch('salt.minion.resolve_dns', mock_resolve_dns), patch('salt.loader.grains', MagicMock(return_value=[])):
        with pytest.raises(SaltClientError):
            minion = salt.minion.Minion(minion_opts)
...
async def test_master_type_disable(minion_opts):
    ...
    """
    minion_opts.update({'master_type': 'disable', 'master': None, '__role': '', 'pub_ret': False, 'file_client': 'local'})
    minion = salt.minion.Minion(minion_opts)
    try:
        try:
