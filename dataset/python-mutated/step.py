...
DEFAULT_STEP_TEMPLATE: Dict[str, Any] = {'env': {'ANYSCALE_CLOUD_ID': str(DEFAULT_CLOUD_ID), 'ANYSCALE_PROJECT': str(DEFAULT_ANYSCALE_PROJECT), 'RELEASE_AWS_BUCKET': str(RELEASE_AWS_BUCKET), 'RELEASE_AWS_LOCATION': 'dev', 'RELEASE_AWS_DB_NAME': 'ray_ci', 'RELEASE_AWS_DB_TABLE': 'release_test_result', 'AWS_REGION': 'us-west-2'}, 'agents': {'queue': str(RELEASE_QUEUE_DEFAULT)}, 'plugins': [{DOCKER_PLUGIN_KEY: {'image': 'rayproject/ray', 'propagate-environment': True, 'volumes': ['/var/lib/buildkite/builds:/var/lib/buildkite/builds', '/usr/local/bin/buildkite-agent:/usr/local/bin/buildkite-agent', f'{DEFAULT_ARTIFACTS_DIR_HOST}:{DEFAULT_ARTIFACTS_DIR_HOST}'], 'environment': ['BUILDKITE_BUILD_PATH=/var/lib/buildkite/builds']}}], 'artifact_paths': [f'{DEFAULT_ARTIFACTS_DIR_HOST}/**/*'], 'priority': 0, 'retry': {'automatic': [{'exit_status': os.environ.get('BUILDKITE_RETRY_CODE', 79), 'limit': os.environ.get('BUILDKITE_MAX_RETRIES', 1)}]}}

def get_step(test: Test, test_collection_file: List[str]=None, report: bool=False, smoke_test: bool=False, env: Optional[Dict]=None, priority_val: int=0, global_config: Optional[str]=None):
    if False:
        print('Hello World!')
    env = env or {}
