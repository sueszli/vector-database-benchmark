...
def build_list_by_location_request(location: str, subscription_id: str, **kwargs: Any) -> HttpRequest:
    ...
    _url = kwargs.pop('template_url', '/subscriptions/{subscriptionId}/providers/Microsoft.DocumentDB/locations/{location}/restorableDatabaseAccounts')
...
def build_get_by_location_request(location: str, instance_id: str, subscription_id: str, **kwargs: Any) -> HttpRequest:
    ...
    api_version: str = kwargs.pop('api_version', _params.pop('api-version', '2023-09-15'))
    accept = _headers.pop('Accept', 'application/json')
    _url = kwargs.pop('template_url', '/subscriptions/{subscriptionId}/providers/Microsoft.DocumentDB/locations/{location}/restorableDatabaseAccounts/{instanceId}')
...
class RestorableDatabaseAccountsOperations:
    ...
    def list_by_location(self, location: str, **kwargs: Any) -> Iterable['_models.RestorableDatabaseAccountGetResult']:
        ...
        def prepare_request(next_link=None):
            if False:
                return 10
            if not next_link:
                request = build_list_by_location_request(location=location, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.list_by_location.metadata['url'], headers=_headers, params=_params)
    ...
    @distributed_trace
    def get_by_location(self, location: str, instance_id: str, **kwargs: Any) -> _models.RestorableDatabaseAccountGetResult:
        if False:
            return 10
        ...
        request = build_get_by_location_request(location=location, instance_id=instance_id, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.get_by_location.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = self._client._pipeline.run(request, stream=_stream, **kwargs)
