...
import itertools
import logging
from typing import Any, Awaitable, Callable, Dict, Generator, Iterable, List, Optional, Sequence, Set, Tuple, overload
...
__all__ = ['resolve_events_with_store']

async def resolve_events_with_store(clock: Clock, room_id: str, room_version: RoomVersion, state_sets: Sequence[StateMap[str]], event_map: Optional[Dict[str, EventBase]], state_res_store: StateResolutionStore) -> StateMap[str]:
    ...
    auth_diff = await _get_auth_chain_difference(room_id, state_sets, event_map, state_res_store)
    full_conflicted_set = set(itertools.chain(itertools.chain.from_iterable(conflicted_state.values()), auth_diff))
    events = await state_res_store.get_events([eid for eid in full_conflicted_set if eid not in event_map], allow_rejected=True)
    event_map.update(events)
    for event in event_map.values():
        if event.room_id != room_id:
            raise Exception('Attempting to state-resolve for room %s with event %s which is in %s' % (room_id, event.event_id, event.room_id))
    ...
    power_events = (eid for eid in full_conflicted_set if _is_power_event(event_map[eid]))
    ...
    resolved_state = await _iterative_auth_checks(clock, room_id, room_version, sorted_power_events, unconflicted_state, event_map, state_res_store)
    logger.debug('resolved power events')
    set_power_events = set(sorted_power_events)
    leftover_events = [ev_id for ev_id in full_conflicted_set if ev_id not in set_power_events]
    ...
    resolved_state = await _iterative_auth_checks(clock, room_id, room_version, leftover_events, resolved_state, event_map, state_res_store)
    logger.debug('resolved')
    resolved_state.update(unconflicted_state)
    logger.debug('done')
    return resolved_state

async def _get_power_level_for_sender(room_id: str, event_id: str, event_map: Dict[str, EventBase], state_res_store: StateResolutionStore) -> int:
    ...
    for aid in event.auth_event_ids():
        aev = await _get_event(room_id, aid, event_map, state_res_store, allow_none=True)
...
async def _get_auth_chain_difference(room_id: str, state_sets: Sequence[StateMap[str]], unpersisted_events: Dict[str, EventBase], state_res_store: StateResolutionStore) -> Set[str]:
        ...
        for state_set in state_sets:
            ...
            for event_id in state_set.values():
                ...
                if event_chain is not None:
                    set_ids.update((e for e in event_chain if e not in unpersisted_events))
                    unpersisted_ids.update((e for e in event_chain if e in unpersisted_events))
                else:
    ...
    difference = await state_res_store.get_auth_chain_difference(room_id, state_sets_ids)
...
async def _add_event_and_auth_chain_to_graph(graph: Dict[str, Set[str]], room_id: str, event_id: str, event_map: Dict[str, EventBase], state_res_store: StateResolutionStore, full_conflicted_set: Set[str]) -> None:
...
async def _reverse_topological_power_sort(clock: Clock, room_id: str, event_ids: Iterable[str], event_map: Dict[str, EventBase], state_res_store: StateResolutionStore, full_conflicted_set: Set[str]) -> List[str]:
    ...
    for (idx, event_id) in enumerate(event_ids, start=1):
        await _add_event_and_auth_chain_to_graph(graph, room_id, event_id, event_map, state_res_store, full_conflicted_set)
        if idx % _AWAIT_AFTER_ITERATIONS == 0:
            await clock.sleep(0)
    event_to_pl = {}
    for (idx, event_id) in enumerate(graph, start=1):
        pl = await _get_power_level_for_sender(room_id, event_id, event_map, state_res_store)
...
async def _iterative_auth_checks(clock: Clock, room_id: str, room_version: RoomVersion, event_ids: List[str], base_state: StateMap[str], event_map: Dict[str, EventBase], state_res_store: StateResolutionStore) -> MutableStateMap[str]:
    ...
    for (idx, event_id) in enumerate(event_ids, start=1):
        ...
        auth_events = {}
        for aid in event.auth_event_ids():
            ev = await _get_event(room_id, aid, event_map, state_res_store, allow_none=True)
            if not ev:
                logger.warning('auth_event id %s for event %s is missing', aid, event_id)
            elif ev.rejected_reason is None:
...
async def _mainline_sort(clock: Clock, room_id: str, event_ids: List[str], resolved_power_event_id: Optional[str], event_map: Dict[str, EventBase], state_res_store: StateResolutionStore) -> List[str]:
    ...
    for (idx, ev_id) in enumerate(event_ids, start=1):
        depth = await _get_mainline_depth_for_event(clock, event_map[ev_id], mainline_map, event_map, state_res_store)
...
async def _get_mainline_depth_for_event(clock: Clock, event: EventBase, mainline_map: Dict[str, int], event_map: Dict[str, EventBase], state_res_store: StateResolutionStore) -> int:
...
@overload
async def _get_event(room_id: str, event_id: str, event_map: Dict[str, EventBase], state_res_store: StateResolutionStore, allow_none: Literal[False]=False) -> EventBase:
    ...

@overload
async def _get_event(room_id: str, event_id: str, event_map: Dict[str, EventBase], state_res_store: StateResolutionStore, allow_none: Literal[True]) -> Optional[EventBase]:
    ...

async def _get_event(room_id: str, event_id: str, event_map: Dict[str, EventBase], state_res_store: StateResolutionStore, allow_none: bool=False) -> Optional[EventBase]:
    ...
    if event.room_id != room_id:
        raise Exception('In state res for room %s, event %s is in %s' % (room_id, event_id, event.room_id))
    return event
