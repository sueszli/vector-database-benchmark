...
class MultiHeadAttentionWithMP(paddle.nn.Layer):
    ...
    def __init__(self, embed_dim, num_heads, add_residual=True, pre_ln=True, attn_dropout=True):
        ...
        self.qkv_proj = paddle.nn.Linear(embed_dim, 3 * self.num_heads * self.head_dim)
        self.out_proj = paddle.nn.Linear(self.num_heads * self.head_dim, embed_dim)
    ...
    def forward(self, x, attn_mask=None):
        ...
        out = paddle.distributed.collective._mp_allreduce(out, use_calc_stream=True, use_model_parallel=True)
...
class TestFusedAttentionPassWithMP(unittest.TestCase):
    ...
    def get_rst(self, use_pass=False):
        ...
        if self.x_data is None:
            ...
            self.mask_data = np.random.rand(batch_size, num_heads // 2, seq_len, seq_len).astype('float32')
        ...
        with paddle.static.program_guard(main_prog, startup_prog):
            ...
            if self.add_mask:
                attn_mask = paddle.static.data(name='attn_mask', shape=[-1, num_heads // 2, seq_len, seq_len], dtype='float32')
            else:
                attn_mask = None
            data_linear = paddle.nn.Linear(seq_len, hidden_size)
            multi_head_attn = MultiHeadAttentionWithMP(hidden_size, num_heads, add_residual=self.add_residual, pre_ln=self.pre_ln, attn_dropout=self.attn_dropout)
        ...
        nccl_id_var = startup_block.create_var(name=base.unique_name.generate('nccl_id'), persistable=True, type=base.core.VarDesc.VarType.RAW)
        startup_block.append_op(type='c_gen_nccl_id', inputs={}, outputs={'Out': nccl_id_var}, attrs={'rank': self.rank, 'endpoint': self.current_endpoint, 'other_endpoints': self.other_endpoints})
        startup_block.append_op(type='c_comm_init', inputs={'X': nccl_id_var}, outputs={}, attrs={'nranks': self.nranks, 'rank': self.rank, 'ring_id': 0, 'device_id': self.gpu_id})
        if use_pass:
        ...
        for i in range(2):
            rst = self.exe.run(main_prog, feed={'x': self.x_data, 'attn_mask': self.mask_data}, fetch_list=[loss])
        return rst
    ...
    def test_pass(self):
        ...
        np.testing.assert_allclose(fused_rst, non_fused_rst, atol=1e-05)
