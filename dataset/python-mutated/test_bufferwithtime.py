...
class TestBufferWithCount(unittest.TestCase):
    ...
    def test_buffer_with_time_basic(self):
        ...
        xs = scheduler.create_hot_observable(on_next(100, 1), on_next(210, 2), on_next(240, 3), on_next(281, 4), on_next(321, 5), on_next(351, 6), on_next(381, 7), on_next(421, 8), on_next(471, 9), on_completed(600))
        ...
        assert results.messages == [on_next(300, '2,3,4'), on_next(370, '4,5,6'), on_next(440, '6,7,8'), on_next(510, '8,9'), on_next(580, ''), on_next(600, ''), on_completed(600)]
    ...
    def test_buffer_with_time_error(self):
        ...
        def create():
            ...
            return xs.pipe(ops.buffer_with_time(100, 70), ops.map(lambda x: ','.join([str(a) for a in x])))
        results = scheduler.start(create)
        assert results.messages == [on_next(300, '2,3,4'), on_next(370, '4,5,6'), on_next(440, '6,7,8'), on_next(510, '8,9'), on_next(580, ''), on_error(600, ex)]
        assert xs.subscriptions == [subscribe(200, 600)]
    ...
    def test_buffer_with_time_basic_same(self):
        ...
        results = scheduler.start(create)
        assert results.messages == [on_next(300, '2,3,4'), on_next(400, '5,6,7'), on_next(500, '8,9'), on_next(600, ''), on_completed(600)]
        assert xs.subscriptions == [subscribe(200, 600)]