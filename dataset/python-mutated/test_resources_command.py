...
@skipIf(not RUN_BY_CANARY and (not CI_OVERRIDE), 'Skip List test cases unless running in CI')
class TestResources(ResourcesIntegBase):
    ...
    def test_successful_transform(self):
        ...
        cmdlist = self.get_resources_command_list(stack_name=None, region=region, output='json', template_file=template_path)
    ...
    def test_invalid_template_file(self):
        ...
        cmdlist = self.get_resources_command_list(stack_name=None, region=region, output='json', template_file=template_path)
        command_result = run_command(cmdlist, cwd=self.working_dir)
        self.assertIn('Error: [InvalidTemplateException("\'Resources\' section is required")] \'Resources\' section is required', command_result.stderr.decode())
    ...
    def test_success_with_stack_name(self):
        ...
        cmdlist = self.get_resources_command_list(stack_name=stack_name, region=region, output='json', template_file=template_path)
        command_result = run_command(cmdlist, cwd=self.working_dir)
        command_output = json.loads(command_result.stdout.decode())
        ...
        self.assert_resource(command_output, 'HelloWorldFunctionHelloWorldPermissionProd', '.*HelloWorldFunctionHelloWorldPermissionProd.*')
        self.assert_resource(command_output, 'ServerlessRestApi', '.*')
        self.assert_resource(command_output, 'ServerlessRestApiProdStage', '.*')
        self.assert_resource(command_output, 'ServerlessRestApiDeployment.*', '.*')
