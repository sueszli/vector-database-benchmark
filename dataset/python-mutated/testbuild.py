...
if __name__ == '__main__':
    ...
    if arg.api_token:
        ...
        if DEFAULT_PROXY:
            s.verify = False
            s.proxies = {'http': 'http://{}'.format(DEFAULT_PROXY), 'https': 'https://{}'.format(DEFAULT_PROXY)}
        s.headers.update({'Authorization': 'Bearer {}'.format(arg.api_token)})
        if arg.list_regions:
            logger(get_regions(s))
            exit(0)
        if verbose:
            logger('arg={}'.format(arg))
        if arg.create:
            try:
                logger(colored('client_ip={}'.format(arg.client_ip), 'cyan'))
                logger(colored('Creating Droplet {}...'.format(name), 'yellow'))
                d = create_droplet(s, name, arg.fingerprint, arg.region, cip=arg.client_ip, branch=arg.branch)
