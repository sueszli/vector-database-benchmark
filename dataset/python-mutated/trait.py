...
from homeassistant.components import alarm_control_panel, binary_sensor, button, camera, climate, cover, event, fan, group, humidifier, input_boolean, input_button, input_select, light, lock, media_player, scene, script, select, sensor, switch, vacuum
...
from .const import CHALLENGE_FAILED_PIN_NEEDED, CHALLENGE_PIN_NEEDED, ERR_ALREADY_ARMED, ERR_ALREADY_DISARMED, ERR_ALREADY_STOPPED, ERR_CHALLENGE_NOT_SETUP, ERR_FUNCTION_NOT_SUPPORTED, ERR_NO_AVAILABLE_CHANNEL, ERR_NOT_SUPPORTED, ERR_UNSUPPORTED_INPUT, ERR_VALUE_OUT_OF_RANGE, FAN_SPEEDS
...
COMMAND_THERMOSTAT_TEMPERATURE_SETPOINT = f'{PREFIX_COMMANDS}ThermostatTemperatureSetpoint'
COMMAND_THERMOSTAT_TEMPERATURE_SET_RANGE = f'{PREFIX_COMMANDS}ThermostatTemperatureSetRange'
...
class BrightnessTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if self.state.domain == light.DOMAIN:
            await self.hass.services.async_call(light.DOMAIN, light.SERVICE_TURN_ON, {ATTR_ENTITY_ID: self.state.entity_id, light.ATTR_BRIGHTNESS_PCT: params['brightness']}, blocking=not self.config.should_report_state, context=data.context)
...
class CameraStreamTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        self.stream_info = {'cameraStreamAccessUrl': f'{get_url(self.hass)}{url}', 'cameraStreamReceiverAppId': CAST_APP_ID_HOMEASSISTANT_MEDIA}
...
class OnOffTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if (domain := self.state.domain) == group.DOMAIN:
            ...
            service = SERVICE_TURN_ON if params['on'] else SERVICE_TURN_OFF
        await self.hass.services.async_call(service_domain, service, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
...
class ColorSettingTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if 'temperature' in params['color']:
            temp = color_util.color_temperature_kelvin_to_mired(params['color']['temperature'])
            min_temp = self.state.attributes[light.ATTR_MIN_MIREDS]
            max_temp = self.state.attributes[light.ATTR_MAX_MIREDS]
            if temp < min_temp or temp > max_temp:
                raise SmartHomeError(ERR_VALUE_OUT_OF_RANGE, f'Temperature should be between {min_temp} and {max_temp}')
            await self.hass.services.async_call(light.DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: self.state.entity_id, light.ATTR_COLOR_TEMP: temp}, blocking=not self.config.should_report_state, context=data.context)
        elif 'spectrumRGB' in params['color']:
            hex_value = f"{params['color']['spectrumRGB']:06x}"
            color = color_util.color_RGB_to_hs(*color_util.rgb_hex_to_rgb_list(hex_value))
            await self.hass.services.async_call(light.DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: self.state.entity_id, light.ATTR_HS_COLOR: color}, blocking=not self.config.should_report_state, context=data.context)
        elif 'spectrumHSV' in params['color']:
            color = params['color']['spectrumHSV']
            saturation = color['saturation'] * 100
            brightness = color['value'] * 255
            await self.hass.services.async_call(light.DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: self.state.entity_id, light.ATTR_HS_COLOR: [color['hue'], saturation], light.ATTR_BRIGHTNESS: brightness}, blocking=not self.config.should_report_state, context=data.context)
...
class SceneTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        await self.hass.services.async_call(self.state.domain, service, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state and self.state.domain not in (button.DOMAIN, input_button.DOMAIN, script.DOMAIN), context=data.context)
...
class DockTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        await self.hass.services.async_call(self.state.domain, vacuum.SERVICE_RETURN_TO_BASE, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
...
class LocatorTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if params.get('silence', False):
            raise SmartHomeError(ERR_FUNCTION_NOT_SUPPORTED, 'Silencing a Locate request is not yet supported')
        await self.hass.services.async_call(self.state.domain, vacuum.SERVICE_LOCATE, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
...
class EnergyStorageTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        """Execute a dock command."""
        raise SmartHomeError(ERR_FUNCTION_NOT_SUPPORTED, 'Controlling charging of a vacuum is not yet supported')
...
class StartStopTrait(_Trait):
    ...
    async def _execute_vacuum(self, command, data, params, challenge):
        ...
        if command == COMMAND_STARTSTOP:
            if params['start']:
                await self.hass.services.async_call(self.state.domain, vacuum.SERVICE_START, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
            else:
                await self.hass.services.async_call(self.state.domain, vacuum.SERVICE_STOP, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
        elif command == COMMAND_PAUSEUNPAUSE:
            if params['pause']:
                await self.hass.services.async_call(self.state.domain, vacuum.SERVICE_PAUSE, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
            else:
                await self.hass.services.async_call(self.state.domain, vacuum.SERVICE_START, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
    ...
    async def _execute_cover(self, command, data, params, challenge):
        ...
        if command == COMMAND_STARTSTOP:
            if params['start'] is False:
                if self.state.state in (cover.STATE_CLOSING, cover.STATE_OPENING) or self.state.attributes.get(ATTR_ASSUMED_STATE):
                    await self.hass.services.async_call(self.state.domain, cover.SERVICE_STOP_COVER, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
                else:
                    raise SmartHomeError(ERR_ALREADY_STOPPED, 'Cover is already stopped')
            else:
                raise SmartHomeError(ERR_NOT_SUPPORTED, 'Starting a cover is not supported')
        else:
...
class TemperatureSettingTrait(_Trait):
    ...
    hvac_to_google = {climate.HVACMode.HEAT: 'heat', climate.HVACMode.COOL: 'cool', climate.HVACMode.OFF: 'off', climate.HVACMode.AUTO: 'auto', climate.HVACMode.HEAT_COOL: 'heatcool', climate.HVACMode.FAN_ONLY: 'fan-only', climate.HVACMode.DRY: 'dry'}
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if command == COMMAND_THERMOSTAT_TEMPERATURE_SETPOINT:
            temp = TemperatureConverter.convert(params['thermostatTemperatureSetpoint'], UnitOfTemperature.CELSIUS, unit)
            if unit == UnitOfTemperature.FAHRENHEIT:
                temp = round(temp)
            if temp < min_temp or temp > max_temp:
                raise SmartHomeError(ERR_VALUE_OUT_OF_RANGE, f'Temperature should be between {min_temp} and {max_temp}')
            await self.hass.services.async_call(climate.DOMAIN, climate.SERVICE_SET_TEMPERATURE, {ATTR_ENTITY_ID: self.state.entity_id, ATTR_TEMPERATURE: temp}, blocking=not self.config.should_report_state, context=data.context)
        elif command == COMMAND_THERMOSTAT_TEMPERATURE_SET_RANGE:
            temp_high = TemperatureConverter.convert(params['thermostatTemperatureSetpointHigh'], UnitOfTemperature.CELSIUS, unit)
            if unit == UnitOfTemperature.FAHRENHEIT:
                temp_high = round(temp_high)
            if temp_high < min_temp or temp_high > max_temp:
                raise SmartHomeError(ERR_VALUE_OUT_OF_RANGE, f'Upper bound for temperature range should be between {min_temp} and {max_temp}')
            temp_low = TemperatureConverter.convert(params['thermostatTemperatureSetpointLow'], UnitOfTemperature.CELSIUS, unit)
            if unit == UnitOfTemperature.FAHRENHEIT:
                temp_low = round(temp_low)
            if temp_low < min_temp or temp_low > max_temp:
                raise SmartHomeError(ERR_VALUE_OUT_OF_RANGE, f'Lower bound for temperature range should be between {min_temp} and {max_temp}')
            ...
            await self.hass.services.async_call(climate.DOMAIN, climate.SERVICE_SET_TEMPERATURE, svc_data, blocking=not self.config.should_report_state, context=data.context)
        elif command == COMMAND_THERMOSTAT_SET_MODE:
            target_mode = params['thermostatMode']
            supported = self.state.attributes.get(ATTR_SUPPORTED_FEATURES)
            if target_mode == 'on':
                await self.hass.services.async_call(climate.DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
                return
            if target_mode == 'off':
                await self.hass.services.async_call(climate.DOMAIN, SERVICE_TURN_OFF, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
                return
            if target_mode in self.google_to_preset:
                await self.hass.services.async_call(climate.DOMAIN, climate.SERVICE_SET_PRESET_MODE, {climate.ATTR_PRESET_MODE: self.google_to_preset[target_mode], ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
                return
            await self.hass.services.async_call(climate.DOMAIN, climate.SERVICE_SET_HVAC_MODE, {ATTR_ENTITY_ID: self.state.entity_id, climate.ATTR_HVAC_MODE: self.google_to_hvac[target_mode]}, blocking=not self.config.should_report_state, context=data.context)
...
class HumiditySettingTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if command == COMMAND_SET_HUMIDITY:
            await self.hass.services.async_call(humidifier.DOMAIN, humidifier.SERVICE_SET_HUMIDITY, {ATTR_ENTITY_ID: self.state.entity_id, humidifier.ATTR_HUMIDITY: params['humidity']}, blocking=not self.config.should_report_state, context=data.context)
...
class LockUnlockTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        await self.hass.services.async_call(lock.DOMAIN, service, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
...
class ArmDisArmTrait(_Trait):
    ...
    state_to_service = {STATE_ALARM_ARMED_HOME: SERVICE_ALARM_ARM_HOME, STATE_ALARM_ARMED_AWAY: SERVICE_ALARM_ARM_AWAY, STATE_ALARM_ARMED_NIGHT: SERVICE_ALARM_ARM_NIGHT, STATE_ALARM_ARMED_CUSTOM_BYPASS: SERVICE_ALARM_ARM_CUSTOM_BYPASS, STATE_ALARM_TRIGGERED: SERVICE_ALARM_TRIGGER}
    state_to_support = {STATE_ALARM_ARMED_HOME: AlarmControlPanelEntityFeature.ARM_HOME, STATE_ALARM_ARMED_AWAY: AlarmControlPanelEntityFeature.ARM_AWAY, STATE_ALARM_ARMED_NIGHT: AlarmControlPanelEntityFeature.ARM_NIGHT, STATE_ALARM_ARMED_CUSTOM_BYPASS: AlarmControlPanelEntityFeature.ARM_CUSTOM_BYPASS, STATE_ALARM_TRIGGERED: AlarmControlPanelEntityFeature.TRIGGER}
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if params['arm'] and (not params.get('cancel')):
        ...
        elif params['arm'] and params.get('cancel') and (self.state.state == STATE_ALARM_PENDING):
            ...
            _verify_pin_challenge(data, self.state, challenge)
            service = SERVICE_ALARM_DISARM
        await self.hass.services.async_call(alarm_control_panel.DOMAIN, service, {ATTR_ENTITY_ID: self.state.entity_id, ATTR_CODE: data.config.secure_devices_pin}, blocking=not self.config.should_report_state, context=data.context)
...
class FanSpeedTrait(_Trait):
    ...
    async def execute_fanspeed(self, data, params):
        ...
        domain = self.state.domain
        if domain == climate.DOMAIN:
            await self.hass.services.async_call(climate.DOMAIN, climate.SERVICE_SET_FAN_MODE, {ATTR_ENTITY_ID: self.state.entity_id, climate.ATTR_FAN_MODE: params['fanSpeed']}, blocking=not self.config.should_report_state, context=data.context)
        if domain == fan.DOMAIN:
            if (fan_speed := params.get('fanSpeed')):
                fan_speed_percent = ordered_list_item_to_percentage(self._ordered_speed, fan_speed)
            else:
                fan_speed_percent = params.get('fanSpeedPercent')
            await self.hass.services.async_call(fan.DOMAIN, fan.SERVICE_SET_PERCENTAGE, {ATTR_ENTITY_ID: self.state.entity_id, fan.ATTR_PERCENTAGE: fan_speed_percent}, blocking=not self.config.should_report_state, context=data.context)
    ...
    async def execute_reverse(self, data, params):
        ...
        if self.state.domain == fan.DOMAIN:
            ...
            await self.hass.services.async_call(fan.DOMAIN, fan.SERVICE_SET_DIRECTION, {ATTR_ENTITY_ID: self.state.entity_id, fan.ATTR_DIRECTION: direction}, blocking=not self.config.should_report_state, context=data.context)
...
class ModesTrait(_Trait):
    ...
    SYNONYMS = {'preset mode': ['preset mode', 'mode', 'preset'], 'sound mode': ['sound mode', 'effects'], 'option': ['option', 'setting', 'mode', 'value']}
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if self.state.domain == fan.DOMAIN:
            ...
            await self.hass.services.async_call(fan.DOMAIN, fan.SERVICE_SET_PRESET_MODE, {ATTR_ENTITY_ID: self.state.entity_id, fan.ATTR_PRESET_MODE: preset_mode}, blocking=not self.config.should_report_state, context=data.context)
            return
        if self.state.domain == input_select.DOMAIN:
            option = settings['option']
            await self.hass.services.async_call(input_select.DOMAIN, input_select.SERVICE_SELECT_OPTION, {ATTR_ENTITY_ID: self.state.entity_id, input_select.ATTR_OPTION: option}, blocking=not self.config.should_report_state, context=data.context)
            return
        if self.state.domain == select.DOMAIN:
            option = settings['option']
            await self.hass.services.async_call(select.DOMAIN, select.SERVICE_SELECT_OPTION, {ATTR_ENTITY_ID: self.state.entity_id, select.ATTR_OPTION: option}, blocking=not self.config.should_report_state, context=data.context)
            return
        if self.state.domain == humidifier.DOMAIN:
            requested_mode = settings['mode']
            await self.hass.services.async_call(humidifier.DOMAIN, humidifier.SERVICE_SET_MODE, {ATTR_MODE: requested_mode, ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
            return
        if self.state.domain == light.DOMAIN:
            requested_effect = settings['effect']
            await self.hass.services.async_call(light.DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: self.state.entity_id, light.ATTR_EFFECT: requested_effect}, blocking=not self.config.should_report_state, context=data.context)
            return
        if self.state.domain == media_player.DOMAIN and (sound_mode := settings.get('sound mode')):
            await self.hass.services.async_call(media_player.DOMAIN, media_player.SERVICE_SELECT_SOUND_MODE, {ATTR_ENTITY_ID: self.state.entity_id, media_player.ATTR_SOUND_MODE: sound_mode}, blocking=not self.config.should_report_state, context=data.context)
        _LOGGER.info('Received an Options command for unrecognised domain %s', self.state.domain)
        return
...
class InputSelectorTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        await self.hass.services.async_call(media_player.DOMAIN, media_player.SERVICE_SELECT_SOURCE, {ATTR_ENTITY_ID: self.state.entity_id, media_player.ATTR_INPUT_SOURCE: requested_source}, blocking=not self.config.should_report_state, context=data.context)
...
class OpenCloseTrait(_Trait):
    ...
    """
    COVER_2FA = (cover.CoverDeviceClass.DOOR, cover.CoverDeviceClass.GARAGE, cover.CoverDeviceClass.GATE)
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if domain == cover.DOMAIN:
            ...
            if command == COMMAND_OPENCLOSE_RELATIVE:
                position = self.state.attributes.get(cover.ATTR_CURRENT_POSITION)
                if position is None:
                    raise SmartHomeError(ERR_NOT_SUPPORTED, 'Current position not know for relative command')
            ...
            if should_verify and self.state.attributes.get(ATTR_DEVICE_CLASS) in OpenCloseTrait.COVER_2FA:
                _verify_pin_challenge(data, self.state, challenge)
            await self.hass.services.async_call(cover.DOMAIN, service, svc_params, blocking=not self.config.should_report_state, context=data.context)
...
class VolumeTrait(_Trait):
    ...
    async def _set_volume_absolute(self, data, level):
        await self.hass.services.async_call(media_player.DOMAIN, media_player.SERVICE_VOLUME_SET, {ATTR_ENTITY_ID: self.state.entity_id, media_player.ATTR_MEDIA_VOLUME_LEVEL: level}, blocking=not self.config.should_report_state, context=data.context)

    async def _execute_set_volume(self, data, params):
        level = max(0, min(100, params['volumeLevel']))
        if not self.state.attributes.get(ATTR_SUPPORTED_FEATURES, 0) & MediaPlayerEntityFeature.VOLUME_SET:
    ...
    async def _execute_volume_relative(self, data, params):
        ...
        if features & MediaPlayerEntityFeature.VOLUME_SET:
            ...
            for _ in range(relative):
                await self.hass.services.async_call(media_player.DOMAIN, svc, {ATTR_ENTITY_ID: self.state.entity_id}, blocking=not self.config.should_report_state, context=data.context)
        else:
            raise SmartHomeError(ERR_NOT_SUPPORTED, 'Command not supported')

    async def _execute_mute(self, data, params):
        mute = params['mute']
        if not self.state.attributes.get(ATTR_SUPPORTED_FEATURES, 0) & MediaPlayerEntityFeature.VOLUME_MUTE:
            raise SmartHomeError(ERR_NOT_SUPPORTED, 'Command not supported')
        await self.hass.services.async_call(media_player.DOMAIN, media_player.SERVICE_VOLUME_MUTE, {ATTR_ENTITY_ID: self.state.entity_id, media_player.ATTR_MEDIA_VOLUME_MUTED: mute}, blocking=not self.config.should_report_state, context=data.context)
...
class TransportControlTrait(_Trait):
    ...
    commands = [COMMAND_MEDIA_NEXT, COMMAND_MEDIA_PAUSE, COMMAND_MEDIA_PREVIOUS, COMMAND_MEDIA_RESUME, COMMAND_MEDIA_SEEK_RELATIVE, COMMAND_MEDIA_SEEK_TO_POSITION, COMMAND_MEDIA_SHUFFLE, COMMAND_MEDIA_STOP]
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if command == COMMAND_MEDIA_SEEK_RELATIVE:
            ...
            if self.state.state == STATE_PLAYING:
                now = dt_util.utcnow()
                upd_at = self.state.attributes.get(media_player.ATTR_MEDIA_POSITION_UPDATED_AT, now)
                seconds_since = (now - upd_at).total_seconds()
            position = self.state.attributes.get(media_player.ATTR_MEDIA_POSITION, 0)
            max_position = self.state.attributes.get(media_player.ATTR_MEDIA_DURATION, 0)
            service_attrs[media_player.ATTR_MEDIA_SEEK_POSITION] = min(max(position + seconds_since + rel_position, 0), max_position)
        elif command == COMMAND_MEDIA_SEEK_TO_POSITION:
            service = media_player.SERVICE_MEDIA_SEEK
            max_position = self.state.attributes.get(media_player.ATTR_MEDIA_DURATION, 0)
            service_attrs[media_player.ATTR_MEDIA_SEEK_POSITION] = min(max(params['absPositionMs'] / 1000, 0), max_position)
            ...
            raise SmartHomeError(ERR_NOT_SUPPORTED, 'Command not supported')
        await self.hass.services.async_call(media_player.DOMAIN, service, service_attrs, blocking=not self.config.should_report_state, context=data.context)
...
class MediaStateTrait(_Trait):
    ...
    commands: list[str] = []
    activity_lookup = {STATE_OFF: 'INACTIVE', STATE_IDLE: 'STANDBY', STATE_PLAYING: 'ACTIVE', STATE_ON: 'STANDBY', STATE_PAUSED: 'STANDBY', STATE_STANDBY: 'STANDBY', STATE_UNAVAILABLE: 'INACTIVE', STATE_UNKNOWN: 'INACTIVE'}
    playback_lookup = {STATE_OFF: 'STOPPED', STATE_IDLE: 'STOPPED', STATE_PLAYING: 'PLAYING', STATE_ON: 'STOPPED', STATE_PAUSED: 'PAUSED', STATE_STANDBY: 'STOPPED', STATE_UNAVAILABLE: 'STOPPED', STATE_UNKNOWN: 'STOPPED'}
...
class ChannelTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        if not channel_number:
            raise SmartHomeError(ERR_NO_AVAILABLE_CHANNEL, 'Channel is not available')
        await self.hass.services.async_call(media_player.DOMAIN, media_player.SERVICE_PLAY_MEDIA, {ATTR_ENTITY_ID: self.state.entity_id, media_player.ATTR_MEDIA_CONTENT_ID: channel_number, media_player.ATTR_MEDIA_CONTENT_TYPE: MediaType.CHANNEL}, blocking=not self.config.should_report_state, context=data.context)
...
class SensorStateTrait(_Trait):
    ...
    """
    sensor_types = {sensor.SensorDeviceClass.AQI: ('AirQuality', 'AQI'), sensor.SensorDeviceClass.CO: ('CarbonMonoxideLevel', 'PARTS_PER_MILLION'), sensor.SensorDeviceClass.CO2: ('CarbonDioxideLevel', 'PARTS_PER_MILLION'), sensor.SensorDeviceClass.PM25: ('PM2.5', 'MICROGRAMS_PER_CUBIC_METER'), sensor.SensorDeviceClass.PM10: ('PM10', 'MICROGRAMS_PER_CUBIC_METER'), sensor.SensorDeviceClass.VOLATILE_ORGANIC_COMPOUNDS: ('VolatileOrganicCompounds', 'PARTS_PER_MILLION')}
