...
PLATFORM = None
COPY_PYTHON = True
PkgListSet(['PYTHON', 'DIRECT', 'GL', 'GLES', 'GLES2'] + DXVERSIONS + ['TINYDISPLAY', 'NVIDIACG', 'EGL', 'EIGEN', 'OPENAL', 'FMODEX', 'VORBIS', 'OPUS', 'FFMPEG', 'SWSCALE', 'SWRESAMPLE', 'ODE', 'BULLET', 'PANDAPHYSICS', 'SPEEDTREE', 'ZLIB', 'PNG', 'JPEG', 'TIFF', 'OPENEXR', 'SQUISH'] + MAYAVERSIONS + MAXVERSIONS + ['FCOLLADA', 'ASSIMP', 'EGG', 'FREETYPE', 'HARFBUZZ', 'VRPN', 'OPENSSL', 'FFTW', 'ARTOOLKIT', 'OPENCV', 'DIRECTCAM', 'VISION', 'GTK3', 'MFC', 'WX', 'FLTK', 'COCOA', 'X11', 'PANDATOOL', 'PVIEW', 'DEPLOYTOOLS', 'SKEL', 'PANDAFX', 'PANDAPARTICLESYSTEM', 'CONTRIB', 'SSE2', 'NEON', 'MIMALLOC'])
...
if COMPILER == 'MSVC':
    ...
    if PkgSkip('DIRECTCAM') == 0:
        LibName('DIRECTCAM', 'strmiids.lib')
    if PkgSkip('DIRECTCAM') == 0:
        LibName('DIRECTCAM', 'quartz.lib')
    if PkgSkip('DIRECTCAM') == 0:
        LibName('DIRECTCAM', 'odbc32.lib')
    if PkgSkip('DIRECTCAM') == 0:
        LibName('DIRECTCAM', 'odbccp32.lib')
    if PkgSkip('MIMALLOC') == 0:
        LibName('MIMALLOC', GetThirdpartyDir() + 'mimalloc/lib/mimalloc-static.lib')
    ...
    if PkgSkip('JPEG') == 0:
        LibName('JPEG', GetThirdpartyDir() + 'jpeg/lib/jpeg-static.lib')
    if PkgSkip('ZLIB') == 0:
        LibName('ZLIB', GetThirdpartyDir() + 'zlib/lib/zlibstatic.lib')
    if PkgSkip('VRPN') == 0:
        LibName('VRPN', GetThirdpartyDir() + 'vrpn/lib/vrpn.lib')
    if PkgSkip('VRPN') == 0:
        LibName('VRPN', GetThirdpartyDir() + 'vrpn/lib/quat.lib')
    if PkgSkip('NVIDIACG') == 0:
        LibName('CGGL', GetThirdpartyDir() + 'nvidiacg/lib/cgGL.lib')
    if PkgSkip('NVIDIACG') == 0:
        LibName('CGDX9', GetThirdpartyDir() + 'nvidiacg/lib/cgD3D9.lib')
    if PkgSkip('NVIDIACG') == 0:
        LibName('NVIDIACG', GetThirdpartyDir() + 'nvidiacg/lib/cg.lib')
    ...
    if PkgSkip('FFTW') == 0:
        LibName('FFTW', GetThirdpartyDir() + 'fftw/lib/fftw3.lib')
    if PkgSkip('ARTOOLKIT') == 0:
        LibName('ARTOOLKIT', GetThirdpartyDir() + 'artoolkit/lib/libAR.lib')
    if PkgSkip('OPENCV') == 0:
        LibName('OPENCV', GetThirdpartyDir() + 'opencv/lib/cv.lib')
    if PkgSkip('OPENCV') == 0:
        LibName('OPENCV', GetThirdpartyDir() + 'opencv/lib/highgui.lib')
    if PkgSkip('OPENCV') == 0:
        LibName('OPENCV', GetThirdpartyDir() + 'opencv/lib/cvaux.lib')
    if PkgSkip('OPENCV') == 0:
        LibName('OPENCV', GetThirdpartyDir() + 'opencv/lib/ml.lib')
    if PkgSkip('OPENCV') == 0:
        LibName('OPENCV', GetThirdpartyDir() + 'opencv/lib/cxcore.lib')
    if PkgSkip('FFMPEG') == 0:
        LibName('FFMPEG', GetThirdpartyDir() + 'ffmpeg/lib/avcodec.lib')
    if PkgSkip('FFMPEG') == 0:
        LibName('FFMPEG', GetThirdpartyDir() + 'ffmpeg/lib/avformat.lib')
    if PkgSkip('FFMPEG') == 0:
        LibName('FFMPEG', GetThirdpartyDir() + 'ffmpeg/lib/avutil.lib')
    if PkgSkip('SWSCALE') == 0:
        LibName('SWSCALE', GetThirdpartyDir() + 'ffmpeg/lib/swscale.lib')
    if PkgSkip('SWRESAMPLE') == 0:
        LibName('SWRESAMPLE', GetThirdpartyDir() + 'ffmpeg/lib/swresample.lib')
    ...
    if not PkgSkip('SPEEDTREE'):
        ...
        if GetOptimize() <= 2:
            debugext = '_d'
        libsuffix = '_v%s_VC100MT%s_Static%s.lib' % (SDK['SPEEDTREEVERSION'], p64ext, debugext)
...
if COMPILER == 'GCC':
    ...
    if GetTarget() == 'darwin':
        if OSX_ARCHS and 'x86_64' not in OSX_ARCHS and ('i386' not in OSX_ARCHS):
            ...
            PkgDisable('FMODEX')
        elif not os.path.isfile(SDK.get('MACOSX', '') + '/usr/lib/libstdc++.6.0.9.tbd') and (not os.path.isfile(SDK.get('MACOSX', '') + '/usr/lib/libstdc++.6.0.9.dylib')):
            if not PkgSkip('FMODEX'):
    ...
    SmartPkgEnable('GLES2', 'glesv2', 'GLESv2', 'GLES2/gl2.h')
    ...
    if PkgSkip('FFMPEG') or GetTarget() == 'darwin':
        ...
        if cv_lib == 'opencv_core':
            ...
            SmartPkgEnable('OPENCV', 'opencv', ('cv', 'highgui', 'cvaux', 'ml', 'cxcore'), ('opencv', 'opencv/cv.h', 'opencv/cxcore.h', 'opencv/highgui.h'))
    else:
        PkgDisable('OPENCV')
    ...
    if not PkgSkip('ASSIMP') and os.path.isfile(GetThirdpartyDir() + 'assimp/lib/libassimp.a'):
...
del_files = ['core.py', 'core.pyc', 'core.pyo', '_core.pyd', '_core.so', 'direct.py', 'direct.pyc', 'direct.pyo', '_direct.pyd', '_direct.so', 'dtoolconfig.pyd', 'dtoolconfig.so']
    ...
    if GetTarget() == 'windows':
        ...
        if not PkgSkip('PYTHON'):
            ...
            if COPY_PYTHON and os.path.isfile(manifest):
                ...
                idents = ()
            for ident in idents:
                sxs_name = '_'.join([ident.get('processorArchitecture'), ident.get('name').lower(), ident.get('publicKeyToken'), ident.get('version')])
...
if GetTarget() == 'windows' and 'VISUALSTUDIO' in SDK:
    vcver = '%s%s' % (SDK['MSVC_VERSION'][0], 0)
...
COMMON_DTOOL_LIBS = ['libp3dtool.dll', 'libp3dtoolconfig.dll']
COMMON_PANDA_LIBS = ['libpanda.dll', 'libpandaexpress.dll'] + COMMON_DTOOL_LIBS
COMMON_EGG2X_LIBS = ['libp3eggbase.lib', 'libp3progbase.lib', 'libp3converter.lib', 'libp3pandatoolbase.lib', 'libpandaegg.dll'] + COMMON_PANDA_LIBS
...
preamble = WriteEmbeddedStringFile('interrogate_preamble_python_native', inputs=['dtool/src/interrogatedb/py_panda.cxx', 'dtool/src/interrogatedb/py_compat.cxx', 'dtool/src/interrogatedb/py_wrappers.cxx', 'dtool/src/interrogatedb/dtool_super_base.cxx'])
...
IGATEFILES += ['typeHandle.h', 'typeHandle_ext.h', 'typeRegistry.h', 'typedObject.h', 'neverFreeMemory.h']
...
IGATEFILES += ['config_dtoolutil.h', 'pandaSystem.h', 'dSearchPath.h', 'executionEnvironment.h', 'textEncoder.h', 'textEncoder_ext.h', 'filename.h', 'filename_ext.h', 'globPattern.h', 'globPattern_ext.h', 'pandaFileStream.h', 'lineStream.h', 'iostream_ext.h']
...
OPTS = ['DIR:panda/metalibs/panda', 'BUILDING:PANDA', 'JPEG', 'PNG', 'HARFBUZZ', 'TIFF', 'OPENEXR', 'ZLIB', 'FREETYPE', 'FFTW', 'ADVAPI', 'WINSOCK2', 'SQUISH', 'NVIDIACG', 'VORBIS', 'OPUS', 'WINUSER', 'WINMM', 'WINGDI', 'IPHLPAPI', 'SETUPAPI', 'INOTIFY', 'IOKIT']
...
if not PkgSkip('VISION'):
    ...
    TargetAdd('p3vision_composite1.obj', opts=OPTS, input='p3vision_composite1.cxx', dep=['dtool_have_ffmpeg.dat', 'dtool_have_opencv.dat', 'dtool_have_directcam.dat'])
...
if PkgSkip('FFMPEG') == 0:
    ...
    TargetAdd('p3ffmpeg_composite1.obj', opts=OPTS, input='p3ffmpeg_composite1.cxx', dep=['dtool_have_swscale.dat', 'dtool_have_swresample.dat'])
...
if not PkgSkip('PANDATOOL') and (not PkgSkip('EGG')):
    ...
    if not PkgSkip('FCOLLADA'):
        TargetAdd('libp3ptloader.dll', input='libp3daeegg.lib')
...
if INSTALLER:
    ...
    MakeInstaller(version=VERSION, outputdir=GetOutputDir(), optimize=GetOptimize(), compressor=COMPRESSOR, debversion=DEBVERSION, rpmversion=RPMVERSION, rpmrelease=RPMRELEASE, python_versions=python_versions)
if WHEEL:
