...
def parse_args():
    ...
    parser.add_argument('--path', '-p', action='append', default=[], help='Path to prepend to sys.path.')
    parser.add_argument('--env', '-v', action='append', default=[], help='Environment variable to set. Should be of the form VAR=VALUE.')
    parser.add_argument('--main_module', '-m', required=True, help='The fully qualified name of the target main module of the exe.')
    parser.add_argument('--exe_path', '-x', required=True, help='Path to the Python executable.')
    args = parser.parse_args()
    for env in args.env:
        if '=' not in env:
            sys.exit('Environment variables should be specified in the form VAR=VALUE.')
    return args
...
def main():
    if False:
        for i in range(10):
            print('nop')
    options = parse_args()
    path_adjustment = 'add_paths([%s])' % ','.join(["'%s'" % path for path in options.path])
    env_adjustment = ''
    ...
    with open(options.exe_path, 'w') as target:
        text = EXE_TEMPLATE.format(script=__file__, path_adjustment=path_adjustment, env_adjustment=env_adjustment, import_stmt=import_stmt, modname=modname)
        target.write(text)
