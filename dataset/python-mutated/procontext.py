...
import torch
from modelscope.models.cv.video_single_object_tracking.models.procontext.procontext import build_procontext
from modelscope.models.cv.video_single_object_tracking.utils.utils import Preprocessor, clip_box, generate_mask_cond, hann2d, sample_target, transform_image_to_crop
...
class ProContEXT:
    ...
    def initialize(self, image, info: dict):
        if False:
            print('Hello World!')
        crop_resize_patches = [sample_target(image, info['init_bbox'], factor, output_sz=self.cfg.TEST.TEMPLATE_SIZE) for factor in self.cfg.TEST.TEMPLATE_FACTOR]
        ...
        if self.cfg.MODEL.BACKBONE.CE_LOC:
            for i in range(len(self.cfg.TEST.TEMPLATE_FACTOR) * 2):
                template_bbox = self.transform_bbox_to_crop(info['init_bbox'], resize_factor[0], template.tensors.device).squeeze(1)
        ...
        for (idx, update_i) in enumerate(self.update_intervals):
            if self.frame_id % update_i == 0 and conf_score > 0.7:
                ...
                for idx_s in range(len(z_patch_arr2)):
                    ...
                    self.z_dict_list[idx_s + len(self.cfg.TEST.TEMPLATE_FACTOR)] = template_t
