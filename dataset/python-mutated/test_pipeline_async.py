...
from azure.core.pipeline.policies import SansIOHTTPPolicy, UserAgentPolicy, DistributedTracingPolicy, AsyncRetryPolicy, AsyncRedirectPolicy, AsyncHTTPPolicy, AsyncRetryPolicy, HttpLoggingPolicy, SensitiveHeaderCleanupPolicy
from azure.core.pipeline.transport import AsyncHttpTransport, AsyncioRequestsTransport, TrioRequestsTransport, AioHttpTransport, HttpRequest
...
async def test_add_custom_policy():
    ...
    pos_retry = policies.index(retry_policy)
    assert pos_boo > pos_retry
    client = AsyncPipelineClient(base_url='test', config=config, per_call_policies=boo_policy, per_retry_policies=foo_policy)
    policies = client._pipeline._impl_policies
    assert boo_policy in policies
    ...
    actual_policies = client._pipeline._impl_policies
    assert foo_policy == actual_policies[2]
    client = AsyncPipelineClient(base_url='test', policies=policies, per_call_policies=boo_policy, per_retry_policies=[foo_policy])
    actual_policies = client._pipeline._impl_policies
    assert boo_policy == actual_policies[0]
