...
DEFAULT_TEMPLATE_HELP_TEXT = 'An HTML template built with Handlebars template language. Leave it blank if you don\'t want to send an email for this action. Use the default Saleor template by providing the "DEFAULT" string as a value.'
...
class EmailConfig:
    ...
    use_tls: bool = False
    use_ssl: bool = False
DEFAULT_EMAIL_CONFIGURATION = [{'name': 'host', 'value': None}, {'name': 'port', 'value': None}, {'name': 'username', 'value': None}, {'name': 'password', 'value': None}, {'name': 'sender_name', 'value': ''}, {'name': 'sender_address', 'value': ''}, {'name': 'use_tls', 'value': False}, {'name': 'use_ssl', 'value': False}]
DEFAULT_EMAIL_CONFIG_STRUCTURE = {'host': {'type': ConfigurationTypeField.STRING, 'help_text': 'The host to use for sending email.', 'label': 'SMTP host'}, 'port': {'type': ConfigurationTypeField.STRING, 'help_text': 'Port to use for the SMTP server.', 'label': 'SMTP port'}, 'username': {'type': ConfigurationTypeField.STRING, 'help_text': 'Username to use for the SMTP server.', 'label': 'SMTP user'}, 'password': {'type': ConfigurationTypeField.PASSWORD, 'help_text': 'Password to use for the SMTP server.', 'label': 'Password'}, 'sender_name': {'type': ConfigurationTypeField.STRING, 'help_text': "Name which will be visible as 'from' name.", 'label': 'Sender name'}, 'sender_address': {'type': ConfigurationTypeField.STRING, 'help_text': "Sender email which will be visible as 'from' email.", 'label': 'Sender email'}, 'use_tls': {'type': ConfigurationTypeField.BOOLEAN, 'help_text': 'Whether to use a TLS (secure) connection when talking to the SMTP server. This is used for explicit TLS connections, generally on port 587. Use TLS/Use SSL are mutually exclusive, so only set one of these settings to True.', 'label': 'Use TLS'}, 'use_ssl': {'type': ConfigurationTypeField.BOOLEAN, 'help_text': 'Whether to use an implicit TLS (secure) connection when talking to the SMTP server. In most email documentation this type of TLS connection is referred to as SSL. It is generally used on port 465. Use TLS/Use SSL are mutually exclusive, so only set one of these settings to True.', 'label': 'Use SSL'}}
