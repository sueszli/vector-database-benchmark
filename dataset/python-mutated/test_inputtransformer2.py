...
MULTILINE_MAGIC = ('a = f()\n%foo \\\nbar\ng()\n'.splitlines(keepends=True), (2, 0), "a = f()\nget_ipython().run_line_magic('foo', ' bar')\ng()\n".splitlines(keepends=True))
INDENTED_MAGIC = ('for a in range(5):\n    %ls\n'.splitlines(keepends=True), (2, 4), "for a in range(5):\n    get_ipython().run_line_magic('ls', '')\n".splitlines(keepends=True))
CRLF_MAGIC = (['a = f()\n', '%ls\r\n', 'g()\n'], (2, 0), ['a = f()\n', "get_ipython().run_line_magic('ls', '')\n", 'g()\n'])
MULTILINE_MAGIC_ASSIGN = ('a = f()\nb = %foo \\\n  bar\ng()\n'.splitlines(keepends=True), (2, 4), "a = f()\nb = get_ipython().run_line_magic('foo', '   bar')\ng()\n".splitlines(keepends=True))
MULTILINE_SYSTEM_ASSIGN = ('a = f()\nb = !foo \\\n  bar\ng()\n'.splitlines(keepends=True), (2, 4), "a = f()\nb = get_ipython().getoutput('foo    bar')\ng()\n".splitlines(keepends=True))
MULTILINE_SYSTEM_ASSIGN_AFTER_DEDENT = ('def test():\n  for i in range(1):\n    print(i)\n  res =! ls\n'.splitlines(keepends=True), (4, 7), "def test():\n  for i in range(1):\n    print(i)\n  res =get_ipython().getoutput(' ls')\n".splitlines(keepends=True))
...
HELP_CONTINUED_LINE = ('a = \\\nzip?\n'.splitlines(keepends=True), (1, 0), ["get_ipython().run_line_magic('pinfo', 'zip')" + '\n'])
HELP_MULTILINE = ('(a,\nb) = zip?\n'.splitlines(keepends=True), (1, 0), ["get_ipython().run_line_magic('pinfo', 'zip')" + '\n'])
...
extra_closing_paren_param = pytest.param('(\n))', 'invalid', None) if sys.version_info >= (3, 12) else pytest.param('(\n))', 'incomplete', 0)
examples = [pytest.param('a = 1', 'complete', None), pytest.param('for a in range(5):', 'incomplete', 4), pytest.param('for a in range(5):\n    if a > 0:', 'incomplete', 8), pytest.param('raise = 2', 'invalid', None), pytest.param('a = [1,\n2,', 'incomplete', 0), extra_closing_paren_param, pytest.param('\\\r\n', 'incomplete', 0), pytest.param("a = '''\n   hi", 'incomplete', 3), pytest.param('def a():\n x=1\n global x', 'invalid', None), pytest.param('a \\ ', 'invalid', None, marks=pytest.mark.xfail(reason='Bug in python 3.9.8 –\xa0bpo 45738', condition=sys.version_info in [(3, 9, 8, 'final', 0), (3, 11, 0, 'alpha', 2)], raises=SystemError, strict=True)), pytest.param('1\\\n+2', 'complete', None)]
