...
VALIDATE_CREDENTIAL = {'app_id': 'valid_app_id', 'api_key': 'valid_key', 'api_secret': 'valid_secret'}
...
def test_is_provider_credentials_valid_or_raise_valid(mocker):
    if False:
        return 10
    mocker.patch('core.third_party.langchain.llms.spark.ChatSpark._generate', return_value=ChatResult(generations=[ChatGeneration(message=AIMessage(content='abc'))]))
