...
model = RandomForestClassifier(n_estimators=10, random_state=0).fit(dummy_data, dummy_target)
...
def test_sklearn_checkpoint(tmp_path):
    ...
    checkpoint = SklearnCheckpoint.from_estimator(estimator=model, path=str(tmp_path), preprocessor=preprocessor)
    assert np.allclose(checkpoint.get_estimator().feature_importances_, model.feature_importances_)
...
def test_predict_feature_columns_pandas():
    ...
    pandas_model = RandomForestClassifier(n_estimators=10, random_state=0).fit(pandas_data, pandas_target)
    preprocessor = DummyPreprocessor()
