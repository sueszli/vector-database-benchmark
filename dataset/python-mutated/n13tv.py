...
@pluginmatcher(re.compile('https?://(?:www\\.)?13tv\\.co\\.il/(live|.*?/)'))
class N13TV(Plugin):
    api_url = 'https://13tv-api.oplayer.io/api/getlink/'
    main_js_url_re = re.compile('type="text/javascript" src="(.*?main\\..+\\.js)"')
    user_id_re = re.compile('"data-ccid":"(.*?)"')
    video_name_re = re.compile('"videoRef":"(.*?)"')
    server_addr_re = re.compile('(.*[^/])(/.*)')
    media_file_re = re.compile('(.*)(\\.[^\\.].*)')
    live_schema = validate.Schema(validate.all([{'Link': validate.url()}], validate.get(0), validate.get('Link')))
    vod_schema = validate.Schema(validate.all([{'ShowTitle': str, 'ProtocolType': validate.all(str, validate.transform(lambda x: x.replace('://', ''))), 'ServerAddress': str, 'MediaRoot': str, 'MediaFile': str, 'Bitrates': str, 'StreamingType': str, 'Token': validate.all(str, validate.transform(lambda x: x.lstrip('?')))}], validate.get(0)))
