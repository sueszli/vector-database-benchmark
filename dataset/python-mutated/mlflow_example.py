...
if __name__ == '__main__':
    ...
    parser.add_argument('--tracking-uri', type=str, help='The tracking URI for the MLflow tracking server.')
    ...
    if not args.smoke_test:
        df = mlflow.search_runs([mlflow.get_experiment_by_name('example').experiment_id])
        print(df)
    tune_with_setup(mlflow_tracking_uri, finish_fast=args.smoke_test)
    if not args.smoke_test:
        df = mlflow.search_runs([mlflow.get_experiment_by_name('mixin_example').experiment_id])
        print(df)