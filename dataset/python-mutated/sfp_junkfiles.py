...
class sfp_junkfiles(SpiderFootPlugin):
    meta = {'name': 'Junk File Finder', 'summary': 'Looks for old/temporary and other similar files.', 'flags': ['slow', 'errorprone', 'invasive'], 'useCases': ['Footprint'], 'categories': ['Crawling and Scanning']}
    opts = {'fileexts': ['tmp', 'bak', 'old'], 'urlextstry': ['asp', 'php', 'jsp'], 'files': ['old', 'passwd', '.htaccess', '.htpasswd', 'Thumbs.db', 'backup'], 'dirs': ['zip', 'tar.gz', 'tgz', 'tar']}
    optdescs = {'fileexts': 'File extensions to try.', 'urlextstry': 'Try those extensions against URLs with these extensions.', 'files': 'Try to fetch each of these files from the directory of the URL.', 'dirs': 'Try to fetch the containing folder with these extensions.'}
    results = None
    hosts = None
    skiphosts = None
    bases = None
