...
from Orange.data import DiscreteVariable, TimeVariable, ContinuousVariable, StringVariable
from Orange.tests import named_file
from Orange.widgets.tests.base import WidgetTest, GuiTest
from Orange.widgets.data import owcsvimport
from Orange.widgets.data.owcsvimport import OWCSVFileImport, pandas_to_table, ColumnType, RowSpec
...
class TestOWCSVFileImport(WidgetTest):
    ...
    def test_basic(self):
        ...
        w.activate_recent(0)
        w.cancel()
    data_regions_options = owcsvimport.Options(encoding='ascii', dialect=csv.excel_tab(), columntypes=[(range(0, 1), ColumnType.Categorical), (range(1, 2), ColumnType.Text), (range(2, 3), ColumnType.Categorical)], rowspec=[(range(0, 1), RowSpec.Header), (range(1, 3), RowSpec.Skipped)])
    data_regions_path = os.path.join(os.path.dirname(__file__), 'data-regions.tab')
    ...
    data_csv_types_options = owcsvimport.Options(encoding='ascii', dialect=csv.excel_tab(), columntypes=[(range(0, 5), ColumnType.Auto)])
