...
class TestVideoNoteWithoutRequest(TestVideoNoteBase):
    ...
    async def test_send_video_note_custom_filename(self, bot, chat_id, video_note_file, monkeypatch):
    ...
    async def test_send_video_note_local_files(self, monkeypatch, bot, chat_id, local_mode):
        try:
            ...
            async def make_assertion(_, data, *args, **kwargs):
                nonlocal test_flag
                if local_mode:
                    test_flag = data.get('video_note') == expected and data.get('thumbnail') == expected
                else:
                    test_flag = isinstance(data.get('video_note'), InputFile) and isinstance(data.get('thumbnail'), InputFile)
...
class TestVideoNoteWithRequest(TestVideoNoteBase):
    ...
    async def test_send_all_args(self, bot, chat_id, video_note_file, video_note, thumb_file):
        message = await bot.send_video_note(chat_id, video_note_file, duration=self.duration, length=self.length, disable_notification=False, protect_content=True, thumbnail=thumb_file)
    ...
    async def test_send_video_note_default_allow_sending_without_reply(self, default_bot, chat_id, video_note, custom):
        reply_to_message = await default_bot.send_message(chat_id, 'test')
        await reply_to_message.delete()
        if custom is not None:
            message = await default_bot.send_video_note(chat_id, video_note, allow_sending_without_reply=custom, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        elif default_bot.defaults.allow_sending_without_reply:
            message = await default_bot.send_video_note(chat_id, video_note, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        else:
            with pytest.raises(BadRequest, match='Message to reply not found'):
                await default_bot.send_video_note(chat_id, video_note, reply_to_message_id=reply_to_message.message_id)

    @pytest.mark.parametrize('default_bot', [{'protect_content': True}], indirect=True)
    async def test_send_video_note_default_protect_content(self, chat_id, default_bot, video_note):
        tasks = asyncio.gather(default_bot.send_video_note(chat_id, video_note), default_bot.send_video_note(chat_id, video_note, protect_content=False))
