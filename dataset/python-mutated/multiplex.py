...
from ray.serve._private.constants import DEFAULT_LATENCY_BUCKET_MS, PUSH_MULTIPLEXED_MODEL_IDS_INTERVAL_S, SERVE_LOGGER_NAME
    ...
    async def shutdown(self):
        ...
        while len(self.models) > 0:
            try:
                ...
                logger.exception(f'Failed to unload model. Error: {e}')
        ...
        if model_id in self.models:
            ...
            async with self._model_cache_lock:
                if model_id in self.models:
                    return self.models[model_id]
                try:
                    if self.max_num_models_per_replica > 0 and len(self.models) >= self.max_num_models_per_replica:
                        await self.unload_model_lru()
                        self._push_multiplexed_replica_info = True
                    ...
                    logger.info(f"Successfully loaded model '{model_id}' in {loaded_time}s.")
                    self._model_load_tasks.discard(model_id)
                    self.model_load_latency_ms.observe((time.time() - load_start_time) * 1000.0)
                    return self.models[model_id]
                except Exception as e:
