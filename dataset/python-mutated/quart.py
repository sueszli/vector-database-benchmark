...
class GraphQLView(BaseGraphQLView):
    ...
    async def get_context(self, request: QuartRequest, response: QuartResponse) -> Dict[str, object]:
        context = await super().get_context(request, response)
        return get_context(context)

    async def process_result(self, request: QuartRequest, result: ExecutionResult) -> GraphQLHTTPResponse:
...
class QuartHttpClient(HttpClient):
    ...
    async def _graphql_request(self, method: Literal['get', 'post'], query: Optional[str]=None, variables: Optional[Dict[str, object]]=None, files: Optional[Dict[str, BytesIO]]=None, headers: Optional[Dict[str, str]]=None, **kwargs: Any) -> Response:
        ...
        if method == 'get':
            ...
            if files:
                kwargs['form'] = body
                kwargs['files'] = {k: FileStorage(v, filename=k) for (k, v) in files.items()}
            else:
                kwargs['data'] = json.dumps(body)
        headers = self._get_headers(method=method, headers=headers, files=files)
        return await self.request(url, method, headers=headers, **kwargs)

    async def request(self, url: str, method: Literal['get', 'post', 'patch', 'put', 'delete'], headers: Optional[Dict[str, str]]=None, **kwargs: Any) -> Response:
        async with self.app.test_app() as test_app:
            async with self.app.app_context():
                client = test_app.test_client()
                response = await getattr(client, method)(url, headers=headers, **kwargs)
        return Response(status_code=response.status_code, data=await response.data, headers=response.headers)
    ...
    async def post(self, url: str, data: Optional[bytes]=None, json: Optional[JSON]=None, headers: Optional[Dict[str, str]]=None) -> Response:
        ...
        return await self.request(url, 'post', **{k: v for (k, v) in kwargs.items() if v is not None})