...
def eq(a: PlaybookEntry | Iterable[PlaybookEntry], b: PlaybookEntry | Iterable[PlaybookEntry]):
    if False:
        for i in range(10):
            print('nop')
...
def _merge_sends(lst: list[commands.Command], ignore_hooks: bool, ignore_logs: bool) -> PlaybookEntryList:
    ...
    for x in lst:
        if isinstance(x, commands.SendData):
            ...
            ignore = ignore_hooks and isinstance(x, commands.StartHook) or (ignore_logs and isinstance(x, commands.Log))
            if not ignore:
                current_send = None
            yield x
...
class Playbook:
    ...
    assert playbook(tcp.TCPLayer(tctx))         << commands.OpenConnection(tctx.server)
        >> reply(None)
    ...
    def __init__(self, layer: Layer, hooks: bool=True, logs: bool=False, expected: PlaybookEntryList | None=None):
        if False:
            for i in range(10):
                print('nop')
        if expected is None:
    ...
    def __lshift__(self, c):
        ...
        two_subsequent_sends_to_the_same_remote = isinstance(c, commands.SendData) and isinstance(prev, commands.SendData) and (prev.connection is c.connection)
            ...
            if isinstance(x, commands.Command):
                ...
                for cmd in cmds:
                    ...
                    if isinstance(cmd, commands.CloseTcpConnection) and cmd.half_close:
                        ...
                        need_to_emulate_log = not self.logs and cmd.level in (logging.DEBUG, logging.INFO) and (pos >= len(self.expected) or not isinstance(self.expected[pos], commands.Log))
                        if need_to_emulate_log:
                            self.expected.insert(pos, cmd)
                    elif isinstance(cmd, commands.StartHook) and (not self.hooks):
                        need_to_emulate_hook = not self.hooks and (pos >= len(self.expected) or not (isinstance(self.expected[pos], commands.StartHook) and self.expected[pos].name == cmd.name))
                ...
                eq(self.expected[i:], self.actual[i:])
            i += 1
        if not eq(self.expected, self.actual):
            self._errored = True
            diffs = list(difflib.ndiff([_fmt_entry(x) for x in self.expected], [_fmt_entry(x) for x in self.actual]))
            if already_asserted:
...
class reply(events.Event):
    ...
    def __init__(self, *args, to: commands.Command | int=-1, side_effect: Callable[[Any], None]=lambda x: None):
        if False:
            return 10
...
def reply_next_layer(child_layer: type[Layer] | Callable[[context.Context], Layer], *args, **kwargs) -> reply:
    if False:
        while True:
            i = 10
