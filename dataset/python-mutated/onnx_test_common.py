...
import unittest
import warnings
from typing import Any, Callable, Collection, Iterable, List, Mapping, Optional, Sequence, Tuple, Type, Union
import numpy as np
import onnxruntime
import pytest
...
_InputArgsType = Optional[Union[torch.Tensor, int, float, bool, Sequence[Any], Mapping[str, Any]]]
_OutputsType = Sequence[_NumericType]
onnx_model_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), os.pardir, 'repos', 'onnx', 'onnx', 'backend', 'test', 'data')
...
QINT_TYPES = (torch.qint8, torch.quint8)
FLOAT_TYPES = (torch.float16, torch.float32)
COMPLEX_TYPES = (torch.complex64,)
TESTED_DTYPES = (torch.bool, *INT_TYPES, *FLOAT_TYPES)
