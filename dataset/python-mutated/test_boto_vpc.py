...
dhcp_options_parameters = {'domain_name': 'example.com', 'domain_name_servers': ['1.2.3.4'], 'ntp_servers': ['5.6.7.8'], 'netbios_name_servers': ['10.0.0.1'], 'netbios_node_type': 2}
...
@pytest.mark.skip(reason='Disabled for Python 3 due to upstream bugs: https://github.com/spulec/moto/issues/548 and https://github.com/gabrielfalcao/HTTPretty/issues/325')
@pytest.mark.skipif(HAS_BOTO is False, reason='The boto module must be installed.')
@pytest.mark.skipif(HAS_MOTO is False, reason='The moto module must be installed.')
@pytest.mark.skipif(_has_required_boto() is False, reason='The boto module must be greater than or equal to version {}'.format(required_boto_version))
