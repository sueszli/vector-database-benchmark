...
def run_treebank(mode, paths, treebank, short_name, temp_output_file, command_args, extra_args):
    ...
    if mode == Mode.TRAIN:
        ...
        train_args = ['--wordvec_dir', paths['WORDVEC_DIR'], '--train_file', train_file, '--output_file', dev_pred_file, '--batch_size', batch_size, '--lang', short_language, '--shorthand', short_name, '--mode', 'train']
    ...
    if mode == Mode.SCORE_DEV or mode == Mode.TRAIN:
        dev_args = ['--wordvec_dir', paths['WORDVEC_DIR'], '--output_file', dev_pred_file, '--lang', short_language, '--shorthand', short_name, '--mode', 'predict']
        if eval_file is None:
    ...
    if mode == Mode.SCORE_TEST:
        test_args = ['--wordvec_dir', paths['WORDVEC_DIR'], '--output_file', test_pred_file, '--lang', short_language, '--shorthand', short_name, '--mode', 'predict']
        if eval_file is None:
