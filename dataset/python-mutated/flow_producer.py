...
class FlowProducer:
    ...
    async def addNode(self, node: dict, parent: dict, queues_opts: dict, pipe):
        ...
        job = Job(queue=queue, name=node.get('name'), data=node.get('data'), opts=jobs_opts, job_id=job_id)
        ...
        if len(node_children) > 0:
            ...
            await self.scripts.addParentJob(job, wait_children_key, pipe)
            children = await self.addChildren(node_children, {'parentOpts': {'id': parent_id, 'queue': queue.qualifiedName}}, queues_opts, pipe)
