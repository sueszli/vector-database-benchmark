    ...
    def append_nearest_interp(self, data):
        if False:
            while True:
                i = 10
        if self.scale > 0.0:
            return F.interpolate(data, scale_factor=self.scale, align_corners=self.align_corners, mode='nearest', data_format=self.data_layout)
        return F.interpolate(data, size=self.resize_shape, align_corners=self.align_corners, mode='nearest', data_format=self.data_layout)
