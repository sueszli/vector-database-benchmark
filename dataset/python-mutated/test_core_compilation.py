...
class ExplicitWithPolymorhpicTest(_poly_fixtures._PolymorphicUnions, AssertsCompiledSQL):
    __dialect__ = 'default'
    default_punion = '(SELECT pjoin.person_id AS person_id, pjoin.company_id AS company_id, pjoin.name AS name, pjoin.type AS type, pjoin.status AS status, pjoin.engineer_name AS engineer_name, pjoin.primary_language AS primary_language, pjoin.manager_name AS manager_name FROM (SELECT engineers.person_id AS person_id, people.company_id AS company_id, people.name AS name, people.type AS type, engineers.status AS status, engineers.engineer_name AS engineer_name, engineers.primary_language AS primary_language, CAST(NULL AS VARCHAR(50)) AS manager_name FROM people JOIN engineers ON people.person_id = engineers.person_id UNION ALL SELECT managers.person_id AS person_id, people.company_id AS company_id, people.name AS name, people.type AS type, managers.status AS status, CAST(NULL AS VARCHAR(50)) AS engineer_name, CAST(NULL AS VARCHAR(50)) AS primary_language, managers.manager_name AS manager_name FROM people JOIN managers ON people.person_id = managers.person_id) AS pjoin) AS anon_1'
...
class ImplicitWithPolymorphicTest(_poly_fixtures._PolymorphicUnions, AssertsCompiledSQL):
...
class RelationshipNaturalInheritedTest(InheritedTest, AssertsCompiledSQL):
    __dialect__ = 'default'
    straight_company_to_person_expected = 'SELECT companies.company_id, companies.name FROM companies JOIN people ON companies.company_id = people.company_id'
    ...
    flat_aliased_pjoin = '(people AS people_1 LEFT OUTER JOIN engineers AS engineers_1 ON people_1.person_id = engineers_1.person_id LEFT OUTER JOIN managers AS managers_1 ON people_1.person_id = managers_1.person_id LEFT OUTER JOIN boss AS boss_1 ON managers_1.person_id = boss_1.boss_id) ON companies.company_id = people_1.company_id'
    aliased_pjoin = '(SELECT people.person_id AS people_person_id, people.company_id AS people_company_id, people.name AS people_name, people.type AS people_type, engineers.person_id AS engineers_person_id, engineers.status AS engineers_status, engineers.engineer_name AS engineers_engineer_name, engineers.primary_language AS engineers_primary_language, managers.person_id AS managers_person_id, managers.status AS managers_status, managers.manager_name AS managers_manager_name, boss.boss_id AS boss_boss_id, boss.golf_swing AS boss_golf_swing FROM people LEFT OUTER JOIN engineers ON people.person_id = engineers.person_id LEFT OUTER JOIN managers ON people.person_id = managers.person_id LEFT OUTER JOIN boss ON managers.person_id = boss.boss_id) AS anon_1 ON companies.company_id = anon_1.people_company_id'
    person_paperwork_expected = 'SELECT companies.company_id, companies.name FROM companies JOIN people ON companies.company_id = people.company_id JOIN paperwork ON people.person_id = paperwork.person_id'
    c_to_p_whereclause = 'SELECT companies.company_id, companies.name FROM companies JOIN people ON companies.company_id = people.company_id WHERE people.name = :name_1'
...
class RelNaturalAliasedJoinsTest(_poly_fixtures._PolymorphicAliasedJoins, RelationshipNaturalInheritedTest):
    label_style = LABEL_STYLE_TABLENAME_PLUS_COL
    straight_company_to_person_expected = 'SELECT companies.company_id, companies.name FROM companies JOIN (SELECT people.person_id AS people_person_id, people.company_id AS people_company_id, people.name AS people_name, people.type AS people_type, engineers.person_id AS engineers_person_id, engineers.status AS engineers_status, engineers.engineer_name AS engineers_engineer_name, engineers.primary_language AS engineers_primary_language, managers.person_id AS managers_person_id, managers.status AS managers_status, managers.manager_name AS managers_manager_name FROM people LEFT OUTER JOIN engineers ON people.person_id = engineers.person_id LEFT OUTER JOIN managers ON people.person_id = managers.person_id) AS pjoin ON companies.company_id = pjoin.people_company_id'
...
class RelNaturalAliasedJoinsDisamTest(_poly_fixtures._PolymorphicAliasedJoins, RelationshipNaturalInheritedTest):
    ...
    straight_company_to_person_expected = 'SELECT companies.company_id, companies.name FROM companies JOIN (SELECT people.person_id AS person_id, people.company_id AS company_id, people.name AS name, people.type AS type, engineers.person_id AS person_id_1, engineers.status AS status, engineers.engineer_name AS engineer_name, engineers.primary_language AS primary_language, managers.person_id AS person_id_2, managers.status AS status_1, managers.manager_name AS manager_name FROM people LEFT OUTER JOIN engineers ON people.person_id = engineers.person_id LEFT OUTER JOIN managers ON people.person_id = managers.person_id) AS pjoin ON companies.company_id = pjoin.company_id'
    person_paperwork_expected = 'SELECT companies.company_id, companies.name FROM companies JOIN (SELECT people.person_id AS person_id, people.company_id AS company_id, people.name AS name, people.type AS type, engineers.person_id AS person_id_1, engineers.status AS status, engineers.engineer_name AS engineer_name, engineers.primary_language AS primary_language, managers.person_id AS person_id_2, managers.status AS status_1, managers.manager_name AS manager_name FROM people LEFT OUTER JOIN engineers ON people.person_id = engineers.person_id LEFT OUTER JOIN managers ON people.person_id = managers.person_id) AS pjoin ON companies.company_id = pjoin.company_id JOIN paperwork ON pjoin.person_id = paperwork.person_id'
    default_pjoin = '(SELECT people.person_id AS person_id, people.company_id AS company_id, people.name AS name, people.type AS type, engineers.person_id AS person_id_1, engineers.status AS status, engineers.engineer_name AS engineer_name, engineers.primary_language AS primary_language, managers.person_id AS person_id_2, managers.status AS status_1, managers.manager_name AS manager_name FROM people LEFT OUTER JOIN engineers ON people.person_id = engineers.person_id LEFT OUTER JOIN managers ON people.person_id = managers.person_id) AS pjoin ON companies.company_id = pjoin.company_id'
    flat_aliased_pjoin = '(SELECT people.person_id AS person_id, people.company_id AS company_id, people.name AS name, people.type AS type, engineers.person_id AS person_id_1, engineers.status AS status, engineers.engineer_name AS engineer_name, engineers.primary_language AS primary_language, managers.person_id AS person_id_2, managers.status AS status_1, managers.manager_name AS manager_name FROM people LEFT OUTER JOIN engineers ON people.person_id = engineers.person_id LEFT OUTER JOIN managers ON people.person_id = managers.person_id) AS pjoin_1 ON companies.company_id = pjoin_1.company_id'
    aliased_pjoin = '(SELECT people.person_id AS person_id, people.company_id AS company_id, people.name AS name, people.type AS type, engineers.person_id AS person_id_1, engineers.status AS status, engineers.engineer_name AS engineer_name, engineers.primary_language AS primary_language, managers.person_id AS person_id_2, managers.status AS status_1, managers.manager_name AS manager_name FROM people LEFT OUTER JOIN engineers ON people.person_id = engineers.person_id LEFT OUTER JOIN managers ON people.person_id = managers.person_id) AS pjoin_1 ON companies.company_id = pjoin_1.company_id'
    c_to_p_whereclause = 'SELECT companies.company_id, companies.name FROM companies JOIN (SELECT people.person_id AS person_id, people.company_id AS company_id, people.name AS name, people.type AS type, engineers.person_id AS person_id_1, engineers.status AS status, engineers.engineer_name AS engineer_name, engineers.primary_language AS primary_language, managers.person_id AS person_id_2, managers.status AS status_1, managers.manager_name AS manager_name FROM people LEFT OUTER JOIN engineers ON people.person_id = engineers.person_id LEFT OUTER JOIN managers ON people.person_id = managers.person_id) AS pjoin ON companies.company_id = pjoin.company_id WHERE pjoin.name = :name_1'
    poly_columns = 'SELECT pjoin.person_id FROM (SELECT people.person_id AS person_id, people.company_id AS company_id, people.name AS name, people.type AS type, engineers.person_id AS person_id_1, engineers.status AS status, engineers.engineer_name AS engineer_name, engineers.primary_language AS primary_language, managers.person_id AS person_id_2, managers.status AS status_1, managers.manager_name AS manager_name FROM people LEFT OUTER JOIN engineers ON people.person_id = engineers.person_id LEFT OUTER JOIN managers ON people.person_id = managers.person_id) AS pjoin'

class JoinedInhTest(InheritedTest, _poly_fixtures._Polymorphic, AssertsCompiledSQL):
    __dialect__ = 'default'
