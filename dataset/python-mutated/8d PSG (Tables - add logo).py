...
import operator
import os
sg.SetOptions(background_color='Black', element_background_color='Black', font=('Calibri', 16, 'bold'), text_color='White')
...
def table_example():
    ...
    col_layout = [[sg.Table(values=data, headings=header, col_widths=(16, 3, 3, 3, 3, 6, 6, 6, 4), auto_size_columns=False, max_col_width=30, justification='right', size=(None, len(data)))]]
    ...
    (b, v) = window.Read()
slayout = [[sg.Image(pathname), sg.Text(' Load AFL (csv) file to display results. '), sg.ReadButton('Load File', size=(10, 1))]]
