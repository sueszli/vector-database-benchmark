...
def get_mnist_classifier(framework):
    ...
    def _get_classifier():
        ...
        if framework == 'pytorch':
            ...
            optimizer = torch.optim.SGD(classifier.model.parameters(), lr=0.1, momentum=0.9, weight_decay=0.0005)
            scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[200, 400], gamma=0.1)
            rs = PyTorchMACER(model=classifier.model, loss=classifier._loss, input_shape=classifier.input_shape, nb_classes=classifier.nb_classes, optimizer=optimizer, clip_values=classifier.clip_values, channels_first=classifier.channels_first, sample_size=100, scale=0.01, alpha=0.001, beta=16.0, gamma=8.0, lmbda=12.0, gaussian_samples=16)
        elif framework == 'tensorflow2':
            import tensorflow as tf
            (classifier, _) = get_image_classifier_tf()
            optimizer = tf.keras.optimizers.SGD(learning_rate=0.01, momentum=0.9, name='SGD', decay=0.0005)
            scheduler = tf.keras.optimizers.schedules.PiecewiseConstantDecay([250, 400], [0.01, 0.001, 0.0001])
            rs = TensorFlowV2MACER(model=classifier.model, nb_classes=classifier.nb_classes, input_shape=classifier.input_shape, loss_object=classifier.loss_object, optimizer=optimizer, train_step=None, channels_first=classifier.channels_first, clip_values=classifier.clip_values, preprocessing_defences=classifier.preprocessing_defences, postprocessing_defences=classifier.postprocessing_defences, preprocessing=classifier.preprocessing, sample_size=100, scale=0.01, alpha=0.001, beta=16.0, gamma=8.0, lmbda=12.0, gaussian_samples=16)
        else:
