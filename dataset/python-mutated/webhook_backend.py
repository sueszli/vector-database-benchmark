...
class WebhookBackend(AWXBaseEmailBackend, CustomNotificationBase):
    MAX_RETRIES = 5
    init_parameters = {'url': {'label': 'Target URL', 'type': 'string'}, 'http_method': {'label': 'HTTP Method', 'type': 'string', 'default': 'POST'}, 'disable_ssl_verification': {'label': 'Verify SSL', 'type': 'bool', 'default': False}, 'username': {'label': 'Username', 'type': 'string', 'default': ''}, 'password': {'label': 'Password', 'type': 'password', 'default': ''}, 'headers': {'label': 'HTTP Headers', 'type': 'object'}}
    recipient_parameter = 'url'
    sender_parameter = None
    DEFAULT_BODY = '{{ job_metadata }}'
    default_messages = {'started': {'body': DEFAULT_BODY}, 'success': {'body': DEFAULT_BODY}, 'error': {'body': DEFAULT_BODY}, 'workflow_approval': {'running': {'body': '{"body": "The approval node \\"{{ approval_node_name }}\\" needs review. This node can be viewed at: {{ workflow_url }}"}'}, 'approved': {'body': '{"body": "The approval node \\"{{ approval_node_name }}\\" was approved. {{ workflow_url }}"}'}, 'timed_out': {'body': '{"body": "The approval node \\"{{ approval_node_name }}\\" has timed out. {{ workflow_url }}"}'}, 'denied': {'body': '{"body": "The approval node \\"{{ approval_node_name }}\\" was denied. {{ workflow_url }}"}'}}}
