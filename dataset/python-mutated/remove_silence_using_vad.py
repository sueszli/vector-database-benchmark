...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='python TTS/bin/remove_silence_using_vad.py -i=VCTK-Corpus/ -o=VCTK-Corpus-removed-silence/ -g=wav48_silence_trimmed/*/*_mic1.flac --trim_just_beginning_and_end True')
    ...
    parser.add_argument('-g', '--glob', type=str, default='**/*.wav', help='path in glob format for acess wavs from input_dir. ex: wav48/*/*.wav')
    parser.add_argument('-t', '--trim_just_beginning_and_end', type=bool, default=True, help='If True this script will trim just the beginning and end nonspeech parts. If False all nonspeech parts will be trim. Default True')
    parser.add_argument('-c', '--use_cuda', type=bool, default=False, help='If True use cuda')
    parser.add_argument('--use_onnx', type=bool, default=False, help='If True use onnx')
