...
CustomTestPolicy.allow_actions([OperationType.LIST], scopes=[OauthScopeType.CLIENT_PRIVATE], condition=lambda policy: policy.test_condition(), override_permission_condition=lambda policy: policy.test_condition())
CustomTestPolicy.allow_actions([OperationType.LIST], scopes=[OauthScopeType.CLIENT_PRIVATE], condition=lambda policy: policy.fail_condition(), override_permission_condition=lambda policy: policy.fail_condition())
CustomTestPolicy.allow_read(['first_name', 'username'], scopes=[OauthScopeType.CLIENT_PRIVATE], on_action=[OperationType.LIST], condition=lambda policy: policy.test_condition(), override_permission_condition=lambda policy: policy.test_condition())
CustomTestPolicy.allow_read(['first_name', 'username'], scopes=[OauthScopeType.CLIENT_PRIVATE], on_action=[OperationType.LIST], condition=lambda policy: policy.fail_condition(), override_permission_condition=lambda policy: policy.fail_condition())
CustomTestPolicy.allow_write(['first_name', 'username'], scopes=[OauthScopeType.CLIENT_PRIVATE], on_action=[OperationType.LIST], condition=lambda policy: policy.test_condition(), override_permission_condition=lambda policy: policy.test_condition())
CustomTestPolicy.allow_write(['first_name', 'username'], scopes=[OauthScopeType.CLIENT_PRIVATE], on_action=[OperationType.LIST], condition=lambda policy: policy.fail_condition(), override_permission_condition=lambda policy: policy.fail_condition())
CustomTestPolicy.allow_query(['username'], scopes=[OauthScopeType.CLIENT_PRIVATE], on_action=[OperationType.LIST], condition=lambda policy: policy.test_condition(), override_permission_condition=lambda policy: policy.test_condition())
CustomTestPolicy.allow_query(['username'], scopes=[OauthScopeType.CLIENT_PRIVATE], on_action=[OperationType.LIST], condition=lambda policy: policy.fail_condition(), override_permission_condition=lambda policy: policy.fail_condition())
...
class BasePolicyTest(BaseApiTestCase, BootstrapMixin):
    ...
    async def test_authorize_attribute_read(self):
        ...
        resource = CustomTestResource(self.user, self.user)
        policy = CustomTestPolicy(resource, self.user)
        await policy.authorize_attribute(AttributeOperationType.READ, 'username', api_operation_action=OperationType.LIST)
        self.cleanup()

    async def test_authorize_attribute_read_is_owner(self):
        policy = CustomTestPolicy(None, secrets.token_urlsafe())
        with patch.object(policy, 'is_owner', lambda : True):
            self.assertTrue(await policy.authorize_attribute(AttributeOperationType.READ, 'id', api_operation_action=OperationType.DETAIL))
    ...
    async def test_authorize_attribute_read_failure_unauthorized_action(self):
        ...
        error = False
        try:
            await policy.authorize_attribute(AttributeOperationType.READ, 'username', api_operation_action=OperationType.DETAIL)
        except ApiError:
            error = True
        self.assertTrue(error)
        self.cleanup()
    ...
    async def test_authorize_attribute_read_failure_unauthorized_attribute(self):
        ...
        error = False
        try:
            await policy.authorize_attribute(AttributeOperationType.READ, 'id', api_operation_action=OperationType.LIST)
        except ApiError:
            error = True
        self.assertTrue(error)
        self.cleanup()
    ...
    async def test_authorize_attribute_write(self):
        ...
        resource = CustomTestResource(self.user, self.user)
        policy = CustomTestPolicy(resource, self.user)
        await policy.authorize_attribute(AttributeOperationType.WRITE, 'username', api_operation_action=OperationType.LIST)
        self.cleanup()

    async def test_authorize_attribute_write_is_owner(self):
        policy = CustomTestPolicy(None, secrets.token_urlsafe())
        with patch.object(policy, 'is_owner', lambda : True):
            self.assertTrue(await policy.authorize_attribute(AttributeOperationType.WRITE, 'id', api_operation_action=OperationType.DETAIL))
    ...
    async def test_authorize_attribute_write_failure_unauthorized_action(self):
        ...
        error = False
        try:
            await policy.authorize_attribute(AttributeOperationType.WRITE, 'username', api_operation_action=OperationType.DETAIL)
        except ApiError:
            error = True
        self.assertTrue(error)
        self.cleanup()
    ...
    async def test_authorize_attribute_write_failure_unauthorized_attribute(self):
        ...
        error = False
        try:
            await policy.authorize_attribute(AttributeOperationType.WRITE, 'id', api_operation_action=OperationType.LIST)
        except ApiError:
            error = True
        self.assertTrue(error)
        self.cleanup()
    ...
    async def test_authorize_query(self):
        ...
        resource = CustomTestResource(self.user, self.user)
        policy = CustomTestPolicy(resource, self.user)
        await policy.authorize_query(dict(username=1), api_operation_action=OperationType.LIST)
        self.cleanup()

    async def test_authorize_query_is_owner(self):
        policy = CustomTestPolicy(None, secrets.token_urlsafe())
        with patch.object(policy, 'is_owner', lambda : True):
            self.assertTrue(await policy.authorize_query(dict(id=1), api_operation_action=OperationType.DETAIL))
    ...
    async def test_authorize_query_failure_unauthorized_action(self):
        ...
        error = False
        try:
            await policy.authorize_query(dict(username=1), api_operation_action=OperationType.DETAIL)
        except ApiError:
            error = True
        self.assertTrue(error)
        self.cleanup()
    ...
    async def test_authorize_query_failure_unauthorized_attribute(self):
        ...
        error = False
        try:
            await policy.authorize_query(dict(id=1), api_operation_action=OperationType.LIST)
        except ApiError:
            error = True
        self.assertTrue(error)
        self.cleanup()
    ...
    async def test_authorize_attributes_read(self):
        ...
        resource = CustomTestResource(self.user, self.user)
        policy = CustomTestPolicy(resource, self.user)
        await policy.authorize_attributes(AttributeOperationType.READ, ['first_name', 'username'], api_operation_action=OperationType.LIST)
        self.cleanup()
    ...
    async def test_authorize_attributes_read_failure(self):
        ...
        error = False
        try:
            await policy.authorize_attributes(AttributeOperationType.READ, ['first_name', 'id', 'username'], api_operation_action=OperationType.LIST)
        except ApiError:
            error = True
        self.assertTrue(error)
        self.cleanup()
    ...
    async def test_authorize_attributes_write(self):
        ...
        resource = CustomTestResource(self.user, self.user)
        policy = CustomTestPolicy(resource, self.user)
        await policy.authorize_attributes(AttributeOperationType.WRITE, ['first_name', 'username'], api_operation_action=OperationType.LIST)
        self.cleanup()
    ...
    async def test_authorize_attributes_write_failure(self):
        ...
        error = False
        try:
            await policy.authorize_attributes(AttributeOperationType.WRITE, ['first_name', 'id', 'username'], api_operation_action=OperationType.LIST)
        except ApiError:
            error = True
        self.assertTrue(error)
        self.cleanup()
    ...
    async def test_authorize_attribute_query_with_permissions_and_override_permissions(self):
        ...
        with patch.object(CustomTestPolicy, 'attribute_rule_with_permissions', _rule):
            resource = CustomTestResource(self.user, self.user)
            policy = CustomTestPolicy(resource, self.user)
            await policy.authorize_attribute(AttributeOperationType.QUERY, 'username', api_operation_action=OperationType.LIST)
        self.cleanup()
...
class BasePolicyWithoutUserAuthenticationTest(BaseApiTestCase, BootstrapMixin):
    ...
    async def test_authorize_query_require_user_authentication_off(self):
        ...
        with patch.object(CustomTestPolicy, 'query_rule', _rule):
            resource = CustomTestResource(None, None)
            policy = CustomTestPolicy(resource, None)
            await policy.authorize_query(dict(query1='1'), api_operation_action=OperationType.UPDATE)
        self.cleanup()
    ...
    async def test_authorize_query_notebook_pipeline_edit_access(self, mock_access_method):
        ...
        with patch.object(CustomTestPolicy, 'query_rule', _rule):
            ...
            await policy.authorize_query(dict(query1='1'), api_operation_action=OperationType.UPDATE)
            with self.assertRaises(ApiError) as context:
                await policy.authorize_query(dict(query2='1'), api_operation_action=OperationType.UPDATE)
                self.assertTrue(context.exception.code == 403)
        self.cleanup()