...
def get_tsdata():
    ...
    for tsdata in [tsdata_train, tsdata_test]:
        tsdata.gen_dt_feature(one_hot_features=['HOUR', 'WEEK']).impute('last').scale(minmax, fit=tsdata is tsdata_train).roll(lookback=100, horizon=10)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_workers', type=int, default=1, help='The number of nodes to be used in the clusterYou can change it depending on your own cluster setting.')
    ...
    forecaster = Seq2SeqForecaster(past_seq_len=100, future_seq_len=10, input_feature_num=x_train.shape[-1], output_feature_num=2, metrics=['mse'], distributed=True, workers_per_node=args.workers_per_node, seed=0)
    ...
    (rmse, smape) = [Evaluator.evaluate(m, y_true=unscale_y_test, y_pred=unscale_yhat, multioutput='raw_values') for m in ['rmse', 'smape']]
