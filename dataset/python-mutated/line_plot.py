...
class LinePlot(Plot):
    ...
    def create_plot(value: pd.DataFrame, x: str, y: str, color: str | None=None, stroke_dash: str | None=None, overlay_point: bool | None=None, title: str | None=None, tooltip: list[str] | str | None=None, x_title: str | None=None, y_title: str | None=None, x_label_angle: float | None=None, y_label_angle: float | None=None, color_legend_title: str | None=None, stroke_dash_legend_title: str | None=None, color_legend_position: Literal['left', 'right', 'top', 'bottom', 'top-left', 'top-right', 'bottom-left', 'bottom-right', 'none'] | None=None, stroke_dash_legend_position: Literal['left', 'right', 'top', 'bottom', 'top-left', 'top-right', 'bottom-left', 'bottom-right', 'none'] | None=None, height: int | None=None, width: int | None=None, x_lim: list[int] | None=None, y_lim: list[int] | None=None, interactive: bool | None=None):
        ...
        if interactive and any([color, stroke_dash]):
            highlight = alt.selection(type='single', on='mouseover', fields=[c for c in [color, stroke_dash] if c], nearest=True)
        if stroke_dash:
            stroke_dash = {'field': stroke_dash, 'legend': AltairPlot.create_legend(position=stroke_dash_legend_position, title=stroke_dash_legend_title or stroke_dash)}
        else:
            stroke_dash = alt.value(alt.Undefined)
        if tooltip:
            encodings['tooltip'] = tooltip
        chart = alt.Chart(value).encode(**encodings)
        points = chart.mark_point(clip=True).encode(opacity=alt.value(alt.Undefined) if overlay_point else alt.value(0))
        lines = chart.mark_line(clip=True).encode(strokeDash=stroke_dash)
        if highlight:
            points = points.add_selection(highlight)
            lines = lines.encode(size=alt.condition(highlight, alt.value(4), alt.value(1)))
