...
class Withdrawal(faust.Record, isodates=True, serializer='json'):
    ...
    amount: float
    date: datetime = None
app = faust.App('faust-withdrawals', version=5, broker='aiokafka://', store='rocksdb://', origin='examples.withdrawals', topic_partitions=4)
withdrawals_topic = app.topic('withdrawals', value_type=Withdrawal)
user_to_total = app.Table('user_to_total', default=int).tumbling(3600).relative_to_stream()
country_to_total = app.Table('country_to_total', default=int).tumbling(10.0, expires=10.0).relative_to_stream()
