...
if __name__ == '__main__':
    ...
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-08)
    ...
    aux_obs = torch.zeros((args.num_steps, args.aux_batch_rollouts) + envs.single_observation_space.shape, dtype=torch.uint8)
        ...
        for auxiliary_update in range(1, args.e_auxiliary + 1):
            ...
            for (i, start) in enumerate(range(0, args.aux_batch_rollouts, args.num_aux_rollouts)):
                ...
                try:
                    ...
                    raise Exception('if running out of CUDA memory, try a higher --n-aux-grad-accum, which trades more time for less gpu memory') from e
