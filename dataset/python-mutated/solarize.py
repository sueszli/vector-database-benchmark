...
class RandomSolarize(IntensityAugmentationBase2D):
    ...
    def __init__(self, thresholds: Union[Tensor, float, Tuple[float, float], List[float]]=0.1, additions: Union[Tensor, float, Tuple[float, float], List[float]]=0.1, same_on_batch: bool=False, p: float=0.5, keepdim: bool=False) -> None:
        if False:
            while True:
                i = 10
        super().__init__(p=p, same_on_batch=same_on_batch, keepdim=keepdim)
        self._param_generator = rg.PlainUniformGenerator((thresholds, 'thresholds', 0.5, (0.0, 1.0)), (additions, 'additions', 0.0, (-0.5, 0.5)))
