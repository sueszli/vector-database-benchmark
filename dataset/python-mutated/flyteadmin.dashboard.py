import typing
from grafanalib.core import Alert, AlertCondition, Dashboard, Graph, GreaterThan, OP_AND, OPS_FORMAT, Row, RTYPE_SUM, SECONDS_FORMAT, SHORT_FORMAT, single_y_axis, Target, TimeRange, YAxes, YAxis, DataSourceInput, MILLISECONDS_FORMAT
DATASOURCE_NAME = 'DS_PROM'
DATASOURCE = '${%s}' % DATASOURCE_NAME

class FlyteAdmin(object):
    APIS = ['create_execution', 'create_launch_plan', 'create_task', 'create_workflow', 'create_node_execution_event', 'create_task_execution_event', 'get_execution', 'get_launch_plan', 'get_task', 'get_workflow', 'get_node_execution', 'get_task_execution', 'get_active_launch_plan', 'list_execution', 'list_launch_plan', 'list_task', 'list_workflow', 'list_node_execution', 'list_task_execution', 'list_active_launch_plans']
    ENTITIES = ['executions', 'task_executions', 'node_executions', 'workflows', 'launch_plans', 'project']
    DB_OPS = ['get', 'list', 'create', 'update', 'list_identifiers', 'delete', 'exists']
    ...
    def create_all_rows(interval: int=5) -> typing.List[Row]:
        ...
        return rows
dashboard = Dashboard(tags=['flyte', 'prometheus', 'flyteadmin', 'flyte-controlplane'], inputs=[DataSourceInput(name=DATASOURCE_NAME, label='Prometheus', description='Prometheus server that connects to Flyte', pluginId='prometheus', pluginName='Prometheus')], editable=False, title='FlyteAdmin Dashboard (via Prometheus)', rows=FlyteAdmin.create_all_rows(), description='FlyteAdmin/Control Plane Dashboard. This is great for monitoring FlyteAdmin and the Service API.').auto_panel_ids()
