...
conf = {'spark.network.timeout': '10000000', 'spark.sql.broadcastTimeout': '7200', 'spark.sql.shuffle.partitions': '2000', 'spark.locality.wait': '0s', 'spark.sql.hive.filesourcePartitionFileCacheSize': '4096000000', 'spark.sql.crossJoin.enabled': 'true', 'spark.serializer': 'org.apache.spark.serializer.KryoSerializer', 'spark.kryo.unsafe': 'true', 'spark.kryoserializer.buffer.max': '1024m', 'spark.task.cpus': '1', 'spark.executor.heartbeatInterval': '200s', 'spark.driver.maxResultSize': '40G'}
...
if __name__ == '__main__':
    ...
    if args.cluster_mode == 'local':
        ...
        sc = init_orca_context('standalone', master=args.master, cores=args.executor_cores, num_nodes=args.num_executors, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, conf=conf, extra_python_lib='utils.py')
    elif args.cluster_mode == 'yarn':
        sc = init_orca_context('yarn-client', cores=args.executor_cores, num_nodes=args.num_executors, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, conf=conf, extra_python_lib='utils.py')
    else:
        invalidInputError(False, "cluster_mode should be one of 'local', 'yarn' and 'standalone', but got " + args.cluster_mode)
    begin = time.time()
    transaction_tbl = FeatureTable.read_json(args.input_transaction).select(['reviewerID', 'asin', 'unixReviewTime']).rename({'reviewerID': 'user', 'asin': 'item', 'unixReviewTime': 'time'}).dropna(columns=['user', 'item'])
    ...
    def process_single_meta(row):
        ...
        return [obj['asin'], cat]
    item_tbl = FeatureTable.read_text(args.input_meta).apply('value', 'value', process_single_meta, dtype='array<string>').apply('value', 'item', lambda x: x[0]).apply('value', 'category', lambda x: x[1]).drop('value')
    ...
    item_tbl = item_tbl.encode_string(['item', 'category'], [item_index, category_index]).fillna(0, ['item', 'category'])
    item_tbl.cache()
    full_tbl = transaction_tbl.encode_string(['item'], [item_index]).fillna(0, ['item']).add_hist_seq(cols=['item'], user_col='user', sort_col='time', min_len=2, max_len=100, num_seqs=1).add_negative_samples(item_size, item_col='item', neg_num=1).add_value_features(columns=['item', 'item_hist_seq'], dict_tbl=item_tbl, key='item', value='category').encode_string(['user'], [user_index]).fillna(0, ['user'])
    full_tbl = full_tbl.drop('time').apply('label', 'label', lambda x: [float(x), 1 - float(x)], 'array<float>').apply('item_hist_seq', 'item_hist_seq_len', len, 'int')
    ...
    for (test_auc, test_accuracy, total_recommendations, stats) in eval_res:
        if total_recommendations > 0:
            ...
            print(f'<Forward pass> ({total_recommendations} samples) infer_time={infer_time} | model_restore_time={model_restore_time} | init_time={init_time} | throughput={thpt_forward_pass} | accurary={test_accuracy} | auc={test_auc}')
    end = time.time()
