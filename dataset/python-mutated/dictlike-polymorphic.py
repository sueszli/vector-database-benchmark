...
if __name__ == '__main__':
    ...
    session.add(shrew)
    session.commit()
    q = session.query(Animal).filter(Animal.facts.any(and_(AnimalFact.key == 'weasel-like', AnimalFact.value == True)))
    print('weasel-like animals', q.all())
    q = session.query(Animal).filter(Animal.with_characteristic('weasel-like', True))
    ...
    q = session.query(Animal).filter(or_(Animal.with_characteristic('poisonous', False), ~Animal.facts.any(AnimalFact.key == 'poisonous')))
