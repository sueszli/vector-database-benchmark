...
class TranscodeValidation(ClaimTestCase):
    ...
    async def asyncSetUp(self):
        ...
        if not file_ogg.exists():
            command = f'-i "{self.video_file_name}" -c:v libtheora -q:v 4 -c:a libvorbis -q:a 4 -c:s copy -c:d copy "{file_ogg}"'
        ...
        if not file_webm.exists():
            command = f'-i "{self.video_file_name}" -c:v libvpx-vp9 -crf 36 -b:v 0 -cpu-used 2 -c:a libopus -b:a 128k -c:s copy -c:d copy "{file_webm}"'
    ...
    async def test_video_format(self):
        ...
        if not file_name.exists():
            command = f'-i "{self.video_file_name}" -c copy -map 0 -c:v libx264 -vf format=yuv444p "{file_name}"'
            with MeasureTime(f'Creating {file_name.name}'):
                (output, code) = await self.analyzer._execute_ffmpeg(command)
                self.assertEqual(code, 0, output)
