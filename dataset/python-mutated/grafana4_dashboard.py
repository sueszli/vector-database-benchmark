...
def present(name, base_dashboards_from_pillar=None, base_panels_from_pillar=None, base_rows_from_pillar=None, dashboard=None, orgname=None, profile='grafana'):
    if False:
        while True:
            i = 10
    ...
    if not old_dashboard:
        if __opts__['test']:
            ...
            return ret
        response = __salt__['grafana4.create_update_dashboard'](dashboard=new_dashboard, overwrite=True, profile=profile)
...
def absent(name, orgname=None, profile='grafana'):
    ...
    return ret
_IGNORED_DASHBOARD_FIELDS = ['id', 'uid', 'originalTitle', 'version']
_IGNORED_ROW_FIELDS = []
_IGNORED_PANEL_FIELDS = ['grid', 'mode', 'tooltip']
_IGNORED_TARGET_FIELDS = ['textEditor']
