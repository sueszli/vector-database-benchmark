...
class BaseParser:
    ...
    async def parse_query_and_authorize(self, value: Any, build_authorize: Callable, return_on_first_failure: bool=False, **kwargs) -> Tuple[Any, bool, ApiError]:
        ...
        (value_parsed, parser_found) = await self.__parse(self.query_parsers.get(self.__class__.__name__) or {}, value, authorize_status, error=error, **kwargs)
    ...
    async def parse_read_attributes_and_authorize(self, value: Any, build_authorize: Callable, **kwargs) -> Any:
        authorize_status = None
        error = None
        try:
            await build_authorize(value)
            ...
            error = err
            authorize_status = AuthorizeStatusType.FAILED
        (value_parsed, parser_found) = await self.__parse(self.read_parsers.get(self.__class__.__name__) or {}, value, authorize_status, **kwargs)
        if error:
            if parser_found:
                await build_authorize(value_parsed)
            else:
    ...
    async def parse_write_attributes_and_authorize(self, value: Any, build_authorize: Callable, **kwargs) -> Any:
        authorize_status = None
        error = None
        try:
            await build_authorize(value)
            ...
            error = err
            authorize_status = AuthorizeStatusType.FAILED
        (value_parsed, parser_found) = await self.__parse(self.write_parsers.get(self.__class__.__name__) or {}, value, authorize_status, **kwargs)
        if error:
            if parser_found:
                await build_authorize(value_parsed)
            else:
    ...
    async def __parse(self, parser_mapping: Dict, value: Any, authorize_status: AuthorizeStatusType, error: ApiError=None, **kwargs) -> Tuple[Any, bool]:
        api_operation_action = self.options.get('api_operation_action') or kwargs.get('api_operation_action') or OperationType.ALL
        parser = None
        parser_scope = parser_mapping.get(self.__current_scope()) or {}
        if parser_scope:
            parser_action = parser_scope.get(api_operation_action) or parser_scope.get(OperationType.ALL)
            if parser_action:
                parser = parser_action.get(authorize_status) or parser_action.get(AuthorizeStatusType.ALL)
        if not parser:
            return (value, False)
