...
class FailExpiredPauses(LoopService):
    ...
    def __init__(self, loop_seconds: float=None, **kwargs):
        if False:
            return 10
        super().__init__(loop_seconds=loop_seconds or PREFECT_API_SERVICES_PAUSE_EXPIRATIONS_LOOP_SECONDS.value(), **kwargs)
        self.batch_size = 200
    ...
    async def run_once(self, db: PrefectDBInterface):
        ...
        while True:
            async with db.session_context(begin_transaction=True) as session:
                query = sa.select(db.FlowRun).where(db.FlowRun.state_type == states.StateType.PAUSED).limit(self.batch_size)
    ...
    async def _mark_flow_run_as_failed(self, session: AsyncSession, flow_run: PrefectDBInterface.FlowRun) -> None:
        """
        ...
        if flow_run.state.state_details.pause_timeout < pendulum.now('UTC'):
            await models.flow_runs.set_flow_run_state(session=session, flow_run_id=flow_run.id, state=states.Failed(message='The flow was paused and never resumed.'), force=True)
