...
from tools.stats.import_test_stats import ADDITIONAL_CI_FILES_FOLDER, TEST_FILE_RATINGS_FILE
...
class CorrelatedWithHistoricalFailures(HeuristicInterface):
    ...
    def get_test_priorities(self, tests: List[str]) -> TestPrioritizations:
        if False:
            return 10
        correlated_tests = get_correlated_tests(ADDITIONAL_CI_FILES_FOLDER / TEST_FILE_RATINGS_FILE)
    ...
    def get_prediction_confidence(self, tests: List[str]) -> Dict[str, float]:
        if False:
            return 10
        test_ratings = get_ratings_for_tests(ADDITIONAL_CI_FILES_FOLDER / TEST_FILE_RATINGS_FILE)
