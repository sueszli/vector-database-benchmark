...
class IndexTrainTestLeakage(TrainTestCheck):
    ...
    def run_logic(self, context: Context) -> CheckResult:
        ...
        if len(index_intersection) > 0:
            ...
            if context.with_display:
                ...
                table = pd.DataFrame([[list(index_intersection)[:self.n_index_to_show]]], index=['Sample of test indexes in train:'])
                display = [text, table]
            else:
                display = None
        else:
    ...
    def add_condition_ratio_less_or_equal(self, max_ratio: float=0):
        ...
        return self.add_condition(f'Ratio of leaking indices is less or equal to {format_percent(max_ratio)}', max_ratio_condition)