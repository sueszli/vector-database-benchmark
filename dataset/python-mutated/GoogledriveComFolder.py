...
class GoogledriveComFolder(BaseDecrypter):
    ...
    __version__ = '0.14'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?drive\\.google\\.com/(?:folderview\\?.*id=|drive/(?:.+?/)?folders/)(?P<ID>[-\\w]+)'
    __config__ = [('enabled', 'bool', 'Activated', True), ('use_premium', 'bool', 'Use premium account if available', True), ('folder_per_package', 'Default;Yes;No', 'Create folder for each package', 'Default'), ('max_wait', 'int', 'Reconnect if waiting time is greater than minutes', 10), ('dl_subfolders', 'bool', 'Download subfolders', False), ('package_subfolder', 'bool', 'Subfolder as a seperate package', False)]
    ...
    NAME_PATTERN = "folderName: '(?P<N>.+?)'"
    OFFLINE_PATTERN = '<TITLE>'
    ...
    def enum_folder(self, folder_id):
        if False:
            while True:
                i = 10
        links = []
        json_data = self.api_request('files', q="'{}' in parents".format(folder_id), pageSize=100, fields='files/id,files/mimeType,nextPageToken', supportsAllDrives='true', includeItemsFromAllDrives='true')
        ...
        while next_page:
            json_data = self.api_request('files', q="'{}' in parents".format(folder_id), pageToken=next_page, pageSize=100, fields='files/id,files/mimeType,nextPageToken', supportsAllDrives='true', includeItemsFromAllDrives='true')
            if json_data is None:
                self.fail('API error')
            if 'error' in json_data:
                self.fail(json_data['error']['message'])
    ...
    def decrypt(self, pyfile):
        if False:
            return 10
        links = []
        json_data = self.api_request('files/{}'.format(self.info['pattern']['ID']), supportsAllDrives='true')
        if json_data is None:
            self.fail('API error')
        if 'error' in json_data:
