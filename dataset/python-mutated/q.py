...
SHOW_SQL = False
sha_algorithms = {1: hashlib.sha1, 224: hashlib.sha224, 256: hashlib.sha256, 386: hashlib.sha384, 512: hashlib.sha512}
...
user_functions = [UserFunctionDef(FunctionType.REGULAR, 'regexp', 'regexp(<regular_expression>,<expr>) = <1|0>', 'Find regexp in string expression. Returns 1 if found or 0 if not', regexp, 2), UserFunctionDef(FunctionType.REGULAR, 'regexp_extract', 'regexp_extract(<regular_expression>,<expr>,group_number) = <substring|null>', 'Get regexp capture group content', regexp_extract, 3), UserFunctionDef(FunctionType.REGULAR, 'sha', 'sha(<expr>,<encoding>,<algorithm>) = <hex-string-of-sha>', 'Calculate sha of some expression. Algorithm can be one of 1,224,256,384,512. For now encoding must be manually provided. Will use the input encoding automatically in the future.', sha, 3), UserFunctionDef(FunctionType.REGULAR, 'sha1', 'sha1(<expr>) = <hex-string-of-sha>', "Exists for backward compatibility only, since it doesn't handle encoding properly. Calculates sha1 of some expression", sha1, 1), UserFunctionDef(FunctionType.REGULAR, 'md5', 'md5(<expr>,<encoding>) = <hex-string-of-md5>', 'Calculate md5 of expression. Returns a hex-string of the result. Currently requires to manually provide the encoding of the data. Will be taken automatically from the input encoding in the future.', md5, 2), UserFunctionDef(FunctionType.REGULAR, 'sqrt', 'sqrt(<expr>) = <square-root>', 'Calculate the square root of the expression', sqrt, 1), UserFunctionDef(FunctionType.REGULAR, 'power', 'power(<expr1>,<expr2>) = <expr1-to-the-power-of-expr2>', 'Raise expr1 to the power of expr2', power, 2), UserFunctionDef(FunctionType.REGULAR, 'file_ext', 'file_ext(<expr>) = <filename-extension-or-empty-string>', 'Get the extension of a filename', file_ext, 1), UserFunctionDef(FunctionType.REGULAR, 'file_folder', 'file_folder(<expr>) = <folder-name-of-filename>', 'Get the folder part of a filename', file_folder, 1), UserFunctionDef(FunctionType.REGULAR, 'file_basename', 'file_basename(<expr>) = <basename-of-filename-including-extension>', 'Get the basename of a filename, including extension if any', file_basename, 1), UserFunctionDef(FunctionType.REGULAR, 'file_basename_no_ext', 'file_basename_no_ext(<expr>) = <basename-of-filename-without-extension>', 'Get the basename of a filename, without the extension if there is one', file_basename_no_ext, 1), UserFunctionDef(FunctionType.AGG, 'percentile', 'percentile(<expr>,<percentile-in-the-range-0-to-1>) = <percentile-value>', 'Calculate the strict percentile of a set of a values.', StrictPercentile, 2), UserFunctionDef(FunctionType.AGG, 'stddev_pop', 'stddev_pop(<expr>) = <stddev-value>', 'Calculate the population standard deviation of a set of values', StdevPopulation, 1), UserFunctionDef(FunctionType.AGG, 'stddev_sample', 'stddev_sample(<expr>) = <stddev-value>', 'Calculate the sample standard deviation of a set of values', StdevSample, 1)]
...
def sqlite_type_to_python_type(sqlite_type):
    if False:
        for i in range(10):
            print('nop')
    SQLITE_AFFINITY_TO_PYTHON_TYPE_NAMES = {'INTEGER': long, 'TEXT': unicode, 'BLOB': bytes, 'REAL': float, 'NUMERIC': float}
...
class Sqlite3DB(object):
    ...
    def add_to_qcatalog_table(self, temp_table_name, content_signature, creation_time, source_type, source):
        ...
        r = self.execute_and_fetch('INSERT INTO %s (qcatalog_entry_id,content_signature_key, temp_table_name,content_signature,creation_time,source_type,source) VALUES (?,?,?,?,?,?,?)' % self.QCATALOG_TABLE_NAME, (str(uuid4()), content_signature_key, temp_table_name, json.dumps(content_signature), creation_time, source_type, source))
        self.conn.commit()
    ...
    def generate_create_table(self, table_name, column_names, column_dict):
        if False:
            print('Hello World!')
        column_name_to_db_type = dict(((n, Sqlite3DB.PYTHON_TO_SQLITE_TYPE_NAMES[t]) for (n, t) in six.iteritems(column_dict)))
        ...
        while idx < len(self.sql_parts):
            ...
            if part.upper() in ['FROM', 'JOIN']:
                if idx == len(self.sql_parts) - 1:
                    raise InvalidQueryException('FROM/JOIN is missing a table name after it')
    ...
    def set_effective_table_name(self, qtable_name, effective_table_name):
        ...
        if qtable_name in self.qtable_name_effective_table_names.keys():
            if self.qtable_name_effective_table_names[qtable_name] != effective_table_name:
                raise Exception('Already set effective table name for qtable %s. Trying to change the effective table name from %s to %s' % (qtable_name, self.qtable_name_effective_table_names[qtable_name], effective_table_name))
...
class TableColumnInferer(object):
    ...
    def _generate_content_signature(self):
        if False:
            for i in range(10):
                print('nop')
        return OrderedDict({'inferred': self.inferred, 'mode': self.mode, 'rows': '\n'.join([','.join(x) for x in self.rows]), 'skip_header': self.skip_header, 'header_row': self.header_row, 'expected_column_count': self.expected_column_count, 'input_delimiter': self.input_delimiter, 'disable_column_type_detection': self.disable_column_type_detection})
    ...
    def determine_type_of_value_list(self, value_list):
        ...
        if len(set(type_list)) == 1:
            ...
            type_list_without_nulls = list(filter(lambda x: x is not None, type_list))
        ...
        if self.header_row is not None:
            ...
            if len(self.header_row) < self.column_count:
                if self.mode == 'strict':
                    ...
                    self.header_row = self.header_row + ['c%s' % (x + len(self.header_row) + 1) for x in range(self.column_count - len(self.header_row))]
    ...
    def _do_relaxed_analysis(self):
        ...
        elif self.expected_column_count is not None:
...
class MaterializedDelimitedFileState(MaterializedState):
    ...
    def materialize_file_list(self, qtable_name):
        ...
        if len(unfound_files) == 1:
            raise FileNotFoundException("No files matching '%s' have been found" % unfound_files[0])
        elif len(unfound_files) > 1:
            raise FileNotFoundException('The following files have not been found for table %s: %s' % (qtable_name, ','.join(unfound_files)))
    ...
    def __analyze_delimited_file(self, database_info):
        ...
        table_creator = TableCreator(self.qtable_name, self.delimited_file_reader, self.input_params, sqlite_db=database_info.sqlite_db, target_sqlite_table_name=target_sqlite_table_name)
        ...
        database_info.sqlite_db.add_to_qcatalog_table(target_sqlite_table_name, self.content_signature, now, self.source_type, self.source)
    ...
    def make_data_available(self, stop_after_analysis):
        ...
        self.mfs_structure = MaterializedStateTableStructure(self.qtable_name, self.atomic_fns, self.db_id, table_creator.column_inferer.get_column_names(), table_creator.column_inferer.get_column_types(), None, self.target_table_name, self.source_type, self.source, self.get_planned_table_name())
...
class MaterialiedDataStreamState(MaterializedDelimitedFileState):
    ...
    def __init__(self, table_source_type, qtable_name, input_params, dialect_id, engine_id, data_stream, stream_target_db):
        if False:
            while True:
                i = 10
...
class MaterializedSqliteState(MaterializedState):
    ...
    def make_data_available(self, stop_after_analysis):
        ...
        self.mfs_structure = MaterializedStateTableStructure(self.qtable_name, [self.qtable_name], self.db_id, column_names, column_types, sqlite_column_types, self.table_name, self.source_type, self.source, self.get_planned_table_name())
        return (database_info, relevant_table)
...
class MaterializedQsqlState(MaterializedState):
    ...
    def choose_db_to_use(self, forced_db_to_use=None, stop_after_analysis=False):
        ...
        if forced_db_to_use:
            ...
            forced_db_to_use.add_to_qcatalog_table(new_table_name, cs, d['creation_time'], d['source_type'], d['source'])
            self.table_name = new_table_name
            self.db_id = forced_db_to_use.db_id
            self.db_to_use = forced_db_to_use
        return
    ...
    def _extract_information(self):
        ...
        sqlite_column_types = list(map(lambda x: x[2].lower(), table_info))
        column_types = list(map(lambda x: sqlite_type_to_python_type(x[2]), table_info))
        self.content_signature = OrderedDict(**json.loads(x['content_signature']))
...
class TableCreator(object):
    ...
    def validate_extra_header_if_needed(self, file_number, filename, col_vals):
        ...
        if header_already_exists:
            ...
            if tuple(self.column_inferer.header_row) != tuple(col_vals):
                raise BadHeaderException("Extra header '{}' in file '{}' mismatches original header '{}' from file '{}'. Table name is '{}'".format(','.join(col_vals), filename, ','.join(self.column_inferer.header_row), self.column_inferer.header_row_filename, self.qtable_name))
    ...
    def _populate(self, dialect, stop_after_analysis=False):
        ...
        try:
            try:
                ...
                raise ColumnCountMismatchException('Strict mode - Expected %s columns instead of %s columns in file %s row %s. Either use relaxed modes or check your delimiter' % (e.expected_col_count, e.actual_col_count, normalized_filename(e.atomic_fn), e.lines_read))
            except FluffyModeColumnCountMismatchException as e:
                raise ColumnCountMismatchException('Deprecated fluffy mode - Too many columns in file %s row %s (%s fields instead of %s fields). Consider moving to either relaxed or strict mode' % (normalized_filename(e.atomic_fn), e.lines_read, e.actual_col_count, e.expected_col_count))
        finally:
            self._flush_inserts()
    ...
    def initialize_numeric_column_indices_if_needed(self):
        ...
        if self.numeric_column_indices is None:
            ...
            self.numeric_column_indices = [idx for (idx, column_type) in enumerate(column_types) if self.sqlite_db.is_numeric_type(column_type)]
    ...
    def normalize_col_vals(self, col_vals):
        ...
        if self.mode == 'strict':
            ...
            return col_vals
        if actual_col_count < expected_col_count:
            col_vals = col_vals + [None for x in range(expected_col_count - actual_col_count)]
        if self.mode == 'relaxed':
            if actual_col_count > expected_col_count:
                xxx = col_vals[:expected_col_count - 1] + [self.input_delimiter.join([v if v is not None else '' for v in col_vals[expected_col_count - 1:]])]
                return xxx
            else:
                return col_vals
    ...
    def _flush_inserts(self):
        ...
        if len(self.buffered_inserts) > 0:
            insert_row_stmt = self.sqlite_db.generate_insert_row(self.target_sqlite_table_name, self.effective_column_names)
    ...
    def _do_create_table(self, filename):
        ...
        create_table_stmt = self.sqlite_db.generate_create_table(self.target_sqlite_table_name, ordered_column_names, column_dict)
...
class QInputParams(object):
    ...
    def __init__(self, skip_header=False, delimiter=' ', input_encoding='UTF-8', gzipped_input=False, with_universal_newlines=False, parsing_mode='relaxed', expected_column_count=None, keep_leading_whitespace_in_values=False, disable_double_double_quoting=False, disable_escaped_double_quoting=False, disable_column_type_detection=False, input_quoting_mode='minimal', stdin_file=None, stdin_filename='-', max_column_length_limit=131072, read_caching=False, write_caching=False, max_attached_sqlite_databases=10):
        if False:
            return 10
...
class QTextAsData(object):
    ...
    def __init__(self, default_input_params=QInputParams(), data_streams_dict=None):
        ...
        self.query_level_db = Sqlite3DB(self.query_level_db_id, 'file:%s?mode=memory&cache=shared' % self.query_level_db_id, '<query-level-db>', create_qcatalog=True)
    ...
    input_quoting_modes = {'minimal': csv.QUOTE_MINIMAL, 'all': csv.QUOTE_ALL, 'none': csv.QUOTE_NONE}
    ...
    def determine_proper_dialect(self, input_params):
        ...
        dialect = {'skipinitialspace': skip_initial_space, 'delimiter': input_params.delimiter, 'quotechar': '"'}
    ...
    def _open_files_and_get_mfss(self, qtable_name, input_params, dialect):
        ...
        if materialized_state_type == MaterializedStateType.DATA_STREAM:
            ...
            ms = MaterializedQsqlState(table_source_type, qtable_name, qsql_filename=qsql_filename, table_name=table_name, engine_id=self.engine_id, input_params=input_params, dialect_id=dialect)
            ...
            ms = MaterializedSqliteState(table_source_type, qtable_name, sqlite_filename=sqlite_filename, table_name=table_name, engine_id=self.engine_id)
    ...
    def _load_data(self, qtable_name, input_params=QInputParams(), stop_after_analysis=False):
        ...
        mfss = self._open_files_and_get_mfss(qtable_name, input_params, dialect_id)
        ...
        for qtable_name in sql_object.qtable_names:
            ...
            for ts in relevant_table_structures:
                ...
                if column_names is None:
                    ...
                    raise BadHeaderException('Column names differ for table %s: %s vs %s' % (qtable_name, ','.join(column_names), ','.join(names)))
                xprint('Comparing column types: %s with %s' % (column_types, types))
                if column_types is None:
                    column_types = types
                elif column_types != types:
                    raise BadHeaderException('Column types differ for table %s: %s vs %s' % (qtable_name, ','.join(column_types), ','.join(types)))
    ...
    def _execute(self, query_str, input_params=None, data_streams=None, stop_after_analysis=False, save_db_to_disk_filename=None):
        ...
        try:
            ...
            return QOutput(data=db_results_obj.results, metadata=QMetadata(table_structures=self.loaded_table_structures_dict, new_table_structures=new_table_structures, output_column_name_list=db_results_obj.query_column_names), warnings=warnings, error=error)
            ...
            error = QError(e, "%s vs %s: Content Signatures for table %s differ at %s (source value '%s' disk signature value '%s')" % (e.original_filename, e.other_filename, e.filenames_str, e.key, e.source_value, e.signature_value), 80)
...
class QOutputParams(object):
    ...
    def __init__(self, delimiter=' ', beautify=False, output_quoting_mode='minimal', formatting=None, output_header=False, encoding=None):
        if False:
            return 10
        self.delimiter = delimiter
...
class QOutputPrinter(object):
    output_quoting_modes = {'minimal': quote_minimal_func, 'all': quote_all_func, 'nonnumeric': quote_nonnumeric_func, 'none': quote_none_func}
    ...
    def _print_output(self, f_out, f_err, results):
        ...
        if self.output_params.formatting:
            formatting_dict = dict([(x.split('=')[0], x.split('=')[1]) for x in self.output_params.formatting.split(',')])
        else:
            formatting_dict = {}
        try:
            ...
            for (rownum, row) in enumerate(data):
                ...
                for (i, col) in enumerate(row):
                    ...
                    elif self.output_params.beautify:
...
def initialize_command_line_parser(p, qrc_filename):
    ...
    try:
        ...
        default_keep_leading_whitespace_in_values = get_option_with_default(p, 'boolean', 'keep_leading_whitespace_in_values', False)
        default_disable_double_double_quoting = get_option_with_default(p, 'boolean', 'disable_double_double_quoting', True)
        default_disable_escaped_double_quoting = get_option_with_default(p, 'boolean', 'disable_escaped_double_quoting', True)
        default_disable_column_type_detection = get_option_with_default(p, 'boolean', 'disable_column_type_detection', False)
    ...
    parser.add_argument('-V', '--verbose', default=default_verbose, action='store_true', help='Print debug info in case of problems')
    parser.add_argument('-S', '--save-db-to-disk', dest='save_db_to_disk_filename', default=default_save_db_to_disk, help='Save database to an sqlite database file')
    parser.add_argument('-C', '--caching-mode', default=default_caching_mode, help='Choose the autocaching mode (none/read/readwrite). Autocaches files to disk db so further queries will be faster. Caching is done to a side-file with the same name of the table, but with an added extension .qsql')
    parser.add_argument('--dump-defaults', action='store_true', help='Dump all default values for parameters and exit. Can be used in order to make sure .qrc file content is being read properly.')
    parser.add_argument('--max-attached-sqlite-databases', default=default_max_attached_sqlite_databases, type=int, help="Set the maximum number of concurrently-attached sqlite dbs. This is a compile time definition of sqlite. q's performance will slow down once this limit is reached for a query, since it will perform table copies in order to avoid that limit.")
    input_data_option_group = parser.add_argument_group('Input Data Options')
    input_data_option_group.add_argument('-H', '--skip-header', default=default_skip_header, action='store_true', help='Skip header row. This has been changed from earlier version - Only one header row is supported, and the header row is used for column naming')
    input_data_option_group.add_argument('-d', '--delimiter', default=default_delimiter, help='Field delimiter. If none specified, then space is used as the delimiter.')
    input_data_option_group.add_argument('-p', '--pipe-delimited', default=default_pipe_delimited, action='store_true', help="Same as -d '|'. Added for convenience and readability")
    input_data_option_group.add_argument('-t', '--tab-delimited', default=default_tab_delimited, action='store_true', help="Same as -d <tab>. Just a shorthand for handling standard tab delimited file You can use $'\\t' if you want (this is how Linux expects to provide tabs in the command line")
    input_data_option_group.add_argument('-e', '--encoding', default=default_encoding, help='Input file encoding. Defaults to UTF-8. set to none for not setting any encoding - faster, but at your own risk...')
    input_data_option_group.add_argument('-z', '--gzipped', default=default_gzipped, action='store_true', help='Data is gzipped. Useful for reading from stdin. For files, .gz means automatic gunzipping')
    input_data_option_group.add_argument('-A', '--analyze-only', default=default_analyze_only, action='store_true', help='Analyze sample input and provide information about data types')
    input_data_option_group.add_argument('-m', '--mode', default=default_mode, help='Data parsing mode. fluffy, relaxed and strict. In strict mode, the -c column-count parameter must be supplied as well')
    input_data_option_group.add_argument('-c', '--column-count', default=default_column_count, help='Specific column count when using relaxed or strict mode')
    input_data_option_group.add_argument('-k', '--keep-leading-whitespace', dest='keep_leading_whitespace_in_values', default=default_keep_leading_whitespace_in_values, action='store_true', help='Keep leading whitespace in values. Default behavior strips leading whitespace off values, in order to provide out-of-the-box usability for simple use cases. If you need to preserve whitespace, use this flag.')
    input_data_option_group.add_argument('--disable-double-double-quoting', default=default_disable_double_double_quoting, action='store_false', help='Disable support for double double-quoting for escaping the double quote character. By default, you can use "" inside double quoted fields to escape double quotes. Mainly for backward compatibility.')
    input_data_option_group.add_argument('--disable-escaped-double-quoting', default=default_disable_escaped_double_quoting, action='store_false', help='Disable support for escaped double-quoting for escaping the double quote character. By default, you can use \\" inside double quoted fields to escape double quotes. Mainly for backward compatibility.')
    input_data_option_group.add_argument('--as-text', dest='disable_column_type_detection', default=default_disable_column_type_detection, action='store_true', help="Don't detect column types - All columns will be treated as text columns")
    input_data_option_group.add_argument('-w', '--input-quoting-mode', default=default_input_quoting_mode, help='Input quoting mode. Possible values are all, minimal and none. Note the slightly misleading parameter name, and see the matching -W parameter for output quoting.')
    input_data_option_group.add_argument('-M', '--max-column-length-limit', default=default_max_column_length_limit, help='Sets the maximum column length.')
    input_data_option_group.add_argument('-U', '--with-universal-newlines', default=default_with_universal_newlines, action='store_true', help='Expect universal newlines in the data. Limitation: -U works only with regular files for now, stdin or .gz files are not supported yet.')
    output_data_option_group = parser.add_argument_group('Output Options')
    output_data_option_group.add_argument('-D', '--output-delimiter', default=default_output_delimiter, help='Field delimiter for output. If none specified, then the -d delimiter is used if present, or space if no delimiter is specified')
    output_data_option_group.add_argument('-P', '--pipe-delimited-output', default=default_pipe_delimited_output, action='store_true', help="Same as -D '|'. Added for convenience and readability.")
    output_data_option_group.add_argument('-T', '--tab-delimited-output', default=default_tab_delimited_output, action='store_true', help="Same as -D <tab>. Just a shorthand for outputting tab delimited output. You can use -D $'\\t' if you want.")
    output_data_option_group.add_argument('-O', '--output-header', default=default_output_header, action='store_true', help="Output header line. Output column-names are determined from the query itself. Use column aliases in order to set your column names in the query. For example, 'select name FirstName,value1/value2 MyCalculation from ...'. This can be used even if there was no header in the input.")
    output_data_option_group.add_argument('-b', '--beautify', default=default_beautify, action='store_true', help='Beautify output according to actual values. Might be slow...')
    output_data_option_group.add_argument('-f', '--formatting', default=default_formatting, help='Output-level formatting, in the format X=fmt,Y=fmt etc, where X,Y are output column numbers (e.g. 1 for first SELECT column etc.')
    output_data_option_group.add_argument('-E', '--output-encoding', default=default_output_encoding, help="Output encoding. Defaults to 'none', leading to selecting the system/terminal encoding")
    output_data_option_group.add_argument('-W', '--output-quoting-mode', default=default_output_quoting_mode, help='Output quoting mode. Possible values are all, minimal, nonnumeric and none. Note the slightly misleading parameter name, and see the matching -w parameter for input quoting.')
    output_data_option_group.add_argument('-L', '--list-user-functions', default=default_list_user_functions, action='store_true', help='List all user functions')
    parser.add_argument('--overwrite-qsql', default=default_overwrite_qsql, help='When used, qsql files (both caches and store-to-db) will be overwritten if they already exist. Use with care.')
    query_option_group = parser.add_argument_group('Query Related Options')
    query_option_group.add_argument('-q', '--query-filename', default=default_query_filename, help='Read query from the provided filename instead of the command line, possibly using the provided query encoding (using -Q).')
    query_option_group.add_argument('-Q', '--query-encoding', default=default_query_encoding, help='query text encoding. Experimental. Please send your feedback on this')
...
def initialize_default_data_streams():
    if False:
        return 10
    data_streams_dict = {'-': DataStream('stdin', '-', sys.stdin)}
...
def parse_options(args, options):
    ...
    if options.query_filename is not None:
        ...
        try:
            ...
            sys.exit(1)
    elif sys.stdin.encoding is not None:
    ...
    try:
        ...
        print('Max column length limit must be an integer larger than 2 (%s)' % options.max_column_length_limit, file=sys.stderr)
        sys.exit(31)
