...
from xonsh.platform import DEFAULT_ENCODING, ON_CYGWIN, ON_DARWIN, ON_LINUX, ON_MSYS, ON_POSIX, ON_WINDOWS, ON_WSL, ON_WSL1, PYTHON_VERSION_INFO, githash, is_readline_available, linux_distro, ptk_version, pygments_version
from xonsh.ply import ply
from xonsh.prompt.base import is_template_string
from xonsh.tools import color_style, color_style_names, is_string, is_superuser, print_color, print_exception, to_bool
...
_XONFIG_SOURCE_FOREIGN_SHELL_COMMAND: dict[str, str] = collections.defaultdict(lambda : 'source-foreign', bash='source-bash', cmd='source-cmd', zsh='source-zsh')
events.doc('on_xonfig_info_requested', '\non_xonfig_info_requested() -> list[tuple[str, str]]\n\nRegister a callable that will return extra info when ``xonfig info`` is called.\n')
