...
for (name, kind) in kinds.items():
    impls['bisect'][name] = {'setup': fill_values, 'ctor': kind, 'func': 'bisect', 'limit': 1000000}
for (name, kind) in kinds.items():
...
for (name, kind) in kinds.items():
    impls['index'][name] = {'setup': fill_values, 'ctor': kind, 'func': 'index', 'limit': 1000000}
for (name, kind) in kinds.items():
    impls['iter'][name] = {'setup': fill_values, 'ctor': kind, 'func': '__iter__', 'limit': 1000000}
for (name, kind) in kinds.items():
    impls['count'][name] = {'setup': fill_values, 'ctor': kind, 'func': 'count', 'limit': 1000000}
        ...
        if pos < 40:
            ...
            for (idx, temp) in enumerate(obj):
                if idx > 100:
                    break
for (name, kind) in kinds.items():
    impls['priorityqueue'][name] = {'setup': fill_values, 'ctor': PriorityQueue(kind), 'func': 'run', 'limit': 1000000}
...
class Multiset(Mixed):
    ...
    def run(self, value):
        ...
        if pos < 75:
            ...
            obj[value]
for (name, kind) in kinds.items():
    impls['multiset'][name] = {'setup': fill_values, 'ctor': Multiset(kind), 'func': 'run', 'limit': 1000000}
...
class Ranking(Mixed):
    ...
    def run(self, value):
        ...
        if pos < 40:
            ...
            obj.remove(value)
for (name, kind) in kinds.items():
    impls['ranking'][name] = {'setup': fill_values, 'ctor': Ranking(kind), 'func': 'run', 'limit': 1000000}
        ...
        if pos < 75:
            ...
            for (idx, temp) in enumerate(obj):
                if idx > 100:
                    break
for (name, kind) in kinds.items():
    impls['neighbor'][name] = {'setup': fill_values, 'ctor': Neighbor(kind), 'func': 'run', 'limit': 1000000}
...
class Intervals(Mixed):
    ...
    def run(self, value):
        ...
        if pos < 30:
            ...
            obj.discard(value)
for (name, kind) in kinds.items():
    impls['intervals'][name] = {'setup': fill_values, 'ctor': Intervals(kind), 'func': 'run', 'limit': 1000000}
limit('intervals', 'List', 100000)
for (name, kind) in kinds.items():
    impls['init'][name] = {'setup': do_nothing, 'ctor': kind, 'func': '__init__', 'limit': 1000000}
