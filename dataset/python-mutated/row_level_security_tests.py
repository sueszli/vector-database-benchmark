...
from tests.integration_tests.fixtures.unicode_dashboard import UNICODE_TBL_NAME, load_unicode_dashboard_with_slice, load_unicode_data
...
class TestRowLevelSecurity(SupersetTestCase):
    ...
    """
    rls_entry = None
    query_obj: dict[str, Any] = dict(groupby=[], metrics=None, filter=[], is_timeseries=False, columns=['value'], granularity=None, from_dttm=None, to_dttm=None, extras={})
    NAME_AB_ROLE = 'NameAB'
    NAME_Q_ROLE = 'NameQ'
    NAMES_A_REGEX = re.compile("name like 'A%'")
    NAMES_B_REGEX = re.compile("name like 'B%'")
    NAMES_Q_REGEX = re.compile("name like 'Q%'")
    BASE_FILTER_REGEX = re.compile("gender = 'boy'")
...
RLS_ALICE_REGEX = re.compile("name = 'Alice'")
RLS_GENDER_REGEX = re.compile("AND \\(gender = 'girl'\\)")

@mock.patch.dict('superset.extensions.feature_flag_manager._feature_flags', EMBEDDED_SUPERSET=True)
