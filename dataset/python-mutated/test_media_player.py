...
from pycec.const import DEVICE_TYPE_NAMES, KEY_BACKWARD, KEY_FORWARD, KEY_MUTE_TOGGLE, KEY_PAUSE, KEY_PLAY, KEY_STOP, KEY_VOLUME_DOWN, KEY_VOLUME_UP, POWER_OFF, POWER_ON, STATUS_PLAY, STATUS_STILL, STATUS_STOP, TYPE_AUDIO, TYPE_PLAYBACK, TYPE_RECORDER, TYPE_TUNER, TYPE_TV, TYPE_UNKNOWN
import pytest
from homeassistant.components.hdmi_cec import EVENT_HDMI_CEC_UNAVAILABLE
from homeassistant.components.media_player import DOMAIN as MEDIA_PLAYER_DOMAIN, MediaPlayerEntityFeature as MPEF
...
async def test_load_platform(hass: HomeAssistant, create_hdmi_network, create_cec_entity) -> None:
...
async def test_load_types(hass: HomeAssistant, create_hdmi_network, create_cec_entity, platform) -> None:
...
async def test_service_on(hass: HomeAssistant, create_hdmi_network, create_cec_entity, assert_state) -> None:
    ...
    await hass.services.async_call(MEDIA_PLAYER_DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: 'media_player.hdmi_3'}, blocking=True)
    await hass.async_block_till_done()
    mock_hdmi_device.turn_on.assert_called_once_with()
    state = hass.states.get('media_player.hdmi_3')
    assert_state(state.state, STATE_ON)

async def test_service_off(hass: HomeAssistant, create_hdmi_network, create_cec_entity, assert_state) -> None:
    ...
    await hass.services.async_call(MEDIA_PLAYER_DOMAIN, SERVICE_TURN_OFF, {ATTR_ENTITY_ID: 'media_player.hdmi_3'}, blocking=True)
...
async def test_supported_features(hass: HomeAssistant, create_hdmi_network, create_cec_entity, type_id, expected_features) -> None:
    """Test that features load as expected."""
    hdmi_network = await create_hdmi_network({'platform': 'media_player'})
    mock_hdmi_device = MockHDMIDevice(logical_address=3, type=type_id, type_name=DEVICE_TYPE_NAMES[type_id])
    await create_cec_entity(hdmi_network, mock_hdmi_device)
    state = hass.states.get('media_player.hdmi_3')
...
@pytest.mark.parametrize(('service', 'extra_data', 'key'), [(SERVICE_VOLUME_DOWN, None, KEY_VOLUME_DOWN), (SERVICE_VOLUME_UP, None, KEY_VOLUME_UP), (SERVICE_VOLUME_MUTE, {'is_volume_muted': True}, KEY_MUTE_TOGGLE), (SERVICE_VOLUME_MUTE, {'is_volume_muted': False}, KEY_MUTE_TOGGLE)])
async def test_volume_services(hass: HomeAssistant, create_hdmi_network, create_cec_entity, service, extra_data, key) -> None:
    ...
    if extra_data:
        data |= extra_data
    await hass.services.async_call(MEDIA_PLAYER_DOMAIN, service, data, blocking=True)
    await hass.async_block_till_done()
    assert mock_hdmi_device.send_command.call_count == 2
    assert_key_press_release(mock_hdmi_device.send_command, dst=3, key=key)

@pytest.mark.parametrize(('service', 'key'), [(SERVICE_MEDIA_NEXT_TRACK, KEY_FORWARD), (SERVICE_MEDIA_PREVIOUS_TRACK, KEY_BACKWARD)])
async def test_track_change_services(hass: HomeAssistant, create_hdmi_network, create_cec_entity, service, key) -> None:
    """Test track change related commands."""
    hdmi_network = await create_hdmi_network({'platform': 'media_player'})
    mock_hdmi_device = MockHDMIDevice(logical_address=3, type=TYPE_RECORDER)
    await create_cec_entity(hdmi_network, mock_hdmi_device)
    await hass.services.async_call(MEDIA_PLAYER_DOMAIN, service, {ATTR_ENTITY_ID: 'media_player.hdmi_3'}, blocking=True)
    await hass.async_block_till_done()
    assert mock_hdmi_device.send_command.call_count == 2
    assert_key_press_release(mock_hdmi_device.send_command, dst=3, key=key)

@pytest.mark.parametrize(('service', 'key', 'expected_state'), [pytest.param(SERVICE_MEDIA_PLAY, KEY_PLAY, STATE_PLAYING, marks=pytest.mark.xfail(reason='The wrong feature is defined, should be PLAY, not PLAY_MEDIA')), (SERVICE_MEDIA_PAUSE, KEY_PAUSE, STATE_PAUSED), (SERVICE_MEDIA_STOP, KEY_STOP, STATE_IDLE)])
async def test_playback_services(hass: HomeAssistant, create_hdmi_network, create_cec_entity, assert_state, service, key, expected_state) -> None:
...
async def test_play_pause_service(hass: HomeAssistant, create_hdmi_network, create_cec_entity, assert_state) -> None:
    """Test play pause service."""
    hdmi_network = await create_hdmi_network({'platform': 'media_player'})
    mock_hdmi_device = MockHDMIDevice(logical_address=3, type=TYPE_RECORDER, status=STATUS_PLAY)
    await create_cec_entity(hdmi_network, mock_hdmi_device)
    await hass.services.async_call(MEDIA_PLAYER_DOMAIN, SERVICE_MEDIA_PLAY_PAUSE, {ATTR_ENTITY_ID: 'media_player.hdmi_3'}, blocking=True)
    await hass.async_block_till_done()
    assert mock_hdmi_device.send_command.call_count == 2
...
async def test_update_state(hass: HomeAssistant, create_hdmi_network, create_cec_entity, type_id, update_data, expected_state) -> None:
...
async def test_starting_state(hass: HomeAssistant, create_hdmi_network, create_cec_entity, data, expected_state) -> None:
...
@pytest.mark.xfail(reason="The code only sets the state to unavailable, doesn't set the `_attr_available` to false.")
async def test_unavailable_status(hass: HomeAssistant, create_hdmi_network, create_cec_entity) -> None:
