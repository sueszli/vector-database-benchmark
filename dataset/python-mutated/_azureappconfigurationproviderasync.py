...
import logging
from typing import Any, Awaitable, Callable, Dict, Iterable, Mapping, Optional, overload, List, Tuple, TYPE_CHECKING, Union
...
@overload
async def load(endpoint: str, credential: 'AsyncTokenCredential', *, selects: Optional[List[SettingSelector]]=None, trim_prefixes: Optional[List[str]]=None, key_vault_options: Optional[AzureAppConfigurationKeyVaultOptions]=None, refresh_on: Optional[List[Tuple[str, str]]]=None, refresh_interval: int=30, on_refresh_success: Optional[Callable]=None, on_refresh_error: Optional[Callable[[Exception], Awaitable[None]]]=None, **kwargs) -> 'AzureAppConfigurationProvider':
    """
...
@overload
async def load(*, connection_string: str, selects: Optional[List[SettingSelector]]=None, trim_prefixes: Optional[List[str]]=None, key_vault_options: Optional[AzureAppConfigurationKeyVaultOptions]=None, refresh_on: Optional[List[Tuple[str, str]]]=None, refresh_interval: int=30, on_refresh_success: Optional[Callable]=None, on_refresh_error: Optional[Callable[[Exception], Awaitable[None]]]=None, **kwargs) -> 'AzureAppConfigurationProvider':
    """
    Loads configuration settings from Azure App Configuration into a Python application.
...
async def _resolve_keyvault_reference(config: 'SecretReferenceConfigurationSetting', provider: 'AzureAppConfigurationProvider') -> str:
        ...
        try:
            async with self._update_lock:
                ...
                for ((key, label), etag) in updated_sentinel_keys.items():
                    try:
                        updated_sentinel = await self._client.get_configuration_setting(key=key, label=label, etag=etag, match_condition=MatchConditions.IfModified, headers=headers, **kwargs)
