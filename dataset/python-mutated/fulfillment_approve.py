...
class FulfillmentApprove(BaseMutation):
    ...
    class Arguments:
        ...
        notify_customer = graphene.Boolean(required=True, description='True if confirmation email should be send.')
        allow_stock_to_be_exceeded = graphene.Boolean(default_value=False, description='True if stock could be exceeded.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.FULFILLMENT_APPROVED, description='Fulfillment is approved.')]
