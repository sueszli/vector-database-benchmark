...
USAGE_MSG = "Usage: '!!swagger <filename>!!' or '!!swagger-http <url>!!'. File must either exist locally and be placed next to the .md that contains the swagger statement, or be an http(s) URL."
...
TOKEN = re.compile('!!swagger(?: (?P<path>[^\\\\/\\s><&:]+))?!!')
TOKEN_HTTP = re.compile('!!swagger-http(?: (?P<path>https?://[^\\s]+))?!!')
