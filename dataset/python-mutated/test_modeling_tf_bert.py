...
if is_tf_available():
    ...
    from transformers.models.bert.modeling_tf_bert import TFBertForMaskedLM, TFBertForMultipleChoice, TFBertForNextSentencePrediction, TFBertForPreTraining, TFBertForQuestionAnswering, TFBertForSequenceClassification, TFBertForTokenClassification, TFBertLMHeadModel, TFBertModel
...
class TFBertModelTester:
    ...
    def create_and_check_for_next_sequence_prediction(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            while True:
                i = 10
...
class TFBertModelTest(TFModelTesterMixin, TFCoreModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TFBertModel, TFBertForMaskedLM, TFBertLMHeadModel, TFBertForNextSentencePrediction, TFBertForPreTraining, TFBertForQuestionAnswering, TFBertForSequenceClassification, TFBertForTokenClassification, TFBertForMultipleChoice) if is_tf_available() else ()
    pipeline_model_mapping = {'feature-extraction': TFBertModel, 'fill-mask': TFBertForMaskedLM, 'question-answering': TFBertForQuestionAnswering, 'text-classification': TFBertForSequenceClassification, 'text-generation': TFBertLMHeadModel, 'token-classification': TFBertForTokenClassification, 'zero-shot': TFBertForSequenceClassification} if is_tf_available() else {}
    ...
    def test_custom_load_tf_weights(self):
        if False:
            i = 10
            return i + 15
        (model, output_loading_info) = TFBertForTokenClassification.from_pretrained('jplu/tiny-tf-bert-random', output_loading_info=True)
...
class TFBertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        ...
        print(output[:, :3, :3])
        expected_slice = tf.constant([[[-0.05243197, -0.04498899, 0.05512108], [-0.07444685, -0.01064632, 0.04352357], [-0.05020351, 0.05530146, 0.00700043]]])
