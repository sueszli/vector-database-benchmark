...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This python script is used to discover and prepend a devops agent path with a specific pypy version. \n        It is not intended to be generic, and should not be be used in general "Use Python Version X" kind of situations.')
    ...
    print('Path should be prepended with discovered location{} [{}]'.format('s' if len(discovered_installer_location) >= 2 else '', ', '.join(discovered_installer_location)))
