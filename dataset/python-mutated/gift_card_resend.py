...
class GiftCardResendInput(BaseInputObjectType):
    ...
    email = graphene.String(required=False, description='Email to which gift card should be send.')
    channel = graphene.String(description='Slug of a channel from which the email should be sent.', required=True)
...
class GiftCardResend(BaseMutation):
    ...
    class Arguments:
        input = GiftCardResendInput(required=True, description='Fields required to resend a gift card.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.NOTIFY_USER, description='A notification for gift card resend.')]
