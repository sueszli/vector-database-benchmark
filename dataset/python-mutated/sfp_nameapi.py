...
class sfp_nameapi(SpiderFootPlugin):
    ...
    def queryEmailAddr(self, qry):
        if False:
            return 10
        res = self.sf.fetchUrl(f"http://api.nameapi.org/rest/v5.3/email/disposableemailaddressdetector?apiKey={self.opts['api_key']}&emailAddress={qry}", timeout=self.opts['_fetchtimeout'], useragent='SpiderFoot')
