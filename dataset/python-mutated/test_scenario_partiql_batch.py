...
@pytest.mark.parametrize('error_code, stop_on_method', [(None, None), ('TestException', 'stub_batch_execute_statement')])
def test_run_scenario(make_stubber, stub_runner, monkeypatch, error_code, stop_on_method):
    ...
    with stub_runner(error_code, stop_on_method) as runner:
        ...
        runner.add(dyn_stubber.stub_batch_execute_statement, ANY, [{'Item': {'title': {'S': 'test'}, 'year': {'N': '2000'}}}, {'Item': {'title': {'S': 'test2'}, 'year': {'N': '2002'}}}])
        runner.add(dyn_stubber.stub_batch_execute_statement, ANY, [])
        runner.add(dyn_stubber.stub_execute_statement, ANY, None, [{'title': {'S': 'test'}, 'year': {'N': '2000'}}])
        runner.add(dyn_stubber.stub_batch_execute_statement, ANY, [])
