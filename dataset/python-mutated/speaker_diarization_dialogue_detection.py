...
from modelscope.outputs import AttentionTextClassificationModelOutput, ModelOutputBase, OutputKeys
from modelscope.utils import logger as logging
from modelscope.utils.constant import Tasks
from modelscope.utils.hub import parse_label_mapping
logger = logging.get_logger()

@HEADS.register_module(Tasks.speaker_diarization_dialogue_detection, module_name=Heads.text_classification)
class TextClassificationHead(TorchHead):

    def __init__(self, hidden_size=768, classifier_dropout=0.1, num_labels=None, **kwargs):
        if False:
            return 10
        super().__init__(hidden_size=hidden_size, classifier_dropout=classifier_dropout, num_labels=num_labels)
...
@MODELS.register_module(Tasks.speaker_diarization_dialogue_detection, module_name=TaskModels.text_classification)
...
@MODELS.register_module(Tasks.speaker_diarization_dialogue_detection, module_name=Models.bert)
