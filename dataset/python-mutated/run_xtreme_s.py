...
import transformers
from transformers import AutoConfig, AutoFeatureExtractor, AutoModelForAudioClassification, AutoModelForCTC, AutoModelForSpeechSeq2Seq, AutoProcessor, AutoTokenizer, HfArgumentParser, Seq2SeqTrainer, Seq2SeqTrainingArguments, Trainer, set_seed
...
TASK_TO_TARGET_COLUMN_NAME = {'fleurs-asr': 'transcription', 'fleurs-lang_id': 'lang_id', 'mls': 'transcription', 'voxpopuli': 'transcription', 'covost2': 'translation', 'minds14': 'intent_class', 'babel': 'transcription'}
...
class ModelArguments:
    ...
    cache_dir: Optional[str] = field(default=None, metadata={'help': 'Where do you want to store the pretrained models and datasets downloaded from huggingface.co'})
    ...
    ctc_zero_infinity: bool = field(default=False, metadata={'help': 'Whether to zero infinite losses and the associated gradients of `torch.nn.CTCLoss`.'})
...
class DataTrainingArguments:
    ...
    the command line.
    """
    dataset_name: str = field(default='google/xtreme_s', metadata={'help': "The name of the dataset to use (via the datasets library). Defaults to 'google/xtreme_s'"})
    task: str = field(default=None, metadata={'help': "The task name of the benchmark to use (via the datasets library). Should be on of: 'fleurs-asr', 'mls', 'voxpopuli', 'covost2', 'minds14', 'fleurs-lang_id', 'babel'."})
    language: str = field(default='all', metadata={'help': 'The language id as defined in the datasets config name or `all` for all languages.'})
    ...
    train_split_name: str = field(default='train', metadata={'help': "The name of the training dataset split to use (via the datasets library). Defaults to 'train'"})
    eval_split_name: str = field(default='validation', metadata={'help': "The name of the evaluation dataset split to use (via the datasets library). Defaults to 'validation'"})
    predict_split_name: str = field(default='test', metadata={'help': "The name of the prediction dataset split to use (via the datasets library). Defaults to 'test'"})
    ...
    target_column_name: str = field(default=None, metadata={'help': 'The name of the dataset column containing the target data (transcription/translation/label). If None, the name will be inferred from the task. Defaults to None.'})
    ...
    chars_to_ignore: Optional[List[str]] = list_field(default=', ? . ! - ; : " “ % ‘ ” �'.split(' '), metadata={'help': 'A list of characters to remove from the transcripts.'})
    max_duration_in_seconds: float = field(default=30.0, metadata={'help': "Filter audio files that are longer than `max_duration_in_seconds` seconds to 'max_duration_in_seconds`"})
    ...
    use_auth_token: bool = field(default=False, metadata={'help': 'If :obj:`True`, will use the token generated when running:obj:`huggingface-cli login` as HTTP bearer authorization for remote files.'})
    ...
    per_lang_metrics: bool = field(default=True, metadata={'help': 'If `True`, compute the test metrics separately for each language, and average the results. If `False` compute the average test metrics in a single pass for all languages at once.'})
