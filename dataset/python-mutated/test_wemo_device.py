...
async def test_async_register_device_longpress_fails(hass: HomeAssistant, pywemo_device, device_registry: dr.DeviceRegistry) -> None:
    ...
    with patch.object(pywemo_device, 'ensure_long_press_virtual_device') as elp:
        ...
        assert await async_setup_component(hass, DOMAIN, {DOMAIN: {CONF_DISCOVERY: False, CONF_STATIC: [MOCK_HOST]}})
...
async def test_long_press_event(hass: HomeAssistant, pywemo_registry, wemo_entity) -> None:
    ...
    await hass.async_add_executor_job(pywemo_registry.callbacks[device.wemo.name], device.wemo, EVENT_TYPE_LONG_PRESS, 'testing_params')
    async with asyncio.timeout(8):
        await got_event.wait()
    assert event_data == {'device_id': wemo_entity.device_id, 'name': device.wemo.name, 'params': 'testing_params', 'type': EVENT_TYPE_LONG_PRESS, 'unique_id': device.wemo.serial_number}

async def test_subscription_callback(hass: HomeAssistant, pywemo_registry, wemo_entity) -> None:
    ...
    await hass.async_add_executor_job(pywemo_registry.callbacks[device.wemo.name], device.wemo, '', '')
    async with asyncio.timeout(8):
        await got_callback.wait()
    assert device.last_update_success

async def test_subscription_update_action_exception(hass: HomeAssistant, pywemo_device, wemo_entity) -> None:
    ...
    await hass.async_add_executor_job(device.subscription_callback, pywemo_device, '', '')
    await hass.async_block_till_done()
...
async def test_subscription_update_exception(hass: HomeAssistant, pywemo_device, wemo_entity) -> None:
...
async def test_async_update_data_subscribed(hass: HomeAssistant, pywemo_registry, pywemo_device, wemo_entity) -> None:
...
async def test_device_info(hass: HomeAssistant, wemo_entity, device_registry: dr.DeviceRegistry) -> None:
    """Verify the DeviceInfo data is set properly."""
    device_entries = list(device_registry.devices.values())
    assert len(device_entries) == 1
    assert device_entries[0].connections == {('upnp', f'uuid:LightSwitch-1_0-{MOCK_SERIAL_NUMBER}')}
...
async def test_dli_device_info(hass: HomeAssistant, wemo_dli_entity, device_registry: dr.DeviceRegistry) -> None:
...
async def test_options_enable_subscription_false(hass, pywemo_registry, pywemo_device, wemo_entity):
    ...
    assert hass.config_entries.async_update_entry(config_entry, options=asdict(wemo_device.Options(enable_subscription=False, enable_long_press=False)))
    await hass.async_block_till_done()
...
async def test_options_enable_long_press_false(hass, pywemo_device, wemo_entity):
    ...
    config_entry = hass.config_entries.async_get_entry(wemo_entity.config_entry_id)
    assert hass.config_entries.async_update_entry(config_entry, options=asdict(wemo_device.Options(enable_long_press=False)))
    await hass.async_block_till_done()
...
class TestInsight:
    ...
    @pytest.mark.parametrize(('subscribed', 'state', 'expected_calls'), [(False, 0, [call(), call(True), call(), call()]), (False, 1, [call(), call(True), call(), call()]), (True, 0, [call(), call(True), call(), call()]), (True, 1, [call(), call(), call()])])
    async def test_should_poll(self, hass, subscribed, state, expected_calls, wemo_entity, pywemo_device, pywemo_registry):
