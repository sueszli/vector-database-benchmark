...
class SokobanEnv(BaseEnv):
    ...
    def _make_env(self, only_info=False):
        if False:
            i = 10
            return i + 15
        return wrap_sokoban(self._env_id, norm_obs=self._cfg.get('norm_obs', EasyDict(use_norm=False)), norm_reward=self._cfg.get('norm_reward', EasyDict(use_norm=False)), only_info=only_info)
