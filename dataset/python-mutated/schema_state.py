...
from homeassistant.components.vacuum import ENTITY_ID_FORMAT, STATE_CLEANING, STATE_DOCKED, STATE_ERROR, STATE_RETURNING, StateVacuumEntity, VacuumEntityFeature
from homeassistant.config_entries import ConfigEntry
from homeassistant.const import ATTR_SUPPORTED_FEATURES, CONF_NAME, STATE_IDLE, STATE_PAUSED
...
FAN_SPEED = 'fan_speed'
STATE = 'state'
POSSIBLE_STATES: dict[str, str] = {STATE_IDLE: STATE_IDLE, STATE_DOCKED: STATE_DOCKED, STATE_ERROR: STATE_ERROR, STATE_PAUSED: STATE_PAUSED, STATE_RETURNING: STATE_RETURNING, STATE_CLEANING: STATE_CLEANING}
...
PLATFORM_SCHEMA_STATE_MODERN = MQTT_BASE_SCHEMA.extend({vol.Optional(CONF_FAN_SPEED_LIST, default=[]): vol.All(cv.ensure_list, [cv.string]), vol.Optional(CONF_NAME): vol.Any(cv.string, None), vol.Optional(CONF_PAYLOAD_CLEAN_SPOT, default=DEFAULT_PAYLOAD_CLEAN_SPOT): cv.string, vol.Optional(CONF_PAYLOAD_LOCATE, default=DEFAULT_PAYLOAD_LOCATE): cv.string, vol.Optional(CONF_PAYLOAD_RETURN_TO_BASE, default=DEFAULT_PAYLOAD_RETURN_TO_BASE): cv.string, vol.Optional(CONF_PAYLOAD_START, default=DEFAULT_PAYLOAD_START): cv.string, vol.Optional(CONF_PAYLOAD_PAUSE, default=DEFAULT_PAYLOAD_PAUSE): cv.string, vol.Optional(CONF_PAYLOAD_STOP, default=DEFAULT_PAYLOAD_STOP): cv.string, vol.Optional(CONF_SEND_COMMAND_TOPIC): valid_publish_topic, vol.Optional(CONF_SET_FAN_SPEED_TOPIC): valid_publish_topic, vol.Optional(CONF_STATE_TOPIC): valid_publish_topic, vol.Optional(CONF_SUPPORTED_FEATURES, default=DEFAULT_SERVICE_STRINGS): vol.All(cv.ensure_list, [vol.In(STRING_TO_SERVICE.keys())]), vol.Optional(CONF_COMMAND_TOPIC): valid_publish_topic, vol.Optional(CONF_RETAIN, default=DEFAULT_RETAIN): cv.boolean}).extend(MQTT_ENTITY_COMMON_SCHEMA.schema).extend(MQTT_VACUUM_SCHEMA.schema)
...
class MqttStateVacuum(MqttEntity, StateVacuumEntity):
    ...
    async def _async_publish_command(self, feature: VacuumEntityFeature) -> None:
        ...
        if self._command_topic is None:
            return
        await self.async_publish(self._command_topic, self._payloads[_FEATURE_PAYLOADS[feature]], qos=self._config[CONF_QOS], retain=self._config[CONF_RETAIN], encoding=self._config[CONF_ENCODING])
    ...
    async def async_set_fan_speed(self, fan_speed: str, **kwargs: Any) -> None:
        ...
        if self._set_fan_speed_topic is None or self.supported_features & VacuumEntityFeature.FAN_SPEED == 0 or fan_speed not in self.fan_speed_list:
            return
        await self.async_publish(self._set_fan_speed_topic, fan_speed, self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
    ...
    async def async_send_command(self, command: str, params: dict[str, Any] | list[Any] | None=None, **kwargs: Any) -> None:
        ...
        if isinstance(params, dict):
            ...
            payload = command
        await self.async_publish(self._send_command_topic, payload, self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])