...
from sanic.exceptions import BadRequest, ExpectationFailed, PayloadTooLarge, RequestCancelled, ServerError, ServiceUnavailable
...
class Http(Stream, metaclass=TouchUpMeta):
    ...
    """
    HEADER_CEILING = 16384
    HEADER_MAX_SIZE = 0
    __touchup__ = ('http1_request_header', 'http1_response_header', 'read')
    __slots__ = ['_send', '_receive_more', 'dispatch', 'recv_buffer', 'protocol', 'expecting_continue', 'stage', 'keep_alive', 'head_only', 'request', 'exception', 'url', 'request_body', 'request_bytes', 'request_bytes_left', 'response', 'response_func', 'response_size', 'response_bytes_left', 'upgrade_websocket']
        ...
        while self.keep_alive and self.stage is Stage.IDLE:
            ...
            try:
                ...
                if not self.protocol.transport:
                    logger.info(f'Request: {self.request.method} {self.request.url} stopped. Transport is closed.')
                    return
                e = RequestCancelled() if self.protocol.conn_info.lost else self.exception or exc
                self.exception = None
            ...
            if self.request_body:
                ...
                self.request_max_size = min(self.request_max_size, self.protocol.request_max_size)
                try:
                    async for _ in self:
                        pass
    ...
    async def http1_request_header(self):
        ...
        try:
            ...
            await self.dispatch('http.lifecycle.read_head', inline=True, context={'head': bytes(head)})
        ...
        self.upgrade_websocket = headers_instance.getone('upgrade', '').lower() == 'websocket'
        try:
            url_bytes = self.url.encode('ASCII')
        except UnicodeEncodeError:
            raise BadRequest('URL may only contain US-ASCII characters.')
        request = self.protocol.request_class(url_bytes=url_bytes, headers=headers_instance, head=bytes(head), version=protocol[5:], method=method, transport=self.protocol.transport, app=self.protocol.app)
        self.protocol.request_class._current.set(request)
        await self.dispatch('http.lifecycle.request', inline=True, context={'request': request})
    ...
    async def http1_response_header(self, data: bytes, end_stream: bool) -> None:
        ...
        if not has_message_body(status):
            ...
            if data or not end_stream or 'content-length' in headers or ('transfer-encoding' in headers):
    ...
    async def error_response(self, exception: Exception) -> None:
        ...
        if self.stage is Stage.HANDLER:
            ...
            request_middleware = not isinstance(exception, (ServiceUnavailable, RequestCancelled))
            try:
    ...
    async def read(self) -> Optional[bytes]:
        ...
        del buf[:size]
        self.request_bytes_left -= size
        await self.dispatch('http.lifecycle.read_body', inline=True, context={'body': data})
        return data
