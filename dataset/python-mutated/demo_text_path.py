...
if __name__ == '__main__':
    ...
    ax1.add_artist(ao)
    for (usetex, ypos, string) in [(False, 0.25, 'textpath supports mathtext'), (True, 0.05, 'textpath supports \\TeX')]:
        ...
        ab = AnnotationBbox(offsetbox2, (0.95, ypos), xycoords='axes fraction', boxcoords='offset points', box_alignment=(1.0, 0.0), frameon=False)
        ax1.add_artist(ab)
    ax1.imshow([[0, 1, 2], [1, 2, 3]], cmap=plt.cm.gist_gray_r, interpolation='bilinear', aspect='auto')
    arr = np.arange(256).reshape(1, 256)
    for (usetex, xpos, string) in [(False, 0.25, '$\\left[\\sum_{n=1}^\\infty\\frac{-e^{i\\pi}}{2^n}\\right]$!'), (True, 0.75, '$\\displaystyle\\left[\\sum_{n=1}^\\infty\\frac{-e^{i\\pi}}{2^n}\\right]$!')]:
