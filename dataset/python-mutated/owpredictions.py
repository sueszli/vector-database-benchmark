...
import numpy
from AnyQt.QtWidgets import QTableView, QSplitter, QToolTip, QStyle, QApplication, QSizePolicy, QPushButton, QStyledItemDelegate, QStyleOptionViewItem
from AnyQt.QtGui import QPainter, QStandardItem, QPen, QColor, QBrush
from AnyQt.QtCore import Qt, QSize, QRect, QRectF, QPoint, QPointF, QLocale, QModelIndex, pyqtSignal, QTimer, QItemSelectionModel, QItemSelection
...
from Orange.widgets.evaluate.utils import ScoreTable, usable_scorers, learner_name, scorer_caller
...
PredictorSlot = NamedTuple('PredictorSlot', [('predictor', Model), ('name', str), ('results', Optional[Results])])
(NO_ERR, DIFF_ERROR, ABSDIFF_ERROR, REL_ERROR, ABSREL_ERROR) = range(5)
ERROR_OPTS = ['(None)', 'Difference', 'Absolute difference', 'Relative', 'Absolute relative']
ERROR_TOOLTIPS = ["Don't show columns with errors", 'Show difference between predicted and actual value', 'Show absolute difference between predicted and actual value', 'Show relative difference between predicted and actual value', 'Show absolute value of relative difference between predicted and actual value']
...
class OWPredictions(OWWidget):
    ...
    class Warning(OWWidget.Warning):
        ...
        wrong_targets = Msg('Some model(s) predict a different target (see more ...)\n{}')
        missing_targets = Msg('Instances with missing targets are ignored while scoring.')
    ...
    PROB_OPTS = ['(None)', 'Classes in data', 'Classes known to the model', 'Classes in data and model']
    PROB_TOOLTIPS = ["Don't show probabilities", 'Show probabilities for classes in the data', "Show probabilities for classes known to the model,\nincluding those that don't appear in this data", 'Show probabilities for classes in data that are also\nknown to the model']
...
if __name__ == '__main__':
    ...
    if iris.domain.has_discrete_class:
        idom = iris.domain
        dom = Domain(idom.attributes, DiscreteVariable(idom.class_var.name, idom.class_var.values[1::-1]))
        iris2 = iris2.transform(dom)
        predictors_ = [Orange.classification.SVMLearner(probability=True)(iris2), Orange.classification.LogisticRegressionLearner()(iris), pred_error]
    elif iris.domain.has_continuous_class:
        predictors_ = [Orange.regression.RidgeRegressionLearner(alpha=1.0)(iris), Orange.regression.LinearRegressionLearner()(iris), pred_error]
    else:
        predictors_ = [pred_error]
    WidgetPreview(OWPredictions).run(set_data=iris, insert_predictor=list(enumerate(predictors_)))