...
class DenyAssignmentsOperations:
    ...
    .. warning::
        **DO NOT** instantiate this class directly.
        ...
        :class:`~azure.mgmt.authorization.v2018_07_01_preview.aio.AuthorizationManagementClient`'s
    ...
    @distributed_trace
    def list_for_resource(self, resource_group_name: str, resource_provider_namespace: str, parent_resource_path: str, resource_type: str, resource_name: str, filter: Optional[str]=None, **kwargs: Any) -> AsyncIterable['_models.DenyAssignment']:
        if False:
            print('Hello World!')
        ...
        api_version: str = kwargs.pop('api_version', _params.pop('api-version', self._api_version or '2018-07-01-preview'))
    ...
    @distributed_trace
    def list_for_resource_group(self, resource_group_name: str, filter: Optional[str]=None, **kwargs: Any) -> AsyncIterable['_models.DenyAssignment']:
        if False:
            i = 10
            return i + 15
    ...
    async def get(self, scope: str, deny_assignment_id: str, **kwargs: Any) -> _models.DenyAssignment:
        ...
        request = build_get_request(scope=scope, deny_assignment_id=deny_assignment_id, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=_stream, **kwargs)
    ...
    async def get_by_id(self, deny_assignment_id: str, **kwargs: Any) -> _models.DenyAssignment:
        ...
        api_version: str = kwargs.pop('api_version', _params.pop('api-version', self._api_version or '2018-07-01-preview'))
        cls: ClsType[_models.DenyAssignment] = kwargs.pop('cls', None)
        request = build_get_by_id_request(deny_assignment_id=deny_assignment_id, api_version=api_version, template_url=self.get_by_id.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=_stream, **kwargs)
    ...
    @distributed_trace
    def list_for_scope(self, scope: str, filter: Optional[str]=None, **kwargs: Any) -> AsyncIterable['_models.DenyAssignment']:
        if False:
            while True:
                i = 10
