...
import numpy as np
import scipy as sp
from sklearn.datasets import fetch_20newsgroups_vectorized, fetch_lfw_people, fetch_olivetti_faces, fetch_openml, fetch_rcv1, make_low_rank_matrix, make_sparse_uncorrelated
...
MAX_MEMORY = int(4000000000.0)
CIFAR_FOLDER = './cifar-10-batches-py/'
SVHN_FOLDER = './SVHN/'
datasets = ['low rank matrix', 'lfw_people', 'olivetti_faces', '20newsgroups', 'mnist_784', 'CIFAR', 'a3a', 'SVHN', 'uncorrelated matrix']
...
if __name__ == '__main__':
    ...
    for dataset_name in datasets:
        ...
        if X is None:
            continue
        print(' >>>>>> Benching sklearn and fbpca on %s %d x %d' % (dataset_name, X.shape[0], X.shape[1]))
        bench_a(X, dataset_name, power_iter, n_oversamples=2, n_comps=np.minimum(n_comps, np.min(X.shape)))
