...
def main_demo() -> None:
    if False:
        i = 10
        return i + 15
    ui.input(label='Text', placeholder='start typing', on_change=lambda e: result.set_text('you typed: ' + e.value), validation={'Input too long': lambda value: len(value) < 20})
    result = ui.label()
...
def more() -> None:
    ...
    def styling():
        ...
        ui.input('styling', value='some text').props('input-style="color: blue" input-class="font-mono"')
        with ui.input(value='custom clear button').classes('w-64') as i:
            ui.button(color='orange-8', on_click=lambda : i.set_value(None), icon='delete').props('flat dense').bind_visibility_from(i, 'value')