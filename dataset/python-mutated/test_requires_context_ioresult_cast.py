from returns.context import ReaderIOResult, ReaderIOResultE, RequiresContextIOResult, RequiresContextIOResultE

def _function(arg: int) -> RequiresContextIOResultE[float, int]:
    if False:
        return 10
    if arg == 0:
        return RequiresContextIOResult.from_failure(ZeroDivisionError('Divided by 0'))
...
def test_requires_context_io_aliases():
    ...
    container3: ReaderIOResultE[float, int] = ReaderIOResultE.from_value(10.0)
