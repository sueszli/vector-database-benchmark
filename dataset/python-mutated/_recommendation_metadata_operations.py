...
def build_get_request(name: str, **kwargs: Any) -> HttpRequest:
    ...
    _url = kwargs.pop('template_url', '/providers/Microsoft.Advisor/metadata/{name}')
...
def build_list_request(**kwargs: Any) -> HttpRequest:
    ...
    api_version = kwargs.pop('api_version', _params.pop('api-version', '2020-01-01'))
    accept = _headers.pop('Accept', 'application/json')
    _url = kwargs.pop('template_url', '/providers/Microsoft.Advisor/metadata')
    _params['api-version'] = _SERIALIZER.query('api_version', api_version, 'str')
    _headers['Accept'] = _SERIALIZER.header('accept', accept, 'str')
    return HttpRequest(method='GET', url=_url, params=_params, headers=_headers, **kwargs)
...
class RecommendationMetadataOperations:
    ...
    @distributed_trace
    def get(self, name: str, **kwargs: Any) -> _models.MetadataEntity:
        if False:
            return 10
        ...
        error_map = {401: ClientAuthenticationError, 409: ResourceExistsError, 304: ResourceNotModifiedError, 404: lambda response: ResourceNotFoundError(response=response, model=self._deserialize(_models.ARMErrorResponseBody, response), error_format=ARMErrorFormat)}
