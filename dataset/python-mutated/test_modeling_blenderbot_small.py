...
from transformers.testing_utils import require_torch, require_torch_fp16, slow, torch_device
...
if is_torch_available():
    ...
    from transformers.models.blenderbot_small.modeling_blenderbot_small import BlenderbotSmallDecoder, BlenderbotSmallEncoder, BlenderbotSmallForCausalLM

def prepare_blenderbot_small_inputs_dict(config, input_ids, decoder_input_ids, attention_mask=None, decoder_attention_mask=None, head_mask=None, decoder_head_mask=None, cross_attn_head_mask=None):
    if False:
        while True:
            i = 10
...
class BlenderbotSmallModelTester:
    ...
    def __init__(self, parent, batch_size=13, seq_length=7, is_training=True, use_labels=False, vocab_size=99, hidden_size=16, num_hidden_layers=2, num_attention_heads=4, intermediate_size=4, hidden_act='gelu', hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, max_position_embeddings=50, eos_token_id=2, pad_token_id=1, bos_token_id=0):
        if False:
            for i in range(10):
                print('nop')
        self.parent = parent
...
class BlenderbotSmallModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'conversational': BlenderbotSmallForConditionalGeneration, 'feature-extraction': BlenderbotSmallModel, 'summarization': BlenderbotSmallForConditionalGeneration, 'text-generation': BlenderbotSmallForCausalLM, 'text2text-generation': BlenderbotSmallForConditionalGeneration, 'translation': BlenderbotSmallForConditionalGeneration} if is_torch_available() else {}
...
class Blenderbot90MIntegrationTests(unittest.TestCase):
    ...
    def test_90_generation_from_long_input(self):
        ...
        assert reply in ("i don't know. i just feel like i'm going to throw up. it's not fun.", "i'm not sure. i just feel like i've been feeling like i have to be in a certain place")
    ...
    def test_90_generation_from_short_input(self):
        ...
        clean_txt = self.tokenizer.decode(generated_utterances[0], skip_special_tokens=True, clean_up_tokenization_spaces=True)
        assert clean_txt in ("have you ever been to a sam club? it's a great club in the south.", "have you ever heard of sam harris? he's an american singer, songwriter, and actor.")
