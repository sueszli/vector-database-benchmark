...
from paddle.distributed.auto_parallel.static.process_group import new_process_group
...
class GPTModel(nn.Layer):
    ...
    def __init__(self, vocab_size, hidden_size=768, num_hidden_layers=4, num_attention_heads=12, intermediate_size=3072, hidden_act='gelu', hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, max_position_embeddings=512, type_vocab_size=16, initializer_range=0.02, pad_token_id=0, topo=None):
        if False:
            i = 10
            return i + 15
        super().__init__()
...
def gpt_pretrain_forward(train_program, startup_program):
    ...
    with static.program_guard(train_program, startup_program), utils.unique_name.guard():
        ...
        gpt = GPTModel(vocab_size=32768, hidden_size=768, num_hidden_layers=2, num_attention_heads=12, intermediate_size=4096, hidden_act='gelu', hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, max_position_embeddings=1024, type_vocab_size=16, initializer_range=0.02, pad_token_id=0, topo=None)
...
class TestGPTPartitioner(unittest.TestCase):
    ...
    def test_gpt_dp_mp(self):
        ...
        (train_program, startup_program, loss) = gpt_pretrain_forward(train_program, startup_program)
        ...
        (auto_parallel_main_prog, auto_parallel_startup_prog, params_grads) = partitioner.partition(complete_train_program, startup_program, params_grads)
        nrank = 4
        weights = ['linear_0.w_0', 'linear_6.w_0', 'linear_10.w_0']
        self.assertTrue(check_tensor_split(auto_parallel_main_prog, weights, complete_train_program, weights, 1, nrank))
        weights = ['word_embeddings', 'linear_9.w_0', 'linear_11.w_0']
        self.assertTrue(check_tensor_split(auto_parallel_main_prog, weights, complete_train_program, weights, 0, nrank))
        weights = ['pos_embeddings', 'layer_norm_0.b_0', 'layer_norm_4.w_0']
        self.assertTrue(check_tensor_split(auto_parallel_main_prog, weights, complete_train_program, weights, 0, 1))
        all_params = sorted([param.name for param in startup_program.all_parameters()])
        allreduce_grads = ['layer_norm_5.tmp_2', 'layer_norm_5.tmp_2', 'layer_norm_5.tmp_2', 'layer_norm_6.tmp_2', 'layer_norm_7.tmp_2', 'layer_norm_7.tmp_2', 'layer_norm_7.tmp_2', 'layer_norm_8.tmp_2']
        ...
        tensor_parallel_allreduce_vars = sorted([op.desc.output_arg_names()[0].split('@')[0] for op in auto_parallel_main_prog.global_block().ops if op.type == 'c_allreduce_sum' and op.attr('op_role') == 1 and (op.desc.attr('ring_id') == mp_ring_id)])
        data_parallel_allreduce_vars = sorted([op.desc.output_arg_names()[0].split('@')[0] for op in auto_parallel_main_prog.global_block().ops if op.type == 'c_allreduce_sum' and op.desc.attr('ring_id') == dp_ring_id])
        self.assertTrue(all_params == data_parallel_allreduce_vars)
        self.assertTrue(allreduce_grads == tensor_parallel_allreduce_vars)
        self.assertTrue(is_valid_completed_program(dist_context, auto_parallel_main_prog))
