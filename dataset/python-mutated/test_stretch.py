...
from astropy.visualization.stretch import AsinhStretch, ContrastBiasStretch, HistEqStretch, InvertedHistEqStretch, InvertedLogStretch, InvertedPowerDistStretch, LinearStretch, LogStretch, PowerDistStretch, PowerStretch, SinhStretch, SqrtStretch, SquaredStretch
DATA = np.array([0.0, 0.25, 0.5, 0.75, 1.0])
RESULTS = {}
RESULTS[LinearStretch()] = np.array([0.0, 0.25, 0.5, 0.75, 1.0])
RESULTS[LinearStretch(intercept=0.5) + LinearStretch(slope=0.5)] = np.array([0.5, 0.625, 0.75, 0.875, 1.0])
...
RESULTS[ContrastBiasStretch(contrast=2.0, bias=0.4)] = np.array([-0.3, 0.2, 0.7, 1.2, 1.7])
