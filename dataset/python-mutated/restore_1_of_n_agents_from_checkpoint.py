...
if __name__ == '__main__':
    ...
    policies = {f'policy_{i}': (None, obs_space, act_space, None) for i in range(args.num_policies)}
    ...
    def policy_mapping_fn(agent_id, episode, worker, **kwargs):
        ...
        return pol_id
    config = PPOConfig().environment(MultiAgentCartPole, env_config={'num_agents': args.num_agents}).framework(args.framework).training(num_sgd_iter=10).multi_agent(policies=policies, policy_mapping_fn=policy_mapping_fn).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    results = tune.Tuner('PPO', param_space=config.to_dict(), run_config=air.RunConfig(stop={'training_iteration': args.pre_training_iters}, verbose=1, checkpoint_config=air.CheckpointConfig(checkpoint_frequency=1, checkpoint_at_end=True))).fit()
    print('Pre-training done.')
    best_checkpoint = results.get_best_result().checkpoint
    print(f'.. best checkpoint was: {best_checkpoint}')
    policy_0_checkpoint = os.path.join(best_checkpoint.to_directory(), 'policies/policy_0')
    ...
    stop = {'episode_reward_mean': args.stop_reward, 'timesteps_total': args.stop_timesteps, 'training_iteration': args.stop_iters}
    ...
    results = tune.run('PPO', stop=stop, config=config.to_dict(), verbose=1)
    if args.as_test:
