...
class assuredworkloadsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'acknowledge_violation': ('name', 'comment', 'non_compliant_org_policy'), 'create_workload': ('parent', 'workload', 'external_id'), 'delete_workload': ('name', 'etag'), 'get_violation': ('name',), 'get_workload': ('name',), 'list_violations': ('parent', 'interval', 'page_size', 'page_token', 'filter'), 'list_workloads': ('parent', 'page_size', 'page_token', 'filter'), 'restrict_allowed_resources': ('name', 'restriction_type'), 'update_workload': ('workload', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=assuredworkloadsCallTransformer()):
    if False:
        print('Hello World!')
