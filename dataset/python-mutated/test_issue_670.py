...
class TestWebClient(unittest.TestCase):
    ...
    def test_issue_670(self):
        ...
        buff.seek(0)
        upload = client.files_upload(file=buff, filename='output.text', filetype='text', title=None)
    ...
    async def test_issue_670_async(self):
        ...
        buff = io.BytesIO(b'here is my data but not sure what is wrong.......')
        buff.seek(0)
        upload = await client.files_upload(file=buff, filename='output.text', filetype='text', title=None)
        self.assertIsNotNone(upload)