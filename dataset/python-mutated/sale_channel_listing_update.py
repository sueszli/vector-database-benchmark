...
from ...dataloaders import PromotionRulesByPromotionIdLoader, SaleChannelListingByPromotionIdLoader

class SaleChannelListingAddInput(BaseInputObjectType):
    channel_id = graphene.ID(required=True, description='ID of a channel.')
    discount_value = PositiveDecimal(required=True, description='The value of the discount.')

    class Meta:
        doc_category = DOC_CATEGORY_DISCOUNTS

class SaleChannelListingInput(BaseInputObjectType):
    add_channels = NonNullList(SaleChannelListingAddInput, description='List of channels to which the sale should be assigned.', required=False)
    remove_channels = NonNullList(graphene.ID, description='List of channels from which the sale should be unassigned.', required=False)
...
class SaleChannelListingUpdate(BaseChannelListingMutation):
    ...
    class Arguments:
        ...
        input = SaleChannelListingInput(required=True, description='Fields required to update sale channel listings.')

    class Meta:
        description = "Manage sale's availability in channels." + DEPRECATED_IN_3X_MUTATION + ' Use `promotionRuleCreate` or `promotionRuleUpdate` mutations instead.'
        doc_category = DOC_CATEGORY_DISCOUNTS
