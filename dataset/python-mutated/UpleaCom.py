...
class UpleaCom(SimpleDownloader):
    ...
    __version__ = '0.21'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?uplea\\.com/dl/\\w{15}'
    ...
    SIZE_REPLACEMENTS = [('ko', 'KiB'), ('mo', 'MiB'), ('go', 'GiB'), ('Ko', 'KiB'), ('Mo', 'MiB'), ('Go', 'GiB')]
    NAME_PATTERN = '<span class="gold-text">(?P<N>.+?)</span>'
    SIZE_PATTERN = '<span class="label label-info agmd">(?P<S>[\\d.,]+) (?P<U>[\\w^_]+?)</span>'
    OFFLINE_PATTERN = '>You followed an invalid or expired link'
    LINK_PATTERN = '"(https?://\\w+\\.uplea\\.com/anonym/.*?)"'
    PREMIUM_ONLY_PATTERN = 'You need to have a Premium subscription to download this file'
    WAIT_PATTERN = 'timeText: ?(\\d+),'
    STEP_PATTERN = '<a href="(/step/.+)">'
    NAME_REPLACEMENTS = [('(<a class="__cf_email__" .+? data-cfemail="(\\w+?)".+)', lambda x: decode_cloudflare_email(x.group(2)))]
