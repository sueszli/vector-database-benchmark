...
from ...core.types import AttributeValueBulkTranslateError, BaseInputObjectType, BaseObjectType, NonNullList
...
class AttributeValueBulkTranslateResult(BaseObjectType):
    translation = graphene.Field(AttributeValueTranslation, description='Attribute value translation data.')
    errors = NonNullList(AttributeValueBulkTranslateError, required=False, description='List of errors occurred on translation attempt.')

    class Meta:
        doc_category = DOC_CATEGORY_ATTRIBUTES

class AttributeValueBulkTranslateInput(BaseInputObjectType):
    id = graphene.ID(required=False, description='Attribute value ID.')
    external_reference = graphene.String(required=False, description='External reference of an attribute value.')
    language_code = LanguageCodeEnum(required=True, description='Translation language code.')
    translation_fields = AttributeValueTranslationInput(required=True, description='Translation fields.')

    class Meta:
        doc_category = DOC_CATEGORY_ATTRIBUTES

class AttributeValueBulkTranslate(BaseBulkTranslateMutation):
    count = graphene.Int(required=True, description='Returns how many translations were created/updated.')
    results = NonNullList(AttributeValueBulkTranslateResult, required=True, default_value=[], description='List of the translations.')

    class Arguments(BaseBulkTranslateMutation.Arguments):
        translations = NonNullList(AttributeValueBulkTranslateInput, required=True, description='List of attribute values translations.')

    class Meta:
        description = 'Creates/updates translations for attributes values.' + ADDED_IN_314 + PREVIEW_FEATURE
    ...
    def clean_translations(cls, data_inputs, index_error_map):
        ...
        for (index, data) in enumerate(data_inputs):
            ...
            try:
                ...
                index_error_map[index].append(AttributeValueBulkTranslateError(message=exc.message, code=AttributeValueTranslateErrorCode.INVALID.value))
                cleaned_inputs_map[index] = None
                continue
            if global_id:
                try:
                    (obj_type, id) = from_global_id_or_error(global_id, only_type='AttributeValue')
                except GraphQLError as exc:
                    index_error_map[index].append(AttributeValueBulkTranslateError(message=str(exc), code=AttributeValueTranslateErrorCode.INVALID.value))
                    cleaned_inputs_map[index] = None
                    continue
                data['id'] = id
