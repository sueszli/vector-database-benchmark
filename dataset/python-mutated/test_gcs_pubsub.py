...
import re
import ray
from ray._private.gcs_pubsub import GcsAioPublisher, GcsAioErrorSubscriber, GcsAioLogSubscriber, GcsAioResourceUsageSubscriber
...
async def test_aio_publish_and_subscribe_logs(ray_start_regular):
    ...
    await subscriber.subscribe()
    publisher = GcsAioPublisher(address=gcs_server_addr)
    log_batch = {'ip': '127.0.0.1', 'pid': 'gcs', 'job': '0001', 'is_err': False, 'lines': ['line 1', 'line 2'], 'actor_name': 'test actor', 'task_name': 'test task'}
