...
if is_torch_available():
    import torch
    from transformers import UniSpeechSatForAudioFrameClassification, UniSpeechSatForCTC, UniSpeechSatForPreTraining, UniSpeechSatForSequenceClassification, UniSpeechSatForXVector, UniSpeechSatModel, Wav2Vec2FeatureExtractor, Wav2Vec2Processor
...
class UniSpeechSatModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (UniSpeechSatForCTC, UniSpeechSatForPreTraining, UniSpeechSatModel, UniSpeechSatForSequenceClassification, UniSpeechSatForAudioFrameClassification, UniSpeechSatForXVector) if is_torch_available() else ()
    pipeline_model_mapping = {'audio-classification': UniSpeechSatForSequenceClassification, 'automatic-speech-recognition': UniSpeechSatForCTC, 'feature-extraction': UniSpeechSatModel} if is_torch_available() else {}
    test_pruning = False
...
class UniSpeechSatRobustModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (UniSpeechSatForCTC, UniSpeechSatForPreTraining, UniSpeechSatModel, UniSpeechSatForSequenceClassification) if is_torch_available() else ()
    test_pruning = False
    test_headmasking = False
    test_torchscript = False
