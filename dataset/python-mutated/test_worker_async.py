...
from azure.communication.jobrouter.models import RoundRobinMode, RouterWorker, RouterChannel, DistributionPolicy, RouterQueue
worker_labels = {'key1': 'WorkerKey', 'key2': 10, 'key3': True}
worker_tags = {'tag1': 'WorkerGenericInfo'}
worker_channels = [RouterChannel(channel_id='fakeChannel1', capacity_cost_per_job=10), RouterChannel(channel_id='fakeChannel2', capacity_cost_per_job=90)]
worker_capacity = 100
...
class TestRouterWorkerAsync(AsyncRouterRecordedTestCase):
    ...
    async def setup_job_queue(self):
        ...
        async with client:
            ...
            job_queue: RouterQueue = RouterQueue(name=job_queue_id, labels=worker_labels, distribution_policy_id=self.get_distribution_policy_id())
    ...
    async def test_create_worker(self):
        ...
        async with router_client:
            router_worker: RouterWorker = RouterWorker(capacity=worker_capacity, labels=worker_labels, tags=worker_tags, queues=worker_queues, channels=worker_channels, available_for_offers=False)
            ...
            RouterWorkerValidator.validate_worker(router_worker, identifier=w_identifier, capacity=worker_capacity, labels=worker_labels, tags=worker_tags, queues=worker_queues, channels=worker_channels, available_for_offers=False)
    ...
    async def test_update_worker(self):
        ...
        async with router_client:
            ...
            RouterWorkerValidator.validate_worker(update_router_worker, identifier=w_identifier, capacity=worker_capacity, labels=updated_worker_labels, tags=worker_tags, queues=worker_queues, channels=worker_channels, available_for_offers=False)
    ...
    async def test_update_worker_w_kwargs(self):
        ...
        async with router_client:
            ...
            router_worker.labels['FakeKey'] = 'FakeWorkerValue'
            updated_worker_labels = router_worker.labels
            update_router_worker = await router_client.upsert_worker(worker_id=w_identifier, labels=updated_worker_labels)
            assert update_router_worker is not None
            RouterWorkerValidator.validate_worker(update_router_worker, identifier=w_identifier, capacity=worker_capacity, labels=updated_worker_labels, tags=worker_tags, queues=worker_queues, channels=worker_channels, available_for_offers=False)
    ...
    async def test_get_worker(self):
        ...
        async with router_client:
            ...
            RouterWorkerValidator.validate_worker(queried_router_worker, identifier=w_identifier, capacity=router_worker.capacity, labels=router_worker.labels, tags=router_worker.tags, queues=router_worker.queues, channels=router_worker.channels, available_for_offers=router_worker.available_for_offers)
    ...
    async def test_list_workers(self):
        ...
        async with router_client:
            for identifier in w_identifiers:
                ...
                RouterWorkerValidator.validate_worker(worker, identifier=identifier, capacity=worker_capacity, labels=worker_labels, tags=worker_tags, queues=worker_queues, channels=worker_channels, available_for_offers=False)
                created_w_response[worker.id] = worker
            router_workers = router_client.list_workers(results_per_page=3, state='inactive', queue_id=self.get_job_queue_id(), channel_id='fakeChannel1')
            async for worker_page in router_workers.by_page():
                ...
                for w_item in list_of_workers:
                    ...
                    if not response_at_creation:
                        continue
                    RouterWorkerValidator.validate_worker(w_item, identifier=response_at_creation.id, capacity=response_at_creation.capacity, labels=response_at_creation.labels, tags=response_at_creation.tags, queues=response_at_creation.queues, channels=response_at_creation.channels, available_for_offers=response_at_creation.available_for_offers)
                    w_count -= 1
            assert w_count == 0