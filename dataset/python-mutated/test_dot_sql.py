...
dot_sql_notyet = pytest.mark.notyet(['snowflake', 'oracle'], reason='snowflake and oracle column names are case insensitive')
dot_sql_never = pytest.mark.never(['dask', 'pandas'], reason='dask and pandas do not accept SQL')
pytestmark = [pytest.mark.xdist_group('dot_sql')]
_NAMES = {'bigquery': 'ibis_gbq_testing.functional_alltypes'}
try:
...
no_sqlglot_dialect = sorted((param(backend, marks=pytest.mark.xfail) for backend in _NO_SQLGLOT_DIALECT))
