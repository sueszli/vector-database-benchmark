...
class FilefoxCc(XFSDownloader):
    ...
    __version__ = '0.02'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?filefox\\.cc/(?P<ID>\\w{12})'
    ...
    DL_LIMIT_PATTERN = "Wait [\\w ]+? or <a href='//filefox.cc/premium'"
    WAIT_PATTERN = '<span class="time-remain">(\\d+)<'
    INFO_PATTERN = '<p>(?P<N>.+?)</p>\\s*<p class="file-size">(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)</p>'
    NAME_PATTERN = '^unmatchable$'
    SIZE_REPLACEMENTS = [('Kb', 'KB'), ('Mb', 'MB'), ('Gb', 'GB')]
    LINK_PATTERN = '<a class="btn btn-default" href="(https://s\\d+.filefox.cc/.+?)"'
