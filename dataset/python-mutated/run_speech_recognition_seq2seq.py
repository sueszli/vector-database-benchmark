...
import transformers
from transformers import AutoConfig, AutoFeatureExtractor, AutoModelForSpeechSeq2Seq, AutoProcessor, AutoTokenizer, HfArgumentParser, Seq2SeqTrainer, Seq2SeqTrainingArguments, set_seed
...
class ModelArguments:
    ...
    forced_decoder_ids: List[List[int]] = field(default=None, metadata={'help': 'A list of pairs of integers which indicates a mapping from generation indices to token indices that will be forced before sampling. For example, [[0, 123]] means the first generated token will always be a token of index 123.'})
    suppress_tokens: List[int] = field(default=None, metadata={'help': 'A list of tokens that will be suppressed at generation.'})
