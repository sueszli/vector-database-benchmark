...
class TestPicklePersistence:
    ...
    async def test_with_multi_file_wo_callback_data(self, pickle_persistence, pickle_files_wo_callback_data):
        user_data = await pickle_persistence.get_user_data()
        assert isinstance(user_data, dict)
        assert user_data[12345]['test1'] == 'test2'
        assert user_data[67890][3] == 'test4'
    ...
    async def test_with_single_file_wo_bot_data(self, pickle_persistence, pickle_files_wo_bot_data):
        pickle_persistence.single_file = True
        user_data = await pickle_persistence.get_user_data()
        assert isinstance(user_data, dict)
        assert user_data[12345]['test1'] == 'test2'
    ...
    async def test_with_single_file_wo_callback_data(self, pickle_persistence, pickle_files_wo_callback_data):
        user_data = await pickle_persistence.get_user_data()
        assert isinstance(user_data, dict)
        assert user_data[12345]['test1'] == 'test2'
        assert user_data[67890][3] == 'test4'
    ...
    async def test_updating_single_file_no_data(self, pickle_persistence):
        pickle_persistence.single_file = True
        assert not any([pickle_persistence.user_data, pickle_persistence.chat_data, pickle_persistence.bot_data, pickle_persistence.callback_data, pickle_persistence.conversations])
        await pickle_persistence.flush()
    ...
    async def test_custom_pickler_unpickler_simple(self, pickle_persistence, good_pickle_files, cdc_bot, recwarn):
        bot = cdc_bot
        update = Update(1)
        update.set_bot(bot)
        ...
        err_msg = 'A load persistent id instruction was encountered,\nbut no persistent_load function was specified.'
        ...
        assert Path(recwarn[-1].filename) == PROJECT_ROOT_PATH / 'telegram' / 'ext' / '_picklepersistence.py', 'wrong stacklevel!'
        pp = PicklePersistence('pickletest', single_file=False, on_flush=False)
        pp.set_bot(bot)
        assert (await pp.get_chat_data())[12345]['unknown_bot_in_user']._bot is None

    async def test_custom_pickler_unpickler_with_custom_objects(self, cdc_bot, pickle_persistence, good_pickle_files):
        bot = cdc_bot
        dict_s = self.DictSub('private', 'normal', bot)
        slot_s = self.SlotsSub('new_var', 'private_var')
        regular = self.NormalClass(12)
        pickle_persistence.set_bot(bot)
        await pickle_persistence.update_user_data(1232, {'sub_dict': dict_s, 'sub_slots': slot_s, 'r': regular})
        pp = PicklePersistence('pickletest', single_file=False, on_flush=False)
        pp.set_bot(bot)
        ...
        assert sub_slots._bot is None
        assert sub_regular.my_var == regular.my_var

    @pytest.mark.parametrize('filepath', ['pickletest', Path('pickletest')], ids=['str filepath', 'pathlib.Path filepath'])
    async def test_filepath_argument_types(self, filepath):
        pick_persist = PicklePersistence(filepath=filepath, on_flush=False)
    ...
    async def test_no_write_if_data_did_not_change(self, pickle_persistence, bot_data, user_data, chat_data, conversations, callback_data):
        pickle_persistence.single_file = True
