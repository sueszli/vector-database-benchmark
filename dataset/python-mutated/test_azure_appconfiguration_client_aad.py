...
from azure.core.exceptions import AzureError, ResourceModifiedError, ResourceNotFoundError, ResourceExistsError
from azure.appconfiguration import ResourceReadOnlyError, ConfigurationSetting, SecretReferenceConfigurationSetting, FeatureFlagConfigurationSetting, FILTER_PERCENTAGE, FILTER_TARGETING, FILTER_TIME_WINDOW
...
class TestAppConfigurationClientAAD(AppConfigTestCase):
    ...
    def test_list_configuration_settings_only_accepttime(self, appconfiguration_endpoint_string, **kwargs):
        ...
        self.set_up(appconfiguration_endpoint_string, is_aad=True)
        exclude_today = self.client.list_configuration_settings(accept_datetime=recorded_variables.setdefault('datetime', str(datetime.datetime.today() + datetime.timedelta(days=-1))))
