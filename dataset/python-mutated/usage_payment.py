import asyncio
import logging
from lbry.error import InsufficientFundsError, ServerPaymentFeeAboveMaxAllowedError, ServerPaymentInvalidAddressError, ServerPaymentWalletLockedError
    ...
    async def _pay(self):
        while self.running:
            ...
            if self.wallet.is_locked:
                ...
                continue
            amount = lbc_to_dewies(features['daily_fee'])
            limit = lbc_to_dewies(self.max_fee)
            if amount > limit:
                log.info('pay loop: amount (%d) > limit (%d)', amount, limit)
                self._on_payment_controller.add_error(ServerPaymentFeeAboveMaxAllowedError(features['daily_fee'], self.max_fee))
                continue
            try:
                tx = await Transaction.create([], [Output.pay_pubkey_hash(amount, self.ledger.address_to_hash160(address))], self.wallet.get_accounts_or_all(None), self.wallet.get_account_or_default(None))
