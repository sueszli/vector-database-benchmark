...
class Checksum(BaseAddon):
    ...
    __version__ = '0.36'
    __status__ = 'testing'
    __config__ = [('enabled', 'bool', 'Activated', False), ('check_checksum', 'bool', 'Check checksum? (If False only size will be verified)', True), ('check_action', 'fail;retry;nothing', 'What to do if check fails?', 'retry'), ('max_tries', 'int', 'Number of retries', 2), ('retry_action', 'fail;nothing', 'What to do if all retries fail?', 'fail'), ('wait_time', 'int', 'Time to wait before each retry (seconds)', 1)]
    __description__ = 'Verify downloaded file size and checksum'
    __license__ = 'GPLv3'
    __authors__ = [('zoidberg', 'zoidberg@mujmail.cz'), ('Walter Purcaro', 'vuolter@gmail.com'), ('stickell', 'l.stickell@yahoo.it'), ('GammaC0de', 'nitzo2001[AT]yahoo[DOT]com')]
    _methodmap = {'sfv': 'crc32', 'crc': 'crc32', 'hash': 'md5'}
    _regexmap = {'sfv': '^(?P<NAME>[^;].+)\\s+(?P<HASH>[0-9A-Fa-f]{8})$', 'md5': '^(?P<NAME>[0-9A-Fa-f]{32})  (?P<FILE>.+)$', 'crc': 'filename=(?P<NAME>.+)\\nsize=(?P<SIZE>\\d+)\\ncrc32=(?P<HASH>[0-9A-Fa-f]{8})$', 'default': '^(?P<HASH>[0-9A-Fa-f]+)\\s+\\*?(?P<NAME>.+)$'}
