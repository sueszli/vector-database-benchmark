...
class OrderGrantRefundCreateLineError(Error):
    code = OrderGrantRefundCreateLineErrorCode(description='The error code.', required=True)
...
class OrderGrantRefundCreateError(Error):
    ...
    lines = NonNullList(OrderGrantRefundCreateLineError, description='List of lines which cause the error.' + ADDED_IN_315 + PREVIEW_FEATURE, required=False)
...
class OrderGrantRefundCreateInput(BaseInputObjectType):
    amount = Decimal(description='Amount of the granted refund. If not provided, the amount will be calculated automatically based on provided `lines` and `grantRefundForShipping`.')
    reason = graphene.String(description='Reason of the granted refund.')
    lines = NonNullList(OrderGrantRefundCreateLineInput, description='Lines to assign to granted refund.' + ADDED_IN_315 + PREVIEW_FEATURE, required=False)
...
class OrderGrantRefundCreate(BaseMutation):
    order = graphene.Field(Order, description='Order which has assigned new grant refund.')
    ...
    class Arguments:
        ...
        input = OrderGrantRefundCreateInput(required=True, description='Fields required to create a granted refund for the order.')

    class Meta:
        description = 'Adds granted refund to the order.' + ADDED_IN_313 + PREVIEW_FEATURE
