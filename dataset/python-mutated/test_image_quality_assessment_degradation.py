...
class ImageQualityAssessmentDegradationTest(unittest.TestCase):
    ...
    def test_run_with_model_from_modelhub(self):
        ...
        pipeline_ins = pipeline(task=Tasks.image_quality_assessment_degradation, model=model)
    ...
    def test_run_with_model_name(self):
        if False:
            for i in range(10):
                print('nop')
        pipeline_ins = pipeline(task=Tasks.image_quality_assessment_degradation, model=self.model_id)
        out_path = pipeline_ins(input=self.test_img)[OutputKeys.SCORES]
        print('pipeline: the out_path is {}'.format(out_path))
    ...
    def test_run_with_default_model(self):
        if False:
            for i in range(10):
                print('nop')
        pipeline_ins = pipeline(task=Tasks.image_quality_assessment_degradation)
        out_path = pipeline_ins(input=self.test_img)[OutputKeys.SCORES]
        print('pipeline: the out_path is {}'.format(out_path))
