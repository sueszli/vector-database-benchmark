...
async def test_websocket_handler_upgrade_to_websocket(http_client, auth_creds, content_type_map, http_server_port, io_loop):
    ...
    request = HTTPRequest(url, headers={'Origin': 'http://example.com', 'Host': 'example.com'})
...
async def test_websocket_handler_bad_token(client_config, http_server, io_loop):
    ...
    """
    token = 'A' * len(getattr(hashlib, client_config.get('hash_type', 'md5'))().hexdigest())
...
async def test_websocket_handler_cors_origin_wildcard(app, http_client, auth_creds, content_type_map, http_server_port, io_loop):
    app.mod_opts['cors_origin'] = '*'
    response = await http_client.fetch('/login', method='POST', body=urllib.parse.urlencode(auth_creds), headers={'Content-Type': content_type_map['form']})
    token = salt.utils.json.loads(response.body)['return'][0]['token']
    url = 'ws://127.0.0.1:{}/all_events/{}'.format(http_server_port, token)
    request = HTTPRequest(url, headers={'Origin': 'http://foo.bar', 'Host': 'example.com'})
    ws = await websocket_connect(request)
    ws.write_message('websocket client ready')
    ws.close()

async def test_cors_origin_single(app, http_client, auth_creds, content_type_map, http_server_port, io_loop):
...
async def test_cors_origin_multiple(app, http_client, auth_creds, content_type_map, http_server_port, io_loop):
