...
class TREC07(base.RemoteDataset):
    ...
    def __init__(self):
        if False:
            print('Hello World!')
        super().__init__(n_samples=75419, n_features=5, task=base.BINARY_CLF, url='https://maxhalford.github.io/files/datasets/trec07p.zip', size=144504829, filename='trec07p.csv')

    def _iter(self):
        if False:
            return 10
        return stream.iter_csv(self.path, target='y', delimiter=',', quotechar='"', field_size_limit=1000000)