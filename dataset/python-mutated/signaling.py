...
class CopyAndPasteSignaling:
    ...
    async def connect(self):
        ...
        (self._read_transport, _) = await loop.connect_read_pipe(lambda : asyncio.StreamReaderProtocol(self._reader), self._read_pipe)
    ...
    async def _connect(self, server):
        ...
        if server:
            ...
            def client_connected(reader, writer):
                ...
                self._writer = writer
                connected.set()
            self._server = await asyncio.start_server(client_connected, host=self._host, port=self._port)
