...
@unittest.skipIf(not _HAS_SKLEARN, 'Missing sklearn. Skipping tests.')
class DecisionTreeBinaryClassificationBostonHousingScikitNumericTest(DecisionTreeClassificationBostonHousingScikitNumericTest):
    ...
    def test_binary_classifier_stress_test(self):
        if False:
            i = 10
            return i + 15
        options = dict(splitter=['best'], max_depth=[1, 10, None], min_samples_split=[2, 10, 0.5], min_samples_leaf=[1, 5], min_weight_fraction_leaf=[0.0, 0.5], max_features=[None, 1, 5], max_leaf_nodes=[None, 20], presort=[False, True])
        import itertools
...
@unittest.skipIf(not _HAS_SKLEARN, 'Missing sklearn. Skipping tests.')
class DecisionTreeMultiClassClassificationBostonHousingScikitNumericTest(DecisionTreeClassificationBostonHousingScikitNumericTest):
