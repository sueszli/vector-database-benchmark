...
from sentry.db.models import BoundedPositiveIntegerField, FlexibleForeignKey, region_silo_only_model, sane_repr
...
from sentry.services.hybrid_cloud.organizationmember_mapping import RpcOrganizationMemberMappingUpdate, organizationmember_mapping_service
...
_OrganizationMemberFlags = TypedDict('_OrganizationMemberFlags', {'sso:linked': bool, 'sso:invalid': bool, 'member-limit:restricted': bool, 'idp:provisioned': bool, 'idp:role-restricted': bool, 'partnership:restricted': bool})
...
invite_status_names = {InviteStatus.APPROVED.value: 'approved', InviteStatus.REQUESTED_TO_BE_INVITED.value: 'requested_to_be_invited', InviteStatus.REQUESTED_TO_JOIN.value: 'requested_to_join'}
...
class OrganizationMember(ReplicatedRegionModel):
    ...
    inviter_id = HybridCloudForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete='SET_NULL')
    invite_status = models.PositiveSmallIntegerField(choices=InviteStatus.as_choices(), default=InviteStatus.APPROVED.value, null=True)
    type = BoundedPositiveIntegerField(default=50, blank=True)
    user_is_active = models.BooleanField(null=False, default=True)
