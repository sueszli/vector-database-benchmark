...
class MyTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        if False:
            i = 10
            return i + 15
        fixtures = {'proc_uptime': ('fixtures/linux-proc/uptime', 'fixtures/linux-proc/uptime.json')}
    ...
    def test_proc_uptime(self):
        if False:
            for i in range(10):
                print('nop')
        "\n        Test '/proc/uptime'\n        "
        self.assertEqual(jc.parsers.proc_uptime.parse(self.f_in['proc_uptime'], quiet=True), self.f_json['proc_uptime'])
