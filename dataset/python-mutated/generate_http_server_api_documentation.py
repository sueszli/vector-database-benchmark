...
TPLS = {}
TPLS['index-response'] = 'HTTP/1.1 200 OK\nContent-Type: application/json\n\n%(index)s\n'
TPLS['api-index-response'] = 'HTTP/1.1 200 OK\nContent-Type: application/json\n\n%(api_index)s\n'
TPLS['get-tx-id-request'] = 'GET /api/v1/transactions/%(txid)s HTTP/1.1\nHost: example.com\n\n'
TPLS['get-tx-id-response'] = 'HTTP/1.1 200 OK\nContent-Type: application/json\n\n%(tx)s\n'
TPLS['get-tx-by-asset-request'] = 'GET /api/v1/transactions?operation=TRANSFER&asset_id=%(txid)s HTTP/1.1\nHost: example.com\n\n'
TPLS['get-tx-by-asset-response'] = 'HTTP/1.1 200 OK\nContent-Type: application/json\n\n[%(tx_transfer)s,\n%(tx_transfer_last)s]\n'
TPLS['post-tx-request'] = 'POST /api/v1/transactions?mode=async HTTP/1.1\nHost: example.com\nContent-Type: application/json\n\n%(tx)s\n'
TPLS['post-tx-response'] = 'HTTP/1.1 202 Accepted\nContent-Type: application/json\n\n%(tx)s\n'
TPLS['get-block-request'] = 'GET /api/v1/blocks/%(blockid)s HTTP/1.1\nHost: example.com\n\n'
TPLS['get-block-response'] = 'HTTP/1.1 200 OK\nContent-Type: application/json\n\n%(block)s\n'
TPLS['get-block-txid-request'] = 'GET /api/v1/blocks?transaction_id=%(txid)s HTTP/1.1\nHost: example.com\n\n'
TPLS['get-block-txid-response'] = 'HTTP/1.1 200 OK\nContent-Type: application/json\n\n%(block_list)s\n'
