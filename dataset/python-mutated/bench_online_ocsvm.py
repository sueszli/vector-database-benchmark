...
for (dat, dataset_name) in enumerate(datasets):
    ...
    results_online[dat] = [fit_time_online, predict_time_online, auc_online, n_samples_train, n_features] + list(tpr_libsvm)
...
x_tickslabels = [(name + '\n' + '$n={:,d}$' + '\n' + '$d={:d}$').format(int(n), int(d)) for (name, n, d) in zip(datasets, n_train_all, n_features_all)]
