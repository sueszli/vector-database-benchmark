...
def build_get_request(best_practice_name: str, **kwargs: Any) -> HttpRequest:
    if False:
        return 10
...
class BestPracticesOperations:
    ...
    @distributed_trace
    def get(self, best_practice_name: str, **kwargs: Any) -> _models.BestPractice:
        if False:
            while True:
                i = 10
        ...
        request = build_get_request(best_practice_name=best_practice_name, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        ...
        def prepare_request(next_link=None):
            if False:
                for i in range(10):
                    print('nop')
            if not next_link:
                request = build_list_by_tenant_request(api_version=api_version, template_url=self.list_by_tenant.metadata['url'], headers=_headers, params=_params)
                request = _convert_request(request)
                request.url = self._client.format_url(request.url)
            else:
