...
PACKET_TYPE = {ADVERTISE: 'ADVERTISE', SEARCHGW: 'SEARCHGW', GWINFO: 'GWINFO', CONNECT: 'CONNECT', CONNACK: 'CONNACK', WILLTOPICREQ: 'WILLTOPICREQ', WILLTOPIC: 'WILLTOPIC', WILLMSGREQ: 'WILLMSGREQ', WILLMSG: 'WILLMSG', REGISTER: 'REGISTER', REGACK: 'REGACK', PUBLISH: 'PUBLISH', PUBACK: 'PUBACK', PUBCOMP: 'PUBCOMP', PUBREC: 'PUBREC', PUBREL: 'PUBREL', SUBSCRIBE: 'SUBSCRIBE', SUBACK: 'SUBACK', UNSUBSCRIBE: 'UNSUBSCRIBE', UNSUBACK: 'UNSUBACK', PINGREQ: 'PINGREQ', PINGRESP: 'PINGRESP', DISCONNECT: 'DISCONNECT', WILLTOPICUPD: 'WILLTOPICUPD', WILLTOPICRESP: 'WILLTOPICRESP', WILLMSGUPD: 'WILLMSGUPD', WILLMSGRESP: 'WILLMSGRESP', ENCAPS_MSG: 'Encapsulated message'}
QOS_LEVELS = {QOS_0: 'Fire and Forget', QOS_1: 'Acknowledged deliver', QOS_2: 'Assured Delivery', QOS_NEG1: 'No Connection required'}
TOPIC_ID_TYPES = {TID_NORMAL: 'Normal ID', TID_PREDEF: 'Pre-defined ID', TID_SHORT: 'Short Topic Name', TID_RESVD: 'Reserved'}
RETURN_CODES = {ACCEPTED: 'Accepted', REJ_CONJ: 'Rejected: congestion', REJ_TID: 'Rejected: invalid topic ID', REJ_NOTSUP: 'Rejected: not supported'}
FLAG_FIELDS = [BitField('dup', 0, 1), BitEnumField('qos', QOS_0, 2, QOS_LEVELS), BitField('retain', 0, 1), BitField('will', 0, 1), BitField('cleansess', 0, 1), BitEnumField('tid_type', TID_NORMAL, 2, TOPIC_ID_TYPES)]
...
class MQTTSN(Packet):
    name = 'MQTT-SN header'
    fields_desc = [VariableFieldLenField('len', None, length_of='len', adjust=_mqttsn_len_adjust), XByteEnumField('type', 0, PACKET_TYPE)]

class MQTTSNAdvertise(Packet):
    name = 'MQTT-SN advertise gateway'
    fields_desc = [ByteField('gw_id', 0), ShortField('duration', 0)]

class MQTTSNSearchGW(Packet):
    name = 'MQTT-SN search gateway'
    fields_desc = [ByteField('radius', 0)]

class MQTTSNGwInfo(Packet):
    name = 'MQTT-SN gateway info'
    fields_desc = [ByteField('gw_id', 0), StrLenField('gw_addr', '', length_from=_mqttsn_length_from(1))]

class MQTTSNConnect(Packet):
    name = 'MQTT-SN connect command'
    fields_desc = FLAG_FIELDS + [ByteField('prot_id', 1), ShortField('duration', 0), StrLenField('client_id', '', length_from=_mqttsn_length_from(4))]

class MQTTSNConnack(Packet):
    name = 'MQTT-SN connect ACK'
    fields_desc = [ByteEnumField('return_code', ACCEPTED, RETURN_CODES)]
...
class MQTTSNWillTopic(Packet):
    ...
    fields_desc = FLAG_FIELDS + [StrLenField('will_topic', '', length_from=_mqttsn_length_from(1))]
...
class MQTTSNRegister(Packet):
    ...
    fields_desc = [ShortField('tid', 0), ShortField('mid', 0), StrLenField('topic_name', '', length_from=_mqttsn_length_from(4))]

class MQTTSNRegack(Packet):
    name = 'MQTT-SN register ACK'
    fields_desc = [ShortField('tid', 0), ShortField('mid', 0), ByteEnumField('return_code', ACCEPTED, RETURN_CODES)]

class MQTTSNPublish(Packet):
    name = 'MQTT-SN publish message'
    fields_desc = FLAG_FIELDS + [ShortField('tid', 0), ShortField('mid', 0), StrLenField('data', '', length_from=_mqttsn_length_from(5))]
...
class MQTTSNSubscribe(Packet):
    ...
    fields_desc = FLAG_FIELDS + [ShortField('mid', 0), ConditionalField(ShortField('tid', None), lambda pkt: pkt.tid_type == 1), ConditionalField(StrFixedLenField('short_topic', None, length=2), lambda pkt: pkt.tid_type == 2), ConditionalField(StrLenField('topic_name', None, length_from=_mqttsn_length_from(3)), lambda pkt: pkt.tid_type not in [1, 2])]

class MQTTSNSuback(Packet):
    name = 'MQTT-SN subscribe ACK'
    fields_desc = FLAG_FIELDS + [ShortField('tid', 0), ShortField('mid', 0), ByteEnumField('return_code', ACCEPTED, RETURN_CODES)]
...
class MQTTSNPingReq(Packet):
    ...
    fields_desc = [StrLenField('client_id', '', length_from=_mqttsn_length_from(0))]
...
class MQTTSNDisconnect(Packet):
    ...
    fields_desc = [ConditionalField(ShortField('duration', None), lambda pkt: hasattr(pkt.underlayer, 'len') and (pkt.underlayer.len is None or pkt.underlayer.len > 2))]
...
class MQTTSNEncaps(Packet):
    ...
    fields_desc = [BitField('resvd', 0, 6), BitField('radius', 0, 2), StrLenField('w_node_id', '', length_from=_mqttsn_length_from(1))]
