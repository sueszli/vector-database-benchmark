...
num_gens = [lambda x: np.int32(x.idx_in_batch - 3), lambda x: np.int32(-1 if x.idx_in_batch % 2 == 0 else 1), lambda x: np.int32((x.idx_in_batch % 3 == 0) - 1), lambda _: np.int32(1), lambda _: np.int32(0), lambda _: np.int32(-1), lambda _: rng.choice([np.int32(-2), np.int32(0), np.int32(2)])]
pred_gens = [lambda x: np.array(x.idx_in_batch < 3), lambda x: np.array(x.idx_in_batch % 2 == 0), lambda x: np.array(x.idx_in_batch % 3 == 0), lambda x: np.array((x.idx_in_batch + x.iteration % 2) % 2 == 0), lambda _: np.array(False), lambda _: rng.choice([np.array(True), np.array(False)])]
...
def test_error_condition():
    ...
    with assert_raises(RuntimeError, glob='Conditions inside `if` statements are restricted to scalar (0-d tensors) inputs, that are placed on CPU. Got a 1-d input as a condition of the `if` statement.'):
        ...
        pipe.build()
        pipe.run()
boolable_types = [bool, np.uint8, np.uint16, np.uint32, np.uint64, np.int8, np.int16, np.int32, np.int64, np.float16, np.float32, np.float64]
