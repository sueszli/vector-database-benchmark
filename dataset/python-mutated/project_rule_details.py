...
from sentry.models.integrations.sentry_app_installation import SentryAppInstallation, prepare_ui_component
...
class ProjectRuleDetailsPutSerializer(serializers.Serializer):
    ...
    conditions = serializers.ListField(child=RuleNodeField(type='condition/event'), help_text='A list of triggers that determine when the rule fires. See [Create an Issue Alert Rule](/api/alerts/create-an-issue-alert-rule-for-a-project) for valid conditions.')
    actions = serializers.ListField(child=RuleNodeField(type='action/event'), help_text='A list of actions that take place when all required conditions and filters for the rule are met. See [Create an Issue Alert Rule](/api/alerts/create-an-issue-alert-rule-for-a-project) for valid actions.')
    ...
    filters = serializers.ListField(child=RuleNodeField(type='filter/event'), required=False, help_text='A list of filters that determine if a rule fires after the necessary conditions have been met. See [Create an Issue Alert Rule](/api/alerts/create-an-issue-alert-rule-for-a-project) for valid filters.')
