...
from pony.orm.ormtypes import numeric_types, comparable_types, SetType, FuncType, MethodType, raw_sql, RawSQLType, normalize, normalize_type, coerce_types, are_comparable_types, Json, QueryType, Array, array_types
from pony.orm import core
from pony.orm.core import EntityMeta, Set, JOIN, OptimizationFailed, Attribute, DescWrapper, special_functions, const_functions, extract_vars, Query, UseAnotherTranslator
NoneType = type(None)
...
typeerror_re_1 = re.compile('\\(\\) takes (no|(?:exactly|at (?:least|most)))(?: (\\d+))? arguments \\((\\d+) given\\)')
typeerror_re_2 = re.compile('\\(\\) takes from (\\d+) to (\\d+) positional arguments but (\\d+) were given')
