...
from qt.core import QAbstractItemView, QComboBox, QDialog, QDialogButtonBox, QHBoxLayout, QIcon, QInputDialog, QItemSelectionModel, QLabel, QLineEdit, QListWidget, QListWidgetItem, QMenu, QPalette, QPushButton, QSize, QStaticText, QStyle, QStyledItemDelegate, Qt, QToolButton, QVBoxLayout, QWidget, pyqtSignal
...
class RuleEdit(QWidget):
    ACTION_MAP = OrderedDict((('remove', _('Remove')), ('replace', _('Replace')), ('keep', _('Keep')), ('capitalize', _('Capitalize')), ('titlecase', _('Title-case')), ('lower', _('Lower-case')), ('upper', _('Upper-case')), ('split', _('Split'))))
    MATCH_TYPE_MAP = OrderedDict((('one_of', _('is one of')), ('not_one_of', _('is not one of')), ('matches', _('matches pattern')), ('not_matches', _('does not match pattern')), ('has', _('contains'))))
    ...
    SPLIT_TOOLTIP = _('The character on which to split tags. Note that technically you can specify a sub-string, not just a single character. Then splitting will happen on the sub-string.')
    REPLACE_TOOLTIP = _('What to replace the tag with. Note that if you use a pattern to match tags, you can replace with parts of the matched pattern. See  the User Manual on how to use regular expressions for details.')
...
class Rules(QWidget):
    ...
    changed = pyqtSignal()
    ACTION_KEY = 'action'
    MSG = _('You can specify rules to filter/transform tags here. Click the "Add rule" button below to get started. The rules will be processed in order for every tag until either a "remove" or a "keep" rule matches.')
...
if __name__ == '__main__':
    app = Application([])
    d = RulesDialog()
    d.rules = [{'action': 'remove', 'query': 'moose', 'match_type': 'one_of', 'replace': ''}, {'action': 'replace', 'query': 'moose,sfdg,sfdg,dfsg,dfgsh,sd,er,erg,egrer,ger,s,fgfsgfsga', 'match_type': 'one_of', 'replace': 'xxxx'}, {'action': 'split', 'query': '/', 'match_type': 'has', 'replace': '/'}]
    d.exec()
