...
from troposphere.validators import boolean, integer, integer_range, network_port, positive_integer

class Firewall(AWSObject):
    resource_type = 'OS::Neutron::Firewall'
    props = {'admin_state_up': (boolean, False), 'description': (str, False), 'firewall_policy_id': (str, True), 'name': (str, False)}

class FirewallPolicy(AWSObject):
    resource_type = 'OS::Neutron::FirewallPolicy'
    props = {'audited': (boolean, False), 'description': (str, False), 'firewall_rules': (list, True), 'name': (str, False), 'shared': (boolean, False)}

class FirewallRule(AWSObject):
    resource_type = 'OS::Neutron::FirewallRule'
    props = {'action': (str, False), 'description': (str, False), 'destination_ip_address': (str, False), 'destination_port': (network_port, False), 'enabled': (boolean, False), 'ip_version': (str, False), 'name': (str, False), 'protocol': (str, False), 'shared': (boolean, False), 'source_ip_address': (str, False), 'source_port': (network_port, False)}
...
class FloatingIP(AWSObject):
    ...
    props = {'fixed_ip_address': (str, False), 'floating_network_id': (str, True), 'port_id': (str, False), 'value_specs': (dict, False)}

class FloatingIPAssociation(AWSObject):
    resource_type = 'OS::Neutron::FloatingIPAssociation'
    props = {'fixed_ip_address': (str, False), 'floatingip_id': (str, True), 'port_id': (str, False)}

class HealthMonitor(AWSObject):
    resource_type = 'OS::Neutron::HealthMonitor'
    props = {'admin_state_up': (boolean, False), 'delay': (positive_integer, True), 'expected_codes': (str, False), 'http_method': (str, False), 'max_retries': (integer, True), 'timeout': (integer, True), 'type': (str, True), 'url_path': (str, False)}
...
class SessionPersistence(AWSProperty):
    props = {'cookie_name': (str, False), 'type': (str, False)}
...
class VIP(AWSProperty):
    props = {'address': (str, False), 'admin_state_up': (boolean, False), 'connection_limit': (integer, True), 'description': (str, False), 'name': (str, False), 'protocol_port': (network_port, True), 'session_persistence': (SessionPersistence, False)}

class Pool(AWSObject):
    resource_type = 'OS::Neutron::Pool'
    props = {'admin_state_up': (boolean, False), 'description': (str, False), 'lb_method': (str, True), 'monitors': (list, False), 'name': (str, False), 'protocol': (str, True), 'subnet_id': (str, True), 'vip': (VIP, False)}
...
class LoadBalancer(AWSObject):
    ...
    props = {'members': (list, False), 'pool_id': (Pool, True), 'protocol_port': (network_port, True)}

class Net(AWSObject):
    resource_type = 'OS::Neutron::Net'
    props = {'admin_state_up': (boolean, False), 'name': (str, False), 'shared': (boolean, False), 'tenant_id': (str, False), 'value_specs': (dict, False)}

class PoolMember(AWSObject):
    resource_type = 'OS::Neutron::PoolMember'
    props = {'address': (str, True), 'admin_state_up': (boolean, False), 'pool_id': (Pool, True), 'protocol_port': (network_port, True), 'weight': (integer_range(0, 256), False)}

class AddressPair(AWSProperty):
    props = {'ip_address': (str, True), 'mac_address': (str, False)}

class FixedIP(AWSProperty):
    props = {'ip_address': (str, False), 'subnet_id': (str, False)}

class Port(AWSObject):
    resource_type = 'OS::Neutron::Port'
    props = {'admin_state_up': (boolean, False), 'allowed_address_pairs': (list, False), 'device_id': (str, False), 'fixed_ips': (list, False), 'mac_address': (str, False), 'name': (str, False), 'network_id': (str, True), 'security_groups': (list, False), 'value_specs': (dict, False)}

class SecurityGroup(AWSObject):
    resource_type = 'OS::Neutron::SecurityGroup'
    props = {'description': (str, True), 'name': (str, False), 'rules': (list, False)}

class SecurityGroupRule(AWSProperty):
    props = {'direction': (str, False), 'ethertype': (str, False), 'port_range_max': (network_port, False), 'port_range_min': (network_port, False), 'protocol': (str, False), 'remote_group_id': (str, False), 'remote_ip_prefix': (str, False), 'remote_mode': (str, False)}
