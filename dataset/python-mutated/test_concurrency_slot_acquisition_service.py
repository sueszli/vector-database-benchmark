...
async def mocked_client(test_database_connection_url):
    async with get_client() as client:
        with mock.patch.object(client, 'increment_concurrency_slots', autospec=True):
            ...
            with mock.patch('prefect.concurrency.services.get_client', lambda : wrapped_client):
...
async def test_returns_successful_response(mocked_client):
    ...
    mocked_method.assert_called_once_with(names=expected_names, slots=expected_slots, mode=expected_mode)

async def test_retries_failed_call_respects_retry_after_header(mocked_client):
    responses = [HTTPStatusError('Limit is locked', request=Request('get', '/'), response=Response(423, headers={'Retry-After': '2'})), Response(200)]
    ...
    with mock.patch('prefect.concurrency.asyncio.asyncio.sleep') as sleep:
        ...
        sleep.assert_called_once_with(float(responses[0].response.headers['Retry-After']))
...
async def test_failed_call_status_code_not_retryable_returns_exception(mocked_client):
    response = HTTPStatusError('Too many requests', request=Request('get', '/'), response=Response(500, headers={'Retry-After': '2'}))
