...
@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_definition_sql', [('/fixtures/facet_cities', '\nCREATE TABLE facet_cities (\n    id integer primary key,\n    name text\n);\n        '.strip()), ('/fixtures/compound_three_primary_keys', '\nCREATE TABLE compound_three_primary_keys (\n  pk1 varchar(30),\n  pk2 varchar(30),\n  pk3 varchar(30),\n  content text,\n  PRIMARY KEY (pk1, pk2, pk3)\n);\nCREATE INDEX idx_compound_three_primary_keys_content ON compound_three_primary_keys(content);\n            '.strip())])
...
async def test_add_filter_redirects(ds_client):
    filter_args = urllib.parse.urlencode({'_filter_column': 'content', '_filter_op': 'startswith', '_filter_value': 'x'})
    ...
    path = path_base + '?' + urllib.parse.urlencode({'_filter_column': 'content', '_filter_op': 'isnull__5', '_filter_value': 'x'})
    response = await ds_client.get(path)
    assert response.status_code == 302
...
async def test_existing_filter_redirects(ds_client):
    filter_args = {'_filter_column_1': 'name', '_filter_op_1': 'contains', '_filter_value_1': 'hello', '_filter_column_2': 'age', '_filter_op_2': 'gte', '_filter_value_2': '22', '_filter_column_3': 'age', '_filter_op_3': 'lt', '_filter_value_3': '30', '_filter_column_4': 'name', '_filter_op_4': 'contains', '_filter_value_4': 'world'}
    path_base = '/fixtures/simple_primary_key'
    path = path_base + '?' + urllib.parse.urlencode(filter_args)
    response = await ds_client.get(path)
    assert response.status_code == 302
    assert_querystring_equal('name__contains=hello&age__gte=22&age__lt=30&name__contains=world', response.headers['Location'].split('?')[1])
    filter_args['_filter_column_3'] = ''
    path = path_base + '?' + urllib.parse.urlencode(filter_args)
    response = await ds_client.get(path)
    assert response.status_code == 302
    assert_querystring_equal('name__contains=hello&age__gte=22&name__contains=world', response.headers['Location'].split('?')[1])
    response = await ds_client.get(path_base + '?_filter_op=exact')
    assert response.status_code == 302
    assert '?' not in response.headers['Location']

@pytest.mark.asyncio
@pytest.mark.parametrize('qs,expected_hidden', (('_facet=_neighborhood', {'_facet': '_neighborhood'}), ('_where=1+=+1&_col=_city_id', {'_where': '1 = 1', '_col': '_city_id'}), ('_facet=_neighborhood&_neighborhood__exact=Downtown', {'_facet': '_neighborhood'}), ('_facet=_neighborhood&_city_id__gt=1', {'_facet': '_neighborhood'})))
async def test_reflected_hidden_form_fields(ds_client, qs, expected_hidden):
    ...
    hidden_inputs = {input['name']: input['value'] for input in form.select('input[type=hidden]')}
    assert hidden_inputs == expected_hidden

@pytest.mark.asyncio
async def test_empty_search_parameter_gets_removed(ds_client):
    path_base = '/fixtures/simple_primary_key'
    path = path_base + '?' + urllib.parse.urlencode({'_search': '', '_filter_column': 'name', '_filter_op': 'exact', '_filter_value': 'chidi'})
    response = await ds_client.get(path)
    assert response.status_code == 302
...
async def test_searchable_view_persists_fts_table(ds_client):
    response = await ds_client.get('/fixtures/searchable_view?_fts_table=searchable_fts&_fts_pk=pk')
    inputs = Soup(response.text, 'html.parser').find('form').findAll('input')
    hiddens = [i for i in inputs if i['type'] == 'hidden']
    assert [('_fts_table', 'searchable_fts'), ('_fts_pk', 'pk')] == [(hidden['name'], hidden['value']) for hidden in hiddens]

@pytest.mark.asyncio
async def test_sort_by_desc_redirects(ds_client):
    path_base = '/fixtures/sortable'
    path = path_base + '?' + urllib.parse.urlencode({'_sort': 'sortable', '_sort_by_desc': '1'})
    response = await ds_client.get(path)
    assert response.status_code == 302
...
async def test_next_does_not_persist_in_hidden_field(ds_client):
    ...
    inputs = Soup(response.text, 'html.parser').find('form').findAll('input')
    hiddens = [i for i in inputs if i['type'] == 'hidden']
    assert [(hidden['name'], hidden['value']) for hidden in hiddens] == [('_size', '1')]
...
async def test_table_html_simple_primary_key(ds_client):
    ...
    assert [['<td class="col-id type-pk"><a href="/fixtures/simple_primary_key/1">1</a></td>', '<td class="col-content type-str">hello</td>'], ['<td class="col-id type-pk"><a href="/fixtures/simple_primary_key/2">2</a></td>', '<td class="col-content type-str">world</td>'], ['<td class="col-id type-pk"><a href="/fixtures/simple_primary_key/3">3</a></td>', '<td class="col-content type-str">\xa0</td>']] == [[str(td) for td in tr.select('td')] for tr in table.select('tbody tr')]
...
async def test_table_csv_json_export_interface(ds_client):
    ...
    assert response.status_code == 200
    links = Soup(response.text, 'html.parser').find('p', {'class': 'export-links'}).findAll('a')
    actual = [link['href'] for link in links]
    expected = ['/fixtures/simple_primary_key.json?id__gt=2', '/fixtures/simple_primary_key.testall?id__gt=2', '/fixtures/simple_primary_key.testnone?id__gt=2', '/fixtures/simple_primary_key.testresponse?id__gt=2', '/fixtures/simple_primary_key.csv?id__gt=2&_size=max', '#export']
    assert expected == actual
    div = Soup(response.text, 'html.parser').find('div', {'class': 'advanced-export'})
    json_links = [a['href'] for a in div.find('p').findAll('a')]
    assert ['/fixtures/simple_primary_key.json?id__gt=2', '/fixtures/simple_primary_key.json?id__gt=2&_shape=array', '/fixtures/simple_primary_key.json?id__gt=2&_shape=array&_nl=on', '/fixtures/simple_primary_key.json?id__gt=2&_shape=object'] == json_links
    form = div.find('form')
    assert form['action'].endswith('/simple_primary_key.csv')
    inputs = [str(input) for input in form.findAll('input')]
    assert ['<input name="_dl" type="checkbox"/>', '<input type="submit" value="Export CSV"/>', '<input name="id__gt" type="hidden" value="2"/>', '<input name="_size" type="hidden" value="max"/>'] == inputs
...
async def test_csv_json_export_links_include_labels_if_foreign_keys(ds_client):
    ...
    links = Soup(response.text, 'html.parser').find('p', {'class': 'export-links'}).findAll('a')
    actual = [link['href'] for link in links]
    expected = ['/fixtures/facetable.json?_labels=on', '/fixtures/facetable.testall?_labels=on', '/fixtures/facetable.testnone?_labels=on', '/fixtures/facetable.testresponse?_labels=on', '/fixtures/facetable.csv?_labels=on&_size=max', '#export']
    assert expected == actual
...
async def test_table_html_no_primary_key(ds_client):
    ...
    assert response.status_code == 200
    table = Soup(response.text, 'html.parser').find('table')
    assert ['content', 'a', 'b', 'c'] == [th.string.strip() for th in table.select('thead th')[2:]]
    expected = [['<td class="col-Link type-pk"><a href="/fixtures/no_primary_key/{}">{}</a></td>'.format(i, i), f'<td class="col-rowid type-int">{i}</td>', f'<td class="col-content type-str">{i}</td>', f'<td class="col-a type-str">a{i}</td>', f'<td class="col-b type-str">b{i}</td>', f'<td class="col-c type-str">c{i}</td>'] for i in range(1, 51)]
...
async def test_table_html_compound_primary_key(ds_client):
    ...
    expected = [['<td class="col-Link type-pk"><a href="/fixtures/compound_primary_key/a,b">a,b</a></td>', '<td class="col-pk1 type-str">a</td>', '<td class="col-pk2 type-str">b</td>', '<td class="col-content type-str">c</td>'], ['<td class="col-Link type-pk"><a href="/fixtures/compound_primary_key/a~2Fb,~2Ec-d">a/b,.c-d</a></td>', '<td class="col-pk1 type-str">a/b</td>', '<td class="col-pk2 type-str">.c-d</td>', '<td class="col-content type-str">c</td>']]
    assert [[str(td) for td in tr.select('td')] for tr in table.select('tbody tr')] == expected
...
async def test_table_html_foreign_key_links(ds_client):
    ...
    assert actual == [['<td class="col-pk type-pk"><a href="/fixtures/foreign_key_references/1">1</a></td>', '<td class="col-foreign_key_with_label type-str"><a href="/fixtures/simple_primary_key/1">hello</a>\xa0<em>1</em></td>', '<td class="col-foreign_key_with_blank_label type-str"><a href="/fixtures/simple_primary_key/3">-</a>\xa0<em>3</em></td>', '<td class="col-foreign_key_with_no_label type-str"><a href="/fixtures/primary_key_multiple_columns/1">1</a></td>', '<td class="col-foreign_key_compound_pk1 type-str">a</td>', '<td class="col-foreign_key_compound_pk2 type-str">b</td>'], ['<td class="col-pk type-pk"><a href="/fixtures/foreign_key_references/2">2</a></td>', '<td class="col-foreign_key_with_label type-none">\xa0</td>', '<td class="col-foreign_key_with_blank_label type-none">\xa0</td>', '<td class="col-foreign_key_with_no_label type-none">\xa0</td>', '<td class="col-foreign_key_compound_pk1 type-none">\xa0</td>', '<td class="col-foreign_key_compound_pk2 type-none">\xa0</td>']]

@pytest.mark.asyncio
async def test_table_html_foreign_key_facets(ds_client):
    response = await ds_client.get('/fixtures/foreign_key_references?_facet=foreign_key_with_blank_label')
    assert response.status_code == 200
    assert '<li><a href="http://localhost/fixtures/foreign_key_references?_facet=foreign_key_with_blank_label&amp;foreign_key_with_blank_label=3" data-facet-value="3">-</a> 1</li>' in response.text

@pytest.mark.asyncio
async def test_table_html_disable_foreign_key_links_with_labels(ds_client):
    response = await ds_client.get('/fixtures/foreign_key_references?_labels=off&_size=1')
    assert response.status_code == 200
    table = Soup(response.text, 'html.parser').find('table')
    actual = [[str(td) for td in tr.select('td')] for tr in table.select('tbody tr')]
    assert actual == [['<td class="col-pk type-pk"><a href="/fixtures/foreign_key_references/1">1</a></td>', '<td class="col-foreign_key_with_label type-str">1</td>', '<td class="col-foreign_key_with_blank_label type-str">3</td>', '<td class="col-foreign_key_with_no_label type-str">1</td>', '<td class="col-foreign_key_compound_pk1 type-str">a</td>', '<td class="col-foreign_key_compound_pk2 type-str">b</td>']]
...
async def test_table_html_foreign_key_custom_label_column(ds_client):
    ...
    assert response.status_code == 200
    table = Soup(response.text, 'html.parser').find('table')
    expected = [['<td class="col-pk type-pk"><a href="/fixtures/custom_foreign_key_label/1">1</a></td>', '<td class="col-foreign_key_with_custom_label type-str"><a href="/fixtures/primary_key_multiple_columns_explicit_label/1">world2</a>\xa0<em>1</em></td>']]
    assert expected == [[str(td) for td in tr.select('td')] for tr in table.select('tbody tr')]

@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_column_options', [('/fixtures/infinity', ['- column -', 'rowid', 'value']), ('/fixtures/primary_key_multiple_columns', ['- column -', 'id', 'content', 'content2']), ('/fixtures/compound_primary_key', ['- column -', 'pk1', 'pk2', 'content'])])
async def test_table_html_filter_form_column_options(path, expected_column_options, ds_client):
    response = await ds_client.get(path)
    assert response.status_code == 200
    form = Soup(response.text, 'html.parser').find('form')
    column_options = [o.attrs.get('value') or o.string for o in form.select('select[name=_filter_column] option')]
...
async def test_table_html_filter_form_still_shows_nocol_columns(ds_client):
    ...
    assert response.status_code == 200
    form = Soup(response.text, 'html.parser').find('form')
    assert [o.string for o in form.select("select[name='_filter_column']")[0].select('option')] == ['- column -', 'pk1', 'pk2', 'content', 'sortable_with_nulls', 'sortable_with_nulls_2', 'text', 'sortable']
...
async def test_compound_primary_key_with_foreign_key_references(ds_client):
    ...
    assert response.status_code == 200
    table = Soup(response.text, 'html.parser').find('table')
    expected = [['<td class="col-Link type-pk"><a href="/fixtures/searchable_tags/1,feline">1,feline</a></td>', '<td class="col-searchable_id type-int"><a href="/fixtures/searchable/1">1</a>\xa0<em>1</em></td>', '<td class="col-tag type-str"><a href="/fixtures/tags/feline">feline</a></td>'], ['<td class="col-Link type-pk"><a href="/fixtures/searchable_tags/2,canine">2,canine</a></td>', '<td class="col-searchable_id type-int"><a href="/fixtures/searchable/2">2</a>\xa0<em>2</em></td>', '<td class="col-tag type-str"><a href="/fixtures/tags/canine">canine</a></td>']]
    assert expected == [[str(td) for td in tr.select('td')] for tr in table.select('tbody tr')]
...
async def test_view_html(ds_client):
    ...
    expected = [['<td class="col-content type-str">hello</td>', '<td class="col-upper_content type-str">HELLO</td>'], ['<td class="col-content type-str">world</td>', '<td class="col-upper_content type-str">WORLD</td>'], ['<td class="col-content type-str">\xa0</td>', '<td class="col-upper_content type-str">\xa0</td>']]
    assert expected == [[str(td) for td in tr.select('td')] for tr in table.select('tbody tr')]
...
async def test_table_metadata(ds_client):
    ...
    assert 'Simple <em>primary</em> key' == inner_html(soup.find('div', {'class': 'metadata-description'}))
    assert_footer_links(soup)

@pytest.mark.asyncio
@pytest.mark.parametrize('path,has_object,has_stream,has_expand', [('/fixtures/no_primary_key', False, True, False), ('/fixtures/complex_foreign_keys', True, False, True)])
...
async def test_extra_where_clauses(ds_client):
    response = await ds_client.get("/fixtures/facetable?_where=_neighborhood='Dogpatch'&_where=_city_id=1")
    soup = Soup(response.text, 'html.parser')
    div = soup.select('.extra-wheres')[0]
    assert '2 extra where clauses' == div.find('h3').text
    hrefs = [a['href'] for a in div.findAll('a')]
    assert ['/fixtures/facetable?_where=_city_id%3D1', '/fixtures/facetable?_where=_neighborhood%3D%27Dogpatch%27'] == hrefs
    inputs = soup.find('form').findAll('input')
    hiddens = [i for i in inputs if i['type'] == 'hidden']
    assert [('_where', "_neighborhood='Dogpatch'"), ('_where', '_city_id=1')] == [(hidden['name'], hidden['value']) for hidden in hiddens]

@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_hidden', [('/fixtures/facetable?_size=10', [('_size', '10')]), ('/fixtures/facetable?_size=10&_ignore=1&_ignore=2', [('_size', '10'), ('_ignore', '1'), ('_ignore', '2')])])
...
@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected_hidden', [('/fixtures/searchable?_search=terry', []), ('/fixtures/searchable?_sort=text2', []), ('/fixtures/searchable?_sort_desc=text2', []), ('/fixtures/searchable?_sort=text2&_where=1', [('_where', '1')])])
...
async def test_binary_data_display_in_table(ds_client):
    ...
    assert response.status_code == 200
    table = Soup(response.text, 'html.parser').find('table')
    expected_tds = [['<td class="col-Link type-pk"><a href="/fixtures/binary_data/1">1</a></td>', '<td class="col-rowid type-int">1</td>', '<td class="col-data type-bytes"><a class="blob-download" href="/fixtures/binary_data/1.blob?_blob_column=data">&lt;Binary:\xa07\xa0bytes&gt;</a></td>'], ['<td class="col-Link type-pk"><a href="/fixtures/binary_data/2">2</a></td>', '<td class="col-rowid type-int">2</td>', '<td class="col-data type-bytes"><a class="blob-download" href="/fixtures/binary_data/2.blob?_blob_column=data">&lt;Binary:\xa07\xa0bytes&gt;</a></td>'], ['<td class="col-Link type-pk"><a href="/fixtures/binary_data/3">3</a></td>', '<td class="col-rowid type-int">3</td>', '<td class="col-data type-none">\xa0</td>']]
    assert expected_tds == [[str(td) for td in tr.select('td')] for tr in table.select('tbody tr')]
...
@pytest.mark.parametrize('params,error', (('?_sort=bad', 'Cannot sort table by bad'), ('?_sort_desc=bad', 'Cannot sort table by bad'), ('?_sort=state&_sort_desc=state', 'Cannot use _sort and _sort_desc at the same time')))
async def test_sort_errors(ds_client, json, params, error):
    path = '/fixtures/facetable{}{}'.format('.json' if json else '', params)
    response = await ds_client.get(path)
    assert response.status_code == 400
    if json:
        assert response.json() == {'ok': False, 'error': error, 'status': 400, 'title': None}
    else:
...
async def test_metadata_sort(ds_client):
    ...
    expected = [['<td class="col-id type-pk"><a href="/fixtures/facet_cities/3">3</a></td>', '<td class="col-name type-str">Detroit</td>'], ['<td class="col-id type-pk"><a href="/fixtures/facet_cities/2">2</a></td>', '<td class="col-name type-str">Los Angeles</td>'], ['<td class="col-id type-pk"><a href="/fixtures/facet_cities/4">4</a></td>', '<td class="col-name type-str">Memnonia</td>'], ['<td class="col-id type-pk"><a href="/fixtures/facet_cities/1">1</a></td>', '<td class="col-name type-str">San Francisco</td>']]
...
async def test_metadata_sort_desc(ds_client):
    ...
    rows = [[str(td) for td in tr.select('td')] for tr in table.select('tbody tr')]
    expected = [['<td class="col-pk type-pk"><a href="/fixtures/attraction_characteristic/2">2</a></td>', '<td class="col-name type-str">Paranormal</td>'], ['<td class="col-pk type-pk"><a href="/fixtures/attraction_characteristic/1">1</a></td>', '<td class="col-name type-str">Museum</td>']]
    assert expected == rows
...
async def test_column_metadata(ds_client):
    ...
    soup = Soup(response.text, 'html.parser')
    dl = soup.find('dl')
    assert [(dt.text, dt.nextSibling.text) for dt in dl.findAll('dt')] == [('name', 'The name of the attraction'), ('address', 'The street address for the attraction')]
    assert soup.select('th[data-column=name]')[0]['data-column-description'] == 'The name of the attraction'
    assert soup.select('th[data-column=address]')[0]['data-column-description'] == 'The street address for the attraction'
...
@pytest.mark.asyncio
@pytest.mark.parametrize('path,expected', (('/fixtures/facetable', 'fixtures: facetable: 15 rows'), ('/fixtures/facetable?on_earth__exact=1', 'fixtures: facetable: 14 rows where on_earth = 1')))
...
@pytest.mark.asyncio
@pytest.mark.parametrize('size,title,length_bytes', ((2000, ' title="2.0 KB"', '2,000'), (20000, ' title="19.5 KB"', '20,000'), (20, '', '20')))
...
@pytest.mark.asyncio
@pytest.mark.parametrize('config', ({'databases': {'foreign_key_labels': {'tables': {'a': {'allow': {'id': 'root'}}}}}}, {'databases': {'foreign_key_labels': {'allow': {'id': 'root'}, 'tables': {'b': {'allow': True}}}}}, {'allow': {'id': 'root'}, 'databases': {'foreign_key_labels': {'tables': {'b': {'allow': True}}}}}))
async def test_foreign_key_labels_obey_permissions(config):
    ...
    await db.execute_write('create table if not exists a(id integer primary key, name text)')
    await db.execute_write("insert or replace into a (id, name) values (1, 'hello')")
    await db.execute_write('create table if not exists b(id integer primary key, name text, a_id integer references a(id))')
    await db.execute_write("insert or replace into b (id, name, a_id) values (1, 'world', 1)")
    ...
    root_a = await ds.client.get('/foreign_key_labels/a.json?_labels=on', cookies=cookies)
    assert root_a.status_code == 200
    root_b = await ds.client.get('/foreign_key_labels/b.json?_labels=on', cookies=cookies)
    assert root_b.status_code == 200
    assert root_b.json() == {'ok': True, 'next': None, 'rows': [{'id': 1, 'name': 'world', 'a_id': {'value': 1, 'label': 'hello'}}], 'truncated': False}
    assert anon_b.json() == {'ok': True, 'next': None, 'rows': [{'id': 1, 'name': 'world', 'a_id': 1}], 'truncated': False}