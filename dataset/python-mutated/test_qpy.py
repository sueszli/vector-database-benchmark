...
def generate_full_circuit():
    ...
    full_circuit = QuantumCircuit(qr_a, qr_b, cr_c, cr_d, name='MyCircuit', metadata={'test': 1, 'a': 2}, global_phase=3.14159)
    full_circuit.h(qr_a)
...
def generate_qft_circuit():
    ...
    k = 5
    state = 1 / np.sqrt(8) * np.array([np.exp(-1j * 2 * np.pi * k * 0 / 8), np.exp(-1j * 2 * np.pi * k * 1 / 8), np.exp(-1j * 2 * np.pi * k * 2 / 8), np.exp(-1j * 2 * np.pi * k * 3 / 8), np.exp(-1j * 2 * np.pi * k * 4 / 8), np.exp(-1j * 2 * np.pi * k * 5 / 8), np.exp(-1j * 2 * np.pi * k * 6 / 8), np.exp(-1j * 2 * np.pi * k * 7 / 8)])
    qubits = 3
...
def generate_control_flow_switch_circuits():
    ...
    case_2.x(1)
    qc.switch(creg, [(0, case_0), ((1, 2), case_1), ((3, 4, CASE_DEFAULT), case_2)], qc.qubits, qc.clbits)
    circuits.append(qc)
    return circuits
...
def generate_schedule_blocks():
    ...
    with builder.build() as block:
        with builder.align_sequential():
            builder.set_frequency(5000000000.0, channels.DriveChannel(0))
            builder.shift_frequency(10000000.0, channels.DriveChannel(1))
    ...
    my_pulse = library.SymbolicPulse(pulse_type='Sinusoidal', duration=100, parameters={'amp': 0.1}, envelope=expr, valid_amp_conditions=sym.Abs(amp) <= 1.0)
    with builder.build() as block:
...
def generate_acquire_instruction_with_kernel_and_discriminator():
    ...
    schedule_blocks = []
    with builder.build() as block:
        builder.acquire(100, AcquireChannel(0), MemorySlot(0), kernel=Kernel(name='my_kernel', my_params_1={'param1': 0.1, 'param2': 0.2}, my_params_2=[0, 1]))
    schedule_blocks.append(block)
    with builder.build() as block:
        builder.acquire(100, AcquireChannel(0), MemorySlot(0), discriminator=Discriminator(name='my_disc', my_params_1={'param1': 0.1, 'param2': 0.2}, my_params_2=[0, 1]))
    schedule_blocks.append(block)
    return schedule_blocks
...
def generate_layout_circuits():
    ...
    qc._layout = TranspileLayout(Layout(input_layout), input_qubit_mapping=input_layout, final_layout=Layout.from_qubit_list([qc.qubits[1], qc.qubits[0], qc.qubits[2]]))
    return [qc]
...
def generate_control_flow_expr():
    ...
    outer4.if_test(expr.logic_and(expr.logic_or(expr.greater(expr.bit_or(cr2_4, 7), 10), expr.equal(expr.bit_and(cr1_4, cr1_4), expr.bit_not(cr1_4))), expr.logic_or(outer_loose, expr.cast(cr1_4, types.Bool()))), inner4, [0], [])
    qc4_loose = Clbit()
    qc4 = QuantumCircuit(QuantumRegister(2, 'qr4'), cr1_4, cr2_4, cr3_4, [qc4_loose], name='cf-expr4')
    qc4.rz(np.pi, 0)
    qc4.switch(expr.logic_and(expr.logic_or(expr.logic_or(expr.less(cr2_4, cr3_4), expr.logic_not(expr.greater_equal(cr3_4, cr2_4))), expr.logic_or(expr.logic_not(expr.less_equal(cr3_4, cr2_4)), expr.greater(cr2_4, cr3_4))), expr.logic_and(expr.equal(cr3_4, 2), expr.not_equal(expr.bit_xor(cr1_4, 15), 15))), [(False, outer4)], [1, 0], list(cr2_4) + [qc4_loose] + list(cr1_4))
    qc4.rz(np.pi, 0)
...
def generate_circuits(version_parts):
    ...
    output_circuits = {'full.qpy': [generate_full_circuit()], 'unitary.qpy': [generate_unitary_gate_circuit()], 'multiple.qpy': generate_random_circuits(), 'string_parameters.qpy': [generate_string_parameters()], 'register_edge_cases.qpy': generate_register_edge_cases(), 'parameterized.qpy': [generate_parameterized_circuit()]}
    ...
    if version_parts >= (0, 19, 1):
        ...
        output_circuits['parameter_vector_expression.qpy'] = [generate_parameter_vector_expression()]
    ...
    if version_parts >= (0, 25, 0):
        output_circuits['acquire_inst_with_kernel_and_disc.qpy'] = generate_acquire_instruction_with_kernel_and_discriminator()
...
def assert_equal(reference, qpy, count, version_parts, bind=None, equivalent=False):
    ...
    if bind is not None:
        ...
        if reference_parameter_names != qpy_parameter_names:
            msg = f'Circuit {count} parameter mismatch: {reference_parameter_names} != {qpy_parameter_names}'
    ...
    if equivalent:
        if not Operator.from_circuit(reference).equiv(Operator.from_circuit(qpy)):
            msg = f'Reference Circuit {count}:\n{reference}\nis not equivalent to qpy loaded circuit {count}:\n{qpy}\n'
            sys.stderr.write(msg)
            sys.exit(1)
    elif reference != qpy:
        msg = f'Reference Circuit {count}:\n{reference}\nis not equivalent to qpy loaded circuit {count}:\n{qpy}\n'
    ...
    if version_parts >= (0, 23, 2) and isinstance(reference, QuantumCircuit):
        for (ref_bit, qpy_bit) in itertools.chain(zip(reference.qubits, qpy.qubits), zip(reference.clbits, qpy.clbits)):
            if ref_bit._register is not None and ref_bit != qpy_bit:
                msg = f'Reference Circuit {count}:\ndeprecated bit-level register information mismatch\nreference bit: {ref_bit}\nloaded bit: {qpy_bit}\n'
                sys.stderr.write(msg)
                sys.exit(1)
    if version_parts >= (0, 24, 2) and isinstance(reference, QuantumCircuit) and (reference.layout != qpy.layout):
...
def load_qpy(qpy_files, version_parts):
    ...
    for (path, circuits) in qpy_files.items():
        ...
        for (i, circuit) in enumerate(circuits):
            ...
            assert_equal(circuit, qpy_circuits[i], i, version_parts, bind=bind, equivalent=equivalent)
...
def _main():
    ...
    parser.add_argument('--version', '-v', help='Optionally specify the version being tested. This will enable additional circuit features to test generating and loading QPY.')
