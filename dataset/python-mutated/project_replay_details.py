...
class ReplayDetailsPermission(ProjectPermission):
    scope_map = {'GET': ['project:read', 'project:write', 'project:admin'], 'POST': ['project:write', 'project:admin'], 'PUT': ['project:write', 'project:admin'], 'DELETE': ['project:read', 'project:write', 'project:admin']}
...
class ProjectReplayDetailsEndpoint(ProjectEndpoint):
    ...
    def get(self, request: Request, project: Project, replay_id: str) -> Response:
        ...
        try:
            ...
            return Response(status=404)
        snuba_response = query_replay_instance(project_id=project.id, replay_id=replay_id, start=filter_params['start'], end=filter_params['end'], organization=project.organization)
