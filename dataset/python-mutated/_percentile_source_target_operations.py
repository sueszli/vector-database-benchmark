...
def build_list_metrics_request(resource_group_name: str, account_name: str, source_region: str, target_region: str, subscription_id: str, *, filter: str, **kwargs: Any) -> HttpRequest:
    if False:
        while True:
            i = 10
    ...
    _url = kwargs.pop('template_url', '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.DocumentDB/databaseAccounts/{accountName}/sourceRegion/{sourceRegion}/targetRegion/{targetRegion}/percentile/metrics')
    path_format_arguments = {'subscriptionId': _SERIALIZER.url('subscription_id', subscription_id, 'str', min_length=1), 'resourceGroupName': _SERIALIZER.url('resource_group_name', resource_group_name, 'str', max_length=90, min_length=1), 'accountName': _SERIALIZER.url('account_name', account_name, 'str', max_length=50, min_length=3, pattern='^[a-z0-9]+(-[a-z0-9]+)*'), 'sourceRegion': _SERIALIZER.url('source_region', source_region, 'str'), 'targetRegion': _SERIALIZER.url('target_region', target_region, 'str')}
...
class PercentileSourceTargetOperations:
    ...
    @distributed_trace
    def list_metrics(self, resource_group_name: str, account_name: str, source_region: str, target_region: str, filter: str, **kwargs: Any) -> Iterable['_models.PercentileMetric']:
        if False:
            while True:
                i = 10
        ...
        def prepare_request(next_link=None):
            if False:
                ...
                return i + 15
            if not next_link:
                request = build_list_metrics_request(resource_group_name=resource_group_name, account_name=account_name, source_region=source_region, target_region=target_region, subscription_id=self._config.subscription_id, filter=filter, api_version=api_version, template_url=self.list_metrics.metadata['url'], headers=_headers, params=_params)
