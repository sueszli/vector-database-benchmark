...
def get_mock_provider():
    if False:
        return 10
    return Provider(id='provider_id', tenant_id='tenant_id', provider_name='replicate', provider_type=ProviderType.CUSTOM.value, encrypted_config='', is_valid=True)
...
def get_mock_model(model_name, model_version, mocker):
    ...
    mock_query.filter.return_value.first.return_value = ProviderModel(provider_name='replicate', model_name=model_name, model_type=ModelType.TEXT_GENERATION.value, encrypted_config=json.dumps({'replicate_api_token': valid_api_key, 'model_version': model_version}), is_valid=True)
    mocker.patch('extensions.ext_database.db.session.query', return_value=mock_query)
    return ReplicateModel(model_provider=model_provider, name=model_name, model_kwargs=model_kwargs)
