...
class TestRedisAutoReleaseConnectionPool:
    ...
    async def create_two_conn(r: redis.Redis):
        if not r.single_connection_client:
    ...
    async def test_do_not_auto_disconnect_redis_created_pool(self, r2: redis.Redis):
        assert r2.auto_close_connection_pool is False, 'The connection pool should not be disconnected as a manually created connection pool was passed in in conftest.py'
    ...
    async def test_auto_release_override_true_manual_created_pool(self, r: redis.Redis):
        ...
        await r.aclose()
        assert self.get_total_connected_connections(r.connection_pool) == 2, 'The connection pool should not be disconnected as a manually created connection pool was passed in in conftest.py'
        assert self.has_no_connected_connections(r.connection_pool)
    ...
    @pytest.mark.parametrize('auto_close_conn_pool', [True, False])
    async def test_negate_auto_close_client_pool(self, r: redis.Redis, auto_close_conn_pool):
        r.auto_close_connection_pool = auto_close_conn_pool
        new_conn = await self.create_two_conn(r)
...
class TestConnectionPool:
    ...
    @asynccontextmanager
    async def get_pool(self, connection_kwargs=None, max_connections=None, connection_class=redis.Connection):
        connection_kwargs = connection_kwargs or {}
        pool = redis.ConnectionPool(connection_class=connection_class, max_connections=max_connections, **connection_kwargs)
        try:
            yield pool
        finally:
    ...
    async def test_connection_creation(self):
        ...
        async with self.get_pool(connection_kwargs=connection_kwargs, connection_class=DummyConnection) as pool:
    ...
    async def test_aclosing(self):
        connection_kwargs = {'foo': 'bar', 'biz': 'baz'}
        pool = redis.ConnectionPool(connection_class=DummyConnection, max_connections=None, **connection_kwargs)
    ...
    async def test_max_connections(self, master_host):
        connection_kwargs = {'host': master_host[0]}
        async with self.get_pool(max_connections=2, connection_kwargs=connection_kwargs) as pool:
    ...
    async def test_repr_contains_db_info_tcp(self):
        connection_kwargs = {'host': 'localhost', 'port': 6379, 'db': 1, 'client_name': 'test-client'}
        async with self.get_pool(connection_kwargs=connection_kwargs, connection_class=redis.Connection) as pool:
            expected = 'ConnectionPool<Connection<host=localhost,port=6379,db=1,client_name=test-client>>'
            assert repr(pool) == expected

    async def test_repr_contains_db_info_unix(self):
        connection_kwargs = {'path': '/abc', 'db': 1, 'client_name': 'test-client'}
        async with self.get_pool(connection_kwargs=connection_kwargs, connection_class=redis.UnixDomainSocketConnection) as pool:
            expected = 'ConnectionPool<UnixDomainSocketConnection<path=/abc,db=1,client_name=test-client>>'
            assert repr(pool) == expected
...
class TestBlockingConnectionPool:
    ...
    async def get_pool(self, connection_kwargs=None, max_connections=10, timeout=20):
        connection_kwargs = connection_kwargs or {}
        pool = redis.BlockingConnectionPool(connection_class=DummyConnection, max_connections=max_connections, timeout=timeout, **connection_kwargs)
        try:
            yield pool
        finally:
            await pool.disconnect(inuse_connections=True)
    ...
    async def test_connection_creation(self, master_host):
        connection_kwargs = {'foo': 'bar', 'biz': 'baz', 'host': master_host[0], 'port': master_host[1]}
        async with self.get_pool(connection_kwargs=connection_kwargs) as pool:
            connection = await pool.get_connection('_')
            assert isinstance(connection, DummyConnection)
            assert connection.kwargs == connection_kwargs
    ...
    async def test_connection_pool_blocks_until_timeout(self, master_host):
        ...
        connection_kwargs = {'host': master_host[0]}
        async with self.get_pool(max_connections=1, timeout=0.1, connection_kwargs=connection_kwargs) as pool:
            c1 = await pool.get_connection('_')
    ...
    async def test_connection_pool_blocks_until_conn_available(self, master_host):
        ...
        """
        connection_kwargs = {'host': master_host[0], 'port': master_host[1]}
        async with self.get_pool(max_connections=1, timeout=2, connection_kwargs=connection_kwargs) as pool:
            c1 = await pool.get_connection('_')
...
class TestConnection:
    ...
    async def test_connect_no_auth_supplied_when_required(self, r):
        ...
        with pytest.raises(redis.AuthenticationError):
            await r.execute_command('DEBUG', 'ERROR', 'ERR Client sent AUTH, but no password is set')
...
class TestHealthCheck:
    ...
    async def test_arbitrary_command_invokes_health_check(self, r):
        if r.connection:
            ...
            with mock.patch.object(r.connection, 'send_command', wraps=r.connection.send_command) as m:
                await r.get('foo')
    ...
    async def test_health_check_in_pipeline(self, r):
        async with r.pipeline(transaction=False) as pipe:
            ...
            with mock.patch.object(pipe.connection, 'send_command', wraps=pipe.connection.send_command) as m:
    ...
    async def test_health_check_in_pubsub_before_subscribe(self, r):
        ...
        with mock.patch.object(p.connection, 'send_command', wraps=p.connection.send_command) as m:
