...
FASTPARQUET_MARK = pytest.mark.skipif(SKIP_FASTPARQUET, reason='fastparquet not found or pyarrow strings are enabled')
SKIP_PYARROW = not pq
SKIP_PYARROW_REASON = 'pyarrow not found'
PYARROW_MARK = pytest.mark.skipif(SKIP_PYARROW, reason=SKIP_PYARROW_REASON)
nrows = 40
npartitions = 15
df = pd.DataFrame({'x': [i * 7 % 5 for i in range(nrows)], 'y': [i * 2.5 for i in range(nrows)]}, index=pd.Index([10 * i for i in range(nrows)], name='myindex'))
