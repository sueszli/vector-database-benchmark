...
class DirectLoggingChecker(BaseChecker):
    ...
    msgs = {'W9001': ("Use a logger object instead of a direct logging function like 'logging.%s()'", 'no-direct-logging', 'Do not use direct calls to logging functions like logging.info(), rather create a logger object with getLogger and use it instead. See https://github.com/deepset-ai/haystack/issues/4202.')}
...
class NoLoggingConfigurationChecker(BaseChecker):
    ...
    msgs = {'W9002': ("Do not use 'logging.basicConfig' in Haystack code: Haystack should not configure any loggers.", 'no-logging-basicconfig', 'Do not configure the logger explicitly, because this would be problematic for users. Always configure the loggers only in scripts that use Haystack, like tutorials, rather than Haystack itself.')}
