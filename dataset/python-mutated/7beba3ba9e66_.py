...
class MigrationSnapshotModel(Base):
    ...
    uuid = sa.Column(sa.String(36), primary_key=True)
    project_uuid = sa.Column(sa.String(36), sa.ForeignKey('projects.uuid', ondelete='CASCADE'))
    pipelines = sa.Column(JSONB)
    project_env_variables = sa.Column(JSONB)
    pipelines_env_variables = sa.Column(JSONB)

class MigrationJobsModel(Base):
    __tablename__ = 'jobs'
    uuid = sa.Column(sa.String(36), primary_key=True)
    project_uuid = sa.Column(sa.String(36))
    pipeline_name = sa.Column(sa.String(255))
    pipeline_uuid = sa.Column(sa.String(36))
    pipeline_definition = sa.Column(JSONB)
    env_variables = sa.Column(JSONB)
    pipeline_run_spec = sa.Column(JSONB)
    snapshot_uuid = sa.Column(sa.String(36))
