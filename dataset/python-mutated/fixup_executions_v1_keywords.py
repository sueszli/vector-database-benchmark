...
class executionsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'cancel_execution': ('name',), 'create_execution': ('parent', 'execution'), 'get_execution': ('name', 'view'), 'list_executions': ('parent', 'page_size', 'page_token', 'view', 'filter', 'order_by')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=executionsCallTransformer()):
    if False:
        i = 10
        return i + 15
