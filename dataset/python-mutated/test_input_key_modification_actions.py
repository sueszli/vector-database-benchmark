...
TEST_INPUTS: dict[str | None, str] = {'empty': '', 'multi-no-punctuation': 'Curse your sudden but inevitable betrayal', 'multi-punctuation': 'We have done the impossible, and that makes us mighty.', 'multi-and-hyphenated': 'Long as she does it quiet-like'}
...
async def test_delete_left_word_from_end() -> None:
    ...
    async with InputTester().run_test() as pilot:
        expected: dict[str | None, str] = {'empty': '', 'multi-no-punctuation': 'Curse your sudden but inevitable ', 'multi-punctuation': 'We have done the impossible, and that makes us ', 'multi-and-hyphenated': 'Long as she does it quiet-'}
        for input in pilot.app.query(Input):
            input.action_end()
            input.action_delete_left_word()
            assert input.cursor_position == len(input.value)
...
async def test_delete_right_word_from_home() -> None:
    ...
    async with InputTester().run_test() as pilot:
        expected: dict[str | None, str] = {'empty': '', 'multi-no-punctuation': 'your sudden but inevitable betrayal', 'multi-punctuation': 'have done the impossible, and that makes us mighty.', 'multi-and-hyphenated': 'as she does it quiet-like'}
        for input in pilot.app.query(Input):
