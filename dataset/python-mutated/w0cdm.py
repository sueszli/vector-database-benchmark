...
class wCDM(FLRW):
    ...
    """
    w0 = Parameter(default=-1.0, doc='Dark energy equation of state.', fvalidate='float')

    def __init__(self, H0, Om0, Ode0, w0=-1.0, Tcmb0=0.0 * u.K, Neff=3.04, m_nu=0.0 * u.eV, Ob0=None, *, name=None, meta=None):
        if False:
            i = 10
            return i + 15
        ...
        if self._Tcmb0.value == 0:
            ...
            self._inv_efunc_scalar_args = (self._Om0, self._Ode0, self._Ok0, self._Ogamma0 + self._Onu0, self._w0)
        else:
            self._inv_efunc_scalar = scalar_inv_efuncs.wcdm_inv_efunc
            self._inv_efunc_scalar_args = (self._Om0, self._Ode0, self._Ok0, self._Ogamma0, self._neff_per_nu, self._nmasslessnu, self._nu_y_list, self._w0)
    ...
    def efunc(self, z):
        ...
        zp1 = aszarr(z) + 1.0
        return sqrt(zp1 ** 2 * ((Or * zp1 + self._Om0) * zp1 + self._Ok0) + self._Ode0 * zp1 ** (3.0 * (1.0 + self._w0)))
    ...
    def inv_efunc(self, z):
        ...
        Or = self._Ogamma0 + (self._Onu0 if not self._massivenu else self._Ogamma0 * self.nu_relative_density(z))
        zp1 = aszarr(z) + 1.0
        return (zp1 ** 2 * ((Or * zp1 + self._Om0) * zp1 + self._Ok0) + self._Ode0 * zp1 ** (3.0 * (1.0 + self._w0))) ** (-0.5)
...
class FlatwCDM(FlatFLRWMixin, wCDM):
    ...
    def __init__(self, H0, Om0, w0=-1.0, Tcmb0=0.0 * u.K, Neff=3.04, m_nu=0.0 * u.eV, Ob0=None, *, name=None, meta=None):
        if False:
            for i in range(10):
                print('nop')
        super().__init__(H0=H0, Om0=Om0, Ode0=0.0, w0=w0, Tcmb0=Tcmb0, Neff=Neff, m_nu=m_nu, Ob0=Ob0, name=name, meta=meta)
        if self._Tcmb0.value == 0:
            ...
            self._inv_efunc_scalar_args = (self._Om0, self._Ode0, self._Ogamma0 + self._Onu0, self._w0)
        else:
            self._inv_efunc_scalar = scalar_inv_efuncs.fwcdm_inv_efunc
            self._inv_efunc_scalar_args = (self._Om0, self._Ode0, self._Ogamma0, self._neff_per_nu, self._nmasslessnu, self._nu_y_list, self._w0)
    ...
    def inv_efunc(self, z):
        ...
        Or = self._Ogamma0 + (self._Onu0 if not self._massivenu else self._Ogamma0 * self.nu_relative_density(z))
        zp1 = aszarr(z) + 1.0
        return (zp1 ** 3 * (Or * zp1 + self._Om0) + self._Ode0 * zp1 ** (3.0 * (1.0 + self._w0))) ** (-0.5)