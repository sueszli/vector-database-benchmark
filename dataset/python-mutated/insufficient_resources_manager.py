...
MSG_TRAIN_START = 'Training has not started in the last {wait_time:.0f} seconds. This could be due to the cluster not having enough resources available. '
MSG_TRAIN_INSUFFICIENT = 'You asked for {asked_cpus} CPUs and {asked_gpus} GPUs, but the cluster only has {cluster_cpus} CPUs and {cluster_gpus} GPUs available. '
MSG_TRAIN_END = 'Stop the training and adjust the required resources (e.g. via the `ScalingConfig` or `resources_per_trial`, or `num_workers` for rllib), or add more resources to your cluster.'
MSG_TUNE_START = 'No trial is running and no new trial has been started within the last {wait_time:.0f} seconds. This could be due to the cluster not having enough resources available. '
MSG_TUNE_INSUFFICIENT = 'You asked for {asked_cpus} CPUs and {asked_gpus} GPUs per trial, but the cluster only has {cluster_cpus} CPUs and {cluster_gpus} GPUs available. '
MSG_TUNE_END = 'Stop the tuning and adjust the required resources (e.g. via the `ScalingConfig` or `resources_per_trial`, or `num_workers` for rllib), or add more resources to your cluster.'
