...
@pluginmatcher(re.compile('https?://(?:www\\.)?ustvnow\\.com/live/(?P<scode>\\w+)/-(?P<id>\\d+)'))
@pluginargument('username', required=True, requires=['password'], metavar='USERNAME', help='Your USTV Now account username')
@pluginargument('password', required=True, sensitive=True, metavar='PASSWORD', help='Your USTV Now account password')
class USTVNow(Plugin):
    _main_js_re = re.compile('src=[\'"](main\\..*\\.js)[\'"]')
    _enc_key_re = re.compile('(?P<key>AES_(?:Key|IV))\\s*:\\s*"(?P<value>[^"]+)"')
    TENANT_CODE = 'ustvnow'
