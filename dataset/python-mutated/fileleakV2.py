...
def http_req(url, method='get', **kwargs):
    if False:
        i = 10
        return i + 15
    proxies = {'https': settings.proxy, 'http': settings.proxy}
...
def get_title(body):
    ...
    result = ''
    title_patten = re.compile(b'<title>([\\s\\S]{1,200})</title>', re.I)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='fileleak', formatter_class=ArgumentDefaultsHelpFormatter)
    parser.add_argument('--version', '-V', action='version', version='%(prog)s 2.1')
    parser.add_argument('--target', '-t', help='目标文件或者URL', required=True)
    parser.add_argument('--dict', '-d', default='dicts/mid.txt', help='自定义字典路径', required=False)
    parser.add_argument('--output', '-o', default='succ.txt', help='输出文件', required=False)
    parser.add_argument('--gen-dict', action='store_true', default=False)
    parser.add_argument('--concurrency-count', '-c', default=8, type=int, help='并发请求数量')
    parser.add_argument('--bool-ratio', default=0.8, type=float, help='页面相似度阈值')
    parser.add_argument('--proxy', '-x', help='代理地址')
