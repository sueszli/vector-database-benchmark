...
from homeassistant.components.climate import FAN_AUTO, ClimateEntity, ClimateEntityFeature, HVACMode
...
OVERKIZ_TO_FAN_MODES: dict[str, str] = {OverkizCommandParam.AUTO: FAN_AUTO, OverkizCommandParam.AWAY: FAN_AWAY, OverkizCommandParam.BOOST: FAN_BOOST, OverkizCommandParam.HIGH: FAN_KITCHEN, '': FAN_BYPASS}
...
class AtlanticHeatRecoveryVentilation(OverkizEntity, ClimateEntity):
    ...
    _attr_supported_features = ClimateEntityFeature.PRESET_MODE | ClimateEntityFeature.FAN_MODE
    ...
    def preset_mode(self) -> str | None:
        ...
        ventilation_configuration = self.executor.select_state(OverkizState.IO_VENTILATION_CONFIGURATION_MODE)
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        ...
        if preset_mode == PRESET_AUTO:
            await self.executor.async_execute_command(OverkizCommand.SET_VENTILATION_CONFIGURATION_MODE, OverkizCommandParam.COMFORT)
            await self._set_ventilation_mode(prog=OverkizCommandParam.OFF)
        if preset_mode == PRESET_PROG:
            await self.executor.async_execute_command(OverkizCommand.SET_VENTILATION_CONFIGURATION_MODE, OverkizCommandParam.STANDARD)
            await self._set_ventilation_mode(prog=OverkizCommandParam.ON)
        if preset_mode == PRESET_MANUAL:
            await self.executor.async_execute_command(OverkizCommand.SET_VENTILATION_CONFIGURATION_MODE, OverkizCommandParam.STANDARD)
            await self._set_ventilation_mode(prog=OverkizCommandParam.OFF)
        await self.executor.async_execute_command(OverkizCommand.REFRESH_VENTILATION_STATE)
        await self.executor.async_execute_command(OverkizCommand.REFRESH_VENTILATION_CONFIGURATION_MODE)
    ...
    def fan_mode(self) -> str | None:
        ...
        if cooling == OverkizCommandParam.ON:
            return FAN_BYPASS
        return OVERKIZ_TO_FAN_MODES[cast(str, self.executor.select_state(OverkizState.IO_AIR_DEMAND_MODE))]

    async def async_set_fan_mode(self, fan_mode: str) -> None:
        """Set new target fan mode."""
        if fan_mode == FAN_BYPASS:
            await self.executor.async_execute_command(OverkizCommand.SET_AIR_DEMAND_MODE, OverkizCommandParam.AUTO)
            ...
            await self.executor.async_execute_command(OverkizCommand.SET_AIR_DEMAND_MODE, FAN_MODES_TO_OVERKIZ[fan_mode])
        await self.executor.async_execute_command(OverkizCommand.REFRESH_VENTILATION_STATE)
    ...
    async def _set_ventilation_mode(self, cooling: str | None=None, prog: str | None=None) -> None:
        ...
        await self.executor.async_execute_command(OverkizCommand.SET_VENTILATION_MODE, ventilation_mode)