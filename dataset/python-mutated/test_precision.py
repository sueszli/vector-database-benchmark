...
from hypothesis.strategies import composite, datetimes, floats, integers, one_of, sampled_from, timedeltas, tuples
...
allclose_jd2 = functools.partial(np.allclose, rtol=np.finfo(float).eps, atol=np.finfo(float).eps)
allclose_sec = functools.partial(np.allclose, rtol=np.finfo(float).eps, atol=np.finfo(float).eps * 24 * 3600)
...
leap_second_deltas = list(zip(leap_second_days[1:], np.diff(leap_second_table['tai_utc'])))
today = Time.now()
...
_utc_bad = [pytest.param(s, marks=pytest.mark.xfail) if s == 'utc' else s for s in STANDARD_TIME_SCALES]
