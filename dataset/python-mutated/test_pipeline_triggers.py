...
from mage_ai.data_preparation.models.triggers import ScheduleInterval, ScheduleStatus, ScheduleType, Trigger, add_or_update_trigger_for_pipeline_and_persist, get_triggers_by_pipeline
from mage_ai.orchestration.db.models.schedules import PipelineSchedule
from mage_ai.tests.api.endpoints.mixins import BaseAPIEndpointTest, build_create_endpoint_tests, build_list_endpoint_tests
...
build_list_endpoint_tests(PipelineTriggerAPIEndpointTest, list_count=1, resource='pipeline_trigger', resource_parent='pipeline', get_resource_parent_id=lambda self: self.pipeline.uuid, result_keys_to_compare=['envs', 'name', 'pipeline_uuid', 'schedule_interval', 'schedule_type', 'settings', 'sla', 'start_time', 'status', 'variables'])
build_create_endpoint_tests(PipelineTriggerAPIEndpointTest, resource='pipeline_trigger', resource_parent='pipeline', get_resource_parent_id=lambda self: self.pipeline.uuid, build_payload=lambda self: dict(name=self.faker.unique.name(), pipeline_schedule_id=self.pipeline_schedule2.id), assert_before_create_count=lambda self: len(get_triggers_by_pipeline(self.pipeline.uuid)) == 1, assert_after_create_count=lambda self: len(get_triggers_by_pipeline(self.pipeline.uuid)) == 2)