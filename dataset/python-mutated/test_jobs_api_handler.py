...
def app_urls():
    if False:
        return 10
    return [('/jobs/(.*)', saltnado.JobsSaltAPIHandler), ('/jobs', saltnado.JobsSaltAPIHandler)]
...
async def test_get(http_client, subtests):
    ...
    response = await http_client.fetch('/jobs/{}'.format(jid), method='GET', follow_redirects=False)
    response_obj = salt.utils.json.loads(response.body)['return'][0]
    assert response_obj
    assert isinstance(response_obj, dict)
