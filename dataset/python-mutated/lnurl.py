...
async def request_lnurl(url: str) -> LNURL6Data:
    ...
    try:
        ...
        raise LNURLError(f"Missing or malformed 'minSendable'/'maxSendable' field in lnurl6 response. e={e!r}") from e
    try:
        comment_allowed = int(lnurl_dict['commentAllowed']) if 'commentAllowed' in lnurl_dict else 0
    except Exception as e:
        raise LNURLError(f"Malformed 'commentAllowed' field in lnurl6 response. e={e!r}") from e
    data = LNURL6Data(callback_url=callback_url, max_sendable_sat=max_sendable_sat, min_sendable_sat=min_sendable_sat, metadata_plaintext=metadata_plaintext, comment_allowed=comment_allowed)
    return data
