...
@MODELS.register_module(Tasks.video_multi_modal_embedding, module_name=Models.video_clip)
class VideoCLIPForMultiModalEmbedding(TorchModel):
    ...
    def forward(self, input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        (sequence_output, visual_output) = self.model.get_sequence_visual_output(input_ids, segment_ids, input_mask, video, video_mask)
        logger.info('text feature: {}'.format(sequence_output[0][0][0]))
        logger.info('video feature: {}'.format(visual_output[0][0][0]))
        output[OutputKeys.VIDEO_EMBEDDING] = visual_output.cpu().detach().numpy()
        output[OutputKeys.TEXT_EMBEDDING] = sequence_output.cpu().detach().numpy()
        return output
