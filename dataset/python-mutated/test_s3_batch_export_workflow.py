...
from posthog.temporal.workflows.s3_batch_export import HeartbeatDetails, S3BatchExportInputs, S3BatchExportWorkflow, S3InsertInputs, get_s3_key, insert_into_s3_activity
...
async def minio_client(bucket_name):
    ...
    """
    async with create_test_client('s3', aws_access_key_id='object_storage_root_user', aws_secret_access_key='object_storage_root_password') as minio_client:
        await minio_client.create_bucket(Bucket=bucket_name)
        yield minio_client
        await delete_all_from_s3(minio_client, bucket_name, key_prefix='/')
        await minio_client.delete_bucket(Bucket=bucket_name)

async def assert_events_in_s3(s3_compatible_client, bucket_name: str, key_prefix: str, events: list[EventValues], compression: str | None=None, exclude_events: list[str] | None=None):
...
async def test_insert_into_s3_activity_puts_data_into_s3(clickhouse_client, bucket_name, minio_client, activity_environment, compression, exclude_events):
    ...
    * Do not have an event name contained in the batch export's exclude_events.
    ...
    (events, _, _) = await generate_test_events_in_clickhouse(client=clickhouse_client, team_id=team_id, start_time=data_interval_start, end_time=data_interval_end, count=10000, count_outside_range=10, count_other_team=10, duplicate=True, properties={'$browser': 'Chrome', '$os': 'Mac OS X'}, person_properties={'utm_medium': 'referral', '$initial_os': 'Linux'})
    (events_with_no_properties, _, _) = await generate_test_events_in_clickhouse(client=clickhouse_client, team_id=team_id, start_time=data_interval_start, end_time=data_interval_end, count=5, count_outside_range=0, count_other_team=0, properties=None, person_properties=None)
    if exclude_events:
        for event_name in exclude_events:
            await generate_test_events_in_clickhouse(client=clickhouse_client, team_id=team_id, start_time=data_interval_start, end_time=data_interval_end, count=5, count_outside_range=0, count_other_team=0, event_name=event_name)
    prefix = str(uuid4())
    insert_inputs = S3InsertInputs(bucket_name=bucket_name, region='us-east-1', prefix=prefix, team_id=team_id, data_interval_start=data_interval_start.isoformat(), data_interval_end=data_interval_end.isoformat(), aws_access_key_id='object_storage_root_user', aws_secret_access_key='object_storage_root_password', compression=compression, exclude_events=exclude_events)
    with override_settings(BATCH_EXPORT_S3_UPLOAD_CHUNK_SIZE_BYTES=5 * 1024 ** 2):
        with mock.patch('posthog.temporal.workflows.s3_batch_export.aioboto3.Session.client', side_effect=create_test_client):
            await activity_environment.run(insert_into_s3_activity, insert_inputs)
    await assert_events_in_s3(minio_client, bucket_name, prefix, events=events + events_with_no_properties, compression=compression, exclude_events=exclude_events)

@pytest_asyncio.fixture
async def s3_batch_export(ateam, s3_key_prefix, bucket_name, compression, interval, exclude_events, temporal_client, encryption):
    destination_data = {'type': 'S3', 'config': {'bucket_name': bucket_name, 'region': 'us-east-1', 'prefix': s3_key_prefix, 'aws_access_key_id': 'object_storage_root_user', 'aws_secret_access_key': 'object_storage_root_password', 'compression': compression, 'exclude_events': exclude_events, 'encryption': encryption, 'kms_key_id': os.getenv('S3_TEST_KMS_KEY_ID') if encryption == 'aws:kms' else None}}
    batch_export_data = {'name': 'my-production-s3-bucket-destination', 'destination': destination_data, 'interval': interval}
    batch_export = await acreate_batch_export(team_id=ateam.pk, name=batch_export_data['name'], destination_data=batch_export_data['destination'], interval=batch_export_data['interval'])
    yield batch_export
...
@pytest.mark.parametrize('compression', [None, 'gzip', 'brotli'], indirect=True)
@pytest.mark.parametrize('exclude_events', [None, ['test-exclude']], indirect=True)
async def test_s3_export_workflow_with_minio_bucket(clickhouse_client, minio_client, ateam, s3_batch_export, bucket_name, interval, compression, exclude_events, s3_key_prefix):
    ...
    (events, _, _) = await generate_test_events_in_clickhouse(client=clickhouse_client, team_id=ateam.pk, start_time=data_interval_start, end_time=data_interval_end, count=100, count_outside_range=10, count_other_team=10, duplicate=True, properties={'$browser': 'Chrome', '$os': 'Mac OS X'}, person_properties={'utm_medium': 'referral', '$initial_os': 'Linux'})
    if exclude_events:
        for event_name in exclude_events:
            await generate_test_events_in_clickhouse(client=clickhouse_client, team_id=ateam.pk, start_time=data_interval_start, end_time=data_interval_end, count=5, count_outside_range=0, count_other_team=0, event_name=event_name)
    workflow_id = str(uuid4())
    inputs = S3BatchExportInputs(team_id=ateam.pk, batch_export_id=str(s3_batch_export.id), data_interval_end=data_interval_end.isoformat(), interval=interval, **s3_batch_export.destination.config)
    async with await WorkflowEnvironment.start_time_skipping() as activity_environment:
        async with Worker(activity_environment.client, task_queue=settings.TEMPORAL_TASK_QUEUE, workflows=[S3BatchExportWorkflow], activities=[create_export_run, insert_into_s3_activity, update_export_run_status], workflow_runner=UnsandboxedWorkflowRunner()):
            with mock.patch('posthog.temporal.workflows.s3_batch_export.aioboto3.Session.client', side_effect=create_test_client):
                await activity_environment.client.execute_workflow(S3BatchExportWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE, retry_policy=RetryPolicy(maximum_attempts=1), execution_timeout=dt.timedelta(minutes=10))
    runs = await afetch_batch_export_runs(batch_export_id=s3_batch_export.id)
    assert len(runs) == 1
    run = runs[0]
    assert run.status == 'Completed'
    await assert_events_in_s3(minio_client, bucket_name, s3_key_prefix, events=events, compression=compression, exclude_events=exclude_events)
...
@pytest.mark.skipif('S3_TEST_BUCKET' not in os.environ or not check_valid_credentials(), reason='AWS credentials not set in environment or missing S3_TEST_BUCKET variable')
...
async def test_s3_export_workflow_with_s3_bucket(s3_client, clickhouse_client, interval, s3_batch_export, bucket_name, compression, s3_key_prefix, encryption, exclude_events, ateam):
    ...
    async with await WorkflowEnvironment.start_time_skipping() as activity_environment:
        async with Worker(activity_environment.client, task_queue=settings.TEMPORAL_TASK_QUEUE, workflows=[S3BatchExportWorkflow], activities=[create_export_run, insert_into_s3_activity, update_export_run_status], workflow_runner=UnsandboxedWorkflowRunner()):
            with mock.patch('posthog.temporal.workflows.s3_batch_export.aioboto3.Session.client', side_effect=create_minio_client):
                await activity_environment.client.execute_workflow(S3BatchExportWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE, retry_policy=RetryPolicy(maximum_attempts=1), execution_timeout=dt.timedelta(seconds=10))
    runs = await afetch_batch_export_runs(batch_export_id=s3_batch_export.id)
    assert len(runs) == 1
    run = runs[0]
    assert run.status == 'Completed'
    await assert_events_in_s3(s3_client, bucket_name, s3_key_prefix, events=events, compression=compression, exclude_events=exclude_events)

async def test_s3_export_workflow_with_minio_bucket_and_a_lot_of_data(clickhouse_client, minio_client, bucket_name, ateam, compression, exclude_events, s3_key_prefix, interval, s3_batch_export):
    ...
    data_interval_end = dt.datetime.fromisoformat('2023-04-25T14:30:00.000000+00:00')
    data_interval_start = data_interval_end - s3_batch_export.interval_time_delta
    (events, _, _) = await generate_test_events_in_clickhouse(client=clickhouse_client, team_id=ateam.pk, start_time=data_interval_start, end_time=data_interval_end, count=1000000, count_outside_range=1000, count_other_team=1000, duplicate=True, properties={'$browser': 'Chrome', '$os': 'Mac OS X'}, person_properties={'utm_medium': 'referral', '$initial_os': 'Linux'})
    workflow_id = str(uuid4())
    inputs = S3BatchExportInputs(team_id=ateam.pk, batch_export_id=str(s3_batch_export.id), data_interval_end=data_interval_end.isoformat(), interval=interval, **s3_batch_export.destination.config)
    async with await WorkflowEnvironment.start_time_skipping() as activity_environment:
        async with Worker(activity_environment.client, task_queue=settings.TEMPORAL_TASK_QUEUE, workflows=[S3BatchExportWorkflow], activities=[create_export_run, insert_into_s3_activity, update_export_run_status], workflow_runner=UnsandboxedWorkflowRunner()):
            with mock.patch('posthog.temporal.workflows.s3_batch_export.aioboto3.Session.client', side_effect=create_test_client):
                await activity_environment.client.execute_workflow(S3BatchExportWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE, retry_policy=RetryPolicy(maximum_attempts=1), execution_timeout=dt.timedelta(seconds=360))
    runs = await afetch_batch_export_runs(batch_export_id=s3_batch_export.id)
    assert len(runs) == 1
    run = runs[0]
    assert run.status == 'Completed'
...
async def test_s3_export_workflow_defaults_to_timestamp_on_null_inserted_at(clickhouse_client, minio_client, bucket_name, compression, interval, s3_batch_export, s3_key_prefix, ateam):
    """Test the S3BatchExport Workflow end-to-end by using a local MinIO bucket instead of S3.
    ...
    data_interval_end = dt.datetime.fromisoformat('2023-04-25T14:30:00.000000+00:00')
    data_interval_start = data_interval_end - s3_batch_export.interval_time_delta
    (events, _, _) = await generate_test_events_in_clickhouse(client=clickhouse_client, team_id=ateam.pk, start_time=data_interval_start, end_time=data_interval_end, count=100, count_outside_range=10, count_other_team=10, duplicate=True, properties={'$browser': 'Chrome', '$os': 'Mac OS X'}, person_properties={'utm_medium': 'referral', '$initial_os': 'Linux'}, inserted_at=None)
    workflow_id = str(uuid4())
    inputs = S3BatchExportInputs(team_id=ateam.pk, batch_export_id=str(s3_batch_export.id), data_interval_end=data_interval_end.isoformat(), interval=interval, **s3_batch_export.destination.config)
    async with await WorkflowEnvironment.start_time_skipping() as activity_environment:
        async with Worker(activity_environment.client, task_queue=settings.TEMPORAL_TASK_QUEUE, workflows=[S3BatchExportWorkflow], activities=[create_export_run, insert_into_s3_activity, update_export_run_status], workflow_runner=UnsandboxedWorkflowRunner()):
    ...
    run = runs[0]
    assert run.status == 'Completed'
    await assert_events_in_s3(minio_client, bucket_name, s3_key_prefix, events, compression)

@pytest.mark.parametrize('s3_key_prefix', ['posthog-{table}/{year}-{month}-{day}/{hour}:{minute}:{second}', 'posthog-{table}/{hour}:{minute}:{second}/{year}-{month}-{day}', 'posthog-{table}/{hour}:{minute}:{second}', 'posthog/{year}-{month}-{day}/{hour}:{minute}:{second}', '{year}-{month}-{day}'], indirect=True)
async def test_s3_export_workflow_with_minio_bucket_and_custom_key_prefix(clickhouse_client, ateam, minio_client, bucket_name, compression, interval, s3_batch_export, s3_key_prefix):
    ...
    run = runs[0]
    assert run.status == 'Completed'
    expected_key_prefix = s3_key_prefix.format(table='events', year=data_interval_end.year, month=data_interval_end.strftime('%m'), day=data_interval_end.strftime('%d'), hour=data_interval_end.strftime('%H'), minute=data_interval_end.strftime('%M'), second=data_interval_end.strftime('%S'))
...
async def test_s3_export_workflow_handles_insert_activity_errors(ateam, s3_batch_export, interval):
    ...
    async with await WorkflowEnvironment.start_time_skipping() as activity_environment:
        async with Worker(activity_environment.client, task_queue=settings.TEMPORAL_TASK_QUEUE, workflows=[S3BatchExportWorkflow], activities=[create_export_run, insert_into_s3_activity_mocked, update_export_run_status], workflow_runner=UnsandboxedWorkflowRunner()):
            with pytest.raises(WorkflowFailureError):
                await activity_environment.client.execute_workflow(S3BatchExportWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE, retry_policy=RetryPolicy(maximum_attempts=1))
        runs = await afetch_batch_export_runs(batch_export_id=s3_batch_export.id)
        assert len(runs) == 1
        run = runs[0]
...
async def test_s3_export_workflow_handles_cancellation(ateam, s3_batch_export, interval):
    ...
    async with await WorkflowEnvironment.start_time_skipping() as activity_environment:
        async with Worker(activity_environment.client, task_queue=settings.TEMPORAL_TASK_QUEUE, workflows=[S3BatchExportWorkflow], activities=[create_export_run, never_finish_activity, update_export_run_status], workflow_runner=UnsandboxedWorkflowRunner()):
            handle = await activity_environment.client.start_workflow(S3BatchExportWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE, retry_policy=RetryPolicy(maximum_attempts=1))
...
async def test_insert_into_s3_activity_heartbeats(clickhouse_client, ateam, bucket_name, s3_batch_export, minio_client, activity_environment, s3_key_prefix):
    ...
    for i in range(1, n_expected_parts + 1):
        ...
        (events, _, _) = await generate_test_events_in_clickhouse(client=clickhouse_client, team_id=ateam.pk, start_time=data_interval_start, end_time=data_interval_end, count=1, count_outside_range=0, count_other_team=0, duplicate=False, properties={'$chonky': 'a' * 5 * 1024 ** 2}, inserted_at=part_inserted_at)
    ...
    insert_inputs = S3InsertInputs(bucket_name=bucket_name, region='us-east-1', prefix=s3_key_prefix, team_id=ateam.pk, data_interval_start=data_interval_start.isoformat(), data_interval_end=data_interval_end.isoformat(), aws_access_key_id='object_storage_root_user', aws_secret_access_key='object_storage_root_password')
    with override_settings(BATCH_EXPORT_S3_UPLOAD_CHUNK_SIZE_BYTES=5 * 1024 ** 2):
        with mock.patch('posthog.temporal.workflows.s3_batch_export.aioboto3.Session.client', side_effect=create_test_client):
            await activity_environment.run(insert_into_s3_activity, insert_inputs)
