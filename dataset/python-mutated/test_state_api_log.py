...
import ray
from ray._private.test_utils import format_web_url, wait_for_condition, wait_until_server_available
...
from ray.core.generated.gcs_pb2 import ActorTableData, TaskEvents, TaskStateUpdate, TaskLogInfo
from ray.dashboard.modules.actor.actor_head import actor_table_data_to_dict
from ray.dashboard.modules.log.log_agent import find_offset_of_content_in_file, find_end_offset_file, find_end_offset_next_n_lines_from_offset, find_start_offset_last_n_lines_from_offset, LogAgentV1Grpc
...
async def _stream_log(context, fp, start, end):
    ...
    result = bytearray()
    async for chunk_res in _stream_log_in_chunk(context=context, file=fp, start_offset=start, end_offset=end, keep_alive_interval_sec=-1):
...
@pytest.mark.parametrize('start_offset,end_offset', [(0, 1 << 20), (1 << 20, 1 << 20), (0, 0), (0, 1), (1 << 16, 1 << 20), (1024, 2042)])
async def test_stream_log_in_chunk(random_ascii_file, start_offset, end_offset):
    ...
    assert expected_file_content == actual_log_content, 'Non-matching content from log streamed'

@pytest.mark.asyncio
@pytest.mark.parametrize('lines_to_tail,total_lines', [(0, 100), (100, 100), (10, 100), (1, 100), (99, 100)])
...
async def test_log_tails(lines_to_tail, total_lines, trailing_new_line, temp_file):
    ...
    _write_lines_and_get_offset_at_index(temp_file, total_lines, trailing_new_line=trailing_new_line)
    test_file = temp_file
    context = MagicMock(grpc.aio.ServicerContext)
    context.done.return_value = False
    start_offset = find_start_offset_last_n_lines_from_offset(test_file, offset=-1, n=lines_to_tail)
    ...
    assert all_lines.count('\n') == (lines_to_tail if trailing_new_line or lines_to_tail == 0 else lines_to_tail - 1), 'Non-matching number of lines tailed'

@pytest.mark.asyncio
@pytest.mark.parametrize('lines_to_tail,total_lines', [(0, 5), (5, 5), (2, 5), (1, 5), (4, 5)])
async def test_log_tails_with_appends(lines_to_tail, total_lines, temp_file):
    ...
    num_new_lines = 2
    _write_lines_and_get_offset_at_index(temp_file, num_new_lines, start_offset=end_offset)
    start_offset = find_start_offset_last_n_lines_from_offset(test_file, offset=-1, n=lines_to_tail + num_new_lines)
    expected_data = _read_file(test_file, start_offset, -1)
    actual_data = await _stream_log(context, test_file, start_offset, -1)
    assert actual_data == expected_data, 'Non-matching data from stream log after append'
    all_lines = actual_data.decode('utf-8')
...
async def test_log_agent_find_task_log_offsets(temp_file):
    ...
    (start_offset, end_offset) = await LogAgentV1Grpc._find_task_log_offsets(task_id, attempt_number, -1, temp_file)
    assert start_offset == expected_start
    assert end_offset == expected_end
    num_tail = 3
    (start_offset, end_offset) = await LogAgentV1Grpc._find_task_log_offsets(task_id, attempt_number, num_tail, temp_file)
    assert end_offset == expected_end
...
async def test_logs_manager_list_logs(logs_manager):
    ...
    logs_client.list_logs.side_effect = [generate_list_logs(['gcs_server.out']), DataSourceUnavailable()]
    with pytest.raises(DataSourceUnavailable):
        result = await logs_manager.list_logs(node_id='3', timeout=30, glob_filter='*gcs*')
    ...
    with pytest.raises(DataSourceUnavailable):
        result = await logs_manager.list_logs(node_id='1', timeout=30, glob_filter='*gcs*')
...
async def test_logs_manager_resolve_file(logs_manager):
    ...
    res = await logs_manager.resolve_filename(node_id=node_id.hex(), log_filename=expected_filename, actor_id=None, task_id=None, pid=None, get_actor_fn=lambda _: True, timeout=10)
    ...
    with pytest.raises(ValueError):
        ...
        await logs_manager.resolve_filename(node_id=node_id.hex(), log_filename=None, actor_id=actor_id, task_id=None, pid=None, get_actor_fn=get_actor_fn, timeout=10)
    actor_id = ActorID(b'2' * 16)
    with pytest.raises(ValueError):
        await logs_manager.resolve_filename(node_id=node_id.hex(), log_filename=None, actor_id=actor_id, task_id=None, pid=None, get_actor_fn=lambda _: generate_actor_data(actor_id, node_id, None), timeout=10)
    actor_id = ActorID(b'2' * 16)
    worker_id = WorkerID(b'3' * 28)
    logs_manager.list_logs = AsyncMock()
    logs_manager.list_logs.return_value = {'worker_out': [f'worker-{worker_id.hex()}-123-123.out'], 'worker_err': []}
    res = await logs_manager.resolve_filename(node_id=node_id.hex(), log_filename=None, actor_id=actor_id, task_id=None, pid=None, get_actor_fn=lambda _: generate_actor_data(actor_id, node_id, worker_id), timeout=10)
    (log_file_name, n) = (res.filename, res.node_id)
    logs_manager.list_logs.assert_awaited_with(node_id.hex(), 10, glob_filter=f'*{worker_id.hex()}*out')
    ...
    logs_client.get_all_task_info.return_value = GetTaskEventsReply(events_by_task=[generate_task_event(task_id, node_id, attempt_number=1, worker_id=worker_id, stdout_file=f'worker-{worker_id.hex()}-123-123.out')])
    ...
    with pytest.raises(FileNotFoundError):
        pid = 456
        logs_manager.list_logs = AsyncMock()
        logs_manager.list_logs.return_value = {'worker_out': ['worker-123-123-123.out'], 'worker_err': []}
        await logs_manager.resolve_filename(node_id=node_id.hex(), log_filename=None, actor_id=None, task_id=None, pid=pid, get_actor_fn=lambda _: generate_actor_data(actor_id, node_id, worker_id), timeout=10)
    pid = 123
    logs_manager.list_logs = AsyncMock()
    logs_manager.list_logs.return_value = {'worker_out': [f'worker-123-123-{pid}.out'], 'worker_err': []}
    res = await logs_manager.resolve_filename(node_id=node_id.hex(), log_filename=None, actor_id=None, task_id=None, pid=pid, get_actor_fn=lambda _: generate_actor_data(actor_id, node_id, worker_id), timeout=10)
    (log_file_name, n) = (res.filename, res.node_id)
    logs_manager.list_logs.assert_awaited_with(node_id.hex(), 10, glob_filter=f'*{pid}*out')
    assert log_file_name == f'worker-123-123-{pid}.out'
    '\n    Test nothing is given.\n    '
    with pytest.raises(FileNotFoundError):
        await logs_manager.resolve_filename(node_id=node_id.hex(), log_filename=None, actor_id=None, task_id=None, pid=None, get_actor_fn=lambda _: generate_actor_data(actor_id, node_id, worker_id), timeout=10)
    ...
    logs_manager.list_logs.assert_awaited_with(node_id.hex(), 10, glob_filter=f'*{pid}*out')
    assert log_file_name == f'worker-123-123-{pid}.out'
    logs_manager.list_logs.return_value = {'worker_out': [], 'worker_err': [f'worker-123-123-{pid}.err']}
    res = await logs_manager.resolve_filename(node_id=node_id.hex(), log_filename=None, actor_id=None, task_id=None, pid=pid, get_actor_fn=lambda _: generate_actor_data(actor_id, node_id, worker_id), timeout=10, suffix='err')
    (log_file_name, n) = (res.filename, res.node_id)
    logs_manager.list_logs.assert_awaited_with(node_id.hex(), 10, glob_filter=f'*{pid}*err')
...
async def test_logs_manager_stream_log(logs_manager):
    ...
    options = GetLogOptions(timeout=30, media_type='file', lines=10, node_id='1', filename='raylet.out')
    i = 0
    ...
    logs_client.stream_log.assert_awaited_with(node_id='1', log_file_name='raylet.out', keep_alive=False, lines=10, interval=None, timeout=30, start_offset=None, end_offset=None)
    logs_client.ip_to_node_id.return_value = '1'
    logs_client.list_logs.side_effect = [generate_list_logs(['worker-0-0-10.out', 'worker-0-0-11.out', 'worker-0-0-10.err'])]
    options = GetLogOptions(timeout=30, media_type='stream', lines=10, interval=0.5, node_id='1', pid='10')
    logs_client.stream_log.return_value = generate_logs_stream(NUM_LOG_CHUNKS)
    i = 0
    async for chunk in logs_manager.stream_logs(options):
        assert chunk.decode('utf-8') == generate_logs_stream_chunk(index=i)
        i += 1
    assert i == NUM_LOG_CHUNKS
    logs_client.stream_log.assert_awaited_with(node_id='1', log_file_name='worker-0-0-10.out', keep_alive=True, lines=10, interval=0.5, timeout=None, start_offset=None, end_offset=None)
...
async def test_logs_manager_keepalive_no_timeout(logs_manager):
    ...
    logs_client.ip_to_node_id = MagicMock()
    logs_client.stream_log.return_value = generate_logs_stream(NUM_LOG_CHUNKS)
    options = GetLogOptions(timeout=30, media_type='stream', lines=10, node_id='1', filename='raylet.out')
    async for chunk in logs_manager.stream_logs(options):
        pass
    logs_client.stream_log.assert_awaited_with(node_id='1', log_file_name='raylet.out', keep_alive=True, lines=10, interval=None, timeout=None, start_offset=None, end_offset=None)
