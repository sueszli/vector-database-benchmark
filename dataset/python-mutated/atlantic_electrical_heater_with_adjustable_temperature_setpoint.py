...
from homeassistant.components.climate import PRESET_BOOST, PRESET_COMFORT, PRESET_ECO, PRESET_NONE, ClimateEntity, ClimateEntityFeature, HVACMode
...
class AtlanticElectricalHeaterWithAdjustableTemperatureSetpoint(OverkizEntity, ClimateEntity):
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        await self.executor.async_execute_command(OverkizCommand.SET_OPERATING_MODE, HVAC_MODE_TO_OVERKIZ[hvac_mode])
    ...
    def preset_mode(self) -> str | None:
        ...
        states = self.device.states
        if (operating_mode := states[OverkizState.CORE_OPERATING_MODE]) and operating_mode.value_as_str == OverkizCommandParam.EXTERNAL:
            return PRESET_EXTERNAL
        if (state := states[OverkizState.IO_TARGET_HEATING_LEVEL]) and state.value_as_str:
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        ...
        await self.executor.async_execute_command(command, PRESET_MODE_TO_OVERKIZ[preset_mode])
