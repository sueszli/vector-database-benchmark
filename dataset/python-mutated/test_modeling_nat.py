...
class NatModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (NatModel, NatForImageClassification, NatBackbone) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': NatModel, 'image-classification': NatForImageClassification} if is_torch_available() else {}
    fx_compatible = False
