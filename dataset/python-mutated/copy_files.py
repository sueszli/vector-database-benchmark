...
def gather_paths(dir_path):
    ...
    if os.path.isdir(dir_path):
        ...
        paths = [dir_path]
    return paths
dest_resp_mapping = {'wheels': 'presigned_resp_prod_wheels', 'branch_wheels': 'presigned_resp_prod_wheels', 'jars': 'presigned_resp_prod_wheels', 'branch_jars': 'presigned_resp_prod_wheels', 'logs': 'presigned_logs'}
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Helper script to upload files to S3 bucket')
    ...
    if os.environ.get('RAYCI_SKIP_UPLOAD', 'false') == 'true':
        ...
        sys.exit(0)
    assert args.destination in {'branch_jars', 'branch_wheels', 'jars', 'logs', 'wheels', 'docker_login'}
