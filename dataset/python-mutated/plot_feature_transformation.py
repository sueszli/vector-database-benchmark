...
(X_train_ensemble, X_train_linear, y_train_ensemble, y_train_linear) = train_test_split(X_full_train, y_full_train, test_size=0.5, random_state=10)
n_estimators = 10
max_depth = 3
...
rf_model = make_pipeline(rf_leaves_yielder, OneHotEncoder(handle_unknown='ignore'), LogisticRegression(max_iter=1000))
...
gbdt_model = make_pipeline(gbdt_leaves_yielder, OneHotEncoder(handle_unknown='ignore'), LogisticRegression(max_iter=1000))
