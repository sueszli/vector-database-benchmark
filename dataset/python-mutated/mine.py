...
if __name__ == '__main__':
    ...
    parser.add_argument('--neighborhood', type=int, default=4, help='Embedding dimension')
    parser.add_argument('--threshold', type=float, default=1.06, help='Threshold on mined bitext')
    parser.add_argument('--valid-size', type=int, default=2000, help='Number of sentences used for validation set')
    parser.add_argument('--min-count', type=int, default=50000, help='Min num sentences used for each language')
    ...
    (y2x_sim, y2x_ind) = knnGPU_sharded(y_batches_f, x_batches_f, args.dim, args.neighborhood, direction='y2x')
    (x2y_sim, x2y_ind) = knnGPU_sharded(x_batches_f, y_batches_f, args.dim, args.neighborhood, direction='x2y')
    ...
    indices = np.stack((np.concatenate((np.arange(x2y_ind.shape[0]), bwd_best)), np.concatenate((fwd_best, np.arange(y2x_ind.shape[0])))), axis=1)
    ...
    src_out = open(f'{directory}/all.{args.src_lang}', mode='w', encoding='utf-8', errors='surrogateescape')
    tgt_out = open(f'{directory}/all.{args.tgt_lang}', mode='w', encoding='utf-8', errors='surrogateescape')
    scores_out = open(f'{directory}/all.scores', mode='w', encoding='utf-8', errors='surrogateescape')
    count = 0
    ...
    with open(f'{directory}/all.{args.src_lang}') as all_s, open(f'{directory}/all.{args.tgt_lang}') as all_t, open(f'{directory}/valid.{args.src_lang}', 'w') as valid_s, open(f'{directory}/valid.{args.tgt_lang}', 'w') as valid_t, open(f'{directory}/train.{args.src_lang}', 'w') as train_s, open(f'{directory}/train.{args.tgt_lang}', 'w') as train_t:
        count = 0
