...
class ViTModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    """
    all_model_classes = (ViTModel, ViTForImageClassification, ViTForMaskedImageModeling) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': ViTModel, 'image-classification': ViTForImageClassification} if is_torch_available() else {}
    fx_compatible = True
    test_pruning = False
...
class ViTModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_interpolate_pos_encoding(self):
        ...
        expected_slice = torch.tensor([[4.234, 4.3906, -6.6692], [4.5463, 1.8928, -6.7257], [4.4429, 0.8496, -5.8585]]).to(torch_device)
