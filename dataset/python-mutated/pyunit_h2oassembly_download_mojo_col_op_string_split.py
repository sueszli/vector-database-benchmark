...
def h2oassembly_download_mojo_col_op_string_split():
    if False:
        for i in range(10):
            print('nop')
    values = [['fdsa fsf as', 'a.a.a'], ['fda.fsf as', 'aa.fa. a'], ['fdsa fsf.as', 'ba. .s .sb'], ['fdsa f.sf as', 'bs.a. .sb']]
    func = H2OFrame.strsplit
    frame = h2o.H2OFrame(python_obj=values, column_names=['a', 'b'], column_types=['string', 'string'])
    assembly = H2OAssembly(steps=[('col_op1', H2OColOp(op=func, col='a', new_col_name=['x1', 'x2'], inplace=False, pattern=' ')), ('col_op2', H2OColOp(op=func, col='b', new_col_name=['y1', 'y2', 'y3'], inplace=False, pattern='\\.'))])
