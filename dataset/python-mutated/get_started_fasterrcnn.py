...
if __name__ == '__main__':
    ...
    if cmdline.config and os.path.exists(cmdline.config):
        ...
        config = {'attack_losses': ['loss_classifier', 'loss_box_reg', 'loss_objectness', 'loss_rpn_box_reg'], 'cuda_visible_devices': '1', 'patch_shape': [450, 450, 3], 'patch_location': [600, 750], 'crop_range': [0, 0], 'brightness_range': [1.0, 1.0], 'rotation_weights': [1, 0, 0, 0], 'sample_size': 1, 'learning_rate': 1.0, 'max_iter': 5000, 'batch_size': 1, 'image_file': 'banner-diverse-group-of-people-2.jpg', 'resume': False, 'path': ''}
    ...
    frcnn = PyTorchFasterRCNN(clip_values=(0, 255), channels_first=False, attack_losses=config['attack_losses'], device_type=device_type)
    ...
    attack = RobustDPatch(frcnn, patch_shape=config['patch_shape'], patch_location=config['patch_location'], crop_range=config['crop_range'], brightness_range=config['brightness_range'], rotation_weights=config['rotation_weights'], sample_size=config['sample_size'], learning_rate=config['learning_rate'], max_iter=1, batch_size=config['batch_size'])
    x = image.copy()
    y = frcnn.predict(x=x)
