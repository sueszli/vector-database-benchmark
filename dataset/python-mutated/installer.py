...
class Installer:
    ...
    def __init__(self, io: IO, env: Env, package: ProjectPackage, locker: Locker, pool: RepositoryPool, config: Config, installed: Repository | None=None, executor: Executor | None=None, disable_cache: bool=False) -> None:
        if False:
            print('Hello World!')
        self._io = io
        self._env = env
        ...
        if executor is None:
            executor = Executor(self._env, self._pool, config, self._io, disable_cache=disable_cache)
    ...
    def requires_synchronization(self, requires_synchronization: bool=True) -> Installer:
        if False:
            while True:
                i = 10
    ...
    def _do_refresh(self) -> int:
        ...
        solver = Solver(self._package, self._pool, locked_repository.packages, locked_repository.packages, self._io)
        use_latest = [p.name for p in locked_repository.packages if p.source_type == 'directory']
        with solver.provider.use_source_root(source_root=self._env.path.joinpath('src')):
    ...
    def _do_install(self) -> int:
        ...
        if self._update:
            ...
            solver = Solver(self._package, self._pool, self._installed_repository.packages, locked_repository.packages, self._io)
            with solver.provider.use_source_root(source_root=self._env.path.joinpath('src')):
            ...
            if not self._locker.is_fresh():
                self._io.write_error_line('<warning>Warning: poetry.lock is not consistent with pyproject.toml. You may be getting improper dependencies. Run `poetry lock [--no-update]` to fix it.</warning>')
            locker_extras = {canonicalize_name(extra) for extra in self._locker.lock_data.get('extras', {})}
            for extra in self._extras:
        ...
        if self._io.is_verbose():
            self._io.write_line('')
            self._io.write_line('<info>Finding the necessary packages for the current system</>')
        packages = lockfile_repo.packages + locked_repository.packages
        pool = RepositoryPool.from_packages(packages, self._config)
        solver = Solver(root, pool, self._installed_repository.packages, locked_repository.packages, NullIO())
        solver.provider.load_deferred(False)
        with solver.use_environment(self._env):
            ops = solver.solve(use_latest=self._whitelist).calculate_operations(with_uninstalls=self._requires_synchronization, synchronize=self._requires_synchronization, skip_directory=self._skip_directory)
        if not self._requires_synchronization:
            from poetry.puzzle.transaction import Transaction
            transaction = Transaction(locked_repository.packages, [(package, 0) for package in lockfile_repo.packages], installed_packages=self._installed_repository.packages, root_package=root)
            ops = [op for op in transaction.calculate_operations(with_uninstalls=True) if op.job_type == 'uninstall'] + ops
    ...
    def _populate_lockfile_repo(self, repo: LockfileRepository, ops: Iterable[Operation]) -> None:
        if False:
            for i in range(10):
                print('nop')
        for op in ops:
    ...
    def _get_operations_from_lock(self, locked_repository: Repository) -> list[Operation]:
        if False:
            for i in range(10):
                print('nop')
    ...
    def _get_extra_packages(self, repo: Repository) -> set[NormalizedName]:
        ...
        if self._update:
            ...
            extras = {canonicalize_name(extra): [canonicalize_name(dependency) for dependency in dependencies] for (extra, dependencies) in raw_extras.items()}
