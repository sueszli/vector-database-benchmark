...
logging.basicConfig(level=logging.DEBUG if debug else logging.INFO if info else logging.WARNING)
logger = logging.getLogger(__name__)
report_files = [report_file for dir in (Path(__file__).parent / 'reports').iterdir() if re.match('^\\d{8}T\\d{6}_', dir.name) and (report_file := (dir / 'report.json')).is_file()]
labels = list[str]()
...
for report_file in sorted(report_files):
    with open(report_file) as f:
        ...
        if 'tests' in data:
            ...
            label = data['agent_git_commit_sha'].rsplit('/', 1)[1][:7]
        else:
            test_tree = data
