...
_datePattern = re.compile('^(?P<date>today|yesterday|(?P<day1>\\d+)\\s+(?P<month1>' + '|'.join(_months) + ')(\\s+(?P<year1>\\d{4}))?|(?P<month2>' + '|'.join(_months) + ')\\s+(?P<day2>\\d+),\\s+(?P<year2>\\d{4}))\\s+at\\s+(?P<hour>\\d+):(?P<minute>\\d+)\\s+(?P<ampm>[ap]m)$')
