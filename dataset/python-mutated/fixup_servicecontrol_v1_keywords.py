...
class servicecontrolCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'allocate_quota': ('service_name', 'allocate_operation', 'service_config_id'), 'check': ('service_name', 'operation', 'service_config_id'), 'report': ('service_name', 'operations', 'service_config_id')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=servicecontrolCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
