...
class sfp_voipbl(SpiderFootPlugin):
    meta = {'name': 'VoIP Blacklist (VoIPBL)', 'summary': 'Check if an IP address or netblock is malicious according to VoIP Blacklist (VoIPBL).', 'flags': [], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'website': 'https://voipbl.org/', 'model': 'FREE_NOAUTH_UNLIMITED', 'references': ['https://voipbl.org/#install', 'https://voipbl.org/#advanced'], 'favIcon': '', 'logo': 'https://voipbl.org/images/scopserv.png', 'description': 'VoIPBL is a distributed VoIP blacklist that is aimed to protect against\nVoIP Fraud and minimizing abuse for network that have publicly accessible PBXs.'}}
    ...
    def retrieveBlacklist(self):
        ...
        res = self.sf.fetchUrl('https://voipbl.org/update', timeout=self.opts['_fetchtimeout'], useragent=self.opts['_useragent'])
        if res['code'] != '200':
