...
def h2oassembly_download_mojo_documented_pipeline():
    ...
    assembly = H2OAssembly(steps=[('col_select', H2OColSelect(['sepal_len', 'petal_len', 'species'])), ('cos_Sepal.Length', H2OColOp(op=H2OFrame.cos, col='sepal_len', inplace=True)), ('str_cnt_Species', H2OColOp(op=H2OFrame.countmatches, col='species', inplace=False, pattern='s'))])
