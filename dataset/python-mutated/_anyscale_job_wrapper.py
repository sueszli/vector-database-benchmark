...
if __name__ == '__main__':
    ...
    parser.add_argument('test_workload', type=str, help='test workload, eg. python workloads/script.py')
    parser.add_argument('--test-workload-timeout', default=3600, type=float, help='test workload timeout (set to <0 for infinite)')
    parser.add_argument('--test-no-raise-on-timeout', action='store_true', help="don't fail on timeout")
    parser.add_argument('--results-cloud-storage-uri', type=str, help='bucket address to upload results.json to', required=False)
    parser.add_argument('--metrics-cloud-storage-uri', type=str, help='bucket address to upload metrics.json to', required=False)
    parser.add_argument('--output-cloud-storage-uri', type=str, help='bucket address to upload output.json to', required=False)
    parser.add_argument('--upload-cloud-storage-uri', type=str, help='root cloud-storage bucket address to upload stuff', required=False)
    parser.add_argument('--artifact-path', type=str, help='user provided artifact path (on head node), must be a single file path', required=False)
    parser.add_argument('--prepare-commands', type=str, nargs='*', help='prepare commands to run')
    parser.add_argument('--prepare-commands-timeouts', default=3600, type=float, nargs='*', help='timeout for prepare commands (set to <0 for infinite)')
