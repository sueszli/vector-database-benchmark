...
from aiogithubapi import GitHub, GitHubAPI, GitHubException, GitHubNotFoundException, GitHubNotModifiedException, GitHubReleaseModel
...
from custom_components.hacs.repositories.base import HACS_MANIFEST_KEYS_TO_EXPORT, REPOSITORY_KEYS_TO_EXPORT, HacsRepository
...
class AdjustedHacs(HacsBase):
    ...
    async def concurrent_register_repository(self, repository_full_name: str, category: str) -> None:
        """Register a repository."""
        await self.async_register_repository(repository_full_name=repository_full_name, category=category, default=True)
    ...
    async def concurrent_update_repository(self, repository: HacsRepository) -> None:
        ...
        if repository.data.last_version not in (None, ''):
            try:
                ...
                response = await self.githubapi.generic(endpoint=f'/repos/{repository.data.full_name}/releases/latest', etag=repository.data.etag_releases)
                ...
                if (releases := response.data) is not None:
                    ...
                    if (next_version := next(iter(repository.data.published_tags), None)) != repository.data.last_version:
        ...
        await repository.common_update(force=repository.data.etag_repository is None, skip_releases=repository.data.releases)

    async def generate_data_for_category(self, category: str, repository_name: str | None, current_data: dict[str, dict[str, Any]], force: bool) -> dict[str, dict[str, Any]]:
        """Generate data for category."""
        removed = [] if repository_name is not None else await self.data_client.get_repositories('removed')
        await self.data.register_base_data(category, {} if force else current_data, removed)
        self.queue.clear()
        ...
        async def _handle_queue():
            ...
            self.log.debug('Can update %s repositories, %s items in queue', can_update, self.queue.pending_tasks)
            if can_update == 0:
    ...
    async def get_category_repositories(self, category: str, repository_name: str | None, removed: list[str]) -> None:
        """Get repositories from category."""
        repositories = await self.async_github_get_hacs_default_file(category) if repository_name is None else []
        ...
        for repo in repositories:
            ...
            if repository is not None:
                ...
                continue
            self.queue.add(self.concurrent_register_repository(repository_full_name=repo, category=category))

    async def summarize_data(self, current_data: dict[str, dict[str, Any]], updated_data: dict[str, dict[str, Any]]) -> int:
        """Summarize data."""
        changed = 0
        for (repo_id, repo_data) in updated_data.items():
            if repo_data.get('etag_releases') != current_data.get(repo_id, {}).get('etag_releases') or repo_data.get('etag_repository') != current_data.get(repo_id, {}).get('etag_repository'):
                changed += 1
        print(json.dumps({'rate_limit': (await self.githubapi.rate_limit()).data.resources.core.as_dict, 'current_count': len(current_data.keys()), 'new_count': len(updated_data.keys()), 'changed': changed} if len(updated_data) > 1 else updated_data, indent=2))
        return changed
...
async def generate_category_data(category: str, repository_name: str=None):
    ...
    async with ClientSession() as session:
        ...
        current_data = next(({key: value} for (key, value) in stored_data.items() if value['full_name'] == repository_name), {}) if repository_name is not None else stored_data
        updated_data = await hacs.generate_data_for_category(category, repository_name, current_data, force=force)
        changed = await hacs.summarize_data(current_data, updated_data)
        if not force and changed == 0 and (repository_name is None) and (len(current_data) == len(updated_data)):
            print('No changes, exiting')
            return
        with open(os.path.join(OUTPUT_DIR, category, 'data.json'), mode='w', encoding='utf-8') as data_file:
            json.dump(updated_data, data_file, cls=JSONEncoder, separators=(',', ':'))
        with open(os.path.join(OUTPUT_DIR, category, 'repositories.json'), mode='w', encoding='utf-8') as repositories_file:
            json.dump([v['full_name'] for v in updated_data.values()], repositories_file, separators=(',', ':'), sort_keys=True)
        with open(os.path.join(OUTPUT_DIR, 'diff', f'{category}_before.json'), mode='w', encoding='utf-8') as data_file:
            json.dump(current_data, data_file, cls=JSONEncoder, sort_keys=True, indent=2)
        with open(os.path.join(OUTPUT_DIR, 'diff', f'{category}_after.json'), mode='w', encoding='utf-8') as data_file:
            json.dump(updated_data, data_file, cls=JSONEncoder, sort_keys=True, indent=2)
if __name__ == '__main__':
    asyncio.run(generate_category_data(sys.argv[1], sys.argv[2] if len(sys.argv) > 2 else None))