...
from airflow_breeze.utils.packages import ProviderPackageDetails, get_provider_details, get_provider_jinja_context, get_provider_packages_metadata, get_provider_requirements, get_removed_provider_ids, get_source_package_path
from airflow_breeze.utils.path_utils import BREEZE_SOURCES_ROOT
...
BACKTICKS_CHECK = re.compile('^(?! {4}).*(^| )`[^`]+`([^_]|$)', re.MULTILINE)
...
TYPE_OF_CHANGE_DESCRIPTION = {TypeOfChange.DOCUMENTATION: 'Documentation only changes - no version change needed, only documentation needs to be updated', TypeOfChange.BUGFIX: 'Bugfix/Misc changes only - bump in PATCHLEVEL version needed', TypeOfChange.FEATURE: 'Feature changes - bump in MINOR version needed', TypeOfChange.BREAKING_CHANGE: 'Breaking changes - bump in MAJOR version needed'}
