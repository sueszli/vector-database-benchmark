...
def sample(cfg, logger):
    ...
    def make(sample):
        ...
        with torch.no_grad():
            for img in sample:
                x = torch.tensor(np.asarray(img, dtype=np.float32), device='cpu', requires_grad=True).cuda() / 127.5 - 1.0
                if x.shape[0] == 4:
                    x = x[:3]
...
if __name__ == '__main__':
    gpu_count = 1
    run(sample, get_cfg_defaults(), description='ALAE-reconstruction-ffhq', default_config='configs/ffhq.yaml', world_size=gpu_count, write_log=False)