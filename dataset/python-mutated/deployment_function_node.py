...
class DeploymentFunctionNode(DAGNode):
    ...
    def __init__(self, func_body: Union[Callable, str], deployment_name, app_name, func_args, func_kwargs, func_options, other_args_to_resolve=None):
        if False:
            while True:
                i = 10
        ...
        if 'deployment_schema' in self._bound_other_args_to_resolve:
            deployment_schema: DeploymentSchema = self._bound_other_args_to_resolve['deployment_schema']
            ...
            self._deployment = deployment_shell.options(func_or_class=func_body, name=self._deployment_name, route_prefix=route_prefix, _init_args=(), _init_kwargs={}, _internal=True)
        else:
            replica_config = ReplicaConfig.create(deployment_def=func_body, init_args=tuple(), init_kwargs=dict(), ray_actor_options=func_options)
            self._deployment: Deployment = Deployment(deployment_name, deployment_config=DeploymentConfig(), replica_config=replica_config, _internal=True)
    ...
    def _copy_impl(self, new_args: List[Any], new_kwargs: Dict[str, Any], new_options: Dict[str, Any], new_other_args_to_resolve: Dict[str, Any]):
        if False:
            print('Hello World!')
        return DeploymentFunctionNode(self._body, self._deployment_name, self._app_name, new_args, new_kwargs, new_options, other_args_to_resolve=new_other_args_to_resolve)
