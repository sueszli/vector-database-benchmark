...
class MSTeams(BaseCollaboration):
    ...
    def say_hello(cls, url):
        if False:
            print('Hello World!')
        r = requests.post(url=url, json={'@type': 'MessageCard', '@context': 'https://schema.org/extensions', 'summary': 'Hello message', 'title': 'Welcome to OpenReplay'})
    ...
    def send_batch(cls, tenant_id, webhook_id, attachments):
        ...
        for i in range(0, len(attachments), 100):
            print(json.dumps({'type': 'message', 'attachments': [{'contentType': 'application/vnd.microsoft.card.adaptive', 'contentUrl': None, 'content': {'$schema': 'http://adaptivecards.io/schemas/adaptive-card.json', 'type': 'AdaptiveCard', 'version': '1.2', 'body': attachments[i:i + 100]}}]}))
            r = requests.post(url=integration['endpoint'], json={'type': 'message', 'attachments': [{'contentType': 'application/vnd.microsoft.card.adaptive', 'contentUrl': None, 'content': {'$schema': 'http://adaptivecards.io/schemas/adaptive-card.json', 'type': 'AdaptiveCard', 'version': '1.2', 'body': attachments[i:i + 100]}}]})
            if r.status_code != 200:
    ...
    def __share(cls, tenant_id, integration_id, attachement):
        ...
        r = requests.post(url=integration['endpoint'], json={'type': 'message', 'attachments': [{'contentType': 'application/vnd.microsoft.card.adaptive', 'contentUrl': None, 'content': {'$schema': 'http://adaptivecards.io/schemas/adaptive-card.json', 'type': 'AdaptiveCard', 'version': '1.5', 'body': [attachement]}}]})
        return r.text
    ...
    def share_session(cls, tenant_id, project_id, session_id, user, comment, integration_id=None):
        ...
        link = f'[{link}]({link})'
        args = {'type': 'ColumnSet', 'style': 'emphasis', 'separator': True, 'bleed': True, 'columns': [{'width': 'stretch', 'items': [{'type': 'TextBlock', 'text': title, 'style': 'heading', 'size': 'Large'}, {'type': 'TextBlock', 'spacing': 'small', 'text': link}]}]}
        if comment and len(comment) > 0:
            args['columns'][0]['items'].append({'type': 'TextBlock', 'spacing': 'small', 'text': comment})
