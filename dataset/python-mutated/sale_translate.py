...
class SaleTranslate(BaseTranslateMutation):
    ...
    class Arguments:
        id = graphene.ID(required=True, description='Sale ID or SaleTranslatableContent ID.')
        language_code = graphene.Argument(LanguageCodeEnum, required=True, description='Translation language code.')
        input = NameTranslationInput(required=True, description='Fields required to update sale translations.')

    class Meta:
        description = 'Creates/updates translations for a sale.' + DEPRECATED_IN_3X_MUTATION + ' Use `PromotionTranslate` mutation instead.'
