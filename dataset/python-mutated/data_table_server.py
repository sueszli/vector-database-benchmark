...
plot_figure = figure(title='Data-Table', height=450, width=600, tools='save,reset', toolbar_location='below')
plot_figure.scatter('x', 'y', source=source, size=10)
columns = [TableColumn(field='x', title='x'), TableColumn(field='y', title='y')]
data_table = DataTable(source=source, columns=columns, index_position=None, autosize_mode='fit_columns', editable=True)
toggle = Toggle()
