...
class AgentSmith(ClientSession):
    ...
    async def do_nefarious_things(self):
        await self.send_request('blockchain.claimtrie.search', {'no_totals': True, 'offset': random.choice(range(0, 300, 20)), 'limit': 20, 'any_tags': random.choice([[random.choice(['gaming', 'games', 'game']) + random.choice(['entertainment', 'playthrough', 'funny']) + random.choice(['xbox', 'xbox one', 'xbox news'])], [random.choice(['aliens', 'alien', 'ufo', 'ufos']) + random.choice(['news', 'sighting', 'sightings'])], [random.choice(['art', 'automotive']), random.choice(['blockchain', 'economics', 'food']), random.choice(['funny', 'learnings', 'nature']), random.choice(['news', 'science', 'technology'])]]), 'not_tags': random.choice([[], ['porn', 'mature', 'xxx', 'nsfw']]), 'order_by': random.choice([['release_time'], ['trending_global', 'trending_mixed'], ['effective_amount']])})
...
class AgentSmithProgram:
    ...
    async def coordinate_nefarious_activity(self):
        ...
        await asyncio.gather(*(s.do_nefarious_things() for s in self.agent_smiths), return_exceptions=True)
    ...
    async def delete_program(self):
        await asyncio.gather(*(s.close() for s in self.agent_smiths))
...
async def main(host, port):
    ...
    for i in range(1000):
        ...
        if activity.done() and activity.result() < 0.9:
            print('more, more, more...')
            await asyncio.gather(*(asyncio.create_task(smiths.make_one_more_of_them()) for _ in range(20)))
        else:
            print('!!!!!!!!!!!!!!')
            print('IS NEO LOSING?')
            print('!!!!!!!!!!!!!!')
            await asyncio.gather(*(asyncio.create_task(smiths.delete_one_smith()) for _ in range(21)))
