...
class CzshareCom(SimpleDownloader):
    ...
    __version__ = '1.11'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?(czshare|sdilej)\\.(com|cz)/(\\d+/|download\\.php\\?).+'
    ...
    NAME_PATTERN = '<div class="tab" id="parameters">\\s*<p>\\s*Cel. n.zev: <a href=.*?>(?P<N>.+?)</a>'
    SIZE_PATTERN = '<div class="tab" id="category">(?:\\s*<p>[^\\n]*</p>)*\\s*Velikost:\\s*(?P<S>[\\d .,]+)(?P<U>[\\w^_]+)\\s*</div>'
    OFFLINE_PATTERN = '<div class="header clearfix">\\s*<h2 class="red">'
    SIZE_REPLACEMENTS = [(' ', '')]
    URL_REPLACEMENTS = [('http://[^/]*/download.php\\?.*?id=(\\w+).*', 'http://sdilej.cz/\\1/x/')]
    CHECK_TRAFFIC = True
    FREE_URL_PATTERN = '<a href="(.+?)" class="page-download">[^>]*alt="(.+?)" /></a>'
    FREE_FORM_PATTERN = '<form action="download\\.php" method="post">\\s*<img src="captcha\\.php" id="captcha" />(.*?)</form>'
    PREMIUM_FORM_PATTERN = '<form action="/profi_down\\.php" method="post">(.*?)</form>'
    FORM_INPUT_PATTERN = '<input[^>]* name="(.+?)" value="(.+?)"[^>]*/>'
    MULTIDL_PATTERN = "<p><font color=\\'red\\'>Z.*?PROFI.</font></p>"
    USER_CREDIT_PATTERN = '<div class="credit">\\s*kredit: <strong>([\\d .,]+)(\\w+)</strong>\\s*</div><!-- .credit -->'
