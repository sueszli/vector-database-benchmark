...
class Test_ec2_ebs_public_snapshot:
    ...
    def test_ec2_default_snapshots(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.ec2.ec2_ebs_public_snapshot.ec2_ebs_public_snapshot.ec2_client', new=EC2(current_audit_info)):
            from prowler.providers.aws.services.ec2.ec2_ebs_public_snapshot.ec2_ebs_public_snapshot import ec2_ebs_public_snapshot
    ...
    def test_ec2_public_snapshot(self):
        ...
        ec2_client.modify_snapshot_attribute(SnapshotId=snapshot.id, Attribute='createVolumePermission', OperationType='add', GroupNames=['all'])
        from prowler.providers.aws.services.ec2.ec2_service import EC2
        current_audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.ec2.ec2_ebs_public_snapshot.ec2_ebs_public_snapshot.ec2_client', new=EC2(current_audit_info)):
            from prowler.providers.aws.services.ec2.ec2_ebs_public_snapshot.ec2_ebs_public_snapshot import ec2_ebs_public_snapshot
            ...
            for snap in results:
                if snap.resource_id == snapshot.id:
                    ...
                    assert snap.status_extended == f'EBS Snapshot {snapshot.id} is currently Public.'
                    assert snap.resource_arn == f'arn:{current_audit_info.audited_partition}:ec2:{AWS_REGION}:{current_audit_info.audited_account}:snapshot/{snapshot.id}'
    ...
    def test_ec2_private_snapshot(self):
        if False:
            while True:
                i = 10
        ec2 = resource('ec2', region_name=AWS_REGION)
        snapshot = volume = ec2.create_volume(Size=80, AvailabilityZone=f'{AWS_REGION}a', Encrypted=True)
        snapshot = volume.create_snapshot(Description='testsnap')
        from prowler.providers.aws.services.ec2.ec2_service import EC2
        current_audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.ec2.ec2_ebs_public_snapshot.ec2_ebs_public_snapshot.ec2_client', new=EC2(current_audit_info)):
