...
class UnsupGenerateConfig(FairseqDataclass):
    ...
    lm_weight: float = field(default=2.0, metadata={'help': 'language model weight'})
    w2l_decoder: DecoderType = field(default=DecoderType.VITERBI, metadata={'help': 'type of decoder to use'})
    kaldi_decoder_config: Optional[KaldiDecoderConfig] = None
    lexicon: Optional[str] = field(default=None, metadata={'help': "path to lexicon. This is also used to 'phonemize' for unsupvised param tuning"})
    lm_model: Optional[str] = field(default=None, metadata={'help': 'path to language model (kenlm or fairseq)'})
    decode_stride: Optional[float] = field(default=None, metadata={'help': 'changing the decoding frequency of the generator'})
    unit_lm: bool = field(default=False, metadata={'help': 'whether to use unit lm'})
    beam_threshold: float = field(default=50.0, metadata={'help': 'beam score threshold'})
    beam_size_token: float = field(default=100.0, metadata={'help': 'max tokens per beam'})
    beam: int = field(default=5, metadata={'help': 'decoder beam size'})
    nbest: int = field(default=1, metadata={'help': 'number of results to return'})
    word_score: float = field(default=1.0, metadata={'help': 'word score to add at end of word'})
    unk_weight: float = field(default=-math.inf, metadata={'help': 'unknown token weight'})
    sil_weight: float = field(default=0.0, metadata={'help': 'silence token weight'})
    targets: Optional[str] = field(default=None, metadata={'help': 'extension of ground truth labels to compute UER'})
    results_path: Optional[str] = field(default=None, metadata={'help': 'where to store results'})
    post_process: Optional[str] = field(default=None, metadata={'help': 'how to post process results'})
    vocab_usage_power: float = field(default=2, metadata={'help': 'for unsupervised param tuning'})
    viterbi_transcript: Optional[str] = field(default=None, metadata={'help': 'for unsupervised param tuning'})
    min_lm_ppl: float = field(default=0, metadata={'help': 'for unsupervised param tuning'})
    min_vt_uer: float = field(default=0, metadata={'help': 'for unsupervised param tuning'})
    blank_weight: float = field(default=0, metadata={'help': 'value to add or set for blank emission'})
    blank_mode: str = field(default='set', metadata={'help': 'can be add or set, how to modify blank emission with blank weight'})
    sil_is_blank: bool = field(default=False, metadata={'help': 'if true, <SIL> token is same as blank token'})
    unsupervised_tuning: bool = field(default=False, metadata={'help': 'if true, returns a score based on unsupervised param selection metric instead of UER'})
    is_ax: bool = field(default=False, metadata={'help': 'if true, assumes we are using ax for tuning and returns a tuple for ax to consume'})
...
def optimize_models(cfg: UnsupGenerateConfig, use_cuda, models):
    ...
    for model in models:
        ...
        if use_cuda:
            model.cuda()
GenResult = namedtuple('GenResult', ['count', 'errs_t', 'gen_timer', 'lengths_hyp_unit_t', 'lengths_hyp_t', 'lengths_t', 'lm_score_t', 'num_feats', 'num_sentences', 'num_symbols', 'vt_err_t', 'vt_length_t'])
