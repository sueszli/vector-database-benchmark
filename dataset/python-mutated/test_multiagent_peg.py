...
if __name__ == '__main__':
    ...
    if ARGS.exp.split('-')[1] == 'peg':
        register_env(temp_env_name, lambda _: PEGDrones(num_pursuer_drones=NUM_PURSUER_DRONES, num_evader_drones=NUM_EVADER_DRONES, aggregate_phy_steps=shared_constants.AGGR_PHY_STEPS, obs=OBS, act=ACT))
    else:
        print('[ERROR] environment not yet implemented')
        exit()
    if ARGS.exp.split('-')[1] == 'peg':
        temp_env = PEGDrones(num_pursuer_drones=NUM_PURSUER_DRONES, num_evader_drones=NUM_EVADER_DRONES, aggregate_phy_steps=shared_constants.AGGR_PHY_STEPS, obs=OBS, act=ACT)
    else:
        print('[ERROR] environment not yet implemented')
        exit()
    ...
    if ARGS.exp.split('-')[1] == 'peg':
        test_env = PEGDrones(num_pursuer_drones=NUM_PURSUER_DRONES, num_evader_drones=NUM_EVADER_DRONES, aggregate_phy_steps=shared_constants.AGGR_PHY_STEPS, obs=OBS, act=ACT, gui=True, record=True)
    else:
        print('[ERROR] environment not yet implemented')
        exit()
    obs = test_env.reset()
