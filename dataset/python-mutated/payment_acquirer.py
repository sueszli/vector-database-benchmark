...
class PaymentAcquirer(models.Model):
    ...
    provider = fields.Selection(selection=[('manual', 'Manual Configuration')], string='Provider', default='manual', required=True)
    ...
    registration_view_template_id = fields.Many2one('ir.ui.view', 'S2S Form Template', domain=[('type', '=', 'qweb')], help='Template for method registration')
    environment = fields.Selection([('test', 'Test'), ('prod', 'Production')], string='Environment', default='test', oldname='env', required=True)
    website_published = fields.Boolean('Visible in Portal / Website', copy=False, help='Make this payment acquirer available (Customer invoices, etc.)')
    auto_confirm = fields.Selection([('none', 'No automatic confirmation'), ('authorize', 'Authorize the amount and confirm the SO on acquirer confirmation (capture manually)'), ('confirm_so', 'Authorize & capture the amount and confirm the SO on acquirer confirmation'), ('generate_and_pay_invoice', 'Authorize & capture the amount, confirm the SO and auto-validate the invoice on acquirer confirmation')], string='Order Confirmation', default='confirm_so', required=True)
    journal_id = fields.Many2one('account.journal', 'Payment Journal', help='Account journal used for automatic payment reconciliation.')
    pre_msg = fields.Html('Help Message', translate=True, help='Message displayed to explain and help the payment process.')
    post_msg = fields.Html('Thanks Message', translate=True, help='Message displayed after having done the payment process.')
    pending_msg = fields.Html('Pending Message', translate=True, default='<i>Pending,</i> Your online payment has been successfully processed. But your order is not validated yet.', help='Message displayed, if order is in pending state after having done the payment process.')
    done_msg = fields.Html('Done Message', translate=True, default='<i>Done,</i> Your online payment has been successfully processed. Thank you for your order.', help='Message displayed, if order is done successfully after having done the payment process.')
    cancel_msg = fields.Html('Cancel Message', translate=True, default='<i>Cancel,</i> Your payment has been cancelled.', help='Message displayed, if order is cancel during the payment process.')
    error_msg = fields.Html('Error Message', translate=True, default='<i>Error,</i> Please be aware that an error occurred during the transaction. The order has been confirmed but will not be paid. Do not hesitate to contact us if you have any questions on the status of your order.', help='Message displayed, if error is occur during the payment process.')
    save_token = fields.Selection([('none', 'Never'), ('ask', 'Let the customer decide'), ('always', 'Always')], string='Store Card Data', default='none', help="Determine if card data is saved as a token automatically or not. Payment tokens allow your customer to reuse their cards in the e-commerce or allow you to charge an invoice directly on a credit card. If set to 'let the customer decide', ecommerce customers will have a checkbox displayed on the payment page.")
    ...
    image = fields.Binary('Image', attachment=True, help='This field holds the image used for this provider, limited to 1024x1024px')
    image_medium = fields.Binary('Medium-sized image', attachment=True, help='Medium-sized image of this provider. It is automatically resized as a 128x128px image, with aspect ratio preserved. Use this field in form views or some kanban views.')
    image_small = fields.Binary('Small-sized image', attachment=True, help='Small-sized image of this provider. It is automatically resized as a 64x64px image, with aspect ratio preserved. Use this field anywhere a small image is required.')
    ...
    def _check_authorization_support(self):
        ...
        return True
    _constraints = [(_check_required_if_provider, 'Required fields not filled', [])]
...
class PaymentTransaction(models.Model):
    ...
    type = fields.Selection([('server2server', 'Server To Server'), ('form', 'Form'), ('form_save', 'Form with tokenization')], 'Type', default='form', required=True)
    state = fields.Selection([('draft', 'Draft'), ('pending', 'Pending'), ('authorized', 'Authorized'), ('done', 'Done'), ('error', 'Error'), ('cancel', 'Canceled')], 'Status', copy=False, default='draft', required=True, track_visibility='onchange')
    ...
    fees = fields.Float('Fees', digits=(16, 2), track_visibility='always', help='Fees amount; set by the system because depends on the acquirer')
    currency_id = fields.Many2one('res.currency', 'Currency', required=True)
    reference = fields.Char('Reference', default=lambda self: self.env['ir.sequence'].next_by_code('payment.transaction'), required=True, help='Internal reference of the TX')
    ...
    callback_eval = fields.Char('S2S Callback', help="        Will be safe_eval with `self` being the current transaction. i.e.:\n            self.env['my.model'].payment_validated(self)", oldname='s2s_cb_eval', groups='base.group_system')
