...
tensor_conversion_short_configs = op_bench.cross_product_configs(M=(8, 16, 32), N=(16, 64, 128), device=['cpu', 'cuda'], tags=['short'])
tensor_conversion_long_configs = op_bench.cross_product_configs(M=(64, 128, 256, 512), N=(256, 512, 1024, 2048), device=['cpu', 'cuda'], tags=['long'])
...
class FloatToHalfTensorConversionBenchmark(op_bench.TorchBenchmarkBase):
    ...
    def init(self, M, N, device):
        if False:
            i = 10
            return i + 15
        self.inputs = {'input': torch.rand(M, N, device=device, requires_grad=False, dtype=torch.float)}
