...
from ..utils import format_field, int_or_none, OnDemandPagedList, smuggle_url

class StoryFireBaseIE(InfoExtractor):
    _VALID_URL_BASE = 'https?://(?:www\\.)?storyfire\\.com/'
...
class StoryFireUserIE(StoryFireBaseIE):
    _VALID_URL = StoryFireBaseIE._VALID_URL_BASE + 'user/(?P<id>[^/]+)/video'
    _TEST = {'url': 'https://storyfire.com/user/UQ986nFxmAWIgnkZQ0ftVhq4nOk2/video', 'info_dict': {'id': 'UQ986nFxmAWIgnkZQ0ftVhq4nOk2'}, 'playlist_mincount': 151}
    _PAGE_SIZE = 20
...
class StoryFireSeriesIE(StoryFireBaseIE):
    _VALID_URL = StoryFireBaseIE._VALID_URL_BASE + 'write/series/stories/(?P<id>[^/?&#]+)'
    _TESTS = [{'url': 'https://storyfire.com/write/series/stories/-Lq6MsuIHLODO6d2dDkr/', 'info_dict': {'id': '-Lq6MsuIHLODO6d2dDkr'}, 'playlist_mincount': 13}, {'url': 'https://storyfire.com/write/series/stories/the_mortal_one/', 'info_dict': {'id': 'the_mortal_one'}, 'playlist_count': 0}]
