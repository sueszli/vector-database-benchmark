...
flags = tf.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('checkpoint_from_path', None, 'Source BERT checkpoint path.')
flags.DEFINE_string('checkpoint_to_path', None, 'Destination BERT checkpoint path.')
flags.DEFINE_string('exclude_patterns', None, 'Comma-delimited string of a list of patterns to exclude variables from source checkpoint.')
flags.DEFINE_integer('num_heads', -1, 'The number of attention heads, used to reshape variables. If it is -1, we do not reshape variables.')
flags.DEFINE_boolean('create_v2_checkpoint', False, 'Whether to create a checkpoint compatible with KerasBERT V2 modeling code.')
