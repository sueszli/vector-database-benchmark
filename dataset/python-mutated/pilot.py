...
class Pilot(Generic[ReturnType]):
    ...
    async def click(self, selector: type[Widget] | str | None=None, offset: tuple[int, int]=(0, 0), shift: bool=False, meta: bool=False, control: bool=False) -> bool:
        ...
        message_arguments = _get_mouse_message_arguments(target_widget, offset, button=1, shift=shift, meta=meta, control=control)
        click_offset = Offset(message_arguments['x'], message_arguments['y'])
        if click_offset not in screen.region:
            raise OutOfBounds('Target offset is outside of currently-visible screen region.')
    ...
    async def hover(self, selector: type[Widget] | str | None | None=None, offset: tuple[int, int]=(0, 0)) -> bool:
        ...
        if selector is not None:
            ...
            target_widget = screen
        message_arguments = _get_mouse_message_arguments(target_widget, offset, button=0)
    ...
    async def _wait_for_screen(self, timeout: float=30.0) -> bool:
        ...
        for child in children:
            if child.call_later(decrement_counter):
                count += 1
        if count:
            wait_for = [asyncio.create_task(count_zero_event.wait()), asyncio.create_task(self.app._exception_event.wait())]
            (_, pending) = await asyncio.wait(wait_for, timeout=timeout, return_when=asyncio.FIRST_COMPLETED)
            for task in pending:
                task.cancel()
