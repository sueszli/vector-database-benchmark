...
import os
import pytest
from localstack.constants import SECONDARY_TEST_AWS_ACCESS_KEY_ID, SECONDARY_TEST_AWS_SECRET_ACCESS_KEY, TEST_AWS_ACCOUNT_ID, TEST_AWS_REGION_NAME
...
STATE_MACHINE_BASIC = {'Comment': 'Hello World example', 'StartAt': 'step1', 'States': {'step1': {'Type': 'Task', 'Resource': '__tbd__', 'Next': 'step2'}, 'step2': {'Type': 'Task', 'Resource': '__tbd__', 'ResultPath': '$.result_value', 'End': True}}}
TEST_LAMBDA_NAME_3 = 'lambda_map_sfn_3'
STATE_MACHINE_MAP = {'Comment': 'Hello Map State', 'StartAt': 'ExampleMapState', 'States': {'ExampleMapState': {'Type': 'Map', 'ItemProcessor': {'ProcessorConfig': {'Mode': 'INLINE'}, 'StartAt': 'CallLambda', 'States': {'CallLambda': {'Type': 'Task', 'Resource': '__tbd__', 'End': True}}}, 'End': True}}}
TEST_LAMBDA_NAME_4 = 'lambda_choice_sfn_4'
STATE_MACHINE_CHOICE = {'StartAt': 'CheckValues', 'States': {'CheckValues': {'Type': 'Choice', 'Choices': [{'And': [{'Variable': '$.x', 'IsPresent': True}, {'Variable': '$.y', 'IsPresent': True}], 'Next': 'Add'}], 'Default': 'MissingValue'}, 'MissingValue': {'Type': 'Fail', 'Cause': 'test'}, 'Add': {'Type': 'Task', 'Resource': '__tbd__', 'ResultPath': '$.added', 'TimeoutSeconds': 10, 'End': True}}}
STATE_MACHINE_CATCH = {'StartAt': 'Start', 'States': {'Start': {'Type': 'Task', 'Resource': 'arn:aws:states:::lambda:invoke', 'Parameters': {'FunctionName': '__tbd__', 'Payload': {lambda_environment.MSG_BODY_RAISE_ERROR_FLAG: 1}}, 'Catch': [{'ErrorEquals': ['Exception', 'Lambda.Unknown', 'ValueError'], 'ResultPath': '$.error', 'Next': 'ErrorHandler'}], 'Next': 'Final'}, 'ErrorHandler': {'Type': 'Task', 'Resource': '__tbd__', 'ResultPath': '$.handled', 'Next': 'Final'}, 'Final': {'Type': 'Task', 'Resource': '__tbd__', 'ResultPath': '$.final', 'End': True}}}
TEST_LAMBDA_NAME_5 = 'lambda_intrinsic_sfn_5'
STATE_MACHINE_INTRINSIC_FUNCS = {'StartAt': 'state0', 'States': {'state0': {'Type': 'Pass', 'Result': {'v1': 1, 'v2': 'v2'}, 'ResultPath': '$', 'Next': 'state1'}, 'state1': {'Type': 'Pass', 'Parameters': {'lambda_params': {'FunctionName': '__tbd__', 'Payload': {'values.$': 'States.Array($.v1, $.v2)'}}}, 'Next': 'state2'}, 'state2': {'Type': 'Task', 'Resource': 'arn:aws:states:::lambda:invoke', 'Parameters': {'FunctionName.$': '$.lambda_params.FunctionName', 'Payload.$': 'States.StringToJson(States.JsonToString($.lambda_params.Payload))'}, 'Next': 'state3'}, 'state3': {'Type': 'Task', 'Resource': '__tbd__', 'ResultSelector': {'payload.$': '$'}, 'ResultPath': '$.result_value', 'End': True}}}
STATE_MACHINE_EVENTS = {'StartAt': 'step1', 'States': {'step1': {'Type': 'Task', 'Resource': 'arn:aws:states:::events:putEvents', 'Parameters': {'Entries': [{'DetailType': 'TestMessage', 'Source': 'TestSource', 'EventBusName': '__tbd__', 'Detail': {'Message': 'Hello from Step Functions!'}}]}, 'End': True}}}
...
def setup_and_tear_down(aws_client):
    ...
    testutil.create_lambda_function(func_name=TEST_LAMBDA_NAME_1, zip_file=zip_file, envvars={'Hello': TEST_RESULT_VALUE}, client=aws_client.lambda_, s3_client=aws_client.s3)
    testutil.create_lambda_function(func_name=TEST_LAMBDA_NAME_2, zip_file=zip_file, envvars={'Hello': TEST_RESULT_VALUE_2}, client=aws_client.lambda_, s3_client=aws_client.s3)
    testutil.create_lambda_function(func_name=TEST_LAMBDA_NAME_3, zip_file=zip_file, envvars={'Hello': 'Replace Value'}, client=aws_client.lambda_, s3_client=aws_client.s3)
    testutil.create_lambda_function(func_name=TEST_LAMBDA_NAME_4, zip_file=zip_file, envvars={'Hello': TEST_RESULT_VALUE_4}, client=aws_client.lambda_, s3_client=aws_client.s3)
    testutil.create_lambda_function(func_name=TEST_LAMBDA_NAME_5, zip_file=zip_file2, client=aws_client.lambda_, s3_client=aws_client.s3)
...
class TestStateMachine:
    ...
    def test_create_choice_state_machine(self, aws_client):
        ...
        lambda_arn_4 = arns.lambda_function_arn(TEST_LAMBDA_NAME_4, TEST_AWS_ACCOUNT_ID, TEST_AWS_REGION_NAME)
        definition['States']['Add']['Resource'] = lambda_arn_4
        definition = json.dumps(definition)
        sm_name = f'choice-{short_uid()}'
        aws_client.stepfunctions.create_state_machine(name=sm_name, definition=definition, roleArn=role_arn)
        ...
        input = {'x': '1', 'y': '2'}
        result = aws_client.stepfunctions.start_execution(stateMachineArn=sm_arn, input=json.dumps(input))
    ...
    def test_create_run_map_state_machine(self, aws_client):
        ...
        lambda_arn_3 = arns.lambda_function_arn(TEST_LAMBDA_NAME_3, TEST_AWS_ACCOUNT_ID, TEST_AWS_REGION_NAME)
        definition['States']['ExampleMapState']['ItemProcessor']['States']['CallLambda']['Resource'] = lambda_arn_3
        definition = json.dumps(definition)
        sm_name = f'map-{short_uid()}'
        aws_client.stepfunctions.create_state_machine(name=sm_name, definition=definition, roleArn=role_arn)
        assert_machine_created(state_machines_before, aws_client.stepfunctions)
        sm_arn = get_machine_arn(sm_name, aws_client.stepfunctions)
        result = aws_client.stepfunctions.start_execution(stateMachineArn=sm_arn, input=json.dumps(test_input))
        assert result.get('executionArn')
    ...
    def test_create_run_state_machine(self, aws_client):
        ...
        lambda_arn_1 = arns.lambda_function_arn(TEST_LAMBDA_NAME_1, TEST_AWS_ACCOUNT_ID, TEST_AWS_REGION_NAME)
        lambda_arn_2 = arns.lambda_function_arn(TEST_LAMBDA_NAME_2, TEST_AWS_ACCOUNT_ID, TEST_AWS_REGION_NAME)
    ...
    def test_intrinsic_functions(self, aws_client):
        ...
        lambda_arn_1 = arns.lambda_function_arn(TEST_LAMBDA_NAME_5, TEST_AWS_ACCOUNT_ID, TEST_AWS_REGION_NAME)
        lambda_arn_2 = arns.lambda_function_arn(TEST_LAMBDA_NAME_5, TEST_AWS_ACCOUNT_ID, TEST_AWS_REGION_NAME)
    ...
    @pytest.mark.skipif(condition=not is_aws_cloud(), reason='Accurate events reporting not yet supported.')
    @markers.aws.needs_fixing
    ...
    def test_create_state_machines_in_parallel(self, cleanups, aws_client):
        ...
        def _create_sm(*_):
            ...
            result = aws_client.stepfunctions.create_state_machine(name=sm_name, definition=definition, roleArn=role_arn)
            assert result['ResponseMetadata']['HTTPStatusCode'] == 200
            cleanups.append(lambda : aws_client.stepfunctions.delete_state_machine(stateMachineArn=result['stateMachineArn']))
            results.append(result)
            aws_client.stepfunctions.describe_state_machine(stateMachineArn=result['stateMachineArn'])
        num_machines = 30
        parallelize(_create_sm, list(range(num_machines)), size=2)
        assert len(results) == num_machines
TEST_STATE_MACHINE = {'StartAt': 's0', 'States': {'s0': {'Type': 'Pass', 'Result': {}, 'End': True}}}
TEST_STATE_MACHINE_2 = {'StartAt': 's1', 'States': {'s1': {'Type': 'Task', 'Resource': 'arn:aws:states:::states:startExecution.sync', 'Parameters': {'Input': {'Comment': 'Hello world!'}, 'StateMachineArn': '__machine_arn__', 'Name': 'ExecutionName'}, 'End': True}}}
TEST_STATE_MACHINE_3 = {'StartAt': 's1', 'States': {'s1': {'Type': 'Task', 'Resource': 'arn:aws:states:::states:startExecution.sync', 'Parameters': {'Input': {'Comment': 'Hello world!'}, 'StateMachineArn': '__machine_arn__', 'Name': 'ExecutionName'}, 'End': True}}}
STS_ROLE_POLICY_DOC = {'Version': '2012-10-17', 'Statement': [{'Effect': 'Allow', 'Principal': {'Service': ['states.amazonaws.com']}, 'Action': 'sts:AssumeRole'}]}

@pytest.mark.skipif(condition=not is_aws_cloud(), reason='Investigate error around states:startExecution.sync')
...
def test_multiregion_nested(aws_client_factory, region_name, statemachine_definition):
    if False:
        while True:
            i = 10
    client1 = aws_client_factory(region_name=region_name, aws_access_key_id=SECONDARY_TEST_AWS_ACCESS_KEY_ID, aws_secret_access_key=SECONDARY_TEST_AWS_SECRET_ACCESS_KEY)
...
def test_default_logging_configuration(create_state_machine, aws_client):
    ...
    try:
        role_arn = aws_client.iam.create_role(RoleName=role_name, AssumeRolePolicyDocument=json.dumps(STS_ROLE_POLICY_DOC))['Role']['Arn']
        ...
        assert result['ResponseMetadata']['HTTPStatusCode'] == 200
        result = aws_client.stepfunctions.describe_state_machine(stateMachineArn=result['stateMachineArn'])
        assert result['ResponseMetadata']['HTTPStatusCode'] == 200
    finally:
...
def test_aws_sdk_task(aws_client):
    ...
    role = aws_client.iam.create_role(RoleName=role_name, AssumeRolePolicyDocument='{"Version": "2012-10-17", "Statement": {"Action": "sts:AssumeRole", "Effect": "Allow", "Principal": {"Service": "states.amazonaws.com"}}}')
    policy = aws_client.iam.create_policy(PolicyDocument='{"Version": "2012-10-17", "Statement": {"Action": "sns:createTopic", "Effect": "Allow", "Resource": "*"}}', PolicyName=policy_name)
    aws_client.iam.attach_role_policy(RoleName=role['Role']['RoleName'], PolicyArn=policy['Policy']['Arn'])
    result = aws_client.stepfunctions.create_state_machine(name=name, definition=json.dumps(statemachine_definition), roleArn=role['Role']['Arn'])
    machine_arn = result['stateMachineArn']
    try:
        ...
        def assert_execution_success(executionArn: str):
            ...
            def _assert_execution_success():
                if False:
                    print('Hello World!')
                status = aws_client.stepfunctions.describe_execution(executionArn=executionArn)['status']
                if status == 'FAILED':
        ...
        def _retry_execution():
            if False:
                for i in range(10):
                    print('nop')
            result = aws_client.stepfunctions.start_execution(stateMachineArn=machine_arn, input=f'{{"Name": "{topic_name}"}}')
            assert wait_until(assert_execution_success(result['executionArn']))
            describe_result = aws_client.stepfunctions.describe_execution(executionArn=result['executionArn'])
