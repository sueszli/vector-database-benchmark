...
class TestSubpartitionCRUD:
    ...
    async def test_collection_crud_async(self):
        ...
        created_collection = await created_db.create_container(id=collection_id, indexing_policy=collection_indexing_policy, partition_key=PartitionKey(path=['/pk1', '/pk2'], kind='MultiHash'), response_hook=created_recorder)
        ...
        container_proxy = await created_db.create_container_if_not_exists(id=created_collection.id, partition_key=PartitionKey(path=['/id1', '/id2', '/id3'], kind='MultiHash'))
    ...
    async def test_partitioned_collection_async(self):
        await self._set_up()
        created_db = self.database_for_test
        collection_definition = {'id': 'test_partitioned_collection ' + str(uuid.uuid4()), 'partitionKey': {'paths': ['/id', '/pk'], 'kind': documents.PartitionKind.MultiHash, 'version': 2}}
        ...
        try:
            created_collection = await created_db.create_container(id=collection_definition['id'], partition_key=collection_definition2['partitionKey'], offer_throughput=offer_throughput)
        ...
        try:
            created_collection = await created_db.create_container(id=collection_definition['id'], partition_key=collection_definition3['partitionKey'], offer_throughput=offer_throughput)
        except exceptions.CosmosHttpResponseError as error:
            assert error.status_code == StatusCodes.BAD_REQUEST
            assert 'Too many partition key paths' in error.message
        await created_db.delete_container(created_collection.id)
    ...
    async def test_partitioned_collection_partition_key_extraction_async(self):
        ...
        created_collection = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path=['/address/state', '/address/city'], kind=documents.PartitionKind.MultiHash))
        ...
        created_collection1 = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path=['/address/state/city', '/address/city/state'], kind=documents.PartitionKind.MultiHash))
        self.OriginalExecuteFunction = _retry_utility_async.ExecuteFunctionAsync
        _retry_utility_async.ExecuteFunctionAsync = self._MockExecuteFunction
        try:
    ...
    async def test_partitioned_collection_partition_key_extraction_special_chars_async(self):
        ...
        created_collection1 = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path=['/"first level\' 1*()"/"le/vel2"', '/"second level\' 1*()"/"le/vel2"'], kind=documents.PartitionKind.MultiHash))
        ...
        del self.last_headers[:]
        collection_definition2 = {'id': 'test_partitioned_collection_partition_key_extraction_special_chars2 ' + str(uuid.uuid4()), 'partitionKey': {'paths': ['/\'first level" 1*()\'/\'first le/vel2\'', '/\'second level" 1*()\'/\'second le/vel2\''], 'kind': documents.PartitionKind.MultiHash}}
        collection_id = 'test_partitioned_collection_partition_key_extraction_special_chars2 ' + str(uuid.uuid4())
        created_collection2 = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path=collection_definition2['partitionKey']['paths'], kind=collection_definition2['partitionKey']['kind']))
    ...
    async def test_partitioned_collection_document_crud_and_query_async(self):
        ...
        created_collection = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path=['/city', '/zipcode'], kind=documents.PartitionKind.MultiHash))
        ...
        read_document = await created_collection.read_item(item=created_document.get('id'), partition_key=[created_document.get('city'), created_document.get('zipcode')])
        ...
        await created_collection.delete_item(item=upserted_document, partition_key=[upserted_document.get('city'), upserted_document.get('zipcode')])
        documentlist = [document async for document in created_collection.query_items(query="SELECT * FROM root r WHERE r.city='" + replaced_document.get('city') + "' and r.zipcode='" + replaced_document.get('zipcode') + "'")]
        assert 1 == len(documentlist)
        documentlist = [document async for document in created_collection.query_items(query="SELECT * FROM root r WHERE r.key='" + replaced_document.get('key') + "'")]
        assert 1 == len(documentlist)
        documentlist = [document async for document in created_collection.query_items(query="SELECT * FROM root r WHERE r.key='" + replaced_document.get('key') + "'", partition_key=[replaced_document.get('city'), replaced_document.get('zipcode')])]
        assert 1 == len(documentlist)
    ...
    async def test_partitioned_collection_prefix_partition_query_async(self):
        ...
        created_collection = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path=['/state', '/city', '/zipcode'], kind=documents.PartitionKind.MultiHash))
        ...
        for document_definition in document_definitions:
            created_documents.append(await created_collection.create_item(body=document_definition))
        ...
        document_list = [document async for document in created_collection.query_items(query='Select * from c', partition_key=['CA'])]
        assert 8 == len(document_list)
        document_list = [document async for document in created_collection.query_items(query='Select * from c', partition_key=['CA', 'Oxnard'])]
        assert 3 == len(document_list)
        document_list = [document async for document in created_collection.query_items(query='Select * from c where c.zipcode = "93033"', partition_key=['CA'])]
        assert 1 == len(document_list)
        document_list = [document async for document in created_collection.query_items(query='Select * from c', partition_key=[None, '93033'])]
        assert 0 == len(document_list)
        document_list = [document async for document in created_collection.query_items(query='Select * from c', partition_key=[255, 255])]
        assert 0 == len(document_list)
        try:
            document_list = [document async for document in created_collection.query_items(query='Select * from c', partition_key=[])]
            pytest.fail('Test did not fail as expected')
        except exceptions.CosmosHttpResponseError as error:
            assert error.status_code == StatusCodes.BAD_REQUEST
