...
class PageHistoryReportFilterSet(WagtailFilterSet):
    ...
    user = django_filters.ModelChoiceFilter(label=_('User'), field_name='user', queryset=lambda request: PageLogEntry.objects.all().get_users())
...
class WorkflowHistoryDetailView(PageWorkflowHistoryViewMixin, history.WorkflowHistoryDetailView):
