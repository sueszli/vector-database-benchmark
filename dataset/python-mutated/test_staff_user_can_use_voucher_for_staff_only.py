...
@pytest.mark.e2e
def test_staff_can_use_voucher_for_staff_only_in_checkout_core_0904(e2e_staff_api_client, e2e_no_permission_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_shipping, permission_manage_product_types_and_attributes, permission_manage_discounts):
    if False:
        i = 10
        return i + 15
    ...
    (voucher_discount_value, voucher_code) = prepare_voucher_for_staff_only(e2e_staff_api_client, channel_id, 'VOUCHER001', 'FIXED', 1)
    lines = [{'variantId': product_variant_id, 'quantity': 1}]
    checkout_data = checkout_create(e2e_no_permission_staff_api_client, lines, channel_slug, email='testEmail@example.com', set_default_billing_address=True, set_default_shipping_address=True)
    ...
    checkout_data = checkout_add_promo_code(e2e_no_permission_staff_api_client, checkout_id, voucher_code)
    unit_price_with_voucher = float(product_variant_price) - voucher_discount_value
    assert checkout_data['lines'][0]['unitPrice']['gross']['amount'] == unit_price_with_voucher
    checkout_data = checkout_delivery_method_update(e2e_no_permission_staff_api_client, checkout_id, shipping_method_id)
    ...
    checkout_dummy_payment_create(e2e_no_permission_staff_api_client, checkout_id, total_gross_amount)
