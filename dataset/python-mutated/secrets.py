...
async def download(context: ConnectorContext, gcp_gsm_env_variable_name: str='GCP_GSM_CREDENTIALS') -> dict[str, Secret]:
    ...
    with_downloaded_secrets = ci_credentials.with_exec(['mkdir', '-p', secrets_path]).with_env_variable('CACHEBUSTER', datetime.datetime.now().isoformat()).with_exec(['ci_credentials', context.connector.technical_name, 'write-to-storage'])
    if context.is_ci:
...
async def upload(context: ConnectorContext, gcp_gsm_env_variable_name: str='GCP_GSM_CREDENTIALS'):
    ...
    secrets_path = f'/{context.connector.code_directory}/secrets'
    ci_credentials = await with_ci_credentials(context, gsm_secret)
    return await ci_credentials.with_directory(secrets_path, context.updated_secrets_dir).with_exec(['ci_credentials', context.connector.technical_name, 'update-secrets'])
