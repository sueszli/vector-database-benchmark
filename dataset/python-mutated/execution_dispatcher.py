    ...
    async def __anext__(self):
        ...
        try:
            ...
            if _is_partitioned_execution_info(e):
                ...
                query_execution_info = _PartitionedQueryExecutionInfo(await self._client._GetQueryPlanThroughGateway(query_to_use, self._resource_link))
    ...
    async def _create_pipelined_execution_context(self, query_execution_info):
        ...
        if query_execution_info.has_aggregates() and (not query_execution_info.has_select_value()):
            if self._options and ('enableCrossPartitionQuery' in self._options and self._options['enableCrossPartitionQuery']):
                raise CosmosHttpResponseError(StatusCodes.BAD_REQUEST, "Cross partition query only supports 'VALUE <AggregateFunc>' for aggregates")
