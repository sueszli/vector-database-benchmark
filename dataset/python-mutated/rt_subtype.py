...
from mypyc.ir.rtypes import RArray, RInstance, RPrimitive, RStruct, RTuple, RType, RTypeVisitor, RUnion, RVoid, is_bit_rprimitive, is_bool_rprimitive, is_int_rprimitive, is_short_int_rprimitive
...
class RTSubtypeVisitor(RTypeVisitor[bool]):
    ...
    def visit_rtuple(self, left: RTuple) -> bool:
        ...
        if isinstance(self.right, RTuple):
            return len(self.right.types) == len(left.types) and all((is_runtime_subtype(t1, t2) for (t1, t2) in zip(left.types, self.right.types)))
        return False
