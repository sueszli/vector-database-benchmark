...
class RelationExtractionTest(unittest.TestCase):
    ...
    def test_run_by_direct_model_download(self):
        ...
        pipeline1 = InformationExtractionPipeline(model, preprocessor=tokenizer)
        pipeline2 = pipeline(Tasks.relation_extraction, model=model, preprocessor=tokenizer)
    ...
    def test_run_with_model_from_modelhub(self):
        ...
        pipeline_ins = pipeline(task=Tasks.relation_extraction, model=model, preprocessor=tokenizer)
    ...
    def test_run_with_model_name(self):
        if False:
            while True:
                i = 10
        pipeline_ins = pipeline(task=Tasks.relation_extraction, model=self.model_id)
        print(pipeline_ins(input=self.sentence))
