...
from aiortc.rtcrtpparameters import RTCRtpCapabilities, RTCRtpCodecCapability, RTCRtpCodecParameters, RTCRtpHeaderExtensionCapability, RTCRtpParameters
from aiortc.rtcrtpsender import RTCRtpSender
from aiortc.rtp import RTCP_PSFB_APP, RTCP_PSFB_PLI, RTCP_RTPFB_NACK, RtcpPsfbPacket, RtcpReceiverInfo, RtcpRrPacket, RtcpRtpfbPacket, RtpPacket, is_rtcp, pack_remb_fci
...
H264_CODEC = RTCRtpCodecParameters(mimeType='video/H264', clockRate=90000, payloadType=98)
...
class RTCRtpSenderTest(TestCase):
    ...
    async def test_handle_rtcp_remb(self):
        async with dummy_dtls_transport_pair() as (local_transport, _):
            ...
            self.assertEqual(sender.kind, 'video')
            await sender.send(RTCRtpParameters(codecs=[VP8_CODEC]))
            packet = RtcpPsfbPacket(fmt=RTCP_PSFB_APP, ssrc=1234, media_ssrc=0, fci=pack_remb_fci(4160000, [sender._ssrc]))
            await sender._handle_rtcp_packet(packet)
    ...
    async def test_handle_rtcp_rr(self):
        async with dummy_dtls_transport_pair() as (local_transport, _):
            ...
            self.assertEqual(sender.kind, 'video')
            await sender.send(RTCRtpParameters(codecs=[VP8_CODEC]))
            packet = RtcpRrPacket(ssrc=1234, reports=[RtcpReceiverInfo(ssrc=sender._ssrc, fraction_lost=0, packets_lost=0, highest_sequence=630, jitter=1906, lsr=0, dlsr=0)])
            await sender._handle_rtcp_packet(packet)
            report = await sender.getStats()
            self.assertIsInstance(report, RTCStatsReport)
            self.assertEqual(sorted([s.type for s in report.values()]), ['outbound-rtp', 'remote-inbound-rtp', 'transport'])
            await sender.stop()
    ...
    async def test_retransmit_with_rtx(self):
        ...
        async with dummy_dtls_transport_pair() as (local_transport, _):
            ...
            self.assertEqual(sender.kind, 'video')
            await sender.send(RTCRtpParameters(codecs=[VP8_CODEC, RTCRtpCodecParameters(mimeType='video/rtx', clockRate=90000, payloadType=101, parameters={'apt': 100})]))
            packet = await queue.get()
            await sender._retransmit(packet.sequence_number)
            await asyncio.sleep(0.1)
            await sender.stop()
