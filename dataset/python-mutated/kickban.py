...
from redbot.core.utils.chat_formatting import pagify, humanize_number, bold, humanize_list, format_perms_list
...
class KickBanMixin(MixinMeta):
    ...
    async def get_invite_for_reinvite(ctx: commands.Context, max_age: int=86400) -> str:
        ...
        for inv in invites:
            ...
            channels_and_perms = ((channel, channel.permissions_for(guild.me)) for channel in guild.text_channels)
    ...
    async def _voice_perm_check(ctx: commands.Context, user_voice_state: Optional[discord.VoiceState], **perms: bool) -> bool:
        ...
        if not voice_channel.permissions_for(ctx.me) >= required_perms:
            await ctx.send(_("I require the {perms} permission(s) in that user's channel to do that.").format(perms=format_perms_list(required_perms)))
            return False
        if ctx.permission_state is commands.PermState.NORMAL and (not voice_channel.permissions_for(ctx.author) >= required_perms):
            await ctx.send(_("You must have the {perms} permission(s) in that user's channel to use this command.").format(perms=format_perms_list(required_perms)))
            return False
        return True
    ...
    async def ban_user(self, user: Union[discord.Member, discord.User, discord.Object], ctx: commands.Context, days: int=0, reason: str=None, create_modlog_case=False) -> Tuple[bool, str]:
        ...
        if isinstance(user, discord.Member):
            if author == user:
                return (False, _('I cannot let you do that. Self-harm is bad {}').format('😔'))
            ...
            if toggle:
                with contextlib.suppress(discord.HTTPException):
                    em = discord.Embed(title=bold(_('You have been banned from {guild}.').format(guild=guild)), color=await self.bot.get_embed_color(user))
                    em.add_field(name=_('**Reason**'), value=reason if reason is not None else _('No reason was given.'), inline=False)
        ...
        if removed_temp:
            log.info('%s (%s) upgraded the tempban for %s to a permaban.', author, author.id, user.id)
            success_message = _('User with ID {user_id} was upgraded from a temporary to a permanent ban.').format(user_id=user.id)
        else:
            ...
            try:
                ...
                log.info('%s (%s) %sned %s (%s), deleting %s days worth of messages.', author, author.id, ban_type, user_handle, user.id, days)
                ...
                log.exception('%s (%s) attempted to %s %s (%s), but an error occurred.', author, author.id, ban_type, user_handle, user.id)
                return (False, _('An unexpected error occurred.'))
        if create_modlog_case:
            await modlog.create_case(self.bot, guild, ctx.message.created_at, ban_type, user, author, reason, until=None, channel=None)
    ...
    async def kick(self, ctx: commands.Context, member: discord.Member, *, reason: str=None):
        ...
        guild = ctx.guild
        if author == member:
            await ctx.send(_('I cannot let you do that. Self-harm is bad {emoji}').format(emoji='😔'))
            return
        ...
        if toggle:
            with contextlib.suppress(discord.HTTPException):
                em = discord.Embed(title=bold(_('You have been kicked from {guild}.').format(guild=guild)), color=await self.bot.get_embed_color(member))
                em.add_field(name=_('**Reason**'), value=reason if reason is not None else _('No reason was given.'), inline=False)
        ...
        try:
            ...
            log.exception('%s (%s) attempted to kick %s (%s), but an error occurred.', author, author.id, member, member.id)
        else:
            await modlog.create_case(self.bot, guild, ctx.message.created_at, 'kick', member, author, reason, until=None, channel=None)
    ...
    async def massban(self, ctx: commands.Context, user_ids: commands.Greedy[RawUserIdConverter], days: Optional[int]=None, *, reason: str=None):
        ...
        for (user_id, member) in members.items():
            try:
                ...
                errors[user_id] = _('Failed to ban user {user_id}: {reason}').format(user_id=user_id, reason=e)
        user_ids = remove_processed(user_ids)
        if not user_ids:
            await show_results()
            return
            ...
            async with self.config.guild(guild).current_tempbans() as tempbans:
                if user_id in tempbans:
                    ...
                    try:
                        ...
                        banned.append(user_id)
            await modlog.create_case(self.bot, guild, ctx.message.created_at, 'hackban', user_id, author, reason, until=None, channel=None)
        await show_results()
    ...
    async def tempban(self, ctx: commands.Context, member: discord.Member, duration: Optional[commands.TimedeltaConverter]=None, days: Optional[int]=None, *, reason: str=None):
        ...
        author = ctx.author
        if author == member:
            await ctx.send(_('I cannot let you do that. Self-harm is bad {}').format('😔'))
            return
        elif not await is_allowed_by_hierarchy(self.bot, self.config, guild, author, member):
            await ctx.send(_('I cannot let you do that. You are not higher than the user in the role hierarchy.'))
            return
        ...
        try:
            ...
            await modlog.create_case(self.bot, guild, ctx.message.created_at, 'tempban', member, author, reason, unban_time)
    ...
    async def softban(self, ctx: commands.Context, member: discord.Member, *, reason: str=None):
        ...
        try:
            msg = await member.send(_('You have been banned and then unbanned as a quick way to delete your messages.\nYou can now join the server again. {invite_link}').format(invite_link=invite))
        except discord.HTTPException:
            msg = None
        try:
        ...
        try:
            ...
            await modlog.create_case(self.bot, guild, ctx.message.created_at, 'softban', member, author, reason, until=None, channel=None)
    ...
    async def voicekick(self, ctx: commands.Context, member: discord.Member, *, reason: str=None):
        ...
        try:
            ...
            await modlog.create_case(self.bot, guild, ctx.message.created_at, 'vkick', member, author, reason, until=None, channel=case_channel)
    ...
    async def voiceunban(self, ctx: commands.Context, member: discord.Member, *, reason: str=None):
        ...
        guild = ctx.guild
        author = ctx.author
        await modlog.create_case(self.bot, guild, ctx.message.created_at, 'voiceunban', member, author, reason, until=None, channel=None)
    ...
    async def voiceban(self, ctx: commands.Context, member: discord.Member, *, reason: str=None):
        ...
        if needs_mute and needs_deafen:
            ...
            return
        await modlog.create_case(self.bot, guild, ctx.message.created_at, 'voiceban', member, author, reason, until=None, channel=None)
    ...
    async def unban(self, ctx: commands.Context, user_id: RawUserIdConverter, *, reason: str=None):
        ...
        try:
            ...
            await modlog.create_case(self.bot, guild, ctx.message.created_at, 'unban', ban_entry.user, author, reason, until=None, channel=None)
        ...
        if await self.config.guild(guild).reinvite_on_unban():
            ...
            if invite:
                try:
                    await user.send(_("You've been unbanned from {server}.\nHere is an invite for that server: {invite_link}").format(server=guild.name, invite_link=invite))
                except discord.Forbidden:
                    await ctx.send(_("I failed to send an invite to that user. Perhaps you may be able to send it for me?\nHere's the invite link: {invite_link}").format(invite_link=invite))
                except discord.HTTPException:
                    await ctx.send(_("Something went wrong when attempting to send that user an invite. Here's the link so you can try: {invite_link}").format(invite_link=invite))