...
group.add_argument('-p', '--path', help='Filter results by library/objfile path.', type=str, default='', dest='path_filter')
group.add_argument('-a', '--all', help='Process all libs/obfjiles including the target executable.', action='store_true', default=False, dest='all_')
parser.add_argument('-r', '--show-readonly', help='Also display read-only entries (which are filtered out by default).', action='store_true', default=False, dest='accept_readonly')
parser.add_argument('symbol_filter', help='Filter results by symbol name.', type=str, nargs='?', default='')
