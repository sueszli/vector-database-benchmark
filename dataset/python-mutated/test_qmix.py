...
from rllib_qmix.qmix import QMixConfig
import ray
...
class AvailActionsTestEnv(MultiAgentEnv):
    ...
    observation_space = Dict({'obs': Dict({'test': Dict({'a': Discrete(2), 'b': MultiDiscrete([2, 3, 4])}), 'state': MultiDiscrete([2, 2, 2])}), 'action_mask': Box(0, 1, (num_actions,))})
