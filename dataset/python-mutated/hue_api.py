...
from homeassistant.components import climate, cover, fan, humidifier, light, media_player, scene, script
from homeassistant.components.climate import SERVICE_SET_TEMPERATURE, ClimateEntityFeature
...
from homeassistant.components.light import ATTR_BRIGHTNESS, ATTR_COLOR_TEMP, ATTR_HS_COLOR, ATTR_TRANSITION, ATTR_XY_COLOR, LightEntityFeature
from homeassistant.components.media_player import ATTR_MEDIA_VOLUME_LEVEL, MediaPlayerEntityFeature
from homeassistant.const import ATTR_ENTITY_ID, ATTR_SUPPORTED_FEATURES, ATTR_TEMPERATURE, SERVICE_CLOSE_COVER, SERVICE_OPEN_COVER, SERVICE_SET_COVER_POSITION, SERVICE_TURN_OFF, SERVICE_TURN_ON, SERVICE_VOLUME_SET, STATE_OFF, STATE_ON, STATE_UNAVAILABLE
...
UNAUTHORIZED_USER = [{'error': {'address': '/', 'description': 'unauthorized user', 'type': '1'}}]
DIMMABLE_SUPPORT_FEATURES = CoverEntityFeature.SET_POSITION | FanEntityFeature.SET_SPEED | MediaPlayerEntityFeature.VOLUME_SET | ClimateEntityFeature.TARGET_TEMPERATURE
...
class HueOneLightChangeView(HomeAssistantView):
    ...
    async def put(self, request: web.Request, username: str, entity_number: str) -> web.Response:
        ...
        parsed: dict[str, Any] = {STATE_ON: False, STATE_BRIGHTNESS: None, STATE_HUE: None, STATE_SATURATION: None, STATE_COLOR_TEMP: None, STATE_XY: None, STATE_TRANSITION: None}
        ...
        for (key, attr) in ((HUE_API_STATE_BRI, STATE_BRIGHTNESS), (HUE_API_STATE_HUE, STATE_HUE), (HUE_API_STATE_SAT, STATE_SATURATION), (HUE_API_STATE_CT, STATE_COLOR_TEMP), (HUE_API_STATE_TRANSITION, STATE_TRANSITION)):
        ...
        if HUE_API_STATE_XY in request_json:
            try:
                parsed[STATE_XY] = (float(request_json[HUE_API_STATE_XY][0]), float(request_json[HUE_API_STATE_XY][1]))
            except ValueError:
                _LOGGER.error('Unable to parse data (2): %s', request_json)
                return self.json_message('Bad request', HTTPStatus.BAD_REQUEST)
        if HUE_API_STATE_BRI in request_json:
            ...
            elif entity.domain in [script.DOMAIN, media_player.DOMAIN, fan.DOMAIN, cover.DOMAIN, climate.DOMAIN, humidifier.DOMAIN]:
        ...
        if entity.domain == light.DOMAIN:
            if parsed[STATE_ON]:
                if light.brightness_supported(color_modes) and parsed[STATE_BRIGHTNESS] is not None:
                ...
                if light.color_temp_supported(color_modes) and parsed[STATE_COLOR_TEMP] is not None:
                    data[ATTR_COLOR_TEMP] = parsed[STATE_COLOR_TEMP]
                if entity_features & LightEntityFeature.TRANSITION and parsed[STATE_TRANSITION] is not None:
                    data[ATTR_TRANSITION] = parsed[STATE_TRANSITION] / 10
        elif entity.domain == script.DOMAIN:
            data['variables'] = {'requested_state': STATE_ON if parsed[STATE_ON] else STATE_OFF}
            ...
            service = None
            if entity_features & ClimateEntityFeature.TARGET_TEMPERATURE and parsed[STATE_BRIGHTNESS] is not None:
            ...
            if entity_features & MediaPlayerEntityFeature.VOLUME_SET and parsed[STATE_BRIGHTNESS] is not None:
                turn_on_needed = True
                domain = entity.domain
            ...
            if entity_features & CoverEntityFeature.SET_POSITION and parsed[STATE_BRIGHTNESS] is not None:
                domain = entity.domain
                service = SERVICE_SET_COVER_POSITION
                data[ATTR_POSITION] = parsed[STATE_BRIGHTNESS]
        elif entity.domain == fan.DOMAIN and entity_features & FanEntityFeature.SET_SPEED and (parsed[STATE_BRIGHTNESS] is not None):
            domain = entity.domain
            data[ATTR_PERCENTAGE] = parsed[STATE_BRIGHTNESS]
        if entity.domain in config.off_maps_to_on_domains:
            service = SERVICE_TURN_ON
        if turn_on_needed:
            hass.async_create_task(hass.services.async_call(core.DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: entity_id}, blocking=True))
        if service is not None:
            state_will_change = parsed[STATE_ON] != (entity.state != STATE_OFF)
            hass.async_create_task(hass.services.async_call(domain, service, data, blocking=True))
            if state_will_change:
                await wait_for_state_change_or_timeout(hass, entity_id, STATE_CACHED_TIMEOUT)
        json_response = [create_hue_success_response(entity_number, HUE_API_STATE_ON, parsed[STATE_ON])]
        for (key, val) in ((STATE_BRIGHTNESS, HUE_API_STATE_BRI), (STATE_HUE, HUE_API_STATE_HUE), (STATE_SATURATION, HUE_API_STATE_SAT), (STATE_COLOR_TEMP, HUE_API_STATE_CT), (STATE_XY, HUE_API_STATE_XY), (STATE_TRANSITION, HUE_API_STATE_TRANSITION)):
            if parsed[key] is not None:
                json_response.append(create_hue_success_response(entity_number, val, parsed[key]))
        if entity.domain in config.off_maps_to_on_domains:
