...
if __name__ == '__main__':
    ...
    for index in range(len(URMs_train)):
        UserKNN_recommenders.append(UserKNNCFRecommender(URM_train=URMs_train[index], verbose=True))
        UserKNN_recommenders[index].fit(topK=497, shrink=0, similarity='tversky', normalize=True, tversky_alpha=2.0, tversky_beta=1.0492068509506025)
    ...
    def BO_func(alpha):
        ...
        recommenders = []
        for index in range(len(URMs_train)):
            recommender = GeneralizedMergedHybridRecommender(URM_train=URMs_train[index], recommenders=[UserKNN_recommenders[index], SLIM_recommenders[index]], verbose=False)
    ...
    recommender = GeneralizedMergedHybridRecommender(URM_train=URMs_train[0], recommenders=[UserKNN_recommenders[0], SLIM_recommenders[0]], verbose=False)
    recommender.fit()
