    ...
    for (key, value) in data.multi_items():
        for v in value:
            if isinstance(v, UploadFile):
                ...
                output[key].append({'filename': v.filename, 'content': content.decode(), 'content_type': v.content_type})
            else:
                output[key].append(v)
    return output
...
async def form_with_headers_handler(request: Request) -> Dict[str, Any]:
    ...
    for (key, value) in data.items():
        if isinstance(value, UploadFile):
            content = await value.read()
            output[key] = {'filename': value.filename, 'content': content.decode(), 'content_type': value.content_type, 'headers': [[name.lower(), value] for (name, value) in value.headers.items()]}
        else:
            output[key] = value
    return output
