...
DEVICE_INFO_TYPES = {'link': {'connections', 'identifiers'}, 'primary': {'configuration_url', 'connections', 'entry_type', 'hw_version', 'identifiers', 'manufacturer', 'model', 'name', 'serial_number', 'suggested_area', 'sw_version', 'via_device'}, 'secondary': {'connections', 'default_manufacturer', 'default_model', 'default_name', 'via_device'}}
    ...
    async def async_load(self) -> None:
        ...
        if data is not None:
            for device in data['devices']:
                devices[device['id']] = DeviceEntry(area_id=device['area_id'], config_entries=set(device['config_entries']), configuration_url=device['configuration_url'], connections={tuple(conn) for conn in device['connections']}, disabled_by=DeviceEntryDisabler(device['disabled_by']) if device['disabled_by'] else None, entry_type=DeviceEntryType(device['entry_type']) if device['entry_type'] else None, hw_version=device['hw_version'], id=device['id'], identifiers={tuple(iden) for iden in device['identifiers']}, manufacturer=device['manufacturer'], model=device['model'], name_by_user=device['name_by_user'], name=device['name'], serial_number=device['serial_number'], sw_version=device['sw_version'], via_device_id=device['via_device_id'])
            for device in data['deleted_devices']:
                deleted_devices[device['id']] = DeletedDeviceEntry(config_entries=set(device['config_entries']), connections={tuple(conn) for conn in device['connections']}, identifiers={tuple(iden) for iden in device['identifiers']}, id=device['id'], orphaned_timestamp=device['orphaned_timestamp'])
