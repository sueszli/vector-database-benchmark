...
class mapsplatformdatasetsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_dataset': ('parent', 'dataset'), 'delete_dataset': ('name', 'force'), 'delete_dataset_version': ('name',), 'get_dataset': ('name', 'published_usage'), 'list_datasets': ('parent', 'page_size', 'page_token'), 'list_dataset_versions': ('name', 'page_size', 'page_token'), 'update_dataset_metadata': ('dataset', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=mapsplatformdatasetsCallTransformer()):
    if False:
        while True:
            i = 10
