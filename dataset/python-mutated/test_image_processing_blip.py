...
class BlipImageProcessingTester(unittest.TestCase):
    ...
    def __init__(self, parent, batch_size=7, num_channels=3, image_size=18, min_resolution=30, max_resolution=400, do_resize=True, size=None, do_normalize=True, do_pad=False, image_mean=[0.48145466, 0.4578275, 0.40821073], image_std=[0.26862954, 0.26130258, 0.27577711], do_convert_rgb=True):
        if False:
            return 10
    ...
    def prepare_image_processor_dict(self):
        if False:
            print('Hello World!')
        return {'do_resize': self.do_resize, 'size': self.size, 'do_normalize': self.do_normalize, 'image_mean': self.image_mean, 'image_std': self.image_std, 'do_convert_rgb': self.do_convert_rgb, 'do_pad': self.do_pad}
...
class BlipImageProcessingTestFourChannels(ImageProcessingTestMixin, unittest.TestCase):
    ...
    @unittest.skip('BlipImageProcessor does not support 4 channels yet')
    ...
    @unittest.skip("BLIP doesn't treat 4 channel PIL and numpy consistently yet")
