...
class Test_s3_bucket_object_versioning:
    ...
    def test_bucket_no_object_versioning(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_object_versioning.s3_bucket_object_versioning.s3_client', new=S3(audit_info)):
                from prowler.providers.aws.services.s3.s3_bucket_object_versioning.s3_bucket_object_versioning import s3_bucket_object_versioning
                check = s3_bucket_object_versioning()
                result = check.execute()
                assert len(result) == 1
                assert result[0].status == 'FAIL'
                assert search('versioning disabled', result[0].status_extended)
    ...
    def test_bucket_object_versioning_enabled(self):
        ...
        s3_client_us_east_1.put_bucket_versioning(Bucket=bucket_name_us, VersioningConfiguration={'Status': 'Enabled'})
        from prowler.providers.aws.services.s3.s3_service import S3
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_object_versioning.s3_bucket_object_versioning.s3_client', new=S3(audit_info)):
                ...
                assert search('versioning enabled', result[0].status_extended)
                assert result[0].resource_id == bucket_name_us
                assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                assert result[0].region == 'us-east-1'