...
weights_arr = ((0.01, 0.01, 0.98), (0.01, 0.05, 0.94), (0.2, 0.1, 0.7), (0.33, 0.33, 0.33))
...
samplers = [FunctionSampler(), RandomOverSampler(random_state=0), SMOTE(random_state=0), ADASYN(random_state=0)]
...
models = {'Without sampler': clf, 'ADASYN sampler': make_pipeline(ADASYN(random_state=0), clf), 'SMOTE sampler': make_pipeline(SMOTE(random_state=0), clf)}
...
samplers = [SMOTE(random_state=0), BorderlineSMOTE(random_state=0, kind='borderline-1'), BorderlineSMOTE(random_state=0, kind='borderline-2'), KMeansSMOTE(kmeans_estimator=MiniBatchKMeans(n_clusters=10, n_init=1, random_state=0), random_state=0), SVMSMOTE(random_state=0)]
for (ax, sampler) in zip(axs, samplers):
    model = make_pipeline(sampler, clf).fit(X, y)
