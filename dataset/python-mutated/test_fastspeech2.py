...
class TestFastSpeech2(TestFairseqSpeech):
    ...
    def test_ljspeech_fastspeech2_checkpoint(self):
        if False:
            for i in range(10):
                print('nop')
        (models, cfg, task, generator) = self.download_and_load_checkpoint('ljspeech_fastspeech2_g2p.pt', arg_overrides={'config_yaml': 'cfg_ljspeech_g2p.yaml', 'vocoder': 'griffin_lim', 'fp16': False})
        batch_iterator = self.get_batch_iterator(task, 'ljspeech_test', 65536, 4096)
