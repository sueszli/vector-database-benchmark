...
from pytest_caching_utils import download_pytest_cache, GithubRepo, PRIdentifier, upload_pytest_cache
TEMP_DIR = './tmp'
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--shard', required='--upload' in sys.argv, help='The shard id')
    parser.add_argument('--repo', required=False, help="The github repository we're running in, in the format 'owner/repo-name'")
...
def main() -> None:
    ...
    if args.upload:
        ...
        if not cache_dir.exists():
            ...
            return
        upload_pytest_cache(pr_identifier=pr_identifier, repo=repo, job_identifier=args.job_identifier, shard=args.shard, cache_dir=cache_dir, bucket=args.bucket, temp_dir=temp_dir)
    if args.download:
        print(f'Downloading cache with args {args}')
        download_pytest_cache(pr_identifier=pr_identifier, repo=repo, job_identifier=args.job_identifier, dest_cache_dir=cache_dir, bucket=args.bucket, temp_dir=temp_dir)
