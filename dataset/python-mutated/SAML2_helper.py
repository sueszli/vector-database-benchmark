...
if SAML2['idp'] is None:
    if len(config('idp_entityId', default='')) > 0 and len(config('idp_sso_url', default='')) > 0 and (len(config('idp_x509cert', default='')) > 0):
        idp = {'entityId': config('idp_entityId'), 'singleSignOnService': {'url': config('idp_sso_url'), 'binding': 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect'}, 'x509cert': config('idp_x509cert')}
        if len(config('idp_sls_url', default='')) > 0:
            idp['singleLogoutService'] = {'url': config('idp_sls_url'), 'binding': 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect'}
if idp is None:
...
async def prepare_request(request: Request):
    ...
    if not path.startswith('/api'):
        path = '/api' + path
    return {'https': 'on' if proto == 'https' else 'off', 'http_host': request.headers['host'] + host_suffix, 'server_port': url_data.port, 'script_name': path, 'get_data': request.args.copy(), 'post_data': request.form.copy(), 'cookie': {'session': session}, 'request': request}
