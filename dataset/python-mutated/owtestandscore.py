...
from typing import Any, Optional, List, Dict, Callable, Sequence, NamedTuple, Tuple
import numpy as np
import baycomp
...
from AnyQt.QtWidgets import QHeaderView, QTableWidget, QLabel, QComboBox, QSizePolicy
...
from Orange.widgets.evaluate.utils import usable_scorers, ScoreTable, learner_name, scorer_caller
...
class OWTestAndScore(OWWidget):
    ...
    (KFold, FeatureFold, ShuffleSplit, LeaveOneOut, TestOnTrain, TestOnTest) = (0, 1, 2, 3, 4, 5)
    NFolds = [2, 3, 5, 10, 20]
    ...
    class Error(OWWidget.Error):
        ...
        class_inconsistent = Msg('Test and train datasets have different target variables.')
        memory_error = Msg('Not enough memory.')
        test_data_incompatible = Msg('Test data may be incompatible with train data.')
    ...
    class Warning(OWWidget.Warning):
        ...
        cant_stratify = Msg("Can't run stratified {}-fold cross validation; the least common class has only {} instances.")

    class Information(OWWidget.Information):
        data_sampled = Msg('Train data has been sampled')
        test_data_sampled = Msg('Test data has been sampled')
        test_data_transformed = Msg('Test data has been transformed to match the train data.')
        cant_stratify_numeric = Msg('Stratification is ignored for regression')
        cant_stratify_multitarget = Msg('Stratification is ignored when there are multiple target variables.')
...
if __name__ == '__main__':
    ...
    if preview_data.domain.class_var.is_discrete:
        prev_learners = [lambda data: 1 / 0, Orange.classification.LogisticRegressionLearner(), Orange.classification.MajorityLearner(), Orange.classification.NaiveBayesLearner()]
    else:
        prev_learners = [lambda data: 1 / 0, Orange.regression.MeanLearner(), Orange.regression.KNNRegressionLearner(), Orange.regression.RidgeRegressionLearner()]
    WidgetPreview(OWTestAndScore).run(set_train_data=preview_data, set_test_data=preview_data, insert_learner=list(enumerate(prev_learners)))