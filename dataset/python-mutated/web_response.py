...
class StreamResponse(BaseClass, HeadersMixin, CookieMixin):
    __slots__ = ('_length_check', '_body', '_keep_alive', '_chunked', '_compression', '_compression_force', '_req', '_payload_writer', '_eof_sent', '_must_be_empty_body', '_body_length', '_state', '_headers', '_status', '_reason', '_cookies', '__weakref__')
    ...
    async def _write_headers(self) -> None:
        ...
        assert writer is not None
        version = request.version
        status_line = 'HTTP/{}.{} {} {}'.format(version[0], version[1], self._status, self._reason)
...
class Response(StreamResponse):
    __slots__ = ('_body_payload', '_compressed_body', '_zlib_executor_size', '_zlib_executor')
    ...
    async def _do_start_compression(self, coding: ContentCoding) -> None:
        ...
        if coding != ContentCoding.identity:
            compressor = ZLibCompressor(encoding=str(coding.value), max_sync_chunk_size=self._zlib_executor_size, executor=self._zlib_executor)
