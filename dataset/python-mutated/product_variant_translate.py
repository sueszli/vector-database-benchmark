...
class ProductVariantTranslate(BaseTranslateMutation):
    ...
    class Arguments:
        id = graphene.ID(required=True, description='ProductVariant ID or ProductVariantTranslatableContent ID.')
        language_code = graphene.Argument(LanguageCodeEnum, required=True, description='Translation language code.')
        input = NameTranslationInput(required=True, description='Fields required to update product variant translations.')
    ...
    def perform_mutation(cls, _root, info: ResolveInfo, /, *, id, input, language_code):
        ...
        variant = product_models.ProductVariant.objects.prefetched_for_webhook().get(pk=variant_pk)
        cls.validate_input(input)
        manager = get_plugin_manager_promise(info.context).get()
        with traced_atomic_transaction():
            (translation, created) = variant.translations.update_or_create(language_code=language_code, defaults=input)
