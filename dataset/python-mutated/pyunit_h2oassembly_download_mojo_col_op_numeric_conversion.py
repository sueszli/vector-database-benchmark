...
def h2oassembly_download_mojo_col_op_numeric_conversion():
    if False:
        i = 10
        return i + 15
    values = [[12.5, '13'], [12, '23.5'], [1.1111, 's'], [124, '0.101']]
    frame = h2o.H2OFrame(python_obj=values, column_names=['a', 'b'], column_types=['numeric', 'string'])
    test_numeric_conversion_function(H2OFrame.asnumeric, frame)
    values = [['15.07.09 1:01', '15.07.09 1:01'], ['30.09.09 23:00', '30.09.09 23:00'], ['3.01.06 13:30', '3.01.06 13:30'], ['30.09.09 23:00', '3.01.06 13:30']]
    frame = h2o.H2OFrame(python_obj=values, column_names=['a', 'b'], column_types=['string', 'string'])
...
def test_numeric_conversion_function(function, frame, **params):
    if False:
        for i in range(10):
            print('nop')
    assembly = H2OAssembly(steps=[('col_op1_' + function.__name__, H2OColOp(op=function, col='a', new_col_name='x', inplace=False, **params)), ('col_op2_' + function.__name__, H2OColOp(op=function, col='b', new_col_name='y', inplace=False, **params))])
