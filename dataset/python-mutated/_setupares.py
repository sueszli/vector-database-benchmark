...
ares_configure_command = ' '.join(['(cd ', quoted_dep_abspath('c-ares'), ' && if [ -r include/ares_build.h ]; then cp include/ares_build.h include/ares_build.h.orig; fi ', ' && sh ./configure --disable-dependency-tracking -C ' + cflags, ' && cp src/lib/ares_config.h include/ares_build.h "$OLDPWD" ', ' && cat include/ares_build.h ', ' && if [ -r include/ares_build.h.orig ]; then mv include/ares_build.h.orig include/ares_build.h; fi)', '> configure-output.txt'])
...
def configure_ares(bext, ext):
    ...
    try:
        ...
        os.chdir(cwd)
ARES = Extension(name='gevent.resolver.cares', sources=['src/gevent/resolver/cares.pyx'], include_dirs=get_include_dirs(*([os.path.join(dep_abspath('c-ares'), 'include'), os.path.join(dep_abspath('c-ares'), 'src', 'lib')] if CARES_EMBED else [])), libraries=list(LIBRARIES), define_macros=list(DEFINE_MACROS), depends=glob_many('src/gevent/resolver/cares_*.[ch]'))
