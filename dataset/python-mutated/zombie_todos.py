...
headers = {'Accept': 'application/vnd.github+json', 'Authorization': f'Bearer {args.GITHUB_TOKEN}', 'X-GitHub-Api-Version': '2022-11-28'}
issues = []
repo_owner = 'rerun-io'
repo_name = 'rerun'
issue_state = 'closed'
...
async def fetch_total_number_of_issue_pages(session):
    ...
    async with session.get(url, headers=headers) as response:
        ...
        if link_header:
            match = re.search('page=(\\d+)>; rel="last"', link_header)
            if match:
...
internal_issue_number_pattern = re.compile('TODO\\((?:#(\\d+))(?:,\\s*(?:#(\\d+)))*\\)')
