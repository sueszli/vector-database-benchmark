...
class SubclassRelationshipTest2(testing.AssertsCompiledSQL, fixtures.DeclarativeMappedTest):
    run_setup_classes = 'once'
    run_setup_mappers = 'once'
    run_inserts = 'once'
    run_deletes = None
...
class SubclassRelationshipTest3(testing.AssertsCompiledSQL, fixtures.DeclarativeMappedTest):
    run_setup_classes = 'once'
    run_setup_mappers = 'once'
    run_inserts = 'once'
    run_deletes = None
    ...
    _query1 = 'SELECT b.id AS b_id, b.type AS b_type, b.a_id AS b_a_id, c.id AS c_id, c.type AS c_type, c.b_id AS c_b_id, a.id AS a_id, a.type AS a_type FROM a LEFT OUTER JOIN b ON a.id = b.a_id AND b.type IN (__[POSTCOMPILE_type_1]) LEFT OUTER JOIN c ON b.id = c.b_id AND c.type IN (__[POSTCOMPILE_type_2]) WHERE a.type IN (__[POSTCOMPILE_type_3])'
    _query2 = 'SELECT bbb.id AS bbb_id, bbb.type AS bbb_type, bbb.a_id AS bbb_a_id, ccc.id AS ccc_id, ccc.type AS ccc_type, ccc.b_id AS ccc_b_id, aaa.id AS aaa_id, aaa.type AS aaa_type FROM a AS aaa LEFT OUTER JOIN b AS bbb ON aaa.id = bbb.a_id AND bbb.type IN (__[POSTCOMPILE_type_1]) LEFT OUTER JOIN c AS ccc ON bbb.id = ccc.b_id AND ccc.type IN (__[POSTCOMPILE_type_2]) WHERE aaa.type IN (__[POSTCOMPILE_type_3])'
    _query3 = 'SELECT bbb.id AS bbb_id, bbb.type AS bbb_type, bbb.a_id AS bbb_a_id, c.id AS c_id, c.type AS c_type, c.b_id AS c_b_id, aaa.id AS aaa_id, aaa.type AS aaa_type FROM a AS aaa LEFT OUTER JOIN b AS bbb ON aaa.id = bbb.a_id AND bbb.type IN (__[POSTCOMPILE_type_1]) LEFT OUTER JOIN c ON bbb.id = c.b_id AND c.type IN (__[POSTCOMPILE_type_2]) WHERE aaa.type IN (__[POSTCOMPILE_type_3])'
