...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Color map optimizer for a reconstruction dataset')
    parser.add_argument('--config', type=str, required=True, help='path to the config for the dataset preprocessed by the Reconstruction System')
    parser.add_argument('--keys', type=str, help='txt file that contains the indices of the keyframes')
    parser.add_argument('--sample_rate', type=int, default=10, help='sampling rate that evenly sample key frames if key.txt is not provided')
    ...
    if keys is None:
        traj = o3d.io.read_pinhole_camera_trajectory(os.path.join(config['path_dataset'], config['template_global_traj']))
