...
class CommunicationRelayClient:
    ...
    async def get_relay_configuration(self, *, user: 'CommunicationUserIdentifier'=None, route_type: Optional[Union[str, 'RouteType']]=None, ttl: Optional[int]=172800, **kwargs) -> 'CommunicationRelayConfiguration':
        ...
        """
        if user is None:
            return await self._network_traversal_service_client.communication_network_traversal.issue_relay_configuration(route_type=route_type, ttl=ttl, **kwargs)
        return await self._network_traversal_service_client.communication_network_traversal.issue_relay_configuration(id=user.properties['id'], route_type=route_type, ttl=ttl, **kwargs)
