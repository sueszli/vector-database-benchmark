...
from salt.utils.virtualbox import HAS_LIBS, XPCOM_ATTRIBUTES, machine_get_machinestate_str, vb_clone_vm, vb_create_machine, vb_destroy_machine, vb_get_box, vb_get_network_addresses, vb_machine_exists, vb_start_vm, vb_stop_vm, vb_wait_for_network_address, vb_xpcom_to_attribute_dict
from tests.integration.cloud.helpers.virtualbox import BASE_BOX_NAME, BOOTABLE_BASE_BOX_NAME, CONFIG_NAME, DEPLOY_PROFILE_NAME, INSTANCE_NAME, PROFILE_NAME, PROVIDER_NAME, VirtualboxCloudTestCase, VirtualboxTestCase
...
@pytest.mark.skipif(HAS_LIBS and vb_machine_exists(BOOTABLE_BASE_BOX_NAME) is False, reason="Bootable VM '{}' not found. Cannot run tests.".format(BOOTABLE_BASE_BOX_NAME))
