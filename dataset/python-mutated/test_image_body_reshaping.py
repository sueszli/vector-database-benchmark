...
class ImageBodyReshapingTest(unittest.TestCase):
    ...
    def pipeline_inference(self, pipeline: Pipeline, input_location: str):
        ...
        if result is not None:
            cv2.imwrite('result_bodyreshaping.png', result[OutputKeys.OUTPUT_IMG])
            print(f"Output written to {osp.abspath('result_body_reshaping.png')}")
        else:
    ...
    def test_run_by_direct_model_download(self):
        ...
        image_body_reshaping = pipeline(Tasks.image_body_reshaping, model=model_dir)
    ...
    def test_run_modelhub(self):
        if False:
            i = 10
            return i + 15
        image_body_reshaping = pipeline(Tasks.image_body_reshaping, model=self.model_id)
        self.pipeline_inference(image_body_reshaping, self.test_image)
