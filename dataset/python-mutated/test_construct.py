...
@testing.parameterize(*testing.product({'dtype': [numpy.float32, numpy.float64, numpy.complex64, numpy.complex128], 'format': ['csr', 'csc', 'coo'], 'm': [3], 'n': [None, 3, 2], 'k': [0, 1]}))
...
@testing.parameterize(*testing.product({'dtype': [numpy.float32, numpy.float64, numpy.complex64, numpy.complex128], 'format': ['csr', 'csc', 'coo']}))
@testing.with_requires('scipy')
class TestIdentity:
...
@testing.parameterize(*testing.product({'random_method': ['random', 'rand'], 'dtype': [numpy.float32, numpy.float64], 'format': ['csr', 'csc', 'coo']}))
class TestRandom:
...
@testing.parameterize(*testing.product({'dtype': [numpy.float32, numpy.float64, numpy.complex64, numpy.complex128], 'format': ['dia', 'csr', 'csc', 'coo']}))
@testing.with_requires('scipy')
class TestDiags:
    ...
    def test_diags_non_square(self, xp, sp):
        ...
        assert x.format == self.format
        return x
_arrs_kron = [[[0]], [[-1]], [[4]], [[10]], [[0], [0]], [[0, 0]], [[1, 2], [3, 4]], [[0, 2], [5, 0]], [[0, 2, -6], [8, 0, 14]], [[5, 4], [0, 0], [6, 0]], [[5, 4, 4], [1, 0, 0], [6, 0, 8]], [[0, 1, 0, 2, 0, 5, 8]], [[0.5, 0.125, 0, 3.25], [0, 2.5, 0, 0]]]
...
@testing.parameterize(*testing.product({'dtype': (numpy.float32, numpy.float64, numpy.complex64, numpy.complex128), 'format': ('csr', 'csc', 'coo'), 'arrA': _arrs_kron, 'arrB': _arrs_kron}))
...
class TestKron:
    ...
    def test_kron(self, xp, sp):
        ...
        return kron
_arrs_kronsum = [[[0]], [[-1]], [[4]], [[10]], [[1, 2], [3, 4]], [[0, 2], [5, 0]], [[0, 2, -6], [8, 0, 14], [0, 3, 0]], [[5, 4, 4], [1, 0, 0], [6, 0, 8]]]

@testing.parameterize(*testing.product({'dtype': (numpy.float32, numpy.float64, numpy.complex64, numpy.complex128), 'format': ('csr', 'csc', 'coo'), 'arrA': _arrs_kronsum, 'arrB': _arrs_kronsum}))
@testing.with_requires('scipy>=1.6')
class TestKronsum:
