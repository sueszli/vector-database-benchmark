...
GRADLE_PROJECT_RE_PATTERN = 'project\\(([\'\\"])(.+?)\\1\\)'
TEST_GRADLE_DEPENDENCIES = ['testImplementation', 'testCompileOnly', 'integrationTestJavaImplementation', 'performanceTestJavaImplementation', 'testFixturesCompileOnly', 'testFixturesImplementation']
...
STRATEGIC_CONNECTOR_THRESHOLDS = {'sl': 200, 'ql': 400}
ALLOWED_HOST_THRESHOLD = {'ql': 300}
...
def get_changed_file(file_name: str, diff_regex: Optional[str]=None) -> Set[str]:
    ...
    changed_acceptance_test_config_paths = {file_path for file_path in airbyte_repo.git.diff(*diff_command_args).split('\n') if file_path.startswith(SOURCE_CONNECTOR_PATH_PREFIX) and file_path.endswith(file_name)}
...
def has_local_cdk_ref(build_file: Path) -> bool:
    ...
    contents = '\n'.join([line.split('//')[0] for line in build_file.read_text().split('\n')])
...
def parse_gradle_dependencies(build_file: Path) -> Tuple[List[Path], List[Path]]:
    ...
    matches = re.findall('(compileOnly|testCompileOnly|testFixturesCompileOnly|testFixturesImplementation|testImplementation|integrationTestJavaImplementation|performanceTestJavaImplementation|implementation|api).*?project\\([\'\\"](.*?)[\'\\"]\\)', dependencies_block)
    if matches:
...
def get_all_gradle_dependencies(build_file: Path, with_test_dependencies: bool=True, found_dependencies: Optional[List[Path]]=None) -> List[Path]:
    if False:
        while True:
            i = 10
...
class Connector:
    ...
    def has_airbyte_docs(self) -> bool:
        if False:
            i = 10
            return i + 15
        return self.metadata and self.metadata.get('documentationUrl') is not None and (BASE_AIRBYTE_DOCS_URL in str(self.metadata.get('documentationUrl')))
    ...
    def get_local_dependency_paths(self, with_test_dependencies: bool=True) -> Set[Path]:
        ...
        if self.language == ConnectorLanguage.JAVA:
            dependencies_paths += get_all_gradle_dependencies(self.code_directory / 'build.gradle', with_test_dependencies=with_test_dependencies)
        return sorted(list(set(dependencies_paths)))

def get_changed_connectors(modified_files: Optional[Set[Union[str, Path]]]=None, source: bool=True, destination: bool=True, third_party: bool=True) -> Set[Connector]:
    if False:
        print('Hello World!')
    ...
    changed_source_connector_files = {file_path for file_path in modified_files if any((file_path.startswith(prefix) for prefix in prefix_to_check)) and SCAFFOLD_CONNECTOR_GLOB not in file_path}
...
def get_all_connectors_in_repo() -> Set[Connector]:
    ...
    return {Connector(_get_relative_connector_folder_name_from_metadata_path(metadata_file)) for metadata_file in glob(f'{repo_path}/{CONNECTOR_PATH_PREFIX}/**/metadata.yaml', recursive=True) if SCAFFOLD_CONNECTOR_GLOB not in metadata_file}
