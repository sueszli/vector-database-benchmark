...
import numpy as np
from AnyQt.QtWidgets import QGraphicsView, QGraphicsScene, QGraphicsItem, QGraphicsSimpleTextItem, QGraphicsTextItem, QGraphicsItemGroup, QGraphicsLineItem, QGraphicsPathItem, QGraphicsRectItem, QSizePolicy
...
import Orange.data
from Orange.data.filter import FilterDiscrete, FilterContinuous, Values, IsDefined
...
class OWBoxPlot(widget.OWWidget):
    ...
    class Warning(widget.OWWidget.Warning):
        no_vars = widget.Msg('Data contains no categorical or numeric variables')
    ...
    _sorting_criteria_attrs = {CompareNone: '', CompareMedians: 'median', CompareMeans: 'mean'}
    _pen_axis_tick = QPen(Qt.white, 5)
    _pen_axis = QPen(Qt.darkGray, 3)
    _pen_median = QPen(QBrush(QColor(255, 255, 0)), 2)
    _pen_paramet = QPen(QBrush(QColor(51, 0, 255)), 2)
    _pen_dotted = QPen(QBrush(QColor(51, 0, 255)), 1)
    _pen_dotted.setStyle(Qt.DotLine)
    _post_line_pen = QPen(Qt.lightGray, 2)
    _post_grp_pen = QPen(Qt.lightGray, 4)
    for pen in (_pen_paramet, _pen_median, _pen_dotted, _pen_axis, _pen_axis_tick, _post_line_pen, _post_grp_pen):
        pen.setCosmetic(True)
        pen.setCapStyle(Qt.RoundCap)
        pen.setJoinStyle(Qt.RoundJoin)
    _pen_axis_tick.setCapStyle(Qt.FlatCap)
    _box_brush = QBrush(QColor(51, 136, 255, 192))
    _attr_brush = QBrush(QColor(51, 0, 255))
