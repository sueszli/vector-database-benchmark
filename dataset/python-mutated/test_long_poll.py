...
from ray.serve._private.long_poll import LongPollClient, LongPollHost, LongPollNamespace, LongPollState, UpdatedObject
from ray.serve.generated.serve_pb2 import ActorNameList, EndpointSet, LongPollRequest, LongPollResult
...
async def test_client_callbacks(serve_instance):
    ...
    _ = LongPollClient(host, {'key_1': key_1_callback, 'key_2': key_2_callback}, call_in_event_loop=get_or_create_event_loop())
    await async_wait_for_condition(lambda : callback_results == {'key_1': 100, 'key_2': 999}, timeout=1)
...
async def test_client_threadsafe(serve_instance):
    ...
    def key_1_callback(_):
        if False:
            ...
            return i + 15
        e.set()
    _ = LongPollClient(host, {'key_1': key_1_callback}, call_in_event_loop=get_or_create_event_loop())
    await e.wait()
