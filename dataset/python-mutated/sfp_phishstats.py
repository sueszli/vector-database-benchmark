...
class sfp_phishstats(SpiderFootPlugin):
    meta = {'name': 'PhishStats', 'summary': 'Check if a netblock or IP address is malicious according to PhishStats.', 'flags': [], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'website': 'https://phishstats.info/', 'model': 'FREE_NOAUTH_UNLIMITED', 'references': ['https://phishstats.info/#apidoc'], 'favIcon': 'https://phishstats.info/phish.ico', 'description': 'PhishStats is a real time Phishing database that gathers phishing URLs from several sources.'}}
    opts = {'checkaffiliates': True, 'netblocklookup': True, 'maxnetblock': 24, 'subnetlookup': True, 'maxsubnet': 24}
    optdescs = {'checkaffiliates': 'Apply checks to affiliates?', 'netblocklookup': 'Look up all IPs on netblocks deemed to be owned by your target for possible blacklisted hosts on the same target subdomain/domain?', 'maxnetblock': 'If looking up owned netblocks, the maximum netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'subnetlookup': 'Look up all IPs on subnets which your target is a part of for blacklisting?', 'maxsubnet': 'If looking up subnets, the maximum subnet size to look up all the IPs within (CIDR value, 24 = /24, 16 = /16, etc.)'}
    results = None
    errorState = False
