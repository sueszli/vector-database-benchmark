...
class DockerForMacCommandHandler(DockerCommandHandler):
    ...
    PROCESSES = dict(app='Docker', driver='com.docker.driver', hyperkit='com.docker.hyperkit', vpnkit='com.docker.vpnkit')
    ...
    def wait_until_stopped(cls) -> None:
        ...
        while any(map(cls.pid, cls.PROCESSES.values())):
            if time.time() - started >= cls.TIMEOUT:
                ...
                return
            time.sleep(0.5)
    commands: CommandDict = dict(start=lambda *_: DockerForMacCommandHandler.start(), stop=lambda *_: DockerForMacCommandHandler.stop(), status=lambda *_: DockerForMacCommandHandler.status())
