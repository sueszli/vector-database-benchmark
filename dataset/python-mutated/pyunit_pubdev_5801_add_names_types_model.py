...
def algo_max_runtime_secs():
    ...
    model = H2OGeneralizedLinearEstimator(family='binomial', alpha=1.0, lambda_search=False, max_iterations=2, seed=seed)
    checkColumnNamesTypesReturned(training1_data, model, ['displacement', 'power', 'weight', 'acceleration', 'year'], y_index='economy_20mpg')
    print('Checking GLRM.....')
...
def checkColumnNamesTypesReturned(training_data, model, x_indices, y_index=0):
    ...
    pyunit_utils.assertModelColNamesTypesCorrect(model._model_json['output']['names'], model._model_json['output']['column_types'], trainNames, trainTypes)
