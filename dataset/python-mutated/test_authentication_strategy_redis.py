...
@pytest.mark.authentication
@pytest.mark.asyncio
async def test_write_token(redis_strategy: RedisStrategy[UserModel, IDType], redis: RedisMock, user):
...
@pytest.mark.authentication
@pytest.mark.asyncio
async def test_destroy_token(redis_strategy: RedisStrategy[UserModel, IDType], redis: RedisMock, user):
    await redis.set(f'{redis_strategy.key_prefix}TOKEN', str(user.id))
