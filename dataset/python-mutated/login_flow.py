...
from homeassistant.components.http.ban import log_invalid_auth, process_success_login, process_wrong_login
...
class WellKnownOAuthInfoView(HomeAssistantView):
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        return self.json({'authorization_endpoint': '/auth/authorize', 'token_endpoint': '/auth/token', 'revocation_endpoint': '/auth/revoke', 'response_types_supported': ['code'], 'service_documentation': 'https://developers.home-assistant.io/docs/auth_api'})
...
class AuthProvidersView(HomeAssistantView):
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        if not onboarding.async_is_user_onboarded(hass):
            return self.json_message(message='Onboarding not finished', status_code=HTTPStatus.BAD_REQUEST, message_code='onboarding_required')
        return self.json([{'name': provider.name, 'id': provider.id, 'type': provider.type} for provider in hass.auth.auth_providers])
...
class LoginFlowBaseView(HomeAssistantView):
    ...
    async def _async_flow_result_to_response(self, request: web.Request, client_id: str, result: data_entry_flow.FlowResult) -> web.Response:
        ...
        if result['type'] != data_entry_flow.FlowResultType.CREATE_ENTRY:
            if result['type'] == data_entry_flow.FlowResultType.FORM and (errors := result.get('errors')) and (errors.get('base') in ('invalid_auth', 'invalid_code')):
                await process_wrong_login(request)
            return self.json(_prepare_result_json(result))
        hass: HomeAssistant = request.app['hass']
        if not await indieauth.verify_redirect_uri(hass, client_id, result['context']['redirect_uri']):
        ...
        if user is not None and (user_access_error := async_user_not_allowed_do_auth(hass, user)):
            return self.json_message(f'Login blocked: {user_access_error}', HTTPStatus.FORBIDDEN)
...
class LoginFlowIndexView(LoginFlowBaseView):
    ...
    async def post(self, request: web.Request, data: dict[str, Any]) -> web.Response:
        ...
        try:
            result = await self._flow_mgr.async_init(handler, context={'ip_address': ip_address(request.remote), 'credential_only': data.get('type') == 'link_user', 'redirect_uri': redirect_uri})
