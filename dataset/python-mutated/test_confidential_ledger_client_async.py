...
from devtools_testutils import PemCertificate, create_combined_bundle, is_live, is_live_and_not_recording, recorded_by_proxy, set_function_recording_options
...
from _shared.constants import TEST_PROXY_CERT, USER_CERTIFICATE_THUMBPRINT, USER_CERTIFICATE_PRIVATE_KEY, USER_CERTIFICATE_PUBLIC_KEY
...
class TestConfidentialLedgerClient(ConfidentialLedgerTestCase):
    ...
    async def create_confidentialledger_client(self, endpoint, ledger_id, is_aad):
        ...
        if not is_aad:
            try:
                await aad_based_client.create_or_update_user(USER_CERTIFICATE_THUMBPRINT, {'assignedRole': 'Administrator'})
            finally:
    ...
    async def test_append_entry_flow_aad_user(self, **kwargs):
        ...
        client = await self.create_confidentialledger_client(confidentialledger_endpoint, confidentialledger_id, is_aad=True)
        try:
    ...
    async def test_append_entry_flow_cert_user(self, **kwargs):
        confidentialledger_endpoint = kwargs.pop('confidentialledger_endpoint')
        confidentialledger_id = kwargs.pop('confidentialledger_id')
        client = await self.create_confidentialledger_client(confidentialledger_endpoint, confidentialledger_id, is_aad=False)
        try:
            await self.append_entry_flow_actions(client)
        finally:
            await client.close()
    ...
    @ConfidentialLedgerPreparer()
    @recorded_by_proxy_async
    async def test_append_entry_flow_with_collection_id_aad_user(self, **kwargs):
        confidentialledger_endpoint = kwargs.pop('confidentialledger_endpoint')
        confidentialledger_id = kwargs.pop('confidentialledger_id')
        client = await self.create_confidentialledger_client(confidentialledger_endpoint, confidentialledger_id, is_aad=True)
        try:
    ...
    @ConfidentialLedgerPreparer()
    @recorded_by_proxy_async
    async def test_append_entry_flow_with_collection_id_cert_user(self, **kwargs):
        confidentialledger_endpoint = kwargs.pop('confidentialledger_endpoint')
        confidentialledger_id = kwargs.pop('confidentialledger_id')
        client = await self.create_confidentialledger_client(confidentialledger_endpoint, confidentialledger_id, is_aad=False)
        try:
    ...
    async def append_entry_flow_with_collection_id_actions(self, client):
        ...
        append_result = await client.create_ledger_entry({'contents': entry_contents}, collection_id=collection_id)
        assert append_result['transactionId']
        ...
        assert latest_entry['contents'] == entry_contents
        assert latest_entry['collectionId'] == append_result_sub_ledger_id
        poller = await client.begin_create_ledger_entry({'contents': f'Test entry 2 from Python SDK. Collection: {collection_id}'}, collection_id=collection_id)
        await poller.wait()
        latest_entry = await client.get_current_ledger_entry(collection_id=collection_id)
        assert latest_entry['transactionId'] != append_result_transaction_id
        assert latest_entry['contents'] != entry_contents
        assert latest_entry['collectionId'] == collection_id
        poller = await client.begin_get_ledger_entry(transaction_id=append_result_transaction_id, collection_id=collection_id)
        original_entry = await poller.result()
        assert original_entry['entry']['transactionId'] == append_result_transaction_id
        assert original_entry['entry']['contents'] == entry_contents
        assert original_entry['entry']['collectionId'] == append_result_sub_ledger_id
