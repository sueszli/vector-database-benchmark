...
class Backend(dns._asyncbackend.Backend):
    ...
    async def make_socket(self, af, socktype, proto=0, source=None, destination=None, timeout=None, ssl_context=None, server_hostname=None):
        if destination is None and socktype == socket.SOCK_DGRAM and _is_win32:
            raise NotImplementedError('destinationless datagram sockets are not supported by asyncio on Windows')
        loop = _get_running_loop()
        if socktype == socket.SOCK_DGRAM:
            (transport, protocol) = await loop.create_datagram_endpoint(_DatagramProtocol, source, family=af, proto=proto, remote_addr=destination)
            return DatagramSocket(af, transport, protocol)
        elif socktype == socket.SOCK_STREAM:
            if destination is None:
                raise ValueError('destination required for stream sockets')
            (r, w) = await _maybe_wait_for(asyncio.open_connection(destination[0], destination[1], ssl=ssl_context, family=af, proto=proto, local_addr=source, server_hostname=server_hostname), timeout)
