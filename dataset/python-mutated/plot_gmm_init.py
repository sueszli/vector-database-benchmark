...
print(__doc__)
(X, y_true) = make_blobs(n_samples=4000, centers=4, cluster_std=0.6, random_state=0)
X = X[:, ::-1]
n_samples = 4000
n_components = 4
...
def get_initial_means(X, init_params, r):
    if False:
        while True:
            i = 10
    gmm = GaussianMixture(n_components=4, init_params=init_params, tol=1e-09, max_iter=0, random_state=r).fit(X)
    return gmm.means_
...
for (n, method) in enumerate(methods):
    ...
    init_time = end - start
    gmm = GaussianMixture(n_components=4, means_init=ini, tol=1e-09, max_iter=2000, random_state=r).fit(X)
