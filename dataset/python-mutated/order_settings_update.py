...
class OrderSettingsUpdateInput(BaseInputObjectType):
    automatically_confirm_all_new_orders = graphene.Boolean(required=False, description='When disabled, all new orders from checkout will be marked as unconfirmed. When enabled orders from checkout will become unfulfilled immediately. By default set to True')
    automatically_fulfill_non_shippable_gift_card = graphene.Boolean(required=False, description='When enabled, all non-shippable gift card orders will be fulfilled automatically. By default set to True.')
...
class OrderSettingsUpdate(BaseMutation):
    ...
    class Arguments:
        input = OrderSettingsUpdateInput(required=True, description='Fields required to update shop order settings.')
