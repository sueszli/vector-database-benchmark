...
class eventarc_publishingCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'publish_channel_connection_events': ('channel_connection', 'events', 'text_events'), 'publish_events': ('channel', 'events', 'text_events')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=eventarc_publishingCallTransformer()):
    if False:
        return 10
