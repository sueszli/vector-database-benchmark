...
class MyTourClass(BaseCase):
    ...
    def test_google_tour(self):
        ...
        self.add_tour_step('Then click here to show it on the map.', '#searchbox-searchbutton', alignment='bottom')
        self.add_tour_step('Or click here to get driving directions.', 'button[aria-label="Directions"]', alignment='bottom', theme='dark')
        self.add_tour_step('Use this button to switch to Satellite view.', 'button[jsaction*="minimap.main;"]', alignment='right')
        self.add_tour_step('Click here to zoom in.', '#widget-zoom-in', alignment='left')
        self.add_tour_step('Or click here to zoom out.', '#widget-zoom-out', alignment='left', theme='light')
        if self.is_element_visible('button[jsaction*="settings.open;"]'):
            self.add_tour_step('Use the Menu button to see more options.', 'button[jsaction*="settings.open;"]', alignment='right')
        elif self.is_element_visible('button[jsaction="navigationrail.more"]'):
            self.add_tour_step('Use the Menu button to see more options.', 'button[jsaction="navigationrail.more"]', alignment='right')
        self.add_tour_step('Or click here to see more Google apps.', '[title="Google apps"]', alignment='left')
        self.add_tour_step('Thanks for using SeleniumBase Tours!', title='End of Guided Tour', theme='light')
