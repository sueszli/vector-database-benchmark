...
class ModelPipe(PipelineLayer):
    ...
    def __init__(self, topology, transformer_layer_num: int=8):
        ...
        super().__init__(layers=self.descs, loss_fn=CriterionPipe(), topology=topology, num_virtual_pipeline_stages=num_virtual_pipeline_stages, seg_method='layer:TransformerNetPipe')
...
class TestDistPPTraining(unittest.TestCase):
    ...
    def setUp(self):
        ...
        strategy.hybrid_configs = {'dp_degree': self.data_parallel_size, 'mp_degree': self.model_parallel_size, 'pp_degree': self.pipeline_parallel_size, 'pp_configs': {'enable_timer': True}}
