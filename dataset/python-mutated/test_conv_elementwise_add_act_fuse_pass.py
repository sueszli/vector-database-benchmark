...
class TestConvElementwiseAddActPass(PassAutoScanTest):
    ...
    def sample_program_config(self, draw):
        ...
        program_config = ProgramConfig(ops=ops, weights={'filter': TensorConfig(shape=f_shape), 'bias': TensorConfig(shape=bias_shape)}, inputs={'input_x': TensorConfig(shape=x_shape), 'residualdata': TensorConfig(shape=res_shape)}, outputs=ops[-1].outputs['Out'])
    ...
    def test(self):
        if False:
            i = 10
            return i + 15
        self.run_and_statis(quant=False, max_examples=400, passes=['conv_elementwise_add_act_fuse_pass'])
