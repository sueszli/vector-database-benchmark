...
class KeyboardKbdd(base.ThreadPoolText):
    ...
    """
    defaults = [('update_interval', 1, 'Update interval in seconds.'), ('configured_keyboards', ['us', 'ir'], "your predefined list of keyboard layouts.example: ['us', 'ir', 'es']"), ('colours', None, "foreground colour for each layouteither 'None' or a list of colours.example: ['ffffff', 'E6F0AF']. ")]
    ...
    async def _config_async(self):
        subscribed = await add_signal_receiver(self._signal_received, session_bus=True, signal_name='layoutChanged', dbus_interface='ru.gentoo.kbdd')
        if not subscribed:
