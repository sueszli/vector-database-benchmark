...
class SkylineWebcamsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        stream_url = self._search_regex('(?:url|source)\\s*:\\s*(["\\\'])(?P<url>(?:https?:)?//.+?\\.m3u8.*?)\\1', webpage, 'stream url', group='url')
        title = self._og_search_title(webpage)
        description = self._og_search_description(webpage)
        return {'id': video_id, 'url': stream_url, 'ext': 'mp4', 'title': title, 'description': description, 'is_live': True}