...
from dns.query import BadResponse, NoDOH, NoDOQ, UDPMode, _compute_times, _have_http2, _matches_destination, _remaining, have_doh, ssl
if have_doh:
    import httpx
...
async def receive_udp(sock: dns.asyncbackend.DatagramSocket, destination: Optional[Any]=None, expiration: Optional[float]=None, ignore_unexpected: bool=False, one_rr_per_rrset: bool=False, keyring: Optional[Dict[dns.name.Name, dns.tsig.Key]]=None, request_mac: Optional[bytes]=b'', ignore_trailing: bool=False, raise_on_truncation: bool=False) -> Any:
    ...
    *sock*, a ``dns.asyncbackend.DatagramSocket``.
    ...
    while 1:
        ...
        if _matches_destination(sock.family, from_address, destination, ignore_unexpected):
            break
    received_time = time.time()
    r = dns.message.from_wire(wire, keyring=keyring, request_mac=request_mac, one_rr_per_rrset=one_rr_per_rrset, ignore_trailing=ignore_trailing, raise_on_truncation=raise_on_truncation)
    return (r, received_time, from_address)

async def udp(q: dns.message.Message, where: str, timeout: Optional[float]=None, port: int=53, source: Optional[str]=None, source_port: int=0, ignore_unexpected: bool=False, one_rr_per_rrset: bool=False, ignore_trailing: bool=False, raise_on_truncation: bool=False, sock: Optional[dns.asyncbackend.DatagramSocket]=None, backend: Optional[dns.asyncbackend.Backend]=None) -> dns.message.Message:
...
async def udp_with_fallback(q: dns.message.Message, where: str, timeout: Optional[float]=None, port: int=53, source: Optional[str]=None, source_port: int=0, ignore_unexpected: bool=False, one_rr_per_rrset: bool=False, ignore_trailing: bool=False, udp_sock: Optional[dns.asyncbackend.DatagramSocket]=None, tcp_sock: Optional[dns.asyncbackend.StreamSocket]=None, backend: Optional[dns.asyncbackend.Backend]=None) -> Tuple[dns.message.Message, bool]:
    ...
    *backend*, a ``dns.asyncbackend.Backend``, or ``None``.  If ``None``,
    ...
    """
    try:
        response = await udp(q, where, timeout, port, source, source_port, ignore_unexpected, one_rr_per_rrset, ignore_trailing, True, udp_sock, backend)
        return (response, False)
    except dns.message.Truncated:
        response = await tcp(q, where, timeout, port, source, source_port, one_rr_per_rrset, ignore_trailing, tcp_sock, backend)
...
async def tls(q: dns.message.Message, where: str, timeout: Optional[float]=None, port: int=853, source: Optional[str]=None, source_port: int=0, one_rr_per_rrset: bool=False, ignore_trailing: bool=False, sock: Optional[dns.asyncbackend.StreamSocket]=None, backend: Optional[dns.asyncbackend.Backend]=None, ssl_context: Optional[ssl.SSLContext]=None, server_hostname: Optional[str]=None) -> dns.message.Message:
    ...
    *backend*, a ``dns.asyncbackend.Backend``, or ``None``.  If ``None``,
    ...
    if sock:
        ...
        if not backend:
            backend = dns.asyncbackend.get_default_backend()
        cm = await backend.make_socket(af, socket.SOCK_STREAM, 0, stuple, dtuple, timeout, ssl_context, server_hostname)
    async with cm as s:
        timeout = _timeout(expiration)
        response = await tcp(q, where, timeout, port, source, source_port, one_rr_per_rrset, ignore_trailing, s, backend)
        end_time = time.time()
...
async def https(q: dns.message.Message, where: str, timeout: Optional[float]=None, port: int=443, source: Optional[str]=None, source_port: int=0, one_rr_per_rrset: bool=False, ignore_trailing: bool=False, client: Optional['httpx.AsyncClient']=None, path: str='/dns-query', post: bool=True, verify: Union[bool, str]=True, bootstrap_address: Optional[str]=None, resolver: Optional['dns.asyncresolver.Resolver']=None, family: Optional[int]=socket.AF_UNSPEC) -> dns.message.Message:
    ...
    *client*, a ``httpx.AsyncClient``.  If provided, the client to use for
    ...
    transport = backend.get_transport_class()(local_address=local_address, http1=True, http2=_have_http2, verify=verify, local_port=local_port, bootstrap_address=bootstrap_address, resolver=resolver, family=family)
    if client:
        cm: contextlib.AbstractAsyncContextManager = NullContext(client)
    else:
        cm = httpx.AsyncClient(http1=True, http2=_have_http2, verify=verify, transport=transport)
    async with cm as the_client:
        if post:
            headers.update({'content-type': 'application/dns-message', 'content-length': str(len(wire))})
            response = await backend.wait_for(the_client.post(url, headers=headers, content=wire), timeout)
        else:
            wire = base64.urlsafe_b64encode(wire).rstrip(b'=')
            twire = wire.decode()
            response = await backend.wait_for(the_client.get(url, headers=headers, params={'dns': twire}), timeout)
    if response.status_code < 200 or response.status_code > 299:
        raise ValueError('{} responded with status code {}\nResponse body: {!r}'.format(where, response.status_code, response.content))
    r = dns.message.from_wire(response.content, keyring=q.keyring, request_mac=q.request_mac, one_rr_per_rrset=one_rr_per_rrset, ignore_trailing=ignore_trailing)
...
async def inbound_xfr(where: str, txn_manager: dns.transaction.TransactionManager, query: Optional[dns.message.Message]=None, port: int=53, timeout: Optional[float]=None, lifetime: Optional[float]=None, source: Optional[str]=None, source_port: int=0, udp_mode: UDPMode=UDPMode.NEVER, backend: Optional[dns.asyncbackend.Backend]=None) -> None:
    ...
    *backend*, a ``dns.asyncbackend.Backend``, or ``None``.  If ``None``,
    ...
    while retry:
        ...
        if not backend:
            backend = dns.asyncbackend.get_default_backend()
        s = await backend.make_socket(af, sock_type, 0, stuple, dtuple, _timeout(expiration))
        async with s:
            if is_udp:
                ...
                while not done:
                    ...
                    if is_udp:
                        ...
                        while True:
                            ...
                            if _matches_destination(af, from_address, destination, True):
                                break
                    else:
...
async def quic(q: dns.message.Message, where: str, timeout: Optional[float]=None, port: int=853, source: Optional[str]=None, source_port: int=0, one_rr_per_rrset: bool=False, ignore_trailing: bool=False, connection: Optional[dns.quic.AsyncQuicConnection]=None, verify: Union[bool, str]=True, backend: Optional[dns.asyncbackend.Backend]=None, server_hostname: Optional[str]=None) -> dns.message.Message:
    ...
    *backend*, a ``dns.asyncbackend.Backend``, or ``None``.  If ``None``,
    ...
    async with cfactory() as context:
        async with mfactory(context, verify_mode=verify, server_name=server_hostname) as the_manager:
            if not connection:
