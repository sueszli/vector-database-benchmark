...
class TestEinsumBF16Op(TestEinsumBinary):
    ...
    def test_check_output(self):
        if False:
            ...
            return i + 15
        if not self.disable:
            self.check_output_with_place(self.place, no_check_set=['InnerCache', 'XShape'])
    ...
    def test_grad(self):
        if False:
            while True:
                i = 10
        if not self.disable:
            self.check_grad_with_place(self.place, [op[0] for op in self.operands], ['Out'], numeric_grad_delta=0.05)
