import io
from electrum.lnmsg import read_bigsize_int, write_bigsize_int, FieldEncodingNotMinimal, UnexpectedEndOfStream, LNSerializer, UnknownMandatoryTLVRecordType, MalformedMsg, MsgTrailingGarbage, MsgInvalidFieldOrder, encode_msg, decode_msg, UnexpectedFieldSizeForEncoder, OnionWireSerializer, UnknownMsgType
...
class TestLNMsg(ElectrumTestCase):
    ...
    def test_read_tlv_stream_tests2(self):
        ...
        self.assertEqual({'tlv1': {'amount_msat': 0}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('0100')), tlv_stream_name='n1'))
        self.assertEqual({'tlv1': {'amount_msat': 1}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('010101')), tlv_stream_name='n1'))
        self.assertEqual({'tlv1': {'amount_msat': 256}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('01020100')), tlv_stream_name='n1'))
        self.assertEqual({'tlv1': {'amount_msat': 65536}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('0103010000')), tlv_stream_name='n1'))
        self.assertEqual({'tlv1': {'amount_msat': 16777216}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('010401000000')), tlv_stream_name='n1'))
        self.assertEqual({'tlv1': {'amount_msat': 4294967296}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('01050100000000')), tlv_stream_name='n1'))
        self.assertEqual({'tlv1': {'amount_msat': 1099511627776}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('0106010000000000')), tlv_stream_name='n1'))
        self.assertEqual({'tlv1': {'amount_msat': 281474976710656}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('010701000000000000')), tlv_stream_name='n1'))
        self.assertEqual({'tlv1': {'amount_msat': 72057594037927936}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('01080100000000000000')), tlv_stream_name='n1'))
        self.assertEqual({'tlv2': {'scid': ShortChannelID.from_components(0, 0, 550)}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('02080000000000000226')), tlv_stream_name='n1'))
        self.assertEqual({'tlv3': {'node_id': bfh('023da092f6980e58d2c037173180e9a465476026ee50f96695963e8efe436f54eb'), 'amount_msat_1': 1, 'amount_msat_2': 2}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('0331023da092f6980e58d2c037173180e9a465476026ee50f96695963e8efe436f54eb00000000000000010000000000000002')), tlv_stream_name='n1'))
        self.assertEqual({'tlv4': {'cltv_delta': 550}}, lnser.read_tlv_stream(fd=io.BytesIO(bfh('fd00fe020226')), tlv_stream_name='n1'))
    ...
    def test_encode_decode_msg__ints_can_be_passed_as_bytes(self):
        if False:
            while True:
                i = 10
        self.assertEqual(bfh(''), encode_msg('channel_update', short_channel_id=ShortChannelID.from_components(54321, 111, 2), channel_flags=b'\x00', message_flags=b'\x01', cltv_expiry_delta=int.to_bytes(144, length=2, byteorder='big', signed=False), htlc_minimum_msat=int.to_bytes(200, length=8, byteorder='big', signed=False), htlc_maximum_msat=int.to_bytes(1000000000, length=8, byteorder='big', signed=False), fee_base_msat=int.to_bytes(500, length=4, byteorder='big', signed=False), fee_proportional_millionths=int.to_bytes(35, length=4, byteorder='big', signed=False), chain_hash=constants.net.rev_genesis_bytes(), timestamp=int.to_bytes(1584320643, length=4, byteorder='big', signed=False)))
        self.assertEqual(('channel_update', {'chain_hash': b'CI\x7f\xd7\xf8&\x95q\x08\xf4\xa3\x0f\xd9\xce\xc3\xae\xbay\x97 \x84\xe9\x0e\xad\x01\xea3\t\x00\x00\x00\x00', 'channel_flags': b'\x00', 'cltv_expiry_delta': 144, 'fee_base_msat': 500, 'fee_proportional_millionths': 35, 'htlc_maximum_msat': 1000000000, 'htlc_minimum_msat': 200, 'message_flags': b'\x01', 'short_channel_id': b'\x00\xd41\x00\x00o\x00\x02', 'signature': bytes(64), 'timestamp': 1584320643}), decode_msg(bfh('')))
        with self.assertRaises(UnexpectedFieldSizeForEncoder):
            encode_msg('channel_update', short_channel_id=ShortChannelID.from_components(54321, 111, 2), channel_flags=b'\x00', message_flags=b'\x01', cltv_expiry_delta=int.to_bytes(144, length=2, byteorder='big', signed=False), htlc_minimum_msat=int.to_bytes(200, length=4, byteorder='big', signed=False), htlc_maximum_msat=int.to_bytes(1000000000, length=8, byteorder='big', signed=False), fee_base_msat=int.to_bytes(500, length=4, byteorder='big', signed=False), fee_proportional_millionths=int.to_bytes(35, length=4, byteorder='big', signed=False), chain_hash=constants.net.rev_genesis_bytes(), timestamp=int.to_bytes(1584320643, length=4, byteorder='big', signed=False))
    ...
    def test_encode_decode_msg__init(self):
        if False:
            return 10
        self.assertEqual(bfh('00100000000220c2'), encode_msg('init', gflen=0, flen=2, features=LnFeatures.OPTION_STATIC_REMOTEKEY_OPT | LnFeatures.GOSSIP_QUERIES_OPT | LnFeatures.GOSSIP_QUERIES_REQ | LnFeatures.OPTION_DATA_LOSS_PROTECT_OPT))
        self.assertEqual(bfh('00100000000220c2'), encode_msg('init', gflen=0, flen=2, features=bfh('20c2')))
        self.assertEqual(bfh('00100000000220c2012043497fd7f826957108f4a30fd9cec3aeba79972084e90ead01ea330900000000'), encode_msg('init', gflen=0, flen=2, features=LnFeatures.OPTION_STATIC_REMOTEKEY_OPT | LnFeatures.GOSSIP_QUERIES_OPT | LnFeatures.GOSSIP_QUERIES_REQ | LnFeatures.OPTION_DATA_LOSS_PROTECT_OPT, init_tlvs={'networks': {'chains': b'CI\x7f\xd7\xf8&\x95q\x08\xf4\xa3\x0f\xd9\xce\xc3\xae\xbay\x97 \x84\xe9\x0e\xad\x01\xea3\t\x00\x00\x00\x00'}}))
        self.assertEqual(('init', {'gflen': 2, 'globalfeatures': b'"\x00', 'flen': 3, 'features': b'\x02\xa2\xa1', 'init_tlvs': {}}), decode_msg(bfh('001000022200000302a2a1')))
    ...
    def test_decode_onion_error(self):
        ...
        self.assertEqual(('incorrect_or_unknown_payment_details', {'htlc_msat': 399694000, 'height': 1938520}), OnionWireSerializer.decode_msg(orf.to_bytes()))
