...
class TestChronosModelSeq2SeqForecaster(TestCase):
    ...
    def test_s2s_forecaster_fit_eva_pred(self):
        ...
        forecaster = Seq2SeqForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, loss='mae', lr=0.01)
    ...
    def test_s2s_forecaster_shape_error(self):
        if False:
            for i in range(10):
                print('nop')
        (train_data, val_data, test_data) = create_data()
        forecaster = Seq2SeqForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=2, loss='mae', lr=0.01)
        with pytest.raises(RuntimeError):
            forecaster.fit(train_data, epochs=2)
    ...
    def test_s2s_forecaster_xshard_input(self):
        ...
        for distributed in [True, False]:
            forecaster = Seq2SeqForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, loss='mae', lr=0.01, distributed=distributed)
            forecaster.fit(train_data, epochs=2)
    ...
    def test_s2s_forecaster_distributed(self):
        ...
        init_orca_context(cores=4, memory='2g')
        forecaster = Seq2SeqForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, loss='mae', lr=0.01, distributed=True)
        forecaster.fit(train_data, epochs=2)
    ...
    def test_s2s_customized_loss_metric(self):
        ...
        forecaster = Seq2SeqForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, loss=loss, metrics=[customized_metric], lr=0.01)
        forecaster.fit(train_data, epochs=2)
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ckpt_name = os.path.join(tmp_dir_name, 'ckpt')
            test_pred_save = forecaster.predict(test_data[0], acceleration=False)
    ...
    def test_forecaster_from_tsdataset(self):
        ...
        s2s = Seq2SeqForecaster.from_tsdataset(train, lstm_hidden_dim=16, lstm_layer_num=1)
        s2s.fit(train, epochs=2, batch_size=32)
        yhat = s2s.predict(test, batch_size=32, acceleration=False)
        test.roll(lookback=s2s.data_config['past_seq_len'], horizon=s2s.data_config['future_seq_len'])
        (_, y_test) = test.to_numpy()
        assert yhat.shape == y_test.shape
        del s2s
        (train, test) = create_tsdataset(roll=False, horizon=[1, 3, 5])
        s2s = Seq2SeqForecaster.from_tsdataset(train, past_seq_len=24, future_seq_len=2, lstm_hidden_dim=16, lstm_layer_num=1)
        s2s.fit(train, epochs=2, batch_size=32)
    ...
    def test_s2s_forecaster_tune_fit(self):
        ...
        forecaster = Seq2SeqForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, loss='mae', lstm_hidden_dim=space.Categorical(32, 64, 128), lr=space.Real(0.001, 0.1, log=True))
    ...
    def test_predict_interval(self):
        if False:
            ...
            return i + 15
        (train_data, val_data, test_data) = create_data()
        forecaster = Seq2SeqForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, loss='mse', metrics=['mse'], lr=0.01)
        forecaster.fit(train_data, epochs=2)
    ...
    def test_forecaster_fit_val_from_tsdataset(self):
        ...
        s2s = Seq2SeqForecaster.from_tsdataset(train, lstm_hidden_dim=16, lstm_layer_num=1)
        s2s.fit(train, val, epochs=2, batch_size=32)
        yhat = s2s.predict(test, batch_size=32, acceleration=False)
        test.roll(lookback=s2s.data_config['past_seq_len'], horizon=s2s.data_config['future_seq_len'])
        (_, y_test) = test.to_numpy()
        assert yhat.shape == y_test.shape
    ...
    def test_s2s_forecaster_eval_shuffle_loader(self):
        ...
        (train_data, val_data, test_data) = create_data()
        forecaster = Seq2SeqForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, loss='mse', lr=0.01)
        forecaster.fit(train_data, epochs=2)
    ...
    def test_s2s_forecaster_numpy_inference_tsdataset(self):
        ...
        forecaster = Seq2SeqForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=2, output_feature_num=2, loss='mse', lr=0.01)
