...
class Test_s3_bucket_no_mfa_delete:
    ...
    def test_no_buckets(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_no_mfa_delete.s3_bucket_no_mfa_delete.s3_client', new=S3(audit_info)):
                from prowler.providers.aws.services.s3.s3_bucket_no_mfa_delete.s3_bucket_no_mfa_delete import s3_bucket_no_mfa_delete
    ...
    def test_bucket_without_mfa(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_no_mfa_delete.s3_bucket_no_mfa_delete.s3_client', new=S3(audit_info)):
                ...
                assert search('MFA Delete disabled', result[0].status_extended)
    ...
    def test_bucket_with_mfa(self):
        ...
        bucket_name_us = 'bucket_test_us'
        s3_client_us_east_1.create_bucket(Bucket=bucket_name_us)
        s3_client_us_east_1.put_bucket_versioning(Bucket=bucket_name_us, VersioningConfiguration={'MFADelete': 'Enabled', 'Status': 'Enabled'})
        from prowler.providers.aws.services.s3.s3_service import S3
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_no_mfa_delete.s3_bucket_no_mfa_delete.s3_client', new=S3(audit_info)) as service_client:
                from prowler.providers.aws.services.s3.s3_bucket_no_mfa_delete.s3_bucket_no_mfa_delete import s3_bucket_no_mfa_delete
                ...
                assert search('MFA Delete enabled', result[0].status_extended)
                assert result[0].resource_id == bucket_name_us
                assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'