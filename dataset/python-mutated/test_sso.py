...
async def mock_get_file(url: str, output_stream: BinaryIO, max_size: Optional[int]=None, headers: Optional[RawHeaders]=None, is_allowed_content_type: Optional[Callable[[str], bool]]=None) -> Tuple[int, Dict[bytes, List[bytes]], str, int]:
    fake_response = FakeResponse(code=404)
    if url == 'http://my.server/me.png':
        fake_response = FakeResponse(code=200, headers=Headers({'Content-Type': ['image/png'], 'Content-Length': [str(len(SMALL_PNG))]}), body=SMALL_PNG)
    if max_size is not None and max_size < len(SMALL_PNG):
        raise SynapseError(HTTPStatus.BAD_GATEWAY, 'Requested file is too large > %r bytes' % (max_size,), Codes.TOO_LARGE)
    if is_allowed_content_type and (not is_allowed_content_type('image/png')):
        raise SynapseError(HTTPStatus.BAD_GATEWAY, "Requested file's content type not allowed for this operation: %s" % 'image/png')
