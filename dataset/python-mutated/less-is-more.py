...
_ax1_title = RangeDict({range(1, 2): 'Less is More', range(2, 5): 'Remove Backgrounds', range(5, 10): 'Remove redundant labels', range(10, 14): 'Remove borders', range(14, 16): 'Reduce colors', range(16, 19): 'Remove special effects', range(19, 22): 'Remove bolding', range(22, 25): 'Lighten labels', range(25, 29): 'Ligthen lines', range(29, 33): 'Or remove lines', range(33, 35): 'Direct label', range(35, 36): 'Less is More'})
_bar_facecolors = RangeDict({range(1, 15): ['#22befc', '#1dB95d', '#ca5a56', '#8b6eaa', '#fcfc38'], range(15, 36): ['#b2b2b2', '#b2b2b2', '#cb5958', '#b2b2b2', '#b2b2b2']})
_bar_edgecolor = RangeDict({range(1, 13): '#000000', range(13, 36): 'None'})
_bar_shadows = RangeDict({range(1, 18): True, range(18, 36): False})
_xticklabels = RangeDict({range(1, 36): ['French\nFries', 'Potato\nChips', 'Bacon\n', 'Pizza\n', 'Chili Dog\n']})
_xticklines_color = RangeDict({range(1, 27): '#000000', range(27, 31): '#b2b2b2', range(34, 36): 'None'})
_yticklabels = RangeDict({range(1, 34): ['0', '100', '200', '300', '400', '500', '600', '700'], range(34, 36): ['', '', '', '', '', '', '', '']})
_yticklabels_weight = RangeDict({range(1, 21): 'bold', range(21, 36): 'normal'})
_yticklabels_color = RangeDict({range(1, 24): '#000000', range(24, 34): '#b2b2b2', range(34, 36): 'None'})
...
_ax3_title = RangeDict({range(1, 7): 'Calories per 100g for different foods', range(7, 36): 'Calories per 100g'})
...
_ax2_spines_linewidth = RangeDict({range(1, 11): (2, 2, 2, 2), range(11, 36): (0, 0, 0, 0)})
_ax3_spines_linewidth = RangeDict({range(1, 12): (2, 2, 2, 2), range(12, 31): (0, 0.75, 0.75, 0), range(31, 32): (0, 0, 0.75, 0), range(32, 36): (0, 0, 0, 0)})
_ax3_spines_color = RangeDict({range(1, 27): ('k', 'k', 'k', 'k'), range(27, 28): ('k', '#b2b2b2', 'k', 'k'), range(28, 36): ('k', '#b2b2b2', '#b2b2b2', 'k')})
...
ax1.text(0.05, 0.05, 'A remake of www.darkhorseanalytics.com', ha='left', va='top', family='Source Sans Pro', weight='light')
ax1.text(0.95, 0.05, 'Made with matplotlib', ha='right', va='top', family='Source Sans Pro')
(x, y) = (0.05, 0.075)
(w, h) = (1 - 2 * x, 1 - 2 * y - 0.1)
...
ax1.text(0.05, 0.95, ax1_title, ha='left', va='top', family='Source Sans Pro', fontsize=32, weight='light')
if ax2_background:
...
for (color, linewidth, axis) in zip(ax3_spines_color, ax3_spines_linewidth, ['top', 'bottom', 'left', 'right']):
...
ax3.set_title(ax3_title, weight=ax3_title_weight, size=12, loc='left', color=ax3_title_color)
...
if direct_label:
    for (x, y) in zip(X, Y):
        ax3.text(x, y - 25, '%d' % y, ha='center', va='top', color='white', zorder=20, transform=ax3.transData, fontsize=9)
if shadow:
