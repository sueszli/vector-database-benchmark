...
from typing import Any, Callable, Dict, FrozenSet, Iterable, List, Literal, Mapping, MutableMapping, Optional, Sequence, Set, Tuple, Type, TypedDict, Union, cast
from snuba_sdk import BooleanCondition, Column, Condition, Direction, Function, Granularity, Limit, Offset, Op
from snuba_sdk.conditions import ConditionGroup
from sentry.api.utils import InvalidParams as UtilsInvalidParams
from sentry.models.project import Project
from sentry.models.release import Release
from sentry.release_health.base import GroupByFieldName, ProjectId, SessionsQueryFunction, SessionsQueryGroup, SessionsQueryResult, SessionsQueryValue
...
from sentry.snuba.sessions_v2 import InvalidParams, NonPreflightOrderByException, QueryDefinition, finite_or_none, get_timestamps, isoformat_z
logger = logging.getLogger(__name__)
Scalar = Union[int, float, None]
GroupKeyDict = TypedDict('GroupKeyDict', {'project': int, 'release': str, 'environment': str, 'session.status': str}, total=False)
...
class SumSessionField(CountField):
    status_to_metric_field = {SessionStatus.HEALTHY: MetricField(None, SessionMRI.HEALTHY.value), SessionStatus.ABNORMAL: MetricField(None, SessionMRI.ABNORMAL.value), SessionStatus.CRASHED: MetricField(None, SessionMRI.CRASHED.value), SessionStatus.ERRORED: MetricField(None, SessionMRI.ERRORED.value), None: MetricField(None, SessionMRI.ALL.value)}
...
class CountUniqueUser(CountField):
    ...
    status_to_metric_field = {SessionStatus.HEALTHY: MetricField(None, SessionMRI.HEALTHY_USER.value), SessionStatus.ABNORMAL: MetricField(None, SessionMRI.ABNORMAL_USER.value), SessionStatus.CRASHED: MetricField(None, SessionMRI.CRASHED_USER.value), SessionStatus.ERRORED: MetricField(None, SessionMRI.ERRORED_USER.value), None: MetricField(None, SessionMRI.ALL_USER.value)}
...
class SimpleForwardingField(Field):
    ...
    """
    field_name_to_metric_name = {'crash_rate(session)': SessionMRI.CRASH_RATE, 'crash_rate(user)': SessionMRI.CRASH_USER_RATE, 'crash_free_rate(session)': SessionMRI.CRASH_FREE_RATE, 'crash_free_rate(user)': SessionMRI.CRASH_FREE_USER_RATE, 'anr_rate()': SessionMRI.ANR_RATE, 'foreground_anr_rate()': SessionMRI.FOREGROUND_ANR_RATE}
...
FIELD_MAP: Mapping[SessionsQueryFunction, Type[Field]] = {'sum(session)': SumSessionField, 'count_unique(user)': CountUniqueUser, 'avg(session.duration)': DurationField, 'p50(session.duration)': DurationField, 'p75(session.duration)': DurationField, 'p90(session.duration)': DurationField, 'p95(session.duration)': DurationField, 'p99(session.duration)': DurationField, 'max(session.duration)': DurationField, 'crash_rate(session)': SimpleForwardingField, 'crash_rate(user)': SimpleForwardingField, 'crash_free_rate(session)': SimpleForwardingField, 'crash_free_rate(user)': SimpleForwardingField, 'anr_rate()': SimpleForwardingField, 'foreground_anr_rate()': SimpleForwardingField}
