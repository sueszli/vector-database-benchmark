...
class CoursevaniaScraper(BaseScraper):
    ...
    async def run(self) -> List:
        ...
        logger.info(f'Page: {self.current_page} of {self.last_page} scraped from coursevania.com')
            ...
            if response is not None:
                ...
                for script_element in soup.find_all('script'):
                    if 'var stm_lms_nonces' in str(script_element):
                        data = json.loads(script_element.string.split(' = ')[1].strip()[:-1])
    ...
    async def get_course_links(self) -> List:
        ...
        """
        query_params = {'offset': self.current_page - 1, 'template': 'courses/grid', 'args': '{"image_d":"img-480-380","per_row":"4","posts_per_page":"12","class":"archive_grid"}', 'action': 'stm_lms_load_content', 'sort': 'date_high', 'nonce': self._nonce}
        headers = {'Host': 'coursevania.com', 'User-Agent': 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:85.0) Gecko/20100101 Firefox/85.0', 'Accept': 'application/json, text/javascript, */*; q=0.01', 'Accept-Language': 'en-US,en;q=0.5', 'Accept-Encoding': 'gzip, deflate, br', 'X-Requested-With': 'XMLHttpRequest', 'Connection': 'keep-alive', 'Referer': 'https://coursevania.com/courses/', 'TE': 'Trailers'}
        query_string = urlencode(query_params)
        response = await http_get(f'{self.DOMAIN}/wp-admin/admin-ajax.php?{query_string}', headers=headers)
        if response is not None:
    ...
    async def get_udemy_course_link(url: str) -> str:
        ...
        if text is not None:
            ...
            udemy_link = soup.find('div', class_='stm-lms-buy-buttons').find('a').get('href')
            return udemy_link
