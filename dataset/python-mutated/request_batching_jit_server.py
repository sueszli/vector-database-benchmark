...
class ModelRunner:
    ...
    async def process_input(self, input):
        our_task = {'done_event': asyncio.Event(loop=app.loop), 'input': input, 'time': app.loop.time()}
...
async def image(request):
    try:
        ...
        return sanic.response.raw(imgByteArr.getvalue(), status=200, content_type='image/jpeg')
