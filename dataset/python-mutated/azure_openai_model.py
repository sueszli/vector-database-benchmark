...
FUNCTION_CALL_MODELS = ['gpt-4', 'gpt-4-32k', 'gpt-35-turbo', 'gpt-35-turbo-16k']
...
class AzureOpenAIModel(BaseLLM):
    ...
    def _init_client(self) -> Any:
        ...
        if self.name == 'text-davinci-003':
            client = EnhanceAzureOpenAI(deployment_name=self.name, streaming=self.streaming, request_timeout=60, openai_api_type='azure', openai_api_version=AZURE_OPENAI_API_VERSION, openai_api_key=self.credentials.get('openai_api_key'), openai_api_base=self.credentials.get('openai_api_base'), callbacks=self.callbacks, **provider_model_kwargs)
        else:
