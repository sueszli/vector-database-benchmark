...
from torch.ao.quantization.backend_config import BackendConfig, BackendPatternConfig, DTypeConfig, DTypeWithConstraints, ObservationType
...
class TestBackendConfig(QuantizationTestCase):
    dtype_config1 = DTypeConfig(input_dtype=torch.quint8, output_dtype=torch.quint8, weight_dtype=torch.qint8, bias_dtype=torch.float)
    dtype_config2 = DTypeConfig(input_dtype=torch.float16, output_dtype=torch.float, is_dynamic=True)
    activation_dtype_with_constraints = DTypeWithConstraints(dtype=torch.quint8, quant_min_lower_bound=0, quant_max_upper_bound=127, scale_min_lower_bound=2 ** (-12))
    weight_dtype_with_constraints = DTypeWithConstraints(dtype=torch.qint8, quant_min_lower_bound=-128, quant_max_upper_bound=127, scale_min_lower_bound=2 ** (-12))
    dtype_config3 = DTypeConfig(input_dtype=activation_dtype_with_constraints, output_dtype=activation_dtype_with_constraints, weight_dtype=weight_dtype_with_constraints)
    dtype_config_dict1_legacy = {'input_dtype': torch.quint8, 'output_dtype': torch.quint8, 'weight_dtype': torch.qint8, 'bias_dtype': torch.float}
    dtype_config_dict2_legacy = {'input_dtype': torch.float16, 'output_dtype': torch.float, 'is_dynamic': True}
    dtype_config_dict1 = {'input_dtype': DTypeWithConstraints(dtype=torch.quint8), 'output_dtype': DTypeWithConstraints(torch.quint8), 'weight_dtype': DTypeWithConstraints(torch.qint8), 'bias_dtype': torch.float}
    dtype_config_dict2 = {'input_dtype': DTypeWithConstraints(dtype=torch.float16), 'output_dtype': DTypeWithConstraints(dtype=torch.float), 'is_dynamic': True}
    dtype_config_dict3 = {'input_dtype': activation_dtype_with_constraints, 'output_dtype': activation_dtype_with_constraints, 'weight_dtype': weight_dtype_with_constraints}
    ...
    _num_tensor_args_to_observation_type = {0: ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT, 1: ObservationType.OUTPUT_SHARE_OBSERVER_WITH_INPUT, 2: ObservationType.OUTPUT_USE_DIFFERENT_OBSERVER_AS_INPUT}
    _input_type_to_index = {'bias': 0, 'input': 1, 'weight': 2}
