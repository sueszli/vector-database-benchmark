...
from ...taxes.utils import create_tax_class, get_tax_configurations, update_country_tax_rates, update_tax_configuration
from ...utils import assign_permissions
from ..utils import draft_order_complete, draft_order_create, draft_order_update, order_lines_create

def prepare_tax_configuration(e2e_staff_api_client, channel_slug, country_code, country_tax_rate, product_tax_rate, prices_entered_with_tax):
    if False:
        print('Hello World!')
    ...
    tax_class_data = create_tax_class(e2e_staff_api_client, 'Product tax class', country_rates)
    tax_class_id = tax_class_data['id']
    return (country_tax_rate, product_tax_rate, tax_class_id)

@pytest.mark.e2e
def test_order_calculate_simple_tax_based_on_product_tax_class_CORE_2006(e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes, permission_manage_shipping, permission_manage_taxes, permission_manage_orders):
    if False:
        while True:
            i = 10
    ...
    (country_tax_rate, product_tax_rate, tax_class_id) = prepare_tax_configuration(e2e_staff_api_client, channel_slug, country_code='US', country_tax_rate=20, product_tax_rate=13, prices_entered_with_tax=False)
    variant_price = '1234'
    ...
    calculated_tax = round(product_variant_price * (product_tax_rate / 100), 2)
    assert order_data['total']['net']['amount'] == product_variant_price
    assert order_data['total']['tax']['amount'] == calculated_tax
    assert order_data['total']['gross']['amount'] == round(product_variant_price + calculated_tax, 2)
    ...
    assert order_data['shippingPrice']['gross']['amount'] == round(shipping_price + shipping_tax, 2)
    ...
    assert order_data['total']['gross']['amount'] == round(calculated_net + total_tax, 2)