...
class TarPartitionLister(object):
    ...
    def __iter__(self):
        ...
        for key in bucket.list_blobs(prefix='/' + prefix):
            url = 'gs://{bucket}/{name}'.format(bucket=key.bucket.name, name=key.name)
            key_last_part = key.name.rsplit('/', 1)[-1]
            match = re.match(storage.VOLUME_REGEXP, key_last_part)
            if match is None:
                logger.warning(msg='unexpected object found in tar volume directory', detail='The unexpected key is stored at "{0}".'.format(url), hint=generic_weird_key_hint_message)
            else:
                yield key_last_part
...
class BackupFetcher(object):
    ...
    def fetch_partition(self, partition_name):
        ...
        logger.info(msg='beginning partition download', detail='The partition being downloaded is {0}.'.format(partition_name), hint='The absolute GCS object is {0}.'.format(part_abs_name))
