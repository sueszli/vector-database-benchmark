...
with DAG(DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), catchup=False, tags=['example', 'gcs', 'gdrive']) as dag:
    ...
    upload_file = LocalFilesystemToGCSOperator(task_id='upload_file', src=LOCAL_PATH, dst=FILE_NAME, bucket=BUCKET_NAME)
    copy_single_file = GCSToGoogleDriveOperator(task_id='copy_single_file', gcp_conn_id=CONNECTION_ID, source_bucket=BUCKET_NAME, source_object=FILE_NAME, destination_object=DRIVE_FILE_NAME)
    detect_file = GoogleDriveFileExistenceSensor(task_id='detect_file', folder_id=FOLDER_ID, file_name=DRIVE_FILE_NAME, gcp_conn_id=CONNECTION_ID)
    upload_gdrive_to_gcs = GoogleDriveToGCSOperator(task_id='upload_gdrive_object_to_gcs', gcp_conn_id=CONNECTION_ID, folder_id=FOLDER_ID, file_name=DRIVE_FILE_NAME, bucket_name=BUCKET_NAME, object_name=OBJECT)
    ...
    [create_bucket >> upload_file >> copy_single_file, create_temp_gcp_connection_task] >> detect_file >> upload_gdrive_to_gcs >> remove_files_from_drive_task >> [delete_bucket, delete_temp_gcp_connection_task]
