...
import aiohttp
import discord
from .errors import APIError, OfflineStream, InvalidTwitchCredentials, InvalidYoutubeCredentials, StreamNotFound, YoutubeQuotaExceeded
...
class YoutubeStream(Stream):
    ...
    async def is_online(self):
        ...
        for video_id in get_video_ids_from_feed(rssdata):
            ...
            params = {'key': self._token['api_key'], 'id': video_id, 'part': 'id,liveStreamingDetails'}
            async with aiohttp.ClientSession() as session:
                async with session.get(YOUTUBE_VIDEOS_ENDPOINT, params=params) as r:
                    ...
                    try:
                        ...
                        log.error("Something went wrong whilst trying to contact the stream service's API.\nRaw response data:\n%r", e)
                        continue
                    ...
                    if stream_data and stream_data != 'None' and (stream_data.get('actualEndTime', None) is None):
        ...
        if self.livestreams:
            params = {'key': self._token['api_key'], 'id': self.livestreams[-1], 'part': 'snippet,liveStreamingDetails'}
            async with aiohttp.ClientSession() as session:
                async with session.get(YOUTUBE_VIDEOS_ENDPOINT, params=params) as r:
                    data = await r.json()
        ...
        if vid_data['liveStreamingDetails'].get('scheduledStartTime', None) is not None:
            if 'actualStartTime' not in vid_data['liveStreamingDetails']:
                ...
                if start_in.total_seconds() > 0:
                    embed.description = _('This stream will start in {time}').format(time=humanize_timedelta(timedelta=timedelta(minutes=start_in.total_seconds() // 60)))
                else:
                    embed.description = _('This stream was scheduled for {min} minutes ago').format(min=round(start_in.total_seconds() * -1 // 60))
                ...
                self.messages = [data for data in self.messages if data['message'] not in to_remove]
        embed.set_author(name=channel_title)
        embed.set_image(url=rnd(thumbnail))
        embed.colour = 9590181
    ...
    async def _fetch_channel_resource(self, resource: str):
        ...
        elif 'pageInfo' in data and 'totalResults' in data['pageInfo'] and (data['pageInfo']['totalResults'] < 1):
            raise StreamNotFound()
...
class TwitchStream(Stream):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(url, headers=header, params=params, timeout=60) as resp:
                    ...
                    if resp.status == 429:
                        log.info('Ratelimited. Trying again at %s.', datetime.fromtimestamp(int(reset)))
                        resp.release()
    ...
    async def is_online(self):
        ...
        (stream_code, stream_data) = await self.get_data(TWITCH_STREAMS_ENDPOINT, {'user_id': self.id})
        if stream_code == 200:
            if not stream_data['data']:
                raise OfflineStream()
            if user_profile_data is None:
                user_profile_data = await self._fetch_user_profile()
            final_data = dict.fromkeys(('game_name', 'followers', 'login', 'profile_image_url', 'view_count'))
            ...
            (__, follows_data) = await self.get_data(TWITCH_FOLLOWS_ENDPOINT, {'broadcaster_id': self.id})
            if follows_data:
