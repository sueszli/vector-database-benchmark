...
async def test_set_store_name(memory_store: MemoryStore) -> None:
    ...
    def handler(request: Request) -> None:
        ...
        request.set_session({'foo': 'bar'})
        return
    app = Litestar([handler], middleware=[ServerSideSessionConfig(store='some_store').middleware], stores={'some_store': memory_store})
    with TestClient(app) as client:
        res = client.get('/')
        assert res.status_code == 200
        assert await memory_store.exists(res.cookies['session'])

async def test_get_set(server_side_session_backend: 'ServerSideSessionBackend', session_data: bytes, memory_store: MemoryStore) -> None:
...
async def test_get_renew_on_access(server_side_session_backend: 'ServerSideSessionBackend', session_data: bytes, memory_store: MemoryStore, frozen_datetime: 'Coordinates') -> None:
...
async def test_get_set_multiple_returns_correct_identity(server_side_session_backend: 'ServerSideSessionBackend', memory_store: MemoryStore) -> None:
...
async def test_delete_idempotence(server_side_session_backend: 'ServerSideSessionBackend', session_data: bytes, memory_store: MemoryStore) -> None:
    await server_side_session_backend.set('foo', session_data, memory_store)
    await server_side_session_backend.delete('foo', memory_store)
    await server_side_session_backend.delete('foo', memory_store)

async def test_max_age_expires(server_side_session_backend: 'ServerSideSessionBackend', session_data: bytes, memory_store: MemoryStore, frozen_datetime: 'Coordinates') -> None:
    server_side_session_backend.config.max_age = 1
    await server_side_session_backend.set('foo', session_data, memory_store)
