...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        while not page or len(results) == PER_PAGE:
            page += 1
            response = await client.get(f'{GITHUB_API_CONTRIBUTORS}?per_page={PER_PAGE}&page={page}', auth=auth)
            results = response.json()
            contributors.extend((contributor for contributor in results if contributor['type'] == GITHUB_USER_TYPE and contributor['login'] not in IGNORED_AUTHOR_LOGINS and (f"@{contributor['login'].lower()}" not in ACKNOWLEDGEMENTS)))
        unacknowledged_users = await asyncio.gather(*(client.get(contributor['url'], auth=auth) for contributor in contributors))
