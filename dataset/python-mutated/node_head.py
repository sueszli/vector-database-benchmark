...
from ray.autoscaler._private.util import LoadMetricsSummary, get_per_node_breakdown_as_dict, parse_usage
...
from ray.dashboard.modules.node.node_consts import FREQUENTY_UPDATE_NODES_INTERVAL_SECONDS, FREQUENT_UPDATE_TIMEOUT_SECONDS
...
class NodeHead(dashboard_utils.DashboardHeadModule):
        ...
        while True:
            try:
                ...
                for node in nodes.values():
                    ...
                    if ip == self._dashboard_head.ip and (not self._head_node_registration_time_s):
                        self._head_node_registration_time_s = time.time() - self._module_start_time
                        await self._gcs_aio_client.internal_kv_put('head_node_id'.encode(), node_id.encode(), overwrite=True, namespace=ray_constants.KV_NAMESPACE_JOB, timeout=2)
                ...
                for node_id in alive_node_ids:
                    if node_id not in agents:
                        ...
                        agent_port = await self._gcs_aio_client.internal_kv_get(key.encode(), namespace=ray_constants.KV_NAMESPACE_DASHBOARD)
                        if agent_port:
                ...
                if head_node_not_registered and self._node_update_cnt * FREQUENTY_UPDATE_NODES_INTERVAL_SECONDS < FREQUENT_UPDATE_TIMEOUT_SECONDS:
                    await asyncio.sleep(FREQUENTY_UPDATE_NODES_INTERVAL_SECONDS)
                else:
                    if head_node_not_registered:
                        logger.warning(f'Head node is not registered even after {FREQUENT_UPDATE_TIMEOUT_SECONDS} seconds. The API server might not work correctly. Please report a Github issue. Internal states :{self.get_internal_states()}')
    ...
    async def get_node_module_internal_state(self, req) -> aiohttp.web.Response:
        return dashboard_optional_utils.rest_response(success=True, message='', **self.get_internal_states())
        ...
        if is_autoscaler_v2():
            ...
            for node in chain(cluster_status.active_nodes, cluster_status.idle_nodes):
                if not node.resource_usage:
                    continue
                usage_dict = {r.resource_name: (r.used, r.total) for r in node.resource_usage.usage}
                per_node_resources[node.node_id] = '\n'.join(parse_usage(usage_dict, verbose=True))
            return per_node_resources
        (status_string, error) = await asyncio.gather(*[self._gcs_aio_client.internal_kv_get(key.encode(), namespace=None, timeout=GCS_RPC_TIMEOUT_SECONDS) for key in [DEBUG_AUTOSCALING_STATUS, DEBUG_AUTOSCALING_ERROR]])
    ...
    async def get_all_nodes(self, req) -> aiohttp.web.Response:
        ...
        if view == 'summary':
            ...
            (all_node_summary, nodes_logical_resources) = await asyncio.gather(all_node_summary_task, nodes_logical_resource_task)
            return dashboard_optional_utils.rest_response(success=True, message='Node summary fetched.', summary=all_node_summary, node_logical_resources=nodes_logical_resources)
            ...
            return dashboard_optional_utils.rest_response(success=True, message='Node hostname list fetched.', host_name_list=list(alive_hostnames))
        else:
            return dashboard_optional_utils.rest_response(success=False, message=f'Unknown view {view}')
    ...
    async def get_node(self, req) -> aiohttp.web.Response:
        ...
        return dashboard_optional_utils.rest_response(success=True, message='Node details fetched.', detail=node_info)
    ...
    async def _update_node_stats(self):
        ...
        for (node_id, stub) in nodes:
            ...
            if node_info['state'] != 'ALIVE':
                continue
            get_node_stats_tasks.append(stub.GetNodeStats(node_manager_pb2.GetNodeStatsRequest(include_memory_info=self._collect_memory_info), timeout=min(2, TIMEOUT)))
        replies = await asyncio.gather(*get_node_stats_tasks, return_exceptions=True)
        for (node_info, reply) in zip(nodes, replies):
            ...
            if isinstance(reply, asyncio.CancelledError):
                ...
                if reply.code() == grpc.StatusCode.DEADLINE_EXCEEDED:
                    logger.exception(f'Cannot reach the node, {node_id}, after timeout {TIMEOUT}. This node may have been overloaded, terminated, or the network is slow.')
                elif reply.code() == grpc.StatusCode.UNAVAILABLE:
                    logger.exception(f'Cannot reach the node, {node_id}. The node may have been terminated.')
                else:
    ...
    async def run(self, server):
        ...
        self._gcs_node_resource_info_stub = gcs_service_pb2_grpc.NodeResourceInfoGcsServiceStub(gcs_channel)
