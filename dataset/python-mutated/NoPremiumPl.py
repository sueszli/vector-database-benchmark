...
class NoPremiumPl(MultiDownloader):
    ...
    __version__ = '0.13'
    __status__ = 'testing'
    __pattern__ = 'https?://direct\\.nopremium\\.pl.+'
    ...
    API_QUERY = {'site': 'nopremium', 'output': 'json', 'username': '', 'password': '', 'url': ''}
    ERROR_CODES = {0: 'Incorrect login credentials', 1: 'Not enough transfer to download - top-up your account', 2: 'Incorrect / dead link', 3: 'Error connecting to hosting, try again later', 9: 'Premium account has expired', 15: 'Hosting no longer supported', 80: 'Too many incorrect login attempts, account blocked for 24h'}
    ...
    def handle_premium(self, pyfile):
        ...
        if 'sdownload' in parsed:
            if parsed['sdownload'] == '1':
                self.fail(self._('Download from {} is possible only using NoPremium.pl website  directly').format(parsed['hosting']))
