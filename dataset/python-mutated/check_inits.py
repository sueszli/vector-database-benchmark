...
_re_one_line_import_struct = re.compile('^_import_structure\\s+=\\s+\\{([^\\}]+)\\}')
_re_import_struct_key_value = re.compile('\\s+"\\S*":\\s+\\[([^\\]]*)\\]')
_re_test_backend = re.compile('^\\s*if\\s+not\\s+is\\_[a-z_]*\\_available\\(\\)')
_re_import_struct_add_one = re.compile('^\\s*_import_structure\\["\\S*"\\]\\.append\\("(\\S*)"\\)')
_re_import_struct_add_many = re.compile('^\\s*_import_structure\\[\\S*\\](?:\\.extend\\(|\\s*=\\s+)\\[([^\\]]*)\\]')
_re_quote_object = re.compile('^\\s+"([^"]+)",')
_re_between_brackets = re.compile('^\\s+\\[([^\\]]+)\\]')
_re_import = re.compile('\\s+from\\s+\\S*\\s+import\\s+([^\\(\\s].*)\\n')
_re_try = re.compile('^\\s*try:')
_re_else = re.compile('^\\s*else:')
    ...
    for (path, directories, files) in os.walk(PATH_TO_TRANSFORMERS):
        ...
        for fname in files:
            ...
            if len(submodule.split('.')) == 1:
                submodules.append(submodule)
    return submodules
IGNORE_SUBMODULES = ['convert_pytorch_checkpoint_to_tf2', 'modeling_flax_pytorch_utils', 'models.esm.openfold_utils', 'modeling_attn_mask_utils']
