...
async def resolve_question_by_name(question: dns.Question, loop: asyncio.AbstractEventLoop, family: socket.AddressFamily, ip: Callable[[str], ipaddress.IPv4Address | ipaddress.IPv6Address]) -> Iterable[dns.ResourceRecord]:
    try:
    ...
    return map(lambda addrinfo: dns.ResourceRecord(name=question.name, type=question.type, class_=question.class_, ttl=dns.ResourceRecord.DEFAULT_TTL, data=ip(addrinfo[4][0]).packed), addrinfos)

async def resolve_question_by_addr(question: dns.Question, loop: asyncio.AbstractEventLoop, suffix: str, sockaddr: Callable[[list[str]], tuple[str, int] | tuple[str, int, int, int]]) -> Iterable[dns.ResourceRecord]:
    try:
    ...
    return [dns.ResourceRecord(name=question.name, type=question.type, class_=question.class_, ttl=dns.ResourceRecord.DEFAULT_TTL, data=dns.domain_names.pack(name))]
...
async def resolve_question(question: dns.Question, loop: asyncio.AbstractEventLoop) -> Iterable[dns.ResourceRecord]:
    ...
    if question.type == dns.types.A:
        ...
        if name_lower.endswith(IP4_PTR_SUFFIX):
            return await resolve_question_by_addr(question=question, loop=loop, suffix=IP4_PTR_SUFFIX, sockaddr=lambda x: (str(ipaddress.IPv4Address('.'.join(x))), 0))
        elif name_lower.endswith(IP6_PTR_SUFFIX):
            return await resolve_question_by_addr(question=question, loop=loop, suffix=IP6_PTR_SUFFIX, sockaddr=lambda x: (str(ipaddress.IPv6Address(bytes.fromhex(''.join(x)))), 0, 0, 0))
        else:
            raise ResolveError(dns.response_codes.FORMERR)
    else:
        raise ResolveError(dns.response_codes.NOTIMP)
...
async def resolve_message(message: dns.Message, loop: asyncio.AbstractEventLoop) -> dns.Message:
    try:
        if not message.query:
            raise ResolveError(dns.response_codes.REFUSED)
...
class DnsResolver:
    ...
    async def dns_request(self, flow: dns.DNSFlow) -> None:
        should_resolve = (isinstance(flow.client_conn.proxy_mode, mode_specs.DnsMode) or (isinstance(flow.client_conn.proxy_mode, mode_specs.WireGuardMode) and flow.server_conn.address == ('10.0.0.53', 53))) and flow.live and (not flow.response) and (not flow.error)
        if should_resolve:
