...
def print_provider(doc: Documentor, provider: BaseProvider, formatters: Dict[str, T], excludes: Optional[List[str]]=None, output: Optional[TextIO]=None) -> None:
    ...
    for (signature, example) in formatters.items():
        ...
        try:
            lines = textwrap.wrap(str(example).expandtabs(), width=150 - margin, initial_indent='# ', subsequent_indent='  ')
...
def print_doc(provider_or_field: Optional[str]=None, args: Optional[List[T]]=None, lang: str=DEFAULT_LOCALE, output: Optional[Union[TextIO, TextIOWrapper]]=None, seed: Optional[float]=None, includes: Optional[List[str]]=None) -> None:
    if False:
        print('Hello World!')
    if args is None:
        args = []
...
class Command:
    ...
    def execute(self) -> None:
        ...
        parser = argparse.ArgumentParser(prog=self.prog_name, description=f'{self.prog_name} version {VERSION}', epilog=epilog, formatter_class=formatter_class)
