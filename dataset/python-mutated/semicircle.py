...
class SemiCircle(JSCSSMixin, Marker):
    ...
    **kwargs
    ...
    def __init__(self, location, radius, direction=None, arc=None, start_angle=None, stop_angle=None, popup=None, tooltip=None, **kwargs):
        if False:
            for i in range(10):
                print('nop')
        ...
        self.options.update(parse_options(start_angle=start_angle, stop_angle=stop_angle))
        if not ((direction is None and arc is None) and (start_angle is not None and stop_angle is not None) or ((direction is not None and arc is not None) and (start_angle is None and stop_angle is None))):
