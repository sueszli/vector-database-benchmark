...
from sanic.exceptions import RequestCancelled, RequestTimeout, ServiceUnavailable
...
class HttpProtocol(HttpProtocolMixin, SanicProtocol, metaclass=TouchUpMeta):
    ...
    """
    HTTP_CLASS = Http
    __touchup__ = ('send', 'connection_task')
    __version__ = HTTP.VERSION_1
    __slots__ = ('request', 'request_handler', 'request_timeout', 'response_timeout', 'keep_alive_timeout', 'request_max_size', 'request_class', 'error_handler', 'access_log', 'state', 'url', '_handler_task', '_http', '_exception', 'recv_buffer')
    ...
    async def connection_task(self):
        ...
        try:
            ...
            await self.app.dispatch('http.lifecycle.begin', inline=True, context={'conn_info': self.conn_info})
            ...
            if self.app.debug and self._http and self.transport and (not self._http.upgrade_websocket):
                ip = self.transport.get_extra_info('peername')
                error_logger.error(f'Connection lost before response written @ {ip} {self._http.request}')
            self._http = None
            self._task = None
            try:
                self.close()
                ...
                await self.app.dispatch('http.lifecycle.complete', inline=True, context={'conn_info': self.conn_info})
                ...
    ...
    async def send(self, data):
        ...
        await self.app.dispatch('http.lifecycle.send', inline=True, context={'data': data})
...
class Http3Protocol(HttpProtocolMixin, ConnectionProtocol):
    HTTP_CLASS = Http3
