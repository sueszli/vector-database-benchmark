...
class ConditionalDetrModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (ConditionalDetrModel, ConditionalDetrForObjectDetection, ConditionalDetrForSegmentation) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': ConditionalDetrModel, 'object-detection': ConditionalDetrForObjectDetection} if is_torch_available() else {}
...
class ConditionalDetrModelIntegrationTests(unittest.TestCase):
    ...
    def default_image_processor(self):
        if False:
            return 10
        return ConditionalDetrImageProcessor.from_pretrained('microsoft/conditional-detr-resnet-50') if is_vision_available() else None
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor([[0.4222, 0.7471, 0.876], [0.6395, -0.2729, 0.7127], [-0.309, 0.7642, 0.9529]]).to(torch_device)
    ...
    def test_inference_object_detection_head(self):
        ...
        expected_slice_logits = torch.tensor([[-10.4372, -5.7558, -8.6764], [-10.541, -5.8704, -8.059], [-10.6827, -6.3469, -8.3923]]).to(torch_device)
        self.assertTrue(torch.allclose(outputs.logits[0, :3, :3], expected_slice_logits, atol=0.0001))
        expected_shape_boxes = torch.Size((1, model.config.num_queries, 4))
        self.assertEqual(outputs.pred_boxes.shape, expected_shape_boxes)
        expected_slice_boxes = torch.tensor([[0.7733, 0.6576, 0.4496], [0.5171, 0.1184, 0.9094], [0.8846, 0.5647, 0.2486]]).to(torch_device)
        ...
        expected_scores = torch.tensor([0.833, 0.8313, 0.8039, 0.6829, 0.5355]).to(torch_device)
