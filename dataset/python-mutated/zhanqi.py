...
STATUS_ONLINE = 4
STATUS_OFFLINE = 0
_room_schema = validate.Schema({'data': validate.any(None, {'status': validate.all(str, validate.transform(int)), 'videoId': str})}, validate.get('data'))

@pluginmatcher(re.compile('https?://(www\\.)?zhanqi\\.tv/(?P<channel>[^/]+)'))
