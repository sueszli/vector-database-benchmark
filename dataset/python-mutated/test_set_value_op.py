...
class TestSetValueItemSlice4(TestSetValueApi):
    ...
    def _call_setitem_static_api(self, x):
        if False:
            return 10
        x = paddle.static.setitem(x, (slice(0, None), slice(1, 2), slice(None)), self.value)
        return x
...
class TestSetValueItemSlice5(TestSetValueApi):
    ...
    def _call_setitem_static_api(self, x):
        if False:
            i = 10
            return i + 15
        x = paddle.static.setitem(x, (slice(0, None), slice(1, 1), slice(None)), self.value)
        return x
...
class TestSetValueItemSliceStep4(TestSetValueApi):
    ...
    def _call_setitem_static_api(self, x):
        if False:
            return 10
        x = paddle.static.setitem(x, (slice(0, None), slice(1, 2, 2), slice(None)), self.value)
        return x
...
class TestSetValueItemNone7(TestSetValueApi):
    ...
    def _call_setitem_static_api(self, x):
        if False:
            i = 10
            return i + 15
        x = paddle.static.setitem(x, (slice(None), None, 1), np.zeros(self.shape)[:, None, 0])
        return x
...
class TestSetValueItemNone8(TestSetValueApi):
    ...
    def _call_setitem_static_api(self, x):
        if False:
            for i in range(10):
                print('nop')
        x = paddle.static.setitem(x, (slice(None), 1, None), np.zeros(self.shape)[:, 0, None])
        return x
...
class TestSetValueItemNone9(TestSetValueApi):
    ...
    def _call_setitem_static_api(self, x):
        if False:
            for i in range(10):
                print('nop')
        x = paddle.static.setitem(x, (None, slice(None), 1, ..., None), np.zeros(self.shape)[0, 0, :, None])
        return x
...
class TestSetValueItemNone10(TestSetValueApi):
    ...
    def _call_setitem_static_api(self, x):
        if False:
            while True:
                i = 10
        x = paddle.static.setitem(x, (..., None, slice(None), None), np.zeros(self.shape)[..., None, :, None])
        return x
    ...
    class TestValueInt(parent):
        ...
        def set_value(self):
            if False:
                i = 10
                return i + 15
            self.value = complex(np.finfo(np.float64).max + 1j * np.finfo(np.float64).min)
...
def create_test_value_numpy_complex128(parent):
    ...
    class TestValueInt(parent):
        ...
        def set_value(self):
            if False:
                print('Hello World!')
            v = complex(np.finfo(np.float64).max + 1j * np.finfo(np.float64).min)
...
class TestSetValueBFloat16(OpTest):
    ...
    def setUp(self):
        ...
        self.attrs = {'axes': [0, 1], 'starts': [0, 0], 'ends': [1, 1], 'steps': [1, 1]}
        self.inputs = {'Input': convert_float_to_uint16(self.data), 'ValueTensor': convert_float_to_uint16(value)}
