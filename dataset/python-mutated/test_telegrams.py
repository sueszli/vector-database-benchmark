...
async def test_store_telegam_history(hass: HomeAssistant, knx: KNXTestKit, hass_storage: dict[str, Any]):
    """Test storing telegram history."""
    await knx.setup_integration({})
    await knx.receive_write('1/3/4', True)
    await hass.services.async_call('knx', 'send', {'address': '2/2/2', 'payload': [1, 2, 3, 4]}, blocking=True)
...
async def test_load_telegam_history(hass: HomeAssistant, knx: KNXTestKit, hass_storage: dict[str, Any]):
...
async def test_remove_telegam_history(hass: HomeAssistant, knx: KNXTestKit, hass_storage: dict[str, Any]):
    """Test telegram history removal when configured to size 0."""
    hass_storage['knx/telegrams_history.json'] = {'version': 1, 'data': MOCK_TELEGRAMS}
    knx.mock_config_entry.data = knx.mock_config_entry.data | {CONF_KNX_TELEGRAM_LOG_SIZE: 0}
    await knx.setup_integration({})
