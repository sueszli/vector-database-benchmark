...
from zipline.pipeline.loaders.blaze import from_blaze, BlazeLoader, NoMetaDataWarning
from zipline.pipeline.loaders.blaze.core import ExprData, NonPipelineField
from zipline.testing import ZiplineTestCase, parameter_space, tmp_asset_finder
from zipline.testing.fixtures import WithAssetFinder
from zipline.testing.predicates import assert_equal, assert_frame_equal, assert_isidentical
from zipline.utils.numpy_utils import float64_dtype, int64_dtype
from zipline.utils.pandas_utils import empty_dataframe, new_pandas, skip_pipeline_new_pandas
nameof = op.attrgetter('name')
dtypeof = op.attrgetter('dtype')
asset_infos = ((make_simple_equity_info(tuple(map(ord, 'ABC')), pd.Timestamp(0), pd.Timestamp('2015')),), (make_simple_equity_info(tuple(map(ord, 'ABCD')), pd.Timestamp(0), pd.Timestamp('2015')),))
