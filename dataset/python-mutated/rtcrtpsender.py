...
from .rtp import RTCP_PSFB_APP, RTCP_PSFB_PLI, RTCP_RTPFB_NACK, RTP_HISTORY_SIZE, AnyRtcpPacket, RtcpByePacket, RtcpPsfbPacket, RtcpRrPacket, RtcpRtpfbPacket, RtcpSdesPacket, RtcpSenderInfo, RtcpSourceInfo, RtcpSrPacket, RtpPacket, unpack_remb_fci, wrap_rtx
from .stats import RTCOutboundRtpStreamStats, RTCRemoteInboundRtpStreamStats, RTCStatsReport
...
class RTCRtpSender:
    ...
    async def getStats(self) -> RTCStatsReport:
        ...
        """
        self.__stats.add(RTCOutboundRtpStreamStats(timestamp=clock.current_datetime(), type='outbound-rtp', id='outbound-rtp_' + str(id(self)), ssrc=self._ssrc, kind=self.__kind, transportId=self.transport._stats_id, packetsSent=self.__packet_count, bytesSent=self.__octet_count, trackId=str(id(self.track))))
    ...
    async def _handle_rtcp_packet(self, packet):
        if isinstance(packet, (RtcpRrPacket, RtcpSrPacket)):
            for report in filter(lambda x: x.ssrc == self._ssrc, packet.reports):
                ...
                self.__stats.add(RTCRemoteInboundRtpStreamStats(timestamp=clock.current_datetime(), type='remote-inbound-rtp', id='remote-inbound-rtp_' + str(id(self)), ssrc=packet.ssrc, kind=self.__kind, transportId=self.transport._stats_id, packetsReceived=self.__packet_count - report.packets_lost, packetsLost=report.packets_lost, jitter=report.jitter, roundTripTime=self.__rtt, fractionLost=report.fraction_lost))
    ...
    async def _next_encoded_frame(self, codec: RTCRtpCodecParameters):
        ...
        if isinstance(data, Frame):
            ...
            self.__force_keyframe = False
            (payloads, timestamp) = await self.__loop.run_in_executor(None, self.__encoder.encode, data, force_keyframe)
        else:
    ...
    async def _retransmit(self, sequence_number: int) -> None:
        ...
        if packet and packet.sequence_number == sequence_number:
            if self.__rtx_payload_type is not None:
                packet = wrap_rtx(packet, payload_type=self.__rtx_payload_type, sequence_number=self.__rtx_sequence_number, ssrc=self._rtx_ssrc)
        ...
        try:
            while True:
                ...
                for (i, payload) in enumerate(enc_frame.payloads):
                    packet = RtpPacket(payload_type=codec.payloadType, sequence_number=sequence_number, timestamp=timestamp)
                    packet.ssrc = self._ssrc
                    packet.payload = payload
                    packet.marker = i == len(enc_frame.payloads) - 1 and 1 or 0
                    packet.extensions.abs_send_time = clock.current_ntp_time() >> 14 & 16777215
    ...
    async def _run_rtcp(self) -> None:
        ...
        try:
            while True:
                ...
                packets: List[AnyRtcpPacket] = [RtcpSrPacket(ssrc=self._ssrc, sender_info=RtcpSenderInfo(ntp_timestamp=self.__ntp_timestamp, rtp_timestamp=self.__rtp_timestamp, packet_count=self.__packet_count, octet_count=self.__octet_count))]
                self.__lsr = self.__ntp_timestamp >> 16 & 4294967295
                self.__lsr_time = time.time()
                if self.__cname is not None:
                    packets.append(RtcpSdesPacket(chunks=[RtcpSourceInfo(ssrc=self._ssrc, items=[(1, self.__cname.encode('utf8'))])]))
