...
class GlueCatalogHook(AwsBaseHook):
    ...
    async def async_get_partitions(self, client: Any, database_name: str, table_name: str, expression: str='', page_size: int | None=None, max_items: int | None=1) -> set[tuple]:
        """
        ...
        partitions = set()
        async for page in paginator.paginate(DatabaseName=database_name, TableName=table_name, Expression=expression, PaginationConfig=config):
