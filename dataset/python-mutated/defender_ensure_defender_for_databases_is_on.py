...
class defender_ensure_defender_for_databases_is_on(Check):
    ...
    def execute(self) -> Check_Report_Azure:
        ...
        for (subscription, pricings) in defender_client.pricings.items():
            if 'SqlServers' in pricings and 'SqlServerVirtualMachines' in pricings and ('OpenSourceRelationalDatabases' in pricings) and ('CosmosDbs' in pricings):
                ...
                if pricings['SqlServers'].pricing_tier != 'Standard' or pricings['SqlServerVirtualMachines'].pricing_tier != 'Standard' or pricings['OpenSourceRelationalDatabases'].pricing_tier != 'Standard' or (pricings['CosmosDbs'].pricing_tier != 'Standard'):
                    report.status = 'FAIL'
