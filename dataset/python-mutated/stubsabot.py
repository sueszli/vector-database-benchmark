...
async def get_diff_info(session: aiohttp.ClientSession, stub_info: StubInfo, pypi_version: packaging.version.Version) -> GithubDiffInfo | None:
...
async def analyze_diff(github_repo_path: str, stub_path: Path, old_tag: str, new_tag: str, *, session: aiohttp.ClientSession) -> DiffAnalysis | None:
...
async def determine_action(stub_path: Path, session: aiohttp.ClientSession) -> Update | NoUpdate | Obsolete:
    ...
    maybe_links: dict[str, str | None] = {'Release': f'{pypi_info.pypi_root}/{relevant_version}', 'Homepage': project_urls.get('Homepage'), 'Repository': stub_info.upstream_repository, 'Changelog': project_urls.get('Changelog') or project_urls.get('Changes') or project_urls.get('Change Log')}
    ...
    if obsolete_since:
        return Obsolete(stub_info.distribution, stub_path, obsolete_since_version=str(obsolete_since.version), obsolete_since_date=obsolete_since.upload_date, links=links)
    if diff_info is None:
        diff_analysis: DiffAnalysis | None = None
    else:
        diff_analysis = await analyze_diff(github_repo_path=diff_info.repo_path, stub_path=stub_path, old_tag=diff_info.old_tag, new_tag=diff_info.new_tag, session=session)
    return Update(distribution=stub_info.distribution, stub_path=stub_path, old_version_spec=stub_info.version_spec, new_version_spec=get_updated_version_spec(stub_info.version_spec, latest_version), links=links, diff_analysis=diff_analysis)
...
async def create_or_update_pull_request(*, title: str, body: str, branch_name: str, session: aiohttp.ClientSession) -> None:
    ...
    async with session.post(f'{TYPESHED_API_URL}/pulls', json={'title': title, 'body': body, 'head': f'{fork_owner}:{branch_name}', 'base': 'main'}, headers=get_github_api_headers()) as response:
        resp_json = await response.json()
        if response.status == HTTPStatus.CREATED:
            pr_number = resp_json['number']
            assert isinstance(pr_number, int)
        elif response.status == HTTPStatus.UNPROCESSABLE_ENTITY and any(('A pull request already exists' in e.get('message', '') for e in resp_json.get('errors', []))):
...
async def update_existing_pull_request(*, title: str, body: str, branch_name: str, session: aiohttp.ClientSession) -> int:
    fork_owner = get_origin_owner()
    async with session.get(f'{TYPESHED_API_URL}/pulls', params={'state': 'open', 'head': f'{fork_owner}:{branch_name}', 'base': 'main'}, headers=get_github_api_headers()) as response:
        response.raise_for_status()
        resp_json = await response.json()
        assert len(resp_json) >= 1
        pr_number = resp_json[0]['number']
        assert isinstance(pr_number, int)
    async with session.patch(f'{TYPESHED_API_URL}/pulls/{pr_number}', json={'title': title, 'body': body}, headers=get_github_api_headers()) as response:
        response.raise_for_status()
    return pr_number

async def update_pull_request_label(*, pr_number: int, session: aiohttp.ClientSession) -> None:
    async with session.post(f'{TYPESHED_API_URL}/issues/{pr_number}/labels', json={'labels': [STUBSABOT_LABEL]}, headers=get_github_api_headers()) as response:
        response.raise_for_status()
...
async def main() -> None:
    ...
    parser.add_argument('--action-level', type=ActionLevel.from_cmd_arg, default=ActionLevel.everything, help='Limit actions performed to achieve dry runs for different levels of dryness')
    parser.add_argument('--action-count-limit', type=int, default=None, help='Limit number of actions performed and the remainder are logged. Useful for testing')
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=conn) as session:
            tasks = [asyncio.create_task(determine_action(stubs_path, session)) for stubs_path in paths_to_update if stubs_path.name not in denylist]
            action_count = 0
