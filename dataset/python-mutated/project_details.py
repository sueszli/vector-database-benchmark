...
from sentry.lang.native.sources import InvalidSourcesError, parse_backfill_sources, parse_sources, redact_source_secrets
...
from sentry.tasks.recap_servers import RECAP_SERVER_TOKEN_OPTION, RECAP_SERVER_URL_OPTION, poll_project_recap_server
...
class ProjectMemberSerializer(serializers.Serializer):
    isBookmarked = serializers.BooleanField(help_text='Enables starring the project within the projects tab. Can be updated with **`project:read`** permission.', required=False)
    isSubscribed = serializers.BooleanField(help_text='Subscribes the member for notifications related to the project. Can be updated with **`project:read`** permission.', required=False)

@extend_schema_serializer(exclude_fields=['options', 'team', 'digestsMinDelay', 'digestsMaxDelay', 'securityToken', 'securityTokenHeader', 'verifySSL', 'defaultEnvironment', 'dataScrubber', 'dataScrubberDefaults', 'sensitiveFields', 'safeFields', 'storeCrashReports', 'relayPiiConfig', 'builtinSymbolSources', 'symbolSources', 'scrubIPAddresses', 'groupingConfig', 'groupingEnhancements', 'fingerprintingRules', 'secondaryGroupingConfig', 'secondaryGroupingExpiry', 'groupingAutoUpdate', 'scrapeJavaScript', 'allowedDomains', 'copy_from_project', 'dynamicSamplingBiases', 'performanceIssueCreationRate', 'performanceIssueCreationThroughPlatform', 'performanceIssueSendToPlatform', 'recapServerUrl', 'recapServerToken'])
class ProjectAdminSerializer(ProjectMemberSerializer):
    name = serializers.CharField(help_text='The name for the project', max_length=200, required=False)
    slug = SentrySlugField(help_text='Uniquely identifies a project and is used for the interface.', max_length=50, required=False)
    platform = serializers.CharField(help_text='The platform for the project', required=False, allow_null=True, allow_blank=True)
    subjectPrefix = serializers.CharField(help_text='Custom prefix for emails from this project.', max_length=200, allow_blank=True, required=False)
    subjectTemplate = serializers.CharField(help_text='The email subject to use (excluding the prefix) for individual alerts. Here are the list of variables you can use:\n- `$title`\n- `$shortID`\n- `$projectID`\n- `$orgID`\n- `${tag:key}` - such as `${tag:environment}` or `${tag:release}`.', max_length=200, required=False)
    resolveAge = EmptyIntegerField(required=False, allow_null=True, help_text="Automatically resolve an issue if it hasn't been seen for this many hours. Set to `0` to disable auto-resolve.")
    ...
    storeCrashReports = serializers.IntegerField(min_value=-1, max_value=STORE_CRASH_REPORTS_MAX, required=False, allow_null=True)
...
class RelaxedProjectPermission(ProjectPermission):
    scope_map = {'GET': ['project:read', 'project:write', 'project:admin'], 'POST': ['project:write', 'project:admin'], 'PUT': ['project:read', 'project:write', 'project:admin'], 'DELETE': ['project:admin']}
