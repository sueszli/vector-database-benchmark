...
@pytest.mark.asyncio
@pytest.mark.parametrize('test_state, expected, event_fixture', [(TestState, {'test_state': {'num': 1}}, 'event1'), (TestState2, {'test_state2': {'num': 1}}, 'event2'), (TestState3, {'test_state3': {'num': 1}}, 'event3')])
async def test_preprocess(test_state, hydrate_middleware, request, event_fixture, expected):
    ...
    state = test_state()
    update = await hydrate_middleware.preprocess(app=app, event=request.getfixturevalue(event_fixture), state=state)
...
async def test_preprocess_multiple_load_events(hydrate_middleware, event1):
    ...
    Args:
        ...
        event1: An Event.
    """
    app = App(state=TestState, load_events={'index': [TestState.test_handler, TestState.test_handler]})
    state = TestState()
...
async def test_preprocess_no_events(hydrate_middleware, event1):
    ...
    """
    state = TestState()
    update = await hydrate_middleware.preprocess(app=App(state=TestState), event=event1, state=state)
    assert isinstance(update, StateUpdate)
    assert update.delta == {'test_state': state.dict()}
