...
class Container(containers.DeclarativeContainer):
    ...
    s3_client = providers.Resource(session.provided.client.call(), service_name='s3')
    sqs_client = providers.Resource(providers.MethodCaller(session.provided.client), service_name='sqs')
    service1 = providers.Factory(Service, s3_client=s3_client, sqs_client=sqs_client)
    service2 = providers.Factory(Service, s3_client=session.provided.client.call(service_name='s3'), sqs_client=session.provided.client.call(service_name='sqs'))
