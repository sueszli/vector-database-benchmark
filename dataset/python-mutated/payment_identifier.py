...
RE_EMAIL = '\\b[A-Za-z0-9._%+-]+@([A-Za-z0-9-]+\\.)+[A-Z|a-z]{2,7}\\b'
RE_DOMAIN = '\\b([A-Za-z0-9-]+\\.)+[A-Z|a-z]{2,7}\\b'
RE_SCRIPT_FN = 'script\\((.*)\\)'
...
class PaymentIdentifier(Logger):
    ...
    @log_exceptions
    async def _do_finalize(self, *, amount_sat: int=None, comment: str=None, on_finished: Callable[['PaymentIdentifier'], None]=None):
        from .invoices import Invoice
        try:
            ...
            if not self.lnurl_data.min_sendable_sat <= amount_sat <= self.lnurl_data.max_sendable_sat:
                self.error = _('Amount must be between {} and {} sat.').format(self.lnurl_data.min_sendable_sat, self.lnurl_data.max_sendable_sat)
    ...
    @log_exceptions
    async def _do_notify_merchant(self, tx: 'Transaction', refund_address: str, *, on_finished: Callable[['PaymentIdentifier'], None]=None):
        try:
