...
async def test_use_linked_inputs_on_change(display: DisplayFixture):
    ...
    def SomeComponent():
        if False:
            for i in range(10):
                print('nop')
        (i_1, i_2) = reactpy.widgets.use_linked_inputs([{'id': 'i_1'}, {'id': 'i_2'}], on_change=value.set_current)
        return reactpy.html.div(i_1, i_2)
    await display.show(SomeComponent)
    input_1 = await display.page.wait_for_selector('#i_1')
    input_2 = await display.page.wait_for_selector('#i_2')
...
async def test_use_linked_inputs_on_change_with_cast(display: DisplayFixture):
    ...
    def SomeComponent():
        if False:
            i = 10
            return i + 15
        (i_1, i_2) = reactpy.widgets.use_linked_inputs([{'id': 'i_1'}, {'id': 'i_2'}], on_change=value.set_current, cast=int)
        return reactpy.html.div(i_1, i_2)
    await display.show(SomeComponent)
    input_1 = await display.page.wait_for_selector('#i_1')
    input_2 = await display.page.wait_for_selector('#i_2')
...
async def test_use_linked_inputs_ignore_empty(display: DisplayFixture):
    ...
    def SomeComponent():
        if False:
            i = 10
            return i + 15
        (i_1, i_2) = reactpy.widgets.use_linked_inputs([{'id': 'i_1'}, {'id': 'i_2'}], on_change=value.set_current, ignore_empty=True)
        return reactpy.html.div(i_1, i_2)
    await display.show(SomeComponent)
    input_1 = await display.page.wait_for_selector('#i_1')
    input_2 = await display.page.wait_for_selector('#i_2')
