...
test_data_class = [('', {'-gst_pipeline_mode': 'invalid'}, False), (os.path.join(tempfile.gettempdir(), 'temp_write'), {}, True), ('Output_twc.mp4', {'-fourcc': 'DIVX', '-fps': 25, '-backend': 'CAP_FFMPEG', '-disable_ffmpeg_window': True, '-color': True, '-gst_pipeline_mode': False}, True), ('Output_twc.avi', {'-fourcc': ['NULL'], '-backend': 'INVALID', '-unknown': 'INVALID', '-fps': -11}, False), ('appsrc ! videoconvert ! avenc_mpeg4 bitrate=100000 ! mp4mux ! filesink location=foo.mp4', {'-gst_pipeline_mode': True} if platform.system() == 'Linux' else {'-gst_pipeline_mode': 'invalid'}, True if platform.system() == 'Linux' else False)]
