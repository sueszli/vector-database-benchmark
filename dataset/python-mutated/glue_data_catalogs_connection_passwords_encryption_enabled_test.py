...
class Test_glue_data_catalogs_connection_passwords_encryption_enabled:
    ...
    def test_glue_no_settings(self):
        ...
        with mock.patch('prowler.providers.aws.services.glue.glue_service.Glue', glue_client):
            from prowler.providers.aws.services.glue.glue_data_catalogs_connection_passwords_encryption_enabled.glue_data_catalogs_connection_passwords_encryption_enabled import glue_data_catalogs_connection_passwords_encryption_enabled
    ...
    def test_glue_catalog_password_unencrypted(self):
        ...
        with mock.patch('prowler.providers.aws.services.glue.glue_service.Glue', glue_client):
            ...
            assert result[0].status_extended == 'Glue data catalog connection password is not encrypted.'
    ...
    def test_glue_catalog_encrypted(self):
        ...
        glue_client = mock.MagicMock
        glue_client.audit_info = self.set_mocked_audit_info()
        glue_client.catalog_encryption_settings = [CatalogEncryptionSetting(mode='DISABLED', tables=False, region=AWS_REGION, password_encryption=True, password_kms_id='kms-key')]
        glue_client.audited_account = '12345678912'
        with mock.patch('prowler.providers.aws.services.glue.glue_service.Glue', glue_client):
            from prowler.providers.aws.services.glue.glue_data_catalogs_connection_passwords_encryption_enabled.glue_data_catalogs_connection_passwords_encryption_enabled import glue_data_catalogs_connection_passwords_encryption_enabled
            check = glue_data_catalogs_connection_passwords_encryption_enabled()
            ...
            assert result[0].status_extended == 'Glue data catalog connection password is encrypted with KMS key kms-key.'
            assert result[0].resource_id == '12345678912'
            assert result[0].region == AWS_REGION