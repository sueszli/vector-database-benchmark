import functools
import sympy as sm
from coremltools.converters.mil.mil.types.symbolic import is_symbolic, isscalar, any_symbolic, any_variadic
...
class expand_dims(Operation):
    ...
    T: fp32
    """
    input_spec = InputSpec(x=ScalarOrTensorInputType(), axes=IntTensorInputType(const=True))
...
class reverse(Operation):
    ...
    T: fp32
    """
    input_spec = InputSpec(x=TensorInputType(), axes=IntTensorInputType(const=True, optional=True))
...
class reverse_sequence(Operation):
    ...
    T: fp32
    """
    input_spec = InputSpec(x=TensorInputType(), lengths=IntTensorInputType(), seq_axis=IntInputType(const=True, default=0), batch_axis=IntInputType(const=True, default=0))
...
class slice_by_size(Operation):
    ...
    T: fp32
    """
    input_spec = InputSpec(x=TensorInputType(), begin=IntTensorInputType(), size=IntTensorInputType())
...
class pixel_shuffle(Operation):
    ...
    T: fp32
    """
    input_spec = InputSpec(x=TensorInputType(), upscale_factor=IntInputType(const=True))
...
class sliding_windows(Operation):
    ...
    T: fp32
    """
    input_spec = InputSpec(x=TensorInputType(), axis=IntInputType(const=True), size=IntInputType(const=True), stride=IntInputType(const=True, default=1))
