...
from ...operations._alerts_operations import build_dismiss_request, build_get_request, build_list_external_request, build_list_request
T = TypeVar('T')
...
class AlertsOperations:
    ...
    async def list(self, scope: str, **kwargs: Any) -> _models.AlertsResult:
        ...
        request = build_list_request(scope=scope, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
    ...
    @overload
    async def dismiss(self, scope: str, alert_id: str, parameters: _models.DismissAlertPayload, *, content_type: str='application/json', **kwargs: Any) -> _models.Alert:
    ...
    @overload
    async def dismiss(self, scope: str, alert_id: str, parameters: IO, *, content_type: str='application/json', **kwargs: Any) -> _models.Alert:
        """Dismisses the specified alert.
    ...
    @distributed_trace_async
    async def dismiss(self, scope: str, alert_id: str, parameters: Union[_models.DismissAlertPayload, IO], **kwargs: Any) -> _models.Alert:
        """Dismisses the specified alert.
        ...
        request = build_dismiss_request(scope=scope, alert_id=alert_id, api_version=api_version, content_type=content_type, json=_json, content=_content, template_url=self.dismiss.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=_stream, **kwargs)
    ...
    @distributed_trace_async
    async def list_external(self, external_cloud_provider_type: Union[str, _models.ExternalCloudProviderType], external_cloud_provider_id: str, **kwargs: Any) -> _models.AlertsResult:
        ...
        api_version: str = kwargs.pop('api_version', _params.pop('api-version', self._config.api_version))
        cls: ClsType[_models.AlertsResult] = kwargs.pop('cls', None)
        request = build_list_external_request(external_cloud_provider_type=external_cloud_provider_type, external_cloud_provider_id=external_cloud_provider_id, api_version=api_version, template_url=self.list_external.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=_stream, **kwargs)
