...
TaxCountryConfigurationDeleteErrorCode = graphene.Enum.from_enum(error_codes.TaxCountryConfigurationDeleteErrorCode)
TaxCountryConfigurationDeleteErrorCode.doc_category = DOC_CATEGORY_TAXES

class TaxCountryConfigurationDeleteError(Error):
    code = TaxCountryConfigurationDeleteErrorCode(description='The error code.', required=True)

    class Meta:
        doc_category = DOC_CATEGORY_TAXES

class TaxCountryConfigurationDelete(BaseMutation):
    tax_country_configuration = graphene.Field(TaxCountryConfiguration, description='Updated tax class rates grouped by a country.')

    class Arguments:
        country_code = CountryCodeEnum(description='Country in which to update the tax class rates.', required=True)
    ...
    def perform_mutation(cls, _root, _info: ResolveInfo, /, **data):
        ...
        rates.delete()
        country_config = TaxCountryConfiguration(country=Country(country_code), tax_class_country_rates=[])
