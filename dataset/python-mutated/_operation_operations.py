...
def build_list_by_tags_request(resource_group_name: str, service_name: str, api_id: str, subscription_id: str, *, filter: Optional[str]=None, top: Optional[int]=None, skip: Optional[int]=None, include_not_tagged_operations: Optional[bool]=None, **kwargs: Any) -> HttpRequest:
    if False:
        for i in range(10):
            print('nop')
    ...
    _url = kwargs.pop('template_url', '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ApiManagement/service/{serviceName}/apis/{apiId}/operationsByTags')
    ...
    if include_not_tagged_operations is not None:
        _params['includeNotTaggedOperations'] = _SERIALIZER.query('include_not_tagged_operations', include_not_tagged_operations, 'bool')
        ...
        def prepare_request(next_link=None):
            if False:
                while True:
                    i = 10
            if not next_link:
                request = build_list_by_tags_request(resource_group_name=resource_group_name, service_name=service_name, api_id=api_id, subscription_id=self._config.subscription_id, filter=filter, top=top, skip=skip, include_not_tagged_operations=include_not_tagged_operations, api_version=api_version, template_url=self.list_by_tags.metadata['url'], headers=_headers, params=_params)
