...
async def update_rule(bangumi_id: int, data: BangumiUpdate):
    with TorrentManager() as manager:
...
@router.get(path='/refresh/poster/{bangumi_id}', response_model=APIResponse, dependencies=[Depends(get_current_user)])
...
async def reset_all():
    with TorrentManager() as manager:
        ...
        return JSONResponse(status_code=200, content={'msg_en': 'Reset all rules successfully.', 'msg_zh': '重置所有规则成功。'})