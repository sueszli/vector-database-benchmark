...
import shutil
import sys
pt_regexp = re.compile('checkpoint(\\d+|_\\d+_\\d+|_[a-z]+)\\.pt')
pt_regexp_epoch_based = re.compile('checkpoint(\\d+)\\.pt')
pt_regexp_update_based = re.compile('checkpoint_\\d+_(\\d+)\\.pt')
