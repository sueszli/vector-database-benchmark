...
import select
from .BaseUtils import getListFromXml, getTextFromXml, getRootTagName, decode_from_s3, encode_to_s3, md5, s3_quote
from .Utils import convertHeaderTupleListToDict, unicodise, deunicodise, check_bucket_name, check_bucket_name_dns_support, getHostnameFromBucket
...
from .Crypto import sign_request_v2, sign_request_v4, checksum_sha256_file, checksum_sha256_buffer, generate_content_md5, hash_file_md5, calculateChecksum, format_param_str
try:
...
class S3(object):
    http_methods = BidirMap(GET=1, PUT=2, HEAD=4, DELETE=8, POST=16, MASK=31)
    targets = BidirMap(SERVICE=256, BUCKET=512, OBJECT=1024, BATCH=2048, MASK=1792)
    operations = BidirMap(UNDEFINED=0, LIST_ALL_BUCKETS=targets['SERVICE'] | http_methods['GET'], BUCKET_CREATE=targets['BUCKET'] | http_methods['PUT'], BUCKET_LIST=targets['BUCKET'] | http_methods['GET'], BUCKET_DELETE=targets['BUCKET'] | http_methods['DELETE'], OBJECT_PUT=targets['OBJECT'] | http_methods['PUT'], OBJECT_GET=targets['OBJECT'] | http_methods['GET'], OBJECT_HEAD=targets['OBJECT'] | http_methods['HEAD'], OBJECT_DELETE=targets['OBJECT'] | http_methods['DELETE'], OBJECT_POST=targets['OBJECT'] | http_methods['POST'], BATCH_DELETE=targets['BATCH'] | http_methods['POST'])
    codes = {'NoSuchBucket': "Bucket '%s' does not exist", 'AccessDenied': "Access to bucket '%s' was denied", 'BucketAlreadyExists': "Bucket '%s' already exists"}
