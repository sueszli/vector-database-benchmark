...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='Autoestimator_pytorch', description='Automatically fit the model and return the best model.')
    parser.add_argument('--cluster_mode', type=str, default='local', help='The mode for the Spark cluster.')
    parser.add_argument('--num_nodes', type=int, default=1, help='The number of nodes to be used in the cluster. You can change it depending on your own cluster setting.')
    ...
    parser.add_argument('--workers_per_node', type=int, default=2, help='The number of workers to run on each node')
    ...
    parser.add_argument('--cpus_per_trial', type=int, default=2, help='The number of cores you want to allocate for each trial.')
    ...
    if args.cluster_mode == 'local':
        init_orca_context(cluster_mode='local', cores=args.cores, num_nodes=args.num_nodes, memory=args.memory, init_ray_on_spark=True)
    elif args.cluster_mode == 'yarn':
        init_orca_context(cluster_mode='yarn-client', cores=args.cores, memory=args.memory, init_ray_on_spark=True)
        ...
        init_orca_context(cluster_mode='k8s', master=args.k8s_master, container_image=args.container_image, cores=args.cores, init_ray_on_spark=True, conf={'spark.driver.host': args.k8s_driver_host, 'spark.driver.port': args.k8s_driver_port})
