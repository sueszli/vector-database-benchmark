...
flags = tf.app.flags
flags.DEFINE_string('save_dir', '/tmp/' + DATA_DIR + '/', 'Directory for saving data.')
flags.DEFINE_string('datafile_name', 'itb_rnn', 'Name of data file for input case.')
flags.DEFINE_integer('synth_data_seed', 5, 'Random seed for RNN generation.')
flags.DEFINE_float('T', 1.0, 'Time in seconds to generate.')
flags.DEFINE_integer('C', 800, 'Number of conditions')
flags.DEFINE_integer('N', 50, 'Number of units for the RNN')
flags.DEFINE_float('train_percentage', 4.0 / 5.0, 'Percentage of train vs validation trials')
flags.DEFINE_integer('nreplications', 5, 'Number of spikifications of the same underlying rates.')
flags.DEFINE_float('tau', 0.025, 'Time constant of RNN')
flags.DEFINE_float('dt', 0.01, 'Time bin')
flags.DEFINE_float('max_firing_rate', 30.0, 'Map 1.0 of RNN to a spikes per second')
flags.DEFINE_float('u_std', 0.25, 'Std dev of input to integration to bound model')
flags.DEFINE_string('checkpoint_path', 'SAMPLE_CHECKPOINT', 'Path to directory with checkpoints of model\n                    trained on integration to bound task. Currently this\n                    is a placeholder which tells the code to grab the\n                    checkpoint that is provided with the code\n                    (in /trained_itb/..). If you have your own checkpoint\n                    you would like to restore, you would point it to\n                    that path.')
FLAGS = flags.FLAGS
...
inputs_ph_t = [tf.placeholder(tf.float32, shape=[None, 1]) for _ in range(ntimesteps)]
...
with tf.Session() as sess:
    ...
    try:
        ...
        assert False, 'No checkpoints to restore from, is the path %s correct?' % model_checkpoint_path
    data_e = []
    u_e = []
    outs_e = []
    for c in range(C):
        ...
        (states_t_bxn, outputs_t_bxn) = sess.run([states_t, outputs_t], feed_dict=feed_dict)
...
spiking_data_e = spikify_data(truth_data_e, rng, dt=FLAGS.dt, max_firing_rate=FLAGS.max_firing_rate)
(train_inds, valid_inds) = get_train_n_valid_inds(E, train_percentage, nreplications)
(data_train_truth, data_valid_truth) = split_list_by_inds(truth_data_e, train_inds, valid_inds)
(data_train_spiking, data_valid_spiking) = split_list_by_inds(spiking_data_e, train_inds, valid_inds)
