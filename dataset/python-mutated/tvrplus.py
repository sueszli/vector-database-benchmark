...
@pluginmatcher(re.compile('https?://(?:www\\.)?tvrplus\\.ro(?:/live/.+|/?$)'))
class TVRPlus(Plugin):
    ...
    def _get_streams(self):
        if False:
            return 10
        try:
            hls_url = self.session.http.get(self.url, schema=validate.Schema(re.compile('(?P<q>["\'])(?P<url>https?://\\S+?\\.m3u8\\S*?)(?P=q)'), validate.get('url')))
        except PluginError:
            return
