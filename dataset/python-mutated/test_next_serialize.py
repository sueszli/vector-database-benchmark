...
from pulumi.asset import AssetArchive, FileArchive, FileAsset, RemoteArchive, RemoteAsset, StringAsset
from pulumi.resource import ComponentResource, CustomResource, DependencyResource, Resource, ResourceOptions
from pulumi.runtime import MockCallArgs, MockResourceArgs, Mocks, ResourceModule, mocks, rpc, rpc_manager, set_mocks, settings
import pulumi
...
class NextSerializationTests(unittest.TestCase):
    ...
    async def test_asset_archive(self):
        ...
        self.assertDictEqual({rpc._special_sig_key: rpc._special_archive_sig, 'assets': {'foo': {rpc._special_sig_key: rpc._special_asset_sig, 'text': 'bar'}}}, prop)
    ...
    async def test_unsupported_sequences(self):
        cases = [range(10), memoryview(bytes(10)), bytes(10), bytearray(10)]
        for case in cases:
    ...
    async def test_dangerous_prop_output(self):
        out = self.create_output(MyOutputTypeDict(values=['foo', 'bar'], items=['yellow', 'purple'], keys=['yes', 'no']), is_known=True)
    ...
    async def test_lifted_unknown(self):
        ...
        out = Output.all(Output.from_input('foo'), Output.from_input(UNKNOWN), Output.from_input([Output.from_input(UNKNOWN), Output.from_input('bar')]))
        self.assertFalse(await out.is_known())
        r7 = out[0]
        ...
        out_dict = Output.all(foo=Output.from_input('foo'), unknown=Output.from_input(UNKNOWN), arr=Output.from_input([Output.from_input(UNKNOWN), Output.from_input('bar')]))
...
class InputTypeSerializationTests(unittest.TestCase):
    ...
    async def test_list_dict_input_type(self):
        ...
        prop = await rpc.serialize_property(it, [])
        self.assertEqual({'a': ['hi'], 'b': ['there'], 'c': {'hello': 'world'}, 'd': {'foo': 'bar'}}, prop)
    ...
    async def test_input_type_with_dict_property(self):
        ...
        self.assertEqual({'tagArgs': {'c': 'hello', 'foo_baz': 'world'}}, prop)
...
class TypeMetaDataSerializationTests(unittest.TestCase):
    ...
    async def test_serialize(self):
        ...
        tests = [{'some_value': 'hello', 'some_foo': SomeFooArgs('first', {'the_first': 'there'}), 'some_bar': {'a': SomeFooArgs('second', {'the_second': 'later'})}}, {'some_value': 'hello', 'some_foo': {'the_first': 'first', 'the_second': {'the_first': 'there'}}, 'some_bar': {'a': {'the_first': 'second', 'the_second': {'the_second': 'later'}}}}, {'some_value': 'hello', 'some_foo': {'theFirst': 'first', 'theSecond': {'the_first': 'there'}}, 'some_bar': {'a': {'theFirst': 'second', 'theSecond': {'the_second': 'later'}}}}]
        for props in tests:
    ...
    async def test_output_translation(self):
        ...
        def transformer(key: str) -> str:
            if False:
                print('Hello World!')
            raise Exception(f"Should not be raised for key '{key}'")
        output = {'someValue': 'hello', 'someFoo': {'theFirst': 'first', 'theSecond': {'the_first': 'there'}}, 'someBar': {'a': {'theFirst': 'second', 'theSecond': {'the_second': 'later'}}}}
...
class OutputValueSerializationTests(unittest.TestCase):
    ...
    async def test_serialize(self):
        ...
        for (value, deps, is_known, is_secret) in gen_test_parameters():
            with self.subTest(value=value, deps=deps, is_known=is_known, is_secret=is_secret):
                ...
                obj: Dict[str, Any] = {rpc._special_sig_key: rpc._special_output_value_sig}
                if is_known:
                    obj['value'] = value
                if is_secret:
                ...
                expected['value'] = obj
                expected_round_trip = Output(resources, future(value if is_known else None), future(is_known), future(is_secret))
    ...
    async def test_serialize_nested_dict(self):
        settings.SETTINGS.feature_support['outputValues'] = True
        inputs = {'value': {'foo': Output(set(), future('bar'), future(True), future(True))}}
        expected = struct_pb2.Struct()
        expected['value'] = {'foo': {rpc._special_sig_key: rpc._special_output_value_sig, 'value': 'bar', 'secret': True}}
        actual = await rpc.serialize_properties(inputs, {}, keep_output_values=True)
        self.assertDictEqual(json_format.MessageToDict(expected), json_format.MessageToDict(actual))
