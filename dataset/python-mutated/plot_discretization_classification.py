...
def get_name(estimator):
    ...
    if name == 'Pipeline':
        ...
        name = ' + '.join(name)
    return name
classifiers = [(make_pipeline(StandardScaler(), LogisticRegression(random_state=0)), {'logisticregression__C': np.logspace(-1, 1, 3)}), (make_pipeline(StandardScaler(), LinearSVC(random_state=0, dual='auto')), {'linearsvc__C': np.logspace(-1, 1, 3)}), (make_pipeline(StandardScaler(), KBinsDiscretizer(encode='onehot', random_state=0), LogisticRegression(random_state=0)), {'kbinsdiscretizer__n_bins': np.arange(5, 8), 'logisticregression__C': np.logspace(-1, 1, 3)}), (make_pipeline(StandardScaler(), KBinsDiscretizer(encode='onehot', random_state=0), LinearSVC(random_state=0, dual='auto')), {'kbinsdiscretizer__n_bins': np.arange(5, 8), 'linearsvc__C': np.logspace(-1, 1, 3)}), (make_pipeline(StandardScaler(), GradientBoostingClassifier(n_estimators=5, random_state=0)), {'gradientboostingclassifier__learning_rate': np.logspace(-2, 0, 5)}), (make_pipeline(StandardScaler(), SVC(random_state=0)), {'svc__C': np.logspace(-1, 1, 3)})]
names = [get_name(e).replace('StandardScaler + ', '') for (e, _) in classifiers]
n_samples = 100
datasets = [make_moons(n_samples=n_samples, noise=0.2, random_state=0), make_circles(n_samples=n_samples, noise=0.2, factor=0.5, random_state=1), make_classification(n_samples=n_samples, n_features=2, n_redundant=0, n_informative=2, random_state=2, n_clusters_per_class=1)]
