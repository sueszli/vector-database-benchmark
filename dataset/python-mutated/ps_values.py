...
TRACKABLE_RESOURCE_METHODS = ['_create_resource', '_initialize', '_destroy_resource']
...
tensor_conversion_registry.register_tensor_conversion_function(AggregatingVariable, _tensor_conversion_aggregate)

def _tensor_conversion_caching(var, dtype=None, name=None, as_ref=False):
    if False:
        return 10
    return var._dense_var_to_tensor(dtype, name, as_ref)
tensor_conversion_registry.register_tensor_conversion_function(CachingVariable, _tensor_conversion_caching)
CachingVariable._overload_overloadable_operators()
