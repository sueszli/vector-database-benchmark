...
class DenseToSparseBatchTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testBasic(self):
        ...
        dataset = dataset_ops.Dataset.from_tensor_slices(components).map(lambda x: array_ops.fill([x], x)).apply(batching.dense_to_sparse_batch(4, [12]))
    ...
    def testWithUnknownShape(self):
        ...
        dataset = dataset_ops.Dataset.from_tensor_slices(components).map(lambda x: array_ops.fill([x, x], x)).apply(batching.dense_to_sparse_batch(4, [5, None]))
        get_next = self.getNext(dataset)
        for start in range(0, len(components), 4):
            results = self.evaluate(get_next())
    ...
    def testWithInvalidShape(self):
        ...
        with self.assertRaisesRegex(ValueError, 'Dimension -2 must be >= 0'):
            dataset_ops.Dataset.from_tensors(input_tensor).apply(batching.dense_to_sparse_batch(4, [-2]))
    ...
    def testShapeErrors(self):
        ...
        def dataset_fn(input_tensor):
            if False:
                for i in range(10):
                    print('nop')
            return dataset_ops.Dataset.from_tensors(input_tensor).apply(batching.dense_to_sparse_batch(4, [12]))
...
class DenseToSparseBatchCheckpointTest(checkpoint_test_base.CheckpointTestBase, parameterized.TestCase):
    ...
    def _build_dataset(self, components):
        if False:
            print('Hello World!')
        return dataset_ops.Dataset.from_tensor_slices(components).map(lambda x: array_ops.fill([x], x)).apply(batching.dense_to_sparse_batch(4, [12]))
