...
AZURE_TOKEN_REFRESH_INTERVAL = 600
...
class ManagedIdentityAPITokenManager(APITokenManager):
    ...
    async def get_token(self):
        async with self.lock:
            if self.token is None or self.last_refresh_time is None or time.time() - self.last_refresh_time > AZURE_TOKEN_REFRESH_INTERVAL:
