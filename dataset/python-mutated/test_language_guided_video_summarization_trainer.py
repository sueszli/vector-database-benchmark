...
class LanguageGuidedVideoSummarizationTrainerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dataset_train = LanguageGuidedVideoSummarizationDataset('train', self.config.dataset, self.cache_path)
        self.dataset_val = LanguageGuidedVideoSummarizationDataset('test', self.config.dataset, self.cache_path)
    ...
    def test_trainer(self):
        if False:
            print('Hello World!')
        kwargs = dict(model=self.model_id, train_dataset=self.dataset_train, eval_dataset=self.dataset_val, max_epochs=2, work_dir=self.tmp_dir)
    ...
    def test_trainer_with_model_and_args(self):
        ...
        kwargs = dict(cfg_file=os.path.join(self.cache_path, ModelFile.CONFIGURATION), model=model, train_dataset=self.dataset_train, eval_dataset=self.dataset_val, max_epochs=2, work_dir=self.tmp_dir)
        trainer = build_trainer(default_args=kwargs)
        trainer.train()
        results_files = os.listdir(self.tmp_dir)
        self.assertIn(f'{trainer.timestamp}.log.json', results_files)
