...
parametrize_casting = parametrize('casting', ['no', 'equiv', 'safe', 'same_kind', 'unsafe'])
...
class TestUnaryUfuncs(TestCase):
    ...
    def test_x_and_out_broadcast(self, ufunc):
        ...
        assert_equal(res_out, res_bcast)
        assert res_out is out
ufunc_op_iop_numeric = [(np.add, operator.__add__, operator.__iadd__), (np.subtract, operator.__sub__, operator.__isub__), (np.multiply, operator.__mul__, operator.__imul__)]
ufuncs_with_dunders = [ufunc for (ufunc, _, _) in ufunc_op_iop_numeric]
numeric_binary_ufuncs = [np.float_power, np.power]
no_complex = [np.floor_divide, np.hypot, np.arctan2, np.copysign, np.fmax, np.fmin, np.fmod, np.heaviside, np.logaddexp, np.logaddexp2, np.maximum, np.minimum]
parametrize_binary_ufuncs = parametrize('ufunc', ufuncs_with_dunders + numeric_binary_ufuncs + no_complex)
"\n 'bitwise_and',\n 'bitwise_or',\n 'bitwise_xor',\n 'equal',\n 'lcm',\n 'ldexp',\n 'left_shift',\n 'less',\n 'less_equal',\n 'gcd',\n 'greater',\n 'greater_equal',\n 'logical_and',\n 'logical_or',\n 'logical_xor',\n 'matmul',\n 'not_equal',\n"
