...
class ZipSheet(Sheet):
    ...
    rowtype = 'files'
    columns = [Column('directory', getter=lambda col, row: str(row[1].parent) if str(row[1].parent) == '.' else str(row[1].parent) + '/'), Column('filename', getter=lambda col, row: row[1].name + row[1].suffix), Column('abspath', type=str, width=0, getter=lambda col, row: row[1]), Column('ext', getter=lambda col, row: row[0].filename.endswith('/') and '/' or row[1].ext), Column('size', getter=lambda col, row: row[0].file_size, type=int), Column('compressed_size', type=int, getter=lambda col, row: row[0].compress_size), Column('date_time', type=date, getter=lambda col, row: datetime.datetime(*row[0].date_time))]
    nKeys = 2
...
class TarSheet(Sheet):
    ...
    rowtype = 'files'
    columns = [ColumnAttr('name'), ColumnAttr('size', type=int), ColumnAttr('mtime', type=date), ColumnAttr('type', type=int), ColumnAttr('mode', type=int), ColumnAttr('uname'), ColumnAttr('gname')]
...
vd.addMenu(Menu('File', Menu('Extract', Menu('current file', 'extract-file'), Menu('current file to', 'extract-file-to'), Menu('selected files', 'extract-selected'), Menu('selected files to', 'extract-selected-to'))))
vd.addGlobals({'ZipSheet': ZipSheet, 'TarSheet': TarSheet})