...
def parse(data: str, raw: bool=False, quiet: bool=False) -> List[JSONDictType]:
    ...
    if jc.utils.has_data(data):
        for line in filter(None, data.splitlines()):
            if '---' in line or line.strip().endswith('bandwidth') or line.strip().endswith('write'):
                continue
            line_list = line.strip().split()
            if line.startswith(' '):
                output_line = {'pool': line_list[0], 'parent': pool_parent}
            else:
                pool_parent = line_list[0]
                output_line = {'pool': pool_parent}
            output_line.update({'cap_alloc': line_list[1], 'cap_free': line_list[2], 'ops_read': line_list[3], 'ops_write': line_list[4], 'bw_read': line_list[5], 'bw_write': line_list[6]})
