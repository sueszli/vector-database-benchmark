...
async def download_image(client, file_link, root_dir):
    ...
    print(f'Async downloading image to {out_path.resolve()}... ', end='', flush=True)
...
async def sample_satellite_download_async():
    ...
    try:
        ...
        boundary = await client.boundaries.create_or_update(party_id=party_id, boundary_id=boundary_id, boundary={'geometry': {'type': 'Polygon', 'coordinates': [[[73.70457172393799, 20.545385304358106], [73.70457172393799, 20.545385304358106], [73.70448589324951, 20.542411534243367], [73.70877742767334, 20.541688176010233], [73.71023654937744, 20.545083911372505], [73.70663166046143, 20.546992723579137], [73.70457172393799, 20.545385304358106]]]}, 'status': '<string>', 'name': '<string>', 'description': '<string>'})
        print('Created')
    job_id = f'{job_id_prefix}-{randint(0, 1000)}'
    print(f'Queuing satellite job {job_id}... ', end='', flush=True)
    satellite_job_poller = await client.scenes.begin_create_satellite_data_ingestion_job(job_id=job_id, job={'boundaryId': boundary_id, 'endDateTime': end_date_time, 'partyId': party_id, 'startDateTime': start_date_time, 'provider': 'Microsoft', 'source': 'Sentinel_2_L2A', 'data': {'imageNames': ['NDVI'], 'imageFormats': ['TIF'], 'imageResolution': [10]}, 'name': '<string>', 'description': '<string>'})
