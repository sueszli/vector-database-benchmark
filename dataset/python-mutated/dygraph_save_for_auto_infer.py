...
from paddle.distributed.fleet.layers.mpu.mp_layers import ColumnParallelLinear, RowParallelLinear, VocabParallelEmbedding
from paddle.distributed.fleet.meta_parallel.parallel_layers.pp_layers import LayerDesc, PipelineLayer
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cmd', default='main', choices=['main', 'save', 'load'])
    parser.add_argument('--strategy', required=False, choices=['single', 'dp', 'mp', 'pp', 'sharding_stage2', 'sharding_stage3', 'static'])
    parser.add_argument('--load_way', choices=['paddle.load', 'load'], required=False)
    parser.add_argument('--load_dir', required=False)
    parser.add_argument('--output_dir', required=False)
    parser.add_argument('--output_param_path', required=False)
    parser.add_argument('--test_case', required=False, choices=['dp:static', 'mp:static', 'pp:static', 'sharding_stage2:static', 'sharding_stage3:static', 'single:static'])
