...
TEST_VERSIONS = ('pypy3-2.4.0', 'pypy-2.6.1', 'pypy-5.0.1', 'pypy-5.3.1', 'pypy3.5-5.7.1-beta', 'pypy3.5-5.9.0', 'pypy3.5-6.0.0', 'pypy3.6-7.1.0', 'pypy3.6-7.1.1', 'pypy3.6-7.2.0', 'pypy3.8-7.3.7', 'native') + tuple(python_versions)
target_base = '/tmp/py-dis/'
lib_prefix = os.path.join(os.environ['HOME'], '.pyenv/versions')
PYC = ('*.pyc',)
PYO = ('*.pyo',)
PYOC = ('*.pyc', '*.pyo')
test_options = {'test': ('./test', PYOC, 'test'), 'max=': 200}
for vers in TEST_VERSIONS:
    if vers.startswith('pypy'):
        if vers.startswith('pypy3.'):
            ...
            short_vers = vers[0:-2]
        test_options[vers] = (os.path.join(lib_prefix, vers, 'lib_pypy'), PYC, 'python-lib' + short_vers)
    elif vers == 'native':
        ...
        short_vers = vers[:3]
        test_options[vers] = (os.path.join(lib_prefix, vers, 'lib', 'python' + short_vers), PYC, 'python-lib' + short_vers)
...
if __name__ == '__main__':
    ...
    (opts, args) = getopt.getopt(sys.argv[1:], '', ['start-with=', 'verify', 'verify-run', 'syntax-verify', 'max=', 'coverage', 'all'] + test_options_keys)
    vers = ''
    for (opt, val) in opts:
        if opt == '--verify':
            do_verify = 'strong'
    ...
    for (src_dir, pattern, target_dir) in test_dirs:
        if os.path.exists(src_dir):
            ...
            failed += do_tests(src_dir, pattern, target_dir, start_with, do_verify, test_options['max='])
        else:
