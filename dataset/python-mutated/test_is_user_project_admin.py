...
def test_is_user_project_admin_returns_true_for_project_admin(test_user, project, project_admin):
    if False:
        return 10
    assert is_user_project_admin(test_user, project) is True

def test_is_user_project_admin_returns_false_for_user_with_no_permission(test_user, project):
    if False:
        while True:
            i = 10
    assert is_user_project_admin(test_user, project) is False

def test_is_user_project_admin_returns_false_for_user_with_admin_permission_of_other_org(admin_user, organisation_two_project_one):
    if False:
        return 10
    assert is_user_project_admin(admin_user, organisation_two_project_one) is False

def test_is_user_project_admin_returns_false_for_user_with_incorrect_permission(admin_user, user_project_permission, user_project_permission_group, organisation_two_project_one):
    if False:
        return 10
