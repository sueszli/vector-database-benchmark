...
class ApiFMaxTest(unittest.TestCase):
    ...
    def test_static_api(self):
        ...
        with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
            ...
            (res,) = exe.run(feed={'x': self.input_x, 'y': self.input_y}, fetch_list=[result_fmax])
        ...
        with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
            ...
            exe = paddle.static.Executor(self.place)
            (res,) = exe.run(feed={'x': self.input_x, 'z': self.input_z}, fetch_list=[result_fmax])
        ...
        with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
            ...
            exe = paddle.static.Executor(self.place)
            (res,) = exe.run(feed={'a': self.input_a, 'c': self.input_c}, fetch_list=[result_fmax])
        ...
        with paddle.static.program_guard(paddle.static.Program(), paddle.static.Program()):
            ...
            exe = paddle.static.Executor(self.place)
            (res,) = exe.run(feed={'b': self.input_b, 'c': self.input_c}, fetch_list=[result_fmax])
...
class TestElementwiseFmaxOp(OpTest):
    ...
    def test_check_grad_ingore_x(self):
        ...
        self.check_grad(['Y'], 'Out', max_relative_error=0.005, no_grad_set=set('X'), check_pir=True)
    ...
    def test_check_grad_ingore_y(self):
        ...
        self.check_grad(['X'], 'Out', max_relative_error=0.005, no_grad_set=set('Y'), check_pir=True)
