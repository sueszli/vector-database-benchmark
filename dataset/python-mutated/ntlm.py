...
from scapy.fields import Field, ByteEnumField, ByteField, ConditionalField, FieldLenField, FlagsField, LEIntField, _StrField, LEShortEnumField, MultipleTypeField, PacketField, PacketListField, LEShortField, StrField, StrFieldUtf16, StrFixedLenField, LEIntEnumField, LEThreeBytesField, StrLenFieldUtf16, UTCTimeField, XStrField, XStrFixedLenField, XStrLenField
...
class NTLM_Header(Packet):
    name = 'NTLM Header'
    fields_desc = [StrFixedLenField('Signature', b'NTLMSSP\x00', length=8), LEIntEnumField('MessageType', 3, {1: 'NEGOTIATE_MESSAGE', 2: 'CHALLENGE_MESSAGE', 3: 'AUTHENTICATE_MESSAGE'})]
    ...
    def dispatch_hook(cls, _pkt=None, *args, **kargs):
        ...
        return cls
_negotiateFlags = ['NTLMSSP_NEGOTIATE_UNICODE', 'NTLM_NEGOTIATE_OEM', 'NTLMSSP_REQUEST_TARGET', 'r10', 'NTLMSSP_NEGOTIATE_SIGN', 'NTLMSSP_NEGOTIATE_SEAL', 'NTLMSSP_NEGOTIATE_DATAGRAM', 'NTLMSSP_NEGOTIATE_LM_KEY', 'r9', 'NTLMSSP_NEGOTIATE_NTLM', 'r8', 'J', 'NTLMSSP_NEGOTIATE_OEM_DOMAIN_SUPPLIED', 'NTLMSSP_NEGOTIATE_OEM_WORKSTATION_SUPPLIED', 'r7', 'NTLMSSP_NEGOTIATE_ALWAYS_SIGN', 'NTLMSSP_TARGET_TYPE_DOMAIN', 'NTLMSSP_TARGET_TYPE_SERVER', 'r6', 'NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY', 'NTLMSSP_NEGOTIATE_IDENTIFY', 'r5', 'NTLMSSP_REQUEST_NON_NT_SESSION_KEY', 'NTLMSSP_NEGOTIATE_TARGET_INFO', 'r4', 'NTLMSSP_NEGOTIATE_VERSION', 'r3', 'r2', 'r1', 'NTLMSSP_NEGOTIATE_128', 'NTLMSSP_NEGOTIATE_KEY_EXCH', 'NTLMSSP_NEGOTIATE_56']
...
class _NTLM_Version(Packet):
    fields_desc = [ByteField('ProductMajorVersion', 0), ByteField('ProductMinorVersion', 0), LEShortField('ProductBuild', 0), LEThreeBytesField('res_ver', 0), ByteEnumField('NTLMRevisionCurrent', 15, {15: 'v15'})]

class NTLM_NEGOTIATE(_NTLMPayloadPacket):
    name = 'NTLM Negotiate'
    messageType = 1
    OFFSET = 40
    fields_desc = [NTLM_Header, FlagsField('NegotiateFlags', 0, -32, _negotiateFlags), LEShortField('DomainNameLen', None), LEShortField('DomainNameMaxLen', None), LEIntField('DomainNameBufferOffset', None), LEShortField('WorkstationNameLen', None), LEShortField('WorkstationNameMaxLen', None), LEIntField('WorkstationNameBufferOffset', None), _NTLM_Version, _NTLMPayloadField('Payload', OFFSET, [_NTLMStrField('DomainName', b''), _NTLMStrField('WorkstationName', b'')])]
...
class Single_Host_Data(Packet):
    fields_desc = [LEIntField('Size', 0), LEIntField('Z4', 0), XStrFixedLenField('CustomData', b'', length=8), XStrFixedLenField('MachineID', b'', length=32)]
...
class AV_PAIR(Packet):
    name = 'NTLM AV Pair'
    fields_desc = [LEShortEnumField('AvId', 0, {0: 'MsvAvEOL', 1: 'MsvAvNbComputerName', 2: 'MsvAvNbDomainName', 3: 'MsvAvDnsComputerName', 4: 'MsvAvDnsDomainName', 5: 'MsvAvDnsTreeName', 6: 'MsvAvFlags', 7: 'MsvAvTimestamp', 8: 'MsvAvSingleHost', 9: 'MsvAvTargetName', 10: 'MsvAvChannelBindings'}), FieldLenField('AvLen', None, length_of='Value', fmt='<H'), MultipleTypeField([(LEIntEnumField('Value', 1, {1: 'constrained', 2: 'MIC integrity', 4: 'SPN from untrusted source'}), lambda pkt: pkt.AvId == 6), (UTCTimeField('Value', None, epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=10000000.0, fmt='<Q'), lambda pkt: pkt.AvId == 7), (PacketField('Value', Single_Host_Data(), Single_Host_Data), lambda pkt: pkt.AvId == 8), (XStrLenField('Value', b'', length_from=lambda pkt: pkt.AvLen), lambda pkt: pkt.AvId == 10)], StrLenFieldUtf16('Value', b'', length_from=lambda pkt: pkt.AvLen))]
...
class NTLM_CHALLENGE(_NTLMPayloadPacket):
    ...
    messageType = 2
    OFFSET = 56
    fields_desc = [NTLM_Header, LEShortField('TargetNameLen', None), LEShortField('TargetNameMaxLen', None), LEIntField('TargetNameBufferOffset', None), FlagsField('NegotiateFlags', 0, -32, _negotiateFlags), XStrFixedLenField('ServerChallenge', None, length=8), XStrFixedLenField('Reserved', None, length=8), LEShortField('TargetInfoLen', None), LEShortField('TargetInfoMaxLen', None), LEIntField('TargetInfoBufferOffset', None), _NTLM_Version, _NTLMPayloadField('Payload', OFFSET, [_NTLMStrField('TargetName', b''), PacketListField('TargetInfo', [AV_PAIR()], AV_PAIR)])]
...
class LM_RESPONSE(Packet):
    fields_desc = [StrFixedLenField('Response', b'', length=24)]

class LMv2_RESPONSE(Packet):
    fields_desc = [StrFixedLenField('Response', b'', length=16), StrFixedLenField('ChallengeFromClient', b'', length=8)]

class NTLM_RESPONSE(Packet):
    fields_desc = [StrFixedLenField('Response', b'', length=24)]

class NTLMv2_CLIENT_CHALLENGE(Packet):
    fields_desc = [ByteField('RespType', 0), ByteField('HiRespType', 0), LEShortField('Reserved1', 0), LEIntField('Reserved2', 0), UTCTimeField('TimeStamp', None, fmt='<Q', epoch=[1601, 1, 1, 0, 0, 0], custom_scaling=10000000.0), StrFixedLenField('ChallengeFromClient', b'12345678', length=8), LEIntField('Reserved3', 0), PacketListField('AvPairs', [AV_PAIR()], AV_PAIR)]
...
class NTLM_AUTHENTICATE(_NTLMPayloadPacket):
    ...
    OFFSET = 88
    NTLM_VERSION = 1
    fields_desc = [NTLM_Header, LEShortField('LmChallengeResponseLen', None), LEShortField('LmChallengeResponseMaxLen', None), LEIntField('LmChallengeResponseBufferOffset', None), LEShortField('NtChallengeResponseLen', None), LEShortField('NtChallengeResponseMaxLen', None), LEIntField('NtChallengeResponseBufferOffset', None), LEShortField('DomainNameLen', None), LEShortField('DomainNameMaxLen', None), LEIntField('DomainNameBufferOffset', None), LEShortField('UserNameLen', None), LEShortField('UserNameMaxLen', None), LEIntField('UserNameBufferOffset', None), LEShortField('WorkstationLen', None), LEShortField('WorkstationMaxLen', None), LEIntField('WorkstationBufferOffset', None), LEShortField('EncryptedRandomSessionKeyLen', None), LEShortField('EncryptedRandomSessionKeyMaxLen', None), LEIntField('EncryptedRandomSessionKeyBufferOffset', None), FlagsField('NegotiateFlags', 0, -32, _negotiateFlags), _NTLM_Version, ConditionalField(XStrFixedLenField('MIC', b'', length=16), lambda pkt: pkt.fields.get('MIC', b'') is not None), _NTLMPayloadField('Payload', OFFSET, [MultipleTypeField([(PacketField('LmChallengeResponse', LMv2_RESPONSE(), LMv2_RESPONSE), lambda pkt: pkt.NTLM_VERSION == 2)], PacketField('LmChallengeResponse', LM_RESPONSE(), LM_RESPONSE)), MultipleTypeField([(PacketField('NtChallengeResponse', NTLMv2_RESPONSE(), NTLMv2_RESPONSE), lambda pkt: pkt.NTLM_VERSION == 2)], PacketField('NtChallengeResponse', NTLM_RESPONSE(), NTLM_RESPONSE)), _NTLMStrField('DomainName', b''), _NTLMStrField('UserName', b''), _NTLMStrField('Workstation', b''), XStrField('EncryptedRandomSessionKey', b'')])]
...
class NEGOEX_EXCHANGE_NTLM_ITEM(ASN1_Packet):
    ...
    ASN1_root = ASN1F_SEQUENCE(ASN1F_SEQUENCE(ASN1F_SEQUENCE(ASN1F_OID('oid', ''), ASN1F_PRINTABLE_STRING('token', ''), explicit_tag=49), explicit_tag=128))
...
class NEGOEX_EXCHANGE_NTLM(ASN1_Packet):
    ...
    """
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SEQUENCE(ASN1F_SEQUENCE(ASN1F_SEQUENCE_OF('items', [], NEGOEX_EXCHANGE_NTLM_ITEM), implicit_tag=160))
...
class NTLMSSP_MESSAGE_SIGNATURE(Packet):
    fields_desc = [LEIntField('Version', 1), StrFixedLenField('Checksum', b'', length=8), LEIntField('SeqNum', 0)]
