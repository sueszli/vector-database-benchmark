...
class PlayerUtilities(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def get_active_player_count(self) -> Tuple[Optional[str], int]:
        try:
            current = next((player.current for player in lavalink.active_players() if player.current is not None), None)
            get_single_title = await self.get_track_description_unformatted(current, self.local_folder_current_path)
    ...
    async def update_bot_presence(self, track: Optional[str], playing_servers: int) -> None:
        if playing_servers == 0:
            ...
            await self.bot.change_presence(activity=discord.Activity(name=track, type=discord.ActivityType.listening))
        elif playing_servers > 1:
            await self.bot.change_presence(activity=discord.Activity(name=_('music in {} servers').format(playing_servers), type=discord.ActivityType.playing))
    ...
    async def _skip_action(self, ctx: commands.Context, skip_to_track: int=None) -> None:
        ...
        if not player.current or (not player.queue and (not autoplay)):
            ...
            if player.current.is_stream:
                ...
                embed.set_footer(text=_('Currently livestreaming {track}').format(track=player.current.title))
            else:
                embed = discord.Embed(title=_("There's nothing in the queue."))
                embed.set_footer(text=_('{time} left on {track}').format(time=time_remain, track=player.current.title))
            await self.send_embed_msg(ctx, embed=embed)
            return
        elif autoplay and (not player.queue):
            embed = discord.Embed(title=_('Track Skipped'), description=await self.get_track_description(player.current, self.local_folder_current_path))
            await self.send_embed_msg(ctx, embed=embed)
            await player.skip()
            return
        queue_to_append = []
        if skip_to_track is not None and skip_to_track != 1:
            if skip_to_track < 1:
                await self.send_embed_msg(ctx, title=_('Track number must be equal to or greater than 1.'))
                return
            elif skip_to_track > len(player.queue):
                await self.send_embed_msg(ctx, title=_('There are only {queuelen} songs currently queued.').format(queuelen=len(player.queue)))
                return
            embed = discord.Embed(title=_('{skip_to_track} Tracks Skipped').format(skip_to_track=skip_to_track))
            await self.send_embed_msg(ctx, embed=embed)
            if player.repeat:
                queue_to_append = player.queue[0:min(skip_to_track - 1, len(player.queue) - 1)]
            player.queue = player.queue[min(skip_to_track - 1, len(player.queue) - 1):len(player.queue)]
        else:
            embed = discord.Embed(title=_('Track Skipped'), description=await self.get_track_description(player.current, self.local_folder_current_path))
            await self.send_embed_msg(ctx, embed=embed)
    ...
    async def _get_spotify_tracks(self, ctx: commands.Context, query: Query, forced: bool=False) -> Union[discord.Message, List[lavalink.Track], lavalink.Track]:
        ...
        if any([not api_data['spotify_client_id'], not api_data['spotify_client_secret']]):
            return await self.send_embed_msg(ctx, title=_('Invalid Environment'), description=_('The owner needs to set the Spotify client ID and Spotify client secret, before Spotify URLs or codes can be used. \nSee `{prefix}audioset spotifyapi` for instructions.').format(prefix=ctx.prefix))
        elif not api_data['youtube_api']:
            return await self.send_embed_msg(ctx, title=_('Invalid Environment'), description=_('The owner needs to set the YouTube API key before Spotify URLs or codes can be used.\nSee `{prefix}audioset youtubeapi` for instructions.').format(prefix=ctx.prefix))
        try:
        ...
        if query.single_track:
            try:
                res = await self.api_interface.spotify_query(ctx, 'track', query.id, skip_youtube=True, notifier=None)
                if not res:
                    title = _('Nothing found.')
                    embed = discord.Embed(title=title)
                    if query.is_local and query.suffix in _PARTIALLY_SUPPORTED_MUSIC_EXT:
                        title = _('Track is not playable.')
                        description = _('**{suffix}** is not a fully supported format and some tracks may not play.').format(suffix=query.suffix)
                        embed = discord.Embed(title=title, description=description)
                    return await self.send_embed_msg(ctx, embed=embed)
            except SpotifyFetchError as error:
                self.update_player_lock(ctx, False)
                return await self.send_embed_msg(ctx, title=error.message.format(prefix=ctx.prefix))
            ...
            try:
                ...
                self.update_player_lock(ctx, False)
                return await self.send_embed_msg(ctx, title=_('Invalid Environment'), description=_('The Spotify API key or client secret has not been set properly. \nUse `{prefix}audioset spotifyapi` for instructions.').format(prefix=ctx.prefix))
            except Exception as e:
                self.update_player_lock(ctx, False)
                raise e
            ...
            try:
                ...
                track_list = await self.fetch_spotify_playlist(ctx, 'album' if query.is_album else 'playlist', query, enqueue_tracks, forced=forced)
            finally:
                self.update_player_lock(ctx, False)
            return track_list
        else:
            return await self.send_embed_msg(ctx, title=_('Unable To Find Tracks'), description=_("This doesn't seem to be a supported Spotify URL or code."))

    async def _enqueue_tracks(self, ctx: commands.Context, query: Union[Query, list], enqueue: bool=True) -> Union[discord.Message, List[lavalink.Track], lavalink.Track]:
        player = lavalink.get_player(ctx.guild.id)
        try:
            if self.play_lock[ctx.guild.id]:
                return await self.send_embed_msg(ctx, title=_('Unable To Get Tracks'), description=_('Wait until the playlist has finished loading.'))
        ...
        if type(query) is not list:
            if not await self.is_query_allowed(self.config, ctx, f'{query}', query_obj=query):
                raise QueryUnauthorized(_('{query} is not an allowed query.').format(query=query.to_string_user()))
            if query.single_track:
            ...
            if not tracks:
                ...
                if await self.config.use_external_lavalink() and query.is_local:
                    embed.description = _('Local tracks will not work if the `Lavalink.jar` cannot see the track.\nThis may be due to permissions or because Lavalink.jar is being run in a different machine than the local tracks.')
        ...
        if not first_track_only and len(tracks) > 1:
            ...
            async for track in AsyncIter(tracks):
                ...
                if not await self.is_query_allowed(self.config, ctx, f'{track.title} {track.author} {track.uri} {str(track_query)}', query_obj=track_query):
            ...
            if len(tracks) > track_len:
                maxlength_msg = _(' {bad_tracks} tracks cannot be queued.').format(bad_tracks=len(tracks) - track_len)
            else:
                maxlength_msg = ''
            playlist_name = escape(playlist_data.name if playlist_data else _('No Title'), formatting=True)
            title = _('Playlist Enqueued') if not query.is_album else _('Album Enqueued')
            embed = discord.Embed(description=bold(f'[{playlist_name}]({playlist_url})', False) if playlist_url else playlist_name, title=title)
            embed.set_footer(text=_('Added {num} tracks to the queue.{maxlength_msg}').format(num=track_len, maxlength_msg=maxlength_msg))
            if not guild_data['shuffle'] and queue_dur > 0:
                embed.set_footer(text=_('{time} until start of playlist playback: starts at #{position} in queue').format(time=queue_total_duration, position=before_queue_length + 1))
            ...
            if not guild_data['shuffle'] and queue_dur > 0:
                embed.set_footer(text=_('{time} until track playback: #{position} in queue').format(time=queue_total_duration, position=before_queue_length + 1))
        if not player.current:
            await player.play()
        self.update_player_lock(ctx, False)
        message = await self.send_embed_msg(ctx, embed=embed)
    ...
    async def fetch_spotify_playlist(self, ctx: commands.Context, stype: str, query: Query, enqueue: bool=False, forced: bool=False):
        player = lavalink.get_player(ctx.guild.id)
        try:
            embed1 = discord.Embed(title=_('Please wait, finding tracks...'))
            playlist_msg = await self.send_embed_msg(ctx, embed=embed1)
            notifier = Notifier(ctx, playlist_msg, {'spotify': _('Getting track {num}/{total}...'), 'youtube': _('Matching track {num}/{total}...'), 'lavalink': _('Loading track {num}/{total}...'), 'lavalink_time': _('Approximate time remaining: {seconds}')})
            track_list = await self.api_interface.spotify_enqueue(ctx, stype, query.id, enqueue=enqueue, player=player, lock=self.update_player_lock, notifier=notifier, forced=forced, query_global=self.global_api_user.get('can_read'))
        except SpotifyFetchError as error:
            self.update_player_lock(ctx, False)
            return await self.send_embed_msg(ctx, title=_('Invalid Environment'), description=error.message.format(prefix=ctx.prefix))
        except TrackEnqueueError:
            self.update_player_lock(ctx, False)
            return await self.send_embed_msg(ctx, title=_('Unable to Get Track'), description=_("I'm unable to get a track from Lavalink at the moment, try again in a few minutes."), error=True)
        except (RuntimeError, aiohttp.ServerDisconnectedError):
            self.update_player_lock(ctx, False)
            error_embed = discord.Embed(title=_('The connection was reset while loading the playlist.'))
    ...
    async def maybe_move_player(self, ctx: commands.Context) -> bool:
        try:
            ...
            return False
        try:
            in_channel = sum((not m.bot for m in ctx.guild.get_member(self.bot.user.id).voice.channel.members))
        except AttributeError:
            return False
        ...
        if in_channel == 0 and user_channel:
            if player.channel != user_channel and (not player.current) and (player.position == 0) and (len(player.queue) == 0):
                await player.move_to(user_channel, self_deaf=await self.config.guild_from_id(ctx.guild.id).auto_deafen())
                return True
        else:
            return False
