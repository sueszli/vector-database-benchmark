        ...
        for mode in [fec.CC_TRUNCATED, fec.CC_TERMINATED, fec.CC_TAILBITING]:
            for packed in [True, False]:
                for rev_pack in [True, False]:
                    with self.subTest(mode=mode, packed=packed, rev_pack=rev_pack):
                        enc = fec.cc_encoder_make(frame_size * 8, k, rate, polys, mode=mode)
                        dec = fec.cc_decoder.make(frame_size * 8, k, rate, polys, mode=mode)
