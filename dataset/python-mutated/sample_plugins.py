...
from ...payment.interface import PaymentGatewayData, TransactionSessionData, TransactionSessionResult
...
class PluginSample(BasePlugin):
    ...
    DEFAULT_CONFIGURATION = [{'name': 'Username', 'value': 'admin'}, {'name': 'Password', 'value': None}, {'name': 'Use sandbox', 'value': False}, {'name': 'API private key', 'value': None}]
    CONFIG_STRUCTURE = {'Username': {'type': ConfigurationTypeField.STRING, 'help_text': 'Username input field', 'label': 'Username'}, 'Password': {'type': ConfigurationTypeField.PASSWORD, 'help_text': 'Password input field', 'label': 'Password'}, 'Use sandbox': {'type': ConfigurationTypeField.BOOLEAN, 'help_text': 'Use sandbox', 'label': 'Use sandbox'}, 'API private key': {'type': ConfigurationTypeField.SECRET, 'help_text': 'API key', 'label': 'Private key'}, 'certificate': {'type': ConfigurationTypeField.SECRET_MULTILINE, 'help_text': '', 'label': 'Multiline certificate'}}
...
class ChannelPluginSample(PluginSample):
    ...
    CONFIG_STRUCTURE = {'input-per-channel': {'type': ConfigurationTypeField.STRING, 'help_text': 'Test input', 'label': 'Input per channel'}}
    ...
    def get_payment_config(self, previous_value):
        if False:
            while True:
                i = 10
        return self.CLIENT_CONFIG
ACTIVE_PLUGINS = (ChannelPluginSample, ActivePaymentGateway, ActivePlugin, ActiveDummyPaymentGateway)
INACTIVE_PLUGINS = (InactivePaymentGateway, PluginInactive, InactiveChannelPluginSample)
