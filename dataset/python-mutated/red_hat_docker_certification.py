...
if __name__ == '__main__':
    ...
    args.add_argument('digest', help='Digest (typically SHA256) of the Docker image intended for certification', metavar='D', type=str)
    args.add_argument('--pid', required=True, help='Red Hat Project ID (PID). To be found in connect.redhat.com', type=str)
    args.add_argument('--api_key', required=True, help='API Key for Red Hat Connect Portal API', type=str)
    args.add_argument('--tag', required=True, help='An existing tag of the docker image published. Make sure the docker image was pushed with this tag for scanning.')
