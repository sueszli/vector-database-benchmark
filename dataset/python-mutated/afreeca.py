...
@pluginmatcher(re.compile('https?://play\\.afreecatv\\.com/(?P<username>\\w+)(?:/(?P<bno>:\\d+))?'))
@pluginargument('username', sensitive=True, requires=['password'], metavar='USERNAME', help='The username used to register with afreecatv.com.')
@pluginargument('password', sensitive=True, metavar='PASSWORD', help='A afreecatv.com account password to use with --afreeca-username.')
@pluginargument('purge-credentials', action='store_true', help='Purge cached AfreecaTV credentials to initiate a new session and reauthenticate.')
class AfreecaTV(Plugin):
    _re_bno = re.compile('var nBroadNo = (?P<bno>\\d+);')
    ...
    _schema_channel = validate.Schema({'CHANNEL': {'RESULT': validate.transform(int), validate.optional('BPWD'): str, validate.optional('BNO'): str, validate.optional('RMD'): str, validate.optional('AID'): str, validate.optional('CDN'): str}}, validate.get('CHANNEL'))
    _schema_stream = validate.Schema({validate.optional('view_url'): validate.url(scheme=validate.any('rtmp', 'http')), 'stream_status': str})
