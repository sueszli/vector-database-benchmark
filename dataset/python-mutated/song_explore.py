...
from PyQt5.QtWidgets import QWidget, QHBoxLayout, QLabel, QVBoxLayout, QSizePolicy, QScrollArea, QFrame
from feeluown.excs import ProviderNotFound, ResourceNotFound
from feeluown.library import SupportsSongHotComments, SupportsSongSimilar, SupportsSongWebUrl, NotSupported, ModelFlags
...
from feeluown.gui.widgets.song_minicard_list import SongMiniCardListDelegate, SongMiniCardListModel, SongMiniCardListView
if TYPE_CHECKING:
    from feeluown.app.gui_app import GuiApp
logger = logging.getLogger(__name__)
err_msg_tpl = '<p style="color: grey; font: small;">该提供方暂不支持{feature}。<br/> 给它实现一下 {interface} 接口来支持该功能吧 ~</p>'
...
class SongWikiLabel(QLabel):
    ...
    async def show_song(self, song, album):
        ...
        for artist in song.artists:
            artists_str_list.append(f'<a href="{reverse(artist)}">{artist.name_display}</a>')
        ...
        album_str = f'<a href="{reverse(album)}">{album.name_display}</a>' if album else ''
        kvs = [('歌手', artists_str), ('所属专辑', or_unknown(album_str)), ('发行日期', or_unknown(date_fmted)), ('曲风', or_unknown(song.genre))]
        lines = []
        for (k, v) in kvs:
...
class SongExploreView(QWidget):
    ...
    async def maybe_show_song_similar(self, provider, song):
        if isinstance(provider, SupportsSongSimilar):
            ...
            model = SongMiniCardListModel(create_reader(songs), fetch_cover_wrapper(self._app))
            self.similar_songs_view.setModel(model)
        else:
            msg = err_msg_tpl.format(feature='查看相似歌曲', interface=SupportsSongSimilar.__name__)
    ...
    async def maybe_show_song_hot_comments(self, provider, song):
        if isinstance(provider, SupportsSongHotComments):
            ...
            msg = err_msg_tpl.format(feature='查看歌曲评论', interface=SupportsSongHotComments.__name__)
    ...
    async def maybe_show_song_lyric(self, song):
        if self._app.playlist.current_song == song:
            ...
            self._app.live_lyric.line_changed.connect(self.lyric_view.on_line_changed, weak=True)
            return
        try:
    ...
    async def maybe_show_song_pic(self, song, album):
        if album:
            aio.run_afn(self.cover_label.show_cover, album.cover, reverse(album) + '/cover')
        elif ModelFlags.v2 in song.meta.flags:
            aio.run_afn(self.cover_label.show_cover, song.pic_url, reverse(song) + '/pic_url')
