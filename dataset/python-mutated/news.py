...
class NewsArticles(datasets.base.RemoteDataset, BanditDataset):
    ...
    def __init__(self):
        if False:
            while True:
                i = 10
        super().__init__(url='http://www.cs.columbia.edu/~jebara/6998/dataset.txt', size=2149159, filename='dataset.txt', unpack=False, directory=pathlib.Path(__file__).parent, n_features=100, n_samples=10000)
