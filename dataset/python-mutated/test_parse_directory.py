...
PARSE = 1
NOTREE = 0
argparser = argparse.ArgumentParser(prog='test_parse_directory', description='Helper program to test directories or files for pegen')
argparser.add_argument('-d', '--directory', help='Directory path containing files to test')
argparser.add_argument('-e', '--exclude', action='append', default=[], help='Glob(s) for matching files to exclude')
argparser.add_argument('-s', '--short', action='store_true', help='Only show errors, in a more Emacs-friendly format')
argparser.add_argument('-v', '--verbose', action='store_true', help='Display detailed errors for failures')
