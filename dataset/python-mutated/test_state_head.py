...
async def test_max_concurrent_in_progress_functions(extra_req_num):
    ...
    res_arr = await asyncio.gather(*[a.fn1() if i % 2 == 0 else a.fn2() for i in range(max_req + extra_req_num)])
    fail_cnt = 0
    for ok in res_arr:
        fail_cnt += 0 if ok else 1
    expected_fail_cnt = max(0, extra_req_num)
    assert fail_cnt == expected_fail_cnt, f'{expected_fail_cnt} out of {max_req + extra_req_num} concurrent runs should fail with max={max_req} but {fail_cnt}.'
    assert a.num_call_ == 0, 'All requests should be done'

@pytest.mark.asyncio
@pytest.mark.parametrize('failures', [[True, True, True, True, True], [False, False, False, False, False], [False, True, False, True, False], [False, False, False, True, True], [True, True, False, False, False]])
async def test_max_concurrent_with_exceptions(failures):
    max_req = 10
    a = A(max_num_call=max_req)
    res_arr = await asyncio.gather(*[a.fn1(err=should_throw_err) for should_throw_err in failures], return_exceptions=True)
