...
class SearchRocketReach:
    ...
    async def do_search(self) -> None:
        try:
            headers = {'Api-Key': self.key, 'Content-Type': 'application/json', 'User-Agent': Core.get_user_agent()}
            next_page = 1
            for count in range(1, self.limit):
                ...
                if 'detail' in result.keys() and 'Request was throttled.' in result['detail']:
                    print(f"RocketReach requests have been throttled; {result['detail'].split(' ', 3)[-1].replace('available', 'availability')}")
                    break
