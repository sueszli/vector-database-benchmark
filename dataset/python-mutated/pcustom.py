...
import tempfile
import pathlib
from tests.utils import gdb_run_cmd, gdb_run_silent_cmd, is_64b, _target, GEF_DEFAULT_TEMPDIR, GefUnitTestGeneric
    ...
    def test_cmd_pcustom(self):
        ...
        with tempfile.TemporaryDirectory(prefix=GEF_DEFAULT_TEMPDIR) as dd:
            ...
            with tempfile.NamedTemporaryFile(dir=dirpath, suffix='.py') as fd:
                ...
                res = gdb_run_cmd('pcustom meh_t 0x1337100', before=[f'gef config pcustom.struct_path {dirpath}'])
                self.assertNoException(res)
    ...
    def test_cmd_pcustom_show(self):
        ...
        with tempfile.TemporaryDirectory(prefix=GEF_DEFAULT_TEMPDIR) as dd:
            ...
            with tempfile.NamedTemporaryFile(dir=dirpath, suffix='.py') as fd:
                ...
                fd.seek(0)
                fd.flush()
                res = gdb_run_cmd('pcustom foo_t', before=[f'gef config pcustom.struct_path {dirpath}'])
                self.assertNoException(res)
                if is_64b():
                ...
                res = gdb_run_silent_cmd('pcustom goo_t 0x1337100', target=_target('pcustom'), before=[f'gef config pcustom.struct_path {dirpath}'])
                self.assertNoException(res)
                if is_64b():
                ...
                res = gdb_run_cmd('pcustom meh_t', before=[f'gef config pcustom.struct_path {dirpath}'])
                self.assertNoException(res)
                self.assertIn("No structure named 'meh_t' found", res)
                res = gdb_run_silent_cmd('pcustom meh_t 0x1337100', target=_target('pcustom'), before=[f'gef config pcustom.struct_path {dirpath}'])
                self.assertNoException(res)
                self.assertIn("No structure named 'meh_t' found", res)