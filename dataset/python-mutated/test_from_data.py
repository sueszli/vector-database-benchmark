...
@testing.parameterize(*testing.product({'ver': tuple(range(max_cuda_array_interface_version + 1)), 'strides': (False, None, True)}))
...
@testing.parameterize(*testing.product({'ver': tuple(range(1, max_cuda_array_interface_version + 1)), 'strides': (False, None, True)}))
@pytest.mark.skipif(cupy.cuda.runtime.is_hip, reason='HIP does not support this')
...
@testing.parameterize(*testing.product({'ndmin': [0, 1, 2, 3], 'copy': [True, False], 'xp': [numpy, cupy]}))
