...
class RandomState(object):
    ...
    def hypergeometric(self, ngood, nbad, nsample, size=None, dtype=int):
        ...
        self._update_seed(y.size)
        return y
    _laplace_kernel = _core.ElementwiseKernel('T x, T loc, T scale', 'T y', 'y = loc + scale * ((x <= 0.5) ? log(x + x): -log(x + x - 1.0))', 'cupy_laplace_kernel')
    ...
    def rayleigh(self, scale=1.0, size=None, dtype=float):
        ...
        return x
    _interval_upper_limit = _core.ElementwiseKernel('T max, T mx', 'T out', 'out = max - (mx != max ? (max - mx) % (mx + 1) : 0)', 'cupy_random_interval_upper_limit')
    _interval_sample_modulo = _core.ElementwiseKernel('T max, T mx', 'T sample', 'if (mx != max) { sample %= mx + 1; }', 'cupy_random_interval_sample_modulo')
    ...
    def _get_indices(self, sample, upper_limit, cond):
        ...
        return indices
    _kernel_get_indices = _core.ElementwiseKernel('raw U csum', 'raw U indices', '\n        int j = 0;\n        if (i > 0) { j = csum[i-1]; }\n        if (csum[i] > j) { indices[j] = i; }\n        ', 'cupy_get_indices')
    ...
    _scale_kernel = _core.ElementwiseKernel('T low, T high', 'T x', 'x = T(low) + x * T(high - low)', 'cupy_scale')
    ...
    def vonmises(self, mu, kappa, size=None, dtype=float):
        ...
        self._update_seed(y.size)
        return y
    _wald_kernel = _core.ElementwiseKernel('T mean, T scale, T U', 'T X', '\n            T mu_2l;\n            T Y;\n            mu_2l = mean / (2*scale);\n            Y = mean*X*X;\n            X = mean + mu_2l*(Y - sqrt(4*scale*Y + Y*Y));\n            if (U > mean/(mean+X))\n            {\n                X = mean*mean/X;\n            }\n        ', 'cupy_wald_scale')
    ...
    def _permutation(self, num):
        ...
        return array
    _gumbel_kernel = _core.ElementwiseKernel('T x, T loc, T scale', 'T y', 'y = T(loc) - log(-log(x)) * T(scale)', 'cupy_gumbel_kernel')
