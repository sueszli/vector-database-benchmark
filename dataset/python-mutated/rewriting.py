...
log2_opt = ReplaceOptim(_v * log(_w) / log(2), _v * log2(_w), cost_function=lambda expr: expr.count(lambda e: e.is_Pow and e.exp.is_negative or (isinstance(e, (log, log2)) and (not e.args[0].is_number))))
log2const_opt = ReplaceOptim(log(2) * log2(_w), log(_w))
logsumexp_2terms_opt = ReplaceOptim(lambda l: isinstance(l, log) and l.args[0].is_Add and (len(l.args[0].args) == 2) and all((isinstance(t, exp) for t in l.args[0].args)), lambda l: Max(*[e.args[0] for e in l.args[0].args]) + log1p(exp(Min(*[e.args[0] for e in l.args[0].args]))))
...
log1p_opt = ReplaceOptim(lambda e: isinstance(e, log), lambda l: expand_log(l.replace(log, lambda arg: log(arg.factor()))).replace(log(_u + 1), log1p(_u)))
