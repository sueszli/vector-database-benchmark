...
class MenuItemCreateInput(MenuItemInput):
    ...
    parent = graphene.ID(description='ID of the parent menu. If empty, menu will be top level menu.', name='parent')

class MenuItemCreate(ModelMutation):

    class Arguments:
        input = MenuItemCreateInput(required=True, description='Fields required to update a menu item. Only one of `url`, `category`, `page`, `collection` is allowed per item.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.MENU_ITEM_CREATED, description='A menu item was created.')]
