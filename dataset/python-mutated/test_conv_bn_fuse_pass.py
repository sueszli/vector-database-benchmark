...
class TestConvBnFusePass(PassAutoScanTest):
    ...
    def sample_predictor_configs(self, program_config):
        ...
        if program_config.ops[0].attrs['use_mkldnn']:
            ...
            config = self.create_inference_config()
            yield (config, ['conv2d', 'elementwise_add'], (1e-05, 1e-05))
            ...
            yield (config, ['conv2d_fusion'], (1e-05, 1e-05))
    ...
    def add_ignore_pass_case(self):
        ...
        def teller1(program_config, predictor_config):
            if False:
                for i in range(10):
                    print('nop')
            if program_config.ops[0].attrs['data_format'] == 'NHWC' and (not predictor_config.mkldnn_enabled()):
                return True
            return False
        self.add_ignore_check_case(teller1, IgnoreReasons.PASS_ACCURACY_ERROR, 'The output format of conv2d is wrong when data_format attribute is NHWC')

    def test(self):
        if False:
            for i in range(10):
                print('nop')
        self.run_and_statis(quant=False, passes=['conv_bn_fuse_pass'])
