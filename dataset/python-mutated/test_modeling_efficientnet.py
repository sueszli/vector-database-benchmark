...
class EfficientNetModelTester:
    ...
    def __init__(self, parent, batch_size=13, image_size=32, num_channels=3, kernel_sizes=[3, 3, 5], in_channels=[32, 16, 24], out_channels=[16, 24, 20], strides=[1, 1, 2], num_block_repeats=[1, 1, 2], expand_ratios=[1, 6, 6], is_training=True, use_labels=True, intermediate_size=37, hidden_act='gelu', num_labels=10):
        if False:
            while True:
                i = 10
        self.parent = parent
    ...
    def get_config(self):
        if False:
            for i in range(10):
                print('nop')
        return EfficientNetConfig(num_channels=self.num_channels, kernel_sizes=self.kernel_sizes, in_channels=self.in_channels, out_channels=self.out_channels, strides=self.strides, num_block_repeats=self.num_block_repeats, expand_ratios=self.expand_ratios, hidden_act=self.hidden_act, num_labels=self.num_labels)
    ...
    def create_and_check_model(self, config, pixel_values, labels):
        ...
        self.parent.assertEqual(result.last_hidden_state.shape, (self.batch_size, config.hidden_dim, self.image_size // 4, self.image_size // 4))
...
class EfficientNetModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': EfficientNetModel, 'image-classification': EfficientNetForImageClassification} if is_torch_available() else {}
    fx_compatible = False
    test_pruning = False
    ...
    def test_hidden_states_output(self):
        ...
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            self.assertListEqual(list(hidden_states[0].shape[-2:]), [self.model_tester.image_size // 2, self.model_tester.image_size // 2])
