...
from prowler.providers.gcp.services.cloudresourcemanager.cloudresourcemanager_client import cloudresourcemanager_client
...
class iam_sa_no_administrative_privileges(Check):
    ...
    def execute(self) -> Check_Report_GCP:
        ...
        for account in iam_client.service_accounts:
            ...
            report.status = 'PASS'
            report.status_extended = f'Account {account.email} has no administrative privileges.'
            for binding in cloudresourcemanager_client.bindings:
                if f'serviceAccount:{account.email}' in binding.members and ('admin' in binding.role.lower() or 'owner' in binding.role.lower() or 'editor' in binding.role.lower()):
                    report.status = 'FAIL'
