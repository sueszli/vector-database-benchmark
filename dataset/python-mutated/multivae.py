...
if __name__ == '__main__':
    ...
    engine = dl.Engine()
    hparams = {'anneal_cap': 0.2, 'total_anneal_steps': 6000}
    callbacks = [dl.NDCGCallback('logits', 'targets', [20, 50, 100]), dl.MAPCallback('logits', 'targets', [20, 50, 100]), dl.MRRCallback('logits', 'targets', [20, 50, 100]), dl.HitrateCallback('logits', 'targets', [20, 50, 100]), dl.BackwardCallback('loss'), dl.OptimizerCallback('loss', accumulation_steps=1), dl.SchedulerCallback()]
    runner = RecSysRunner()
    runner.train(model=model, optimizer=optimizer, engine=engine, hparams=hparams, scheduler=lr_scheduler, loaders=loaders, num_epochs=100, verbose=True, timeit=False, callbacks=callbacks, logdir='./logs_multivae')