...
if __name__ == '__main__':
    ...
    config = PPOConfig().environment('open_spiel_env').framework(args.framework).callbacks(LeagueBasedSelfPlayCallback).rollouts(num_envs_per_worker=5).training(num_sgd_iter=20).multi_agent(policies={'main': PolicySpec(), 'main_0': PolicySpec(), 'main_exploiter_0': PolicySpec(policy_class=RandomPolicy), 'main_exploiter_1': PolicySpec(), 'league_exploiter_0': PolicySpec(policy_class=RandomPolicy), 'league_exploiter_1': PolicySpec()}, policy_mapping_fn=policy_mapping_fn, policies_to_train=['main']).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    stop = {'timesteps_total': args.stop_timesteps, 'training_iteration': args.stop_iters}
    results = None
    if not args.from_checkpoint:
        results = tune.Tuner('PPO', param_space=config, run_config=air.RunConfig(stop=stop, checkpoint_config=air.CheckpointConfig(checkpoint_at_end=True, checkpoint_frequency=10), verbose=3)).fit()
