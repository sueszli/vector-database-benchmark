...
@pluginmatcher(re.compile('https?://fanxing\\.kugou\\.com/(?P<room_id>\\d+)'))
class Kugou(Plugin):
    _roomid_re = re.compile("roomId:\\s*'(\\d+)'")
    _room_stream_list_schema = validate.Schema({'data': validate.any(None, {'httpflv': validate.url()})}, validate.get('httpflv_room_stream_list_schema'))
    _stream_hv_schema = validate.Schema(validate.any(None, [{'httpshls': [validate.url()], 'httpsflv': [validate.url()]}]))
    _stream_data_schema = validate.Schema({'msg': str, 'code': int, 'data': {'status': int, 'vertical': _stream_hv_schema, 'horizontal': _stream_hv_schema, 'roomId': int}})
