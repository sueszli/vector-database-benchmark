...
from ..const import ATTR_ATTRIBUTE_ID, ATTR_ATTRIBUTE_NAME, ATTR_VALUE, REPORT_CONFIG_ASAP, REPORT_CONFIG_DEFAULT, REPORT_CONFIG_IMMEDIATE, REPORT_CONFIG_MAX_INT, REPORT_CONFIG_MIN_INT, SIGNAL_ATTR_UPDATED, UNKNOWN
from . import AttrReportConfig, ClientClusterHandler, ClusterHandler
if TYPE_CHECKING:
    from ..endpoint import Endpoint
_LOGGER = logging.getLogger(__name__)

@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(registries.SMARTTHINGS_HUMIDITY_CLUSTER)
...
@registries.CLUSTER_HANDLER_ONLY_CLUSTERS.register(64768)
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(64768)
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(registries.TUYA_MANUFACTURER_CLUSTER)
...
@registries.CLUSTER_HANDLER_ONLY_CLUSTERS.register(64704)
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(64704)
class OppleRemote(ClusterHandler):
    ...
    async def async_initialize_cluster_handler_specific(self, from_cache: bool) -> None:
        ...
        if self.cluster.endpoint.model in ('lumi.motion.ac02', 'lumi.motion.agl04'):
            interval = self.cluster.get('detection_interval', self.cluster.get(258))
            if interval is not None:
                self.debug('Loaded detection interval at startup: %s', interval)
                self.cluster.endpoint.ias_zone.reset_s = int(interval)

@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(registries.SMARTTHINGS_ACCELERATION_CLUSTER)
class SmartThingsAcceleration(ClusterHandler):
    """Smart Things Acceleration cluster handler."""
    REPORT_CONFIG = (AttrReportConfig(attr='acceleration', config=REPORT_CONFIG_ASAP), AttrReportConfig(attr='x_axis', config=REPORT_CONFIG_ASAP), AttrReportConfig(attr='y_axis', config=REPORT_CONFIG_ASAP), AttrReportConfig(attr='z_axis', config=REPORT_CONFIG_ASAP))
...
@registries.CLIENT_CLUSTER_HANDLER_REGISTRY.register(64561)
...
class InovelliConfigEntityClusterHandler(ClusterHandler):
    ...
    async def issue_individual_led_effect(self, led_number: int=1, effect_type: SingleLEDEffectType | int=SingleLEDEffectType.Fast_Blink, color: int=200, level: int=100, duration: int=3, **kwargs: Any) -> None:
        """Issue individual LED effect command.

        This command is used to issue an LED effect to the specified LED on the device.
        """
        await self.individual_led_effect(led_number, effect_type, color, level, duration, expect_reply=False)

@registries.CLUSTER_HANDLER_ONLY_CLUSTERS.register(registries.IKEA_AIR_PURIFIER_CLUSTER)
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(registries.IKEA_AIR_PURIFIER_CLUSTER)
...
@registries.CLUSTER_HANDLER_ONLY_CLUSTERS.register(64640)
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(64640)
