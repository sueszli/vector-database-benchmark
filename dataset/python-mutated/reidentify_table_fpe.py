...
def reidentify_table_with_fpe(project: str, table_header: List[str], table_rows: List[List[str]], reid_field_names: List[str], key_name: str=None, wrapped_key: bytes=None, alphabet: str=None) -> None:
    if False:
        i = 10
        return i + 15
    ...
    reidentify_config = {'record_transformations': {'field_transformations': [{'primitive_transformation': {'crypto_replace_ffx_fpe_config': crypto_replace_ffx_fpe_config}, 'fields': reid_field_names}]}}
    parent = f'projects/{project}/locations/global'
    response = dlp.reidentify_content(request={'parent': parent, 'reidentify_config': reidentify_config, 'item': item})
...
if __name__ == '__main__':
    ...
    parser.add_argument('reid_field_names', help='A list of fields in table to re-identify.')
    ...
    reidentify_table_with_fpe(args.project, args.table_header, args.table_rows, args.reid_field_names, wrapped_key=base64.b64decode(args.wrapped_key), key_name=args.key_name, alphabet=args.alphabet)