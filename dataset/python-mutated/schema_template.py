...
from homeassistant.components.light import ATTR_BRIGHTNESS, ATTR_COLOR_TEMP, ATTR_EFFECT, ATTR_FLASH, ATTR_HS_COLOR, ATTR_TRANSITION, ENTITY_ID_FORMAT, ColorMode, LightEntity, LightEntityFeature, filter_supported_color_modes
from homeassistant.const import CONF_NAME, CONF_OPTIMISTIC, CONF_STATE_TEMPLATE, STATE_OFF, STATE_ON
...
from ..const import CONF_COMMAND_TOPIC, CONF_ENCODING, CONF_QOS, CONF_RETAIN, CONF_STATE_TOPIC, PAYLOAD_NONE
...
VALUE_TEMPLATES = (CONF_BLUE_TEMPLATE, CONF_BRIGHTNESS_TEMPLATE, CONF_COLOR_TEMP_TEMPLATE, CONF_EFFECT_TEMPLATE, CONF_GREEN_TEMPLATE, CONF_RED_TEMPLATE, CONF_STATE_TEMPLATE)
PLATFORM_SCHEMA_MODERN_TEMPLATE = MQTT_RW_SCHEMA.extend({vol.Optional(CONF_BLUE_TEMPLATE): cv.template, vol.Optional(CONF_BRIGHTNESS_TEMPLATE): cv.template, vol.Optional(CONF_COLOR_TEMP_TEMPLATE): cv.template, vol.Required(CONF_COMMAND_OFF_TEMPLATE): cv.template, vol.Required(CONF_COMMAND_ON_TEMPLATE): cv.template, vol.Optional(CONF_EFFECT_LIST): vol.All(cv.ensure_list, [cv.string]), vol.Optional(CONF_EFFECT_TEMPLATE): cv.template, vol.Optional(CONF_GREEN_TEMPLATE): cv.template, vol.Optional(CONF_MAX_MIREDS): cv.positive_int, vol.Optional(CONF_MIN_MIREDS): cv.positive_int, vol.Optional(CONF_NAME): vol.Any(cv.string, None), vol.Optional(CONF_RED_TEMPLATE): cv.template, vol.Optional(CONF_STATE_TEMPLATE): cv.template}).extend(MQTT_ENTITY_COMMON_SCHEMA.schema).extend(MQTT_LIGHT_SCHEMA_SCHEMA.schema)
DISCOVERY_SCHEMA_TEMPLATE = vol.All(PLATFORM_SCHEMA_MODERN_TEMPLATE.extend({}, extra=vol.REMOVE_EXTRA))
...
class MqttLightTemplate(MqttEntity, LightEntity, RestoreEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if ATTR_HS_COLOR in kwargs:
            ...
            if CONF_BRIGHTNESS_TEMPLATE in self._config:
                ...
                brightness = kwargs.get(ATTR_BRIGHTNESS, self._attr_brightness if self._attr_brightness is not None else 255)
        ...
        await self.async_publish(str(self._topics[CONF_COMMAND_TOPIC]), self._command_templates[CONF_COMMAND_ON_TEMPLATE](None, values), self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
        if self._optimistic:
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        if ATTR_TRANSITION in kwargs:
            values['transition'] = kwargs[ATTR_TRANSITION]
        await self.async_publish(str(self._topics[CONF_COMMAND_TOPIC]), self._command_templates[CONF_COMMAND_OFF_TEMPLATE](None, values), self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
        if self._optimistic:
            self.async_write_ha_state()