...
async def test_events(rest_manager, notifier: Notifier):
    ...
    notifier[notifications.tunnel_removed](circuit_id=1234, bytes_up=0, bytes_down=0, uptime=1000, additional_info='test')
    ...
    messages_to_wait_for = {'channel_entity_updated', 'watch_folder_corrupt_file', 'tribler_new_version', 'channel_discovered', 'torrent_finished', 'low_space', 'tunnel_removed', 'remote_query_results', 'tribler_exception'}
...
async def test_should_skip_message(events_endpoint):
    assert not events_endpoint._shutdown and (not events_endpoint.events_responses)
    message = Mock()
