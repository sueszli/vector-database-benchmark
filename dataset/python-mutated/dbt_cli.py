...
from dbt.main import DBTArgumentParser, _add_common_arguments, _add_defer_arguments, _add_selection_arguments, _add_table_mutability_arguments, _build_base_subparser, _build_build_subparser, _build_clean_subparser, _build_compile_subparser, _build_debug_subparser, _build_deps_subparser, _build_docs_generate_subparser, _build_docs_serve_subparser, _build_docs_subparser, _build_init_subparser, _build_list_subparser, _build_parse_subparser, _build_run_operation_subparser, _build_run_subparser, _build_seed_subparser, _build_snapshot_subparser, _build_source_freshness_subparser, _build_source_subparser, _build_test_subparser, adapter_management, read_user_config
...
class DBTCli:
    ...
    def invoke(self) -> Tuple[Any, bool]:
        ...
        if self.__logger:
            self.__logger.info('Invoke dbt with the following arguments:\n' + 'dbt ' + ' '.join(self.__args).replace(' --', ' \\\n  --'))
        user_config = read_user_config(flags.PROFILES_DIR)
        flags.set_from_args(self.__parsed_args, user_config)
        dbt.tracking.initialize_from_flags()
        dbt_logger = LoggerConfig(name='mage_dbt', use_colors=True, logger=self.__logger)
    ...
    def to_pandas(self) -> Tuple[Optional[pd.DataFrame], Any, bool]:
        ...
        if len(self.__result.results) >= 1 and hasattr(self.__result.results[0], 'agate_table'):
            df = pd.DataFrame(data=self.__result.results[0].agate_table.rows, columns=self.__result.results[0].agate_table.column_names)
        return (df, self.__result, self.__success)

    def __build_show_subparser(self, subparsers: argparse.ArgumentParser, base_subparser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        if False:
            for i in range(10):
                print('nop')
        ...
        show_sub = subparsers.add_parser('show', parents=[base_subparser], help='Generates executable SQL for a named resource or inline query,\n            runs that SQL, and returns a preview of the results.\n            Does not materialize anything to the warehouse.')
        show_sub.add_argument('--inline', help='Pass SQL inline to dbt compile and show')
        show_sub.add_argument('--limit', type=int, default=5, help='Limit the number of results returned by dbt show')
        show_sub.add_argument('--output', choices=['json', 'text'], default='text', help='Output format for dbt compile and dbt show')
        show_sub.set_defaults(cls=ShowTask, which='show', rpc_method='show')
        return show_sub

    def __parse_args(self, args: List[str], cls: argparse.ArgumentParser=DBTArgumentParser) -> None:
        if False:
            print('Hello World!')
        ...
        _add_common_arguments(run_sub, compile_sub, generate_sub, test_sub, show_sub, seed_sub, parse_sub, build_sub)
        _add_selection_arguments(run_sub, compile_sub, generate_sub, test_sub, snapshot_sub, show_sub, seed_sub)
...
class ShowRunner(CompileRunner):
    ...
    def execute(self, compiled_node, manifest):
        ...
        if 'sql_header' in compiled_node.unrendered_config:
            compiled_node.compiled_code = compiled_node.unrendered_config['sql_header'] + compiled_node.compiled_code
        if self.config.args.limit >= 0 and type(self.adapter).__name__ not in SKIP_LIMIT_ADAPTER_NAMES:
            compiled_node.compiled_code = compiled_node.compiled_code + '\nLIMIT ' + str(self.config.args.limit)
        (adapter_response, execute_result) = self.adapter.execute(compiled_node.compiled_code, fetch=True)
        end_time = time.time()
        return RunResult(node=compiled_node, status=RunStatus.Success, timing=[], thread_id=threading.current_thread().name, execution_time=end_time - start_time, message=None, adapter_response=adapter_response.to_dict() if isinstance(adapter_response, dict) else {}, agate_table=execute_result, failures=None)
...
class ShowTask(CompileTask):
    ...
    def get_node_selector(self) -> ResourceTypeSelector:
        ...
        return ResourceTypeSelector(graph=self.graph, manifest=self.manifest, previous_state=self.previous_state, resource_types=[NodeType.Model, NodeType.Analysis])