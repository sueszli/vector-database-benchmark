...
class ProxyResponseGenerator(_ProxyResponseGeneratorBase):
    ...
    async def _get_next_streaming_result(self) -> Any:
        ...
        (done, _) = await asyncio.wait(tasks, return_when=FIRST_COMPLETED, timeout=calculate_remaining_timeout(timeout_s=self._timeout_s, start_time_s=self._start_time_s, curr_time_s=time.time()))
    ...
    async def _get_unary_result(self) -> Any:
        ...
        if self._disconnected_task is not None:
            tasks.append(self._disconnected_task)
        (done, _) = await asyncio.wait(tasks, return_when=FIRST_COMPLETED, timeout=self._timeout_s)
