...
from homeassistant.components.stream.const import ATTR_SETTINGS, CONF_LL_HLS, CONF_PART_DURATION, CONF_SEGMENT_DURATION, DOMAIN, HLS_PROVIDER
from homeassistant.components.stream.core import Part
from homeassistant.core import HomeAssistant
from homeassistant.setup import async_setup_component
from .common import FAKE_TIME, DefaultSegment as Segment, dynamic_stream_settings, generate_h264_video
...
async def test_ll_hls_stream(hass: HomeAssistant, hls_stream, stream_worker_sync) -> None:
    """Test hls stream.

    Purposefully not mocking anything here to test full
    integration with the stream component.
    """
    await async_setup_component(hass, 'stream', {'stream': {CONF_LL_HLS: True, CONF_SEGMENT_DURATION: SEGMENT_DURATION, CONF_PART_DURATION: TEST_PART_DURATION - 0.01}})
    ...
    playlist_response = await hls_client.get(playlist_url + f'?_HLS_msn={num_playlist_segments - 1}')
    assert playlist_response.status == HTTPStatus.OK
    playlist = await playlist_response.text()
    segment_re = re.compile('^(?P<segment_url>./segment/\\d+\\.m4s)')
    ...
    def check_part_is_moof_mdat(data: bytes):
        ...
        if mdat_length + moof_length != len(data):
            return False
        return True
    part_re = re.compile('#EXT-X-PART:DURATION=(?P<part_duration>[0-9]{1,}.[0-9]{3,}),URI="(?P<part_url>.+?)"(,INDEPENDENT=YES)?')
    datetime_re = re.compile('#EXT-X-PROGRAM-DATE-TIME:(?P<datetime>.+)')
    inf_re = re.compile('#EXTINF:(?P<segment_duration>[0-9]{1,}.[0-9]{3,}),')
    ...
    for line in playlist.splitlines():
        ...
        if match:
            ...
            if len(datetimes) > 1:
                datetime_duration = (datetimes[-1] - datetimes.popleft()).total_seconds()
                if segment_duration:
                    assert math.isclose(datetime_duration, segment_duration, rel_tol=0.001)
        ...
        if match:
            ...
            if len(part_durations) > 1:
                assert math.isclose(sum(part_durations), segment_duration, rel_tol=0.001)
                tested[inf_re] = True
...
async def test_ll_hls_playlist_view(hass: HomeAssistant, hls_stream, stream_worker_sync) -> None:
    """Test rendering the hls playlist with 1 and 2 output segments."""
    await async_setup_component(hass, 'stream', {'stream': {CONF_LL_HLS: True, CONF_SEGMENT_DURATION: SEGMENT_DURATION, CONF_PART_DURATION: TEST_PART_DURATION}})
    ...
    assert await resp.text() == make_playlist(sequence=0, segments=[make_segment_with_parts(i, len(segment.parts), PART_INDEPENDENT_PERIOD) for i in range(2)], hint=make_hint(2, 0), segment_duration=SEGMENT_DURATION, part_target_duration=hls.stream_settings.part_target_duration)
    ...
    resp = await hls_client.get('/playlist.m3u8')
    assert resp.status == HTTPStatus.OK
    assert await resp.text() == make_playlist(sequence=0, segments=[make_segment_with_parts(i, len(segment.parts), PART_INDEPENDENT_PERIOD) for i in range(3)], hint=make_hint(3, 0), segment_duration=SEGMENT_DURATION, part_target_duration=hls.stream_settings.part_target_duration)
    stream_worker_sync.resume()
    await stream.stop()

async def test_ll_hls_msn(hass: HomeAssistant, hls_stream, stream_worker_sync, hls_sync) -> None:
    ...
    msn_requests = asyncio.gather(*(hls_client.get(f'/playlist.m3u8?_HLS_msn={i}') for i in range(4)))
    ...
    msn_requests = asyncio.gather(*(hls_client.get(f'/playlist.m3u8?_HLS_msn={i}') for i in range(6)))
...
async def test_ll_hls_playlist_bad_msn_part(hass: HomeAssistant, hls_stream, stream_worker_sync) -> None:
    ...
    assert (await hls_client.get('/playlist.m3u8?_HLS_part=1')).status == HTTPStatus.BAD_REQUEST
    ...
    for part in remaining_parts[:num_completed_parts]:
        segment.async_add_part(part, 0)
    assert (await hls_client.get('/playlist.m3u8?_HLS_msn=4')).status == HTTPStatus.BAD_REQUEST
    assert (await hls_client.get(f'/playlist.m3u8?_HLS_msn=1&_HLS_part={num_completed_parts - 1 + hass.data[DOMAIN][ATTR_SETTINGS].hls_advance_part_limit}')).status == HTTPStatus.BAD_REQUEST
    stream_worker_sync.resume()

async def test_ll_hls_playlist_rollover_part(hass: HomeAssistant, hls_stream, stream_worker_sync, hls_sync) -> None:
    ...
    requests = asyncio.gather(*[hls_client.get(f'/playlist.m3u8?_HLS_msn=1&_HLS_part={len(segment.parts) - 1}'), hls_client.get(f'/playlist.m3u8?_HLS_msn=1&_HLS_part={len(segment.parts)}'), hls_client.get(f'/playlist.m3u8?_HLS_msn=1&_HLS_part={len(segment.parts) + 1}'), hls_client.get('/playlist.m3u8?_HLS_msn=2&_HLS_part=0')])
    await hls_sync.wait_for_handler()
    segment = create_segment(sequence=2)
    hls.put(segment)
    await hass.async_block_till_done()
...
async def test_ll_hls_playlist_msn_part(hass: HomeAssistant, hls_stream, stream_worker_sync, hls_sync) -> None:
    ...
    hls_sync.reset_request_pool(num_completed_parts + int(-(-hass.data[DOMAIN][ATTR_SETTINGS].hls_advance_part_limit // 1)))
    msn_requests = asyncio.gather(*(hls_client.get(f'/playlist.m3u8?_HLS_msn=1&_HLS_part={i}') for i in range(num_completed_parts + int(-(-hass.data[DOMAIN][ATTR_SETTINGS].hls_advance_part_limit // 1)))))
...
async def test_get_part_segments(hass: HomeAssistant, hls_stream, stream_worker_sync, hls_sync) -> None:
    ...
    for _ in range(num_completed_parts):
        segment.async_add_part(remaining_parts.pop(0), 0)
    requests = asyncio.gather(*(hls_client.get(f'/segment/1.{part}.m4s') for part in range(num_completed_parts)))
    responses = await requests
    assert all((response.status == HTTPStatus.OK for response in responses))
    assert all([await responses[i].read() == segment.parts[i].data for i in range(len(responses))])
    ...
    assert await response.read() == ALT_SEQUENCE_BYTES[:len(hls.get_segment(2).parts[0].data)]
    stream_worker_sync.resume()