...
from ...operations._customization_policies_operations import build_get_request, build_list_request
...
class CustomizationPoliciesOperations:
    ...
    .. warning::
        **DO NOT** instantiate this class directly.
        ...
        :attr:`customization_policies` attribute.
    """
    models = _models
    ...
    def list(self, region_id: str, pc_name: str, filter: Optional[str]=None, **kwargs: Any) -> AsyncIterable['_models.CustomizationPolicy']:
        ...
        def prepare_request(next_link=None):
            if False:
                print('Hello World!')
            if not next_link:
                request = build_list_request(region_id=region_id, pc_name=pc_name, subscription_id=self._config.subscription_id, filter=filter, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
    ...
    async def get(self, region_id: str, pc_name: str, customization_policy_name: str, **kwargs: Any) -> _models.CustomizationPolicy:
        ...
        :param pc_name: The private cloud name. Required.
        :type pc_name: str
        :param customization_policy_name: customization policy name. Required.
        ...
        api_version = kwargs.pop('api_version', _params.pop('api-version', self._config.api_version))
        cls = kwargs.pop('cls', None)
        request = build_get_request(region_id=region_id, pc_name=pc_name, customization_policy_name=customization_policy_name, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        pipeline_response = await self._client._pipeline.run(request, stream=False, **kwargs)
        response = pipeline_response.http_response
