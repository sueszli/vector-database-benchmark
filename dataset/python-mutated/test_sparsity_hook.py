...
dummy_dataset = create_dummy_test_dataset(np.random.random(size=(5,)), np.random.randint(0, 4, (1,)), 10)
...
class SparsityHookTest(unittest.TestCase):
    ...
    def test_sparsity_hook(self):
        if False:
            while True:
                i = 10
        json_cfg = {'task': 'image_classification', 'train': {'work_dir': self.tmp_dir, 'dataloader': {'batch_size_per_gpu': 2, 'workers_per_gpu': 1}, 'hooks': [{'type': 'SparsityHook', 'pruning_method': 'pst', 'config': {'weight_rank': 1, 'mask_rank': 1, 'final_sparsity': 0.9, 'frequency': 1}}]}}
        ...
        self.assertEqual(torch.mean(1.0 * (trainer.model.linear.weight == 0)), 0.9)
