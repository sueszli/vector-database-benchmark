...
async def test_emc_create_connection_success(default_conf, caplog, mocker):
    default_conf.update({'external_message_consumer': {'enabled': True, 'producers': [{'name': 'default', 'host': _TEST_WS_HOST, 'port': _TEST_WS_PORT, 'ws_token': _TEST_WS_TOKEN}], 'wait_timeout': 60, 'ping_timeout': 60, 'sleep_timeout': 60}})
    ...
    try:
        ...
        assert log_has_re('Connected to channel.+', caplog)
    finally:
        emc.shutdown()

@pytest.mark.parametrize('host,port', [(_TEST_WS_HOST, -1), ('10000.1241..2121/', _TEST_WS_PORT)])
async def test_emc_create_connection_invalid_url(default_conf, caplog, mocker, host, port):
    default_conf.update({'external_message_consumer': {'enabled': True, 'producers': [{'name': 'default', 'host': host, 'port': port, 'ws_token': _TEST_WS_TOKEN}], 'wait_timeout': 60, 'ping_timeout': 60, 'sleep_timeout': 60}})
    dp = DataProvider(default_conf, None, None, None)
    ...
    try:
        ...
        assert log_has_re('.+ is an invalid WebSocket URL .+', caplog)
    finally:
        emc.shutdown()
...
async def test_emc_receive_messages_valid(default_conf, caplog, mocker):
    caplog.set_level(logging.DEBUG)
    default_conf.update({'external_message_consumer': {'enabled': True, 'producers': [{'name': 'default', 'host': _TEST_WS_HOST, 'port': _TEST_WS_PORT, 'ws_token': _TEST_WS_TOKEN}], 'wait_timeout': 1, 'ping_timeout': 60, 'sleep_time': 60}})
    mocker.patch('freqtrade.rpc.external_message_consumer.ExternalMessageConsumer.start', MagicMock())
    lock = asyncio.Lock()
    test_producer = default_conf['external_message_consumer']['producers'][0]
    dp = DataProvider(default_conf, None, None, None)
    ...
    try:
        ...
        assert log_has_re('Received message of type `whitelist`.+', caplog)
    finally:
        emc.shutdown()
...
async def test_emc_receive_messages_invalid(default_conf, caplog, mocker):
    ...
    try:
        ...
        loop.call_soon(functools.partial(change_running, emc=emc))
        await emc._receive_messages(TestChannel(), test_producer, lock)
        assert log_has_re('Invalid message from.+', caplog)
    finally:
        emc.shutdown()

async def test_emc_receive_messages_timeout(default_conf, caplog, mocker):
    default_conf.update({'external_message_consumer': {'enabled': True, 'producers': [{'name': 'default', 'host': _TEST_WS_HOST, 'port': _TEST_WS_PORT, 'ws_token': _TEST_WS_TOKEN}], 'wait_timeout': 0.1, 'ping_timeout': 1, 'sleep_time': 1}})
    mocker.patch('freqtrade.rpc.external_message_consumer.ExternalMessageConsumer.start', MagicMock())
    lock = asyncio.Lock()
    test_producer = default_conf['external_message_consumer']['producers'][0]
    dp = DataProvider(default_conf, None, None, None)
    ...
    try:
        ...
        with pytest.raises(asyncio.TimeoutError):
            await emc._receive_messages(TestChannel(), test_producer, lock)
        assert log_has_re('Ping error.+', caplog)
    finally:
        emc.shutdown()

async def test_emc_receive_messages_handle_error(default_conf, caplog, mocker):
    default_conf.update({'external_message_consumer': {'enabled': True, 'producers': [{'name': 'default', 'host': _TEST_WS_HOST, 'port': _TEST_WS_PORT, 'ws_token': _TEST_WS_TOKEN}], 'wait_timeout': 1, 'ping_timeout': 1, 'sleep_time': 1}})
    mocker.patch('freqtrade.rpc.external_message_consumer.ExternalMessageConsumer.start', MagicMock())
    lock = asyncio.Lock()
    test_producer = default_conf['external_message_consumer']['producers'][0]
    dp = DataProvider(default_conf, None, None, None)
    ...
    try:
        ...
        loop.call_soon(functools.partial(change_running, emc=emc))
        await emc._receive_messages(TestChannel(), test_producer, lock)
        assert log_has_re('Error handling producer message.+', caplog)
    finally:
        emc.shutdown()