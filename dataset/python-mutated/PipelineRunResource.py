...
from mage_ai.orchestration.pipeline_scheduler import configure_pipeline_run_payload, stop_pipeline_run
...
class PipelineRunResource(DatabaseResource):
    ...
    async def collection(self, query_arg, meta, user, **kwargs):
        ...
        if pipeline_tags:
            ...
            pipeline_uuids_with_tags = pipeline_tag_cache.get_pipeline_uuids_with_tags(pipeline_tags)
        ...
        results = PipelineRun.query.filter(PipelineRun.pipeline_schedule_id.in_(repo_pipeline_schedule_ids)).options(selectinload(PipelineRun.block_runs)).options(selectinload(PipelineRun.pipeline_schedule)).join(PipelineSchedule, PipelineRun.pipeline_schedule_id == PipelineSchedule.id)
        if global_data_product_uuid is not None:
            results = results.filter(PipelineSchedule.global_data_product_uuid == global_data_product_uuid)
        else:
        ...
        if order_by:
            ...
            initial_results = results.order_by(PipelineRun.execution_date.desc(), PipelineRun.id.desc())
    ...
    async def process_collection(self, query_arg, meta, user, **kwargs):
        ...
        if (meta or {}).get(META_KEY_LIMIT, None) is not None and total_results.count() >= 1 and (not disable_retries_grouping) and (pipeline_uuid is not None or pipeline_schedule_id is not None):
            first_result = results[0]
            first_execution_date = first_result.execution_date
            additional_results = total_results.filter(PipelineRun.execution_date == first_execution_date)
            ...
            additional_results = total_results.filter(PipelineRun.execution_date == last_execution_date)
            filter_dates.append(last_result.execution_date)
            results = list(filter(lambda x: x.execution_date not in filter_dates, results)) + additional_results.all()
        results_size = len(results)
        has_next = results_size > limit
        final_end_idx = results_size - 1 if has_next else results_size
        result_set = self.build_result_set(results[0:final_end_idx], user, **kwargs)
        result_set.metadata = {'count': total_count, 'next': has_next}
        if include_pipeline_uuids:
