...
@op(config_schema=[int], out={'out_1': Out(List[int]), 'out_2': Out(List[int]), 'out_3': Out(List[int]), 'out_4': Out(List[int])})
...
@op(config_schema={'fail': Field(bool, is_required=False, default_value=False)}, out=Out(int, is_required=False))
...
@graph(description='Demo diamond-shaped graph that has four-path parallel structure of ops.')
def sleepy():
    if False:
        return 10
    giver_res = giver()
    will_fail(total(in_1=sleeper(units=giver_res.out_1), in_2=sleeper(units=giver_res.out_2), in_3=sleeper(units=giver_res.out_3), in_4=sleeper(units=giver_res.out_4)))
