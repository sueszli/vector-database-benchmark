...
import pytest
import ray
from ray._private.external_storage import create_url_with_offset, parse_url_with_offset, _get_unique_spill_filename, FileSystemStorage, ExternalStorageSmartOpenImpl
from ray._private.internal_api import memory_summary
from ray._private.test_utils import wait_for_condition
from ray._raylet import GcsClientOptions
from ray.tests.conftest import buffer_object_spilling_config, file_system_object_spilling_config, mock_distributed_fs_object_spilling_config
...
async def test_spill_during_get(object_spilling_config, shutdown_only, is_async):
    (object_spilling_config, _) = object_spilling_config
    address = ray.init(num_cpus=1, object_store_memory=100 * 1024 * 1024, _system_config={'automatic_object_spilling_enabled': True, 'object_store_full_delay_ms': 100, 'max_io_workers': 1, 'object_spilling_config': object_spilling_config, 'min_spilling_size': 0, 'worker_register_timeout_seconds': 600})
    if is_async:
    ...
    assert duration <= timedelta(seconds=timeout_seconds), 'Concurrent gets took too long. Maybe IO workers are not started properly.'
    assert_no_thrashing(address['address'])
