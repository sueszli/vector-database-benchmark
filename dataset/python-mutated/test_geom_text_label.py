...
import pandas as pd
import pytest
from plotnine import aes, geom_label, geom_point, geom_text, ggplot, scale_size_continuous, scale_y_continuous
from plotnine.data import mtcars
from plotnine.exceptions import PlotnineWarning
is_CI = os.environ.get('CI') is not None
n = 5
labels = ['ggplot', 'aesthetics', 'data', 'geoms', '$\\mathbf{statistics^2}$', 'scales', 'coordinates']
data = pd.DataFrame({'x': [1] * n, 'y': range(n), 'label': labels[:n], 'z': range(n), 'angle': np.linspace(0, 90, n)})
