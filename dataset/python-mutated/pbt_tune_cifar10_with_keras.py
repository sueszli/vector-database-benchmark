...
if __name__ == '__main__':
    ...
    space = {'epochs': 1, 'batch_size': 64, 'lr': tune.grid_search([10 ** (-4), 10 ** (-5)]), 'decay': tune.sample_from(lambda spec: spec.config.lr / 100.0), 'dropout': tune.grid_search([0.25, 0.5])}
    if args.smoke_test:
        space['lr'] = 10 ** (-4)
        space['dropout'] = 0.5
    perturbation_interval = 10
    pbt = PopulationBasedTraining(time_attr='training_iteration', perturbation_interval=perturbation_interval, hyperparam_mutations={'dropout': lambda _: np.random.uniform(0, 1)})
    tuner = tune.Tuner(tune.with_resources(Cifar10Model, resources={'cpu': 1, 'gpu': 1}), run_config=train.RunConfig(name='pbt_cifar10', stop={'mean_accuracy': 0.8, 'training_iteration': 30}, checkpoint_config=train.CheckpointConfig(checkpoint_frequency=perturbation_interval, checkpoint_score_attribute='mean_accuracy', num_to_keep=2)), tune_config=tune.TuneConfig(scheduler=pbt, num_samples=4, metric='mean_accuracy', mode='max'), param_space=space)
    results = tuner.fit()
