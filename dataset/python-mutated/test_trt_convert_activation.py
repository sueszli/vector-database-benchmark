...
class TrtConvertActivationTest(TrtLayerAutoScanTest):
    ...
    def sample_program_configs(self):
        ...
        for dims in [0, 1, 4]:
            for batch in [1, 4]:
                for op_type in ['relu', 'sigmoid', 'relu6', 'elu', 'selu', 'silu', 'softsign', 'stanh', 'thresholded_relu', 'celu', 'logsigmoid', 'tanh_shrink', 'softplus', 'hard_swish', 'hard_sigmoid', 'leaky_relu']:
                    for beta in [0.67]:
                        for alpha in [0.67]:
                            self.dims = dims
                            dics = [{}]
                            ...
                            if op_type == 'hard_swish':
                                dics = [{'threshold': 6.0, 'scale': 6.0, 'offset': 3.0}]
    ...
    def sample_predictor_configs(self, program_config) -> (paddle_infer.Config, List[int], float):
        ...
        def generate_trt_nodes_num(attrs, dynamic_shape):
            ...
            if (runtime_version[0] * 1000 + runtime_version[1] * 100 + runtime_version[2] * 10 < 8600 and self.dims == 0) and program_config.ops[0].type in ['celu', 'logsigmoid', 'tanh_shrink']:
                return (0, 3)
            return (1, 2)
