...
from ray.rllib.algorithms.sac.sac_torch_policy import _get_dist_class, stats, build_sac_model_and_action_dist, optimizer_fn, ComputeTDErrorMixin, setup_late_mixins, action_distribution_fn
...
from ray.rllib.utils.torch_utils import apply_grad_clipping, convert_to_torch_tensor, concat_multi_gpu_td_errors
...
def apply_gradients_fn(policy, gradients):
    if False:
        while True:
            i = 10
    return
CQLTorchPolicy = build_policy_class(name='CQLTorchPolicy', framework='torch', loss_fn=cql_loss, get_default_config=lambda : ray.rllib.algorithms.cql.cql.CQLConfig(), stats_fn=cql_stats, postprocess_fn=postprocess_trajectory, extra_grad_process_fn=apply_grad_clipping, optimizer_fn=cql_optimizer_fn, validate_spaces=validate_spaces, before_loss_init=cql_setup_late_mixins, make_model_and_action_dist=build_sac_model_and_action_dist, extra_learn_fetches_fn=concat_multi_gpu_td_errors, mixins=[TargetNetworkMixin, ComputeTDErrorMixin], action_distribution_fn=action_distribution_fn, compute_gradients_fn=compute_gradients_fn, apply_gradients_fn=apply_gradients_fn)