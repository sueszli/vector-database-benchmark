...
from homeassistant.components.recorder.db_schema import Events, EventTypes, States, StatesMeta
from homeassistant.components.recorder.models import process_timestamp
from homeassistant.components.recorder.queries import select_event_type_ids
from homeassistant.components.recorder.tasks import EntityIDMigrationTask, EntityIDPostMigrationTask, EventsContextIDMigrationTask, EventTypeIDMigrationTask, StatesContextIDMigrationTask
from homeassistant.components.recorder.util import session_scope
from homeassistant.core import HomeAssistant
import homeassistant.util.dt as dt_util
from homeassistant.util.ulid import bytes_to_ulid, ulid_at_time, ulid_to_bytes
from .common import async_attach_db_engine, async_recorder_block_till_done, async_wait_recording_done
...
async def test_migrate_events_context_ids(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    ...
    assert empty_context_id_event['context_id_bin'].startswith(b'\x01\xb50\xeeO(')
    ...
    assert bytes_to_ulid(ulid_context_id_event['context_id_bin']) == '01ARZ3NDEKTSV4RRFFQ69G5FAV'
    assert ulid_context_id_event['context_user_id_bin'] == b'\x94\x00\xfa\xce\xe4W\x11\xea\xa90\x8b\xfd=\x19\xe4t'
    assert bytes_to_ulid(ulid_context_id_event['context_parent_id_bin']) == '01ARZ3NDEKTSV4RRFFQ69G5FA2'
    ...
    assert invalid_context_id_event['context_id_bin'].startswith(b'\x01\xb50\xeeO(')
    ...
    assert garbage_context_id_event['context_id_bin'].startswith(b'\x01)~$\xdf(')
    assert garbage_context_id_event['context_user_id_bin'] is None
    assert garbage_context_id_event['context_parent_id_bin'] is None
    event_with_garbage_context_id_no_time_fired_ts = events_by_type['event_with_garbage_context_id_no_time_fired_ts']
    ...
    assert event_with_garbage_context_id_no_time_fired_ts['context_id_bin'].startswith(expected_ulid_fallback_start)
    assert event_with_garbage_context_id_no_time_fired_ts['context_user_id_bin'] is None
    assert event_with_garbage_context_id_no_time_fired_ts['context_parent_id_bin'] is None

@pytest.mark.parametrize('enable_migrate_context_ids', [True])
async def test_migrate_states_context_ids(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    """Test we can migrate old uuid context ids and ulid context ids to binary format."""
    instance = await async_setup_recorder_instance(hass)
    await async_wait_recording_done(hass)
    importlib.import_module(SCHEMA_MODULE)
    ...
    assert empty_context_id['context_id_bin'].startswith(b'\x01X\x0f\x12\xaf(')
    ...
    assert bytes_to_ulid(ulid_context_id['context_id_bin']) == '01ARZ3NDEKTSV4RRFFQ69G5FAV'
    assert ulid_context_id['context_user_id_bin'] == b'\x94\x00\xfa\xce\xe4W\x11\xea\xa90\x8b\xfd=\x19\xe4t'
    assert bytes_to_ulid(ulid_context_id['context_parent_id_bin']) == '01ARZ3NDEKTSV4RRFFQ69G5FA2'
    ...
    assert invalid_context_id['context_id_bin'].startswith(b'\x01X\x0f\x12\xaf(')
    ...
    assert garbage_context_id['context_id_bin'].startswith(b'\x01X\x0f\x12\xaf(')
    assert garbage_context_id['context_user_id_bin'] is None
    assert garbage_context_id['context_parent_id_bin'] is None
    human_readable_uuid_context_id = states_by_entity_id['state.human_readable_uuid_context_id']
    ...
    assert human_readable_uuid_context_id['context_id_bin'] == b'\n\xe2\x97\x99\xeeNOE\x81\x16\xf5\x82\xd7\xd3\xeee'
    assert human_readable_uuid_context_id['context_user_id_bin'] == b'\n\xe2\x97\x99\xeeNOE\x81\x16\xf5\x82\xd7\xd3\xeee'
    assert human_readable_uuid_context_id['context_parent_id_bin'] == b'\n\xe2\x97\x99\xeeNOE\x81\x16\xf5\x82\xd7\xd3\xeee'

@pytest.mark.parametrize('enable_migrate_event_type_ids', [True])
async def test_migrate_event_type_ids(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
...
async def test_migrate_entity_ids(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
...
@pytest.mark.parametrize('enable_migrate_entity_ids', [True])
async def test_post_migrate_entity_ids(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    """Test we can migrate entity_ids to the StatesMeta table."""
    instance = await async_setup_recorder_instance(hass)
    await async_wait_recording_done(hass)
    importlib.import_module(SCHEMA_MODULE)
...
@pytest.mark.parametrize('enable_migrate_entity_ids', [True])
async def test_migrate_null_entity_ids(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    """Test we can migrate entity_ids to the StatesMeta table."""
    instance = await async_setup_recorder_instance(hass)
    await async_wait_recording_done(hass)
    importlib.import_module(SCHEMA_MODULE)
...
@pytest.mark.parametrize('enable_migrate_event_type_ids', [True])
async def test_migrate_null_event_type_ids(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
...
async def test_stats_timestamp_conversion_is_reentrant(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
...
async def test_stats_timestamp_with_one_by_one(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    ...
    final_short_term_result = await hass.async_add_executor_job(_get_all_stats, old_db_schema.StatisticsShortTerm)
    final_short_term_result = sorted(final_short_term_result, key=lambda row: row['start_ts'])
    assert final_short_term_result == [{'created': None, 'created_ts': one_year_ago.timestamp(), 'id': 1, 'last_reset': None, 'last_reset_ts': one_year_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': one_year_ago.timestamp(), 'state': 1.0, 'sum': None}, {'created': None, 'created_ts': six_months_ago.timestamp(), 'id': 2, 'last_reset': None, 'last_reset_ts': six_months_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': six_months_ago.timestamp(), 'state': 1.0, 'sum': None}, {'created': None, 'created_ts': one_month_ago.timestamp(), 'id': 3, 'last_reset': None, 'last_reset_ts': one_month_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': one_month_ago.timestamp(), 'state': 1.0, 'sum': None}]
    final_result = await hass.async_add_executor_job(_get_all_stats, old_db_schema.Statistics)
    final_result = sorted(final_result, key=lambda row: row['start_ts'])
    assert final_result == [{'created': None, 'created_ts': one_year_ago.timestamp(), 'id': 1, 'last_reset': None, 'last_reset_ts': one_year_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': one_year_ago.timestamp(), 'state': 1.0, 'sum': None}, {'created': None, 'created_ts': six_months_ago.timestamp(), 'id': 2, 'last_reset': None, 'last_reset_ts': six_months_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': six_months_ago.timestamp(), 'state': 1.0, 'sum': None}, {'created': None, 'created_ts': one_month_ago.timestamp(), 'id': 3, 'last_reset': None, 'last_reset_ts': one_month_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': one_month_ago.timestamp(), 'state': 1.0, 'sum': None}]

async def test_stats_timestamp_with_one_by_one_removes_duplicates(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    ...
    assert final_short_term_result == [{'created': None, 'created_ts': one_year_ago.timestamp(), 'id': 1, 'last_reset': None, 'last_reset_ts': one_year_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': one_year_ago.timestamp(), 'state': 1.0, 'sum': None}, {'created': None, 'created_ts': six_months_ago.timestamp(), 'id': 2, 'last_reset': None, 'last_reset_ts': six_months_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': six_months_ago.timestamp(), 'state': 1.0, 'sum': None}, {'created': None, 'created_ts': one_month_ago.timestamp(), 'id': 3, 'last_reset': None, 'last_reset_ts': one_month_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': one_month_ago.timestamp(), 'state': 1.0, 'sum': None}]
    final_result = await hass.async_add_executor_job(_get_all_stats, old_db_schema.Statistics)
    assert final_result == [{'created': None, 'created_ts': six_months_ago.timestamp(), 'id': 2, 'last_reset': None, 'last_reset_ts': six_months_ago.timestamp(), 'max': None, 'mean': None, 'metadata_id': 1000, 'min': None, 'start': None, 'start_ts': six_months_ago.timestamp(), 'state': 1.0, 'sum': None}]