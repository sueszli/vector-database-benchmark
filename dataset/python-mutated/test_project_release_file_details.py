...
from sentry.api.endpoints.project_release_file_details import INVALID_UPDATE_MESSAGE, ClosesDependentFiles
...
class ReleaseFileDetailsTest(APITestCase):
    ...
    def test_simple(self):
        ...
        url = reverse('sentry-api-0-project-release-file-details', kwargs={'organization_slug': project.organization.slug, 'project_slug': project.slug, 'version': release.version, 'file_id': releasefile.id})
    ...
    def _get(self, file_id, postfix=''):
        if False:
            return 10
        url = reverse('sentry-api-0-project-release-file-details', kwargs={'organization_slug': self.project.organization.slug, 'project_slug': self.project.slug, 'version': self.release.version, 'file_id': file_id})
...
class ReleaseFileUpdateTest(APITestCase):
    ...
    def test_update_archived(self):
        ...
        self.create_release_archive()
        id = urlsafe_b64encode(b'_~/index.js')
        url = reverse('sentry-api-0-project-release-file-details', kwargs={'organization_slug': self.organization.slug, 'project_slug': self.project.slug, 'version': self.release.version, 'file_id': id})
        response = self.client.put(url, {'name': 'foobar'})
        assert response.status_code == 400
        assert response.data == {'detail': INVALID_UPDATE_MESSAGE}
...
class ReleaseFileDeleteTest(APITestCase):
    ...
    def test_delete_archived(self):
        ...
        url = lambda id: reverse('sentry-api-0-project-release-file-details', kwargs={'organization_slug': self.organization.slug, 'project_slug': self.project.slug, 'version': self.release.version, 'file_id': id})
        id = urlsafe_b64encode(b'_~/index.js')
