...
class QuanDense(Layer):
    ...
    def __init__(self, n_units=100, act=None, bitW=8, bitA=8, use_gemm=False, W_init=tl.initializers.truncated_normal(stddev=0.05), b_init=tl.initializers.constant(value=0.0), in_channels=None, name=None):
        if False:
            print('Hello World!')
        ...
        if self.in_channels is not None:
            ...
            self._built = True
        logging.info('QuanDense  %s: %d %s' % (self.name, n_units, self.act.__name__ if self.act is not None else 'No Activation'))
