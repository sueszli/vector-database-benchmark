...
class XPUTestROIAlignOp(XPUOpTestWrapper):
    ...
    class TestROIAlignOp(XPUOpTest):
        ...
        def set_data(self):
            ...
            self.attrs = {'spatial_scale': self.spatial_scale, 'pooled_height': self.pooled_height, 'pooled_width': self.pooled_width, 'sampling_ratio': self.sampling_ratio, 'aligned': self.continuous_coordinate}
        ...
        def pre_calc(self, x_i, roi_xmin, roi_ymin, roi_bin_grid_h, roi_bin_grid_w, bin_size_h, bin_size_w):
            ...
            bilinear_pos = np.zeros([self.channels, self.pooled_height, self.pooled_width, count, 4], np.float32)
            bilinear_w = np.zeros([self.pooled_height, self.pooled_width, count, 4], np.float32)
        ...
        def calc_roi_align(self):
            if False:
                i = 10
                return i + 15
            self.out_data = np.zeros((self.rois_num, self.channels, self.pooled_height, self.pooled_width)).astype(self.dtype)
