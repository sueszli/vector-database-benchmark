...
def internlm_attention_forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_value: Optional[Tuple[torch.Tensor]]=None, output_attentions: bool=False, use_cache: bool=False) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
    ...
    if query_states.device.type == 'xpu' and (not (self.training and query_states.requires_grad)):
        (query_states, key_states) = apply_rotary_pos_emb_no_cache_xpu(query_states, key_states, position_ids, 'internlm')
    else:
        (cos, sin) = self.rotary_emb(value_states, seq_len=kv_seq_len)
        (query_states, key_states) = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids, 'internlm')
    if past_key_value is not None:
    ...
    if attn_output.size() != (bsz, self.num_heads, q_len, self.head_dim):
        invalidInputError(False, f'`attn_output` should be of size {(bsz, self.num_heads, q_len, self.head_dim)}, but is {attn_output.size()}')
