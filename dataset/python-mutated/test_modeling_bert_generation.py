...
class BertGenerationEncoderTester:
    ...
    def prepare_config_and_inputs_for_decoder(self):
        ...
        return (config, input_ids, input_mask, token_labels, encoder_hidden_states, encoder_attention_mask)

    def create_and_check_model(self, config, input_ids, input_mask, token_labels, **kwargs):
        if False:
            i = 10
            return i + 15
    ...
    def create_and_check_model_as_decoder(self, config, input_ids, input_mask, token_labels, encoder_hidden_states, encoder_attention_mask, **kwargs):
        if False:
            while True:
                i = 10
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, input_ids, input_mask, token_labels, encoder_hidden_states, encoder_attention_mask, **kwargs):
        if False:
            i = 10
            return i + 15
        config.is_decoder = True
    ...
    def create_and_check_for_causal_lm(self, config, input_ids, input_mask, token_labels, *args):
        if False:
            for i in range(10):
                print('nop')
...
class BertGenerationEncoderTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': BertGenerationEncoder, 'text-generation': BertGenerationDecoder} if is_torch_available() else {}
    ...
    def test_model_as_decoder_with_default_input_mask(self):
        if False:
            print('Hello World!')
        (config, input_ids, input_mask, token_labels, encoder_hidden_states, encoder_attention_mask) = self.model_tester.prepare_config_and_inputs_for_decoder()
        input_mask = None
        self.model_tester.create_and_check_model_as_decoder(config, input_ids, input_mask, token_labels, encoder_hidden_states, encoder_attention_mask)
...
class BertGenerationEncoderIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head_absolute_embedding(self):
        ...
        expected_slice = torch.tensor([[[0.1775, 0.0083, -0.0321], [1.6002, 0.1287, 0.3912], [2.1473, 0.5791, 0.6066]]])
...
class BertGenerationDecoderIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head_absolute_embedding(self):
        ...
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[[-0.5788, -2.5994, -3.7054], [0.0438, 4.7997, 1.8795], [1.5862, 6.6409, 4.4638]]])
        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=0.0001))