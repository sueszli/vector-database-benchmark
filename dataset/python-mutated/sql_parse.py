...
from sqlparse.sql import Identifier, IdentifierList, Parenthesis, remove_quotes, Token, TokenList, Where
...
def insert_rls_in_predicate(token_list: TokenList, database_id: int, default_schema: Optional[str]) -> TokenList:
    ...
    return token_list
SQLOXITE_DIALECTS = {'ansi': {'trino', 'trinonative', 'presto'}, 'hive': {'hive', 'databricks'}, 'ms': {'mssql'}, 'mysql': {'mysql'}, 'postgres': {'cockroachdb', 'hana', 'netezza', 'postgres', 'postgresql', 'redshift', 'vertica'}, 'snowflake': {'snowflake'}, 'sqlite': {'sqlite', 'gsheets', 'shillelagh'}, 'clickhouse': {'clickhouse'}}
