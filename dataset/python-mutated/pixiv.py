...
@pluginmatcher(re.compile('https?://sketch\\.pixiv\\.net/@?(?P<user>[^/]+)'))
@pluginargument('sessionid', requires=['devicetoken'], sensitive=True, metavar='SESSIONID', help="The pixiv.net sessionid that's used in pixiv's PHPSESSID cookie.")
@pluginargument('devicetoken', sensitive=True, metavar='DEVICETOKEN', help="The pixiv.net device token that's used in pixiv's device_token cookie.")
@pluginargument('purge-credentials', action='store_true', help='Purge cached Pixiv credentials to initiate a new session and reauthenticate.')
@pluginargument('performer', metavar='USER', help='Select a co-host stream instead of the owner stream.')
class Pixiv(Plugin):
    _post_key_re = re.compile('name=["\']post_key["\']\\svalue=["\'](?P<data>[^"\']+)["\']')
    _user_dict_schema = validate.Schema({'user': {'unique_name': str, 'name': str}, validate.optional('hls_movie'): {'url': str}})
    _user_schema = validate.Schema({'owner': _user_dict_schema, 'performers': [validate.any(_user_dict_schema, None)]})
    _data_lives_schema = validate.Schema({'data': {'lives': [_user_schema]}}, validate.get('data'), validate.get('lives'))
