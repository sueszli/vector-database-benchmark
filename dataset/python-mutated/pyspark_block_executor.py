...
class PySparkBlockExecutor(BlockExecutor):
    ...
    def __init__(self, pipeline: Pipeline, block_uuid: str, execution_partition: str=None):
        ...
        self.resource_manager = EmrResourceManager(pipeline.repo_config.s3_bucket, pipeline.repo_config.s3_path_prefix)
    ...
    def _execute(self, analyze_outputs: bool=False, global_vars: Dict=None, update_status: bool=False, **kwargs) -> None:
        if False:
            i = 10
            return i + 15
    ...
    def submit_spark_job(self):
        if False:
            for i in range(10):
                print('nop')
        step = {'name': f'run_mage_block_{self.pipeline.uuid}_{self.block_uuid}', 'jars': self.executor_config.spark_jars, 'script_uri': self.spark_script_path, 'script_args': []}
        return emr.submit_spark_job(cluster_name=step['name'], steps=[step], bootstrap_script_path=self.resource_manager.bootstrap_script_path, emr_config=self.executor_config, log_uri=self.resource_manager.log_uri)