...
from AnyQt.QtWidgets import QLabel, QLineEdit, QTextBrowser, QSplitter, QTreeView, QStyleOptionViewItem, QStyledItemDelegate, QStyle, QApplication, QHBoxLayout, QComboBox
from AnyQt.QtGui import QStandardItemModel, QStandardItem, QBrush, QColor
from AnyQt.QtCore import Qt, QSize, QObject, QThread, QModelIndex, QSortFilterProxyModel, QItemSelectionModel, pyqtSlot as Slot, pyqtSignal as Signal
...
class UniformHeightIndicatorDelegate(UniformHeightDelegate, gui.IndicatorItemDelegate):
    pass
...
class OWDataSets(OWWidget):
    ...
    HEADER_SCHEMA = [['islocal', {'label': ''}], ['title', {'label': 'Title'}], ['size', {'label': 'Size'}], ['instances', {'label': 'Instances'}], ['variables', {'label': 'Variables'}], ['target', {'label': 'Target'}], ['tags', {'label': 'Tags'}]]
    ...
    class Warning(OWWidget.Warning):
        only_local_datasets = Msg('Could not fetch datasets list, only local cached datasets are shown')

    class Outputs:
        data = Output('Data', Orange.data.Table)
    selected_id = Setting(None)
    language = Setting(DEFAULT_LANG)
    splitter_state = Setting(b'')
    header_state = Setting(b'')
