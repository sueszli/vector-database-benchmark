...
class TestTaskStateTrigger:
    ...
    async def test_task_state_trigger(self, session):
        ...
        dag_run = DagRun(dag_id=dag.dag_id, run_type='manual', execution_date=timezone.datetime(2022, 1, 1), run_id=self.RUN_ID)
        session.add(dag_run)
        session.commit()
        ...
        session.commit()
        trigger = TaskStateTrigger(dag_id=dag.dag_id, task_id=instance.task_id, states=self.STATES, execution_dates=[timezone.datetime(2022, 1, 1)], poll_interval=0.2, trigger_start_time=trigger_start_time)
...
class TestDagStateTrigger:
    ...
    async def test_dag_state_trigger(self, session):
        ...
        session.add(dag_run)
        session.commit()
        trigger = DagStateTrigger(dag_id=dag.dag_id, states=self.STATES, execution_dates=[timezone.datetime(2022, 1, 1)], poll_interval=0.2)
        task = asyncio.create_task(trigger.run().__anext__())
        await asyncio.sleep(0.5)
        assert task.done() is False
