...
class MLP:
  ...
  """
    ...
    def __init__(self, num_classes=NUM_CLASSES, input_size=INPUT_SIZE, hidden_units=HIDDEN_UNITS):
        if False:
            print('Hello World!')
        self.w1 = np.random.uniform(size=[input_size, hidden_units]).astype(np.float32, copy=False)
        self.w2 = np.random.uniform(size=[hidden_units, num_classes]).astype(np.float32, copy=False)
        self.b1 = np.random.uniform(size=[1, hidden_units]).astype(np.float32, copy=False)
        self.b2 = np.random.uniform(size=[1, num_classes]).astype(np.float32, copy=False)
