...
class MultiTradeStrategy(bt.Strategy):
    ...
    """
    params = dict(period=15, stake=1, printout=False, onlylong=False, mtrade=False)
...
def runstrategy():
    ...
    cerebro.addstrategy(MultiTradeStrategy, period=args.period, onlylong=args.onlylong, stake=args.stake, printout=args.printout, mtrade=args.mtrade)
...
def parse_args():
    ...
    parser.add_argument('--mtrade', action='store_true', help='Activate MultiTrade Ids')
    ...
    parser.add_argument('--printout', action='store_true', help='Print operation log from strategy')
