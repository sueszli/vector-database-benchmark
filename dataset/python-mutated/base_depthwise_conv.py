...
class BaseDepthwiseConv(Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.add_weight(name='kernel', shape=depthwise_shape, initializer=self.depthwise_initializer, regularizer=self.depthwise_regularizer, constraint=self.depthwise_constraint, trainable=True, dtype=self.dtype)
        if self.use_bias:
            self.bias = self.add_weight(name='bias', shape=(self.depth_multiplier * input_channel,), initializer=self.bias_initializer, regularizer=self.bias_regularizer, constraint=self.bias_constraint, trainable=True, dtype=self.dtype)
        else:
            self.bias = None
        self.built = True
    ...
    def call(self, inputs):
        ...
        outputs = ops.depthwise_conv(inputs, self.kernel, strides=self.strides, padding=self.padding, dilation_rate=self.dilation_rate, data_format=self.data_format)
        if self.use_bias:
            if self.data_format == 'channels_last':
    ...
    def compute_output_shape(self, input_shape):
        ...
        return compute_conv_output_shape(input_shape, self.depth_multiplier * input_channel, self.kernel_size, strides=self.strides, padding=self.padding, data_format=self.data_format, dilation_rate=self.dilation_rate)
