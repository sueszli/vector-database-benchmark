...
class SearchVirustotal:
    ...
    async def do_search(self) -> None:
        headers = {'User-Agent': Core.get_user_agent(), 'Accept': 'application/json', 'x-apikey': self.key}
    ...
    async def parse_hostnames(data, word):
        ...
        for attribute in data:
            ...
            total_subdomains.update({value['value'].replace('"', '').replace('www.', '') for value in attributes['last_dns_records'] if word in value['value']})
            if 'last_https_certificate' in attributes.keys():
                total_subdomains.update({value.replace('"', '').replace('www.', '') for value in attributes['last_https_certificate']['extensions']['subject_alternative_name'] if word in value})
        total_subdomains = list(sorted(total_subdomains))
        total_subdomains = [x for x in total_subdomains if 'edgekey.net' not in str(x) and 'akadns.net' not in str(x) and ('include:_spf' not in str(x))]
