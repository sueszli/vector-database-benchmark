...
from torch.testing._internal.common_dtype import floating_types, floating_and_complex_types_and, get_all_fp_dtypes, complex_types_and
from torch.testing._internal.common_device_type import _TestParametrizer, _update_param_kwargs, toleranceOverride, tol, skipCUDAIfCudnnVersionLessThan, skipCUDAIfRocm, precisionOverride, skipMeta, skipMPS, skipCUDAVersionIn
from torch.testing._internal.common_methods_invocations import DecorateInfo
from torch.testing._internal.common_nn import nllloss_reference, get_reduction
from torch.testing._internal.common_utils import freeze_rng_state, set_single_threaded_if_parallel_tbb, skipIfMps, GRADCHECK_NONDET_TOL, TEST_WITH_ROCM, IS_WINDOWS
from types import ModuleType
from typing import List, Tuple, Type, Set, Dict
MODULE_NAMESPACES: List[ModuleType] = [torch.nn.modules, torch.ao.nn.qat.modules, torch.ao.nn.quantizable.modules, torch.ao.nn.quantized.modules, torch.ao.nn.quantized.modules]
MODULES_TO_SKIP: Set[Type] = {torch.nn.Module, torch.nn.Container, torch.nn.NLLLoss2d, torch.ao.nn.quantized.MaxPool2d, torch.ao.nn.quantized.MaxPool2d}
MODULE_CLASSES: List[Type] = list(chain(*[[getattr(namespace, module_name) for module_name in namespace.__all__] for namespace in MODULE_NAMESPACES]))
...
rnn_gru_lstm_module_info_decorators = (DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_grad', active_if=TEST_CUDNN and (not TEST_WITH_ROCM), device_type='cuda'), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_gradgrad', active_if=TEST_CUDNN and (not TEST_WITH_ROCM), device_type='cuda'), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_non_contiguous_tensors', active_if=TEST_CUDNN and (not TEST_WITH_ROCM), device_type='cuda'), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_non_contiguous_tensors', active_if=TEST_CUDNN and TEST_WITH_ROCM, dtypes=(torch.float,), device_type='cuda'), DecorateInfo(skipCUDAVersionIn([(11, 7)]), 'TestExpandedWeightModule', 'test_module', device_type='cuda'), DecorateInfo(skipCUDAVersionIn([(11, 7)]), 'TestDecomp', 'test_rnn_decomp_module', device_type='cuda'))
...
module_db: List[ModuleInfo] = [ModuleInfo(torch.nn.AdaptiveAvgPool1d, module_inputs_func=module_inputs_torch_nn_AdaptiveAvgPool1d, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.AdaptiveAvgPool2d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_inputs_func=module_inputs_torch_nn_AdaptiveAvgPool2d, skips=(DecorateInfo(skipMPS), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', active_if=lambda p: p['training']))), ModuleInfo(torch.nn.AdaptiveAvgPool3d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_inputs_func=module_inputs_torch_nn_AdaptiveAvgPool3d, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipMPS))), ModuleInfo(torch.nn.AdaptiveMaxPool1d, module_inputs_func=module_inputs_torch_nn_AdaptiveMaxPool1d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.AdaptiveMaxPool2d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_inputs_func=module_inputs_torch_nn_AdaptiveMaxPool2d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.AdaptiveMaxPool3d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_inputs_func=module_inputs_torch_nn_AdaptiveMaxPool3d, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipMPS))), ModuleInfo(torch.nn.AvgPool1d, module_inputs_func=module_inputs_torch_nn_AvgPool1d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.AvgPool2d, module_inputs_func=module_inputs_torch_nn_AvgPool2d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', active_if=lambda p: p['training'], device_type='cuda'))), ModuleInfo(torch.nn.AvgPool3d, module_inputs_func=module_inputs_torch_nn_AvgPool3d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipMPS))), ModuleInfo(torch.nn.BatchNorm1d, train_and_eval_differ=True, module_inputs_func=module_inputs_torch_nn_BatchNorm1d, skips=(DecorateInfo(skipMPS), DecorateInfo(unittest.expectedFailure, 'TestEagerFusionModuleInfo', 'test_aot_autograd_symbolic_module_exhaustive', active_if=lambda p: p['training']), DecorateInfo(unittest.expectedFailure, 'TestEagerFusionModuleInfo', 'test_aot_autograd_module_exhaustive', active_if=lambda p: p['training']))), ModuleInfo(torch.nn.BatchNorm2d, train_and_eval_differ=True, module_inputs_func=module_inputs_torch_nn_BatchNorm2d, skips=(DecorateInfo(skipMPS), DecorateInfo(unittest.expectedFailure, 'TestEagerFusionModuleInfo', 'test_aot_autograd_symbolic_module_exhaustive', active_if=lambda p: p['training']), DecorateInfo(unittest.expectedFailure, 'TestEagerFusionModuleInfo', 'test_aot_autograd_module_exhaustive', active_if=lambda p: p['training']))), ModuleInfo(torch.nn.BatchNorm3d, train_and_eval_differ=True, module_inputs_func=module_inputs_torch_nn_BatchNorm3d, skips=(DecorateInfo(skipMPS), DecorateInfo(unittest.expectedFailure, 'TestEagerFusionModuleInfo', 'test_aot_autograd_symbolic_module_exhaustive', active_if=lambda p: p['training']), DecorateInfo(unittest.expectedFailure, 'TestEagerFusionModuleInfo', 'test_aot_autograd_module_exhaustive', active_if=lambda p: p['training']))), ModuleInfo(torch.nn.CELU, module_inputs_func=module_inputs_torch_nn_CELU, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.Conv1d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=1, lazy=False), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=7603), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64])), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'),)), ModuleInfo(torch.nn.Conv2d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=2, lazy=False), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=7603), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='cuda', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='mps', dtypes=[torch.float32])), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'),)), ModuleInfo(torch.nn.Conv3d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=3, lazy=False), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=8005), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipMPS), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format')), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'),)), ModuleInfo(torch.nn.ConvTranspose1d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=1, lazy=False, transposed=True), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, dtypes=floating_and_complex_types_and(torch.chalf), skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=7603), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipIfMps, 'TestModule', dtypes=complex_types_and(torch.chalf, torch.float64, torch.complex128)), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_cpu_gpu_parity', dtypes=(torch.chalf,), device_type='cuda'), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_pickle', dtypes=(torch.chalf,))), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'), DecorateInfo(precisionOverride({torch.chalf: 0.005}), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.ConvTranspose2d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=2, lazy=False, transposed=True), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, dtypes=floating_and_complex_types_and(torch.chalf), skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=7603), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipIfMps, 'TestModule', dtypes=complex_types_and(torch.chalf, torch.float64, torch.complex128)), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', dtypes=(torch.complex32, torch.complex64, torch.complex128)), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='cuda', dtypes=[torch.float64, torch.complex128]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='mps', dtypes=[torch.float32]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_cpu_gpu_parity', dtypes=(torch.chalf,), device_type='cuda'), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_pickle', dtypes=(torch.chalf,))), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'), DecorateInfo(precisionOverride({torch.chalf: 0.005}), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.ConvTranspose3d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=3, lazy=False, transposed=True), dtypes=floating_and_complex_types_and(torch.chalf), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=8005), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipMPS), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format'), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='cuda', dtypes=[torch.complex32, torch.complex64], active_if=TEST_WITH_ROCM), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_cpu_gpu_parity', dtypes=(torch.chalf,), device_type='cuda'), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_pickle', dtypes=(torch.chalf,))), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'), DecorateInfo(precisionOverride({torch.complex64: 0.0001}), 'TestModule', 'test_cpu_gpu_parity'), DecorateInfo(precisionOverride({torch.chalf: 0.005}), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.ELU, module_inputs_func=module_inputs_torch_nn_ELU, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.FractionalMaxPool2d, module_inputs_func=module_inputs_torch_nn_FractionalMaxPool2d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, skips=(DecorateInfo(skipMPS), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.FractionalMaxPool3d, module_inputs_func=module_inputs_torch_nn_FractionalMaxPool3d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, skips=(DecorateInfo(skipMPS), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.L1Loss, module_inputs_func=module_inputs_torch_nn_L1Loss, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.LazyConv1d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=1, lazy=True), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=7603), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipMeta), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64])), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'),)), ModuleInfo(torch.nn.LazyConv2d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=2, lazy=True), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=7603), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipMeta), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='cuda', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='mps', dtypes=[torch.float32])), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'),)), ModuleInfo(torch.nn.LazyConv3d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=3, lazy=True), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=8005), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipMeta), DecorateInfo(skipMPS), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format')), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'),)), ModuleInfo(torch.nn.LazyConvTranspose1d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=1, lazy=True, transposed=True), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=7603), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipMeta), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64])), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'),)), ModuleInfo(torch.nn.LazyConvTranspose2d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=2, lazy=True, transposed=True), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=7603), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipMeta), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='cuda', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='mps', dtypes=[torch.float32])), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'),)), ModuleInfo(torch.nn.LazyConvTranspose3d, module_inputs_func=partial(module_inputs_torch_nn_ConvNd, N=3, lazy=True, transposed=True), gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, module_memformat_affects_out=True, skips=(DecorateInfo(skipCUDAIfCudnnVersionLessThan(version=8005), 'TestModule', 'test_memory_format'), DecorateInfo(skipCUDAIfRocm, 'TestModule', 'test_memory_format', dtypes=[torch.float32]), DecorateInfo(skipMeta), DecorateInfo(skipMPS), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format')), decorators=(DecorateInfo(precisionOverride({torch.float32: 0.0001}), 'TestModule', 'test_memory_format'),)), ModuleInfo(torch.nn.Linear, module_inputs_func=module_inputs_torch_nn_Linear, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.Bilinear, module_inputs_func=module_inputs_torch_nn_Bilinear, decorators=[DecorateInfo(toleranceOverride({torch.float32: tol(atol=0.0001, rtol=0.0001), torch.float64: tol(atol=0.0001, rtol=0.0001)}), 'TestModule', 'test_forward', device_type='cpu')], skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.LPPool1d, module_inputs_func=module_inputs_torch_nn_LPPool1d, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_grad'), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_gradgrad'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.LPPool2d, module_inputs_func=module_inputs_torch_nn_LPPool2d, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_grad'), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_gradgrad'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', active_if=lambda p: p['training'], device_type='mps'))), ModuleInfo(torch.nn.MaxPool1d, module_inputs_func=module_inputs_torch_nn_MaxPool1d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.MaxPool2d, module_inputs_func=module_inputs_torch_nn_MaxPool2d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.MaxPool3d, module_inputs_func=module_inputs_torch_nn_MaxPool3d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.NLLLoss, module_inputs_func=module_inputs_torch_nn_NLLLoss, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.GaussianNLLLoss, module_inputs_func=module_inputs_torch_nn_GaussianNLLLoss, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.CrossEntropyLoss, module_inputs_func=module_inputs_torch_nn_CrossEntropyLoss, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.GELU, module_inputs_func=module_inputs_torch_nn_GELU, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.GLU, module_inputs_func=module_inputs_torch_nn_GLU, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.GroupNorm, module_inputs_func=module_inputs_torch_nn_GroupNorm, dtypes=get_all_fp_dtypes(include_bfloat16=True, include_half=True), skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64, torch.bfloat16]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_cpu_gpu_parity'), DecorateInfo(toleranceOverride({torch.float32: tol(atol=0.0001, rtol=0.0001)}), 'TestModule', 'test_memory_format', device_type='cpu'), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='cuda'), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='mps'), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_grad', active_if=TEST_WITH_ROCM, device_type='cuda'))), ModuleInfo(torch.nn.Hardshrink, module_inputs_func=module_inputs_torch_nn_Hardshrink, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.Hardswish, module_inputs_func=module_inputs_torch_nn_Hardswish, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', active_if=lambda p: p['training'], device_type='mps')), supports_gradgrad=False), ModuleInfo(torch.nn.Hardtanh, module_inputs_func=module_inputs_torch_nn_Hardtanh, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.InstanceNorm1d, module_inputs_func=partial(module_inputs_torch_nn_InstanceNormNd, N=1), train_and_eval_differ=True, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.InstanceNorm2d, module_inputs_func=partial(module_inputs_torch_nn_InstanceNormNd, N=2), train_and_eval_differ=True, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.InstanceNorm3d, module_inputs_func=partial(module_inputs_torch_nn_InstanceNormNd, N=3), train_and_eval_differ=True, skips=(DecorateInfo(skipMPS), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.LocalResponseNorm, module_inputs_func=module_inputs_torch_nn_LocalResponseNorm, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.LayerNorm, module_inputs_func=module_inputs_torch_nn_LayerNorm, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.TransformerEncoder, train_and_eval_differ=True, module_inputs_func=module_inputs_torch_nn_TransformerEncoder, decorators=[DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_gradgrad', device_type='cpu')], skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_factory_kwargs'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.TransformerEncoderLayer, train_and_eval_differ=True, module_inputs_func=module_inputs_torch_nn_TransformerEncoderLayer, decorators=[DecorateInfo(toleranceOverride({torch.float32: tol(atol=0.0001, rtol=0.0001)}), 'TestModule', 'test_non_contiguous_tensors', device_type='cpu', active_if=IS_WINDOWS), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_gradgrad', device_type='cpu')], skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.TransformerDecoderLayer, module_inputs_func=module_inputs_torch_nn_TransformerDecoderLayer, decorators=[DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_gradgrad', device_type='cpu')], skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.Transformer, module_inputs_func=module_inputs_torch_nn_Transformer, decorators=[DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_gradgrad', device_type='cpu')], skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.MultiheadAttention, train_and_eval_differ=True, module_inputs_func=module_inputs_torch_nn_MultiheadAttention, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.Embedding, module_inputs_func=module_inputs_torch_nn_Embedding, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.ReLU, module_inputs_func=module_inputs_torch_nn_ReLU, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', active_if=lambda p: p['training'], device_type='mps'))), ModuleInfo(torch.nn.LeakyReLU, module_inputs_func=module_inputs_torch_nn_LeakyReLU, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.ReLU6, module_inputs_func=module_inputs_torch_nn_ReLU6, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.PReLU, module_inputs_func=module_inputs_torch_nn_PReLU, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.RNNCell, module_inputs_func=partial(module_inputs_torch_nn_RNN_GRU_Cell, is_rnn=True), module_error_inputs_func=module_error_inputs_torch_nn_RNN_GRU_Cell, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.GRUCell, module_inputs_func=module_inputs_torch_nn_RNN_GRU_Cell, module_error_inputs_func=module_error_inputs_torch_nn_RNN_GRU_Cell, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.LSTMCell, module_inputs_func=module_inputs_torch_nn_LSTMCell, module_error_inputs_func=module_error_inputs_torch_nn_LSTMCell, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.Sigmoid, module_inputs_func=module_inputs_torch_nn_Sigmoid, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', active_if=lambda p: p['training'], device_type='mps'))), ModuleInfo(torch.nn.LogSigmoid, module_inputs_func=module_inputs_torch_nn_LogSigmoid, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.SiLU, module_inputs_func=module_inputs_torch_nn_SiLU, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.Softmax, module_inputs_func=module_inputs_torch_nn_Softmax, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.Softmax2d, module_inputs_func=module_inputs_torch_nn_Softmax2d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.LogSoftmax, module_inputs_func=module_inputs_torch_nn_LogSoftmax, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.Softmin, module_inputs_func=module_inputs_torch_nn_Softmin, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.Softplus, module_inputs_func=module_inputs_torch_nn_Softplus, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.Softshrink, module_inputs_func=module_inputs_torch_nn_Softshrink, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.Softsign, module_inputs_func=module_inputs_torch_nn_Softsign, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.Tanh, module_inputs_func=module_inputs_torch_nn_Tanh, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', active_if=lambda p: p['training'], device_type='mps'))), ModuleInfo(torch.nn.Tanhshrink, module_inputs_func=module_inputs_torch_nn_Tanhshrink, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', active_if=lambda p: p['training'], device_type='mps'))), ModuleInfo(torch.nn.Threshold, module_inputs_func=module_inputs_torch_nn_Threshold, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.Mish, module_inputs_func=module_inputs_torch_nn_Mish, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.RNN, train_and_eval_differ=True, module_inputs_func=partial(module_inputs_torch_nn_RNN_GRU, is_rnn=True), module_error_inputs_func=module_error_inputs_torch_nn_RNN_GRU, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),), decorators=rnn_gru_lstm_module_info_decorators), ModuleInfo(torch.nn.GRU, train_and_eval_differ=True, module_inputs_func=partial(module_inputs_torch_nn_RNN_GRU, is_rnn=False), module_error_inputs_func=module_error_inputs_torch_nn_RNN_GRU, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),), decorators=rnn_gru_lstm_module_info_decorators), ModuleInfo(torch.nn.LSTM, train_and_eval_differ=True, module_inputs_func=module_inputs_torch_nn_LSTM, module_error_inputs_func=module_error_inputs_torch_nn_RNN_GRU, skips=(DecorateInfo(skipMPS),), decorators=rnn_gru_lstm_module_info_decorators), ModuleInfo(torch.nn.ReflectionPad1d, module_inputs_func=module_inputs_torch_nn_ReflectionPad1d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.ReflectionPad2d, module_inputs_func=module_inputs_torch_nn_ReflectionPad2d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='cuda'), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='mps'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.ReflectionPad3d, module_inputs_func=module_inputs_torch_nn_ReflectionPad3d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='cuda'), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='mps'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.ReplicationPad1d, module_inputs_func=module_inputs_torch_nn_ReplicationPad1d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.ReplicationPad2d, module_inputs_func=module_inputs_torch_nn_ReplicationPad2d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='cuda'), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='mps'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.ReplicationPad3d, module_inputs_func=module_inputs_torch_nn_ReplicationPad3d, gradcheck_nondet_tol=GRADCHECK_NONDET_TOL, skips=(DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='cuda'), DecorateInfo(unittest.skip('Skipped!'), 'TestModule', 'test_memory_format', device_type='mps'), DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]))), ModuleInfo(torch.nn.SELU, module_inputs_func=module_inputs_torch_nn_SELU, skips=(DecorateInfo(skipMPS),)), ModuleInfo(torch.nn.ZeroPad1d, module_inputs_func=module_inputs_torch_nn_ZeroPad1d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.ZeroPad2d, module_inputs_func=module_inputs_torch_nn_ZeroPad2d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='mps'))), ModuleInfo(torch.nn.ZeroPad3d, module_inputs_func=module_inputs_torch_nn_ZeroPad3d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='mps'))), ModuleInfo(torch.nn.CircularPad1d, module_inputs_func=module_inputs_torch_nn_CircularPad1d, module_error_inputs_func=module_error_inputs_torch_nn_Pad1d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.CircularPad2d, module_inputs_func=module_inputs_torch_nn_CircularPad2d, module_error_inputs_func=module_error_inputs_torch_nn_Pad2d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.CircularPad3d, module_inputs_func=module_inputs_torch_nn_CircularPad3d, module_error_inputs_func=module_error_inputs_torch_nn_Pad3d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format'))), ModuleInfo(torch.nn.ConstantPad1d, module_inputs_func=module_inputs_torch_nn_ConstantPad1d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]),)), ModuleInfo(torch.nn.ConstantPad2d, module_inputs_func=module_inputs_torch_nn_ConstantPad2d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='mps'))), ModuleInfo(torch.nn.ConstantPad3d, module_inputs_func=module_inputs_torch_nn_ConstantPad3d, skips=(DecorateInfo(skipIfMps, 'TestModule', dtypes=[torch.float64]), DecorateInfo(unittest.expectedFailure, 'TestModule', 'test_memory_format', device_type='mps')))]