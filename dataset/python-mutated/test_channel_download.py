...
@pytest.fixture
async def channel_seeder(channel_tdef, tmp_path_factory):
    config = LibtorrentSettings()
    config.dht = False
    config.upnp = False
    config.natpmp = False
    config.lsd = False
    seeder_dlmgr = DownloadManager(state_dir=tmp_path_factory.mktemp('state_dir'), config=config, notifier=MagicMock(), peer_mid=b'0000')
...
async def gigachannel_manager(metadata_store, download_manager: DownloadManager):
    manager = GigaChannelManager(state_dir=metadata_store.channels_dir.parent, download_manager=download_manager, metadata_store=metadata_store, notifier=MagicMock())
    yield manager
