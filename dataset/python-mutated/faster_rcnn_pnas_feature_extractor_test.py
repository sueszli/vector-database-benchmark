...
class FasterRcnnPNASFeatureExtractorTest(tf.test.TestCase):
    ...
    def _build_feature_extractor(self, first_stage_features_stride):
        if False:
            return 10
        return frcnn_pnas.FasterRCNNPNASFeatureExtractor(is_training=False, first_stage_features_stride=first_stage_features_stride, batch_norm_trainable=False, reuse_weights=None, weight_decay=0.0)
    ...
    def test_filter_scaling_computation(self):
        if False:
            print('Hello World!')
        expected_filter_scaling = {((4, 8), 2): 1.0, ((4, 8), 7): 2.0, ((4, 8), 8): 2.0, ((4, 8), 9): 4.0}
        for (args, filter_scaling) in expected_filter_scaling.items():
            (reduction_indices, start_cell_num) = args
            self.assertAlmostEqual(frcnn_pnas._filter_scaling(reduction_indices, start_cell_num), filter_scaling)
