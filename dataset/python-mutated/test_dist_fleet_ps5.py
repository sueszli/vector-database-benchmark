...
class TestPSPassWithBow(unittest.TestCase):
    ...
    def net(self):
        ...
        q_emb = paddle.static.nn.embedding(input=q, is_distributed=is_distributed, size=[dict_dim, emb_dim], param_attr=base.ParamAttr(initializer=paddle.nn.initializer.Constant(value=0.01), name='__emb__', learning_rate=emb_lr), is_sparse=is_sparse)
        ...
        nt_emb = paddle.static.nn.embedding(input=nt, is_distributed=is_distributed, size=[dict_dim, emb_dim], param_attr=base.ParamAttr(initializer=paddle.nn.initializer.Constant(value=0.01), name='__emb__tmp_', learning_rate=emb_lr), is_sparse=False)
    ...
    def test(self):
        ...
        fleet.init(role)
        (loss, acc, _) = self.net()
        optimizer = paddle.optimizer.Adam(learning_rate=paddle.optimizer.lr.ExponentialDecay(learning_rate=base_lr, gamma=0.969))
