...
from airflow.providers.google.cloud.operators.dataprep import DataprepCopyFlowOperator, DataprepDeleteFlowOperator, DataprepGetJobGroupOperator, DataprepGetJobsForJobGroupOperator, DataprepRunFlowOperator, DataprepRunJobGroupOperator
...
WRITE_SETTINGS = {'writesettings': [{'path': GCS_BUCKET_PATH + f'adhoc_{RECIPE_NAME}.csv', 'action': 'create', 'format': 'csv'}]}
log = logging.getLogger(__name__)
with models.DAG(DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), catchup=False, tags=['example', 'dataprep'], render_template_as_native_obj=True) as dag:
    create_bucket_task = GCSCreateBucketOperator(task_id='create_bucket', bucket_name=GCS_BUCKET_NAME, project_id=GCP_PROJECT_ID)
    ...
    copy_task = DataprepCopyFlowOperator(task_id='copy_flow', dataprep_conn_id=CONNECTION_ID, project_id=GCP_PROJECT_ID, flow_id=FLOW_ID, name=f'copy_{DATASET_NAME}')
    run_job_group_task = DataprepRunJobGroupOperator(task_id='run_job_group', dataprep_conn_id=CONNECTION_ID, project_id=GCP_PROJECT_ID, body_request={'wrangledDataset': {'id': DATASET_WRANGLED_ID}, 'overrides': WRITE_SETTINGS})
    run_flow_task = DataprepRunFlowOperator(task_id='run_flow', dataprep_conn_id=CONNECTION_ID, project_id=GCP_PROJECT_ID, flow_id=FLOW_COPY_ID, body_request={})
    get_job_group_task = DataprepGetJobGroupOperator(task_id='get_job_group', dataprep_conn_id=CONNECTION_ID, project_id=GCP_PROJECT_ID, job_group_id="{{ task_instance.xcom_pull('run_flow')['data'][0]['id'] }}", embed='', include_deleted=False)
    get_jobs_for_job_group_task = DataprepGetJobsForJobGroupOperator(task_id='get_jobs_for_job_group', dataprep_conn_id=CONNECTION_ID, job_group_id="{{ task_instance.xcom_pull('run_flow')['data'][0]['id'] }}")
    check_flow_status_sensor = DataprepJobGroupIsFinishedSensor(task_id='check_flow_status', dataprep_conn_id=CONNECTION_ID, job_group_id="{{ task_instance.xcom_pull('run_flow')['data'][0]['id'] }}")
    check_job_group_status_sensor = DataprepJobGroupIsFinishedSensor(task_id='check_job_group_status', dataprep_conn_id=CONNECTION_ID, job_group_id="{{ task_instance.xcom_pull('run_job_group')['id'] }}")
    delete_flow_task = DataprepDeleteFlowOperator(task_id='delete_flow', dataprep_conn_id=CONNECTION_ID, flow_id="{{ task_instance.xcom_pull('copy_flow')['id'] }}")
    delete_flow_task.trigger_rule = TriggerRule.ALL_DONE
    delete_flow_task_original = DataprepDeleteFlowOperator(task_id='delete_flow_original', dataprep_conn_id=CONNECTION_ID, flow_id="{{ task_instance.xcom_pull('create_flow')['id'] }}", trigger_rule=TriggerRule.ALL_DONE)
    ...
    delete_bucket_task = GCSDeleteBucketOperator(task_id='delete_bucket', bucket_name=GCS_BUCKET_NAME, trigger_rule=TriggerRule.ALL_DONE)
    delete_connection = BashOperator(task_id='delete_connection', bash_command=f'airflow connections delete {CONNECTION_ID}', trigger_rule=TriggerRule.ALL_DONE)
    chain(create_bucket_task, create_connection_task, [create_imported_dataset_task, create_flow_task], create_wrangled_dataset_task, create_output_task, create_write_settings_task, copy_task, [run_job_group_task, run_flow_task], [get_job_group_task, get_jobs_for_job_group_task], [check_flow_status_sensor, check_job_group_status_sensor], delete_dataset_task, [delete_flow_task, delete_flow_task_original], [delete_bucket_task, delete_connection])
