...
def test_multiline_command(commandline, context):
    if False:
        for i in range(10):
            print('nop')
    assert_match(commandline, context, is_main_command=True)
NESTING_EXAMPLES = ((f'echo $({X})', '$('), (f'echo @$({X})', '@$('), (f'echo $(echo $({X}))', '$('), (f'echo @(x + $({X}))', '$('), (f'!({X})', '!('), (f'$[{X}]', '$['), (f'![{X}]', '!['))
NESTED_SIMPLE_CMD_EXAMPLES = [(nesting, f'simple {X}', CommandContext(args=(CommandArg('simple'),), arg_index=1, subcmd_opening=prefix)) for (nesting, prefix) in NESTING_EXAMPLES[1:]]
...
NESTING_MALFORMATIONS = (lambda s: s[:-1], lambda s: s + s[-1], lambda s: s[-1] + s, lambda s: s + '$(', lambda s: '$(' + s)
...
def test_malformed_subcmd(nesting, commandline, context, malformation):
    ...
    assert_match(nested_commandline, command_context=context, python_context=None)
MALFORMED_SUBCOMMANDS_NESTINGS = ((f'echo $(a $({X}', '$('), (f'echo $(a $(b; {X}', ''), (f'$(echo $(a $({X}', '$('), (f'echo $[a $({X}]', '$('), (f'echo $(a $[{X})', '$['), (f'echo @(x = $({X}', '$('), (f'echo @(a; x = $({X}', '$('), (f'echo @(x = $(a; {X}', ''))
...
SUBCMD_BORDER_EXAMPLES = ((f'{X}$(echo)', CommandContext((), 0, suffix='$(echo)')), (f'${X}(echo)', CommandContext((), 0, prefix='$', suffix='(echo)')), (f'$(echo){X}', CommandContext((), 0, prefix='$(echo)')), (f'${X}( echo)', CommandContext((), 0, prefix='$', suffix='( echo)')), (f'$(echo ){X}', CommandContext((), 0, prefix='$(echo )')))
...
MULTIPLE_CMD_SIMPLE_EXAMPLES = [(keyword, ('echo hi', f'simple {X}'), CommandContext(args=(CommandArg('simple'),), arg_index=1)) for keyword in MULTIPLE_COMMAND_KEYWORDS]
...
MULTIPLE_COMMAND_EXTENSIVE_EXAMPLES = tuple(itertools.chain((((first, second.replace(X, '')), first_context) for ((first, first_context), (second, second_context)) in EXTENSIVE_COMMAND_PAIRS), (((first.replace(X, ''), second), second_context) for ((first, first_context), (second, second_context)) in EXTENSIVE_COMMAND_PAIRS), (((first.replace(X, ''), second, third.replace(X, '')), second_context) for ((first, _1), (second, second_context), (third, _3)) in zip(COMMAND_EXAMPLES[:3], COMMAND_EXAMPLES[3:6], COMMAND_EXAMPLES[6:9])), (((first.replace(X, ''), second.replace(X, ''), third), third_context) for ((first, _1), (second, _2), (third, third_context)) in zip(COMMAND_EXAMPLES[:3], COMMAND_EXAMPLES[3:6], COMMAND_EXAMPLES[6:9]))))
...
MULTIPLE_COMMAND_BORDER_EXAMPLES = tuple(itertools.chain(itertools.chain(*(((f'ls{ws1}{X}{kwd}{ws2}echo', CommandContext((CommandArg('ls'),), 1) if ws1 else CommandContext((), 0, prefix='ls')),) for (ws1, ws2, kwd) in itertools.product(('', ' '), ('', ' '), ('&&', ';')))), ((f'ls {X}and echo', CommandContext((CommandArg('ls'), CommandArg('echo')), 1, suffix='and')), (f'ls and{X} echo', CommandContext((CommandArg('ls'), CommandArg('echo')), 1, prefix='and')), (f'ls ||{X} echo', CommandContext((CommandArg('ls'), CommandArg('echo')), 1, prefix='||'))), ((f'ls a{X}nd echo', CommandContext((CommandArg('ls'), CommandArg('echo')), 1, prefix='a', suffix='nd')), (f'ls &{X}& echo', CommandContext((CommandArg('ls'), CommandArg('echo')), 1, prefix='&', suffix='&')))))
...
PYTHON_EXAMPLES = ((f'x = {X}', PythonContext('x = ', 4)), (f'a {X}= x; b = y', PythonContext('a = x; b = y', 2)), (f'a {X}= x\nb = $(ls)', PythonContext('a = x\nb = $(ls)', 2)))
PYTHON_NESTING_EXAMPLES = (f'echo @({X})', f'echo $(echo @({X}))', f'echo @(x + @({X}))')
