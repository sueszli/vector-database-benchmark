...
import numpy as np
import pandas as pd
from featuretools.primitives import TimeSinceLastFalse, TimeSinceLastMax, TimeSinceLastMin, TimeSinceLastTrue

class TestTimeSinceLastFalse:
    primitive = TimeSinceLastFalse
    cutoff_time = datetime(2011, 4, 9, 11, 31, 27)
    times = pd.Series([datetime(2011, 4, 9, 10, 30, i * 6) for i in range(5)] + [datetime(2011, 4, 9, 10, 31, i * 9) for i in range(4)])
