...
class SyncOperationTests(BaseApiTestCase):
    ...
    async def test_execute_create_user(self):
        ...
        operation = self.build_create_operation(dict(remote_repo_link='test_link', repo_path=self.repo_path, branch='main', auth_type='https', user_git_settings=dict(username='username1', email=email, access_token='abc123')), user=user)
        ...
        self.assertEqual(get_secret_value(user.git_settings['access_token_secret_name'], self.repo_path), 'abc123')

    async def test_execute_create_both(self):
        email = self.faker.email()
        user = create_user(email=email, roles=1)
        operation = self.build_create_operation(dict(remote_repo_link='test_link', repo_path=self.repo_path, branch='main', auth_type='https', username='username', email='admin@admin.com', access_token='abc123', user_git_settings=dict(username='another_username', email=email, access_token='def456')), user=user)
        response = await operation.execute()
        self.assertEqual(response['sync']['remote_repo_link'], 'test_link')
        preferences = get_preferences(user=user).sync_config
        ...
        self.assertEqual(get_secret_value(project_preferences['access_token_secret_name'], self.repo_path), 'abc123')
        self.assertEqual(get_secret_value(user.git_settings['access_token_secret_name'], self.repo_path), 'def456')