...
def resume_instance(project_id: str, zone: str, instance_name: str) -> None:
    ...
    if instance.status != compute_v1.Instance.Status.SUSPENDED.name:
        raise RuntimeError(f'Only suspended instances can be resumed. Instance {instance_name} is in {instance.status} state.')
    operation = instance_client.resume(project=project_id, zone=zone, instance=instance_name)
