import logging
from typing import TYPE_CHECKING, Collection, Dict, Iterable, List, Optional, Set, Tuple, cast
from synapse.api.constants import EventContentFields
from synapse.logging import issue9533_logger
from synapse.logging.opentracing import SynapseTags, log_kv, set_tag, start_active_span, trace
...
class DeviceInboxWorkerStore(SQLBaseStore):
    ...
    async def get_messages_for_user_devices(self, user_ids: Collection[str], from_stream_id: int, to_stream_id: int) -> Dict[Tuple[str, str], List[JsonDict]]:
        """
        ...
        """
        (user_id_device_id_to_messages, last_processed_stream_id) = await self._get_device_messages(user_ids=user_ids, from_stream_id=from_stream_id, to_stream_id=to_stream_id)
        assert last_processed_stream_id == to_stream_id, 'Expected _get_device_messages to process all to-device messages up to `to_stream_id`'
        return user_id_device_id_to_messages

    async def get_messages_for_device(self, user_id: str, device_id: str, from_stream_id: int, to_stream_id: int, limit: int=100) -> Tuple[List[JsonDict], int]:
        """
        ...
        """
        (user_id_device_id_to_messages, last_processed_stream_id) = await self._get_device_messages(user_ids=[user_id], device_id=device_id, from_stream_id=from_stream_id, to_stream_id=to_stream_id, limit=limit)
    ...
    async def _get_device_messages(self, user_ids: Collection[str], from_stream_id: int, to_stream_id: int, device_id: Optional[str]=None, limit: Optional[int]=None) -> Tuple[Dict[Tuple[str, str], List[JsonDict]], int]:
        """
        ...
        if len(user_ids) > 1 and device_id is not None:
            raise AssertionError("Programming error: 'device_id' cannot be supplied to _get_device_messages when >1 user_id has been provided")
        if limit is not None and device_id is None:
            raise AssertionError("Programming error: _get_device_messages was passed 'limit' without a specific user_id/device_id")
        user_ids_to_query: Set[str] = set()
        device_ids_to_query: Set[str] = set()
        if device_id is not None:
            device_ids_to_query.add(device_id)
        for user_id in user_ids:
            if self._device_inbox_stream_cache.has_entity_changed(user_id, from_stream_id):
        ...
        return await self.db_pool.runInteraction('get_device_messages', get_device_messages_txn)

    @trace
    async def delete_messages_for_device(self, user_id: str, device_id: Optional[str], up_to_stream_id: int, limit: Optional[int]=None) -> int:
        """
        Args:
        ...
        def delete_messages_for_device_txn(txn: LoggingTransaction) -> int:
            ...
            return txn.rowcount
        count = await self.db_pool.runInteraction('delete_messages_for_device', delete_messages_for_device_txn)
        log_kv({'message': f'deleted {count} messages for device', 'count': count})
        if count == limit:
            return count
        updated_last_deleted_stream_id = self._last_device_delete_cache.get((user_id, device_id), 0)
        self._last_device_delete_cache[user_id, device_id] = max(updated_last_deleted_stream_id, up_to_stream_id)
        return count

    @trace
    async def get_new_device_msgs_for_remote(self, destination: str, last_stream_id: int, current_stream_id: int, limit: int) -> Tuple[List[JsonDict], int]:
        """
        Args:
        ...
        return await self.db_pool.runInteraction('get_new_device_msgs_for_remote', get_new_messages_for_remote_destination_txn)

    @trace
    async def delete_device_msgs_for_remote(self, destination: str, up_to_stream_id: int) -> None:
        ...
        await self.db_pool.runInteraction('delete_device_msgs_for_remote', delete_messages_for_remote_destination_txn)

    async def get_all_new_device_messages(self, instance_name: str, last_id: int, current_id: int, limit: int) -> Tuple[List[Tuple[int, tuple]], int, bool]:
        ...
        return await self.db_pool.runInteraction('get_all_new_device_messages', get_all_new_device_messages_txn)

    @trace
    async def add_messages_to_device_inbox(self, local_messages_by_user_then_device: Dict[str, Dict[str, JsonDict]], remote_messages_by_destination: Dict[str, JsonDict]) -> int:
        ...
        async with self._device_inbox_id_gen.get_next() as stream_id:
            ...
            await self.db_pool.runInteraction('add_messages_to_device_inbox', add_messages_txn, now_ms, stream_id)
            for user_id in local_messages_by_user_then_device.keys():
                self._device_inbox_stream_cache.entity_has_changed(user_id, stream_id)
            for destination in remote_messages_by_destination.keys():
                self._device_federation_outbox_stream_cache.entity_has_changed(destination, stream_id)
        return self._device_inbox_id_gen.get_current_token()

    async def add_messages_from_remote_to_device_inbox(self, origin: str, message_id: str, local_messages_by_user_then_device: Dict[str, Dict[str, JsonDict]]) -> int:
        assert self._can_write_to_device
        ...
        async with self._device_inbox_id_gen.get_next() as stream_id:
            now_ms = self._clock.time_msec()
            await self.db_pool.runInteraction('add_messages_from_remote_to_device_inbox', add_messages_txn, now_ms, stream_id)
            for user_id in local_messages_by_user_then_device.keys():
                self._device_inbox_stream_cache.entity_has_changed(user_id, stream_id)
        return stream_id
...
class DeviceInboxBackgroundUpdateStore(SQLBaseStore):
    ...
    async def _background_drop_index_device_inbox(self, progress: JsonDict, batch_size: int) -> int:
    ...
    async def _remove_dead_devices_from_device_inbox(self, progress: JsonDict, batch_size: int) -> int:
        ...
        finished = await self.db_pool.runInteraction('_remove_devices_from_device_inbox_txn', _remove_dead_devices_from_device_inbox_txn)
        if finished:
            await self.db_pool.updates._end_background_update(self.REMOVE_DEAD_DEVICES_FROM_INBOX)
        return batch_size
