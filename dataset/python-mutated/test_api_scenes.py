...
async def test_get_scenes(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, scene_data) -> None:
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_get_scene(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, scene_data) -> None:
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_save_scene(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, scene_data, remove_json) -> None:
    """Test saving an Insteon scene."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, scene_data)
    mock_add_or_update_scene = AsyncMock(return_value=(20, ResponseStatus.SUCCESS))
    with patch.object(pyinsteon.managers.scene_manager, 'devices', devices), patch.object(scenes, 'async_add_or_update_scene', mock_add_or_update_scene):
        scene = await pyinsteon.managers.scene_manager.async_get_scene(20)
        scene['devices']['1a1a1a'] = []
        links = _scene_to_array(scene)
        await ws_client.send_json({ID: 1, TYPE: 'insteon/scene/save', 'scene_id': 20, 'name': 'Some scene name', 'links': links})
        msg = await ws_client.receive_json()
        result = msg['result']
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_save_new_scene(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, scene_data, remove_json) -> None:
    ...
    with patch.object(pyinsteon.managers.scene_manager, 'devices', devices), patch.object(scenes, 'async_add_or_update_scene', mock_add_or_update_scene):
        ...
        scene['devices']['1a1a1a'] = []
        links = _scene_to_array(scene)
        await ws_client.send_json({ID: 1, TYPE: 'insteon/scene/save', 'scene_id': -1, 'name': 'Some scene name', 'links': links})
        msg = await ws_client.receive_json()
        result = msg['result']
        assert result['result']
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_save_scene_error(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, scene_data, remove_json) -> None:
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_delete_scene(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, scene_data, remove_json) -> None:
    """Test delete an Insteon scene."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, scene_data)
    mock_delete_scene = AsyncMock(return_value=ResponseStatus.SUCCESS)
    with patch.object(pyinsteon.managers.scene_manager, 'devices', devices), patch.object(scenes, 'async_delete_scene', mock_delete_scene):
        await ws_client.send_json({ID: 1, TYPE: 'insteon/scene/delete', 'scene_id': 20})
        msg = await ws_client.receive_json()
        result = msg['result']
        assert result['result']
        assert result['scene_id'] == 20