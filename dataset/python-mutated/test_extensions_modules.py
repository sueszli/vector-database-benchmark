...
class TestExtensionsModules:
    ...
    def test_module_usage(self, deploy_cfn_template, register_extension, snapshot, aws_client):
        ...
        register_extension(extension_type='MODULE', extension_name='LocalStack::Testing::TestModule::MODULE', artifact_path=artifact_path)
        template_path = os.path.join(os.path.dirname(__file__), '../../../templates/registry/module.yml')
        module_bucket_name = f'bucket-module-{short_uid()}'
        stack = deploy_cfn_template(template_path=template_path, parameters={'BucketName': module_bucket_name}, max_wait=300)
