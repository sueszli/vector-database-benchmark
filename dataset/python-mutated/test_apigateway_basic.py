...
from localstack.constants import APPLICATION_JSON, TEST_AWS_ACCESS_KEY_ID, TEST_AWS_ACCOUNT_ID, TEST_AWS_REGION_NAME
from localstack.services.apigateway.helpers import TAG_KEY_CUSTOM_ID, connect_api_gateway_to_sqs, get_resource_for_path, get_rest_api_paths, host_based_url, path_based_url
...
from tests.aws.services.apigateway.apigateway_fixtures import UrlType, api_invoke_url, create_rest_api_deployment, create_rest_api_integration, create_rest_api_integration_response, create_rest_api_method_response, create_rest_api_stage, create_rest_resource, create_rest_resource_method, delete_rest_api, get_rest_api, update_rest_api_deployment, update_rest_api_stage
from tests.aws.services.apigateway.conftest import APIGATEWAY_ASSUME_ROLE_POLICY, APIGATEWAY_LAMBDA_POLICY, APIGATEWAY_STEPFUNCTIONS_POLICY, STEPFUNCTIONS_ASSUME_ROLE_POLICY
from tests.aws.services.lambda_.test_lambda import TEST_LAMBDA_AWS_PROXY, TEST_LAMBDA_HTTP_RUST, TEST_LAMBDA_NODEJS, TEST_LAMBDA_NODEJS_APIGW_502, TEST_LAMBDA_NODEJS_APIGW_INTEGRATION, TEST_LAMBDA_PYTHON, TEST_LAMBDA_PYTHON_ECHO
...
ApiGatewayLambdaProxyIntegrationTestResult = namedtuple('ApiGatewayLambdaProxyIntegrationTestResult', ['data', 'resource', 'result', 'url', 'path_with_replace'])
...
class TestAPIGateway:
    ...
    TEST_API_GATEWAY_AUTHORIZER = {'name': 'test', 'type': 'TOKEN', 'providerARNs': ['arn:aws:cognito-idp:us-east-1:123412341234:userpool/us-east-1_123412341'], 'authType': 'custom', 'authorizerUri': 'arn:aws:apigateway:us-east-1:lambda:path/2015-03-31/functions/' + 'arn:aws:lambda:us-east-1:123456789012:function:myApiAuthorizer/invocations', 'authorizerCredentials': 'arn:aws:iam::123456789012:role/apigAwsProxyRole', 'identitySource': 'method.request.header.Authorization', 'identityValidationExpression': '.*', 'authorizerResultTtlInSeconds': 300}
