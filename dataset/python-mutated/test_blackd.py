...
class BlackDTestCase(AioHTTPTestCase):
    ...
    async def test_blackd_request_syntax_error(self) -> None:
        ...
        self.assertTrue(content.startswith('Cannot parse'), msg=f"Expected error to start with 'Cannot parse', got {repr(content)}")

    @unittest_run_loop
    async def test_blackd_unsupported_version(self) -> None:
        response = await self.client.post('/', data=b'what', headers={blackd.PROTOCOL_VERSION_HEADER: '2'})
        self.assertEqual(response.status, 501)

    @unittest_run_loop
    async def test_blackd_supported_version(self) -> None:
        response = await self.client.post('/', data=b'what', headers={blackd.PROTOCOL_VERSION_HEADER: '1'})
        self.assertEqual(response.status, 200)
    ...
    async def test_blackd_invalid_python_variant(self) -> None:
        ...
        async def check(header_value: str, expected_status: int=400) -> None:
            response = await self.client.post('/', data=b'what', headers={blackd.PYTHON_VARIANT_HEADER: header_value})
    ...
    async def test_blackd_pyi(self) -> None:
        ...
        response = await self.client.post('/', data=source, headers={blackd.PYTHON_VARIANT_HEADER: 'pyi'})
        self.assertEqual(response.status, 200)
        self.assertEqual(await response.text(), expected)

    @unittest_run_loop
    async def test_blackd_diff(self) -> None:
        diff_header = re.compile('(In|Out)\\t\\d\\d\\d\\d-\\d\\d-\\d\\d \\d\\d:\\d\\d:\\d\\d\\.\\d\\d\\d\\d\\d\\d\\+\\d\\d:\\d\\d')
        (source, _) = read_data('miscellaneous', 'blackd_diff')
        (expected, _) = read_data('miscellaneous', 'blackd_diff.diff')
        response = await self.client.post('/', data=source, headers={blackd.DIFF_HEADER: 'true'})
        self.assertEqual(response.status, 200)
    ...
    async def test_blackd_python_variant(self) -> None:
        ...
        async def check(header_value: str, expected_status: int) -> None:
            ...
            self.assertEqual(response.status, expected_status, msg=await response.text())
        await check('3.6', 200)
    ...
    async def test_blackd_line_length(self) -> None:
        response = await self.client.post('/', data=b'print("hello")\n', headers={blackd.LINE_LENGTH_HEADER: '7'})
        self.assertEqual(response.status, 200)
    ...
    async def test_blackd_skip_first_source_line(self) -> None:
        ...
        self.assertEqual(response.status, 400)
        response = await self.client.post('/', data=invalid_first_line, headers={blackd.SKIP_SOURCE_FIRST_LINE: 'true'})
        self.assertEqual(response.status, 200)
    ...
    async def test_cors_preflight(self) -> None:
        response = await self.client.options('/', headers={'Access-Control-Request-Method': 'POST', 'Origin': '*', 'Access-Control-Request-Headers': 'Content-Type'})
        self.assertEqual(response.status, 200)
