...
EMPTY_BODY_ABSTRACT: Final = ErrorMessage('If the method is meant to be abstract, use @abc.abstractmethod', codes.EMPTY_BODY)
...
INVALID_EXCEPTION_TYPE: Final = ErrorMessage('Exception type must be derived from BaseException (or be a tuple of exception classes)')
...
INVALID_RETURN_TYPE_FOR_GENERATOR: Final = ErrorMessage('The return type of a generator function should be "Generator" or one of its supertypes')
INVALID_RETURN_TYPE_FOR_ASYNC_GENERATOR: Final = ErrorMessage('The return type of an async generator function should be "AsyncGenerator" or one of its supertypes')
...
COVARIANT_OVERRIDE_OF_MUTABLE_ATTRIBUTE: Final = ErrorMessage('Covariant override of a mutable attribute', code=codes.MUTABLE_OVERRIDE)
...
CANNOT_ACCESS_INIT: Final = 'Accessing "__init__" on an instance is unsound, since instance.__init__ could be from an incompatible subclass'
...
KEYWORD_ARGUMENT_REQUIRES_STR_KEY_TYPE: Final = ErrorMessage('Keyword argument only valid with "str" key type in call to "dict"')
...
TYPE_ALWAYS_TRUE: Final = ErrorMessage('{} which does not implement __bool__ or __len__ so it could always be true in boolean context', code=codes.TRUTHY_BOOL)
TYPE_ALWAYS_TRUE_UNIONTYPE: Final = ErrorMessage('{} of which no members implement __bool__ or __len__ so it could always be true in boolean context', code=codes.TRUTHY_BOOL)
FUNCTION_ALWAYS_TRUE: Final = ErrorMessage('Function {} could always be true in boolean context', code=codes.TRUTHY_FUNCTION)
ITERABLE_ALWAYS_TRUE: Final = ErrorMessage('{} which can always be true in boolean context. Consider using {} instead.', code=codes.TRUTHY_ITERABLE)
...
SUPER_ENCLOSING_POSITIONAL_ARGS_REQUIRED: Final = ErrorMessage('"super()" requires one or two positional arguments in enclosing function')
MISSING_OR_INVALID_SELF_TYPE: Final = ErrorMessage('Self argument missing for a non-static method (or an invalid type for self)')
ERASED_SELF_TYPE_NOT_SUPERTYPE: Final = ErrorMessage('The erased type of self "{}" is not a supertype of its class "{}"')
...
ENUM_MEMBERS_ATTR_WILL_BE_OVERRIDEN: Final = ErrorMessage('Assigned "__members__" will be overridden by "Enum" internally')
CANNOT_OVERRIDE_INSTANCE_VAR: Final = ErrorMessage('Cannot override instance variable (previously declared on base class "{}") with class variable')
CANNOT_OVERRIDE_CLASS_VAR: Final = ErrorMessage('Cannot override class variable (previously declared on base class "{}") with instance variable')
...
RUNTIME_PROTOCOL_EXPECTED: Final = ErrorMessage('Only @runtime_checkable protocols can be used with instance and class checks')
...
TYPE_IGNORE_WITH_ERRCODE_ON_MODULE: Final = ErrorMessage('type ignore with error code is not supported for modules; use `# mypy: disable-error-code="{}"`', codes.SYNTAX)
