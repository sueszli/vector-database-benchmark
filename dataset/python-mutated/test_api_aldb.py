...
from homeassistant.components.insteon.api.aldb import ALDB_RECORD, DEVICE_ADDRESS, ID, TYPE
...
async def test_get_aldb(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    ...
    with patch.object(insteon.api.aldb, 'devices', devices):
        await ws_client.send_json({ID: 2, TYPE: 'insteon/aldb/get', DEVICE_ADDRESS: '33.33.33'})
        msg = await ws_client.receive_json()
        result = msg['result']
        assert len(result) == 5

@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_change_aldb_record(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    """Test changing an Insteon device's All-Link Database record."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, aldb_data)
    change_rec = _aldb_dict(4079)
    with patch.object(insteon.api.aldb, 'devices', devices):
        await ws_client.send_json({ID: 2, TYPE: 'insteon/aldb/change', DEVICE_ADDRESS: '33.33.33', ALDB_RECORD: change_rec})
        msg = await ws_client.receive_json()
        assert msg['success']
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_create_aldb_record(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    """Test creating a new Insteon All-Link Database record."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, aldb_data)
    new_rec = _aldb_dict(4079)
    with patch.object(insteon.api.aldb, 'devices', devices):
        await ws_client.send_json({ID: 2, TYPE: 'insteon/aldb/create', DEVICE_ADDRESS: '33.33.33', ALDB_RECORD: new_rec})
        msg = await ws_client.receive_json()
        assert msg['success']
        assert len(devices['33.33.33'].aldb.pending_changes) == 1
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_write_aldb(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    """Test writing an Insteon device's All-Link Database."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, aldb_data)
    with patch.object(insteon.api.aldb, 'devices', devices):
        await ws_client.send_json({ID: 2, TYPE: 'insteon/aldb/write', DEVICE_ADDRESS: '33.33.33'})
        msg = await ws_client.receive_json()
        assert msg['success']
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_load_aldb(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    """Test loading an Insteon device's All-Link Database."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, aldb_data)
    with patch.object(insteon.api.aldb, 'devices', devices):
        await ws_client.send_json({ID: 2, TYPE: 'insteon/aldb/load', DEVICE_ADDRESS: 'AA.AA.AA'})
        msg = await ws_client.receive_json()
        assert msg['success']
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_reset_aldb(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    """Test resetting an Insteon device's All-Link Database."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, aldb_data)
    record = _aldb_dict(4079)
    devices['33.33.33'].aldb.modify(mem_addr=record['mem_addr'], in_use=record['in_use'], group=record['group'], controller=record['is_controller'], target=record['target'], data1=record['data1'], data2=record['data2'], data3=record['data3'])
    assert devices['33.33.33'].aldb.pending_changes
    with patch.object(insteon.api.aldb, 'devices', devices):
        await ws_client.send_json({ID: 2, TYPE: 'insteon/aldb/reset', DEVICE_ADDRESS: '33.33.33'})
        msg = await ws_client.receive_json()
        assert msg['success']
        assert not devices['33.33.33'].aldb.pending_changes

@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_default_links(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    """Test getting an Insteon device's All-Link Database."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, aldb_data)
    with patch.object(insteon.api.aldb, 'devices', devices):
        await ws_client.send_json({ID: 2, TYPE: 'insteon/aldb/add_default_links', DEVICE_ADDRESS: '33.33.33'})
        msg = await ws_client.receive_json()
        assert msg['success']
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_notify_on_aldb_status(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    """Test getting an Insteon device's All-Link Database."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, aldb_data)
    with patch.object(insteon.api.aldb, 'devices', devices):
        await ws_client.send_json({ID: 2, TYPE: 'insteon/aldb/notify', DEVICE_ADDRESS: '33.33.33'})
        msg = await ws_client.receive_json()
        assert msg['success']
...
@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_notify_on_aldb_record_added(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    """Test getting an Insteon device's All-Link Database."""
    (ws_client, devices) = await _setup(hass, hass_ws_client, aldb_data)
    with patch.object(insteon.api.aldb, 'devices', devices):
        await ws_client.send_json({ID: 2, TYPE: 'insteon/aldb/notify', DEVICE_ADDRESS: '33.33.33'})
        msg = await ws_client.receive_json()
        assert msg['success']
        pub.sendMessage(f'333333.{ALDB_LINK_CHANGED}', record='some record', sender=Address('11.11.11'), deleted=False)
        msg = await ws_client.receive_json()
        assert msg['event']['type'] == 'record_loaded'

@pytest.mark.parametrize('expected_lingering_tasks', [True])
async def test_bad_address(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, aldb_data) -> None:
    ...
    for call in ['get', 'write', 'load', 'reset', 'add_default_links', 'notify']:
        ws_id += 1
        await ws_client.send_json({ID: ws_id, TYPE: f'insteon/aldb/{call}', DEVICE_ADDRESS: '99.99.99'})
        msg = await ws_client.receive_json()
        assert not msg['success']
        assert msg['error']['message'] == INSTEON_DEVICE_NOT_FOUND
    for call in ['change', 'create']:
        ws_id += 1
        await ws_client.send_json({ID: ws_id, TYPE: f'insteon/aldb/{call}', DEVICE_ADDRESS: '99.99.99', ALDB_RECORD: record})
        msg = await ws_client.receive_json()
        assert not msg['success']
        assert msg['error']['message'] == INSTEON_DEVICE_NOT_FOUND