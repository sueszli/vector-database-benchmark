...
class OpenAssistant(AsyncGeneratorProvider):
    ...
    @classmethod
    async def create_async_generator(cls, model: str, messages: Messages, proxy: str=None, cookies: dict=None, **kwargs) -> AsyncResult:
        if not cookies:
        ...
        async with ClientSession(cookies=cookies, headers=headers) as session:
            ...
            data = {'chat_id': chat_id, 'content': f'<s>[INST]\n{format_prompt(messages)}\n[/INST]', 'parent_id': None}
            ...
            data = {'chat_id': chat_id, 'parent_id': parent_id, 'model_config_name': model if model else cls.model, 'sampling_parameters': {'top_k': 50, 'top_p': None, 'typical_p': None, 'temperature': 0.35, 'repetition_penalty': 1.1111111111111112, 'max_new_tokens': 1024, **kwargs}, 'plugins': []}
