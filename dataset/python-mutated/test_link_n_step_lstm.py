...
@testing.parameterize(*testing.product({'hidden_none': [True, False]}))
class TestNStepLSTM(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rnn = links.NStepLSTM(self.n_layers, self.in_size, self.out_size, self.dropout)
    ...
    def check_forward(self, h_data, c_data, xs_data):
        ...
        for (batch, seq) in enumerate(self.xs):
            for layer in range(self.n_layers):
                ...
                hs = []
                for x in seq:
                    i = sigmoid(x.dot(p.w0.array.T) + h_prev.dot(p.w4.array.T) + p.b0.array + p.b4.array)
                    f = sigmoid(x.dot(p.w1.array.T) + h_prev.dot(p.w5.array.T) + p.b1.array + p.b5.array)
                    c_bar = numpy.tanh(x.dot(p.w2.array.T) + h_prev.dot(p.w6.array.T) + p.b2.array + p.b6.array)
                    o = sigmoid(x.dot(p.w3.array.T) + h_prev.dot(p.w7.array.T) + p.b3.array + p.b7.array)
    ...
    def test_forward_gpu_train(self):
        ...
        with chainer.using_config('use_cudnn', 'always'), chainer.using_config('train', True):
            self.check_forward(cuda.to_gpu(self.h), cuda.to_gpu(self.c), [cuda.to_gpu(x) for x in self.xs])
    ...
    def test_forward_nonzero_gpu_test(self):
        ...
        with testing.assert_warns(DeprecationWarning):
            self.rnn.to_gpu(1)
        with chainer.using_config('use_cudnn', 'always'), chainer.using_config('train', False):
            self.check_forward(cuda.to_gpu(self.h, 1), cuda.to_gpu(self.c, 1), [cuda.to_gpu(x, 1) for x in self.xs])
    ...
    def check_backward(self, h_data, c_data, xs_data, gh_data, gc_data, gys_data):
        if False:
            i = 10
            return i + 15
        ...
        if self.hidden_none:
            ...
            gradient_check.check_backward(fun, tuple(in_data), tuple([gh_data, gc_data] + gys_data), tuple(params), eps=0.01, rtol=0.001, atol=0.001)

    def test_backward_cpu(self):
        if False:
            i = 10
            return i + 15
        self.check_backward(self.h, self.c, self.xs, self.gh, self.gc, self.gys)
    ...
    def test_backward_gpu(self):
        ...
        with chainer.using_config('use_cudnn', 'always'):
            self.check_backward(cuda.to_gpu(self.h), cuda.to_gpu(self.c), [cuda.to_gpu(x) for x in self.xs], cuda.to_gpu(self.gh), cuda.to_gpu(self.gc), [cuda.to_gpu(gy) for gy in self.gys])
...
class TestNStepBiLSTM(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rnn = links.NStepBiLSTM(self.n_layers, self.in_size, self.out_size, self.dropout)
        for layer in self.rnn:
            for p in layer.params():
                p.array[...] = numpy.random.uniform(-1, 1, p.shape)
        self.rnn.cleargrads()
