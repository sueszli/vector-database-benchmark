...
TRANSLATION_TYPE = [('field', 'Field'), ('model', 'Object'), ('report', 'Report/Template'), ('selection', 'Selection'), ('view', 'View'), ('help', 'Help'), ('code', 'Code'), ('constraint', 'Constraint'), ('sql_constraint', 'SQL Constraint')]
...
class IrTranslation(models.Model):
    ...
    source = fields.Text(string='Source term', compute='_compute_source', inverse='_inverse_source', search='_search_source')
    value = fields.Text(string='Translation Value')
    module = fields.Char(index=True, help='Module this term belongs to')
    state = fields.Selection([('to_translate', 'To Translate'), ('inprogress', 'Translation in Progress'), ('translated', 'Translated')], string='Status', default='to_translate', help='Automatically set to let administators find new terms that might need to be translated')
    comments = fields.Text(string='Translation comments', index=True)
    _sql_constraints = [('lang_fkey_res_lang', 'FOREIGN KEY(lang) REFERENCES res_lang(code)', 'Language code of translation item must be among known languages')]
