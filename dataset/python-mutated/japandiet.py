import re
from ..utils import ExtractorError, clean_html, int_or_none, join_nonempty, parse_qs, smuggle_url, traverse_obj, try_call, unsmuggle_url
...
class ShugiinItvLiveIE(ShugiinItvBaseIE):
    ...
    _TESTS = [{'url': 'https://www.shugiintv.go.jp/jp/index.php', 'info_dict': {'_type': 'playlist', 'title': 'All proceedings for today'}, 'playlist_mincount': 1}]
...
class ShugiinItvLiveRoomIE(ShugiinItvBaseIE):
    ...
    _TESTS = [{'url': 'https://www.shugiintv.go.jp/jp/index.php?room_id=room01', 'info_dict': {'id': 'room01', 'title': '内閣委員会'}, 'skip': 'this runs for a time and not every day'}, {'url': 'https://www.shugiintv.go.jp/jp/index.php?room_id=room11', 'info_dict': {'id': 'room11', 'title': '外務委員会'}, 'skip': 'this runs for a time and not every day'}]
...
class ShugiinItvVodIE(ShugiinItvBaseIE):
    ...
    _TESTS = [{'url': 'https://www.shugiintv.go.jp/jp/index.php?ex=VL&media_type=&deli_id=53846', 'info_dict': {'id': '53846', 'title': 'ウクライナ大統領国会演説（オンライン）', 'release_date': '20220323', 'chapters': 'count:4'}}, {'url': 'https://www.shugiintv.go.jp/en/index.php?ex=VL&media_type=&deli_id=53846', 'only_matching': True}]
