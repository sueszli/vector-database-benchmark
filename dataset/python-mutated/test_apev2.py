...
from picard.formats import apev2, open_
...
VALID_KEYS = {' valid Key}', '{ $ome tag~}', 'xx', 'x' * 255}
INVALID_KEYS = {'invalid\x7fkey', 'invalid\x19key', '', 'x', 'x' * 256, 'ID3', 'TAG', 'OggS', 'MP+'}
...
class MonkeysAudioTest(CommonApeTests.ApeTestCase):
    ...
    expected_info = {'length': 82, '~channels': '2', '~sample_rate': '44100', '~bits_per_sample': '16'}
...
class MusepackSV7Test(CommonApeTests.ApeTestCase):
    ...
    supports_ratings = False
    expected_info = {'length': 91, '~channels': '2', '~sample_rate': '44100'}
    unexpected_info = ['~video']
...
class OptimFROGLosslessTest(CommonApeTests.ApeTestCase):
    testfile = 'test.ofr'
    supports_ratings = False
    expected_info = {'length': 0, '~channels': '2', '~sample_rate': '48000'}
    unexpected_info = ['~video']
