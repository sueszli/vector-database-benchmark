...
def main(location):
    ...
    def GraphColumn(name, key):
        if False:
            return 10
        layout = [[sg.Graph(gsize, (-gsize[0] // 2, 0), (gsize[0] // 2, gsize[1]), key=key + '-GRAPH-')], [sg.T(size=(5, 1), justification='c', font='Courier 10', k=key + '-GAUGE VALUE-')]]
    ...
    layout = [[sg.Button(image_data=sg.red_x, button_color=('black', 'black'), key='Exit', tooltip='Closes window'), sg.Text('     CPU Core Usage')]]
    ...
    window = sg.Window('CPU Cores Usage Widget', layout, keep_on_top=True, auto_size_buttons=False, grab_anywhere=True, no_titlebar=True, default_button_element_size=(12, 1), return_keyboard_events=True, alpha_channel=TRANSPARENCY, use_default_focus=False, finalize=True, location=location, right_click_menu=[[''], 'Exit'])
    graphs = [DashGraph(gsize, window['-CPU-' + str(i) + '-GRAPH-'], window['-CPU-' + str(i) + '-GAUGE VALUE-'], 0, colors[i % 6]) for i in range(num_cores)]
    while True:
