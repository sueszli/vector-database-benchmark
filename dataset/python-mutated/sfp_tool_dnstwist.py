...
class sfp_tool_dnstwist(SpiderFootPlugin):
    meta = {'name': 'Tool - DNSTwist', 'summary': 'Identify bit-squatting, typo and other similar domains to the target using a local DNSTwist installation.', 'flags': ['tool'], 'useCases': ['Footprint', 'Investigate'], 'categories': ['DNS'], 'toolDetails': {'name': 'DNSTwist', 'description': 'See what sort of trouble users can get in trying to type your domain name. Find lookalike domains that adversaries can use to attack you. Can detect typosquatters, phishing attacks, fraud, and brand impersonation. Useful as an additional source of targeted threat intelligence.', 'website': 'https://github.com/elceef/dnstwist', 'repository': 'https://github.com/elceef/dnstwist'}}
    opts = {'pythonpath': 'python', 'dnstwistpath': '', 'skipwildcards': True}
    optdescs = {'pythonpath': "Path to Python interpreter to use for DNSTwist. If just 'python' then it must be in your PATH.", 'dnstwistpath': 'Path to the where the dnstwist.py file lives. Optional.', 'skipwildcards': 'Skip TLDs and sub-TLDs that have wildcard DNS.'}
    results = None
    errorState = False
