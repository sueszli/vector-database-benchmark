...
def benchmark_sparse_lengths_sum(dtype_str, categorical_limit, embedding_size, average_len, batch_size, iterations, flush_cache):
    ...
    def f(_, outputs):
        if False:
            return 10
        lengths = np.random.randint(int(np.round(average_len * 0.75)), int(np.round(average_len * 1.25)) + 1, batch_size).astype(np.int32)
    ...
    print('{} billion sums per cycle'.format(embedding_size * workspace.FetchBlob('indices').size / runtimes[2 if flush_cache else 1] / 1000000.0))
