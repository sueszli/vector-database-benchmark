...
if __name__ == '__main__':
    ...
    envs = envpool.make(args.env_id, env_type='gym', num_envs=args.num_envs, episodic_life=True, reward_clip=True, seed=args.seed, repeat_action_probability=0.25)
    ...
    optimizer = optim.Adam(combined_parameters, lr=args.learning_rate, eps=1e-05)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            rnd_next_obs = ((next_obs[:, 3, :, :].reshape(args.num_envs, 1, 84, 84) - torch.from_numpy(obs_rms.mean).to(device)) / torch.sqrt(torch.from_numpy(obs_rms.var).to(device))).clip(-5, 5).float()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.clip_vloss:
                    ...
                    ext_v_clipped = b_ext_values[mb_inds] + torch.clamp(new_ext_values - b_ext_values[mb_inds], -args.clip_coef, args.clip_coef)
