...
class TbSevenPl(MultiDownloader):
    ...
    SIZE_PATTERN = 'Rozmiar: <span class="type red">(?P<S>[\\d.,]+) (?P<U>[\\w_^]+)<'
    LINK_PATTERN = '<a href="(.+?)" target="_blank">Pobierz</a>'

    def handle_premium(self, pyfile):
        if False:
            for i in range(10):
                print('nop')
        self.data = self.load('https://tb7.pl/mojekonto/sciagaj', post={'step': 1, 'content': pyfile.url})
        ...
        self.data = self.load('https://tb7.pl/mojekonto/sciagaj', post={'step': 2, '0': 'on'})
