...
Usage:
   ...
   CUDA_VISIBLE_DEVICES=0 python3 examples/wav2vec/gen_audio_embedding.py /fsx/data/VoxLingua107/manifest --path ${fdir}     --task audio_classification --batch-size 90 --gen-subset test     --infer-manifest /fsx/data/VoxLingua107/manifest/test.tsv     --infer-xtimes 10 --infer-max-sample-size 160000 --output-path $odir 
    ...
    python3  examples/wav2vec/gen_audio_embedding.py /fsx/data/VoxLingua107/manifest --path ${fdir}         --task audio_classification --batch-size 90 --gen-subset test         --infer-manifest /fsx/data/VoxLingua107/manifest/test.tsv         --infer-xtimes 10 --infer-max-sample-size 160000 --output-path $odir
...
if __name__ == '__main__':
    ...
    (models, _model_args, task) = checkpoint_utils.load_model_ensemble_and_task([args.path], arg_overrides=arg_overrides, task=None, state=state)
    model = models[0]
    model.eval()
    if use_cuda:
        model.cuda()
    ...
    itr = task.get_batch_iterator(dataset=infer_dataset, max_sentences=args.batch_size).next_epoch_itr(shuffle=False)
    list_uttname = []
    list_latent = []
    list_logit = []
    list_target = []
    ...
    with open(output_path, 'wb') as ww:
        np.savez(ww, name=list_uttname, latent=list_latent, target=list_target, logit=list_logit, src_len=list_src_len)
    print('=' * 10 + ' REPORT ' + '=' * 10)
    print(f'| latent saved in {output_path}')
    print(f'| list_uttname.shape={list_uttname.shape!r}, list_latent.shape={list_latent.shape!r}, list_target.shape={list_target.shape!r}, list_logit.shape={list_logit.shape!r}, list_src_len.shape={list_src_len.shape!r}')