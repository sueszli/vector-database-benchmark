...
class GfycatMIMEParser(BaseMIMEParser):
    ...
    """
    pattern = re.compile('https?://(www\\.)?gfycat\\.com/[^.]+$')
...
class YoutubeMIMEParser(BaseMIMEParser):
    ...
    """
    pattern = re.compile("(?:https?://)?(m\\.)?(?:youtu\\.be/|(?:www\\.)?youtube\\.com/watch(?:\\.php)?\\'?.*v=)([a-zA-Z0-9\\-_]+)")
...
class VimeoMIMEParser(BaseMIMEParser):
    ...
    Assign a custom mime-type so they can be referenced in mailcap.
    """
    pattern = re.compile('https?://(www\\.)?vimeo\\.com/\\d+$')
...
class RedditUploadsMIMEParser(BaseMIMEParser):
    ...
    """
    pattern = re.compile('https://i\\.reddituploads\\.com/.+$')
...
class RedditVideoMIMEParser(BaseMIMEParser):
    ...
    """
    pattern = re.compile('https://v\\.redd\\.it/.+$')
...
class ImgurApiMIMEParser(BaseMIMEParser):
    ...
    """
    CLIENT_ID = None
    pattern = re.compile('https?://(w+\\.)?(m\\.)?imgur\\.com/((?P<domain>a|album|gallery)/)?(?P<hash>[a-zA-Z0-9]+)$')
...
class ImgurScrapeMIMEParser(BaseMIMEParser):
    ...
    """
    pattern = re.compile('https?://(w+\\.)?(m\\.)?imgur\\.com/[^.]+$')
...
class InstagramMIMEParser(OpenGraphMIMEParser):
    ...
    """
    pattern = re.compile('https?://(www\\.)?instagr((am\\.com)|\\.am)/p/[^.]+$')

class StreamableMIMEParser(OpenGraphMIMEParser):
    """
    Streamable uses the Open Graph protocol
    """
    pattern = re.compile('https?://(www\\.)?streamable\\.com/[^.]+$')
...
class LiveleakMIMEParser(BaseMIMEParser):
    ...
    """
    pattern = re.compile('https?://((www|m)\\.)?liveleak\\.com/view\\?i=\\w+$')
...
class ClippitUserMIMEParser(BaseMIMEParser):
    ...
    """
    pattern = re.compile('https?://(www\\.)?clippituser\\.tv/c/.+$')
...
class WorldStarHipHopMIMEParser(BaseMIMEParser):
    ...
    Sometimes only one video source is available
    """
    pattern = re.compile('https?://((www|m)\\.)?worldstarhiphop\\.com/videos/video.php\\?v=\\w+$')
    ...
    def get_mimetype(url):
        ...
        return (url, None)
parsers = [StreamjaMIMEParser, ClippitUserMIMEParser, StreamableMIMEParser, InstagramMIMEParser, GfycatMIMEParser, ImgurApiMIMEParser, RedditUploadsMIMEParser, RedditVideoMIMEParser, YoutubeMIMEParser, VimeoMIMEParser, LiveleakMIMEParser, FlickrMIMEParser, GifsMIMEParser, GiphyMIMEParser, ImgflipMIMEParser, LivememeMIMEParser, MakeamemeMIMEParser, WorldStarHipHopMIMEParser, GifvMIMEParser, BaseMIMEParser]