...
class TestCompositeBatchNorm(unittest.TestCase):
    ...
    def compare_backward(self):
        ...
        res_origin = expect_grad(tensor_data, running_mean, running_variance, weight, bias, attrs.training, attrs.momentum, attrs.epsilon, attrs.data_format, attrs.use_global_stats)
        ...
        res_prim = cal_composite(np_data, np_running_mean, np_running_variance, np_weight, np_bias)
        ...
        for idx in range(len(res_origin)):
            ...
            rtol = 1e-05
            atol = 1e-05
            if not isinstance(framework._current_expected_place(), core.CPUPlace) and attrs.data_format == 'NHWC':
                rtol = 0.0001
                atol = 0.0001
                if idx in (1, 2):
                    continue
            np.testing.assert_allclose(origin_item, prim_item, rtol=rtol, atol=atol, err_msg=f'Check diff failed of output: {vars_name[idx]} with data_format: {attrs.data_format}')
