...
class HacsDataClient:
    ...
    async def _do_request(self, filename: str, section: str | None=None) -> dict[str, dict[str, Any]] | list[str]:
        """Do request."""
        endpoint = '/'.join([v for v in [section, filename] if v is not None])
        try:
            response = await self._session.get(f'https://data-v2.hacs.xyz/{endpoint}', timeout=ClientTimeout(total=60), headers={'User-Agent': self._client_name, 'If-None-Match': self._etags.get(endpoint, '')})
