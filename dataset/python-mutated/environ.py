...
from xonsh.ansi_colors import ansi_color_escape_code_to_name, ansi_color_name_to_escape_code, ansi_reverse_style, ansi_style_by_name
from xonsh.built_ins import XSH
from xonsh.codecache import run_script_with_cache
from xonsh.dirstack import _get_cwd
from xonsh.events import events
from xonsh.lazyasd import LazyBool, lazyobject
from xonsh.platform import BASH_COMPLETIONS_DEFAULT, DEFAULT_ENCODING, ON_CYGWIN, ON_LINUX, ON_WINDOWS, PATH_DEFAULT, os_environ
from xonsh.tools import DefaultNotGiven, DefaultNotGivenType, adjust_shlvl, always_false, always_true, bool_or_int_to_str, bool_or_none_to_str, bool_to_str, csv_to_set, detype, dict_to_str, dynamic_cwd_tuple_to_str, ensure_string, env_path_to_str, executables_in, history_tuple_to_str, intensify_colors_on_win_setter, is_bool, is_bool_or_none, is_completion_mode, is_completions_display_value, is_dynamic_cwd_width, is_env_path, is_float, is_history_backend, is_history_tuple, is_int, is_logfile_opt, is_nonstring_seq_of_strings, is_path, is_regex, is_string, is_string_or_callable, is_string_set, is_tok_color_dict, is_valid_shlvl, logfile_opt_to_str, path_to_str, pathsep_to_upper_seq, print_color, print_exception, ptk2_color_depth_setter, seq_to_upper_pathsep, set_to_csv, str_to_env_path, str_to_path, swap_values, to_bool, to_bool_or_int, to_bool_or_none, to_completion_mode, to_completions_display_value, to_dynamic_cwd_tuple, to_history_tuple, to_int_or_none, to_itself, to_logfile_opt, to_repr_pretty_, to_shlvl, to_tok_color_dict
...
ENSURERS = {'bool': (is_bool, to_bool, bool_to_str), 'str': (is_string, ensure_string, ensure_string), 'path': (is_path, str_to_path, path_to_str), 'env_path': (is_env_path, str_to_env_path, env_path_to_str), 'float': (is_float, float, str), 'int': (is_int, int, str)}
...
class GeneralSetting(Xettings):
    """General"""
    AUTO_CONTINUE = Var.with_default(False, 'If ``True``, automatically resume stopped jobs when they are disowned. When stopped jobs are disowned and this option is ``False``, a warning will print information about how to continue the stopped process.')
    COMMANDS_CACHE_SAVE_INTERMEDIATE = Var.with_default(False, 'If enabled, the CommandsCache is saved between runs and can reduce the startup time.')
    ENABLE_COMMANDS_CACHE = Var(default=True, doc='command names in a directory are cached when enabled.', doc_default='TrueOn some platforms it may not be accurate enough(e.g. Windows, Linux save mtime in seconds). setting it to False would disable the caching mechanism and may slow down the shell')
    HOSTNAME = Var.with_default(default=default_value(lambda env: platform.node()), doc='Automatically set to the name of the current host.', type_str='str')
    HOSTTYPE = Var.with_default(default=default_value(lambda env: platform.machine()), doc='Automatically set to a string that fully describes the system type on which xonsh is executing.', type_str='str')
    LANG = Var.with_default(default='C.UTF-8', doc='Fallback locale setting for systems where it matters', type_str='str')
    ...
    OLDPWD = Var.with_default('.', 'Used to represent a previous present working directory.', is_configurable=False)
    PATH = Var.with_default(PATH_DEFAULT, 'List of strings representing where to look for executables.', type_str='env_path', doc_default="On Windows: it is ``Path`` value of register's ``HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment``. On Mac OSX: ``('/usr/local/bin', '/usr/bin', '/bin', '/usr/sbin', '/sbin')`` On Linux & on Cygwin & on MSYS, when detected that the distro is like arch, the default PATH is ``('/usr/local/sbin', '/usr/local/bin', '/usr/bin', '/usr/bin/site_perl', '/usr/bin/vendor_perl', '/usr/bin/core_perl')`` and otherwise is ``('~/bin', '/usr/local/sbin', '/usr/local/bin', '/usr/sbin','/usr/bin', '/sbin', '/bin', '/usr/games', '/usr/local/games')``")
    PATHEXT = Var(is_nonstring_seq_of_strings, pathsep_to_upper_seq, seq_to_upper_pathsep, ['.COM', '.EXE', '.BAT', '.CMD'] if ON_WINDOWS else [], 'Sequence of extension strings (eg, ``.EXE``) for filtering valid executables by. Each element must be uppercase.')
    RAISE_SUBPROC_ERROR = Var.with_default(False, 'Whether or not to raise an error if a subprocess (captured or uncaptured) returns a non-zero exit status, which indicates failure. This is most useful in xonsh scripts or modules where failures should cause an end to execution. This is less useful at a terminal. The error that is raised is a ``subprocess.CalledProcessError``.')
    LAST_RETURN_CODE = Var.with_default(0, 'Integer return code of the last command. Only updated during interactive use, i.e. not during execution of scripts.')
    SHLVL = Var(is_valid_shlvl, to_shlvl, str, 0, "Shell nesting level typed as integer, mirrors bash's $SHLVL.", is_configurable=False)
    XONSH_SUBPROC_CAPTURED_PRINT_STDERR = Var.with_default(False, 'If ``True`` the stderr from captured subproc will be printed automatically.')
    TERM = Var.no_default('str', 'TERM is sometimes set by the terminal emulator. This is used (when valid) to determine whether the terminal emulator can support the selected shell, or whether or not to set the title. Users shouldn\'t need to set this themselves. Note that this variable should be set as early as possible in order to ensure it is effective. Here are a few options:\n\n* Set this from the program that launches xonsh. On POSIX systems, \n  this can be performed by using env, e.g. \n  ``/usr/bin/env TERM=xterm-color xonsh`` or similar.\n* From the xonsh command line, namely ``xonsh -DTERM=xterm-color``.\n* In the config file with ``{"env": {"TERM": "xterm-color"}}``.\n* Lastly, in xonshrc with ``$TERM``\n\nIdeally, your terminal emulator will set this correctly but that does not always happen.', is_configurable=False)
    XONSH_CAPTURE_ALWAYS = Var.with_default(False, "Try to capture output of commands run without explicit capturing.\nIf True, xonsh will capture the output of commands run directly or in ``![]``to the session history.\nSetting to True has the following disadvantages:\n* Some interactive commands won't work properly (like when ``git`` invokes an interactive editor).\n  For more information see discussion at https://github.com/xonsh/xonsh/issues/3672.\n* Stopping these commands with ^Z (i.e. ``SIGTSTP``)\n  is disabled as it causes deadlocked terminals.\n  ``SIGTSTP`` may still be issued and only the physical pressing\n  of ``Ctrl+Z`` is ignored.\n\nRegardless of this value, commands run in ``$()``, ``!()`` or with an IO redirection (``>`` or ``|``) will always be captured.\nSetting this to True depends on ``$THREAD_SUBPROCS`` being True.")
    THREAD_SUBPROCS = Var(is_bool_or_none, to_bool_or_none, bool_or_none_to_str, not ON_CYGWIN, 'Note: The ``$XONSH_CAPTURE_ALWAYS`` variable introduces finer control and you should probably use that instead.\n\nWhether or not to try to run subrocess mode in a Python thread, when trying to capture its output. There are various trade-offs.\n\nIf True, xonsh is able capture & store the stdin, stdout, and stderr \n  of threadable subprocesses.\nThe disadvantages are listed in ``$XONSH_CAPTURE_ALWAYS``.\nThe desired effect is often up to the command, user, or use case.\n\nNone values are for internal use only and are used to turn off threading when loading xonshrc files. This is done because Bash was automatically placing new xonsh instances in the background at startup when threadable subprocs were used. Please see https://github.com/xonsh/xonsh/pull/3705 for more information.\n')
    UPDATE_OS_ENVIRON = Var.with_default(False, 'If True ``os_environ`` will always be updated when the xonsh environment changes. The environment can be reset to the default value by calling ``__xonsh__.env.undo_replace_env()``')
    XDG_CONFIG_HOME = Var.with_default(os.path.expanduser(os.path.join('~', '.config')), 'Open desktop standard configuration home dir. This is the same default as used in the standard.', is_configurable=False, doc_default='``~/.config``', type_str='str')
    XDG_DATA_HOME = Var.with_default(os.path.expanduser(os.path.join('~', '.local', 'share')), 'Open desktop standard data home dir. This is the same default as used in the standard.', doc_default='``~/.local/share``', type_str='str')
    XDG_CACHE_HOME = Var.with_default(os.path.expanduser(os.path.join('~', '.cache')), 'The base directory relative to which user-specific non-essential data files should be stored.', doc_default='``~/.cache``', type_str='str')
    XDG_DATA_DIRS = Var.with_default(xdg_data_dirs, 'A list of directories where system level data files are stored.', type_str='env_path')
    XONSHRC = Var.with_default(default_xonshrc, 'A list of the locations of run control files, if they exist.  User defined run control file will supersede values set in system-wide control file if there is a naming collision. $THREAD_SUBPROCS=None when reading in run control files.', type_str='env_path')
    XONSHRC_DIR = Var.with_default(default_xonshrcdir, 'A list of directories, from which all .xsh|.py files will be loaded at startup, sorted in lexographic order. Files in these directories are loaded after any files in XONSHRC.', type_str='env_path')
    XONSH_COMPLETER_DIRS = Var.with_default(default_completer_dirs, 'A list of paths where Xonsh searches for command completions.\nAny completions defined are lazy loaded when needed.\nThe name of the completer file should match that of the completing command.\nThe file should contain a function with the signature\n``xonsh_complete(ctx: CommandContext) -> Iterator[RichCompletion|str]``.\n', type_str='env_path')
    XONSH_APPEND_NEWLINE = Var.with_default(xonsh_append_newline, 'Append new line when a partial line is preserved in output.', doc_default='``$XONSH_INTERACTIVE``', type_str='bool')
    XONSH_CACHE_SCRIPTS = Var.with_default(True, 'Controls whether the code for scripts run from xonsh will be cached (``True``) or re-compiled each time (``False``).')
    XONSH_CACHE_EVERYTHING = Var.with_default(False, 'Controls whether all code (including code entered at the interactive prompt) will be cached.')
    XONSH_CONFIG_DIR = Var.with_default(xonsh_config_dir, 'This is the location where xonsh user-level configuration information is stored.', is_configurable=False, type_str='str')
    XONSH_SYS_CONFIG_DIR = Var.with_default(xonsh_sys_config_dir, 'This is the location where xonsh system-level configuration information is stored.', is_configurable=False, type_str='str')
    XONSH_COLOR_STYLE = Var.with_default('default', 'Sets the color style for xonsh colors. This is a style name, not a color map. Run ``xonfig styles`` to see the available styles.', type_str='str')
    XONSH_DATETIME_FORMAT = Var.with_default('%Y-%m-%d %H:%M', 'The format that is used for ``datetime.strptime()`` in various places, i.e the history timestamp option.', type_str='str')
    XONSH_DEBUG = Var(always_false, to_debug, bool_or_int_to_str, 0, 'Sets the xonsh debugging level. This may be an integer or a boolean. Setting it to ``1`` will get some basic information like input transformation, command replacement. With ``2`` or a higher number will make more debugging information presented, like PLY parsing messages.', is_configurable=False)
    XONSH_DATA_DIR = Var.with_default(xonsh_data_dir, 'This is the location where xonsh data files are stored, such as history, generated completers ...', doc_default='``$XDG_DATA_HOME/xonsh``', type_str='str')
    XONSH_CACHE_DIR = Var.with_default(xonsh_cache_dir, 'This is the location where cache files used by xonsh are stored, such as commands-cache...', doc_default='``$XDG_CACHE_HOME/xonsh``', type_str='str')
    XONSH_ENCODING = Var.with_default(DEFAULT_ENCODING, 'This is the encoding that xonsh should use for subprocess operations.', doc_default='``sys.getdefaultencoding()``', type_str='str')
    XONSH_ENCODING_ERRORS = Var.with_default('surrogateescape', "The flag for how to handle encoding errors should they happen. Any string flag that has been previously registered with Python is allowed. See the 'Python codecs documentation' (https://docs.python.org/3/library/codecs.html#error-handlers) for more information and available options.", doc_default='``surrogateescape``', type_str='str')
    XONSH_INTERACTIVE = Var.with_default(True, '``True`` if xonsh is running interactively, and ``False`` otherwise.', is_configurable=False)
    XONSH_LOGIN = Var.with_default(False, '``True`` if xonsh is running as a login shell, and ``False`` otherwise.', is_configurable=False)
    XONSH_PROC_FREQUENCY = Var.with_default(0.0001, 'The process frequency is the time that xonsh process threads sleep for while running command pipelines. The value has units of seconds [s].')
    XONSH_SHOW_TRACEBACK = Var.with_default(False, 'Controls if a traceback is shown if exceptions occur in the shell. Set to ``True`` to always show traceback or ``False`` to always hide. If undefined then the traceback is hidden but a notice is shown on how to enable the full traceback.')
    XONSH_SOURCE = Var.with_default('', "When running a xonsh script, this variable contains the absolute path to the currently executing script's file.", is_configurable=False)
    XONSH_STORE_STDIN = Var.with_default(False, 'Whether or not to store the stdin that is supplied to the ``!()`` and ``![]`` operators.')
    XONSH_STYLE_OVERRIDES = Var(is_tok_color_dict, to_tok_color_dict, dict_to_str, {}, "A dictionary containing custom prompt_toolkit/pygments style definitions.\nThe following style definitions are supported:\n\n    - ``pygments.token.Token`` - ``$XONSH_STYLE_OVERRIDES[Token.Keyword] = '#ff0000'``\n    - pygments token name (string) - ``$XONSH_STYLE_OVERRIDES['Token.Keyword'] = '#ff0000'``\n    - ptk style name (string) - ``$XONSH_STYLE_OVERRIDES['pygments.keyword'] = '#ff0000'``\n\n(The rules above are all have the same effect.)")
    XONSH_TRACE_SUBPROC = Var.with_default(False, 'Set to ``True`` to show arguments list of every executed subprocess command.')
    XONSH_TRACE_COMPLETIONS = Var.with_default(False, 'Set to ``True`` to show completers invoked and their return values.')
    XONSH_TRACE_SUBPROC_FUNC = Var.with_default(None, doc='A callback function used to format the trace output shown when $XONSH_TRACE_SUBPROC=True.', doc_default='By default it just prints ``cmds`` like below.\n\n.. code-block:: python\n\n    def tracer(cmds: list, captured: Union[bool, str]):\n        print(f"TRACE SUBPROC: {cmds}, captured={captured}", file=sys.stderr)\n')
    XONSH_TRACEBACK_LOGFILE = Var(is_logfile_opt, to_logfile_opt, logfile_opt_to_str, None, 'Specifies a file to store the traceback log to, regardless of whether ``XONSH_SHOW_TRACEBACK`` has been set. Its value must be a writable file or None / the empty string if traceback logging is not desired. Logging to a file is not enabled by default.')
    XONTRIBS_AUTOLOAD_DISABLED = Var(default=False, doc='Controls auto-loading behaviour of xontrib packages at the startup.\n* Set this to ``True`` to disable autoloading completely.\n* Setting this to a list of xontrib names will block loading those specifically.\n', doc_default='Xontribs with ``xonsh.xontrib`` entrypoint will be loaded automatically by default.\n')
    STAR_PATH = Var.no_default('env_path', pattern=re.compile('\\w*PATH$'))
    STAR_DIRS = Var.no_default('env_path', pattern=re.compile('\\w*DIRS$'))

class ChangeDirSetting(Xettings):
    """``cd`` Behavior"""
    AUTO_CD = Var.with_default(False, doc='Flag to enable changing to a directory by entering the dirname or full path only (without the cd command).')
    AUTO_PUSHD = Var.with_default(False, doc='Flag for automatically pushing directories onto the directory stack.')
    CDPATH = Var.with_default((), 'A list of paths to be used as roots for a cd, breaking compatibility with Bash, xonsh always prefer an existing relative path.', type_str='env_path')
    DIRSTACK_SIZE = Var.with_default(20, 'Maximum size of the directory stack.')
    PUSHD_MINUS = Var.with_default(False, 'Flag for directory pushing functionality. False is the normal behavior.')
    PUSHD_SILENT = Var.with_default(False, 'Whether or not to suppress directory stack manipulation output.')
    COMPLETE_DOTS = Var.with_default('matching', doc="Flag to specify how current and previous directories should be tab completed  ('./', '../'):\n    - ``always`` Always complete paths with ./ and ../\n    - ``never`` Never complete paths with ./ and ../\n    - ``matching`` Complete if path starts with . or ..")

class InterpreterSetting(Xettings):
    """Interpreter Behavior"""
    DOTGLOB = Var.with_default(False, 'Globbing files with "*" or "**" will also match dotfiles, or those \'hidden\' files whose names begin with a literal \'.\'. Such files are filtered out by default.')
    EXPAND_ENV_VARS = Var.with_default(True, 'Toggles whether environment variables are expanded inside of strings in subprocess mode.')
    FOREIGN_ALIASES_SUPPRESS_SKIP_MESSAGE = Var.with_default(False, 'Whether or not foreign aliases should suppress the message that informs the user when a foreign alias has been skipped because it already exists in xonsh.', is_configurable=True)
    ...
    GLOB_SORTED = Var.with_default(True, 'Toggles whether globbing results are manually sorted. If ``False``, the results are returned in arbitrary order.')

class PromptSetting(Xettings):
    """Interactive Prompt"""
    COLOR_INPUT = Var.with_default(True, 'Flag for syntax highlighting interactive input.')
    COLOR_RESULTS = Var.with_default(True, 'Flag for syntax highlighting return values.')
    DYNAMIC_CWD_WIDTH = Var(is_dynamic_cwd_width, to_dynamic_cwd_tuple, dynamic_cwd_tuple_to_str, (float('inf'), 'c'), 'Maximum length in number of characters or as a percentage for the ``cwd`` prompt variable. For example, "20" is a twenty character width and "10%" is ten percent of the number of columns available.')
    DYNAMIC_CWD_ELISION_CHAR = Var.with_default('', "The string used to show a shortened directory in a shortened cwd, e.g. ``'…'``.")
    ...
    LS_COLORS = Var(is_lscolors, LsColors.convert, detype, default_lscolors, "Color settings for ``ls`` command line utility and, with ``$SHELL_TYPE='prompt_toolkit'``, file arguments in subprocess mode.", doc_default='``*.7z=1;0;31:*.Z=1;0;31:*.aac=0;36:*.ace=1;0;31:*.alz=1;0;31:*.arc=1;0;31:*.arj=1;0;31:*.asf=1;0;35:*.au=0;36:*.avi=1;0;35:*.bmp=1;0;35:*.bz=1;0;31:*.bz2=1;0;31:*.cab=1;0;31:*.cgm=1;0;35:*.cpio=1;0;31:*.deb=1;0;31:*.dl=1;0;35:*.dwm=1;0;31:*.dz=1;0;31:*.ear=1;0;31:*.emf=1;0;35:*.esd=1;0;31:*.flac=0;36:*.flc=1;0;35:*.fli=1;0;35:*.flv=1;0;35:*.gif=1;0;35:*.gl=1;0;35:*.gz=1;0;31:*.jar=1;0;31:*.jpeg=1;0;35:*.jpg=1;0;35:*.lha=1;0;31:*.lrz=1;0;31:*.lz=1;0;31:*.lz4=1;0;31:*.lzh=1;0;31:*.lzma=1;0;31:*.lzo=1;0;31:*.m2v=1;0;35:*.m4a=0;36:*.m4v=1;0;35:*.mid=0;36:*.midi=0;36:*.mjpeg=1;0;35:*.mjpg=1;0;35:*.mka=0;36:*.mkv=1;0;35:*.mng=1;0;35:*.mov=1;0;35:*.mp3=0;36:*.mp4=1;0;35:*.mp4v=1;0;35:*.mpc=0;36:*.mpeg=1;0;35:*.mpg=1;0;35:*.nuv=1;0;35:*.oga=0;36:*.ogg=0;36:*.ogm=1;0;35:*.ogv=1;0;35:*.ogx=1;0;35:*.opus=0;36:*.pbm=1;0;35:*.pcx=1;0;35:*.pgm=1;0;35:*.png=1;0;35:*.ppm=1;0;35:*.qt=1;0;35:*.ra=0;36:*.rar=1;0;31:*.rm=1;0;35:*.rmvb=1;0;35:*.rpm=1;0;31:*.rz=1;0;31:*.sar=1;0;31:*.spx=0;36:*.svg=1;0;35:*.svgz=1;0;35:*.swm=1;0;31:*.t7z=1;0;31:*.tar=1;0;31:*.taz=1;0;31:*.tbz=1;0;31:*.tbz2=1;0;31:*.tga=1;0;35:*.tgz=1;0;31:*.tif=1;0;35:*.tiff=1;0;35:*.tlz=1;0;31:*.txz=1;0;31:*.tz=1;0;31:*.tzo=1;0;31:*.tzst=1;0;31:*.vob=1;0;35:*.war=1;0;31:*.wav=0;36:*.webm=1;0;35:*.wim=1;0;31:*.wmv=1;0;35:*.xbm=1;0;35:*.xcf=1;0;35:*.xpm=1;0;35:*.xspf=0;36:*.xwd=1;0;35:*.xz=1;0;31:*.yuv=1;0;35:*.z=1;0;31:*.zip=1;0;31:*.zoo=1;0;31:*.zst=1;0;31:bd=40;0;33:ca=0;30;41:cd=40;0;33:di=1;0;34:do=1;0;35:ex=1;0;32:ln=1;0;36:mh=0:mi=0:or=40;0;31:ow=0;34;42:pi=40;0;33:rs=0:sg=0;30;43:so=1;0;35:st=0;37;44:su=0;37;41:tw=0;30;42``')
    MULTILINE_PROMPT = Var(is_string_or_callable, ensure_string, ensure_string, '.', 'Prompt text for 2nd+ lines of input, may be str or function which returns a str.')
    MULTILINE_PROMPT_PRE = Var(is_string_or_callable, ensure_string, ensure_string, DefaultNotGiven, 'Indicator inserted before the line continuation marks set in ``$MULTILINE_PROMPT``. Can be used to mark the start of a semantic continuation prompt (see `Semantic Prompts <https://gitlab.freedesktop.org/Per_Bothner/specifications/blob/master/proposals/semantic-prompts.md>`_ or `WezTerm <https://wezfurlong.org/wezterm/shell-integration.html>`_ for more details). May be str or function which returns a str.')
    MULTILINE_PROMPT_POS = Var(is_string_or_callable, ensure_string, ensure_string, DefaultNotGiven, 'Indicator inserted after the line continuation marks set in ``$MULTILINE_PROMPT``. Can be used to mark the end of a semantic continuation prompt and the beginning of user input (see `Semantic Prompts <https://gitlab.freedesktop.org/Per_Bothner/specifications/blob/master/proposals/semantic-prompts.md>`_ or `WezTerm <https://wezfurlong.org/wezterm/shell-integration.html>`_ for more details). May be str or function which returns a str.')
    PRETTY_PRINT_RESULTS = Var.with_default(True, 'Flag for "pretty printing" return values.')
    PROMPT = Var(is_string_or_callable, ensure_string, ensure_string, prompt.default_prompt(), "The prompt text. May contain keyword arguments which are auto-formatted, see 'Customizing the Prompt' at http://xon.sh/tutorial.html#customizing-the-prompt. This value is never inherited from parent processes.", doc_default='``xonsh.environ.DEFAULT_PROMPT``')
    PROMPT_FIELDS = Var.with_default(default_prompt_fields, validate=always_true, convert=None, detype=None, doc="Dictionary containing variables to be used when formatting $PROMPT and $TITLE. See 'Customizing the Prompt' http://xon.sh/tutorial.html#customizing-the-prompt", is_configurable=False)
    PROMPT_REFRESH_INTERVAL = Var.with_default(0.0, 'Interval (in seconds) to evaluate and update ``$PROMPT``, ``$RIGHT_PROMPT`` and ``$BOTTOM_TOOLBAR``. The default is zero (no update). NOTE: ``$UPDATE_PROMPT_ON_KEYPRESS`` must be set to ``True`` for this variable to take effect.')
    PROMPT_TOKENS_FORMATTER = Var(validate=callable, convert=None, detype=None, default=prompt.prompt_tokens_formatter_default, doc='Final processor that receives all tokens in the prompt template. It gives option to format the prompt with different prefix based on other tokens values. Highly useful for implementing something like powerline theme.', doc_default='``xonsh.prompt.base.prompt_tokens_formatter_default``')
    RIGHT_PROMPT = Var(is_string_or_callable, ensure_string, ensure_string, '', 'Template string for right-aligned text at the prompt. This may be parametrized in the same way as the ``$PROMPT`` variable. Currently, this is only available in the prompt-toolkit shell.')
    BOTTOM_TOOLBAR = Var(is_string_or_callable, ensure_string, ensure_string, '', 'Template string for the bottom toolbar. This may be parametrized in the same way as the ``$PROMPT`` variable. Currently, this is only available in the prompt-toolkit shell.')
    SHELL_TYPE = Var.with_default('best', "Which shell is used. Currently two base shell types are supported:\n\n    - ``readline`` that is backed by Python's readline module\n    - ``prompt_toolkit`` that uses external library of the same name\n    - ``random`` selects a random shell from the above on startup\n    - ``best`` selects the most feature-rich shell available on the\n       user's system\n\nTo use the ``prompt_toolkit`` shell you need to have the `prompt_toolkit <https://github.com/jonathanslenders/python-prompt-toolkit>`_ library installed. To specify which shell should be used, do so in the run control file. It also accepts a class type that inherits from ``xonsh.base_shell.BaseShell``", doc_default='``best``')
    SUGGEST_COMMANDS = Var.with_default(True, 'When a user types an invalid command, xonsh will try to offer suggestions of similar valid commands if this is True.')
    SUGGEST_MAX_NUM = Var.with_default(5, 'xonsh will show at most this many suggestions in response to an invalid command. If negative, there is no limit to how many suggestions are shown.')
    SUGGEST_THRESHOLD = Var.with_default(3, 'An error threshold. If the Levenshtein distance between the entered command and a valid command is less than this value, the valid command will be offered as a suggestion.  Also used for "fuzzy" tab completion of paths.')
    SUPPRESS_BRANCH_TIMEOUT_MESSAGE = Var.with_default(False, 'Whether or not to suppress branch timeout warning messages when getting {gitstatus} PROMPT_FIELD.')
    TITLE = Var(is_string_or_callable, ensure_string, ensure_string, DEFAULT_TITLE, "The title text for the window in which xonsh is running. Formatted in the same manner as ``$PROMPT``, see 'Customizing the Prompt' http://xon.sh/tutorial.html#customizing-the-prompt.", doc_default='``xonsh.environ.DEFAULT_TITLE``')
    UPDATE_PROMPT_ON_KEYPRESS = Var.with_default(False, 'Disables caching the prompt between commands, so that it would be reevaluated on each keypress. Disabled by default because of the incurred performance penalty.')
    VC_BRANCH_TIMEOUT = Var.with_default(0.2 if ON_WINDOWS else 0.1, 'The timeout (in seconds) for version control branch computations. This is a timeout per subprocess call, so the total time to compute will be larger than this in many cases.')
    VC_GIT_INCLUDE_UNTRACKED = Var.with_default(False, "Whether or not untracked file changes should count as 'dirty' in git.")
    VC_HG_SHOW_BRANCH = Var.with_default(True, 'Whether or not to show the Mercurial branch in the prompt.')
    VIRTUAL_ENV = Var.no_default('str', 'Path to the currently active Python environment.', is_configurable=False)
    XONSH_HISTORY_MATCH_ANYWHERE = Var.with_default(False, 'When searching history from a partial string (by pressing up arrow), match command history anywhere in a given line (not just the start)', doc_default='False')
    XONSH_STDERR_PREFIX = Var.with_default('', 'A format string, using the same keys and colors as ``$PROMPT``, that is prepended whenever stderr is displayed. This may be used in conjunction with ``$XONSH_STDERR_POSTFIX`` to close out the block.For example, to have stderr appear on a red background, the prefix & postfix pair would be "{BACKGROUND_RED}" & "{RESET}".')
    XONSH_STDERR_POSTFIX = Var.with_default('', 'A format string, using the same keys and colors as ``$PROMPT``, that is appended whenever stderr is displayed. This may be used in conjunction with ``$XONSH_STDERR_PREFIX`` to start the block.For example, to have stderr appear on a red background, the prefix & postfix pair would be "{BACKGROUND_RED}" & "{RESET}".')

class PromptHistorySetting(Xettings):
    """Interactive Prompt History"""
    XONSH_HISTORY_BACKEND = Var(is_history_backend, to_itself, ensure_string, 'json', "Set which history backend to use. Options are: 'json', 'sqlite', and 'dummy'. The default is 'json'. ``XONSH_HISTORY_BACKEND`` also accepts a class type that inherits from ``xonsh.history.base.History``, or its instance.")
    XONSH_HISTORY_FILE = Var.with_default(None, 'Location of history file set by history backend (default) or set by the user.', is_configurable=False, doc_default='None', type_str='path')
    HISTCONTROL = Var(is_string_set, csv_to_set, set_to_csv, set(), 'A set of strings (comma-separated list in string form) of options that determine what commands are saved to the history list. By default all commands are saved. Options are as follows:\n\n- ``ignoredups`` will not save the command if it matches the previous command\n- ``ignoreerr`` will cause any commands that fail (i.e. return non-zero exit status) to not be added to the history list\n- ``ignorespace`` will not save the command if it begins with a space\n- ``erasedups`` will remove all previous commands that matches and updates the frequency (Note: only supported in sqlite backend)', can_store_as_str=True)
    XONSH_HISTORY_SIZE = Var(is_history_tuple, to_history_tuple, history_tuple_to_str, (8128, 'commands'), "Value and units tuple that sets the size of history after garbage collection. Canonical units are:\n\n- ``commands`` for the number of past commands executed,\n- ``files`` for the number of history files to keep,\n- ``s`` for the number of seconds in the past that are allowed, and\n- ``b`` for the number of bytes that history may consume.\n\nCommon abbreviations, such as '6 months' or '1 GB' are also allowed.", doc_default="``(8128, 'commands')`` or ``'8128 commands'``")
    XONSH_STORE_STDOUT = Var.with_default(False, 'Store the ``stdout`` and ``stderr`` streams to the history. Requires that XONSH_CAPTURE_ALWAYS is True.')
    XONSH_HISTORY_SAVE_CWD = Var.with_default(True, 'Save current working directory to the history.', doc_default='True')
    XONSH_HISTORY_IGNORE_REGEX = Var(is_regex, to_itself, ensure_string, None, "Set a filter criteria for history items using a regular expression. Any matching items will not be retained in the history. Example: ``$XONSH_HISTORY_IGNORE_REGEX = '(^echo|^.*\\#\\#\\#\\s*|.*\\#\\#\\#\\s*$)'`` - skip commands that start from ``echo`` or ``###``, or end from ``###``.")

class PTKSetting(PromptSetting):
    """Prompt Toolkit shell
    Only usable with ``$SHELL_TYPE=prompt_toolkit.``
    """
    AUTO_SUGGEST = Var.with_default(True, 'Enable automatic command suggestions based on history, like in the fish shell.\n\nPressing the right arrow key inserts the currently displayed suggestion. ')
    AUTO_SUGGEST_IN_COMPLETIONS = Var.with_default(False, 'Places the auto-suggest result as the first option in the completions. This enables you to tab complete the auto-suggestion.')
    MOUSE_SUPPORT = Var.with_default(False, 'Enable mouse support in the ``prompt_toolkit`` shell. This allows clicking for positioning the cursor or selecting a completion. In some terminals however, this disables the ability to scroll back through the history of the terminal. Only usable with ``$SHELL_TYPE=prompt_toolkit``')
    PROMPT_TOOLKIT_COLOR_DEPTH = Var(always_false, ptk2_color_depth_setter, ensure_string, '', 'The color depth used by prompt toolkit 2. Possible values are: ``DEPTH_1_BIT``, ``DEPTH_4_BIT``, ``DEPTH_8_BIT``, ``DEPTH_24_BIT`` colors. Default is an empty string which means that prompt toolkit decide.')
    PTK_STYLE_OVERRIDES = Var(is_tok_color_dict, to_tok_color_dict, dict_to_str, {}, 'A dictionary containing custom prompt_toolkit style definitions. (deprecated)')
    VI_MODE = Var.with_default(False, 'Flag to enable ``vi_mode`` in the ``prompt_toolkit`` shell.')
    XONSH_AUTOPAIR = Var.with_default(False, 'Whether Xonsh will auto-insert matching parentheses, brackets, and quotes. Only available under the prompt-toolkit shell.')
    XONSH_COPY_ON_DELETE = Var.with_default(False, 'Whether to copy words/lines to clipboard on deletion (must be set in .xonshrc file).Only available under the prompt-toolkit shell.')
    XONSH_CTRL_BKSP_DELETION = Var.with_default(False, 'Delete a word on CTRL-Backspace (like ALT-Backspace). This will only work when your terminal emulator sends ``\\x7f`` on backspace and ``\\x08`` on CTRL-Backspace (which is configurable on most terminal emulators). On windows, the keys are reversed.')

class AsyncPromptSetting(PTKSetting):
    """Asynchronous Prompt
    Load $PROMPT in background without blocking read-eval loop.
    """
    ASYNC_INVALIDATE_INTERVAL = Var.with_default(0.05, 'When ENABLE_ASYNC_PROMPT is True, it may call the redraw frequently. This is to group such calls into one that happens within that timeframe. The number is set in seconds.')
    ASYNC_PROMPT_THREAD_WORKERS = Var(is_int, to_int_or_none, str, None, "Define the number of workers used by the ASYNC_PROMPT's pool. By default it is the same as defined by Python's concurrent.futures.ThreadPoolExecutor class.")
    ENABLE_ASYNC_PROMPT = Var.with_default(False, 'When enabled the prompt is rendered using threads. $PROMPT_FIELD that take long will be updated in the background and will not affect prompt speed. ')

class AutoCompletionSetting(Xettings):
    """Tab-completion behavior."""
    ALIAS_COMPLETIONS_OPTIONS_BY_DEFAULT = Var.with_default(doc="If True, :py:class:`xonsh.completers.argparser.ArgparseCompleter` based completions\nwill show options (e.g. -h, ...) without requesting explicitly with option prefix (e.g. '-').", default=False)
    ALIAS_COMPLETIONS_OPTIONS_LONGEST = Var.with_default(doc='Whether to show all options or just the longest for\nthe :py:class:`xonsh.completers.argparser.ArgparseCompleter` based completions.\nFor example, with ``-h``, ``--help`` both denoting ``help`` action.\nIf True, then only ``--help`` is shown.\nThis is to reduce the noise in generated completions.', default=False)
    CMD_COMPLETIONS_SHOW_DESC = Var.with_default(doc='If True, command completions will show description part with path to the binary and alias in case of xonsh functions.', default=False, type_str='bool')
    BASH_COMPLETIONS = Var.with_default(doc='This is a list (or tuple) of strings that specifies where the ``bash_completion`` script may be found. The first valid path will be used. For better performance, bash-completion v2.x is recommended since it lazy-loads individual completion scripts. For both bash-completion v1.x and v2.x, paths of individual completion scripts (like ``.../completes/ssh``) do not need to be included here. The default values are platform dependent, but reasonable. To specify an alternate list, do so in the run control file.', default=BASH_COMPLETIONS_DEFAULT, doc_default="Normally this is:\n\n    ``('/usr/share/bash-completion/bash_completion', )``\n\nBut, on Mac it is:\n\n    ``('/usr/local/share/bash-completion/bash_completion', '/usr/local/etc/bash_completion', '/opt/homebrew/share/bash-completion/bash_completion'),``\n\nOther OS-specific defaults may be added in the future.", type_str='env_path')
    CASE_SENSITIVE_COMPLETIONS = Var.with_default(ON_LINUX, 'Sets whether completions should be case sensitive or case insensitive.', doc_default='True on Linux, False otherwise.')
    COMPLETIONS_BRACKETS = Var.with_default(True, 'Flag to enable/disable inclusion of square brackets and parentheses in Python attribute completions.', doc_default='True')
    COMPLETION_QUERY_LIMIT = Var.with_default(100, 'The number of completions to display before the user is asked for confirmation.')
    FUZZY_PATH_COMPLETION = Var.with_default(True, "Toggles 'fuzzy' matching of paths for tab completion, which is only used as a fallback if no other completions succeed but can be used as a way to adjust for typographical errors. If ``True``, then, e.g., ``xonhs`` will match ``xonsh``.")
    SUBSEQUENCE_PATH_COMPLETION = Var.with_default(True, 'Toggles subsequence matching of paths for tab completion. If ``True``, then, e.g., ``~/u/ro`` can match ``~/lou/carcolh``.')

class PTKCompletionSetting(AutoCompletionSetting):
    """Prompt Toolkit tab-completion"""
    COMPLETIONS_CONFIRM = Var.with_default(True, 'While tab-completions menu is displayed, press <Enter> to confirm completion instead of running command. This only affects the prompt-toolkit shell.')
    COMPLETIONS_DISPLAY = Var(is_completions_display_value, to_completions_display_value, str, 'multi', "Configure if and how Python completions are displayed by the ``prompt_toolkit`` shell.\n\nThis option does not affect Bash completions, auto-suggestions, etc.\n\nChanging it at runtime will take immediate effect, so you can quickly disable and enable completions during shell sessions.\n\n- If ``$COMPLETIONS_DISPLAY`` is ``none`` or ``false``, do not display those completions.\n- If ``$COMPLETIONS_DISPLAY`` is ``single``, display completions in a\n  single column while typing.\n- If ``$COMPLETIONS_DISPLAY`` is ``multi`` or ``true``, display completions in multiple columns while typing.\n\n- If ``$COMPLETIONS_DISPLAY`` is ``readline``, display completions\n  will emulate the behavior of readline.\n\nThese option values are not case- or type-sensitive, so e.g. writing ``$COMPLETIONS_DISPLAY = None`` and ``$COMPLETIONS_DISPLAY = 'none'`` are equivalent. Only usable with ``$SHELL_TYPE=prompt_toolkit``")
    COMPLETIONS_MENU_ROWS = Var.with_default(5, 'Number of rows to reserve for tab-completions menu if ``$COMPLETIONS_DISPLAY`` is ``single`` or ``multi``. This only affects the prompt-toolkit shell.')
    COMPLETION_MODE = Var(is_completion_mode, to_completion_mode, str, 'default', "Mode of tab completion in prompt-toolkit shell (only).\n\n'default', the default, selects the common prefix of completions on first TAB,\nthen cycles through all completions.\n'menu-complete' selects the first whole completion on the first TAB, \nthen cycles through the remaining completions, then the common prefix.")
    COMPLETION_IN_THREAD = Var.with_default(False, 'When generating the completions takes time, it’s better to do this in a background thread. When this is True, background threads is used for completion.')
    UPDATE_COMPLETIONS_ON_KEYPRESS = Var.with_default(False, 'Completions display is evaluated and presented whenever a key is pressed. This avoids the need to press TAB, except to cycle through the possibilities. This currently only affects the prompt-toolkit shell.')

class WindowsSetting(GeneralSetting):
    """Windows OS
    Windows OS specific settings
    """
    ANSICON = Var.no_default('str', 'This is used on Windows to set the title, if available.', is_configurable=False)
    FORCE_POSIX_PATHS = Var.with_default(False, 'Forces forward slashes (``/``) on Windows systems when using auto completion if set to anything truthy.', is_configurable=ON_WINDOWS)
    INTENSIFY_COLORS_ON_WIN = Var(always_false, intensify_colors_on_win_setter, bool_to_str, True, 'Enhance style colors for readability when using the default terminal (``cmd.exe``) on Windows. Blue colors, which are hard to read, are replaced with cyan. Other colors are generally replaced by their bright counter parts.', is_configurable=ON_WINDOWS)
