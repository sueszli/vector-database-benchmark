...
def train_wrapper(config, ray_params):
    if False:
        return 10
    train_ray(path='/data/classification.parquet', num_workers=None, num_boost_rounds=100, num_files=64, regression=False, use_gpu=False, ray_params=ray_params, xgboost_params=config)
if __name__ == '__main__':
    ...
    start = time.time()
    analysis = tune.run(tune.with_parameters(train_wrapper, ray_params=ray_params), config=search_space, num_samples=32, resources_per_trial=ray_params.get_tune_resources())
