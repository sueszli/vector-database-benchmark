    ...
    def execute(self):
        if False:
            for i in range(10):
                print('nop')
        findings = []
        max_security_group_rules = ec2_client.audit_config.get('max_security_group_rules', 50)
        for security_group in ec2_client.security_groups:
            ...
            if len(security_group.ingress_rules) > max_security_group_rules or len(security_group.egress_rules) > max_security_group_rules:
                report.status = 'FAIL'
