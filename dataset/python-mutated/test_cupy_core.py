...
functions = [lambda x: x, lambda x: da.expm1(x), lambda x: 2 * x, lambda x: x / 2, lambda x: x ** 2, lambda x: x + x, lambda x: x * x, lambda x: x[0], lambda x: x[:, 1], lambda x: x[:1, None, 1:3], lambda x: x.T, lambda x: da.transpose(x, (1, 2, 0)), lambda x: x.sum(), lambda x: da.empty_like(x), lambda x: da.ones_like(x), lambda x: da.zeros_like(x), lambda x: da.full_like(x, 5), pytest.param(lambda x: x.mean(), marks=pytest.mark.skipif(cupy_version < parse_version('6.4.0'), reason='Requires CuPy 6.4.0+ (with https://github.com/cupy/cupy/pull/2418)')), pytest.param(lambda x: x.moment(order=0)), lambda x: x.moment(order=2), pytest.param(lambda x: x.std(), marks=pytest.mark.skipif(cupy_version < parse_version('6.4.0'), reason='Requires CuPy 6.4.0+ (with https://github.com/cupy/cupy/pull/2418)')), pytest.param(lambda x: x.var(), marks=pytest.mark.skipif(cupy_version < parse_version('6.4.0'), reason='Requires CuPy 6.4.0+ (with https://github.com/cupy/cupy/pull/2418)')), pytest.param(lambda x: x.dot(np.arange(x.shape[-1])), marks=pytest.mark.xfail(reason='cupy.dot(numpy) fails')), pytest.param(lambda x: x.dot(np.eye(x.shape[-1])), marks=pytest.mark.xfail(reason='cupy.dot(numpy) fails')), pytest.param(lambda x: da.tensordot(x, np.ones(x.shape[:2]), axes=[(0, 1), (0, 1)]), marks=pytest.mark.xfail(reason='cupy.dot(numpy) fails')), lambda x: x.sum(axis=0), lambda x: x.max(axis=0), lambda x: x.sum(axis=(1, 2)), lambda x: x.astype(np.complex128), lambda x: x.map_blocks(lambda x: x * 2), pytest.param(lambda x: x.round(1)), lambda x: x.reshape((x.shape[0] * x.shape[1], x.shape[2])), lambda x: x.rechunk(x.shape).reshape((x.shape[1], x.shape[0], x.shape[2])), lambda x: x.reshape((x.shape[0], x.shape[1], x.shape[2] / 2, x.shape[2] / 2)), lambda x: abs(x), lambda x: x > 0.5, lambda x: x.rechunk((4, 4, 4)), lambda x: x.rechunk((2, 2, 1)), pytest.param(lambda x: da.einsum('ijk,ijk', x, x)), lambda x: np.isneginf(x), lambda x: np.isposinf(x), lambda x: np.isreal(x), lambda x: np.iscomplex(x), lambda x: np.real(x), lambda x: np.imag(x), lambda x: np.exp(x), lambda x: np.fix(x), lambda x: np.i0(x.reshape((24,))), lambda x: np.sinc(x), lambda x: np.nan_to_num(x), lambda x: np.max(x), lambda x: np.min(x), lambda x: np.prod(x), lambda x: np.any(x), lambda x: np.all(x), lambda x: np.nansum(x), lambda x: np.nanprod(x), lambda x: np.nanmin(x), lambda x: np.nanmax(x), pytest.param(lambda x: np.angle(x), marks=pytest.mark.skipif(not dask.utils.has_keyword(cupy.angle, 'deg'), reason='Requires `deg` argument in `cupy.angle()` introduced in https://github.com/cupy/cupy/pull/6905')), pytest.param(lambda x: np.angle(x, True), marks=pytest.mark.skipif(not dask.utils.has_keyword(cupy.angle, 'deg'), reason='Requires `deg` argument in `cupy.angle()` introduced in https://github.com/cupy/cupy/pull/6905'))]
