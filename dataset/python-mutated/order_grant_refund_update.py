...
from .order_grant_refund_utils import assign_order_lines, get_input_lines_data, handle_lines_with_quantity_already_refunded, shipping_costs_already_granted

class OrderGrantRefundUpdateLineError(Error):
    code = OrderGrantRefundUpdateLineErrorCode(description='The error code.', required=True)
    line_id = graphene.ID(description='The ID of the line related to the error.', required=True)

class OrderGrantRefundUpdateError(Error):
    code = OrderGrantRefundUpdateErrorCode(description='The error code.', required=True)
    add_lines = NonNullList(OrderGrantRefundUpdateLineError, description='List of lines to add which cause the error.' + ADDED_IN_315 + PREVIEW_FEATURE, required=False)
    remove_lines = NonNullList(OrderGrantRefundUpdateLineError, description='List of lines to remove which cause the error.' + ADDED_IN_315 + PREVIEW_FEATURE, required=False)
...
class OrderGrantRefundUpdateLineAddInput(BaseInputObjectType):
    ...
    quantity = graphene.Int(description='The quantity of line items to be marked to refund.', required=True)
...
class OrderGrantRefundUpdateInput(BaseInputObjectType):
    amount = Decimal(description='Amount of the granted refund. if not provided and `addLines` or `removeLines` or `grantRefundForShipping` is provided, amount will be calculated automatically.')
    reason = graphene.String(description='Reason of the granted refund.')
    add_lines = NonNullList(OrderGrantRefundUpdateLineAddInput, description='Lines to assign to granted refund.' + ADDED_IN_315 + PREVIEW_FEATURE, required=False)
    remove_lines = NonNullList(graphene.ID, description='Lines to remove from granted refund.' + ADDED_IN_315 + PREVIEW_FEATURE, required=False)
    grant_refund_for_shipping = graphene.Boolean(description='Determine if granted refund should include shipping costs.' + ADDED_IN_315 + PREVIEW_FEATURE, required=False)

    class Meta:
        doc_category = DOC_CATEGORY_ORDERS

class OrderGrantRefundUpdate(BaseMutation):
    order = graphene.Field(Order, description='Order which has assigned updated grant refund.')
    granted_refund = graphene.Field(OrderGrantedRefund, description='Created granted refund.')

    class Arguments:
        id = graphene.ID(description='ID of the granted refund.', required=True)
        input = OrderGrantRefundUpdateInput(required=True, description='Fields required to update a granted refund.')
