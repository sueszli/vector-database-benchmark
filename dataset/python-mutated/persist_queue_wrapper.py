...
from ..sql_statements import PERSIST_QUEUE_BULK_PLAYED, PERSIST_QUEUE_CREATE_INDEX, PERSIST_QUEUE_CREATE_TABLE, PERSIST_QUEUE_DELETE_SCHEDULED, PERSIST_QUEUE_DROP_TABLE, PERSIST_QUEUE_FETCH_ALL, PERSIST_QUEUE_PLAYED, PERSIST_QUEUE_UPSERT, PRAGMA_FETCH_user_version, PRAGMA_SET_journal_mode, PRAGMA_SET_read_uncommitted, PRAGMA_SET_temp_store, PRAGMA_SET_user_version
...
class QueueInterface:
    ...
    async def enqueued(self, guild_id: int, room_id: int, track: lavalink.Track):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            executor.submit(self.database.cursor().execute, PERSIST_QUEUE_UPSERT, {'guild_id': int(guild_id), 'room_id': int(room_id), 'played': False, 'time': enqueue_time, 'track': json.dumps(track), 'track_id': track_identifier})