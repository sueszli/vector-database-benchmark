...
def etscalc(y, n, x, m, error, trend, season, alpha, beta, gamma, phi, e, amse, nmse):
    ...
    for i in range(n):
        ...
        if val > 0.0:
            ...
            lik2 += math.log(val + 1e-08)
    if lik > 0.0:
        lik = n * math.log(lik)
    else:
        lik = n * math.log(lik + 1e-08)
    if error == MULT:
        lik += 2 * lik2
    return lik
...
@njit(nogil=NOGIL, cache=CACHE)
def initparam(alpha: float, beta: float, gamma: float, phi: float, trendtype: str, seasontype: str, damped: bool, lower: np.ndarray, upper: np.ndarray, m: int, bounds: str):
    if False:
        i = 10
        return i + 15
    if bounds == 'admissible':
        lower[:3] = lower[:3] * 0
        upper[:3] = upper[:3] * 0 + 0.001
...
def admissible(alpha: float, beta: float, gamma: float, phi: float, m: int):
    ...
    if np.isnan(gamma):
        ...
        P[:2] = np.array([phi * (1 - alpha - gamma), alpha + beta - alpha * phi + gamma - 1])
...
def check_param(alpha: float, beta: float, gamma: float, phi: float, lower: np.ndarray, upper: np.ndarray, bounds: str, m: int):
    if False:
        print('Hello World!')
    if bounds != 'admissible':
    ...
    if seasontype != 'N':
        if n < 4:
            ...
            if seasontype == 'A':
                ...
                if not min(y) > 0:
                    raise Exception('Multiplicative seasonality is not appropriate for zero and negative values')
                y_d = dict(seasonal=y / (coefs[0] + coefs[1] * X_fourier[:, 1]))
        else:
        ...
        if seasontype == 'A':
            ...
            init_seas = np.clip(init_seas, a_min=0.01, a_max=None)
            if init_seas.sum() > m:
                init_seas = init_seas / np.sum(init_seas + 0.01)
            y_sa = y / np.clip(y_d['seasonal'], a_min=0.01, a_max=None)
    else:
        m = 1
        init_seas = []
        y_sa = y
    ...
    if trendtype == 'N':
        ...
        if trendtype == 'A':
            ...
            if abs(l0) < 1e-08:
                l0 = 1e-07
            b0 = (l + 2 * b) / l0
            if math.isclose(b0, 0.0, abs_tol=1e-08):
                div = 1e-08
            else:
                div = b0
            l0 = l0 / div
            if abs(b0) > 10000000000.0:
                b0 = np.sign(b0) * 10000000000.0
            if l0 < 1e-08 or b0 < 1e-08:
                l0 = max(y_sa[0], 0.001)
                if math.isclose(y_sa[0], 0.0, abs_tol=1e-08):
                    div = 1e-08
                else:
                    div = y_sa[0]
                b0 = max(y_sa[1] / div, 0.001)
...
@njit(nogil=NOGIL, cache=CACHE)
def pegelsresid_C(y: np.ndarray, m: int, init_state: np.ndarray, errortype: str, trendtype: str, seasontype: str, damped: bool, alpha: float, beta: float, gamma: float, phi: float, nmse: int):
    if False:
        return 10
    n = len(y)
    p = len(init_state)
    ...
    lik = etscalc(y=y, n=n, x=x, m=m, error=switch(errortype), trend=switch(trendtype), season=switch(seasontype), alpha=alpha, beta=beta, gamma=gamma, phi=phi, e=e, amse=amse, nmse=nmse)
    x = x.reshape((n + 1, p))
    if not np.isnan(lik):
...
@njit(nogil=NOGIL, cache=CACHE)
def nelder_mead_ets(x0: np.ndarray, args: Tuple=(), lower: np.ndarray=np.empty(0), upper: np.ndarray=np.empty(0), init_step: float=0.05, zero_pert: float=0.0001, alpha: float=1.0, gamma: float=2.0, rho: float=0.5, sigma: float=0.5, max_iter: int=2000, tol_std: float=1e-10, adaptive: bool=False):
    if False:
        while True:
            i = 10
...
@njit(nogil=NOGIL, cache=CACHE)
def ets_target_fn(par, p_y, p_nstate, p_errortype, p_trendtype, p_seasontype, p_damped, p_lower, p_upper, p_opt_crit, p_nmse, p_bounds, p_m, p_optAlpha, p_optBeta, p_optGamma, p_optPhi, p_givenAlpha, p_givenBeta, p_givenGamma, p_givenPhi, alpha, beta, gamma, phi):
    if False:
        i = 10
        return i + 15
    j = 0
    ...
    amse = np.zeros(30)
    e = np.zeros(n)
    lik = etscalc(p_y, n, state, p_m, p_errortype, p_trendtype, p_seasontype, alpha, beta, gamma, phi, e, amse, p_nmse)
...
def optimize_ets_target_fn(x0, par, y, nstate, errortype, trendtype, seasontype, damped, par_noopt, lowerb, upperb, opt_crit, nmse, bounds, m, pnames, pnames2):
    if False:
        for i in range(10):
            print('nop')
    ...
    if seasontype == 'N':
        gamma = 0.0
    res = nelder_mead_ets(x0, args=(y, nstate, switch(errortype), switch(trendtype), switch(seasontype), damped, lowerb, upperb, opt_crit, nmse, bounds, m, optAlpha, optBeta, optGamma, optPhi, givenAlpha, givenBeta, givenGamma, givenPhi, alpha, beta, gamma, phi), lower=lowerb, upper=upperb, tol_std=0.0001, max_iter=1000, adaptive=True)
    return res

def etsmodel(y: np.ndarray, m: int, errortype: str, trendtype: str, seasontype: str, damped: bool, alpha: float, beta: float, gamma: float, phi: float, lower: np.ndarray, upper: np.ndarray, opt_crit: str, nmse: int, bounds: str, maxit: int=2000, control=None, seed=None, trace: bool=False):
    if False:
        return 10
    if seasontype == 'N':
        m = 1
    par_ = initparam(alpha, beta, gamma, phi, trendtype, seasontype, damped, lower, upper, m, bounds)
    ...
    np_ = len(par)
    if np_ >= len(y) - 1:
        return dict(aic=np.inf, bic=np.inf, aicc=np.inf, mse=np.inf, amse=np.inf, fit=None, par=par, states=init_state)
    fred = optimize_ets_target_fn(x0=par, par=par_, y=y, nstate=nstate, errortype=errortype, trendtype=trendtype, seasontype=seasontype, damped=damped, par_noopt=par_noopt, lowerb=lower, upperb=upper, opt_crit=opt_crit, nmse=nmse, bounds=bounds, m=m, pnames=par_.keys(), pnames2=par_noopt.keys())
    fit_par = fred.x
    init_state = fit_par[-nstate:]
    if seasontype != 'N':
        init_state = np.hstack([init_state, m * (seasontype == 'M') - init_state[1 + (trendtype != 'N'):nstate].sum()])
    j = 0
    ...
    if damped:
        if not np.isnan(fit_par[j]):
            phi = fit_par[j]
    (amse, e, states, lik) = pegelsresid_C(y, m, init_state, errortype, trendtype, seasontype, damped, alpha, beta, gamma, phi, nmse)
    np_ = np_ + 1
    ny = len(y)
    aic = lik + 2 * np_
    ...
    return dict(loglik=-0.5 * lik, aic=aic, bic=bic, aicc=aicc, mse=mse, amse=amse, fit=fred, residuals=e, components=f"{errortype}{trendtype}{seasontype}{('D' if damped else 'N')}", m=m, nstate=nstate, fitted=fits, states=states, par=fit_par, sigma2=sigma2, n_params=np_)
...
def ets_f(y, m, model='ZZZ', damped=None, alpha=None, beta=None, gamma=None, phi=None, additive_only=None, blambda=None, biasadj=None, lower=None, upper=None, opt_crit='lik', nmse=3, bounds='both', ic='aicc', restrict=True, allow_multiplicative_trend=False, use_initial_values=False, maxit=2000):
    if False:
        i = 10
        return i + 15
    if alpha is None:
    ...
    if is_constant(y):
        return etsmodel(y=y, m=m, errortype='A', trendtype='N', seasontype='N', alpha=0.9999, beta=beta, gamma=gamma, phi=phi, damped=False, lower=lower, upper=upper, opt_crit=opt_crit, nmse=nmse, bounds=bounds, maxit=maxit)
    if isinstance(model, dict):
        ...
        (amse, e, states, lik) = pegelsresid_C(y=y, m=m, init_state=init_state, errortype=errortype, trendtype=trendtype, seasontype=seasontype, damped=damped, alpha=alpha, beta=beta, gamma=gamma, phi=phi, nmse=nmse)
        fred = model['fit']
        nstate = len(init_state)
    ...
    if m == 1:
        if seasontype == 'A' or seasontype == 'M':
            ...
            seasontype = 'N'
    if restrict:
        if errortype == 'A' and (trendtype == 'M' or seasontype == 'M') or (errortype == 'M' and trendtype == 'M' and (seasontype == 'A')) or (additive_only and (errortype == 'M' or trendtype == 'M' or seasontype == 'M')):
        ...
        for ttype in trendtype:
            for stype in seasontype:
                for dtype in damped:
                    ...
                    if stype != 'N' and m == 1:
                        continue
                    fit = etsmodel(y, m, etype, ttype, stype, dtype, alpha, beta, gamma, phi, lower=lower, upper=upper, opt_crit=opt_crit, nmse=nmse, bounds=bounds, maxit=maxit)
                    fit_ic = fit[ic]
...
def _class3models(h, sigma, last_state, season_length, error, trend, seasonality, damped, alpha, beta, gamma, phi):
    if False:
        for i in range(10):
            print('nop')
    if damped == 'N':
    ...
    if trend != 'N':
        ...
        H1 = np.array([[1]])
    H2 = np.concatenate((np.zeros(season_length - 1), np.array([1]))).reshape(1, season_length)
    if trend == 'N':
        F1 = 1
        G1 = alpha
    else:
    ...
    f2_top = np.concatenate((np.zeros(season_length - 1), np.array([1]))).reshape(1, season_length)
    f2_bottom = np.c_[np.identity(season_length - 1), np.zeros(season_length - 1).reshape(season_length - 1, 1)]
    F2 = np.r_[f2_top, f2_bottom]
    G2 = np.zeros((season_length, season_length))
    G2[0, season_length - 1] = gamma
    Mh = np.matmul(last_state[0:p - season_length].reshape(last_state[0:p - season_length].shape[0], 1), last_state[p - season_length:p].reshape(1, season_length))
    ...
    for i in range(0, h):
        ...
        exp4 = np.matmul(K, np.matmul(Vh + vecMh * vecMh.reshape(vecMh.shape[0], 1), np.transpose(K)))
...
def _compute_pred_intervals(model, forecasts, h, level):
    ...
    if error == 'A' and trend == 'N' and (seasonality == 'N') and (damped == 'N'):
        ...
        sigmah = _class3models(h, sigma, last_state, season_length, error, trend, seasonality, damped, alpha, beta, gamma, phi)
    else:
        np.random.seed(1)
        ...
        for k in range(nsim):
            ...
            yhat = np.zeros(h)
            etssimulate(last_state, season_length, switch(error), switch(trend), switch(seasonality), alpha, beta, gamma, phi, h, yhat, e)
            y_path[k,] = yhat
