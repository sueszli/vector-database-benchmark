...
class FreakhareComFolder(SimpleDecrypter):
    ...
    __version__ = '0.09'
    __status__ = 'testing'
    __pattern__ = 'http://(?:www\\.)?freakshare\\.com/folder/.+'
    ...
    LINK_PATTERN = '<a href="(http://freakshare\\.com/files/.+?)" target="_blank">'
    NAME_PATTERN = 'Folder:</b> (?P<N>.+)'
    PAGES_PATTERN = 'Pages: +(\\d+)'
    ...
    def load_page(self, page_n):
        ...
        if not hasattr(self, 'f_id') and (not hasattr(self, 'f_md5')):
            m = re.search('http://freakshare.com/\\?x=folder&f_id=(\\d+)&f_md5=(\\w+)', self.data)
            if m is not None:
                self.f_id = m.group(1)
                self.f_md5 = m.group(2)
        return self.load('http://freakshare.com/', get={'x': 'folder', 'f_id': self.f_id, 'f_md5': self.f_md5, 'entrys': '20', 'page': page_n - 1, 'order': ''})