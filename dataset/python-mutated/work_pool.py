...
from prefect.workers.utilities import get_available_work_pool_types, get_default_base_job_template_for_infrastructure_type
work_pool_app = PrefectTyper(name='work-pool', help='Commands for working with work pools.')
...
async def create(name: str=typer.Argument(..., help='The name of the work pool.'), base_job_template: typer.FileText=typer.Option(None, '--base-job-template', help='The path to a JSON file containing the base job template to use. If unspecified, Prefect will use the default base job template for the given worker type.'), paused: bool=typer.Option(False, '--paused', help='Whether or not to create the work pool in a paused state.'), type: str=typer.Option(None, '-t', '--type', help='The type of work pool to create.'), set_as_default: bool=typer.Option(False, '--set-as-default', help='Whether or not to use the created work pool as the local default for deployment.')):
    """
    ...
    if type is None:
        async with get_collections_metadata_client() as collections_client:
            if not is_interactive():
                exit_with_error('When not using an interactive terminal, you must supply a `--type` value.')
            worker_metadata = await collections_client.read_worker_metadata()
            worker = prompt_select_from_table(app.console, 'What infrastructure type would you like to use for this work pool?', columns=[{'header': 'Infrastructure Type', 'key': 'display_name'}, {'header': 'Description', 'key': 'description'}], data=[worker for collection in worker_metadata.values() for worker in collection.values()], table_kwargs={'show_lines': True})
            type = worker['type']
    available_work_pool_types = await get_available_work_pool_types()
    if type not in available_work_pool_types:
        exit_with_error(f"Unknown work pool type {type!r}. Please choose from {', '.join(available_work_pool_types)}.")
    if base_job_template is None:
        template_contents = await get_default_base_job_template_for_infrastructure_type(type)
    else:
        template_contents = json.load(base_job_template)
    async with get_client() as client:
        try:
            wp = WorkPoolCreate(name=name, type=type, base_job_template=template_contents, is_paused=paused)
            ...
            if not work_pool.is_paused and (not work_pool.is_managed_pool):
                ...
                app.console.print(f'\t[blue]prefect worker start --pool {work_pool.name}[/]\n')
            if set_as_default:
                set_work_pool_as_default(work_pool.name)
            exit_with_success('')
        except ObjectAlreadyExists:
            exit_with_error(f'Work pool named {name!r} already exists. Please try creating your work pool again with a different name.')

@work_pool_app.command()
async def ls(verbose: bool=typer.Option(False, '--verbose', '-v', help='Show additional information about work pools.')):
    """
    List work pools.
    ...
    """
    table = Table(title='Work Pools', caption='(**) denotes a paused pool', caption_style='red')
    ...
    for pool in sorted(pools, key=sort_by_created_key):
        row = [f'{pool.name} [red](**)' if pool.is_paused else pool.name, str(pool.type), str(pool.id), f'[red]{pool.concurrency_limit}' if pool.concurrency_limit else '[blue]None']
        if verbose:
...
@work_pool_app.command()
async def inspect(name: str=typer.Argument(..., help='The name of the work pool to inspect.')):
    """
    Inspect a work pool.
...
@work_pool_app.command()
async def pause(name: str=typer.Argument(..., help='The name of the work pool to pause.')):
    """
    Pause a work pool.
    ...
    async with get_client() as client:
        try:
            await client.update_work_pool(work_pool_name=name, work_pool=WorkPoolUpdate(is_paused=True))
        except ObjectNotFound as exc:
            exit_with_error(exc)
        exit_with_success(f'Paused work pool {name!r}')

@work_pool_app.command()
async def resume(name: str=typer.Argument(..., help='The name of the work pool to resume.')):
    """
    Resume a work pool.
    ...
    async with get_client() as client:
        try:
            await client.update_work_pool(work_pool_name=name, work_pool=WorkPoolUpdate(is_paused=False))
        except ObjectNotFound as exc:
            exit_with_error(exc)
...
async def update(name: str=typer.Argument(..., help='The name of the work pool to update.'), base_job_template: typer.FileText=typer.Option(None, '--base-job-template', help='The path to a JSON file containing the base job template to use. If unspecified, Prefect will use the default base job template for the given worker type. If None, the base job template will not be modified.'), concurrency_limit: int=typer.Option(None, '--concurrency-limit', help='The concurrency limit for the work pool. If None, the concurrency limit will not be modified.'), description: str=typer.Option(None, '--description', help='The description for the work pool. If None, the description will not be modified.')):
    ...
    async with get_client() as client:
        try:
            await client.update_work_pool(work_pool_name=name, work_pool=wp)
        except ObjectNotFound:
            exit_with_error('Work pool named {name!r} does not exist.')
        exit_with_success(f'Updated work pool {name!r}')

@work_pool_app.command()
async def delete(name: str=typer.Argument(..., help='The name of the work pool to delete.')):
    """
    Delete a work pool.
...
@work_pool_app.command()
async def set_concurrency_limit(name: str=typer.Argument(..., help='The name of the work pool to update.'), concurrency_limit: int=typer.Argument(..., help='The new concurrency limit for the work pool.')):
    """
    ...
    async with get_client() as client:
        try:
            await client.update_work_pool(work_pool_name=name, work_pool=WorkPoolUpdate(concurrency_limit=concurrency_limit))
        except ObjectNotFound as exc:
            exit_with_error(exc)
        exit_with_success(f'Set concurrency limit for work pool {name!r} to {concurrency_limit}')

@work_pool_app.command()
async def clear_concurrency_limit(name: str=typer.Argument(..., help='The name of the work pool to update.')):
    """
    ...
    async with get_client() as client:
        try:
            await client.update_work_pool(work_pool_name=name, work_pool=WorkPoolUpdate(concurrency_limit=None))
        except ObjectNotFound as exc:
            exit_with_error(exc)
        exit_with_success(f'Cleared concurrency limit for work pool {name!r}')

@work_pool_app.command()
async def get_default_base_job_template(type: str=typer.Option(None, '-t', '--type', help='The type of work pool for which to get the default base job template.'), file: str=typer.Option(None, '-f', '--file', help='If set, write the output to a file.')):
    """
    ...
    """
    base_job_template = await get_default_base_job_template_for_infrastructure_type(type)
    if base_job_template is None:
        exit_with_error(f"Unknown work pool type {type!r}. Please choose from {', '.join(await get_available_work_pool_types())}.")
    if file is None:
...
@work_pool_app.command()
async def preview(name: str=typer.Argument(None, help='The name or ID of the work pool to preview'), hours: int=typer.Option(None, '-h', '--hours', help='The number of hours to look ahead; defaults to 1 hour')):
    """
    ...
    async with get_client() as client:
        try:
            responses = await client.get_scheduled_flow_runs_for_work_pool(work_pool_name=name)
        except ObjectNotFound as exc:
            exit_with_error(exc)
