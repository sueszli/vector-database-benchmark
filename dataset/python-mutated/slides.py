...
class Channel(models.Model):
    ...
    _order = 'sequence, id'
    _order_by_strategy = {'most_viewed': 'total_views desc', 'most_voted': 'likes desc', 'latest': 'date_published desc'}
    ...
    promote_strategy = fields.Selection([('none', 'No Featured Presentation'), ('latest', 'Latest Published'), ('most_voted', 'Most Voted'), ('most_viewed', 'Most Viewed'), ('custom', 'Featured Presentation')], string='Featuring Policy', default='most_voted', required=True)
    ...
    publish_template_id = fields.Many2one('mail.template', string='Published Template', help='Email template to send slide publication through email', default=lambda self: self.env['ir.model.data'].xmlid_to_res_id('website_slides.slide_template_published'))
    share_template_id = fields.Many2one('mail.template', string='Shared Template', help='Email template used when sharing a slide', default=lambda self: self.env['ir.model.data'].xmlid_to_res_id('website_slides.slide_template_shared'))
    visibility = fields.Selection([('public', 'Public'), ('private', 'Private'), ('partial', 'Show channel but restrict presentations')], default='public', required=True)
    group_ids = fields.Many2many('res.groups', 'rel_channel_groups', 'channel_id', 'group_id', string='Channel Groups', help='Groups allowed to see presentations in this channel')
    access_error_msg = fields.Html('Error Message', help='Message to display when not accessible due to access rights', default='<p>This channel is private and its content is restricted to some users.</p>', translate=html_translate, sanitize_attributes=False)
    upload_group_ids = fields.Many2many('res.groups', 'rel_upload_groups', 'channel_id', 'group_id', string='Upload Groups', help='Groups allowed to upload presentations in this channel. If void, every user can upload.')
...
class SlideTag(models.Model):
    ...
    _sql_constraints = [('slide_tag_unique', 'UNIQUE(name)', 'A tag must be unique!')]
...
class Slide(models.Model):
    ...
    _inherit = ['mail.thread', 'website.seo.metadata', 'website.published.mixin']
    _description = 'Slides'
    _PROMOTIONAL_FIELDS = ['__last_update', 'name', 'image_thumb', 'image_medium', 'slide_type', 'total_views', 'category_id', 'channel_id', 'description', 'tag_ids', 'write_date', 'create_date', 'website_published', 'website_url', 'website_meta_title', 'website_meta_description', 'website_meta_keywords']
    _sql_constraints = [('name_uniq', 'UNIQUE(channel_id, name)', 'The slide name must be unique within a channel')]
    ...
    download_security = fields.Selection([('none', 'No One'), ('user', 'Authentified Users Only'), ('public', 'Everyone')], string='Download Security', required=True, default='user')
    ...
    slide_type = fields.Selection([('infographic', 'Infographic'), ('presentation', 'Presentation'), ('document', 'Document'), ('video', 'Video')], string='Type', required=True, default='document', help='The document type will be set automatically based on the document URL and properties (e.g. height and width for presentation and document).')
    ...
    website_message_ids = fields.One2many('mail.message', 'res_id', domain=lambda self: [('model', '=', self._name), ('message_type', '=', 'comment')], string='Website Messages', help='Website communication history')
