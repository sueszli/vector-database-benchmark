...
class Playlist(EventEmitter, Serializable):
    ...
    async def add_entry(self, song_url, *, head, **meta):
        ...
        entry = URLPlaylistEntry(self, song_url, info.get('title', 'Untitled'), info.get('duration', None) or None, self.downloader.ytdl.prepare_filename(info), **meta)
    ...
    async def import_from(self, playlist_url, head, **meta):
        ...
        for item in info['entries']:
            if item:
                try:
                    entry = URLPlaylistEntry(self, item[url_field], item.get('title', 'Untitled'), item.get('duration', 0) or 0, self.downloader.ytdl.prepare_filename(item), **meta)
                    self._add_entry(entry, head=head)
