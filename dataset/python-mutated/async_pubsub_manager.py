...
class AsyncPubSubManager(AsyncManager):
    ...
    async def can_disconnect(self, sid, namespace):
        if self.is_connected(sid, namespace):
            ...
            await self._publish({'method': 'disconnect', 'sid': sid, 'namespace': namespace or '/', 'host_id': self.host_id})
    ...
    async def close_room(self, room, namespace=None):
        ...
        await self._handle_close_room(message)
        await self._publish(message)
    ...
    async def _handle_emit(self, message):
        ...
        if remote_callback is not None and len(remote_callback) == 3:
            ...
            callback = None
        await super().emit(message['event'], message['data'], namespace=message.get('namespace'), room=message.get('room'), skip_sid=message.get('skip_sid'), callback=callback)
    ...
    async def _return_callback(self, host_id, sid, namespace, callback_id, *args):
        if host_id == self.host_id:
            ...
            await self._publish({'method': 'callback', 'host_id': host_id, 'sid': sid, 'namespace': namespace, 'id': callback_id, 'args': args})

    async def _handle_disconnect(self, message):
        await self.server.disconnect(sid=message.get('sid'), namespace=message.get('namespace'), ignore_queue=True)
    ...
    async def _handle_close_room(self, message):
        await super().close_room(room=message.get('room'), namespace=message.get('namespace'))
                ...
                async for message in self._listen():
                    ...
                    if data and 'method' in data:
                        ...
                        try:
                            ...
                            self.server.logger.exception('Unknown error in pubsub listening task')
            except asyncio.CancelledError:
                break
            except:
                import traceback
