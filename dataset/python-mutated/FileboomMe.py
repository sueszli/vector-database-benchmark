...
class FileboomMe(SimpleDownloader):
    ...
    __version__ = '0.12'
    __status__ = 'testing'
    __pattern__ = 'https?://f(?:ile)?boom\\.me/file/(?P<ID>\\w+)'
    ...
    def api_info(self, url):
        ...
        if file_info['code'] != 200 or len(file_info['files']) == 0 or file_info['files'][0].get('is_available', False) is False:
            return {'status': 1}
        else:
            return {'name': file_info['files'][0]['name'], 'size': file_info['files'][0]['size'], 'md5': file_info['files'][0]['md5'], 'access': file_info['files'][0]['access'], 'status': 2 if file_info['files'][0]['is_available'] else 1}
    ...
    def handle_free(self, pyfile):
        ...
        try:
            api_data = self.api_request('GetUrl', file_id=file_id, free_download_key=None, captcha_challenge=None, captcha_response=None)
            ...
            if exc.code == 406:
                for i in range(10):
                    ...
                    try:
                        api_data = self.api_request('GetUrl', file_id=file_id, free_download_key=None, captcha_challenge=api_data['challenge'], captcha_response=captcha_response)
                    except BadHeader as exc:
                        if exc.code == 406:
                            api_data = json.loads(exc.content)
                            if api_data['errorCode'] == 31:
                                self.captcha.invalid()
                                continue
                            elif api_data['errorCode'] == 42:
                ...
                api_data = self.api_request('GetUrl', file_id=file_id, free_download_key=free_download_key, captcha_challenge=None, captcha_response=None)
                if api_data['code'] == 200:
