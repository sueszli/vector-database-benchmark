...
class Test_s3_account_level_public_access_blocks:
    ...
    def test_bucket_account_public_block(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info), mock.patch('prowler.providers.aws.services.s3.s3_account_level_public_access_blocks.s3_account_level_public_access_blocks.s3_client', new=S3(audit_info)), mock.patch('prowler.providers.aws.services.s3.s3_account_level_public_access_blocks.s3_account_level_public_access_blocks.s3control_client', new=S3Control(audit_info)):
            from prowler.providers.aws.services.s3.s3_account_level_public_access_blocks.s3_account_level_public_access_blocks import s3_account_level_public_access_blocks
            check = s3_account_level_public_access_blocks()
            result = check.execute()
            assert len(result) == 1
            assert result[0].status == 'PASS'
            assert result[0].status_extended == f'Block Public Access is configured for the account {AWS_ACCOUNT_NUMBER}.'
    ...
    def test_bucket_without_account_public_block(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info), mock.patch('prowler.providers.aws.services.s3.s3_account_level_public_access_blocks.s3_account_level_public_access_blocks.s3_client', new=S3(audit_info)), mock.patch('prowler.providers.aws.services.s3.s3_account_level_public_access_blocks.s3_account_level_public_access_blocks.s3control_client', new=S3Control(audit_info)):
            ...
            assert result[0].status_extended == f'Block Public Access is not configured for the account {AWS_ACCOUNT_NUMBER}.'
            assert result[0].resource_id == AWS_ACCOUNT_NUMBER
            assert result[0].resource_arn == AWS_ACCOUNT_ARN
            assert result[0].region == AWS_REGION
