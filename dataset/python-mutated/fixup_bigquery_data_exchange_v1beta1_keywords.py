...
class bigquery_data_exchangeCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_data_exchange': ('parent', 'data_exchange_id', 'data_exchange'), 'create_listing': ('parent', 'listing_id', 'listing'), 'delete_data_exchange': ('name',), 'delete_listing': ('name',), 'get_data_exchange': ('name',), 'get_iam_policy': ('resource', 'options'), 'get_listing': ('name',), 'list_data_exchanges': ('parent', 'page_size', 'page_token'), 'list_listings': ('parent', 'page_size', 'page_token'), 'list_org_data_exchanges': ('organization', 'page_size', 'page_token'), 'set_iam_policy': ('resource', 'policy', 'update_mask'), 'subscribe_listing': ('name', 'destination_dataset'), 'test_iam_permissions': ('resource', 'permissions'), 'update_data_exchange': ('update_mask', 'data_exchange'), 'update_listing': ('update_mask', 'listing')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=bigquery_data_exchangeCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
