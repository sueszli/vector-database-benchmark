...
class BaseGraphQLHTTPConsumer(ChannelsConsumer, AsyncHttpConsumer):
    ...
    async def handle(self, body: bytes) -> None:
        ...
        try:
            ...
            await self.send_response(response.status, response.content, headers=response.headers)
        except HTTPException as e:
            await self.send_response(e.status_code, e.reason.encode())

class GraphQLHTTPConsumer(BaseGraphQLHTTPConsumer, AsyncBaseHTTPView[ChannelsRequest, ChannelsResponse, TemporalResponse, Context, RootValue]):
