...
class BaseKafkaTableBuilder(object):
    ...
    async def _init_consumer(self):
        if not self.consumer:
            self.consumer = AIOKafkaConsumer(self.topic, loop=self.loop, bootstrap_servers=bootstrap_servers, auto_offset_reset='earliest')
...
class ConsistencyChecker(object):
    ...
    async def wait_no_lag(self):
        ...
        while True:
            ...
            positions = {TopicPartition(topic=source, partition=partition): offset + 1 for (partition, offset, _, _) in partition_resps}
