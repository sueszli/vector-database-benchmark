...
class TestPtBr(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile('[A-Z]{3}-\\d{1}[A-Z]{1}\\d{2}')

class TestPtPt(_SimpleAutomotiveTestMixin):
    """Test pt_PT automotive provider methods"""
    license_plate_pattern: Pattern = re.compile('\\d{2}-\\d{2}-[A-Z]{2}|\\d{2}-[A-Z]{2}-\\d{2}|[A-Z]{2}-\\d{2}-\\d{2}|[A-Z]{2}-\\d{2}-[A-Z]{2}')

class TestHeIl(_SimpleAutomotiveTestMixin):
    license_plate_pattern: Pattern = re.compile('(\\d{3}-\\d{2}-\\d{3})|(\\d{2}-\\d{3}-\\d{2})')

class TestHuHu(_SimpleAutomotiveTestMixin):
    """Test hu_HU automotive provider methods"""
    license_plate_pattern: Pattern = re.compile('[A-Z]{3}-\\d{3}')

class TestDeDe(_SimpleAutomotiveTestMixin):
    """Test de_DE automotive provider methods"""
    license_plate_pattern: Pattern = re.compile('(?P<prefix>[A-Z\\u00D6\\u00DC]{1,3})-[A-Z]{1,2}-[1-9]{1,4}', re.UNICODE)
...
class TestSvSe(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile('[A-Z]{3} \\d{2}[\\dA-Z]')
...
class TestEnPh(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile('[A-Z]{2}\\d{4,5}|[A-Z]{3}\\d{3,4}')
    motorcycle_pattern: Pattern = re.compile('[A-Z]{2}\\d{4,5}')
    automobile_pattern: Pattern = re.compile('[A-Z]{3}\\d{3,4}')
...
class TestRuRu(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile('(?:(?P<private_plate_prefix>[{0}]\\d\\d\\d[{0}][{0}])|(?P<public_transport_plate_prefix>[{0}][{0}]\\d\\d\\d)|(?P<trailer_plate_prefix>[{0}][{0}]\\d\\d\\d\\d)|(?P<police_plate_prefix>[{0}]\\d\\d\\d\\d)|(?P<military_plate_prefix>\\d\\d\\d\\d[{0}][{0}])|(?P<plate_number_special>00\\dCD\\d|00\\dD\\d\\d\\d|00\\dT\\d\\d\\d)) (?P<plate_suffix>.*)'.format(_plate_letters))
...
class TestFrFr(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile('\\d{3}-[A-Z]{3}-\\d{2}|[A-Z]{2}-\\d{3}-[A-Z]{2}')

class TestItIt(_SimpleAutomotiveTestMixin):
    """Test it_IT automotive provider methods"""
    license_plate_pattern: Pattern = re.compile('[A-Z]{2}\\d{3}[A-Z]{2}')

class TestNoNo(_SimpleAutomotiveTestMixin):
    """Test no_NO automotive provider methods"""
    license_plate_pattern: Pattern = re.compile('[A-Z]{2} \\d{5}')

class TestEsCo(_SimpleAutomotiveTestMixin):
    """Test es_CO automotive provider methods"""
    license_plate_pattern = re.compile('[A-Z]{3}\\d{3}|[A-Z]{3}\\d{2}[A-Z]|T\\d{4}|[A-Z]{2}\\d{4}|R\\d{5}|S\\d{5}')

class TestEsEs:
    """Test es_ES automotive provider methods"""
    new_format_pattern: Pattern = re.compile('\\d{4}\\s[A-Z]{3}')
    old_format_pattern: Pattern = re.compile('(?P<province_prefix>[A-Z]{1,2})\\s\\d{4}\\s[A-Z]{2}')
...
class TestThTh(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile('(\\d [ก-ฮ]{2} \\d{1,4})|([ก-ฮ]{2} \\d{1,4})|([ก-ฮ]{3} \\d{1,3})|(\\d{2}-\\d{4})')

class TestTrTr(_SimpleAutomotiveTestMixin):
    """Test tr_TR automotive provider methods"""
    license_plate_pattern: Pattern = re.compile('\\d{2} [A-Z] \\d{4}|\\d{2} [A-Z] \\d{5}|\\d{2} [A-Z]{2} \\d{3}|\\d{2} [A-Z]{2} \\d{4}|\\d{2} [A-Z]{3} \\d{2}|\\d{2} [A-Z]{3} \\d{3}')
...
class TestRoRo(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile('(?P<prefix>[A-Z]{1,2})-\\d{2,3}-[A-Z]{3}')
...
class TestElGr(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern = re.compile('^(?P<prefix>[A-Z]{2,3}) \\d{4}$')

class TestNlNl(_SimpleAutomotiveTestMixin):
    """Test nl_NL automotive provider methods"""
    license_plate_car_pattern = re.compile('\\d{2}-[BDFGHJKLNPRSTVXZ][A-Z]-[A-Z]{2}|\\d{2}-[BDFGHJKLNPRSTVXZ][A-Z]{2}-\\d|\\d-[KSTVXZ][A-Z]{2}-\\d{2}|[BDFGHJKLNPRSTVXZ][A-Z]-\\d{3}-[A-Z]|[BDFGHJKLNPRSTVXZ]-\\d{3}-[A-Z]{2}')
    license_plate_motorbike_pattern = re.compile('M[A-Z]-[A-Z]{2}-\\d{2}|\\d{2}-M[A-Z]-[A-Z]{2}')
    license_plate_pattern = re.compile(license_plate_car_pattern.pattern + '|' + license_plate_motorbike_pattern.pattern)
...
class TestViVn(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile('\\d{2}[ABCDĐEFGHKLMNPSTUVXYZ]-\\d{5}')
...
class TestDeCh(_SimpleAutomotiveTestMixin):
    ...
    license_plate_pattern: Pattern = re.compile('[A-Z]{2}-\\d{1,3}\\s?\\d{0,3}')

class TestNlBe(_SimpleAutomotiveTestMixin):
    """Test nl_BE automotive provider methods"""
    license_plate_pattern: Pattern = re.compile('(\\d{3}-[A-Z]{3})|([A-Z]{3}-\\d{3})|([1-2]-[A-Z]{3}-\\d{3})')

class TestZhCn(_SimpleAutomotiveTestMixin):
    """Test zh_CN automotive provider methods"""
    license_plate_pattern: Pattern = re.compile('^[京津冀晋蒙辽吉黑沪苏浙皖闽赣鲁豫鄂湘粤桂琼川贵云渝藏陕甘青宁新]{1}[A-Z]{1}-[A-Z0-9]{5}')

class TestZhTw(_SimpleAutomotiveTestMixin):
    """Test zh_TW automotive provider methods"""
    license_plate_pattern: Pattern = re.compile('([A-Z]{2}-\\d{4})|(\\d{4}-[A-Z]{2})|([A-Z]{3}-\\d{4})|([A-Z]{3}-\\d{3})')