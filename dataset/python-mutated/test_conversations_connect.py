...
from integration_tests.env_variable_names import SLACK_SDK_TEST_CONNECT_INVITE_SENDER_BOT_TOKEN, SLACK_SDK_TEST_CONNECT_INVITE_RECEIVER_BOT_TOKEN, SLACK_SDK_TEST_CONNECT_INVITE_RECEIVER_BOT_USER_ID
...
class TestWebClient(unittest.TestCase):
    ...
    async def test_async(self):
        ...
        try:
            ...
            self.assertIsNotNone(new_channel['channel']['id'])
            channel_id = new_channel['channel']['id']
            invite: SlackResponse = await sender.conversations_inviteShared(channel=new_channel['channel']['id'], user_ids=os.environ[SLACK_SDK_TEST_CONNECT_INVITE_RECEIVER_BOT_USER_ID])
            self.assertIsNotNone(invite['invite_id'])
            accepted: SlackResponse = await receiver.conversations_acceptSharedInvite(channel_name=unique_channel_name, invite_id=invite['invite_id'])
            self.assertIsNone(accepted['error'])
