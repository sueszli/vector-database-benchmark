...
from esphome.core import CORE, ID, Define, EnumValue, HexInt, Lambda, Library, TimePeriod, TimePeriodMicroseconds, TimePeriodMilliseconds, TimePeriodMinutes, TimePeriodNanoseconds, TimePeriodSeconds
...
SafeExpType = Union[Expression, bool, str, str, int, float, TimePeriod, type[bool], type[int], type[float], Sequence[Any]]
...
async def process_lambda(value: Lambda, parameters: list[tuple[SafeExpType, str]], capture: str='=', return_type: SafeExpType=None) -> Generator[LambdaExpression, None, None]:
    ...
    for (i, id) in enumerate(value.requires_ids):
        ...
        if full_id is not None and isinstance(full_id.type, MockObjClass) and (full_id.type.inherits_from(GlobalsComponent) or full_id.type.inherits_from(RestoringGlobalsComponent) or full_id.type.inherits_from(RestoringGlobalStringComponent)):
...
async def templatable(value: Any, args: list[tuple[SafeExpType, str]], output_type: Optional[SafeExpType], to_exp: Any=None):
