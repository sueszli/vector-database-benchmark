...
class ClassroomTopicSummaryDict(topic_domain.TopicSummaryDict):
    ...
    is_published: bool
SCHEMA_FOR_CLASSROOM_ID = {'type': 'basestring', 'validators': [{'id': 'is_regex_matched', 'regex_pattern': constants.ENTITY_ID_REGEX}]}

class ClassroomDataHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
...
class DefaultClassroomRedirectPage(base.BaseHandler[Dict[str, str], Dict[str, str]]):
...
class ClassroomAdminPage(base.BaseHandler[Dict[str, str], Dict[str, str]]):
...
class ClassroomAdminDataHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
...
class NewClassroomIdHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
...
class ClassroomHandler(base.BaseHandler[ClassroomHandlerNormalizedPayloadDict, Dict[str, str]]):
    """Edits classroom data."""
    GET_HANDLER_ERROR_RETURN_TYPE = feconf.HANDLER_TYPE_JSON
    URL_PATH_ARGS_SCHEMAS = {'classroom_id': {'schema': SCHEMA_FOR_CLASSROOM_ID}}
    HANDLER_ARGS_SCHEMAS = {'GET': {}, 'PUT': {'classroom_dict': {'schema': {'type': 'object_dict', 'object_class': classroom_config_domain.Classroom}}}, 'DELETE': {}}
...
class ClassroomIdHandler(base.BaseHandler[Dict[str, str], Dict[str, str]]):
