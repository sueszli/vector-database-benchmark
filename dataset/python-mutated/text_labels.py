...
plugin = lightbulb.Plugin('TextLabels')
plugin.add_checks(lightbulb.guild_only)
DISCORD_GRAY = 3092790
...
class LabelModal(miru.Modal):
    ...
    async def callback(self, context: miru.ModalContext) -> None:
        ...
        await context.respond(f'Sending {self.label}=`{val}` for `{self.content.value}` (edited={edited}) to the backend.', flags=hikari.MessageFlag.EPHEMERAL)
        ...
        conn: Connection = context.bot.d.db
        ...
        if guild_settings is None or guild_settings.log_channel_id is None:
            ...
            return
        embed = hikari.Embed(title='Message Label', description=f'{context.author.mention} labeled a message as `{self.label}`.', timestamp=datetime.now().astimezone(), color=65280).set_author(name=context.author.username, icon=context.author.avatar_url).add_field('Total Labeled Message', '0', inline=True).add_field('Server Ranking', '0/0', inline=True).add_field('Global Ranking', '0/0', inline=True)
...
class LabelSelect(miru.View):
    ...
    @miru.select(options=[hikari.SelectMenuOption(label='Toxic Language', value='toxic_language', description='The message contains toxic language.', is_default=False, emoji=None), hikari.SelectMenuOption(label='Illegal Activity', value='illegal_activity', description='The message encourages illegal activity.', is_default=False, emoji=None), hikari.SelectMenuOption(label='Good Quality', value='good_quality', description='The message is good quality.', is_default=False, emoji=None), hikari.SelectMenuOption(label='Bad Quality', value='bad_quality', description='The message is bad quality.', is_default=False, emoji=None), hikari.SelectMenuOption(label='Spam', value='spam', description='The message is spam.', is_default=False, emoji=None)], min_values=1, max_values=1)
...
async def label_message_text(ctx: lightbulb.MessageContext):
    ...
    label_select_view = LabelSelect(msg.content, timeout=60)
