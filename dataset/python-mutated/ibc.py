...
class IBCPolicy(BehaviourCloningPolicy):
    ...
    """
    config = dict(type='ibc', cuda=False, on_policy=False, continuous=True, model=dict(stochastic_optim=dict(type='mcmc')), learn=dict(train_epoch=30, batch_size=256, optim=dict(learning_rate=1e-05, weight_decay=0.0, beta1=0.9, beta2=0.999)), eval=dict(evaluator=dict(eval_freq=10000)))
