...
def create_tsdataset(roll=True, horizon=5, val_ratio=0):
    ...
    if val_ratio == 0:
        (train, _, test) = TSDataset.from_pandas(df=df, dt_col='timeseries', target_col=['value1', 'value2'], with_split=True)
        if roll:
            for tsdata in [train, test]:
                tsdata.roll(lookback=24, horizon=horizon)
        return (train, test)
    else:
        (train, val, test) = TSDataset.from_pandas(df=df, dt_col='timeseries', target_col=['value1', 'value2'], with_split=True, val_ratio=val_ratio)
        if roll:
...
def create_tsdataset_val(roll=True, horizon=5):
    ...
    df.reset_index(inplace=True)
    df.rename(columns={'index': 'timeseries'}, inplace=True)
    (train, val, test) = TSDataset.from_pandas(df=df, dt_col='timeseries', target_col=['value1', 'value2'], with_split=True, val_ratio=0.1)
    if roll:
        for tsdata in [train, test]:
            tsdata.roll(lookback=24, horizon=horizon)
    return (train, val, test)
...
class TestChronosModelTCNForecaster(TestCase):
    ...
    def test_tcn_forecaster_fit_eva_pred(self):
        ...
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, num_channels=[16, 16], loss='mae', lr=0.01)
    ...
    def test_tcn_forecaster_fit_loader(self):
        ...
        forecaster.quantize(calib_data=train_loader, val_data=val_loader, metric='mae', framework='pytorch_fx', relative_drop=0.99, max_trials=3)
        ...
        forecaster.quantize(calib_data=train_loader, val_data=val_loader, metric='mae', framework='onnxrt_qlinearops', relative_drop=0.99, max_trials=3)
    ...
    def test_tcn_forecaster_fit_loader_normalization_decomposation(self):
        if False:
            print('Hello World!')
        (train_loader, val_loader, test_loader) = create_data(loader=True)
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, normalization=True, decomposition_kernel_size=3, num_channels=[16, 16], loss='mae', lr=0.01)
        train_loss = forecaster.fit(train_loader, epochs=2)
        forecaster.quantize(calib_data=train_loader, val_data=val_loader, metric='mae', framework='pytorch_fx', relative_drop=0.99, max_trials=3)
        q_yhat = forecaster.predict(data=test_loader, quantize=True, acceleration=False)
        yhat = forecaster.predict(data=test_loader, acceleration=False)
    ...
    def test_tcn_forecaster_tune(self):
        ...
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, num_channels=[16, 16], loss='mae', metrics=['mae', 'mse', 'mape'], lr=space.Real(0.001, 0.01, log=True))
    ...
    def test_tcn_forecaster_tune_multi_processes(self):
        ...
        storage = 'sqlite:///example_tcn.db'
        if os.path.exists('./example_tcn.db'):
            os.remove('./example_tcn.db')
        import bigdl.nano.automl.hpo.space as space
        (train_data, val_data, _) = create_data(loader=False)
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, num_channels=[16, 16], loss='mae', metrics=['mae', 'mse', 'mape'], lr=space.Real(0.001, 0.01, log=True))
        forecaster.tune(train_data, validation_data=val_data, n_trials=2, target_metric='mse', direction='minimize', study_name=name, storage=storage, n_parallels=2)
        forecaster.fit(train_data, epochs=2)
        os.remove('./example_tcn.db')
    ...
    def test_tcn_forecaster_tune_multi_processes_normalization_decomposation(self):
        ...
        import bigdl.nano.automl.hpo.space as space
        (train_data, val_data, _) = create_data(loader=False)
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, normalization=True, decomposition_kernel_size=3, num_channels=[16, 16], loss='mae', metrics=['mae', 'mse', 'mape'], lr=space.Real(0.001, 0.01, log=True))
        forecaster.tune(train_data, validation_data=val_data, n_trials=2, target_metric='mse', direction='minimize', study_name=name, storage=storage, n_parallels=2)
        forecaster.fit(train_data, epochs=2)
        os.remove('./example_tcn.db')
    ...
    def test_tcn_forecaster_onnx_methods(self):
        if False:
            for i in range(10):
                print('nop')
        (train_data, val_data, test_data) = create_data()
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, num_channels=[16, 16], lr=0.01)
        forecaster.fit(train_data, epochs=2)
        try:
            import onnx
            import onnxruntime
    ...
    def test_tcn_forecaster_onnx_methods_normalization_decomposation(self):
        if False:
            return 10
        (train_data, val_data, test_data) = create_data()
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, normalization=True, decomposition_kernel_size=3, num_channels=[16, 16], lr=0.01)
        forecaster.fit(train_data, epochs=2)
        try:
            import onnx
            import onnxruntime
    ...
    def test_tcn_forecaster_openvino_methods(self):
        ...
        try:
            ...
            pass
        forecaster.quantize(calib_data=train_data, metric='mae', framework='openvino')
    ...
    def test_tcn_forecaster_openvino_methods_loader(self):
        ...
        try:
            ...
            pass
        forecaster.quantize(calib_data=train_loader, metric='mse', framework='openvino')
    ...
    def test_tcn_forecaster_openvino_methods_tsdataset(self):
        ...
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=2, output_feature_num=2, kernel_size=4, num_channels=[16, 16], lr=0.01)
        ...
        try:
            ...
            pass
        forecaster.quantize(calib_data=train, metric='mse', framework='openvino')
    ...
    def test_tcn_forecaster_quantization(self):
        ...
        forecaster.internal.train()
        forecaster.internal.eval()
        forecaster.quantize(train_data, val_data=val_data, metric='rmse', relative_drop=0.99, max_trials=3)
        pred_q = forecaster.predict(test_data[0], quantize=True, acceleration=False)
        eval_q = forecaster.evaluate(test_data, quantize=True, acceleration=False)
        with tempfile.TemporaryDirectory() as tmp_dir_name:
    ...
    def test_tcn_forecaster_quantization_onnx_tuning(self):
        ...
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, num_channels=[16, 16], lr=0.01)
        forecaster.fit(train_data, epochs=2)
        forecaster.quantize(train_data, val_data=val_data, metric='mse', relative_drop=0.99, max_trials=3, framework='onnxrt_qlinearops')
        pred_q = forecaster.predict_with_onnx(test_data[0], quantize=True)
        eval_q = forecaster.evaluate_with_onnx(test_data, quantize=True)
        with tempfile.TemporaryDirectory() as tmp_dir_name:
    ...
    def test_tcn_forecaster_runtime_error(self):
        if False:
            print('Hello World!')
        (train_data, val_data, test_data) = create_data()
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=3, lr=0.01)
        with pytest.raises(RuntimeError):
            with tempfile.TemporaryDirectory() as tmp_dir_name:
                ckpt_name = os.path.join(tmp_dir_name, 'ckpt')
                forecaster.save(ckpt_name)
    ...
    def test_tcn_forecaster_shape_error(self):
        if False:
            ...
            return i + 15
        (train_data, val_data, test_data) = create_data()
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=2, kernel_size=3, lr=0.01)
        with pytest.raises(RuntimeError):
            forecaster.fit(train_data, epochs=2)
    ...
    def test_tcn_forecaster_xshard_input(self):
        ...
        for distributed in [True, False]:
            forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=3, lr=0.01, distributed=distributed)
            forecaster.fit(train_data, epochs=2)
    ...
    def test_tcn_forecaster_xshard_input_of_validation_data(self):
        ...
        val_data = XShards.partition(val_data).transform_shard(transform_to_dict)
        test_data = XShards.partition(test_data).transform_shard(transform_to_dict_x)
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=3, lr=0.01, distributed=False)
    ...
    def test_tcn_forecaster_distributed(self):
        ...
        init_orca_context(cores=4, memory='2g')
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=3, lr=0.01, distributed=True)
        forecaster.fit(train_data, epochs=2)
    ...
    def test_tcn_forecaster_distributed_normalization_decomposation(self):
        ...
        (_train_loader, _, _test_loader) = create_data(loader=True)
        init_orca_context(cores=4, memory='2g')
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=3, normalization=True, decomposition_kernel_size=3, lr=0.01, distributed=True)
        forecaster.fit(train_data, epochs=2)
        distributed_pred = forecaster.predict(test_data[0], acceleration=False)
        distributed_eval = forecaster.evaluate(val_data, acceleration=False)
        model = forecaster.get_model()
    ...
    def test_tcn_customized_loss_metric(self):
        ...
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=3, loss=loss, metrics=[customized_metric], lr=0.01)
        forecaster.fit(train_loader, epochs=2)
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ckpt_name = os.path.join(tmp_dir_name, 'ckpt')
            test_pred_save = forecaster.predict(test_data[0], acceleration=False)
    ...
    def test_forecaster_from_tsdataset(self):
        ...
        test.roll(lookback=tcn.data_config['past_seq_len'], horizon=tcn.data_config['future_seq_len'])
        (_, y_test) = test.to_numpy()
        assert yhat.shape == y_test.shape
        del tcn
        (train, test) = create_tsdataset(roll=False, horizon=[1, 3, 5])
        tcn = TCNForecaster.from_tsdataset(train, past_seq_len=24, future_seq_len=5, num_channels=[16] * 3)
        tcn.fit(train, epochs=2, batch_size=32)
    ...
    def test_forecaster_from_tsdataset_data_loader_onnx(self):
        ...
        tcn.quantize(calib_data=loader, metric='mse', framework='pytorch_fx', relative_drop=0.99, max_trials=3)
        q_yhat = tcn.predict(test, quantize=True, acceleration=False)
        q_res = tcn.evaluate(test_loader, quantize=True, acceleration=False)
        tcn.quantize(calib_data=loader, metric='mse', framework='onnxrt_qlinearops', relative_drop=0.99, max_trials=3)
    ...
    def test_tcn_forecaster_fit_earlystop_patience(self):
        ...
        (train_data, val_data, test_data) = create_data()
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, num_channels=[16, 16], loss='mae', lr=0.01)
        train_loss = forecaster.fit(train_data, val_data, validation_mode='earlystop', earlystop_patience=6, epochs=10)
    ...
    def test_tcn_forecaster_tune_save_load(self):
        ...
        import bigdl.nano.automl.hpo.space as space
        (train_data, val_data, test_data) = create_data()
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, num_channels=space.Categorical([16, 16], [4, 4]), lr=0.01)
        forecaster.tune(train_data, validation_data=val_data, n_trials=2, target_metric='mse', direction='minimize')
        forecaster.fit(train_data, epochs=2)
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ckpt_name = os.path.join(tmp_dir_name, 'ckpt')
    ...
    def test_predict_interval_with_numpy(self):
        if False:
            ...
            return i + 15
        (train_data, val_data, test_data) = create_data()
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, num_channels=[16, 16, 16], loss='mse', metrics=['mse'], lr=0.01)
        forecaster.fit(train_data, epochs=2)
    ...
    def test_predict_interval_with_tsdataset(self):
        ...
        forecaster = TCNForecaster.from_tsdataset(train, num_channels=[16] * 3)
    ...
    def test_predict_interval_without_validation_data(self):
        ...
        forecaster.fit(train_data, epochs=2)
        with pytest.raises(RuntimeError):
            (y_pred, std) = forecaster.predict_interval(data=test_data[0], repetition_times=5)
    ...
    def test_forecaster_fit_val_from_tsdataset(self):
        ...
        assert yhat.shape == y_test.shape
        del tcn
        (train, val, test) = create_tsdataset(roll=False, horizon=[1, 3, 5], val_ratio=0.1)
        tcn = TCNForecaster.from_tsdataset(train, past_seq_len=24, future_seq_len=5, num_channels=[16] * 3)
        tcn.fit(train, val, epochs=2, batch_size=32)
        yhat = tcn.predict(test, batch_size=None, acceleration=False)
        test.roll(lookback=tcn.data_config['past_seq_len'], horizon=tcn.data_config['future_seq_len'])
    ...
    def test_forecaster_optimize_numpy(self):
        ...
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, kernel_size=4, num_channels=[16, 16, 16], loss='mse', metrics=['mse'], lr=0.01)
        forecaster.fit(train_data, epochs=2)
        forecaster.optimize(train_data=train_data, validation_data=val_data, batch_size=32)
    ...
    def test_tcn_forecaster_eval_shuffle_loader(self):
        ...
        (train_data, val_data, test_data) = create_data()
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, loss='mse', lr=0.01)
        forecaster.fit(train_data, epochs=2)
    ...
    def test_tcn_forecaster_dummy_encoder(self):
        ...
        from numpy.testing import assert_almost_equal
        (train_data, val_data, test_data) = create_data()
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=1, output_feature_num=1, dummy_encoder=True, loss='mse', lr=0.01)
        forecaster.fit(train_data, epochs=2)
        test_loader_shuffle_f = DataLoader(TensorDataset(torch.from_numpy(test_data[0]), torch.from_numpy(test_data[1])), batch_size=32, shuffle=False)
        test_loader_shuffle_t = DataLoader(TensorDataset(torch.from_numpy(test_data[0]), torch.from_numpy(test_data[1])), batch_size=32, shuffle=True)
        eval_f = forecaster.evaluate(test_loader_shuffle_f)
    ...
    def test_tcn_forecaster_export_forecasting_pipeline(self):
        ...
        test_data = TSDataset.from_pandas(df=test_df, dt_col='timeseries', target_col=['value1', 'value2'], deploy_mode=True)
    ...
    def test_tcn_forecaster_numpy_inference_tsdataset(self):
        ...
        forecaster = TCNForecaster(past_seq_len=24, future_seq_len=5, input_feature_num=2, output_feature_num=2, kernel_size=4, num_channels=[16, 16], loss='mae', lr=0.01)
