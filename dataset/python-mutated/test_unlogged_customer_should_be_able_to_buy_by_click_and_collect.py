...
def prepare_shop_click_and_collect(e2e_staff_api_client):
    if False:
        while True:
            i = 10
    warehouse_data = create_warehouse(e2e_staff_api_client)
    warehouse_id = warehouse_data['id']
    update_warehouse(e2e_staff_api_client, warehouse_data['id'], is_private=False, click_and_collect_option='LOCAL')
...
@pytest.mark.e2e
def test_unlogged_customer_buy_by_click_and_collect_CORE_0105(e2e_not_logged_api_client, e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes):
    if False:
        return 10
    permissions = [permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes]
    assign_permissions(e2e_staff_api_client, permissions)
    (channel_id, channel_slug, warehouse_id) = prepare_shop_click_and_collect(e2e_staff_api_client)
    variant_price = 10
    ...
    checkout_data = checkout_delivery_method_update(e2e_not_logged_api_client, checkout_id, collection_point['id'])
