...
class RequestValidationOperation:
    ...
    def __init__(self, next_app: ASGIApp, *, operation: AbstractOperation, strict_validation: bool=False, validator_map: t.Optional[dict]=None) -> None:
        if False:
            for i in range(10):
                print('nop')
    ...
    def validate_mime_type(self, mime_type: str) -> None:
        ...
        if not self._operation.consumes:
            return
        media_type_dict = MediaTypeDict([(c.lower(), None) for c in self._operation.consumes])
        if mime_type.lower() not in media_type_dict:
            raise UnsupportedMediaTypeProblem(detail=f'Invalid Content-type ({mime_type}), expected {self._operation.consumes}')
        ...
        if not hasattr(self, '_security_query_params'):
            ...
            for security_req in self._operation.security:
                for scheme_name in security_req:
                    ...
                    if security_scheme['type'] == 'apiKey' and security_scheme['in'] == 'query':
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send):
        ...
        uri_parser = uri_parser_class(self._operation.parameters, self._operation.body_definition())
        parameter_validator_cls = self._validator_map['parameter']
        parameter_validator = parameter_validator_cls(self._operation.parameters, uri_parser=uri_parser, strict_validation=self.strict_validation, security_query_params=self.security_query_params)
        ...
        if schema:
            try:
                body_validator = self._validator_map['body'][mime_type]
            except KeyError:
                logging.info(f'Skipping validation. No validator registered for content type: {mime_type}.')
            else:
                validator = body_validator(schema=schema, required=self._operation.request_body.get('required', False), nullable=utils.is_nullable(self._operation.body_definition(mime_type)), encoding=encoding, strict_validation=self.strict_validation, uri_parser=self._operation.uri_parser_class(self._operation.parameters, self._operation.body_definition()))
...
class RequestValidationAPI(RoutedAPI[RequestValidationOperation]):
    ...
    def __init__(self, *args, strict_validation=False, validator_map=None, uri_parser_class=None, **kwargs):
        if False:
            print('Hello World!')
    ...
    def make_operation(self, operation: AbstractOperation) -> RequestValidationOperation:
        if False:
            return 10
        return RequestValidationOperation(self.next_app, operation=operation, strict_validation=self.strict_validation, validator_map=self.validator_map)
