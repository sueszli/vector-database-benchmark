...
overcooked_ppo_config = dict(exp_name='overcooked_ppo_seed0', env=dict(collector_env_num=8, evaluator_env_num=10, n_evaluator_episode=10, concat_obs=False, stop_value=80), policy=dict(cuda=True, multi_agent=True, action_space='discrete', model=dict(obs_shape=(26, 5, 4), action_shape=6, action_space='discrete'), learn=dict(epoch_per_collect=4, batch_size=128, learning_rate=0.0005, entropy_weight=0.01, value_norm=True), collect=dict(n_sample=1024, discount_factor=0.99, gae_lambda=0.95)))
overcooked_ppo_config = EasyDict(overcooked_ppo_config)
main_config = overcooked_ppo_config
cartpole_ppo_create_config = dict(env=dict(type='overcooked_game', import_names=['dizoo.overcooked.envs.overcooked_env']), env_manager=dict(type='subprocess'), policy=dict(type='ppo'))
