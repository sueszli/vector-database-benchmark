from sympy.holonomic import DifferentialOperator, HolonomicFunction, DifferentialOperators, from_hyper, from_meijerg, expr_to_holonomic
...
def test_series():
    ...
    r = (p * t + q).series(n=10)
    s = 1 + x - x ** 2 + 4 * x ** 3 / 3 - 17 * x ** 4 / 24 + 31 * x ** 5 / 30 - 481 * x ** 6 / 720 + 71 * x ** 7 / 105 - 20159 * x ** 8 / 40320 + 379 * x ** 9 / 840 + O(x ** 10)
    assert r == s
    p = HolonomicFunction(6 + 6 * x - 3 * x ** 2 - (10 * x - 3 * x ** 2 - 3 * x ** 3) * Dx + (4 - 6 * x ** 3 + 2 * x ** 4) * Dx ** 2, x, 0, [0, 1]).series(n=7)
    q = x + x ** 3 / 6 - 3 * x ** 4 / 16 + x ** 5 / 20 - 23 * x ** 6 / 960 + O(x ** 7)
    assert p == q
    p = HolonomicFunction(6 + 6 * x - 3 * x ** 2 - (10 * x - 3 * x ** 2 - 3 * x ** 3) * Dx + (4 - 6 * x ** 3 + 2 * x ** 4) * Dx ** 2, x, 0, [1, 0]).series(n=7)
    ...
    assert expr_to_holonomic(cos(x - 1) ** 2 / (x - 1) ** 2, x0=1, y0={-2: [1, 0, -1]}).series(n=10) == (cos(x - 1) ** 2 / (x - 1) ** 2).series(x0=1, n=10)
