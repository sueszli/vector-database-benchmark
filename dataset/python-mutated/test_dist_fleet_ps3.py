...
class TestPSPassWithBow(unittest.TestCase):
    ...
    def net(self):
        ...
        nt_emb = paddle.static.nn.embedding(input=nt, is_distributed=is_distributed, size=[dict_dim, emb_dim], param_attr=base.ParamAttr(initializer=paddle.nn.initializer.Constant(value=0.01), name='__emb__', learning_rate=emb_lr), is_sparse=is_sparse)
    ...
    def test(self):
        ...
        role = fleet.UserDefinedRoleMaker(current_id=0, role=role_maker.Role.WORKER, worker_num=2, server_endpoints=endpoints)
        fleet.init(role)
        (loss, acc, _) = self.net()
