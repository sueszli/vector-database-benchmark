...
class KnowledgeDatabase:
    ...
    def add_operation(self, operation: StatementOperation, signature: bytes, is_local_peer: bool=False, is_auto_generated: bool=False, counter_increment: int=1) -> bool:
        ...
        if not op:
            self.instance.StatementOp(statement=statement, peer=peer, operation=operation.operation, clock=operation.clock, signature=signature, auto_generated=is_auto_generated)
    ...
    def _get_statements(self, source_type: Optional[ResourceType], source_name: Optional[str], statements_getter: Callable[[Entity], Entity], target_condition: Callable[[], bool], condition: Callable[[], bool], case_sensitive: bool) -> Iterator[str]:
        if False:
            for i in range(10):
                print('nop')
    ...
    def get_subjects_intersection(self, subjects_type: Optional[ResourceType], objects: Set[str], predicate: Optional[ResourceType], case_sensitive: bool=True) -> Set[str]:
        if False:
            for i in range(10):
                print('nop')
        if not objects:
