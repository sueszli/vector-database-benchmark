...
class Test_iam_role_administratoraccess_policy:
    ...
    def test_no_roles(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_role_administratoraccess_policy.iam_role_administratoraccess_policy.iam_client', new=IAM(current_audit_info)):
            from prowler.providers.aws.services.iam.iam_role_administratoraccess_policy.iam_role_administratoraccess_policy import iam_role_administratoraccess_policy
    ...
    def test_role_without_administratoraccess_policy(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_role_administratoraccess_policy.iam_role_administratoraccess_policy.iam_client', new=IAM(current_audit_info)):
            ...
            assert result[0].status_extended == 'IAM Role test does not have AdministratorAccess policy.'
    ...
    def test_role_with_administratoraccess_policy(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_role_administratoraccess_policy.iam_role_administratoraccess_policy.iam_client', new=IAM(current_audit_info)):
            ...
            assert result[0].status_extended == 'IAM Role test has AdministratorAccess policy attached.'
            assert result[0].resource_id == 'test'
            assert result[0].resource_arn == response['Role']['Arn']
            assert result[0].resource_tags == []
    ...
    def test_only_aws_service_linked_roles(self):
        ...
        current_audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_role_administratoraccess_policy.iam_role_administratoraccess_policy.iam_client', new=iam_client):
            from prowler.providers.aws.services.iam.iam_role_administratoraccess_policy.iam_role_administratoraccess_policy import iam_role_administratoraccess_policy
            check = iam_role_administratoraccess_policy()
            result = check.execute()
            assert len(result) == 0