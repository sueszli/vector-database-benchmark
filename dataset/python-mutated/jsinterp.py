...
import operator
import re
from .utils import NO_DEFAULT, ExtractorError, function_with_repr, js_to_json, remove_quotes, truncate_string, unified_timestamp, write_string
...
def _js_ternary(cndn, if_true=True, if_false=False):
    ...
    with contextlib.suppress(TypeError):
        if math.isnan(cndn):
            return if_false
    return if_true
_OPERATORS = {'?': None, '??': None, '||': None, '&&': None, '|': _js_bit_op(operator.or_), '^': _js_bit_op(operator.xor), '&': _js_bit_op(operator.and_), '===': operator.is_, '!==': operator.is_not, '==': _js_eq_op(operator.eq), '!=': _js_eq_op(operator.ne), '<=': _js_comp_op(operator.le), '>=': _js_comp_op(operator.ge), '<': _js_comp_op(operator.lt), '>': _js_comp_op(operator.gt), '>>': _js_bit_op(operator.rshift), '<<': _js_bit_op(operator.lshift), '+': _js_arith_op(operator.add), '-': _js_arith_op(operator.sub), '*': _js_arith_op(operator.mul), '%': _js_mod, '/': _js_div, '**': _js_exp}
_COMP_OPERATORS = {'===', '!==', '==', '!=', '<=', '>=', '<', '>'}
_NAME_RE = '[a-zA-Z_$][\\w$]*'
