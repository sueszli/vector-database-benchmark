...
class UnitKivyApp(object):
    ...
    async def do_touch_down_up(self, pos=None, widget=None, duration=0.2, pos_jitter=None, widget_jitter=False, jitter_dt=1 / 15.0, end_on_pos=False):
        if widget is None:
            (x, y) = pos
        elif pos is None:
        ...
        while time.perf_counter() - ts < duration:
            ...
            touch.touch_move(x + (random.random() * 2 - 1) * dx, y + (random.random() * 2 - 1) * dy)
            await self.wait_clock_frames(1)
    ...
    async def do_touch_drag(self, pos=None, widget=None, widget_loc=('center_x', 'center_y'), dx=0, dy=0, target_pos=None, target_widget=None, target_widget_offset=(0, 0), target_widget_loc=('center_x', 'center_y'), long_press=0, duration=0.2, drag_n=5):
        ...
        if widget is None:
            ...
            (x, y) = widget.to_window(*pos, initial=False)
            (tx, ty) = widget.to_window(pos[0] + dx, pos[1] + dy, initial=False)
        if target_pos is not None:
            if target_widget is None:
                (tx, ty) = target_pos
            else:
                (tx, ty) = target_pos = target_widget.to_window(*target_pos, initial=False)
        ...
        for i in range(drag_n):
            await self.async_sleep(max(0.0, duration - (time.perf_counter() - ts0)) / (drag_n - i))
    ...
    async def do_touch_drag_follow(self, pos=None, widget=None, widget_loc=('center_x', 'center_y'), target_pos=None, target_widget=None, target_widget_offset=(0, 0), target_widget_loc=('center_x', 'center_y'), long_press=0, duration=0.2, drag_n=5, max_n=25):
        ...
        while not (math.isclose(touch.x, tx) and math.isclose(touch.y, ty)):
            if i >= max_n:
                raise Exception('Exceeded the maximum number of iterations, but {} != {}'.format(touch.pos, (tx, ty)))
    ...
    async def do_touch_drag_path(self, path, axis_widget=None, long_press=0, duration=0.2):
        ...
        for (i, (x2, y2)) in enumerate(path):
            await self.async_sleep(max(0.0, duration - (time.perf_counter() - ts0)) / (n - i))
    ...
    async def do_keyboard_key(self, key, modifiers=(), duration=0.05, num_press=1):
        from kivy.core.window import Window
        if key == ' ':
            key = 'spacebar'
        key_lower = key.lower()
        ...
        if set(modifiers) - known_modifiers:
            raise ValueError('Unknown modifiers "{}"'.format(set(modifiers) - known_modifiers))
        special_keys = {27: 'escape', 9: 'tab', 8: 'backspace', 13: 'enter', 127: 'del', 271: 'enter', 273: 'up', 274: 'down', 275: 'right', 276: 'left', 278: 'home', 279: 'end', 280: 'pgup', 281: 'pgdown', 300: 'numlock', 301: 'capslock', 145: 'screenlock'}
        text = None
        try:
            text = chr(key_code)
            if key_lower != key:
        ...
        for i in range(num_press):
            ...
            if key not in known_modifiers and key_code not in special_keys and (not known_modifiers & set(modifiers)):
