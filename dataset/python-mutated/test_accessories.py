...
import pytest
from homeassistant.components.homekit.accessories import HomeAccessory, HomeBridge, HomeDriver
from homeassistant.components.homekit.const import ATTR_DISPLAY_NAME, ATTR_INTEGRATION, ATTR_VALUE, BRIDGE_MODEL, BRIDGE_NAME, BRIDGE_SERIAL_NUMBER, CHAR_FIRMWARE_REVISION, CHAR_HARDWARE_REVISION, CHAR_MANUFACTURER, CHAR_MODEL, CHAR_NAME, CHAR_SERIAL_NUMBER, CONF_LINKED_BATTERY_CHARGING_SENSOR, CONF_LINKED_BATTERY_SENSOR, CONF_LOW_BATTERY_THRESHOLD, MANUFACTURER, SERV_ACCESSORY_INFO
from homeassistant.components.homekit.util import format_version
from homeassistant.const import ATTR_BATTERY_CHARGING, ATTR_BATTERY_LEVEL, ATTR_ENTITY_ID, ATTR_HW_VERSION, ATTR_MANUFACTURER, ATTR_MODEL, ATTR_SERVICE, ATTR_SW_VERSION, STATE_OFF, STATE_ON, STATE_UNAVAILABLE, __version__ as hass_version
from homeassistant.core import HomeAssistant
from homeassistant.helpers.event import TRACK_STATE_CHANGE_CALLBACKS
from tests.common import async_mock_service

async def test_accessory_cancels_track_state_change_on_stop(hass: HomeAssistant, hk_driver) -> None:
    """Ensure homekit state changed listeners are unsubscribed on reload."""
    entity_id = 'sensor.accessory'
    hass.states.async_set(entity_id, None)
    acc = HomeAccessory(hass, hk_driver, 'Home Accessory', entity_id, 2, {'platform': 'isy994'})
    with patch('homeassistant.components.homekit.accessories.HomeAccessory.async_update_state'):
        await acc.run()
...
async def test_home_accessory(hass: HomeAssistant, hk_driver) -> None:
    ...
    assert serv.get_characteristic(CHAR_SERIAL_NUMBER).value == 'light.accessory_that_exceeds_the_maximum_maximum_maximum_maximum'
    acc3 = HomeAccessory(hass, hk_driver, 'Home Accessory that exceeds the maximum maximum maximum maximum maximum maximum length', entity_id2, 4, {ATTR_MODEL: 'Awesome Model that exceeds the maximum maximum maximum maximum maximum maximum length', ATTR_MANUFACTURER: 'Lux Brands that exceeds the maximum maximum maximum maximum maximum maximum length', ATTR_SW_VERSION: '0.4.3 that exceeds the maximum maximum maximum maximum maximum maximum length', ATTR_INTEGRATION: 'luxe that exceeds the maximum maximum maximum maximum maximum maximum length'})
    assert acc3.available is False
    serv = acc3.services[0]
    assert serv.get_characteristic(CHAR_NAME).value == 'Home Accessory that exceeds the maximum maximum maximum maximum '
    assert serv.get_characteristic(CHAR_MANUFACTURER).value == 'Lux Brands that exceeds the maximum maximum maximum maximum maxi'
    assert serv.get_characteristic(CHAR_MODEL).value == 'Awesome Model that exceeds the maximum maximum maximum maximum m'
    assert serv.get_characteristic(CHAR_SERIAL_NUMBER).value == 'light.accessory_that_exceeds_the_maximum_maximum_maximum_maximum'
    assert serv.get_characteristic(CHAR_FIRMWARE_REVISION).value == '0.4.3'
    acc4 = HomeAccessory(hass, hk_driver, 'Home Accessory that exceeds the maximum maximum maximum maximum maximum maximum length', entity_id2, 5, {ATTR_MODEL: 'Awesome Model that exceeds the maximum maximum maximum maximum maximum maximum length', ATTR_MANUFACTURER: 'Lux Brands that exceeds the maximum maximum maximum maximum maximum maximum length', ATTR_SW_VERSION: 'will_not_match_regex', ATTR_INTEGRATION: 'luxe that exceeds the maximum maximum maximum maximum maximum maximum length'})
    assert acc4.available is False
    serv = acc4.services[0]
    assert serv.get_characteristic(CHAR_NAME).value == 'Home Accessory that exceeds the maximum maximum maximum maximum '
    assert serv.get_characteristic(CHAR_MANUFACTURER).value == 'Lux Brands that exceeds the maximum maximum maximum maximum maxi'
    assert serv.get_characteristic(CHAR_MODEL).value == 'Awesome Model that exceeds the maximum maximum maximum maximum m'
    assert serv.get_characteristic(CHAR_SERIAL_NUMBER).value == 'light.accessory_that_exceeds_the_maximum_maximum_maximum_maximum'
    ...
    await hass.async_block_till_done()
    with patch('homeassistant.components.homekit.accessories.HomeAccessory.async_update_state') as mock_async_update_state:
        await acc.run()
        await hass.async_block_till_done()
...
async def test_accessory_with_missing_basic_service_info(hass: HomeAssistant, hk_driver) -> None:
    """Test HomeAccessory class."""
    entity_id = 'sensor.accessory'
    hass.states.async_set(entity_id, 'on')
    acc = HomeAccessory(hass, hk_driver, 'Home Accessory', entity_id, 3, {ATTR_MODEL: None, ATTR_MANUFACTURER: None, ATTR_SW_VERSION: None, ATTR_INTEGRATION: None})
...
async def test_accessory_with_hardware_revision(hass: HomeAssistant, hk_driver) -> None:
    ...
    entity_id = 'sensor.accessory'
    hass.states.async_set(entity_id, 'on')
    acc = HomeAccessory(hass, hk_driver, 'Home Accessory', entity_id, 3, {ATTR_MODEL: None, ATTR_MANUFACTURER: None, ATTR_SW_VERSION: None, ATTR_HW_VERSION: '1.2.3', ATTR_INTEGRATION: None})
...
async def test_battery_service(hass: HomeAssistant, hk_driver, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with patch('homeassistant.components.homekit.accessories.HomeAccessory.async_update_state') as mock_async_update_state:
        hass.states.async_set(entity_id, None, {ATTR_BATTERY_LEVEL: 10, ATTR_BATTERY_CHARGING: True})
        await hass.async_block_till_done()
        state = hass.states.get(entity_id)
        mock_async_update_state.assert_called_with(state)
    with patch('homeassistant.components.homekit.accessories.HomeAccessory.async_update_state'):
    ...
    with patch('homeassistant.components.homekit.accessories.HomeAccessory.async_update_state'):
        hass.states.async_set(entity_id, None, {ATTR_BATTERY_LEVEL: 100, ATTR_BATTERY_CHARGING: False})
        await hass.async_block_till_done()
    assert acc._char_battery.value == 100
    assert acc._char_low_battery.value == 0
    assert acc._char_charging.value == 0

async def test_linked_battery_sensor(hass: HomeAssistant, hk_driver, caplog: pytest.LogCaptureFixture) -> None:
    ...
    await hass.async_block_till_done()
    acc = HomeAccessory(hass, hk_driver, 'Battery Service', entity_id, 2, {CONF_LINKED_BATTERY_SENSOR: linked_battery})
    ...
    await hass.async_block_till_done()
    acc = HomeAccessory(hass, hk_driver, 'Battery Service', entity_id, 3, {CONF_LINKED_BATTERY_SENSOR: linked_battery, CONF_LOW_BATTERY_THRESHOLD: 50})
    with patch('homeassistant.components.homekit.accessories.HomeAccessory.async_update_state') as mock_async_update_state:
        await acc.run()
        await hass.async_block_till_done()
        state = hass.states.get(entity_id)
...
async def test_linked_battery_charging_sensor(hass: HomeAssistant, hk_driver, caplog: pytest.LogCaptureFixture) -> None:
    ...
    await hass.async_block_till_done()
    acc = HomeAccessory(hass, hk_driver, 'Battery Service', entity_id, 2, {CONF_LINKED_BATTERY_CHARGING_SENSOR: linked_battery_charging_sensor})
...
async def test_linked_battery_sensor_and_linked_battery_charging_sensor(hass: HomeAssistant, hk_driver, caplog: pytest.LogCaptureFixture) -> None:
    ...
    hass.states.async_set(linked_battery_charging_sensor, STATE_ON, None)
    await hass.async_block_till_done()
    acc = HomeAccessory(hass, hk_driver, 'Battery Service', entity_id, 2, {CONF_LINKED_BATTERY_SENSOR: linked_battery, CONF_LINKED_BATTERY_CHARGING_SENSOR: linked_battery_charging_sensor})
    assert acc.linked_battery_sensor == linked_battery
    with patch('homeassistant.components.homekit.accessories.HomeAccessory.async_update_state') as mock_async_update_state:
        await acc.run()
        await hass.async_block_till_done()
...
async def test_missing_linked_battery_charging_sensor(hass: HomeAssistant, hk_driver, caplog: pytest.LogCaptureFixture) -> None:
...
async def test_missing_linked_battery_sensor(hass: HomeAssistant, hk_driver, caplog: pytest.LogCaptureFixture) -> None:
...
async def test_battery_appears_after_startup(hass: HomeAssistant, hk_driver, caplog: pytest.LogCaptureFixture) -> None:
...
async def test_call_service(hass: HomeAssistant, hk_driver, events) -> None:
    ...
    test_value = 'value'
    acc.async_call_service(test_domain, test_service, {ATTR_ENTITY_ID: entity_id}, test_value)
    await hass.async_block_till_done()
    assert len(events) == 1
    assert events[0].data == {ATTR_ENTITY_ID: acc.entity_id, ATTR_DISPLAY_NAME: acc.display_name, ATTR_SERVICE: test_service, ATTR_VALUE: test_value}
