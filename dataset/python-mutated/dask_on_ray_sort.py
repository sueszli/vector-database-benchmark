...
if __name__ == '__main__':
    ...
    parser.add_argument('--nbytes', type=int, default=1000000)
    parser.add_argument('--npartitions', type=int, default=100, required=False)
    parser.add_argument('--max-partition-size', type=int, default=1000000000, required=False)
    ...
    print(trial(client, args.data_dir, 1000, 10, args.generate_only, s3_bucket=args.s3_bucket, file_path=args.file_path))
    print('WARMUP DONE')
    ...
    output = trial(client, args.data_dir, args.nbytes, npartitions, args.generate_only, s3_bucket=args.s3_bucket, file_path=args.file_path)
    ...
    with open('output.csv', 'a+') as csvfile:
        ...
        if write_header:
            writer.writeheader()
        row = {'num_nodes': args.num_nodes, 'nbytes': args.nbytes, 'npartitions': npartitions, 'dask_tasks': args.dask_tasks, 'dask_nprocs': args.dask_nprocs, 'dask_nthreads': args.dask_nthreads, 'dask_memlimit': args.dask_memlimit}
