...
from mage_ai.data_cleaner.transformer_actions.constants import CONSTANT_IMPUTATION_DEFAULTS, INVALID_VALUE_PLACEHOLDERS
...
REGEX_INTEGER_PATTERN = '^\\-{0,1}\\s*(?:(?:[$€¥₹£]|Rs|CAD){0,1}\\s*(?:[0-9]+(?:,[0-9]+)*|[0-9]+){0,1}(?:\\.0*)?|(?:[0-9]+(?:,[0-9]+)*|[0-9]+){0,1}(?:\\.0*)?\\s*(?:[元€$]|CAD){0,1})$'
REGEX_INTEGER = re.compile(REGEX_INTEGER_PATTERN)
REGEX_LIST_PATTERN = '^[\\[(](?:[^\\n\\r],?)*[\\])]$'
REGEX_LIST = re.compile(REGEX_LIST_PATTERN)
REGEX_NUMBER_PATTERN = '^\\-{0,1}\\s*(?:(?:[$€¥₹£]|Rs|CAD){0,1}\\s*(?:[0-9]+(?:,[0-9]+)*|[0-9]+){0,1}(?:\\.[0-9]*){0,1}|(?:[0-9]+(?:,[0-9]+)*|[0-9]+){0,1}(?:\\.[0-9]*){0,1}\\s*(?:[元€$]|CAD){0,1})\\s*\\%{0,1}$'
REGEX_NUMBER = re.compile(REGEX_NUMBER_PATTERN)
REGEX_PHONE_NUMBER_PATTERN = '^\\s*(?:\\+?(\\d{1,3}))?[-. (]*(\\d{3})[-. )]*(\\d{3})[-. ]*(\\d{4})(?: *x(\\d+))?\\s*$'
REGEX_PHONE_NUMBER = re.compile(REGEX_PHONE_NUMBER_PATTERN)
REGEX_ZIP_CODE_PATTERN = '^\\d{3,5}(?:[-\\s]\\d{4})?$'
