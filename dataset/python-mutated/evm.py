...
from ..core.smtlib import SelectedSolver, BitVec, Array, ArrayProxy, Operators, Constant, ArrayVariable, ArrayStore, BitVecConstant, translate_to_smtlib, to_constant, simplify, get_depth, issymbolic, get_taints, istainted, taint_with
...
def globalfakesha3(data):
    if False:
        while True:
            i = 10
    return None
consts.add('oog', default='ignore', description="Default behavior for symbolic gas.pedantic: Fully faithful. Test at every instruction. Forks.complete: Mostly faithful. Test at BB limit. Forks.concrete: Incomplete. Concretize gas to MIN/MAX values. Forks.optimistic: Try to not fail due to OOG. If it can be enough gas use it. Ignore the path to OOG. Wont forkpesimistic: Try OOG asap. Fail soon. Ignore the path with enough gas.ignore: Ignore gas. Instructions won't consume gas")
consts.add('txfail', default='optimistic', description='Default behavior for transaction failing because not enough funds.optimistic: Assume there is always enough funds to pay for value and gas. Wont forkpessimistic: Assume the balance is never enough for paying fo a transaction. Wont forkboth: Will fork for both options if possible.')
consts.add('calldata_max_offset', default=1024 * 1024, description='Max calldata offset to explore with. Iff offset or size in a calldata related instruction are symbolic it will be constrained to this constant')
consts.add('calldata_max_size', default=-1, description='Max calldata size to explore in each CALLDATACOPY. Iff size in a calldata related instruction are symbolic it will be constrained to be less than this constant. -1 means free(only use when gas is being tracked)')
...
TOOHIGHMEM = 4096
DEFAULT_FORK = 'istanbul'
PendingTransaction = namedtuple('PendingTransaction', ['type', 'address', 'price', 'data', 'caller', 'value', 'gas', 'failed'])
EVMLog = namedtuple('EVMLog', ['address', 'memlog', 'topics'])
BlockHeader = namedtuple('BlockHeader', ['blocknumber', 'timestamp', 'difficulty', 'gaslimit', 'coinbase'])
...
class Transaction:
    __slots__ = ('_sort', 'address', 'price', 'data', 'caller', 'value', 'depth', '_return_data', '_result', 'gas', '_used_gas')
...
class EVM(Eventful):
    ...
    """
    _published_events = {'evm_execute_instruction', 'evm_read_storage', 'evm_write_storage', 'evm_read_memory', 'evm_write_memory', 'evm_read_code', 'evm_write_code', 'decode_instruction', 'on_unsound_symbolication', 'solve'}
...
class EVMWorld(Platform):
    _published_events = {'evm_read_storage', 'evm_write_storage', 'evm_read_code', 'evm_write_code', 'decode_instruction', 'execute_instruction', 'open_transaction', 'close_transaction', 'symbolic_function', 'solve'}
