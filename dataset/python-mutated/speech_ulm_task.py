...
class SpeechUnitModelingConfig(FairseqDataclass):
    ...
    max_token_duration: int = field(default=20, metadata={'help': 'all token durations are capped to this value'})
    tokens_per_sample: int = field(default=1024, metadata={'help': 'tokens in a sample'})
    max_target_positions: int = field(default=1024, metadata={'help': 'max target positions'})
    ignore_duration_input: bool = field(default=False, metadata={'help': 'whether token durations should be zeroed out'})
    discrete_duration: bool = field(default=False, metadata={'help': 'treat duration as discrete variable'})
    ignore_f0_input: bool = field(default=False, metadata={'help': 'whether F0 should be zeroed out'})
    discrete_f0: bool = field(default=False, metadata={'help': 'load quantized f0. get bin from config'})
    log_f0: bool = field(default=False, metadata={'help': 'whether f0 should be modeled in log space'})
    normalize_f0_mean: bool = field(default=False, metadata={'help': 'whether normalize f0 by speaker mean'})
    normalize_f0_std: bool = field(default=False, metadata={'help': 'whether normalize f0 by speaker stddev'})
    interpolate_f0: bool = field(default=False, metadata={'help': 'whether interpolate f0 for non-voiced segments'})
    stream_shifts: str = field(default='0,0', metadata={'help': 'comma-separated integer list denoting right-shift for duration and pitch streams'})
