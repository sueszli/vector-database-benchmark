...
class Test_kms_cmk_are_used:
    ...
    def test_kms_no_keys(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.kms.kms_cmk_are_used.kms_cmk_are_used.kms_client', new=KMS(current_audit_info)):
            from prowler.providers.aws.services.kms.kms_cmk_are_used.kms_cmk_are_used import kms_cmk_are_used
    ...
    def test_kms_key_with_deletion(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.kms.kms_cmk_are_used.kms_cmk_are_used.kms_client', new=KMS(current_audit_info)):
            ...
            assert len(result) == 1
            assert result[0].status == 'PASS'
            assert result[0].status_extended == f"KMS CMK {key['KeyId']} is not being used but it has scheduled deletion."
            assert result[0].resource_id == key['KeyId']
            assert result[0].resource_arn == key['Arn']
    ...
    def test_kms_disabled_key(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.kms.kms_cmk_are_used.kms_cmk_are_used.kms_client', new=KMS(current_audit_info)):
            ...
            assert result[0].status_extended == f"KMS CMK {key['KeyId']} is not being used."
            assert result[0].resource_id == key['KeyId']
            assert result[0].resource_arn == key['Arn']