...
class ClapProcessorTest(unittest.TestCase):
    ...
    def test_save_load_pretrained_additional_features(self):
        ...
        processor = ClapProcessor.from_pretrained(self.tmpdirname, bos_token='(BOS)', eos_token='(EOS)', do_normalize=False, padding_value=1.0)
    ...
    def test_model_input_names(self):
        ...
        tokenizer = self.get_tokenizer()
        processor = ClapProcessor(tokenizer=tokenizer, feature_extractor=feature_extractor)
        self.assertListEqual(processor.model_input_names[2:], feature_extractor.model_input_names, msg='`processor` and `feature_extractor` model input names do not match')