...
class TaskRunnerStandardTestSuite(ABC):
    ...
    async def test_async_tasks_run_sequentially_with_sequential_concurrency_type(self, task_runner, tmp_file):
        if task_runner.concurrency_type != TaskConcurrencyType.SEQUENTIAL:
            pytest.skip(f'This test does not apply to {task_runner.concurrency_type} task runners.')
    ...
    async def test_async_tasks_run_concurrently_with_nonsequential_concurrency_type(self, task_runner, tmp_file):
        if task_runner.concurrency_type == TaskConcurrencyType.SEQUENTIAL:
            pytest.skip(f'This test does not apply to {task_runner.concurrency_type} task runners.')
    ...
    async def test_async_tasks_run_concurrently_with_task_group_with_all_concurrency_types(self, task_runner, tmp_file):
    ...
    async def test_async_subflows_run_sequentially_with_all_concurrency_types(self, task_runner, tmp_file):
    ...
    async def test_async_subflows_run_concurrently_with_task_group_with_all_concurrency_types(self, task_runner, tmp_file):
    ...
    async def test_submit_and_wait(self, task_runner):
        ...
        async def fake_orchestrate_task_run(example_kwarg):
            return State(type=StateType.COMPLETED, data=example_kwarg)
        async with task_runner.start():
            await task_runner.submit(key=task_run.id, call=partial(fake_orchestrate_task_run, example_kwarg=1))
    ...
    async def test_wait_captures_exceptions_as_crashed_state(self, task_runner, exception):
        if task_runner.concurrency_type != TaskConcurrencyType.PARALLEL:
            pytest.skip(f'This will abort the run for {task_runner.concurrency_type} task runners.')
        task_run = TaskRun(flow_run_id=uuid4(), task_key='foo', dynamic_key='bar')

        async def fake_orchestrate_task_run():
            raise exception
        async with task_runner.start():
            await task_runner.submit(key=task_run.id, call=fake_orchestrate_task_run)
            state = await task_runner.wait(task_run.id, 5)
            assert state is not None, 'wait timed out'
            assert isinstance(state, State), 'wait should return a state'
    ...
    async def test_async_tasks_run_sequentially_with_sequential_task_runners(self, task_runner, tmp_file):
        if task_runner.concurrency_type != TaskConcurrencyType.SEQUENTIAL:
            pytest.skip(f'This test does not apply to {task_runner.concurrency_type} task runners.')
    ...
    async def test_async_tasks_run_concurrently_with_parallel_task_runners(self, task_runner, tmp_file):
        if task_runner.concurrency_type != TaskConcurrencyType.PARALLEL:
            pytest.skip(f'This test does not apply to {task_runner.concurrency_type} task runners.')
