...
from azure.core.exceptions import ResourceExistsError, ResourceModifiedError, ResourceNotFoundError, ResourceNotModifiedError
...
class AzureAppConfigurationClient:
    ...
    async def get_configuration_setting(self, key: str, label: Optional[str]=None, etag: Optional[str]='*', match_condition: MatchConditions=MatchConditions.Unconditionally, **kwargs) -> Union[None, ConfigurationSetting]:
        ...
        try:
            key_value = await self._impl.get_key_value(key=key, label=label, accept_datetime=accept_datetime, if_match=prep_if_match(etag, match_condition), if_none_match=prep_if_none_match(etag, match_condition), error_map=error_map, **kwargs)
    ...
    @distributed_trace_async
    async def add_configuration_setting(self, configuration_setting: ConfigurationSetting, **kwargs) -> ConfigurationSetting:
        ...
        try:
            key_value_added = await self._impl.put_key_value(entity=key_value, key=key_value.key, label=key_value.label, if_none_match='*', headers=custom_headers, error_map=error_map)
            return ConfigurationSetting._from_generated(key_value_added)
        except binascii.Error as exc:
            raise binascii.Error('Connection string secret has incorrect padding') from exc

    @distributed_trace_async
    async def set_configuration_setting(self, configuration_setting: ConfigurationSetting, match_condition: MatchConditions=MatchConditions.Unconditionally, **kwargs) -> ConfigurationSetting:
        ...
        if match_condition == MatchConditions.IfMissing:
            error_map.update({412: ResourceExistsError})
        try:
            key_value_set = await self._impl.put_key_value(entity=key_value, key=key_value.key, label=key_value.label, if_match=prep_if_match(configuration_setting.etag, match_condition), if_none_match=prep_if_none_match(etag, match_condition), headers=custom_headers, error_map=error_map)
            return ConfigurationSetting._from_generated(key_value_set)
        except binascii.Error as exc:
            raise binascii.Error('Connection string secret has incorrect padding') from exc

    @distributed_trace_async
    async def delete_configuration_setting(self, key: str, label: Optional[str]=None, **kwargs) -> ConfigurationSetting:
        ...
        if match_condition == MatchConditions.IfMissing:
            error_map.update({412: ResourceExistsError})
        try:
            key_value_deleted = await self._impl.delete_key_value(key=key, label=label, if_match=prep_if_match(etag, match_condition), headers=custom_headers, error_map=error_map)
    ...
    @distributed_trace_async
    async def set_read_only(self, configuration_setting: ConfigurationSetting, read_only: bool=True, **kwargs) -> ConfigurationSetting:
        ...
        try:
            if read_only:
                key_value = await self._impl.put_lock(key=configuration_setting.key, label=configuration_setting.label, if_match=prep_if_match(configuration_setting.etag, match_condition), if_none_match=prep_if_none_match(configuration_setting.etag, match_condition), error_map=error_map, **kwargs)
            else:
                key_value = await self._impl.delete_lock(key=configuration_setting.key, label=configuration_setting.label, if_match=prep_if_match(configuration_setting.etag, match_condition), if_none_match=prep_if_none_match(configuration_setting.etag, match_condition), error_map=error_map, **kwargs)
            return ConfigurationSetting._from_generated(key_value)
        except binascii.Error as exc:
            raise binascii.Error('Connection string secret has incorrect padding') from exc

    @distributed_trace_async
    async def begin_create_snapshot(self, name: str, filters: List[ConfigurationSettingsFilter], *, composition_type: Optional[Literal['key', 'key_label']]=None, retention_period: Optional[int]=None, tags: Optional[Dict[str, str]]=None, **kwargs) -> AsyncLROPoller[ConfigurationSnapshot]:
        ...
        try:
            return cast(AsyncLROPoller[ConfigurationSnapshot], await self._impl.begin_create_snapshot(name=name, entity=snapshot._to_generated(), cls=ConfigurationSnapshot._from_deserialized, **kwargs))
        except binascii.Error:
            raise binascii.Error('Connection string secret has incorrect padding')

    @distributed_trace_async
    async def archive_snapshot(self, name: str, *, match_condition: MatchConditions=MatchConditions.Unconditionally, etag: Optional[str]=None, **kwargs) -> ConfigurationSnapshot:
        ...
        if match_condition == MatchConditions.IfMissing:
            error_map.update({412: ResourceExistsError})
        try:
            generated_snapshot = await self._impl.update_snapshot(name=name, entity=SnapshotUpdateParameters(status=SnapshotStatus.ARCHIVED), if_match=prep_if_match(etag, match_condition), if_none_match=prep_if_none_match(etag, match_condition), error_map=error_map, **kwargs)
            return ConfigurationSnapshot._from_generated(generated_snapshot)
        except binascii.Error:
            raise binascii.Error('Connection string secret has incorrect padding')

    @distributed_trace_async
    async def recover_snapshot(self, name: str, *, match_condition: MatchConditions=MatchConditions.Unconditionally, etag: Optional[str]=None, **kwargs) -> ConfigurationSnapshot:
        ...
        if match_condition == MatchConditions.IfMissing:
            error_map.update({412: ResourceExistsError})
        try:
            generated_snapshot = await self._impl.update_snapshot(name=name, entity=SnapshotUpdateParameters(status=SnapshotStatus.READY), if_match=prep_if_match(etag, match_condition), if_none_match=prep_if_none_match(etag, match_condition), error_map=error_map, **kwargs)
            return ConfigurationSnapshot._from_generated(generated_snapshot)
        except binascii.Error:
            raise binascii.Error('Connection string secret has incorrect padding')
    ...
    async def get_snapshot(self, name: str, *, fields: Optional[List[str]]=None, **kwargs) -> ConfigurationSnapshot:
        ...
        """
        try:
            generated_snapshot = await self._impl.get_snapshot(name=name, if_match=None, if_none_match=None, select=fields, **kwargs)
            return ConfigurationSnapshot._from_generated(generated_snapshot)
        except binascii.Error:
            raise binascii.Error('Connection string secret has incorrect padding')
