...
def main():
    ...
    parser.add_argument('--output', default='', metavar='FILE', help='print outputs into a pretty format')
...
def sentence_bleu(hypothesis, reference):
    ...
    for i in range(1, 4):
        bleu.counts[i] += 1
        bleu.totals[i] += 1
    bleu = sacrebleu.BLEU.compute_bleu(bleu.counts, bleu.totals, bleu.sys_len, bleu.ref_len, smooth_method='exp')
    return bleu.score
