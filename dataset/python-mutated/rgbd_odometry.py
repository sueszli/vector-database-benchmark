...
if __name__ == '__main__':
    parser = ConfigParser()
    parser.add('--config', is_config_file=True, help='YAML config file path.Please refer to config.py for the options,and default_config.yml for default settings It overrides the default config file, but will be overridden by other command line inputs.')
    parser.add('--default_dataset', help='Default dataset is used when config file is not provided. Default dataset may be selected from the following options: [lounge, jack_jack]', default='lounge')
    ...
    info = o3d.t.pipelines.odometry.compute_odometry_information_matrix(depth_src, depth_dst, intrinsic, res.transformation, 0.07)
    ...
    (success, trans, info) = o3d.pipelines.odometry.compute_rgbd_odometry(rgbd_src_legacy, rgbd_dst_legacy, intrinsic_legacy, np.eye(4), o3d.pipelines.odometry.RGBDOdometryJacobianFromHybridTerm())
    print(trans, info)
