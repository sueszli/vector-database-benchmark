...
class ProjectReplayRecordingSegmentIndexEndpoint(ProjectEndpoint):
    ...
    def get(self, request: Request, project, replay_id: str) -> Response:
        ...
        return self.paginate(request=request, response_cls=StreamingHttpResponse, response_kwargs={'content_type': 'application/json'}, paginator_cls=GenericOffsetPaginator, data_fn=functools.partial(fetch_segments_metadata, project.id, replay_id), on_results=download_segments)