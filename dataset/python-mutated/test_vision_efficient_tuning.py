...
from modelscope.models.cv.vision_efficient_tuning.model import VisionEfficientTuningModel
...
class VisionEfficientTuningTest(unittest.TestCase):
    ...
    @unittest.skipUnless(test_level() >= 2, 'skip test in current test level')
    def test_vision_efficient_tuning_sidetuning_load_model_from_pretrained(self):
        if False:
            print('Hello World!')
        model_id = 'damo/cv_vitb16_classification_vision-efficient-tuning-sidetuning'
        model = Model.from_pretrained(model_id)
