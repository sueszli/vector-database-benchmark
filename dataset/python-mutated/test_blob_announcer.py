...
class TestBlobAnnouncer(AsyncioTestCase):
    ...
    def add_peer_to_routing_table(self, adder, being_added):
        if False:
            i = 10
            return i + 15
        adder.protocol.add_peer(make_kademlia_peer(being_added.protocol.node_id, being_added.protocol.external_ip, being_added.protocol.udp_port))

    @contextlib.asynccontextmanager
    async def _test_network_context(self, peer_count=200):
        self.peer_addresses = [(constants.generate_id(i), socket.inet_ntoa(int(i + 16777217).to_bytes(length=4, byteorder='big'))) for i in range(1, peer_count + 1)]
        try:
    ...
    async def chain_peer(self, node_id, address):
        ...
        peer = last_node.protocol.get_rpc_peer(make_kademlia_peer(previous_last_node.protocol.node_id, previous_last_node.protocol.external_ip, previous_last_node.protocol.udp_port))
        await peer.ping()
        return last_node
    ...
    async def test_announce_blobs(self):
        ...
        async with self._test_network_context(peer_count=100):
            ...
            await self.storage.add_blobs(*((constants.generate_id(value).hex(), 1024, 0, True) for value in range(1000, 1090)), finished=True)
            ...
            self.blob_announcer.start(batch_size=10)
    ...
    async def test_popular_blob(self):
        ...
        async with self._test_network_context(peer_count=peer_count):
            ...
            for (i, node) in enumerate(self.nodes.values()):
                ...
                peer = node.protocol.get_rpc_peer(make_kademlia_peer(announced_to.protocol.node_id, announced_to.protocol.external_ip, announced_to.protocol.udp_port))
                ...
                if i - 1 > constants.K:
                    ...
                    self.assertEqual(len(peers_for_blob[b'contacts']), 8)
