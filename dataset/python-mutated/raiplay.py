...
@pluginmatcher(name='live', pattern=re.compile('https?://(?:www\\.)?raiplay\\.it/dirette/(\\w+)/?'))
@pluginmatcher(name='vod', pattern=re.compile('https?://(?:www\\.)?raiplay\\.it/video/.+'))
@pluginargument('email', requires=['password'], metavar='EMAIL', help='The email used to register with raiplay.it.')
@pluginargument('password', prompt='Enter raiplay.it account password', sensitive=True, metavar='PASSWORD', help='A raiplay.it account password to use with --raiplay-email.')
@pluginargument('purge-credentials', action='store_true', help='Purge cached RaiPlay credentials to initiate a new session and reauthenticate.')
