...
class TwitterToolkit(BaseToolkit, ABC):
    ...
    def get_env_keys(self) -> List[ToolConfiguration]:
        if False:
            while True:
                i = 10
        return [ToolConfiguration(key='TWITTER_API_KEY', key_type=ToolConfigKeyType.STRING, is_required=True, is_secret=True), ToolConfiguration(key='TWITTER_API_SECRET', key_type=ToolConfigKeyType.STRING, is_required=True, is_secret=True)]