...
class GlobalConfig:
    ...
    not_found_error = botocore.exceptions.ClientError({'Error': {'Code': 'ResourceNotFoundException', 'Message': 'Test-defined error'}}, 'msg')
    topic_rule_not_found_error = botocore.exceptions.ClientError({'Error': {'Code': 'UnauthorizedException', 'Message': 'Test-defined error'}}, 'msg')
    error_content = {'Error': {'Code': 101, 'Message': 'Test-defined error'}}
    policy_ret = dict(policyName='testpolicy', policyDocument='{"Version": "2012-10-17", "Statement": [{"Action": ["iot:Publish"], "Resource": ["*"], "Effect": "Allow"}]}', policyArn='arn:aws:iot:us-east-1:123456:policy/my_policy', policyVersionId=1, defaultVersionId=1)
    topic_rule_ret = dict(ruleName='testrule', sql="SELECT * FROM 'iot/test'", description='topic rule description', createdAt='1970-01-01', actions=[{'iot': {'functionArn': 'arn:aws:::function'}}], ruleDisabled=True)
    ...
    thing_type_ret = dict(thingTypeName=thing_type_name, thingTypeProperties=dict(thingTypeDescription=thing_type_desc, searchableAttributes=[thing_type_attr_1]), thingTypeMetadata=dict(deprecated=False, creationDate='2010-08-01 15:54:49.699000+00:00'))
    deprecated_thing_type_ret = dict(thingTypeName=thing_type_name, thingTypeProperties=dict(thingTypeDescription=thing_type_desc, searchableAttributes=[thing_type_attr_1]), thingTypeMetadata=dict(deprecated=True, creationDate='2010-08-01 15:54:49.699000+00:00', deprecationDate='2010-08-02 15:54:49.699000+00:00'))
