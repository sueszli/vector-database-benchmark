...
async def test_datasette_constructor():
    ...
    assert databases == [{'name': '_memory', 'route': '_memory', 'path': None, 'size': 0, 'is_mutable': False, 'is_memory': True, 'hash': None}]
...
@pytest.mark.asyncio
@pytest.mark.parametrize('actor,config,permissions,should_allow,expected_private', ((None, ALLOW_ROOT, ['view-instance'], False, False), (ROOT, ALLOW_ROOT, ['view-instance'], True, True), (None, {'databases': {'_memory': ALLOW_ROOT}}, [('view-database', '_memory')], False, False), (ROOT, {'databases': {'_memory': ALLOW_ROOT}}, [('view-database', '_memory')], True, True), (ROOT, {'allow': True}, ['view-instance'], True, False)))
async def test_datasette_ensure_permissions_check_visibility(actor, config, permissions, should_allow, expected_private):
