...
def temporal_range(range_lower, range_upper):
    if False:
        return 10
    return orm.with_loader_criteria(HasTemporal, lambda cls: cls.timestamp.between(range_lower, range_upper), include_aliases=True)
if __name__ == '__main__':
    ...
    sess.add_all([p1, p2])
    sess.commit()
    parents = sess.query(Parent).populate_existing().options(temporal_range(datetime.datetime(2009, 10, 16, 12, 0, 0), datetime.datetime(2009, 10, 18, 12, 0, 0))).all()
    assert parents[0] == p2
    assert parents[0].children == [c5]
    sess.expire_all()
    parents = sess.query(Parent).options(temporal_range(datetime.datetime(2009, 10, 16, 12, 0, 0), datetime.datetime(2009, 10, 18, 12, 0, 0))).options(selectinload(Parent.children)).all()
    assert parents[0] == p2
    assert parents[0].children == [c5]
    sess.expire_all()
    print('------------------')
    parents = sess.execute(select(Parent).execution_options(populate_existing=True).options(temporal_range(datetime.datetime(2009, 10, 15, 11, 0, 0), datetime.datetime(2009, 10, 18, 12, 0, 0))).join(Parent.children).filter(Child.id == 2)).scalars().all()
    assert parents[0] == p1
    print('-------------------')
