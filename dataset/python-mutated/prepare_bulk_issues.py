...
def render_template_string(template_string: str, context: dict[str, Any], autoescape: bool=True, keep_trailing_newline: bool=False) -> str:
    ...
    content: str = template.render(context)
    return content
option_github_token = click.option('--github-token', type=str, required=True, help=textwrap.dedent('\n        GitHub token used to authenticate.\n        You can omit it if you have GITHUB_TOKEN env variable set\n        Can be generated with:\n        https://github.com/settings/tokens/new?description=Write%20issues&scopes=repo:status,public_repo'), envvar='GITHUB_TOKEN')
option_dry_run = click.option('--dry-run', is_flag=True, help='Do not create issues, just print the issues to be created')
option_csv_file = click.option('--csv-file', type=str, required=True, help='CSV file to bulk load. The first column is used to group the remaining rows and name them')
option_project = click.option('--project', type=str, help='Project to create issues in')
option_repository = click.option('--repository', type=str, default='apache/airflow', help='Repo to use')
option_title = click.option('--title', type=str, required='true', help='Title of the issues to create (might contain {{ name }} to indicate the name of the group)')
option_labels = click.option('--labels', type=str, help='Labels to assign to the issues (comma-separated)')
option_template_file = click.option('--template-file', type=str, required=True, help='Jinja template file to use for issue content')
option_max_issues = click.option('--max-issues', type=int, help='Maximum number of issues to create')
option_start_from = click.option('--start-from', type=int, default=0, help='Start from issue number N (useful if you are blocked by secondary rate limit)')
