...
class RecruitmentStage(models.Model):
    ...
    job_id = fields.Many2one('hr.job', string='Job Specific', ondelete='cascade', help='Specific job that uses this stage. Other jobs will not use this stage.')
    ...
    fold = fields.Boolean('Folded in Recruitment Pipe', help='This stage is folded in the kanban view when there are no records in that stage to display.')
...
class RecruitmentDegree(models.Model):
    ...
    _sql_constraints = [('name_uniq', 'unique (name)', 'The name of the Degree of Recruitment must be unique!')]
...
class Applicant(models.Model):
    ...
    stage_id = fields.Many2one('hr.recruitment.stage', 'Stage', track_visibility='onchange', domain="['|', ('job_id', '=', False), ('job_id', '=', job_id)]", copy=False, index=True, group_expand='_read_group_stage_ids', default=_default_stage_id)
    last_stage_id = fields.Many2one('hr.recruitment.stage', 'Last Stage', help='Stage of the applicant before being in the current stage. Used for lost cases analysis.')
