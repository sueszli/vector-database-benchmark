...
import time
import warnings
from scapy.arch import get_if_addr, get_if_addr6, read_nameservers
...
from scapy.fields import BitEnumField, BitField, ByteEnumField, ByteField, ConditionalField, Field, FieldLenField, FlagsField, I, IP6Field, IntField, MultipleTypeField, PacketListField, ShortEnumField, ShortField, StrField, StrLenField, UTCTimeField
...
class DNSTextField(StrLenField):
    ...
    def i2m(self, pkt, s):
        ...
        return ret_s
edns0types = {0: 'Reserved', 1: 'LLQ', 2: 'UL', 3: 'NSID', 4: 'Reserved', 5: 'PING', 8: 'edns-client-subnet', 10: 'COOKIE', 15: 'Extended DNS Error'}
...
class DNSRROPT(Packet):
    ...
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 41, dnstypes), ShortField('rclass', 4096), ByteField('extrcode', 0), ByteField('version', 0), BitEnumField('z', 32768, 16, {32768: 'D0'}), FieldLenField('rdlen', None, length_of='rdata', fmt='H'), PacketListField('rdata', [], EDNS0TLV, length_from=lambda pkt: pkt.rdlen)]
...
class EDNS0ClientSubnet(Packet):
    ...
    fields_desc = [ShortEnumField('optcode', 8, edns0types), FieldLenField('optlen', None, 'address', fmt='H', adjust=lambda pkt, x: x + 4), ShortField('family', 1), FieldLenField('source_plen', None, length_of='address', fmt='B', adjust=lambda pkt, x: x * 8), ByteField('scope_plen', 0), MultipleTypeField([(ClientSubnetv4('address', '192.168.0.0', length_from=lambda p: p.source_plen), lambda pkt: pkt.family == 1), (ClientSubnetv6('address', '2001:db8::', length_from=lambda p: p.source_plen), lambda pkt: pkt.family == 2)], ClientSubnetv4('address', '192.168.0.0', length_from=lambda p: p.source_plen))]
extended_dns_error_codes = {0: 'Other', 1: 'Unsupported DNSKEY Algorithm', 2: 'Unsupported DS Digest Type', 3: 'Stale Answer', 4: 'Forged Answer', 5: 'DNSSEC Indeterminate', 6: 'DNSSEC Bogus', 7: 'Signature Expired', 8: 'Signature Not Yet Valid', 9: 'DNSKEY Missing', 10: 'RRSIGs Missing', 11: 'No Zone Key Bit Set', 12: 'NSEC Missing', 13: 'Cached Error', 14: 'Not Ready', 15: 'Blocked', 16: 'Censored', 17: 'Filtered', 18: 'Prohibited', 19: 'Stale NXDOMAIN Answer', 20: 'Not Authoritative', 21: 'Not Supported', 22: 'No Reachable Authority', 23: 'Network Error', 24: 'Invalid Data', 25: 'Signature Expired before Valid', 26: 'Too Early', 27: 'Unsupported NSEC3 Iterations Value', 28: 'Unable to conform to policy', 29: 'Synthesized'}

class EDNS0ExtendedDNSError(Packet):
    name = 'DNS EDNS0 Extended DNS Error'
    fields_desc = [ShortEnumField('optcode', 15, edns0types), FieldLenField('optlen', None, length_of='extra_text', fmt='!H', adjust=lambda pkt, x: x + 2), ShortEnumField('info_code', 0, extended_dns_error_codes), StrLenField('extra_text', '', length_from=lambda pkt: pkt.optlen - 2)]
dnssecalgotypes = {0: 'Reserved', 1: 'RSA/MD5', 2: 'Diffie-Hellman', 3: 'DSA/SHA-1', 4: 'Reserved', 5: 'RSA/SHA-1', 6: 'DSA-NSEC3-SHA1', 7: 'RSASHA1-NSEC3-SHA1', 8: 'RSA/SHA-256', 9: 'Reserved', 10: 'RSA/SHA-512', 11: 'Reserved', 12: 'GOST R 34.10-2001', 13: 'ECDSA Curve P-256 with SHA-256', 14: 'ECDSA Curve P-384 with SHA-384', 252: 'Reserved for Indirect Keys', 253: 'Private algorithms - domain name', 254: 'Private algorithms - OID', 255: 'Reserved'}
...
class DNSRRMX(_DNSRRdummy):
    ...
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 6, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), ShortField('preference', 0), DNSStrField('exchange', '')]

class DNSRRSOA(_DNSRRdummy):
    name = 'DNS SOA Resource Record'
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 6, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), DNSStrField('mname', ''), DNSStrField('rname', ''), IntField('serial', 0), IntField('refresh', 0), IntField('retry', 0), IntField('expire', 0), IntField('minimum', 0)]

class DNSRRRSIG(_DNSRRdummy):
    name = 'DNS RRSIG Resource Record'
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 46, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), ShortEnumField('typecovered', 1, dnstypes), ByteEnumField('algorithm', 5, dnssecalgotypes), ByteField('labels', 0), IntField('originalttl', 0), UTCTimeField('expiration', 0), UTCTimeField('inception', 0), ShortField('keytag', 0), DNSStrField('signersname', ''), StrField('signature', '')]
...
class DNSRRDNSKEY(_DNSRRdummy):
    ...
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 48, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), FlagsField('flags', 256, 16, 'S???????Z???????'), ByteField('protocol', 3), ByteEnumField('algorithm', 5, dnssecalgotypes), StrField('publickey', '')]

class DNSRRDS(_DNSRRdummy):
    name = 'DNS DS Resource Record'
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 43, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), ShortField('keytag', 0), ByteEnumField('algorithm', 5, dnssecalgotypes), ByteEnumField('digesttype', 5, dnssecdigesttypes), StrField('digest', '')]
...
class DNSRRNSEC3(_DNSRRdummy):
    ...
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 50, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), ByteField('hashalg', 0), BitEnumField('flags', 0, 8, {1: 'Opt-Out'}), ShortField('iterations', 0), FieldLenField('saltlength', 0, fmt='!B', length_of='salt'), StrLenField('salt', '', length_from=lambda x: x.saltlength), FieldLenField('hashlength', 0, fmt='!B', length_of='nexthashedownername'), StrLenField('nexthashedownername', '', length_from=lambda x: x.hashlength), RRlistField('typebitmaps', '')]

class DNSRRNSEC3PARAM(_DNSRRdummy):
    name = 'DNS NSEC3PARAM Resource Record'
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 51, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), ByteField('hashalg', 0), ByteField('flags', 0), ShortField('iterations', 0), FieldLenField('saltlength', 0, fmt='!B', length_of='salt'), StrLenField('salt', '', length_from=lambda pkt: pkt.saltlength)]

class DNSRRSRV(_DNSRRdummy):
    name = 'DNS SRV Resource Record'
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 33, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), ShortField('priority', 0), ShortField('weight', 0), ShortField('port', 0), DNSStrField('target', '')]
...
class DNSRRTSIG(_DNSRRdummy):
    ...
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 250, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), ShortField('rdlen', None), DNSStrField('algo_name', 'hmac-sha1'), TimeSignedField('time_signed', 0), ShortField('fudge', 0), FieldLenField('mac_len', 20, fmt='!H', length_of='mac_data'), StrLenField('mac_data', '', length_from=lambda pkt: pkt.mac_len), ShortField('original_id', 0), ShortField('error', 0), FieldLenField('other_len', 0, fmt='!H', length_of='other_data'), StrLenField('other_data', '', length_from=lambda pkt: pkt.other_len)]
DNSRR_DISPATCHER = {6: DNSRRSOA, 15: DNSRRMX, 33: DNSRRSRV, 41: DNSRROPT, 43: DNSRRDS, 46: DNSRRRSIG, 47: DNSRRNSEC, 48: DNSRRDNSKEY, 50: DNSRRNSEC3, 51: DNSRRNSEC3PARAM, 250: DNSRRTSIG, 32769: DNSRRDLV}

class DNSRR(Packet):
    name = 'DNS Resource Record'
    show_indent = 0
    fields_desc = [DNSStrField('rrname', ''), ShortEnumField('type', 1, dnstypes), ShortEnumField('rclass', 1, dnsclasses), IntField('ttl', 0), FieldLenField('rdlen', None, length_of='rdata', fmt='H'), MultipleTypeField([(IPField('rdata', '0.0.0.0'), lambda pkt: pkt.type == 1), (IP6Field('rdata', '::'), lambda pkt: pkt.type == 28), (DNSStrField('rdata', '', length_from=lambda pkt: pkt.rdlen), lambda pkt: pkt.type in [2, 3, 4, 5, 12]), (DNSTextField('rdata', [], length_from=lambda pkt: pkt.rdlen), lambda pkt: pkt.type == 16)], StrLenField('rdata', '', length_from=lambda pkt: pkt.rdlen))]
