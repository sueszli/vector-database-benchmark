...
from .const import AUTH, DATA_PERSONS, DATA_SCHEDULES, DOMAIN, MANUFACTURER, NETATMO_CREATE_BATTERY, NETATMO_CREATE_CAMERA, NETATMO_CREATE_CAMERA_LIGHT, NETATMO_CREATE_CLIMATE, NETATMO_CREATE_COVER, NETATMO_CREATE_LIGHT, NETATMO_CREATE_ROOM_SENSOR, NETATMO_CREATE_SELECT, NETATMO_CREATE_SENSOR, NETATMO_CREATE_SWITCH, NETATMO_CREATE_WEATHER_SENSOR, PLATFORMS, WEBHOOK_ACTIVATION, WEBHOOK_DEACTIVATION, WEBHOOK_NACAMERA_CONNECTION, WEBHOOK_PUSH_TYPE
...
EVENT = 'event'
PUBLISHERS = {ACCOUNT: 'async_update_topology', HOME: 'async_update_status', WEATHER: 'async_update_weather_stations', AIR_CARE: 'async_update_air_care', PUBLIC: 'async_update_public_weather', EVENT: 'async_update_events'}
BATCH_SIZE = 3
DEFAULT_INTERVALS = {ACCOUNT: 10800, HOME: 300, WEATHER: 600, AIR_CARE: 300, PUBLIC: 600, EVENT: 600}
SCAN_INTERVAL = 60
...
class NetatmoDataHandler:
    ...
    async def async_setup(self) -> None:
        ...
        self.config_entry.async_on_unload(async_track_time_interval(self.hass, self.async_update, timedelta(seconds=SCAN_INTERVAL)))
        self.config_entry.async_on_unload(async_dispatcher_connect(self.hass, f'signal-{DOMAIN}-webhook-None', self.handle_event))
    ...
    async def async_fetch_data(self, signal_name: str) -> None:
        ...
        try:
            await getattr(self.account, self.publisher[signal_name].method)(**self.publisher[signal_name].kwargs)
    ...
    async def subscribe(self, publisher: str, signal_name: str, update_callback: CALLBACK_TYPE | None, **kwargs: Any) -> None:
        ...
        self.publisher[signal_name] = NetatmoPublisher(name=signal_name, interval=DEFAULT_INTERVALS[publisher], next_scan=time() + DEFAULT_INTERVALS[publisher], subscriptions={update_callback}, method=PUBLISHERS[publisher], kwargs=kwargs)
        try:
    ...
    async def async_dispatch(self) -> None:
        ...
        for home in self.account.homes.values():
            ...
            self.hass.data[DOMAIN][DATA_PERSONS][home.entity_id] = {person.entity_id: person.pseudo for person in home.persons.values()}
