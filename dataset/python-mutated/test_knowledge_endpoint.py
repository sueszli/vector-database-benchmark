...
async def test_add_invalid_tag(rest_api):
    ...
    infohash = b'a' * 20
    await do_request(rest_api, f'knowledge/{hexlify(infohash)}', request_type='PATCH', expected_code=400, post_data=post_data)
...
async def test_modify_tags(rest_api, tribler_db):
    ...
    with freeze_time('2015-01-01') as frozen_time:
        await do_request(rest_api, f'knowledge/{infohash}', request_type='PATCH', expected_code=200, post_data=post_data)
        with db_session:
