...
class TaskRunCount(PrefectBaseModel):
    completed: int = Field(default=..., description='The number of completed task runs.')
...
async def read_dashboard_task_run_counts(task_runs: schemas.filters.TaskRunFilter, flows: Optional[schemas.filters.FlowFilter]=None, flow_runs: Optional[schemas.filters.FlowRunFilter]=None, deployments: Optional[schemas.filters.DeploymentFilter]=None, work_pools: Optional[schemas.filters.WorkPoolFilter]=None, work_queues: Optional[schemas.filters.WorkQueueFilter]=None, db: PrefectDBInterface=Depends(provide_database_interface)) -> List[TaskRunCount]:
    if task_runs.start_time is None or task_runs.start_time.after_ is None:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail='task_runs.start_time.after_ is required')
    task_runs.state = schemas.filters.TaskRunFilterState(type=schemas.filters.TaskRunFilterStateType(any_=list(schemas.states.TERMINAL_STATES)))
    bucket_count = 20
    start_time = task_runs.start_time.after_.start_of('minute')
    end_time = cast(DateTimeTZ, task_runs.start_time.before_.end_of('minute') if task_runs.start_time.before_ else pendulum.now('UTC').end_of('minute'))
    window = end_time - start_time
    delta = window.as_timedelta() / bucket_count
    async with db.session_context(begin_transaction=False) as session:
        start_datetime = datetime(start_time.year, start_time.month, start_time.day, start_time.hour, start_time.minute, start_time.second, start_time.microsecond, start_time.timezone)
        bucket_expression = _sqlite_bucket_expression(db, delta, start_datetime) if db.dialect.name == 'sqlite' else _postgres_bucket_expression(db, delta, start_datetime)
        raw_counts = (await models.task_runs._apply_task_run_filters(sa.select(bucket_expression, sa.func.min(db.TaskRun.end_time).label('oldest'), sa.func.sum(sa.case((db.TaskRun.state_type.in_(FAILED_STATES), 1), else_=0)).label('failed_count'), sa.func.sum(sa.case((db.TaskRun.state_type.notin_(FAILED_STATES), 1), else_=0)).label('successful_count')), flow_filter=flows, flow_run_filter=flow_runs, task_run_filter=task_runs, deployment_filter=deployments, work_pool_filter=work_pools, work_queue_filter=work_queues)).group_by('bucket', db.TaskRun.start_time).subquery()
        query = sa.select(raw_counts.c.bucket.label('bucket'), sa.func.min(raw_counts.c.oldest).label('oldest'), sa.func.sum(raw_counts.c.failed_count).label('failed_count'), sa.func.sum(raw_counts.c.successful_count).label('successful_count')).select_from(raw_counts).group_by(raw_counts.c.bucket).order_by(sa.asc('oldest'))
