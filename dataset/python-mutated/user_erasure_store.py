...
class UserErasureWorkerStore(CacheInvalidationWorkerStore):
    ...
    async def is_user_erased(self, user_id: str) -> bool:
        ...
        """
        result = await self.db_pool.simple_select_onecol(table='erased_users', keyvalues={'user_id': user_id}, retcol='1', desc='is_user_erased')
        return bool(result)
    ...
    async def are_users_erased(self, user_ids: Iterable[str]) -> Mapping[str, bool]:
        ...
        """
        rows = cast(List[Tuple[str]], await self.db_pool.simple_select_many_batch(table='erased_users', column='user_id', iterable=user_ids, retcols=('user_id',), desc='are_users_erased'))
