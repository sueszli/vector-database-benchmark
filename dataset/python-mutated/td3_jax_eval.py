...
def evaluate(model_path: str, make_env: Callable, env_id: str, eval_episodes: int, run_name: str, Model: nn.Module, capture_video: bool=True, exploration_noise: float=0.1, seed=1):
    ...
    with open(model_path, 'rb') as f:
        (actor_params, qf1_params, qf2_params) = flax.serialization.from_bytes((actor_params, qf1_params, qf2_params), f.read())
    actor.apply = jax.jit(actor.apply)
    qf.apply = jax.jit(qf.apply)
    episodic_returns = []
    while len(episodic_returns) < eval_episodes:
        actions = actor.apply(actor_params, obs)
        actions = np.array([(jax.device_get(actions)[0] + np.random.normal(0, max_action * exploration_noise, size=envs.single_action_space.shape)).clip(envs.single_action_space.low, envs.single_action_space.high)])
...
if __name__ == '__main__':
    ...
    model_path = hf_hub_download(repo_id='cleanrl/HalfCheetah-v4-td3_continuous_action_jax-seed1', filename='td3_continuous_action_jax.cleanrl_model')
    evaluate(model_path, make_env, 'HalfCheetah-v4', eval_episodes=10, run_name=f'eval', Model=(Actor, QNetwork), exploration_noise=0.1)