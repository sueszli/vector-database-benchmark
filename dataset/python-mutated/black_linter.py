...
def _run_command(args: List[str], *, stdin: BinaryIO, timeout: int) -> 'subprocess.CompletedProcess[bytes]':
    ...
    try:
        return subprocess.run(args, stdin=stdin, capture_output=True, shell=IS_WINDOWS, timeout=timeout, check=True)
    finally:
...
def check_file(filename: str, retries: int, timeout: int) -> List[LintMessage]:
    ...
    try:
        ...
        return [LintMessage(path=filename, line=None, char=None, code='BLACK', severity=LintSeverity.ERROR, name='timeout', original=None, replacement=None, description="black timed out while trying to process a file. Please report an issue in pytorch/pytorch with the label 'module: lint'")]
