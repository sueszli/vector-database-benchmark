...
class Higgs(base.RemoteDataset):
    ...
    def __init__(self):
        if False:
            for i in range(10):
                print('nop')
        super().__init__(n_samples=11000000, n_features=28, task=base.BINARY_CLF, url='https://archive.ics.uci.edu/ml/machine-learning-databases/00280/HIGGS.csv.gz', size=2816407858, unpack=False)
    ...
    def _iter(self):
        ...
        return stream.iter_csv(self.path, fieldnames=['is_signal', *features], target='is_signal', converters={'is_signal': lambda x: x.startswith('1'), **{f: float for f in features}})