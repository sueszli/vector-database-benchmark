...
RATE_LIMIT_EXCEEDED_COUNTER = Counter('rate_limit_exceeded_total', 'Dropped requests due to rate-limiting, per team_id, scope and path.', labelnames=[LABEL_TEAM_ID, 'scope', LABEL_PATH])
RATE_LIMIT_BYPASSED_COUNTER = Counter('rate_limit_bypassed_total', 'Requests that should be dropped by rate-limiting but allowed by configuration.', labelnames=[LABEL_TEAM_ID, LABEL_PATH])
DECIDE_RATE_LIMIT_EXCEEDED_COUNTER = Counter('decide_rate_limit_exceeded_total', 'Dropped requests due to rate-limiting, per token.', labelnames=['token'])
...
path_by_team_pattern = re.compile('/api/projects/(\\d+)/')
path_by_org_pattern = re.compile('/api/organizations/(.+)/')
