...
class AsyncRetryHandler:
    ...
    async def can_retry_async(self, *, state: RetryState, request: HttpRequest, response: Optional[HttpResponse]=None, error: Optional[Exception]=None) -> bool:
        if state.current_attempt >= self.max_retry_count:
            return False
        return await self._can_retry_async(state=state, request=request, response=response, error=error)

    async def _can_retry_async(self, *, state: RetryState, request: HttpRequest, response: Optional[HttpResponse]=None, error: Optional[Exception]=None) -> bool:
...
__all__ = ['RetryState', 'HttpRequest', 'HttpResponse', 'RetryIntervalCalculator', 'BackoffRetryIntervalCalculator', 'default_interval_calculator']