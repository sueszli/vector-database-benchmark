...
_arg_mapping = dict(raw=lambda dvi, delta: delta, u1=lambda dvi, delta: dvi._arg(1, signed=False), u4=lambda dvi, delta: dvi._arg(4, signed=False), s4=lambda dvi, delta: dvi._arg(4, signed=True), slen=lambda dvi, delta: dvi._arg(delta, signed=True) if delta else None, slen1=lambda dvi, delta: dvi._arg(delta + 1, signed=True), ulen1=lambda dvi, delta: dvi._arg(delta + 1, signed=False), olen1=lambda dvi, delta: dvi._arg(delta + 1, signed=delta == 3))
...
if __name__ == '__main__':
    ...
    with Dvi(args.filename, args.dpi) as dvi:
        ...
        for page in dvi:
            print(f'=== new page === (w: {page.width}, h: {page.height}, d: {page.descent})')
            for (font, group) in itertools.groupby(page.text, lambda text: text.font):
                print(f"font: {font.texname.decode('latin-1')!r}\tscale: {font._scale / 2 ** 20}")
                print('x', 'y', 'glyph', 'chr', 'w', '(glyphs)', sep='\t')
                for text in group:
                    print(text.x, text.y, text.glyph, chr(text.glyph) if chr(text.glyph).isprintable() else '.', text.width, sep='\t')
            if page.boxes:
