...
class TestCsrMatrix:
    ...
    def test_indices(self):
        ...
        testing.assert_array_equal(self.m.indices, cupy.array([0, 1, 3, 2], self.dtype))
    ...
    def test_indptr(self):
        ...
        testing.assert_array_equal(self.m.indptr, cupy.array([0, 2, 3, 4], self.dtype))
...
class TestCsrMatrixInit:
    ...
    def test_shape_none(self, xp, sp):
        if False:
            return 10
        x = sp.csr_matrix((self.data(xp), self.indices(xp), self.indptr(xp)), shape=None)
        assert x.shape == (3, 4)
    ...
    def test_dtype(self, xp, sp):
        ...
        x = sp.csr_matrix((data, self.indices(xp), self.indptr(xp)), dtype=self.dtype)
    ...
    def test_intlike_shape(self, xp, sp):
        if False:
            for i in range(10):
                print('nop')
        s = sp.csr_matrix((self.data(xp), self.indices(xp), self.indptr(xp)), shape=(xp.array(self.shape[0]), xp.int32(self.shape[1])))
    ...
    def test_shape_invalid(self):
        ...
        for (xp, sp) in ((numpy, scipy.sparse), (cupy, sparse)):
            with pytest.raises(ValueError):
                sp.csr_matrix((self.data(xp), self.indices(xp), self.indptr(xp)), shape=(2,))
    ...
    def test_data_invalid(self):
        ...
        for (xp, sp) in ((numpy, scipy.sparse), (cupy, sparse)):
            with pytest.raises(ValueError):
                sp.csr_matrix(('invalid', self.indices(xp), self.indptr(xp)), shape=self.shape)
    ...
    def test_data_invalid_ndim(self):
        ...
        for (xp, sp) in ((numpy, scipy.sparse), (cupy, sparse)):
            with pytest.raises(ValueError):
                sp.csr_matrix((self.data(xp)[None], self.indices(xp), self.indptr(xp)), shape=self.shape)
    ...
    def test_indices_invalid(self):
        ...
        for (xp, sp) in ((numpy, scipy.sparse), (cupy, sparse)):
            with pytest.raises(ValueError):
                sp.csr_matrix((self.data(xp), 'invalid', self.indptr(xp)), shape=self.shape)
    ...
    def test_indices_invalid_ndim(self):
        ...
        for (xp, sp) in ((numpy, scipy.sparse), (cupy, sparse)):
            with pytest.raises(ValueError):
                sp.csr_matrix((self.data(xp), self.indices(xp)[None], self.indptr(xp)), shape=self.shape)
    ...
    def test_indptr_invalid(self):
        ...
        for (xp, sp) in ((numpy, scipy.sparse), (cupy, sparse)):
            with pytest.raises(ValueError):
                sp.csr_matrix((self.data(xp), self.indices(xp), 'invalid'), shape=self.shape)
    ...
    def test_indptr_invalid_ndim(self):
        ...
        for (xp, sp) in ((numpy, scipy.sparse), (cupy, sparse)):
            with pytest.raises(ValueError):
                sp.csr_matrix((self.data(xp), self.indices(xp), self.indptr(xp)[None]), shape=self.shape)
    ...
    def test_data_indices_different_length(self):
        ...
        for (xp, sp) in ((numpy, scipy.sparse), (cupy, sparse)):
            ...
            with pytest.raises(ValueError):
                sp.csr_matrix((data, self.indices(xp), self.indptr(xp)), shape=self.shape)
    ...
    def test_indptr_invalid_length(self):
        ...
        for (xp, sp) in ((numpy, scipy.sparse), (cupy, sparse)):
            ...
            with pytest.raises(ValueError):
                sp.csr_matrix((self.data(xp), self.indices(xp), indptr), shape=self.shape)

    def test_unsupported_dtype(self):
        if False:
            print('Hello World!')
        with pytest.raises(ValueError):
            sparse.csr_matrix((self.data(cupy), self.indices(cupy), self.indptr(cupy)), shape=self.shape, dtype='i')
...
@testing.parameterize(*testing.product({'make_method': ['_make', '_make_unordered', '_make_empty', '_make_duplicate', '_make_shape'], 'dtype': [numpy.float32, numpy.float64, numpy.complex64, numpy.complex128]}))
@testing.with_requires('scipy')
class TestCsrMatrixScipyComparison:
    ...
    def test_dot_dense_matrix(self, xp, sp):
        if False:
            i = 10
            return i + 15
        if cupyx.cusparse.getVersion() == 11702 and self.make_method == '_make_duplicate':
    ...
    def test_has_canonical_format2(self, xp, sp):
        if False:
            for i in range(10):
                print('nop')
        M = sp.csr_matrix((xp.array([2], dtype=self.dtype), xp.array([0]), xp.array([0, 1])))
    ...
    @testing.with_requires('scipy>=1.11')
    @testing.numpy_cupy_allclose(sp_name='sp', rtol=2e-07)
...
@testing.parameterize(*testing.product({'axis': [None, -2, -1, 0, 1], 'dense': [False, True]}))
...
@testing.parameterize(*testing.product({'dtype': [numpy.float32, numpy.float64], 'ufunc': ['arcsin', 'arcsinh', 'arctan', 'arctanh', 'ceil', 'deg2rad', 'expm1', 'floor', 'log1p', 'rad2deg', 'rint', 'sign', 'sin', 'sinh', 'sqrt', 'tan', 'tanh', 'trunc']}))
@testing.with_requires('scipy')
class TestUfunc:
...
@testing.parameterize(*testing.product({'dtype': [numpy.float32, numpy.float64, cupy.complex64, cupy.complex128]}))
@testing.with_requires('scipy>=1.4.0')
...
@testing.parameterize(*testing.product({'make_method': ['_make', '_make_unordered', '_make_duplicate'], 'dtype': [numpy.float32, numpy.float64, cupy.complex64, cupy.complex128]}))
@testing.with_requires('scipy')
@pytest.mark.skipif(cupy.cuda.cub._get_cuda_build_version() >= 11000, reason='CUDA built-in CUB SpMV is buggy, see cupy/cupy#3822')
@pytest.mark.skipif(runtime.is_hip, reason='hipCUB does not provide spmv')
@pytest.mark.skipif(not cupy.cuda.cub.available, reason='The CUB routine is not enabled')
class TestCubSpmv:
...
@testing.parameterize(*testing.product({'a_dtype': ['float32', 'float64', 'complex64', 'complex128'], 'b_dtype': ['float32', 'float64', 'complex64', 'complex128'], 'shape': [(4, 25), (10, 10), (25, 4)], 'nz_rate': [0.1, 0.5], 'opt': ['maximum', 'minimum']}))
@testing.with_requires('scipy')
...
@testing.parameterize(*testing.product({'a_dtype': ['float32', 'float64', 'complex64', 'complex128'], 'b_dtype': ['float32', 'float64', 'complex64', 'complex128'], 'shape': [(6, 15), (15, 6)], 'opt': ['_eq_', '_ne_', '_lt_', '_gt_', '_le_', '_ge_']}))
...
@testing.parameterize(*testing.product({'shape': [(8, 5), (5, 5), (5, 8)]}))
