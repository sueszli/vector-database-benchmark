...
class DataportCz(SimpleDownloader):
    ...
    __version__ = '0.47'
    __status__ = 'testing'
    __pattern__ = 'http://(?:www\\.)?dataport\\.cz/file/(.+)'
    ...
    NAME_PATTERN = '<span itemprop="name">(?P<N>.+?)</span>'
    SIZE_PATTERN = '<td class="fil">Velikost</td>\\s*<td>(?P<S>[^<]+)</td>'
    OFFLINE_PATTERN = '<h2>Soubor nebyl nalezen</h2>'
    CAPTCHA_PATTERN = '<section id="captcha_bg">\\s*<img src="(.*?)"'
    FREE_SLOTS_PATTERN = 'Počet volných slotů: <span class="darkblue">(\\d+)</span><br />'
