...
class SimpleLSTMRNN(paddle.nn.Layer):
    ...
    def __init__(self, hidden_size, num_steps, num_layers=2, init_scale=0.1, dropout=None):
        ...
        for i in range(self._num_layers):
            weight_1 = self.create_parameter(attr=paddle.ParamAttr(initializer=paddle.nn.initializer.Uniform(low=-self._init_scale, high=self._init_scale)), shape=[self._hidden_size * 2, self._hidden_size * 4], dtype='float32', default_initializer=paddle.nn.initializer.Uniform(low=-self._init_scale, high=self._init_scale))
            self.weight_1_arr.append(self.add_parameter('w_%d' % i, weight_1))
            bias_1 = self.create_parameter(attr=paddle.ParamAttr(initializer=paddle.nn.initializer.Uniform(low=-self._init_scale, high=self._init_scale)), shape=[self._hidden_size * 4], dtype='float32', default_initializer=paddle.nn.initializer.Constant(0.0))
    ...
    def forward(self, input_embedding, init_hidden=None, init_cell=None):
        ...
        for index in range(self._num_steps):
            ...
            for k in range(self._num_layers):
                ...
                (i, j, f, o) = paddle.split(x=gate_input, num_or_sections=4, axis=-1)
                ...
                if self._dropout is not None and self._dropout > 0.0:
                    step_input = paddle.nn.functional.dropout(step_input, dropout_prob=self._dropout, dropout_implementation='upscale_in_train')
...
class PtbModel(paddle.nn.Layer):
    ...
    def __init__(self, hidden_size, vocab_size, num_layers=2, num_steps=20, init_scale=0.1, dropout=None):
        ...
        self.embedding = paddle.nn.Embedding(vocab_size, hidden_size, sparse=False, weight_attr=paddle.ParamAttr(name='embedding_para', initializer=paddle.nn.initializer.Uniform(low=-init_scale, high=init_scale)))
        self.softmax_weight = self.create_parameter(attr=paddle.ParamAttr(), shape=[self.hidden_size, self.vocab_size], dtype='float32', default_initializer=paddle.nn.initializer.Uniform(low=-self.init_scale, high=self.init_scale))
        self.softmax_bias = self.create_parameter(attr=paddle.ParamAttr(), shape=[self.vocab_size], dtype='float32', default_initializer=paddle.nn.initializer.Uniform(low=-self.init_scale, high=self.init_scale))
    ...
    def forward(self, input, label, init_hidden, init_cell):
        ...
        if self.dropout is not None and self.dropout > 0.0:
            x_emb = paddle.nn.functional.dropout(x_emb, dropout_prob=self.dropout, dropout_implementation='upscale_in_train')
...
def train(place):
    ...
    for epoch_id in range(max_epoch):
        ...
        init_cell = paddle.to_tensor(data=init_cell_data, dtype=None, place=None, stop_gradient=True)
