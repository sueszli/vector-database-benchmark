...
from scapy.fields import BitEnumField, BitField, BitMultiEnumField, ByteEnumField, ByteField, ConditionalField, FCSField, FieldLenField, FieldListField, FlagsField, IntField, LEFieldLenField, LEIntField, LELongField, LEShortEnumField, LEShortField, LESignedIntField, MayEnd, MultipleTypeField, OUIField, PacketField, PacketListField, ReversePadField, ScalingField, ShortField, StrField, StrFixedLenField, StrLenField, XByteField, XStrFixedLenField
...
_rt_present = ['TSFT', 'Flags', 'Rate', 'Channel', 'FHSS', 'dBm_AntSignal', 'dBm_AntNoise', 'Lock_Quality', 'TX_Attenuation', 'dB_TX_Attenuation', 'dBm_TX_Power', 'Antenna', 'dB_AntSignal', 'dB_AntNoise', 'RXFlags', 'TXFlags', 'b17', 'b18', 'ChannelPlus', 'MCS', 'A_MPDU', 'VHT', 'timestamp', 'HE', 'HE_MU', 'HE_MU_other_user', 'zero_length_psdu', 'L_SIG', 'TLV', 'RadiotapNS', 'VendorNS', 'Ext']
...
_rt_channelflags2 = ['res1', 'res2', 'res3', 'res4', 'Turbo', 'CCK', 'OFDM', '2GHz', '5GHz', 'Passive', 'Dynamic_CCK_OFDM', 'GFSK', 'GSM', 'StaticTurbo', '10MHz', '5MHz', '20MHz', '40MHz_ext_channel_above', '40MHz_ext_channel_below', 'res5', 'res6', 'res7', 'res8', 'res9']
...
_rt_vhtbandwidth = {0: '20MHz', 1: '40MHz', 2: '40MHz', 3: '40MHz', 4: '80MHz', 5: '80MHz', 6: '80MHz', 7: '80MHz', 8: '80MHz', 9: '80MHz', 10: '80MHz', 11: '160MHz', 12: '160MHz', 13: '160MHz', 14: '160MHz', 15: '160MHz', 16: '160MHz', 17: '160MHz', 18: '160MHz', 19: '160MHz', 20: '160MHz', 21: '160MHz', 22: '160MHz', 23: '160MHz', 24: '160MHz', 25: '160MHz'}
...
_rt_hemuother_per_user_known = ['user field position', 'STA-ID', 'NSTS', 'Tx Beamforming', 'Spatial Configuration', 'MCS', 'DCM', 'Coding']
...
class RadioTapTLV(Packet):
    fields_desc = [LEShortEnumField('type', 0, _rt_present), LEShortField('length', None), ConditionalField(OUIField('oui', 0), lambda pkt: pkt.type == 30), ConditionalField(ByteField('subtype', 0), lambda pkt: pkt.type == 30), ConditionalField(LEShortField('presence_type', 0), lambda pkt: pkt.type == 30), ConditionalField(LEShortField('reserved', 0), lambda pkt: pkt.type == 30), StrLenField('data', b'', length_from=lambda pkt: pkt.length), StrLenField('pad', None, length_from=lambda pkt: -pkt.length % 4)]
...
class RadioTap(Packet):
    ...
    fields_desc = [ByteField('version', 0), ByteField('pad', 0), LEShortField('len', None), FlagsField('present', None, -32, _rt_present), ConditionalField(PacketListField('Ext', [], next_cls_cb=_next_radiotap_extpm), lambda pkt: pkt.present and pkt.present.Ext), ConditionalField(ReversePadField(LELongField('mac_timestamp', 0), 8), lambda pkt: pkt.present and pkt.present.TSFT), ConditionalField(FlagsField('Flags', None, -8, _rt_flags), lambda pkt: pkt.present and pkt.present.Flags), ConditionalField(ScalingField('Rate', 0, scaling=0.5, unit='Mbps', fmt='B'), lambda pkt: pkt.present and pkt.present.Rate), ConditionalField(ReversePadField(LEShortField('ChannelFrequency', 0), 2), lambda pkt: pkt.present and pkt.present.Channel), ConditionalField(FlagsField('ChannelFlags', None, -16, _rt_channelflags), lambda pkt: pkt.present and pkt.present.Channel), ConditionalField(ScalingField('dBm_AntSignal', 0, unit='dBm', fmt='b'), lambda pkt: pkt.present and pkt.present.dBm_AntSignal), ConditionalField(ScalingField('dBm_AntNoise', 0, unit='dBm', fmt='b'), lambda pkt: pkt.present and pkt.present.dBm_AntNoise), ConditionalField(ReversePadField(LEShortField('Lock_Quality', 0), 2), lambda pkt: pkt.present and pkt.present.Lock_Quality), ConditionalField(ByteField('Antenna', 0), lambda pkt: pkt.present and pkt.present.Antenna), ConditionalField(ReversePadField(FlagsField('RXFlags', None, -16, _rt_rxflags), 2), lambda pkt: pkt.present and pkt.present.RXFlags), ConditionalField(ReversePadField(FlagsField('TXFlags', None, -16, _rt_txflags), 2), lambda pkt: pkt.present and pkt.present.TXFlags), ConditionalField(ReversePadField(FlagsField('ChannelPlusFlags', None, -32, _rt_channelflags2), 4), lambda pkt: pkt.present and pkt.present.ChannelPlus), ConditionalField(LEShortField('ChannelPlusFrequency', 0), lambda pkt: pkt.present and pkt.present.ChannelPlus), ConditionalField(ByteField('ChannelPlusNumber', 0), lambda pkt: pkt.present and pkt.present.ChannelPlus), ConditionalField(ReversePadField(FlagsField('knownMCS', None, -8, _rt_knownmcs), 2), lambda pkt: pkt.present and pkt.present.MCS), ConditionalField(BitField('Ness_LSB', 0, 1), lambda pkt: pkt.present and pkt.present.MCS), ConditionalField(BitField('STBC_streams', 0, 2), lambda pkt: pkt.present and pkt.present.MCS), ConditionalField(BitEnumField('FEC_type', 0, 1, {0: 'BCC', 1: 'LDPC'}), lambda pkt: pkt.present and pkt.present.MCS), ConditionalField(BitEnumField('HT_format', 0, 1, {0: 'mixed', 1: 'greenfield'}), lambda pkt: pkt.present and pkt.present.MCS), ConditionalField(BitEnumField('guard_interval', 0, 1, {0: 'Long_GI', 1: 'Short_GI'}), lambda pkt: pkt.present and pkt.present.MCS), ConditionalField(BitEnumField('MCS_bandwidth', 0, 2, _rt_bandwidth), lambda pkt: pkt.present and pkt.present.MCS), ConditionalField(ByteField('MCS_index', 0), lambda pkt: pkt.present and pkt.present.MCS), ConditionalField(ReversePadField(LEIntField('A_MPDU_ref', 0), 4), lambda pkt: pkt.present and pkt.present.A_MPDU), ConditionalField(FlagsField('A_MPDU_flags', None, -32, _rt_a_mpdu_flags), lambda pkt: pkt.present and pkt.present.A_MPDU), ConditionalField(ReversePadField(FlagsField('KnownVHT', None, -16, _rt_knownvht), 2), lambda pkt: pkt.present and pkt.present.VHT), ConditionalField(FlagsField('PresentVHT', None, -8, _rt_presentvht), lambda pkt: pkt.present and pkt.present.VHT), ConditionalField(ByteEnumField('VHT_bandwidth', 0, _rt_vhtbandwidth), lambda pkt: pkt.present and pkt.present.VHT), ConditionalField(StrFixedLenField('mcs_nss', 0, length=5), lambda pkt: pkt.present and pkt.present.VHT), ConditionalField(ByteField('GroupID', 0), lambda pkt: pkt.present and pkt.present.VHT), ConditionalField(ShortField('PartialAID', 0), lambda pkt: pkt.present and pkt.present.VHT), ConditionalField(ReversePadField(LELongField('timestamp', 0), 8), lambda pkt: pkt.present and pkt.present.timestamp), ConditionalField(LEShortField('ts_accuracy', 0), lambda pkt: pkt.present and pkt.present.timestamp), ConditionalField(BitEnumField('ts_unit', 0, 4, {0: 'milliseconds', 1: 'microseconds', 2: 'nanoseconds'}), lambda pkt: pkt.present and pkt.present.timestamp), ConditionalField(BitField('ts_position', 0, 4), lambda pkt: pkt.present and pkt.present.timestamp), ConditionalField(FlagsField('ts_flags', None, 8, _rt_tsflags), lambda pkt: pkt.present and pkt.present.timestamp), ConditionalField(ReversePadField(LEShortField('he_data1', 0), 2), lambda pkt: pkt.present and pkt.present.HE), ConditionalField(LEShortField('he_data2', 0), lambda pkt: pkt.present and pkt.present.HE), ConditionalField(LEShortField('he_data3', 0), lambda pkt: pkt.present and pkt.present.HE), ConditionalField(LEShortField('he_data4', 0), lambda pkt: pkt.present and pkt.present.HE), ConditionalField(LEShortField('he_data5', 0), lambda pkt: pkt.present and pkt.present.HE), ConditionalField(LEShortField('he_data6', 0), lambda pkt: pkt.present and pkt.present.HE), ConditionalField(ReversePadField(LEShortField('hemu_flags1', 0), 2), lambda pkt: pkt.present and pkt.present.HE_MU), ConditionalField(LEShortField('hemu_flags2', 0), lambda pkt: pkt.present and pkt.present.HE_MU), ConditionalField(FieldListField('RU_channel1', [], ByteField('', 0), length_from=lambda x: 4), lambda pkt: pkt.present and pkt.present.HE_MU), ConditionalField(FieldListField('RU_channel2', [], ByteField('', 0), length_from=lambda x: 4), lambda pkt: pkt.present and pkt.present.HE_MU), ConditionalField(ReversePadField(LEShortField('hemuou_per_user_1', 32767), 2), lambda pkt: pkt.present and pkt.present.HE_MU_other_user), ConditionalField(LEShortField('hemuou_per_user_2', 63), lambda pkt: pkt.present and pkt.present.HE_MU_other_user), ConditionalField(ByteField('hemuou_per_user_position', 0), lambda pkt: pkt.present and pkt.present.HE_MU_other_user), ConditionalField(FlagsField('hemuou_per_user_known', 0, -16, _rt_hemuother_per_user_known), lambda pkt: pkt.present and pkt.present.HE_MU_other_user), ConditionalField(ReversePadField(FlagsField('lsig_data1', 0, -16, ['rate', 'length']), 2), lambda pkt: pkt.present and pkt.present.L_SIG), ConditionalField(BitField('lsig_length', 0, 12, tot_size=-2), lambda pkt: pkt.present and pkt.present.L_SIG), ConditionalField(BitField('lsig_rate', 0, 4, end_tot_size=-2), lambda pkt: pkt.present and pkt.present.L_SIG), ConditionalField(ReversePadField(PacketListField('tlvs', [], RadioTapTLV), 4), lambda pkt: pkt.present and pkt.present.TLV), StrLenField('notdecoded', '', length_from=lambda pkt: 0)]
    ...
    def post_build(self, p, pay):
        ...
        return p + pay
_dot11_subtypes = {0: {0: 'Association Request', 1: 'Association Response', 2: 'Reassociation Request', 3: 'Reassociation Response', 4: 'Probe Request', 5: 'Probe Response', 6: 'Timing Advertisement', 8: 'Beacon', 9: 'ATIM', 10: 'Disassociation', 11: 'Authentication', 12: 'Deauthentication', 13: 'Action', 14: 'Action No Ack'}, 1: {2: 'Trigger', 3: 'TACK', 4: 'Beamforming Report Poll', 5: 'VHT/HE NDP Announcement', 6: 'Control Frame Extension', 7: 'Control Wrapper', 8: 'Block Ack Request', 9: 'Block Ack', 10: 'PS-Poll', 11: 'RTS', 12: 'CTS', 13: 'Ack', 14: 'CF-End', 15: 'CF-End+CF-Ack'}, 2: {0: 'Data', 1: 'Data+CF-Ack', 2: 'Data+CF-Poll', 3: 'Data+CF-Ack+CF-Poll', 4: 'Null (no data)', 5: 'CF-Ack (no data)', 6: 'CF-Poll (no data)', 7: 'CF-Ack+CF-Poll (no data)', 8: 'QoS Data', 9: 'QoS Data+CF-Ack', 10: 'QoS Data+CF-Poll', 11: 'QoS Data+CF-Ack+CF-Poll', 12: 'QoS Null (no data)', 14: 'QoS CF-Poll (no data)', 15: 'QoS CF-Ack+CF-Poll (no data)'}, 3: {0: 'DMG Beacon', 1: 'S1G Beacon'}}
_dot11_cfe = {2: 'Poll', 3: 'SPR', 4: 'Grant', 5: 'DMG CTS', 6: 'DMG DTS', 7: 'Grant Ack', 8: 'SSW', 9: 'SSW-Feedback', 10: 'SSW-Ack'}
_dot11_addr_meaning = [['RA=DA', 'TA=SA', 'BSSID/STA', None], ['RA', 'TA', None, None], [[['RA=DA', 'RA=DA'], ['RA=BSSID', 'RA']], [['TA=SA', 'TA=BSSID'], ['TA=SA', 'TA']], [['BSSID', 'SA'], ['DA', 'DA']], [[None, None], ['SA', 'BSSID']]], ['BSSID', None, None, None]]
...
class Dot11(Packet):
    name = '802.11'
    fields_desc = [BitMultiEnumField('subtype', 0, 4, _dot11_subtypes, lambda pkt: pkt.type), BitEnumField('type', 0, 2, ['Management', 'Control', 'Data', 'Extension']), BitField('proto', 0, 2), ConditionalField(BitEnumField('cfe', 0, 4, _dot11_cfe), lambda pkt: (pkt.type, pkt.subtype) == (1, 6)), MultipleTypeField([(FlagsField('FCfield', 0, 4, ['pw-mgt', 'MD', 'protected', 'order']), lambda pkt: (pkt.type, pkt.subtype) == (1, 6))], FlagsField('FCfield', 0, 8, ['to-DS', 'from-DS', 'MF', 'retry', 'pw-mgt', 'MD', 'protected', 'order'])), ShortField('ID', 0), _Dot11MacField('addr1', ETHER_ANY, 1), ConditionalField(_Dot11MacField('addr2', ETHER_ANY, 2), lambda pkt: pkt.type != 1 or pkt.subtype in [4, 5, 6, 8, 9, 10, 11, 14, 15]), ConditionalField(_Dot11MacField('addr3', ETHER_ANY, 3), lambda pkt: pkt.type in [0, 2] or ((pkt.type, pkt.subtype) == (1, 6) and pkt.cfe == 6)), ConditionalField(LEShortField('SC', 0), lambda pkt: pkt.type != 1), ConditionalField(_Dot11MacField('addr4', ETHER_ANY, 4), lambda pkt: pkt.type == 2 and pkt.FCfield & 3 == 3)]
...
class _Dot11EltUtils(Packet):
    ...
    def network_stats(self):
        ...
        return summary
_dot11_info_elts_ids = {0: 'SSID', 1: 'Supported Rates', 2: 'FHset', 3: 'DSSS Set', 4: 'CF Set', 5: 'TIM', 6: 'IBSS Set', 7: 'Country', 10: 'Request', 11: 'BSS Load', 12: 'EDCA Set', 13: 'TSPEC', 14: 'TCLAS', 15: 'Schedule', 16: 'Challenge text', 32: 'Power Constraint', 33: 'Power Capability', 36: 'Supported Channels', 37: 'Channel Switch Announcement', 42: 'ERP', 45: 'HT Capabilities', 46: 'QoS Capability', 48: 'RSN', 50: 'Extended Supported Rates', 52: 'Neighbor Report', 61: 'HT Operation', 74: 'Overlapping BSS Scan Parameters', 107: 'Interworking', 127: 'Extendend Capabilities', 191: 'VHT Capabilities', 192: 'VHT Operation', 221: 'Vendor Specific'}
_dot11_elt_deprecated_names = {'Rates': 1, 'DSset': 3, 'CFset': 4, 'IBSSset': 6, 'challenge': 16, 'PowerCapability': 33, 'Channels': 36, 'ERPinfo': 42, 'HTinfo': 45, 'RSNinfo': 48, 'ESRates': 50, 'ExtendendCapatibilities': 127, 'VHTCapabilities': 191, 'Vendor': 221}
...
class Dot11EltDSSSet(Dot11Elt):
    ...
    match_subclass = True
    fields_desc = [ByteEnumField('ID', 3, _dot11_id_enum), ByteField('len', 1), ByteField('channel', 0)]

class Dot11EltERP(Dot11Elt):
    name = '802.11 ERP'
    match_subclass = True
    fields_desc = [ByteEnumField('ID', 42, _dot11_id_enum), ByteField('len', 1), BitField('NonERP_Present', 0, 1), BitField('Use_Protection', 0, 1), BitField('Barker_Preamble_Mode', 0, 1), BitField('res', 0, 5)]

class RSNCipherSuite(Packet):
    name = 'Cipher suite'
    fields_desc = [OUIField('oui', 4012), ByteEnumField('cipher', 4, {0: 'Use group cipher suite', 1: 'WEP-40', 2: 'TKIP', 3: 'OCB', 4: 'CCMP-128', 5: 'WEP-104', 6: 'BIP-CMAC-128', 7: 'Group addressed traffic not allowed', 8: 'GCMP-128', 9: 'GCMP-256', 10: 'CCMP-256', 11: 'BIP-GMAC-128', 12: 'BIP-GMAC-256', 13: 'BIP-CMAC-256'})]
...
class AKMSuite(Packet):
    name = 'AKM suite'
    fields_desc = [OUIField('oui', 4012), ByteEnumField('suite', 1, {0: 'Reserved', 1: '802.1X', 2: 'PSK', 3: 'FT-802.1X', 4: 'FT-PSK', 5: 'WPA-SHA256', 6: 'PSK-SHA256', 7: 'TDLS', 8: 'SAE', 9: 'FT-SAE', 10: 'AP-PEER-KEY', 11: 'WPA-SHA256-SUITE-B', 12: 'WPA-SHA384-SUITE-B', 13: 'FT-802.1X-SHA384', 14: 'FILS-SHA256', 15: 'FILS-SHA384', 16: 'FT-FILS-SHA256', 17: 'FT-FILS-SHA384', 18: 'OWE'})]
...
class Dot11EltCountry(Dot11Elt):
    name = '802.11 Country'
    match_subclass = True
    fields_desc = [ByteEnumField('ID', 7, _dot11_id_enum), ByteField('len', None), StrFixedLenField('country_string', b'\x00\x00\x00', length=3), MayEnd(PacketListField('descriptors', [], Dot11EltCountryConstraintTriplet, length_from=lambda pkt: pkt.len - 3 - pkt.len % 3)), ConditionalField(ByteField('pad', 0), lambda pkt: (len(pkt.descriptors) + 1) % 2 if pkt.len is None else pkt.len % 3)]
...
class Dot11EltMicrosoftWPA(Dot11EltVendorSpecific):
    ...
    ID = 221
    oui = 20722
    fields_desc = Dot11EltVendorSpecific.fields_desc[:3] + [XByteField('type', 1)] + Dot11EltRSN.fields_desc[2:8]

class Dot11EltCSA(Dot11Elt):
    name = '802.11 CSA Element'
    fields_desc = [ByteEnumField('ID', 37, _dot11_id_enum), ByteField('len', 3), ByteField('mode', 0), ByteField('new_channel', 0), ByteField('channel_switch_count', 0)]

class Dot11EltOBSS(Dot11Elt):
    name = '802.11 OBSS Scan Parameters Element'
    fields_desc = [ByteEnumField('ID', 74, _dot11_id_enum), ByteField('len', 14), LEShortField('Passive_Dwell', 0), LEShortField('Active_Dwell', 0), LEShortField('Scan_Interval', 0), LEShortField('Passive_Total_Per_Channel', 0), LEShortField('Active_Total_Per_Channel', 0), LEShortField('Delay', 0), LEShortField('Activity_Threshold', 0)]

class Dot11VHTOperationInfo(Packet):
    name = '802.11 VHT Operation Information'
    fields_desc = [ByteField('channel_width', 0), ByteField('channel_center0', 36), ByteField('channel_center1', 0)]
...
class Dot11EltVHTOperation(Dot11Elt):
    ...
    fields_desc = [ByteEnumField('ID', 192, _dot11_id_enum), ByteField('len', 5), PacketField('VHT_Operation_Info', Dot11VHTOperationInfo(), Dot11VHTOperationInfo), FieldListField('mcs_set', [0], BitField('SS', 0, size=2), count_from=lambda x: 8)]

class Dot11Beacon(_Dot11EltUtils):
    name = '802.11 Beacon'
    fields_desc = [LELongField('timestamp', 0), LEShortField('beacon_interval', 100), FlagsField('cap', 0, 16, capability_list)]
...
class Dot11AssoReq(_Dot11EltUtils):
    ...
    fields_desc = [FlagsField('cap', 0, 16, capability_list), LEShortField('listen_interval', 200)]
...
class Dot11ReassoReq(_Dot11EltUtils):
    ...
    fields_desc = [FlagsField('cap', 0, 16, capability_list), LEShortField('listen_interval', 200), MACField('current_AP', ETHER_ANY)]
...
class Dot11Action(Packet):
    name = '802.11 Action'
    fields_desc = [ByteEnumField('category', 0, {0: 'Spectrum Management', 1: 'QoS', 2: 'DLS', 3: 'Block', 4: 'Public', 5: 'Radio Measurement', 6: 'Fast BSS Transition', 7: 'HT', 8: 'SA Query', 9: 'Protected Dual of Public Action', 10: 'WNM', 11: 'Unprotected WNM', 12: 'TDLS', 13: 'Mesh', 14: 'Multihop', 15: 'Self-protected', 16: 'DMG', 17: 'Reserved Wi-Fi Alliance', 18: 'Fast Session Transfer', 19: 'Robust AV Streaming', 20: 'Unprotected DMG', 21: 'VHT'})]

class Dot11WNM(Packet):
    name = '802.11 WNM Action'
    fields_desc = [ByteEnumField('action', 0, {0: 'Event Request', 1: 'Event Report', 2: 'Diagnostic Request', 3: 'Diagnostic Report', 4: 'Location Configuration Request', 5: 'Location Configuration Response', 6: 'BSS Transition Management Query', 7: 'BSS Transition Management Request', 8: 'BSS Transition Management Response', 9: 'FMS Request', 10: 'FMS Response', 11: 'Collocated Interference Request', 12: 'Collocated Interference Report', 13: 'TFS Request', 14: 'TFS Response', 15: 'TFS Notify', 16: 'WNM Sleep Mode Request', 17: 'WNM Sleep Mode Response', 18: 'TIM Broadcast Request', 19: 'TIM Broadcast Response', 20: 'QoS Traffic Capability Update', 21: 'Channel Usage Request', 22: 'Channel Usage Response', 23: 'DMS Request', 24: 'DMS Response', 25: 'Timing Measurement Request', 26: 'WNM Notification Request', 27: 'WNM Notification Response', 28: 'WNM-Notify Response'})]

class SubelemTLV(Packet):
    fields_desc = [ByteField('type', 0), LEFieldLenField('len', None, fmt='B', length_of='value'), FieldListField('value', [], ByteField('', 0), length_from=lambda p: p.len)]

class BSSTerminationDuration(Packet):
    name = 'BSS Termination Duration'
    fields_desc = [ByteField('id', 4), ByteField('len', 10), LELongField('TSF', 0), LEShortField('duration', 0)]
...
class NeighborReport(Packet):
    ...
    fields_desc = [ByteField('type', 0), ByteField('len', 13), MACField('BSSID', ETHER_ANY), BitField('AP_reach', 0, 2, tot_size=-4), BitField('security', 0, 1), BitField('key_scope', 0, 1), BitField('capabilities', 0, 6), BitField('mobility', 0, 1), BitField('HT', 0, 1), BitField('VHT', 0, 1), BitField('FTM', 0, 1), BitField('reserved', 0, 18, end_tot_size=-4), ByteField('op_class', 0), ByteField('channel', 0), ByteField('phy_type', 0), ConditionalField(PacketListField('subelems', SubelemTLV(), SubelemTLV, length_from=lambda p: p.len - 13), lambda p: p.len > 13)]
btm_request_mode = ['Preferred_Candidate_List_Included', 'Abridged', 'Disassociation_Imminent', 'BSS_Termination_Included', 'ESS_Disassociation_Imminent']

class Dot11BSSTMRequest(Packet):
    name = 'BSS Transition Management Request'
    fields_desc = [ByteField('token', 0), FlagsField('mode', 0, 8, btm_request_mode), LEShortField('disassociation_timer', 0), ByteField('validity_interval', 0), ConditionalField(PacketField('termination_duration', BSSTerminationDuration(), BSSTerminationDuration), lambda p: p.mode and p.mode.BSS_Termination_Included), ConditionalField(ByteField('url_len', 0), lambda p: p.mode and p.mode.ESS_Disassociation_Imminent), ConditionalField(StrLenField('url', '', length_from=lambda p: p.url_len), lambda p: p.mode and p.mode.ESS_Disassociation_Imminent != 0), ConditionalField(PacketListField('neighbor_report', NeighborReport(), NeighborReport), lambda p: p.mode and p.mode.Preferred_Candidate_List_Included)]
btm_status_code = ['Accept', 'Reject-Unspecified_reject_reason', 'Reject-Insufficient_Beacon_or_Probe_Response_frames', 'Reject-Insufficient_available_capacity_from_all_candidates', 'Reject-BSS_termination_undesired', 'Reject-BSS_termination_delay_requested', 'Reject-STA_BSS_Transition_Candidate_List_provided', 'Reject-No_suitable_BSS_transition_candidates', 'Reject-Leaving_ESS']

class Dot11BSSTMResponse(Packet):
    name = 'BSS Transition Management Response'
    fields_desc = [ByteField('token', 0), ByteEnumField('status', 0, btm_status_code), ByteField('termination_delay', 0), ConditionalField(MACField('target', ETHER_ANY), lambda p: p.status == 0), ConditionalField(PacketListField('neighbor_report', NeighborReport(), NeighborReport), lambda p: p.status == 6)]

class Dot11SpectrumManagement(Packet):
    name = '802.11 Spectrum Management Action'
    fields_desc = [ByteEnumField('action', 0, {0: 'Measurement Request', 1: 'Measurement Report', 2: 'TPC Request', 3: 'TPC Report', 4: 'Channel Switch Announcement'})]

class Dot11CSA(Packet):
    name = 'Channel Switch Announcement Frame'
    fields_desc = [PacketField('CSA', Dot11EltCSA(), Dot11EltCSA)]
...
class Dot11WEP(Dot11Encrypted):
    ...
    fields_desc = [StrFixedLenField('iv', b'\x00\x00\x00', 3), ByteField('keyid', 0), StrField('wepdata', None, remain=4), IntField('icv', None)]
...
class Dot11TKIP(Dot11Encrypted):
    ...
    fields_desc = [ByteField('TSC1', 0), ByteField('WEPSeed', 0), ByteField('TSC0', 0), BitField('key_id', 0, 2), BitField('ext_iv', 0, 1), BitField('res', 0, 5), ConditionalField(ByteField('TSC2', 0), lambda pkt: pkt.ext_iv), ConditionalField(ByteField('TSC3', 0), lambda pkt: pkt.ext_iv), ConditionalField(ByteField('TSC4', 0), lambda pkt: pkt.ext_iv), ConditionalField(ByteField('TSC5', 0), lambda pkt: pkt.ext_iv), StrField('data', None)]

class Dot11CCMP(Dot11Encrypted):
    name = '802.11 CCMP packet'
    fields_desc = [ByteField('PN0', 0), ByteField('PN1', 0), ByteField('res0', 0), BitField('key_id', 0, 2), BitField('ext_iv', 0, 1), BitField('res1', 0, 5), ByteField('PN2', 0), ByteField('PN3', 0), ByteField('PN4', 0), ByteField('PN5', 0), StrField('data', None)]
bind_top_down(RadioTap, Dot11FCS, present=2, Flags=16)
bind_top_down(Dot11, Dot11QoS, type=2, subtype=12)
...
bind_layers(Dot11Action, Dot11SpectrumManagement, category=0)
bind_layers(Dot11SpectrumManagement, Dot11CSA, action=4)
bind_layers(Dot11Action, Dot11WNM, category=10)
