...
class TestConvGeluMkldnnFusePass(PassAutoScanTest):
    ...
    def sample_program_config(self, draw):
        ...
        ops_config = [{'op_type': 'conv2d', 'op_inputs': {'Input': ['input_data'], 'Filter': ['input_weight']}, 'op_outputs': {'Output': ['conv_output']}, 'op_attrs': {'data_format': data_format, 'dilations': dilations, 'padding_algorithm': padding_algorithm, 'groups': groups, 'paddings': paddings, 'strides': strides}}, {'op_type': 'gelu', 'op_inputs': {'X': ['conv_output']}, 'op_outputs': {'Out': ['gelu_output']}, 'op_attrs': {'approximate': approximate}}]
        ops = self.generate_op_config(ops_config)
        program_config = ProgramConfig(ops=ops, weights={'input_weight': TensorConfig(data_gen=partial(generate_weight))}, inputs={'input_data': TensorConfig(data_gen=partial(generate_input))}, outputs=['gelu_output'])
