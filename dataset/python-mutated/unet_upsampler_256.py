...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if mask is not None:
            ...
            mask = torch.cat((pad_mask, mask.unsqueeze(1).unsqueeze(1)), dim=-1)
...
class SuperResUNet256(nn.Module):
    ...
    def __init__(self, in_dim=6, out_dim=3, dim=256, text_dim=1024, context_dim=512, dim_mult=[1, 2, 2, 3, 4], num_heads=None, head_dim=64, num_res_blocks=2, attn_scales=[1 / 16], resblock_resample=True, use_conv=True, use_scale_shift_norm=True, dropout=0.1):
        if False:
            return 10
        embed_dim = dim * 4
        ...
        self.noise_time_embedding = nn.Sequential(nn.Linear(dim, embed_dim), nn.SiLU(), nn.Linear(embed_dim, embed_dim))
        self.pool_embedding = nn.Sequential(nn.LayerNorm(text_dim), nn.Linear(text_dim, embed_dim))
        self.text_embedding = nn.Sequential(nn.LayerNorm(text_dim), nn.Linear(text_dim, context_dim), nn.SiLU(), nn.Linear(context_dim, context_dim))
        ...
        self.middle = nn.ModuleList([ResidualBlock(out_dim, embed_dim, out_dim, use_scale_shift_norm, 1.0, dropout), AttentionBlock(out_dim, context_dim, num_heads, head_dim), ResidualBlock(out_dim, embed_dim, out_dim, use_scale_shift_norm, 1.0, dropout)])
    ...
    def forward(self, x, t, lx, lt, y, context, mask):
        ...
        t = self.time_embedding(sinusoidal_embedding(t, self.dim)) + self.noise_time_embedding(sinusoidal_embedding(lt, self.dim)) + self.pool_embedding(y)
        context = self.text_embedding(context)
        if lx.shape[-2:] != x.shape[-2:]:
            lx = F.interpolate(lx, x.shape[-2:], mode='bilinear', align_corners=False)
