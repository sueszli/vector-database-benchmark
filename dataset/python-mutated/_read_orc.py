...
from awswrangler.s3._read import _apply_partition_filter, _check_version_id, _extract_partitions_dtypes_from_table_details, _get_path_ignore_suffix, _get_path_root, _get_paths_for_glue_table, _TableMetadataReader
...
def _read_orc_metadata_file(s3_client: Optional['S3Client'], path: str, s3_additional_kwargs: Optional[Dict[str, str]], use_threads: Union[bool, int], version_id: Optional[str]=None) -> pa.schema:
    if False:
        while True:
            i = 10
...
class _ORCTableMetadataReader(_TableMetadataReader):
    ...
    def _read_metadata_file(self, s3_client: Optional['S3Client'], path: str, s3_additional_kwargs: Optional[Dict[str, str]], use_threads: Union[bool, int], version_id: Optional[str]=None, coerce_int96_timestamp_unit: Optional[str]=None) -> pa.schema:
        if False:
            for i in range(10):
                print('nop')
        return _read_orc_metadata_file(s3_client=s3_client, path=path, s3_additional_kwargs=s3_additional_kwargs, use_threads=use_threads, version_id=version_id)

def _read_orc_file(s3_client: Optional['S3Client'], path: str, path_root: Optional[str], columns: Optional[List[str]], s3_additional_kwargs: Optional[Dict[str, str]], use_threads: Union[bool, int], version_id: Optional[str]=None) -> pa.Table:
    if False:
        print('Hello World!')
    ...
    with open_s3_object(path=path, mode='rb', version_id=version_id, use_threads=use_threads, s3_block_size=s3_block_size, s3_additional_kwargs=s3_additional_kwargs, s3_client=s3_client) as f:
        ...
        return _add_table_partitions(table=orc_file.read(columns=columns), path=path, path_root=path_root)

@engine.dispatch_on_engine
def _read_orc(paths: List[str], path_root: Optional[str], schema: Optional[pa.schema], columns: Optional[List[str]], use_threads: Union[bool, int], parallelism: int, version_ids: Optional[Dict[str, str]], s3_client: Optional['S3Client'], s3_additional_kwargs: Optional[Dict[str, Any]], arrow_kwargs: Dict[str, Any]) -> pd.DataFrame:
    if False:
        while True:
            i = 10
    executor: _BaseExecutor = _get_executor(use_threads=use_threads)
    tables = executor.map(_read_orc_file, s3_client, paths, itertools.repeat(path_root), itertools.repeat(columns), itertools.repeat(s3_additional_kwargs), itertools.repeat(use_threads), [version_ids.get(p) if isinstance(version_ids, dict) else None for p in paths])
...
@apply_configs
def read_orc(path: Union[str, List[str]], path_root: Optional[str]=None, dataset: bool=False, path_suffix: Union[str, List[str], None]=None, path_ignore_suffix: Union[str, List[str], None]=None, ignore_empty: bool=True, partition_filter: Optional[Callable[[Dict[str, str]], bool]]=None, columns: Optional[List[str]]=None, validate_schema: bool=False, last_modified_begin: Optional[datetime.datetime]=None, last_modified_end: Optional[datetime.datetime]=None, version_id: Optional[Union[str, Dict[str, str]]]=None, dtype_backend: Literal['numpy_nullable', 'pyarrow']='numpy_nullable', use_threads: Union[bool, int]=True, ray_args: Optional[RaySettings]=None, boto3_session: Optional[boto3.Session]=None, s3_additional_kwargs: Optional[Dict[str, Any]]=None, pyarrow_additional_kwargs: Optional[Dict[str, Any]]=None) -> pd.DataFrame:
    if False:
        return 10
    ...
    if validate_schema:
        ...
        schema = metadata_reader.validate_schemas(paths=paths, path_root=path_root, columns=columns, validate_schema=validate_schema, s3_client=s3_client, version_ids=version_ids, use_threads=use_threads, s3_additional_kwargs=s3_additional_kwargs)
    ...
    return _read_orc(paths, path_root=path_root, schema=schema, columns=columns, use_threads=use_threads, parallelism=ray_args.get('parallelism', -1), s3_client=s3_client, s3_additional_kwargs=s3_additional_kwargs, arrow_kwargs=arrow_kwargs, version_ids=version_ids)

@_utils.validate_distributed_kwargs(unsupported_kwargs=['boto3_session', 's3_additional_kwargs', 'dtype_backend'])
@apply_configs
def read_orc_table(table: str, database: str, filename_suffix: Union[str, List[str], None]=None, filename_ignore_suffix: Union[str, List[str], None]=None, catalog_id: Optional[str]=None, partition_filter: Optional[Callable[[Dict[str, str]], bool]]=None, columns: Optional[List[str]]=None, validate_schema: bool=True, dtype_backend: Literal['numpy_nullable', 'pyarrow']='numpy_nullable', use_threads: Union[bool, int]=True, ray_args: Optional[RaySettings]=None, boto3_session: Optional[boto3.Session]=None, s3_additional_kwargs: Optional[Dict[str, Any]]=None, pyarrow_additional_kwargs: Optional[Dict[str, Any]]=None) -> pd.DataFrame:
    if False:
        for i in range(10):
            print('nop')
    ...
    df = read_orc(path=paths, path_root=path_root, dataset=True, path_suffix=filename_suffix if path_root is None else None, path_ignore_suffix=filename_ignore_suffix if path_root is None else None, columns=columns, validate_schema=validate_schema, dtype_backend=dtype_backend, use_threads=use_threads, ray_args=ray_args, boto3_session=boto3_session, s3_additional_kwargs=s3_additional_kwargs, pyarrow_additional_kwargs=pyarrow_additional_kwargs)
    return _data_types.cast_pandas_with_athena_types(df=df, dtype=_extract_partitions_dtypes_from_table_details(response=res), dtype_backend=dtype_backend)

@_utils.validate_distributed_kwargs(unsupported_kwargs=['boto3_session'])
@apply_configs
def read_orc_metadata(path: Union[str, List[str]], dataset: bool=False, version_id: Optional[Union[str, Dict[str, str]]]=None, path_suffix: Optional[str]=None, path_ignore_suffix: Union[str, List[str], None]=None, ignore_empty: bool=True, ignore_null: bool=False, dtype: Optional[Dict[str, str]]=None, sampling: float=1.0, use_threads: Union[bool, int]=True, boto3_session: Optional[boto3.Session]=None, s3_additional_kwargs: Optional[Dict[str, Any]]=None) -> _ReadTableMetadataReturnValue:
    if False:
        for i in range(10):
            print('nop')
    ...
    (columns_types, partitions_types, _) = reader.read_table_metadata(path=path, version_id=version_id, path_suffix=path_suffix, path_ignore_suffix=path_ignore_suffix, ignore_empty=ignore_empty, ignore_null=ignore_null, dtype=dtype, sampling=sampling, dataset=dataset, use_threads=use_threads, s3_additional_kwargs=s3_additional_kwargs, boto3_session=boto3_session)
