...
def upgrade():
    if False:
        i = 10
        return i + 15
    op.add_column('jobs', sa.Column('last_scheduled_time', postgresql.TIMESTAMP(timezone=True), nullable=True))
    op.create_index(op.f('ix_jobs_last_scheduled_time'), 'jobs', ['last_scheduled_time'], unique=False)
