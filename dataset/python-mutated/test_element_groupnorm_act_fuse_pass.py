...
class TestElementGNActPass(PassAutoScanTest):
    ...
    def sample_predictor_configs(self, program_config):
        ...
        config.set_trt_dynamic_shape_info({'input_data_x': [1, 160, 1, 1], 'input_data_y': [1, 160, 1, 1]}, {'input_data_x': [4, 1280, 64, 64], 'input_data_y': [4, 1280, 1, 1]}, {'input_data_x': [1, 320, 1, 1], 'input_data_y': [1, 320, 1, 1]})
        yield (config, ['skip_groupnorm_act'], (0.003, 0.001))
    ...
    def sample_program_config(self, draw):
        ...
        attrs = [{'axis': axis, 'epsilon': epsilon, 'groups': groups}, {'batch_size': batch_size, 'input_dim_x': [channel, hw, hw], 'input_dim_y': [channel, 1, 1]}]
    ...
    def test(self):
        if False:
            for i in range(10):
                print('nop')
        self.run_and_statis(quant=False, max_examples=50, passes=['elementwise_groupnorm_act_pass'], max_duration=250, min_success_num=50)
