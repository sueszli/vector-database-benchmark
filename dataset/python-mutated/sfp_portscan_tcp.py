...
class sfp_portscan_tcp(SpiderFootPlugin):
    meta = {'name': 'Port Scanner - TCP', 'summary': 'Scans for commonly open TCP ports on Internet-facing systems.', 'flags': ['slow', 'invasive'], 'useCases': ['Footprint', 'Investigate'], 'categories': ['Crawling and Scanning']}
    opts = {'ports': ['21', '22', '23', '25', '53', '79', '80', '81', '88', '110', '111', '113', '119', '123', '137', '138', '139', '143', '161', '179', '389', '443', '445', '465', '512', '513', '514', '515', '3306', '5432', '1521', '2638', '1433', '3389', '5900', '5901', '5902', '5903', '5631', '631', '636', '990', '992', '993', '995', '1080', '8080', '8888', '9000'], 'timeout': 15, 'maxthreads': 10, 'randomize': True, 'netblockscan': True, 'netblockscanmax': 24}
    optdescs = {'maxthreads': 'Number of ports to try to open simultaneously (number of threads to spawn at once.)', 'ports': "The TCP ports to scan. Prefix with an '@' to iterate through a file containing ports to try (one per line), e.g. @C:\\ports.txt or @/home/bob/ports.txt. Or supply a URL to load the list from there.", 'timeout': 'Seconds before giving up on a port.', 'randomize': 'Randomize the order of ports scanned.', 'netblockscan': 'Port scan all IPs within identified owned netblocks?', 'netblockscanmax': 'Maximum netblock/subnet size to scan IPs within (CIDR value, 24 = /24, 16 = /16, etc.)'}
    results = None
    portlist = list()
    portResults = dict()
    lock = None
