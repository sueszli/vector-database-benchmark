...
from fairseq.modules import LayerNorm, TransformerSentenceEncoder, TransformerSentenceEncoderLayer
...
class DiscriminativeNMTRerankerConfig(FairseqDataclass):
    pretrained_model: str = field(default='', metadata={'help': 'pretrained model to load'})
    sentence_rep: SENTENCE_REP_CHOICES = field(default='head', metadata={'help': 'method to transform the output of the transformer stack to a sentence-level representation'})
    ...
    classifier_dropout: float = field(default=0.0, metadata={'help': 'classifier dropout probability'})
    ...
    activation_fn: ACTIVATION_FN_CHOICES = field(default='relu', metadata={'help': 'activation function to use'})
    freeze_embeddings: bool = field(default=False, metadata={'help': 'freeze embeddings in the pretrained model'})
    n_trans_layers_to_freeze: int = field(default=0, metadata={'help': 'number of layers to freeze in the pretrained transformer model'})
    joint_classification: JOINT_CLASSIFICATION_CHOICES = field(default='none', metadata={'help': 'method to compute joint features for classification'})
    num_joint_layers: int = field(default=1, metadata={'help': 'number of joint layers'})
    joint_normalize_before: bool = field(default=False, metadata={'help': 'apply layer norm on the input to the joint layer'})

@register_model('discriminative_nmt_reranker', dataclass=DiscriminativeNMTRerankerConfig)
