...
def test_gam_model_predict():
    ...
    h2o_model_no_early_stop = H2OGeneralizedAdditiveEstimator(family='binomial', gam_columns=['C11'], scale=[0.0001], score_each_iteration=True)
    ...
    for ind in range(len(early_stop_metrics)):
        ...
        h2o_model = H2OGeneralizedAdditiveEstimator(family='binomial', gam_columns=['C11'], scale=[0.0001], stopping_rounds=max_stopping_rounds, score_each_iteration=True, stopping_metric=early_stop_metrics[ind], stopping_tolerance=max_tolerance)
    ...
    try:
        h2o_model = H2OGeneralizedAdditiveEstimator(family='binomial', gam_columns=['C11'], scale=[0.0001], stopping_rounds=max_stopping_rounds, score_each_iteration=True, stopping_metric=early_stop_metrics[ind], stopping_tolerance=max_tolerance, lambda_search=True, nlambdas=3)
