...
def test_sparse_nested_fields(destination_type: DestinationType):
    ...
    try:
        ...
        dbt_test_utils.set_target_schema(target_schema)
        clean_up_args = {'destination_type': [destination_type], 'test_type': 'ephemeral', 'tmp_folders': [str(test_root_dir)]}
        dbt_test_utils.clean_tmp_tables(**clean_up_args)
...
def setup_dbt_sparse_nested_streams_test(destination_type: DestinationType, test_resource_name: str, test_root_dir: str, sync_number: int):
    ...
    copy_test_files(os.path.join('resources', test_resource_name, 'dbt_test_config', f'sync{sync_number}_expectations'), test_directory, destination_type, replace_identifiers)
    test_directory = os.path.join(test_root_dir, 'tests')
    shutil.rmtree(test_directory, ignore_errors=True)
    os.makedirs(test_directory, exist_ok=True)
    copy_test_files(os.path.join('resources', test_resource_name, 'dbt_test_config', f'sync{sync_number}_assertions'), test_directory, destination_type, replace_identifiers)
