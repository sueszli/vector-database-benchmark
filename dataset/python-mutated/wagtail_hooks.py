...
from wagtail.admin.panels import FieldPanel, ObjectList, PublishingPanel, TabbedInterface
...
from wagtail.test.testapp.models import DraftStateModel, FullFeaturedSnippet, ModeratedModel, RevisableChildModel, RevisableModel, VariousOnDeleteModel
from wagtail.test.testapp.views import JSONModelViewSetGroup, MiscellaneousViewSetGroup, SearchTestModelViewSet, ToyViewSetGroup, animated_advert_chooser_viewset
...
class FullFeaturedSnippetViewSet(SnippetViewSet):
    ...
    list_display = ['text', 'country_code', 'get_foo_country_code', UpdatedAtColumn(), 'modulo_two', BooleanColumn('tristate')]
    list_export = ['text', 'country_code', 'get_foo_country_code', 'some_date', 'some_number', 'first_published_at']
...
class RevisableChildModelViewSet(SnippetViewSet):
    ...
    edit_handler = TabbedInterface([ObjectList([FieldPanel('text')], heading='Main'), ObjectList([FieldPanel('secret_text', permission='superuser')], heading='Other', help_text='Other panels help text')], help_text='Top-level help text')
...
class DraftStateModelViewSet(SnippetViewSet):
    ...
    menu_order = -999999
    panels = [FieldPanel('text'), PublishingPanel()]
...
class ModeratedModelViewSet(SnippetViewSet):
    ...
    list_filter = {'text': ['exact', 'contains'], 'first_published_at': ['exact', 'lt', 'gt']}
