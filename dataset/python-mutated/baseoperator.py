...
from typing import TYPE_CHECKING, Any, Callable, ClassVar, Collection, Iterable, Sequence, TypeVar, Union, cast
import attr
import pendulum
...
from airflow.exceptions import AirflowException, FailStopDagInvalidTriggerRule, RemovedInAirflow3Warning, TaskDeferralError, TaskDeferred
from airflow.lineage import apply_lineage, prepare_lineage
from airflow.models.abstractoperator import DEFAULT_IGNORE_FIRST_DEPENDS_ON_PAST, DEFAULT_OWNER, DEFAULT_POOL_SLOTS, DEFAULT_PRIORITY_WEIGHT, DEFAULT_QUEUE, DEFAULT_RETRIES, DEFAULT_RETRY_DELAY, DEFAULT_TASK_EXECUTION_TIMEOUT, DEFAULT_TRIGGER_RULE, DEFAULT_WAIT_FOR_PAST_DEPENDS_BEFORE_SKIPPING, DEFAULT_WEIGHT_RULE, AbstractOperator
...
_PARTIAL_DEFAULTS = {'owner': DEFAULT_OWNER, 'trigger_rule': DEFAULT_TRIGGER_RULE, 'depends_on_past': False, 'ignore_first_depends_on_past': DEFAULT_IGNORE_FIRST_DEPENDS_ON_PAST, 'wait_for_past_depends_before_skipping': DEFAULT_WAIT_FOR_PAST_DEPENDS_BEFORE_SKIPPING, 'wait_for_downstream': False, 'retries': DEFAULT_RETRIES, 'queue': DEFAULT_QUEUE, 'pool_slots': DEFAULT_POOL_SLOTS, 'execution_timeout': DEFAULT_TASK_EXECUTION_TIMEOUT, 'retry_delay': DEFAULT_RETRY_DELAY, 'retry_exponential_backoff': False, 'priority_weight': DEFAULT_PRIORITY_WEIGHT, 'weight_rule': DEFAULT_WEIGHT_RULE, 'inlets': [], 'outlets': []}
...
class BaseOperator(AbstractOperator, metaclass=BaseOperatorMeta):
    ...
    ui_fgcolor: str = '#000'
    pool: str = ''
    _base_operator_shallow_copy_attrs: tuple[str, ...] = ('user_defined_macros', 'user_defined_filters', 'params')
    shallow_copy_attrs: Sequence[str] = ()
    operator_extra_links: Collection[BaseOperatorLink] = ()
    __serialized_fields: frozenset[str] | None = None
    partial: Callable[..., OperatorPartial] = _PartialDescriptor()
    ...
    deps: frozenset[BaseTIDep] = frozenset({NotInRetryPeriodDep(), PrevDagrunDep(), TriggerRuleDep(), NotPreviouslySkippedDep()})
