...
class sfp_threatfox(SpiderFootPlugin):
    meta = {'name': 'ThreatFox', 'summary': 'Check if an IP address is malicious according to ThreatFox.', 'flags': [], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'model': 'FREE_NOAUTH_UNLIMITED', 'references': ['https://threatfox.abuse.ch/api/'], 'website': 'https://threatfox.abuse.ch', 'favIcon': 'https://threatfox.abuse.ch/favicon.ico', 'logo': 'https://threatfox.abuse.ch/images/threatfox_logo.png', 'description': 'ThreatFox is a free platform from abuse.ch with the goal of sharingindicators of compromise (IOCs) associated with malware with the infosec community,AV vendors and threat intelligence providers.'}}
    ...
    def query(self, qry):
        ...
        res = self.sf.fetchUrl('https://threatfox-api.abuse.ch/api/v1/', useragent=self.opts['_useragent'], timeout=self.opts['_fetchtimeout'], headers=headers, postData=json.dumps(params))
        time.sleep(1)
