...
from langchain.schema import AgentAction, AgentFinish, AIMessage, HumanMessage, OutputParserException, BaseMessage, get_buffer_string
...
class AutoSummarizingStructuredChatAgent(StructuredChatAgent, CalcTokenMixin):
    ...
    def summarize_messages(self, intermediate_steps: List[Tuple[AgentAction, str]], **kwargs):
        ...
        if len(intermediate_steps) >= 2 and self.summary_model_instance:
            ...
            should_summary_messages = [AIMessage(content=observation) for (_, observation) in should_summary_intermediate_steps]
