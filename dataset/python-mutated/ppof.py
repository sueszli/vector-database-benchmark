...
from ding.rl_utils import ppo_data, ppo_error, ppo_policy_error, ppo_policy_data, gae, gae_data, ppo_error_continuous, get_gae, ppo_policy_error_continuous, ArgmaxSampler, MultinomialSampler, ReparameterizationSampler, MuSampler, HybridStochasticSampler, HybridDeterminsticSampler, value_transform, value_inv_transform, symlog, inv_symlog
from ding.utils import POLICY_REGISTRY, RunningMeanStd

@POLICY_REGISTRY.register('ppof')
class PPOFPolicy:
    config = dict(type='ppo', on_policy=True, cuda=True, action_space='discrete', discount_factor=0.99, gae_lambda=0.95, epoch_per_collect=10, batch_size=64, learning_rate=0.0003, lr_scheduler=None, weight_decay=0, value_weight=0.5, entropy_weight=0.01, clip_ratio=0.2, adv_norm=True, value_norm='baseline', ppo_param_init=True, grad_norm=0.5, n_sample=128, unroll_len=1, deterministic_eval=True, model=dict())
