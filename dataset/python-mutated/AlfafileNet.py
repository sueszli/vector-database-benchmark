...
class AlfafileNet(SimpleDownloader):
    ...
    __version__ = '0.07'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?(alfafile\\.net)/file/(?P<ID>\\w+)'
    ...
    URL_REPLACEMENTS = [(__pattern__ + '.*', 'https://alfafile.net/file/\\g<ID>')]
    COOKIES = [('alfafile.net', 'lang', 'en')]
    NAME_PATTERN = '<strong id="st_file_name" title="(?P<N>.+?)"'
    SIZE_PATTERN = '<span class="size">(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)<'
    LINK_PATTERN = '<a href="(.+?)" class="big_button"><span>Download</span></a>'
    DL_LIMIT_PATTERN = 'Try again in (.+?)<'
    PREMIUM_ONLY_PATTERN = 'In order to buy premium access'
