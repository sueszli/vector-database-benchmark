...
class LogHead(dashboard_utils.DashboardHeadModule):
    ...
    async def insert_log_url_to_node_info(self, node_info):
        ...
        log_url = self.LOG_URL_TEMPLATE.format(ip=node_info.get('ip'), port=agent_http_port)
    ...
    async def get_log_index(self, req) -> aiohttp.web.Response:
        ...
        for (node_id, ports) in DataSource.agents.items():
            ...
            agent_ips.add(ip)
            index_text = self.LOG_INDEX_TEXT_TEMPLATE.format(node_id=node_id, node_ip=ip)
        ...
        if self._dashboard_head.ip not in agent_ips:
            index_text = self.LOG_INDEX_TEXT_TEMPLATE.format(node_id='head', node_ip=self._dashboard_head.ip)
            log_url = self.LOG_URL_TEMPLATE.format(ip=self._dashboard_head.ip, port=self._dashboard_head.http_port)
            index_text_log_url.append((index_text, log_url))
        return aiohttp.web.Response(text=self._directory_as_html(index_text_log_url), content_type='text/html')
    ...
    async def get_log_from_proxy(self, req) -> aiohttp.web.StreamResponse:
        ...
        async with self._proxy_session.request(req.method, encoded_url, data=body, headers=req.headers) as r:
            sr = aiohttp.web.StreamResponse(status=r.status, reason=r.reason, headers=req.headers)
