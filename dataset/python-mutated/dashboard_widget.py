...
from sentry.db.models import ArrayField, BoundedPositiveIntegerField, FlexibleForeignKey, Model, region_silo_only_model, sane_repr
...
class DashboardWidgetTypes(TypesClass):
    ...
    ISSUE = 1
    METRICS = 2
    TYPES = [(DISCOVER, 'discover'), (ISSUE, 'issue'), (METRICS, 'metrics')]
...
class DashboardWidgetDisplayTypes(TypesClass):
    ...
    BIG_NUMBER = 6
    TOP_N = 7
    TYPES = [(LINE_CHART, 'line'), (AREA_CHART, 'area'), (STACKED_AREA_CHART, 'stacked_area'), (BAR_CHART, 'bar'), (TABLE, 'table'), (BIG_NUMBER, 'big_number'), (TOP_N, 'top_n')]
    TYPE_NAMES = [t[1] for t in TYPES]
