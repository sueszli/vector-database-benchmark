...
@dashboard_utils.dashboard_module(enable=env_bool(test_consts.TEST_MODULE_ENVIRONMENT_KEY, False))
class TestHead(dashboard_utils.DashboardHeadModule):
    ...
    async def dump(self, req) -> aiohttp.web.Response:
        ...
        if key is None:
            all_data = {k: dict(v) for (k, v) in DataSource.__dict__.items() if not k.startswith('_')}
            return dashboard_optional_utils.rest_response(success=True, message='Fetch all data from datacenter success.', **all_data)
        else:
            data = dict(DataSource.__dict__.get(key))
            return dashboard_optional_utils.rest_response(success=True, message=f'Fetch {key} from datacenter success.', **{key: data})

    @routes.get('/test/notified_agents')
    async def get_notified_agents(self, req) -> aiohttp.web.Response:
        return dashboard_optional_utils.rest_response(success=True, message='Fetch notified agents success.', **self._notified_agents)
    ...
    async def test_aiohttp_cache(self, req) -> aiohttp.web.Response:
        ...
        return dashboard_optional_utils.rest_response(success=True, message='OK', value=value, timestamp=time.time())
