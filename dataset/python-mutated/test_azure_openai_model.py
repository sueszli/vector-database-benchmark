...
def get_mock_provider():
    if False:
        for i in range(10):
            print('nop')
    return Provider(id='provider_id', tenant_id='tenant_id', provider_name='azure_openai', provider_type=ProviderType.CUSTOM.value, encrypted_config='', is_valid=True)
...
def get_mock_azure_openai_model(model_name, mocker):
    ...
    mock_query.filter.return_value.first.return_value = ProviderModel(provider_name='azure_openai', model_name=model_name, model_type=ModelType.TEXT_GENERATION.value, encrypted_config=json.dumps({'openai_api_base': valid_openai_api_base, 'openai_api_key': valid_openai_api_key, 'base_model_name': model_name}), is_valid=True)
    mocker.patch('extensions.ext_database.db.session.query', return_value=mock_query)
    return AzureOpenAIModel(model_provider=provider, name=model_name, model_kwargs=model_kwargs)
