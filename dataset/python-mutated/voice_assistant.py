...
import wave
from aioesphomeapi import VoiceAssistantAudioSettings, VoiceAssistantCommandFlag, VoiceAssistantEventType
from homeassistant.components import stt, tts
from homeassistant.components.assist_pipeline import AudioSettings, PipelineEvent, PipelineEventType, PipelineNotFound, PipelineStage, WakeWordSettings, async_pipeline_from_audio_stream, select as pipeline_select
...
class VoiceAssistantUDPServer(asyncio.DatagramProtocol):
    ...
    async def run_pipeline(self, device_id: str, conversation_id: str | None, flags: int=0, audio_settings: VoiceAssistantAudioSettings | None=None) -> None:
        ...
        tts_audio_output = 'wav' if self.device_info.voice_assistant_version >= 2 else 'mp3'
        ...
        try:
            await async_pipeline_from_audio_stream(self.hass, context=self.context, event_callback=self._event_callback, stt_metadata=stt.SpeechMetadata(language='', format=stt.AudioFormats.WAV, codec=stt.AudioCodecs.PCM, bit_rate=stt.AudioBitRates.BITRATE_16, sample_rate=stt.AudioSampleRates.SAMPLERATE_16000, channel=stt.AudioChannels.CHANNEL_MONO), stt_stream=self._iterate_packets(), pipeline_id=pipeline_select.get_chosen_pipeline(self.hass, DOMAIN, self.device_info.mac_address), conversation_id=conversation_id, device_id=device_id, tts_audio_output=tts_audio_output, start_stage=start_stage, wake_word_settings=WakeWordSettings(timeout=5), audio_settings=AudioSettings(noise_suppression_level=audio_settings.noise_suppression_level, auto_gain_dbfs=audio_settings.auto_gain, volume_multiplier=audio_settings.volume_multiplier, is_vad_enabled=bool(flags & VoiceAssistantCommandFlag.USE_VAD)))
            await self._tts_done.wait()
            _LOGGER.debug('Pipeline finished')
        except PipelineNotFound:
            self.handle_event(VoiceAssistantEventType.VOICE_ASSISTANT_ERROR, {'code': 'pipeline not found', 'message': 'Selected pipeline not found'})
            _LOGGER.warning('Pipeline not found')
        except WakeWordDetectionAborted:
            pass
        except WakeWordDetectionError as e:
            self.handle_event(VoiceAssistantEventType.VOICE_ASSISTANT_ERROR, {'code': e.code, 'message': e.message})
        finally:
            self.handle_finished()
        ...
        try:
            ...
            with io.BytesIO(data) as wav_io:
                with wave.open(wav_io, 'rb') as wav_file:
                    ...
                    if sample_rate != 16000 or sample_width != 2 or sample_channels != 1:
                        raise ValueError('Expected rate/width/channels as 16000/2/1, got {sample_rate}/{sample_width}/{sample_channels}}')
            ...
            while samples_left > 0:
                ...
                await asyncio.sleep(samples_in_chunk / stt.AudioSampleRates.SAMPLERATE_16000 * 0.9)
                sample_offset += samples_in_chunk
        finally:
            self.handle_event(VoiceAssistantEventType.VOICE_ASSISTANT_TTS_STREAM_END, {})
            self._tts_task = None
            self._tts_done.set()