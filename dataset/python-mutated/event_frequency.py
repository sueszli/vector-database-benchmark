...
from sentry.types.condition_activity import FREQUENCY_CONDITION_BUCKET_SIZE, ConditionActivity, round_to_five_minute
from sentry.utils import metrics
from sentry.utils.snuba import options_override
standard_intervals = {'1m': ('one minute', timedelta(minutes=1)), '5m': ('5 minutes', timedelta(minutes=5)), '15m': ('15 minutes', timedelta(minutes=15)), '1h': ('one hour', timedelta(hours=1)), '1d': ('one day', timedelta(hours=24)), '1w': ('one week', timedelta(days=7)), '30d': ('30 days', timedelta(days=30))}
comparison_intervals = {'5m': ('5 minutes', timedelta(minutes=5)), '15m': ('15 minutes', timedelta(minutes=15)), '1h': ('one hour', timedelta(hours=1)), '1d': ('one day', timedelta(hours=24)), '1w': ('one week', timedelta(days=7)), '30d': ('30 days', timedelta(days=30))}
COMPARISON_TYPE_COUNT = 'count'
COMPARISON_TYPE_PERCENT = 'percent'
comparison_types = {COMPARISON_TYPE_COUNT: COMPARISON_TYPE_COUNT, COMPARISON_TYPE_PERCENT: COMPARISON_TYPE_PERCENT}

class EventFrequencyForm(forms.Form):
    intervals = standard_intervals
    interval = forms.ChoiceField(choices=[(key, label) for (key, (label, _)) in sorted(intervals.items(), key=lambda key____label__duration: key____label__duration[1][1])])
    value = forms.IntegerField(widget=forms.TextInput())
    comparisonType = forms.ChoiceField(choices=list(sorted(comparison_types.items(), key=lambda item: item[1])), required=False)
    comparisonInterval = forms.ChoiceField(choices=[(key, label) for (key, (label, _)) in sorted(comparison_intervals.items(), key=lambda item: item[1][1])], required=False)
    ...
    def get_preview_aggregate(self) -> Tuple[str, str]:
        if False:
            while True:
                i = 10
        return ('uniq', 'user')
percent_intervals = {'1m': ('1 minute', timedelta(minutes=1)), '5m': ('5 minutes', timedelta(minutes=5)), '10m': ('10 minutes', timedelta(minutes=10)), '30m': ('30 minutes', timedelta(minutes=30)), '1h': ('1 hour', timedelta(minutes=60))}
percent_intervals_to_display = {'5m': ('5 minutes', timedelta(minutes=5)), '10m': ('10 minutes', timedelta(minutes=10)), '30m': ('30 minutes', timedelta(minutes=30)), '1h': ('1 hour', timedelta(minutes=60))}
MIN_SESSIONS_TO_FIRE = 50

class EventFrequencyPercentForm(EventFrequencyForm):
    intervals = percent_intervals_to_display
    interval = forms.ChoiceField(choices=[(key, label) for (key, (label, duration)) in sorted(percent_intervals_to_display.items(), key=lambda key____label__duration: key____label__duration[1][1])])
