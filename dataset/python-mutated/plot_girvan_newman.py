...
modularity_df = pd.DataFrame([[k + 1, nx.community.modularity(G, communities[k])] for k in range(len(communities))], columns=['k', 'modularity'])
...
modularity_df.plot.bar(x='k', ax=ax[2], color='#F2D140', title='Modularity Trend for Girvan-Newman Community Detection')
plt.show()