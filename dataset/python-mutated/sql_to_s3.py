...
FILE_OPTIONS_MAP = {FILE_FORMAT.CSV: FileOptions('r+', '.csv', 'to_csv'), FILE_FORMAT.JSON: FileOptions('r+', '.json', 'to_json'), FILE_FORMAT.PARQUET: FileOptions('rb+', '.parquet', 'to_parquet')}
...
class SqlToS3Operator(BaseOperator):
    ...
    """
    template_fields: Sequence[str] = ('s3_bucket', 's3_key', 'query', 'sql_conn_id')
    template_ext: Sequence[str] = ('.sql',)
    template_fields_renderers = {'query': 'sql', 'pd_kwargs': 'json'}
