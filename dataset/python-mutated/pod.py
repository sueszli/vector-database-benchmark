        ...
        try:
            while True:
                ...
                if container_state == ContainerState.TERMINATED:
                    yield TriggerEvent({'name': self.pod_name, 'namespace': self.pod_namespace, 'status': 'success', 'message': 'All containers inside pod have started successfully.'})
                    return
                    ...
                    if pod_status == PodPhase.PENDING and container_state != ContainerState.RUNNING:
                        ...
                        if delta.total_seconds() >= self.startup_timeout:
                            message = f'Pod took longer than {self.startup_timeout} seconds to start. Check the pod events in kubernetes to determine why.'
                            yield TriggerEvent({'name': self.pod_name, 'namespace': self.pod_namespace, 'status': 'timeout', 'message': message})
                            return
                        else:
                    ...
                    yield TriggerEvent({'name': self.pod_name, 'namespace': self.pod_namespace, 'status': 'failed', 'message': pod.status.message})
                    return
        except CancelledError:
            if self.get_logs:
                self.log.info('Outputting container logs...')
                await self._get_async_hook().read_logs(name=self.pod_name, namespace=self.pod_namespace)
            if self.on_finish_action == OnFinishAction.DELETE_POD:
                self.log.info('Deleting pod...')
                await self._get_async_hook().delete_pod(name=self.pod_name, namespace=self.pod_namespace)
            yield TriggerEvent({'name': self.pod_name, 'namespace': self.pod_namespace, 'status': 'cancelled', 'message': 'Pod execution was cancelled'})
        except Exception as e:
            self.log.exception('Exception occurred while checking pod phase:')
            yield TriggerEvent({'name': self.pod_name, 'namespace': self.pod_namespace, 'status': 'error', 'message': str(e)})
