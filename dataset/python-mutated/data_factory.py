...
from airflow.providers.microsoft.azure.hooks.data_factory import AzureDataFactoryAsyncHook, AzureDataFactoryPipelineRunStatus
...
class ADFPipelineRunStatusSensorTrigger(BaseTrigger):
    ...
    async def run(self) -> AsyncIterator[TriggerEvent]:
        ...
        try:
            while True:
                try:
                    pipeline_status = await hook.get_adf_pipeline_run_status(run_id=self.run_id, resource_group_name=self.resource_group_name, factory_name=self.factory_name)
                    executed_after_token_refresh = False
                    if pipeline_status == AzureDataFactoryPipelineRunStatus.FAILED:
                        yield TriggerEvent({'status': 'error', 'message': f'Pipeline run {self.run_id} has Failed.'})
                        return
                ...
                while self.end_time > time.time():
                    try:
                        ...
                        if pipeline_status in AzureDataFactoryPipelineRunStatus.FAILURE_STATES:
                            yield TriggerEvent({'status': 'error', 'message': f'The pipeline run {self.run_id} has {pipeline_status}.', 'run_id': self.run_id})
                            return
                        elif pipeline_status == AzureDataFactoryPipelineRunStatus.SUCCEEDED:
                            yield TriggerEvent({'status': 'success', 'message': f'The pipeline run {self.run_id} has {pipeline_status}.', 'run_id': self.run_id})
                            return
                        ...
                        if executed_after_token_refresh:
                            ...
                            raise
                yield TriggerEvent({'status': 'error', 'message': f'Timeout: The pipeline run {self.run_id} has {pipeline_status}.', 'run_id': self.run_id})
            else:
                yield TriggerEvent({'status': 'success', 'message': f'The pipeline run {self.run_id} has {pipeline_status} status.', 'run_id': self.run_id})
        except Exception as e:
            if self.run_id:
                try:
                    await hook.cancel_pipeline_run(run_id=self.run_id, resource_group_name=self.resource_group_name, factory_name=self.factory_name)
