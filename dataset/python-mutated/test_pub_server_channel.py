...
pytestmark = [pytest.mark.skip_on_freebsd(reason='Temporarily skipped on FreeBSD.'), pytest.mark.skip_on_spawning_platform(reason='These tests are currently broken on spawning platforms. Need to be rewritten.')]
...
async def test_pub_channel(master_opts, io_loop):
    server = salt.transport.zeromq.PublishServer(master_opts, pub_host='127.0.0.1', pub_port=4506, pull_path=os.path.join(master_opts['sock_dir'], 'publish_pull.ipc'))
    payloads = []

    async def publish_payload(payload):
        await server.publish_payload(payload)
        payloads.append(payload)
    io_loop.add_callback(server.publisher, publish_payload, ioloop=io_loop)
