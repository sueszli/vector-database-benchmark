...
from typing import Optional, Union, List, Iterable, Dict, NoReturn, Set, TypeVar, Callable, Awaitable, Any, Literal, MutableMapping, Set, overload, TYPE_CHECKING
...
from ._settings_caches import PrefixManager, IgnoreManager, WhitelistBlacklistManager, DisabledCogCache, I18nManager
...
class Red(commands.GroupMixin, RPCMixin, dpy_commands.bot.AutoShardedBot):
    ...
    async def allowed_by_whitelist_blacklist(self, who: Optional[Union[discord.Member, discord.User]]=None, *, who_id: Optional[int]=None, guild: Optional[discord.Guild]=None, role_ids: Optional[List[int]]=None) -> bool:
        """
    ...
    async def message_eligible_as_command(self, message: discord.Message) -> bool:
        ...
        if message.author.bot:
            return False
        if isinstance(channel, discord.PartialMessageable) and channel.type is not discord.ChannelType.private:
            return False
        if guild:
            assert isinstance(channel, (discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.Thread))
    ...
    async def ignored_channel_or_guild(self, ctx: Union[commands.Context, discord.Message, discord.Interaction]) -> bool:
        """
        ...
        surpass_ignore = is_private or perms.manage_guild or await self.is_owner(author) or await self.is_admin(author)
        if surpass_ignore:
            return True
        ...
        if perms.manage_threads:
            return True
        thread_ignored = await self._ignored_cache.get_ignored_channel(thread, check_category=False)
    ...
    async def get_embed_color(self, location: discord.abc.Messageable) -> discord.Color:
        ...
        if guild and await self._config.guild(guild).use_bot_color() and (not isinstance(location, discord.Member)):
    ...
    async def _pre_connect(self) -> None:
        ...
        if ver_info != last_system_info['python_version']:
            ...
            if any(LIB_PATH.iterdir()):
                ...
                LIB_PATH.mkdir()
                asyncio.create_task(send_to_owners_with_prefix_replaced(self, 'We detected a change in minor Python version and cleared packages in lib folder.\nThe instance was started with no cogs, please load Downloader and use `[p]cog reinstallreqs` to regenerate lib folder. After that, restart the bot to get all of your previously loaded cogs loaded again.'))
        ...
        if system_changed and (not python_version_changed):
            asyncio.create_task(send_to_owners_with_prefix_replaced(self, "We detected a possible change in machine's operating system or architecture. You might need to regenerate your lib folder if 3rd-party cogs stop working properly.\nTo regenerate lib folder, load Downloader and use `[p]cog reinstallreqs`."))
        if packages:
            try:
            ...
            for package in packages:
                try:
                    ...
                    if spec is None:
                        log.error('Failed to load package %s (package was not found in any cog path)', package)
    ...
    async def embed_requested(self, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, commands.Context, discord.User, discord.Member, discord.Thread], *, command: Optional[commands.Command]=None, check_permissions: bool=True) -> bool:
        """
        ...
        if isinstance(channel, dpy_commands.Context):
            ...
            channel = channel.author if isinstance(channel.channel, discord.DMChannel) else channel.channel
        if isinstance(channel, (discord.GroupChannel, discord.DMChannel, discord.PartialMessageable)):
            raise TypeError('You cannot pass a GroupChannel, DMChannel, or PartialMessageable to this method.')
                ...
                for m in (f'<@{self.user.id}> ', f'<@!{self.user.id}> '):
                    if message.content.startswith(m):
                        ...
                        if isinstance(command, commands.commands._AlwaysAvailableMixin):
                            ...
                            ctx.prefix = m
                        break
            if ctx.invoked_with and isinstance(message.channel, discord.PartialMessageable) and (message.channel.type is not discord.ChannelType.private):
                log.warning('Discarded a command message (ID: %s) with PartialMessageable channel: %r', message.id, message.channel)
            else:
    ...
    async def remove_cog(self, cogname: str, /, *, guild: Optional[discord.abc.Snowflake]=discord.utils.MISSING, guilds: List[discord.abc.Snowflake]=discord.utils.MISSING) -> Optional[commands.Cog]:
    ...
    async def enable_app_command(self, command_name: str, command_type: discord.AppCommandType=discord.AppCommandType.chat_input) -> None:
        """
    ...
    async def disable_app_command(self, command_name: str, command_type: discord.AppCommandType=discord.AppCommandType.chat_input) -> None:
        """
    ...
    async def list_enabled_app_commands(self) -> Dict[str, Dict[str, Optional[int]]]:
        ...
        return {'slash': curr_slash_commands, 'message': curr_message_commands, 'user': curr_user_commands}
    ...
    async def add_cog(self, cog: commands.Cog, /, *, override: bool=False, guild: Optional[discord.abc.Snowflake]=discord.utils.MISSING, guilds: List[discord.abc.Snowflake]=discord.utils.MISSING) -> None:
        if not isinstance(cog, commands.Cog):
            raise RuntimeError(f'The {cog.__class__.__name__} cog in the {cog.__module__} package does not inherit from the commands.Cog base class. The cog author must update the cog to adhere to this requirement.')
    ...
    async def get_owner_notification_destinations(self) -> List[Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.User]]:
        """
        ...
        for user_id in self.owner_ids:
            if user_id not in opt_outs:
                ...
                if user and (not user.bot):
                    ...
                    log.warning('Owner with ID %s is missing in user cache, ignoring owner notification destination.', user_id)
        ...
        for channel_id in channel_ids:
            ...
            if channel:
                ...
                log.warning('Channel with ID %s is not available, ignoring owner notification destination.', channel_id)
        return destinations
    ...
    async def send_to_owners(self, content=None, **kwargs):
        ...
        async def wrapped_send(location, content=None, **kwargs):
            try:
                ...
                log.error('I could not send an owner notification to %s (%s)', location, location.id, exc_info=_exc)
    ...
    async def _core_data_deletion(self, *, requester: Literal['discord_deleted_user', 'owner', 'user', 'user_strict'], user_id: int):
    ...
    async def handle_data_deletion_request(self, *, requester: Literal['discord_deleted_user', 'owner', 'user', 'user_strict'], user_id: int) -> DataDeletionResults:
        """
    ...
    async def _handle_data_deletion_request(self, *, requester: Literal['discord_deleted_user', 'owner', 'user', 'user_strict'], user_id: int) -> DataDeletionResults:
        """
        ...
        DataDeletionResults
        """
        extension_handlers = {extension_name: handler for (extension_name, extension) in self.extensions.items() if (handler := getattr(extension, 'red_delete_data_for_user', None))}
        cog_handlers = {cog_qualname: cog.red_delete_data_for_user for (cog_qualname, cog) in self.cogs.items()}
        special_handlers = {'Red Core Modlog API': modlog._process_data_deletion, 'Red Core Bank API': bank._process_data_deletion, 'Red Core Bot Data': self._core_data_deletion}
        failures = {'extension': [], 'cog': [], 'unhandled': []}
        ...
        handlers = [*(wrapper(coro, 'extension', name) for (name, coro) in extension_handlers.items()), *(wrapper(coro, 'cog', name) for (name, coro) in cog_handlers.items()), *(wrapper(coro, 'extension', name) for (name, coro) in special_handlers.items())]
        await asyncio.gather(*handlers)
        return DataDeletionResults(failed_modules=failures['extension'], failed_cogs=failures['cog'], unhandled=failures['unhandled'])

    async def send_interactive(self, channel: discord.abc.Messageable, messages: Iterable[str], *, user: Optional[discord.User]=None, box_lang: Optional[str]=None, timeout: int=15, join_character: str='') -> List[discord.Message]:
        """
        ...
        for (idx, page) in enumerate(messages, 1):
            ...
            if n_remaining > 0:
                if n_remaining == 1:
                    prompt_text = _('There is still one message remaining. Type {command_1} to continue or {command_2} to upload all contents as a file.')
                else:
                    prompt_text = _('There are still {count} messages remaining. Type {command_1} to continue or {command_2} to upload all contents as a file.')
                query = await channel.send(prompt_text.format(count=n_remaining, command_1='`more`', command_2='`file`'))
                pred = MessagePredicate.lower_contained_in(('more', 'file'), channel=channel, user=user)
                try:
                    resp = await self.wait_for('message', check=pred, timeout=timeout)
                except asyncio.TimeoutError:
                    with contextlib.suppress(discord.HTTPException):
                        await query.delete()
                    break
                    ...
                    try:
                        ...
                        with contextlib.suppress(discord.HTTPException):
                            await query.delete()
                    if pred.result == 1:
                        ret.append(await channel.send(file=text_to_file(join_character.join(messages))))
                        break
        return ret