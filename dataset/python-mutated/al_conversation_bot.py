...
class AugLoopConversationBot(ConversationBot):
    ...
    def __init__(self, role: ConversationRole, augLoopParams: AugLoopParams, instantiation_parameters: Dict[str, str]):
        if False:
            for i in range(10):
                print('nop')
    ...
    async def generate_response(self, session: RetryClient, conversation_history: List[ConversationTurn], max_history: int, turn_number: int=0) -> Tuple[dict, dict, int, dict]:
        ...
        parsed_response = {'samples': response_data['messages'], 'id': response_data['id']}
