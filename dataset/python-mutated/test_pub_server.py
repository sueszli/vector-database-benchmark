...
async def test_pub_channel(master_opts, minion_opts, io_loop):
    ...
    server = salt.transport.tcp.TCPPublishServer(master_opts, pub_host='127.0.0.1', pub_port=master_opts['publish_port'], pull_path=os.path.join(master_opts['sock_dir'], 'publish_pull.ipc'))
    client = salt.transport.tcp.TCPPubClient(minion_opts, io_loop, host='127.0.0.1', port=master_opts['publish_port'])
    payloads = []
    publishes = []
    ...
    io_loop.add_callback(server.publisher, publish_payload, presence_callback, remove_presence_callback)
