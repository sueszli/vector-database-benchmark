...
SCHEMA_IP_BAN_ENTRY: Final = vol.Schema({vol.Optional('banned_at'): vol.Any(None, cv.datetime)})
...
async def process_wrong_login(request: Request) -> None:
    ...
    with suppress(herror):
        (remote_host, _, _) = await hass.async_add_executor_job(gethostbyaddr, request.remote)
    ...
    persistent_notification.async_create(hass, notification_msg, 'Login attempt failed', NOTIFICATION_ID_LOGIN)
    ...
    if request.app[KEY_FAILED_LOGIN_ATTEMPTS][remote_addr] >= request.app[KEY_LOGIN_THRESHOLD]:
        ...
        persistent_notification.async_create(hass, f'Too many login attempts from {remote_addr}', 'Banning IP address', NOTIFICATION_ID_BAN)
...
class IpBanManager:
    ...
    async def async_load(self) -> None:
        ...
        try:
            list_ = await self.hass.async_add_executor_job(load_yaml_config_file, self.path)
