...
from aiortc.rtcdtlstransport import RTCCertificate, RTCDtlsFingerprint, RTCDtlsParameters, RTCDtlsTransport, RtpRouter
from aiortc.rtcrtpparameters import RTCRtpCodecParameters, RTCRtpDecodingParameters, RTCRtpReceiveParameters
from aiortc.rtp import RTCP_PSFB_APP, RTCP_PSFB_PLI, RTCP_RTPFB_NACK, RtcpByePacket, RtcpPsfbPacket, RtcpReceiverInfo, RtcpRrPacket, RtcpRtpfbPacket, RtcpSenderInfo, RtcpSrPacket, RtpPacket, pack_remb_fci
...
class RTCDtlsTransportTest(TestCase):
    ...
    async def test_data(self):
        ...
        await asyncio.gather(session1.start(session2.getLocalParameters()), session2.start(session1.getLocalParameters()))
    ...
    async def test_rtp(self):
        ...
        session1._register_rtp_receiver(receiver1, RTCRtpReceiveParameters(codecs=[RTCRtpCodecParameters(mimeType='audio/PCMU', clockRate=8000, payloadType=0)], encodings=[RTCRtpDecodingParameters(ssrc=1831097322, payloadType=0)]))
        certificate2 = RTCCertificate.generateCertificate()
        session2 = RTCDtlsTransport(transport2, [certificate2])
        receiver2 = DummyRtpReceiver()
        session2._register_rtp_receiver(receiver2, RTCRtpReceiveParameters(codecs=[RTCRtpCodecParameters(mimeType='audio/PCMU', clockRate=8000, payloadType=0)], encodings=[RTCRtpDecodingParameters(ssrc=4028317929, payloadType=0)]))
        await asyncio.gather(session1.start(session2.getLocalParameters()), session2.start(session1.getLocalParameters()))
    ...
    async def test_bad_client_fingerprint(self):
        ...
        certificate2 = RTCCertificate.generateCertificate()
        session2 = RTCDtlsTransport(transport2, [certificate2])
        bogus_parameters = RTCDtlsParameters(fingerprints=[RTCDtlsFingerprint(algorithm='sha-256', value='bogus_fingerprint')])
        await asyncio.gather(session1.start(bogus_parameters), session2.start(session1.getLocalParameters()))
    ...
    async def test_handshake_error(self, mock_do_handshake):
        mock_do_handshake.side_effect = SSL.Error([('SSL routines', '', 'decryption failed or bad record mac')])
        (transport1, transport2) = dummy_ice_transport_pair()
        certificate1 = RTCCertificate.generateCertificate()
        session1 = RTCDtlsTransport(transport1, [certificate1])
        certificate2 = RTCCertificate.generateCertificate()
