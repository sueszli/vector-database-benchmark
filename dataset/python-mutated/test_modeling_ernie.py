...
if is_torch_available():
    import torch
    from transformers import MODEL_FOR_PRETRAINING_MAPPING, ErnieForCausalLM, ErnieForMaskedLM, ErnieForMultipleChoice, ErnieForNextSentencePrediction, ErnieForPreTraining, ErnieForQuestionAnswering, ErnieForSequenceClassification, ErnieForTokenClassification, ErnieModel
...
class ErnieModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (ErnieModel, ErnieForCausalLM, ErnieForMaskedLM, ErnieForMultipleChoice, ErnieForNextSentencePrediction, ErnieForPreTraining, ErnieForQuestionAnswering, ErnieForSequenceClassification, ErnieForTokenClassification) if is_torch_available() else ()
    all_generative_model_classes = (ErnieForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': ErnieModel, 'fill-mask': ErnieForMaskedLM, 'question-answering': ErnieForQuestionAnswering, 'text-classification': ErnieForSequenceClassification, 'text-generation': ErnieForCausalLM, 'token-classification': ErnieForTokenClassification, 'zero-shot': ErnieForSequenceClassification} if is_torch_available() else {}
    fx_compatible = False
