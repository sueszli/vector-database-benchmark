...
IGNORE_WARNINGS = (dict(category=RuntimeWarning, message='All-NaN|Mean of empty slice|Degrees of freedom <= 0|invalid value encountered in sqrt'),)
...
class DataInfo(metaclass=DataInfoMeta):
    ...
    info_summary_attributes = staticmethod(data_info_factory(names=_info_summary_attrs, funcs=[partial(_get_data_attribute, attr=attr) for attr in _info_summary_attrs]))
    info_summary_stats = staticmethod(data_info_factory(names=_stats, funcs=[getattr(np, 'nan' + stat) for stat in _stats]))
