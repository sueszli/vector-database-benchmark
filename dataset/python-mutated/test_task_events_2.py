...
import ray
from ray._private.state_api_test_utils import PidActor, get_state_api_manager, verify_tasks_running_or_terminated, verify_failed_task
from ray.util.state.common import ListApiOptions, StateResource
from ray._private.test_utils import async_wait_for_condition_async_predicate, run_string_as_driver, run_string_as_driver_nonblocking, wait_for_condition
from ray.util.state import StateApiClient, list_actors, list_jobs, list_tasks
_SYSTEM_CONFIG = {'task_events_report_interval_ms': 100, 'metrics_report_interval_ms': 200, 'enable_timeline': False, 'gcs_mark_task_failed_on_job_done_delay_ms': 1000, 'gcs_mark_task_failed_on_worker_dead_delay_ms': 1000}
...
async def test_task_events_gc_jobs(shutdown_only):
    ...
    """
    ctx = ray.init(num_cpus=8, _system_config={'task_events_max_num_task_in_gcs': 3, 'task_events_skip_driver_for_test': True, 'task_events_report_interval_ms': 100})
    ...
    async def verify_tasks(task_name: str):
        resp = await manager.list_tasks(option=ListApiOptions(filters=[('name', '=', task_name), ('job_id', '=', get_last_job())]))
    ...
    for i in range(10):
        ...
        await async_wait_for_condition_async_predicate(verify_tasks, task_name=f'f.{i}', retry_interval_ms=500)
