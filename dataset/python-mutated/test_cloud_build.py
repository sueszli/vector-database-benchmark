...
BUILD = {'source': REPO_SOURCE, 'steps': [{'name': 'gcr.io/cloud-builders/gcloud', 'entrypoint': '/bin/sh', 'args': ['-c', 'ls']}], 'status': 'SUCCESS'}
BUILD_TRIGGER = {'name': 'test-cloud-build-trigger', 'trigger_template': {'project_id': PROJECT_ID, 'repo_name': 'test_repo', 'branch_name': 'master'}, 'filename': 'cloudbuild.yaml'}
OPERATION = {'metadata': {'build': {'id': BUILD_ID}}}
TRIGGER_ID = '32488e7f-09d6-4fe9-a5fb-4ca1419a6e7a'
TEST_BUILD_INSTANCE = dict(id='test-build-id-9832662', status=3, steps=[{'name': 'ubuntu', 'env': [], 'args': [], 'dir_': '', 'id': '', 'wait_for': [], 'entrypoint': '', 'secret_env': [], 'volumes': [], 'status': 0, 'script': ''}], name='', project_id='', status_detail='', images=[], logs_bucket='', build_trigger_id='', log_url='', substitutions={}, tags=[], secrets=[], timing={}, service_account='', warnings=[])
