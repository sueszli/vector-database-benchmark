...
STREAM_WEIGHTS = {'low': 240, 'mid': 420, 'high': 720, 'ultra': 1080}
STREAM_NAMES = {'120k': 'low', '328k': 'mid', '864k': 'high'}
_api_schema = validate.Schema({'error': bool, validate.optional('code'): str, validate.optional('message'): str, validate.optional('data'): object})
_media_schema = validate.Schema({validate.optional('name'): validate.any(str, None), validate.optional('series_name'): validate.any(str, None), validate.optional('media_type'): validate.any(str, None), 'stream_data': validate.any(None, {'streams': validate.all([{'quality': validate.any(str, None), 'url': validate.url(scheme='http', path=validate.endswith('.m3u8')), validate.optional('video_encode_id'): str}])})})
_login_schema = validate.Schema({'auth': validate.any(str, None), 'expires': validate.all(str, validate.transform(parse_datetime)), 'user': {'username': validate.any(str, None), 'email': str}})
_session_schema = validate.Schema({'session_id': str}, validate.get('session_id'))
