...
def main(tokens_per_request: int, batch_size: int, num_replicas: int, num_trials: int, trial_runtime: float):
    if False:
        for i in range(10):
            print('nop')
    app = Caller.bind(Downstream.options(num_replicas=num_replicas).bind(tokens_per_request), tokens_per_request=tokens_per_request, batch_size=batch_size, num_trials=num_trials, trial_runtime=trial_runtime)
    h = serve.run(app).options(use_new_handle_api=True)
    (mean, stddev) = h.run_benchmark.remote().result()
    print('DeploymentHandle streaming throughput {}: {} +- {} tokens/s'.format(f'(num_replicas={num_replicas}, tokens_per_request={tokens_per_request}, batch_size={batch_size})', mean, stddev))
