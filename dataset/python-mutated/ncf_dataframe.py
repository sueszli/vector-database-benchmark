...
if cluster_mode == 'local':
    ...
    init_orca_context(cluster_mode='k8s', master=master, container_image=image_name_k8s, num_nodes=1, memory='128g', cores=4)
...
df = spark.read.csv(new_rating_files, sep=':', header=True, inferSchema=True).toDF('user', 'item', 'label', 'timestamp')
...
epochs = 1
model_dir = './logs-ncf'
estimator = Estimator.from_graph(inputs=[model.user, model.item], outputs=[model.class_number], labels=[model.label], loss=model.loss, optimizer=model.optim, model_dir=model_dir, metrics={'loss': model.loss})
print('INFO estimator created success!')
estimator.fit(data=train_data, batch_size=batch_size, epochs=epochs, feature_cols=['user', 'item'], label_cols=['label'], validation_data=test_data)
