...
def train_mlp(model, shard_level, use_multi_precision, output_dir, amp_level='O1', sync_buffers=False, dp_group=None):
    if False:
        for i in range(10):
            print('nop')
    ...
    (model, optimizer, scaler) = group_sharded_parallel(model=model, optimizer=optimizer, level=shard_level, scaler=scaler, sync_buffers=sync_buffers, dp_group=dp_group)
    if shard_level == 'os_g':
