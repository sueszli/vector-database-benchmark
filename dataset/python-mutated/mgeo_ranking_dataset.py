...
class MGeoRankingDataset(TorchCustomDataset):
    ...
    def __get_test_item__(self, index):
        ...
        if 'gis' in self.text_fileds:
            ...
            pos_sequences_gis = [json.loads(ele['gis']) for ele in group[self.pos_sequence]]
            neg_sequences_gis = [json.loads(ele['gis']) for ele in group[self.neg_sequence]]
            ...
            examples_gis = None
        sample = {'qid': torch.LongTensor([int(qid)] * len(labels)), self.preprocessor.first_sequence: qry, self.preprocessor.second_sequence: examples, self.preprocessor.first_sequence_gis: qry_gis, self.preprocessor.second_sequence_gis: examples_gis, 'labels': torch.LongTensor(labels)}
    ...
    def __get_train_item__(self, index):
        ...
        if 'gis' in self.text_fileds:
            ...
            neg_sequences_gis = None
            examples_gis = None
        sample = {self.preprocessor.first_sequence: qry, self.preprocessor.second_sequence: examples, self.preprocessor.first_sequence_gis: qry_gis, self.preprocessor.second_sequence_gis: examples_gis}
        return self.prepare_sample(sample)
