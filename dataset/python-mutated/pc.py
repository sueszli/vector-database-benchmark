...
class ProcedureCloningBFSPolicy(Policy):
    ...
    config = dict(type='pc', cuda=False, on_policy=False, continuous=False, max_bfs_steps=100, learn=dict(update_per_collect=1, batch_size=32, learning_rate=1e-05, lr_decay=False, decay_epoch=30, decay_rate=0.1, warmup_lr=0.0001, warmup_epoch=3, optimizer='SGD', momentum=0.9, weight_decay=0.0001), collect=dict(unroll_len=1, noise=False, noise_sigma=0.2, noise_range=dict(min=-0.5, max=0.5)), eval=dict(), other=dict(replay_buffer=dict(replay_buffer_size=10000)))
    ...
    def _init_learn(self):
        ...
        if self._cfg.learn.optimizer == 'SGD':
            self._optimizer = SGD(self._model.parameters(), lr=self._cfg.learn.learning_rate, weight_decay=self._cfg.learn.weight_decay, momentum=self._cfg.learn.momentum)
    ...
    def process_states(self, observations, maze_maps):
        ...
        loc = torch.nn.functional.one_hot((observations[:, 0] * self._maze_size + observations[:, 1]).long(), self._maze_size * self._maze_size).long()
