...
def build_get_request(scope: str, operation_id: str, **kwargs: Any) -> HttpRequest:
    ...
    _url = kwargs.pop('template_url', '/{scope}/providers/Microsoft.Authorization/roleManagementAlertOperations/{operationId}')
    path_format_arguments = {'scope': _SERIALIZER.url('scope', scope, 'str', skip_quote=True), 'operationId': _SERIALIZER.url('operation_id', operation_id, 'str', skip_quote=True)}
...
class AlertOperationOperations:
    ...
    def get(self, scope: str, operation_id: str, **kwargs: Any) -> _models.AlertOperationResult:
        ...
        request = build_get_request(scope=scope, operation_id=operation_id, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
