...
from airflow.providers.amazon.aws.hooks.eks import ClusterStates, EksHook, FargateProfileStates, NodegroupStates
...
FARGATE_TERMINAL_STATES = frozenset({FargateProfileStates.ACTIVE, FargateProfileStates.CREATE_FAILED, FargateProfileStates.DELETE_FAILED, FargateProfileStates.NONEXISTENT})
NODEGROUP_TERMINAL_STATES = frozenset({NodegroupStates.ACTIVE, NodegroupStates.CREATE_FAILED, NodegroupStates.DELETE_FAILED, NodegroupStates.NONEXISTENT})
...
class EksFargateProfileStateSensor(EksBaseSensor):
    ...
    :param aws_conn_id: The Airflow connection used for AWS credentials. (templated)
         ...
         maintained on each worker node).
    """
    template_fields: Sequence[str] = ('cluster_name', 'fargate_profile_name', 'target_state', 'aws_conn_id', 'region')
    ui_color = '#ff9900'
    ui_fgcolor = '#232F3E'
...
class EksNodegroupStateSensor(EksBaseSensor):
    ...
    :param aws_conn_id: The Airflow connection used for AWS credentials. (templated)
         ...
         maintained on each worker node).
    """
    template_fields: Sequence[str] = ('cluster_name', 'nodegroup_name', 'target_state', 'aws_conn_id', 'region')
    ui_color = '#ff9900'
    ui_fgcolor = '#232F3E'
