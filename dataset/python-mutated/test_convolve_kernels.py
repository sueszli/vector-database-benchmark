...
from astropy.convolution.kernels import Box2DKernel, Gaussian2DKernel, Moffat2DKernel, Tophat2DKernel
...
for shape in SHAPES_ODD + NOSHAPE:
    for width in WIDTHS:
        KERNELS.append(Gaussian2DKernel(x_stddev=width, x_size=shape[0], y_size=shape[1], mode='oversample', factor=10))
        KERNELS.append(Box2DKernel(width=width, x_size=shape[0], y_size=shape[1], mode='oversample', factor=10))
        KERNELS.append(Tophat2DKernel(radius=width, x_size=shape[0], y_size=shape[1], mode='oversample', factor=10))
        KERNELS.append(Moffat2DKernel(gamma=width, alpha=2, x_size=shape[0], y_size=shape[1], mode='oversample', factor=10))
