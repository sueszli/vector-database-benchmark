...
class RegNetModelTester:
    ...
    def get_config(self):
        if False:
            while True:
                i = 10
        return RegNetConfig(num_channels=self.num_channels, embeddings_size=self.embeddings_size, hidden_sizes=self.hidden_sizes, depths=self.depths, hidden_act=self.hidden_act, num_labels=self.num_labels)
...
class RegNetModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': RegNetModel, 'image-classification': RegNetForImageClassification} if is_torch_available() else {}
    test_pruning = False
...
class RegNetModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        if False:
            while True:
                i = 10
        return AutoImageProcessor.from_pretrained(REGNET_PRETRAINED_MODEL_ARCHIVE_LIST[0]) if is_vision_available() else None
    ...
    def test_inference_image_classification_head(self):
        ...
        expected_slice = torch.tensor([-0.418, -1.5051, -3.4836]).to(torch_device)
