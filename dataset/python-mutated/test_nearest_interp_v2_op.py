...
def nearest_interp_test(x, OutSize=None, SizeTensor=None, Scale=None, data_layout='NCHW', out_d=-1, out_h=-1, out_w=-1, scale=[], interp_method='nearest', align_corners=True, align_mode=0):
    if False:
        while True:
            i = 10
    ...
    return paddle._C_ops.nearest_interp(x, OutSize, SizeTensor, Scale, data_layout, out_d, out_h, out_w, scale, interp_method, align_corners, align_mode)

def nearest_neighbor_interp_np(X, out_h, out_w, scale_h=0, scale_w=0, out_size=None, actual_shape=None, align_corners=True, data_layout='NCHW'):
    if False:
        while True:
            i = 10
...
def nearest_neighbor_interp3d_np(X, out_d, out_h, out_w, scale_d=0, scale_h=0, scale_w=0, out_size=None, actual_shape=None, align_corners=True, data_layout='NCHW'):
    if False:
        while True:
            i = 10
    'nearest neighbor interpolation implement in shape [N, C, H, W]'
...
class TestNearestInterpOp(OpTest):
    ...
    def setUp(self):
        ...
        if len(self.input_shape) == 4:
            output_np = nearest_neighbor_interp_np(input_np, out_h, out_w, scale_h, scale_w, self.out_size, self.actual_shape, self.align_corners, self.data_layout)
        elif len(self.input_shape) == 5:
            output_np = nearest_neighbor_interp3d_np(input_np, out_d, out_h, out_w, scale_d, scale_h, scale_w, self.out_size, self.actual_shape, self.align_corners, self.data_layout)
        ...
        if len(self.input_shape) == 5:
            self.attrs = {'out_d': self.out_d, 'out_h': self.out_h, 'out_w': self.out_w, 'interp_method': self.interp_method, 'align_corners': self.align_corners, 'data_layout': self.data_layout}
        else:
...
class TestNearestInterpOpUint8(OpTest):
    ...
    def setUp(self):
        ...
        if self.scale:
            ...
            out_h = self.out_h
            out_w = self.out_w
        output_np = nearest_neighbor_interp_np(input_np, out_h, out_w, 0, 0, self.out_size, self.actual_shape, self.align_corners)
        self.inputs = {'X': input_np}
        if self.out_size is not None:
            self.inputs['OutSize'] = self.out_size
        self.attrs = {'out_h': self.out_h, 'out_w': self.out_w, 'interp_method': self.interp_method, 'align_corners': self.align_corners}
        if self.scale:
            if isinstance(self.scale, (float, int)):
                if self.scale > 0:
                    self.scale = [self.scale]
...
class TestNearestInterpOpAPI_dy(unittest.TestCase):
    ...
    def test_case(self):
        ...
        with base.dygraph.guard(place):
            ...
            expect_res = nearest_neighbor_interp_np(input_data, out_h=12, out_w=12, align_corners=False)
            out = interpolate(x=input_x, scale_factor=scale, mode='nearest', align_corners=False)
...
class TestNearestInterp3DOpAPI_dy(unittest.TestCase):
    ...
    def test_case(self):
        ...
        with base.dygraph.guard(place):
            ...
            input_x = paddle.to_tensor(input_data)
            scale = paddle.to_tensor(scale_np)
            expect_res = nearest_neighbor_interp3d_np(input_data, out_d=12, out_h=12, out_w=12, align_corners=False)
            out = interpolate(x=input_x, scale_factor=scale, mode='nearest', align_corners=False, data_format='NCDHW')
            np.testing.assert_allclose(out.numpy(), expect_res, rtol=1e-05)
...
class TestNearestInterp3DOpForFloat16(unittest.TestCase):
    ...
    def check_main(self, x_np, dtype):
        ...
        y = interpolate(x, scale_factor=self.scale, mode=self.interp_method, align_corners=self.align_corners, data_format=self.data_layout)
        x_g = paddle.grad(y, x)
...
class TestNearestInterpOpForFloat16(unittest.TestCase):
    ...
    def check_main(self, x_np, dtype):
        ...
        x = paddle.to_tensor(x_np)
        x.stop_gradient = False
        y = interpolate(x, scale_factor=self.scale, mode=self.interp_method, align_corners=self.align_corners)
        x_g = paddle.grad(y, x)
        y_np = y[0].numpy().astype('float32')
        x_g_np = x_g[0].numpy().astype('float32')
        paddle.enable_static()
