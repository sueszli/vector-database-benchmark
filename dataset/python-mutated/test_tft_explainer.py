...
if TORCH_AVAILABLE:
    ...
    class TestTFTExplainer:
        freq = 'MS'
        series_lin_pos = tg.linear_timeseries(length=10, freq=freq).with_static_covariates(pd.Series([0.0, 0.5], index=['cat', 'num']))
        series_sine = tg.sine_timeseries(length=10, freq=freq)
        series_mv1 = series_lin_pos.stack(series_sine)
        series_lin_neg = tg.linear_timeseries(start_value=1, end_value=0, length=10, freq=freq).with_static_covariates(pd.Series([1.0, 0.5], index=['cat', 'num']))
