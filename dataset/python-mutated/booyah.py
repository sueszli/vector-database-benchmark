...
@pluginmatcher(re.compile('https?://(?:www\\.)?booyah\\.live/(?:(?P<type>channels|clips|vods)/)?(?P<id>[^?]+)'))
class Booyah(Plugin):
    ...
    auth_schema = validate.Schema({'expiry_time': int, 'uid': int})
    vod_schema = validate.Schema({'user': {'nickname': str}, 'playback': {'name': str, 'endpoint_list': [{'stream_url': validate.url(), 'resolution': validate.all(int, validate.transform(lambda x: f'{x}p'))}]}})
    live_schema = validate.Schema({'user': {'nickname': str}, 'channel': {'channel_id': int, 'name': str, 'is_streaming': bool, validate.optional('hostee'): {'channel_id': int, 'nickname': str}}})
