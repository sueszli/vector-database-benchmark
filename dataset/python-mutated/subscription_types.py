...
from ...discount.models import PromotionRuleTranslation, PromotionTranslation, VoucherTranslation
...
from ...payment.interface import ListStoredPaymentMethodsRequestData, PaymentMethodInitializeTokenizationRequestData, PaymentMethodProcessTokenizationRequestData, PaymentMethodTokenizationBaseRequestData, StoredPaymentMethodRequestDeleteData, TransactionActionData, TransactionSessionData
from ...product.models import CategoryTranslation, CollectionTranslation, ProductTranslation, ProductVariantTranslation
...
from ..core.descriptions import ADDED_IN_32, ADDED_IN_34, ADDED_IN_35, ADDED_IN_36, ADDED_IN_37, ADDED_IN_38, ADDED_IN_310, ADDED_IN_311, ADDED_IN_312, ADDED_IN_313, ADDED_IN_314, ADDED_IN_315, ADDED_IN_316, ADDED_IN_317, ADDED_IN_318, DEPRECATED_IN_3X_EVENT, PREVIEW_FEATURE
from ..core.doc_category import DOC_CATEGORY_CHECKOUT, DOC_CATEGORY_DISCOUNTS, DOC_CATEGORY_GIFT_CARDS, DOC_CATEGORY_MISC, DOC_CATEGORY_ORDERS, DOC_CATEGORY_PAYMENTS, DOC_CATEGORY_PRODUCTS, DOC_CATEGORY_SHIPPING, DOC_CATEGORY_TAXES, DOC_CATEGORY_USERS
...
TRANSLATIONS_TYPES_MAP = {ProductTranslation: translation_types.ProductTranslation, CollectionTranslation: translation_types.CollectionTranslation, CategoryTranslation: translation_types.CategoryTranslation, AttributeTranslation: translation_types.AttributeTranslation, AttributeValueTranslation: translation_types.AttributeValueTranslation, ProductVariantTranslation: translation_types.ProductVariantTranslation, PageTranslation: translation_types.PageTranslation, ShippingMethodTranslation: translation_types.ShippingMethodTranslation, VoucherTranslation: translation_types.VoucherTranslation, MenuItemTranslation: translation_types.MenuItemTranslation, PromotionTranslation: translation_types.PromotionTranslation, PromotionRuleTranslation: translation_types.PromotionRuleTranslation}
...
class Event(graphene.Interface):
    ...
    issuing_principal = graphene.Field(IssuingPrincipal, description='The user or application that triggered the event.')
    recipient = graphene.Field('saleor.graphql.app.types.App', description='The application receiving the webhook.')
...
class AccountOperationBase(AbstractType):
    redirect_url = graphene.String(description='The URL to redirect the user after he accepts the request.', required=False)
    user = graphene.Field(UserType, description='The user the event relates to.')
    channel = graphene.Field('saleor.graphql.channel.types.Channel', description='The channel data.')
...
class AccountConfirmationRequested(SubscriptionObjectType, AccountOperationBase):
    ...
    class Meta:
        ...
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when account confirmation requested. This event is always sent. enableAccountConfirmationByEmail flag set to True is not required.' + ADDED_IN_315
        doc_category = DOC_CATEGORY_USERS

class AccountChangeEmailRequested(SubscriptionObjectType, AccountOperationBase):
    new_email = graphene.String(description='The new email address the user wants to change to.')

    class Meta:
        root_type = None
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when account change email is requested.' + ADDED_IN_315
        doc_category = DOC_CATEGORY_USERS
...
class AccountEmailChanged(SubscriptionObjectType, AccountOperationBase):
    new_email = graphene.String(description='The new email address.')
...
class AccountSetPasswordRequested(SubscriptionObjectType, AccountOperationBase):
    ...
    class Meta:
        ...
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when setting a new password is requested.' + ADDED_IN_315
        doc_category = DOC_CATEGORY_USERS
...
class AddressBase(AbstractType):
    address = graphene.Field('saleor.graphql.account.types.Address', description='The address the event relates to.')
...
class AppBase(AbstractType):
    app = graphene.Field('saleor.graphql.app.types.App', description='The application the event relates to.')
...
class AttributeBase(AbstractType):
    attribute = graphene.Field('saleor.graphql.attribute.types.Attribute', description='The attribute the event relates to.')
...
class AttributeValueBase(AbstractType):
    attribute_value = graphene.Field('saleor.graphql.attribute.types.AttributeValue', description='The attribute value the event relates to.')
...
class CategoryBase(AbstractType):
    category = graphene.Field('saleor.graphql.product.types.Category', description='The category the event relates to.')
...
class ChannelBase(AbstractType):
    channel = graphene.Field('saleor.graphql.channel.types.Channel', description='The channel the event relates to.')
...
class OrderBase(AbstractType):
    order = graphene.Field('saleor.graphql.order.types.Order', description='The order the event relates to.')
...
class OrderPaid(SubscriptionObjectType, OrderBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Payment has been made. The order may be partially or fully paid.' + ADDED_IN_314 + PREVIEW_FEATURE
...
class OrderRefunded(SubscriptionObjectType, OrderBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'The order received a refund. The order may be partially or fully refunded.' + ADDED_IN_314 + PREVIEW_FEATURE
...
class OrderExpired(SubscriptionObjectType, OrderBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when order becomes expired.' + ADDED_IN_313 + PREVIEW_FEATURE
...
class OrderBulkCreated(SubscriptionObjectType):
    orders = NonNullList(Order, description='The orders the event relates to.')
    ...
    class Meta:
        ...
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when orders are imported.' + ADDED_IN_314 + PREVIEW_FEATURE
...
class GiftCardBase(AbstractType):
    gift_card = graphene.Field('saleor.graphql.giftcard.types.GiftCard', description='The gift card the event relates to.')
...
class GiftCardSent(SubscriptionObjectType, GiftCardBase):
    channel = graphene.String(description='Slug of a channel for which this gift card email was sent.')
    sent_to_email = graphene.String(description='E-mail address to which gift card was sent.')

    class Meta:
        root_type = None
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when gift card is e-mailed.' + ADDED_IN_313 + PREVIEW_FEATURE
...
class GiftCardExportCompleted(SubscriptionObjectType):
    export = graphene.Field('saleor.graphql.csv.types.ExportFile', description='The export file for gift cards.')
...
class MenuBase(AbstractType):
    menu = graphene.Field('saleor.graphql.menu.types.Menu', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The menu the event relates to.')
...
class MenuItemBase(AbstractType):
    menu_item = graphene.Field('saleor.graphql.menu.types.MenuItem', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The menu item the event relates to.')
...
class ProductBase(AbstractType):
    product = graphene.Field('saleor.graphql.product.types.Product', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The product the event relates to.')
    category = graphene.Field('saleor.graphql.product.types.categories.Category', description='The category of the product.')
...
class ProductMediaBase(AbstractType):
    product_media = graphene.Field('saleor.graphql.product.types.ProductMedia', description='The product media the event relates to.')
...
class ProductVariantBase(AbstractType):
    product_variant = graphene.Field('saleor.graphql.product.types.ProductVariant', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The product variant the event relates to.')
...
class ProductVariantMetadataUpdated(SubscriptionObjectType, ProductVariantBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when product variant metadata is updated.' + ADDED_IN_38

class ProductVariantOutOfStock(SubscriptionObjectType, ProductVariantBase):
    warehouse = graphene.Field('saleor.graphql.warehouse.types.Warehouse', description='Look up a warehouse.')
...
class ProductVariantStockUpdated(SubscriptionObjectType, ProductVariantBase):
    ...
    class Meta:
        ...
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when product variant stock is updated.' + ADDED_IN_311 + PREVIEW_FEATURE
...
class ProductExportCompleted(SubscriptionObjectType):
    export = graphene.Field('saleor.graphql.csv.types.ExportFile', description='The export file for products.')
...
class SaleBase(AbstractType):
    sale = graphene.Field('saleor.graphql.discount.types.Sale', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The sale the event relates to.')
...
class SaleCreated(SubscriptionObjectType, SaleBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when new sale is created.' + ADDED_IN_32 + DEPRECATED_IN_3X_EVENT + ' Use `PromotionCreated` event instead.'
...
class SaleUpdated(SubscriptionObjectType, SaleBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when sale is updated.' + ADDED_IN_32 + DEPRECATED_IN_3X_EVENT + ' Use `PromotionUpdated` event instead.'
...
class SaleDeleted(SubscriptionObjectType, SaleBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when sale is deleted.' + ADDED_IN_32 + DEPRECATED_IN_3X_EVENT + ' Use `PromotionDeleted` event instead.'

class SaleToggle(SubscriptionObjectType, SaleBase):
    sale = graphene.Field('saleor.graphql.discount.types.Sale', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The sale the event relates to.' + ADDED_IN_35)

    class Meta:
        root_type = 'Sale'
        enable_dry_run = True
        description = 'The event informs about the start or end of the sale.' + ADDED_IN_35 + DEPRECATED_IN_3X_EVENT + ' Use `PromotionStarted` and `PromotionEnded` events instead.'
        interfaces = (Event,)

class PromotionBase(AbstractType):
    promotion = graphene.Field('saleor.graphql.discount.types.Promotion', description='The promotion the event relates to.')
...
class PromotionCreated(SubscriptionObjectType, PromotionBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when new promotion is created.' + ADDED_IN_317 + PREVIEW_FEATURE
...
class PromotionUpdated(SubscriptionObjectType, PromotionBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when promotion is updated.' + ADDED_IN_317 + PREVIEW_FEATURE
...
class PromotionDeleted(SubscriptionObjectType, PromotionBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when promotion is deleted.' + ADDED_IN_317 + PREVIEW_FEATURE
...
class PromotionStarted(SubscriptionObjectType, PromotionBase):
    ...
    class Meta:
        root_type = 'Promotion'
        enable_dry_run = True
        description = 'The event informs about the start of the promotion.' + ADDED_IN_317 + PREVIEW_FEATURE
        interfaces = (Event,)
...
class PromotionEnded(SubscriptionObjectType, PromotionBase):
    ...
    class Meta:
        root_type = 'Promotion'
        enable_dry_run = True
        description = 'The event informs about the end of the promotion.' + ADDED_IN_317 + PREVIEW_FEATURE
        interfaces = (Event,)

class PromotionRuleBase(AbstractType):
    promotion_rule = graphene.Field('saleor.graphql.discount.types.PromotionRule', description='The promotion rule the event relates to.')
...
class PromotionRuleCreated(SubscriptionObjectType, PromotionRuleBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when new promotion rule is created.' + ADDED_IN_317 + PREVIEW_FEATURE
...
class PromotionRuleUpdated(SubscriptionObjectType, PromotionRuleBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when new promotion rule is updated.' + ADDED_IN_317 + PREVIEW_FEATURE
...
class PromotionRuleDeleted(SubscriptionObjectType, PromotionRuleBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when new promotion rule is deleted.' + ADDED_IN_317 + PREVIEW_FEATURE

class InvoiceBase(AbstractType):
    invoice = graphene.Field('saleor.graphql.invoice.types.Invoice', description='The invoice the event relates to.')
    order = graphene.Field('saleor.graphql.order.types.Order', description='Order related to the invoice.' + ADDED_IN_310)
...
class InvoiceRequested(SubscriptionObjectType, InvoiceBase):
    order = graphene.Field('saleor.graphql.order.types.Order', required=True, description='Order related to the invoice.' + ADDED_IN_310)
...
class FulfillmentBase(AbstractType):
    fulfillment = graphene.Field('saleor.graphql.order.types.Fulfillment', description='The fulfillment the event relates to.')
    order = graphene.Field('saleor.graphql.order.types.Order', description='The order the fulfillment belongs to.')
...
class FulfillmentCreated(SubscriptionObjectType, FulfillmentBase):
    notify_customer = graphene.Boolean(description='If true, the app should send a notification to the customer.' + ADDED_IN_316, required=True)
...
class FulfillmentApproved(SubscriptionObjectType, FulfillmentBase):
    notify_customer = graphene.Boolean(description='If true, send a notification to the customer.' + ADDED_IN_316, required=True)
...
class UserBase(AbstractType):
    user = graphene.Field('saleor.graphql.account.types.User', description='The user the event relates to.')
...
class CollectionBase(AbstractType):
    collection = graphene.Field('saleor.graphql.product.types.collections.Collection', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The collection the event relates to.')
...
class CheckoutBase(AbstractType):
    checkout = graphene.Field('saleor.graphql.checkout.types.Checkout', description='The checkout the event relates to.')
...
class CheckoutFullyPaid(SubscriptionObjectType, CheckoutBase):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when checkout is fully paid with transactions.' + ADDED_IN_313 + PREVIEW_FEATURE
...
class PageBase(AbstractType):
    page = graphene.Field('saleor.graphql.page.types.Page', description='The page the event relates to.')
...
class PageTypeBase(AbstractType):
    page_type = graphene.Field('saleor.graphql.page.types.PageType', description='The page type the event relates to.')
...
class PermissionGroupBase(AbstractType):
    permission_group = graphene.Field('saleor.graphql.account.types.Group', description='The permission group the event relates to.')
...
class ShippingPriceBase(AbstractType):
    shipping_method = graphene.Field('saleor.graphql.shipping.types.ShippingMethodType', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The shipping method the event relates to.')
    shipping_zone = graphene.Field('saleor.graphql.shipping.types.ShippingZone', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The shipping zone the shipping method belongs to.')
...
class ShippingZoneBase(AbstractType):
    shipping_zone = graphene.Field('saleor.graphql.shipping.types.ShippingZone', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The shipping zone the event relates to.')
...
class StaffSetPasswordRequested(SubscriptionObjectType, AccountOperationBase):
    ...
    class Meta:
        ...
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when setting a new password for staff is requested.' + ADDED_IN_315
        doc_category = DOC_CATEGORY_USERS

class TransactionAction(SubscriptionObjectType, AbstractType):
    action_type = graphene.Field(TransactionActionEnum, required=True, description='Determines the action type.')
    amount = PositiveDecimal(description='Transaction request amount. Null when action type is VOID.')
    currency = graphene.String(description='Currency code.' + ADDED_IN_316, required=True)
...
class TransactionActionBase(AbstractType):
    transaction = graphene.Field(TransactionItem, description='Look up a transaction.')
    action = graphene.Field(TransactionAction, required=True, description='Requested action data.')
...
class TransactionChargeRequested(TransactionActionBase, SubscriptionObjectType):
    ...
    class Meta:
        ...
        root_type = None
        enable_dry_run = False
        description = 'Event sent when transaction charge is requested.' + ADDED_IN_313 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS

class TransactionRefundRequested(TransactionActionBase, SubscriptionObjectType):
    granted_refund = graphene.Field(OrderGrantedRefund, description='Granted refund related to refund request.' + ADDED_IN_315 + PREVIEW_FEATURE)

    class Meta:
        interfaces = (Event,)
        root_type = None
        enable_dry_run = False
        description = 'Event sent when transaction refund is requested.' + ADDED_IN_313 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS
...
class TransactionCancelationRequested(TransactionActionBase, SubscriptionObjectType):
    ...
    class Meta:
        ...
        root_type = None
        enable_dry_run = False
        description = 'Event sent when transaction cancelation is requested.' + ADDED_IN_313 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS

class PaymentGatewayInitializeSession(SubscriptionObjectType):
    source_object = graphene.Field(OrderOrCheckout, description='Checkout or order', required=True)
    data = graphene.Field(JSON, description='Payment gateway data in JSON format, received from storefront.')
    amount = graphene.Field(PositiveDecimal, description='Amount requested for initializing the payment gateway.')

    class Meta:
        interfaces = (Event,)
        root_type = None
        enable_dry_run = False
        description = 'Event sent when user wants to initialize the payment gateway.' + ADDED_IN_313 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS
...
class TransactionProcessAction(SubscriptionObjectType, AbstractType):
    amount = PositiveDecimal(description='Transaction amount to process.', required=True)
...
class TransactionSessionBase(SubscriptionObjectType, AbstractType):
    transaction = graphene.Field(TransactionItem, description='Look up a transaction.', required=True)
    source_object = graphene.Field(OrderOrCheckout, description='Checkout or order', required=True)
    data = graphene.Field(JSON, description='Payment gateway data in JSON format, received from storefront.')
    merchant_reference = graphene.String(description='Merchant reference assigned to this payment.', required=True)
    customer_ip_address = graphene.String(description="The customer's IP address. If not provided as a parameter in the mutation, Saleor will try to determine the customer's IP address on its own." + ADDED_IN_316)
    action = graphene.Field(TransactionProcessAction, description='Action to proceed for the transaction', required=True)
...
class TransactionInitializeSession(TransactionSessionBase):
    ...
    class Meta:
        ...
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when user starts processing the payment.' + ADDED_IN_313 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS
...
class TransactionProcessSession(TransactionSessionBase):
    ...
    class Meta:
        ...
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when user has additional payment action to process.' + ADDED_IN_313 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS

class ListStoredPaymentMethods(SubscriptionObjectType):
    user = graphene.Field(UserType, description='The user for which the app should return a list of payment methods.', required=True)
    channel = graphene.Field('saleor.graphql.channel.types.Channel', description='Channel in context which was used to fetch the list of payment methods.', required=True)

    class Meta:
        root_type = None
        enable_dry_run = False
        interfaces = (Event,)
        description = 'List payment methods stored for the user by payment gateway.' + ADDED_IN_315 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS
...
class TransactionItemMetadataUpdated(SubscriptionObjectType):
    ...
    class Meta:
        ...
        enable_dry_run = True
        interfaces = (Event,)
        description = 'Event sent when transaction item metadata is updated.' + ADDED_IN_38
        doc_category = DOC_CATEGORY_PAYMENTS
...
class StoredPaymentMethodDeleteRequested(SubscriptionObjectType):
    user = graphene.Field(UserType, description='The user for which the app should proceed with payment method delete request.', required=True)
    payment_method_id = graphene.Field(graphene.String, description='The ID of the payment method that should be deleted by the payment gateway.', required=True)
    channel = graphene.Field('saleor.graphql.channel.types.Channel', description='Channel related to the requested delete action.', required=True)

    class Meta:
        root_type = None
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when user requests to delete a payment method.' + ADDED_IN_316 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS
...
class PaymentMethodTokenizationBase(AbstractType):
    user = graphene.Field(UserType, description='The user related to the requested action.', required=True)
    channel = graphene.Field('saleor.graphql.channel.types.Channel', description='Channel related to the requested action.', required=True)
    data = graphene.Field(JSON, description='Payment gateway data in JSON format, received from storefront.')
...
class PaymentGatewayInitializeTokenizationSession(SubscriptionObjectType, PaymentMethodTokenizationBase):

    class Meta:
        root_type = None
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent to initialize a new session in payment gateway to store the payment method. ' + ADDED_IN_316 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS

class PaymentMethodInitializeTokenizationSession(SubscriptionObjectType, PaymentMethodTokenizationBase):
    payment_flow_to_support = TokenizedPaymentFlowEnum(description='The payment flow that the tokenized payment method should support.', required=True)

    class Meta:
        root_type = None
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when user requests a tokenization of payment method.' + ADDED_IN_316 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS
...
class PaymentMethodProcessTokenizationSession(SubscriptionObjectType, PaymentMethodTokenizationBase):
    id = graphene.String(description='The ID returned by app from `PAYMENT_METHOD_INITIALIZE_TOKENIZATION_SESSION` webhook.', required=True)

    class Meta:
        root_type = None
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Event sent when user continues a tokenization of payment method.' + ADDED_IN_316 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PAYMENTS
...
class TranslationTypes(Union):
    ...
    class Meta:
        types = tuple(TRANSLATIONS_TYPES_MAP.values()) + (translation_types.SaleTranslation,)
...
class TranslationBase(AbstractType):
    translation = graphene.Field(TranslationTypes, description='The translation the event relates to.')
...
class VoucherBase(AbstractType):
    voucher = graphene.Field('saleor.graphql.discount.types.Voucher', channel=graphene.String(description='Slug of a channel for which the data should be returned.'), description='The voucher the event relates to.')
...
class VoucherCodeExportCompleted(SubscriptionObjectType):
    export = graphene.Field('saleor.graphql.csv.types.ExportFile', description='The export file for voucher codes.')
...
class PaymentBase(AbstractType):
    payment = graphene.Field('saleor.graphql.payment.types.Payment', description='Look up a payment.')
...
class ShippingListMethodsForCheckout(SubscriptionObjectType, CheckoutBase):
    shipping_methods = NonNullList(ShippingMethod, description='Shipping methods that can be used with this checkout.' + ADDED_IN_36)
...
class CalculateTaxes(SubscriptionObjectType):
    ...
    class Meta:
        ...
        enable_dry_run = False
        interfaces = (Event,)
        description = 'Synchronous webhook for calculating checkout/order taxes.' + ADDED_IN_37
...
class WarehouseBase(AbstractType):
    warehouse = graphene.Field('saleor.graphql.warehouse.types.Warehouse', description='The warehouse the event relates to.')
...
class Subscription(SubscriptionObjectType):
    event = graphene.Field(Event, description='Look up subscription event.' + ADDED_IN_32)
...
class ThumbnailCreated(SubscriptionObjectType):
    ...
    object_id = graphene.ID(description='Object the thumbnail refers to.' + ADDED_IN_312)
...
WEBHOOK_TYPES_MAP = {WebhookEventAsyncType.ACCOUNT_CONFIRMATION_REQUESTED: AccountConfirmationRequested, WebhookEventAsyncType.ACCOUNT_CHANGE_EMAIL_REQUESTED: AccountChangeEmailRequested, WebhookEventAsyncType.ACCOUNT_EMAIL_CHANGED: AccountEmailChanged, WebhookEventAsyncType.ACCOUNT_SET_PASSWORD_REQUESTED: AccountSetPasswordRequested, WebhookEventAsyncType.ACCOUNT_CONFIRMED: AccountConfirmed, WebhookEventAsyncType.ACCOUNT_DELETE_REQUESTED: AccountDeleteRequested, WebhookEventAsyncType.ACCOUNT_DELETED: AccountDeleted, WebhookEventAsyncType.ADDRESS_CREATED: AddressCreated, WebhookEventAsyncType.ADDRESS_UPDATED: AddressUpdated, WebhookEventAsyncType.ADDRESS_DELETED: AddressDeleted, WebhookEventAsyncType.APP_INSTALLED: AppInstalled, WebhookEventAsyncType.APP_UPDATED: AppUpdated, WebhookEventAsyncType.APP_DELETED: AppDeleted, WebhookEventAsyncType.APP_STATUS_CHANGED: AppStatusChanged, WebhookEventAsyncType.ATTRIBUTE_CREATED: AttributeCreated, WebhookEventAsyncType.ATTRIBUTE_UPDATED: AttributeUpdated, WebhookEventAsyncType.ATTRIBUTE_DELETED: AttributeDeleted, WebhookEventAsyncType.ATTRIBUTE_VALUE_CREATED: AttributeValueCreated, WebhookEventAsyncType.ATTRIBUTE_VALUE_UPDATED: AttributeValueUpdated, WebhookEventAsyncType.ATTRIBUTE_VALUE_DELETED: AttributeValueDeleted, WebhookEventAsyncType.CATEGORY_CREATED: CategoryCreated, WebhookEventAsyncType.CATEGORY_UPDATED: CategoryUpdated, WebhookEventAsyncType.CATEGORY_DELETED: CategoryDeleted, WebhookEventAsyncType.CHANNEL_CREATED: ChannelCreated, WebhookEventAsyncType.CHANNEL_UPDATED: ChannelUpdated, WebhookEventAsyncType.CHANNEL_DELETED: ChannelDeleted, WebhookEventAsyncType.CHANNEL_STATUS_CHANGED: ChannelStatusChanged, WebhookEventAsyncType.CHANNEL_METADATA_UPDATED: ChannelMetadataUpdated, WebhookEventAsyncType.GIFT_CARD_CREATED: GiftCardCreated, WebhookEventAsyncType.GIFT_CARD_UPDATED: GiftCardUpdated, WebhookEventAsyncType.GIFT_CARD_DELETED: GiftCardDeleted, WebhookEventAsyncType.GIFT_CARD_SENT: GiftCardSent, WebhookEventAsyncType.GIFT_CARD_STATUS_CHANGED: GiftCardStatusChanged, WebhookEventAsyncType.GIFT_CARD_METADATA_UPDATED: GiftCardMetadataUpdated, WebhookEventAsyncType.GIFT_CARD_EXPORT_COMPLETED: GiftCardExportCompleted, WebhookEventAsyncType.MENU_CREATED: MenuCreated, WebhookEventAsyncType.MENU_UPDATED: MenuUpdated, WebhookEventAsyncType.MENU_DELETED: MenuDeleted, WebhookEventAsyncType.MENU_ITEM_CREATED: MenuItemCreated, WebhookEventAsyncType.MENU_ITEM_UPDATED: MenuItemUpdated, WebhookEventAsyncType.MENU_ITEM_DELETED: MenuItemDeleted, WebhookEventAsyncType.ORDER_CREATED: OrderCreated, WebhookEventAsyncType.ORDER_UPDATED: OrderUpdated, WebhookEventAsyncType.ORDER_CONFIRMED: OrderConfirmed, WebhookEventAsyncType.ORDER_FULLY_PAID: OrderFullyPaid, WebhookEventAsyncType.ORDER_PAID: OrderPaid, WebhookEventAsyncType.ORDER_REFUNDED: OrderRefunded, WebhookEventAsyncType.ORDER_FULLY_REFUNDED: OrderFullyRefunded, WebhookEventAsyncType.ORDER_FULFILLED: OrderFulfilled, WebhookEventAsyncType.ORDER_CANCELLED: OrderCancelled, WebhookEventAsyncType.ORDER_EXPIRED: OrderExpired, WebhookEventAsyncType.ORDER_METADATA_UPDATED: OrderMetadataUpdated, WebhookEventAsyncType.ORDER_BULK_CREATED: OrderBulkCreated, WebhookEventAsyncType.DRAFT_ORDER_CREATED: DraftOrderCreated, WebhookEventAsyncType.DRAFT_ORDER_UPDATED: DraftOrderUpdated, WebhookEventAsyncType.DRAFT_ORDER_DELETED: DraftOrderDeleted, WebhookEventAsyncType.PRODUCT_CREATED: ProductCreated, WebhookEventAsyncType.PRODUCT_UPDATED: ProductUpdated, WebhookEventAsyncType.PRODUCT_DELETED: ProductDeleted, WebhookEventAsyncType.PRODUCT_METADATA_UPDATED: ProductMetadataUpdated, WebhookEventAsyncType.PRODUCT_EXPORT_COMPLETED: ProductExportCompleted, WebhookEventAsyncType.PRODUCT_MEDIA_CREATED: ProductMediaCreated, WebhookEventAsyncType.PRODUCT_MEDIA_UPDATED: ProductMediaUpdated, WebhookEventAsyncType.PRODUCT_MEDIA_DELETED: ProductMediaDeleted, WebhookEventAsyncType.PRODUCT_VARIANT_CREATED: ProductVariantCreated, WebhookEventAsyncType.PRODUCT_VARIANT_UPDATED: ProductVariantUpdated, WebhookEventAsyncType.PRODUCT_VARIANT_OUT_OF_STOCK: ProductVariantOutOfStock, WebhookEventAsyncType.PRODUCT_VARIANT_BACK_IN_STOCK: ProductVariantBackInStock, WebhookEventAsyncType.PRODUCT_VARIANT_STOCK_UPDATED: ProductVariantStockUpdated, WebhookEventAsyncType.PRODUCT_VARIANT_DELETED: ProductVariantDeleted, WebhookEventAsyncType.PRODUCT_VARIANT_METADATA_UPDATED: ProductVariantMetadataUpdated, WebhookEventAsyncType.SALE_CREATED: SaleCreated, WebhookEventAsyncType.SALE_UPDATED: SaleUpdated, WebhookEventAsyncType.SALE_DELETED: SaleDeleted, WebhookEventAsyncType.SALE_TOGGLE: SaleToggle, WebhookEventAsyncType.PROMOTION_CREATED: PromotionCreated, WebhookEventAsyncType.PROMOTION_UPDATED: PromotionUpdated, WebhookEventAsyncType.PROMOTION_DELETED: PromotionDeleted, WebhookEventAsyncType.PROMOTION_STARTED: PromotionStarted, WebhookEventAsyncType.PROMOTION_ENDED: PromotionEnded, WebhookEventAsyncType.PROMOTION_RULE_CREATED: PromotionRuleCreated, WebhookEventAsyncType.PROMOTION_RULE_UPDATED: PromotionRuleUpdated, WebhookEventAsyncType.PROMOTION_RULE_DELETED: PromotionRuleDeleted, WebhookEventAsyncType.INVOICE_REQUESTED: InvoiceRequested, WebhookEventAsyncType.INVOICE_DELETED: InvoiceDeleted, WebhookEventAsyncType.INVOICE_SENT: InvoiceSent, WebhookEventAsyncType.FULFILLMENT_CREATED: FulfillmentCreated, WebhookEventAsyncType.FULFILLMENT_TRACKING_NUMBER_UPDATED: FulfillmentTrackingNumberUpdated, WebhookEventAsyncType.FULFILLMENT_CANCELED: FulfillmentCanceled, WebhookEventAsyncType.FULFILLMENT_APPROVED: FulfillmentApproved, WebhookEventAsyncType.FULFILLMENT_METADATA_UPDATED: FulfillmentMetadataUpdated, WebhookEventAsyncType.CUSTOMER_CREATED: CustomerCreated, WebhookEventAsyncType.CUSTOMER_UPDATED: CustomerUpdated, WebhookEventAsyncType.CUSTOMER_METADATA_UPDATED: CustomerMetadataUpdated, WebhookEventAsyncType.COLLECTION_CREATED: CollectionCreated, WebhookEventAsyncType.COLLECTION_UPDATED: CollectionUpdated, WebhookEventAsyncType.COLLECTION_DELETED: CollectionDeleted, WebhookEventAsyncType.COLLECTION_METADATA_UPDATED: CollectionMetadataUpdated, WebhookEventAsyncType.CHECKOUT_CREATED: CheckoutCreated, WebhookEventAsyncType.CHECKOUT_UPDATED: CheckoutUpdated, WebhookEventAsyncType.CHECKOUT_FULLY_PAID: CheckoutFullyPaid, WebhookEventAsyncType.CHECKOUT_METADATA_UPDATED: CheckoutMetadataUpdated, WebhookEventAsyncType.PAGE_CREATED: PageCreated, WebhookEventAsyncType.PAGE_UPDATED: PageUpdated, WebhookEventAsyncType.PAGE_DELETED: PageDeleted, WebhookEventAsyncType.PAGE_TYPE_CREATED: PageTypeCreated, WebhookEventAsyncType.PAGE_TYPE_UPDATED: PageTypeUpdated, WebhookEventAsyncType.PAGE_TYPE_DELETED: PageTypeDeleted, WebhookEventAsyncType.PERMISSION_GROUP_CREATED: PermissionGroupCreated, WebhookEventAsyncType.PERMISSION_GROUP_UPDATED: PermissionGroupUpdated, WebhookEventAsyncType.PERMISSION_GROUP_DELETED: PermissionGroupDeleted, WebhookEventAsyncType.SHIPPING_PRICE_CREATED: ShippingPriceCreated, WebhookEventAsyncType.SHIPPING_PRICE_UPDATED: ShippingPriceUpdated, WebhookEventAsyncType.SHIPPING_PRICE_DELETED: ShippingPriceDeleted, WebhookEventAsyncType.SHIPPING_ZONE_CREATED: ShippingZoneCreated, WebhookEventAsyncType.SHIPPING_ZONE_UPDATED: ShippingZoneUpdated, WebhookEventAsyncType.SHIPPING_ZONE_DELETED: ShippingZoneDeleted, WebhookEventAsyncType.SHIPPING_ZONE_METADATA_UPDATED: ShippingZoneMetadataUpdated, WebhookEventAsyncType.STAFF_CREATED: StaffCreated, WebhookEventAsyncType.STAFF_UPDATED: StaffUpdated, WebhookEventAsyncType.STAFF_DELETED: StaffDeleted, WebhookEventAsyncType.STAFF_SET_PASSWORD_REQUESTED: StaffSetPasswordRequested, WebhookEventAsyncType.TRANSACTION_ITEM_METADATA_UPDATED: TransactionItemMetadataUpdated, WebhookEventAsyncType.TRANSLATION_CREATED: TranslationCreated, WebhookEventAsyncType.TRANSLATION_UPDATED: TranslationUpdated, WebhookEventAsyncType.VOUCHER_CREATED: VoucherCreated, WebhookEventAsyncType.VOUCHER_UPDATED: VoucherUpdated, WebhookEventAsyncType.VOUCHER_DELETED: VoucherDeleted, WebhookEventAsyncType.VOUCHER_METADATA_UPDATED: VoucherMetadataUpdated, WebhookEventAsyncType.VOUCHER_CODE_EXPORT_COMPLETED: VoucherCodeExportCompleted, WebhookEventAsyncType.WAREHOUSE_CREATED: WarehouseCreated, WebhookEventAsyncType.WAREHOUSE_UPDATED: WarehouseUpdated, WebhookEventAsyncType.WAREHOUSE_DELETED: WarehouseDeleted, WebhookEventAsyncType.WAREHOUSE_METADATA_UPDATED: WarehouseMetadataUpdated, WebhookEventAsyncType.THUMBNAIL_CREATED: ThumbnailCreated, WebhookEventSyncType.PAYMENT_AUTHORIZE: PaymentAuthorize, WebhookEventSyncType.PAYMENT_CAPTURE: PaymentCaptureEvent, WebhookEventSyncType.PAYMENT_REFUND: PaymentRefundEvent, WebhookEventSyncType.PAYMENT_VOID: PaymentVoidEvent, WebhookEventSyncType.PAYMENT_CONFIRM: PaymentConfirmEvent, WebhookEventSyncType.PAYMENT_PROCESS: PaymentProcessEvent, WebhookEventSyncType.PAYMENT_LIST_GATEWAYS: PaymentListGateways, WebhookEventSyncType.TRANSACTION_CANCELATION_REQUESTED: TransactionCancelationRequested, WebhookEventSyncType.TRANSACTION_CHARGE_REQUESTED: TransactionChargeRequested, WebhookEventSyncType.TRANSACTION_REFUND_REQUESTED: TransactionRefundRequested, WebhookEventSyncType.ORDER_FILTER_SHIPPING_METHODS: OrderFilterShippingMethods, WebhookEventSyncType.CHECKOUT_FILTER_SHIPPING_METHODS: CheckoutFilterShippingMethods, WebhookEventSyncType.SHIPPING_LIST_METHODS_FOR_CHECKOUT: ShippingListMethodsForCheckout, WebhookEventSyncType.CHECKOUT_CALCULATE_TAXES: CalculateTaxes, WebhookEventSyncType.ORDER_CALCULATE_TAXES: CalculateTaxes, WebhookEventSyncType.PAYMENT_GATEWAY_INITIALIZE_SESSION: PaymentGatewayInitializeSession, WebhookEventSyncType.TRANSACTION_INITIALIZE_SESSION: TransactionInitializeSession, WebhookEventSyncType.TRANSACTION_PROCESS_SESSION: TransactionProcessSession, WebhookEventAsyncType.SHOP_METADATA_UPDATED: ShopMetadataUpdated, WebhookEventSyncType.LIST_STORED_PAYMENT_METHODS: ListStoredPaymentMethods, WebhookEventSyncType.STORED_PAYMENT_METHOD_DELETE_REQUESTED: StoredPaymentMethodDeleteRequested, WebhookEventSyncType.PAYMENT_GATEWAY_INITIALIZE_TOKENIZATION_SESSION: PaymentGatewayInitializeTokenizationSession, WebhookEventSyncType.PAYMENT_METHOD_INITIALIZE_TOKENIZATION_SESSION: PaymentMethodInitializeTokenizationSession, WebhookEventSyncType.PAYMENT_METHOD_PROCESS_TOKENIZATION_SESSION: PaymentMethodProcessTokenizationSession}