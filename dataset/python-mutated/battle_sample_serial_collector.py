...
class BattleSampleSerialCollector(ISerialCollector):
    ...
    def __init__(self, cfg: EasyDict, env: BaseEnvManager=None, policy: List[namedtuple]=None, tb_logger: 'SummaryWriter'=None, exp_name: Optional[str]='default_experiment', instance_name: Optional[str]='collector') -> None:
        if False:
            return 10
    ...
    def reset_policy(self, _policy: Optional[List[namedtuple]]=None) -> None:
        ...
        if _policy is not None:
            assert len(_policy) > 1, 'battle sample collector needs more than 1 policy, but found {}'.format(len(_policy))
            ...
            self._policy_collect_data = [getattr(self._policy[i], 'collect_data', True) for i in range(self._policy_num)]
    ...
    def reset(self, _policy: Optional[List[namedtuple]]=None, _env: Optional[BaseEnvManager]=None) -> None:
        ...
        self._traj_buffer = {env_id: {policy_id: TrajBuffer(maxlen=self._traj_len) for policy_id in range(self._policy_num)} for env_id in range(self._env_num)}
    ...
    def collect(self, n_sample: Optional[int]=None, train_iter: int=0, drop_extra: bool=True, policy_kwargs: Optional[dict]=None) -> Tuple[List[Any], List[Any]]:
        if False:
            for i in range(10):
                print('nop')
            ...
            for (env_id, timestep) in timesteps.items():
                ...
                with self._timer:
                    for (policy_id, policy) in enumerate(self._policy):
                        ...
                        transition = self._policy[policy_id].process_transition(self._obs_pool[env_id][policy_id], self._policy_output_pool[env_id][policy_id], policy_timestep)
                        ...
                        if timestep.done or len(self._traj_buffer[env_id][policy_id]) == self._traj_len:
                            transitions = to_tensor_transitions(self._traj_buffer[env_id][policy_id], not self._deepcopy_obs)
                ...
                if timestep.done:
                    ...
                    info = {'time': self._env_info[env_id]['time'], 'step': self._env_info[env_id]['step'], 'train_sample': self._env_info[env_id]['train_sample']}
    ...
    def _output_log(self, train_iter: int) -> None:
        ...
        if train_iter - self._last_train_iter >= self._collect_print_freq and len(self._episode_info) > 0:
            ...
            info = {'episode_count': episode_count, 'envstep_count': envstep_count, 'avg_envstep_per_episode': envstep_count / episode_count, 'avg_envstep_per_sec': envstep_count / duration, 'avg_episode_per_sec': episode_count / duration, 'collect_time': duration, 'total_envstep_count': self._total_envstep_count, 'total_episode_count': self._total_episode_count, 'total_duration': self._total_duration}
