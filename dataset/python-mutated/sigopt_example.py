...
if __name__ == '__main__':
    ...
    algo = SigOptSearch(space, name='SigOpt Example Experiment', metric='mean_loss', mode='min')
    scheduler = AsyncHyperBandScheduler(metric='mean_loss', mode='min')
    tuner = tune.Tuner(easy_objective, run_config=train.RunConfig(name='my_exp'), tune_config=tune.TuneConfig(search_alg=algo, scheduler=scheduler, num_samples=4 if args.smoke_test else 100), param_space={'steps': 10})
    results = tuner.fit()
