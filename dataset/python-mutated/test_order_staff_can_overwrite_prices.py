...
@pytest.mark.e2e
def test_order_staff_can_overwrite_prices_CORE_0202(e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes, permission_manage_shipping, permission_manage_orders):
    if False:
        while True:
            i = 10
    ...
    (_product_id, product_variant_id, _product_variant_price) = prepare_product(e2e_staff_api_client, warehouse_id, channel_id, regular_variant_price)
    ...
    lines = [{'variantId': product_variant_id, 'quantity': 1, 'price': 3}]
