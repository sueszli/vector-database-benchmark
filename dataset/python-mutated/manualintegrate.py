...
sincos_botheven_condition = uncurry(lambda a, b, m, n, i, s: m.is_even and n.is_even and m.is_nonnegative and n.is_nonnegative)
sincos_botheven = trig_rewriter(lambda a, b, m, n, i, symbol: ((1 - cos(2 * a * symbol)) / 2) ** (m / 2) * ((1 + cos(2 * b * symbol)) / 2) ** (n / 2))
sincos_sinodd_condition = uncurry(lambda a, b, m, n, i, s: m.is_odd and m >= 3)
sincos_sinodd = trig_rewriter(lambda a, b, m, n, i, symbol: (1 - cos(a * symbol) ** 2) ** ((m - 1) / 2) * sin(a * symbol) * cos(b * symbol) ** n)
sincos_cosodd_condition = uncurry(lambda a, b, m, n, i, s: n.is_odd and n >= 3)
sincos_cosodd = trig_rewriter(lambda a, b, m, n, i, symbol: (1 - sin(b * symbol) ** 2) ** ((n - 1) / 2) * cos(b * symbol) * sin(a * symbol) ** m)
tansec_seceven_condition = uncurry(lambda a, b, m, n, i, s: n.is_even and n >= 4)
tansec_seceven = trig_rewriter(lambda a, b, m, n, i, symbol: (1 + tan(b * symbol) ** 2) ** (n / 2 - 1) * sec(b * symbol) ** 2 * tan(a * symbol) ** m)
tansec_tanodd_condition = uncurry(lambda a, b, m, n, i, s: m.is_odd)
tansec_tanodd = trig_rewriter(lambda a, b, m, n, i, symbol: (sec(a * symbol) ** 2 - 1) ** ((m - 1) / 2) * tan(a * symbol) * sec(b * symbol) ** n)
...
cotcsc_csceven = trig_rewriter(lambda a, b, m, n, i, symbol: (1 + cot(b * symbol) ** 2) ** (n / 2 - 1) * csc(b * symbol) ** 2 * cot(a * symbol) ** m)
cotcsc_cotodd_condition = uncurry(lambda a, b, m, n, i, s: m.is_odd)
cotcsc_cotodd = trig_rewriter(lambda a, b, m, n, i, symbol: (csc(a * symbol) ** 2 - 1) ** ((m - 1) / 2) * cot(a * symbol) * csc(b * symbol) ** n)
...
def substitution_rule(integral):
    ...
    if substitutions:
        ...
        if len(ways) > 1:
            ...
            return ways[0]
partial_fractions_rule = rewriter(lambda integrand, symbol: integrand.is_rational_function(), lambda integrand, symbol: integrand.apart(symbol))
cancel_rule = rewriter(lambda integrand, symbol: True, lambda integrand, symbol: integrand.cancel())
distribute_expand_rule = rewriter(lambda integrand, symbol: isinstance(integrand, (Pow, Mul)) or all((arg.is_Pow or arg.is_polynomial(symbol) for arg in integrand.args)), lambda integrand, symbol: integrand.expand())
trig_expand_rule = rewriter(lambda integrand, symbol: len({a.args[0] for a in integrand.atoms(TrigonometricFunction)}) > 1, lambda integrand, symbol: integrand.expand(trig=True))
