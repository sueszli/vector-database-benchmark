...
class apikeys_key_rotated_in_90_days(Check):
    ...
    def execute(self) -> Check_Report_GCP:
        ...
        for key in apikeys_client.keys:
            ...
            if (datetime.now(timezone.utc) - datetime.strptime(key.creation_time, '%Y-%m-%dT%H:%M:%S.%f%z')).days > 90:
                report.status = 'FAIL'
                report.status_extended = f'API key {key.name} creation date has more than 90 days.'
