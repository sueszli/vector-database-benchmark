...
with DAG(dag_id=DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), catchup=False, tags=['example', 'mysql', 'gcs']) as dag:
    ...
    setup_mysql = SSHOperator(task_id='setup_mysql', ssh_hook=ComputeEngineSSHHook(user='username', instance_name=DB_INSTANCE_NAME, zone=ZONE, project_id=PROJECT_ID, use_oslogin=False, use_iap_tunnel=False, cmd_timeout=180), command=SETUP_MYSQL, retries=2)
    ...
    upload_mysql_to_gcs = MySQLToGCSOperator(task_id='mysql_to_gcs', sql=SQL_SELECT, bucket=BUCKET_NAME, filename=FILE_NAME, export_format='csv')
    delete_mysql_connection = BashOperator(task_id='delete_mysql_connection', bash_command=f'airflow connections delete {CONNECTION_ID}', trigger_rule=TriggerRule.ALL_DONE)
    ...
    create_instance >> setup_mysql >> get_public_ip_task >> setup_mysql_connection_task >> create_firewall_rule >> create_sql_table >> insert_data
    [insert_data, create_bucket] >> upload_mysql_to_gcs >> [delete_instance, delete_bucket, delete_mysql_connection, delete_firewall_rule]
