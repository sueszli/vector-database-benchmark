...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> None:
    ...
    async def async_discover(tasmota_automation: TasmotaTrigger, discovery_hash: DiscoveryHashType) -> None:
        """Discover and add a Tasmota device automation."""
        if tasmota_automation.automation_type == AUTOMATION_TYPE_TRIGGER:
            await device_trigger.async_setup_trigger(hass, tasmota_automation, config_entry, discovery_hash)
    hass.data[DATA_REMOVE_DISCOVER_COMPONENT.format('device_automation')] = async_dispatcher_connect(hass, TASMOTA_DISCOVERY_ENTITY_NEW.format('device_automation'), async_discover)
    hass.data[DATA_UNSUB].append(hass.bus.async_listen(EVENT_DEVICE_REGISTRY_UPDATED, async_device_removed, event_filter=_async_device_removed_filter))