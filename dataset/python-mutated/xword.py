...
class CrosswordsSheet(Sheet):
    rowtype = 'puzzles'
    columns = [Column('Author', getter=lambda col, row: row.author), Column('Copyright', getter=lambda col, row: row.copyright), Column('Notes', getter=lambda col, row: row.notes), Column('Postscript', getter=lambda col, row: ''.join((x for x in row.postscript if ord(x) >= ord(' ')))), Column('Preamble', getter=lambda col, row: row.preamble), Column('Title', getter=lambda col, row: row.title)]
...
class GridSheet(Sheet):
    rowtype = 'gridrow'
    colorizers = [CellColorizer(7, 'color_xword_active', lambda s, c, r, v: r and s.pos in s.cells[s.rows.index(r), c])]
...
class CrosswordSheet(Sheet):
    rowtype = 'clues'
    columns = [Column('clue_number', getter=lambda col, row: row[0][0] + str(row[0][1])), Column('clue', getter=lambda col, row: row[1]), Column('answer', getter=lambda col, row: row[2])]
