    ...
    def execute(self):
        ...
        for db_instance in rds_client.db_instances:
            ...
            if db_instance.cluster_id:
                if db_instance.cluster_arn in rds_client.db_clusters and rds_client.db_clusters[db_instance.cluster_arn].multi_az:
                    report.status = 'PASS'
            ...
            elif db_instance.multi_az:
                report.status = 'PASS'
                report.status_extended = f'RDS Instance {db_instance.id} has multi-AZ enabled.'
            else:
                report.status = 'FAIL'
                report.status_extended = f'RDS Instance {db_instance.id} does not have multi-AZ enabled.'
