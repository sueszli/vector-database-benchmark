...
from mage_ai.cache.block_action_object.constants import OBJECT_TYPE_BLOCK_FILE, OBJECT_TYPE_CUSTOM_BLOCK_TEMPLATE, OBJECT_TYPE_MAGE_TEMPLATE
from mage_ai.data_preparation.models.block import Block
from mage_ai.data_preparation.models.block.utils import clean_name, is_dynamic_block, is_dynamic_block_child, should_reduce_output
from mage_ai.data_preparation.models.constants import FILE_EXTENSION_TO_BLOCK_LANGUAGE, BlockLanguage, BlockType, PipelineType
from mage_ai.data_preparation.models.custom_templates.custom_block_template import CustomBlockTemplate
from mage_ai.data_preparation.models.pipeline import Pipeline
from mage_ai.data_preparation.templates.data_integrations.constants import TEMPLATE_TYPE_DATA_INTEGRATION
...
class BlockResource(GenericResource):
    ...
    async def create(self, payload, user, **kwargs):
        ...
        if block_action_object:
            ...
            if OBJECT_TYPE_BLOCK_FILE == object_type:
                ...
                if TEMPLATE_TYPE_DATA_INTEGRATION != object_from_cache.get('template_type'):
                    language = object_from_cache.get('language')
                for key in ['template_type', 'template_variables']:
                    if object_from_cache.get(key):
                        payload_config[key] = object_from_cache.get(key)
                payload['configuration'] = merge_dict(payload.get('configuration') or {}, object_from_cache.get('configuration') or {})
        ...
        if payload.get('type') == BlockType.DBT and language == BlockLanguage.SQL and content:
            ...
            dbt_block = DBTBlock(name, clean_name(name), BlockType.DBT, configuration=payload.get('configuration'), language=language)
            if dbt_block.file_path and dbt_block.file.exists():
                raise Exception('DBT model at that folder location already exists.                     Please choose a different model name, or add a DBT model by                     selecting single model from file.')
        block_attributes = dict(color=payload.get('color'), config=payload_config, configuration=payload.get('configuration'), downstream_block_uuids=payload.get('downstream_blocks', []), extension_uuid=payload.get('extension_uuid'), language=language, pipeline=pipeline, priority=payload.get('priority'), upstream_block_uuids=payload.get('upstream_blocks', []))
        ...
        if replicated_block_uuid:
            ...
            if replicated_block:
                ...
                error.update(message=f'Replicated block {replicated_block_uuid} ' + f'does not exist in pipeline {pipeline.uuid}.')
        ...
        if payload_config and payload_config.get('custom_template_uuid'):
            ...
            block = custom_template.create_block(block_name, pipeline, extension_uuid=block_attributes.get('extension_uuid'), priority=block_attributes.get('priority'), upstream_block_uuids=block_attributes.get('upstream_block_uuids'))
            content = custom_template.load_template_content()
        else:
            block = Block.create(block_name, block_type, get_repo_path(), require_unique_name=require_unique_name, **block_attributes)
        if content:
        ...
        if block:
            await UsageStatisticLogger().block_create(block, block_action_object=block_action_object, custom_template=custom_template, payload_config=payload_config, pipeline=pipeline, replicated_block=replicated_block)
    ...
    async def update(self, payload, **kwargs):
        ...
        self.model.update(payload, update_state=update_state)
