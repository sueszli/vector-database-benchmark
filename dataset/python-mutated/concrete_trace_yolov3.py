...
prefix = 'C:\\Works\\PyTorch-YOLOv3'
model = models.load_model('%s/config/yolov3.cfg' % prefix, '%s/weights/yolov3.weights' % prefix).cpu()
model.eval()
...
def recompile_from_code(code: str, global_vars: dict):
    if False:
        for i in range(10):
            print('nop')
    func_dict = {}
    global_vars = {**global_vars, 'inf': math.inf, 'nan': math.nan, 'NoneType': type(None), 'torch': torch, 'device': torch.device}
