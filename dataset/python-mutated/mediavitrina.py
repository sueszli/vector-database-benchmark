...
@pluginmatcher(re.compile('https?://(?:www\\.)?(?:\n    chetv\n    |\n    ctc(?:love)?\n    |\n    domashniy\n)\\.ru/(?:live|online)', re.VERBOSE))
@pluginmatcher(re.compile('https?://player\\.mediavitrina\\.ru/.+/player\\.html'))
class MediaVitrina(Plugin):
    _re_url_json = re.compile('https://media\\.mediavitrina\\.ru/(?:proxy)?api/v3/[\\w-]+/playlist/[\\w-]+_as_array\\.json[^\\"\']+')
