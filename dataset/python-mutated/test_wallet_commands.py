...
class WalletCommands(CommandTestCase):
    ...
    async def test_wallet_syncing_status(self):
        ...
        self.assertEqual({'is_encrypted': None, 'is_syncing': None, 'is_locked': None}, self.daemon.jsonrpc_wallet_status())
        wallet.wallet_manager = wallet_manager
        self.assertEqual({'is_encrypted': False, 'is_syncing': False, 'is_locked': False}, self.daemon.jsonrpc_wallet_status())
    ...
    async def test_sending_to_scripthash_address(self):
        ...
        self.assertEqual(await self.blockchain.get_balance(), str(float(bal) + 3))
        await self.assertBalance(self.account, '7.999877')
        await self.wallet_send('3.0', p2sh_address1)
        self.assertEqual(await self.blockchain.get_balance(), str(float(bal) + 7))
    ...
    async def test_balance_caching(self):
        ...
        await self.ledger.tasks_are_done()
        wallet_balance = self.daemon.jsonrpc_wallet_balance
        ledger = self.ledger
        query_count = self.ledger.db.db.query_count
        expected = {'total': '20.0', 'available': '20.0', 'reserved': '0.0', 'reserved_subtotals': {'claims': '0.0', 'supports': '0.0', 'tips': '0.0'}}
        ...
        expected = {'total': '19.979893', 'available': '18.979893', 'reserved': '1.0', 'reserved_subtotals': {'claims': '1.0', 'supports': '0.0', 'tips': '0.0'}}
        query_count = self.ledger.db.db.query_count
    ...
    async def test_granular_balances(self):
        ...
        wallet_balance = self.daemon.jsonrpc_wallet_balance
        expected = {'total': '10.0', 'available': '10.0', 'reserved': '0.0', 'reserved_subtotals': {'claims': '0.0', 'supports': '0.0', 'tips': '0.0'}}
        ...
        expected = {'total': '9.977534', 'available': '6.977534', 'reserved': '3.0', 'reserved_subtotals': {'claims': '1.0', 'supports': '2.0', 'tips': '0.0'}}
        self.assertEqual(await account_balance(), expected)
        self.assertEqual(await wallet_balance(), expected)
        address2 = await self.daemon.jsonrpc_address_unused(account2.id)
        tx = await self.daemon.jsonrpc_account_send('1.0', address2, blocking=True)
        await self.confirm_tx(tx.id)
        self.assertEqual(await account_balance(), {'total': '8.97741', 'available': '5.97741', 'reserved': '3.0', 'reserved_subtotals': {'claims': '1.0', 'supports': '2.0', 'tips': '0.0'}})
        self.assertEqual(await wallet_balance(), {'total': '9.97741', 'available': '6.97741', 'reserved': '3.0', 'reserved_subtotals': {'claims': '1.0', 'supports': '2.0', 'tips': '0.0'}})
        support1 = await self.support_create(self.get_claim_id(stream1), '0.3', tip=True, wallet_id=wallet2.id)
        self.assertEqual(await account_balance(), {'total': '9.27741', 'available': '5.97741', 'reserved': '3.3', 'reserved_subtotals': {'claims': '1.0', 'supports': '2.0', 'tips': '0.3'}})
        self.assertEqual(await wallet_balance(), {'total': '10.27741', 'available': '6.97741', 'reserved': '3.3', 'reserved_subtotals': {'claims': '1.0', 'supports': '2.0', 'tips': '0.3'}})
        tx = await self.daemon.jsonrpc_support_abandon(txid=support1['txid'], nout=0, blocking=True)
        await self.confirm_tx(tx.id)
        self.assertEqual(await account_balance(), {'total': '9.277303', 'available': '6.277303', 'reserved': '3.0', 'reserved_subtotals': {'claims': '1.0', 'supports': '2.0', 'tips': '0.0'}})
        self.assertEqual(await wallet_balance(), {'total': '10.277303', 'available': '7.277303', 'reserved': '3.0', 'reserved_subtotals': {'claims': '1.0', 'supports': '2.0', 'tips': '0.0'}})
        stream2 = await self.stream_create('granularity-is-cool', '0.1', account_id=account2.id, funding_account_ids=[account2.id])
        await self.support_create(self.get_claim_id(stream2), '0.2', tip=True, wallet_id=wallet2.id)
        self.assertEqual(await account_balance(), {'total': '9.277303', 'available': '6.277303', 'reserved': '3.0', 'reserved_subtotals': {'claims': '1.0', 'supports': '2.0', 'tips': '0.0'}})
        self.assertEqual(await wallet_balance(), {'total': '10.439196', 'available': '7.139196', 'reserved': '3.3', 'reserved_subtotals': {'claims': '1.1', 'supports': '2.0', 'tips': '0.2'}})
...
class WalletEncryptionAndSynchronization(CommandTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.daemon2 = await self.add_daemon(seed='chest sword toast envelope bottom stomach absent carbon smart garage balance margin twelve')
    ...
    async def test_sync(self):
        ...
        self.assertDictEqual(daemon.jsonrpc_preference_get(), {'one': '1', 'conflict': '1', 'another': 'B', 'fruit': ['peach', 'apricot']})
        self.assertDictEqual(daemon2.jsonrpc_preference_get(), {'two': '2', 'conflict': '2', 'another': 'A'})
        ...
        self.assertDictEqual(daemon.jsonrpc_preference_get(), {'one': '1', 'two': '2', 'conflict': '2', 'another': 'B', 'fruit': ['peach', 'apricot']})
        self.daemon2.wallet_manager.default_account.channel_keys['mqs77XbdnuxWN4cXrjKbSoGLkvAHa4f4B8'] = '-----BEGIN EC PRIVATE KEY-----\nMHQCAQEEIBZRTZ7tHnYCH3IE9mCo95466L/ShYFhXGrjmSMFJw8eoAcGBSuBBAAK\noUQDQgAEmucoPz9nI+ChZrfhnh0RZ/bcX0r2G0pYBmoNKovtKzXGa8y07D66MWsW\nqXptakqO/9KddIkBu5eJNSUZzQCxPQ==\n-----END EC PRIVATE KEY-----\n'
        channel = await self.create_nondeterministic_channel('@foo', '0.1', unhexlify('3056301006072a8648ce3d020106052b8104000a034200049ae7283f3f6723e0a166b7e19e1d1167f6dc5f4af61b4a58066a0d2a8bed2b35c66bccb4ec3eba316b16a97a6d6a4a8effd29d748901bb9789352519cd00b13d'), self.daemon2, blocking=True)
        ...
        data = await daemon2.jsonrpc_sync_apply('password')
        await daemon.jsonrpc_sync_apply('password', data=data['data'], blocking=True)
        self.assertEqual(daemon2.wallet_manager.default_account.channel_keys, daemon.wallet_manager.default_wallet.accounts[1].channel_keys)

    async def test_encryption_and_locking(self):
        daemon = self.daemon
        wallet = daemon.wallet_manager.default_wallet
        wallet.save()
        self.assertEqual(daemon.jsonrpc_wallet_status(), {'is_locked': False, 'is_encrypted': False, 'is_syncing': False})
        ...
        self.assertEqual(daemon.jsonrpc_wallet_status(), {'is_locked': False, 'is_encrypted': True, 'is_syncing': False})
        ...
        daemon.jsonrpc_wallet_lock()
        self.assertEqual(daemon.jsonrpc_wallet_status(), {'is_locked': True, 'is_encrypted': True, 'is_syncing': False})
    ...
    async def test_encryption_with_imported_channel(self):
        ...
        self.assertEqual(daemon2.jsonrpc_wallet_status(), {'is_locked': False, 'is_encrypted': True, 'is_syncing': False})
    ...
    async def test_sync_with_encryption_and_password_change(self):
        ...
        self.assertEqual(daemon.jsonrpc_wallet_status(), {'is_locked': False, 'is_encrypted': True, 'is_syncing': True})
        self.assertEqual(daemon.jsonrpc_preference_get(ENCRYPT_ON_DISK), {'encrypt-on-disk': True})
        self.assertWalletEncrypted(wallet.storage.path, True)
        daemon.jsonrpc_wallet_lock()
        ...
        self.assertEqual(daemon2.jsonrpc_wallet_status(), {'is_locked': False, 'is_encrypted': True, 'is_syncing': True})
