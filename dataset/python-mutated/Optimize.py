...
import cython
cython.declare(UtilityCode=object, EncodedString=object, bytes_literal=object, encoded_string=object, Nodes=object, ExprNodes=object, PyrexTypes=object, Builtin=object, UtilNodes=object, _py_int_types=object)
        ...
        if isinstance(node.operand1, ExprNodes.UnicodeNode) and isinstance(node.operand2, ExprNodes.TupleNode):
            if not node.operand2.mult_factor:
                ...
                if fstring is not None:
                    return fstring
        return self.visit_BinopNode(node)
    _parse_string_format_regex = u'(%(?:(?:[-0-9]+|[ ])?(?:[.][0-9]+)?)?.)'
