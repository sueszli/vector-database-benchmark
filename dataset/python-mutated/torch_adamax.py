...
class Adamax(torch_parallel_optimizer.TorchParallelOptimizer, optimizers.Adamax):
    ...
    def _parallel_update_step(self, grads, variables, learning_rate):
        ...
        m_list = [self._m[self._get_variable_index(variable)].value for variable in keras_variables]
        u_list = [self._u[self._get_variable_index(variable)].value for variable in keras_variables]
        ...
        torch._foreach_add_(variables, torch._foreach_div(torch._foreach_mul(m_list, lr), torch._foreach_mul(torch._foreach_add(u_list, self.epsilon), 1 - beta_1_power)), alpha=-1)