...
class CPUStatsSheet(Sheet):
    rowtype = 'CPUs'
    columns = [ColumnItem('cpu', 0, type=int, keycol=0), ColumnItem('cpu_pct', 1, type=float)]
...
class MemStatsSheet(Sheet):
    rowtype = ''
    columns = [AttrColumn('t', type=date, fmtstr='%H:%M:%S'), AttrColumn('meminfo'), AttrColumn('virtmem'), AttrColumn('swapmem')]
    nKeys = 1
...
class UsefulProcessesSheet(Sheet):
    columns = [Column('pid', type=int, getter=lambda c, r: r[0].pid), Column('name', getter=lambda c, r: r[0].name()), Column('status', getter=lambda c, r: r[0].status()), Column('cmdline', getter=lambda c, r: ' '.join(r[0].cmdline())), Column('user', getter=lambda c, r: r[0].username()), Column('real_uid', type=int, width=0, getter=lambda c, r: r[0].uids()[0]), Column('effective_uid', type=int, width=0, getter=lambda c, r: r[0].uids()[1]), Column('mem_uss', type=int, getter=lambda c, r: r[1].mem_uss), Column('user_time_s', type=float, getter=lambda c, r: r[0].cpu_times()[0]), Column('system_time_s', type=float, getter=lambda c, r: r[0].cpu_times()[1])]
...
class ProcessesSheet(Sheet):
    columns = [Column('pid', type=int, getter=lambda c, r: r.pid), Column('name', getter=lambda c, r: r.name()), Column('status', getter=lambda c, r: r.status()), Column('parent_pid', type=int, getter=lambda c, r: r.ppid()), Column('exe', getter=lambda c, r: r.exe()), Column('cmdline', getter=lambda c, r: ' '.join(r.cmdline())), Column('cwd', getter=lambda c, r: r.cwd()), Column('user', getter=lambda c, r: r.username()), Column('real_uid', type=int, width=0, getter=lambda c, r: r.uids()[0]), Column('effective_uid', type=int, width=0, getter=lambda c, r: r.uids()[1]), Column('saved_uid', type=int, width=0, getter=lambda c, r: r.uids()[2]), Column('real_gid', type=int, width=0, getter=lambda c, r: r.gids()[0]), Column('effective_gid', type=int, width=0, getter=lambda c, r: r.gids()[1]), Column('saved_gid', type=int, width=0, getter=lambda c, r: r.gids()[2]), Column('create_time', type=date, getter=lambda c, r: r.create_time()), Column('cpu_num', type=int, getter=lambda c, r: r.cpu_num()), Column('cpu_percent', type=float, getter=lambda c, r: r.cpu_percent()), Column('tty', getter=lambda c, r: r.terminal()), Column('nice', getter=lambda c, r: r.nice()), Column('ioclass', getter=lambda c, r: r.ionice()[0]), Column('ionice', getter=lambda c, r: r.ionice()[1]), Column('user_time_s', type=float, getter=lambda c, r: r.cpu_times()[0]), Column('system_time_s', type=float, getter=lambda c, r: r.cpu_times()[1]), Column('children_user_time_s', type=float, width=0, getter=lambda c, r: r.cpu_times()[2]), Column('children_system_time_s', type=float, width=0, getter=lambda c, r: r.cpu_times()[3]), Column('read_ops', type=int, getter=lambda c, r: r.io_counters()[0]), Column('write_ops', type=int, getter=lambda c, r: r.io_counters()[1]), Column('read_bytes', type=int, getter=lambda c, r: r.io_counters()[2]), Column('write_bytes', type=int, getter=lambda c, r: r.io_counters()[3]), Column('voluntary_ctx_switches', type=int, getter=lambda c, r: r.num_ctx_switches()[0]), Column('involuntary_ctx_switches', type=int, getter=lambda c, r: r.num_ctx_switches()[1]), Column('num_fds', type=int, getter=lambda c, r: r.num_fds()), Column('num_threads', type=int, getter=lambda c, r: r.num_threads())]
    nKeys = 2
...
class RlimitsSheet(Sheet):
    columns = [ColumnItem('rlimit', 0), Column('soft', type=int, getter=lambda c, r: c.sheet.soft(r), setter=lambda c, r, v: c.sheet.set_soft(r, v)), Column('hard', type=int, getter=lambda c, r: c.sheet.hard(r), setter=lambda c, r, v: c.sheet.set_hard(r, v))]
