...
from airflow.cli.cli_config import DAG_CLI_DICT, ActionCommand, DefaultHelpParser, GroupCommand, core_commands
...
try:
    ...
    log.error('Ensure all dependencies are met and try again. If using a Celery based executor install a 3.3.0+ version of the Celery provider. If using a Kubernetes executor, install a 7.4.0+ version of the CNCF provider')
try:
...
if len(ALL_COMMANDS_DICT) < len(airflow_commands):
    ...
    raise CliConflictError(f"The following CLI {len(dup)} command(s) are defined more than once: {sorted(dup)}\nThis can be due to the executor '{ExecutorLoader.get_default_executor_name()}' redefining core airflow CLI commands.")
