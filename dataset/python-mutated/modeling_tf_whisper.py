...
from ...modeling_tf_utils import TFCausalLanguageModelingLoss, TFModelInputType, TFPreTrainedModel, keras_serializable, unpack_inputs
...
TF_WHISPER_PRETRAINED_MODEL_ARCHIVE_LIST = ['openai/whisper-base']
...
class TFWhisperDecoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config: WhisperConfig, **kwargs):
        ...
        self.embed_tokens = tf.keras.layers.Embedding(input_dim=config.vocab_size, output_dim=config.d_model, embeddings_initializer=tf.keras.initializers.TruncatedNormal(stddev=self.config.init_std), name='embed_tokens')
