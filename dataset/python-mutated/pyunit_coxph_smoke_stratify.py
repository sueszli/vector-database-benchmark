...
def coxph_smoke_strata():
    ...
    assert coxph.formula() == 'Surv(start, stop, event) ~ age + surgery + strata(transplant)', "Expected formula to be 'Surv(start, stop, event) ~ age + surgery + strata(transplant)' but it was " + coxph.formula()
