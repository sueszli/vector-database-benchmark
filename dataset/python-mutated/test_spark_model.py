...
def get_mock_provider(valid_app_id, valid_api_key, valid_api_secret):
    if False:
        return 10
    return Provider(id='provider_id', tenant_id='tenant_id', provider_name='spark', provider_type=ProviderType.CUSTOM.value, encrypted_config=json.dumps({'app_id': valid_app_id, 'api_key': valid_api_key, 'api_secret': valid_api_secret}), is_valid=True)
...
def get_mock_model(model_name):
    ...
    return SparkModel(model_provider=model_provider, name=model_name, model_kwargs=model_kwargs)
