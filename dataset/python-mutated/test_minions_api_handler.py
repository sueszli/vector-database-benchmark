...
def app_urls():
    if False:
        print('Hello World!')
    return [('/minions/(.*)', saltnado.MinionSaltAPIHandler), ('/minions', saltnado.MinionSaltAPIHandler)]

async def test_get_no_mid(http_client, salt_minion, salt_sub_minion):
    response = await http_client.fetch('/minions', method='GET', follow_redirects=False)
...
async def test_get(http_client, salt_minion):
    response = await http_client.fetch('/minions/{}'.format(salt_minion.id), method='GET', follow_redirects=False)
    response_obj = salt.utils.json.loads(response.body)
    assert len(response_obj['return']) == 1
    assert isinstance(response_obj['return'][0], dict)
...
async def test_post(http_client, salt_minion, salt_sub_minion):
    ...
    response = await http_client.fetch('/minions', method='POST', body=salt.utils.json.dumps(low))
    response_obj = salt.utils.json.loads(response.body)
...
async def test_post_with_incorrect_client(http_client):
    ...
    with pytest.raises(HTTPError) as exc:
        await http_client.fetch('/minions', method='POST', body=salt.utils.json.dumps(low))
    assert exc.value.code == 400

@pytest.mark.slow_test
async def test_mem_leak_in_event_listener(http_client, salt_minion, app):
    for i in range(10):
        await http_client.fetch('/minions/{}'.format(salt_minion.id), method='GET', follow_redirects=False)
