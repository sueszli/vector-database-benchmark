...
class api_keysCallTransformer(cst.CSTTransformer):
    CTRL_PARAMS: Tuple[str] = ('retry', 'timeout', 'metadata')
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_key': ('parent', 'key', 'key_id'), 'delete_key': ('name', 'etag'), 'get_key': ('name',), 'get_key_string': ('name',), 'list_keys': ('parent', 'page_size', 'page_token', 'show_deleted'), 'lookup_key': ('key_string',), 'undelete_key': ('name',), 'update_key': ('key', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=api_keysCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
