...
class transcoderCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_job': ('parent', 'job'), 'create_job_template': ('parent', 'job_template', 'job_template_id'), 'delete_job': ('name', 'allow_missing'), 'delete_job_template': ('name', 'allow_missing'), 'get_job': ('name',), 'get_job_template': ('name',), 'list_jobs': ('parent', 'page_size', 'page_token', 'filter', 'order_by'), 'list_job_templates': ('parent', 'page_size', 'page_token', 'filter', 'order_by')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=transcoderCallTransformer()):
    if False:
        return 10
