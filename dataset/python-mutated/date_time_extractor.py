...
_REGEXP_GROUP_YEAR = '((?:19|20|21)\\d\\d)'
...
_REGEXP_GROUP_MIN = '([0-5]\\d)'
_REGEXP_GROUP_SEC = '([0-5]\\d)'
_REGEXP_DELIM = '[-:_\\., ]*'
_NOT_A_NUMBER = '(?<!\\d)'
REGEXP_NO_TZ = re.compile(_NOT_A_NUMBER + _REGEXP_DELIM.join([_REGEXP_GROUP_YEAR, _REGEXP_GROUP_MONTH, _REGEXP_GROUP_DAY, _REGEXP_GROUP_HOUR, _REGEXP_GROUP_MIN, _REGEXP_GROUP_SEC]))
REGEXP_WHATSAPP = re.compile('^(?:IMG|VID)[-_](\\d{4})(\\d{2})(\\d{2})(?:[-_]WA(\\d+))?')
REGEXP_WHATSAPP_GROUP_MAPPING = ['year', 'month', 'day', 'microsecond']
PREDEFINED_REGEXPS = {'default': (REGEXP_NO_TZ, None), 'whatsapp': (REGEXP_WHATSAPP, REGEXP_WHATSAPP_GROUP_MAPPING)}
REGEXP_GROUP_MAPPINGS = {'year': 0, 'month': 1, 'day': 2, 'hour': 3, 'minute': 4, 'second': 5, 'microsecond': 6}
...
def set_as_other_rule(rule):
    ...
    return rule
PREDEFINED_RULES_PARAMS = list(map(set_as_default_rule, DEFAULT_RULES_PARAMS)) + list(map(set_as_other_rule, OTHER_RULES_PARAMS))
