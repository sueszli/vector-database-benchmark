from collections.abc import Iterable, Mapping, Callable as ACallable
from itertools import zip_longest
from enum import Enum
from typing import Any, Generic, TypeVar, Tuple, List, Pattern as RegexPattern, Callable
import inspect
from pampy.helpers import UnderscoreType, HeadType, TailType, get_lambda_args_error_msg, BoxedArgs, PaddedValue, NoDefault, is_typing_stuff, is_dataclass, is_generic, is_newtype, is_union, pairwise, peek, get_real_type, get_extra
T = TypeVar('T')
...
def match_value(pattern, value) -> Tuple[bool, List]:
    ...
    if value is PaddedValue:
        ...
        elif isinstance(return_value, tuple) and len(return_value) == 2 and isinstance(return_value[0], bool) and isinstance(return_value[1], list):
            return return_value
        else:
            raise MatchError('Warning! pattern function %s is not returning a boolean nor a tuple of (boolean, list), but instead %s' % (pattern, return_value))
