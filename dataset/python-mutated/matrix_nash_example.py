...
import pyspiel
FLAGS = flags.FLAGS
flags.DEFINE_string('game', 'first_sealed_auction(max_value=6)', 'Game (short name plus optional parameters).')
flags.DEFINE_float('tol', 1e-07, 'Tolerance for determining dominance.')
flags.DEFINE_enum('mode', 'all', ['all', 'pure', 'one'], 'Whether to find all extreme equilibria, all pure equilibria, or just one equilibrium.')
flags.DEFINE_enum('solver', 'nashpy', ['nashpy', 'lrsnash', 'linear'], 'Solver to use for finding mixed equilibria. (lrsnash needs to be installed separately to work.)')
flags.DEFINE_string('lrsnash_path', None, 'Full path to lrsnash solver (searches PATH by default).')
flags.DEFINE_integer('lrsnash_max_denom', 1000, 'Maximum denominator to use when converting payoffs to rationals for lrsnash solver.')
