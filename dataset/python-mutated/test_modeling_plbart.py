...
if is_torch_available():
    import torch
    from transformers import AutoTokenizer, PLBartForCausalLM, PLBartForConditionalGeneration, PLBartForSequenceClassification, PLBartModel
    from transformers.models.plbart.modeling_plbart import PLBartDecoder, PLBartEncoder

def prepare_plbart_inputs_dict(config, input_ids, decoder_input_ids, attention_mask=None, decoder_attention_mask=None, head_mask=None, decoder_head_mask=None, cross_attn_head_mask=None):
    if False:
        return 10
...
class PLBartModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        ...
        output_with_past_key_values = model(next_tokens, attention_mask=next_attention_mask, past_key_values=past_key_values)
...
class PLBartJavaCsIntegrationTest(AbstractSeq2SeqIntegrationTest):
    ...
    src_text = ['public int maximum(int a, int b, int c){return Math.max(a, Math.max(b, c));}', 'public int product(int a, int b, int c){return a*b*c;}']
    tgt_text = ['public int maximum(int a, int b, int c){return Math.Max(', 'public int Product(int a, int b, int c){return a * b *']
    ...
    def test_java_cs_generate_one(self):
        if False:
            while True:
                i = 10
        batch = self.tokenizer(['public int maximum(int a, int b, int c){return Math.max(a, Math.max(b, c));}'], return_tensors='pt')
    ...
    def test_plbart_fast_forward(self):
        if False:
            i = 10
            return i + 15
        config = PLBartConfig(vocab_size=99, d_model=24, encoder_layers=2, decoder_layers=2, encoder_attention_heads=2, decoder_attention_heads=2, encoder_ffn_dim=32, decoder_ffn_dim=32, max_position_embeddings=48, add_final_layer_norm=True)
...
class PLBartBaseIntegrationTest(AbstractSeq2SeqIntegrationTest):
    ...
    def test_base_generate(self):
        ...
        translated_tokens = self.model.generate(input_ids=inputs['input_ids'].to(torch_device), decoder_start_token_id=self.tokenizer.lang_code_to_id[src_lan])
        decoded = self.tokenizer.batch_decode(translated_tokens, skip_special_tokens=True)
        self.assertEqual(self.tgt_text[0], decoded[0])
    ...
    def test_fill_mask(self):
        ...
        src_lan = self.tokenizer._convert_lang_code_special_format('en_XX')
        outputs = self.model.generate(inputs['input_ids'], decoder_start_token_id=self.tokenizer.lang_code_to_id[src_lan], num_beams=1)
