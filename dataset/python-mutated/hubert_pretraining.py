...
class HubertPretrainingConfig(FairseqDataclass):
    ...
    fine_tuning: bool = field(default=False, metadata={'help': 'set to true if fine-tuning Hubert'})
    labels: List[str] = field(default_factory=lambda : ['ltr'], metadata={'help': 'extension of the label files to load, frame-level labels for pre-training, and sequence-level label for fine-tuning'})
    label_dir: Optional[str] = field(default=None, metadata={'help': 'if set, looks for labels in this directory instead'})
    label_rate: float = field(default=-1.0, metadata={'help': 'label frame rate. -1.0 for sequence label'})
    ...
    single_target: Optional[bool] = field(default=False, metadata={'help': 'if set, AddTargetDatasets outputs same keys as AddTargetDataset'})
