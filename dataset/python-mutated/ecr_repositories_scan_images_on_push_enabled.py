    ...
    def execute(self):
        ...
        for registry in ecr_client.registries.values():
            for repository in registry.repositories:
                ...
                report.status = 'PASS'
                report.status_extended = f'ECR repository {repository.name} has scan on push enabled.'
                if not repository.scan_on_push:
                    report.status = 'FAIL'
                    report.status_extended = f'ECR repository {repository.name} has scan on push disabled.'
