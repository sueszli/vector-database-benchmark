...
if is_torch_available():
    ...
    from transformers import BlipForConditionalGeneration, BlipForImageTextRetrieval, BlipForQuestionAnswering, BlipModel, BlipTextModel, BlipVisionModel
...
class BlipModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': BlipModel, 'image-to-text': BlipForConditionalGeneration} if is_torch_available() else {}
    fx_compatible = False
    test_head_masking = False
    test_pruning = False
    test_resize_embeddings = False
...
class BlipVQAModelTester:
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        config_and_inputs = self.prepare_config_and_inputs()
        (config, input_ids, attention_mask, pixel_values) = config_and_inputs
        inputs_dict = {'input_ids': input_ids, 'labels': input_ids, 'decoder_input_ids': input_ids, 'attention_mask': attention_mask, 'pixel_values': pixel_values}
        return (config, inputs_dict)
...
class BlipVQAModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_forward_signature(self):
        ...
        for model_class in self.all_model_classes:
            ...
            expected_args = ['input_ids', 'attention_mask', 'labels', 'decoder_input_ids', 'decoder_attention_mask']
            for arg in expected_args:
                self.assertTrue(arg in args, f'Argument {arg} of forward function signature should include {arg}. Found {args}.')
...
class BlipModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_captioning(self):
        ...
        predictions = model.generate(**inputs)
        self.assertEqual(predictions[0].tolist(), [30522, 1037, 3861, 1997, 1037, 2450, 1998, 2014, 3899, 2006, 1996, 3509, 102])
    ...
    def test_inference_image_captioning_fp16(self):
        if False:
            return 10
        model = BlipForConditionalGeneration.from_pretrained('Salesforce/blip-image-captioning-base', torch_dtype=torch.float16).to(torch_device)
        processor = BlipProcessor.from_pretrained('Salesforce/blip-image-captioning-base')
        image = prepare_img()
    ...
    def test_inference_itm(self):
        ...
        self.assertTrue(torch.allclose(torch.nn.Softmax()(out_itm[0].cpu()), expected_scores, rtol=0.001, atol=0.001))
        self.assertTrue(torch.allclose(out[0].cpu(), torch.Tensor([[0.5162]]), rtol=0.001, atol=0.001))