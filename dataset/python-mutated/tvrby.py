...
@pluginmatcher(re.compile('https?://(?:www\\.)?tvr\\.by/televidenie/belarus'))
class TVRBy(Plugin):
    file_re = re.compile('(?P<url>https://stream\\.hoster\\.by[^"\',]+\\.m3u8[^"\',]*)')
    player_re = re.compile('["\'](?P<url>[^"\']+tvr\\.by/plugines/online-tv-main\\.php[^"\']+)["\']')
    stream_schema = validate.Schema(validate.all(validate.transform(file_re.finditer), validate.transform(list), [validate.get('url')], validate.transform(set), validate.transform(list)))
