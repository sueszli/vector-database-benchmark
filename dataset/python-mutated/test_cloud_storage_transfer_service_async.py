...
import pytest
from airflow.providers.google.cloud.hooks.cloud_storage_transfer_service import CloudDataTransferServiceAsyncHook
...
class TestCloudDataTransferServiceAsyncHook:
    ...
    async def test_get_jobs(self, mock_list_jobs_request, mock_get_conn):
        ...
        mock_list_jobs_request.assert_called_once_with(filter=json.dumps(dict(project_id=TEST_PROJECT_ID, job_names=job_names)))
    ...
    async def test_get_last_operation(self, mock_deserialize, mock_conn, hook_async):
        ...
        operation = await hook_async.get_latest_operation(job=mock.MagicMock(latest_operation_name=latest_operation_name))
