...
from ding.framework.middleware import OffPolicyLearner, StepCollector, interaction_evaluator, data_pusher, CkptSaver, online_logger, termination_checker
...
def main():
    ...
    with task.start(async_mode=False, ctx=OnlineRLContext()):
        ...
        collector_env = SubprocessEnvManagerV2(env_fn=[lambda : AtariEnv(cfg=c) for c in collector_env_cfg], cfg=cfg.env.manager)
        evaluator_env = SubprocessEnvManagerV2(env_fn=[lambda : AtariEnv(cfg=c) for c in evaluator_env_cfg], cfg=cfg.env.manager)
        ...
        task.use(termination_checker(max_env_step=10000000.0))
        task.run()
