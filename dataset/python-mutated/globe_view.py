...
layers = [pdk.Layer('GeoJsonLayer', id='base-map', data=COUNTRIES, stroked=False, filled=True, get_fill_color=[200, 200, 200]), pdk.Layer('ColumnLayer', id='power-plant', data=df, get_elevation='capacity_mw', get_position=['longitude', 'latitude'], elevation_scale=100, pickable=True, auto_highlight=True, radius=20000, get_fill_color='color')]
deck = pdk.Deck(views=[view], initial_view_state=view_state, tooltip={'text': '{name}, {primary_fuel} plant, {country}'}, layers=layers, map_provider=None, parameters={'cull': True})
