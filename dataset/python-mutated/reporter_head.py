...
from ray._private.ray_constants import DEBUG_AUTOSCALING_ERROR, DEBUG_AUTOSCALING_STATUS, DEBUG_AUTOSCALING_STATUS_LEGACY, GLOBAL_GRPC_OPTIONS, KV_NAMESPACE_CLUSTER
...
EMOJI_WARNING = '&#x26A0;&#xFE0F;'
WARNING_FOR_MULTI_TASK_IN_A_WORKER = 'Warning: This task is running in a worker process that is running multiple tasks. This can happen if you are profiling a task right as it finishes or if youare using the Async Actor or Threaded Actors pattern. The information that follows may come from any of these tasks:'
SVG_STYLE = '<style>\n    svg {\n        width: 100%;\n        height: 100%;\n    }\n</style>\n'
...
class ReportHead(dashboard_utils.DashboardHeadModule):
    ...
    def __init__(self, dashboard_head):
        ...
        self.service_discovery = PrometheusServiceDiscoveryWriter(self._gcs_address, temp_dir)
    ...
    async def _update_stubs(self, change):
        ...
        if change.new:
            ...
            channel = ray._private.utils.init_grpc_channel(f'{ip}:{ports[1]}', options=options, asynchronous=True)
            stub = reporter_pb2_grpc.ReporterServiceStub(channel)
            self._stubs[ip] = stub

    @routes.get('/api/v0/cluster_metadata')
    async def get_cluster_metadata(self, req):
        return dashboard_optional_utils.rest_response(success=True, message='', **self.cluster_metadata)
    ...
    async def get_cluster_status(self, req):
        ...
        (legacy_status, formatted_status_string, error) = await asyncio.gather(*[self._gcs_aio_client.internal_kv_get(key.encode(), namespace=None, timeout=GCS_RPC_TIMEOUT_SECONDS) for key in [DEBUG_AUTOSCALING_STATUS_LEGACY, DEBUG_AUTOSCALING_STATUS, DEBUG_AUTOSCALING_ERROR]])
        formatted_status = json.loads(formatted_status_string.decode()) if formatted_status_string else {}
        if not return_formatted_output:
            return dashboard_optional_utils.rest_response(success=True, message='Got cluster status.', autoscaling_status=legacy_status.decode() if legacy_status else None, autoscaling_error=error.decode() if error else None, cluster_status=formatted_status if formatted_status else None)
        else:
            return dashboard_optional_utils.rest_response(success=True, message='Got formatted cluster status.', cluster_status=debug_status(formatted_status_string, error, address=self._gcs_address))
    ...
    async def get_task_ids_running_in_a_worker(self, worker_id: str) -> List[str]:
        ...
        """
        option = ListApiOptions(filters=[('worker_id', '=', worker_id), ('state', '=', 'RUNNING')], detail=True, timeout=10)
        tasks_in_a_worker_result = await self._state_api.list_tasks(option=option)
        tasks_in_a_worker = tasks_in_a_worker_result.result
        task_ids_in_a_worker = [task.get('task_id') for task in tasks_in_a_worker if task and 'task_id' in task]
        return task_ids_in_a_worker

    async def get_worker_details_for_running_task(self, task_id: str, attempt_number: int) -> Tuple[Optional[int], Optional[str]]:
        """
        ...
        option = ListApiOptions(filters=[('task_id', '=', task_id), ('attempt_number', '=', attempt_number)], detail=True, timeout=10)
        ...
        state = tasks[0]['state']
        if state != 'RUNNING':
            raise ValueError(f'The task attempt is not running: the current state is {state}.')
    ...
    async def get_task_traceback(self, req) -> aiohttp.web.Response:
        ...
        try:
            (pid, _) = await self.get_worker_details_for_running_task(task_id, attempt_number)
        except ValueError as e:
            raise aiohttp.web.HTTPInternalServerError(text=str(e))
        logger.info('Sending stack trace request to {}:{} with native={}'.format(ip, pid, native))
        reply = await reporter_stub.GetTraceback(reporter_pb2.GetTracebackRequest(pid=pid, native=native))
        ...
        try:
            (_, worker_id) = await self.get_worker_details_for_running_task(task_id, attempt_number)
        except ValueError as e:
            raise aiohttp.web.HTTPInternalServerError(text=str(e))
        ...
        return aiohttp.web.Response(text=WARNING_FOR_MULTI_TASK_IN_A_WORKER + str(task_ids_in_a_worker) + '\n' + reply.output if len(task_ids_in_a_worker) > 1 else reply.output)
    ...
    async def get_task_cpu_profile(self, req) -> aiohttp.web.Response:
        ...
        try:
            ...
            raise aiohttp.web.HTTPInternalServerError(text=str(e))
        logger.info('Sending CPU profiling request to {}:{} for {} with native={}'.format(ip, pid, task_id, native))
        reply = await reporter_stub.CpuProfiling(reporter_pb2.CpuProfilingRequest(pid=pid, duration=duration, format=format, native=native))
        ...
        task_ids_in_a_worker = await self.get_task_ids_running_in_a_worker(worker_id)
        return aiohttp.web.Response(body='<p style="color: #E37400;">{} {} </br> </p> </br>'.format(EMOJI_WARNING, WARNING_FOR_MULTI_TASK_IN_A_WORKER + str(task_ids_in_a_worker)) + SVG_STYLE + reply.output if len(task_ids_in_a_worker) > 1 else SVG_STYLE + reply.output, headers={'Content-Type': 'text/html'})
    ...
    async def get_traceback(self, req) -> aiohttp.web.Response:
        ...
        native = req.query.get('native', False) == '1'
        logger.info('Sending stack trace request to {}:{} with native={}'.format(req.query.get('ip'), pid, native))
        reply = await reporter_stub.GetTraceback(reporter_pb2.GetTracebackRequest(pid=pid, native=native))
        if reply.success:
            logger.info('Returning stack trace, size {}'.format(len(reply.output)))
    ...
    async def cpu_profile(self, req) -> aiohttp.web.Response:
        ...
        format = req.query.get('format', 'flamegraph')
        native = req.query.get('native', False) == '1'
        logger.info('Sending CPU profiling request to {}:{} with native={}'.format(req.query.get('ip'), pid, native))
        reply = await reporter_stub.CpuProfiling(reporter_pb2.CpuProfilingRequest(pid=pid, duration=duration, format=format, native=native))
        if reply.success:
            logger.info('Returning profiling response, size {}'.format(len(reply.output)))
            return aiohttp.web.Response(body=reply.output, headers={'Content-Type': 'image/svg+xml' if format == 'flamegraph' else 'text/plain'})
        else:
            return aiohttp.web.HTTPInternalServerError(text=reply.output)

    async def run(self, server):
        gcs_channel = self._dashboard_head.aiogrpc_gcs_channel
        self._state_api_data_source_client = StateDataSourceClient(gcs_channel, self._dashboard_head.gcs_aio_client)
        ...
        cluster_metadata = await self._dashboard_head.gcs_aio_client.internal_kv_get(CLUSTER_METADATA_KEY, namespace=KV_NAMESPACE_CLUSTER)
        ...
        while True:
            try:
                ...
                logger.exception('Error receiving node physical stats from reporter agent.')
