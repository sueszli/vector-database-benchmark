...
class GiftCardUpdateInput(GiftCardInput):
    remove_tags = NonNullList(graphene.String, description='The gift card tags to remove.' + ADDED_IN_31)
    balance_amount = PositiveDecimal(description='The gift card balance amount.' + ADDED_IN_31, required=False)
...
class GiftCardUpdate(GiftCardCreate):
    ...
    class Arguments:
        ...
        input = GiftCardUpdateInput(required=True, description='Fields required to update a gift card.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.GIFT_CARD_UPDATED, description='A gift card was updated.')]
