...
from homeassistant.const import ATTR_IDENTIFIERS, ATTR_MANUFACTURER, ATTR_MODEL, ATTR_NAME, ATTR_VIA_DEVICE, CONF_HOST, CONF_PORT
from homeassistant.core import HomeAssistant
from homeassistant.exceptions import HomeAssistantError
from homeassistant.helpers import device_registry as dr
from homeassistant.helpers.device_registry import DeviceInfo
from .const import DEVICE_SUPPORT, DOMAIN, MANUFACTURER_EDS, MANUFACTURER_HOBBYBOARDS, MANUFACTURER_MAXIM
from .model import OWDeviceDescription
DEVICE_COUPLERS = {'1F': ['aux', 'main']}
DEVICE_MANUFACTURER = {'7E': MANUFACTURER_EDS, 'EF': MANUFACTURER_HOBBYBOARDS}
...
class OneWireHub:
    ...
    async def connect(self, host: str, port: int) -> None:
        ...
        try:
            self.owproxy = await self.hass.async_add_executor_job(protocol.proxy, host, port)
    ...
    async def initialize(self, config_entry: ConfigEntry) -> None:
        ...
        for device in self.devices:
            ...
            device_registry.async_get_or_create(config_entry_id=config_entry.entry_id, identifiers=device_info[ATTR_IDENTIFIERS], manufacturer=device_info[ATTR_MANUFACTURER], model=device_info[ATTR_MODEL], name=device_info[ATTR_NAME], via_device=device_info.get(ATTR_VIA_DEVICE))
