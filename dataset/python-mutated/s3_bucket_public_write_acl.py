                    ...
                    if not (bucket.public_access_block.ignore_public_acls and bucket.public_access_block.restrict_public_buckets):
                        for grantee in bucket.acl_grantees:
                            if grantee.type in 'Group':
                                if ('AllUsers' in grantee.URI or 'AuthenticatedUsers' in grantee.URI) and (grantee.permission == 'FULL_CONTROL' or grantee.permission == 'WRITE' or grantee.permission == 'WRITE_ACP'):
                                    report.status = 'FAIL'
