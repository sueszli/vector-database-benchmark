...
async def test_upload(http_client: HttpClient):
    ...
    response = await http_client.query(query, variables={'textFile': None}, files={'textFile': f})
...
async def test_file_list_upload(http_client: HttpClient):
    ...
    response = await http_client.query(query=query, variables={'files': [None, None]}, files={'file1': file1, 'file2': file2})
...
async def test_nested_file_list(http_client: HttpClient):
    ...
    file1 = BytesIO(b'strawberry1')
    file2 = BytesIO(b'strawberry2')
    response = await http_client.query(query=query, variables={'folder': {'files': [None, None]}}, files={'file1': file1, 'file2': file2})
    data = response.json['data']
...
async def test_upload_single_and_list_file_together(http_client: HttpClient):
    ...
    response = await http_client.query(query=query, variables={'files': [None, None], 'textFile': None}, files={'file1': file1, 'file2': file2, 'textFile': file3})
    data = response.json['data']
    assert len(data['readFiles']) == 2
    assert data['readFiles'][0] == 'strawberry1'
    assert data['readFiles'][1] == 'strawberry2'
...
async def test_upload_invalid_query(http_client: HttpClient):
    ...
    assert response.json['errors'] == [{'locations': [{'column': 5, 'line': 4}], 'message': 'Syntax Error: Expected Name, found <EOF>.'}]

async def test_upload_missing_file(http_client: HttpClient):
    f = BytesIO(b'strawberry')
    query = '\n    mutation($textFile: Upload!) {\n        readText(textFile: $textFile)\n    }\n    '
    response = await http_client.query(query, variables={'textFile': None}, files={'a': f})
...
async def test_extra_form_data_fields_are_ignored(http_client: HttpClient):
    ...
    f = BytesIO(b'strawberry')
    fields = {'operations': operations, 'map': file_map, 'extra_field': extra_field_data, 'textFile': ('textFile.txt', f.read(), 'text/plain')}
    (data, header) = encode_multipart_formdata(fields)
    response = await http_client.post(url='/graphql', data=data, headers={'content-type': header, 'content-length': f'{len(data)}'})
    assert response.status_code == 200
    assert response.json['data'] == {'readText': 'strawberry'}
...
async def test_sending_invalid_form_data(http_client: HttpClient):
    ...
    assert response.status_code == 400
    assert 'No GraphQL query found in the request' in response.text or 'Unable to parse the multipart body' in response.text
...
async def test_sending_invalid_json_body(http_client: HttpClient):
    ...
    operations = '}'
    file_map = json.dumps({'textFile': ['variables.textFile']})
    fields = {'operations': operations, 'map': file_map, 'textFile': ('textFile.txt', f.read(), 'text/plain')}
    (data, header) = encode_multipart_formdata(fields)
    response = await http_client.post('/graphql', data=data, headers={'content-type': header, 'content-length': f'{len(data)}'})
    assert response.status_code == 400
    assert 'Unable to parse the multipart body' in response.text or 'Unable to parse request body as JSON' in response.text