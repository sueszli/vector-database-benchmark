...
class TestCollectiveRunnerBase:
    ...
    def initCommunicator(self, program, rank, nranks, wait_port, current_endpoint, endpoints):
        ...
        nccl_id_var = block.create_var(name=nameGen.generate('nccl_id'), persistable=True, type=core.VarDesc.VarType.RAW)
        block.append_op(type='c_gen_nccl_id', inputs={}, outputs={'Out': nccl_id_var}, attrs={'rank': rank, 'endpoint': current_endpoint, 'other_endpoints': other_endpoints})
        block.append_op(type='c_comm_init', inputs={'X': nccl_id_var}, outputs={}, attrs={'nranks': nranks, 'rank': rank, 'ring_id': self.global_ring_id})
...
def runtime_main(test_class, col_type, sub_type):
    ...
    args['dynamic_static_unified_comm'] = bool(int(os.getenv('FLAGS_dynamic_static_unified_comm', '0')))
...
class TestDistBase(unittest.TestCase):
    ...
    def _run_cluster(self, model_file, envs):
        ...
        env0 = {'FLAGS_selected_gpus': '0', 'PADDLE_TRAINER_ID': '0', 'PADDLE_TRAINERS_NUM': '2', 'PADDLE_TRAINER_ENDPOINTS': self._ps_endpoints, 'PADDLE_CURRENT_ENDPOINT': w0_ep}
        env1 = {'FLAGS_selected_gpus': '1', 'PADDLE_TRAINER_ID': '1', 'PADDLE_TRAINERS_NUM': '2', 'PADDLE_TRAINER_ENDPOINTS': self._ps_endpoints, 'PADDLE_CURRENT_ENDPOINT': w1_ep}
        cur_pid = os.getpid()
    ...
    def check_with_place(self, model_file, col_type, check_error_log=False, need_envs={}):
        if False:
            i = 10
            return i + 15
        required_envs = {'FLAGS_fraction_of_gpu_memory_to_use': '0.15', 'FLAGS_eager_delete_tensor_gb': '0.0', 'PATH': os.getenv('PATH'), 'PYTHONPATH': os.getenv('PYTHONPATH', ''), 'LD_LIBRARY_PATH': os.getenv('LD_LIBRARY_PATH', ''), 'LD_PRELOAD': os.getenv('LD_PRELOAD', ''), 'GLOG_v': '3', 'NCCL_P2P_DISABLE': '1', 'FLAGS_dynamic_static_unified_comm': '0', 'DTYPE': 'float32'}
        ...
        if col_type == 'allgather':
            ...
            need_result = input1 + input2
            np.testing.assert_allclose(tr0_out[0], need_result, rtol=1e-05, atol=1e-05)
            np.testing.assert_allclose(tr1_out[0], need_result, rtol=1e-05, atol=1e-05)
        elif col_type == 'reduce_scatter':
            tmp = input1 + input2
            need_result1 = tmp[0:tmp.shape[0] // 2]
            need_result2 = tmp[tmp.shape[0] // 2:]
            np.testing.assert_allclose(tr0_out[0], need_result1, rtol=1e-05, atol=1e-05)
            np.testing.assert_allclose(tr1_out[0], need_result2, rtol=1e-05, atol=1e-05)
