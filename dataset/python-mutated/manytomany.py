...
class LogoColor(TestModel):
    ...
    color = ForeignKeyField(Color, field=Color.name)
...
class TestManyToMany(ModelTestCase):
    ...
    user_to_note = {'gargie': [1, 2], 'huey': [2, 3], 'mickey': [3, 4], 'zaizee': [4, 5]}
...
class TestForeignKeyPrimaryKeyManyToMany(ModelTestCase):
    ...
    test_data = (('huey', ('cats', 'evil')), ('zaizee', ('cats', 'good')), ('mickey', ('dogs', 'good')), ('zombie', ()))
...
class TestMultipleManyToManySameTables(ModelTestCase):
    database = get_in_memory_db()
    requires = [Permission, Visitor, Visitor.allowed.through_model, Visitor.denied.through_model]
