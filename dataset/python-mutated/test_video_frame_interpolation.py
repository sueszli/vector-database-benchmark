...
class VideoFrameInterpolationTest(unittest.TestCase):
    ...
    def test_run_by_direct_model_download(self):
        ...
        pipeline = VideoFrameInterpolationPipeline(cache_path)
        pipeline.group_key = self.task
        out_video_path = pipeline(input={'video': self.test_video})[OutputKeys.OUTPUT_VIDEO]
        print('pipeline: the output video path is {}'.format(out_video_path))
    ...
    def test_run_with_model_from_modelhub(self):
        if False:
            return 10
        pipeline_ins = pipeline(task=Tasks.video_frame_interpolation, model=self.model_id)
        out_video_path = pipeline_ins(input=self.test_video)[OutputKeys.OUTPUT_VIDEO]
        print('pipeline: the output video path is {}'.format(out_video_path))
        pipeline_ins = pipeline(task=Tasks.video_frame_interpolation, model=self.model_practical_id)
        out_video_path = pipeline_ins(input={'video': self.test_video})[OutputKeys.OUTPUT_VIDEO]
        print('pipeline: the output video path is {}'.format(out_video_path))
