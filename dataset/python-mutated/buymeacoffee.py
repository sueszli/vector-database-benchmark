...
async def buymeacoffee(email, client, out):
    ...
    def get_random_string(length):
        ...
        return result_str
    headers = {'User-Agent': random.choice(ua['browsers']['chrome']), 'Accept': 'application/json, text/javascript, */*; q=0.01', 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', 'X-Requested-With': 'XMLHttpRequest', 'Origin': 'https://www.buymeacoffee.com', 'DNT': '1', 'TE': 'Trailers'}
    ...
    r = await client.post('https://www.buymeacoffee.com/auth/validate_email_and_password', headers=headers, cookies=cookies, data=data)
    if r.status_code == 200:
        data = r.json()
