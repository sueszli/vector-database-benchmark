...
def downgrade():
    if False:
        return 10
    with op.batch_alter_table('flow_run', schema=None) as batch_op:
        batch_op.add_column(sa.Column('flow_runner_config', sqlite.JSON(), nullable=True))
        batch_op.add_column(sa.Column('empirical_config', sqlite.JSON(), server_default=sa.text("'{}'"), nullable=False))
        batch_op.add_column(sa.Column('flow_runner_type', sa.VARCHAR(), nullable=True))
        batch_op.create_index('ix_flow_run__flow_runner_type', ['flow_runner_type'], unique=False)
    with op.batch_alter_table('deployment', schema=None) as batch_op:
        batch_op.add_column(sa.Column('flow_runner_config', sqlite.JSON(), nullable=True))
        batch_op.add_column(sa.Column('flow_runner_type', sa.VARCHAR(), nullable=True))