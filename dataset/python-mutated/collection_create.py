...
from ....core.types import BaseInputObjectType, CollectionError, NonNullList, SeoInput, Upload
...
class CollectionInput(BaseInputObjectType):
    ...
    description = JSONString(description='Description of the collection.' + RICH_CONTENT)
    background_image = Upload(description='Background image file.')
    background_image_alt = graphene.String(description='Alt text for an image.')
    seo = SeoInput(description='Search engine optimization fields.')
    publication_date = Date(description=f'Publication date. ISO 8601 standard. {DEPRECATED_IN_3X_INPUT}')
    metadata = NonNullList(MetadataInput, description='Fields required to update the collection metadata.' + ADDED_IN_38, required=False)
    private_metadata = NonNullList(MetadataInput, description='Fields required to update the collection private metadata.' + ADDED_IN_38, required=False)

    class Meta:
        doc_category = DOC_CATEGORY_PRODUCTS

class CollectionCreateInput(CollectionInput):
    products = NonNullList(graphene.ID, description='List of products to be added to the collection.', name='products')
...
class CollectionCreate(ModelMutation):
    ...
    class Arguments:
        input = CollectionCreateInput(required=True, description='Fields required to create a collection.')
