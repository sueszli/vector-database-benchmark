...
def set_order_values(apps, _schema_editor):
    ...
    queryset = GiftCardEvent.objects.filter(type__in=['used_in_order', 'bought']).order_by('pk')
...
class Migration(migrations.Migration):
    dependencies = [('giftcard', '0013_giftcardevent_order')]
    operations = [migrations.RunPython(set_order_values, migrations.RunPython.noop)]