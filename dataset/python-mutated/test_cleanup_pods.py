...
class TestCleanupDeployment:
    ...
    cron_tests = [('release-name', '*/5 * * * *', '*/5 * * * *'), ('something-else', '@hourly', '@hourly'), ('custom-name', '{{- add 3 (regexFind ".$" (adler32sum .Release.Name)) -}}-59/15 * * * *', '7-59/15 * * * *'), ('airflow-rules', '{{- add 3 (regexFind ".$" (adler32sum .Release.Name)) -}}-59/15 * * * *', '10-59/15 * * * *')]
