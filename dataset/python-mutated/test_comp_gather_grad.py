...
@param.parameterized_class(('primal0', 'index', 'axis', 'x_dtype', 'index_dtype', 'v', 'count'), [(np.random.rand(100), np.array([1, 3, 5]), 0, np.float32, np.int32, np.random.rand(3), 0), (np.random.rand(10, 20), np.array([1, 3, 5]), 0, np.float64, np.int64, np.random.rand(3, 20), 1), (np.random.rand(10, 20), np.array([1, 1, 3]), 0, np.float32, np.int32, np.random.rand(3, 20), 2), (np.random.rand(3, 88, 30), np.array([1, 3, 5]), 1, np.float32, np.int32, np.random.rand(3, 3, 30), 3), (np.random.rand(10, 88, 10), np.array([1, 3, 5]), 0, np.float16, np.int32, np.random.rand(3, 88, 10), 4)])
class TestGatherGradComp(unittest.TestCase):
    ...
    def test_cinn(self):
        ...
        for i in range(len(dy_res)):
            np.testing.assert_allclose(comp_st_cinn_res[i].numpy(), dy_res[i].numpy(), rtol=1e-06, atol=1e-06)
    ...
    def test_tanh_grad_comp(self):
        ...
        def actual(primal0, index, axis, v):
            ...
            with paddle.static.program_guard(mp, sp):
                ...
                index_tmp = paddle.static.data('index', index.shape, index.dtype)
            ...
            exe.run(sp)
            out = exe.run(program=mp, feed={'primal0': primal0, 'index': index, 'v': v}, fetch_list=[res[0].name])
            return out[0]
        ...
        dx = None
        ddx = None
        if not (self.count == 4 and isinstance(framework._current_expected_place(), framework.core.CPUPlace)):
            dx = actual(self.primal0, self.index, self.axis, self.v)
            ddx = desired(self.primal0, self.index, self.axis, self.v)
        if self.count >= 3 and isinstance(framework._current_expected_place(), framework.core.CPUPlace):
            pass
        elif self.count == 4 and (not isinstance(framework._current_expected_place(), framework.core.CPUPlace)):
            np.testing.assert_allclose(actual=dx, desired=ddx, rtol=0.001, atol=0)
        elif self.count == 1:
            np.testing.assert_allclose(actual=dx, desired=ddx, rtol=1e-15, atol=1e-15)
        else:
            np.testing.assert_allclose(actual=dx, desired=ddx, rtol=1e-05, atol=0)
        core._set_prim_backward_enabled(False)
        paddle.disable_static()
