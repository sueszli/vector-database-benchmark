...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test scipy\'s hyp2f1 against mpmath\'s on a grid in the complex plane over a grid of parameter values. Saves output to file specified in positional argument "outpath". Caution: With default arguments, the generated output file is roughly 700MB in size. Script may take several hours to finish if "--n_jobs" is set to 1.')
    parser.add_argument('outpath', type=str, help='Path to output tsv file.')
    parser.add_argument('--n_jobs', type=int, default=1, help='Number of jobs for multiprocessing.')
    parser.add_argument('--box_size', type=float, default=2.0, help='hyp2f1 is evaluated in box of side_length 2*box_size centered at the origin.')
    parser.add_argument('--grid_size', type=int, default=20, help='hyp2f1 is evaluated on grid_size * grid_size grid in box of side length 2*box_size centered at the origin.')
    parser.add_argument('--parameter_groups', type=int, nargs='+', default=None, help='Restrict to supplied parameter groups. See the Docstring for this module for more info on parameter groups. Calculate for all parameter groups by default.')
    parser.add_argument('--regions', type=int, nargs='+', default=None, help='Restrict to argument z only within the supplied regions. See the Docstring for this module for more info on regions. Calculate for all regions by default.')
    parser.add_argument('--no_mp', action='store_true', help='If this flag is set, do not compute results with mpmath. Saves time if results have already been computed elsewhere. Fills in "expected" column with None values.')
    args = parser.parse_args()
    compute_mp = not args.no_mp
    print(args.parameter_groups)
    main(args.outpath, n_jobs=args.n_jobs, box_size=args.box_size, grid_size=args.grid_size, parameter_groups=args.parameter_groups, regions=args.regions, compute_mp=compute_mp)