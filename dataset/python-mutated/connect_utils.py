...
_ConnectionParameters = collections.namedtuple('ConnectionParameters', ['user', 'password', 'database', 'ssl', 'sslmode', 'direct_tls', 'server_settings', 'target_session_attrs'])
_ClientConfiguration = collections.namedtuple('ConnectionConfiguration', ['command_timeout', 'statement_cache_size', 'max_cached_statement_lifetime', 'max_cacheable_statement_size'])
...
async def _create_ssl_connection(protocol_factory, host, port, *, loop, ssl_context, ssl_is_advisory=False):
    (tr, pr) = await loop.create_connection(lambda : TLSUpgradeProto(loop, host, port, ssl_context, ssl_is_advisory), host, port)
    ...
    if hasattr(loop, 'start_tls'):
        ...
        if do_ssl_upgrade:
            conn_factory = functools.partial(conn_factory, ssl=ssl_context, server_hostname=host)
        sock = _get_socket(tr)
        sock = sock.dup()
        _set_nodelay(sock)
        tr.close()
...
async def __connect_addr(params, retry, addr, loop, config, connection_class, record_class, params_input):
    ...
    if isinstance(addr, str):
        ...
        connector = _create_ssl_connection(proto_factory, *addr, loop=loop, ssl_context=params.ssl, ssl_is_advisory=params.sslmode == SSLMode.prefer)
    else:
        connector = loop.create_connection(proto_factory, *addr)
    (tr, pr) = await connector
    try:
        await connected
    except (exceptions.InvalidAuthorizationSpecificationError, exceptions.ConnectionDoesNotExistError):
        tr.close()
        if retry and (params.sslmode == SSLMode.allow and (not pr.is_ssl) or (params.sslmode == SSLMode.prefer and pr.is_ssl)):
...
async def _accept_any(_):
    return True
target_attrs_check = {SessionAttribute.any: _accept_any, SessionAttribute.primary: _accept_in_hot_standby(False), SessionAttribute.standby: _accept_in_hot_standby(True), SessionAttribute.prefer_standby: _accept_in_hot_standby(True), SessionAttribute.read_write: _accept_read_only(False), SessionAttribute.read_only: _accept_read_only(True)}
...
async def _connect(*, loop, connection_class, record_class, **kwargs):
    ...
    for addr in addrs:
        try:
            conn = await _connect_addr(addr=addr, loop=loop, params=params, config=config, connection_class=connection_class, record_class=record_class)
    ...
    await asyncio.gather(*(c.close() for c in candidates if c is not chosen_connection), return_exceptions=True)
    if chosen_connection:
        return chosen_connection
    raise last_error or exceptions.TargetServerAttributeNotMatched('None of the hosts match the target attribute requirement {!r}'.format(target_attr))

async def _cancel(*, loop, addr, params: _ConnectionParameters, backend_pid, backend_secret):
    ...
    elif params.ssl and params.sslmode != SSLMode.allow:
        (tr, pr) = await _create_ssl_connection(CancelProto, *addr, loop=loop, ssl_context=params.ssl, ssl_is_advisory=params.sslmode == SSLMode.prefer)
    else:
