...
HOOK_TYPES = ('commit-msg', 'post-checkout', 'post-commit', 'post-merge', 'post-rewrite', 'pre-commit', 'pre-merge-commit', 'pre-push', 'pre-rebase', 'prepare-commit-msg')
...
_STAGES = {'commit': 'pre-commit', 'merge-commit': 'pre-merge-commit', 'push': 'pre-push'}
...
_meta = (('check-hooks-apply', (('name', 'Check hooks apply to the repository'), ('files', f'^{re.escape(C.CONFIG_FILE)}$'), ('entry', _entry('check_hooks_apply')))), ('check-useless-excludes', (('name', 'Check for useless excludes'), ('files', f'^{re.escape(C.CONFIG_FILE)}$'), ('entry', _entry('check_useless_excludes')))), ('identity', (('name', 'identity'), ('verbose', True), ('entry', _entry('identity')))))
...
META_HOOK_DICT = cfgv.Map('Hook', 'id', cfgv.Required('id', cfgv.check_string), cfgv.Required('id', cfgv.check_one_of(tuple((k for (k, _) in _meta)))), cfgv.Optional('language', cfgv.check_one_of({'system'}), 'system'), NotAllowed('entry', cfgv.check_any), *(cfgv.ConditionalOptional(key, cfgv.check_any, value, 'id', hook_id) for (hook_id, values) in _meta for (key, value) in values), *(cfgv.OptionalNoDefault(item.key, item.check_fn) if item.key in {'name', 'language', 'entry'} else item for item in MANIFEST_HOOK_DICT.items))
...
LOCAL_HOOK_DICT = cfgv.Map('Hook', 'id', *MANIFEST_HOOK_DICT.items, OptionalSensibleRegexAtHook('files', cfgv.check_string), OptionalSensibleRegexAtHook('exclude', cfgv.check_string))
CONFIG_REPO_DICT = cfgv.Map('Repository', 'repo', cfgv.Required('repo', cfgv.check_string), cfgv.ConditionalRecurse('hooks', cfgv.Array(CONFIG_HOOK_DICT), 'repo', cfgv.NotIn(LOCAL, META)), cfgv.ConditionalRecurse('hooks', cfgv.Array(LOCAL_HOOK_DICT), 'repo', LOCAL), cfgv.ConditionalRecurse('hooks', cfgv.Array(META_HOOK_DICT), 'repo', META), WarnMutableRev('rev', cfgv.check_string, condition_key='repo', condition_value=cfgv.NotIn(LOCAL, META), ensure_absent=True), cfgv.WarnAdditionalKeys(('repo', 'rev', 'hooks'), warn_unknown_keys_repo))
DEFAULT_LANGUAGE_VERSION = cfgv.Map('DefaultLanguageVersion', None, cfgv.NoAdditionalKeys(language_names), *(cfgv.Optional(x, cfgv.check_string, C.DEFAULT) for x in language_names))
CONFIG_SCHEMA = cfgv.Map('Config', None, cfgv.RequiredRecurse('repos', cfgv.Array(CONFIG_REPO_DICT)), cfgv.Optional('default_install_hook_types', cfgv.check_array(cfgv.check_one_of(HOOK_TYPES)), ['pre-commit']), cfgv.OptionalRecurse('default_language_version', DEFAULT_LANGUAGE_VERSION, {}), StagesMigration('default_stages', STAGES), cfgv.Optional('files', check_string_regex, ''), cfgv.Optional('exclude', check_string_regex, '^$'), cfgv.Optional('fail_fast', cfgv.check_bool, False), cfgv.Optional('minimum_pre_commit_version', cfgv.check_and(cfgv.check_string, check_min_version), '0'), cfgv.WarnAdditionalKeys(('repos', 'default_install_hook_types', 'default_language_version', 'default_stages', 'files', 'exclude', 'fail_fast', 'minimum_pre_commit_version', 'ci'), warn_unknown_keys_root), OptionalSensibleRegexAtTop('files', cfgv.check_string), OptionalSensibleRegexAtTop('exclude', cfgv.check_string), cfgv.OptionalNoDefault('ci', cfgv.check_type(dict)))

class InvalidConfigError(FatalError):
    pass
load_config = functools.partial(cfgv.load_from_filename, schema=CONFIG_SCHEMA, load_strategy=yaml_load, exc_tp=InvalidConfigError)