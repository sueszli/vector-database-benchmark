...
@pluginmatcher(re.compile('https?://(?:www\\.)?wasd\\.tv/(?P<nickname>[^/]+)/?$'))
class WASD(Plugin):
    _media_schema = validate.Schema({'user_id': int, 'media_container_online_status': str, 'media_container_status': str, 'media_container_streams': [{'stream_media': [{'media_id': int, 'media_meta': {'media_url': validate.any(str, None), 'media_archive_url': validate.any(str, None)}, 'media_status': validate.any('STOPPED', 'RUNNING'), 'media_type': 'HLS'}]}]})
    _api_schema = validate.Schema({'result': validate.any(_media_schema, validate.all(list, validate.get(0), _media_schema), [])}, validate.get('result'))
    _api_nicknames_schema = validate.Schema({'result': {'channel_id': int}}, validate.get('result'), validate.get('channel_id'))
