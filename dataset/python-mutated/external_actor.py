...
class ExternalActor(ReplicatedRegionModel):
    ...
    provider = BoundedPositiveIntegerField(choices=((ExternalProviders.EMAIL, 'email'), (ExternalProviders.SLACK, 'slack'), (ExternalProviders.MSTEAMS, 'msteams'), (ExternalProviders.PAGERDUTY, 'pagerduty'), (ExternalProviders.GITHUB, 'github'), (ExternalProviders.GITHUB_ENTERPRISE, 'github_enterprise'), (ExternalProviders.GITLAB, 'gitlab'), (ExternalProviders.CUSTOM, 'custom_scm')))
    ...
    class Meta:
        ...
        constraints = [models.CheckConstraint(check=Q(user_id__isnull=False) | Q(team_id__isnull=False), name='external_actor_team_or_user')]
...
post_save.connect(lambda instance, **kwargs: process_resource_change(instance, **kwargs), sender=ExternalActor, weak=False)
post_delete.connect(lambda instance, **kwargs: process_resource_change(instance, **kwargs), sender=ExternalActor, weak=False)