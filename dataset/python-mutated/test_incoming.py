...
async def incoming_transfer():
    settings = EVASettings(block_size=10, termination=Termination(enabled=False))
    eva = EVAProtocol(community=Mock(), settings=settings)
    peer = Mock()
    transfer = IncomingTransfer(container=eva.incoming, info=b'info', data_size=100, nonce=0, protocol_task_group=eva.task_group, send_message=Mock(), on_complete=blank, on_error=blank, peer=peer, settings=settings)
...
async def test_send_acknowledge_updated(incoming_transfer: IncomingTransfer):
    ...
    await asyncio.gather(update_transfer(), incoming_transfer.send_acknowledge())
