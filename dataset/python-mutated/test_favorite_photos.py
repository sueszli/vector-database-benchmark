...
class FavoritePhotosTest(TestCase):
    ...
    def test_tag_my_photos_as_favorite(self):
        ...
        response = self.client.post('/api/photosedit/favorite/', format='json', data=payload, headers=headers)
        data = response.json()
    ...
    def test_untag_my_photos_as_favorite(self):
        if False:
            print('Hello World!')
        photos1 = create_test_photos(number_of_photos=1, owner=self.user1, rating=self.user1.favorite_min_rating)
    ...
    def test_tag_nonexistent_photo_as_favorite(self, logger):
        ...
        self.assertEqual(0, len(data['updated']))
        self.assertEqual(0, len(data['not_updated']))
        logger.assert_called_with('Could not set photo nonexistent_photo to favorite. It does not exist.')