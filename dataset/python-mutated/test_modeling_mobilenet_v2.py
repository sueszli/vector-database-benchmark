...
class MobileNetV2ModelTester:
    ...
    def create_and_check_model(self, config, pixel_values, labels, pixel_labels):
        ...
        self.parent.assertEqual(result.pooler_output.shape, (self.batch_size, self.last_hidden_size))
...
class MobileNetV2ModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': MobileNetV2Model, 'image-classification': MobileNetV2ForImageClassification, 'image-segmentation': MobileNetV2ForSemanticSegmentation} if is_torch_available() else {}
    test_pruning = False
...
class MobileNetV2ModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        if False:
            return 10
        return MobileNetV2ImageProcessor.from_pretrained('google/mobilenet_v2_1.0_224') if is_vision_available() else None
    ...
    def test_inference_semantic_segmentation(self):
        ...
        expected_slice = torch.tensor([[[17.579, 17.7581, 18.3355], [18.3257, 18.423, 18.8973], [18.6169, 18.865, 19.2187]], [[-2.1595, -2.0977, -2.3741], [-2.4226, -2.3028, -2.6835], [-2.7819, -2.5991, -2.7706]], [[4.2058, 4.8317, 4.7638], [4.4136, 5.0361, 4.9383], [4.5028, 4.9644, 4.8734]]], device=torch_device)
