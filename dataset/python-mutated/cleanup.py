...
class Cleanup(commands.Cog):
    ...
    async def check_100_plus(ctx: commands.Context, number: int) -> bool:
        ...
        if ctx.assume_yes:
            return True
        prompt = await ctx.send(_('Are you sure you want to delete {number} messages?').format(number=humanize_number(number)) + ' (yes/no)')
    ...
    @staticmethod
    async def get_messages_for_deletion(*, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.DMChannel, discord.Thread], number: Optional[int]=None, check: Callable[[discord.Message], bool]=lambda x: True, limit: Optional[int]=None, before: Union[discord.Message, datetime]=None, after: Union[discord.Message, datetime]=None, delete_pinned: bool=False) -> List[discord.Message]:
        """
    ...
    async def send_optional_notification(self, num: int, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.DMChannel, discord.Thread], *, subtract_invoking: bool=False) -> None:
        """
        ...
        if not channel.guild or await self.config.guild(channel.guild).notify():
            ...
            if num == 1:
                ...
                await channel.send(_('{num} messages were deleted.').format(num=humanize_number(num)), delete_after=5)

    @staticmethod
    async def get_message_from_reference(channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.Thread], reference: discord.MessageReference) -> Optional[discord.Message]:
        message = None
    ...
    async def text(self, ctx: commands.Context, text: str, number: positive_int, delete_pinned: bool=False):
        ...
        **Arguments:**
        ...
        def check(m):
            ...
            if text in m.content:
                ...
                return False
        to_delete = await self.get_messages_for_deletion(channel=channel, number=number, check=check, before=ctx.message, delete_pinned=delete_pinned)
        to_delete.append(ctx.message)
        reason = "{} ({}) deleted {} messages containing '{}' in channel #{}.".format(author, author.id, humanize_number(len(to_delete), override_locale='en_us'), text, channel.id)
        log.info(reason)
    ...
    @commands.mod_or_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    async def user(self, ctx: commands.Context, user: Union[discord.Member, RawUserIdConverter], number: positive_int, delete_pinned: bool=False):
        ...
        **Arguments:**
        ...
        to_delete = await self.get_messages_for_deletion(channel=channel, number=number, check=check, before=ctx.message, delete_pinned=delete_pinned)
        to_delete.append(ctx.message)
        reason = '{} ({}) deleted {} messages made by {} ({}) in channel #{}.'.format(author, author.id, humanize_number(len(to_delete), override_locale='en_US'), member or '???', _id, channel.name)
        log.info(reason)
        await mass_purge(to_delete, channel, reason=reason)
        await self.send_optional_notification(len(to_delete), channel, subtract_invoking=True)
    ...
    @commands.mod_or_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    async def after(self, ctx: commands.Context, message_id: Optional[RawMessageIds], delete_pinned: bool=False):
        ...
        **Arguments:**
        ...
        to_delete = await self.get_messages_for_deletion(channel=channel, number=None, after=after, delete_pinned=delete_pinned)
        reason = '{} ({}) deleted {} messages in channel #{}.'.format(author, author.id, humanize_number(len(to_delete), override_locale='en_US'), channel.name)
        log.info(reason)
        await mass_purge(to_delete, channel, reason=reason)
    ...
    @commands.mod_or_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    async def before(self, ctx: commands.Context, message_id: Optional[RawMessageIds], number: positive_int, delete_pinned: bool=False):
        ...
        **Arguments:**
        ...
        if before is None:
            raise commands.BadArgument
        to_delete = await self.get_messages_for_deletion(channel=channel, number=number, before=before, delete_pinned=delete_pinned)
        to_delete.append(ctx.message)
        reason = '{} ({}) deleted {} messages in channel #{}.'.format(author, author.id, humanize_number(len(to_delete), override_locale='en_US'), channel.name)
        log.info(reason)
        await mass_purge(to_delete, channel, reason=reason)
    ...
    @commands.mod_or_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    async def between(self, ctx: commands.Context, one: RawMessageIds, two: RawMessageIds, delete_pinned: bool=False):
        ...
        **Arguments:**
        ...
        to_delete = await self.get_messages_for_deletion(channel=channel, before=mtwo, after=mone, delete_pinned=delete_pinned)
        to_delete.append(ctx.message)
        reason = '{} ({}) deleted {} messages in channel #{}.'.format(author, author.id, humanize_number(len(to_delete), override_locale='en_US'), channel.name)
        log.info(reason)
        await mass_purge(to_delete, channel, reason=reason)
    ...
    @commands.mod_or_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    async def messages(self, ctx: commands.Context, number: positive_int, delete_pinned: bool=False):
        ...
        **Arguments:**
        ...
        if number > 100:
            ...
            if not cont:
                return
        to_delete = await self.get_messages_for_deletion(channel=channel, number=number, before=ctx.message, delete_pinned=delete_pinned)
        to_delete.append(ctx.message)
        reason = '{} ({}) deleted {} messages in channel #{}.'.format(author, author.id, len(to_delete), channel.name)
        log.info(reason)
        await mass_purge(to_delete, channel, reason=reason)
        await self.send_optional_notification(len(to_delete), channel, subtract_invoking=True)
    ...
    @commands.mod_or_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    async def cleanup_bot(self, ctx: commands.Context, number: positive_int, delete_pinned: bool=False):
        ...
        **Arguments:**
        ...
        if alias_cog is not None:
            alias_names: Set[str] = set((a.name for a in await alias_cog._aliases.get_global_aliases())) | set((a.name for a in await alias_cog._aliases.get_guild_aliases(ctx.guild)))
        ...
        to_delete = await self.get_messages_for_deletion(channel=channel, number=number, check=check, before=ctx.message, delete_pinned=delete_pinned)
        to_delete.append(ctx.message)
        reason = '{} ({}) deleted {} command messages in channel #{}.'.format(author, author.id, humanize_number(len(to_delete), override_locale='en_US'), channel.name)
        log.info(reason)
        await mass_purge(to_delete, channel, reason=reason)
        await self.send_optional_notification(len(to_delete), channel, subtract_invoking=True)
    ...
    async def cleanup_self(self, ctx: commands.Context, number: positive_int, match_pattern: str=None, delete_pinned: bool=False):
        ...
        **Arguments:**
        ...
        reason = '{} ({}) deleted {} messages sent by the bot in {}.'.format(author, author.id, humanize_number(len(to_delete), override_locale='en_US'), channel_name)
        log.info(reason)
        if can_mass_purge:
            await mass_purge(to_delete, channel, reason=reason)
        else:
        ...
        **Arguments:**
        ...
        def check(m):
            ...
            if c in msgs:
                ...
                msgs.append(c)
                return False
        to_delete = await self.get_messages_for_deletion(channel=ctx.channel, limit=number, check=check, before=ctx.message)
        if len(to_delete) > 100:
            cont = await self.check_100_plus(ctx, len(to_delete))
            if not cont:
                return
        log.info('%s (%s) deleted %s spam messages in channel %s (%s).', ctx.author, ctx.author.id, len(to_delete), ctx.channel, ctx.channel.id)
        to_delete.append(ctx.message)
