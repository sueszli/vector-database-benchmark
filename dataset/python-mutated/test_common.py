...
async def test_with_python_connector_installed_from_setup(context_with_setup, python_connector_base_image_address, latest_cdk_version):
    ...
    container = await common.with_python_connector_installed(context_with_setup, python_container, str(context_with_setup.connector.code_directory))
    cdk_install_latest_output = await container.with_exec(['pip', 'uninstall', '-y', f'airbyte-cdk=={latest_cdk_version}'], skip_entrypoint=True).with_exec(['pip', 'install', f'airbyte-cdk=={latest_cdk_version}'], skip_entrypoint=True).stdout()
