...
COMPARISON_OPERATORS = {'eq': lambda a, b: a == b, 'ne': lambda a, b: a != b, 'g': lambda a, b: a > b, 'l': lambda a, b: a < b, 'ge': lambda a, b: a >= b, 'le': lambda a, b: a <= b}

class SourceManager:
    filter_fields = {'rowid', 'status', 'file_name', 'added_on', 'download_path', 'claim_name', 'claim_height', 'claim_id', 'outpoint', 'txid', 'nout', 'channel_claim_id', 'channel_name', 'completed'}
    set_filter_fields = {'claim_ids': 'claim_id', 'channel_claim_ids': 'channel_claim_id', 'outpoints': 'outpoint'}
    ...
    async def create(self, file_path: str, key: Optional[bytes]=None, iv_generator: Optional[typing.Generator[bytes, None, None]]=None) -> ManagedDownloadSource:
        raise NotImplementedError()
