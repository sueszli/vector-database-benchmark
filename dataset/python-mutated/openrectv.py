...
@pluginmatcher(re.compile('https?://(?:www\\.)?openrec\\.tv/(?:live|movie)/(?P<id>[^/]+)'))
@pluginargument('email', requires=['password'], metavar='EMAIL', help='The email associated with your openrectv account, required to access any openrectv stream.')
@pluginargument('password', sensitive=True, metavar='PASSWORD', help='An openrectv account password to use with --openrectv-email.')
class OPENRECtv(Plugin):
    _stores_re = re.compile('window.stores\\s*=\\s*({.*?});', re.DOTALL | re.MULTILINE)
    _config_re = re.compile('window.sharedConfig\\s*=\\s*({.*?});', re.DOTALL | re.MULTILINE)
    ...
    _info_schema = validate.Schema({validate.optional('id'): str, validate.optional('title'): str, validate.optional('movie_type'): str, validate.optional('onair_status'): validate.any(None, int), validate.optional('public_type'): str, validate.optional('media'): {'url': validate.any(None, validate.url()), 'url_public': validate.any(None, validate.url()), 'url_ull': validate.any(None, validate.url())}, validate.optional('subs_trial_media'): {'url': validate.any(None, validate.url()), 'url_ull': validate.any(None, validate.url())}})
    _subscription_schema = validate.Schema({validate.optional('status'): int, validate.optional('data'): {'items': [{'media': {'url': validate.any(None, validate.url())}}]}})
    _login_schema = validate.Schema({validate.optional('error_message'): str, 'status': int, validate.optional('data'): object})
