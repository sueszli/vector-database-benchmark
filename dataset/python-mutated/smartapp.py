...
from pysmartthings import APP_TYPE_WEBHOOK, CAPABILITIES, CLASSIFICATION_AUTOMATION, App, AppOAuth, AppSettings, InstalledAppStatus, SmartThings, SourceType, Subscription, SubscriptionEntity
...
async def find_app(hass: HomeAssistant, api):
    ...
    for app in [app for app in apps if app.app_name.startswith(APP_NAME_PREFIX)]:
        ...
        if settings.settings.get(SETTINGS_INSTANCE_ID) == hass.data[DOMAIN][CONF_INSTANCE_ID]:
            return app
...
async def validate_installed_app(api, installed_app_id: str):
    ...
    if installed_app.installed_app_status != InstalledAppStatus.AUTHORIZED:
        raise RuntimeWarning("Installed SmartApp instance '{}' ({}) is not AUTHORIZED but instead {}".format(installed_app.display_name, installed_app.installed_app_id, installed_app.installed_app_status))
...
async def create_app(hass: HomeAssistant, api):
    ...
    _LOGGER.debug("Updated App Settings for SmartApp '%s' (%s)", app.app_name, app.app_id)
...
async def update_app(hass: HomeAssistant, app):
    ...
    if update_required:
        await app.save()
        _LOGGER.debug("SmartApp '%s' (%s) updated with latest settings", app.app_name, app.app_id)
...
async def setup_smartapp_endpoint(hass: HomeAssistant, fresh_install: bool):
    ...
    if fresh_install or not (config := (await store.async_load())):
        config = {CONF_INSTANCE_ID: str(uuid4()), CONF_WEBHOOK_ID: secrets.token_hex(), CONF_CLOUDHOOK_URL: None}
        await store.async_save(config)
    webhook.async_register(hass, DOMAIN, 'SmartApp', config[CONF_WEBHOOK_ID], smartapp_webhook)
    cloudhook_url = config.get(CONF_CLOUDHOOK_URL)
    if cloudhook_url is None and cloud.async_active_subscription(hass) and (not hass.config_entries.async_entries(DOMAIN)):
        cloudhook_url = await cloud.async_create_cloudhook(hass, config[CONF_WEBHOOK_ID])
        config[CONF_CLOUDHOOK_URL] = cloudhook_url
        await store.async_save(config)
        _LOGGER.debug("Created cloudhook '%s'", cloudhook_url)
    dispatcher = Dispatcher(signal_prefix=SIGNAL_SMARTAPP_PREFIX, connect=functools.partial(async_dispatcher_connect, hass), send=functools.partial(async_dispatcher_send, hass))
    path = urlparse(cloudhook_url).path if cloudhook_url else webhook.async_generate_path(config[CONF_WEBHOOK_ID])
    ...
    hass.data[DOMAIN] = {DATA_MANAGER: manager, CONF_INSTANCE_ID: config[CONF_INSTANCE_ID], DATA_BROKERS: {}, CONF_WEBHOOK_ID: config[CONF_WEBHOOK_ID], CONF_CLOUDHOOK_URL: config.get(CONF_CLOUDHOOK_URL)}
    _LOGGER.debug('Setup endpoint for %s', cloudhook_url if cloudhook_url else webhook.async_generate_url(hass, config[CONF_WEBHOOK_ID]))
...
async def unload_smartapp_endpoint(hass: HomeAssistant):
    ...
    if cloudhook_url and cloud.async_is_logged_in(hass):
        ...
        store = Store[dict[str, Any]](hass, STORAGE_VERSION, STORAGE_KEY)
        await store.async_save({CONF_INSTANCE_ID: hass.data[DOMAIN][CONF_INSTANCE_ID], CONF_WEBHOOK_ID: hass.data[DOMAIN][CONF_WEBHOOK_ID], CONF_CLOUDHOOK_URL: None})
...
async def smartapp_sync_subscriptions(hass: HomeAssistant, auth_token: str, location_id: str, installed_app_id: str, devices):
    ...
    async def create_subscription(target: str):
        ...
        try:
            ...
            _LOGGER.debug("Created subscription for '%s' under app '%s'", target, installed_app_id)
        except Exception as error:
            _LOGGER.error("Failed to create subscription for '%s' under app '%s': %s", target, installed_app_id, error)

    async def delete_subscription(sub: SubscriptionEntity):
        try:
            await api.delete_subscription(installed_app_id, sub.subscription_id)
            _LOGGER.debug("Removed subscription for '%s' under app '%s' because it was no longer needed", sub.capability, installed_app_id)
        except Exception as error:
            _LOGGER.error("Failed to remove subscription for '%s' under app '%s': %s", sub.capability, installed_app_id, error)
    capabilities = set()
    ...
    if capability_count > SUBSCRIPTION_WARNING_LIMIT:
        _LOGGER.warning("Some device attributes may not receive push updates and there may be subscription creation failures under app '%s' because %s subscriptions are required but there is a limit of %s per app", installed_app_id, capability_count, SUBSCRIPTION_WARNING_LIMIT)
    _LOGGER.debug("Synchronizing subscriptions for %s capabilities under app '%s': %s", capability_count, installed_app_id, capabilities)
...
async def _continue_flow(hass: HomeAssistant, app_id: str, location_id: str, installed_app_id: str, refresh_token: str):
    """Continue a config flow if one is in progress for the specific installed app."""
    unique_id = format_unique_id(app_id, location_id)
    flow = next((flow for flow in hass.config_entries.flow.async_progress_by_handler(DOMAIN) if flow['context'].get('unique_id') == unique_id), None)
    if flow is not None:
        await hass.config_entries.flow.async_configure(flow['flow_id'], {CONF_INSTALLED_APP_ID: installed_app_id, CONF_REFRESH_TOKEN: refresh_token})
        _LOGGER.debug("Continued config flow '%s' for SmartApp '%s' under parent app '%s'", flow['flow_id'], installed_app_id, app_id)

async def smartapp_install(hass: HomeAssistant, req, resp, app):
    """Handle a SmartApp installation and continue the config flow."""
    await _continue_flow(hass, app.app_id, req.location_id, req.installed_app_id, req.refresh_token)
    _LOGGER.debug("Installed SmartApp '%s' under parent app '%s'", req.installed_app_id, app.app_id)
...
async def smartapp_update(hass: HomeAssistant, req, resp, app):
    ...
    entry = next((entry for entry in hass.config_entries.async_entries(DOMAIN) if entry.data.get(CONF_INSTALLED_APP_ID) == req.installed_app_id), None)
    if entry:
        hass.config_entries.async_update_entry(entry, data={**entry.data, CONF_REFRESH_TOKEN: req.refresh_token})
        _LOGGER.debug("Updated config entry '%s' for SmartApp '%s' under parent app '%s'", entry.entry_id, req.installed_app_id, app.app_id)
    await _continue_flow(hass, app.app_id, req.location_id, req.installed_app_id, req.refresh_token)
    _LOGGER.debug("Updated SmartApp '%s' under parent app '%s'", req.installed_app_id, app.app_id)
...
async def smartapp_uninstall(hass: HomeAssistant, req, resp, app):
    ...
    _LOGGER.debug("Uninstalled SmartApp '%s' under parent app '%s'", req.installed_app_id, app.app_id)
