    ...
    def test_get_correct_box_encoding_and_class_prediction_shapes(self):
        ...
        def graph_fn(image_features, proposal_boxes):
            if False:
                while True:
                    i = 10
            rfcn_box_predictor = box_predictor.RfcnKerasBoxPredictor(is_training=False, num_classes=2, conv_hyperparams=self._build_conv_hyperparams(), freeze_batchnorm=False, num_spatial_bins=[3, 3], depth=4, crop_size=[12, 12], box_code_size=4)
            box_predictions = rfcn_box_predictor([image_features], proposal_boxes=proposal_boxes)
