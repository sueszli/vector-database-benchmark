...
from prefect.infrastructure.kubernetes import KubernetesImagePullPolicy, KubernetesJob, KubernetesManifest
...
async def test_task_group_start_returns_job_pid(mock_k8s_batch_client, mock_k8s_client, mock_watch, monkeypatch):
    expected_value = f'{MOCK_CLUSTER_UID}:default:mock-k8s-v1-job'
    async with anyio.create_task_group() as tg:
        result = await tg.start(KubernetesJob(command=['echo', 'hello'], name='test').run)
        assert result == expected_value

async def test_missing_job_returns_bad_status_code(mock_k8s_batch_client, mock_k8s_client, mock_watch, caplog):
    mock_k8s_batch_client.read_namespaced_job.side_effect = ApiException(status=404, reason='Job not found')
    k8s_job = KubernetesJob(command=['echo', 'hello'])
...
class TestKill:
    ...
    async def test_kill_calls_delete_namespaced_job(self, mock_k8s_batch_client, mock_k8s_client, mock_watch, monkeypatch):
        await KubernetesJob(command=['echo', 'hello'], name='test').kill(infrastructure_pid=f'{MOCK_CLUSTER_UID}:default:mock-k8s-v1-job', grace_seconds=0)
        assert len(mock_k8s_batch_client.mock_calls) == 1
        mock_k8s_batch_client.delete_namespaced_job.assert_called_once_with(name='mock-k8s-v1-job', namespace='default', grace_period_seconds=0, propagation_policy='Foreground')

    async def test_kill_uses_foreground_propagation_policy(self, mock_k8s_batch_client, mock_k8s_client, mock_watch, monkeypatch):
        await KubernetesJob(command=['echo', 'hello'], name='test').kill(infrastructure_pid=f'{MOCK_CLUSTER_UID}:default:mock-k8s-v1-job', grace_seconds=0)
        assert len(mock_k8s_batch_client.mock_calls) == 1
        assert len(mock_k8s_batch_client.mock_calls) == 1
        mock_k8s_batch_client.delete_namespaced_job.assert_called_once_with(name='mock-k8s-v1-job', namespace='default', grace_period_seconds=0, propagation_policy='Foreground')

    async def test_kill_uses_correct_grace_seconds(self, mock_k8s_batch_client, mock_k8s_client, mock_watch, monkeypatch):
        GRACE_SECONDS = 42
        await KubernetesJob(command=['echo', 'hello'], name='test').kill(infrastructure_pid=f'{MOCK_CLUSTER_UID}:default:mock-k8s-v1-job', grace_seconds=GRACE_SECONDS)
        assert len(mock_k8s_batch_client.mock_calls) == 1
        mock_k8s_batch_client.delete_namespaced_job.assert_called_once_with(name='mock-k8s-v1-job', namespace='default', grace_period_seconds=GRACE_SECONDS, propagation_policy='Foreground')

    async def test_kill_raises_infra_not_available_on_mismatched_cluster_namespace(self, mock_k8s_batch_client, mock_k8s_client, mock_watch, monkeypatch):
        BAD_NAMESPACE = 'dog'
        with pytest.raises(InfrastructureNotAvailable, match=f"The job is running in namespace {BAD_NAMESPACE!r} but this block is configured to use 'default'"):
            await KubernetesJob(command=['echo', 'hello'], name='test').kill(infrastructure_pid=f'{MOCK_CLUSTER_UID}:{BAD_NAMESPACE}:mock-k8s-v1-job', grace_seconds=0)

    async def test_kill_raises_infra_not_available_on_mismatched_cluster_uid(self, mock_k8s_batch_client, mock_k8s_client, mock_watch, monkeypatch):
        BAD_CLUSTER = '4321'
        with pytest.raises(InfrastructureNotAvailable, match="Unable to kill job 'mock-k8s-v1-job': The job is running on another cluster."):
            await KubernetesJob(command=['echo', 'hello'], name='test').kill(infrastructure_pid=f'{BAD_CLUSTER}:default:mock-k8s-v1-job', grace_seconds=0)

    async def test_kill_raises_infrastructure_not_found_on_404(self, mock_k8s_batch_client, mock_k8s_client, mock_watch, monkeypatch):
        mock_k8s_batch_client.delete_namespaced_job.side_effect = [ApiException(status=404)]
        with pytest.raises(InfrastructureNotFound, match="Unable to kill job 'mock-k8s-v1-job': The job was not found."):
            await KubernetesJob(command=['echo', 'hello'], name='test').kill(infrastructure_pid=f'{MOCK_CLUSTER_UID}:default:mock-k8s-v1-job', grace_seconds=0)

    async def test_kill_passes_other_k8s_api_errors_through(self, mock_k8s_batch_client, mock_k8s_client, mock_watch, monkeypatch):
        mock_k8s_batch_client.delete_namespaced_job.side_effect = [ApiException(status=400)]
        with pytest.raises(ApiException):
            await KubernetesJob(command=['echo', 'hello'], name='test').kill(infrastructure_pid=f'{MOCK_CLUSTER_UID}:default:dog', grace_seconds=0)
...
async def test_sets_environment_variables(mock_k8s_client, mock_watch, mock_k8s_batch_client):
    await KubernetesJob(command=['echo', 'hello'], env={'foo': 'FOO', 'bar': 'BAR'}).run()
    mock_k8s_batch_client.create_namespaced_job.assert_called_once()
    manifest = mock_k8s_batch_client.create_namespaced_job.call_args[0][1]
    pod = manifest['spec']['template']['spec']
    env = pod['containers'][0]['env']
    assert env == [{'name': key, 'value': value} for (key, value) in {**KubernetesJob._base_environment(), 'foo': 'FOO', 'bar': 'BAR'}.items()]

async def test_allows_unsetting_environment_variables(mock_k8s_client, mock_watch, mock_k8s_batch_client):
    assert 'PREFECT_TEST_MODE' in KubernetesJob._base_environment()
    await KubernetesJob(command=['echo', 'hello'], env={'PREFECT_TEST_MODE': None}).run()
    mock_k8s_batch_client.create_namespaced_job.assert_called_once()
    manifest = mock_k8s_batch_client.create_namespaced_job.call_args[0][1]
    pod = manifest['spec']['template']['spec']
    env = pod['containers'][0]['env']
...
async def test_watch_stops_after_backoff_limit_reached(mock_k8s_client, mock_watch, mock_k8s_batch_client):
    mock_k8s_batch_client.read_namespaced_job.return_value.status.completion_time = None
    job_pod = MagicMock(spec=kubernetes.client.V1Pod)
    job_pod.status.phase = 'Running'
...
async def test_watch_handles_deleted_job(mock_k8s_client, mock_watch, mock_k8s_batch_client):
    mock_k8s_batch_client.read_namespaced_job.return_value.status.completion_time = None
...
async def test_watch_handles_pod_without_exit_code(mock_k8s_client, mock_watch, mock_k8s_batch_client):
    mock_k8s_batch_client.read_namespaced_job.return_value.status.completion_time = None
    job_pod = MagicMock(spec=kubernetes.client.V1Pod)
    job_pod.status.phase = 'Running'
    mock_container_status = MagicMock(spec=kubernetes.client.V1ContainerStatus)
