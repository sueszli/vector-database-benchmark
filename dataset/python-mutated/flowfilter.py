...
class FAsset(_Action):
    ...
    ASSET_TYPES = [re.compile(x) for x in [b'text/javascript', b'application/x-javascript', b'application/javascript', b'text/css', b'image/.*', b'font/.*', b'application/font.*']]
...
filter_unary: Sequence[type[_Action]] = [FAsset, FErr, FHTTP, FMarked, FReplay, FReplayClient, FReplayServer, FReq, FResp, FTCP, FUDP, FDNS, FWebSocket, FAll]
filter_rex: Sequence[type[_Rex]] = [FBod, FBodRequest, FBodResponse, FContentType, FContentTypeRequest, FContentTypeResponse, FDomain, FDst, FHead, FHeadRequest, FHeadResponse, FMethod, FSrc, FUrl, FMeta, FMarker, FComment]
filter_int = [FCode]
...
help.sort()
help.extend([('!', 'unary not'), ('&', 'and'), ('|', 'or'), ('(...)', 'grouping')])