...
class _TokenChallenge(Challenge):
    ...
    token: bytes = jose.field('token', encoder=jose.encode_b64jose, decoder=functools.partial(jose.decode_b64jose, size=TOKEN_SIZE, minimum=True))
...
class KeyAuthorizationChallenge(_TokenChallenge, metaclass=abc.ABCMeta):
    ...
    thumbprint_hash_function = KeyAuthorizationChallengeResponse.thumbprint_hash_function
