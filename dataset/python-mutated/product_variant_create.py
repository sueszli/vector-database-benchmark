...
class PreorderSettingsInput(BaseInputObjectType):
    global_threshold = graphene.Int(description='The global threshold for preorder variant.')
...
class ProductVariantInput(BaseInputObjectType):
    attributes = NonNullList(AttributeValueInput, required=False, description='List of attributes specific to this variant.')
    sku = graphene.String(description='Stock keeping unit.')
    name = graphene.String(description='Variant name.', required=False)
    track_inventory = graphene.Boolean(description="Determines if the inventory of this variant should be tracked. If false, the quantity won't change when customers buy this item. If the field is not provided, `Shop.trackInventoryByDefault` will be used.")
    weight = WeightScalar(description='Weight of the Product Variant.', required=False)
    preorder = PreorderSettingsInput(description='Determines if variant is in preorder.' + ADDED_IN_31)
    quantity_limit_per_customer = graphene.Int(required=False, description='Determines maximum quantity of `ProductVariant`,that can be bought in a single checkout.' + ADDED_IN_31)
    metadata = NonNullList(MetadataInput, description='Fields required to update the product variant metadata.' + ADDED_IN_38, required=False)
    private_metadata = NonNullList(MetadataInput, description='Fields required to update the product variant private metadata.' + ADDED_IN_38, required=False)
    external_reference = graphene.String(description='External ID of this product variant.' + ADDED_IN_310, required=False)

    class Meta:
        doc_category = DOC_CATEGORY_PRODUCTS

class ProductVariantCreateInput(ProductVariantInput):
    attributes = NonNullList(AttributeValueInput, required=True, description='List of attributes specific to this variant.')
    product = graphene.ID(description='Product ID of which type is the variant.', name='product', required=True)
    stocks = NonNullList(StockInput, description='Stocks of a product available for sale.', required=False)
...
class ProductVariantCreate(ModelMutation):
    ...
    class Arguments:
        input = ProductVariantCreateInput(required=True, description='Fields required to create a product variant.')
