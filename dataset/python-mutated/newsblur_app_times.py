...
class AppTimes(View):
    ...
    def stats(self):
        if False:
            while True:
                i = 10
        stats = settings.MONGOANALYTICSDB.nbanalytics.page_loads.aggregate([{'$match': {'date': {'$gt': datetime.datetime.now() - datetime.timedelta(minutes=5)}}}, {'$group': {'_id': '$server', 'page_load': {'$avg': '$page_load'}}}])
        return list(stats)