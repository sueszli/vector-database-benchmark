...
async def test_response_content(send_request):
    response = await send_request(request=HttpRequest('GET', '/basic/bytes'))
    assert response.status_code == 200
    assert response.reason == 'OK'
...
async def test_response_text(send_request):
    response = await send_request(request=HttpRequest('GET', '/basic/string'))
    assert response.status_code == 200
    assert response.reason == 'OK'
    content = await response.read()
    assert content == b'Hello, world!'
...
async def test_response_html(send_request):
    response = await send_request(request=HttpRequest('GET', '/basic/html'))
    assert response.status_code == 200
    assert response.reason == 'OK'
    content = await response.read()
...
async def test_raise_for_status(client):
    ...
    with pytest.raises(HttpResponseError):
        response.raise_for_status()
    response = await client.send_request(HttpRequest('GET', '/errors/500'), retry_total=0)
...
async def test_response_no_charset_with_ascii_content(send_request):
    ...
    """
    response = await send_request(request=HttpRequest('GET', '/encoding/no-charset'))
...
async def test_response_no_charset_with_iso_8859_1_content(send_request):
    ...
    about encoding flow
    """
    response = await send_request(request=HttpRequest('GET', '/encoding/iso-8859-1'))
    assert response.text() == 'Accented: �sterreich'
    assert response.encoding is None

@pytest.mark.asyncio
async def test_json(send_request):
    response = await send_request(request=HttpRequest('GET', '/basic/json'))
    assert response.json() == {'greeting': 'hello', 'recipient': 'world'}
    assert response.encoding is None

@pytest.mark.asyncio
async def test_json_with_specified_encoding(send_request):
    response = await send_request(request=HttpRequest('GET', '/encoding/json'))
    assert response.json() == {'greeting': 'hello', 'recipient': 'world'}
    assert response.encoding == 'utf-16'

@pytest.mark.asyncio
async def test_emoji(send_request):
    response = await send_request(request=HttpRequest('GET', '/encoding/emoji'))
    assert response.text() == '👩'

@pytest.mark.asyncio
async def test_emoji_family_with_skin_tone_modifier(send_request):
    response = await send_request(request=HttpRequest('GET', '/encoding/emoji-family-skin-tone-modifier'))
    assert response.text() == '👩🏻\u200d👩🏽\u200d👧🏾\u200d👦🏿 SSN: 859-98-0987'

@pytest.mark.asyncio
async def test_korean_nfc(send_request):
    response = await send_request(request=HttpRequest('GET', '/encoding/korean'))
    assert response.text() == '아가'

@pytest.mark.asyncio
async def test_urlencoded_content(send_request):
    await send_request(request=HttpRequest('POST', '/urlencoded/pet/add/1', data={'pet_type': 'dog', 'pet_food': 'meat', 'name': 'Fido', 'pet_age': 42}))
