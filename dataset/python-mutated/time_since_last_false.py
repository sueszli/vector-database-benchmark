...
class TimeSinceLastFalse(AggregationPrimitive):
    ...
    input_types = [[ColumnSchema(logical_type=Datetime, semantic_tags={'time_index'}), ColumnSchema(logical_type=Boolean)], [ColumnSchema(logical_type=Datetime, semantic_tags={'time_index'}), ColumnSchema(logical_type=BooleanNullable)]]
    ...
    def get_function(self):
        ...
        def time_since_last_false(datetime_col, bool_col, time=None):
            if False:
                i = 10
                return i + 15
            df = pd.DataFrame({'datetime': datetime_col, 'bool': bool_col}).dropna()
            if df.empty:
                return np.nan
