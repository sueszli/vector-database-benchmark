...
class VideoInstanceSegmentationTest(unittest.TestCase):
    ...
    def test_run_modelhub(self):
        ...
        seg_pipeline = pipeline(Tasks.video_instance_segmentation, model=self.model_id, max_video_frames=20)
    ...
    def test_run_modelhub_default_model(self):
        if False:
            print('Hello World!')
        video_path = 'data/test/videos/kitti-step_testing_image_02_0000.mp4'
        seg_pipeline = pipeline(Tasks.video_instance_segmentation, max_video_frames=20)
        result = seg_pipeline(video_path)
        print(f'video instance segmentation output:\n {result}.')
