...
logger = get_logger()
base_search_fields = {'site': fields.String(required=False, description='站点URL'), 'hostname': fields.String(description='主机名'), 'ip': fields.String(description='ip'), 'title': fields.String(description='标题'), 'http_server': fields.String(description='Web servers'), 'headers': fields.String(description='headers'), 'finger.name': fields.String(description='指纹'), 'status': fields.Integer(description='状态码'), 'favicon.hash': fields.Integer(description='favicon hash'), 'task_id': fields.String(description='任务 ID'), 'tag': fields.String(description='标签列表')}
...
add_site_tag_fields = ns.model('AddSiteTagFields', {'tag': fields.String(required=True, description='添加站点标签'), '_id': fields.String(description='站点ID', required=True)})
...
delete_site_tag_fields = ns.model('DeleteSiteTagFields', {'tag': fields.String(required=True, description='删除站点标签'), '_id': fields.String(description='站点ID', required=True)})
...
class DeleteSiteTagARL(ARLResource):
    ...
    def post(self):
        ...
        utils.conn_db('site').update_one(query, {'$set': {'tag': tag_list}})
        return utils.build_ret(ErrorMsg.Success, {'tag': tag})
delete_site_fields = ns.model('deleteSiteFields', {'_id': fields.List(fields.String(required=True, description='站点 _id'))})
