...
async def test_async(provider: type[BaseProvider]):
    ...
    try:
        response = await asyncio.wait_for(ChatCompletion.create_async(model=models.default, messages=messages, provider=provider), 30)
...
async def test_async_list(providers: list[type[BaseProvider]]):
    responses: list = [test_async(_provider) for _provider in providers]
