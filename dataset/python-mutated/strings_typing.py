...
class stringview_model(models.StructModel):
    _members = (('data', types.CPointer(types.char)), ('bytes', size_type), ('length', size_type))
...
class udf_string_model(models.StructModel):
    _members = (('m_data', types.CPointer(types.char)), ('m_bytes', size_type), ('m_size', size_type))
...
id_unary_funcs = ['isalpha', 'isalnum', 'isdecimal', 'isdigit', 'isupper', 'islower', 'isspace', 'isnumeric', 'istitle']
...
for func in string_return_attrs:
    setattr(StringViewAttrs, f'resolve_{func}', create_binary_attr(func, udf_string))
...
for func in id_unary_funcs:
    setattr(StringViewAttrs, f'resolve_{func}', create_identifier_attr(func, types.boolean))
for func in string_unary_funcs:
    setattr(StringViewAttrs, f'resolve_{func}', create_identifier_attr(func, udf_string))
