        ...
        try:
            async with await hook.get_async_conn():
                while not blob_exists:
                    blob_exists = await hook.check_for_blob_async(container_name=self.container_name, blob_name=self.blob_name)
                    if blob_exists:
                        message = f'Blob {self.blob_name} found in container {self.container_name}.'
                        yield TriggerEvent({'status': 'success', 'message': message})
                    else:
                        message = f'Blob {self.blob_name} not available yet in container {self.container_name}. Sleeping for {self.poke_interval} seconds'
        ...
        try:
            async with await hook.get_async_conn():
                while not prefix_exists:
                    prefix_exists = await hook.check_for_prefix_async(container_name=self.container_name, prefix=self.prefix, include=self.include, delimiter=self.delimiter)
                    if prefix_exists:
                        message = f'Prefix {self.prefix} found in container {self.container_name}.'
                        yield TriggerEvent({'status': 'success', 'message': message})
                    else:
                        message = f'Prefix {self.prefix} not available yet in container {self.container_name}. Sleeping for {self.poke_interval} seconds'
                        self.log.info(message)
                        await asyncio.sleep(self.poke_interval)
        except Exception as e:
            yield TriggerEvent({'status': 'error', 'message': str(e)})