...
if __name__ == '__main__':
    ...
    if args.cluster_mode == 'local':
        sc = init_orca_context(cluster_mode='local', cores=args.executor_cores, memory=args.executor_memory)
        ...
        sc = init_orca_context(cluster_mode='yarn', cores=args.executor_cores, num_nodes=args.num_executors, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, object_store_memory='80g')
    ...
    cat_cols = ['user_id', 'article_id', 'net_status', 'exop_position', 'device', 'city', 'age', 'gender', 'cat_1']
    continuous_cols = ['img_num', 'flush_nums']
    feature_max_idx = {'user_id': 40000, 'article_id': 200000, 'net_status': 1004, 'exop_position': 2000, 'device': 2000, 'city': 1379, 'age': 1005, 'gender': 1003, 'cat_1': 1038}
    train_tbl = FeatureTable.read_parquet(args.train_data_path)
    valid_tbl = FeatureTable.read_parquet(args.test_data_path)
    train_multi_task(train_tbl, valid_tbl, args.model_save_path, args.model_type, cat_cols, continuous_cols, feature_max_idx)
    test_multi_task(valid_tbl, args.model_save_path, args.model_type, cat_cols, continuous_cols, feature_max_idx)
    stop_orca_context()