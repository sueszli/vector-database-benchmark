...
class FourSharedCom(SimpleDownloader):
    ...
    __version__ = '0.37'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?4shared(-china)?\\.com/(account/)?(download|get|file|document|photo|video|audio|mp3|office|rar|zip|archive|music)/.+'
    ...
    SIZE_PATTERN = '<span title="Size: (?P<S>[\\d.,]+) (?P<U>[\\w^_]+)">'
    OFFLINE_PATTERN = 'The file link that you requested is not valid\\.|This file was deleted.'
    NAME_REPLACEMENTS = [('&#(\\d+).', lambda m: chr(int(m.group(1))))]
    SIZE_REPLACEMENTS = [(',', '')]
    DIRECT_LINK = False
    LOGIN_ACCOUNT = True
    LINK_FREE_PATTERN = 'name="d3link" value="(.*?)"'
    LINK_BTN_PATTERN = 'id="btnLink" href="(.*?)"'
    ID_PATTERN = 'name="d3fid" value="(.*?)"'
    ...
    def handle_free(self, pyfile):
        ...
        if m is not None:
            ...
            link = re.sub('/(download|get|file|document|photo|video|audio)/', '/get/', pyfile.url)
        ...
        try:
            ...
            id = m.group(1)
            res = self.load(f'http://www.4shared.com/web/d2/getFreeDownloadLimitInfo?fileId={id}')
            self.log_debug(res)
        except Exception:
            pass
        self.wait(20)