...
def upgrade():
    ...
    op.alter_column('pending_oidc_publishers', 'added_by_id', existing_type=postgresql.UUID(), nullable=False)

def downgrade():
    if False:
        return 10
    op.alter_column('pending_oidc_publishers', 'added_by_id', existing_type=postgresql.UUID(), nullable=True)