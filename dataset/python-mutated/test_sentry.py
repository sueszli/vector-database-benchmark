...
import strawberry
if TYPE_CHECKING:
    from strawberry.extensions.tracing.sentry import SentryTracingExtension, SentryTracingExtensionSync
...
@pytest.mark.asyncio
async def test_sentry_tracer(sentry_extension: Tuple[SentryTracingExtension, MagicMock], mocker: MockerFixture):
...
@pytest.mark.asyncio
async def test_uses_operation_name(sentry_extension: Tuple[SentryTracingExtension, MagicMock]):
    (extension, mock) = sentry_extension
    schema = strawberry.Schema(query=Query, mutation=Mutation, extensions=[extension])
...
@pytest.mark.asyncio
async def test_uses_operation_type(sentry_extension: Tuple[SentryTracingExtension, MagicMock]):
    (extension, mock) = sentry_extension
    schema = strawberry.Schema(query=Query, mutation=Mutation, extensions=[extension])
    query = '\n        mutation MyMutation {\n            sayHi\n        }\n    '
    with pytest.warns(DeprecationWarning, match='The Sentry tracing extension is deprecated'):
...
@pytest.mark.asyncio
async def test_uses_operation_subscription(sentry_extension: Tuple[SentryTracingExtension, MagicMock]):
    (extension, mock) = sentry_extension
    schema = strawberry.Schema(query=Query, mutation=Mutation, extensions=[extension])
    query = '\n        subscription MySubscription {\n            onHi\n        }\n    '
    with pytest.warns(DeprecationWarning, match='The Sentry tracing extension is deprecated'):
        await schema.execute(query, operation_name='MySubscription')
    mock.configure_scope().__enter__().span.start_child().set_tag.assert_any_call('graphql.operation_type', 'subscription')
