...
from config import access_token, access_token_secret, bearer_token, consumer_key, consumer_secret, tape, user_id
...
class TweepyAsyncClientTests(IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        if False:
            i = 10
            return i + 15
        self.client = AsyncClient(bearer_token, consumer_key, consumer_secret, access_token or user_id, access_token_secret)
    ...
    @tape.use_cassette('test_asyncclient_manage_and_lookup_direct_messages.yaml')
    async def test_manage_and_lookup_direct_messages(self):
        user_ids = [145336962, 750362064426721281]
        response = await self.client.create_direct_message(participant_id=user_ids[1], text='Testing 1')
        dm_conversation_id = response.data['dm_conversation_id']
        await self.client.create_direct_message(dm_conversation_id=dm_conversation_id, text='Testing 2')
        await self.client.create_direct_message_conversation(text='Testing', participant_ids=user_ids)
    ...
    async def test_manage_and_get_pinned_lists(self):
        ...
        await self.client.update_list(list_id, description='Test List Description')
        await self.client.delete_list(list_id)

    @tape.use_cassette('test_asyncclient_create_and_get_compliance_job_and_jobs.yaml')
