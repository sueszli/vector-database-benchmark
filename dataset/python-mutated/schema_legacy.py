...
from homeassistant.components.vacuum import ATTR_STATUS, ENTITY_ID_FORMAT, VacuumEntity, VacuumEntityFeature
...
SERVICE_TO_STRING = {VacuumEntityFeature.TURN_ON: 'turn_on', VacuumEntityFeature.TURN_OFF: 'turn_off', VacuumEntityFeature.PAUSE: 'pause', VacuumEntityFeature.STOP: 'stop', VacuumEntityFeature.RETURN_HOME: 'return_home', VacuumEntityFeature.FAN_SPEED: 'fan_speed', VacuumEntityFeature.BATTERY: 'battery', VacuumEntityFeature.STATUS: 'status', VacuumEntityFeature.SEND_COMMAND: 'send_command', VacuumEntityFeature.LOCATE: 'locate', VacuumEntityFeature.CLEAN_SPOT: 'clean_spot'}
STRING_TO_SERVICE = {v: k for (k, v) in SERVICE_TO_STRING.items()}
DEFAULT_SERVICES = VacuumEntityFeature.TURN_ON | VacuumEntityFeature.TURN_OFF | VacuumEntityFeature.STOP | VacuumEntityFeature.RETURN_HOME | VacuumEntityFeature.STATUS | VacuumEntityFeature.BATTERY | VacuumEntityFeature.CLEAN_SPOT
...
class MqttVacuum(MqttEntity, VacuumEntity):
    ...
    async def _async_publish_command(self, feature: VacuumEntityFeature) -> None:
        ...
        if self._command_topic is None:
            return
        await self.async_publish(self._command_topic, self._payloads[_COMMANDS[feature]['payload']], qos=self._qos, retain=self._retain, encoding=self._encoding)
    ...
    async def async_set_fan_speed(self, fan_speed: str, **kwargs: Any) -> None:
        ...
        if self._set_fan_speed_topic is None or self.supported_features & VacuumEntityFeature.FAN_SPEED == 0 or fan_speed not in self.fan_speed_list:
            return None
        await self.async_publish(self._set_fan_speed_topic, fan_speed, self._qos, self._retain, self._encoding)
    ...
    async def async_send_command(self, command: str, params: dict[str, Any] | list[Any] | None=None, **kwargs: Any) -> None:
        ...
        await self.async_publish(self._send_command_topic, message_payload, self._qos, self._retain, self._encoding)
