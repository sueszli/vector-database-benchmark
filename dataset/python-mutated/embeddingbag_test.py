...
class EmbeddingBagBenchmark(op_bench.TorchBenchmarkBase):
    ...
    def init(self, embeddingbags, dim, mode, input_size, offset, sparse, include_last_offset, device):
        if False:
            while True:
                i = 10
        self.embedding = torch.nn.EmbeddingBag(num_embeddings=embeddingbags, embedding_dim=dim, mode=mode, include_last_offset=include_last_offset, sparse=sparse).to(device=device)
...
class EmbeddingBenchmark(op_bench.TorchBenchmarkBase):
    ...
    def init(self, num_embeddings, embedding_dim, input_size, device):
        if False:
            return 10
        self.embedding = torch.nn.Embedding(num_embeddings=num_embeddings, embedding_dim=embedding_dim).to(device=device)
        numpy.random.seed((1 << 32) - 1)
