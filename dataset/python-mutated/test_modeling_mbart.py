...
if is_torch_available():
    import torch
    from transformers import AutoTokenizer, BatchEncoding, MBartForCausalLM, MBartForConditionalGeneration, MBartForQuestionAnswering, MBartForSequenceClassification, MBartModel
...
class MBartModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (MBartModel, MBartForConditionalGeneration, MBartForSequenceClassification, MBartForQuestionAnswering) if is_torch_available() else ()
    all_generative_model_classes = (MBartForConditionalGeneration,) if is_torch_available() else ()
    pipeline_model_mapping = {'conversational': MBartForConditionalGeneration, 'feature-extraction': MBartModel, 'fill-mask': MBartForConditionalGeneration, 'question-answering': MBartForQuestionAnswering, 'summarization': MBartForConditionalGeneration, 'text-classification': MBartForSequenceClassification, 'text-generation': MBartForCausalLM, 'text2text-generation': MBartForConditionalGeneration, 'translation': MBartForConditionalGeneration, 'zero-shot': MBartForSequenceClassification} if is_torch_available() else {}
