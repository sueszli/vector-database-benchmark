...
import pandas
import pytest
from dagster import AssetIn, AssetKey, DailyPartitionsDefinition, DynamicPartitionsDefinition, IOManagerDefinition, MetadataValue, MultiPartitionKey, MultiPartitionsDefinition, Out, StaticPartitionsDefinition, TableColumn, TableSchema, TimeWindowPartitionMapping, asset, build_input_context, build_output_context, fs_io_manager, instance_for_test, job, materialize, op
from dagster._core.errors import DagsterInvariantViolationError
from dagster._core.storage.db_io_manager import TableSlice
from dagster_snowflake import build_snowflake_io_manager
from dagster_snowflake.resources import SnowflakeResource
from dagster_snowflake_pandas import SnowflakePandasIOManager, SnowflakePandasTypeHandler, snowflake_pandas_io_manager
from dagster_snowflake_pandas.snowflake_pandas_type_handler import _add_missing_timezone, _convert_string_to_timestamp, _convert_timestamp_to_string
from pandas import DataFrame, Timestamp
resource_config = {'database': 'database_abc', 'account': 'account_abc', 'user': 'user_abc', 'password': 'password_abc', 'warehouse': 'warehouse_abc'}
IS_BUILDKITE = os.getenv('BUILDKITE') is not None
SHARED_BUILDKITE_SNOWFLAKE_CONF: Mapping[str, Any] = {'account': os.getenv('SNOWFLAKE_ACCOUNT', ''), 'user': 'BUILDKITE', 'password': os.getenv('SNOWFLAKE_BUILDKITE_PASSWORD', '')}
DATABASE = 'TEST_SNOWFLAKE_IO_MANAGER'
SCHEMA = 'SNOWFLAKE_IO_MANAGER_SCHEMA'
pythonic_snowflake_io_manager = SnowflakePandasIOManager(database=DATABASE, **SHARED_BUILDKITE_SNOWFLAKE_CONF)
old_snowflake_io_manager = snowflake_pandas_io_manager.configured({**SHARED_BUILDKITE_SNOWFLAKE_CONF, 'database': DATABASE})
