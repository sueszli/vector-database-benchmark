...
class gsuiteaddonsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_deployment': ('parent', 'deployment_id', 'deployment'), 'delete_deployment': ('name', 'etag'), 'get_authorization': ('name',), 'get_deployment': ('name',), 'get_install_status': ('name',), 'install_deployment': ('name',), 'list_deployments': ('parent', 'page_size', 'page_token'), 'replace_deployment': ('deployment',), 'uninstall_deployment': ('name',)}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=gsuiteaddonsCallTransformer()):
    if False:
        return 10
