...
async def test_connection_data_extractor() -> None:
    request = factory.post(path='/a/b/c', headers={'Common': 'abc', 'Special': '123', 'Content-Type': 'application/json; charset=utf-8'}, cookies=[Cookie(key='regular'), Cookie(key='auth')], query_params={'first': ['1', '2', '3'], 'second': ['jeronimo']}, data={'hello': 'world'})
    ...
    assert await extracted_data.get('body') == await request.json()
...
async def test_parse_json_data() -> None:
    ...
    assert await ConnectionDataExtractor(parse_body=True)(request).get('body') == await request.json()
    assert await ConnectionDataExtractor()(request).get('body') == await request.body()
...
async def test_parse_form_data() -> None:
    ...
    assert await ConnectionDataExtractor(parse_body=True)(request).get('body') == dict(await request.form())
...
async def test_response_data_extractor() -> None:
    ...
    extracted_data = extractor(messages)
