...
async def after_exception_handler(exc: Exception, scope: 'Scope') -> None:
    ...
    if not hasattr(state, 'error_count'):
        ...
        state.error_count += 1
    logger.info('an exception of type %s has occurred for requested path %s and the application error count is %d.', type(exc).__name__, scope['path'], state.error_count)
