...
from reactpy.testing import BackendFixture, DisplayFixture, assert_reactpy_did_log, assert_reactpy_did_not_log, poll
...
async def test_that_js_module_unmount_is_called(display: DisplayFixture):
    SomeComponent = reactpy.web.export(reactpy.web.module_from_file('set-flag-when-unmount-is-called', JS_FIXTURES_DIR / 'set-flag-when-unmount-is-called.js'), 'SomeComponent')
    ...
    set_current_component.current(reactpy.html.h1({'id': 'some-other-component'}, 'some other component'))
...
async def test_module_from_url(browser):
    ...
    app.static('/simple-button.js', str(JS_FIXTURES_DIR / 'simple-button.js'), content_type='text/javascript')
    SimpleButton = reactpy.web.export(reactpy.web.module_from_url('/simple-button.js', resolve_exports=False), 'SimpleButton')
...
async def test_module_from_file(display: DisplayFixture):
    SimpleButton = reactpy.web.export(reactpy.web.module_from_file('simple-button', JS_FIXTURES_DIR / 'simple-button.js'), 'SimpleButton')
...
async def test_module_exports_multiple_components(display: DisplayFixture):
    (Header1, Header2) = reactpy.web.export(reactpy.web.module_from_file('exports-two-components', JS_FIXTURES_DIR / 'exports-two-components.js'), ['Header1', 'Header2'])
...
async def test_imported_components_can_render_children(display: DisplayFixture):
    module = reactpy.web.module_from_file('component-can-have-child', JS_FIXTURES_DIR / 'component-can-have-child.js')
    (Parent, Child) = reactpy.web.export(module, ['Parent', 'Child'])
    await display.show(lambda : Parent(Child({'index': 1}), Child({'index': 2}), Child({'index': 3})))
