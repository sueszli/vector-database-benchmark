...
from ray.dashboard.modules.job.common import JobDeleteResponse, JobSubmitRequest, JobSubmitResponse, JobStopResponse, JobLogsResponse
...
class JobAgent(dashboard_utils.DashboardAgentModule):
    ...
    async def submit_job(self, req: Request) -> Response:
        ...
        try:
            ...
            submission_id = await self.get_job_manager().submit_job(entrypoint=submit_request.entrypoint, submission_id=request_submission_id, runtime_env=submit_request.runtime_env, metadata=submit_request.metadata, entrypoint_num_cpus=submit_request.entrypoint_num_cpus, entrypoint_num_gpus=submit_request.entrypoint_num_gpus, entrypoint_memory=submit_request.entrypoint_memory, entrypoint_resources=submit_request.entrypoint_resources)
    ...
    async def stop_job(self, req: Request) -> Response:
        ...
        job = await find_job_by_ids(self._dashboard_agent.gcs_aio_client, self.get_job_manager().job_info_client(), job_or_submission_id)
        if not job:
    ...
    async def get_job_logs(self, req: Request) -> Response:
        ...
        resp = JobLogsResponse(logs=self.get_job_manager().get_job_logs(job.submission_id))
        return Response(text=json.dumps(dataclasses.asdict(resp)), content_type='application/json')
