...
class MenuItemTranslate(BaseTranslateMutation):
    ...
    class Arguments:
        id = graphene.ID(required=True, description='MenuItem ID or MenuItemTranslatableContent ID.')
        language_code = graphene.Argument(LanguageCodeEnum, required=True, description='Translation language code.')
        input = NameTranslationInput(required=True, description='Fields required to update menu item translations.')
