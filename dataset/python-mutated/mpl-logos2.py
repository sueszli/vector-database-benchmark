...
def create_text_axes(fig, height_px):
    ...
    path2 = TextPath((310, -13), f'Version {matplotlib.__version__}', size=height_px * 0.12, prop=fp)
    angle = 4.25
    trans = mtrans.Affine2D().skew_deg(angle, 0)
    patch = PathPatch(path, transform=trans + ax.transData, color=MPL_BLUE, lw=0)
    patch1 = PathPatch(path1, transform=trans + ax.transData, color=MPL_BLUE, lw=0)
