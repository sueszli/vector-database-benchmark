...
parser.add_argument('--run', default='PPO', choices=['DQN', 'PPO'], help='The RLlib-registered algorithm to use.')
...
parser.add_argument('--env', type=str, default='3DBall', choices=['3DBall', '3DBallHard', 'FoodCollector', 'GridFoodCollector', 'Pyramids', 'SoccerStrikersVsGoalie', 'Sorter', 'Tennis', 'VisualHallway', 'Walker'], help='The name of the Env to run in the Unity3D editor (feel free to add more to this script!)')
parser.add_argument('--port', type=int, default=SERVER_PORT, help="The Policy server's port to listen on for ExternalEnv client conections.")
parser.add_argument('--checkpoint-freq', type=int, default=10, help='The frequency with which to create checkpoint files of the learnt Policies.')
parser.add_argument('--no-restore', action='store_true', help='Whether to load the Policy weights from a previous checkpoint')
if __name__ == '__main__':
    ...
    config = get_trainable_cls(args.run).get_default_config().framework(args.framework).rollouts(num_rollout_workers=args.num_workers, rollout_fragment_length=20, enable_connectors=False).environment(env=None, observation_space=gym.spaces.Box(float('-inf'), float('inf'), (8,)), action_space=gym.spaces.Box(-1.0, 1.0, (2,))).training(train_batch_size=256).multi_agent(policies=policies, policy_mapping_fn=policy_mapping_fn).offline_data(input_=_input).evaluation(off_policy_estimation_methods={})
    algo = config.build()
