        ...
        def check_node(node):
            ...
            if isinstance(node, ast.Constant):
                ...
                if const != GLOBAL_CONST or name not in self.pure_functions:
                    return NOT_CONST
                return min(check_nodes(node.args), check_nodes((i.value for i in node.keywords)))
            elif isinstance(node, ast.FormattedValue):
                return min(check_node(node.value), check_node(node.format_spec))
            elif isinstance(node, ast.JoinedStr):
                return check_nodes(node.values)
            elif isinstance(node, (ast.Attribute, ast.Name)):
                return check_name(node)[0]
            elif isinstance(node, ast.Subscript):
                return min(check_node(node.value), check_node(node.slice))
            elif isinstance(node, ast.Starred):
                return check_node(node.value)
            elif isinstance(node, (ast.List, ast.Tuple)):
                return check_nodes(node.elts)
            elif isinstance(node, ast.Slice):
                return min(check_node(node.lower), check_node(node.upper), check_node(node.step))
            return NOT_CONST
