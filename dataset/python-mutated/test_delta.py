...
def test_deltaproduct_add_kd_kd():
    ...
    assert dp(KD(i, k) + KD(j, k), (k, 1, l)) == KD(l, 0) + KD(i, 1) * KD(l, 1) + KD(j, 1) * KD(l, 1) + KD(i, 1) * KD(j, 2) * KD(l, 2) + KD(j, 1) * KD(i, 2) * KD(l, 2)
    assert dp(KD(i, k) + KD(j, k), (k, l, 3)) == KD(l, 4) + KD(i, 3) * KD(l, 3) + KD(j, 3) * KD(l, 3) + KD(i, 2) * KD(j, 3) * KD(l, 2) + KD(i, 3) * KD(j, 2) * KD(l, 2)
    assert dp(KD(i, k) + KD(j, k), (k, l, m)) == KD(l, m + 1) + KD(i, m) * KD(l, m) + KD(j, m) * KD(l, m) + KD(i, m) * KD(j, m - 1) * KD(l, m - 1) + KD(i, m - 1) * KD(j, m) * KD(l, m - 1)
...
def test_deltaproduct_mul_x_add_y_kd():
    if False:
        return 10
    assert dp(x * (y + KD(i, j)), (j, 1, 3)) == (x * y) ** 3 + x * (x * y) ** 2 * KD(i, 1) + x * y * x * (x * y) * KD(i, 2) + (x * y) ** 2 * x * KD(i, 3)
    assert dp(x * (y + KD(i, j)), (j, 1, 1)) == x * (y + KD(i, 1))
    assert dp(x * (y + KD(i, j)), (j, 2, 2)) == x * (y + KD(i, 2))
    assert dp(x * (y + KD(i, j)), (j, 3, 3)) == x * (y + KD(i, 3))
    assert dp(x * (y + KD(i, j)), (j, 1, k)) == (x * y) ** k + Piecewise(((x * y) ** (i - 1) * x * (x * y) ** (k - i), And(1 <= i, i <= k)), (0, True)).expand()
    assert dp(x * (y + KD(i, j)), (j, k, 3)) == ((x * y) ** (-k + 4) + Piecewise(((x * y) ** (i - k) * x * (x * y) ** (3 - i), And(k <= i, i <= 3)), (0, True))).expand()
    assert dp(x * (y + KD(i, j)), (j, k, l)) == ((x * y) ** (-k + l + 1) + Piecewise(((x * y) ** (i - k) * x * (x * y) ** (l - i), And(k <= i, i <= l)), (0, True))).expand()
...
def test_deltasummation_add_mul_x_kd_kd():
    if False:
        while True:
            i = 10
    assert ds(x * KD(i, k) + KD(j, k), (k, 1, 3)) == piecewise_fold(Piecewise((x, And(1 <= i, i <= 3)), (0, True)) + Piecewise((1, And(1 <= j, j <= 3)), (0, True)))
    assert ds(x * KD(i, k) + KD(j, k), (k, 1, 1)) == piecewise_fold(Piecewise((x, Eq(i, 1)), (0, True)) + Piecewise((1, Eq(j, 1)), (0, True)))
    assert ds(x * KD(i, k) + KD(j, k), (k, 2, 2)) == piecewise_fold(Piecewise((x, Eq(i, 2)), (0, True)) + Piecewise((1, Eq(j, 2)), (0, True)))
    assert ds(x * KD(i, k) + KD(j, k), (k, 3, 3)) == piecewise_fold(Piecewise((x, Eq(i, 3)), (0, True)) + Piecewise((1, Eq(j, 3)), (0, True)))
    assert ds(x * KD(i, k) + KD(j, k), (k, 1, l)) == piecewise_fold(Piecewise((x, And(1 <= i, i <= l)), (0, True)) + Piecewise((1, And(1 <= j, j <= l)), (0, True)))
    assert ds(x * KD(i, k) + KD(j, k), (k, l, 3)) == piecewise_fold(Piecewise((x, And(l <= i, i <= 3)), (0, True)) + Piecewise((1, And(l <= j, j <= 3)), (0, True)))
    assert ds(x * KD(i, k) + KD(j, k), (k, l, m)) == piecewise_fold(Piecewise((x, And(l <= i, i <= m)), (0, True)) + Piecewise((1, And(l <= j, j <= m)), (0, True)))
...
def test_deltasummation_mul_add_x_y_add_kd_kd():
    if False:
        i = 10
        return i + 15
    assert ds((x + y) * (KD(i, k) + KD(j, k)), (k, 1, 3)) == piecewise_fold(Piecewise((x + y, And(1 <= i, i <= 3)), (0, True)) + Piecewise((x + y, And(1 <= j, j <= 3)), (0, True)))
    assert ds((x + y) * (KD(i, k) + KD(j, k)), (k, 1, 1)) == piecewise_fold(Piecewise((x + y, Eq(i, 1)), (0, True)) + Piecewise((x + y, Eq(j, 1)), (0, True)))
    assert ds((x + y) * (KD(i, k) + KD(j, k)), (k, 2, 2)) == piecewise_fold(Piecewise((x + y, Eq(i, 2)), (0, True)) + Piecewise((x + y, Eq(j, 2)), (0, True)))
    assert ds((x + y) * (KD(i, k) + KD(j, k)), (k, 3, 3)) == piecewise_fold(Piecewise((x + y, Eq(i, 3)), (0, True)) + Piecewise((x + y, Eq(j, 3)), (0, True)))
    assert ds((x + y) * (KD(i, k) + KD(j, k)), (k, 1, l)) == piecewise_fold(Piecewise((x + y, And(1 <= i, i <= l)), (0, True)) + Piecewise((x + y, And(1 <= j, j <= l)), (0, True)))
    assert ds((x + y) * (KD(i, k) + KD(j, k)), (k, l, 3)) == piecewise_fold(Piecewise((x + y, And(l <= i, i <= 3)), (0, True)) + Piecewise((x + y, And(l <= j, j <= 3)), (0, True)))
    assert ds((x + y) * (KD(i, k) + KD(j, k)), (k, l, m)) == piecewise_fold(Piecewise((x + y, And(l <= i, i <= m)), (0, True)) + Piecewise((x + y, And(l <= j, j <= m)), (0, True)))
...
def test_deltasummation_mul_x_add_y_kd():
    ...
    assert ds(x * (y + KD(i, j)), (j, k, 3)) == Piecewise(((4 - k) * x * y + x, And(k <= i, i <= 3)), ((4 - k) * x * y, True))
    assert ds(x * (y + KD(i, j)), (j, k, l)) == Piecewise(((l - k + 1) * x * y + x, And(k <= i, i <= l)), ((l - k + 1) * x * y, True))

def test_deltasummation_mul_x_add_y_twokd():
    if False:
        while True:
            i = 10
    assert ds(x * (y + 2 * KD(i, j)), (j, 1, 3)) == Piecewise((3 * x * y + 2 * x, And(1 <= i, i <= 3)), (3 * x * y, True))
    ...
    assert ds(x * (y + 2 * KD(i, j)), (j, 1, k)) == Piecewise((k * x * y + 2 * x, And(1 <= i, i <= k)), (k * x * y, True))
    assert ds(x * (y + 2 * KD(i, j)), (j, k, 3)) == Piecewise(((4 - k) * x * y + 2 * x, And(k <= i, i <= 3)), ((4 - k) * x * y, True))
    assert ds(x * (y + 2 * KD(i, j)), (j, k, l)) == Piecewise(((l - k + 1) * x * y + 2 * x, And(k <= i, i <= l)), ((l - k + 1) * x * y, True))
