...
class TestPresenter(BasePresenter):
    default_attributes = ['id', 'username']
...
class TestPolicy(BasePolicy):
    pass
TestPolicy.allow_read(TestPresenter.default_attributes + ['user'], scopes=[OauthScope.CLIENT_ALL, OauthScope.CLIENT_PRIVATE, OauthScope.CLIENT_PUBLIC])
...
class BasePresenterTest(BaseApiTestCase, BootstrapMixin):
    ...
    async def test_present_resource_database_resource(self):
        ...
        result = await TestPresenter.present_resource([resource, resource], self.user)
        self.assertEqual(result, [dict(id=resource.id, username=resource.username), dict(id=resource.id, username=resource.username)])
        self.cleanup()
    ...
    async def test_present_models(self):
        ...
        self.assertEqual(result, [dict(id=model.id, username=model.username) for model in self.users])
        self.cleanup()
    ...
    async def test_present_with_resource_as_attribute(self):
        ...
        self.assertEqual(result, dict(id=resource.id, user=dict(id=user.id, username=user.username), username=resource.username))
    ...
    async def test_present_failed_policy_authorizations(self):
        ...
        Override4TestPresenter.register_format('with_user', Override4TestPresenter.default_attributes + ['user'])
        ...
        Override4TestPolicy.allow_read(['username'], scopes=[OauthScope.CLIENT_ALL, OauthScope.CLIENT_PRIVATE, OauthScope.CLIENT_PUBLIC])
        result = await presenter.present(format='with_user')
        self.assertEqual(result, dict(id=resource.id, user=dict(username=user.username)))