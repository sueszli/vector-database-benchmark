...
from ..const import CLUSTER_HANDLER_ELECTRICAL_MEASUREMENT, REPORT_CONFIG_DEFAULT, REPORT_CONFIG_OP, SIGNAL_ATTR_UPDATED
from . import AttrReportConfig, ClusterHandler

@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(homeautomation.ApplianceEventAlerts.cluster_id)
class ApplianceEventAlerts(ClusterHandler):
    """Appliance Event Alerts cluster handler."""

@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(homeautomation.ApplianceIdentification.cluster_id)
class ApplianceIdentification(ClusterHandler):
    """Appliance Identification cluster handler."""

@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(homeautomation.ApplianceStatistics.cluster_id)
class ApplianceStatistics(ClusterHandler):
    """Appliance Statistics cluster handler."""

@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(homeautomation.Diagnostic.cluster_id)
class Diagnostic(ClusterHandler):
    """Diagnostic cluster handler."""

@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(homeautomation.ElectricalMeasurement.cluster_id)
class ElectricalMeasurementClusterHandler(ClusterHandler):
    ...
    REPORT_CONFIG = (AttrReportConfig(attr='active_power', config=REPORT_CONFIG_OP), AttrReportConfig(attr='active_power_max', config=REPORT_CONFIG_DEFAULT), AttrReportConfig(attr='apparent_power', config=REPORT_CONFIG_OP), AttrReportConfig(attr='rms_current', config=REPORT_CONFIG_OP), AttrReportConfig(attr='rms_current_max', config=REPORT_CONFIG_DEFAULT), AttrReportConfig(attr='rms_voltage', config=REPORT_CONFIG_OP), AttrReportConfig(attr='rms_voltage_max', config=REPORT_CONFIG_DEFAULT), AttrReportConfig(attr='ac_frequency', config=REPORT_CONFIG_OP), AttrReportConfig(attr='ac_frequency_max', config=REPORT_CONFIG_DEFAULT))
    ZCL_INIT_ATTRS = {'ac_current_divisor': True, 'ac_current_multiplier': True, 'ac_power_divisor': True, 'ac_power_multiplier': True, 'ac_voltage_divisor': True, 'ac_voltage_multiplier': True, 'ac_frequency_divisor': True, 'ac_frequency_multiplier': True, 'measurement_type': True, 'power_divisor': True, 'power_multiplier': True, 'power_factor': True}

    async def async_update(self):
        """Retrieve latest state."""
        self.debug('async_update')
        attrs = [a['attr'] for a in self.REPORT_CONFIG if a['attr'] not in self.cluster.unsupported_attributes]
        result = await self.get_attributes(attrs, from_cache=False, only_cache=False)
        if result:
            for (attr, value) in result.items():
                self.async_send_signal(f'{self.unique_id}_{SIGNAL_ATTR_UPDATED}', self.cluster.find_attribute(attr).id, attr, value)
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(homeautomation.MeterIdentification.cluster_id)
