...
version_error_messages = ["Cannot satisfy 'fftw@:1.0' and 'fftw@1.1:", '        required because quantum-espresso depends on fftw@:1.0', '          required because quantum-espresso ^fftw@1.1: requested from CLI', '        required because quantum-espresso ^fftw@1.1: requested from CLI']
external_error_messages = ['Attempted to build package quantum-espresso which is not buildable and does not have a satisfying external', "        'quantum-espresso~veritas' is an external constraint for quantum-espresso which was not satisfied", "        'quantum-espresso+veritas' required", '        required because quantum-espresso+veritas requested from CLI']
variant_error_messages = ["'fftw' required multiple values for single-valued variant 'mpi'", "    Requested '~mpi' and '+mpi'", '        required because quantum-espresso depends on fftw+mpi when +invino', '          required because quantum-espresso+invino ^fftw~mpi requested from CLI', '        required because quantum-espresso+invino ^fftw~mpi requested from CLI']
external_config = {'packages:quantum-espresso': {'buildable': False, 'externals': [{'spec': 'quantum-espresso@1.0~veritas', 'prefix': '/path/to/qe'}]}}
