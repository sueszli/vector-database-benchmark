...
if is_torch_available():
    import torch
    from transformers import BigBirdPegasusForCausalLM, BigBirdPegasusForConditionalGeneration, BigBirdPegasusForQuestionAnswering, BigBirdPegasusForSequenceClassification, BigBirdPegasusModel, PegasusTokenizer
    from transformers.models.bigbird_pegasus.modeling_bigbird_pegasus import BigBirdPegasusDecoder, BigBirdPegasusEncoder
MODEL_ID = 'google/bigbird-pegasus-large-pubmed'

def prepare_bigbird_pegasus_inputs_dict(config, input_ids, decoder_input_ids, attention_mask=None, decoder_attention_mask=None):
    if False:
        while True:
            i = 10
    ...
    input_dict = {'input_ids': input_ids, 'decoder_input_ids': decoder_input_ids, 'attention_mask': attention_mask, 'decoder_attention_mask': attention_mask}
    input_dict = {k: input_dict[k].to(torch_device) for k in input_dict}
    return input_dict

class BigBirdPegasusModelTester:

    def __init__(self, parent, batch_size=7, seq_length=256, is_training=True, use_labels=False, vocab_size=99, hidden_size=32, num_hidden_layers=2, num_attention_heads=4, intermediate_size=31, hidden_act='gelu_fast', hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, max_position_embeddings=260, eos_token_id=1, pad_token_id=0, bos_token_id=2, attention_type='block_sparse', use_bias=False, block_size=16, num_random_blocks=3, scale_embedding=True):
        if False:
            for i in range(10):
                print('nop')
        self.parent = parent
...
class BigBirdPegasusModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (BigBirdPegasusModel, BigBirdPegasusForConditionalGeneration, BigBirdPegasusForSequenceClassification, BigBirdPegasusForQuestionAnswering) if is_torch_available() else ()
    all_generative_model_classes = (BigBirdPegasusForConditionalGeneration,) if is_torch_available() else ()
    pipeline_model_mapping = {'conversational': BigBirdPegasusForConditionalGeneration, 'feature-extraction': BigBirdPegasusModel, 'question-answering': BigBirdPegasusForQuestionAnswering, 'summarization': BigBirdPegasusForConditionalGeneration, 'text-classification': BigBirdPegasusForSequenceClassification, 'text-generation': BigBirdPegasusForCausalLM, 'text2text-generation': BigBirdPegasusForConditionalGeneration, 'translation': BigBirdPegasusForConditionalGeneration, 'zero-shot': BigBirdPegasusForSequenceClassification} if is_torch_available() else {}
    ...
    def test_batched_forward_block_sparse(self):
        if False:
            for i in range(10):
                print('nop')
        self._check_batched_forward(attn_type='block_sparse', tolerance=0.1)

    def _check_batched_forward(self, attn_type, tolerance=0.001):
        if False:
            for i in range(10):
                print('nop')
        ...
        attention_mask = torch.tensor([[1] * 3 * chunk_length + [0] * chunk_length, [1] * 4 * chunk_length], device=torch_device, dtype=torch.long)
    ...
    def test_auto_padding(self):
        ...
        self.assertTrue(torch.allclose(output1, output2, atol=1e-05))
    ...
    def test_for_change_to_full_attn(self):
        ...
        self.assertTrue(torch.allclose(outputs1, outputs2, atol=1e-05))
...
class BigBirdPegasusModelIntegrationTests(unittest.TestCase):
    ...
    def _get_dummy_target_ids(self):
        if False:
            while True:
                i = 10
        ids = torch.tensor([[13, 6, 1, 4, 12, 4, 8, 10, 4, 6, 3, 5, 8, 7, 9, 9]], dtype=torch.long, device=torch_device)
        return ids
    ...
    def test_inference_block_sparse(self):
        ...
        self.assertTrue(torch.allclose(prediction_logits[0, 4:8, 128:156], expected_prediction_logits_slice, atol=0.0001))
    ...
    def test_seq_to_seq_generation(self):
        ...
        generated = tokenizer.batch_decode(hypotheses_batch.tolist(), clean_up_tokenization_spaces=True, skip_special_tokens=True)
        self.assertTrue(generated == [EXPECTED_LEP, EXPECTED_MAGNET])

class BigBirdPegasusStandaloneDecoderModelTester:

    def __init__(self, parent, vocab_size=99, batch_size=7, d_model=32, decoder_seq_length=7, is_training=True, is_decoder=True, use_attention_mask=True, use_cache=False, use_labels=True, decoder_start_token_id=2, decoder_ffn_dim=32, decoder_layers=2, encoder_attention_heads=4, decoder_attention_heads=4, max_position_embeddings=30, is_encoder_decoder=False, pad_token_id=0, bos_token_id=1, eos_token_id=2, scope=None, attention_type='original_full', use_bias=True, block_size=16, num_random_blocks=3):
        if False:
            print('Hello World!')
        self.parent = parent
        self.batch_size = batch_size
    ...
    def create_and_check_decoder_model_attention_mask_past(self, config, input_ids, attention_mask, lm_labels):
        ...
        output_from_no_past_slice = output_from_no_past[:, next_input_ids.shape[-1] - 1, random_slice_idx].detach()
        output_from_past_slice = output_from_past[:, 0, random_slice_idx].detach()
        assert torch.allclose(output_from_past_slice, output_from_no_past_slice, atol=0.5)
