...
from pygments.token import Comment, Error, Generic, Keyword, Name, Number, Operator, Punctuation, String, Text, Token, Whitespace, _TokenType
from xonsh.built_ins import XSH
from xonsh.color_tools import BASE_XONSH_COLORS, RE_BACKGROUND, RE_XONSH_COLOR, find_closest_color, iscolor, make_palette, warn_deprecated_no_color
...
from xonsh.platform import os_environ, ptk_version_info, pygments_version_info, win_ansi_support
...
from xonsh.tools import ANSICOLOR_NAMES_MAP, FORMATTER, ON_WINDOWS, PTK_NEW_OLD_COLOR_MAP, hardcode_colors_for_win10, intensify_colors_for_cmd_exe
Color = Token.Color
NON_PYGMENTS_RULES: dict[str, dict[str, str]] = {}
PTK_SPECIFIC_VALUES = frozenset({'reverse', 'noreverse', 'hidden', 'nohidden', 'blink', 'noblink'})
FALLBACK_STYLE_DICT = LazyObject(lambda : {token: value for (token, value) in DEFAULT_STYLE_DICT.items() if str(token).startswith('Token.PTK')}, globals(), 'FALLBACK_STYLE_DICT')
...
color_file_extension_RE = LazyObject(lambda : re.compile('.*(\\.\\w+)$'), globals(), 'color_file_extension_RE')
...
class XonshLexer(Python3Lexer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
    tokens = {'mode_switch_brackets': [('(\\$)(\\{)', bygroups(Keyword, Punctuation), 'py_curly_bracket'), ('(@)(\\()', bygroups(Keyword, Punctuation), 'py_bracket'), ('([\\!\\$])(\\()', bygroups(Keyword, Punctuation), ('subproc_bracket', 'subproc_start')), ('(@\\$)(\\()', bygroups(Keyword, Punctuation), ('subproc_bracket', 'subproc_start')), ('([\\!\\$])(\\[)', bygroups(Keyword, Punctuation), ('subproc_square_bracket', 'subproc_start')), ('(g?)(`)', bygroups(String.Affix, String.Backtick), 'backtick_re')], 'subproc_bracket': [('\\)', Punctuation, '#pop'), include('subproc')], 'subproc_square_bracket': [('\\]', Punctuation, '#pop'), include('subproc')], 'py_bracket': [('\\)', Punctuation, '#pop'), include('root')], 'py_curly_bracket': [('\\}', Punctuation, '#pop'), include('root')], 'backtick_re': [('[\\.\\^\\$\\*\\+\\?\\[\\]\\|]', String.Regex), ('({[0-9]+}|{[0-9]+,[0-9]+})\\??', String.Regex), ('\\\\([0-9]+|[AbBdDsSwWZabfnrtuUvx\\\\])', String.Escape), ('`', String.Backtick, '#pop'), ('[^`\\.\\^\\$\\*\\+\\?\\[\\]\\|]+', String.Backtick)], 'root': [('\\?', Keyword), ('(?<=\\w)!', Keyword), ('\\$\\w+', Name.Variable), ('\\(', Punctuation, 'py_bracket'), ('\\{', Punctuation, 'py_curly_bracket'), include('mode_switch_brackets'), inherit], 'subproc_start': [('\\s+', Whitespace), (COMMAND_TOKEN_RE, subproc_cmd_callback, '#pop'), ('', Whitespace, '#pop')], 'subproc': [include('mode_switch_brackets'), ('&&|\\|\\||\\band\\b|\\bor\\b', Operator, 'subproc_start'), ('"(\\\\\\\\|\\\\[0-7]+|\\\\.|[^"\\\\])*"', String.Double), ("'(\\\\\\\\|\\\\[0-7]+|\\\\.|[^'\\\\])*'", String.Single), ('(?<=\\w|\\s)!', Keyword, 'subproc_macro'), ('^!', Keyword, 'subproc_macro'), (';', Punctuation, 'subproc_start'), ('&|=', Punctuation), ('\\|', Punctuation, 'subproc_start'), ('\\s+', Text), ('[^=\\s\\[\\]{}()$"\\\'`<&|;]+', subproc_arg_callback), ('<', Text), ('\\$\\w+', Name.Variable)], 'subproc_macro': [('(\\s*)([^\\n]+)', bygroups(Whitespace, String)), ('', Whitespace, '#pop')]}
