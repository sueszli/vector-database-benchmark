...
from sentry.db.models import BaseManager, FlexibleForeignKey, Model, region_silo_only_model, sane_repr
...
class RuleSnoozeManager(BaseManager['RuleSnooze']):
    ...
    def is_snoozed_for_all(self, rule=None, alert_rule=None):
        ...
        return RuleSnooze.objects.filter(user_id__isnull=True, rule=rule, alert_rule=alert_rule).exists()
...
class RuleSnooze(Model):
    ...
    class Meta:
        ...
        constraints = [CheckConstraint(check=Q(rule__isnull=False, alert_rule__isnull=True) | Q(rule__isnull=True, alert_rule__isnull=False), name='rule_or_alert_rule'), UniqueConstraint(fields=['rule'], condition=Q(user_id__isnull=True), name='unique_rule_user'), UniqueConstraint(fields=['alert_rule'], condition=Q(user_id__isnull=True), name='unique_alert_rule_user')]
