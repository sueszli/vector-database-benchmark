import unittest
from transformers import MODEL_FOR_SEQ_TO_SEQ_CAUSAL_LM_MAPPING, TF_MODEL_FOR_SEQ_TO_SEQ_CAUSAL_LM_MAPPING, SummarizationPipeline, TFPreTrainedModel, pipeline
...
class SummarizationPipelineTests(unittest.TestCase):
    ...
    def run_pipeline_test(self, summarizer, _):
        ...
        outputs = summarizer('(CNN)The Palestinian Authority officially became ', num_beams=2, min_length=2, max_length=5)
        self.assertEqual(outputs, [{'summary_text': ANY(str)}])
        model_can_handle_longer_seq = ['SwitchTransformersConfig', 'T5Config', 'LongT5Config', 'LEDConfig', 'PegasusXConfig', 'FSMTConfig', 'M2M100Config', 'ProphetNetConfig']
        if model.config.__class__.__name__ not in model_can_handle_longer_seq:
            if not (isinstance(model, TFPreTrainedModel) and get_gpu_count() > 0 and (len(summarizer.model.trainable_weights) > 0)):
    ...
    def test_small_model_pt(self):
        ...
        self.assertEqual(outputs, [{'summary_text': 'เข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไปเข้าไป'}])
