...
if __name__ == '__main__':
    ...
    if sentry_url is None:
        logger.critical('Sentry url is not defined. To define sentry url use:EXPORT SENTRY_URL=<sentry_url>\nIf you want to disable sentry, then define the following:EXPORT SENTRY_URL=')
        exit(1)
    version_py = Path(arguments.repo) / 'src/tribler/core/version.py'
    logger.info(f'Write info to: {version_py}')
    version_py.write_text(f'version_id = "{ref_name}"\nbuild_date = "{build_time}"\ncommit_id = "{commit}"\nsentry_url = "{sentry_url}"\n')