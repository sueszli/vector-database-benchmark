...
if __name__ == '__main__':
    batch_size = 1
    epoch_size = int(1000.0) * batch_size
    buffer_size = int(100.0)
    gamma = 0.99
    lr = 0.0003
    ...
    runner.train(engine=dl.CPUEngine(), model=model, optimizer=optimizer, loaders=loaders, logdir='./logs_dqn', num_epochs=10, verbose=True, valid_loader='_epoch_', valid_metric='v_reward', minimize_valid_metric=False, load_best_on_end=True, callbacks=[GameCallback(env=env, rollout_buffer=rollout_buffer)])
