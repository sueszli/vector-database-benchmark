...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    async_add_entities([WyomingWakeWordProvider(hass, config_entry, service)])
...
class WyomingWakeWordProvider(wake_word.WakeWordDetectionEntity):
    ...
    async def get_supported_wake_words(self) -> list[wake_word.WakeWord]:
        ...
        info = await load_wyoming_info(self.service.host, self.service.port, retries=0, timeout=1)
        if info is not None:
            wake_service = info.wake[0]
            self._supported_wake_words = [wake_word.WakeWord(id=ww.name, name=ww.description or ww.name) for ww in wake_service.models]
        return self._supported_wake_words

    async def _async_process_audio_stream(self, stream: AsyncIterable[tuple[bytes, int]], wake_word_id: str | None) -> wake_word.DetectionResult | None:
        ...
        try:
            async with AsyncTcpClient(self.service.host, self.service.port) as client:
                await client.write_event(Detect(names=[wake_word_id] if wake_word_id else None).event())
                ...
                try:
                    while True:
                        ...
                        if wake_task in done:
                            ...
                            if Detection.is_type(event.type):
                                ...
                                if wake_word_id and detection.name != wake_word_id:
                                    _LOGGER.warning('Expected wake word %s but got %s, skipping', wake_word_id, detection.name)
                                    wake_task = asyncio.create_task(client.read_event())
                                ...
                                return wake_word.DetectionResult(wake_word_id=detection.name, timestamp=detection.timestamp, queued_audio=queued_audio)
                            wake_task = asyncio.create_task(client.read_event())
                            pending.add(wake_task)
                        if audio_task in done:
                            ...
                            chunk = AudioChunk(rate=16000, width=2, channels=1, audio=chunk_bytes, timestamp=chunk_timestamp)
