...
class DumpCerts(procdump.ProcDump):
    ...
    rules = {}
    if has_yara:
        rules = yara.compile(sources={'x509': 'rule x509 {strings: $a = {30 82 ?? ?? 30 82 ?? ??} condition: $a}', 'pkcs': 'rule pkcs {strings: $a = {30 82 ?? ?? 02 01 00} condition: $a}'})
    type_map = {'x509': '_X509_PUBLIC_CERT', 'pkcs': '_PKCS_PRIVATE_CERT'}
