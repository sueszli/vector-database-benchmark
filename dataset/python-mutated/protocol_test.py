...
from ..connections import AsyncBytesWriter, AsyncTextWriter, create_memory_text_reader, MemoryBytesWriter
from ..protocol import ClientCapabilities, ContentChange, DiagnosticTag, DidChangeTextDocumentParameters, DidCloseTextDocumentParameters, DidOpenTextDocumentParameters, DidSaveTextDocumentParameters, DocumentUri, Info, InitializationOptions, InitializeParameters, LspHoverResponse, PublishDiagnosticsClientCapabilities, PublishDiagnosticsClientTagSupport, PyreHoverResponse, read_json_rpc, ReadChannelClosedError, ShowStatusRequestClientCapabilities, TextDocumentClientCapabilities, TextDocumentIdentifier, TextDocumentItem, TextDocumentSyncClientCapabilities, WindowClientCapabilities, write_json_rpc, write_json_rpc_ignore_connection_error
T = TypeVar('T')
...
class LSPInputOutputTest(testslide.TestCase):
    ...
    async def test_read_json_rpc(self) -> None:
        ...
        async def assert_not_parsed(input: str, exception_type: Type[Exception]=json_rpc.ParseError) -> None:
        ...
        await assert_not_parsed('Content-Length: 4\r\n\r\n{"jsonrpc": "2.0", "id": 0, "method": "foo"}')
        await assert_parses('Content-Length: 27\r\n\r\n{"jsonrpc": "2.0", "id": 0}Content-Length: 44\r\n\r\n{"jsonrpc": "2.0", "id": 0, "method": "foo"}', expected=json_rpc.Request(id=0, method='foo'))
        await assert_parses('Content-Length: 44\r\n\r\n{"jsonrpc": "2.0", "id": 0, "method": "foo"}', expected=json_rpc.Request(id=0, method='foo'))
        await assert_parses('CONTENT-LENGTH: 44\r\n\r\n{"jsonrpc": "2.0", "id": 0, "method": "foo"}', expected=json_rpc.Request(id=0, method='foo'))
        await assert_parses('Content-Length: 44\r\nContent-Type: application/vscode-jsonrpc; charset=utf-8\r\n\r\n{"jsonrpc": "2.0", "id": 0, "method": "foo"}', expected=json_rpc.Request(id=0, method='foo'))
        await assert_parses('Content-Type: application/vscode-jsonrpc; charset=utf-8\r\nContent-Length: 44\r\n\r\n{"jsonrpc": "2.0", "id": 0, "method": "foo"}', expected=json_rpc.Request(id=0, method='foo'))
    ...
    async def test_write_lsp(self) -> None:
        ...
        async def assert_write(response: json_rpc.Response, expected: str) -> None:
            ...
            self.assertEqual(actual, expected)
        await assert_write(json_rpc.SuccessResponse(id=0, result=42), expected='Content-Length: 41\r\n\r\n' + json.dumps({'jsonrpc': '2.0', 'id': 0, 'result': 42}))
        await assert_write(json_rpc.ErrorResponse(id=0, code=42, message='derp'), expected='Content-Length: 69\r\n\r\n' + json.dumps({'jsonrpc': '2.0', 'id': 0, 'error': {'code': 42, 'message': 'derp'}}))

    @setup.async_test
    async def test_write_json_rpc_ignore_connection_error(self) -> None:
        await write_json_rpc_ignore_connection_error(AsyncTextWriter(ExceptionRaisingBytesWriter(ConnectionResetError())), json_rpc.ErrorResponse(id=None, code=42, message='dummy message'))
