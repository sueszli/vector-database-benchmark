...
with outfile.open('w') as f, contextlib.redirect_stdout(f):
    ...
    category('Lifecycle', '', [addonmanager.LoadHook, hooks.RunningHook, hooks.ConfigureHook, hooks.DoneHook])
    category('Connection', '', [server_hooks.ClientConnectedHook, server_hooks.ClientDisconnectedHook, server_hooks.ServerConnectHook, server_hooks.ServerConnectedHook, server_hooks.ServerDisconnectedHook])
    category('HTTP', '', [http.HttpRequestHeadersHook, http.HttpRequestHook, http.HttpResponseHeadersHook, http.HttpResponseHook, http.HttpErrorHook, http.HttpConnectHook, http.HttpConnectUpstreamHook])
    category('DNS', '', [dns.DnsRequestHook, dns.DnsResponseHook, dns.DnsErrorHook])
    category('TCP', '', [tcp.TcpStartHook, tcp.TcpMessageHook, tcp.TcpEndHook, tcp.TcpErrorHook])
    category('UDP', '', [udp.UdpStartHook, udp.UdpMessageHook, udp.UdpEndHook, udp.UdpErrorHook])
    category('QUIC', '', [quic.QuicStartClientHook, quic.QuicStartServerHook])
    category('TLS', '', [tls.TlsClienthelloHook, tls.TlsStartClientHook, tls.TlsStartServerHook, tls.TlsEstablishedClientHook, tls.TlsEstablishedServerHook, tls.TlsFailedClientHook, tls.TlsFailedServerHook])
    category('WebSocket', '', [websocket.WebsocketStartHook, websocket.WebsocketMessageHook, websocket.WebsocketEndHook])
    category('SOCKSv5', '', [modes.Socks5AuthHook])
    category('AdvancedLifecycle', '', [layer.NextLayerHook, hooks.UpdateHook, log.AddLogHook])
