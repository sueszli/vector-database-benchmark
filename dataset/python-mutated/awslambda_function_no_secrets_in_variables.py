...
class awslambda_function_no_secrets_in_variables(Check):
    ...
    def execute(self):
        ...
        for function in awslambda_client.functions.values():
            ...
            report.status = 'PASS'
            report.status_extended = f'No secrets found in Lambda function {function.name} variables.'
            if function.environment:
                temp_env_data_file = tempfile.NamedTemporaryFile(delete=False)
                temp_env_data_file.write(bytes(json.dumps(function.environment, indent=2), encoding='raw_unicode_escape'))
