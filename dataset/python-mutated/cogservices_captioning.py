...
FEATURE_NAMES = ['tags', 'objects', 'caption', 'denseCaptions', 'read', 'smartCrops', 'people']
LANGUAGE = 'en'
...
async def azure_cognitive_services_caption(session: RetryClient, token_manager, kv_token_manager: KeyVaultAPITokenManager, image_url: Optional[str]=None, image_data: Optional[bytes]=None, tag_confidence_thresh: float=0.2, timeout: int=10) -> str:
    ...
    headers = {'Content-Type': 'application/json' if image_url is not None else 'application/octet-stream', 'Ocp-Apim-Subscription-Key': await kv_token_manager.get_token(), 'Authorization': f'Bearer {await token_manager.get_token()}'}
    ...
    async with session.post(url, headers=headers, data=body, params=params, timeout=ClientTimeout(total=timeout)) as response:
