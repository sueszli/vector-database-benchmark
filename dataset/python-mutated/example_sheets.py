...
with DAG(DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), catchup=False, tags=['example', 'sheets']) as dag:
    ...
    print_spreadsheet_url = BashOperator(task_id='print_spreadsheet_url', bash_command=f"echo {XComArg(create_spreadsheet, key='spreadsheet_url')}")
    upload_gcs_to_sheet = GCSToGoogleSheetsOperator(task_id='upload_gcs_to_sheet', bucket_name=BUCKET_NAME, object_name="{{ task_instance.xcom_pull('upload_sheet_to_gcs')[0] }}", spreadsheet_id="{{ task_instance.xcom_pull(task_ids='create_spreadsheet', key='spreadsheet_id') }}", gcp_conn_id=CONNECTION_ID)
    ...
    [create_bucket, create_temp_sheets_connection_task] >> create_spreadsheet >> print_spreadsheet_url >> upload_sheet_to_gcs >> upload_gcs_to_sheet >> [delete_bucket, delete_temp_sheets_connection_task]
