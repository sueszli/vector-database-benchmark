...
def run(train_batch_size, val_batch_size, epochs, lr, momentum):
    ...
    wandb_logger = WandBLogger(project='pytorch-ignite-integration', name='ignite-mnist-example', config={'train_batch_size': train_batch_size, 'val_batch_size': val_batch_size, 'epochs': epochs, 'lr': lr, 'momentum': momentum})
    wandb_logger.attach_output_handler(trainer, event_name=Events.ITERATION_COMPLETED(every=100), tag='training', output_transform=lambda loss: {'batchloss': loss})
    for (tag, evaluator) in [('training', train_evaluator), ('validation', validation_evaluator)]:
        wandb_logger.attach_output_handler(evaluator, event_name=Events.EPOCH_COMPLETED, tag=tag, metric_names=['loss', 'accuracy'], global_step_transform=lambda *_: trainer.state.iteration)
    wandb_logger.attach_opt_params_handler(trainer, event_name=Events.ITERATION_COMPLETED(every=100), optimizer=optimizer)
    ...
    model_checkpoint = ModelCheckpoint(wandb_logger.run.dir, n_saved=2, filename_prefix='best', score_function=score_function, score_name='validation_accuracy', global_step_transform=global_step_from_engine(trainer))
