...
class TriangularLRScheduleConfig(FairseqDataclass):
    max_lr: float = field(default='???', metadata={'help': 'max learning rate, must be more than cfg.lr'})
    lr_period_updates: float = field(default=5000, metadata={'help': 'initial number of updates per period (cycle length)'})
    lr_shrink: float = field(default=0.1, metadata={'help': 'shrink factor for annealing'})
    shrink_min: bool = field(default=False, metadata={'help': 'if set, also shrinks min lr'})
