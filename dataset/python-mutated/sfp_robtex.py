...
class sfp_robtex(SpiderFootPlugin):
    meta = {'name': 'Robtex', 'summary': 'Search Robtex.com for hosts sharing the same IP.', 'flags': [], 'useCases': ['Footprint', 'Investigate', 'Passive'], 'categories': ['Passive DNS'], 'dataSource': {'website': 'https://www.robtex.com/', 'model': 'FREE_NOAUTH_UNLIMITED', 'references': ['https://www.robtex.com/api/'], 'favIcon': 'https://www.robtex.com/favicon.ico', 'logo': 'https://www.robtex.com/favicon.ico', 'description': 'Robtex is used for various kinds of research of IP numbers, Domain names, etc\nRobtex uses various sources to gather public information about IP numbers, domain names, host names, Autonomous systems, routes etc. It then indexes the data in a big database and provide free access to the data.'}}
    opts = {'verify': True, 'netblocklookup': True, 'maxnetblock': 24, 'maxv6netblock': 120, 'cohostsamedomain': False, 'maxcohost': 100, 'subnetlookup': False, 'maxsubnet': 24, 'maxv6subnet': 120}
    optdescs = {'verify': 'Verify co-hosts are valid by checking if they still resolve to the shared IP.', 'netblocklookup': 'Look up all IPs on netblocks deemed to be owned by your target for possible co-hosts on the same target subdomain/domain?', 'maxnetblock': 'If looking up owned netblocks, the maximum netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'maxv6netblock': 'If looking up owned netblocks, the maximum IPv6 netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'cohostsamedomain': 'Treat co-hosted sites on the same target domain as co-hosting?', 'maxcohost': 'Stop reporting co-hosted sites after this many are found, as it would likely indicate web hosting.', 'subnetlookup': 'Look up all IPs on subnets which your target is a part of?', 'maxsubnet': 'If looking up subnets, the maximum subnet size to look up all the IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'maxv6subnet': 'If looking up subnets, the maximum IPv6 subnet size to look up all the IPs within (CIDR value, 24 = /24, 16 = /16, etc.)'}
    results = None
    errorState = False
    cohostcount = 0
