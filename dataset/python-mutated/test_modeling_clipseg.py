...
from transformers.testing_utils import is_flax_available, is_pt_flax_cross_test, require_torch, require_vision, slow, torch_device
...
class CLIPSegModelTester:
    ...
    def __init__(self, parent, text_kwargs=None, vision_kwargs=None, is_training=True, extract_layers=(1,)):
        if False:
            for i in range(10):
                print('nop')
    ...
    def get_config(self):
        if False:
            print('Hello World!')
        return CLIPSegConfig.from_text_vision_configs(self.text_model_tester.get_config(), self.vision_model_tester.get_config(), projection_dim=64, reduce_dim=32, extract_layers=self.extract_layers)
    ...
    def create_and_check_model_for_image_segmentation(self, config, input_ids, attention_maks, pixel_values):
        ...
        self.parent.assertEqual(result.logits.shape, (self.vision_model_tester.batch_size, self.vision_model_tester.image_size, self.vision_model_tester.image_size))
        self.parent.assertEqual(result.conditional_embeddings.shape, (self.text_model_tester.batch_size, config.projection_dim))
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False):
        ...
        if return_labels:
            if model_class.__name__ == 'CLIPSegForImageSegmentation':
                ...
                inputs_dict['labels'] = torch.zeros([batch_size, height, width], device=torch_device, dtype=torch.float)
        return inputs_dict
...
class CLIPSegModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_segmentation(self):
        ...
        self.assertEqual(outputs.logits.shape, torch.Size((3, 352, 352)))
        expected_masks_slice = torch.tensor([[-7.4613, -7.4785, -7.3628], [-7.3268, -7.0899, -7.1333], [-6.9838, -6.79, -6.8913]]).to(torch_device)
        self.assertTrue(torch.allclose(outputs.logits[0, :3, :3], expected_masks_slice, atol=0.001))
        expected_conditional = torch.tensor([0.5601, -0.0314, 0.198]).to(torch_device)
        expected_pooled_output = torch.tensor([0.5036, -0.2681, -0.2644]).to(torch_device)
        self.assertTrue(torch.allclose(outputs.conditional_embeddings[0, :3], expected_conditional, atol=0.001))
        self.assertTrue(torch.allclose(outputs.pooled_output[0, :3], expected_pooled_output, atol=0.001))