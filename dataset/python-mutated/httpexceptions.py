...
class _HTTPMove(HTTPRedirection):
    ...
    body_template_obj = Template('${explanation} ${location}; you should be redirected automatically.\n${detail}\n${html_comment}')
...
class HTTPMethodNotAllowed(HTTPClientError):
    ...
    body_template_obj = Template('The method ${REQUEST_METHOD} is not allowed for this resource. ${br}${br}\n${detail}')
...
class HTTPFailedDependency(HTTPClientError):
    ...
    explanation = 'The method could not be performed because the requested action dependended on another action and that action failed'
...
class HTTPTooManyRequests(HTTPClientError):
    ...
    explanation = 'The action could not be performed because there were too many requests by the client.'
...
for (name, value) in list(globals().items()):
    if isinstance(value, type) and issubclass(value, HTTPException) and (value not in {HTTPClientError, HTTPServerError}) and (not name.startswith('_')):
