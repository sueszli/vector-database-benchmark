...
with DAG(dag_id=DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), tags=['example'], catchup=False) as dag:
    ...
    video_ids_to_s3 = GoogleApiToS3Operator(task_id='video_ids_to_s3', google_api_service_name='youtube', google_api_service_version='v3', google_api_endpoint_path='youtube.search.list', gcp_conn_id=conn_id_name, google_api_endpoint_params={'part': 'snippet', 'channelId': YOUTUBE_CHANNEL_ID, 'maxResults': 50, 'publishedAfter': YOUTUBE_VIDEO_PUBLISHED_AFTER, 'publishedBefore': YOUTUBE_VIDEO_PUBLISHED_BEFORE, 'type': 'video', 'fields': 'items/id/videoId'}, google_api_response_via_xcom='video_ids_response', s3_destination_key=f'https://s3.us-west-2.amazonaws.com/{s3_bucket_name}/youtube_search', s3_overwrite=True)
    transform_video_ids_task = transform_video_ids()
    video_data_to_s3 = GoogleApiToS3Operator(task_id='video_data_to_s3', google_api_service_name='youtube', google_api_service_version='v3', gcp_conn_id=conn_id_name, google_api_endpoint_path='youtube.videos.list', google_api_endpoint_params={'part': YOUTUBE_VIDEO_PARTS, 'maxResults': 50, 'fields': YOUTUBE_VIDEO_FIELDS}, google_api_endpoint_params_via_xcom='video_ids', s3_destination_key=f'https://s3.us-west-2.amazonaws.com/{s3_bucket_name}/youtube_videos', s3_overwrite=True)
    ...
    chain(test_context, set_up_connection, create_s3_bucket, wait_for_bucket_creation, video_ids_to_s3, transform_video_ids_task, video_data_to_s3, delete_s3_bucket)
