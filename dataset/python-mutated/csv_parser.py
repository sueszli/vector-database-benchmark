...
class CsvParser(FileTypeParser):
    _MAX_BYTES_PER_FILE_FOR_SCHEMA_INFERENCE = 1000000
    ...
    async def infer_schema(self, config: FileBasedStreamConfig, file: RemoteFile, stream_reader: AbstractFileBasedStreamReader, logger: logging.Logger) -> SchemaType:
        ...
        if input_schema:
            return input_schema
        config_format = _extract_format(config)
        type_inferrer_by_field: Dict[str, _TypeInferrer] = defaultdict(lambda : _JsonTypeInferrer(config_format.true_values, config_format.false_values, config_format.null_values) if config_format.inference_type != InferenceType.NONE else _DisabledTypeInferrer())
        ...
        for row in data_generator:
            ...
            read_bytes += len(row) - 1
            if read_bytes >= self._MAX_BYTES_PER_FILE_FOR_SCHEMA_INFERENCE:
                break
        if not type_inferrer_by_field:
            raise AirbyteTracedException(message=f'Could not infer schema as there are no rows in {file.uri}. If having an empty CSV file is expected, ignore this. Else, please contact Airbyte.', failure_type=FailureType.config_error)
