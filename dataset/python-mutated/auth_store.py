...
from . import models
from .const import ACCESS_TOKEN_EXPIRATION, GROUP_ID_ADMIN, GROUP_ID_READ_ONLY, GROUP_ID_USER
...
class AuthStore:
    ...
    async def async_create_user(self, name: str | None, is_owner: bool | None=None, is_active: bool | None=None, system_generated: bool | None=None, credentials: models.Credentials | None=None, group_ids: list[str] | None=None, local_only: bool | None=None) -> models.User:
        ...
        for group_id in group_ids or []:
            ...
            groups.append(group)
        kwargs: dict[str, Any] = {'name': name, 'groups': groups, 'perm_lookup': self._perm_lookup}
        for (attr_name, value) in (('is_owner', is_owner), ('is_active', is_active), ('local_only', local_only), ('system_generated', system_generated)):
    ...
    async def async_update_user(self, user: models.User, name: str | None=None, is_active: bool | None=None, group_ids: list[str] | None=None, local_only: bool | None=None) -> None:
        ...
        for (attr_name, value) in (('name', name), ('is_active', is_active), ('local_only', local_only)):
            if value is not None:
    ...
    async def async_create_refresh_token(self, user: models.User, client_id: str | None=None, client_name: str | None=None, client_icon: str | None=None, token_type: str=models.TOKEN_TYPE_NORMAL, access_token_expiration: timedelta=ACCESS_TOKEN_EXPIRATION, credential: models.Credentials | None=None) -> models.RefreshToken:
        """Create a new token for a user."""
        kwargs: dict[str, Any] = {'user': user, 'client_id': client_id, 'token_type': token_type, 'access_token_expiration': access_token_expiration, 'credential': credential}
        if client_name:
    ...
    async def _async_load_task(self) -> None:
        ...
        for group_dict in data.get('groups', []):
            ...
            if policy is None:
                ...
                continue
            groups[group_dict['id']] = models.Group(id=group_dict['id'], name=name, policy=policy, system_generated=system_generated)
        ...
        for user_dict in data['users']:
            ...
            users[user_dict['id']] = models.User(name=user_dict['name'], groups=user_groups, id=user_dict['id'], is_owner=user_dict['is_owner'], is_active=user_dict['is_active'], system_generated=user_dict['system_generated'], perm_lookup=perm_lookup, local_only=user_dict.get('local_only', False))
        ...
        for rt_dict in data['refresh_tokens']:
            ...
            if created_at is None:
                getLogger(__name__).error('Ignoring refresh token %(id)s with invalid created_at %(created_at)s for user_id %(user_id)s', rt_dict)
                continue
            ...
            if (last_used_at_str := rt_dict.get('last_used_at')):
                ...
                last_used_at = None
            token = models.RefreshToken(id=rt_dict['id'], user=users[rt_dict['user_id']], client_id=rt_dict['client_id'], client_name=rt_dict.get('client_name'), client_icon=rt_dict.get('client_icon'), token_type=token_type, created_at=created_at, access_token_expiration=timedelta(seconds=rt_dict['access_token_expiration']), token=rt_dict['token'], jwt_key=rt_dict['jwt_key'], last_used_at=last_used_at, last_used_ip=rt_dict.get('last_used_ip'), version=rt_dict.get('version'))
