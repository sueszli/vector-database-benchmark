...
import unittest
import numpy as np
from transformers.testing_utils import is_pt_flax_cross_test, require_flax, require_torch, require_vision, slow, torch_device
...
if is_flax_available():
    from transformers import FlaxBertModel, FlaxCLIPVisionModel, FlaxVisionTextDualEncoderModel, FlaxViTModel, VisionTextDualEncoderConfig, VisionTextDualEncoderProcessor
...
class FlaxViTBertModelTest(VisionTextDualEncoderMixin, unittest.TestCase):
    ...
    def get_pretrained_model_and_inputs(self):
        ...
        batch_size = 13
        pixel_values = floats_tensor([batch_size, model.config.vision_config.num_channels, model.config.vision_config.image_size, model.config.vision_config.image_size])
    ...
    def prepare_config_and_inputs(self):
        ...
        return {'text_config': text_config, 'vision_config': vision_config, 'pixel_values': pixel_values, 'attention_mask': attention_mask, 'input_ids': input_ids, 'token_type_ids': token_type_ids}
...
class FlaxCLIPVisionBertModelTest(VisionTextDualEncoderMixin, unittest.TestCase):
    ...
    def get_pretrained_model_and_inputs(self):
        if False:
            return 10
        model = FlaxVisionTextDualEncoderModel.from_vision_text_pretrained('hf-internal-testing/tiny-random-clip', 'hf-internal-testing/tiny-bert', vision_from_pt=True, text_from_pt=True)
        batch_size = 13
        pixel_values = floats_tensor([batch_size, model.config.vision_config.num_channels, model.config.vision_config.image_size, model.config.vision_config.image_size])
        input_ids = ids_tensor([batch_size, 4], model.config.text_config.vocab_size)
        attention_mask = random_attention_mask([batch_size, 4])
...
class FlaxVisionTextDualEncoderIntegrationTest(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        self.assertTrue(np.allclose(outputs.logits_per_image, expected_logits, atol=0.001))