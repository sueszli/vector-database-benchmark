...
class IndependentMaternGP(TimeSeriesModel):
    ...
    def __init__(self, nu=1.5, dt=1.0, obs_dim=1, length_scale_init=None, kernel_scale_init=None, obs_noise_scale_init=None):
        if False:
            i = 10
            return i + 15
        self.nu = nu
    ...
    def _get_init_dist(self):
        if False:
            print('Hello World!')
        return torch.distributions.MultivariateNormal(self.obs_matrix.new_zeros(self.obs_dim, self.kernel.state_dim), self.kernel.stationary_covariance().squeeze(-3))

    def _get_obs_dist(self):
        if False:
            return 10
        return dist.Normal(self.obs_matrix.new_zeros(self.obs_dim, 1, 1), self.obs_noise_scale.unsqueeze(-1).unsqueeze(-1)).to_event(1)
    ...
    def get_dist(self, duration=None):
        ...
        trans_dist = MultivariateNormal(self.obs_matrix.new_zeros(self.obs_dim, 1, self.kernel.state_dim), process_covar.unsqueeze(-3))
        trans_matrix = trans_matrix.unsqueeze(-3)
        return dist.GaussianHMM(self._get_init_dist(), trans_matrix, trans_dist, self.obs_matrix, self._get_obs_dist(), duration=duration)
    ...
    def _forecast(self, dts, filtering_state, include_observation_noise=True):
        ...
        predicted_function_covar = torch.matmul(trans_matrix.transpose(-1, -2), torch.matmul(filtering_state.covariance_matrix, trans_matrix))[..., 0, 0] + process_covar[..., 0, 0]
...
class LinearlyCoupledMaternGP(TimeSeriesModel):
    ...
    def __init__(self, nu=1.5, dt=1.0, obs_dim=2, num_gps=1, length_scale_init=None, kernel_scale_init=None, obs_noise_scale_init=None):
        if False:
            i = 10
            return i + 15
        self.nu = nu
        ...
        self.num_gps = num_gps
        super().__init__()
        self.kernel = MaternKernel(nu=nu, num_gps=num_gps, length_scale_init=length_scale_init, kernel_scale_init=kernel_scale_init)
    ...
    def _get_obs_matrix(self):
        if False:
            print('Hello World!')
        return self.A.repeat_interleave(self.kernel.state_dim, dim=0) * self.A.new_tensor([1.0] + [0.0] * (self.kernel.state_dim - 1)).repeat(self.num_gps).unsqueeze(-1)
    ...
    def get_dist(self, duration=None):
        ...
        return dist.GaussianHMM(self._get_init_dist(), trans_matrix, trans_dist, self._get_obs_matrix(), self._get_obs_dist(), duration=duration)
    ...
    def _forecast(self, dts, filtering_state, include_observation_noise=True, full_covar=True):
        ...
        trans_mat = block_diag_embed(trans_mat)
        process_covar = block_diag_embed(process_covar)
        obs_matrix = self._get_obs_matrix()
        trans_obs = torch.matmul(trans_mat, obs_matrix)
        predicted_mean = torch.matmul(filtering_state.loc.unsqueeze(-2), trans_obs).squeeze(-2)
        predicted_function_covar = torch.matmul(trans_obs.transpose(-1, -2), torch.matmul(filtering_state.covariance_matrix, trans_obs))
        predicted_function_covar = predicted_function_covar + torch.matmul(obs_matrix.transpose(-1, -2), torch.matmul(process_covar, obs_matrix))
        if include_observation_noise:
...
class DependentMaternGP(TimeSeriesModel):
    ...
    def __init__(self, nu=1.5, dt=1.0, obs_dim=1, linearly_coupled=False, length_scale_init=None, obs_noise_scale_init=None):
        if False:
            for i in range(10):
                print('nop')
        if nu != 1.5:
        ...
        self.obs_noise_scale = PyroParam(obs_noise_scale_init, constraint=constraints.positive)
        self.wiener_noise_tril = PyroParam(torch.eye(obs_dim) + 0.03 * torch.randn(obs_dim, obs_dim).tril(-1), constraint=constraints.lower_cholesky)
        if linearly_coupled:
    ...
    def _get_obs_matrix(self):
        ...
        if self.obs_matrix.size(0) == self.obs_dim:
            ...
            return self.obs_matrix.repeat_interleave(self.kernel.state_dim, dim=0) * self.obs_matrix.new_tensor(selector).repeat(self.obs_dim).unsqueeze(-1)
        else:
    ...
    def _get_init_dist(self, stationary_covariance):
        if False:
            i = 10
            return i + 15
        return torch.distributions.MultivariateNormal(self.obs_matrix.new_zeros(self.full_state_dim), stationary_covariance)
    ...
    def _stationary_covariance(self):
        ...
        block = 2.0 * self.kernel.mask00 + (rho_i - rho_j) * (self.kernel.mask01 - self.kernel.mask10) + 2.0 * rho_i * rho_j * self.kernel.mask11
    ...
    def _get_trans_dist(self, trans_matrix, stationary_covariance):
        if False:
            print('Hello World!')
        covar = stationary_covariance - torch.matmul(trans_matrix.transpose(-1, -2), torch.matmul(stationary_covariance, trans_matrix))
    ...
    def get_dist(self, duration=None):
        ...
        return dist.GaussianHMM(self._get_init_dist(stat_covar), trans_matrix, trans_dist, self._get_obs_matrix(), self._get_obs_dist(), duration=duration)
    ...
    def _forecast(self, dts, filtering_state, include_observation_noise=True):
        ...
        predicted_mean = torch.matmul(filtering_state.loc.unsqueeze(-2), trans_obs).squeeze(-2)
        predicted_function_covar = torch.matmul(trans_obs.transpose(-1, -2), torch.matmul(filtering_state.covariance_matrix, trans_obs)) + torch.matmul(obs_matrix.t(), torch.matmul(trans_dist.covariance_matrix, obs_matrix))
        if include_observation_noise:
            predicted_function_covar = predicted_function_covar + self.obs_noise_scale.pow(2.0)
        return (predicted_mean, predicted_function_covar)
