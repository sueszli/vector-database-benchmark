...
class DisconnectedShape(LayoutElement):
    ...
    def _paint_content_box(self, page: Page, available_space: Rectangle) -> None:
        ...
        content = 'q %f %f %f RG %d w ' % (float(stroke_rgb.red), float(stroke_rgb.green), float(stroke_rgb.blue), float(self._line_width))
        for l in self._lines:
            content += ' %f %f m %f %f l ' % (float(l[0][0]), float(l[0][1]), float(l[1][0]), float(l[1][1]))
        content += ' S Q'
    ...
    def move_to(self, lower_left_x: Decimal, lower_left_y: Decimal) -> 'DisconnectedShape':
        if False:
            return 10
        ...
        self._lines = [((x[0][0] + delta_x, x[0][1] + delta_y), (x[1][0] + delta_x, x[1][1] + delta_y)) for x in self._lines]
        return self
    ...
    def rotate(self, angle_in_radians: float) -> 'Shape':
        ...
        self._lines = [((a * l[0][0] + c * l[0][1], b * l[0][0] + d * l[0][1]), (a * l[1][0] + c * l[1][1], b * l[1][0] + d * l[1][1])) for l in self._lines]
        return self

    def scale_down(self, max_width: Decimal, max_height: Decimal, preserve_aspect_ratio: bool=True) -> 'DisconnectedShape':
        if False:
            return 10
        ...
        if preserve_aspect_ratio:
            ...
            h_scale = w_scale
        if w_scale < 1:
            self._lines = [((x[0][0] * w_scale, x[0][1]), (x[1][0] * w_scale, x[1][1])) for x in self._lines]
        if h_scale < 1:
            self._lines = [((x[0][0], x[0][1] * h_scale), (x[1][0], x[1][1] * h_scale)) for x in self._lines]
        return self

    def scale_up(self, max_width: Decimal, max_height: Decimal, preserve_aspect_ratio: bool=True) -> 'DisconnectedShape':
        if False:
            for i in range(10):
                print('nop')
        '\n        This method scales this Shape down to fit a given max. width / height\n        '
