...
@pytest.mark.parametrize('ws_transport, expected_result', [(mock.MagicMock(transport=mock.MagicMock(get_extra_info=lambda name, default=None: {'test': 'existent'}.get(name, default))), 'existent'), (None, 'default'), (mock.MagicMock(transport=None), 'default')])
async def test_get_extra_info(make_request: Any, mocker: Any, ws_transport: Any, expected_result: Any) -> None:
    valid_key = 'test'
