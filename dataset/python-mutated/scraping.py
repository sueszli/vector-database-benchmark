...
htmltags = re.compile('<(noscript|script)>.*?<\\/\\1>', re.S)
wikilinks = re.compile('href="/wiki/(.*?)"')
wiki_base = 'https://en.wikipedia.org/wiki/'
stackexchangelinks = re.compile('(?:(https:\\/\\/\\w+)\\.stack\\w+\\.com|)\\/questions\\/(\\d+\\/[\\w\\d\\/-]+)')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--mode', default='auto', choices=['auto', 'wiki', 'math_stack', 'physics_stack'], help='What website to scrape. Choices: `auto` determine by input, `wiki` wikipedia,                         `math_stack` math.stackexchange, `physics_stack` physics.stackexchange.')
    parser.add_argument(nargs='*', dest='url', default=['https://en.wikipedia.org/wiki/Mathematics', 'https://en.wikipedia.org/wiki/Physics'], help='starting url(s). Default: Mathematics, Physics wiki pages')
