...
class ACRExchangeClient(object):
    ...
    async def get_acr_access_token(self, challenge: str, **kwargs) -> Optional[str]:
        ...
        return await self.exchange_refresh_token_for_access_token(refresh_token, service=parsed_challenge['service'], scope=parsed_challenge['scope'], **kwargs)
    ...
    async def exchange_aad_token_for_refresh_token(self, service: str, **kwargs) -> str:
        ...
        refresh_token = await auth_operation.exchange_aad_access_token_for_acr_refresh_token(grant_type=PostContentSchemaGrantType.ACCESS_TOKEN, service=service, access_token=token.token, **kwargs)
        return refresh_token.refresh_token if refresh_token.refresh_token is not None else ''

    async def exchange_refresh_token_for_access_token(self, refresh_token: str, service: str, scope: str, **kwargs) -> Optional[str]:
        auth_operation = cast(AuthenticationOperations, self._client.authentication)
        access_token = await auth_operation.exchange_acr_refresh_token_for_acr_access_token(service, scope, refresh_token, **kwargs)
