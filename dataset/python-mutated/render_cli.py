...
class TextAsDefault(NamedTuple):
    text: str
CUSTOM = {'discovery': ComponentBuilder.entry_points_for('virtualenv.discovery'), 'creator': ComponentBuilder.entry_points_for('virtualenv.create'), 'seeder': ComponentBuilder.entry_points_for('virtualenv.seed'), 'activators': ComponentBuilder.entry_points_for('virtualenv.activate')}
...
class CliTable(SphinxDirective):
    ...
    def _build_table(self, options, title, description):
        ...
        options_group += body
        return table
    plugins: ClassVar[dict[str, str]] = {'creator': 'virtualenv.create', 'seed': 'virtualenv.seed', 'activators': 'virtualenv.activate', 'discovery': 'virtualenv.discovery'}
