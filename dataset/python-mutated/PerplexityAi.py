...
class PerplexityAi(AsyncProvider):
    ...
    @classmethod
    async def create_async(cls, model: str, messages: list[dict[str, str]], proxy: str=None, **kwargs) -> str:
        ...
        async with AsyncSession(headers=headers, proxies={'https': proxy}, impersonate='chrome107') as session:
            ...
            response = await session.get(url, params={'t': timestamp(), 'sid': sid})
            response.raise_for_status()
            data = '424' + json.dumps(['perplexity_ask', format_prompt(messages), {'version': '2.1', 'source': 'default', 'language': 'en', 'timezone': time.tzname[0], 'search_focus': 'internet', 'mode': 'concise'}])
            response = await session.post(url, params={'t': timestamp(), 'sid': sid}, data=data)
            response.raise_for_status()
            while True:
                response = await session.get(url, params={'t': timestamp(), 'sid': sid})
                ...
                for line in response.text.splitlines():
                    if line.startswith('434'):
                        ...
                        cls._sources = [{'title': source['name'], 'url': source['url'], 'snippet': source['snippet']} for source in result['web_results']]
