...
if __name__ == '__main__':
    ...
    parser.add_argument('--squad_input_filename', dest='squad_input_filename', help='A dataset with a SQuAD JSON format.', metavar='SQUAD_in', required=True)
    parser.add_argument('--dpr_output_filename', dest='dpr_output_filename', help='The name of the DPR JSON formatted output file', metavar='DPR_out', required=True)
    parser.add_argument('--num_hard_negative_ctxs', dest='num_hard_negative_ctxs', help='Number of hard negative contexts to use', metavar='num_hard_negative_ctxs', default=30)
    parser.add_argument('--split_dataset', dest='split_dataset', action='store_true', help='Whether to split the created dataset or not (default: False)')
    args = parser.parse_args()
    preprocessor = PreProcessor(split_length=100, split_overlap=0, clean_empty_lines=False, split_respect_sentence_boundary=False, clean_whitespace=False)
    ...
    main(squad_input_filename=squad_input_filename, dpr_output_filename=dpr_output_filename, preprocessor=preprocessor, document_store_type_config=('ElasticsearchDocumentStore', store_dpr_config), retriever_type_config=('BM25Retriever', retriever_bm25_config), num_hard_negative_ctxs=num_hard_negative_ctxs, split_dataset=split_dataset)