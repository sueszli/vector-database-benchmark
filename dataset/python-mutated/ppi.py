...
class PPI_Hdr(Packet):
    name = 'PPI Header'
    fields_desc = [LEShortEnumField('pfh_type', 0, PPI_TYPES), LenField('pfh_length', None, fmt='<H')]
...
class PPI(Packet):
    ...
    fields_desc = [ByteField('version', 0), ByteField('flags', 0), FieldLenField('len', None, length_of='headers', fmt='<H', adjust=lambda p, x: x + 8), LEIntField('dlt', None), PacketListField('headers', [], PPI_Hdr, length_from=lambda p: p.len - 8)]
