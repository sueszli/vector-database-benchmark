...
class Test_s3_bucket_public_access:
    ...
    def test_no_buckets(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3control_client', new=S3Control(audit_info)):
                    from prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access import s3_bucket_public_access
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert len(result) == 1
                    assert result[0].status == 'PASS'
                    assert search('not public', result[0].status_extended)
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert search('public access due to bucket ACL', result[0].status_extended)
                    assert result[0].resource_id == bucket_name_us
                    assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                    assert result[0].region == AWS_REGION
    ...
    def test_bucket_public_policy(self):
        ...
        s3_client.put_bucket_policy(Bucket=bucket_name_us, Policy=public_write_policy)
        from prowler.providers.aws.services.s3.s3_service import S3, S3Control
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert len(result) == 1
                    assert result[0].status == 'FAIL'
                    assert search('public access due to bucket policy', result[0].status_extended)
                    assert result[0].resource_id == bucket_name_us
                    assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                    assert result[0].region == AWS_REGION
    ...
    def test_bucket_can_not_retrieve_public_access_block(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            ...
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3_client', new=s3):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access.s3control_client', new=S3Control(audit_info)):
                    from prowler.providers.aws.services.s3.s3_bucket_public_access.s3_bucket_public_access import s3_bucket_public_access
                    check = s3_bucket_public_access()
                    result = check.execute()
