...
def test_default_constructor(mock_auto_tokenizer, mock_boto3_session):
    ...
    layer = SageMakerHFTextGenerationInvocationLayer(model_name_or_path='some_fake_model', max_length=99, aws_access_key_id='some_fake_id', aws_secret_access_key='some_fake_key', aws_session_token='some_fake_token', aws_profile_name='some_fake_profile', aws_region_name='fake_region')
...
def test_constructor_with_model_kwargs(mock_auto_tokenizer, mock_boto3_session):
    ...
    layer = SageMakerHFTextGenerationInvocationLayer(model_name_or_path='some_fake_model', **model_kwargs, **model_kwargs_rejected)
...
def test_short_prompt_is_not_truncated(mock_boto3_session):
    ...
    with patch('transformers.AutoTokenizer.from_pretrained', return_value=mock_tokenizer):
        layer = SageMakerHFTextGenerationInvocationLayer('some_fake_endpoint', max_length=max_length_generated_text, model_max_length=total_model_max_length)
...
def test_supports_for_valid_aws_configuration():
    ...
    with patch('haystack.nodes.prompt.invocation_layer.aws_base.AWSBaseInvocationLayer.get_aws_session', return_value=mock_session):
        supported = SageMakerHFTextGenerationInvocationLayer.supports(model_name_or_path='some_sagemaker_deployed_model', aws_profile_name='some_real_profile')
...
def test_supports_not_on_invalid_aws_profile_name():
    ...
    with patch('boto3.Session') as mock_boto3_session:
        ...
        with pytest.raises(SageMakerConfigurationError, match='Failed to initialize the session'):
            SageMakerHFTextGenerationInvocationLayer.supports(model_name_or_path='some_fake_model', aws_profile_name='some_fake_profile')
...
def test_supports_not_triggered_for_invalid_iam_profile():
    ...
    assert not SageMakerHFTextGenerationInvocationLayer.supports(model_name_or_path='fake_endpoint', aws_profile_name='invalid-profile')