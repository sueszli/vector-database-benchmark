...
def upgrade():
    if False:
        while True:
            i = 10
    op.add_column('users', sa.Column('totp_secret', sa.LargeBinary(length=20), nullable=True))
    op.add_column('users', sa.Column('two_factor_allowed', sa.Boolean, nullable=False, server_default=sa.false()))
