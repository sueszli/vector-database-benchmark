...
if __name__ == '__main__':
    ...
    parser.add_argument('--session-id', help='ID of the DetectIntent session. Defaults to a random UUID.', default=str(uuid.uuid4()))
    parser.add_argument('--language-code', help='Language code of the query. Defaults to "en-US".', default='en-US')
    parser.add_argument('--knowledge-base-id', help='The id of the Knowledge Base to query against', required=True)
    parser.add_argument('texts', nargs='+', type=str, help='Text inputs.')
    args = parser.parse_args()
    detect_intent_knowledge(args.project_id, args.session_id, args.language_code, args.knowledge_base_id, args.texts)