...
from transformers import AdamW, AutoConfig, AutoModelForSequenceClassification, AutoTokenizer, DataCollatorWithPadding, default_data_collator, get_scheduler, set_seed
...
class FTModelArguments:
    ...
    model_name_or_path: str = dataclasses.field(metadata={'help': 'Path to pretrained model or model identifier from huggingface.co/models.'})
    use_fast_tokenizer: Optional[bool] = dataclasses.field(default=True, metadata={'help': 'Whether to use one of the fast tokenizer (backed by the tokenizers library) or not.'})
    cache_dir: Optional[str] = dataclasses.field(default=None, metadata={'help': 'Where do you want to store the pretrained models downloaded from huggingface.co.'})
...
class FTTrainingArguments:
    ...
    output_dir: str = dataclasses.field(metadata={'help': 'The output directory where the model predictions and checkpoints will be written.'})
    do_train: Optional[bool] = dataclasses.field(default=False, metadata={'help': 'Whether to run training or not.'})
    do_eval: Optional[bool] = dataclasses.field(default=False, metadata={'help': 'Whether to run evaluation on the validation set or not.'})
    do_predict: Optional[bool] = dataclasses.field(default=False, metadata={'help': 'Whether to run inference on the inference set or not.'})
    seed: Optional[int] = dataclasses.field(default=42, metadata={'help': 'Random seed that will be set at the beginning of training.'})
    per_device_train_batch_size: Optional[int] = dataclasses.field(default=8, metadata={'help': 'The batch size per GPU/TPU core/CPU for training.'})
    per_device_eval_batch_size: Optional[int] = dataclasses.field(default=8, metadata={'help': 'The batch size per GPU/TPU core/CPU for evaluation.'})
    weight_decay: Optional[float] = dataclasses.field(default=0.0, metadata={'help': 'The weight decay to apply (if not zero) to all layers except all bias and LayerNorm weights in [`AdamW`] optimizer.'})
    learning_rate: Optional[float] = dataclasses.field(default=5e-05, metadata={'help': 'The initial learning rate for [`AdamW`] optimizer.'})
    gradient_accumulation_steps: Optional[int] = dataclasses.field(default=1, metadata={'help': 'Number of updates steps to accumulate the gradients for, before performing a backward/update pass.'})
    max_steps: Optional[int] = dataclasses.field(default=-1, metadata={'help': 'If set to a positive number, the total number of training steps to perform. Overrides `num_train_epochs`.'})
    lr_scheduler_type: Optional[str] = dataclasses.field(default='linear', metadata={'help': 'The scheduler type to use.'})
    warmup_steps: Optional[int] = dataclasses.field(default=1, metadata={'help': 'Number of steps used for a linear warmup from 0 to `learning_rate`. Overrides any effect of `warmup_ratio`.'})
    evaluation_strategy: Optional[str] = dataclasses.field(default='no', metadata={'help': 'The evaluation strategy to adopt during training. Possible values are: ["no", "step", "epoch]'})
    eval_steps: Optional[int] = dataclasses.field(default=1, metadata={'help': 'Number of update steps between two evaluations if `evaluation_strategy="steps"`.'})
    eval_metric: Optional[str] = dataclasses.field(default='accuracy', metadata={'help': 'The evaluation metric used for the task.'})
    keep_checkpoint_max: Optional[int] = dataclasses.field(default=1, metadata={'help': 'The maximum number of best checkpoint files to keep.'})
    early_stopping_patience: Optional[int] = dataclasses.field(default=10, metadata={'help': 'Number of evaluation calls with no improvement after which training will be stopped.'})
    early_stopping_threshold: Optional[float] = dataclasses.field(default=0.0, metadata={'help': 'How much the specified evaluation metric must improve to satisfy early stopping conditions.'})
