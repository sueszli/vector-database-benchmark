...
def set_privacy(request, collection_id):
    ...
    if request.method == 'POST':
        ...
        if restriction:
            ...
            form = CollectionViewRestrictionForm(initial={'restriction_type': 'none'})
    if restriction_exists_on_ancestor:
        return render_modal_workflow(request, 'wagtailadmin/collection_privacy/ancestor_privacy.html', None, {'collection_with_restriction': restriction.collection})
    else:
        return render_modal_workflow(request, 'wagtailadmin/collection_privacy/set_privacy.html', None, {'collection': collection, 'form': form}, json_data={'step': 'set_privacy'})