...
def gbm_mojo_reproducibility_info():
    ...
    w.set_names(['weight'])
    train = train.cbind(w)
    model = H2OGradientBoostingEstimator(ntrees=10, max_depth=5, min_rows=10, learn_rate=0.1, distribution='multinomial', weights_column='weight', calibrate_model=True, calibration_frame=calib)
