...
def test_external_obtain_access_tokens_plugin_not_active(api_client, customer_user):
    if False:
        return 10
    variables = {'pluginId': 'pluginId1', 'input': json.dumps({'code': 'ABCD', 'state': 'stata-data'})}
...
def test_external_obtain_access_tokens(mock_refresh_token_middleware, api_client, customer_user, monkeypatch):
    if False:
        while True:
            i = 10
    ...
    monkeypatch.setattr('saleor.plugins.manager.PluginsManager.external_obtain_access_tokens', mocked_plugin_fun)
    variables = {'pluginId': 'pluginId1', 'input': json.dumps({'code': 'ABCD', 'state': 'stata-data'})}
    response = api_client.post_graphql(MUTATION_EXTERNAL_REFRESH, variables)
    content = get_graphql_content(response)
    data = content['data']['externalObtainAccessTokens']
