...
import doctest
import inspect
from torch.testing._internal.common_utils import TestCase, run_tests, TEST_NUMPY, TEST_LIBROSA, TEST_MKL, first_sample, TEST_WITH_ROCM, make_tensor
from torch.testing._internal.common_device_type import instantiate_device_type_tests, ops, dtypes, onlyNativeDeviceTypes, skipCPUIfNoFFT, deviceCountAtLeast, onlyCUDA, OpDTypes, skipIf, toleranceOverride, tol
from torch.testing._internal.common_methods_invocations import spectral_funcs, SpectralFuncType
...
try:
    ...
    pass
REFERENCE_NORM_MODES = (None, 'forward', 'backward', 'ortho') if version.parse(np.__version__) >= version.parse('1.20.0') and (not has_scipy_fft or version.parse(scipy.__version__) >= version.parse('1.6.0')) else (None, 'ortho')
