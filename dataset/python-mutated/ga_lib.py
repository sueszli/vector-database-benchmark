...
FLAGS = flags.FLAGS
USE_REWARD_CACHE = True
GENES = bf.CHARS
MAX_PROGRAM_STEPS = 500
STEP_BONUS = True
ALPHANUM_CHARS = ['_'] + [chr(ord('a') + i_) for i_ in range(26)] + [chr(ord('A') + i_) for i_ in range(26)] + [chr(ord('0') + i_) for i_ in range(10)]
Result = namedtuple('Result', ['reward', 'inputs', 'code_outputs', 'target_outputs', 'type_in', 'type_out', 'base', 'correct'])
...
GaResult = namedtuple('GaResult', ['population', 'best_code', 'reward', 'generations', 'num_programs', 'solution_found', 'max_generations', 'max_num_programs'])
