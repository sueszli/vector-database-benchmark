...
class TestMissingValuesFiller:
    ...
    const_series = TimeSeries.from_times_and_values(time, np.array([2.0] * len(time)), static_covariates=static_covariate)
    const_series_with_holes = TimeSeries.from_times_and_values(time, np.array([2.0] * 10 + [np.nan] * 5 + [2.0] * 10 + [np.nan] * 5), static_covariates=static_covariate)
    lin = [float(i) for i in range(len(time))]
    lin_series = TimeSeries.from_times_and_values(time, np.array(lin))
    lin_series_with_holes = TimeSeries.from_times_and_values(time, np.array(lin[0:10] + [np.nan] * 5 + lin[15:24] + [np.nan] * 5 + [lin[29]]))
