...
from posthog.models.property.util import PropertyGroup, get_property_string_expr, get_single_or_multi_property_string_expr, parse_prop_grouped_clauses, prop_filter_json_extract
from posthog.models.team import Team
from posthog.queries.person_distinct_id_query import get_team_distinct_ids_query
from posthog.queries.person_query import PersonQuery
from posthog.queries.property_optimizer import PropertyOptimizer
from posthog.queries.util import PersonPropertiesMode
from posthog.test.base import BaseTest, ClickhouseTestMixin, _create_event, _create_person, cleanup_materialized_columns, snapshot_clickhouse_queries
...
TEST_BREAKDOWN_PROCESSING = [('$browser', 'events', 'prop', 'properties', 'replaceRegexpAll(JSONExtractRaw(properties, \'$browser\'), \'^"|"$\', \'\') AS prop'), (['$browser'], 'events', 'value', 'properties', 'array(replaceRegexpAll(JSONExtractRaw(properties, \'$browser\'), \'^"|"$\', \'\')) AS value'), (['$browser', '$browser_version'], 'events', 'prop', 'properties', 'array(replaceRegexpAll(JSONExtractRaw(properties, \'$browser\'), \'^"|"$\', \'\'),replaceRegexpAll(JSONExtractRaw(properties, \'$browser_version\'), \'^"|"$\', \'\')) AS prop')]
...
TEST_BREAKDOWN_PROCESSING_MATERIALIZED = [(['$browser'], 'events', 'value', 'properties', 'person_properties', 'array(replaceRegexpAll(JSONExtractRaw(properties, \'$browser\'), \'^"|"$\', \'\')) AS value', 'array("mat_pp_$browser") AS value'), (['$browser', '$browser_version'], 'events', 'prop', 'properties', 'group2_properties', 'array(replaceRegexpAll(JSONExtractRaw(properties, \'$browser\'), \'^"|"$\', \'\'),replaceRegexpAll(JSONExtractRaw(properties, \'$browser_version\'), \'^"|"$\', \'\')) AS prop', 'array("mat_gp2_$browser",replaceRegexpAll(JSONExtractRaw(properties, \'$browser_version\'), \'^"|"$\', \'\')) AS prop')]
