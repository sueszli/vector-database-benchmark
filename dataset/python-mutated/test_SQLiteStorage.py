...
class StorageTest(AsyncioTestCase):
    ...
    async def store_fake_stream(self, stream_hash, blobs=None, file_name='fake_file', key='DEADBEEF'):
        ...
        descriptor = StreamDescriptor(asyncio.get_event_loop(), self.blob_dir, file_name, key, file_name, blobs, stream_hash)
    ...
    async def make_and_store_fake_stream(self, blob_count=2, stream_hash=None):
        ...
        blobs = [BlobInfo(i + 1, 100, 'DEADBEEF', 0, random_lbry_hash()) for i in range(blob_count)]
...
class TestSQLiteStorage(StorageTest):
    ...
    async def test_supports_storage(self):
        ...
        random_supports = [{'txid': random_lbry_hash(), 'nout': i, 'address': f'addr{i}', 'amount': f'{i}.0'} for i in range(20)]
        expected_supports = {}
...
class FileStorageTests(StorageTest):
    ...
    async def test_store_file(self):
        ...
        blob_data_rate = 0
        file_name = 'test file'
        await self.storage.save_published_file(stream_hash, file_name, download_directory, blob_data_rate)
        files = await self.storage.get_all_lbry_files()
...
class ContentClaimStorageTests(StorageTest):
    ...
    async def test_store_content_claim(self):
        ...
        stream_hashes = await self.storage.get_old_stream_hashes_for_claim_id(fake_claim_info['claim_id'], stream_hash)
        self.assertListEqual(stream_hashes, [])
        ...
        with self.assertRaisesRegex(Exception, 'mismatching claim ids when updating stream deadbeefdeadbeefdeadbeefdeadbeefdeadbeef vs beef0002beef0002beef0002beef0002beef0002'):
