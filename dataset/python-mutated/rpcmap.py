...
from impacket.dcerpc.v5.rpch import RPC_PROXY_CONN_A1_401_ERR, RPC_PROXY_INVALID_RPC_PORT_ERR, RPC_PROXY_HTTP_IN_DATA_401_ERR, RPC_PROXY_CONN_A1_0X6BA_ERR, RPC_PROXY_CONN_A1_404_ERR, RPC_PROXY_RPC_OUT_DATA_404_ERR
...
if __name__ == '__main__':
    ...
    try:
        lookuper = RPCMap(options.stringbinding, options.auth_level, options.brute_uuids, uuids, options.brute_opnums, options.opnum_max, options.brute_versions, options.version_max)
        ...
        if RPC_PROXY_INVALID_RPC_PORT_ERR in error_text:
            logging.critical('This usually means the target is a MS Exchange Server, and connections to this rpc port on this host are not allowed (try port 6001)')
        if RPC_PROXY_RPC_OUT_DATA_404_ERR in error_text or RPC_PROXY_CONN_A1_404_ERR in error_text:
            logging.critical('This usually means the target is a MS Exchange Server, and connections to the specified RPC server are not allowed')
        ...
        if RPC_PROXY_HTTP_IN_DATA_401_ERR in error_text or RPC_PROXY_CONN_A1_401_ERR in error_text:
            if lookuper.get_rpc_transport().get_auth_type() == AUTH_BASIC and transportdomain == '':
                logging.critical('RPC proxy basic authentication might require you to specify the domain. Your domain is empty!')
