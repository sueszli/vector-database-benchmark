...
class GoogleSearchToolkit(BaseToolkit, ABC):
    ...
    def get_env_keys(self) -> List[ToolConfiguration]:
        if False:
            return 10
        return [ToolConfiguration(key='GOOGLE_API_KEY', key_type=ToolConfigKeyType.STRING, is_required=True, is_secret=True), ToolConfiguration(key='SEARCH_ENGINE_ID', key_type=ToolConfigKeyType.STRING, is_required=True, is_secret=True)]