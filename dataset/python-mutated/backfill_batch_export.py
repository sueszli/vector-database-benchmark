...
from posthog.temporal.workflows.batch_exports import CreateBatchExportBackfillInputs, UpdateBatchExportBackfillStatusInputs, create_batch_export_backfill_model, update_batch_export_backfill_model_status
...
async def get_schedule_frequency(schedule_id: str) -> float:
    ...
    """
    client = await connect(settings.TEMPORAL_HOST, settings.TEMPORAL_PORT, settings.TEMPORAL_NAMESPACE, settings.TEMPORAL_CLIENT_ROOT_CA, settings.TEMPORAL_CLIENT_CERT, settings.TEMPORAL_CLIENT_KEY)
...
async def backfill_schedule(inputs: BackfillScheduleInputs) -> None:
    ...
    if details:
        ...
        details = HeartbeatDetails(schedule_id=inputs.schedule_id, start_at=last_activity_details.start_at, end_at=last_activity_details.end_at, wait_start_at=last_activity_details.wait_start_at)
    ...
    for (backfill_start_at, backfill_end_at) in full_backfill_range:
        ...
        backfill = temporalio.client.ScheduleBackfill(start_at=backfill_start_at, end_at=backfill_end_at, overlap=temporalio.client.ScheduleOverlapPolicy.ALLOW_ALL)
        await handle.backfill(backfill)
        details = HeartbeatDetails(schedule_id=inputs.schedule_id, start_at=backfill_start_at.isoformat(), end_at=backfill_end_at.isoformat(), wait_start_at=utcnow.isoformat())
        await wait_for_schedule_backfill_in_range_with_heartbeat(details, client, heartbeat_timeout, inputs.wait_delay)

async def wait_for_schedule_backfill_in_range_with_heartbeat(heartbeat_details: HeartbeatDetails, client: temporalio.client.Client, heartbeat_timeout: dt.timedelta | None=None, wait_delay: float=5.0):
    """Decide if heartbeating is required while waiting for a backfill in range to finish."""
    if heartbeat_timeout:
        wait_func = heartbeat_details.make_activity_heartbeat_while_running(wait_for_schedule_backfill_in_range, heartbeat_every=dt.timedelta(seconds=1))
    else:
        wait_func = wait_for_schedule_backfill_in_range
    await wait_func(client, heartbeat_details.schedule_id, dt.datetime.fromisoformat(heartbeat_details.start_at), dt.datetime.fromisoformat(heartbeat_details.end_at), dt.datetime.fromisoformat(heartbeat_details.wait_start_at), wait_delay)

async def wait_for_schedule_backfill_in_range(client: temporalio.client.Client, schedule_id: str, start_at: dt.datetime, end_at: dt.datetime, now: dt.datetime, wait_delay: float=5.0) -> None:
    ...
    if await check_temporal_schedule_exists(client, schedule_id) is False:
        raise TemporalScheduleNotFoundError(schedule_id)
    query = f'TemporalScheduledById="{schedule_id}" AND TemporalScheduledStartTime >= "{start_at.isoformat()}" AND TemporalScheduledStartTime <= "{end_at.isoformat()}" AND StartTime >= "{now.isoformat()}"'
...
class BackfillBatchExportWorkflow(PostHogWorkflow):
    ...
    async def run(self, inputs: BackfillBatchExportInputs) -> None:
        ...
        create_batch_export_backfill_inputs = CreateBatchExportBackfillInputs(team_id=inputs.team_id, batch_export_id=inputs.batch_export_id, start_at=inputs.start_at, end_at=inputs.end_at, status='Running')
        backfill_id = await temporalio.workflow.execute_activity(create_batch_export_backfill_model, create_batch_export_backfill_inputs, start_to_close_timeout=dt.timedelta(minutes=5), retry_policy=temporalio.common.RetryPolicy(initial_interval=dt.timedelta(seconds=10), maximum_interval=dt.timedelta(seconds=60), maximum_attempts=0, non_retryable_error_types=['NotNullViolation', 'IntegrityError']))
        update_inputs = UpdateBatchExportBackfillStatusInputs(id=backfill_id, status='Completed')
        frequency_seconds = await temporalio.workflow.execute_activity(get_schedule_frequency, inputs.batch_export_id, start_to_close_timeout=dt.timedelta(minutes=1), retry_policy=temporalio.common.RetryPolicy(maximum_attempts=0))
        ...
        backfill_schedule_inputs = BackfillScheduleInputs(schedule_id=inputs.batch_export_id, start_at=inputs.start_at, end_at=inputs.end_at, frequency_seconds=frequency_seconds, buffer_limit=inputs.buffer_limit, wait_delay=inputs.wait_delay)
        try:
            await temporalio.workflow.execute_activity(backfill_schedule, backfill_schedule_inputs, retry_policy=temporalio.common.RetryPolicy(initial_interval=dt.timedelta(seconds=10), maximum_interval=dt.timedelta(seconds=60), non_retryable_error_types=['TemporalScheduleNotFoundError']), start_to_close_timeout=dt.timedelta(minutes=5 * number_of_expected_runs), heartbeat_timeout=dt.timedelta(minutes=2))
