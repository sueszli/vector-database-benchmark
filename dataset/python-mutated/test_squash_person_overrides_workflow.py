...
from posthog.models.person_overrides.sql import DROP_KAFKA_PERSON_OVERRIDES_TABLE_SQL, DROP_PERSON_OVERRIDES_CREATE_MATERIALIZED_VIEW_SQL, KAFKA_PERSON_OVERRIDES_TABLE_SQL, PERSON_OVERRIDES_CREATE_MATERIALIZED_VIEW_SQL, PERSON_OVERRIDES_CREATE_TABLE_SQL
from posthog.temporal.workflows.squash_person_overrides import QueryInputs, SerializablePersonOverrideToDelete, SquashPersonOverridesInputs, SquashPersonOverridesWorkflow, delete_squashed_person_overrides_from_clickhouse, delete_squashed_person_overrides_from_postgres, drop_dictionary, prepare_dictionary, prepare_person_overrides, select_persons_to_delete, squash_events_partition
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_prepare_dictionary_with_older_overrides_present(query_inputs, activity_environment, person_overrides_data, older_overrides):
...
async def test_select_persons_to_delete(query_inputs, activity_environment, person_overrides_data):
    ...
    expected = [SerializablePersonOverrideToDelete(team_id, person_override.old_person_id, person_override.override_person_id, OVERRIDES_CREATED_AT.isoformat(), 1, OLDEST_EVENT_AT.isoformat()) for (team_id, person_overrides) in person_overrides_data.items() for person_override in person_overrides]
    assert is_equal_sorted(to_delete, expected)

@pytest.mark.django_db
@pytest.mark.asyncio
async def test_select_persons_to_delete_selects_persons_in_older_partitions(query_inputs, activity_environment, person_overrides_data, older_overrides):
    ...
    to_delete = await activity_environment.run(select_persons_to_delete, query_inputs)
    expected = [SerializablePersonOverrideToDelete(team_id, person_override.old_person_id, person_override.override_person_id, OVERRIDES_CREATED_AT.isoformat(), 1, OLDEST_EVENT_AT.isoformat()) for (team_id, person_overrides) in person_overrides_data.items() for person_override in person_overrides]
    expected.extend([SerializablePersonOverrideToDelete(team_id, person_override.old_person_id, person_override.override_person_id, '2019-12-01T00:00:00+00:00', 1, '2019-12-01T00:00:00+00:00') for (team_id, person_overrides) in older_overrides.items() for person_override in person_overrides])
    assert is_equal_sorted(to_delete, expected)
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_select_persons_to_squash_with_different_partition(query_inputs, activity_environment, person_overrides_table):
...
async def test_select_persons_to_delete_with_newer_merges(query_inputs, activity_environment, person_overrides_data):
    ...
    for (team_id, person_override) in person_overrides_data.items():
        for (old_person_id, override_person_id) in person_override:
            override_person_id = uuid4()
            values: PersonOverrideValues = {'team_id': team_id, 'old_person_id': old_person_id, 'override_person_id': override_person_id, 'merged_at': OVERRIDES_CREATED_AT + timedelta(hours=1), 'oldest_event': OLDEST_EVENT_AT + timedelta(hours=1), 'created_at': OVERRIDES_CREATED_AT + timedelta(hours=1), 'version': 1}
            newer_overrides[team_id].add(PersonOverrideTuple(old_person_id, override_person_id))
            newer_values_to_insert.append(values)
    sync_execute('INSERT INTO person_overrides (*) VALUES', newer_values_to_insert)
    query_inputs.dry_run = False
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_squash_events_partition_dry_run(query_inputs, activity_environment, person_overrides_data, events_to_override):
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_squash_events_partition_with_older_overrides(query_inputs, activity_environment, person_overrides_data, events_to_override, older_overrides):
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_squash_events_partition_with_newer_overrides(query_inputs, activity_environment, person_overrides_data, events_to_override, newer_overrides):
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_squash_events_partition_with_limited_team_ids(query_inputs, activity_environment, person_overrides_data, events_to_override):
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_delete_squashed_person_overrides_from_clickhouse(query_inputs, activity_environment, person_overrides_data):
    ...
    delete.
    """
    persons_to_delete = [SerializablePersonOverrideToDelete(team_id, person_override.old_person_id, person_override.override_person_id, OVERRIDES_CREATED_AT.isoformat(), 1, OLDEST_EVENT_AT.isoformat()) for (team_id, person_overrides) in person_overrides_data.items() for person_override in person_overrides]
    query_inputs.partition_ids = ['202001']
    query_inputs.latest_created_at = OVERRIDES_CREATED_AT
    query_inputs.dry_run = False
    query_inputs.person_overrides_to_delete = persons_to_delete
    not_overriden_id = uuid4()
    not_overriden_person: PersonOverrideValues = {'team_id': 1, 'old_person_id': not_overriden_id, 'override_person_id': uuid4(), 'merged_at': OVERRIDES_CREATED_AT, 'oldest_event': OLDEST_EVENT_AT, 'created_at': OVERRIDES_CREATED_AT, 'version': 1}
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_delete_squashed_person_overrides_from_clickhouse_dry_run(query_inputs, activity_environment, person_overrides_data):
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_delete_squashed_person_overrides_from_postgres(query_inputs, activity_environment, team_id, person_overrides, pg_connection):
    """Test we can delete person overrides that have already been squashed.
    ...
    person_overrides_to_delete = [SerializablePersonOverrideToDelete(team_id, person_overrides.old_person_id, person_overrides.override_person_id, OVERRIDES_CREATED_AT.isoformat(), 1, OLDEST_EVENT_AT.isoformat())]
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_delete_squashed_person_overrides_from_postgres_dry_run(query_inputs, activity_environment, team_id, person_overrides, pg_connection):
    """Test we do not delete person overrides when dry_run=True."""
    with pg_connection:
        with pg_connection.cursor() as cursor:
            cursor.execute('SELECT id, team_id, uuid FROM posthog_personoverridemapping')
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_delete_squashed_person_overrides_from_postgres_with_newer_override(query_inputs, activity_environment, team_id, person_overrides, pg_connection):
    ...
    with pg_connection:
        with pg_connection.cursor() as cursor:
            cursor.execute('\n                UPDATE posthog_personoverride\n                SET version = version + 1\n                WHERE\n                    team_id = %(team_id)s\n                    AND old_person_id = %(old_person_id)s\n                ', {'team_id': team_id, 'old_person_id': [mapping[0] for mapping in mappings if mapping[2] == person_overrides.old_person_id][0]})
    person_overrides_to_delete = [SerializablePersonOverrideToDelete(team_id, person_overrides.old_person_id, person_overrides.override_person_id, OVERRIDES_CREATED_AT.isoformat(), 1, OLDEST_EVENT_AT.isoformat())]
    query_inputs.person_overrides_to_delete = person_overrides_to_delete
    query_inputs.dry_run = False
    await activity_environment.run(delete_squashed_person_overrides_from_postgres, query_inputs)
    with pg_connection:
        with pg_connection.cursor() as cursor:
            ...
            assert old_person_id == [mapping[0] for mapping in mappings if mapping[2] == person_overrides.old_person_id][0]
            assert override_person_id == [mapping[0] for mapping in mappings if mapping[2] == person_overrides.override_person_id][0]
            assert version == 2

@pytest.mark.django_db
@pytest.mark.asyncio
async def test_squash_person_overrides_workflow(query_inputs, events_to_override, person_overrides_data, person_overrides, person_overrides_table):
    ...
    inputs = SquashPersonOverridesInputs(partition_ids=['202001'], dry_run=False)
    async with Worker(client, task_queue=settings.TEMPORAL_TASK_QUEUE, workflows=[SquashPersonOverridesWorkflow], activities=[prepare_person_overrides, prepare_dictionary, select_persons_to_delete, squash_events_partition, drop_dictionary, delete_squashed_person_overrides_from_clickhouse, delete_squashed_person_overrides_from_postgres], workflow_runner=UnsandboxedWorkflowRunner()):
        await client.execute_workflow(SquashPersonOverridesWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE)
    assert_events_have_been_overriden(events_to_override, person_overrides_data)
    rows = sync_execute('SELECT team_id, old_person_id FROM person_overrides')
    assert len(rows) == 0
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_squash_person_overrides_workflow_with_newer_overrides(query_inputs, events_to_override, person_overrides_data, person_overrides, newer_overrides):
...
@pytest.mark.django_db
@pytest.mark.asyncio
async def test_squash_person_overrides_workflow_with_limited_team_ids(query_inputs, events_to_override, person_overrides_data, person_overrides):
    """Test the squash_person_overrides workflow end-to-end."""
    client = await Client.connect(f'{settings.TEMPORAL_HOST}:{settings.TEMPORAL_PORT}', namespace=settings.TEMPORAL_NAMESPACE)
    workflow_id = str(uuid4())
    random_team = random.choice(list(person_overrides_data.keys()))
    inputs = SquashPersonOverridesInputs(partition_ids=['202001'], team_ids=[random_team], dry_run=False)
    async with Worker(client, task_queue=settings.TEMPORAL_TASK_QUEUE, workflows=[SquashPersonOverridesWorkflow], activities=[prepare_person_overrides, prepare_dictionary, select_persons_to_delete, squash_events_partition, drop_dictionary, delete_squashed_person_overrides_from_clickhouse, delete_squashed_person_overrides_from_postgres], workflow_runner=UnsandboxedWorkflowRunner()):
        await client.execute_workflow(SquashPersonOverridesWorkflow.run, inputs, id=workflow_id, task_queue=settings.TEMPORAL_TASK_QUEUE)
    with pytest.raises(AssertionError):
        assert_events_have_been_overriden(events_to_override, person_overrides_data)
