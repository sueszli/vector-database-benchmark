...
class NovafileCom(XFSDownloader):
    ...
    __version__ = '0.13'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?novafile\\.(?:com|org)/(?:file/)?\\w{12}'
    ...
    __license__ = 'GPLv3'
    __authors__ = [('zoidberg', 'zoidberg@mujmail.cz'), ('stickell', 'l.stickell@yahoo.it')]
    PLUGIN_DOMAIN = 'novafile.com'
    ERROR_PATTERN = 'class="alert.+?alert-separate".*?>\\s*(?:<p>)?(.*?)\\s*</'
    WAIT_PATTERN = '<p>Please wait <span id="count".*?>(\\d+)</span> seconds</p>'
    DL_LIMIT_PATTERN = 'You have to wait (.+?) until the next download becomes available.'
    LINK_PATTERN = '<a href="(https://s\\d+\\.novafile\\.(?:com|org)/.*?)" class="btn btn-green">Download File</a>'
