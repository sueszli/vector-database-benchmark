...
if __name__ == '__main__':
    ...
    config_list = [{'op_types': ['Conv2d'], 'sparse_ratio': 0.5}, *[{'op_names': [name], 'target_names': ['_output_'], 'target_settings': {'_output_': {'align': {'module_name': name.replace('bn', 'conv') if 'bn' in name else name.replace('downsample.1', 'downsample.0'), 'target_name': 'weight', 'dims': [0]}, 'granularity': 'per_channel'}}} for name in bn_list]]
    ...
    config_list = [{'op_types': ['Conv2d'], 'lambda': 0.1, 'apply_method': 'mse'}]
