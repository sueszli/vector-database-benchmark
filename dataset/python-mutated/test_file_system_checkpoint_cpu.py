...
from torch.distributed._shard.sharding_spec import ChunkShardingSpec, EnumerableShardingSpec, ShardingSpec, ShardMetadata
...
from torch.testing._internal.distributed._shard.sharded_tensor._test_st_common import MyShardedModel1
...
class TestDistributedStateDictSaveLoadWithSharedTensor(ShardedTensorTestBase):
    ...
    def test_read_write_shard_tensor(self, thread_count) -> None:
        ...
        path = paths[0]
        spec = ChunkShardingSpec(dim=0, placements=['rank:0', 'rank:1'])
...
class TestDistributedReshardOnLoad(ShardedTensorTestBase):
    ...
    def test_load_with_different_shard_plan(self, thread_count) -> None:
        ...
        specs = [ChunkShardingSpec(dim=0, placements=['rank:0', 'rank:1']), ChunkShardingSpec(dim=0, placements=['rank:0', 'rank:1', 'rank:1', 'rank:0']), EnumerableShardingSpec(shards=[ShardMetadata(shard_offsets=[0, 0], shard_sizes=[2, 20], placement='rank:0'), ShardMetadata(shard_offsets=[2, 0], shard_sizes=[1, 20], placement='rank:1'), ShardMetadata(shard_offsets=[3, 0], shard_sizes=[3, 20], placement='rank:0'), ShardMetadata(shard_offsets=[6, 0], shard_sizes=[3, 20], placement='rank:1'), ShardMetadata(shard_offsets=[9, 0], shard_sizes=[1, 20], placement='rank:0')]), EnumerableShardingSpec(shards=[ShardMetadata(shard_offsets=[0, 0], shard_sizes=[8, 20], placement='rank:1'), ShardMetadata(shard_offsets=[8, 0], shard_sizes=[2, 20], placement='rank:0')])]
        for s0 in specs:
            for s1 in specs:
                if s0 == s1:
                    continue
    ...
    def test_load_rowwise_to_colwise(self, thread_count) -> None:
        ...
        path = self.get_file_path()
        self.assertEqual(self.world_size, dist.get_world_size())
        src_spec = ChunkShardingSpec(dim=0, placements=['rank:0', 'rank:1'])
        dst_spec = ChunkShardingSpec(dim=1, placements=['rank:0', 'rank:1'])
    ...
    def test_switch_between_sharded_tensor_to_tensor(self, thread_count) -> None:
        ...
        path = self.get_file_path()
        tensor_size = 32
        specs = [ChunkShardingSpec(dim=0, placements=['rank:0', 'rank:1']), ChunkShardingSpec(dim=0, placements=['rank:0', 'rank:1', 'rank:1', 'rank:0']), EnumerableShardingSpec(shards=[ShardMetadata(shard_offsets=[0], shard_sizes=[8], placement='rank:1'), ShardMetadata(shard_offsets=[8], shard_sizes=[tensor_size - 8], placement='rank:0')]), EnumerableShardingSpec(shards=[ShardMetadata(shard_offsets=[0], shard_sizes=[10], placement='rank:0'), ShardMetadata(shard_offsets=[10], shard_sizes=[tensor_size - 10], placement='rank:1')])]
        for save_spec in specs:
            for load_spec in specs:
                save_dict = {'sharded': sharded_tensor.rand(save_spec, tensor_size), 'replicated': torch.rand(tensor_size, device='cpu')}
                fs_writer = FileSystemWriter(path=path, thread_count=thread_count)
                save_state_dict(state_dict=save_dict, storage_writer=fs_writer)
                load_dict = {'sharded': torch.zeros(tensor_size, device='cpu'), 'replicated': sharded_tensor.zeros(load_spec, tensor_size)}
                fs_reader = FileSystemReader(path=path)
