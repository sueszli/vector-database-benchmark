...
def test_regex_rule__nosemgrep(run_semgrep_in_tmp: RunSemgrep, snapshot):
    if False:
        print('Hello World!')
    snapshot.assert_match(run_semgrep_in_tmp('rules/regex-nosemgrep.yaml', target_name='basic/regex-nosemgrep.txt').stdout, 'results.json')
...
def test_nosem_rule__invalid_id(run_semgrep_in_tmp: RunSemgrep, snapshot):
    if False:
        while True:
            i = 10
    (stdout, stderr) = run_semgrep_in_tmp('rules/nosem.yaml', target_name='nosem_invalid_id', assert_exit_code=2)
...
def test_nosem_rule__with_disable_nosem(run_semgrep_in_tmp: RunSemgrep, snapshot):
    if False:
        return 10
    snapshot.assert_match(run_semgrep_in_tmp('rules/nosem.yaml', options=['--disable-nosem']).stdout, 'results.json')