...
class ColumnsSheet(Sheet):
    ...
    columns = [ColumnAttr('sheet', type=str), ColumnAttr('name', help='rename the column on the source sheet'), ColumnAttr('keycol', type=int, width=0), ColumnAttr('width', type=int, help='set the column width (`0` to hide completely)'), ColumnAttr('height', type=int, max_help=1, help='set a maximum height for the row, if this column will fill it'), ColumnAttr('hoffset', type=int, width=0), ColumnAttr('voffset', type=int, width=0), ColumnAttr('type', 'typestr', help='convert all values to a specific type'), ColumnAttr('fmtstr', help='use a custom format string, either C-style (`%0.4f`) or Python-style (`{{:0.4f}}`)'), ColumnAttr('formatter', max_help=1, help='use a custom format function (**{col.help_formatters}**)'), ColumnAttr('displayer', max_help=1, help='use a custom display function (**{col.help_displayers}**)'), ValueColumn('value', help='change the value of this cell on the source sheet'), ColumnAttr('expr', max_help=1, help='change the main column parameter'), ColumnAttr('ncalcs', type=int, width=0, cache=False), ColumnAttr('maxtime', type=float, width=0, cache=False), ColumnAttr('totaltime', type=float, width=0, cache=False)]
    nKeys = 2
    colorizers = [RowColorizer(7, 'color_key_col', lambda s, c, r, v: r and r.keycol), RowColorizer(8, 'color_hidden_col', lambda s, c, r, v: r and r.hidden)]
...
vd._lastInputs = collections.defaultdict(dict)

class LastInputsSheet(JsonLinesSheet):
    columns = [ColumnItem('type'), ColumnItem('input')]
...
vd.addGlobals({'ColumnsSheet': ColumnsSheet, 'MetaSheet': MetaSheet, 'VisiDataMetaSheet': VisiDataMetaSheet})
