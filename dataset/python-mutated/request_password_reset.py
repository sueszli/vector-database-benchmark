...
class RequestPasswordReset(BaseMutation):
    ...
    class Arguments:
        ...
        redirect_url = graphene.String(required=True, description='URL of a view where users should be redirected to reset the password. URL in RFC 1808 format.')
        channel = graphene.String(description='Slug of a channel which will be used for notify user. Optional when only one channel exists.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.NOTIFY_USER, description='A notification for password reset.'), WebhookEventInfo(type=WebhookEventAsyncType.ACCOUNT_SET_PASSWORD_REQUESTED, description='Setting a new password for the account is requested.'), WebhookEventInfo(type=WebhookEventAsyncType.STAFF_SET_PASSWORD_REQUESTED, description='Setting a new password for the staff account is requested.')]
