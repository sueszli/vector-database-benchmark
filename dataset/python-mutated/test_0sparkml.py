...
if sys.platform == 'darwin' or 'nt' in os.name:
    ...
    try:
        ...
        spark = pyspark.sql.SparkSession.builder.appName('MyApp').master('local[2]').config('spark.jars.packages', f'com.microsoft.azure:synapseml_2.12:0.11.3{postfix_version}org.apache.hadoop:hadoop-azure:3.3.5,com.microsoft.azure:azure-storage:8.6.6,org.mlflow:mlflow-spark:{mlflow.__version__}').config('spark.jars.repositories', 'https://mmlspark.azureedge.net/maven').config('spark.sql.debug.maxToStringFields', '100').config('spark.driver.extraJavaOptions', '-Xss1m').config('spark.executor.extraJavaOptions', '-Xss1m').getOrCreate()
        spark.sparkContext._conf.set('spark.mlflow.pysparkml.autolog.logModelAllowlistFile', 'https://mmlspark.blob.core.windows.net/publicwasb/log_model_allowlist.txt')
