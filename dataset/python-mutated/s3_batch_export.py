...
from posthog.temporal.workflows.batch_exports import BatchExportTemporaryFile, CreateBatchExportRunInputs, UpdateBatchExportRunStatusInputs, create_export_run, execute_batch_export_insert_activity, get_data_interval, get_results_iterator, get_rows_count
...
class S3MultiPartUpload:
    ...
    async def s3_client(self):
        ...
        async with self._session.client('s3', region_name=self.region_name, aws_access_key_id=self.aws_access_key_id, aws_secret_access_key=self.aws_secret_access_key) as client:
            yield client
    ...
    async def start(self) -> str:
        ...
        async with self.s3_client() as s3_client:
            multipart_response = await s3_client.create_multipart_upload(Bucket=self.bucket_name, Key=self.key, **optional_kwargs)
    ...
    async def complete(self) -> str:
        if self.is_upload_in_progress() is False:
            raise NoUploadInProgressError()
        async with self.s3_client() as s3_client:
            response = await s3_client.complete_multipart_upload(Bucket=self.bucket_name, Key=self.key, UploadId=self.upload_id, MultipartUpload={'Parts': self.parts})
        self.upload_id = None
        self.parts = []
    ...
    async def abort(self):
        ...
        if self.is_upload_in_progress() is False:
            raise NoUploadInProgressError()
        async with self.s3_client() as s3_client:
            await s3_client.abort_multipart_upload(Bucket=self.bucket_name, Key=self.key, UploadId=self.upload_id)
        self.upload_id = None
        self.parts = []
    ...
    async def upload_part(self, body: BatchExportTemporaryFile, rewind: bool=True):
        ...
        if rewind is True:
            body.rewind()
        reader = io.BufferedReader(body)
        async with self.s3_client() as s3_client:
            response = await s3_client.upload_part(Bucket=self.bucket_name, Key=self.key, PartNumber=next_part_number, UploadId=self.upload_id, Body=reader)
        reader.detach()
...
async def initialize_and_resume_multipart_upload(inputs: S3InsertInputs) -> tuple[S3MultiPartUpload, str]:
    ...
    s3_upload = S3MultiPartUpload(bucket_name=inputs.bucket_name, key=key, encryption=inputs.encryption, kms_key_id=inputs.kms_key_id, region_name=inputs.region, aws_access_key_id=inputs.aws_access_key_id, aws_secret_access_key=inputs.aws_secret_access_key)
    details = activity.info().heartbeat_details
    try:
        (interval_start, upload_state) = HeartbeatDetails.from_activity_details(details)
    except IndexError:
        interval_start = inputs.data_interval_start
        logger.debug('Did not receive details from previous activity Excecution. Export will start from the beginning %s', interval_start)
    except Exception:
        interval_start = inputs.data_interval_start
        logger.warning('Did not receive details from previous activity Excecution due to an unexpected error. Export will start from the beginning %s', interval_start)
    else:
        logger.info('Received details from previous activity. Export will attempt to resume from %s', interval_start)
        s3_upload.continue_from_state(upload_state)
        if inputs.compression == 'brotli':
            interval_start = inputs.data_interval_start
            logger.info(f'Export will start from the beginning as we are using brotli compression: %s', interval_start)
...
async def insert_into_s3_activity(inputs: S3InsertInputs):
    ...
    async with get_client() as client:
        ...
        results_iterator = get_results_iterator(client=client, team_id=inputs.team_id, interval_start=interval_start, interval_end=inputs.data_interval_end, exclude_events=inputs.exclude_events, include_events=inputs.include_events, include_person_properties=True)
        result = None
        ...
        async def worker_shutdown_handler():
            ...
            logger.warn(f'Worker shutting down! Reporting back latest exported part {last_uploaded_part_timestamp}')
        ...
        async with s3_upload as s3_upload:
            with BatchExportTemporaryFile(compression=inputs.compression) as local_results_file:
                ...
                async def flush_to_s3(last_uploaded_part_timestamp: str, last=False):
                    logger.debug('Uploading %spart %s containing %s records with size %s bytes', 'last ' if last else '', s3_upload.part_number + 1, local_results_file.records_since_last_reset, local_results_file.bytes_since_last_reset)
                ...
                for result in results_iterator:
                    record = {'created_at': result['created_at'], 'distinct_id': result['distinct_id'], 'elements_chain': result['elements_chain'], 'event': result['event'], 'inserted_at': result['inserted_at'], 'person_id': result['person_id'], 'person_properties': result['person_properties'], 'properties': result['properties'], 'timestamp': result['timestamp'], 'uuid': result['uuid']}
...
class S3BatchExportWorkflow(PostHogWorkflow):
    ...
    async def run(self, inputs: S3BatchExportInputs):
        ...
        insert_inputs = S3InsertInputs(bucket_name=inputs.bucket_name, region=inputs.region, prefix=inputs.prefix, team_id=inputs.team_id, aws_access_key_id=inputs.aws_access_key_id, aws_secret_access_key=inputs.aws_secret_access_key, data_interval_start=data_interval_start.isoformat(), data_interval_end=data_interval_end.isoformat(), compression=inputs.compression, exclude_events=inputs.exclude_events, include_events=inputs.include_events, encryption=inputs.encryption, kms_key_id=inputs.kms_key_id)
        await execute_batch_export_insert_activity(insert_into_s3_activity, insert_inputs, non_retryable_error_types=['ParamValidationError', 'ClientError'], update_inputs=update_inputs)