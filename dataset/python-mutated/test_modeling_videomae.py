...
if is_torch_available():
    ...
    from transformers import MODEL_FOR_VIDEO_CLASSIFICATION_MAPPING, VideoMAEForPreTraining, VideoMAEForVideoClassification, VideoMAEModel
...
class VideoMAEModelTester:
    ...
    def __init__(self, parent, batch_size=13, image_size=10, num_channels=3, patch_size=2, tubelet_size=2, num_frames=2, is_training=True, use_labels=True, hidden_size=32, num_hidden_layers=2, num_attention_heads=4, intermediate_size=37, hidden_act='gelu', hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, type_sequence_label_size=10, initializer_range=0.02, mask_ratio=0.9, scope=None):
        if False:
            i = 10
            return i + 15
        self.parent = parent
...
class VideoMAEModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    """
    all_model_classes = (VideoMAEModel, VideoMAEForPreTraining, VideoMAEForVideoClassification) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': VideoMAEModel, 'video-classification': VideoMAEForVideoClassification} if is_torch_available() else {}
    test_pruning = False
    ...
    def test_attention_outputs(self):
        ...
        if not self.has_attentions:
            ...
            for model_class in self.all_model_classes:
                ...
                seq_len = num_visible_patches if model_class == VideoMAEForPreTraining else self.model_tester.seq_length
...
class VideoMAEModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_for_video_classification(self):
        if False:
            i = 10
            return i + 15
        model = VideoMAEForVideoClassification.from_pretrained('MCG-NJU/videomae-base-finetuned-kinetics').to(torch_device)
    ...
    def test_inference_for_pretraining(self):
        ...
        expected_slice = torch.tensor([[0.7994, 0.9612, 0.8508], [0.7401, 0.8958, 0.8302], [0.5862, 0.7468, 0.7325]], device=torch_device)
        self.assertEqual(outputs.logits.shape, expected_shape)
        ...
        self.assertTrue(torch.allclose(outputs.loss, expected_loss, atol=0.0001))
        model = VideoMAEForPreTraining.from_pretrained('MCG-NJU/videomae-base-short', norm_pix_loss=False).to(torch_device)
        with torch.no_grad():
            outputs = model(**inputs)
