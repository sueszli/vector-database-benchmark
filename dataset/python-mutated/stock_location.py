...
class Location(models.Model):
    ...
    location_id = fields.Many2one('stock.location', 'Parent Location', index=True, ondelete='cascade', help="The parent location that includes this location. Example : The 'Dispatch Zone' is the 'Gate 1' parent location.")
    ...
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env['res.company']._company_default_get('stock.location'), index=True, help='Let this field empty if this location is shared between companies')
...
class Route(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env['res.company']._company_default_get('stock.location.route'), index=True, help='Leave this field empty if this route is shared between all companies')
...
class PushedFlow(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env['res.company']._company_default_get('procurement.order'), index=True)
    route_id = fields.Many2one('stock.location.route', 'Route')
    location_from_id = fields.Many2one('stock.location', 'Source Location', index=True, ondelete='cascade', required=True, help='This rule can be applied when a move is confirmed that has this location as destination location')
    location_dest_id = fields.Many2one('stock.location', 'Destination Location', index=True, ondelete='cascade', required=True, help='The new location where the goods need to go')
    delay = fields.Integer('Delay (days)', default=0, help='Number of days needed to transfer the goods')
    picking_type_id = fields.Many2one('stock.picking.type', 'Picking Type', required=True, help='This is the picking type that will be put on the stock moves')
    auto = fields.Selection([('manual', 'Manual Operation'), ('transparent', 'Automatic No Step Added')], string='Automatic Move', default='manual', index=True, required=True, help="The 'Manual Operation' value will create a stock move after the current one.With 'Automatic No Step Added', the location is replaced in the original move.")
