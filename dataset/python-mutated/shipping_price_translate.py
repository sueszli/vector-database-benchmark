...
class ShippingPriceTranslationInput(NameTranslationInput):
    description = JSONString(description='Translated shipping method description.' + RICH_CONTENT)

class ShippingPriceTranslate(BaseTranslateMutation):

    class Arguments:
        id = graphene.ID(required=True, description='ShippingMethodType ID or ShippingMethodTranslatableContent ID.')
        language_code = graphene.Argument(LanguageCodeEnum, required=True, description='Translation language code.')
        input = ShippingPriceTranslationInput(required=True, description='Fields required to update shipping price translations.')
