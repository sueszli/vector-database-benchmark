...
class InvoiceRequestDelete(ModelMutation):
    ...
    class Arguments:
        id = graphene.ID(required=True, description='ID of an invoice to request the deletion.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.INVOICE_DELETED, description='An invoice was requested to delete.')]
    ...
    def perform_mutation(cls, _root, info: ResolveInfo, /, *, id):
        ...
        events.invoice_requested_deletion_event(user=info.context.user, app=app, invoice=invoice)
