...
obs_space = 5
action_space = 4
cfg = dict(cuda=True, on_policy=False, priority=True, priority_IS_weight=True, model=dict(obs_shape=obs_space, action_shape=action_space, encoder_hidden_size_list=[128, 128, 512]), discount_factor=0.99, burnin_step=2, nstep=5, learn_unroll_len=20, burning_step=5, learn=dict(value_rescale=True, update_per_collect=8, batch_size=64, learning_rate=0.0005, target_update_theta=0.001, lambda1=1.0, lambda2=1.0, lambda3=1e-05, lambda_one_step_td=1, margin_function=0.8, per_train_iter_k=0, ignore_done=False), collect=dict(n_sample=32, traj_len_inf=True, env_num=8, pho=1 / 4), eval=dict(env_num=8), other=dict(eps=dict(type='exp', start=0.95, end=0.1, decay=100000), replay_buffer=dict(replay_buffer_size=int(10000.0), alpha=0.6, beta=0.4)))
cfg = EasyDict(cfg)
