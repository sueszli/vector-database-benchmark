...
__protobuf__ = proto.module(package='google.cloud.osconfig.v1alpha', manifest={'InstanceOSPoliciesCompliance', 'GetInstanceOSPoliciesComplianceRequest', 'ListInstanceOSPoliciesCompliancesRequest', 'ListInstanceOSPoliciesCompliancesResponse'})
...
class InstanceOSPoliciesCompliance(proto.Message):
    ...
    class OSPolicyCompliance(proto.Message):
        ...
        os_policy_assignment: str = proto.Field(proto.STRING, number=2)
        state: config_common.OSPolicyComplianceState = proto.Field(proto.ENUM, number=4, enum=config_common.OSPolicyComplianceState)
        os_policy_resource_compliances: MutableSequence[config_common.OSPolicyResourceCompliance] = proto.RepeatedField(proto.MESSAGE, number=5, message=config_common.OSPolicyResourceCompliance)
    name: str = proto.Field(proto.STRING, number=1)
    instance: str = proto.Field(proto.STRING, number=2)
    state: config_common.OSPolicyComplianceState = proto.Field(proto.ENUM, number=3, enum=config_common.OSPolicyComplianceState)
    detailed_state: str = proto.Field(proto.STRING, number=4)
    detailed_state_reason: str = proto.Field(proto.STRING, number=5)
    os_policy_compliances: MutableSequence[OSPolicyCompliance] = proto.RepeatedField(proto.MESSAGE, number=6, message=OSPolicyCompliance)
    last_compliance_check_time: timestamp_pb2.Timestamp = proto.Field(proto.MESSAGE, number=7, message=timestamp_pb2.Timestamp)
    last_compliance_run_id: str = proto.Field(proto.STRING, number=8)

class GetInstanceOSPoliciesComplianceRequest(proto.Message):
    """A request message for getting OS policies compliance data for
...
class ListInstanceOSPoliciesCompliancesRequest(proto.Message):
    """A request message for listing OS policies compliance data for
...
class ListInstanceOSPoliciesCompliancesResponse(proto.Message):
    """A response message for listing OS policies compliance data
    ...
    def raw_page(self):
        if False:
            print('Hello World!')
        return self
    instance_os_policies_compliances: MutableSequence['InstanceOSPoliciesCompliance'] = proto.RepeatedField(proto.MESSAGE, number=1, message='InstanceOSPoliciesCompliance')
