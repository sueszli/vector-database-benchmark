...
class SwaggerUIAPI(AbstractSpecAPI):
    ...
    async def _get_openapi_json(self, request):
        jsonifier = Jsonifier()
        return StarletteResponse(content=jsonifier.dumps(self._spec_for_prefix(request)), status_code=200, media_type='application/json')

    async def _get_openapi_yaml(self, request):
        return StarletteResponse(content=yamldumper(self._spec_for_prefix(request)), status_code=200, media_type='text/yaml')
    ...
    async def _get_swagger_ui_home(self, req):
        ...
        template_variables = {'request': req, 'openapi_spec_url': base_path + self.options.openapi_spec_path, **self.options.swagger_ui_template_arguments}
        if self.options.swagger_ui_config:
    ...
    async def _get_swagger_ui_config(self, request):
        return StarletteResponse(status_code=200, media_type='application/json', content=json.dumps(self.options.swagger_ui_config))
