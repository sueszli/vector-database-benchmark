...
class TestWasbBlobSensor:
    _config = {'container_name': 'container', 'blob_name': 'blob', 'wasb_conn_id': 'conn_id', 'timeout': 100}
...
class TestWasbBlobAsyncSensor:
    ...
    SENSOR = WasbBlobSensor(task_id='wasb_blob_async_sensor', container_name=TEST_DATA_STORAGE_CONTAINER_NAME, blob_name=TEST_DATA_STORAGE_BLOB_NAME, timeout=5, deferrable=True)
...
class TestWasbPrefixSensor:
    _config = {'container_name': 'container', 'prefix': 'prefix', 'wasb_conn_id': 'conn_id', 'timeout': 100}
...
class TestWasbPrefixAsyncSensor:
    ...
    def create_context(self, task, dag=None):
        ...
        task_instance.xcom_push = mock.Mock()
        return {'dag': dag, 'ts': execution_date.isoformat(), 'task': task, 'ti': task_instance, 'task_instance': task_instance, 'run_id': dag_run.run_id, 'dag_run': dag_run, 'execution_date': execution_date, 'data_interval_end': execution_date, 'logical_date': execution_date}
    SENSOR = WasbPrefixSensor(task_id='wasb_prefix_sensor_async', container_name=TEST_DATA_STORAGE_CONTAINER_NAME, prefix=TEST_DATA_STORAGE_BLOB_PREFIX, deferrable=True)
