...
import pytest
from homeassistant.components.binary_sensor import DOMAIN as BINARY_SENSOR_DOMAIN, BinarySensorDeviceClass, BinarySensorEntityDescription
from homeassistant.components.bluetooth import DOMAIN, FALLBACK_MAXIMUM_STALE_ADVERTISEMENT_SECONDS, BluetoothChange, BluetoothScanningMode, BluetoothServiceInfoBleak
from homeassistant.components.bluetooth.const import UNAVAILABLE_TRACK_SECONDS
from homeassistant.components.bluetooth.passive_update_processor import STORAGE_KEY, PassiveBluetoothDataProcessor, PassiveBluetoothDataUpdate, PassiveBluetoothEntityKey, PassiveBluetoothProcessorCoordinator, PassiveBluetoothProcessorEntity
from homeassistant.components.sensor import DOMAIN as SENSOR_DOMAIN, SensorDeviceClass, SensorEntityDescription
...
from . import inject_bluetooth_service_info, inject_bluetooth_service_info_bleak, patch_all_discovered_devices
from tests.common import MockConfigEntry, MockEntityPlatform, async_fire_time_changed, async_test_home_assistant
...
async def test_basic_usage(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    ...
    coordinator = PassiveBluetoothProcessorCoordinator(hass, _LOGGER, 'aa:bb:cc:dd:ee:ff', BluetoothScanningMode.ACTIVE, _mock_update_method)
    ...
    cancel_async_add_entity_key_listener = processor.async_add_entity_key_listener(_async_entity_key_listener, entity_key)
    ...
    cancel_listener = processor.async_add_listener(_all_listener)
    cancel_async_add_entities_listener = processor.async_add_entities_listener(mock_entity, mock_add_entities)
...
async def test_entity_key_is_dispatched_on_entity_key_change(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
...
async def test_unavailable_after_no_data(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    """Test that the coordinator is unavailable after no data for a while."""
    start_monotonic = time.monotonic()
    with patch('bleak.BleakScanner.discovered_devices_and_advertisement_data', {'44:44:33:11:23:45': (MagicMock(address='44:44:33:11:23:45'), MagicMock())}):
        await async_setup_component(hass, DOMAIN, {DOMAIN: {}})
    ...
    mock_entity = MagicMock()
    mock_add_entities = MagicMock()
    processor.async_add_entities_listener(mock_entity, mock_add_entities)
    assert coordinator.available is False
    assert processor.available is False
    now = time.monotonic()
    service_info_at_time = BluetoothServiceInfoBleak(name='Generic', address='aa:bb:cc:dd:ee:ff', rssi=-95, manufacturer_data={1: b'\x01\x01\x01\x01\x01\x01\x01\x01'}, service_data={}, service_uuids=[], source='local', time=now, device=MagicMock(), advertisement=MagicMock(), connectable=True)
    ...
    with patch('homeassistant.components.bluetooth.manager.MONOTONIC_TIME', return_value=monotonic_now), patch_all_discovered_devices([MagicMock(address='44:44:33:11:23:45')]):
        async_fire_time_changed(hass, dt_util.utcnow() + timedelta(seconds=UNAVAILABLE_TRACK_SECONDS))
        await hass.async_block_till_done()
    assert coordinator.available is False
    assert processor.available is False
...
async def test_no_updates_once_stopping(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    ...
    def _all_listener(data: PassiveBluetoothDataUpdate | None) -> None:
        ...
        'Mock an all listener.'
        all_events.append(data)
    processor.async_add_listener(_all_listener)
    inject_bluetooth_service_info(hass, GENERIC_BLUETOOTH_SERVICE_INFO)
    assert len(all_events) == 1
...
async def test_exception_from_update_method(hass: HomeAssistant, caplog: pytest.LogCaptureFixture, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    ...
    def _async_register_callback(_hass, _callback, _matcher, _mode):
        ...
        return lambda : None
    processor = PassiveBluetoothDataProcessor(_async_generate_mock_data)
    with patch('homeassistant.components.bluetooth.update_coordinator.async_register_callback', _async_register_callback):
        unregister_processor = coordinator.async_register_processor(processor)
        cancel_coordinator = coordinator.async_start()
...
async def test_bad_data_from_update_method(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
...
async def test_integration_with_entity(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    ...
    processor.async_add_listener(MagicMock())
    mock_add_entities = MagicMock()
    processor.async_add_entities_listener(PassiveBluetoothProcessorEntity, mock_add_entities)
    entity_key_events = []
    ...
    def _async_entity_key_listener(data: PassiveBluetoothDataUpdate | None) -> None:
        ...
        'Mock entity key listener.'
        entity_key_events.append(data)
    cancel_async_add_entity_key_listener = processor.async_add_entity_key_listener(_async_entity_key_listener, PassiveBluetoothEntityKey(key='humidity', device_id='primary'))
    inject_bluetooth_service_info(hass, GENERIC_BLUETOOTH_SERVICE_INFO)
    assert len(mock_add_entities.mock_calls) == 1
    assert len(entity_key_events) == 1
    inject_bluetooth_service_info(hass, GENERIC_BLUETOOTH_SERVICE_INFO_2)
    ...
    assert len(mock_add_entities.mock_calls) == 2
    assert len(entity_key_events) == 2
    entities = [*mock_add_entities.mock_calls[0][1][0], *mock_add_entities.mock_calls[1][1][0]]
    ...
    assert entity_one.device_info == {'identifiers': {('bluetooth', 'aa:bb:cc:dd:ee:ff-remote')}, 'manufacturer': 'Govee', 'model': 'H5178-REMOTE', 'name': 'B5178D6FB Remote'}
    assert entity_one.entity_key == PassiveBluetoothEntityKey(key='temperature', device_id='remote')
    cancel_async_add_entity_key_listener()
    cancel_coordinator()
NO_DEVICES_BLUETOOTH_SERVICE_INFO = BluetoothServiceInfo(name='Generic', address='aa:bb:cc:dd:ee:ff', rssi=-95, manufacturer_data={1: b'\x01\x01\x01\x01\x01\x01\x01\x01'}, service_data={}, service_uuids=[], source='local')
NO_DEVICES_BLUETOOTH_SERVICE_INFO_2 = BluetoothServiceInfo(name='Generic', address='aa:bb:cc:dd:ee:ff', rssi=-95, manufacturer_data={1: b'\x01\x01\x01\x01\x01\x01\x01\x01', 2: b'\x02'}, service_data={}, service_uuids=[], source='local')
NO_DEVICES_PASSIVE_BLUETOOTH_DATA_UPDATE = PassiveBluetoothDataUpdate(devices={}, entity_data={PassiveBluetoothEntityKey('temperature', None): 14.5, PassiveBluetoothEntityKey('pressure', None): 1234}, entity_descriptions={PassiveBluetoothEntityKey('temperature', None): SensorEntityDescription(key='temperature', name='Temperature', native_unit_of_measurement=UnitOfTemperature.CELSIUS, device_class=SensorDeviceClass.TEMPERATURE), PassiveBluetoothEntityKey('pressure', None): SensorEntityDescription(key='pressure', name='Pressure', native_unit_of_measurement='hPa', device_class=SensorDeviceClass.PRESSURE)})

async def test_integration_with_entity_without_a_device(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    ...
    assert entity_one.device_info == {'identifiers': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'connections': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'name': 'Generic'}
    assert entity_one.entity_key == PassiveBluetoothEntityKey(key='temperature', device_id=None)
    cancel_coordinator()

async def test_passive_bluetooth_entity_with_entity_platform(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    ...
    coordinator.async_register_processor(processor)
    cancel_coordinator = coordinator.async_start()
    processor.async_add_entities_listener(PassiveBluetoothProcessorEntity, lambda entities: hass.async_create_task(entity_platform.async_add_entities(entities)))
    inject_bluetooth_service_info(hass, NO_DEVICES_BLUETOOTH_SERVICE_INFO)
    await hass.async_block_till_done()
    inject_bluetooth_service_info(hass, NO_DEVICES_BLUETOOTH_SERVICE_INFO_2)
    await hass.async_block_till_done()
    assert hass.states.get('test_domain.test_platform_aa_bb_cc_dd_ee_ff_temperature') is not None
    assert hass.states.get('test_domain.test_platform_aa_bb_cc_dd_ee_ff_pressure') is not None
    cancel_coordinator()
SENSOR_PASSIVE_BLUETOOTH_DATA_UPDATE = PassiveBluetoothDataUpdate(devices={None: DeviceInfo(name='Test Device', model='Test Model', manufacturer='Test Manufacturer')}, entity_data={PassiveBluetoothEntityKey('pressure', None): 1234}, entity_names={PassiveBluetoothEntityKey('pressure', None): 'Pressure'}, entity_descriptions={PassiveBluetoothEntityKey('pressure', None): SensorEntityDescription(key='pressure', native_unit_of_measurement='hPa', device_class=SensorDeviceClass.PRESSURE)})
BINARY_SENSOR_PASSIVE_BLUETOOTH_DATA_UPDATE = PassiveBluetoothDataUpdate(devices={None: DeviceInfo(name='Test Device', model='Test Model', manufacturer='Test Manufacturer')}, entity_data={PassiveBluetoothEntityKey('motion', None): True}, entity_names={PassiveBluetoothEntityKey('motion', None): 'Motion'}, entity_descriptions={PassiveBluetoothEntityKey('motion', None): BinarySensorEntityDescription(key='motion', device_class=BinarySensorDeviceClass.MOTION)})
DEVICE_ONLY_PASSIVE_BLUETOOTH_DATA_UPDATE = PassiveBluetoothDataUpdate(devices={None: DeviceInfo(name='Test Device', model='Test Model', manufacturer='Test Manufacturer')}, entity_data={}, entity_names={}, entity_descriptions={})

async def test_integration_multiple_entity_platforms(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    ...
    coordinator = PassiveBluetoothProcessorCoordinator(hass, _LOGGER, 'aa:bb:cc:dd:ee:ff', BluetoothScanningMode.ACTIVE, _mock_update_method)
    assert coordinator.available is False
    binary_sensor_processor = PassiveBluetoothDataProcessor(lambda service_info: BINARY_SENSOR_PASSIVE_BLUETOOTH_DATA_UPDATE)
    sensor_processor = PassiveBluetoothDataProcessor(lambda service_info: SENSOR_PASSIVE_BLUETOOTH_DATA_UPDATE)
    ...
    sensor_processor.async_add_entities_listener(PassiveBluetoothProcessorEntity, mock_add_sensor_entities)
    binary_sensor_processor.async_add_entities_listener(PassiveBluetoothProcessorEntity, mock_add_binary_sensor_entities)
    inject_bluetooth_service_info(hass, GENERIC_BLUETOOTH_SERVICE_INFO)
    assert len(mock_add_binary_sensor_entities.mock_calls) == 1
    assert len(mock_add_sensor_entities.mock_calls) == 1
    binary_sensor_entities = [*mock_add_binary_sensor_entities.mock_calls[0][1][0]]
    sensor_entities = [*mock_add_sensor_entities.mock_calls[0][1][0]]
    ...
    assert sensor_entity_one.device_info == {'identifiers': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'connections': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'manufacturer': 'Test Manufacturer', 'model': 'Test Model', 'name': 'Test Device'}
    assert sensor_entity_one.entity_key == PassiveBluetoothEntityKey(key='pressure', device_id=None)
    binary_sensor_entity_one: PassiveBluetoothProcessorEntity = binary_sensor_entities[0]
    ...
    assert binary_sensor_entity_one.device_info == {'identifiers': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'connections': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'manufacturer': 'Test Manufacturer', 'model': 'Test Model', 'name': 'Test Device'}
    assert binary_sensor_entity_one.entity_key == PassiveBluetoothEntityKey(key='motion', device_id=None)
    cancel_coordinator()

async def test_exception_from_coordinator_update_method(hass: HomeAssistant, caplog: pytest.LogCaptureFixture, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    """Test we handle exceptions from the update method."""
    await async_setup_component(hass, DOMAIN, {DOMAIN: {}})
    run_count = 0
...
async def test_integration_multiple_entity_platforms_with_reload_and_restart(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None, hass_storage: dict[str, Any]) -> None:
    ...
    coordinator = PassiveBluetoothProcessorCoordinator(hass, _LOGGER, 'aa:bb:cc:dd:ee:ff', BluetoothScanningMode.ACTIVE, _mock_update_method)
    assert coordinator.available is False
    binary_sensor_processor = PassiveBluetoothDataProcessor(lambda service_info: BINARY_SENSOR_PASSIVE_BLUETOOTH_DATA_UPDATE, BINARY_SENSOR_DOMAIN)
    sensor_processor = PassiveBluetoothDataProcessor(lambda service_info: SENSOR_PASSIVE_BLUETOOTH_DATA_UPDATE, SENSOR_DOMAIN)
    unregister_binary_sensor_processor = coordinator.async_register_processor(binary_sensor_processor, BinarySensorEntityDescription)
    unregister_sensor_processor = coordinator.async_register_processor(sensor_processor, SensorEntityDescription)
    cancel_coordinator = coordinator.async_start()
    binary_sensor_processor.async_add_listener(MagicMock())
    sensor_processor.async_add_listener(MagicMock())
    mock_add_sensor_entities = MagicMock()
    ...
    current_entry.set(entry)
    coordinator = PassiveBluetoothProcessorCoordinator(hass, _LOGGER, 'aa:bb:cc:dd:ee:ff', BluetoothScanningMode.ACTIVE, _mock_update_method)
    binary_sensor_processor = PassiveBluetoothDataProcessor(lambda service_info: DEVICE_ONLY_PASSIVE_BLUETOOTH_DATA_UPDATE, BINARY_SENSOR_DOMAIN)
    sensor_processor = PassiveBluetoothDataProcessor(lambda service_info: DEVICE_ONLY_PASSIVE_BLUETOOTH_DATA_UPDATE, SENSOR_DOMAIN)
    sensor_processor.async_add_entities_listener(PassiveBluetoothProcessorEntity, mock_add_sensor_entities)
    binary_sensor_processor.async_add_entities_listener(PassiveBluetoothProcessorEntity, mock_add_binary_sensor_entities)
    unregister_binary_sensor_processor = coordinator.async_register_processor(binary_sensor_processor, BinarySensorEntityDescription)
    unregister_sensor_processor = coordinator.async_register_processor(sensor_processor, SensorEntityDescription)
    ...
    sensor_entity_one: PassiveBluetoothProcessorEntity = sensor_entities[0]
    sensor_entity_one.hass = hass
    assert sensor_entity_one.available is False
    assert sensor_entity_one.unique_id == 'aa:bb:cc:dd:ee:ff-pressure'
    assert sensor_entity_one.device_info == {'identifiers': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'connections': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'manufacturer': 'Test Manufacturer', 'model': 'Test Model', 'name': 'Test Device'}
    assert sensor_entity_one.entity_key == PassiveBluetoothEntityKey(key='pressure', device_id=None)
    binary_sensor_entity_one: PassiveBluetoothProcessorEntity = binary_sensor_entities[0]
    binary_sensor_entity_one.hass = hass
    assert binary_sensor_entity_one.available is False
    assert binary_sensor_entity_one.unique_id == 'aa:bb:cc:dd:ee:ff-motion'
    assert binary_sensor_entity_one.device_info == {'identifiers': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'connections': {('bluetooth', 'aa:bb:cc:dd:ee:ff')}, 'manufacturer': 'Test Manufacturer', 'model': 'Test Model', 'name': 'Test Device'}
    assert binary_sensor_entity_one.entity_key == PassiveBluetoothEntityKey(key='motion', device_id=None)
    cancel_coordinator()
