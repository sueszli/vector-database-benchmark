...
class Test_cloudwatch_log_metric_filter_sign_in_without_mfa:
    ...
    def test_cloudwatch_no_log_groups(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.cloudwatch.cloudwatch_log_metric_filter_sign_in_without_mfa.cloudwatch_log_metric_filter_sign_in_without_mfa.logs_client', new=Logs(current_audit_info)), mock.patch('prowler.providers.aws.services.cloudwatch.cloudwatch_log_metric_filter_sign_in_without_mfa.cloudwatch_log_metric_filter_sign_in_without_mfa.cloudwatch_client', new=CloudWatch(current_audit_info)), mock.patch('prowler.providers.aws.services.cloudwatch.cloudwatch_log_metric_filter_sign_in_without_mfa.cloudwatch_log_metric_filter_sign_in_without_mfa.cloudtrail_client', new=Cloudtrail(current_audit_info)):
            from prowler.providers.aws.services.cloudwatch.cloudwatch_log_metric_filter_sign_in_without_mfa.cloudwatch_log_metric_filter_sign_in_without_mfa import cloudwatch_log_metric_filter_sign_in_without_mfa
    ...
    def test_cloudwatch_trail_with_log_group_with_metric(self):
        ...
        logs_client.create_log_group(logGroupName='/log-group/test')
        cloudtrail_client.create_trail(Name='test_trail', S3BucketName='test', CloudWatchLogsLogGroupArn=f'arn:aws:logs:{AWS_REGION}:{DEFAULT_ACCOUNT_ID}:log-group:/log-group/test:*')
        logs_client.put_metric_filter(logGroupName='/log-group/test', filterName='test-filter', filterPattern='{ ($.eventName = ConsoleLogin) && ($.additionalEventData.MFAUsed != Yes) }', metricTransformations=[{'metricName': 'my-metric', 'metricNamespace': 'my-namespace', 'metricValue': '$.value'}])
        from prowler.providers.aws.services.cloudtrail.cloudtrail_service import Cloudtrail
        from prowler.providers.aws.services.cloudwatch.cloudwatch_service import CloudWatch, Logs
        current_audit_info = self.set_mocked_audit_info()
        from prowler.providers.common.models import Audit_Metadata
    ...
    def test_cloudwatch_trail_with_log_group_with_metric_and_alarm_with_newlines(self):
        ...
        logs_client.create_log_group(logGroupName='/log-group/test')
        cloudtrail_client.create_trail(Name='test_trail', S3BucketName='test', CloudWatchLogsLogGroupArn=f'arn:aws:logs:{AWS_REGION}:{DEFAULT_ACCOUNT_ID}:log-group:/log-group/test:*')
        logs_client.put_metric_filter(logGroupName='/log-group/test', filterName='test-filter', filterPattern='{ ($.eventName = ConsoleLogin) &&\n ($.additionalEventData.MFAUsed != Yes) }', metricTransformations=[{'metricName': 'my-metric', 'metricNamespace': 'my-namespace', 'metricValue': '$.value'}])
        cloudwatch_client.put_metric_alarm(AlarmName='test-alarm', MetricName='my-metric', Namespace='my-namespace', Period=10, EvaluationPeriods=5, Statistic='Average', Threshold=2, ComparisonOperator='GreaterThanThreshold', ActionsEnabled=True)
        from prowler.providers.aws.services.cloudtrail.cloudtrail_service import Cloudtrail
        from prowler.providers.aws.services.cloudwatch.cloudwatch_service import CloudWatch, Logs
        current_audit_info = self.set_mocked_audit_info()
