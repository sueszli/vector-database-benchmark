...
class TorrentSource(ManagedDownloadSource):
    ...
    filter_fields.update({'bt_infohash'})
...
class TorrentManager(SourceManager):
    ...
    filter_fields.update({'bt_infohash', 'blobs_remaining', 'blobs_in_stream'})
    ...
    async def _load_stream(self, rowid: int, bt_infohash: str, file_name: Optional[str], download_directory: Optional[str], status: str, claim: Optional['StoredContentClaim'], content_fee: Optional['Transaction'], added_on: Optional[int]):
        stream = TorrentSource(self.loop, self.config, self.storage, identifier=bt_infohash, file_name=file_name, download_directory=download_directory, status=status, claim=claim, rowid=rowid, content_fee=content_fee, analytics_manager=self.analytics_manager, added_on=added_on, torrent_session=self.torrent_session)
        self.add(stream)
    ...
    async def create(self, file_path: str, key: Optional[bytes]=None, iv_generator: Optional[typing.Generator[bytes, None, None]]=None):
        raise NotImplementedError
