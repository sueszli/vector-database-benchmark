...
if __name__ == '__main__':
    ...
    parser.add_argument('--adversarial_accuracy_threshold', type=float, help='Model accuracy threshold on adversarial samples', default=0.2)
    ...
    metrics = robustness_evaluation(object_storage_url, object_storage_username, object_storage_password, data_bucket_name, result_bucket_name, model_id, feature_testset_path=feature_testset_path, label_testset_path=label_testset_path, clip_values=clip_values, nb_classes=nb_classes, input_shape=input_shape, model_class_file=model_class_file, model_class_name=model_class_name, LossFn=LossFn, Optimizer=Optimizer, epsilon=epsilon)
