    ...
    def sample_program_configs(self):
        ...
        for dims in [4, 3, 2, 1]:
            for shape in [[10, 12, -1, -1], [8, 64, -1, -1], [6, 8, -1]]:
                ...
                ops_config = [{'op_type': 'expand_v2', 'op_inputs': dics_intput[0], 'op_outputs': {'Out': ['expand_v2_out']}, 'op_attrs': dics[0]}]
    ...
    def sample_predictor_configs(self, program_config) -> (paddle_infer.Config, List[int], float):
        ...
        def generate_dynamic_shape(attrs):
            if False:
                return 10
            if self.dims == 4:
                self.dynamic_shape.min_input_shape = {'expand_v2_input': [1, 1, 4, 6]}
                self.dynamic_shape.max_input_shape = {'expand_v2_input': [10, 1, 4, 6]}
                self.dynamic_shape.opt_input_shape = {'expand_v2_input': [1, 1, 4, 6]}
            elif self.dims == 3:
                self.dynamic_shape.min_input_shape = {'expand_v2_input': [1, 8, 6]}
                self.dynamic_shape.max_input_shape = {'expand_v2_input': [4, 8, 6]}
    ...
    def sample_program_configs(self):
        ...
        for dims in [1]:
            for shape in [[10, 12, -1, -1], [8, 64, -1, -1]]:
                dics = [{'shape': shape}]
                self.dims = dims
                dics_intput = [{'X': ['expand_v2_input'], 'Shape': ['shapeT1_data']}]
                ops_config = [{'op_type': 'fill_constant', 'op_inputs': {}, 'op_outputs': {'Out': ['shapeT1_data']}, 'op_attrs': {'dtype': 2, 'str_value': '10', 'shape': [1]}}, {'op_type': 'expand_v2', 'op_inputs': dics_intput[0], 'op_outputs': {'Out': ['expand_v2_out']}, 'op_attrs': dics[0]}]
                ops = self.generate_op_config(ops_config)
                program_config = ProgramConfig(ops=ops, weights={}, inputs={'expand_v2_input': TensorConfig(data_gen=partial(generate_input1, dics))}, outputs=['expand_v2_out'])
                yield program_config
    ...
    def sample_program_configs(self):
        ...
        for dims in [4, 3]:
            for shape in [[10, 12, -1, -1], [8, 64, -1, -1]]:
                dics = [{'shape': shape}]
                self.dims = dims
                dics_intput = [{'X': ['expand_v2_input'], 'expand_shapes_tensor': ['shapeT1_data', 'shapeT2_data', 'shapeT3_data', 'shapeT4_data']}]
    ...
    def sample_predictor_configs(self, program_config) -> (paddle_infer.Config, List[int], float):
        ...
        def generate_dynamic_shape():
            ...
            if self.dims == 4:
                ...
                self.dynamic_shape.max_input_shape = {'expand_v2_input': [4, 4, 6]}
