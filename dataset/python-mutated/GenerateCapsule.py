...
if __name__ == '__main__':
    ...
    parser.add_argument('InputFile', type=argparse.FileType('rb'), nargs='?', help='Input binary payload filename.')
    parser.add_argument('-o', '--output', dest='OutputFile', type=argparse.FileType('wb'), help='Output filename.')
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('-e', '--encode', dest='Encode', action='store_true', help='Encode file')
    group.add_argument('-d', '--decode', dest='Decode', action='store_true', help='Decode file')
    group.add_argument('--dump-info', dest='DumpInfo', action='store_true', help='Display FMP Payload Header information')
    parser.add_argument('-j', '--json-file', dest='JsonFile', type=argparse.FileType('r'), help='JSON configuration file for multiple payloads and embedded drivers.')
    parser.add_argument('--capflag', dest='CapsuleFlag', action='append', default=[], choices=['PersistAcrossReset', 'InitiateReset'], help='Capsule flag can be PersistAcrossReset or InitiateReset or not set')
    parser.add_argument('--capoemflag', dest='CapsuleOemFlag', type=ValidateUnsignedInteger, default=0, help='Capsule OEM Flag is an integer between 0x0000 and 0xffff.')
    parser.add_argument('--guid', dest='Guid', type=ValidateRegistryFormatGuid, help='The FMP/ESRT GUID in registry format.  Required for single payload encode operations.')
    parser.add_argument('--hardware-instance', dest='HardwareInstance', type=ValidateUnsignedInteger, default=0, help='The 64-bit hardware instance.  The default is 0x0000000000000000')
    parser.add_argument('--monotonic-count', dest='MonotonicCount', type=ValidateUnsignedInteger, default=0, help='64-bit monotonic count value in header.  Default is 0x0000000000000000.')
    parser.add_argument('--fw-version', dest='FwVersion', type=ValidateUnsignedInteger, help='The 32-bit version of the binary payload (e.g. 0x11223344 or 5678).  Required for encode operations.')
    parser.add_argument('--lsv', dest='LowestSupportedVersion', type=ValidateUnsignedInteger, help='The 32-bit lowest supported version of the binary payload (e.g. 0x11223344 or 5678).  Required for encode operations.')
    parser.add_argument('--pfx-file', dest='SignToolPfxFile', type=argparse.FileType('rb'), help='signtool PFX certificate filename.')
    parser.add_argument('--subject-name', dest='SignToolSubjectName', help='signtool certificate subject name.')
    parser.add_argument('--signer-private-cert', dest='OpenSslSignerPrivateCertFile', type=argparse.FileType('rb'), help='OpenSSL signer private certificate filename.')
    parser.add_argument('--other-public-cert', dest='OpenSslOtherPublicCertFile', type=argparse.FileType('rb'), help='OpenSSL other public certificate filename.')
    parser.add_argument('--trusted-public-cert', dest='OpenSslTrustedPublicCertFile', type=argparse.FileType('rb'), help='OpenSSL trusted public certificate filename.')
    parser.add_argument('--signing-tool-path', dest='SigningToolPath', help='Path to signtool or OpenSSL tool.  Optional if path to tools are already in PATH.')
    parser.add_argument('--embedded-driver', dest='EmbeddedDriver', type=argparse.FileType('rb'), action='append', default=[], help='Path to embedded UEFI driver to add to capsule.')
    ...
    parser.add_argument('-q', '--quiet', dest='Quiet', action='store_true', help='Disable all messages except fatal errors.')
    parser.add_argument('--debug', dest='Debug', type=int, metavar='[0-9]', choices=range(0, 10), default=0, help='Set debug level')
    parser.add_argument('--update-image-index', dest='UpdateImageIndex', type=ValidateUnsignedInteger, default=1, help='unique number identifying the firmware image within the device ')
