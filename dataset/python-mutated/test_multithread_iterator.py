...
@testing.parameterize(*testing.product({'n_threads': [1, 2], 'order_sampler': [None, lambda order, _: numpy.random.permutation(len(order))]}))
class TestMultithreadIterator(unittest.TestCase):
    ...
    def setUp(self):
        if False:
            for i in range(10):
                print('nop')
        self.options = {'n_threads': self.n_threads, 'order_sampler': self.order_sampler}
    ...
    def test_iterator_not_repeat(self):
        if False:
            for i in range(10):
                print('nop')
        dataset = [1, 2, 3, 4, 5]
        it = iterators.MultithreadIterator(dataset, 2, repeat=False, **self.options)
    ...
    def test_supported_reset_repeat(self):
        if False:
            for i in range(10):
                print('nop')
        dataset = [1, 2, 3, 4]
        it = iterators.MultithreadIterator(dataset, 2, repeat=True, **self.options)
        it.next()
        it.next()
        it.reset()
...
class TestMultithreadIteratorNoSameIndicesOrderSampler(unittest.TestCase):
    ...
    def test_no_same_indices_order_sampler(self):
        ...
        dataset = [1, 2, 3, 4, 5, 6]
        batchsize = 5
        it = iterators.MultithreadIterator(dataset, batchsize, order_sampler=NoSameIndicesOrderSampler(batchsize))
        for _ in range(5):
            batch = it.next()
...
class TestMultithreadIteratorInvalidOrderSampler(unittest.TestCase):
    ...
    def test_invalid_order_sampler(self):
        ...
        with self.assertRaises(ValueError):
            it = iterators.MultithreadIterator(dataset, 6, order_sampler=InvalidOrderSampler())
            it.next()
