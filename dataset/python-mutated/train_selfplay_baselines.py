...
PRED_NUM_EPOCHS = 10
PRED_LEARNING_RATE = 0.0003
PREY_NUM_EPOCHS = 3
PREY_LEARNING_RATE = 0.0001
env_config = {'Obs': OBS, 'Act': ACT, 'Env': ENV, 'Hierarchy': '2D:evorobotpy2:predprey:1v1'}
training_config = {'pred_algorithm': PRED_ALGO, 'prey_algorithm': PREY_ALGO, 'num_rounds': NUM_ROUNDS, 'save_freq': SAVE_FREQ, 'num_timesteps': NUM_TIMESTEPS, 'num_eval_episodes': NUM_EVAL_EPISODES, 'num_workers': WORKERS, 'seed': SEED_VALUE, 'eval_freq': EVAL_FREQ, 'framework': 'stable_baselines3', 'agent_selection': 'latest', 'opponent_selection': OPPONENT_SELECTION, 'eval_opponent_selection': EVAL_OPPONENT_SELECTION, 'training_schema': 'alternating', 'ranking': 'none', 'final_save_freq': FINAL_SAVE_FREQ, 'eval_metric': EVAL_METRIC, 'num_sampled_per_round': NUM_SAMPLED_OPPONENT_PER_ROUND, 'sample_after_rollout': SAMPLE_AFTER_ROLLOUT}
pred_algorithm_config = {'policy': 'MlpPolicy', 'clip_range': 0.2, 'ent_coef': 0.0, 'lr': PRED_LEARNING_RATE, 'batch_size': 64, 'gamma': 0.99, 'n_epochs': PRED_NUM_EPOCHS}
prey_algorithm_config = {'policy': 'MlpPolicy', 'clip_range': 0.2, 'ent_coef': 0.0, 'lr': PREY_LEARNING_RATE, 'batch_size': 64, 'gamma': 0.99, 'n_epochs': PREY_NUM_EPOCHS}
...
if __name__ == '__main__':
    ...
    wandb.init(project='Behavioral-Learning-Thesis', group='self-play', config=wandb_experiment_config, sync_tensorboard=True, monitor_gym=True, save_code=True, notes='Debugging why the graphs are not appearing')
    wandb.run.name = wandb.run.name + '-debug-v4'
    wandb.run.save()
