...
class sfp_tool_snallygaster(SpiderFootPlugin):
    meta = {'name': 'Tool - snallygaster', 'summary': 'Finds file leaks and other security problems on HTTP servers.', 'flags': ['tool'], 'useCases': ['Footprint', 'Investigate'], 'categories': ['Crawling and Scanning'], 'toolDetails': {'name': 'snallygaster', 'description': "snallygaster is a tool that looks for files accessible on web servers that shouldn't be public and can pose a security risk. Typical examples include publicly accessible git repositories, backup files potentially containing passwords  or database dumps. In addition, it contains a few checks  for other security vulnerabilities.", 'website': 'https://github.com/hannob/snallygaster', 'repository': 'https://github.com/hannob/snallygaster'}}
    ...
    def producedEvents(self):
        if False:
            for i in range(10):
                print('nop')
        return ['VULNERABILITY_GENERAL', 'VULNERABILITY_CVE_CRITICAL', 'VULNERABILITY_CVE_HIGH', 'VULNERABILITY_CVE_MEDIUM', 'VULNERABILITY_CVE_LOW']
