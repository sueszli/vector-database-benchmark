    ...
    async def create(self, payload, _, **kwargs):
        ...
        if token and provider:
            ...
            if not email:
                ...
                if not user:
                    ...
                    user = User.create(username=username, email=email, roles_new=roles)
        ...
        if AUTHENTICATION_MODE.lower() == 'ldap':
            ...
            if not user:
                ...
                if not roles and LDAP_DEFAULT_ACCESS is not None and (LDAP_DEFAULT_ACCESS in [r for r in Role.DefaultRole]):
                    default_role = Role.get_role(LDAP_DEFAULT_ACCESS)
                    if default_role:
                        roles.append(default_role)
                user = User.create(roles_new=roles, username=email)
