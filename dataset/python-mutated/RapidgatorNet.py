...
class RapidgatorNet(SimpleDownloader):
    ...
    __version__ = '0.59'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?(?:rapidgator\\.(?:net|asia|)|rg\\.to)/file/(?P<ID>\\w+)'
    ...
    __license__ = 'GPLv3'
    __authors__ = [('zoidberg', 'zoidberg@mujmail.cz'), ('chrox', None), ('stickell', 'l.stickell@yahoo.it'), ('Walter Purcaro', 'vuolter@gmail.com'), ('GammaCode', 'nitzo2001[AT]yahoo[DOT]com')]
    COOKIES = [('rapidgator.net', 'lang', 'en')]
    NAME_PATTERN = '<title>Download file (?P<N>.*)</title>'
    SIZE_PATTERN = 'File size:\\s*<strong>(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)</strong>'
    OFFLINE_PATTERN = '>(404 File not found|Error 404)'
    JSVARS_PATTERN = "\\s+var\\s*(startTimerUrl|getDownloadUrl|captchaUrl|fid|secs)\\s*=\\s*\\'?(.*?)\\'?;"
    PREMIUM_ONLY_PATTERN = 'You can download files up to|This file can be downloaded by premium only<'
    DOWNLOAD_LIMIT_ERROR_PATTERN = 'You have reached your (daily|hourly) downloads limit'
    IP_BLOCKED_ERROR_PATTERN = 'You can`t download more than 1 file at a time in free mode\\.'
    WAIT_PATTERN = '(?:Delay between downloads must be not less than|Try again in).+'
    LINK_FREE_PATTERN = "return '(https?://\\w+.rapidgator.net/.*)';"
    RECAPTCHA_PATTERN = '"http://api\\.recaptcha\\.net/challenge\\?k=(.*?)"'
    SOLVEMEDIA_PATTERN = 'http://api\\.solvemedia\\.com/papi/challenge\\.script\\?k=(.*?)"'
    URL_REPLACEMENTS = [(__pattern__ + '.*', 'https://rapidgator.net/file/\\g<ID>')]
