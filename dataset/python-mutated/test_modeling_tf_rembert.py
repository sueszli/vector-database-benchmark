...
if is_tf_available():
    ...
    from transformers import TFRemBertForCausalLM, TFRemBertForMaskedLM, TFRemBertForMultipleChoice, TFRemBertForQuestionAnswering, TFRemBertForSequenceClassification, TFRemBertForTokenClassification, TFRemBertModel
...
class TFRemBertModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TFRemBertModel, TFRemBertForCausalLM, TFRemBertForMaskedLM, TFRemBertForQuestionAnswering, TFRemBertForSequenceClassification, TFRemBertForTokenClassification, TFRemBertForMultipleChoice) if is_tf_available() else ()
...
class TFRemBertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_model(self):
        ...
        expected_implementation = tf.constant([[[0.0754, -0.2022, 0.1904], [-0.3354, -0.3692, -0.4791], [-0.2314, -0.6729, -0.0749], [-0.0396, -0.3105, -0.4234], [-0.1571, -0.0525, 0.5353]]])
        tf.debugging.assert_near(output['last_hidden_state'][:, :, :3], expected_implementation, atol=0.0001)