...
tf.flags.DEFINE_string('input_files', None, 'Comma-separated list of globs matching the input files. The format of the input files is assumed to be a list of newline-separated sentences, where each sentence is already tokenized.')
tf.flags.DEFINE_string('vocab_file', '', '(Optional) existing vocab file. Otherwise, a new vocab file is created and written to the output directory. The file format is a list of newline-separated words, where the word id is the corresponding 0-based index in the file.')
...
tf.flags.DEFINE_integer('max_sentence_length', 30, 'If > 0, exclude sentences whose encode, decode_pre ORdecode_post sentence exceeds this length.')
