...
def test_zero_sample():
    if False:
        while True:
            i = 10
    m = MeanError()
    with pytest.raises(NotComputableError, match='MeanError must have at least one example before it can be computed'):
        m.compute()
...
def test_integration():
    ...
    def _test(y_pred, y, batch_size):
        ...
        np_ans = np_sum / np_len
        assert np_ans == pytest.approx(me, rel=0.0001)

    def get_test_cases():
        if False:
            i = 10
            return i + 15
        test_cases = [(torch.rand(size=(50,)), torch.rand(size=(50,)), 1), (torch.rand(size=(50, 1)), torch.rand(size=(50, 1)), 10)]
        return test_cases
    for _ in range(5):
