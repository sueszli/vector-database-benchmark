...
class TestBase(IPUOpTest):
    ...
    def run_model(self, exec_mode):
        ...
        if self.is_fp16_mode(exec_mode):
            ...
            to_fp16_var_names = paddle.static.amp.cast_model_to_fp16(self.main_prog, amp_list)
        ...
        if self.is_ipu_mode(exec_mode):
            ...
            ipu_strategy.set_graph_config(is_training=self.is_training, num_ipus=self.num_ipus, enable_manual_shard=self.enable_manual_shard)
        ...
        for _ in range(self.epoch):
            out = exe.run(program, feed=self.feed_fp32, fetch_list=self.fetch_list)
            result.append(out)
