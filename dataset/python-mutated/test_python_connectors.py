...
class TestUnitTests:
    ...
    def context_for_certified_connector_with_setup(self, certified_connector_with_setup, dagger_client):
        if False:
            print('Hello World!')
        context = ConnectorContext(pipeline_name='test unit tests', connector=certified_connector_with_setup, git_branch='test', git_revision='test', report_output_prefix='test', is_local=True, use_remote_secrets=True)
    ...
    def context_for_connector_with_poetry(self, connector_with_poetry, dagger_client):
        if False:
            for i in range(10):
                print('nop')
        context = ConnectorContext(pipeline_name='test unit tests', connector=connector_with_poetry, git_branch='test', git_revision='test', report_output_prefix='test', is_local=True, use_remote_secrets=True)
        context.dagger_client = dagger_client
        context.connector_secrets = {}
        return context
    ...
    async def test__run_for_setup_py(self, context_for_certified_connector_with_setup, certified_container_with_setup):
        ...
        assert 'Total coverage:' in result.stdout, 'The pytest-cov package should be installed in the test environment and test coverage report should be displayed.'
        ...
        assert context_for_certified_connector_with_setup.connector.technical_name in pip_freeze_output, 'The connector should be installed in the test environment.'
    ...
    async def test__run_for_poetry(self, context_for_connector_with_poetry, container_with_poetry):
        ...
        assert context_for_connector_with_poetry.connector.technical_name in pip_freeze_output, 'The connector should be installed in the test environment.'
        assert 'pytest' in pip_freeze_output, 'The pytest package should be installed in the test environment.'