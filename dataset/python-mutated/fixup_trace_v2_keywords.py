...
class traceCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'batch_write_spans': ('name', 'spans'), 'create_span': ('name', 'span_id', 'display_name', 'start_time', 'end_time', 'parent_span_id', 'attributes', 'stack_trace', 'time_events', 'links', 'status', 'same_process_as_parent_span', 'child_span_count', 'span_kind')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=traceCallTransformer()):
    if False:
        print('Hello World!')
