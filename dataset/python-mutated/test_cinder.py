...
from cinder import async_cached_classproperty, async_cached_property, cached_classproperty, cached_property, strict_module_patch, StrictModule
    ...
    async def test_get_awaiter_frame(self):
        ...
        async def f2():
            ...
            if CINDERJIT_ENABLED and cinderjit.jit_frame_mode() == 1:
                ...
                self.assertEqual(str(context.exception), 'Fetching awaiter frame is not supported.')
            else:
                awaiter_frame = cinder._get_awaiter_frame()
...
class CachedCostItemWithSlots:
    ...
    def cost_impl(self):
        if False:
            ...
            return i + 15
        return 42
CachedCostItemWithSlots.cost = cached_property(CachedCostItemWithSlots.cost_impl, CachedCostItemWithSlots.cost)
...
@unittest.skipUnless(hasattr(_asyncio, '_ALVResultFuture'), 'requires the C _asyncio module')
