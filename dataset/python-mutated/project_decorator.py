...
class ProjectDecorator(FlowDecorator):
    ...
    name = 'project'
    defaults = {'name': None}
    options = {'production': dict(is_flag=True, default=False, show_default=True, help="Use the @project's production branch. To use a custom branch, use --branch."), 'branch': dict(default=None, show_default=False, help='Use the given branch name under @project. The default is the user name if --production is not specified.')}
