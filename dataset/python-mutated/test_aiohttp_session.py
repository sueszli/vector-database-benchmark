...
class TestAiohttpSession:
    ...
    async def test_create_proxy_session_chained_proxies(self):
        proxy_chain = ['socks4://proxy.url/', 'socks5://proxy.url/', 'http://user:password@127.0.0.1:3128']
    ...
    async def test_make_request(self, bot: MockedBot, aresponses: ResponsesMockServer):
        aresponses.add(aresponses.ANY, '/bot42:TEST/method', 'post', aresponses.Response(status=200, text='{"ok": true, "result": 42}', headers={'Content-Type': 'application/json'}))
    ...
    async def test_make_request_network_error(self, error):
        ...
        async with Bot('42:TEST').context() as bot:
            with patch('aiohttp.client.ClientSession._request', new_callable=AsyncMock, side_effect=side_effect):
                with pytest.raises(TelegramNetworkError):
                    await bot.get_me()

    async def test_stream_content(self, aresponses: ResponsesMockServer):
        aresponses.add(aresponses.ANY, aresponses.ANY, 'get', aresponses.Response(status=200, body=b'\x0c' * 10))
        async with AiohttpSession() as session:
            stream = session.stream_content('https://www.python.org/static/img/python-logo.png', timeout=5, chunk_size=1, raise_for_status=True)
    ...
    async def test_stream_content_404(self, aresponses: ResponsesMockServer):
        aresponses.add(aresponses.ANY, aresponses.ANY, 'get', aresponses.Response(status=404, body=b'File not found'))
        async with AiohttpSession() as session:
            stream = session.stream_content('https://www.python.org/static/img/python-logo.png', timeout=5, chunk_size=1, raise_for_status=True)
    ...
    async def test_context_manager(self):
        async with AiohttpSession() as session:
            ...
            with patch('aiogram.client.session.aiohttp.AiohttpSession.create_session', new_callable=AsyncMock) as mocked_create_session, patch('aiogram.client.session.aiohttp.AiohttpSession.close', new_callable=AsyncMock) as mocked_close:
