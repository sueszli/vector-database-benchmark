...
class CosmoboxOrg(XFSDownloader):
    ...
    __version__ = '0.03'
    __status__ = 'testing'
    __pattern__ = 'https?://cosmobox\\.org/\\w{12}'
    __config__ = [('enabled', 'bool', 'Activated', True), ('chk_filesize', 'bool', 'Check file size', True), ('max_wait', 'int', 'Reconnect if waiting time is greater than minutes', 10)]
    ...
    NAME_PATTERN = "You're downloading: (?P<N>.+?)<"
    SIZE_PATTERN = '<span class="label label-default">(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)</span>'
    WAIT_PATTERN = '<span class="circle"><span class="seconds">(\\d+)</span></span>'
