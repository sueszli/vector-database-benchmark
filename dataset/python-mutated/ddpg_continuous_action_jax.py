...
if __name__ == '__main__':
    ...
    @jax.jit
    def update_critic(actor_state: TrainState, qf1_state: TrainState, observations: np.ndarray, actions: np.ndarray, next_observations: np.ndarray, rewards: np.ndarray, terminations: np.ndarray):
        if False:
            return 10
    ...
    @jax.jit
    def update_actor(actor_state: TrainState, qf1_state: TrainState, observations: np.ndarray):
        if False:
            for i in range(10):
                print('nop')
    ...
    for global_step in range(args.total_timesteps):
        if global_step < args.learning_starts:
            ...
            actions = np.array([(jax.device_get(actions)[0] + np.random.normal(0, actor.action_scale * args.exploration_noise)[0]).clip(envs.single_action_space.low, envs.single_action_space.high)])
        ...
        if global_step > args.learning_starts:
            ...
            (qf1_state, qf1_loss_value, qf1_a_values) = update_critic(actor_state, qf1_state, data.observations.numpy(), data.actions.numpy(), data.next_observations.numpy(), data.rewards.flatten().numpy(), data.dones.flatten().numpy())
            if global_step % args.policy_frequency == 0:
                (actor_state, qf1_state, actor_loss_value) = update_actor(actor_state, qf1_state, data.observations.numpy())
    ...
    if args.save_model:
        ...
        with open(model_path, 'wb') as f:
            f.write(flax.serialization.to_bytes([actor_state.params, qf1_state.params]))
