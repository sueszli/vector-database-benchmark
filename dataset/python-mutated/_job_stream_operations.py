...
class JobStreamOperations:
    ...
    async def get(self, resource_group_name: str, automation_account_name: str, job_name: str, job_stream_id: str, client_request_id: Optional[str]=None, **kwargs: Any) -> _models.JobStream:
        ...
        request = build_get_request(resource_group_name=resource_group_name, automation_account_name=automation_account_name, job_name=job_name, job_stream_id=job_stream_id, subscription_id=self._config.subscription_id, client_request_id=client_request_id, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
    ...
    @distributed_trace
    def list_by_job(self, resource_group_name: str, automation_account_name: str, job_name: str, filter: Optional[str]=None, client_request_id: Optional[str]=None, **kwargs: Any) -> AsyncIterable['_models.JobStream']:
        if False:
            for i in range(10):
                print('nop')
        ...
        def prepare_request(next_link=None):
            if False:
                ...
                return i + 15
            if not next_link:
                request = build_list_by_job_request(resource_group_name=resource_group_name, automation_account_name=automation_account_name, job_name=job_name, subscription_id=self._config.subscription_id, filter=filter, client_request_id=client_request_id, api_version=api_version, template_url=self.list_by_job.metadata['url'], headers=_headers, params=_params)
                request = _convert_request(request)
                request.url = self._client.format_url(request.url)
            else:
