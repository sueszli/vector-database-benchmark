...
async def test_migrate_matching_port(hass: HomeAssistant, mock_connect_zigpy_app) -> None:
    """Test automatic migration."""
    config_entry = MockConfigEntry(data={'device': {'path': '/dev/ttyTEST123'}, 'radio_type': 'ezsp'}, domain=DOMAIN, options={}, title='Test', version=3)
    config_entry.add_to_hass(hass)
    migration_data = {'new_discovery_info': {'name': 'Test Updated', 'port': {'path': 'socket://some/virtual_port', 'baudrate': 115200, 'flow_control': 'hardware'}, 'radio_type': 'efr32'}, 'old_discovery_info': {'hw': {'name': 'Test', 'port': {'path': '/dev/ttyTEST123', 'baudrate': 115200, 'flow_control': 'hardware'}, 'radio_type': 'efr32'}}}
    ...
    assert config_entry.data == {'device': {'path': 'socket://some/virtual_port', 'baudrate': 115200, 'flow_control': 'hardware'}, 'radio_type': 'ezsp'}
    assert config_entry.title == 'Test Updated'
    await migration_helper.async_finish_migration()

@patch('homeassistant.components.zha.radio_manager.ZhaRadioManager.detect_radio_type', mock_detect_radio_type())
@patch('homeassistant.components.zha.async_setup_entry', AsyncMock(return_value=True))
async def test_migrate_matching_port_usb(hass: HomeAssistant, mock_connect_zigpy_app) -> None:
    """Test automatic migration."""
    config_entry = MockConfigEntry(data={'device': {'path': '/dev/ttyTEST123'}, 'radio_type': 'ezsp'}, domain=DOMAIN, options={}, title='Test', version=3)
    config_entry.add_to_hass(hass)
    migration_data = {'new_discovery_info': {'name': 'Test Updated', 'port': {'path': 'socket://some/virtual_port', 'baudrate': 115200, 'flow_control': 'hardware'}, 'radio_type': 'efr32'}, 'old_discovery_info': {'usb': UsbServiceInfo('/dev/ttyTEST123', 'blah', 'blah', None, None, None)}}
    migration_helper = radio_manager.ZhaMultiPANMigrationHelper(hass, config_entry)
    assert await migration_helper.async_initiate_migration(migration_data)
    assert config_entry.data == {'device': {'path': 'socket://some/virtual_port', 'baudrate': 115200, 'flow_control': 'hardware'}, 'radio_type': 'ezsp'}
    assert config_entry.title == 'Test Updated'
...
async def test_migrate_matching_port_config_entry_not_loaded(hass: HomeAssistant, mock_connect_zigpy_app) -> None:
    """Test automatic migration."""
    config_entry = MockConfigEntry(data={'device': {'path': '/dev/ttyTEST123'}, 'radio_type': 'ezsp'}, domain=DOMAIN, options={}, title='Test')
    config_entry.add_to_hass(hass)
...
@patch('homeassistant.components.zha.radio_manager.ZhaRadioManager.async_restore_backup_step_1', side_effect=OSError)
async def test_migrate_matching_port_retry(mock_restore_backup_step_1, hass: HomeAssistant, mock_connect_zigpy_app) -> None:
    """Test automatic migration."""
    config_entry = MockConfigEntry(data={'device': {'path': '/dev/ttyTEST123'}, 'radio_type': 'ezsp'}, domain=DOMAIN, options={}, title='Test')
    config_entry.add_to_hass(hass)
    config_entry.state = config_entries.ConfigEntryState.SETUP_IN_PROGRESS
...
async def test_migrate_non_matching_port(hass: HomeAssistant, mock_connect_zigpy_app) -> None:
    """Test automatic migration."""
    config_entry = MockConfigEntry(data={'device': {'path': '/dev/ttyTEST123'}, 'radio_type': 'ezsp'}, domain=DOMAIN, options={}, title='Test')
    config_entry.add_to_hass(hass)
    migration_data = {'new_discovery_info': {'name': 'Test Updated', 'port': {'path': 'socket://some/virtual_port', 'baudrate': 115200, 'flow_control': 'hardware'}, 'radio_type': 'efr32'}, 'old_discovery_info': {'hw': {'name': 'Test', 'port': {'path': '/dev/ttyTEST456', 'baudrate': 115200, 'flow_control': 'hardware'}, 'radio_type': 'efr32'}}}
    migration_helper = radio_manager.ZhaMultiPANMigrationHelper(hass, config_entry)
    assert not await migration_helper.async_initiate_migration(migration_data)
    assert config_entry.data == {'device': {'path': '/dev/ttyTEST123'}, 'radio_type': 'ezsp'}
    assert config_entry.title == 'Test'

async def test_migrate_initiate_failure(hass: HomeAssistant, mock_connect_zigpy_app) -> None:
...
async def test_detect_radio_type_success(radio_manager: ZhaRadioManager) -> None:
    ...
    with patch('bellows.zigbee.application.ControllerApplication.probe', return_value=False), patch('zigpy_znp.zigbee.application.ControllerApplication.probe', return_value=True):
        assert await radio_manager.detect_radio_type() == ProbeResult.RADIO_TYPE_DETECTED
        assert radio_manager.radio_type == RadioType.znp

async def test_detect_radio_type_failure_wrong_firmware(radio_manager: ZhaRadioManager) -> None:
    """Test radio type detection, wrong firmware."""
    with patch('homeassistant.components.zha.radio_manager.AUTOPROBE_RADIOS', ()), patch('homeassistant.components.zha.radio_manager.repairs.wrong_silabs_firmware.warn_on_wrong_silabs_firmware', return_value=True):
        assert await radio_manager.detect_radio_type() == ProbeResult.WRONG_FIRMWARE_INSTALLED
        assert radio_manager.radio_type is None

async def test_detect_radio_type_failure_no_detect(radio_manager: ZhaRadioManager) -> None:
    """Test radio type detection, no firmware detected."""
    with patch('homeassistant.components.zha.radio_manager.AUTOPROBE_RADIOS', ()), patch('homeassistant.components.zha.radio_manager.repairs.wrong_silabs_firmware.warn_on_wrong_silabs_firmware', return_value=False):
