...
grpc_port = 9000
grpc_servicer_functions = ['user_defined_protos_pb2_grpc.add_UserDefinedServiceServicer_to_server', 'user_defined_protos_pb2_grpc.add_ImageClassificationServiceServicer_to_server']
serve.start(grpc_options=gRPCOptions(port=grpc_port, grpc_servicer_functions=grpc_servicer_functions))
import time
from typing import Generator
from user_defined_protos_pb2 import UserDefinedMessage, UserDefinedMessage2, UserDefinedResponse, UserDefinedResponse2
import ray
from ray import serve
...
class GrpcDeployment:
    ...
    async def Multiplexing(self, user_message: UserDefinedMessage2) -> UserDefinedResponse2:
        ...
        user_response = UserDefinedResponse2(greeting=f'Method2 called model, {model}')
        return user_response
...
print(f'Applications: {response.application_names}')
...
metadata = (('application', app_name), ('request_id', request_id), ('multiplexed_model_id', multiplexed_model_id))
(response, call) = stub.Multiplexing.with_call(request=request, metadata=metadata)
print(f'greeting: {response.greeting}')
for (key, value) in call.trailing_metadata():
    print(f'trailing metadata key: {key}, value {value}')
import grpc
from user_defined_protos_pb2_grpc import UserDefinedServiceStub
from user_defined_protos_pb2 import UserDefinedMessage
channel = grpc.insecure_channel('localhost:9000')
...
from user_defined_protos_pb2 import ImageClass, ImageData
from ray import serve
...
g2 = ImageClassifier.options(name='grpc-image-classifier').bind(image_downloader, data_preprocessor)
app2 = 'app2'
