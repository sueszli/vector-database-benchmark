...
class OpenAIClient(AIClient):
    ...
    async def inference_with_prompt(self, variable_values: Dict[str, str], prompt_template: str, is_json_response: bool=True):
    ...
    async def find_block_params(self, block_description: str):
        messages = [{'role': 'user', 'content': block_description}]
        response = await openai.ChatCompletion.acreate(model='gpt-3.5-turbo-0613', messages=messages, functions=TEMPLATE_CLASSIFICATION_FUNCTION, function_call={'name': CLASSIFICATION_FUNCTION_NAME})
        response_message = response['choices'][0]['message']
        if response_message.get('function_call'):
            function_args = json.loads(response_message['function_call']['arguments'])
            (block_type, block_language, pipeline_type, config) = self.__load_template_params(function_args)
            output = {}
