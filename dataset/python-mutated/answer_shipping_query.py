...
class AnswerShippingQuery(TelegramMethod[bool]):
    ...
    if TYPE_CHECKING:
        ...
        def __init__(__pydantic__self__, *, shipping_query_id: str, ok: bool, shipping_options: Optional[List[ShippingOption]]=None, error_message: Optional[str]=None, **__pydantic_kwargs: Any) -> None:
            if False:
                while True:
                    i = 10
            super().__init__(shipping_query_id=shipping_query_id, ok=ok, shipping_options=shipping_options, error_message=error_message, **__pydantic_kwargs)