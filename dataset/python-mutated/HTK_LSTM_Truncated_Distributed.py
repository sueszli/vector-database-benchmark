...
if __name__ == '__main__':
    ...
    try:
        htk_lstm_truncated(features_file, labels_file, label_mapping_file, minibatch_size=args['minibatch_size'], epoch_size=args['epoch_size'], num_quantization_bits=args['quantized_bits'], block_size=args['block_samples'], warm_up=args['distributed_after'], max_epochs=args['num_epochs'], log_to_file=args['logdir'], num_mbs_per_log=100, gen_heartbeat=False, tensorboard_logdir=args['tensorboard_logdir'], cv_freq=args['cvfreq'])
    finally:
        os.chdir(abs_path)
