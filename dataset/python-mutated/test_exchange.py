...
from freqtrade.exceptions import DDosProtection, DependencyException, ExchangeError, InsufficientFundsError, InvalidOrderException, OperationalException, PricingError, TemporaryError
from freqtrade.exchange import Binance, Bittrex, Exchange, Kraken, market_is_active, timeframe_to_prev_date
...
async def test__async_get_historic_ohlcv(default_conf, mocker, caplog, exchange_name, candle_type):
    ...
    pair = 'ETH/USDT'
    (respair, restf, _, res, _) = await exchange._async_get_historic_ohlcv(pair, '5m', 1500000000000, candle_type=candle_type, is_new_pair=False)
    assert respair == pair
    assert restf == '5m'
    ...
    (respair, restf, _, res, _) = await exchange._async_get_historic_ohlcv(pair, '5m', since_ms=start_ts, candle_type=candle_type, is_new_pair=False, until_ms=end_ts)
