...
async def test_ctor_content_type_without_text() -> None:
    with pytest.deprecated_call(match='^content_type without text is deprecated since 4\\.0 and scheduled for removal in 5\\.0 \\(#3462\\)$'):
...
async def test_ctor_text_for_empty_body() -> None:
    with pytest.deprecated_call(match='^text argument is deprecated for HTTP status 205 since 4\\.0 and scheduled for removal in 5\\.0 \\(#3462\\),the response should be provided without a body$'):
...
class TestHTTPMethodNotAllowed:
    ...
    async def test_ctor(self) -> None:
        resp = web.HTTPMethodNotAllowed('GET', ['POST', 'PUT'], headers={'X-Custom': 'value'}, reason='Unsupported', text='text', content_type='custom')
        assert resp.method == 'GET'
        assert resp.allowed_methods == {'POST', 'PUT'}
        assert resp.text == 'text'
        compare: Mapping[str, str] = {'X-Custom': 'value', 'Content-Type': 'custom', 'Allow': 'POST,PUT'}
        assert resp.headers == compare
