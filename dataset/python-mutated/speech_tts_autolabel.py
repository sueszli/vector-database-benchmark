...
try:
    ...
    raise ImportError('pls install tts-autolabel with                       "pip install tts-autolabel -f                       https://modelscope.oss-cn-beijing.aliyuncs.com/releases/repo.html"')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--para_ids', default='all', help='you can use this variable to config your auto labeling paragraph ids,         all means all in the dir, none means no paragraph 1 means 1 para only,         1 2 means 1 and 2, transcipt/prosody/wav should be named exactly the same!!!')
    parser.add_argument('--resource', type=str, default=DEFAULT_RESOURCE_MODEL_ID)
    parser.add_argument('--resource_revision', type=str, default=None, help='resource directory')
    parser.add_argument('--input_wav', help='personal user input wav dir')
    parser.add_argument('--work_dir', help='autolabel work dir')
    parser.add_argument('--gender', default='female', help='personal user gender')
    parser.add_argument('--develop_mode', type=int, default=1)
    parser.add_argument('--stage', type=int, default=1, help='auto labeling stage, 0 means qualification and 1 means labeling')
    parser.add_argument('--process_num', type=int, default=4, help='kaldi bin parallel execution process number')
    parser.add_argument('--has_para', dest='has_para', action='store_true', help='paragraph')
    parser.add_argument('--no_para', dest='has_para', action='store_false', help='no paragraph')
    parser.add_argument('--enable_enh', dest='enable_enh', action='store_true', help='enable audio enhancement')
    parser.add_argument('--disable_enh', dest='enable_enh', action='store_false', help='disable audio enhancement')
    parser.set_defaults(has_para=True)
    parser.set_defaults(enable_enh=False)
    args = parser.parse_args()
    logger.info(args.enable_enh)
    (ret_code, report) = run_auto_label(args.input_wav, args.work_dir, args.para_ids, args.resource, args.resource_revision, args.gender, args.stage, args.process_num, args.develop_mode, args.has_para, args.enable_enh)
