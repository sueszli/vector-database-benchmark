...
class TFSamModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': TFSamModel, 'mask-generation': TFSamModel} if is_tf_available() else {}
    test_pruning = False
    ...
    def check_pt_tf_outputs(self, tf_outputs, pt_outputs, model_class, tol=0.0005, name='outputs', attributes=None):
        if False:
            return 10
        super().check_pt_tf_outputs(tf_outputs=tf_outputs, pt_outputs=pt_outputs, model_class=model_class, tol=tol, name=name, attributes=attributes)
...
class TFSamModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_mask_generation_no_point(self):
        ...
        self.assertTrue(np.allclose(scores[-1].numpy(), np.array(0.4515), atol=0.0002))
        self.assertTrue(np.allclose(masks.numpy(), np.array([-4.1807, -3.4949, -3.4483]), atol=0.01))
    ...
    def test_inference_mask_generation_one_point_one_bb(self):
        ...
        scores = tf.squeeze(outputs.iou_scores)
        masks = outputs.pred_masks[0, 0, 0, 0, :3]
        self.assertTrue(np.allclose(scores[-1], np.array(0.9566), atol=0.0002))
        self.assertTrue(np.allclose(masks.numpy(), np.array([-12.7657, -12.3683, -12.5985]), atol=0.02))
    ...
    def test_inference_mask_generation_batched_points_batched_images(self):
        ...
        scores = tf.squeeze(outputs.iou_scores)
        masks = outputs.pred_masks[0, 0, 0, 0, :3]
        EXPECTED_SCORES = np.array([[[0.6765, 0.9379, 0.8803], [0.6765, 0.9379, 0.8803], [0.6765, 0.9379, 0.8803], [0.6765, 0.9379, 0.8803]], [[0.3317, 0.7264, 0.7646], [0.6765, 0.9379, 0.8803], [0.6765, 0.9379, 0.8803], [0.6765, 0.9379, 0.8803]]])
        EXPECTED_MASKS = np.array([-2.8552, -2.799, -2.9612])
        self.assertTrue(np.allclose(scores.numpy(), EXPECTED_SCORES, atol=0.001))
        self.assertTrue(np.allclose(masks.numpy(), EXPECTED_MASKS, atol=0.03))
    ...
    def test_inference_mask_generation_one_point_one_bb_zero(self):
        ...
        input_points = [[[820, 1080]]]
        labels = [[0]]
        inputs = processor(images=raw_image, input_boxes=input_boxes, input_points=input_points, input_labels=labels, return_tensors='tf')
        outputs = model(**inputs)
        scores = tf.squeeze(outputs.iou_scores)
        self.assertTrue(np.allclose(scores[-1].numpy(), np.array(0.7894), atol=0.0001))
    ...
    def test_inference_mask_generation_one_point(self):
        ...
        outputs = model(**inputs)
        scores = tf.squeeze(outputs.iou_scores)
        self.assertTrue(np.allclose(scores[-1], np.array(0.9675), atol=0.0001))
        input_points = [[[400, 650]]]
        inputs = processor(images=raw_image, input_points=input_points, return_tensors='tf')
        outputs = model(**inputs)
        scores = tf.squeeze(outputs.iou_scores)
        self.assertTrue(np.allclose(scores[-1].numpy(), np.array(0.9675), atol=0.0001))
    ...
    def test_inference_mask_generation_two_points(self):
        ...
        outputs = model(**inputs)
        scores = tf.squeeze(outputs.iou_scores)
        self.assertTrue(np.allclose(scores[-1].numpy(), np.array(0.9762), atol=0.0001))
        inputs = processor(images=raw_image, input_points=input_points, return_tensors='tf')
        outputs = model(**inputs)
        scores = tf.squeeze(outputs.iou_scores)
        self.assertTrue(np.allclose(scores[-1].numpy(), np.array(0.9762), atol=0.0001))
    ...
    def test_inference_mask_generation_two_points_batched(self):
        ...
        input_labels = [[1, 1], [1]]
        inputs = processor(images=[raw_image, raw_image], input_points=input_points, input_labels=input_labels, return_tensors='tf')
        outputs = model(**inputs)
        scores = tf.squeeze(outputs.iou_scores)
        self.assertTrue(np.allclose(scores[0][-1].numpy(), np.array(0.9762), atol=0.0001))
        self.assertTrue(np.allclose(scores[1][-1], np.array(0.9637), atol=0.0001))
    ...
    def test_inference_mask_generation_one_box(self):
        ...
        outputs = model(**inputs)
        scores = tf.squeeze(outputs.iou_scores)
        self.assertTrue(np.allclose(scores[-1].numpy(), np.array(0.7937), atol=0.0001))
    ...
    def test_inference_mask_generation_batched_image_one_point(self):
        ...
        self.assertTrue(np.allclose(scores_batched[1, :].numpy(), scores_single.numpy(), atol=0.0001))
    ...
    def test_inference_mask_generation_two_points_point_batch(self):
        ...
        self.assertTrue(np.allclose(iou_scores.numpy(), np.array([[[0.9105, 0.9825, 0.9675], [0.7646, 0.7943, 0.7774]]]), atol=0.0001, rtol=0.0001))
    ...
    def test_inference_mask_generation_three_boxes_point_batch(self):
        ...
        EXPECTED_IOU = np.array([[[0.9773, 0.9881, 0.9522], [0.5996, 0.7661, 0.7937], [0.5996, 0.7661, 0.7937]]])
        input_boxes = tf.expand_dims(input_boxes, 0)
        inputs = processor(raw_image, input_boxes=input_boxes, return_tensors='tf')
        outputs = model(**inputs)
        iou_scores = outputs.iou_scores
        self.assertTrue(iou_scores.shape == (1, 3, 3))
        self.assertTrue(np.allclose(iou_scores.numpy(), EXPECTED_IOU, atol=0.0001, rtol=0.0001))