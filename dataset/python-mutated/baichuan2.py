...
try:
    ...
    xops = None
    logger.warning('Xformers is not installed correctly. If you want to use memory_efficient_attention to accelerate training use the following command to install Xformers\npip install xformers.')
...
def baichuan_13b_rms_norm_forward(self, hidden_states):
    ...
    if hidden_states.device.type == 'xpu' and (not (self.training and hidden_states.requires_grad)):
        if get_ipex_version() <= '2.0.110+xpu':
            ...
            (hidden_states, _) = torch.ops.torch_ipex.fast_rms_norm(hidden_states, [self.weight.size(0)], self.weight, None, self.epsilon)
    else:
...
def baichuan_attention_forward_7b(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_value: Optional[Tuple[torch.Tensor]]=None, output_attentions: bool=False, use_cache: bool=False) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
    ...
    if xops is not None and self.training:
        ...
        attn_output = xops.memory_efficient_attention(query_states, key_states, value_states, attn_bias=xops.LowerTriangularMask())
    else:
