...
import yaml
from filters import cartesian_prod_mapping, to_composite_grad_opmaker_name, to_input_name, to_int_array_tensor_name, to_int_array_tensors_name, to_op_attr_type, to_opmaker_name, to_opmaker_name_cstr, to_pascal_case, to_scalar_tensor_name, to_variable_names
from generate_op import add_compat_name, add_fluid_name
from jinja2 import Environment, FileSystemLoader, StrictUndefined
from parse_utils import to_named_dict
from tests_utils import is_base_op, is_composite_op, is_initializer_list, is_scalar, is_vec, supports_inplace, supports_no_need_buffer
file_loader = FileSystemLoader(Path(__file__).parent / 'templates')
env = Environment(loader=file_loader, keep_trailing_newline=True, trim_blocks=True, lstrip_blocks=True, undefined=StrictUndefined, extensions=['jinja2.ext.do'])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate operator file from op yaml.')
    parser.add_argument('--ops_yaml_path', type=str, help='parsed static ops yaml file.')
    parser.add_argument('--backward_yaml_path', type=str, help='parsed static backward ops yaml file.')
    parser.add_argument('--op_compat_yaml_path', type=str, help='ops args compat yaml file.')
    parser.add_argument('--op_version_yaml_path', type=str, help='ops version yaml file.')
    parser.add_argument('--output_op_path', type=str, help='path to save generated operators.')
    parser.add_argument('--output_arg_map_path', type=str, help='path to save generated argument mapping functions.')
    args = parser.parse_args()
    main(args.ops_yaml_path, args.backward_yaml_path, args.op_compat_yaml_path, args.op_version_yaml_path, args.output_op_path, args.output_arg_map_path)