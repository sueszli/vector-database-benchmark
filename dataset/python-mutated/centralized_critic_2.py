...
if __name__ == '__main__':
    ...
    ModelCatalog.register_custom_model('cc_model', YetAnotherTorchCentralizedCriticModel if args.framework == 'torch' else YetAnotherCentralizedCriticModel)
    action_space = Discrete(2)
    observer_space = Dict({'own_obs': Discrete(6), 'opponent_obs': Discrete(6), 'opponent_action': Discrete(2)})
    config = PPOConfig().experimental(_enable_new_api_stack=False).environment(TwoStepGame).framework(args.framework).rollouts(batch_mode='complete_episodes', num_rollout_workers=0, enable_connectors=False).callbacks(FillInActions).training(model={'custom_model': 'cc_model'}).multi_agent(policies={'pol1': (None, observer_space, action_space, {}), 'pol2': (None, observer_space, action_space, {})}, policy_mapping_fn=lambda agent_id, episode, worker, **kwargs: 'pol1' if agent_id == 0 else 'pol2', observation_fn=central_critic_observer).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    stop = {'training_iteration': args.stop_iters, 'timesteps_total': args.stop_timesteps, 'episode_reward_mean': args.stop_reward}
    tuner = tune.Tuner('PPO', param_space=config.to_dict(), run_config=air.RunConfig(stop=stop, verbose=1))
    results = tuner.fit()
    if args.as_test:
