...
class CommandObj:
    ...
    async def get_responses(self, ctx):
        intro = _('Welcome to the interactive random {cc} maker!\nEvery message you send will be added as one of the random responses to choose from once this {cc} is triggered. To exit this interactive menu, type `{quit}`').format(cc='customcommand', quit='exit()')
        ...
        while True:
            ...
            if msg.content.lower() == 'exit()':
                ...
                await ctx.send(_("The text response you're trying to create has more than 2000 characters.\nI cannot send messages that are longer than 2000 characters, please try again."))
                continue
            else:
                try:
    ...
    async def create(self, ctx: commands.Context, command: str, *, response: Union[str, List[str]]):
        ...
        ccinfo = {'author': {'id': author.id, 'name': str(author)}, 'command': command, 'cooldowns': {}, 'created_at': self.get_now(), 'editors': [], 'response': response}
        await self.db(ctx.guild).commands.set_raw(command, value=ccinfo)

    async def edit(self, ctx: commands.Context, command: str, *, response=None, cooldowns: Mapping[str, int]=None, ask_for: bool=True):
        ...
        if ask_for and (not response):
            ...
            if pred.result is True:
                ...
                try:
                    resp = await self.bot.wait_for('message', check=MessagePredicate.same_context(ctx), timeout=180)
                except asyncio.TimeoutError:
                    await ctx.send(_('Response timed out, please try again later.'))
                    raise CommandNotEdited()
...
class CustomCommands(commands.Cog):
        ...
        if isinstance(command['response'], str):
            ...
            if await ctx.embed_requested():
                ...
                for (number, response) in enumerate(command['response'], start=1):
                    ...
                    if len(raw) > 2048:
                        raw = f'{raw[:2045]}...'
                    embed = discord.Embed(title=_('Response #{num}/{total}').format(num=number, total=len(command['response'])), description=raw, colour=colour)
    ...
    async def cc_search(self, ctx: commands.Context, *, query):
        ...
        **Arguments:**
        ...
        extracted = rapidfuzz.process.extract(query, list(cc_commands.keys()), processor=rapidfuzz.utils.default_process)
        accepted = []
        ...
        if await ctx.embed_requested():
            ...
            embed = discord.Embed(title=_('Search results'), description=content, colour=await ctx.embed_colour())
    ...
    async def cc_create_random(self, ctx: commands.Context, command: str.lower):
        ...
        **Arguments:**
        ...
        try:
            ...
            await ctx.send(_('This command already exists. Use `{command}` to edit it.').format(command=f'{ctx.clean_prefix}customcom edit'))
    ...
    async def cc_cooldown(self, ctx, command: str.lower, cooldown: int=None, *, per: str.lower='member'):
        ...
        **Arguments:**
        ...
        if cooldown is None:
            ...
            if cooldowns:
                ...
                for (per, rate) in cooldowns.items():
                    cooldown.append(_('A {} may call this command every {} seconds').format(per, rate))
        ...
        try:
            ...
            await ctx.send(_("That command doesn't exist. Use `{command}` to add it.").format(command=f'{ctx.clean_prefix}customcom create'))
    ...
    async def cc_list(self, ctx: commands.Context):
        ...
        if not cc_dict:
            await ctx.send(_('There are no custom commands in this server. Use `{command}` to start adding some.').format(command=f'{ctx.clean_prefix}customcom create'))
            return
        ...
        if await ctx.embed_requested():
            ...
            for (idx, page) in enumerate(pages, start=1):
                embed = discord.Embed(title=_('Custom Command List'), description=page, colour=await ctx.embed_colour())
    ...
    async def cc_show(self, ctx, command_name: str):
        ...
        **Arguments:**
        ...
        text = _('Command: {command_name}\nAuthor: {author}\nCreated: {created_at}\nType: {type}\n').format(command_name=command_name, author=author, created_at=cmd['created_at'], type=_type)
