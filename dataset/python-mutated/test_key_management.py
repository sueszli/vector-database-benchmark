...
@pytest.mark.parametrize('error_code, stop_on_action', [(None, None), ('TestException', 'stub_create_key'), ('TestException', 'stub_list_keys'), ('TestException', 'stub_describe_key'), ('TestException', 'stub_disable_key'), ('TestException', 'stub_enable_key'), ('TestException', 'stub_generate_data_key'), ('TestException', 'stub_schedule_key_deletion')])
def test_key_management(make_stubber, stub_runner, monkeypatch, error_code, stop_on_action):
    ...
    inputs = [description, 'n', 'y', key_id, 'y', 'y', 'y']
    ...
    with stub_runner(error_code, stop_on_action) as runner:
        ...
        runner.add(kms_stubber.stub_list_keys, 10, [key_id] * 10, marker='test-token', raise_and_continue=True)
