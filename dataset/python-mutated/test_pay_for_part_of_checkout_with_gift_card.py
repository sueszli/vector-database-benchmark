...
from .utils import checkout_add_promo_code, checkout_complete, checkout_create, checkout_delivery_method_update, raw_checkout_dummy_payment_create

@pytest.mark.e2e
def test_gift_card_partial_payment_for_checkout_core_1103(e2e_logged_api_client, e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_shipping, permission_manage_product_types_and_attributes, permission_manage_gift_card):
    if False:
        for i in range(10):
            print('nop')
    ...
    gift_card = create_gift_card(e2e_staff_api_client, gift_card_amount, 'USD', active=True)
    ...
    checkout_data = checkout_add_promo_code(e2e_logged_api_client, checkout_id, gift_card_code)
    total_gross_amount = checkout_data['totalPrice']['gross']['amount']
    total_with_gift_card = round(calculated_subtotal + shipping_price - gift_card_amount, 2)
