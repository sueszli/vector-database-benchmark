...
import mido
from pedalboard._pedalboard import WrappedBool, strip_common_float_suffixes, normalize_python_parameter_name
import pytest
import pedalboard
import numpy as np
from typing import Optional
TEST_EFFECT_PLUGIN_BASE_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'plugins', 'effect')
TEST_INSTRUMENT_PLUGIN_BASE_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'plugins', 'instrument')
...
AVAILABLE_EFFECT_PLUGINS_IN_TEST_ENVIRONMENT = [os.path.basename(filename) for filename in glob(os.path.join(TEST_EFFECT_PLUGIN_BASE_PATH, platform.system(), '*'))]
AVAILABLE_INSTRUMENT_PLUGINS_IN_TEST_ENVIRONMENT = [os.path.basename(filename) for filename in glob(os.path.join(TEST_INSTRUMENT_PLUGIN_BASE_PATH, platform.system(), '*'))]
if os.getenv('CIBW_TEST_REQUIRES') or os.getenv('CI'):
    AVAILABLE_EFFECT_PLUGINS_IN_TEST_ENVIRONMENT = [f for f in AVAILABLE_EFFECT_PLUGINS_IN_TEST_ENVIRONMENT if 'component' not in f]
    AVAILABLE_INSTRUMENT_PLUGINS_IN_TEST_ENVIRONMENT = [f for f in AVAILABLE_INSTRUMENT_PLUGINS_IN_TEST_ENVIRONMENT if 'component' not in f]
...
AVAILABLE_PLUGINS_IN_TEST_ENVIRONMENT = AVAILABLE_EFFECT_PLUGINS_IN_TEST_ENVIRONMENT + AVAILABLE_INSTRUMENT_PLUGINS_IN_TEST_ENVIRONMENT
...
def is_container_plugin(filename: str):
    ...
    for klass in pedalboard._AVAILABLE_PLUGIN_CLASSES:
        try:
            ...
            pass
    return False
AVAILABLE_CONTAINER_EFFECT_PLUGINS_IN_TEST_ENVIRONMENT = [filename for filename in AVAILABLE_EFFECT_PLUGINS_IN_TEST_ENVIRONMENT if is_container_plugin(filename)]
