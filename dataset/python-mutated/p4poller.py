...
class P4Source(base.ReconfigurablePollingChangeSource, util.ComparableMixin):
    ...
    compare_attrs = ('p4port', 'p4user', 'p4passwd', 'p4base', 'p4bin', 'pollInterval', 'pollAtLaunch', 'server_tz', 'pollRandomDelayMin', 'pollRandomDelayMax')
    env_vars = ['P4CLIENT', 'P4PORT', 'P4PASSWD', 'P4USER', 'P4CHARSET', 'P4CONFIG', 'P4TICKETS', 'PATH', 'HOME']
    changes_line_re = re.compile("Change (?P<num>\\d+) on \\S+ by \\S+@\\S+ '.*'$")
    describe_header_re = re.compile('Change \\d+ by (?P<who>\\S+)@\\S+ on (?P<when>.+)$')
    file_re = re.compile('^\\.\\.\\. (?P<path>[^#]+)#\\d+ [/\\w]+$')
    datefmt = '%Y/%m/%d %H:%M:%S'
    parent = None
    last_change = None
    loop = None
