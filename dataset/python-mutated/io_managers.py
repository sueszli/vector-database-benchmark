...
from dagster import AssetKey, AssetMaterialization, ConfigurableIOManagerFactory, InitResourceContext, IOManager
...
class ConfigurableLocalOutputNotebookIOManager(ConfigurableIOManagerFactory):
    ...
    base_dir: Optional[str] = Field(default=None, description='Base directory to use for output notebooks. Defaults to the Dagster instance storage directory if not provided.')
    asset_key_prefix: List[str] = Field(default=[], description='Asset key prefix to apply to assets materialized for output notebooks. Defaults to no prefix.')
