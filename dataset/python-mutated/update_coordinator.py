...
from homeassistant.core import CALLBACK_TYPE, Event, HassJob, HassJobType, HomeAssistant, callback
...
class DataUpdateCoordinator(BaseDataUpdateCoordinatorProtocol, Generic[_DataT]):
    ...
    async def async_config_entry_first_refresh(self) -> None:
        ...
        """
        await self._async_refresh(log_failures=False, raise_on_auth_failed=True, raise_on_entry_error=True)
    ...
    async def _async_refresh(self, log_failures: bool=True, raise_on_auth_failed: bool=False, scheduled: bool=False, raise_on_entry_error: bool=False) -> None:
        ...
        try:
            ...
            if self.last_update_success:
                if log_failures:
                    self.logger.error('Config entry setup failed while fetching %s data: %s', self.name, err)
                self.last_update_success = False
            ...
            if self.last_update_success:
                if log_failures:
                    self.logger.error('Authentication failed while fetching %s data: %s', self.name, err)
                self.last_update_success = False
            ...
            self.last_exception = err
            self.last_update_success = False
            self.logger.exception('Unexpected error fetching %s data: %s', self.name, err)
        else:
            ...
            if log_timing:
                self.logger.debug('Finished fetching %s data in %.3f seconds (success: %s)', self.name, monotonic() - start, self.last_update_success)
...
class BaseCoordinatorEntity(entity.Entity, Generic[_BaseDataUpdateCoordinatorT]):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self.async_on_remove(self.coordinator.async_add_listener(self._handle_coordinator_update, self.coordinator_context))
