...
def upgrade():
    if False:
        i = 10
        return i + 15
    op.add_column('packages', sa.Column('allow_legacy_files', sa.Boolean(), nullable=True))
    op.execute("UPDATE packages SET allow_legacy_files = 't'")
    op.alter_column('packages', 'allow_legacy_files', nullable=False, server_default=sa.text('false'))
