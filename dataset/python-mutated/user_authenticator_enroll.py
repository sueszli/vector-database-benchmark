...
DISALLOWED_NEW_ENROLLMENT_ERR = {'details': 'New enrollments for this 2FA interface are not allowed'}

class TotpRestSerializer(serializers.Serializer):
    otp = serializers.CharField(label='Authenticator token', help_text='Code from authenticator', required=True, max_length=20, trim_whitespace=False)

class SmsRestSerializer(serializers.Serializer):
    phone = serializers.CharField(label='Phone number', help_text='Phone number to send SMS code', required=True, max_length=20, trim_whitespace=False)
    otp = serializers.CharField(label='Authenticator code', help_text='Code from authenticator', required=False, allow_null=True, allow_blank=True, max_length=20, trim_whitespace=False)

class U2fRestSerializer(serializers.Serializer):
    deviceName = serializers.CharField(label='Device name', required=False, allow_null=True, allow_blank=True, max_length=60, trim_whitespace=False, default=lambda : petname.generate(2, ' ', letters=10).title())
