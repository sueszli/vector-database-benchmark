...
def test_period_dtype_mismatch(dtype2):
    ...
    assert find_common_type([dtype, dtype2]) == object
    assert find_common_type([dtype2, dtype]) == object
interval_dtypes = [IntervalDtype(np.int64, 'right'), IntervalDtype(np.float64, 'right'), IntervalDtype(np.uint64, 'right'), IntervalDtype(DatetimeTZDtype(unit='ns', tz='US/Eastern'), 'right'), IntervalDtype('M8[ns]', 'right'), IntervalDtype('m8[ns]', 'right')]
