...
class OrganizationOptionManager(OptionManager['OrganizationOption']):
    ...
    def get_value_bulk(self, instances: Sequence[Organization], key: str) -> Mapping[Organization, Any]:
        if False:
            while True:
                i = 10
    ...
    def get_value(self, organization: Organization, key: str, default: Value | None=None, validate: ValidateFunction | None=None) -> Value:
        if False:
            while True:
                i = 10
    ...
    def set_value(self, organization: Organization, key: str, value: Value) -> bool:
        if False:
            i = 10
            return i + 15
        (inst, created) = self.create_or_update(organization=organization, key=key, values={'value': value})
    ...
    def reload_cache(self, organization_id: int, update_reason: str) -> Mapping[str, Value]:
        ...
        if update_reason != 'organizationoption.get_all_values':
            schedule_invalidate_project_config(organization_id=organization_id, trigger=update_reason)
        cache_key = self._make_key(organization_id)
