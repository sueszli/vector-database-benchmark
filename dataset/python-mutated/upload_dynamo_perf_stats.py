...
ARTIFACT_REGEX = re.compile('test-reports-test-(?P<name>\\w+)-\\d+-\\d+-(?P<runner>[\\w\\.]+)_(?P<job>\\d+).zip')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Upload dynamo perf stats from S3 to Rockset')
    parser.add_argument('--workflow-run-id', type=int, required=True, help='id of the workflow to get perf stats from')
    ...
    parser.add_argument('--head-branch', type=str, required=True, help='Head branch of the workflow')
    args = parser.parse_args()
    perf_stats = upload_dynamo_perf_stats_to_rockset(args.repo, args.workflow_run_id, args.workflow_run_attempt, args.head_branch)
    upload_to_rockset(collection='torch_dynamo_perf_stats', docs=perf_stats, workspace='inductor')