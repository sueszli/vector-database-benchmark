...
from frappe.model import child_table_fields, datetime_fields, default_fields, display_fieldtypes, float_like_fields, get_permitted_fields, table_fields
from frappe.model.docstatus import DocStatus
from frappe.model.naming import set_new_name
from frappe.model.utils.link_count import notify_link_count
from frappe.modules import load_doctype_module
from frappe.utils import cast_fieldtype, cint, compare, cstr, flt, is_a_property, now, sanitize_html, strip_html
from frappe.utils.html_utils import unescape_html
if TYPE_CHECKING:
    from frappe.model.document import Document
D = TypeVar('D', bound='Document')
max_positive_value = {'smallint': 2 ** 15 - 1, 'int': 2 ** 31 - 1, 'bigint': 2 ** 63 - 1}
DOCTYPE_TABLE_FIELDS = [_dict(fieldname='fields', options='DocField'), _dict(fieldname='permissions', options='DocPerm'), _dict(fieldname='actions', options='DocType Action'), _dict(fieldname='links', options='DocType Link'), _dict(fieldname='states', options='DocType State')]
...
class BaseDocument:
    _reserved_keywords = frozenset(('doctype', 'meta', 'flags', 'parent_doc', '_table_fields', '_valid_columns', '_doc_before_save', '_table_fieldnames', '_reserved_keywords', 'permitted_fieldnames', 'dont_update_if_missing'))
