...
from ding.framework.middleware import OffPolicyLearner, StepCollector, interaction_evaluator, CkptSaver, data_pusher, online_logger, termination_checker, eps_greedy_handler
...
def main():
    ...
    with task.start(async_mode=False, ctx=OnlineRLContext()):
        collector_env = BaseEnvManagerV2(env_fn=[lambda : PettingZooEnv(cfg.env) for _ in range(cfg.env.collector_env_num)], cfg=cfg.env.manager)
        evaluator_env = BaseEnvManagerV2(env_fn=[lambda : PettingZooEnv(cfg.env) for _ in range(cfg.env.evaluator_env_num)], cfg=cfg.env.manager)
        ...
        task.use(termination_checker(max_env_step=int(1000000.0)))
        task.run()
