...
SQL_MULTIPLE_STMTS = 'create or replace table user_test (i int); insert into user_test (i) values (200); insert into user_test (i) values (300); select i from user_test order by i;'
_PASSWORD = 'snowflake42'
SINGLE_STMT = 'select i from user_test order by i;'
BASE_CONNECTION_KWARGS: dict = {'login': 'user', 'conn_type': 'snowflake', 'password': 'pw', 'schema': 'public', 'extra': {'database': 'db', 'account': 'airflow', 'warehouse': 'af_wh', 'region': 'af_region', 'role': 'af_role'}}
CONN_PARAMS = {'account': 'airflow', 'application': 'AIRFLOW', 'authenticator': 'snowflake', 'database': 'db', 'password': 'pw', 'region': 'af_region', 'role': 'af_role', 'schema': 'public', 'session_parameters': None, 'user': 'user', 'warehouse': 'af_wh'}
HEADERS = {'Content-Type': 'application/json', 'Authorization': 'Bearer newT0k3n', 'Accept': 'application/json', 'User-Agent': 'snowflakeSQLAPI/1.0', 'X-Snowflake-Authorization-Token-Type': 'KEYPAIR_JWT'}
GET_RESPONSE = {'resultSetMetaData': {'numRows': 10000, 'format': 'jsonv2', 'rowType': [{'name': 'SEQ8()', 'database': '', 'schema': '', 'table': '', 'scale': 0, 'precision': 19}, {'name': 'RANDSTR(1000, RANDOM())', 'database': '', 'schema': '', 'table': ''}], 'partitionInfo': [{'rowCount': 12344, 'uncompressedSize': 14384873}, {'rowCount': 43746, 'uncompressedSize': 43748274, 'compressedSize': 746323}]}, 'code': '090001', 'statementStatusUrl': '/api/v2/statements/{handle}?requestId={id5}&partition=10', 'sqlState': '00000', 'statementHandle': '{handle}', 'message': 'Statement executed successfully.', 'createdOn': 1620151693299}
...
class TestSnowflakeSqlApiHook:
    ...
    async def test_get_sql_api_query_status_async(self, mock_get, mock_geturl_header_params, status_code, response, expected_response):
