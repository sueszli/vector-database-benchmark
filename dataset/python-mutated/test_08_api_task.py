...
class TestApiTask(TestApiBase):
    ...
    uri_prefix = '/api/task'
    server_id = {}
    task_data = {'name': u'提交一个测试上线单', 'project_id': 1, 'servers': u'127.0.0.1,192.168.0.1', 'commit_id': u'a89eb23c', 'branch': u'master', 'file_transmission_mode': 0, 'file_list': u'*.log'}
    task_name_2 = u'The Second bill'
    task_data_2 = {'name': u'The Second Bill', 'project_id': 1, 'servers': u'1,2', 'commit_id': u'a89eb23c', 'branch': u'master', 'file_transmission_mode': 0, 'file_list': u'*.log'}
    task_data_2_update = {'name': u'The Second Bill Edit', 'project_id': 1, 'servers': u'1,2', 'commit_id': u'a89eb23c', 'branch': u'master', 'file_transmission_mode': 0, 'file_list': u'*.log,*.txt'}
    task_data_remove = {'name': u'A Task To Be Removed', 'project_id': 1, 'servers': u'1,2,3', 'commit_id': u'a89eb23c', 'branch': u'master', 'file_transmission_mode': 0, 'file_list': u'*.log'}
    ...
    def test_get_list_query(self, user, testapp, client):
        if False:
            print('Hello World!')
        'test list should create 2 users at least, due to test pagination, searching.'
        query = {'page': 1, 'size': 1, 'kw': self.task_name_2}
        response = {'count': 1}
        resp = client.get('%s/?%s' % (self.uri_prefix, urlencode(query)))
        response_success(resp)
        resp_dict = resp_json(resp)
