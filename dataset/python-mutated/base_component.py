...
from esphome.const import CONF_VISIBLE
...
def NextionName(value):
    ...
    return value
CONFIG_BASE_COMPONENT_SCHEMA = cv.Schema({cv.GenerateID(CONF_NEXTION_ID): cv.use_id(Nextion), cv.Optional(CONF_BACKGROUND_COLOR): cv.use_id(color), cv.Optional(CONF_FOREGROUND_COLOR): cv.use_id(color), cv.Optional(CONF_VISIBLE, default=True): cv.boolean})
CONFIG_TEXT_COMPONENT_SCHEMA = CONFIG_BASE_COMPONENT_SCHEMA.extend(cv.Schema({cv.Required(CONF_COMPONENT_NAME): NextionName, cv.Optional(CONF_FONT_ID): cv.int_range(min=0, max=255)}))
CONFIG_BINARY_SENSOR_SCHEMA = CONFIG_BASE_COMPONENT_SCHEMA.extend(cv.Schema({cv.Optional(CONF_COMPONENT_NAME): NextionName, cv.Optional(CONF_VARIABLE_NAME): NextionName}))
CONFIG_SENSOR_COMPONENT_SCHEMA = CONFIG_BINARY_SENSOR_SCHEMA.extend(cv.Schema({cv.Optional(CONF_FONT_ID): cv.int_range(min=0, max=255)}))
CONFIG_SWITCH_COMPONENT_SCHEMA = CONFIG_SENSOR_COMPONENT_SCHEMA.extend(cv.Schema({cv.Optional(CONF_FOREGROUND_PRESSED_COLOR): cv.use_id(color), cv.Optional(CONF_BACKGROUND_PRESSED_COLOR): cv.use_id(color)}))
...
async def setup_component_core_(var, config, arg):
    if CONF_VARIABLE_NAME in config:
        ...
        cg.add(var.set_variable_name(config[CONF_COMPONENT_NAME], config[CONF_COMPONENT_NAME] + arg))
