import copy
import logging
from fairseq.models import FairseqEncoder, FairseqEncoderModel, FairseqLanguageModel, register_model, register_model_architecture
...
from fairseq.models.speech_to_speech.modules.transformer_decoder_aug import AugTransformerUnitDecoder
from fairseq.models.speech_to_speech.modules.transformer_encoder import TransformerEncoderNoEmb
from fairseq.models.speech_to_speech.s2s_conformer import S2UTConformerModel
from fairseq.models.speech_to_speech.s2s_transformer import TransformerUnitDecoder, base_multitask_text_transformer_decoder_arch, s2ut_architecture_base
...
class UnityConformerModel(S2UTConformerModel):
    ...
    @classmethod
    def build_multitask_decoder(cls, args, tgt_dict, in_dim, is_first_pass_decoder, decoder_layers, decoder_embed_dim, decoder_attention_heads):
        if False:
            while True:
                i = 10
    ...
    def build_decoder(cls, args, tgt_dict, aug_attn=False):
        ...
        return decoder_cls(_args, tgt_dict, embed_tokens)
    ...
    def build_model(cls, args, task):
        ...
        decoder = cls.build_decoder(args, task.target_dictionary, aug_attn=getattr(args, 'synthesizer_augmented_cross_attention', False))
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, prev_output_tokens_mt, tgt_speaker=None, return_all_hiddens=False):
        if False:
            for i in range(10):
                print('nop')
        ...
        if self.synthesizer_encoder is not None:
            t2u_encoder_out = self.synthesizer_encoder(x, mt_decoder_padding_mask, return_all_hiddens=return_all_hiddens)
        else:
