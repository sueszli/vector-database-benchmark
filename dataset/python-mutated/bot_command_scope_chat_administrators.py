...
class BotCommandScopeChatAdministrators(BotCommandScope):
    ...
    """
    type: Literal[BotCommandScopeType.CHAT_ADMINISTRATORS] = BotCommandScopeType.CHAT_ADMINISTRATORS
    'Scope type, must be *chat_administrators*'
    chat_id: Union[int, str]
    'Unique identifier for the target chat or username of the target supergroup (in the format :code:`@supergroupusername`)'
    if TYPE_CHECKING:

        def __init__(__pydantic__self__, *, type: Literal[BotCommandScopeType.CHAT_ADMINISTRATORS]=BotCommandScopeType.CHAT_ADMINISTRATORS, chat_id: Union[int, str], **__pydantic_kwargs: Any) -> None:
            if False:
                return 10
