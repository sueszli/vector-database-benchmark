...
def upgrade():
    ...
    with op.batch_alter_table('flow_run', schema=None) as batch_op:
        batch_op.drop_constraint('fk_flow_run__deployment_id__deployment', type_='foreignkey')
...
def downgrade():
    ...
    op.execute('PRAGMA foreign_keys=OFF')
    with op.batch_alter_table('flow_run', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_flow_run__deployment_id__deployment', 'deployment', ['deployment_id'], ['id'])
    op.execute('PRAGMA foreign_keys=ON')