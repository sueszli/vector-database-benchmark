...
if __name__ == '__main__':
    ...
    parser.add_argument('--clean_notebooks', action='store_true', default=False, help='Whether to clean existing notebooks in docs/tutorial. Notebooks are copied from docs/jupyter to docs/tutorial.')
    parser.add_argument('--execute_notebooks', default='auto', choices=('auto', 'always', 'never'), help='Jupyter notebook execution mode.')
    parser.add_argument('--delete_notebooks', action='store_true', default=False, help='Delete all *.ipynb files recursively in the output folder. This is for CI only, please use with caution.')
    parser.add_argument('--py_api_rst', default='always', choices=('always', 'never'), help='Build Python API documentation in reST format.')
    parser.add_argument('--py_example_rst', default='always', choices=('always', 'never'), help='Build Python example documentation in reST format.')
    parser.add_argument('--sphinx', action='store_true', default=False, help='Build Sphinx for main docs and Python API docs.')
    parser.add_argument('--doxygen', action='store_true', default=False, help='Build Doxygen for C++ API docs.')
    parser.add_argument('--is_release', action='store_true', default=False, help='Show Open3D version number rather than git hash.')
    parser.add_argument('--parallel', action='store_true', default=False, help='Enable parallel Sphinx build.')
    ...
    if args.sphinx:
        ...
        skip_notebooks = args.execute_notebooks == 'never' and args.clean_notebooks
        sdb = SphinxDocsBuilder(pwd, html_output_dir, args.is_release, skip_notebooks, args.parallel)
        sdb.run()
    else:
