...
from typing import TYPE_CHECKING, Any, Callable, Iterable, Iterator, List, Mapping, Optional, Sequence, Tuple, TypeVar, Union, cast
import click
import tomli
...
from dagster._core.workspace.load_target import CompositeTarget, EmptyWorkspaceTarget, GrpcServerTarget, ModuleTarget, PackageTarget, PyProjectFileTarget, PythonFileTarget, WorkspaceFileTarget, WorkspaceLoadTarget
...
def are_all_keys_empty(kwargs: ClickArgMapping, keys: Iterable[str]) -> bool:
    ...
    for key in keys:
        if kwargs.get(key):
            return False
    return True
WORKSPACE_CLI_ARGS = ('workspace', 'python_file', 'working_directory', 'package_name', 'module_name', 'attribute', 'repository_yaml', 'grpc_host', 'grpc_port', 'grpc_socket')
