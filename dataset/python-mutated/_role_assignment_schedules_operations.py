...
def build_get_request(scope: str, role_assignment_schedule_name: str, **kwargs: Any) -> HttpRequest:
    ...
    _url = kwargs.pop('template_url', '/{scope}/providers/Microsoft.Authorization/roleAssignmentSchedules/{roleAssignmentScheduleName}')
    path_format_arguments = {'scope': _SERIALIZER.url('scope', scope, 'str', skip_quote=True), 'roleAssignmentScheduleName': _SERIALIZER.url('role_assignment_schedule_name', role_assignment_schedule_name, 'str')}
...
class RoleAssignmentSchedulesOperations:
    ...
    def get(self, scope: str, role_assignment_schedule_name: str, **kwargs: Any) -> _models.RoleAssignmentSchedule:
        ...
        request = build_get_request(scope=scope, role_assignment_schedule_name=role_assignment_schedule_name, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        ...
        return deserialized
    get.metadata = {'url': '/{scope}/providers/Microsoft.Authorization/roleAssignmentSchedules/{roleAssignmentScheduleName}'}

    @distributed_trace
    def list_for_scope(self, scope: str, filter: Optional[str]=None, **kwargs: Any) -> Iterable['_models.RoleAssignmentSchedule']:
        if False:
            i = 10
            return i + 15
