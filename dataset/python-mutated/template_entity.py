...
from homeassistant.const import CONF_ENTITY_PICTURE_TEMPLATE, CONF_FRIENDLY_NAME, CONF_ICON, CONF_ICON_TEMPLATE, CONF_NAME, STATE_UNKNOWN
from homeassistant.core import CALLBACK_TYPE, Context, HomeAssistant, State, callback, validate_state
from homeassistant.exceptions import TemplateError
import homeassistant.helpers.config_validation as cv
from homeassistant.helpers.entity import Entity
from homeassistant.helpers.event import EventStateChangedData, TrackTemplate, TrackTemplateResult, TrackTemplateResultInfo, async_track_template_result
...
TEMPLATE_ENTITY_COMMON_SCHEMA = vol.Schema({vol.Optional(CONF_ATTRIBUTES): vol.Schema({cv.string: cv.template}), vol.Optional(CONF_AVAILABILITY): cv.template}).extend(TEMPLATE_ENTITY_BASE_SCHEMA.schema)
...
class TemplateEntity(Entity):
    ...
    async def async_run_script(self, script: Script, *, run_variables: _VarsType | None=None, context: Context | None=None) -> None:
        ...
        if run_variables is None:
            run_variables = {}
        await script.async_run(run_variables={'this': TemplateStateFromEntityId(self.hass, self.entity_id), **run_variables}, context=context)