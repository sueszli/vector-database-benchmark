...
def tune_tensorflow_mnist(num_workers: int=2, num_samples: int=2, use_gpu: bool=False):
    if False:
        return 10
    trainer = TensorflowTrainer(train_loop_per_worker=train_func, scaling_config=ScalingConfig(num_workers=num_workers, use_gpu=use_gpu))
    tuner = Tuner(trainer, tune_config=TuneConfig(num_samples=num_samples, metric='accuracy', mode='max'), param_space={'train_loop_config': {'lr': tune.loguniform(0.0001, 0.1), 'batch_size': tune.choice([32, 64, 128]), 'epochs': 3}})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-samples', type=int, default=2, help='Sets number of samples for training.')
    ...
    if args.smoke_test:
        ...
        tune_tensorflow_mnist(num_workers=args.num_workers, num_samples=args.num_samples, use_gpu=args.use_gpu)