...
from bokeh.models.dom import ColorRef, Div, Index, Span, Styles, Template, ToggleGroup, ValueRef
...
for ec in df[econf]:
    ...
    while n != 0:
        ...
        (ec, n) = re.subn('\\[(?P<name>\\w+)\\]', replace, ec)
    sc = {}
    for m in re.finditer('(?P<shell>\\d+)\\w(?P<count>\\d+)', ec):
        s = int(m.group('shell'))
        c = int(m.group('count'))
        sc.setdefault(s, 0)
        sc[s] += c
...
cmap = {'alkali metal': '#a6cee3', 'alkaline earth metal': '#1f78b4', 'metal': '#d93b43', 'halogen': '#999d9a', 'metalloid': '#e08d49', 'noble gas': '#eaeaea', 'nonmetal': '#f1d4Af', 'transition metal': '#599d7A'}
TOOLTIPS = [('Name', '@name'), ('Atomic number', '@{atomic number}'), ('Atomic mass', '@{atomic mass}'), ('Type', '@metal'), ('CPK color', '$color[hex, swatch]:CPK'), ('Electronic configuration', '@{electronic configuration}')]
...
p = figure(title='Periodic Table (omitting LA and AC Series)', width=1000, height=450, x_range=groups, y_range=list(reversed(periods)), tools='hover', toolbar_location=None, tooltips=tooltips())
r = p.rect('group', 'period', 0.95, 0.95, source=df, fill_alpha=0.6, legend_field='metal', color=factor_cmap('metal', palette=list(cmap.values()), factors=list(cmap.keys())))
