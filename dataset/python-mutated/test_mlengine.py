...
from airflow.providers.google.cloud.operators.mlengine import MLEngineCreateModelOperator, MLEngineCreateVersionOperator, MLEngineDeleteModelOperator, MLEngineDeleteVersionOperator, MLEngineGetModelOperator, MLEngineListVersionsOperator, MLEngineManageModelOperator, MLEngineManageVersionOperator, MLEngineSetDefaultVersionOperator, MLEngineStartBatchPredictionJobOperator, MLEngineStartTrainingJobOperator, MLEngineTrainingCancelJobOperator
...
TEST_MODEL = {'name': TEST_MODEL_NAME}
TEST_VERSION = {'name': 'v1', 'deploymentUri': 'gs://some-bucket/jobs/test_training/model.pb', 'runtimeVersion': '1.6'}
MLENGINE_AI_PATH = 'airflow.providers.google.cloud.operators.mlengine.{}'

class TestMLEngineBatchPredictionOperator:
    INPUT_MISSING_ORIGIN = {'dataFormat': 'TEXT', 'inputPaths': ['gs://legal-bucket/fake-input-path/*'], 'outputPath': 'gs://legal-bucket/fake-output-path', 'region': 'us-east1'}
    SUCCESS_MESSAGE_MISSING_INPUT = {'jobId': 'test_prediction', 'labels': {'some': 'labels'}, 'predictionOutput': {'outputPath': 'gs://fake-output-path', 'predictionCount': 5000, 'errorCount': 0, 'nodeHours': 2.78}, 'state': 'SUCCEEDED'}
    BATCH_PREDICTION_DEFAULT_ARGS = {'project_id': 'test-project', 'job_id': 'test_prediction', 'labels': {'some': 'labels'}, 'region': 'us-east1', 'data_format': 'TEXT', 'input_paths': ['gs://legal-bucket-dash-Capital/legal-input-path/*'], 'output_path': 'gs://12_legal_bucket_underscore_number/legal-output-path', 'task_id': 'test-prediction'}
...
class TestMLEngineTrainingCancelJobOperator:
    TRAINING_DEFAULT_ARGS = {'project_id': 'test-project', 'job_id': 'test_training', 'task_id': 'test-training'}
...
class TestMLEngineVersionOperator:
    VERSION_DEFAULT_ARGS = {'project_id': 'test-project', 'model_name': 'test-model', 'task_id': 'test-version'}
...
class TestMLEngineStartTrainingJobOperator:
    TRAINING_DEFAULT_ARGS = {'project_id': 'test-project', 'job_id': 'test_training', 'package_uris': ['gs://some-bucket/package1'], 'training_python_module': 'trainer', 'training_args': "--some_arg='aaa'", 'region': 'us-east1', 'scale_tier': 'STANDARD_1', 'labels': {'some': 'labels'}, 'task_id': 'test-training'}
    TRAINING_INPUT = {'jobId': 'test_training', 'labels': {'some': 'labels'}, 'trainingInput': {'scaleTier': 'STANDARD_1', 'packageUris': ['gs://some-bucket/package1'], 'pythonModule': 'trainer', 'args': "--some_arg='aaa'", 'region': 'us-east1'}}
