...
from sentry.incidents.models import INCIDENT_STATUS, AlertRule, AlertRuleThresholdType, Incident, IncidentStatus, IncidentTrigger
from sentry.utils.assets import get_asset_url
from sentry.utils.http import absolute_uri
QUERY_AGGREGATION_DISPLAY = {'count()': 'events', 'count_unique(tags[sentry:user])': 'users affected', 'percentage(sessions_crashed, sessions)': '% sessions crash free rate', 'percentage(users_crashed, users)': '% users crash free rate'}
LOGO_URL = absolute_uri(get_asset_url('sentry', 'images/sentry-email-avatar.png'))
TEXT_COMPARISON_DELTA = {5: 'same time 5 minutes ago', 15: 'same time 15 minutes ago', 60: 'same time one hour ago', 1440: 'same time one day ago', 10080: 'same time one week ago', 43200: 'same time one month ago'}
