...
from flet_runtime.utils import get_package_web_dir, patch_index_html, patch_manifest_json
...
class FletStaticFiles(StaticFiles):
    ...
    * `websocket_endpoint_path` (str, optional) - absolute URL of Flet app WebSocket handler. Default is `{app_mount_path}/ws`.
    ...
    async def __config(self, root_path: str):
        ...
        if self.__assets_dir and os.path.exists(os.path.join(self.__assets_dir, self.index)):
            shutil.copyfile(os.path.join(self.__assets_dir, self.index), os.path.join(temp_dir, self.index))
        else:
            shutil.copyfile(os.path.join(web_dir, self.index), os.path.join(temp_dir, self.index))
        if self.__assets_dir and os.path.exists(os.path.join(self.__assets_dir, self.manifest_json)):
            shutil.copyfile(os.path.join(self.__assets_dir, self.manifest_json), os.path.join(temp_dir, self.manifest_json))
        else:
            shutil.copyfile(os.path.join(web_dir, self.manifest_json), os.path.join(temp_dir, self.manifest_json))
        ws_path = self.__websocket_endpoint_path
        if not ws_path:
            ws_path = self.__app_mount_path.strip('/')
            ws_path = f"{('' if ws_path == '' else '/')}{ws_path}/ws"
        patch_index_html(index_path=os.path.join(temp_dir, self.index), base_href=self.__app_mount_path, websocket_endpoint_path=ws_path, app_name=self.__app_name, app_description=self.__app_description, web_renderer=WebRenderer(self.__web_renderer), use_color_emoji=self.__use_color_emoji, route_url_strategy=self.__route_url_strategy)
        patch_manifest_json(manifest_path=os.path.join(temp_dir, self.manifest_json), app_name=self.__app_name, app_short_name=self.__app_short_name, app_description=self.__app_description)
