...
def create_data(tf_data=False, batch_size=32):
    ...
    if tf_data:
        ...
        valid_data = from_tensor_slices(valid_data).batch(batch_size).cache().prefetch(tf.data.AUTOTUNE)
...
class TestSeq2SeqForecaster(TestCase):
    ...
    def setUp(self):
        ...
        self.forecaster = Seq2SeqForecaster(past_seq_len=10, future_seq_len=2, input_feature_num=10, output_feature_num=2)
    ...
    def test_seq2seq_forecaster_evaluate(self):
        ...
        forecaster = Seq2SeqForecaster.from_tsdataset(train_tsdata, past_seq_len=24, future_seq_len=2, lstm_hidden_dim=16, lstm_layer_num=2)
    ...
    def test_s2s_customized_loss_metric(self):
        ...
        from bigdl.chronos.forecaster.tf.seq2seq_forecaster import Seq2SeqForecaster
        self.forecaster = Seq2SeqForecaster(past_seq_len=10, future_seq_len=2, input_feature_num=10, output_feature_num=2, loss=loss, metrics=[customized_metric], lr=0.01)
        self.forecaster.fit(train_data, epochs=2, batch_size=32)
        yhat = self.forecaster.predict(test_data)
        with tempfile.TemporaryDirectory() as tmp_dir_file:
            tmp_dir_file = os.path.join(tmp_dir_file, 'seq2seq.ckpt')
    ...
    def test_s2s_from_tsdataset(self):
        ...
        s2s = Seq2SeqForecaster.from_tsdataset(train, lstm_hidden_dim=16, lstm_layer_num=2)
        s2s.fit(train, epochs=2, batch_size=32)
        yhat = s2s.predict(test, batch_size=32)
        test.roll(lookback=s2s.model_config['past_seq_len'], horizon=s2s.model_config['future_seq_len'])
        (_, y_test) = test.to_numpy()
        assert yhat.shape == y_test.shape
        del s2s
        (train, _, test) = create_tsdataset(roll=False)
        s2s = Seq2SeqForecaster.from_tsdataset(train, past_seq_len=24, future_seq_len=2, lstm_hidden_dim=16, lstm_layer_num=2)
        s2s.fit(train, epochs=2, batch_size=32)
    ...
    def test_s2s_forecaster_distributed(self):
        ...
        forecaster = Seq2SeqForecaster(past_seq_len=10, future_seq_len=2, input_feature_num=10, output_feature_num=2, loss='mae', distributed=True)
    ...
    def test_s2s_forecaster_distributed_illegal_input(self):
        ...
        from bigdl.orca import init_orca_context, stop_orca_context
        init_orca_context(cores=4, memory='4g')
        forecaster = Seq2SeqForecaster(past_seq_len=10, future_seq_len=2, input_feature_num=2, output_feature_num=2, loss='mae', distributed=True)
        (train_data, _, test_data) = create_data(tf_data=True)
