...
import io
default_marks = [pytest.mark.skipif(not (o3d._build_config['BUILD_TENSORFLOW_OPS'] or o3d._build_config['BUILD_PYTORCH_OPS']), reason='ml ops not built')]
MLModules = namedtuple('MLModules', ['module', 'ops', 'layers', 'classes', 'device', 'cpu_device', 'device_is_gpu'])
_ml_modules = {}
try:
    import warnings
    ...
    _ml_modules['tf'] = MLModules(tf, ml3d_ops, ml3d_layers, None, 'CPU:0', 'CPU:0', False)
    ...
    if tf_gpu_devices and o3d._build_config['BUILD_CUDA_MODULE']:
        _ml_modules['tf_gpu'] = MLModules(tf, ml3d_ops, ml3d_layers, None, 'GPU:0', 'CPU:0', True)
except ImportError:
    pass
try:
    ...
    _ml_modules['torch'] = MLModules(torch, ml3d_ops, ml3d_layers, ml3d_classes, 'cpu', 'cpu', False)
    if torch.cuda.is_available() and o3d._build_config['BUILD_CUDA_MODULE']:
        _ml_modules['torch_cuda'] = MLModules(torch, ml3d_ops, ml3d_layers, ml3d_classes, 'cuda', 'cpu', True)
except ImportError:
    pass
