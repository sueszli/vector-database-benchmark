...
class SetColorNonStroking(CanvasOperator):
    ...
    def get_number_of_operands(self) -> int:
        ...
        if non_stroke_color_space == 'DeviceRGB':
            return 3
        if isinstance(non_stroke_color_space, typing.List) and len(non_stroke_color_space) == 4 and (non_stroke_color_space[0] == 'Separation'):
            return 1
    ...
    def invoke(self, canvas_stream_processor: 'CanvasStreamProcessor', operands: typing.List[AnyPDFType]=[], event_listeners: typing.List['EventListener']=[]) -> None:
        ...
        if non_stroke_color_space == 'DeviceCMYK':
            ...
            canvas.graphics_state.non_stroke_color = CMYKColor(operands[0], operands[1], operands[2], operands[3])
            return
        if non_stroke_color_space == 'DeviceGray':
            assert isinstance(operands[0], Decimal), 'Operand 0 of scn must be a Decimal'
        ...
        if non_stroke_color_space == 'DeviceRGB':
            ...
            return
        if isinstance(non_stroke_color_space, typing.List) and non_stroke_color_space[0] == 'Separation':
            assert isinstance(operands[0], Decimal), 'Operand 0 of scn must be a Decimal'
