...
class MirrorcreatorCom(BaseDecrypter):
    ...
    __version__ = '0.03'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?(?:mirrorcreator\\.com/(?:files/|download\\.php\\?uid=)|mir\\.cr/)(?P<ID>\\w{8})'
    __config__ = [('enabled', 'bool', 'Activated', True), ('hosters_priority', 'str', 'Prefered hoster priority (bar-separated)', ''), ('ignored_hosters', 'str', 'Ignored hoster list (bar-separated)', ''), ('grab_all', 'bool', 'Grab all URLs (default only first match)', False)]
    __description__ = 'Mirrorcreator.com decrypter plugin'
    __license__ = 'GPLv3'
    __authors__ = [('GammaC0de', 'nitzo2001[AT]yahoo[DOT]com')]
    URL_REPLACEMENTS = [(__pattern__ + '.*', 'https://www.mirrorcreator.com/download.php?uid=\\g<ID>')]
    OFFLINE_PATTERN = '>Unfortunately, the link you have clicked is not available|>Error - Link disabled or is invalid|>Links Unavailable as the File Belongs to Suspended Account\\\\. <|>Links Unavailable\\\\.<'
    LINK_PATTERN = '<div class="highlight redirecturl">(.+?)<'
