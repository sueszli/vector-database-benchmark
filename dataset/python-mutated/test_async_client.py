...
@pytest.mark.parametrize('url', [pytest.param('invalid://example.org', id='scheme-not-http(s)'), pytest.param('://example.org', id='no-scheme'), pytest.param('http://', id='no-host')])
@pytest.mark.anyio
...
async def test_context_managed_transport():
    ...
    async with httpx.AsyncClient(transport=transport):
        pass
    assert transport.events == ['transport.__aenter__', 'transport.aclose', 'transport.__aexit__']
...
async def test_context_managed_transport_and_mount():
    ...
    async with httpx.AsyncClient(transport=transport, mounts={'http://www.example.org': mounted}):
        pass
    assert transport.events == ['transport.__aenter__', 'transport.aclose', 'transport.__aexit__']
    assert mounted.events == ['mounted.__aenter__', 'mounted.aclose', 'mounted.__aexit__']
