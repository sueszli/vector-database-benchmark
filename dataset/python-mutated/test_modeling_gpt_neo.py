...
if is_torch_available():
    import torch
    from transformers import GPT_NEO_PRETRAINED_MODEL_ARCHIVE_LIST, GPT2Tokenizer, GPTNeoForCausalLM, GPTNeoForQuestionAnswering, GPTNeoForSequenceClassification, GPTNeoForTokenClassification, GPTNeoModel
...
class GPTNeoModelTester:
    ...
    def create_and_check_gpt_neo_model_attention_mask_past(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        if False:
            print('Hello World!')
        model = GPTNeoModel(config=config)
        model.to(torch_device)
    ...
    def create_and_check_gpt_neo_model_past_large_inputs(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        if False:
            for i in range(10):
                print('nop')
        model = GPTNeoModel(config=config)
    ...
    def create_and_check_gpt_neo_for_question_answering(self, config, input_ids, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, *args):
        if False:
            i = 10
            return i + 15
    ...
    def create_and_check_gpt_neo_for_sequence_classification(self, config, input_ids, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, *args):
        if False:
            return 10
        config.num_labels = self.num_labels
    ...
    def create_and_check_gpt_neo_for_token_classification(self, config, input_ids, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, *args):
        if False:
            print('Hello World!')
        config.num_labels = self.num_labels
...
class GPTNeoModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (GPTNeoModel, GPTNeoForCausalLM, GPTNeoForQuestionAnswering, GPTNeoForSequenceClassification, GPTNeoForTokenClassification) if is_torch_available() else ()
    all_generative_model_classes = (GPTNeoForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': GPTNeoModel, 'question-answering': GPTNeoForQuestionAnswering, 'text-classification': GPTNeoForSequenceClassification, 'text-generation': GPTNeoForCausalLM, 'token-classification': GPTNeoForTokenClassification, 'zero-shot': GPTNeoForSequenceClassification} if is_torch_available() else {}
    fx_compatible = True
    ...
    def _get_hidden_states(self):
        if False:
            print('Hello World!')
        return torch.tensor([[[0.4983, -0.7584, -1.6944, 0.544], [2.6918, 0.4206, 0.4176, 0.2055], [-0.0071, -0.0405, -1.492, -0.363], [1.0492, 0.1599, -1.7648, 0.2419], [-1.8348, 2.0514, -0.1946, 0.3203], [0.7672, -1.16, -1.7118, -0.9056], [0.2986, 0.5372, 0.7729, -0.1927], [0.0285, 0.2629, -1.1156, -1.1992]]], dtype=torch.float32, device=torch_device)
...
class GPTNeoModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_batch_generation(self):
        ...
        sentences = ['Hello, my dog is a little', 'Today, I am']
        ...
        expected_output_sentence = ['Hello, my dog is a little bit of a kitty. She is a very sweet and loving', 'Today, I am going to talk about the best way to get a job in the']
