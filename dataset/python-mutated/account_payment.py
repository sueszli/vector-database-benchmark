...
class AccountPayment(models.Model):
    ...
    payment_token_id = fields.Many2one('payment.token', string='Saved payment token', domain=[('acquirer_id.auto_confirm', '!=', 'authorize')], help='Note that tokens from acquirers set to only authorize transactions (instead of capturing the amount) are not available.')
