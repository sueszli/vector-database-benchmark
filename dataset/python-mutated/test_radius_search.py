...
def test_radius_search(dtype, ml, num_points_queries, metric, ignore_query_point, return_distances, normalize_distances, index_dtype):
    if False:
        print('Hello World!')
    ...
    layer = ml.layers.RadiusSearch(metric=metric, ignore_query_point=ignore_query_point, return_distances=return_distances, normalize_distances=normalize_distances, index_dtype=index_dtype_)
    ans = mltest.run_op(ml, ml.device, True, layer, points, queries=queries, radii=radii)
