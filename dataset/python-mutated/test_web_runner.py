...
@pytest.mark.skipif(platform.system() == 'Windows', reason='the test is not valid for Windows')
...
@pytest.mark.parametrize(('value', 'expected'), ((2, 2), (None, 5), ('2', 2)))
...
@pytest.mark.skipif(platform.system() != 'Windows', reason='Proactor Event loop present only in Windows')
async def test_named_pipe_runner_wrong_loop(app: Any, selector_loop: Any, pipe_name: Any) -> None:
    runner = web.AppRunner(app)
    await runner.setup()
    with pytest.raises(RuntimeError):
...
@pytest.mark.skipif(platform.system() != 'Windows', reason='Proactor Event loop present only in Windows')
async def test_named_pipe_runner_proactor_loop(proactor_loop: Any, app: Any, pipe_name: Any) -> None:
    runner = web.AppRunner(app)
    await runner.setup()
