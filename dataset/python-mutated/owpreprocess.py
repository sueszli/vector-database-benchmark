...
import numpy
from AnyQt.QtWidgets import QWidget, QButtonGroup, QGroupBox, QRadioButton, QSlider, QDoubleSpinBox, QComboBox, QSpinBox, QListView, QLabel, QScrollArea, QVBoxLayout, QHBoxLayout, QFormLayout, QSizePolicy, QApplication, QCheckBox
...
from Orange.preprocess import Continuize, ProjectPCA, RemoveNaNRows, ProjectCUR, Scale as _Scale, Randomize as _Randomize, RemoveSparse
...
from Orange.widgets.data.utils.preprocess import BaseEditor, blocked, StandardItemModel, DescriptionRole, ParametersRole, Controller, SequenceFlow
...
class DiscretizeEditor(BaseEditor):
    ...
    Discretizers = {NoDisc: (None, {}), EqualWidth: (preprocess.discretize.EqualWidth, {'n': 4}), EqualFreq: (preprocess.discretize.EqualFreq, {'n': 4}), Drop: (_NoneDisc, {}), EntropyMDL: (preprocess.discretize.EntropyMDL, {'force': False})}
    Names = {NoDisc: 'None', EqualWidth: 'Equal width discretization', EqualFreq: 'Equal frequency discretization', Drop: 'Remove numeric features', EntropyMDL: 'Entropy-MDL discretization'}
...
class ContinuizeEditor(BaseEditor):
    ...
    Continuizers = OrderedDict([(Continuize.FrequentAsBase, 'Most frequent is base'), (Continuize.Indicators, 'One feature per value'), (Continuize.RemoveMultinomial, 'Remove non-binary features'), (Continuize.Remove, 'Remove categorical features'), (Continuize.AsOrdinal, 'Treat as ordinal'), (Continuize.AsNormalizedOrdinal, 'Divide by number of values')])
...
class ImputeEditor(BaseEditor):
    (NoImputation, Constant, Average, Model, Random, DropRows, DropColumns) = (0, 1, 2, 3, 4, 5, 6)
    Imputers = {NoImputation: (None, {}), Average: (preprocess.impute.Average(), {}), Random: (preprocess.impute.Random(), {}), DropRows: (None, {})}
    Names = {NoImputation: "Don't impute.", Constant: 'Replace with constant', Average: 'Average/Most frequent', Model: 'Model based imputer', Random: 'Replace with random value', DropRows: 'Remove rows with missing values.'}
...
class FeatureSelectEditor(BaseEditor):
    MEASURES = [('Information Gain', preprocess.score.InfoGain), ('Gain ratio', preprocess.score.GainRatio), ('Gini index', preprocess.score.Gini), ('ReliefF', preprocess.score.ReliefF), ('Fast Correlation Based Filter', preprocess.score.FCBF), ('ANOVA', preprocess.score.ANOVA), ('Chi2', preprocess.score.Chi2), ('RReliefF', preprocess.score.RReliefF), ('Univariate Linear Regression', preprocess.score.UnivariateLinearRegression)]
...
class Scale(BaseEditor):
    (CenterByMean, ScaleBySD, NormalizeBySD, NormalizeBySpan_ZeroBased, NormalizeSpan_NonZeroBased) = (0, 1, 2, 3, 4)
    Names = {NormalizeBySD: 'Standardize to μ=0, σ²=1', CenterByMean: 'Center to μ=0', ScaleBySD: 'Scale to σ²=1', NormalizeSpan_NonZeroBased: 'Normalize to interval [-1, 1]', NormalizeBySpan_ZeroBased: 'Normalize to interval [0, 1]'}
...
PREPROCESS_ACTIONS = [PreprocessAction('Discretize', 'orange.preprocess.discretize', 'Discretization', Description('Discretize Continuous Variables', icon_path('Discretize.svg')), DiscretizeEditor), PreprocessAction('Continuize', 'orange.preprocess.continuize', 'Continuization', Description('Continuize Discrete Variables', icon_path('Continuize.svg')), ContinuizeEditor), PreprocessAction('Impute', 'orange.preprocess.impute', 'Impute', Description('Impute Missing Values', icon_path('Impute.svg')), ImputeEditor), PreprocessAction('Feature Selection', 'orange.preprocess.fss', 'Feature Selection', Description('Select Relevant Features', icon_path('SelectColumns.svg')), FeatureSelectEditor), PreprocessAction('Random Feature Selection', 'orange.preprocess.randomfss', 'Random Feature Selection', Description('Select Random Features', icon_path('SelectColumnsRandom.svg')), RandomFeatureSelectEditor), PreprocessAction('Normalize', 'orange.preprocess.scale', 'Scale', Description('Normalize Features', icon_path('Normalize.svg')), Scale), PreprocessAction('Randomize', 'orange.preprocess.randomize', 'Randomization', Description('Randomize', icon_path('Random.svg')), Randomize), PreprocessAction('Remove Sparse', 'orange.preprocess.remove_sparse', 'Feature Selection', Description('Remove Sparse Features', icon_path('PurgeDomain.svg')), RemoveSparseEditor), PreprocessAction('PCA', 'orange.preprocess.pca', 'PCA', Description('Principal Component Analysis', icon_path('PCA.svg')), PCA), PreprocessAction('CUR', 'orange.preprocess.cur', 'CUR', Description('CUR Matrix Decomposition', icon_path('SelectColumns.svg')), CUR)]
