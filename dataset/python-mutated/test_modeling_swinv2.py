...
class Swinv2ModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (Swinv2Model, Swinv2ForImageClassification, Swinv2ForMaskedImageModeling) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': Swinv2Model, 'image-classification': Swinv2ForImageClassification} if is_torch_available() else {}
    fx_compatible = False
    test_pruning = False
...
class Swinv2ModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        if False:
            while True:
                i = 10
        return AutoImageProcessor.from_pretrained('microsoft/swinv2-tiny-patch4-window8-256') if is_vision_available() else None
    ...
    def test_inference_image_classification_head(self):
        if False:
            i = 10
            return i + 15
        model = Swinv2ForImageClassification.from_pretrained('microsoft/swinv2-tiny-patch4-window8-256').to(torch_device)
