...
from homeassistant.components.bluetooth.active_update_coordinator import _T, ActiveBluetoothDataUpdateCoordinator
...
async def test_bleak_error_during_polling(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
...
async def test_generic_exception_during_polling(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
...
async def test_polling_debounce(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    """Test basic usage of the ActiveBluetoothDataUpdateCoordinator."""
    await async_setup_component(hass, DOMAIN, {DOMAIN: {}})
    poll_count = 0
...
async def test_polling_debounce_with_custom_debouncer(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
    """Test basic usage of the ActiveBluetoothDataUpdateCoordinator."""
    await async_setup_component(hass, DOMAIN, {DOMAIN: {}})
    poll_count = 0
    ...
    async def _poll_method(service_info: BluetoothServiceInfoBleak) -> dict[str, Any]:
        ...
        await asyncio.sleep(0.0001)
        return {'poll_count': poll_count}
    coordinator = MyCoordinator(hass=hass, logger=_LOGGER, address='aa:bb:cc:dd:ee:ff', mode=BluetoothScanningMode.ACTIVE, needs_poll_method=_needs_poll, poll_method=_poll_method, poll_debouncer=Debouncer(hass, _LOGGER, cooldown=0.1, immediate=True))
    assert coordinator.available is False
    mock_listener = MagicMock()
    unregister_listener = coordinator.async_add_listener(mock_listener)
    cancel = coordinator.async_start()
...
async def test_polling_rejecting_the_first_time(hass: HomeAssistant, mock_bleak_scanner_start: MagicMock, mock_bluetooth_adapters: None) -> None:
