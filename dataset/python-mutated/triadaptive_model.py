...
class TriAdaptiveModel(nn.Module):
    ...
    def __init__(self, language_model1: LanguageModel, language_model2: LanguageModel, language_model3: LanguageModel, prediction_heads: List[PredictionHead], embeds_dropout_prob: float=0.1, device: Optional[torch.device]=None, lm1_output_types: Optional[Union[str, List[str]]]=None, lm2_output_types: Optional[Union[str, List[str]]]=None, lm3_output_types: Optional[Union[str, List[str]]]=None, loss_aggregation_fn: Optional[Callable]=None):
        if False:
            i = 10
            return i + 15
    ...
    def save(self, save_dir: Union[str, Path], lm1_name: str='lm1', lm2_name: str='lm2', lm3_name: str='lm3'):
        ...
        for (name, model) in zip([lm1_name, lm2_name, lm3_name], [self.language_model1, self.language_model2, self.language_model3]):
    ...
    @classmethod
    def load(cls, load_dir: Path, device: torch.device, strict: bool=False, lm1_name: str='lm1', lm2_name: str='lm2', lm3_name: str='lm3', processor: Optional[Processor]=None):
        if False:
            for i in range(10):
                print('nop')
            ...
            for (head, lm1_out, lm2_out) in zip(self.prediction_heads, self.lm1_output_types, self.lm2_output_types):
                if pooled_output[0] is not None:
                    if lm1_out == 'per_sequence' or lm1_out == 'per_sequence_continuous':
                        ...
                        raise ValueError('Unknown extraction strategy from TriAdaptive language_model1: {}'.format(lm1_out))
                else:
                    output1 = None
                if pooled_output[1] is not None:
                    if lm2_out == 'per_sequence' or lm2_out == 'per_sequence_continuous':
                        ...
                        raise ValueError('Unknown extraction strategy from TriAdaptive language_model2: {}'.format(lm2_out))
                else:
                    output2 = None
    ...
    def forward_lm(self, **kwargs):
        ...
        if 'query_input_ids' in kwargs.keys():
            (pooled_output1, _) = self.language_model1(input_ids=kwargs.get('query_input_ids'), segment_ids=kwargs.get('query_segment_ids'), attention_mask=kwargs.get('query_attention_mask'), output_hidden_states=False, output_attentions=False)
        ...
        if 'passage_input_ids' in kwargs.keys():
            ...
            if all(table_mask):
                (pooled_output2, _) = self.language_model3(input_ids=passage_input_ids, segment_ids=passage_segment_ids, attention_mask=passage_attention_mask, output_hidden_states=False, output_attentions=False)
                ...
                (pooled_output_tables, _) = self.language_model3(input_ids=table_input_ids, segment_ids=table_segment_ids, attention_mask=table_attention_mask, output_hidden_states=False, output_attentions=False)
                ...
                (pooled_output_text, _) = self.language_model2(input_ids=text_input_ids, segment_ids=text_segment_ids, attention_mask=text_attention_mask, output_hidden_states=False, output_attentions=False)
                last_table_idx = 0
                last_text_idx = 0
                combined_outputs = []
                ...
                assert pooled_output_tables.shape[-1] == pooled_output_text.shape[-1], 'Passage embedding model and table embedding model use different embedding sizes'
                pooled_output_combined = combined_outputs.view(-1, embedding_size)
                pooled_output[1] = pooled_output_combined
            else:
                (pooled_output2, _) = self.language_model2(input_ids=passage_input_ids, attention_mask=passage_attention_mask, segment_ids=passage_segment_ids, output_hidden_states=False, output_attentions=False)
                pooled_output[1] = pooled_output2
    ...
    def verify_vocab_size(self, vocab_size1: int, vocab_size2: int, vocab_size3: int):
        ...
        msg = f"Vocab size of tokenizer {vocab_size3} doesn't match with model {model3_vocab_len}. If you added a custom vocabulary to the tokenizer, make sure to supply 'n_added_tokens' to get_language_model() and BertStyleLM.load()"
