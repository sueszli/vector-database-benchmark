...
featureType = {0: 'Null', 1: 'Point', 3: 'PolyLine', 5: 'Polygon', 8: 'MultiPoint', 11: 'PointZ', 13: 'PolyLineZ', 15: 'PolygonZ', 18: 'MultiPointZ', 21: 'PointM', 23: 'PolyLineM', 25: 'PolygonM', 28: 'MultiPointM', 31: 'MultiPatch'}
...
class IMPORTGIS_OT_shapefile_props_dialog(Operator):
    ...
    def listObjects(self, context):
        ...
        return objs
    reprojection: BoolProperty(name='Specifiy shapefile CRS', description="Specifiy shapefile CRS if it's different from scene CRS", default=False)
    shpCRS: EnumProperty(name='Shapefile CRS', description='Choose a Coordinate Reference System', items=listPredefCRS)
    vertsElevSource: EnumProperty(name='Elevation source', description='Select the source of vertices z value', items=[('NONE', 'None', 'Flat geometry'), ('GEOM', 'Geometry', 'Use z value from shape geometry if exists'), ('FIELD', 'Field', 'Extract z elevation value from an attribute field'), ('OBJ', 'Object', 'Get z elevation value from an existing ground mesh')], default='GEOM')
    ...
    fieldElevName: EnumProperty(name='Elev. field', description='Choose field', items=listFields)
    useFieldExtrude: BoolProperty(name='Extrusion from field', description='Extract z extrusion value from an attribute field', default=False)
    fieldExtrudeName: EnumProperty(name='Field', description='Choose field', items=listFields)
    extrusionAxis: EnumProperty(name='Extrude along', description='Select extrusion axis', items=[('Z', 'z axis', 'Extrude along Z axis'), ('NORMAL', 'Normal', 'Extrude along normal')])
    separateObjects: BoolProperty(name='Separate objects', description='Warning : can be very slow with lot of features', default=False)
    useFieldName: BoolProperty(name='Object name from field', description='Extract name for created objects from an attribute field', default=False)
...
class IMPORTGIS_OT_shapefile(Operator):
    ...
    elevSource: StringProperty(name='Elevation source', description='Elevation source', default='GEOM')
    ...
    extrusionAxis: EnumProperty(name='Extrude along', description='Select extrusion axis', items=[('Z', 'z axis', 'Extrude along Z axis'), ('NORMAL', 'Normal', 'Extrude along normal')])
    separateObjects: BoolProperty(name='Separate objects', description='Import to separate objects instead one large object', default=False)
    ...
    def execute(self, context):
        ...
        return {'FINISHED'}
classes = [IMPORTGIS_OT_shapefile_file_dialog, IMPORTGIS_OT_shapefile_props_dialog, IMPORTGIS_OT_shapefile]
