...
all_nonoptional_primitives = [np.int8, np.int16, np.int32, np.int64, np.float32, np.float64, bool, bytes, str]
...
basic_map_types = [Mapping[key_type, value_type] for (key_type, value_type) in itertools.product(all_primitives, all_primitives)]
...
@parameterized_class([{'pickler': pickle}, {'pickler': dill}, {'pickler': cloudpickle}])
