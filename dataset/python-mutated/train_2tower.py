...
numeric_cols = ['enaging_user_follower_count', 'enaging_user_following_count', 'engaged_with_user_follower_count', 'engaged_with_user_following_count', 'len_hashtags', 'len_domains', 'len_links']
cat_cols = ['engaged_with_user_is_verified', 'enaging_user_is_verified', 'present_media', 'tweet_type', 'language']
ratio_cols = ['engaged_with_user_follower_following_ratio', 'enaging_user_follower_following_ratio']
embed_cols = ['hashtags', 'present_links', 'present_domains']
id_cols = ['enaging_user_id', 'tweet_id', 'engaged_with_user_id']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backend', type=str, default='ray', choices=('spark', 'ray'), help='The backend of Orca Estimator, either ray or spark.')
    ...
    parser.add_argument('--model_dir', default='recsys_2tower', type=str, help='model directory name (default: recsys_2tower)')
    parser.add_argument('--data_dir', type=str, help='data directory of processed features for the two tower model')
    ...
    if args.cluster_mode == 'local':
        ...
        sc = init_orca_context('standalone', master=args.master, cores=args.executor_cores, num_nodes=args.num_executors, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, conf=spark_conf)
    elif args.cluster_mode == 'yarn':
        sc = init_orca_context('yarn-client', cores=args.executor_cores, num_nodes=args.num_executors, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, conf=spark_conf, extra_python_lib='model.py', object_store_memory='80g')
    ...
    train_config = {'lr': 0.001, 'user_col_info': user_info, 'item_col_info': item_info, 'inter_op_parallelism': 4, 'intra_op_parallelism': args.executor_cores}
    train(train_config, train_tbl, test_tbl, epochs=args.epochs, batch_size=args.batch_size, model_dir=args.model_dir, backend=args.backend)
