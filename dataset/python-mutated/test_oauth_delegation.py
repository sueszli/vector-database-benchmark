...
from signedjson.key import encode_verify_key_base64, generate_signing_key, get_verify_key
from signedjson.sign import sign_json
from twisted.test.proto_helpers import MemoryReactor
from synapse.api.errors import AuthError, Codes, InvalidClientTokenError, OAuthInsufficientScopeError, SynapseError
...
async def get_json(url: str) -> JsonDict:
    if url == WELL_KNOWN:
        return {'issuer': ISSUER, 'authorization_endpoint': AUTHORIZATION_ENDPOINT, 'token_endpoint': TOKEN_ENDPOINT, 'jwks_uri': JWKS_URI, 'userinfo_endpoint': USERINFO_ENDPOINT, 'introspection_endpoint': INTROSPECTION_ENDPOINT, 'response_types_supported': ['code'], 'subject_types_supported': ['public'], 'id_token_signing_alg_values_supported': ['RS256']}
    elif url == JWKS_URI:
        return {'keys': []}
    return {}
...
class MSC3861OAuthDelegation(HomeserverTestCase):
    servlets = [account.register_servlets, devices.register_servlets, keys.register_servlets, register.register_servlets, login.register_servlets, logout.register_servlets, admin.register_servlets]
