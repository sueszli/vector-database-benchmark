...
class TestModerations:
    ...
    def test_method_create_with_all_params(self, client: OpenAI) -> None:
        if False:
            for i in range(10):
                print('nop')
        moderation = client.moderations.create(input='I want to kill them.', model='text-moderation-stable')
        assert_matches_type(ModerationCreateResponse, moderation, path=['response'])
    ...
    def test_raw_response_create(self, client: OpenAI) -> None:
        if False:
            print('Hello World!')
        response = client.moderations.with_raw_response.create(input='I want to kill them.')
...
class TestAsyncModerations:
    ...
    async def test_method_create(self, client: AsyncOpenAI) -> None:
        moderation = await client.moderations.create(input='I want to kill them.')
        assert_matches_type(ModerationCreateResponse, moderation, path=['response'])

    @parametrize
    async def test_method_create_with_all_params(self, client: AsyncOpenAI) -> None:
        moderation = await client.moderations.create(input='I want to kill them.', model='text-moderation-stable')
        assert_matches_type(ModerationCreateResponse, moderation, path=['response'])

    @parametrize
    async def test_raw_response_create(self, client: AsyncOpenAI) -> None:
        response = await client.moderations.with_raw_response.create(input='I want to kill them.')
        assert response.http_request.headers.get('X-Stainless-Lang') == 'python'
        moderation = response.parse()
        assert_matches_type(ModerationCreateResponse, moderation, path=['response'])