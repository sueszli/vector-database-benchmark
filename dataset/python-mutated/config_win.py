        ...
        for d in self.dlls:
            if not getattr(d, '_configured', False):
                ...
                if d.found and d.link and (not d.link.lib_dir):
                    ...
                    print('Building lib from {}: {}.lib...'.format(os.path.basename(d.lib_dir), basename))
...
def setup():
    ...
    DEPS.add('SDL', 'SDL2', ['SDL2-[1-9].*'], '(lib){0,1}SDL2\\.dll$', required=1)
    DEPS.add('PORTMIDI', 'portmidi', ['portmidi'], 'portmidi\\.dll$', find_header='portmidi\\.h')
    DEPS.add_dummy('PORTTIME')
    DEPS.add('MIXER', 'SDL2_mixer', ['SDL2_mixer-[1-9].*'], '(lib){0,1}SDL2_mixer\\.dll$', ['SDL'])
    DEPS.add('PNG', 'png', ['SDL2_image-[2-9].*', 'libpng-[1-9].*'], '(png|libpng)[-0-9]*\\.dll$', ['z'], find_header='png\\.h', find_lib='(lib)?png1[-0-9]*\\.lib')
    DEPS.add('JPEG', 'jpeg', ['SDL2_image-[2-9].*', 'jpeg-9*'], '(lib){0,1}jpeg-9\\.dll$', find_header='jpeglib\\.h', find_lib='(lib)?jpeg-9\\.lib')
    DEPS.add('IMAGE', 'SDL2_image', ['SDL2_image-[1-9].*'], '(lib){0,1}SDL2_image\\.dll$', ['SDL', 'jpeg', 'png', 'tiff'], 0)
    DEPS.add('FONT', 'SDL2_ttf', ['SDL2_ttf-[2-9].*'], '(lib){0,1}SDL2_ttf\\.dll$', ['SDL', 'z', 'freetype'])
    DEPS.add('FREETYPE', 'freetype', ['freetype'], 'freetype[-0-9]*\\.dll$', find_header='ft2build\\.h', find_lib='freetype[-0-9]*\\.lib')
    DEPS.configure()
...
def setup_prebuilt_sdl2(prebuilt_dir):
    ...
    Dependency.lib_hunt.extend(['', 'lib', os.path.join('lib', get_machine_type()), os.path.join('lib', get_machine_type(), 'optional')])
    Dependency.inc_hunt.append('')
    DEPS = DependencyGroup()
    DEPS.add('SDL', 'SDL2', ['SDL2-[1-9].*'], '(lib){0,1}SDL2\\.dll$', required=1)
    fontDep = DEPS.add('FONT', 'SDL2_ttf', ['SDL2_ttf-[2-9].*'], '(lib){0,1}SDL2_ttf\\.dll$', ['SDL'])
    imageDep = DEPS.add('IMAGE', 'SDL2_image', ['SDL2_image-[1-9].*'], '(lib){0,1}SDL2_image\\.dll$', ['SDL', 'jpeg', 'png', 'tiff'], 0)
    mixerDep = DEPS.add('MIXER', 'SDL2_mixer', ['SDL2_mixer-[1-9].*'], '(lib){0,1}SDL2_mixer\\.dll$', ['SDL'])
    DEPS.add('PORTMIDI', 'portmidi', ['portmidi'], 'portmidi\\.dll$', find_header='portmidi\\.h')
    DEPS.add_dummy('PORTTIME')
    DEPS.configure()
    DEPS.add('FREETYPE', 'freetype', ['freetype'], 'freetype[-0-9]*\\.dll$', find_header='ft2build\\.h', find_lib='freetype[-0-9]*\\.lib')
    png = DEPS.add('PNG', 'png', ['SDL2_image-[2-9].*', 'libpng-[1-9].*'], '(png|libpng)[-0-9]*\\.dll$', ['z'], find_header='png\\.h', find_lib='(lib)?png1[-0-9]*\\.lib')
    png.path = imageDep.path
    png.inc_dir = [os.path.join(prebuilt_dir, 'include').replace('\\', '/')]
    png.found = True
    jpeg = DEPS.add('JPEG', 'jpeg', ['SDL2_image-[2-9].*', 'jpeg-9*'], '(lib){0,1}jpeg-9\\.dll$', find_header='jpeglib\\.h', find_lib='(lib)?jpeg-9\\.lib')
    jpeg.path = imageDep.path
    jpeg.inc_dir = [os.path.join(prebuilt_dir, 'include').replace('\\', '/')]
    jpeg.found = True
    dllPaths = {'png': imageDep.path, 'jpeg': imageDep.path, 'tiff': imageDep.path, 'z': imageDep.path, 'webp': imageDep.path, 'ogg': mixerDep.path, 'modplug': mixerDep.path, 'opus': mixerDep.path, 'opusfile': mixerDep.path}
    _add_sdl2_dll_deps(DEPS)
...
def main(auto_config=False):
    ...
    download_kwargs = {'x86': False, 'x64': False}
