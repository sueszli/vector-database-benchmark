...
classifier = PyTorchClassifier(model=model, clip_values=(min_pixel_value, max_pixel_value), preprocessing=(cifar_mu, cifar_std), loss=criterion, optimizer=opt, input_shape=(3, 32, 32), nb_classes=10)
proxy_classifier = PyTorchClassifier(model=proxy_model, clip_values=(min_pixel_value, max_pixel_value), preprocessing=(cifar_mu, cifar_std), loss=criterion, optimizer=proxy_opt, input_shape=(3, 32, 32), nb_classes=10)
attack = ProjectedGradientDescent(classifier, norm=np.inf, eps=8.0 / 255.0, eps_step=2.0 / 255.0, max_iter=10, targeted=False, num_random_init=1, batch_size=128, verbose=False)
...
print('Accuracy on benign test samples after adversarial training: %.2f%%' % (np.sum(x_test_pred == np.argmax(y_test, axis=1)) / x_test.shape[0] * 100))
attack_test = ProjectedGradientDescent(classifier, norm=np.inf, eps=8.0 / 255.0, eps_step=2.0 / 255.0, max_iter=20, targeted=False, num_random_init=1, batch_size=128, verbose=False)
x_test_attack = attack_test.generate(x_test, y=y_test)
x_test_attack_pred = np.argmax(classifier.predict(x_test_attack), axis=1)
print('Accuracy on original PGD adversarial samples after adversarial training: %.2f%%' % (np.sum(x_test_attack_pred == np.argmax(y_test, axis=1)) / x_test.shape[0] * 100))