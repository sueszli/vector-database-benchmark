...
FLAGS = flags.FLAGS
flags.DEFINE_string('input_tflite_file', None, 'Full path name to the input TFLite model file.')
flags.DEFINE_string('output_source_file', None, 'Full path name to the output TFLite Micro model (C++ Source) file).')
flags.DEFINE_string('output_header_file', None, 'Full filepath of the output C header file.')
flags.DEFINE_string('array_variable_name', None, 'Name to use for the C data array variable.')
flags.DEFINE_integer('line_width', 80, 'Width to use for formatting.')
flags.DEFINE_string('include_guard', None, 'Name to use for the C header include guard.')
flags.DEFINE_string('include_path', None, 'Optional path to include in generated source file.')
flags.DEFINE_boolean('use_tensorflow_license', False, 'Whether to prefix the generated files with the TF Apache2 license.')
