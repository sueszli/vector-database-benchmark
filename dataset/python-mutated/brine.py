...
Example::
    ...
    True
"""
from network.lib.rpc.lib.compat import Struct, BytesIO, is_py3k, BYTES_LITERAL
...
TAG_LONG = BYTES_LITERAL('\t')
TAG_STR1 = BYTES_LITERAL('\n')
TAG_STR2 = BYTES_LITERAL('\x0b')
TAG_STR3 = BYTES_LITERAL('\x0c')
TAG_STR4 = BYTES_LITERAL('\r')
...
if is_py3k:
    IMM_INTS = dict(((i, bytes([i + 80])) for i in range(-48, 160)))
    xrange = range
else:
    IMM_INTS = dict(((i, chr(i + 80)) for i in range(-48, 160)))
I1 = Struct('!B')
I4 = Struct('!L')
F8 = Struct('!d')
C16 = Struct('!dd')
...
if is_py3k:
    ...
    simple_types = frozenset([type(None), int, long, bool, float, str, unicode, complex, type(NotImplemented), type(Ellipsis)])
