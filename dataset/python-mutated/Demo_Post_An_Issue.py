...
def _github_issue_post_make_markdown(issue_type, operating_system, os_ver, psg_port, psg_ver, gui_ver, python_ver, python_exp, prog_exp, used_gui, gui_notes, cb_docs, cb_demos, cb_demo_port, cb_readme_other, cb_command_line, cb_issues, cb_github, detailed_desc, code):
    if False:
        while True:
            i = 10
...
def _github_issue_help():
    ...
    layout = [[sg.T('Goals', font=heading_font, pad=(0, 0))], [HelpText(help_goals)], [sg.T('Why?', font=heading_font, pad=(0, 0))], [HelpText(help_why)], [sg.T('FAQ', font=heading_font, pad=(0, 0))], [HelpText(help_explain)], [sg.T('Experience (optional)', font=heading_font)], [HelpText(help_experience)], [sg.T('Steps', font=heading_font, pad=(0, 0))], [HelpText(help_steps)], [sg.B('Close')]]
...
def main_open_github_issue():
    ...
    v_size = (15, 1)
    frame_versions = [[sg.T('Python', size=v_size), sg.In(sg.sys.version, size=(20, 1), k='-VER PYTHON-')], [sg.T('PySimpleGUI', size=v_size), sg.In(sg.ver, size=(20, 1), k='-VER PSG-')], [sg.T('tkinter', size=v_size), sg.In(sg.tclversion_detailed, size=(20, 1), k='-VER TK-')]]
    frame_platforms = [[sg.T('OS                 '), sg.T('Details')], [sg.Radio('Windows', 2, sg.running_windows(), size=(8, 1), k='-OS WIN-'), sg.In(size=(8, 1), k='-OS WIN VER-')], [sg.Radio('Linux', 2, sg.running_linux(), size=(8, 1), k='-OS LINUX-'), sg.In(size=(8, 1), k='-OS LINUX VER-')], [sg.Radio('Mac', 2, sg.running_mac(), size=(8, 1), k='-OS MAC-'), sg.In(size=(8, 1), k='-OS MAC VER-')], [sg.Radio('Other', 2, size=(8, 1), k='-OS OTHER-'), sg.In(size=(8, 1), k='-OS OTHER VER-')]]
    frame_experience = [[sg.T('Optional Experience Info')], [sg.In(size=(4, 1), k='-EXP PROG-'), sg.T('Years Programming')], [sg.In(size=(4, 1), k='-EXP PYTHON-'), sg.T('Years Writing Python')], [sg.CB('Previously programmed a GUI', k='-CB PRIOR GUI-')], [sg.T('Share more if you want....')], [sg.In(size=(25, 1), k='-EXP NOTES-')]]
    checklist = (('Searched main docs for your problem', 'www.PySimpleGUI.org'), ('Looked for Demo Programs that are similar to your goal ', 'http://Demos.PySimpleGUI.org'), ('If not tkinter - looked for Demo Programs for specific port', ''), ('For non tkinter - Looked at readme for your specific port if not PySimpleGUI (Qt, WX, Remi)', ''), ('Run your program outside of your debugger (from a command line)', ''), ('Searched through Issues (open and closed) to see if already reported', 'http://Issues.PySimpleGUI.org'), ('Tried using the PySimpleGUI.py file on GitHub. Your problem may have already been fixed vut not released.', ''))
    ...
    top_layout = [[sg.Col([[sg.Text('Open A GitHub Issue (* = Required Info)', font='_ 15')]], expand_x=True), sg.Col([[sg.B('Help')]])], [sg.Frame('Title *', [[sg.Input(k='-TITLE-', size=(50, 1), font='_ 14', focus=True)]], font=font_frame)], sg.vtop([sg.Frame('Platform *', frame_platforms, font=font_frame), sg.Frame('Type of Issue *', frame_type, font=font_frame), sg.Frame('Versions *', frame_versions, font=font_frame), sg.Frame('Experience', frame_experience, font=font_frame)]), [sg.Frame('Checklist * (note that you can click the links)', frame_checklist, font=font_frame)], [sg.HorizontalSeparator()], [sg.T(sg.SYMBOL_DOWN + ' If you need more room for details grab the dot and drag to expand', background_color='red', text_color='white')]]
    bottom_layout = [[sg.TabGroup([[sg.Tab('Details', frame_details), sg.Tab('Code', frame_code), sg.Tab('Markdown', frame_markdown)]], k='-TABGROUP-')], [sg.Text(size=(12, 1), key='-OUT-')]]
    layout_pane = sg.Pane([sg.Col(top_layout), sg.Col(bottom_layout)], key='-PANE-')
    layout = [[layout_pane], [sg.Col([[sg.B('Post Issue'), sg.B('Create Markdown Only'), sg.B('Quit')]], expand_x=False, expand_y=False)]]
    ...
    while True:
        ...
        if event in (sg.WINDOW_CLOSE_ATTEMPTED_EVENT, 'Quit'):
            if sg.popup_yes_no('Do you really want to exit?', 'If you have not clicked Post Issue button and then clicked "Submit New Issue" button then your issue will not have been submitted to GitHub.Do no exit until you have PASTED the information from Markdown tab into an issue?') == 'Yes':
                break
        ...
        if event == 'Help':
            ...
            if not _github_issue_post_validate(values, checklist, issue_types):
                continue
            markdown = _github_issue_post_make_markdown(issue_type, operating_system, os_ver, 'tkinter', values['-VER PSG-'], values['-VER TK-'], values['-VER PYTHON-'], values['-EXP PYTHON-'], values['-EXP PROG-'], 'Yes' if values['-CB PRIOR GUI-'] else 'No', values['-EXP NOTES-'], *checkboxes, values['-ML DETAILS-'], values['-ML CODE-'])
