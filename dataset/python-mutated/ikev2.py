...
"""
import struct
from scapy.packet import Packet, Raw, bind_bottom_up, bind_layers, bind_top_down, split_bottom_up
from scapy.fields import ByteEnumField, ByteField, ConditionalField, FieldLenField, FieldListField, FlagsField, IP6Field, IPField, IntField, MultiEnumField, MultipleTypeField, PacketField, PacketLenField, PacketListField, ShortEnumField, ShortField, StrLenField, X3BytesField, XByteField, XStrFixedLenField, XStrLenField
...
class IKEv2(_IKEv2_Packet):
    name = 'IKEv2'
    fields_desc = [XStrFixedLenField('init_SPI', '', 8), XStrFixedLenField('resp_SPI', '', 8), ByteEnumField('next_payload', 0, IKEv2PayloadTypes), XByteField('version', 32), ByteEnumField('exch_type', 0, IKEv2ExchangeTypes), FlagsField('flags', 0, 8, ['res0', 'res1', 'res2', 'Initiator', 'Version', 'Response', 'res6', 'res7']), IntField('id', 0), IntField('length', None)]
...
class IKEv2_Payload(_IKEv2_Packet):
    name = 'IKEv2 Payload'
    fields_desc = [ByteEnumField('next_payload', None, IKEv2PayloadTypes), FlagsField('flags', 0, 8, ['critical']), ShortField('length', None), XStrLenField('load', '', length_from=lambda pkt: pkt.length - 4)]
...
class IKEv2_Transform(IKEv2_Payload):
    ...
    fields_desc = IKEv2_Payload.fields_desc[:2] + [ShortField('length', 8), ByteEnumField('transform_type', None, IKEv2TransformTypes), ByteField('res2', 0), MultiEnumField('transform_id', None, IKEv2TransformAlgorithms, depends_on=lambda pkt: pkt.transform_type, fmt='H'), ConditionalField(IKEv2_Key_Length_Attribute('key_length'), lambda pkt: pkt.length > 8)]

class IKEv2_Proposal(IKEv2_Payload):
    name = 'IKEv2 Proposal'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ByteField('proposal', 1), ByteEnumField('proto', 1, IKEv2ProtocolTypes), FieldLenField('SPIsize', None, 'SPI', 'B'), ByteField('trans_nb', None), XStrLenField('SPI', '', length_from=lambda pkt: pkt.SPIsize), PacketLenField('trans', conf.raw_layer(), IKEv2_Transform, length_from=lambda pkt: pkt.length - 8 - pkt.SPIsize)]

class IKEv2_AUTH(IKEv2_Payload):
    name = 'IKEv2 Authentication'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ByteEnumField('auth_type', None, IKEv2AuthenticationTypes), X3BytesField('res2', 0), XStrLenField('load', '', length_from=lambda pkt: pkt.length - 8)]

class IKEv2_VendorID(IKEv2_Payload):
    name = 'IKEv2 Vendor ID'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [XStrLenField('vendorID', '', length_from=lambda pkt: pkt.length - 4)]
...
class IPv4TrafficSelector(TrafficSelector):
    ...
    fields_desc = [ByteEnumField('TS_type', 7, IKEv2TrafficSelectorTypes), ByteEnumField('IP_protocol_ID', None, IPProtocolIDs), ShortField('length', 16), ShortField('start_port', 0), ShortField('end_port', 65535), IPField('starting_address_v4', '192.168.0.1'), IPField('ending_address_v4', '192.168.0.255')]

class IPv6TrafficSelector(TrafficSelector):
    name = 'IKEv2 IPv6 Traffic Selector'
    fields_desc = [ByteEnumField('TS_type', 8, IKEv2TrafficSelectorTypes), ByteEnumField('IP_protocol_ID', None, IPProtocolIDs), ShortField('length', 20), ShortField('start_port', 0), ShortField('end_port', 65535), IP6Field('starting_address_v6', '2001::'), IP6Field('ending_address_v6', '2001::')]

class EncryptedTrafficSelector(TrafficSelector):
    name = 'IKEv2 Encrypted Traffic Selector'
    fields_desc = [ByteEnumField('TS_type', 9, IKEv2TrafficSelectorTypes), ByteEnumField('IP_protocol_ID', None, IPProtocolIDs), ShortField('length', 16), ByteField('res', 0), X3BytesField('starting_address_FC', 0), ByteField('res2', 0), X3BytesField('ending_address_FC', 0), ByteField('starting_R_CTL', 0), ByteField('ending_R_CTL', 0), ByteField('starting_type', 0), ByteField('ending_type', 0)]
...
class IKEv2_TSi(IKEv2_Payload):
    ...
    fields_desc = IKEv2_Payload.fields_desc[:3] + [FieldLenField('number_of_TSs', None, fmt='B', count_of='traffic_selector'), X3BytesField('res2', 0), PacketListField('traffic_selector', None, TrafficSelector, length_from=lambda pkt: pkt.length - 8, count_from=lambda pkt: pkt.number_of_TSs)]
...
class IKEv2_Delete(IKEv2_Payload):
    name = 'IKEv2 Delete'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ByteEnumField('proto', None, {0: 'Reserved', 1: 'IKE', 2: 'AH', 3: 'ESP'}), FieldLenField('SPIsize', None, 'SPI', 'B'), ShortField('SPInum', 0), FieldListField('SPI', [], XStrLenField('', '', length_from=lambda pkt: pkt.SPIsize), count_from=lambda pkt: pkt.SPInum)]

class IKEv2_SA(IKEv2_Payload):
    name = 'IKEv2 SA'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [PacketLenField('prop', conf.raw_layer(), IKEv2_Proposal, length_from=lambda pkt: pkt.length - 4)]

class IKEv2_Nonce(IKEv2_Payload):
    name = 'IKEv2 Nonce'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [XStrLenField('nonce', '', length_from=lambda pkt: pkt.length - 4)]

class IKEv2_Notify(IKEv2_Payload):
    name = 'IKEv2 Notify'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ByteEnumField('proto', None, IKEv2ProtocolTypes), FieldLenField('SPIsize', None, 'SPI', 'B'), ShortEnumField('type', 0, IKEv2NotifyMessageTypes), XStrLenField('SPI', '', length_from=lambda pkt: pkt.SPIsize), ConditionalField(XStrLenField('notify', '', length_from=lambda pkt: pkt.length - 8 - pkt.SPIsize), lambda pkt: pkt.type not in (16407, 16408)), ConditionalField(ByteEnumField('gw_id_type', 1, IKEv2GatewayIDTypes), lambda pkt: pkt.type in (16407, 16408)), ConditionalField(FieldLenField('gw_id_len', None, 'gw_id', 'B'), lambda pkt: pkt.type in (16407, 16408)), ConditionalField(MultipleTypeField([(IPField('gw_id', '127.0.0.1'), lambda x: x.gw_id_type == 1), (IP6Field('gw_id', '::1'), lambda x: x.gw_id_type == 5)], StrLenField('gw_id', '', length_from=lambda x: x.gw_id_len)), lambda pkt: pkt.type in (16407, 16408)), ConditionalField(XStrLenField('nonce', '', length_from=lambda x: x.length - 10 - x.gw_id_len), lambda pkt: pkt.type == 16407)]

class IKEv2_KE(IKEv2_Payload):
    name = 'IKEv2 Key Exchange'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ShortEnumField('group', 0, IKEv2TransformAlgorithms[4]), ShortField('res2', 0), XStrLenField('ke', '', length_from=lambda pkt: pkt.length - 8)]

class IKEv2_IDi(IKEv2_Payload):
    name = 'IKEv2 Identification - Initiator'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ByteEnumField('IDtype', 1, {1: 'IPv4_addr', 2: 'FQDN', 3: 'Email_addr', 5: 'IPv6_addr', 11: 'Key'}), X3BytesField('res2', 0), MultipleTypeField([(IPField('ID', '127.0.0.1'), lambda pkt: pkt.IDtype == 1), (IP6Field('ID', '::1'), lambda pkt: pkt.IDtype == 5)], XStrLenField('ID', '', length_from=lambda pkt: pkt.length - 8))]

class IKEv2_IDr(IKEv2_Payload):
...
class ConfigurationAttribute(Packet):
    ...
    fields_desc = [ShortEnumField('type', 1, IKEv2ConfigurationAttributeTypes), FieldLenField('length', None, 'value', 'H'), MultipleTypeField([(IPField('value', '127.0.0.1'), lambda pkt: pkt.length == 4 and pkt.type in (1, 2, 3, 4, 6, 20)), (IP6Field('value', '::1'), lambda pkt: pkt.length == 16 and pkt.type in (10, 12, 21))], XStrLenField('value', '', length_from=lambda pkt: pkt.length))]
...
class IKEv2_CP(IKEv2_Payload):
    name = 'IKEv2 Configuration'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ByteEnumField('CFGType', 1, IKEv2ConfigurationPayloadCFGTypes), X3BytesField('res2', 0), PacketListField('attributes', None, ConfigurationAttribute, length_from=lambda pkt: pkt.length - 8)]

class IKEv2_Encrypted_Fragment(IKEv2_Payload):
    name = 'IKEv2 Encrypted and Authenticated Fragment'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ShortField('frag_number', 1), ShortField('frag_total', 1), XStrLenField('load', '', length_from=lambda pkt: pkt.length - 8)]

class IKEv2_CERTREQ(IKEv2_Payload):
    name = 'IKEv2 Certificate Request'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ByteEnumField('cert_encoding', 0, IKEv2CertificateEncodings), XStrLenField('cert_authority', '', length_from=lambda pkt: pkt.length - 5)]

class IKEv2_CERT(IKEv2_Payload):
    name = 'IKEv2 Certificate'
    fields_desc = IKEv2_Payload.fields_desc[:3] + [ByteEnumField('cert_encoding', 4, IKEv2CertificateEncodings), MultipleTypeField([(PacketLenField('cert_data', X509_Cert(), X509_Cert, length_from=lambda pkt: pkt.length - 5), lambda pkt: pkt.cert_encoding == 4), (PacketLenField('cert_data', X509_CRL(), X509_CRL, length_from=lambda pkt: pkt.length - 5), lambda pkt: pkt.cert_encoding == 7)], XStrLenField('cert_data', '', length_from=lambda pkt: pkt.length - 5))]
