...
class UploadshipCom(XFSDownloader):
    ...
    __version__ = '0.02'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?uploadship\\.com/\\w{16}'
    ...
    NAME_PATTERN = '<span style="display: block; line-height: 17px; word-break: break-all;"><small>(?P<N>.+?)</small></span>'
    SIZE_PATTERN = '<i class="fa fa-hdd-o" aria-hidden="true"></td><td>((?P<S>[\\d.,]+) (?P<U>[\\w^_]+)) :'
    WAIT_PATTERN = "\\$\\('\\.download-timer-seconds'\\)\\.html\\((\\d+)\\);"
    FORM_INPUTS_MAP = {'d': re.compile('^1')}
    LINK_PATTERN = 'href="(.+?)" target="_blank" class="btn btn-go">Click Here To Download'
    ERROR_PATTERN = None
    ...
    def handle_free(self, pyfile):
        ...
        if secret is not None:
            ...
            secret_data = self.load('https://www.uploadship.com/' + secret.group(1) + '.php', get=get_data)
            inputs[secret.group(1)] = secret_data
        self.handle_captcha(inputs)
        self.data = self.load(pyfile.url, post=inputs, ref=self.pyfile.url, redirect=False)
