...
DST_DIR = os.path.abspath(os.path.join(DOC_SRC_DIR, 'examples', 'notebooks', 'generated'))
...
parser.add_argument('--fp', type=str, default=None, help='Path to notebook to convert. Converts all notebooks in `directory` by default.')
parser.add_argument('--directory', type=str, default=None, help='Path to notebook directory to convert')
parser.add_argument('--to', type=str, default='html', help="Type to convert to. One of `{'html', 'rst'}`")
parser.add_argument('--timeout', type=int, default=1000, help='Seconds to allow for each cell before timing out')
parser.add_argument('--kernel_name', type=str, default=None, help='Name of kernel to execute with')
parser.add_argument('--skip-execution', dest='skip_execution', action='store_true', help='Skip execution notebooks before converting')
parser.add_argument('--execute-only', dest='execute_only', action='store_true', help='Execute notebooks but do not convert to html')
parser.add_argument('--parallel', dest='parallel', action='store_true', help='Execute notebooks in parallel')
parser.add_argument('--report-errors', dest='report_errors', action='store_true', help='Report errors that occur when executing notebooks')
parser.add_argument('--fail-on-error', dest='error_fail', action='store_true', help='Fail when an error occurs when executing a cell in a notebook.')
parser.add_argument('--skip-existing', dest='skip_existing', action='store_true', help='Skip execution of an executed file exists and is newer than the notebook.')
parser.add_argument('--execution-blacklist', type=str, default=None, help='Comma separated list of notebook names to skip, e.g,slow-notebook.ipynb,other-notebook.ipynb')
parser.set_defaults(parallel=True, skip_execution=False, report_errors=True, error_fail=False, skip_existing=False)
