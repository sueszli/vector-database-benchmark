...
class AnnotationCCodeWriter(CCodeWriter):
    ...
    _onclick_attr = ' onclick="{0}"'.format("(function(s){    s.display =  s.display === 'block' ? 'none' : 'block'})(this.nextElementSibling.style)".replace(' ', ''))
    ...
    def _save_annotation_body(self, cython_code, generated_code, annotation_items, scopes, covered_lines=None):
        ...
        return outlist
_parse_code = re.compile(b'(?P<refnanny>__Pyx_X?(?:GOT|GIVE)REF|__Pyx_RefNanny[A-Za-z]+)|(?P<trace>__Pyx_Trace[A-Za-z]+)|(?:(?P<pyx_macro_api>__Pyx_[A-Z][A-Z_]+)|(?P<pyx_c_api>(?:__Pyx_[A-Z][a-z_][A-Za-z_]*)|__pyx_convert_[A-Za-z_]*)|(?P<py_macro_api>Py[A-Z][a-z]+_[A-Z][A-Z_]+)|(?P<py_c_api>Py[A-Z][a-z]+_[A-Z][a-z][A-Za-z_]*))(?=\\()|(?P<error_goto>(?:(?<=;) *if [^;]* +)?__PYX_ERR\\([^)]+\\))'.decode('ascii')).sub
_replace_pos_comment = re.compile(b'^\\s*/\\*(?:(?:[^*]|\\*[^/])*\\n)+\\s*\\*/\\s*\\n'.decode('ascii'), re.M).sub
