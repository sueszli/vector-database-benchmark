...
class Lesson:
    ...
    async def start(self, filename: str, is_app: bool, q: Q):
        ...
        if is_app:
            ...
            cmd = [sys.executable, '-m', 'uvicorn', '--host', '0.0.0.0', '--port', str(q.app.app_port), f"{filename.replace('.py', '')}:main"]
...
async def setup_page(q: Q):
    ...
    with open(os.path.join(university_dir, 'static', 'university.js'), 'r') as f:
        js_code = f.read()
    template = Template(js_code).substitute(baseURL=_base_url, snippets1=q.app.snippets1, snippets2=q.app.snippets2, py_content=py_content)
    q.page['meta'] = ui.meta_card(box='', title=app_title, scripts=[ui.script(f'{_base_url}assets/loader.min.js')], script=ui.inline_script(content=template, requires=['require'], targets=['monaco-editor']), layouts=[ui.layout(breakpoint='xs', zones=[ui.zone('mobile_header'), ui.zone('main', direction=ui.ZoneDirection.COLUMN, zones=[ui.zone('description'), ui.zone('rhs', size='100vh')]), ui.zone('nav')]), ui.layout(breakpoint='l', zones=[ui.zone('header'), ui.zone('main', size='calc(100vh - 76px)', direction=ui.ZoneDirection.ROW, zones=[ui.zone('description', size='30%'), ui.zone('rhs', size='70%')])])])
    nav_links = [('docs', 'Docs', 'https://wave.h2o.ai/docs/getting-started'), ('discussions', 'Forum', 'https://github.com/h2oai/wave/discussions'), ('cloud', 'AI Cloud', 'https://h2o.ai/platform/ai-cloud/')]
    q.page['header'] = ui.header_card(box='header', title=app_title, subtitle='Learn Wave interactively', image=f'{_base_url}assets/h2o-logo.svg', items=[], secondary_items=[ui.links(inline=True, items=[ui.link(label=link[1], path=link[2], target='_blank') for link in nav_links])])
    q.page['mobile_header'] = ui.header_card(box='mobile_header', title=app_title, subtitle='Learn Wave interactively', image=f'{_base_url}assets/h2o-logo.svg', nav=[ui.nav_group('Links', items=[ui.nav_item(name=link[0], label=link[1]) for link in nav_links])])
    q.page['mobile_nav'] = ui.form_card(box='nav', items=[])
    q.page['code'] = ui.markup_card(box=ui.box('rhs', height='calc((100vh - 80px - 30px) / 2)'), title='', content='<div id="monaco-editor" style="position: absolute; top: 45px; bottom: 15px; right: 15px; left: 15px"/>')
...
async def serve(q: Q):
    if not q.app.initialized:
        ...
        if os.path.exists(base_snippets_path) and os.path.exists(component_snippets_path):
            ...
            (q.app.snippets1, q.app.snippets2) = await q.site.upload([os.path.join(vsc_extension_path, 'base-snippets.json'), os.path.join(vsc_extension_path, 'component-snippets.json')])
