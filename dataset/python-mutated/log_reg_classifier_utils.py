...
from snips_nlu.constants import DATA, ENTITY, INTENTS, TEXT, UNKNOWNWORD, UTTERANCES
...
WORD_REGEX = re.compile('\\w+(\\s+\\w+)*')
UNKNOWNWORD_REGEX = re.compile('%s(\\s+%s)*' % (UNKNOWNWORD, UNKNOWNWORD))
