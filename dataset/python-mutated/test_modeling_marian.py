...
if is_torch_available():
    import torch
    from transformers import AutoConfig, AutoModelWithLMHead, AutoTokenizer, MarianModel, MarianMTModel, TranslationPipeline
    from transformers.models.marian.convert_marian_to_pytorch import ORG_NAME, convert_hf_name_to_opus_name, convert_opus_name_to_hf_name
    from transformers.models.marian.modeling_marian import MarianDecoder, MarianEncoder, MarianForCausalLM, shift_tokens_right
...
class MarianModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'conversational': MarianMTModel, 'feature-extraction': MarianModel, 'summarization': MarianMTModel, 'text-generation': MarianForCausalLM, 'text2text-generation': MarianMTModel, 'translation': MarianMTModel} if is_torch_available() else {}
...
class MarianIntegrationTest(unittest.TestCase):
    src = 'en'
    tgt = 'de'
    src_text = ['I am a small frog.', 'Now I can forget the 100 words of german that I know.', 'Tom asked his teacher for advice.', "That's how I would do it.", "Tom really admired Mary's courage.", 'Turn around and close your eyes.']
    expected_text = ['Ich bin ein kleiner Frosch.', 'Jetzt kann ich die 100 Wörter des Deutschen vergessen, die ich kenne.', 'Tom bat seinen Lehrer um Rat.', 'So würde ich das machen.', 'Tom bewunderte Marias Mut wirklich.', 'Drehen Sie sich um und schließen Sie die Augen.']
...
class TestMarian_EN_FR(MarianIntegrationTest):
    ...
    expected_text = ['Je suis une petite grenouille.', "Maintenant, je peux oublier les 100 mots d'allemand que je connais."]
...
class TestMarian_FR_EN(MarianIntegrationTest):
    src = 'fr'
    tgt = 'en'
    src_text = ['Donnez moi le micro.', 'Tom et Mary étaient assis à une table.']
...
class TestMarian_en_ROMANCE(MarianIntegrationTest):
    ...
    src = 'en'
    tgt = 'ROMANCE'
    src_text = [">>fr<< Don't spend so much time watching TV.", '>>pt<< Your message has been sent.', ">>es<< He's two years older than me."]
    expected_text = ['Ne passez pas autant de temps à regarder la télé.', 'A sua mensagem foi enviada.', 'Es dos años más viejo que yo.']
...
class TestMarian_FI_EN_V2(MarianIntegrationTest):
    src = 'fi'
    tgt = 'en'
    src_text = ['minä tykkään kirjojen lukemisesta', 'Pidän jalkapallon katsomisesta']
