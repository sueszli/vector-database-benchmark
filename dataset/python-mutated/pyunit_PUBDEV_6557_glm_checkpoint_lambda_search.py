...
def glm_alpha_lambda_arrays():
    ...
    for l in range(0, len(r['lambdas'])):
        ...
        pyunit_utils.assertEqualCoeffDicts(cs, m.coef(), tol=0.01)
        pyunit_utils.assertEqualCoeffDicts(cs_norm, m.coef_norm(), tol=0.01)
        ...
        if l == best_submodel_index:
            pyunit_utils.assertEqualModelMetrics(m._model_json['output']['training_metrics'], mL._model_json['output']['training_metrics'])
        else:
