...
if __name__ == '__main__':
    ...
    parser.add_option('-b', '--build', action='append', dest='binfiles', help='build a DFU file from given BINFILES', metavar='BINFILES')
    parser.add_option('-D', '--device', action='store', dest='device', help='build for DEVICE, defaults to %s' % DEFAULT_DEVICE, metavar='DEVICE')
    parser.add_option('-d', '--dump', action='store_true', dest='dump_images', default=False, help='dump contained images to current directory')
    ...
    if options.binfiles and len(args) == 1:
        ...
        for arg in options.binfiles:
            try:
                ...
                sys.exit(1)
            try:
                address = int(address, 0) & 4294967295
            except ValueError:
