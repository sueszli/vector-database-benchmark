...
class InstallationPushEventWebhookTest(APITestCase):
    ...
    def test_simple(self):
        ...
        with assume_test_silo_mode(SiloMode.CONTROL):
            inst = Integration.objects.create(provider='github_apps', external_id='12345', name='dummyorg')
            inst.add_organization(self.project.organization)
        Repository.objects.create(organization_id=project.organization.id, external_id='35129377', provider='github_apps', name='baxterthehacker/public-repo')
        response = self.client.post(path=url, data=PUSH_EVENT_EXAMPLE_INSTALLATION, content_type='application/json', HTTP_X_GITHUB_EVENT='push', HTTP_X_HUB_SIGNATURE='sha1=56a3df597e02adbc17fb617502c70e19d96a6136', HTTP_X_GITHUB_DELIVERY=str(uuid4()))
