...
def test_scoring():
    ...
    assert round(score1, 2) == 0.97
    assert round(score2, 2) == 0.95
    (f, p) = combined_ftest_5x2cv(estimator1=clf1, estimator2=clf2, X=X, y=y, scoring='accuracy', random_seed=1)
    assert round(f, 3) == 1.053, f
    assert round(p, 3) == 0.509, p
    (f, p) = combined_ftest_5x2cv(estimator1=clf1, estimator2=clf2, X=X, y=y, scoring='f1_macro', random_seed=1)
    if Version(sklearn_version) < Version('0.20'):
        assert round(f, 3) == -1.51, f
...
def test_regressor():
    ...
    (f, p) = combined_ftest_5x2cv(estimator1=reg1, estimator2=reg2, X=X, y=y, random_seed=1)
