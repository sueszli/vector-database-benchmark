...
class TestGumbel(unittest.TestCase):
    ...
    def test_sample(self):
        ...
        np.testing.assert_allclose(self.samples.mean(axis=0), scipy.stats.gumbel_r.mean(self.loc, scale=self.scale), rtol=0.1, atol=config.ATOL.get(str(self.scale.dtype)))
        np.testing.assert_allclose(self.samples.var(axis=0), scipy.stats.gumbel_r.var(self.loc, scale=self.scale), rtol=0.1, atol=config.ATOL.get(str(self.scale.dtype)))
...
class TestGumbelPDF(unittest.TestCase):
    ...
    def setUp(self):
        ...
        executor.run(startup_program)
        [self.prob, self.log_prob, self.cdf] = executor.run(main_program, feed=self.feeds, fetch_list=fetch_list)

    def test_prob(self):
        if False:
            for i in range(10):
                print('nop')
        np.testing.assert_allclose(self.prob, scipy.stats.gumbel_r.pdf(self.value, self.loc, self.scale), rtol=config.RTOL.get(str(self.loc.dtype)), atol=config.ATOL.get(str(self.loc.dtype)))

    def test_log_prob(self):
        if False:
            while True:
                i = 10
        np.testing.assert_allclose(self.log_prob, scipy.stats.gumbel_r.logpdf(self.value, self.loc, self.scale), rtol=config.RTOL.get(str(self.loc.dtype)), atol=config.ATOL.get(str(self.loc.dtype)))

    def test_cdf(self):
        if False:
            while True:
                i = 10
        np.testing.assert_allclose(self.cdf, scipy.stats.gumbel_r.cdf(self.value, self.loc, self.scale), rtol=0.3, atol=config.ATOL.get(str(self.loc.dtype)))
