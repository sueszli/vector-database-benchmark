...
tf.flags.DEFINE_string('mode', 'eval', 'One of [sample, eval, dump_emb, dump_lstm_emb]. "sample" mode samples future word predictions, using FLAGS.prefix as prefix (prefix could be left empty). "eval" mode calculates perplexity of the FLAGS.input_data. "dump_emb" mode dumps word and softmax embeddings to FLAGS.save_dir. embeddings are dumped in the same order as words in vocabulary. All words in vocabulary are dumped.dump_lstm_emb dumps lstm embeddings of FLAGS.sentence to FLAGS.save_dir.')
tf.flags.DEFINE_string('pbtxt', '', 'GraphDef proto text file used to construct model structure.')
...
tf.flags.DEFINE_string('save_dir', '', 'Used for "dump_emb" mode to save word embeddings.')
tf.flags.DEFINE_string('prefix', '', 'Used for "sample" mode to predict next words.')
tf.flags.DEFINE_integer('max_sample_words', 100, 'Sampling stops either when </S> is met or this number of steps has passed.')
tf.flags.DEFINE_integer('num_samples', 3, 'Number of samples to generate for the prefix.')
tf.flags.DEFINE_string('sentence', '', 'Used as input for "dump_lstm_emb" mode.')
tf.flags.DEFINE_string('input_data', '', 'Input data files for eval model.')
tf.flags.DEFINE_integer('max_eval_steps', 1000000, 'Maximum mumber of steps to run "eval" mode.')
BATCH_SIZE = 1
NUM_TIMESTEPS = 1
MAX_WORD_LEN = 50
