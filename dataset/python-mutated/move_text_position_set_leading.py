...
class MoveTextPositionSetLeading(CanvasOperator):
    ...
    def invoke(self, canvas_stream_processor: 'CanvasStreamProcessor', operands: typing.List[AnyPDFType]=[], event_listeners: typing.List['EventListener']=[]) -> None:
        ...
        set_text_leading_op: typing.Optional[CanvasOperator] = canvas_stream_processor.get_operator('TL')
        assert set_text_leading_op, 'Operand TL must be defined for operator TD to function'
        set_text_leading_op.invoke(canvas_stream_processor, [bDecimal(-operands[1])], event_listeners)
        move_text_position_op: typing.Optional[CanvasOperator] = canvas_stream_processor.get_operator('Td')
        assert move_text_position_op, 'Operand Td must be defined for operator TD to function'
