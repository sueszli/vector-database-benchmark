...
class FilecryptCc(BaseDecrypter):
    ...
    __version__ = '0.50'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?filecrypt\\.(?:cc|co)/Container/\\w+'
    __config__ = [('enabled', 'bool', 'Activated', True), ('handle_mirror_pages', 'bool', 'Handle Mirror Pages', True)]
    ...
    URL_REPLACEMENTS = [('filecrypt.co', 'filecrypt.cc')]
    DLC_LINK_PATTERN = 'onclick="DownloadDLC\\(\\\'(.+)\\\'\\);">'
    WEBLINK_PATTERN = '<button onclick=\\"[\\w\\-]+?/\\*\\d+?\\*/\\(\'([\\w/-]+?)\','
    MIRROR_PAGE_PATTERN = '"[\\w]*" href="(https?://(?:www\\.)?filecrypt.cc/Container/\\w+\\.html\\?mirror=\\d+)">'
    CAPTCHA_PATTERN = '<h2>Security prompt</h2>'
    INTERNAL_CAPTCHA_PATTERN = '<img id="nc" .* src="(.+?)"'
    CIRCLE_CAPTCHA_PATTERN = '<input type="image" src="(.+?)"'
    KEY_CAPTCHA_PATTERN = "<script language=JavaScript src='(http://backs\\.keycaptcha\\.com/swfs/cap\\.js)'"
    SOLVEMEDIA_CAPTCHA_PATTERN = '<script type="text/javascript" src="(https?://api(?:-secure)?\\.solvemedia\\.com/papi/challenge.+?)"'
