...
class AsyncTokenRotator:
    ...
    def __init__(self, *, client_id: str, client_secret: str, client: Optional[AsyncWebClient]=None):
        if False:
            print('Hello World!')
    ...
    async def perform_token_rotation(self, *, installation: Installation, minutes_before_expiration: int=120) -> Optional[Installation]:
        ...
        """
        rotated_bot: Optional[Bot] = await self.perform_bot_token_rotation(bot=installation.to_bot(), minutes_before_expiration=minutes_before_expiration)
        rotated_installation = await self.perform_user_token_rotation(installation=installation, minutes_before_expiration=minutes_before_expiration)
    ...
    async def perform_bot_token_rotation(self, *, bot: Bot, minutes_before_expiration: int=120) -> Optional[Bot]:
        ...
        if bot.bot_token_expires_at > time() + minutes_before_expiration * 60:
            return None
        try:
            refresh_response = await self.client.oauth_v2_access(client_id=self.client_id, client_secret=self.client_secret, grant_type='refresh_token', refresh_token=bot.bot_refresh_token)
    ...
    async def perform_user_token_rotation(self, *, installation: Installation, minutes_before_expiration: int=120) -> Optional[Installation]:
        ...
        if installation.user_token_expires_at > time() + minutes_before_expiration * 60:
            return None
        try:
            refresh_response = await self.client.oauth_v2_access(client_id=self.client_id, client_secret=self.client_secret, grant_type='refresh_token', refresh_token=installation.user_refresh_token)
