...
class PaymentMethodProcessTokenization(BaseMutation):
    result = PaymentMethodTokenizationResultEnum(description='A status of the payment method tokenization.', required=True)
    id = graphene.String(description='The identifier of the payment method.')
    data = JSON(description='A data returned by the payment app.')

    class Arguments:
        id = graphene.String(required=True, description='The identifier of the payment gateway app to process payment method tokenization.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventSyncType.PAYMENT_METHOD_PROCESS_TOKENIZATION_SESSION, description='The customer continues payment method tokenization.')]
    ...
    def _perform_mutation(cls, root, info, id, channel, data=None):
        ...
        (response, errors) = handle_payment_method_action(info, 'payment_method_process_tokenization', PaymentMethodProcessTokenizationRequestData(id=id, user=user, channel=channel, data=data), PaymentMethodProcessTokenizationErrorCode)
