...
class TestAddTextAnnotation(TestCase):
    ...
    def test_add_text_annotation(self):
        ...
        layout.add(self.get_test_header(test_description='This test adds a TextAnnotation to a PDF.'))
        w: Decimal = pdf.get_page(0).get_page_info().get_width()
        h: Decimal = pdf.get_page(0).get_page_info().get_height()
        pdf.get_page(0).add_annotation(TextAnnotation(bounding_box=Rectangle(w / Decimal(2) - Decimal(32), h / Decimal(2) - Decimal(32), Decimal(64), Decimal(64)), contents='Lorem Ipsum Dolor Sit Amet', text_annotation_icon=TextAnnotationIconType.KEY))
