...
from homeassistant.core import CALLBACK_TYPE, DOMAIN as HOMEASSISTANT_DOMAIN, CoreState, EntityServiceResponse, HomeAssistant, ServiceCall, SupportsResponse, callback, split_entity_id, valid_entity_id
from homeassistant.exceptions import HomeAssistantError, PlatformNotReady
from homeassistant.generated import languages
from homeassistant.setup import async_start_setup
from homeassistant.util.async_ import run_callback_threadsafe
from . import config_validation as cv, device_registry as dev_reg, entity_registry as ent_reg, service, translation
...
class EntityPlatform:
    ...
    async def _async_setup_platform(self, async_create_setup_task: Callable[[], Awaitable[None]], tries: int=0) -> bool:
        ...
        async def get_translations(language: str, category: str, integration: str) -> dict[str, Any]:
            """Get entity translations."""
            try:
                return await translation.async_get_translations(hass, language, category, {integration})
            except Exception as err:
                _LOGGER.debug('Could not load translations for %s', integration, exc_info=err)
            return {}
        self.component_translations = await get_translations(hass.config.language, 'entity_component', self.domain)
        self.platform_translations = await get_translations(hass.config.language, 'entity', self.platform_name)
        if object_id_language == hass.config.language:
            ...
            self.object_id_component_translations = await get_translations(object_id_language, 'entity_component', self.domain)
            self.object_id_platform_translations = await get_translations(object_id_language, 'entity', self.platform_name)
        ...
        with async_start_setup(hass, [full_name]):
            try:
                ...
                logger.exception('Error while setting up %s platform for %s', self.platform_name, self.domain)
                return False
            finally:
                warn_task.cancel()
    ...
    async def async_add_entities(self, new_entities: Iterable[Entity], update_before_add: bool=False) -> None:
        ...
        tasks = [self._async_add_entity(entity, update_before_add, entity_registry) for entity in new_entities]
        if not tasks:
            return
        ...
        try:
            ...
            self.logger.exception('Error adding entities for domain %s with platform %s', self.domain, self.platform_name)
            raise
        if self.config_entry and self.config_entry.pref_disable_polling or self._async_unsub_polling is not None or (not any((entity.should_poll for entity in self.entities.values()))):
            return
        self._async_unsub_polling = async_track_time_interval(self.hass, self._update_entity_states, self.scan_interval, name=f'EntityPlatform poll {self.domain}.{self.platform_name}')
    ...
    async def _async_add_entity(self, entity: Entity, update_before_add: bool, entity_registry: EntityRegistry) -> None:
        ...
        if entity.unique_id is not None:
            ...
            if self.config_entry and (device_info := entity.device_info):
                try:
                    device = dev_reg.async_get(self.hass).async_get_or_create(config_entry_id=self.config_entry.entry_id, **device_info)
            ...
            entry = entity_registry.async_get_or_create(self.domain, self.platform_name, entity.unique_id, capabilities=entity.capability_attributes, config_entry=self.config_entry, device_id=device.id if device else None, disabled_by=disabled_by, entity_category=entity.entity_category, get_initial_options=entity.get_initial_entity_options, has_entity_name=entity.has_entity_name, hidden_by=hidden_by, known_object_ids=self.entities.keys(), original_device_class=entity.device_class, original_icon=entity.icon, original_name=entity_name, suggested_object_id=suggested_object_id, supported_features=entity.supported_features, translation_key=entity.translation_key, unit_of_measurement=entity.unit_of_measurement)
    ...
    async def _update_entity_states(self, now: datetime) -> None:
        ...
        async with self._process_updates:
            if self._update_in_sequence or len(self.entities) <= 1:
                ...
                return
            if (tasks := [entity.async_update_ha_state(True) for entity in self.entities.values() if entity.should_poll]):
                await asyncio.gather(*tasks)
