...
class TestAsyncWebClient(unittest.TestCase):
    ...
    async def test_uploading_text_files_async(self):
        ...
        upload = await client.files_upload(channels=self.channel_id, title='Good Old Slack Logo', filename=filename, file=file)
    ...
    async def test_uploading_binary_files_async(self):
        ...
        upload = await client.files_upload(channels=self.channel_id, title='Good Old Slack Logo', filename='slack_logo.png', file=file)
        self.assertIsNotNone(upload)
        deletion = await client.files_delete(file=upload['file']['id'])
        self.assertIsNotNone(deletion)
    ...
    async def test_uploading_file_with_token_param_async(self):
        ...
        current_dir = os.path.dirname(__file__)
        file = f'{current_dir}/../../tests/data/slack_logo.png'
        upload = await client.files_upload(token=self.bot_token, channels=self.channel_id, title='Good Old Slack Logo', filename='slack_logo.png', file=file)
        self.assertIsNotNone(upload)
