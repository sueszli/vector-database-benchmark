...
def spark_get_duplicates(config: Settings, df: DataFrame, supported_columns: Sequence) -> Tuple[Dict[str, Any], Optional[DataFrame]]:
    if False:
        i = 10
        return i + 15
    ...
    duplicated_df = df.groupBy(df.columns).agg(F.count('*').alias(duplicates_key)).withColumn(duplicates_key, F.col(duplicates_key).cast('int')).filter(F.col(duplicates_key) > 1)
    metrics['n_duplicates'] = duplicated_df.count()
    metrics['p_duplicates'] = metrics['n_duplicates'] / df.count()
    return (metrics, duplicated_df.orderBy(duplicates_key, ascending=False).limit(n_head).toPandas())