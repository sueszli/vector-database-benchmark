...
def create_tsdataset(roll=True, horizon=1):
    ...
    df = pd.DataFrame(np.random.rand(1000, 2), columns=['value1', 'value2'], index=timeseries)
...
class TestChronosModelLSTMForecaster(TestCase):
    ...
    def test_lstm_forecaster_fit_eva_pred(self):
        ...
        forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=2, hidden_dim=[32, 16], layer_num=2, dropout=[0.1, 0.2], loss='mae', lr=0.01)
    ...
    def test_lstm_forecaster_fit_loader(self):
        ...
        forecaster.quantize(calib_data=train_loader, val_data=val_loader, metric='mae', framework='pytorch_fx')
        ...
        forecaster.quantize(calib_data=train_loader, val_data=val_loader, metric='mae', framework='onnxrt_qlinearops')
    ...
    def test_lstm_forecaster_onnx_methods(self):
        if False:
            while True:
                i = 10
        (train_data, val_data, test_data) = create_data()
        forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=2, loss='mae', lr=0.01)
        forecaster.fit(train_data, epochs=2)
        try:
            import onnx
            import onnxruntime
    ...
    def test_lstm_forecaster_quantization(self):
        ...
        forecaster.quantize(train_data, val_data=val_data, metric='mse', relative_drop=0.99, max_trials=3)
        pred_q = forecaster.predict(test_data[0], quantize=True, acceleration=False)
        eval_q = forecaster.evaluate(test_data, quantize=True, acceleration=False)
        with tempfile.TemporaryDirectory() as tmp_dir_name:
    ...
    def test_lstm_forecaster_shape_error(self):
        if False:
            ...
            return i + 15
        (train_data, val_data, test_data) = create_data()
        forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=1, loss='mae', lr=0.01)
        with pytest.raises(RuntimeError):
            forecaster.fit(train_data, epochs=2)
    ...
    def test_lstm_forecaster_xshard_input(self):
        ...
        for distributed in [True, False]:
            forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=2, loss='mae', lr=0.01, distributed=distributed)
            forecaster.fit(train_data, epochs=2)
    ...
    def test_lstm_forecaster_distributed(self):
        ...
        init_orca_context(cores=4, memory='2g')
        forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=2, loss='mae', lr=0.01, distributed=True)
        forecaster.fit(train_data, epochs=2)
    ...
    def test_lstm_customized_loss_metric(self):
        ...
        forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=2, loss=loss, metrics=[customized_metric], lr=0.01)
        forecaster.fit(train_data, epochs=2)
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ckpt_name = os.path.join(tmp_dir_name, 'ckpt')
            test_pred_save = forecaster.predict(test_data[0], acceleration=False)
    ...
    def test_forecaster_from_tsdataset(self):
        ...
        lstm = LSTMForecaster.from_tsdataset(train, hidden_dim=16, layer_num=2)
        lstm.fit(train, epochs=2, batch_size=32)
        yhat = lstm.predict(test, batch_size=32, acceleration=False)
        test.roll(lookback=lstm.data_config['past_seq_len'], horizon=lstm.data_config['future_seq_len'])
        (_, y_test) = test.to_numpy()
        assert yhat.shape == y_test.shape
        del lstm
        (train, test) = create_tsdataset(roll=False, horizon=[1, 3, 5])
        lstm = LSTMForecaster.from_tsdataset(train, past_seq_len=24, hidden_dim=16, layer_num=2)
        lstm.fit(train, epochs=2, batch_size=32)
    ...
    def test_forecaster_from_tsdataset_data_loader_onnx(self):
        ...
        loader = train.to_torch_data_loader(lookback=24, horizon=1)
        test_loader = test.to_torch_data_loader(lookback=24, horizon=1)
        ...
        lstm.quantize(calib_data=loader, metric='mse', framework='pytorch_fx')
        ...
        lstm.quantize(calib_data=loader, metric='mse', framework='onnxrt_qlinearops')
    ...
    def test_lstm_forecaster_fit_earlystop_patience(self):
        ...
        (train_data, val_data, _) = create_data()
        forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=2, hidden_dim=[32, 16], layer_num=2, dropout=[0.1, 0.2], loss='mae', lr=0.01)
        val_loss = forecaster.fit(train_data, val_data, validation_mode='earlystop', earlystop_patience=6, epochs=10)
    ...
    def test_lstm_forecaster_tune_fit(self):
        ...
        forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=2, hidden_dim=space.Categorical(16, 32, 64), layer_num=1, dropout=0.1, loss='mae', metrics=['mse'], lr=0.01)
        forecaster.tune(train_data, val_data, epochs=10, n_trials=3, target_metric='mse', direction='minimize')
    ...
    def test_predict_interval(self):
        if False:
            ...
            return i + 15
        (train_data, val_data, test_data) = create_data()
        forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=2, hidden_dim=16, layer_num=1, dropout=0.1, loss='mae', metrics=['mse'], lr=0.01)
        forecaster.fit(train_data, epochs=2)
    ...
    def test_forecaster_fit_val_from_tsdataset(self):
        ...
        lstm = LSTMForecaster.from_tsdataset(train, hidden_dim=16, layer_num=2)
        lstm.fit(train, val, epochs=2, batch_size=32)
        yhat = lstm.predict(test, batch_size=32, acceleration=False)
        test.roll(lookback=lstm.data_config['past_seq_len'], horizon=lstm.data_config['future_seq_len'])
        (_, y_test) = test.to_numpy()
        assert yhat.shape == y_test.shape
    ...
    def test_lstm_forecaster_eval_shuffle_loader(self):
        ...
        (train_data, val_data, test_data) = create_data()
        forecaster = LSTMForecaster(past_seq_len=24, input_feature_num=2, output_feature_num=2, loss='mse', lr=0.01)
        forecaster.fit(train_data, epochs=2)
