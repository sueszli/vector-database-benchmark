...
def check_fortran_verbose_flag(self, *k, **kw):
    ...
    return x
LINKFLAGS_IGNORED = ['-lang*', '-lcrt[a-zA-Z0-9\\.]*\\.o', '-lc$', '-lSystem', '-libmil', '-LIST:*', '-LNO:*']
if os.name == 'nt':
    LINKFLAGS_IGNORED.extend(['-lfrt*', '-luser32', '-lkernel32', '-ladvapi32', '-lmsvcrt', '-lshell32', '-lmingw', '-lmoldname'])
else:
    LINKFLAGS_IGNORED.append('-lgcc*')
...
def getoutput(conf, cmd, stdin=False):
    ...
    return (out, err)
ROUTINES_CODE = '      subroutine foobar()\n      return\n      end\n      subroutine foo_bar()\n      return\n      end\n'
