...
class ConvolutionalBoxPredictorTest(test_case.TestCase):
    ...
    def test_get_boxes_for_five_aspect_ratios_per_location(self):
        ...
        def graph_fn(image_features):
            if False:
                while True:
                    i = 10
            conv_box_predictor = box_predictor_builder.build_convolutional_box_predictor(is_training=False, num_classes=0, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), min_depth=0, max_depth=32, num_layers_before_predictor=1, use_dropout=True, dropout_keep_prob=0.8, kernel_size=1, box_code_size=4)
    ...
    def test_get_predictions_with_feature_maps_of_dynamic_shape(self):
        ...
        expected_variable_set = set(['BoxPredictor/Conv2d_0_1x1_32/biases', 'BoxPredictor/Conv2d_0_1x1_32/weights', 'BoxPredictor/BoxEncodingPredictor/biases', 'BoxPredictor/BoxEncodingPredictor/weights', 'BoxPredictor/ClassPredictor/biases', 'BoxPredictor/ClassPredictor/weights'])
    ...
    def test_use_depthwise_convolution(self):
        if False:
            print('Hello World!')
        image_features = tf.placeholder(dtype=tf.float32, shape=[4, None, None, 64])
        conv_box_predictor = box_predictor_builder.build_convolutional_box_predictor(is_training=False, num_classes=0, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), min_depth=0, max_depth=32, num_layers_before_predictor=1, dropout_keep_prob=0.8, kernel_size=3, box_code_size=4, use_dropout=True, use_depthwise=True)
        box_predictions = conv_box_predictor.predict([image_features], num_predictions_per_location=[5], scope='BoxPredictor')
        box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
        objectness_predictions = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
        init_op = tf.global_variables_initializer()
    ...
    def test_no_dangling_outputs(self):
        ...
        tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
        bad_dangling_ops = []
...
class WeightSharedConvolutionalBoxPredictorTest(test_case.TestCase):
    ...
    def test_get_boxes_for_five_aspect_ratios_per_location(self):
        ...
        def graph_fn(image_features):
            if False:
                while True:
                    i = 10
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=False, num_classes=0, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=1, box_code_size=4)
            box_predictions = conv_box_predictor.predict([image_features], num_predictions_per_location=[5], scope='BoxPredictor')
            box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
            objectness_predictions = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
            return (box_encodings, objectness_predictions)
    ...
    def test_bias_predictions_to_background_with_sigmoid_score_conversion(self):
        ...
        def graph_fn(image_features):
            if False:
                return 10
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=True, num_classes=2, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=1, class_prediction_bias_init=-4.6, box_code_size=4)
            box_predictions = conv_box_predictor.predict([image_features], num_predictions_per_location=[5], scope='BoxPredictor')
    ...
    def test_get_multi_class_predictions_for_five_aspect_ratios_per_location(self):
        ...
        def graph_fn(image_features):
            if False:
                return 10
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=False, num_classes=num_classes_without_background, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=1, box_code_size=4)
            box_predictions = conv_box_predictor.predict([image_features], num_predictions_per_location=[5], scope='BoxPredictor')
            box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
            class_predictions_with_background = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
            return (box_encodings, class_predictions_with_background)
        ...
        def graph_fn(image_features1, image_features2):
            if False:
                for i in range(10):
                    print('nop')
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=False, num_classes=num_classes_without_background, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=1, box_code_size=4)
            box_predictions = conv_box_predictor.predict([image_features1, image_features2], num_predictions_per_location=[5, 5], scope='BoxPredictor')
            box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
            class_predictions_with_background = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
            return (box_encodings, class_predictions_with_background)
    ...
    def test_get_multi_class_predictions_from_feature_maps_of_different_depth(self):
        ...
        def graph_fn(image_features1, image_features2, image_features3):
            if False:
                ...
                return i + 15
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=False, num_classes=num_classes_without_background, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=1, box_code_size=4)
            box_predictions = conv_box_predictor.predict([image_features1, image_features2, image_features3], num_predictions_per_location=[5, 5, 5], scope='BoxPredictor')
            box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
            class_predictions_with_background = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
            return (box_encodings, class_predictions_with_background)
        image_features1 = np.random.rand(4, 8, 8, 64).astype(np.float32)
    ...
    def test_predictions_multiple_feature_maps_share_weights_separate_batchnorm(self):
        ...
        def graph_fn(image_features1, image_features2):
            if False:
                while True:
                    i = 10
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=False, num_classes=num_classes_without_background, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=2, box_code_size=4)
            box_predictions = conv_box_predictor.predict([image_features1, image_features2], num_predictions_per_location=[5, 5], scope='BoxPredictor')
            box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
            class_predictions_with_background = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
            return (box_encodings, class_predictions_with_background)
    ...
    def test_predictions_multiple_feature_maps_share_weights_without_batchnorm(self):
        ...
        def graph_fn(image_features1, image_features2):
            if False:
                i = 10
                return i + 15
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=False, num_classes=num_classes_without_background, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=2, box_code_size=4, apply_batch_norm=False)
            box_predictions = conv_box_predictor.predict([image_features1, image_features2], num_predictions_per_location=[5, 5], scope='BoxPredictor')
            box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
            class_predictions_with_background = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
            return (box_encodings, class_predictions_with_background)
        with self.test_session(graph=tf.Graph()):
            graph_fn(tf.random_uniform([4, 32, 32, 3], dtype=tf.float32), tf.random_uniform([4, 16, 16, 3], dtype=tf.float32))
            actual_variable_set = set([var.op.name for var in tf.trainable_variables()])
        expected_variable_set = set(['BoxPredictor/WeightSharedConvolutionalBoxPredictor/BoxPredictionTower/conv2d_0/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/BoxPredictionTower/conv2d_0/biases', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/BoxPredictionTower/conv2d_1/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/BoxPredictionTower/conv2d_1/biases', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/BoxPredictor/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/BoxPredictor/biases', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/ClassPredictionTower/conv2d_0/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/ClassPredictionTower/conv2d_0/biases', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/ClassPredictionTower/conv2d_1/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/ClassPredictionTower/conv2d_1/biases', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/ClassPredictor/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/ClassPredictor/biases'])
        self.assertEqual(expected_variable_set, actual_variable_set)
    ...
    def test_predictions_multiple_feature_maps_share_weights_with_depthwise(self):
        ...
        def graph_fn(image_features1, image_features2):
            if False:
                i = 10
                return i + 15
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=False, num_classes=num_classes_without_background, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=2, box_code_size=4, apply_batch_norm=False, use_depthwise=True)
            box_predictions = conv_box_predictor.predict([image_features1, image_features2], num_predictions_per_location=[5, 5], scope='BoxPredictor')
            box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
            class_predictions_with_background = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
            return (box_encodings, class_predictions_with_background)
    ...
    def test_no_batchnorm_params_when_batchnorm_is_not_configured(self):
        ...
        def graph_fn(image_features1, image_features2):
            if False:
                i = 10
                return i + 15
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=False, num_classes=num_classes_without_background, conv_hyperparams_fn=self._build_conv_arg_scope_no_batch_norm(), depth=32, num_layers_before_predictor=2, box_code_size=4, apply_batch_norm=False)
            box_predictions = conv_box_predictor.predict([image_features1, image_features2], num_predictions_per_location=[5, 5], scope='BoxPredictor')
            box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
            class_predictions_with_background = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
            return (box_encodings, class_predictions_with_background)
    ...
    def test_predictions_share_weights_share_tower_separate_batchnorm(self):
        ...
        def graph_fn(image_features1, image_features2):
            if False:
                print('Hello World!')
            conv_box_predictor = box_predictor_builder.build_weight_shared_convolutional_box_predictor(is_training=False, num_classes=num_classes_without_background, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=2, box_code_size=4, share_prediction_tower=True)
            box_predictions = conv_box_predictor.predict([image_features1, image_features2], num_predictions_per_location=[5, 5], scope='BoxPredictor')
            box_encodings = tf.concat(box_predictions[box_predictor.BOX_ENCODINGS], axis=1)
            class_predictions_with_background = tf.concat(box_predictions[box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND], axis=1)
            return (box_encodings, class_predictions_with_background)
    ...
    def test_predictions_share_weights_share_tower_without_batchnorm(self):
        ...
        with self.test_session(graph=tf.Graph()):
            graph_fn(tf.random_uniform([4, 32, 32, 3], dtype=tf.float32), tf.random_uniform([4, 16, 16, 3], dtype=tf.float32))
            actual_variable_set = set([var.op.name for var in tf.trainable_variables()])
        expected_variable_set = set(['BoxPredictor/WeightSharedConvolutionalBoxPredictor/PredictionTower/conv2d_0/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/PredictionTower/conv2d_0/biases', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/PredictionTower/conv2d_1/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/PredictionTower/conv2d_1/biases', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/BoxPredictor/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/BoxPredictor/biases', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/ClassPredictor/weights', 'BoxPredictor/WeightSharedConvolutionalBoxPredictor/ClassPredictor/biases'])
        self.assertEqual(expected_variable_set, actual_variable_set)
    ...
    def test_other_heads_predictions(self):
        ...
        def graph_fn(image_features):
            if False:
                return 10
            box_prediction_head = box_head.WeightSharedConvolutionalBoxHead(box_code_size)
            class_prediction_head = class_head.WeightSharedConvolutionalClassHead(num_classes_without_background + 1)
            other_heads = {other_head_name: mask_head.WeightSharedConvolutionalMaskHead(num_classes_without_background, mask_height=mask_height, mask_width=mask_width)}
            conv_box_predictor = box_predictor.WeightSharedConvolutionalBoxPredictor(is_training=False, num_classes=num_classes_without_background, box_prediction_head=box_prediction_head, class_prediction_head=class_prediction_head, other_heads=other_heads, conv_hyperparams_fn=self._build_arg_scope_with_conv_hyperparams(), depth=32, num_layers_before_predictor=2)
            box_predictions = conv_box_predictor.predict([image_features], num_predictions_per_location=[num_predictions_per_location], scope='BoxPredictor')
