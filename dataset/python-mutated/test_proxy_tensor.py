...
from torch.fx.experimental.symbolic_shapes import eval_guards, bind_symbols, fx_placeholder_vals, fx_placeholder_targets, guard_int, GuardOnDataDependentSymNode
...
make_fx_failures = {xfail('allclose'), xfail('equal'), skip('new_empty'), skip('empty_like'), skip('empty'), skip('empty_permuted'), skip('linalg.lstsq', 'grad_oriented'), skip('nn.functional.max_unpool1d', '', device_type='cpu'), skip('nn.functional.max_unpool2d', '', device_type='cpu'), skip('nn.functional.max_unpool3d', '', device_type='cpu'), skip('linalg.lstsq'), skip('item'), xfail('cov'), xfail('nn.functional.gaussian_nll_loss'), xfail('tensor_split'), xfail('corrcoef'), xfail('quantile'), xfail('nanquantile'), xfail('narrow'), xfail('sparse.sampled_addmm'), xfail('sparse.mm', 'reduce'), skip('to_sparse'), skip('block_diag'), skip('empty_strided', '', device_type='cpu')}
fake_tensor_failures = {skip('nn.functional.nll_loss')}
symbolic_tensor_failures = {xfail('linalg.eig'), xfail('linalg.eigvals'), xfail('combinations', ''), xfail('frexp', ''), xfail('geqrf', ''), xfail('histc', ''), xfail('histogram', ''), xfail('histogramdd', ''), xfail('isin', ''), xfail('kthvalue', ''), xfail('nanquantile', ''), xfail('narrow', ''), xfail('nn.functional.binary_cross_entropy', ''), xfail('nn.functional.cross_entropy', ''), xfail('nn.functional.ctc_loss'), xfail('nn.functional.fractional_max_pool2d', ''), xfail('nn.functional.fractional_max_pool3d', ''), xfail('nn.functional.interpolate', 'linear'), xfail('nn.functional.interpolate', 'trilinear'), xfail('nn.functional.pixel_unshuffle', ''), xfail('quantile', ''), xfail('resize_', ''), xfail('resize_as_', ''), xfail('unique_consecutive', ''), xfail('unique', ''), xfail('fft.fft', ''), xfail('fft.hfft2', ''), xfail('fft.hfft', ''), xfail('fft.hfftn', ''), xfail('fft.ifft', ''), xfail('fft.ihfft2', ''), xfail('fft.ihfft', ''), xfail('fft.ihfftn', ''), xfail('fft.ihfft2', ''), xfail('fft.irfft2', ''), xfail('fft.irfft', ''), xfail('fft.irfftn', ''), xfail('fft.rfft2', ''), xfail('fft.rfft', ''), xfail('fft.rfftn', ''), xfail('stft', '')}
symbolic_tensor_segfaults = {skip('nn.functional.batch_norm')}
symbolic_tensor_failures.update(symbolic_tensor_segfaults)
outplace_symbolic_tensor_failures = {xfail('i0', '')}
inplace_symbolic_tensor_failures = {xfail('float_power', ''), xfail('unique', '')}
out_symbolic_tensor_failures = {xfail('_native_batch_norm_legit', ''), xfail('aminmax', ''), xfail('angle', ''), xfail('argmax', ''), xfail('argmin', ''), xfail('bmm', ''), xfail('cummax', ''), xfail('cummin', ''), xfail('fft.fft2', ''), xfail('fft.fftn', ''), xfail('fft.ifft2', ''), xfail('fft.ifftn', ''), xfail('gather', ''), xfail('i0', ''), xfail('linalg.cholesky', ''), xfail('linalg.cholesky_ex', ''), xfail('linalg.det', ''), xfail('linalg.det', 'singular'), xfail('linalg.eigh', ''), xfail('linalg.inv', ''), xfail('linalg.inv_ex', ''), xfail('linalg.ldl_factor', ''), xfail('linalg.ldl_factor_ex', ''), xfail('linalg.lu', ''), xfail('linalg.lu_factor', ''), xfail('linalg.lu_factor_ex', ''), xfail('linalg.pinv', ''), xfail('linalg.pinv', 'hermitian'), xfail('linalg.qr', ''), xfail('linalg.slogdet', ''), xfail('linalg.solve_ex', ''), xfail('linalg.svd', ''), xfail('linalg.svdvals', ''), xfail('lu', ''), xfail('lu_unpack', ''), xfail('max', 'reduction_with_dim'), xfail('min', 'reduction_with_dim'), xfail('mode', ''), xfail('nn.functional.avg_pool2d', ''), xfail('nn.functional.linear', ''), xfail('qr', ''), xfail('round', ''), xfail('round', 'decimals_0'), xfail('round', 'decimals_3'), xfail('round', 'decimals_neg_3'), xfail('scatter_add', ''), xfail('scatter', ''), xfail('sort', ''), xfail('svd', ''), xfail('take_along_dim', ''), xfail('topk', ''), xfail('triangular_solve', ''), xfail('view_copy', '')}
out_symbolic_tensor_segfaults = {skip('nanmean', '')}
