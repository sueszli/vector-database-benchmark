...
from flet_core.protocol import Actions, Command, CommandEncoder, Message, PageCommandRequestPayload, PageCommandResponsePayload, PageCommandsBatchRequestPayload, PageCommandsBatchResponsePayload, PageEventPayload, PageSessionCreatedPayload, RegisterHostClientRequestPayload, RegisterHostClientResponsePayload
...
class AsyncWebSocketConnection(Connection):
    ...
    def __init__(self, server_address: str, page_name: str, assets_dir: Optional[str], auth_token: Optional[str], on_event=None, on_session_created=None):
        if False:
            i = 10
            return i + 15
        super().__init__()
        ...
        while True:
            try:
                ...
                if attempt == 0 and (not self.__is_reconnecting):
                    raise Exception(f'Failed to connect Flet server in {self.__CONNECT_ATTEMPTS} attempts.')
                attempt -= 1
    ...
    async def __register_host_client(self):
        payload = RegisterHostClientRequestPayload(hostClientID=self.__host_client_id, pageName=self.page_name, assetsDir=self.__assets_dir, authToken=self.__auth_token, permissions=None)
        response = await self._send_message_with_result(Actions.REGISTER_HOST_CLIENT, payload)
    ...
    async def __start_loops(self):
        ...
        (done, pending) = await asyncio.wait([self.__receive_loop_task, self.__send_loop_task], return_when=asyncio.FIRST_COMPLETED)
        failed = False
        for task in done:
            name = task.get_name()
    ...
    async def __on_ws_message(self, data):
        ...
        if msg.id:
            ...
            if self.__on_session_created is not None:
                asyncio.create_task(self.__on_session_created(PageSessionCreatedPayload(**msg.payload)))
        else:
            print(msg.payload)
    ...
    async def send_command_async(self, session_id: str, command: Command):
        ...
        response = await self._send_message_with_result(Actions.PAGE_COMMAND_FROM_HOST, payload)
    ...
    async def send_commands_async(self, session_id: str, commands: List[Command]):
        ...
        response = await self._send_message_with_result(Actions.PAGE_COMMANDS_BATCH_FROM_HOST, payload)
