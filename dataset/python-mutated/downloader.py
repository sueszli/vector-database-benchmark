...
class Downloader(commands.Cog):
    ...
    async def cog_before_invoke(self, ctx: commands.Context) -> None:
        ...
        if self._ready_raised:
            await ctx.send("There was an error during Downloader's initialization. Check logs for more information.")
    ...
    async def _schema_0_to_1(self):
        ...
        async with self.config.installed_cogs() as new_cog_conf:
            for cog_json in old_conf:
                ...
                new_cog_conf[repo_name][module_name] = {'repo_name': repo_name, 'module_name': module_name, 'commit': '', 'pinned': False}
    ...
    async def installed_cogs(self) -> Tuple[InstalledModule, ...]:
        ...
        return tuple((InstalledModule.from_json(cog_json, self._repo_manager) for repo_json in installed.values() for cog_json in repo_json.values()))
    ...
    async def installed_libraries(self) -> Tuple[InstalledModule, ...]:
        ...
        return tuple((InstalledModule.from_json(lib_json, self._repo_manager) for repo_json in installed.values() for lib_json in repo_json.values()))
    ...
    async def _available_updates(self, cogs: Iterable[InstalledModule]) -> Tuple[Tuple[Installable, ...], Tuple[Installable, ...]]:
        """
    ...
    async def _install_cogs(self, cogs: Iterable[Installable]) -> Tuple[Tuple[InstalledModule, ...], Tuple[Installable, ...]]:
    ...
    async def _reinstall_libraries(self, libraries: Iterable[Installable]) -> Tuple[Tuple[InstalledModule, ...], Tuple[Installable, ...]]:
        ...
        for (repo, libs_by_commit) in repos.values():
            ...
            for (commit, libs) in libs_by_commit.items():
                await repo.checkout(commit)
                (installed, failed) = await repo.install_libraries(target_dir=self.SHAREDLIB_PATH, req_target_dir=self.LIB_PATH, libraries=libs)
    ...
    async def pipinstall(self, ctx: commands.Context, *deps: str) -> None:
        ...
        **Arguments**
        ...
        if success:
            ...
            await ctx.send(_('Some libraries failed to install. Please check your logs for a complete list.') if len(deps) > 1 else _('The library failed to install. Please check your logs for a complete list.'))
    ...
    async def _repo_add(self, ctx: commands.Context, name: str, repo_url: str, branch: str=None) -> None:
        """Add a new repo.
        ...
        **Arguments**
        ...
        if re.match('^[a-zA-Z0-9_\\-\\.]+$', name) is None:
            await ctx.send(_('Repo names can only contain characters A-z, numbers, underscores, hyphens, and dots.'))
            return
        try:
            async with ctx.typing():
                repo = await self._repo_manager.add_repo(name=name, url=repo_url, branch=branch)
        except errors.ExistingGitRepo:
            await ctx.send(_('The repo name you provided is already in use. Please choose another name.'))
        except errors.CloningError as err:
            await ctx.send(_('Something went wrong during the cloning process. See logs for more information.'))
            log.exception('Something went wrong whilst cloning %s (to revision: %s)', repo_url, branch, exc_info=err)
        except OSError:
            log.exception('Something went wrong trying to add repo %s under name %s', repo_url, name)
            await ctx.send(_('Something went wrong trying to add that repo. See logs for more information.'))
        else:
    ...
    async def _repo_del(self, ctx: commands.Context, *repos: Repo) -> None:
        ...
        **Arguments**
        ...
        await ctx.send((_('Successfully deleted repos: ') if len(repos) > 1 else _('Successfully deleted the repo: ')) + humanize_list([inline(i.name) for i in set(repos)]))
    ...
    async def _repo_info(self, ctx: commands.Context, repo: Repo) -> None:
        ...
        **Arguments**
        ...
        information += _('Made by: {author}\nDescription:\n{description}').format(author=made_by, description=repo.description or '')
        msg = _('Information on {repo_name} repo:{information}').format(repo_name=inline(repo.name), information=box(information))
        await ctx.send(msg)
    ...
    async def _cog_reinstallreqs(self, ctx: commands.Context) -> None:
        ...
        async with ctx.typing():
            ...
            for repo in repos:
                (installed_libs, failed_libs) = await repo.install_libraries(target_dir=self.SHAREDLIB_PATH, req_target_dir=self.LIB_PATH)
                all_installed_libs += installed_libs
                all_failed_libs += failed_libs
        message = ''
        if failed_reqs:
            message += (_('Failed to install requirements: ') if len(failed_reqs) > 1 else _('Failed to install the requirement: ')) + humanize_list(tuple(map(inline, failed_reqs)))
        if all_failed_libs:
        ...
        if message:
            await self.send_pagified(ctx, _('Cog requirements and shared libraries for all installed cogs have been reinstalled but there were some errors:\n') + message)
        else:
    ...
    @cog.command(name='installversion', usage='<repo> <revision> <cogs...>', require_var_positional=True)
    async def _cog_installversion(self, ctx: commands.Context, repo: Repo, revision: str, *cog_names: str) -> None:
    ...
    async def _cog_installrev(self, ctx: commands.Context, repo: Repo, rev: Optional[str], cog_names: Iterable[str]) -> None:
        commit = None
        async with ctx.typing():
            if rev is not None:
                try:
                    ...
                    for candidate in e.candidates:
                        msg += f'**{candidate.object_type} {candidate.rev}** - {candidate.description}\n'
                    await self.send_pagified(ctx, msg)
                    return
                except errors.UnknownRevision:
                    await ctx.send(_('Error: there is no revision `{rev}` in repo `{repo.name}`').format(rev=rev, repo=repo))
                    return
            ...
            async with repo.checkout(commit, exit_to_rev=repo.branch):
                ...
                failed_reqs = await self._install_requirements(cogs)
                if failed_reqs:
                    message += (_('\nFailed to install requirements: ') if len(failed_reqs) > 1 else _('\nFailed to install the requirement: ')) + humanize_list(tuple(map(inline, failed_reqs)))
                    await self.send_pagified(ctx, message)
                    return
            ...
            if failed_libs:
                ...
                message = (_('\nFailed to install shared libraries for `{repo.name}` repo: ') if len(libnames) > 1 else _('\nFailed to install shared library for `{repo.name}` repo: ')).format(repo=repo) + humanize_list(libnames) + message
            if failed_cogs:
                cognames = [inline(cog.name) for cog in failed_cogs]
                message = (_('\nFailed to install cogs: ') if len(failed_cogs) > 1 else _('\nFailed to install the cog: ')) + humanize_list(cognames) + message
            if installed_cogs:
                cognames = [inline(cog.name) for cog in installed_cogs]
                message = (_('Successfully installed cogs: ') if len(installed_cogs) > 1 else _('Successfully installed the cog: ')) + humanize_list(cognames) + (_("\nThese cogs are now pinned and won't get updated automatically. To change this, use `{prefix}cog unpin <cog>`").format(prefix=ctx.clean_prefix) if rev is not None else '') + _('\nYou can load them using {command_1}. To see end user data statements, you can use {command_2}.').format(command_1=inline(f'{ctx.clean_prefix}load <cogs...>'), command_2=inline(f'{ctx.clean_prefix}cog info <repo> <cog>')) + message
    ...
    async def _cog_uninstall(self, ctx: commands.Context, *cogs: InstalledCog) -> None:
        ...
        **Arguments**
        ...
        async with ctx.typing():
            ...
            message = ''
            if uninstalled_cogs:
                message += (_('Successfully uninstalled cogs: ') if len(uninstalled_cogs) > 1 else _('Successfully uninstalled the cog: ')) + humanize_list(uninstalled_cogs)
            if failed_cogs:
                if len(failed_cogs) > 1:
                    message += _("\nDownloader has removed these cogs from the installed cogs list but it wasn't able to find their files: ") + humanize_list(tuple(map(inline, failed_cogs))) + _('\nThey were most likely removed without using {command_1}.\nYou may need to remove those files manually if the cogs are still usable. If so, ensure the cogs have been unloaded with {command_2}.').format(command_1=inline(f'{ctx.clean_prefix}cog uninstall'), command_2=inline(f"{ctx.clean_prefix}unload {' '.join(failed_cogs)}"))
                else:
                    message += _("\nDownloader has removed this cog from the installed cogs list but it wasn't able to find its files: ") + inline(failed_cogs[0]) + _('\nIt was most likely removed without using {command_1}.\nYou may need to remove those files manually if the cog is still usable. If so, ensure the cog has been unloaded with {command_2}.').format(command_1=inline(f'{ctx.clean_prefix}cog uninstall'), command_2=inline(f'{ctx.clean_prefix}unload {failed_cogs[0]}'))
        await self.send_pagified(ctx, message)
    ...
    async def _cog_pin(self, ctx: commands.Context, *cogs: InstalledCog) -> None:
        ...
        **Arguments**
        ...
        if pinned:
            ...
            message += (_('Pinned cogs: ') if len(pinned) > 1 else _('Pinned cog: ')) + humanize_list(cognames)
        if already_pinned:
    ...
    async def _cog_unpin(self, ctx: commands.Context, *cogs: InstalledCog) -> None:
        ...
        **Arguments**
        ...
        if unpinned:
            ...
            message += (_('Unpinned cogs: ') if len(unpinned) > 1 else _('Unpinned cog: ')) + humanize_list(cognames)
        if not_pinned:
            message += (_("\nThese cogs weren't pinned: ") if len(not_pinned) > 1 else _('\nThis cog was already not pinned: ')) + humanize_list(not_pinned)
        await self.send_pagified(ctx, message)
    ...
    async def _cog_listpinned(self, ctx: commands.Context):
        ...
        if pinned_list:
            message = '\n'.join((f"({inline(cog.commit[:7] or _('unknown'))}) {cog.name}" for cog in pinned_list))
        else:
            message = _('None.')
    ...
    async def _cog_checkforupdates(self, ctx: commands.Context) -> None:
        ...
        async with ctx.typing():
            ...
            if cogs_to_update:
                ...
                message += (_('These cogs can be updated: ') if len(cognames) > 1 else _('This cog can be updated: ')) + humanize_list(tuple(map(inline, cognames)))
            if libs_to_update:
    ...
    async def _cog_updatetoversion(self, ctx: commands.Context, reload: Optional[bool], repo: Repo, revision: str, *cogs: InstalledCog) -> None:
    ...
    async def _cog_update_logic(self, ctx: commands.Context, *, repo: Optional[Repo]=None, repos: Optional[List[Repo]]=None, rev: Optional[str]=None, cogs: Optional[List[InstalledModule]]=None) -> None:
        failed_repos = set()
        ...
        async with ctx.typing():
            if repo is not None:
                ...
                try:
                    ...
                    message = _('Error: there is no revision `{rev}` in repo `{repo.name}`').format(rev=rev, repo=repo)
                    await ctx.send(message)
                    return
                await repo.checkout(commit)
                (cogs_to_check, __) = await self._get_cogs_to_check(repos=[repo], cogs=cogs, update_repos=False)
            else:
            ...
            if not cogs_to_check:
                ...
                if pinned_cogs:
                    ...
                    message += (_("\nThese cogs are pinned and therefore weren't checked: ") if len(cognames) > 1 else _("\nThis cog is pinned and therefore wasn't checked: ")) + humanize_list(tuple(map(inline, cognames)))
            else:
                (cogs_to_update, libs_to_update) = await self._available_updates(cogs_to_check)
                updates_available = cogs_to_update or libs_to_update
                (cogs_to_update, filter_message) = self._filter_incorrect_cogs(cogs_to_update)
                if updates_available:
                    (updated_cognames, message) = await self._update_cogs_and_libs(ctx, cogs_to_update, libs_to_update, current_cog_versions=cogs_to_check)
                elif repos:
                    ...
                    if cogs:
                        ...
                        message += _('Cogs from provided repo are already up to date with this revision.')
                elif cogs:
        ...
        repos_with_libs = {inline(module.repo.name) for module in cogs_to_update + libs_to_update if module.repo.available_libraries}
        if repos_with_libs:
    ...
    async def _cog_list(self, ctx: commands.Context, repo: Repo) -> None:
        ...
        **Arguments**
        ...
        installed_str = '\n'.join(('- {}{}'.format(i.name, ': {}'.format(i.short) if i.short else '') for i in sorted(installed_cogs_in_repo, key=sort_function)))
        ...
        available_cogs = [cog for cog in repo.available_cogs if not (cog.hidden or cog in installed_cogs_in_repo)]
        available_str = '\n'.join(('+ {}{}'.format(cog.name, ': {}'.format(cog.short) if cog.short else '') for cog in sorted(available_cogs, key=sort_function)))
    ...
    async def _cog_info(self, ctx: commands.Context, repo: Repo, cog_name: str) -> None:
        ...
        **Arguments**
        ...
        if cog is None:
            await ctx.send(_('There is no cog `{cog_name}` in the repo `{repo.name}`').format(cog_name=cog_name, repo=repo))
            return
        msg = _('Information on {cog_name}:\n{description}\n\nEnd user data statement:\n{end_user_data_statement}\n\nMade by: {author}\nRequirements: {requirements}').format(cog_name=cog.name, description=cog.description or '', end_user_data_statement=cog.end_user_data_statement or _("Author of the cog didn't provide end user data statement."), author=', '.join(cog.author) or _('Missing from info.json'), requirements=', '.join(cog.requirements) or 'None')
        for page in pagify(msg):
            await ctx.send(box(page))

    async def is_installed(self, cog_name: str) -> Union[Tuple[bool, InstalledModule], Tuple[bool, None]]:
    ...
    async def _filter_incorrect_cogs_by_names(self, repo: Repo, cog_names: Iterable[str]) -> Tuple[Tuple[Installable, ...], str]:
    ...
    async def _get_cogs_to_check(self, *, repos: Optional[Iterable[Repo]]=None, cogs: Optional[Iterable[InstalledModule]]=None, update_repos: bool=True) -> Tuple[Set[InstalledModule], List[str]]:
        failed = []
        if not (cogs or repos):
            if update_repos:
                (__, failed) = await self._repo_manager.update_repos()
            cogs_to_check = {cog for cog in await self.installed_cogs() if cog.repo is not None and cog.repo.name not in failed}
        else:
            if not repos:
            ...
            if cogs:
                ...
                cogs_to_check = {cog for cog in await self.installed_cogs() if cog.repo is not None and cog.repo in repos}
        return (cogs_to_check, failed)

    async def _update_cogs_and_libs(self, ctx: commands.Context, cogs_to_update: Iterable[Installable], libs_to_update: Iterable[Installable], current_cog_versions: Iterable[InstalledModule]) -> Tuple[Set[str], str]:
        ...
        if failed_reqs:
            return (set(), (_('Failed to install requirements: ') if len(failed_reqs) > 1 else _('Failed to install the requirement: ')) + humanize_list(tuple(map(inline, failed_reqs))))
        ...
        if installed_cogs:
            ...
            if any(enabled_slash.values()):
                message += _('\nYou may need to resync your slash commands with `{prefix}slash sync`.').format(prefix=ctx.prefix)
        if failed_cogs:
            cognames = [cog.name for cog in failed_cogs]
            message += (_('\nFailed to update cogs: ') if len(failed_cogs) > 1 else _('\nFailed to update cog: ')) + humanize_list(tuple(map(inline, cognames)))
        if not cogs_to_update:
            message = _('No cogs were updated.')
        if installed_libs:
            message += _('\nSome shared libraries were updated, you should restart the bot to bring the changes into effect.') if len(installed_libs) > 1 else _('\nA shared library was updated, you should restart the bot to bring the changes into effect.')
        if failed_libs:
            libnames = [lib.name for lib in failed_libs]
    ...
    async def _ask_for_cog_reload(self, ctx: commands.Context, updated_cognames: Set[str]) -> None:
        ...
        if not ctx.assume_yes:
            message = _('Would you like to reload the updated cogs?') if len(updated_cognames) > 1 else _('Would you like to reload the updated cog?')
    ...
    async def findcog(self, ctx: commands.Context, command_name: str) -> None:
        ...
        **Arguments**
        ...
        if cog:
            ...
            if installed:
                made_by = humanize_list(cog_installable.author) if cog_installable.author else _('Missing from info.json')
                repo_url = _('Missing from installed repos') if cog_installable.repo is None else cog_installable.repo.clean_url
                repo_name = _('Missing from installed repos') if cog_installable.repo is None else cog_installable.repo.name
        ...
        if await ctx.embed_requested():
            ...
            if installed and cog_installable.repo is not None and cog_installable.repo.branch:
                embed.add_field(name=_('Repo branch:'), value=cog_installable.repo.branch, inline=False)
            await ctx.send(embed=embed)
        else:
            msg = _('Command:          {command}\nCog package name: {cog_pkg}\nCog name:         {cog}\nMade by:          {author}\nRepo name:        {repo_name}\nRepo URL:         {repo_url}\n').format(command=command_name, cog_pkg=cog_pkg_name, cog=cog_name, author=made_by, repo_url=repo_url, repo_name=repo_name)
            if installed and cog_installable.repo is not None and cog_installable.repo.branch:
