...
class PaymentGatewayConfig(BaseObjectType):
    ...
    data = graphene.Field(JSON, description='The JSON data required to initialize the payment gateway.')
...
class PaymentGatewayToInitialize(BaseInputObjectType):
    id = graphene.String(required=True, description='The identifier of the payment gateway app to initialize.')
    data = graphene.Field(JSON, description='The data that will be passed to the payment gateway.')

    class Meta:
        doc_category = DOC_CATEGORY_PAYMENTS

class PaymentGatewayInitialize(TransactionSessionBase):
    gateway_configs = common_types.NonNullList(PaymentGatewayConfig, description='List of payment gateway configurations.')
    ...
    class Arguments:
        ...
        payment_gateways = graphene.List(graphene.NonNull(PaymentGatewayToInitialize), description='List of payment gateways to initialize.', required=False)

    class Meta:
        doc_category = DOC_CATEGORY_PAYMENTS
        description = 'Initializes a payment gateway session. It triggers the webhook `PAYMENT_GATEWAY_INITIALIZE_SESSION`, to the requested `paymentGateways`. If `paymentGateways` is not provided, the webhook will be send to all subscribed payment gateways.' + ADDED_IN_313 + PREVIEW_FEATURE
