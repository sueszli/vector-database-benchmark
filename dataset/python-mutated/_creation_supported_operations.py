...
class CreationSupportedOperations:
    ...
    @distributed_trace
    def list(self, datadog_organization_id: str, **kwargs: Any) -> AsyncIterable['_models.CreateResourceSupportedResponse']:
        if False:
            print('Hello World!')
        ...
        def prepare_request(next_link=None):
            if False:
                while True:
                    i = 10
            if not next_link:
                request = build_list_request(subscription_id=self._config.subscription_id, datadog_organization_id=datadog_organization_id, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
    ...
    async def get(self, datadog_organization_id: str, **kwargs: Any) -> _models.CreateResourceSupportedResponse:
        ...
        request = build_get_request(subscription_id=self._config.subscription_id, datadog_organization_id=datadog_organization_id, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=_stream, **kwargs)
