...
import graphene
import pendulum
from dagster._core.definitions.run_request import AddDynamicPartitionsRequest, DeleteDynamicPartitionsRequest, RunRequest
from dagster._core.definitions.schedule_definition import ScheduleExecutionData
from dagster._core.definitions.selector import ScheduleSelector, SensorSelector
from dagster._core.definitions.sensor_definition import SensorExecutionData
from dagster._core.scheduler.instigation import DynamicPartitionsRequestResult, InstigatorState, InstigatorTick, InstigatorType, ScheduleInstigatorData, SensorInstigatorData
...
from .errors import GrapheneError, GraphenePythonError, GrapheneRepositoryLocationNotFound, GrapheneRepositoryNotFoundError, GrapheneScheduleNotFoundError, GrapheneSensorNotFoundError
...
class GrapheneInstigationState(graphene.ObjectType):
    ...
    runs = graphene.Field(non_null_list('dagster_graphql.schema.pipelines.pipeline.GrapheneRun'), limit=graphene.Int())
    runsCount = graphene.NonNull(graphene.Int)
    tick = graphene.Field(GrapheneInstigationTick, timestamp=graphene.Float())
    ticks = graphene.Field(non_null_list(GrapheneInstigationTick), dayRange=graphene.Int(), dayOffset=graphene.Int(), limit=graphene.Int(), cursor=graphene.String(), statuses=graphene.List(graphene.NonNull(GrapheneInstigationTickStatus)), beforeTimestamp=graphene.Float(), afterTimestamp=graphene.Float())
    nextTick = graphene.Field(GrapheneDryRunInstigationTick)
    runningCount = graphene.NonNull(graphene.Int)
...
class GrapheneInstigationStateOrError(graphene.Union):
    ...
    class Meta:
        ...
        types = (GrapheneInstigationState, GrapheneInstigationStateNotFoundError, GraphenePythonError)
...
types = [GrapheneDryRunInstigationTick, GrapheneDryRunInstigationTicks, GrapheneInstigationTypeSpecificData, GrapheneInstigationState, GrapheneInstigationStateNotFoundError, GrapheneInstigationStateOrError, GrapheneInstigationStates, GrapheneInstigationStatesOrError, GrapheneInstigationTick, GrapheneScheduleData, GrapheneSensorData]