...
from typing import TYPE_CHECKING, Any, AsyncContextManager, Callable, Coroutine, List, Optional, Type, TypeVar, Union
...
class Updater(AsyncContextManager['Updater']):
    ...
    """
    __slots__ = ('bot', 'update_queue', '_last_update_id', '_running', '_initialized', '_httpd', '__lock', '__polling_task')
    ...
    async def start_polling(self, poll_interval: float=0.0, timeout: int=10, bootstrap_retries: int=-1, read_timeout: float=2, write_timeout: ODVInput[float]=DEFAULT_NONE, connect_timeout: ODVInput[float]=DEFAULT_NONE, pool_timeout: ODVInput[float]=DEFAULT_NONE, allowed_updates: Optional[List[str]]=None, drop_pending_updates: Optional[bool]=None, error_callback: Optional[Callable[[TelegramError], None]]=None) -> 'asyncio.Queue[object]':
        ...
        if error_callback and asyncio.iscoroutinefunction(error_callback):
            raise TypeError('The `error_callback` must not be a coroutine function! Use an ordinary function instead. ')
    ...
    async def _start_polling(self, poll_interval: float, timeout: int, read_timeout: float, write_timeout: ODVInput[float], connect_timeout: ODVInput[float], pool_timeout: ODVInput[float], bootstrap_retries: int, drop_pending_updates: Optional[bool], allowed_updates: Optional[List[str]], ready: asyncio.Event, error_callback: Optional[Callable[[TelegramError], None]]) -> None:
        _LOGGER.debug('Updater started (polling)')
        await self._bootstrap(bootstrap_retries, drop_pending_updates=drop_pending_updates, webhook_url='', allowed_updates=None)
        _LOGGER.debug('Bootstrap done')

        async def polling_action_cb() -> bool:
            try:
                updates = await self.bot.get_updates(offset=self._last_update_id, timeout=timeout, read_timeout=read_timeout, connect_timeout=connect_timeout, write_timeout=write_timeout, pool_timeout=pool_timeout, allowed_updates=allowed_updates)
            ...
            if updates:
                if not self.running:
                    _LOGGER.critical('Updater stopped unexpectedly. Pulled updates will be ignored and pulled again on restart.')
                else:
        ...
        self.__polling_task = asyncio.create_task(self._network_loop_retry(action_cb=polling_action_cb, on_err_cb=error_callback or default_error_callback, description='getting Updates', interval=poll_interval), name='Updater:start_polling:polling_task')

        async def _get_updates_cleanup() -> None:
            _LOGGER.debug('Calling `get_updates` one more time to mark all fetched updates as read.')
            await self.bot.get_updates(offset=self._last_update_id, timeout=0, read_timeout=read_timeout, connect_timeout=connect_timeout, write_timeout=write_timeout, pool_timeout=pool_timeout, allowed_updates=allowed_updates)
        self.__polling_cleanup_cb = _get_updates_cleanup
        if ready is not None:
            ready.set()

    async def start_webhook(self, listen: str='127.0.0.1', port: int=80, url_path: str='', cert: Optional[Union[str, Path]]=None, key: Optional[Union[str, Path]]=None, bootstrap_retries: int=0, webhook_url: Optional[str]=None, allowed_updates: Optional[List[str]]=None, drop_pending_updates: Optional[bool]=None, ip_address: Optional[str]=None, max_connections: int=40, secret_token: Optional[str]=None) -> 'asyncio.Queue[object]':
        """
        ...
        if not WEBHOOKS_AVAILABLE:
            raise RuntimeError('To use `start_webhook`, PTB must be installed via `pip install "python-telegram-bot[webhooks]"`.')
        async with self.__lock:
            if self.running:
                raise RuntimeError('This Updater is already running!')
            if not self._initialized:
            ...
            try:
                ...
                await self._start_webhook(listen=listen, port=port, url_path=url_path, cert=cert, key=key, bootstrap_retries=bootstrap_retries, drop_pending_updates=drop_pending_updates, webhook_url=webhook_url, allowed_updates=allowed_updates, ready=webhook_ready, ip_address=ip_address, max_connections=max_connections, secret_token=secret_token)
    ...
    async def _start_webhook(self, listen: str, port: int, url_path: str, bootstrap_retries: int, allowed_updates: Optional[List[str]], cert: Optional[Union[str, Path]]=None, key: Optional[Union[str, Path]]=None, drop_pending_updates: Optional[bool]=None, webhook_url: Optional[str]=None, ready: Optional[asyncio.Event]=None, ip_address: Optional[str]=None, max_connections: int=40, secret_token: Optional[str]=None) -> None:
        ...
        if cert is not None and key is not None:
            try:
                ssl_ctx: Optional[ssl.SSLContext] = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)
        ...
        if not webhook_url:
            webhook_url = self._gen_webhook_url(protocol='https' if ssl_ctx else 'http', listen=listen, port=port, url_path=url_path)
        await self._bootstrap(cert=Path(cert).read_bytes() if cert else None, max_retries=bootstrap_retries, drop_pending_updates=drop_pending_updates, webhook_url=webhook_url, allowed_updates=allowed_updates, ip_address=ip_address, max_connections=max_connections, secret_token=secret_token)
    ...
    async def _network_loop_retry(self, action_cb: Callable[..., Coroutine], on_err_cb: Callable[[TelegramError], None], description: str, interval: float) -> None:
    ...
    async def _bootstrap(self, max_retries: int, webhook_url: Optional[str], allowed_updates: Optional[List[str]], drop_pending_updates: Optional[bool]=None, cert: Optional[bytes]=None, bootstrap_interval: float=1, ip_address: Optional[str]=None, max_connections: int=40, secret_token: Optional[str]=None) -> None:
        ...
        async def bootstrap_set_webhook() -> bool:
            ...
            if drop_pending_updates:
                _LOGGER.debug('Dropping pending updates from Telegram server')
            await self.bot.set_webhook(url=webhook_url, certificate=cert, allowed_updates=allowed_updates, ip_address=ip_address, drop_pending_updates=drop_pending_updates, max_connections=max_connections, secret_token=secret_token)
            return False
        ...
        if drop_pending_updates or not webhook_url:
            await self._network_loop_retry(bootstrap_del_webhook, bootstrap_on_err_cb, 'bootstrap del webhook', bootstrap_interval)
            retries = 0
        if webhook_url:
            await self._network_loop_retry(bootstrap_set_webhook, bootstrap_on_err_cb, 'bootstrap set webhook', bootstrap_interval)
    ...
    async def _stop_polling(self) -> None:
        ...
        if self.__polling_task:
            ...
            if self.__polling_cleanup_cb:
                ...
                _LOGGER.warning('No polling cleanup callback defined. The last fetched updates may be fetched again on the next polling start.')