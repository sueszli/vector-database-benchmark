...
class InputTester(App[None]):
    ...
    def compose(self) -> ComposeResult:
        if False:
            print('Hello World!')
        for (value, input_id) in (('', 'empty'), ('Shiny', 'single-word'), ('Curse your sudden but inevitable betrayal', 'multi-no-punctuation'), ('We have done the impossible, and that makes us mighty.', 'multi-punctuation'), ('Long as she does it quiet-like', 'multi-and-hyphenated')):
...
async def test_input_left_word_from_end() -> None:
    ...
    async with InputTester().run_test() as pilot:
        expected_at: dict[str | None, int] = {'empty': 0, 'single-word': 0, 'multi-no-punctuation': 33, 'multi-punctuation': 47, 'multi-and-hyphenated': 26}
        for input in pilot.app.query(Input):
            input.action_end()
            input.action_cursor_left_word()
...
async def test_input_right_word_from_home() -> None:
    ...
    async with InputTester().run_test() as pilot:
        expected_at: dict[str | None, int] = {'empty': 0, 'single-word': 5, 'multi-no-punctuation': 6, 'multi-punctuation': 3, 'multi-and-hyphenated': 5}
        for input in pilot.app.query(Input):
...
async def test_input_right_word_to_the_end() -> None:
    ...
    async with InputTester().run_test() as pilot:
        expected_hops: dict[str | None, int] = {'empty': 0, 'single-word': 1, 'multi-no-punctuation': 6, 'multi-punctuation': 10, 'multi-and-hyphenated': 7}
        for input in pilot.app.query(Input):
            hops = 0
