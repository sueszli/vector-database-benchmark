...
def _is_rng_op(opr):
    if False:
        i = 10
        return i + 15
    return isinstance(opr, (mops.Dropout, mops.BetaRNG, mops.GammaRNG, mops.GaussianRNG, mops.PermutationRNG, mops.PoissonRNG, mops.ShuffleRNG, mops.UniformRNG, mops.ExponentialRNG, mops.MultinomialRNG))
...
class DropoutMaskCanonicalizer(Pass):
    ...
    def __call__(self, tr) -> Any:
        ...
        for eqn in tr.eqns:
            ...
            new_mask_var = AbstractVar(mask_var.id, (int(np.prod(inp_shape)),), mask_var.dtype)
...
class TraceResult:
    ...
    def _str_var(self, var):
        ...
        dtype_to_str = {'float16': 'f16', 'float32': 'f32', 'int8': 'i8', 'int32': 'i32', 'int64': 'i64', 'uint8': 'u8', 'uint32': 'u32', 'uint64': 'u64', 'bool': 'i1-bool'}
        if isinstance(var, int):
            var = self.vars[var]
        var_dtype = None
        try:
...
def mge_dtype_to_ir_type(mge_dtype):
    ...
    assert isinstance(mge_dtype, np.dtype), f'arg should be numpy dtype, but is {mge_dtype}'
...
def _numpy_array_constant(x: np.ndarray, canonicalize_types) -> Sequence[ir.Value]:
    ...
    if x.dtype == np.bool_:
        ...
        if nelems == 1:
            x = np.array(0 if x.item() == 0 else 255, np.uint8)
...
def _ndarray_constant_handler(val: np.ndarray, canonicalize_types) -> Sequence[ir.Value]:
    if False:
        for i in range(10):
            print('nop')
    if np.any(np.equal(0, val.strides)) and val.size > 0:
        ...
        collapsed_val = val[tuple((0 if ax in zero_stride_axes else slice(None) for ax in range(val.ndim)))]
        if canonicalize_types:
            collapsed_val = np.asarray(collapsed_val, dtype.canonicalize_dtype(collapsed_val.dtype))
        out = hlo.BroadcastInDimOp(ir.RankedTensorType.get(val.shape, mge_dtype_to_ir_type(collapsed_val.dtype)), _numpy_array_constant(collapsed_val, canonicalize_types=False)[0], dense_int_elements(other_axes)).result
        return (out,)
    else:
        return _numpy_array_constant(val, canonicalize_types)
_constant_handlers[np.ndarray] = _ndarray_constant_handler
for _scalar_type in [np.int8, np.int16, np.int32, np.int64, np.uint8, np.uint16, np.uint32, np.uint64, np.float16, np.float32, np.float64, np.complex64, np.complex128, np.bool_, np.longlong, dtype.bfloat16]:
...
def _mge_varinfo_constant_handler(val, canonicalize_dtypes):
    ...
    return _numpy_array_constant(np.asarray(val.bound_data, val.dtype), canonicalize_dtypes)
...
def precision_attr(lhs_prec, rhs_prec) -> ir.ArrayAttr:
    ...
    dtype_to_precision = {'float32': 'DEFAULT'}
...
def dense_bool_elements(xs: Sequence[bool]) -> ir.DenseElementsAttr:
    ...
    if len(xs) == 1:
        a = np.array(0 if a.item() == 0 else 255, np.uint8)
    return ir.DenseElementsAttr.get(a, type=ir.IntegerType.get_signless(1), shape=[len(xs)])
...
def get_irnode_dtype(irnode):
    ...
    if isinstance(irnode, (list, tuple, ir.OpResultList)):
        assert len(irnode) == 1
        irnode = irnode[0]
    assert isinstance(irnode, (ir.RankedTensorType, ir.BlockArgument, ir.OpResult)), type(irnode)
    if not isinstance(irnode, ir.RankedTensorType):
        irnode = ir.RankedTensorType(irnode.type)
...
def module_to_string(module: ir.Module) -> str:
    if False:
        print('Hello World!')
    output = io.StringIO()
    module.operation.print(file=output, enable_debug_info=True, print_generic_op_form=False)
