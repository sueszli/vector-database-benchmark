...
async def flow_run(prefect_client: PrefectClient):
    flow_run = await prefect_client.create_flow_run(flow=example_process_worker_flow, state=State(type=StateType.SCHEDULED, state_details=StateDetails(scheduled_time=pendulum.now('utc').subtract(minutes=5))))
    return flow_run
...
async def test_worker_process_run_flow_run(flow_run, patch_run_process, work_pool, monkeypatch):
    mock: AsyncMock = patch_run_process()
    patch_client(monkeypatch)
    async with ProcessWorker(work_pool_name=work_pool.name) as worker:
        worker._work_pool = work_pool
        result = await worker.run(flow_run, configuration=await worker._get_configuration(flow_run))
...
async def test_worker_process_run_flow_run_with_env_variables_job_config_defaults(flow_run, patch_run_process, work_pool_with_default_env, monkeypatch):
    monkeypatch.setenv('EXISTING_ENV_VAR', 'from_os')
    mock: AsyncMock = patch_run_process()
    patch_client(monkeypatch)
    async with ProcessWorker(work_pool_name=work_pool_with_default_env.name) as worker:
        ...
        assert mock.call_args.kwargs['env']['CONFIG_ENV_VAR'] == 'from_job_configuration'

async def test_worker_process_run_flow_run_with_env_variables_from_overrides(flow_run, patch_run_process, work_pool_with_default_env, monkeypatch):
    monkeypatch.setenv('EXISTING_ENV_VAR', 'from_os')
    mock: AsyncMock = patch_run_process()
...
async def test_process_created_then_marked_as_started(flow_run, mock_open_process, work_pool, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match='Started called!'):
        async with ProcessWorker(work_pool_name=work_pool.name) as worker:
            worker._work_pool = work_pool
            await worker.run(flow_run=flow_run, configuration=fake_configuration, task_status=fake_status)
    fake_status.started.assert_called_once()
    mock_open_process.assert_awaited_once()

@pytest.mark.parametrize('exit_code,help_message', [(-9, 'This indicates that the process exited due to a SIGKILL signal'), (247, 'This indicates that the process was terminated due to high memory usage.')])
async def test_process_worker_logs_exit_code_help_message(exit_code, help_message, caplog, patch_run_process, flow_run, work_pool, monkeypatch):
    patch_client(monkeypatch)
    patch_run_process(returncode=exit_code)
    async with ProcessWorker(work_pool_name=work_pool.name) as worker:
        worker._work_pool = work_pool
        result = await worker.run(flow_run=flow_run, configuration=await worker._get_configuration(flow_run))
        assert result.status_code == exit_code
        record = caplog.records[-1]
        assert record.levelname == 'ERROR'
        assert help_message in record.message

@pytest.mark.skipif(sys.platform != 'win32', reason='subprocess.CREATE_NEW_PROCESS_GROUP is only defined in Windows')
async def test_windows_process_worker_run_sets_process_group_creation_flag(patch_run_process, flow_run, work_pool, monkeypatch):
    mock = patch_run_process()
    patch_client(monkeypatch)
    async with ProcessWorker(work_pool_name=work_pool.name) as worker:
        worker._work_pool = work_pool
        await worker.run(flow_run=flow_run, configuration=await worker._get_configuration(flow_run))
    mock.assert_awaited_once()
    (_, kwargs) = mock.call_args
    assert kwargs.get('creationflags') == mock.CREATE_NEW_PROCESS_GROUP

@pytest.mark.skipif(sys.platform == 'win32', reason='The asyncio.open_process_*.creationflags argument is only supported on Windows')
async def test_unix_process_worker_run_does_not_set_creation_flag(patch_run_process, flow_run, work_pool, monkeypatch):
    mock = patch_run_process()
    patch_client(monkeypatch)
    async with ProcessWorker(work_pool_name=work_pool.name) as worker:
        worker._work_pool = work_pool
...
async def test_process_worker_working_dir_override(flow_run, patch_run_process, work_pool, monkeypatch):
    mock: AsyncMock = patch_run_process()
...
async def test_process_worker_stream_output_override(flow_run, patch_run_process, work_pool, monkeypatch):
    mock: AsyncMock = patch_run_process()
    patch_client(monkeypatch, overrides={})
    async with ProcessWorker(work_pool_name=work_pool.name) as worker:
        worker._work_pool = work_pool
...
async def test_process_worker_uses_correct_default_command(flow_run, patch_run_process, work_pool, monkeypatch):
    mock: AsyncMock = patch_run_process()
...
async def test_process_worker_command_override(flow_run, patch_run_process, work_pool, monkeypatch):
    mock: AsyncMock = patch_run_process()
...
async def test_task_status_receives_infrastructure_pid(work_pool, patch_run_process, monkeypatch, flow_run):
    patch_client(monkeypatch)
    fake_status = MagicMock(spec=anyio.abc.TaskStatus)
    async with ProcessWorker(work_pool_name=work_pool.name) as worker:
        worker._work_pool = work_pool
        result = await worker.run(flow_run=flow_run, configuration=await worker._get_configuration(flow_run), task_status=fake_status)
...
async def test_process_kill_mismatching_hostname(monkeypatch, work_pool):
    ...
    async with ProcessWorker(work_pool_name=work_pool.name) as worker:
        with pytest.raises(InfrastructureNotAvailable):
            await worker.kill_infrastructure(infrastructure_pid=infrastructure_pid, configuration=ProcessJobConfiguration())
...
async def test_process_kill_sends_sigterm_then_sigkill(monkeypatch, work_pool):
    ...
    grace_seconds = 2
    async with ProcessWorker(work_pool_name=work_pool.name) as worker:
        await worker.kill_infrastructure(infrastructure_pid=infrastructure_pid, grace_seconds=grace_seconds, configuration=ProcessJobConfiguration())
    os_kill.assert_has_calls([call(12345, signal.SIGTERM), call(12345, 0), call(12345, signal.SIGKILL)])
...
async def test_process_kill_early_return(monkeypatch, work_pool):
    ...
    async with ProcessWorker(work_pool_name=work_pool.name) as worker:
        await worker.kill_infrastructure(infrastructure_pid=infrastructure_pid, grace_seconds=grace_seconds, configuration=ProcessJobConfiguration())
    os_kill.assert_has_calls([call(12345, signal.SIGTERM), call(12345, 0)])
    anyio_sleep.assert_called_once_with(3)

@pytest.mark.skipif(sys.platform != 'win32', reason='CTRL_BREAK_EVENT is only defined in Windows')
