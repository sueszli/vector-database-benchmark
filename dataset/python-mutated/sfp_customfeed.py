...
malchecks = {'Custom Threat Data': {'id': '_customfeed', 'checks': ['ip', 'netblock', 'asn', 'domain'], 'regex': '^{0}$'}}

class sfp_customfeed(SpiderFootPlugin):
    meta = {'name': 'Custom Threat Feed', 'summary': 'Check if a host/domain, netblock, ASN or IP is malicious according to your custom feed.', 'flags': [], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems']}
    opts = {'checkaffiliates': True, 'checkcohosts': True, 'url': '', 'cacheperiod': 0}
    optdescs = {'url': 'The URL where the feed can be found. Exact matching is performed so the format must be a single line per host, ASN, domain, IP or netblock.', 'checkaffiliates': 'Apply checks to affiliates?', 'checkcohosts': "Apply checks to sites found to be co-hosted on the target's IP?", 'cacheperiod': 'Maximum age of data in hours before re-downloading. 0 to always download.'}
    results = None
    errorState = False
