...
CONFIG_SCHEMA = AUTH_PROVIDER_SCHEMA.extend({vol.Required(CONF_TRUSTED_NETWORKS): vol.All(cv.ensure_list, [ip_network]), vol.Optional(CONF_TRUSTED_USERS, default={}): vol.Schema({ip_network: vol.All(cv.ensure_list, [vol.Or(cv.uuid4_hex, vol.Schema({vol.Required(CONF_GROUP): str}))])}), vol.Optional(CONF_ALLOW_BYPASS_LOGIN, default=False): cv.boolean}, extra=vol.PREVENT_EXTRA)
    ...
    async def async_login_flow(self, context: dict[str, Any] | None) -> LoginFlow:
        ...
        for (ip_net, user_or_group_list) in self.trusted_users.items():
            if ip_addr not in ip_net:
                continue
            user_list = [user_id for user_id in user_or_group_list if isinstance(user_id, str)]
            group_list = [group[CONF_GROUP] for group in user_or_group_list if isinstance(group, dict)]
            flattened_group_list = [group for sublist in group_list for group in sublist]
            available_users = [user for user in available_users if user.id in user_list or any((group.id in flattened_group_list for group in user.groups))]
            break
        return TrustedNetworksLoginFlow(self, ip_addr, {user.id: user.name for user in available_users}, self.config[CONF_ALLOW_BYPASS_LOGIN])
...
class TrustedNetworksLoginFlow(LoginFlow):
    ...
    async def async_step_init(self, user_input: dict[str, str] | None=None) -> FlowResult:
        ...
        if self._allow_bypass_login and len(self._available_users) == 1:
            return await self.async_finish({'user': next(iter(self._available_users.keys()))})
        return self.async_show_form(step_id='init', data_schema=vol.Schema({vol.Required('user'): vol.In(self._available_users)}))