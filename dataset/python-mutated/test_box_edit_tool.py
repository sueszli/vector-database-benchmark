...
from bokeh.models import BoxEditTool, ColumnDataSource, CustomJS, Div, Plot, Range1d, Rect
...
def _make_plot(dimensions='both', num_objects: int=0) -> Plot:
    ...
    code = RECORD('x', 'source.data.x', final=False) + RECORD('y', 'source.data.y', final=False) + RECORD('width', 'source.data.width', final=False) + RECORD('height', 'source.data.height')
...
class Test_BoxEditTool:
    ...
    def test_double_click_triggers_draw(self, single_plot_page: SinglePlotPage) -> None:
        ...
        expected = {'x': [1, 2, 1.2162162162162162], 'y': [1, 1, 1.875], 'width': [0.5, 0.5, 0.8108108108108109], 'height': [0.5, 0.5, 0.75]}
    ...
    def test_shift_drag_triggers_draw(self, single_plot_page: SinglePlotPage) -> None:
        ...
        time.sleep(0.5)
        page.eval_custom_action()
        expected = {'x': [1, 2, 1.0135135135135136], 'y': [1, 1, 2.0625], 'width': [0.5, 0.5, 0.4054054054054054], 'height': [0.5, 0.5, 0.3750000000000002]}
        assert cds_data_almost_equal(page.results, expected)
        assert page.has_no_console_errors()
    ...
    def test_drag_moves_box(self, single_plot_page: SinglePlotPage) -> None:
        ...
        time.sleep(0.5)
        page.eval_custom_action()
        expected = {'x': [1, 2, 1.6216216216216217], 'y': [1, 1, 1.5000000000000002], 'width': [0.5, 0.5, 0.8108108108108109], 'height': [0.5, 0.5, 0.75]}
        assert cds_data_almost_equal(page.results, expected)
        assert page.has_no_console_errors()
    ...
    def test_num_objects_limits_drawn_boxes(self, single_plot_page: SinglePlotPage) -> None:
        ...
        time.sleep(0.5)
        page.eval_custom_action()
        expected = {'x': [2, 1.0135135135135136], 'y': [1, 2.0625], 'width': [0.5, 0.4054054054054054], 'height': [0.5, 0.3750000000000002]}
        assert cds_data_almost_equal(page.results, expected)
        assert page.has_no_console_errors()
