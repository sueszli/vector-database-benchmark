...
OrganisationSubscriptionInformationCacheDict = typing.Dict[int, OrganisationSubscriptionInformationCache]
...
def update_caches(update_cache_entities: typing.Tuple[SubscriptionCacheEntity, ...]):
    ...
    organisations = Organisation.objects.select_related('subscription_information_cache', 'subscription').all()
    organisation_info_cache_dict: typing.Dict[int, OrganisationSubscriptionInformationCache] = {org.id: getattr(org, 'subscription_information_cache', None) or OrganisationSubscriptionInformationCache(organisation=org) for org in organisations}
    ...
    OrganisationSubscriptionInformationCache.objects.bulk_update(to_update, fields=['api_calls_24h', 'api_calls_7d', 'api_calls_30d', 'allowed_seats', 'allowed_30d_api_calls', 'chargebee_email', 'chargebee_updated_at', 'influx_updated_at'])

def _update_caches_with_influx_data(organisation_info_cache_dict: OrganisationSubscriptionInformationCacheDict) -> None:
    if False:
        while True:
            i = 10
...
def _update_caches_with_chargebee_data(organisations: typing.Iterable[Organisation], organisation_info_cache_dict: OrganisationSubscriptionInformationCacheDict):
    if False:
        print('Hello World!')
    "\n    Mutates the provided organisation_info_cache_dict in place to add information about the organisation's\n    chargebee plan.\n    "
    ...
    for organisation in organisations:
        ...
        if not subscription or subscription.subscription_id is None or subscription.payment_method != CHARGEBEE:
            continue
