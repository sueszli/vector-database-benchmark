...
class CountedOctetString(Structure):
    ...
    """
    structure = (('length', '!H=0'), ('_data', '_-data', 'self["length"]'), ('data', ':'))
...
class KeyBlock(Structure):
    structure = (('keytype', '!H=0'), ('keyvalue', ':', CountedOctetString))
...
class KeytabPrincipal:
    ...
    class PrincipalHeader1(Structure):
        structure = (('num_components', '!H=0'),)

    class PrincipalHeader2(Structure):
        structure = (('name_type', '!L=0'),)
...
class KeytabEntry:
    ...
    class KeytabEntryMainpart(Structure):
      ...
      keytab_entry {
          ...
          uint16_t num_components;    /* sub 1 if version 0x501 */  |          counted_octet_string realm;                               | \\  Keytab
        ...
        """
        structure = (('size', '!l=0'), ('principal', ':', KeytabPrincipal), ('timestamp', '!L=0'), ('vno8', '!B=0'), ('keyblock', ':', KeyBlock))
...
class Keytab:
    GetkeyEnctypePreference = (Enctype.AES256.value, Enctype.AES128.value, Enctype.RC4.value)

    class MiniHeader(Structure):
        structure = (('file_format_version', '!H=0x0502'),)
