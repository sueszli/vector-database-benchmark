...
from sentry.db.models import BaseManager, BoundedBigIntegerField, BoundedPositiveIntegerField, FlexibleForeignKey, JSONField, Model, region_silo_only_model, sane_repr
if TYPE_CHECKING:
...
class GroupLink(Model):
    ...
    linked_type = BoundedPositiveIntegerField(default=LinkedType.commit, choices=((LinkedType.commit, _('Commit')), (LinkedType.pull_request, _('Pull Request')), (LinkedType.issue, _('Tracker Issue'))))
    linked_id = BoundedBigIntegerField()
    relationship = BoundedPositiveIntegerField(default=Relationship.references, choices=((Relationship.resolves, _('Resolves')), (Relationship.references, _('Linked'))))
    data = JSONField()
