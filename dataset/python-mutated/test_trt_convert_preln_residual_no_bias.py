            ...
            for epsilon in [1e-05]:
                for begin_norm_axis in [2]:
                    for enable_int8 in [False, True]:
                        ...
                        ops_config = [{'op_type': 'elementwise_add', 'op_inputs': {'X': ['inputX_data'], 'Y': ['inputY_data']}, 'op_outputs': {'Out': ['ele_out']}, 'op_attrs': {'axis': -1}}, {'op_type': 'layer_norm', 'op_inputs': {'X': ['ele_out'], 'Bias': ['Bias'], 'Scale': ['Scale']}, 'op_outputs': {'Y': ['layernorm_out'], 'Mean': ['Mean'], 'Variance': ['Variance']}, 'op_attrs': dics[0]}]
                        ops = self.generate_op_config(ops_config)
                        program_config = ProgramConfig(ops=ops, weights={'Bias': TensorConfig(data_gen=partial(generate_weight1, dics)), 'Scale': TensorConfig(data_gen=partial(generate_weight2, dics))}, inputs={'inputX_data': TensorConfig(data_gen=partial(generate_input1, dics, batch)), 'inputY_data': TensorConfig(data_gen=partial(generate_input2, dics, batch))}, outputs=['ele_out', 'layernorm_out'])
