...
class Playlist:
    ...
    async def save(self):
        ...
        await self.playlist_api.upsert(scope, playlist_id=int(self.id), playlist_name=self.name, scope_id=scope_id, author_id=self.author_id, playlist_url=self.url, tracks=self.tracks)
    ...
    @classmethod
    async def from_json(cls, bot: Red, playlist_api: PlaylistWrapper, scope: str, playlist_number: int, data: PlaylistFetchResult, **kwargs) -> 'Playlist':
        ...
        **kwargs
        ...
        tracks = data.tracks
        return cls(bot=bot, playlist_api=playlist_api, guild=guild, scope=scope, author=author, playlist_id=playlist_id, name=name, playlist_url=playlist_url, tracks=tracks)
...
class PlaylistCompat23:
    ...
    @classmethod
    async def from_json(cls, bot: Red, playlist_api: PlaylistWrapper, scope: str, playlist_number: int, data: MutableMapping, **kwargs) -> 'PlaylistCompat23':
        """Get a Playlist object from the provided information.
        Parameters
        ----------
        bot: Red
    ...
    async def save(self):
        ...
        await self.playlist_api.upsert(scope, playlist_id=int(self.id), playlist_name=self.name, scope_id=scope_id, author_id=self.author, playlist_url=self.url, tracks=self.tracks)

async def get_all_playlist_for_migration23(bot: Red, playlist_api: PlaylistWrapper, config: Config, scope: str, guild: Union[discord.Guild, int]=None) -> List[PlaylistCompat23]:
    """
    ...
    if scope == PlaylistScope.GLOBAL.value:
        return [await PlaylistCompat23.from_json(bot, playlist_api, scope, playlist_number, playlist_data, guild=guild, author=int(playlist_data.get('author', 0))) async for (playlist_number, playlist_data) in AsyncIter(playlists.items())]
    elif scope == PlaylistScope.USER.value:
        return [await PlaylistCompat23.from_json(bot, playlist_api, scope, playlist_number, playlist_data, guild=guild, author=int(user_id)) async for (user_id, scopedata) in AsyncIter(playlists.items()) async for (playlist_number, playlist_data) in AsyncIter(scopedata.items())]
    else:
        return [await PlaylistCompat23.from_json(bot, playlist_api, scope, playlist_number, playlist_data, guild=int(guild_id), author=int(playlist_data.get('author', 0))) async for (guild_id, scopedata) in AsyncIter(playlists.items()) async for (playlist_number, playlist_data) in AsyncIter(scopedata.items())]

async def get_playlist(playlist_number: int, scope: str, bot: Red, playlist_api: PlaylistWrapper, guild: Union[discord.Guild, int]=None, author: Union[discord.abc.User, int]=None) -> Playlist:
    """
    ...
    return await Playlist.from_json(bot, playlist_api, scope_standard, playlist_number, playlist_data, guild=guild, author=author)

async def get_all_playlist(scope: str, bot: Red, playlist_api: PlaylistWrapper, guild: Union[discord.Guild, int]=None, author: Union[discord.abc.User, int]=None, specified_user: bool=False) -> List[Playlist]:
    """
    Gets all playlist for the specified scope.
    Parameters
    ----------
    ...
    async for playlist in AsyncIter(playlists):
        playlist_list.append(await Playlist.from_json(bot, playlist_api, scope, playlist.playlist_id, playlist, guild=guild, author=author))
    return playlist_list

async def get_all_playlist_converter(scope: str, bot: Red, playlist_api: PlaylistWrapper, arg: str, guild: Union[discord.Guild, int]=None, author: Union[discord.abc.User, int]=None) -> List[Playlist]:
    """
    Gets all playlist for the specified scope.
    Parameters
    ----------
    ...
    """
    (scope_standard, scope_id) = prepare_config_scope(bot, scope, author, guild)
    playlists = await playlist_api.fetch_all_converter(scope_standard, playlist_name=arg, playlist_id=arg)
    playlist_list = []
    async for playlist in AsyncIter(playlists):
        playlist_list.append(await Playlist.from_json(bot, playlist_api, scope, playlist.playlist_id, playlist, guild=guild, author=author))
    return playlist_list

async def create_playlist(ctx: commands.Context, playlist_api: PlaylistWrapper, scope: str, playlist_name: str, playlist_url: Optional[str]=None, tracks: Optional[List[MutableMapping]]=None, author: Optional[discord.User]=None, guild: Optional[discord.Guild]=None) -> Optional[Playlist]:
    ...
    `MissingAuthor`
        Trying to access the User scope without an user id.
    """
    playlist = Playlist(ctx.bot, playlist_api, scope, author.id if author else None, ctx.message.id, playlist_name, playlist_url, tracks, guild or ctx.guild)
    await playlist.save()
    return playlist

async def reset_playlist(bot: Red, playlist_api: PlaylistWrapper, scope: str, guild: Union[discord.Guild, int]=None, author: Union[discord.abc.User, int]=None) -> None:
...
async def delete_playlist(bot: Red, playlist_api: PlaylistWrapper, scope: str, playlist_id: Union[str, int], guild: discord.Guild, author: Union[discord.abc.User, int]=None) -> None:
