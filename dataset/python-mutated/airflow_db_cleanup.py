...
from airflow.models import DAG, DagModel, DagRun, Log, SlaMiss, TaskInstance, Variable, XCom
...
PRINT_DELETES = False
ENABLE_DELETE = True
DATABASE_OBJECTS = [{'airflow_db_model': DagRun, 'age_check_column': DagRun.execution_date, 'keep_last': True, 'keep_last_filters': [DagRun.external_trigger.is_(False)], 'keep_last_group_by': DagRun.dag_id}, {'airflow_db_model': TaskInstance, 'age_check_column': TaskInstance.execution_date if AIRFLOW_VERSION < ['2', '2', '0'] else TaskInstance.start_date, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None}, {'airflow_db_model': Log, 'age_check_column': Log.dttm, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None}, {'airflow_db_model': XCom, 'age_check_column': XCom.execution_date if AIRFLOW_VERSION < ['2', '2', '5'] else XCom.timestamp, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None}, {'airflow_db_model': SlaMiss, 'age_check_column': SlaMiss.execution_date, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None}, {'airflow_db_model': DagModel, 'age_check_column': DagModel.last_parsed_time, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None}]
try:
    from airflow.models import TaskReschedule
    DATABASE_OBJECTS.append({'airflow_db_model': TaskReschedule, 'age_check_column': TaskReschedule.execution_date if AIRFLOW_VERSION < ['2', '2', '0'] else TaskReschedule.start_date, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None})
except Exception as e:
    logging.error(e)
try:
    from airflow.models import TaskFail
    DATABASE_OBJECTS.append({'airflow_db_model': TaskFail, 'age_check_column': TaskFail.execution_date, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None})
except Exception as e:
    logging.error(e)
if AIRFLOW_VERSION < ['2', '4', '0']:
    try:
        from airflow.models import RenderedTaskInstanceFields
        DATABASE_OBJECTS.append({'airflow_db_model': RenderedTaskInstanceFields, 'age_check_column': RenderedTaskInstanceFields.execution_date, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None})
    except Exception as e:
        logging.error(e)
try:
    from airflow.models import ImportError
    DATABASE_OBJECTS.append({'airflow_db_model': ImportError, 'age_check_column': ImportError.timestamp, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None, 'do_not_delete_by_dag_id': True})
except Exception as e:
    logging.error(e)
if AIRFLOW_VERSION < ['2', '6', '0']:
    try:
        from airflow.jobs.base_job import BaseJob
        DATABASE_OBJECTS.append({'airflow_db_model': BaseJob, 'age_check_column': BaseJob.latest_heartbeat, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None})
    except Exception as e:
        logging.error(e)
else:
    try:
        from airflow.jobs.job import Job
        DATABASE_OBJECTS.append({'airflow_db_model': Job, 'age_check_column': Job.latest_heartbeat, 'keep_last': False, 'keep_last_filters': None, 'keep_last_group_by': None})
    except Exception as e:
        logging.error(e)
default_args = {'owner': DAG_OWNER_NAME, 'depends_on_past': False, 'email': ALERT_EMAIL_ADDRESSES, 'email_on_failure': True, 'email_on_retry': False, 'start_date': START_DATE, 'retries': 1, 'retry_delay': timedelta(minutes=1)}
...
print_configuration = PythonOperator(task_id='print_configuration', python_callable=print_configuration_function, provide_context=True, dag=dag)
...
def analyze_db():
    ...
    session.commit()
    session.close()
analyze_op = PythonOperator(task_id='analyze_query', python_callable=analyze_db, provide_context=True, dag=dag)
cleanup_session_op = PythonOperator(task_id='cleanup_sessions', python_callable=cleanup_sessions, provide_context=True, dag=dag)
cleanup_session_op.set_downstream(analyze_op)
for db_object in DATABASE_OBJECTS:
    cleanup_op = PythonOperator(task_id='cleanup_' + str(db_object['airflow_db_model'].__name__), python_callable=cleanup_function, params=db_object, provide_context=True, dag=dag)
