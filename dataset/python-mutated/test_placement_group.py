...
if __name__ == '__main__':
    ...
    pg_launchers = []
    for i in range(3):
        pre_created_pgs = [placement_group(BUNDLES, strategy='STRICT_SPREAD') for _ in range(pre_created_num_pgs // 3)]
    ...
    result['success'] = 1
    result['perf_metrics'] = [{'perf_metric_name': 'avg_pg_create_time_ms', 'perf_metric_value': result['avg_pg_create_time_ms'], 'perf_metric_type': 'LATENCY'}, {'perf_metric_name': 'avg_pg_remove_time_ms', 'perf_metric_value': result['avg_pg_remove_time_ms'], 'perf_metric_type': 'LATENCY'}]
    print(f'Avg placement group creating time: {total_creating_time / total_trial * 1000} ms')
    print(f'Avg placement group removing time: {total_removing_time / total_trial * 1000} ms')
    print('PASSED.')
