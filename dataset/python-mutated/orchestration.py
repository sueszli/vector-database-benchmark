...
from prefect.client.schemas.actions import ArtifactCreate, BlockDocumentCreate, BlockDocumentUpdate, BlockSchemaCreate, BlockTypeCreate, BlockTypeUpdate, ConcurrencyLimitCreate, DeploymentCreate, DeploymentFlowRunCreate, DeploymentUpdate, FlowCreate, FlowRunCreate, FlowRunNotificationPolicyCreate, FlowRunUpdate, LogCreate, TaskRunCreate, TaskRunUpdate, WorkPoolCreate, WorkPoolUpdate, WorkQueueCreate, WorkQueueUpdate
from prefect.client.schemas.filters import ArtifactCollectionFilter, ArtifactFilter, DeploymentFilter, FlowFilter, FlowRunFilter, FlowRunNotificationPolicyFilter, LogFilter, TaskRunFilter, WorkerFilter, WorkPoolFilter, WorkQueueFilter, WorkQueueFilterName
from prefect.client.schemas.objects import Artifact, ArtifactCollection, BlockDocument, BlockSchema, BlockType, ConcurrencyLimit, Constant, Deployment, Flow, FlowRunNotificationPolicy, FlowRunPolicy, Log, Parameter, QueueFilter, TaskRunPolicy, TaskRunResult, Variable, Worker, WorkPool, WorkQueue
from prefect.client.schemas.responses import DeploymentResponse, FlowRunResponse, WorkerFlowRunResponse
from prefect.client.schemas.schedules import SCHEDULE_TYPES
from prefect.client.schemas.sorting import ArtifactCollectionSort, ArtifactSort, DeploymentSort, FlowRunSort, FlowSort, LogSort, TaskRunSort
from prefect.deprecated.data_documents import DataDocument
from prefect.events.schemas import Automation, ExistingAutomation
from prefect.logging import get_logger
from prefect.settings import PREFECT_API_DATABASE_CONNECTION_URL, PREFECT_API_ENABLE_HTTP2, PREFECT_API_KEY, PREFECT_API_REQUEST_TIMEOUT, PREFECT_API_TLS_INSECURE_SKIP_VERIFY, PREFECT_API_URL, PREFECT_CLOUD_API_URL, PREFECT_UNIT_TEST_MODE
...
class PrefectClient:
    ...
    async def create_flow_from_name(self, flow_name: str) -> UUID:
        ...
        response = await self._client.post('/flows/', json=flow_data.dict(json_compatible=True))
    ...
    async def read_flows(self, *, flow_filter: FlowFilter=None, flow_run_filter: FlowRunFilter=None, task_run_filter: TaskRunFilter=None, deployment_filter: DeploymentFilter=None, work_pool_filter: WorkPoolFilter=None, work_queue_filter: WorkQueueFilter=None, sort: FlowSort=None, limit: int=None, offset: int=0) -> List[Flow]:
        ...
        """
        body = {'flows': flow_filter.dict(json_compatible=True) if flow_filter else None, 'flow_runs': flow_run_filter.dict(json_compatible=True, exclude_unset=True) if flow_run_filter else None, 'task_runs': task_run_filter.dict(json_compatible=True) if task_run_filter else None, 'deployments': deployment_filter.dict(json_compatible=True) if deployment_filter else None, 'work_pools': work_pool_filter.dict(json_compatible=True) if work_pool_filter else None, 'work_queues': work_queue_filter.dict(json_compatible=True) if work_queue_filter else None, 'sort': sort, 'limit': limit, 'offset': offset}
    ...
    async def create_flow_run_from_deployment(self, deployment_id: UUID, *, parameters: Dict[str, Any]=None, context: dict=None, state: prefect.states.State=None, name: str=None, tags: Iterable[str]=None, idempotency_key: str=None, parent_task_run_id: UUID=None, work_queue_name: str=None) -> FlowRun:
        """
        ...
        tags = tags or []
        flow_run_create = DeploymentFlowRunCreate(parameters=parameters, context=context, state=state.to_state_create(), tags=tags, name=name, idempotency_key=idempotency_key, parent_task_run_id=parent_task_run_id)
        if work_queue_name:
            flow_run_create.work_queue_name = work_queue_name
        response = await self._client.post(f'/deployments/{deployment_id}/create_flow_run', json=flow_run_create.dict(json_compatible=True, exclude_unset=True))
        return FlowRun.parse_obj(response.json())

    async def create_flow_run(self, flow: 'FlowObject', name: str=None, parameters: Dict[str, Any]=None, context: dict=None, tags: Iterable[str]=None, parent_task_run_id: UUID=None, state: 'prefect.states.State'=None) -> FlowRun:
        """
        ...
        flow_run_create = FlowRunCreate(flow_id=flow_id, flow_version=flow.version, name=name, parameters=parameters, context=context, tags=list(tags or []), parent_task_run_id=parent_task_run_id, state=state.to_state_create(), empirical_policy=FlowRunPolicy(retries=flow.retries, retry_delay=flow.retry_delay_seconds))
    ...
    async def update_flow_run(self, flow_run_id: UUID, flow_version: Optional[str]=None, parameters: Optional[dict]=None, name: Optional[str]=None, tags: Optional[Iterable[str]]=None, empirical_policy: Optional[FlowRunPolicy]=None, infrastructure_pid: Optional[str]=None) -> httpx.Response:
        """
        ...
        return await self._client.patch(f'/flow_runs/{flow_run_id}', json=flow_run_data.dict(json_compatible=True, exclude_unset=True))
    ...
    async def create_concurrency_limit(self, tag: str, concurrency_limit: int) -> UUID:
        ...
        response = await self._client.post('/concurrency_limits/', json=concurrency_limit_create.dict(json_compatible=True))
        concurrency_limit_id = response.json().get('id')
        if not concurrency_limit_id:
            raise httpx.RequestError(f'Malformed response: {response}')
        return UUID(concurrency_limit_id)

    async def read_concurrency_limit_by_tag(self, tag: str):
        """
        ...
        """
        try:
            response = await self._client.get(f'/concurrency_limits/tag/{tag}')
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            else:
    ...
    async def read_concurrency_limits(self, limit: int, offset: int):
        """
    ...
    async def reset_concurrency_limit_by_tag(self, tag: str, slot_override: Optional[List[Union[UUID, str]]]=None):
        """
        ...
        try:
            await self._client.post(f'/concurrency_limits/tag/{tag}/reset', json=dict(slot_override=slot_override))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            else:
    ...
    async def delete_concurrency_limit_by_tag(self, tag: str):
        """
        ...
        """
        try:
            await self._client.delete(f'/concurrency_limits/tag/{tag}')
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            else:
    ...
    async def create_work_queue(self, name: str, tags: Optional[List[str]]=None, description: Optional[str]=None, is_paused: Optional[bool]=None, concurrency_limit: Optional[int]=None, priority: Optional[int]=None, work_pool_name: Optional[str]=None) -> WorkQueue:
        """
        Create a work queue.
        ...
        """
        if tags:
            warnings.warn('The use of tags for creating work queue filters is deprecated. This option will be removed on 2023-02-23.', DeprecationWarning)
    ...
    async def get_runs_in_work_queue(self, id: UUID, limit: int=10, scheduled_before: datetime.datetime=None) -> List[FlowRun]:
        """
    ...
    async def delete_work_queue_by_id(self, id: UUID):
        """
    ...
    async def create_block_type(self, block_type: BlockTypeCreate) -> BlockType:
        ...
        """
        try:
            response = await self._client.post('/block_types/', json=block_type.dict(json_compatible=True, exclude_unset=True, exclude={'id'}))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_409_CONFLICT:
                raise prefect.exceptions.ObjectAlreadyExists(http_exc=e) from e
            else:
    ...
    async def create_block_schema(self, block_schema: BlockSchemaCreate) -> BlockSchema:
        ...
        """
        try:
            response = await self._client.post('/block_schemas/', json=block_schema.dict(json_compatible=True, exclude_unset=True, exclude={'id', 'block_type', 'checksum'}))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_409_CONFLICT:
                raise prefect.exceptions.ObjectAlreadyExists(http_exc=e) from e
            else:
    ...
    async def create_block_document(self, block_document: Union[BlockDocument, BlockDocumentCreate], include_secrets: bool=True) -> BlockDocument:
        ...
        if isinstance(block_document, BlockDocument):
            block_document = BlockDocumentCreate.parse_obj(block_document.dict(json_compatible=True, include_secrets=include_secrets, exclude_unset=True, exclude={'id', 'block_schema', 'block_type'}))
        try:
            response = await self._client.post('/block_documents/', json=block_document.dict(json_compatible=True, include_secrets=include_secrets, exclude_unset=True, exclude={'id', 'block_schema', 'block_type'}))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_409_CONFLICT:
                raise prefect.exceptions.ObjectAlreadyExists(http_exc=e) from e
            else:
    ...
    async def update_block_document(self, block_document_id: UUID, block_document: BlockDocumentUpdate):
        """
        Update a block document in the Prefect API.
        """
        try:
            await self._client.patch(f'/block_documents/{block_document_id}', json=block_document.dict(json_compatible=True, exclude_unset=True, include={'data', 'merge_existing_data', 'block_schema_id'}, include_secrets=True))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            else:
    ...
    async def read_block_schema_by_checksum(self, checksum: str, version: Optional[str]=None) -> BlockSchema:
        """
    ...
    async def update_block_type(self, block_type_id: UUID, block_type: BlockTypeUpdate):
        ...
        """
        try:
            await self._client.patch(f'/block_types/{block_type_id}', json=block_type.dict(json_compatible=True, exclude_unset=True, include=BlockTypeUpdate.updatable_fields(), include_secrets=True))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            else:
    ...
    async def delete_block_type(self, block_type_id: UUID):
        ...
        try:
            ...
            if e.response.status_code == 404:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            elif e.response.status_code == status.HTTP_403_FORBIDDEN and e.response.json()['detail'] == 'protected block types cannot be deleted.':
                raise prefect.exceptions.ProtectedBlockError('Protected block types cannot be deleted.') from e
            else:
                raise
    ...
    async def get_most_recent_block_schema_for_block_type(self, block_type_id: UUID) -> Optional[BlockSchema]:
        """
        ...
        """
        try:
            response = await self._client.post('/block_schemas/filter', json={'block_schemas': {'block_type_id': {'any_': [str(block_type_id)]}}, 'limit': 1})
        except httpx.HTTPStatusError:
            raise
        return BlockSchema.parse_obj(response.json()[0]) if response.json() else None

    async def read_block_document(self, block_document_id: UUID, include_secrets: bool=True):
        """
        ...
        """
        assert block_document_id is not None, 'Unexpected ID on block document. Was it persisted?'
        try:
            response = await self._client.get(f'/block_documents/{block_document_id}', params=dict(include_secrets=include_secrets))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            else:
    ...
    async def read_block_document_by_name(self, name: str, block_type_slug: str, include_secrets: bool=True):
        """
        ...
        """
        try:
            response = await self._client.get(f'/block_types/slug/{block_type_slug}/block_documents/name/{name}', params=dict(include_secrets=include_secrets))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            else:
    ...
    async def read_block_documents(self, block_schema_type: Optional[str]=None, offset: Optional[int]=None, limit: Optional[int]=None, include_secrets: bool=True):
        """
        ...
        """
        response = await self._client.post('/block_documents/filter', json=dict(block_schema_type=block_schema_type, offset=offset, limit=limit, include_secrets=include_secrets))
    ...
    async def read_block_documents_by_type(self, block_type_slug: str, offset: Optional[int]=None, limit: Optional[int]=None, include_secrets: bool=True) -> List[BlockDocument]:
        ...
        Returns:
            A list of block documents
        """
        response = await self._client.get(f'/block_types/slug/{block_type_slug}/block_documents', params=dict(offset=offset, limit=limit, include_secrets=include_secrets))
        return pydantic.parse_obj_as(List[BlockDocument], response.json())

    async def create_deployment(self, flow_id: UUID, name: str, version: str=None, schedule: SCHEDULE_TYPES=None, parameters: Dict[str, Any]=None, description: str=None, work_queue_name: str=None, work_pool_name: str=None, tags: List[str]=None, storage_document_id: UUID=None, manifest_path: str=None, path: str=None, entrypoint: str=None, infrastructure_document_id: UUID=None, infra_overrides: Dict[str, Any]=None, parameter_openapi_schema: dict=None, is_schedule_active: Optional[bool]=None, pull_steps: Optional[List[dict]]=None, enforce_parameter_schema: Optional[bool]=None) -> UUID:
        """
        Create a deployment.
        ...
        """
        deployment_create = DeploymentCreate(flow_id=flow_id, name=name, version=version, schedule=schedule, parameters=dict(parameters or {}), tags=list(tags or []), work_queue_name=work_queue_name, description=description, storage_document_id=storage_document_id, path=path, entrypoint=entrypoint, manifest_path=manifest_path, infrastructure_document_id=infrastructure_document_id, infra_overrides=infra_overrides or {}, parameter_openapi_schema=parameter_openapi_schema, is_schedule_active=is_schedule_active, pull_steps=pull_steps, enforce_parameter_schema=enforce_parameter_schema)
        if work_pool_name is not None:
            deployment_create.work_pool_name = work_pool_name
        exclude = {field for field in ['work_pool_name', 'work_queue_name'] if field not in deployment_create.__fields_set__}
        ...
        response = await self._client.post('/deployments/', json=json)
    ...
    async def update_schedule(self, deployment_id: UUID, active: bool=True):
        ...
        await self._client.post(f'/deployments/{deployment_id}/{path}')

    async def update_deployment(self, deployment: Deployment, schedule: SCHEDULE_TYPES=None, is_schedule_active: bool=None):
        deployment_update = DeploymentUpdate(version=deployment.version, schedule=schedule if schedule is not None else deployment.schedule, is_schedule_active=is_schedule_active if is_schedule_active is not None else deployment.is_schedule_active, description=deployment.description, work_queue_name=deployment.work_queue_name, tags=deployment.tags, manifest_path=deployment.manifest_path, path=deployment.path, entrypoint=deployment.entrypoint, parameters=deployment.parameters, storage_document_id=deployment.storage_document_id, infrastructure_document_id=deployment.infrastructure_document_id, infra_overrides=deployment.infra_overrides, enforce_parameter_schema=deployment.enforce_parameter_schema)
        ...
        if deployment.enforce_parameter_schema is None:
            exclude.add('enforce_parameter_schema')
        await self._client.patch(f'/deployments/{deployment.id}', json=deployment_update.dict(json_compatible=True, exclude=exclude))

    async def _create_deployment_from_schema(self, schema: DeploymentCreate) -> UUID:
        """
        Create a deployment from a prepared `DeploymentCreate` schema.
        """
        response = await self._client.post('/deployments/', json=schema.dict(json_compatible=True))
        deployment_id = response.json().get('id')
        if not deployment_id:
            raise httpx.RequestError(f'Malformed response: {response}')
        return UUID(deployment_id)
    ...
    async def read_deployments(self, *, flow_filter: FlowFilter=None, flow_run_filter: FlowRunFilter=None, task_run_filter: TaskRunFilter=None, deployment_filter: DeploymentFilter=None, work_pool_filter: WorkPoolFilter=None, work_queue_filter: WorkQueueFilter=None, limit: int=None, sort: DeploymentSort=None, offset: int=0) -> List[DeploymentResponse]:
        ...
        Returns:
            a list of Deployment model representations
                of the deployments
        """
        body = {'flows': flow_filter.dict(json_compatible=True) if flow_filter else None, 'flow_runs': flow_run_filter.dict(json_compatible=True, exclude_unset=True) if flow_run_filter else None, 'task_runs': task_run_filter.dict(json_compatible=True) if task_run_filter else None, 'deployments': deployment_filter.dict(json_compatible=True) if deployment_filter else None, 'work_pools': work_pool_filter.dict(json_compatible=True) if work_pool_filter else None, 'work_pool_queues': work_queue_filter.dict(json_compatible=True) if work_queue_filter else None, 'limit': limit, 'offset': offset, 'sort': sort}
    ...
    async def set_flow_run_state(self, flow_run_id: UUID, state: 'prefect.states.State', force: bool=False) -> OrchestrationResult:
        """
        ...
        try:
            response = await self._client.post(f'/flow_runs/{flow_run_id}/set_state', json=dict(state=state_create.dict(json_compatible=True), force=force))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            else:
    ...
    async def read_flow_run_states(self, flow_run_id: UUID) -> List[prefect.states.State]:
        """
        ...
        """
        response = await self._client.get('/flow_run_states/', params=dict(flow_run_id=str(flow_run_id)))
    ...
    async def set_task_run_name(self, task_run_id: UUID, name: str):
        ...
        return await self._client.patch(f'/task_runs/{task_run_id}', json=task_run_data.dict(json_compatible=True, exclude_unset=True))

    async def create_task_run(self, task: 'TaskObject', flow_run_id: UUID, dynamic_key: str, name: str=None, extra_tags: Iterable[str]=None, state: prefect.states.State=None, task_inputs: Dict[str, List[Union[TaskRunResult, Parameter, Constant]]]=None) -> TaskRun:
        """
        Create a task run
        ...
        if state is None:
            state = prefect.states.Pending()
        task_run_data = TaskRunCreate(name=name, flow_run_id=flow_run_id, task_key=task.task_key, dynamic_key=dynamic_key, tags=list(tags), task_version=task.version, empirical_policy=TaskRunPolicy(retries=task.retries, retry_delay=task.retry_delay_seconds, retry_jitter_factor=task.retry_jitter_factor), state=state.to_state_create(), task_inputs=task_inputs or {})
        response = await self._client.post('/task_runs/', json=task_run_data.dict(json_compatible=True))
    ...
    async def read_task_runs(self, *, flow_filter: FlowFilter=None, flow_run_filter: FlowRunFilter=None, task_run_filter: TaskRunFilter=None, deployment_filter: DeploymentFilter=None, sort: TaskRunSort=None, limit: int=None, offset: int=0) -> List[TaskRun]:
        """
    ...
    async def set_task_run_state(self, task_run_id: UUID, state: prefect.states.State, force: bool=False) -> OrchestrationResult:
        ...
        response = await self._client.post(f'/task_runs/{task_run_id}/set_state', json=dict(state=state_create.dict(json_compatible=True), force=force))
        return OrchestrationResult.parse_obj(response.json())
    ...
    async def create_logs(self, logs: Iterable[Union[LogCreate, dict]]) -> None:
        ...
        """
        serialized_logs = [log.dict(json_compatible=True) if isinstance(log, LogCreate) else log for log in logs]
        await self._client.post('/logs/', json=serialized_logs)

    async def create_flow_run_notification_policy(self, block_document_id: UUID, is_active: bool=True, tags: List[str]=None, state_names: List[str]=None, message_template: Optional[str]=None) -> UUID:
        """
        ...
        response = await self._client.post('/flow_run_notification_policies/', json=policy.dict(json_compatible=True))
        policy_id = response.json().get('id')
        if not policy_id:
            raise httpx.RequestError(f'Malformed response: {response}')
        return UUID(policy_id)

    async def read_flow_run_notification_policies(self, flow_run_notification_policy_filter: FlowRunNotificationPolicyFilter, limit: Optional[int]=None, offset: int=0) -> List[FlowRunNotificationPolicy]:
        """
        ...
        """
        body = {'flow_run_notification_policy_filter': flow_run_notification_policy_filter.dict(json_compatible=True) if flow_run_notification_policy_filter else None, 'limit': limit, 'offset': offset}
    ...
    async def read_logs(self, log_filter: LogFilter=None, limit: int=None, offset: int=None, sort: LogSort=LogSort.TIMESTAMP_ASC) -> None:
        """
        Read flow and task run logs.
        """
        body = {'logs': log_filter.dict(json_compatible=True) if log_filter else None, 'limit': limit, 'offset': offset, 'sort': sort}
    ...
    async def resolve_datadoc(self, datadoc: DataDocument) -> Any:
        ...
        if not isinstance(datadoc, DataDocument):
            raise TypeError(f'`resolve_datadoc` received invalid type {type(datadoc).__name__}')
    ...
    async def send_worker_heartbeat(self, work_pool_name: str, worker_name: str, heartbeat_interval_seconds: Optional[float]=None):
        """
        ...
        """
        await self._client.post(f'/work_pools/{work_pool_name}/workers/heartbeat', json={'name': worker_name, 'heartbeat_interval_seconds': heartbeat_interval_seconds})
    ...
    async def read_workers_for_work_pool(self, work_pool_name: str, worker_filter: Optional[WorkerFilter]=None, offset: Optional[int]=None, limit: Optional[int]=None) -> List[Worker]:
        ...
        """
        response = await self._client.post(f'/work_pools/{work_pool_name}/workers/filter', json={'worker_filter': worker_filter.dict(json_compatible=True, exclude_unset=True) if worker_filter else None, 'offset': offset, 'limit': limit})
    ...
    async def read_work_pools(self, limit: Optional[int]=None, offset: int=0, work_pool_filter: Optional[WorkPoolFilter]=None) -> List[WorkPool]:
        ...
        """
        body = {'limit': limit, 'offset': offset, 'work_pools': work_pool_filter.dict(json_compatible=True) if work_pool_filter else None}
    ...
    async def create_work_pool(self, work_pool: WorkPoolCreate) -> WorkPool:
        ...
        """
        try:
            response = await self._client.post('/work_pools/', json=work_pool.dict(json_compatible=True, exclude_unset=True))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_409_CONFLICT:
                raise prefect.exceptions.ObjectAlreadyExists(http_exc=e) from e
            else:
    ...
    async def update_work_pool(self, work_pool_name: str, work_pool: WorkPoolUpdate):
        ...
        """
        try:
            await self._client.patch(f'/work_pools/{work_pool_name}', json=work_pool.dict(json_compatible=True, exclude_unset=True))
        except httpx.HTTPStatusError as e:
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise prefect.exceptions.ObjectNotFound(http_exc=e) from e
            else:
    ...
    async def read_work_queues(self, work_pool_name: Optional[str]=None, work_queue_filter: Optional[WorkQueueFilter]=None, limit: Optional[int]=None, offset: Optional[int]=None) -> List[WorkQueue]:
        ...
        """
        json = {'work_queues': work_queue_filter.dict(json_compatible=True, exclude_unset=True) if work_queue_filter else None, 'limit': limit, 'offset': offset}
        if work_pool_name:
            try:
    ...
    async def get_scheduled_flow_runs_for_deployments(self, deployment_ids: List[UUID], scheduled_before: Optional[datetime.datetime]=None, limit: Optional[int]=None):
        body: Dict[str, Any] = dict(deployment_ids=[str(id) for id in deployment_ids])
        if scheduled_before:
            body['scheduled_before'] = str(scheduled_before)
        if limit:
            body['limit'] = limit
        response = await self._client.post('/deployments/get_scheduled_flow_runs', json=body)
        return pydantic.parse_obj_as(List[FlowRunResponse], response.json())

    async def get_scheduled_flow_runs_for_work_pool(self, work_pool_name: str, work_queue_names: Optional[List[str]]=None, scheduled_before: Optional[datetime.datetime]=None) -> List[WorkerFlowRunResponse]:
        """
        ...
        if scheduled_before:
            body['scheduled_before'] = str(scheduled_before)
        response = await self._client.post(f'/work_pools/{work_pool_name}/get_scheduled_flow_runs', json=body)
    ...
    async def create_artifact(self, artifact: ArtifactCreate) -> Artifact:
        ...
        """
        response = await self._client.post('/artifacts/', json=artifact.dict(json_compatible=True, exclude_unset=True))
        return pydantic.parse_obj_as(Artifact, response.json())

    async def read_artifacts(self, *, artifact_filter: ArtifactFilter=None, flow_run_filter: FlowRunFilter=None, task_run_filter: TaskRunFilter=None, sort: ArtifactSort=None, limit: int=None, offset: int=0) -> List[Artifact]:
        """
        ...
        """
        body = {'artifacts': artifact_filter.dict(json_compatible=True) if artifact_filter else None, 'flow_runs': flow_run_filter.dict(json_compatible=True) if flow_run_filter else None, 'task_runs': task_run_filter.dict(json_compatible=True) if task_run_filter else None, 'sort': sort, 'limit': limit, 'offset': offset}
        response = await self._client.post('/artifacts/filter', json=body)
        return pydantic.parse_obj_as(List[Artifact], response.json())

    async def read_latest_artifacts(self, *, artifact_filter: ArtifactCollectionFilter=None, flow_run_filter: FlowRunFilter=None, task_run_filter: TaskRunFilter=None, sort: ArtifactCollectionSort=None, limit: int=None, offset: int=0) -> List[ArtifactCollection]:
        """
        Query the Prefect API for artifacts. Only artifacts matching all criteria will
        be returned.
        Args:
    ...
    async def create_automation(self, automation: Automation) -> UUID:
        ...
        response = await self._client.post('/automations/', json=automation.dict(json_compatible=True))
        return UUID(response.json()['id'])

    async def read_resource_related_automations(self, resource_id: str) -> List[ExistingAutomation]:
        if self.server_type != ServerType.CLOUD:
            raise RuntimeError('Automations are only supported for Prefect Cloud.')
    ...
    async def increment_concurrency_slots(self, names: List[str], slots: int, mode: str) -> httpx.Response:
        return await self._client.post('/v2/concurrency_limits/increment', json={'names': names, 'slots': slots, 'mode': mode})

    async def release_concurrency_slots(self, names: List[str], slots: int, occupancy_seconds: float) -> httpx.Response:
        return await self._client.post('/v2/concurrency_limits/decrement', json={'names': names, 'slots': slots, 'occupancy_seconds': occupancy_seconds})
