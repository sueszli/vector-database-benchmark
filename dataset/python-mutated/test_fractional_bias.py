...
def test_zero_sample():
    if False:
        while True:
            i = 10
    m = FractionalBias()
    with pytest.raises(NotComputableError, match='FractionalBias must have at least one example before it can be computed'):
        m.compute()
...
def test_wrong_input_shapes():
    if False:
        ...
        return i + 15
    m = FractionalBias()
    with pytest.raises(ValueError, match='Input data shapes should be the same, but given'):
...
def test_integration():
    ...
    def get_test_cases():
        if False:
            while True:
                i = 10
        test_cases = [(torch.rand(size=(100,)), torch.rand(size=(100,)), 10), (torch.rand(size=(100, 1)), torch.rand(size=(100, 1)), 20)]
        return test_cases
    for _ in range(5):
...
def _test_distrib_compute(device, tol=1e-05):
    if False:
        for i in range(10):
            print('nop')
    rank = idist.get_rank()
...
def _test_distrib_integration(device, tol=1e-05):
    if False:
        for i in range(10):
            print('nop')
    rank = idist.get_rank()
...
def test_distrib_single_device_xla():
    if False:
        while True:
            i = 10
    device = idist.device()
    _test_distrib_compute(device, tol=0.0001)
