...
if __name__ == '__main__':
    ...
    def _input(ioctx):
        ...
        if ioctx.worker_index > 0 or ioctx.worker.num_workers == 0:
            ...
            return None
    config = get_trainable_cls(args.run).get_default_config().environment(env=None, observation_space=gym.spaces.Box(float('-inf'), float('inf'), (4,)), action_space=gym.spaces.Discrete(2)).framework(args.framework).callbacks(MyCallbacks if args.callbacks_verbose else None).offline_data(input_=_input).rollouts(num_rollout_workers=args.num_workers, enable_connectors=False).evaluation(off_policy_estimation_methods={}).debugging(log_level='INFO')
    config.experimental(_enable_new_api_stack=False)
    if args.run == 'DQN' or args.run == 'APEX' or args.run == 'R2D2':
        config.update_from_dict({'num_steps_sampled_before_learning_starts': 100, 'min_sample_timesteps_per_iteration': 200, 'n_step': 3, 'rollout_fragment_length': 4, 'train_batch_size': 8})
        config.model.update({'fcnet_hiddens': [64], 'fcnet_activation': 'linear'})
        if args.run == 'R2D2':
            config.model['use_lstm'] = args.use_lstm
    elif args.run == 'IMPALA':
        config.update_from_dict({'num_gpus': 0, 'model': {'use_lstm': args.use_lstm}})
    else:
        config.update_from_dict({'rollout_fragment_length': 1000, 'train_batch_size': 4000, 'model': {'use_lstm': args.use_lstm}})
    ...
    if args.no_tune:
        ...
        tune.Tuner(args.run, param_space=config, run_config=air.RunConfig(stop=stop, verbose=2)).fit()