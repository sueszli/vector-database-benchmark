...
def single_evaluate(eval_ob: dict) -> list[bool]:
    ...
    process = subprocess.Popen(['python', '-u', '-m', 'gpt_engineer.cli.main', eval_ob['project_root'], '--steps', 'eval_new_code', '--temperature', '0'], stdout=log_file, stderr=log_file, bufsize=0)
...
@app.command()
def main(test_file_path: str=typer.Argument('evals/new_code_eval.yaml', help='path')):
    if False:
        i = 10
        return i + 15
