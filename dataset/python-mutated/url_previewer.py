...
class UrlPreviewer:
    ...
    async def _do_preview(self, url: str, user: UserID, ts: int) -> bytes:
        ...
        if cache_result and cache_result.expires_ts > ts and (cache_result.response_code // 100 == 2):
        ...
        if _is_media(media_info.media_type):
            ...
            dims = await self.media_repo._generate_thumbnails(None, file_id, file_id, media_info.media_type, url_cache=True)
            og = {'og:description': media_info.download_name, 'og:image': f'mxc://{self.server_name}/{media_info.filesystem_id}', 'og:image:type': media_info.media_type, 'matrix:image:size': media_info.media_length}
            if dims:
            ...
            if tree is not None:
                ...
                if oembed_url:
                    try:
                        oembed_info = await self._handle_url(oembed_url, user, allow_data_urls=True)
                    except Exception as e:
                        logger.warning('oEmbed fetch failed during URL preview: %s errored with %s', oembed_url, e)
                    else:
                        (og_from_oembed, author_name, expiration_ms) = await self._handle_oembed_response(url, oembed_info, expiration_ms)
            ...
            (og, author_name, expiration_ms) = await self._handle_oembed_response(url, media_info, expiration_ms)
            await self._precache_image_url(user, media_info, og)
        else:
        ...
        for (k, v) in og.items():
            if len(k) > OG_TAG_NAME_MAXLEN or len(str(v)) > OG_TAG_VALUE_MAXLEN:
                logger.warning('Pruning overlong tag %s from OG data', k[:OG_TAG_NAME_MAXLEN])
        ...
        await self.store.store_url_cache(url, media_info.response_code, media_info.etag, media_info.created_ts_ms + expiration_ms, jsonog, media_info.filesystem_id, media_info.created_ts_ms)
    ...
    async def _download_url(self, url: str, output_stream: BinaryIO) -> DownloadResult:
        ...
        try:
            ...
            (length, headers, uri, code) = await self.client.get_file(url, output_stream=output_stream, max_size=self.max_spider_size, headers={b'Accept-Language': self.url_preview_accept_language, b'User-Agent': ['Synapse (bot; +https://github.com/matrix-org/synapse)']}, is_allowed_content_type=_is_previewable)
        except SynapseError:
            raise
        except DNSLookupError:
            raise SynapseError(502, 'DNS resolution failure during URL preview generation', Codes.UNKNOWN)
        except Exception as e:
            logger.warning('Error downloading %s: %r', url, e)
            raise SynapseError(500, 'Failed to download content: %s' % (traceback.format_exception_only(sys.exc_info()[0], e),), Codes.UNKNOWN)
        ...
        return DownloadResult(length, uri, code, media_type, download_name, expires, etag)

    async def _parse_data_url(self, url: str, output_stream: BinaryIO) -> DownloadResult:
        """
        Parses a data: URL.
        ...
        try:
            ...
            raise SynapseError(500, 'Failed to parse data URL: %s' % (traceback.format_exception_only(sys.exc_info()[0], e),), Codes.UNKNOWN)
        return DownloadResult(length=output_stream.tell(), uri=url, response_code=200, media_type=url_info.headers.get_content_type(), download_name=None, expires=ONE_HOUR, etag=None)

    async def _handle_url(self, url: str, user: UserID, allow_data_urls: bool=False) -> MediaInfo:
        """
        ...
        if self._is_url_blocked(url):
            raise SynapseError(403, 'URL blocked by url pattern blocklist entry', Codes.UNKNOWN)
        file_id = datetime.date.today().isoformat() + '_' + random_string(16)
        file_info = FileInfo(server_name=None, file_id=file_id, url_cache=True)
        with self.media_storage.store_into_file(file_info) as (f, fname, finish):
            if url.startswith('data:'):
                if not allow_data_urls:
                    raise SynapseError(500, 'Previewing of data: URLs is forbidden', Codes.UNKNOWN)
        ...
        try:
            ...
            await self.store.store_local_media(media_id=file_id, media_type=download_result.media_type, time_now_ms=time_now_ms, upload_name=download_result.download_name, media_length=download_result.length, user_id=user, url_cache=url)
        except Exception as e:
            logger.error('Error handling downloaded %s: %r', url, e)
            raise
        return MediaInfo(media_type=download_result.media_type, media_length=download_result.length, download_name=download_result.download_name, created_ts_ms=time_now_ms, filesystem_id=file_id, filename=fname, uri=download_result.uri, response_code=download_result.response_code, expires=download_result.expires, etag=download_result.etag)

    async def _precache_image_url(self, user: UserID, media_info: MediaInfo, og: JsonDict) -> None:
        """
        ...
        try:
            ...
            logger.warning('Pre-caching image failed during URL preview: %s errored with %s', image_url, e)
            return
        if _is_media(image_info.media_type):
            file_id = image_info.filesystem_id
            dims = await self.media_repo._generate_thumbnails(None, file_id, file_id, image_info.media_type, url_cache=True)
            if dims:
                og['og:image:width'] = dims['width']
                og['og:image:height'] = dims['height']
            else:
    ...
    async def _handle_oembed_response(self, url: str, media_info: MediaInfo, expiration_ms: int) -> Tuple[JsonDict, Optional[str], int]:
        """
    ...
    async def _expire_url_cache_data(self) -> None:
        ...
        for media_id in media_ids:
            ...
            try:
                ...
                logger.warning('Failed to remove media while clearing url preview cache: %r: %s', media_id, e)
                continue
        ...
        if removed_media:
            logger.debug('Deleted %d entries from url preview cache', len(removed_media))
        else:
        ...
        for media_id in media_ids:
            ...
            try:
                ...
                logger.warning('Failed to remove media from url preview cache: %r: %s', media_id, e)
                continue
            dirs = self.filepaths.url_cache_filepath_dirs_to_delete(media_id)
            try_remove_parent_dirs(dirs)
