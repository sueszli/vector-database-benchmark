...
from integration_tests.samples.token_rotation.util import parse_body, extract_enterprise_id, extract_user_id, extract_team_id, extract_is_enterprise_install, extract_content_type
import logging
import os
...
installation_store = SQLite3InstallationStore(database='test.db', client_id=client_id, logger=logger)
installation_store.init()
token_rotator = TokenRotator(client_id=client_id, client_secret=client_secret)
state_store = SQLite3OAuthStateStore(database='test.db', logger=logger, expiration_seconds=300)
state_store.init()
import json
...
def rotate_tokens(enterprise_id: Optional[str]=None, team_id: Optional[str]=None, user_id: Optional[str]=None, is_enterprise_install: Optional[bool]=None):
    if False:
        return 10
    installation = installation_store.find_installation(enterprise_id=enterprise_id, team_id=team_id, user_id=user_id, is_enterprise_install=is_enterprise_install)
    if installation is not None:
        updated_installation = token_rotator.perform_token_rotation(installation=installation, minutes_before_expiration=60 * 24 * 365)
...
def slack_app():
    ...
    rotate_tokens(enterprise_id=extract_enterprise_id(body), team_id=extract_team_id(body), user_id=extract_user_id(body), is_enterprise_install=extract_is_enterprise_install(body))
...
authorization_url_generator = AuthorizeUrlGenerator(client_id=client_id, scopes=scopes, user_scopes=user_scopes)
redirect_page_renderer = RedirectUriPageRenderer(install_path='/slack/install', redirect_uri_path='/slack/oauth_redirect')
...
def oauth_start():
    ...
    return f'<html><head><link rel="icon" href="data:,"></head><body><a href="{url}"><img alt=""Add to Slack"" height="40" width="139" src="https://platform.slack-edge.com/img/add_to_slack.png" srcset="https://platform.slack-edge.com/img/add_to_slack.png 1x, https://platform.slack-edge.com/img/add_to_slack@2x.png 2x" /></a></body></html>'
