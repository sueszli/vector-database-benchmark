...
async def get_container_from_tarball_path(dagger_client: dagger.Client, tarball_path: Path):
    ...
    container_under_test_tar_file = dagger_client.host().directory(str(tarball_path.parent), include=tarball_path.name).file(tarball_path.name)
    try:
...
class ConnectorRunner:
    ...
    async def call_read(self, config: SecretDict, catalog: ConfiguredAirbyteCatalog, raise_container_error: bool=False, enable_caching: bool=True) -> List[AirbyteMessage]:
        return await self._run(['read', '--config', self.IN_CONTAINER_CONFIG_PATH, '--catalog', self.IN_CONTAINER_CATALOG_PATH], raise_container_error, config=config, catalog=catalog, enable_caching=enable_caching)

    async def call_read_with_state(self, config: SecretDict, catalog: ConfiguredAirbyteCatalog, state: dict, raise_container_error: bool=False, enable_caching: bool=True) -> List[AirbyteMessage]:
        return await self._run(['read', '--config', self.IN_CONTAINER_CONFIG_PATH, '--catalog', self.IN_CONTAINER_CATALOG_PATH, '--state', self.IN_CONTAINER_STATE_PATH], raise_container_error, config=config, catalog=catalog, state=state, enable_caching=enable_caching)
    ...
    async def _run(self, airbyte_command: List[str], raise_container_error: bool, config: SecretDict=None, catalog: dict=None, state: Union[dict, list]=None, enable_caching=True) -> List[AirbyteMessage]:
        ...
        try:
            ...
            if output_too_big:
                ...
                raise e
            elif isinstance(e, dagger.ExecError):
    ...
    async def _read_output_from_file(self, airbyte_command: list, container: dagger.Container) -> str:
        ...
        container = container.with_exec(['sh', '-c', ' '.join(airbyte_command) + f' > {self.IN_CONTAINER_OUTPUT_PATH} 2>&1 | tee -a {self.IN_CONTAINER_OUTPUT_PATH}'], skip_entrypoint=True)
