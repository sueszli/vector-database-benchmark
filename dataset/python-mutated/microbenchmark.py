...
async def trial(intermediate_handles: bool, num_replicas: int, max_batch_size: int, max_concurrent_queries: int, data_size: str) -> Dict[str, float]:
    results = {}
    trial_key_base = f'replica:{num_replicas}/batch_size:{max_batch_size}/concurrent_queries:{max_concurrent_queries}/data_size:{data_size}/intermediate_handle:{intermediate_handles}'
    print(f'intermediate_handles={intermediate_handles},num_replicas={num_replicas},max_batch_size={max_batch_size},max_concurrent_queries={max_concurrent_queries},data_size={data_size}')
    app = build_app(intermediate_handles, num_replicas, max_batch_size, max_concurrent_queries)
    serve.run(app)
    if data_size == 'small':
        data = None
    ...
    async with aiohttp.ClientSession() as session:
        ...
        (single_client_avg_tps, single_client_std_tps) = await run_throughput_benchmark(single_client, multiplier=CALLS_PER_BATCH)
        print('\t{} {} +- {} requests/s'.format('single client {} data'.format(data_size), single_client_avg_tps, single_client_std_tps))
    ...
    (multi_client_avg_tps, _) = await run_throughput_benchmark(many_clients, multiplier=CALLS_PER_BATCH * len(clients))
...
async def main():
    ...
    for intermediate_handles in [False, True]:
        for num_replicas in [1, 8]:
            for (max_batch_size, max_concurrent_queries) in [(1, 1), (1, 10000), (10000, 10000)]:
                for data_size in ['small']:
                    results.update(await trial(intermediate_handles, num_replicas, max_batch_size, max_concurrent_queries, data_size))
