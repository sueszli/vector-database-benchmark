...
parser.add_argument('--stop-timesteps', type=int, default=20000, help='Number of timesteps to train.')
parser.add_argument('--stop-reward', type=float, default=0.7, help='Reward at which we stop training.')
...
if __name__ == '__main__':
    ...
    config = PPOConfig().environment(SimpleCorridor, env_config={'corridor_length': 10}).rollouts(num_rollout_workers=0).evaluation(evaluation_num_workers=2, evaluation_interval=1, evaluation_duration='auto' if args.evaluation_parallel_to_training else 10, evaluation_parallel_to_training=args.evaluation_parallel_to_training, evaluation_config=PPOConfig.overrides(env_config={'corridor_length': 5}), custom_evaluation_function=eval_fn).framework(args.framework).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
