...
START_CODE_BLOCK_RE = re.compile('^((```[\\w.+\\-]+\\n+(?!```))|(```\\n*))')
T = TypeVar('T')
...
class DevOutput:
    ...
    @classmethod
    async def from_debug(cls, ctx: commands.Context, *, source: str, source_cache: SourceCache, env: Dict[str, Any]) -> DevOutput:
        output = cls(ctx, source=source, source_cache=source_cache, filename=f'<debug command - snippet #{source_cache.take_next_index()}>', env=env)
        await output.run_debug()
        return output

    @classmethod
    async def from_eval(cls, ctx: commands.Context, *, source: str, source_cache: SourceCache, env: Dict[str, Any]) -> DevOutput:
        output = cls(ctx, source=source, source_cache=source_cache, filename=f'<eval command - snippet #{source_cache.take_next_index()}>', env=env)
        await output.run_eval()
        return output

    @classmethod
    async def from_repl(cls, ctx: commands.Context, *, source: str, source_cache: SourceCache, env: Dict[str, Any]) -> DevOutput:
        output = cls(ctx, source=source, source_cache=source_cache, filename=f'<repl session - snippet #{source_cache.take_next_index()}>', env=env)
...
class Dev(commands.Cog):
    ...
    async def debug(self, ctx: commands.Context, *, code: str) -> None:
        ...
        output = await DevOutput.from_debug(ctx, source=source, source_cache=self.source_cache, env=env)
    ...
    async def _eval(self, ctx: commands.Context, *, body: str) -> None:
        ...
        output = await DevOutput.from_eval(ctx, source=source, source_cache=self.source_cache, env=env)
    ...
    async def repl(self, ctx: commands.Context) -> None:
        ...
        if ctx.channel.id in self.sessions:
            if self.sessions[ctx.channel.id]:
                ...
                await ctx.send(_('Already running a REPL session in this channel. Resume the REPL with `{}repl resume`.').format(ctx.prefix))
            return
        env = self.get_environment(ctx)
        env['__builtins__'] = __builtins__
        env['_'] = None
        self.sessions[ctx.channel.id] = True
        await ctx.send(_('Enter code to execute or evaluate. `exit()` or `quit` to exit. `{}repl pause` to pause.').format(ctx.prefix))
        while True:
            response = await ctx.bot.wait_for('message', check=MessagePredicate.regex('^`', ctx))
            ...
            if source in ('quit', 'exit', 'exit()'):
                ...
                return
            output = await DevOutput.from_repl(ctx, source=source, source_cache=self.source_cache, env=env)
            try:
