...
from ...core.types import BaseInputObjectType, BaseObjectType, NonNullList, StockBulkUpdateError
...
class StockBulkResult(BaseObjectType):
    ...
    errors = NonNullList(StockBulkUpdateError, required=False, description='List of errors occurred on create or update attempt.')
...
class StockBulkUpdateInput(BaseInputObjectType):
    ...
    variant_external_reference = graphene.String(required=False, description='Variant external reference.')
    warehouse_id = graphene.ID(required=False, description='Warehouse ID.')
    warehouse_external_reference = graphene.String(required=False, description='Warehouse external reference.')
...
class StockBulkUpdate(BaseMutation):
    ...
    results = NonNullList(StockBulkResult, required=True, default_value=[], description='List of the updated stocks.')

    class Arguments:
        stocks = NonNullList(StockBulkUpdateInput, required=True, description='Input list of stocks to update.')
    ...
    class Meta:
        description = 'Updates stocks for a given variant and warehouse.' + ADDED_IN_313 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PRODUCTS
        permissions = (ProductPermissions.MANAGE_PRODUCTS,)
        error_type_class = StockBulkUpdateError
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.PRODUCT_VARIANT_STOCK_UPDATED, description='A product variant stock details were updated.')]
