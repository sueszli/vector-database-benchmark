...
class WeChatWxa(BaseWeChatAPI):
    ...
    def get_wxa_code(self, path, width=430, auto_color=False, line_color=None, is_hyaline=False):
        if False:
            i = 10
            return i + 15
        ...
        return self._post('wxa/getwxacode', data={'path': path, 'width': width, 'auto_color': auto_color, 'line_color': line_color, 'is_hyaline': is_hyaline})

    def get_wxa_code_unlimited(self, scene, width=430, auto_color=False, line_color=None, page=None, is_hyaline=False):
        if False:
            return 10
        '\n        创建小程序码（接口B：适用于需要的码数量极多，或仅临时使用的业务场景）\n        详情请参考\n        https://developers.weixin.qq.com/miniprogram/dev/api-backend/open-api/qr-code/wxacode.getUnlimited.html\n        '
        if line_color is None:
            line_color = {'r': '0', 'g': '0', 'b': '0'}
        return self._post('wxa/getwxacodeunlimit', data=optionaldict(scene=scene, page=page, width=width, auto_color=auto_color, line_color=line_color, is_hyaline=is_hyaline))
    ...
    def send_template_message(self, user_id, template_id, data, form_id, page=None, color=None, emphasis_keyword=None):
        ...
        tpl_data = optionaldict(touser=user_id, template_id=template_id, page=page, form_id=form_id, data=data, color=color, emphasis_keyword=emphasis_keyword)
    ...
    def modify_domain(self, action, request_domain=(), wsrequest_domain=(), upload_domain=(), download_domain=()):
        ...
        return self._post('wxa/modify_domain', data={'action': action, 'requestdomain': request_domain, 'wsrequestdomain': wsrequest_domain, 'uploaddomain': upload_domain, 'downloaddomain': download_domain})
    ...
    def commit(self, template_id, ext_json, version, description):
        ...
        return self._post('wxa/commit', data={'template_id': template_id, 'ext_json': ext_json, 'user_version': version, 'user_desc': description})
    ...
    def get_url_scheme(self, is_expire=False, expire_type=0, expire_time=None, expire_interval=None, jump_wxa=()):
        ...
        return self._post('wxa/generatescheme', data=optionaldict(jump_wxa=jump_wxa, is_expire=is_expire, expire_type=expire_type, expire_time=expire_time, expire_interval=expire_interval))

    def get_url_link(self, path=None, query=None, is_expire=False, expire_type=0, expire_time=None, expire_interval=None, cloud_base=()):
        if False:
            for i in range(10):
                print('nop')
        ...
        return self._post('wxa/generate_urllink', data=optionaldict(path=path, query=query, is_expire=is_expire, expire_type=expire_type, expire_time=expire_time, expire_interval=expire_interval, cloud_base=cloud_base))
    ...
    def add_template(self, template_short_id, keyword_id_list):
        ...
        return self._post('cgi-bin/wxopen/template/add', data={'id': template_short_id, 'keyword_id_list': keyword_id_list}, result_processor=lambda x: x['template_id'])
    ...
    def get_phone_number(self, code):
        ...
        return self._post('wxa/business/getuserphonenumber', data={'code': code})
    ...
    def check_text_security(self, content, open_id=None, scene=None, nickname=None, title=None, signature=None, version=2):
        ...
        if open_id is None:
            ...
            data = {'content': content, 'openid': open_id, 'scene': scene, 'nickname': nickname, 'title': title, 'signature': signature, 'version': version}
    ...
    def get_paid_unionid(self, openid, transaction_id=None, mch_id=None, out_trade_no=None):
        ...
        return self._get('wxa/getpaidunionid', params={'openid': openid, 'transaction_id': transaction_id, 'mch_id': mch_id, 'out_trade_no': out_trade_no}, result_processor=lambda x: x['unionid'])
    ...
    def ocr_idcard(self, mode: str='photo', img_url: str=None, filename: str=None, file_bytes: bytes=None, mime_type: str=None) -> dict:
        if False:
            print('Hello World!')
    ...
    def ocr_biz_license(self, img_url: str=None, filename: str=None, file_bytes: bytes=None, mime_type: str=None) -> dict:
        if False:
            return 10
