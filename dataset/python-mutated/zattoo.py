...
@pluginargument('email', requires=['password'], metavar='EMAIL', help='The email associated with your zattoo account, required to access any zattoo stream.')
@pluginargument('password', sensitive=True, metavar='PASSWORD', help='A zattoo account password to use with --zattoo-email.')
@pluginargument('purge-credentials', action='store_true', help='Purge cached zattoo credentials to initiate a new session and reauthenticate.')
@pluginargument('stream-types', metavar='TYPES', type=comma_list_filter(STREAMS_ZATTOO), default=['dash'], help=f"""\n        A comma-delimited list of stream types which should be used.\n\n        The following types are allowed: {','.join(STREAMS_ZATTOO)}\n\n        Default is "dash".\n    """)
class Zattoo(Plugin):
