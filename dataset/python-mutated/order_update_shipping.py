...
from .utils import SHIPPING_METHOD_UPDATE_FIELDS, EditableOrderValidationMixin, ShippingMethodUpdateMixin, clean_order_update_shipping

class OrderUpdateShippingInput(BaseInputObjectType):
    shipping_method = graphene.ID(description='ID of the selected shipping method, pass null to remove currently assigned shipping method.', name='shippingMethod')

    class Meta:
        doc_category = DOC_CATEGORY_ORDERS

class OrderUpdateShipping(EditableOrderValidationMixin, ShippingMethodUpdateMixin, BaseMutation):
    order = graphene.Field(Order, description='Order with updated shipping method.')

    class Arguments:
        id = graphene.ID(required=True, name='order', description='ID of the order to update a shipping method.')
        input = OrderUpdateShippingInput(description='Fields required to change shipping method of the order.', required=True)
