...
if __name__ == '__main__':
    ...
    q_optimizer = optim.Adam(list(qf1.parameters()) + list(qf2.parameters()), lr=args.q_lr, eps=0.0001)
    actor_optimizer = optim.Adam(list(actor.parameters()), lr=args.policy_lr, eps=0.0001)
    if args.autotune:
        target_entropy = -args.target_entropy_scale * torch.log(1 / torch.tensor(envs.single_action_space.n))
        log_alpha = torch.zeros(1, requires_grad=True, device=device)
        alpha = log_alpha.exp().item()
        a_optimizer = optim.Adam([log_alpha], lr=args.q_lr, eps=0.0001)
    else:
        alpha = args.alpha
