...
from sklearn.metrics import accuracy_score, f1_score, hamming_loss, jaccard_similarity_score
from sklearn.utils._testing import ignore_warnings
METRICS = {'f1': partial(f1_score, average='micro'), 'f1-by-sample': partial(f1_score, average='samples'), 'accuracy': accuracy_score, 'hamming': hamming_loss, 'jaccard': jaccard_similarity_score}
FORMATS = {'sequences': lambda y: [list(np.flatnonzero(s)) for s in y], 'dense': lambda y: y, 'csr': sp.csr_matrix, 'csc': sp.csc_matrix}
...
if __name__ == '__main__':
    ...
    ap.add_argument('metrics', nargs='*', default=sorted(METRICS), help='Specifies metrics to benchmark, defaults to all. Choices are: {}'.format(sorted(METRICS)))
    ap.add_argument('--formats', nargs='+', choices=sorted(FORMATS), help='Specifies multilabel formats to benchmark (defaults to all).')
    ...
    ap.add_argument('--density', type=float, default=0.2, help='The average density of labels per sample')
    ap.add_argument('--plot', choices=['classes', 'density', 'samples'], default=None, help='Plot time with respect to this parameter varying up to the specified value')
    ...
    results = benchmark([METRICS[k] for k in args.metrics], [FORMATS[k] for k in args.formats], args.samples, args.classes, args.density, args.n_times)
    _tabulate(results, args.metrics, args.formats)
    if args.plot is not None:
        print('Displaying plot', file=sys.stderr)
        title = 'Multilabel metrics with %s' % ', '.join(('{0}={1}'.format(field, getattr(args, field)) for field in ['samples', 'classes', 'density'] if args.plot != field))
