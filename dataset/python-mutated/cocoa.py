    ...
    class WebKitHost(WebKit.WKWebView):
        ...
        def mouseDragged_(self, event):
            ...
            if i.frameless and i.easy_drag:
                ...
                newOrigin = AppKit.NSMakePoint(currentLocation.x - self.initialLocation.x, currentLocation.y - self.initialLocation.y)
        ...
        def handler(cookies):
            ...
            for c in cookies:
                ...
                if domain not in self.url:
                    continue
                data = {'name': c.name(), 'value': c.value(), 'path': c.path(), 'domain': c.domain(), 'expires': c.expiresDate(), 'secure': c.isSecure(), 'httponly': c.isHTTPOnly(), 'samesite': c.SameSitePolicy()}
...
def get_screens():
    if False:
        print('Hello World!')
    screens = [Screen(s.frame().size.width, s.frame().size.height, s.frame()) for s in AppKit.NSScreen.screens()]
    return screens
