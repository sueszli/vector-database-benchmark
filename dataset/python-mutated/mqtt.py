...
CONTROL_PACKET_TYPE = {1: 'CONNECT', 2: 'CONNACK', 3: 'PUBLISH', 4: 'PUBACK', 5: 'PUBREC', 6: 'PUBREL', 7: 'PUBCOMP', 8: 'SUBSCRIBE', 9: 'SUBACK', 10: 'UNSUBSCRIBE', 11: 'UNSUBACK', 12: 'PINGREQ', 13: 'PINGRESP', 14: 'DISCONNECT', 15: 'AUTH'}
QOS_LEVEL = {0: 'At most once delivery', 1: 'At least once delivery', 2: 'Exactly once delivery'}

class MQTT(Packet):
    name = 'MQTT fixed header'
    fields_desc = [BitEnumField('type', 1, 4, CONTROL_PACKET_TYPE), BitEnumField('DUP', 0, 1, {0: 'Disabled', 1: 'Enabled'}), BitEnumField('QOS', 0, 2, QOS_LEVEL), BitEnumField('RETAIN', 0, 1, {0: 'Disabled', 1: 'Enabled'}), VariableFieldLenField('len', None, length_of='len', adjust=lambda pkt, x: len(pkt.payload))]
PROTOCOL_LEVEL = {3: 'v3.1', 4: 'v3.1.1', 5: 'v5.0'}

class MQTTConnect(Packet):
    name = 'MQTT connect'
    fields_desc = [FieldLenField('length', None, length_of='protoname'), StrLenField('protoname', '', length_from=lambda pkt: pkt.length), ByteEnumField('protolevel', 5, PROTOCOL_LEVEL), BitEnumField('usernameflag', 0, 1, {0: 'Disabled', 1: 'Enabled'}), BitEnumField('passwordflag', 0, 1, {0: 'Disabled', 1: 'Enabled'}), BitEnumField('willretainflag', 0, 1, {0: 'Disabled', 1: 'Enabled'}), BitEnumField('willQOSflag', 0, 2, QOS_LEVEL), BitEnumField('willflag', 0, 1, {0: 'Disabled', 1: 'Enabled'}), BitEnumField('cleansess', 0, 1, {0: 'Disabled', 1: 'Enabled'}), BitEnumField('reserved', 0, 1, {0: 'Disabled', 1: 'Enabled'}), ShortField('klive', 0), FieldLenField('clientIdlen', None, length_of='clientId'), StrLenField('clientId', '', length_from=lambda pkt: pkt.clientIdlen), ConditionalField(FieldLenField('wtoplen', None, length_of='willtopic'), lambda pkt: pkt.willflag == 1), ConditionalField(StrLenField('willtopic', '', length_from=lambda pkt: pkt.wtoplen), lambda pkt: pkt.willflag == 1), ConditionalField(FieldLenField('wmsglen', None, length_of='willmsg'), lambda pkt: pkt.willflag == 1), ConditionalField(StrLenField('willmsg', '', length_from=lambda pkt: pkt.wmsglen), lambda pkt: pkt.willflag == 1), ConditionalField(FieldLenField('userlen', None, length_of='username'), lambda pkt: pkt.usernameflag == 1), ConditionalField(StrLenField('username', '', length_from=lambda pkt: pkt.userlen), lambda pkt: pkt.usernameflag == 1), ConditionalField(FieldLenField('passlen', None, length_of='password'), lambda pkt: pkt.passwordflag == 1), ConditionalField(StrLenField('password', '', length_from=lambda pkt: pkt.passlen), lambda pkt: pkt.passwordflag == 1)]
...
class MQTTConnack(Packet):
    name = 'MQTT connack'
    fields_desc = [ByteField('sessPresentFlag', 0), ByteEnumField('retcode', 0, RETURN_CODE)]

class MQTTPublish(Packet):
    name = 'MQTT publish'
    fields_desc = [FieldLenField('length', None, length_of='topic'), StrLenField('topic', '', length_from=lambda pkt: pkt.length), ConditionalField(ShortField('msgid', None), lambda pkt: pkt.underlayer.QOS == 1 or pkt.underlayer.QOS == 2), StrLenField('value', '', length_from=lambda pkt: pkt.underlayer.len - pkt.length - 2 if pkt.underlayer.QOS == 0 else pkt.underlayer.len - pkt.length - 4)]
