...
parser.add_argument('-a', '--alphabet', metavar='charset', default=string.ascii_lowercase, type=str.encode, help='The alphabet to use in the cyclic pattern')
parser.add_argument('-n', '--length', metavar='length', type=int, help='Size of the unique subsequences (defaults to the pointer size for the current arch)')
group = parser.add_mutually_exclusive_group(required=False)
group.add_argument('-l', '-o', '--offset', '--lookup', dest='lookup', metavar='lookup_value', type=str, help='Do a lookup instead of printing the sequence (accepts constant values as well as expressions)')
group.add_argument('count', type=int, nargs='?', default=100, help='Number of characters to print from the sequence (default: print the entire sequence)')
