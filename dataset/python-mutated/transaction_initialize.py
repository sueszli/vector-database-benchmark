...
class TransactionInitialize(TransactionSessionBase):
    transaction = graphene.Field(TransactionItem, description='The initialized transaction.')
    transaction_event = graphene.Field(TransactionEvent, description='The event created for the initialized transaction.')
    data = graphene.Field(JSON, description='The JSON data required to finalize the payment.')

    class Arguments:
        id = graphene.ID(description='The ID of the checkout or order.', required=True)
        amount = graphene.Argument(PositiveDecimal, description='The amount requested for initializing the payment gateway. If not provided, the difference between checkout.total - transactions that are already processed will be send.')
        action = graphene.Argument(TransactionFlowStrategyEnum, description='The expected action called for the transaction. By default, the `channel.paymentSettings.defaultTransactionFlowStrategy` will be used.The field can be used only by app that has `HANDLE_PAYMENTS` permission.')
        customer_ip_address = graphene.String(description="The customer's IP address. If not provided Saleor will try to determine the customer's IP address on its own. The customer's IP address will be passed to the payment app. The IP should be in ipv4 or ipv6 format. The field can be used only by an app that has `HANDLE_PAYMENTS` permission." + ADDED_IN_316)
        payment_gateway = graphene.Argument(PaymentGatewayToInitialize, description='Payment gateway used to initialize the transaction.', required=True)

    class Meta:
        doc_category = DOC_CATEGORY_PAYMENTS
        description = 'Initializes a transaction session. It triggers the webhook `TRANSACTION_INITIALIZE_SESSION`, to the requested `paymentGateways`. ' + ADDED_IN_313 + PREVIEW_FEATURE
