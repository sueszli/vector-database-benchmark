...
class AccountRequestDeletion(BaseMutation):
    ...
    class Arguments:
        redirect_url = graphene.String(required=True, description='URL of a view where users should be redirected to delete their account. URL in RFC 1808 format.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.NOTIFY_USER, description='A notification for account delete request.'), WebhookEventInfo(type=WebhookEventAsyncType.ACCOUNT_DELETE_REQUESTED, description='An account delete requested.')]
