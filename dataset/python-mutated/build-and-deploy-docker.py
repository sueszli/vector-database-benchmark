...
subprocess.check_call(['docker', 'buildx', 'build', '--tag', 'localtesting', '--load', '--build-arg', f'MITMPROXY_WHEEL={whl.name}', '.'], cwd=docker_build_dir)
r = subprocess.run(['docker', 'run', '--rm', '-v', f"{root / 'release'}:/release", 'localtesting', 'mitmdump', '-s', '/release/selftest.py'], capture_output=True)
print(r.stdout.decode())
assert 'Self-test successful' in r.stdout.decode()
assert r.returncode == 0
subprocess.check_call(['docker', 'login', '-u', os.environ['DOCKER_USERNAME'], '-p', os.environ['DOCKER_PASSWORD']])

def _buildx(docker_tag):
    if False:
        print('Hello World!')
    subprocess.check_call(['docker', 'buildx', 'build', '--tag', docker_tag, '--push', '--platform', 'linux/amd64,linux/arm64', '--build-arg', f'MITMPROXY_WHEEL={whl.name}', '.'], cwd=docker_build_dir)
if branch == 'main':
