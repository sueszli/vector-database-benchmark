...
@pytest.mark.parametrize('query,expected', [('select 1', 1), ('select 1, 2', None), ('select 1 as num union select 2 as num', None)])
@pytest.mark.asyncio
...
@pytest.mark.parametrize('tables,exists', ((['facetable', 'searchable', 'tags', 'searchable_tags'], True), (['foo', 'bar', 'baz'], False)))
@pytest.mark.asyncio
...
async def test_view_names(db):
    ...
    assert view_names == ['paginated_view', 'simple_view', 'searchable_view', 'searchable_view_configured_by_metadata']

@pytest.mark.asyncio
async def test_execute_write_block_true(db):
    await db.execute_write('update roadside_attractions set name = ? where pk = ?', ['Mystery!', 1])
...
async def test_execute_write_script(db):
    await db.execute_write_script('create table foo (id integer primary key); create table bar (id integer primary key);')
    table_names = await db.table_names()
...
async def test_execute_write_many(db):
    ...
    await db.execute_write_many('insert into foomany (id) values (?)', [(1,), (10,), (100,)])
...
async def test_attached_databases(app_client_two_attached_databases_crossdb_enabled):
    database = app_client_two_attached_databases_crossdb_enabled.ds.get_database('_memory')
