...
class Availability(Directive):
    ...
    def run(self):
        ...
        pnode = nodes.paragraph(availability_ref + self.arguments[0], classes=['availability'])
...
class AuditEvent(Directive):
    ...
    final_argument_whitespace = True
    _label = ['Raises an :ref:`auditing event <auditing>` {name} with no arguments.', 'Raises an :ref:`auditing event <auditing>` {name} with argument {args}.', 'Raises an :ref:`auditing event <auditing>` {name} with arguments {args}.']
    ...
    def run(self):
        ...
        text = label.format(name='``{}``'.format(name), args=', '.join(('``{}``'.format(a) for a in args if a)))
        ...
        if info is not new_info:
            if not self._do_args_match(info['args'], new_info['args']):
                self.logger.warn('Mismatched arguments for audit-event {}: {!r} != {!r}'.format(name, info['args'], new_info['args']))
        ids = []
        try:
            ...
            target = None
        if not target:
            target = 'audit_event_{}_{}'.format(re.sub('\\W', '_', name), len(info['source']))
            ids.append(target)
...
class DeprecatedRemoved(Directive):
    ...
    def run(self):
        ...
        if len(node):
            ...
            para = nodes.paragraph('', '', nodes.inline('', '%s.' % text, classes=['versionmodified']), translatable=False)
            node.append(para)
        env = self.state.document.settings.env
...
whatsnew_re = re.compile("(?im)^what's new in (.*?)\\??$")
...
class PydocTopicsBuilder(Builder):
    ...
    def write(self, *ignored):
        ...
        for label in status_iterator(pydoc_topic_labels, 'building topics... ', length=len(pydoc_topic_labels)):
    ...
    def finish(self):
        ...
        try:
            ...
            f.close()
opcode_sig_re = re.compile('(\\w+(?:\\+\\d)?)(?:\\s*\\((.*)\\))?')
...
def parse_opcode_signature(env, sig, signode):
    ...
    return opname.strip()
pdbcmd_sig_re = re.compile('([a-z()!]+)\\s*(.*)')
...
def process_audit_events(app, doctree, fromdocname):
    ...
    group = nodes.tgroup('', nodes.colspec(colwidth=30), nodes.colspec(colwidth=55), nodes.colspec(colwidth=15), cols=3)
    head = nodes.thead()
    body = nodes.tbody()
    table += group
    group += head
    ...
    for name in sorted(getattr(env, 'all_audit_events', ())):
        ...
        for (i, (doc, label)) in backlinks:
            if isinstance(label, str):
                ...
                try:
                    ref['refuri'] = '{}#{}'.format(app.builder.get_relative_uri(fromdocname, doc), label)
                except NoUri:
                    continue
                node += ref
        row += nodes.entry('', node)
