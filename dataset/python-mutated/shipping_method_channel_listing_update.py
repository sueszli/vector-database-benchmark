...
class ShippingMethodChannelListingAddInput(BaseInputObjectType):
    ...
    price = PositiveDecimal(description='Shipping price of the shipping method in this channel.')
    minimum_order_price = PositiveDecimal(description='Minimum order price to use this shipping method.')
    maximum_order_price = PositiveDecimal(description='Maximum order price to use this shipping method.')

    class Meta:
        doc_category = DOC_CATEGORY_SHIPPING

class ShippingMethodChannelListingInput(BaseInputObjectType):
    add_channels = NonNullList(ShippingMethodChannelListingAddInput, description='List of channels to which the shipping method should be assigned.', required=False)
    remove_channels = NonNullList(graphene.ID, description='List of channels from which the shipping method should be unassigned.', required=False)

    class Meta:
        doc_category = DOC_CATEGORY_SHIPPING

class ShippingMethodChannelListingUpdate(BaseChannelListingMutation):
    shipping_method = graphene.Field(ShippingMethodType, description='An updated shipping method instance.')

    class Arguments:
        id = graphene.ID(required=True, description='ID of a shipping method to update.')
        input = ShippingMethodChannelListingInput(required=True, description='Fields required to update shipping method channel listings.')
