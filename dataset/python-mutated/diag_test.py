...
diag_configs_short = op_bench.config_list(attr_names=['dim', 'M', 'N', 'diagonal', 'out'], attrs=[[1, 64, 64, 0, True], [2, 128, 128, -10, False], [1, 256, 256, 20, True]], cross_product_configs={'device': ['cpu', 'cuda']}, tags=['short'])
...
class DiagBenchmark(op_bench.TorchBenchmarkBase):
    ...
    def init(self, dim, M, N, diagonal, out, device):
        if False:
            for i in range(10):
                print('nop')
        self.inputs = {'input': torch.rand(M, N, device=device) if dim == 2 else torch.rand(M, device=device), 'diagonal': diagonal, 'out': out, 'out_tensor': torch.tensor(())}
