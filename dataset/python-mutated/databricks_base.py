...
from tenacity import AsyncRetrying, RetryError, Retrying, retry_if_exception, stop_after_attempt, wait_exponential
...
class BaseDatabricksHook(BaseHook):
    ...
    extra_parameters = ['token', 'host', 'use_azure_managed_identity', 'azure_ad_endpoint', 'azure_resource_id', 'azure_tenant_id', 'service_principal_oauth']
    ...
    async def _a_get_sp_token(self, resource: str) -> str:
        ...
        try:
            async for attempt in self._a_get_retry_object():
                with attempt:
                    async with self._session.post(resource, auth=aiohttp.BasicAuth(self.databricks_conn.login, self.databricks_conn.password), data='grant_type=client_credentials&scope=all-apis', headers={**self.user_agent_header, 'Content-Type': 'application/x-www-form-urlencoded'}, timeout=self.token_timeout_seconds) as resp:
                        resp.raise_for_status()
                        jsn = await resp.json()
                        jsn['expires_on'] = int(time.time() + jsn['expires_in'])
                    self._is_oauth_token_valid(jsn)
        ...
        try:
            async for attempt in self._a_get_retry_object():
                with attempt:
                    if self.databricks_conn.extra_dejson.get('use_azure_managed_identity', False):
                        params = {'api-version': '2018-02-01', 'resource': resource}
                        async with self._session.get(url=AZURE_METADATA_SERVICE_TOKEN_URL, params=params, headers={**self.user_agent_header, 'Metadata': 'true'}, timeout=self.token_timeout_seconds) as resp:
                            resp.raise_for_status()
                            jsn = await resp.json()
                    else:
                        tenant_id = self.databricks_conn.extra_dejson['azure_tenant_id']
                        data = {'grant_type': 'client_credentials', 'client_id': self.databricks_conn.login, 'resource': resource, 'client_secret': self.databricks_conn.password}
                        azure_ad_endpoint = self.databricks_conn.extra_dejson.get('azure_ad_endpoint', AZURE_DEFAULT_AD_ENDPOINT)
                        async with self._session.post(url=AZURE_TOKEN_SERVICE_URL.format(azure_ad_endpoint, tenant_id), data=data, headers={**self.user_agent_header, 'Content-Type': 'application/x-www-form-urlencoded'}, timeout=self.token_timeout_seconds) as resp:
                            resp.raise_for_status()
                            jsn = await resp.json()
                    self._is_oauth_token_valid(jsn)
                    self.oauth_tokens[resource] = jsn
    ...
    async def _a_check_azure_metadata_service(self):
        ...
        try:
            async with self._session.get(url=AZURE_METADATA_SERVICE_INSTANCE_URL, params={'api-version': '2021-02-01'}, headers={'Metadata': 'true'}, timeout=2) as resp:
                jsn = await resp.json()
            if 'compute' not in jsn or 'azEnvironment' not in jsn['compute']:
                raise AirflowException(f"Was able to fetch some metadata, but it doesn't look like Azure Metadata: {jsn}")
        except (requests_exceptions.RequestException, ValueError) as e:
    ...
    async def _a_do_api_call(self, endpoint_info: tuple[str, str], json: dict[str, Any] | None=None):
        ...
        try:
            async for attempt in self._a_get_retry_object():
                with attempt:
                    async with request_func(url, json=json, auth=auth, headers={**headers, **self.user_agent_header}, timeout=self.timeout_seconds) as response:
                        response.raise_for_status()
