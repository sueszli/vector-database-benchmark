...
class confidentialcomputingCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_challenge': ('parent', 'challenge'), 'verify_attestation': ('challenge', 'tpm_attestation', 'gcp_credentials', 'confidential_space_info', 'token_options')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=confidentialcomputingCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
