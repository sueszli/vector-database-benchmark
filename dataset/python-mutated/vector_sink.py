...
if __name__ == '__main__':
    ...
    parser.add_argument('-H', '--host', default='0.0.0.0', help='local host name (domain name or IP address)')
    parser.add_argument('-p', '--port', type=int, default=65500, help='port value to listen to for connection')
    parser.add_argument('-s', '--packet-size', type=int, default=1471, help='packet size.')
    parser.add_argument('--no-eof', action='store_true', default=False, help="don't send EOF on disconnect")
    args = parser.parse_args()
    top_block = vector_sink(args.host, args.port, args.packet_size, not args.no_eof)
    try:
        top_block.run()
