...
from ray.util.state.common import GetLogOptions, protobuf_to_task_state_dict, DEFAULT_RPC_TIMEOUT
...
class LogsManager:
    ...
    async def list_logs(self, node_id: str, timeout: int, glob_filter: str='*') -> Dict[str, List[str]]:
    ...
    async def stream_logs(self, options: GetLogOptions) -> AsyncIterable[bytes]:
        ...
        res = await self.resolve_filename(node_id=node_id, log_filename=options.filename, actor_id=options.actor_id, task_id=options.task_id, attempt_number=options.attempt_number, pid=options.pid, get_actor_fn=DataSource.actors.get, timeout=options.timeout, suffix=options.suffix, submission_id=options.submission_id)
        keep_alive = options.media_type == 'stream'
        stream = await self.client.stream_log(node_id=res.node_id, log_file_name=res.filename, keep_alive=keep_alive, lines=options.lines, interval=options.interval, timeout=options.timeout if not keep_alive else None, start_offset=res.start_offset, end_offset=res.end_offset)
    ...
    async def _resolve_job_filename(self, sub_job_id: str) -> Tuple[str, str]:
        ...
        if node_id is None:
            raise ValueError(f'Job {sub_job_id} has no driver node id info. This is likely a bug. Please file an issue.')
        log_filename = JOB_LOGS_PATH_TEMPLATE.format(submission_id=sub_job_id)
        return (node_id, log_filename)

    async def _resolve_worker_file(self, node_id: str, worker_id: Optional[str], pid: Optional[int], suffix: str, timeout: int) -> Optional[str]:
        """Resolve worker log file."""
        if worker_id is not None and pid is not None:
            raise ValueError(f'Only one of worker id({worker_id}) or pid({pid}) should be provided.')
        if worker_id is not None:
            log_files = await self.list_logs(node_id, timeout, glob_filter=f'*{worker_id}*{suffix}')
        else:
            log_files = await self.list_logs(node_id, timeout, glob_filter=f'*{pid}*{suffix}')
        for filename in [*log_files['worker_out'], *log_files['worker_err']]:
            if worker_id is not None:
                ...
                worker_pid_from_filename = int(WORKER_LOG_PATTERN.match(filename).group(3))
                if worker_pid_from_filename == pid:
                    return filename
        return None

    async def _resolve_actor_filename(self, actor_id: str, get_actor_fn: Callable[[str], Dict], suffix: str, timeout: int):
        """
        ...
        if not worker_id:
            raise ValueError(f'Worker ID for Actor ID {actor_id} not found. Actor is not scheduled yet.')
        node_id = actor_data['address'].get('rayletId')
        if not node_id:
            raise ValueError(f'Node ID for Actor ID {actor_id} not found. Actor is not scheduled yet.')
        self._verify_node_registered(node_id)
        log_filename = await self._resolve_worker_file(node_id=node_id, worker_id=worker_id, pid=None, suffix=suffix, timeout=timeout)
        return (node_id, log_filename)

    async def _resolve_task_filename(self, task_id: str, attempt_number: int, suffix: str, timeout: int):
        """
        ...
        reply = await self.client.get_all_task_info(filters=[('task_id', '=', task_id)], timeout=timeout)
        if len(reply.events_by_task) == 0:
            raise FileNotFoundError(f'Could not find log file for task: {task_id} (attempt {attempt_number}) with suffix: {suffix}')
        task_event = None
        ...
        if task_event is None:
            raise FileNotFoundError(f'Could not find log file for task attempt:{task_id}({attempt_number})')
        ...
        if node_id is None:
            raise FileNotFoundError(f'Could not find log file for task attempt.{task_id}({attempt_number}) due to missing node info.')
        if log_info is None and actor_id is not None:
            raise FileNotFoundError(f"For actor task, please query actor log for actor({actor_id}): e.g. ray logs actor --id {actor_id} . Or set RAY_ENABLE_RECORD_ACTOR_TASK_LOGGING=1 in actor's runtime env or when starting the cluster. Recording actor task's log could be expensive, so Ray turns it off by default.")
        elif log_info is None:
            raise FileNotFoundError(f'Could not find log file for task attempt:{task_id}({attempt_number}).Worker id = {worker_id}, node id = {node_id},log_info = {log_info}')
        filename_key = 'stdout_file' if suffix == 'out' else 'stderr_file'
        log_filename = log_info.get(filename_key, None)
        if log_filename is None:
            raise FileNotFoundError(f'Missing log filename info in {log_info} for task {task_id},attempt {attempt_number}')
    ...
    async def resolve_filename(self, *, node_id: Optional[str]=None, log_filename: Optional[str]=None, actor_id: Optional[str]=None, task_id: Optional[str]=None, attempt_number: Optional[int]=None, pid: Optional[str]=None, get_actor_fn: Optional[Callable[[str], Dict]]=None, timeout: int=DEFAULT_RPC_TIMEOUT, suffix: str='out', submission_id: Optional[str]=None) -> ResolvedStreamFileInfo:
        ...
        if suffix not in ['out', 'err']:
            raise ValueError(f'Suffix {suffix} is not supported. ')
        if actor_id:
            (node_id, log_filename) = await self._resolve_actor_filename(actor_id, get_actor_fn, suffix, timeout)
        elif task_id:
            (node_id, log_filename, start_offset, end_offset) = await self._resolve_task_filename(task_id, attempt_number, suffix, timeout)
        elif submission_id:
            (node_id, log_filename) = await self._resolve_job_filename(submission_id)
        elif pid:
            if node_id is None:
                raise ValueError(f'Node id needs to be specified for resolving filenames of pid {pid}')
            self._verify_node_registered(node_id)
            log_filename = await self._resolve_worker_file(node_id=node_id, worker_id=None, pid=pid, suffix=suffix, timeout=timeout)
        if log_filename is None:
            raise FileNotFoundError(f'Could not find a log file. Please make sure the given option exists in the cluster.\n\tnode_id: {node_id}\n\tfilename: {log_filename}\n\tactor_id: {actor_id}\n\ttask_id: {task_id}\n\tpid: {pid}\n\tsuffix: {suffix}\n\tsubmission_id: {submission_id}\n\tattempt_number: {attempt_number}\n')
        res = ResolvedStreamFileInfo(node_id=node_id, filename=log_filename, start_offset=start_offset, end_offset=end_offset)
