...
from metaflow.plugins.aws.step_functions.production_token import load_token, new_token, store_token
...
def resolve_token(name, token_prefix, obj, authorize, given_token, generate_new_token, is_project):
    ...
    if workflow is None:
        obj.echo('It seems this is the first time you are deploying *%s* to Airflow.' % name)
        prev_token = None
    else:
    ...
    if prev_token is not None:
        ...
        if prev_user != get_username() and authorize != prev_token:
            obj.echo('There is an existing version of *%s* on Airflow which was deployed by the user *%s*.' % (name, prev_user))
    ...
    obj.echo('If you want to authorize other people to deploy new versions of this flow to Airflow, they need to call')
...
@click.option('--name', default=None, type=str, help='Airflow DAG name. The flow name is used instead if this option is not specified')
@click.pass_obj
...
@click.option('--tag', 'tags', multiple=True, default=None, help='Annotate all objects produced by Airflow DAG executions with the given tag. You can specify this option multiple times to attach multiple tags.')
@click.option('--is-paused-upon-creation', default=False, is_flag=True, help='Generated Airflow DAG is paused/unpaused upon creation.')
@click.option('--namespace', 'user_namespace', default=None, help='Change the namespace from the default to the given tag. See run --help for more information.')
@click.option('--max-workers', default=100, show_default=True, help='Maximum number of parallel processes.')
@click.option('--workflow-timeout', default=None, type=int, help='Workflow timeout in seconds. Enforced only for scheduled DAGs.')
@click.option('--worker-pool', default=None, show_default=True, help='Worker pool for Airflow DAG execution.')
@click.pass_obj
def create(obj, file, authorize=None, generate_new_token=False, given_token=None, tags=None, is_paused_upon_creation=False, user_namespace=None, max_workers=None, workflow_timeout=None, worker_pool=None):
    if False:
        for i in range(10):
            print('nop')
    if os.path.abspath(sys.argv[0]) == os.path.abspath(file):
        raise MetaflowException('Airflow DAG file name cannot be the same as flow file name')
    _validate_workflow(obj.flow, obj.graph, obj.flow_datastore, obj.metadata, workflow_timeout)
    obj.echo('Compiling *%s* to Airflow DAG...' % obj.dag_name, bold=True)
    token = resolve_token(obj.dag_name, obj.token_prefix, obj, authorize, given_token, generate_new_token, obj.is_project)
    flow = make_flow(obj, obj.dag_name, token, tags, is_paused_upon_creation, user_namespace, max_workers, workflow_timeout, worker_pool, file)
    with open(file, 'w') as f:
        f.write(flow.compile())
    obj.echo('DAG *{dag_name}* for flow *{name}* compiled to Airflow successfully.\n'.format(dag_name=obj.dag_name, name=current.flow_name), bold=True)
...
def _validate_foreach_constraints(graph):
    ...
    def traverse_graph(node, state):
        ...
        if node.type == 'foreach' and node.is_inside_foreach:
            raise NotSupportedException('Step *%s* is a foreach step called within a foreach step. This type of graph is currently not supported with Airflow.' % node.name)
        ...
        if node.type in ('start', 'linear', 'join', 'foreach'):
            ...
            if 'foreach_stack' in state and len(state['foreach_stack']) > 2:
                raise NotSupportedException('The foreach step *%s* created by step *%s* needs to have an immediate join step. Step *%s* is invalid since it is a linear step with a foreach. This type of graph is currently not supported with Airflow.' % (state['foreach_stack'][1], state['foreach_stack'][0], state['foreach_stack'][-1]))
...
def _validate_workflow(flow, graph, flow_datastore, metadata, workflow_timeout):
    ...
    for (var, param) in flow._get_parameters():
        ...
        if 'default' not in param.kwargs:
            raise MetaflowException('Parameter *%s* does not have a default value. A default value is required for parameters when deploying flows on Airflow.' % param.name)
    _validate_foreach_constraints(graph)
    for node in graph:
        if node.parallel_foreach:
            raise AirflowException('Deploying flows with @parallel decorator(s) to Airflow is not supported currently.')
        if any([d.name == 'batch' for d in node.decorators]):
            raise NotSupportedException("Step *%s* is marked for execution on AWS Batch with Airflow which isn't currently supported." % node.name)
    SUPPORTED_DATASTORES = ('azure', 's3', 'gs')
    if flow_datastore.TYPE not in SUPPORTED_DATASTORES:
        raise AirflowException('Datastore type `%s` is not supported with `airflow create`. Please choose from datastore of type %s when calling `airflow create`' % (str(flow_datastore.TYPE), 'or '.join(['`%s`' % x for x in SUPPORTED_DATASTORES])))
    schedule = flow._flow_decorators.get('schedule')
    if not schedule:
        return
    schedule = schedule[0]
    if schedule.timezone is not None:
        raise AirflowException('`airflow create` does not support scheduling with `timezone`.')
...
def resolve_dag_name(name):
    ...
    if project:
        ...
        if dag_name and VALID_NAME.search(dag_name):
            raise MetaflowException("Name '%s' contains invalid characters. Please construct a name using regex %s" % (dag_name, VALID_NAME.pattern))
        ...
        if name and VALID_NAME.search(name):
            raise MetaflowException("Name '%s' contains invalid characters. Please construct a name using regex %s" % (name, VALID_NAME.pattern))
