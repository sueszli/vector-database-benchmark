...
class TFResNetModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': TFResNetModel, 'image-classification': TFResNetForImageClassification} if is_tf_available() else {}
    test_pruning = False
...
class TFResNetModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        if False:
            for i in range(10):
                print('nop')
        return AutoImageProcessor.from_pretrained(TF_RESNET_PRETRAINED_MODEL_ARCHIVE_LIST[0]) if is_vision_available() else None
