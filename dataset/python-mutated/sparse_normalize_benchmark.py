...
def benchmark_sparse_normalize(categorical_limit, embedding_size, average_len, batch_size, iterations, flush_cache, fp16):
    if False:
        i = 10
        return i + 15
    ...
    net = core.Net('mynet')
    op = core.CreateOperator('Float16SparseNormalize' if fp16 else 'SparseNormalize', ['X_fp16', 'indices'] if fp16 else ['X', 'indices'], 'X_fp16' if fp16 else 'X')
    ...
    print('{} GB/sec'.format((2 if fp16 else 4) * embedding_size * workspace.FetchBlob('indices').size / runtimes[2 if flush_cache else 1] / 1000000.0))
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='minimal benchmark for sparse lengths sum.')
    parser.add_argument('-e', '--embedding-size', type=int, default=600000, help='Lookup table size.')
    ...
    benchmark_sparse_normalize(args.embedding_size, args.embedding_dim, args.average_len, args.batch_size, args.iteration, args.flush_cache, args.fp16)