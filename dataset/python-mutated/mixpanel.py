...
class MixpanelWrapper(AbstractBaseIdentityIntegrationWrapper):
    ...
    def __init__(self, config: MixpanelConfiguration):
        ...
        self.headers = {'Accept': 'text/plain', 'X-Mixpanel-Integration-ID': 'flagsmith'}

    def _identify_user(self, user_data: dict) -> None:
        if False:
            print('Hello World!')
        response = requests.post(self.url, headers=self.headers, json=user_data)
        logger.debug('Sent event to Mixpanel. Response code was: %s' % response.status_code)
        logger.debug('Sent event to Mixpanel. Response content was: %s' % response.content)
    ...
    def generate_user_data(self, identity: Identity, feature_states: typing.List[FeatureState], trait_models: typing.List[Trait]=None) -> dict:
        ...
        return [{'$token': self.api_key, '$distinct_id': identity.identifier, '$set': feature_properties, '$ip': '0'}]