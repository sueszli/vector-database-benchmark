...
class ModInfo(MixinMeta):
    ...
    async def rename(self, ctx: commands.Context, member: discord.Member, *, nickname: str=''):
        ...
        if not nickname:
            ...
            return
        if not ((me.guild_permissions.manage_nicknames or me.guild_permissions.administrator) and me.top_role > member.top_role and (member != ctx.guild.owner)):
            await ctx.send(_('I do not have permission to rename that member. They may be higher than or equal to me in the role hierarchy.'))
        elif ctx.author != member and (not await is_allowed_by_hierarchy(self.bot, self.config, ctx.guild, ctx.author, member)):
    ...
    async def userinfo(self, ctx, *, member: discord.Member=None):
        ...
        if roles:
            ...
            if len(role_str) > 1024:
                continuation_string = _('and {numeric_number} more roles not displayed due to embed limits.')
        ...
        if role_str is not None:
            data.add_field(name=_('Roles') if len(roles) > 1 else _('Role'), value=role_str, inline=False)
        for (single_form, plural_form, names) in ((_('Previous Username'), _('Previous Usernames'), usernames), (_('Previous Global Display Name'), _('Previous Global Display Names'), display_names), (_('Previous Server Nickname'), _('Previous Server Nicknames'), nicks)):
            if names:
                data.add_field(name=plural_form if len(names) > 1 else single_form, value=filter_invites(', '.join(names)), inline=False)
    ...
    async def names(self, ctx: commands.Context, *, member: discord.Member):
        ...
        (usernames, display_names, nicks) = await self.get_names(member)
        parts = []
        for (header, names) in ((_('Past 20 usernames:'), usernames), (_('Past 20 global display names:'), display_names), (_('Past 20 server nicknames:'), nicks)):
            if names:
