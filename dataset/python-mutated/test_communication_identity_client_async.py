...
class TestClientAsync(ACSIdentityTestCase):
    ...
    async def test_create_user_and_token_with_scope_chat_and_voip(self):
        identity_client = self.create_client_from_connection_string()
        async with identity_client:
            (user, token_response) = await identity_client.create_user_and_token(scopes=[CommunicationTokenScope.CHAT, CommunicationTokenScope.VOIP])
        assert user.properties.get('id') is not None
        assert token_response.token is not None
    ...
    async def test_get_token_from_token_credential_with_scope_chat_and_voip(self):
        ...
        async with identity_client:
            user = await identity_client.create_user()
            token_response = await identity_client.get_token(user, scopes=[CommunicationTokenScope.CHAT, CommunicationTokenScope.VOIP])
        assert user.properties.get('id') is not None
        assert token_response.token is not None
    ...
    async def test_get_token_for_teams_user_with_expired_token(self):
        ...
        async with identity_client:
            with pytest.raises(Exception) as ex:
                ...
                await identity_client.get_token_for_teams_user(self.expired_teams_token, self.m365_client_id, user_object_id)
        assert str(ex.value.status_code) == '401'
        assert ex.value.message is not None
