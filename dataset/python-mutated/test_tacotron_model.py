...
class TacotronCapacitronTrainTest(unittest.TestCase):
    ...
    def test_train_step():
        if False:
            for i in range(10):
                print('nop')
        config = TacotronConfig(num_chars=32, num_speakers=10, use_speaker_embedding=True, out_channels=513, decoder_output_dim=80, use_capacitron_vae=True, capacitron_vae=CapacitronVAEConfig(), optimizer='CapacitronOptimizer', optimizer_params={'RAdam': {'betas': [0.9, 0.998], 'weight_decay': 1e-06}, 'SGD': {'lr': 1e-05, 'momentum': 0.9}})
        batch = dict({})
