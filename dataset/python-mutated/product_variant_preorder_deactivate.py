...
class ProductVariantPreorderDeactivate(BaseMutation):
    product_variant = graphene.Field(ProductVariant, description='Product variant with ended preorder.')

    class Arguments:
        id = graphene.ID(required=True, description='ID of a variant which preorder should be deactivated.')

    class Meta:
        description = 'Deactivates product variant preorder. It changes all preorder allocation into regular allocation.' + ADDED_IN_31
