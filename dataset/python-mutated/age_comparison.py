...
class AgeComparisonType:
    OLDER = 'older'
    NEWER = 'newer'
timeranges = {'minute': ('minute(s)', timedelta(minutes=1)), 'hour': ('hour(s)', timedelta(hours=1)), 'day': ('day(s)', timedelta(days=1)), 'week': ('week(s)', timedelta(days=7))}
...
class AgeComparisonFilter(EventFilter):
    ...
    form_fields = {'comparison_type': {'type': 'choice', 'choices': age_comparison_choices}, 'value': {'type': 'number', 'placeholder': 10}, 'time': {'type': 'choice', 'choices': get_timerange_choices()}}
