...
class AlldebridCom(MultiDownloader):
    ...
    __version__ = '0.66'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:\\w+\\.)?(?:alldebrid\\.com|debrid\\.it|alld\\.io)/(?:dl|f)/[\\w^_]+'
    __config__ = [('enabled', 'bool', 'Activated', True), ('use_premium', 'bool', 'Use premium account if available', True), ('fallback', 'bool', 'Fallback to free download if premium fails', False), ('chk_filesize', 'bool', 'Check file size', True), ('max_wait', 'int', 'Reconnect if waiting time is greater than minutes', 10), ('revert_failed', 'bool', 'Revert to standard download if fails', True), ('stream_quality', 'Lowest;LD 144p;LD 240p;SD 380p;HQ 480p;HD 720p;HD 1080p;Highest', 'Quality to download from stream hosters', 'Highest')]
    __description__ = 'Alldebrid.com multi-downloader plugin'
    __license__ = 'GPLv3'
    __authors__ = [('Andy Voigt', 'spamsales@online.de'), ('GammaC0de', 'nitzo2001[AT]yahoo[DOT]com')]
    DISPOSITION = False
    URL_REPLACEMENTS = [('https?://(?:www\\.)?mega(?:\\.co)?\\.nz/#N!(?P<ID>[\\w^_]+)!(?P<KEY>[\\w\\-,=]+)###n=(?P<OWNER>[\\w^_]+)', lambda m: 'https://mega.nz/#!{}!{}~~{}'.format(m.group('ID'), m.group('KEY'), m.group('OWNER'))), ('https?://(?:www\\.)?mega(?:\\.co)?\\.nz/.*', lambda m: m.group(0).replace('_', '/'))]
