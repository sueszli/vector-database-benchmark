...
from typing import Callable, Dict, Final, List, Mapping, Optional, Sequence, Set, Tuple, TypedDict, Union
MYPY = False
if MYPY:
...
(email_models, suggestion_models) = models.Registry.import_models([models.Names.EMAIL, models.Names.SUGGESTION])
app_identity_services = models.Registry.import_app_identity_services()
transaction_services = models.Registry.import_transaction_services()
secrets_services = models.Registry.import_secrets_services()
PlatformParameterRegistry = platform_parameter_registry.Registry
NEW_REVIEWER_EMAIL_DATA: Dict[str, Dict[str, str]] = {constants.CONTRIBUTION_RIGHT_CATEGORY_REVIEW_TRANSLATION: {'review_category': 'translations', 'to_check': 'translation suggestions', 'description_template': '%s language translations', 'rights_message_template': 'review translation suggestions made by contributors in the %s language'}, constants.CONTRIBUTION_RIGHT_CATEGORY_REVIEW_VOICEOVER: {'review_category': 'voiceovers', 'to_check': 'voiceover applications', 'description_template': '%s language voiceovers', 'rights_message_template': 'review voiceover applications made by contributors in the %s language'}, constants.CONTRIBUTION_RIGHT_CATEGORY_REVIEW_QUESTION: {'review_category': 'questions', 'to_check': 'question suggestions', 'description': 'questions', 'rights_message': 'review question suggestions made by contributors'}}
REMOVED_REVIEWER_EMAIL_DATA: Dict[str, Dict[str, str]] = {constants.CONTRIBUTION_RIGHT_CATEGORY_REVIEW_TRANSLATION: {'review_category': 'translation', 'role_description_template': 'translation reviewer role in the %s language', 'rights_message_template': 'review translation suggestions made by contributors in the %s language', 'contribution_allowed': 'translations'}, constants.CONTRIBUTION_RIGHT_CATEGORY_REVIEW_VOICEOVER: {'review_category': 'voiceover', 'role_description_template': 'voiceover reviewer role in the %s language', 'rights_message_template': 'review voiceover applications made by contributors in the %s language', 'contribution_allowed': 'voiceovers'}, constants.CONTRIBUTION_RIGHT_CATEGORY_REVIEW_QUESTION: {'review_category': 'question', 'role_description': 'question reviewer role', 'rights_message': 'review question suggestions made by contributors', 'contribution_allowed': 'questions'}}
EMAIL_SENDER_NAME: platform_parameter_domain.PlatformParameter = PlatformParameterRegistry.create_platform_parameter(platform_parameter_list.ParamNames.EMAIL_SENDER_NAME, 'The default sender name for outgoing emails.', platform_parameter_domain.DataTypes.STRING, default='Site Admin')
EMAIL_FOOTER: platform_parameter_domain.PlatformParameter = PlatformParameterRegistry.create_platform_parameter(platform_parameter_list.ParamNames.EMAIL_FOOTER, 'The footer to append to all outgoing emails. (This should be written in HTML and include an unsubscribe link.)', platform_parameter_domain.DataTypes.STRING, default='You can change your email preferences via the <a href="%s%s">Preferences</a> page.' % (feconf.OPPIA_SITE_URL, feconf.PREFERENCES_URL))
_PLACEHOLDER_SUBJECT: Final = 'THIS IS A PLACEHOLDER.'
_PLACEHOLDER_HTML_BODY: Final = 'THIS IS A <b>PLACEHOLDER</b> AND SHOULD BE REPLACED.'
SIGNUP_EMAIL_SUBJECT_CONTENT: platform_parameter_domain.PlatformParameter = PlatformParameterRegistry.create_platform_parameter(platform_parameter_list.ParamNames.SIGNUP_EMAIL_SUBJECT_CONTENT, 'Content of email sent after a new user signs up. Set the email subject. These emails are only sent if the functionality is enabled in feconf.py.', platform_parameter_domain.DataTypes.STRING, default=_PLACEHOLDER_SUBJECT)
SIGNUP_EMAIL_BODY_CONTENT: platform_parameter_domain.PlatformParameter = PlatformParameterRegistry.create_platform_parameter(platform_parameter_list.ParamNames.SIGNUP_EMAIL_BODY_CONTENT, 'Content of email sent after a new user signs up. (The email body should be written with HTML and not include a salutation or footer.) These emails are only sent if the functionality is enabled in feconf.py.', platform_parameter_domain.DataTypes.STRING, default=_PLACEHOLDER_HTML_BODY)
...
EDITOR_ROLE_EMAIL_HTML_ROLES: Dict[str, str] = {rights_domain.ROLE_OWNER: EXPLORATION_ROLE_MANAGER, rights_domain.ROLE_EDITOR: EXPLORATION_ROLE_EDITOR, rights_domain.ROLE_VOICE_ARTIST: EXPLORATION_ROLE_VOICE_ARTIST, rights_domain.ROLE_VIEWER: EXPLORATION_ROLE_PLAYTESTER}
_EDITOR_ROLE_EMAIL_HTML_RIGHTS: Dict[str, str] = {'can_manage': '<li>Change the exploration permissions</li><br>', 'can_edit': '<li>Edit the exploration</li><br>', 'can_voiceover': '<li>Voiceover the exploration</li><br>', 'can_play': '<li>View and playtest the exploration</li><br>'}
EDITOR_ROLE_EMAIL_RIGHTS_FOR_ROLE: Dict[str, str] = {EXPLORATION_ROLE_MANAGER: _EDITOR_ROLE_EMAIL_HTML_RIGHTS['can_manage'] + _EDITOR_ROLE_EMAIL_HTML_RIGHTS['can_edit'] + _EDITOR_ROLE_EMAIL_HTML_RIGHTS['can_play'], EXPLORATION_ROLE_EDITOR: _EDITOR_ROLE_EMAIL_HTML_RIGHTS['can_edit'] + _EDITOR_ROLE_EMAIL_HTML_RIGHTS['can_play'], EXPLORATION_ROLE_VOICE_ARTIST: _EDITOR_ROLE_EMAIL_HTML_RIGHTS['can_voiceover'] + _EDITOR_ROLE_EMAIL_HTML_RIGHTS['can_play'], EXPLORATION_ROLE_PLAYTESTER: _EDITOR_ROLE_EMAIL_HTML_RIGHTS['can_play']}
UNPUBLISH_EXPLORATION_EMAIL_HTML_BODY: platform_parameter_domain.PlatformParameter = PlatformParameterRegistry.create_platform_parameter(platform_parameter_list.ParamNames.UNPUBLISH_EXPLORATION_EMAIL_HTML_BODY, 'Default content for the email sent after an exploration is unpublished by a moderator. These emails are only sent if the functionality is enabled in feconf.py. Leave this field blank if emails should not be sent.', platform_parameter_domain.DataTypes.STRING, default="I'm writing to inform you that I have unpublished the above exploration.")
CONTRIBUTOR_DASHBOARD_REVIEWER_NOTIFICATION_EMAIL_DATA: Dict[str, str] = {'email_body_template': 'Hi %s,<br><br>There are new review opportunities that we think you might be interested in on the <a href="%s%s">Contributor Dashboard</a>. Here are some examples of contributions that have been waiting the longest for review:<br><br><ul>%s</ul><br>Please take some time to review any of the above contributions (if they still need a review) or any other contributions on the dashboard. We appreciate your help!<br><br>Thanks again, and happy reviewing!<br>- The Oppia Contributor Dashboard Team<br><br>%s', 'email_subject': 'Contributor Dashboard Reviewer Opportunities'}
CURRICULUM_ADMIN_CHAPTER_NOTIFICATION_EMAIL_DATA: Dict[str, str] = {'overdue_chapters_template': 'The following stories have unpublished chapters which are behind schedule. Please publish them or adjust the planned publication date.<br><br><ol>%s</ol>', 'upcoming_chapters_template': 'The following stories have unpublished chapters which are due for publication in the next ' + str(constants.CHAPTER_PUBLICATION_NOTICE_PERIOD_IN_DAYS) + ' days. Please ensure they are published on or before the planned date or adjust the planned publication date.<br><br><ol>%s</ol>', 'email_subject': 'Chapter Publication Notifications'}
HTML_FOR_SUGGESTION_DESCRIPTION: Dict[str, Dict[str, Union[str, Callable[[Dict[str, str]], Tuple[str, ...]]]]] = {'suggestion_template': {feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT: '<li>The following %s translation suggestion was submitted for review %s ago:<br>%s</li><br>', feconf.SUGGESTION_TYPE_ADD_QUESTION: '<li>The following question suggestion was submitted for review %s ago:<br>%s</li><br>'}, 'suggestion_template_values_getter_functions': {feconf.SUGGESTION_TYPE_TRANSLATE_CONTENT: lambda values_dict: (values_dict['language'], values_dict['review_wait_time'], values_dict['suggestion_content']), feconf.SUGGESTION_TYPE_ADD_QUESTION: lambda values_dict: (values_dict['review_wait_time'], values_dict['suggestion_content'])}}
ADMIN_NOTIFICATION_FOR_REVIEWER_SHORTAGE_EMAIL_DATA: Dict[str, str] = {'email_body_template': 'Hi %s,<br><br>In the <a href="%s%s#/roles">admin roles page,</a> please add reviewers to the Contributor Dashboard community by entering their username(s) and allow reviewing for the suggestion types that need more reviewers bolded below.<br><br>%sThanks so much - we appreciate your help!<br><br>Best Wishes!<br>- The Oppia Contributor Dashboard Team', 'email_subject': 'Reviewers Needed for Contributor Dashboard', 'one_language_template': 'There have been <b>%s translation suggestions</b> created on the <a href="%s%s">Contributor Dashboard page</a> where there are not enough reviewers.<br><br>', 'multi_language_template': 'There have been <b>translation suggestions</b> created on the <a href="%s%s">Contributor Dashboard page</a> in languages where there are not enough reviewers. The languages that need more reviewers are:<br><ul>%s</ul><br>', 'question_template': 'There have been <b>question suggestions</b> created on the <a href="%s%s">Contributor Dashboard page</a> where there are not enough reviewers.<br><br>' % (feconf.OPPIA_SITE_URL, feconf.CONTRIBUTOR_DASHBOARD_URL)}
ADMIN_NOTIFICATION_FOR_SUGGESTIONS_NEEDING_REVIEW_EMAIL_DATA: Dict[str, str] = {'email_body_template': 'Hi %s,<br><br>There are suggestions on the <a href="%s%s">Contributor Dashboard</a> that have been waiting for more than %s days for review. Please take a look at the suggestions mentioned below and help them get reviewed by going to the <a href="%s%s#/roles">admin roles page</a> and either:<br><br><ul><li>Add more reviewers to the suggestion types that have suggestions waiting too long for a review</li><br><li>Find the existing reviewers and email reviewers directly about the suggestions waiting for a review</li><br></ul><br>Here are the suggestions that have been waiting too long for a review:<br><br><ul>%s</ul><br>Thanks so much - we appreciate your help!<br>Best Wishes!<br><br>- The Oppia Contributor Dashboard Team', 'email_subject': 'Contributor Dashboard Suggestions Have Been Waiting Too Long for Review'}
CONTRIBUTOR_RANK_ACHIEVEMENT_NOTIFICATION: Dict[str, Dict[str, Dict[str, str]]] = {feconf.CONTRIBUTION_TYPE_TRANSLATION: {feconf.CONTRIBUTION_SUBTYPE_ACCEPTANCE: {'email_body_template': 'Hi %s,<br><br>This is to let you know that you have successfully achieved the %s rank for submitting translations in %s. Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="%s%s">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team', 'email_subject': 'Oppia Translator Rank Achievement!'}, feconf.CONTRIBUTION_SUBTYPE_REVIEW: {'email_body_template': 'Hi %s,<br><br>This is to let you know that you have successfully achieved the %s rank for reviewing translations in %s. Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="%s%s">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team', 'email_subject': 'Oppia Translation Reviewer Rank Achievement!'}, feconf.CONTRIBUTION_SUBTYPE_EDIT: {'email_body_template': 'Hi %s,<br><br>This is to let you know that you have successfully achieved the %s rank for correcting translations in %s. Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="%s%s">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team', 'email_subject': 'Oppia Translation Reviewer Rank Achievement!'}}, feconf.CONTRIBUTION_TYPE_QUESTION: {feconf.CONTRIBUTION_SUBTYPE_ACCEPTANCE: {'email_body_template': 'Hi %s,<br><br>This is to let you know that you have successfully achieved the %s rank for submitting practice questions. Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="%s%s">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team', 'email_subject': 'Oppia Question Submitter Rank Achievement!'}, feconf.CONTRIBUTION_SUBTYPE_REVIEW: {'email_body_template': 'Hi %s,<br><br>This is to let you know that you have successfully achieved the %s rank for reviewing  practice questions. Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="%s%s">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team', 'email_subject': 'Oppia Question Reviewer Rank Achievement!'}, feconf.CONTRIBUTION_SUBTYPE_EDIT: {'email_body_template': 'Hi %s,<br><br>This is to let you know that you have successfully achieved the %s rank for correcting practice questions. Your efforts help Oppia grow better every day and support students around the world.<br><br>You can check all the achievements you earned in the <a href="%s%s">Contributor Dashboard</a>.<br><br>Best wishes and we hope you can continue to contribute!<br><br>The Oppia Contributor Dashboard Team', 'email_subject': 'Oppia Question Reviewer Rank Achievement!'}}}
SENDER_VALIDATORS: Dict[str, Union[bool, Callable[[str], bool]]] = {feconf.EMAIL_INTENT_SIGNUP: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_UNPUBLISH_EXPLORATION: user_services.is_moderator, feconf.EMAIL_INTENT_DAILY_BATCH: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_EDITOR_ROLE_NOTIFICATION: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_FEEDBACK_MESSAGE_NOTIFICATION: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_SUGGESTION_NOTIFICATION: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_SUBSCRIPTION_NOTIFICATION: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_QUERY_STATUS_NOTIFICATION: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_MARKETING: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_DELETE_EXPLORATION: user_services.is_moderator, feconf.EMAIL_INTENT_REPORT_BAD_CONTENT: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_ONBOARD_REVIEWER: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_REMOVE_REVIEWER: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_REVIEW_CREATOR_DASHBOARD_SUGGESTIONS: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_NOTIFY_CURRICULUM_ADMINS_CHAPTERS: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_ADDRESS_CONTRIBUTOR_DASHBOARD_SUGGESTIONS: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_REVIEW_CONTRIBUTOR_DASHBOARD_SUGGESTIONS: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_NOTIFY_CONTRIBUTOR_DASHBOARD_ACHIEVEMENTS: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_ADD_CONTRIBUTOR_DASHBOARD_REVIEWERS: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_ACCOUNT_DELETED: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.BULK_EMAIL_INTENT_MARKETING: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.BULK_EMAIL_INTENT_IMPROVE_EXPLORATION: user_services.is_curriculum_admin, feconf.BULK_EMAIL_INTENT_CREATE_EXPLORATION: user_services.is_curriculum_admin, feconf.BULK_EMAIL_INTENT_CREATOR_REENGAGEMENT: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.BULK_EMAIL_INTENT_ML_JOB_FAILURE: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.BULK_EMAIL_INTENT_LEARNER_REENGAGEMENT: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.BULK_EMAIL_INTENT_TEST: lambda x: x == feconf.SYSTEM_COMMITTER_ID, feconf.EMAIL_INTENT_ML_JOB_FAILURE: lambda x: x == feconf.SYSTEM_COMMITTER_ID}
