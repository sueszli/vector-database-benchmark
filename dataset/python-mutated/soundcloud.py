...
from ..utils import error_to_compat_str, ExtractorError, float_or_none, int_or_none, KNOWN_EXTENSIONS, mimetype2ext, parse_qs, str_or_none, try_get, unified_timestamp, update_url_query, url_or_none, urlhandle_detect_ext

class SoundcloudEmbedIE(InfoExtractor):
    _VALID_URL = 'https?://(?:w|player|p)\\.soundcloud\\.com/player/?.*?\\burl=(?P<id>.+)'
    _EMBED_REGEX = ['<iframe[^>]+src=(["\\\'])(?P<url>(?:https?://)?(?:w\\.)?soundcloud\\.com/player.+?)\\1']
    _TEST = {'url': 'https://w.soundcloud.com/player/?visual=true&url=https%3A%2F%2Fapi.soundcloud.com%2Fplaylists%2F922213810&show_artwork=true&maxwidth=640&maxheight=960&dnt=1&secret_token=s-ziYey', 'only_matching': True}
...
class SoundcloudBaseIE(InfoExtractor):
    ...
    _ARTWORK_MAP = {'mini': 16, 'tiny': 20, 'small': 32, 'badge': 47, 't67x67': 67, 'large': 100, 't300x300': 300, 'crop': 400, 't500x500': 500, 'original': 0}
...
class SoundcloudUserIE(SoundcloudPagedPlaylistBaseIE):
    ...
    _TESTS = [{'url': 'https://soundcloud.com/soft-cell-official', 'info_dict': {'id': '207965082', 'title': 'Soft Cell (All)'}, 'playlist_mincount': 28}, {'url': 'https://soundcloud.com/soft-cell-official/tracks', 'info_dict': {'id': '207965082', 'title': 'Soft Cell (Tracks)'}, 'playlist_mincount': 27}, {'url': 'https://soundcloud.com/soft-cell-official/albums', 'info_dict': {'id': '207965082', 'title': 'Soft Cell (Albums)'}, 'playlist_mincount': 1}, {'url': 'https://soundcloud.com/jcv246/sets', 'info_dict': {'id': '12982173', 'title': 'Jordi / cv (Sets)'}, 'playlist_mincount': 2}, {'url': 'https://soundcloud.com/jcv246/reposts', 'info_dict': {'id': '12982173', 'title': 'Jordi / cv (Reposts)'}, 'playlist_mincount': 6}, {'url': 'https://soundcloud.com/clalberg/likes', 'info_dict': {'id': '11817582', 'title': 'clalberg (Likes)'}, 'playlist_mincount': 5}, {'url': 'https://soundcloud.com/grynpyret/spotlight', 'info_dict': {'id': '7098329', 'title': 'Grynpyret (Spotlight)'}, 'playlist_mincount': 1}]
    _BASE_URL_MAP = {'all': 'stream/users/%s', 'tracks': 'users/%s/tracks', 'albums': 'users/%s/albums', 'sets': 'users/%s/playlists', 'reposts': 'stream/users/%s/reposts', 'likes': 'users/%s/likes', 'spotlight': 'users/%s/spotlight'}
...
class SoundcloudUserPermalinkIE(SoundcloudPagedPlaylistBaseIE):
    ...
    _TESTS = [{'url': 'https://api.soundcloud.com/users/30909869', 'info_dict': {'id': '30909869', 'title': 'neilcic'}, 'playlist_mincount': 23}]
...
class SoundcloudTrackStationIE(SoundcloudPagedPlaylistBaseIE):
    ...
    _TESTS = [{'url': 'https://soundcloud.com/stations/track/officialsundial/your-text', 'info_dict': {'id': '286017854', 'title': 'Track station: your text'}, 'playlist_mincount': 47}]
...
class SoundcloudPlaylistIE(SoundcloudPlaylistBaseIE):
    ...
    _TESTS = [{'url': 'https://api.soundcloud.com/playlists/4110309', 'info_dict': {'id': '4110309', 'title': "TILT Brass - Bowery Poetry Club, August '03 [Non-Site SCR 02]", 'description': 're:.*?TILT Brass - Bowery Poetry Club'}, 'playlist_count': 6}]
...
class SoundcloudSearchIE(SoundcloudBaseIE, SearchInfoExtractor):
    ...
    _TESTS = [{'url': 'scsearch15:post-avant jazzcore', 'info_dict': {'id': 'post-avant jazzcore', 'title': 'post-avant jazzcore'}, 'playlist_count': 15}]
