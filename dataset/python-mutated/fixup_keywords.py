...
class recaptchaenterpriseCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'annotate_assessment': ('name', 'annotation'), 'create_assessment': ('parent', 'assessment'), 'create_key': ('parent', 'key'), 'delete_key': ('name',), 'get_key': ('name',), 'list_keys': ('parent', 'page_size', 'page_token'), 'update_key': ('key', 'update_mask')}
    ...
    def leave_Call(self, original: cst.Call, updated: cst.Call) -> cst.CSTNode:
        ...
        if any((k.keyword.value == 'request' for k in kwargs)):
            return updated
        (kwargs, ctrl_kwargs) = partition(lambda a: not a.keyword.value in self.CTRL_PARAMS, kwargs)
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=recaptchaenterpriseCallTransformer()):
    if False:
        print('Hello World!')
