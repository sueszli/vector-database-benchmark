...
def test_dataproc_batch(test_bucket, bq_dataset):
    ...
    BATCH_ID = f'summit-dag-expansion-test-{TEST_ID}'
    BATCH_CONFIG = {'runtime_config': {'version': '1.1'}, 'pyspark_batch': {'main_python_file_uri': PROCESSING_PYTHON_FILE, 'args': [BUCKET_NAME, f'{BQ_DESTINATION_DATASET_NAME}.{BQ_DESTINATION_TABLE_NAME}', f'{BQ_DESTINATION_DATASET_NAME}.{BQ_NORMALIZED_TABLE_NAME}', f'{BQ_DESTINATION_DATASET_NAME}.{BQ_PRCP_MEAN_TABLE_NAME}', f'{BQ_DESTINATION_DATASET_NAME}.{BQ_SNOW_MEAN_TABLE_NAME}', f'{BQ_DESTINATION_DATASET_NAME}.{BQ_PHX_PRCP_TABLE_NAME}', f'{BQ_DESTINATION_DATASET_NAME}.{BQ_PHX_SNOW_TABLE_NAME}']}}
...
def bq_dataset(test_bucket):
    ...
    job_config = bigquery.LoadJobConfig(schema=[bigquery.SchemaField('ID', 'STRING'), bigquery.SchemaField('LATITUDE', 'FLOAT'), bigquery.SchemaField('LONGITUDE', 'FLOAT'), bigquery.SchemaField('STATE', 'STRING'), bigquery.SchemaField('DATE', 'STRING'), bigquery.SchemaField('ELEMENT', 'STRING'), bigquery.SchemaField('VALUE', 'FLOAT')], skip_leading_rows=1, source_format=bigquery.SourceFormat.CSV)
    uri = f'gs://{BUCKET_NAME}/{TEST_CSV_FILE}'
    load_job = BQ_CLIENT.load_table_from_uri(uri, BQ_DESTINATION_TABLE_ID, job_config=job_config)
    load_job.result()
    destination_table = BQ_CLIENT.get_table(BQ_DESTINATION_TABLE_ID)
