...
class Repository(NamedTuple):
    ...
    async def clone(self: Self, checkout_dir: Path) -> AsyncIterator[Path]:
        ...
        git_clone_command = ['git', 'clone', '--config', 'advice.detachedHead=false', '--quiet', '--depth', '1', '--no-tags']
        if self.ref:
            git_clone_command.extend(['--branch', self.ref])
        git_clone_command.extend([f'https://github.com/{self.org}/{self.repo}', checkout_dir])
        git_clone_process = await create_subprocess_exec(*git_clone_command, env={'GIT_TERMINAL_PROMPT': '0'})
        status_code = await git_clone_process.wait()
        logger.debug(f'Finished cloning {self.org}/{self.repo} with status {status_code}')
        yield (await self._get_commit(checkout_dir))
...
async def check(*, ruff: Path, path: Path, name: str, select: str='', ignore: str='', exclude: str='', show_fixes: bool=False) -> Sequence[str]:
    ...
    start = time.time()
    proc = await create_subprocess_exec(ruff.absolute(), *ruff_args, '.', stdout=PIPE, stderr=PIPE, cwd=path)
    (result, err) = await proc.communicate()
    end = time.time()
    logger.debug(f'Finished checking {name} with {ruff} in {end - start:.2f}')
    if proc.returncode != 0:
        raise RuffError(err.decode('utf8'))
    lines = [line for line in result.decode('utf8').splitlines() if not SUMMARY_LINE_RE.match(line)]
    return sorted(lines)
...
async def compare(ruff1: Path, ruff2: Path, repo: Repository, checkouts: Path | None=None) -> Diff | None:
        ...
        async with repo.clone(checkout_dir) as checkout_sha:
            try:
                async with asyncio.TaskGroup() as tg:
                    check1 = tg.create_task(check(ruff=ruff1, path=checkout_dir, name=f'{repo.org}/{repo.repo}', select=repo.select, ignore=repo.ignore, exclude=repo.exclude, show_fixes=repo.show_fixes))
                    check2 = tg.create_task(check(ruff=ruff2, path=checkout_dir, name=f'{repo.org}/{repo.repo}', select=repo.select, ignore=repo.ignore, exclude=repo.exclude, show_fixes=repo.show_fixes))
...
def read_projects_jsonl(projects_jsonl: Path) -> dict[tuple[str, str], Repository]:
    ...
    return repositories
DIFF_LINE_RE = re.compile('^(?P<pre>[+-]) (?P<inner>(?P<path>[^:]+):(?P<lnum>\\d+):\\d+:) (?P<post>.*)$')
T = TypeVar('T')

async def main(*, ruff1: Path, ruff2: Path, projects_jsonl: Path | None, checkouts: Path | None=None) -> None:
    """Check two versions of ruff against a corpus of open-source code."""
    if projects_jsonl:
    ...
    results = await asyncio.gather(*[limited_parallelism(compare(ruff1, ruff2, repo, checkouts)) for repo in repositories.values()], return_exceptions=True)
    ...
    if total_removed == 0 and total_added == 0 and (errors == 0):
        ...
        for ((org, repo), diff) in diffs.items():
            if isinstance(diff, Exception):
                ...
                print(f'https://github.com/{repo.org}/{repo.repo} ref {repo.ref} select {repo.select} ignore {repo.ignore} exclude {repo.exclude}')
                print('<p>')
                print()
                print('```')
                print(str(diff))
                ...
                print('</details>')
                for line in diff_lines:
                    matches = re.search(': ([A-Z]{1,4}[0-9]{3,4})', line)
                    if matches is None:
                        continue
        ...
        if len(rule_changes.keys()) > 0:
            ...
            print('| ---- | ------- | --------- | -------- |')
            for (rule, (additions, removals)) in sorted(rule_changes.items(), key=lambda x: x[1][0] + x[1][1], reverse=True):
                print(f'| {rule} | {additions + removals} | {additions} | {removals} |')
    logger.debug(f'Finished {len(repositories)} repositories')
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Check two versions of ruff against a corpus of open-source code.', epilog='scripts/check_ecosystem.py <path/to/ruff1> <path/to/ruff2>')
    parser.add_argument('--projects', type=Path, help='Optional JSON files to use over the default repositories. Supports both github_search_*.jsonl and known-github-tomls.jsonl.')
    parser.add_argument('--checkouts', type=Path, help='Location for the git checkouts, in case you want to save them (defaults to temporary directory)')
    parser.add_argument('-v', '--verbose', action='store_true', help='Activate debug logging')
    parser.add_argument('ruff1', type=Path)
    parser.add_argument('ruff2', type=Path)
    ...
    main_task = asyncio.ensure_future(main(ruff1=args.ruff1, ruff2=args.ruff2, projects_jsonl=args.projects, checkouts=args.checkouts))
