...
class Test_iam_password_policy_reuse_24:
    ...
    def test_iam_password_policy_reuse_prevention_equal_24(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_password_policy_reuse_24.iam_password_policy_reuse_24.iam_client', new=IAM(current_audit_info)):
            from prowler.providers.aws.services.iam.iam_password_policy_reuse_24.iam_password_policy_reuse_24 import iam_password_policy_reuse_24
            check = iam_password_policy_reuse_24()
            result = check.execute()
            assert len(result) == 1
            assert result[0].status == 'PASS'
            assert result[0].status_extended == 'IAM password policy reuse prevention is equal to 24.'
    ...
    def test_iam_password_policy_reuse_prevention_less_24(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_password_policy_reuse_24.iam_password_policy_reuse_24.iam_client', new=IAM(current_audit_info)):
            ...
            assert result[0].status_extended == 'IAM password policy reuse prevention is less than 24 or not set.'
            assert result[0].resource_id == AWS_ACCOUNT_NUMBER
            assert result[0].resource_arn == AWS_ACCOUNT_ARN
            assert result[0].region == AWS_REGION