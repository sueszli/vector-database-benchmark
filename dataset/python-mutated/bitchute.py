...
from ..utils import ExtractorError, OnDemandPagedList, clean_html, get_element_by_class, get_element_by_id, get_elements_html_by_class, int_or_none, orderedSet, parse_count, parse_duration, traverse_obj, unified_strdate, urlencode_postdata

class BitChuteIE(InfoExtractor):
    _VALID_URL = 'https?://(?:www\\.)?bitchute\\.com/(?:video|embed|torrent/[^/]+)/(?P<id>[^/?#&]+)'
    _EMBED_REGEX = [f"""<(?:script|iframe)[^>]+\\bsrc=(["\\'])(?P<url>{_VALID_URL})"""]
    _TESTS = [{'url': 'https://www.bitchute.com/video/UGlrF9o9b-Q/', 'md5': '7e427d7ed7af5a75b5855705ec750e2b', 'info_dict': {'id': 'UGlrF9o9b-Q', 'ext': 'mp4', 'title': 'This is the first video on #BitChute !', 'description': 'md5:a0337e7b1fe39e32336974af8173a034', 'thumbnail': 're:^https?://.*\\.jpg$', 'uploader': 'BitChute', 'upload_date': '20170103'}}, {'url': 'https://www.bitchute.com/video/2s6B3nZjAk7R/', 'md5': '05c12397d5354bf24494885b08d24ed1', 'info_dict': {'id': '2s6B3nZjAk7R', 'ext': 'mp4', 'filesize': 71537926, 'title': 'STYXHEXENHAMMER666 - Election Fraud, Clinton 2020, EU Armies, and Gun Control', 'description': 'md5:228ee93bd840a24938f536aeac9cf749', 'thumbnail': 're:^https?://.*\\.jpg$', 'uploader': 'BitChute', 'upload_date': '20181113'}, 'params': {'check_formats': None}}, {'url': 'https://www.bitchute.com/video/WEnQU7XGcTdl/', 'info_dict': {'id': 'WEnQU7XGcTdl', 'ext': 'mp4', 'title': 'Impartial Truth - Ein Letzter Appell an die Vernunft'}, 'params': {'skip_download': True}, 'skip': 'Georestricted in DE'}, {'url': 'https://www.bitchute.com/embed/lbb5G1hjPhw/', 'only_matching': True}, {'url': 'https://www.bitchute.com/torrent/Zee5BE49045h/szoMrox2JEI.webtorrent', 'only_matching': True}]
    _GEO_BYPASS = False
    _HEADERS = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.57 Safari/537.36', 'Referer': 'https://www.bitchute.com/'}
...
class BitChuteChannelIE(InfoExtractor):
    ...
    _TESTS = [{'url': 'https://www.bitchute.com/channel/bitchute/', 'info_dict': {'id': 'bitchute', 'title': 'BitChute', 'description': 'md5:5329fb3866125afa9446835594a9b138'}, 'playlist': [{'md5': '7e427d7ed7af5a75b5855705ec750e2b', 'info_dict': {'id': 'UGlrF9o9b-Q', 'ext': 'mp4', 'filesize': None, 'title': 'This is the first video on #BitChute !', 'description': 'md5:a0337e7b1fe39e32336974af8173a034', 'thumbnail': 're:^https?://.*\\.jpg$', 'uploader': 'BitChute', 'upload_date': '20170103', 'duration': 16, 'view_count': int}}], 'params': {'skip_download': True, 'playlist_items': '-1'}}, {'url': 'https://www.bitchute.com/playlist/wV9Imujxasw9/', 'playlist_mincount': 20, 'info_dict': {'id': 'wV9Imujxasw9', 'title': 'Bruce MacDonald and "The Light of Darkness"', 'description': 'md5:04913227d2714af1d36d804aa2ab6b1e'}}]
    _TOKEN = 'zyG6tQcGPE5swyAEFLqKUwMuMMuF6IO2DZ6ZDQjGfsL0e4dcTLwqkTTul05Jdve7'
    PAGE_SIZE = 25
    HTML_CLASS_NAMES = {'channel': {'container': 'channel-videos-container', 'title': 'channel-videos-title', 'description': 'channel-videos-text'}, 'playlist': {'container': 'playlist-video', 'title': 'title', 'description': 'description'}}
