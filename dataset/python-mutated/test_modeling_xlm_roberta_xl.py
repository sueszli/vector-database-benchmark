...
if is_torch_available():
    import torch
    from transformers import XLMRobertaXLForCausalLM, XLMRobertaXLForMaskedLM, XLMRobertaXLForMultipleChoice, XLMRobertaXLForQuestionAnswering, XLMRobertaXLForSequenceClassification, XLMRobertaXLForTokenClassification, XLMRobertaXLModel
    from transformers.models.xlm_roberta_xl.modeling_xlm_roberta_xl import XLMRobertaXLEmbeddings, create_position_ids_from_input_ids
...
class XLMRobertaXLModelTester:
    ...
    def get_config(self):
        if False:
            print('Hello World!')
        return XLMRobertaXLConfig(vocab_size=self.vocab_size, hidden_size=self.hidden_size, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, hidden_act=self.hidden_act, hidden_dropout_prob=self.hidden_dropout_prob, attention_probs_dropout_prob=self.attention_probs_dropout_prob, max_position_embeddings=self.max_position_embeddings, type_vocab_size=self.type_vocab_size, initializer_range=self.initializer_range)
...
class XLMRobertaModelXLIntegrationTest(unittest.TestCase):
    ...
    def test_xlm_roberta_xl(self):
        ...
        expected_output_shape = torch.Size((1, 12, 2560))
        expected_output_values_last_dim = torch.tensor([[0.011, 0.0605, 0.0354, 0.0689, 0.0066, 0.0691, 0.0302, 0.0412, 0.086, 0.0036, 0.0405, 0.017]], device=torch_device)
    ...
    def test_xlm_roberta_xxl(self):
        ...
        input_ids = torch.tensor([[0, 581, 10269, 83, 99942, 136, 60742, 23, 70, 80583, 18276, 2]], device=torch_device)
        expected_output_shape = torch.Size((1, 12, 4096))
        expected_output_values_last_dim = torch.tensor([[0.0046, 0.0146, 0.0227, 0.0126, 0.0219, 0.0175, -0.0101, 0.0006, 0.0124, 0.0209, -0.0063, 0.0096]], device=torch_device)
        output = model(input_ids)['last_hidden_state'].detach()
        self.assertEqual(output.shape, expected_output_shape)
        self.assertTrue(torch.allclose(output[:, :, -1], expected_output_values_last_dim, atol=0.001))