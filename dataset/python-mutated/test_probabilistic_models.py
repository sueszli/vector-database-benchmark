...
from darts.models import ARIMA, BATS, TBATS, CatBoostModel, ExponentialSmoothing, LightGBMModel, LinearRegressionModel, NotImportedModule, XGBModel
from darts.tests.conftest import tfm_kwargs
from darts.utils import timeseries_generation as tg
logger = get_logger(__name__)
try:
    import torch
    from darts.models import BlockRNNModel, DLinearModel, NBEATSModel, RNNModel, TCNModel, TFTModel, TiDEModel, TransformerModel
    from darts.models.forecasting.torch_forecasting_model import TorchForecastingModel
    from darts.utils.likelihood_models import BernoulliLikelihood, BetaLikelihood, CauchyLikelihood, ContinuousBernoulliLikelihood, DirichletLikelihood, ExponentialLikelihood, GammaLikelihood, GaussianLikelihood, GeometricLikelihood, GumbelLikelihood, HalfNormalLikelihood, LaplaceLikelihood, LogNormalLikelihood, NegativeBinomialLikelihood, PoissonLikelihood, QuantileRegression, WeibullLikelihood
...
models_cls_kwargs_errs = [(ExponentialSmoothing, {}, 0.3, None), (ARIMA, {'p': 1, 'd': 0, 'q': 1, 'random_state': 42}, 0.03, None)]
models_cls_kwargs_errs += [(BATS, {'use_trend': False, 'use_damped_trend': False, 'use_box_cox': True, 'use_arma_errors': False, 'random_state': 42}, 0.04, None), (TBATS, {'use_trend': False, 'use_damped_trend': False, 'use_box_cox': True, 'use_arma_errors': False, 'random_state': 42}, 0.04, 0.04)]
if TORCH_AVAILABLE:
    models_cls_kwargs_errs += [(RNNModel, {'input_chunk_length': 2, 'training_length': 10, 'n_epochs': 20, 'random_state': 0, 'likelihood': GaussianLikelihood(), **tfm_kwargs}, 0.02, 0.04), (TCNModel, {'input_chunk_length': 10, 'output_chunk_length': 5, 'n_epochs': 60, 'random_state': 0, 'likelihood': GaussianLikelihood(), **tfm_kwargs}, 0.06, 0.05), (BlockRNNModel, {'input_chunk_length': 10, 'output_chunk_length': 5, 'n_epochs': 20, 'random_state': 0, 'likelihood': GaussianLikelihood(), **tfm_kwargs}, 0.03, 0.04), (TransformerModel, {'input_chunk_length': 10, 'output_chunk_length': 5, 'n_epochs': 20, 'random_state': 0, 'likelihood': GaussianLikelihood(), **tfm_kwargs}, 0.03, 0.04), (NBEATSModel, {'input_chunk_length': 10, 'output_chunk_length': 5, 'n_epochs': 10, 'random_state': 0, 'likelihood': GaussianLikelihood(), **tfm_kwargs}, 0.15, 0.26), (TFTModel, {'input_chunk_length': 10, 'output_chunk_length': 5, 'n_epochs': 10, 'random_state': 0, 'add_relative_index': True, 'likelihood': GaussianLikelihood(), **tfm_kwargs}, 0.02, 0.1), (TiDEModel, {'input_chunk_length': 10, 'output_chunk_length': 5, 'n_epochs': 10, 'random_state': 0, 'likelihood': GaussianLikelihood(), **tfm_kwargs}, 0.06, 0.1)]
