...
class Test_ec2_networkacl_allow_ingress_tcp_port_22:
    ...
    def test_ec2_default_nacls(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.ec2.ec2_networkacl_allow_ingress_tcp_port_22.ec2_networkacl_allow_ingress_tcp_port_22.ec2_client', new=EC2(current_audit_info)):
            from prowler.providers.aws.services.ec2.ec2_networkacl_allow_ingress_tcp_port_22.ec2_networkacl_allow_ingress_tcp_port_22 import ec2_networkacl_allow_ingress_tcp_port_22
    ...
    def test_ec2_non_default_compliant_nacl(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.ec2.ec2_networkacl_allow_ingress_tcp_port_22.ec2_networkacl_allow_ingress_tcp_port_22.ec2_client', new=EC2(current_audit_info)):
            ...
            assert result[0].status_extended == f'Network ACL {result[0].resource_id} has SSH port 22 open to the Internet.'
    ...
    def test_ec2_non_compliant_nacl(self):
        ...
        ec2_client.create_network_acl_entry(NetworkAclId=nacl_id, RuleNumber=100, Protocol='6', PortRange={'From': 22, 'To': 22}, RuleAction='allow', Egress=False, CidrBlock='0.0.0.0/0')
        from prowler.providers.aws.services.ec2.ec2_service import EC2
        current_audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.ec2.ec2_networkacl_allow_ingress_tcp_port_22.ec2_networkacl_allow_ingress_tcp_port_22.ec2_client', new=EC2(current_audit_info)):
            from prowler.providers.aws.services.ec2.ec2_networkacl_allow_ingress_tcp_port_22.ec2_networkacl_allow_ingress_tcp_port_22 import ec2_networkacl_allow_ingress_tcp_port_22
            ...
            for nacl in result:
                if nacl.resource_id == nacl_id:
                    ...
                    assert result[0].region in (AWS_REGION, 'eu-west-1')
                    assert result[0].resource_tags == []
                    assert nacl.status_extended == f'Network ACL {nacl_id} has SSH port 22 open to the Internet.'
    ...
    def test_ec2_compliant_nacl(self):
        ...
        vpc_id = ec2_client.create_vpc(CidrBlock='10.0.0.0/16')['Vpc']['VpcId']
        nacl_id = ec2_client.create_network_acl(VpcId=vpc_id)['NetworkAcl']['NetworkAclId']
        ec2_client.create_network_acl_entry(NetworkAclId=nacl_id, RuleNumber=100, Protocol='6', PortRange={'From': 22, 'To': 22}, RuleAction='allow', Egress=False, CidrBlock='10.0.0.2/32')
        from prowler.providers.aws.services.ec2.ec2_service import EC2
        current_audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.ec2.ec2_networkacl_allow_ingress_tcp_port_22.ec2_networkacl_allow_ingress_tcp_port_22.ec2_client', new=EC2(current_audit_info)):
            from prowler.providers.aws.services.ec2.ec2_networkacl_allow_ingress_tcp_port_22.ec2_networkacl_allow_ingress_tcp_port_22 import ec2_networkacl_allow_ingress_tcp_port_22
            ...
            for nacl in result:
                if nacl.resource_id == nacl_id:
                    ...
                    assert result[0].region in (AWS_REGION, 'eu-west-1')
                    assert result[0].resource_tags == []
                    assert nacl.status_extended == f'Network ACL {nacl_id} does not have SSH port 22 open to the Internet.'
                    assert nacl.resource_arn == f'arn:{current_audit_info.audited_partition}:ec2:{AWS_REGION}:{current_audit_info.audited_account}:network-acl/{nacl_id}'
