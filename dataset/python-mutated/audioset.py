...
class AudioSetCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_audioset_perms_global_whitelist_add(self, ctx: commands.Context, *, keyword: str):
        ...
        if exists:
            ...
            return await self.send_embed_msg(ctx, title=_('Whitelist Modified'), description=_('Added `{whitelisted}` to the whitelist.').format(whitelisted=keyword))
    ...
    async def command_audioset_perms_global_whitelist_list(self, ctx: commands.Context):
        ...
        pages = list((discord.Embed(title=_('Global Whitelist'), description=page, colour=embed_colour) for page in pages))
    ...
    async def command_audioset_perms_global_whitelist_clear(self, ctx: commands.Context):
        ...
        return await self.send_embed_msg(ctx, title=_('Whitelist Modified'), description=_('All entries have been removed from the whitelist.'))
    ...
    async def command_audioset_perms_global_whitelist_delete(self, ctx: commands.Context, *, keyword: str):
        ...
        if not exists:
            ...
            return await self.send_embed_msg(ctx, title=_('Whitelist Modified'), description=_('Removed `{whitelisted}` from the whitelist.').format(whitelisted=keyword))
    ...
    async def command_audioset_perms_global_blacklist_add(self, ctx: commands.Context, *, keyword: str):
        ...
        if exists:
            ...
            return await self.send_embed_msg(ctx, title=_('Blacklist Modified'), description=_('Added `{blacklisted}` to the blacklist.').format(blacklisted=keyword))
    ...
    async def command_audioset_perms_global_blacklist_list(self, ctx: commands.Context):
        ...
        for (i, entry) in enumerate(blacklist, 1):
            ...
            if i != total:
                ...
                pages.append(box(text, lang='ini'))
        embed_colour = await ctx.embed_colour()
        pages = list((discord.Embed(title=_('Global Blacklist'), description=page, colour=embed_colour) for page in pages))
        await menu(ctx, pages)
    ...
    async def command_audioset_perms_global_blacklist_clear(self, ctx: commands.Context):
        ...
        return await self.send_embed_msg(ctx, title=_('Blacklist Modified'), description=_('All entries have been removed from the blacklist.'))
    ...
    async def command_audioset_perms_global_blacklist_delete(self, ctx: commands.Context, *, keyword: str):
        ...
        if not exists:
            ...
            return await self.send_embed_msg(ctx, title=_('Blacklist Modified'), description=_('Removed `{blacklisted}` from the blacklist.').format(blacklisted=keyword))
    ...
    async def command_audioset_perms_whitelist_list(self, ctx: commands.Context):
        ...
        for (i, entry) in enumerate(whitelist, 1):
            ...
            if i != total:
                ...
                pages.append(box(text, lang='ini'))
        embed_colour = await ctx.embed_colour()
        pages = list((discord.Embed(title=_('Whitelist'), description=page, colour=embed_colour) for page in pages))
        await menu(ctx, pages)
    ...
    async def command_audioset_perms_blacklist_list(self, ctx: commands.Context):
        ...
        for (i, entry) in enumerate(blacklist, 1):
            ...
            if i != total:
                ...
                pages.append(box(text, lang='ini'))
        embed_colour = await ctx.embed_colour()
        pages = list((discord.Embed(title=_('Blacklist'), description=page, colour=embed_colour) for page in pages))
        await menu(ctx, pages)
    ...
    async def command_audioset_autoplay_toggle(self, ctx: commands.Context):
        ...
        msg = _('Auto-play when queue ends: {true_or_false}.').format(true_or_false=_('Enabled') if not autoplay else _('Disabled'))
    ...
    async def command_audioset_autoplay_playlist(self, ctx: commands.Context, playlist_matches: PlaylistConverter, *, scope_data: ScopeParser=None):
        ...
        **Guild** can be one of the following:
        ...
        if self.playlist_api is None:
            return await self.send_embed_msg(ctx, title=_('Playlists Are Not Available'), description=_('The playlist section of Audio is currently unavailable'), footer=None if not await self.bot.is_owner(ctx.author) else _('Check your logs.'))
        ...
        if playlist is None:
            return await self.send_embed_msg(ctx, title=_('No Playlist Found'), description=_("Could not match '{arg}' to a playlist").format(arg=playlist_arg))
        try:
            tracks = playlist.tracks
            if not tracks:
                return await self.send_embed_msg(ctx, title=_('No Tracks Found'), description=_('Playlist {name} has no tracks.').format(name=playlist.name))
            ...
            return await self.send_embed_msg(ctx, title=_('No Playlist Found'), description=_('Playlist {id} does not exist in {scope} scope.').format(id=playlist_arg, scope=self.humanize_scope(scope, the=True)))
        except MissingGuild:
            return await self.send_embed_msg(ctx, title=_('Missing Arguments'), description=_('You need to specify the Guild ID for the guild to lookup.'))
        else:
            return await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Playlist {name} (`{id}`) [**{scope}**] will be used for autoplay.').format(name=playlist.name, id=playlist.id, scope=self.humanize_scope(scope, ctx=guild if scope == PlaylistScope.GUILD.value else author)))
    ...
    async def command_audioset_autoplay_reset(self, ctx: commands.Context):
        ...
        playlist_data = dict(enabled=True, id=42069, name="Aikaterna's curated tracks", scope=PlaylistScope.GLOBAL.value)
        await self.config.guild(ctx.guild).autoplaylist.set(playlist_data)
        return await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Set auto-play playlist to play recently played tracks.'))
    ...
    async def command_audioset_global_historical_queue(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Global daily queues: {true_or_false}.').format(true_or_false=_('Enabled') if not daily_playlists else _('Disabled')))
    ...
    async def command_audioset_historical_queue(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Daily queues: {true_or_false}.').format(true_or_false=_('Enabled') if not daily_playlists else _('Disabled')))
    ...
    async def command_audioset_dc(self, ctx: commands.Context):
        ...
        autoplay = await self.config.guild(ctx.guild).auto_play()
        msg = ''
        msg += _('Auto-disconnection at queue end: {true_or_false}.').format(true_or_false=_('Enabled') if not disconnect else _('Disabled'))
    ...
    async def command_audioset_dj(self, ctx: commands.Context):
        ...
        if dj_role is None:
            await self.send_embed_msg(ctx, title=_('Missing DJ Role'), description=_('Please set a role to use with DJ mode. Enter the role name or ID now.'))
            try:
                ...
                return await self.send_embed_msg(ctx, title=_('Response timed out, try again later.'))
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('DJ role: {true_or_false}.').format(true_or_false=_('Enabled') if not dj_enabled else _('Disabled')))
    ...
    async def command_audioset_emptydisconnect(self, ctx: commands.Context, seconds: int):
        ...
        """
        if seconds < 0:
            return await self.send_embed_msg(ctx, title=_('Invalid Time'), description=_("Seconds can't be less than zero."))
        if 10 > seconds > 0:
            seconds = 10
        if seconds == 0:
            enabled = False
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Empty disconnect disabled.'))
        else:
            enabled = True
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Empty disconnect timer set to {num_seconds}.').format(num_seconds=self.get_time_string(seconds)))
    ...
    async def command_audioset_emptypause(self, ctx: commands.Context, seconds: int):
        ...
        if seconds == 0:
            enabled = False
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Empty pause disabled.'))
        else:
            enabled = True
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Empty pause timer set to {num_seconds}.').format(num_seconds=self.get_time_string(seconds)))
    ...
    async def command_audioset_lyrics(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Prefer tracks with lyrics: {true_or_false}.').format(true_or_false=_('Enabled') if not prefer_lyrics else _('Disabled')))
    ...
    async def command_audioset_jukebox(self, ctx: commands.Context, price: int):
        ...
        if price < 0:
            return await self.send_embed_msg(ctx, title=_('Invalid Price'), description=_("Price can't be less than zero."))
        if price == 0:
            jukebox = False
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Jukebox mode disabled.'))
        else:
            jukebox = True
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Track queueing command price set to {price} {currency}.').format(price=humanize_number(price), currency=await bank.get_currency_name(ctx.guild)))
    ...
    async def command_audioset_localpath(self, ctx: commands.Context, *, local_path=None):
        ...
        if not local_path:
            ...
            return await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('The localtracks path location has been reset to {localpath}').format(localpath=str(cog_data_path(raw_name='Audio').absolute())))
        info_msg = _("This setting is only for bot owners to set a localtracks folder location In the example below, the full path for 'ParentDirectory' must be passed to this command.\n```\nParentDirectory\n  |__ localtracks  (folder)\n  |     |__ Awesome Album Name  (folder)\n  |           |__01 Cool Song.mp3\n  |           |__02 Groovy Song.mp3\n```\nThe folder path given to this command must contain the localtracks folder.\n**This folder and files need to be visible to the user where `Lavalink.jar` is being run from.**\nUse this command with no path given to reset it to the default, the Audio data directory for this bot.\nDo you want to continue to set the provided path for local tracks?")
        ...
        if not temp.exists() or not temp.is_dir():
            return await self.send_embed_msg(ctx, title=_('Invalid Path'), description=_('{local_path} does not seem like a valid path.').format(local_path=local_path))
        if not temp.localtrack_folder.exists():
            warn_msg = _('`{localtracks}` does not exist. The path will still be saved, but please check the path and create a localtracks folder in `{localfolder}` before attempting to play local tracks.').format(localfolder=temp.absolute(), localtracks=temp.localtrack_folder.absolute())
        ...
        return await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('The localtracks path location has been set to {localpath}').format(localpath=local_path))
    ...
    async def command_audioset_maxlength(self, ctx: commands.Context, seconds: Union[int, str]):
        ...
        if seconds < 0:
            return await self.send_embed_msg(ctx, title=_('Invalid length'), description=_("Length can't be less than zero."))
        if seconds == 0:
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Track max length disabled.'))
        else:
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Track max length set to {seconds}.').format(seconds=self.get_time_string(seconds)))
    ...
    async def command_audioset_notify(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Notify mode: {true_or_false}.').format(true_or_false=_('Enabled') if not notify else _('Disabled')))
    ...
    async def command_audioset_auto_deafen(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Auto Deafen: {true_or_false}.').format(true_or_false=_('Enabled') if not auto_deafen else _('Disabled')))
    ...
    async def command_audioset_restrict(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Commercial links only: {true_or_false}.').format(true_or_false=_('Enabled') if not restrict else _('Disabled')))
    ...
    async def command_audioset_role(self, ctx: commands.Context, *, role_name: discord.Role):
        ...
        await self.send_embed_msg(ctx, title=_('Settings Changed'), description=_('DJ role set to: {role.name}.').format(role=dj_role_obj))
    ...
    async def command_audioset_settings(self, ctx: commands.Context):
        ...
        msg += _('Auto-play:        [{autoplay}]\n').format(autoplay=_('Enabled') if autoplay else _('Disabled'))
        if emptydc_enabled:
        ...
        msg += _('Max volume:       [{max_volume}%]\nPersist queue:    [{persist_queue}]\nRepeat:           [{repeat}]\nShuffle:          [{shuffle}]\nShuffle bumped:   [{bumpped_shuffle}]\nSong notify msgs: [{notify}]\nSongs as status:  [{status}]\nSpotify search:   [{countrycode}]\n').format(max_volume=maxvolume, countrycode=countrycode, persist_queue=persist_queue, repeat=song_repeat, shuffle=song_shuffle, notify=song_notify, status=song_status, bumpped_shuffle=bumpped_shuffle)
        if thumbnail:
            msg += _('Thumbnails:       [{0}]\n').format(_('Enabled') if thumbnail else _('Disabled'))
        if vote_percent > 0:
            msg += _('Vote skip:        [{vote_enabled}]\nSkip percentage:  [{vote_percent}%]\n').format(vote_percent=vote_percent, vote_enabled=_('Enabled') if vote_enabled else _('Disabled'))
    ...
    async def command_audioset_logs(self, ctx: commands.Context):
        ...
        try:
            ...
            if check(logs):
                ...
                if check(zip_name):
                    await ctx.send(_('Logs are too large, you can find them in {path}').format(path=zip_name.absolute()))
                    zip_name = None
                else:
    ...
    async def command_audioset_status(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Song titles as status: {true_or_false}.').format(true_or_false=_('Enabled') if not status else _('Disabled')))
    ...
    async def command_audioset_thumbnail(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Thumbnail display: {true_or_false}.').format(true_or_false=_('Enabled') if not thumbnail else _('Disabled')))
    ...
    async def command_audioset_vote(self, ctx: commands.Context, percent: int):
        ...
        if percent == 0:
            enabled = False
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Voting disabled. All users can use queue management commands.'))
        else:
            enabled = True
            await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Vote percentage set to {percent}%.').format(percent=percent))
    ...
    async def command_audioset_youtubeapi(self, ctx: commands.Context):
        ...
        message = _(f'1. Go to Google Developers Console and log in with your Google account.\n(https://console.developers.google.com/)\n2. You should be prompted to create a new project (name does not matter).\n3. Click on Enable APIs and Services at the top.\n4. In the list of APIs choose or search for YouTube Data API v3 and click on it. Choose Enable.\n5. Click on Credentials on the left navigation bar.\n6. Click on Create Credential at the top.\n7. At the top click the link for "API key".\n8. No application restrictions are needed. Click Create at the bottom.\n9. You now have a key to add to `{{prefix}}set api youtube api_key <your_api_key_here>`').format(prefix=ctx.prefix)
    ...
    async def command_audioset_spotifyapi(self, ctx: commands.Context):
        ...
        message = _('1. Go to Spotify developers and log in with your Spotify account.\n(https://developer.spotify.com/dashboard/applications)\n2. Click "Create An App".\n3. Fill out the form provided with your app name, etc.\n4. When asked if you\'re developing commercial integration select "No".\n5. Accept the terms and conditions.\n6. Copy your client ID and your client secret into:\n`{prefix}set api spotify client_id <your_client_id_here> client_secret <your_client_secret_here>`').format(prefix=ctx.prefix)
        await ctx.maybe_send_embed(message)
    ...
    async def command_audioset_countrycode(self, ctx: commands.Context, country: str):
        ...
        if len(country) != 2:
            return await self.send_embed_msg(ctx, title=_('Invalid Country Code'), description=_('Please use an official [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) code.'))
        country = country.upper()
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Country Code set to {country}.').format(country=country))
    ...
    async def command_audioset_cacheage(self, ctx: commands.Context, age: int):
        ...
        msg = ''
        if age < 7:
            msg = _('Cache age cannot be less than 7 days. If you wish to disable it run {prefix}audioset cache.\n').format(prefix=ctx.prefix)
            age = 7
    ...
    async def command_audioset_persist_queue(self, ctx: commands.Context):
        ...
        await self.send_embed_msg(ctx, title=_('Setting Changed'), description=_('Persisting queues: {true_or_false}.').format(true_or_false=_('Enabled') if not persist_cache else _('Disabled')))
    ...
    async def command_audioset_restart(self, ctx: commands.Context):
        ...
        async with ctx.typing():
            try:
                ...
                await self.send_embed_msg(ctx, title=_('Failed To Shutdown Lavalink Node'), description=_('Please reload Audio (`{prefix}reload audio`).').format(prefix=ctx.prefix))
            await self.send_embed_msg(ctx, title=_('Restarting Lavalink'), description=_('It can take a couple of minutes for Lavalink to fully start up.'))
    ...
    async def command_audioset_maxvolume(self, ctx: commands.Context, max_volume: int):
        ...
        if max_volume < 1:
            return await self.send_embed_msg(ctx, title=_('Error'), description=_("Music without sound isn't music at all. Try setting the volume higher then 0%."))
        elif max_volume > 150:
            max_volume = 150
            await self.send_embed_msg(ctx, title=_('Setting changed'), description=_('The maximum volume has been limited to 150%, be easy on your ears.'))
        else:
            await self.send_embed_msg(ctx, title=_('Setting changed'), description=_('The maximum volume has been limited to {max_volume}%.').format(max_volume=max_volume))
