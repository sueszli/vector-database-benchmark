...
from azure.core.exceptions import AzureError, ServiceRequestError, ServiceRequestTimeoutError, ServiceResponseError, ServiceResponseTimeoutError
...
async def test_retry_seekable_file(http_request):
    ...
    with open(file.name, 'rb') as f:
        form_data_content = {'fileContent': f, 'fileName': f.name}
...
async def test_retry_timeout(http_request):
    ...
    transport = Mock(spec=AsyncHttpTransport, send=Mock(wraps=send), connection_config=ConnectionConfiguration(connection_timeout=timeout * 2), sleep=asyncio.sleep)
...
async def test_timeout_defaults(http_request):
    ...
    async def send(request, **kwargs):
        ...
        return response
    transport = Mock(spec_set=AsyncHttpTransport, send=Mock(wraps=send), sleep=Mock(side_effect=Exception('policy should not sleep: its first send succeeded')))
...
async def test_does_not_sleep_after_timeout(combinations, http_request):
    ...
    timeout = 1
    transport = Mock(spec=AsyncHttpTransport, send=Mock(side_effect=transport_error('oops')), sleep=Mock(wraps=asyncio.sleep))
    pipeline = AsyncPipeline(transport, [AsyncRetryPolicy(timeout=timeout)])
