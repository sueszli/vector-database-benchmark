...
if is_torch_available():
    import torch
    from transformers import GPT2Tokenizer, OPTForCausalLM, OPTForQuestionAnswering, OPTForSequenceClassification, OPTModel
...
class OPTModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (OPTModel, OPTForCausalLM, OPTForSequenceClassification, OPTForQuestionAnswering) if is_torch_available() else ()
    all_generative_model_classes = (OPTForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': OPTModel, 'question-answering': OPTForQuestionAnswering, 'text-classification': OPTForSequenceClassification, 'text-generation': OPTForCausalLM, 'zero-shot': OPTForSequenceClassification} if is_torch_available() else {}
