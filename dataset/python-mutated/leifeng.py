...
def onQQMessage(bot, contact, member, content):
    ...
    if content == '--stop':
        ...
        if '小V' in str(member) or '小v' in str(member) or '瓜大电视台' in str(member) or ('QQ小冰' in str(member)):
            return
        if getattr(member, 'uin', None) == bot.conf.qq:
            return ()
        else:
            QAfile = open('/home/qqbot/qa.txt', 'r')
            strQA = QAfile.read()
            stQA = strQA.split('Q')
            QAfile.close()
            tmp = 0
            if not content == '' and tmp < 6 and (len(content) > 3) and (member != None) and (len(content) < 30) and (not content == '没有') and (not content == '西工大') and (not content == '可以') and ('问' in content or '知道' in content or '？' in content):
