...
class FQFPolicy(DQNPolicy):
    ...
    Config:
        ...
        == ==================== ======== ============== ======================================== =======================
    """
    config = dict(type='fqf', cuda=False, on_policy=False, priority=False, discount_factor=0.97, nstep=1, learn=dict(update_per_collect=3, batch_size=64, learning_rate_fraction=2.5e-09, learning_rate_quantile=5e-05, target_update_freq=100, kappa=1.0, ent_coef=0, ignore_done=False), collect=dict(unroll_len=1), eval=dict(), other=dict(eps=dict(type='exp', start=0.95, end=0.1, decay=10000), replay_buffer=dict(replay_buffer_size=10000)))
