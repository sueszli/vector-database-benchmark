...
async def test_iterate_streaming_request():
    events = iter(({'type': 'http.request', 'body': b'Hello, ', 'more_body': True}, {'type': 'http.request', 'body': b'World', 'more_body': True}, {'type': 'http.request', 'body': b'!\n', 'more_body': True}, {'type': 'http.request', 'body': b'', 'more_body': False}, {'type': 'http.disconnect'}))

    async def receive():
        event = next(events)
        assert event['type'] != 'http.disconnect', 'would hang until the client times out'
        return event
