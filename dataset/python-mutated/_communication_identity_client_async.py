...
from .._generated.aio._client import CommunicationIdentityClient as CommunicationIdentityClientGen
...
class CommunicationIdentityClient:
    ...
    def __init__(self, endpoint: str, credential: Union[AsyncTokenCredential, AzureKeyCredential], **kwargs) -> None:
        ...
        self._identity_service_client = CommunicationIdentityClientGen(self._endpoint, api_version=self._api_version, authentication_policy=get_authentication_policy(endpoint, credential, decode_url=True, is_async=True), sdk_moniker=SDK_MONIKER, **kwargs)
    ...
    @distributed_trace_async
    async def create_user_and_token(self, scopes: List[Union[str, 'CommunicationTokenScope']], **kwargs) -> Tuple['CommunicationUserIdentifier', AccessToken]:
        ...
        identity_access_token = await self._identity_service_client.communication_identity.create(body=request_body, **kwargs)
    ...
    @distributed_trace_async
    async def get_token(self, user: CommunicationUserIdentifier, scopes: List[Union[str, 'CommunicationTokenScope']], **kwargs) -> AccessToken:
        ...
        access_token = await self._identity_service_client.communication_identity.issue_access_token(user.properties['id'], body=request_body, **kwargs)
    ...
    async def revoke_tokens(self, user: CommunicationUserIdentifier, **kwargs) -> None:
        ...
        :rtype: None
        """
        return await self._identity_service_client.communication_identity.revoke_access_tokens(user.properties['id'] if user else None, **kwargs)

    @distributed_trace_async
    async def get_token_for_teams_user(self, aad_token, client_id, user_object_id, **kwargs):
        ...
        access_token = await self._identity_service_client.communication_identity.exchange_teams_user_access_token(body=request_body, **kwargs)
        return AccessToken(access_token.token, access_token.expires_on)
