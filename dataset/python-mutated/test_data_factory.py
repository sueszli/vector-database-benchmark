...
import pytest
from airflow.providers.microsoft.azure.triggers.data_factory import ADFPipelineRunStatusSensorTrigger, AzureDataFactoryPipelineRunStatus, AzureDataFactoryTrigger
...
class TestADFPipelineRunStatusSensorTrigger:
    TRIGGER = ADFPipelineRunStatusSensorTrigger(run_id=RUN_ID, azure_data_factory_conn_id=AZURE_DATA_FACTORY_CONN_ID, resource_group_name=RESOURCE_GROUP_NAME, factory_name=DATAFACTORY_NAME, poke_interval=POKE_INTERVAL)
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('mock_status', ['Queued'])
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('mock_status', ['InProgress'])
    @mock.patch(f'{MODULE}.hooks.data_factory.AzureDataFactoryAsyncHook.get_adf_pipeline_run_status')
    async def test_adf_pipeline_run_status_sensors_trigger_run_inprogress(self, mock_data_factory, mock_status):
        """
        Test if the task is run is in trigger successfully.
        """
        mock_data_factory.return_value = mock_status
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('mock_status', ['Succeeded'])
    @mock.patch(f'{MODULE}.hooks.data_factory.AzureDataFactoryAsyncHook.get_adf_pipeline_run_status')
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('mock_status, mock_message', [('Failed', f'Pipeline run {RUN_ID} has Failed.')])
    @mock.patch(f'{MODULE}.hooks.data_factory.AzureDataFactoryAsyncHook.get_adf_pipeline_run_status')
    async def test_adf_pipeline_run_status_sensors_trigger_failed(self, mock_data_factory, mock_status, mock_message):
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('mock_status, mock_message', [('Cancelled', f'Pipeline run {RUN_ID} has been Cancelled.')])
    @mock.patch(f'{MODULE}.hooks.data_factory.AzureDataFactoryAsyncHook.get_adf_pipeline_run_status')
    async def test_adf_pipeline_run_status_sensors_trigger_cancelled(self, mock_data_factory, mock_status, mock_message):
        """Test if the task is run is in trigger failure status."""
        mock_data_factory.return_value = mock_status
        generator = self.TRIGGER.run()
        actual = await generator.asend(None)
    ...
    @mock.patch(f'{MODULE}.hooks.data_factory.AzureDataFactoryAsyncHook.get_adf_pipeline_run_status')
    async def test_adf_pipeline_run_status_sensors_trigger_exception(self, mock_data_factory, mock_refresh_token):
...
class TestAzureDataFactoryTrigger:
    TRIGGER = AzureDataFactoryTrigger(run_id=AZ_PIPELINE_RUN_ID, resource_group_name=AZ_RESOURCE_GROUP_NAME, factory_name=AZ_FACTORY_NAME, azure_data_factory_conn_id=AZ_DATA_FACTORY_CONN_ID, end_time=AZ_PIPELINE_END_TIME)
    ...
    async def test_azure_data_factory_trigger_run_without_wait(self, mock_pipeline_run_status):
        ...
        trigger = AzureDataFactoryTrigger(run_id=AZ_PIPELINE_RUN_ID, resource_group_name=AZ_RESOURCE_GROUP_NAME, factory_name=AZ_FACTORY_NAME, azure_data_factory_conn_id=AZ_DATA_FACTORY_CONN_ID, wait_for_termination=False, end_time=AZ_PIPELINE_END_TIME)
        generator = trigger.run()
        actual = await generator.asend(None)
        expected = TriggerEvent({'status': 'success', 'message': f'The pipeline run {AZ_PIPELINE_RUN_ID} has {AzureDataFactoryPipelineRunStatus.SUCCEEDED} status.', 'run_id': AZ_PIPELINE_RUN_ID})
    ...
    async def test_azure_data_factory_trigger_run_success(self, mock_pipeline_run_status):
        ...
        generator = self.TRIGGER.run()
        actual = await generator.asend(None)
        expected = TriggerEvent({'status': 'success', 'message': f'The pipeline run {AZ_PIPELINE_RUN_ID} has {AzureDataFactoryPipelineRunStatus.SUCCEEDED}.', 'run_id': AZ_PIPELINE_RUN_ID})
    ...
    async def test_azure_data_factory_trigger_run_failed(self, mock_pipeline_run_status):
        ...
        generator = self.TRIGGER.run()
        actual = await generator.asend(None)
        expected = TriggerEvent({'status': 'error', 'message': f'The pipeline run {AZ_PIPELINE_RUN_ID} has {AzureDataFactoryPipelineRunStatus.FAILED}.', 'run_id': AZ_PIPELINE_RUN_ID})
        assert expected == actual
    ...
    async def test_azure_data_factory_trigger_run_cancelled(self, mock_pipeline_run_status):
        ...
        generator = self.TRIGGER.run()
        actual = await generator.asend(None)
        expected = TriggerEvent({'status': 'error', 'message': f'The pipeline run {AZ_PIPELINE_RUN_ID} has {AzureDataFactoryPipelineRunStatus.CANCELLED}.', 'run_id': AZ_PIPELINE_RUN_ID})
        assert expected == actual

    @pytest.mark.asyncio
    @mock.patch(f'{MODULE}.hooks.data_factory.AzureDataFactoryAsyncHook.cancel_pipeline_run')
    @mock.patch(f'{MODULE}.hooks.data_factory.AzureDataFactoryAsyncHook.get_adf_pipeline_run_status')
    async def test_azure_data_factory_trigger_run_exception(self, mock_pipeline_run_status, mock_cancel_pipeline_run):
        ...
        task = [i async for i in self.TRIGGER.run()]
        response = TriggerEvent({'status': 'error', 'message': 'Test exception', 'run_id': AZ_PIPELINE_RUN_ID})
        assert len(task) == 1
    ...
    async def test_azure_data_factory_trigger_run_timeout(self, mock_pipeline_run_status):
        ...
        mock_pipeline_run_status.return_value = AzureDataFactoryPipelineRunStatus.QUEUED
        trigger = AzureDataFactoryTrigger(run_id=AZ_PIPELINE_RUN_ID, resource_group_name=AZ_RESOURCE_GROUP_NAME, factory_name=AZ_FACTORY_NAME, azure_data_factory_conn_id=AZ_DATA_FACTORY_CONN_ID, end_time=time.time())
        generator = trigger.run()
        actual = await generator.asend(None)
        expected = TriggerEvent({'status': 'error', 'message': f'Timeout: The pipeline run {AZ_PIPELINE_RUN_ID} has {AzureDataFactoryPipelineRunStatus.QUEUED}.', 'run_id': AZ_PIPELINE_RUN_ID})
        assert expected == actual