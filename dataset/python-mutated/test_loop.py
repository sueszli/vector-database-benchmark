...
async def test_fold_collect_future_result(subtests):
    ...
    containers: List[Tuple[Iterable[FutureResult[int, str]], FutureResult[int, str]]] = [([], FutureSuccess(10)), ([FutureSuccess(1)], FutureSuccess(11)), ([FutureSuccess(1), FutureSuccess(2)], FutureSuccess(13)), ([FutureFailure('a'), FutureSuccess(1), FutureSuccess(2)], FutureFailure('a')), ([FutureFailure('a'), FutureFailure('b')], FutureFailure('a')), ([FutureSuccess(1), FutureFailure('a')], FutureFailure('a'))]
    for (iterable, sequence) in containers:
        with subtests.test(iterable=iterable, sequence=sequence):
            assert await Fold.loop(iterable, sequence.from_value(10), _sum_two) == await sequence
