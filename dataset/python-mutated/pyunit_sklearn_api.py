...
sklearn_transformer_methods = ['fit', 'fit_transform', 'transform', 'inverse_transform', 'get_params', 'set_params']
    ...
    for cl_name in class_names:
        for name in _make_transformer_names(cl_name):
            ...
            for meth in sklearn_transformer_methods:
                assert _has_method(cls, meth), 'Class {} is missing method {}'.format(name, meth)
            _check_exposed_in_h2o_sklearn_module(cls)
pyunit_utils.run_tests([test_automl_estimators_exposed_in_h2o_sklearn_automl_module, test_algos_estimators_exposed_in_h2o_sklearn_estimators_module, test_transformers_exposed_in_h2o_sklean_transforms_module])