...
class TestFunctionalConv2D(TestCase):
    ...
    def static_graph_case_2(self):
        ...
        with base.unique_name.guard():
            with base.program_guard(main, start):
                ...
                y = F.conv2d(x, weight, None if self.no_bias else bias, padding=self.padding, stride=self.stride, dilation=self.dilation, groups=self.groups, data_format=self.data_format)
                if self.act == 'sigmoid':
                    y = F.sigmoid(y)
        exe = base.Executor(self.place)
        exe.run(start)
    ...
    def dygraph_case(self):
        ...
        with dg.guard(self.place):
            ...
            y = F.conv2d(x, weight, bias, padding=self.padding, stride=self.stride, dilation=self.dilation, groups=self.groups, data_format=self.data_format)
            if self.act == 'sigmoid':
                y = F.sigmoid(y)
            out = y.numpy()
        return out
...
class TestFunctionalConv2DErrorCase12(TestCase):
    ...
    def dygraph_case(self):
        ...
        with dg.guard():
            ...
            y = F.conv2d(x, w, b, padding=self.padding, stride=self.stride, dilation=self.dilation, groups=self.groups, data_format=self.data_format)
