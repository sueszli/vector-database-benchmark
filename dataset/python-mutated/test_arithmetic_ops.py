...
bin_input_kinds = list_product(['cpu', 'gpu'], all_input_kinds) + list_product(['cpu_scalar', 'gpu_scalar', 'const'], ['cpu', 'gpu'])
...
integer_types = [np.bool_, np.int8, np.int16, np.int32, np.int64, np.uint8, np.uint16, np.uint32, np.uint64]
...
selected_bin_input_kinds = [('cpu', 'cpu'), ('gpu', 'gpu'), ('cpu', 'cpu_scalar'), ('gpu', 'gpu_scalar'), ('const', 'cpu'), ('const', 'gpu')]
selected_ternary_input_kinds = [('cpu', 'cpu', 'cpu'), ('gpu', 'gpu', 'gpu'), ('cpu', 'const', 'const'), ('gpu', 'const', 'const'), ('gpu', 'cpu', 'cpu_scalar'), ('cpu_scalar', 'cpu_scalar', 'cpu_scalar')]
bench_ternary_input_kinds = [('cpu', 'cpu', 'cpu'), ('gpu', 'gpu', 'gpu'), ('cpu', 'const', 'const'), ('gpu', 'const', 'const'), ('cpu', 'cpu', 'const'), ('gpu', 'gpu', 'const')]
...
math_function_operations = [(lambda x: math.sqrt(x), lambda x: np.sqrt(x), 'sqrt', pos_range, 1e-06), (lambda x: math.rsqrt(x), lambda x: 1.0 / np.sqrt(x), 'rsqrt', pos_range, 1e-05), (lambda x: math.cbrt(x), lambda x: np.cbrt(x), 'cbrt', default_range, 1e-06), (lambda x: math.exp(x), lambda x: np.exp(x), 'exp', limited_range, 1e-06), (lambda x: math.log(x), lambda x: np.log(x), 'log', pos_range, 1e-06), (lambda x: math.log2(x), lambda x: np.log2(x), 'log2', pos_range, 1e-06), (lambda x: math.log10(x), lambda x: np.log10(x), 'log10', pos_range, 1e-06), (lambda x: math.fabs(x), lambda x: np.fabs(x), 'fabs', default_range, 1e-06), (lambda x: math.floor(x), lambda x: np.floor(x), 'floor', default_range, 1e-06), (lambda x: math.ceil(x), lambda x: np.ceil(x), 'ceil', default_range, 1e-06), (lambda x: math.sin(x), lambda x: np.sin(x), 'sin', default_range, 1e-06), (lambda x: math.cos(x), lambda x: np.cos(x), 'cos', default_range, 1e-06), (lambda x: math.tan(x), lambda x: np.tan(x), 'tan', default_range, 1e-06), (lambda x: math.asin(x), lambda x: np.arcsin(x), 'asin', one_range, 1e-06), (lambda x: math.acos(x), lambda x: np.arccos(x), 'acos', one_range, 1e-06), (lambda x: math.atan(x), lambda x: np.arctan(x), 'atan', default_range, 1e-06), (lambda x: math.sinh(x), lambda x: np.sinh(x), 'sinh', default_range, 1e-06), (lambda x: math.cosh(x), lambda x: np.cosh(x), 'cosh', default_range, 1e-06), (lambda x: math.tanh(x), lambda x: np.tanh(x), 'tanh', default_range, 1e-06), (lambda x: math.asinh(x), lambda x: np.arcsinh(x), 'asinh', limited_range, 1e-06), (lambda x: math.acosh(x), lambda x: np.arccosh(x), 'acosh', pos_range, 1e-06), (lambda x: math.atanh(x), lambda x: np.arctanh(x), 'atanh', one_range, 1e-06)]
sane_operations = [(lambda x, y: x + y, '+', default_range), (lambda x, y: x - y, '-', default_range), (lambda x, y: x * y, '*', default_range), ((lambda x, y: x ** y, sane_pow), '**', pow_range), ((lambda x, y: math.pow(x, y), sane_pow), 'pow', pow_range), ((lambda x, y: math.min(x, y), lambda x, y: np.minimum(x, y)), 'min', default_range), ((lambda x, y: math.max(x, y), lambda x, y: np.maximum(x, y)), 'max', default_range)]
floaty_operations = [((lambda x, y: x / y, lambda x, y: x / y), '/', default_range), ((lambda x, y: math.fpow(x, y), sane_pow), 'fpow', pow_range), ((lambda x, y: math.atan2(x, y), lambda x, y: np.arctan2(x, y)), 'atan2', default_range)]
bitwise_operations = [(lambda x, y: x & y, '&'), (lambda x, y: x | y, '|'), (lambda x, y: x ^ y, '^')]
comparisons_operations = [(lambda x, y: x == y, '=='), (lambda x, y: x != y, '!='), (lambda x, y: x < y, '<'), (lambda x, y: x <= y, '<='), (lambda x, y: x > y, '>'), (lambda x, y: x >= y, '>=')]
...
def check_raises_te(kinds, types, op, shape, _):
    if False:
        print('Hello World!')
    check_raises(kinds, types, op, shape)
bool_disallowed = [(lambda x, y: x + y, '+'), (lambda x, y: x - y, '-'), (lambda x, y: x / y, '/'), (lambda x, y: x / y, '//'), (lambda x, y: x ** y, '**')]
