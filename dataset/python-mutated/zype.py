...
from ..utils import dict_get, ExtractorError, int_or_none, js_to_json, parse_iso8601

class ZypeIE(InfoExtractor):
    _ID_RE = '[\\da-fA-F]+'
    _COMMON_RE = '//player\\.zype\\.com/embed/%s\\.(?:js|json|html)\\?.*?(?:access_token|(?:ap[ip]|player)_key)='
    _VALID_URL = 'https?:%s[^&]+' % (_COMMON_RE % ('(?P<id>%s)' % _ID_RE))
    _EMBED_REGEX = [f"""<script[^>]+\\bsrc=(["\\'])(?P<url>(?:https?:)?{_COMMON_RE % _ID_RE}.+?)\\1"""]
    _TEST = {'url': 'https://player.zype.com/embed/5b400b834b32992a310622b9.js?api_key=jZ9GUhRmxcPvX7M3SlfejB6Hle9jyHTdk2jVxG7wOHPLODgncEKVdPYBhuz9iWXQ&autoplay=false&controls=true&da=false', 'md5': 'eaee31d474c76a955bdaba02a505c595', 'info_dict': {'id': '5b400b834b32992a310622b9', 'ext': 'mp4', 'title': 'Smoky Barbecue Favorites', 'thumbnail': 're:^https?://.*\\.jpe?g', 'description': 'md5:5ff01e76316bd8d46508af26dc86023b', 'timestamp': 1504915200, 'upload_date': '20170909'}}
