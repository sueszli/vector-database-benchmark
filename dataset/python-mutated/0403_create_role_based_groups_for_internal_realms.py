...
def create_role_based_system_groups_for_internal_realms(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    if False:
        return 10
...
def delete_role_based_system_groups_for_internal_realms(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    if False:
        for i in range(10):
            print('nop')
    UserGroup = apps.get_model('zerver', 'UserGroup')
    ...
    with transaction.atomic():
        ...
        UserGroupMembership.objects.filter(user_group__realm=realm, user_group__is_system_group=True).delete()
...
class Migration(migrations.Migration):
    ...
    """
    dependencies = [('zerver', '0402_alter_usertopic_visibility_policy')]
    operations = [migrations.RunPython(create_role_based_system_groups_for_internal_realms, reverse_code=delete_role_based_system_groups_for_internal_realms, elidable=True)]