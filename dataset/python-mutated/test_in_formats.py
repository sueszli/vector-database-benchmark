...
async def test_urlencoded_ctype(http_client, client_headers, content_type_map):
    ...
    with patch('salt.client.LocalClient.run_job', return_value={'jid': '20131219215650131543', 'minions': ['jerry']}):
        response = await http_client.fetch('/', method='POST', body=body, headers=client_headers)
    assert response.code == 200
