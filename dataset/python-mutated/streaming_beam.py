...
SCHEMA = ','.join(['url:STRING', 'num_reviews:INTEGER', 'score:FLOAT64', 'first_date:TIMESTAMP', 'last_date:TIMESTAMP'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_table', help='Output BigQuery table for results specified as: PROJECT:DATASET.TABLE or DATASET.TABLE.')
    parser.add_argument('--input_subscription', help='Input PubSub subscription of the form "projects/<PROJECT>/subscriptions/<SUBSCRIPTION>."')
    parser.add_argument('--window_interval_sec', default=60, type=int, help='Window interval in seconds for grouping incoming messages.')
    (args, beam_args) = parser.parse_known_args()
    run(input_subscription=args.input_subscription, output_table=args.output_table, window_interval_sec=args.window_interval_sec, beam_args=beam_args)