...
def test_define_dataset():
    ...
    numGamCols = 3
    seed = 54321
    assert numGamCols <= ncol * realFrac, 'Number of real columns {0} should exceed the number of gam columns {1}'.format(ncol * realFrac, numGamCols)
    gamDataSet = generate_dataset(family, nrow, ncol, realFrac, intFrac, enumFrac, missing_fraction, factorRange, numericRange, targetFactor, numGamCols, seed)
    assert gamDataSet.nrow == nrow, 'Dataset number of row: {0}, expected number of row: {1}'.format(gamDataSet.nrow, nrow)
    assert gamDataSet.ncol == 1 + ncol, 'Dataset number of row: {0}, expected number of row: {1}'.format(gamDataSet.ncol, 1 + ncol)

def generate_dataset(family, nrow, ncol, realFrac, intFrac, enumFrac, missingFrac, factorRange, numericRange, targetFactor, numGamCols, seed):
    if False:
        print('Hello World!')
    if family == 'binomial':
        responseFactor = 2
    ...
    trainData = random_dataset(nrow, ncol, realFrac=realFrac, intFrac=intFrac, enumFrac=enumFrac, factorR=factorRange, integerR=numericRange, responseFactor=responseFactor, misFrac=missingFrac, randSeed=seed)
    myX = trainData.names
    myY = 'response'
    myX.remove(myY)
...
def random_dataset(nrow, ncol, realFrac=0.4, intFrac=0.3, enumFrac=0.3, factorR=10, integerR=100, responseFactor=1, misFrac=0.01, randSeed=None):
    ...
    if ncol == 1 and realFrac >= 1.0:
        ...
        return h2o.create_frame(rows=nrow, cols=ncol, missing_fraction=misFrac, has_response=True, response_factors=responseFactor, integer_range=integerR, seed=randSeed, **fractions)
    ...
    fractions['string_fraction'] = 0
    fractions['binary_fraction'] = 0
    df = h2o.create_frame(rows=nrow, cols=ncol - real_part.ncol, missing_fraction=misFrac, has_response=True, response_factors=responseFactor, integer_range=integerR, seed=randSeed, **fractions)
