...
class MaxpTable(UnknownTable):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.version > 1.0:
            raise UnsupportedFont('This font has a maxp table with version: %s' % self.version)
        if self.version == 1.0:
            self.fields = ('_version', 'num_glyphs', 'max_points', 'max_contours', 'max_composite_points', 'max_composite_contours', 'max_zones', 'max_twilight_points', 'max_storage', 'max_function_defs', 'max_instruction_defs', 'max_stack_elements', 'max_size_of_instructions', 'max_component_elements', 'max_component_depth')
