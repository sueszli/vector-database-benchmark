...
@testing.parameterize(*testing.product({'x_dtype': [numpy.float16, numpy.float32, numpy.float64], 'W_dtype': [numpy.float16, numpy.float32, numpy.float64], 'nobias': [True, False]}))
class TestDepthwiseConvolution2D(unittest.TestCase):
    ...
    def setUp(self):
        if False:
            for i in range(10):
                print('nop')
        self.link = links.DepthwiseConvolution2D(3, 2, 3, stride=2, pad=1, initialW=chainer.initializers.Normal(1, self.W_dtype), initial_bias=chainer.initializers.Normal(1, self.x_dtype))
        self.link.cleargrads()
        self.x = numpy.random.uniform(-1, 1, (2, 3, 4, 3)).astype(self.x_dtype)
        self.gy = numpy.random.uniform(-1, 1, (2, 6, 2, 2)).astype(self.x_dtype)
...
class TestDepthWiseConvolution2DParameterShapePlaceholder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.link.cleargrads()
        self.gy = numpy.random.uniform(-1, 1, (2, 6, 2, 2)).astype(numpy.float32)
