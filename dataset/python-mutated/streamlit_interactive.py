...
with st.spinner('Please wait...'):
    if uploaded_Resume is not None and st.session_state['jobDescriptionUploaded'] == 'Uploaded' and (uploaded_JobDescription is not None) and (st.session_state['jobDescriptionUploaded'] == 'Uploaded'):
        ...
        with st.container():
            ...
            with jobDescriptionCol:
                with st.expander('Extracted Keywords'):
                    ...
                    annotated_text(create_annotated_text(selected_jd['clean_data'], selected_jd['extracted_keywords'], 'KW', '#0B666A'))
        with st.container():
            (resumeCol, jobDescriptionCol) = st.columns(2)
            with resumeCol:
        ...
        with st.container():
            ...
            with resumeCol:
                with st.expander('Keywords & Values'):
                    ...
                    fig = go.Figure(data=[go.Table(header=dict(values=['Keyword', 'Value'], font=dict(size=12, color='white'), fill_color='#1d2078'), cells=dict(values=[list(keyword_dict.keys()), list(keyword_dict.values())], line_color='darkslategray', fill_color='#6DA9E4'))])
        ...
        with st.container():
            ...
            with resumeCol:
                with st.expander('Key Topics'):
                    fig = px.treemap(df1, path=['keyword'], values='value', color_continuous_scale='Rainbow', title='Key Terms/Topics Extracted from your Resume')
                    st.plotly_chart(fig, use_container_width=True)
            with jobDescriptionCol:
                with st.expander('Key Topics'):
                    fig = px.treemap(df2, path=['keyword'], values='value', color_continuous_scale='Rainbow', title='Key Terms/Topics Extracted from Job Description')
                    st.plotly_chart(fig, use_container_width=True)
        ...
        if os.path.exists(config_file_path):
            ...
            if config_data:
                ...
                if similarity_score < 60:
                    ...
                    score_color = 'orange'
                st.markdown(f'Similarity Score obtained for the resume and job description is <span style="color:{score_color};font-size:24px; font-weight:Bold">{similarity_score}</span>', unsafe_allow_html=True)
        else:
