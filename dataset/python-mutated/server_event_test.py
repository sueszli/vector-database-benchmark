...
class ServerEventTest(testslide.TestCase):
    ...
    async def test_async_waiter(self) -> None:
        ...
        async def assert_ok(event_output: str, wait_on_initialization: bool) -> None:
            await server_event.Waiter(wait_on_initialization=wait_on_initialization).async_wait_on(connections.create_memory_text_reader(event_output))

        async def assert_raises(event_output: str, wait_on_initialization: bool, exception: Type[Exception]=server_event.EventParsingException) -> None:
            with self.assertRaises(exception):
                await server_event.Waiter(wait_on_initialization=wait_on_initialization).async_wait_on(connections.create_memory_text_reader(event_output))
        await assert_raises('garbage', wait_on_initialization=False)
        await assert_raises('[]', wait_on_initialization=False)
        await assert_ok('["SocketCreated", "/path/to/socket"]', wait_on_initialization=False)
        await assert_raises('["ServerInitialized"]', wait_on_initialization=False)
        await assert_raises('["Exception", "message"]', wait_on_initialization=False, exception=server_event.ServerStartException)
        await assert_raises('garbage', wait_on_initialization=True)
        await assert_raises('[]', wait_on_initialization=True)
        await assert_raises('["SocketCreated", "/path/to/socket"]', wait_on_initialization=True)
        await assert_raises('["Exception", "message"]', wait_on_initialization=True, exception=server_event.ServerStartException)
        await assert_raises('["SocketCreated", "/path/to/socket"]\n' + '["ServerException", "message"]', wait_on_initialization=True)
        await assert_raises('["SocketCreated", "/path/to/socket"]\n' + '["SocketCreated", "/path/to/socket"]', wait_on_initialization=True)
        await assert_ok('["SocketCreated", "/path/to/socket"]\n' + '["ServerInitialized"]', wait_on_initialization=True)