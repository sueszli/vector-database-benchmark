        ...
        for path in mpl.glob('**/*.py'):
            ...
            for node in ast.walk(tree):
                for child in ast.iter_child_nodes(node):
                    child.parent = node
            v.visit(tree)
    proc = subprocess.run(['stubtest', '--mypy-config-file=pyproject.toml', '--allowlist=ci/mypy-stubtest-allowlist.txt', f'--allowlist={p}', 'matplotlib'], cwd=root, env=os.environ | {'MPLBACKEND': 'agg'})
    try:
        os.unlink(f.name)
    except OSError:
        pass
