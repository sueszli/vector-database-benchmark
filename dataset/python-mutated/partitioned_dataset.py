...
from kedro.io.core import VERSION_KEY, VERSIONED_FLAG_KEY, AbstractDataset, DatasetError, parse_dataset_definition
from kedro.io.data_catalog import CREDENTIALS_KEY
from kedro.utils import load_obj
DATASET_CREDENTIALS_KEY = 'dataset_credentials'
CHECKPOINT_CREDENTIALS_KEY = 'checkpoint_credentials'
KEY_PROPAGATION_WARNING = 'Top-level %(keys)s will not propagate into the %(target)s since %(keys)s were explicitly defined in the %(target)s config.'
...
class PartitionedDataset(AbstractDataset):
    ...
    `lazy saving <https://docs.kedro.org/en/stable/data/    partitioned_and_incremental_datasets.html#partitioned-dataset-lazy-saving>`_.
...
_DEPRECATED_CLASSES = {'PartitionedDataSet': PartitionedDataset, 'IncrementalDataSet': IncrementalDataset}
