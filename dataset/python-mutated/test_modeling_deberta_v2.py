...
if is_torch_available():
    import torch
    from transformers import DebertaV2ForMaskedLM, DebertaV2ForMultipleChoice, DebertaV2ForQuestionAnswering, DebertaV2ForSequenceClassification, DebertaV2ForTokenClassification, DebertaV2Model
...
class DebertaV2ModelTester(object):
    ...
    def create_and_check_deberta_model(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            for i in range(10):
                print('nop')
    ...
    def create_and_check_deberta_for_masked_lm(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            return 10
    ...
    def create_and_check_deberta_for_sequence_classification(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            i = 10
            return i + 15
    ...
    def create_and_check_deberta_for_token_classification(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            while True:
                i = 10
        config.num_labels = self.num_labels
    ...
    def create_and_check_deberta_for_question_answering(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            return 10
    ...
    def create_and_check_deberta_for_multiple_choice(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            i = 10
            return i + 15
...
class DebertaV2ModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (DebertaV2Model, DebertaV2ForMaskedLM, DebertaV2ForSequenceClassification, DebertaV2ForTokenClassification, DebertaV2ForQuestionAnswering, DebertaV2ForMultipleChoice) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': DebertaV2Model, 'fill-mask': DebertaV2ForMaskedLM, 'question-answering': DebertaV2ForQuestionAnswering, 'text-classification': DebertaV2ForSequenceClassification, 'token-classification': DebertaV2ForTokenClassification, 'zero-shot': DebertaV2ForSequenceClassification} if is_torch_available() else {}
    fx_compatible = True
...
class DebertaV2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor([[[0.2356, 0.1948, 0.0369], [-0.1063, 0.3586, -0.5152], [-0.6399, -0.0259, -0.2525]]])
        self.assertTrue(torch.allclose(output[:, 1:4, 1:4], expected_slice, atol=0.0001), f'{output[:, 1:4, 1:4]}')