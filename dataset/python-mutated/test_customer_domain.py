...
class OrganizationTestEndpoint(Endpoint):
    ...
    def post(self, request, organization_slug):
        ...
        return Response({'organization_slug': organization_slug, 'subdomain': request.subdomain, 'activeorg': request.session.get('activeorg', None)})
urlpatterns = [re_path('^api/0/(?P<organization_slug>[^\\/]+)/$', OrganizationTestEndpoint.as_view(), name='org-events-endpoint'), re_path('^api/0/(?P<organization_slug>[^\\/]+)/nameless/$', OrganizationTestEndpoint.as_view()), re_path('^logout/$', AuthLogoutView.as_view(), name='sentry-logout')]
...
@override_settings(ROOT_URLCONF=__name__, SENTRY_SELF_HOSTED=False, SENTRY_USE_CUSTOMER_DOMAINS=True)
