...
class secretmanagerCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'access_secret_version': ('name',), 'add_secret_version': ('parent', 'payload'), 'create_secret': ('parent', 'secret_id', 'secret'), 'delete_secret': ('name',), 'destroy_secret_version': ('name',), 'disable_secret_version': ('name',), 'enable_secret_version': ('name',), 'get_iam_policy': ('resource', 'options'), 'get_secret': ('name',), 'get_secret_version': ('name',), 'list_secrets': ('parent', 'page_size', 'page_token'), 'list_secret_versions': ('parent', 'page_size', 'page_token'), 'set_iam_policy': ('resource', 'policy', 'update_mask'), 'test_iam_permissions': ('resource', 'permissions'), 'update_secret': ('secret', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=secretmanagerCallTransformer()):
    if False:
        while True:
            i = 10
