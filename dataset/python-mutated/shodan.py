...
from lib.settings import API_URLS, HOST_FILE, write_to_file
...
class ShodanAPIHook(object):
    ...
    def search(self):
        ...
        try:
            req = requests.get(API_URLS['shodan'].format(query=self.query, token=self.token), proxies=self.proxy, headers=self.user_agent)
