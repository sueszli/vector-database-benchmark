...
async def test_shell_cmd(db, tmpdir, request):
    ...
    s = new_spawner(db, cmd=[sys.executable, '-m', 'jupyterhub.tests.mocksu'], shell_cmd=['bash', '--rcfile', str(f), '-i', '-c'])
    server = orm.Server()
    db.add(server)
    db.commit()
...
@pytest.mark.parametrize('have_scopes, request_scopes, expected_scopes', [(['self'], ['inherit'], ['inherit']), (['self'], [], ['access:servers!server=USER/', 'users:activity!user']), (['self'], ['admin:groups', 'read:servers!server'], ['users:activity!user', 'read:servers!server=USER/']), (['self', 'read:groups!group=x', 'users:activity'], ['admin:groups', 'users:activity'], ['read:groups!group=x', 'read:groups:name!group=x', 'users:activity'])])
async def test_server_token_scopes(app, username, create_user_with_scopes, have_scopes, request_scopes, expected_scopes):
    """Token provided by spawner is not in the db
...
async def test_spawner_env(db):
    env_overrides = {'JUPYTERHUB_API_URL': 'https://test.horse/hub/api', 'TEST_KEY': 'value'}
...
async def test_hub_connect_url(db):
    ...
    assert env['JUPYTERHUB_ACTIVITY_URL'] == 'https://example.com/api/users/%s/activity' % name
...
async def test_spawner_oauth_scopes(app, user):
    ...
    assert sorted(oauth_client.allowed_scopes) == sorted(allowed_scopes + list(access_scopes(oauth_client)))
