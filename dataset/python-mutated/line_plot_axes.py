...
graph_size = (900, 700)
N = 30
margin_x = (canvas_size[0] - graph_size[0]) / 2.0
margin_y = (canvas_size[1] - graph_size[1]) / 2.0
pos_xax = np.array([[margin_x, canvas_size[1] - margin_y], [canvas_size[0] - margin_x, canvas_size[1] - margin_y]])
pos_yax = np.array([[margin_x, canvas_size[1] - margin_y], [margin_x, margin_y]])
pos = np.zeros((N, 2), dtype=np.float32)
pos[:, 0] = np.linspace(margin_x, canvas_size[0] - margin_x, N)
pos[:, 1] = np.clip(np.random.normal(size=N, scale=100, loc=canvas_size[1] / 2.0), margin_y, canvas_size[0] - margin_y)
