...
with DAG(dag_id=DAG_ID, start_date=datetime(2021, 1, 1), schedule='@once', catchup=False, tags=['example']) as dag:
    ...
    sql_to_s3_task = SqlToS3Operator(task_id='sql_to_s3_task', sql_conn_id=conn_id_name, query=SQL_QUERY, s3_bucket=bucket_name, s3_key=key, replace=True)
    sql_to_s3_task_with_groupby = SqlToS3Operator(task_id='sql_to_s3_with_groupby_task', sql_conn_id=conn_id_name, query=SQL_QUERY, s3_bucket=bucket_name, s3_key=key, replace=True, groupby_kwargs={'by': 'color'})
    ...
    chain(test_context, create_bucket, set_up_sg, create_cluster, wait_cluster_available, set_up_connection, create_table_redshift_data, insert_data, sql_to_s3_task, sql_to_s3_task_with_groupby, delete_bucket, delete_cluster, delete_sg)
