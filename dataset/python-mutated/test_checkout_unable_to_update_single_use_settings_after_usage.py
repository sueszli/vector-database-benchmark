...
from ....vouchers.utils import create_voucher, create_voucher_channel_listing, raw_update_voucher
...
def prepare_voucher(e2e_staff_api_client, channel_id, voucher_code_list, voucher_discount_type, voucher_discount_value, voucher_type):
    if False:
        for i in range(10):
            print('nop')
    input_data = {'addCodes': voucher_code_list, 'discountValueType': voucher_discount_type, 'type': voucher_type, 'singleUse': True}
...
def test_checkout_unable_to_update_single_use_settings_after_usage_CORE_0926(e2e_logged_api_client, e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_shipping, permission_manage_product_types_and_attributes, permission_manage_discounts, permission_manage_checkouts):
    ...
    (voucher_code_list, voucher_id) = prepare_voucher(e2e_staff_api_client, channel_id, voucher_code_list=['single_use_1', 'single_use_2'], voucher_discount_type='PERCENTAGE', voucher_discount_value=15, voucher_type='ENTIRE_ORDER')
    ...
    data = checkout_add_promo_code(e2e_logged_api_client, checkout_id, voucher_code_list[0])
    ...
    assert discounted_unit_price == unit_price - round(float(product_variant_price) * 15 / 100, 2)
    checkout_dummy_payment_create(e2e_logged_api_client, checkout_id, discounted_total_gross)
    ...
    assert errors['message'] == 'Cannot change single use setting when any voucher code has already been used.'