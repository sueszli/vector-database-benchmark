...
class TestGCSToTrinoOperator:
    ...
    def test_execute_without_schema(self, mock_tempfile, mock_gcs_hook, mock_trino_hook):
        ...
        op = GCSToTrinoOperator(task_id=TASK_ID, source_bucket=BUCKET, source_object=PATH, trino_table=TRINO_TABLE, trino_conn_id=TRINO_CONN_ID, gcp_conn_id=GCP_CONN_ID, impersonation_chain=IMPERSONATION_CHAIN)
        op.execute(None)
    ...
    def test_execute_schema_fields(self, mock_tempfile, mock_gcs_hook, mock_trino_hook):
        ...
        mock_tempfile.return_value.__enter__.return_value = file_handle
        mock_tempfile.return_value.__enter__.return_value.name = filename
        op = GCSToTrinoOperator(task_id=TASK_ID, source_bucket=BUCKET, source_object=PATH, trino_table=TRINO_TABLE, trino_conn_id=TRINO_CONN_ID, gcp_conn_id=GCP_CONN_ID, schema_fields=SCHEMA_FIELDS, impersonation_chain=IMPERSONATION_CHAIN)
        op.execute(None)
        mock_gcs_hook.assert_called_once_with(gcp_conn_id=GCP_CONN_ID, impersonation_chain=IMPERSONATION_CHAIN)
        mock_trino_hook.assert_called_once_with(trino_conn_id=TRINO_CONN_ID)
        mock_download = mock_gcs_hook.return_value.download
    ...
    def test_execute_schema_json(self, mock_tempfile, mock_gcs_hook, mock_trino_hook, mock_json_loader):
        ...
        op = GCSToTrinoOperator(task_id=TASK_ID, source_bucket=BUCKET, source_object=PATH, trino_table=TRINO_TABLE, trino_conn_id=TRINO_CONN_ID, gcp_conn_id=GCP_CONN_ID, schema_object=SCHEMA_JSON, impersonation_chain=IMPERSONATION_CHAIN)
        op.execute(None)
        mock_gcs_hook.assert_called_once_with(gcp_conn_id=GCP_CONN_ID, impersonation_chain=IMPERSONATION_CHAIN)
        mock_trino_hook.assert_called_once_with(trino_conn_id=TRINO_CONN_ID)
        mock_download = mock_gcs_hook.return_value.download
