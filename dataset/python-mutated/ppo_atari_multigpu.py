...
def parse_args():
    ...
    parser.add_argument('--device-ids', nargs='+', default=[], help='the device ids that subprocess workers will use')
    parser.add_argument('--backend', type=str, default='gloo', choices=['gloo', 'nccl', 'mpi'], help='the id of the environment')
...
if __name__ == '__main__':
    ...
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-05)
        ...
        for step in range(0, args.num_steps):
            ...
            for item in info:
                if 'episode' in item.keys() and local_rank == 0:
                    ...
                    break
        print(f'local_rank: {local_rank}, action.sum(): {action.sum()}, update: {update}, agent.actor.weight.sum(): {agent.actor.weight.sum()}')
        with torch.no_grad():
