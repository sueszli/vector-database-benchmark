...
class AttributeValueUpdate(AttributeValueCreate, ModelWithExtRefMutation):
    ...
    class Arguments:
        id = graphene.ID(required=False, description='ID of an AttributeValue to update.')
        external_reference = graphene.String(required=False, description=f'External ID of an AttributeValue to update. {ADDED_IN_310}')
        input = AttributeValueUpdateInput(required=True, description='Fields required to update an AttributeValue.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.ATTRIBUTE_VALUE_UPDATED, description='An attribute value was updated.'), WebhookEventInfo(type=WebhookEventAsyncType.ATTRIBUTE_UPDATED, description='An attribute was updated.')]
