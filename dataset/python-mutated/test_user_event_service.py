...
from google.cloud.retail_v2.services.user_event_service import UserEventServiceAsyncClient, UserEventServiceClient, transports
from google.cloud.retail_v2.types import common, import_config, product, promotion, purge_config, user_event, user_event_service
...
async def test_write_user_event_async(transport: str='grpc_asyncio', request_type=user_event_service.WriteUserEventRequest):
    client = UserEventServiceAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.write_user_event), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(user_event.UserEvent(event_type='event_type_value', visitor_id='visitor_id_value', session_id='session_id_value', experiment_ids=['experiment_ids_value'], attribution_token='attribution_token_value', cart_id='cart_id_value', search_query='search_query_value', filter='filter_value', order_by='order_by_value', offset=647, page_categories=['page_categories_value'], uri='uri_value', referrer_uri='referrer_uri_value', page_view_id='page_view_id_value', entity='entity_value'))
...
async def test_write_user_event_field_headers_async():
    client = UserEventServiceAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = user_event_service.WriteUserEventRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.write_user_event), '__call__') as call:
