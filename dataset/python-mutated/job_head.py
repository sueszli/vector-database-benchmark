...
from ray._private.runtime_env.packaging import package_exists, pin_runtime_env_uri, upload_package_to_gcs
from ray.dashboard.modules.job.common import JobDeleteResponse, http_uri_components_to_uri, JobSubmitRequest, JobSubmitResponse, JobStopResponse, JobLogsResponse, JobInfoStorageClient
from ray.dashboard.modules.job.pydantic_models import JobDetails, JobType
from ray.dashboard.modules.job.utils import parse_and_validate_request, get_driver_jobs, find_job_by_ids
...
class JobHead(dashboard_utils.DashboardHeadModule):
    ...
    async def choose_agent(self) -> Optional[JobAgentSubmissionClient]:
        ...
        while True:
            ...
            agent_infos = {key: value for (key, value) in raw_agent_infos.items() if value.get('httpPort', -1) > 0}
    ...
    async def get_package(self, req: Request) -> Response:
        package_uri = http_uri_components_to_uri(protocol=req.match_info['protocol'], package_name=req.match_info['package_name'])
        ...
        if not package_exists(package_uri):
            return Response(text=f'Package {package_uri} does not exist', status=aiohttp.web.HTTPNotFound.status_code)
        return Response()
    ...
    async def submit_job(self, req: Request) -> Response:
        ...
        try:
            job_agent_client = await asyncio.wait_for(self.choose_agent(), timeout=dashboard_consts.WAIT_AVAILABLE_AGENT_TIMEOUT)
            resp = await job_agent_client.submit_job_internal(submit_request)
        except asyncio.TimeoutError:
            return Response(text='No available agent to submit job, please try again later.', status=aiohttp.web.HTTPInternalServerError.status_code)
        except (TypeError, ValueError):
            return Response(text=traceback.format_exc(), status=aiohttp.web.HTTPBadRequest.status_code)
        except Exception:
            return Response(text=traceback.format_exc(), status=aiohttp.web.HTTPInternalServerError.status_code)
        return Response(text=json.dumps(dataclasses.asdict(resp)), content_type='application/json', status=aiohttp.web.HTTPOk.status_code)
    ...
    async def stop_job(self, req: Request) -> Response:
        ...
        job = await find_job_by_ids(self._dashboard_head.gcs_aio_client, self._job_info_client, job_or_submission_id)
        if not job:
            return Response(text=f'Job {job_or_submission_id} does not exist', status=aiohttp.web.HTTPNotFound.status_code)
        if job.type is not JobType.SUBMISSION:
            return Response(text='Can only stop submission type jobs', status=aiohttp.web.HTTPBadRequest.status_code)
        try:
            job_agent_client = await asyncio.wait_for(self.choose_agent(), timeout=dashboard_consts.WAIT_AVAILABLE_AGENT_TIMEOUT)
            resp = await job_agent_client.stop_job_internal(job.submission_id)
        except Exception:
            return Response(text=traceback.format_exc(), status=aiohttp.web.HTTPInternalServerError.status_code)
        return Response(text=json.dumps(dataclasses.asdict(resp)), content_type='application/json')
    ...
    async def delete_job(self, req: Request) -> Response:
        ...
        if not job:
            return Response(text=f'Job {job_or_submission_id} does not exist', status=aiohttp.web.HTTPNotFound.status_code)
        if job.type is not JobType.SUBMISSION:
            return Response(text='Can only delete submission type jobs', status=aiohttp.web.HTTPBadRequest.status_code)
        try:
            job_agent_client = await asyncio.wait_for(self.choose_agent(), timeout=dashboard_consts.WAIT_AVAILABLE_AGENT_TIMEOUT)
    ...
    async def get_job_info(self, req: Request) -> Response:
        ...
        if not job:
            return Response(text=f'Job {job_or_submission_id} does not exist', status=aiohttp.web.HTTPNotFound.status_code)
        return Response(text=json.dumps(job.dict()), content_type='application/json')

    @routes.get('/api/jobs/')
    async def list_jobs(self, req: Request) -> Response:
        (driver_jobs, submission_job_drivers) = await get_driver_jobs(self._dashboard_head.gcs_aio_client)
        ...
        return Response(text=json.dumps([*[submission_job.dict() for submission_job in submission_jobs], *[job_info.dict() for job_info in driver_jobs.values()]]), content_type='application/json')
    ...
    async def get_job_logs(self, req: Request) -> Response:
        ...
        if not job:
            return Response(text=f'Job {job_or_submission_id} does not exist', status=aiohttp.web.HTTPNotFound.status_code)
        if job.type is not JobType.SUBMISSION:
            return Response(text='Can only get logs of submission type jobs', status=aiohttp.web.HTTPBadRequest.status_code)
        try:
            ...
            if driver_agent_http_address is None:
                ...
                if driver_node_id not in self._agents:
                    self._agents[driver_node_id] = JobAgentSubmissionClient(driver_agent_http_address)
