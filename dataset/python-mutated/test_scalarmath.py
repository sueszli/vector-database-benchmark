...
from torch.testing._internal.common_utils import instantiate_parametrized_tests, parametrize, run_tests, slowTest as slow, subtest, TEST_WITH_TORCHDYNAMO, TestCase, xpassIfTorchDynamo
if TEST_WITH_TORCHDYNAMO:
    import numpy as np
    from numpy.testing import _gen_alignment_data, assert_, assert_almost_equal, assert_equal
else:
    import torch._numpy as np
    from torch._numpy.testing import _gen_alignment_data, assert_, assert_almost_equal, assert_equal
skip = functools.partial(skipif, True)
IS_PYPY = False
types = [np.bool_, np.byte, np.ubyte, np.short, np.intc, np.int_, np.longlong, np.single, np.double, np.csingle, np.cdouble]
floating_types = np.floating.__subclasses__()
complex_floating_types = np.complexfloating.__subclasses__()
objecty_things = [object(), None]
reasonable_operators_for_scalars = [operator.lt, operator.le, operator.eq, operator.ne, operator.ge, operator.gt, operator.add, operator.floordiv, operator.mod, operator.mul, operator.pow, operator.sub, operator.truediv]
