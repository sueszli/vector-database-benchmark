...
class test_gam_gridsearch_error:
    ...
    def setup_data(self):
        ...
        for alpha_param in self.hyper_parameters['alpha']:
            self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family='gaussian', gam_columns=['C11', 'C12', 'C13'], keep_gam_cols=True, scale=[1, 1, 1], num_knots=[5, 6, 7], alpha=alpha_param, lambda_=[0.01]))

    def train_models(self):
        if False:
            for i in range(10):
                print('nop')
        self.h2o_model = H2OGridSearch(H2OGeneralizedAdditiveEstimator(family='gaussian', gam_columns=['C11', 'C12', 'C13'], keep_gam_cols=True, scale=[1, 1, 1], num_knots=[5, 6, 7]), self.hyper_parameters)
