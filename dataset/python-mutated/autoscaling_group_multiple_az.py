...
class autoscaling_group_multiple_az(Check):
    ...
    def execute(self):
        ...
        for group in autoscaling_client.groups:
            ...
            report.status = 'FAIL'
            report.status_extended = f'Autoscaling group {group.name} has only one availability zones.'
            if len(group.availability_zones) > 1:
                report.status = 'PASS'
                report.status_extended = f'Autoscaling group {group.name} has multiple availability zones.'
