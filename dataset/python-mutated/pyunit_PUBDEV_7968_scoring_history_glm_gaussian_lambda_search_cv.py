...
def testGLMGaussianScoringHistory():
    ...
    model_cv = glm(family='gaussian', score_each_iteration=True, nfolds=3, fold_assignment='modulo', seed=1234, lambda_search=True, nlambdas=10)
    model_cv.train(x=myX, y=myY, training_frame=training_data, validation_frame=test_data)
    model_score_each_cv = glm(family='gaussian', score_iteration_interval=1, nfolds=3, fold_assignment='modulo', seed=1234, lambda_search=True, nlambdas=10)
    ...
    model_cv_4th = glm(family='gaussian', score_iteration_interval=4, nfolds=3, fold_assignment='modulo', seed=1234, lambda_search=True, nlambdas=10)
