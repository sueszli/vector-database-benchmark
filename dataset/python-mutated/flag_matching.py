...
from .feature_flag import FeatureFlag, FeatureFlagHashKeyOverride, get_feature_flags_for_team_in_cache, set_feature_flags_for_team_in_cache
logger = structlog.get_logger(__name__)
__LONG_SCALE__ = float(1152921504606846975)
FLAG_MATCHING_QUERY_TIMEOUT_MS = 300
FLAG_EVALUATION_ERROR_COUNTER = Counter('flag_evaluation_error_total', 'Failed decide requests with reason.', labelnames=['reason'])
FLAG_HASH_KEY_WRITES_COUNTER = Counter('flag_hash_key_writes_total', 'Attempts to write hash key overrides to the database.', labelnames=[LABEL_TEAM_ID, 'successful_write'])
FLAG_CACHE_HIT_COUNTER = Counter('flag_cache_hit_total', 'Whether we could get all flags from the cache or not.', labelnames=[LABEL_TEAM_ID, 'cache_hit'])
