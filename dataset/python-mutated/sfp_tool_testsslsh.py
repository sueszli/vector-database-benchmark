...
class sfp_tool_testsslsh(SpiderFootPlugin):
    meta = {'name': 'Tool - testssl.sh', 'summary': 'Identify various TLS/SSL weaknesses, including Heartbleed, CRIME and ROBOT.', 'flags': ['tool'], 'useCases': ['Footprint', 'Investigate'], 'categories': ['Crawling and Scanning'], 'toolDetails': {'name': 'testssl.sh', 'description': "testssl.sh is a free command line tool which checks a server's service on any port for the support of TLS/SSL ciphers, protocols as well as some cryptographic flaws.", 'website': 'https://testssl.sh', 'repository': 'https://github.com/drwetter/testssl.sh'}}
    opts = {'testsslsh_path': '', 'netblockscan': True, 'netblockscanmax': 24, 'mincve': 'LOW'}
    optdescs = {'testsslsh_path': 'Path to your testssl.sh executable. Must be set.', 'netblockscan': 'Test all IPs within identified owned netblocks?', 'netblockscanmax': 'Maximum netblock/subnet size to test IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'mincve': 'Only report CVEs equal to or higher than this level, must be either LOW, MEDIUM, HIGH or CRITICAL.'}
    results = None
    errorState = False
