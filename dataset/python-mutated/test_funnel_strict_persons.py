...
class TestFunnelStrictStepsPersons(ClickhouseTestMixin, APIBaseTest):
    ...
    def _create_sample_data_multiple_dropoffs(self):
        ...
        events_by_person = {}
        for i in range(5):
            events_by_person[f'user_{i}'] = [{'event': 'step one', 'timestamp': datetime(2021, 5, 1)}, {'event': 'step fake', 'timestamp': datetime(2021, 5, 2)}, {'event': 'step two', 'timestamp': datetime(2021, 5, 3)}, {'event': 'step three', 'timestamp': datetime(2021, 5, 5)}]
        for i in range(5, 15):
            events_by_person[f'user_{i}'] = [{'event': 'step one', 'timestamp': datetime(2021, 5, 1)}, {'event': 'step two', 'timestamp': datetime(2021, 5, 3)}]
    ...
    def test_strict_funnel_person_recordings(self):
        ...
        _create_event(event='interupting step', distinct_id='user_1', team=self.team, timestamp=(timezone.now() + timedelta(days=2)).strftime('%Y-%m-%d %H:%M:%S.%f'), properties={'$session_id': 's2', '$window_id': 'w2'}, event_uuid='21111111-1111-1111-1111-111111111111')
        _create_event(event='step three', distinct_id='user_1', team=self.team, timestamp=(timezone.now() + timedelta(days=3)).strftime('%Y-%m-%d %H:%M:%S.%f'), properties={'$session_id': 's2', '$window_id': 'w2'}, event_uuid='21111111-1111-1111-1111-111111111111')
