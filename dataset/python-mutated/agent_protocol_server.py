...
from forge.sdk.schema import Artifact, Step, StepRequestBody, Task, TaskArtifactsListResponse, TaskListResponse, TaskRequestBody, TaskStepsListResponse
...
class AgentProtocolServer:
    ...
    async def start(self, port: int=8000, router: APIRouter=base_router):
        ...
        app = FastAPI(title='AutoGPT Server', description='Forked from AutoGPT Forge; Modified version of The Agent Protocol.', version='v0.4')
        origins = ['*']
        app.add_middleware(CORSMiddleware, allow_origins=origins, allow_credentials=True, allow_methods=['*'], allow_headers=['*'])
        app.include_router(router, prefix='/ap/v1')
        script_dir = os.path.dirname(os.path.realpath(__file__))
        frontend_path = pathlib.Path(script_dir).joinpath('../../../../frontend/build/web').resolve()
        if os.path.exists(frontend_path):
            ...
            logger.warning(f'Frontend not found. {frontend_path} does not exist. The frontend will not be available.')
    ...
    async def create_task(self, task_request: TaskRequestBody) -> Task:
        ...
        task_agent = await generate_agent_for_task(task=task_request.input, app_config=self.app_config, llm_provider=self.llm_provider)
        task = await self.db.create_task(input=task_request.input, additional_input=task_request.additional_input)
    ...
    async def execute_step(self, task_id: str, step_request: StepRequestBody) -> Step:
        ...
        agent = configure_agent_with_state(state=self.agent_manager.retrieve_state(task_agent_id(task_id)), app_config=self.app_config, llm_provider=self.llm_provider)
        agent.workspace.on_write_file = lambda path: self.db.create_artifact(task_id=task_id, file_name=path.parts[-1], relative_path=str(path))
        ...
        if not is_init_step and agent.event_history.current_episode and (not agent.event_history.current_episode.result):
            ...
            logger.debug(f'Agent proposed command {execute_command}({fmt_kwargs(execute_command_args)}). User input/feedback: {repr(user_input)}')
        step = await self.db.create_step(task_id=task_id, input=step_request, is_last=execute_command == finish.__name__ and execute_approved)
        if execute_command:
            assert execute_command_args is not None
            if step.is_last and execute_command == finish.__name__:
                assert execute_command_args
                step = await self.db.update_step(task_id=task_id, step_id=step.step_id, output=execute_command_args['reason'])
                return step
            if execute_command == ask_user.__name__:
                ...
                step = await self.db.update_step(task_id=task_id, step_id=step.step_id, status='running')
                execute_result = await agent.execute(command_name=execute_command, command_args=execute_command_args)
            else:
                assert user_input
                execute_result = await agent.execute(command_name='human_feedback', command_args={}, user_input=user_input)
        try:
            (next_command, next_command_args, raw_output) = await agent.propose_action()
            logger.debug(f'AI output: {raw_output}')
        except Exception as e:
            step = await self.db.update_step(task_id=task_id, step_id=step.step_id, status='completed', output=f'An error occurred while proposing the next action: {e}')
            return step
        output = f'Command `{execute_command}({fmt_kwargs(execute_command_args)})` returned:' + ('\n\n' if '\n' in str(execute_result) else ' ') + f'{execute_result}\n\n' if execute_command_args and execute_command != 'ask_user' else ''
        output += f"{raw_output['thoughts']['speak']}\n\n"
        output += f'Next Command: {next_command}({fmt_kwargs(next_command_args)})' if next_command != 'ask_user' else next_command_args['question']
        additional_output = {**({'last_action': {'name': execute_command, 'args': execute_command_args, 'result': orjson.loads(execute_result.json()) if not isinstance(execute_result, ActionErrorResult) else {'error': str(execute_result.error), 'reason': execute_result.reason}}} if not is_init_step else {}), **raw_output}
        step = await self.db.update_step(task_id=task_id, step_id=step.step_id, status='completed', output=output, additional_output=additional_output)
    ...
    async def create_artifact(self, task_id: str, file: UploadFile, relative_path: str) -> Artifact:
        ...
        artifact = await self.db.create_artifact(task_id=task_id, file_name=file_name, relative_path=relative_path, agent_created=False)
        return artifact
