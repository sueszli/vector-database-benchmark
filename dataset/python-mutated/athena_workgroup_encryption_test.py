...
class Test_athena_workgroup_encryption:
    ...
    def test_primary_workgroup_not_encrypted(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.athena.athena_workgroup_encryption.athena_workgroup_encryption.athena_client', new=Athena(current_audit_info)):
            from prowler.providers.aws.services.athena.athena_workgroup_encryption.athena_workgroup_encryption import athena_workgroup_encryption
            check = athena_workgroup_encryption()
            result = check.execute()
            assert len(result) == 1
            assert result[0].status == 'FAIL'
            assert result[0].status_extended == f'Athena WorkGroup {ATHENA_PRIMARY_WORKGROUP} does not encrypt the query results.'
    ...
    def test_primary_workgroup_encrypted(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.athena.athena_workgroup_encryption.athena_workgroup_encryption.athena_client', new=Athena(current_audit_info)):
            ...
            assert result[0].status_extended == f'Athena WorkGroup {ATHENA_PRIMARY_WORKGROUP} encrypts the query results using SSE_S3.'
            assert result[0].resource_id == ATHENA_PRIMARY_WORKGROUP
            assert result[0].resource_arn == ATHENA_PRIMARY_WORKGROUP_ARN
            assert result[0].region == AWS_REGION
            assert result[0].resource_tags == []