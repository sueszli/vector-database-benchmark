...
from fairseq.data import AppendTokenDataset, Dictionary, IdDataset, NestedDictionaryDataset, NumelDataset, PadDataset, PrependTokenDataset, StripTokenDataset, TokenBlockDataset, data_utils
...
class SpanMaskedLMConfig(FairseqDataclass):
    ...
    noise_density: float = field(default=0.15, metadata={'help': 'What fraction of the tokens to select as noise'})
    mean_noise_span_length: float = field(default=3, metadata={'help': 'Mean noise span length, must be >= 1'})
...
class SpanMaskedLMTask(FairseqTask):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = SpanMaskedTokensDataset(dataset, self.dictionary, noise_density=self.cfg.noise_density, mean_noise_span_length=self.cfg.mean_noise_span_length, shuffle=self.cfg.shuffle, seed=self.cfg.seed)
        logger.info('Split: {0}, Loaded {1} samples of span_masked_tokens_dataset'.format(split, len(self.datasets[split])))
