...
try:
    ...
    print('You need to install module coverage (version %s or newer required).\nSee https://coverage.readthedocs.io/en/latest/ or \nhttps://launchpad.net/ubuntu/+source/python-coverage/' % minver)
    sys.exit(-1)
omit_dir_patterns = ['benchmark', 'examples', 'pyglet', 'test_external']
omit_dir_re = re.compile('|'.join(omit_dir_patterns))
source_re = re.compile('.*\\.py$')
...
parser.add_argument('-c', '--use-cache', action='store_true', default=False, help='Use cached data.')
parser.add_argument('-d', '--report-dir', default='covhtml', help='Directory to put the generated report in.')
parser.add_argument('--slow', action='store_true', dest='slow', default=False, help='Run slow functions also.')
...
if __name__ == '__main__':
    ...
    print('Open %s in your web browser to view the report' % os.sep.join([report_dir, 'index.html']))