...
async def test_write_file_fails_if_content_exists(test_file_name: Path, agent: Agent):
    new_content = 'This is new content.\n'
    file_ops.log_operation('write', test_file_name, agent=agent, checksum=file_ops.text_checksum(new_content))
    with pytest.raises(DuplicateOperationError):
        await file_ops.write_to_file(test_file_name, new_content, agent=agent)

@pytest.mark.asyncio
async def test_write_file_succeeds_if_content_different(test_file_with_content_path: Path, agent: Agent):
