...
class sfp_zetalytics(SpiderFootPlugin):
    ...
    meta = {'name': 'Zetalytics', 'summary': 'Query the Zetalytics database for hosts on your target domain(s).', 'flags': ['apikey'], 'useCases': ['Passive'], 'categories': ['Passive DNS'], 'dataSource': {'website': 'https://zetalytics.com/', 'model': 'FREE_AUTH_LIMITED', 'references': ['https://zonecruncher.com/api-v1-docs/'], 'apiKeyInstructions': ['Visit https://mailchi.mp/zetalytics/trial-access-request', 'Register a free account and request an API key'], 'favIcon': 'https://zetalytics.com/favicon.ico', 'logo': 'https://zetalytics.com/assets/images/logo.png', 'description': 'Zetalytics database provides several useful endpoints to perform passive DNS analysis'}}
    opts = {'api_key': '', 'verify': True}
    optdescs = {'api_key': 'Zetalytics API Key.', 'verify': 'Verify that any hostnames found on the target domain still resolve?'}
    results = None
    errorState = False
