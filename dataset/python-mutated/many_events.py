from dagster import AssetMaterialization, ExpectationResult, In, MetadataValue, Nothing, Out, Output, file_relative_path, graph, op
MARKDOWN_EXAMPLE = 'markdown_example.md'
raw_files = ['raw_file_users', 'raw_file_groups', 'raw_file_events', 'raw_file_friends', 'raw_file_pages', 'raw_file_fans', 'raw_file_event_admins', 'raw_file_group_admins']
...
def create_raw_file_op(name):
    ...
    def raw_file_op(_context):
        ...
        yield Output(name)
    return raw_file_op
raw_tables = ['raw_users', 'raw_groups', 'raw_events', 'raw_friends', 'raw_pages', 'raw_fans', 'raw_event_admins', 'raw_group_admins']
...
many_events_job = many_events.to_job(name='many_events_job', description='Demo job that yields AssetMaterializations and ExpectationResults, along with the various forms of metadata that can be attached to them.')
many_events_subset_job = many_events.to_job(name='many_events_subset_job', op_selection=['many_materializations_and_passing_expectations*'])