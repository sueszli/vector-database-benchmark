...
class GigaChannelManager(TaskManager):
            ...
            with db_session:
                for channel in self.mds.ChannelMetadata.get_my_channels().where(lambda g: g.status == COMMITTED):
                    ...
                    if channel_download is None:
                        self._logger.warning('Torrent for personal channel %s %i does not exist.', hexlify(channel.public_key), channel.id_)
                        self.regenerate_channel_torrent(channel.public_key, channel.id_)
                    else:
                        self.register_task(f'Check personal channel {(hexlify(channel.public_key), channel.id_)}', self.check_and_regen_personal_channel_torrent, channel.public_key, channel.id_, channel_download)
        except Exception:
    ...
    async def process_queued_channels(self):
        ...
        while self.channels_processing_queue:
            ...
            if action == PROCESS_CHANNEL_DIR:
                await self.process_channel_dir_threaded(data)
            elif action == REMOVE_CHANNEL_DOWNLOAD:
                await self.remove_channel_download(data)
            elif action == CLEANUP_UNSUBSCRIBED_CHANNEL:
                self.cleanup_channel(data)
        self.processing = False
    ...
    async def updated_my_channel(self, tdef):
        ...
        if my_channel and my_channel.status == COMMITTED and (not self.download_manager.download_exists(bytes(my_channel.infohash))):
            dcfg = DownloadConfig(state_dir=self.state_dir)
            dcfg.set_dest_dir(self.mds.channels_dir)
            dcfg.set_channel_download(True)
