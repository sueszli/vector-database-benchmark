...
if is_torch_available():
    import torch
    from transformers import YosoForMaskedLM, YosoForMultipleChoice, YosoForQuestionAnswering, YosoForSequenceClassification, YosoForTokenClassification, YosoModel
...
class YosoModelTester:
    ...
    def get_config(self):
        if False:
            while True:
                i = 10
        return YosoConfig(vocab_size=self.vocab_size, hidden_size=self.hidden_size, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, hidden_act=self.hidden_act, hidden_dropout_prob=self.hidden_dropout_prob, attention_probs_dropout_prob=self.attention_probs_dropout_prob, max_position_embeddings=self.max_position_embeddings, type_vocab_size=self.type_vocab_size, is_decoder=False, initializer_range=self.initializer_range)
...
class YosoModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (YosoModel, YosoForMaskedLM, YosoForMultipleChoice, YosoForQuestionAnswering, YosoForSequenceClassification, YosoForTokenClassification) if is_torch_available() else ()
    test_pruning = False
    test_headmasking = False
    test_torchscript = False
    all_generative_model_classes = ()
    pipeline_model_mapping = {'feature-extraction': YosoModel, 'fill-mask': YosoForMaskedLM, 'question-answering': YosoForQuestionAnswering, 'text-classification': YosoForSequenceClassification, 'token-classification': YosoForTokenClassification, 'zero-shot': YosoForSequenceClassification} if is_torch_available() else {}
...
class YosoModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor([[[-0.0611, 0.1242, 0.084], [0.028, -0.0048, 0.1125], [0.0106, 0.0226, 0.0751]]])
    ...
    def test_inference_masked_lm(self):
        ...
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[[-2.1313, -3.7285, -2.2407], [-2.7047, -3.3314, -2.6408], [0.0629, -2.5166, -0.3356]]])
        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=0.0001))
    ...
    def test_inference_masked_lm_long_input(self):
        ...
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[[-2.3914, -4.3742, -5.0956], [-4.0988, -4.2384, -7.0406], [-3.1427, -3.7192, -6.68]]])
        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=0.0001))