...
from ray.rllib.examples.env.recommender_system_envs_with_recsim import InterestEvolutionRecSimEnv, InterestExplorationRecSimEnv, LongTermSatisfactionRecSimEnv
from ray.rllib.utils.test_utils import check_learning_achieved
from ray.tune.logger import pretty_print
from ray.tune.registry import get_trainable_cls
parser = argparse.ArgumentParser()
parser.add_argument('--run', type=str, default='SlateQ', choices=['SlateQ', 'DQN'], help='Select agent policy. Choose from: DQN and SlateQ. Default value: SlateQ.')
parser.add_argument('--framework', choices=['tf', 'tf2', 'torch'], default='torch', help='The DL framework specifier.')
parser.add_argument('--env', type=str, default='interest-evolution', choices=['interest-evolution', 'interest-exploration', 'long-term-satisfaction'], help='Select the RecSim env to use.')
parser.add_argument('--random-test-episodes', type=int, default=0, help='The number of test episodes to run with a random agent to figure out up front what the random baseline reward is.')
parser.add_argument('--use-tune', action='store_true', help="Run with Tune so that the results are logged into Tensorboard. For debugging, it's easier to run without Ray Tune.")
parser.add_argument('--tune-num-samples', type=int, default=1)
parser.add_argument('--env-num-candidates', type=int, default=100, help="The number of candidates that the agent has to pick `--env-slate-size` from each timestep. These candidates will be sampled by the environment's built-in document sampler model.")
parser.add_argument('--num-steps-sampled-before-learning_starts', type=int, default=20000, help='Number of timesteps to collect from rollout workers before we start sampling from replay buffers for learning..')
parser.add_argument('--env-slate-size', type=int, default=2, help='The size of the slate to recommend (from out of `--env-num-candidates` sampled docs) each timestep.')
parser.add_argument('--env-dont-resample-documents', action='store_true', help='Whether to NOT resample `--env-num-candidates` docs each timestep. If set, the env will only sample `--env-num-candidates` once at the beginning and the agent always has to pick `--env-slate-size` docs from this sample.')
