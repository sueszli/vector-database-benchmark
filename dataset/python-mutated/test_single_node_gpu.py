...
@pytest.mark.skipif(literal_eval(os.getenv('TEST_SAGEMAKER', 'False')) is not True, reason='Skipping test because should only be run when releasing minor transformers version')
@pytest.mark.usefixtures('sm_env')
@parameterized_class([{'framework': 'pytorch', 'script': 'run_glue.py', 'model_name_or_path': 'distilbert-base-cased', 'instance_type': 'ml.g4dn.xlarge', 'results': {'train_runtime': 650, 'eval_accuracy': 0.6, 'eval_loss': 0.9}}, {'framework': 'tensorflow', 'script': 'run_tf.py', 'model_name_or_path': 'distilbert-base-cased', 'instance_type': 'ml.g4dn.xlarge', 'results': {'train_runtime': 600, 'eval_accuracy': 0.3, 'eval_loss': 0.9}}])
