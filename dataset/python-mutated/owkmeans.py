...
from AnyQt.QtCore import Qt, QTimer, QAbstractTableModel, QModelIndex, QThread, pyqtSlot as Slot
...
from Orange.widgets.utils.annotated_data import ANNOTATED_DATA_SIGNAL_NAME, add_columns
...
class OWKMeans(widget.OWWidget):
    name = 'k-Means'
    description = 'k-Means clustering algorithm with silhouette-based quality estimation.'
    ...
    class Outputs:
        annotated_data = Output(ANNOTATED_DATA_SIGNAL_NAME, Table, default=True, replaces=['Annotated Data'])
        centroids = Output('Centroids', Table)

    class Error(widget.OWWidget.Error):
        failed = widget.Msg('Clustering failed\nError: {}')
        not_enough_data = widget.Msg('Too few ({}) unique data instances for {} clusters')
        no_attributes = widget.Msg('Data is missing features.')

    class Warning(widget.OWWidget.Warning):
        no_silhouettes = widget.Msg('Silhouette scores are not computed for >{} samples'.format(SILHOUETTE_MAX_SAMPLES))
        not_enough_data = widget.Msg('Too few ({}) unique data instances for {} clusters')
        no_sparse_normalization = widget.Msg('Sparse data cannot be normalized')
    INIT_METHODS = (('Initialize with KMeans++', 'k-means++'), ('Random initialization', 'random'))
