...
class redshift_cluster_audit_logging(Check):
    ...
    def execute(self):
        ...
        for cluster in redshift_client.clusters:
            ...
            report.status = 'PASS'
            report.status_extended = f'Redshift Cluster {cluster.arn} has audit logging enabled.'
            if not cluster.logging_enabled:
                report.status = 'FAIL'
                report.status_extended = f'Redshift Cluster {cluster.arn} has audit logging disabled.'
