...
class ExperimentalFeaturesStore(CacheInvalidationWorkerStore):
    ...
    async def list_enabled_features(self, user_id: str) -> FrozenSet[str]:
        ...
        """
        enabled = cast(List[Tuple[str]], await self.db_pool.simple_select_list(table='per_user_experimental_features', keyvalues={'user_id': user_id, 'enabled': True}, retcols=('feature',)))
        return frozenset((feature[0] for feature in enabled))

    async def set_features_for_user(self, user: str, features: Dict['ExperimentalFeature', bool]) -> None:
        """
        ...
        for (feature, enabled) in features.items():
            await self.db_pool.simple_upsert(table='per_user_experimental_features', keyvalues={'feature': feature, 'user_id': user}, values={'enabled': enabled}, insertion_values={'user_id': user, 'feature': feature})
