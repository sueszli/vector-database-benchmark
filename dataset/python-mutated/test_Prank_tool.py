...
if sys.platform == 'win32':
    ...
    likely_dirs = ['', prog_files, os.path.join(prog_files, 'Prank')] + sys.path
    ...
    if 'not found' not in output and 'not recognized' not in output:
        if 'prank' in output.lower():
            prank_exe = 'prank'
if not prank_exe:
    raise MissingExternalDependencyError('Install PRANK if you want to use the Bio.Align.Applications wrapper.')
