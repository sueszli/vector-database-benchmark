...
import numpy as np
from AnyQt.QtWidgets import QSizePolicy, QStyle, QButtonGroup, QRadioButton, QComboBox
...
class OWAggregateColumns(widget.OWWidget):
    ...
    Operations = {'Sum': OpDesc('Sum', np.nansum), 'Product': OpDesc('Product', np.nanprod), 'Min': OpDesc('Minimal value', np.nanmin, True), 'Max': OpDesc('Maximal value', np.nanmax, True), 'Mean': OpDesc('Mean value', np.nanmean, True), 'Variance': OpDesc('Variance', np.nanvar), 'Median': OpDesc('Median', np.nanmedian, True)}
