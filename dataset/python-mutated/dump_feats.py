import argparse
import logging
from examples.textless_nlp.gslm.speech2unit.pretrained.utils import get_and_dump_features
...
if __name__ == '__main__':
    '\n    Example command:\n    python ~/speechbot/clustering/dump_logmelfank_feats.py         --manifest_path /checkpoint/kushall/data/LJSpeech-1.1/asr_input_wavs_16k/train.tsv\n        --out_features_path /checkpoint/kushall/experiments/speechbot/logmelfbank/features/ljspeech/train.npy\n    '
    parser = get_parser()
    args = parser.parse_args()
    logger = get_logger()
    logger.info(args)
    logger.info(f'Extracting {args.feature_type} acoustic features...')
    get_and_dump_features(feature_type=args.feature_type, checkpoint_path=args.checkpoint_path, layer=args.layer, manifest_path=args.manifest_path, sample_pct=args.sample_pct, flatten=True, out_features_path=args.out_features_path)
