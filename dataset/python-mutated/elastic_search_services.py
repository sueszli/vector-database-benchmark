...
ES = elasticsearch.Elasticsearch('%s:%s' % (feconf.ES_HOST, feconf.ES_LOCALHOST_PORT) if feconf.ES_CLOUD_ID is None else None, cloud_id=feconf.ES_CLOUD_ID, http_auth=(feconf.ES_USERNAME, secrets_services.get_secret('ES_PASSWORD')) if feconf.ES_CLOUD_ID else None, timeout=30)
