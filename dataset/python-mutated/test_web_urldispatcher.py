...
@pytest.mark.parametrize('show_index,status,prefix,data', [pytest.param(False, 403, '/', None, id='index_forbidden'), pytest.param(True, 200, '/', b'<html>\n<head>\n<title>Index of /.</title>\n</head>\n<body>\n<h1>Index of /.</h1>\n<ul>\n<li><a href="/my_dir">my_dir/</a></li>\n<li><a href="/my_file">my_file</a></li>\n</ul>\n</body>\n</html>', id='index_root'), pytest.param(True, 200, '/static', b'<html>\n<head>\n<title>Index of /.</title>\n</head>\n<body>\n<h1>Index of /.</h1>\n<ul>\n<li><a href="/static/my_dir">my_dir/</a></li>\n<li><a href="/static/my_file">my_file</a></li>\n</ul>\n</body>\n</html>', id='index_static')])
async def test_access_root_of_static_handler(tmp_path: pathlib.Path, aiohttp_client: AiohttpClient, show_index: bool, status: int, prefix: str, data: Optional[bytes]) -> None:
...
async def test_follow_symlink(tmp_path: pathlib.Path, aiohttp_client: AiohttpClient) -> None:
    data = 'hello world'
...
async def test_access_to_the_file_with_spaces(tmp_path: pathlib.Path, aiohttp_client: AiohttpClient, dir_name: str, filename: str, data: str) -> None:
...
async def test_access_non_existing_resource(tmp_path: pathlib.Path, aiohttp_client: AiohttpClient) -> None:
    app = web.Application()
...
async def test_url_escaping(aiohttp_client: AiohttpClient, registered_path: str, request_url: str) -> None:
    app = web.Application()
...
async def test_unauthorized_folder_access(tmp_path: pathlib.Path, aiohttp_client: AiohttpClient) -> None:
    my_dir = tmp_path / 'my_dir'
    my_dir.mkdir()
    app = web.Application()
...
async def test_access_symlink_loop(tmp_path: pathlib.Path, aiohttp_client: AiohttpClient) -> None:
...
async def test_access_special_resource(tmp_path: pathlib.Path, aiohttp_client: AiohttpClient) -> None:
    app = web.Application()
    with mock.patch('pathlib.Path.__new__') as path_constructor:
        special = MagicMock()
...
async def test_static_head(tmp_path: pathlib.Path, aiohttp_client: AiohttpClient) -> None:
...
async def test_static_absolute_url(aiohttp_client: AiohttpClient, tmp_path: pathlib.Path) -> None:
    app = web.Application()
...
async def test_for_issue_5250(aiohttp_client: AiohttpClient, tmp_path: pathlib.Path) -> None:
    app = web.Application()
...
@pytest.mark.xfail(raises=AssertionError, reason='Regression in v3.7: https://github.com/aio-libs/aiohttp/issues/5621')
@pytest.mark.parametrize(('route_definition', 'urlencoded_path', 'expected_http_resp_status'), (('/467,802,24834/hello', '/467%2C802%2C24834/hello', 200), ('/{user_ids:([0-9]+)(,([0-9]+))*}/hello', '/467%2C802%2C24834/hello', 200), ('/1%2C3/hello', '/1%2C3/hello', 404)), ids=('urldecoded_route', 'urldecoded_route_with_regex', 'urlencoded_route'))
async def test_decoded_url_match(aiohttp_client: AiohttpClient, route_definition: str, urlencoded_path: str, expected_http_resp_status: int) -> None:
    app = web.Application()
