...
def test_wrong_input_shapes():
    ...
    with raises(ValueError, match='Input data shapes should be the same, but given'):
...
def test_zero_div():
    ...
    m = MeanAbsoluteRelativeError()
    with raises(NotComputableError, match='The ground truth has 0'):
        m.update((a, ground_truth))
...
def test_zero_sample():
    if False:
        return 10
    m = MeanAbsoluteRelativeError()
    with raises(NotComputableError, match='MeanAbsoluteRelativeError must have at least one sample'):
        m.compute()
