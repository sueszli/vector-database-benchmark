...
class ReadDTO(DataclassDTO[Person]):
    config = DTOConfig(exclude={'email', 'address.street', 'children.0.email', 'children.0.address'}, rename_fields={'address': 'location'})
...
def get_person(name: str) -> Person:
    ...
    return Person(name=name, age=30, email=f'email_of_{name}@example.com', address=address, children=[child1, child2])
