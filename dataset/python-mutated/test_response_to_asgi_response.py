...
async def test_to_response_async_await(anyio_backend: str) -> None:
    ...
    handler._signature_model = SignatureModel.create(dependency_name_set=set(), fn=handler.fn, data_dto=None, parsed_signature=ParsedSignature.from_fn(handler.fn, {}), type_decoders=[])
    response = await handler.to_response(data=handler.fn(data=person_instance), app=Litestar(route_handlers=[handler]), request=RequestFactory().get(route_handler=handler))
    assert loads(response.body) == msgspec.to_builtins(person_instance)
...
async def test_to_response_returning_litestar_response() -> None:
    ...
    with create_test_client(handler) as client:
        ...
        response = await route_handler.to_response(data=route_handler.fn(), app=client.app, request=RequestFactory().get())
        assert isinstance(response, ASGIResponse)

@pytest.mark.parametrize('expected_response', [StarletteResponse(status_code=HTTP_200_OK, content=b'abc'), PlainTextResponse(content='abc'), HTMLResponse(content='<div><span/></div'), JSONResponse(status_code=HTTP_200_OK, content={})])
async def test_to_response_returning_starlette_response(expected_response: StarletteResponse, anyio_backend: str) -> None:
...
@pytest.mark.parametrize('iterator, should_raise', [[my_generator(), False], [my_async_generator(), False], [MySyncIterator(), False], [MyAsyncIterator(), False], [my_generator, False], [my_async_generator, False], [MyAsyncIterator, False], [MySyncIterator, False], [[1, 2, 3, 4], False], ['abc', False], [b'abc', False], [{'key': 1}, False], [[{'key': 1}], False], [1, True], [None, True]])
...
async def test_to_response_template_response(anyio_backend: str, tmp_path: Path) -> None:
    ...
    app = Litestar(route_handlers=[], template_config=TemplateConfig(engine=JinjaTemplateEngine, directory=tmp_path))
    with create_test_client(handler, template_config=TemplateConfig(engine=JinjaTemplateEngine, directory=tmp_path)) as client:
        route: HTTPRoute = client.app.routes[0]
        route_handler = route.route_handlers[0]
        response = await route_handler.to_response(data=route_handler.fn(), app=client.app, request=RequestFactory(app=app).get())
        assert isinstance(response, ASGIResponse)
        encoded_headers = response.encode_headers()
        assert (b'local-header', b'123') in encoded_headers
        assert (b'response-header', b'abc') in encoded_headers
...
@pytest.mark.parametrize('content', [my_generator(), my_async_generator(), MySyncIterator(), MyAsyncIterator(), my_generator, my_async_generator, MyAsyncIterator, MySyncIterator, [1, 2, 3, 4], 'abc', b'abc', {'key': 1}, [{'key': 1}]])
...
@pytest.mark.parametrize('content', [my_generator(), my_async_generator(), my_generator, my_async_generator, MySyncIterator(), MyAsyncIterator(), MyAsyncIterator, MySyncIterator])
