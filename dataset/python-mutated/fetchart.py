...
class AlbumArtOrg(RemoteArtSource):
    ...
    PAT = 'href\\s*=\\s*"([^>"]*)"[^>]*title\\s*=\\s*"View larger image"'
...
class LastFM(RemoteArtSource):
    NAME = 'Last.fm'
    SIZES = OrderedDict([('mega', (300, 300)), ('extralarge', (300, 300)), ('large', (174, 174)), ('medium', (64, 64)), ('small', (34, 34))])
...
class CoverArtUrl(RemoteArtSource):
    ...
    def get(self, album, plugin, paths):
        ...
        if image_url:
            ...
            self._log.debug(f'Cover art URL not found for {album}')
            return
SOURCES_ALL = ['filesystem', 'coverart', 'itunes', 'amazon', 'albumart', 'wikipedia', 'google', 'fanarttv', 'lastfm', 'spotify']
ART_SOURCES = {'filesystem': FileSystem, 'coverart': CoverArtArchive, 'itunes': ITunesStore, 'albumart': AlbumArtOrg, 'amazon': Amazon, 'wikipedia': Wikipedia, 'google': GoogleImages, 'fanarttv': FanartTV, 'lastfm': LastFM, 'spotify': Spotify, 'cover_art_url': CoverArtUrl}
SOURCE_NAMES = {v: k for (k, v) in ART_SOURCES.items()}

class FetchArtPlugin(plugins.BeetsPlugin, RequestMixin):
    PAT_PX = '(0|[1-9][0-9]*)px'
    PAT_PERCENT = '(100(\\.00?)?|[1-9]?[0-9](\\.[0-9]{1,2})?)%'
