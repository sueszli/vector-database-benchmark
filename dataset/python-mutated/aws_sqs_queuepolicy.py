...
class SQSQueuePolicyProvider(ResourceProvider[SQSQueuePolicyProperties]):
    ...
    def create(self, request: ResourceRequest[SQSQueuePolicyProperties]) -> ProgressEvent[SQSQueuePolicyProperties]:
        if False:
            for i in range(10):
                print('nop')
        ...
        physical_resource_id = util.generate_default_name(stack_name=request.stack_name, logical_resource_id=request.logical_resource_id)
        model['Id'] = physical_resource_id
        return ProgressEvent(status=OperationStatus.SUCCESS, resource_model=model, custom_context=request.custom_context)

    def read(self, request: ResourceRequest[SQSQueuePolicyProperties]) -> ProgressEvent[SQSQueuePolicyProperties]:
        if False:
            for i in range(10):
                print('nop')
    ...
    def delete(self, request: ResourceRequest[SQSQueuePolicyProperties]) -> ProgressEvent[SQSQueuePolicyProperties]:
        if False:
            for i in range(10):
                print('nop')
        '\n        Delete a resource\n        '
    ...
    def update(self, request: ResourceRequest[SQSQueuePolicyProperties]) -> ProgressEvent[SQSQueuePolicyProperties]:
        if False:
            i = 10
            return i + 15
        '\n        Update a resource\n        '
