...
class K8sBlockExecutor(BlockExecutor):
    ...
    def _execute(self, block_run_id: int=None, global_vars: Dict=None, **kwargs) -> None:
        if False:
            return 10
        ...
        job_manager = K8sJobManager(job_name=f'mage-{job_name_prefix}-block-{block_run_id}', logger=self.logger, logging_tags=kwargs.get('tags', dict()), namespace=namespace)
