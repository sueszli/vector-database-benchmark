...
def _get_policy_adm_setting(policy_name, policy_class, return_full_policy_names, hierarchical_return):
    ...
    (success, policy_obj, _, _) = salt.modules.win_lgpo._lookup_admin_template(policy_name=policy_name, policy_class=policy_class, adml_language='en-US')
    if success:
        return salt.modules.win_lgpo._get_policy_adm_setting(admx_policy=policy_obj, policy_class=policy_class, adml_language='en-US', return_full_policy_names=return_full_policy_names, hierarchical_return=hierarchical_return)
...
def test_not_configured_hierarchical():
    ...
    expected = {'Computer Configuration': {'Administrative Templates': {'Printers': {'PointAndPrint_Restrictions_Win7': 'Not Configured'}}}}
    assert result == expected
...
def test_not_configured_full_names_hierarchical():
    ...
    expected = {'Computer Configuration': {'Administrative Templates': {'Printers': {'Point and Print Restrictions': 'Not Configured'}}}}
    assert result == expected