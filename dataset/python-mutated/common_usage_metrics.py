...
current_dau_gauge = Gauge('synapse_admin_daily_active_users', 'Current daily active users count')
...
class CommonUsageMetricsManager:
    ...
    async def setup(self) -> None:
        ...
        run_as_background_process(desc='common_usage_metrics_update_gauges', func=self._update_gauges)
        self._clock.looping_call(run_as_background_process, 5 * 60 * 1000, desc='common_usage_metrics_update_gauges', func=self._update_gauges)
    ...
    async def _collect(self) -> CommonUsageMetrics:
        ...
        return CommonUsageMetrics(daily_active_users=dau_count)
