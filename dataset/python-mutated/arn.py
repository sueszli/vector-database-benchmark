...
from prowler.providers.aws.lib.arn.error import RoleArnParsingEmptyResource, RoleArnParsingIAMRegionNotEmpty, RoleArnParsingInvalidAccountID, RoleArnParsingInvalidResourceType, RoleArnParsingPartitionEmpty, RoleArnParsingServiceNotIAMnorSTS
...
def parse_iam_credentials_arn(arn: str) -> ARN:
    ...
    elif arn_parsed.partition is None or arn_parsed.partition == '':
    ...
    elif arn_parsed.account_id is None or len(arn_parsed.account_id) != 12 or (not arn_parsed.account_id.isnumeric()):
        raise RoleArnParsingInvalidAccountID
    elif arn_parsed.resource_type != 'role' and arn_parsed.resource_type != 'user' and (arn_parsed.resource_type != 'assumed-role'):
...
def is_valid_arn(arn: str) -> bool:
    ...
    regex = '^arn:aws(-cn|-us-gov|-iso|-iso-b)?:[a-zA-Z0-9\\-]+:([a-z]{2}-[a-z]+-\\d{1})?:(\\d{12})?:[a-zA-Z0-9\\-_\\/:\\.]+(:\\d+)?$'
