...
from .rtcrtpparameters import RTCRtpCapabilities, RTCRtpCodecParameters, RTCRtpReceiveParameters
from .rtp import RTCP_PSFB_APP, RTCP_PSFB_PLI, RTCP_RTPFB_NACK, RTP_HISTORY_SIZE, AnyRtcpPacket, RtcpByePacket, RtcpPsfbPacket, RtcpReceiverInfo, RtcpRrPacket, RtcpRtpfbPacket, RtcpSrPacket, RtpPacket, clamp_packets_lost, pack_remb_fci, unwrap_rtx
from .stats import RTCInboundRtpStreamStats, RTCRemoteOutboundRtpStreamStats, RTCStatsReport
...
class RTCRtpReceiver:
    ...
    async def getStats(self) -> RTCStatsReport:
        ...
        for (ssrc, stream) in self.__remote_streams.items():
            self.__stats.add(RTCInboundRtpStreamStats(timestamp=clock.current_datetime(), type='inbound-rtp', id='inbound-rtp_' + str(id(self)), ssrc=ssrc, kind=self.__kind, transportId=self.transport._stats_id, packetsReceived=stream.packets_received, packetsLost=stream.packets_lost, jitter=stream.jitter))
    ...
    async def receive(self, parameters: RTCRtpReceiveParameters) -> None:
        ...
        if not self.__started:
            ...
            self.__decoder_thread = threading.Thread(target=decoder_worker, name=self.__kind + '-decoder', args=(asyncio.get_event_loop(), self.__decoder_queue, self._track._queue))
    ...
    async def _handle_rtcp_packet(self, packet: AnyRtcpPacket) -> None:
        ...
        if isinstance(packet, RtcpSrPacket):
            self.__stats.add(RTCRemoteOutboundRtpStreamStats(timestamp=clock.current_datetime(), type='remote-outbound-rtp', id=f'remote-outbound-rtp_{id(self)}', ssrc=packet.ssrc, kind=self.__kind, transportId=self.transport._stats_id, packetsSent=packet.sender_info.packet_count, bytesSent=packet.sender_info.octet_count, remoteTimestamp=clock.datetime_from_ntp(packet.sender_info.ntp_timestamp)))
            self.__lsr[packet.ssrc] = packet.sender_info.ntp_timestamp >> 16 & 4294967295
    ...
    async def _handle_rtp_packet(self, packet: RtpPacket, arrival_time_ms: int) -> None:
        ...
        if self.__remote_bitrate_estimator is not None:
            if packet.extensions.abs_send_time is not None:
                remb = self.__remote_bitrate_estimator.add(abs_send_time=packet.extensions.abs_send_time, arrival_time_ms=arrival_time_ms, payload_size=len(packet.payload) + packet.padding_size, ssrc=packet.ssrc)
                if self.__rtcp_ssrc is not None and remb is not None:
                    rtcp_packet = RtcpPsfbPacket(fmt=RTCP_PSFB_APP, ssrc=self.__rtcp_ssrc, media_ssrc=0, fci=pack_remb_fci(*remb))
        ...
        if is_rtx(codec):
            ...
            packet = unwrap_rtx(packet, payload_type=codec.payloadType, ssrc=original_ssrc)
        ...
        try:
            while True:
                ...
                for (ssrc, stream) in self.__remote_streams.items():
                    ...
                    reports.append(RtcpReceiverInfo(ssrc=ssrc, fraction_lost=stream.fraction_lost, packets_lost=stream.packets_lost, highest_sequence=stream.max_seq, jitter=stream.jitter, lsr=lsr, dlsr=dlsr))
    ...
    async def _send_rtcp_nack(self, media_ssrc: int, lost: List[int]) -> None:
        ...
        if self.__rtcp_ssrc is not None:
            packet = RtcpRtpfbPacket(fmt=RTCP_RTPFB_NACK, ssrc=self.__rtcp_ssrc, media_ssrc=media_ssrc)
            packet.lost = lost
            await self._send_rtcp(packet)
