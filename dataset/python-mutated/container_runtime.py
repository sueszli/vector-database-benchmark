...
class ContainerRuntime(object):
    ...
    async def execute_cmd(self, **kwargs) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        """
        process = await asyncio.create_subprocess_shell(**kwargs, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        ...
        self.logger.debug(f'Executed a command with return code: {returncode} command: {kwargs}')
    ...
    async def image_exists(self, image_name: str) -> bool:
        ...
        if self.container_runtime == RuntimeType.Docker:
            ...
            cmd = f'crictl -r unix://{self.container_runtime_socket} inspecti -q {image_name}'
            (result, _, _) = await self.execute_cmd(cmd=cmd)
        self.logger.debug(f"Checked existence of image '{image_name}': exists = {result}")
        return result
    ...
    async def _pull_image_for_docker_with_buildah(self, image_name: str) -> bool:
        ...
        cmd = f"buildah pull --tls-verify=false {image_name} && buildah push --disable-compression '{image_name}' docker-daemon:{image_name} && buildah rmi {image_name} && buildah rmi -p"
    ...
    async def _pull_image(self, image_name: str) -> bool:
        ...
        if self.container_runtime == RuntimeType.Docker:
            try:
                ...
                result = False
                if 'certificate' in e.message.lower() or 'timeout exceeded' in e.message.lower():
                    result = await self._pull_image_for_docker_with_buildah(image_name)
        elif self.container_runtime == RuntimeType.Containerd:
            cmd = f'ctr -n k8s.io -a {self.container_runtime_socket} i pull {image_name} --skip-verify '
            (result, _, _) = await self.execute_cmd(cmd=cmd)
        return result
    ...
    async def _push_image_for_docker_with_buildah(self, image_name: str) -> bool:
        ...
        cmd = f"buildah pull docker-daemon:{image_name} && buildah push --tls-verify=false '{image_name}' && buildah rmi {image_name} && buildah rmi -p"
        (success, _, _) = await self.execute_cmd(cmd=cmd)
        return success
        ...
        if self.container_runtime == RuntimeType.Docker:
            try:
                ...
                if _failed_to_reach_registry(aiodocker_resp):
                    self.logger.warning('Failed to reach the registry, trying push through buildah.')
            ...
            cmd = f'ctr -n k8s.io -a {self.container_runtime_socket} i push {image_name} --skip-verify '
