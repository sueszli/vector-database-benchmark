...
class sfp_securitytrails(SpiderFootPlugin):
    meta = {'name': 'SecurityTrails', 'summary': 'Obtain Passive DNS and other information from SecurityTrails', 'flags': ['apikey'], 'useCases': ['Investigate', 'Passive'], 'categories': ['Search Engines'], 'dataSource': {'website': 'https://securitytrails.com/', 'model': 'FREE_AUTH_LIMITED', 'references': ['https://docs.securitytrails.com/docs', 'https://docs.securitytrails.com/reference#general'], 'apiKeyInstructions': ['Visit https://securitytrails.com', 'Register a free account', 'Navigate to https://securitytrails.com/app/account/credentials', "The API key is listed under 'API Key'"], 'favIcon': 'https://securitytrails.com/user/themes/lego/favicon/apple-touch-icon.png', 'logo': 'https://securitytrails.com/user/themes/lego/favicon/apple-touch-icon.png', 'description': 'Data for Security companies, researchers and teams. Fast, always up API that allows you to access current and historical data. The API is paid via a simple pricing structure that allows you to embed our data into your applications.\nSearch nearly 3 billion historical and current WHOIS data and WHOIS changes.'}}
    opts = {'api_key': '', 'verify': True, 'cohostsamedomain': False, 'maxcohost': 100}
    optdescs = {'api_key': 'SecurityTrails API key.', 'verify': 'Verify co-hosts are valid by checking if they still resolve to the shared IP.', 'cohostsamedomain': 'Treat co-hosted sites on the same target domain as co-hosting?', 'maxcohost': 'Stop reporting co-hosted sites after this many are found, as it would likely indicate web hosting.'}
    results = None
    errorState = False
    cohostcount = 0
