...
def python_prior_box(input, image, min_sizes, max_sizes=None, aspect_ratios=[1.0], variances=[0.1, 0.1, 0.2, 0.2], flip=False, clip=False, step_w=0, step_h=0, offset=0.5, min_max_aspect_ratios_order=False, name=None):
    if False:
        i = 10
        return i + 15
    return paddle.vision.ops.prior_box(input, image, min_sizes=min_sizes, max_sizes=max_sizes, aspect_ratios=aspect_ratios, variance=variances, flip=flip, clip=clip, steps=[step_w, step_h], offset=offset, name=name, min_max_aspect_ratios_order=min_max_aspect_ratios_order)
...
class TestPriorBoxOp(OpTest):
    ...
    def set_data(self):
        ...
        self.attrs = {'min_sizes': self.min_sizes, 'aspect_ratios': self.aspect_ratios, 'variances': self.variances, 'flip': self.flip, 'clip': self.clip, 'step_w': self.step_w, 'step_h': self.step_h, 'offset': self.offset, 'min_max_aspect_ratios_order': self.min_max_aspect_ratios_order}
...
class TestPriorBoxAPI(unittest.TestCase):
    ...
    def test_dygraph_with_static(self):
        ...
        (box, var) = paddle.vision.ops.prior_box(input=input, image=image, min_sizes=self.min_sizes, clip=True, flip=True)
        exe = paddle.static.Executor()
        (box_np, var_np) = exe.run(paddle.static.default_main_program(), feed={'input': self.input_np, 'image': self.image_np}, fetch_list=[box, var])
        paddle.disable_static()
        inputs_dy = paddle.to_tensor(self.input_np)
        image_dy = paddle.to_tensor(self.image_np)
        (box_dy, var_dy) = paddle.vision.ops.prior_box(input=inputs_dy, image=image_dy, min_sizes=self.min_sizes, clip=True, flip=True)
