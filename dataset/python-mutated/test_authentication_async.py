...
from azure.core.pipeline.policies import AsyncBearerTokenCredentialPolicy, SansIOHTTPPolicy, AsyncRedirectPolicy, SensitiveHeaderCleanupPolicy
...
async def test_bearer_policy_token_caching(http_request):
    ...
    policies = [AsyncBearerTokenCredentialPolicy(credential, 'scope'), Mock(send=Mock(return_value=get_completed_future(Mock())))]
    pipeline = AsyncPipeline(transport=Mock, policies=policies)
    ...
    policies = [AsyncBearerTokenCredentialPolicy(credential, 'scope'), Mock(send=lambda _: get_completed_future(Mock()))]
    pipeline = AsyncPipeline(transport=Mock(), policies=policies)
    await pipeline.run(http_request('GET', 'https://spam.eggs'))
    assert get_token_calls == 1
    await pipeline.run(http_request('GET', 'https://spam.eggs'))
    assert get_token_calls == 2
...
async def test_bearer_policy_optionally_enforces_https(http_request):
    ...
    pipeline = AsyncPipeline(transport=Mock(send=assert_option_popped), policies=[AsyncBearerTokenCredentialPolicy(credential, 'scope')])
