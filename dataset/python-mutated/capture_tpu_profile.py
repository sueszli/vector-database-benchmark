...
FLAGS = flags.FLAGS
flags.DEFINE_string('gcp_project', None, 'Project name for the Cloud TPU-enabled project. If not specified, we will attempt to automatically detect the GCE project from metadata.')
flags.DEFINE_string('tpu_zone', None, help='GCE zone where the Cloud TPU is located in. If not specified, we will attempt to automatically detect the GCE project from metadata.')
flags.DEFINE_string('tpu', None, 'Name of the Cloud TPU for Cluster Resolvers. You must specify either this flag or --service_addr.')
flags.DEFINE_string('service_addr', None, 'Address of TPU profiler service e.g. localhost:8466, you must specify either this flag or --tpu.')
flags.DEFINE_string('workers_list', None, 'The list of worker TPUs that we are about to profile e.g. 10.0.1.2:8466, 10.0.1.3:8466. You can specify this flag with --tpu or --service_addr to profile a subset of tpu nodes. You can also use only--tpu and leave this flag unspecified to profile all the tpus.')
flags.DEFINE_string('logdir', None, 'Path of TensorBoard log directory e.g. /tmp/tb_log, gs://tb_bucket')
flags.DEFINE_integer('duration_ms', 0, 'Duration of tracing or monitoring in ms.')
flags.DEFINE_integer('num_tracing_attempts', 3, 'Automatically retry N times when no trace event is collected.')
flags.DEFINE_boolean('include_dataset_ops', True, 'Deprecated.')
flags.DEFINE_integer('host_tracer_level', 2, 'Adjust host tracer level to control the verbosity  of the TraceMe event being collected.')
flags.DEFINE_integer('monitoring_level', 0, 'Choose a monitoring level between 1 and 2 to monitor your TPU job continuously. Level 2 is more verbose than level 1 and shows more metrics.')
flags.DEFINE_integer('num_queries', 100, 'This script will run monitoring for num_queries before it stops.')
