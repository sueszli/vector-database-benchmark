...
class SavingsPlanOrderOperations:
    ...
    async def get(self, savings_plan_order_id: str, **kwargs: Any) -> _models.SavingsPlanOrderModel:
        ...
        request = build_get_request(savings_plan_order_id=savings_plan_order_id, expand=self._config.expand, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
    ...
    async def elevate(self, savings_plan_order_id: str, **kwargs: Any) -> _models.RoleAssignmentEntity:
        ...
        request = build_elevate_request(savings_plan_order_id=savings_plan_order_id, api_version=api_version, template_url=self.elevate.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=False, **kwargs)
        response = pipeline_response.http_response
