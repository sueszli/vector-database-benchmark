...
from transformers import FlaxWav2Vec2ForPreTraining, HfArgumentParser, TrainingArguments, Wav2Vec2Config, Wav2Vec2FeatureExtractor, is_tensorboard_available
...
class ModelArguments:
    ...
    min_gumbel_temperature: Optional[float] = field(default=0.1, metadata={'help': 'Minimum temperature for gumbel softmax.'})
...
class DataTrainingArguments:
    ...
    max_duration_in_seconds: Optional[float] = field(default=20.0, metadata={'help': 'Filter audio files that are longer than `max_duration_in_seconds` seconds'})
    pad_to_multiple_of: Optional[int] = field(default=1024, metadata={'help': 'If set will pad the sequence to a multiple of the provided value. This is important to avoid triggering recompilations on TPU'})
