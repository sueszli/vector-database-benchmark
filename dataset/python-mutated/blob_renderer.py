...
async def render_blob(datasette, database, rows, columns, request, table, view_name):
    ...
    if _BLOB_HASH in request.args:
        ...
        for row in rows:
            ...
            raise BadRequest('Link has expired - the requested binary content has changed or could not be found.')
    else:
        row = rows[0]
    value = row[blob_column]
    filename_bits = []
    ...
    headers = {'X-Content-Type-Options': 'nosniff', 'Content-Disposition': f'attachment; filename="{filename}"'}
    return Response(body=value or b'', status=200, headers=headers, content_type='application/binary')

@hookimpl
def register_output_renderer():
    if False:
        return 10
    return {'extension': 'blob', 'render': render_blob, 'can_render': lambda : False}