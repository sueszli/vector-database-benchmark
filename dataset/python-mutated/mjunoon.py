...
@pluginmatcher(re.compile('https?://(?:www\\.)?mjunoon\\.tv/(?:watch/)?([\\w-]+)'))
class Mjunoon(Plugin):
    ...
    is_live_channel_re = re.compile('"isLiveBroadcast":\\s*"(true|undefined)"')
    main_chunk_js_url_re = re.compile('<script src="(/static/js/main\\.\\w+\\.chunk\\.js)"></script>')
    js_credentials_re = re.compile('data:{email:"(?P<email>.*?)",password:"(?P<password>.*?)"}')
    js_cipher_data_re = re.compile('createDecipheriv\\("(?P<algorithm>.*?)","(?P<key>.*?)","(?P<iv>.*?)"\\)')
    token_schema = validate.Schema({'token': str, 'token_type': str, 'expires_in': int})
    encrypted_data_schema = validate.Schema({'eData': str}, validate.get('eData'))
    stream_schema = validate.Schema({'data': {'live_stream_url': validate.url(), 'channel_name': str, 'meta_title': validate.any(None, str), 'genres': validate.all(validate.transform(lambda x: x.split(',')[0]), str)}}, validate.get('data'))
    encryption_algorithm = {'aes-256-cbc': AES.MODE_CBC}
