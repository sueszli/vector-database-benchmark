...
IGNORED = {'do_xcom_push', 'email_on_failure', 'email_on_retry', 'post_execute', 'pre_execute', 'default_args', 'task_concurrency', 'HIDE_ATTRS_FROM_UI', '_dag', 'output', 'partial', 'shallow_copy_attrs', 'expand_input', 'partial_kwargs'}
...
BO_CLS = next((node for node in ast.iter_child_nodes(BO_MOD) if isinstance(node, ast.ClassDef) and node.name == 'BaseOperator'))
BO_INIT = next((node for node in ast.iter_child_nodes(BO_CLS) if isinstance(node, ast.FunctionDef) and node.name == '__init__'))
BO_PARTIAL = next((node for node in ast.iter_child_nodes(BO_MOD) if isinstance(node, ast.FunctionDef) and node.name == 'partial'))
MO_CLS = next((node for node in ast.iter_child_nodes(MO_MOD) if isinstance(node, ast.ClassDef) and node.name == 'MappedOperator'))
...
if __name__ == '__main__':
    results = [check_baseoperator_partial_arguments(), check_operator_member_parity()]
