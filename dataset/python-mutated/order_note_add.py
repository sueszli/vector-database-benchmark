...
from ...core.descriptions import ADDED_IN_315, DEPRECATED_IN_3X_INPUT, DEPRECATED_IN_3X_MUTATION, PREVIEW_FEATURE
...
class OrderNoteAdd(OrderNoteCommon):
    ...
    class Arguments(OrderNoteCommon.Arguments):
        id = graphene.ID(required=True, description='ID of the order to add a note for.', name='order')
    ...
    def perform_mutation(cls, _root, info: ResolveInfo, /, *, id: str, input):
        ...
        with transaction.atomic():
            event = events.order_note_added_event(order=order, user=info.context.user, app=app, message=cleaned_input['message'])
...
class OrderAddNoteInput(BaseInputObjectType):
    message = graphene.String(description='Note message.' + DEPRECATED_IN_3X_INPUT, name='message', required=True)
...
class OrderAddNote(OrderNoteAdd):
    ...
    class Arguments(OrderNoteAdd.Arguments):
        input = OrderAddNoteInput(required=True, description='Fields required to create a note for the order.')
