...
class EditChatInviteLink(TelegramMethod[ChatInviteLink]):
    ...
    if TYPE_CHECKING:
        ...
        def __init__(__pydantic__self__, *, chat_id: Union[int, str], invite_link: str, name: Optional[str]=None, expire_date: Optional[Union[datetime.datetime, datetime.timedelta, int]]=None, member_limit: Optional[int]=None, creates_join_request: Optional[bool]=None, **__pydantic_kwargs: Any) -> None:
            if False:
                return 10
            super().__init__(chat_id=chat_id, invite_link=invite_link, name=name, expire_date=expire_date, member_limit=member_limit, creates_join_request=creates_join_request, **__pydantic_kwargs)