...
class Test_iam_role_cross_account_readonlyaccess_policy:
    ...
    def test_no_roles(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_role_cross_account_readonlyaccess_policy.iam_role_cross_account_readonlyaccess_policy.iam_client', new=IAM(current_audit_info)):
            from prowler.providers.aws.services.iam.iam_role_cross_account_readonlyaccess_policy.iam_role_cross_account_readonlyaccess_policy import iam_role_cross_account_readonlyaccess_policy
    ...
    def test_role_without_readonlyaccess_policy(self):
        ...
        iam = client('iam')
        role_name = 'test'
        assume_role_policy_document = {'Version': '2012-10-17', 'Statement': {'Sid': 'test', 'Effect': 'Allow', 'Principal': {'AWS': f'arn:aws:iam::{AWS_ACCOUNT_ID}:root'}, 'Action': 'sts:AssumeRole'}}
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_role_cross_account_readonlyaccess_policy.iam_role_cross_account_readonlyaccess_policy.iam_client', new=IAM(current_audit_info)):
            ...
            assert result[0].status_extended == 'IAM Role test does not have ReadOnlyAccess policy.'
    ...
    def test_internal_role_with_readonlyaccess_policy(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_role_cross_account_readonlyaccess_policy.iam_role_cross_account_readonlyaccess_policy.iam_client', new=IAM(current_audit_info)):
            ...
            assert len(result) == 1
            assert result[0].status == 'PASS'
            assert result[0].status_extended == 'IAM Role test has read-only access but is not cross account.'
            assert result[0].resource_id == 'test'
            assert result[0].resource_arn == response['Role']['Arn']
            assert result[0].resource_tags == []
    ...
    def test_cross_account_role_with_readonlyaccess_policy(self):
        ...
        iam = client('iam')
        role_name = 'test'
        assume_role_policy_document = {'Version': '2012-10-17', 'Statement': {'Sid': 'test', 'Effect': 'Allow', 'Principal': {'AWS': 'arn:aws:iam::012345678910:root'}, 'Action': 'sts:AssumeRole'}}
        response = iam.create_role(RoleName=role_name, AssumeRolePolicyDocument=dumps(assume_role_policy_document))
        iam.attach_role_policy(RoleName=role_name, PolicyArn='arn:aws:iam::aws:policy/ReadOnlyAccess')
        current_audit_info = self.set_mocked_audit_info()
        from prowler.providers.aws.services.iam.iam_service import IAM
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_role_cross_account_readonlyaccess_policy.iam_role_cross_account_readonlyaccess_policy.iam_client', new=IAM(current_audit_info)):
            ...
            assert result[0].status_extended == 'IAM Role test gives cross account read-only access.'
            assert result[0].resource_id == 'test'
            assert result[0].resource_arn == response['Role']['Arn']
            assert result[0].resource_tags == []
    ...
    def test_only_aws_service_linked_roles(self):
        ...
        current_audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_role_cross_account_readonlyaccess_policy.iam_role_cross_account_readonlyaccess_policy.iam_client', new=iam_client):
            from prowler.providers.aws.services.iam.iam_role_cross_account_readonlyaccess_policy.iam_role_cross_account_readonlyaccess_policy import iam_role_cross_account_readonlyaccess_policy
            check = iam_role_cross_account_readonlyaccess_policy()
            result = check.execute()
            assert len(result) == 0