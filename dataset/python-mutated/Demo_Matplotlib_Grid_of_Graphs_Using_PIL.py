...
col_listbox = [[sg.Listbox(values=list(fig_dict.keys()), select_mode=sg.SELECT_MODE_EXTENDED, enable_events=True, size=(28, len(list(fig_dict))), key='-LISTBOX-')], [sg.Exit(size=(5, 2))]]
image_col = sg.Col([[sg.Image(k=(i, j)) for i in range(4)] for j in range(4)])
layout = [[sg.Text('Matplotlib Grid of Plots Using PIL', font='current 18')], [sg.Col(col_listbox, element_justification='c'), image_col]]
