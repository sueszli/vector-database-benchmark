...
@pluginmatcher(re.compile('https?://(www\\.)?clubbingtv\\.com/'))
@pluginargument('username', required=True, requires=['password'], help='The username used to register with Clubbing TV.')
@pluginargument('password', required=True, sensitive=True, help='A Clubbing TV account password to use with --clubbingtv-username.')
class ClubbingTV(Plugin):
    _login_url = 'https://www.clubbingtv.com/user/login'
    _live_re = re.compile('playerInstance\\.setup\\({\\s*"file"\\s*:\\s*"(?P<stream_url>.+?)"', re.DOTALL)
    _vod_re = re.compile('<iframe src="(?P<stream_url>.+?)"')
