...
from typing import TYPE_CHECKING, Any, Callable, Dict, Iterable, List, Literal, Mapping, Optional, Tuple, Type, Union, cast
...
from .helpers import BaseTimerContext, BasicAuth, HeadersMixin, TimerNoop, basicauth_from_netrc, is_expected_content_type, netrc_from_env, noop, parse_mimetype, reify, set_result
    ...
    async def json(self, *, encoding: Optional[str]=None, loads: JSONDecoder=DEFAULT_JSON_DECODER, content_type: Optional[str]='application/json') -> Any:
        ...
        if content_type:
            if not is_expected_content_type(self.content_type, content_type):
                raise ContentTypeError(self.request_info, self.history, message='Attempt to decode JSON with unexpected mimetype: %s' % self.content_type, headers=self.headers)
        if encoding is None:
            encoding = self.get_encoding()
