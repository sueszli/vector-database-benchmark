...
def buildModelCoeffVarimpCheck(train_data, y, gamX, family):
    ...
    numPCoeffs += numKnots[0] + spline_orders[0] - 2
    numPCoeffs += numKnots[1] - 1
    numPCoeffs += numKnots[2] + spline_orders[2] - 2 - 1
    numPCoeffs += 1
    h2o_model = H2OGeneralizedAdditiveEstimator(family=family, gam_columns=gamX, scale=[1, 1, 1], bs=[2, 0, 3], spline_orders=[10, -1, 10], num_knots=numKnots)
    h2o_model.train(x=x, y=y, training_frame=train_data)
    h2oCoeffs = h2o_model.coef()
    assert len(h2oCoeffs) == numPCoeffs, 'expected number of coefficients: {0}, actual number of coefficients: {1}'.format(numPCoeffs, len(h2oCoeffs))
    h2oCoeffsStandardized = h2o_model.coef_norm()
    assert len(h2oCoeffsStandardized) == numPCoeffs, 'expected number of coefficients: {0}, actual number of coefficients:{1}'.format(numPCoeffs, len(h2oCoeffsStandardized))
