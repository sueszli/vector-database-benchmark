...
class DialogModelingTest(unittest.TestCase):
    ...
    def generate_and_print_dialog_response(self, pipelines: List[DialogModelingPipeline]):
        ...
        for (step, item) in enumerate(self.test_case['sng0073']['log']):
            ...
            result = pipelines[step % pipeline_len]({'user_input': user, 'history': result})
    ...
    def test_run_by_direct_model_download(self):
        ...
        model = SpaceForDialogModeling(model_dir=cache_path, text_field=preprocessor.text_field, config=preprocessor.config)
        pipelines = [DialogModelingPipeline(model=model, preprocessor=preprocessor)]
