...
class Test_acm_certificates_transparency_logs_enabled:
    ...
    def test_acm_certificate_with_logging(self):
        ...
        acm_client = mock.MagicMock
        acm_client.certificates = [Certificate(arn=certificate_arn, id=certificate_id, name=certificate_name, type=certificate_type, expiration_days=365, transparency_logging=True, region=AWS_REGION)]
        with mock.patch('prowler.providers.aws.services.acm.acm_service.ACM', new=acm_client):
            from prowler.providers.aws.services.acm.acm_certificates_transparency_logs_enabled.acm_certificates_transparency_logs_enabled import acm_certificates_transparency_logs_enabled
            check = acm_certificates_transparency_logs_enabled()
            result = check.execute()
            ...
            assert result[0].status_extended == f'ACM Certificate {certificate_id} for {certificate_name} has Certificate Transparency logging enabled.'
    ...
    def test_acm_certificate_without_logging(self):
        ...
        certificate_type = 'AMAZON_ISSUED'
        acm_client = mock.MagicMock
        acm_client.certificates = [Certificate(arn=certificate_arn, id=certificate_id, name=certificate_name, type=certificate_type, expiration_days=365, transparency_logging=False, region=AWS_REGION)]
        with mock.patch('prowler.providers.aws.services.acm.acm_service.ACM', new=acm_client):
            from prowler.providers.aws.services.acm.acm_certificates_transparency_logs_enabled.acm_certificates_transparency_logs_enabled import acm_certificates_transparency_logs_enabled
            check = acm_certificates_transparency_logs_enabled()
            result = check.execute()
            ...
            assert result[0].status_extended == f'ACM Certificate {certificate_id} for {certificate_name} has Certificate Transparency logging disabled.'
            assert result[0].resource_id == certificate_id
            assert result[0].resource_arn == certificate_arn
            assert result[0].region == AWS_REGION
            assert result[0].resource_tags == []