...
async def _async_process_single_integration_platform_component(hass: HomeAssistant, component_name: str, integration: Integration | Exception, integration_platform: IntegrationPlatform) -> None:
    ...
    if isinstance(integration, Exception):
        _LOGGER.exception('Error importing integration %s for %s', component_name, integration_platform.platform_name)
        return
    ...
    try:
        ...
        if f'{component_name}.{platform_name}' not in str(err):
            _LOGGER.exception('Unexpected error importing %s/%s.py', component_name, platform_name)
        return
    try:
        await integration_platform.process_platform(hass, component_name, platform)
    except Exception:
        _LOGGER.exception('Error processing platform %s.%s', component_name, platform_name)
...
async def _async_process_integration_platform_for_component(hass: HomeAssistant, component_name: str) -> None:
    ...
    tasks = [asyncio.create_task(_async_process_single_integration_platform_component(hass, component_name, integrations[component_name], integration_platform), name=f'process integration platform {integration_platform.platform_name} for {component_name}') for integration_platform in integration_platforms if component_name not in integration_platform.seen_components]
    if tasks:
...
async def async_process_integration_platforms(hass: HomeAssistant, platform_name: str, process_platform: Callable[[HomeAssistant, str, Any], Awaitable[None]]) -> None:
    ...
    if DATA_INTEGRATION_PLATFORMS not in hass.data:
        ...
        async def _async_component_loaded(event: Event) -> None:
            ...
            await _async_process_integration_platform_for_component(hass, event.data[ATTR_COMPONENT])
        ...
        hass.bus.async_listen(EVENT_COMPONENT_LOADED, _async_component_loaded, event_filter=_async_component_loaded_filter)
    integration_platforms: list[IntegrationPlatform] = hass.data[DATA_INTEGRATION_PLATFORMS]
    ...
    if (top_level_components := [comp for comp in hass.config.components if '.' not in comp]):
        integrations = await async_get_integrations(hass, top_level_components)
        tasks = [asyncio.create_task(_async_process_single_integration_platform_component(hass, comp, integrations[comp], integration_platform), name=f'process integration platform {platform_name} for {comp}') for comp in top_level_components if comp not in integration_platform.seen_components]
        if tasks:
            await asyncio.gather(*tasks)