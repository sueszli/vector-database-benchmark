...
_default_re = re.compile('Default is (.+)\\.\\n')
_note_re = re.compile('Note: (.*?)(?:\\n\\n|\\n*$)', re.DOTALL)
_option_line_re = re.compile('^(?!\\s{2,}%\\(prog\\)s|\\s{2,}--\\w[\\w-]*\\w\\b|Example: )(.+)$', re.MULTILINE)
_option_re = re.compile('(?:^|(?<=\\s))(?P<arg>--\\w[\\w-]*\\w)(?P<val>=\\w+)?\\b')
_prog_re = re.compile('%\\(prog\\)s')
_percent_re = re.compile('%%')
_inline_code_block_re = re.compile('(?<!`)`([^`]+?)`')
_example_inline_code_block_re = re.compile('(?<=^Example: )(.+)$', re.MULTILINE)
...
class ArgparseDirective(Directive):
    has_content = True
    option_spec = {'module': unchanged, 'attr': unchanged}
    _headlines = ['^', '~']
