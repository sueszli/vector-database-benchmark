...
if __name__ == '__main__':
    ...
    parser.add_argument('--index', type=int, default=os.getenv('BUILDKITE_PARALLEL_JOB', 1))
    parser.add_argument('--count', type=int, default=os.getenv('BUILDKITE_PARALLEL_JOB_COUNT', 1))
    parser.add_argument('--tag_filters', type=str, help='Accepts the same string as in bazel test --test_tag_filters to apply the filters during gathering targets here.')
    parser.add_argument('--sharding_strategy', type=str, default='optimal', help="What sharding strategy to use. Can be 'optimal' (try to make sure each shard takes up around the same time) or 'naive' (assign the same number of targets to each shard).")
    ...
    if args.sharding_strategy not in ('optimal', 'naive'):
        parser.error(f"--sharding_strategy must be either 'optimal' or 'naive', got {args.sharding_strategy}")
    my_targets = main(targets=args.targets, index=args.index, count=args.count, tests_only=args.tests_only, exclude_manual=args.exclude_manual, tag_filters=args.tag_filters, sharding_strategy=args.sharding_strategy, debug=args.debug)
