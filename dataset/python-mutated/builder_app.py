...
import gradio as gr
themes = [gr.themes.Base, gr.themes.Default, gr.themes.Soft, gr.themes.Monochrome, gr.themes.Glass]
    ...
    with gr.Row():
        with gr.Column(scale=1, elem_id='controls', min_width=400):
            ...
            with gr.Tabs():
                ...
                theme_var_input = []
                core_color_suggestions = [f'*primary_{i}' for i in palette_range] + [f'*secondary_{i}' for i in palette_range] + [f'*neutral_{i}' for i in palette_range]
                variable_suggestions = {'fill': core_color_suggestions[:], 'color': core_color_suggestions[:], 'text_size': [f'*text_{i}' for i in size_range], 'radius': [f'*radius_{i}' for i in size_range], 'padding': [f'*spacing_{i}' for i in size_range], 'gap': [f'*spacing_{i}' for i in size_range], 'weight': ['100', '200', '300', '400', '500', '600', '700', '800'], 'shadow': ['none'], 'border_width': []}
                for variable in flat_variables:
                    ...
                    for style_type in variable_suggestions:
                        if style_type in variable:
                            ...
                            break
                (variable_suggestions['fill'], variable_suggestions['color']) = (variable_suggestions['fill'] + variable_suggestions['color'][len(core_color_suggestions):], variable_suggestions['color'] + variable_suggestions['fill'][len(core_color_suggestions):])
                        ...
                        for variable in variables:
                            ...
                            for style_type in variable_suggestions:
                                if style_type in variable:
                                    ...
                                    break
                            dropdown = gr.Dropdown(label=variable, info=get_docstr(variable), choices=suggestions, allow_custom_value=True)
        ...
        with gr.Column(scale=6, elem_id='app'):
            with gr.Column(variant='panel'):
                ...
                with gr.Accordion('Upload to Hub', open=False):
                    gr.Markdown('You can save your theme on the Hugging Face Hub. HF API write token can be found [here](https://huggingface.co/settings/tokens).')
                    with gr.Row():
                        theme_name = gr.Textbox(label='Theme Name')
                        theme_hf_token = gr.Textbox(label='Hugging Face Write Token')
                        theme_version = gr.Textbox(label='Version', placeholder='Leave blank to automatically update version.')
            ...
            name = gr.Textbox(label='Name', info='Full name, including middle name. No special characters.', placeholder='John Doe', value='John Doe', interactive=True)
            with gr.Row():
            ...
            with gr.Row():
                with gr.Column(variant='panel', scale=1):
                    gr.Markdown('## Panel 1')
                    radio = gr.Radio(['A', 'B', 'C'], label='Radio', info='Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.')
                    drop = gr.Dropdown(['Option 1', 'Option 2', 'Option 3'], show_label=False)
                    drop_2 = gr.Dropdown(['Option A', 'Option B', 'Option C'], multiselect=True, value=['Option A'], label='Dropdown', interactive=True)
                    check = gr.Checkbox(label='Go')
                with gr.Column(variant='panel', scale=2):
                    img = gr.Image('https://gradio-static-files.s3.us-west-2.amazonaws.com/header-image.jpg', label='Image', height=320)
                    with gr.Row():
                        ...
                        go_btn.click(go, [radio, drop, drop_2, check, name], img, api_name=False)
            ...
            gr.Examples(examples=[['A', 'Option 1', ['Option B'], True], ['B', 'Option 2', ['Option B', 'Option C'], False]], inputs=[radio, drop, drop_2, check], label='Examples')
            with gr.Row():
            ...
            with gr.Row():
                ...
                gr.Gallery([('https://gradio-static-files.s3.us-west-2.amazonaws.com/lion.jpg', 'lion'), ('https://gradio-static-files.s3.us-west-2.amazonaws.com/logo.png', 'logo'), ('https://gradio-static-files.s3.us-west-2.amazonaws.com/tower.jpg', 'tower')], height='200px', columns=2)
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    chat_btn.click(lambda history: history + [['How are you?', 'I am good.']] + (time.sleep(2) or []), chatbot, chatbot, api_name=False)
        ...
        theme_inputs = [primary_hue, secondary_hue, neutral_hue] + primary_hues + secondary_hues + neutral_hues + [text_size, spacing_size, radius_size] + text_sizes + spacing_sizes + radius_sizes + main_fonts + main_is_google + mono_fonts + mono_is_google + theme_var_input
        ...
        attach_rerender(secondary_hue.select(load_color, secondary_hue, secondary_hues, api_name=False).then)
        ...
        attach_rerender(spacing_size.change(load_size, spacing_size, spacing_sizes, api_name=False).then)
        attach_rerender(radius_size.change(load_size, radius_size, radius_sizes, api_name=False).then)
        attach_rerender(load_theme_btn.click(load_theme, base_theme_dropdown, theme_inputs, api_name=False).then)
        for theme_box in text_sizes + spacing_sizes + radius_sizes + main_fonts + mono_fonts:
        ...
        attach_rerender(undo_btn.click(undo, [history], [history, base_theme_dropdown] + theme_inputs, api_name=False).then)
        ...
        upload_to_hub_btn.click(lambda : 'Uploading...', None, upload_to_hub_btn, api_name=False).then(upload_to_hub, {current_theme, theme_name, theme_hf_token, theme_version}, [theme_upload_status, upload_to_hub_btn], api_name=False)
