...
fill_short_configs = op_bench.config_list(attr_names=['N'], attrs=[[1], [1024], [2048]], cross_product_configs={'device': ['cpu', 'cuda'], 'dtype': [torch.int32]}, tags=['short'])
fill_long_configs = op_bench.cross_product_configs(N=[10, 1000], device=get_all_device_types(), dtype=[torch.bool, torch.int8, torch.uint8, torch.int16, torch.int32, torch.int64, torch.half, torch.float, torch.double], tags=['long'])
