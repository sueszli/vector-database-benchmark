...
class apikeys_api_restrictions_configured(Check):
    ...
    def execute(self) -> Check_Report_GCP:
        ...
        for key in apikeys_client.keys:
            ...
            if key.restrictions == {} or any([target.get('service') == 'cloudapis.googleapis.com' for target in key.restrictions['apiTargets']]):
                report.status = 'FAIL'
                report.status_extended = f"API key {key.name} doens't have restrictions configured."
