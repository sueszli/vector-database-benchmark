...
async def callback_discord(auth_code: str, request: Request, db: Session=Depends(deps.get_db)):
    redirect_uri = f'{get_callback_uri(request)}/discord'
    async with aiohttp.ClientSession(raise_for_status=True) as session:
        async with session.post('https://discord.com/api/oauth2/token', data={'client_id': Settings.AUTH_DISCORD_CLIENT_ID, 'client_secret': Settings.AUTH_DISCORD_CLIENT_SECRET, 'grant_type': 'authorization_code', 'code': auth_code, 'redirect_uri': redirect_uri, 'scope': 'identify'}) as token_response:
            token_response_json = await token_response.json()
            access_token = token_response_json['access_token']
        async with session.get('https://discord.com/api/users/@me', headers={'Authorization': f'Bearer {access_token}'}) as user_response:
