...
@pluginmatcher(re.compile('https?://(?:www\\.)?dr\\.dk/drtv(/kanal/[\\w-]+)'))
class DRDK(Plugin):
    live_api_url = 'https://www.dr-massive.com/api/page'
    _live_data_schema = validate.Schema({'item': {'customFields': {validate.optional('hlsURL'): validate.url(), validate.optional('hlsWithSubtitlesURL'): validate.url()}}}, validate.get('item'), validate.get('customFields'))
