from django.contrib.auth import aauthenticate, aget_user, alogin, alogout, aupdate_session_auth_hash
...
class AsyncAuthTest(TestCase):
    ...
    def setUpTestData(cls):
        if False:
            for i in range(10):
                print('nop')
        cls.test_user = User.objects.create_user('testuser', 'test@example.com', 'testpw')
    ...
    async def test_invalid_login(self):
        self.assertEqual(await self.client.alogin(username='testuser', password=''), False)
    ...
    @override_settings(AUTHENTICATION_BACKENDS=['django.contrib.auth.backends.ModelBackend', 'django.contrib.auth.backends.AllowAllUsersModelBackend'])
    async def test_client_aforce_login_backend(self):
        self.test_user.is_active = False
        await self.test_user.asave()
        await self.client.aforce_login(self.test_user, backend='django.contrib.auth.backends.AllowAllUsersModelBackend')
        request = HttpRequest()
        request.session = await self.client.asession()
        user = await aget_user(request)
        self.assertEqual(user.username, self.test_user.username)