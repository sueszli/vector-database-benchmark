import typing
from grafanalib.core import Alert, AlertCondition, Dashboard, Graph, GreaterThan, OP_AND, OPS_FORMAT, Row, RTYPE_SUM, SECONDS_FORMAT, SHORT_FORMAT, single_y_axis, Target, TimeRange, YAxes, YAxis, MILLISECONDS_FORMAT, Templating, Template, DataSourceInput
...
project_template = Template(name='project', dataSource=DATASOURCE, query='label_values(flyte:propeller:all:collector:flyteworkflow, project)', sort=True)
domain_template = Template(name='domain', dataSource=DATASOURCE, query='label_values(flyte:propeller:all:collector:flyteworkflow, domain)', sort=True)
wf_template = Template(name='workflow', dataSource=DATASOURCE, query='label_values(flyte:propeller:all:collector:flyteworkflow, wf)', sort=True, allValue='.*', includeAll=True)
dashboard = Dashboard(tags=['flyte', 'prometheus', 'flyteuser', 'flyte-user'], inputs=[DataSourceInput(name=DATASOURCE_NAME, label='Prometheus', description='Prometheus server that connects to Flyte', pluginId='prometheus', pluginName='Prometheus')], editable=False, title='Flyte User Dashboard (via Prometheus)', rows=FlyteUserDashboard.create_all_rows(interval=5), templating=Templating(list=[project_template, domain_template, wf_template]), description='Flyte User Dashboard. This is great to get a birds-eye and drill down view of executions in your Flyte cluster. Useful for the user.').auto_panel_ids()
