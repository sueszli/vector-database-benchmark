...
from AnyQt.QtCore import Qt, QSize, QItemSelection, QItemSelectionModel, pyqtSignal
from AnyQt.QtGui import QStandardItem, QColor, QStandardItemModel
from AnyQt.QtWidgets import QTableView, QSizePolicy, QHeaderView, QStyledItemDelegate, QCheckBox, QFrame, QWidget, QGridLayout
from Orange.data import Table, DiscreteVariable, Variable, Domain, ContinuousVariable, TimeVariable
...
from Orange.statistics.util import nanmin, nanmax, nanunique, nansum, nanvar, nanmean, nanmedian, nanmode, bincount
...
class Pivot:
    ...
    (Count, Count_defined, Sum, Mean, Min, Max, Mode, Median, Var, Majority) = Functions = [Function(i, *fdef) for (i, fdef) in enumerate((('Count', len), ('Count defined', count_defined), ('Sum', lambda x: nansum(x, axis=0) if x.shape[0] > 0 else 0), ('Mean', wrapstat(nanmean)), ('Min', wrapstat(nanmin)), ('Max', wrapstat(nanmax)), ('Mode', mode), ('Median', wrapstat(nanmedian)), ('Var', wrapstat(nanvar)), ('Majority', majority)))]
...
class OWPivot(OWWidget):
    ...
    class Warning(OWWidget.Warning):
        ...
        renamed_vars = Msg('Some variables have been renamed in some tablesto avoid duplicates.\n{}')
    ...
    AGGREGATIONS = (Pivot.Count, Pivot.Count_defined, None, Pivot.Sum, Pivot.Mean, Pivot.Var, Pivot.Median, 2, Pivot.Mode, Pivot.Min, Pivot.Max, None, Pivot.Majority)
    MAX_VALUES = 100
