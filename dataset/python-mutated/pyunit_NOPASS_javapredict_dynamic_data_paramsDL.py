...
def javapredict_dynamic_data():
    ...
    if random.randint(0, 1):
        params['activation'] = random.sample(['Rectifier', 'Tanh', 'TanhWithDropout', 'RectifierWithDropout', 'MaxoutWithDropout'], 1)[0]
    if random.randint(0, 1):
    ...
    x.remove('response')
    y = 'response'
    pyunit_utils.javapredict(algo='deeplearning', equality=None, train=train, test=None, x=x, y=y, compile_only=True, **params)
