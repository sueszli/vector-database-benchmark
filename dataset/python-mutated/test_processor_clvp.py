...
class ClvpProcessorTest(unittest.TestCase):
    ...
    def test_save_load_pretrained_additional_features(self):
        ...
        processor = ClvpProcessor.from_pretrained(self.tmpdirname, pad_token='(PAD)', sampling_rate=16000)
    ...
    def test_model_input_names(self):
        ...
        tokenizer = self.get_tokenizer()
        processor = ClvpProcessor(tokenizer=tokenizer, feature_extractor=feature_extractor)
        self.assertListEqual(sorted(processor.model_input_names), sorted(set(feature_extractor.model_input_names + tokenizer.model_input_names)), msg='`processor` and `feature_extractor` model input names do not match')