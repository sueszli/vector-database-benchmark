...
async def create_user_chat_repository(session: AsyncSession=Depends(create_session), user_id: str=Depends(auth.get_current_user_id)) -> UserChatRepository:
...
class UserRateLimiter(RateLimiter):
    ...
    def __init__(self, times: int=100, milliseconds: int=0, seconds: int=0, minutes: int=1, hours: int=0) -> None:
        if False:
            i = 10
            return i + 15
