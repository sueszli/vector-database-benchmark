...
async def test_create_remote_search_request(rest_api, mock_gigachannel_community):
    ...
    search_txt = 'foo'
    await do_request(rest_api, f'remote_query?txt_filter={search_txt}', request_type='PUT', expected_code=200, expected_json={'request_uuid': str(request_uuid), 'peers': peers})
    assert sent['txt_filter'] == search_txt
    sent.clear()
    channel_pk = 'ff'
    await do_request(rest_api, f'remote_query?channel_pk={channel_pk}&metadata_type=torrent', request_type='PUT', expected_code=200)
...
async def test_get_channels_peers(rest_api, metadata_store, mock_gigachannel_community):
    ...
    result = await do_request(rest_api, 'remote_query/channels_peers', request_type='GET', expected_code=200)
