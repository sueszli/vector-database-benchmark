...
class MegasharesCom(SimpleDownloader):
    ...
    __version__ = '0.37'
    __status__ = 'testing'
    __pattern__ = 'http://(?:www\\.)?(d\\d{2}\\.)?megashares\\.com/((index\\.php)?\\?d\\d{2}=|dl/)\\w+'
    ...
    NAME_PATTERN = '<h1 class="black xxl"[^>]*title="(?P<N>.+?)">'
    SIZE_PATTERN = '<strong><span class="black">Filesize:</span></strong> (?P<S>[\\d.,]+) (?P<U>[\\w^_]+)'
    OFFLINE_PATTERN = '<dd class="red">(Invalid Link Request|Link has been deleted|Invalid link)'
    LINK_PATTERN = '<div id="show_download_button_{}".*?>\\s*<a href="(.+?)">'
    PASSPORT_LEFT_PATTERN = 'Your Download Passport is: <.*?>(\\w+).*?You have.*?<.*?>.*?([\\d.]+) (\\w+)'
    PASSPORT_RENEW_PATTERN = '(\\d+):<strong>(\\d+)</strong>:<strong>(\\d+)</strong>'
    REACTIVATE_NUM_PATTERN = '<input[^>]*id="random_num" value="(\\d+)" />'
    REACTIVATE_PASSPORT_PATTERN = '<input[^>]*id="passport_num" value="(\\w+)" />'
    REQUEST_URI_PATTERN = 'var request_uri = "(.+?)";'
    NO_SLOTS_PATTERN = '<dd class="red">All download slots for this link are currently filled'
