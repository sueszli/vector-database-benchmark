...
from sphinx.addnodes import desc, desc_content, desc_name, desc_parameter, desc_parameterlist, desc_sig_name, desc_sig_space, desc_signature, desc_signature_line, pending_xref
from sphinx.domains.c import DefinitionError, DefinitionParser, Symbol, _id_prefix, _macroKeywords, _max_id
...
def test_domain_c_ast_expressions():
    ...
    for suffix in ['', 'f', 'F', 'l', 'L']:
        for e in ['5e42', '5e+42', '5e-42', '5.', '5.e42', '5.e+42', '5.e-42', '.5', '.5e42', '.5e+42', '.5e-42', '5.0', '5.0e42', '5.0e+42', '5.0e-42', "1'2'3e7'8'9", "1'2'3.e7'8'9", ".4'5'6e7'8'9", "1'2'3.4'5'6e7'8'9"]:
            expr = e + suffix
            exprCheck(expr)
        for e in ['ApF', 'Ap+F', 'Ap-F', 'A.', 'A.pF', 'A.p+F', 'A.p-F', '.A', '.ApF', '.Ap+F', '.Ap-F', 'A.B', 'A.BpF', 'A.Bp+F', 'A.Bp-F', "A'B'Cp1'2'3", "A'B'C.p1'2'3", ".D'E'Fp1'2'3", "A'B'C.D'E'Fp1'2'3"]:
            expr = '0x' + e + suffix
            exprCheck(expr)
    exprCheck('"abc\\"cba"')
    for p in ['', 'u8', 'u', 'U', 'L']:
...
def test_domain_c_ast_fundamental_types():
    ...
    def types():
        ...
        yield from ('__float80', '_Float64x', '__float128', '_Float128', '__ibm128')
        yield '__fp16'
        for sat in ('', '_Sat  '):
...
def test_domain_c_ast_type_definitions():
    ...
    check('type', '{key}bool const b', {1: 'b'}, key='typedef')
    check('type', '{key}void (*gpio_callback_t)(struct device *port, uint32_t pin)', {1: 'gpio_callback_t'}, key='typedef')
...
def test_domain_c_ast_function_definitions():
    ...
    for cvr in cvrs:
        ...
        check('function', f'void f(int arr[{cvr}{space}static 42])', {1: 'f'}, output=f'void f(int arr[static{space}{cvr} 42])')
    check('function', 'void f(int arr[const static volatile 42])', {1: 'f'}, output='void f(int arr[static volatile const 42])')
    with pytest.raises(DefinitionError):
...
def test_domain_c_ast_attributes():
    ...
    check('member', '[ [ ] ] int f', {1: 'f'}, output='[[ ]] int f')
    ...
    with pytest.raises(DefinitionError):
        parse('member', 'paren_attr({]}) int f')
    check('function', 'static inline __attribute__(()) void f()', {1: 'f'}, output='__attribute__(()) static inline void f()')
    check('function', '[[attr1]] [[attr2]] void f()', {1: 'f'})
    check('member', 'int *[[attr1]] [[attr2]] i', {1: 'i'})
    check('member', 'int *const [[attr1]] [[attr2]] volatile i', {1: 'i'}, output='int *[[attr1]] [[attr2]] volatile const i')
    check('member', 'int *[[attr1]] [[attr2]] *i', {1: 'i'})
    ...
    check('function', 'LIGHTGBM_C_EXPORT int LGBM_BoosterFree(int handle)', {1: 'LGBM_BoosterFree'})
...
def filter_warnings(warning, file):
    ...
    res = [l for l in lines if 'domain-c' in l and f'{file}.rst' in l and ("WARNING: document isn't included in any toctree" not in l)]
...
def test_domain_c_build_function_param_target(app, warning):
    ...
    assert entries == [('c.function_param_target.f', 'i', 'i'), ('c.function_param_target.f', 'f.i', 'f.i')]
...
def test_domain_c_parse_cfunction(app):
    if False:
        for i in range(10):
            print('nop')
    text = '.. c:function:: PyObject* PyType_GenericAlloc(PyTypeObject *type, Py_ssize_t nitems)'
    doctree = restructuredtext.parse(app, text)
    assert_node(doctree[1], addnodes.desc, desctype='function', domain='c', objtype='function', no_index=False)
...
def test_domain_c_parse_cmember(app):
    ...
    doctree = restructuredtext.parse(app, text)
    assert_node(doctree[1], addnodes.desc, desctype='member', domain='c', objtype='member', no_index=False)
...
def test_domain_c_parse_cvar(app):
    ...
    doctree = restructuredtext.parse(app, text)
    assert_node(doctree[1], addnodes.desc, desctype='var', domain='c', objtype='var', no_index=False)
...
@pytest.mark.sphinx('html', confoverrides={'c_maximum_signature_line_length': len('str hello(str name)')})
...
@pytest.mark.sphinx('html', confoverrides={'c_maximum_signature_line_length': len('str hello(str name)'), 'maximum_signature_line_length': 1})
...
def test_domain_c_c_maximum_signature_line_length_in_html(app, status, warning):
    ...
    expected = '\n<dl>\n<dd><span class="n"><span class="pre">str</span></span><span class="w"> </span><span class="n"><span class="pre">name</span></span>,</dd>\n</dl>\n\n<span class="sig-paren">)</span><a class="headerlink" href="#c.hello" title="Link to this definition">¶</a><br /></dt>\n'
