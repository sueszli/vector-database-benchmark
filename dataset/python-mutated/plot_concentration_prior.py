...
estimators = [('Finite mixture with a Dirichlet distribution\nprior and $\\gamma_0=$', BayesianGaussianMixture(weight_concentration_prior_type='dirichlet_distribution', n_components=2 * n_components, reg_covar=0, init_params='random', max_iter=1500, mean_precision_prior=0.8, random_state=random_state), [0.001, 1, 1000]), ('Infinite mixture with a Dirichlet process\n prior and$\\gamma_0=$', BayesianGaussianMixture(weight_concentration_prior_type='dirichlet_process', n_components=2 * n_components, reg_covar=0, init_params='random', max_iter=1500, mean_precision_prior=0.8, random_state=random_state), [1, 1000, 100000])]
...
for (title, estimator, concentrations_prior) in estimators:
    ...
    plt.subplots_adjust(bottom=0.04, top=0.9, hspace=0.05, wspace=0.05, left=0.03, right=0.99)
    gs = gridspec.GridSpec(3, len(concentrations_prior))
    for (k, concentration) in enumerate(concentrations_prior):
        estimator.weight_concentration_prior = concentration
        estimator.fit(X)
        plot_results(plt.subplot(gs[0:2, k]), plt.subplot(gs[2, k]), estimator, X, y, '%s$%.1e$' % (title, concentration), plot_title=k == 0)
plt.show()