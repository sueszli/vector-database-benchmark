...
conn_group.add_argument('-s', '--sukebei', default=False, action='store_true', help='Query torrent info on sukebei.Nyaa.si')
...
req_group.add_argument('hash_or_id', metavar='HASH_OR_ID', help='Torrent ID or hash (hex, 40 characters) to query for')
...
if __name__ == '__main__':
    ...
    if not (id_match or hex_hash_match):
        raise Exception("Given argument '{}' doesn't seem like an ID or a hex hash.".format(api_query))
    if id_match:
    ...
    if args.raw:
        ...
        if errors:
            ...
            info_str = INFO_TEMPLATE.format(formatted_filesize=formatted_filesize, flag_info=flag_info, **response)
            print(info_str)
            if args.magnet:
