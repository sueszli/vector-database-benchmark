...
class test_Topic:
    ...
    async def test_publish_message__wait_enabled(self, *, topic, app):
        ...
        headers = {'k': 'v'}
        fm = topic.as_future_message(key='foo', value='bar', partition=130, timestamp=312.5134, headers=headers, key_serializer='json', value_serializer='json', callback=callback)
        ...
        producer.send_and_wait.coro.assert_called_once_with(topic.get_topic_name(), key, value, headers=headers, partition=130, timestamp=312.5134)
    ...
    async def test_send__attachments_enabled(self, *, topic, app):
        ...
        with patch('faust.topics.current_event') as current_event:
            await topic.send(key='k', value='v', partition=3, timestamp=312.41, headers={'k': 'v'}, schema=schema, key_serializer='foo', value_serializer='bar', callback=callback, force=False)
            current_event.assert_called_once_with()
            current_event()._attach.assert_called_once_with(topic, 'k', 'v', partition=3, timestamp=312.41, headers={'k': 'v'}, schema=schema, key_serializer='foo', value_serializer='bar', callback=callback)
    ...
    async def test_send__attachments_no_event(self, *, topic, app):
        ...
        with patch('faust.topics.current_event', Mock(return_value=None)):
            await topic.send(key='k', value='v', partition=3, timestamp=312.41, headers={'k': 'v'}, key_serializer='foo', value_serializer='bar', callback=callback, force=False)
            topic._send_now.assert_called_once_with('k', 'v', partition=3, timestamp=312.41, headers={'k': 'v'}, schema=None, key_serializer='foo', value_serializer='bar', callback=callback)
    ...
    @pytest.mark.asyncio
    async def test_decode_empty_error_callback(self, *, topic, message_empty_value):
        topic = topic.derive_topic(value_serializer='json', value_type=Dummy)
    ...
    @pytest.mark.asyncio
    async def test_decode_allow_empty(self, *, topic_allow_empty, message_empty_value):
    ...
    async def test_maybe_declare(self, *, topic):
        ...
        await asyncio.gather(topic.maybe_declare(), topic.maybe_declare(), topic.maybe_declare(), topic.maybe_declare(), topic.maybe_declare(), topic.maybe_declare())
    ...
    async def test_declare(self, *, topic):
        ...
        topic.topics = ['foo', 'bar']
        await topic.declare()
        producer.create_topic.coro.assert_has_calls([call(topic='foo', partitions=101, replication=202, config=topic.config, compacting=topic.compacting, deleting=topic.deleting, retention=topic.retention), call(topic='bar', partitions=101, replication=202, config=topic.config, compacting=topic.compacting, deleting=topic.deleting, retention=topic.retention)])
    ...
    async def test_declare__defaults(self, *, topic):
        ...
        topic.topics = ['foo', 'bar']
        await topic.declare()
        producer.create_topic.coro.assert_has_calls([call(topic='foo', partitions=topic.app.conf.topic_partitions, replication=topic.app.conf.topic_replication_factor, config=topic.config, compacting=topic.compacting, deleting=topic.deleting, retention=topic.retention), call(topic='bar', partitions=topic.app.conf.topic_partitions, replication=topic.app.conf.topic_replication_factor, config=topic.config, compacting=topic.compacting, deleting=topic.deleting, retention=topic.retention)])
