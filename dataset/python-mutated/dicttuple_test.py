...
SIMPLE_TEST_CASES = [DictTuple({'x': 1, 'y': 2}), DictTuple([('x', 1), ('y', 2)]), DictTuple([1, 2], ['x', 'y'])]
...
TEST_CASES: list[Case] = [Case(values=[1, 2, 3], keys=['x', 'y', 'z']), Case(values=[1, 2, 3], keys=str)]
...
@pytest.mark.parametrize('dict_tuple,case', [(c.generate_one(), c) for c in TEST_CASES])
...
def test_keys_should_not_be_int(args: Callable[[], tuple]) -> None:
    ...
    with pytest.raises(ValueError, match='int'):
        DictTuple(*args())
LENGTH_MISMACTH_TEST_CASES = [Case(values=[1, 2, 3], keys=['x', 'y']), Case(values=[1, 2], keys=['x', 'y', 'z'])]
