...
@param.parameterized_class(('primal0', 'primal1', 'dtype'), [(np.random.rand(2, 3, 4), np.random.rand(2, 3, 4), np.float32), (np.random.rand(2, 3, 3, 4), np.random.rand(3, 1, 4), np.float32), (np.random.rand(2, 3, 3, 4), np.random.rand(2, 3, 1, 4), np.float32), (np.random.rand(2, 3, 3, 4), np.random.rand(2, 3, 1, 4), np.float32), (np.random.rand(2, 1, 3, 4), np.random.rand(2, 3, 1, 4), np.float32), (np.random.rand(2, 3, 3, 4), np.random.rand(2, 1, 1, 4), np.float32)])
class TestSubGradComp(unittest.TestCase):
    ...
    def test_tanh_grad_comp(self):
        ...
        def actual(primal0, primal1):
            ...
            exe.run(sp)
            out = exe.run(program=mp, feed={'primal0': primal0, 'primal1': primal1}, fetch_list=[res[0].name, res[1].name])
            return (out[0], out[1])
        ...
        def desired(primal0, primal1):
            ...
            (mp, sp) = (paddle.static.Program(), paddle.static.Program())
            with paddle.static.program_guard(mp, sp):
                x = paddle.static.data('primal0', self.primal0.shape, self.primal0.dtype)
                y = paddle.static.data('primal1', self.primal1.shape, self.primal1.dtype)
                x.stop_gradient = False
                y.stop_gradient = False
                out = paddle.subtract(x, y)
                res = paddle.static.gradients([out], [x, y])
            exe = paddle.static.Executor()
            exe.run(sp)
            out = exe.run(program=mp, feed={'primal0': self.primal0, 'primal1': self.primal1}, fetch_list=[res[0].name, res[1].name])
            return (out[0], out[1])
