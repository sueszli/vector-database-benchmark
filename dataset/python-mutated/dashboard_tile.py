...
class Text(models.Model):
    ...
    last_modified_by: models.ForeignKey = models.ForeignKey('User', on_delete=models.SET_NULL, null=True, blank=True, related_name='modified_text_tiles')
...
class DashboardTile(models.Model):
    ...
    class Meta:
        ...
        constraints = [UniqueConstraint(fields=['dashboard', 'insight'], name=f'unique_dashboard_insight', condition=Q(('insight__isnull', False))), UniqueConstraint(fields=['dashboard', 'text'], name=f'unique_dashboard_text', condition=Q(('text__isnull', False))), models.CheckConstraint(check=build_check(('insight', 'text')), name='dash_tile_exactly_one_related_object')]
