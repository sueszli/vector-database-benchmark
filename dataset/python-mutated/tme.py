...
class TMEPlugin(SupplierBarcodeMixin, SettingsMixin, InvenTreePlugin):
    ...
    SETTINGS = {'SUPPLIER_ID': {'name': _('Supplier'), 'description': _("The Supplier which acts as 'TME'"), 'model': 'company.company'}}
    TME_IS_QRCODE_REGEX = re.compile('([^\\s:]+:[^\\s:]+\\s+)+(\\S+(\\s|$)+)+')
    TME_IS_BARCODE2D_REGEX = re.compile('(([^\\s]+)(\\s+|$))+')
    TME_QRCODE_FIELDS = {'PN': SupplierBarcodeMixin.SUPPLIER_PART_NUMBER, 'PO': SupplierBarcodeMixin.CUSTOMER_ORDER_NUMBER, 'MPN': SupplierBarcodeMixin.MANUFACTURER_PART_NUMBER, 'QTY': SupplierBarcodeMixin.QUANTITY}
