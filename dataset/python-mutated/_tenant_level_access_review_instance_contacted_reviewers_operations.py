...
def build_list_request(schedule_definition_id: str, id: str, **kwargs: Any) -> HttpRequest:
    ...
    _url = kwargs.pop('template_url', '/providers/Microsoft.Authorization/accessReviewScheduleDefinitions/{scheduleDefinitionId}/instances/{id}/contactedReviewers')
    path_format_arguments = {'scheduleDefinitionId': _SERIALIZER.url('schedule_definition_id', schedule_definition_id, 'str'), 'id': _SERIALIZER.url('id', id, 'str')}
    ...
    def list(self, schedule_definition_id: str, id: str, **kwargs: Any) -> Iterable['_models.AccessReviewContactedReviewer']:
        ...
        def prepare_request(next_link=None):
            if False:
                return 10
            if not next_link:
                request = build_list_request(schedule_definition_id=schedule_definition_id, id=id, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
