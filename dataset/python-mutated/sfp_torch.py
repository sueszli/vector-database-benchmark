...
class sfp_torch(SpiderFootPlugin):
    meta = {'name': 'TORCH', 'summary': "Search Tor 'TORCH' search engine for mentions of the target domain.", 'flags': ['errorprone', 'tor'], 'useCases': ['Footprint', 'Investigate'], 'categories': ['Search Engines'], 'dataSource': {'website': 'https://torchsearch.wordpress.com/', 'model': 'FREE_NOAUTH_UNLIMITED', 'description': "Torch or TorSearch is the best search engine for the hidden part of the internet. They're also the oldest and longest running search engine on Tor.\nTorch claims to have over one billion dark net pages indexed. They also don't censor search results or track what you search for."}}
    opts = {'fetchlinks': True, 'pages': 20, 'fullnames': True}
    optdescs = {'fetchlinks': 'Fetch the darknet pages (via TOR, if enabled) to verify they mention your target.', 'pages': 'Number of results pages to iterate through.', 'fullnames': 'Search for human names?'}
    results = None
