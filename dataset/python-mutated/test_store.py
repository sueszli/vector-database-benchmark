...
from torch.testing._internal.common_distributed import skip_if_win32, create_tcp_store, tp_transports
...
class TestMultiThreadedWait(MultiThreadedTestCase):
    stores = [dist.FileStore(tempfile.NamedTemporaryFile(delete=False).name, 1), dist.HashStore(), dist.PrefixStore('pre', dist.FileStore(tempfile.NamedTemporaryFile(delete=False).name, 1)), create_tcp_store(), create_tcp_store(use_libuv=True), dist.PrefixStore('pre', create_tcp_store()), dist.PrefixStore('pre', create_tcp_store(use_libuv=True))]
