...
class MaeImagePretrainingTask(FairseqTask):
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass=None, **kwargs):
        ...
        self.datasets[split] = MaeImageDataset(root=data_path if cfg.multi_data is None else cfg.multi_data, split=split, input_size=cfg.input_size, local_cache_path=cfg.local_cache_path, key=cfg.key, beit_transforms=cfg.beit_transforms, target_transform=cfg.target_transform, no_transform=cfg.no_transform, compute_mask=compute_mask, dataset_type=cfg.dataset_type, **mask_args)
        if cfg.subsample < 1:
            self.datasets[split] = SubsampleDataset(self.datasets[split], cfg.subsample, shuffle=True, seed=cfg.seed)
