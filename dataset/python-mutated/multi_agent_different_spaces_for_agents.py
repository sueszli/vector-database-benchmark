...
if __name__ == '__main__':
    ...
    config = get_trainable_cls(args.run).get_default_config().environment(env=BasicMultiAgentMultiSpaces).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0'))).training(train_batch_size=1024).rollouts(num_rollout_workers=1, rollout_fragment_length='auto').framework(args.framework).multi_agent(policies={'main0', 'main1'}, policy_mapping_fn=lambda aid, episode, worker, **kw: f'main{aid[-1]}', policies_to_train=['main0'])
    results = tune.Tuner(args.run, run_config=air.RunConfig(stop=stop), param_space=config).fit()
    if not results:
        raise ValueError('No results returned from tune.run(). Something must have gone wrong.')
    ray.shutdown()