...
class OrganizationStatsSummaryTest(APITestCase, OutcomesSnubaTest):
    ...
    def do_request(self, query, user=None, org=None):
        ...
        url = reverse('sentry-api-0-organization-stats-summary', kwargs={'organization_slug': (org or self.organization).slug})
    ...
    def test_future_request(self):
        ...
        assert response.data == {'start': '2021-03-14T12:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': []}
    ...
    def test_unknown_category(self):
        ...
        assert response.status_code == 400, response.content
        assert response.data == {'detail': 'Invalid category: "scoobydoo"'}
    ...
    def test_unknown_outcome(self):
        ...
        assert response.status_code == 400, response.content
        assert response.data == {'detail': 'Invalid outcome: "scoobydoo"'}
    ...
    def test_resolution_invalid(self):
        if False:
            while True:
                i = 10
        self.login_as(user=self.user)
        make_request = functools.partial(self.client.get, reverse('sentry-api-0-organization-stats-summary', args=[self.org.slug]))
    ...
    def test_attachment_filter_only(self):
        ...
        assert response.status_code == 400, response.content
        assert response.data == {'detail': 'if filtering by attachment no other category may be present'}
    ...
    def test_user_all_accessible(self):
        ...
        assert response.data == {'start': '2021-03-14T00:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': []}
    ...
    def test_open_membership_semantics(self):
        ...
        assert response.status_code == 200
        assert response.data == {'start': '2021-03-14T00:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': [{'id': self.project.id, 'slug': self.project.slug, 'stats': [{'category': 'error', 'outcomes': {'abuse': 0, 'accepted': 6, 'client_discard': 0, 'filtered': 0, 'invalid': 0, 'rate_limited': 0}, 'totals': {'dropped': 0, 'sum(quantity)': 6}}]}, {'id': self.project2.id, 'slug': self.project2.slug, 'stats': [{'category': 'transaction', 'outcomes': {'abuse': 0, 'accepted': 0, 'client_discard': 0, 'filtered': 0, 'invalid': 0, 'rate_limited': 1}, 'totals': {'dropped': 1, 'sum(quantity)': 1}}]}]}
    ...
    def test_org_simple(self):
        if False:
            for i in range(10):
                print('nop')
        make_request = functools.partial(self.client.get, reverse('sentry-api-0-organization-stats-summary', args=[self.org.slug]))
        response = make_request({'statsPeriod': '2d', 'interval': '1d', 'field': ['sum(quantity)']})
        assert response.status_code == 200, response.content
        assert response.data == {'start': '2021-03-13T00:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': [{'id': self.project.id, 'slug': self.project.slug, 'stats': [{'category': 'attachment', 'outcomes': {'accepted': 0, 'filtered': 0, 'rate_limited': 1024, 'invalid': 0, 'abuse': 0, 'client_discard': 0}, 'totals': {'dropped': 1024, 'sum(quantity)': 1024}}, {'category': 'error', 'outcomes': {'accepted': 6, 'filtered': 0, 'rate_limited': 0, 'invalid': 0, 'abuse': 0, 'client_discard': 0}, 'totals': {'dropped': 0, 'sum(quantity)': 6}}]}, {'id': self.project2.id, 'slug': self.project2.slug, 'stats': [{'category': 'transaction', 'outcomes': {'accepted': 0, 'filtered': 0, 'rate_limited': 1, 'invalid': 0, 'abuse': 0, 'client_discard': 0}, 'totals': {'dropped': 1, 'sum(quantity)': 1}}]}]}
    ...
    def test_org_multiple_fields(self):
        if False:
            for i in range(10):
                print('nop')
        make_request = functools.partial(self.client.get, reverse('sentry-api-0-organization-stats-summary', args=[self.org.slug]))
        response = make_request({'statsPeriod': '2d', 'interval': '1d', 'field': ['sum(quantity)', 'sum(times_seen)']})
        assert response.status_code == 200, response.content
        assert response.data == {'start': '2021-03-13T00:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': [{'id': self.project.id, 'slug': self.project.slug, 'stats': [{'category': 'attachment', 'outcomes': {'accepted': 0, 'filtered': 0, 'rate_limited': 1025, 'invalid': 0, 'abuse': 0, 'client_discard': 0}, 'totals': {'dropped': 1025, 'sum(quantity)': 1024, 'sum(times_seen)': 1}}, {'category': 'error', 'outcomes': {'accepted': 12, 'filtered': 0, 'rate_limited': 0, 'invalid': 0, 'abuse': 0, 'client_discard': 0}, 'totals': {'dropped': 0, 'sum(quantity)': 6, 'sum(times_seen)': 6}}]}, {'id': self.project2.id, 'slug': self.project2.slug, 'stats': [{'category': 'transaction', 'outcomes': {'accepted': 0, 'filtered': 0, 'rate_limited': 2, 'invalid': 0, 'abuse': 0, 'client_discard': 0}, 'totals': {'dropped': 2, 'sum(quantity)': 1, 'sum(times_seen)': 1}}]}]}
    ...
    def test_org_project_totals_per_project(self):
        if False:
            return 10
        make_request = functools.partial(self.client.get, reverse('sentry-api-0-organization-stats-summary', args=[self.org.slug]))
        response_per_group = make_request({'statsPeriod': '1d', 'interval': '1h', 'field': ['sum(times_seen)'], 'category': ['error', 'transaction']})
        assert response_per_group.status_code == 200, response_per_group.content
        assert response_per_group.data == {'start': '2021-03-13T13:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': [{'id': self.project.id, 'slug': self.project.slug, 'stats': [{'category': 'error', 'outcomes': {'abuse': 0, 'accepted': 6, 'client_discard': 0, 'filtered': 0, 'invalid': 0, 'rate_limited': 0}, 'totals': {'dropped': 0, 'sum(times_seen)': 6}}]}, {'id': self.project2.id, 'slug': self.project2.slug, 'stats': [{'category': 'transaction', 'outcomes': {'abuse': 0, 'accepted': 0, 'client_discard': 0, 'filtered': 0, 'invalid': 0, 'rate_limited': 1}, 'totals': {'dropped': 1, 'sum(times_seen)': 1}}]}]}
    ...
    def test_project_filter(self):
        ...
        assert response.status_code == 200, response.content
        assert response.data == {'start': '2021-03-14T00:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': [{'id': self.project.id, 'slug': self.project.slug, 'stats': [{'category': 'error', 'outcomes': {'abuse': 0, 'accepted': 6, 'client_discard': 0, 'filtered': 0, 'invalid': 0, 'rate_limited': 0}, 'totals': {'dropped': 0, 'sum(quantity)': 6}}]}]}
    ...
    def test_reason_filter(self):
        ...
        assert response.status_code == 200, response.content
        assert response.data == {'start': '2021-03-14T00:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': [{'id': self.project.id, 'slug': self.project.slug, 'stats': [{'category': 'attachment', 'reason': 'spike_protection', 'outcomes': {'accepted': 0, 'filtered': 0, 'rate_limited': 1, 'invalid': 0, 'abuse': 0, 'client_discard': 0}, 'totals': {'dropped': 1, 'sum(times_seen)': 1}}]}, {'id': self.project2.id, 'slug': self.project2.slug, 'stats': [{'category': 'transaction', 'reason': 'spike_protection', 'outcomes': {'accepted': 0, 'filtered': 0, 'rate_limited': 1, 'invalid': 0, 'abuse': 0, 'client_discard': 0}, 'totals': {'dropped': 1, 'sum(times_seen)': 1}}]}]}
    ...
    def test_outcome_filter(self):
        ...
        assert response.status_code == 200, response.content
        assert response.data == {'start': '2021-03-14T00:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': [{'id': self.project.id, 'slug': self.project.slug, 'stats': [{'category': 'error', 'outcomes': {'accepted': 6}, 'totals': {'sum(quantity)': 6}}]}]}
    ...
    def test_category_filter(self):
        ...
        assert response.status_code == 200, response.content
        assert response.data == {'start': '2021-03-14T00:00:00Z', 'end': '2021-03-14T12:28:00Z', 'projects': [{'id': self.project.id, 'slug': self.project.slug, 'stats': [{'category': 'error', 'outcomes': {'accepted': 6, 'filtered': 0, 'rate_limited': 0, 'invalid': 0, 'abuse': 0, 'client_discard': 0}, 'totals': {'dropped': 0, 'sum(quantity)': 6}}]}]}

    def test_download(self):
        if False:
            print('Hello World!')
        make_request = functools.partial(self.client.get, reverse('sentry-api-0-organization-stats-summary', args=[self.org.slug]))
        response = make_request({'statsPeriod': '2d', 'interval': '1d', 'field': ['sum(quantity)', 'sum(times_seen)'], 'download': True})
