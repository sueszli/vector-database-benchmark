...
@pluginmatcher(name='default', pattern=re.compile('https?://(?:stream|tvpstream\\.vod)\\.tvp\\.pl(?:/(?:\\?channel_id=(?P<channel_id>\\d+))?)?$'))
@pluginmatcher(name='vod', pattern=re.compile('https?://vod\\.tvp\\.pl/[^/]+/.+,(?P<vod_id>\\d+)$'))
@pluginmatcher(name='tvp_info', pattern=re.compile('https?://(?:www\\.)?tvp\\.info/'))
class TVP(Plugin):
