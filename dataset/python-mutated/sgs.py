...
import warnings
import isort
INGREDIENTS_START = re.compile('\\s*#\\s*<INGREDIENT ([\\w\\d_-]+)>')
INGREDIENTS_END = re.compile('\\s*#\\s*</INGREDIENT>')
IMPORTS_FILL = re.compile('\\s*#\\s*<IMPORTS/>')
INGREDIENT_FILL = re.compile('\\s*#\\s*<INGREDIENT ([\\d\\w_-]+)\\s?/>')
REGION_START = re.compile('#\\s*<REGION ([\\d\\w_-]+)>')
REGION_END = re.compile('#\\s*</REGION ([\\d\\w_-]+)>')
HEADER = '# This file is automatically generated. Please do not modify it directly.\n# Find the relevant recipe file in the samples/recipes or samples/ingredients\n# directory and apply your changes there.\n'
...
IGNORED_OUTPUT_FILES = (re.compile('.*noxfile\\.py$'), re.compile('.*noxfile_config\\.py$'), re.compile('.*README\\.md$'), re.compile('.*requirements\\.txt$'), re.compile('.*requirements-test\\.txt$'), re.compile('.*?/tests/.*'), re.compile('.*?/__pycache__/.*'), re.compile('.*?sponge_log.xml.*'))
