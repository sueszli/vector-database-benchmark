...
class network_managementCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_connectivity_test': ('parent', 'test_id', 'resource'), 'delete_connectivity_test': ('name',), 'get_connectivity_test': ('name',), 'list_connectivity_tests': ('parent', 'page_size', 'page_token', 'filter', 'order_by'), 'rerun_connectivity_test': ('name',), 'update_connectivity_test': ('update_mask', 'resource')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=network_managementCallTransformer()):
    if False:
        return 10
