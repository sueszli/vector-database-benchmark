...
from ray.tests.test_state_api import generate_task_data, generate_task_event, generate_actor_data, generate_object_info
from ray.util.state.common import DEFAULT_RPC_TIMEOUT, SummaryApiOptions, Link, NestedTaskSummary, TaskSummaries, DRIVER_TASK_ID_PREFIX
...
async def test_api_manager_summary_tasks(state_api_manager):
    ...
    data_source_client.get_all_task_info.side_effect = [generate_task_data([generate_task_event(id=ids[0].binary(), func_or_class=first_task_name, state=TaskStatus.PENDING_NODE_ASSIGNMENT, type=TaskType.NORMAL_TASK), generate_task_event(id=ids[1].binary(), func_or_class=first_task_name, state=TaskStatus.PENDING_NODE_ASSIGNMENT, type=TaskType.NORMAL_TASK), generate_task_event(id=ids[2].binary(), func_or_class=first_task_name, state=TaskStatus.PENDING_NODE_ASSIGNMENT, type=TaskType.NORMAL_TASK), generate_task_event(id=ids[3].binary(), func_or_class=first_task_name, state=TaskStatus.RUNNING, type=TaskType.NORMAL_TASK), generate_task_event(id=ids[4].binary(), func_or_class=second_task_name, state=TaskStatus.PENDING_NODE_ASSIGNMENT, type=TaskType.ACTOR_TASK)])]
...
async def test_api_manager_summary_objects(state_api_manager):
    ...
    data_source_client.get_object_info.side_effect = [GetObjectsInfoReply(core_workers_stats=[generate_object_info(object_ids[0].binary(), size_bytes=1024 ** 2, callsite=first_callsite, task_state=TaskStatus.PENDING_NODE_ASSIGNMENT, local_ref_count=2, attempt_number=0, pid=1, ip='123', worker_type=WorkerType.WORKER, pinned_in_memory=False), generate_object_info(object_ids[1].binary(), size_bytes=1024 ** 2, callsite=first_callsite, task_state=TaskStatus.PENDING_NODE_ASSIGNMENT, local_ref_count=2, pid=2, ip='123', worker_type=WorkerType.WORKER), generate_object_info(object_ids[2].binary(), size_bytes=-1, callsite=first_callsite, task_state=TaskStatus.RUNNING, local_ref_count=1, attempt_number=0, pid=3, ip='1234', worker_type=WorkerType.WORKER)], total=3), GetObjectsInfoReply(core_workers_stats=[generate_object_info(object_ids[3].binary(), size_bytes=1024 ** 2 * 2, callsite=first_callsite, task_state=TaskStatus.RUNNING, local_ref_count=1, attempt_number=0, pid=1, ip='1234', worker_type=WorkerType.WORKER), generate_object_info(object_ids[4].binary(), size_bytes=1024 ** 2, callsite=second_callsite, task_state=TaskStatus.RUNNING, local_ref_count=4, pid=1, attempt_number=0, ip='1234', worker_type=WorkerType.DRIVER)], total=2)]
