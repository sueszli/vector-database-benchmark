...
from sentry.api.serializers.rest_framework import ListField, ReleaseHeadCommitSerializer, ReleaseHeadCommitSerializerDeprecated, ReleaseWithVersionSerializer
from sentry.api.utils import get_auth_api_token_type
from sentry.exceptions import InvalidSearchQuery
from sentry.models.activity import Activity
from sentry.models.orgauthtoken import is_org_auth_token_auth, update_org_auth_token_last_used
from sentry.models.project import Project
from sentry.models.release import Release, ReleaseCommitError, ReleaseProject, ReleaseStatus, SemverFilter
from sentry.search.events.constants import OPERATOR_TO_DJANGO, RELEASE_ALIAS, RELEASE_STAGE_ALIAS, SEMVER_ALIAS, SEMVER_BUILD_ALIAS, SEMVER_PACKAGE_ALIAS
...
_release_suffix = re.compile('^(.*)\\s+\\(([^)]+)\\)\\s*$')
...
class OrganizationReleasesEndpoint(OrganizationReleasesBaseEndpoint, EnvironmentMixin, ReleaseAnalyticsMixin):
    publish_status = {'GET': ApiPublishStatus.UNKNOWN, 'POST': ApiPublishStatus.UNKNOWN}
    SESSION_SORTS = frozenset(['crash_free_sessions', 'crash_free_users', 'sessions', 'users', 'sessions_24h', 'users_24h'])
