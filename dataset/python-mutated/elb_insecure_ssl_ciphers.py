...
class elb_insecure_ssl_ciphers(Check):
    ...
    def execute(self):
        if False:
            ...
            return i + 15
        findings = []
        secure_ssl_policies = ['ELBSecurityPolicy-TLS-1-2-2017-01']
        for lb in elb_client.loadbalancers:
            ...
            report.status = 'PASS'
            report.status_extended = f'ELB {lb.name} does not have insecure SSL protocols or ciphers.'
            for listener in lb.listeners:
                if listener.protocol == 'HTTPS' and (not any((check in listener.policies for check in secure_ssl_policies))):
                    report.status = 'FAIL'
