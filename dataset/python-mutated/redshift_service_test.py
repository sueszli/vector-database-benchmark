...
test_policy = {'Statement': [{'Effect': 'Allow', 'Principal': {'AWS': f'{AWS_ACCOUNT_NUMBER}'}, 'Action': ['redshift:Publish'], 'Resource': f'arn:aws:redshift:{AWS_REGION}:{AWS_ACCOUNT_NUMBER}:{topic_name}'}]}
...
def mock_make_api_call(self, operation_name, kwarg):
    ...
    if operation_name == 'DescribeLoggingStatus':
        return {'LoggingEnabled': True, 'BucketName': test_bucket_name}
    if operation_name == 'DescribeClusterSnapshots':
        return {'Snapshots': [{'SnapshotIdentifier': uuid4()}]}
...
class Test_Redshift_Service:
    ...
    def test_describe_clusters(self):
        ...
        response = redshift_client.create_cluster(DBName='test', ClusterIdentifier=cluster_id, ClusterType='single-node', NodeType='ds2.xlarge', MasterUsername='user', MasterUserPassword='password', PubliclyAccessible=True, Tags=[{'Key': 'test', 'Value': 'test'}])
        audit_info = self.set_mocked_audit_info()
        redshift = Redshift(audit_info)
        ...
        assert redshift.clusters[0].endpoint_address == response['Cluster']['Endpoint']['Address']
        assert redshift.clusters[0].allow_version_upgrade == response['Cluster']['AllowVersionUpgrade']
        assert redshift.clusters[0].tags == [{'Key': 'test', 'Value': 'test'}]
    ...
    def test_describe_logging_status(self):
        if False:
            ...
            return i + 15
        redshift_client = client('redshift', region_name=AWS_REGION)
        response = redshift_client.create_cluster(DBName='test', ClusterIdentifier=cluster_id, ClusterType='single-node', NodeType='ds2.xlarge', MasterUsername='user', MasterUserPassword='password', PubliclyAccessible=True)
        audit_info = self.set_mocked_audit_info()
        redshift = Redshift(audit_info)
        assert len(redshift.clusters) == 1
        assert redshift.clusters[0].id == cluster_id
