...
data = fetch_openml(data_id=179, as_frame=True, parser='pandas')
(X, y) = (data.data, data.target)
cat_columns = make_column_selector(dtype_include='category')(X)
preprocessing = make_column_transformer((OrdinalEncoder(), cat_columns), remainder='passthrough', verbose_feature_names_out=False)
X = pd.DataFrame(preprocessing.fit_transform(X), columns=preprocessing.get_feature_names_out())
...
est = HistGradientBoostingClassifier(loss='log_loss', learning_rate=lr, max_iter=n_trees, max_bins=max_bins, max_leaf_nodes=n_leaf_nodes, categorical_features=is_categorical, early_stopping=False, random_state=0, verbose=verbose)
...
if args.lightgbm:
    ...
    est.set_params(max_cat_to_onehot=1)
