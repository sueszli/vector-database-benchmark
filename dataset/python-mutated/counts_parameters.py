...
if __name__ == '__main__':
    ...
    parser.add_argument('--pruning_method', choices=['l0', 'topK', 'sigmoied_threshold'], type=str, required=True, help='Pruning Method (l0 = L0 regularization, topK = Movement pruning, sigmoied_threshold = Soft movement pruning)')
    parser.add_argument('--threshold', type=float, required=False, help='For `topK`, it is the level of remaining weights (in %) in the fine-pruned model. For `sigmoied_threshold`, it is the threshold \tau against which the (sigmoied) scores are compared. Not needed for `l0`')
    parser.add_argument('--serialization_dir', type=str, required=True, help='Folder containing the model that was previously fine-pruned')
