...
def find_api_url(stack_name):
    ...
    try:
        ...
        print("Couldn't find the REST URL for your API. Try running the following at the command prompt:\n")
        print(f"""\taws cloudformation describe-stacks --stack-name {stack_name} --query "Stacks[0].Outputs[?OutputKey=='EndpointURL'].OutputValue" --output text""")
    else:
        return api_url
...
def demo():
    ...
    parser.add_argument('--api_url', help=f'The base URL of the REST API for the demo. If not specified, the demo looks it up in the {stack_name} AWS CloudFormation stack.')
    ...
    if api_url is not None:
        ...
        historical_data = [{'state': random_state, 'date': (datetime.date.today() - datetime.timedelta(days=index)).isoformat(), 'cases': index * random.randint(1, 100), 'deaths': index * random.randint(0, 20)} for index in range(1, 11)]
        ...
        print(f'Response: {state_response.status_code}')
        pprint.pprint(state_response.json())
        past_date = (datetime.date.today() - datetime.timedelta(days=random.randint(2, 10))).isoformat()
        ...
        date_response = requests.get(date_url)
        print(f'Response: {date_response.status_code}')
        print(f'Sending DELETE request to remove all historical data from {random_state}.')
        state_response = requests.delete(state_url)
        print(f'Response: {state_response.status_code}')
        print(f'Send GET to {state_url} (expect new random data for today).')
        state_response = requests.get(state_url)
        print(f'Response: {state_response.status_code}\n\t{state_response.json()}')
        print('You can remove all resources created for this demo by running the following at a command prompt:')
