...
class LayerDesc:
    ...
    def __repr__(self):
        if False:
            for i in range(10):
                print('nop')
        return layer_to_str(self.layer_func.__name__, *self.inputs, **self.kwargs)
...
class SegmentLayers:
        ...
        if isinstance(self.method, list):
            ...
            def check_sanity():
                ...
                for part in seg_method:
                    ...
                    assert part <= self.num_items, f'part[{part}] should be less than num_items[{self.num_items}]'
            check_sanity()
            if self.num_parts == source_num_parts + 1:
                ...
                raise ValueError('We set seg_method as {}, this length is {}, but the number of stages is {}'.format(seg_method, len(seg_method), self.num_parts))
            ...
            assert sum(weights) % actual_num_parts == 0, 'number of layers ({}) should be divided by part number({})'.format(sum(weights), actual_num_parts)
    ...
    def _gen_layer_weight(self, layername):
        ...
        assert len(weight_idxs) > 0, "weight_idxs' length should be greater than 0"
        return weight_idxs
...
class PipelineLayerChunk(nn.Layer):
    ...
    def forward(self, *args, **kwargs):
        if False:
            while True:
                i = 10
        raise PermissionError('The forward function of PipelineLayerChunk cannot be called directly. Please call forward function of PipelineLayer.')
...
class PipelineLayer(nn.Layer):
    ...
    def __init__(self, layers, num_stages=None, topology=None, loss_fn=None, seg_method='uniform', recompute_interval=0, recompute_ctx=None, num_virtual_pipeline_stages=None):
        ...
        if num_virtual_pipeline_stages:
            assert isinstance(num_virtual_pipeline_stages, int), 'virtual_pipeline_stage should be None or an int'
            if num_virtual_pipeline_stages > 1:
                logger.info('set num_virtual_pipeline_stages > 1 means using interleave scheduler instead of 1f1b scheduler')
                assert isinstance(seg_method, str), 'seg_method should be a str for interleave scheduler'
                assert seg_method.startswith('layer:'), 'seg_method shoud be start with layer: for interleave scheduler'
        ...
        if recompute_interval > 0:
            ...
            logger.info('Start Recompute for PipeLineParallel. recompute_offload: {}, recompute_partition: {}'.format(offload, partition))
        ...
        if self._topo:
            ...
            if num_stages:
                assert self._num_stages == num_stages, 'num_stages should be equal to be %d' % self._num_stages
        else:
            if world_size % num_stages != 0:
                raise ValueError(f'should provide correct num_stages({num_stages}) which can be divided by world_size({world_size})')
            dp_num = world_size // num_stages
            self._topo = fleet.CommunicateTopology(['data', 'pipe', 'model'], [dp_num, num_stages, 1])
            self._stage_id = self._topo.get_coord(self.global_rank).pipe
            self._num_stages = self._topo.get_dim_size('pipe')
    ...
    def get_stage_from_index(self, layer_idx):
        ...
        for virtual_pp_rank in range(self._num_virtual_pipeline_stages):
            ...
            for stage in range(self._num_stages):
                if self.segment_parts[start_idx + stage] <= layer_idx < self.segment_parts[start_idx + stage + 1]:
                    return stage
    ...
    def _construct_shared_comm(self):
        ...
        shared_layer_names = {s.layer_name for s in layers_desc if isinstance(s, SharedLayerDesc)}
        for key in shared_layer_names:
            shared_layers = []
            for (idx, layer) in enumerate(layers_desc):
                if isinstance(layer, SharedLayerDesc) and layer.layer_name == key:
                    shared_layers.append(idx)
            shared_stages = {self.get_stage_from_index(idx) for idx in shared_layers}
                ...
                for sharding in range(self._sharding_degree):
                    for mp in range(self._mp_degree):
                        ...
                        for s in sorted(shared_stages):
                            shared_ranks.append(self._topo.get_rank_from_stage(self.global_rank, pipe=s, data=dp, sharding=sharding, model=mp))
                        ...
                        if self.global_rank in shared_ranks:
                            ...
                            if key in self.shared_layers:
                                shared_comm[key] = {'ranks': shared_ranks, 'group': group, 'weight_attr': self.shared_weight_attrs[key], 'layer': self.shared_layers[key]}
        return shared_comm
    ...
    def _synchronize_shared_weights(self):
        ...
        for (key, comm) in self.shared_comm.items():
            with paddle.framework.no_grad():
                paddle.distributed.broadcast(getattr(comm['layer'], comm['weight_attr']), src=min(comm['ranks']), group=comm['group'])
    ...
    def allreduce_shared_weight_gradients(self):
        ...
        for (key, comm) in self.shared_comm.items():
            ...
            if framework.in_dynamic_mode():
                with paddle.framework.no_grad():
                    paddle.distributed.all_reduce(param.grad if not hasattr(param, 'main_grad') else param.main_grad, group=comm['group'])
            else:
                with paddle.framework.no_grad():
                    framework._dygraph_tracer().trace_op(type='c_allreduce_sum', inputs={'X': param._grad_ivar()}, outputs={'Out': param._grad_ivar()}, attrs={'ring_id': comm['group'].id, 'use_calc_stream': True})
    ...
    def _segment_network_for_interleave(self, seg_method):
        ...
        seg = SegmentLayers(self._layers_desc, num_parts=self._num_stages, method=seg_method, num_virtual_pipeline_stage=self._num_virtual_pipeline_stages)
        self.segment_parts = seg.do_segment()
        logger.info(f'segment with method: {seg_method}; result: ' + ', '.join((str(arg) for arg in self.segment_parts)))
        for i in range(self._stage_id, self._total_stages_with_virtual_stages, self._num_stages):
    ...
    def _print_segmentation_for_debug(self):
        ...
        for stage in range(self._num_stages * self._num_virtual_pipeline_stages):
            ...
            logger.info('stage={}, global_rank={} ,layer_number={}'.format(stage, self.global_rank, end - start))
            for (index, layer) in enumerate(self._layers_desc[start:end]):
                logger.info(f'{index + start}: {str(layer)}')
        if self._num_virtual_pipeline_stages > 1:
            for stage in range(self._num_stages):
                stage_to_virtual_stage_info = f'stage {stage} contains virtual stages: '
                for i in range(stage, self._total_stages_with_virtual_stages, self._num_stages):
        ...
        for (index, layer) in enumerate(self._layers_desc[start:end]):
            ...
            if isinstance(layer, nn.Layer):
                ...
                if layer.layer_name not in self.shared_layers:
                    ...
                    self.shared_weight_attrs[layer.layer_name] = layer.shared_weight_attr
                    for param in self.shared_layers[layer.layer_name].parameters():
                        param.is_firstly_shared = True
                if layer.forward_func is None:
                    run_function.append(self.shared_layers[layer.layer_name])
                else:
                    run_function.append(partial(layer.forward_func, self.shared_layers[layer.layer_name]))
                    if self._num_virtual_pipeline_stages > 1:
                        run_function.add_sublayer(layer.layer_name, self.shared_layers[layer.layer_name])
            elif isinstance(layer, LayerDesc):
    ...
    def forward(self, input, chunk_id=None):
        ...
        if chunk_id is not None:
            ...
            assert self._num_virtual_pipeline_stages > 1, 'chunk_id is only valid when using virtual pipeline stage'
            assert chunk_id < len(self._model_chunks), f'The virtual pipeline only has {len(self._model_chunks)} chunks, but received chunk_id {chunk_id}.'
        ...
        if self._recompute_interval == 0:
            ...
            for start_idx in range(0, num_layers, self._recompute_interval):
                ...
                if self._need_recompute(funcs, input):
                    input = recompute_hybrid(self.recompute_ctx, self.forward_function(start_idx, end_idx), *input)
                else:
    ...
    def _need_recompute(self, funcs, inputs):
        if False:
            for i in range(10):
                print('nop')
        if not any((not input_.stop_gradient for input_ in inputs if isinstance(input_, paddle.Tensor))):
            return False
    ...
    def save_state_dict(self, path):
        ...
        def _offset_dirname(ckpt_dir, local_layer_idx, local_chunk_id=None):
            ...
            if self._num_virtual_pipeline_stages > 1:
                ...
                virtual_pipeline_stage_message = f'-virtual_pp_stage_{local_chunk_id:0>2d}'
    ...
    def set_state_dir(self, path):
        ...
        def _load_model(run_functions, local_chunk_id=None):
            ...
            for (idx, layer) in enumerate(run_functions):
                ...
                if self._num_virtual_pipeline_stages > 1:
                    assert local_chunk_id is not None
                    layer_save_path = layer_save_path + f'-virtual_pp_stage_{local_chunk_id:0>2d}'
