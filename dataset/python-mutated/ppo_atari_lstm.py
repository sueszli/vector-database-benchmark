...
class Agent(nn.Module):
    ...
    def __init__(self, envs):
        if False:
            return 10
        super().__init__()
        self.network = nn.Sequential(layer_init(nn.Conv2d(1, 32, 8, stride=4)), nn.ReLU(), layer_init(nn.Conv2d(32, 64, 4, stride=2)), nn.ReLU(), layer_init(nn.Conv2d(64, 64, 3, stride=1)), nn.ReLU(), nn.Flatten(), layer_init(nn.Linear(64 * 7 * 7, 512)), nn.ReLU())
    ...
    def get_states(self, x, lstm_state, done):
        ...
        for (h, d) in zip(hidden, done):
            (h, lstm_state) = self.lstm(h.unsqueeze(0), ((1.0 - d).view(1, -1, 1) * lstm_state[0], (1.0 - d).view(1, -1, 1) * lstm_state[1]))
            new_hidden += [h]
    ...
    for update in range(1, num_updates + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                (_, newlogprob, entropy, newvalue, _) = agent.get_action_and_value(b_obs[mb_inds], (initial_lstm_state[0][:, mbenvinds], initial_lstm_state[1][:, mbenvinds]), b_dones[mb_inds], b_actions.long()[mb_inds])
