...
if is_torch_available():
    import torch
    from transformers import MODEL_MAPPING, SegformerForImageClassification, SegformerForSemanticSegmentation, SegformerModel
...
class SegformerModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (SegformerModel, SegformerForSemanticSegmentation, SegformerForImageClassification) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': SegformerModel, 'image-classification': SegformerForImageClassification, 'image-segmentation': SegformerForSemanticSegmentation} if is_torch_available() else {}
    fx_compatible = True
...
class SegformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_segmentation_ade(self):
        ...
        model = SegformerForSemanticSegmentation.from_pretrained('nvidia/segformer-b0-finetuned-ade-512-512').to(torch_device)
        image = prepare_img()
        ...
        expected_slice = torch.tensor([[[-4.631, -5.5232, -6.2356], [-5.1921, -6.1444, -6.5996], [-5.4424, -6.279, -6.7574]], [[-12.1391, -13.3122, -13.9554], [-12.8732, -13.9352, -14.3563], [-12.9438, -13.8226, -14.2513]], [[-12.5134, -13.4686, -14.4915], [-12.8669, -14.4343, -14.7758], [-13.2523, -14.5819, -15.0694]]]).to(torch_device)
        self.assertTrue(torch.allclose(outputs.logits[0, :3, :3, :3], expected_slice, atol=0.0001))
    ...
    def test_inference_image_segmentation_city(self):
        if False:
            while True:
                i = 10
        image_processor = SegformerImageProcessor(image_scale=(512, 512), keep_ratio=False, align=False, do_random_crop=False)
        model = SegformerForSemanticSegmentation.from_pretrained('nvidia/segformer-b1-finetuned-cityscapes-1024-1024').to(torch_device)
        image = prepare_img()
        encoded_inputs = image_processor(images=image, return_tensors='pt')
        pixel_values = encoded_inputs.pixel_values.to(torch_device)
        with torch.no_grad():
        ...
        expected_shape = torch.Size((1, model.config.num_labels, 128, 128))
        self.assertEqual(outputs.logits.shape, expected_shape)
        expected_slice = torch.tensor([[[-13.5748, -13.9111, -12.65], [-14.35, -15.3683, -14.2328], [-14.7532, -16.0424, -15.6087]], [[-17.1651, -15.8725, -12.9653], [-17.258, -17.3718, -14.8223], [-16.6058, -16.8783, -16.7452]], [[-3.6456, -3.0209, -1.4203], [-3.0797, -3.1959, -2.0], [-1.8757, -1.9217, -1.6997]]]).to(torch_device)
        self.assertTrue(torch.allclose(outputs.logits[0, :3, :3, :3], expected_slice, atol=0.1))
