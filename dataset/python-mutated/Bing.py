...
class Tones:
    ...
    balanced = 'Balanced'
    precise = 'Precise'
default_cookies = {'SRCHD': 'AF=NOFORM', 'PPLState': '1', 'KievRPSSecAuth': '', 'SUID': '', 'SRCHUSR': '', 'SRCHHPGUSR': ''}
...
async def create_conversation(session: ClientSession, tone: str, image: str=None, proxy: str=None) -> Conversation:
    ...
    async with session.get(url, proxy=proxy) as response:
        ...
        if isinstance(image, str):
            try:
                config = {'visualSearch': {'maxImagePixels': 360000, 'imageCompressionRate': 0.7, 'enableFaceBlurDebug': 0}}
                ...
                async with session.post('https://www.bing.com/images/kblob', data=data, headers=headers, proxy=proxy) as image_upload_response:
                    ...
                    result['originalImageUrl'] = 'https://www.bing.com/images/blob?bcid=' + result['blurredBcid'] if config['visualSearch']['enableFaceBlurDebug'] else 'https://www.bing.com/images/blob?bcid=' + result['bcid']
...
async def delete_conversation(session: ClientSession, conversation: Conversation, proxy: str=None) -> list:
    ...
    json = {'conversationId': conversation.conversationId, 'conversationSignature': conversation.conversationSignature, 'participant': {'id': conversation.clientId}, 'source': 'cib', 'optionsSets': ['autosave']}
...
class Defaults:
    ...
    allowedMessageTypes = ['ActionRequest', 'Chat', 'Context', 'Progress', 'SemanticSerp', 'GenerateContentQuery', 'SearchQuery', 'RenderCardRequest']
    sliceIds = ['abv2', 'srdicton', 'convcssclick', 'stylewv2', 'contctxp2tf', '802fluxv1pc_a', '806log2sphs0', '727savemem', '277teditgnds0', '207hlthgrds0']
    location = {'locale': 'en-US', 'market': 'en-US', 'region': 'US', 'locationHints': [{'country': 'United States', 'state': 'California', 'city': 'Los Angeles', 'timezoneoffset': 8, 'countryConfidence': 8, 'Center': {'Latitude': 34.0536909, 'Longitude': -118.242766}, 'RegionType': 2, 'SourceType': 1}]}
    headers = {'accept': '*/*', 'accept-language': 'en-US,en;q=0.9', 'cache-control': 'max-age=0', 'sec-ch-ua': '"Chromium";v="110", "Not A(Brand";v="24", "Microsoft Edge";v="110"', 'sec-ch-ua-arch': '"x86"', 'sec-ch-ua-bitness': '"64"', 'sec-ch-ua-full-version': '"110.0.1587.69"', 'sec-ch-ua-full-version-list': '"Chromium";v="110.0.5481.192", "Not A(Brand";v="24.0.0.0", "Microsoft Edge";v="110.0.1587.69"', 'sec-ch-ua-mobile': '?0', 'sec-ch-ua-model': '""', 'sec-ch-ua-platform': '"Windows"', 'sec-ch-ua-platform-version': '"15.0.0"', 'sec-fetch-dest': 'document', 'sec-fetch-mode': 'navigate', 'sec-fetch-site': 'none', 'sec-fetch-user': '?1', 'upgrade-insecure-requests': '1', 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36 Edg/110.0.1587.69', 'x-edge-shopping-flag': '1', 'x-forwarded-for': ip_address}
    optionsSets = ['nlu_direct_response_filter', 'deepleo', 'disable_emoji_spoken_text', 'responsible_ai_policy_235', 'enablemm', 'iyxapbing', 'iycapbing', 'gencontentv3', 'fluxsrtrunc', 'fluxtrunc', 'fluxv1', 'rai278', 'replaceurl', 'eredirecturl', 'nojbfedge']
...
async def stream_generate(prompt: str, tone: str, image: str=None, context: str=None, proxy: str=None, cookies: dict=None):
    async with ClientSession(timeout=ClientTimeout(total=900), cookies=cookies, headers=Defaults.headers) as session:
