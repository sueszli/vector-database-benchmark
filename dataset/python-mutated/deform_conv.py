...
try:
    ...
    try:
        ...
        deform_conv_cpu = cpp_extension.load(name='deform_conv_cpu', sources=[os.path.join(dcn_dir, 'src', 'deform_conv_cpu.cpp'), os.path.join(dcn_dir, 'src', 'deform_conv_cpu_kernel.cpp')])
        warnings.warn('Done.')
        dcn_cpu_ready = True
    except Exception as error:
        warnings.warn(' '.join(["Failed to import and/or compile 'deform_conv_cpu' with the following error", '{}'.format(error), 'Deformable convulution and DBNet will not be able to run on CPU.']))
        dcn_cpu_ready = False
if torch.cuda.is_available():
    try:
        ...
        try:
            ...
            warnings.warn('(This may take a while if this module is loaded for the first time.)')
            cuda_sources = [os.path.join(dcn_dir, 'src', src_file) for src_file in ['deform_conv_cuda.cpp', 'deform_conv_cuda_kernel.cu']]
            deform_conv_cuda = cpp_extension.load(name='deform_conv_cuda', sources=[os.path.join(dcn_dir, 'src', 'deform_conv_cuda.cpp'), os.path.join(dcn_dir, 'src', 'deform_conv_cuda_kernel.cu')])
            warnings.warn('Done.')
            dcn_cuda_ready = True
        except Exception as error:
            warnings.warn(' '.join(["Failed to import or compile 'deform_conv_cuda' with the following error", '{}'.format(error), 'Deformable convulution and DBNet will not be able to run on GPU.']))
