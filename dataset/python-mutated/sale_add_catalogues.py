...
from ....core.utils import WebhookEventInfo, from_global_id_or_error, raise_validation_error
from ...utils import CatalogueInfo, convert_catalogue_info_into_predicate, convert_migrated_sale_predicate_to_catalogue_info, merge_catalogues_info
from .sale_base_catalogue import SaleBaseCatalogueMutation

class SaleAddCatalogues(SaleBaseCatalogueMutation):

    class Meta:
        description = 'Adds products, categories, collections to a sale.' + DEPRECATED_IN_3X_MUTATION + ' Use `promotionRuleCreate` mutation instead.'
        doc_category = DOC_CATEGORY_DISCOUNTS
        permissions = (DiscountPermissions.MANAGE_DISCOUNTS,)
        error_type_class = DiscountError
        error_type_field = 'discount_errors'
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.SALE_UPDATED, description='A sale was updated.')]
    ...
    def perform_mutation(cls, _root, info: ResolveInfo, /, *, id: str, input):
        ...
        with traced_atomic_transaction():
            new_catalogue_info = cls.add_items_to_catalogue(rules, previous_catalogue_info, input)
    ...
    def get_instance(cls, _info: ResolveInfo, id):
        ...
        if type == 'Promotion':
            raise_validation_error(field='id', message="Provided ID refers to Promotion model. Please use 'promotionRuleCreate' mutation instead.", code=DiscountErrorCode.INVALID.value)
    ...
    @classmethod
    def add_items_to_catalogue(cls, rules: list[PromotionRule], previous_catalogue_info: CatalogueInfo, input) -> Optional[dict]:
        if False:
            while True:
                i = 10
        catalogue_info_to_add = cls.get_catalogue_info_from_input(input)
        if any(catalogue_info_to_add):
            new_catalogue = merge_catalogues_info(previous_catalogue_info, catalogue_info_to_add)
