...
parser.add_argument('--wandb-project', type=str, default='anonymous-rl-code/action-guidance', help='the name of wandb project (e.g. cleanrl/cleanrl)')
parser.add_argument('--feature-of-interest', type=str, default='charts/episodic_return/ProduceCombatUnitRewardFunction', help='which feature to be plotted on the y-axis')
parser.add_argument('--hyper-params-tuned', nargs='+', default=['shift', 'adaptation'], help='the hyper parameters tuned')
parser.add_argument('--interested-exp-names', nargs='+', default=['ppo_ac_positive_reward-shift-800000--adaptation-1000000--positive_likelihood-1-', 'ppo_ac_positive_reward-shift-800000--adaptation-1000000--positive_likelihood-0-'], help='the hyper parameters tuned')
parser.add_argument('--samples', type=int, default=500, help='the sampled point of the run')
parser.add_argument('--smooth-weight', type=float, default=0.9, help='the weight parameter of the exponential moving average')
parser.add_argument('--last-n-episodes', type=int, default=50, help='for analysis only; the last n episodes from which the mean of the feature of interest is calculated')
...
env_dict = {'MicrortsAttackShapedReward-v1': 'MicrortsAttackHRL-v1', 'MicrortsProduceCombatUnitsShapedReward-v1': 'MicrortsProduceCombatUnitHRL-v1', 'MicrortsRandomEnemyShapedReward3-v1': 'MicrortsRandomEnemyHRL3-v1'}
exp_convert_dict = {'ppo_positive_reward-positive_likelihood-0-': 'sparse reward - no PLO', 'ppo': 'sparse reward', 'ppo_ac_positive_reward-shift-2000000--adaptation-2000000--positive_likelihood-0-': 'action guidance - multi-agent  w/ PLO', 'ppo_ac_positive_reward-shift-2000000--adaptation-7000000--positive_likelihood-0-': 'action guidance - long adaptation w/ PLO', 'ppo_ac_positive_reward-shift-800000--adaptation-1000000--positive_likelihood-0-': 'action guidance - short adaptation w/ PLO', 'ppo_ac_positive_reward-shift-2000000--adaptation-7000000--positive_likelihood-1-': 'action guidance - long adaptation', 'ppo_ac_positive_reward-shift-800000--adaptation-1000000--positive_likelihood-1-': 'action guidance - short adaptation', 'pposhaped': 'shaped reward'}
...
interested_exp_names = sorted(list(set(exp_names)))
...
for env in set(all_df['env_id']):
    ...
    ax = sns.lineplot(data=data.groupby(['seed', 'algo']).apply(_smooth).loc[data['algo'].isin(interested_exp_names)], x='global_step', y=args.feature_of_interest, hue='algo', units='seed', estimator=None, palette=current_palette_dict, alpha=0.2)
    sns.lineplot(data=data.groupby(['seed', 'algo']).apply(_smooth).loc[data['algo'].isin(interested_exp_names)], x='global_step', y=args.feature_of_interest, hue='algo', ci=None, palette=current_palette_dict, linewidth=2.0)
    ax.set(xlabel=args.x_label, ylabel=args.y_label)
    (handles, labels) = ax.get_legend_handles_labels()
    legend = ax.legend(handles=handles[1:len(labels) // 2], labels=labels[1:len(labels) // 2], loc='upper center', bbox_to_anchor=(0.5, -0.2), fancybox=True)
    for text in legend.get_texts():
        text.set_text(exp_convert_dict[text.get_text()])
    ...
    for algo in interested_exp_names:
        ...
        last_n_episodes_features = algo_data[algo_data['global_step'] > last_n_episodes_global_step].groupby(['seed']).mean()[args.feature_of_interest]
