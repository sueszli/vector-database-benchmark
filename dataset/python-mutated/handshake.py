...
from scapy.fields import ByteEnumField, ByteField, Field, FieldLenField, IntField, PacketField, PacketLenField, PacketListField, ShortEnumField, ShortField, StrFixedLenField, StrLenField, ThreeBytesField, UTCTimeField
...
from scapy.layers.tls.extensions import _ExtensionsLenField, _ExtensionsField, _cert_status_type, TLS_Ext_SupportedVersion_CH, TLS_Ext_SignatureAlgorithms, TLS_Ext_SupportedVersion_SH, TLS_Ext_EarlyDataIndication, _tls_hello_retry_magic, TLS_Ext_ExtendedMasterSecret, TLS_Ext_EncryptThenMAC
...
_tls_handshake_type = {0: 'hello_request', 1: 'client_hello', 2: 'server_hello', 3: 'hello_verify_request', 4: 'session_ticket', 6: 'hello_retry_request', 8: 'encrypted_extensions', 11: 'certificate', 12: 'server_key_exchange', 13: 'certificate_request', 14: 'server_hello_done', 15: 'certificate_verify', 16: 'client_key_exchange', 20: 'finished', 21: 'certificate_url', 22: 'certificate_status', 23: 'supplemental_data', 24: 'key_update'}
...
class TLSClientHello(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 1, _tls_handshake_type), ThreeBytesField('msglen', None), _TLSClientVersionField('version', None, _tls_version), _GMTUnixTimeField('gmt_unix_time', None), _TLSRandomBytesField('random_bytes', None, 28), FieldLenField('sidlen', None, fmt='B', length_of='sid'), _SessionIDField('sid', '', length_from=lambda pkt: pkt.sidlen), FieldLenField('cipherslen', None, fmt='!H', length_of='ciphers'), _CipherSuitesField('ciphers', None, _tls_cipher_suites, itemfmt='!H', length_from=lambda pkt: pkt.cipherslen), FieldLenField('complen', None, fmt='B', length_of='comp'), _CompressionMethodsField('comp', [0], _tls_compression_algs, itemfmt='B', length_from=lambda pkt: pkt.complen), _ExtensionsLenField('extlen', None, length_of='ext'), _ExtensionsField('ext', None, length_from=lambda pkt: pkt.msglen - (pkt.sidlen or 0) - (pkt.cipherslen or 0) - (pkt.complen or 0) - 40)]
...
_tls_13_server_hello_fields = [ByteEnumField('msgtype', 2, _tls_handshake_type), ThreeBytesField('msglen', None), _TLSVersionField('version', 771, _tls_version), _TLSRandomBytesField('random_bytes', None, 32), FieldLenField('sidlen', None, length_of='sid', fmt='B'), _SessionIDField('sid', '', length_from=lambda pkt: pkt.sidlen), ShortEnumField('cipher', None, _tls_cipher_suites), _CompressionMethodsField('comp', [0], _tls_compression_algs, itemfmt='B', length_from=lambda pkt: 1), _ExtensionsLenField('extlen', None, length_of='ext'), _ExtensionsField('ext', None, length_from=lambda pkt: pkt.msglen - 38)]
...
class TLS13Certificate(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 11, _tls_handshake_type), ThreeBytesField('msglen', None), FieldLenField('cert_req_ctxt_len', None, fmt='B', length_of='cert_req_ctxt'), StrLenField('cert_req_ctxt', '', length_from=lambda pkt: pkt.cert_req_ctxt_len), _ASN1CertLenField('certslen', None, length_of='certs'), _ASN1CertAndExtListField('certs', [], _ASN1CertAndExt, length_from=lambda pkt: pkt.certslen)]
...
class TLSServerKeyExchange(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 12, _tls_handshake_type), ThreeBytesField('msglen', None), _TLSServerParamsField('params', None, length_from=lambda pkt: pkt.msglen), _TLSSignatureField('sig', None, length_from=lambda pkt: pkt.msglen - len(pkt.params))]
...
_tls_client_certificate_types = {1: 'rsa_sign', 2: 'dss_sign', 3: 'rsa_fixed_dh', 4: 'dss_fixed_dh', 5: 'rsa_ephemeral_dh_RESERVED', 6: 'dss_ephemeral_dh_RESERVED', 20: 'fortezza_dms_RESERVED', 64: 'ecdsa_sign', 65: 'rsa_fixed_ecdh', 66: 'ecdsa_fixed_ecdh'}
...
class URLAndOptionalHash(Packet):
    ...
    fields_desc = [FieldLenField('urllen', None, length_of='url'), StrLenField('url', '', length_from=lambda pkt: pkt.urllen), FieldLenField('hash_present', None, fmt='B', length_of='hash', adjust=lambda pkt, x: int(math.ceil(x / 20.0))), StrLenField('hash', '', length_from=lambda pkt: 20 * pkt.hash_present)]
...
class TLSCertificateURL(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 21, _tls_handshake_type), ThreeBytesField('msglen', None), ByteEnumField('certchaintype', None, _tls_cert_chain_types), FieldLenField('uahlen', None, length_of='uah'), PacketListField('uah', [], URLAndOptionalHash, length_from=lambda pkt: pkt.uahlen)]
...
class TLSCertificateStatus(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 22, _tls_handshake_type), ThreeBytesField('msglen', None), ByteEnumField('status_type', 1, _cert_status_type), ThreeBytesLenField('responselen', None, length_of='response'), _StatusField('response', None, Raw, length_from=lambda pkt: pkt.responselen)]
...
class SupDataEntryUM(Packet):
    ...
    fields_desc = [ShortField('sdtype', None), FieldLenField('len', None, length_of='data', adjust=lambda pkt, x: x + 2), FieldLenField('dlen', None, length_of='data'), PacketListField('data', [], UserMappingData, length_from=lambda pkt: pkt.dlen)]
...
class TLSSupplementalData(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 23, _tls_handshake_type), ThreeBytesField('msglen', None), ThreeBytesLenField('sdatalen', None, length_of='sdata'), PacketListField('sdata', [], SupDataEntry, length_from=lambda pkt: pkt.sdatalen)]
...
class TLSNewSessionTicket(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 4, _tls_handshake_type), ThreeBytesField('msglen', None), IntField('lifetime', 4294967295), FieldLenField('ticketlen', None, length_of='ticket'), StrLenField('ticket', '', length_from=lambda pkt: pkt.ticketlen)]
...
class TLS13NewSessionTicket(_TLSHandshake):
    ...
    fields_desc = [ByteEnumField('msgtype', 4, _tls_handshake_type), ThreeBytesField('msglen', None), IntField('ticket_lifetime', 4294967295), IntField('ticket_age_add', 0), FieldLenField('noncelen', None, fmt='B', length_of='ticket_nonce'), StrLenField('ticket_nonce', '', length_from=lambda pkt: pkt.noncelen), FieldLenField('ticketlen', None, length_of='ticket'), StrLenField('ticket', '', length_from=lambda pkt: pkt.ticketlen), _ExtensionsLenField('extlen', None, length_of='ext'), _ExtensionsField('ext', None, length_from=lambda pkt: (pkt.msglen - (pkt.ticketlen or 0) - pkt.noncelen or 0) - 13)]
...
_tls_handshake_cls = {0: TLSHelloRequest, 1: TLSClientHello, 2: TLSServerHello, 3: TLSHelloVerifyRequest, 4: TLSNewSessionTicket, 8: TLSEncryptedExtensions, 11: TLSCertificate, 12: TLSServerKeyExchange, 13: TLSCertificateRequest, 14: TLSServerHelloDone, 15: TLSCertificateVerify, 16: TLSClientKeyExchange, 20: TLSFinished, 21: TLSCertificateURL, 22: TLSCertificateStatus, 23: TLSSupplementalData}
