...
if is_torch_available():
    import torch
    from transformers import PhiForCausalLM, PhiForSequenceClassification, PhiForTokenClassification, PhiModel
...
class PhiModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (PhiModel, PhiForCausalLM, PhiForSequenceClassification, PhiForTokenClassification) if is_torch_available() else ()
    all_generative_model_classes = (PhiForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': PhiModel, 'text-classification': PhiForSequenceClassification, 'text-generation': PhiForCausalLM, 'token-classification': PhiForTokenClassification, 'zero-shot': PhiForSequenceClassification} if is_torch_available() else {}
