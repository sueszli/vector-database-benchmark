...
from paddle.static.quantization import AddQuantDequantPass, OutScaleForInferencePass, OutScaleForTrainingPass, QuantizationFreezePass, QuantizationTransformPass
...
class TestQuantizationScalePass(unittest.TestCase):
    ...
    def quantization_scale(self, use_cuda, seed, activation_quant_type, weight_quant_type='abs_max', for_ci=False):
        if False:
            print('Hello World!')
        ...
        mapping_table_path = os.path.join(tempdir.name, 'quant_scale_model' + dev_name + '.txt')
        ...
        with paddle.static.scope_guard(scope):
            ...
            feed_vars = [server_program.global_block().var(name) for name in feed_list]
            paddle.static.save_inference_model(save_path, feed_vars, [loss], exe, program=server_program, clip_extra=True)
        tempdir.cleanup()
    ...
    def test_quant_scale_cuda(self):
        ...
        if core.is_compiled_with_cuda():
            with paddle.utils.unique_name.guard():
                self.quantization_scale(True, seed=1, activation_quant_type='moving_average_abs_max', weight_quant_type='channel_wise_abs_max', for_ci=True)
    ...
    def test_quant_scale_cpu(self):
        if False:
            ...
            return i + 15
        with paddle.utils.unique_name.guard():
            self.quantization_scale(False, seed=2, activation_quant_type='moving_average_abs_max', weight_quant_type='channel_wise_abs_max', for_ci=True)
