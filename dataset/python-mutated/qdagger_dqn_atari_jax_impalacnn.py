...
if __name__ == '__main__':
    ...
    q_state = TrainState.create(apply_fn=q_network.apply, params=q_network.init(q_key, envs.observation_space.sample()), target_params=q_network.init(q_key, envs.observation_space.sample()), tx=optax.adam(learning_rate=args.learning_rate))
    ...
    teacher_episodic_returns = evaluate(teacher_model_path, make_env, args.env_id, eval_episodes=args.teacher_eval_episodes, run_name=f'{run_name}-teacher-eval', Model=TeacherModel, epsilon=0.05, capture_video=False)
    writer.add_scalar('charts/teacher/avg_episodic_return', np.mean(teacher_episodic_returns), 0)
    teacher_rb = ReplayBuffer(args.buffer_size, envs.single_observation_space, envs.single_action_space, 'cpu', optimize_memory_usage=True, handle_timeout_termination=False)
    ...
    def update(q_state, observations, actions, next_observations, rewards, dones, distill_coeff):
        ...
        def loss(params, td_target, teacher_q_values, distill_coeff):
            if False:
                i = 10
                return i + 15
            student_q_values = q_network.apply(params, observations)
            q_pred = student_q_values[np.arange(student_q_values.shape[0]), actions.squeeze()]
        ...
        ((loss_value, (q_loss, q_pred, distill_loss)), grads) = jax.value_and_grad(loss, has_aux=True)(q_state.params, td_target, teacher_q_values, distill_coeff)
    ...
    for global_step in track(range(args.offline_steps), description='offline student training'):
        ...
        (loss, q_loss, old_val, distill_loss, q_state) = update(q_state, data.observations.numpy(), data.actions.numpy(), data.next_observations.numpy(), data.rewards.flatten().numpy(), data.dones.flatten().numpy(), 1.0)
    ...
    for global_step in track(range(args.total_timesteps), description='online student training'):
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                (loss, q_loss, old_val, distill_loss, q_state) = update(q_state, data.observations.numpy(), data.actions.numpy(), data.next_observations.numpy(), data.rewards.flatten().numpy(), data.dones.flatten().numpy(), distill_coeff)
                if global_step % 100 == 0:
