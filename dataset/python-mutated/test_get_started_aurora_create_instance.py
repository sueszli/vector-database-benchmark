...
class MockManager:
    ...
    def __init__(self, stub_runner, cluster_data, input_mocker):
        ...
        self.scenario_args = [{'DBClusterIdentifier': self.cluster_id, 'Engine': self.db_engine, 'EngineVersion': self.db_engine_version}]
    ...
    def setup_stubs(self, error, stop_on, stubber):
        ...
        with self.stub_runner(error, stop_on) as runner:
            ...
            runner.add(stubber.stub_describe_orderable_db_instance_options, self.db_engine, self.db_engine_version, self.instance_opts)
            runner.add(stubber.stub_create_db_instance, self.instance_id, self.db_engine, self.instance_opts[self.instance_choice - 1]['DBInstanceClass'], cluster_id=self.cluster_id)
...
@pytest.mark.parametrize('error, stop_on_index', [('TESTERROR-stub_describe_db_instances', 0), ('TESTERROR-stub_describe_orderable_db_instances', 1), ('TESTERROR-stub_create_db_instance', 2), ('TESTERROR-stub_describe_db_instances', 3)])
