...
from hypothesis.strategies._internal.core import binary, lists, permutations, sampled_from
    ...
    def _hypothesis_do_random(self, method, kwargs):
        if False:
            while True:
                i = 10
        raise NotImplementedError
RANDOM_METHODS = [name for name in ['_randbelow', 'betavariate', 'binomialvariate', 'choice', 'choices', 'expovariate', 'gammavariate', 'gauss', 'getrandbits', 'lognormvariate', 'normalvariate', 'paretovariate', 'randint', 'random', 'randrange', 'sample', 'shuffle', 'triangular', 'uniform', 'vonmisesvariate', 'weibullvariate', 'randbytes'] if hasattr(Random, name)]
