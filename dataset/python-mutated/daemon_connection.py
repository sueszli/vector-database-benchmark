...
class AsyncConnection:
    ...
    async def _send_request_no_logging(self, request: str) -> str:
    ...
    async def create(socket_path: Path) -> AsyncIterator[AsyncConnection]:
        async with connections.connect_async(socket_path) as (reader, writer):
            yield AsyncConnection(reader, writer)

async def send_async_raw_request(socket_path: Path, request: str) -> str:
    async with AsyncConnection.create(socket_path) as connection:
        return await connection.send_request(request)

async def attempt_send_async_raw_request(socket_path: Path, request: str) -> Union[DaemonConnectionFailure, str]:
    try:
        return await send_async_raw_request(socket_path, request)
    except (connections.ConnectionFailure, asyncio.IncompleteReadError, ConnectionError) as error:
        return DaemonConnectionFailure(f'Could not establish connection with an existing Pyre server at {socket_path}: {error}. Type: {type(error)}. Stacktrace: {traceback.format_exc(limit=None, chain=True)}')