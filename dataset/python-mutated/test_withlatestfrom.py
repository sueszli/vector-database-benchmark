...
class TestWithLatestFrom(unittest.TestCase):
    ...
    def test_with_latest_from_never_never(self):
        ...
        def create():
            if False:
                for i in range(10):
                    print('nop')
            return e1.pipe(ops.with_latest_from(e2), ops.map(sum))
    ...
    def test_with_latest_from_empty_never(self):
        ...
        def create():
            if False:
                print('Hello World!')
            return e2.pipe(ops.with_latest_from(e1), ops.map(sum))
        results = scheduler.start(create)
    ...
    def test_with_latest_from_mapper_throws(self):
        ...
        def create():
            if False:
                return 10
            return e1.pipe(ops.with_latest_from(e2), ops.map(lambda xy: _raise(ex)))
        results = scheduler.start(create)
