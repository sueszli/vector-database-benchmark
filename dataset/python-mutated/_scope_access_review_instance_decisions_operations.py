...
def build_list_request(scope: str, schedule_definition_id: str, id: str, *, filter: Optional[str]=None, **kwargs: Any) -> HttpRequest:
    if False:
        return 10
    ...
    _url = kwargs.pop('template_url', '/{scope}/providers/Microsoft.Authorization/accessReviewScheduleDefinitions/{scheduleDefinitionId}/instances/{id}/decisions')
...
class ScopeAccessReviewInstanceDecisionsOperations:
    ...
    @distributed_trace
    def list(self, scope: str, schedule_definition_id: str, id: str, filter: Optional[str]=None, **kwargs: Any) -> Iterable['_models.AccessReviewDecision']:
        if False:
            for i in range(10):
                print('nop')
        ...
        def prepare_request(next_link=None):
            if False:
                ...
                return i + 15
            if not next_link:
                request = build_list_request(scope=scope, schedule_definition_id=schedule_definition_id, id=id, filter=filter, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
    ...
    list.metadata = {'url': '/{scope}/providers/Microsoft.Authorization/accessReviewScheduleDefinitions/{scheduleDefinitionId}/instances/{id}/decisions'}