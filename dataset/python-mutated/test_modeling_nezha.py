...
if is_torch_available():
    import torch
    from transformers import MODEL_FOR_PRETRAINING_MAPPING, NezhaForMaskedLM, NezhaForMultipleChoice, NezhaForNextSentencePrediction, NezhaForPreTraining, NezhaForQuestionAnswering, NezhaForSequenceClassification, NezhaForTokenClassification, NezhaModel
...
class NezhaModelTester:
    ...
    def get_config(self):
        ...
        return NezhaConfig(vocab_size=self.vocab_size, hidden_size=self.hidden_size, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, hidden_act=self.hidden_act, hidden_dropout_prob=self.hidden_dropout_prob, attention_probs_dropout_prob=self.attention_probs_dropout_prob, max_position_embeddings=self.max_position_embeddings, type_vocab_size=self.type_vocab_size, is_decoder=False, initializer_range=self.initializer_range)
...
class NezhaModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (NezhaModel, NezhaForMaskedLM, NezhaForMultipleChoice, NezhaForNextSentencePrediction, NezhaForPreTraining, NezhaForQuestionAnswering, NezhaForSequenceClassification, NezhaForTokenClassification) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': NezhaModel, 'fill-mask': NezhaForMaskedLM, 'question-answering': NezhaForQuestionAnswering, 'text-classification': NezhaForSequenceClassification, 'token-classification': NezhaForTokenClassification, 'zero-shot': NezhaForSequenceClassification} if is_torch_available() else {}
    fx_compatible = True
...
class NezhaModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_nezha_model(self):
        ...
        expected_slice = torch.tensor([[[0.0685, 0.2441, 0.1102], [0.06, 0.1906, 0.1349], [0.0221, 0.0819, 0.0586]]])
    ...
    def test_inference_nezha_masked_lm(self):
        ...
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[-2.7939, -1.7902, -2.2189], [-2.8585, -1.8908, -2.3723], [-2.6499, -1.775, -2.2558]])
        self.assertTrue(torch.allclose(output[:, 1:4, 1:4], expected_slice, atol=0.0001))