...
class OptionListApp(App[None]):
    ...
    def compose(self) -> ComposeResult:
        if False:
            print('Hello World!')
        yield OptionList('0', Option('1'), Separator(), Option('2', disabled=True), None, Option('3', id='3'), Option('4', id='4', disabled=True))
...
async def test_add_later() -> None:
    ...
    async with OptionListApp().run_test() as pilot:
        ...
        option_list.add_options([Option('more still'), 'Yet more options', 'so many options!'])
...
async def test_adding_multiple_duplicates_at_once() -> None:
    ...
    async with OptionListApp().run_test() as pilot:
        ...
        assert option_list.option_count == 5
        with pytest.raises(DuplicateID):
            option_list.add_options([Option('dupe', id='42'), Option('dupe', id='42')])
        assert option_list.option_count == 5