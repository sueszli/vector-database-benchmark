...
if __name__ == '__main__':
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                clipped_noise = (torch.randn_like(data.actions, device=device) * args.policy_noise).clamp(-args.noise_clip, args.noise_clip) * target_actor.action_scale
    ...
    if args.save_model:
        ...
        episodic_returns = evaluate(model_path, make_env, args.env_id, eval_episodes=10, run_name=f'{run_name}-eval', Model=(Actor, QNetwork), device=device, exploration_noise=args.exploration_noise)
