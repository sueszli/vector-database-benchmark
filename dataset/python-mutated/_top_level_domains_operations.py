...
class TopLevelDomainsOperations:
    ...
    async def get(self, name: str, **kwargs: Any) -> _models.TopLevelDomain:
        ...
        request = build_get_request(name=name, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=_stream, **kwargs)
    ...
    @overload
    def list_agreements(self, name: str, agreement_option: _models.TopLevelDomainAgreementOption, *, content_type: str='application/json', **kwargs: Any) -> AsyncIterable['_models.TldLegalAgreement']:
        if False:
            while True:
                i = 10
    ...
    @overload
    def list_agreements(self, name: str, agreement_option: IO, *, content_type: str='application/json', **kwargs: Any) -> AsyncIterable['_models.TldLegalAgreement']:
        if False:
            print('Hello World!')
        'Gets all legal agreements that user needs to accept before purchasing a domain.\n\n        Gets all legal agreements that user needs to accept before purchasing a domain.\n\n        :param name: Name of the top-level domain. Required.\n        :type name: str\n        :param agreement_option: Domain agreement options. Required.\n        :type agreement_option: IO\n        :keyword content_type: Body Parameter content-type. Content type parameter for binary body.\n         Default value is "application/json".\n        :paramtype content_type: str\n        :keyword callable cls: A custom type or function that will be passed the direct response\n        :return: An iterator like instance of either TldLegalAgreement or the result of cls(response)\n        :rtype:\n         ~azure.core.async_paging.AsyncItemPaged[~azure.mgmt.web.v2021_01_15.models.TldLegalAgreement]\n        :raises ~azure.core.exceptions.HttpResponseError:\n        '

    @distributed_trace
    def list_agreements(self, name: str, agreement_option: Union[_models.TopLevelDomainAgreementOption, IO], **kwargs: Any) -> AsyncIterable['_models.TldLegalAgreement']:
        if False:
            while True:
                i = 10
        ...
        def prepare_request(next_link=None):
            if False:
                ...
                return i + 15
            if not next_link:
                request = build_list_agreements_request(name=name, subscription_id=self._config.subscription_id, api_version=api_version, content_type=content_type, json=_json, content=_content, template_url=self.list_agreements.metadata['url'], headers=_headers, params=_params)
                request = _convert_request(request)
                request.url = self._client.format_url(request.url)
            else:
                _parsed_next_link = urllib.parse.urlparse(next_link)
