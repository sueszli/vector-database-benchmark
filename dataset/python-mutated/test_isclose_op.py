...
class TestIscloseOp(OpTest):
    ...
    def setUp(self):
        ...
        self.outputs = {'Out': np.isclose(self.inputs['Input'], self.inputs['Other'], rtol=self.rtol, atol=self.atol, equal_nan=self.equal_nan)}
    ...
    def test_api_case(self):
        ...
        for place in places:
            ...
            with paddle.static.program_guard(main, startup):
                ...
                fetches = exe.run(main, feed={'x': x_data, 'y': y_data}, fetch_list=[result])
...
class TestIscloseOpCp64(unittest.TestCase):
    ...
    def test_cp64(self):
        if False:
            return 10
        x_data = (np.random.rand(10, 10) + 1j * np.random.rand(10, 10)).astype(np.complex64)
        y_data = (np.random.rand(10, 10) + 1j * np.random.rand(10, 10)).astype(np.complex64)
        main = paddle.static.Program()
        startup = paddle.static.Program()
        with paddle.static.program_guard(main, startup):
...
class TestIscloseOpCp128(unittest.TestCase):
    ...
    def test_cp128(self):
        if False:
            for i in range(10):
                print('nop')
        x_data = (np.random.rand(10, 10) + 1j * np.random.rand(10, 10)).astype(np.complex128)
        y_data = (np.random.rand(10, 10) + 1j * np.random.rand(10, 10)).astype(np.complex128)
        main = paddle.static.Program()
        startup = paddle.static.Program()
        with paddle.static.program_guard(main, startup):
            x = paddle.static.data(shape=[10, 10], name='x', dtype=np.complex128)
            y = paddle.static.data(shape=[10, 10], name='y', dtype=np.complex128)
            out = paddle.isclose(x, y, rtol=1e-05, atol=1e-08)
            if core.is_compiled_with_cuda():
                place = paddle.CUDAPlace(0)
                exe = paddle.static.Executor(place)
...
class TestIscloseOpDoubleTol(TestIscloseOp):
    ...
    def set_args(self):
        if False:
            i = 10
            return i + 15
        self.input = np.array([1.0, 1e-09]).astype('float64')
