...
class GlobalCacheWrapper:
    ...
    def __init__(self, bot: Red, config: Config, session: aiohttp.ClientSession, cog: Union['Audio', Cog]):
        if False:
            i = 10
            return i + 15
        self.bot = bot
    ...
    async def get_call(self, query: Optional[Query]=None) -> dict:
        ...
        try:
            ...
            with contextlib.suppress(aiohttp.ContentTypeError, asyncio.TimeoutError):
                async with self.session.get(api_url, timeout=aiohttp.ClientTimeout(total=await self.config.global_db_get_timeout()), headers={'Authorization': self.api_key, 'X-Token': self._handshake_token}, params={'query': query}) as r:
                    search_response = await r.json(loads=json.loads)
                    log.trace('GET || Ping %s || Status code %s || %s', r.headers.get('x-process-time'), r.status, query)
    ...
    async def get_spotify(self, title: str, author: Optional[str]) -> dict:
        ...
        try:
            ...
            if self.api_key is None:
                return {}
            with contextlib.suppress(aiohttp.ContentTypeError, asyncio.TimeoutError):
                async with self.session.get(api_url, timeout=aiohttp.ClientTimeout(total=await self.config.global_db_get_timeout()), headers={'Authorization': self.api_key, 'X-Token': self._handshake_token}, params=params) as r:
                    search_response = await r.json(loads=json.loads)
                    log.trace('GET/spotify || Ping %s || Status code %s || %s - %s', r.headers.get('x-process-time'), r.status, title, author)
            if 'tracks' not in search_response:
                return {}
            return search_response
        except Exception as exc:
    ...
    async def post_call(self, llresponse: LoadResult, query: Optional[Query]) -> None:
        try:
            ...
            if self.api_key is None:
                return None
            api_url = f'{_API_URL}api/v2/queries'
            async with self.session.post(api_url, json=llresponse._raw, headers={'Authorization': self.api_key, 'X-Token': self._handshake_token}, params={'query': query}) as r:
                await r.read()
                log.trace('GET || Ping %s || Status code %s || %s', r.headers.get('x-process-time'), r.status, query)
        except Exception as exc:
    ...
    async def report_invalid(self, id: str) -> None:
        ...
        with contextlib.suppress(Exception):
            async with self.session.delete(api_url, headers={'Authorization': self.api_key, 'X-Token': self._handshake_token}, params={'id': id}) as r:
                await r.read()
    ...
    async def get_perms(self):
        ...
        with contextlib.suppress(Exception):
            async with aiohttp.ClientSession(json_serialize=json.dumps) as session:
                async with session.get(f'{_API_URL}api/v2/users/me', headers={'Authorization': self.api_key, 'X-Token': self._handshake_token}) as resp:
                    if resp.status == 200:
