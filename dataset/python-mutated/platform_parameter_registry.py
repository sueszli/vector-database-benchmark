...
(config_models, suggestion_models) = models.Registry.import_models([models.Names.CONFIG, models.Names.SUGGESTION])
ParamNames = platform_parameter_list.ParamNames

class Registry:
    """Registry of all platform parameters."""
    DEFAULT_VALUE_BY_TYPE_DICT: Dict[platform_parameter_domain.DataTypes, Union[bool, str, int, float]] = {platform_parameter_domain.DataTypes.BOOL: False, platform_parameter_domain.DataTypes.NUMBER: 0, platform_parameter_domain.DataTypes.STRING: ''}
...
Registry.create_feature_flag(ParamNames.DUMMY_FEATURE_FLAG_FOR_E2E_TESTS, 'This is a dummy feature flag for the e2e tests.', platform_parameter_domain.FeatureStages.PROD)
Registry.create_platform_parameter(ParamNames.DUMMY_PARAMETER, 'This is a dummy platform parameter.', platform_parameter_domain.DataTypes.STRING)
Registry.create_feature_flag(ParamNames.END_CHAPTER_CELEBRATION, 'This flag is for the end chapter celebration feature.', platform_parameter_domain.FeatureStages.PROD)
Registry.create_feature_flag(ParamNames.CHECKPOINT_CELEBRATION, 'This flag is for the checkpoint celebration feature.', platform_parameter_domain.FeatureStages.PROD)
Registry.create_feature_flag(ParamNames.CONTRIBUTOR_DASHBOARD_ACCOMPLISHMENTS, 'This flag enables showing per-contributor accomplishments on the' + ' contributor dashboard.', platform_parameter_domain.FeatureStages.PROD)
Registry.create_feature_flag(ParamNames.ANDROID_BETA_LANDING_PAGE, 'This flag is for Android beta promo landing page.', platform_parameter_domain.FeatureStages.PROD)
Registry.create_feature_flag(ParamNames.BLOG_PAGES, 'This flag is for blog home page, blog author profile page and blog post' + ' page.', platform_parameter_domain.FeatureStages.PROD)
Registry.create_feature_flag(ParamNames.DIAGNOSTIC_TEST, 'This flag is for the diagnostic test functionality.', platform_parameter_domain.FeatureStages.PROD)
Registry.create_feature_flag(ParamNames.SERIAL_CHAPTER_LAUNCH_CURRICULUM_ADMIN_VIEW, 'This flag is for serial chapter launch feature and making changes only' + 'in the curriculum admin view.', platform_parameter_domain.FeatureStages.TEST)
Registry.create_feature_flag(ParamNames.SERIAL_CHAPTER_LAUNCH_LEARNER_VIEW, 'This flag is for serial chapter launch feature and making changes only' + 'in the learner view.', platform_parameter_domain.FeatureStages.TEST)
Registry.create_feature_flag(ParamNames.SHOW_REDESIGNED_LEARNER_DASHBOARD, 'This flag is to show redesigned learner dashboard.', platform_parameter_domain.FeatureStages.DEV)
Registry.create_feature_flag(ParamNames.SHOW_TRANSLATION_SIZE, 'This flag is to show translation size on translation cards in' + 'contributor dashboard.', platform_parameter_domain.FeatureStages.DEV)
Registry.create_feature_flag(ParamNames.SHOW_FEEDBACK_UPDATES_IN_PROFILE_PIC_DROPDOWN, 'This flag is to show feedback updates in the' + 'profile pic drop-down menu.', platform_parameter_domain.FeatureStages.DEV)
Registry.create_feature_flag(ParamNames.CD_ADMIN_DASHBOARD_NEW_UI, 'This flag is to show new contributor admin dashboard.', platform_parameter_domain.FeatureStages.TEST)
Registry.create_feature_flag(ParamNames.IS_IMPROVEMENTS_TAB_ENABLED, 'Exposes the Improvements Tab for creators in the exploration editor.', platform_parameter_domain.FeatureStages.PROD)
Registry.create_feature_flag(ParamNames.LEARNER_GROUPS_ARE_ENABLED, 'Enable learner groups feature', platform_parameter_domain.FeatureStages.PROD)
Registry.create_platform_parameter(ParamNames.PROMO_BAR_ENABLED, 'Whether the promo bar should be enabled for all users', platform_parameter_domain.DataTypes.BOOL)
Registry.create_platform_parameter(ParamNames.PROMO_BAR_MESSAGE, 'The message to show to all users if the promo bar is enabled', platform_parameter_domain.DataTypes.STRING)
Registry.create_platform_parameter(ParamNames.ALWAYS_ASK_LEARNERS_FOR_ANSWER_DETAILS, 'Always ask learners for answer details. For testing -- do not use', platform_parameter_domain.DataTypes.BOOL)
Registry.create_platform_parameter(ParamNames.MAX_NUMBER_OF_TAGS_ASSIGNED_TO_BLOG_POST, 'The maximum number of tags that can be selected to categorize the blog post', platform_parameter_domain.DataTypes.NUMBER, default=10)
Registry.create_platform_parameter(ParamNames.HIGH_BOUNCE_RATE_TASK_STATE_BOUNCE_RATE_CREATION_THRESHOLD, 'The bounce-rate a state must exceed to create a new improvements task.', platform_parameter_domain.DataTypes.NUMBER, default=0.2)
Registry.create_platform_parameter(ParamNames.HIGH_BOUNCE_RATE_TASK_STATE_BOUNCE_RATE_OBSOLETION_THRESHOLD, 'The bounce-rate a state must fall under to discard its improvement task.', platform_parameter_domain.DataTypes.NUMBER, default=0.2)
Registry.create_platform_parameter(ParamNames.HIGH_BOUNCE_RATE_TASK_MINIMUM_EXPLORATION_STARTS, 'The minimum number of times an exploration is started before it can generate high bounce-rate improvements tasks.', platform_parameter_domain.DataTypes.NUMBER, default=100)
Registry.create_platform_parameter(ParamNames.CONTRIBUTOR_DASHBOARD_REVIEWER_EMAILS_IS_ENABLED, 'Enable sending Contributor Dashboard reviewers email notifications about suggestions that need review. The default value is false.', platform_parameter_domain.DataTypes.BOOL)
Registry.create_platform_parameter(ParamNames.ENABLE_ADMIN_NOTIFICATIONS_FOR_SUGGESTIONS_NEEDING_REVIEW, 'Enable sending admins email notifications if there are Contributor Dashboard suggestions that have been waiting for a review for more than %s days. The default value is false.' % suggestion_models.SUGGESTION_REVIEW_WAIT_TIME_THRESHOLD_IN_DAYS, platform_parameter_domain.DataTypes.BOOL)
Registry.create_platform_parameter(ParamNames.ENABLE_ADMIN_NOTIFICATIONS_FOR_REVIEWER_SHORTAGE, 'Enable sending admins email notifications if Contributor Dashboard reviewers are needed in specific suggestion types. The default value is false.', platform_parameter_domain.DataTypes.BOOL)
Registry.create_platform_parameter(ParamNames.MAX_NUMBER_OF_SUGGESTIONS_PER_REVIEWER, 'The maximum number of Contributor Dashboard suggestions per reviewer.If the number of suggestions per reviewer surpasses this maximum, for any given suggestion type on the dashboard, the admins are notified by email.', platform_parameter_domain.DataTypes.NUMBER, default=5)
Registry.create_platform_parameter(ParamNames.RECORD_PLAYTHROUGH_PROBABILITY, 'The probability of recording playthroughs', platform_parameter_domain.DataTypes.NUMBER, default=0.2)