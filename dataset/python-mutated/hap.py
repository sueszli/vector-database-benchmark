...
class HomematicipAuth:
    ...
    async def async_setup(self) -> bool:
        ...
        try:
            self.auth = await self.get_auth(self.hass, self.config.get(HMIPC_HAPID), self.config.get(HMIPC_PIN))
...
class HomematicipHAP:
    ...
    async def async_setup(self, tries: int=0) -> bool:
        ...
        try:
            self.home = await self.get_hap(self.hass, self.config_entry.data.get(HMIPC_HAPID), self.config_entry.data.get(HMIPC_AUTHTOKEN), self.config_entry.data.get(HMIPC_NAME))
        except HmipcConnectionError as err:
            raise ConfigEntryNotReady from err
        except Exception as err:
            _LOGGER.error('Error connecting with HomematicIP Cloud: %s', err)
            return False
        _LOGGER.info('Connected to HomematicIP with HAP %s', self.config_entry.unique_id)
    ...
    async def async_connect(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                _LOGGER.error('Error connecting to HomematicIP with HAP %s. Retrying in %d seconds', self.config_entry.unique_id, retry_delay)
            if self._ws_close_requested:
                break
            self._ws_close_requested = False
            tries += 1
            try:
                self._retry_task = self.hass.async_create_task(asyncio.sleep(retry_delay))
    ...
    async def async_reset(self) -> bool:
        ...
        await self.hass.config_entries.async_unload_platforms(self.config_entry, PLATFORMS)
    ...
    def shutdown(self, event) -> None:
        ...
        _LOGGER.debug('Reset connection to access point id %s', self.config_entry.unique_id)

    async def get_hap(self, hass: HomeAssistant, hapid: str | None, authtoken: str | None, name: str | None) -> AsyncHome:
        """Create a HomematicIP access point object."""
