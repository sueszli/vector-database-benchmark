...
class FileuploadNet(SimpleDownloader):
    ...
    __version__ = '0.08'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?file-upload\\.net/(?:en/)?download-\\d+/.+'
    ...
    NAME_PATTERN = '<title>File-Upload.net - (?P<N>.+?)<'
    SIZE_PATTERN = '</label><span>(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)'
    OFFLINE_PATTERN = 'Datei existiert nicht'
    LINK_FREE_PATTERN = "<a href='(.+?)' title='download' onclick"
