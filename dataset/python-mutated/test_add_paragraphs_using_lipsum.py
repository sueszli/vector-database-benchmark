...
class TestAddParagraphsUsingLipsum(TestCase):
    ...
    def test_add_paragraphs_in_lipsum_style(self):
        ...
        layout.add(self.get_test_header(test_description='This test creates a PDF with dummy text in it.'))
        random.seed(2048)
        for s in [Lipsum.generate_lipsum_text(random.choice([5, 6, 7])) for _ in range(0, 5)]:
    ...
    def test_add_paragraphs_in_agatha_christie_style(self):
        ...
        layout.add(self.get_test_header(test_description='This test creates a PDF with dummy text in it.'))
        random.seed(2048)
        for s in [Lipsum.generate_agatha_christie_text(random.choice([5, 6, 7])) for _ in range(0, 5)]:
            layout.add(Paragraph(s))
    ...
    def test_add_paragraphs_in_aa_milne_style(self):
        ...
        layout.add(self.get_test_header(test_description='This test creates a PDF with dummy text in it.'))
        random.seed(2048)
        for s in [Lipsum.generate_alan_alexander_milne_text(random.choice([5, 6, 7])) for _ in range(0, 5)]:
            layout.add(Paragraph(s))
    ...
    def test_add_paragraphs_in_emily_bronte_style(self):
        ...
        layout.add(self.get_test_header(test_description='This test creates a PDF with dummy text in it.'))
        random.seed(2048)
        for s in [Lipsum.generate_emily_bronte_text(random.choice([5, 6, 7])) for _ in range(0, 5)]:
            layout.add(Paragraph(s))
