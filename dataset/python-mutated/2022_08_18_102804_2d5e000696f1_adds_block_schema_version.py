...
def upgrade():
    if False:
        i = 10
        return i + 15
    op.add_column('block_schema', sa.Column('version', sa.String(), server_default='non-versioned', nullable=False))
    op.drop_index('uq_block_schema__checksum', table_name='block_schema')
    op.create_index('uq_block_schema__checksum_version', 'block_schema', ['checksum', 'version'], unique=True)
...
def downgrade():
    ...
    op.create_index('uq_block_schema__checksum', 'block_schema', ['checksum'], unique=False)
