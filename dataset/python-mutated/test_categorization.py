...
_DELETE_KEYWORDS = [x for x in os.environ.get('SENTRY_TEST_GROUPING_DELETE_KEYWORDS', '').lower().split(',') if x]
...
def get_stacktrace_render(data):
    ...
    return '\n'.join(rv)
INPUTS = [CategorizationInput(fname) for fname in os.listdir(_fixture_path) if fname.endswith('.json')]
