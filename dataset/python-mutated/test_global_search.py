...
class TestGlobalSearch(FrappeTestCase):
    ...
    def insert_test_events(self):
        ...
        for text in phrases:
            frappe.get_doc(dict(doctype='Event', subject=text, repeat_on='Monthly', starts_on=now_datetime())).insert()
    ...
    def test_insert_child_table(self):
        if False:
            print('Hello World!')
        frappe.db.delete('Event')
        phrases = ['Hydrus is a small constellation in the deep southern sky. ', 'It was first depicted on a celestial atlas by Johann Bayer in his 1603 Uranometria. ', 'The French explorer and astronomer Nicolas Louis de Lacaille charted the brighter stars and gave their Bayer designations in 1756. ', 'Its name means "male water snake", as opposed to Hydra, a much larger constellation that represents a female water snake. ', 'It remains below the horizon for most Northern Hemisphere observers.', 'The brightest star is the 2.8-magnitude Beta Hydri, also the closest reasonably bright star to the south celestial pole. ', 'Pulsating between magnitude 3.26 and 3.33, Gamma Hydri is a variable red giant some 60 times the diameter of our Sun. ', 'Lying near it is VW Hydri, one of the brightest dwarf novae in the heavens. ', 'Four star systems have been found to have exoplanets to date, most notably HD 10180, which could bear up to nine planetary companions.']
        for text in phrases:
    ...
    def test_web_page_index(self):
        ...
        results = global_search.web_search(text='unsubscribe', scope='manufacturing" UNION ALL SELECT 1,2,3,4,doctype from __global_search')
