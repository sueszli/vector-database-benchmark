...
@pytest.fixture(params=[*list(all_implementations()), default_implementation], ids=lambda imp: imp.__name__, scope='module')
async def display(page, request):
    ...
    async with BackendFixture(implementation=imp, options=opts) as server:
        async with DisplayFixture(backend=server, driver=page, url_prefix=url_prefix) as display:
            yield display
...
async def test_use_location(display: DisplayFixture):
    ...
    for loc in [Location('/something', ''), Location('/something/file.txt', ''), Location('/another/something', ''), Location('/another/something/file.txt', ''), Location('/another/something/file.txt', '?key=value'), Location('/another/something/file.txt', '?key1=value1&key2=value2')]:
...
async def test_customized_head(imp: BackendType, page):
    ...
    async with BackendFixture(implementation=imp, options=imp.Options(head=html.title(custom_title))) as server:
