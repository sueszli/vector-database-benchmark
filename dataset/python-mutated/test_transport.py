...
from elasticsearch.exceptions import ApiError, ConnectionError, ElasticsearchWarning, UnsupportedProductError
...
class DummyNode(BaseAsyncNode):
    ...
    async def perform_request(self, *args, **kwargs):
        ...
        return NodeApiResponse(ApiResponseMeta(status=self.resp_status, headers=HttpHeaders(self.resp_headers), http_version='1.1', duration=0.0, node=self.config), self.resp_data)
...
class TestTransport:
    ...
    async def test_request_timeout_extracted_from_params_and_passed(self):
        client = AsyncElasticsearch('http://localhost:9200', meta_header=False, node_class=DummyNode)
    ...
    async def test_opaque_id(self):
        client = AsyncElasticsearch('http://localhost:9200', meta_header=False, node_class=DummyNode, opaque_id='app-1')
        await client.info()
        calls = client.transport.node_pool.get().calls
        assert 1 == len(calls)
        assert calls[0][0] == ('GET', '/')
    ...
    async def test_client_meta_header(self):
        ...
        assert re.search('^es=[0-9.]+p?,py=[0-9.]+p?,t=[0-9.]+p?$', headers['x-elastic-client-meta'])

        class DummyNodeWithMeta(DummyNode):
            _CLIENT_META_HTTP_CLIENT = ('dm', '1.2.3')
        client = AsyncElasticsearch('http://localhost:9200', node_class=DummyNodeWithMeta)
        await client.info(headers={'CustoM': 'header'})
        calls = client.transport.node_pool.get().calls
        assert 1 == len(calls)
        headers = calls[0][1]['headers']
        assert re.search('^es=[0-9.]+p?,py=[0-9.]+p?,t=[0-9.]+p?,dm=1.2.3$', headers['x-elastic-client-meta'])
    ...
    async def test_client_meta_header_not_sent(self):
        ...
        calls = client.transport.node_pool.get().calls
        assert 1 == len(calls)
        assert calls[0][1]['headers'] == {'accept': 'application/vnd.elasticsearch+json; compatible-with=8'}
    ...
    async def test_body_surrogates_replaced_encoded_into_bytes(self):
        ...
        calls = client.transport.node_pool.get().calls
        assert 1 == len(calls)
        assert calls[0][1]['body'] == b'{"query":{"match":"\xe4\xbd\xa0\xe5\xa5\xbd\xed\xa9\xaa"}}'
    ...
    async def test_request_will_fail_after_x_retries(self):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'exception': ConnectionError('abandon ship!')})], node_class=DummyNode)
    ...
    async def test_failed_connection_will_be_marked_as_dead(self):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'exception': ConnectionError('abandon ship!')}), NodeConfig('http', 'localhost', 9201, _extras={'exception': ConnectionError('abandon ship!')})], node_class=DummyNode)
        with pytest.raises(ConnectionError):
            await client.info()
    ...
    async def test_resurrected_connection_will_be_marked_as_live_on_success(self):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200), NodeConfig('http', 'localhost', 9201)], node_class=DummyNode)
    ...
    @pytest.mark.parametrize(['nodes_info_response', 'node_host'], [(CLUSTER_NODES, '1.1.1.1'), (CLUSTER_NODES_7x_PUBLISH_HOST, 'somehost.tld')])
    async def test_sniff_will_use_seed_connections(self, nodes_info_response, node_host):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'data': nodes_info_response})], node_class=DummyNode, sniff_on_start=True)
    ...
    async def test_sniff_on_start_ignores_sniff_timeout(self):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'data': CLUSTER_NODES})], node_class=DummyNode, sniff_on_start=True, sniff_timeout=12, meta_header=False)
        await client.transport._async_call()
        assert client.transport._sniffing_task is not None
        await client.transport._sniffing_task
        calls = client.transport.node_pool.all()[0].calls
        assert len(calls) == 1
        assert calls[0] == (('GET', '/_nodes/_all/http'), {'body': None, 'headers': {'accept': 'application/vnd.elasticsearch+json; compatible-with=8'}, 'request_timeout': None})

    async def test_sniff_uses_sniff_timeout(self):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'data': CLUSTER_NODES})], node_class=DummyNode, sniff_before_requests=True, sniff_timeout=12, meta_header=False)
        await client.info()
        assert client.transport._sniffing_task is not None
        await client.transport._sniffing_task
        calls = client.transport.node_pool.all()[0].calls
        assert len(calls) == 2
        assert calls[0] == (('GET', '/'), {'body': None, 'headers': {'accept': 'application/vnd.elasticsearch+json; compatible-with=8'}, 'request_timeout': DEFAULT})
        assert calls[1] == (('GET', '/_nodes/_all/http'), {'body': None, 'headers': {'accept': 'application/vnd.elasticsearch+json; compatible-with=8'}, 'request_timeout': 12})
    ...
    async def test_sniff_reuses_node_instances(self):
        client = AsyncElasticsearch([NodeConfig('http', '1.1.1.1', 123, _extras={'data': CLUSTER_NODES})], node_class=DummyNode, sniff_on_start=True)
        assert len(client.transport.node_pool) == 1
        await client.info()
        assert len(client.transport.node_pool) == 1

    @pytest.mark.parametrize(['extra_key', 'extra_value'], [('exception', ConnectionError('Abandon ship!')), ('status', 500)])
    async def test_sniff_on_node_failure_triggers(self, extra_key, extra_value):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={extra_key: extra_value}), NodeConfig('http', 'localhost', 9201, _extras={'data': CLUSTER_NODES})], node_class=DummyNode, sniff_on_node_failure=True, randomize_nodes_in_pool=False, max_retries=0)
    ...
    async def test_sniff_after_n_seconds(self, event_loop):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'data': CLUSTER_NODES})], node_class=DummyNode, min_delay_between_sniffing=5)
        ...
        assert 'http://1.1.1.1:123' in (node.base_url for node in client.transport.node_pool.all())
        assert event_loop.time() - 1 < client.transport._last_sniffed_at < event_loop.time() + 0.01

    @pytest.mark.parametrize('kwargs', [{'sniff_on_start': True}, {'sniff_on_connection_fail': True}, {'sniff_on_node_failure': True}, {'sniff_before_requests': True}, {'sniffer_timeout': 1}, {'sniff_timeout': 1}])
    async def test_sniffing_disabled_on_elastic_cloud(self, kwargs):
        with pytest.raises(ValueError) as e:
            AsyncElasticsearch(cloud_id='cluster:dXMtZWFzdC0xLmF3cy5mb3VuZC5pbyQ0ZmE4ODIxZTc1NjM0MDMyYmVkMWNmMjIxMTBlMmY5NyQ0ZmE4ODIxZTc1NjM0MDMyYmVkMWNmMjIxMTBlMmY5Ng==', **kwargs)
        assert str(e.value) == 'Sniffing should not be enabled when connecting to Elastic Cloud'

    async def test_sniff_on_start_close_unlocks_async_calls(self, event_loop):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'delay': 10, 'data': CLUSTER_NODES})], node_class=DummyNode, sniff_on_start=True)
        tasks = []
    ...
    async def test_sniffing_master_only_filtered_by_default(self):
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'data': CLUSTER_NODES_MASTER_ONLY})], node_class=DummyNode, sniff_on_start=True)
        await client.transport._async_call()
    ...
    async def test_sniff_node_callback(self):
        ...
        client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'data': CLUSTER_NODES_MASTER_ONLY})], node_class=DummyNode, sniff_on_start=True, sniffed_node_callback=sniffed_node_callback)
        await client.transport._async_call()
        assert len(client.transport.node_pool) == 2
    ...
    async def test_sniffing_deprecated_host_info_callback(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'data': CLUSTER_NODES_MASTER_ONLY})], node_class=DummyNode, sniff_on_start=True, host_info_callback=host_info_callback)
            await client.transport._async_call()
        assert len(w) == 1
        assert w[0].category == DeprecationWarning
        assert str(w[0].message) == "The 'host_info_callback' parameter is deprecated in favor of 'sniffed_node_callback'"
        assert len(client.transport.node_pool) == 2
        ports = {node.config.port for node in client.transport.node_pool.all()}
        assert ports == {9200, 124}
...
async def test_unsupported_product_error(headers):
    client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'headers': headers})], meta_header=False, node_class=DummyNode)
    with pytest.raises(UnsupportedProductError) as e:
        await client.info()
    assert str(e.value) == 'The client noticed that the server is not Elasticsearch and we do not support this unknown product'
    calls = client.transport.node_pool.get().calls
    assert len(calls) == 1
    assert calls[0] == (('GET', '/'), {'body': None, 'headers': {'accept': 'application/vnd.elasticsearch+json; compatible-with=8'}, 'request_timeout': DEFAULT})
...
async def test_unsupported_product_error_not_raised_on_non_2xx(status):
    client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'headers': {}, 'status': status})], meta_header=False, node_class=DummyNode)
    try:
        await client.info()
...
async def test_api_error_raised_before_product_error(status):
    client = AsyncElasticsearch([NodeConfig('http', 'localhost', 9200, _extras={'headers': {'X-elastic-product': 'BAD HEADER'}, 'status': status})], meta_header=False, node_class=DummyNode)
...
async def test_warning_header(headers):
    ...
    assert str(w[0].message) == '[xpack.monitoring.history.duration] setting was deprecated in Elasticsearch and will be removed in a future release! See the breaking changes documentation for the next major version.'