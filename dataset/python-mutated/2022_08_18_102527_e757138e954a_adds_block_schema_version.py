...
def upgrade():
    ...
    with op.batch_alter_table('block_schema', schema=None) as batch_op:
        batch_op.add_column(sa.Column('version', sa.String(), server_default='non-versioned', nullable=False))
        batch_op.drop_index('uq_block_schema__checksum')
        batch_op.create_index('uq_block_schema__checksum_version', ['checksum', 'version'], unique=True)
