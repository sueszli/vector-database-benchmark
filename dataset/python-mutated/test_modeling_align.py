...
from transformers.testing_utils import is_flax_available, require_torch, require_vision, slow, torch_device
...
class AlignVisionModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=AlignVisionConfig, has_text_modality=False, hidden_size=37)
...
class AlignModelTester:
    ...
    def get_config(self):
        if False:
            return 10
        return AlignConfig.from_text_vision_configs(self.text_model_tester.get_config(), self.vision_model_tester.get_config(), projection_dim=64)
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        inputs_dict = {'input_ids': input_ids, 'token_type_ids': token_type_ids, 'attention_mask': input_mask, 'pixel_values': pixel_values, 'return_loss': True}
        return (config, inputs_dict)
        ...
        for model_class in self.all_model_classes:
            ...
            for (name, param) in model.named_parameters():
                if param.requires_grad:
                    if name == 'temperature':
                        self.assertAlmostEqual(param.data.item(), 1.0, delta=0.001, msg=f'Parameter {name} of model {model_class} seems not properly initialized')
