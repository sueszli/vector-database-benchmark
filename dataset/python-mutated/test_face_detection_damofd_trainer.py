...
class TestFaceDetectionScrfdTrainerSingleGPU(unittest.TestCase):
    ...
    def test_trainer_from_scratch(self):
        if False:
            while True:
                i = 10
        kwargs = dict(cfg_file=os.path.join(self.cache_path, 'DamoFD_lms.py'), work_dir=self.tmp_dir, train_root=self.train_root, val_root=self.val_root, total_epochs=self.max_epochs, cfg_modify_fn=self._cfg_modify_fn)
        trainer = build_trainer(name=Trainers.face_detection_scrfd, default_args=kwargs)
        trainer.train()
    ...
    def test_trainer_finetune(self):
        ...
        kwargs = dict(cfg_file=os.path.join(self.cache_path, 'DamoFD_lms.py'), work_dir=self.tmp_dir, train_root=self.train_root, val_root=self.val_root, total_epochs=self.max_epochs, resume_from=os.path.join(self.cache_path, ModelFile.TORCH_MODEL_FILE), cfg_modify_fn=self._cfg_modify_fn)
        trainer = build_trainer(name=Trainers.face_detection_scrfd, default_args=kwargs)
        trainer.train()
        results_files = os.listdir(self.tmp_dir)
        self.assertIn(f'{trainer.timestamp}.log.json', results_files)
...
class TestFaceDetectionScrfdTrainerMultiGpus(DistributedTestCase):
    ...
    def test_multi_gpus_finetune(self):
        ...
        pretrain_epoch = 640
        self.max_epochs += pretrain_epoch
        kwargs = dict(cfg_file=os.path.join(self.cache_path, 'DamoFD_lms.py'), work_dir=self.tmp_dir, train_root=self.train_root, val_root=self.val_root, total_epochs=self.max_epochs, resume_from=os.path.join(self.cache_path, ModelFile.TORCH_MODEL_FILE), launcher='pytorch')
