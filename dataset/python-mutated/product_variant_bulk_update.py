...
from .product_variant_bulk_create import BulkAttributeValueInput, ProductVariantBulkCreate, ProductVariantBulkCreateInput, ProductVariantBulkResult, clean_price, get_results

class ProductVariantStocksUpdateInput(BaseInputObjectType):
    create = NonNullList(StockInput, description='List of warehouses to create stocks.', required=False)
    update = NonNullList(StockUpdateInput, description='List of stocks to update.', required=False)
    remove = NonNullList(graphene.ID, description='List of stocks to remove.', required=False)
...
class ChannelListingUpdateInput(BaseInputObjectType):
    ...
    preorder_threshold = graphene.Int(description='The threshold for preorder variant in channel.')

    class Meta:
        doc_category = DOC_CATEGORY_PRODUCTS

class ProductVariantChannelListingUpdateInput(BaseInputObjectType):
    create = NonNullList(ProductVariantChannelListingAddInput, description='List of channels to create variant channel listings.', required=False)
    update = NonNullList(ChannelListingUpdateInput, description='List of channel listings to update.', required=False)
    remove = NonNullList(graphene.ID, description='List of channel listings to remove.', required=False)
...
class ProductVariantBulkUpdateInput(ProductVariantBulkCreateInput):
    ...
    attributes = NonNullList(BulkAttributeValueInput, required=False, description='List of attributes specific to this variant.')
    stocks = graphene.Field(ProductVariantStocksUpdateInput, description='Stocks input.' + ADDED_IN_312 + PREVIEW_FEATURE, required=False)
    channel_listings = graphene.Field(ProductVariantChannelListingUpdateInput, description='Channel listings input.' + ADDED_IN_312 + PREVIEW_FEATURE, required=False)
...
class ProductVariantBulkUpdate(BaseMutation):
    count = graphene.Int(required=True, default_value=0, description='Returns how many objects were updated.')
    results = NonNullList(ProductVariantBulkResult, required=True, default_value=[], description='List of the updated variants.')

    class Arguments:
        variants = NonNullList(ProductVariantBulkUpdateInput, required=True, description='Input list of product variants to update.')
        product_id = graphene.ID(description='ID of the product to update the variants for.', name='product', required=True)
    ...
    class Meta:
        description = 'Update multiple product variants.' + ADDED_IN_311 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_PRODUCTS
