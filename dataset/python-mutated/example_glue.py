...
with DAG(dag_id=DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), tags=['example'], catchup=False) as dag:
    ...
    glue_crawler_config = {'Name': glue_crawler_name, 'Role': role_arn, 'DatabaseName': glue_db_name, 'Targets': {'S3Targets': [{'Path': f'{bucket_name}/input'}]}}
    create_bucket = S3CreateBucketOperator(task_id='create_bucket', bucket_name=bucket_name)
    upload_csv = S3CreateObjectOperator(task_id='upload_csv', s3_bucket=bucket_name, s3_key='input/input.csv', data=EXAMPLE_CSV, replace=True)
    upload_script = S3CreateObjectOperator(task_id='upload_script', s3_bucket=bucket_name, s3_key='etl_script.py', data=EXAMPLE_SCRIPT.format(db_name=glue_db_name, bucket_name=bucket_name), replace=True)
    crawl_s3 = GlueCrawlerOperator(task_id='crawl_s3', config=glue_crawler_config)
    crawl_s3.wait_for_completion = False
    wait_for_crawl = GlueCrawlerSensor(task_id='wait_for_crawl', crawler_name=glue_crawler_name)
    submit_glue_job = GlueJobOperator(task_id='submit_glue_job', job_name=glue_job_name, script_location=f's3://{bucket_name}/etl_script.py', s3_bucket=bucket_name, iam_role_name=role_name, create_job_kwargs={'GlueVersion': '3.0', 'NumberOfWorkers': 2, 'WorkerType': 'G.1X'})
    submit_glue_job.wait_for_completion = False
    wait_for_job = GlueJobSensor(task_id='wait_for_job', job_name=glue_job_name, run_id=submit_glue_job.output, verbose=True)
    ...
    log_cleanup = prune_logs([('/aws-glue/crawlers', glue_crawler_name), ('/aws-glue/jobs/logs-v2', submit_glue_job.output), ('/aws-glue/jobs/error', submit_glue_job.output), ('/aws-glue/jobs/output', submit_glue_job.output)])
    chain(test_context, create_bucket, upload_csv, upload_script, crawl_s3, wait_for_crawl, submit_glue_job, wait_for_job, glue_cleanup(glue_crawler_name, glue_job_name, glue_db_name), delete_bucket, log_cleanup)
