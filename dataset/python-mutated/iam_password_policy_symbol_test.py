...
class Test_iam_password_policy_symbol:
    ...
    def test_iam_password_policy_no_symbol_flag(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_password_policy_symbol.iam_password_policy_symbol.iam_client', new=IAM(current_audit_info)):
            from prowler.providers.aws.services.iam.iam_password_policy_symbol.iam_password_policy_symbol import iam_password_policy_symbol
            check = iam_password_policy_symbol()
            result = check.execute()
            assert len(result) == 1
            assert result[0].status == 'FAIL'
            assert search('IAM password policy does not require at least one symbol.', result[0].status_extended)
    ...
    def test_iam_password_policy_symbol_flag(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_password_policy_symbol.iam_password_policy_symbol.iam_client', new=IAM(current_audit_info)):
            ...
            assert search('IAM password policy requires at least one symbol.', result[0].status_extended)
            assert result[0].resource_id == AWS_ACCOUNT_NUMBER
            assert result[0].resource_arn == AWS_ACCOUNT_ARN
            assert result[0].region == AWS_REGION