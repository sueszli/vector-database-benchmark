...
class TaskFail(Base):
    ...
    __table_args__ = (Index('idx_task_fail_task_instance', dag_id, task_id, run_id, map_index), ForeignKeyConstraint([dag_id, task_id, run_id, map_index], ['task_instance.dag_id', 'task_instance.task_id', 'task_instance.run_id', 'task_instance.map_index'], name='task_fail_ti_fkey', ondelete='CASCADE'))
    dag_run = relationship('DagRun', primaryjoin='and_(\n            TaskFail.dag_id == foreign(DagRun.dag_id),\n            TaskFail.run_id == foreign(DagRun.run_id),\n        )', viewonly=True)
