...
from torch.testing._internal.common_utils import TestCase, to_gpu, freeze_rng_state, is_iterable, gradcheck, gradgradcheck, set_default_dtype
...
for (padding_mode, cpp_padding_mode) in zip(['reflect', 'circular', 'replicate', 'zeros'], ['torch::kReflect', 'torch::kCircular', 'torch::kReplicate', 'torch::kZeros']):
    for d in (1, 2, 3):
        ...
        new_module_tests.append(dict(module_name=f'Conv{d}d', constructor_args=(2, 3, 3, 2, padding, 1, 1, True, padding_mode), cpp_constructor_args=f'torch::nn::Conv{d}dOptions(2, 3, 3)\n                                        .stride(2)\n                                        .padding({cpp_padding})\n                                        .dilation(1)\n                                        .groups(1)\n                                        .bias(true)\n                                        .padding_mode({cpp_padding_mode})', input_size=input_size, output_size=output_size, cudnn=True, desc=f'{padding_mode}_stride2_pad2', with_tf32=True, tf32_precision=0.05, default_dtype=torch.double))
non_linear_activations_no_batch = ['ELU', 'Hardshrink', 'Hardsigmoid', 'Hardtanh', 'Hardswish', 'LeakyReLU', 'LogSigmoid', 'PReLU', 'ReLU', 'ReLU6', 'RReLU', 'SELU', 'CELU', 'GELU', 'GLU', 'Sigmoid', 'SiLU', 'Mish', 'Softplus', 'Softshrink', 'Softsign', 'Tanh', 'Tanhshrink', 'Threshold']
non_linear_activations_extra_info: Dict[str, dict] = {'CELU': {'constructor_args': (2.0,), 'default_dtype': torch.double}, 'Threshold': {'constructor_args': (2.0, 1.0)}, 'Hardsigmoid': {'check_gradgrad': False, 'check_jit': False, 'default_dtype': torch.double}, 'Hardswish': {'check_gradgrad': False, 'check_jit': False, 'default_dtype': torch.double}, 'RReLU': {'test_cuda': False, 'default_dtype': torch.double}, 'ELU': {'default_dtype': torch.double}, 'GELU': {'default_dtype': torch.double}, 'GLU': {'default_dtype': torch.double}, 'Hardshrink': {'default_dtype': torch.double}, 'Hardtanh': {'default_dtype': torch.double}, 'LeakyReLU': {'default_dtype': torch.double}, 'LogSigmoid': {'default_dtype': torch.double}, 'Mish': {'default_dtype': torch.double}, 'PReLU': {'default_dtype': torch.double}, 'ReLU6': {'default_dtype': torch.double}, 'ReLU': {'default_dtype': torch.double}, 'SELU': {'default_dtype': torch.double}, 'SiLU': {'default_dtype': torch.double}, 'Sigmoid': {'default_dtype': torch.double}, 'Softplus': {'default_dtype': torch.double}, 'Softshrink': {'default_dtype': torch.double}, 'Softsign': {'default_dtype': torch.double}, 'Tanh': {'default_dtype': torch.double}, 'Tanhshrink': {'default_dtype': torch.double}}
for non_linear_activation in non_linear_activations_no_batch:
    activation_test_info = dict(module_name=non_linear_activation, input_size=(4,), reference_fn=single_batch_reference_fn, desc='no_batch_dim', test_cpp_api_parity=False)
...
for (name, reduction) in product(regression_criterion_no_batch, reductions):
    regression_test_info = dict(fullname=f'{name}_no_batch_dim_{reduction}', constructor=lambda *args, name=name: getattr(nn, name)(reduction=reduction), input_size=(3,), target_size=(3,), reference_fn=single_batch_reference_criterion_fn, test_cpp_api_parity=False, default_dtype=torch.double)
    criterion_tests.append(regression_test_info)
for reduction in reductions:
    regression_test_info = dict(fullname=f'KLDivLoss_no_batch_dim_{reduction}', constructor=lambda : nn.KLDivLoss(reduction=reduction), input_fn=lambda : torch.rand((3,)).log(), target_fn=lambda : torch.rand((3,)), reference_fn=single_batch_reference_criterion_fn, test_cpp_api_parity=False, default_dtype=torch.double)
    criterion_tests.append(regression_test_info)
classification_criterion_no_batch = [('BCELoss', lambda : torch.sigmoid(torch.randn(9, dtype=torch.double)), lambda : torch.randn(9, dtype=torch.double).gt(0).to(torch.double)), ('BCEWithLogitsLoss', lambda : torch.randn(9, dtype=torch.double), lambda : torch.randn(9, dtype=torch.double)), ('HingeEmbeddingLoss', lambda : torch.randn(9, dtype=torch.double), lambda : torch.tensor([-1, 1, 1] * 3)), ('MultiLabelMarginLoss', lambda : torch.randn(4, dtype=torch.double), lambda : torch.tensor([3, 0, -1, 1])), ('SoftMarginLoss', lambda : torch.randn(9, dtype=torch.double), lambda : torch.tensor([-1, 1, 1] * 3)), ('NLLLoss', lambda : F.log_softmax(torch.randn(3, dtype=torch.double), dim=0), lambda : torch.tensor(1)), ('CosineEmbeddingLoss', lambda : (torch.randn(9, dtype=torch.double), torch.randn(9, dtype=torch.double)), lambda : torch.tensor(1, dtype=torch.double)), ('MarginRankingLoss', lambda : (torch.randn(()), torch.randn(())), lambda : torch.randn(()).sign()), ('TripletMarginLoss', lambda : (torch.randn(9, dtype=torch.double), torch.randn(9, dtype=torch.double)), lambda : torch.randn(9, dtype=torch.double)), ('MultiLabelSoftMarginLoss', lambda : torch.randn(9, dtype=torch.double), lambda : torch.randn(9))]
classification_criterion_no_batch_extra_info: Dict[str, dict] = {'MultiLabelMarginLoss': {'check_gradgrad': False}}
classification_cpp_parity = {'BCELoss': False, 'BCEWithLogitsLoss': False, 'HingeEmbeddingLoss': False, 'NLLLoss': False, 'SoftMarginLoss': False}
reductions = ['none', 'mean', 'sum']
for ((name, input_fn, target_fn), reduction) in product(classification_criterion_no_batch, reductions):
    classification_test_info = dict(fullname=f'{name}_no_batch_dim_{reduction}', constructor=lambda *args, name=name: getattr(nn, name)(reduction=reduction), input_fn=lambda f=input_fn: f(), target_fn=lambda f=target_fn: f(), reference_fn=single_batch_reference_criterion_fn, test_cpp_api_parity=True, has_parity=classification_cpp_parity.get(name, True))
