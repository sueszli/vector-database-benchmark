...
wanted = ['blastx', 'blastp', 'blastn', 'tblastn', 'tblastx', 'rpsblast+', 'rpsblast', 'rpstblastn', 'psiblast', 'blast_formatter', 'deltablast', 'makeblastdb']
exe_names = {}
...
for folder in likely_dirs:
    ...
    for name in wanted[:]:
        ...
        if not os.path.isfile(exe_name):
            continue
        child = subprocess.Popen(exe_name + ' -h', stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, shell=sys.platform != 'win32')
...
if len(set(exe_names).difference(optional)) < len(set(wanted).difference(optional)):
    raise MissingExternalDependencyError('Install the NCBI BLAST+ command line tools if you want to use the Bio.Blast.Applications wrapper.')
