...
default = 'default'
if not isinstance(default, bool) and isinstance(default, int) or (isinstance(default, str) and default):
    pass
...
if task_id in task_dict and task_dict[task_id] is not task or task_id in used_group_ids:
    pass
...
{x for x in l if a and b or c}
new_list = [x for sublist in all_lists if a and b or c for x in sublist if isinstance(operator, list) and x in operator or x != operator]