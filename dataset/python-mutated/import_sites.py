...
async def maigret_check(site, site_data, username, status, logger):
    ...
    for (username, status) in [(username, status)]:
        results = await maigret(username, {site: site_data}, logger, query_notify, timeout=TIMEOUT, forced=True, no_progressbar=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base', '-b', metavar='BASE_FILE', dest='base_file', default='maigret/resources/data.json', help='JSON file with sites data to update.')
    ...
    parser.add_argument('--info', '-vv', action='store_true', dest='info', default=False, help='Display service information.')
    parser.add_argument('--verbose', '-v', action='store_true', dest='verbose', default=False, help='Display extra information and metrics.')
    parser.add_argument('-d', '--debug', '-vvv', action='store_true', dest='debug', default=False, help='Saving debugging information and sites responses in debug.txt.')
    parser.add_argument('urls_file', metavar='URLS_FILE', action='store', help='File with base site URLs')
    ...
    logging.basicConfig(format='[%(filename)s:%(lineno)d] %(levelname)-3s  %(asctime)s %(message)s', datefmt='%H:%M:%S', level=log_level)
    ...
    for site in tqdm.asyncio.tqdm(urls):
        ...
        site_data = {'url': site, 'urlMain': main_page_url, 'name': domain_raw}
        try:
