...
class LGamma(function_node.FunctionNode):
    ...
    def forward_cpu(self, x):
        ...
        if _lgamma_cpu is None:
            try:
                ...
                raise ImportError('SciPy is not available. Forward computation of lgamma can not be done.')
    ...
    def forward_gpu(self, x):
        if False:
            print('Hello World!')
        self.retain_inputs((0,))
        return (utils.force_array(cuda.cupyx.scipy.special.gammaln(x[0]), dtype=x[0].dtype),)
