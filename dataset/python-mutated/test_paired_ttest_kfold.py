...
def test_scoring():
    ...
    (t, p) = paired_ttest_kfold_cv(estimator1=clf1, estimator2=clf2, X=X, y=y, scoring='accuracy', random_seed=1)
    assert round(t, 3) == -1.861, t
    assert round(p, 3) == 0.096, p
    (t, p) = paired_ttest_kfold_cv(estimator1=clf1, estimator2=clf2, X=X, y=y, scoring='recall_micro', random_seed=1)
    assert round(t, 3) == -1.861, t
    assert round(p, 3) == 0.096, p
...
def test_regressor():
    ...
    (t, p) = paired_ttest_kfold_cv(estimator1=reg1, estimator2=reg2, X=X, y=y, random_seed=1)
