...
missing_deps = {r[0].strip() for d in dependencies for r in [d.split('=>')] if len(r) == 2 and r[1].strip() == 'not found'}
missing_packages = []
for d in missing_deps:
    all_packages = find_packages(d)
    packages = [p for p in all_packages if not any((p.endswith(suffix) for suffix in ['-dbg', '-test', 'tests', '-dev', '-mesa']))]
    for p in packages:
