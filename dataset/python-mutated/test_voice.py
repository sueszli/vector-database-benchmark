...
class TestVoiceWithoutRequest(TestVoiceBase):
    ...
    def test_de_json(self, bot):
        if False:
            i = 10
            return i + 15
        json_dict = {'file_id': self.voice_file_id, 'file_unique_id': self.voice_file_unique_id, 'duration': self.duration, 'mime_type': self.mime_type, 'file_size': self.file_size}
...
class TestVoiceWithRequest(TestVoiceBase):
    ...
    async def test_send_all_args(self, bot, chat_id, voice_file, voice):
        message = await bot.send_voice(chat_id, voice_file, duration=self.duration, caption=self.caption, disable_notification=False, protect_content=True, parse_mode='Markdown')
    ...
    async def test_send_voice_caption_entities(self, bot, chat_id, voice_file):
        ...
        message = await bot.send_voice(chat_id, voice_file, caption=test_string, caption_entities=entities)
    ...
    async def test_send_voice_default_parse_mode_2(self, default_bot, chat_id, voice):
        test_markdown_string = '_Italic_ *Bold* `Code`'
        message = await default_bot.send_voice(chat_id, voice, caption=test_markdown_string, parse_mode=None)
    ...
    async def test_send_voice_default_parse_mode_3(self, default_bot, chat_id, voice):
        test_markdown_string = '_Italic_ *Bold* `Code`'
        message = await default_bot.send_voice(chat_id, voice, caption=test_markdown_string, parse_mode='HTML')
        assert message.caption == test_markdown_string
        assert message.caption_markdown == escape_markdown(test_markdown_string)

    @pytest.mark.parametrize('default_bot', [{'protect_content': True}], indirect=True)
    async def test_send_voice_default_protect_content(self, chat_id, default_bot, voice):
        tasks = asyncio.gather(default_bot.send_voice(chat_id, voice), default_bot.send_voice(chat_id, voice, protect_content=False))
    ...
    async def test_send_voice_default_allow_sending_without_reply(self, default_bot, chat_id, voice, custom):
        reply_to_message = await default_bot.send_message(chat_id, 'test')
        await reply_to_message.delete()
        if custom is not None:
            message = await default_bot.send_voice(chat_id, voice, allow_sending_without_reply=custom, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        elif default_bot.defaults.allow_sending_without_reply:
            message = await default_bot.send_voice(chat_id, voice, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        else:
            with pytest.raises(BadRequest, match='Message to reply not found'):
                await default_bot.send_voice(chat_id, voice, reply_to_message_id=reply_to_message.message_id)
