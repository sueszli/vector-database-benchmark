...
from ray.serve._private.router import PowerOfTwoChoicesReplicaScheduler, Query, ReplicaWrapper, RequestMetadata
...
@pytest.mark.asyncio
@pytest.mark.parametrize('pow_2_scheduler', [{'prefer_local_node': True, 'prefer_local_az': True}, {'prefer_local_node': True, 'prefer_local_az': False}, {'prefer_local_node': False, 'prefer_local_az': True}, {'prefer_local_node': False, 'prefer_local_az': False}], indirect=True)
...
async def test_only_task_cancelled(pow_2_scheduler, fake_query):
    ...
    while time.time() - start < 10:
        ...
        raise TimeoutError('Scheduling task and pending assignment still around after 10s.')
...
@pytest.mark.asyncio
@pytest.mark.parametrize('pow_2_scheduler', [{'prefer_local_node': True, 'prefer_local_az': True}, {'prefer_local_node': True, 'prefer_local_az': False}], indirect=True)
...
@pytest.mark.asyncio
@pytest.mark.parametrize('pow_2_scheduler', [{'prefer_local_node': True, 'prefer_local_az': True, 'az': SCHEDULER_AZ}], indirect=True)
async def test_prefer_replica_in_same_az(pow_2_scheduler, fake_query):
    ...
    * lastly fall back to all replicas
    ...
    s = pow_2_scheduler
    loop = get_or_create_event_loop()
    r1 = FakeReplicaWrapper('r1', node_id=SCHEDULER_NODE_ID, availability_zone=SCHEDULER_AZ)
    r2 = FakeReplicaWrapper('r2', node_id='some_other_node_in_the_stratosphere', availability_zone=SCHEDULER_AZ)
    r3 = FakeReplicaWrapper('r3', node_id='some_other_node_in_the_stratosphere', availability_zone='some_other_az_in_the_solar_system')
    r1.set_queue_state_response(0, accepted=True)
    r2.set_queue_state_response(0, accepted=True)
...
@pytest.mark.asyncio
@pytest.mark.parametrize('pow_2_scheduler', [{'prefer_local_az': False, 'az': SCHEDULER_AZ}], indirect=True)
...
@pytest.mark.asyncio
@pytest.mark.parametrize('pow_2_scheduler', [{'prefer_local_node': False, 'prefer_local_az': True, 'az': SCHEDULER_AZ}], indirect=True)
async def test_prefer_replica_in_same_az_without_prefer_node(pow_2_scheduler, fake_query):
    """
...
@pytest.mark.asyncio
@pytest.mark.parametrize('pow_2_scheduler', [{'prefer_local_node': True, 'prefer_local_az': False, 'az': SCHEDULER_AZ}], indirect=True)
async def test_prefer_replica_on_same_node_without_prefer_az(pow_2_scheduler, fake_query):
    """
...
class TestModelMultiplexing:
    ...
    async def test_multiple_queries_with_different_model_ids(self, pow_2_scheduler):
        ...
        s.update_replicas([r1, r2, r3])
        for _ in range(10):
            tasks = [loop.create_task(s.choose_replica_for_query(query_with_model_id('m1'))), loop.create_task(s.choose_replica_for_query(query_with_model_id('m2'))), loop.create_task(s.choose_replica_for_query(query_with_model_id('m3'))), loop.create_task(s.choose_replica_for_query(query_with_model_id('m1'))), loop.create_task(s.choose_replica_for_query(query_with_model_id('m2'))), loop.create_task(s.choose_replica_for_query(query_with_model_id('m3')))]
            (done, _) = await asyncio.wait(tasks, timeout=0.1)
            assert len(done) == len(tasks)
            assert all([tasks[0].result() == r1, tasks[1].result() == r2, tasks[2].result() == r3, tasks[3].result() == r1, tasks[4].result() == r2, tasks[5].result() == r3])
    ...
    @pytest.mark.asyncio
    async def test_tasks_scheduled_fifo_among_model_ids(self, pow_2_scheduler, fake_query):
        """
        ...
        m2_tasks = []
        for _ in range(10):
            m1_tasks.append(loop.create_task(s.choose_replica_for_query(query_with_model_id('m1'))))
            m2_tasks.append(loop.create_task(s.choose_replica_for_query(query_with_model_id('m2'))))
        ...
        for i in range(10):
            r1.set_queue_state_response(0, accepted=True)
            r2.set_queue_state_response(0, accepted=True)
            (done, pending) = await asyncio.wait(m1_tasks, return_when=asyncio.FIRST_COMPLETED)
            assert done.pop() == m1_tasks[0]
            m1_tasks = m1_tasks[1:]
            (done, pending) = await asyncio.wait(m2_tasks, return_when=asyncio.FIRST_COMPLETED)
