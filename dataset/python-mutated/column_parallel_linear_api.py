...
class TestColumnParallelLinearAPI(TestCollectiveAPIRunnerBase):
    ...
    def get_model(self, main_prog, startup_program, rank):
        ...
        with base.program_guard(main_prog, startup_program):
            ...
            if rank == 0:
                param_attr = paddle.base.ParamAttr(initializer=paddle.nn.initializer.Assign(np_array[:, 0:8]))
            else:
                param_attr = paddle.base.ParamAttr(initializer=paddle.nn.initializer.Assign(np_array[:, 8:16]))
            linear_out = paddle.distributed.split(data, size=(1000, 16), operation='linear', axis=1, num_partitions=2, weight_attr=param_attr, bias_attr=True)
            return [linear_out]
