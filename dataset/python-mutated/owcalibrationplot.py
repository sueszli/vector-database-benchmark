...
from Orange.widgets.evaluate.contexthandlers import EvaluationResultsContextHandler
from Orange.widgets.evaluate.utils import results_for_preview, check_can_calibrate
...
MetricDefinition = namedtuple('metric_definition', ('name', 'functions', 'short_names', 'explanation'))
Metrics = [MetricDefinition(*args) for args in (('Calibration curve', None, (), ''), ('Classification accuracy', (Curves.ca,), (), ''), ('F1', (Curves.f1,), (), ''), ('Sensitivity and specificity', (Curves.sensitivity, Curves.specificity), ('sens', 'spec'), '<p><b>Sensitivity</b> (falling) is the proportion of correctly detected positive instances (TP&nbsp;/&nbsp;P).</p><p><b>Specificity</b> (rising) is the proportion of detected negative instances (TN&nbsp;/&nbsp;N).</p>'), ('Precision and recall', (Curves.precision, Curves.recall), ('prec', 'recall'), '<p><b>Precision</b> (rising) is the fraction of retrieved instances that are relevant, TP&nbsp;/&nbsp;(TP&nbsp;+&nbsp;FP).</p><p><b>Recall</b> (falling) is the proportion of discovered relevant instances, TP&nbsp;/&nbsp;P.</p>'), ('Pos and neg predictive value', (Curves.ppv, Curves.npv), ('PPV', 'TPV'), '<p><b>Positive predictive value</b> (rising) is the proportion of correct positives, TP&nbsp;/&nbsp;(TP&nbsp;+&nbsp;FP).</p><p><b>Negative predictive value</b> is the proportion of correct negatives, TN&nbsp;/&nbsp;(TN&nbsp;+&nbsp;FN).</p>'), ('True and false positive rate', (Curves.tpr, Curves.fpr), ('TPR', 'FPR'), '<p><b>True and false positive rate</b> are proportions of detected and omitted positive instances</p>'))]
...
class OWCalibrationPlot(widget.OWWidget):
    ...
    class Error(widget.OWWidget.Error):
        non_discrete_target = Msg('Calibration plot requires a categorical target variable.')
        empty_input = widget.Msg('Empty result on input. Nothing to display.')
        nan_classes = widget.Msg('Remove test data instances with unknown classes.')
        all_target_class = widget.Msg('All data instances belong to target class.')
        no_target_class = widget.Msg('No data instances belong to target class.')

    class Warning(widget.OWWidget.Warning):
        omitted_folds = widget.Msg('Test folds where all data belongs to (non)-target are not shown.')
        omitted_nan_prob_points = widget.Msg("Instance for which the model couldn't compute probabilities areskipped.")
