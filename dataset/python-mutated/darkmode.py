...
import dataclasses
import collections
from typing import Any, Iterator, Mapping, MutableMapping, Optional, Set, Tuple, Union, Sequence, List
...
class Variant(enum.Enum):
    ...
    qt_515_3 = enum.auto()
    qt_63 = enum.auto()
_ALGORITHMS = {'brightness-rgb': 2, 'lightness-hsl': 3, 'lightness-cielab': 4}
_ALGORITHMS_NEW = {'brightness-rgb': 1, 'lightness-hsl': 2, 'lightness-cielab': 3}
_IMAGE_POLICIES = {'always': 0, 'never': 1, 'smart': 2}
_PAGE_POLICIES = {'always': 0, 'smart': 1}
_BOOLS = {True: 'true', False: 'false'}
_INT_BOOLS = {True: '1', False: '0'}
...
class _Definition:
    ...
    def copy_add_setting(self, setting: _Setting) -> '_Definition':
        ...
        return new
_DEFINITIONS: MutableMapping[Variant, _Definition] = {Variant.qt_515_2: _Definition(_Setting('enabled', 'Enabled', _BOOLS), _Setting('algorithm', 'InversionAlgorithm', _ALGORITHMS), _Setting('policy.images', 'ImagePolicy', _IMAGE_POLICIES), _Setting('contrast', 'Contrast'), _Setting('grayscale.all', 'Grayscale', _BOOLS), _Setting('policy.page', 'PagePolicy', _PAGE_POLICIES), _Setting('threshold.text', 'TextBrightnessThreshold'), _Setting('threshold.background', 'BackgroundBrightnessThreshold'), _Setting('grayscale.images', 'ImageGrayscale'), mandatory={'enabled', 'policy.images'}, prefix='forceDarkMode'), Variant.qt_515_3: _Definition(_Setting('enabled', 'forceDarkModeEnabled', _BOOLS), _Setting('algorithm', 'InversionAlgorithm', _ALGORITHMS_NEW), _Setting('policy.images', 'ImagePolicy', _IMAGE_POLICIES), _Setting('contrast', 'ContrastPercent'), _Setting('grayscale.all', 'IsGrayScale', _BOOLS), _Setting('threshold.text', 'TextBrightnessThreshold'), _Setting('threshold.background', 'BackgroundBrightnessThreshold'), _Setting('grayscale.images', 'ImageGrayScalePercent'), mandatory={'enabled', 'policy.images'}, prefix='', switch_names={'enabled': _BLINK_SETTINGS, None: 'dark-mode-settings'})}
_DEFINITIONS[Variant.qt_63] = _DEFINITIONS[Variant.qt_515_3].copy_add_setting(_Setting('increase_text_contrast', 'IncreaseTextContrast', _INT_BOOLS))
_SettingValType = Union[str, usertypes.Unset]
_PREFERRED_COLOR_SCHEME_DEFINITIONS: Mapping[Variant, Mapping[_SettingValType, str]] = {Variant.qt_515_2: {'dark': '1', 'light': '2', 'auto': '2', usertypes.UNSET: '2'}, Variant.qt_515_3: {'dark': '0', 'light': '1'}, Variant.qt_63: {'dark': '0', 'light': '1'}}
