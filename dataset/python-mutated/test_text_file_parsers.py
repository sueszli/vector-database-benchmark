...
def mock_latex_file():
    ...
    with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.tex') as f:
        ...
        f.write(latex_str)
    return f.name
respective_file_creation_functions = {'.txt': mock_text_file, '.csv': mock_csv_file, '.pdf': mock_pdf_file, '.docx': mock_docx_file, '.json': mock_json_file, '.xml': mock_xml_file, '.yaml': mock_yaml_file, '.html': mock_html_file, '.md': mock_md_file, '.tex': mock_latex_file}
