            ...
            for batch in [1, 2, 4]:
                for start_axis in range(0, test_dims):
                    ...
                    for stop_axis in range(test_start, dims):
                        ...
                        op_outputs = {'Out': ['output_data'], 'XShape': ['xshape_data']}
                        ops_config = [{'op_type': type, 'op_inputs': {'X': ['input_data']}, 'op_outputs': op_outputs, 'op_attrs': {'start_axis': start_axis, 'stop_axis': stop_axis}}]
    ...
    def sample_predictor_configs(self, program_config) -> (paddle_infer.Config, List[int], float):
        ...
        def generate_trt_nodes_num(attrs, dynamic_shape):
            ...
            if ver[0] * 1000 + ver[1] * 100 + ver[0] * 10 >= 7000:
                if dynamic_shape:
                    return (1, 2)
                elif attrs[0]['start_axis'] == 0 or self.dims == 0 or self.dims == 1:
                    return (0, 3)
                else:
                    return (1, 2)
            else:
