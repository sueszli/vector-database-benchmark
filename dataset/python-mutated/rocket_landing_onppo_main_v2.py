...
def wrapped_rocket_env(task, max_steps):
    if False:
        i = 10
        return i + 15
    return DingEnvWrapper(Rocket(task=task, max_steps=max_steps), cfg={'env_wrapper': [lambda env: RocketLandingWrapper(env), lambda env: EvalEpisodeReturnWrapper(env)]})
