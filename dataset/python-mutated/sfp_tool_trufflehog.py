...
class sfp_tool_trufflehog(SpiderFootPlugin):
    meta = {'name': 'Tool - TruffleHog', 'summary': 'Searches through git repositories for high entropy strings and secrets, digging deep into commit history.', 'flags': ['tool', 'slow'], 'useCases': ['Footprint', 'Investigate'], 'categories': ['Crawling and Scanning'], 'toolDetails': {'name': 'TruffleHog', 'description': 'TruffleHog searches through git repositories for secrets, digging deep into commit history and branches. This is effective at finding secrets accidentally committed.', 'website': 'https://github.com/trufflesecurity/truffleHog', 'repository': 'https://github.com/trufflesecurity/truffleHog'}}
    opts = {'entropy': False, 'allrepos': False, 'trufflehog_path': ''}
    optdescs = {'trufflehog_path': 'Path to your trufflehog binary. Must be set.', 'entropy': 'Enable entropy checks? If disabled, TruffleHog will solely rely on high-signal regular expressions to identify secrets.', 'allrepos': 'Search all code repositories found. By default TruffleHog only searches those linked from the target website.'}
    results = None
    errorState = False
