...
from behave4cmd0.command_shell_proc import BehaveWinCommandOutputProcessor, TracebackLineNormalizer, ExceptionWithPathNormalizer
import re
import pytest
...
class PathNormalizer(object):
    ...
    def __call__(self, output):
        if False:
            ...
            return i + 15
        pass
line_processor_configerrors = [ExceptionWithPathNormalizer(u"ConfigError: No steps directory in '(?P<path>.*)'", 'ConfigError: No steps directory in'), BehaveWinCommandOutputProcessor.line_processors[1]]
line_processor_parsererrors = [ExceptionWithPathNormalizer(u'ParserError: Failed to parse "(?P<path>.*)"', u'ParserError: Failed to parse'), BehaveWinCommandOutputProcessor.line_processors[2]]
line_processor_ioerrors = [ExceptionWithPathNormalizer(u"No such file or directory: '(?P<path>.*)'", u'[Errno 2] No such file or directory:'), BehaveWinCommandOutputProcessor.line_processors[3]]
line_processor_traceback = [ExceptionWithPathNormalizer('^\\s*File "(?P<path>.*)", line \\d+, in ', '  File "'), BehaveWinCommandOutputProcessor.line_processors[4]]
