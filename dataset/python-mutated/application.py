...
def load_command(name: str) -> Callable[[], Command]:
    ...
    def _load() -> Command:
        ...
        return command
    return _load
COMMANDS = ['about', 'add', 'build', 'check', 'config', 'init', 'install', 'lock', 'new', 'publish', 'remove', 'run', 'search', 'shell', 'show', 'update', 'version', 'cache clear', 'cache list', 'debug info', 'debug resolve', 'env info', 'env list', 'env remove', 'env use', 'self add', 'self install', 'self lock', 'self remove', 'self update', 'self show', 'self show plugins', 'source add', 'source remove', 'source show']
