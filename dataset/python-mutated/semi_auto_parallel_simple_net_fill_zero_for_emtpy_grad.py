...
class TestSimpleNetWithEmtpyGradForSemiAutoParallel(TestSimpleNetForSemiAutoParallel):
    ...
    def run_dynamic_empty_grad(self, layer, shard_input=False):
        ...
        if shard_input:
            ...
            label = dist.shard_tensor(image, dist_attr=dist.DistAttr(mesh=self._mesh, sharding_specs=['x', None]))
        out = layer(image)
    ...
    def test_demo_net(self):
        if False:
            return 10
        mp_layer = dist.shard_layer(DemoNet('empty_grad_demo'), self._mesh, self.shard_fn)
        self.run_dynamic_empty_grad(mp_layer)
