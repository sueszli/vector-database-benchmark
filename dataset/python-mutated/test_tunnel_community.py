...
class ProxyFactory:
    ...
    async def get(self, exitnode=False, start_lt=False) -> TriblerTunnelCommunity:
        ...
        community = await create_tunnel_community(self.temp_path_factory, tunnel_community_config, exit_node_enable=exitnode, start_lt=start_lt)
...
async def proxy_factory(tmp_path_factory: TempPathFactory):
    ...
    test_community.global_dht_services = defaultdict(list)
...
async def create_tunnel_community(temp_path_factory: TempPathFactory, config: Optional[TunnelCommunitySettings]=None, exit_node_enable: bool=False, start_lt: bool=False) -> TriblerTunnelCommunity:
    """
    ...
    if start_lt:
        ...
        download_manager = DownloadManager(state_dir=temp_path_factory.mktemp('state_dir'), config=download_manager_settings, peer_mid=MagicMock(), socks_listen_ports=socks_ports, notifier=MagicMock())
    ...
    ipv8 = TriblerMockIPv8('curve25519', TriblerTunnelCommunity, settings={'max_circuits': 1}, config=config, socks_servers=socks_servers, dlmgr=download_manager)
    if start_lt:
...
async def start_anon_download(tunnel_community: TriblerTunnelCommunity, seeder_port: int, torrent_def: TorrentDef, hops: int=1) -> Download:
    """
...
async def deliver_messages(timeout: float=0.1):
    """
...
async def test_anon_download(proxy_factory: ProxyFactory, video_seeder: DownloadManager, video_tdef: TorrentDef, tunnel_community: TriblerTunnelCommunity, crash_on_error):
    """
    ...
    download_manager.set_download_states_callback(download_manager.sesscb_states_callback, interval=0.1)
...
@pytest.mark.tunneltest
async def test_hidden_services(proxy_factory: ProxyFactory, hidden_seeder_comm: TriblerTunnelCommunity, video_tdef: TorrentDef, crash_on_error):
    """
    ...
    download = await start_anon_download(leecher_community, hidden_seeder_comm.download_manager.libtorrent_port, video_tdef, hops=1)
