...
async def run_api_agent(task: ChallengeData, config: Dict[str, Any], artifacts_location: str, timeout: int) -> None:
    host_value = None
...
async def copy_agent_artifacts_into_temp_folder(api_instance, task_id):
    ...
    for artifact in artifacts.artifacts:
        ...
        if artifact.relative_path:
            path = artifact.relative_path if not artifact.relative_path.startswith('/') else artifact.relative_path[1:]
            directory_location = pathlib.Path(os.path.dirname(directory_location / path))
        ...
        with open(file_path, 'wb') as f:
            content = await api_instance.download_agent_task_artifact(task_id=task_id, artifact_id=artifact.artifact_id)
            f.write(content)
...
async def upload_artifacts(api_instance: ApiClient, artifacts_location: str, task_id: str, type: str) -> None:
    for file_path in get_list_of_file_paths(artifacts_location, type):
        relative_path: Optional[str] = '/'.join(file_path.split(f'{type}/', 1)[-1].split('/')[:-1])
        if not relative_path:
            relative_path = None
        await api_instance.upload_agent_task_artifacts(task_id=task_id, file=file_path, relative_path=relative_path)
