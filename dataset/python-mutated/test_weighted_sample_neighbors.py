...
class TestWeightedSampleNeighbors(unittest.TestCase):
    ...
    def test_sample_result(self):
        ...
        (out_neighbors, out_count) = paddle.geometric.weighted_sample_neighbors(row, colptr, weight, nodes, sample_size=self.sample_size)
    ...
    def test_sample_result_static(self):
        ...
        with paddle.static.program_guard(paddle.static.Program()):
            ...
            weight = paddle.static.data(name='weight', shape=self.weight.shape, dtype=self.weight.dtype)
            nodes = paddle.static.data(name='nodes', shape=self.nodes.shape, dtype=self.nodes.dtype)
            (out_neighbors, out_count) = paddle.geometric.weighted_sample_neighbors(row, colptr, weight, nodes, sample_size=self.sample_size)
            exe = paddle.static.Executor(paddle.CPUPlace())
            ret = exe.run(feed={'row': self.row, 'colptr': self.colptr, 'weight': self.weight, 'nodes': self.nodes}, fetch_list=[out_neighbors, out_count])
    ...
    def test_raise_errors(self):
        ...
        def check_eid_error():
            if False:
                print('Hello World!')
            paddle.geometric.weighted_sample_neighbors(row, colptr, weight, nodes, sample_size=self.sample_size, return_eids=True)
    ...
    def test_sample_result_with_eids(self):
        ...
        (out_neighbors, out_count, out_eids) = paddle.geometric.weighted_sample_neighbors(row, colptr, weight, nodes, eids=eids, sample_size=self.sample_size, return_eids=True)
        paddle.enable_static()
        with paddle.static.program_guard(paddle.static.Program()):
            row = paddle.static.data(name='row', shape=self.row.shape, dtype=self.row.dtype)
            colptr = paddle.static.data(name='colptr', shape=self.colptr.shape, dtype=self.colptr.dtype)
            ...
            (out_neighbors, out_count, out_eids) = paddle.geometric.weighted_sample_neighbors(row, colptr, weight, nodes, sample_size=self.sample_size, eids=eids, return_eids=True)
            exe = paddle.static.Executor(paddle.CPUPlace())
            ret = exe.run(feed={'row': self.row, 'colptr': self.colptr, 'weight': self.weight, 'nodes': self.nodes, 'eids': self.edges_id}, fetch_list=[out_neighbors, out_count, out_eids])
