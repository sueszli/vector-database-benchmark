...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This python script modifies build.sh files for the azure ml samples repository. Inputs are a ' + 'folder containing the azure-ml whl, the root of a cloned azureml-samples repo, and an optional build id. ' + 'Retrieves the necessary connection string from BLOB_CONNECTION_STRING.')
    parser.add_argument('--ml-repo', dest='ml_root', help='The root of a fresh clone of the ml samples repo.')
    parser.add_argument('--ml-wheel-folder', dest='wheel_folder', help='The folder from where the the azure ml wheel will reside.')
    parser.add_argument('--build-id', dest='build_id', default='a-manual-run', help='The folder from where the the azure ml wheel will reside.')
