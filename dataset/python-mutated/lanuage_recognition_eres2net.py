...
@MODELS.register_module(Tasks.speech_language_recognition, module_name=Models.eres2net_lre)
class LanguageRecognitionERes2Net(TorchModel):
    """A speech language recognition model using the ERes2Net architecture as the backbone.
    Args:
    ...
    def __init__(self, model_dir, model_config: Dict[str, Any], *args, **kwargs):
        ...
        self.encoder = ERes2Net(embed_dim=self.embed_dim, m_channels=self.m_channels)
        self.backend = LinearClassifier(input_dim=self.embed_dim, out_neurons=len(self.model_config['languages']))
