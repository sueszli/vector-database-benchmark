...
from golem.task.requestedtaskmanager import CreateTaskParams, RequestedTaskManager, ComputingNodeDefinition
...
class TestRequestedTaskManager:
    ...
    async def test_restore_tasks_timedout(self, freezer, mock_client):
        ...
        self.rtm._time_out_subtask.assert_called_once_with(task_id, subtask.subtask_id)

    @pytest.mark.asyncio
    async def test_restore_tasks_schedule(self, mock_client, monkeypatch):
        self._add_next_subtask_to_client_mock(mock_client)
        task_id = await self._create_task(task_timeout=20, subtask_timeout=20)
        await self.rtm.init_task(task_id)
        self.rtm.start_task(task_id)
        computing_node = self._get_computing_node()
        subtask = await self.rtm.get_next_subtask(task_id, computing_node)
        ...
        schedule.assert_has_calls([call(subtask.subtask_id, ANY, ANY), call(task_id, ANY, ANY)])
    ...
    async def test_init_task(self, mock_client):
        ...
        mock_client.create_task.return_value = Mock(env_id=env_id, prerequisites=prerequisites, inf_requirements=Infrastructure(min_memory_mib=2000.0))
        await self.rtm.init_task(task_id)
        row = RequestedTask.get(RequestedTask.task_id == task_id)
        assert row.status == TaskStatus.creating
        assert row.env_id == env_id
        assert row.prerequisites == prerequisites
        mock_client.create_task.assert_called_once_with(row.task_id, row.max_subtasks, row.app_params)
    ...
    async def test_get_next_subtask(self, mock_client):
        ...
        row = RequestedSubtask.get(RequestedSubtask.task == task_id, RequestedSubtask.subtask_id == res.subtask_id)
        assert row.task_id == task_id
        assert row.computing_node.node_id == computing_node.node_id
        assert row.computing_node.name == computing_node.name
        mock_client.next_subtask.assert_called_once_with(task_id=task_id, subtask_id=res.subtask_id, opaque_node_id=ANY)
    ...
    async def test_verify(self, freezer, mock_client):
        ...
        task_id = await self._start_task()
        subtask = await self.rtm.get_next_subtask(task_id, self._get_computing_node())
        ...
        subtask_row = RequestedSubtask.get(RequestedSubtask.task == task_id, RequestedSubtask.subtask_id == subtask_id)
    ...
    async def test_task_timeout_with_subtask(self, mock_client):
        ...
        subtask_id = (await self.rtm.get_next_subtask(task_id, self._get_computing_node())).subtask_id
        await asyncio.sleep(task_timeout)
        task = RequestedTask.get(RequestedTask.task_id == task_id)
        subtask = RequestedSubtask.get(RequestedSubtask.task == task_id, RequestedSubtask.subtask_id == subtask_id)
    ...
    async def test_subtask_timeout(self, mock_client):
        ...
        task_timeout = 10
        subtask_timeout = 1
        task_id = await self._start_task(task_timeout=task_timeout, subtask_timeout=subtask_timeout)
        subtask_id = (await self.rtm.get_next_subtask(task_id, self._get_computing_node())).subtask_id
    ...
    async def test_delete_task(self, mock_client):
        task_id = await self._start_task(task_timeout=10.0)
    ...
    async def test_duplicate_task(self, mock_client):
        ...
        duplicated_task_id = await self.rtm.duplicate_task(task_id, new_output_dir)
        assert duplicated_task_id != task_id
        task = RequestedTask.get(RequestedTask.task_id == task_id)
        duplicated_task = RequestedTask.get(RequestedTask.task_id == duplicated_task_id)
    ...
    async def test_discard_subtasks(self, mock_client):
        ...
        subtask = await self.rtm.get_next_subtask(task_id, self._get_computing_node())
        self._add_next_subtask_to_client_mock(mock_client)
        subtask2 = await self.rtm.get_next_subtask(task_id, self._get_computing_node(node_id='testnodeid2'))
        subtask_ids = [subtask.subtask_id, subtask2.subtask_id]
        mock_client.discard_subtasks.return_value = subtask_ids
        discarded_subtask_ids = await self.rtm.discard_subtasks(task_id, subtask_ids)
        assert discarded_subtask_ids == subtask_ids
        for subtask_id in discarded_subtask_ids:
            row = RequestedSubtask.get(RequestedSubtask.task == task_id, RequestedSubtask.subtask_id == subtask_id)
