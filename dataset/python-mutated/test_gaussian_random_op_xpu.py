...
class XPUTestGaussianRandomOp(XPUOpTestWrapper):
    ...
    class TestGaussianRandomOp(XPUOpTest):
        ...
        def setUp(self):
            ...
            self.attrs = {'shape': [123, 92], 'mean': self.mean, 'std': self.std, 'seed': 10, 'use_mkldnn': self.use_mkldnn, 'dtype': typeid_dict[self.in_type_str]}
            paddle.seed(10)
    ...
    class TestGaussianRandomOp_ShapeTensorList(TestGaussianRandomOp):
        ...
        def setUp(self):
            ...
            self.attrs = {'shape': self.infer_shape, 'mean': self.mean, 'std': self.std, 'seed': self.seed, 'use_mkldnn': self.use_mkldnn, 'dtype': typeid_dict[self.in_type_str]}
    ...
    class TestGaussianRandomOp1_ShapeTensor(TestGaussianRandomOp):
        ...
        def setUp(self):
            ...
            self.attrs = {'mean': self.mean, 'std': self.std, 'seed': self.seed, 'use_mkldnn': self.use_mkldnn, 'dtype': typeid_dict[self.in_type_str]}
            self.outputs = {'Out': np.zeros((123, 92), dtype=self.dtype)}
