...
from miasm.expression.expression import ExprInt, ExprCond, ExprId, get_expr_ids, ExprAssign, ExprLoc
from miasm.expression.simplifications import expr_simp
from miasm.ir.symbexec import SymbolicExecutionEngine, get_block
from miasm.ir.translators.translator import Translator
machine = Machine('x86_32')
parser = OptionParser(usage='usage: %prog [options] file')
parser.add_option('-a', '--address', dest='address', metavar='ADDRESS', help='address to disasemble', default='0')
...
if __name__ == '__main__':
    ...
    symbexec = SymbolicExecutionEngine(lifter)
    asmcfg = parse_asm.parse_txt(machine.mn, 32, '\n    init:\n    PUSH argv\n    PUSH argc\n    PUSH ret_addr\n    ', loc_db)
    ...
    fix_args = {argc_loc: ExprId('argc', argc_loc.size), argv_loc: ExprId('argv', argv_loc.size), ret_addr_loc: ret_addr}
    ...
    for (addr, reqs_cond) in all_info:
        ...
        for (expr, value) in reqs_cond:
            ...
            expr_test = ExprCond(expr, ExprInt(1, value.size), ExprInt(0, value.size))
        ...
        try:
            cases = subprocess.check_output(['stp', '-p', '--SMTLIB2', 'out.dot'])
        except OSError as e:
