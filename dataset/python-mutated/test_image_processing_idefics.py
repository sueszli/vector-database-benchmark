...
class IdeficsImageProcessingTester(unittest.TestCase):
    ...
    def __init__(self, parent, batch_size=7, num_channels=3, image_size=18, min_resolution=30, max_resolution=400, size=None, image_mean=[0.48145466, 0.4578275, 0.40821073], image_std=[0.26862954, 0.26130258, 0.27577711]):
        if False:
            print('Hello World!')
    ...
    def prepare_image_processor_dict(self):
        if False:
            i = 10
            return i + 15
        return {'image_mean': self.image_mean, 'image_std': self.image_std, 'image_size': self.image_size}
...
class IdeficsImageProcessingTest(ImageProcessingTestMixin, unittest.TestCase):
    ...
    def test_torchvision_numpy_transforms_equivalency(self):
        ...
        transform = transforms.Compose([convert_to_rgb, transforms.Resize((image_size, image_size), interpolation=transforms.InterpolationMode.BICUBIC), transforms.ToTensor(), transforms.Normalize(mean=image_mean, std=image_std)])
