...
class LinearWithWarmup(PolynomialDecay):
    ...
    def __init__(self, optimizer: torch.optim.Optimizer, num_epochs: int, num_steps_per_epoch: int, warmup_steps: int=100, last_epoch: int=-1) -> None:
        if False:
            print('Hello World!')
        super().__init__(optimizer, num_epochs, num_steps_per_epoch, power=1.0, warmup_steps=warmup_steps, end_learning_rate=0.0, last_epoch=last_epoch)