...
class DatabaseResourceTest(BaseApiTestCase):
    ...
    async def test_process_collection(self):
        self.bootstrap()
        result_set = await UserResource.process_collection({}, {}, self.user, **self.options)
        self.assertEqual([r.model for r in result_set], self.users)
        self.assertEqual(result_set.metadata, dict(count=len(self.users), next=False))
        result_set = await UserResource.process_collection(dict(username=self.user.username), {}, self.user, **self.options)
        self.assertEqual([r.model for r in result_set], [self.user])
        self.assertEqual(result_set.metadata, dict(count=1, next=False))
        result_set = await UserResource.process_collection({}, dict(_limit=1), self.user, **self.options)
        self.assertEqual([r.model for r in result_set], self.users[:1])
        self.assertEqual(result_set.metadata, dict(count=2, next=True))
        result_set = await UserResource.process_collection({}, dict(_limit=1, _offset=1), self.user, **self.options)
        self.assertEqual([r.model for r in result_set], self.users[1:])
        self.assertEqual(result_set.metadata, dict(count=2, next=False))
        self.cleanup()

    async def test_collection(self):
        self.bootstrap()
        models = UserResource.collection(dict(username=self.user.username), {}, self.user, **self.options)
    ...
    async def test_collection_with_parent_model(self):
        self.bootstrap()
        user_roles = [UserRole.create(role_id=2 if idx == 2 else 0, user_id=user.id) for (idx, user) in enumerate(self.users)]
        models = TestUserRoleResource.collection(dict(role_id=0), {}, self.user, parent_model=self.user, **self.options)
        self.assertEqual(len(list(models)), 1)
