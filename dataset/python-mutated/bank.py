...
__all__ = ('is_owner_if_bank_global', 'Account', 'get_balance', 'can_spend', 'set_balance', 'withdraw_credits', 'deposit_credits', 'transfer_credits', 'wipe_bank', 'bank_prune', 'get_leaderboard', 'get_leaderboard_position', 'get_account', 'is_global', 'set_global', 'get_bank_name', 'set_bank_name', 'get_currency_name', 'set_currency_name', 'get_max_balance', 'set_max_balance', 'get_default_balance', 'set_default_balance', 'AbortPurchase', 'cost')
_MAX_BALANCE = 2 ** 63 - 1
_SCHEMA_VERSION = 1
_DEFAULT_GLOBAL = {'schema_version': 0, 'is_global': False, 'bank_name': 'Twentysix bank', 'currency': 'credits', 'default_balance': 100, 'max_balance': _MAX_BALANCE}
...
async def withdraw_credits(member: discord.Member, amount: int) -> int:
    ...
    if amount > bal:
        raise ValueError('Insufficient funds {} > {}'.format(humanize_number(amount, override_locale='en_US'), humanize_number(bal, override_locale='en_US')))
...
async def set_currency_name(name: str, guild: discord.Guild=None) -> str:
    ...
    if await is_global():
        ...
        raise RuntimeError('Guild must be provided if setting the currency name of a guild-specific bank.')
    return name
...
async def set_max_balance(amount: int, guild: discord.Guild=None) -> int:
    ...
    if not 0 < amount <= _MAX_BALANCE:
        raise ValueError('Amount must be greater than zero and less than {max}.'.format(max=humanize_number(_MAX_BALANCE, override_locale='en_US')))
    if await is_global():
        ...
        raise RuntimeError('Guild must be provided if setting the maximum balance of a guild-specific bank.')
    return amount
...
async def set_default_balance(amount: int, guild: discord.Guild=None) -> int:
    ...
    if not 0 <= amount <= max_bal:
        raise ValueError('Amount must be greater than or equal zero and less than or equal {max}.'.format(max=humanize_number(max_bal, override_locale='en_US')))
    if await is_global():
