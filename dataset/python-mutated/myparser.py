...
class Parser:
    ...
    async def genericClean(self) -> None:
        self.results = self.results.replace('<em>', '').replace('<b>', '').replace('</b>', '').replace('</em>', '').replace('%3a', '').replace('<strong>', '').replace('</strong>', '').replace('<wbr>', '').replace('</wbr>', '')
    ...
    async def emails(self):
        ...
        reg_emails = re.compile("[a-zA-Z0-9.\\-_+#~!$&\\',;=:]+" + '@' + '[a-zA-Z0-9.-]*' + self.word.replace('www.', ''))
        self.temp = reg_emails.findall(self.results)
        emails = await self.unique()
        true_emails = {str(email)[1:].lower().strip() if len(str(email)) > 1 and str(email)[0] == '.' else len(str(email)) > 1 and str(email).lower().strip() for email in emails}
        return true_emails
    ...
    async def hostnames(self):
        ...
        reg_hosts = re.compile('[a-zA-Z0-9.-]*\\.' + self.word.replace('www.', ''))
    ...
    async def set(self):
        reg_sets = re.compile('>[a-zA-Z\\d]*</a></font>')
    ...
    async def urls(self) -> Set[str]:
        found = re.finditer('(http|https)://(www\\.)?trello.com/([a-zA-Z\\d\\-_\\.]+/?)*', self.results)
