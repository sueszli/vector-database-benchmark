...
class StateTaskServiceApiGateway(StateTaskServiceCallback):
    _SUPPORTED_API_PARAM_BINDINGS: Final[dict[str, set[str]]] = {SupportedApiCalls.invoke: set(TaskParameters.__required_keys__)}
    _FORBIDDEN_HTTP_HEADERS_PREFIX: Final[set[str]] = {'X-Forwarded', 'X-Amz', 'X-Amzn'}
    _FORBIDDEN_HTTP_HEADERS: Final[set[str]] = {'Authorization', 'Connection', 'Content-md5', 'Expect', 'Host', 'Max-Forwards', 'Proxy-Authenticate', 'Server', 'TE', 'Transfer-Encoding', 'Trailer', 'Upgrade', 'Via', 'Www-Authenticate'}
