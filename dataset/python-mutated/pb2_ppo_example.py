...
if __name__ == '__main__':
    ...
    if args.env_name in ['BipedalWalker-v2', 'BipedalWalker-v3']:
        ...
        horizon = 1000
    pbt = PopulationBasedTraining(time_attr=args.criteria, metric='episode_reward_mean', mode='max', perturbation_interval=args.t_ready, resample_probability=args.perturb, quantile_fraction=args.perturb, hyperparam_mutations={'lambda': lambda : random.uniform(0.9, 1.0), 'clip_param': lambda : random.uniform(0.1, 0.5), 'lr': lambda : random.uniform(0.001, 1e-05), 'train_batch_size': lambda : random.randint(1000, 60000)}, custom_explore_fn=explore)
    pb2 = PB2(time_attr=args.criteria, metric='episode_reward_mean', mode='max', perturbation_interval=args.t_ready, quantile_fraction=args.perturb, hyperparam_bounds={'lambda': [0.9, 1.0], 'clip_param': [0.1, 0.5], 'lr': [1e-05, 0.001], 'train_batch_size': [1000, 60000]})
    ...
    analysis = run(args.algo, name='{}_{}_{}_seed{}_{}'.format(timelog, args.method, args.env_name, str(args.seed), args.filename), scheduler=methods[args.method], verbose=1, num_samples=args.num_samples, stop={args.criteria: args.max}, config={'env': args.env_name, 'log_level': 'INFO', 'seed': args.seed, 'kl_coeff': 1.0, 'num_gpus': 0, 'horizon': horizon, 'observation_filter': 'MeanStdFilter', 'model': {'fcnet_hiddens': [int(args.net.split('_')[0]), int(args.net.split('_')[1])], 'free_log_std': True}, 'num_sgd_iter': 10, 'sgd_minibatch_size': 128, 'lambda': sample_from(lambda spec: random.uniform(0.9, 1.0)), 'clip_param': sample_from(lambda spec: random.uniform(0.1, 0.5)), 'lr': sample_from(lambda spec: random.uniform(0.001, 1e-05)), 'train_batch_size': sample_from(lambda spec: random.randint(1000, 60000))})
