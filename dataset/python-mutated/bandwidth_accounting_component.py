from tribler.core.components.bandwidth_accounting.community.bandwidth_accounting_community import BandwidthAccountingCommunity, BandwidthAccountingTestnetCommunity
...
class BandwidthAccountingComponent(Component):
    ...
    async def run(self):
        ...
        self.database = BandwidthDatabase(database_path, self._ipv8_component.peer.public_key.key_to_bin(), store_all_transactions=store_all_transactions)
        kwargs = {'max_peers': -1} if unlimited_peers else {}
        self.community = bandwidth_cls(self._ipv8_component.peer, self._ipv8_component.ipv8.endpoint, self._ipv8_component.ipv8.network, settings=config.bandwidth_accounting, database=self.database, **kwargs)
