            ...
            for actFun in allAct:
                for missingValuesHandling in missingValues:
                    for setAllFactor in allFactors:
                        ...
                        print('Activation function: {0}, missing value handling: {1}, skippAllFactor: {2}'.format(actFun, missingValuesHandling, setAllFactor))
                        runComparisonTests(encoderOn, actFun, missingValuesHandling, setAllFactor, train, test, x)

def runComparisonTests(autoEncoder, actFun, missingValuesHandling, setAllFactor, train, test, x):
    if False:
        print('Hello World!')
    params = set_params(actFun, missingValuesHandling, setAllFactor, autoEncoder)
    if autoEncoder:
        try:
            deeplearningModel = build_save_model(params, x, train)
    ...
    (pred_h2o, pred_mojo) = pyunit_utils.mojo_predict(deeplearningModel, TMPDIR, MOJONAME)
    pred_pojo = pyunit_utils.pojo_predict(deeplearningModel, TMPDIR, MOJONAME)
    h2o.save_model(deeplearningModel, path=TMPDIR, force=True)
...
def set_params(actFun, missingValuesHandling, setAllFactor, enableEncoder=False):
    if False:
        for i in range(10):
            print('nop')
    dropOutRatio = 0.25
    (hiddens, hidden_dropout_ratios) = random_networkSize(actFun)
    seed = 12345
    if 'dropout' in actFun:
        params = {'hidden': hiddens, 'standardize': True, 'missing_values_handling': missingValuesHandling, 'activation': actFun, 'use_all_factor_levels': setAllFactor, 'hidden_dropout_ratios': hidden_dropout_ratios, 'input_dropout_ratio': dropOutRatio, 'autoencoder': enableEncoder, 'seed': seed, 'reproducible': True}
    else:
        params = {'hidden': hiddens, 'standardize': True, 'missing_values_handling': missingValuesHandling, 'activation': actFun, 'use_all_factor_levels': setAllFactor, 'input_dropout_ratio': dropOutRatio, 'autoencoder': enableEncoder, 'seed': seed, 'reproducible': True}
    print(params)
    return params
