...
class functionsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'call_function': ('name', 'data'), 'create_function': ('location', 'function'), 'delete_function': ('name',), 'generate_download_url': ('name', 'version_id'), 'generate_upload_url': ('parent', 'kms_key_name'), 'get_function': ('name',), 'get_iam_policy': ('resource', 'options'), 'list_functions': ('parent', 'page_size', 'page_token'), 'set_iam_policy': ('resource', 'policy', 'update_mask'), 'test_iam_permissions': ('resource', 'permissions'), 'update_function': ('function', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=functionsCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
