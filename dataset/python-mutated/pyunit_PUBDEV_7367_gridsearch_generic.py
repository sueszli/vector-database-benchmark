    ...
    def setup_data(self):
        ...
        for lambda_param in self.hyper_parameters['lambda']:
            for alpha_param in self.hyper_parameters['alpha']:
                self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family='multinomial', gam_columns=['C6', 'C7', 'C8'], keep_gam_cols=True, bs=[0, 1, 3], seed=1234, alpha=alpha_param, lambda_=lambda_param))

    def train_models(self):
        if False:
            return 10
        self.h2o_model = H2OGridSearch(H2OGeneralizedAdditiveEstimator(family='multinomial', gam_columns=['C6', 'C7', 'C8'], keep_gam_cols=True, bs=[0, 1, 3], seed=1234), self.hyper_parameters)
