...
class SuccessiveHalving(abc.ABC):
    ...
    def __init__(self, models, metric: metrics.base.Metric, budget: int, eta=2, verbose=False, **print_kwargs):
        if False:
            while True:
                i = 10
    ...
    def learn_one(self, x, y):
        ...
        if self._s > 1 and self._n_iterations == self._r:
            ...
            self._rankings[:self._s] = sorted(self._rankings[:self._s], key=lambda i: self._metrics[i].get(), reverse=self.metric.bigger_is_better)
            cutoff = math.ceil(self._s / self.eta)
            if self.verbose:
                print('\t'.join((f'[{self._n_rungs}]', f'{self._s - cutoff} removed', f'{cutoff} left', f'{self._r} iterations', f'budget used: {self._budget_used}', f'budget left: {self.budget - self._budget_used}', f'best {self._metrics[self._rankings[0]]}')), **self.print_kwargs)
            self._s = cutoff
