...
def gml_data_set(filepath_gml):
    if False:
        while True:
            i = 10
    return GMLDataSet(filepath=filepath_gml, load_args={'destringizer': int}, save_args={'stringizer': str})
...
def versioned_gml_data_set(filepath_gml, load_version, save_version):
    if False:
        i = 10
        return i + 15
    return GMLDataSet(filepath=filepath_gml, version=Version(load_version, save_version), load_args={'destringizer': int}, save_args={'stringizer': str})
...
class TestGMLDataSet:
    ...
    def test_load_missing_file(self, gml_data_set):
        ...
        pattern = 'Failed while loading data from data set GMLDataSet\\(.*\\)'
    ...
    @pytest.mark.parametrize('filepath,instance_type', [('s3://bucket/file.gml', S3FileSystem), ('file:///tmp/test.gml', LocalFileSystem), ('/tmp/test.gml', LocalFileSystem), ('gcs://bucket/file.gml', GCSFileSystem), ('https://example.com/file.gml', HTTPFileSystem)])
...
class TestGMLDataSetVersioned:
    ...
    def test_no_versions(self, versioned_gml_data_set):
        ...
        pattern = 'Did not find any versions for GMLDataSet\\(.+\\)'
        with pytest.raises(DatasetError, match=pattern):
    ...
    def test_prevent_override(self, versioned_gml_data_set, dummy_graph_data):
        ...
        versioned_gml_data_set.save(dummy_graph_data)
        pattern = "Save path \\'.+\\' for GMLDataSet\\(.+\\) must not exist if versioning is enabled"
        with pytest.raises(DatasetError, match=pattern):
            versioned_gml_data_set.save(dummy_graph_data)
    ...
    def test_save_version_warning(self, versioned_gml_data_set, load_version, save_version, dummy_graph_data):
        if False:
            i = 10
            return i + 15
        'Check the warning when saving to the path that differs from\n        the subsequent load path.'
        pattern = f"Save version '{save_version}' did not match load version '{load_version}' for GMLDataSet\\(.+\\)"
    ...
    def test_version_str_repr(self, load_version, save_version):
        ...
        ds_versioned = GMLDataSet(filepath=filepath, version=Version(load_version, save_version))
    ...
    def test_versioning_existing_dataset(self, gml_data_set, versioned_gml_data_set, dummy_graph_data):
        if False:
            i = 10
            return i + 15
        ...
        pattern = f'(?=.*file with the same name already exists in the directory)(?=.*{versioned_gml_data_set._filepath.parent.as_posix()})'
        with pytest.raises(DatasetError, match=pattern):
            versioned_gml_data_set.save(dummy_graph_data)
