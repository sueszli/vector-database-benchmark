...
import subprocess
import six
from ctypes import c_int, c_bool, c_char_p, c_char, POINTER, c_uint, c_uint32, c_uint64, c_double, c_short, create_string_buffer, cdll, pointer, c_void_p, CFUNCTYPE
...
class _GObject(Structure):
    _fields_ = [('g_type_instance', _GTypeInstance), ('ref_count', c_uint), ('qdata', POINTER(_GData))]

class _GError(Structure):
    _fields_ = [('domain', c_uint32), ('code', c_int), ('message', c_char_p)]

class _GArray(Structure):
    _fields_ = [('data', POINTER(c_char)), ('len', c_uint32)]
...
class _AtspiObject(Structure):
    _fields_ = [('parent', _GObject), ('accessible_parent', POINTER(_AtspiApplication)), ('path', c_char_p)]

class _AtspiStateSet(Structure):
    _fields_ = [('fake', c_uint64 * 4), ('states', c_uint64)]

class _AtspiAction(Structure):
    _fields_ = [('parent', _GTypeInterface)]
...
class _AtspiTextRange(Structure):
    _fields_ = [('start_offset', c_int), ('end_offset', c_int), ('content', c_char_p)]

class _AtspiRange(Structure):
    _fields_ = [('start_offset', c_int), ('end_offset', c_int)]

class _AtspiAccessible(Structure):
    pass
_AtspiAccessible._fields_ = [('parent', _AtspiObject), ('accessible_parent', POINTER(_AtspiAccessible)), ('children', POINTER(_GPtrArray)), ('role', _AtspiRole), ('gint', c_int), ('name', c_char_p), ('description', c_char_p), ('states', POINTER(_AtspiStateSet)), ('attributes', POINTER(_GHashTable)), ('cached_properties', c_uint), ('priv', POINTER(_AtspiAccessiblePrivate))]
...
class AtspiComponent(object):
    ...
    _get_accessible_at_point.argtypes = [POINTER(_AtspiComponent), c_int, c_int, _AtspiCoordType, POINTER(POINTER(_GError))]
    ...
    _set_extents.argtypes = [POINTER(_AtspiComponent), c_int, c_int, c_int, c_int, _AtspiCoordType, POINTER(POINTER(_GError))]
    _set_extents.restype = c_bool
    _set_position = IATSPI().get_iface_func('atspi_component_set_position')
    _set_position.argtypes = [POINTER(_AtspiComponent), c_int, c_int, _AtspiCoordType, POINTER(POINTER(_GError))]
    ...
    if _scroll_to_point:
        _scroll_to_point.argtypes = [POINTER(_AtspiComponent), _AtspiCoordType, c_int, c_int, POINTER(POINTER(_GError))]
