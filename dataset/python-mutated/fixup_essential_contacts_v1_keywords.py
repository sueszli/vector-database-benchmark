...
class essential_contactsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'compute_contacts': ('parent', 'notification_categories', 'page_size', 'page_token'), 'create_contact': ('parent', 'contact'), 'delete_contact': ('name',), 'get_contact': ('name',), 'list_contacts': ('parent', 'page_size', 'page_token'), 'send_test_message': ('contacts', 'resource', 'notification_category'), 'update_contact': ('contact', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=essential_contactsCallTransformer()):
    if False:
        return 10
