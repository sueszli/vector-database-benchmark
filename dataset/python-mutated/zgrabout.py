...
from ivre.active.data import add_cert_hostnames, add_hostname, create_ssl_cert, handle_http_content, handle_http_headers
...
_EXPR_CENTREON_VERSION = re.compile(re.escape('<td class="LoginInvitVersion"><br />') + '\\s+((?:[0-9]+\\.)+[0-9]+)\\s+' + re.escape('</td>') + '|' + re.escape('<span>') + '\\s+v\\.\\ ((?:[0-9]+\\.)+[0-9]+)\\s+' + re.escape('</span>'))
ntlm_values = ['Target_Name', 'NetBIOS_Domain_Name', 'NetBIOS_Computer_Name', 'DNS_Domain_Name', 'DNS_Computer_Name', 'DNS_Tree_Name', 'Product_Version', 'NTLM_Version']
...
ZGRAB_PARSERS = {'http': zgrap_parser_http, 'jarm': zgrap_parser_jarm}