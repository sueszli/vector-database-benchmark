...
if is_torch_available():
    import torch
    from transformers import Wav2Vec2FeatureExtractor, WavLMForAudioFrameClassification, WavLMForCTC, WavLMForSequenceClassification, WavLMForXVector, WavLMModel
...
class WavLMModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (WavLMForCTC, WavLMModel, WavLMForAudioFrameClassification, WavLMForSequenceClassification, WavLMForXVector) if is_torch_available() else ()
    pipeline_model_mapping = {'audio-classification': WavLMForSequenceClassification, 'automatic-speech-recognition': WavLMForCTC, 'feature-extraction': WavLMModel} if is_torch_available() else {}
    test_pruning = False
