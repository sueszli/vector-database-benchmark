...
class TestFusedFFNOp(OpTest):
    ...
    def Base(self):
        ...
        if self.pre_layer_norm:
            ...
            linear2_out = self.linear2(self.dropout(self.activation(self.linear1(ln1_out))))
            ...
            linear2_out = self.linear2(self.dropout(self.activation(self.linear1(tensor_src))))
            dropout2_out = residual + self.dropout2(linear2_out)
...
class APITestStaticFusedFFN(unittest.TestCase):
    ...
    def run_fused_feedforward(self, x_data, linear1_weight_data, linear1_bias_data, linear2_weight_data, linear2_bias_data, ln1_scale_data, ln1_bias_data, ln2_scale_data, ln2_bias_data):
        if False:
            print('Hello World!')
        ...
        with paddle.static.program_guard(main, startup):
            x = paddle.static.data(name='x', shape=[self.batch_size, self.d_model, self.dim_feedforward], dtype=self.dtype)
            linear1_weight = paddle.static.data(name='linear1_weight', shape=[self.d_model, self.dim_feedforward], dtype=self.dtype)
            linear1_bias = paddle.static.data(name='linear1_bias', shape=[self.dim_feedforward])
            linear2_weight = paddle.static.data(name='linear2_weight', shape=[self.dim_feedforward, self.d_model], dtype=self.dtype)
            linear2_bias = paddle.static.data(name='linear2_bias', shape=[self.d_model])
            ln1_scale = paddle.static.data(name='ln1_scale', shape=[self.d_model])
            ln1_bias = paddle.static.data(name='ln1_bias', shape=[self.d_model])
            ln2_scale = paddle.static.data(name='ln2_scale', shape=[self.d_model])
            ...
            fetch = exe.run(feed={'x': x_data, 'linear1_weight': linear1_weight_data, 'linear1_bias': linear1_bias_data, 'linear2_weight': linear2_weight_data, 'linear2_bias': linear2_bias_data, 'ln1_scale': ln1_scale_data, 'ln1_bias': ln1_bias_data, 'ln2_scale': ln2_scale_data, 'ln2_bias': ln2_bias_data}, fetch_list=[fused_out])
        return fetch

    def run_base_ffn(self, x_data, linear1_weight_data, linear1_bias_data, linear2_weight_data, linear2_bias_data, ln1_scale_data, ln1_bias_data, ln2_scale_data, ln2_bias_data):
        if False:
            for i in range(10):
                print('nop')
        main = paddle.static.Program()
        ...
        with paddle.static.program_guard(main, startup):
            ...
            ln_out = F.layer_norm(dropout2_out, normalized_shape=[self.d_model], weight=ln2_scale, bias=ln2_bias)
            exe = paddle.static.Executor(paddle.CUDAPlace(0))
            fetch = exe.run(feed={'x': x_data, 'linear1_weight': linear1_weight_data, 'linear1_bias': linear1_bias_data, 'linear2_weight': linear2_weight_data, 'linear2_bias': linear2_bias_data, 'ln1_scale': ln1_scale_data, 'ln1_bias': ln1_bias_data, 'ln2_scale': ln2_scale_data, 'ln2_bias': ln2_bias_data}, fetch_list=[ln_out])
        return fetch
    ...
    def test_static(self):
        ...
        x_data = np.random.random((self.batch_size, self.d_model, self.dim_feedforward)).astype(self.dtype)
        linear1_weight_data = np.random.random((self.d_model, self.dim_feedforward)).astype(self.dtype)
        linear1_bias_data = np.zeros(self.dim_feedforward).astype(self.dtype)
        linear2_weight_data = np.random.random((self.dim_feedforward, self.d_model)).astype(self.dtype)
        ...
        fused_feedforward_res = self.run_fused_feedforward(x_data, linear1_weight_data, linear1_bias_data, linear2_weight_data, linear2_bias_data, ln1_scale_data, ln1_bias_data, ln2_scale_data, ln2_bias_data)
        base_ffn_res = self.run_base_ffn(x_data, linear1_weight_data, linear1_bias_data, linear2_weight_data, linear2_bias_data, ln1_scale_data, ln1_bias_data, ln2_scale_data, ln2_bias_data)
        np.testing.assert_allclose(fused_feedforward_res, base_ffn_res, rtol=1e-05, atol=0.001)
