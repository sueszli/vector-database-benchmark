...
parser.add_argument('--stop-reward', type=float, default=0.0, help='Reward at which we stop training.')
...
if __name__ == '__main__':
    ...
    if args.flat:
        results = tune.Tuner('PPO', run_config=air.RunConfig(stop=stop), param_space=PPOConfig().environment(WindyMazeEnv).rollouts(num_rollout_workers=0).framework(args.framework).to_dict()).fit()
    else:
        ...
        config = PPOConfig().environment(HierarchicalWindyMazeEnv).framework(args.framework).rollouts(num_rollout_workers=0).training(entropy_coeff=0.01).multi_agent(policies={'high_level_policy': (None, maze.observation_space, Discrete(4), PPOConfig.overrides(gamma=0.9)), 'low_level_policy': (None, Tuple([maze.observation_space, Discrete(4)]), maze.action_space, PPOConfig.overrides(gamma=0.0))}, policy_mapping_fn=policy_mapping_fn).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
        results = tune.Tuner('PPO', param_space=config.to_dict(), run_config=air.RunConfig(stop=stop, verbose=1)).fit()
    if args.as_test:
