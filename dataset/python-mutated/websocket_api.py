...
from .pipeline import AudioSettings, PipelineData, PipelineError, PipelineEvent, PipelineEventType, PipelineInput, PipelineRun, PipelineStage, WakeWordSettings, async_get_pipeline
...
@websocket_api.websocket_command(vol.All(websocket_api.BASE_COMMAND_MESSAGE_SCHEMA.extend({vol.Required('type'): 'assist_pipeline/run', vol.Required('start_stage'): lambda val: PipelineStage(val), vol.Required('end_stage'): lambda val: PipelineStage(val), vol.Optional('input'): dict, vol.Optional('pipeline'): str, vol.Optional('conversation_id'): vol.Any(str, None), vol.Optional('device_id'): vol.Any(str, None), vol.Optional('timeout'): vol.Any(float, int)}), cv.key_value_schemas('start_stage', {PipelineStage.WAKE_WORD: vol.Schema({vol.Required('input'): {vol.Required('sample_rate'): int, vol.Optional('timeout'): vol.Any(float, int), vol.Optional('audio_seconds_to_buffer'): vol.Any(float, int), vol.Optional('noise_suppression_level'): int, vol.Optional('auto_gain_dbfs'): int, vol.Optional('volume_multiplier'): float, vol.Optional('no_vad'): bool, vol.Optional('no_chunking'): bool}}, extra=vol.ALLOW_EXTRA), PipelineStage.STT: vol.Schema({vol.Required('input'): {vol.Required('sample_rate'): int}}, extra=vol.ALLOW_EXTRA), PipelineStage.INTENT: vol.Schema({vol.Required('input'): {'text': str}}, extra=vol.ALLOW_EXTRA), PipelineStage.TTS: vol.Schema({vol.Required('input'): {'text': str}}, extra=vol.ALLOW_EXTRA)})))
...
async def websocket_run(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict[str, Any]) -> None:
    ...
    try:
        ...
        connection.send_error(msg['id'], 'pipeline-not-found', f'Pipeline not found: id={pipeline_id}')
        return
    ...
    input_args: dict[str, Any] = {'conversation_id': msg.get('conversation_id'), 'device_id': msg.get('device_id')}
    if start_stage in (PipelineStage.WAKE_WORD, PipelineStage.STT):
        msg_input = msg['input']
        audio_queue: asyncio.Queue[bytes] = asyncio.Queue()
        incoming_sample_rate = msg_input['sample_rate']
        if start_stage == PipelineStage.WAKE_WORD:
            wake_word_settings = WakeWordSettings(timeout=msg['input'].get('timeout', DEFAULT_WAKE_WORD_TIMEOUT), audio_seconds_to_buffer=msg_input.get('audio_seconds_to_buffer', 0))

        async def stt_stream() -> AsyncGenerator[bytes, None]:
            state = None
            while (chunk := (await audio_queue.get())):
                if incoming_sample_rate != 16000:
                    (chunk, state) = audioop.ratecv(chunk, 2, 1, incoming_sample_rate, 16000, state)
                yield chunk
        ...
        (handler_id, unregister_handler) = connection.async_register_binary_handler(handle_binary)
        input_args['stt_metadata'] = stt.SpeechMetadata(language=pipeline.stt_language or pipeline.language, format=stt.AudioFormats.WAV, codec=stt.AudioCodecs.PCM, bit_rate=stt.AudioBitRates.BITRATE_16, sample_rate=stt.AudioSampleRates.SAMPLERATE_16000, channel=stt.AudioChannels.CHANNEL_MONO)
        input_args['stt_stream'] = stt_stream()
        audio_settings = AudioSettings(noise_suppression_level=msg_input.get('noise_suppression_level', 0), auto_gain_dbfs=msg_input.get('auto_gain_dbfs', 0), volume_multiplier=msg_input.get('volume_multiplier', 1.0), is_vad_enabled=not msg_input.get('no_vad', False), is_chunking_enabled=not msg_input.get('no_chunking', False))
    ...
    input_args['run'] = PipelineRun(hass, context=connection.context(msg), pipeline=pipeline, start_stage=start_stage, end_stage=end_stage, event_callback=lambda event: connection.send_event(msg['id'], event), runner_data={'stt_binary_handler_id': handler_id, 'timeout': timeout}, wake_word_settings=wake_word_settings, audio_settings=audio_settings or AudioSettings())
...
@websocket_api.websocket_command({vol.Required('type'): 'assist_pipeline/language/list'})
@websocket_api.async_response
async def websocket_list_languages(hass: HomeAssistant, connection: websocket_api.connection.ActiveConnection, msg: dict[str, Any]) -> None:
    ...
    if tts_language_tags:
        ...
        if pipeline_languages is not None:
            ...
            pipeline_languages = languages
    connection.send_result(msg['id'], {'languages': sorted(pipeline_languages) if pipeline_languages else pipeline_languages})