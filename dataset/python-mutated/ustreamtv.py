...
class UStreamTVWsClient(WebsocketClient):
    ...
    _schema_cmd = validate.Schema({'cmd': str, 'args': [{str: object}]})
    _schema_stream_formats = validate.Schema({'streams': [validate.any(validate.all({'contentType': 'video/mp4', 'sourceStreamVersion': int, 'initUrl': str, 'segmentUrl': str, 'bitrate': int, 'height': int}, validate.transform(lambda obj: StreamFormatVideo(**obj))), validate.all({'contentType': 'audio/mp4', 'sourceStreamVersion': int, 'initUrl': str, 'segmentUrl': str, 'bitrate': int, validate.optional('language'): str}, validate.transform(lambda obj: StreamFormatAudio(**obj))), object)]})
    _schema_stream_segments = validate.Schema({'chunkId': int, 'chunkTime': int, 'contentAccess': validate.all({'accessList': [{'data': {'path': str}}]}, validate.get(('accessList', 0, 'data', 'path'))), 'hashes': {validate.transform(int): str}})
    ...
    def _handle_module_info_stream(self, data: Dict):
        ...
        self._set_ready()
    _MESSAGE_HANDLERS: Dict[str, Dict[str, Callable[['UStreamTVWsClient', Any], None]]] = {'warning': {'code': _handle_warning}, 'reject': {'cluster': _handle_reject_cluster, 'referrerLock': _handle_reject_referrer_lock, 'nonexistent': _handle_reject_nonexistent, 'geoLock': _handle_reject_geo_lock}, 'moduleInfo': {'cdnConfig': _handle_module_info_cdn_config, 'stream': _handle_module_info_stream}}
...
@pluginargument('password', sensitive=True, argument_name='ustream-password', metavar='PASSWORD', help='A password to access password protected UStream.tv channels.')
