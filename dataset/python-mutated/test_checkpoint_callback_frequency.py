...
@pytest.mark.parametrize(('epochs', 'val_check_interval', 'expected'), [(1, 1.0, 1), (2, 1.0, 2), (1, 0.25, 4), (2, 0.3, 6)])
def test_default_checkpoint_freq(save_mock, tmpdir, epochs: int, val_check_interval: float, expected: int):
    if False:
        return 10
    model = BoringModel()
    trainer = Trainer(default_root_dir=tmpdir, max_epochs=epochs, enable_model_summary=False, val_check_interval=val_check_interval, limit_val_batches=1, enable_progress_bar=False)
    trainer.fit(model)
    assert save_mock.call_count == expected

@mock.patch('torch.save')
@pytest.mark.parametrize(('k', 'epochs', 'val_check_interval', 'expected'), [(1, 1, 1.0, 1), (2, 2, 1.0, 2), (2, 1, 0.25, 4), (2, 2, 0.3, 6)])
...
def test_top_k(save_mock, tmpdir, k: int, epochs: int, val_check_interval: float, expected: int, save_last: bool):
    ...
    class TestModel(BoringModel):
        ...
        def training_step(self, batch, batch_idx):
            ...
            loss = loss / (loss + 1e-07)
            loss += self.last_coeff
            self.log('my_loss', loss)
            self.last_coeff *= 0.999
            return loss
    model = TestModel()
    trainer = Trainer(callbacks=[callbacks.ModelCheckpoint(dirpath=tmpdir, monitor='my_loss', save_top_k=k, save_last=save_last)], default_root_dir=tmpdir, max_epochs=epochs, enable_model_summary=False, val_check_interval=val_check_interval)
    trainer.fit(model)
    if save_last:
        expected = expected
    assert save_mock.call_count == expected
...
def test_top_k_ddp(save_mock, tmpdir, k, epochs, val_check_interval, expected):
    ...
    model = TestModel()
    trainer = Trainer(callbacks=[callbacks.ModelCheckpoint(dirpath=tmpdir, monitor='my_loss_step', save_top_k=k, mode='max')], default_root_dir=tmpdir, enable_progress_bar=False, max_epochs=epochs, enable_model_summary=False, val_check_interval=val_check_interval, strategy='ddp', accelerator='gpu', devices=2, limit_train_batches=64, limit_val_batches=32)
    trainer.fit(model)
