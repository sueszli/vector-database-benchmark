...
from aiortc.rtcrtpparameters import RTCRtpCapabilities, RTCRtpCodecCapability, RTCRtpCodecParameters, RTCRtpEncodingParameters, RTCRtpHeaderExtensionCapability, RTCRtpReceiveParameters, RTCRtpRtxParameters
from aiortc.rtcrtpreceiver import NackGenerator, RemoteStreamTrack, RTCRtpReceiver, RTCRtpSynchronizationSource, StreamStatistics, TimestampMapper
...
class RTCRtpReceiverTest(CodecTestCase):
    ...
    async def test_rtp_and_rtcp(self):
        async with create_receiver('audio') as receiver:
            ...
            self.assertEqual(sorted([s.type for s in report.values()]), ['inbound-rtp', 'remote-outbound-rtp', 'transport'])
    ...
    async def test_rtp_rtx(self):
        async with create_receiver('video') as receiver:
            receiver._track = RemoteStreamTrack(kind='video')
            await receiver.receive(RTCRtpReceiveParameters(codecs=[VP8_CODEC, RTCRtpCodecParameters(mimeType='video/rtx', clockRate=90000, payloadType=101, parameters={'apt': 100})], encodings=[RTCRtpEncodingParameters(ssrc=1234, payloadType=100, rtx=RTCRtpRtxParameters(ssrc=2345))]))
    ...
    async def test_rtp_rtx_unknown_ssrc(self):
        async with create_receiver('video') as receiver:
            receiver._track = RemoteStreamTrack(kind='video')
            await receiver.receive(RTCRtpReceiveParameters(codecs=[VP8_CODEC, RTCRtpCodecParameters(mimeType='video/rtx', clockRate=90000, payloadType=101, parameters={'apt': 100})]))
