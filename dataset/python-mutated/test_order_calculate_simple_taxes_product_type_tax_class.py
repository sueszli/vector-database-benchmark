...
from ...product.utils import create_category, create_product, create_product_channel_listing, create_product_type, create_product_variant, create_product_variant_channel_listing, update_product_type
...
def prepare_tax_configuration(e2e_staff_api_client, channel_slug, country_code, country_tax_rate, product_type_tax_rate, prices_entered_with_tax):
    ...
    tax_config_data = update_tax_configuration(e2e_staff_api_client, tax_config_id, charge_taxes=True, tax_calculation_strategy='FLAT_RATES', display_gross_prices=True, prices_entered_with_tax=True)
...
@pytest.mark.e2e
def test_order_calculate_simple_tax_based_on_product_type_tax_class_CORE_2004(e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes, permission_manage_shipping, permission_manage_taxes, permission_manage_orders):
    if False:
        print('Hello World!')
    ...
    (country_tax_rate, product_type_tax_rate, tax_class_id) = prepare_tax_configuration(e2e_staff_api_client, channel_slug, country_code='US', country_tax_rate=21, product_type_tax_rate=11, prices_entered_with_tax=True)
    variant_price = '4.22'
    ...
    calculated_tax = round(product_variant_price * product_type_tax_rate / (100 + product_type_tax_rate), 2)
    ...
    shipping_tax = round(shipping_price * country_tax_rate / (100 + country_tax_rate), 2)
