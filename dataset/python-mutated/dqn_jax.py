...
def parse_args():
    ...
    parser.add_argument('--target-network-frequency', type=int, default=500, help='the timesteps it takes to update the target network')
    parser.add_argument('--batch-size', type=int, default=128, help='the batch size of sample from the reply memory')
    ...
    parser.add_argument('--exploration-fraction', type=float, default=0.5, help='the fraction of `total-timesteps` it takes from start-e to go end-e')
...
if __name__ == '__main__':
    ...
    q_state = TrainState.create(apply_fn=q_network.apply, params=q_network.init(q_key, obs), target_params=q_network.init(q_key, obs), tx=optax.adam(learning_rate=args.learning_rate))
    q_network.apply = jax.jit(q_network.apply)
    q_state = q_state.replace(target_params=optax.incremental_update(q_state.params, q_state.target_params, 1))
    rb = ReplayBuffer(args.buffer_size, envs.single_observation_space, envs.single_action_space, 'cpu', handle_timeout_termination=False)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                (loss, old_val, q_state) = update(q_state, data.observations.numpy(), data.actions.numpy(), data.next_observations.numpy(), data.rewards.flatten().numpy(), data.dones.flatten().numpy())
