...
if __name__ == '__main__':
    ...
    option_parser = optparse.OptionParser(usage='Usage: %prog option [options]\n\nImport analysis for imports between SymPy modules.')
    option_group = optparse.OptionGroup(option_parser, 'Analysis options', 'Options that define what to do. Exactly one of these must be given.')
    option_group.add_option('--problems', help='Print all import problems, that is: If an import pulls in a package instead of a module (e.g. sympy.core instead of sympy.core.add); if it imports a symbol that is already present; if it imports a symbol from somewhere other than the defining module.', action='count')
    option_group.add_option('--origins', help='For each imported symbol in each module, print the module that defined it. (This is useful for import refactoring.)', action='count')
    option_parser.add_option_group(option_group)
    option_group = optparse.OptionGroup(option_parser, 'Sort options', 'These options define the sort order for output lines. At most one of these options is allowed. Unsorted output will reflect the order in which imports happened.')
