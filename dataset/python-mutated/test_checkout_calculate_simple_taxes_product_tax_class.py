...
@pytest.mark.e2e
def test_checkout_calculate_simple_tax_based_on_product_tax_class_CORE_2005(e2e_staff_api_client, e2e_not_logged_api_client, permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes, permission_manage_shipping, permission_manage_taxes):
    if False:
        return 10
    ...
    (country_tax_rate, product_tax_rate, tax_class_id) = prepare_tax_configuration(e2e_staff_api_client, channel_slug, country_code='US', country_tax_rate=23, product_tax_rate=5, prices_entered_with_tax=True)
    variant_price = '25.55'
    ...
    calculated_tax = round(product_variant_price * product_tax_rate / (100 + product_tax_rate), 2)
    assert checkout_data['totalPrice']['tax']['amount'] == calculated_tax
    assert checkout_data['totalPrice']['net']['amount'] == round(product_variant_price - calculated_tax, 2)
    ...
    assert checkout_data['shippingPrice']['net']['amount'] == round(shipping_price - shipping_tax, 2)
