...
class StateBackgroundUpdateStore(StateGroupBackgroundUpdateStore):
    ...
    CURRENT_STATE_EVENTS_STREAM_ORDERING_INDEX_UPDATE_NAME = 'current_state_events_stream_ordering_idx'
    ROOM_MEMBERSHIPS_STREAM_ORDERING_INDEX_UPDATE_NAME = 'room_memberships_stream_ordering_idx'
    LOCAL_CURRENT_MEMBERSHIP_STREAM_ORDERING_INDEX_UPDATE_NAME = 'local_current_membership_stream_ordering_idx'
    ...
    async def _background_deduplicate_state(self, progress: dict, batch_size: int) -> int:
        ...
        if max_group is None:
            rows = await self.db_pool.execute('_background_deduplicate_state', 'SELECT coalesce(max(id), 0) FROM state_groups')
            max_group = rows[0][0]
        ...
        (finished, result) = await self.db_pool.runInteraction(self.STATE_GROUP_DEDUPLICATION_UPDATE_NAME, reindex_txn)
        if finished:
            await self.db_pool.updates._end_background_update(self.STATE_GROUP_DEDUPLICATION_UPDATE_NAME)
    ...
    async def _background_index_state(self, progress: dict, batch_size: int) -> int:
        ...
        await self.db_pool.updates._end_background_update(self.STATE_GROUP_INDEX_UPDATE_NAME)
        return 1