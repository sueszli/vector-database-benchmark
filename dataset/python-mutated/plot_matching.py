...
coords_orig = corner_peaks(corner_harris(img_orig_gray), threshold_rel=0.001, min_distance=5)
coords_warped = corner_peaks(corner_harris(img_warped_gray), threshold_rel=0.001, min_distance=5)
...
print('Ground truth:')
print(f'Scale: ({tform.scale[1]:.4f}, {tform.scale[0]:.4f}), Translation: ({tform.translation[1]:.4f}, {tform.translation[0]:.4f}), Rotation: {-tform.rotation:.4f}')
print('Affine transform:')
print(f'Scale: ({model.scale[0]:.4f}, {model.scale[1]:.4f}), Translation: ({model.translation[0]:.4f}, {model.translation[1]:.4f}), Rotation: {model.rotation:.4f}')
print('RANSAC:')
print(f'Scale: ({model_robust.scale[0]:.4f}, {model_robust.scale[1]:.4f}), Translation: ({model_robust.translation[0]:.4f}, {model_robust.translation[1]:.4f}), Rotation: {model_robust.rotation:.4f}')
(fig, ax) = plt.subplots(nrows=2, ncols=1)
plt.gray()
inlier_idxs = np.nonzero(inliers)[0]
plot_matches(ax[0], img_orig_gray, img_warped_gray, src, dst, np.column_stack((inlier_idxs, inlier_idxs)), matches_color='b')
ax[0].axis('off')
ax[0].set_title('Correct correspondences')
outlier_idxs = np.nonzero(outliers)[0]
plot_matches(ax[1], img_orig_gray, img_warped_gray, src, dst, np.column_stack((outlier_idxs, outlier_idxs)), matches_color='r')
ax[1].axis('off')
