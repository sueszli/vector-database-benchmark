...
async def test_create_torrent(rest_api, tmp_path, download_manager):
    ...
    post_data = {'files': [str(torrent_path / 'video.avi'), str(torrent_path / 'video.avi.torrent')], 'description': 'Video of my cat', 'trackers': 'http://localhost/announce', 'name': 'test_torrent', 'export_dir': str(tmp_path)}
    response_dict = await do_request(rest_api, 'createtorrent?download=1', expected_code=200, request_type='POST', post_data=post_data)
    assert response_dict['torrent']
    assert start_download.call_args[1]['config'].get_hops() == DownloadDefaultsSettings().number_hops
...
async def test_create_torrent_io_error(rest_api, download_manager):
    ...
    error_response = await do_request(rest_api, 'createtorrent', expected_code=500, request_type='POST', post_data=post_data)
    expected_response = {'error': {'code': 'OSError', 'handled': True, 'message': 'test'}}
...
async def test_create_torrent_of_large_size(client_max_size, rest_api):
    ...
    """
    post_data = {'description': ''.join((random.choice(string.ascii_letters) for _ in range(client_max_size)))}
    error_response = await do_request(rest_api, 'createtorrent', expected_code=HTTP_REQUEST_ENTITY_TOO_LARGE, request_type='POST', post_data=post_data)
