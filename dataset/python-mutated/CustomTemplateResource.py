...
from mage_ai.data_preparation.models.custom_templates.constants import DIRECTORY_FOR_BLOCK_TEMPLATES, DIRECTORY_FOR_PIPELINE_TEMPLATES
...
from mage_ai.data_preparation.models.custom_templates.utils import flatten_files, get_templates, group_and_hydrate_files
...
class CustomTemplateResource(GenericResource):
    ...
    async def create(self, payload, user, **kwargs):
        ...
        if DIRECTORY_FOR_BLOCK_TEMPLATES == object_type:
            ...
            if not custom_template:
                custom_template = CustomBlockTemplate(**ignore_keys(payload, ['uuid', OBJECT_TYPE_KEY]))
                if user:
                    custom_template.user = dict(username=user.username)
                custom_template.content = fetch_template_source(custom_template.block_type, payload.get('config', {}), language=custom_template.language)
            ...
            if not custom_template:
                ...
                custom_template = CustomPipelineTemplate.create_from_pipeline(pipeline, template_uuid, name=payload.get('name'), description=payload.get('description'))
                if user:
                    custom_template.user = dict(username=user.username)
                custom_template.save()
        if custom_template:
    ...
    async def update(self, payload, **kwargs):
        ...
        if DIRECTORY_FOR_BLOCK_TEMPLATES == object_type:
            cache = await BlockActionObjectCache.initialize_cache()
            cache.update_custom_block_template(self.model, remove=True)
        for (key, value) in ignore_keys(payload, ['uuid', OBJECT_TYPE_KEY]).items():
