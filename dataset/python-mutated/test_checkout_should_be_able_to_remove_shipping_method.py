...
@pytest.mark.e2e
def test_checkout_should_be_able_to_remove_shipping_method_CORE_0115(e2e_staff_api_client, e2e_not_logged_api_client, permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes, permission_manage_shipping, permission_manage_orders, permission_manage_checkouts):
    if False:
        for i in range(10):
            print('nop')
    ...
    checkout_data = checkout_delivery_method_update(e2e_not_logged_api_client, checkout_id)
    total_gross_amount = checkout_data['totalPrice']['gross']['amount']
