...
def make_window(loc):
    ...
    layout = [[sg.T(title, text_color='black', background_color='#FFFF88', k='-TITLE-')], [sg.ML(text, size=(30, 5), background_color='#FFFF88', no_scrollbar=True, k='-ML-', border_width=0, expand_y=True, expand_x=True, font=text_font), sg.Sizegrip(background_color='#FFFF88')]]
    window = sg.Window('Postit', layout, no_titlebar=True, grab_anywhere=True, margins=(0, 0), background_color='#FFFF88', element_padding=(0, 0), location=loc, right_click_menu=[[''], ['Edit Me', 'Change Font', 'Alpha', [str(x) for x in range(1, 11)], 'Choose Title', 'Exit']], keep_on_top=True, font='_ 20', right_click_menu_font=text_font, resizable=True, finalize=True, alpha_channel=alpha)
