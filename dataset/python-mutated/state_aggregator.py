...
from ray.util.state.common import protobuf_message_to_dict, ActorState, JobState, ListApiOptions, ListApiResponse, NodeState, ObjectState, PlacementGroupState, RuntimeEnvState, SummaryApiResponse, RAY_MAX_LIMIT_FROM_API_SERVER, SummaryApiOptions, TaskSummaries, StateSchema, SupportedFilterType, TaskState, WorkerState, StateSummary, ActorSummaries, ObjectSummaries, ClusterEventState, filter_fields, PredicateType, protobuf_to_task_state_dict
...
GCS_QUERY_FAILURE_WARNING = "Failed to query data from GCS. It is due to (1) GCS is unexpectedly failed. (2) GCS is overloaded. (3) There's an unexpected network issue. Please check the gcs_server.out log to find the root cause."
NODE_QUERY_FAILURE_WARNING = "Failed to query data from {type}. Queried {total} {type} and {network_failures} {type} failed to reply. It is due to (1) {type} is unexpectedly failed. (2) {type} is overloaded. (3) There's an unexpected network issue. Please check the {log_command} to find the root cause."
...
class StateAPIManager:
    ...
    async def list_actors(self, *, option: ListApiOptions) -> ListApiResponse:
        ...
        """
        try:
            reply = await self._client.get_all_actor_info(timeout=option.timeout, filters=option.filters)
        except DataSourceUnavailable:
            raise DataSourceUnavailable(GCS_QUERY_FAILURE_WARNING)
        result = []
        for message in reply.actor_table_data:
            data = protobuf_message_to_dict(message=message, fields_to_decode=['actor_id', 'owner_id', 'job_id', 'node_id', 'placement_group_id'])
            result.append(data)
        ...
        return ListApiResponse(result=result, total=reply.total, num_after_truncation=num_after_truncation, num_filtered=num_filtered)
    ...
    async def list_placement_groups(self, *, option: ListApiOptions) -> ListApiResponse:
        ...
        for message in reply.placement_group_table_data:
            data = protobuf_message_to_dict(message=message, fields_to_decode=['placement_group_id', 'creator_job_id', 'node_id'])
            result.append(data)
        num_after_truncation = len(result)
        result = self._filter(result, option.filters, PlacementGroupState, option.detail)
        num_filtered = len(result)
        result.sort(key=lambda entry: entry['placement_group_id'])
        return ListApiResponse(result=list(islice(result, option.limit)), total=reply.total, num_after_truncation=num_after_truncation, num_filtered=num_filtered)
    ...
    async def list_nodes(self, *, option: ListApiOptions) -> ListApiResponse:
        ...
        for message in reply.node_info_list:
            data = protobuf_message_to_dict(message=message, fields_to_decode=['node_id'])
        ...
        result = list(islice(result, option.limit))
        return ListApiResponse(result=result, total=total_nodes, num_after_truncation=num_after_truncation, num_filtered=num_filtered)
    ...
    async def list_workers(self, *, option: ListApiOptions) -> ListApiResponse:
        ...
        for message in reply.worker_table_data:
            data = protobuf_message_to_dict(message=message, fields_to_decode=['worker_id', 'raylet_id'])
    ...
    async def list_jobs(self, *, option: ListApiOptions) -> ListApiResponse:
        try:
            ...
            raise DataSourceUnavailable(GCS_QUERY_FAILURE_WARNING)
        return ListApiResponse(result=result, total=total, num_after_truncation=total, num_filtered=num_filtered)
    ...
    async def list_tasks(self, *, option: ListApiOptions) -> ListApiResponse:
        ...
        """
        try:
            reply = await self._client.get_all_task_info(timeout=option.timeout, filters=option.filters, exclude_driver=option.exclude_driver)
        except DataSourceUnavailable:
            raise DataSourceUnavailable(GCS_QUERY_FAILURE_WARNING)
        ...
        result = list(islice(result, option.limit))
        return ListApiResponse(result=result, total=num_total, num_after_truncation=num_after_truncation, num_filtered=num_filtered)
    ...
    async def list_objects(self, *, option: ListApiOptions) -> ListApiResponse:
        ...
        replies = await asyncio.gather(*[self._client.get_object_info(node_id, timeout=option.timeout) for node_id in raylet_ids], return_exceptions=True)
        ...
        for (reply, _) in zip(replies, raylet_ids):
            ...
            for core_worker_stat in reply.core_workers_stats:
                worker_stats.append(protobuf_message_to_dict(message=core_worker_stat, fields_to_decode=['object_id'], preserving_proto_field_name=False))
        partial_failure_warning = None
        if len(raylet_ids) > 0 and unresponsive_nodes > 0:
            warning_msg = NODE_QUERY_FAILURE_WARNING.format(type='raylet', total=len(raylet_ids), network_failures=unresponsive_nodes, log_command='raylet.out')
            if unresponsive_nodes == len(raylet_ids):
                raise DataSourceUnavailable(warning_msg)
            partial_failure_warning = f'The returned data may contain incomplete result. {warning_msg}'
        result = []
        ...
        for entry in memory_table.table:
            ...
            data['task_status'] = 'NIL' if data['task_status'] == '-' else data['task_status']
            result.append(data)
        callsite_warning = []
        callsite_enabled = env_integer('RAY_record_ref_creation_sites', 0)
        if not callsite_enabled:
            callsite_warning.append('Callsite is not being recorded. To record callsite information for each ObjectRef created, set env variable RAY_record_ref_creation_sites=1 during `ray start` and `ray.init`.')
        num_after_truncation = len(result)
        result = self._filter(result, option.filters, ObjectState, option.detail)
        num_filtered = len(result)
        result.sort(key=lambda entry: entry['object_id'])
        result = list(islice(result, option.limit))
        return ListApiResponse(result=result, partial_failure_warning=partial_failure_warning, total=total_objects, num_after_truncation=num_after_truncation, num_filtered=num_filtered, warnings=callsite_warning)
    ...
    async def list_runtime_envs(self, *, option: ListApiOptions) -> ListApiResponse:
        ...
        replies = await asyncio.gather(*[self._client.get_runtime_envs_info(node_id, timeout=option.timeout) for node_id in agent_ids], return_exceptions=True)
        result = []
        unresponsive_nodes = 0
        total_runtime_envs = 0
        for (node_id, reply) in zip(self._client.get_all_registered_runtime_env_agent_ids(), replies):
            if isinstance(reply, DataSourceUnavailable):
                unresponsive_nodes += 1
                continue
            elif isinstance(reply, Exception):
            ...
            for state in states:
                ...
                data['runtime_env'] = RuntimeEnv.deserialize(data['runtime_env']).to_dict()
                data['node_id'] = node_id
                result.append(data)
        partial_failure_warning = None
        if len(agent_ids) > 0 and unresponsive_nodes > 0:
            warning_msg = NODE_QUERY_FAILURE_WARNING.format(type='agent', total=len(agent_ids), network_failures=unresponsive_nodes, log_command='dashboard_agent.log')
        ...
        result = list(islice(result, option.limit))
        return ListApiResponse(result=result, partial_failure_warning=partial_failure_warning, total=total_runtime_envs, num_after_truncation=num_after_truncation, num_filtered=num_filtered)
    ...
    async def list_cluster_events(self, *, option: ListApiOptions) -> ListApiResponse:
        ...
        num_filtered = len(result)
        result = list(islice(result, option.limit))
        return ListApiResponse(result=result, total=total, num_after_truncation=num_after_truncation, num_filtered=num_filtered)
    ...
    async def summarize_tasks(self, option: SummaryApiOptions) -> SummaryApiResponse:
        ...
        result = await self.list_tasks(option=ListApiOptions(timeout=option.timeout, limit=RAY_MAX_LIMIT_FROM_API_SERVER, filters=option.filters, detail=summary_by == 'lineage'))
        if summary_by == 'func_name':
            summary_results = TaskSummaries.to_summary_by_func_name(tasks=result.result)
        else:
            actors = await self.list_actors(option=ListApiOptions(timeout=option.timeout, limit=RAY_MAX_LIMIT_FROM_API_SERVER, detail=True))
            summary_results = TaskSummaries.to_summary_by_lineage(tasks=result.result, actors=actors.result)
        summary = StateSummary(node_id_to_summary={'cluster': summary_results})
        warnings = result.warnings
        if summary_results.total_actor_scheduled + summary_results.total_actor_tasks + summary_results.total_tasks < result.num_filtered:
            warnings = warnings or []
            warnings.append('There is missing data in this aggregation. Possibly due to task data being evicted to preserve memory.')
        return SummaryApiResponse(total=result.total, result=summary, partial_failure_warning=result.partial_failure_warning, warnings=warnings, num_after_truncation=result.num_after_truncation, num_filtered=result.num_filtered)

    async def summarize_actors(self, option: SummaryApiOptions) -> SummaryApiResponse:
        result = await self.list_actors(option=ListApiOptions(timeout=option.timeout, limit=RAY_MAX_LIMIT_FROM_API_SERVER, filters=option.filters))
        summary = StateSummary(node_id_to_summary={'cluster': ActorSummaries.to_summary(actors=result.result)})
        return SummaryApiResponse(total=result.total, result=summary, partial_failure_warning=result.partial_failure_warning, warnings=result.warnings, num_after_truncation=result.num_after_truncation, num_filtered=result.num_filtered)

    async def summarize_objects(self, option: SummaryApiOptions) -> SummaryApiResponse:
        result = await self.list_objects(option=ListApiOptions(timeout=option.timeout, limit=RAY_MAX_LIMIT_FROM_API_SERVER, filters=option.filters))
        summary = StateSummary(node_id_to_summary={'cluster': ObjectSummaries.to_summary(objects=result.result)})
        return SummaryApiResponse(total=result.total, result=summary, partial_failure_warning=result.partial_failure_warning, warnings=result.warnings, num_after_truncation=result.num_after_truncation, num_filtered=result.num_filtered)

    async def generate_task_timeline(self, job_id: Optional[str]) -> List[dict]:
        filters = [('job_id', '=', job_id)] if job_id else None
        result = await self.list_tasks(option=ListApiOptions(detail=True, filters=filters, limit=10000))
