...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', default='snapshot', type=str, help='snapshot directory name (default: snapshot)')
    ...
    if args.cluster_mode == 'local':
        ...
        sc = init_orca_context('yarn-client', cores=args.executor_cores, num_nodes=args.num_executor, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, conf=spark_conf, object_store_memory='40g', init_ray_on_spark=True, extra_python_lib='model.py,evaluation.py')
    elif args.cluster_mode == 'spark-submit':
        sc = init_orca_context('spark-submit', object_store_memory='40g')
    else:
        invalidInputError(False, "cluster_mode should be one of 'local', 'yarn' and 'spark-submit', but got " + args.cluster_mode)
    num_cols = ['enaging_user_follower_count', 'enaging_user_following_count', 'engaged_with_user_follower_count', 'engaged_with_user_following_count', 'len_hashtags', 'len_domains', 'len_links']
    cat_cols = ['engaged_with_user_is_verified', 'enaging_user_is_verified', 'present_media', 'tweet_type', 'language', 'present_media_language']
    embed_cols = ['enaging_user_id', 'engaged_with_user_id', 'hashtags', 'present_links', 'present_domains']
    ...
    test_user_ids = test.select('engaged_with_user_id').cast('engaged_with_user_id', 'str').to_list('engaged_with_user_id')
    ...
    train = train.reindex(embed_cols, reindex_tbls).transform_min_max_scale(num_cols, min_max_dict).merge_cols(all_features, 'feature').select(['label', 'feature']).apply('label', 'label', lambda x: [float(x)], dtype='array<float>')
    test = test.reindex(embed_cols, reindex_tbls).transform_min_max_scale(num_cols, min_max_dict).merge_cols(all_features, 'feature').select(['label', 'feature']).apply('label', 'label', lambda x: [float(x)], dtype='array<float>')
    test.cache()
    ...
    est = Estimator.from_torch(model=model_creator, optimizer=optim_creator, loss=criterion, metrics=[Accuracy(), AUC()], use_tqdm=True, backend='ray', config=config)
    train_stats = est.fit(data=train.df, feature_cols=['feature'], label_cols=['label'], epochs=args.epochs, batch_size=args.batch_size)
    valid_stats = est.evaluate(data=test.df, feature_cols=['feature'], label_cols=['label'], batch_size=args.batch_size)
