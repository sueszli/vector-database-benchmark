...
for i in range(len(x1)):
    plt.annotate('%d%%' % y1[i], (x1[i], y1[i]), xytext=(0, 1), fontsize='x-small', color=color, textcoords='offset points', va='bottom', ha='center')
ax.bar(x2, y2, color=color, hatch='/')
for i in range(len(x2)):
    plt.annotate('%d%%' % y2[i], (x2[i], y2[i]), xytext=(0, 1), fontsize='x-small', color=color, textcoords='offset points', va='bottom', ha='center')
ax.set_yticks([])
