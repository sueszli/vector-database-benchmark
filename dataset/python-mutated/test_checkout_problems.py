...
def test_checkout_with_out_of_stock_when_stocks_dont_exist(api_client, checkout_with_items_and_shipping):
    if False:
        print('Hello World!')
    checkout = checkout_with_items_and_shipping
    checkout_id = to_global_id_or_none(checkout)
...
def test_checkout_problem_out_of_stock_without_available_quantity(api_client, checkout_with_items_and_shipping):
    if False:
        for i in range(10):
            print('nop')
    checkout = checkout_with_items_and_shipping
...
def test_checkout_problems_with_reservation(api_client, checkout_with_items):
    ...
    Reservation.objects.create(checkout_line=checkout_line, stock=stock, quantity_reserved=checkout_line.quantity, reserved_until=reserved_until)
    variables = {'id': checkout_id, 'channel': checkout.channel.slug}
    response = api_client.post_graphql(QUERY_CHECKOUT_WITH_PROBLEMS, variables)
    content = get_graphql_content(response)
    assert content['data']['checkout']['id'] == checkout_id
...
def test_checkout_with_out_of_stock_with_allocations_and_not_enough_items(api_client, checkout_with_items_and_shipping, order_line):
    if False:
        print('Hello World!')
    checkout = checkout_with_items_and_shipping
    checkout_id = to_global_id_or_none(checkout)
    ...
    Allocation.objects.create(order_line=order_line, stock=stock, quantity_allocated=line_quantity)
    variables = {'id': checkout_id, 'channel': checkout.channel.slug}
    response = api_client.post_graphql(QUERY_CHECKOUT_WITH_PROBLEMS, variables)
    content = get_graphql_content(response)
    assert content['data']['checkout']['id'] == checkout_id
    assert len(content['data']['checkout']['problems']) == 1
    assert content['data']['checkout']['problems'][0]['__typename'] == 'CheckoutLineProblemInsufficientStock'
    assert content['data']['checkout']['problems'][0]['availableQuantity'] == free_quantity
    assert content['data']['checkout']['problems'][0]['line']['id'] == to_global_id_or_none(checkout_line)
    assert content['data']['checkout']['problems'][0]['variant']['id'] == to_global_id_or_none(checkout_line.variant)

def test_checkout_out_of_stock_without_tracking_inventory(api_client, checkout_with_items_and_shipping):
    if False:
        print('Hello World!')
    checkout = checkout_with_items_and_shipping
    checkout_id = to_global_id_or_none(checkout)
...
def test_checkout_with_multiple_same_variant_and_out_of_stock(api_client, checkout_with_items_and_shipping):
    if False:
        print('Hello World!')
    checkout = checkout_with_items_and_shipping
    checkout_id = to_global_id_or_none(checkout)
...
def test_checkout_problems_when_product_is_not_published(checkout_with_items_and_shipping, api_client):
    if False:
        for i in range(10):
            print('nop')
    checkout = checkout_with_items_and_shipping
...
def test_checkout_problems_when_product_doesnt_have_channel_listing(checkout_with_items_and_shipping, api_client):
    if False:
        while True:
            i = 10
    checkout = checkout_with_items_and_shipping
...
def test_checkout_problems_when_product_is_not_available_to_purchase(checkout_with_items_and_shipping, api_client):
    if False:
        print('Hello World!')
    checkout = checkout_with_items_and_shipping
    checkout_line = checkout.lines.first()
...
def test_checkout_problems_when_product_variant_doesnt_have_channel_listing(checkout_with_items_and_shipping, api_client):
    if False:
        for i in range(10):
            print('nop')
    checkout = checkout_with_items_and_shipping
...
def test_checkout_problems_when_variant_channel_listing_doesnt_have_price_amount(checkout_with_items_and_shipping, api_client):
    if False:
        for i in range(10):
            print('nop')
    checkout = checkout_with_items_and_shipping
