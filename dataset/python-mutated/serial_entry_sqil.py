...
def serial_pipeline_sqil(input_cfg: Union[str, Tuple[dict, dict]], expert_cfg: Union[str, Tuple[dict, dict]], seed: int=0, env_setting: Optional[List[Any]]=None, model: Optional[torch.nn.Module]=None, expert_model: Optional[torch.nn.Module]=None, max_train_iter: Optional[int]=int(10000000000.0), max_env_step: Optional[int]=int(10000000000.0)) -> 'Policy':
    ...
    if cfg.policy.get('expert_random_collect_size', 0) > 0:
        random_collect(expert_cfg.policy, expert_policy, expert_collector, expert_collector_env, expert_commander, expert_buffer)
    while True:
        ...
        expert_data = expert_collector.collect(train_iter=learner.train_iter, policy_kwargs=expert_collect_kwargs)
