...
async def get_retry_limiter(destination: str, clock: Clock, store: DataStore, ignore_backoff: bool=False, **kwargs: Any) -> 'RetryDestinationLimiter':
    ...
    if retry_timings:
        ...
        if not ignore_backoff and retry_last_ts + retry_interval > now:
            raise NotRetryingDestination(retry_last_ts=retry_last_ts, retry_interval=retry_interval, destination=destination)
    backoff_on_failure = not ignore_backoff
    return RetryDestinationLimiter(destination, clock, store, failure_ts, retry_interval, backoff_on_failure=backoff_on_failure, **kwargs)

async def filter_destinations_by_retry_limiter(destinations: StrCollection, clock: Clock, store: DataStore, retry_due_within_ms: int=0) -> StrCollection:
    ...
    now = int(clock.time_msec())
    return [destination for (destination, timings) in retry_timings.items() if timings is None or timings.retry_last_ts + timings.retry_interval <= now + retry_due_within_ms]
