...
class ManagedStream(ManagedDownloadSource):
    ...
    async def start(self, timeout: Optional[float]=None, save_now: bool=False):
        ...
        if not await self.blob_manager.storage.file_exists(self.sd_hash):
            if save_now:
                if not self._file_name:
                    self._file_name = await get_next_available_file_name(self.loop, self.download_directory, self._file_name or sanitize_file_name(self.suggested_file_name))
            ...
            self.rowid = await self.blob_manager.storage.save_downloaded_file(self.stream_hash, file_name, download_dir, 0.0, added_on=self._added_on)
    ...
    async def _aiter_read_stream(self, start_blob_num: Optional[int]=0, connection_id: int=0) -> typing.AsyncIterator[typing.Tuple['BlobInfo', bytes]]:
    ...
    async def stream_file(self, request: Request) -> StreamResponse:
        log.info('stream file to browser for lbry://%s#%s (sd hash %s...)', self.claim_name, self.claim_id, self.sd_hash[:6])
        (headers, size, skip_blobs, first_blob_start_offset) = self._prepare_range_response_headers(request.headers.get('range', 'bytes=0-'))
        await self.start()
        ...
        try:
            async for (blob_info, decrypted) in self._aiter_read_stream(skip_blobs, connection_id=self.STREAMING_ID):
                ...
                if blob_info.blob_num == len(self.descriptor.blobs) - 2 or len(decrypted) + wrote >= size:
                    ...
                    log.debug('sending browser final blob (%i/%i)', blob_info.blob_num + 1, len(self.descriptor.blobs) - 1)
                ...
                wrote += len(decrypted)
                log.info('sent browser %sblob %i/%i', '(final) ' if response._eof_sent else '', blob_info.blob_num + 1, len(self.descriptor.blobs) - 1)
    ...
    async def _save_file(self, output_path: str):
        log.info('save file for lbry://%s#%s (sd hash %s...) -> %s', self.claim_name, self.claim_id, self.sd_hash[:6], output_path)
        self.saving.set()
        ...
        try:
            ...
            if self.analytics_manager:
                self.loop.create_task(self.analytics_manager.send_download_finished(self.download_id, self.claim_name, self.sd_hash))
            self.finished_writing.set()
            log.info('finished saving file for lbry://%s#%s (sd hash %s...) -> %s', self.claim_name, self.claim_id, self.sd_hash[:6], self.full_path)
            ...
            if isinstance(err, asyncio.TimeoutError):
                ...
                await self.blob_manager.storage.change_file_download_dir_and_file_name(self.stream_hash, None, None)
    ...
    async def save_file(self, file_name: Optional[str]=None, download_directory: Optional[str]=None):
        ...
        self._file_name = await get_next_available_file_name(self.loop, self.download_directory, file_name or self._file_name or sanitize_file_name(self.suggested_file_name))
        await self.blob_manager.storage.change_file_download_dir_and_file_name(self.stream_hash, self.download_directory, self.file_name)
    ...
    async def upload_to_reflector(self, host: str, port: int) -> typing.List[str]:
        ...
        try:
            ...
            log.info('we have %i/%i needed blobs needed by reflector for lbry://%s#%s', len(we_have), len(needed), self.claim_name, self.claim_id)
            ...
            if isinstance(err, asyncio.CancelledError):
                ...
                log.warning('stopped uploading %s#%s to reflector because blobs were deleted or moved', self.claim_name, self.claim_id)
            else:
    ...
    async def _delayed_stop(self):
        ...
        while self._running.is_set():
            if self.saving.is_set() or self.streaming.is_set():
                ...
                stalled_count += 1
            if stalled_count > 1:
                log.info('stopping inactive download for lbry://%s#%s (%s...)', self.claim_name, self.claim_id, self.sd_hash[:6])
                await self.stop()
                return
