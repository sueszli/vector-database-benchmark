...
class Opchatgpts(AsyncGeneratorProvider):
    ...
    async def create_async_generator(cls, model: str, messages: Messages, proxy: str=None, **kwargs) -> AsyncResult:
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36', 'Accept': '*/*', 'Accept-Language': 'de,en-US;q=0.7,en;q=0.3', 'Origin': cls.url, 'Alt-Used': 'opchatgpts.net', 'Referer': f'{cls.url}/chatgpt-free-use/', 'Sec-Fetch-Dest': 'empty', 'Sec-Fetch-Mode': 'cors', 'Sec-Fetch-Site': 'same-origin'}
        async with ClientSession(headers=headers) as session:
            data = {'botId': 'default', 'chatId': random_string(), 'contextId': 28, 'customId': None, 'messages': messages, 'newMessage': messages[-1]['content'], 'session': 'N/A', 'stream': True}
