...
async def test_all_initially_enabled() -> None:
    ...
    async with DisableApp().run_test() as pilot:
        assert all((not node.disabled for node in pilot.app.screen.query('#test-container > *')))

async def test_enabled_widgets_have_enabled_pseudo_class() -> None:
    """All enabled widgets should have the :enabled pseudoclass."""
    async with DisableApp().run_test() as pilot:
        assert all((node.has_pseudo_class('enabled') and (not node.has_pseudo_class('disabled')) for node in pilot.app.screen.query('#test-container > *')))
...
async def test_all_individually_disabled() -> None:
    ...
    async with DisableApp().run_test() as pilot:
        for node in pilot.app.screen.query('VerticalScroll > *'):
            node.disabled = True
        assert all((node.disabled for node in pilot.app.screen.query('#test-container > *')))
...
async def test_disabled_widgets_have_disabled_pseudo_class() -> None:
    ...
    async with DisableApp().run_test() as pilot:
        for node in pilot.app.screen.query('#test-container > *'):
            node.disabled = True
        assert all((node.has_pseudo_class('disabled') and (not node.has_pseudo_class('enabled')) for node in pilot.app.screen.query('#test-container > *')))
...
@pytest.mark.parametrize('widget', [Button, Checkbox, DataTable, DirectoryTree, Input, ListView, OptionList, RadioSet, Select, Switch])
