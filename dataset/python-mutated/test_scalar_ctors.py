...
import pytest
from torch.testing._internal.common_utils import instantiate_parametrized_tests, parametrize, run_tests, subtest, TEST_WITH_TORCHDYNAMO, TestCase, xpassIfTorchDynamo
...
int_types = [subtest(np.byte, name='np_byte'), subtest(np.short, name='np_short'), subtest(np.intc, name='np_intc'), subtest(np.int_, name='np_int_'), subtest(np.longlong, name='np_longlong')]
uint_types = [np.ubyte]
