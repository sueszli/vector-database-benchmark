...
def testGLMGaussianScoringHistory():
    if False:
        return 10
    col_list_compare = ['iterations', 'objective', 'negative_log_likelihood', 'training_rmse', 'validation_rmse', 'training_mae', 'validation_mae', 'training_deviance', 'validation_deviance', 'deviance_train', 'deviance_test']
    ...
    model_cv = glm(family='gaussian', score_each_iteration=True, nfolds=3, fold_assignment='modulo', seed=1234, generate_scoring_history=True)
    model_cv.train(x=myX, y=myY, training_frame=training_data, validation_frame=test_data)
    model_score_each_cv = glm(family='gaussian', score_iteration_interval=1, nfolds=3, fold_assignment='modulo', seed=1234, generate_scoring_history=True)
    ...
    model_cv_4th = glm(family='gaussian', score_iteration_interval=4, nfolds=3, fold_assignment='modulo', seed=1234, generate_scoring_history=True)
