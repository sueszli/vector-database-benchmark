...
CALLBACK_DUMP_FILE = os.environ.get('CALLBACK_DUMP_FILE', '/tmp/callback_dump_file.json')
...
if __name__ == '__main__':
    ...
    ray.init()
    run_config = train.RunConfig(storage_path=STORAGE_PATH, name=EXP_NAME, checkpoint_config=train.CheckpointConfig(num_to_keep=1), callbacks=[StatefulCallback()])
    if RUNNER_TYPE == 'tuner':
        tuner(experiment_path, run_config)
    elif RUNNER_TYPE == 'trainer':
        trainer(experiment_path, run_config)
    else:
        raise NotImplementedError("`RUNNER_TYPE` environment var must be one of ['tuner', 'trainer']")