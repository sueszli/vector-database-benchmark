...
from mage_ai.orchestration.db.models.oauth import Permission, Role, RolePermission, UserRole
...
for method_name in dir(Base):
    ...
    setattr(BaseOperationsWithUserAuthenticationAndPermissionsTest, method_name.replace('run_mixin_', ''), _test)
    if method_name.startswith('run_mixin_test_execute_'):
        ...
        setattr(BaseOperationsWithUserAuthenticationAndPermissionsTest, method_name.replace('run_mixin_', '') + '_with_parent', _test_with_parent)