...
class Authenticator(dns_common_lexicon.LexiconDNSAuthenticator):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._add_provider_option('email', 'email address associated with LuaDNS account', 'auth_username')
        self._add_provider_option('token', f'API token for LuaDNS account, obtained from {ACCOUNT_URL}', 'auth_token')
    ...
    def more_info(self) -> str:
        if False:
            i = 10
            return i + 15
        return 'This plugin configures a DNS TXT record to respond to a dns-01 challenge using ' + 'the LuaDNS API.'
