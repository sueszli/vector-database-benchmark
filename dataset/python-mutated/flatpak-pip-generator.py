...
parser.add_argument('--cleanup', choices=['scripts', 'all'], help='Select what to clean up after build')
parser.add_argument('--requirements-file', '-r', help='Specify requirements.txt file')
parser.add_argument('--build-only', action='store_const', dest='cleanup', const='all', help='Clean up all files after build')
parser.add_argument('--build-isolation', action='store_true', default=False, help="Do not disable build isolation. Mostly useful on pip that does't support the feature.")
parser.add_argument('--ignore-installed', type=lambda s: s.split(','), default='', help='Comma-separated list of package names for which pip should ignore already installed packages. Useful when the package is installed in the SDK but not in the runtime.')
parser.add_argument('--checker-data', action='store_true', help='Include x-checker-data in output for the "Flatpak External Data Checker"')
parser.add_argument('--output', '-o', help='Specify output file name')
parser.add_argument('--runtime', help='Specify a flatpak to run pip inside of a sandbox, ensures python version compatibility')
...
if opts.python2:
    ...
    pip_executable = 'pip3'
if opts.runtime:
    flatpak_cmd = ['flatpak', '--devel', '--share=network', '--filesystem=/tmp', '--command={}'.format(pip_executable), 'run', opts.runtime]
    if opts.requirements_file:
        requirements_file = os.path.expanduser(opts.requirements_file)
...
if opts.output:
    ...
    output_package = 'python{}-{}'.format(python_version, os.path.basename(opts.requirements_file).replace('.txt', ''))
elif len(packages) == 1:
    output_package = 'python{}-{}'.format(python_version, packages[0].name)
else:
...
with tempfile.TemporaryDirectory(prefix=tempdir_prefix) as tempdir:
    pip_download = flatpak_cmd + ['download', '--exists-action=i', '--dest', tempdir, '-r', requirements_file]
    if use_hash:
                ...
                for f in files[name]:
                    if not f.endswith('.zip'):
                        try:
                            ...
                            pass
    vcs_packages = {x.name: {'vcs': x.vcs, 'revision': x.revision, 'uri': x.uri} for x in packages if x.vcs}
...
for package in packages:
    ...
    with tempfile.TemporaryDirectory(prefix='{}-{}'.format(tempdir_prefix, package.name)) as tempdir:
        pip_download = flatpak_cmd + ['download', '--exists-action=i', '--dest', tempdir]
        try:
    ...
    pip_command = [pip_executable, 'install', '--verbose', '--exists-action=i', '--no-index', '--find-links="file://${PWD}"', '--prefix=${FLATPAK_DEST}', '"{}"'.format(name_for_pip)]
    if package.name in opts.ignore_installed:
        pip_command.append('--ignore-installed')
    if package.name.casefold() in system_packages:
        print(f'{package.name} is in system_packages, adding --ignore-installed')
        pip_command.append('--ignore-installed')
    if not opts.build_isolation:
        pip_command.append('--no-build-isolation')
    module = OrderedDict([('name', module_name), ('buildsystem', 'simple'), ('build-commands', [' '.join(pip_command)]), ('sources', package_sources)])
    if opts.cleanup == 'all':
        module['cleanup'] = ['*']
...
if len(modules) == 1:
    ...
    pypi_module = {'name': output_package, 'buildsystem': 'simple', 'build-commands': [], 'modules': modules}
print()
