...
async def find_local_dependencies_in_setup_py(python_package: Container) -> List[str]:
    ...
    for dependency_line in dependency_in_requires_txt:
        if 'file://' in dependency_line:
            match = re.search('file:///(.+)', dependency_line)
            if match:
...
async def find_local_python_dependencies(context: PipelineContext, package_source_code_path: str, search_dependencies_in_setup_py: bool=True, search_dependencies_in_requirements_txt: bool=True) -> List[str]:
...
async def with_installed_python_package(context: PipelineContext, python_environment: Container, package_source_code_path: str, additional_dependency_groups: Optional[List]=None, exclude: Optional[List]=None, include: Optional[List]=None) -> Container:
...
async def apply_python_development_overrides(context: ConnectorContext, connector_container: Container) -> Container:
    if context.use_local_cdk:
        ...
        connector_container = connector_container.with_mounted_directory(f'/{path_to_cdk}', directory_to_mount).with_exec(['pip', 'install', '--no-deps', f'/{path_to_cdk}'], skip_entrypoint=True)
    return connector_container

async def with_python_connector_installed(context: ConnectorContext, python_container: Container, connector_source_path: str, additional_dependency_groups: Optional[List]=None, exclude: Optional[List]=None, include: Optional[List]=None) -> Container:
    """Install an airbyte python connectors  dependencies."""
    await hacks.cache_latest_cdk(context)
    container = await with_installed_python_package(context, python_container, connector_source_path, additional_dependency_groups=additional_dependency_groups, exclude=exclude, include=include)
