...
class ProjectMemberIndexEndpoint(ProjectEndpoint):
    ...
    def get(self, request: Request, project) -> Response:
        if False:
            for i in range(10):
                print('nop')
        queryset = OrganizationMember.objects.filter(Q(user_is_active=True, user_id__isnull=False) | Q(user_id__isnull=True), organization=project.organization, teams__in=project.teams.all()).distinct()
