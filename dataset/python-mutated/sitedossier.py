...
class SearchSitedossier:
    ...
    async def do_search(self):
        ...
        bot_string = 'Our web servers have detected unusual or excessive requests from your computer or network. Please enter the unique "word" below to confirm that you are a human interactively using this site.'
        if (stop_conditions[0] not in base_response and stop_conditions[1] not in base_response) and bot_string not in base_response:
            ...
            for i in range(101, total_number, 100):
                ...
                if bot_string in response:
                    ...
                    response = await AsyncFetcher.fetch_all([iter_url], headers={'User-Agent': Core.get_user_agent()}, proxy=self.proxy)
                    response = response[0]
                    if bot_string in response:
                        new_sleep_time = get_delay() * 30 * get_delay()
                        print(f'Still triggering a captcha, sleeping longer for: {new_sleep_time} and skipping this batch: {iter_url}')
                        await asyncio.sleep(new_sleep_time)
                        flagged_counter += 1
