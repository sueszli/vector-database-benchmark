...
if HAS_TABULATE:
    ...
    def bench_all(model: Union[torch.nn.Module, Callable], sample_input: Union[torch.Tensor, Any], num_iters: int=5, optimizer: Optional[torch.optim.Optimizer]=None, loss_fn: Union[torch.nn.Module, Callable, None]=None):
        if False:
            return 10
            ...
            if backend == 'inductor':
                ...
                for mode in mode_options:
                    ...
                    try:
                        ...
                        (compilation_time, running_time) = benchmark_compile(model, sample_input, num_iters, backend, mode, optimizer, loss_fn)
                    finally:
                        if torch.cuda.is_available():
                            _disable_tensor_cores()
                            table.append(['Training' if optimizer else 'Inference', backend if backend else '-', mode if mode is not None else '-', f'{compilation_time} ms ' if compilation_time else '-', f'{running_time} ms ' if running_time else '-'])
            else:
                torch._dynamo.reset()
                (compilation_time, running_time) = benchmark_compile(model, sample_input, num_iters, backend, None, optimizer, loss_fn)
                if running_time is not None:
                    table.append(['Training' if optimizer else 'Inference', backend, '-', f'{compilation_time} ms ' or '-', f'{running_time} ms '])
