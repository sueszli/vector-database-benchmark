...
class Test_kms_cmk_rotation_enabled:
    ...
    def test_kms_no_key(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.kms.kms_cmk_rotation_enabled.kms_cmk_rotation_enabled.kms_client', new=KMS(current_audit_info)):
            from prowler.providers.aws.services.kms.kms_cmk_rotation_enabled.kms_cmk_rotation_enabled import kms_cmk_rotation_enabled
    ...
    def test_kms_cmk_rotation_enabled(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.kms.kms_cmk_rotation_enabled.kms_cmk_rotation_enabled.kms_client', new=KMS(current_audit_info)):
            ...
            assert result[0].status_extended == f"KMS CMK {key['KeyId']} has automatic rotation enabled."
    ...
    def test_kms_cmk_rotation_disabled(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.kms.kms_cmk_rotation_enabled.kms_cmk_rotation_enabled.kms_client', new=KMS(current_audit_info)):
            ...
            assert result[0].status_extended == f"KMS CMK {key['KeyId']} has automatic rotation disabled."
            assert result[0].resource_id == key['KeyId']
            assert result[0].resource_arn == key['Arn']