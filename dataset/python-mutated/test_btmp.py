...
raw = b'\x06\x00\x00\x00Nt\x00\x00ssh:notty\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00garet\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00::1\x00' + b'\x00' * 260 + b'\xdd\xc7\xc2Y\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
pack = (6, 29774, b'ssh:notty' + b'\x00' * 23, b'\x00\x00\x00\x00', b'garet' + b'\x00' * 27, b'::1' + b'\x00' * 253, 0, 0, 0, 1505937373, 0, 0, 0, 0, 16777216)
...
def test_default_invalid_time_range():
    ...
    ret = btmp.validate(config)
    assert ret == (False, 'The time_range parameter for btmp beacon must contain start & end options.')
...
def test_match():
    ...
    with patch('salt.utils.files.fopen', mock_open(read_data=raw)):
        with patch('struct.unpack', MagicMock(return_value=pack)):
            ...
            ret = btmp.validate(config)
            assert ret == (True, 'Valid beacon configuration')
            _expected = [{'addr': 1505937373, 'exit_status': 0, 'inittab': '', 'hostname': '::1', 'PID': 29774, 'session': 0, 'user': 'garet', 'time': 0, 'line': 'ssh:notty', 'type': 6}]
            ret = btmp.beacon(config)
...
def test_match_time():
    ...
    with patch('salt.utils.files.fopen', mock_open(read_data=raw)):
        ...
        with patch('datetime.datetime', MagicMock()), patch('datetime.datetime.now', MagicMock(return_value=mock_now)):
            with patch('struct.unpack', MagicMock(return_value=pack)):
                config = [{'users': {'garet': {'time_range': {'end': '09-22-2017 5pm', 'start': '09-22-2017 3pm'}}}}]
                ret = btmp.validate(config)
                assert ret == (True, 'Valid beacon configuration')
                _expected = [{'addr': 1505937373, 'exit_status': 0, 'inittab': '', 'hostname': '::1', 'PID': 29774, 'session': 0, 'user': 'garet', 'time': 0, 'line': 'ssh:notty', 'type': 6}]
                ret = btmp.beacon(config)
...
def test_match_group():
    ...
    for groupadd in ('salt.modules.aix_group', 'salt.modules.mac_group', 'salt.modules.pw_group', 'salt.modules.solaris_group', 'salt.modules.win_groupadd'):
        mock_group_info = {'passwd': 'x', 'gid': 100, 'name': 'users', 'members': ['garet']}
        with patch('salt.utils.files.fopen', mock_open(read_data=raw)):
            with patch('time.time', MagicMock(return_value=1506121200)):
                with patch('struct.unpack', MagicMock(return_value=pack)):
                    with patch('{}.info'.format(groupadd), new=MagicMock(return_value=mock_group_info)):
                        config = [{'group': {'users': {'time_range': {'end': '5pm', 'start': '3pm'}}}}]
                        ret = btmp.validate(config)
                        assert ret == (True, 'Valid beacon configuration')
                        _expected = [{'addr': 1505937373, 'exit_status': 0, 'inittab': '', 'hostname': '::1', 'PID': 29774, 'session': 0, 'user': 'garet', 'time': 0, 'line': 'ssh:notty', 'type': 6}]
                        ret = btmp.beacon(config)
