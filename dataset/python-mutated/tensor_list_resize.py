...
def make_tensor_list_resize_tests(options):
    ...
    test_parameters = [{'element_dtype': [tf.float32, tf.int32], 'num_elements': [4, 5, 6], 'element_shape': [[], [5], [3, 3]], 'new_size': [1, 3, 5, 7]}]
    ...
    def build_graph(parameters):
        ...
        tensor_list = list_ops.tensor_list_resize(tensor_list, parameters['new_size'])
        out = list_ops.tensor_list_stack(tensor_list, element_dtype=parameters['element_dtype'])
        return ([data], [out])
