...
class OrderDiscountUpdate(OrderDiscountCommon):
    ...
    class Arguments:
        discount_id = graphene.ID(description='ID of a discount to update.', required=True)
        input = OrderDiscountCommonInput(required=True, description='Fields required to update a discount for the order.')
