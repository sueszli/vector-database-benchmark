...
async def test_zeromq_async_pub_channel_publish_port(temp_salt_master):
    ...
    """
    opts = dict(temp_salt_master.config.copy(), ipc_mode='ipc', pub_hwm=0, recon_randomize=False, publish_port=455505, recon_default=1, recon_max=2, master_ip='127.0.0.1', acceptance_wait_time=5, acceptance_wait_time_max=5, sign_pub_messages=False)
...
async def test_publish_client_connect_server_down(transport, io_loop):
    ...
    port = 111222
    if transport == 'zeromq':
        client = salt.transport.zeromq.PublishClient(opts, io_loop, host=host, port=port)
