...
async def test_http_processing_error(session: Any) -> None:
    ...
    response = ClientResponse('get', URL('http://del-cl-resp.org'), request_info=request_info, writer=WriterMock(), continue100=None, timer=TimerNoop(), traces=[], loop=loop, session=session)
...
@pytest.mark.skipif(sys.implementation.name != 'cpython', reason='Other implementations has different GC strategies')
...
async def test_response_eof(loop: Any, session: Any) -> None:
    response = ClientResponse('get', URL('http://def-cl-resp.org'), request_info=mock.Mock(), writer=None, continue100=None, timer=TimerNoop(), traces=[], loop=loop, session=session)
    response._closed = False
...
async def test_get_encoding_body_none(loop: Any, session: Any) -> None:
    ...
    content = response.content = mock.Mock()
    content.read.side_effect = side_effect
    with pytest.raises(RuntimeError, match='^Cannot compute fallback encoding of a not yet read body$'):
...
async def test_json_extended_content_type(loop: Any, session: Any) -> None:
    ...
    def side_effect(*args, **kwargs):
        ...
        fut.set_result('{"тест": "пройден"}'.encode('cp1251'))
        return fut
    response._headers = {'Content-Type': 'application/this.is-1_content+subtype+json;charset=cp1251'}
    content = response.content = mock.Mock()
    content.read.side_effect = side_effect
    res = await response.json()
    assert res == {'тест': 'пройден'}
...
async def test_response_read_triggers_callback(loop: Any, session: Any) -> None:
    ...
    response = ClientResponse(response_method, response_url, request_info=mock.Mock, writer=WriterMock(), continue100=None, timer=TimerNoop(), loop=loop, session=session, traces=[trace])
    ...
    assert trace.send_response_chunk_received.call_args == mock.call(response_method, response_url, response_body)
