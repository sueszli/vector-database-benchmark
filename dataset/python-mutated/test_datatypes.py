...
dtypes = [('FIXED', dt.int64), ('REAL', dt.float64), ('TEXT', dt.string), ('DATE', dt.date), ('TIMESTAMP', dt.Timestamp(scale=9)), ('VARIANT', dt.json), ('TIMESTAMP_LTZ', dt.Timestamp(timezone='UTC', scale=9)), ('TIMESTAMP_TZ', dt.Timestamp(timezone='UTC', scale=9)), ('TIMESTAMP_NTZ', dt.Timestamp(scale=9)), ('OBJECT', dt.Map(dt.string, dt.json)), ('ARRAY', dt.Array(dt.json)), ('BINARY', dt.binary), ('TIME', dt.time), ('BOOLEAN', dt.boolean)]
...
broken_timestamps = pytest.mark.xfail(raises=AssertionError, reason='snowflake-sqlalchemy timestamp types are broken and do not preserve scale information')
