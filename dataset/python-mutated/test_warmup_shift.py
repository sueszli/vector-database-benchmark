...
@testing.parameterize({'init': 1, 'warmup_start': 0.1, 'warmup_iter': 100, 'expect': [0.1, 0.991, 1, 1]}, {'init': 0.1, 'warmup_start': 1, 'warmup_iter': 10, 'expect': [1, 0.19, 0.1, 0.1]}, {'init': 1, 'warmup_start': -1, 'warmup_iter': 10, 'expect': [-1, 0.8, 1, 1]}, {'init': 1, 'warmup_start': -1, 'warmup_iter': 2, 'expect': [-1, 0, 1, 1]}, {'init': 0.1, 'warmup_start': 1, 'warmup_iter': 2, 'expect': [1, 0.55, 0.1, 0.1]})
class TestWarmupShift(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.extension = extensions.WarmupShift('x', self.warmup_start, self.warmup_iter, self.init, self.optimizer)
        self.interval = 1
    ...
    def test_basic(self):
        if False:
            while True:
                i = 10
        self.optimizer.x = 0
        extension = extensions.WarmupShift('x', self.warmup_start, self.warmup_iter, self.init, self.optimizer)
    ...
    def test_with_optimizer(self):
        ...
        optimizer = mock.Mock()
        optimizer.x = 0
        extension = extensions.WarmupShift('x', self.warmup_start, self.warmup_iter, self.init, optimizer)
