...
from airflow.providers.amazon.aws.operators.dms import DmsCreateTaskOperator, DmsDeleteTaskOperator, DmsDescribeTasksOperator, DmsStartTaskOperator, DmsStopTaskOperator
...
SAMPLE_DATA = [('Airflow', '2015'), ('OpenOffice', '2012'), ('Subversion', '2000'), ('NiFi', '2006')]
SG_IP_PERMISSION = {'FromPort': 5432, 'IpProtocol': 'All', 'IpRanges': [{'CidrIp': '0.0.0.0/0'}]}
...
with DAG(DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), tags=['example'], catchup=False) as dag:
    ...
    rds_db_name = f'{env_id}_source_database'
    ...
    table_definition = {'TableCount': '1', 'Tables': [{'TableName': rds_table_name, 'TableColumns': [{'ColumnName': TABLE_HEADERS[0], 'ColumnType': 'STRING', 'ColumnNullable': 'false', 'ColumnIsPk': 'true'}, {'ColumnName': TABLE_HEADERS[1], 'ColumnType': 'STRING', 'ColumnLength': '4'}], 'TableColumnsTotal': '2'}]}
    table_mappings = {'rules': [{'rule-type': 'selection', 'rule-id': '1', 'rule-name': '1', 'object-locator': {'schema-name': 'public', 'table-name': rds_table_name}, 'rule-action': 'include'}]}
    ...
    create_db_instance = RdsCreateDbInstanceOperator(task_id='create_db_instance', db_instance_identifier=rds_instance_name, db_instance_class='db.t3.micro', engine=RDS_ENGINE, rds_kwargs={'DBName': rds_db_name, 'AllocatedStorage': 20, 'MasterUsername': RDS_USERNAME, 'MasterUserPassword': RDS_PASSWORD, 'PubliclyAccessible': True, 'VpcSecurityGroupIds': [create_sg]})
    create_assets = create_dms_assets(db_name=rds_db_name, instance_name=rds_instance_name, replication_instance_name=dms_replication_instance_name, bucket_name=bucket_name, role_arn=role_arn, source_endpoint_identifier=source_endpoint_identifier, target_endpoint_identifier=target_endpoint_identifier, table_definition=table_definition)
    create_task = DmsCreateTaskOperator(task_id='create_task', replication_task_id=dms_replication_task_id, source_endpoint_arn=create_assets['source_endpoint_arn'], target_endpoint_arn=create_assets['target_endpoint_arn'], replication_instance_arn=create_assets['replication_instance_arn'], table_mappings=table_mappings)
    task_arn = cast(str, create_task.output)
    start_task = DmsStartTaskOperator(task_id='start_task', replication_task_arn=task_arn)
    describe_tasks = DmsDescribeTasksOperator(task_id='describe_tasks', describe_tasks_kwargs={'Filters': [{'Name': 'replication-instance-arn', 'Values': [create_assets['replication_instance_arn']]}]}, do_xcom_push=False)
    await_task_start = DmsTaskBaseSensor(task_id='await_task_start', replication_task_arn=task_arn, target_statuses=['running'], termination_statuses=['stopped', 'deleting', 'failed'], poke_interval=10)
    stop_task = DmsStopTaskOperator(task_id='stop_task', replication_task_arn=task_arn)
    await_task_stop = DmsTaskCompletedSensor(task_id='await_task_stop', replication_task_arn=task_arn)
    await_task_stop.poke_interval = 10
    delete_task = DmsDeleteTaskOperator(task_id='delete_task', replication_task_arn=task_arn)
    delete_task.trigger_rule = TriggerRule.ALL_DONE
    delete_assets = delete_dms_assets(replication_instance_arn=create_assets['replication_instance_arn'], source_endpoint_arn=create_assets['source_endpoint_arn'], target_endpoint_arn=create_assets['target_endpoint_arn'], source_endpoint_identifier=source_endpoint_identifier, target_endpoint_identifier=target_endpoint_identifier, replication_instance_name=dms_replication_instance_name)
    ...
    chain(test_context, create_s3_bucket, get_vpc_id, create_sg, create_db_instance, create_sample_table(rds_instance_name, rds_db_name, rds_table_name), create_assets, create_task, start_task, describe_tasks, await_task_start, stop_task, await_task_stop, delete_task, delete_assets, delete_db_instance, delete_security_group(create_sg, security_group_name), delete_s3_bucket)
