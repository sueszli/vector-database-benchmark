...
class FormatterFunctions:
    error_function_body = 'def evaluate(self, formatter, kwargs, mi, locals):\n\treturn "' + _('Duplicate user function name {0}. Change the name or ensure that the functions are identical') + '"'
...
class BuiltinStrcmp(BuiltinFormatterFunction):
    ...
    arg_count = 5
    category = 'Relational'
    __doc__ = doc = _('strcmp(x, y, lt, eq, gt) -- does a case-insensitive comparison of x and y as strings. Returns lt if x < y. Returns eq if x == y. Otherwise returns gt. In many cases the lexical comparison operators (>, <, == etc) can replace this function.')
...
class BuiltinCmp(BuiltinFormatterFunction):
    ...
    category = 'Relational'
    arg_count = 5
    __doc__ = doc = _('cmp(x, y, lt, eq, gt) -- compares x and y after converting both to numbers. Returns lt if x < y. Returns eq if x == y. Otherwise returns gt. In many cases the numeric comparison operators (>#, <#, ==# etc) can replace this function.')
...
class BuiltinFirstMatchingCmp(BuiltinFormatterFunction):
    ...
    category = 'Relational'
    arg_count = -1
    __doc__ = doc = _('first_matching_cmp(val, [cmp1, result1,]+, else_result) -- compares "val < cmpN" in sequence, returning resultN for the first comparison that succeeds. Returns else_result if no comparison succeeds. Example: first_matching_cmp(10,5,"small",10,"middle",15,"large","giant") returns "large". The same example with a first value of 16 returns "giant".')
...
class BuiltinStrcat(BuiltinFormatterFunction):
    ...
    __doc__ = doc = _('strcat(a [, b]*) -- can take any number of arguments. Returns the string formed by concatenating all the arguments')
...
class BuiltinAdd(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'Arithmetic'
    __doc__ = doc = _('add(x [, y]*) -- returns the sum of its arguments. Throws an exception if an argument is not a number. This function can often be replaced with the + operator.')
...
class BuiltinSubtract(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Arithmetic'
    __doc__ = doc = _('subtract(x, y) -- returns x - y. Throws an exception if either x or y are not numbers. This function can often be replaced with the - operator.')
...
class BuiltinMultiply(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'Arithmetic'
    __doc__ = doc = _('multiply(x [, y]*) -- returns the product of its arguments. Throws an exception if any argument is not a number. This function can often be replaced with the * operator.')
...
class BuiltinDivide(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Arithmetic'
    __doc__ = doc = _('divide(x, y) -- returns x / y. Throws an exception if either x or y are not numbers. This function can often be replaced with the / operator.')
...
class BuiltinCeiling(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Arithmetic'
    __doc__ = doc = _('ceiling(x) -- returns the smallest integer greater than or equal to x. Throws an exception if x is not a number.')
...
class BuiltinFloor(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Arithmetic'
    __doc__ = doc = _('floor(x) -- returns the largest integer less than or equal to x. Throws an exception if x is not a number.')
...
class BuiltinRound(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Arithmetic'
    __doc__ = doc = _('round(x) -- returns the nearest integer to x. Throws an exception if x is not a number.')
...
class BuiltinMod(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Arithmetic'
    __doc__ = doc = _('mod(x) -- returns floor(remainder of x / y). Throws an exception if either x or y is not a number.')
...
class BuiltinFractionalPart(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Arithmetic'
    __doc__ = doc = _('fractional_part(x) -- returns the value after the decimal point.  For example, fractional_part(3.14) returns 0.14. Throws an exception if x is not a number.')
...
class BuiltinEval(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Recursion'
    __doc__ = doc = _("eval(template) -- evaluates the template, passing the local variables (those 'assign'ed to) instead of the book metadata.  This permits using the template processor to construct complex results from local variables. Because the { and } characters are special, you must use [[ for the { character and ]] for the } character; they are converted automatically. Note also that prefixes and suffixes (the `|prefix|suffix` syntax) cannot be used in the argument to this function when using template program mode.")
...
class BuiltinAssign(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Other'
    __doc__ = doc = _('assign(id, val) -- assigns val to id, then returns val. id must be an identifier, not an expression. This function can often be replaced with the = operator.')
...
class BuiltinPrint(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'Other'
    __doc__ = doc = _('print(a[, b]*) -- prints the arguments to standard output. Unless you start calibre from the command line (calibre-debug -g), the output will go to a black hole.')
...
class BuiltinRawField(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'Get values from metadata'
    __doc__ = doc = _("raw_field(lookup_name [, optional_default]) -- returns the metadata field named by lookup_name without applying any formatting. It evaluates and returns the optional second argument 'default' if the field is undefined ('None').")
...
class BuiltinRawList(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Get values from metadata'
    __doc__ = doc = _('raw_list(lookup_name, separator) -- returns the metadata list named by lookup_name without applying any formatting or sorting and with items separated by separator.')
...
class BuiltinSubstr(BuiltinFormatterFunction):
    ...
    arg_count = 3
    category = 'String manipulation'
    __doc__ = doc = _("substr(str, start, end) -- returns the start'th through the end'th characters of str. The first character in str is the zero'th character. If end is negative, then it indicates that many characters counting from the right. If end is zero, then it indicates the last character. For example, substr('12345', 1, 0) returns '2345', and substr('12345', 1, -1) returns '234'.")
...
class BuiltinLookup(BuiltinFormatterFunction):
    ...
    __doc__ = doc = _('lookup(val, [pattern, field,]+ else_field) -- like switch, except the arguments are field (metadata) names, not text. The value of the appropriate field will be fetched and used. Note that because composite columns are fields, you can use this function in one composite field to use the value of some other composite field. This is extremely useful when constructing variable save paths')
...
class BuiltinTest(BuiltinFormatterFunction):
    ...
    __doc__ = doc = _('test(val, text if not empty, text if empty) -- return `text if not empty` if val is not empty, otherwise return `text if empty`')
...
class BuiltinContains(BuiltinFormatterFunction):
    ...
    arg_count = 4
    category = 'If-then-else'
    __doc__ = doc = _('contains(val, pattern, text if match, text if not match) -- checks if val contains matches for the regular expression `pattern`. Returns `text if match` if matches are found, otherwise it returns `text if no match`')
...
class BuiltinSwitch(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'Iterating over values'
    __doc__ = doc = _('switch(val, [pattern, value,]+ else_value) -- for each `pattern, value` pair, checks if `val` matches the regular expression `pattern` and if so, returns that `value`. If no pattern matches, then `else_value` is returned. You can have as many `pattern, value` pairs as you want')
...
class BuiltinStrcatMax(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'String manipulation'
    __doc__ = doc = _('strcat_max(max, string1 [, prefix2, string2]*) -- Returns a string formed by concatenating the arguments. The returned value is initialized to string1. `Prefix, string` pairs are added to the end of the value as long as the resulting string length is less than `max`. String1 is returned even if string1 is longer than max. You can pass as many `prefix, string` pairs as you wish.')
...
class BuiltinRe(BuiltinFormatterFunction):
    ...
    arg_count = 3
    category = 'String manipulation'
    __doc__ = doc = _('re(val, pattern, replacement) -- return val after applying the regular expression. All instances of `pattern` are replaced with `replacement`. As in all of calibre, these are Python-compatible regular expressions')
...
class BuiltinSwapAroundComma(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'String manipulation'
    __doc__ = doc = _('swap_around_comma(val) -- given a value of the form "B, A", return "A B". This is most useful for converting names in LN, FN format to FN LN. If there is no comma, the function returns val unchanged')
...
class BuiltinIfempty(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'If-then-else'
    __doc__ = doc = _('ifempty(val, text if empty) -- return val if val is not empty, otherwise return `text if empty`')
...
class BuiltinShorten(BuiltinFormatterFunction):
    ...
    arg_count = 4
    category = 'String manipulation'
    __doc__ = doc = _("shorten(val, left chars, middle text, right chars) -- Return a shortened version of val, consisting of `left chars` characters from the beginning of val, followed by `middle text`, followed by `right chars` characters from the end of the string. `Left chars` and `right chars` must be integers. For example, assume the title of the book is `Ancient English Laws in the Times of Ivanhoe`, and you want it to fit in a space of at most 15 characters. If you use {title:shorten(9,-,5)}, the result will be `Ancient E-anhoe`. If the field's length is less than left chars + right chars + the length of `middle text`, then the field will be used intact. For example, the title `The Dome` would not be changed.")
...
class BuiltinCount(BuiltinFormatterFunction):
    ...
    category = 'List manipulation'
    aliases = ['list_count']
    __doc__ = doc = _('count(val, separator) -- interprets the value as a list of items separated by `separator`, returning the number of items in the list. Most lists use a comma as the separator, but authors uses an ampersand. Examples: {tags:count(,)}, {authors:count(&)}. Aliases: count(), list_count()')
...
class BuiltinListCountMatching(BuiltinFormatterFunction):
    ...
    __doc__ = doc = _("list_count_matching(list, pattern, separator) -- interprets 'list' as a list of items separated by 'separator', returning the number of items in the list that match the regular expression 'pattern'. Aliases: list_count_matching(), count_matching()")
...
class BuiltinSelect(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'List lookup'
    __doc__ = doc = _('select(val, key) -- interpret the value as a comma-separated list of items, with the items being "id:value". Find the pair with the id equal to key, and return the corresponding value. Returns the empty string if no match is found.')
...
class BuiltinFormatsSizes(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('formats_sizes() -- return a comma-separated list of colon-separated items representing sizes in bytes of the formats of a book. You can use the select function to get the size for a specific format. Note that format names are always uppercase, as in EPUB.')
...
class BuiltinFormatsPaths(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('formats_paths() -- return a comma-separated list of colon-separated items representing full path to the formats of a book. You can use the select function to get the path for a specific format. Note that format names are always uppercase, as in EPUB.')
...
class BuiltinSubitems(BuiltinFormatterFunction):
    ...
    period_pattern = re.compile('(?<=[^\\.\\s])\\.(?=[^\\.\\s])', re.U)
...
class BuiltinFormatDateField(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Formatting values'
    __doc__ = doc = _("format_date_field(field_name, format_string) -- format the value in the field 'field_name', which must be the lookup name of date field, either standard or custom. See 'format_date' for the formatting codes. This function is much faster than format_date and should be used when you are formatting the value in a field (column). It can't be used for computed dates or dates in string variables. Example: format_date_field('pubdate', 'yyyy.MM.dd')")
...
class BuiltinBooksize(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('booksize() -- return value of the size field. This function works only in the GUI. If you want to use this value in save-to-disk or send-to-device templates then you must make a custom "Column built from other columns", use the function in that column\'s template, and use that column\'s value in your save/send templates')
...
class BuiltinOndevice(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('ondevice() -- return Yes if ondevice is set, otherwise return the empty string. This function works only in the GUI. If you want to use this value in save-to-disk or send-to-device templates then you must make a custom "Column built from other columns", use the function in that column\'s template, and use that column\'s value in your save/send templates')
...
class BuiltinAnnotationCount(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('annotation_count() -- return the total number of annotations of all types attached to the current book. This function works only in the GUI.')
...
class BuiltinIsMarked(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _("is_marked() -- check whether the book is 'marked' in calibre. If it is then return the value of the mark, either 'true' or the comma-separated list of named marks. Returns '' if the book is not marked.")
...
class BuiltinFirstNonEmpty(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'Iterating over values'
    __doc__ = doc = _('first_non_empty(value [, value]*) -- returns the first value that is not empty. If all values are empty, then the empty string is returned. You can have as many values as you want.')
...
class BuiltinAnd(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'Boolean'
    __doc__ = doc = _('and(value [, value]*) -- returns the string "1" if all values are not empty, otherwise returns the empty string. This function works well with test or first_non_empty. You can have as many values as you want. In many cases the && operator can replace this function.')
...
class BuiltinOr(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'Boolean'
    __doc__ = doc = _('or(value [, value]*) -- returns the string "1" if any value is not empty, otherwise returns the empty string. This function works well with test or first_non_empty. You can have as many values as you want.  In many cases the || operator can replace this function.')
...
class BuiltinNot(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Boolean'
    __doc__ = doc = _('not(value) -- returns the string "1" if the value is empty, otherwise returns the empty string. This function works well with test or first_non_empty.  In many cases the ! operator can replace this function.')
...
class BuiltinListRemoveDuplicates(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'List manipulation'
    __doc__ = doc = _('list_remove_duplicates(list, separator) -- return a list made by removing duplicate items in the source list. If items differ only in case, the last of them is returned. The items in source list are separated by separator, as are the items in the returned list.')
...
class BuiltinListDifference(BuiltinFormatterFunction):
    ...
    arg_count = 3
    category = 'List manipulation'
    __doc__ = doc = _('list_difference(list1, list2, separator) -- return a list made by removing from list1 any item found in list2, using a case-insensitive comparison. The items in list1 and list2 are separated by separator, as are the items in the returned list.')
...
class BuiltinListIntersection(BuiltinFormatterFunction):
    ...
    arg_count = 3
    category = 'List manipulation'
    __doc__ = doc = _('list_intersection(list1, list2, separator) -- return a list made by removing from list1 any item not found in list2, using a case-insensitive comparison. The items in list1 and list2 are separated by separator, as are the items in the returned list.')
...
class BuiltinListSort(BuiltinFormatterFunction):
    ...
    arg_count = 3
    category = 'List manipulation'
    __doc__ = doc = _('list_sort(list, direction, separator) -- return list sorted using a case-insensitive sort. If direction is zero, the list is sorted ascending, otherwise descending. The list items are separated by separator, as are the items in the returned list.')
...
class BuiltinListReGroup(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'List manipulation'
    __doc__ = doc = _('list_re_group(src_list, separator, include_re, search_re [, group_template]+) -- Like list_re except replacements are not optional. It uses re_group(list_item, search_re, group_template, ...) when doing the replacements on the resulting list.')
...
class BuiltinToday(BuiltinFormatterFunction):
    ...
    __doc__ = doc = _('today() -- return a date string for today. This value is designed for use in format_date or days_between, but can be manipulated like any other string. The date is in ISO format.')
...
class BuiltinDaysBetween(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Date functions'
    __doc__ = doc = _('days_between(date1, date2) -- return the number of days between date1 and date2. The number is positive if date1 is greater than date2, otherwise negative. If either date1 or date2 are not dates, the function returns the empty string.')
...
class BuiltinLanguageStrings(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Get values from metadata'
    __doc__ = doc = _('language_strings(lang_codes, localize) -- return the strings for the language codes passed in lang_codes. If localize is zero, return the strings in English. If localize is not zero, return the strings in the language of the current locale. Lang_codes is a comma-separated list.')
...
class BuiltinLanguageCodes(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Get values from metadata'
    __doc__ = doc = _('language_codes(lang_strings) -- return the language codes for the strings passed in lang_strings. The strings must be in the language of the current locale. Lang_strings is a comma-separated list.')
...
class BuiltinCurrentLibraryName(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('current_library_name() -- return the last name on the path to the current calibre library. This function can be called in template program mode using the template "{:\'current_library_name()\'}".')
...
class BuiltinCurrentLibraryPath(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('current_library_path() -- return the path to the current calibre library. This function can be called in template program mode using the template "{:\'current_library_path()\'}".')
...
class BuiltinFinishFormatting(BuiltinFormatterFunction):
    ...
    arg_count = 4
    category = 'Formatting values'
    __doc__ = doc = _('finish_formatting(val, fmt, prefix, suffix) -- apply the format, prefix, and suffix to a value in the same way as done in a template like `{series_index:05.2f| - |- }`. For example, the following program produces the same output as the above template: program: finish_formatting(field("series_index"), "05.2f", " - ", " - ")')
...
class BuiltinVirtualLibraries(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('virtual_libraries() -- return a comma-separated list of Virtual libraries that contain this book. This function works only in the GUI. If you want to use these values in save-to-disk or send-to-device templates then you must make a custom "Column built from other columns", use the function in that column\'s template, and use that column\'s value in your save/send templates')
...
class BuiltinCurrentVirtualLibraryName(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('current_virtual_library_name() -- return the name of the current virtual library if there is one, otherwise the empty string. Library name case is preserved. Example: "program: current_virtual_library_name()".')
...
class BuiltinUserCategories(BuiltinFormatterFunction):
    ...
    arg_count = 0
    category = 'Get values from metadata'
    __doc__ = doc = _('user_categories() -- return a comma-separated list of the user categories that contain this book. This function works only in the GUI. If you want to use these values in save-to-disk or send-to-device templates then you must make a custom "Column built from other columns", use the function in that column\'s template, and use that column\'s value in your save/send templates')
...
class BuiltinTransliterate(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'String manipulation'
    __doc__ = doc = _('transliterate(a) -- Returns a string in a latin alphabet formed by approximating the sound of the words in the source string. For example, if the source is "{0}" the function returns "{1}".').format('Фёдор Миха́йлович Достоевский', 'Fiodor Mikhailovich Dostoievskii')
...
class BuiltinGetLink(BuiltinFormatterFunction):
    ...
    __doc__ = doc = _("get_link(field_name, field_value) -- fetch the link for field 'field_name' with value 'field_value'. If there is no attached link, return ''. Example: get_link('tags', 'Fiction') returns the link attached to the tag 'Fiction'.")
...
class BuiltinConnectedDeviceName(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Get values from metadata'
    __doc__ = doc = _("connected_device_name(storage_location) -- if a device is connected then return the device name, otherwise return the empty string. Each storage location on a device can have a different name. The location names are 'main', 'carda' and 'cardb'. This function works only in the GUI.")
...
class BuiltinConnectedDeviceUUID(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Get values from metadata'
    __doc__ = doc = _("connected_device_uuid(storage_location) -- if a device is connected then return the device uuid (unique id), otherwise return the empty string. Each storage location on a device has a different uuid. The location names are 'main', 'carda' and 'cardb'. This function works only in the GUI.")
...
class BuiltinRatingToStars(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Formatting values'
    __doc__ = doc = _('rating_to_stars(value, use_half_stars) -- Returns the rating as string of star characters. The value is a number between 0 and 5. Set use_half_stars to 1 if you want half star characters for custom ratings columns that support non-integer ratings, for example 2.5.')
...
class BuiltinSetGlobals(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'other'
    __doc__ = doc = _('set_globals(id[=expression] [, id[=expression]]*) -- Sets "global variables" that can be passed into the formatter. The globals are given the name of the id passed in. The value of the id is used unless an expression is provided.')
...
class BuiltinFieldExists(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'If-then-else'
    __doc__ = doc = _("field_exists(field_name) -- checks if a field (column) named field_name exists, returning '1' if so and '' if not.")
...
class BuiltinCharacter(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'String manipulation'
    __doc__ = doc = _("character(character_name) -- returns the character named by character_name. For example, character('newline') returns a newline character ('\\n'). The supported character names are 'newline', 'return', 'tab', and 'backslash'.")
...
class BuiltinToHex(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'String manipulation'
    __doc__ = doc = _('to_hex(val) -- returns the string encoded in hex. This is useful when constructing calibre URLs.')
...
class BuiltinBookCount(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Template database functions'
    __doc__ = doc = _('book_count(query, use_vl) -- returns the count of books found by searching for query. If use_vl is 0 (zero) then virtual libraries are ignored. This function can be used only in the GUI.')
...
class BuiltinBookValues(BuiltinFormatterFunction):
    ...
    arg_count = 4
    category = 'Template database functions'
    __doc__ = doc = _('book_values(column, query, sep, use_vl) -- returns a list of the values contained in the column "column", separated by "sep", in the books found by searching for "query". If use_vl is 0 (zero) then virtual libraries are ignored. This function can be used only in the GUI.')
...
class BuiltinExtraFileNames(BuiltinFormatterFunction):
    ...
    arg_count = -1
    category = 'Template database functions'
    __doc__ = doc = _("extra_file_names(sep [, pattern]) -- returns a sep-separated list of extra files in the book's '{}/' folder. If the optional parameter 'pattern', a regular expression, is supplied then the list is filtered to files that match pattern. The pattern match is case insensitive. This function can be used only in the GUI.").format(DATA_DIR_NAME)
...
class BuiltinExtraFileSize(BuiltinFormatterFunction):
    ...
    arg_count = 1
    category = 'Template database functions'
    __doc__ = doc = _("extra_file_size(file_name) -- returns the size in bytes of the extra file 'file_name' in the book's '{}/' folder if it exists, otherwise -1.This function can be used only in the GUI.").format(DATA_DIR_NAME)
...
class BuiltinExtraFileModtime(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Template database functions'
    __doc__ = doc = _("extra_file_modtime(file_name, format_string) -- returns the modification time of the extra file 'file_name' in the book's '{}/' folder if it exists, otherwise -1.0. The modtime is formatted according to 'format_string' (see format_date()). If 'format_string' is empty, returns the modtime as the floating point number of seconds since the epoch. The epoch is OS dependent. This function can be used only in the GUI.").format(DATA_DIR_NAME)
...
class BuiltinGetNote(BuiltinFormatterFunction):
    ...
    arg_count = 3
    category = 'Template database functions'
    __doc__ = doc = _("get_note(field_name, field_value, plain_text) -- fetch the note for field 'field_name' with value 'field_value'. If 'plain_text' is empty, return the note's HTML. If 'plain_text' is non-empty, return the note's plain text. If the note doesn't exist, return '' in both cases. Example: get_note('tags', 'Fiction', '') returns the HTML of the note attached to the tag 'Fiction'.")
...
class BuiltinHasNote(BuiltinFormatterFunction):
    ...
    arg_count = 2
    category = 'Template database functions'
    __doc__ = doc = _("has_note(field_name, field_value) -- return '1' if the value 'field_value' in the field 'field_name' has an attached note, '' otherwise. Example: has_note('tags', 'Fiction') returns '1' if the tag 'fiction' has an attached note, '' otherwise.")
