...
from airbyte_cdk.connector_builder.connector_builder_handler import DEFAULT_MAXIMUM_NUMBER_OF_PAGES_PER_SLICE, DEFAULT_MAXIMUM_NUMBER_OF_SLICES, DEFAULT_MAXIMUM_RECORDS, TestReadLimits, create_source, get_limits, resolve_manifest
...
from airbyte_cdk.models import AirbyteLogMessage, AirbyteMessage, AirbyteRecordMessage, AirbyteStream, ConfiguredAirbyteCatalog, ConfiguredAirbyteStream, ConnectorSpecification, DestinationSyncMode, Level, SyncMode
...
_page_size = 2
MANIFEST = {'version': '0.30.3', 'definitions': {'retriever': {'paginator': {'type': 'DefaultPaginator', 'page_size': _page_size, 'page_size_option': {'inject_into': 'request_parameter', 'field_name': 'page_size'}, 'page_token_option': {'inject_into': 'path', 'type': 'RequestPath'}, 'pagination_strategy': {'type': 'CursorPagination', 'cursor_value': '{{ response._metadata.next }}', 'page_size': _page_size}}, 'partition_router': {'type': 'ListPartitionRouter', 'values': ['0', '1', '2', '3', '4', '5', '6', '7'], 'cursor_field': 'item_id'}, 'requester': {'path': '/v3/marketing/lists', 'authenticator': {'type': 'BearerAuthenticator', 'api_token': '{{ config.apikey }}'}, 'request_parameters': {'a_param': '10'}}, 'record_selector': {'extractor': {'field_path': ['result']}}}}, 'streams': [{'type': 'DeclarativeStream', '$parameters': _stream_options, 'retriever': '#/definitions/retriever'}], 'check': {'type': 'CheckStream', 'stream_names': ['lists']}, 'spec': {'connection_specification': {'$schema': 'http://json-schema.org/draft-07/schema#', 'type': 'object', 'required': [], 'properties': {}, 'additionalProperties': True}, 'type': 'Spec'}}
OAUTH_MANIFEST = {'version': '0.30.3', 'definitions': {'retriever': {'paginator': {'type': 'DefaultPaginator', 'page_size': _page_size, 'page_size_option': {'inject_into': 'request_parameter', 'field_name': 'page_size'}, 'page_token_option': {'inject_into': 'path', 'type': 'RequestPath'}, 'pagination_strategy': {'type': 'CursorPagination', 'cursor_value': '{{ response._metadata.next }}', 'page_size': _page_size}}, 'partition_router': {'type': 'ListPartitionRouter', 'values': ['0', '1', '2', '3', '4', '5', '6', '7'], 'cursor_field': 'item_id'}, 'requester': {'path': '/v3/marketing/lists', 'authenticator': {'type': 'OAuthAuthenticator', 'api_token': '{{ config.apikey }}'}, 'request_parameters': {'a_param': '10'}}, 'record_selector': {'extractor': {'field_path': ['result']}}}}, 'streams': [{'type': 'DeclarativeStream', '$parameters': _stream_options, 'retriever': '#/definitions/retriever'}], 'check': {'type': 'CheckStream', 'stream_names': ['lists']}, 'spec': {'connection_specification': {'$schema': 'http://json-schema.org/draft-07/schema#', 'type': 'object', 'required': [], 'properties': {}, 'additionalProperties': True}, 'type': 'Spec'}}
RESOLVE_MANIFEST_CONFIG = {'__injected_declarative_manifest': MANIFEST, '__command': 'resolve_manifest'}
TEST_READ_CONFIG = {'__injected_declarative_manifest': MANIFEST, '__command': 'test_read', '__test_read_config': {'max_pages_per_slice': 2, 'max_slices': 5, 'max_records': 10}}
DUMMY_CATALOG = {'streams': [{'stream': {'name': 'dummy_stream', 'json_schema': {'$schema': 'http://json-schema.org/draft-07/schema#', 'type': 'object', 'properties': {}}, 'supported_sync_modes': ['full_refresh'], 'source_defined_cursor': False}, 'sync_mode': 'full_refresh', 'destination_sync_mode': 'overwrite'}]}
...
MOCK_RESPONSE = {'result': [{'id': 1, 'name': 'Nora Moon', 'position': 'director'}, {'id': 2, 'name': 'Hae Sung Jung', 'position': 'cinematographer'}, {'id': 3, 'name': 'Arthur Zenneranski', 'position': 'composer'}]}
