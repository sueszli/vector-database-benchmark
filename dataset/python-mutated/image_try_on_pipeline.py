...
@PIPELINES.register_module(Tasks.image_try_on, module_name=Pipelines.image_try_on)
class SALForImageTryOnPipeline(Pipeline):
    ...
    def forward(self, input: Dict[str, Any]) -> Dict[str, Any]:
        if False:
            while True:
                i = 10
        try_on_image = try_on_infer.infer(self.model, self.model_path, input['person_input_path'], input['garment_input_path'], input['mask_input_path'], self.device)
