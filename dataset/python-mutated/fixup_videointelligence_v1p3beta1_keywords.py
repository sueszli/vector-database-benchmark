...
class videointelligenceCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'annotate_video': ('features', 'input_uri', 'input_content', 'video_context', 'output_uri', 'location_id'), 'streaming_annotate_video': ('video_config', 'input_content')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=videointelligenceCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
