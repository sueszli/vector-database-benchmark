...
telescope_lines = pwndbg.gdblib.config.add_param('telescope-lines', 8, 'number of lines to printed by the telescope command')
skip_repeating_values = pwndbg.gdblib.config.add_param('telescope-skip-repeating-val', True, 'whether to skip repeating values of the telescope command')
skip_repeating_values_minimum = pwndbg.gdblib.config.add_param('telescope-skip-repeating-val-minimum', 3, 'minimum amount of repeated values before skipping lines')
print_framepointer_offset = pwndbg.gdblib.config.add_param('telescope-framepointer-offset', True, 'print offset to framepointer for each address, if sufficiently small')
offset_separator = theme.add_param('telescope-offset-separator', '│', 'offset separator of the telescope command')
offset_delimiter = theme.add_param('telescope-offset-delimiter', ':', 'offset delimiter of the telescope command')
repeating_marker = theme.add_param('telescope-repeating-marker', '... ↓', 'repeating values marker of the telescope command')
parser = argparse.ArgumentParser(description='Recursively dereferences pointers starting at the specified address.')
parser.add_argument('-r', '--reverse', dest='reverse', action='store_true', default=False, help='Show <count> previous addresses instead of next ones')
parser.add_argument('-f', '--frame', dest='frame', action='store_true', default=False, help='Show the stack frame, from rsp to rbp')
parser.add_argument('address', nargs='?', default='$sp', type=int, help='The address to telescope at.')
...
parser.add_argument('offset', nargs='?', default=0, type=int, help='Element offset from $sp (support negative offset)')
...
parser = argparse.ArgumentParser(description='Dereferences on stack data, printing the entire stack frame with specified count and offset .')
parser.add_argument('count', nargs='?', default=8, type=int, help='number of element to dump')
parser.add_argument('offset', nargs='?', default=0, type=int, help='Element offset from $sp (support negative offset)')
