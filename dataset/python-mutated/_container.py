...
from .._base import build_options as _build_options, validate_cache_staleness_value, _deserialize_throughput, _replace_throughput, GenerateGuidId
...
class ContainerProxy(object):
    ...
    async def is_system_key(self) -> bool:
        if self._is_system_key is None:
            ...
            self._is_system_key = properties['partitionKey']['systemKey'] if 'systemKey' in properties['partitionKey'] else False
    ...
    async def read(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        self._properties = await self.client_connection.ReadContainer(collection_link, options=request_options, **kwargs)
        if response_hook:
    ...
    async def create_item(self, body: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        ...
        result = await self.client_connection.CreateItem(database_or_container_link=self.container_link, document=body, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, result)
        return result

    @distributed_trace_async
    async def read_item(self, item: Union[str, Dict[str, Any]], partition_key: Union[str, int, float, bool], **kwargs: Any) -> Dict[str, Any]:
    ...
    async def upsert_item(self, body: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        ...
        if post_trigger_include is not None:
            request_options['postTriggerInclude'] = post_trigger_include
        result = await self.client_connection.UpsertItem(database_or_container_link=self.container_link, document=body, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, result)
        return result

    @distributed_trace_async
    async def replace_item(self, item: Union[str, Dict[str, Any]], body: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        ...
        if post_trigger_include is not None:
            request_options['postTriggerInclude'] = post_trigger_include
        result = await self.client_connection.ReplaceItem(document_link=item_link, new_document=body, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, result)
        return result

    @distributed_trace_async
    async def patch_item(self, item: Union[str, Dict[str, Any]], partition_key: Union[str, int, float, bool], patch_operations: List[Dict[str, Any]], **kwargs: Any) -> Dict[str, Any]:
        ...
        item_link = self._get_document_link(item)
        result = await self.client_connection.PatchItem(document_link=item_link, operations=patch_operations, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, result)
        return result

    @distributed_trace_async
    async def delete_item(self, item: Union[str, Dict[str, Any]], partition_key: Union[str, int, float, bool], **kwargs: Any) -> None:
    ...
    async def get_throughput(self, **kwargs: Any) -> ThroughputProperties:
        ...
        query_spec = {'query': 'SELECT * FROM root r WHERE r.resource=@link', 'parameters': [{'name': '@link', 'value': link}]}
        throughput_properties = [throughput async for throughput in self.client_connection.QueryOffers(query_spec, **kwargs)]
        if len(throughput_properties) == 0:
            raise CosmosResourceNotFoundError(status_code=StatusCodes.NOT_FOUND, message='Could not find ThroughputProperties for container ' + self.container_link)
        if response_hook:
    ...
    async def replace_throughput(self, throughput: Union[int, ThroughputProperties], **kwargs: Any) -> ThroughputProperties:
        ...
        throughput_properties = [throughput async for throughput in self.client_connection.QueryOffers(query_spec, **kwargs)]
        if len(throughput_properties) == 0:
            raise CosmosResourceNotFoundError(status_code=StatusCodes.NOT_FOUND, message='Could not find Offer for container ' + self.container_link)
        new_offer = throughput_properties[0].copy()
        _replace_throughput(throughput=throughput, new_throughput_properties=new_offer)
        data = await self.client_connection.ReplaceOffer(offer_link=throughput_properties[0]['_self'], offer=throughput_properties[0], **kwargs)
        if response_hook:
    ...
    @distributed_trace_async
    async def get_conflict(self, conflict: Union[str, Dict[str, Any]], partition_key: Union[str, int, float, bool], **kwargs: Any) -> Dict[str, Any]:
        ...
        response_hook = kwargs.pop('response_hook', None)
        request_options['partitionKey'] = self._set_partition_key(partition_key)
        result = await self.client_connection.ReadConflict(conflict_link=self._get_conflict_link(conflict), options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, result)
        return result

    @distributed_trace_async
    async def delete_conflict(self, conflict: Union[str, Dict[str, Any]], partition_key: Union[str, int, float, bool], **kwargs: Any) -> None:
        ...
        response_hook = kwargs.pop('response_hook', None)
        request_options['partitionKey'] = self._set_partition_key(partition_key)
        result = await self.client_connection.DeleteConflict(conflict_link=self._get_conflict_link(conflict), options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, result)

    @distributed_trace_async
    async def delete_all_items_by_partition_key(self, partition_key: Union[str, int, float, bool], **kwargs: Any) -> None:
        ...
        response_hook = kwargs.pop('response_hook', None)
        request_options['partitionKey'] = self._set_partition_key(partition_key)
        await self.client_connection.DeleteAllItemsByPartitionKey(collection_link=self.container_link, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, None)

    @distributed_trace_async
    async def execute_item_batch(self, batch_operations: List[Tuple[Any]], partition_key: Union[str, int, float, bool], **kwargs) -> List[Dict[str, Any]]:
        ...
        response_hook = kwargs.pop('response_hook', None)
        request_options['disableAutomaticIdGeneration'] = True
        result = await self.client_connection.Batch(collection_link=self.container_link, batch_operations=batch_operations, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, result)
        return result