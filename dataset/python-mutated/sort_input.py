...
class SortInputObjectType(BaseInputObjectType):
    direction = graphene.Argument(OrderDirection, required=True, description='Specifies the direction in which to sort.')
...
class ChannelSortInputObjectType(SortInputObjectType):
    channel = graphene.Argument(graphene.String, description=f'Specifies the channel in which to sort the data.{DEPRECATED_IN_3X_INPUT} Use root-level channel argument instead.')
