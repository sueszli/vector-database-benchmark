...
class TestLoadBalancerProcessRequest:
    ...
    async def test_workers_not_ready_with_cold_start_proxy(self, monkeypatch):
        ...
        load_balancer = _LoadBalancer(input_type=Text, output_type=Text, endpoint='/predict', cold_start_proxy=ColdStartProxy('url'))
    ...
    async def test_workers_not_ready_without_cold_start_proxy(self, monkeypatch):
        load_balancer = _LoadBalancer(input_type=Text, output_type=Text, endpoint='/predict')
        req_id = uuid.uuid4().hex
