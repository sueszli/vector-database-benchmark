...
class datacatalog_lineageCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'batch_search_link_processes': ('parent', 'links', 'page_size', 'page_token'), 'create_lineage_event': ('parent', 'lineage_event', 'request_id'), 'create_process': ('parent', 'process', 'request_id'), 'create_run': ('parent', 'run', 'request_id'), 'delete_lineage_event': ('name', 'allow_missing'), 'delete_process': ('name', 'allow_missing'), 'delete_run': ('name', 'allow_missing'), 'get_lineage_event': ('name',), 'get_process': ('name',), 'get_run': ('name',), 'list_lineage_events': ('parent', 'page_size', 'page_token'), 'list_processes': ('parent', 'page_size', 'page_token'), 'list_runs': ('parent', 'page_size', 'page_token'), 'process_open_lineage_run_event': ('parent', 'open_lineage', 'request_id'), 'search_links': ('parent', 'source', 'target', 'page_size', 'page_token'), 'update_process': ('process', 'update_mask', 'allow_missing'), 'update_run': ('run', 'update_mask', 'allow_missing')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=datacatalog_lineageCallTransformer()):
    if False:
        while True:
            i = 10
