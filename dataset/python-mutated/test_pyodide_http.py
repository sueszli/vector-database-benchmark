...
async def test_pyfetch_create_file(selenium):
    ...
    assert pathlib.Path('/home/pyodide/console.html').read_text().find('fatal error.') > 3000
...
async def test_pyfetch_raise_for_status(selenium, raise_for_status_fixture):
    ...
    with pytest.raises(OSError, match='504 Server Error: GATEWAY TIMEOUT for url: .*/status_504'):
        resp.raise_for_status()
...
async def test_pyfetch_unpack_archive(selenium):
    import pathlib
    from pyodide.http import pyfetch
    resp = await pyfetch('https://files.pythonhosted.org/packages/93/8d/e1e98360dc899e533cb3dd857494f2571b129bdffcee76365009b2bb507c/example_pypi_package-0.1.0.tar.gz')
    await resp.unpack_archive()
    names = [f.stem for f in pathlib.Path('/home/pyodide/example_pypi_package-0.1.0').glob('*')]
    assert names == ['LICENSE', 'MANIFEST', 'PKG-INFO', 'README', 'pyproject', 'setup', 'setup', 'src', 'tests']
