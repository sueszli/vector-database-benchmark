...
class sfp_sublist3r(SpiderFootPlugin):
    meta = {'name': 'Sublist3r PassiveDNS', 'summary': "Passive subdomain enumeration using Sublist3r's API", 'useCases': ['Investigate', 'Footprint', 'Passive'], 'categories': ['Passive DNS'], 'dataSource': {'website': 'https://api.sublist3r.com', 'model': 'FREE_NOAUTH_UNLIMITED', 'description': 'This is the API queried by the Sublist3r tool.'}}
    opts = {}
    optdescs = {}
    results = None
    ...
    def query(self, domain):
        ...
        ret = []
        res = self.sf.fetchUrl(url, useragent=self.opts.get('_useragent', 'Spiderfoot'), headers={'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8', 'Accept-Language': 'en-US,en;q=0.8', 'Accept-Encoding': 'gzip'})
        try:
