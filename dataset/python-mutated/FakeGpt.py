...
class FakeGpt(AsyncGeneratorProvider):
    ...
    async def create_async_generator(cls, model: str, messages: Messages, proxy: str=None, **kwargs) -> AsyncResult:
        headers = {'Accept-Language': 'en-US', 'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Safari/537.36', 'Referer': 'https://chat-shared2.zhile.io/?v=2', 'sec-ch-ua': '"Google Chrome";v="117", "Not;A=Brand";v="8", "Chromium";v="117"', 'sec-ch-ua-platform': '"Linux"', 'sec-ch-ua-mobile': '?0'}
        async with ClientSession(headers=headers, cookie_jar=cls._cookie_jar) as session:
            if not cls._access_token:
                ...
                data = {'token_key': random.choice(token_ids), 'session_password': random_string()}
            ...
            headers = {'Content-Type': 'application/json', 'Accept': 'text/event-stream', 'X-Authorization': f'Bearer {cls._access_token}'}
            prompt = format_prompt(messages)
            data = {'action': 'next', 'messages': [{'id': str(uuid.uuid4()), 'author': {'role': 'user'}, 'content': {'content_type': 'text', 'parts': [prompt]}, 'metadata': {}}], 'parent_message_id': str(uuid.uuid4()), 'model': 'text-davinci-002-render-sha', 'plugin_ids': [], 'timezone_offset_min': -120, 'suggestions': [], 'history_and_training_disabled': True, 'arkose_token': '', 'force_paragen': False}
            last_message = ''
