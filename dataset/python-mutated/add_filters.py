...
from calibre.gui2.tag_mapper import RuleEdit as RuleEditBase, RuleEditDialog as RuleEditDialogBase, RuleItem as RuleItemBase, Rules as RulesBase, Tester as TesterBase, RulesDialog as RulesDialogBase
from calibre.utils.config import JSONConfig
add_filters = JSONConfig('add-filter-rules')

class RuleEdit(RuleEditBase):
    ACTION_MAP = OrderedDict((('ignore', _('Ignore')), ('add', _('Add'))))
    MATCH_TYPE_MAP = OrderedDict((('startswith', _('starts with')), ('not_startswith', _('does not start with')), ('endswith', _('ends with')), ('not_endswith', _('does not end with')), ('glob', _('matches glob pattern')), ('not_glob', _('does not match glob pattern')), ('matches', _('matches regex pattern')), ('not_matches', _('does not match regex pattern'))))
...
class Rules(RulesBase):
    ...
    MSG = _('You can specify rules to add/ignore files here. They will be used when recursively adding files from folders/archives and also when auto-adding. Click the "Add Rule" button below to get started. The rules will be processed in order for every file until either an "add" or an "ignore" rule matches. If no rules match, the file will be added only if its file extension is of a known e-book type.')
...
if __name__ == '__main__':
    app = Application([])
    d = RulesDialog()
    d.rules = [{'action': 'ignore', 'query': 'ignore-me', 'match_type': 'startswith'}, {'action': 'add', 'query': '*.moose', 'match_type': 'glob'}]
    d.exec()
