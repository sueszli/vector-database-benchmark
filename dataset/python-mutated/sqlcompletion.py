...
Column = namedtuple('Column', ['table_refs', 'require_last_table', 'local_tables', 'qualifiable', 'context'])
...
named_query_regex = re.compile('^\\s*\\\\ns\\s+[A-z0-9\\-_]+\\s+')
...
def _split_multiple_statements(full_text, text_before_cursor, parsed):
    ...
    return (full_text, text_before_cursor, statement)
SPECIALS_SUGGESTION = {'dT': Datatype, 'df': Function, 'dt': Table, 'dv': View, 'sf': Function}
