...
class TernaryDense(Layer):
    ...
    def __init__(self, n_units=100, act=None, use_gemm=False, W_init=tl.initializers.truncated_normal(stddev=0.05), b_init=tl.initializers.constant(value=0.0), in_channels=None, name=None):
        if False:
            return 10
        ...
        if self.in_channels is not None:
            ...
            self._built = True
        logging.info('TernaryDense  %s: %d %s' % (self.name, n_units, self.act.__name__ if self.act is not None else 'No Activation'))
