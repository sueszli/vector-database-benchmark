...
class sfp_spur(SpiderFootPlugin):
    meta = {'name': 'spur.us', 'summary': 'Obtain information about any malicious activities involving IP addresses found', 'flags': ['apikey'], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'website': 'https://spur.us/', 'model': 'COMMERCIAL_ONLY', 'references': ['https://spur.us/api'], 'apiKeyInstructions': ['Visit https://spur.us', 'Register an account', 'Pay for a subscription', 'Navigate to https://spur.us/app/context/tokens', "The API key is listed under 'Token'"], 'favIcon': 'https://www.google.com/s2/favicons?domain=https://spur.us', 'logo': 'https://spur.us/app/horizontal_logo.svg', 'description': 'We expose VPNs, residential proxies, botnets, anonymization behavior, geo-fraud, and more. Anonymous infrastructure has changed; it is time the security industry caught up.\nIdentify commercial and private VPN exit points along with the name of the service. We expose over 80 different commercial providers.'}}
    opts = {'api_key': '', 'checkaffiliates': True, 'subnetlookup': False, 'netblocklookup': True, 'maxnetblock': 24, 'maxsubnet': 24}
    optdescs = {'api_key': 'spur.us API Key', 'checkaffiliates': 'Check affiliates?', 'subnetlookup': 'Look up all IPs on subnets which your target is a part of?', 'netblocklookup': 'Look up all IPs on netblocks deemed to be owned by your target for possible blacklisted hosts on the same target subdomain/domain?', 'maxnetblock': 'If looking up owned netblocks, the maximum netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'maxsubnet': 'If looking up subnets, the maximum subnet size to look up all the IPs within (CIDR value, 24 = /24, 16 = /16, etc.)'}
    results = None
    errorState = False
