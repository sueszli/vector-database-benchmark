...
models = {'samantha': '1e3be7fe89e94a809408b1154a2ee3e1', 'gpt-3.5-turbo': '8077335db7cd47e29f7de486612cc7fd', 'gpt-4': '01c8de4fbfc548df903712b0922a4e01'}
...
class Myshell(AsyncGeneratorProvider):
    ...
    @classmethod
    async def create_async_generator(cls, model: str, messages: Messages, proxy: str=None, timeout: int=90, **kwargs) -> AsyncResult:
        if not model:
        ...
        async with ClientSession(headers={'User-Agent': user_agent}) as session:
            async with session.ws_connect('wss://api.myshell.ai/ws/?EIO=4&transport=websocket', autoping=False, timeout=timeout, proxy=proxy) as wss:
                ...
                chat_data = json.dumps(['text_chat', {'reqId': str(uuid.uuid4()), 'botUid': bot_id, 'sourceFrom': 'myshellWebsite', 'text': text, **generate_signature(text)}])
                chat_start = '42/chat,'
...
def generate_timestamp() -> str:
    if False:
        while True:
            i = 10
    return str(int(str(int(time.time() * 1000))[:-1] + str(sum((2 * int(digit) if idx % 2 == 0 else 3 * int(digit) for (idx, digit) in enumerate(str(int(time.time() * 1000))[:-1]))) % 10)))
...
def generate_signature(text: str):
    ...
    return {'signature': signature, 'timestamp': timestamp, 'version': version}
