...
import numpy as np
import pytest
from prefect.server.schemas.actions import BlockTypeUpdate, DeploymentCreate, DeploymentUpdate, FlowRunCreate, WorkPoolCreate, WorkPoolUpdate

@pytest.mark.parametrize('test_params,expected_dict', [({'param': 1}, {'param': 1}), ({'param': '1'}, {'param': '1'}), ({'param': {1: 2}}, {'param': {'1': 2}}), ({'df': {'col': {0: '1'}}}, {'df': {'col': {'0': '1'}}}), ({'df': {'col': {0: np.float64(1.0)}}}, {'df': {'col': {'0': 1.0}}})])
...
class TestWorkPoolCreate:
    ...
    @pytest.mark.parametrize('template', [{'job_configuration': {'thing_one': '{{ expected_variable }}'}, 'variables': {'properties': {'wrong_variable': {}}, 'required': []}}, {'job_configuration': {'thing_one': '{{ expected_variable_1 }}', 'thing_two': '{{ expected_variable_2 }}'}, 'variables': {'properties': {'not_expected_variable_1': {}, 'expected_variable_2': {}}, 'required': []}}])
    async def test_validate_base_job_template_fails(self, template):
        ...
        with pytest.raises(ValueError, match='Your job configuration uses the following undeclared variable\\(s\\): expected_variable'):
            WorkPoolCreate(name='test', base_job_template=template)

    @pytest.mark.parametrize('template', [dict(), {'job_configuration': {'thing_one': '{{ expected_variable }}'}, 'variables': {'properties': {'expected_variable': {}}, 'required': []}}])
