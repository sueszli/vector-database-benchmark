...
if is_torch_available():
    import torch
    from transformers import BLOOM_PRETRAINED_MODEL_ARCHIVE_LIST, BloomForCausalLM, BloomForQuestionAnswering, BloomForSequenceClassification, BloomForTokenClassification, BloomModel, BloomTokenizerFast
...
class BloomModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (BloomModel, BloomForCausalLM, BloomForSequenceClassification, BloomForTokenClassification, BloomForQuestionAnswering) if is_torch_available() else ()
    all_generative_model_classes = (BloomForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': BloomModel, 'question-answering': BloomForQuestionAnswering, 'text-classification': BloomForSequenceClassification, 'text-generation': BloomForCausalLM, 'token-classification': BloomForTokenClassification, 'zero-shot': BloomForSequenceClassification} if is_torch_available() else {}
    fx_compatible = True
