...
import io
import argparse
register_pattern = re.compile('\\s*(MP_REGISTER_MODULE|MP_REGISTER_EXTENSIBLE_MODULE)\\(MP_QSTR_(.*?),\\s*(.*?)\\);', flags=re.DOTALL)
delegation_pattern = re.compile('\\s*(?:MP_REGISTER_MODULE_DELEGATION)\\((.*?),\\s*(.*?)\\);', flags=re.DOTALL)
