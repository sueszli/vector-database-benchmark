...
class SourceProcessor:
    ...
    re_patterns = [(re.compile('(\\s*)#\\s*!viztracer:\\s*(log_.*?\\(.*\\))\\s*$'), line_transform), (re.compile('(.*\\S.*)#\\s*!viztracer:\\s*log\\s*$'), inline_transform), (re.compile('(\\s*)#\\s*!viztracer:\\s*(log_.*?\\(.*\\))\\s*if\\s+(.*?)\\s*$'), line_transform_condition), (re.compile('(.*\\S.*)#\\s*!viztracer:\\s*log\\s*if\\s+(.*?)\\s*$'), inline_transform_condition)]
