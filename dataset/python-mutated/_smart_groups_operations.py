...
class SmartGroupsOperations:
    ...
    async def get_by_id(self, smart_group_id: str, **kwargs: Any) -> _models.SmartGroup:
        ...
        request = build_get_by_id_request(smart_group_id=smart_group_id, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.get_by_id.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=False, **kwargs)
        response = pipeline_response.http_response
    ...
    @distributed_trace_async
    async def change_state(self, smart_group_id: str, new_state: Union[str, _models.AlertState], **kwargs: Any) -> _models.SmartGroup:
        ...
        api_version: Literal['2019-05-05-preview'] = kwargs.pop('api_version', _params.pop('api-version', '2019-05-05-preview'))
        cls: ClsType[_models.SmartGroup] = kwargs.pop('cls', None)
        request = build_change_state_request(smart_group_id=smart_group_id, subscription_id=self._config.subscription_id, new_state=new_state, api_version=api_version, template_url=self.change_state.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=False, **kwargs)
        response = pipeline_response.http_response
    ...
    async def get_history(self, smart_group_id: str, **kwargs: Any) -> _models.SmartGroupModification:
        ...
        request = build_get_history_request(smart_group_id=smart_group_id, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.get_history.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=False, **kwargs)
        response = pipeline_response.http_response
