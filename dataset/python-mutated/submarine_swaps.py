...
from .bitcoin import script_to_p2wsh, opcodes, p2wsh_nested_script, push_script, is_segwit_address, construct_witness
...
class SwapManager(Logger):
    ...
    async def pay_invoice(self, key):
        ...
        self.invoices_to_pay[key] = 1000000000000
        try:
    ...
    async def normal_swap(self, *, lightning_amount_sat: int, expected_onchain_amount_sat: int, password, tx: PartialTransaction=None, channels=None) -> str:
    ...
    async def request_normal_swap(self, lightning_amount_sat, expected_onchain_amount_sat, channels=None):
        ...
        request_data = {'invoiceAmount': lightning_amount_sat, 'refundPublicKey': refund_pubkey.hex()}
        response = await self.network.async_send_http_on_proxy('post', self.api_url + '/createnormalswap', json=request_data, timeout=30)
        ...
        if onchain_amount > expected_onchain_amount_sat:
            raise Exception(f'fswap check failed: onchain_amount is more than what we estimated: {onchain_amount} > {expected_onchain_amount_sat}')
        if locktime - self.network.get_local_height() >= 144:
            raise Exception('fswap check failed: locktime too far in future')
        (swap, invoice, _) = self.add_normal_swap(redeem_script=redeem_script, locktime=locktime, lightning_amount_sat=lightning_amount_sat, onchain_amount_sat=onchain_amount, payment_hash=payment_hash, our_privkey=refund_privkey, their_pubkey=claim_pubkey, invoice=invoice, prepay=False)
        return (swap, invoice)
    ...
    async def wait_for_htlcs_and_broadcast(self, swap, invoice, tx, channels=None):
        ...
        request_data = {'preimageHash': payment_hash.hex(), 'invoice': invoice, 'refundPublicKey': refund_pubkey.hex()}
        response = await self.network.async_send_http_on_proxy('post', self.api_url + '/addswapinvoice', json=request_data, timeout=30)
        data = json.loads(response)
    ...
    async def request_swap_for_tx(self, tx: 'PartialTransaction'):
        ...
        (swap, invoice) = await self.request_normal_swap(lightning_amount_sat=lightning_amount_sat, expected_onchain_amount_sat=change_amount)
        tx.replace_output_address(DummyAddress.SWAP, swap.lockup_address)
    ...
    async def reverse_swap(self, *, lightning_amount_sat: int, expected_onchain_amount_sat: int, channels=None) -> Optional[str]:
        ...
        preimage = os.urandom(32)
        payment_hash = sha256(preimage)
        request_data = {'type': 'reversesubmarine', 'pairId': 'BTC/BTC', 'orderSide': 'buy', 'invoiceAmount': lightning_amount_sat, 'preimageHash': payment_hash.hex(), 'claimPublicKey': our_pubkey.hex()}
        response = await self.network.async_send_http_on_proxy('post', self.api_url + '/createswap', json=request_data, timeout=30)
        data = json.loads(response)
        ...
        if onchain_amount < expected_onchain_amount_sat:
            raise Exception(f'rswap check failed: onchain_amount is less than what we expected: {onchain_amount} < {expected_onchain_amount_sat}')
        ...
        if int(invoice_amount) != lightning_amount_sat:
            raise Exception(f'rswap check failed: invoice_amount ({invoice_amount}) not what we requested ({lightning_amount_sat})')
        swap = self.add_reverse_swap(redeem_script=redeem_script, locktime=locktime, privkey=privkey, preimage=preimage, payment_hash=payment_hash, prepay_hash=prepay_hash, onchain_amount_sat=onchain_amount, lightning_amount_sat=lightning_amount_sat)
        if fee_invoice:
    ...
    async def get_pairs(self) -> None:
        ...
        from .network import Network
        try:
            response = await Network.async_send_http_on_proxy('get', self.api_url + '/getpairs', timeout=30)
