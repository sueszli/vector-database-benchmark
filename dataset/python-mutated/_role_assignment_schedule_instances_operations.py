...
def build_get_request(scope: str, role_assignment_schedule_instance_name: str, **kwargs: Any) -> HttpRequest:
    ...
    api_version: str = kwargs.pop('api_version', _params.pop('api-version', '2020-10-01'))
    accept = _headers.pop('Accept', 'application/json')
    _url = kwargs.pop('template_url', '/{scope}/providers/Microsoft.Authorization/roleAssignmentScheduleInstances/{roleAssignmentScheduleInstanceName}')
    path_format_arguments = {'scope': _SERIALIZER.url('scope', scope, 'str', skip_quote=True), 'roleAssignmentScheduleInstanceName': _SERIALIZER.url('role_assignment_schedule_instance_name', role_assignment_schedule_instance_name, 'str')}
    _url: str = _format_url_section(_url, **path_format_arguments)
    _params['api-version'] = _SERIALIZER.query('api_version', api_version, 'str')
    _headers['Accept'] = _SERIALIZER.header('accept', accept, 'str')
    return HttpRequest(method='GET', url=_url, params=_params, headers=_headers, **kwargs)
...
class RoleAssignmentScheduleInstancesOperations:
    ...
    @distributed_trace
    def list_for_scope(self, scope: str, filter: Optional[str]=None, **kwargs: Any) -> Iterable['_models.RoleAssignmentScheduleInstance']:
        if False:
            for i in range(10):
                print('nop')
    ...
    @distributed_trace
    def get(self, scope: str, role_assignment_schedule_instance_name: str, **kwargs: Any) -> _models.RoleAssignmentScheduleInstance:
        if False:
            while True:
                i = 10
        ...
        request = build_get_request(scope=scope, role_assignment_schedule_instance_name=role_assignment_schedule_instance_name, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = self._client._pipeline.run(request, stream=_stream, **kwargs)
