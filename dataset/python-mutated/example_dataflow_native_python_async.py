...
with DAG(DAG_ID, default_args=default_args, schedule='@once', start_date=datetime(2021, 1, 1), catchup=False, tags=['example', 'dataflow']) as dag:
    ...
    upload_file = LocalFilesystemToGCSOperator(task_id='upload_file_to_bucket', src=PYTHON_FILE_LOCAL_PATH, dst=PYTHON_FILE_NAME, bucket=BUCKET_NAME)
    ...
    wait_for_python_job_async_done = DataflowJobStatusSensor(task_id='wait_for_python_job_async_done', job_id="{{task_instance.xcom_pull('start_python_job_async')['dataflow_job_id']}}", expected_statuses={DataflowJobStatus.JOB_STATE_DONE}, location=LOCATION)
    ...
    def check_metric_scalar_gte(metric_name: str, value: int) -> Callable:
        ...
        return callback
    wait_for_python_job_async_metric = DataflowJobMetricsSensor(task_id='wait_for_python_job_async_metric', job_id="{{task_instance.xcom_pull('start_python_job_async')['dataflow_job_id']}}", location=LOCATION, callback=check_metric_scalar_gte(metric_name='Service-cpu_num_seconds', value=100), fail_on_terminal_state=False)
    ...
    def check_message(messages: list[dict]) -> bool:
        ...
        for message in messages:
            if 'Adding workflow start and stop steps.' in message.get('messageText', ''):
                return True
        return False
    wait_for_python_job_async_message = DataflowJobMessagesSensor(task_id='wait_for_python_job_async_message', job_id="{{task_instance.xcom_pull('start_python_job_async')['dataflow_job_id']}}", location=LOCATION, callback=check_message, fail_on_terminal_state=False)
    ...
    def check_autoscaling_event(autoscaling_events: list[dict]) -> bool:
        ...
        for autoscaling_event in autoscaling_events:
            if 'Worker pool started.' in autoscaling_event.get('description', {}).get('messageText', ''):
                return True
        return False
    wait_for_python_job_async_autoscaling_event = DataflowJobAutoScalingEventsSensor(task_id='wait_for_python_job_async_autoscaling_event', job_id="{{task_instance.xcom_pull('start_python_job_async')['dataflow_job_id']}}", location=LOCATION, callback=check_autoscaling_event, fail_on_terminal_state=False)
    delete_bucket = GCSDeleteBucketOperator(task_id='delete_bucket', bucket_name=BUCKET_NAME, trigger_rule=TriggerRule.ALL_DONE)
    create_bucket >> upload_file >> start_python_job_async >> [wait_for_python_job_async_done, wait_for_python_job_async_metric, wait_for_python_job_async_message, wait_for_python_job_async_autoscaling_event] >> delete_bucket
