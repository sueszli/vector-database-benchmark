...
from golem.apps import manager as appmanager, load_app_from_json_file, AppDefinition
...
async def test_task(work_dir: Path, task_params_path: str, app_definition: AppDefinition, resources: List[str], max_subtasks: int) -> None:
    ...
    register_environments(work_dir=str(work_dir), env_manager=env_manager, dev_mode=True)
    rtm_work_dir = work_dir / 'rtm'
    rtm_work_dir.mkdir()
    rtm = requestedtaskmanager.RequestedTaskManager(env_manager, app_manager, public_key=b'', root_path=rtm_work_dir)
    task_computer_work_dir = work_dir / 'computer'
    task_computer_work_dir.mkdir()
    task_computer = taskcomputer.NewTaskComputer(env_manager, task_computer_work_dir)
    output_dir = work_dir / 'output'
    output_dir.mkdir()
    golem_params = requestedtaskmanager.CreateTaskParams(app_id=app_definition.id, name='testtask', task_timeout=TASK_TIMEOUT, subtask_timeout=SUBTASK_TIMEOUT, output_directory=output_dir, resources=list(map(Path, resources)), max_subtasks=max_subtasks, max_price_per_hour=1, concent_enabled=False)
    ...
    computing_node = requestedtaskmanager.ComputingNodeDefinition(node_id='id', name='test')
    while await deferred_from_future(rtm.has_pending_subtasks(task_id)):
        print('Getting next subtask')
        subtask_def = await deferred_from_future(rtm.get_next_subtask(task_id, computing_node))
        print('subtask', subtask_def)
        task_header = mock.Mock(task_id=task_id, environment=app_definition.requestor_env, environment_prerequisites=env_prerequisites, subtask_timeout=SUBTASK_TIMEOUT, deadline=time.time() + TASK_TIMEOUT)
        ctd = {'subtask_id': subtask_def.subtask_id, 'extra_data': subtask_def.params, 'performance': 0, 'deadline': time.time() + SUBTASK_TIMEOUT}
        task_computer.task_given(task_header, ctd)
        for resource in subtask_def.resources:
            shutil.copy2(rtm.get_subtask_inputs_dir(task_id) / resource, task_computer.get_subtask_inputs_dir())
        (task_computer.get_subtask_inputs_dir().parent / subtask_def.subtask_id).mkdir()
        result_path = await task_computer.compute()
        print(f'result_path={result_path}')
        subtask_output_dir = rtm.get_subtask_outputs_dir(task_id, subtask_def.subtask_id)
        subtask_output_dir.mkdir()
        shutil.copy2(result_path, subtask_output_dir)
        print('Starting verification')
        verdict = await deferred_from_future(rtm.verify(task_id, subtask_def.subtask_id))
        assert verdict
...
async def _task(task_params_path, app_definition_path, resources, max_subtasks, work_dir, leave_work_dir) -> None:
    work_dir = Path(tempfile.mkdtemp(dir=work_dir))
    print('work_dir', work_dir)
    db = database.Database(model.db, fields=model.DB_FIELDS, models=model.DB_MODELS, db_dir=str(work_dir / 'database'))
    try:
        await test_task(work_dir, task_params_path, app_definition_path, list(resources), max_subtasks)
    finally:
        db.close()
