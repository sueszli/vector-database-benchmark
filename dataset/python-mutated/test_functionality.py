...
from vispy.testing import requires_application, requires_pyopengl, SkipTest, run_tests_if_main, assert_equal, assert_true
...
N = quad.shape[0] * 4
buf3 = np.row_stack([quad + (0, 0, 0), quad + (0, 1, 0), quad + (1, 1, 0), quad + (1, 0, 0)]).astype(np.float32)
buf2 = ((buf3 + 1.0) * 0.5)[:, :2]
...
buf4[:, 3] = 1.0
buf4 = buf4[:, :4]
elements = np.arange(0, N, 0.5).astype(np.uint8)[::2]
helements = None
