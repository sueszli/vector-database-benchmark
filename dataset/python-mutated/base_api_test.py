...
async def do_real_request(port, endpoint, expected_code=200, expected_json=None, request_type='GET', post_data=None, headers=None, json_response=True):
    ...
    async with ClientSession() as session:
        async with session.request(request_type, url, data=data, headers=headers, ssl=False) as response:
            (status, response) = (response.status, await response.json(content_type=None) if json_response else await response.read())
            assert status == expected_code, response
            if response is not None and expected_json is not None:
                assert expected_json == response
            return response

async def do_request(test_client, url, expected_code=200, expected_json=None, request_type='GET', post_data=None, headers=None, json_response=True, params: Optional[Dict]=None):
    post_data = post_data or {}
    data = json.dumps(path_to_str(post_data)) if isinstance(post_data, (dict, list)) else post_data
    headers = headers or {'User-Agent': 'Tribler ' + version_id}
