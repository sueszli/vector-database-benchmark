...
def comparison_test():
    ...
    if len(ret.nodes) == 1:
        ...
        dataSeed = 17
        ntrees = 17
        h2oParamsS = {'ntrees': ntrees, 'max_depth': 4, 'seed': runSeed, 'learn_rate': 0.7, 'col_sample_rate_per_tree': 0.9, 'min_rows': 5, 'score_tree_interval': ntrees + 1, 'dmatrix_type': 'sparse', 'tree_method': 'exact', 'backend': 'cpu'}
        nativeParam = {'colsample_bytree': h2oParamsS['col_sample_rate_per_tree'], 'tree_method': 'exact', 'seed': h2oParamsS['seed'], 'booster': 'gbtree', 'objective': 'binary:logistic', 'eta': h2oParamsS['learn_rate'], 'grow_policy': 'depthwise', 'alpha': 0.0, 'subsample': 1.0, 'colsample_bylevel': 1.0, 'max_delta_step': 0.0, 'min_child_weight': h2oParamsS['min_rows'], 'gamma': 0.0, 'max_depth': h2oParamsS['max_depth'], 'eval_metric': ['auc', 'aucpr']}
        nrows = 10000
        ncols = 11
        factorL = 0
        numCols = 11
        enumCols = ncols - numCols
        trainFile = pyunit_utils.genTrainFrame(nrows, numCols, enumCols=enumCols, enumFactors=factorL, miscfrac=0.5, randseed=dataSeed)
        print(trainFile)
        myX = trainFile.names
        y = 'response'
        ...
        pyunit_utils.check_xgb_var_imp(trainFile, h2oModelS, nativeTrain, nativeModel, tolerance=1e-06)
        print('Comparing H2OXGBoost results with native XGBoost result when DMatrix is set to sparse.....')
        pyunit_utils.summarizeResult_binomial(h2oPredictS, nativePred, h2oTrainTimeS, nativeTrainTime, h2oPredictTimeS, nativeScoreTime, tolerance=1e-06)
