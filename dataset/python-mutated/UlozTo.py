...
class UlozTo(SimpleDownloader):
    ...
    __version__ = '1.52'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?(uloz\\.to|ulozto\\.(cz|sk|net)|bagruj\\.cz|zachowajto\\.pl|pornfile\\.cz|pinkfile\\.cz)/(?:live/)?(?P<ID>[!\\w]+/[^/?]*)'
    __config__ = [('enabled', 'bool', 'Activated', True), ('use_premium', 'bool', 'Use premium account if available', True), ('fallback', 'bool', 'Fallback to free download if premium fails', True), ('chk_filesize', 'bool', 'Check file size', True), ('captcha', 'Image;Sound', 'Captcha recognition', 'Image'), ('max_wait', 'int', 'Reconnect if waiting time is greater than minutes', 10)]
    __description__ = 'Uloz.to downloader plugin'
    __license__ = 'GPLv3'
    __authors__ = [('zoidberg', 'zoidberg@mujmail.cz'), ('ondrej', 'git@ondrej.it'), ('astran', 'martin.hromadko@gmail.com')]
    NAME_PATTERN = '(<p>File <strong>|<title>)(?P<N>.+?)(<| \\|)'
    SIZE_PATTERN = '<span id="fileSize">.*?(?P<S>[\\d.,]+\\s[kMG]?B)</span>'
    OFFLINE_PATTERN = '<title>404 - Page not found</title>|<h1 class="h1">File (has been deleted|was banned)</h1>'
    TEMP_OFFLINE_PATTERN = '<title>500 - Internal Server Error</title>'
    URL_REPLACEMENTS = [('http://', 'https://'), ('(uloz\\.to|ulozto\\.(cz|sk|net)|bagruj\\.cz|zachowajto\\.pl|pornfile\\.cz|pinkfile\\.cz)', 'ulozto.net')]
    SIZE_REPLACEMENTS = [('([\\d.]+)\\s([kMG])B', convert_decimal_prefix)]
    CHECK_TRAFFIC = True
    ADULT_PATTERN = 'PINKfile.cz'
    PASSWD_PATTERN = '<div class="passwordProtectedFile">'
    VIPLINK_PATTERN = '<a href=".+?\\?disclaimer=1" class="linkVip">'
    TOKEN_PATTERN = '<input type="hidden" name="_token_" .*?value="(.+?)"'
