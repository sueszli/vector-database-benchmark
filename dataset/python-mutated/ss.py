    ...
    for item in o_field:
        ...
        if ':' in item:
            ...
            if key == 'timer':
                ...
                val = ast.literal_eval(re.sub('([a-z0-9\\.]+)', '"\\1"', val))
                val = {'timer_name': val[0], 'expire_time': val[1], 'retrans': val[2]}
                opts[key] = val
            if key == 'users':
                key = 'process_id'
                val = val.replace('(', '[').replace(')', ']')
                val = ast.literal_eval(re.sub('([a-z]+=[0-9]+)', '"\\1"', val))
                data = {}
                for rec in val:
                    params = {}
                    params['user'] = rec[0]
                    for i in [x for x in rec if '=' in x]:
                        (k, v) = i.split('=')
                        params[k] = v
                    data.update({params['pid']: {'user': params['user'], 'file_descriptor': params['fd']}})
                val = data
            opts[key] = val
    return opts
    ...
    if jc.utils.has_data(data):
        ...
        for entry in cleandata[1:]:
            ...
            if entry[0] not in string.whitespace:
                ...
                if re.search('ino:|uid:|sk:|users:|timer:', entry_list[-1]):
