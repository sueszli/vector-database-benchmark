...
from dagster._core.definitions.data_version import SKIP_PARTITION_DATA_VERSION_DEPENDENCY_THRESHOLD, CachingStaleStatusResolver, StaleStatus
...
parser = argparse.ArgumentParser(prog='partition_staleness', description=DESC)
parser.add_argument('--num-partitions', type=int, default=SKIP_PARTITION_DATA_VERSION_DEPENDENCY_THRESHOLD - 1, help='Set the number of partitions in `root` asset. Defaults to `SKIP_PARTITION_DATA_VERSION_DEPENDENCY_THRESHOLD`-1, which means the benchmark will test the maximum number of upstream partitions Dagster will tolerate when attempting to accurately compute staleness (exceeding this threshold means Dagster skips staleness calculation).')
parser.add_argument('--override-partition-limit', action=argparse.BooleanOptionalAction, default=True, help='Override the `SKIP_PARTITION_DATA_VERSION_DEPENDENCY_THRESHOLD` with the value of `--num-partitions`.')
