...
from transformers.testing_utils import TestCasePlus, require_torch, require_torch_fp16, slow, torch_device
...
class MegaModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels, encoder_hidden_states, encoder_attention_mask):
        ...
        output_from_past = model(next_tokens, attention_mask=next_mask, encoder_hidden_states=encoder_hidden_states, encoder_attention_mask=encoder_attention_mask, past_key_values=past_key_values, output_hidden_states=True)['hidden_states'][0]
    ...
    def create_and_check_decoder_model_with_chunking(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels, encoder_hidden_states, encoder_attention_mask):
        if False:
            return 10
    ...
    def create_and_check_bidirectionality(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            i = 10
            return i + 15
    ...
    def check_chunking_shorter_sequence(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            while True:
                i = 10
        config.use_chunking = True
    ...
    def check_chunking_longer_sequence(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            while True:
                i = 10
        config.use_chunking = True
        ...
        model.to(torch_device)
        model.eval()
        result = model(input_ids.repeat(1, 8))
        self.parent.assertEqual(result[0].shape, (self.batch_size, self.seq_length * 8, self.hidden_size))

    def check_laplace_self_attention(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            i = 10
            return i + 15
        config.attention_activation = 'laplace'
    ...
    def check_relu2_self_attention(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            print('Hello World!')
    ...
    def check_sequence_length_beyond_max_positions(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            for i in range(10):
                print('nop')
...
class MegaModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (MegaForCausalLM, MegaForMaskedLM, MegaModel, MegaForSequenceClassification, MegaForTokenClassification, MegaForMultipleChoice, MegaForQuestionAnswering) if is_torch_available() else ()
    all_generative_model_classes = (MegaForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': MegaModel, 'fill-mask': MegaForMaskedLM, 'question-answering': MegaForQuestionAnswering, 'text-classification': MegaForSequenceClassification, 'text-generation': MegaForCausalLM, 'token-classification': MegaForTokenClassification, 'zero-shot': MegaForSequenceClassification} if is_torch_available() else {}
    fx_compatible = False
    ...
    @unittest.skip(reason='Calling `self.attention_function` in `MegaMovingAverageGatedAttention.forward` changes the submodules on device 1 to device 0 (also changes `requires_grad`). No idea how this could happen for now.')
...
class MegaModelIntegrationTest(TestCasePlus):
    ...
    def test_inference_masked_lm(self):
        ...
        expected_slice = torch.tensor([[[67.8389, 10.147, -32.7148], [-11.1655, 29.1152, 23.1304], [-3.8015, 66.0397, 29.6733]]])
    ...
    def test_inference_no_head(self):
        ...
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[[1.1767, -0.6349, 2.8494], [-0.5109, -0.7745, 1.9495], [-0.3287, -0.2111, 3.3367]]])
        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=0.0001))