...
class TestIncremental(BaseTest):
    ...
    async def test_two_sequential_reads(self, connector_config: SecretDict, configured_catalog_for_incremental: ConfiguredAirbyteCatalog, docker_runner: ConnectorRunner):
        """
        ...
        if is_per_stream:
            ...
            for (stream_name, stream_state) in latest_state.items():
                ...
                state_input.append({'type': 'STREAM', 'stream': {'stream_descriptor': stream_descriptor, 'stream_state': stream_state}})
        else:
    ...
    async def test_read_sequential_slices(self, inputs: IncrementalConfig, connector_config, configured_catalog_for_incremental, docker_runner: ConnectorRunner):
        """
    ...
    async def test_state_with_abnormally_large_values(self, connector_config, configured_catalog, future_state, docker_runner: ConnectorRunner):
