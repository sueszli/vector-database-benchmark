...
from airflow.providers.amazon.aws.operators.s3 import S3CreateBucketOperator, S3CreateObjectOperator, S3DeleteBucketOperator, S3DeleteObjectsOperator
...
SAMPLE_DATA = '1,Caipirinha,Cachaca\n2,Bramble,Gin\n3,Daiquiri,Rum\n'
...
with DAG(dag_id=DAG_ID, start_date=datetime(2023, 1, 1), schedule='@once', catchup=False, tags=['example']) as dag:
    ...
    wait_cluster_available = RedshiftClusterSensor(task_id='wait_cluster_available', cluster_identifier=redshift_cluster_identifier, target_status='available', poke_interval=5, timeout=60 * 30)
    ...
    create_object = S3CreateObjectOperator(task_id='create_object', s3_bucket=s3_bucket_name, s3_key=s3_key, data=SAMPLE_DATA, replace=True)
    create_table = SQLExecuteQueryOperator(task_id='create_sample_table', conn_id=conn_id_name, sql=f'\n            CREATE TABLE IF NOT EXISTS {SQL_TABLE_NAME} (\n            cocktail_id INT NOT NULL,\n            cocktail_name VARCHAR NOT NULL,\n            base_spirit VARCHAR NOT NULL);\n          ')
    ...
    transfer_s3_to_sql = S3ToSqlOperator(task_id='transfer_s3_to_sql', s3_bucket=s3_bucket_name, s3_key=s3_key, table=SQL_TABLE_NAME, column_list=SQL_COLUMN_LIST, parser=parse_csv_to_list, sql_conn_id=conn_id_name)
    ...
    transfer_s3_to_sql_generator = S3ToSqlOperator(task_id='transfer_s3_to_sql_paser_to_generator', s3_bucket=s3_bucket_name, s3_key=s3_key, table=SQL_TABLE_NAME, column_list=SQL_COLUMN_LIST, parser=parse_csv_to_generator, sql_conn_id=conn_id_name)
    check_table = SQLTableCheckOperator(task_id='check_table', conn_id=conn_id_name, table=SQL_TABLE_NAME, checks={'row_count_check': {'check_statement': 'COUNT(*) = 6'}})
    drop_table = SQLExecuteQueryOperator(conn_id=conn_id_name, trigger_rule=TriggerRule.ALL_DONE, task_id='drop_table', sql=f'DROP TABLE {SQL_TABLE_NAME}')
    delete_s3_objects = S3DeleteObjectsOperator(trigger_rule=TriggerRule.ALL_DONE, task_id='delete_objects', bucket=s3_bucket_name, keys=s3_key)
    delete_s3_bucket = S3DeleteBucketOperator(trigger_rule=TriggerRule.ALL_DONE, task_id='delete_bucket', bucket_name=s3_bucket_name, force_delete=True)
    delete_cluster = RedshiftDeleteClusterOperator(task_id='delete_cluster', cluster_identifier=redshift_cluster_identifier, trigger_rule=TriggerRule.ALL_DONE)
    delete_sg = delete_security_group(sec_group_id=set_up_sg, sec_group_name=sg_name)
    chain(test_context, set_up_sg, create_cluster, wait_cluster_available, set_up_connection, create_bucket, create_object, create_table, transfer_s3_to_sql, transfer_s3_to_sql_generator, check_table, drop_table, delete_s3_objects, delete_s3_bucket, delete_cluster, delete_sg)
