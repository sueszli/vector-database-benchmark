...
class TFConvNextV2ModelTester:
    ...
    def get_config(self):
        if False:
            for i in range(10):
                print('nop')
        return ConvNextV2Config(num_channels=self.num_channels, hidden_sizes=self.hidden_sizes, depths=self.depths, num_stages=self.num_stages, hidden_act=self.hidden_act, is_decoder=False, initializer_range=self.initializer_range)
...
class TFConvNextV2ModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': TFConvNextV2Model, 'image-classification': TFConvNextV2ForImageClassification} if is_tf_available() else {}
    test_pruning = False
    test_onnx = False
...
class TFConvNextV2ModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        if False:
            print('Hello World!')
        return ConvNextImageProcessor.from_pretrained('facebook/convnextv2-tiny-1k-224') if is_vision_available() else None
    ...
    def test_inference_image_classification_head(self):
        ...
        self.assertTrue(np.allclose(outputs.logits[0, :3].numpy(), expected_slice, atol=0.0001))