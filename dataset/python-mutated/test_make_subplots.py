...
import pytest
from plotly.graph_objs import Annotation, Annotations, Data, Figure, Font, Layout, layout, Scene, XAxis, YAxis
...
class TestMakeSubplots(TestCase):
    ...
    def test_single_plot(self):
        if False:
            while True:
                i = 10
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.0, 1.0], anchor='y'), yaxis1=YAxis(domain=[0.0, 1.0], anchor='x')))
        self.assertEqual(tls.make_subplots().to_plotly_json(), expected.to_plotly_json())

    def test_two_row(self):
        if False:
            i = 10
            return i + 15
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.0, 1.0], anchor='y'), xaxis2=XAxis(domain=[0.0, 1.0], anchor='y2'), yaxis1=YAxis(domain=[0.575, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.0, 0.425], anchor='x2')))
    ...
    def test_two_row_bottom_left(self):
        if False:
            while True:
                i = 10
        expected = Figure(data=Data(), layout=Layout(xaxis1=layout.XAxis(domain=[0.0, 1.0], anchor='y'), xaxis2=layout.XAxis(domain=[0.0, 1.0], anchor='y2'), yaxis1=layout.YAxis(domain=[0.0, 0.425], anchor='x'), yaxis2=layout.YAxis(domain=[0.575, 1.0], anchor='x2')))
    ...
    def test_two_column(self):
        if False:
            return 10
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.0, 0.45], anchor='y'), xaxis2=XAxis(domain=[0.55, 1.0], anchor='y2'), yaxis1=YAxis(domain=[0.0, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.0, 1.0], anchor='x2')))
    ...
    def test_specs_bottom_left(self):
        if False:
            while True:
                i = 10
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.0, 0.2888888888888889], anchor='y'), xaxis2=XAxis(domain=[0.0, 0.2888888888888889], anchor='y2'), xaxis3=XAxis(domain=[0.35555555555555557, 0.6444444444444445], anchor='y3'), xaxis4=XAxis(domain=[0.7111111111111111, 1.0], anchor='y4'), yaxis1=YAxis(domain=[0.0, 0.425], anchor='x'), yaxis2=YAxis(domain=[0.575, 1.0], anchor='x2'), yaxis3=YAxis(domain=[0.575, 1.0], anchor='x3'), yaxis4=YAxis(domain=[0.575, 1.0], anchor='x4')))
        fig = tls.make_subplots(rows=2, cols=3, specs=[[{}, None, None], [{}, {}, {}]], start_cell='bottom-left')
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_specs_colspan(self):
        if False:
            i = 10
            return i + 15
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.0, 1.0], anchor='y'), xaxis2=XAxis(domain=[0.0, 0.45], anchor='y2'), xaxis3=XAxis(domain=[0.55, 1.0], anchor='y3'), xaxis4=XAxis(domain=[0.0, 0.45], anchor='y4'), xaxis5=XAxis(domain=[0.55, 1.0], anchor='y5'), yaxis1=YAxis(domain=[0.7333333333333333, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.36666666666666664, 0.6333333333333333], anchor='x2'), yaxis3=YAxis(domain=[0.36666666666666664, 0.6333333333333333], anchor='x3'), yaxis4=YAxis(domain=[0.0, 0.26666666666666666], anchor='x4'), yaxis5=YAxis(domain=[0.0, 0.26666666666666666], anchor='x5')))
    ...
    def test_specs_rowspan2(self):
        ...
        fig = tls.make_subplots(rows=3, cols=3, specs=[[{}, {}, {'rowspan': 2}], [{'colspan': 2}, None, None], [{'colspan': 3}, None, None]])
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_specs_colspan_rowpan(self):
        ...
        fig = tls.make_subplots(rows=3, cols=3, specs=[[{'colspan': 2, 'rowspan': 2}, None, {}], [None, None, {}], [{}, {}, {}]])
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_specs_is_3d(self):
        if False:
            while True:
                i = 10
        expected = Figure(data=Data(), layout=Layout(scene=Scene(domain={'y': [0.575, 1.0], 'x': [0.0, 0.45]}), scene2=Scene(domain={'y': [0.0, 0.425], 'x': [0.0, 0.45]}), xaxis1=XAxis(domain=[0.55, 1.0], anchor='y'), xaxis2=XAxis(domain=[0.55, 1.0], anchor='y2'), yaxis1=YAxis(domain=[0.575, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.0, 0.425], anchor='x2')))
        fig = tls.make_subplots(rows=2, cols=2, specs=[[{'is_3d': True}, {}], [{'is_3d': True}, {}]])
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())

    def test_specs_padding(self):
        if False:
            return 10
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.1, 0.5], anchor='y'), xaxis2=XAxis(domain=[0.5, 1.0], anchor='y2'), xaxis3=XAxis(domain=[0.0, 0.5], anchor='y3'), xaxis4=XAxis(domain=[0.5, 0.9], anchor='y4'), yaxis1=YAxis(domain=[0.5, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.7, 1.0], anchor='x2'), yaxis3=YAxis(domain=[0.0, 0.3], anchor='x3'), yaxis4=YAxis(domain=[0.0, 0.5], anchor='x4')))
        fig = tls.make_subplots(rows=2, cols=2, horizontal_spacing=0, vertical_spacing=0, specs=[[{'l': 0.1}, {'b': 0.2}], [{'t': 0.2}, {'r': 0.1}]])
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_specs_rounding_rounds_down(self):
        ...
        self.assertTrue(all((fig.layout[f"yaxis{(i if i > 1 else '')}"]['domain'][0] <= 1.0 for i in range(1, n_subplots + 1))))

    def test_specs_padding_bottom_left(self):
        if False:
            for i in range(10):
                print('nop')
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.1, 0.5], anchor='y'), xaxis2=XAxis(domain=[0.5, 1.0], anchor='y2'), xaxis3=XAxis(domain=[0.0, 0.5], anchor='y3'), xaxis4=XAxis(domain=[0.5, 0.9], anchor='y4'), yaxis1=YAxis(domain=[0.0, 0.5], anchor='x'), yaxis2=YAxis(domain=[0.2, 0.5], anchor='x2'), yaxis3=YAxis(domain=[0.5, 0.8], anchor='x3'), yaxis4=YAxis(domain=[0.5, 1.0], anchor='x4')))
        fig = tls.make_subplots(rows=2, cols=2, horizontal_spacing=0, vertical_spacing=0, specs=[[{'l': 0.1}, {'b': 0.2}], [{'t': 0.2}, {'r': 0.1}]], start_cell='bottom-left')
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())

    def test_shared_xaxes(self):
        if False:
            print('Hello World!')
        expected = Figure(data=Data(), layout={'xaxis': {'anchor': 'y', 'domain': [0.0, 0.2888888888888889], 'matches': 'x4', 'showticklabels': False}, 'xaxis2': {'anchor': 'y2', 'domain': [0.35555555555555557, 0.6444444444444445], 'matches': 'x5', 'showticklabels': False}, 'xaxis3': {'anchor': 'y3', 'domain': [0.7111111111111111, 1.0], 'matches': 'x6', 'showticklabels': False}, 'xaxis4': {'anchor': 'y4', 'domain': [0.0, 0.2888888888888889]}, 'xaxis5': {'anchor': 'y5', 'domain': [0.35555555555555557, 0.6444444444444445]}, 'xaxis6': {'anchor': 'y6', 'domain': [0.7111111111111111, 1.0]}, 'yaxis': {'anchor': 'x', 'domain': [0.575, 1.0]}, 'yaxis2': {'anchor': 'x2', 'domain': [0.575, 1.0]}, 'yaxis3': {'anchor': 'x3', 'domain': [0.575, 1.0]}, 'yaxis4': {'anchor': 'x4', 'domain': [0.0, 0.425]}, 'yaxis5': {'anchor': 'x5', 'domain': [0.0, 0.425]}, 'yaxis6': {'anchor': 'x6', 'domain': [0.0, 0.425]}})
    ...
    def test_insets(self):
        if False:
            i = 10
            return i + 15
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.0, 0.45], anchor='y'), xaxis2=XAxis(domain=[0.55, 1.0], anchor='y2'), xaxis3=XAxis(domain=[0.0, 0.45], anchor='y3'), xaxis4=XAxis(domain=[0.55, 1.0], anchor='y4'), xaxis5=XAxis(domain=[0.865, 0.955], anchor='y5'), yaxis1=YAxis(domain=[0.575, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.575, 1.0], anchor='x2'), yaxis3=YAxis(domain=[0.0, 0.425], anchor='x3'), yaxis4=YAxis(domain=[0.0, 0.425], anchor='x4'), yaxis5=YAxis(domain=[0.085, 0.2975], anchor='x5')))
        fig = tls.make_subplots(rows=2, cols=2, insets=[{'cell': (2, 2), 'l': 0.7, 'w': 0.2, 'b': 0.2, 'h': 0.5}])
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_insets_bottom_left(self):
        ...
        fig = tls.make_subplots(rows=2, cols=2, insets=[{'cell': (2, 2), 'l': 0.7, 'w': 0.2, 'b': 0.2, 'h': 0.5}], start_cell='bottom-left')
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())

    def test_insets_multiple(self):
        if False:
            return 10
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.0, 1.0], anchor='y'), xaxis2=XAxis(domain=[0.0, 1.0], anchor='y2'), xaxis3=XAxis(domain=[0.8, 1.0], anchor='y3'), xaxis4=XAxis(domain=[0.8, 1.0], anchor='y4'), yaxis1=YAxis(domain=[0.575, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.0, 0.425], anchor='x2'), yaxis3=YAxis(domain=[0.575, 1.0], anchor='x3'), yaxis4=YAxis(domain=[0.0, 0.425], anchor='x4')))
        fig = tls.make_subplots(rows=2, insets=[{'cell': (1, 1), 'l': 0.8}, {'cell': (2, 1), 'l': 0.8}])
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_insets_multiple_bottom_left(self):
        if False:
            i = 10
            return i + 15
        expected = Figure(data=Data(), layout=Layout(xaxis1=XAxis(domain=[0.0, 1.0], anchor='y'), xaxis2=XAxis(domain=[0.0, 1.0], anchor='y2'), xaxis3=XAxis(domain=[0.8, 1.0], anchor='y3'), xaxis4=XAxis(domain=[0.8, 1.0], anchor='y4'), yaxis1=YAxis(domain=[0.0, 0.425], anchor='x'), yaxis2=YAxis(domain=[0.575, 1.0], anchor='x2'), yaxis3=YAxis(domain=[0.0, 0.425], anchor='x3'), yaxis4=YAxis(domain=[0.575, 1.0], anchor='x4')))
        fig = tls.make_subplots(rows=2, insets=[{'cell': (1, 1), 'l': 0.8}, {'cell': (2, 1), 'l': 0.8}], start_cell='bottom-left')
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_subplot_titles_2x1(self):
        if False:
            while True:
                i = 10
        expected = Figure(data=Data(), layout=Layout(annotations=Annotations([Annotation(x=0.5, y=1.0, xref='paper', yref='paper', text='Title 1', showarrow=False, font=Font(size=16), xanchor='center', yanchor='bottom'), Annotation(x=0.5, y=0.375, xref='paper', yref='paper', text='Title 2', showarrow=False, font=Font(size=16), xanchor='center', yanchor='bottom')]), xaxis1=XAxis(domain=[0.0, 1.0], anchor='y'), xaxis2=XAxis(domain=[0.0, 1.0], anchor='y2'), yaxis1=YAxis(domain=[0.625, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.0, 0.375], anchor='x2')))
    ...
    def test_subplot_titles_shared_axes(self):
        if False:
            while True:
                i = 10
        expected = Figure(data=Data(), layout={'annotations': [{'font': {'size': 16}, 'showarrow': False, 'text': 'Title 1', 'x': 0.225, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 2', 'x': 0.775, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 3', 'x': 0.225, 'xanchor': 'center', 'xref': 'paper', 'y': 0.375, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 4', 'x': 0.775, 'xanchor': 'center', 'xref': 'paper', 'y': 0.375, 'yanchor': 'bottom', 'yref': 'paper'}], 'xaxis': {'anchor': 'y', 'domain': [0.0, 0.45], 'matches': 'x3', 'showticklabels': False}, 'xaxis2': {'anchor': 'y2', 'domain': [0.55, 1.0], 'matches': 'x4', 'showticklabels': False}, 'xaxis3': {'anchor': 'y3', 'domain': [0.0, 0.45]}, 'xaxis4': {'anchor': 'y4', 'domain': [0.55, 1.0]}, 'yaxis': {'anchor': 'x', 'domain': [0.625, 1.0]}, 'yaxis2': {'anchor': 'x2', 'domain': [0.625, 1.0], 'matches': 'y', 'showticklabels': False}, 'yaxis3': {'anchor': 'x3', 'domain': [0.0, 0.375]}, 'yaxis4': {'anchor': 'x4', 'domain': [0.0, 0.375], 'matches': 'y3', 'showticklabels': False}})
        fig = tls.make_subplots(rows=2, cols=2, subplot_titles=('Title 1', 'Title 2', 'Title 3', 'Title 4'), shared_xaxes=True, shared_yaxes=True)
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_subplot_titles_shared_axes_all(self):
        if False:
            while True:
                i = 10
        expected = Figure(data=Data(), layout={'annotations': [{'font': {'size': 16}, 'showarrow': False, 'text': 'Title 1', 'x': 0.225, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 2', 'x': 0.775, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 3', 'x': 0.225, 'xanchor': 'center', 'xref': 'paper', 'y': 0.375, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 4', 'x': 0.775, 'xanchor': 'center', 'xref': 'paper', 'y': 0.375, 'yanchor': 'bottom', 'yref': 'paper'}], 'xaxis': {'anchor': 'y', 'domain': [0.0, 0.45], 'matches': 'x3', 'showticklabels': False}, 'xaxis2': {'anchor': 'y2', 'domain': [0.55, 1.0], 'matches': 'x3', 'showticklabels': False}, 'xaxis3': {'anchor': 'y3', 'domain': [0.0, 0.45]}, 'xaxis4': {'anchor': 'y4', 'domain': [0.55, 1.0], 'matches': 'x3'}, 'yaxis': {'anchor': 'x', 'domain': [0.625, 1.0], 'matches': 'y3'}, 'yaxis2': {'anchor': 'x2', 'domain': [0.625, 1.0], 'matches': 'y3', 'showticklabels': False}, 'yaxis3': {'anchor': 'x3', 'domain': [0.0, 0.375]}, 'yaxis4': {'anchor': 'x4', 'domain': [0.0, 0.375], 'matches': 'y3', 'showticklabels': False}})
        fig = tls.make_subplots(rows=2, cols=2, subplot_titles=('Title 1', 'Title 2', 'Title 3', 'Title 4'), shared_xaxes='all', shared_yaxes='all')
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_subplot_titles_shared_axes_rows_columns(self):
        if False:
            while True:
                i = 10
        expected = Figure(data=Data(), layout={'annotations': [{'font': {'size': 16}, 'showarrow': False, 'text': 'Title 1', 'x': 0.225, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 2', 'x': 0.775, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 3', 'x': 0.225, 'xanchor': 'center', 'xref': 'paper', 'y': 0.375, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 4', 'x': 0.775, 'xanchor': 'center', 'xref': 'paper', 'y': 0.375, 'yanchor': 'bottom', 'yref': 'paper'}], 'xaxis': {'anchor': 'y', 'domain': [0.0, 0.45]}, 'xaxis2': {'anchor': 'y2', 'domain': [0.55, 1.0], 'matches': 'x'}, 'xaxis3': {'anchor': 'y3', 'domain': [0.0, 0.45]}, 'xaxis4': {'anchor': 'y4', 'domain': [0.55, 1.0], 'matches': 'x3'}, 'yaxis': {'anchor': 'x', 'domain': [0.625, 1.0], 'matches': 'y3'}, 'yaxis2': {'anchor': 'x2', 'domain': [0.625, 1.0], 'matches': 'y4'}, 'yaxis3': {'anchor': 'x3', 'domain': [0.0, 0.375]}, 'yaxis4': {'anchor': 'x4', 'domain': [0.0, 0.375]}})
        fig = tls.make_subplots(rows=2, cols=2, subplot_titles=('Title 1', 'Title 2', 'Title 3', 'Title 4'), shared_xaxes='rows', shared_yaxes='columns')
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_subplot_titles_irregular_layout(self):
        if False:
            while True:
                i = 10
        expected = Figure(data=Data(), layout=Layout(annotations=Annotations([Annotation(x=0.225, y=1.0, xref='paper', yref='paper', text='Title 1', showarrow=False, font=Font(size=16), xanchor='center', yanchor='bottom'), Annotation(x=0.775, y=1.0, xref='paper', yref='paper', text='Title 2', showarrow=False, font=Font(size=16), xanchor='center', yanchor='bottom'), Annotation(x=0.5, y=0.375, xref='paper', yref='paper', text='Title 3', showarrow=False, font=Font(size=16), xanchor='center', yanchor='bottom')]), xaxis1=XAxis(domain=[0.0, 0.45], anchor='y'), xaxis2=XAxis(domain=[0.55, 1.0], anchor='y2'), xaxis3=XAxis(domain=[0.0, 1.0], anchor='y3'), yaxis1=YAxis(domain=[0.625, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.625, 1.0], anchor='x2'), yaxis3=YAxis(domain=[0.0, 0.375], anchor='x3')))
        fig = tls.make_subplots(rows=2, cols=2, subplot_titles=('Title 1', 'Title 2', 'Title 3'), specs=[[{}, {}], [{'colspan': 2}, None]])
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())

    def test_subplot_titles_insets(self):
        if False:
            return 10
        expected = Figure(data=Data(), layout=Layout(annotations=Annotations([Annotation(x=0.85, y=1.0, xref='paper', yref='paper', text='Inset', showarrow=False, font=Font(size=16), xanchor='center', yanchor='bottom')]), xaxis1=XAxis(domain=[0.0, 1.0], anchor='y'), xaxis2=XAxis(domain=[0.7, 1.0], anchor='y2'), yaxis1=YAxis(domain=[0.0, 1.0], anchor='x'), yaxis2=YAxis(domain=[0.3, 1.0], anchor='x2')))
        fig = tls.make_subplots(insets=[{'cell': (1, 1), 'l': 0.7, 'b': 0.3}], subplot_titles=('', 'Inset'))
        self.assertEqual(fig, expected)

    def test_subplot_titles_array(self):
        if False:
            return 10
        expected = tls.make_subplots(insets=[{'cell': (1, 1), 'l': 0.7, 'b': 0.3}], subplot_titles=('', 'Inset'))
        fig = tls.make_subplots(insets=[{'cell': (1, 1), 'l': 0.7, 'b': 0.3}], subplot_titles=['', 'Inset'])
        self.assertEqual(fig, expected)
    ...
    def test_subplot_titles_empty(self):
        ...
        fig = tls.make_subplots(insets=[{'cell': (1, 1), 'l': 0.7, 'b': 0.3}], subplot_titles=[])
        self.assertEqual(fig, expected)
    ...
    def test_large_columns_no_errors(self):
        ...
        fig = tls.make_subplots(100, 1, vertical_spacing=v_space, specs=[[{'is_3d': True}] for _ in range(100)])

    def test_row_width_and_column_width(self):
        if False:
            while True:
                i = 10
        expected = Figure({'data': [], 'layout': {'annotations': [{'font': {'size': 16}, 'showarrow': False, 'text': 'Title 1', 'x': 0.405, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 2', 'x': 0.9550000000000001, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 3', 'x': 0.405, 'xanchor': 'center', 'xref': 'paper', 'y': 0.1875, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 4', 'x': 0.9550000000000001, 'xanchor': 'center', 'xref': 'paper', 'y': 0.1875, 'yanchor': 'bottom', 'yref': 'paper'}], 'xaxis': {'anchor': 'y', 'domain': [0.0, 0.81]}, 'xaxis2': {'anchor': 'y2', 'domain': [0.91, 1.0]}, 'xaxis3': {'anchor': 'y3', 'domain': [0.0, 0.81]}, 'xaxis4': {'anchor': 'y4', 'domain': [0.91, 1.0]}, 'yaxis': {'anchor': 'x', 'domain': [0.4375, 1.0]}, 'yaxis2': {'anchor': 'x2', 'domain': [0.4375, 1.0]}, 'yaxis3': {'anchor': 'x3', 'domain': [0.0, 0.1875]}, 'yaxis4': {'anchor': 'x4', 'domain': [0.0, 0.1875]}}})
        fig = tls.make_subplots(rows=2, cols=2, subplot_titles=('Title 1', 'Title 2', 'Title 3', 'Title 4'), row_heights=[3, 1], column_widths=[9, 1])
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
        fig = tls.make_subplots(rows=2, cols=2, subplot_titles=('Title 1', 'Title 2', 'Title 3', 'Title 4'), row_width=[1, 3], column_width=[9, 1])
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_row_width_and_shared_yaxes(self):
        if False:
            for i in range(10):
                print('nop')
        expected = Figure({'data': [], 'layout': {'annotations': [{'font': {'size': 16}, 'showarrow': False, 'text': 'Title 1', 'x': 0.225, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 2', 'x': 0.775, 'xanchor': 'center', 'xref': 'paper', 'y': 1.0, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 3', 'x': 0.225, 'xanchor': 'center', 'xref': 'paper', 'y': 0.1875, 'yanchor': 'bottom', 'yref': 'paper'}, {'font': {'size': 16}, 'showarrow': False, 'text': 'Title 4', 'x': 0.775, 'xanchor': 'center', 'xref': 'paper', 'y': 0.1875, 'yanchor': 'bottom', 'yref': 'paper'}], 'xaxis': {'anchor': 'y', 'domain': [0.0, 0.45]}, 'xaxis2': {'anchor': 'y2', 'domain': [0.55, 1.0]}, 'xaxis3': {'anchor': 'y3', 'domain': [0.0, 0.45]}, 'xaxis4': {'anchor': 'y4', 'domain': [0.55, 1.0]}, 'yaxis': {'anchor': 'x', 'domain': [0.4375, 1.0]}, 'yaxis2': {'anchor': 'x2', 'domain': [0.4375, 1.0], 'matches': 'y', 'showticklabels': False}, 'yaxis3': {'anchor': 'x3', 'domain': [0.0, 0.1875]}, 'yaxis4': {'anchor': 'x4', 'domain': [0.0, 0.1875], 'matches': 'y3', 'showticklabels': False}}})
        fig = tls.make_subplots(rows=2, cols=2, row_heights=[3, 1], shared_yaxes=True, subplot_titles=('Title 1', 'Title 2', 'Title 3', 'Title 4'))
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_secondary_y(self):
        ...
        expected = Figure({'data': [], 'layout': {'xaxis': {'anchor': 'y', 'domain': [0.0, 0.94]}, 'yaxis': {'anchor': 'x', 'domain': [0.0, 1.0]}, 'yaxis2': {'anchor': 'x', 'overlaying': 'y', 'side': 'right'}}})
        self.assertEqual(fig.to_plotly_json(), expected.to_plotly_json())
    ...
    def test_secondary_y_traces(self):
        ...
        expected = Figure({'data': [{'name': 'First', 'type': 'scatter', 'y': [1, 3, 2], 'xaxis': 'x', 'yaxis': 'y'}, {'name': 'second', 'type': 'scatter', 'y': [2, 1, 3], 'xaxis': 'x', 'yaxis': 'y2'}], 'layout': {'xaxis': {'anchor': 'y', 'domain': [0.0, 0.94]}, 'yaxis': {'anchor': 'x', 'domain': [0.0, 1.0]}, 'yaxis2': {'anchor': 'x', 'overlaying': 'y', 'side': 'right'}}})
    ...
    def test_secondary_y_subplots(self):
        if False:
            i = 10
            return i + 15
        fig = subplots.make_subplots(rows=2, cols=2, specs=[[{'secondary_y': True}, {'secondary_y': True}], [{'secondary_y': True}, {'secondary_y': True}]])
        fig.add_scatter(y=[1, 3, 2], name='First', row=1, col=1)
        fig.add_scatter(y=[2, 1, 3], name='Second', row=1, col=1, secondary_y=True)
...
def test_make_subplots_spacing_error():
    if False:
        while True:
            i = 10
    for match in [('^%s spacing cannot be greater than \\(1 / \\(%s - 1\\)\\) = %f.' % ('Vertical', 'rows', 1.0 / 50.0)).replace('.', '\\.'), 'The resulting plot would have 51 rows \\(rows=51\\)\\.$']:
        with pytest.raises(ValueError, match=match):
            fig = subplots.make_subplots(51, 1, vertical_spacing=0.0201)
    for match in [('^%s spacing cannot be greater than \\(1 / \\(%s - 1\\)\\) = %f.' % ('Horizontal', 'cols', 1.0 / 50.0)).replace('.', '\\.'), 'The resulting plot would have 51 columns \\(cols=51\\)\\.$']:
        with pytest.raises(ValueError, match=match):
            fig = subplots.make_subplots(1, 51, horizontal_spacing=0.0201)
    try:
        fig = subplots.make_subplots(51, 1, vertical_spacing=0.02)
    except ValueError:
        assert False
    try:
        fig = subplots.make_subplots(1, 51, horizontal_spacing=0.02)
    except ValueError:
        assert False
    try:
    ...
    with pytest.raises(ValueError, match='^Vertical spacing must be between 0 and 1\\.$'):
