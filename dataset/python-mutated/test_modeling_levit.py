...
if is_torch_available():
    import torch
    from transformers import MODEL_FOR_IMAGE_CLASSIFICATION_MAPPING, MODEL_MAPPING, LevitForImageClassification, LevitForImageClassificationWithTeacher, LevitModel
...
class LevitModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    """
    all_model_classes = (LevitModel, LevitForImageClassification, LevitForImageClassificationWithTeacher) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': LevitModel, 'image-classification': (LevitForImageClassification, LevitForImageClassificationWithTeacher)} if is_torch_available() else {}
    test_pruning = False
