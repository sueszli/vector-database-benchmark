...
import base64
import asyncio
from azure.security.attestation.aio import AttestationAdministrationClient, AttestationClient
from azure.security.attestation import AttestationType, AttestationPolicyToken, CertificateModification
...
class AttestationClientPolicySamples(object):
    ...
    async def get_policy_aad(self):
        ...
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(os.environ.get('ATTESTATION_AAD_URL'), credential) as admin_client:
    ...
    async def set_policy_aad_unsecured(self):
        ...
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(os.environ.get('ATTESTATION_AAD_URL'), credential) as admin_client:
            ...
            (set_result, _) = await admin_client.set_policy(AttestationType.OPEN_ENCLAVE, new_policy)
    ...
    async def reset_policy_aad_secured(self):
        ...
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(os.environ.get('ATTESTATION_AAD_URL'), credential, signing_key=rsa_key, signing_certificate=cert) as admin_client:
    ...
    async def set_policy_aad_secured(self):
        ...
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(os.environ.get('ATTESTATION_AAD_URL'), credential) as admin_client:
            rsa_key = create_rsa_key()
            cert = create_x509_certificate(rsa_key, u'TestCertificate')
            (set_result, _) = await admin_client.set_policy(AttestationType.SGX_ENCLAVE, 'version= 1.0;authorizationrules{=> permit();};issuancerules {};', signing_key=rsa_key, signing_certificate=cert)
            print('Policy Set Resolution: ', set_result.policy_resolution)
    ...
    async def set_policy_validate_hash(self):
        ...
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(os.environ.get('ATTESTATION_AAD_URL'), credential) as admin_client:
            ...
            (set_result, _) = await admin_client.set_policy(AttestationType.SGX_ENCLAVE, policy_to_set, signing_key=rsa_key, signing_certificate=cert)
            print('Policy Set Resolution: ', set_result.policy_resolution)
            print('Resulting policy signer should match the input certificate:')
            print('Policy Signer: ', set_result.policy_signer.certificates[0])
            print('Certificate:   ', cert)
    ...
    async def reset_policy_isolated(self):
        ...
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(os.environ.get('ATTESTATION_ISOLATED_URL'), credential) as admin_client:
            (set_result, _) = await admin_client.reset_policy(AttestationType.SGX_ENCLAVE, signing_key=isolated_key, signing_certificate=isolated_certificate)
            print('Policy Set Resolution: ', set_result.policy_resolution)
    ...
    async def get_policy_isolated(self):
        ...
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(endpoint, credential) as admin_client:
            (get_result, _) = await admin_client.get_policy(AttestationType.SGX_ENCLAVE, validate_issuer=True, issuer=endpoint)
    ...
    async def set_policy_isolated_secured(self):
        ...
        print('Set Secured Policy on an AAD mode attestation instance.')
        endpoint = os.environ.get('ATTESTATION_ISOLATED_URL')
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(endpoint, credential, signing_key=self.isolated_key, signing_certificate=self.isolated_certificate) as admin_client:
            (set_result, _) = await admin_client.set_policy(AttestationType.SGX_ENCLAVE, 'version= 1.0;authorizationrules{=> permit();};issuancerules {};', validation_slack=1.0)
            print('Policy Set Resolution: ', set_result.policy_resolution)
            print('Resulting policy signer should match the input certificate:')
            print('Policy Signer: ', set_result.policy_signer.certificates[0])
    ...
    async def add_remove_policy_management_certificate(self):
        ...
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(endpoint, credential) as admin_client:
            ...
            (add_result, _) = await admin_client.add_policy_management_certificate(new_certificate, signing_key=self.isolated_key, signing_certificate=self.isolated_certificate, validation_slack=1.0)
        ...
        async with DefaultAzureCredential() as credential, AttestationAdministrationClient(endpoint, credential) as admin_client:
            (remove_result, _) = await admin_client.remove_policy_management_certificate(new_certificate, signing_key=self.isolated_key, signing_certificate=self.isolated_certificate)
    ...
    async def _attest_open_enclave(self, client_uri):
        ...
        async with DefaultAzureCredential() as credential, AttestationClient(client_uri, credential) as attest_client:
            await attest_client.attest_open_enclave(oe_report, runtime_data=runtime_data)
