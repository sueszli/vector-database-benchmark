...
import yaml
COMMON_ENV_VARIABLES = {'OMP_NUM_THREADS': 1, 'TRANSFORMERS_IS_CI': True, 'PYTEST_TIMEOUT': 120, 'RUN_PIPELINE_TESTS': False, 'RUN_PT_TF_CROSS_TESTS': False, 'RUN_PT_FLAX_CROSS_TESTS': False}
...
torch_and_tf_job = CircleCIJob('torch_and_tf', additional_env={'RUN_PT_TF_CROSS_TESTS': True}, install_steps=['sudo apt-get -y update && sudo apt-get install -y libsndfile1-dev espeak-ng git-lfs cmake', 'git lfs install', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[sklearn,tf-cpu,torch,testing,sentencepiece,torch-speech,vision]', 'pip install -U --upgrade-strategy eager tensorflow_probability', 'pip install -U --upgrade-strategy eager -e git+https://github.com/huggingface/accelerate@main#egg=accelerate'], marker='is_pt_tf_cross_test', pytest_options={'rA': None, 'durations': 0})
torch_and_flax_job = CircleCIJob('torch_and_flax', additional_env={'RUN_PT_FLAX_CROSS_TESTS': True}, install_steps=['sudo apt-get -y update && sudo apt-get install -y libsndfile1-dev espeak-ng', 'pip install -U --upgrade-strategy eager --upgrade pip', 'pip install -U --upgrade-strategy eager .[sklearn,flax,torch,testing,sentencepiece,torch-speech,vision]', 'pip install -U --upgrade-strategy eager -e git+https://github.com/huggingface/accelerate@main#egg=accelerate'], marker='is_pt_flax_cross_test', pytest_options={'rA': None, 'durations': 0})
torch_job = CircleCIJob('torch', install_steps=['sudo apt-get -y update && sudo apt-get install -y libsndfile1-dev espeak-ng time', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[sklearn,torch,testing,sentencepiece,torch-speech,vision,timm]', 'pip install -U --upgrade-strategy eager -e git+https://github.com/huggingface/accelerate@main#egg=accelerate'], parallelism=1, pytest_num_workers=6)
tf_job = CircleCIJob('tf', install_steps=['sudo apt-get -y update && sudo apt-get install -y libsndfile1-dev espeak-ng cmake', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[sklearn,tf-cpu,testing,sentencepiece,tf-speech,vision]', 'pip install -U --upgrade-strategy eager tensorflow_probability'], parallelism=1)
flax_job = CircleCIJob('flax', install_steps=['sudo apt-get -y update && sudo apt-get install -y libsndfile1-dev espeak-ng', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[flax,testing,sentencepiece,flax-speech,vision]'], parallelism=1)
pipelines_torch_job = CircleCIJob('pipelines_torch', additional_env={'RUN_PIPELINE_TESTS': True}, install_steps=['sudo apt-get -y update && sudo apt-get install -y libsndfile1-dev espeak-ng', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[sklearn,torch,testing,sentencepiece,torch-speech,vision,timm,video]'], marker='is_pipeline_test', pytest_num_workers=6)
pipelines_tf_job = CircleCIJob('pipelines_tf', additional_env={'RUN_PIPELINE_TESTS': True}, install_steps=['sudo apt-get -y update && sudo apt-get install -y cmake', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[sklearn,tf-cpu,testing,sentencepiece,vision]', 'pip install -U --upgrade-strategy eager tensorflow_probability'], marker='is_pipeline_test')
custom_tokenizers_job = CircleCIJob('custom_tokenizers', additional_env={'RUN_CUSTOM_TOKENIZERS': True}, install_steps=['sudo apt-get -y update && sudo apt-get install -y cmake', {'name': 'install jumanpp', 'command': 'wget https://github.com/ku-nlp/jumanpp/releases/download/v2.0.0-rc3/jumanpp-2.0.0-rc3.tar.xz\ntar xvf jumanpp-2.0.0-rc3.tar.xz\nmkdir jumanpp-2.0.0-rc3/bld\ncd jumanpp-2.0.0-rc3/bld\nsudo cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr/local\nsudo make install\n'}, 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[ja,testing,sentencepiece,jieba,spacy,ftfy,rjieba]', 'python -m unidic download'], parallelism=None, resource_class=None, tests_to_run=['./tests/models/bert_japanese/test_tokenization_bert_japanese.py', './tests/models/openai/test_tokenization_openai.py', './tests/models/clip/test_tokenization_clip.py'])
examples_torch_job = CircleCIJob('examples_torch', additional_env={'OMP_NUM_THREADS': 8}, cache_name='torch_examples', install_steps=['sudo apt-get -y update && sudo apt-get install -y libsndfile1-dev espeak-ng', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[sklearn,torch,sentencepiece,testing,torch-speech]', 'pip install -U --upgrade-strategy eager -r examples/pytorch/_tests_requirements.txt', 'pip install -U --upgrade-strategy eager -e git+https://github.com/huggingface/accelerate@main#egg=accelerate'], pytest_num_workers=1)
examples_tensorflow_job = CircleCIJob('examples_tensorflow', cache_name='tensorflow_examples', install_steps=['sudo apt-get -y update && sudo apt-get install -y cmake', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[sklearn,tensorflow,sentencepiece,testing]', 'pip install -U --upgrade-strategy eager -r examples/tensorflow/_tests_requirements.txt'])
examples_flax_job = CircleCIJob('examples_flax', cache_name='flax_examples', install_steps=['pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[flax,testing,sentencepiece]', 'pip install -U --upgrade-strategy eager -r examples/flax/_tests_requirements.txt'])
hub_job = CircleCIJob('hub', additional_env={'HUGGINGFACE_CO_STAGING': True}, install_steps=['sudo apt-get -y update && sudo apt-get install git-lfs', 'git config --global user.email "ci@dummy.com"', 'git config --global user.name "ci"', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[torch,sentencepiece,testing,vision]'], marker='is_staging_test', pytest_num_workers=1)
onnx_job = CircleCIJob('onnx', install_steps=['sudo apt-get -y update && sudo apt-get install -y cmake', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[torch,tf,testing,sentencepiece,onnxruntime,vision,rjieba]'], pytest_options={'k onnx': None}, pytest_num_workers=1)
exotic_models_job = CircleCIJob('exotic_models', install_steps=['sudo apt-get -y update && sudo apt-get install -y libsndfile1-dev', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[torch,testing,vision]', 'pip install -U --upgrade-strategy eager torchvision', 'pip install -U --upgrade-strategy eager scipy', "pip install -U --upgrade-strategy eager 'git+https://github.com/facebookresearch/detectron2.git'", 'sudo apt install tesseract-ocr', 'pip install -U --upgrade-strategy eager pytesseract', 'pip install -U --upgrade-strategy eager natten', 'pip install -U --upgrade-strategy eager python-Levenshtein', 'pip install -U --upgrade-strategy eager opencv-python', 'pip install -U --upgrade-strategy eager nltk'], tests_to_run=['tests/models/*layoutlmv*', 'tests/models/*nat', 'tests/models/deta', 'tests/models/nougat'], pytest_num_workers=1, pytest_options={'durations': 100})
repo_utils_job = CircleCIJob('repo_utils', install_steps=['pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager .[quality,testing,torch]'], parallelism=None, pytest_num_workers=1, resource_class='large', tests_to_run='tests/repo_utils')
py_command = 'from utils.tests_fetcher import get_doctest_files; to_test = get_doctest_files() + ["dummy.py"]; to_test = " ".join(to_test); print(to_test)'
py_command = f"$(python3 -c '{py_command}')"
command = f'echo "{py_command}" > pr_documentation_tests_temp.txt'
doc_test_job = CircleCIJob('pr_documentation_tests', additional_env={'TRANSFORMERS_VERBOSITY': 'error', 'DATASETS_VERBOSITY': 'error', 'SKIP_CUDA_DOCTEST': '1'}, install_steps=['sudo apt-get -y update && sudo apt-get install -y libsndfile1-dev espeak-ng time ffmpeg', 'pip install --upgrade --upgrade-strategy eager pip', 'pip install -U --upgrade-strategy eager -e .[dev]', 'pip install -U --upgrade-strategy eager -e git+https://github.com/huggingface/accelerate@main#egg=accelerate', 'pip install --upgrade --upgrade-strategy eager pytest pytest-sugar', 'pip install -U --upgrade-strategy eager natten', 'find -name __pycache__ -delete', 'find . -name \\*.pyc -delete', 'touch dummy.py', {'name': 'Get files to test', 'command': command}, {'name': 'Show information in `Get files to test`', 'command': 'cat pr_documentation_tests_temp.txt'}, {'name': 'Get the last line in `pr_documentation_tests.txt`', 'command': 'tail -n1 pr_documentation_tests_temp.txt | tee pr_documentation_tests.txt'}], tests_to_run='$(cat pr_documentation_tests.txt)', pytest_options={'-doctest-modules': None, 'doctest-glob': '*.md', 'dist': 'loadfile', 'rvsA': None}, command_timeout=1200, pytest_num_workers=1)
REGULAR_TESTS = [torch_and_tf_job, torch_and_flax_job, torch_job, tf_job, flax_job, custom_tokenizers_job, hub_job, onnx_job, exotic_models_job]
EXAMPLES_TESTS = [examples_torch_job, examples_tensorflow_job, examples_flax_job]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fetcher_folder', type=str, default=None, help='Only test that all tests and modules are accounted for.')
