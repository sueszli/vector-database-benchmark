...
from ..utils import int_or_none, str_to_int, try_get, url_or_none, unified_strdate, parse_duration

class TedBaseIE(InfoExtractor):
    _VALID_URL_BASE = 'https?://www\\.ted\\.com/(?:{type})(?:/lang/[^/#?]+)?/(?P<id>[\\w-]+)'
...
class TedTalkIE(TedBaseIE):
    ...
    _TESTS = [{'url': 'https://www.ted.com/talks/candace_parker_how_to_break_down_barriers_and_not_accept_limits', 'md5': '47e82c666d9c3261d4fe74748a90aada', 'info_dict': {'id': '86532', 'ext': 'mp4', 'title': 'How to break down barriers and not accept limits', 'description': 'md5:000707cece219d1e165b11550d612331', 'view_count': int, 'tags': ['personal growth', 'equality', 'activism', 'motivation', 'social change', 'sports'], 'uploader': 'Candace Parker', 'duration': 676.0, 'upload_date': '20220114', 'release_date': '20211201', 'thumbnail': 're:http.*\\.jpg'}}]
...
class TedPlaylistIE(TedBaseIE):
    _VALID_URL = TedBaseIE._VALID_URL_BASE.format(type='playlists(?:/\\d+)?')
    _TESTS = [{'url': 'https://www.ted.com/playlists/171/the_most_popular_talks_of_all', 'info_dict': {'id': '171', 'title': 'The most popular talks of all time', 'description': 'md5:d2f22831dc86c7040e733a3cb3993d78'}, 'playlist_mincount': 25}]
...
class TedEmbedIE(InfoExtractor):
    ...
    _EMBED_REGEX = [f"""<iframe[^>]+?src=(["\\'])(?P<url>{_VALID_URL}.+?)\\1"""]
    _TESTS = [{'url': 'https://embed.ted.com/talks/janet_stovall_how_to_get_serious_about_diversity_and_inclusion_in_the_workplace', 'info_dict': {'id': '21802', 'ext': 'mp4', 'title': 'How to get serious about diversity and inclusion in the workplace', 'description': 'md5:0978aafe396e05341f8ecc795d22189d', 'view_count': int, 'tags': list, 'uploader': 'Janet Stovall', 'duration': 664.0, 'upload_date': '20180822', 'release_date': '20180719', 'thumbnail': 're:http.*\\.jpg'}}]
