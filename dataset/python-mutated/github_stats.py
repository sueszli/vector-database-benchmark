...
from gh_api import get_paged_request, make_auth_header, get_pull_request, is_pull_request, get_milestone_id, get_issues_list, get_authors
...
MILESTONE_TEMPLATE = 'The full list can be seen `on GitHub <https://github.com/{project}/milestone/{milestone_id}?closed=1>`__\n'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--since-tag', type=str, help='The git tag to use for the starting point (typically the last major release).')
    parser.add_argument('--milestone', type=str, help='The GitHub milestone to use for filtering issues [optional].')
    parser.add_argument('--days', type=int, help='The number of days of data to summarize (use this or --since-tag).')
    parser.add_argument('--project', type=str, default='matplotlib/matplotlib', help='The project to summarize.')
    parser.add_argument('--links', action='store_true', default=False, help='Include links to all closed Issues and PRs in the output.')
    ...
    if milestone:
        ...
        issues_and_pulls = get_issues_list(project=project, milestone=milestone_id, state='closed', auth=True)
    ...
    title = f"GitHub statistics for {milestone.lstrip('v')} {today.strftime('(%b %d, %Y)')}"
    ncommits = 0
    all_authors = []
    if tag:
        since_tag = f'{tag}..'
        ...
        all_authors.extend(check_output(author_cmd, encoding='utf-8', errors='replace').splitlines())
    pr_authors = []
    for pr in pulls:
    ...
    if milestone:
        ...
        milestone_str = ''
    if opts.links:
        links = LINKS_TEMPLATE.format(n_pulls=n_pulls, pull_request_report=report(pulls, show_urls), n_issues=n_issues, issue_report=report(issues, show_urls))
    else:
        links = ''
    print(REPORT_TEMPLATE.format(title=title, title_underline='=' * len(title), since_day=since_day, tag=tag, today=today.strftime('%Y/%m/%d'), n_issues=n_issues, n_pulls=n_pulls, milestone=milestone_str, nauthors=len(unique_authors), ncommits=ncommits, unique_authors='\n'.join(unique_authors), links=links))