...
class dns_dnssec_disabled(Check):
    ...
    def execute(self) -> Check_Report_GCP:
        ...
        for managed_zone in dns_client.managed_zones:
            ...
            report.status = 'PASS'
            report.status_extended = f'Cloud DNS {managed_zone.name} has DNSSEC enabled.'
            if not managed_zone.dnssec:
                report.status = 'FAIL'
                report.status_extended = f"Cloud DNS {managed_zone.name} doesn't have DNSSEC enabled."
