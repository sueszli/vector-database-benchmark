...
from mage_ai.authentication.operation_history.utils import load_pipelines_detail_async, record_create_pipeline_async, record_detail_pipeline_async
from mage_ai.data_preparation.models.constants import BlockLanguage, BlockType, PipelineStatus
from mage_ai.data_preparation.models.custom_templates.custom_pipeline_template import CustomPipelineTemplate
from mage_ai.data_preparation.models.pipeline import Pipeline
from mage_ai.data_preparation.models.project import Project
from mage_ai.data_preparation.models.project.constants import FeatureUUID
from mage_ai.data_preparation.models.triggers import ScheduleStatus, get_trigger_configs_by_name, update_triggers_for_pipeline_and_persist
...
class PipelineResource(BaseResource):
    ...
    async def collection(self, query, meta, user, **kwargs):
        ...
        if from_history_days is not None and is_number(from_history_days):
            timestamp_start = (datetime.utcnow() - timedelta(hours=24 * int(from_history_days))).timestamp()
            history = await load_pipelines_detail_async(timestamp_start=timestamp_start)
            history = sorted(history, key=lambda x: int(x.timestamp), reverse=True)
            for h in history:
        ...
        for pipeline in pipelines:
            ...
            if pipeline_statuses and (PipelineStatus.ACTIVE in pipeline_statuses and any((s.status == ScheduleStatus.ACTIVE for s in pipeline.schedules)) or (PipelineStatus.INACTIVE in pipeline_statuses and len(pipeline.schedules) > 0 and all((s.status == ScheduleStatus.INACTIVE for s in pipeline.schedules))) or (PipelineStatus.NO_SCHEDULES in pipeline_statuses and len(pipeline.schedules) == 0)):
        ...
        return self.build_result_set(pipelines, user, **kwargs)
    ...
    async def create(self, payload, user, **kwargs):
        ...
        if template_uuid:
            ...
            pipeline = Pipeline.create(name, pipeline_type=pipeline_type, repo_path=get_repo_path())
            if llm_payload:
                ...
                if LLMUseCase.GENERATE_PIPELINE_WITH_DESCRIPTION == llm_use_case:
                    ...
                    for (block_number, block_payload_orig) in llm_response.items():
                        ...
                        block_resource = await BlockResource.create(merge_dict(dict(name=block_uuid, type=block_payload.get('block_type')), ignore_keys(block_payload, ['block_type', 'upstream_blocks'])), user, **merge_dict(kwargs, dict(parent_model=pipeline)))
                        upstream_block_uuids = block_payload.get('upstream_blocks')
                        pipeline.add_block(block_resource.model, None, priority=len(upstream_block_uuids) if upstream_block_uuids else 0, widget=False)
                        blocks_mapping[block_number] = dict(block=block_resource.model, upstream_block_uuids=upstream_block_uuids)
        ...
        if pipeline:
            await UsageStatisticLogger().pipeline_create(pipeline, clone_pipeline_uuid=clone_pipeline_uuid, llm_payload=llm_payload, template_uuid=template_uuid)
        await record_create_pipeline_async(resource_uuid=pipeline.uuid, user=user.id if user else None)
    ...
    async def member(self, pk, user, **kwargs):
        ...
        if api_operation_action != DELETE:
            ...
            switch_active_kernel(kernel_name, emr_config=pipeline.executor_config if kernel_name == KernelName.PYSPARK else None)
        if api_operation_action == DETAIL:
            if Project(pipeline.repo_config).is_feature_enabled(FeatureUUID.OPERATION_HISTORY):
                await record_detail_pipeline_async(resource_uuid=pipeline.uuid, user=user.id if user else None)
    ...
    async def update(self, payload, **kwargs):
        if 'add_upstream_for_block_uuid' in payload:
            ...
            if BlockType.DBT == block.type and block.language == BlockLanguage.SQL:
                upstream_dbt_blocks_by_uuid = {block.uuid: block for block in block.upstream_dbt_blocks()}
        ...
        if llm_payload:
            ...
            async def _add_markdown_block(block_doc: str, block_uuid: str, priority: int):
                return await BlockResource.create(dict(content=block_doc.strip() if block_doc else block_doc, language=BlockLanguage.MARKDOWN, name=f'Documentation for {block_uuid}', priority=priority, type=BlockType.MARKDOWN), self.current_user, **merge_dict(kwargs, dict(parent_model=self.model)))
        ...
        await self.model.update(ignore_keys(payload, ['add_upstream_for_block_uuid']), update_content=update_content)
        try:
            kernel_name = PIPELINE_TO_KERNEL_NAME[self.model.type]
            switch_active_kernel(kernel_name, emr_config=self.model.executor_config if kernel_name == KernelName.PYSPARK else None)
