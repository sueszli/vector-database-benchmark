...
from .executors import AsyncExecutor, AsyncioSimpleExecutor, AsyncioProgressbarQueueExecutor
from .result import QueryResult, QueryStatus
from .sites import MaigretDatabase, MaigretSite
from .types import QueryOptions, QueryResultWrapper
from .utils import get_random_user_agent, ascii_data_display
SUPPORTED_IDS = ('username', 'yandex_public_id', 'gaia_id', 'vk_id', 'ok_id', 'wikimapia_uid', 'steam_id', 'uidme_uguid', 'yelp_userid')
BAD_CHARS = '#'
...
class SimpleAiohttpChecker(CheckerBase):
    ...
    async def check(self, future) -> Tuple[str, int, Optional[CheckError]]:
        ...
        try:
            ...
            if sys.version_info.minor > 6 and (isinstance(e, ssl.SSLCertVerificationError) or isinstance(e, ssl.SSLError)):
...
async def check_site_for_username(site, username, options: QueryOptions, logger, query_notify, *args, **kwargs) -> Tuple[str, QueryResultWrapper]:
    ...
    response_result = process_site_result(response, query_notify, logger, default_result, site)
...
async def maigret(username: str, site_dict: Dict[str, MaigretSite], logger, query_notify=None, proxy=None, tor_proxy=None, i2p_proxy=None, timeout=3, is_parsing_enabled=False, id_type='username', debug=False, forced=False, max_connections=100, no_progressbar=False, cookies=None, retries=0, check_domains=False, *args, **kwargs) -> QueryResultWrapper:
    ...
    clearweb_checker = SimpleAiohttpChecker(proxy=proxy, cookie_jar=cookie_jar, logger=logger)
    tor_checker = CheckerMock()
    if tor_proxy:
        tor_checker = ProxiedAiohttpChecker(proxy=tor_proxy, cookie_jar=cookie_jar, logger=logger)
    i2p_checker = CheckerMock()
    if i2p_proxy:
        i2p_checker = ProxiedAiohttpChecker(proxy=i2p_proxy, cookie_jar=cookie_jar, logger=logger)
    dns_checker = CheckerMock()
    if check_domains:
        dns_checker = AiodnsDomainResolver(logger=logger)
    if logger.level == logging.DEBUG:
    ...
    if no_progressbar:
        ...
        executor = AsyncioProgressbarQueueExecutor(*args, logger=logger, in_parallel=max_connections, timeout=timeout + 0.5, **kwargs)
    options: QueryOptions = {}
    options['cookies'] = cookie_jar
    options['checkers'] = {'': clearweb_checker, 'tor': tor_checker, 'dns': dns_checker, 'i2p': i2p_checker}
    ...
    while attempts:
        ...
        for (sitename, site) in site_dict.items():
            if sitename not in sites:
                continue
            default_result: QueryResultWrapper = {'site': site, 'status': QueryResult(username, sitename, '', QueryStatus.UNKNOWN, error=CheckError('Request failed'))}
            tasks_dict[sitename] = (check_site_for_username, [site, username, options, logger, query_notify], {'default': (sitename, default_result), 'retry': retries - attempts + 1})
        ...
        if not sites:
            break
        if attempts:
            query_notify.warning(f'Restarting checks for {len(sites)} sites... ({attempts} attempts left)')
...
async def site_self_check(site: MaigretSite, logger, semaphore, db: MaigretDatabase, silent=False, proxy=None, tor_proxy=None, i2p_proxy=None):
    changes = {'disabled': False}
    check_data = [(site.username_claimed, QueryStatus.CLAIMED), (site.username_unclaimed, QueryStatus.AVAILABLE)]
    logger.info(f'Checking {site.name}...')
    for (username, status) in check_data:
        async with semaphore:
            results_dict = await maigret(username=username, site_dict={site.name: site}, logger=logger, timeout=30, id_type=site.type, forced=True, no_progressbar=True, retries=1, proxy=proxy, tor_proxy=tor_proxy, i2p_proxy=i2p_proxy)
...
async def self_check(db: MaigretDatabase, site_data: dict, logger, silent=False, max_connections=10, proxy=None, tor_proxy=None, i2p_proxy=None) -> bool:
    ...
    for (_, site) in all_sites.items():
        check_coro = site_self_check(site, logger, sem, db, silent, proxy, tor_proxy, i2p_proxy)
