...
class ProductAttributevalue(models.Model):
    ...
    price_extra = fields.Float('Attribute Price Extra', compute='_compute_price_extra', inverse='_set_price_extra', default=0.0, digits=dp.get_precision('Product Price'), help='Price Extra: Extra price for the variant with this attribute value on sale price. eg. 200 price extra, 1000 + 200 = 1200.')
    price_ids = fields.One2many('product.attribute.price', 'value_id', 'Attribute Prices', readonly=True)
    _sql_constraints = [('value_company_uniq', 'unique (name,attribute_id)', 'This attribute value already exists !')]
