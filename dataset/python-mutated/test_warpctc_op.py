...
def warpctc_wrapper(Logits, Label, LogitsLength=None, LabelLength=None, blank=0, norm_by_times=False):
    if False:
        print('Hello World!')
    return paddle._C_ops.warpctc(Logits, Label, LogitsLength, LabelLength, blank, norm_by_times)
...
class TestWarpCTCOp(OpTest):
    ...
    def setUp(self):
        ...
        self.config()
        logits = np.random.uniform(0.1, 1.0, [sum(self.logits_lod[0]), self.num_classes]).astype('float32')
        softmax = np.apply_along_axis(stable_softmax, 1, logits)
        labels = np.random.randint(0, self.num_classes - 1, [sum(self.labels_lod[0]), 1], dtype='int32')
        ...
        self.inputs = {'Logits': (logits, self.logits_lod), 'Label': (labels, self.labels_lod)}
    ...
    def test_check_grad(self):
        ...
        if core.is_compiled_with_rocm():
            self.check_grad(['Logits'], 'Loss', max_relative_error=0.009, check_dygraph=False)
        else:
            self.check_grad(['Logits'], 'Loss', max_relative_error=0.007, check_dygraph=False)
...
class TestWarpCTCOpWithPadding(OpTest):
    ...
    def setUp(self):
        ...
        self.python_out_sig = ['Loss']
        self.config()
        logits = np.random.uniform(0.1, 1.0, [sum(self.logits_length), self.num_classes]).astype('float32')
        softmax = np.apply_along_axis(stable_softmax, 1, logits)
...
class TestWarpCTCOpFp64(OpTest):
    ...
    def setUp(self):
        ...
        self.python_out_sig = ['Loss']
        self.config()
        logits = np.random.uniform(0.1, 1.0, [sum(self.logits_length), self.num_classes]).astype('float64')
        softmax = np.apply_along_axis(stable_softmax, 1, logits)
        labels = np.random.randint(0, self.num_classes - 1, [sum(self.labels_length), 1], dtype='int32')
        ctc = CTCForward(softmax, self.logits_lod, labels, self.labels_lod, self.num_classes, self.batch_size, self.blank, self.norm_by_times)
        loss = ctc.forward()
...
class TestWarpCTCOpError(unittest.TestCase):
    ...
    def test_errors(self):
        ...
        with program_guard(Program(), Program()):
            logits = paddle.static.data(name='logits', shape=[5, 16, 6], dtype='float32')
            ...
            def test_label_len_Variable():
                ...
                paddle.nn.functional.ctc_loss(log_probs=logits, labels=label, input_lengths=logits_length, label_length=label_length_data, reduction='none')
...
class TestCTCLossAPICase(unittest.TestCase):
    ...
    def test_class_api(self):
        ...
        self.blank = 0
        self.norm_by_times = False
        logits = np.random.uniform(0.1, 1.0, [max(self.logits_length), self.batch_size, self.num_classes]).astype('float32')
        softmax = np.apply_along_axis(stable_softmax, -1, logits)
