...
class kms_inventoryCallTransformer(cst.CSTTransformer):
    CTRL_PARAMS: Tuple[str] = ('retry', 'timeout', 'metadata')
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'get_protected_resources_summary': ('name',), 'list_crypto_keys': ('parent', 'page_size', 'page_token'), 'search_protected_resources': ('scope', 'crypto_key', 'page_size', 'page_token', 'resource_types')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=kms_inventoryCallTransformer()):
    if False:
        i = 10
        return i + 15
