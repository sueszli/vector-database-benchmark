...
for (i, black_ridges) in enumerate([True, False]):
    for (j, (func, sigmas)) in enumerate([(original, None), (meijering, [1]), (meijering, range(1, 5)), (sato, [1]), (sato, range(1, 5)), (frangi, [1]), (frangi, range(1, 5)), (hessian, [1]), (hessian, range(1, 5))]):
        ...
        if j == 0:
            axes[i, j].set_ylabel(f'black_ridges = {black_ridges!r}')
        axes[i, j].set_xticks([])
        axes[i, j].set_yticks([])
plt.tight_layout()
plt.show()