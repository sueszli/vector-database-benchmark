...
from examples.speech_to_text.data_utils import create_zip, extract_fbank_features, filter_manifest_df, gen_config_yaml, gen_vocab, get_zip_manifest, load_df_from_tsv, save_df_to_tsv, cal_gcmvn_stats
import torch
...
def process(args):
    ...
    for lang in MUSTC.LANGUAGES:
        ...
        for split in MUSTC.SPLITS:
            ...
            if args.use_audio_input:
                ...
                for (waveform, sample_rate, _, _, _, utt_id) in tqdm(dataset):
                    ...
                    sf.write((audio_root / f'{utt_id}.flac').as_posix(), _wavform.T.numpy(), tgt_sample_rate)
            else:
                ...
                for (waveform, sample_rate, _, _, _, utt_id) in tqdm(dataset):
                    features = extract_fbank_features(waveform, sample_rate, audio_root / f'{utt_id}.npy')
                    if split == 'train' and args.cmvn_type == 'global':
        ...
        if args.use_audio_input:
            ...
            gen_config_yaml(cur_root, spm_filename=spm_filename_prefix + '.model', yaml_filename=f'config_{args.task}.yaml', specaugment_policy='lb', cmvn_type=args.cmvn_type, gcmvn_path=cur_root / 'gcmvn.npz' if args.cmvn_type == 'global' else None)
...
def process_joint(args):
    ...
    assert all(((cur_root / f'en-{lang}').is_dir() for lang in MUSTC.LANGUAGES)), 'do not have downloaded data available for all 8 languages'
    ...
    gen_config_yaml(cur_root, spm_filename=spm_filename_prefix + '.model', yaml_filename=f'config_{args.task}.yaml', specaugment_policy='ld', prepend_tgt_lang_tag=args.task == 'st')
    for lang in MUSTC.LANGUAGES:
        for split in MUSTC.SPLITS:
...
def main():
    ...
    parser.add_argument('--cmvn-type', default='utterance', choices=['global', 'utterance'], help='The type of cepstral mean and variance normalization')
    parser.add_argument('--gcmvn-max-num', default=150000, type=int, help='Maximum number of sentences to use to estimate global mean and variance')
