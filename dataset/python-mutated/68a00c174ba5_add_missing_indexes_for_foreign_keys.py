...
def upgrade():
    if False:
        i = 10
        return i + 15
    op.create_index(op.f('ix_blacklist_blacklisted_by'), 'blacklist', ['blacklisted_by'], unique=False)
    op.create_index(op.f('ix_ses_events_email_id'), 'ses_events', ['email_id'], unique=False)
    op.get_bind().commit()
    with op.get_context().autocommit_block():
        op.create_index('journals_submitted_by_idx', 'journals', ['submitted_by'], unique=False, postgresql_concurrently=True)
