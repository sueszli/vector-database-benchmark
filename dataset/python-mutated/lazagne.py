...
class LaZagne(PupyModule):
    ...
    """
    dependencies = {'all': ['whole', 'sqlite3', 'xml', 'calendar', 'ConfigParser', 'lazagne', 'pyasn1'], 'linux': ['secretstorage', 'crypt'], 'windows': ['sqlite3.dll', 'pypykatz']}
    FILTER = ''.join([len(repr(chr(x))) == 3 and chr(x) or '.' for x in range(256)])
    TYPESMAP = {'password': 'plaintext', 'hash': 'hash', 'key': 'key', 'cmd': 'cmd', 'defaultpassword': 'lsa'}
    NON_TABLE = set(['Ssh', 'Secretstorage', 'Libsecret', 'Cli'])
    FILTER_COLUMNS = set(['CredType', 'Category', 'SavePassword'])
