...
if is_torch_available():
    import torch
    from transformers import MODEL_FOR_PRETRAINING_MAPPING, BertForMaskedLM, BertForMultipleChoice, BertForNextSentencePrediction, BertForPreTraining, BertForQuestionAnswering, BertForSequenceClassification, BertForTokenClassification, BertLMHeadModel, BertModel, logging
...
class BertModelTester:
    ...
    def get_config(self):
        ...
        return BertConfig(vocab_size=self.vocab_size, hidden_size=self.hidden_size, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, hidden_act=self.hidden_act, hidden_dropout_prob=self.hidden_dropout_prob, attention_probs_dropout_prob=self.attention_probs_dropout_prob, max_position_embeddings=self.max_position_embeddings, type_vocab_size=self.type_vocab_size, is_decoder=False, initializer_range=self.initializer_range)
    ...
    def create_and_check_model_for_causal_lm_as_decoder(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels, encoder_hidden_states, encoder_attention_mask):
        if False:
            i = 10
            return i + 15
        config.add_cross_attention = True
        ...
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels, encoder_hidden_states=encoder_hidden_states, encoder_attention_mask=encoder_attention_mask)
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels, encoder_hidden_states=encoder_hidden_states)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.seq_length, self.vocab_size))
...
class BertModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (BertModel, BertLMHeadModel, BertForMaskedLM, BertForMultipleChoice, BertForNextSentencePrediction, BertForPreTraining, BertForQuestionAnswering, BertForSequenceClassification, BertForTokenClassification) if is_torch_available() else ()
    all_generative_model_classes = (BertLMHeadModel,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': BertModel, 'fill-mask': BertForMaskedLM, 'question-answering': BertForQuestionAnswering, 'text-classification': BertForSequenceClassification, 'text-generation': BertLMHeadModel, 'token-classification': BertForTokenClassification, 'zero-shot': BertForSequenceClassification} if is_torch_available() else {}
    fx_compatible = True
    ...
    def test_for_warning_if_padding_and_no_attention_mask(self):
        if False:
            i = 10
            return i + 15
        (config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels) = self.model_tester.prepare_config_and_inputs()
...
class BertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head_absolute_embedding(self):
        ...
        self.assertTrue(torch.allclose(output[:, 1:4, 1:4], expected_slice, atol=0.0001))
    ...
    def test_inference_no_head_relative_embedding_key(self):
        ...
        expected_shape = torch.Size((1, 11, 768))
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[[0.0756, 0.3142, -0.5128], [0.3761, 0.3462, -0.5477], [0.2052, 0.376, -0.124]]])
        self.assertTrue(torch.allclose(output[:, 1:4, 1:4], expected_slice, atol=0.0001))
    ...
    def test_inference_no_head_relative_embedding_key_query(self):
        ...
        expected_shape = torch.Size((1, 11, 768))
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[[0.6496, 0.3784, 0.8203], [0.8148, 0.5656, 0.2636], [-0.0681, 0.5597, 0.7045]]])
        self.assertTrue(torch.allclose(output[:, 1:4, 1:4], expected_slice, atol=0.0001))