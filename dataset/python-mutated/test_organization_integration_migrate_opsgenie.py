...
class OrganizationIntegrationAlertRulesTest(APITestCase):
    ...
    @patch('sentry.integrations.opsgenie.integration.OpsgenieIntegration.schedule_migrate_opsgenie_plugin')
    def test_simple(self, mock_migrate_opsgenie_plugin, mock_metrics):
        if False:
            print('Hello World!')
        integration = self.create_integration(organization=self.organization, provider='opsgenie', external_id='cool_opsgenie')
        path = self.get_path(integration_id=integration.id)
        response = self.client.put(path, format='json')
