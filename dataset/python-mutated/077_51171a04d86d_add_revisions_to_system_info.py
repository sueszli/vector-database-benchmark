...
def upgrade():
    ...
    if skip_based_on_legacy_engine_version(op, __name__):
        return
    op.add_column('system_info', sa.Column('state', sa.UnicodeText, nullable=False, server_default='active'))
    op.add_column('system_info_revision', sa.Column('state', sa.UnicodeText, nullable=False, server_default='active'))
    op.add_column('system_info_revision', sa.Column('expired_id', sa.UnicodeText))
    op.add_column('system_info_revision', sa.Column('revision_timestamp', sa.TIMESTAMP))
    op.add_column('system_info_revision', sa.Column('expired_timestamp', sa.TIMESTAMP))
...
def downgrade():
    if False:
        while True:
            i = 10
    op.create_unique_constraint('system_info_revision_key_key', 'system_info_revision', ['key'])
