...
@pytest.mark.parametrize('style, value', [('grid_size_rows', 3), ('grid_size_columns', 3), ('grid_gutter_vertical', 4), ('grid_gutter_horizontal', 4), ('grid_rows', '1fr 3fr'), ('grid_columns', '1fr 3fr')])
async def test_programmatic_style_change_updates_children(style: str, value: object):
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.pause()
        assert sizes != [(lbl.size.width, lbl.size.height) for lbl in app.screen.query(Label)]

@pytest.mark.parametrize('style, value', [('align_horizontal', 'right'), ('align_vertical', 'bottom'), ('align', ('right', 'bottom'))])
async def test_programmatic_align_change_updates_children_position(style: str, value: str):
    ...
    async with app.run_test() as pilot:
        ...
        setattr(app.query_one(Grid).styles, style, value)
        await pilot.pause()
        assert offsets != [(lbl.region.x, lbl.region.y) for lbl in app.screen.query(Label)]