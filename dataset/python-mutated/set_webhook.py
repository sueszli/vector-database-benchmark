...
class SetWebhook(TelegramMethod[bool]):
    ...
    if TYPE_CHECKING:
        ...
        def __init__(__pydantic__self__, *, url: str, certificate: Optional[InputFile]=None, ip_address: Optional[str]=None, max_connections: Optional[int]=None, allowed_updates: Optional[List[str]]=None, drop_pending_updates: Optional[bool]=None, secret_token: Optional[str]=None, **__pydantic_kwargs: Any) -> None:
            if False:
                i = 10
                return i + 15
            super().__init__(url=url, certificate=certificate, ip_address=ip_address, max_connections=max_connections, allowed_updates=allowed_updates, drop_pending_updates=drop_pending_updates, secret_token=secret_token, **__pydantic_kwargs)