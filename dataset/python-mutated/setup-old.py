...
if 1:
    ...
    for (k, v) in os.environ.items():
        log(f'    {k}: {v}')
assert os.path.abspath(os.getcwd()) == os.path.abspath(f'{__file__}/..'), f'Current directory must be the PyMuPDF directory'
...
ALPINE = DEFAULT + ['jbig2dec', 'jpeg', 'openjp2', 'harfbuzz']
ARCH_LINUX = DEFAULT + ['jbig2dec', 'openjp2', 'jpeg', 'freetype', 'gumbo']
NIX = ARCH_LINUX + ['harfbuzz']
OPENSUSE = NIX + ['png16']
DEBIAN = OPENSUSE + ['mujs']
FEDORA = NIX + ['leptonica', 'tesseract']
LIBRARIES = {'default': DEFAULT, 'ubuntu': DEBIAN, 'arch': ARCH_LINUX, 'manjaro': ARCH_LINUX, 'artix': ARCH_LINUX, 'opensuse': OPENSUSE, 'fedora': FEDORA, 'alpine': ALPINE, 'nix': NIX, 'debian': DEBIAN}
...
if 'sdist' in sys.argv:
    get_mupdf_tgz()
if '-h' not in sys.argv and '--help' not in sys.argv and (0 or 'bdist_wheel' in sys.argv or 'build' in sys.argv or ('bdist' in sys.argv) or ('install' in sys.argv)):
    ...
    if mupdf_local:
        ...
        if windows:
            ...
            command = f'cd {mupdf_local}&&"{devenv}" platform/win32/mupdf.sln /Build "ReleaseTesseract|{windows_config}" /Project mupdf'
        else:
    ...
    if mupdf_local and (linux or openbsd or freebsd):
        ...
        libraries = [f'mupdf', f'mupdf-third']
    elif linux:
...
module = Extension('fitz._fitz', ['fitz/fitz.i'], language='c++', include_dirs=include_dirs, library_dirs=library_dirs, libraries=libraries, extra_compile_args=extra_compile_args, extra_link_args=extra_link_args, swig_opts=extra_swig_args)
setup_py_cwd = os.path.dirname(__file__)
classifiers = ['Development Status :: 5 - Production/Stable', 'Intended Audience :: Developers', 'Intended Audience :: Information Technology', 'Operating System :: MacOS', 'Operating System :: Microsoft :: Windows', 'Operating System :: POSIX :: Linux', 'Programming Language :: C', 'Programming Language :: Python :: 3 :: Only', 'Programming Language :: Python :: Implementation :: CPython', 'Topic :: Utilities', 'Topic :: Multimedia :: Graphics', 'Topic :: Software Development :: Libraries']
with open(os.path.join(setup_py_cwd, 'README.md'), encoding='utf-8') as f:
    readme = f.read()
setup(name='PyMuPDF', version='1.22.6', description='Python bindings for the PDF toolkit and renderer MuPDF', long_description=readme, long_description_content_type='text/markdown', classifiers=classifiers, url='https://github.com/pymupdf/PyMuPDF', author='Artifex', author_email='support@artifex.com', cmdclass={'build_py': build_ext_first}, ext_modules=[module], python_requires='>=3.7', py_modules=['fitz.fitz', 'fitz.utils', 'fitz.__main__'], license='GNU AFFERO GPL 3.0', project_urls={'Documentation': 'https://pymupdf.readthedocs.io/', 'Source': 'https://github.com/pymupdf/pymupdf', 'Tracker': 'https://github.com/pymupdf/PyMuPDF/issues', 'Changelog': 'https://pymupdf.readthedocs.io/en/latest/changes.html'})