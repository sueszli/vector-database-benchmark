...
from snips_nlu.constants import END, LANGUAGE_DE, LANGUAGE_EN, LANGUAGE_ES, LANGUAGE_FR, RESOLVED_VALUE, RES_MATCH_RANGE, SNIPS_NUMBER, START, VALUE
from snips_nlu.languages import get_default_sep, get_punctuation_regex, supports_num2words
from snips_nlu.preprocessing import tokenize_light
AND_UTTERANCES = {LANGUAGE_EN: ['and', '&'], LANGUAGE_FR: ['et', '&'], LANGUAGE_ES: ['y', '&'], LANGUAGE_DE: ['und', '&']}
AND_REGEXES = {language: re.compile('|'.join(('(?<=\\s)%s(?=\\s)' % re.escape(u) for u in utterances)), re.IGNORECASE) for (language, utterances) in iteritems(AND_UTTERANCES)}
