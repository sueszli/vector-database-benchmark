...
class Conv3DTestCase(unittest.TestCase):
    ...
    def __init__(self, methodName='runTest', batch_size=4, spartial_shape=(8, 8, 8), num_channels=6, num_filters=8, filter_size=3, padding=0, stride=1, dilation=1, groups=1, no_bias=False, data_format='NCDHW', dtype='float32'):
        if False:
            for i in range(10):
                print('nop')
    ...
    def base_layer(self, place):
        ...
        with base.unique_name.guard():
            with base.program_guard(main, start):
                ...
                y_var = paddle.static.nn.conv3d(x_var, self.num_filters, self.filter_size, padding=self.padding, stride=self.stride, dilation=self.dilation, groups=self.groups, param_attr=weight_attr, bias_attr=bias_attr, data_format=self.data_format)
        ...
        with base.unique_name.guard():
            with base.program_guard(main, start):
                ...
                if not self.no_bias:
                    ...
                    b_var = None
                y_var = F.conv3d(x_var, w_var, b_var, padding=self.padding, stride=self.stride, dilation=self.dilation, groups=self.groups, data_format=self.data_format)
    ...
    def paddle_nn_layer(self):
        ...
        conv = nn.Conv3D(self.num_channels, self.num_filters, self.filter_size, padding=self.padding, stride=self.stride, dilation=self.dilation, groups=self.groups, data_format=self.data_format)
...
def add_cases(suite):
    ...
    suite.addTest(Conv3DTestCase(methodName='runTest', filter_size=(3, 2, 3), padding='valid'))
    ...
    suite.addTest(Conv3DTestCase(methodName='runTest', padding=[[0, 0], [0, 0], [1, 2], [2, 1], [2, 2]]))
    suite.addTest(Conv3DTestCase(methodName='runTest', data_format='NDHWC'))
    suite.addTest(Conv3DTestCase(methodName='runTest', data_format='NDHWC', padding=[[0, 0], [1, 1], [3, 3], [2, 2], [0, 0]]))
    suite.addTest(Conv3DTestCase(methodName='runTest', groups=2, padding='valid'))
    suite.addTest(Conv3DTestCase(methodName='runTest', num_filters=6, num_channels=3, groups=3, padding='valid'))
...
def add_error_cases(suite):
    ...
    suite.addTest(Conv3DErrorTestCase(methodName='runTest', num_channels=5, groups=2, padding=[-1, 1, 3]))
