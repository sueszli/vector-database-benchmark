...
class UseruploadNet(XFSDownloader):
    ...
    __version__ = '0.02'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?userupload\\.net/\\w{12}'
    ...
    NAME_PATTERN = '<title>Download (?P<N>.+?)</title>'
    SIZE_PATTERN = '<span>File Size : (?P<S>[\\d.,]+) (?P<U>[\\w^_]+)</span>'
    LINK_PATTERN = '<a href="(.+?)" type="button" class="btn btn-primary btn-block mb-4">'
