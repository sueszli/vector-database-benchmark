...
CONFIG_SCHEMA = AUTH_PROVIDER_SCHEMA.extend({vol.Required(CONF_API_PASSWORD): cv.string}, extra=vol.PREVENT_EXTRA)
...
class LegacyLoginFlow(LoginFlow):
    ...
    async def async_step_init(self, user_input: dict[str, str] | None=None) -> FlowResult:
        ...
        if user_input is not None:
            try:
                cast(LegacyApiPasswordAuthProvider, self._auth_provider).async_validate_login(user_input['password'])
            except InvalidAuthError:
                errors['base'] = 'invalid_auth'
            if not errors:
                return await self.async_finish({})
        return self.async_show_form(step_id='init', data_schema=vol.Schema({vol.Required('password'): str}), errors=errors)