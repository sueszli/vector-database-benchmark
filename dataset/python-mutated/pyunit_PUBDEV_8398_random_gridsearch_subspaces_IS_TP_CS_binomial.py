...
class test_random_gam_gridsearch_specific:
    ...
    hyper_parameters = {'lambda': [1, 2], 'subspaces': [{'scale': [[0.001], [0.0002]], 'num_knots': [[5], [10]], 'bs': [[1], [0]], 'gam_columns': [[['c_0']], [['c_1']]]}, {'scale': [[0.001, 0.001, 0.001], [0.0002, 0.0002, 0.0002]], 'bs': [[0, 2, 1], [2, 3, 1]], 'num_knots': [[5, 10, 12], [6, 11, 13]], 'gam_columns': [[['c_0'], ['c_1'], ['c_3', 'c_4', 'c_5']], [['c_1'], ['c_2'], ['c_4', 'c_5', 'c_6']]]}]}
    manual_gam_models = []
    num_grid_models = 0
    ...
    def setup_data(self):
        ...
        self.h2o_data = h2o.import_file(path=pyunit_utils.locate('smalldata/gam_test/synthetic_20Cols_binomial_20KRows.csv'))
                    ...
                    for gam_columns in subspace['gam_columns']:
                        for num_knots in subspace['num_knots']:
                            for bsVal in subspace['bs']:
                                ...
                                self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family='binomial', gam_columns=gam_columns, scale=scale, num_knots=num_knots, bs=bsVal, lambda_=lambda_, seed=1))

    def train_models(self):
        if False:
            i = 10
            return i + 15
        self.h2o_model = H2OGridSearch(H2OGeneralizedAdditiveEstimator(family='binomial', keep_gam_cols=True, seed=1), hyper_params=self.hyper_parameters, search_criteria=self.search_criteria)
    ...
    def match_models(self):
        ...
        for model in self.manual_gam_models:
            ...
            for grid_search_model in self.h2o_model.models:
                if grid_search_model.actual_params['gam_columns'] == gam_columns and grid_search_model.actual_params['scale'] == scale and (grid_search_model.actual_params['num_knots'] == num_knots) and (grid_search_model.actual_params['bs'] == bsVal) and (grid_search_model.actual_params['lambda'] == lambda_):
                    ...
                    print(model.coef())
                    pyunit_utils.assertEqualCoeffDicts(grid_search_model.coef(), model.coef(), tol=1e-06)
                    break
