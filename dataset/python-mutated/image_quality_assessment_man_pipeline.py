...
from modelscope.models.cv.image_quality_assessment_man import ImageQualityAssessmentMAN
...
@PIPELINES.register_module(Tasks.image_quality_assessment_mos, module_name=Pipelines.image_quality_assessment_man)
class ImageQualityAssessmentMANPipeline(Pipeline):
    """ Image Quality Assessment MAN Pipeline which will use Multi-dimension Attention Network
        ...
        >>> assessment_predictor = pipeline(Tasks.image_quality_assessment_man,             model='damo/cv_man_image-quality-assessment')
    ...
    def __init__(self, model: Union[ImageQualityAssessmentMAN, str], preprocessor=ImageQualityAssessmentMANPreprocessor(), **kwargs):
        if False:
            for i in range(10):
                print('nop')
