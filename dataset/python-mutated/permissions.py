...
from .converters import CogOrCommand, RuleType, ClearableRuleType, GuildUniqueObjectFinder, GlobalUniqueObjectFinder
...
class Permissions(commands.Cog):
            ...
            for (obj_name, rules_dict) in obj_type_rules.items():
                ...
                for (guild_id, guild_rules) in rules_dict.items():
                    ...
                    if _uid in guild_rules:
                        if obj:
                            await self._remove_rule(CogOrCommand(typename, obj.qualified_name, obj), user_id, int(guild_id))
                        else:
    ...
    async def __permissions_hook(self, ctx: commands.Context) -> Optional[bool]:
        ...
        if ctx.guild:
            if ctx.author == ctx.guild.owner:
                if ctx.command in (self.permissions, self.permissions_acl, self.permissions_acl_getguild, self.permissions_acl_setguild, self.permissions_acl_updateguild, self.permissions_addguildrule, self.permissions_clearguildrules, self.permissions_removeguildrule, self.permissions_setdefaultguildrule, self.permissions_canrun, self.permissions_explain):
                    return True
        return None
    ...
    async def permissions_canrun(self, ctx: commands.Context, user: discord.Member, *, command: str):
        ...
        if com is None:
            ...
            try:
                ...
                can = False
            out = success(_('That user can run the specified command.')) if can else error(_('That user can not run the specified command.'))
        await ctx.send(out)
    ...
    async def permissions_acl_yaml_example(self, ctx: commands.Context):
        ...
        await ctx.send(_('Example YAML for setting rules:\n') + box(textwrap.dedent('                        COMMAND:\n                            ping:\n                                12345678901234567: true\n                                56789012345671234: false\n                        COG:\n                            General:\n                                56789012345671234: true\n                                12345678901234567: false\n                                default: false\n                        '), lang='yaml'))
    ...
    async def permissions_addglobalrule(self, ctx: commands.Context, allow_or_deny: RuleType, cog_or_command: CogOrCommand, *who_or_what: GlobalUniqueObjectFinder):
        ...
        for w in who_or_what:
            await self._add_rule(rule=cast(bool, allow_or_deny), cog_or_cmd=cog_or_command, model_id=w.id, guild_id=0)
        await ctx.send(_('Rule added.'))

    @commands.guild_only()
    @commands.guildowner_or_permissions(administrator=True)
    @permissions.command(name='addserverrule', aliases=['addguildrule'], require_var_positional=True)
    async def permissions_addguildrule(self, ctx: commands.Context, allow_or_deny: RuleType, cog_or_command: CogOrCommand, *who_or_what: GuildUniqueObjectFinder):
        ...
        """
        for w in who_or_what:
            await self._add_rule(rule=cast(bool, allow_or_deny), cog_or_cmd=cog_or_command, model_id=w.id, guild_id=ctx.guild.id)
        await ctx.send(_('Rule added.'))

    @commands.is_owner()
    @permissions.command(name='removeglobalrule', require_var_positional=True)
    async def permissions_removeglobalrule(self, ctx: commands.Context, cog_or_command: CogOrCommand, *who_or_what: GlobalUniqueObjectFinder):
    ...
    @commands.guild_only()
    @commands.guildowner_or_permissions(administrator=True)
    @permissions.command(name='removeserverrule', aliases=['removeguildrule'], require_var_positional=True)
    async def permissions_removeguildrule(self, ctx: commands.Context, cog_or_command: CogOrCommand, *who_or_what: GlobalUniqueObjectFinder):
        ...
        """
        for w in who_or_what:
            await self._remove_rule(cog_or_cmd=cog_or_command, model_id=w.id, guild_id=ctx.guild.id)
        await ctx.send(_('Rule removed.'))

    @commands.guild_only()
    @commands.guildowner_or_permissions(administrator=True)
    @permissions.command(name='setdefaultserverrule', aliases=['setdefaultguildrule'])
    async def permissions_setdefaultguildrule(self, ctx: commands.Context, allow_or_deny: ClearableRuleType, cog_or_command: CogOrCommand):
        ...
        """
        await self._set_default_rule(rule=cast(Optional[bool], allow_or_deny), cog_or_cmd=cog_or_command, guild_id=ctx.guild.id)
        await ctx.send(_('Default set.'))

    @commands.is_owner()
    @permissions.command(name='setdefaultglobalrule')
    async def permissions_setdefaultglobalrule(self, ctx: commands.Context, allow_or_deny: ClearableRuleType, cog_or_command: CogOrCommand):
        ...
        rule for. This is case sensitive.
        """
        await self._set_default_rule(rule=cast(Optional[bool], allow_or_deny), cog_or_cmd=cog_or_command, guild_id=GLOBAL)
        await ctx.send(_('Default set.'))
    ...
    async def _on_cog_add(self, cog: commands.Cog) -> None:
        self._load_rules_for(cog_or_command=cog, rule_dict=await self.config.custom(COG, cog.__class__.__name__).all())
        cog.requires.ready_event.set()

    async def _on_command_add(self, command: commands.Command) -> None:
        self._load_rules_for(cog_or_command=command, rule_dict=await self.config.custom(COMMAND, command.qualified_name).all())
        command.requires.ready_event.set()

    async def _add_rule(self, rule: bool, cog_or_cmd: CogOrCommand, model_id: int, guild_id: int) -> None:
        """Add a rule.
    ...
    async def _set_default_rule(self, rule: Optional[bool], cog_or_cmd: CogOrCommand, guild_id: int) -> None:
    ...
    async def _permissions_acl_set(self, ctx: commands.Context, guild_id: int, update: bool) -> None:
        ...
        try:
            ...
            await ctx.send(_('Your YAML file did not match the schema: ') + translate(e.errors[-1]))
        else:
