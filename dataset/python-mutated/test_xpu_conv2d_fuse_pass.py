...
class TestConv2dXPUFusePass(PassAutoScanTest):
    ...
    def sample_predictor_configs(self, program_config):
        ...
        yield (config, ['conv2d_xpu'], (0.001, 0.001))
    ...
    def sample_program_config(self, draw):
        ...
        x_shape = draw(st.lists(st.integers(min_value=12, max_value=12), min_size=4, max_size=4))
        x_shape[1] = draw(st.integers(min_value=1, max_value=10))
        w_shape = draw(st.lists(st.integers(min_value=3, max_value=3), min_size=4, max_size=4))
        if data_format == 'NCHW':
            w_shape[1] = x_shape[1]
        padding_algorithm = draw(st.sampled_from(['SAME', 'VALID']))
        groups = draw(st.integers(min_value=1, max_value=1))
        dilations = draw(st.lists(st.integers(min_value=1, max_value=1), min_size=2, max_size=2))
        paddings = draw(st.lists(st.integers(min_value=1, max_value=1), min_size=2, max_size=2))
        ...
        conv2d_op = OpConfig('conv2d', inputs={'Input': ['conv2d_input'], 'Filter': ['conv2d_weight']}, outputs={'Output': ['conv2d_out']}, data_format=data_format, dilations=dilations, padding_algorithm=padding_algorithm, groups=groups, paddings=paddings, strides=strides, has_bias=False)
        ew_bias_op = OpConfig('elementwise_add', inputs={'X': ['conv2d_out'], 'Y': ['ew_bias']}, outputs={'Out': ['add_out']}, axis=axis)
        ops = [conv2d_op, ew_bias_op]
        if has_relu:
            relu_op = OpConfig('relu', inputs={'X': ['add_out']}, outputs={'Out': ['relu_out']})
            ops.append(relu_op)
        program_config = ProgramConfig(ops=ops, inputs={'conv2d_input': TensorConfig(data_gen=partial(generate_data, x_shape))}, weights={'conv2d_weight': TensorConfig(data_gen=partial(generate_data, w_shape)), 'ew_bias': TensorConfig(data_gen=partial(generate_data, ew_bias_shape))}, outputs=ops[-1].outputs['Out'])
    ...
    def test(self):
        if False:
            i = 10
            return i + 15
        self.run_and_statis(quant=False, max_examples=25, passes=['conv2d_xpu_fuse_pass'])
