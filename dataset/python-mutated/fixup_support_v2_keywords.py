...
class supportCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'close_case': ('name',), 'create_case': ('parent', 'case'), 'create_comment': ('parent', 'comment'), 'escalate_case': ('name', 'escalation'), 'get_case': ('name',), 'list_attachments': ('parent', 'page_size', 'page_token'), 'list_cases': ('parent', 'filter', 'page_size', 'page_token'), 'list_comments': ('parent', 'page_size', 'page_token'), 'search_case_classifications': ('query', 'page_size', 'page_token'), 'search_cases': ('parent', 'query', 'page_size', 'page_token'), 'update_case': ('case', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=supportCallTransformer()):
    if False:
        while True:
            i = 10
