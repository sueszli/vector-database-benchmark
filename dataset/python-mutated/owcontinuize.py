...
from AnyQt.QtCore import Qt, QSize, QAbstractListModel, QObject, QItemSelectionModel
from AnyQt.QtGui import QColor
from AnyQt.QtWidgets import QButtonGroup, QRadioButton, QListView
from orangewidget.utils import listview
from orangewidget.utils.itemmodels import SeparatedListDelegate, LabelledSeparator
...
Continuize = SimpleNamespace(Default=DefaultId, **{v.name: v.value for v in Continuizer.MultinomialTreatment})
...
class OWContinuize(widget.OWWidget):
    name = 'Continuize'
    description = 'Transform categorical attributes into numeric and, ' + 'optionally, scale numeric values.'
    ...
    class Error(widget.OWWidget.Error):
        unsupported_sparse = widget.Msg('Some chosen methods do not support sparse data: {}')
    want_control_area = False
    settings_version = 3
    disc_var_hints: Dict[str, int] = Setting({DefaultKey: ContinuizationDefault}, schema_only=True)
    cont_var_hints: Dict[str, int] = Setting({DefaultKey: NormalizationDefault}, schema_only=True)
