...
def main(data_dir='./data'):
    ...
    for pretrained in ['glove.6B.300d.txt']:
        config = configure.Config(data_dir=data_dir, for_preprocessing=True, pretrained_embeddings=pretrained, word_embedding_size=300)
        embeddings.PretrainedEmbeddingLoader(config).build()
    utils.log('CONSTRUCTING DEV SETS')
    for task_name in ['chunk']:
        config = configure.Config(data_dir=data_dir, for_preprocessing=True)
        task_data_dir = os.path.join(config.raw_data_topdir, task_name) + '/'
        train_sentences = word_level_data.TaggedDataLoader(config, task_name, False).get_labeled_sentences('train')
    ...
    for task_name in ['chunk']:
        for (i, label_encoding) in enumerate(['BIOES']):
            config = configure.Config(data_dir=data_dir, for_preprocessing=True, label_encoding=label_encoding)
            ...
            if token_level:
                ...
                utils.log('  Writing label mapping for', task_name.upper(), label_encoding)
            utils.log(' ', len(loader.label_mapping), 'classes')
            utils.write_cpickle(loader.label_mapping, loader.label_mapping_path)
