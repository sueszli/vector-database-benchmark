...
from ..utils import ExtractorError, determine_ext, float_or_none, format_field, int_or_none, str_or_none, traverse_obj, try_get, unescapeHTML, unified_timestamp, url_or_none, urlencode_postdata
...
class RokfinPlaylistBaseIE(InfoExtractor):
    _TYPES = {'video': 'post', 'audio': 'post', 'stream': 'stream', 'dead_stream': 'stream', 'stack': 'stack'}
...
class RokfinStackIE(RokfinPlaylistBaseIE):
    ...
    _TESTS = [{'url': 'https://www.rokfin.com/stack/271/Tulsi-Gabbard-Portsmouth-Townhall-FULL--Feb-9-2020', 'playlist_count': 8, 'info_dict': {'id': '271'}}]
...
class RokfinChannelIE(RokfinPlaylistBaseIE):
    ...
    _TESTS = [{'url': 'https://rokfin.com/TheConvoCouch', 'playlist_mincount': 100, 'info_dict': {'id': '12071-new', 'title': 'TheConvoCouch - New', 'description': 'md5:bb622b1bca100209b91cd685f7847f06'}}]
    _TABS = {'new': 'posts', 'top': 'top', 'videos': 'video', 'podcasts': 'audio', 'streams': 'stream', 'stacks': 'stack'}
...
class RokfinSearchIE(SearchInfoExtractor):
    ...
    _TYPES = {'video': (('id', 'raw'), 'post'), 'audio': (('id', 'raw'), 'post'), 'stream': (('content_id', 'raw'), 'stream'), 'dead_stream': (('content_id', 'raw'), 'stream'), 'stack': (('content_id', 'raw'), 'stack')}
    _TESTS = [{'url': 'rkfnsearch5:"zelenko"', 'playlist_count': 5, 'info_dict': {'id': '"zelenko"', 'title': '"zelenko"'}}]
    _db_url = None
    _db_access_key = None
