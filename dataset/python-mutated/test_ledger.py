...
class MockNetwork:
    ...
    async def get_transaction_batch(self, txids, restricted):
        return {txid: await self.get_transaction_and_merkle(txid) for txid in txids}
...
class TestSynchronization(LedgerTestCase):
    ...
    async def test_update_history(self):
        ...
        self.ledger.network = MockNetwork([{'tx_hash': txid1, 'height': 0}, {'tx_hash': txid2, 'height': 1}, {'tx_hash': txid3, 'height': 2}], {txid1: hexlify(get_transaction(get_output(1)).raw), txid2: hexlify(get_transaction(get_output(2)).raw), txid3: hexlify(get_transaction(get_output(3)).raw)})
        ...
        address_details = await self.ledger.db.get_address(address=address)
        self.assertEqual(address_details['history'], f'{txid1}:0:{txid2}:1:{txid3}:2:')
        ...
        address_details = await self.ledger.db.get_address(address=address)
        self.assertEqual(address_details['history'], f'{txid1}:0:{txid2}:1:{txid3}:2:{txid4}:3:')
...
class BlockchainReorganizationTests(LedgerTestCase):
    ...
    async def test_1_block_reorganization(self):
        self.ledger.network = MocHeaderNetwork({10: {'height': 10, 'count': 5, 'hex': hexlify(HEADERS[block_bytes(10):block_bytes(15)])}, 15: {'height': 15, 'count': 0, 'hex': b''}})
        ...
        await self.ledger.receive_header([{'height': 11, 'hex': hexlify(self.make_header(block_height=11))}])

    async def test_3_block_reorganization(self):
        self.ledger.network = MocHeaderNetwork({10: {'height': 10, 'count': 5, 'hex': hexlify(HEADERS[block_bytes(10):block_bytes(15)])}, 11: {'height': 11, 'count': 1, 'hex': hexlify(self.make_header(block_height=11))}, 12: {'height': 12, 'count': 1, 'hex': hexlify(self.make_header(block_height=12))}, 15: {'height': 15, 'count': 0, 'hex': b''}})
        headers = self.ledger.headers
        await headers.connect(0, HEADERS[:block_bytes(10)])
        self.add_header(block_height=len(headers))
        self.add_header(block_height=len(headers))
        ...
        await self.ledger.receive_header([{'height': 13, 'hex': hexlify(self.make_header(block_height=13))}])
...
class BasicAccountingTests(LedgerTestCase):
    ...
    async def test_balance(self):
        ...
        tx = Transaction(is_verified=True).add_outputs([Output.pay_pubkey_hash(100, hash160)])
        await self.ledger.db.insert_transaction(tx)
        await self.ledger.db.save_transaction_io(tx, address, hash160, f'{tx.id}:1:')
        self.assertEqual(await self.account.get_balance(), 100)
        tx = Transaction(is_verified=True).add_outputs([Output.pay_claim_name_pubkey_hash(100, 'foo', b'', hash160)])
        await self.ledger.db.insert_transaction(tx)
        await self.ledger.db.save_transaction_io(tx, address, hash160, f'{tx.id}:1:')
        self.assertEqual(await self.account.get_balance(), 100)
    ...
    async def test_get_utxo(self):
        ...
        hash160 = self.ledger.address_to_hash160(address)
        tx = Transaction(is_verified=True).add_outputs([Output.pay_pubkey_hash(100, hash160)])
        await self.ledger.db.save_transaction_io('insert', tx, address, hash160, f'{tx.id}:1:')
        utxos = await self.account.get_utxos()
        self.assertEqual(len(utxos), 1)
        tx = Transaction(is_verified=True).add_inputs([Input.spend(utxos[0])])
        await self.ledger.db.save_transaction_io('insert', tx, address, hash160, f'{tx.id}:1:')
