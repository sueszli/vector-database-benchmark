...
from azure.containerregistry import RepositoryProperties, ArtifactManifestProperties, ArtifactManifestOrder, ArtifactTagProperties, ArtifactTagOrder, ArtifactArchitecture, ArtifactOperatingSystem, DigestValidationError
from azure.containerregistry.aio import ContainerRegistryClient
from azure.containerregistry._helpers import DOCKER_MANIFEST, OCI_IMAGE_MANIFEST, DEFAULT_CHUNK_SIZE
from azure.core.exceptions import ResourceNotFoundError, ClientAuthenticationError, HttpResponseError, ServiceRequestError, ServiceResponseError
...
class TestContainerRegistryClientAsync(AsyncContainerRegistryTestClass):
    ...
    async def test_update_repository_properties_kwargs(self, containerregistry_endpoint):
        ...
        self.sleep(5)
        async with self.create_registry_client(containerregistry_endpoint) as client:
            received = await client.update_repository_properties(repo, can_delete=False, can_read=False, can_write=False, can_list=False)
            self.assert_all_properties(received, False)
            received = await client.update_repository_properties(repo, can_delete=True, can_read=True, can_write=True, can_list=True)
            self.assert_all_properties(received, True)
            await client.delete_repository(repo)
    ...
    async def test_list_registry_artifacts_descending(self, containerregistry_endpoint):
        async with self.create_registry_client(containerregistry_endpoint) as client:
            ...
            count = 0
            async for artifact in client.list_manifest_properties(HELLO_WORLD, order_by=ArtifactManifestOrder.LAST_UPDATED_ON_DESCENDING):
    ...
    async def test_list_registry_artifacts_ascending(self, containerregistry_endpoint):
        async with self.create_registry_client(containerregistry_endpoint) as client:
            prev_last_updated_on = None
            count = 0
            async for artifact in client.list_manifest_properties(HELLO_WORLD, order_by=ArtifactManifestOrder.LAST_UPDATED_ON_ASCENDING):
                if prev_last_updated_on:
                    assert artifact.last_updated_on > prev_last_updated_on
                prev_last_updated_on = artifact.last_updated_on
                count += 1
    ...
    async def test_update_manifest_properties_kwargs(self, containerregistry_endpoint):
        ...
        self.sleep(5)
        async with self.create_registry_client(containerregistry_endpoint) as client:
            received = await client.update_manifest_properties(repo, tag, can_delete=False, can_read=False, can_write=False, can_list=False)
            self.assert_all_properties(received, False)
            received = await client.update_manifest_properties(repo, tag, can_delete=True, can_read=True, can_write=True, can_list=True)
            self.assert_all_properties(received, True)
            await client.delete_repository(repo)
    ...
    async def test_update_tag_properties_kwargs(self, containerregistry_endpoint):
        ...
        self.sleep(5)
        async with self.create_registry_client(containerregistry_endpoint) as client:
            received = await client.update_tag_properties(repo, tag, can_delete=False, can_read=False, can_write=False, can_list=False)
            self.assert_all_properties(received, False)
            received = await client.update_tag_properties(repo, tag, can_delete=True, can_read=True, can_write=True, can_list=True)
            self.assert_all_properties(received, True)
            await client.delete_repository(repo)
    ...
    async def test_construct_container_registry_client(self, **kwargs):
        ...
        async with ContainerRegistryClient(endpoint=containerregistry_endpoint, credential=credential, audience='https://microsoft.com') as client:
    ...
    async def test_set_audience(self, containerregistry_endpoint):
        ...
        async with ContainerRegistryClient(endpoint=containerregistry_endpoint, credential=credential, audience=valid_audience) as client:
            async for repo in client.list_repository_names():
                pass
        if valid_audience == get_audience(AzureAuthorityHosts.AZURE_PUBLIC_CLOUD):
            ...
            async with ContainerRegistryClient(endpoint=containerregistry_endpoint, credential=credential, audience=invalid_audience) as client:
                with pytest.raises(ClientAuthenticationError):
                    async for repo in client.list_repository_names():
                        pass
...
class TestContainerRegistryClientAsyncUnitTests:
    ...
    async def test_manifest_digest_validation(self):
        ...
        async def send_in_set_manifest(request: PipelineRequest, **kwargs) -> MyMagicMock:
            ...
            return MyMagicMock(status_code=201, headers={'Docker-Content-Digest': content_digest}, content_type='application/json; charset=utf-8', text=self.text)
        ...
        async def send_in_get_manifest(request: PipelineRequest, **kwargs) -> MyMagicMock:
            ...
            return MyMagicMock(status_code=200, headers={'Docker-Content-Digest': content_digest, 'Content-Type': content_type, 'Content-Length': len(b'hello world')}, read=read, json=json)
        async with ContainerRegistryClient(endpoint=self.containerregistry_endpoint, transport=MyMagicMock(send=send_in_set_manifest)) as client:
        ...
        async with ContainerRegistryClient(endpoint=self.containerregistry_endpoint, transport=MyMagicMock(send=send_in_get_manifest)) as client:
            with pytest.raises(DigestValidationError) as exp:
    ...
    async def test_blob_digest_validation(self):
        ...
        async def send_in_upload_blob(request: PipelineRequest, **kwargs) -> MyMagicMock:
            if request.method == 'PUT':
                ...
                return MyMagicMock(status_code=202, headers={'Location': '/v2/test-repo/blobs/uploads/fake_location'}, content_type='application/json; charset=utf-8', text=self.text)

        async def iter_bytes() -> AsyncIterator[bytes]:
            yield b'{"hello": "world"}'

        async def send_in_download_blob(request: PipelineRequest, **kwargs) -> MyMagicMock:
            return MyMagicMock(status_code=206, headers={'Content-Range': 'bytes 0-27/28', 'Content-Length': '28'}, content_type='application/octet-stream', text=self.text, iter_bytes=iter_bytes)
        async with ContainerRegistryClient(endpoint=self.containerregistry_endpoint, transport=MyMagicMock(send=send_in_upload_blob)) as client:
            with pytest.raises(DigestValidationError) as exp:
                await client.upload_blob('test-repo', BytesIO(b'{"hello": "world"}'))
            assert str(exp.value) == 'The server-computed digest does not match the client-computed digest.'
        async with ContainerRegistryClient(endpoint=self.containerregistry_endpoint, transport=MyMagicMock(send=send_in_download_blob)) as client:
            digest = hashlib.sha256(b'hello world').hexdigest()
    ...
    async def test_deserialize_manifest(self):
        ...
        async def send(request: PipelineRequest, **kwargs) -> MyMagicMock:
            return MyMagicMock(status_code=200, content_type='application/json; charset=utf-8', text=get_manifest)
        async with ContainerRegistryClient(endpoint=self.containerregistry_endpoint, transport=MyMagicMock(send=send)) as client:
