...
class Model_SEQ2SEQ_Test(CustomTestCase):
    ...
    def test_basic_simpleSeq2Seq(self):
        if False:
            for i in range(10):
                print('nop')
        model_ = Seq2seq(decoder_seq_length=5, cell_enc=tf.keras.layers.GRUCell, cell_dec=tf.keras.layers.GRUCell, n_layer=3, n_units=128, embedding_layer=tl.layers.Embedding(vocabulary_size=self.vocab_size, embedding_size=self.embedding_size))
