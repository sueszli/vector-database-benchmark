...
from sanic.exceptions import BadRequest, PayloadTooLarge, SanicException, ServerError
...
try:
    ...
    from aioquic.h3.events import DatagramReceived, DataReceived, H3Event, HeadersReceived, WebTransportStreamDataReceived
...
class HTTPReceiver(Receiver, Stream):
    ...
    async def run(self, exception: Optional[Exception]=None):
        ...
        if exception:
            logger.info(f'{Colors.BLUE}[exception]: {Colors.RED}{exception}{Colors.END}', exc_info=True, extra={'verbosity': 1})
            await self.error_response(exception)
        else:
            try:
                logger.info(f'{Colors.BLUE}[request]:{Colors.END} {self.request}', extra={'verbosity': 1})
    ...
    async def send(self, data: bytes, end_stream: bool) -> None:
        """Send data to client"""
        logger.debug(f'{Colors.BLUE}[send]: {Colors.GREEN}data={data.decode()} end_stream={end_stream}{Colors.END}', extra={'verbosity': 2})
...
class Http3:
    ...
    if HTTP3_AVAILABLE:
        HANDLER_PROPERTY_MAPPING = {DataReceived: 'stream_id', HeadersReceived: 'stream_id', DatagramReceived: 'flow_id', WebTransportStreamDataReceived: 'session_id'}
