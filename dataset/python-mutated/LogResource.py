...
class LogResource(GenericResource):
    ...
    async def __pipeline_logs(self, pipeline: Pipeline, query_arg, meta) -> List[Dict]:
        ...
        columns = [a.execution_date, a.pipeline_schedule_id, a.pipeline_schedule_id, a.pipeline_uuid, a.variables]
        ...
        for block in pipeline.blocks_by_uuid.values():
            logger = LoggerManagerFactory.get_logger_manager(partition=LOG_PARTITION_EDIT_PIPELINE, pipeline_uuid=pipeline_uuid, subpartition=block.uuid)
        ...
        return [{'block_run_logs': block_run_logs, 'pipeline_run_logs': pipeline_run_logs, 'total_block_run_log_count': total_block_run_log_count, 'total_pipeline_run_log_count': total_pipeline_run_log_count}]