...
class MessageEntity(MutableTelegramObject):
    ...
    if TYPE_CHECKING:
        ...
        def __init__(__pydantic__self__, *, type: str, offset: int, length: int, url: Optional[str]=None, user: Optional[User]=None, language: Optional[str]=None, custom_emoji_id: Optional[str]=None, **__pydantic_kwargs: Any) -> None:
            if False:
                while True:
                    i = 10
            super().__init__(type=type, offset=offset, length=length, url=url, user=user, language=language, custom_emoji_id=custom_emoji_id, **__pydantic_kwargs)

    def extract_from(self, text: str) -> str:
        if False:
            while True:
                i = 10
        return remove_surrogates(add_surrogates(text)[self.offset * 2:(self.offset + self.length) * 2])