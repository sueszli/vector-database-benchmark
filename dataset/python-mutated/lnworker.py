...
from typing import Optional, Sequence, Tuple, List, Set, Dict, TYPE_CHECKING, NamedTuple, Union, Mapping, Any, Iterable, AsyncGenerator, DefaultDict, Callable, Awaitable
import threading
import socket
import aiohttp
import json
...
from .lnutil import Outpoint, LNPeerAddr, get_compressed_pubkey_from_bech32, extract_nodeid, PaymentFailure, split_host_port, ConnStringFormatError, generate_keypair, LnKeyFamily, LOCAL, REMOTE, MIN_FINAL_CLTV_DELTA_FOR_INVOICE, NUM_MAX_EDGES_IN_PAYMENT_PATH, SENT, RECEIVED, HTLCOwner, UpdateAddHtlc, Direction, LnFeatures, ShortChannelID, HtlcLog, derive_payment_secret_from_payment_preimage, NoPathFound, InvalidGossipMsg
...
from .i18n import _
from .lnrouter import RouteEdge, LNPaymentRoute, LNPaymentPath, is_route_within_budget, NoChannelPolicy, LNPathInconsistent
...
class LNGossip(LNWorker):
    ...
    async def process_gossip(self, chan_anns, node_anns, chan_upds):
        ...
        categorized_chan_upds = await run_in_thread(partial(self.channel_db.add_channel_updates, chan_upds, max_age=self.max_age))
...
class LNWallet(LNWorker):
    ...
    async def sync_with_remote_watchtower(self):
        while True:
            ...
            if not (parsed_url.scheme == 'https' or is_private_netaddress(parsed_url.hostname)):
                self.logger.warning(f"got watchtower URL for remote tower but we won't use it! can only use HTTPS (except if private IP): not using {watchtower_url!r}")
                continue
            try:
    ...
    async def handle_onchain_state(self, chan: Channel):
        if type(chan) is ChannelBackup:
            util.trigger_callback('channel', self.wallet, chan)
            return
        if chan.get_state() in (ChannelState.OPEN, ChannelState.SHUTDOWN) and chan.should_be_closed_due_to_expiring_htlcs(self.network.get_local_height()):
    ...
    async def open_channel_just_in_time(self, next_peer, next_amount_msat_htlc, next_cltv_abs, payment_hash, next_onion):
        try:
            ...
            (next_chan, funding_tx) = await self.open_channel_with_peer(next_peer, funding_sat, push_sat=0, zeroconf=True, public=False, opening_fee=channel_opening_fee, password=password)
            ...
            htlc = next_peer.send_htlc(chan=next_chan, payment_hash=payment_hash, amount_msat=next_amount_msat_htlc, cltv_abs=next_cltv_abs, onion=next_onion)
    ...
    @log_exceptions
    async def open_channel_with_peer(self, peer, funding_sat, *, push_sat: int=0, public: bool=False, zeroconf: bool=False, opening_fee: int=None, password=None):
        coins = self.wallet.get_spendable_coins(None)
        node_id = peer.pubkey
        funding_tx = self.mktx_for_open_channel(coins=coins, funding_sat=funding_sat, node_id=node_id, fee_est=None)
        (chan, funding_tx) = await self._open_channel_coroutine(peer=peer, funding_tx=funding_tx, funding_sat=funding_sat, push_sat=push_sat, public=public, zeroconf=zeroconf, opening_fee=opening_fee, password=password)
        return (chan, funding_tx)

    @log_exceptions
    async def _open_channel_coroutine(self, *, peer: Peer, funding_tx: PartialTransaction, funding_sat: int, push_sat: int, public: bool, zeroconf=False, opening_fee=None, password: Optional[str]) -> Tuple[Channel, PartialTransaction]:
        coro = peer.channel_establishment_flow(funding_tx=funding_tx, funding_sat=funding_sat, push_msat=push_sat * 1000, public=public, zeroconf=zeroconf, opening_fee=opening_fee, temp_channel_id=os.urandom(32))
    ...
    @log_exceptions
    async def pay_invoice(self, invoice: str, *, amount_msat: int=None, attempts: int=None, full_path: LNPaymentPath=None, channels: Optional[Sequence[Channel]]=None) -> Tuple[bool, List[HtlcLog]]:
        ...
        self.logger.info(f'pay_invoice starting session for RHASH={payment_hash.hex()}. using_trampoline={self.uses_trampoline()}. invoice_features={invoice_features.get_names()}')
        self.set_invoice_status(key, PR_INFLIGHT)
        budget = PaymentFeeBudget.default(invoice_amount_msat=amount_to_pay)
        success = False
        try:
            await self.pay_to_node(node_pubkey=invoice_pubkey, payment_hash=payment_hash, payment_secret=payment_secret, amount_to_pay=amount_to_pay, min_final_cltv_delta=min_final_cltv_delta, r_tags=r_tags, invoice_features=invoice_features, attempts=attempts, full_path=full_path, channels=channels, budget=budget)
            success = True
            ...
            self.logger.info(f'pay_invoice ending session for RHASH={payment_hash.hex()}. success={success!r}')
        if success:
    ...
    async def pay_to_node(self, *, node_pubkey: bytes, payment_hash: bytes, payment_secret: bytes, amount_to_pay: int, min_final_cltv_delta: int, r_tags, invoice_features: int, attempts: int=None, full_path: LNPaymentPath=None, fwd_trampoline_onion: OnionPacket=None, budget: PaymentFeeBudget, channels: Optional[Sequence[Channel]]=None) -> None:
        assert budget
        ...
        try:
            while True:
                if (amount_to_send := paysession.get_outstanding_amount_to_send()) > 0:
                    ...
                    routes = self.create_routes_for_payment(paysession=paysession, amount_msat=amount_to_send, full_path=full_path, fwd_trampoline_onion=fwd_trampoline_onion, channels=channels, budget=budget._replace(fee_msat=remaining_fee_budget_msat))
                    async for (sent_htlc_info, cltv_delta, trampoline_onion) in routes:
                        await self.pay_to_route(paysession=paysession, sent_htlc_info=sent_htlc_info, min_final_cltv_delta=cltv_delta, trampoline_onion=trampoline_onion)
                ...
                self.logger.info(f'UPDATE_FAIL_HTLC. code={repr(code)}. decoded_data={failure_msg.decode_data()}. data={data.hex()!r}')
                self.logger.info(f'error reported by {erring_node_id.hex()}')
                if code == OnionFailureCode.MPP_TIMEOUT:
                    raise PaymentFailure(failure_msg.code_name())
                if fwd_trampoline_onion and code in [OnionFailureCode.TRAMPOLINE_FEE_INSUFFICIENT, OnionFailureCode.TRAMPOLINE_EXPIRY_TOO_SOON]:
                    raise failure_msg
                if self.uses_trampoline():
                    paysession.handle_failed_trampoline_htlc(htlc_log=htlc_log, failure_msg=failure_msg)
                else:
                    self.handle_error_code_from_failed_htlc(route=route, sender_idx=sender_idx, failure_msg=failure_msg, amount=htlc_log.amount_msat)
    ...
    async def pay_to_route(self, *, paysession: PaySession, sent_htlc_info: SentHtlcInfo, min_final_cltv_delta: int, trampoline_onion: Optional[OnionPacket]=None) -> None:
        """Sends a single HTLC."""
        shi = sent_htlc_info
        del sent_htlc_info
        ...
        htlc = peer.pay(route=shi.route, chan=chan, amount_msat=shi.amount_msat, total_msat=shi.bucket_msat, payment_hash=paysession.payment_hash, min_final_cltv_delta=min_final_cltv_delta, payment_secret=shi.payment_secret_bucket, trampoline_onion=trampoline_onion)
    ...
    async def create_routes_for_payment(self, *, paysession: PaySession, amount_msat: int, fwd_trampoline_onion: OnionPacket=None, full_path: LNPaymentPath=None, channels: Optional[Sequence[Channel]]=None, budget: PaymentFeeBudget) -> AsyncGenerator[Tuple[SentHtlcInfo, int, Optional[OnionPacket]], None]:
        ...
        if channels:
            ...
            my_active_channels = [chan for chan in self.channels.values() if chan.is_active() and (not chan.is_frozen_for_sending())]
        random.shuffle(my_active_channels)
        split_configurations = self.suggest_splits(amount_msat=amount_msat, final_total_msat=paysession.amount_to_pay, my_active_channels=my_active_channels, invoice_features=paysession.invoice_features, r_tags=paysession.r_tags)
                ...
                if self.uses_trampoline():
                    ...
                    for (trampoline_node_id, trampoline_parts) in per_trampoline_channel_amounts.items():
                        ...
                        if trampoline_node_id == paysession.invoice_pubkey:
                            ...
                            (trampoline_route, trampoline_onion, per_trampoline_amount_with_fees, per_trampoline_cltv_delta) = create_trampoline_route_and_onion(amount_msat=per_trampoline_amount, total_msat=paysession.amount_to_pay, min_final_cltv_delta=paysession.min_final_cltv_delta, my_pubkey=self.node_keypair.pubkey, invoice_pubkey=paysession.invoice_pubkey, invoice_features=paysession.invoice_features, node_id=trampoline_node_id, r_tags=paysession.r_tags, payment_hash=paysession.payment_hash, payment_secret=paysession.payment_secret, local_height=local_height, trampoline_fee_level=paysession.trampoline_fee_level, use_two_trampolines=paysession.use_two_trampolines, failed_routes=paysession.failed_trampoline_routes, budget=budget._replace(fee_msat=budget.fee_msat // len(per_trampoline_channel_amounts)))
                        ...
                        for (chan_id, part_amount_msat) in trampoline_parts:
                            ...
                            route = [RouteEdge(start_node=self.node_keypair.pubkey, end_node=trampoline_node_id, short_channel_id=chan.short_channel_id, fee_base_msat=0, fee_proportional_millionths=0, cltv_delta=0, node_features=trampoline_features)]
                            self.logger.info(f'adding route {part_amount_msat} {delta_fee} {margin}')
                            shi = SentHtlcInfo(route=route, payment_secret_orig=paysession.payment_secret, payment_secret_bucket=per_trampoline_secret, amount_msat=part_amount_msat_with_fees, bucket_msat=per_trampoline_amount_with_fees, amount_receiver_msat=part_amount_msat, trampoline_fee_level=paysession.trampoline_fee_level, trampoline_route=trampoline_route)
                    ...
                    for ((chan_id, _), part_amounts_msat) in sc.config.items():
                        for part_amount_msat in part_amounts_msat:
                            ...
                            route = await run_in_thread(partial(self.create_route_for_single_htlc, amount_msat=part_amount_msat, invoice_pubkey=paysession.invoice_pubkey, min_final_cltv_delta=paysession.min_final_cltv_delta, r_tags=paysession.r_tags, invoice_features=paysession.invoice_features, my_sending_channels=[channel] if is_multichan_mpp else my_active_channels, full_path=full_path, budget=budget._replace(fee_msat=budget.fee_msat // sc.config.number_parts())))
                            shi = SentHtlcInfo(route=route, payment_secret_orig=paysession.payment_secret, payment_secret_bucket=paysession.payment_secret, amount_msat=part_amount_msat, bucket_msat=paysession.amount_to_pay, amount_receiver_msat=part_amount_msat, trampoline_fee_level=None, trampoline_route=None)
    ...
    async def rebalance_channels(self, chan1: Channel, chan2: Channel, *, amount_msat: int):
        ...
        (lnaddr, invoice) = self.get_bolt11_invoice(payment_hash=payment_hash, amount_msat=amount_msat, message='rebalance', expiry=3600, fallback_address=None, channels=[chan2])
