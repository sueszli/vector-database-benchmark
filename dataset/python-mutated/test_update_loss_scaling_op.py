...
def update_loss_scaling_wrapper(x, found_inf, prev_loss_scaling, num_good_steps, num_bad_steps, incr_every_n_steps, decr_every_n_nan_or_inf, incr_ratio, decr_ratio, stop_update=False):
    if False:
        return 10
    amp_nn.update_loss_scaling([x], found_inf, prev_loss_scaling, num_good_steps, num_bad_steps, incr_every_n_steps, decr_every_n_nan_or_inf, incr_ratio, decr_ratio, stop_update)
...
class TestUpdateLossScalingOp(OpTest):
    ...
    def setUp(self):
        ...
        self.python_out_sig = ['out0', 'LossScaling', 'OutGoodSteps', 'OutBadSteps']
    ...
    def init(self):
        ...
        self.decr_ratio = 0.8
        self.init_dtype()
        self.prev_loss_scaling = np.array([2048]).astype(self.loss_scaling_dtype)
...
class TestUpdateLossScalingBF16Op(OpTest):
    ...
    def setUp(self):
        ...
        self.inputs = {'X': [('x0', convert_float_to_uint16(x))], 'FoundInfinite': found_inf, 'PrevLossScaling': self.prev_loss_scaling, 'InGoodSteps': self.num_good_steps, 'InBadSteps': self.num_bad_steps}
        self.outputs = {'Out': [('out0', convert_float_to_uint16(x))], 'LossScaling': self.prev_loss_scaling * self.incr_ratio, 'OutGoodSteps': self.zero_steps, 'OutBadSteps': self.zero_steps}
...
class TestUpdateLossScalingOpBad(TestUpdateLossScalingOp):
    ...
    def setUp(self):
        ...
        x[i[0]][j[0]] = np.inf
        self.inputs = {'X': [('x0', x)], 'FoundInfinite': found_inf, 'PrevLossScaling': self.prev_loss_scaling, 'InGoodSteps': self.num_good_steps, 'InBadSteps': self.num_bad_steps, 'StopUpdate': self.stop_update}
