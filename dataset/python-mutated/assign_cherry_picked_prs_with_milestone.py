...
PR_PATTERN = re.compile('.*\\(#([0-9]+)\\)')
ISSUE_MATCH_IN_BODY = re.compile(' #([0-9]+)[^0-9]')
...
option_assume_yes = click.option('--assume-yes', is_flag=True, help='Assume yes answer to question')
option_previous_release = click.option('--previous-release', type=str, required=True, help='commit reference (for example hash or tag) of the previous release.')
option_current_release = click.option('--current-release', type=str, required=True, help='commit reference (for example hash or tag) of the current release.')
option_github_token = click.option('--github-token', type=str, required=True, help=textwrap.dedent('\n        GitHub token used to authenticate.\n        You can set omit it if you have GITHUB_TOKEN env variable set\n        Can be generated with:\n        https://github.com/settings/tokens/new?description=Read%20Write%20isssues&scopes=repo'), envvar='GITHUB_TOKEN')
option_limit_pr_count = click.option('--limit-pr-count', type=int, default=None, help='Limit PR count processes (useful for testing small subset of PRs).')
option_dry_run = click.option('--dry-run', is_flag=True, help='Do not make any changes, just show what would have been done', envvar='DRY_RUN')
option_skip_assigned = click.option('--skip-assigned', is_flag=True, help='Skip PRs already correctly assigned to the right milestone')
option_milestone_number = click.option('--milestone-number', type=int, required=True, help='Milestone number to set. See https://github.com/apache/airflow/milestones to find milestone id')
option_print_summary = click.option('--print-summary', is_flag=True, help='Produce summary of the changes cherry-picked in the file specified. Implies --skip-assigned')
option_output_folder = click.option('--output-folder', type=str, help='Folder where files with commit hashes will be store. Implies --print-summary and --skip-assigned')
