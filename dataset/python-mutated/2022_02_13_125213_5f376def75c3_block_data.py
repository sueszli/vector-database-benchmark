...
def upgrade():
    if False:
        return 10
    op.create_table('block_data', sa.Column('id', prefect.server.utilities.database.UUID(), server_default=sa.text('(GEN_RANDOM_UUID())'), nullable=False), sa.Column('created', prefect.server.utilities.database.Timestamp(timezone=True), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False), sa.Column('updated', prefect.server.utilities.database.Timestamp(timezone=True), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=False), sa.Column('name', sa.String(), nullable=False), sa.Column('blockref', sa.String(), nullable=False), sa.Column('data', prefect.server.utilities.database.JSON(astext_type=sa.Text()), server_default='{}', nullable=False), sa.PrimaryKeyConstraint('id', name=op.f('pk_block_data')))
    op.create_index(op.f('ix_block_data__name'), 'block_data', ['name'], unique=True)
    op.create_index(op.f('ix_block_data__updated'), 'block_data', ['updated'], unique=False)
