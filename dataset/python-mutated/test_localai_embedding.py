...
def get_mock_provider():
    if False:
        i = 10
        return i + 15
    return Provider(id='provider_id', tenant_id='tenant_id', provider_name='localai', provider_type=ProviderType.CUSTOM.value, encrypted_config='', is_valid=True)
...
def get_mock_embedding_model(mocker):
    ...
    mock_query.filter.return_value.first.return_value = ProviderModel(provider_name='localai', model_name=model_name, model_type=ModelType.EMBEDDINGS.value, encrypted_config=json.dumps({'server_url': server_url}), is_valid=True)
    mocker.patch('extensions.ext_database.db.session.query', return_value=mock_query)
    return LocalAIEmbedding(model_provider=model_provider, name=model_name)
