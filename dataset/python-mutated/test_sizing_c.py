...
@pytest.mark.parametrize('callback_props', [dict(fixed_columns=dict(headers=True), fixed_rows=dict(headers=True))])
def test_szng003_c_on_prop_change(test, fixed_columns, fixed_rows, merge_duplicate_headers, callback_props):
    if False:
        print('Hello World!')
