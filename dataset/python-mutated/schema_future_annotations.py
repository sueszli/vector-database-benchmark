...
import dataclasses
from typing import Any, AsyncGenerator, AsyncIterable, AsyncIterator, Generator, Iterable, Iterator, List, NamedTuple, Optional, cast
...
class FruitAsync(relay.Node):
    ...
    @classmethod
    async def resolve_nodes(cls, *, info: Optional[Info[None, None]]=None, node_ids: Iterable[str], required: bool=False) -> Iterable[Optional[Self]]:
        if node_ids is not None:
            return [fruits_async[nid] if required else fruits_async.get(nid) for nid in node_ids]
...
fruits = {str(f.id): f for f in [Fruit(id=1, name='Banana', color='yellow'), Fruit(id=2, name='Apple', color='red'), Fruit(id=3, name='Pineapple', color='yellow'), Fruit(id=4, name='Grape', color='purple'), Fruit(id=5, name='Orange', color='orange')]}
fruits_async = {k: FruitAsync(id=v.id, name=v.name, color=v.color) for (k, v) in fruits.items()}
...
class Query:
    ...
    node_with_async_permissions: relay.Node = relay.node(permission_classes=[DummyPermission])
    ...
    fruits_lazy: relay.ListConnection[Annotated[Fruit, strawberry.lazy('tests.relay.schema')]] = relay.connection(resolver=fruits_resolver)
    fruits_async: relay.ListConnection[FruitAsync] = relay.connection(resolver=fruits_async_resolver)
    fruits_custom_pagination: FruitCustomPaginationConnection = relay.connection(resolver=fruits_resolver)
    ...
    @relay.connection(relay.ListConnection[Fruit])
    async def fruits_custom_resolver_async_iterable(self, info: Info[None, None], name_endswith: Optional[str]=None) -> AsyncIterable[Fruit]:
        for f in fruits.values():
            if name_endswith is None or f.name.endswith(name_endswith):
                yield f

    @relay.connection(relay.ListConnection[Fruit])
    async def fruits_custom_resolver_async_iterator(self, info: Info[None, None], name_endswith: Optional[str]=None) -> AsyncIterator[Fruit]:
        for f in fruits.values():
            if name_endswith is None or f.name.endswith(name_endswith):
                yield f

    @relay.connection(relay.ListConnection[Fruit])
    async def fruits_custom_resolver_async_generator(self, info: Info[None, None], name_endswith: Optional[str]=None) -> AsyncGenerator[Fruit, None]:
        for f in fruits.values():
            if name_endswith is None or f.name.endswith(name_endswith):
                yield f
