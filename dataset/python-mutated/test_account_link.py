...
async def test_setup_provide_implementation(hass: HomeAssistant) -> None:
    ...
    legacy_entry = MockConfigEntry(domain='legacy', version=1, data={'auth_implementation': 'cloud'})
    none_cloud_entry = MockConfigEntry(domain='no_cloud', version=1, data={'auth_implementation': 'somethingelse'})
    none_cloud_entry.add_to_hass(hass)
    legacy_entry.add_to_hass(hass)
    account_link.async_setup(hass)
    with patch('homeassistant.components.cloud.account_link._get_services', return_value=[{'service': 'test', 'min_version': '0.1.0'}, {'service': 'too_new', 'min_version': '1000000.0.0'}, {'service': 'dev', 'min_version': '2022.9.0'}, {'service': 'deprecated', 'min_version': '0.1.0', 'accepts_new_authorizations': False}, {'service': 'legacy', 'min_version': '0.1.0', 'accepts_new_authorizations': False}, {'service': 'no_cloud', 'min_version': '0.1.0', 'accepts_new_authorizations': False}]), patch('homeassistant.components.cloud.account_link.HA_VERSION', '2022.9.0.dev20220817'):
        assert await config_entry_oauth2_flow.async_get_implementations(hass, 'non_existing') == {}
        assert await config_entry_oauth2_flow.async_get_implementations(hass, 'too_new') == {}
        assert await config_entry_oauth2_flow.async_get_implementations(hass, 'deprecated') == {}
        assert await config_entry_oauth2_flow.async_get_implementations(hass, 'no_cloud') == {}
        implementations = await config_entry_oauth2_flow.async_get_implementations(hass, 'test')
        legacy_implementations = await config_entry_oauth2_flow.async_get_implementations(hass, 'legacy')
        dev_implementations = await config_entry_oauth2_flow.async_get_implementations(hass, 'dev')
...
async def test_get_services_cached(hass: HomeAssistant) -> None:
    ...
    services = 1
    with patch.object(account_link, 'CACHE_TIMEOUT', 0), patch('hass_nabucasa.account_link.async_fetch_available_services', side_effect=lambda _: services) as mock_fetch:
...
async def test_get_services_error(hass: HomeAssistant) -> None:
    """Test that we cache services."""
    hass.data['cloud'] = None
    with patch.object(account_link, 'CACHE_TIMEOUT', 0), patch('hass_nabucasa.account_link.async_fetch_available_services', side_effect=asyncio.TimeoutError):
        assert await account_link._get_services(hass) == []
        assert account_link.DATA_SERVICES not in hass.data

async def test_implementation(hass: HomeAssistant, flow_handler, current_request_with_host: None) -> None:
    ...
    helper = Mock(async_get_authorize_url=AsyncMock(return_value='http://example.com/auth'), async_get_tokens=Mock(return_value=flow_finished))
    with patch('hass_nabucasa.account_link.AuthorizeAccountHelper', return_value=helper):
        result = await hass.config_entries.flow.async_init(TEST_DOMAIN, context={'source': config_entries.SOURCE_USER})
    assert result['type'] == data_entry_flow.FlowResultType.EXTERNAL_STEP
    assert result['url'] == 'http://example.com/auth'
    flow_finished.set_result({'refresh_token': 'mock-refresh', 'access_token': 'mock-access', 'expires_in': 10, 'token_type': 'bearer'})
    await hass.async_block_till_done()
    ...
    assert result['data']['token'] == {'refresh_token': 'mock-refresh', 'access_token': 'mock-access', 'token_type': 'bearer', 'expires_in': 10}
    entry = hass.config_entries.async_entries(TEST_DOMAIN)[0]
    assert await config_entry_oauth2_flow.async_get_config_entry_implementation(hass, entry) is impl