...
def __virtual__():
    ...
    if not __opts__.get('features', {}).get('x509_v2'):
        return (False, 'x509_v2 needs to be explicitly enabled by setting `x509_v2: true` in the minion configuration value `features` until Salt 3008 (Argon).')
    return __virtualname__

def create_certificate(ca_server=None, signing_policy=None, encoding='pem', append_certs=None, pkcs12_passphrase=None, pkcs12_encryption_compat=False, pkcs12_friendlyname=None, path=None, overwrite=True, raw=False, **kwargs):
    if False:
        while True:
            i = 10
    ...
    if 'algorithm' in kwargs:
        salt.utils.versions.warn_until(3009, '`algorithm` has been renamed to `digest`. Please update your code.')
        kwargs['digest'] = kwargs.pop('algorithm')
    ignored_params = {'text', 'version', 'serial_bits'}.intersection(kwargs)
    if ignored_params:
    ...
    if 'days_valid' not in kwargs and 'not_after' not in kwargs:
        try:
            salt.utils.versions.warn_until(3009, 'The default value for `days_valid` will change to 30. Please adapt your code accordingly.')
            kwargs['days_valid'] = 365
        except RuntimeError:
            pass
    if encoding not in ['der', 'pem', 'pkcs7_der', 'pkcs7_pem', 'pkcs12']:
        raise CommandExecutionError(f"Invalid value '{encoding}' for encoding. Valid: der, pem, pkcs7_der, pkcs7_pem, pkcs12")
    if kwargs.get('digest', 'sha256').lower() not in ['sha1', 'sha224', 'sha256', 'sha384', 'sha512', 'sha512_224', 'sha512_256', 'sha3_224', 'sha3_256', 'sha3_384', 'sha3_512']:
        raise CommandExecutionError(f"Invalid value '{kwargs['digest']}' for digest. Valid: sha1, sha224, sha256, sha384, sha512, sha512_224, sha512_256, sha3_224, sha3_256, sha3_384, sha3_512")
    ...
    if encoding == 'pkcs12' and 'private_key' not in kwargs:
        raise SaltInvocationError('Creating a PKCS12-encoded certificate without embedded private key is unsupported')
    if 'signing_private_key' not in kwargs and (not ca_server):
        raise SaltInvocationError('Creating a certificate locally at least requires a signing private key.')
    ...
    if ca_server:
        if signing_policy is None:
            raise SaltInvocationError('signing_policy must be specified to request a certificate from a remote ca_server')
        (cert, private_key_loaded) = _create_certificate_remote(ca_server, signing_policy, **kwargs)
    else:
    ...
    if encoding == 'pkcs12':
        out = encode_certificate(cert, append_certs=append_certs, encoding=encoding, private_key=private_key_loaded, pkcs12_passphrase=pkcs12_passphrase, pkcs12_encryption_compat=pkcs12_encryption_compat, pkcs12_friendlyname=pkcs12_friendlyname, raw=bool(path) or raw)
    else:
        out = encode_certificate(cert, append_certs=append_certs, encoding=encoding, raw=bool(path) or raw)
    if path is None:
        return out
    if encoding == 'pem':
        return write_pem(out.decode(), path, overwrite=overwrite, pem_type='CERTIFICATE')
    with salt.utils.files.fopen(path, 'wb') as fp_:
        fp_.write(out)
    return f'Certificate written to {path}'

def _create_certificate_remote(ca_server, signing_policy, private_key=None, private_key_passphrase=None, **kwargs):
    if False:
        for i in range(10):
            print('nop')
    private_key_loaded = None
    if private_key:
        private_key_loaded = x509util.load_privkey(private_key, passphrase=private_key_passphrase)
        kwargs['public_key'] = x509util.to_der(private_key_loaded.public_key())
    elif kwargs.get('public_key'):
        kwargs['public_key'] = x509util.to_der(x509util.load_pubkey(kwargs['public_key']))
    if kwargs.get('csr'):
    ...
    try:
        ...
        raise CommandExecutionError(f'ca_server did not return a certificate: {result}') from err

def _create_certificate_local(digest='sha256', copypath=None, prepend_cn=False, **kwargs):
    if False:
        i = 10
        return i + 15
    (builder, signing_private_key, private_key_loaded, _) = x509util.build_crt(**kwargs)
    algorithm = None
    if x509util.get_key_type(signing_private_key) not in [x509util.KEY_TYPE.ED25519, x509util.KEY_TYPE.ED448]:
    ...
    if copypath:
        ...
        if prepend_cn:
            try:
                prepend = cert.subject.get_attributes_for_oid(x509util.NAME_ATTRS_OID['CN'])[0].value + '-'
            except IndexError:
                pass
        write_pem(text=x509util.to_pem(cert), path=os.path.join(copypath, f'{prepend}{cert.serial_number:x}.crt'), pem_type='CERTIFICATE')
    return (cert, private_key_loaded)

def encode_certificate(certificate, encoding='pem', append_certs=None, private_key=None, private_key_passphrase=None, pkcs12_passphrase=None, pkcs12_encryption_compat=False, pkcs12_friendlyname=None, raw=False):
    if False:
        i = 10
        return i + 15
    ...
    if encoding != 'pkcs12' and private_key:
        raise SaltInvocationError('Embedding private keys is only supported for pkcs12 encoding')
    ...
    if encoding in ['der', 'pem']:
        ...
        if pkcs12_passphrase is None:
            ...
            if pkcs12_encryption_compat:
                cipher = serialization.PrivateFormat.PKCS12.encryption_builder().kdf_rounds(50000).key_cert_algorithm(serialization.pkcs12.PBES.PBESv1SHA1And3KeyTripleDESCBC).hmac_hash(hashes.SHA1()).build(pkcs12_passphrase)
            else:
                cipher = serialization.BestAvailableEncryption(pkcs12_passphrase)
        crt_bytes = serialization.pkcs12.serialize_key_and_certificates(name=salt.utils.stringutils.to_bytes(pkcs12_friendlyname) if pkcs12_friendlyname else None, key=private_key, cert=cert, cas=append_certs, encryption_algorithm=cipher)
    else:
        try:
            crt_bytes = serialization.pkcs7.serialize_certificates([cert] + append_certs, encoding=getattr(serialization.Encoding, 'PEM' if encoding == 'pkcs7_pem' else 'DER'))
        except AttributeError as err:
            raise CommandExecutionError('Serialization to pkcs7 requires at least cryptography release 37.') from err
    if raw:
        return crt_bytes
...
def create_crl(signing_private_key, revoked, signing_cert=None, signing_private_key_passphrase=None, include_expired=False, days_valid=None, digest='sha256', encoding='pem', extensions=None, path=None, raw=False, **kwargs):
    if False:
        for i in range(10):
            print('nop')
    ...
    if days_valid is None:
        try:
            salt.utils.versions.warn_until(3009, 'The default value for `days_valid` will change to 7. Please adapt your code accordingly.')
            days_valid = 100
        except RuntimeError:
            days_valid = 7
    revoked_parsed = []
    for rev in revoked:
        ...
        if len(rev) == 1 and isinstance(rev[next(iter(rev))], list):
            salt.utils.versions.warn_until(3009, 'Revoked certificates should be specified as a simple list of dicts.')
            for val in rev[next(iter(rev))]:
                parsed.update(val)
        if 'reason' in (parsed or rev):
            salt.utils.versions.warn_until(3009, 'The `reason` parameter for revoked certificates should be specified in extensions:CRLReason.')
            salt.utils.dictupdate.set_dict_key_value(parsed or rev, 'extensions:CRLReason', (parsed or rev).pop('reason'))
        revoked_parsed.append(parsed or rev)
    revoked = revoked_parsed
    if encoding not in ['der', 'pem']:
        raise CommandExecutionError(f"Invalid value '{encoding}' for encoding. Valid: der, pem")
    if digest.lower() not in ['sha1', 'sha224', 'sha256', 'sha384', 'sha512', 'sha512_224', 'sha512_256', 'sha3_224', 'sha3_256', 'sha3_384', 'sha3_512']:
        raise CommandExecutionError(f"Invalid value '{digest}' for digest. Valid: sha1, sha224, sha256, sha384, sha512, sha512_224, sha512_256, sha3_224, sha3_256, sha3_384, sha3_512")
    (builder, signing_private_key) = x509util.build_crl(signing_private_key, revoked, signing_cert=signing_cert, signing_private_key_passphrase=signing_private_key_passphrase, include_expired=include_expired, days_valid=days_valid, extensions=extensions)
    algorithm = None
    if x509util.get_key_type(signing_private_key) not in [x509util.KEY_TYPE.ED25519, x509util.KEY_TYPE.ED448]:
        algorithm = x509util.get_hashing_algorithm(digest)
...
def create_csr(private_key, private_key_passphrase=None, digest='sha256', encoding='pem', path=None, raw=False, **kwargs):
    if False:
        while True:
            i = 10
    ...
    ignored_params = {'text', 'version'}.intersection(kwargs)
    if ignored_params:
        salt.utils.versions.kwargs_warn_until(ignored_params, 'Potassium')
    kwargs = x509util.ensure_cert_kwargs_compat(kwargs)
    if encoding not in ['der', 'pem']:
    ...
    if digest.lower() not in ['sha1', 'sha224', 'sha256', 'sha384', 'sha512', 'sha512_224', 'sha512_256', 'sha3_224', 'sha3_256', 'sha3_384', 'sha3_512']:
        raise CommandExecutionError(f"Invalid value '{digest}' for digest. Valid: sha1, sha224, sha256, sha384, sha512, sha512_224, sha512_256, sha3_224, sha3_256, sha3_384, sha3_512")
    (builder, private_key) = x509util.build_csr(private_key, private_key_passphrase=private_key_passphrase, **kwargs)
    algorithm = None
    if x509util.get_key_type(private_key) not in [x509util.KEY_TYPE.ED25519, x509util.KEY_TYPE.ED448]:
        algorithm = x509util.get_hashing_algorithm(digest)
...
def create_private_key(algo='rsa', keysize=None, passphrase=None, encoding='pem', pkcs12_encryption_compat=False, path=None, raw=False, **kwargs):
    if False:
        return 10
    ...
    if 'bits' in kwargs:
        salt.utils.versions.warn_until(3009, '`bits` has been renamed to `keysize`. Please update your code.')
        keysize = kwargs.pop('bits')
    ignored_params = {'cipher', 'verbose', 'text'}.intersection(kwargs)
    if ignored_params:
        salt.utils.versions.kwargs_warn_until(ignored_params, 'Potassium')
    ...
    if encoding not in ['der', 'pem', 'pkcs12']:
        raise CommandExecutionError(f"Invalid value '{encoding}' for encoding. Valid: der, pem, pkcs12")
    out = encode_private_key(_generate_pk(algo=algo, keysize=keysize), encoding=encoding, passphrase=passphrase, pkcs12_encryption_compat=pkcs12_encryption_compat, raw=bool(path) or raw)
    if path is None:
        return out
    if encoding == 'pem':
        return write_pem(out.decode(), path, pem_type='(?:(RSA|ENCRYPTED) )?PRIVATE KEY')
    with salt.utils.files.fopen(path, 'wb') as fp_:
        fp_.write(out)
    return

def encode_private_key(private_key, encoding='pem', passphrase=None, private_key_passphrase=None, pkcs12_encryption_compat=False, raw=False):
    if False:
        for i in range(10):
            print('nop')
    ...
    if passphrase is None:
        ...
        if encoding == 'pkcs12' and pkcs12_encryption_compat:
            cipher = serialization.PrivateFormat.PKCS12.encryption_builder().kdf_rounds(50000).key_cert_algorithm(serialization.pkcs12.PBES.PBESv1SHA1And3KeyTripleDESCBC).hmac_hash(hashes.SHA1()).build(passphrase)
        else:
            cipher = serialization.BestAvailableEncryption(passphrase)
    if encoding in ['der', 'pem']:
        pk_encoding = getattr(serialization.Encoding, encoding.upper())
        pk_bytes = private_key.private_bytes(encoding=pk_encoding, format=serialization.PrivateFormat.PKCS8, encryption_algorithm=cipher)
    else:
        pk_bytes = serialization.pkcs12.serialize_key_and_certificates(name=None, key=private_key, cert=None, cas=None, encryption_algorithm=cipher)
    if raw:
        return pk_bytes
    if encoding == 'pem':
...
def expires(certificate, days=0):
    ...
    cert = x509util.load_cert(certificate)
    return cert.not_valid_after <= datetime.datetime.utcnow() + datetime.timedelta(days=days)
...
def expired(certificate):
    ...
    cert = x509util.load_cert(certificate)
    try:
        ret['cn'] = cert.subject.get_attributes_for_oid(x509util.NAME_ATTRS_OID['CN'])[0].value
    except IndexError:
        pass
...
def get_public_key(key, passphrase=None, asObj=None):
    ...
    try:
        ...
        pass
    try:
        return x509util.to_pem(x509util.load_cert(key, passphrase=passphrase).public_key()).decode()
    except (CommandExecutionError, SaltInvocationError):
        pass
    try:
        return x509util.to_pem(x509util.load_privkey(key, passphrase=passphrase).public_key()).decode()
    except (CommandExecutionError, SaltInvocationError):
        pass
    try:
        ...
        pass
    raise CommandExecutionError('Could not load key as certificate, public key, private key or CSR')
...
def get_signing_policy(signing_policy, ca_server=None):
    ...
    if ca_server is None:
        ...
        if signing_policy not in __context__[ckey][ca_server]:
            policy_ = _query_remote(ca_server, signing_policy, {}, get_signing_policy_only=True)
            if 'signing_cert' in policy_:
                policy_['signing_cert'] = x509util.to_pem(x509util.load_cert(policy_['signing_cert'])).decode()
    ...
    for (name, long_names) in x509util.NAME_ATTRS_ALT_NAMES.items():
        for long_name in long_names:
            if long_name in policy:
                salt.utils.versions.warn_until(3009, f'Found {long_name} in {signing_policy}. Please migrate to the short name: {name}')
                policy[name] = policy.pop(long_name)
    for (extname, long_names) in x509util.EXTENSIONS_ALT_NAMES.items():
        for long_name in long_names:
            if long_name in policy:
                salt.utils.versions.warn_until(3009, f'Found {long_name} in {signing_policy}. Please migrate to the short name: {extname}')
...
def read_certificate(certificate):
    ...
    ret = {'version': cert.version.value + 1, 'key_size': cert.public_key().key_size if key_type in ['ec', 'rsa'] else None, 'key_type': key_type, 'serial_number': x509util.dec2hex(cert.serial_number), 'fingerprints': {'sha1': x509util.pretty_hex(cert.fingerprint(algorithm=hashes.SHA1())), 'sha256': x509util.pretty_hex(cert.fingerprint(algorithm=hashes.SHA256()))}, 'subject': _parse_dn(cert.subject), 'subject_hash': x509util.pretty_hex(_get_name_hash(cert.subject)), 'subject_str': cert.subject.rfc4514_string(), 'issuer': _parse_dn(cert.issuer), 'issuer_hash': x509util.pretty_hex(_get_name_hash(cert.issuer)), 'issuer_str': cert.issuer.rfc4514_string(), 'not_before': cert.not_valid_before.strftime(x509util.TIME_FMT), 'not_after': cert.not_valid_after.strftime(x509util.TIME_FMT), 'public_key': get_public_key(cert), 'extensions': _parse_extensions(cert.extensions)}
    try:
    ...
    if __opts__['fips_mode'] is False:
        ret['fingerprints']['md5'] = x509util.pretty_hex(cert.fingerprint(algorithm=hashes.MD5()))
    return ret
...
def read_crl(crl):
    ...
    crl = x509util.load_crl(crl)
    ret = {'issuer': _parse_dn(crl.issuer), 'last_update': crl.last_update.strftime(x509util.TIME_FMT), 'next_update': crl.next_update.strftime(x509util.TIME_FMT), 'revoked_certificates': {}, 'extensions': _parse_extensions(crl.extensions)}
    try:
    ...
    for revoked in crl:
        ret['revoked_certificates'].update({x509util.dec2hex(revoked.serial_number).replace(':', ''): {'revocation_date': revoked.revocation_date.strftime(x509util.TIME_FMT), 'extensions': _parse_crl_entry_extensions(revoked.extensions)}})
    return ret
...
def read_csr(csr):
    ...
    return {'key_size': csr.public_key().key_size if key_type in ['ec', 'rsa'] else None, 'key_type': key_type, 'subject': _parse_dn(csr.subject), 'subject_hash': x509util.pretty_hex(_get_name_hash(csr.subject)), 'subject_str': csr.subject.rfc4514_string(), 'public_key_hash': x509util.pretty_hex(cx509.SubjectKeyIdentifier.from_public_key(csr.public_key()).digest), 'extensions': _parse_extensions(csr.extensions)}

def sign_remote_certificate(signing_policy, kwargs, get_signing_policy_only=False, **more_kwargs):
    if False:
        i = 10
        return i + 15
    ...
    try:
        ...
        if not signing_policy:
            ret['errors'].append('signing_policy must be specified and defined on signing minion')
            return ret
        ...
        if get_signing_policy_only:
            ...
            if 'signing_cert' in signing_policy:
                try:
                    signing_policy['signing_cert'] = x509util.to_der(x509util.load_cert(signing_policy['signing_cert']))
                except (CommandExecutionError, SaltInvocationError) as err:
                    ret['data'] = None
        ...
        log.error(str(err))
        return {'data': None, 'errors': ['Failed building the signing policy. See CA server log for details.']}
    try:
...
def _query_remote(ca_server, signing_policy, kwargs, get_signing_policy_only=False):
    if False:
        for i in range(10):
            print('nop')
    result = __salt__['publish.publish'](ca_server, 'x509.sign_remote_certificate', arg=[signing_policy, kwargs, get_signing_policy_only])
    if not result:
        raise SaltInvocationError('ca_server did not respond. Salt master must permit peers to call the sign_remote_certificate function.')
    result = result[next(iter(result))]
    if not isinstance(result, dict) or 'data' not in result:
        log.error(f'Received invalid return value from ca_server: {result}')
        raise CommandExecutionError('Received invalid return value from ca_server. See minion log for details')
    if result.get('errors'):
        raise CommandExecutionError('ca_server reported errors:\n' + '\n'.join(result['errors']))
    return result['data']
...
def verify_signature(certificate, signing_pub_key=None, signing_pub_key_passphrase=None):
    ...
    cert = x509util.load_cert(certificate)
    pubkey = x509util.load_pubkey(get_public_key(signing_pub_key or certificate, passphrase=signing_pub_key_passphrase))
...
def _make_pem_regex(pem_type):
    ...
    return re.compile(f'\\s*(?P<pem_header>-----BEGIN {pem_type}-----)\\s+(?:(?P<proc_type>Proc-Type: 4,ENCRYPTED)\\s*)?(?:(?P<dek_info>DEK-Info: (?:DES-[3A-Z\\-]+,[0-9A-F]{{{{16}}}}|[0-9A-Z\\-]+,[0-9A-F]{{{{32}}}}))\\s*)?(?P<pem_body>.+?)\\s+(?P<pem_footer>-----END {pem_type}-----)\\s*', re.DOTALL)
...
def _generate_pk(algo='rsa', keysize=None):
    ...
    raise SaltInvocationError(f'Invalid algorithm specified for generating private key: {algo}. Valid: rsa, ec, ed25519, ed448')
...
def _get_name_hash(name, digest='sha1'):
    ...
    if digest.lower() not in ['sha1', 'md5']:
        raise ValueError(f'Invalid hashing algorithm for name hash: {digest}. Only SHA1 and MD5 are allowed')
...
def _match_minions(test, minion):
    ...
    if '@' in test:
        ...
        if minion not in match:
            raise CommandExecutionError('Could not verify if minion matches compound matching expression. Make sure the ca_server is allowed to run `match.compound` on the requesting minion')
        return match[minion]
    else:
