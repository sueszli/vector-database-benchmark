...
from ...utils import checkout_add_promo_code, checkout_complete, checkout_create, checkout_delivery_method_update, checkout_dummy_payment_create, checkout_lines_add, raw_checkout_add_promo_code
...
def prepare_voucher(e2e_staff_api_client, channel_id, voucher_code, voucher_discount_type, voucher_discount_value, voucher_type, products):
    if False:
        return 10
    input = {'code': voucher_code, 'discountValueType': voucher_discount_type, 'type': voucher_type, 'minCheckoutItemsQuantity': 3, 'usageLimit': 1, 'singleUse': True, 'products': products}
...
@pytest.mark.e2e
def test_checkout_with_voucher_with_min_item_quantity_core_0908(e2e_not_logged_api_client, e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_shipping, permission_manage_product_types_and_attributes, permission_manage_discounts, permission_manage_checkouts):
    if False:
        print('Hello World!')
    ...
    (product_id, product_variant_id, product_variant_price) = prepare_product(e2e_staff_api_client, warehouse_id, channel_id, variant_price=19.99)
    (voucher_code, voucher_id) = prepare_voucher(e2e_staff_api_client, channel_id, voucher_code='PERCENTAGE_VOUCHER', voucher_discount_type='PERCENTAGE', voucher_discount_value=10, voucher_type='SPECIFIC_PRODUCT', products=product_id)
    ...
    data = raw_checkout_add_promo_code(e2e_not_logged_api_client, checkout_id, voucher_code)
    error = data['errors'][0]
    ...
    checkout_data = checkout_lines_add(e2e_staff_api_client, checkout_id, lines)
    ...
    assert discounted_unit_price == unit_price - round(float(product_variant_price) * 10 / 100, 2)
