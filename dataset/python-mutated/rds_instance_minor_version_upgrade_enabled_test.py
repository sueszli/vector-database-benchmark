...
class Test_rds_instance_minor_version_upgrade_enabled:
    ...
    def test_rds_no_instances(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.rds.rds_instance_minor_version_upgrade_enabled.rds_instance_minor_version_upgrade_enabled.rds_client', new=RDS(audit_info)):
                from prowler.providers.aws.services.rds.rds_instance_minor_version_upgrade_enabled.rds_instance_minor_version_upgrade_enabled import rds_instance_minor_version_upgrade_enabled
    ...
    def test_rds_instance_no_auto_upgrade(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.rds.rds_instance_minor_version_upgrade_enabled.rds_instance_minor_version_upgrade_enabled.rds_client', new=RDS(audit_info)):
                ...
                assert search('does not have minor version upgrade enabled', result[0].status_extended)
    ...
    def test_rds_instance_with_auto_upgrade(self):
        if False:
            print('Hello World!')
        conn = client('rds', region_name=AWS_REGION)
        conn.create_db_instance(DBInstanceIdentifier='db-master-1', AllocatedStorage=10, Engine='postgres', DBName='staging-postgres', DBInstanceClass='db.m1.small', AutoMinorVersionUpgrade=True)
        from prowler.providers.aws.services.rds.rds_service import RDS
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.rds.rds_instance_minor_version_upgrade_enabled.rds_instance_minor_version_upgrade_enabled.rds_client', new=RDS(audit_info)):
                ...
                assert search('has minor version upgrade enabled', result[0].status_extended)
                assert result[0].resource_id == 'db-master-1'
                assert result[0].region == AWS_REGION
                assert result[0].resource_arn == f'arn:aws:rds:{AWS_REGION}:{AWS_ACCOUNT_NUMBER}:db:db-master-1'
                assert result[0].resource_tags == []