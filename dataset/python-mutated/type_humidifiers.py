...
from homeassistant.components.humidifier import ATTR_CURRENT_HUMIDITY, ATTR_HUMIDITY, ATTR_MAX_HUMIDITY, ATTR_MIN_HUMIDITY, DEFAULT_MAX_HUMIDITY, DEFAULT_MIN_HUMIDITY, DOMAIN, SERVICE_SET_HUMIDITY, HumidifierDeviceClass
from homeassistant.const import ATTR_DEVICE_CLASS, ATTR_ENTITY_ID, PERCENTAGE, SERVICE_TURN_OFF, SERVICE_TURN_ON, STATE_ON
from homeassistant.core import State, callback
from homeassistant.helpers.event import EventStateChangedData, async_track_state_change_event
from homeassistant.helpers.typing import EventType
from .accessories import TYPES, HomeAccessory
from .const import CHAR_ACTIVE, CHAR_CURRENT_HUMIDIFIER_DEHUMIDIFIER, CHAR_CURRENT_HUMIDITY, CHAR_DEHUMIDIFIER_THRESHOLD_HUMIDITY, CHAR_HUMIDIFIER_THRESHOLD_HUMIDITY, CHAR_TARGET_HUMIDIFIER_DEHUMIDIFIER, CONF_LINKED_HUMIDITY_SENSOR, PROP_MAX_VALUE, PROP_MIN_STEP, PROP_MIN_VALUE, SERV_HUMIDIFIER_DEHUMIDIFIER
_LOGGER = logging.getLogger(__name__)
HC_HUMIDIFIER = 1
HC_DEHUMIDIFIER = 2
HC_HASS_TO_HOMEKIT_DEVICE_CLASS = {HumidifierDeviceClass.HUMIDIFIER: HC_HUMIDIFIER, HumidifierDeviceClass.DEHUMIDIFIER: HC_DEHUMIDIFIER}
HC_HASS_TO_HOMEKIT_DEVICE_CLASS_NAME = {HumidifierDeviceClass.HUMIDIFIER: 'Humidifier', HumidifierDeviceClass.DEHUMIDIFIER: 'Dehumidifier'}
HC_DEVICE_CLASS_TO_TARGET_CHAR = {HC_HUMIDIFIER: CHAR_HUMIDIFIER_THRESHOLD_HUMIDITY, HC_DEHUMIDIFIER: CHAR_DEHUMIDIFIER_THRESHOLD_HUMIDITY}
HC_STATE_INACTIVE = 0
HC_STATE_IDLE = 1
HC_STATE_HUMIDIFYING = 2
HC_STATE_DEHUMIDIFYING = 3
BASE_VALID_VALUES = {'Inactive': HC_STATE_INACTIVE, 'Idle': HC_STATE_IDLE}
VALID_VALUES_BY_DEVICE_CLASS = {HumidifierDeviceClass.HUMIDIFIER: {**BASE_VALID_VALUES, 'Humidifying': HC_STATE_HUMIDIFYING}, HumidifierDeviceClass.DEHUMIDIFIER: {**BASE_VALID_VALUES, 'Dehumidifying': HC_STATE_DEHUMIDIFYING}}
...
class HumidifierDehumidifier(HomeAccessory):
    ...
    async def run(self) -> None:
        ...
        """
        if self.linked_humidity_sensor:
            self._subscriptions.append(async_track_state_change_event(self.hass, [self.linked_humidity_sensor], self.async_update_current_humidity_event))
        await super().run()
