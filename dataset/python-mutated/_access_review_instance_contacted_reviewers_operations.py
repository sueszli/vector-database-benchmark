...
def build_list_request(schedule_definition_id: str, id: str, subscription_id: str, **kwargs: Any) -> HttpRequest:
    ...
    _url = kwargs.pop('template_url', '/subscriptions/{subscriptionId}/providers/Microsoft.Authorization/accessReviewScheduleDefinitions/{scheduleDefinitionId}/instances/{id}/contactedReviewers')
    path_format_arguments = {'subscriptionId': _SERIALIZER.url('subscription_id', subscription_id, 'str'), 'scheduleDefinitionId': _SERIALIZER.url('schedule_definition_id', schedule_definition_id, 'str'), 'id': _SERIALIZER.url('id', id, 'str')}
...
class AccessReviewInstanceContactedReviewersOperations:
    ...
    @distributed_trace
    def list(self, schedule_definition_id: str, id: str, **kwargs: Any) -> Iterable['_models.AccessReviewContactedReviewer']:
        if False:
            while True:
                i = 10
        ...
        def prepare_request(next_link=None):
            if False:
                ...
                return i + 15
            if not next_link:
                request = build_list_request(schedule_definition_id=schedule_definition_id, id=id, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
