...
def upgrade():
    ...
    op.add_column('deployment', sa.Column('parameter_openapi_schema', prefect.server.utilities.database.JSON(), nullable=True))
    with op.batch_alter_table('deployment', schema=None) as batch_op:
        batch_op.add_column(sa.Column('storage_document_id', prefect.server.utilities.database.UUID(), nullable=True))
        batch_op.create_foreign_key(batch_op.f('fk_deployment__storage_document_id__block_document'), 'block_document', ['storage_document_id'], ['id'], ondelete='CASCADE')
...
def downgrade():
    ...
    with op.batch_alter_table('deployment', schema=None) as batch_op:
        ...
        batch_op.drop_constraint(batch_op.f('fk_deployment__storage_document_id__block_document'), type_='foreignkey')
