...
class test_BarrierState:
    ...
    async def test_parallel_join(self):
        ...
        assert p.size == 0
        await asyncio.gather(self.adder(p), self.fulfiller(p), self.finalizer(p, 1.0), self.joiner(p))
    ...
    async def test_parallel_iterate(self):
        ...
        assert not p.pending
        assert p.size == 0
        (done, pending) = await asyncio.wait([self.adder(p), self.fulfiller(p), self.finalizer(p, 1.0), self.consumer(p)], timeout=5.0)
        if pending:
            raise Exception(f'Test did not return in 5s:\n  DONE_TASKS={done}\n  PENDING_TASKS={pending}\n  size={p.size}\n  total={p.total}\n  fulfilled={p.fulfilled}\n  pending={len(p.pending)}\n  done={p.done()}  result={(p.result() if p.done() else None)}')
...
class test_ReplyConsumer:
    ...
    async def test_drain_replies(self, *, c):
        responses = [ReqRepResponse(key='key1', value='value1', correlation_id='id1'), ReqRepResponse(key='key2', value='value2', correlation_id='id2')]
