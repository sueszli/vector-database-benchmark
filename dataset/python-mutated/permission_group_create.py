...
class PermissionGroupInput(BaseInputObjectType):
    add_permissions = NonNullList(PermissionEnum, description='List of permission code names to assign to this group.', required=False)
    add_users = NonNullList(graphene.ID, description='List of users to assign to this group.', required=False)
    add_channels = NonNullList(graphene.ID, description='List of channels to assign to this group.' + ADDED_IN_314 + PREVIEW_FEATURE)
...
class PermissionGroupCreateInput(PermissionGroupInput):
    ...
    restricted_access_to_channels = graphene.Boolean(description='Determine if the group has restricted access to channels.  DEFAULT: False' + ADDED_IN_314 + PREVIEW_FEATURE, default_value=False, required=False)
...
class PermissionGroupCreate(ModelMutation):
    ...
    class Arguments:
        input = PermissionGroupCreateInput(description='Input fields to create permission group.', required=True)
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.PERMISSION_GROUP_CREATED)]
