...
from sentry.db.models import BoundedBigIntegerField, BoundedPositiveIntegerField, FlexibleForeignKey, Model, region_silo_only_model
...
indexstore = LazyServiceWrapper(NodeStorage, settings.SENTRY_INDEXSTORE, settings.SENTRY_INDEXSTORE_OPTIONS, metrics_path='indexstore')
...
class FlatFileIndexState(Model):
    ...
    indexing_state = models.IntegerField(choices=ArtifactBundleIndexingState.choices(), db_index=True)
    date_added = models.DateTimeField(default=timezone.now)
