...
if is_torch_available():
    import torch
    from transformers import GPTJ_PRETRAINED_MODEL_ARCHIVE_LIST, AutoTokenizer, GPTJForCausalLM, GPTJForQuestionAnswering, GPTJForSequenceClassification, GPTJModel
...
class GPTJModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': GPTJModel, 'question-answering': GPTJForQuestionAnswering, 'text-classification': GPTJForSequenceClassification, 'text-generation': GPTJForCausalLM, 'zero-shot': GPTJForSequenceClassification} if is_torch_available() else {}
    fx_compatible = True
    test_pruning = False
