...
class TFRegNetModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': TFRegNetModel, 'image-classification': TFRegNetForImageClassification} if is_tf_available() else {}
    test_pruning = False
    test_onnx = False
...
class RegNetModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        if False:
            while True:
                i = 10
        return AutoImageProcessor.from_pretrained(TF_REGNET_PRETRAINED_MODEL_ARCHIVE_LIST[0]) if is_vision_available() else None
    ...
    def test_inference_image_classification_head(self):
        ...
        expected_slice = tf.constant([-0.418, -1.5051, -3.4836])
