...
flags.DEFINE_float('step_size', 0.0001, 'Step size for training')
flags.DEFINE_list('hidden_layer_sizes', None, 'Number of hidden units and layers in the network')
...
flags.DEFINE_string('checkpoint_file', None, 'Provides weights and optimzer state to resume training')
...
def batch(dataset, batch_size: int):
    ...
    observations = np.zeros([batch_size] + GAME.information_state_tensor_shape(), np.float32)
...
def main(argv):
    ...
    if FLAGS.data_path is None:
        raise app.UsageError('Please generate your own supervised training data and supply the locallocation as --data_path')
