...
from typing import TYPE_CHECKING, Any, Awaitable, Dict, Generator, Generic, List, Match, NoReturn, Optional, Type, Union
...
_STORING_DATA_WIKI = 'https://github.com/python-telegram-bot/python-telegram-bot/wiki/Storing-bot%2C-user-and-chat-related-data'
...
class CallbackContext(Generic[BT, UD, CD, BD]):
    ...
    """
    __slots__ = ('_application', '_chat_id', '_user_id', 'args', 'matches', 'error', 'job', 'coroutine', '__dict__')
    ...
    async def refresh_data(self) -> None:
        ...
        if self.application.persistence:
            ...
            if self.application.persistence.store_data.chat_data and self._chat_id is not None:
                await self.application.persistence.refresh_chat_data(chat_id=self._chat_id, chat_data=self.chat_data)
            if self.application.persistence.store_data.user_data and self._user_id is not None:
                await self.application.persistence.refresh_user_data(user_id=self._user_id, user_data=self.user_data)
