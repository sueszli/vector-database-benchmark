...
_MAYBE_NESTED_PARENS = textwrap.dedent('\n    (?:\n        (?: [^(]* [(] [^()]* [)] )* [^(]*\n    )\n')
CAPI_FUNC = textwrap.dedent(f'\n    (?:\n        ^\n        \\s*\n        PyAPI_FUNC \\s*\n        [(]\n        {_ind(_MAYBE_NESTED_PARENS, 2)}\n        [)] \\s*\n        (\\w+)  # <func>\n        \\s* [(]\n    )\n')
CAPI_DATA = textwrap.dedent(f'\n    (?:\n        ^\n        \\s*\n        PyAPI_DATA \\s*\n        [(]\n        {_ind(_MAYBE_NESTED_PARENS, 2)}\n        [)] \\s*\n        (\\w+)  # <data>\n        \\b [^(]\n    )\n')
CAPI_INLINE = textwrap.dedent('\n    (?:\n        ^\n        \\s*\n        static \\s+ inline \\s+\n        .*?\n        \\s+\n        ( \\w+ )  # <inline>\n        \\s* [(]\n    )\n')
CAPI_MACRO = textwrap.dedent('\n    (?:\n        (\\w+)  # <macro>\n        [(]\n    )\n')
CAPI_CONSTANT = textwrap.dedent('\n    (?:\n        (\\w+)  # <constant>\n        \\s+ [^(]\n    )\n')
CAPI_DEFINE = textwrap.dedent(f'\n    (?:\n        ^\n        \\s* [#] \\s* define \\s+\n        (?:\n            {_ind(CAPI_MACRO, 3)}\n            |\n            {_ind(CAPI_CONSTANT, 3)}\n            |\n            (?:\n                # ignored\n                \\w+   # <defined_name>\n                \\s*\n                $\n            )\n        )\n    )\n')
CAPI_RE = re.compile(textwrap.dedent(f'\n    (?:\n        {_ind(CAPI_FUNC, 2)}\n        |\n        {_ind(CAPI_DATA, 2)}\n        |\n        {_ind(CAPI_INLINE, 2)}\n        |\n        {_ind(CAPI_DEFINE, 2)}\n    )\n'), re.VERBOSE)
KINDS = ['func', 'data', 'inline', 'macro', 'constant']
...
def _parse_line(line, prev=None):
    ...
    for (kind, name) in results:
        if name:
            ...
            return line
    return None
LEVELS = ['stable', 'cpython', 'private', 'internal']
...
def _get_level(filename, name, *, _cpython=INCLUDE_CPYTHON + os.path.sep, _internal=INCLUDE_INTERNAL + os.path.sep):
    ...
    if filename.startswith(_internal):
        ...
        raise NotImplementedError
GROUPINGS = {'kind': KINDS, 'level': LEVELS}
...
def _get_sortkey(sort, _groupby, _columns):
    ...
    if sort is True or sort is None:
        ...
        raise NotImplementedError
    raise NotImplementedError
_MARKERS = {'level': {'S': 'stable', 'C': 'cpython', 'P': 'private', 'I': 'internal'}, 'kind': {'F': 'func', 'D': 'data', 'I': 'inline', 'M': 'macro', 'C': 'constant'}}
...
_FORMATS = {'table': render_table, 'full': render_full, 'summary': render_summary}