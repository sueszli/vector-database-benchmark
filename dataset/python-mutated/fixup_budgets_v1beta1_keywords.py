...
class budgetsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_budget': ('parent', 'budget'), 'delete_budget': ('name',), 'get_budget': ('name',), 'list_budgets': ('parent', 'scope', 'page_size', 'page_token'), 'update_budget': ('budget', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=budgetsCallTransformer()):
    if False:
        while True:
            i = 10
