...
class TestL2Decay(unittest.TestCase):
    ...
    def test_l2decay_regularizer(self):
        ...
        mul_x = block.create_parameter(dtype='float32', shape=[5, 10], lod_level=0, name='mul.x', regularizer=regularizer.L2Decay(0.5))
...
class TestL1Decay(unittest.TestCase):
    ...
    def test_l2decay_regularizer(self):
        ...
        program = framework.Program()
        block = program.global_block()
        mul_x = block.create_parameter(dtype='float32', shape=[5, 10], lod_level=0, name='mul.x', regularizer=regularizer.L1Decay(0.5))
        self.assertIsNotNone(mul_x.regularizer)
