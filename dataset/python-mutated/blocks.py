...
from gradio import analytics, components, networking, processing_utils, queueing, routes, strings, themes, utils, wasm_utils
...
from gradio.exceptions import DuplicateBlockError, InvalidApiNameError, InvalidBlockError, InvalidComponentError
from gradio.helpers import create_tracker, skip, special_args
from gradio.state_holder import SessionState
from gradio.themes import Default as DefaultTheme
from gradio.themes import ThemeClass as Theme
from gradio.tunneling import BINARY_FILENAME, BINARY_FOLDER, BINARY_PATH, BINARY_URL, CURRENT_TUNNELS
from gradio.utils import TupleNoPrint, check_function_inputs_match, component_or_layout_class, get_cancel_function, get_continuous_fn, get_package_version
try:
    import spaces
except Exception:
    spaces = None
...
BUILT_IN_THEMES: dict[str, Theme] = {t.name: t for t in [themes.Base(), themes.Default(), themes.Monochrome(), themes.Soft(), themes.Glass()]}
...
class Blocks(BlockContext, BlocksEvents, metaclass=BlocksMeta):
    ...
    async def call_function(self, fn_index: int, processed_input: list[Any], iterator: AsyncIterator[Any] | None=None, requests: routes.Request | list[routes.Request] | None=None, event_id: str | None=None, event_data: EventData | None=None, in_event_listener: bool=False):
        """
        ...
        start = time.time()
        fn = utils.get_function_with_locals(fn=block_fn.fn, blocks=self, event_id=event_id, in_event_listener=in_event_listener, request=request)
        if iterator is None:
            if block_fn.inputs_as_dict:
                processed_input = [dict(zip(block_fn.inputs, processed_input))]
            (processed_input, progress_index, _) = special_args(block_fn.fn, processed_input, request, event_data)
            ...
            if inspect.iscoroutinefunction(fn):
                ...
                prediction = await anyio.to_thread.run_sync(fn, *processed_input, limiter=self.limiter)
        else:
            prediction = None
        if inspect.isgeneratorfunction(fn) or inspect.isasyncgenfunction(fn):
            try:
                ...
                prediction = components._Keywords.FINISHED_ITERATING if n_outputs == 1 else (components._Keywords.FINISHED_ITERATING,) * n_outputs
                iterator = None
        duration = time.time() - start
        return {'prediction': prediction, 'duration': duration, 'is_generating': is_generating, 'iterator': iterator}
    ...
    async def process_api(self, fn_index: int, inputs: list[Any], state: SessionState | None=None, request: routes.Request | list[routes.Request] | None=None, iterator: AsyncIterator | None=None, session_hash: str | None=None, event_id: str | None=None, event_data: EventData | None=None, in_event_listener: bool=True) -> dict[str, Any]:
        """
        ...
        if batch:
            ...
            if not all((x == batch_size for x in batch_sizes)):
                raise ValueError(f'All inputs to a batch function must have the same length but instead have sizes: {batch_sizes}.')
            ...
            result = await self.call_function(fn_index, list(zip(*inputs)), None, request, event_id, event_data, in_event_listener)
            ...
            result = await self.call_function(fn_index, inputs, old_iterator, request, event_id, event_data, in_event_listener)
            data = self.postprocess_data(fn_index, result['prediction'], state)
            (is_generating, iterator) = (result['is_generating'], result['iterator'])
            if is_generating or was_generating:
                data = self.handle_streaming_outputs(fn_index, data, session_hash=session_hash, run=id(old_iterator) if was_generating else id(iterator))
        block_fn.total_runtime += result['duration']
        block_fn.total_runs += 1
        return {'data': data, 'is_generating': is_generating, 'iterator': iterator, 'duration': result['duration'], 'average_duration': block_fn.total_runtime / block_fn.total_runs}
