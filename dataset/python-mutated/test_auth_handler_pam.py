...
async def test_bad_pwd_pam_chsh_service(http_client, auth_creds, content_type_map, service):
    """
    ...
    with pytest.raises(HTTPError) as exc:
        await http_client.fetch('/login', method='POST', body=urllib.parse.urlencode(auth_creds), headers={'Content-Type': content_type_map['form']})
    assert exc.value.code == 401

@pytest.mark.parametrize('service', ['chsh', 'login'])
async def test_good_pwd_pam_chsh_service(http_client, auth_creds, content_type_map, service):
    """
