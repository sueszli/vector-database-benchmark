...
class AsyncEngineTest(EngineFixture):
    ...
    @async_test
    async def test_interrupt_ctxmanager_connection(self, async_engine, async_trans_ctx_manager_fixture):
    ...
    async def test_connection_eq_ne(self, async_engine):
        async with async_engine.connect() as conn:
            c2 = _async_engine.AsyncConnection(async_engine, conn.sync_connection)
            eq_(conn, c2)
    ...
    async def test_transaction_eq_ne(self, async_engine):
        async with async_engine.connect() as conn:
            t1 = await conn.begin()
            t2 = _async_engine.AsyncTransaction._regenerate_proxy_for_target(t1._proxied)
            eq_(t1, t2)
            is_false(t1 == None)
    ...
    async def test_transaction_accessor(self, async_connection):
        ...
        is_(conn.get_nested_transaction().sync_transaction, nested.sync_transaction)
    ...
    async def test_get_dbapi_connection_raise(self, async_connection):
        with testing.expect_raises_message(exc.InvalidRequestError, 'AsyncConnection.connection accessor is not implemented as the attribute'):
    ...
    async def test_isolation_level(self, async_connection):
        conn = async_connection
        sync_isolation_level = await greenlet_spawn(conn.sync_connection.get_isolation_level)
    ...
    async def test_connection_not_started(self, async_engine):
        ...
        testing.assert_raises_message(asyncio_exc.AsyncContextNotStarted, 'AsyncConnection context has not been started and object has not been awaited.', conn.begin)
    ...
    async def test_conn_transaction_not_started(self, async_engine):
        async with async_engine.connect() as conn:
            trans = conn.begin()
            with expect_raises_message(asyncio_exc.AsyncContextNotStarted, 'AsyncTransaction context has not been started and object has not been awaited.'):
                (await trans.rollback(),)

    @testing.requires.queue_pool
    @async_test
    async def test_pool_exhausted_some_timeout(self, async_engine):
        engine = create_async_engine(testing.db.url, pool_size=1, max_overflow=0, pool_timeout=0.1)
    ...
    async def test_pool_exhausted_no_timeout(self, async_engine):
        engine = create_async_engine(testing.db.url, pool_size=1, max_overflow=0, pool_timeout=0)
        async with engine.connect():
            with expect_raises(exc.TimeoutError):
                await engine.connect()
    ...
    async def test_create_async_engine_server_side_cursor(self, async_engine):
        with expect_raises_message(asyncio_exc.AsyncMethodRequired, "Can't set server_side_cursors for async engine globally"):
            create_async_engine(testing.db.url, server_side_cursors=True)
    ...
    @async_test
    async def test_async_creator_accepts_args_if_called_directly(self, async_testing_engine):
        ...
        conn = await greenlet_spawn(async_dbapi.connect, 5, y=10, z=8, async_creator_fn=async_creator)
        try:
...
class AsyncEventTest(EngineFixture):
    ...
    async def test_no_async_listeners(self, async_engine):
        with testing.expect_raises_message(NotImplementedError, 'asynchronous events are not implemented at this time.  Apply synchronous listeners to the AsyncEngine.sync_engine or AsyncConnection.sync_connection attributes.'):
    ...
    async def test_sync_before_cursor_execute_engine(self, async_engine):
        ...
        eq_(canary.mock_calls, [mock.call(sync_conn, mock.ANY, 'select 1', mock.ANY, mock.ANY, False)])
    ...
    async def test_event_on_sync_connection(self, async_engine):
        ...
        async with async_engine.connect() as conn:
            ...
            async with conn.begin():
                eq_(canary.mock_calls, [mock.call(conn.sync_connection)])
...
class AsyncInspection(EngineFixture):
    ...
    async def test_inspect_engine(self, async_engine):
        with testing.expect_raises_message(exc.NoInspectionAvailable, 'Inspection on an AsyncEngine is currently not supported.'):
            inspect(async_engine)

    @async_test
    async def test_inspect_connection(self, async_engine):
        async with async_engine.connect() as conn:
            with testing.expect_raises_message(exc.NoInspectionAvailable, 'Inspection on an AsyncConnection is currently not supported.'):
                inspect(conn)
...
class AsyncResultTest(EngineFixture):
    ...
    async def test_no_ss_cursor_w_execute(self, async_engine):
        ...
        async with async_engine.connect() as conn:
            ...
            with expect_raises_message(async_exc.AsyncMethodRequired, "Can't use the AsyncConnection.execute\\(\\) method with a server-side cursor. Use the AsyncConnection.stream\\(\\) method for an async streaming result set."):
    ...
    async def test_no_ss_cursor_w_exec_driver_sql(self, async_engine):
        async with async_engine.connect() as conn:
            conn = await conn.execution_options(stream_results=True)
            with expect_raises_message(async_exc.AsyncMethodRequired, "Can't use the AsyncConnection.exec_driver_sql\\(\\) method with a server-side cursor. Use the AsyncConnection.stream\\(\\) method for an async streaming result set."):
    ...
    async def test_stream_scalars_ctxmanager(self, async_engine):
        async with async_engine.connect() as conn:
            conn = await conn.execution_options(stream_results=True)
            async with conn.stream_scalars(select(self.tables.users)) as result:
                assert not result._real_result._soft_closed
                assert not result.closed
                with expect_raises_message(Exception, 'hi'):
                    i = 0
    ...
    @testing.combinations((None,), ('scalars',), ('mappings',), argnames='filter_')
    @async_test
    async def test_all(self, async_engine, filter_):
        ...
        async with async_engine.connect() as conn:
            ...
            if filter_ == 'mappings':
                eq_(all_, [{'user_id': i, 'user_name': 'name%d' % i} for i in range(1, 20)])
            elif filter_ == 'scalars':
                eq_(all_, ['name%d' % i for i in range(1, 20)])
            else:
                eq_(all_, [(i, 'name%d' % i) for i in range(1, 20)])

    @testing.combinations((None,), ('scalars',), ('stream_scalars',), ('mappings',), argnames='filter_')
    @async_test
    async def test_aiter(self, async_engine, filter_):
        ...
        async with async_engine.connect() as conn:
            ...
            async for row in result:
                rows.append(row)
            if filter_ == 'mappings':
                eq_(rows, [{'user_id': i, 'user_name': 'name%d' % i} for i in range(1, 20)])
            elif filter_ in ('scalars', 'stream_scalars'):
                eq_(rows, ['name%d' % i for i in range(1, 20)])
            else:
    ...
    async def test_unique_all(self, async_engine):
        users = self.tables.users
        async with async_engine.connect() as conn:
            result = await conn.stream(union_all(select(users), select(users)).order_by(users.c.user_id))
    ...
    @async_test
    async def test_partitions(self, async_engine, filter_, yield_per, yield_per_type):
        users = self.tables.users
        async with async_engine.connect() as conn:
            stmt = select(users)
            ...
            check_result = []
            assert isinstance(result._real_result.cursor_strategy, _cursor.BufferedRowCursorFetchStrategy)
            if yield_per:
                ...
                async for partition in result.partitions(partition_size):
                    check_result.append(partition)
            ranges = [(i, min(20, i + partition_size)) for i in range(1, 21, partition_size)]
            if filter_ == 'mappings':
                eq_(check_result, [[{'user_id': i, 'user_name': 'name%d' % i} for i in range(a, b)] for (a, b) in ranges])
            elif filter_ == 'scalars':
                eq_(check_result, [['name%d' % i for i in range(a, b)] for (a, b) in ranges])
            else:
                eq_(check_result, [[(i, 'name%d' % i) for i in range(a, b)] for (a, b) in ranges])
    ...
    async def test_one_success(self, async_engine, filter_):
        users = self.tables.users
        async with async_engine.connect() as conn:
            result = await conn.stream(select(users).limit(1).order_by(users.c.user_name))
            if filter_ == 'mappings':
                result = result.mappings()
            elif filter_ == 'scalars':
    ...
    async def test_one_no_result(self, async_engine):
        users = self.tables.users
        async with async_engine.connect() as conn:
            result = await conn.stream(select(users).where(users.c.user_name == 'nonexistent'))
    ...
    async def test_one_multi_result(self, async_engine):
        users = self.tables.users
        async with async_engine.connect() as conn:
            result = await conn.stream(select(users).where(users.c.user_name.in_(['name3', 'name5'])))
            with expect_raises_message(exc.MultipleResultsFound, 'Multiple rows were found when exactly one was required'):
                await result.one()
...
class TextSyncDBAPI(fixtures.TestBase):
    ...
    @async_test
    @combinations(lambda conn: conn.exec_driver_sql('select 1'), lambda conn: conn.stream(text('select 1')), lambda conn: conn.execute(text('select 1')), argnames='case')
    async def test_sync_driver_execution(self, async_engine, case):
        with expect_raises_message(exc.AwaitRequired, 'The current operation required an async execution but none was'):
            async with async_engine.connect() as conn:
                await case(conn)
    ...
    async def test_sync_driver_run_sync(self, async_engine):
        async with async_engine.connect() as conn:
            res = await conn.run_sync(lambda conn: conn.scalar(text('select 1')))
            assert res == 1
...
class AsyncProxyTest(EngineFixture, fixtures.TestBase):
    ...
    async def test_get_connection(self, async_engine):
        async with async_engine.connect() as conn:
            is_(AsyncConnection._retrieve_proxy_for_target(conn.sync_connection), conn)
