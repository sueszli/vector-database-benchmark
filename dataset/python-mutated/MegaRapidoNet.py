...
class MegaRapidoNet(MultiDownloader):
    ...
    __version__ = '0.12'
    __status__ = 'testing'
    __pattern__ = 'http://(?:www\\.)?\\w+\\.megarapido\\.net/\\?file=\\w+'
    ...
    LINK_PREMIUM_PATTERN = '<\\s*?a[^>]*?title\\s*?=\\s*?["\\\'].*?download["\\\'][^>]*?href=["\\\']([^"\\\']+)'
    ERROR_PATTERN = '<\\s*?div[^>]*?class\\s*?=\\s*?["\\\']?alert-message error.*?>([^<]*)'

    def handle_premium(self, pyfile):
        if False:
            return 10
        self.data = self.load('http://megarapido.net/gerar.php', post={'rand': random_with_n_digits(16), 'urllist': pyfile.url, 'links': pyfile.url, 'exibir': 'normal', 'usar': 'premium', 'user': self.account.get_data('sid'), 'autoreset': ''})
