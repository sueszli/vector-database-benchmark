...
from prefect.infrastructure.container import CONTAINER_LABELS, DockerContainer, DockerRegistry, ImagePullPolicy
...
async def test_kill_calls_container_stop_with_correct_grace_seconds(mock_docker_client):
    """Happy path for kill"""
    GRACE_SECONDS = 42
    await DockerContainer().kill(infrastructure_pid=f'{FAKE_BASE_URL}:{FAKE_CONTAINER_ID}', grace_seconds=GRACE_SECONDS)
    mock_docker_client.containers.get.return_value.stop.assert_called_with(timeout=GRACE_SECONDS)

async def test_kill_raises_infra_not_available_with_bad_host_url(mock_docker_client):
    BAD_BASE_URL = 'bad-base-url'
    expected_string = ''.join([f'Unable to stop container {FAKE_CONTAINER_ID!r}: the current Docker API ', f'URL {mock_docker_client.api.base_url!r} does not match the expected ', f'API base URL {BAD_BASE_URL}.'])
    with pytest.raises(InfrastructureNotAvailable, match=re.escape(expected_string)):
        await DockerContainer().kill(infrastructure_pid=f'{BAD_BASE_URL}:{FAKE_CONTAINER_ID}', grace_seconds=0)

async def test_kill_raises_infra_not_found_with_bad_container_id(mock_docker_client):
    mock_docker_client.containers.get.side_effect = [docker.errors.NotFound('msg')]
    BAD_CONTAINER_ID = 'bad-container-id'
    with pytest.raises(InfrastructureNotFound, match=f'Unable to stop container {BAD_CONTAINER_ID!r}: The container was not found.'):
        await DockerContainer().kill(infrastructure_pid=f'{FAKE_BASE_URL}:{BAD_CONTAINER_ID}', grace_seconds=0)
...
async def test_uses_image_registry_setting_after_save(mock_docker_client):
    await DockerRegistry(username='foo', password='bar', registry_url='example.test').save('test-docker-registry')
    await DockerContainer(command=['echo', 'hello'], image_registry=await DockerRegistry.load('test-docker-registry'), image_pull_policy='ALWAYS').save('test-docker-container')
