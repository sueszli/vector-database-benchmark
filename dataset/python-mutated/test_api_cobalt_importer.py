...
class TestCobaltApiImporter(DojoTestCase):
    ...
    def test_prepare_client_do_not_match(self):
        ...
        with self.assertRaisesRegex(ValidationError, 'API Scan Configuration for Cobalt\\.io and Product do not match\\. Product: "" \\(None\\), config\\.product: "Product" \\(None\\)'):
    ...
    def test_prepare_client_no_configuration(self, mock_foo):
        ...
        with self.assertRaisesRegex(ValidationError, 'There are no API Scan Configurations for this Product\\. Please add at least one API Scan Configuration for Cobalt\\.io to this Product\\. Product: "Product" \\(None\\)'):
            cobalt_api_importer = CobaltApiImporter()
            cobalt_api_importer.prepare_client(self.test)
            mock_foo.filter.assert_called_with(product=self.product)
