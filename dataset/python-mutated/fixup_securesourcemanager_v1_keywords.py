...
class securesourcemanagerCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_instance': ('parent', 'instance_id', 'instance', 'request_id'), 'create_repository': ('parent', 'repository', 'repository_id'), 'delete_instance': ('name', 'request_id'), 'delete_repository': ('name', 'allow_missing'), 'get_iam_policy_repo': ('resource', 'options'), 'get_instance': ('name',), 'get_repository': ('name',), 'list_instances': ('parent', 'page_size', 'page_token', 'filter', 'order_by'), 'list_repositories': ('parent', 'page_size', 'page_token', 'filter'), 'set_iam_policy_repo': ('resource', 'policy', 'update_mask'), 'test_iam_permissions_repo': ('resource', 'permissions')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=securesourcemanagerCallTransformer()):
    if False:
        return 10
