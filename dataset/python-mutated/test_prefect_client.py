...
from prefect.client.schemas.actions import ArtifactCreate, LogCreate, VariableCreate, WorkPoolCreate, WorkPoolUpdate
from prefect.client.schemas.filters import ArtifactFilter, ArtifactFilterKey, FlowFilter, FlowRunFilter, FlowRunNotificationPolicyFilter, LogFilter, LogFilterFlowRunId
from prefect.client.schemas.objects import Flow, FlowRunNotificationPolicy, FlowRunPolicy, StateType, TaskRun, Variable, WorkQueue
...
from prefect.settings import PREFECT_API_DATABASE_MIGRATE_ON_START, PREFECT_API_KEY, PREFECT_API_TLS_INSECURE_SKIP_VERIFY, PREFECT_API_URL, PREFECT_CLOUD_API_URL, PREFECT_UNIT_TEST_MODE, temporary_settings
...
class TestClientProxyAwareness:
    ...
    async def test_client_is_aware_of_https_proxy(self, remote_https_api: httpx.URL, https_proxy: httpcore.URL):
        httpx_client = get_client()._client
        assert isinstance(httpx_client, httpx.AsyncClient)
        transport_for_api = httpx_client._transport_for_url(remote_https_api)
        assert isinstance(transport_for_api, httpx.AsyncHTTPTransport)
    ...
    async def test_client_is_aware_of_http_proxy(self, remote_http_api: httpx.URL, http_proxy: httpcore.URL):
        httpx_client = get_client()._client
        assert isinstance(httpx_client, httpx.AsyncClient)
    ...
    async def test_client_context_lifespan_is_robust_to_threaded_concurrency(self):
        ...
        async def enter_client(context):
            with context:
                ...
                async with PrefectClient(app):
                    await anyio.sleep(random.random())
        threads = [threading.Thread(target=anyio.run, args=(enter_client, prefect.context.SettingsContext.get().copy())) for _ in range(100)]
        ...
        async def enter_client_many_times(context):
            with context:
                async with anyio.create_task_group() as tg:
                    for _ in range(100):
                        tg.start_soon(enter_client)
        threads = [threading.Thread(target=anyio.run, args=(enter_client_many_times, prefect.context.SettingsContext.get().copy())) for _ in range(100)]
        for thread in threads:
            thread.start()
        for thread in threads:
            thread.join(3)
...
async def test_create_then_read_deployment(prefect_client, infrastructure_document_id, storage_document_id):
    ...
    deployment_id = await prefect_client.create_deployment(flow_id=flow_id, name='test-deployment', version='git-commit-hash', manifest_path='path/file.json', schedule=schedule, parameters={'foo': 'bar'}, tags=['foo', 'bar'], infrastructure_document_id=infrastructure_document_id, storage_document_id=storage_document_id, parameter_openapi_schema={})
...
async def test_updating_deployment(prefect_client, infrastructure_document_id, storage_document_id):
    ...
    updated_schedule = IntervalSchedule(interval=timedelta(seconds=86399))
    await prefect_client.update_deployment(initial_lookup, schedule=updated_schedule, is_schedule_active=False)
...
async def test_read_deployment_by_name(prefect_client):
    ...
    flow_id = await prefect_client.create_flow(foo)
    schedule = IntervalSchedule(interval=timedelta(days=1))
    deployment_id = await prefect_client.create_deployment(flow_id=flow_id, name='test-deployment', manifest_path='file.json', schedule=schedule)
...
async def test_create_flow_run_from_deployment(prefect_client: PrefectClient, deployment):
    ...
    assert pendulum.now('utc').diff(flow_run.state.state_details.scheduled_time).in_seconds() < 1
...
async def test_create_flow_run_from_deployment_with_options(prefect_client, deployment):
    flow_run = await prefect_client.create_flow_run_from_deployment(deployment.id, name='test-run-name', tags={'foo', 'bar'}, state=Pending(message='test'), parameters={'foo': 'bar'})
...
async def test_read_filtered_logs(session, prefect_client, deployment):
    ...
    logs = [LogCreate(name='prefect.flow_runs', level=20, message=f'Log from flow_run {id}.', timestamp=datetime.now(tz=timezone.utc), flow_run_id=id) for id in flow_runs]
...
async def test_prefect_api_tls_insecure_skip_verify_setting_set_to_true(monkeypatch):
    with temporary_settings(updates={PREFECT_API_TLS_INSECURE_SKIP_VERIFY: True}):
        ...
        get_client()
    mock.assert_called_once_with(headers=ANY, verify=False, transport=ANY, base_url=ANY, timeout=ANY)
...
async def test_prefect_api_tls_insecure_skip_verify_setting_set_to_false(monkeypatch):
    with temporary_settings(updates={PREFECT_API_TLS_INSECURE_SKIP_VERIFY: False}):
        ...
        monkeypatch.setattr('prefect.client.orchestration.PrefectHttpxClient', mock)
        get_client()
    mock.assert_called_once_with(headers=ANY, transport=ANY, base_url=ANY, timeout=ANY)
...
class TestResolveDataDoc:
    ...
    async def test_resolves_data_document(self, prefect_client):
        innermost = await prefect_client.resolve_datadoc(DataDocument.encode('cloudpickle', 'hello'))
        assert innermost == 'hello'

    async def test_resolves_nested_data_documents(self, prefect_client):
        innermost = await prefect_client.resolve_datadoc(DataDocument.encode('cloudpickle', DataDocument.encode('json', 'hello')))
        assert innermost == 'hello'

    async def test_resolves_nested_data_documents_when_inner_is_bytes(self, prefect_client):
        innermost = await prefect_client.resolve_datadoc(DataDocument.encode('cloudpickle', DataDocument.encode('json', 'hello').json().encode()))
        assert innermost == 'hello'
...
class TestClientWorkQueues:
    ...
    async def deployment(self, prefect_client, infrastructure_document_id):
        ...
        deployment_id = await prefect_client.create_deployment(flow_id=flow_id, name='test-deployment', manifest_path='file.json', schedule=schedule, parameters={'foo': 'bar'}, work_queue_name='wq', infrastructure_document_id=infrastructure_document_id)
    ...
    async def test_get_runs_from_queue_includes(self, session, prefect_client, deployment):
...
@pytest.mark.parametrize('on_create, expected_value', [(True, True), (False, False), (None, True)])
async def test_update_deployment_schedule_active_does_not_overwrite_when_not_provided(prefect_client, flow_run, on_create, expected_value):
    deployment_id = await prefect_client.create_deployment(flow_id=flow_run.flow_id, name='test-deployment', manifest_path='file.json', parameters={'foo': 'bar'}, work_queue_name='wq', is_schedule_active=on_create)
...
@pytest.mark.parametrize('on_create, after_create, on_update, after_update', [(False, False, True, True), (True, True, False, False), (None, True, False, False)])
async def test_update_deployment_schedule_active_overwrites_when_provided(prefect_client, flow_run, on_create, after_create, on_update, after_update):
    deployment_id = await prefect_client.create_deployment(flow_id=flow_run.flow_id, name='test-deployment', manifest_path='file.json', parameters={'foo': 'bar'}, work_queue_name='wq', is_schedule_active=on_create)
    deployment = await prefect_client.read_deployment(deployment_id)
...
class TestVariables:
    ...
    async def variable(self, client):
        res = await client.post('/variables/', json=VariableCreate(name='my_variable', value='my-value', tags=['123', '456']).dict(json_compatible=True))
    ...
    async def variables(self, client):
        ...
        for variable in variables:
            res = await client.post('/variables/', json=variable.dict(json_compatible=True))
            assert res.status_code == 201
...
class TestAutomations:
    ...
    async def test_create_not_cloud_runtime_error(self, prefect_client, automation: Automation):
        with pytest.raises(RuntimeError, match='Automations are only supported for Prefect Cloud.'):
    ...
    async def test_create_automation(self, cloud_client, automation: Automation):
        with respx.mock(base_url=PREFECT_CLOUD_API_URL.value()) as router:
            ...
            assert json.loads(create_route.calls[0].request.content) == automation.dict(json_compatible=True)
            assert automation_id == UUID(created_automation['id'])

    async def test_delete_owned_automations_not_cloud_runtime_error(self, prefect_client):
        with pytest.raises(RuntimeError, match='Automations are only supported for Prefect Cloud.'):
