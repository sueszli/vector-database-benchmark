...
class Test_efs_encryption_at_rest_enabled:
    ...
    def test_efs_encryption_enabled(self):
        ...
        with mock.patch('prowler.providers.aws.services.efs.efs_service.EFS', efs_client):
            from prowler.providers.aws.services.efs.efs_encryption_at_rest_enabled.efs_encryption_at_rest_enabled import efs_encryption_at_rest_enabled
    ...
    def test_efs_encryption_disabled(self):
        ...
        efs_client = mock.MagicMock
        efs_arn = f'arn:aws:elasticfilesystem:{AWS_REGION}:{AWS_ACCOUNT_NUMBER}:file-system/{file_system_id}'
        efs_client.filesystems = [FileSystem(id=file_system_id, arn=efs_arn, region=AWS_REGION, policy=None, backup_policy=backup_valid_policy_status, encrypted=False)]
        with mock.patch('prowler.providers.aws.services.efs.efs_service.EFS', efs_client):
            from prowler.providers.aws.services.efs.efs_encryption_at_rest_enabled.efs_encryption_at_rest_enabled import efs_encryption_at_rest_enabled
            check = efs_encryption_at_rest_enabled()
            result = check.execute()
