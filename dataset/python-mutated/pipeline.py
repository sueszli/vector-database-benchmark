...
class DeployOrchestrator(Step):
    ...
    deploy_dagster_command = ['dagster-cloud', 'serverless', 'deploy-python-executable', '--location-name', 'metadata_service_orchestrator', '--location-file', 'dagster_cloud.yaml', '--organization', 'airbyte-connectors', '--deployment', 'prod', '--python-version', '3.9']
    ...
    async def _run(self) -> StepResult:
        ...
        dagster_cloud_api_token_secret: dagger.Secret = get_secret_host_variable(self.context.dagger_client, 'DAGSTER_CLOUD_METADATA_API_TOKEN')
        container_to_run = python_with_dependencies.with_mounted_directory('/src', parent_dir).with_secret_variable('DAGSTER_CLOUD_API_TOKEN', dagster_cloud_api_token_secret).with_workdir(f'/src/orchestrator').with_exec(['/bin/sh', '-c', 'poetry2setup >> setup.py']).with_exec(self.deploy_dagster_command)
...
async def run_metadata_orchestrator_deploy_pipeline(is_local: bool, git_branch: str, git_revision: str, gha_workflow_run_url: Optional[str], dagger_logs_url: Optional[str], pipeline_start_timestamp: Optional[int], ci_context: Optional[str]) -> bool:
    metadata_pipeline_context = PipelineContext(pipeline_name='Metadata Service Orchestrator Unit Test Pipeline', is_local=is_local, git_branch=git_branch, git_revision=git_revision, gha_workflow_run_url=gha_workflow_run_url, dagger_logs_url=dagger_logs_url, pipeline_start_timestamp=pipeline_start_timestamp, ci_context=ci_context)
    async with dagger.Connection(DAGGER_CONFIG) as dagger_client:
        ...
        async with metadata_pipeline_context:
            ...
            metadata_pipeline_context.report = Report(pipeline_context=metadata_pipeline_context, steps_results=steps_results, name='METADATA ORCHESTRATOR DEPLOY RESULTS')
