...
if is_torch_available():
    import torch
    from transformers import CTRL_PRETRAINED_MODEL_ARCHIVE_LIST, CTRLForSequenceClassification, CTRLLMHeadModel, CTRLModel
...
class CTRLModelTester:
    ...
    def get_config(self):
        if False:
            print('Hello World!')
        return CTRLConfig(vocab_size=self.vocab_size, n_embd=self.hidden_size, n_layer=self.num_hidden_layers, n_head=self.num_attention_heads, dff=self.intermediate_size, n_positions=self.max_position_embeddings, pad_token_id=self.pad_token_id)
...
class CTRLModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': CTRLModel, 'text-classification': CTRLForSequenceClassification, 'text-generation': CTRLLMHeadModel, 'zero-shot': CTRLForSequenceClassification} if is_torch_available() else {}
    test_pruning = True
