...
class Test_compute_loadbalancer_logging_enabled:
    ...
    def test_compute_no_load_balancers(self):
        ...
        with mock.patch('prowler.providers.gcp.services.compute.compute_loadbalancer_logging_enabled.compute_loadbalancer_logging_enabled.compute_client', new=compute_client):
            from prowler.providers.gcp.services.compute.compute_loadbalancer_logging_enabled.compute_loadbalancer_logging_enabled import compute_loadbalancer_logging_enabled
    ...
    def test_one_compliant_load_balancer(self):
        ...
        load_balancer = LoadBalancer(name='test', id='test_id', project_id=GCP_PROJECT_ID, logging=True, service='test')
        compute_client = mock.MagicMock
        compute_client.project_ids = [GCP_PROJECT_ID]
    ...
    def test_one_uncompliant_load_balancer(self):
        if False:
            while True:
                i = 10
        from prowler.providers.gcp.services.compute.compute_service import LoadBalancer
        load_balancer = LoadBalancer(name='test', id='test_id', project_id=GCP_PROJECT_ID, logging=False, service='test')
        compute_client = mock.MagicMock
        compute_client.project_ids = [GCP_PROJECT_ID]
        compute_client.load_balancers = [load_balancer]
        compute_client.region = 'global'
