...
if is_torch_available():
    ...
    from transformers.models.time_series_transformer.modeling_time_series_transformer import TimeSeriesTransformerDecoder, TimeSeriesTransformerEncoder

@require_torch
class TimeSeriesTransformerModelTester:

    def __init__(self, parent, batch_size=13, prediction_length=7, context_length=14, cardinality=19, embedding_dimension=5, num_time_features=4, is_training=True, hidden_size=64, num_hidden_layers=2, num_attention_heads=4, intermediate_size=4, hidden_act='gelu', hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, lags_sequence=[1, 2, 3, 4, 5]):
        if False:
            i = 10
            return i + 15
        self.parent = parent
...
class TimeSeriesTransformerModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TimeSeriesTransformerModel, TimeSeriesTransformerForPrediction) if is_torch_available() else ()
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=TimeSeriesTransformerConfig, has_text_modality=False, prediction_length=self.model_tester.prediction_length)
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            self.assertEqual(len(attentions), self.model_tester.num_hidden_layers)
            self.assertListEqual(list(attentions[0].shape[-3:]), [self.model_tester.num_attention_heads, encoder_seq_length, encoder_seq_length])
            out_len = len(outputs)
            correct_outlen = 7
            ...
            self.assertListEqual(list(decoder_attentions[0].shape[-3:]), [self.model_tester.num_attention_heads, decoder_seq_length, decoder_seq_length])
        ...
        self.assertListEqual(list(self_attentions[0].shape[-3:]), [self.model_tester.num_attention_heads, encoder_seq_length, encoder_seq_length])
    ...
    @parameterized.expand([(1, 5, [1]), (1, 5, [1, 10, 15]), (1, 5, [3, 6, 9, 10]), (2, 5, [1, 2, 7]), (2, 5, [2, 3, 4, 6]), (4, 5, [1, 5, 9, 11]), (4, 5, [7, 8, 13, 14])])
    def test_create_network_inputs(self, prediction_length, context_length, lags_sequence):
        ...
        config = TimeSeriesTransformerConfig(prediction_length=prediction_length, context_length=context_length, lags_sequence=lags_sequence, scaling=False, num_parallel_samples=10, num_static_categorical_features=1, cardinality=[1], embedding_dimension=[2], num_static_real_features=1)
        model = TimeSeriesTransformerModel(config)
        batch = {'static_categorical_features': torch.tensor([[0]], dtype=torch.int64), 'static_real_features': torch.tensor([[0.0]], dtype=torch.float32), 'past_time_features': torch.arange(history_length, dtype=torch.float32).view(1, history_length, 1), 'past_values': torch.arange(history_length, dtype=torch.float32).view(1, history_length), 'past_observed_mask': torch.arange(history_length, dtype=torch.float32).view(1, history_length)}
        batch['future_time_features'] = torch.arange(history_length, history_length + 1, dtype=torch.float32).view(1, 1, 1)
        ...
        batch['future_time_features'] = torch.arange(history_length, history_length + prediction_length, dtype=torch.float32).view(1, prediction_length, 1)
        batch['future_values'] = torch.arange(history_length, history_length + prediction_length, dtype=torch.float32).view(1, prediction_length)
        (transformer_inputs, loc, scale, _) = model.create_network_inputs(**batch)
        ...
        (transformer_inputs, loc, scale, _) = model.create_network_inputs(**batch)
        lagged_sequence = model.get_lagged_subsequences(sequence=batch['past_values'], subsequences_length=1, shift=1)
        assert transformer_inputs[0, ..., 0][-1] + 1 == lagged_sequence[0, ..., 0][-1]
        future_values = torch.arange(history_length, history_length + prediction_length, dtype=torch.float32).view(1, prediction_length)
...
class TimeSeriesTransformerModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        batch = prepare_batch()
        with torch.no_grad():
            output = model(past_values=batch['past_values'], past_time_features=batch['past_time_features'], past_observed_mask=batch['past_observed_mask'], static_categorical_features=batch['static_categorical_features'], static_real_features=batch['static_real_features'], future_values=batch['future_values'], future_time_features=batch['future_time_features']).last_hidden_state
        expected_shape = torch.Size((64, model.config.context_length, model.config.d_model))
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[0.8196, -1.5131, 1.462], [1.1268, -1.3238, 1.5997], [1.5098, -1.0715, 1.7359]], device=torch_device)
    ...
    def test_inference_head(self):
        ...
        with torch.no_grad():
            output = model(past_values=batch['past_values'], past_time_features=batch['past_time_features'], past_observed_mask=batch['past_observed_mask'], static_categorical_features=batch['static_categorical_features'], static_real_features=batch['static_real_features'], future_time_features=batch['future_time_features']).encoder_last_hidden_state
        expected_shape = torch.Size((64, model.config.context_length, model.config.d_model))
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[-1.2957, -1.028, -0.6045], [-0.7017, -0.8193, -0.3717], [-1.0449, -0.8149, 0.1405]], device=torch_device)
        self.assertTrue(torch.allclose(output[0, :3, :3], expected_slice, atol=TOLERANCE))
    ...
    def test_seq_to_seq_generation(self):
        ...
        batch = prepare_batch('val-batch.pt')
        with torch.no_grad():
            outputs = model.generate(static_categorical_features=batch['static_categorical_features'], static_real_features=batch['static_real_features'], past_time_features=batch['past_time_features'], past_values=batch['past_values'], future_time_features=batch['future_time_features'], past_observed_mask=batch['past_observed_mask'])
