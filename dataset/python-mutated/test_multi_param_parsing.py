...
expected_response = {'i': 1, 's': 'a-str', 'data': {'foo4': 44, 'bar4': '44bar'}, 'nested-data': {'foo': 11, 'bar': '11bar', 'd2': {'foo2': 22, 'bar2': '22bar', 'd3': {'foo3': 33, 'bar3': '33bar'}}}}
test_client_args = {'/test-multi-query': {'query': 'i=1&s=a-str&foo4=44&bar4=44bar&foo=11&bar=11bar&foo2=22&bar2=22bar&foo3=33&bar3=33bar'}, '/test-multi-path/{i}/{s}/{foo4}/{bar4}/{foo}/{bar}/{foo2}/{bar2}/{foo3}/{bar3}/': {'path': '/test-multi-path/1/a-str/44/44bar/11/11bar/22/22bar/33/33bar/'}, '/test-multi-header': {'headers': {'i': 1, 'foo': 11, 'bar2': '22bar', 'foo3': 33}}, '/test-multi-cookie': {'COOKIES': {'i': 1, 'foo': 11, 'bar2': '22bar', 'foo3': 33}}, '/test-multi-form': {'POST': {'i': 1, 'foo': 11, 'bar2': '22bar', 'foo3': 33}}, '/test-multi-body': {'json': expected_response}, '/test-multi-body-file': {'FILES': {'file': test_file}, 'POST': {k: json.dumps(v) if isinstance(v, dict) else str(v) for (k, v) in expected_response.items()}}, '/test-multi-form-file': {'FILES': {'file': test_file}, 'POST': {'i': 1, 'foo': 11, 'bar2': '22bar', 'foo3': 33}}, '/test-multi-body-form': {'POST': {'i': '1', 'foo': 11, 'bar2': '22bar', 'foo3': 33, 'data': '{"foo4": 44, "bar4": "44bar"}'}}, '/test-multi-form-body': {'POST': {'i': 1, 'nested-data': '{"foo": 11, "d2": {"bar2": "22bar", "d3": {"foo3": 33}}}'}}, '/test-multi-body-form-file': {'FILES': {'file': test_file}, 'POST': {'i': '1', 'foo': 11, 'bar2': '22bar', 'foo3': 33, 'data': '{"foo4": 44, "bar4": "44bar"}'}}, '/test-multi-form-body-file': {'FILES': {'file': test_file}, 'POST': {'i': 1, 'nested-data': '{"foo": 11, "d2": {"bar2": "22bar", "d3": {"foo3": 33}}}'}}}
