...
class iam_credentialsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'generate_access_token': ('name', 'scope', 'delegates', 'lifetime'), 'generate_id_token': ('name', 'audience', 'delegates', 'include_email'), 'sign_blob': ('name', 'payload', 'delegates'), 'sign_jwt': ('name', 'payload', 'delegates')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=iam_credentialsCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
