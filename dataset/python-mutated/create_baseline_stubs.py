...
async def get_upstream_repo_url(project: str) -> str | None:
    async with aiohttp.ClientSession() as session:
        ...
        urls_to_check.extend((url for (url_name, url) in project_urls.items() if url_name not in url_names_probably_pointing_to_source))
        for url in urls_to_check:
            url = re.sub('^(https?://)?(www\\.)?', 'https://', url)
