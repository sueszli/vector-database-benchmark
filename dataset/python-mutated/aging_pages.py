...
class AgingPagesReportFilterSet(WagtailFilterSet):
    last_published_at = django_filters.DateTimeFilter(label=_('Last published before'), lookup_expr='lte', widget=AdminDateInput)
    content_type = ContentTypeFilter(label=_('Type'), queryset=lambda request: get_content_types_for_filter())
...
class AgingPagesView(PageReportView):
    ...
    export_headings = {'status_string': _('Status'), 'last_published_at': _('Last published at'), 'last_published_by_user': _('Last published by'), 'content_type': _('Type')}
    list_export = ['title', 'status_string', 'last_published_at', 'last_published_by_user', 'content_type']
