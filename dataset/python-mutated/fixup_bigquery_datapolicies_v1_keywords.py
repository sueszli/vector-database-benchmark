...
class bigquery_datapoliciesCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_data_policy': ('parent', 'data_policy'), 'delete_data_policy': ('name',), 'get_data_policy': ('name',), 'get_iam_policy': ('resource', 'options'), 'list_data_policies': ('parent', 'page_size', 'page_token', 'filter'), 'rename_data_policy': ('name', 'new_data_policy_id'), 'set_iam_policy': ('resource', 'policy', 'update_mask'), 'test_iam_permissions': ('resource', 'permissions'), 'update_data_policy': ('data_policy', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=bigquery_datapoliciesCallTransformer()):
    if False:
        print('Hello World!')
