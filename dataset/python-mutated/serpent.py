...
VERSION = '2020.2.1'
valid_commands = ['setup', 'update', 'modules', 'grab_frames', 'launch', 'play', 'record', 'generate', 'activate', 'deactivate', 'plugins', 'train', 'capture', 'visual_debugger', 'window_name', 'record_inputs', 'dashboard', 'object_recognition']
...
command_function_mapping = {'setup': setup, 'update': update, 'modules': modules, 'grab_frames': grab_frames, 'activate': activate, 'deactivate': deactivate, 'plugins': plugins, 'launch': launch, 'play': play, 'record': record, 'generate': generate, 'train': train, 'capture': capture, 'visual_debugger': visual_debugger, 'window_name': window_name, 'record_inputs': record_inputs, 'dashboard': dashboard, 'object_recognition': object_recognition}
command_description_mapping = {'setup': 'Perform first time setup for the framework', 'update': 'Update the framework to the latest version', 'modules': "List the install status of the framework's optional modules", 'grab_frames': 'Start the frame grabber', 'activate': 'Activate a plugin', 'deactivate': 'Deactivate a plugin', 'plugins': 'List all locally-available plugins', 'launch': 'Launch a game through a plugin', 'play': 'Play a game with a game agent through plugins', 'record': 'Record player input from a game', 'generate': 'Generate code for game and game agent plugins', 'train': 'Train a context classifier with collected context frames', 'capture': 'Capture frames, screen regions and contexts from a game', 'visual_debugger': 'Launch the visual debugger', 'window_name': "Launch a utility to find a game's window name", 'record_inputs': 'Start the input recorder', 'dashboard': 'Launch the dashboard', 'object_recognition': 'Perform object recognition on collected frames'}
