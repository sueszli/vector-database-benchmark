...
async def test_playback(tdata, mode, concurrency):
    ...
    with taddons.context(cp, ps, tls) as tctx:
        ...
        if mode == 'https':
            ...
            server_args['ssl'].load_cert_chain(certfile=tdata.path('mitmproxy/net/data/verificationcerts/trusted-leaf.crt'), keyfile=tdata.path('mitmproxy/net/data/verificationcerts/trusted-leaf.key'))
            tctx.configure(tls, ssl_verify_upstream_trusted_ca=tdata.path('mitmproxy/net/data/verificationcerts/trusted-root.crt'))
