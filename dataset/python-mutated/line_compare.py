...
for p in (p1, p2, p3):
    ...
    x = np.sin(t) * 10
    y = np.cos(t) * 10
    l1 = p.line(x, y * ys, color='#2222aa', line_width=6, line_cap='butt', line_join='round', line_dash=(10, 6, 3, 6, 3, 6))
    t = np.linspace(0, 4 * np.pi, 150)
    x = t - 5
    y = np.sin(t) + 5
    l2 = p.line(x, y * ys, color='#22aa22', line_width=6, line_cap='butt', line_join='round')
    t = np.arange(10)
    x = t - 5
    y = 0.3 * (t + 1) * (t % 2 * 2 - 1) - 6
    l3 = p.line(x, y * ys, color='#aa2222', line_width=6, line_cap='butt', line_join='round', line_dash=(10, 10))
    l4 = p.line(y, x * ys, color='#aa2222', line_width=6, line_cap='butt', line_join='round', line_dash=(10, 10))
...
def make_dropdown(prop, menu):
    ...
    return dropdown
sliders = [make_slider('line_width', start=0.2, end=16, value=5), make_slider('line_dash_offset', start=0, end=100, value=1), make_dropdown('line_cap', [('butt', 'butt'), ('round', 'round'), ('square', 'square')]), make_dropdown('line_join', [('miter', 'miter'), ('round', 'round'), ('bevel', 'bevel')])]
