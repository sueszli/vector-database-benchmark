...
class ConvertAsmFile(CommonUtils):
    ...
    def ConvertAsmFile(self, inputFile, outputFile=None):
        ...
        if self.outputFilename == '-' and (not self.diff):
            ...
            f.close()
            self.output.close()
    endOfLineRe = re.compile('\n                                 \\s* ( ; .* )? \\n $\n                             ', re.VERBOSE | re.MULTILINE)
    begOfLineRe = re.compile('\n                                 \\s*\n                             ', re.VERBOSE)
    ...
    def Convert(self, lines):
        ...
        for line in lines:
            ...
            if line.strip() != '':
                self.oldAsmEmptyLineCount = 0
    procDeclRe = re.compile('\n                                (?: ASM_PFX \\s* [(] \\s* )?\n                                ([\\w@][\\w@0-9]*) \\s*\n                                [)]? \\s+\n                                PROC\n                                (?: \\s+ NEAR | FAR )?\n                                (?: \\s+ C )?\n                                (?: \\s+ (PUBLIC | PRIVATE) )?\n                                (?: \\s+ USES ( (?: \\s+ \\w[\\w0-9]* )+ ) )?\n                                \\s* $\n                            ', re.VERBOSE | re.IGNORECASE)
    procEndRe = re.compile('\n                               ([\\w@][\\w@0-9]*) \\s+\n                               ENDP\n                               \\s* $\n                           ', re.VERBOSE | re.IGNORECASE)
    varAndTypeSubRe = ' (?: [\\w@][\\w@0-9]* ) (?: \\s* : \\s* \\w+ )? '
    publicRe = re.compile('\n                              PUBLIC \\s+\n                              ( %s (?: \\s* , \\s* %s )* )\n                              \\s* $\n                          ' % (varAndTypeSubRe, varAndTypeSubRe), re.VERBOSE | re.IGNORECASE)
    varAndTypeSubRe = re.compile(varAndTypeSubRe, re.VERBOSE | re.IGNORECASE)
    macroDeclRe = re.compile('\n                                 ([\\w@][\\w@0-9]*) \\s+\n                                 MACRO\n                                 \\s* $\n                             ', re.VERBOSE | re.IGNORECASE)
    sectionDeclRe = re.compile('\n                                   ([\\w@][\\w@0-9]*) \\s+\n                                   ( SECTION | ENDS )\n                                   \\s* $\n                               ', re.VERBOSE | re.IGNORECASE)
    externRe = re.compile('\n                              EXTE?RN \\s+ (?: C \\s+ )?\n                              ([\\w@][\\w@0-9]*) \\s* : \\s* (\\w+)\n                              \\s* $\n                           ', re.VERBOSE | re.IGNORECASE)
    externdefRe = re.compile('\n                                 EXTERNDEF \\s+ (?: C \\s+ )?\n                                 ([\\w@][\\w@0-9]*) \\s* : \\s* (\\w+)\n                                 \\s* $\n                             ', re.VERBOSE | re.IGNORECASE)
    protoRe = re.compile('\n                             ([\\w@][\\w@0-9]*) \\s+\n                             PROTO\n                             (?: \\s+ .* )?\n                             \\s* $\n                         ', re.VERBOSE | re.IGNORECASE)
    defineDataRe = re.compile('\n                                  ([\\w@][\\w@0-9]*) \\s+\n                                  ( db | dw | dd | dq ) \\s+\n                                  ( .*? )\n                                  \\s* $\n                              ', re.VERBOSE | re.IGNORECASE)
    equRe = re.compile('\n                           ([\\w@][\\w@0-9]*) \\s+ EQU \\s+ (\\S.*?)\n                           \\s* $\n                       ', re.VERBOSE | re.IGNORECASE)
    ignoreRe = re.compile('\n                              \\. (?: const |\n                                     mmx |\n                                     model |\n                                     xmm |\n                                     x?list |\n                                     [3-6]86p?\n                                 ) |\n                              page\n                              (?: \\s+ .* )?\n                              \\s* $\n                          ', re.VERBOSE | re.IGNORECASE)
    whitespaceRe = re.compile('\\s+', re.MULTILINE)
    ...
    def EmitAsmReplaceOp(self, oldAsm, oldOp, newOp, endOfLine):
        ...
        self.EmitAsmWithComment(oldAsm, newAsm, endOfLine)
    hexNumRe = re.compile('0*((?=[\\da-f])\\d*(?<=\\d)[\\da-f]*)h', re.IGNORECASE)
    ...
    def ConvertLabelByte(self, oldAsm):
        ...
        return newAsm
    unaryBitwiseOpRe = re.compile('\n                                      ( NOT )\n                                      (?= \\s+ \\S )\n                                  ', re.VERBOSE | re.IGNORECASE)
    binaryBitwiseOpRe = re.compile('\n                                       ( \\S \\s+ )\n                                       ( AND | OR | SHL | SHR )\n                                       (?= \\s+ \\S )\n                                   ', re.VERBOSE | re.IGNORECASE)
    bitwiseOpReplacements = {'not': '~', 'and': '&', 'shl': '<<', 'shr': '>>', 'or': '|'}
    ...
    def FwordUnsupportedCheck(self, oldAsm):
        ...
        return newAsm
    __common_conversion_routines__ = (ConvertAnonymousLabels, ConvertPtr, FwordUnsupportedCheck, ConvertBitwiseOp, ConvertLabelByte, ConvertSection)
