...
from transformers.testing_utils import require_accelerate, require_torch, require_torch_accelerator, require_torch_fp16, slow, torch_device
...
class PvtModelTester:
    ...
    def get_config(self):
        if False:
            while True:
                i = 10
        return PvtConfig(image_size=self.image_size, num_channels=self.num_channels, num_encoder_blocks=self.num_encoder_blocks, depths=self.depths, hidden_sizes=self.hidden_sizes, num_attention_heads=self.num_attention_heads, hidden_act=self.hidden_act, hidden_dropout_prob=self.hidden_dropout_prob, attention_probs_dropout_prob=self.attention_probs_dropout_prob, initializer_range=self.initializer_range)
...
class PvtModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': PvtModel, 'image-classification': PvtForImageClassification} if is_torch_available() else {}
    ...
    def test_hidden_states_output(self):
        ...
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            self.assertListEqual(list(hidden_states[0].shape[-3:]), [self.model_tester.batch_size, (self.model_tester.image_size // 4) ** 2, self.model_tester.image_size // 4])
        (config, inputs_dict) = self.model_tester.prepare_config_and_inputs_for_common()
        for model_class in self.all_model_classes:
...
class PvtModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_model(self):
        ...
        expected_slice = torch.tensor([[-0.3086, 1.0402, 1.1816], [-0.288, 0.5781, 0.6124], [0.148, 0.6129, -0.059]]).to(torch_device)
