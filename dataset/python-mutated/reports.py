...
class ConnectorReport(Report):
    ...
    async def to_html(self) -> str:
        env = Environment(loader=PackageLoader('pipelines.airbyte_ci.connectors.test.steps'), autoescape=select_autoescape(), trim_blocks=False, lstrip_blocks=True)
        ...
        template_context = {'connector_name': self.pipeline_context.connector.technical_name, 'step_results': self.steps_results, 'run_duration': self.run_duration, 'created_at': self.created_at.isoformat(), 'connector_version': self.pipeline_context.connector.version, 'gha_workflow_run_url': None, 'dagger_logs_url': None, 'git_branch': self.pipeline_context.git_branch, 'git_revision': self.pipeline_context.git_revision, 'commit_url': None, 'icon_url': local_icon_path.as_uri()}
        if self.pipeline_context.is_ci:
            ...
            template_context['icon_url'] = f'https://raw.githubusercontent.com/airbytehq/airbyte/{self.pipeline_context.git_revision}/{self.pipeline_context.connector.code_directory}/icon.svg'
