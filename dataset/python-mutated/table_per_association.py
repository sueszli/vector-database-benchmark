...
class HasAddresses:
    ...
    def addresses(cls):
        if False:
            i = 10
            return i + 15
        address_association = Table('%s_addresses' % cls.__tablename__, cls.metadata, Column('address_id', ForeignKey('address.id'), primary_key=True), Column('%s_id' % cls.__tablename__, ForeignKey('%s.id' % cls.__tablename__), primary_key=True))
...
session.add_all([Customer(name='customer 1', addresses=[Address(street='123 anywhere street', city='New York', zip='10110'), Address(street='40 main street', city='San Francisco', zip='95732')]), Supplier(company_name='Ace Hammers', addresses=[Address(street='2569 west elm', city='Detroit', zip='56785')])])
session.commit()
