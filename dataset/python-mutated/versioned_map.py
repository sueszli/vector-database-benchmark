...
class ConfigData(Base):
    ...
    elements = relationship('ConfigValueAssociation', collection_class=attribute_keyed_dict('name'), backref=backref('config_data'), lazy='subquery')
...
class ConfigValue(Base):
    ...
    originating_config_id = Column(Integer, ForeignKey('config.id'), nullable=False)
...
if __name__ == '__main__':
    ...
    sess = Session()
    config = ConfigData({'user_name': 'twitter', 'hash_id': '4fedffca37eaf', 'x': 27, 'y': 450})
    sess.add(config)
    sess.commit()
    ...
    assert config.data == {'user_name': 'yahoo', 'hash_id': '4fedffca37eaf', 'x': 27, 'y': 450}
    old_config = sess.query(ConfigData).get(version_one)
    assert old_config.data == {'user_name': 'twitter', 'hash_id': '4fedffca37eaf', 'x': 27, 'y': 450}
    history = sess.query(ConfigValue).filter(ConfigValue.name == 'user_name').order_by(ConfigValue.originating_config_id).all()
    assert [(h.value, h.originating_config_id) for h in history] == [('twitter', version_one), ('yahoo', version_two)]