...
from azure.appconfiguration import ResourceReadOnlyError, ConfigurationSetting, ConfigurationSettingsFilter, SecretReferenceConfigurationSetting, FeatureFlagConfigurationSetting, FILTER_PERCENTAGE, FILTER_TARGETING, FILTER_TIME_WINDOW
from azure.appconfiguration.aio import AzureAppConfigurationClient
from asynctestcase import AsyncAppConfigTestCase
from consts import KEY, LABEL, TEST_VALUE, TEST_CONTENT_TYPE, LABEL_RESERVED_CHARS, PAGE_SIZE, KEY_UUID
...
class TestAppConfigurationClientAsync(AsyncAppConfigTestCase):
    ...
    async def test_add_configuration_setting(self, appconfiguration_connection_string):
        async with self.create_client(appconfiguration_connection_string) as client:
            ...
            with pytest.raises(ResourceExistsError):
                await client.add_configuration_setting(ConfigurationSetting(key=test_config_setting.key, label=test_config_setting.label))
    ...
    async def test_set_configuration_setting(self, appconfiguration_connection_string):
        async with self.create_client(appconfiguration_connection_string) as client:
            ...
            assert to_set_kv.key == set_kv.key and to_set_kv.label == to_set_kv.label and (to_set_kv.value == set_kv.value) and (to_set_kv.content_type == set_kv.content_type) and (to_set_kv.tags == set_kv.tags) and (to_set_kv.etag != set_kv.etag)
    ...
    async def test_get_configuration_setting_with_etag(self, appconfiguration_connection_string):
        async with self.create_client(appconfiguration_connection_string) as client:
            ...
            with pytest.raises(ResourceModifiedError):
                await client.get_configuration_setting(compare_kv.key, compare_kv.label, etag='wrong etag', match_condition=MatchConditions.IfNotModified)
            with pytest.raises(ResourceNotFoundError):
    ...
    async def test_delete_configuration_setting_with_etag(self, appconfiguration_connection_string):
        async with self.create_client(appconfiguration_connection_string) as client:
            ...
            with pytest.raises(ResourceModifiedError):
                await client.delete_configuration_setting(to_delete_kv.key, etag='wrong etag', match_condition=MatchConditions.IfNotModified)
    ...
    async def test_list_configuration_settings_fields(self, appconfiguration_connection_string):
        await self.set_up(appconfiguration_connection_string)
        items = await self.convert_to_list(self.client.list_configuration_settings(key_filter='*', label_filter=LABEL, fields=['key', 'content_type']))
        assert len(items) == 1
    ...
    async def test_list_configuration_settings_correct_etag(self, appconfiguration_connection_string):
        async with self.create_client(appconfiguration_connection_string) as client:
            ...
            items = await self.convert_to_list(client.list_configuration_settings(key_filter=to_list_kv.key, label_filter=to_list_kv.label, headers=custom_headers))
            assert len(items) == 1
    ...
    async def test_list_configuration_settings_multi_pages(self, appconfiguration_connection_string):
        async with self.create_client(appconfiguration_connection_string) as client:
            try:
                [await client.add_configuration_setting(ConfigurationSetting(key='multi_' + str(i) + KEY_UUID, label='multi_label_' + str(i), value='multi value')) for i in range(PAGE_SIZE + 1)]
            except ResourceExistsError:
                pass
            items = await self.convert_to_list(client.list_configuration_settings(key_filter='multi_*'))
            assert len(items) > PAGE_SIZE
            try:
                [await client.delete_configuration_setting(key='multi_' + str(i) + KEY_UUID, label='multi_label_' + str(i)) for i in range(PAGE_SIZE + 1)]
            except AzureError:
                pass
    ...
    async def test_list_configuration_settings_no_label(self, appconfiguration_connection_string):
        await self.set_up(appconfiguration_connection_string)
        items = await self.convert_to_list(self.client.list_configuration_settings(label_filter='\x00'))
        assert len(items) > 0
        await self.tear_down()
    ...
    async def test_list_configuration_settings_only_accepttime(self, appconfiguration_connection_string, **kwargs):
        ...
        async with self.create_client(appconfiguration_connection_string) as client:
            ...
            revision = await self.convert_to_list(client.list_configuration_settings(accept_datetime=recorded_variables.get('timestamp')))
    ...
    async def test_list_revisions_key_label(self, appconfiguration_connection_string):
        ...
        items = await self.convert_to_list(self.client.list_revisions(label_filter=to_list1.label, key_filter=to_list1.key))
        assert len(items) >= 2
    ...
    async def test_list_revisions_fields(self, appconfiguration_connection_string):
        await self.set_up(appconfiguration_connection_string)
        items = await self.convert_to_list(self.client.list_revisions(key_filter='*', label_filter=LABEL, fields=['key', 'content_type']))
    ...
    async def test_list_revisions_correct_etag(self, appconfiguration_connection_string):
        async with self.create_client(appconfiguration_connection_string) as client:
            ...
            to_list_kv = await client.get_configuration_setting(to_list_kv.key, to_list_kv.label)
            custom_headers = {'If-Match': to_list_kv.etag}
            items = await self.convert_to_list(client.list_revisions(key_filter=to_list_kv.key, label_filter=to_list_kv.label, headers=custom_headers))
            assert len(items) >= 1
            assert all((x.key == to_list_kv.key and x.label == to_list_kv.label for x in items))
            await client.delete_configuration_setting(to_list_kv.key)
    ...
    async def test_update_snapshot_status_with_etag(self, appconfiguration_connection_string):
        ...
        created_snapshot = await response.result()
        with pytest.raises(ResourceModifiedError):
            await self.client.archive_snapshot(name=snapshot_name, etag='wrong etag', match_condition=MatchConditions.IfNotModified)
