...
for file_listing_line in file_listing.stdout:
    ...
    locale = re.search('locale/(\\w+)/LC_MESSAGES', str(filename)).group(1)
    if locale == 'en':
        continue
...
LANGUAGE_OVERRIDES = {'tet': 'Tetum', 'ht': 'Haitian'}
...
language_names = [(get_language_name(locale_string), locale_string) for locale_string in authors_by_locale.keys()]
language_names.sort()
