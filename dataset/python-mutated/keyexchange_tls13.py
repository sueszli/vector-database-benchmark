...
from scapy.fields import FieldLenField, IntField, PacketField, PacketLenField, PacketListField, ShortEnumField, ShortField, StrFixedLenField, StrLenField, XStrLenField
from scapy.packet import Packet
from scapy.layers.tls.extensions import TLS_Ext_Unknown, _tls_ext
from scapy.layers.tls.crypto.groups import _tls_named_curves, _tls_named_ffdh_groups, _tls_named_groups, _tls_named_groups_generate, _tls_named_groups_import, _tls_named_groups_pubbytes
...
class TLS_Ext_KeyShare_CH(TLS_Ext_Unknown):
    ...
    fields_desc = [ShortEnumField('type', 51, _tls_ext), ShortField('len', None), FieldLenField('client_shares_len', None, length_of='client_shares'), PacketListField('client_shares', [], KeyShareEntry, length_from=lambda pkt: pkt.client_shares_len)]
...
class TLS_Ext_KeyShare_HRR(TLS_Ext_Unknown):
    ...
    fields_desc = [ShortEnumField('type', 51, _tls_ext), ShortField('len', None), ShortEnumField('selected_group', None, _tls_named_groups)]

class TLS_Ext_KeyShare_SH(TLS_Ext_Unknown):
    name = 'TLS Extension - Key Share (for ServerHello)'
    fields_desc = [ShortEnumField('type', 51, _tls_ext), ShortField('len', None), PacketField('server_share', None, KeyShareEntry)]
...
class Ticket(Packet):
    ...
    fields_desc = [StrFixedLenField('key_name', None, 16), StrFixedLenField('iv', None, 16), FieldLenField('encstatelen', None, length_of='encstate'), StrLenField('encstate', '', length_from=lambda pkt: pkt.encstatelen), StrFixedLenField('mac', None, 32)]
...
class PSKIdentity(Packet):
    name = 'PSK Identity'
    fields_desc = [FieldLenField('identity_len', None, length_of='identity'), TicketField('identity', '', Ticket, length_from=lambda pkt: pkt.identity_len), IntField('obfuscated_ticket_age', 0)]
...
class TLS_Ext_PreSharedKey_CH(TLS_Ext_Unknown):
    ...
    fields_desc = [ShortEnumField('type', 41, _tls_ext), ShortField('len', None), FieldLenField('identities_len', None, length_of='identities'), PacketListField('identities', [], PSKIdentity, length_from=lambda pkt: pkt.identities_len), FieldLenField('binders_len', None, length_of='binders'), PacketListField('binders', [], PSKBinderEntry, length_from=lambda pkt: pkt.binders_len)]

class TLS_Ext_PreSharedKey_SH(TLS_Ext_Unknown):
    name = 'TLS Extension - Pre Shared Key (for ServerHello)'
    fields_desc = [ShortEnumField('type', 41, _tls_ext), ShortField('len', None), ShortField('selected_identity', None)]
