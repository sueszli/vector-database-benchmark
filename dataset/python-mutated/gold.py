...
NON_REVENUE_STATUSES = ('declined', 'chargeback', 'fudge', 'invalid', 'refunded', 'reversed')
...
gold_table = sa.Table('reddit_gold', METADATA, sa.Column('trans_id', sa.String, nullable=False, primary_key=True), sa.Column('status', sa.String, nullable=False), sa.Column('date', sa.DateTime(timezone=True), nullable=False, default=sa.func.now()), sa.Column('payer_email', sa.String, nullable=False), sa.Column('paying_id', sa.String, nullable=False), sa.Column('pennies', sa.Integer, nullable=False), sa.Column('secret', sa.String, nullable=True), sa.Column('account_id', sa.String, nullable=True), sa.Column('days', sa.Integer, nullable=True), sa.Column('subscr_id', sa.String, nullable=True), sa.Column('gilding_type', sa.String, nullable=True))
indices = [index_str(gold_table, 'status', 'status'), index_str(gold_table, 'date', 'date'), index_str(gold_table, 'account_id', 'account_id'), index_str(gold_table, 'secret', 'secret'), index_str(gold_table, 'payer_email', 'payer_email'), index_str(gold_table, 'subscr_id', 'subscr_id')]
...
class GoldRevenueGoalByDate(object):
    ...
    _extra_schema_creation_args = {'column_name_class': UTF8_TYPE, 'default_validation_class': INT_TYPE}
...
class GildingsByThing(tdb_cassandra.View):
    _use_db = True
    _extra_schema_creation_args = {'key_validation_class': UTF8_TYPE, 'column_name_class': UTF8_TYPE}
...
class GildingsByDay(tdb_cassandra.View):
    ...
    _extra_schema_creation_args = {'key_validation_class': ASCII_TYPE, 'column_name_class': TIME_UUID_TYPE, 'default_validation_class': UTF8_TYPE}
...
PENNIES_PER_SERVER_SECOND = {datetime.strptime(datestr, '%Y/%m/%d').date(): v for (datestr, v) in g.live_config['pennies_per_server_second'].iteritems()}
