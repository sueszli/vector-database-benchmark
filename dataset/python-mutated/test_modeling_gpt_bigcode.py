...
if is_torch_available():
    import torch
    from transformers import GPT2TokenizerFast, GPTBigCodeForCausalLM, GPTBigCodeForSequenceClassification, GPTBigCodeForTokenClassification, GPTBigCodeModel
...
class GPTBigCodeModelTester:
    ...
    def create_and_check_gpt_bigcode_model_attention_mask_past(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        if False:
            while True:
                i = 10
        model = GPTBigCodeModel(config=config)
    ...
    def create_and_check_gpt_bigcode_model_past_large_inputs(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        if False:
            print('Hello World!')
        model = GPTBigCodeModel(config=config)
        model.to(torch_device)
    ...
    def create_and_check_gpt_bigcode_for_sequence_classification(self, config, input_ids, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, *args):
        if False:
            i = 10
            return i + 15
    ...
    def create_and_check_gpt_bigcode_for_token_classification(self, config, input_ids, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, *args):
        if False:
            return 10
        config.num_labels = self.num_labels
...
class GPTBigCodeModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (GPTBigCodeModel, GPTBigCodeForCausalLM, GPTBigCodeForSequenceClassification, GPTBigCodeForTokenClassification) if is_torch_available() else ()
    all_generative_model_classes = (GPTBigCodeForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': GPTBigCodeModel, 'text-classification': GPTBigCodeForSequenceClassification, 'text-generation': GPTBigCodeForCausalLM, 'token-classification': GPTBigCodeForTokenClassification, 'zero-shot': GPTBigCodeForSequenceClassification} if is_torch_available() else {}
    fx_compatible = False
...
@unittest.skipIf(not is_torch_greater_or_equal_than_1_12, reason='`GPTBigCode` checkpoints use `PytorchGELUTanh` which requires `torch>=1.12.0`.')
@slow
@require_torch
...
class GPTBigCodeMQATest(unittest.TestCase):
    ...
    def get_attention(self, multi_query):
        if False:
            i = 10
            return i + 15
        config = GPTBigCodeConfig.from_pretrained('bigcode/gpt_bigcode-santacoder', multi_query=multi_query, attn_pdrop=0, resid_pdrop=0)
    ...
    def test_mqa_reduces_to_mha(self, seed, is_train_mode=True):
        ...
        with torch.no_grad():
            ...
            mha_c_weight = torch.cat([mqa_q_weight, mqa_kv_weight.expand(num_heads, 2, head_dim, embed_dim)], dim=1).view(3 * num_heads * head_dim, embed_dim)
            ...
            mha_c_bias = torch.cat([mqa_q_bias, mqa_kv_bias.expand(num_heads, 2, head_dim)], dim=1).view(3 * num_heads * head_dim)
        ...
        self.assertTrue(torch.allclose(attention_mha_result, attention_mqa_result, atol=1e-05))