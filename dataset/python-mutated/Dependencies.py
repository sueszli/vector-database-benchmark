...
bool_or = object()
distutils_settings = {'name': str, 'sources': list, 'define_macros': list, 'undef_macros': list, 'libraries': transitive_list, 'library_dirs': transitive_list, 'runtime_library_dirs': transitive_list, 'include_dirs': transitive_list, 'extra_objects': list, 'extra_compile_args': transitive_list, 'extra_link_args': transitive_list, 'export_symbols': list, 'depends': transitive_list, 'language': transitive_str, 'np_pythran': bool_or}
...
dependency_regex = re.compile('(?:^\\s*from +([0-9a-zA-Z_.]+) +cimport)|(?:^\\s*cimport +([0-9a-zA-Z_.]+(?: *, *[0-9a-zA-Z_.]+)*))|(?:^\\s*cdef +extern +from +[\'\\"]([^\'\\"]+)[\'\\"])|(?:^\\s*include +[\'\\"]([^\'\\"]+)[\'\\"])', re.M)
dependency_after_from_regex = re.compile('(?:^\\s+\\(([0-9a-zA-Z_., ]*)\\)[#\\n])|(?:^\\s+([0-9a-zA-Z_., ]*)[#\\n])', re.M)
