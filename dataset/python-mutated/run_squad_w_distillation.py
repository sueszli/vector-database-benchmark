...
def evaluate(args, model, tokenizer, prefix=''):
    ...
    if args.model_type in ['xlnet', 'xlm']:
        predictions = compute_predictions_log_probs(examples, features, all_results, args.n_best_size, args.max_answer_length, output_prediction_file, output_nbest_file, output_null_log_odds_file, model.config.start_n_top, model.config.end_n_top, args.version_2_with_negative, tokenizer, args.verbose_logging)
    else:
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    cached_features_file = os.path.join(os.path.dirname(input_file), 'cached_distillation_{}_{}_{}'.format('dev' if evaluate else 'train', list(filter(None, args.model_name_or_path.split('/'))).pop(), str(args.max_seq_length)))
    if os.path.exists(cached_features_file) and (not args.overwrite_cache):
        ...
        try:
            ...
            raise DeprecationWarning('You seem to be loading features from an older version of this script please delete the file %s in order for it to be created again' % cached_features_file)
    else:
...
def main():
    ...
    parser.add_argument('--alpha_ce', default=0.5, type=float, help='Distillation loss linear weight. Only for distillation.')
    parser.add_argument('--alpha_squad', default=0.5, type=float, help='True SQuAD loss linear weight. Only for distillation.')
    ...
    if args.teacher_type is not None:
        ...
        teacher_config = teacher_config_class.from_pretrained(args.teacher_name_or_path, cache_dir=args.cache_dir if args.cache_dir else None)
        teacher = teacher_model_class.from_pretrained(args.teacher_name_or_path, config=teacher_config, cache_dir=args.cache_dir if args.cache_dir else None)
