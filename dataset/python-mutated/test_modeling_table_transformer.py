...
class TableTransformerModelTester:
    ...
    def __init__(self, parent, batch_size=8, is_training=True, use_labels=True, hidden_size=32, num_hidden_layers=2, num_attention_heads=8, intermediate_size=4, hidden_act='gelu', hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, num_queries=12, num_channels=3, min_size=200, max_size=200, n_targets=8, num_labels=3):
        if False:
            i = 10
            return i + 15
        self.parent = parent
...
class TableTransformerModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TableTransformerModel, TableTransformerForObjectDetection) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': TableTransformerModel, 'object-detection': TableTransformerForObjectDetection} if is_torch_available() else {}
...
class TableTransformerModelIntegrationTests(unittest.TestCase):
    ...
    def test_table_detection(self):
        ...
        self.assertEqual(outputs.logits.shape, expected_shape)
        expected_logits = torch.tensor([[-6.7329, -16.959, 6.7447], [-8.0038, -22.3071, 6.9288], [-7.2445, -20.9855, 7.3465]], device=torch_device)
        self.assertTrue(torch.allclose(outputs.logits[0, :3, :3], expected_logits, atol=0.0001))
        expected_boxes = torch.tensor([[0.4868, 0.1764, 0.6729], [0.6674, 0.4621, 0.3864], [0.472, 0.1757, 0.6362]], device=torch_device)
        self.assertTrue(torch.allclose(outputs.pred_boxes[0, :3, :3], expected_boxes, atol=0.001))