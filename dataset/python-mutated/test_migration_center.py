import os
try:
    from unittest import mock
    from unittest.mock import AsyncMock
except ImportError:
    import mock
from collections.abc import Iterable
import json
import math
from google.api_core import future, gapic_v1, grpc_helpers, grpc_helpers_async, operation, operations_v1, path_template
from google.api_core import client_options
from google.api_core import exceptions as core_exceptions
from google.api_core import operation_async
import google.auth
from google.auth import credentials as ga_credentials
from google.auth.exceptions import MutualTLSChannelError
from google.cloud.location import locations_pb2
from google.longrunning import operations_pb2
from google.oauth2 import service_account
from google.protobuf import empty_pb2
from google.protobuf import field_mask_pb2
from google.protobuf import json_format
from google.protobuf import timestamp_pb2
from google.type import date_pb2
from google.type import money_pb2
import grpc
from grpc.experimental import aio
from proto.marshal.rules import wrappers
from proto.marshal.rules.dates import DurationRule, TimestampRule
import pytest
from requests import PreparedRequest, Request, Response
from requests.sessions import Session
from google.cloud.migrationcenter_v1.services.migration_center import MigrationCenterAsyncClient, MigrationCenterClient, pagers, transports
from google.cloud.migrationcenter_v1.types import migrationcenter

def client_cert_source_callback():
    if False:
        return 10
    return (b'cert bytes', b'key bytes')

def modify_default_endpoint(client):
    if False:
        print('Hello World!')
    return 'foo.googleapis.com' if 'localhost' in client.DEFAULT_ENDPOINT else client.DEFAULT_ENDPOINT

def test__get_default_mtls_endpoint():
    if False:
        print('Hello World!')
    api_endpoint = 'example.googleapis.com'
    api_mtls_endpoint = 'example.mtls.googleapis.com'
    sandbox_endpoint = 'example.sandbox.googleapis.com'
    sandbox_mtls_endpoint = 'example.mtls.sandbox.googleapis.com'
    non_googleapi = 'api.example.com'
    assert MigrationCenterClient._get_default_mtls_endpoint(None) is None
    assert MigrationCenterClient._get_default_mtls_endpoint(api_endpoint) == api_mtls_endpoint
    assert MigrationCenterClient._get_default_mtls_endpoint(api_mtls_endpoint) == api_mtls_endpoint
    assert MigrationCenterClient._get_default_mtls_endpoint(sandbox_endpoint) == sandbox_mtls_endpoint
    assert MigrationCenterClient._get_default_mtls_endpoint(sandbox_mtls_endpoint) == sandbox_mtls_endpoint
    assert MigrationCenterClient._get_default_mtls_endpoint(non_googleapi) == non_googleapi

@pytest.mark.parametrize('client_class,transport_name', [(MigrationCenterClient, 'grpc'), (MigrationCenterAsyncClient, 'grpc_asyncio'), (MigrationCenterClient, 'rest')])
def test_migration_center_client_from_service_account_info(client_class, transport_name):
    if False:
        i = 10
        return i + 15
    creds = ga_credentials.AnonymousCredentials()
    with mock.patch.object(service_account.Credentials, 'from_service_account_info') as factory:
        factory.return_value = creds
        info = {'valid': True}
        client = client_class.from_service_account_info(info, transport=transport_name)
        assert client.transport._credentials == creds
        assert isinstance(client, client_class)
        assert client.transport._host == ('migrationcenter.googleapis.com:443' if transport_name in ['grpc', 'grpc_asyncio'] else 'https://migrationcenter.googleapis.com')

@pytest.mark.parametrize('transport_class,transport_name', [(transports.MigrationCenterGrpcTransport, 'grpc'), (transports.MigrationCenterGrpcAsyncIOTransport, 'grpc_asyncio'), (transports.MigrationCenterRestTransport, 'rest')])
def test_migration_center_client_service_account_always_use_jwt(transport_class, transport_name):
    if False:
        while True:
            i = 10
    with mock.patch.object(service_account.Credentials, 'with_always_use_jwt_access', create=True) as use_jwt:
        creds = service_account.Credentials(None, None, None)
        transport = transport_class(credentials=creds, always_use_jwt_access=True)
        use_jwt.assert_called_once_with(True)
    with mock.patch.object(service_account.Credentials, 'with_always_use_jwt_access', create=True) as use_jwt:
        creds = service_account.Credentials(None, None, None)
        transport = transport_class(credentials=creds, always_use_jwt_access=False)
        use_jwt.assert_not_called()

@pytest.mark.parametrize('client_class,transport_name', [(MigrationCenterClient, 'grpc'), (MigrationCenterAsyncClient, 'grpc_asyncio'), (MigrationCenterClient, 'rest')])
def test_migration_center_client_from_service_account_file(client_class, transport_name):
    if False:
        while True:
            i = 10
    creds = ga_credentials.AnonymousCredentials()
    with mock.patch.object(service_account.Credentials, 'from_service_account_file') as factory:
        factory.return_value = creds
        client = client_class.from_service_account_file('dummy/file/path.json', transport=transport_name)
        assert client.transport._credentials == creds
        assert isinstance(client, client_class)
        client = client_class.from_service_account_json('dummy/file/path.json', transport=transport_name)
        assert client.transport._credentials == creds
        assert isinstance(client, client_class)
        assert client.transport._host == ('migrationcenter.googleapis.com:443' if transport_name in ['grpc', 'grpc_asyncio'] else 'https://migrationcenter.googleapis.com')

def test_migration_center_client_get_transport_class():
    if False:
        while True:
            i = 10
    transport = MigrationCenterClient.get_transport_class()
    available_transports = [transports.MigrationCenterGrpcTransport, transports.MigrationCenterRestTransport]
    assert transport in available_transports
    transport = MigrationCenterClient.get_transport_class('grpc')
    assert transport == transports.MigrationCenterGrpcTransport

@pytest.mark.parametrize('client_class,transport_class,transport_name', [(MigrationCenterClient, transports.MigrationCenterGrpcTransport, 'grpc'), (MigrationCenterAsyncClient, transports.MigrationCenterGrpcAsyncIOTransport, 'grpc_asyncio'), (MigrationCenterClient, transports.MigrationCenterRestTransport, 'rest')])
@mock.patch.object(MigrationCenterClient, 'DEFAULT_ENDPOINT', modify_default_endpoint(MigrationCenterClient))
@mock.patch.object(MigrationCenterAsyncClient, 'DEFAULT_ENDPOINT', modify_default_endpoint(MigrationCenterAsyncClient))
def test_migration_center_client_client_options(client_class, transport_class, transport_name):
    if False:
        while True:
            i = 10
    with mock.patch.object(MigrationCenterClient, 'get_transport_class') as gtc:
        transport = transport_class(credentials=ga_credentials.AnonymousCredentials())
        client = client_class(transport=transport)
        gtc.assert_not_called()
    with mock.patch.object(MigrationCenterClient, 'get_transport_class') as gtc:
        client = client_class(transport=transport_name)
        gtc.assert_called()
    options = client_options.ClientOptions(api_endpoint='squid.clam.whelk')
    with mock.patch.object(transport_class, '__init__') as patched:
        patched.return_value = None
        client = client_class(transport=transport_name, client_options=options)
        patched.assert_called_once_with(credentials=None, credentials_file=None, host='squid.clam.whelk', scopes=None, client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_MTLS_ENDPOINT': 'never'}):
        with mock.patch.object(transport_class, '__init__') as patched:
            patched.return_value = None
            client = client_class(transport=transport_name)
            patched.assert_called_once_with(credentials=None, credentials_file=None, host=client.DEFAULT_ENDPOINT, scopes=None, client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_MTLS_ENDPOINT': 'always'}):
        with mock.patch.object(transport_class, '__init__') as patched:
            patched.return_value = None
            client = client_class(transport=transport_name)
            patched.assert_called_once_with(credentials=None, credentials_file=None, host=client.DEFAULT_MTLS_ENDPOINT, scopes=None, client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_MTLS_ENDPOINT': 'Unsupported'}):
        with pytest.raises(MutualTLSChannelError):
            client = client_class(transport=transport_name)
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_CLIENT_CERTIFICATE': 'Unsupported'}):
        with pytest.raises(ValueError):
            client = client_class(transport=transport_name)
    options = client_options.ClientOptions(quota_project_id='octopus')
    with mock.patch.object(transport_class, '__init__') as patched:
        patched.return_value = None
        client = client_class(client_options=options, transport=transport_name)
        patched.assert_called_once_with(credentials=None, credentials_file=None, host=client.DEFAULT_ENDPOINT, scopes=None, client_cert_source_for_mtls=None, quota_project_id='octopus', client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)
    options = client_options.ClientOptions(api_audience='https://language.googleapis.com')
    with mock.patch.object(transport_class, '__init__') as patched:
        patched.return_value = None
        client = client_class(client_options=options, transport=transport_name)
        patched.assert_called_once_with(credentials=None, credentials_file=None, host=client.DEFAULT_ENDPOINT, scopes=None, client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience='https://language.googleapis.com')

@pytest.mark.parametrize('client_class,transport_class,transport_name,use_client_cert_env', [(MigrationCenterClient, transports.MigrationCenterGrpcTransport, 'grpc', 'true'), (MigrationCenterAsyncClient, transports.MigrationCenterGrpcAsyncIOTransport, 'grpc_asyncio', 'true'), (MigrationCenterClient, transports.MigrationCenterGrpcTransport, 'grpc', 'false'), (MigrationCenterAsyncClient, transports.MigrationCenterGrpcAsyncIOTransport, 'grpc_asyncio', 'false'), (MigrationCenterClient, transports.MigrationCenterRestTransport, 'rest', 'true'), (MigrationCenterClient, transports.MigrationCenterRestTransport, 'rest', 'false')])
@mock.patch.object(MigrationCenterClient, 'DEFAULT_ENDPOINT', modify_default_endpoint(MigrationCenterClient))
@mock.patch.object(MigrationCenterAsyncClient, 'DEFAULT_ENDPOINT', modify_default_endpoint(MigrationCenterAsyncClient))
@mock.patch.dict(os.environ, {'GOOGLE_API_USE_MTLS_ENDPOINT': 'auto'})
def test_migration_center_client_mtls_env_auto(client_class, transport_class, transport_name, use_client_cert_env):
    if False:
        for i in range(10):
            print('nop')
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_CLIENT_CERTIFICATE': use_client_cert_env}):
        options = client_options.ClientOptions(client_cert_source=client_cert_source_callback)
        with mock.patch.object(transport_class, '__init__') as patched:
            patched.return_value = None
            client = client_class(client_options=options, transport=transport_name)
            if use_client_cert_env == 'false':
                expected_client_cert_source = None
                expected_host = client.DEFAULT_ENDPOINT
            else:
                expected_client_cert_source = client_cert_source_callback
                expected_host = client.DEFAULT_MTLS_ENDPOINT
            patched.assert_called_once_with(credentials=None, credentials_file=None, host=expected_host, scopes=None, client_cert_source_for_mtls=expected_client_cert_source, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_CLIENT_CERTIFICATE': use_client_cert_env}):
        with mock.patch.object(transport_class, '__init__') as patched:
            with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
                with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=client_cert_source_callback):
                    if use_client_cert_env == 'false':
                        expected_host = client.DEFAULT_ENDPOINT
                        expected_client_cert_source = None
                    else:
                        expected_host = client.DEFAULT_MTLS_ENDPOINT
                        expected_client_cert_source = client_cert_source_callback
                    patched.return_value = None
                    client = client_class(transport=transport_name)
                    patched.assert_called_once_with(credentials=None, credentials_file=None, host=expected_host, scopes=None, client_cert_source_for_mtls=expected_client_cert_source, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_CLIENT_CERTIFICATE': use_client_cert_env}):
        with mock.patch.object(transport_class, '__init__') as patched:
            with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=False):
                patched.return_value = None
                client = client_class(transport=transport_name)
                patched.assert_called_once_with(credentials=None, credentials_file=None, host=client.DEFAULT_ENDPOINT, scopes=None, client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)

@pytest.mark.parametrize('client_class', [MigrationCenterClient, MigrationCenterAsyncClient])
@mock.patch.object(MigrationCenterClient, 'DEFAULT_ENDPOINT', modify_default_endpoint(MigrationCenterClient))
@mock.patch.object(MigrationCenterAsyncClient, 'DEFAULT_ENDPOINT', modify_default_endpoint(MigrationCenterAsyncClient))
def test_migration_center_client_get_mtls_endpoint_and_cert_source(client_class):
    if False:
        print('Hello World!')
    mock_client_cert_source = mock.Mock()
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_CLIENT_CERTIFICATE': 'true'}):
        mock_api_endpoint = 'foo'
        options = client_options.ClientOptions(client_cert_source=mock_client_cert_source, api_endpoint=mock_api_endpoint)
        (api_endpoint, cert_source) = client_class.get_mtls_endpoint_and_cert_source(options)
        assert api_endpoint == mock_api_endpoint
        assert cert_source == mock_client_cert_source
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_CLIENT_CERTIFICATE': 'false'}):
        mock_client_cert_source = mock.Mock()
        mock_api_endpoint = 'foo'
        options = client_options.ClientOptions(client_cert_source=mock_client_cert_source, api_endpoint=mock_api_endpoint)
        (api_endpoint, cert_source) = client_class.get_mtls_endpoint_and_cert_source(options)
        assert api_endpoint == mock_api_endpoint
        assert cert_source is None
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_MTLS_ENDPOINT': 'never'}):
        (api_endpoint, cert_source) = client_class.get_mtls_endpoint_and_cert_source()
        assert api_endpoint == client_class.DEFAULT_ENDPOINT
        assert cert_source is None
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_MTLS_ENDPOINT': 'always'}):
        (api_endpoint, cert_source) = client_class.get_mtls_endpoint_and_cert_source()
        assert api_endpoint == client_class.DEFAULT_MTLS_ENDPOINT
        assert cert_source is None
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_CLIENT_CERTIFICATE': 'true'}):
        with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=False):
            (api_endpoint, cert_source) = client_class.get_mtls_endpoint_and_cert_source()
            assert api_endpoint == client_class.DEFAULT_ENDPOINT
            assert cert_source is None
    with mock.patch.dict(os.environ, {'GOOGLE_API_USE_CLIENT_CERTIFICATE': 'true'}):
        with mock.patch('google.auth.transport.mtls.has_default_client_cert_source', return_value=True):
            with mock.patch('google.auth.transport.mtls.default_client_cert_source', return_value=mock_client_cert_source):
                (api_endpoint, cert_source) = client_class.get_mtls_endpoint_and_cert_source()
                assert api_endpoint == client_class.DEFAULT_MTLS_ENDPOINT
                assert cert_source == mock_client_cert_source

@pytest.mark.parametrize('client_class,transport_class,transport_name', [(MigrationCenterClient, transports.MigrationCenterGrpcTransport, 'grpc'), (MigrationCenterAsyncClient, transports.MigrationCenterGrpcAsyncIOTransport, 'grpc_asyncio'), (MigrationCenterClient, transports.MigrationCenterRestTransport, 'rest')])
def test_migration_center_client_client_options_scopes(client_class, transport_class, transport_name):
    if False:
        for i in range(10):
            print('nop')
    options = client_options.ClientOptions(scopes=['1', '2'])
    with mock.patch.object(transport_class, '__init__') as patched:
        patched.return_value = None
        client = client_class(client_options=options, transport=transport_name)
        patched.assert_called_once_with(credentials=None, credentials_file=None, host=client.DEFAULT_ENDPOINT, scopes=['1', '2'], client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)

@pytest.mark.parametrize('client_class,transport_class,transport_name,grpc_helpers', [(MigrationCenterClient, transports.MigrationCenterGrpcTransport, 'grpc', grpc_helpers), (MigrationCenterAsyncClient, transports.MigrationCenterGrpcAsyncIOTransport, 'grpc_asyncio', grpc_helpers_async), (MigrationCenterClient, transports.MigrationCenterRestTransport, 'rest', None)])
def test_migration_center_client_client_options_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    if False:
        while True:
            i = 10
    options = client_options.ClientOptions(credentials_file='credentials.json')
    with mock.patch.object(transport_class, '__init__') as patched:
        patched.return_value = None
        client = client_class(client_options=options, transport=transport_name)
        patched.assert_called_once_with(credentials=None, credentials_file='credentials.json', host=client.DEFAULT_ENDPOINT, scopes=None, client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)

def test_migration_center_client_client_options_from_dict():
    if False:
        while True:
            i = 10
    with mock.patch('google.cloud.migrationcenter_v1.services.migration_center.transports.MigrationCenterGrpcTransport.__init__') as grpc_transport:
        grpc_transport.return_value = None
        client = MigrationCenterClient(client_options={'api_endpoint': 'squid.clam.whelk'})
        grpc_transport.assert_called_once_with(credentials=None, credentials_file=None, host='squid.clam.whelk', scopes=None, client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)

@pytest.mark.parametrize('client_class,transport_class,transport_name,grpc_helpers', [(MigrationCenterClient, transports.MigrationCenterGrpcTransport, 'grpc', grpc_helpers), (MigrationCenterAsyncClient, transports.MigrationCenterGrpcAsyncIOTransport, 'grpc_asyncio', grpc_helpers_async)])
def test_migration_center_client_create_channel_credentials_file(client_class, transport_class, transport_name, grpc_helpers):
    if False:
        i = 10
        return i + 15
    options = client_options.ClientOptions(credentials_file='credentials.json')
    with mock.patch.object(transport_class, '__init__') as patched:
        patched.return_value = None
        client = client_class(client_options=options, transport=transport_name)
        patched.assert_called_once_with(credentials=None, credentials_file='credentials.json', host=client.DEFAULT_ENDPOINT, scopes=None, client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch.object(grpc_helpers, 'create_channel') as create_channel:
        creds = ga_credentials.AnonymousCredentials()
        file_creds = ga_credentials.AnonymousCredentials()
        load_creds.return_value = (file_creds, None)
        adc.return_value = (creds, None)
        client = client_class(client_options=options, transport=transport_name)
        create_channel.assert_called_with('migrationcenter.googleapis.com:443', credentials=file_creds, credentials_file=None, quota_project_id=None, default_scopes=('https://www.googleapis.com/auth/cloud-platform',), scopes=None, default_host='migrationcenter.googleapis.com', ssl_credentials=None, options=[('grpc.max_send_message_length', -1), ('grpc.max_receive_message_length', -1)])

@pytest.mark.parametrize('request_type', [migrationcenter.ListAssetsRequest, dict])
def test_list_assets(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_assets), '__call__') as call:
        call.return_value = migrationcenter.ListAssetsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response = client.list_assets(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListAssetsRequest()
    assert isinstance(response, pagers.ListAssetsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_assets_empty_call():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.list_assets), '__call__') as call:
        client.list_assets()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListAssetsRequest()

@pytest.mark.asyncio
async def test_list_assets_async(transport: str='grpc_asyncio', request_type=migrationcenter.ListAssetsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_assets), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListAssetsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value']))
        response = await client.list_assets(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListAssetsRequest()
    assert isinstance(response, pagers.ListAssetsAsyncPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

@pytest.mark.asyncio
async def test_list_assets_async_from_dict():
    await test_list_assets_async(request_type=dict)

def test_list_assets_field_headers():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListAssetsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_assets), '__call__') as call:
        call.return_value = migrationcenter.ListAssetsResponse()
        client.list_assets(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_list_assets_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListAssetsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_assets), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListAssetsResponse())
        await client.list_assets(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_list_assets_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_assets), '__call__') as call:
        call.return_value = migrationcenter.ListAssetsResponse()
        client.list_assets(parent='parent_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

def test_list_assets_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.list_assets(migrationcenter.ListAssetsRequest(), parent='parent_value')

@pytest.mark.asyncio
async def test_list_assets_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_assets), '__call__') as call:
        call.return_value = migrationcenter.ListAssetsResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListAssetsResponse())
        response = await client.list_assets(parent='parent_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_list_assets_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.list_assets(migrationcenter.ListAssetsRequest(), parent='parent_value')

def test_list_assets_pager(transport_name: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_assets), '__call__') as call:
        call.side_effect = (migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset(), migrationcenter.Asset()], next_page_token='abc'), migrationcenter.ListAssetsResponse(assets=[], next_page_token='def'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset()], next_page_token='ghi'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset()]), RuntimeError)
        metadata = ()
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((('parent', ''),)),)
        pager = client.list_assets(request={})
        assert pager._metadata == metadata
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.Asset) for i in results))

def test_list_assets_pages(transport_name: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_assets), '__call__') as call:
        call.side_effect = (migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset(), migrationcenter.Asset()], next_page_token='abc'), migrationcenter.ListAssetsResponse(assets=[], next_page_token='def'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset()], next_page_token='ghi'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset()]), RuntimeError)
        pages = list(client.list_assets(request={}).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_assets_async_pager():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_assets), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset(), migrationcenter.Asset()], next_page_token='abc'), migrationcenter.ListAssetsResponse(assets=[], next_page_token='def'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset()], next_page_token='ghi'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset()]), RuntimeError)
        async_pager = await client.list_assets(request={})
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager:
            responses.append(response)
        assert len(responses) == 6
        assert all((isinstance(i, migrationcenter.Asset) for i in responses))

@pytest.mark.asyncio
async def test_list_assets_async_pages():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_assets), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset(), migrationcenter.Asset()], next_page_token='abc'), migrationcenter.ListAssetsResponse(assets=[], next_page_token='def'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset()], next_page_token='ghi'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset()]), RuntimeError)
        pages = []
        async for page_ in (await client.list_assets(request={})).pages:
            pages.append(page_)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetAssetRequest, dict])
def test_get_asset(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_asset), '__call__') as call:
        call.return_value = migrationcenter.Asset(name='name_value', sources=['sources_value'], assigned_groups=['assigned_groups_value'])
        response = client.get_asset(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetAssetRequest()
    assert isinstance(response, migrationcenter.Asset)
    assert response.name == 'name_value'
    assert response.sources == ['sources_value']
    assert response.assigned_groups == ['assigned_groups_value']

def test_get_asset_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_asset), '__call__') as call:
        client.get_asset()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetAssetRequest()

@pytest.mark.asyncio
async def test_get_asset_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetAssetRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_asset), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Asset(name='name_value', sources=['sources_value'], assigned_groups=['assigned_groups_value']))
        response = await client.get_asset(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetAssetRequest()
    assert isinstance(response, migrationcenter.Asset)
    assert response.name == 'name_value'
    assert response.sources == ['sources_value']
    assert response.assigned_groups == ['assigned_groups_value']

@pytest.mark.asyncio
async def test_get_asset_async_from_dict():
    await test_get_asset_async(request_type=dict)

def test_get_asset_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetAssetRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_asset), '__call__') as call:
        call.return_value = migrationcenter.Asset()
        client.get_asset(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_asset_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetAssetRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_asset), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Asset())
        await client.get_asset(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_asset_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_asset), '__call__') as call:
        call.return_value = migrationcenter.Asset()
        client.get_asset(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_asset_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_asset(migrationcenter.GetAssetRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_asset_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_asset), '__call__') as call:
        call.return_value = migrationcenter.Asset()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Asset())
        response = await client.get_asset(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_asset_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_asset(migrationcenter.GetAssetRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateAssetRequest, dict])
def test_update_asset(request_type, transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_asset), '__call__') as call:
        call.return_value = migrationcenter.Asset(name='name_value', sources=['sources_value'], assigned_groups=['assigned_groups_value'])
        response = client.update_asset(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateAssetRequest()
    assert isinstance(response, migrationcenter.Asset)
    assert response.name == 'name_value'
    assert response.sources == ['sources_value']
    assert response.assigned_groups == ['assigned_groups_value']

def test_update_asset_empty_call():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.update_asset), '__call__') as call:
        client.update_asset()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateAssetRequest()

@pytest.mark.asyncio
async def test_update_asset_async(transport: str='grpc_asyncio', request_type=migrationcenter.UpdateAssetRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_asset), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Asset(name='name_value', sources=['sources_value'], assigned_groups=['assigned_groups_value']))
        response = await client.update_asset(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateAssetRequest()
    assert isinstance(response, migrationcenter.Asset)
    assert response.name == 'name_value'
    assert response.sources == ['sources_value']
    assert response.assigned_groups == ['assigned_groups_value']

@pytest.mark.asyncio
async def test_update_asset_async_from_dict():
    await test_update_asset_async(request_type=dict)

def test_update_asset_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateAssetRequest()
    request.asset.name = 'name_value'
    with mock.patch.object(type(client.transport.update_asset), '__call__') as call:
        call.return_value = migrationcenter.Asset()
        client.update_asset(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'asset.name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_update_asset_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateAssetRequest()
    request.asset.name = 'name_value'
    with mock.patch.object(type(client.transport.update_asset), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Asset())
        await client.update_asset(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'asset.name=name_value') in kw['metadata']

def test_update_asset_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_asset), '__call__') as call:
        call.return_value = migrationcenter.Asset()
        client.update_asset(asset=migrationcenter.Asset(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].asset
        mock_val = migrationcenter.Asset(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

def test_update_asset_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.update_asset(migrationcenter.UpdateAssetRequest(), asset=migrationcenter.Asset(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.asyncio
async def test_update_asset_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_asset), '__call__') as call:
        call.return_value = migrationcenter.Asset()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Asset())
        response = await client.update_asset(asset=migrationcenter.Asset(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].asset
        mock_val = migrationcenter.Asset(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

@pytest.mark.asyncio
async def test_update_asset_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.update_asset(migrationcenter.UpdateAssetRequest(), asset=migrationcenter.Asset(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.parametrize('request_type', [migrationcenter.BatchUpdateAssetsRequest, dict])
def test_batch_update_assets(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.batch_update_assets), '__call__') as call:
        call.return_value = migrationcenter.BatchUpdateAssetsResponse()
        response = client.batch_update_assets(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.BatchUpdateAssetsRequest()
    assert isinstance(response, migrationcenter.BatchUpdateAssetsResponse)

def test_batch_update_assets_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.batch_update_assets), '__call__') as call:
        client.batch_update_assets()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.BatchUpdateAssetsRequest()

@pytest.mark.asyncio
async def test_batch_update_assets_async(transport: str='grpc_asyncio', request_type=migrationcenter.BatchUpdateAssetsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.batch_update_assets), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.BatchUpdateAssetsResponse())
        response = await client.batch_update_assets(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.BatchUpdateAssetsRequest()
    assert isinstance(response, migrationcenter.BatchUpdateAssetsResponse)

@pytest.mark.asyncio
async def test_batch_update_assets_async_from_dict():
    await test_batch_update_assets_async(request_type=dict)

def test_batch_update_assets_field_headers():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.BatchUpdateAssetsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.batch_update_assets), '__call__') as call:
        call.return_value = migrationcenter.BatchUpdateAssetsResponse()
        client.batch_update_assets(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_batch_update_assets_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.BatchUpdateAssetsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.batch_update_assets), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.BatchUpdateAssetsResponse())
        await client.batch_update_assets(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_batch_update_assets_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.batch_update_assets), '__call__') as call:
        call.return_value = migrationcenter.BatchUpdateAssetsResponse()
        client.batch_update_assets(parent='parent_value', requests=[migrationcenter.UpdateAssetRequest(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))])
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].requests
        mock_val = [migrationcenter.UpdateAssetRequest(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))]
        assert arg == mock_val

def test_batch_update_assets_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.batch_update_assets(migrationcenter.BatchUpdateAssetsRequest(), parent='parent_value', requests=[migrationcenter.UpdateAssetRequest(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))])

@pytest.mark.asyncio
async def test_batch_update_assets_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.batch_update_assets), '__call__') as call:
        call.return_value = migrationcenter.BatchUpdateAssetsResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.BatchUpdateAssetsResponse())
        response = await client.batch_update_assets(parent='parent_value', requests=[migrationcenter.UpdateAssetRequest(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))])
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].requests
        mock_val = [migrationcenter.UpdateAssetRequest(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))]
        assert arg == mock_val

@pytest.mark.asyncio
async def test_batch_update_assets_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.batch_update_assets(migrationcenter.BatchUpdateAssetsRequest(), parent='parent_value', requests=[migrationcenter.UpdateAssetRequest(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))])

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteAssetRequest, dict])
def test_delete_asset(request_type, transport: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_asset), '__call__') as call:
        call.return_value = None
        response = client.delete_asset(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteAssetRequest()
    assert response is None

def test_delete_asset_empty_call():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.delete_asset), '__call__') as call:
        client.delete_asset()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteAssetRequest()

@pytest.mark.asyncio
async def test_delete_asset_async(transport: str='grpc_asyncio', request_type=migrationcenter.DeleteAssetRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_asset), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        response = await client.delete_asset(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteAssetRequest()
    assert response is None

@pytest.mark.asyncio
async def test_delete_asset_async_from_dict():
    await test_delete_asset_async(request_type=dict)

def test_delete_asset_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteAssetRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_asset), '__call__') as call:
        call.return_value = None
        client.delete_asset(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_delete_asset_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteAssetRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_asset), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        await client.delete_asset(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_delete_asset_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_asset), '__call__') as call:
        call.return_value = None
        client.delete_asset(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_delete_asset_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.delete_asset(migrationcenter.DeleteAssetRequest(), name='name_value')

@pytest.mark.asyncio
async def test_delete_asset_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_asset), '__call__') as call:
        call.return_value = None
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        response = await client.delete_asset(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_delete_asset_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.delete_asset(migrationcenter.DeleteAssetRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.BatchDeleteAssetsRequest, dict])
def test_batch_delete_assets(request_type, transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.batch_delete_assets), '__call__') as call:
        call.return_value = None
        response = client.batch_delete_assets(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.BatchDeleteAssetsRequest()
    assert response is None

def test_batch_delete_assets_empty_call():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.batch_delete_assets), '__call__') as call:
        client.batch_delete_assets()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.BatchDeleteAssetsRequest()

@pytest.mark.asyncio
async def test_batch_delete_assets_async(transport: str='grpc_asyncio', request_type=migrationcenter.BatchDeleteAssetsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.batch_delete_assets), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        response = await client.batch_delete_assets(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.BatchDeleteAssetsRequest()
    assert response is None

@pytest.mark.asyncio
async def test_batch_delete_assets_async_from_dict():
    await test_batch_delete_assets_async(request_type=dict)

def test_batch_delete_assets_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.BatchDeleteAssetsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.batch_delete_assets), '__call__') as call:
        call.return_value = None
        client.batch_delete_assets(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_batch_delete_assets_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.BatchDeleteAssetsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.batch_delete_assets), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        await client.batch_delete_assets(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_batch_delete_assets_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.batch_delete_assets), '__call__') as call:
        call.return_value = None
        client.batch_delete_assets(parent='parent_value', names=['names_value'])
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].names
        mock_val = ['names_value']
        assert arg == mock_val

def test_batch_delete_assets_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.batch_delete_assets(migrationcenter.BatchDeleteAssetsRequest(), parent='parent_value', names=['names_value'])

@pytest.mark.asyncio
async def test_batch_delete_assets_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.batch_delete_assets), '__call__') as call:
        call.return_value = None
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        response = await client.batch_delete_assets(parent='parent_value', names=['names_value'])
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].names
        mock_val = ['names_value']
        assert arg == mock_val

@pytest.mark.asyncio
async def test_batch_delete_assets_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.batch_delete_assets(migrationcenter.BatchDeleteAssetsRequest(), parent='parent_value', names=['names_value'])

@pytest.mark.parametrize('request_type', [migrationcenter.ReportAssetFramesRequest, dict])
def test_report_asset_frames(request_type, transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.report_asset_frames), '__call__') as call:
        call.return_value = migrationcenter.ReportAssetFramesResponse()
        response = client.report_asset_frames(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ReportAssetFramesRequest()
    assert isinstance(response, migrationcenter.ReportAssetFramesResponse)

def test_report_asset_frames_empty_call():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.report_asset_frames), '__call__') as call:
        client.report_asset_frames()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ReportAssetFramesRequest()

@pytest.mark.asyncio
async def test_report_asset_frames_async(transport: str='grpc_asyncio', request_type=migrationcenter.ReportAssetFramesRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.report_asset_frames), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ReportAssetFramesResponse())
        response = await client.report_asset_frames(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ReportAssetFramesRequest()
    assert isinstance(response, migrationcenter.ReportAssetFramesResponse)

@pytest.mark.asyncio
async def test_report_asset_frames_async_from_dict():
    await test_report_asset_frames_async(request_type=dict)

def test_report_asset_frames_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ReportAssetFramesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.report_asset_frames), '__call__') as call:
        call.return_value = migrationcenter.ReportAssetFramesResponse()
        client.report_asset_frames(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_report_asset_frames_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ReportAssetFramesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.report_asset_frames), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ReportAssetFramesResponse())
        await client.report_asset_frames(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.parametrize('request_type', [migrationcenter.AggregateAssetsValuesRequest, dict])
def test_aggregate_assets_values(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.aggregate_assets_values), '__call__') as call:
        call.return_value = migrationcenter.AggregateAssetsValuesResponse()
        response = client.aggregate_assets_values(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.AggregateAssetsValuesRequest()
    assert isinstance(response, migrationcenter.AggregateAssetsValuesResponse)

def test_aggregate_assets_values_empty_call():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.aggregate_assets_values), '__call__') as call:
        client.aggregate_assets_values()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.AggregateAssetsValuesRequest()

@pytest.mark.asyncio
async def test_aggregate_assets_values_async(transport: str='grpc_asyncio', request_type=migrationcenter.AggregateAssetsValuesRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.aggregate_assets_values), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.AggregateAssetsValuesResponse())
        response = await client.aggregate_assets_values(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.AggregateAssetsValuesRequest()
    assert isinstance(response, migrationcenter.AggregateAssetsValuesResponse)

@pytest.mark.asyncio
async def test_aggregate_assets_values_async_from_dict():
    await test_aggregate_assets_values_async(request_type=dict)

def test_aggregate_assets_values_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.AggregateAssetsValuesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.aggregate_assets_values), '__call__') as call:
        call.return_value = migrationcenter.AggregateAssetsValuesResponse()
        client.aggregate_assets_values(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_aggregate_assets_values_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.AggregateAssetsValuesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.aggregate_assets_values), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.AggregateAssetsValuesResponse())
        await client.aggregate_assets_values(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.parametrize('request_type', [migrationcenter.CreateImportJobRequest, dict])
def test_create_import_job(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.create_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateImportJobRequest()
    assert isinstance(response, future.Future)

def test_create_import_job_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.create_import_job), '__call__') as call:
        client.create_import_job()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateImportJobRequest()

@pytest.mark.asyncio
async def test_create_import_job_async(transport: str='grpc_asyncio', request_type=migrationcenter.CreateImportJobRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateImportJobRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_create_import_job_async_from_dict():
    await test_create_import_job_async(request_type=dict)

def test_create_import_job_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateImportJobRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_create_import_job_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateImportJobRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.create_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_create_import_job_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_import_job(parent='parent_value', import_job=migrationcenter.ImportJob(name='name_value'), import_job_id='import_job_id_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].import_job
        mock_val = migrationcenter.ImportJob(name='name_value')
        assert arg == mock_val
        arg = args[0].import_job_id
        mock_val = 'import_job_id_value'
        assert arg == mock_val

def test_create_import_job_flattened_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.create_import_job(migrationcenter.CreateImportJobRequest(), parent='parent_value', import_job=migrationcenter.ImportJob(name='name_value'), import_job_id='import_job_id_value')

@pytest.mark.asyncio
async def test_create_import_job_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_import_job(parent='parent_value', import_job=migrationcenter.ImportJob(name='name_value'), import_job_id='import_job_id_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].import_job
        mock_val = migrationcenter.ImportJob(name='name_value')
        assert arg == mock_val
        arg = args[0].import_job_id
        mock_val = 'import_job_id_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_create_import_job_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.create_import_job(migrationcenter.CreateImportJobRequest(), parent='parent_value', import_job=migrationcenter.ImportJob(name='name_value'), import_job_id='import_job_id_value')

@pytest.mark.parametrize('request_type', [migrationcenter.ListImportJobsRequest, dict])
def test_list_import_jobs(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__') as call:
        call.return_value = migrationcenter.ListImportJobsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response = client.list_import_jobs(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListImportJobsRequest()
    assert isinstance(response, pagers.ListImportJobsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_import_jobs_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__') as call:
        client.list_import_jobs()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListImportJobsRequest()

@pytest.mark.asyncio
async def test_list_import_jobs_async(transport: str='grpc_asyncio', request_type=migrationcenter.ListImportJobsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListImportJobsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value']))
        response = await client.list_import_jobs(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListImportJobsRequest()
    assert isinstance(response, pagers.ListImportJobsAsyncPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

@pytest.mark.asyncio
async def test_list_import_jobs_async_from_dict():
    await test_list_import_jobs_async(request_type=dict)

def test_list_import_jobs_field_headers():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListImportJobsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__') as call:
        call.return_value = migrationcenter.ListImportJobsResponse()
        client.list_import_jobs(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_list_import_jobs_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListImportJobsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListImportJobsResponse())
        await client.list_import_jobs(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_list_import_jobs_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__') as call:
        call.return_value = migrationcenter.ListImportJobsResponse()
        client.list_import_jobs(parent='parent_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

def test_list_import_jobs_flattened_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.list_import_jobs(migrationcenter.ListImportJobsRequest(), parent='parent_value')

@pytest.mark.asyncio
async def test_list_import_jobs_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__') as call:
        call.return_value = migrationcenter.ListImportJobsResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListImportJobsResponse())
        response = await client.list_import_jobs(parent='parent_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_list_import_jobs_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.list_import_jobs(migrationcenter.ListImportJobsRequest(), parent='parent_value')

def test_list_import_jobs_pager(transport_name: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__') as call:
        call.side_effect = (migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob(), migrationcenter.ImportJob()], next_page_token='abc'), migrationcenter.ListImportJobsResponse(import_jobs=[], next_page_token='def'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob()], next_page_token='ghi'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob()]), RuntimeError)
        metadata = ()
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((('parent', ''),)),)
        pager = client.list_import_jobs(request={})
        assert pager._metadata == metadata
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.ImportJob) for i in results))

def test_list_import_jobs_pages(transport_name: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__') as call:
        call.side_effect = (migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob(), migrationcenter.ImportJob()], next_page_token='abc'), migrationcenter.ListImportJobsResponse(import_jobs=[], next_page_token='def'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob()], next_page_token='ghi'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob()]), RuntimeError)
        pages = list(client.list_import_jobs(request={}).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_import_jobs_async_pager():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob(), migrationcenter.ImportJob()], next_page_token='abc'), migrationcenter.ListImportJobsResponse(import_jobs=[], next_page_token='def'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob()], next_page_token='ghi'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob()]), RuntimeError)
        async_pager = await client.list_import_jobs(request={})
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager:
            responses.append(response)
        assert len(responses) == 6
        assert all((isinstance(i, migrationcenter.ImportJob) for i in responses))

@pytest.mark.asyncio
async def test_list_import_jobs_async_pages():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_import_jobs), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob(), migrationcenter.ImportJob()], next_page_token='abc'), migrationcenter.ListImportJobsResponse(import_jobs=[], next_page_token='def'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob()], next_page_token='ghi'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob()]), RuntimeError)
        pages = []
        async for page_ in (await client.list_import_jobs(request={})).pages:
            pages.append(page_)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetImportJobRequest, dict])
def test_get_import_job(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_import_job), '__call__') as call:
        call.return_value = migrationcenter.ImportJob(name='name_value', display_name='display_name_value', state=migrationcenter.ImportJob.ImportJobState.IMPORT_JOB_STATE_PENDING, asset_source='asset_source_value')
        response = client.get_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetImportJobRequest()
    assert isinstance(response, migrationcenter.ImportJob)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.state == migrationcenter.ImportJob.ImportJobState.IMPORT_JOB_STATE_PENDING
    assert response.asset_source == 'asset_source_value'

def test_get_import_job_empty_call():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_import_job), '__call__') as call:
        client.get_import_job()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetImportJobRequest()

@pytest.mark.asyncio
async def test_get_import_job_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetImportJobRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ImportJob(name='name_value', display_name='display_name_value', state=migrationcenter.ImportJob.ImportJobState.IMPORT_JOB_STATE_PENDING, asset_source='asset_source_value'))
        response = await client.get_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetImportJobRequest()
    assert isinstance(response, migrationcenter.ImportJob)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.state == migrationcenter.ImportJob.ImportJobState.IMPORT_JOB_STATE_PENDING
    assert response.asset_source == 'asset_source_value'

@pytest.mark.asyncio
async def test_get_import_job_async_from_dict():
    await test_get_import_job_async(request_type=dict)

def test_get_import_job_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetImportJobRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_import_job), '__call__') as call:
        call.return_value = migrationcenter.ImportJob()
        client.get_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_import_job_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetImportJobRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ImportJob())
        await client.get_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_import_job_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_import_job), '__call__') as call:
        call.return_value = migrationcenter.ImportJob()
        client.get_import_job(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_import_job_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_import_job(migrationcenter.GetImportJobRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_import_job_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_import_job), '__call__') as call:
        call.return_value = migrationcenter.ImportJob()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ImportJob())
        response = await client.get_import_job(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_import_job_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_import_job(migrationcenter.GetImportJobRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteImportJobRequest, dict])
def test_delete_import_job(request_type, transport: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.delete_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteImportJobRequest()
    assert isinstance(response, future.Future)

def test_delete_import_job_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.delete_import_job), '__call__') as call:
        client.delete_import_job()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteImportJobRequest()

@pytest.mark.asyncio
async def test_delete_import_job_async(transport: str='grpc_asyncio', request_type=migrationcenter.DeleteImportJobRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteImportJobRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_delete_import_job_async_from_dict():
    await test_delete_import_job_async(request_type=dict)

def test_delete_import_job_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteImportJobRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_delete_import_job_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteImportJobRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.delete_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_delete_import_job_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_import_job(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_delete_import_job_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.delete_import_job(migrationcenter.DeleteImportJobRequest(), name='name_value')

@pytest.mark.asyncio
async def test_delete_import_job_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_import_job(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_delete_import_job_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.delete_import_job(migrationcenter.DeleteImportJobRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateImportJobRequest, dict])
def test_update_import_job(request_type, transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.update_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateImportJobRequest()
    assert isinstance(response, future.Future)

def test_update_import_job_empty_call():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.update_import_job), '__call__') as call:
        client.update_import_job()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateImportJobRequest()

@pytest.mark.asyncio
async def test_update_import_job_async(transport: str='grpc_asyncio', request_type=migrationcenter.UpdateImportJobRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateImportJobRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_update_import_job_async_from_dict():
    await test_update_import_job_async(request_type=dict)

def test_update_import_job_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateImportJobRequest()
    request.import_job.name = 'name_value'
    with mock.patch.object(type(client.transport.update_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'import_job.name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_update_import_job_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateImportJobRequest()
    request.import_job.name = 'name_value'
    with mock.patch.object(type(client.transport.update_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.update_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'import_job.name=name_value') in kw['metadata']

def test_update_import_job_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_import_job(import_job=migrationcenter.ImportJob(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].import_job
        mock_val = migrationcenter.ImportJob(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

def test_update_import_job_flattened_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.update_import_job(migrationcenter.UpdateImportJobRequest(), import_job=migrationcenter.ImportJob(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.asyncio
async def test_update_import_job_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_import_job(import_job=migrationcenter.ImportJob(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].import_job
        mock_val = migrationcenter.ImportJob(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

@pytest.mark.asyncio
async def test_update_import_job_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.update_import_job(migrationcenter.UpdateImportJobRequest(), import_job=migrationcenter.ImportJob(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.parametrize('request_type', [migrationcenter.ValidateImportJobRequest, dict])
def test_validate_import_job(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.validate_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.validate_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ValidateImportJobRequest()
    assert isinstance(response, future.Future)

def test_validate_import_job_empty_call():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.validate_import_job), '__call__') as call:
        client.validate_import_job()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ValidateImportJobRequest()

@pytest.mark.asyncio
async def test_validate_import_job_async(transport: str='grpc_asyncio', request_type=migrationcenter.ValidateImportJobRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.validate_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.validate_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ValidateImportJobRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_validate_import_job_async_from_dict():
    await test_validate_import_job_async(request_type=dict)

def test_validate_import_job_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ValidateImportJobRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.validate_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.validate_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_validate_import_job_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ValidateImportJobRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.validate_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.validate_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_validate_import_job_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.validate_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.validate_import_job(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_validate_import_job_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.validate_import_job(migrationcenter.ValidateImportJobRequest(), name='name_value')

@pytest.mark.asyncio
async def test_validate_import_job_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.validate_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.validate_import_job(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_validate_import_job_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.validate_import_job(migrationcenter.ValidateImportJobRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.RunImportJobRequest, dict])
def test_run_import_job(request_type, transport: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.run_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.run_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.RunImportJobRequest()
    assert isinstance(response, future.Future)

def test_run_import_job_empty_call():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.run_import_job), '__call__') as call:
        client.run_import_job()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.RunImportJobRequest()

@pytest.mark.asyncio
async def test_run_import_job_async(transport: str='grpc_asyncio', request_type=migrationcenter.RunImportJobRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.run_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.run_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.RunImportJobRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_run_import_job_async_from_dict():
    await test_run_import_job_async(request_type=dict)

def test_run_import_job_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.RunImportJobRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.run_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.run_import_job(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_run_import_job_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.RunImportJobRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.run_import_job), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.run_import_job(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_run_import_job_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.run_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.run_import_job(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_run_import_job_flattened_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.run_import_job(migrationcenter.RunImportJobRequest(), name='name_value')

@pytest.mark.asyncio
async def test_run_import_job_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.run_import_job), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.run_import_job(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_run_import_job_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.run_import_job(migrationcenter.RunImportJobRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.GetImportDataFileRequest, dict])
def test_get_import_data_file(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_import_data_file), '__call__') as call:
        call.return_value = migrationcenter.ImportDataFile(name='name_value', display_name='display_name_value', format_=migrationcenter.ImportJobFormat.IMPORT_JOB_FORMAT_RVTOOLS_XLSX, state=migrationcenter.ImportDataFile.State.CREATING)
        response = client.get_import_data_file(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetImportDataFileRequest()
    assert isinstance(response, migrationcenter.ImportDataFile)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.format_ == migrationcenter.ImportJobFormat.IMPORT_JOB_FORMAT_RVTOOLS_XLSX
    assert response.state == migrationcenter.ImportDataFile.State.CREATING

def test_get_import_data_file_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_import_data_file), '__call__') as call:
        client.get_import_data_file()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetImportDataFileRequest()

@pytest.mark.asyncio
async def test_get_import_data_file_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetImportDataFileRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_import_data_file), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ImportDataFile(name='name_value', display_name='display_name_value', format_=migrationcenter.ImportJobFormat.IMPORT_JOB_FORMAT_RVTOOLS_XLSX, state=migrationcenter.ImportDataFile.State.CREATING))
        response = await client.get_import_data_file(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetImportDataFileRequest()
    assert isinstance(response, migrationcenter.ImportDataFile)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.format_ == migrationcenter.ImportJobFormat.IMPORT_JOB_FORMAT_RVTOOLS_XLSX
    assert response.state == migrationcenter.ImportDataFile.State.CREATING

@pytest.mark.asyncio
async def test_get_import_data_file_async_from_dict():
    await test_get_import_data_file_async(request_type=dict)

def test_get_import_data_file_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetImportDataFileRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_import_data_file), '__call__') as call:
        call.return_value = migrationcenter.ImportDataFile()
        client.get_import_data_file(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_import_data_file_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetImportDataFileRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_import_data_file), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ImportDataFile())
        await client.get_import_data_file(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_import_data_file_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_import_data_file), '__call__') as call:
        call.return_value = migrationcenter.ImportDataFile()
        client.get_import_data_file(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_import_data_file_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_import_data_file(migrationcenter.GetImportDataFileRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_import_data_file_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_import_data_file), '__call__') as call:
        call.return_value = migrationcenter.ImportDataFile()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ImportDataFile())
        response = await client.get_import_data_file(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_import_data_file_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_import_data_file(migrationcenter.GetImportDataFileRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.ListImportDataFilesRequest, dict])
def test_list_import_data_files(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__') as call:
        call.return_value = migrationcenter.ListImportDataFilesResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response = client.list_import_data_files(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListImportDataFilesRequest()
    assert isinstance(response, pagers.ListImportDataFilesPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_import_data_files_empty_call():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__') as call:
        client.list_import_data_files()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListImportDataFilesRequest()

@pytest.mark.asyncio
async def test_list_import_data_files_async(transport: str='grpc_asyncio', request_type=migrationcenter.ListImportDataFilesRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListImportDataFilesResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value']))
        response = await client.list_import_data_files(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListImportDataFilesRequest()
    assert isinstance(response, pagers.ListImportDataFilesAsyncPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

@pytest.mark.asyncio
async def test_list_import_data_files_async_from_dict():
    await test_list_import_data_files_async(request_type=dict)

def test_list_import_data_files_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListImportDataFilesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__') as call:
        call.return_value = migrationcenter.ListImportDataFilesResponse()
        client.list_import_data_files(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_list_import_data_files_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListImportDataFilesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListImportDataFilesResponse())
        await client.list_import_data_files(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_list_import_data_files_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__') as call:
        call.return_value = migrationcenter.ListImportDataFilesResponse()
        client.list_import_data_files(parent='parent_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

def test_list_import_data_files_flattened_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.list_import_data_files(migrationcenter.ListImportDataFilesRequest(), parent='parent_value')

@pytest.mark.asyncio
async def test_list_import_data_files_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__') as call:
        call.return_value = migrationcenter.ListImportDataFilesResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListImportDataFilesResponse())
        response = await client.list_import_data_files(parent='parent_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_list_import_data_files_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.list_import_data_files(migrationcenter.ListImportDataFilesRequest(), parent='parent_value')

def test_list_import_data_files_pager(transport_name: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__') as call:
        call.side_effect = (migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()], next_page_token='abc'), migrationcenter.ListImportDataFilesResponse(import_data_files=[], next_page_token='def'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile()], next_page_token='ghi'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()]), RuntimeError)
        metadata = ()
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((('parent', ''),)),)
        pager = client.list_import_data_files(request={})
        assert pager._metadata == metadata
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.ImportDataFile) for i in results))

def test_list_import_data_files_pages(transport_name: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__') as call:
        call.side_effect = (migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()], next_page_token='abc'), migrationcenter.ListImportDataFilesResponse(import_data_files=[], next_page_token='def'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile()], next_page_token='ghi'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()]), RuntimeError)
        pages = list(client.list_import_data_files(request={}).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_import_data_files_async_pager():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()], next_page_token='abc'), migrationcenter.ListImportDataFilesResponse(import_data_files=[], next_page_token='def'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile()], next_page_token='ghi'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()]), RuntimeError)
        async_pager = await client.list_import_data_files(request={})
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager:
            responses.append(response)
        assert len(responses) == 6
        assert all((isinstance(i, migrationcenter.ImportDataFile) for i in responses))

@pytest.mark.asyncio
async def test_list_import_data_files_async_pages():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_import_data_files), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()], next_page_token='abc'), migrationcenter.ListImportDataFilesResponse(import_data_files=[], next_page_token='def'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile()], next_page_token='ghi'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()]), RuntimeError)
        pages = []
        async for page_ in (await client.list_import_data_files(request={})).pages:
            pages.append(page_)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.CreateImportDataFileRequest, dict])
def test_create_import_data_file(request_type, transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_import_data_file), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.create_import_data_file(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateImportDataFileRequest()
    assert isinstance(response, future.Future)

def test_create_import_data_file_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.create_import_data_file), '__call__') as call:
        client.create_import_data_file()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateImportDataFileRequest()

@pytest.mark.asyncio
async def test_create_import_data_file_async(transport: str='grpc_asyncio', request_type=migrationcenter.CreateImportDataFileRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_import_data_file), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_import_data_file(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateImportDataFileRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_create_import_data_file_async_from_dict():
    await test_create_import_data_file_async(request_type=dict)

def test_create_import_data_file_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateImportDataFileRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_import_data_file), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_import_data_file(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_create_import_data_file_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateImportDataFileRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_import_data_file), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.create_import_data_file(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_create_import_data_file_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_import_data_file), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_import_data_file(parent='parent_value', import_data_file=migrationcenter.ImportDataFile(name='name_value'), import_data_file_id='import_data_file_id_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].import_data_file
        mock_val = migrationcenter.ImportDataFile(name='name_value')
        assert arg == mock_val
        arg = args[0].import_data_file_id
        mock_val = 'import_data_file_id_value'
        assert arg == mock_val

def test_create_import_data_file_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.create_import_data_file(migrationcenter.CreateImportDataFileRequest(), parent='parent_value', import_data_file=migrationcenter.ImportDataFile(name='name_value'), import_data_file_id='import_data_file_id_value')

@pytest.mark.asyncio
async def test_create_import_data_file_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_import_data_file), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_import_data_file(parent='parent_value', import_data_file=migrationcenter.ImportDataFile(name='name_value'), import_data_file_id='import_data_file_id_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].import_data_file
        mock_val = migrationcenter.ImportDataFile(name='name_value')
        assert arg == mock_val
        arg = args[0].import_data_file_id
        mock_val = 'import_data_file_id_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_create_import_data_file_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.create_import_data_file(migrationcenter.CreateImportDataFileRequest(), parent='parent_value', import_data_file=migrationcenter.ImportDataFile(name='name_value'), import_data_file_id='import_data_file_id_value')

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteImportDataFileRequest, dict])
def test_delete_import_data_file(request_type, transport: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_import_data_file), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.delete_import_data_file(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteImportDataFileRequest()
    assert isinstance(response, future.Future)

def test_delete_import_data_file_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.delete_import_data_file), '__call__') as call:
        client.delete_import_data_file()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteImportDataFileRequest()

@pytest.mark.asyncio
async def test_delete_import_data_file_async(transport: str='grpc_asyncio', request_type=migrationcenter.DeleteImportDataFileRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_import_data_file), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_import_data_file(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteImportDataFileRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_delete_import_data_file_async_from_dict():
    await test_delete_import_data_file_async(request_type=dict)

def test_delete_import_data_file_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteImportDataFileRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_import_data_file), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_import_data_file(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_delete_import_data_file_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteImportDataFileRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_import_data_file), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.delete_import_data_file(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_delete_import_data_file_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_import_data_file), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_import_data_file(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_delete_import_data_file_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.delete_import_data_file(migrationcenter.DeleteImportDataFileRequest(), name='name_value')

@pytest.mark.asyncio
async def test_delete_import_data_file_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_import_data_file), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_import_data_file(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_delete_import_data_file_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.delete_import_data_file(migrationcenter.DeleteImportDataFileRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.ListGroupsRequest, dict])
def test_list_groups(request_type, transport: str='grpc'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_groups), '__call__') as call:
        call.return_value = migrationcenter.ListGroupsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response = client.list_groups(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListGroupsRequest()
    assert isinstance(response, pagers.ListGroupsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_groups_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.list_groups), '__call__') as call:
        client.list_groups()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListGroupsRequest()

@pytest.mark.asyncio
async def test_list_groups_async(transport: str='grpc_asyncio', request_type=migrationcenter.ListGroupsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_groups), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListGroupsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value']))
        response = await client.list_groups(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListGroupsRequest()
    assert isinstance(response, pagers.ListGroupsAsyncPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

@pytest.mark.asyncio
async def test_list_groups_async_from_dict():
    await test_list_groups_async(request_type=dict)

def test_list_groups_field_headers():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListGroupsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_groups), '__call__') as call:
        call.return_value = migrationcenter.ListGroupsResponse()
        client.list_groups(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_list_groups_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListGroupsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_groups), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListGroupsResponse())
        await client.list_groups(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_list_groups_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_groups), '__call__') as call:
        call.return_value = migrationcenter.ListGroupsResponse()
        client.list_groups(parent='parent_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

def test_list_groups_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.list_groups(migrationcenter.ListGroupsRequest(), parent='parent_value')

@pytest.mark.asyncio
async def test_list_groups_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_groups), '__call__') as call:
        call.return_value = migrationcenter.ListGroupsResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListGroupsResponse())
        response = await client.list_groups(parent='parent_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_list_groups_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.list_groups(migrationcenter.ListGroupsRequest(), parent='parent_value')

def test_list_groups_pager(transport_name: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_groups), '__call__') as call:
        call.side_effect = (migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group(), migrationcenter.Group()], next_page_token='abc'), migrationcenter.ListGroupsResponse(groups=[], next_page_token='def'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group()], next_page_token='ghi'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group()]), RuntimeError)
        metadata = ()
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((('parent', ''),)),)
        pager = client.list_groups(request={})
        assert pager._metadata == metadata
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.Group) for i in results))

def test_list_groups_pages(transport_name: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_groups), '__call__') as call:
        call.side_effect = (migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group(), migrationcenter.Group()], next_page_token='abc'), migrationcenter.ListGroupsResponse(groups=[], next_page_token='def'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group()], next_page_token='ghi'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group()]), RuntimeError)
        pages = list(client.list_groups(request={}).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_groups_async_pager():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_groups), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group(), migrationcenter.Group()], next_page_token='abc'), migrationcenter.ListGroupsResponse(groups=[], next_page_token='def'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group()], next_page_token='ghi'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group()]), RuntimeError)
        async_pager = await client.list_groups(request={})
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager:
            responses.append(response)
        assert len(responses) == 6
        assert all((isinstance(i, migrationcenter.Group) for i in responses))

@pytest.mark.asyncio
async def test_list_groups_async_pages():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_groups), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group(), migrationcenter.Group()], next_page_token='abc'), migrationcenter.ListGroupsResponse(groups=[], next_page_token='def'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group()], next_page_token='ghi'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group()]), RuntimeError)
        pages = []
        async for page_ in (await client.list_groups(request={})).pages:
            pages.append(page_)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetGroupRequest, dict])
def test_get_group(request_type, transport: str='grpc'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_group), '__call__') as call:
        call.return_value = migrationcenter.Group(name='name_value', display_name='display_name_value', description='description_value')
        response = client.get_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetGroupRequest()
    assert isinstance(response, migrationcenter.Group)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'

def test_get_group_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_group), '__call__') as call:
        client.get_group()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetGroupRequest()

@pytest.mark.asyncio
async def test_get_group_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetGroupRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Group(name='name_value', display_name='display_name_value', description='description_value'))
        response = await client.get_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetGroupRequest()
    assert isinstance(response, migrationcenter.Group)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'

@pytest.mark.asyncio
async def test_get_group_async_from_dict():
    await test_get_group_async(request_type=dict)

def test_get_group_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetGroupRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_group), '__call__') as call:
        call.return_value = migrationcenter.Group()
        client.get_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_group_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetGroupRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Group())
        await client.get_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_group_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_group), '__call__') as call:
        call.return_value = migrationcenter.Group()
        client.get_group(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_group_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_group(migrationcenter.GetGroupRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_group_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_group), '__call__') as call:
        call.return_value = migrationcenter.Group()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Group())
        response = await client.get_group(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_group_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_group(migrationcenter.GetGroupRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.CreateGroupRequest, dict])
def test_create_group(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.create_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateGroupRequest()
    assert isinstance(response, future.Future)

def test_create_group_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.create_group), '__call__') as call:
        client.create_group()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateGroupRequest()

@pytest.mark.asyncio
async def test_create_group_async(transport: str='grpc_asyncio', request_type=migrationcenter.CreateGroupRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateGroupRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_create_group_async_from_dict():
    await test_create_group_async(request_type=dict)

def test_create_group_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateGroupRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_create_group_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateGroupRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.create_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_create_group_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_group(parent='parent_value', group=migrationcenter.Group(name='name_value'), group_id='group_id_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].group
        mock_val = migrationcenter.Group(name='name_value')
        assert arg == mock_val
        arg = args[0].group_id
        mock_val = 'group_id_value'
        assert arg == mock_val

def test_create_group_flattened_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.create_group(migrationcenter.CreateGroupRequest(), parent='parent_value', group=migrationcenter.Group(name='name_value'), group_id='group_id_value')

@pytest.mark.asyncio
async def test_create_group_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_group(parent='parent_value', group=migrationcenter.Group(name='name_value'), group_id='group_id_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].group
        mock_val = migrationcenter.Group(name='name_value')
        assert arg == mock_val
        arg = args[0].group_id
        mock_val = 'group_id_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_create_group_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.create_group(migrationcenter.CreateGroupRequest(), parent='parent_value', group=migrationcenter.Group(name='name_value'), group_id='group_id_value')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateGroupRequest, dict])
def test_update_group(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.update_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateGroupRequest()
    assert isinstance(response, future.Future)

def test_update_group_empty_call():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.update_group), '__call__') as call:
        client.update_group()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateGroupRequest()

@pytest.mark.asyncio
async def test_update_group_async(transport: str='grpc_asyncio', request_type=migrationcenter.UpdateGroupRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateGroupRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_update_group_async_from_dict():
    await test_update_group_async(request_type=dict)

def test_update_group_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateGroupRequest()
    request.group.name = 'name_value'
    with mock.patch.object(type(client.transport.update_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'group.name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_update_group_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateGroupRequest()
    request.group.name = 'name_value'
    with mock.patch.object(type(client.transport.update_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.update_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'group.name=name_value') in kw['metadata']

def test_update_group_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_group(group=migrationcenter.Group(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].group
        mock_val = migrationcenter.Group(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

def test_update_group_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.update_group(migrationcenter.UpdateGroupRequest(), group=migrationcenter.Group(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.asyncio
async def test_update_group_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_group(group=migrationcenter.Group(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].group
        mock_val = migrationcenter.Group(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

@pytest.mark.asyncio
async def test_update_group_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.update_group(migrationcenter.UpdateGroupRequest(), group=migrationcenter.Group(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteGroupRequest, dict])
def test_delete_group(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.delete_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteGroupRequest()
    assert isinstance(response, future.Future)

def test_delete_group_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.delete_group), '__call__') as call:
        client.delete_group()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteGroupRequest()

@pytest.mark.asyncio
async def test_delete_group_async(transport: str='grpc_asyncio', request_type=migrationcenter.DeleteGroupRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteGroupRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_delete_group_async_from_dict():
    await test_delete_group_async(request_type=dict)

def test_delete_group_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteGroupRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_delete_group_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteGroupRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.delete_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_delete_group_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_group(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_delete_group_flattened_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.delete_group(migrationcenter.DeleteGroupRequest(), name='name_value')

@pytest.mark.asyncio
async def test_delete_group_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_group(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_delete_group_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.delete_group(migrationcenter.DeleteGroupRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.AddAssetsToGroupRequest, dict])
def test_add_assets_to_group(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.add_assets_to_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.add_assets_to_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.AddAssetsToGroupRequest()
    assert isinstance(response, future.Future)

def test_add_assets_to_group_empty_call():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.add_assets_to_group), '__call__') as call:
        client.add_assets_to_group()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.AddAssetsToGroupRequest()

@pytest.mark.asyncio
async def test_add_assets_to_group_async(transport: str='grpc_asyncio', request_type=migrationcenter.AddAssetsToGroupRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.add_assets_to_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.add_assets_to_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.AddAssetsToGroupRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_add_assets_to_group_async_from_dict():
    await test_add_assets_to_group_async(request_type=dict)

def test_add_assets_to_group_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.AddAssetsToGroupRequest()
    request.group = 'group_value'
    with mock.patch.object(type(client.transport.add_assets_to_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.add_assets_to_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'group=group_value') in kw['metadata']

@pytest.mark.asyncio
async def test_add_assets_to_group_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.AddAssetsToGroupRequest()
    request.group = 'group_value'
    with mock.patch.object(type(client.transport.add_assets_to_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.add_assets_to_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'group=group_value') in kw['metadata']

def test_add_assets_to_group_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.add_assets_to_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.add_assets_to_group(group='group_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].group
        mock_val = 'group_value'
        assert arg == mock_val

def test_add_assets_to_group_flattened_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.add_assets_to_group(migrationcenter.AddAssetsToGroupRequest(), group='group_value')

@pytest.mark.asyncio
async def test_add_assets_to_group_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.add_assets_to_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.add_assets_to_group(group='group_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].group
        mock_val = 'group_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_add_assets_to_group_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.add_assets_to_group(migrationcenter.AddAssetsToGroupRequest(), group='group_value')

@pytest.mark.parametrize('request_type', [migrationcenter.RemoveAssetsFromGroupRequest, dict])
def test_remove_assets_from_group(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.remove_assets_from_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.remove_assets_from_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.RemoveAssetsFromGroupRequest()
    assert isinstance(response, future.Future)

def test_remove_assets_from_group_empty_call():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.remove_assets_from_group), '__call__') as call:
        client.remove_assets_from_group()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.RemoveAssetsFromGroupRequest()

@pytest.mark.asyncio
async def test_remove_assets_from_group_async(transport: str='grpc_asyncio', request_type=migrationcenter.RemoveAssetsFromGroupRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.remove_assets_from_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.remove_assets_from_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.RemoveAssetsFromGroupRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_remove_assets_from_group_async_from_dict():
    await test_remove_assets_from_group_async(request_type=dict)

def test_remove_assets_from_group_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.RemoveAssetsFromGroupRequest()
    request.group = 'group_value'
    with mock.patch.object(type(client.transport.remove_assets_from_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.remove_assets_from_group(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'group=group_value') in kw['metadata']

@pytest.mark.asyncio
async def test_remove_assets_from_group_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.RemoveAssetsFromGroupRequest()
    request.group = 'group_value'
    with mock.patch.object(type(client.transport.remove_assets_from_group), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.remove_assets_from_group(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'group=group_value') in kw['metadata']

def test_remove_assets_from_group_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.remove_assets_from_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.remove_assets_from_group(group='group_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].group
        mock_val = 'group_value'
        assert arg == mock_val

def test_remove_assets_from_group_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.remove_assets_from_group(migrationcenter.RemoveAssetsFromGroupRequest(), group='group_value')

@pytest.mark.asyncio
async def test_remove_assets_from_group_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.remove_assets_from_group), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.remove_assets_from_group(group='group_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].group
        mock_val = 'group_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_remove_assets_from_group_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.remove_assets_from_group(migrationcenter.RemoveAssetsFromGroupRequest(), group='group_value')

@pytest.mark.parametrize('request_type', [migrationcenter.ListErrorFramesRequest, dict])
def test_list_error_frames(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_error_frames), '__call__') as call:
        call.return_value = migrationcenter.ListErrorFramesResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response = client.list_error_frames(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListErrorFramesRequest()
    assert isinstance(response, pagers.ListErrorFramesPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_error_frames_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.list_error_frames), '__call__') as call:
        client.list_error_frames()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListErrorFramesRequest()

@pytest.mark.asyncio
async def test_list_error_frames_async(transport: str='grpc_asyncio', request_type=migrationcenter.ListErrorFramesRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_error_frames), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListErrorFramesResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value']))
        response = await client.list_error_frames(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListErrorFramesRequest()
    assert isinstance(response, pagers.ListErrorFramesAsyncPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

@pytest.mark.asyncio
async def test_list_error_frames_async_from_dict():
    await test_list_error_frames_async(request_type=dict)

def test_list_error_frames_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListErrorFramesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_error_frames), '__call__') as call:
        call.return_value = migrationcenter.ListErrorFramesResponse()
        client.list_error_frames(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_list_error_frames_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListErrorFramesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_error_frames), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListErrorFramesResponse())
        await client.list_error_frames(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_list_error_frames_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_error_frames), '__call__') as call:
        call.return_value = migrationcenter.ListErrorFramesResponse()
        client.list_error_frames(parent='parent_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

def test_list_error_frames_flattened_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.list_error_frames(migrationcenter.ListErrorFramesRequest(), parent='parent_value')

@pytest.mark.asyncio
async def test_list_error_frames_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_error_frames), '__call__') as call:
        call.return_value = migrationcenter.ListErrorFramesResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListErrorFramesResponse())
        response = await client.list_error_frames(parent='parent_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_list_error_frames_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.list_error_frames(migrationcenter.ListErrorFramesRequest(), parent='parent_value')

def test_list_error_frames_pager(transport_name: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_error_frames), '__call__') as call:
        call.side_effect = (migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()], next_page_token='abc'), migrationcenter.ListErrorFramesResponse(error_frames=[], next_page_token='def'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame()], next_page_token='ghi'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()]), RuntimeError)
        metadata = ()
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((('parent', ''),)),)
        pager = client.list_error_frames(request={})
        assert pager._metadata == metadata
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.ErrorFrame) for i in results))

def test_list_error_frames_pages(transport_name: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_error_frames), '__call__') as call:
        call.side_effect = (migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()], next_page_token='abc'), migrationcenter.ListErrorFramesResponse(error_frames=[], next_page_token='def'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame()], next_page_token='ghi'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()]), RuntimeError)
        pages = list(client.list_error_frames(request={}).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_error_frames_async_pager():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_error_frames), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()], next_page_token='abc'), migrationcenter.ListErrorFramesResponse(error_frames=[], next_page_token='def'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame()], next_page_token='ghi'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()]), RuntimeError)
        async_pager = await client.list_error_frames(request={})
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager:
            responses.append(response)
        assert len(responses) == 6
        assert all((isinstance(i, migrationcenter.ErrorFrame) for i in responses))

@pytest.mark.asyncio
async def test_list_error_frames_async_pages():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_error_frames), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()], next_page_token='abc'), migrationcenter.ListErrorFramesResponse(error_frames=[], next_page_token='def'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame()], next_page_token='ghi'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()]), RuntimeError)
        pages = []
        async for page_ in (await client.list_error_frames(request={})).pages:
            pages.append(page_)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetErrorFrameRequest, dict])
def test_get_error_frame(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_error_frame), '__call__') as call:
        call.return_value = migrationcenter.ErrorFrame(name='name_value')
        response = client.get_error_frame(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetErrorFrameRequest()
    assert isinstance(response, migrationcenter.ErrorFrame)
    assert response.name == 'name_value'

def test_get_error_frame_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_error_frame), '__call__') as call:
        client.get_error_frame()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetErrorFrameRequest()

@pytest.mark.asyncio
async def test_get_error_frame_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetErrorFrameRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_error_frame), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ErrorFrame(name='name_value'))
        response = await client.get_error_frame(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetErrorFrameRequest()
    assert isinstance(response, migrationcenter.ErrorFrame)
    assert response.name == 'name_value'

@pytest.mark.asyncio
async def test_get_error_frame_async_from_dict():
    await test_get_error_frame_async(request_type=dict)

def test_get_error_frame_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetErrorFrameRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_error_frame), '__call__') as call:
        call.return_value = migrationcenter.ErrorFrame()
        client.get_error_frame(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_error_frame_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetErrorFrameRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_error_frame), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ErrorFrame())
        await client.get_error_frame(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_error_frame_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_error_frame), '__call__') as call:
        call.return_value = migrationcenter.ErrorFrame()
        client.get_error_frame(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_error_frame_flattened_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_error_frame(migrationcenter.GetErrorFrameRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_error_frame_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_error_frame), '__call__') as call:
        call.return_value = migrationcenter.ErrorFrame()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ErrorFrame())
        response = await client.get_error_frame(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_error_frame_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_error_frame(migrationcenter.GetErrorFrameRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.ListSourcesRequest, dict])
def test_list_sources(request_type, transport: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_sources), '__call__') as call:
        call.return_value = migrationcenter.ListSourcesResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response = client.list_sources(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListSourcesRequest()
    assert isinstance(response, pagers.ListSourcesPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_sources_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.list_sources), '__call__') as call:
        client.list_sources()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListSourcesRequest()

@pytest.mark.asyncio
async def test_list_sources_async(transport: str='grpc_asyncio', request_type=migrationcenter.ListSourcesRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_sources), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListSourcesResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value']))
        response = await client.list_sources(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListSourcesRequest()
    assert isinstance(response, pagers.ListSourcesAsyncPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

@pytest.mark.asyncio
async def test_list_sources_async_from_dict():
    await test_list_sources_async(request_type=dict)

def test_list_sources_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListSourcesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_sources), '__call__') as call:
        call.return_value = migrationcenter.ListSourcesResponse()
        client.list_sources(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_list_sources_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListSourcesRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_sources), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListSourcesResponse())
        await client.list_sources(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_list_sources_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_sources), '__call__') as call:
        call.return_value = migrationcenter.ListSourcesResponse()
        client.list_sources(parent='parent_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

def test_list_sources_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.list_sources(migrationcenter.ListSourcesRequest(), parent='parent_value')

@pytest.mark.asyncio
async def test_list_sources_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_sources), '__call__') as call:
        call.return_value = migrationcenter.ListSourcesResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListSourcesResponse())
        response = await client.list_sources(parent='parent_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_list_sources_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.list_sources(migrationcenter.ListSourcesRequest(), parent='parent_value')

def test_list_sources_pager(transport_name: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_sources), '__call__') as call:
        call.side_effect = (migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source(), migrationcenter.Source()], next_page_token='abc'), migrationcenter.ListSourcesResponse(sources=[], next_page_token='def'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source()], next_page_token='ghi'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source()]), RuntimeError)
        metadata = ()
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((('parent', ''),)),)
        pager = client.list_sources(request={})
        assert pager._metadata == metadata
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.Source) for i in results))

def test_list_sources_pages(transport_name: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_sources), '__call__') as call:
        call.side_effect = (migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source(), migrationcenter.Source()], next_page_token='abc'), migrationcenter.ListSourcesResponse(sources=[], next_page_token='def'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source()], next_page_token='ghi'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source()]), RuntimeError)
        pages = list(client.list_sources(request={}).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_sources_async_pager():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_sources), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source(), migrationcenter.Source()], next_page_token='abc'), migrationcenter.ListSourcesResponse(sources=[], next_page_token='def'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source()], next_page_token='ghi'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source()]), RuntimeError)
        async_pager = await client.list_sources(request={})
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager:
            responses.append(response)
        assert len(responses) == 6
        assert all((isinstance(i, migrationcenter.Source) for i in responses))

@pytest.mark.asyncio
async def test_list_sources_async_pages():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_sources), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source(), migrationcenter.Source()], next_page_token='abc'), migrationcenter.ListSourcesResponse(sources=[], next_page_token='def'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source()], next_page_token='ghi'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source()]), RuntimeError)
        pages = []
        async for page_ in (await client.list_sources(request={})).pages:
            pages.append(page_)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetSourceRequest, dict])
def test_get_source(request_type, transport: str='grpc'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_source), '__call__') as call:
        call.return_value = migrationcenter.Source(name='name_value', display_name='display_name_value', description='description_value', type_=migrationcenter.Source.SourceType.SOURCE_TYPE_UPLOAD, priority=898, managed=True, pending_frame_count=2007, error_frame_count=1820, state=migrationcenter.Source.State.ACTIVE)
        response = client.get_source(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetSourceRequest()
    assert isinstance(response, migrationcenter.Source)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'
    assert response.type_ == migrationcenter.Source.SourceType.SOURCE_TYPE_UPLOAD
    assert response.priority == 898
    assert response.managed is True
    assert response.pending_frame_count == 2007
    assert response.error_frame_count == 1820
    assert response.state == migrationcenter.Source.State.ACTIVE

def test_get_source_empty_call():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_source), '__call__') as call:
        client.get_source()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetSourceRequest()

@pytest.mark.asyncio
async def test_get_source_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetSourceRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_source), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Source(name='name_value', display_name='display_name_value', description='description_value', type_=migrationcenter.Source.SourceType.SOURCE_TYPE_UPLOAD, priority=898, managed=True, pending_frame_count=2007, error_frame_count=1820, state=migrationcenter.Source.State.ACTIVE))
        response = await client.get_source(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetSourceRequest()
    assert isinstance(response, migrationcenter.Source)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'
    assert response.type_ == migrationcenter.Source.SourceType.SOURCE_TYPE_UPLOAD
    assert response.priority == 898
    assert response.managed is True
    assert response.pending_frame_count == 2007
    assert response.error_frame_count == 1820
    assert response.state == migrationcenter.Source.State.ACTIVE

@pytest.mark.asyncio
async def test_get_source_async_from_dict():
    await test_get_source_async(request_type=dict)

def test_get_source_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetSourceRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_source), '__call__') as call:
        call.return_value = migrationcenter.Source()
        client.get_source(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_source_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetSourceRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_source), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Source())
        await client.get_source(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_source_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_source), '__call__') as call:
        call.return_value = migrationcenter.Source()
        client.get_source(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_source_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_source(migrationcenter.GetSourceRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_source_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_source), '__call__') as call:
        call.return_value = migrationcenter.Source()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Source())
        response = await client.get_source(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_source_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_source(migrationcenter.GetSourceRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.CreateSourceRequest, dict])
def test_create_source(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.create_source(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateSourceRequest()
    assert isinstance(response, future.Future)

def test_create_source_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.create_source), '__call__') as call:
        client.create_source()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateSourceRequest()

@pytest.mark.asyncio
async def test_create_source_async(transport: str='grpc_asyncio', request_type=migrationcenter.CreateSourceRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_source), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_source(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateSourceRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_create_source_async_from_dict():
    await test_create_source_async(request_type=dict)

def test_create_source_field_headers():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateSourceRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_source(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_create_source_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateSourceRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_source), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.create_source(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_create_source_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_source(parent='parent_value', source=migrationcenter.Source(name='name_value'), source_id='source_id_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].source
        mock_val = migrationcenter.Source(name='name_value')
        assert arg == mock_val
        arg = args[0].source_id
        mock_val = 'source_id_value'
        assert arg == mock_val

def test_create_source_flattened_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.create_source(migrationcenter.CreateSourceRequest(), parent='parent_value', source=migrationcenter.Source(name='name_value'), source_id='source_id_value')

@pytest.mark.asyncio
async def test_create_source_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_source(parent='parent_value', source=migrationcenter.Source(name='name_value'), source_id='source_id_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].source
        mock_val = migrationcenter.Source(name='name_value')
        assert arg == mock_val
        arg = args[0].source_id
        mock_val = 'source_id_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_create_source_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.create_source(migrationcenter.CreateSourceRequest(), parent='parent_value', source=migrationcenter.Source(name='name_value'), source_id='source_id_value')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateSourceRequest, dict])
def test_update_source(request_type, transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.update_source(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateSourceRequest()
    assert isinstance(response, future.Future)

def test_update_source_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.update_source), '__call__') as call:
        client.update_source()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateSourceRequest()

@pytest.mark.asyncio
async def test_update_source_async(transport: str='grpc_asyncio', request_type=migrationcenter.UpdateSourceRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_source), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_source(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateSourceRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_update_source_async_from_dict():
    await test_update_source_async(request_type=dict)

def test_update_source_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateSourceRequest()
    request.source.name = 'name_value'
    with mock.patch.object(type(client.transport.update_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_source(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'source.name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_update_source_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateSourceRequest()
    request.source.name = 'name_value'
    with mock.patch.object(type(client.transport.update_source), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.update_source(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'source.name=name_value') in kw['metadata']

def test_update_source_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_source(source=migrationcenter.Source(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].source
        mock_val = migrationcenter.Source(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

def test_update_source_flattened_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.update_source(migrationcenter.UpdateSourceRequest(), source=migrationcenter.Source(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.asyncio
async def test_update_source_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_source(source=migrationcenter.Source(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].source
        mock_val = migrationcenter.Source(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

@pytest.mark.asyncio
async def test_update_source_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.update_source(migrationcenter.UpdateSourceRequest(), source=migrationcenter.Source(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteSourceRequest, dict])
def test_delete_source(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.delete_source(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteSourceRequest()
    assert isinstance(response, future.Future)

def test_delete_source_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.delete_source), '__call__') as call:
        client.delete_source()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteSourceRequest()

@pytest.mark.asyncio
async def test_delete_source_async(transport: str='grpc_asyncio', request_type=migrationcenter.DeleteSourceRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_source), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_source(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteSourceRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_delete_source_async_from_dict():
    await test_delete_source_async(request_type=dict)

def test_delete_source_field_headers():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteSourceRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_source(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_delete_source_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteSourceRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_source), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.delete_source(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_delete_source_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_source(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_delete_source_flattened_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.delete_source(migrationcenter.DeleteSourceRequest(), name='name_value')

@pytest.mark.asyncio
async def test_delete_source_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_source), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_source(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_delete_source_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.delete_source(migrationcenter.DeleteSourceRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.ListPreferenceSetsRequest, dict])
def test_list_preference_sets(request_type, transport: str='grpc'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__') as call:
        call.return_value = migrationcenter.ListPreferenceSetsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response = client.list_preference_sets(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListPreferenceSetsRequest()
    assert isinstance(response, pagers.ListPreferenceSetsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_preference_sets_empty_call():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__') as call:
        client.list_preference_sets()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListPreferenceSetsRequest()

@pytest.mark.asyncio
async def test_list_preference_sets_async(transport: str='grpc_asyncio', request_type=migrationcenter.ListPreferenceSetsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListPreferenceSetsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value']))
        response = await client.list_preference_sets(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListPreferenceSetsRequest()
    assert isinstance(response, pagers.ListPreferenceSetsAsyncPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

@pytest.mark.asyncio
async def test_list_preference_sets_async_from_dict():
    await test_list_preference_sets_async(request_type=dict)

def test_list_preference_sets_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListPreferenceSetsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__') as call:
        call.return_value = migrationcenter.ListPreferenceSetsResponse()
        client.list_preference_sets(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_list_preference_sets_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListPreferenceSetsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListPreferenceSetsResponse())
        await client.list_preference_sets(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_list_preference_sets_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__') as call:
        call.return_value = migrationcenter.ListPreferenceSetsResponse()
        client.list_preference_sets(parent='parent_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

def test_list_preference_sets_flattened_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.list_preference_sets(migrationcenter.ListPreferenceSetsRequest(), parent='parent_value')

@pytest.mark.asyncio
async def test_list_preference_sets_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__') as call:
        call.return_value = migrationcenter.ListPreferenceSetsResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListPreferenceSetsResponse())
        response = await client.list_preference_sets(parent='parent_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_list_preference_sets_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.list_preference_sets(migrationcenter.ListPreferenceSetsRequest(), parent='parent_value')

def test_list_preference_sets_pager(transport_name: str='grpc'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__') as call:
        call.side_effect = (migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()], next_page_token='abc'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[], next_page_token='def'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet()], next_page_token='ghi'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()]), RuntimeError)
        metadata = ()
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((('parent', ''),)),)
        pager = client.list_preference_sets(request={})
        assert pager._metadata == metadata
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.PreferenceSet) for i in results))

def test_list_preference_sets_pages(transport_name: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__') as call:
        call.side_effect = (migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()], next_page_token='abc'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[], next_page_token='def'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet()], next_page_token='ghi'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()]), RuntimeError)
        pages = list(client.list_preference_sets(request={}).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_preference_sets_async_pager():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()], next_page_token='abc'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[], next_page_token='def'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet()], next_page_token='ghi'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()]), RuntimeError)
        async_pager = await client.list_preference_sets(request={})
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager:
            responses.append(response)
        assert len(responses) == 6
        assert all((isinstance(i, migrationcenter.PreferenceSet) for i in responses))

@pytest.mark.asyncio
async def test_list_preference_sets_async_pages():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_preference_sets), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()], next_page_token='abc'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[], next_page_token='def'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet()], next_page_token='ghi'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()]), RuntimeError)
        pages = []
        async for page_ in (await client.list_preference_sets(request={})).pages:
            pages.append(page_)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetPreferenceSetRequest, dict])
def test_get_preference_set(request_type, transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_preference_set), '__call__') as call:
        call.return_value = migrationcenter.PreferenceSet(name='name_value', display_name='display_name_value', description='description_value')
        response = client.get_preference_set(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetPreferenceSetRequest()
    assert isinstance(response, migrationcenter.PreferenceSet)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'

def test_get_preference_set_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_preference_set), '__call__') as call:
        client.get_preference_set()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetPreferenceSetRequest()

@pytest.mark.asyncio
async def test_get_preference_set_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetPreferenceSetRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_preference_set), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.PreferenceSet(name='name_value', display_name='display_name_value', description='description_value'))
        response = await client.get_preference_set(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetPreferenceSetRequest()
    assert isinstance(response, migrationcenter.PreferenceSet)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'

@pytest.mark.asyncio
async def test_get_preference_set_async_from_dict():
    await test_get_preference_set_async(request_type=dict)

def test_get_preference_set_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetPreferenceSetRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_preference_set), '__call__') as call:
        call.return_value = migrationcenter.PreferenceSet()
        client.get_preference_set(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_preference_set_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetPreferenceSetRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_preference_set), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.PreferenceSet())
        await client.get_preference_set(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_preference_set_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_preference_set), '__call__') as call:
        call.return_value = migrationcenter.PreferenceSet()
        client.get_preference_set(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_preference_set_flattened_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_preference_set(migrationcenter.GetPreferenceSetRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_preference_set_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_preference_set), '__call__') as call:
        call.return_value = migrationcenter.PreferenceSet()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.PreferenceSet())
        response = await client.get_preference_set(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_preference_set_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_preference_set(migrationcenter.GetPreferenceSetRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.CreatePreferenceSetRequest, dict])
def test_create_preference_set(request_type, transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.create_preference_set(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreatePreferenceSetRequest()
    assert isinstance(response, future.Future)

def test_create_preference_set_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.create_preference_set), '__call__') as call:
        client.create_preference_set()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreatePreferenceSetRequest()

@pytest.mark.asyncio
async def test_create_preference_set_async(transport: str='grpc_asyncio', request_type=migrationcenter.CreatePreferenceSetRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_preference_set), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_preference_set(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreatePreferenceSetRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_create_preference_set_async_from_dict():
    await test_create_preference_set_async(request_type=dict)

def test_create_preference_set_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreatePreferenceSetRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_preference_set(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_create_preference_set_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreatePreferenceSetRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_preference_set), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.create_preference_set(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_create_preference_set_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_preference_set(parent='parent_value', preference_set=migrationcenter.PreferenceSet(name='name_value'), preference_set_id='preference_set_id_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].preference_set
        mock_val = migrationcenter.PreferenceSet(name='name_value')
        assert arg == mock_val
        arg = args[0].preference_set_id
        mock_val = 'preference_set_id_value'
        assert arg == mock_val

def test_create_preference_set_flattened_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.create_preference_set(migrationcenter.CreatePreferenceSetRequest(), parent='parent_value', preference_set=migrationcenter.PreferenceSet(name='name_value'), preference_set_id='preference_set_id_value')

@pytest.mark.asyncio
async def test_create_preference_set_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_preference_set(parent='parent_value', preference_set=migrationcenter.PreferenceSet(name='name_value'), preference_set_id='preference_set_id_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].preference_set
        mock_val = migrationcenter.PreferenceSet(name='name_value')
        assert arg == mock_val
        arg = args[0].preference_set_id
        mock_val = 'preference_set_id_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_create_preference_set_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.create_preference_set(migrationcenter.CreatePreferenceSetRequest(), parent='parent_value', preference_set=migrationcenter.PreferenceSet(name='name_value'), preference_set_id='preference_set_id_value')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdatePreferenceSetRequest, dict])
def test_update_preference_set(request_type, transport: str='grpc'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.update_preference_set(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdatePreferenceSetRequest()
    assert isinstance(response, future.Future)

def test_update_preference_set_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.update_preference_set), '__call__') as call:
        client.update_preference_set()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdatePreferenceSetRequest()

@pytest.mark.asyncio
async def test_update_preference_set_async(transport: str='grpc_asyncio', request_type=migrationcenter.UpdatePreferenceSetRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_preference_set), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_preference_set(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdatePreferenceSetRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_update_preference_set_async_from_dict():
    await test_update_preference_set_async(request_type=dict)

def test_update_preference_set_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdatePreferenceSetRequest()
    request.preference_set.name = 'name_value'
    with mock.patch.object(type(client.transport.update_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_preference_set(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'preference_set.name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_update_preference_set_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdatePreferenceSetRequest()
    request.preference_set.name = 'name_value'
    with mock.patch.object(type(client.transport.update_preference_set), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.update_preference_set(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'preference_set.name=name_value') in kw['metadata']

def test_update_preference_set_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_preference_set(preference_set=migrationcenter.PreferenceSet(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].preference_set
        mock_val = migrationcenter.PreferenceSet(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

def test_update_preference_set_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.update_preference_set(migrationcenter.UpdatePreferenceSetRequest(), preference_set=migrationcenter.PreferenceSet(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.asyncio
async def test_update_preference_set_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_preference_set(preference_set=migrationcenter.PreferenceSet(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].preference_set
        mock_val = migrationcenter.PreferenceSet(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

@pytest.mark.asyncio
async def test_update_preference_set_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.update_preference_set(migrationcenter.UpdatePreferenceSetRequest(), preference_set=migrationcenter.PreferenceSet(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.parametrize('request_type', [migrationcenter.DeletePreferenceSetRequest, dict])
def test_delete_preference_set(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.delete_preference_set(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeletePreferenceSetRequest()
    assert isinstance(response, future.Future)

def test_delete_preference_set_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.delete_preference_set), '__call__') as call:
        client.delete_preference_set()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeletePreferenceSetRequest()

@pytest.mark.asyncio
async def test_delete_preference_set_async(transport: str='grpc_asyncio', request_type=migrationcenter.DeletePreferenceSetRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_preference_set), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_preference_set(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeletePreferenceSetRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_delete_preference_set_async_from_dict():
    await test_delete_preference_set_async(request_type=dict)

def test_delete_preference_set_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeletePreferenceSetRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_preference_set(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_delete_preference_set_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeletePreferenceSetRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_preference_set), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.delete_preference_set(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_delete_preference_set_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_preference_set(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_delete_preference_set_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.delete_preference_set(migrationcenter.DeletePreferenceSetRequest(), name='name_value')

@pytest.mark.asyncio
async def test_delete_preference_set_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_preference_set), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_preference_set(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_delete_preference_set_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.delete_preference_set(migrationcenter.DeletePreferenceSetRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.GetSettingsRequest, dict])
def test_get_settings(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_settings), '__call__') as call:
        call.return_value = migrationcenter.Settings(name='name_value', preference_set='preference_set_value')
        response = client.get_settings(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetSettingsRequest()
    assert isinstance(response, migrationcenter.Settings)
    assert response.name == 'name_value'
    assert response.preference_set == 'preference_set_value'

def test_get_settings_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_settings), '__call__') as call:
        client.get_settings()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetSettingsRequest()

@pytest.mark.asyncio
async def test_get_settings_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetSettingsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_settings), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Settings(name='name_value', preference_set='preference_set_value'))
        response = await client.get_settings(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetSettingsRequest()
    assert isinstance(response, migrationcenter.Settings)
    assert response.name == 'name_value'
    assert response.preference_set == 'preference_set_value'

@pytest.mark.asyncio
async def test_get_settings_async_from_dict():
    await test_get_settings_async(request_type=dict)

def test_get_settings_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetSettingsRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_settings), '__call__') as call:
        call.return_value = migrationcenter.Settings()
        client.get_settings(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_settings_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetSettingsRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_settings), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Settings())
        await client.get_settings(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_settings_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_settings), '__call__') as call:
        call.return_value = migrationcenter.Settings()
        client.get_settings(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_settings_flattened_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_settings(migrationcenter.GetSettingsRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_settings_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_settings), '__call__') as call:
        call.return_value = migrationcenter.Settings()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Settings())
        response = await client.get_settings(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_settings_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_settings(migrationcenter.GetSettingsRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateSettingsRequest, dict])
def test_update_settings(request_type, transport: str='grpc'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_settings), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.update_settings(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateSettingsRequest()
    assert isinstance(response, future.Future)

def test_update_settings_empty_call():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.update_settings), '__call__') as call:
        client.update_settings()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateSettingsRequest()

@pytest.mark.asyncio
async def test_update_settings_async(transport: str='grpc_asyncio', request_type=migrationcenter.UpdateSettingsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.update_settings), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_settings(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.UpdateSettingsRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_update_settings_async_from_dict():
    await test_update_settings_async(request_type=dict)

def test_update_settings_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateSettingsRequest()
    request.settings.name = 'name_value'
    with mock.patch.object(type(client.transport.update_settings), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_settings(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'settings.name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_update_settings_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.UpdateSettingsRequest()
    request.settings.name = 'name_value'
    with mock.patch.object(type(client.transport.update_settings), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.update_settings(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'settings.name=name_value') in kw['metadata']

def test_update_settings_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_settings), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.update_settings(settings=migrationcenter.Settings(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].settings
        mock_val = migrationcenter.Settings(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

def test_update_settings_flattened_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.update_settings(migrationcenter.UpdateSettingsRequest(), settings=migrationcenter.Settings(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.asyncio
async def test_update_settings_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.update_settings), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.update_settings(settings=migrationcenter.Settings(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].settings
        mock_val = migrationcenter.Settings(name='name_value')
        assert arg == mock_val
        arg = args[0].update_mask
        mock_val = field_mask_pb2.FieldMask(paths=['paths_value'])
        assert arg == mock_val

@pytest.mark.asyncio
async def test_update_settings_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.update_settings(migrationcenter.UpdateSettingsRequest(), settings=migrationcenter.Settings(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

@pytest.mark.parametrize('request_type', [migrationcenter.CreateReportConfigRequest, dict])
def test_create_report_config(request_type, transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_report_config), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.create_report_config(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateReportConfigRequest()
    assert isinstance(response, future.Future)

def test_create_report_config_empty_call():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.create_report_config), '__call__') as call:
        client.create_report_config()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateReportConfigRequest()

@pytest.mark.asyncio
async def test_create_report_config_async(transport: str='grpc_asyncio', request_type=migrationcenter.CreateReportConfigRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_report_config), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_report_config(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateReportConfigRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_create_report_config_async_from_dict():
    await test_create_report_config_async(request_type=dict)

def test_create_report_config_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateReportConfigRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_report_config), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_report_config(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_create_report_config_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateReportConfigRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_report_config), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.create_report_config(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_create_report_config_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_report_config), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_report_config(parent='parent_value', report_config=migrationcenter.ReportConfig(name='name_value'), report_config_id='report_config_id_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].report_config
        mock_val = migrationcenter.ReportConfig(name='name_value')
        assert arg == mock_val
        arg = args[0].report_config_id
        mock_val = 'report_config_id_value'
        assert arg == mock_val

def test_create_report_config_flattened_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.create_report_config(migrationcenter.CreateReportConfigRequest(), parent='parent_value', report_config=migrationcenter.ReportConfig(name='name_value'), report_config_id='report_config_id_value')

@pytest.mark.asyncio
async def test_create_report_config_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_report_config), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_report_config(parent='parent_value', report_config=migrationcenter.ReportConfig(name='name_value'), report_config_id='report_config_id_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].report_config
        mock_val = migrationcenter.ReportConfig(name='name_value')
        assert arg == mock_val
        arg = args[0].report_config_id
        mock_val = 'report_config_id_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_create_report_config_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.create_report_config(migrationcenter.CreateReportConfigRequest(), parent='parent_value', report_config=migrationcenter.ReportConfig(name='name_value'), report_config_id='report_config_id_value')

@pytest.mark.parametrize('request_type', [migrationcenter.GetReportConfigRequest, dict])
def test_get_report_config(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_report_config), '__call__') as call:
        call.return_value = migrationcenter.ReportConfig(name='name_value', display_name='display_name_value', description='description_value')
        response = client.get_report_config(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetReportConfigRequest()
    assert isinstance(response, migrationcenter.ReportConfig)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'

def test_get_report_config_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_report_config), '__call__') as call:
        client.get_report_config()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetReportConfigRequest()

@pytest.mark.asyncio
async def test_get_report_config_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetReportConfigRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_report_config), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ReportConfig(name='name_value', display_name='display_name_value', description='description_value'))
        response = await client.get_report_config(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetReportConfigRequest()
    assert isinstance(response, migrationcenter.ReportConfig)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'

@pytest.mark.asyncio
async def test_get_report_config_async_from_dict():
    await test_get_report_config_async(request_type=dict)

def test_get_report_config_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetReportConfigRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_report_config), '__call__') as call:
        call.return_value = migrationcenter.ReportConfig()
        client.get_report_config(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_report_config_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetReportConfigRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_report_config), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ReportConfig())
        await client.get_report_config(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_report_config_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_report_config), '__call__') as call:
        call.return_value = migrationcenter.ReportConfig()
        client.get_report_config(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_report_config_flattened_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_report_config(migrationcenter.GetReportConfigRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_report_config_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_report_config), '__call__') as call:
        call.return_value = migrationcenter.ReportConfig()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ReportConfig())
        response = await client.get_report_config(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_report_config_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_report_config(migrationcenter.GetReportConfigRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.ListReportConfigsRequest, dict])
def test_list_report_configs(request_type, transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_report_configs), '__call__') as call:
        call.return_value = migrationcenter.ListReportConfigsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response = client.list_report_configs(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListReportConfigsRequest()
    assert isinstance(response, pagers.ListReportConfigsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_report_configs_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.list_report_configs), '__call__') as call:
        client.list_report_configs()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListReportConfigsRequest()

@pytest.mark.asyncio
async def test_list_report_configs_async(transport: str='grpc_asyncio', request_type=migrationcenter.ListReportConfigsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_report_configs), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListReportConfigsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value']))
        response = await client.list_report_configs(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListReportConfigsRequest()
    assert isinstance(response, pagers.ListReportConfigsAsyncPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

@pytest.mark.asyncio
async def test_list_report_configs_async_from_dict():
    await test_list_report_configs_async(request_type=dict)

def test_list_report_configs_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListReportConfigsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_report_configs), '__call__') as call:
        call.return_value = migrationcenter.ListReportConfigsResponse()
        client.list_report_configs(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_list_report_configs_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListReportConfigsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_report_configs), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListReportConfigsResponse())
        await client.list_report_configs(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_list_report_configs_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_report_configs), '__call__') as call:
        call.return_value = migrationcenter.ListReportConfigsResponse()
        client.list_report_configs(parent='parent_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

def test_list_report_configs_flattened_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.list_report_configs(migrationcenter.ListReportConfigsRequest(), parent='parent_value')

@pytest.mark.asyncio
async def test_list_report_configs_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_report_configs), '__call__') as call:
        call.return_value = migrationcenter.ListReportConfigsResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListReportConfigsResponse())
        response = await client.list_report_configs(parent='parent_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_list_report_configs_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.list_report_configs(migrationcenter.ListReportConfigsRequest(), parent='parent_value')

def test_list_report_configs_pager(transport_name: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_report_configs), '__call__') as call:
        call.side_effect = (migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig(), migrationcenter.ReportConfig()], next_page_token='abc'), migrationcenter.ListReportConfigsResponse(report_configs=[], next_page_token='def'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig()], next_page_token='ghi'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig()]), RuntimeError)
        metadata = ()
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((('parent', ''),)),)
        pager = client.list_report_configs(request={})
        assert pager._metadata == metadata
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.ReportConfig) for i in results))

def test_list_report_configs_pages(transport_name: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_report_configs), '__call__') as call:
        call.side_effect = (migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig(), migrationcenter.ReportConfig()], next_page_token='abc'), migrationcenter.ListReportConfigsResponse(report_configs=[], next_page_token='def'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig()], next_page_token='ghi'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig()]), RuntimeError)
        pages = list(client.list_report_configs(request={}).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_report_configs_async_pager():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_report_configs), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig(), migrationcenter.ReportConfig()], next_page_token='abc'), migrationcenter.ListReportConfigsResponse(report_configs=[], next_page_token='def'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig()], next_page_token='ghi'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig()]), RuntimeError)
        async_pager = await client.list_report_configs(request={})
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager:
            responses.append(response)
        assert len(responses) == 6
        assert all((isinstance(i, migrationcenter.ReportConfig) for i in responses))

@pytest.mark.asyncio
async def test_list_report_configs_async_pages():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_report_configs), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig(), migrationcenter.ReportConfig()], next_page_token='abc'), migrationcenter.ListReportConfigsResponse(report_configs=[], next_page_token='def'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig()], next_page_token='ghi'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig()]), RuntimeError)
        pages = []
        async for page_ in (await client.list_report_configs(request={})).pages:
            pages.append(page_)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteReportConfigRequest, dict])
def test_delete_report_config(request_type, transport: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_report_config), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.delete_report_config(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteReportConfigRequest()
    assert isinstance(response, future.Future)

def test_delete_report_config_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.delete_report_config), '__call__') as call:
        client.delete_report_config()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteReportConfigRequest()

@pytest.mark.asyncio
async def test_delete_report_config_async(transport: str='grpc_asyncio', request_type=migrationcenter.DeleteReportConfigRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_report_config), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_report_config(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteReportConfigRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_delete_report_config_async_from_dict():
    await test_delete_report_config_async(request_type=dict)

def test_delete_report_config_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteReportConfigRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_report_config), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_report_config(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_delete_report_config_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteReportConfigRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_report_config), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.delete_report_config(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_delete_report_config_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_report_config), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_report_config(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_delete_report_config_flattened_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.delete_report_config(migrationcenter.DeleteReportConfigRequest(), name='name_value')

@pytest.mark.asyncio
async def test_delete_report_config_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_report_config), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_report_config(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_delete_report_config_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.delete_report_config(migrationcenter.DeleteReportConfigRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.CreateReportRequest, dict])
def test_create_report(request_type, transport: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_report), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.create_report(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateReportRequest()
    assert isinstance(response, future.Future)

def test_create_report_empty_call():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.create_report), '__call__') as call:
        client.create_report()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateReportRequest()

@pytest.mark.asyncio
async def test_create_report_async(transport: str='grpc_asyncio', request_type=migrationcenter.CreateReportRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.create_report), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_report(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.CreateReportRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_create_report_async_from_dict():
    await test_create_report_async(request_type=dict)

def test_create_report_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateReportRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_report), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_report(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_create_report_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.CreateReportRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.create_report), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.create_report(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_create_report_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_report), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.create_report(parent='parent_value', report=migrationcenter.Report(name='name_value'), report_id='report_id_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].report
        mock_val = migrationcenter.Report(name='name_value')
        assert arg == mock_val
        arg = args[0].report_id
        mock_val = 'report_id_value'
        assert arg == mock_val

def test_create_report_flattened_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.create_report(migrationcenter.CreateReportRequest(), parent='parent_value', report=migrationcenter.Report(name='name_value'), report_id='report_id_value')

@pytest.mark.asyncio
async def test_create_report_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.create_report), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.create_report(parent='parent_value', report=migrationcenter.Report(name='name_value'), report_id='report_id_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val
        arg = args[0].report
        mock_val = migrationcenter.Report(name='name_value')
        assert arg == mock_val
        arg = args[0].report_id
        mock_val = 'report_id_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_create_report_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.create_report(migrationcenter.CreateReportRequest(), parent='parent_value', report=migrationcenter.Report(name='name_value'), report_id='report_id_value')

@pytest.mark.parametrize('request_type', [migrationcenter.GetReportRequest, dict])
def test_get_report(request_type, transport: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_report), '__call__') as call:
        call.return_value = migrationcenter.Report(name='name_value', display_name='display_name_value', description='description_value', type_=migrationcenter.Report.Type.TOTAL_COST_OF_OWNERSHIP, state=migrationcenter.Report.State.PENDING)
        response = client.get_report(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetReportRequest()
    assert isinstance(response, migrationcenter.Report)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'
    assert response.type_ == migrationcenter.Report.Type.TOTAL_COST_OF_OWNERSHIP
    assert response.state == migrationcenter.Report.State.PENDING

def test_get_report_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.get_report), '__call__') as call:
        client.get_report()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetReportRequest()

@pytest.mark.asyncio
async def test_get_report_async(transport: str='grpc_asyncio', request_type=migrationcenter.GetReportRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.get_report), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Report(name='name_value', display_name='display_name_value', description='description_value', type_=migrationcenter.Report.Type.TOTAL_COST_OF_OWNERSHIP, state=migrationcenter.Report.State.PENDING))
        response = await client.get_report(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.GetReportRequest()
    assert isinstance(response, migrationcenter.Report)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'
    assert response.type_ == migrationcenter.Report.Type.TOTAL_COST_OF_OWNERSHIP
    assert response.state == migrationcenter.Report.State.PENDING

@pytest.mark.asyncio
async def test_get_report_async_from_dict():
    await test_get_report_async(request_type=dict)

def test_get_report_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetReportRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_report), '__call__') as call:
        call.return_value = migrationcenter.Report()
        client.get_report(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_get_report_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.GetReportRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.get_report), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Report())
        await client.get_report(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_get_report_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_report), '__call__') as call:
        call.return_value = migrationcenter.Report()
        client.get_report(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_get_report_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.get_report(migrationcenter.GetReportRequest(), name='name_value')

@pytest.mark.asyncio
async def test_get_report_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_report), '__call__') as call:
        call.return_value = migrationcenter.Report()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.Report())
        response = await client.get_report(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_get_report_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.get_report(migrationcenter.GetReportRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.ListReportsRequest, dict])
def test_list_reports(request_type, transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_reports), '__call__') as call:
        call.return_value = migrationcenter.ListReportsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response = client.list_reports(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListReportsRequest()
    assert isinstance(response, pagers.ListReportsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_reports_empty_call():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.list_reports), '__call__') as call:
        client.list_reports()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListReportsRequest()

@pytest.mark.asyncio
async def test_list_reports_async(transport: str='grpc_asyncio', request_type=migrationcenter.ListReportsRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.list_reports), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListReportsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value']))
        response = await client.list_reports(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.ListReportsRequest()
    assert isinstance(response, pagers.ListReportsAsyncPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

@pytest.mark.asyncio
async def test_list_reports_async_from_dict():
    await test_list_reports_async(request_type=dict)

def test_list_reports_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListReportsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_reports), '__call__') as call:
        call.return_value = migrationcenter.ListReportsResponse()
        client.list_reports(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

@pytest.mark.asyncio
async def test_list_reports_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.ListReportsRequest()
    request.parent = 'parent_value'
    with mock.patch.object(type(client.transport.list_reports), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListReportsResponse())
        await client.list_reports(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'parent=parent_value') in kw['metadata']

def test_list_reports_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_reports), '__call__') as call:
        call.return_value = migrationcenter.ListReportsResponse()
        client.list_reports(parent='parent_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

def test_list_reports_flattened_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.list_reports(migrationcenter.ListReportsRequest(), parent='parent_value')

@pytest.mark.asyncio
async def test_list_reports_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_reports), '__call__') as call:
        call.return_value = migrationcenter.ListReportsResponse()
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(migrationcenter.ListReportsResponse())
        response = await client.list_reports(parent='parent_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].parent
        mock_val = 'parent_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_list_reports_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.list_reports(migrationcenter.ListReportsRequest(), parent='parent_value')

def test_list_reports_pager(transport_name: str='grpc'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_reports), '__call__') as call:
        call.side_effect = (migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report(), migrationcenter.Report()], next_page_token='abc'), migrationcenter.ListReportsResponse(reports=[], next_page_token='def'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report()], next_page_token='ghi'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report()]), RuntimeError)
        metadata = ()
        metadata = tuple(metadata) + (gapic_v1.routing_header.to_grpc_metadata((('parent', ''),)),)
        pager = client.list_reports(request={})
        assert pager._metadata == metadata
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.Report) for i in results))

def test_list_reports_pages(transport_name: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials, transport=transport_name)
    with mock.patch.object(type(client.transport.list_reports), '__call__') as call:
        call.side_effect = (migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report(), migrationcenter.Report()], next_page_token='abc'), migrationcenter.ListReportsResponse(reports=[], next_page_token='def'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report()], next_page_token='ghi'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report()]), RuntimeError)
        pages = list(client.list_reports(request={}).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.asyncio
async def test_list_reports_async_pager():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_reports), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report(), migrationcenter.Report()], next_page_token='abc'), migrationcenter.ListReportsResponse(reports=[], next_page_token='def'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report()], next_page_token='ghi'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report()]), RuntimeError)
        async_pager = await client.list_reports(request={})
        assert async_pager.next_page_token == 'abc'
        responses = []
        async for response in async_pager:
            responses.append(response)
        assert len(responses) == 6
        assert all((isinstance(i, migrationcenter.Report) for i in responses))

@pytest.mark.asyncio
async def test_list_reports_async_pages():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials)
    with mock.patch.object(type(client.transport.list_reports), '__call__', new_callable=mock.AsyncMock) as call:
        call.side_effect = (migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report(), migrationcenter.Report()], next_page_token='abc'), migrationcenter.ListReportsResponse(reports=[], next_page_token='def'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report()], next_page_token='ghi'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report()]), RuntimeError)
        pages = []
        async for page_ in (await client.list_reports(request={})).pages:
            pages.append(page_)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteReportRequest, dict])
def test_delete_report(request_type, transport: str='grpc'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_report), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/spam')
        response = client.delete_report(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteReportRequest()
    assert isinstance(response, future.Future)

def test_delete_report_empty_call():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    with mock.patch.object(type(client.transport.delete_report), '__call__') as call:
        client.delete_report()
        call.assert_called()
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteReportRequest()

@pytest.mark.asyncio
async def test_delete_report_async(transport: str='grpc_asyncio', request_type=migrationcenter.DeleteReportRequest):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    with mock.patch.object(type(client.transport.delete_report), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_report(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == migrationcenter.DeleteReportRequest()
    assert isinstance(response, future.Future)

@pytest.mark.asyncio
async def test_delete_report_async_from_dict():
    await test_delete_report_async(request_type=dict)

def test_delete_report_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteReportRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_report), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_report(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

@pytest.mark.asyncio
async def test_delete_report_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = migrationcenter.DeleteReportRequest()
    request.name = 'name_value'
    with mock.patch.object(type(client.transport.delete_report), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/op'))
        await client.delete_report(request)
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=name_value') in kw['metadata']

def test_delete_report_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_report), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        client.delete_report(name='name_value')
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

def test_delete_report_flattened_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client.delete_report(migrationcenter.DeleteReportRequest(), name='name_value')

@pytest.mark.asyncio
async def test_delete_report_flattened_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_report), '__call__') as call:
        call.return_value = operations_pb2.Operation(name='operations/op')
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation(name='operations/spam'))
        response = await client.delete_report(name='name_value')
        assert len(call.mock_calls)
        (_, args, _) = call.mock_calls[0]
        arg = args[0].name
        mock_val = 'name_value'
        assert arg == mock_val

@pytest.mark.asyncio
async def test_delete_report_flattened_error_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        await client.delete_report(migrationcenter.DeleteReportRequest(), name='name_value')

@pytest.mark.parametrize('request_type', [migrationcenter.ListAssetsRequest, dict])
def test_list_assets_rest(request_type):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListAssetsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListAssetsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_assets(request)
    assert isinstance(response, pagers.ListAssetsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_assets_rest_required_fields(request_type=migrationcenter.ListAssetsRequest):
    if False:
        for i in range(10):
            print('nop')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_assets._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_assets._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('filter', 'order_by', 'page_size', 'page_token', 'view'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ListAssetsResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ListAssetsResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.list_assets(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_list_assets_rest_unset_required_fields():
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.list_assets._get_unset_required_fields({})
    assert set(unset_fields) == set(('filter', 'orderBy', 'pageSize', 'pageToken', 'view')) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_list_assets_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_list_assets') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_list_assets') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ListAssetsRequest.pb(migrationcenter.ListAssetsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ListAssetsResponse.to_json(migrationcenter.ListAssetsResponse())
        request = migrationcenter.ListAssetsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ListAssetsResponse()
        client.list_assets(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_list_assets_rest_bad_request(transport: str='rest', request_type=migrationcenter.ListAssetsRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_assets(request)

def test_list_assets_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListAssetsResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListAssetsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.list_assets(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/assets' % client.transport._host, args[1])

def test_list_assets_rest_flattened_error(transport: str='rest'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.list_assets(migrationcenter.ListAssetsRequest(), parent='parent_value')

def test_list_assets_rest_pager(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with mock.patch.object(Session, 'request') as req:
        response = (migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset(), migrationcenter.Asset()], next_page_token='abc'), migrationcenter.ListAssetsResponse(assets=[], next_page_token='def'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset()], next_page_token='ghi'), migrationcenter.ListAssetsResponse(assets=[migrationcenter.Asset(), migrationcenter.Asset()]))
        response = response + response
        response = tuple((migrationcenter.ListAssetsResponse.to_json(x) for x in response))
        return_values = tuple((Response() for i in response))
        for (return_val, response_val) in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        req.side_effect = return_values
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        pager = client.list_assets(request=sample_request)
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.Asset) for i in results))
        pages = list(client.list_assets(request=sample_request).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetAssetRequest, dict])
def test_get_asset_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/assets/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Asset(name='name_value', sources=['sources_value'], assigned_groups=['assigned_groups_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Asset.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_asset(request)
    assert isinstance(response, migrationcenter.Asset)
    assert response.name == 'name_value'
    assert response.sources == ['sources_value']
    assert response.assigned_groups == ['assigned_groups_value']

def test_get_asset_rest_required_fields(request_type=migrationcenter.GetAssetRequest):
    if False:
        i = 10
        return i + 15
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_asset._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_asset._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('view',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.Asset()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.Asset.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_asset(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_asset_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_asset._get_unset_required_fields({})
    assert set(unset_fields) == set(('view',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_asset_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_asset') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_asset') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetAssetRequest.pb(migrationcenter.GetAssetRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.Asset.to_json(migrationcenter.Asset())
        request = migrationcenter.GetAssetRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.Asset()
        client.get_asset(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_asset_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetAssetRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/assets/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_asset(request)

def test_get_asset_rest_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Asset()
        sample_request = {'name': 'projects/sample1/locations/sample2/assets/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Asset.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_asset(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/assets/*}' % client.transport._host, args[1])

def test_get_asset_rest_flattened_error(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_asset(migrationcenter.GetAssetRequest(), name='name_value')

def test_get_asset_rest_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateAssetRequest, dict])
def test_update_asset_rest(request_type):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'asset': {'name': 'projects/sample1/locations/sample2/assets/sample3'}}
    request_init['asset'] = {'name': 'projects/sample1/locations/sample2/assets/sample3', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'labels': {}, 'attributes': {}, 'machine_details': {'uuid': 'uuid_value', 'machine_name': 'machine_name_value', 'create_time': {}, 'core_count': 1073, 'memory_mb': 967, 'power_state': 1, 'architecture': {'cpu_architecture': 'cpu_architecture_value', 'cpu_name': 'cpu_name_value', 'vendor': 'vendor_value', 'cpu_thread_count': 1703, 'cpu_socket_count': 1720, 'bios': {'bios_name': 'bios_name_value', 'id': 'id_value', 'manufacturer': 'manufacturer_value', 'version': 'version_value', 'release_date': {'year': 433, 'month': 550, 'day': 318}, 'smbios_uuid': 'smbios_uuid_value'}, 'firmware_type': 1, 'hyperthreading': 1}, 'guest_os': {'os_name': 'os_name_value', 'family': 1, 'version': 'version_value', 'config': {'issue': 'issue_value', 'fstab': {'entries': [{'spec': 'spec_value', 'file': 'file_value', 'vfstype': 'vfstype_value', 'mntops': 'mntops_value', 'freq': 430, 'passno': 660}]}, 'hosts': {'entries': [{'ip': 'ip_value', 'host_names': ['host_names_value1', 'host_names_value2']}]}, 'nfs_exports': {'entries': [{'export_directory': 'export_directory_value', 'hosts': ['hosts_value1', 'hosts_value2']}]}, 'selinux_mode': 1}, 'runtime': {'services': {'entries': [{'service_name': 'service_name_value', 'state': 1, 'start_mode': 1, 'exe_path': 'exe_path_value', 'cmdline': 'cmdline_value', 'pid': 317}]}, 'processes': {'entries': [{'pid': 317, 'exe_path': 'exe_path_value', 'cmdline': 'cmdline_value', 'user': 'user_value', 'attributes': {}}]}, 'network': {'scan_time': {}, 'connections': {'entries': [{'protocol': 'protocol_value', 'local_ip_address': 'local_ip_address_value', 'local_port': 1071, 'remote_ip_address': 'remote_ip_address_value', 'remote_port': 1200, 'state': 1, 'pid': 317, 'process_name': 'process_name_value'}]}}, 'last_boot_time': {}, 'domain': 'domain_value', 'machine_name': 'machine_name_value', 'installed_apps': {'entries': [{'application_name': 'application_name_value', 'vendor': 'vendor_value', 'install_time': {}, 'path': 'path_value', 'version': 'version_value'}]}, 'open_file_list': {'entries': [{'command': 'command_value', 'user': 'user_value', 'file_type': 'file_type_value', 'file_path': 'file_path_value'}]}}}, 'network': {'primary_ip_address': 'primary_ip_address_value', 'public_ip_address': 'public_ip_address_value', 'primary_mac_address': 'primary_mac_address_value', 'adapters': {'entries': [{'adapter_type': 'adapter_type_value', 'mac_address': 'mac_address_value', 'addresses': {'entries': [{'ip_address': 'ip_address_value', 'subnet_mask': 'subnet_mask_value', 'bcast': 'bcast_value', 'fqdn': 'fqdn_value', 'assignment': 1}]}}]}}, 'disks': {'total_capacity_bytes': 2135, 'total_free_bytes': 1707, 'disks': {'entries': [{'capacity_bytes': 1492, 'free_bytes': 1064, 'disk_label': 'disk_label_value', 'disk_label_type': 'disk_label_type_value', 'interface_type': 1, 'partitions': {'entries': [{'type_': 'type__value', 'file_system': 'file_system_value', 'mount_point': 'mount_point_value', 'capacity_bytes': 1492, 'free_bytes': 1064, 'uuid': 'uuid_value', 'sub_partitions': {}}]}, 'hw_address': 'hw_address_value', 'vmware': {'backing_type': 1, 'shared': True, 'vmdk_mode': 1, 'rdm_compatibility': 1}}]}}, 'platform': {'vmware_details': {'vcenter_version': 'vcenter_version_value', 'esx_version': 'esx_version_value', 'osid': 'osid_value', 'vcenter_folder': 'vcenter_folder_value', 'vcenter_uri': 'vcenter_uri_value', 'vcenter_vm_id': 'vcenter_vm_id_value'}, 'aws_ec2_details': {'machine_type_label': 'machine_type_label_value', 'location': 'location_value'}, 'azure_vm_details': {'machine_type_label': 'machine_type_label_value', 'location': 'location_value', 'provisioning_state': 'provisioning_state_value'}, 'generic_details': {'location': 'location_value'}, 'physical_details': {'location': 'location_value'}}}, 'insight_list': {'insights': [{'migration_insight': {'fit': {'fit_level': 1}, 'compute_engine_target': {'shape': {'memory_mb': 967, 'physical_core_count': 2029, 'logical_core_count': 1899, 'series': 'series_value', 'machine_type': 'machine_type_value', 'storage': [{'type_': 1, 'size_gb': 739}]}}}, 'generic_insight': {'message_id': 1041, 'default_message': 'default_message_value', 'additional_information': ['additional_information_value1', 'additional_information_value2']}}], 'update_time': {}}, 'performance_data': {'daily_resource_usage_aggregations': [{'date': {}, 'cpu': {'utilization_percentage': {'average': 0.731, 'median': 0.622, 'nintey_fifth_percentile': 0.2449, 'peak': 0.417}}, 'memory': {'utilization_percentage': {}}, 'network': {'ingress_bps': {}, 'egress_bps': {}}, 'disk': {'iops': {}}}]}, 'sources': ['sources_value1', 'sources_value2'], 'assigned_groups': ['assigned_groups_value1', 'assigned_groups_value2']}
    test_field = migrationcenter.UpdateAssetRequest.meta.fields['asset']

    def get_message_fields(field):
        if False:
            for i in range(10):
                print('nop')
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['asset'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['asset'][field])):
                    del request_init['asset'][field][i][subfield]
            else:
                del request_init['asset'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Asset(name='name_value', sources=['sources_value'], assigned_groups=['assigned_groups_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Asset.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.update_asset(request)
    assert isinstance(response, migrationcenter.Asset)
    assert response.name == 'name_value'
    assert response.sources == ['sources_value']
    assert response.assigned_groups == ['assigned_groups_value']

def test_update_asset_rest_required_fields(request_type=migrationcenter.UpdateAssetRequest):
    if False:
        i = 10
        return i + 15
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_asset._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_asset._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id', 'update_mask'))
    jsonified_request.update(unset_fields)
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.Asset()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'patch', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.Asset.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.update_asset(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_update_asset_rest_unset_required_fields():
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.update_asset._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId', 'updateMask')) & set(('updateMask', 'asset'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_update_asset_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_update_asset') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_update_asset') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.UpdateAssetRequest.pb(migrationcenter.UpdateAssetRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.Asset.to_json(migrationcenter.Asset())
        request = migrationcenter.UpdateAssetRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.Asset()
        client.update_asset(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_update_asset_rest_bad_request(transport: str='rest', request_type=migrationcenter.UpdateAssetRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'asset': {'name': 'projects/sample1/locations/sample2/assets/sample3'}}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.update_asset(request)

def test_update_asset_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Asset()
        sample_request = {'asset': {'name': 'projects/sample1/locations/sample2/assets/sample3'}}
        mock_args = dict(asset=migrationcenter.Asset(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Asset.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.update_asset(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{asset.name=projects/*/locations/*/assets/*}' % client.transport._host, args[1])

def test_update_asset_rest_flattened_error(transport: str='rest'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.update_asset(migrationcenter.UpdateAssetRequest(), asset=migrationcenter.Asset(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

def test_update_asset_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.BatchUpdateAssetsRequest, dict])
def test_batch_update_assets_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.BatchUpdateAssetsResponse()
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.BatchUpdateAssetsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.batch_update_assets(request)
    assert isinstance(response, migrationcenter.BatchUpdateAssetsResponse)

def test_batch_update_assets_rest_required_fields(request_type=migrationcenter.BatchUpdateAssetsRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).batch_update_assets._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).batch_update_assets._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.BatchUpdateAssetsResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.BatchUpdateAssetsResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.batch_update_assets(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_batch_update_assets_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.batch_update_assets._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('parent', 'requests'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_batch_update_assets_rest_interceptors(null_interceptor):
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_batch_update_assets') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_batch_update_assets') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.BatchUpdateAssetsRequest.pb(migrationcenter.BatchUpdateAssetsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.BatchUpdateAssetsResponse.to_json(migrationcenter.BatchUpdateAssetsResponse())
        request = migrationcenter.BatchUpdateAssetsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.BatchUpdateAssetsResponse()
        client.batch_update_assets(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_batch_update_assets_rest_bad_request(transport: str='rest', request_type=migrationcenter.BatchUpdateAssetsRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.batch_update_assets(request)

def test_batch_update_assets_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.BatchUpdateAssetsResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value', requests=[migrationcenter.UpdateAssetRequest(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))])
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.BatchUpdateAssetsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.batch_update_assets(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/assets:batchUpdate' % client.transport._host, args[1])

def test_batch_update_assets_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.batch_update_assets(migrationcenter.BatchUpdateAssetsRequest(), parent='parent_value', requests=[migrationcenter.UpdateAssetRequest(update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))])

def test_batch_update_assets_rest_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteAssetRequest, dict])
def test_delete_asset_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/assets/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = None
        response_value = Response()
        response_value.status_code = 200
        json_return_value = ''
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.delete_asset(request)
    assert response is None

def test_delete_asset_rest_required_fields(request_type=migrationcenter.DeleteAssetRequest):
    if False:
        i = 10
        return i + 15
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_asset._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_asset._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = None
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'delete', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = ''
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.delete_asset(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_delete_asset_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.delete_asset._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_delete_asset_rest_interceptors(null_interceptor):
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_delete_asset') as pre:
        pre.assert_not_called()
        pb_message = migrationcenter.DeleteAssetRequest.pb(migrationcenter.DeleteAssetRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        request = migrationcenter.DeleteAssetRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        client.delete_asset(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()

def test_delete_asset_rest_bad_request(transport: str='rest', request_type=migrationcenter.DeleteAssetRequest):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/assets/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.delete_asset(request)

def test_delete_asset_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = None
        sample_request = {'name': 'projects/sample1/locations/sample2/assets/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = ''
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.delete_asset(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/assets/*}' % client.transport._host, args[1])

def test_delete_asset_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.delete_asset(migrationcenter.DeleteAssetRequest(), name='name_value')

def test_delete_asset_rest_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.BatchDeleteAssetsRequest, dict])
def test_batch_delete_assets_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = None
        response_value = Response()
        response_value.status_code = 200
        json_return_value = ''
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.batch_delete_assets(request)
    assert response is None

def test_batch_delete_assets_rest_required_fields(request_type=migrationcenter.BatchDeleteAssetsRequest):
    if False:
        for i in range(10):
            print('nop')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request_init['names'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).batch_delete_assets._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    jsonified_request['names'] = 'names_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).batch_delete_assets._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    assert 'names' in jsonified_request
    assert jsonified_request['names'] == 'names_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = None
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = ''
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.batch_delete_assets(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_batch_delete_assets_rest_unset_required_fields():
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.batch_delete_assets._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('parent', 'names'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_batch_delete_assets_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_batch_delete_assets') as pre:
        pre.assert_not_called()
        pb_message = migrationcenter.BatchDeleteAssetsRequest.pb(migrationcenter.BatchDeleteAssetsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        request = migrationcenter.BatchDeleteAssetsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        client.batch_delete_assets(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()

def test_batch_delete_assets_rest_bad_request(transport: str='rest', request_type=migrationcenter.BatchDeleteAssetsRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.batch_delete_assets(request)

def test_batch_delete_assets_rest_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = None
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value', names=['names_value'])
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = ''
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.batch_delete_assets(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/assets:batchDelete' % client.transport._host, args[1])

def test_batch_delete_assets_rest_flattened_error(transport: str='rest'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.batch_delete_assets(migrationcenter.BatchDeleteAssetsRequest(), parent='parent_value', names=['names_value'])

def test_batch_delete_assets_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ReportAssetFramesRequest, dict])
def test_report_asset_frames_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init['frames'] = {'frames_data': [{'machine_details': {'uuid': 'uuid_value', 'machine_name': 'machine_name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'core_count': 1073, 'memory_mb': 967, 'power_state': 1, 'architecture': {'cpu_architecture': 'cpu_architecture_value', 'cpu_name': 'cpu_name_value', 'vendor': 'vendor_value', 'cpu_thread_count': 1703, 'cpu_socket_count': 1720, 'bios': {'bios_name': 'bios_name_value', 'id': 'id_value', 'manufacturer': 'manufacturer_value', 'version': 'version_value', 'release_date': {'year': 433, 'month': 550, 'day': 318}, 'smbios_uuid': 'smbios_uuid_value'}, 'firmware_type': 1, 'hyperthreading': 1}, 'guest_os': {'os_name': 'os_name_value', 'family': 1, 'version': 'version_value', 'config': {'issue': 'issue_value', 'fstab': {'entries': [{'spec': 'spec_value', 'file': 'file_value', 'vfstype': 'vfstype_value', 'mntops': 'mntops_value', 'freq': 430, 'passno': 660}]}, 'hosts': {'entries': [{'ip': 'ip_value', 'host_names': ['host_names_value1', 'host_names_value2']}]}, 'nfs_exports': {'entries': [{'export_directory': 'export_directory_value', 'hosts': ['hosts_value1', 'hosts_value2']}]}, 'selinux_mode': 1}, 'runtime': {'services': {'entries': [{'service_name': 'service_name_value', 'state': 1, 'start_mode': 1, 'exe_path': 'exe_path_value', 'cmdline': 'cmdline_value', 'pid': 317}]}, 'processes': {'entries': [{'pid': 317, 'exe_path': 'exe_path_value', 'cmdline': 'cmdline_value', 'user': 'user_value', 'attributes': {}}]}, 'network': {'scan_time': {}, 'connections': {'entries': [{'protocol': 'protocol_value', 'local_ip_address': 'local_ip_address_value', 'local_port': 1071, 'remote_ip_address': 'remote_ip_address_value', 'remote_port': 1200, 'state': 1, 'pid': 317, 'process_name': 'process_name_value'}]}}, 'last_boot_time': {}, 'domain': 'domain_value', 'machine_name': 'machine_name_value', 'installed_apps': {'entries': [{'application_name': 'application_name_value', 'vendor': 'vendor_value', 'install_time': {}, 'path': 'path_value', 'version': 'version_value'}]}, 'open_file_list': {'entries': [{'command': 'command_value', 'user': 'user_value', 'file_type': 'file_type_value', 'file_path': 'file_path_value'}]}}}, 'network': {'primary_ip_address': 'primary_ip_address_value', 'public_ip_address': 'public_ip_address_value', 'primary_mac_address': 'primary_mac_address_value', 'adapters': {'entries': [{'adapter_type': 'adapter_type_value', 'mac_address': 'mac_address_value', 'addresses': {'entries': [{'ip_address': 'ip_address_value', 'subnet_mask': 'subnet_mask_value', 'bcast': 'bcast_value', 'fqdn': 'fqdn_value', 'assignment': 1}]}}]}}, 'disks': {'total_capacity_bytes': 2135, 'total_free_bytes': 1707, 'disks': {'entries': [{'capacity_bytes': 1492, 'free_bytes': 1064, 'disk_label': 'disk_label_value', 'disk_label_type': 'disk_label_type_value', 'interface_type': 1, 'partitions': {'entries': [{'type_': 'type__value', 'file_system': 'file_system_value', 'mount_point': 'mount_point_value', 'capacity_bytes': 1492, 'free_bytes': 1064, 'uuid': 'uuid_value', 'sub_partitions': {}}]}, 'hw_address': 'hw_address_value', 'vmware': {'backing_type': 1, 'shared': True, 'vmdk_mode': 1, 'rdm_compatibility': 1}}]}}, 'platform': {'vmware_details': {'vcenter_version': 'vcenter_version_value', 'esx_version': 'esx_version_value', 'osid': 'osid_value', 'vcenter_folder': 'vcenter_folder_value', 'vcenter_uri': 'vcenter_uri_value', 'vcenter_vm_id': 'vcenter_vm_id_value'}, 'aws_ec2_details': {'machine_type_label': 'machine_type_label_value', 'location': 'location_value'}, 'azure_vm_details': {'machine_type_label': 'machine_type_label_value', 'location': 'location_value', 'provisioning_state': 'provisioning_state_value'}, 'generic_details': {'location': 'location_value'}, 'physical_details': {'location': 'location_value'}}}, 'report_time': {}, 'labels': {}, 'attributes': {}, 'performance_samples': [{'sample_time': {}, 'memory': {'utilized_percentage': 0.2023}, 'cpu': {'utilized_percentage': 0.2023}, 'network': {'average_ingress_bps': 0.20090000000000002, 'average_egress_bps': 0.1895}, 'disk': {'average_iops': 0.1269}}], 'trace_token': 'trace_token_value'}]}
    test_field = migrationcenter.ReportAssetFramesRequest.meta.fields['frames']

    def get_message_fields(field):
        if False:
            for i in range(10):
                print('nop')
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['frames'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['frames'][field])):
                    del request_init['frames'][field][i][subfield]
            else:
                del request_init['frames'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ReportAssetFramesResponse()
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ReportAssetFramesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.report_asset_frames(request)
    assert isinstance(response, migrationcenter.ReportAssetFramesResponse)

def test_report_asset_frames_rest_required_fields(request_type=migrationcenter.ReportAssetFramesRequest):
    if False:
        for i in range(10):
            print('nop')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request_init['source'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    assert 'source' not in jsonified_request
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).report_asset_frames._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'source' in jsonified_request
    assert jsonified_request['source'] == request_init['source']
    jsonified_request['parent'] = 'parent_value'
    jsonified_request['source'] = 'source_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).report_asset_frames._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('source',))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    assert 'source' in jsonified_request
    assert jsonified_request['source'] == 'source_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ReportAssetFramesResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ReportAssetFramesResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.report_asset_frames(request)
            expected_params = [('source', ''), ('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_report_asset_frames_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.report_asset_frames._get_unset_required_fields({})
    assert set(unset_fields) == set(('source',)) & set(('parent', 'source'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_report_asset_frames_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_report_asset_frames') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_report_asset_frames') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ReportAssetFramesRequest.pb(migrationcenter.ReportAssetFramesRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ReportAssetFramesResponse.to_json(migrationcenter.ReportAssetFramesResponse())
        request = migrationcenter.ReportAssetFramesRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ReportAssetFramesResponse()
        client.report_asset_frames(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_report_asset_frames_rest_bad_request(transport: str='rest', request_type=migrationcenter.ReportAssetFramesRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.report_asset_frames(request)

def test_report_asset_frames_rest_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.AggregateAssetsValuesRequest, dict])
def test_aggregate_assets_values_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.AggregateAssetsValuesResponse()
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.AggregateAssetsValuesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.aggregate_assets_values(request)
    assert isinstance(response, migrationcenter.AggregateAssetsValuesResponse)

def test_aggregate_assets_values_rest_required_fields(request_type=migrationcenter.AggregateAssetsValuesRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).aggregate_assets_values._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).aggregate_assets_values._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.AggregateAssetsValuesResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.AggregateAssetsValuesResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.aggregate_assets_values(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_aggregate_assets_values_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.aggregate_assets_values._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_aggregate_assets_values_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_aggregate_assets_values') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_aggregate_assets_values') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.AggregateAssetsValuesRequest.pb(migrationcenter.AggregateAssetsValuesRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.AggregateAssetsValuesResponse.to_json(migrationcenter.AggregateAssetsValuesResponse())
        request = migrationcenter.AggregateAssetsValuesRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.AggregateAssetsValuesResponse()
        client.aggregate_assets_values(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_aggregate_assets_values_rest_bad_request(transport: str='rest', request_type=migrationcenter.AggregateAssetsValuesRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.aggregate_assets_values(request)

def test_aggregate_assets_values_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.CreateImportJobRequest, dict])
def test_create_import_job_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init['import_job'] = {'name': 'name_value', 'display_name': 'display_name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'complete_time': {}, 'state': 1, 'labels': {}, 'asset_source': 'asset_source_value', 'validation_report': {'file_validations': [{'file_name': 'file_name_value', 'row_errors': [{'row_number': 1088, 'vm_name': 'vm_name_value', 'vm_uuid': 'vm_uuid_value', 'errors': [{'error_details': 'error_details_value', 'severity': 1}]}], 'partial_report': True, 'file_errors': {}}], 'job_errors': {}}, 'execution_report': {'frames_reported': 1602, 'execution_errors': {}, 'total_rows_count': 1750}}
    test_field = migrationcenter.CreateImportJobRequest.meta.fields['import_job']

    def get_message_fields(field):
        if False:
            i = 10
            return i + 15
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['import_job'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['import_job'][field])):
                    del request_init['import_job'][field][i][subfield]
            else:
                del request_init['import_job'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.create_import_job(request)
    assert response.operation.name == 'operations/spam'

def test_create_import_job_rest_required_fields(request_type=migrationcenter.CreateImportJobRequest):
    if False:
        for i in range(10):
            print('nop')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request_init['import_job_id'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    assert 'importJobId' not in jsonified_request
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_import_job._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'importJobId' in jsonified_request
    assert jsonified_request['importJobId'] == request_init['import_job_id']
    jsonified_request['parent'] = 'parent_value'
    jsonified_request['importJobId'] = 'import_job_id_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_import_job._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('import_job_id', 'request_id'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    assert 'importJobId' in jsonified_request
    assert jsonified_request['importJobId'] == 'import_job_id_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.create_import_job(request)
            expected_params = [('importJobId', ''), ('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_create_import_job_rest_unset_required_fields():
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.create_import_job._get_unset_required_fields({})
    assert set(unset_fields) == set(('importJobId', 'requestId')) & set(('parent', 'importJobId', 'importJob'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_create_import_job_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_create_import_job') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_create_import_job') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.CreateImportJobRequest.pb(migrationcenter.CreateImportJobRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.CreateImportJobRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.create_import_job(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_create_import_job_rest_bad_request(transport: str='rest', request_type=migrationcenter.CreateImportJobRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.create_import_job(request)

def test_create_import_job_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value', import_job=migrationcenter.ImportJob(name='name_value'), import_job_id='import_job_id_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.create_import_job(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/importJobs' % client.transport._host, args[1])

def test_create_import_job_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.create_import_job(migrationcenter.CreateImportJobRequest(), parent='parent_value', import_job=migrationcenter.ImportJob(name='name_value'), import_job_id='import_job_id_value')

def test_create_import_job_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ListImportJobsRequest, dict])
def test_list_import_jobs_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListImportJobsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListImportJobsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_import_jobs(request)
    assert isinstance(response, pagers.ListImportJobsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_import_jobs_rest_required_fields(request_type=migrationcenter.ListImportJobsRequest):
    if False:
        i = 10
        return i + 15
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_import_jobs._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_import_jobs._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('filter', 'order_by', 'page_size', 'page_token', 'view'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ListImportJobsResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ListImportJobsResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.list_import_jobs(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_list_import_jobs_rest_unset_required_fields():
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.list_import_jobs._get_unset_required_fields({})
    assert set(unset_fields) == set(('filter', 'orderBy', 'pageSize', 'pageToken', 'view')) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_list_import_jobs_rest_interceptors(null_interceptor):
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_list_import_jobs') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_list_import_jobs') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ListImportJobsRequest.pb(migrationcenter.ListImportJobsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ListImportJobsResponse.to_json(migrationcenter.ListImportJobsResponse())
        request = migrationcenter.ListImportJobsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ListImportJobsResponse()
        client.list_import_jobs(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_list_import_jobs_rest_bad_request(transport: str='rest', request_type=migrationcenter.ListImportJobsRequest):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_import_jobs(request)

def test_list_import_jobs_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListImportJobsResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListImportJobsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.list_import_jobs(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/importJobs' % client.transport._host, args[1])

def test_list_import_jobs_rest_flattened_error(transport: str='rest'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.list_import_jobs(migrationcenter.ListImportJobsRequest(), parent='parent_value')

def test_list_import_jobs_rest_pager(transport: str='rest'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with mock.patch.object(Session, 'request') as req:
        response = (migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob(), migrationcenter.ImportJob()], next_page_token='abc'), migrationcenter.ListImportJobsResponse(import_jobs=[], next_page_token='def'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob()], next_page_token='ghi'), migrationcenter.ListImportJobsResponse(import_jobs=[migrationcenter.ImportJob(), migrationcenter.ImportJob()]))
        response = response + response
        response = tuple((migrationcenter.ListImportJobsResponse.to_json(x) for x in response))
        return_values = tuple((Response() for i in response))
        for (return_val, response_val) in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        req.side_effect = return_values
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        pager = client.list_import_jobs(request=sample_request)
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.ImportJob) for i in results))
        pages = list(client.list_import_jobs(request=sample_request).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetImportJobRequest, dict])
def test_get_import_job_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ImportJob(name='name_value', display_name='display_name_value', state=migrationcenter.ImportJob.ImportJobState.IMPORT_JOB_STATE_PENDING, asset_source='asset_source_value')
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ImportJob.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_import_job(request)
    assert isinstance(response, migrationcenter.ImportJob)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.state == migrationcenter.ImportJob.ImportJobState.IMPORT_JOB_STATE_PENDING
    assert response.asset_source == 'asset_source_value'

def test_get_import_job_rest_required_fields(request_type=migrationcenter.GetImportJobRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_import_job._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_import_job._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('view',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ImportJob()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ImportJob.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_import_job(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_import_job_rest_unset_required_fields():
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_import_job._get_unset_required_fields({})
    assert set(unset_fields) == set(('view',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_import_job_rest_interceptors(null_interceptor):
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_import_job') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_import_job') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetImportJobRequest.pb(migrationcenter.GetImportJobRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ImportJob.to_json(migrationcenter.ImportJob())
        request = migrationcenter.GetImportJobRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ImportJob()
        client.get_import_job(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_import_job_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetImportJobRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_import_job(request)

def test_get_import_job_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ImportJob()
        sample_request = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ImportJob.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_import_job(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/importJobs/*}' % client.transport._host, args[1])

def test_get_import_job_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_import_job(migrationcenter.GetImportJobRequest(), name='name_value')

def test_get_import_job_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteImportJobRequest, dict])
def test_delete_import_job_rest(request_type):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.delete_import_job(request)
    assert response.operation.name == 'operations/spam'

def test_delete_import_job_rest_required_fields(request_type=migrationcenter.DeleteImportJobRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_import_job._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_import_job._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('force', 'request_id'))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'delete', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.delete_import_job(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_delete_import_job_rest_unset_required_fields():
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.delete_import_job._get_unset_required_fields({})
    assert set(unset_fields) == set(('force', 'requestId')) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_delete_import_job_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_delete_import_job') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_delete_import_job') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.DeleteImportJobRequest.pb(migrationcenter.DeleteImportJobRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.DeleteImportJobRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.delete_import_job(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_delete_import_job_rest_bad_request(transport: str='rest', request_type=migrationcenter.DeleteImportJobRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.delete_import_job(request)

def test_delete_import_job_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.delete_import_job(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/importJobs/*}' % client.transport._host, args[1])

def test_delete_import_job_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.delete_import_job(migrationcenter.DeleteImportJobRequest(), name='name_value')

def test_delete_import_job_rest_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateImportJobRequest, dict])
def test_update_import_job_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'import_job': {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}}
    request_init['import_job'] = {'name': 'projects/sample1/locations/sample2/importJobs/sample3', 'display_name': 'display_name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'complete_time': {}, 'state': 1, 'labels': {}, 'asset_source': 'asset_source_value', 'validation_report': {'file_validations': [{'file_name': 'file_name_value', 'row_errors': [{'row_number': 1088, 'vm_name': 'vm_name_value', 'vm_uuid': 'vm_uuid_value', 'errors': [{'error_details': 'error_details_value', 'severity': 1}]}], 'partial_report': True, 'file_errors': {}}], 'job_errors': {}}, 'execution_report': {'frames_reported': 1602, 'execution_errors': {}, 'total_rows_count': 1750}}
    test_field = migrationcenter.UpdateImportJobRequest.meta.fields['import_job']

    def get_message_fields(field):
        if False:
            while True:
                i = 10
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['import_job'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['import_job'][field])):
                    del request_init['import_job'][field][i][subfield]
            else:
                del request_init['import_job'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.update_import_job(request)
    assert response.operation.name == 'operations/spam'

def test_update_import_job_rest_required_fields(request_type=migrationcenter.UpdateImportJobRequest):
    if False:
        return 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_import_job._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_import_job._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id', 'update_mask'))
    jsonified_request.update(unset_fields)
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'patch', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.update_import_job(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_update_import_job_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.update_import_job._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId', 'updateMask')) & set(('updateMask', 'importJob'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_update_import_job_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_update_import_job') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_update_import_job') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.UpdateImportJobRequest.pb(migrationcenter.UpdateImportJobRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.UpdateImportJobRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.update_import_job(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_update_import_job_rest_bad_request(transport: str='rest', request_type=migrationcenter.UpdateImportJobRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'import_job': {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.update_import_job(request)

def test_update_import_job_rest_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'import_job': {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}}
        mock_args = dict(import_job=migrationcenter.ImportJob(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.update_import_job(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{import_job.name=projects/*/locations/*/importJobs/*}' % client.transport._host, args[1])

def test_update_import_job_rest_flattened_error(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.update_import_job(migrationcenter.UpdateImportJobRequest(), import_job=migrationcenter.ImportJob(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

def test_update_import_job_rest_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ValidateImportJobRequest, dict])
def test_validate_import_job_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.validate_import_job(request)
    assert response.operation.name == 'operations/spam'

def test_validate_import_job_rest_required_fields(request_type=migrationcenter.ValidateImportJobRequest):
    if False:
        return 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).validate_import_job._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).validate_import_job._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.validate_import_job(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_validate_import_job_rest_unset_required_fields():
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.validate_import_job._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_validate_import_job_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_validate_import_job') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_validate_import_job') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ValidateImportJobRequest.pb(migrationcenter.ValidateImportJobRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.ValidateImportJobRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.validate_import_job(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_validate_import_job_rest_bad_request(transport: str='rest', request_type=migrationcenter.ValidateImportJobRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.validate_import_job(request)

def test_validate_import_job_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.validate_import_job(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/importJobs/*}:validate' % client.transport._host, args[1])

def test_validate_import_job_rest_flattened_error(transport: str='rest'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.validate_import_job(migrationcenter.ValidateImportJobRequest(), name='name_value')

def test_validate_import_job_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.RunImportJobRequest, dict])
def test_run_import_job_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.run_import_job(request)
    assert response.operation.name == 'operations/spam'

def test_run_import_job_rest_required_fields(request_type=migrationcenter.RunImportJobRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).run_import_job._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).run_import_job._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.run_import_job(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_run_import_job_rest_unset_required_fields():
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.run_import_job._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_run_import_job_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_run_import_job') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_run_import_job') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.RunImportJobRequest.pb(migrationcenter.RunImportJobRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.RunImportJobRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.run_import_job(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_run_import_job_rest_bad_request(transport: str='rest', request_type=migrationcenter.RunImportJobRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.run_import_job(request)

def test_run_import_job_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'name': 'projects/sample1/locations/sample2/importJobs/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.run_import_job(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/importJobs/*}:run' % client.transport._host, args[1])

def test_run_import_job_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.run_import_job(migrationcenter.RunImportJobRequest(), name='name_value')

def test_run_import_job_rest_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.GetImportDataFileRequest, dict])
def test_get_import_data_file_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3/importDataFiles/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ImportDataFile(name='name_value', display_name='display_name_value', format_=migrationcenter.ImportJobFormat.IMPORT_JOB_FORMAT_RVTOOLS_XLSX, state=migrationcenter.ImportDataFile.State.CREATING)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ImportDataFile.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_import_data_file(request)
    assert isinstance(response, migrationcenter.ImportDataFile)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.format_ == migrationcenter.ImportJobFormat.IMPORT_JOB_FORMAT_RVTOOLS_XLSX
    assert response.state == migrationcenter.ImportDataFile.State.CREATING

def test_get_import_data_file_rest_required_fields(request_type=migrationcenter.GetImportDataFileRequest):
    if False:
        return 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_import_data_file._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_import_data_file._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ImportDataFile()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ImportDataFile.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_import_data_file(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_import_data_file_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_import_data_file._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_import_data_file_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_import_data_file') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_import_data_file') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetImportDataFileRequest.pb(migrationcenter.GetImportDataFileRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ImportDataFile.to_json(migrationcenter.ImportDataFile())
        request = migrationcenter.GetImportDataFileRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ImportDataFile()
        client.get_import_data_file(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_import_data_file_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetImportDataFileRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3/importDataFiles/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_import_data_file(request)

def test_get_import_data_file_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ImportDataFile()
        sample_request = {'name': 'projects/sample1/locations/sample2/importJobs/sample3/importDataFiles/sample4'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ImportDataFile.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_import_data_file(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/importJobs/*/importDataFiles/*}' % client.transport._host, args[1])

def test_get_import_data_file_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_import_data_file(migrationcenter.GetImportDataFileRequest(), name='name_value')

def test_get_import_data_file_rest_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ListImportDataFilesRequest, dict])
def test_list_import_data_files_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListImportDataFilesResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListImportDataFilesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_import_data_files(request)
    assert isinstance(response, pagers.ListImportDataFilesPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_import_data_files_rest_required_fields(request_type=migrationcenter.ListImportDataFilesRequest):
    if False:
        for i in range(10):
            print('nop')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_import_data_files._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_import_data_files._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('filter', 'order_by', 'page_size', 'page_token'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ListImportDataFilesResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ListImportDataFilesResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.list_import_data_files(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_list_import_data_files_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.list_import_data_files._get_unset_required_fields({})
    assert set(unset_fields) == set(('filter', 'orderBy', 'pageSize', 'pageToken')) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_list_import_data_files_rest_interceptors(null_interceptor):
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_list_import_data_files') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_list_import_data_files') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ListImportDataFilesRequest.pb(migrationcenter.ListImportDataFilesRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ListImportDataFilesResponse.to_json(migrationcenter.ListImportDataFilesResponse())
        request = migrationcenter.ListImportDataFilesRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ListImportDataFilesResponse()
        client.list_import_data_files(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_list_import_data_files_rest_bad_request(transport: str='rest', request_type=migrationcenter.ListImportDataFilesRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_import_data_files(request)

def test_list_import_data_files_rest_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListImportDataFilesResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2/importJobs/sample3'}
        mock_args = dict(parent='parent_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListImportDataFilesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.list_import_data_files(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*/importJobs/*}/importDataFiles' % client.transport._host, args[1])

def test_list_import_data_files_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.list_import_data_files(migrationcenter.ListImportDataFilesRequest(), parent='parent_value')

def test_list_import_data_files_rest_pager(transport: str='rest'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with mock.patch.object(Session, 'request') as req:
        response = (migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()], next_page_token='abc'), migrationcenter.ListImportDataFilesResponse(import_data_files=[], next_page_token='def'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile()], next_page_token='ghi'), migrationcenter.ListImportDataFilesResponse(import_data_files=[migrationcenter.ImportDataFile(), migrationcenter.ImportDataFile()]))
        response = response + response
        response = tuple((migrationcenter.ListImportDataFilesResponse.to_json(x) for x in response))
        return_values = tuple((Response() for i in response))
        for (return_val, response_val) in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        req.side_effect = return_values
        sample_request = {'parent': 'projects/sample1/locations/sample2/importJobs/sample3'}
        pager = client.list_import_data_files(request=sample_request)
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.ImportDataFile) for i in results))
        pages = list(client.list_import_data_files(request=sample_request).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.CreateImportDataFileRequest, dict])
def test_create_import_data_file_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request_init['import_data_file'] = {'name': 'name_value', 'display_name': 'display_name_value', 'format_': 1, 'create_time': {'seconds': 751, 'nanos': 543}, 'state': 1, 'upload_file_info': {'signed_uri': 'signed_uri_value', 'headers': {}, 'uri_expiration_time': {}}}
    test_field = migrationcenter.CreateImportDataFileRequest.meta.fields['import_data_file']

    def get_message_fields(field):
        if False:
            print('Hello World!')
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['import_data_file'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['import_data_file'][field])):
                    del request_init['import_data_file'][field][i][subfield]
            else:
                del request_init['import_data_file'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.create_import_data_file(request)
    assert response.operation.name == 'operations/spam'

def test_create_import_data_file_rest_required_fields(request_type=migrationcenter.CreateImportDataFileRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request_init['import_data_file_id'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    assert 'importDataFileId' not in jsonified_request
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_import_data_file._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'importDataFileId' in jsonified_request
    assert jsonified_request['importDataFileId'] == request_init['import_data_file_id']
    jsonified_request['parent'] = 'parent_value'
    jsonified_request['importDataFileId'] = 'import_data_file_id_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_import_data_file._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('import_data_file_id', 'request_id'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    assert 'importDataFileId' in jsonified_request
    assert jsonified_request['importDataFileId'] == 'import_data_file_id_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.create_import_data_file(request)
            expected_params = [('importDataFileId', ''), ('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_create_import_data_file_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.create_import_data_file._get_unset_required_fields({})
    assert set(unset_fields) == set(('importDataFileId', 'requestId')) & set(('parent', 'importDataFileId', 'importDataFile'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_create_import_data_file_rest_interceptors(null_interceptor):
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_create_import_data_file') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_create_import_data_file') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.CreateImportDataFileRequest.pb(migrationcenter.CreateImportDataFileRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.CreateImportDataFileRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.create_import_data_file(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_create_import_data_file_rest_bad_request(transport: str='rest', request_type=migrationcenter.CreateImportDataFileRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2/importJobs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.create_import_data_file(request)

def test_create_import_data_file_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'parent': 'projects/sample1/locations/sample2/importJobs/sample3'}
        mock_args = dict(parent='parent_value', import_data_file=migrationcenter.ImportDataFile(name='name_value'), import_data_file_id='import_data_file_id_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.create_import_data_file(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*/importJobs/*}/importDataFiles' % client.transport._host, args[1])

def test_create_import_data_file_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.create_import_data_file(migrationcenter.CreateImportDataFileRequest(), parent='parent_value', import_data_file=migrationcenter.ImportDataFile(name='name_value'), import_data_file_id='import_data_file_id_value')

def test_create_import_data_file_rest_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteImportDataFileRequest, dict])
def test_delete_import_data_file_rest(request_type):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3/importDataFiles/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.delete_import_data_file(request)
    assert response.operation.name == 'operations/spam'

def test_delete_import_data_file_rest_required_fields(request_type=migrationcenter.DeleteImportDataFileRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_import_data_file._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_import_data_file._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'delete', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.delete_import_data_file(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_delete_import_data_file_rest_unset_required_fields():
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.delete_import_data_file._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_delete_import_data_file_rest_interceptors(null_interceptor):
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_delete_import_data_file') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_delete_import_data_file') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.DeleteImportDataFileRequest.pb(migrationcenter.DeleteImportDataFileRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.DeleteImportDataFileRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.delete_import_data_file(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_delete_import_data_file_rest_bad_request(transport: str='rest', request_type=migrationcenter.DeleteImportDataFileRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/importJobs/sample3/importDataFiles/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.delete_import_data_file(request)

def test_delete_import_data_file_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'name': 'projects/sample1/locations/sample2/importJobs/sample3/importDataFiles/sample4'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.delete_import_data_file(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/importJobs/*/importDataFiles/*}' % client.transport._host, args[1])

def test_delete_import_data_file_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.delete_import_data_file(migrationcenter.DeleteImportDataFileRequest(), name='name_value')

def test_delete_import_data_file_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ListGroupsRequest, dict])
def test_list_groups_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListGroupsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListGroupsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_groups(request)
    assert isinstance(response, pagers.ListGroupsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_groups_rest_required_fields(request_type=migrationcenter.ListGroupsRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_groups._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_groups._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('filter', 'order_by', 'page_size', 'page_token'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ListGroupsResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ListGroupsResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.list_groups(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_list_groups_rest_unset_required_fields():
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.list_groups._get_unset_required_fields({})
    assert set(unset_fields) == set(('filter', 'orderBy', 'pageSize', 'pageToken')) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_list_groups_rest_interceptors(null_interceptor):
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_list_groups') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_list_groups') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ListGroupsRequest.pb(migrationcenter.ListGroupsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ListGroupsResponse.to_json(migrationcenter.ListGroupsResponse())
        request = migrationcenter.ListGroupsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ListGroupsResponse()
        client.list_groups(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_list_groups_rest_bad_request(transport: str='rest', request_type=migrationcenter.ListGroupsRequest):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_groups(request)

def test_list_groups_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListGroupsResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListGroupsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.list_groups(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/groups' % client.transport._host, args[1])

def test_list_groups_rest_flattened_error(transport: str='rest'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.list_groups(migrationcenter.ListGroupsRequest(), parent='parent_value')

def test_list_groups_rest_pager(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with mock.patch.object(Session, 'request') as req:
        response = (migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group(), migrationcenter.Group()], next_page_token='abc'), migrationcenter.ListGroupsResponse(groups=[], next_page_token='def'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group()], next_page_token='ghi'), migrationcenter.ListGroupsResponse(groups=[migrationcenter.Group(), migrationcenter.Group()]))
        response = response + response
        response = tuple((migrationcenter.ListGroupsResponse.to_json(x) for x in response))
        return_values = tuple((Response() for i in response))
        for (return_val, response_val) in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        req.side_effect = return_values
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        pager = client.list_groups(request=sample_request)
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.Group) for i in results))
        pages = list(client.list_groups(request=sample_request).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetGroupRequest, dict])
def test_get_group_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/groups/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Group(name='name_value', display_name='display_name_value', description='description_value')
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Group.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_group(request)
    assert isinstance(response, migrationcenter.Group)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'

def test_get_group_rest_required_fields(request_type=migrationcenter.GetGroupRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_group._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_group._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.Group()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.Group.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_group(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_group_rest_unset_required_fields():
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_group._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_group_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_group') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_group') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetGroupRequest.pb(migrationcenter.GetGroupRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.Group.to_json(migrationcenter.Group())
        request = migrationcenter.GetGroupRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.Group()
        client.get_group(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_group_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetGroupRequest):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/groups/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_group(request)

def test_get_group_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Group()
        sample_request = {'name': 'projects/sample1/locations/sample2/groups/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Group.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_group(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/groups/*}' % client.transport._host, args[1])

def test_get_group_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_group(migrationcenter.GetGroupRequest(), name='name_value')

def test_get_group_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.CreateGroupRequest, dict])
def test_create_group_rest(request_type):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init['group'] = {'name': 'name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'labels': {}, 'display_name': 'display_name_value', 'description': 'description_value'}
    test_field = migrationcenter.CreateGroupRequest.meta.fields['group']

    def get_message_fields(field):
        if False:
            i = 10
            return i + 15
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['group'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['group'][field])):
                    del request_init['group'][field][i][subfield]
            else:
                del request_init['group'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.create_group(request)
    assert response.operation.name == 'operations/spam'

def test_create_group_rest_required_fields(request_type=migrationcenter.CreateGroupRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request_init['group_id'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    assert 'groupId' not in jsonified_request
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_group._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'groupId' in jsonified_request
    assert jsonified_request['groupId'] == request_init['group_id']
    jsonified_request['parent'] = 'parent_value'
    jsonified_request['groupId'] = 'group_id_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_group._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('group_id', 'request_id'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    assert 'groupId' in jsonified_request
    assert jsonified_request['groupId'] == 'group_id_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.create_group(request)
            expected_params = [('groupId', ''), ('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_create_group_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.create_group._get_unset_required_fields({})
    assert set(unset_fields) == set(('groupId', 'requestId')) & set(('parent', 'groupId', 'group'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_create_group_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_create_group') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_create_group') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.CreateGroupRequest.pb(migrationcenter.CreateGroupRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.CreateGroupRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.create_group(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_create_group_rest_bad_request(transport: str='rest', request_type=migrationcenter.CreateGroupRequest):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.create_group(request)

def test_create_group_rest_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value', group=migrationcenter.Group(name='name_value'), group_id='group_id_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.create_group(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/groups' % client.transport._host, args[1])

def test_create_group_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.create_group(migrationcenter.CreateGroupRequest(), parent='parent_value', group=migrationcenter.Group(name='name_value'), group_id='group_id_value')

def test_create_group_rest_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateGroupRequest, dict])
def test_update_group_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'group': {'name': 'projects/sample1/locations/sample2/groups/sample3'}}
    request_init['group'] = {'name': 'projects/sample1/locations/sample2/groups/sample3', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'labels': {}, 'display_name': 'display_name_value', 'description': 'description_value'}
    test_field = migrationcenter.UpdateGroupRequest.meta.fields['group']

    def get_message_fields(field):
        if False:
            i = 10
            return i + 15
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['group'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['group'][field])):
                    del request_init['group'][field][i][subfield]
            else:
                del request_init['group'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.update_group(request)
    assert response.operation.name == 'operations/spam'

def test_update_group_rest_required_fields(request_type=migrationcenter.UpdateGroupRequest):
    if False:
        for i in range(10):
            print('nop')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_group._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_group._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id', 'update_mask'))
    jsonified_request.update(unset_fields)
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'patch', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.update_group(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_update_group_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.update_group._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId', 'updateMask')) & set(('updateMask', 'group'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_update_group_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_update_group') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_update_group') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.UpdateGroupRequest.pb(migrationcenter.UpdateGroupRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.UpdateGroupRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.update_group(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_update_group_rest_bad_request(transport: str='rest', request_type=migrationcenter.UpdateGroupRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'group': {'name': 'projects/sample1/locations/sample2/groups/sample3'}}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.update_group(request)

def test_update_group_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'group': {'name': 'projects/sample1/locations/sample2/groups/sample3'}}
        mock_args = dict(group=migrationcenter.Group(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.update_group(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{group.name=projects/*/locations/*/groups/*}' % client.transport._host, args[1])

def test_update_group_rest_flattened_error(transport: str='rest'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.update_group(migrationcenter.UpdateGroupRequest(), group=migrationcenter.Group(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

def test_update_group_rest_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteGroupRequest, dict])
def test_delete_group_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/groups/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.delete_group(request)
    assert response.operation.name == 'operations/spam'

def test_delete_group_rest_required_fields(request_type=migrationcenter.DeleteGroupRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_group._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_group._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'delete', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.delete_group(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_delete_group_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.delete_group._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_delete_group_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_delete_group') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_delete_group') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.DeleteGroupRequest.pb(migrationcenter.DeleteGroupRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.DeleteGroupRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.delete_group(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_delete_group_rest_bad_request(transport: str='rest', request_type=migrationcenter.DeleteGroupRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/groups/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.delete_group(request)

def test_delete_group_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'name': 'projects/sample1/locations/sample2/groups/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.delete_group(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/groups/*}' % client.transport._host, args[1])

def test_delete_group_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.delete_group(migrationcenter.DeleteGroupRequest(), name='name_value')

def test_delete_group_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.AddAssetsToGroupRequest, dict])
def test_add_assets_to_group_rest(request_type):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'group': 'projects/sample1/locations/sample2/groups/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.add_assets_to_group(request)
    assert response.operation.name == 'operations/spam'

def test_add_assets_to_group_rest_required_fields(request_type=migrationcenter.AddAssetsToGroupRequest):
    if False:
        return 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['group'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).add_assets_to_group._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['group'] = 'group_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).add_assets_to_group._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'group' in jsonified_request
    assert jsonified_request['group'] == 'group_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.add_assets_to_group(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_add_assets_to_group_rest_unset_required_fields():
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.add_assets_to_group._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('group', 'assets'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_add_assets_to_group_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_add_assets_to_group') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_add_assets_to_group') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.AddAssetsToGroupRequest.pb(migrationcenter.AddAssetsToGroupRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.AddAssetsToGroupRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.add_assets_to_group(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_add_assets_to_group_rest_bad_request(transport: str='rest', request_type=migrationcenter.AddAssetsToGroupRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'group': 'projects/sample1/locations/sample2/groups/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.add_assets_to_group(request)

def test_add_assets_to_group_rest_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'group': 'projects/sample1/locations/sample2/groups/sample3'}
        mock_args = dict(group='group_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.add_assets_to_group(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{group=projects/*/locations/*/groups/*}:addAssets' % client.transport._host, args[1])

def test_add_assets_to_group_rest_flattened_error(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.add_assets_to_group(migrationcenter.AddAssetsToGroupRequest(), group='group_value')

def test_add_assets_to_group_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.RemoveAssetsFromGroupRequest, dict])
def test_remove_assets_from_group_rest(request_type):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'group': 'projects/sample1/locations/sample2/groups/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.remove_assets_from_group(request)
    assert response.operation.name == 'operations/spam'

def test_remove_assets_from_group_rest_required_fields(request_type=migrationcenter.RemoveAssetsFromGroupRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['group'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).remove_assets_from_group._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['group'] = 'group_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).remove_assets_from_group._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'group' in jsonified_request
    assert jsonified_request['group'] == 'group_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.remove_assets_from_group(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_remove_assets_from_group_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.remove_assets_from_group._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('group', 'assets'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_remove_assets_from_group_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_remove_assets_from_group') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_remove_assets_from_group') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.RemoveAssetsFromGroupRequest.pb(migrationcenter.RemoveAssetsFromGroupRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.RemoveAssetsFromGroupRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.remove_assets_from_group(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_remove_assets_from_group_rest_bad_request(transport: str='rest', request_type=migrationcenter.RemoveAssetsFromGroupRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'group': 'projects/sample1/locations/sample2/groups/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.remove_assets_from_group(request)

def test_remove_assets_from_group_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'group': 'projects/sample1/locations/sample2/groups/sample3'}
        mock_args = dict(group='group_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.remove_assets_from_group(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{group=projects/*/locations/*/groups/*}:removeAssets' % client.transport._host, args[1])

def test_remove_assets_from_group_rest_flattened_error(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.remove_assets_from_group(migrationcenter.RemoveAssetsFromGroupRequest(), group='group_value')

def test_remove_assets_from_group_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ListErrorFramesRequest, dict])
def test_list_error_frames_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2/sources/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListErrorFramesResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListErrorFramesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_error_frames(request)
    assert isinstance(response, pagers.ListErrorFramesPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_error_frames_rest_required_fields(request_type=migrationcenter.ListErrorFramesRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_error_frames._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_error_frames._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('page_size', 'page_token', 'view'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ListErrorFramesResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ListErrorFramesResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.list_error_frames(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_list_error_frames_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.list_error_frames._get_unset_required_fields({})
    assert set(unset_fields) == set(('pageSize', 'pageToken', 'view')) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_list_error_frames_rest_interceptors(null_interceptor):
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_list_error_frames') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_list_error_frames') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ListErrorFramesRequest.pb(migrationcenter.ListErrorFramesRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ListErrorFramesResponse.to_json(migrationcenter.ListErrorFramesResponse())
        request = migrationcenter.ListErrorFramesRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ListErrorFramesResponse()
        client.list_error_frames(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_list_error_frames_rest_bad_request(transport: str='rest', request_type=migrationcenter.ListErrorFramesRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2/sources/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_error_frames(request)

def test_list_error_frames_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListErrorFramesResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2/sources/sample3'}
        mock_args = dict(parent='parent_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListErrorFramesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.list_error_frames(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*/sources/*}/errorFrames' % client.transport._host, args[1])

def test_list_error_frames_rest_flattened_error(transport: str='rest'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.list_error_frames(migrationcenter.ListErrorFramesRequest(), parent='parent_value')

def test_list_error_frames_rest_pager(transport: str='rest'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with mock.patch.object(Session, 'request') as req:
        response = (migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()], next_page_token='abc'), migrationcenter.ListErrorFramesResponse(error_frames=[], next_page_token='def'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame()], next_page_token='ghi'), migrationcenter.ListErrorFramesResponse(error_frames=[migrationcenter.ErrorFrame(), migrationcenter.ErrorFrame()]))
        response = response + response
        response = tuple((migrationcenter.ListErrorFramesResponse.to_json(x) for x in response))
        return_values = tuple((Response() for i in response))
        for (return_val, response_val) in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        req.side_effect = return_values
        sample_request = {'parent': 'projects/sample1/locations/sample2/sources/sample3'}
        pager = client.list_error_frames(request=sample_request)
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.ErrorFrame) for i in results))
        pages = list(client.list_error_frames(request=sample_request).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetErrorFrameRequest, dict])
def test_get_error_frame_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/sources/sample3/errorFrames/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ErrorFrame(name='name_value')
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ErrorFrame.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_error_frame(request)
    assert isinstance(response, migrationcenter.ErrorFrame)
    assert response.name == 'name_value'

def test_get_error_frame_rest_required_fields(request_type=migrationcenter.GetErrorFrameRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_error_frame._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_error_frame._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('view',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ErrorFrame()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ErrorFrame.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_error_frame(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_error_frame_rest_unset_required_fields():
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_error_frame._get_unset_required_fields({})
    assert set(unset_fields) == set(('view',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_error_frame_rest_interceptors(null_interceptor):
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_error_frame') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_error_frame') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetErrorFrameRequest.pb(migrationcenter.GetErrorFrameRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ErrorFrame.to_json(migrationcenter.ErrorFrame())
        request = migrationcenter.GetErrorFrameRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ErrorFrame()
        client.get_error_frame(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_error_frame_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetErrorFrameRequest):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/sources/sample3/errorFrames/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_error_frame(request)

def test_get_error_frame_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ErrorFrame()
        sample_request = {'name': 'projects/sample1/locations/sample2/sources/sample3/errorFrames/sample4'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ErrorFrame.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_error_frame(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/sources/*/errorFrames/*}' % client.transport._host, args[1])

def test_get_error_frame_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_error_frame(migrationcenter.GetErrorFrameRequest(), name='name_value')

def test_get_error_frame_rest_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ListSourcesRequest, dict])
def test_list_sources_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListSourcesResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListSourcesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_sources(request)
    assert isinstance(response, pagers.ListSourcesPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_sources_rest_required_fields(request_type=migrationcenter.ListSourcesRequest):
    if False:
        i = 10
        return i + 15
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_sources._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_sources._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('filter', 'order_by', 'page_size', 'page_token'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ListSourcesResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ListSourcesResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.list_sources(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_list_sources_rest_unset_required_fields():
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.list_sources._get_unset_required_fields({})
    assert set(unset_fields) == set(('filter', 'orderBy', 'pageSize', 'pageToken')) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_list_sources_rest_interceptors(null_interceptor):
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_list_sources') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_list_sources') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ListSourcesRequest.pb(migrationcenter.ListSourcesRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ListSourcesResponse.to_json(migrationcenter.ListSourcesResponse())
        request = migrationcenter.ListSourcesRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ListSourcesResponse()
        client.list_sources(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_list_sources_rest_bad_request(transport: str='rest', request_type=migrationcenter.ListSourcesRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_sources(request)

def test_list_sources_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListSourcesResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListSourcesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.list_sources(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/sources' % client.transport._host, args[1])

def test_list_sources_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.list_sources(migrationcenter.ListSourcesRequest(), parent='parent_value')

def test_list_sources_rest_pager(transport: str='rest'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with mock.patch.object(Session, 'request') as req:
        response = (migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source(), migrationcenter.Source()], next_page_token='abc'), migrationcenter.ListSourcesResponse(sources=[], next_page_token='def'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source()], next_page_token='ghi'), migrationcenter.ListSourcesResponse(sources=[migrationcenter.Source(), migrationcenter.Source()]))
        response = response + response
        response = tuple((migrationcenter.ListSourcesResponse.to_json(x) for x in response))
        return_values = tuple((Response() for i in response))
        for (return_val, response_val) in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        req.side_effect = return_values
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        pager = client.list_sources(request=sample_request)
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.Source) for i in results))
        pages = list(client.list_sources(request=sample_request).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetSourceRequest, dict])
def test_get_source_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/sources/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Source(name='name_value', display_name='display_name_value', description='description_value', type_=migrationcenter.Source.SourceType.SOURCE_TYPE_UPLOAD, priority=898, managed=True, pending_frame_count=2007, error_frame_count=1820, state=migrationcenter.Source.State.ACTIVE)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Source.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_source(request)
    assert isinstance(response, migrationcenter.Source)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'
    assert response.type_ == migrationcenter.Source.SourceType.SOURCE_TYPE_UPLOAD
    assert response.priority == 898
    assert response.managed is True
    assert response.pending_frame_count == 2007
    assert response.error_frame_count == 1820
    assert response.state == migrationcenter.Source.State.ACTIVE

def test_get_source_rest_required_fields(request_type=migrationcenter.GetSourceRequest):
    if False:
        return 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_source._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_source._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.Source()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.Source.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_source(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_source_rest_unset_required_fields():
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_source._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_source_rest_interceptors(null_interceptor):
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_source') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_source') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetSourceRequest.pb(migrationcenter.GetSourceRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.Source.to_json(migrationcenter.Source())
        request = migrationcenter.GetSourceRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.Source()
        client.get_source(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_source_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetSourceRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/sources/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_source(request)

def test_get_source_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Source()
        sample_request = {'name': 'projects/sample1/locations/sample2/sources/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Source.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_source(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/sources/*}' % client.transport._host, args[1])

def test_get_source_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_source(migrationcenter.GetSourceRequest(), name='name_value')

def test_get_source_rest_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.CreateSourceRequest, dict])
def test_create_source_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init['source'] = {'name': 'name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'display_name': 'display_name_value', 'description': 'description_value', 'type_': 1, 'priority': 898, 'managed': True, 'pending_frame_count': 2007, 'error_frame_count': 1820, 'state': 1}
    test_field = migrationcenter.CreateSourceRequest.meta.fields['source']

    def get_message_fields(field):
        if False:
            print('Hello World!')
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['source'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['source'][field])):
                    del request_init['source'][field][i][subfield]
            else:
                del request_init['source'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.create_source(request)
    assert response.operation.name == 'operations/spam'

def test_create_source_rest_required_fields(request_type=migrationcenter.CreateSourceRequest):
    if False:
        for i in range(10):
            print('nop')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request_init['source_id'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    assert 'sourceId' not in jsonified_request
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_source._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'sourceId' in jsonified_request
    assert jsonified_request['sourceId'] == request_init['source_id']
    jsonified_request['parent'] = 'parent_value'
    jsonified_request['sourceId'] = 'source_id_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_source._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id', 'source_id'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    assert 'sourceId' in jsonified_request
    assert jsonified_request['sourceId'] == 'source_id_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.create_source(request)
            expected_params = [('sourceId', ''), ('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_create_source_rest_unset_required_fields():
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.create_source._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId', 'sourceId')) & set(('parent', 'sourceId', 'source'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_create_source_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_create_source') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_create_source') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.CreateSourceRequest.pb(migrationcenter.CreateSourceRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.CreateSourceRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.create_source(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_create_source_rest_bad_request(transport: str='rest', request_type=migrationcenter.CreateSourceRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.create_source(request)

def test_create_source_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value', source=migrationcenter.Source(name='name_value'), source_id='source_id_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.create_source(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/sources' % client.transport._host, args[1])

def test_create_source_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.create_source(migrationcenter.CreateSourceRequest(), parent='parent_value', source=migrationcenter.Source(name='name_value'), source_id='source_id_value')

def test_create_source_rest_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateSourceRequest, dict])
def test_update_source_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'source': {'name': 'projects/sample1/locations/sample2/sources/sample3'}}
    request_init['source'] = {'name': 'projects/sample1/locations/sample2/sources/sample3', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'display_name': 'display_name_value', 'description': 'description_value', 'type_': 1, 'priority': 898, 'managed': True, 'pending_frame_count': 2007, 'error_frame_count': 1820, 'state': 1}
    test_field = migrationcenter.UpdateSourceRequest.meta.fields['source']

    def get_message_fields(field):
        if False:
            return 10
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['source'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['source'][field])):
                    del request_init['source'][field][i][subfield]
            else:
                del request_init['source'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.update_source(request)
    assert response.operation.name == 'operations/spam'

def test_update_source_rest_required_fields(request_type=migrationcenter.UpdateSourceRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_source._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_source._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id', 'update_mask'))
    jsonified_request.update(unset_fields)
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'patch', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.update_source(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_update_source_rest_unset_required_fields():
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.update_source._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId', 'updateMask')) & set(('updateMask', 'source'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_update_source_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_update_source') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_update_source') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.UpdateSourceRequest.pb(migrationcenter.UpdateSourceRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.UpdateSourceRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.update_source(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_update_source_rest_bad_request(transport: str='rest', request_type=migrationcenter.UpdateSourceRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'source': {'name': 'projects/sample1/locations/sample2/sources/sample3'}}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.update_source(request)

def test_update_source_rest_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'source': {'name': 'projects/sample1/locations/sample2/sources/sample3'}}
        mock_args = dict(source=migrationcenter.Source(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.update_source(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{source.name=projects/*/locations/*/sources/*}' % client.transport._host, args[1])

def test_update_source_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.update_source(migrationcenter.UpdateSourceRequest(), source=migrationcenter.Source(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

def test_update_source_rest_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteSourceRequest, dict])
def test_delete_source_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/sources/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.delete_source(request)
    assert response.operation.name == 'operations/spam'

def test_delete_source_rest_required_fields(request_type=migrationcenter.DeleteSourceRequest):
    if False:
        return 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_source._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_source._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'delete', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.delete_source(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_delete_source_rest_unset_required_fields():
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.delete_source._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_delete_source_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_delete_source') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_delete_source') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.DeleteSourceRequest.pb(migrationcenter.DeleteSourceRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.DeleteSourceRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.delete_source(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_delete_source_rest_bad_request(transport: str='rest', request_type=migrationcenter.DeleteSourceRequest):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/sources/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.delete_source(request)

def test_delete_source_rest_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'name': 'projects/sample1/locations/sample2/sources/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.delete_source(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/sources/*}' % client.transport._host, args[1])

def test_delete_source_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.delete_source(migrationcenter.DeleteSourceRequest(), name='name_value')

def test_delete_source_rest_error():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ListPreferenceSetsRequest, dict])
def test_list_preference_sets_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListPreferenceSetsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListPreferenceSetsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_preference_sets(request)
    assert isinstance(response, pagers.ListPreferenceSetsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_preference_sets_rest_required_fields(request_type=migrationcenter.ListPreferenceSetsRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_preference_sets._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_preference_sets._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('order_by', 'page_size', 'page_token'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ListPreferenceSetsResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ListPreferenceSetsResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.list_preference_sets(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_list_preference_sets_rest_unset_required_fields():
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.list_preference_sets._get_unset_required_fields({})
    assert set(unset_fields) == set(('orderBy', 'pageSize', 'pageToken')) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_list_preference_sets_rest_interceptors(null_interceptor):
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_list_preference_sets') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_list_preference_sets') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ListPreferenceSetsRequest.pb(migrationcenter.ListPreferenceSetsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ListPreferenceSetsResponse.to_json(migrationcenter.ListPreferenceSetsResponse())
        request = migrationcenter.ListPreferenceSetsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ListPreferenceSetsResponse()
        client.list_preference_sets(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_list_preference_sets_rest_bad_request(transport: str='rest', request_type=migrationcenter.ListPreferenceSetsRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_preference_sets(request)

def test_list_preference_sets_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListPreferenceSetsResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListPreferenceSetsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.list_preference_sets(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/preferenceSets' % client.transport._host, args[1])

def test_list_preference_sets_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.list_preference_sets(migrationcenter.ListPreferenceSetsRequest(), parent='parent_value')

def test_list_preference_sets_rest_pager(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with mock.patch.object(Session, 'request') as req:
        response = (migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()], next_page_token='abc'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[], next_page_token='def'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet()], next_page_token='ghi'), migrationcenter.ListPreferenceSetsResponse(preference_sets=[migrationcenter.PreferenceSet(), migrationcenter.PreferenceSet()]))
        response = response + response
        response = tuple((migrationcenter.ListPreferenceSetsResponse.to_json(x) for x in response))
        return_values = tuple((Response() for i in response))
        for (return_val, response_val) in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        req.side_effect = return_values
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        pager = client.list_preference_sets(request=sample_request)
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.PreferenceSet) for i in results))
        pages = list(client.list_preference_sets(request=sample_request).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.GetPreferenceSetRequest, dict])
def test_get_preference_set_rest(request_type):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.PreferenceSet(name='name_value', display_name='display_name_value', description='description_value')
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.PreferenceSet.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_preference_set(request)
    assert isinstance(response, migrationcenter.PreferenceSet)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'

def test_get_preference_set_rest_required_fields(request_type=migrationcenter.GetPreferenceSetRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_preference_set._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_preference_set._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.PreferenceSet()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.PreferenceSet.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_preference_set(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_preference_set_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_preference_set._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_preference_set_rest_interceptors(null_interceptor):
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_preference_set') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_preference_set') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetPreferenceSetRequest.pb(migrationcenter.GetPreferenceSetRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.PreferenceSet.to_json(migrationcenter.PreferenceSet())
        request = migrationcenter.GetPreferenceSetRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.PreferenceSet()
        client.get_preference_set(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_preference_set_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetPreferenceSetRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_preference_set(request)

def test_get_preference_set_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.PreferenceSet()
        sample_request = {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.PreferenceSet.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_preference_set(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/preferenceSets/*}' % client.transport._host, args[1])

def test_get_preference_set_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_preference_set(migrationcenter.GetPreferenceSetRequest(), name='name_value')

def test_get_preference_set_rest_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.CreatePreferenceSetRequest, dict])
def test_create_preference_set_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init['preference_set'] = {'name': 'name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'display_name': 'display_name_value', 'description': 'description_value', 'virtual_machine_preferences': {'target_product': 1, 'region_preferences': {'preferred_regions': ['preferred_regions_value1', 'preferred_regions_value2']}, 'commitment_plan': 1, 'sizing_optimization_strategy': 1, 'compute_engine_preferences': {'machine_preferences': {'allowed_machine_series': [{'code': 'code_value'}]}, 'license_type': 1}, 'vmware_engine_preferences': {'cpu_overcommit_ratio': 0.2154, 'memory_overcommit_ratio': 0.24910000000000002, 'storage_deduplication_compression_ratio': 0.4168, 'commitment_plan': 1}, 'sole_tenancy_preferences': {'cpu_overcommit_ratio': 0.2154, 'host_maintenance_policy': 1, 'commitment_plan': 1, 'node_types': [{'node_name': 'node_name_value'}]}}}
    test_field = migrationcenter.CreatePreferenceSetRequest.meta.fields['preference_set']

    def get_message_fields(field):
        if False:
            while True:
                i = 10
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['preference_set'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['preference_set'][field])):
                    del request_init['preference_set'][field][i][subfield]
            else:
                del request_init['preference_set'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.create_preference_set(request)
    assert response.operation.name == 'operations/spam'

def test_create_preference_set_rest_required_fields(request_type=migrationcenter.CreatePreferenceSetRequest):
    if False:
        i = 10
        return i + 15
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request_init['preference_set_id'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    assert 'preferenceSetId' not in jsonified_request
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_preference_set._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'preferenceSetId' in jsonified_request
    assert jsonified_request['preferenceSetId'] == request_init['preference_set_id']
    jsonified_request['parent'] = 'parent_value'
    jsonified_request['preferenceSetId'] = 'preference_set_id_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_preference_set._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('preference_set_id', 'request_id'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    assert 'preferenceSetId' in jsonified_request
    assert jsonified_request['preferenceSetId'] == 'preference_set_id_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.create_preference_set(request)
            expected_params = [('preferenceSetId', ''), ('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_create_preference_set_rest_unset_required_fields():
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.create_preference_set._get_unset_required_fields({})
    assert set(unset_fields) == set(('preferenceSetId', 'requestId')) & set(('parent', 'preferenceSetId', 'preferenceSet'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_create_preference_set_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_create_preference_set') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_create_preference_set') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.CreatePreferenceSetRequest.pb(migrationcenter.CreatePreferenceSetRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.CreatePreferenceSetRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.create_preference_set(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_create_preference_set_rest_bad_request(transport: str='rest', request_type=migrationcenter.CreatePreferenceSetRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.create_preference_set(request)

def test_create_preference_set_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value', preference_set=migrationcenter.PreferenceSet(name='name_value'), preference_set_id='preference_set_id_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.create_preference_set(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/preferenceSets' % client.transport._host, args[1])

def test_create_preference_set_rest_flattened_error(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.create_preference_set(migrationcenter.CreatePreferenceSetRequest(), parent='parent_value', preference_set=migrationcenter.PreferenceSet(name='name_value'), preference_set_id='preference_set_id_value')

def test_create_preference_set_rest_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdatePreferenceSetRequest, dict])
def test_update_preference_set_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'preference_set': {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3'}}
    request_init['preference_set'] = {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'display_name': 'display_name_value', 'description': 'description_value', 'virtual_machine_preferences': {'target_product': 1, 'region_preferences': {'preferred_regions': ['preferred_regions_value1', 'preferred_regions_value2']}, 'commitment_plan': 1, 'sizing_optimization_strategy': 1, 'compute_engine_preferences': {'machine_preferences': {'allowed_machine_series': [{'code': 'code_value'}]}, 'license_type': 1}, 'vmware_engine_preferences': {'cpu_overcommit_ratio': 0.2154, 'memory_overcommit_ratio': 0.24910000000000002, 'storage_deduplication_compression_ratio': 0.4168, 'commitment_plan': 1}, 'sole_tenancy_preferences': {'cpu_overcommit_ratio': 0.2154, 'host_maintenance_policy': 1, 'commitment_plan': 1, 'node_types': [{'node_name': 'node_name_value'}]}}}
    test_field = migrationcenter.UpdatePreferenceSetRequest.meta.fields['preference_set']

    def get_message_fields(field):
        if False:
            return 10
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['preference_set'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['preference_set'][field])):
                    del request_init['preference_set'][field][i][subfield]
            else:
                del request_init['preference_set'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.update_preference_set(request)
    assert response.operation.name == 'operations/spam'

def test_update_preference_set_rest_required_fields(request_type=migrationcenter.UpdatePreferenceSetRequest):
    if False:
        for i in range(10):
            print('nop')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_preference_set._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_preference_set._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id', 'update_mask'))
    jsonified_request.update(unset_fields)
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'patch', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.update_preference_set(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_update_preference_set_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.update_preference_set._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId', 'updateMask')) & set(('updateMask', 'preferenceSet'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_update_preference_set_rest_interceptors(null_interceptor):
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_update_preference_set') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_update_preference_set') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.UpdatePreferenceSetRequest.pb(migrationcenter.UpdatePreferenceSetRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.UpdatePreferenceSetRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.update_preference_set(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_update_preference_set_rest_bad_request(transport: str='rest', request_type=migrationcenter.UpdatePreferenceSetRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'preference_set': {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3'}}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.update_preference_set(request)

def test_update_preference_set_rest_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'preference_set': {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3'}}
        mock_args = dict(preference_set=migrationcenter.PreferenceSet(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.update_preference_set(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{preference_set.name=projects/*/locations/*/preferenceSets/*}' % client.transport._host, args[1])

def test_update_preference_set_rest_flattened_error(transport: str='rest'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.update_preference_set(migrationcenter.UpdatePreferenceSetRequest(), preference_set=migrationcenter.PreferenceSet(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

def test_update_preference_set_rest_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.DeletePreferenceSetRequest, dict])
def test_delete_preference_set_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.delete_preference_set(request)
    assert response.operation.name == 'operations/spam'

def test_delete_preference_set_rest_required_fields(request_type=migrationcenter.DeletePreferenceSetRequest):
    if False:
        i = 10
        return i + 15
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_preference_set._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_preference_set._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'delete', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.delete_preference_set(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_delete_preference_set_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.delete_preference_set._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_delete_preference_set_rest_interceptors(null_interceptor):
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_delete_preference_set') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_delete_preference_set') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.DeletePreferenceSetRequest.pb(migrationcenter.DeletePreferenceSetRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.DeletePreferenceSetRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.delete_preference_set(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_delete_preference_set_rest_bad_request(transport: str='rest', request_type=migrationcenter.DeletePreferenceSetRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.delete_preference_set(request)

def test_delete_preference_set_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'name': 'projects/sample1/locations/sample2/preferenceSets/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.delete_preference_set(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/preferenceSets/*}' % client.transport._host, args[1])

def test_delete_preference_set_rest_flattened_error(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.delete_preference_set(migrationcenter.DeletePreferenceSetRequest(), name='name_value')

def test_delete_preference_set_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.GetSettingsRequest, dict])
def test_get_settings_rest(request_type):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/settings'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Settings(name='name_value', preference_set='preference_set_value')
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Settings.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_settings(request)
    assert isinstance(response, migrationcenter.Settings)
    assert response.name == 'name_value'
    assert response.preference_set == 'preference_set_value'

def test_get_settings_rest_required_fields(request_type=migrationcenter.GetSettingsRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_settings._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_settings._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.Settings()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.Settings.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_settings(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_settings_rest_unset_required_fields():
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_settings._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_settings_rest_interceptors(null_interceptor):
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_settings') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_settings') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetSettingsRequest.pb(migrationcenter.GetSettingsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.Settings.to_json(migrationcenter.Settings())
        request = migrationcenter.GetSettingsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.Settings()
        client.get_settings(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_settings_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetSettingsRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/settings'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_settings(request)

def test_get_settings_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Settings()
        sample_request = {'name': 'projects/sample1/locations/sample2/settings'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Settings.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_settings(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/settings}' % client.transport._host, args[1])

def test_get_settings_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_settings(migrationcenter.GetSettingsRequest(), name='name_value')

def test_get_settings_rest_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.UpdateSettingsRequest, dict])
def test_update_settings_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'settings': {'name': 'projects/sample1/locations/sample2/settings'}}
    request_init['settings'] = {'name': 'projects/sample1/locations/sample2/settings', 'preference_set': 'preference_set_value'}
    test_field = migrationcenter.UpdateSettingsRequest.meta.fields['settings']

    def get_message_fields(field):
        if False:
            for i in range(10):
                print('nop')
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['settings'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['settings'][field])):
                    del request_init['settings'][field][i][subfield]
            else:
                del request_init['settings'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.update_settings(request)
    assert response.operation.name == 'operations/spam'

def test_update_settings_rest_required_fields(request_type=migrationcenter.UpdateSettingsRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_settings._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).update_settings._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id', 'update_mask'))
    jsonified_request.update(unset_fields)
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'patch', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.update_settings(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_update_settings_rest_unset_required_fields():
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.update_settings._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId', 'updateMask')) & set(('updateMask', 'settings'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_update_settings_rest_interceptors(null_interceptor):
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_update_settings') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_update_settings') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.UpdateSettingsRequest.pb(migrationcenter.UpdateSettingsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.UpdateSettingsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.update_settings(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_update_settings_rest_bad_request(transport: str='rest', request_type=migrationcenter.UpdateSettingsRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'settings': {'name': 'projects/sample1/locations/sample2/settings'}}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.update_settings(request)

def test_update_settings_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'settings': {'name': 'projects/sample1/locations/sample2/settings'}}
        mock_args = dict(settings=migrationcenter.Settings(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.update_settings(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{settings.name=projects/*/locations/*/settings}' % client.transport._host, args[1])

def test_update_settings_rest_flattened_error(transport: str='rest'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.update_settings(migrationcenter.UpdateSettingsRequest(), settings=migrationcenter.Settings(name='name_value'), update_mask=field_mask_pb2.FieldMask(paths=['paths_value']))

def test_update_settings_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.CreateReportConfigRequest, dict])
def test_create_report_config_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request_init['report_config'] = {'name': 'name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'display_name': 'display_name_value', 'description': 'description_value', 'group_preferenceset_assignments': [{'group': 'group_value', 'preference_set': 'preference_set_value'}]}
    test_field = migrationcenter.CreateReportConfigRequest.meta.fields['report_config']

    def get_message_fields(field):
        if False:
            return 10
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['report_config'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['report_config'][field])):
                    del request_init['report_config'][field][i][subfield]
            else:
                del request_init['report_config'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.create_report_config(request)
    assert response.operation.name == 'operations/spam'

def test_create_report_config_rest_required_fields(request_type=migrationcenter.CreateReportConfigRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request_init['report_config_id'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    assert 'reportConfigId' not in jsonified_request
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_report_config._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'reportConfigId' in jsonified_request
    assert jsonified_request['reportConfigId'] == request_init['report_config_id']
    jsonified_request['parent'] = 'parent_value'
    jsonified_request['reportConfigId'] = 'report_config_id_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_report_config._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('report_config_id', 'request_id'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    assert 'reportConfigId' in jsonified_request
    assert jsonified_request['reportConfigId'] == 'report_config_id_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.create_report_config(request)
            expected_params = [('reportConfigId', ''), ('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_create_report_config_rest_unset_required_fields():
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.create_report_config._get_unset_required_fields({})
    assert set(unset_fields) == set(('reportConfigId', 'requestId')) & set(('parent', 'reportConfigId', 'reportConfig'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_create_report_config_rest_interceptors(null_interceptor):
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_create_report_config') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_create_report_config') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.CreateReportConfigRequest.pb(migrationcenter.CreateReportConfigRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.CreateReportConfigRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.create_report_config(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_create_report_config_rest_bad_request(transport: str='rest', request_type=migrationcenter.CreateReportConfigRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.create_report_config(request)

def test_create_report_config_rest_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value', report_config=migrationcenter.ReportConfig(name='name_value'), report_config_id='report_config_id_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.create_report_config(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/reportConfigs' % client.transport._host, args[1])

def test_create_report_config_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.create_report_config(migrationcenter.CreateReportConfigRequest(), parent='parent_value', report_config=migrationcenter.ReportConfig(name='name_value'), report_config_id='report_config_id_value')

def test_create_report_config_rest_error():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.GetReportConfigRequest, dict])
def test_get_report_config_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ReportConfig(name='name_value', display_name='display_name_value', description='description_value')
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ReportConfig.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_report_config(request)
    assert isinstance(response, migrationcenter.ReportConfig)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'

def test_get_report_config_rest_required_fields(request_type=migrationcenter.GetReportConfigRequest):
    if False:
        return 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_report_config._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_report_config._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ReportConfig()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ReportConfig.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_report_config(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_report_config_rest_unset_required_fields():
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_report_config._get_unset_required_fields({})
    assert set(unset_fields) == set(()) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_report_config_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_report_config') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_report_config') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetReportConfigRequest.pb(migrationcenter.GetReportConfigRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ReportConfig.to_json(migrationcenter.ReportConfig())
        request = migrationcenter.GetReportConfigRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ReportConfig()
        client.get_report_config(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_report_config_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetReportConfigRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_report_config(request)

def test_get_report_config_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ReportConfig()
        sample_request = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ReportConfig.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_report_config(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/reportConfigs/*}' % client.transport._host, args[1])

def test_get_report_config_rest_flattened_error(transport: str='rest'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_report_config(migrationcenter.GetReportConfigRequest(), name='name_value')

def test_get_report_config_rest_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ListReportConfigsRequest, dict])
def test_list_report_configs_rest(request_type):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListReportConfigsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListReportConfigsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_report_configs(request)
    assert isinstance(response, pagers.ListReportConfigsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_report_configs_rest_required_fields(request_type=migrationcenter.ListReportConfigsRequest):
    if False:
        i = 10
        return i + 15
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_report_configs._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_report_configs._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('filter', 'order_by', 'page_size', 'page_token'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ListReportConfigsResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ListReportConfigsResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.list_report_configs(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_list_report_configs_rest_unset_required_fields():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.list_report_configs._get_unset_required_fields({})
    assert set(unset_fields) == set(('filter', 'orderBy', 'pageSize', 'pageToken')) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_list_report_configs_rest_interceptors(null_interceptor):
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_list_report_configs') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_list_report_configs') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ListReportConfigsRequest.pb(migrationcenter.ListReportConfigsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ListReportConfigsResponse.to_json(migrationcenter.ListReportConfigsResponse())
        request = migrationcenter.ListReportConfigsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ListReportConfigsResponse()
        client.list_report_configs(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_list_report_configs_rest_bad_request(transport: str='rest', request_type=migrationcenter.ListReportConfigsRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_report_configs(request)

def test_list_report_configs_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListReportConfigsResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        mock_args = dict(parent='parent_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListReportConfigsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.list_report_configs(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*}/reportConfigs' % client.transport._host, args[1])

def test_list_report_configs_rest_flattened_error(transport: str='rest'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.list_report_configs(migrationcenter.ListReportConfigsRequest(), parent='parent_value')

def test_list_report_configs_rest_pager(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with mock.patch.object(Session, 'request') as req:
        response = (migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig(), migrationcenter.ReportConfig()], next_page_token='abc'), migrationcenter.ListReportConfigsResponse(report_configs=[], next_page_token='def'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig()], next_page_token='ghi'), migrationcenter.ListReportConfigsResponse(report_configs=[migrationcenter.ReportConfig(), migrationcenter.ReportConfig()]))
        response = response + response
        response = tuple((migrationcenter.ListReportConfigsResponse.to_json(x) for x in response))
        return_values = tuple((Response() for i in response))
        for (return_val, response_val) in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        req.side_effect = return_values
        sample_request = {'parent': 'projects/sample1/locations/sample2'}
        pager = client.list_report_configs(request=sample_request)
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.ReportConfig) for i in results))
        pages = list(client.list_report_configs(request=sample_request).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteReportConfigRequest, dict])
def test_delete_report_config_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.delete_report_config(request)
    assert response.operation.name == 'operations/spam'

def test_delete_report_config_rest_required_fields(request_type=migrationcenter.DeleteReportConfigRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_report_config._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_report_config._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('force', 'request_id'))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'delete', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.delete_report_config(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_delete_report_config_rest_unset_required_fields():
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.delete_report_config._get_unset_required_fields({})
    assert set(unset_fields) == set(('force', 'requestId')) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_delete_report_config_rest_interceptors(null_interceptor):
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_delete_report_config') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_delete_report_config') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.DeleteReportConfigRequest.pb(migrationcenter.DeleteReportConfigRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.DeleteReportConfigRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.delete_report_config(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_delete_report_config_rest_bad_request(transport: str='rest', request_type=migrationcenter.DeleteReportConfigRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.delete_report_config(request)

def test_delete_report_config_rest_flattened():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.delete_report_config(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/reportConfigs/*}' % client.transport._host, args[1])

def test_delete_report_config_rest_flattened_error(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.delete_report_config(migrationcenter.DeleteReportConfigRequest(), name='name_value')

def test_delete_report_config_rest_error():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.CreateReportRequest, dict])
def test_create_report_rest(request_type):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
    request_init['report'] = {'name': 'name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'display_name': 'display_name_value', 'description': 'description_value', 'type_': 1, 'state': 1, 'summary': {'all_assets_stats': {'total_memory_bytes': 1954, 'total_storage_bytes': 2046, 'total_cores': 1183, 'total_assets': 1302, 'memory_utilization_chart': {'used': 433, 'free': 418}, 'storage_utilization_chart': {}, 'operating_system': {'data_points': [{'label': 'label_value', 'value': 0.541}]}, 'core_count_histogram': {'buckets': [{'lower_bound': 1184, 'upper_bound': 1187, 'count': 553}]}, 'memory_bytes_histogram': {}, 'storage_bytes_histogram': {}}, 'group_findings': [{'display_name': 'display_name_value', 'description': 'description_value', 'asset_aggregate_stats': {}, 'overlapping_asset_count': 2478, 'preference_set_findings': [{'display_name': 'display_name_value', 'description': 'description_value', 'machine_preferences': {'target_product': 1, 'region_preferences': {'preferred_regions': ['preferred_regions_value1', 'preferred_regions_value2']}, 'commitment_plan': 1, 'sizing_optimization_strategy': 1, 'compute_engine_preferences': {'machine_preferences': {'allowed_machine_series': [{'code': 'code_value'}]}, 'license_type': 1}, 'vmware_engine_preferences': {'cpu_overcommit_ratio': 0.2154, 'memory_overcommit_ratio': 0.24910000000000002, 'storage_deduplication_compression_ratio': 0.4168, 'commitment_plan': 1}, 'sole_tenancy_preferences': {'cpu_overcommit_ratio': 0.2154, 'host_maintenance_policy': 1, 'commitment_plan': 1, 'node_types': [{'node_name': 'node_name_value'}]}}, 'monthly_cost_total': {'currency_code': 'currency_code_value', 'units': 563, 'nanos': 543}, 'monthly_cost_compute': {}, 'monthly_cost_os_license': {}, 'monthly_cost_network_egress': {}, 'monthly_cost_storage': {}, 'monthly_cost_other': {}, 'compute_engine_finding': {'allocated_regions': ['allocated_regions_value1', 'allocated_regions_value2'], 'allocated_asset_count': 2224, 'machine_series_allocations': [{'machine_series': {}, 'allocated_asset_count': 2224}], 'allocated_disk_types': [1]}, 'vmware_engine_finding': {'allocated_regions': ['allocated_regions_value1', 'allocated_regions_value2'], 'allocated_asset_count': 2224, 'node_allocations': [{'vmware_node': {'code': 'code_value'}, 'node_count': 1070, 'allocated_asset_count': 2224}]}, 'sole_tenant_finding': {'allocated_regions': ['allocated_regions_value1', 'allocated_regions_value2'], 'allocated_asset_count': 2224, 'node_allocations': [{'node': {}, 'node_count': 1070, 'allocated_asset_count': 2224}]}}]}]}}
    test_field = migrationcenter.CreateReportRequest.meta.fields['report']

    def get_message_fields(field):
        if False:
            for i in range(10):
                print('nop')
        message_fields = []
        if hasattr(field, 'message') and field.message:
            is_field_type_proto_plus_type = not hasattr(field.message, 'DESCRIPTOR')
            if is_field_type_proto_plus_type:
                message_fields = field.message.meta.fields.values()
            else:
                message_fields = field.message.DESCRIPTOR.fields
        return message_fields
    runtime_nested_fields = [(field.name, nested_field.name) for field in get_message_fields(test_field) for nested_field in get_message_fields(field)]
    subfields_not_in_runtime = []
    for (field, value) in request_init['report'].items():
        result = None
        is_repeated = False
        if isinstance(value, list) and len(value):
            is_repeated = True
            result = value[0]
        if isinstance(value, dict):
            result = value
        if result and hasattr(result, 'keys'):
            for subfield in result.keys():
                if (field, subfield) not in runtime_nested_fields:
                    subfields_not_in_runtime.append({'field': field, 'subfield': subfield, 'is_repeated': is_repeated})
    for subfield_to_delete in subfields_not_in_runtime:
        field = subfield_to_delete.get('field')
        field_repeated = subfield_to_delete.get('is_repeated')
        subfield = subfield_to_delete.get('subfield')
        if subfield:
            if field_repeated:
                for i in range(0, len(request_init['report'][field])):
                    del request_init['report'][field][i][subfield]
            else:
                del request_init['report'][field][subfield]
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.create_report(request)
    assert response.operation.name == 'operations/spam'

def test_create_report_rest_required_fields(request_type=migrationcenter.CreateReportRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request_init['report_id'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    assert 'reportId' not in jsonified_request
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_report._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    assert 'reportId' in jsonified_request
    assert jsonified_request['reportId'] == request_init['report_id']
    jsonified_request['parent'] = 'parent_value'
    jsonified_request['reportId'] = 'report_id_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).create_report._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('report_id', 'request_id'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    assert 'reportId' in jsonified_request
    assert jsonified_request['reportId'] == 'report_id_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'post', 'query_params': pb_request}
            transcode_result['body'] = pb_request
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.create_report(request)
            expected_params = [('reportId', ''), ('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_create_report_rest_unset_required_fields():
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.create_report._get_unset_required_fields({})
    assert set(unset_fields) == set(('reportId', 'requestId')) & set(('parent', 'reportId', 'report'))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_create_report_rest_interceptors(null_interceptor):
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_create_report') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_create_report') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.CreateReportRequest.pb(migrationcenter.CreateReportRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.CreateReportRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.create_report(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_create_report_rest_bad_request(transport: str='rest', request_type=migrationcenter.CreateReportRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.create_report(request)

def test_create_report_rest_flattened():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'parent': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
        mock_args = dict(parent='parent_value', report=migrationcenter.Report(name='name_value'), report_id='report_id_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.create_report(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*/reportConfigs/*}/reports' % client.transport._host, args[1])

def test_create_report_rest_flattened_error(transport: str='rest'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.create_report(migrationcenter.CreateReportRequest(), parent='parent_value', report=migrationcenter.Report(name='name_value'), report_id='report_id_value')

def test_create_report_rest_error():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.GetReportRequest, dict])
def test_get_report_rest(request_type):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3/reports/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Report(name='name_value', display_name='display_name_value', description='description_value', type_=migrationcenter.Report.Type.TOTAL_COST_OF_OWNERSHIP, state=migrationcenter.Report.State.PENDING)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Report.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_report(request)
    assert isinstance(response, migrationcenter.Report)
    assert response.name == 'name_value'
    assert response.display_name == 'display_name_value'
    assert response.description == 'description_value'
    assert response.type_ == migrationcenter.Report.Type.TOTAL_COST_OF_OWNERSHIP
    assert response.state == migrationcenter.Report.State.PENDING

def test_get_report_rest_required_fields(request_type=migrationcenter.GetReportRequest):
    if False:
        print('Hello World!')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_report._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).get_report._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('view',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.Report()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.Report.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.get_report(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_get_report_rest_unset_required_fields():
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.get_report._get_unset_required_fields({})
    assert set(unset_fields) == set(('view',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_get_report_rest_interceptors(null_interceptor):
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_get_report') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_get_report') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.GetReportRequest.pb(migrationcenter.GetReportRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.Report.to_json(migrationcenter.Report())
        request = migrationcenter.GetReportRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.Report()
        client.get_report(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_get_report_rest_bad_request(transport: str='rest', request_type=migrationcenter.GetReportRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3/reports/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_report(request)

def test_get_report_rest_flattened():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.Report()
        sample_request = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3/reports/sample4'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.Report.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.get_report(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/reportConfigs/*/reports/*}' % client.transport._host, args[1])

def test_get_report_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.get_report(migrationcenter.GetReportRequest(), name='name_value')

def test_get_report_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

@pytest.mark.parametrize('request_type', [migrationcenter.ListReportsRequest, dict])
def test_list_reports_rest(request_type):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'parent': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListReportsResponse(next_page_token='next_page_token_value', unreachable=['unreachable_value'])
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListReportsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_reports(request)
    assert isinstance(response, pagers.ListReportsPager)
    assert response.next_page_token == 'next_page_token_value'
    assert response.unreachable == ['unreachable_value']

def test_list_reports_rest_required_fields(request_type=migrationcenter.ListReportsRequest):
    if False:
        while True:
            i = 10
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['parent'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_reports._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['parent'] = 'parent_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).list_reports._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('filter', 'order_by', 'page_size', 'page_token', 'view'))
    jsonified_request.update(unset_fields)
    assert 'parent' in jsonified_request
    assert jsonified_request['parent'] == 'parent_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = migrationcenter.ListReportsResponse()
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'get', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            return_value = migrationcenter.ListReportsResponse.pb(return_value)
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.list_reports(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_list_reports_rest_unset_required_fields():
    if False:
        while True:
            i = 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.list_reports._get_unset_required_fields({})
    assert set(unset_fields) == set(('filter', 'orderBy', 'pageSize', 'pageToken', 'view')) & set(('parent',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_list_reports_rest_interceptors(null_interceptor):
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_list_reports') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_list_reports') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.ListReportsRequest.pb(migrationcenter.ListReportsRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = migrationcenter.ListReportsResponse.to_json(migrationcenter.ListReportsResponse())
        request = migrationcenter.ListReportsRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = migrationcenter.ListReportsResponse()
        client.list_reports(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_list_reports_rest_bad_request(transport: str='rest', request_type=migrationcenter.ListReportsRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'parent': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_reports(request)

def test_list_reports_rest_flattened():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = migrationcenter.ListReportsResponse()
        sample_request = {'parent': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
        mock_args = dict(parent='parent_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        return_value = migrationcenter.ListReportsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.list_reports(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{parent=projects/*/locations/*/reportConfigs/*}/reports' % client.transport._host, args[1])

def test_list_reports_rest_flattened_error(transport: str='rest'):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.list_reports(migrationcenter.ListReportsRequest(), parent='parent_value')

def test_list_reports_rest_pager(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with mock.patch.object(Session, 'request') as req:
        response = (migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report(), migrationcenter.Report()], next_page_token='abc'), migrationcenter.ListReportsResponse(reports=[], next_page_token='def'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report()], next_page_token='ghi'), migrationcenter.ListReportsResponse(reports=[migrationcenter.Report(), migrationcenter.Report()]))
        response = response + response
        response = tuple((migrationcenter.ListReportsResponse.to_json(x) for x in response))
        return_values = tuple((Response() for i in response))
        for (return_val, response_val) in zip(return_values, response):
            return_val._content = response_val.encode('UTF-8')
            return_val.status_code = 200
        req.side_effect = return_values
        sample_request = {'parent': 'projects/sample1/locations/sample2/reportConfigs/sample3'}
        pager = client.list_reports(request=sample_request)
        results = list(pager)
        assert len(results) == 6
        assert all((isinstance(i, migrationcenter.Report) for i in results))
        pages = list(client.list_reports(request=sample_request).pages)
        for (page_, token) in zip(pages, ['abc', 'def', 'ghi', '']):
            assert page_.raw_page.next_page_token == token

@pytest.mark.parametrize('request_type', [migrationcenter.DeleteReportRequest, dict])
def test_delete_report_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3/reports/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.delete_report(request)
    assert response.operation.name == 'operations/spam'

def test_delete_report_rest_required_fields(request_type=migrationcenter.DeleteReportRequest):
    if False:
        for i in range(10):
            print('nop')
    transport_class = transports.MigrationCenterRestTransport
    request_init = {}
    request_init['name'] = ''
    request = request_type(**request_init)
    pb_request = request_type.pb(request)
    jsonified_request = json.loads(json_format.MessageToJson(pb_request, including_default_value_fields=False, use_integers_for_enums=False))
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_report._get_unset_required_fields(jsonified_request)
    jsonified_request.update(unset_fields)
    jsonified_request['name'] = 'name_value'
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).delete_report._get_unset_required_fields(jsonified_request)
    assert not set(unset_fields) - set(('request_id',))
    jsonified_request.update(unset_fields)
    assert 'name' in jsonified_request
    assert jsonified_request['name'] == 'name_value'
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request = request_type(**request_init)
    return_value = operations_pb2.Operation(name='operations/spam')
    with mock.patch.object(Session, 'request') as req:
        with mock.patch.object(path_template, 'transcode') as transcode:
            pb_request = request_type.pb(request)
            transcode_result = {'uri': 'v1/sample_method', 'method': 'delete', 'query_params': pb_request}
            transcode.return_value = transcode_result
            response_value = Response()
            response_value.status_code = 200
            json_return_value = json_format.MessageToJson(return_value)
            response_value._content = json_return_value.encode('UTF-8')
            req.return_value = response_value
            response = client.delete_report(request)
            expected_params = [('$alt', 'json;enum-encoding=int')]
            actual_params = req.call_args.kwargs['params']
            assert expected_params == actual_params

def test_delete_report_rest_unset_required_fields():
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials)
    unset_fields = transport.delete_report._get_unset_required_fields({})
    assert set(unset_fields) == set(('requestId',)) & set(('name',))

@pytest.mark.parametrize('null_interceptor', [True, False])
def test_delete_report_rest_interceptors(null_interceptor):
    if False:
        return 10
    transport = transports.MigrationCenterRestTransport(credentials=ga_credentials.AnonymousCredentials(), interceptor=None if null_interceptor else transports.MigrationCenterRestInterceptor())
    client = MigrationCenterClient(transport=transport)
    with mock.patch.object(type(client.transport._session), 'request') as req, mock.patch.object(path_template, 'transcode') as transcode, mock.patch.object(operation.Operation, '_set_result_from_operation'), mock.patch.object(transports.MigrationCenterRestInterceptor, 'post_delete_report') as post, mock.patch.object(transports.MigrationCenterRestInterceptor, 'pre_delete_report') as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migrationcenter.DeleteReportRequest.pb(migrationcenter.DeleteReportRequest())
        transcode.return_value = {'method': 'post', 'uri': 'my_uri', 'body': pb_message, 'query_params': pb_message}
        req.return_value = Response()
        req.return_value.status_code = 200
        req.return_value.request = PreparedRequest()
        req.return_value._content = json_format.MessageToJson(operations_pb2.Operation())
        request = migrationcenter.DeleteReportRequest()
        metadata = [('key', 'val'), ('cephalopod', 'squid')]
        pre.return_value = (request, metadata)
        post.return_value = operations_pb2.Operation()
        client.delete_report(request, metadata=[('key', 'val'), ('cephalopod', 'squid')])
        pre.assert_called_once()
        post.assert_called_once()

def test_delete_report_rest_bad_request(transport: str='rest', request_type=migrationcenter.DeleteReportRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request_init = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3/reports/sample4'}
    request = request_type(**request_init)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.delete_report(request)

def test_delete_report_rest_flattened():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation(name='operations/spam')
        sample_request = {'name': 'projects/sample1/locations/sample2/reportConfigs/sample3/reports/sample4'}
        mock_args = dict(name='name_value')
        mock_args.update(sample_request)
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        client.delete_report(**mock_args)
        assert len(req.mock_calls) == 1
        (_, args, _) = req.mock_calls[0]
        assert path_template.validate('%s/v1/{name=projects/*/locations/*/reportConfigs/*/reports/*}' % client.transport._host, args[1])

def test_delete_report_rest_flattened_error(transport: str='rest'):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    with pytest.raises(ValueError):
        client.delete_report(migrationcenter.DeleteReportRequest(), name='name_value')

def test_delete_report_rest_error():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')

def test_credentials_transport_error():
    if False:
        i = 10
        return i + 15
    transport = transports.MigrationCenterGrpcTransport(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    transport = transports.MigrationCenterGrpcTransport(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client = MigrationCenterClient(client_options={'credentials_file': 'credentials.json'}, transport=transport)
    transport = transports.MigrationCenterGrpcTransport(credentials=ga_credentials.AnonymousCredentials())
    options = client_options.ClientOptions()
    options.api_key = 'api_key'
    with pytest.raises(ValueError):
        client = MigrationCenterClient(client_options=options, transport=transport)
    options = mock.Mock()
    options.api_key = 'api_key'
    with pytest.raises(ValueError):
        client = MigrationCenterClient(client_options=options, credentials=ga_credentials.AnonymousCredentials())
    transport = transports.MigrationCenterGrpcTransport(credentials=ga_credentials.AnonymousCredentials())
    with pytest.raises(ValueError):
        client = MigrationCenterClient(client_options={'scopes': ['1', '2']}, transport=transport)

def test_transport_instance():
    if False:
        for i in range(10):
            print('nop')
    transport = transports.MigrationCenterGrpcTransport(credentials=ga_credentials.AnonymousCredentials())
    client = MigrationCenterClient(transport=transport)
    assert client.transport is transport

def test_transport_get_channel():
    if False:
        print('Hello World!')
    transport = transports.MigrationCenterGrpcTransport(credentials=ga_credentials.AnonymousCredentials())
    channel = transport.grpc_channel
    assert channel
    transport = transports.MigrationCenterGrpcAsyncIOTransport(credentials=ga_credentials.AnonymousCredentials())
    channel = transport.grpc_channel
    assert channel

@pytest.mark.parametrize('transport_class', [transports.MigrationCenterGrpcTransport, transports.MigrationCenterGrpcAsyncIOTransport, transports.MigrationCenterRestTransport])
def test_transport_adc(transport_class):
    if False:
        print('Hello World!')
    with mock.patch.object(google.auth, 'default') as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport_class()
        adc.assert_called_once()

@pytest.mark.parametrize('transport_name', ['grpc', 'rest'])
def test_transport_kind(transport_name):
    if False:
        i = 10
        return i + 15
    transport = MigrationCenterClient.get_transport_class(transport_name)(credentials=ga_credentials.AnonymousCredentials())
    assert transport.kind == transport_name

def test_transport_grpc_default():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    assert isinstance(client.transport, transports.MigrationCenterGrpcTransport)

def test_migration_center_base_transport_error():
    if False:
        return 10
    with pytest.raises(core_exceptions.DuplicateCredentialArgs):
        transport = transports.MigrationCenterTransport(credentials=ga_credentials.AnonymousCredentials(), credentials_file='credentials.json')

def test_migration_center_base_transport():
    if False:
        print('Hello World!')
    with mock.patch('google.cloud.migrationcenter_v1.services.migration_center.transports.MigrationCenterTransport.__init__') as Transport:
        Transport.return_value = None
        transport = transports.MigrationCenterTransport(credentials=ga_credentials.AnonymousCredentials())
    methods = ('list_assets', 'get_asset', 'update_asset', 'batch_update_assets', 'delete_asset', 'batch_delete_assets', 'report_asset_frames', 'aggregate_assets_values', 'create_import_job', 'list_import_jobs', 'get_import_job', 'delete_import_job', 'update_import_job', 'validate_import_job', 'run_import_job', 'get_import_data_file', 'list_import_data_files', 'create_import_data_file', 'delete_import_data_file', 'list_groups', 'get_group', 'create_group', 'update_group', 'delete_group', 'add_assets_to_group', 'remove_assets_from_group', 'list_error_frames', 'get_error_frame', 'list_sources', 'get_source', 'create_source', 'update_source', 'delete_source', 'list_preference_sets', 'get_preference_set', 'create_preference_set', 'update_preference_set', 'delete_preference_set', 'get_settings', 'update_settings', 'create_report_config', 'get_report_config', 'list_report_configs', 'delete_report_config', 'create_report', 'get_report', 'list_reports', 'delete_report', 'get_location', 'list_locations', 'get_operation', 'cancel_operation', 'delete_operation', 'list_operations')
    for method in methods:
        with pytest.raises(NotImplementedError):
            getattr(transport, method)(request=object())
    with pytest.raises(NotImplementedError):
        transport.close()
    with pytest.raises(NotImplementedError):
        transport.operations_client
    remainder = ['kind']
    for r in remainder:
        with pytest.raises(NotImplementedError):
            getattr(transport, r)()

def test_migration_center_base_transport_with_credentials_file():
    if False:
        i = 10
        return i + 15
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.migrationcenter_v1.services.migration_center.transports.MigrationCenterTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        load_creds.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.MigrationCenterTransport(credentials_file='credentials.json', quota_project_id='octopus')
        load_creds.assert_called_once_with('credentials.json', scopes=None, default_scopes=('https://www.googleapis.com/auth/cloud-platform',), quota_project_id='octopus')

def test_migration_center_base_transport_with_adc():
    if False:
        i = 10
        return i + 15
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.migrationcenter_v1.services.migration_center.transports.MigrationCenterTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.MigrationCenterTransport()
        adc.assert_called_once()

def test_migration_center_auth_adc():
    if False:
        return 10
    with mock.patch.object(google.auth, 'default', autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        MigrationCenterClient()
        adc.assert_called_once_with(scopes=None, default_scopes=('https://www.googleapis.com/auth/cloud-platform',), quota_project_id=None)

@pytest.mark.parametrize('transport_class', [transports.MigrationCenterGrpcTransport, transports.MigrationCenterGrpcAsyncIOTransport])
def test_migration_center_transport_auth_adc(transport_class):
    if False:
        i = 10
        return i + 15
    with mock.patch.object(google.auth, 'default', autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport_class(quota_project_id='octopus', scopes=['1', '2'])
        adc.assert_called_once_with(scopes=['1', '2'], default_scopes=('https://www.googleapis.com/auth/cloud-platform',), quota_project_id='octopus')

@pytest.mark.parametrize('transport_class', [transports.MigrationCenterGrpcTransport, transports.MigrationCenterGrpcAsyncIOTransport, transports.MigrationCenterRestTransport])
def test_migration_center_transport_auth_gdch_credentials(transport_class):
    if False:
        for i in range(10):
            print('nop')
    host = 'https://language.com'
    api_audience_tests = [None, 'https://language2.com']
    api_audience_expect = [host, 'https://language2.com']
    for (t, e) in zip(api_audience_tests, api_audience_expect):
        with mock.patch.object(google.auth, 'default', autospec=True) as adc:
            gdch_mock = mock.MagicMock()
            type(gdch_mock).with_gdch_audience = mock.PropertyMock(return_value=gdch_mock)
            adc.return_value = (gdch_mock, None)
            transport_class(host=host, api_audience=t)
            gdch_mock.with_gdch_audience.assert_called_once_with(e)

@pytest.mark.parametrize('transport_class,grpc_helpers', [(transports.MigrationCenterGrpcTransport, grpc_helpers), (transports.MigrationCenterGrpcAsyncIOTransport, grpc_helpers_async)])
def test_migration_center_transport_create_channel(transport_class, grpc_helpers):
    if False:
        for i in range(10):
            print('nop')
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch.object(grpc_helpers, 'create_channel', autospec=True) as create_channel:
        creds = ga_credentials.AnonymousCredentials()
        adc.return_value = (creds, None)
        transport_class(quota_project_id='octopus', scopes=['1', '2'])
        create_channel.assert_called_with('migrationcenter.googleapis.com:443', credentials=creds, credentials_file=None, quota_project_id='octopus', default_scopes=('https://www.googleapis.com/auth/cloud-platform',), scopes=['1', '2'], default_host='migrationcenter.googleapis.com', ssl_credentials=None, options=[('grpc.max_send_message_length', -1), ('grpc.max_receive_message_length', -1)])

@pytest.mark.parametrize('transport_class', [transports.MigrationCenterGrpcTransport, transports.MigrationCenterGrpcAsyncIOTransport])
def test_migration_center_grpc_transport_client_cert_source_for_mtls(transport_class):
    if False:
        return 10
    cred = ga_credentials.AnonymousCredentials()
    with mock.patch.object(transport_class, 'create_channel') as mock_create_channel:
        mock_ssl_channel_creds = mock.Mock()
        transport_class(host='squid.clam.whelk', credentials=cred, ssl_channel_credentials=mock_ssl_channel_creds)
        mock_create_channel.assert_called_once_with('squid.clam.whelk:443', credentials=cred, credentials_file=None, scopes=None, ssl_credentials=mock_ssl_channel_creds, quota_project_id=None, options=[('grpc.max_send_message_length', -1), ('grpc.max_receive_message_length', -1)])
    with mock.patch.object(transport_class, 'create_channel', return_value=mock.Mock()):
        with mock.patch('grpc.ssl_channel_credentials') as mock_ssl_cred:
            transport_class(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
            (expected_cert, expected_key) = client_cert_source_callback()
            mock_ssl_cred.assert_called_once_with(certificate_chain=expected_cert, private_key=expected_key)

def test_migration_center_http_transport_client_cert_source_for_mtls():
    if False:
        while True:
            i = 10
    cred = ga_credentials.AnonymousCredentials()
    with mock.patch('google.auth.transport.requests.AuthorizedSession.configure_mtls_channel') as mock_configure_mtls_channel:
        transports.MigrationCenterRestTransport(credentials=cred, client_cert_source_for_mtls=client_cert_source_callback)
        mock_configure_mtls_channel.assert_called_once_with(client_cert_source_callback)

def test_migration_center_rest_lro_client():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    transport = client.transport
    assert isinstance(transport.operations_client, operations_v1.AbstractOperationsClient)
    assert transport.operations_client is transport.operations_client

@pytest.mark.parametrize('transport_name', ['grpc', 'grpc_asyncio', 'rest'])
def test_migration_center_host_no_port(transport_name):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), client_options=client_options.ClientOptions(api_endpoint='migrationcenter.googleapis.com'), transport=transport_name)
    assert client.transport._host == ('migrationcenter.googleapis.com:443' if transport_name in ['grpc', 'grpc_asyncio'] else 'https://migrationcenter.googleapis.com')

@pytest.mark.parametrize('transport_name', ['grpc', 'grpc_asyncio', 'rest'])
def test_migration_center_host_with_port(transport_name):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), client_options=client_options.ClientOptions(api_endpoint='migrationcenter.googleapis.com:8000'), transport=transport_name)
    assert client.transport._host == ('migrationcenter.googleapis.com:8000' if transport_name in ['grpc', 'grpc_asyncio'] else 'https://migrationcenter.googleapis.com:8000')

@pytest.mark.parametrize('transport_name', ['rest'])
def test_migration_center_client_transport_session_collision(transport_name):
    if False:
        print('Hello World!')
    creds1 = ga_credentials.AnonymousCredentials()
    creds2 = ga_credentials.AnonymousCredentials()
    client1 = MigrationCenterClient(credentials=creds1, transport=transport_name)
    client2 = MigrationCenterClient(credentials=creds2, transport=transport_name)
    session1 = client1.transport.list_assets._session
    session2 = client2.transport.list_assets._session
    assert session1 != session2
    session1 = client1.transport.get_asset._session
    session2 = client2.transport.get_asset._session
    assert session1 != session2
    session1 = client1.transport.update_asset._session
    session2 = client2.transport.update_asset._session
    assert session1 != session2
    session1 = client1.transport.batch_update_assets._session
    session2 = client2.transport.batch_update_assets._session
    assert session1 != session2
    session1 = client1.transport.delete_asset._session
    session2 = client2.transport.delete_asset._session
    assert session1 != session2
    session1 = client1.transport.batch_delete_assets._session
    session2 = client2.transport.batch_delete_assets._session
    assert session1 != session2
    session1 = client1.transport.report_asset_frames._session
    session2 = client2.transport.report_asset_frames._session
    assert session1 != session2
    session1 = client1.transport.aggregate_assets_values._session
    session2 = client2.transport.aggregate_assets_values._session
    assert session1 != session2
    session1 = client1.transport.create_import_job._session
    session2 = client2.transport.create_import_job._session
    assert session1 != session2
    session1 = client1.transport.list_import_jobs._session
    session2 = client2.transport.list_import_jobs._session
    assert session1 != session2
    session1 = client1.transport.get_import_job._session
    session2 = client2.transport.get_import_job._session
    assert session1 != session2
    session1 = client1.transport.delete_import_job._session
    session2 = client2.transport.delete_import_job._session
    assert session1 != session2
    session1 = client1.transport.update_import_job._session
    session2 = client2.transport.update_import_job._session
    assert session1 != session2
    session1 = client1.transport.validate_import_job._session
    session2 = client2.transport.validate_import_job._session
    assert session1 != session2
    session1 = client1.transport.run_import_job._session
    session2 = client2.transport.run_import_job._session
    assert session1 != session2
    session1 = client1.transport.get_import_data_file._session
    session2 = client2.transport.get_import_data_file._session
    assert session1 != session2
    session1 = client1.transport.list_import_data_files._session
    session2 = client2.transport.list_import_data_files._session
    assert session1 != session2
    session1 = client1.transport.create_import_data_file._session
    session2 = client2.transport.create_import_data_file._session
    assert session1 != session2
    session1 = client1.transport.delete_import_data_file._session
    session2 = client2.transport.delete_import_data_file._session
    assert session1 != session2
    session1 = client1.transport.list_groups._session
    session2 = client2.transport.list_groups._session
    assert session1 != session2
    session1 = client1.transport.get_group._session
    session2 = client2.transport.get_group._session
    assert session1 != session2
    session1 = client1.transport.create_group._session
    session2 = client2.transport.create_group._session
    assert session1 != session2
    session1 = client1.transport.update_group._session
    session2 = client2.transport.update_group._session
    assert session1 != session2
    session1 = client1.transport.delete_group._session
    session2 = client2.transport.delete_group._session
    assert session1 != session2
    session1 = client1.transport.add_assets_to_group._session
    session2 = client2.transport.add_assets_to_group._session
    assert session1 != session2
    session1 = client1.transport.remove_assets_from_group._session
    session2 = client2.transport.remove_assets_from_group._session
    assert session1 != session2
    session1 = client1.transport.list_error_frames._session
    session2 = client2.transport.list_error_frames._session
    assert session1 != session2
    session1 = client1.transport.get_error_frame._session
    session2 = client2.transport.get_error_frame._session
    assert session1 != session2
    session1 = client1.transport.list_sources._session
    session2 = client2.transport.list_sources._session
    assert session1 != session2
    session1 = client1.transport.get_source._session
    session2 = client2.transport.get_source._session
    assert session1 != session2
    session1 = client1.transport.create_source._session
    session2 = client2.transport.create_source._session
    assert session1 != session2
    session1 = client1.transport.update_source._session
    session2 = client2.transport.update_source._session
    assert session1 != session2
    session1 = client1.transport.delete_source._session
    session2 = client2.transport.delete_source._session
    assert session1 != session2
    session1 = client1.transport.list_preference_sets._session
    session2 = client2.transport.list_preference_sets._session
    assert session1 != session2
    session1 = client1.transport.get_preference_set._session
    session2 = client2.transport.get_preference_set._session
    assert session1 != session2
    session1 = client1.transport.create_preference_set._session
    session2 = client2.transport.create_preference_set._session
    assert session1 != session2
    session1 = client1.transport.update_preference_set._session
    session2 = client2.transport.update_preference_set._session
    assert session1 != session2
    session1 = client1.transport.delete_preference_set._session
    session2 = client2.transport.delete_preference_set._session
    assert session1 != session2
    session1 = client1.transport.get_settings._session
    session2 = client2.transport.get_settings._session
    assert session1 != session2
    session1 = client1.transport.update_settings._session
    session2 = client2.transport.update_settings._session
    assert session1 != session2
    session1 = client1.transport.create_report_config._session
    session2 = client2.transport.create_report_config._session
    assert session1 != session2
    session1 = client1.transport.get_report_config._session
    session2 = client2.transport.get_report_config._session
    assert session1 != session2
    session1 = client1.transport.list_report_configs._session
    session2 = client2.transport.list_report_configs._session
    assert session1 != session2
    session1 = client1.transport.delete_report_config._session
    session2 = client2.transport.delete_report_config._session
    assert session1 != session2
    session1 = client1.transport.create_report._session
    session2 = client2.transport.create_report._session
    assert session1 != session2
    session1 = client1.transport.get_report._session
    session2 = client2.transport.get_report._session
    assert session1 != session2
    session1 = client1.transport.list_reports._session
    session2 = client2.transport.list_reports._session
    assert session1 != session2
    session1 = client1.transport.delete_report._session
    session2 = client2.transport.delete_report._session
    assert session1 != session2

def test_migration_center_grpc_transport_channel():
    if False:
        for i in range(10):
            print('nop')
    channel = grpc.secure_channel('http://localhost/', grpc.local_channel_credentials())
    transport = transports.MigrationCenterGrpcTransport(host='squid.clam.whelk', channel=channel)
    assert transport.grpc_channel == channel
    assert transport._host == 'squid.clam.whelk:443'
    assert transport._ssl_channel_credentials == None

def test_migration_center_grpc_asyncio_transport_channel():
    if False:
        for i in range(10):
            print('nop')
    channel = aio.secure_channel('http://localhost/', grpc.local_channel_credentials())
    transport = transports.MigrationCenterGrpcAsyncIOTransport(host='squid.clam.whelk', channel=channel)
    assert transport.grpc_channel == channel
    assert transport._host == 'squid.clam.whelk:443'
    assert transport._ssl_channel_credentials == None

@pytest.mark.parametrize('transport_class', [transports.MigrationCenterGrpcTransport, transports.MigrationCenterGrpcAsyncIOTransport])
def test_migration_center_transport_channel_mtls_with_client_cert_source(transport_class):
    if False:
        while True:
            i = 10
    with mock.patch('grpc.ssl_channel_credentials', autospec=True) as grpc_ssl_channel_cred:
        with mock.patch.object(transport_class, 'create_channel') as grpc_create_channel:
            mock_ssl_cred = mock.Mock()
            grpc_ssl_channel_cred.return_value = mock_ssl_cred
            mock_grpc_channel = mock.Mock()
            grpc_create_channel.return_value = mock_grpc_channel
            cred = ga_credentials.AnonymousCredentials()
            with pytest.warns(DeprecationWarning):
                with mock.patch.object(google.auth, 'default') as adc:
                    adc.return_value = (cred, None)
                    transport = transport_class(host='squid.clam.whelk', api_mtls_endpoint='mtls.squid.clam.whelk', client_cert_source=client_cert_source_callback)
                    adc.assert_called_once()
            grpc_ssl_channel_cred.assert_called_once_with(certificate_chain=b'cert bytes', private_key=b'key bytes')
            grpc_create_channel.assert_called_once_with('mtls.squid.clam.whelk:443', credentials=cred, credentials_file=None, scopes=None, ssl_credentials=mock_ssl_cred, quota_project_id=None, options=[('grpc.max_send_message_length', -1), ('grpc.max_receive_message_length', -1)])
            assert transport.grpc_channel == mock_grpc_channel
            assert transport._ssl_channel_credentials == mock_ssl_cred

@pytest.mark.parametrize('transport_class', [transports.MigrationCenterGrpcTransport, transports.MigrationCenterGrpcAsyncIOTransport])
def test_migration_center_transport_channel_mtls_with_adc(transport_class):
    if False:
        print('Hello World!')
    mock_ssl_cred = mock.Mock()
    with mock.patch.multiple('google.auth.transport.grpc.SslCredentials', __init__=mock.Mock(return_value=None), ssl_credentials=mock.PropertyMock(return_value=mock_ssl_cred)):
        with mock.patch.object(transport_class, 'create_channel') as grpc_create_channel:
            mock_grpc_channel = mock.Mock()
            grpc_create_channel.return_value = mock_grpc_channel
            mock_cred = mock.Mock()
            with pytest.warns(DeprecationWarning):
                transport = transport_class(host='squid.clam.whelk', credentials=mock_cred, api_mtls_endpoint='mtls.squid.clam.whelk', client_cert_source=None)
            grpc_create_channel.assert_called_once_with('mtls.squid.clam.whelk:443', credentials=mock_cred, credentials_file=None, scopes=None, ssl_credentials=mock_ssl_cred, quota_project_id=None, options=[('grpc.max_send_message_length', -1), ('grpc.max_receive_message_length', -1)])
            assert transport.grpc_channel == mock_grpc_channel

def test_migration_center_grpc_lro_client():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc')
    transport = client.transport
    assert isinstance(transport.operations_client, operations_v1.OperationsClient)
    assert transport.operations_client is transport.operations_client

def test_migration_center_grpc_lro_async_client():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc_asyncio')
    transport = client.transport
    assert isinstance(transport.operations_client, operations_v1.OperationsAsyncClient)
    assert transport.operations_client is transport.operations_client

def test_asset_path():
    if False:
        for i in range(10):
            print('nop')
    project = 'squid'
    location = 'clam'
    asset = 'whelk'
    expected = 'projects/{project}/locations/{location}/assets/{asset}'.format(project=project, location=location, asset=asset)
    actual = MigrationCenterClient.asset_path(project, location, asset)
    assert expected == actual

def test_parse_asset_path():
    if False:
        i = 10
        return i + 15
    expected = {'project': 'octopus', 'location': 'oyster', 'asset': 'nudibranch'}
    path = MigrationCenterClient.asset_path(**expected)
    actual = MigrationCenterClient.parse_asset_path(path)
    assert expected == actual

def test_error_frame_path():
    if False:
        for i in range(10):
            print('nop')
    project = 'cuttlefish'
    location = 'mussel'
    source = 'winkle'
    error_frame = 'nautilus'
    expected = 'projects/{project}/locations/{location}/sources/{source}/errorFrames/{error_frame}'.format(project=project, location=location, source=source, error_frame=error_frame)
    actual = MigrationCenterClient.error_frame_path(project, location, source, error_frame)
    assert expected == actual

def test_parse_error_frame_path():
    if False:
        print('Hello World!')
    expected = {'project': 'scallop', 'location': 'abalone', 'source': 'squid', 'error_frame': 'clam'}
    path = MigrationCenterClient.error_frame_path(**expected)
    actual = MigrationCenterClient.parse_error_frame_path(path)
    assert expected == actual

def test_group_path():
    if False:
        for i in range(10):
            print('nop')
    project = 'whelk'
    location = 'octopus'
    group = 'oyster'
    expected = 'projects/{project}/locations/{location}/groups/{group}'.format(project=project, location=location, group=group)
    actual = MigrationCenterClient.group_path(project, location, group)
    assert expected == actual

def test_parse_group_path():
    if False:
        while True:
            i = 10
    expected = {'project': 'nudibranch', 'location': 'cuttlefish', 'group': 'mussel'}
    path = MigrationCenterClient.group_path(**expected)
    actual = MigrationCenterClient.parse_group_path(path)
    assert expected == actual

def test_import_data_file_path():
    if False:
        i = 10
        return i + 15
    project = 'winkle'
    location = 'nautilus'
    import_job = 'scallop'
    import_data_file = 'abalone'
    expected = 'projects/{project}/locations/{location}/importJobs/{import_job}/importDataFiles/{import_data_file}'.format(project=project, location=location, import_job=import_job, import_data_file=import_data_file)
    actual = MigrationCenterClient.import_data_file_path(project, location, import_job, import_data_file)
    assert expected == actual

def test_parse_import_data_file_path():
    if False:
        for i in range(10):
            print('nop')
    expected = {'project': 'squid', 'location': 'clam', 'import_job': 'whelk', 'import_data_file': 'octopus'}
    path = MigrationCenterClient.import_data_file_path(**expected)
    actual = MigrationCenterClient.parse_import_data_file_path(path)
    assert expected == actual

def test_import_job_path():
    if False:
        i = 10
        return i + 15
    project = 'oyster'
    location = 'nudibranch'
    import_job = 'cuttlefish'
    expected = 'projects/{project}/locations/{location}/importJobs/{import_job}'.format(project=project, location=location, import_job=import_job)
    actual = MigrationCenterClient.import_job_path(project, location, import_job)
    assert expected == actual

def test_parse_import_job_path():
    if False:
        return 10
    expected = {'project': 'mussel', 'location': 'winkle', 'import_job': 'nautilus'}
    path = MigrationCenterClient.import_job_path(**expected)
    actual = MigrationCenterClient.parse_import_job_path(path)
    assert expected == actual

def test_preference_set_path():
    if False:
        return 10
    project = 'scallop'
    location = 'abalone'
    preference_set = 'squid'
    expected = 'projects/{project}/locations/{location}/preferenceSets/{preference_set}'.format(project=project, location=location, preference_set=preference_set)
    actual = MigrationCenterClient.preference_set_path(project, location, preference_set)
    assert expected == actual

def test_parse_preference_set_path():
    if False:
        for i in range(10):
            print('nop')
    expected = {'project': 'clam', 'location': 'whelk', 'preference_set': 'octopus'}
    path = MigrationCenterClient.preference_set_path(**expected)
    actual = MigrationCenterClient.parse_preference_set_path(path)
    assert expected == actual

def test_report_path():
    if False:
        print('Hello World!')
    project = 'oyster'
    location = 'nudibranch'
    report_config = 'cuttlefish'
    report = 'mussel'
    expected = 'projects/{project}/locations/{location}/reportConfigs/{report_config}/reports/{report}'.format(project=project, location=location, report_config=report_config, report=report)
    actual = MigrationCenterClient.report_path(project, location, report_config, report)
    assert expected == actual

def test_parse_report_path():
    if False:
        return 10
    expected = {'project': 'winkle', 'location': 'nautilus', 'report_config': 'scallop', 'report': 'abalone'}
    path = MigrationCenterClient.report_path(**expected)
    actual = MigrationCenterClient.parse_report_path(path)
    assert expected == actual

def test_report_config_path():
    if False:
        while True:
            i = 10
    project = 'squid'
    location = 'clam'
    report_config = 'whelk'
    expected = 'projects/{project}/locations/{location}/reportConfigs/{report_config}'.format(project=project, location=location, report_config=report_config)
    actual = MigrationCenterClient.report_config_path(project, location, report_config)
    assert expected == actual

def test_parse_report_config_path():
    if False:
        while True:
            i = 10
    expected = {'project': 'octopus', 'location': 'oyster', 'report_config': 'nudibranch'}
    path = MigrationCenterClient.report_config_path(**expected)
    actual = MigrationCenterClient.parse_report_config_path(path)
    assert expected == actual

def test_settings_path():
    if False:
        return 10
    project = 'cuttlefish'
    location = 'mussel'
    expected = 'projects/{project}/locations/{location}/settings'.format(project=project, location=location)
    actual = MigrationCenterClient.settings_path(project, location)
    assert expected == actual

def test_parse_settings_path():
    if False:
        for i in range(10):
            print('nop')
    expected = {'project': 'winkle', 'location': 'nautilus'}
    path = MigrationCenterClient.settings_path(**expected)
    actual = MigrationCenterClient.parse_settings_path(path)
    assert expected == actual

def test_source_path():
    if False:
        print('Hello World!')
    project = 'scallop'
    location = 'abalone'
    source = 'squid'
    expected = 'projects/{project}/locations/{location}/sources/{source}'.format(project=project, location=location, source=source)
    actual = MigrationCenterClient.source_path(project, location, source)
    assert expected == actual

def test_parse_source_path():
    if False:
        print('Hello World!')
    expected = {'project': 'clam', 'location': 'whelk', 'source': 'octopus'}
    path = MigrationCenterClient.source_path(**expected)
    actual = MigrationCenterClient.parse_source_path(path)
    assert expected == actual

def test_common_billing_account_path():
    if False:
        i = 10
        return i + 15
    billing_account = 'oyster'
    expected = 'billingAccounts/{billing_account}'.format(billing_account=billing_account)
    actual = MigrationCenterClient.common_billing_account_path(billing_account)
    assert expected == actual

def test_parse_common_billing_account_path():
    if False:
        for i in range(10):
            print('nop')
    expected = {'billing_account': 'nudibranch'}
    path = MigrationCenterClient.common_billing_account_path(**expected)
    actual = MigrationCenterClient.parse_common_billing_account_path(path)
    assert expected == actual

def test_common_folder_path():
    if False:
        while True:
            i = 10
    folder = 'cuttlefish'
    expected = 'folders/{folder}'.format(folder=folder)
    actual = MigrationCenterClient.common_folder_path(folder)
    assert expected == actual

def test_parse_common_folder_path():
    if False:
        i = 10
        return i + 15
    expected = {'folder': 'mussel'}
    path = MigrationCenterClient.common_folder_path(**expected)
    actual = MigrationCenterClient.parse_common_folder_path(path)
    assert expected == actual

def test_common_organization_path():
    if False:
        print('Hello World!')
    organization = 'winkle'
    expected = 'organizations/{organization}'.format(organization=organization)
    actual = MigrationCenterClient.common_organization_path(organization)
    assert expected == actual

def test_parse_common_organization_path():
    if False:
        i = 10
        return i + 15
    expected = {'organization': 'nautilus'}
    path = MigrationCenterClient.common_organization_path(**expected)
    actual = MigrationCenterClient.parse_common_organization_path(path)
    assert expected == actual

def test_common_project_path():
    if False:
        for i in range(10):
            print('nop')
    project = 'scallop'
    expected = 'projects/{project}'.format(project=project)
    actual = MigrationCenterClient.common_project_path(project)
    assert expected == actual

def test_parse_common_project_path():
    if False:
        print('Hello World!')
    expected = {'project': 'abalone'}
    path = MigrationCenterClient.common_project_path(**expected)
    actual = MigrationCenterClient.parse_common_project_path(path)
    assert expected == actual

def test_common_location_path():
    if False:
        return 10
    project = 'squid'
    location = 'clam'
    expected = 'projects/{project}/locations/{location}'.format(project=project, location=location)
    actual = MigrationCenterClient.common_location_path(project, location)
    assert expected == actual

def test_parse_common_location_path():
    if False:
        return 10
    expected = {'project': 'whelk', 'location': 'octopus'}
    path = MigrationCenterClient.common_location_path(**expected)
    actual = MigrationCenterClient.parse_common_location_path(path)
    assert expected == actual

def test_client_with_default_client_info():
    if False:
        for i in range(10):
            print('nop')
    client_info = gapic_v1.client_info.ClientInfo()
    with mock.patch.object(transports.MigrationCenterTransport, '_prep_wrapped_messages') as prep:
        client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), client_info=client_info)
        prep.assert_called_once_with(client_info)
    with mock.patch.object(transports.MigrationCenterTransport, '_prep_wrapped_messages') as prep:
        transport_class = MigrationCenterClient.get_transport_class()
        transport = transport_class(credentials=ga_credentials.AnonymousCredentials(), client_info=client_info)
        prep.assert_called_once_with(client_info)

@pytest.mark.asyncio
async def test_transport_close_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport='grpc_asyncio')
    with mock.patch.object(type(getattr(client.transport, 'grpc_channel')), 'close') as close:
        async with client:
            close.assert_not_called()
        close.assert_called_once()

def test_get_location_rest_bad_request(transport: str='rest', request_type=locations_pb2.GetLocationRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    request = json_format.ParseDict({'name': 'projects/sample1/locations/sample2'}, request)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_location(request)

@pytest.mark.parametrize('request_type', [locations_pb2.GetLocationRequest, dict])
def test_get_location_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = locations_pb2.Location()
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_location(request)
    assert isinstance(response, locations_pb2.Location)

def test_list_locations_rest_bad_request(transport: str='rest', request_type=locations_pb2.ListLocationsRequest):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    request = json_format.ParseDict({'name': 'projects/sample1'}, request)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_locations(request)

@pytest.mark.parametrize('request_type', [locations_pb2.ListLocationsRequest, dict])
def test_list_locations_rest(request_type):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = locations_pb2.ListLocationsResponse()
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_locations(request)
    assert isinstance(response, locations_pb2.ListLocationsResponse)

def test_cancel_operation_rest_bad_request(transport: str='rest', request_type=operations_pb2.CancelOperationRequest):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    request = json_format.ParseDict({'name': 'projects/sample1/locations/sample2/operations/sample3'}, request)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.cancel_operation(request)

@pytest.mark.parametrize('request_type', [operations_pb2.CancelOperationRequest, dict])
def test_cancel_operation_rest(request_type):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/operations/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = None
        response_value = Response()
        response_value.status_code = 200
        json_return_value = '{}'
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.cancel_operation(request)
    assert response is None

def test_delete_operation_rest_bad_request(transport: str='rest', request_type=operations_pb2.DeleteOperationRequest):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    request = json_format.ParseDict({'name': 'projects/sample1/locations/sample2/operations/sample3'}, request)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.delete_operation(request)

@pytest.mark.parametrize('request_type', [operations_pb2.DeleteOperationRequest, dict])
def test_delete_operation_rest(request_type):
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/operations/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = None
        response_value = Response()
        response_value.status_code = 200
        json_return_value = '{}'
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.delete_operation(request)
    assert response is None

def test_get_operation_rest_bad_request(transport: str='rest', request_type=operations_pb2.GetOperationRequest):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    request = json_format.ParseDict({'name': 'projects/sample1/locations/sample2/operations/sample3'}, request)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.get_operation(request)

@pytest.mark.parametrize('request_type', [operations_pb2.GetOperationRequest, dict])
def test_get_operation_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2/operations/sample3'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.Operation()
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.get_operation(request)
    assert isinstance(response, operations_pb2.Operation)

def test_list_operations_rest_bad_request(transport: str='rest', request_type=operations_pb2.ListOperationsRequest):
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = request_type()
    request = json_format.ParseDict({'name': 'projects/sample1/locations/sample2'}, request)
    with mock.patch.object(Session, 'request') as req, pytest.raises(core_exceptions.BadRequest):
        response_value = Response()
        response_value.status_code = 400
        response_value.request = Request()
        req.return_value = response_value
        client.list_operations(request)

@pytest.mark.parametrize('request_type', [operations_pb2.ListOperationsRequest, dict])
def test_list_operations_rest(request_type):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport='rest')
    request_init = {'name': 'projects/sample1/locations/sample2'}
    request = request_type(**request_init)
    with mock.patch.object(type(client.transport._session), 'request') as req:
        return_value = operations_pb2.ListOperationsResponse()
        response_value = Response()
        response_value.status_code = 200
        json_return_value = json_format.MessageToJson(return_value)
        response_value._content = json_return_value.encode('UTF-8')
        req.return_value = response_value
        response = client.list_operations(request)
    assert isinstance(response, operations_pb2.ListOperationsResponse)

def test_delete_operation(transport: str='grpc'):
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = operations_pb2.DeleteOperationRequest()
    with mock.patch.object(type(client.transport.delete_operation), '__call__') as call:
        call.return_value = None
        response = client.delete_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert response is None

@pytest.mark.asyncio
async def test_delete_operation_async(transport: str='grpc'):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = operations_pb2.DeleteOperationRequest()
    with mock.patch.object(type(client.transport.delete_operation), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        response = await client.delete_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert response is None

def test_delete_operation_field_headers():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = operations_pb2.DeleteOperationRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.delete_operation), '__call__') as call:
        call.return_value = None
        client.delete_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

@pytest.mark.asyncio
async def test_delete_operation_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = operations_pb2.DeleteOperationRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.delete_operation), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        await client.delete_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

def test_delete_operation_from_dict():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_operation), '__call__') as call:
        call.return_value = None
        response = client.delete_operation(request={'name': 'locations'})
        call.assert_called()

@pytest.mark.asyncio
async def test_delete_operation_from_dict_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.delete_operation), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        response = await client.delete_operation(request={'name': 'locations'})
        call.assert_called()

def test_cancel_operation(transport: str='grpc'):
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = operations_pb2.CancelOperationRequest()
    with mock.patch.object(type(client.transport.cancel_operation), '__call__') as call:
        call.return_value = None
        response = client.cancel_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert response is None

@pytest.mark.asyncio
async def test_cancel_operation_async(transport: str='grpc'):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = operations_pb2.CancelOperationRequest()
    with mock.patch.object(type(client.transport.cancel_operation), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        response = await client.cancel_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert response is None

def test_cancel_operation_field_headers():
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = operations_pb2.CancelOperationRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.cancel_operation), '__call__') as call:
        call.return_value = None
        client.cancel_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

@pytest.mark.asyncio
async def test_cancel_operation_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = operations_pb2.CancelOperationRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.cancel_operation), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        await client.cancel_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

def test_cancel_operation_from_dict():
    if False:
        return 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.cancel_operation), '__call__') as call:
        call.return_value = None
        response = client.cancel_operation(request={'name': 'locations'})
        call.assert_called()

@pytest.mark.asyncio
async def test_cancel_operation_from_dict_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.cancel_operation), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(None)
        response = await client.cancel_operation(request={'name': 'locations'})
        call.assert_called()

def test_get_operation(transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = operations_pb2.GetOperationRequest()
    with mock.patch.object(type(client.transport.get_operation), '__call__') as call:
        call.return_value = operations_pb2.Operation()
        response = client.get_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert isinstance(response, operations_pb2.Operation)

@pytest.mark.asyncio
async def test_get_operation_async(transport: str='grpc'):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = operations_pb2.GetOperationRequest()
    with mock.patch.object(type(client.transport.get_operation), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation())
        response = await client.get_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert isinstance(response, operations_pb2.Operation)

def test_get_operation_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = operations_pb2.GetOperationRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.get_operation), '__call__') as call:
        call.return_value = operations_pb2.Operation()
        client.get_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

@pytest.mark.asyncio
async def test_get_operation_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = operations_pb2.GetOperationRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.get_operation), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation())
        await client.get_operation(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

def test_get_operation_from_dict():
    if False:
        for i in range(10):
            print('nop')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_operation), '__call__') as call:
        call.return_value = operations_pb2.Operation()
        response = client.get_operation(request={'name': 'locations'})
        call.assert_called()

@pytest.mark.asyncio
async def test_get_operation_from_dict_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.get_operation), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.Operation())
        response = await client.get_operation(request={'name': 'locations'})
        call.assert_called()

def test_list_operations(transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = operations_pb2.ListOperationsRequest()
    with mock.patch.object(type(client.transport.list_operations), '__call__') as call:
        call.return_value = operations_pb2.ListOperationsResponse()
        response = client.list_operations(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert isinstance(response, operations_pb2.ListOperationsResponse)

@pytest.mark.asyncio
async def test_list_operations_async(transport: str='grpc'):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = operations_pb2.ListOperationsRequest()
    with mock.patch.object(type(client.transport.list_operations), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.ListOperationsResponse())
        response = await client.list_operations(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert isinstance(response, operations_pb2.ListOperationsResponse)

def test_list_operations_field_headers():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = operations_pb2.ListOperationsRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.list_operations), '__call__') as call:
        call.return_value = operations_pb2.ListOperationsResponse()
        client.list_operations(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

@pytest.mark.asyncio
async def test_list_operations_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = operations_pb2.ListOperationsRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.list_operations), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.ListOperationsResponse())
        await client.list_operations(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

def test_list_operations_from_dict():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_operations), '__call__') as call:
        call.return_value = operations_pb2.ListOperationsResponse()
        response = client.list_operations(request={'name': 'locations'})
        call.assert_called()

@pytest.mark.asyncio
async def test_list_operations_from_dict_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_operations), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(operations_pb2.ListOperationsResponse())
        response = await client.list_operations(request={'name': 'locations'})
        call.assert_called()

def test_list_locations(transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = locations_pb2.ListLocationsRequest()
    with mock.patch.object(type(client.transport.list_locations), '__call__') as call:
        call.return_value = locations_pb2.ListLocationsResponse()
        response = client.list_locations(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert isinstance(response, locations_pb2.ListLocationsResponse)

@pytest.mark.asyncio
async def test_list_locations_async(transport: str='grpc'):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = locations_pb2.ListLocationsRequest()
    with mock.patch.object(type(client.transport.list_locations), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(locations_pb2.ListLocationsResponse())
        response = await client.list_locations(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert isinstance(response, locations_pb2.ListLocationsResponse)

def test_list_locations_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = locations_pb2.ListLocationsRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.list_locations), '__call__') as call:
        call.return_value = locations_pb2.ListLocationsResponse()
        client.list_locations(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

@pytest.mark.asyncio
async def test_list_locations_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = locations_pb2.ListLocationsRequest()
    request.name = 'locations'
    with mock.patch.object(type(client.transport.list_locations), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(locations_pb2.ListLocationsResponse())
        await client.list_locations(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations') in kw['metadata']

def test_list_locations_from_dict():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_locations), '__call__') as call:
        call.return_value = locations_pb2.ListLocationsResponse()
        response = client.list_locations(request={'name': 'locations'})
        call.assert_called()

@pytest.mark.asyncio
async def test_list_locations_from_dict_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_locations), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(locations_pb2.ListLocationsResponse())
        response = await client.list_locations(request={'name': 'locations'})
        call.assert_called()

def test_get_location(transport: str='grpc'):
    if False:
        i = 10
        return i + 15
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = locations_pb2.GetLocationRequest()
    with mock.patch.object(type(client.transport.get_location), '__call__') as call:
        call.return_value = locations_pb2.Location()
        response = client.get_location(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert isinstance(response, locations_pb2.Location)

@pytest.mark.asyncio
async def test_get_location_async(transport: str='grpc_asyncio'):
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
    request = locations_pb2.GetLocationRequest()
    with mock.patch.object(type(client.transport.get_location), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(locations_pb2.Location())
        response = await client.get_location(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    assert isinstance(response, locations_pb2.Location)

def test_get_location_field_headers():
    if False:
        while True:
            i = 10
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    request = locations_pb2.GetLocationRequest()
    request.name = 'locations/abc'
    with mock.patch.object(type(client.transport.get_location), '__call__') as call:
        call.return_value = locations_pb2.Location()
        client.get_location(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations/abc') in kw['metadata']

@pytest.mark.asyncio
async def test_get_location_field_headers_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    request = locations_pb2.GetLocationRequest()
    request.name = 'locations/abc'
    with mock.patch.object(type(client.transport.get_location), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(locations_pb2.Location())
        await client.get_location(request)
        assert len(call.mock_calls) == 1
        (_, args, _) = call.mock_calls[0]
        assert args[0] == request
    (_, _, kw) = call.mock_calls[0]
    assert ('x-goog-request-params', 'name=locations/abc') in kw['metadata']

def test_get_location_from_dict():
    if False:
        print('Hello World!')
    client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_locations), '__call__') as call:
        call.return_value = locations_pb2.Location()
        response = client.get_location(request={'name': 'locations/abc'})
        call.assert_called()

@pytest.mark.asyncio
async def test_get_location_from_dict_async():
    client = MigrationCenterAsyncClient(credentials=ga_credentials.AnonymousCredentials())
    with mock.patch.object(type(client.transport.list_locations), '__call__') as call:
        call.return_value = grpc_helpers_async.FakeUnaryUnaryCall(locations_pb2.Location())
        response = await client.get_location(request={'name': 'locations'})
        call.assert_called()

def test_transport_close():
    if False:
        for i in range(10):
            print('nop')
    transports = {'rest': '_session', 'grpc': '_grpc_channel'}
    for (transport, close_name) in transports.items():
        client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
        with mock.patch.object(type(getattr(client.transport, close_name)), 'close') as close:
            with client:
                close.assert_not_called()
            close.assert_called_once()

def test_client_ctx():
    if False:
        print('Hello World!')
    transports = ['rest', 'grpc']
    for transport in transports:
        client = MigrationCenterClient(credentials=ga_credentials.AnonymousCredentials(), transport=transport)
        with mock.patch.object(type(client.transport), 'close') as close:
            close.assert_not_called()
            with client:
                pass
            close.assert_called()

@pytest.mark.parametrize('client_class,transport_class', [(MigrationCenterClient, transports.MigrationCenterGrpcTransport), (MigrationCenterAsyncClient, transports.MigrationCenterGrpcAsyncIOTransport)])
def test_api_key_credentials(client_class, transport_class):
    if False:
        return 10
    with mock.patch.object(google.auth._default, 'get_api_key_credentials', create=True) as get_api_key_credentials:
        mock_cred = mock.Mock()
        get_api_key_credentials.return_value = mock_cred
        options = client_options.ClientOptions()
        options.api_key = 'api_key'
        with mock.patch.object(transport_class, '__init__') as patched:
            patched.return_value = None
            client = client_class(client_options=options)
            patched.assert_called_once_with(credentials=mock_cred, credentials_file=None, host=client.DEFAULT_ENDPOINT, scopes=None, client_cert_source_for_mtls=None, quota_project_id=None, client_info=transports.base.DEFAULT_CLIENT_INFO, always_use_jwt_access=True, api_audience=None)