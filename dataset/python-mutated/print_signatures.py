...
if __name__ == '__main__':
    ...
    if args.method == 'from_modulelist':
        ...
        for api_name in all_api_names_sorted:
            ...
            print("{} ({}, ('document', '{}'))".format(api_name, api_info['signature'] if 'signature' in api_info else 'ArgSpec()', md5(api_info['docstring'])))
    if len(ErrorSet) == 0:
        sys.exit(0)
    else:
        for erroritem in ErrorSet:
            print(f'Error, new function {erroritem} is unreachable', file=sys.stderr)
        sys.exit(1)