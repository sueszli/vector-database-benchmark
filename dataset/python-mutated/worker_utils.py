...
async def get_worker_id(api_key: str=Depends(get_api_key), protocol_version: str=Depends(get_protocol_version)) -> models.DbWorker:
    """Get the ID of a worker from its API key and protocol version."""
    logger.info(f'get_worker: api_key={api_key!r}, protocol_version={protocol_version!r}')
    ...
    if worker is None:
        raise fastapi.HTTPException(status_code=fastapi.status.HTTP_401_UNAUTHORIZED, detail='Invalid API key')
    return worker.id

async def get_worker(worker_id: str=Depends(get_worker_id), session: database.AsyncSession=Depends(deps.create_session)) -> models.DbWorker:
    query = sqlmodel.select(models.DbWorker).where(models.DbWorker.id == worker_id)
    worker = (await session.exec(query)).one()
    return worker

async def send_worker_request(websocket: fastapi.WebSocket, request: inference.WorkerRequest):
    return await websocket.send_text(request.json())

async def receive_worker_response(websocket: fastapi.WebSocket) -> inference.WorkerResponse:
    return pydantic.parse_raw_as(inference.WorkerResponse, await websocket.receive_text())

async def receive_worker_info(websocket: fastapi.WebSocket) -> inference.WorkerInfo:
...
async def build_work_request(session: database.AsyncSession, message_id: str) -> inference.WorkRequest:
    """
    ...
    """
    query = sqlmodel.select(models.DbMessage).options(sqlalchemy.orm.selectinload(models.DbMessage.chat).selectinload(models.DbChat.messages).selectinload(models.DbMessage.reports)).where(models.DbMessage.id == message_id)
    ...
    thread = inference.Thread(messages=[m.to_read() for m in reversed(thread_msgs)])
    return inference.WorkRequest(thread=thread, parameters=message.work_parameters)