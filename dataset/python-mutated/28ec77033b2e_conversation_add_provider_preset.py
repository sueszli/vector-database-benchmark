...
def upgrade() -> None:
    if False:
        while True:
            i = 10
    op.add_column('conversation', sa.Column('provider', sa.String(), nullable=False, server_default=''))
    op.add_column('conversation', sa.Column('preset', sa.String(), nullable=False, server_default=''))
    op.execute("UPDATE conversation SET provider = 'ai21' WHERE model IN ('j2-large', 'j2-grande', 'j2-jumbo', 'j2-large-instruct', 'j2-grande-instruct', 'j2-jumbo-instruct');")
    op.execute("UPDATE conversation SET provider = 'chat_openai' WHERE model IN ('gpt-3.5-turbo', 'gpt-3.5-turbo-0301', 'gpt-4', 'gpt-4-0314', 'gpt-4-32k', 'gpt-4-32k-0314');")
