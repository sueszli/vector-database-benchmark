...
from modelscope.outputs.outputs import TASK_OUTPUTS, OutputKeys, OutputTypes, OutputTypeSchema
from modelscope.pipeline_inputs import INPUT_TYPE, INPUT_TYPE_SCHEMA, TASK_INPUTS, InputType
...
meta_type_schema_map = {'str': 'string', 'int': 'integer', 'float': 'number', 'bool': 'boolean', 'Dict': 'object', 'dict': 'object', 'list': 'array', 'List': 'array', 'Union': 'object', 'Input': 'object', 'object': 'object'}
...
def pipeline_output_to_service_multipart_output(output):
    ...
    pass
base64_decoder_map = {InputType.IMAGE: decode_base64_to_image, InputType.TEXT: return_origin, InputType.AUDIO: decode_base64_to_audio, InputType.VIDEO: decode_base64_to_video, InputType.BOX: decode_box, InputType.DICT: return_origin, InputType.LIST: return_origin, InputType.NUMBER: return_origin}
...
base64_encoder_map = {'image': encode_numpy_image_to_base64, 'video': encode_video_to_base64, 'pcm': encode_pcm_to_base64, 'wav': encode_wav_to_base64, 'bytes': encode_bytes_to_base64}
type_to_python_type = {np.int64: int}
...
if __name__ == '__main__':
    ...
    for (key, value) in index['index'].items():
        ...
        if reg == 'PIPELINES' and task_name != 'default':
            print(f"value['filepath']: {value['filepath']}, class_name: {class_name}")
            (input, parameters) = get_pipeline_input_parameters(value['filepath'], class_name)
            try:
                if task_name in TASK_INPUTS and task_name in TASK_OUTPUTS:
                    parameters_schema = generate_pipeline_parameters_schema(parameters)
                    input_schema = get_input_schema(task_name, None)
                    output_schema = get_output_schema(task_name)
                    schema = {'input': input_schema, 'parameters': parameters_schema, 'output': output_schema}
                else:
                    logger.warning('Task: %s input is defined: %s, output is defined: %s which is not completed' % (task_name, task_name in TASK_INPUTS, task_name in TASK_OUTPUTS))
                    input_schema = None
                    output_schema = None
                    ...
                    if task_name in TASK_OUTPUTS:
                        output_schema = get_output_schema(task_name)
                    parameters_schema = generate_pipeline_parameters_schema(parameters)
                    schema = {'input': input_schema if input_schema else parameters_schema, 'parameters': parameters_schema if input_schema else {}, 'output': output_schema if output_schema else {'type': 'object'}}
