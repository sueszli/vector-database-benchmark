...
from .base_polling import _failed, BadStatus, BadResponse, OperationFailed, _SansIOLROBasePolling, _raise_if_bad_http_status_and_method
...
class AsyncLROBasePolling(_SansIOLROBasePolling[PollingReturnType_co, AsyncPipelineClient[HttpRequestTypeVar, AsyncHttpResponseTypeVar], HttpRequestTypeVar, AsyncHttpResponseTypeVar], AsyncPollingMethod[PollingReturnType_co]):
    ...
    async def request_status(self, status_link: str) -> PipelineResponse[HttpRequestTypeVar, AsyncHttpResponseTypeVar]:
        ...
        if is_rest(self._initial_response.http_response):
            ...
            return cast(PipelineResponse[HttpRequestTypeVar, AsyncHttpResponseTypeVar], await self._client.send_request(rest_request, _return_pipeline_response=True, **self._operation_config))
        request = cast(HttpRequestTypeVar, self._client.get(status_link))
        return cast(PipelineResponse[HttpRequestTypeVar, AsyncHttpResponseTypeVar], await self._client._pipeline.run(request, stream=False, **self._operation_config))
__all__ = ['AsyncLROBasePolling']