...
from transformers import CONFIG_MAPPING, FLAX_MODEL_FOR_MASKED_LM_MAPPING, AutoConfig, AutoTokenizer, FlaxAutoModelForMaskedLM, HfArgumentParser, PreTrainedTokenizerBase, TensorType, TrainingArguments, is_tensorboard_available, set_seed
...
class DataTrainingArguments:
    ...
    text_column_name: str = field(default='text', metadata={'help': 'The name of the column to retrieve the training text.'})
...
if __name__ == '__main__':
    ...
    logging.basicConfig(format='%(asctime)s - %(levelname)s - %(name)s - %(message)s', level='INFO', datefmt='[%X]')
    ...
    if data_args.dataset_name is not None:
        dataset = load_dataset(data_args.dataset_name, data_args.dataset_config_name, cache_dir=model_args.cache_dir, streaming=True, split='train')
    ...
    if model_args.model_name_or_path:
        model = FlaxAutoModelForMaskedLM.from_pretrained(model_args.model_name_or_path, config=config, seed=training_args.seed, dtype=getattr(jnp, model_args.dtype))
    else:
    ...
    for step in range(num_train_steps):
        ...
        if step % training_args.logging_steps == 0 and step > 0:
            steps.write(f"Step... ({step} | Loss: {train_metric['loss'].mean()}, Learning Rate: {train_metric['learning_rate'].mean()})")
        ...
        if step % training_args.eval_steps == 0 and step > 0:
            ...
            if jax.process_index() == 0:
                ...
                model.save_pretrained(training_args.output_dir, params=params, push_to_hub=training_args.push_to_hub, commit_message=f'Saving weights and logs of step {step + 1}')
        steps.update(1)