...
class WebhookDryRun(BaseMutation):
    payload = JSONString(description="JSON payload, that would be sent out to webhook's target URL.")

    class Arguments:
        query = graphene.String(description='The subscription query that defines the webhook event and its payload.', required=True)
        object_id = graphene.ID(description='The ID of an object to serialize.', required=True)

    class Meta:
        description = 'Performs a dry run of a webhook event. Supports a single event (the first, if multiple provided in the `query`). Requires permission relevant to processed event.' + ADDED_IN_311 + PREVIEW_FEATURE
    ...
    def validate_event_type(cls, event_type, object_id):
        ...
        if not (model_name or enable_dry_run) and event_type:
            ...
            raise_validation_error(field='query', message=f'Event type: {event_name} not supported.', code=WebhookDryRunErrorCode.TYPE_NOT_SUPPORTED)
        if model != model_name:
            raise_validation_error(field='objectId', message="ObjectId doesn't match event type.", code=WebhookDryRunErrorCode.INVALID_ID)
    ...
    def validate_permissions(cls, info, event_type):
        ...
        if (permission := (WebhookEventAsyncType.PERMISSIONS.get(event_type) if event_type else None)):
            ...
            if codename not in user_permissions:
                raise_validation_error(message=f"The user doesn't have required permission: {codename}.", code=WebhookDryRunErrorCode.MISSING_PERMISSION)
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        if all([event_type, object, query]):
            ...
            payload = generate_payload_from_subscription(event_type, object, query, request)
