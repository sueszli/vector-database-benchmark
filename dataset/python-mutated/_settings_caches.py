...
class PrefixManager:
    ...
    async def set_prefixes(self, guild: Optional[discord.Guild]=None, prefixes: Optional[List[str]]=None):
        gid: Optional[int] = guild.id if guild else None
        ...
        if any((prefix.startswith('/') for prefix in prefixes)):
            raise ValueError("Prefixes cannot start with '/', as it conflicts with Discord's slash commands.")
...
class I18nManager:
    ...
    async def set_locale(self, guild: Union[discord.Guild, None], locale: Union[str, None]) -> None:
    ...
    async def set_regional_format(self, guild: Union[discord.Guild, None], regional_format: Union[str, None]) -> None:
...
class IgnoreManager:
    ...
    async def get_ignored_channel(self, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.ForumChannel, discord.Thread], check_category: bool=True) -> bool:
        ret: bool
        cid: int = channel.id
        cat_id: Optional[int] = channel.category.id if check_category and channel.category else None
    ...
    async def set_ignored_channel(self, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.Thread, discord.ForumChannel, discord.CategoryChannel], set_to: bool):
        cid: int = channel.id
...
class WhitelistBlacklistManager:
    ...
    async def discord_deleted_user(self, user_id: int):
        async with self._access_lock:
            async for (guild_id_or_none, ids) in AsyncIter(self._cached_whitelist.items(), steps=100):
                ids.discard(user_id)
            async for (guild_id_or_none, ids) in AsyncIter(self._cached_blacklist.items(), steps=100):
                ids.discard(user_id)
    ...
    async def add_to_whitelist(self, guild: Optional[discord.Guild], role_or_user: Iterable[int]):
        async with self._access_lock:
            ...
            if gid is None:
                ...
                if gid not in self._cached_whitelist:
                    self._cached_whitelist[gid] = set(await self._config.guild_from_id(gid).whitelist())
                self._cached_whitelist[gid].update(role_or_user)
    ...
    async def remove_from_whitelist(self, guild: Optional[discord.Guild], role_or_user: Iterable[int]):
        async with self._access_lock:
            gid: Optional[int] = guild.id if guild else None
            role_or_user = role_or_user or []
            if not all((isinstance(r_or_u, int) for r_or_u in role_or_user)):
    ...
    async def add_to_blacklist(self, guild: Optional[discord.Guild], role_or_user: Iterable[int]):
        async with self._access_lock:
            ...
            if gid is None:
                ...
                if gid not in self._cached_blacklist:
                    self._cached_blacklist[gid] = set(await self._config.guild_from_id(gid).blacklist())
                self._cached_blacklist[gid].update(role_or_user)
    ...
    async def remove_from_blacklist(self, guild: Optional[discord.Guild], role_or_user: Iterable[int]):
        async with self._access_lock:
            gid: Optional[int] = guild.id if guild else None
            role_or_user = role_or_user or []
            if not all((isinstance(r_or_u, int) for r_or_u in role_or_user)):
