...
from synapse.http.server import DirectServeHtmlResource, DirectServeJsonResource, respond_with_html
...
class AvailabilityCheckResource(DirectServeJsonResource):
    ...
    async def _async_render_GET(self, request: Request) -> Tuple[int, JsonDict]:
        ...
        is_available = await self._sso_handler.check_username_availability(localpart, session_id)
...
class AccountDetailsResource(DirectServeHtmlResource):
    ...
    async def _async_render_GET(self, request: Request) -> None:
        ...
        template_params = {'idp': self._sso_handler.get_identity_providers()[idp_id], 'user_attributes': {'display_name': session.display_name, 'emails': session.emails, 'localpart': localpart}}
    ...
    async def _async_render_POST(self, request: SynapseRequest) -> None:
        ...
        try:
            ...
            try:
                emails_to_use: List[str] = [val.decode('utf-8') for val in request.args.get(b'use_email', [])]
            except ValueError:
            ...
            return
        await self._sso_handler.handle_submit_username_request(request, session_id, localpart, use_display_name, emails_to_use)