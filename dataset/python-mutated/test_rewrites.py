...
sub_param_from_const = p.ScalarParameter >> d.Literal(const(param_values)[_], dtype=_.dtype)
...
simplifications = AnyOf(p.Add(x, p.Literal(0)) >> x, p.Add(p.Literal(0), x) >> x, p.Subtract(x, p.Literal(0)) >> x, p.Multiply(x, p.Literal(1)) >> x, p.Multiply(p.Literal(1), x) >> x, p.Multiply(x, p.Literal(0)) >> zero.op(), p.Multiply(p.Literal(0), x) >> zero.op(), p.Divide(x, p.Literal(1)) >> x, p.Divide(p.Literal(0), x) >> zero.op(), p.Divide(x, y) & Check(x == y) >> one.op(), fold_literal_add)
