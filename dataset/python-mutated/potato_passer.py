...
class PotatoPasser:
    ...
    async def pass_potato(self, potato: int, target: int):
        ...
        if potato % self.print_every == 0:
            print(f'running, name {self.name}, count {self.count}, potato {potato}, target {target}')
        if potato >= target:
...
async def main():
    ...
    parser.add_argument('--sleep-secs', type=float, help='Sleep seconds before sending message to next actor')
    args = parser.parse_args()
    actors = []
    for i in range(args.num_actors):
        this_actor = 'actor' + str(i)
        next_actor = 'actor' + str((i + 1) % args.num_actors)
        actor = PotatoPasser.options(name=this_actor, scheduling_strategy='SPREAD').remote(this_actor, next_actor, args.sleep_secs)
        actors.append(actor)
