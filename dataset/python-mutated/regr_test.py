...
from utils import PYTHON_VERSION, PackageInfo, VenvInfo, colored, get_all_testcase_directories, get_mypy_req, make_venv, print_error, testcase_dir_from_package_name
...
parser.add_argument('packages_to_test', type=package_with_test_cases, nargs='*', action='extend', help='Test only these packages (defaults to all typeshed stubs that have test cases). Use "stdlib" to test the standard library test cases.')
parser.add_argument('--all', action='store_true', help='Run tests on all available platforms and versions (defaults to "False"). Note that this cannot be specified if --platform and/or --python-version are specified.')
...
parser.add_argument('--platform', dest='platforms_to_test', choices=SUPPORTED_PLATFORMS, nargs='*', action='extend', help='Run mypy for certain OS platforms (defaults to sys.platform). Note that this cannot be specified if --all is also specified.')
parser.add_argument('-p', '--python-version', dest='versions_to_test', choices=SUPPORTED_VERSIONS, nargs='*', action='extend', help='Run mypy for certain Python versions (defaults to sys.version_info[:2]). Note that this cannot be specified if --all is also specified.')
