...
from fastchat.model.model_adapter import add_model_args, get_conversation_template, get_generate_stream_function
...
if __name__ == '__main__':
    ...
    parser.add_argument('--limit-worker-concurrency', type=int, default=5, help='Limit the model concurrency to prevent OOM.')
    parser.add_argument('--stream-interval', type=int, default=2)
    parser.add_argument('--no-register', action='store_true')
    args = parser.parse_args()
    logger.info(f'args: {args}')
    if args.gpus:
        invalidInputError(len(args.gpus.split(',')) > args.num_gpus, f'Larger --num-gpus ({{args.num_gpus}}) than --gpus {{args.gpus}}!')
        os.environ['CUDA_VISIBLE_DEVICES'] = args.gpus
    gptq_config = GptqConfig(ckpt=args.gptq_ckpt or args.model_path, wbits=args.gptq_wbits, groupsize=args.gptq_groupsize, act_order=args.gptq_act_order)
    awq_config = AWQConfig(ckpt=args.awq_ckpt or args.model_path, wbits=args.awq_wbits, groupsize=args.awq_groupsize)
    worker = ModelWorker(args.controller_address, args.worker_address, worker_id, args.model_path, args.model_names, args.limit_worker_concurrency, no_register=args.no_register, device=args.device, num_gpus=args.num_gpus, max_gpu_memory=args.max_gpu_memory, load_8bit=args.load_8bit, cpu_offloading=args.cpu_offloading, gptq_config=gptq_config, awq_config=awq_config, stream_interval=args.stream_interval, conv_template=args.conv_template)
