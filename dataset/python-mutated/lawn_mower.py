...
from .const import CONF_ENCODING, CONF_QOS, CONF_RETAIN, DEFAULT_OPTIMISTIC, DEFAULT_RETAIN
...
PLATFORM_SCHEMA_MODERN = MQTT_BASE_SCHEMA.extend({vol.Optional(CONF_ACTIVITY_VALUE_TEMPLATE): cv.template, vol.Optional(CONF_ACTIVITY_STATE_TOPIC): valid_subscribe_topic, vol.Optional(CONF_DOCK_COMMAND_TEMPLATE): cv.template, vol.Optional(CONF_DOCK_COMMAND_TOPIC): valid_publish_topic, vol.Optional(CONF_NAME): vol.Any(cv.string, None), vol.Optional(CONF_OPTIMISTIC, default=DEFAULT_OPTIMISTIC): cv.boolean, vol.Optional(CONF_PAUSE_COMMAND_TEMPLATE): cv.template, vol.Optional(CONF_PAUSE_COMMAND_TOPIC): valid_publish_topic, vol.Optional(CONF_RETAIN, default=DEFAULT_RETAIN): cv.boolean, vol.Optional(CONF_START_MOWING_COMMAND_TEMPLATE): cv.template, vol.Optional(CONF_START_MOWING_COMMAND_TOPIC): valid_publish_topic}).extend(MQTT_ENTITY_COMMON_SCHEMA.schema)
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    await async_setup_entity_entry_helper(hass, config_entry, MqttLawnMower, lawn_mower.DOMAIN, async_add_entities, DISCOVERY_SCHEMA, PLATFORM_SCHEMA_MODERN)
...
class MqttLawnMower(MqttEntity, LawnMowerEntity, RestoreEntity):
    ...
    async def _subscribe_topics(self) -> None:
        ...
        if self._attr_assumed_state and (last_state := (await self.async_get_last_state())):
    ...
    async def _async_operate(self, option: str, activity: LawnMowerActivity) -> None:
        ...
        await self.async_publish(self._command_topics[option], payload, self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
