...
class SandwichModel(nn.Module):
    ...
    def __init__(self, fea_dim, cnn_dim_1, cnn_dim_2, cnn_kernel_size, rnn_dim_1, rnn_dim_2, rnn_dups, rnn_layers, dropout, device, **params):
        if False:
            print('Hello World!')
        ...
        super().__init__()
        self.first_encoder = CNNKRNNEncoder(cnn_input_dim=fea_dim, cnn_output_dim=cnn_dim_1, cnn_kernel_size=cnn_kernel_size, rnn_output_dim=rnn_dim_1, rnn_dup_num=rnn_dups, rnn_layers=rnn_layers, dropout=dropout, device=device)
        self.second_encoder = CNNKRNNEncoder(cnn_input_dim=rnn_dim_1, cnn_output_dim=cnn_dim_2, cnn_kernel_size=cnn_kernel_size, rnn_output_dim=rnn_dim_2, rnn_dup_num=rnn_dups, rnn_layers=rnn_layers, dropout=dropout, device=device)
...
class Sandwich(Model):
    ...
    def __init__(self, fea_dim=6, cnn_dim_1=64, cnn_dim_2=32, cnn_kernel_size=3, rnn_dim_1=16, rnn_dim_2=8, rnn_dups=3, rnn_layers=2, dropout=0, n_epochs=200, lr=0.001, metric='', batch_size=2000, early_stop=20, loss='mse', optimizer='adam', GPU=0, seed=None, **kwargs):
        if False:
            while True:
                i = 10
        ...
        self.seed = seed
        self.logger.info('Sandwich parameters setting:\nfea_dim : {}\ncnn_dim_1 : {}\ncnn_dim_2 : {}\ncnn_kernel_size : {}\nrnn_dim_1 : {}\nrnn_dim_2 : {}\nrnn_dups : {}\nrnn_layers : {}\ndropout : {}\nn_epochs : {}\nlr : {}\nmetric : {}\nbatch_size: {}\nearly_stop : {}\noptimizer : {}\nloss_type : {}\nvisible_GPU : {}\nuse_GPU : {}\nseed : {}'.format(fea_dim, cnn_dim_1, cnn_dim_2, cnn_kernel_size, rnn_dim_1, rnn_dim_2, rnn_dups, rnn_layers, dropout, n_epochs, lr, metric, batch_size, early_stop, optimizer.lower(), loss, GPU, self.use_gpu, seed))
        if self.seed is not None:
            np.random.seed(self.seed)
            torch.manual_seed(self.seed)
        self.sandwich_model = SandwichModel(fea_dim=self.fea_dim, cnn_dim_1=self.cnn_dim_1, cnn_dim_2=self.cnn_dim_2, cnn_kernel_size=self.cnn_kernel_size, rnn_dim_1=self.rnn_dim_1, rnn_dim_2=self.rnn_dim_2, rnn_dups=self.rnn_dups, rnn_layers=self.rnn_layers, dropout=self.dropout, device=self.device)
