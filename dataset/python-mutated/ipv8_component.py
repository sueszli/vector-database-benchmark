...
class Ipv8Component(Component):
    ...
    async def run(self):
        ...
        if config.ipv8.rendezvous_stats:
            self.rendezvous_db = RendezvousDatabase(db_path=self.session.config.state_dir / STATEDIR_DB_DIR / self.RENDEZVOUS_DB_NAME)
        ...
        ipv8_config_builder = ConfigBuilder().set_port(port).set_address(address).clear_overlays().clear_keys().set_working_directory(str(config.state_dir)).set_walker_interval(config.ipv8.walk_interval)
        if config.gui_test_mode:
            endpoint = DispatcherEndpoint([])
        else:
            endpoint = DispatcherEndpoint(['UDPIPv4'], UDPIPv4={'port': port, 'ip': address})
        ipv8 = IPv8(ipv8_config_builder.finalize(), enable_statistics=config.ipv8.statistics and (not config.gui_test_mode), endpoint_override=endpoint)
        if config.ipv8.rendezvous_stats:
        ...
        if config.ipv8.walk_scaling_enabled and (not config.gui_test_mode):
            ...
            IPv8Monitor(ipv8, config.ipv8.walk_interval, config.ipv8.walk_scaling_upper_limit).start(self._task_manager)
