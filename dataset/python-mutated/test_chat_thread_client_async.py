...
async def test_send_message():
    ...
    try:
        ...
        metadata = {'tags': 'tag'}
        create_message_result = await chat_thread_client.send_message(content, sender_display_name=sender_display_name, metadata=metadata)
...
async def test_send_message_w_type():
    ...
    for chat_message_type in chat_message_types:
        ...
        try:
            content = 'hello world'
            sender_display_name = 'sender name'
            create_message_result = await chat_thread_client.send_message(content, chat_message_type=chat_message_type, sender_display_name=sender_display_name)
            create_message_result_id = create_message_result.id
        except:
            raised = True
        assert raised == False
...
async def test_list_read_receipts_with_results_per_page_and_skip():
    ...
    async def mock_send(*_, **__):
        return mock_response(status_code=200, json_payload={'value': [{'chatMessageId': message_id_1, 'senderCommunicationIdentifier': {'rawId': 'string', 'communicationUser': {'id': 'string'}}}]})
    chat_thread_client = ChatThreadClient('https://endpoint', credential, thread_id, transport=Mock(send=mock_send))
    read_receipts = None
    try:
