...
from ...core.types import AttributeBulkUpdateError, BaseInputObjectType, BaseObjectType, NonNullList
from ...core.utils import WebhookEventAsyncType, WebhookEventInfo, from_global_id_or_error, get_duplicated_values
...
class AttributeBulkUpdateResult(BaseObjectType):
    ...
    errors = NonNullList(AttributeBulkUpdateError, required=False, description='List of errors occurred on update attempt.')
...
class AttributeBulkUpdateInput(BaseInputObjectType):
    ...
    external_reference = graphene.String(description='External ID of this attribute.', required=False)
...
class AttributeBulkUpdate(BaseMutation):
    count = graphene.Int(required=True, description='Returns how many objects were updated.')
    results = NonNullList(AttributeBulkUpdateResult, required=True, default_value=[], description='List of the updated attributes.')

    class Arguments:
        attributes = NonNullList(AttributeBulkUpdateInput, required=True, description='Input list of attributes to update.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.ATTRIBUTE_UPDATED, description='An attribute was updated. Optionally called when new attribute value was created or deleted.'), WebhookEventInfo(type=WebhookEventAsyncType.ATTRIBUTE_VALUE_CREATED, description='Called optionally when an attribute value was created.'), WebhookEventInfo(type=WebhookEventAsyncType.ATTRIBUTE_VALUE_DELETED, description='Called optionally when an attribute value was deleted.')]
