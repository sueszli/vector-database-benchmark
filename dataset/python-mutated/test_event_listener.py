...
def _check_skip(grains):
    ...
    if grains['os'] == 'MacOS':
        return True
    return False
pytestmark = [pytest.mark.slow_test, pytest.mark.skip_initial_gh_actions_failure(skip=_check_skip)]
...
async def test_simple(sock_dir):
    ...
    with eventpublisher_process(sock_dir):
        with salt.utils.event.MasterEvent(sock_dir) as me:
            request = Request()
            event_listener = saltnado.EventListener({}, {'sock_dir': sock_dir, 'transport': 'zeromq'})
            await asyncio.sleep(1)
            event_future = event_listener.get_event(request, 'evt1')
...
async def test_set_event_handler(sock_dir):
    ...
    with eventpublisher_process(sock_dir):
        with salt.utils.event.MasterEvent(sock_dir) as me:
            ...
            event_listener = saltnado.EventListener({}, {'sock_dir': sock_dir, 'transport': 'zeromq'})
            await asyncio.sleep(1)
            event_future = event_listener.get_event(request, tag='evt', timeout=1)
            me.fire_event({'data': 'foo'}, 'evt')
            await event_future
...
async def test_timeout(sock_dir):
    ...
    with eventpublisher_process(sock_dir):
        ...
        event_listener = saltnado.EventListener({}, {'sock_dir': sock_dir, 'transport': 'zeromq'})
        await asyncio.sleep(1)
        event_future = event_listener.get_event(request, tag='evt1', timeout=1)
...
async def test_clean_by_request(sock_dir, io_loop):
    ...
    with eventpublisher_process(sock_dir):
        ...
        with salt.utils.event.MasterEvent(sock_dir) as me:
            ...
            dummy_request_future_2 = event_listener.get_event(request2, timeout=10, tag='evt4')
