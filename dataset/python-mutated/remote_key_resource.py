...
from synapse.http.servlet import RestServlet, parse_and_validate_json_object_from_request, parse_integer
...
class RemoteKey(RestServlet):
    ...
    def register(self, http_server: HttpServer) -> None:
        if False:
            print('Hello World!')
        http_server.register_paths('GET', (re.compile('^/_matrix/key/v2/query/(?P<server>[^/]*)(/(?P<key_id>[^/]*))?$'),), self.on_GET, self.__class__.__name__)
        http_server.register_paths('POST', (re.compile('^/_matrix/key/v2/query$'),), self.on_POST, self.__class__.__name__)

    async def on_GET(self, request: Request, server: str, key_id: Optional[str]=None) -> Tuple[int, JsonDict]:
        if server and key_id:
            logger.warning('Request for remote server key with deprecated key ID (logging to determine usage level for future removal): %s / %s', server, key_id)
            minimum_valid_until_ts = parse_integer(request, 'minimum_valid_until_ts')
            query = {server: {key_id: _KeyQueryCriteriaDataModel(minimum_valid_until_ts=minimum_valid_until_ts)}}
        else:
            query = {server: {}}
    ...
    async def query_keys(self, query: Dict[str, Dict[str, _KeyQueryCriteriaDataModel]], query_remote_on_cache_miss: bool=False) -> JsonDict:
        logger.info('Handling query for keys %r', query)
        server_keys: Dict[Tuple[str, str], Optional[FetchKeyResultForRemote]] = {}
        for (server_name, key_ids) in query.items():
            if key_ids:
                results: Mapping[str, Optional[FetchKeyResultForRemote]] = await self.store.get_server_keys_json_for_remote(server_name, key_ids)
            else:
                results = await self.store.get_all_server_keys_json_for_remote(server_name)
            server_keys.update((((server_name, key_id), res) for (key_id, res) in results.items()))
        ...
        for ((server_name, key_id), key_result) in server_keys.items():
            ...
            if key_result is None:
                ...
                req_key = query.get(server_name, {}).get(key_id, _KeyQueryCriteriaDataModel(minimum_valid_until_ts=None))
                req_valid_until = req_key.minimum_valid_until_ts
                if req_valid_until is not None:
                    if ts_valid_until_ms < req_valid_until:
                        logger.debug('Cached response for %r/%r is older than requested: valid_until (%r) < minimum_valid_until (%r)', server_name, key_id, ts_valid_until_ms, req_valid_until)
                        miss = True
                    else:
                        logger.debug('Cached response for %r/%r is newer than requested: valid_until (%r) >= minimum_valid_until (%r)', server_name, key_id, ts_valid_until_ms, req_valid_until)
                elif (ts_added_ms + ts_valid_until_ms) / 2 < time_now_ms:
                    logger.debug('Cached response for %r/%r is too old: (added (%r) + valid_until (%r)) / 2 < now (%r)', server_name, key_id, ts_added_ms, ts_valid_until_ms, time_now_ms)
                    miss = True
                else:
                    logger.debug('Cached response for %r/%r is still valid: (added (%r) + valid_until (%r)) / 2 < now (%r)', server_name, key_id, ts_added_ms, ts_valid_until_ms, time_now_ms)
                json_results.add(key_result.key_json)
            if miss and query_remote_on_cache_miss:
                if self.federation_domain_whitelist is None or server_name in self.federation_domain_whitelist:
                    cache_misses.setdefault(server_name, {})[key_id] = 0
        if cache_misses:
            await yieldable_gather_results(lambda t: self.fetcher.get_keys(*t), ((server_name, list(keys), 0) for (server_name, keys) in cache_misses.items()))
