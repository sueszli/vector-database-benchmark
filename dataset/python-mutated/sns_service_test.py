...
test_policy = {'Statement': [{'Effect': 'Allow', 'Principal': {'AWS': f'{AWS_ACCOUNT_NUMBER}'}, 'Action': ['sns:Publish'], 'Resource': f'arn:aws:sns:{AWS_REGION}:{AWS_ACCOUNT_NUMBER}:{topic_name}'}]}
...
def mock_make_api_call(self, operation_name, kwarg):
    ...
    if operation_name == 'GetTopicAttributes':
        return {'Attributes': {'Policy': dumps(test_policy), 'KmsMasterKeyId': kms_key_id}}
...
class Test_SNS_Service:
    ...
    def test__list_topics__(self):
        ...
        sns_client.create_topic(Name=topic_name, Tags=[{'Key': 'test', 'Value': 'test'}])
        audit_info = self.set_mocked_audit_info()
        sns = SNS(audit_info)
        assert len(sns.topics) == 1
        assert sns.topics[0].name == topic_name
        assert sns.topics[0].arn == f'arn:aws:sns:{AWS_REGION}:{AWS_ACCOUNT_NUMBER}:{topic_name}'
        assert sns.topics[0].region == AWS_REGION
        assert sns.topics[0].tags == [{'Key': 'test', 'Value': 'test'}]
