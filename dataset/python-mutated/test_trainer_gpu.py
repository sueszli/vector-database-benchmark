...
from modelscope.utils.test_utils import DistributedTestCase, create_dummy_test_dataset, test_level
...
dummy_dataset_small = create_dummy_test_dataset(np.random.random(size=(5,)), np.random.randint(0, 4, (1,)), 20)
dummy_dataset_big = create_dummy_test_dataset(np.random.random(size=(5,)), np.random.randint(0, 4, (1,)), 40)
...
@unittest.skipIf(not torch.cuda.is_available() or torch.cuda.device_count() <= 1 or version.parse(torch.__version__) >= version.parse('1.9.0'), 'skip on torch 1.9 or above')
...
@unittest.skipIf(not torch.cuda.is_available() or torch.cuda.device_count() <= 1, 'skip on torch 1.9 or above')
