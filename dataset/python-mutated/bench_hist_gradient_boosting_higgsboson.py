...
def load_data():
    ...
    toc = time()
    print(f'Loaded {df.values.nbytes / 1000000000.0:0.3f} GB in {toc - tic:0.3f}s')
    return df
...
est = HistGradientBoostingClassifier(loss='log_loss', learning_rate=lr, max_iter=n_trees, max_bins=max_bins, max_leaf_nodes=n_leaf_nodes, early_stopping=False, random_state=0, verbose=1, interaction_cst=interaction_cst, max_features=max_features)
