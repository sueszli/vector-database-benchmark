...
from flet_core.protocol import ClientActions, ClientMessage, Command, CommandEncoder, PageCommandResponsePayload, PageCommandsBatchResponsePayload, RegisterWebClientRequestPayload
...
class FletApp(LocalConnection):
    ...
    async def handle(self, websocket: WebSocket):
        ...
        * `websocket` (WebSocket) - Websocket instance.
        ...
        if not self.__upload_endpoint_path:
            self.__upload_endpoint_path = f"{('' if self.page_name == '' else '/')}{self.page_name}/upload"
    ...
    async def __on_event(self, e):
        session = await app_manager.get_session(self.__get_unique_session_id(e.sessionID))
        if session is not None:
            ...
            if e.eventTarget == 'page' and e.eventName == 'close':
                ...
                await app_manager.delete_session(self.__get_unique_session_id(e.sessionID))
    ...
    async def __on_session_created(self, session_data):
        ...
        try:
            ...
            print(f'Unhandled error processing page session {self.__page.session_id}:', traceback.format_exc())
            await self.__page.error_async(f'There was an error while processing your request: {e}')
    ...
    async def __receive_loop(self):
        try:
            ...
            if self.__page:
                await app_manager.disconnect_session(self.__get_unique_session_id(self.__page.session_id), self.__session_timeout_seconds)
    ...
    async def __on_message(self, data: str):
        ...
        if msg.action == ClientActions.REGISTER_WEB_CLIENT:
            ...
            new_session = True
            if not self._client_details.sessionId or await app_manager.get_session(self.__get_unique_session_id(self._client_details.sessionId)) is None:
                self._client_details.sessionId = random_string(16)
                self.__page = Page(self, self._client_details.sessionId)
                await app_manager.add_session(self.__get_unique_session_id(self._client_details.sessionId), self.__page)
            else:
                logger.info(f'Existing session requested: {self._client_details.sessionId}')
                self.__page = await app_manager.get_session(self.__get_unique_session_id(self._client_details.sessionId))
                new_session = False
            ...
            self.__page._set_attr('platformBrightness', self._client_details.platformBrightness, False)
            ...
            self.__page._set_attr('windowWidth', self._client_details.windowWidth, False)
            self.__page._set_attr('windowHeight', self._client_details.windowHeight, False)
            ...
            self.__page._set_attr('clientIP', self.__websocket.client.host if self.__websocket.client else '', False)
            self.__page._set_attr('clientUserAgent', self.__websocket.headers['user-agent'] if 'user-agent' in self.__websocket.headers else '', False)
            p = self.__page.snapshot.get('page')
            if not p:
                p = {'i': 'page', 't': 'page', 'p': '', 'c': []}
                self.__page.snapshot['page'] = p
            self.__page.copy_attrs(p)
            await self.__send(self._create_register_web_client_response(controls=self.__page.snapshot))
            if new_session:
                asyncio.create_task(self.__on_session_created(self._create_session_handler_arg()))
            else:
                await app_manager.reconnect_session(self.__get_unique_session_id(self._client_details.sessionId), self)
            ...
            if self.__on_event is not None:
                asyncio.create_task(self.__on_event(self._create_page_event_handler_arg(msg)))
        elif msg.action == ClientActions.UPDATE_CONTROL_PROPS:
            if self.__on_event is not None:
                asyncio.create_task(self.__on_event(self._create_update_control_props_handler_arg(msg)))
        else:
    ...
    async def __process_oauth_authorize_command(self, attrs: Dict[str, Any]):
        ...
        state = OAuthState(session_id=self.__get_unique_session_id(self._client_details.sessionId), expires_at=datetime.utcnow() + timedelta(seconds=self.__oauth_state_timeout_seconds), complete_page_html=attrs.get('completePageHtml', None), complete_page_url=attrs.get('completePageUrl', None))
    ...
    async def send_command_async(self, session_id: str, command: Command):
        if command.name == 'oauthAuthorize':
            (result, message) = await self.__process_oauth_authorize_command(command.attrs)
        else:
    ...
    async def send_commands_async(self, session_id: str, commands: List[Command]):
        ...
        if len(messages) > 0:
            await self.__send(ClientMessage(ClientActions.PAGE_CONTROLS_BATCH, messages))
