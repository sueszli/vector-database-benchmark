...
class eks_control_plane_logging_all_types_enabled(Check):
    ...
    def execute(self):
        ...
        for cluster in eks_client.clusters:
            ...
            report.status = 'FAIL'
            report.status_extended = f'Control plane logging is not enabled for EKS cluster {cluster.name}.'
            if cluster.logging and cluster.logging.enabled:
                if all((item in cluster.logging.types for item in ['api', 'audit', 'authenticator', 'controllerManager', 'scheduler'])):
                    report.status = 'PASS'
