...
mse_model = ess / (wls_fit.df_model + 1)
...
for weights in [1 / incomesq, 1 / incomesq ** 2, np.sqrt(incomesq)]:
    ...
    wls_fit_rm2 = sm.WLS(data.endog[olskeep], data.exog[olskeep, :], weights=weights[olskeep]).fit()
...
for i in range(4):
    ...
    plt.title('var%d' % i)
plt.figtext(0.5, 0.935, 'OLS Bootstrap', ha='center', color='black', weight='bold', size='large')
...
for i in range(4):
    ...
    plt.title('var%d' % i)
plt.figtext(0.5, 0.935, 'WLS rm2 Bootstrap', ha='center', color='black', weight='bold', size='large')
