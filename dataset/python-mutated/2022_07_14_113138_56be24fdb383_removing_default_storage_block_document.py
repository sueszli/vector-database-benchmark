...
def downgrade():
    if False:
        ...
        return i + 15
    with op.batch_alter_table('block_document', schema=None) as batch_op:
        batch_op.add_column(sa.Column('is_default_storage_block_document', sa.BOOLEAN(), server_default=sa.text("'0'"), nullable=True))
        batch_op.create_index('ix_block_document__is_default_storage_block_document', ['is_default_storage_block_document'], unique=False)