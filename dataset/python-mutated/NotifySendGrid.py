...
SENDGRID_HTTP_ERROR_MAP = {401: 'Unauthorized - You do not have authorization to make the request.', 413: 'Payload To Large - The JSON payload you have included in your request is too large.', 429: 'Too Many Requests - The number of requests you have made exceeds SendGrid’s rate limitations.'}
...
class NotifySendGrid(NotifyBase):
    ...
    templates = ('{schema}://{apikey}:{from_email}', '{schema}://{apikey}:{from_email}/{targets}')
    template_tokens = dict(NotifyBase.template_tokens, **{'apikey': {'name': _('API Key'), 'type': 'string', 'private': True, 'required': True, 'regex': ('^[A-Z0-9._-]+$', 'i')}, 'from_email': {'name': _('Source Email'), 'type': 'string', 'required': True}, 'target_email': {'name': _('Target Email'), 'type': 'string', 'map_to': 'targets'}, 'targets': {'name': _('Targets'), 'type': 'list:string'}})
    template_args = dict(NotifyBase.template_args, **{'to': {'alias_of': 'targets'}, 'cc': {'name': _('Carbon Copy'), 'type': 'list:string'}, 'bcc': {'name': _('Blind Carbon Copy'), 'type': 'list:string'}, 'template': {'name': _('Template'), 'type': 'string'}})
    template_kwargs = {'template_data': {'name': _('Template Data'), 'prefix': '+'}}
