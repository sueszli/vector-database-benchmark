...
with DAG(dag_id=DAG_ID, schedule='@once', start_date=datetime(2022, 1, 1), catchup=False, tags=['example']) as dag:
    ...
    upload_csv = S3CreateObjectOperator(task_id='upload_csv', s3_bucket=bucket_name, s3_key='source_folder/data.csv', data='col1,col2\n"data1","data2"\n', replace=True)
    run_flow = AppflowRunOperator(task_id='run_flow', flow_name=flow_name)
    ...
    chain(test_context, create_bucket, setup_bucket_permissions(bucket_name), upload_csv, create_s3_to_s3_flow(flow_name, bucket_name, source_folder), run_flow, delete_flow(flow_name), delete_bucket)
