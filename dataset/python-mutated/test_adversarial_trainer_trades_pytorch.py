...
def get_adv_trainer(framework, image_dl_estimator):
    ...
    def _get_adv_trainer():
        ...
        if framework == 'pytorch':
            ...
            attack = ProjectedGradientDescent(classifier, norm=np.inf, eps=0.3, eps_step=0.03, max_iter=20, targeted=False, num_random_init=1, batch_size=128, verbose=False)
...
@pytest.mark.only_with_platform('pytorch')
@pytest.mark.parametrize('label_format', ['one_hot', 'numerical'])
def test_adversarial_trainer_trades_pytorch_fit_generator_and_predict(get_adv_trainer, fix_get_mnist_subset, image_data_generator, label_format):
    if False:
        for i in range(10):
            print('nop')
    (x_train_mnist, y_train_mnist, x_test_mnist, y_test_mnist) = fix_get_mnist_subset
