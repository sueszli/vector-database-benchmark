...
class workflowsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_workflow': ('parent', 'workflow', 'workflow_id'), 'delete_workflow': ('name',), 'get_workflow': ('name', 'revision_id'), 'list_workflows': ('parent', 'page_size', 'page_token', 'filter', 'order_by'), 'update_workflow': ('workflow', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=workflowsCallTransformer()):
    if False:
        i = 10
        return i + 15
