...
from .....core.jwt import JWT_REFRESH_TOKEN_COOKIE_NAME, JWT_REFRESH_TYPE, create_access_token
...
class RefreshToken(BaseMutation):
    ...
    class Arguments:
        ...
        csrf_token = graphene.String(required=False, description='CSRF token required to refresh token. This argument is required when `refreshToken` is provided as a cookie.')

    class Meta:
        description = f'Refresh JWT token. Mutation tries to take refreshToken from the input. If it fails it will try to take `refreshToken` from the http-only cookie `{JWT_REFRESH_TOKEN_COOKIE_NAME}`. `csrfToken` is required when `refreshToken` is provided as a cookie.'
