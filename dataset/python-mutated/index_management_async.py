...
async def explicitly_exclude_from_index(db):
    ...
    try:
        ...
        doc2 = await created_Container.create_item(body={'id': 'doc2', 'orderId': 'order2'}, indexing_directive=documents.IndexingDirective.Exclude)
...
async def use_manual_indexing(db):
    ...
    try:
        await delete_container_if_exists(db, CONTAINER_ID)
        created_Container = await db.create_container(id=CONTAINER_ID, indexing_policy={'automatic': False}, partition_key=PARTITION_KEY)
        properties = await created_Container.read()
        print(created_Container)
        ...
        print('Document read by ID: \n', docRead['id'])
        doc2 = await created_Container.create_item(body={'id': 'doc2', 'orderId': 'order2'}, indexing_directive=documents.IndexingDirective.Include)
        print('\n' + '-' * 25 + 'Document doc2 created with order2' + '-' * 25)
        print(doc2)
        query = {'query': 'SELECT * FROM r WHERE r.orderId=@orderNo', 'parameters': [{'name': '@orderNo', 'value': 'order2'}]}
        await query_documents_with_custom_query(created_Container, query)
...
async def exclude_paths_from_index(db):
    ...
    try:
        ...
        created_Container = await db.create_container(id=container_to_create['id'], indexing_policy=container_to_create['indexingPolicy'], partition_key=PARTITION_KEY)
        properties = await created_Container.read()
        print(created_Container)
...
async def use_range_indexes_on_strings(db):
    ...
    try:
        ...
        created_Container = await db.create_container(id=container_definition['id'], indexing_policy=container_definition['indexingPolicy'], partition_key=PARTITION_KEY)
        properties = await created_Container.read()
        print(created_Container)
...
async def perform_index_transformations(db):
    try:
        ...
        created_Container = await db.replace_container(container=created_Container.id, partition_key=PARTITION_KEY, indexing_policy=properties['indexingPolicy'])
        properties = await created_Container.read()
        print_dictionary_items(properties['indexingPolicy'])
...
async def perform_multi_orderby_query(db):
    try:
        ...
        created_container = await db.create_container(id=CONTAINER_ID, indexing_policy=indexing_policy, partition_key=PARTITION_KEY)
        print(created_container)
        properties = await created_container.read()
