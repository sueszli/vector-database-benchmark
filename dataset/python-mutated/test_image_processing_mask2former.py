...
class Mask2FormerImageProcessingTester(unittest.TestCase):
    ...
    def get_fake_mask2former_outputs(self):
        if False:
            for i in range(10):
                print('nop')
        return Mask2FormerForUniversalSegmentationOutput(class_queries_logits=torch.randn((self.batch_size, self.num_queries, self.num_classes + 1)), masks_queries_logits=torch.randn((self.batch_size, self.num_queries, self.height, self.width)))
