...
def window_processor(key, events):
    ...
    print(f'processing window:{len(values)} events,mean: {mean:.2f},timestamp {timestamp}')
    sink.send_soon(value=AggModel(date=timestamp, count=count, mean=mean))
tumbling_table = app.Table(TABLE, default=list, partitions=PARTITIONS, on_window_close=window_processor).tumbling(WINDOW, expires=timedelta(seconds=WINDOW_EXPIRES)).relative_to_field(RawModel.date)
