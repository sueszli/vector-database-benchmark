...
for p in [DEVPATH] + list(DEPS_PATH.iterdir()):
    if p.name.startswith('.') or (not p.is_dir() and (not ((p / 'setup.py').exists() or (p / 'pyproject.toml').exists()))):
        continue
    try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--install', nargs='+', default=REPOS.keys(), help='Space-separated list of distribution names to install, e.g. qtconsole spyder-kernels. If option not provided, then all of the repos in spyder/external-deps are installed')
    parser.add_argument('--no-install', nargs='+', default=[], help='Space-separated list of distribution names to exclude from install. Default is empty list.')
    parser.add_argument('--not-editable', action='store_true', default=False, help='Install in standard mode, not editable mode.')
