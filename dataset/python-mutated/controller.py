...
from starlite import BackgroundTasks, Controller, MediaType, Provide, Request, Response, WebSocket, get, post, websocket
...
from strawberry.subscriptions.protocols.graphql_transport_ws import WS_4406_PROTOCOL_NOT_ACCEPTABLE
from .handlers.graphql_transport_ws_handler import GraphQLTransportWSHandler as BaseGraphQLTransportWSHandler
...
if TYPE_CHECKING:
    ...
    MergedContext = Union['BaseContext', Union[Dict[str, Any], Dict[str, BackgroundTasks], Dict[str, Request[Any, Any]], Dict[str, Response[Any]], Dict[str, websocket]]]
CustomContext = Union['BaseContext', Dict[str, Any]]

async def _context_getter(custom_context: Optional[CustomContext], request: Request[Any, Any], response: Response[Any]) -> MergedContext:
    if isinstance(custom_context, BaseContext):
        custom_context.request = request
        return custom_context
    default_context = {'request': request, 'response': response}
