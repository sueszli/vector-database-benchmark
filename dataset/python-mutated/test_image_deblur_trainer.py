...
from modelscope.msdatasets.dataset_cls.custom_datasets.gopro_image_deblurring_dataset import GoproImageDeblurringDataset
...
class ImageDeblurTrainerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        dataset_train = MsDataset.load('GOPRO', namespace='damo', subset_name='default', split='test', download_mode=DownloadMode.REUSE_DATASET_IF_EXISTS)._hf_ds
        dataset_val = MsDataset.load('GOPRO', namespace='damo', subset_name='subset', split='test', download_mode=DownloadMode.REUSE_DATASET_IF_EXISTS)._hf_ds
        self.dataset_train = GoproImageDeblurringDataset(dataset_train, self.config.dataset, is_train=True)
        self.dataset_val = GoproImageDeblurringDataset(dataset_val, self.config.dataset, is_train=False)
