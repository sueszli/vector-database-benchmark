...
__all__ = ['AppletHostSerializer', 'AppletHostDeploymentSerializer', 'AppletHostAccountSerializer', 'AppletHostAppletReportSerializer', 'AppletHostStartupSerializer']

class DeployOptionsSerializer(serializers.Serializer):
    LICENSE_MODE_CHOICES = ((4, _('Per Session')), (2, _('Per Device')))
    SESSION_PER_USER = ((0, _('Disabled')), (1, _('Enabled')))
    CORE_HOST = serializers.CharField(default=settings.SITE_URL, label=_('Core API'), max_length=1024, help_text=_(' \n        Tips: The application release machine communicates with the Core service. \n        If the release machine and the Core service are on the same network segment, \n        it is recommended to fill in the intranet address, otherwise fill in the current site URL \n        <br> \n        eg: https://172.16.10.110 or https://dev.jumpserver.com\n        '))
    ...
    RDS_fSingleSessionPerUser = serializers.ChoiceField(choices=SESSION_PER_USER, default=1, label=_('RDS Single Session Per User'))
...
class AppletHostSerializer(HostSerializer):
    ...
    load = LabeledChoiceField(read_only=True, label=_('Load status'), choices=const.ComponentLoad.choices)

    class Meta(HostSerializer.Meta):
        model = AppletHost
        fields = HostSerializer.Meta.fields + ['auto_create_accounts', 'accounts_create_amount', 'load', 'date_synced', 'deploy_options', 'using_same_account']
        extra_kwargs = {**HostSerializer.Meta.extra_kwargs, 'date_synced': {'read_only': True}, 'auto_create_accounts': {'help_text': _('These accounts are used to connect to the published application, the account is now divided into two types, one is dedicated to each account, each user has a private account, the other is public, when the application does not support multiple open and the special has been used, the public account will be used to connect')}, 'accounts_create_amount': {'help_text': _('The number of public accounts created automatically')}, 'using_same_account': {'help_text': _('Connect to the host using the same account first. For security reasons, please set the configuration item CACHE_LOGIN_PASSWORD_ENABLED=true and restart the service to enable it.')}}
...
class AppletHostDeploymentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['status', 'date_created', 'date_updated', 'date_start', 'date_finished']
