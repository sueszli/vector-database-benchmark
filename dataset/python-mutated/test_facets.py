...
async def test_column_facet_suggest(ds_client):
    facet = ColumnFacet(ds_client.ds, Request.fake('/'), database='fixtures', sql='select * from facetable', table='facetable')
...
async def test_column_facet_suggest_skip_if_already_selected(ds_client):
    facet = ColumnFacet(ds_client.ds, Request.fake('/?_facet=planet_int&_facet=on_earth'), database='fixtures', sql='select * from facetable', table='facetable')
    suggestions = await facet.suggest()
...
async def test_column_facet_suggest_skip_if_enabled_by_metadata(ds_client):
    facet = ColumnFacet(ds_client.ds, Request.fake('/'), database='fixtures', sql='select * from facetable', table='facetable', metadata={'facets': ['_city_id']})
    suggestions = [s['name'] for s in await facet.suggest()]
    assert ['created', 'planet_int', 'on_earth', 'state', '_neighborhood', 'tags', 'complex_array'] == suggestions

@pytest.mark.asyncio
async def test_column_facet_results(ds_client):
    facet = ColumnFacet(ds_client.ds, Request.fake('/?_facet=_city_id'), database='fixtures', sql='select * from facetable', table='facetable')
...
async def test_column_facet_from_metadata_cannot_be_hidden(ds_client):
    ...
    (buckets, timed_out) = await facet.facet_results()
    assert [] == timed_out
    assert [{'name': '_city_id', 'type': 'column', 'hideable': False, 'toggle_url': '/', 'results': [{'value': 1, 'label': 'San Francisco', 'count': 6, 'toggle_url': 'http://localhost/?_city_id__exact=1', 'selected': False}, {'value': 2, 'label': 'Los Angeles', 'count': 4, 'toggle_url': 'http://localhost/?_city_id__exact=2', 'selected': False}, {'value': 3, 'label': 'Detroit', 'count': 4, 'toggle_url': 'http://localhost/?_city_id__exact=3', 'selected': False}, {'value': 4, 'label': 'Memnonia', 'count': 1, 'toggle_url': 'http://localhost/?_city_id__exact=4', 'selected': False}], 'truncated': False}] == buckets

@pytest.mark.asyncio
@pytest.mark.skipif(not detect_json1(), reason='Requires the SQLite json1 module')
async def test_array_facet_suggest(ds_client):
    facet = ArrayFacet(ds_client.ds, Request.fake('/'), database='fixtures', sql='select * from facetable', table='facetable')
    suggestions = await facet.suggest()
    assert [{'name': 'tags', 'type': 'array', 'toggle_url': 'http://localhost/?_facet_array=tags'}] == suggestions

@pytest.mark.asyncio
@pytest.mark.skipif(not detect_json1(), reason='Requires the SQLite json1 module')
async def test_array_facet_suggest_not_if_all_empty_arrays(ds_client):
    facet = ArrayFacet(ds_client.ds, Request.fake('/'), database='fixtures', sql="select * from facetable where tags = '[]'", table='facetable')
    suggestions = await facet.suggest()
    assert [] == suggestions
...
async def test_array_facet_results(ds_client):
    facet = ArrayFacet(ds_client.ds, Request.fake('/?_facet_array=tags'), database='fixtures', sql='select * from facetable', table='facetable')
    (buckets, timed_out) = await facet.facet_results()
    assert [] == timed_out
...
async def test_json_array_with_blanks_and_nulls():
    ...
    data = response.json()
    assert data['suggested_facets'] == [{'name': 'json_column', 'type': 'array', 'toggle_url': 'http://localhost/test_json_array/foo.json?_extra=suggested_facets&_facet_array=json_column'}]
...
async def test_facet_size():
    ...
    for i in range(1, 51):
        for j in range(1, 4):
            await db.execute_write('insert into neighbourhoods (city, neighbourhood) values (?, ?)', ['City {}'.format(i), 'Neighbourhood {}'.format(j)])
    response = await ds.client.get('/test_facet_size/neighbourhoods.json?_extra=suggested_facets')
    data = response.json()
    assert data['suggested_facets'] == [{'name': 'neighbourhood', 'toggle_url': 'http://localhost/test_facet_size/neighbourhoods.json?_extra=suggested_facets&_facet=neighbourhood'}]
    response2 = await ds.client.get('/test_facet_size/neighbourhoods.json?_facet_size=50&_extra=suggested_facets')
    data2 = response2.json()
    assert sorted(data2['suggested_facets'], key=lambda f: f['name']) == [{'name': 'city', 'toggle_url': 'http://localhost/test_facet_size/neighbourhoods.json?_facet_size=50&_extra=suggested_facets&_facet=city'}, {'name': 'neighbourhood', 'toggle_url': 'http://localhost/test_facet_size/neighbourhoods.json?_facet_size=50&_extra=suggested_facets&_facet=neighbourhood'}]
    response3 = await ds.client.get('/test_facet_size/neighbourhoods.json?_facet_size=50&_facet=city')
    data3 = response3.json()
    assert len(data3['facet_results']['results']['city']['results']) == 50
    ds._settings['max_returned_rows'] = 20
    response4 = await ds.client.get('/test_facet_size/neighbourhoods.json?_facet_size=50&_facet=city')
    data4 = response4.json()
    assert len(data4['facet_results']['results']['city']['results']) == 20
    response5 = await ds.client.get('/test_facet_size/neighbourhoods.json?_facet_size=max&_facet=city')
    data5 = response5.json()
    assert len(data5['facet_results']['results']['city']['results']) == 20
    orig_metadata = ds._metadata_local
    try:
        ds._metadata_local = {'databases': {'test_facet_size': {'tables': {'neighbourhoods': {'facet_size': 6}}}}}
        ...
        ds._metadata_local['databases']['test_facet_size']['tables']['neighbourhoods']['facet_size'] = 'max'
        data7 = (await ds.client.get('/test_facet_size/neighbourhoods.json?_facet=city')).json()
...
def test_other_types_of_facet_in_metadata():
    if False:
        i = 10
        return i + 15
    with make_app_client(metadata={'databases': {'fixtures': {'tables': {'facetable': {'facets': ['state', {'array': 'tags'}, {'date': 'created'}]}}}}}) as client:
        response = client.get('/fixtures/facetable')
        for fragment in ('<strong>created (date)\n', '<strong>tags (array)\n', '<strong>state\n'):
            assert fragment in response.text

@pytest.mark.asyncio
async def test_conflicting_facet_names_json(ds_client):
    response = await ds_client.get('/fixtures/facetable.json?_facet=created&_facet_date=created&_facet=tags&_facet_array=tags')
    assert set(response.json()['facet_results']['results'].keys()) == {'created', 'tags', 'created_2', 'tags_2'}

@pytest.mark.asyncio
async def test_facet_against_in_memory_database():
    ds = Datasette()
    db = ds.add_memory_database('mem')
    await db.execute_write('create table t (id integer primary key, name text, name2 text)')
    to_insert = [{'name': 'one', 'name2': '1'} for _ in range(800)] + [{'name': 'two', 'name2': '2'} for _ in range(300)]
    print(to_insert)
    await db.execute_write_many('insert into t (name, name2) values (:name, :name2)', to_insert)
