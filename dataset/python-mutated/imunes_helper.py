...
class NodeNatCommand(Command, NodeNameValidatorMixin):
    iptables_nat_cmds = ['iptables --policy FORWARD DROP'.split(), 'iptables -t nat -A POSTROUTING -o {{if1}} -j MASQUERADE'.split(), 'iptables -A FORWARD -i {{if1}} -o {{if2}} -m state --state RELATED,ESTABLISHED -j ACCEPT'.split(), 'iptables -A FORWARD -i {{if2}} -o {{if1}} -j ACCEPT'.split()]
