...
async def report_to_slack(output, webhook):
    payload = {'text': f"lbrynet {__version__} ({system_info.get_platform()['platform']}) time to first byte:\n{output}"}
...
async def main(cmd_args=None):
    ...
    try:
        for page in range(1, cmd_args.download_pages + 1):
            ...
            kwargs = {'page': page, 'claim_type': 'stream', 'any_tags': ['art', 'automotive', 'blockchain', 'comedy', 'economics', 'education', 'gaming', 'music', 'news', 'science', 'sports', 'technology'], 'order_by': ['trending_global', 'trending_mixed'], 'no_totals': True}
            if not cmd_args.allow_fees:
                kwargs['fee_amount'] = 0
            response = await daemon_rpc(conf, 'claim_search', **kwargs)
            if 'error' in response or not response.get('items'):
                print(f'Error getting claim list page {page}:')
                print(response)
                return 1
            else:
                url_to_claim.update({claim['permanent_url']: claim for claim in response['items'] if claim['value_type'] == 'stream'})
    ...
    download_successes = []
    failed_to = {}
    await asyncio.gather(*(daemon_rpc(conf, 'file_delete', delete_from_download_dir=True, claim_name=claim['name']) for claim in url_to_claim.values() if not cmd_args.keep_files))
    for (i, (url, claim)) in enumerate(url_to_claim.items()):
        ...
        if not cmd_args.head_blob_only:
            (downloaded, amount_downloaded, blobs_in_stream) = await wait_for_done(conf, claim['name'], cmd_args.stall_download_timeout)
            if downloaded:
            ...
            print(f'downloaded {amount_downloaded}/{blobs_in_stream} blobs for {url} at {mbs}mb/s')
    ...
    if first_byte_times:
        result += f'Worst first byte time: {round(max(first_byte_times), 2)}\nBest first byte time: {round(min(first_byte_times), 2)}\n*95% confidence time-to-first-byte: {confidence(first_byte_times, 1.984)}s*\n99% confidence time-to-first-byte:  {confidence(first_byte_times, 2.626)}s\nVariance: {variance(first_byte_times)}\n'
    if download_successes:
        result += f'Downloaded {len(download_successes)}/{len(url_to_claim)}\nBest stream download speed: {round(max(download_speeds), 2)}mb/s\nWorst stream download speed: {round(min(download_speeds), 2)}mb/s\n95% confidence download speed: {confidence(download_speeds, 1.984, False)}mb/s\n99% confidence download speed:  {confidence(download_speeds, 2.626, False)}mb/s\n'
