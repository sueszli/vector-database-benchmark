...
import time
if len(sys.argv) == 1:
    fname = sg.popup_get_file('Document Browser', 'Document file to open', no_window=True, file_types=(('PDF Files', '*.pdf'), ('XPS Files', '*.*xps'), ('Epub Files', '*.epub'), ('Fiction Books', '*.fb2'), ('Comic Books', '*.cbz'), ('HTML', '*.htm*')))
else:
    fname = sys.argv[1]
if not fname:
...
layout = [[sg.ReadButton('Next'), sg.ReadButton('Prev'), sg.Text('Page:'), goto, sg.Text('(%i)' % page_count), sg.ReadButton('Zoom'), sg.Text('(toggle on/off, use arrows to navigate while zooming)')], [image_elem]]
window = sg.Window(title, layout, return_keyboard_events=True, location=(0, 0), use_default_focus=False, no_titlebar=False)
...
while True:
    ...
    if event in enter_buttons:
        try:
            cur_page = int(value['-PageNumber-']) - 1
            while cur_page < 0:
                cur_page += page_count
        except:
            cur_page = 0
    ...
    (data, clip_pos) = get_page(cur_page, zoom=zoom, max_size=max_size, first=False)
