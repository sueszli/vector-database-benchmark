...
try:
    ...
    rich.print('[bold]error[/]: [red]No API token provided. Please set `RTD_API_TOKEN`.[/]', file=sys.stderr)
    sys.exit(1)
...
for redirect in sorted(rtd_redirects['results'], key=operator.itemgetter('type', 'from_url', 'to_url')):
...
with get_rtd_api() as rtd_api:
    for redirect_id in redirects_to_remove:
        ...
        if response.status_code != 204:
            ...
            exit_code = 1
    for (src, dst) in redirects_to_add.items():
        response = rtd_api.post('redirects/', json={'from_url': src, 'to_url': dst, 'type': 'exact'})
        response.raise_for_status()
