...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--short-image-name', required=True, help='Short image name to create manifests for')
    arg_parser.add_argument('--hist-line-dir', required=True, type=Path, help='Directory to save history line')
    arg_parser.add_argument('--manifest-dir', required=True, type=Path, help='Directory to save manifest file')
    arg_parser.add_argument('--registry', required=True, type=str, choices=['docker.io', 'quay.io'], help='Image registry')
    arg_parser.add_argument('--owner', required=True, help='Owner of the image')
    args = arg_parser.parse_args()
    LOGGER.info(f'Current build timestamp: {BUILD_TIMESTAMP}')
    write_manifest(args.short_image_name, args.registry, args.owner, args.hist_line_dir, args.manifest_dir)