...
FLAGS = flags.FLAGS
flags.DEFINE_string('saved_model_dir', platform_test.test_src_dir_path('python/compiler/tensorrt/model_tests/sample_model'), 'The directory to the testing SavedModel.')
flags.DEFINE_string('saved_model_signature_key', signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY, 'The signature key of the testing SavedModel being used.')
flags.DEFINE_multi_string('saved_model_tags', (tag_constants.SERVING,), 'The tags of the testing SavedModel being used.')
flags.DEFINE_integer('batch_size', 128, 'The batch size used to run the testing model with.')
flags.DEFINE_boolean('use_tf2', True, 'Whether to test with TF2 behavior or not (TF1).')
flags.DEFINE_boolean('use_int8', True, 'Whether to convert with INT8 precision.')
flags.DEFINE_enum('latency_baseline', 'GPU', ['CPU', 'GPU'], 'The baseline version for latency improvement analysis.')
flags.DEFINE_enum('numerics_baseline', 'CPU', ['CPU', 'GPU'], 'The baseline version for numerical difference analysis.')
flags.DEFINE_float('fp32_speedup_tolerance', 0.9, 'Log errors whenever mean TensorRT fp32 speedup is lower than the tolerance.')
flags.DEFINE_float('fp16_speedup_tolerance', 0.9, 'Log errors whenever mean TensorRT fp16 speedup is lower than the tolerance.')
flags.DEFINE_float('int8_speedup_tolerance', 0.9, 'Log errors whenever mean TensorRT int8 speedup is lower than the tolerance.')
flags.DEFINE_float('fp32_abs_tolerance', 1e-05, 'Log errors whenever mean TensorRT fp32 absolute difference is larger than the tolerance.')
flags.DEFINE_float('fp16_abs_tolerance', 0.05, 'Log errors whenever mean TensorRT fp16 absolute difference is larger than the tolerance.')
flags.DEFINE_float('int8_abs_tolerance', 0.5, 'Log errors whenever mean TensorRT int8 absolute difference is larger than the tolerance.')
flags.DEFINE_integer('gpu_memory_limit_mb', None, 'Limitation on the device memory being used during TensorRT compilation and inference.')
flags.DEFINE_string('output_dir', None, 'Output directory of analysis results.')
flags.DEFINE_enum('output_format', 'CSV', ['CSV', 'JSON'], 'Output format of analysis results.')
