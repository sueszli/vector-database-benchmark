...
from modelscope.models.nlp import LSTMForTokenClassificationWithCRF, ModelForTokenClassification
...
class PartOfSpeechTest(unittest.TestCase):
    ...
    def test_run_by_direct_model_download(self):
        ...
        pipeline2 = pipeline(Tasks.part_of_speech, model=model, preprocessor=tokenizer)
    ...
    def test_run_lstmcrf_new_with_model_name(self):
        if False:
            return 10
        pipeline_ins = pipeline(task=Tasks.part_of_speech, model=self.lstmcrf_news_model_id)
        print(pipeline_ins(input=self.sentence))
