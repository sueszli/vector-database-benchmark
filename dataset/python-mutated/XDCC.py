...
class XDCC(BaseDownloader):
    ...
    __version__ = '0.55'
    __status__ = 'testing'
    __pattern__ = 'xdcc://(?P<SERVER>.*?)/#?(?P<CHAN>.*?)/(?P<BOT>.*?)/#?(?P<PACK>\\d+)/?'
    __config__ = [('nick', 'str', 'Nickname', 'pyload'), ('ident', 'str', 'Ident', 'pyloadident'), ('realname', 'str', 'Realname', 'pyloadreal'), ('try_resume', 'bool', 'Request XDCC resume?', True), ('nick_pw', 'str', 'Registered nickname password (optional)', ''), ('queued_timeout', 'int', 'Time to wait before failing if queued (minutes, 0 = infinite)', 300), ('queue_query_interval', 'int', 'Interval to query queue position when queued (minutes, 0 = disabled)', 3), ('response_timeout', 'int', 'XDCC Bot response timeout (seconds, minimum 60)', 300), ('waiting_opts', 'str', 'Time to wait before requesting pack from the XDCC Bot (format: ircserver/channel/wait_seconds, ...)', 0), ('invite_opts', 'str', 'Invite bots options (format ircserver/channel/invitebot/password, ...)', ''), ('channel_opts', 'str', 'Join custom channel before joining channel (format: ircserver/channel/customchannel, ...)', '')]
    __description__ = 'Download from IRC XDCC bot'
    __license__ = 'GPLv3'
    __authors__ = [('jeix', 'jeix@hasnomail.com'), ('GammaC0de', 'nitzo2001[AT]yahoo[DOT]com')]
    RE_QUEUED = re.compile('Added you to the (?:main|idle) queue for pack \\d+ \\("[^"]+"\\) in position (\\d+)')
    RE_QUEUE_STAT = re.compile('^Queued \\w+ for ".+?", in position (\\d+).+?([\\dhm]+) or less remaining')
    RE_XDCC = re.compile('\\x01DCC SEND "?(?P<NAME>.*?)"? (?P<IP>\\d+) (?P<PORT>\\d+)(?: (?P<SIZE>\\d+))?\\x01')
