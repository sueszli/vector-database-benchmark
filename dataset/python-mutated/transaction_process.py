...
from .....payment.utils import get_final_session_statuses, handle_transaction_process_session
...
class TransactionProcess(BaseMutation):
    transaction = graphene.Field(TransactionItem, description='The processed transaction.')
    transaction_event = graphene.Field(TransactionEvent, description='The event created for the processed transaction.')
    data = graphene.Field(JSON, description='The json data required to finalize the payment.')

    class Arguments:
        id = graphene.ID(description='The ID of the transaction to process.', required=True)
    ...
    class Meta:
        ...
        description = 'Processes a transaction session. It triggers the webhook `TRANSACTION_PROCESS_SESSION`, to the assigned `paymentGateways`. ' + ADDED_IN_313 + PREVIEW_FEATURE
