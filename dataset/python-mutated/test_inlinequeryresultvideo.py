import pytest
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, InlineQueryResultVideo, InlineQueryResultVoice, InputTextMessageContent, MessageEntity
...
class TestInlineQueryResultVideoWithoutRequest(TestInlineQueryResultVideoBase):
    ...
    def test_expected_values(self, inline_query_result_video):
        ...
        assert inline_query_result_video.input_message_content.to_dict() == self.input_message_content.to_dict()
    ...
    def test_caption_entities_always_tuple(self):
        if False:
            for i in range(10):
                print('nop')
        video = InlineQueryResultVideo(self.id_, self.video_url, self.mime_type, self.thumbnail_url, self.title)
    ...
    def test_to_dict(self, inline_query_result_video):
        ...
        assert inline_query_result_video_dict['video_width'] == inline_query_result_video.video_width
        assert inline_query_result_video_dict['video_height'] == inline_query_result_video.video_height
        assert inline_query_result_video_dict['video_duration'] == inline_query_result_video.video_duration
        assert inline_query_result_video_dict['thumbnail_url'] == inline_query_result_video.thumbnail_url
        assert inline_query_result_video_dict['title'] == inline_query_result_video.title
        assert inline_query_result_video_dict['description'] == inline_query_result_video.description
        ...
        assert inline_query_result_video_dict['caption_entities'] == [ce.to_dict() for ce in inline_query_result_video.caption_entities]
        assert inline_query_result_video_dict['input_message_content'] == inline_query_result_video.input_message_content.to_dict()
        assert inline_query_result_video_dict['reply_markup'] == inline_query_result_video.reply_markup.to_dict()

    def test_equality(self):
        if False:
            while True:
                i = 10
        a = InlineQueryResultVideo(self.id_, self.video_url, self.mime_type, self.thumbnail_url, self.title)
        b = InlineQueryResultVideo(self.id_, self.video_url, self.mime_type, self.thumbnail_url, self.title)
        c = InlineQueryResultVideo(self.id_, '', self.mime_type, self.thumbnail_url, self.title)
        d = InlineQueryResultVideo('', self.video_url, self.mime_type, self.thumbnail_url, self.title)
