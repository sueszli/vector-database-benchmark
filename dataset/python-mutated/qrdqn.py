...
class QRDQNPolicy(DQNPolicy):
    ...
    Config:
        ...
        == ==================== ======== ============== ======================================== =======================
    """
    config = dict(type='qrdqn', cuda=False, on_policy=False, priority=False, discount_factor=0.97, nstep=1, learn=dict(update_per_collect=3, batch_size=64, learning_rate=0.001, target_update_freq=100, ignore_done=False), collect=dict(unroll_len=1), eval=dict(), other=dict(eps=dict(type='exp', start=0.95, end=0.1, decay=10000), replay_buffer=dict(replay_buffer_size=10000)))
    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        data_n = qrdqn_nstep_td_data(q_value, target_q_value, data['action'], target_q_action, data['reward'], data['done'], tau, data['weight'])
        value_gamma = data.get('value_gamma')
        (loss, td_error_per_sample) = qrdqn_nstep_td_error(data_n, self._gamma, nstep=self._nstep, value_gamma=value_gamma)
