        ...
        if run is None:
            for step in flow:
                ...
                for task_pathspec in cli_check_dict:
                    ...
                    checker.assert_card(step.name, task_id, 'test_editable_card', '%d' % number, card_hash=card['hash'], exact_match=True)
        else:
            for step in flow:
                if step.name == 'end':
                    checker.assert_artifact(step.name, 'here', True)
                ...
                for task_id in meta_check_dict:
                    ...
                    assert_equals(len(default_editable_cards) == 1, True)
                    card = default_editable_cards[0]
                    checker.assert_card(step.name, task_id, 'test_editable_card', '%d' % random_number, card_hash=card['hash'], exact_match=False)