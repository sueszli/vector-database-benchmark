...
try:
    ...
    GCSFileSystem = None
TWO_FEATURES_EXAMPLES = [torch.from_numpy(np.array([1, 5], dtype='float32')), torch.from_numpy(np.array([3, 10], dtype='float32')), torch.from_numpy(np.array([-14, 0], dtype='float32')), torch.from_numpy(np.array([0.5, 0.5], dtype='float32'))]
TWO_FEATURES_PREDICTIONS = [PredictionResult(ex, pred) for (ex, pred) in zip(TWO_FEATURES_EXAMPLES, torch.Tensor([f1 * 2.0 + f2 * 3 + 0.5 for (f1, f2) in TWO_FEATURES_EXAMPLES]).reshape(-1, 1))]
TWO_FEATURES_DICT_OUT_PREDICTIONS = [PredictionResult(p.example, {'output1': p.inference, 'output2': p.inference}) for p in TWO_FEATURES_PREDICTIONS]
KEYED_TORCH_EXAMPLES = [{'k1': torch.from_numpy(np.array([1], dtype='float32')), 'k2': torch.from_numpy(np.array([1.5], dtype='float32'))}, {'k1': torch.from_numpy(np.array([5], dtype='float32')), 'k2': torch.from_numpy(np.array([5.5], dtype='float32'))}, {'k1': torch.from_numpy(np.array([-3], dtype='float32')), 'k2': torch.from_numpy(np.array([-3.5], dtype='float32'))}, {'k1': torch.from_numpy(np.array([10.0], dtype='float32')), 'k2': torch.from_numpy(np.array([10.5], dtype='float32'))}]
KEYED_TORCH_PREDICTIONS = [PredictionResult(ex, pred) for (ex, pred) in zip(KEYED_TORCH_EXAMPLES, torch.Tensor([example['k1'] * 2.0 + 0.5 + (example['k2'] * 2.0 + 0.5) for example in KEYED_TORCH_EXAMPLES]).reshape(-1, 1))]
KEYED_TORCH_HELPER_PREDICTIONS = [PredictionResult(ex, pred) for (ex, pred) in zip(KEYED_TORCH_EXAMPLES, torch.Tensor([example['k1'] * 2.0 + 0.5 + (example['k2'] * 2.0 + 0.5) + 0.5 for example in KEYED_TORCH_EXAMPLES]).reshape(-1, 1))]
KEYED_TORCH_DICT_OUT_PREDICTIONS = [PredictionResult(p.example, {'output1': p.inference, 'output2': p.inference}) for p in KEYED_TORCH_PREDICTIONS]
