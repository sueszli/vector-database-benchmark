...
class CustomerDelete(CustomerDeleteMixin, UserDelete):
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.CUSTOMER_DELETED, description='A customer account was deleted.')]

    class Arguments:
        id = graphene.ID(required=False, description='ID of a customer to delete.')
        external_reference = graphene.String(required=False, description=f'External ID of a customer to update. {ADDED_IN_310}')
