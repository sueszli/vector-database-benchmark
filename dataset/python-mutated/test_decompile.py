...
expr = t.a.sum()
countries = ibis.table([('name', 'string'), ('continent', 'string'), ('population', 'int'), ('area_km2', 'int')], name='countries')
asian_countries = countries.filter(countries.continent == 'AS')
top_with_highest_population = asian_countries.order_by(asian_countries.population.desc()).limit(10)
overall_population_density = asian_countries.population.sum() / asian_countries.area_km2.sum()
population_density_per_country = asian_countries.group_by('name').aggregate(_.population.sum() / _.area_km2.sum())
one = ibis.literal(1)
two = ibis.literal(2)
three = one + two
nine = three ** 2
