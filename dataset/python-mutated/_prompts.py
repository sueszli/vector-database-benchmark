...
from prefect.client.schemas.schedules import SCHEDULE_TYPES, CronSchedule, IntervalSchedule, RRuleSchedule
from prefect.client.utilities import inject_client
from prefect.deployments.base import _get_git_remote_origin_url, _search_for_flow_functions
...
@inject_client
async def prompt_select_work_pool(console: Console, prompt: str='Which work pool would you like to deploy this flow to?', client: PrefectClient=None) -> str:
    work_pools = await client.read_work_pools()
    work_pool_options = [work_pool.dict() for work_pool in work_pools if work_pool.type != 'prefect-agent']
...
async def prompt_build_custom_docker_image(console: Console, deployment_config: dict):
...
async def prompt_push_custom_docker_image(console: Console, deployment_config: dict, build_docker_image_step: dict):
...
@inject_client
async def prompt_create_work_pool(console: Console, client: PrefectClient=None):
...
@inject_client
async def prompt_select_remote_flow_storage(console: Console, client: PrefectClient=None) -> Optional[str]:
    ...
    valid_storage_options_for_context = [row for row in flow_storage_options if row is not None and row['slug'] in valid_slugs_for_context]
...
@inject_client
async def prompt_select_blob_storage_credentials(console: Console, storage_provider: str, client: PrefectClient=None) -> str:
    """
    ...
    if existing_credentials_blocks:
        selected_credentials_block = prompt_select_from_table(console, prompt=f'Select from your existing {pretty_creds_block_type} credential blocks', columns=[{'header': f'{pretty_storage_provider} Credentials Blocks', 'key': 'name'}], data=[{'name': block.name} for block in existing_credentials_blocks], opt_out_message='Create a new credentials block')
