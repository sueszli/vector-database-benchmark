...
_FLAG_SINGLE_QUOTE_PAT = re.compile("\\s*--([^=]+)='([^']*)'")
_FLAG_DOUBLE_QUOTE_PAT = re.compile('\\s*--([^=]+)="([^"]*)"')
_FLAG_NO_QUOTE_PAT = re.compile('\\s*--([^=]+)=(\\S*)')
_FLAG_NO_EQUAL_PAT = re.compile('\\s*--([^=]+)\\s*')
...
VALID_FLAG_NAMES = [FLAG_NAME_ENABLE, FLAG_NAME_TRACE_MODE, FLAG_NAME_TRACE_SCALAR_OPS, FLAG_NAME_SUBMODE, FLAG_NAME_EXCLUDED_OPNAMES, FLAG_NAME_EXCLUDED_OPTYPES, FLAG_NAME_INCLUDED_OPNAMES, FLAG_NAME_INCLUDED_OPTYPES, FLAG_NAME_TRACE_DIR, FLAG_NAME_REPORT_FILE, FLAG_NAME_USE_TEST_UNDECLARED_OUTPUTS_DIR, FLAG_NAME_OP_RANGE, FLAG_NAME_DUMP_BEFORE_AFTER_GRAPHS, FLAG_NAME_TRACE_LEVEL, FLAG_NAME_SUMMARY_SIGNATURES, FLAG_NAME_SUMMARY_PER_CORE, FLAG_NAME_TEMP_CACHE_VAR, FLAG_NAME_FINGERPRINT_DIR, FLAG_NAME_INSPECT_TRACE, FLAG_FLUSH_SUMMARY]
_OP_RANGE_PAT = re.compile('(\\d+):(\\d+)')
...
TT_SUMMARY_SIGNATURES = (TT_SUMMARY_NORM, TT_SUMMARY_MAX, TT_SUMMARY_MIN, TT_SUMMARY_SPARSITY, TT_SUMMARY_MEAN, TT_SUMMARY_VAR, TT_SUMMARY_SIZE, TT_SUMMARY_MAX_ABS)
FLAGS = flags.FLAGS
DELTA_THRESHOLD = flags.DEFINE_float('delta_threshold', default=0.5, help='Log if history based diff crosses this threshold.')
TT_CHECK_FILTER = flags.DEFINE_bool('tt_check_filter', default=False, help='Terminate early to check op name filtering.')
TT_SINGLE_CORE_SUMMARIES = flags.DEFINE_bool('tt_single_core_summaries', default=False, help='Report single core metric and avoid aggregation.')
