...
if __name__ == '__main__':
    ...
    if sys.version_info.major == 2 and sys.version_info.minor == 7 and (sys.version_info.micro < 16):
    ...
    parser.add_argument('-computer-name', action='store', metavar='COMPUTER-NAME$', help='Name of computer to add.If omitted, a random DESKTOP-[A-Z0-9]{8} will be used.')
    parser.add_argument('-computer-pass', action='store', metavar='password', help='Password to set to computerIf omitted, a random [A-Za-z0-9]{32} will be used.')
    ...
    parser.add_argument('-method', choices=['SAMR', 'LDAPS'], default='SAMR', help="Method of adding the computer.SAMR works over SMB.LDAPS has some certificate requirementsand isn't always available.")
    parser.add_argument('-port', type=int, choices=[139, 445, 636], help='Destination port to connect to. SAMR defaults to 445, LDAPS to 636.')
    ...
    group.add_argument('-computer-group', action='store', metavar='CN=Computers,DC=test,DC=local', help='Group to which the account will be added.If omitted, CN=Computers will be used,')
    ...
    group.add_argument('-dc-ip', action='store', metavar='ip', help="IP of the domain controller to use. Useful if you can't translate the FQDN.specified in the account parameter will be used")
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
