...
from pyatmo.modules.device_types import DEVICE_DESCRIPTION_MAP, DeviceType as NetatmoDeviceType
...
class NetatmoBase(Entity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        for publisher in self._publishers:
            ...
            if 'home_id' in publisher:
                await self.data_handler.subscribe(publisher['name'], signal_name, self.async_update_callback, home_id=publisher['home_id'])
            elif publisher['name'] == PUBLIC:
                await self.data_handler.subscribe(publisher['name'], signal_name, self.async_update_callback, lat_ne=publisher['lat_ne'], lon_ne=publisher['lon_ne'], lat_sw=publisher['lat_sw'], lon_sw=publisher['lon_sw'])
            else:
                await self.data_handler.subscribe(publisher['name'], signal_name, self.async_update_callback)
            if any((sub is None for sub in self.data_handler.publisher[signal_name].subscriptions)):
    ...
    async def async_will_remove_from_hass(self) -> None:
        ...
        for publisher in self._publishers:
            await self.data_handler.unsubscribe(publisher[SIGNAL_NAME], self.async_update_callback)
