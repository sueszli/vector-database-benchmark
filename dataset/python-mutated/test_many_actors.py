...
try:
    ...
    print(e)
print(f'Success! Started {MAX_ACTORS_IN_CLUSTER} actors in {end_time - start_time}s. ({rate} actors/s)')
results = {'actors_per_second': rate, 'num_actors': MAX_ACTORS_IN_CLUSTER, 'time': end_time - start_time, 'success': '1', '_peak_memory': round(used_gb, 2), '_peak_process_memory': usage}
if not is_smoke_test:
    results['perf_metrics'] = [{'perf_metric_name': 'actors_per_second', 'perf_metric_value': rate, 'perf_metric_type': 'THROUGHPUT'}]
