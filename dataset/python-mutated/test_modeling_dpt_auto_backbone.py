...
class DPTModelTester:
    ...
    def __init__(self, parent, batch_size=2, num_channels=3, image_size=32, patch_size=16, use_labels=True, num_labels=3, is_training=True, hidden_size=4, num_hidden_layers=2, num_attention_heads=2, intermediate_size=8, out_features=['stage1', 'stage2'], apply_layernorm=False, reshape_hidden_states=False, neck_hidden_sizes=[2, 2], fusion_hidden_size=6):
        if False:
            print('Hello World!')
        self.parent = parent
    ...
    def get_config(self):
        if False:
            print('Hello World!')
        return DPTConfig(backbone_config=self.get_backbone_config(), neck_hidden_sizes=self.neck_hidden_sizes, fusion_hidden_size=self.fusion_hidden_size)

    def get_backbone_config(self):
        if False:
            i = 10
            return i + 15
        return Dinov2Config(image_size=self.image_size, patch_size=self.patch_size, num_channels=self.num_channels, hidden_size=self.hidden_size, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, is_training=self.is_training, out_features=self.out_features, reshape_hidden_states=self.reshape_hidden_states)
...
class DPTModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_depth_estimation(self):
        ...
        expected_slice = torch.tensor([[6.0433, 7.1636, 7.4268], [6.9047, 7.2471, 7.2355], [7.9261, 8.0631, 8.0244]]).to(torch_device)
