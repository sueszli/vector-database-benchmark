...
from paddle.distributed.auto_parallel.static.operators.common import get_distributed_operator_impl_container
...
class TestCompatible(unittest.TestCase):
    ...
    def test_matmulv2_matmul_2_compatible(self):
        ...
        with static.program_guard(program, start_program), utils.unique_name.guard():
            matmulx3 = static.data(name='matmulx3', shape=[6, 2, 6], dtype='float32')
            matmuly3 = static.data(name='matmuly3', shape=[6, 6], dtype='float32')
            output1 = paddle.matmul(x=matmulx3, y=matmuly3)
            matmulx4 = static.data(name='matmulx4', shape=[6, 6, 2, 6], dtype='float32')
            matmuly4 = static.data(name='matmuly4', shape=[6, 6, 6, 6], dtype='float32')
    ...
    def test_matmulv2_matmul_1_compatible(self):
        ...
        with static.program_guard(program, start_program), utils.unique_name.guard():
            ...
            matmuly3 = static.data(name='matmuly3', shape=[6, 6], dtype='float32')
            output1 = paddle.matmul(x=matmulx3, y=matmuly3)
            matmulx4 = static.data(name='matmulx4', shape=[6, 6, 6, 6], dtype='float32')
            matmuly4 = static.data(name='matmuly4', shape=[6, 6, 6, 6], dtype='float32')
            output2 = paddle.matmul(x=matmulx4, y=matmuly4)
        ops = program.global_block().ops
        vars = program.global_block().vars
