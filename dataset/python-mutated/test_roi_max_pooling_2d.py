...
@testing.parameterize(*testing.product({'dtype': [numpy.float32, numpy.float64], 'outsize': [5, 7, (5, 7), (numpy.int32(5), numpy.int32(7))], 'spatial_scale': [0.6, 1.0, 2, numpy.float32(0.6), numpy.int32(2)]}))
class TestROIMaxPooling2D(unittest.TestCase):
    ...
    def check_forward(self, x_data, roi_data, roi_index_data):
        ...
        y = functions.roi_max_pooling_2d(x, rois, roi_indices, outsize=self.outsize, spatial_scale=self.spatial_scale)
    ...
    def test_forward_cpu_gpu_equal(self):
        ...
        y_cpu = functions.roi_max_pooling_2d(x_cpu, rois_cpu, roi_indices_cpu, outsize=self.outsize, spatial_scale=self.spatial_scale)
        x_gpu = chainer.Variable(cuda.to_gpu(self.x))
        rois_gpu = chainer.Variable(cuda.to_gpu(self.rois))
        roi_indices_gpu = chainer.Variable(cuda.to_gpu(self.roi_indices))
        y_gpu = functions.roi_max_pooling_2d(x_gpu, rois_gpu, roi_indices_gpu, outsize=self.outsize, spatial_scale=self.spatial_scale)
    ...
    def check_backward(self, x_data, roi_data, roi_index_data, y_grad):
        ...
        def f(x, rois, roi_indices):
            ...
            y = functions.where(xp.isinf(y.array), xp.zeros(y.shape, dtype=y.dtype), y)
            return y
