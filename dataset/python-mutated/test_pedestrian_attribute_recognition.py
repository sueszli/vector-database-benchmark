...
class PedestrianAttributeRecognitionTest(unittest.TestCase):
    ...
    def test_run_modelhub_with_image_file(self):
        if False:
            i = 10
            return i + 15
        pedestrian_attribute_recognition = pipeline(self.task, model=self.model_id)
        self.pipeline_inference(pedestrian_attribute_recognition, self.test_image)
    ...
    def test_run_modelhub_with_image_input(self):
        if False:
            ...
            return i + 15
        pedestrian_attribute_recognition = pipeline(self.task, model=self.model_id)
        self.pipeline_inference(pedestrian_attribute_recognition, Image.open(self.test_image))
