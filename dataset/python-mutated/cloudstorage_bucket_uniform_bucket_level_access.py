...
class cloudstorage_bucket_uniform_bucket_level_access(Check):
    ...
    def execute(self) -> Check_Report_GCP:
        ...
        for bucket in cloudstorage_client.buckets:
            ...
            report.status = 'PASS'
            report.status_extended = f'Bucket {bucket.name} has uniform Bucket Level Access enabled.'
            if not bucket.uniform_bucket_level_access:
                report.status = 'FAIL'
                report.status_extended = f'Bucket {bucket.name} has uniform Bucket Level Access disabled.'
