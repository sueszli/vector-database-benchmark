...
from geopy.adapters import AdapterHTTPError, AioHTTPAdapter, BaseAsyncAdapter, RequestsAdapter, URLLibAdapter
from geopy.exc import GeocoderAuthenticationFailure, GeocoderParseError, GeocoderServiceError
...
async def test_not_available_adapters_raise(adapter_cls):
    with patch.object(geopy.geocoders.options, 'default_adapter_factory', adapter_cls):
        with pytest.raises(ImportError):
            async with make_dummy_async_geocoder():
                pass

async def test_geocoder_constructor_uses_https_proxy(timeout, proxy_server, proxy_url, remote_website_trusted_https):
    base_http = urlopen(remote_website_trusted_https, timeout=timeout)
    base_html = base_http.read().decode()
    async with make_dummy_async_geocoder(proxies={'https': proxy_url}, timeout=timeout) as geocoder_dummy:
...
async def test_geocoder_http_proxy_auth_is_respected(timeout, proxy_server, remote_website_http, with_scheme):
    ...
    async with make_dummy_async_geocoder(proxies={'http': proxy_url}, timeout=timeout) as geocoder_dummy:
        assert 0 == len(proxy_server.requests)
        assert base_html == await geocoder_dummy.geocode(remote_website_http)
        assert 1 == len(proxy_server.requests)

@pytest.mark.parametrize('with_scheme', [False, True])
async def test_geocoder_https_proxy_auth_is_respected(timeout, proxy_server, remote_website_trusted_https, with_scheme):
    proxy_server.set_auth('user', 'test')
    base_http = urlopen(remote_website_trusted_https, timeout=timeout)
    base_html = base_http.read().decode()
    proxy_url = proxy_server.get_proxy_url(with_scheme=with_scheme)
...
async def test_geocoder_http_proxy_auth_error(timeout, proxy_server, proxy_url, remote_website_http):
    proxy_server.set_auth('user', 'test')
    async with make_dummy_async_geocoder(proxies={'http': proxy_url}, timeout=timeout) as geocoder_dummy:
...
async def test_geocoder_https_proxy_auth_error(timeout, proxy_server, proxy_url, remote_website_trusted_https):
    proxy_server.set_auth('user', 'test')
    async with make_dummy_async_geocoder(proxies={'https': proxy_url}, timeout=timeout) as geocoder_dummy:
...
async def test_ssl_context_with_proxy_is_respected(timeout, proxy_server, proxy_url, remote_website_trusted_https):
    bad_ctx = ssl.create_default_context(cafile=CERT_SELFSIGNED_CA)
    async with make_dummy_async_geocoder(proxies={'https': proxy_url}, ssl_context=bad_ctx, timeout=timeout) as geocoder_dummy:
        assert 0 == len(proxy_server.requests)
...
@pytest.mark.skipif(WITH_SYSTEM_PROXIES, reason="There're active system proxies")
async def test_ssl_context_without_proxy_is_respected(timeout, remote_website_trusted_https):
    bad_ctx = ssl.create_default_context(cafile=CERT_SELFSIGNED_CA)
    async with make_dummy_async_geocoder(ssl_context=bad_ctx, timeout=timeout) as geocoder_dummy:
        with pytest.raises(GeocoderServiceError) as excinfo:
            await geocoder_dummy.geocode(remote_website_trusted_https)
        assert 'SSL' in str(excinfo.value)

async def test_geocoder_constructor_uses_http_proxy(timeout, proxy_server, proxy_url, remote_website_http):
    base_http = urlopen(remote_website_http, timeout=timeout)
    base_html = base_http.read().decode()
    async with make_dummy_async_geocoder(proxies={'http': proxy_url}, timeout=timeout) as geocoder_dummy:
        assert 0 == len(proxy_server.requests)
...
async def test_geocoder_constructor_uses_str_proxy(timeout, proxy_server, proxy_url, remote_website_http):
    base_http = urlopen(remote_website_http, timeout=timeout)
    base_html = base_http.read().decode()
    async with make_dummy_async_geocoder(proxies=proxy_url, timeout=timeout) as geocoder_dummy:
        assert 0 == len(proxy_server.requests)
        assert base_html == await geocoder_dummy.geocode(remote_website_http)
        assert 1 == len(proxy_server.requests)
...
async def test_get_json_plain(remote_website_http_json_plain, timeout):
    async with make_dummy_async_geocoder(timeout=timeout) as geocoder_dummy:
        result = await geocoder_dummy.geocode(remote_website_http_json_plain, is_json=True)
        assert isinstance(result, dict)
...
async def test_system_proxies_are_respected_by_default(inject_proxy_to_system_env, timeout, proxy_server, remote_website_http, adapter_factory, proxy_url):
    async with make_dummy_async_geocoder(timeout=timeout) as geocoder_dummy:
        assert 0 == len(proxy_server.requests)
        await geocoder_dummy.geocode(remote_website_http)
        assert 1 == len(proxy_server.requests)

async def test_system_proxies_are_respected_with_none(inject_proxy_to_system_env, timeout, proxy_server, remote_website_http, adapter_factory, proxy_url):
    async with make_dummy_async_geocoder(proxies=None, timeout=timeout) as geocoder_dummy:
        assert 0 == len(proxy_server.requests)
        await geocoder_dummy.geocode(remote_website_http)
        assert 1 == len(proxy_server.requests)

async def test_system_proxies_are_reset_with_empty_dict(inject_proxy_to_system_env, timeout, proxy_server, remote_website_http):
    async with make_dummy_async_geocoder(proxies={}, timeout=timeout) as geocoder_dummy:
        assert 0 == len(proxy_server.requests)
        await geocoder_dummy.geocode(remote_website_http)
        assert 0 == len(proxy_server.requests)

async def test_string_value_overrides_system_proxies(inject_proxy_to_system_env, timeout, proxy_server, proxy_url, remote_website_http):
