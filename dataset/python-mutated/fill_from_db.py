...
sslmode = config('DB_SSLMODE', cast=Choices(['disable', 'allow', 'prefer', 'require', 'verify-ca', 'verify-full']), default='allow')
...
async def main():
    ...
    with pg_client.PostgresClient() as conn:
        conn.execute('SELECT session_id, user_id FROM sessions WHERE session_id IN ({session_id_list})'.format(session_id_list=','.join(sessionids)))
...
cron_jobs = [{'func': main, 'trigger': IntervalTrigger(seconds=config('REPLACE_INTERVAL_USERID', default=60, cast=int)), 'misfire_grace_time': 60, 'max_instances': 1}]
