...
def match(tgt, delimiter=DEFAULT_TARGET_DELIM, opts=None, minion_id=None):
    ...
    if delimiter not in tgt:
        log.error('Got insufficient arguments for pillar PCRE match statement from master')
        return False
    if 'pillar' in opts:
        pillar = opts['pillar']
    ...
    return salt.utils.data.subdict_match(pillar, tgt, delimiter=delimiter, regex_match=True)