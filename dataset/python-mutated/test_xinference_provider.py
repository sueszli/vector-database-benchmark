...
VALIDATE_CREDENTIAL = {'model_uid': 'fake-model-uid', 'server_url': 'http://127.0.0.1:9997/'}
...
def test_is_credentials_valid_or_raise_valid(mocker):
    if False:
        print('Hello World!')
    mocker.patch('core.third_party.langchain.llms.xinference_llm.XinferenceLLM._call', return_value='abc')
...
def test_encrypt_model_credentials(mock_encrypt, mocker):
    ...
    mocker.patch('core.model_providers.providers.xinference_provider.XinferenceProvider._get_extra_credentials', return_value={'model_handle_type': 'generate', 'model_format': 'ggmlv3'})
