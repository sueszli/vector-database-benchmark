...
class LeagueDemoCollector(ISerialCollector):
    ...
    def reset(self, _policy: Optional[List[namedtuple]]=None, _env: Optional[BaseEnvManager]=None) -> None:
        ...
        self._traj_buffer = {env_id: {policy_id: TrajBuffer(maxlen=self._traj_len) for policy_id in range(2)} for env_id in range(self._env_num)}
    ...
    def collect(self, n_episode: Optional[int]=None, train_iter: int=0, policy_kwargs: Optional[dict]=None) -> Tuple[List[Any], List[Any]]:
        if False:
            print('Hello World!')
        ...
        while True:
            ...
            for (env_id, timestep) in timesteps.items():
                ...
                if timestep.done:
                    ...
                    info = {'reward0': timestep.info[0]['eval_episode_return'], 'reward1': timestep.info[1]['eval_episode_return'], 'time': self._env_info[env_id]['time'], 'step': self._env_info[env_id]['step'], 'probs0': probs0, 'probs1': probs1}
                    collected_episode += 1
    ...
    def _output_log(self, train_iter: int) -> None:
        ...
        if train_iter - self._last_train_iter >= self._collect_print_freq and len(self._episode_info) > 0:
            ...
            info.update({'probs0_select_action0': sum([p[0] for p in probs0]) / len(probs0), 'probs0_select_action1': sum([p[1] for p in probs0]) / len(probs0), 'probs1_select_action0': sum([p[0] for p in probs1]) / len(probs1), 'probs1_select_action1': sum([p[1] for p in probs1]) / len(probs1)})
