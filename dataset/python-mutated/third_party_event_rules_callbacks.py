...
CHECK_EVENT_ALLOWED_CALLBACK = Callable[[EventBase, StateMap[EventBase]], Awaitable[Tuple[bool, Optional[dict]]]]
ON_CREATE_ROOM_CALLBACK = Callable[[Requester, dict, bool], Awaitable]
CHECK_THREEPID_CAN_BE_INVITED_CALLBACK = Callable[[str, str, StateMap[EventBase]], Awaitable[bool]]
CHECK_VISIBILITY_CAN_BE_MODIFIED_CALLBACK = Callable[[str, StateMap[EventBase], str], Awaitable[bool]]
...
class ThirdPartyEventRulesModuleApiCallbacks:
    ...
    async def check_event_allowed(self, event: EventBase, context: UnpersistedEventContextBase) -> Tuple[bool, Optional[dict]]:
        ...
        for callback in self._check_event_allowed_callbacks:
            try:
                (res, replacement_data) = await delay_cancellation(callback(event, state_events))
            except CancelledError:
                raise
            except SynapseError as e:
                raise e
            except Exception:
                raise ModuleFailedException('Failed to run `check_event_allowed` module API callback')
            if res is False:
                return (res, None)
    ...
    async def on_create_room(self, requester: Requester, config: dict, is_requester_admin: bool) -> None:
    ...
    async def check_threepid_can_be_invited(self, medium: str, address: str, room_id: str) -> bool:
        ...
        for callback in self._check_threepid_can_be_invited_callbacks:
            try:
                threepid_can_be_invited = await delay_cancellation(callback(medium, address, state_events))
    ...
    async def check_visibility_can_be_modified(self, room_id: str, new_visibility: str) -> bool:
        ...
        for callback in self._check_visibility_can_be_modified_callbacks:
            try:
                visibility_can_be_modified = await delay_cancellation(callback(room_id, state_events, new_visibility))
    ...
    async def check_can_shutdown_room(self, user_id: Optional[str], room_id: str) -> bool:
    ...
    async def check_can_deactivate_user(self, user_id: str, by_admin: bool) -> bool:
        ...
        for callback in self._check_can_deactivate_user_callbacks:
            try:
                can_deactivate_user = await delay_cancellation(callback(user_id, by_admin))
    ...
    async def on_profile_update(self, user_id: str, new_profile: ProfileInfo, by_admin: bool, deactivation: bool) -> None:
    ...
    async def on_user_deactivation_status_changed(self, user_id: str, deactivated: bool, by_admin: bool) -> None:
    ...
    async def on_add_user_third_party_identifier(self, user_id: str, medium: str, address: str) -> None:
    ...
    async def on_remove_user_third_party_identifier(self, user_id: str, medium: str, address: str) -> None:
        """Called when an association between a user's Matrix ID and a third-party ID
