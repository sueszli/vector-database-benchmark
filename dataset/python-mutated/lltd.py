...
from scapy.fields import BitField, FlagsField, ByteField, ByteEnumField, ShortField, ShortEnumField, ThreeBytesField, IntField, IntEnumField, LongField, MultiEnumField, FieldLenField, FieldListField, PacketListField, StrLenField, StrLenFieldUtf16, ConditionalField, MACField
...
class LLTDHello(Packet):
    name = 'LLTD - Hello'
    show_summary = False
    fields_desc = [ShortField('gen_number', 0), MACField('current_mapper_address', ETHER_ANY), MACField('apparent_mapper_address', ETHER_ANY)]
...
class LLTDEmiteeDesc(Packet):
    ...
    fields_desc = [ByteEnumField('type', 0, {0: 'Train', 1: 'Probe'}), ByteField('pause', 0), MACField('src', None), MACField('dst', ETHER_ANY)]

class LLTDEmit(Packet):
    name = 'LLTD - Emit'
    fields_desc = [FieldLenField('descs_count', None, count_of='descs_list', fmt='H'), PacketListField('descs_list', [], LLTDEmiteeDesc, count_from=lambda pkt: pkt.descs_count)]
...
class LLTDRecveeDesc(Packet):
    ...
    fields_desc = [ShortEnumField('type', 0, {0: 'Probe', 1: 'ARP or ICMPv6'}), MACField('real_src', ETHER_ANY), MACField('ether_src', ETHER_ANY), MACField('ether_dst', ETHER_ANY)]

class LLTDQueryResp(Packet):
    name = 'LLTD - Query Response'
    fields_desc = [FlagsField('flags', 0, 2, 'ME'), BitField('descs_count', None, 14), PacketListField('descs_list', [], LLTDRecveeDesc, count_from=lambda pkt: pkt.descs_count)]
...
class LLTDQueryLargeTlv(Packet):
    ...
    fields_desc = [ByteEnumField('type', 14, {14: 'Icon image', 17: 'Friendly Name', 19: 'Hardware ID', 22: 'AP Association Table', 24: 'Detailed Icon Image', 26: 'Component Table', 28: 'Repeater AP Table'}), ThreeBytesField('offset', 0)]
...
class LLTDAttribute(Packet):
    ...
    show_indent = False
    show_summary = False
    fields_desc = [ByteEnumField('type', 0, {0: 'End Of Property', 1: 'Host ID', 2: 'Characteristics', 3: 'Physical Medium', 7: 'IPv4 Address', 9: '802.11 Max Rate', 10: 'Performance Counter Frequency', 12: 'Link Speed', 14: 'Icon Image', 15: 'Machine Name', 18: 'Device UUID', 20: 'QoS Characteristics', 21: '802.11 Physical Medium', 24: 'Detailed Icon Image'}), FieldLenField('len', None, length_of='value', fmt='B'), StrLenField('value', '', length_from=lambda pkt: pkt.len)]
...
class LLTDAttributeHostID(LLTDAttribute):
    ...
    fields_desc = [ByteField('len', 6), MACField('mac', ETHER_ANY)]
...
class LLTDAttributeMachineName(LLTDAttribute):
    ...
    fields_desc = [FieldLenField('len', None, length_of='hostname', fmt='B'), StrLenFieldUtf16('hostname', '', length_from=lambda pkt: pkt.len)]
...
class LLTDAttributeDeviceUUID(LLTDAttribute):
    ...
    fields_desc = [FieldLenField('len', None, length_of='uuid', fmt='B'), StrLenField('uuid', b'\x00' * 16, length_from=lambda pkt: pkt.len)]
...
class LLTDAttributeQOSCharacteristics(LLTDAttribute):
    ...
    fields_desc = [ByteField('len', 4), FlagsField('flags', 0, 3, 'EQP'), BitField('reserved1', 0, 13), ShortField('reserved2', 0)]
...
class LLTDAttribute80211PhysicalMedium(LLTDAttribute):
    ...
    fields_desc = [ByteField('len', 1), ByteEnumField('medium', 0, {0: 'Unknown', 1: 'FHSS 2.4 GHz', 2: 'DSSS 2.4 GHz', 3: 'IR Baseband', 4: 'OFDM 5 GHz', 5: 'HRDSSS', 6: 'ERP'})]

@_register_lltd_specific_class(25)
class LLTDAttributeSeesList(LLTDAttribute):
    name = 'LLTD Attribute - Sees List Working Set'
    fields_desc = [ByteField('len', 2), ShortField('max_entries', 0)]
bind_layers(Ether, LLTD, type=35033)
