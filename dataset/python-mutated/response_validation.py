    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send):
        ...
        async def wrapped_send(message: t.MutableMapping[str, t.Any]) -> None:
            ...
            if message['type'] == 'http.response.start':
                ...
                response_definition = self._operation.response_definition(status, mime_type)
                self.validate_required_headers(headers, response_definition)
                try:
                    body_validator = self._validator_map['response'][mime_type]
                except KeyError:
                    logging.info(f'Skipping validation. No validator registered for content type: {mime_type}.')
                else:
                    validator = body_validator(scope, schema=self._operation.response_schema(status, mime_type), nullable=utils.is_nullable(self._operation.response_definition(status, mime_type)), encoding=encoding)
