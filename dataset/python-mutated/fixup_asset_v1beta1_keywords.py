...
class assetCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'batch_get_assets_history': ('parent', 'asset_names', 'content_type', 'read_time_window'), 'export_assets': ('parent', 'output_config', 'read_time', 'asset_types', 'content_type')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=assetCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
