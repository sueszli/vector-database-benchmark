...
class MrpUnbuild(models.Model):
    ...
    product_id = fields.Many2one('product.product', 'Product', required=True, states={'done': [('readonly', True)]})
    product_qty = fields.Float('Quantity', required=True, states={'done': [('readonly', True)]})
    product_uom_id = fields.Many2one('product.uom', 'Unit of Measure', required=True, states={'done': [('readonly', True)]})
    bom_id = fields.Many2one('mrp.bom', 'Bill of Material', domain=[('product_tmpl_id', '=', 'product_id.product_tmpl_id')], required=True, states={'done': [('readonly', True)]})
    mo_id = fields.Many2one('mrp.production', 'Manufacturing Order', domain="[('product_id', '=', product_id), ('state', 'in', ['done', 'cancel'])]", states={'done': [('readonly', True)]})
    ...
    consume_line_ids = fields.One2many('stock.move', 'consume_unbuild_id', readonly=True, help='')
    produce_line_ids = fields.One2many('stock.move', 'unbuild_id', readonly=True, help='')
    state = fields.Selection([('draft', 'Draft'), ('done', 'Done')], string='Status', default='draft', index=True)
