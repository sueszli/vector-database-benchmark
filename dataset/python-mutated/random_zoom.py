...
class RandomZoom(TFDataLayer):
    ...
    **Note:** This layer is safe to use inside a `tf.data` pipeline
    ...
    """
    _FACTOR_VALIDATION_ERROR = 'The `factor` argument should be a number (or a list of two numbers) in the range [-1.0, 1.0]. '
    _SUPPORTED_FILL_MODE = ('reflect', 'wrap', 'constant', 'nearest')
    _SUPPORTED_INTERPOLATION = ('nearest', 'bilinear')

    def __init__(self, height_factor, width_factor=None, fill_mode='reflect', interpolation='bilinear', seed=None, fill_value=0.0, data_format=None, **kwargs):
        if False:
            for i in range(10):
                print('nop')
