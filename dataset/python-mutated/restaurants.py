...
class Restaurants(base.RemoteDataset):
    ...
    def __init__(self):
        if False:
            return 10
        super().__init__(n_samples=252108, n_features=7, task=base.REG, url='https://maxhalford.github.io/files/datasets/kaggle_recruit_restaurants.zip', size=28881242, filename='kaggle_recruit_restaurants.csv')

    def _iter(self):
        if False:
            return 10
        return stream.iter_csv(self.path, target='visitors', converters={'latitude': float, 'longitude': float, 'visitors': int, 'is_holiday': ast.literal_eval}, parse_dates={'date': '%Y-%m-%d'})