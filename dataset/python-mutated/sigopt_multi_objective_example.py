...
if __name__ == '__main__':
    ...
    space = [{'name': 'w1', 'type': 'double', 'bounds': {'min': 0, 'max': 1}}]
    algo = SigOptSearch(space, name='SigOpt Example Multi Objective Experiment', observation_budget=4 if args.smoke_test else 100, metric=['average', 'std', 'sharpe'], mode=['max', 'min', 'obs'])
    tuner = tune.Tuner(easy_objective, run_config=train.RunConfig(name='my_exp'), tune_config=tune.TuneConfig(search_alg=algo, num_samples=4 if args.smoke_test else 100), param_space={'total_weight': 1})
    results = tuner.fit()
