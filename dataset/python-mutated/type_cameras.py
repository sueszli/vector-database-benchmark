...
class Camera(HomeAccessory, PyhapCamera):
    ...
    async def run(self) -> None:
        ...
        if self._char_motion_detected:
            ...
            self._subscriptions.append(async_track_state_change_event(self.hass, [self.linked_motion_sensor], self._async_update_motion_state_event))
        if self._char_doorbell_detected:
            assert self.linked_doorbell_sensor
            self._subscriptions.append(async_track_state_change_event(self.hass, [self.linked_doorbell_sensor], self._async_update_doorbell_state_event))
        await super().run()
    ...
    async def start_stream(self, session_info: dict[str, Any], stream_config: dict[str, Any]) -> bool:
        ...
        if not opened:
            ...
            return False
        _LOGGER.info('[%s] Started stream process - PID %d', session_info['id'], stream.process.pid)
        ...
        session_info[FFMPEG_LOGGER] = asyncio.create_task(self._async_log_stderr_stream(stderr_reader))
