...
def leaked_processes():
    ...
    result = subprocess.check_output("ps aux | grep '{}' | grep -v grep || true".format(EnvWithSubprocess.UNIQUE_CMD), shell=True)
    return result
if __name__ == '__main__':
    ...
    run_experiments({'demo': {'run': 'PPO', 'env': 'subproc', 'num_samples': 1, 'config': {'num_workers': 1, 'env_config': {'tmp_file1': tmp1, 'tmp_file2': tmp2, 'tmp_file3': tmp3, 'tmp_file4': tmp4}, 'framework': 'tf'}, 'stop': {'training_iteration': 1}}})
    ray.shutdown()
