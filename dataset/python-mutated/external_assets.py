...
async def handle_report_asset_materialization_request(context: BaseWorkspaceRequestContext, request: Request) -> JSONResponse:
    ...
    if body_content_type is None:
        ...
        return JSONResponse({'error': f'Unhandled content type {body_content_type}, expect no body or application/json'}, status_code=400)
    asset_key = _asset_key_from_request(ReportAssetMatParam.asset_key, request, json_body)
    if asset_key is None:
        return JSONResponse({'error': 'Empty asset key, must provide asset key as url path after /report_asset_materialization/ or query param asset_key.'}, status_code=400)
    ...
    if ReportAssetMatParam.metadata in json_body:
        ...
        try:
            ...
            return JSONResponse({'error': f'Error parsing metadata json: {exc}'}, status_code=400)
    try:
        mat = AssetMaterialization(asset_key=asset_key, partition=partition, metadata=metadata, description=description, tags=tags)
    except Exception as exc:
        return JSONResponse({'error': f'Error constructing AssetMaterialization: {exc}'}, status_code=400)
    context.instance.report_runless_asset_event(mat)
    return JSONResponse({})

async def handle_report_asset_check_request(context: BaseWorkspaceRequestContext, request: Request) -> JSONResponse:
    body_content_type = request.headers.get('content-type')
    if body_content_type is None:
        json_body = {}
    elif body_content_type == 'application/json':
    ...
    if asset_key is None:
        return JSONResponse({'error': 'Empty asset key, must provide asset key as url path after /report_asset_check_evaluation/ or query param asset_key.'}, status_code=400)
    passed = _value_from_body_or_params(ReportAssetCheckEvalParam.passed, request, json_body)
    check_name = _value_from_body_or_params(ReportAssetCheckEvalParam.check_name, request, json_body)
    ...
    if ReportAssetCheckEvalParam.passed in json_body:
        ...
        try:
            ...
            return JSONResponse({'error': f"Error parsing 'passed': {exc}"}, status_code=400)
    else:
        return JSONResponse({'error': "Missing required parameter 'passed'."}, status_code=400)
    metadata = {}
    if ReportAssetCheckEvalParam.metadata in json_body:
        ...
        try:
            ...
            return JSONResponse({'error': f'Error parsing metadata json: {exc}'}, status_code=400)
    try:
        evaluation = AssetCheckEvaluation(check_name=check_name, passed=passed, asset_key=asset_key, metadata=metadata, severity=AssetCheckSeverity(severity))
    except Exception as exc:
        return JSONResponse({'error': f'Error constructing AssetMaterialization: {exc}'}, status_code=400)
    context.instance.report_runless_asset_event(evaluation)
    return JSONResponse({})

async def handle_report_asset_observation_request(context: BaseWorkspaceRequestContext, request: Request) -> JSONResponse:
    body_content_type = request.headers.get('content-type')
    if body_content_type is None:
        json_body = {}
    elif body_content_type == 'application/json':
    ...
    if data_version is not None:
        ...
        tags[DATA_VERSION_IS_USER_PROVIDED_TAG] = 'true'
    try:
        observation = AssetObservation(asset_key=asset_key, partition=partition, metadata=metadata, description=description, tags=tags)
    except Exception as exc:
        return JSONResponse({'error': f'Error constructing AssetMaterialization: {exc}'}, status_code=400)
