...
def serial_pipeline_pc(input_cfg: Union[str, Tuple[dict, dict]], seed: int=0, model: Optional[torch.nn.Module]=None, max_iter=int(1000000.0)) -> Union['Policy', bool]:
    if False:
        for i in range(10):
            print('nop')
    ...
    for epoch in range(cfg.policy.learn.train_epoch):
        ...
        for (_, test_data) in enumerate(test_dataloader):
            (observations, bfs_input_maps, bfs_output_maps) = (test_data['obs'], test_data['bfs_in'].long(), test_data['bfs_out'].long())
            states = observations
