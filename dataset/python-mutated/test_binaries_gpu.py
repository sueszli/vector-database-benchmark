...
from tests.utils import create_dummy_data, generate_main, preprocess_lm_data, preprocess_translation_data, train_language_model, train_translation_model
...
@unittest.skipIf(int(torch.__version__[2]) < 10, reason='quantized kernels are only supported on CPU')
@unittest.skipIf(not torch.cuda.is_available(), 'test requires a GPU')
