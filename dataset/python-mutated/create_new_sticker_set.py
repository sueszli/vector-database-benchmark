...
class CreateNewStickerSet(TelegramMethod[bool]):
    ...
    if TYPE_CHECKING:
        ...
        def __init__(__pydantic__self__, *, user_id: int, name: str, title: str, stickers: List[InputSticker], sticker_format: str, sticker_type: Optional[str]=None, needs_repainting: Optional[bool]=None, **__pydantic_kwargs: Any) -> None:
            if False:
                for i in range(10):
                    print('nop')
            super().__init__(user_id=user_id, name=name, title=title, stickers=stickers, sticker_format=sticker_format, sticker_type=sticker_type, needs_repainting=needs_repainting, **__pydantic_kwargs)