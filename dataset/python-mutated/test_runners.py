...
from faust.livecheck.exceptions import LiveCheckError, TestFailed, TestRaised, TestSkipped, TestTimeout
...
class test_TestRunner:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize('exc,raises,callback', [(TestSkipped('foo'), None, 'on_skipped'), (TestTimeout('bar'), None, 'on_timeout'), (AssertionError('baz'), TestFailed, 'on_failed'), (LiveCheckError('xuz'), None, 'on_error'), (KeyError('muz'), TestRaised, 'on_error')])
    async def test_execute__error_callbacks(self, exc, raises, callback, *, runner, execution):
        if raises is None:
            raises = type(exc)
        cb = AsyncMock()
        ...
        with pytest.raises(raises):
            await self._do_execute(runner, execution, active=True, expired=False, side_effect=exc)
    ...
    async def _do_execute(self, runner, execution, active=True, expired=False, side_effect=None):
