...
__all__ = ['create_start_link', 'create_startgroup_link', 'create_deep_link', 'create_telegram_link', 'encode_payload', 'decode_payload']
import re
...
BAD_PATTERN = re.compile('[^A-z0-9-]')

async def create_start_link(bot: Bot, payload: str, encode: bool=False, encoder: Optional[Callable[[bytes], bytes]]=None) -> str:
    """
    ...
    return create_deep_link(username=cast(str, username), link_type='start', payload=payload, encode=encode, encoder=encoder)

async def create_startgroup_link(bot: Bot, payload: str, encode: bool=False, encoder: Optional[Callable[[bytes], bytes]]=None) -> str:
    """
    ...
    """
    username = (await bot.me()).username
    return create_deep_link(username=cast(str, username), link_type='startgroup', payload=payload, encode=encode, encoder=encoder)
