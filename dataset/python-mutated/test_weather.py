...
from homeassistant.components.tomorrowio.config_flow import _get_config_schema, _get_unique_id
from homeassistant.components.tomorrowio.const import ATTRIBUTION, CONF_TIMESTEP, DEFAULT_NAME, DEFAULT_TIMESTEP, DOMAIN
from homeassistant.components.weather import ATTR_CONDITION_SUNNY, ATTR_FORECAST, ATTR_FORECAST_CONDITION, ATTR_FORECAST_DEW_POINT, ATTR_FORECAST_HUMIDITY, ATTR_FORECAST_PRECIPITATION, ATTR_FORECAST_PRECIPITATION_PROBABILITY, ATTR_FORECAST_TEMP, ATTR_FORECAST_TEMP_LOW, ATTR_FORECAST_TIME, ATTR_FORECAST_WIND_BEARING, ATTR_FORECAST_WIND_SPEED, ATTR_WEATHER_HUMIDITY, ATTR_WEATHER_OZONE, ATTR_WEATHER_PRECIPITATION_UNIT, ATTR_WEATHER_PRESSURE, ATTR_WEATHER_PRESSURE_UNIT, ATTR_WEATHER_TEMPERATURE, ATTR_WEATHER_TEMPERATURE_UNIT, ATTR_WEATHER_VISIBILITY, ATTR_WEATHER_VISIBILITY_UNIT, ATTR_WEATHER_WIND_BEARING, ATTR_WEATHER_WIND_SPEED, ATTR_WEATHER_WIND_SPEED_UNIT, DOMAIN as WEATHER_DOMAIN, SERVICE_GET_FORECAST
...
async def _setup_config_entry(hass: HomeAssistant, config: dict[str, Any]) -> State:
    ...
    config_entry = MockConfigEntry(title=DEFAULT_NAME, domain=DOMAIN, data=data, options={CONF_TIMESTEP: DEFAULT_TIMESTEP}, unique_id=_get_unique_id(hass, data), version=1)
...
async def _setup_legacy(hass: HomeAssistant, config: dict[str, Any]) -> State:
    ...
    for entity_name in ('hourly', 'nowcast'):
        registry.async_get_or_create(WEATHER_DOMAIN, DOMAIN, f'{_get_unique_id(hass, data)}_{entity_name}', disabled_by=er.RegistryEntryDisabler.INTEGRATION, suggested_object_id=f'tomorrow_io_{entity_name}')
    with freeze_time(datetime(2021, 3, 6, 23, 59, 59, tzinfo=dt_util.UTC)) as frozen_time:
        await _setup_config_entry(hass, config)
        for entity_name in ('hourly', 'nowcast'):
...
async def test_legacy_config_entry(hass: HomeAssistant) -> None:
    ...
    for entity_name in ('hourly', 'nowcast'):
        registry.async_get_or_create(WEATHER_DOMAIN, DOMAIN, f'{_get_unique_id(hass, data)}_{entity_name}')
    await _setup(hass, API_V4_ENTRY_DATA)
...
async def test_v4_weather(hass: HomeAssistant, tomorrowio_config_entry_update) -> None:
    ...
    assert weather_state.attributes[ATTR_FORECAST][0] == {ATTR_FORECAST_CONDITION: ATTR_CONDITION_SUNNY, ATTR_FORECAST_TIME: '2021-03-07T11:00:00+00:00', ATTR_FORECAST_PRECIPITATION: 0, ATTR_FORECAST_PRECIPITATION_PROBABILITY: 0, ATTR_FORECAST_TEMP: 45.9, ATTR_FORECAST_TEMP_LOW: 26.1, ATTR_FORECAST_DEW_POINT: 12.8, ATTR_FORECAST_HUMIDITY: 58, ATTR_FORECAST_WIND_BEARING: 239.6, ATTR_FORECAST_WIND_SPEED: 34.16}
    ...
    assert weather_state.attributes[ATTR_WEATHER_WIND_SPEED] == 33.59
...
async def test_v4_forecast_service(hass: HomeAssistant, snapshot: SnapshotAssertion) -> None:
    """Test multiple forecast."""
    weather_state = await _setup(hass, API_V4_ENTRY_DATA)
    entity_id = weather_state.entity_id
    for forecast_type in ('daily', 'hourly'):
        response = await hass.services.async_call(WEATHER_DOMAIN, SERVICE_GET_FORECAST, {'entity_id': entity_id, 'type': forecast_type}, blocking=True, return_response=True)
        assert response['forecast'] != []
        assert response == snapshot

async def test_v4_bad_forecast(hass: HomeAssistant, freezer: FrozenDateTimeFactory, tomorrowio_config_entry_update, snapshot: SnapshotAssertion) -> None:
    ...
    await hass.async_block_till_done()
    response = await hass.services.async_call(WEATHER_DOMAIN, SERVICE_GET_FORECAST, {'entity_id': entity_id, 'type': 'hourly'}, blocking=True, return_response=True)
    assert response['forecast'][0]['precipitation_probability'] is None

@pytest.mark.parametrize('forecast_type', ['daily', 'hourly'])
async def test_forecast_subscription(hass: HomeAssistant, hass_ws_client: WebSocketGenerator, freezer: FrozenDateTimeFactory, snapshot: SnapshotAssertion, forecast_type: str) -> None:
    """Test multiple forecast."""
    client = await hass_ws_client(hass)
    freezer.move_to(datetime(2021, 3, 6, 23, 59, 59, tzinfo=dt_util.UTC))
    weather_state = await _setup(hass, API_V4_ENTRY_DATA)
    entity_id = weather_state.entity_id
    await client.send_json_auto_id({'type': 'weather/subscribe_forecast', 'forecast_type': forecast_type, 'entity_id': entity_id})
