...
def upgrade():
    if False:
        print('Hello World!')
    op.create_index(op.f('ix_ses_emails_status'), 'ses_emails', ['status'], unique=False)
    op.drop_constraint('ses_events_email_id_fkey', 'ses_events', type_='foreignkey')
    op.create_foreign_key(None, 'ses_events', 'ses_emails', ['email_id'], ['id'], ondelete='CASCADE', initially='DEFERRED', deferrable=True)
