...
class MachineReadingComprehensionTest(unittest.TestCase):
    ...
    def test_run_mrc_for_ner_by_direct_model_download(self):
        ...
        pipeline1 = MachineReadingComprehensionForNERPipeline(model, preprocessor=tokenizer)
        pipeline2 = pipeline(Tasks.machine_reading_comprehension, model=model, preprocessor=tokenizer)
    ...
    def test_run_mrc_for_ner_with_model_from_modelhub(self):
        ...
        pipeline_ins = pipeline(task=Tasks.machine_reading_comprehension, model=model, preprocessor=tokenizer)
        print(f'sentence: {self.sentence}\npipeline:{pipeline_ins(input=self.sentence)}')
    ...
    def test_run_mrc_for_ner_with_model_name(self):
        if False:
            while True:
                i = 10
        pipeline_ins = pipeline(task=Tasks.machine_reading_comprehension, model=self.model_id)
