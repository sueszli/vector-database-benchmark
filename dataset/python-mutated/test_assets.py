...
from zipline.assets import Asset, ExchangeInfo, Equity, Future, AssetDBWriter, AssetFinder
from zipline.assets.assets import OwnershipPeriod
from zipline.assets.synthetic import make_commodity_future_info, make_rotating_equity_info, make_simple_equity_info
from six import itervalues, integer_types
from toolz import valmap, concat
from zipline.assets.asset_writer import check_version_info, write_version_info, _futures_defaults, SQLITE_MAX_VARIABLE_NUMBER
from zipline.assets.asset_db_schema import ASSET_DB_VERSION
from zipline.assets.asset_db_migrations import downgrade
from zipline.errors import EquitiesNotFound, FutureContractsNotFound, MultipleSymbolsFound, MultipleSymbolsFoundForFuzzySymbol, MultipleValuesFoundForField, MultipleValuesFoundForSid, NoValueForSid, AssetDBVersionError, SameSymbolUsedAcrossCountries, SidsNotFound, SymbolNotFound, AssetDBImpossibleDowngrade, ValueNotFoundForField
from zipline.testing import all_subindices, empty_assets_db, parameter_space, powerset, tmp_assets_db, tmp_asset_finder
from zipline.testing.predicates import assert_equal, assert_not_equal
from zipline.testing.fixtures import WithAssetFinder, ZiplineTestCase, WithTradingCalendars, WithTmpDir, WithInstanceTmpDir
...
class AssetTestCase(TestCase):
    asset_attrs = [name for (name, value) in vars(Asset).items() if isinstance(value, GetSetDescriptorType)]
    asset = Asset(1337, symbol='DOGE', asset_name='DOGECOIN', start_date=pd.Timestamp('2013-12-08 9:31', tz='UTC'), end_date=pd.Timestamp('2014-06-25 11:21', tz='UTC'), first_traded=pd.Timestamp('2013-12-08 9:31', tz='UTC'), auto_close_date=pd.Timestamp('2014-06-26 11:21', tz='UTC'), exchange_info=ExchangeInfo('THE MOON', 'MOON', '??'))
    ...
    asset5 = Asset(5, exchange_info=ExchangeInfo('still testing', 'still testing', '??'))
