...
class SaleQuoteTemplate(models.Model):
    ...
    require_payment = fields.Selection([(0, 'Not mandatory on website quote validation'), (1, 'Immediate after website order validation'), (2, 'Immediate after website order validation and save a token')], 'Payment', help='Require immediate payment by the customer when validating the order from the website quote')
...
class SaleQuoteLine(models.Model):
    ...
    _order = 'sequence, id'
    sequence = fields.Integer('Sequence', help='Gives the sequence order when displaying a list of sale quote lines.', default=10)
    quote_id = fields.Many2one('sale.quote.template', 'Quotation Template Reference', required=True, ondelete='cascade', index=True)
...
class SaleQuoteOption(models.Model):
    ...
    template_id = fields.Many2one('sale.quote.template', 'Quotation Template Reference', ondelete='cascade', index=True, required=True)
    name = fields.Text('Description', required=True, translate=True)
    product_id = fields.Many2one('product.product', 'Product', domain=[('sale_ok', '=', True)], required=True)
    layout_category_id = fields.Many2one('sale.layout_category', string='Section')
