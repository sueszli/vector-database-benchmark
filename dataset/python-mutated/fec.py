...
from visidata import vd, Path, Sheet, TextSheet, Column, ColumnAttr, ColumnItem, ENTER, asyncthread, Progress, addGlobals
...
class DiveSheet(Sheet):
    ...
    def dive(self):
        ...
        if success == False:
            return
        vd.push(vs)
DiveSheet.addCommand(ENTER, 'dive-row', 'vd.sheet.dive()')
...
class FECItemizationSheet(Sheet):
    ...
    def dive(self):
        ...
        vs.reload()
        vd.push(vs)
FECItemizationSheet.addCommand(ENTER, 'dive-row', 'vd.sheet.dive()')

class FECScheduleSheet(Sheet):
    """A sheet to display the list of itemized schedules in a filing."""
    rowtype = 'schedules'
    columns = [ColumnAttr('schedule', 'schedule_name', width=14), ColumnAttr('name', width=0), ColumnAttr('size', type=int)]
    nKeys = 1
    ...
    def reload(self):
        ...
        for schedule_name in self.source.keys():
            ...
            self.addRow(vs)
FECScheduleSheet.addCommand(ENTER, 'dive-row', 'vd.push(cursorRow)')
COMPONENT_SHEET_CLASSES = {'header': DiveSheet, 'summary': DiveSheet, 'itemization': FECScheduleSheet, 'text': FECItemizationSheet, 'F99_text': TextSheet}

class FECFiling(Sheet):
    """A sheet representing an entire .fec file."""
    rowtype = 'components'
    filing = None
    columns = [ColumnAttr('component', 'component_name', width=14), ColumnAttr('name', width=0), ColumnAttr('size', type=int)]
    nKeys = 1
    ...
    def reload(self):
        ...
        for item in item_iter:
            ...
            if dtype in ['header', 'summary']:
                ...
                sheet_row.size += 1
FECFiling.addCommand(ENTER, 'dive-row', 'vd.push(cursorRow)')

def open_fec(p):
    if False:
        print('Hello World!')
    return FECFiling(p.name, source=p)
addGlobals({'open_fec': open_fec, 'DiveSheet': DiveSheet})