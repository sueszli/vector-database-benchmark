...
def upgrade():
    if False:
        return 10
    op.create_index(op.f('ix_projects_sitemap_bucket'), 'projects', ['sitemap_bucket'], unique=False)
    op.create_index(op.f('ix_users_sitemap_bucket'), 'users', ['sitemap_bucket'], unique=False)
    op.create_index('journakls_submitted_date_id_idx', 'journals', ['submitted_date', 'id'], unique=False)
