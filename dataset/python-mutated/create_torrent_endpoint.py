...
from tribler.core.components.restapi.rest.rest_endpoint import HTTP_BAD_REQUEST, RESTEndpoint, RESTResponse, MAX_REQUEST_SIZE
...
class CreateTorrentEndpoint(RESTEndpoint):
    ...
    @docs(tags=['Libtorrent'], summary='Create a torrent from local files and return it in base64 encoding.', parameters=[{'in': 'query', 'name': 'download', 'description': 'Flag indicating whether or not to start downloading', 'type': 'boolean', 'required': False}], responses={200: {'schema': schema(CreateTorrentResponse={'torrent': 'base64 encoded torrent file'}), 'examples': {'Success': {'success': True}}}, HTTP_BAD_REQUEST: {'schema': HandledErrorSchema, 'examples': {'Error': {'error': 'files parameter missing'}}}})
    @json_schema(schema(CreateTorrentRequest={'files': [String], 'name': String, 'description': String, 'trackers': [String], 'export_dir': String}))
