...
class test_gam_gridsearch_specific:
    ...
    myX = []
    myY = []
    hyper_parameters = {'alpha': [0.1, 0.9], 'lambda': [0.01], 'scale': [[1, 1, 1], [2, 2, 2]], 'num_knots': [[5, 5, 5], [5, 6, 7]]}
    manual_gam_models = []
    h2o_model = []
    num_grid_models = 0
    num_expected_models = len(hyper_parameters['alpha']) * len(hyper_parameters['lambda'] * len(hyper_parameters['scale'] * len(hyper_parameters['num_knots'])))
            ...
            for alpha_param in self.hyper_parameters['alpha']:
                for scale_param in self.hyper_parameters['scale']:
                    for num_knots_param in self.hyper_parameters['num_knots']:
                        self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family='gaussian', gam_columns=['C11', 'C12', 'C13'], keep_gam_cols=True, scale=scale_param, bs=[3, 2, 0], num_knots=num_knots_param, alpha=alpha_param, lambda_=lambda_param))

    def train_models(self):
        if False:
            i = 10
            return i + 15
        self.h2o_model = H2OGridSearch(H2OGeneralizedAdditiveEstimator(family='gaussian', gam_columns=['C11', 'C12', 'C13'], keep_gam_cols=True, bs=[3, 2, 0]), self.hyper_parameters)
    ...
    def match_models(self):
        ...
        for model in self.manual_gam_models:
            ...
            for grid_search_model in self.h2o_model.models:
                if grid_search_model.actual_params['alpha'] == alpha and grid_search_model.actual_params['lambda'] == lambda_ and (grid_search_model.actual_params['scale'] == scale) and (grid_search_model.actual_params['num_knots'] == num_knots):
                    self.num_grid_models += 1
                    pyunit_utils.assertCoefDictEqual(grid_search_model.coef(), model.coef())
                    break
