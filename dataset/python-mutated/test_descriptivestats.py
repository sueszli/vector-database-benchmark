...
from statsmodels.stats.descriptivestats import Description, describe, sign_test
pytestmark = pytest.mark.filterwarnings('ignore::DeprecationWarning:statsmodels.stats.descriptivestats')
...
def test_sign_test():
    ...
    assert_equal(M, 4)
data5 = [[25, 'Bob', True, 1.2], [41, 'John', False, 0.5], [30, 'Alice', True, 0.3]]
data1 = np.array([(1, 2, 'a', 'aa'), (2, 3, 'b', 'bb'), (2, 4, 'b', 'cc')], dtype=[('alpha', float), ('beta', int), ('gamma', '|S1'), ('delta', '|S2')])
...
SPECIAL = (('ci', ('lower_ci', 'upper_ci')), ('jarque_bera', ('jarque_bera', 'jarque_bera_pval')), ('mode', ('mode', 'mode_freq')), ('top', tuple([f'top_{i}' for i in range(1, 6)])), ('freq', tuple([f'freq_{i}' for i in range(1, 6)])))
