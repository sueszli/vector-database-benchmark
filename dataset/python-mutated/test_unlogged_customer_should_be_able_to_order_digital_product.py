import pytest
from ..product.utils import create_category, create_digital_content, create_product, create_product_channel_listing, create_product_type, create_product_variant, create_product_variant_channel_listing
from ..shop.utils.preparing_shop import prepare_shop
from ..utils import assign_permissions
from .utils import checkout_billing_address_update, checkout_complete, checkout_create, checkout_dummy_payment_create
...
def prepare_product(e2e_staff_api_client):
    ...
    product_type_data = create_product_type(e2e_staff_api_client, is_shipping_required=False, is_digital=True)
...
@pytest.mark.e2e
def test_process_checkout_with_digital_product_CORE_0101(e2e_not_logged_api_client, e2e_staff_api_client, permission_manage_product_types_and_attributes, permission_manage_channels, permission_manage_products, permission_manage_shipping):
    if False:
        print('Hello World!')
    permissions = [permission_manage_product_types_and_attributes, permission_manage_channels, permission_manage_products, permission_manage_shipping]
    assign_permissions(e2e_staff_api_client, permissions)
    (product_variant_id, channel_slug) = prepare_product(e2e_staff_api_client)
