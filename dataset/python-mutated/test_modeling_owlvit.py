...
class OwlViTModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': OwlViTModel, 'zero-shot-object-detection': OwlViTForObjectDetection} if is_torch_available() else {}
    fx_compatible = False
    test_head_masking = False
    test_pruning = False
    test_resize_embeddings = False
...
class OwlViTModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_object_detection(self):
        ...
        expected_slice_boxes = torch.tensor([[0.0691, 0.0445, 0.1373], [0.1592, 0.0456, 0.3192], [0.1632, 0.0423, 0.2478]]).to(torch_device)
