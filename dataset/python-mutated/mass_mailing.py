...
class MassMailingContact(models.Model):
    ...
    list_id = fields.Many2one('mail.mass_mailing.list', string='Mailing List', ondelete='cascade', required=True, default=lambda self: self.env['mail.mass_mailing.list'].search([], limit=1, order='id desc'))
...
class MassMailingCampaign(models.Model):
    ...
    stage_id = fields.Many2one('mail.mass_mailing.stage', string='Stage', required=True, default=lambda self: self.env['mail.mass_mailing.stage'].search([], limit=1))
    user_id = fields.Many2one('res.users', string='Responsible', required=True, default=lambda self: self.env.uid)
    campaign_id = fields.Many2one('utm.campaign', 'campaign_id', required=True, ondelete='cascade', help='This name helps you tracking your different campaign efforts, e.g. Fall_Drive, Christmas_Special')
    source_id = fields.Many2one('utm.source', string='Source', help='This is the link source, e.g. Search Engine, another domain,or name of email list', default=lambda self: self.env.ref('utm.utm_source_newsletter'))
    medium_id = fields.Many2one('utm.medium', string='Medium', help='This is the delivery method, e.g. Postcard, Email, or Banner Ad', default=lambda self: self.env.ref('utm.utm_medium_email'))
    tag_ids = fields.Many2many('mail.mass_mailing.tag', 'mail_mass_mailing_tag_rel', 'tag_id', 'campaign_id', string='Tags')
    mass_mailing_ids = fields.One2many('mail.mass_mailing', 'mass_mailing_campaign_id', string='Mass Mailings')
    unique_ab_testing = fields.Boolean(string='AB Testing', help='If checked, recipients will be mailed only once, allowing to send various mailings in a single campaign to test the effectiveness of the mailings.')
    color = fields.Integer(string='Color Index')
...
class MassMailing(models.Model):
    ...
    active = fields.Boolean(default=True)
    email_from = fields.Char(string='From', required=True, default=lambda self: self.env['mail.message']._get_default_from())
    create_date = fields.Datetime(string='Creation Date')
    ...
    attachment_ids = fields.Many2many('ir.attachment', 'mass_mailing_ir_attachments_rel', 'mass_mailing_id', 'attachment_id', string='Attachments')
    keep_archives = fields.Boolean(string='Keep Archives')
    mass_mailing_campaign_id = fields.Many2one('mail.mass_mailing.campaign', string='Mass Mailing Campaign')
    campaign_id = fields.Many2one('utm.campaign', string='Campaign', help='This name helps you tracking your different campaign efforts, e.g. Fall_Drive, Christmas_Special')
    source_id = fields.Many2one('utm.source', string='Subject', required=True, ondelete='cascade', help='This is the link source, e.g. Search Engine, another domain, or name of email list')
    medium_id = fields.Many2one('utm.medium', string='Medium', help='This is the delivery method, e.g. Postcard, Email, or Banner Ad', default=lambda self: self.env.ref('utm.utm_medium_email'))
    clicks_ratio = fields.Integer(compute='_compute_clicks_ratio', string='Number of Clicks')
    state = fields.Selection([('draft', 'Draft'), ('in_queue', 'In Queue'), ('sending', 'Sending'), ('done', 'Sent')], string='Status', required=True, copy=False, default='draft')
    color = fields.Integer(related='mass_mailing_campaign_id.color', string='Color Index')
    reply_to_mode = fields.Selection([('thread', 'Followers of leads/applicants'), ('email', 'Specified Email Address')], string='Reply-To Mode', required=True)
    reply_to = fields.Char(string='Reply To', help='Preferred Reply-To Address', default=lambda self: self.env['mail.message']._get_default_from())
    ...
    contact_list_ids = fields.Many2many('mail.mass_mailing.list', 'mail_mass_mailing_list_rel', string='Mailing Lists')
    contact_ab_pc = fields.Integer(string='A/B Testing percentage', help='Percentage of the contacts that will be mailed. Recipients will be taken randomly.', default=100)
