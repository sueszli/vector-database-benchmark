...
class test_agc(gr_unittest.TestCase):
    ...
    def test_003_sets(self):
        if False:
            print('Hello World!')
        agc = analog.agc2_cc(0.001, 0.1, 1, 1)
    ...
    def test_004_sets(self):
        if False:
            print('Hello World!')
        agc = analog.agc2_ff(0.001, 0.1, 1, 1)
        agc.set_attack_rate(1)
        agc.set_decay_rate(2)
        agc.set_reference(1.1)
        agc.set_gain(1.1)
    ...
    def test_006_000_agc3_setters(self):
        if False:
            for i in range(10):
                print('nop')
        agc = analog.agc3_cc(0.001, 0.1, 1)
        agc.set_attack_rate(1)
        agc.set_decay_rate(2)
        agc.set_reference(1.1)
        agc.set_gain(1.1)
    ...
    def test_006_001_agc3(self, stride=1):
        ...
        ref = 1
        agc = analog.agc3_cc(0.01, 0.001, ref, ref, stride)
        tb.connect(src1, agc)
        tb.connect(agc, head)
        ...
        for (idx, x) in enumerate(result):
            self.assertAlmostEqual(x, ref, 4, f'failed at pos {idx} (stride = {stride})')
    ...
    def test_006_003_agc3_setter_errors(self):
        ...
        settables = ('attack_rate', 'decay_rate', 'reference', 'gain', 'max_gain')
        troublemakers = (getattr(default_src, f'set_{prop}') for prop in settables)
    ...
    def test_006_004_agc3(self, stride=1):
        ...
        carrier_freq = 200
        envelope_freq = 5
        N = samp_rate / envelope_freq
        ref = 1.0
        m = 0.99
        mul = blocks.multiply_cc(1)
        carrier_src = analog.sig_source_c(samp_rate, analog.GR_COS_WAVE, carrier_freq, 1.0, 0.0, 0)
        envelope_src = analog.sig_source_c(samp_rate, analog.GR_COS_WAVE, envelope_freq, m, 1, 0)
        ...
        result = [abs(x) for x in dst_data]
        for (idx, x) in enumerate(result):
            self.assertAlmostEqual(x, ref, None, f'failed at pos {idx} (stride = {stride})', 0.1)
