...
import re
import subprocess
device = {'name': 'hwmt40p', 'login_name': 'hwmt40p-K9000-maliG78', 'ip': 'box86.br.megvii-inc.com', 'port': 2200, 'thread_number': 3}
...
def main():
    ...
    for b in thread_number:
        cmd = []
        cmd1 = 'cd {} && ./load_and_run {} -multithread {} --fast-run --fast_run_algo_policy fastrun.cache --iter 1 --warmup-iter 1 --no-sanity-check --weight-preprocess'.format(workspace, m, b)
        cmd2 = 'cd {} && ./load_and_run {} -multithread {} --fast_run_algo_policy fastrun.cache --iter 20 --warmup-iter 5 --no-sanity-check --weight-preprocess '.format(workspace, m, b)
        cmd.append(cmd1)
        cmd.append(cmd2)
        raw_log = ssh.cmd(cmd)
