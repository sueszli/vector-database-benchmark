...
from legacy_test.test_dist_base import TestDistRunnerBase, _insert_comm_op, runtime_main
import paddle
...
def cnn_model(data):
    if False:
        for i in range(10):
            print('nop')
    conv_pool_1 = simple_img_conv_pool(input=data, filter_size=5, num_filters=20, pool_size=2, pool_stride=2, act='relu', param_attr=base.ParamAttr(initializer=paddle.nn.initializer.Constant(value=0.01)))
    conv_pool_2 = simple_img_conv_pool(input=conv_pool_1, filter_size=5, num_filters=50, pool_size=2, pool_stride=2, act='relu', param_attr=base.ParamAttr(initializer=paddle.nn.initializer.Constant(value=0.01)))
    SIZE = 10
...
class TestDistMnistDGC(TestDistRunnerBase):
    ...
    def get_model(self, batch_size=2, use_dgc=False, build_strategy=None):
        ...
        if not use_dgc:
            ...
            opt = paddle.distributed.fleet.meta_optimizers.DGCMomentumOptimizer(learning_rate=self.lr, momentum=0.9, rampup_begin_step=2, num_trainers=build_strategy.num_trainers if build_strategy else None)
        if use_dgc:
            assert build_strategy is not None, 'build_strategy can be None with dgc'
