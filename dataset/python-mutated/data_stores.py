...
if DATABASE_URL:
    ...
    if ssl_configurations:
        ...
        ssl_configuration = ''
    DATABASE_URL = 'postgres://{}{}{}{}:{}/{}{}'.format(DATABASES['default']['USER'], ':' + DATABASES['default']['PASSWORD'] if DATABASES['default']['PASSWORD'] else '', '@' if DATABASES['default']['USER'] or DATABASES['default']['PASSWORD'] else '', DATABASES['default']['HOST'], DATABASES['default']['PORT'], DATABASES['default']['NAME'], ssl_configuration)
else:
...
CLICKHOUSE_ALLOW_PER_SHARD_EXECUTION = get_from_env('CLICKHOUSE_ALLOW_PER_SHARD_EXECUTION', False, type_cast=str_to_bool)
try:
...
SESSION_RECORDING_KAFKA_MAX_REQUEST_SIZE_BYTES: int = get_from_env('SESSION_RECORDING_KAFKA_MAX_REQUEST_SIZE_BYTES', 1024 * 1024, type_cast=int)
KAFKA_SECURITY_PROTOCOL = os.getenv('KAFKA_SECURITY_PROTOCOL', None)
SESSION_RECORDING_KAFKA_SECURITY_PROTOCOL = os.getenv('SESSION_RECORDING_KAFKA_SECURITY_PROTOCOL', KAFKA_SECURITY_PROTOCOL)
...
KAFKA_EVENTS_PLUGIN_INGESTION: str = f'{KAFKA_PREFIX}events_plugin_ingestion{SUFFIX}'
KAFKA_EVENTS_PLUGIN_INGESTION_TOPIC: str = os.getenv('KAFKA_EVENTS_PLUGIN_INGESTION_TOPIC', KAFKA_EVENTS_PLUGIN_INGESTION)
...
if not REDIS_URL and get_from_env('POSTHOG_REDIS_HOST', ''):
    REDIS_URL = 'redis://:{}@{}:{}/'.format(os.getenv('POSTHOG_REDIS_PASSWORD', ''), os.getenv('POSTHOG_REDIS_HOST', ''), os.getenv('POSTHOG_REDIS_PORT', '6379'))
SESSION_RECORDING_REDIS_URL = REDIS_URL
if get_from_env('POSTHOG_SESSION_RECORDING_REDIS_HOST', ''):
    SESSION_RECORDING_REDIS_URL = 'redis://{}:{}/'.format(os.getenv('POSTHOG_SESSION_RECORDING_REDIS_HOST', ''), os.getenv('POSTHOG_SESSION_RECORDING_REDIS_PORT', '6379'))
if not REDIS_URL:
    raise ImproperlyConfigured('Env var REDIS_URL or POSTHOG_REDIS_HOST is absolutely required to run this software.\nIf upgrading from PostHog 1.0.10 or earlier, see here: https://posthog.com/docs/deployment/upgrading-posthog#upgrading-from-before-1011')
...
CACHES = {'default': {'BACKEND': 'django_redis.cache.RedisCache', 'LOCATION': REDIS_URL if not REDIS_READER_URL else [REDIS_URL, REDIS_READER_URL], 'OPTIONS': {'CLIENT_CLASS': 'django_redis.client.DefaultClient', 'COMPRESSOR': 'posthog.caching.tolerant_zlib_compressor.TolerantZlibCompressor'}, 'KEY_PREFIX': 'posthog'}}
if TEST:
