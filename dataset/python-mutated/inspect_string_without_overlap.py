...
def inspect_string_without_overlap(project: str, content_string: str) -> None:
    ...
    custom_info_types = [{'info_type': {'name': 'EMAIL_ADDRESS'}, 'exclusion_type': google.cloud.dlp_v2.CustomInfoType.ExclusionType.EXCLUSION_TYPE_EXCLUDE}]
    rule_set = [{'info_types': [{'name': 'DOMAIN_NAME'}], 'rules': [{'exclusion_rule': {'exclude_info_types': {'info_types': [{'name': 'EMAIL_ADDRESS'}]}, 'matching_type': google.cloud.dlp_v2.MatchingType.MATCHING_TYPE_PARTIAL_MATCH}}]}]
    inspect_config = {'info_types': info_types_to_locate, 'custom_info_types': custom_info_types, 'rule_set': rule_set, 'include_quote': True}
