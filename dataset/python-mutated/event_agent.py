...
class EventAgent(dashboard_utils.DashboardAgentModule):
    ...
    async def _connect_to_dashboard(self):
        ...
        while True:
            try:
                dashboard_rpc_address = await self._gcs_aio_client.internal_kv_get(dashboard_consts.DASHBOARD_RPC_ADDRESS.encode(), namespace=ray_constants.KV_NAMESPACE_DASHBOARD, timeout=1)
                ...
                if dashboard_rpc_address:
                    ...
                    channel = utils.init_grpc_channel(dashboard_rpc_address, options=options, asynchronous=True)
                    return event_pb2_grpc.ReportEventServiceStub(channel)
            except Exception:
                logger.exception('Connect to dashboard failed.')
            await asyncio.sleep(event_consts.RETRY_CONNECT_TO_DASHBOARD_INTERVAL_SECONDS)
    ...
    async def report_events(self):
        ...
        for _ in range(event_consts.EVENT_AGENT_RETRY_TIMES):
            ...
            logger.error('Report event failed: %s', data_str[:limit] + (data_str[limit:] and '...'))
    ...
    async def get_internal_states(self):
        ...
        return {'total_events_reported': self.total_event_reported, 'Total_report_request': self.total_request_sent, 'queue_size': self._cached_events.qsize(), 'total_uptime': elapsed}

    async def run(self, server):
        self._stub = await self._connect_to_dashboard()
        self._monitor = monitor_events(self._event_dir, lambda data: create_task(self._cached_events.put(data)), self.monitor_thread_pool_executor)
        await asyncio.gather(self.report_events())
