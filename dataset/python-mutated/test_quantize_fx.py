...
from torch.ao.quantization.quantize_fx import prepare_fx, convert_fx, convert_to_reference_fx, _convert_to_reference_decomposed_fx, prepare_qat_fx, fuse_fx
from torch.ao.quantization.fx.quantize_handler import DefaultNodeQuantizeHandler
from torch.ao.quantization.fx.match_utils import _is_match, MatchAllNode
from torch.ao.quantization import QuantType
from torch.ao.quantization.quant_type import _get_quant_type_to_str
from torch.ao.quantization import QuantStub, DeQuantStub, QuantWrapper, default_qconfig, default_dynamic_qconfig, default_per_channel_qconfig, default_qat_qconfig, default_reuse_input_qconfig, default_symmetric_qnnpack_qconfig, default_symmetric_qnnpack_qat_qconfig, per_channel_dynamic_qconfig, float16_dynamic_qconfig, float16_static_qconfig, float_qparams_weight_only_qconfig, float_qparams_weight_only_qconfig_4bit, get_default_qconfig, get_default_qat_qconfig, get_default_qconfig_mapping, get_default_qat_qconfig_mapping, fuse_modules, fuse_modules_qat, prepare, prepare_qat, convert, quantize_dynamic, default_placeholder_observer, default_weight_observer, PerChannelMinMaxObserver, FixedQParamsFakeQuantize, FixedQParamsObserver, FusedMovingAvgObsFakeQuantize, FakeQuantize, MovingAverageMinMaxObserver, HistogramObserver, ReuseInputObserver, QConfig, default_embedding_qat_qconfig
from torch.ao.quantization.backend_config import get_fbgemm_backend_config, get_qnnpack_backend_config, BackendConfig, BackendPatternConfig, DTypeConfig, DTypeWithConstraints, ObservationType
from torch.ao.quantization.backend_config.native import get_test_only_legacy_native_backend_config
from torch.ao.quantization.qconfig_mapping import _get_symmetric_qnnpack_qconfig_mapping, _get_symmetric_qnnpack_qat_qconfig_mapping, _GLOBAL_DICT_KEY, _MODULE_NAME_DICT_KEY, _MODULE_NAME_OBJECT_TYPE_ORDER_DICT_KEY, _MODULE_NAME_REGEX_DICT_KEY, _OBJECT_TYPE_DICT_KEY, QConfigMapping
from torch.ao.quantization.fx.qconfig_mapping_utils import _get_object_type_qconfig, _get_module_name_qconfig, _get_module_name_regex_qconfig, _maybe_adjust_qconfig_for_module_name_object_type_order
from torch.ao.quantization.fx.pattern_utils import _DEFAULT_FUSION_PATTERNS, _DEFAULT_QUANTIZATION_PATTERNS, _DEFAULT_OUTPUT_FAKE_QUANTIZE_MAP, _DEFAULT_OUTPUT_OBSERVER_MAP, _register_fusion_pattern, _register_quant_pattern, get_default_output_activation_post_process_map
from torch.ao.quantization.fx.custom_config import STANDALONE_MODULE_NAME_DICT_KEY, STANDALONE_MODULE_CLASS_DICT_KEY, FLOAT_TO_OBSERVED_DICT_KEY, OBSERVED_TO_QUANTIZED_DICT_KEY, NON_TRACEABLE_MODULE_NAME_DICT_KEY, NON_TRACEABLE_MODULE_CLASS_DICT_KEY, INPUT_QUANTIZED_INDEXES_DICT_KEY, OUTPUT_QUANTIZED_INDEXES_DICT_KEY, PRESERVED_ATTRIBUTES_DICT_KEY, FuseCustomConfig, ConvertCustomConfig, PrepareCustomConfig, StandaloneModuleConfigEntry
import torch.ao.quantization.fx.lstm_utils
from torch.ao.quantization.fx.utils import _reroute_tuple_getitem_pattern, NodeInfo
...
from torch.ao.quantization.observer import default_fixed_qparams_range_0to1_observer, default_fixed_qparams_range_neg1to1_observer, MinMaxObserver, _is_activation_post_process
from hypothesis import given, settings
from hypothesis import strategies as st
from torch.testing._internal.common_cuda import TEST_MULTIGPU, TEST_CUDA
from torch.testing._internal.common_quantization import LinearReluLinearModel, LinearReluModel, LinearBnLeakyReluModel, LinearTanhModel, ConvBnAddReluModel, QuantizationTestCase, skipIfNoFBGEMM, skipIfNoQNNPACK, skip_if_no_torchvision, train_one_epoch, run_ddp, test_only_eval_fn, test_only_train_fn, ModelForConvTransposeBNFusion, get_supported_device_types, skipIfNoONEDNN
from torch.testing._internal.common_quantization import LinearModelWithSubmodule, ResNetBase, RNNDynamicModel, RNNCellDynamicModel
from torch.testing._internal.common_quantized import supported_qengines, override_qengines, override_quantized_engine
