...
from seaborn._testing import assert_plots_equal, assert_colors_equal
from seaborn._compat import get_legend_handles
rs = np.random.RandomState(0)

class TestFacetGrid:
    df = pd.DataFrame(dict(x=rs.normal(size=60), y=rs.gamma(4, size=60), a=np.repeat(list('abc'), 20), b=np.tile(list('mn'), 30), c=np.tile(list('tuv'), 20), d=np.tile(list('abcdefghijkl'), 5)))
...
class TestPairGrid:
    ...
    df = pd.DataFrame(dict(x=rs.normal(size=60), y=rs.randint(0, 4, size=60), z=rs.gamma(3, size=60), a=np.repeat(list('abc'), 20), b=np.repeat(list('abcdefghijkl'), 5)))
