...
def cv_nfolds_gbm():
    ...
    try:
        set_best_cv(False)
        model_default = H2OGradientBoostingEstimator(nfolds=5, distribution='bernoulli', ntrees=500, score_tree_interval=3, stopping_rounds=2, seed=42)
        try:
            set_parallel(True)
        ...
        model_sequential = H2OGradientBoostingEstimator(nfolds=5, distribution='bernoulli', ntrees=500, score_tree_interval=3, stopping_rounds=2, seed=42)
