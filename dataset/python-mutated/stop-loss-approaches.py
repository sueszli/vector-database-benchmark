...
class ManualStopOrStopTrail(BaseStrategy):
    params = dict(stop_loss=0.02, trail=False)
...
class AutoStopOrStopTrail(BaseStrategy):
    params = dict(stop_loss=0.02, trail=False, buy_limit=False)
    buy_order = None
...
APPROACHES = dict(manual=ManualStopOrStopTrail, manualcheat=ManualStopOrStopTrailCheat, auto=AutoStopOrStopTrail)
