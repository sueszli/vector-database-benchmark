...
from prefect.server.orchestration.rules import BaseUniversalTransform, FlowOrchestrationContext, OrchestrationContext, TaskOrchestrationContext
...
def COMMON_GLOBAL_TRANSFORMS():
    if False:
        for i in range(10):
            print('nop')
    return [SetRunStateType, SetRunStateName, SetRunStateTimestamp, SetStartTime, SetEndTime, IncrementRunTime, SetExpectedStartTime, SetNextScheduledStartTime, UpdateStateDetails]
...
class GlobalFlowPolicy(BaseOrchestrationPolicy):
    ...
    def priority():
        if False:
            for i in range(10):
                print('nop')
        return COMMON_GLOBAL_TRANSFORMS() + [UpdateSubflowParentTask, UpdateSubflowStateDetails, IncrementFlowRunCount, RemoveResumingIndicator]
...
class GlobalTaskPolicy(BaseOrchestrationPolicy):
    ...
    def priority():
        if False:
            return 10
        return COMMON_GLOBAL_TRANSFORMS() + [IncrementTaskRunCount]
...
class SetEndTime(BaseUniversalTransform):
    ...
    async def before_transition(self, context: OrchestrationContext) -> None:
        if self.nullified_transition():
            return
        if context.initial_state and context.initial_state.is_final() and (not context.proposed_state.is_final()):
...
class IncrementRunTime(BaseUniversalTransform):
    ...
    async def before_transition(self, context: OrchestrationContext) -> None:
        ...
        if context.initial_state and context.initial_state.is_running():
            context.run.total_run_time += context.proposed_state.timestamp - context.initial_state.timestamp
    ...
    async def before_transition(self, context: OrchestrationContext) -> None:
        ...
        if not context.run.expected_start_time:
            if context.proposed_state.is_scheduled():
                context.run.expected_start_time = context.proposed_state.state_details.scheduled_time
            else:
...
class SetNextScheduledStartTime(BaseUniversalTransform):
    ...
    async def before_transition(self, context: OrchestrationContext) -> None:
        ...
        if context.proposed_state.is_scheduled():
            context.run.next_scheduled_start_time = context.proposed_state.state_details.scheduled_time
...
class UpdateSubflowParentTask(BaseUniversalTransform):
    ...
    async def after_transition(self, context: OrchestrationContext) -> None:
        ...
        if context.run.parent_task_run_id is not None:
            subflow_parent_task_state = context.validated_state.copy(reset_fields=True, include={'type', 'timestamp', 'name', 'message', 'state_details', 'data'})
            subflow_parent_task_state.state_details.child_flow_run_id = context.run.id
            await models.task_runs.set_task_run_state(session=context.session, task_run_id=context.run.parent_task_run_id, state=subflow_parent_task_state, force=True)
...
class UpdateSubflowStateDetails(BaseUniversalTransform):
    ...
    async def before_transition(self, context: OrchestrationContext) -> None:
        if self.nullified_transition():
            return
        if context.run.parent_task_run_id is not None:
            context.proposed_state.state_details.task_run_id = context.run.parent_task_run_id
