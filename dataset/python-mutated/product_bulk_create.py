...
from ...core.types import BaseInputObjectType, BaseObjectType, MediaInput, NonNullList, ProductBulkCreateError, SeoInput
...
from .product_variant_bulk_create import ProductVariantBulkCreate, ProductVariantBulkCreateInput
...
class ProductChannelListingCreateInput(BaseInputObjectType):
    ...
    is_published = graphene.Boolean(description='Determines if object is visible to customers.')
    published_at = graphene.types.datetime.DateTime(description='Publication date time. ISO 8601 standard.')
    visible_in_listings = graphene.Boolean(description="Determines if product is visible in product listings (doesn't apply to product collections).")
    is_available_for_purchase = graphene.Boolean(description='Determines if product should be available for purchase in this channel. This does not guarantee the availability of stock. When set to `False`, this product is still visible to customers, but it cannot be purchased.')
    available_for_purchase_at = graphene.DateTime(description='A start date time from which a product will be available for purchase. When not set and `isAvailable` is set to True, the current day is assumed.')
...
class ProductBulkResult(BaseObjectType):
    ...
    errors = NonNullList(ProductBulkCreateError, required=False, description='List of errors occurred on create attempt.')
...
class ProductBulkCreateInput(ProductCreateInput):
    ...
    collections = NonNullList(graphene.ID, description='List of IDs of collections that the product belongs to.', name='collections')
    ...
    tax_class = graphene.ID(description='ID of a tax class to assign to this product. If not provided, product will use the tax class which is assigned to the product type.', required=False)
    ...
    metadata = NonNullList(MetadataInput, description='Fields required to update the product metadata.', required=False)
    private_metadata = NonNullList(MetadataInput, description='Fields required to update the product private metadata.', required=False)
    external_reference = graphene.String(description='External ID of this product.', required=False)
    product_type = graphene.ID(description='ID of the type that product belongs to.', name='productType', required=True)
    media = NonNullList(MediaInput, description='List of media inputs associated with the product.', required=False)
    channel_listings = NonNullList(ProductChannelListingCreateInput, description='List of channels in which the product is available.', required=False)
    variants = NonNullList(ProductVariantBulkCreateInput, required=False, description='Input list of product variants to create.')

    class Meta:
        doc_category = DOC_CATEGORY_PRODUCTS

class ProductBulkCreate(BaseMutation):
    count = graphene.Int(required=True, description='Returns how many objects were created.')
    results = NonNullList(ProductBulkResult, required=True, default_value=[], description='List of the created products.')

    class Arguments:
        products = NonNullList(ProductBulkCreateInput, required=True, description='Input list of products to create.')
