...
from airflow.providers.amazon.aws.operators.sagemaker import SageMakerDeleteModelOperator, SageMakerEndpointConfigOperator, SageMakerEndpointOperator, SageMakerModelOperator, SageMakerTrainingOperator
...
with DAG(dag_id=DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), tags=['example'], catchup=False) as dag:
    ...
    test_setup = set_up(env_id=test_context[ENV_ID_KEY], role_arn=test_context[ROLE_ARN_KEY])
    create_bucket = S3CreateBucketOperator(task_id='create_bucket', bucket_name=test_setup['bucket_name'])
    upload_data = S3CreateObjectOperator(task_id='upload_data', s3_bucket=test_setup['bucket_name'], s3_key=f"{test_setup['input_data_s3_key']}/train.csv", data=TRAIN_DATA)
    ...
    configure_endpoint = SageMakerEndpointConfigOperator(task_id='configure_endpoint', config=test_setup['endpoint_config_config'])
    deploy_endpoint = SageMakerEndpointOperator(task_id='deploy_endpoint', config=test_setup['deploy_endpoint_config'])
    deploy_endpoint.wait_for_completion = False
    await_endpoint = SageMakerEndpointSensor(task_id='await_endpoint', endpoint_name=test_setup['endpoint_name'])
    delete_model = SageMakerDeleteModelOperator(task_id='delete_model', trigger_rule=TriggerRule.ALL_DONE, config={'ModelName': test_setup['model_name']})
    ...
    log_cleanup = prune_logs([('/aws/sagemaker/TrainingJobs', test_context[ENV_ID_KEY])])
    chain(test_context, test_setup, create_bucket, upload_data, train_model, create_model, configure_endpoint, deploy_endpoint, await_endpoint, call_endpoint(test_setup['endpoint_name']), delete_endpoint_config(test_setup['endpoint_config_job_name']), delete_endpoint(test_setup['endpoint_name']), delete_model, delete_bucket, log_cleanup, archive_logs(f"/aws/sagemaker/Endpoints/{test_setup['endpoint_name']}"))
