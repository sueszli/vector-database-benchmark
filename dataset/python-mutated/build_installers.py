...
scientific_packages = {'cython': '', 'matplotlib': '', 'numpy': '', 'openpyxl': '', 'pandas': '', 'scipy': '', 'sympy': ''}
p = ArgumentParser()
p.add_argument('--no-local', action='store_true', help='Do not use local conda packages')
p.add_argument('--debug', action='store_true', help='Do not delete build files')
p.add_argument('--arch', action='store_true', help='Print machine architecture tag and exit.')
p.add_argument('--ext', action='store_true', help='Print installer extension for this platform and exit.')
p.add_argument('--artifact-name', action='store_true', help='Print computed artifact name and exit.')
p.add_argument('--extra-specs', nargs='+', default=[], help='One or more extra conda specs to add to the installer')
p.add_argument('--licenses', action='store_true', help='Post-process licenses AFTER having built the installer. This must be run as a separate step.')
p.add_argument('--images', action='store_true', help='Generate background images from the logo (test only)')
p.add_argument('--cert-id', default=None, help='Apple Developer ID Application certificate common name.')
p.add_argument('--install-type', choices=INSTALL_CHOICES, default=INSTALL_CHOICES[0], help='Installer type.')
args = p.parse_args()
yaml = YAML()
yaml.indent(mapping=2, sequence=4, offset=2)
indent4 = partial(indent, prefix='    ')
SPYVER = get_version(SPYREPO, normalize=False).lstrip('v').split('+')[0]
specs = {'python': '=' + PY_VER, 'spyder': '=' + SPYVER}
