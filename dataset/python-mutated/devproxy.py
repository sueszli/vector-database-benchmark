...
class DevProxy:
    ...
    async def proxy_handler(self, req):
        ...
        query = req.query
        try:
            async with method(upstream_url, headers=headers, params=query, allow_redirects=False, data=req.content) as request:
                response = aiohttp.web.StreamResponse(status=request.status, headers=request.headers)
    ...
    async def index_handler(self, req):
        ...
        index = tpl.render(configjson=json.dumps(self.config), custom_templates={}, config=self.config)
