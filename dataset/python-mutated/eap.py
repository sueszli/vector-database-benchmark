...
"""
import struct
from scapy.fields import BitEnumField, BitField, ByteEnumField, ByteField, ConditionalField, FieldLenField, FieldListField, IntField, LenField, LongField, PacketField, PacketListField, PadField, ShortField, StrLenField, XByteField, XIntField, XStrField, XStrFixedLenField, XStrLenField
from scapy.packet import Packet, Padding, bind_bottom_up, bind_layers, bind_top_down
from scapy.layers.l2 import SourceMACField, Ether, CookedLinux, GRE, SNAP
from scapy.config import conf
from scapy.compat import orb, chb
eapol_versions = {1: '802.1X-2001', 2: '802.1X-2004', 3: '802.1X-2010'}
eapol_types = {0: 'EAP-Packet', 1: 'EAPOL-Start', 2: 'EAPOL-Logoff', 3: 'EAPOL-Key', 4: 'EAPOL-Encapsulated-ASF-Alert', 5: 'EAPOL-MKA', 6: 'EAPOL-Announcement (Generic)', 7: 'EAPOL-Announcement (Specific)', 8: 'EAPOL-Announcement-Req'}
...
class EAP(Packet):
    ...
    """
    name = 'EAP'
    fields_desc = [ByteEnumField('code', 4, eap_codes), ByteField('id', 0), ShortField('len', None), ConditionalField(ByteEnumField('type', 0, eap_types), lambda pkt: pkt.code not in [EAP.SUCCESS, EAP.FAILURE]), ConditionalField(FieldListField('desired_auth_types', [], ByteEnumField('auth_type', 0, eap_types), length_from=lambda pkt: pkt.len - 4), lambda pkt: pkt.code == EAP.RESPONSE and pkt.type == 3), ConditionalField(StrLenField('identity', '', length_from=lambda pkt: pkt.len - 5), lambda pkt: pkt.code == EAP.RESPONSE and hasattr(pkt, 'type') and (pkt.type == 1)), ConditionalField(StrLenField('message', '', length_from=lambda pkt: pkt.len - 5), lambda pkt: pkt.code == EAP.REQUEST and hasattr(pkt, 'type') and (pkt.type == 1))]
    REQUEST = 1
    RESPONSE = 2
    SUCCESS = 3
    FAILURE = 4
...
class EAP_TLS(EAP):
    ...
    name = 'EAP-TLS'
    match_subclass = True
    fields_desc = [ByteEnumField('code', 1, eap_codes), ByteField('id', 0), FieldLenField('len', None, fmt='H', length_of='tls_data', adjust=lambda p, x: x + 10 if p.L == 1 else x + 6), ByteEnumField('type', 13, eap_types), BitField('L', 0, 1), BitField('M', 0, 1), BitField('S', 0, 1), BitField('reserved', 0, 5), ConditionalField(IntField('tls_message_len', 0), lambda pkt: pkt.L == 1), XStrLenField('tls_data', '', length_from=lambda pkt: 0 if pkt.len is None else pkt.len - (6 + 4 * pkt.L))]
...
class EAP_TTLS(EAP):
    ...
    name = 'EAP-TTLS'
    match_subclass = True
    fields_desc = [ByteEnumField('code', 1, eap_codes), ByteField('id', 0), FieldLenField('len', None, fmt='H', length_of='data', adjust=lambda p, x: x + 10 if p.L == 1 else x + 6), ByteEnumField('type', 21, eap_types), BitField('L', 0, 1), BitField('M', 0, 1), BitField('S', 0, 1), BitField('reserved', 0, 2), BitField('version', 0, 3), ConditionalField(IntField('message_len', 0), lambda pkt: pkt.L == 1), XStrLenField('data', '', length_from=lambda pkt: 0 if pkt.len is None else pkt.len - (6 + 4 * pkt.L))]
...
class EAP_PEAP(EAP):
    ...
    name = 'PEAP'
    match_subclass = True
    fields_desc = [ByteEnumField('code', 1, eap_codes), ByteField('id', 0), FieldLenField('len', None, fmt='H', length_of='tls_data', adjust=lambda p, x: x + 10 if p.L == 1 else x + 6), ByteEnumField('type', 25, eap_types), BitField('L', 0, 1), BitField('M', 0, 1), BitField('S', 0, 1), BitField('reserved', 0, 3), BitField('version', 1, 2), ConditionalField(IntField('tls_message_len', 0), lambda pkt: pkt.L == 1), XStrLenField('tls_data', '', length_from=lambda pkt: 0 if pkt.len is None else pkt.len - (6 + 4 * pkt.L))]
...
class EAP_FAST(EAP):
    ...
    name = 'EAP-FAST'
    match_subclass = True
    fields_desc = [ByteEnumField('code', 1, eap_codes), ByteField('id', 0), FieldLenField('len', None, fmt='H', length_of='data', adjust=lambda p, x: x + 10 if p.L == 1 else x + 6), ByteEnumField('type', 43, eap_types), BitField('L', 0, 1), BitField('M', 0, 1), BitField('S', 0, 1), BitField('reserved', 0, 2), BitField('version', 0, 3), ConditionalField(IntField('message_len', 0), lambda pkt: pkt.L == 1), XStrLenField('data', '', length_from=lambda pkt: 0 if pkt.len is None else pkt.len - (6 + 4 * pkt.L))]
...
_param_set_cls = {1: 'MKALivePeerListParamSet', 2: 'MKAPotentialPeerListParamSet', 3: 'MKASAKUseParamSet', 4: 'MKADistributedSAKParamSet', 255: 'MKAICVSet'}
...
class MKABasicParamSet(Packet):
    ...
    fields_desc = [ByteField('mka_version_id', 0), ByteField('key_server_priority', 0), BitField('key_server', 0, 1), BitField('macsec_desired', 0, 1), BitField('macsec_capability', 0, 2), BitField('param_set_body_len', 0, 12), PacketField('SCI', MACsecSCI(), MACsecSCI), XStrFixedLenField('actor_member_id', '', length=12), XIntField('actor_message_number', 0), XIntField('algorithm_agility', 0), PadField(XStrLenField('cak_name', '', length_from=lambda pkt: pkt.param_set_body_len - 28), 4, padwith=b'\x00')]
...
class MKAPeerListTuple(Packet):
    ...
    fields_desc = [XStrFixedLenField('member_id', '', length=12), XStrFixedLenField('message_number', '', length=4)]
...
class MKADistributedCAKParamSet(MKAParamSet):
    ...
    fields_desc = [PadField(ByteEnumField('param_set_type', 5, _parameter_set_types), 2, padwith=b'\x00'), ShortField('param_set_body_len', 0), XStrFixedLenField('cak_aes_key_wrap', '', length=MKAParamSet.EAPOL_MKA_DEFAULT_KEY_WRAP_LEN), XStrField('cak_key_name', '')]
...
class MKAPDU(Packet):
    ...
    """
    name = 'MKPDU'
    fields_desc = [PacketField('basic_param_set', '', MKABasicParamSet), MKAParamSetPacketListField('parameter_sets', [], MKAParamSet)]
...
bind_bottom_up(Ether, EAPOL, type=34958)
bind_top_down(Ether, EAPOL, dst='01:80:c2:00:00:03', type=34958)
