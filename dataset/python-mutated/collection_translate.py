...
class CollectionTranslate(BaseTranslateMutation):
    ...
    class Arguments:
        id = graphene.ID(required=True, description='Collection ID or CollectionTranslatableContent ID.')
        language_code = graphene.Argument(LanguageCodeEnum, required=True, description='Translation language code.')
        input = TranslationInput(required=True, description='Fields required to update collection translations.')
