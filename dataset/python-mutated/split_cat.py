...
from ..pattern_matcher import Arg, CallFunction, CallFunctionVarArgs, CallMethodVarArgs, config_flag, FailedMatch, get_arg_value, Ignored, KeywordArg, ListOf, Match, MatchContext, MULTIPLE, PatternExpr, register_graph_pattern, RepeatedExpr
from .pre_grad import merge_getitem_cat_pass, merge_splits_pass, normalization_pass, split_cat_pass, unbind_stack_pass
...
getitem_unbind = ListOf(GetItem(CallFunction(torch.unbind, KeywordArg('unbind_input'), dim=KeywordArg('dim'), _users=MULTIPLE), Ignored(), _users=MULTIPLE), partial=True)
...
getitem_split = ListOf(CallFunction(operator.getitem, TorchSplit(Ignored(), KeywordArg('split_sections')), Ignored(), _users=MULTIPLE), partial=True)
