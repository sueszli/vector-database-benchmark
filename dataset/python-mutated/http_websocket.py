...
from typing import Any, Callable, Final, List, NamedTuple, Optional, Pattern, Set, Tuple, Union, cast
...
if NO_EXTENSIONS:
    ...
    try:
        ...
        _websocket_mask = _websocket_mask_python
_WS_DEFLATE_TRAILING: Final[bytes] = bytes([0, 0, 255, 255])
...
class WebSocketWriter:
    ...
    async def _send_frame(self, message: bytes, opcode: int, compress: Optional[int]=None) -> None:
        ...
        if (compress or self.compress) and opcode < 8:
            if compress:
                compressobj = ZLibCompressor(level=zlib.Z_BEST_SPEED, wbits=-compress, max_sync_chunk_size=WEBSOCKET_MAX_SYNC_CHUNK_SIZE)
            else:
                if not self._compressobj:
                    self._compressobj = ZLibCompressor(level=zlib.Z_BEST_SPEED, wbits=-self.compress, max_sync_chunk_size=WEBSOCKET_MAX_SYNC_CHUNK_SIZE)
