...
async def test__async_get_historic_ohlcv_binance(default_conf, mocker, caplog, candle_type):
    ...
    pair = 'ETH/BTC'
    (respair, restf, restype, res, _) = await exchange._async_get_historic_ohlcv(pair, '5m', 1500000000000, is_new_pair=False, candle_type=candle_type)
    assert respair == pair
    assert restf == '5m'
    assert restype == candle_type
    assert exchange._api_async.fetch_ohlcv.call_count > 400
    exchange._api_async.fetch_ohlcv.reset_mock()
    (_, _, _, res, _) = await exchange._async_get_historic_ohlcv(pair, '5m', 1500000000000, is_new_pair=True, candle_type=candle_type)
    assert exchange._api_async.fetch_ohlcv.call_count == 2
    assert res == ohlcv
    assert log_has_re('Candle-data for ETH/BTC available starting with .*', caplog)
