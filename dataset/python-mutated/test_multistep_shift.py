...
@testing.parameterize({'init': 2.0, 'gamma': 0.1, 'step_value': [1, 3, 5], 'expect': [2.0, 0.2, 0.2, 0.02, 0.02, 0.002]}, {'init': -2.0, 'gamma': 0.1, 'step_value': [1, 3, 5], 'expect': [-2.0, -0.2, -0.2, -0.02, -0.02, -0.002]}, {'init': 2.0, 'gamma': 2, 'step_value': [1, 3, 5], 'expect': [2.0, 4.0, 4.0, 8.0, 8.0, 16.0]}, {'init': -2.0, 'gamma': 2, 'step_value': [1, 3, 5], 'expect': [-2.0, -4.0, -4.0, -8.0, -8.0, -16.0]})
class TestMutistepShift(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.extension = extensions.MultistepShift('x', self.gamma, self.step_value, self.init, self.optimizer)
        self.interval = 1
    ...
    def test_basic(self):
        if False:
            print('Hello World!')
        self.optimizer.x = 0
        extension = extensions.MultistepShift('x', self.gamma, self.step_value, self.init, self.optimizer)
    ...
    def test_with_optimizer(self):
        ...
        optimizer = mock.Mock()
        optimizer.x = 0
        extension = extensions.MultistepShift('x', self.gamma, self.step_value, self.init, optimizer)
