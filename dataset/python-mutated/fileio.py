...
__all__ = ['EmptyMatchTreatment', 'MatchFiles', 'MatchAll', 'MatchContinuously', 'ReadableFile', 'ReadMatches', 'WriteToFiles']
...
_DEFAULT_FILE_NAME_TEMPLATE = '{prefix}-{start}-{end}-{pane}-{shard:05d}-of-{total_shards:05d}{suffix}{compression}'
...
def single_file_naming(prefix, suffix=None) -> FileNaming:
    ...
    return _inner
_FileResult = collections.namedtuple('FileResult', ['file_name', 'shard_index', 'total_shards', 'window', 'pane', 'destination'])
...
class _RemoveOldDuplicates(beam.DoFn):
  ...
  unchanged."""
    TIME_STATE = CombiningValueStateSpec('count', combine_fn=partial(max, default=0.0))
