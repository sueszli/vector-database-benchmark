...
flags.DEFINE_integer('sims_per_entry', 1000, 'Number of simulations to run to estimate each elementof the game outcome matrix.')
...
flags.DEFINE_string('training_strategy_selector', 'probabilistic', "Which strategy selector to use. Choices are  - 'top_k_probabilities': select top `number_policies_selected` strategies.  - 'probabilistic': Randomly samples `number_policies_selected` strategies with probability equal to their selection probabilities.  - 'uniform': Uniformly sample `number_policies_selected` strategies.  - 'rectified': Select every non-zero-selection-probability strategy available to each player.")
flags.DEFINE_string('oracle_type', 'BR', 'Choices are DQN, PG (Policy Gradient) or BR (exact Best Response)')
flags.DEFINE_integer('number_training_episodes', int(10000.0), 'Number training episodes per RL policy. Used for PG and DQN')
...
flags.DEFINE_float('pi_learning_rate', 0.001, 'Policy learning rate.')
flags.DEFINE_float('dqn_learning_rate', 0.01, 'DQN learning rate.')
