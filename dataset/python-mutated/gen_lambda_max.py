...
class gen_lambda_max(Atom):
    ...
    def numeric(self, values):
        ...
        return LA.eigh(a=values[0], b=values[1], eigvals_only=True, eigvals=(lo, hi))[0]
    ...
    def validate_arguments(self) -> None:
        ...
        if not self.args[0].ndim == 2 or self.args[0].shape[0] != self.args[0].shape[1] or self.args[1].shape[0] != self.args[1].shape[1] or (self.args[0].shape != self.args[1].shape):
            raise ValueError("The arguments '%s' and '%s' to gen_lambda_max must be square and have the same dimensions." % (self.args[0].name(), self.args[1].name()))
