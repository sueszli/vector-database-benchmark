...
class InceptionV2Test(tf.test.TestCase):
    ...
    def testBuildBaseNetwork(self):
        ...
        expected_endpoints = ['Mixed_3b', 'Mixed_3c', 'Mixed_4a', 'Mixed_4b', 'Mixed_4c', 'Mixed_4d', 'Mixed_4e', 'Mixed_5a', 'Mixed_5b', 'Mixed_5c', 'Conv2d_1a_7x7', 'MaxPool_2a_3x3', 'Conv2d_2b_1x1', 'Conv2d_2c_3x3', 'MaxPool_3a_3x3']
    ...
    def testBuildOnlyUptoFinalEndpoint(self):
        ...
        batch_size = 5
        (height, width) = (224, 224)
        endpoints = ['Conv2d_1a_7x7', 'MaxPool_2a_3x3', 'Conv2d_2b_1x1', 'Conv2d_2c_3x3', 'MaxPool_3a_3x3', 'Mixed_3b', 'Mixed_3c', 'Mixed_4a', 'Mixed_4b', 'Mixed_4c', 'Mixed_4d', 'Mixed_4e', 'Mixed_5a', 'Mixed_5b', 'Mixed_5c']
    ...
    def testBuildAndCheckAllEndPointsUptoMixed5c(self):
        ...
        endpoints_shapes = {'Mixed_3b': [batch_size, 28, 28, 256], 'Mixed_3c': [batch_size, 28, 28, 320], 'Mixed_4a': [batch_size, 14, 14, 576], 'Mixed_4b': [batch_size, 14, 14, 576], 'Mixed_4c': [batch_size, 14, 14, 576], 'Mixed_4d': [batch_size, 14, 14, 576], 'Mixed_4e': [batch_size, 14, 14, 576], 'Mixed_5a': [batch_size, 7, 7, 1024], 'Mixed_5b': [batch_size, 7, 7, 1024], 'Mixed_5c': [batch_size, 7, 7, 1024], 'Conv2d_1a_7x7': [batch_size, 112, 112, 64], 'MaxPool_2a_3x3': [batch_size, 56, 56, 64], 'Conv2d_2b_1x1': [batch_size, 56, 56, 64], 'Conv2d_2c_3x3': [batch_size, 56, 56, 192], 'MaxPool_3a_3x3': [batch_size, 28, 28, 192]}
    ...
    def testBuildEndPointsWithDepthMultiplierLessThanOne(self):
        ...
        (_, end_points_with_multiplier) = inception.inception_v2(inputs, num_classes, scope='depth_multiplied_net', depth_multiplier=0.5)
    ...
    def testBuildEndPointsWithDepthMultiplierGreaterThanOne(self):
        ...
        (_, end_points) = inception.inception_v2(inputs, num_classes)
        endpoint_keys = [key for key in end_points.keys() if key.startswith('Mixed') or key.startswith('Conv')]
        (_, end_points_with_multiplier) = inception.inception_v2(inputs, num_classes, scope='depth_multiplied_net', depth_multiplier=2.0)
        for key in endpoint_keys:
            original_depth = end_points[key].get_shape().as_list()[3]
            new_depth = end_points_with_multiplier[key].get_shape().as_list()[3]
    ...
    def testBuildEndPointsWithUseSeparableConvolutionFalse(self):
        ...
        endpoint_keys = [key for key in end_points.keys() if key.startswith('Mixed') or key.startswith('Conv')]
        (_, end_points_with_replacement) = inception.inception_v2_base(inputs, use_separable_conv=False)
        for key in endpoint_keys:
    ...
    def testBuildEndPointsNCHWDataFormat(self):
        ...
        (_, end_points_with_replacement) = inception.inception_v2_base(inputs_in_nchw, use_separable_conv=False, data_format='NCHW')
        for key in endpoint_keys:
            transposed_original_shape = tf.transpose(end_points[key], [0, 3, 1, 2]).get_shape().as_list()
            self.assertTrue(key in end_points_with_replacement)
            new_shape = end_points_with_replacement[key].get_shape().as_list()
    ...
    def testBuildBaseNetworkWithoutRootBlock(self):
        ...
        (_, end_points) = inception.inception_v2_base(inputs, include_root_block=False)
        endpoints_shapes = {'Mixed_3b': [batch_size, 28, 28, 256], 'Mixed_3c': [batch_size, 28, 28, 320], 'Mixed_4a': [batch_size, 14, 14, 576], 'Mixed_4b': [batch_size, 14, 14, 576], 'Mixed_4c': [batch_size, 14, 14, 576], 'Mixed_4d': [batch_size, 14, 14, 576], 'Mixed_4e': [batch_size, 14, 14, 576], 'Mixed_5a': [batch_size, 7, 7, 1024], 'Mixed_5b': [batch_size, 7, 7, 1024], 'Mixed_5c': [batch_size, 7, 7, 1024]}
        self.assertItemsEqual(endpoints_shapes.keys(), end_points.keys())
        for endpoint_name in endpoints_shapes:
            expected_shape = endpoints_shapes[endpoint_name]
            self.assertTrue(endpoint_name in end_points)
    ...
    def testGlobalPoolUnknownImageShape(self):
        ...
        with self.test_session() as sess:
            inputs = tf.placeholder(tf.float32, shape=(batch_size, None, None, 3))
            (logits, end_points) = inception.inception_v2(inputs, num_classes, global_pool=True)
            self.assertTrue(logits.op.name.startswith('InceptionV2/Logits'))
            self.assertListEqual(logits.get_shape().as_list(), [batch_size, num_classes])
            pre_pool = end_points['Mixed_5c']
            feed_dict = {inputs: input_np}
