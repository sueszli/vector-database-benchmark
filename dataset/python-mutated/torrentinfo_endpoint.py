...
from tribler.core.components.restapi.rest.rest_endpoint import HTTP_BAD_REQUEST, HTTP_INTERNAL_SERVER_ERROR, RESTEndpoint, RESTResponse
from tribler.core.utilities.rest_utils import FILE_SCHEME, HTTPS_SCHEME, HTTP_SCHEME, MAGNET_SCHEME, scheme_from_url, url_to_path
...
class TorrentInfoEndpoint(RESTEndpoint):
    ...
    @docs(tags=['Libtorrent'], summary='Return metainfo from a torrent found at a provided URI.', parameters=[{'in': 'query', 'name': 'torrent', 'description': 'URI for which to return torrent information. This URI can either represent a file location, a magnet link or a HTTP(S) url.', 'type': 'string', 'required': True}], responses={200: {'description': 'Return a hex-encoded json-encoded string with torrent metainfo', 'schema': schema(GetMetainfoResponse={'metainfo': String})}})
    async def get_torrent_info(self, request):
        ...
        self.download_manager.notifier[notifications.torrent_metadata_added](tdef_to_metadata_dict(TorrentDef.load_from_dict(metainfo)))
        ...
        return RESTResponse({'metainfo': encoded_metainfo, 'download_exists': download and (not download_is_metainfo_request)})