...
from transformers import AutoModelForSequenceClassification, AutoTokenizer, HfArgumentParser, Trainer, TrainingArguments, set_seed, utils
...
class TeacherModelArguments:
    ...
    hypothesis_template: Optional[str] = field(default='This example is {}.', metadata={'help': 'Template used to turn class names into mock hypotheses for teacher NLI model. Must include {{}} where class name is inserted.'})
    teacher_batch_size: Optional[int] = field(default=32, metadata={'help': 'Batch size for generating teacher predictions.'})
    multi_label: Optional[bool] = field(default=False, metadata={'help': 'Allow multiple classes to be true rather than forcing them to sum to 1 (sometimes called multi-class multi-label classification).'})
    temperature: Optional[float] = field(default=1.0, metadata={'help': 'Temperature applied to teacher softmax for distillation.'})
...
class DistillTrainingArguments(TrainingArguments):
    ...
    per_device_eval_batch_size: int = field(default=128, metadata={'help': 'Batch size per GPU/TPU core/CPU for evaluation.'})
    ...
    do_eval: bool = field(default=True, metadata={'help': 'Whether to evaluate the agreement of the final student predictions and the teacher predictions after training.'})
    save_total_limit: Optional[int] = field(default=0, metadata={'help': 'Limit the total amount of checkpoints. Deletes the older checkpoints in the output_dir. Default is 0 (no checkpoints).'})
