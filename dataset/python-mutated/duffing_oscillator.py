...
ps = []
for i in range(2):
    p = figure(width=600, height=600, x_range=Range1d(-2.0, 2.0), y_range=Range1d(-2.0, 2.0), toolbar_location=None, background_fill_color='#f6f6f6')
    p.xaxis.axis_label = '$$x \\text{ (m)}$$'
    p.yaxis.axis_label = '$$\\frac{dx}{dt} \\text{ (m/s)}$$'
    ps.append(p)
...
delta_slider = Slider(start=0, end=0.6, value=delta, step=0.05, title='$$\\delta \\text{ (damping factor, 1/s)}$$')
gamma_slider = Slider(start=0, end=0.6, value=gamma, step=0.05, title='$$\\gamma \\text{ (amplitude of forcing, m/s}^2)$$')
omega_slider = Slider(start=0.5, end=3.0, value=omega, step=0.05, title='$$\\omega \\text{ (frequency of forcing, 1/s)}$$')
for slider in [delta_slider, gamma_slider, omega_slider]:
    slider.on_change('value', slider_callback)
big = {'font-size': '150%'}
col = [Div(text='Duffing oscillator', styles=big), Div(text='$$\\frac{d^2x}{dt^2} + \\delta \\frac{dx}{dt} - \\frac{dV}{dx} = \\gamma \\cos \\omega t,$$', styles=big), Div(text='where $$\\frac{dV}{dx} = -x^3 + x$$', styles=big), Paragraph(text='Equation is integrated forward in time using implicit finite difference method.'), Paragraph(text='$$x_i$$ is the value of $$x$$ at time $$t_i = i \\Delta t$$ for integer $$i$$ and fixed timestep $$\\Delta t$$.'), Paragraph(text='$$x_{i+1}$$ is determined from $$x_{i-1}$$ and $$x_i$$ using'), Paragraph(text='$$x_{i+1} = \\frac{2\\beta x_i + (\\alpha-\\beta) x_{i-1} - \\frac{dV}{dx}(x_i) + \\gamma \\cos \\omega t_i}{\\alpha+\\beta}$$'), Paragraph(text='where $$\\alpha = \\frac{\\delta}{2\\Delta t}$$ and $$\\beta = \\frac{1}{\\Delta t^2}$$'), delta_slider, gamma_slider, omega_slider]
doc = curdoc()
