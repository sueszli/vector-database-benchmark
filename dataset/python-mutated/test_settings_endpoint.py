...
async def test_set_settings(rest_api, tribler_config):
    ...
    """
    post_data = {'download_defaults': {'seeding_mode': 'ratio', 'seeding_ratio': 3, 'seeding_time': 123}}
    await do_request(rest_api, 'settings', expected_code=200, request_type='POST', post_data=post_data)
...
async def test_set_rate_settings(rest_api, tribler_config):
    ...
    """
    extra_rate = 1024 * 1024 * 1024
    post_data = {'libtorrent': {'max_download_rate': MAX_LIBTORRENT_RATE_LIMIT + extra_rate, 'max_upload_rate': MAX_LIBTORRENT_RATE_LIMIT + extra_rate}}
    await do_request(rest_api, 'settings', expected_code=200, request_type='POST', post_data=post_data)
