...
class TriblerContentDelegate(TriblerButtonsDelegate, CategoryLabelMixin, RatingControlMixin, DownloadControlsMixin, HealthLabelMixin, ChannelStateMixin, SubscribedControlMixin, TagsMixin):
...
class HealthStatusDisplay(QObject):
    indicator_side = 10
    indicator_border = 6
    health_colors = {HEALTH_GOOD: QColor(Qt.green), HEALTH_DEAD: QColor(Qt.red), HEALTH_MOOT: QColor(Qt.yellow), HEALTH_UNCHECKED: QColor('#B5B5B5'), HEALTH_CHECKING: QColor(Qt.yellow), HEALTH_ERROR: QColor(Qt.red)}
...
class RatingControl(QObject, CheckClickedMixin):
    ...
    """
    rating_colors = {'BACKGROUND': QColor('#444444'), 'FOREGROUND': QColor('#BBBBBB')}
    clicked = pyqtSignal(QModelIndex)
