...
class TestROD(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (self.X_train, self.X_test, self.y_train, self.y_test) = generate_data(n_train=self.n_train, n_test=self.n_test, n_features=4, contamination=self.contamination, random_state=42)
        self.clf = ROD()
    ...
    def test_invocation(self):
        ...
        assert_array_equal(scores, rod_3D(np.hstack((X_2D, np.zeros(shape=(X_2D.shape[0], 3 - X_2D.shape[1])))))[0])
        scores = ROD().fit(X_3D).decision_scores_
        assert_array_equal(scores, rod_3D(X_3D)[0])
        scores = ROD().fit(X_4D).decision_scores_
        assert_array_equal(scores, rod_nD(X_4D, False, self.gm, self.data_scaler, self.angles_scalers1, self.angles_scalers2)[0])
    ...
    def test_process_sub(self):
        ...
        assert_equal([0.5, 0.5, 0.5], process_sub(subspace, self.gm, self.median, self.angles_scalers1, self.angles_scalers2)[0])

    def test_parallel_vs_non_parallel(self):
        if False:
            return 10
        assert_equal(rod_nD(self.X_train, False, self.gm, self.data_scaler, self.angles_scalers1, self.angles_scalers2)[0], rod_nD(self.X_train, True, self.gm, self.data_scaler, self.angles_scalers1, self.angles_scalers2)[0])
