...
class _FPGrowthParams(HasPredictionCol):
    ...
    .. versionadded:: 3.0.0
    """
    itemsCol: Param[str] = Param(Params._dummy(), 'itemsCol', 'items column name', typeConverter=TypeConverters.toString)
    minSupport: Param[float] = Param(Params._dummy(), 'minSupport', 'Minimal support level of the frequent pattern. [0.0, 1.0]. ' + 'Any pattern that appears more than (minSupport * size-of-the-dataset) ' + 'times will be output in the frequent itemsets.', typeConverter=TypeConverters.toFloat)
    numPartitions: Param[int] = Param(Params._dummy(), 'numPartitions', 'Number of partitions (at least 1) used by parallel FP-growth. ' + 'By default the param is not set, ' + 'and partition number of the input dataset is used.', typeConverter=TypeConverters.toInt)
    minConfidence: Param[float] = Param(Params._dummy(), 'minConfidence', 'Minimal confidence for generating Association Rule. [0.0, 1.0]. ' + 'minConfidence will not affect the mining for frequent itemsets, ' + 'but will affect the association rules generation.', typeConverter=TypeConverters.toFloat)
...
class PrefixSpan(JavaParams):
    ...
    """
    _input_kwargs: Dict[str, Any]
    minSupport: Param[float] = Param(Params._dummy(), 'minSupport', 'The minimal support level of the ' + 'sequential pattern. Sequential pattern that appears more than ' + '(minSupport * size-of-the-dataset) times will be output. Must be >= 0.', typeConverter=TypeConverters.toFloat)
    maxPatternLength: Param[int] = Param(Params._dummy(), 'maxPatternLength', 'The maximal length of the sequential pattern. Must be > 0.', typeConverter=TypeConverters.toInt)
    maxLocalProjDBSize: Param[int] = Param(Params._dummy(), 'maxLocalProjDBSize', 'The maximum number of items (including delimiters used in the ' + 'internal storage format) allowed in a projected database before ' + 'local processing. If a projected database exceeds this size, ' + 'another iteration of distributed prefix growth is run. ' + 'Must be > 0.', typeConverter=TypeConverters.toInt)
    sequenceCol: Param[str] = Param(Params._dummy(), 'sequenceCol', 'The name of the sequence column in ' + 'dataset, rows with nulls in this column are ignored.', typeConverter=TypeConverters.toString)
