...
class AsyncAnonymousAccessCredential(AsyncTokenCredential):
    ...
    async def get_token(self, *scopes: str, claims: Optional[str]=None, tenant_id: Optional[str]=None, **kwargs) -> AccessToken:
...
class AnonymousACRExchangeClient(object):
    ...
    def __init__(self, endpoint: str, **kwargs: Any) -> None:
        ...
        self._client = ContainerRegistry(credential=AsyncAnonymousAccessCredential(), url=endpoint, sdk_moniker=USER_AGENT, authentication_policy=ExchangeClientAuthenticationPolicy(), **kwargs)

    async def get_acr_access_token(self, challenge: str, **kwargs) -> Optional[str]:
        parsed_challenge = _parse_challenge(challenge)
        return await self.exchange_refresh_token_for_access_token('', service=parsed_challenge['service'], scope=parsed_challenge['scope'], grant_type=TokenGrantType.PASSWORD, **kwargs)

    async def exchange_refresh_token_for_access_token(self, refresh_token: str, service: str, scope: str, grant_type: Union[str, TokenGrantType], **kwargs) -> Optional[str]:
        auth_operation = cast(AuthenticationOperations, self._client.authentication)
        access_token = await auth_operation.exchange_acr_refresh_token_for_acr_access_token(service=service, scope=scope, refresh_token=refresh_token, grant_type=grant_type, **kwargs)
