import logging
from esphome.const import CONF_DISABLED_BY_DEFAULT, CONF_ENTITY_CATEGORY, CONF_ICON, CONF_INTERNAL, CONF_NAME, CONF_SETUP_PRIORITY, CONF_UPDATE_INTERVAL, CONF_TYPE_ID, CONF_OTA, CONF_SAFE_MODE, KEY_PAST_SAFE_MODE
...
async def register_component(var, config):
    ...
    if id_ not in CORE.component_ids:
        raise ValueError(f'Component ID {id_} was not declared to inherit from Component, or was registered twice. Please create a bug report with your configuration.')
    ...
    try:
        ...
        _LOGGER.warning('Error while finding name of component, please report this', exc_info=e)
    if name is not None:
...
async def past_safe_mode():
    safe_mode_enabled = CONF_OTA in CORE.config and CORE.config[CONF_OTA][CONF_SAFE_MODE]
