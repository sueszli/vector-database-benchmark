...
if __name__ == '__main__':
    ...
    for k in sorted(ordered.keys()):
        ...
        print('* PR %s: %s (%s)' % (hyperlink, ordered[k], ' '.join([hyperlink_user(u) for u in pr_authors])))
        for a in pr_authors:
            authors.add(a)
    if summary:
        print('\nTotal PRs: %s\n' % len(ordered))
        ...
        print()
    print('Authors:\n')
    [print('* %s' % hyperlink_user(x)) for x in sorted(authors, key=lambda x: x.login.lower())]
    if summary:
