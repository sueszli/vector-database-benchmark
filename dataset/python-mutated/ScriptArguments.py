...
from ..common import CONFIG_CACHE_FORCE_DEFAULT_ARGUMENTS, debug
from . import Util
from . import Config
from . import Registry
from . import WinSDK
from .Exceptions import MSVCInternalError, MSVCSDKVersionNotFound, MSVCToolsetVersionNotFound, MSVCSpectreLibsNotFound, MSVCArgumentError
from . import Dispatcher
Dispatcher.register_modulename(__name__)
_ARGUMENT_BOOLEAN_TRUE_LEGACY = (True, '1')
_ARGUMENT_BOOLEAN_TRUE = (True,)
re_sdk_version_100 = re.compile('^10[.][0-9][.][0-9]{5}[.][0-9]{1}$')
re_sdk_version_81 = re.compile('^8[.]1$')
re_sdk_dispatch_map = {'10.0': re_sdk_version_100, '8.1': re_sdk_version_81}
...
for (msvc_version, sxs_version, sxs_bugfix) in [('14.2', '14.28.16.8', '14.28')]:
...
re_vcvars_uwp = re.compile('(?:(?<!\\S)|^)(?P<uwp>(?:uwp|store))(?:(?!\\S)|$)', re.IGNORECASE)
re_vcvars_sdk = re.compile('(?:(?<!\\S)|^)(?P<sdk>(?:[1-9][0-9]*[.]\\S*))(?:(?!\\S)|$)', re.IGNORECASE)
re_vcvars_toolset = re.compile('(?:(?<!\\S)|^)(?P<toolset_arg>(?:[-]{1,2}|[/])vcvars_ver[=](?P<toolset>\\S*))(?:(?!\\S)|$)', re.IGNORECASE)
re_vcvars_spectre = re.compile('(?:(?<!\\S)|^)(?P<spectre_arg>(?:[-]{1,2}|[/])vcvars_spectre_libs[=](?P<spectre>\\S*))(?:(?!\\S)|$)', re.IGNORECASE)
...
MSVC_VERSION_ARGS_DEFINITION = namedtuple('MSVCVersionArgsDefinition', ['version', 'vs_def'])
