...
class rpc_if_id_vector_t(NDRSTRUCT):
    structure = (('count', ULONG), ('if_id', rpc_if_id_p_t_array))
    structure64 = (('count', ULONGLONG), ('if_id', rpc_if_id_p_t_array))
...
class inq_if_idsResponse(NDRCALL):
    structure = (('if_id_vector', rpc_if_id_vector_p_t), ('status', error_status))
...
class inq_statsResponse(NDRCALL):
    structure = (('count', ULONG), ('statistics', DWORD_ARRAY), ('status', error_status))
...
class inq_princ_name(NDRCALL):
    opnum = 4
    structure = (('authn_proto', ULONG), ('princ_name_size', ULONG))

class inq_princ_nameResponse(NDRCALL):
    structure = (('princ_name', NDRUniConformantVaryingArray), ('status', error_status))
OPNUMS = {0: (inq_if_ids, inq_if_idsResponse), 1: (inq_stats, inq_statsResponse), 2: (is_server_listening, is_server_listeningResponse), 3: (stop_server_listening, stop_server_listeningResponse), 4: (inq_princ_name, inq_princ_nameResponse)}
