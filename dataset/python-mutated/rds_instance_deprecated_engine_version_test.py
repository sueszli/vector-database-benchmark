...
class Test_rds_instance_deprecated_engine_version:
    ...
    def test_rds_no_instances(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.rds.rds_instance_deprecated_engine_version.rds_instance_deprecated_engine_version.rds_client', new=RDS(audit_info)):
                from prowler.providers.aws.services.rds.rds_instance_deprecated_engine_version.rds_instance_deprecated_engine_version import rds_instance_deprecated_engine_version
    ...
    def test_rds_instance_no_deprecated_engine_version(self):
        ...
        conn.create_db_instance(DBInstanceIdentifier='db-master-1', AllocatedStorage=10, Engine='mysql', EngineVersion='8.0.32', DBName='staging-mysql', DBInstanceClass='db.m1.small')
        from prowler.providers.aws.services.rds.rds_service import RDS
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.rds.rds_instance_deprecated_engine_version.rds_instance_deprecated_engine_version.rds_client', new=RDS(audit_info)):
                ...
                assert result[0].status_extended == 'RDS instance db-master-1 is not using a deprecated engine mysql with version 8.0.32.'
    ...
    def test_rds_instance_deprecated_engine_version(self):
        if False:
            ...
            return i + 15
        conn = client('rds', region_name=AWS_REGION)
        conn.create_db_instance(DBInstanceIdentifier='db-master-2', AllocatedStorage=10, Engine='mysql', EngineVersion='8.0.23', DBName='staging-mysql', DBInstanceClass='db.m1.small')
        from prowler.providers.aws.services.rds.rds_service import RDS
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.rds.rds_instance_deprecated_engine_version.rds_instance_deprecated_engine_version.rds_client', new=RDS(audit_info)):
                ...
                assert result[0].status_extended == 'RDS instance db-master-2 is using a deprecated engine mysql with version 8.0.23.'
                assert result[0].resource_id == 'db-master-2'
                assert result[0].region == AWS_REGION
                assert result[0].resource_arn == f'arn:aws:rds:{AWS_REGION}:{AWS_ACCOUNT_NUMBER}:db:db-master-2'
                assert result[0].resource_tags == []