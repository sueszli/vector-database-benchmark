...
class PlaylistWrapper:
    ...
    async def fetch(self, scope: str, playlist_id: int, scope_id: int) -> Optional[PlaylistFetchResult]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            for future in concurrent.futures.as_completed([executor.submit(self.database.cursor().execute, self.statement.get_one, {'playlist_id': playlist_id, 'scope_id': scope_id, 'scope_type': scope_type})]):
                try:
    ...
    async def fetch_all(self, scope: str, scope_id: int, author_id=None) -> List[PlaylistFetchResult]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            if author_id is not None:
                for future in concurrent.futures.as_completed([executor.submit(self.database.cursor().execute, self.statement.get_all_with_filter, {'scope_type': scope_type, 'scope_id': scope_id, 'author_id': author_id})]):
                    try:
                        row_result = future.result()
                    except Exception as exc:
                        log.verbose('Failed to complete playlist fetch from database', exc_info=exc)
                ...
                for future in concurrent.futures.as_completed([executor.submit(self.database.cursor().execute, self.statement.get_all, {'scope_type': scope_type, 'scope_id': scope_id})]):
                    try:
                        row_result = future.result()
                    except Exception as exc:
                        log.verbose('Failed to complete playlist fetch from database', exc_info=exc)
    ...
    async def fetch_all_converter(self, scope: str, playlist_name, playlist_id) -> List[PlaylistFetchResult]:
        ...
        output = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            for future in concurrent.futures.as_completed([executor.submit(self.database.cursor().execute, self.statement.get_all_converter, {'scope_type': scope_type, 'playlist_name': playlist_name, 'playlist_id': playlist_id})]):
                try:
                    row_result = future.result()
                except Exception as exc:
    ...
    async def delete(self, scope: str, playlist_id: int, scope_id: int):
        ...
        scope_type = self.get_scope_type(scope)
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            executor.submit(self.database.cursor().execute, self.statement.delete, {'playlist_id': playlist_id, 'scope_id': scope_id, 'scope_type': scope_type})
    ...
    async def drop(self, scope: str):
        ...
        scope_type = self.get_scope_type(scope)
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            executor.submit(self.database.cursor().execute, self.statement.delete_scope, {'scope_type': scope_type})
    ...
    async def upsert(self, scope: str, playlist_id: int, playlist_name: str, scope_id: int, author_id: int, playlist_url: Optional[str], tracks: List[MutableMapping]):
        """Insert or update a playlist into the database."""
        scope_type = self.get_scope_type(scope)
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            executor.submit(self.database.cursor().execute, self.statement.upsert, {'scope_type': str(scope_type), 'playlist_id': int(playlist_id), 'playlist_name': str(playlist_name), 'scope_id': int(scope_id), 'author_id': int(author_id), 'playlist_url': playlist_url, 'tracks': json.dumps(tracks)})

    async def handle_playlist_user_id_deletion(self, user_id: int):
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            executor.submit(self.database.cursor().execute, self.statement.drop_user_playlists, {'user_id': user_id})