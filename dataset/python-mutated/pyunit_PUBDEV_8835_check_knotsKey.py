...
def test_gam_knots_key():
    ...
    numKnots = [5, 5, 5]
    h2o_model = H2OGeneralizedAdditiveEstimator(family='multinomial', gam_columns=['C6', 'C7', 'C8'], scale=[1, 1, 1], num_knots=numKnots, bs=[3, 3, 3], knot_ids=[frameKnots1.key, frameKnots2.key, frameKnots3.key], seed=1234)
    h2o_model.train(x=x, y=y, training_frame=h2o_data)
    h2oCoeffs = h2o_model.coef()
    h2o_model2 = H2OGeneralizedAdditiveEstimator(family='multinomial', gam_columns=['C6', 'C7', 'C8'], scale=[1, 1, 1], bs=[3, 3, 3], num_knots=numKnots, seed=1234)
    ...
    for kNames in keyNames:
        ...
        assert diff < 0.01, 'expected coefficients: {0}.  actual coefficients: {1}'.format(h2oCoeffs['coefficients'][kNames], h2oCoeffs2['coefficients'][kNames])
