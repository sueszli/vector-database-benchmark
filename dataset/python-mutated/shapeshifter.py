...
class ShapeShifter(EvasionAttack):
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray]=None, **kwargs) -> np.ndarray:
        ...
        if not self.graph_available:
            ...
            if self.texture_as_input:
                ...
                (self.project_texture_op, self.current_image_assign_to_input_image_op, self.accumulated_gradients_op, self.final_attack_optimization_op, self.current_variable, self.current_value) = self._build_graph(initial_shape=x.shape, custom_loss=custom_loss, rendering_function=rendering_function)
            else:
                (self.project_texture_op, self.current_image_assign_to_input_image_op, self.accumulated_gradients_op, self.final_attack_optimization_op, self.current_variable, self.current_value) = self._build_graph(initial_shape=x.shape, custom_loss=custom_loss)
        if self.texture_as_input:
        ...
        result = self._attack_training(x=x, y=label, mask=mask, target_class=target_class, victim_class=victim_class, project_texture_op=self.project_texture_op, current_image_assign_to_input_image_op=self.current_image_assign_to_input_image_op, accumulated_gradients_op=self.accumulated_gradients_op, final_attack_optimization_op=self.final_attack_optimization_op, current_variable=self.current_variable, current_value=self.current_value)
        return result[1]

    def _attack_training(self, x: np.ndarray, y: Dict[str, List[np.ndarray]], mask: Optional[np.ndarray], target_class: int, victim_class: int, project_texture_op: 'Tensor', current_image_assign_to_input_image_op: 'Tensor', accumulated_gradients_op: 'Tensor', final_attack_optimization_op: 'Tensor', current_variable: 'Tensor', current_value: 'Tensor') -> List[np.ndarray]:
        if False:
            print('Hello World!')
    ...
    def _build_graph(self, initial_shape: Tuple[int, ...], custom_loss: Optional['Tensor']=None, rendering_function: Optional['Callable']=None) -> Tuple['Tensor', ...]:
        if False:
            print('Hello World!')
        ...
        if self.texture_as_input:
            ...
            if self.use_spectral:
                initial_value = np.zeros((2, initial_shape[0], initial_shape[3], initial_shape[1], int(np.ceil(initial_shape[2] / 2) + 1)))
                current_texture_variable = tf.Variable(initial_value=initial_value, dtype=tf.float32, name='current_texture_variable')
            ...
            background_phd = tf.placeholder(dtype=tf.float32, shape=initial_input.shape.as_list(), name='background_phd')
            image_frame_phd = tf.placeholder(dtype=tf.float32, shape=[initial_shape[0], None, None, 4], name='image_frame_phd')
            ...
            current_image_variable = tf.Variable(initial_value=np.zeros(initial_input.shape.as_list()), dtype=tf.float32, name='current_image_variable')
            random_transformation_phd = tf.placeholder(dtype=tf.float32, shape=initial_input.shape.as_list(), name='random_transformation_phd')
        ...
        current_image_assign_to_input_image_op = tf.assign(ref=self.estimator.input_images, value=current_image, name='current_image_assign_to_input_image_op')
        if self.texture_as_input:
            total_loss = self._create_attack_loss(initial_input=initial_input, current_value=current_texture, custom_loss=custom_loss)
        else:
            total_loss = self._create_attack_loss(initial_input=initial_input, current_value=current_image, custom_loss=custom_loss)
        ...
        if self.texture_as_input:
            sum_gradients = tf.Variable(initial_value=np.zeros(current_texture_variable.shape.as_list()), trainable=False, name='sum_gradients', dtype=tf.float32, collections=[tf.GraphKeys.GLOBAL_VARIABLES, tf.GraphKeys.LOCAL_VARIABLES])
        else:
            sum_gradients = tf.Variable(initial_value=np.zeros(current_image_variable.shape.as_list()), trainable=False, name='sum_gradients', dtype=tf.float32, collections=[tf.GraphKeys.GLOBAL_VARIABLES, tf.GraphKeys.LOCAL_VARIABLES])
        num_gradients = tf.Variable(initial_value=0.0, trainable=False, name='count_gradients', collections=[tf.GraphKeys.GLOBAL_VARIABLES, tf.GraphKeys.LOCAL_VARIABLES])
        accumulated_sum_gradients = tf.assign_add(sum_gradients, gradients)
        accumulated_num_gradients = tf.assign_add(num_gradients, 1.0)
        final_gradients = tf.div(accumulated_sum_gradients, tf.maximum(accumulated_num_gradients, 1.0), name='final_gradients')
        if self.sign_gradients:
            final_gradients = tf.sign(final_gradients)
        accumulated_gradients_op = tf.group([accumulated_sum_gradients, accumulated_num_gradients], name='accumulated_gradients_op')
        if self.texture_as_input:
            final_attack_optimization_op = optimizer.apply_gradients(grads_and_vars=[(final_gradients, current_texture_variable)], name='final_attack_optimization_op')
            return (project_texture_op, current_image_assign_to_input_image_op, accumulated_gradients_op, final_attack_optimization_op, current_texture_variable, current_texture)
        final_attack_optimization_op = optimizer.apply_gradients(grads_and_vars=[(final_gradients, current_image_variable)], name='final_attack_optimization_op')
        return (project_texture_op, current_image_assign_to_input_image_op, accumulated_gradients_op, final_attack_optimization_op, current_image_variable, current_image)
    ...
    def _create_attack_loss(self, initial_input: 'Tensor', current_value: 'Tensor', custom_loss: Optional['Tensor']=None) -> 'Tensor':
        if False:
            return 10
        ...
        if custom_loss is not None:
            total_loss = tf.add_n([partial_faster_rcnn_loss, partial_box_loss, partial_rpn_loss, weight_similarity_loss, custom_loss], name='total_loss')
        else:
            total_loss = tf.add_n([partial_faster_rcnn_loss, partial_box_loss, partial_rpn_loss, weight_similarity_loss], name='total_loss')
        return total_loss
    ...
    def _create_faster_rcnn_loss(self) -> 'Tensor':
        ...
        weight_rpn_classifier_loss = tf.multiply(x=rpn_classifier_loss, y=rpn_classifier_weight, name='weight_rpn_classifier_loss')
        ...
        weight_rpn_localizer_loss = tf.multiply(x=rpn_localizer_loss, y=rpn_localizer_weight, name='weight_rpn_localizer_loss')
        ...
        weight_box_classifier_loss = tf.multiply(x=box_classifier_loss, y=box_classifier_weight, name='weight_box_classifier_loss')
        ...
        weight_box_localizer_loss = tf.multiply(x=box_localizer_loss, y=box_localizer_weight, name='weight_box_localizer_loss')
        partial_loss = tf.add_n([weight_rpn_classifier_loss, weight_rpn_localizer_loss, weight_box_classifier_loss, weight_box_localizer_loss], name='partial_faster_rcnn_loss')
        return partial_loss
    ...
    def _create_box_loss(self) -> 'Tensor':
        ...
        box_nontarget_logit = tf.reduce_max(class_predictions_with_background * (1 - target_class_one_hot) - 10000 * target_class_one_hot, axis=-1)
        ...
        weight_box_target_cw_loss = tf.multiply(x=box_target_cw_loss, y=box_target_cw_weight, name='weight_box_target_cw_loss')
        ...
        box_nonvictim_logit = tf.reduce_max(class_predictions_with_background * (1 - victim_class_one_hot) - 10000 * victim_class_one_hot, axis=-1)
        ...
        weight_box_victim_cw_loss = tf.multiply(x=box_victim_cw_loss, y=box_victim_cw_weight, name='weight_box_victim_cw_loss')
        partial_loss = tf.add_n([weight_box_target_loss, weight_box_victim_loss, weight_box_target_cw_loss, weight_box_victim_cw_loss], name='partial_box_loss')
        return partial_loss
    ...
    def _create_rpn_loss(self) -> 'Tensor':
        ...
        rpn_objectness_predictions_with_background = self.estimator.predictions['rpn_objectness_predictions_with_background']
        rpn_objectness_predictions_with_background = tf.reshape(rpn_objectness_predictions_with_background, (-1, rpn_objectness_predictions_with_background.shape[-1]))
        ...
        weight_rpn_background_loss = tf.multiply(x=rpn_background_loss, y=rpn_background_weight, name='weight_rpn_background_loss')
        ...
        weight_rpn_foreground_loss = tf.multiply(x=rpn_foreground_loss, y=rpn_foreground_weight, name='weight_rpn_foreground_loss')
        ...
        partial_loss = tf.add_n([weight_rpn_background_loss, weight_rpn_foreground_loss, weight_rpn_cw_loss], name='partial_rpn_loss')
        return partial_loss
    ...
    def _check_params(self) -> None:
        ...
        if self.optimizer not in ['RMSPropOptimizer', 'MomentumOptimizer', 'GradientDescentOptimizer', 'AdamOptimizer']:
            raise ValueError('Optimizer only includes one of the following choices: `GradientDescentOptimizer`, `MomentumOptimizer`, `RMSPropOptimizer`, `AdamOptimizer`.')
        if self.optimizer in ['RMSPropOptimizer', 'MomentumOptimizer']:
        ...
        if not isinstance(self.sign_gradients, bool):
            raise ValueError('The choice of whether to use the sign of gradients for the optimization must be of type bool.')
        ...
        if not isinstance(self.texture_as_input, bool):
            raise ValueError('The choice of whether textures are used as inputs instead of images must be of type bool.')
