...
def test_triple_clip_values_error(art_warning):
    ...
    try:
        ...
        with pytest.raises(ValueError, match=exc_msg):
            LFilter(numerator_coef=np.array([0.1, 0.2, 0.3]), denominator_coef=np.array([0.1, 0.2, 0.3]), clip_values=(0, 1, 2))
    except ARTTestException as e:
        art_warning(e)
...
def test_relation_clip_values_error(art_warning):
    ...
    try:
        ...
        with pytest.raises(ValueError, match=exc_msg):
            LFilter(numerator_coef=np.array([0.1, 0.2, 0.3]), denominator_coef=np.array([0.1, 0.2, 0.3]), clip_values=(1, 0))
    except ARTTestException as e:
        art_warning(e)
...
def test_check_params(art_warning):
    ...
    try:
        ...
        with pytest.raises(ValueError):
            _ = LFilter(numerator_coef=np.array([0.1, 0.2, -0.1, -0.2]), denominator_coef=np.array([0.0, 0.1, 0.3, 0.4]))
        with pytest.raises(ValueError):
            _ = LFilter(numerator_coef=[0.1, 0.2, -0.1, -0.2], denominator_coef=np.array([1.0, 0.1, 0.3, 0.4]))
        with pytest.raises(ValueError):
            _ = LFilter(numerator_coef=np.array([0.1, 0.2, -0.1, -0.2]), denominator_coef=np.array([1.0, 0.1, 0.3, 0.4]), axis=1.0)
        with pytest.raises(ValueError):
            _ = LFilter(numerator_coef=np.array([0.1, 0.2, -0.1, -0.2]), denominator_coef=np.array([1.0, 0.1, 0.3, 0.4]), initial_cond=1.0)
        with pytest.raises(ValueError):
            _ = LFilter(numerator_coef=np.array([0.1, 0.2, -0.1, -0.2]), denominator_coef=np.array([1.0, 0.1, 0.3, 0.4]), verbose='True')
    except ARTTestException as e:
        art_warning(e)