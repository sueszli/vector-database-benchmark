...
def _zoneoutlstm(func, c, h, x, c_creator, h_creator):
    ...
    with chainer.using_device(device):
        ...
        h_next = h * h_creator.flag_h + _sigmoid(o) * xp.tanh(c_tmp) * h_creator.flag_x
    return (c_next, h_next)

@testing.parameterize({'in_size': 10, 'out_size': 10, 'c_ratio': 0.5, 'h_ratio': 0.25}, {'in_size': 10, 'out_size': 40, 'c_ratio': 0.25, 'h_ratio': 0.5}, {'in_size': 10, 'out_size': 10, 'c_ratio': 0.3, 'h_ratio': 0.3}, {'in_size': 10, 'out_size': 10, 'c_ratio': 1.0, 'h_ratio': 1.0}, {'in_size': 10, 'out_size': 40, 'c_ratio': 0.0, 'h_ratio': 0.0})
class TestZoneoutlstm(unittest.TestCase):

    def setUp(self):
        if False:
            return 10
        self.link = links.StatefulZoneoutLSTM(self.in_size, self.out_size, c_ratio=self.c_ratio, h_ratio=self.h_ratio)
    ...
    def check_forward(self, c_data, h_data, x_data):
        ...
        h1 = self.link(x)
        c1 = self.link.c
        (c1_expect, h1_expect) = _zoneoutlstm(self.link, c_data, h_data, x_data, c1.creator, h1.creator)
        ...
        h2 = self.link(x)
        c2 = self.link.c
        (c2_expect, h2_expect) = _zoneoutlstm(self.link, c1_expect, h1_expect, x_data, c2.creator, h2.creator)
    ...
    def check_backward(self, c_data, h_data, x_data, y_grad):
        ...
        def f():
            ...
            (c, y) = _zoneoutlstm(self.link, c_data, h_data, x_data, c_creator, y_creator)
            return (y,)
