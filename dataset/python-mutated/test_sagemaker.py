...
from airflow.providers.amazon.aws.hooks.sagemaker import LogState, SageMakerHook, secondary_training_status_changed, secondary_training_status_message
...
create_training_params = {'AlgorithmSpecification': {'TrainingImage': image, 'TrainingInputMode': 'File'}, 'RoleArn': role, 'OutputDataConfig': {'S3OutputPath': output_url}, 'ResourceConfig': {'InstanceCount': 2, 'InstanceType': 'ml.c4.8xlarge', 'VolumeSizeInGB': 50}, 'TrainingJobName': job_name, 'HyperParameters': {'k': '10', 'feature_dim': '784', 'mini_batch_size': '500', 'force_dense': 'True'}, 'StoppingCondition': {'MaxRuntimeInSeconds': 60 * 60}, 'InputDataConfig': [{'ChannelName': 'train', 'DataSource': {'S3DataSource': {'S3DataType': 'S3Prefix', 'S3Uri': data_url, 'S3DataDistributionType': 'FullyReplicated'}}, 'CompressionType': 'None', 'RecordWrapperType': 'None'}, {'ChannelName': 'train_fs', 'DataSource': {'FileSystemDataSource': {'DirectoryPath': '/tmp', 'FileSystemAccessMode': 'ro', 'FileSystemId': 'fs-abc', 'FileSystemType': 'FSxLustre'}}, 'CompressionType': 'None', 'RecordWrapperType': 'None'}]}
create_tuning_params = {'HyperParameterTuningJobName': job_name, 'HyperParameterTuningJobConfig': {'Strategy': 'Bayesian', 'HyperParameterTuningJobObjective': {'Type': 'Maximize', 'MetricName': 'test_metric'}, 'ResourceLimits': {'MaxNumberOfTrainingJobs': 123, 'MaxParallelTrainingJobs': 123}, 'ParameterRanges': {'IntegerParameterRanges': [{'Name': 'k', 'MinValue': '2', 'MaxValue': '10'}]}}, 'TrainingJobDefinition': {'StaticHyperParameters': create_training_params['HyperParameters'], 'AlgorithmSpecification': create_training_params['AlgorithmSpecification'], 'RoleArn': 'string', 'InputDataConfig': create_training_params['InputDataConfig'], 'OutputDataConfig': create_training_params['OutputDataConfig'], 'ResourceConfig': create_training_params['ResourceConfig'], 'StoppingCondition': dict(MaxRuntimeInSeconds=60 * 60)}}
create_transform_params = {'TransformJobName': job_name, 'ModelName': model_name, 'BatchStrategy': 'MultiRecord', 'TransformInput': {'DataSource': {'S3DataSource': {'S3DataType': 'S3Prefix', 'S3Uri': data_url}}}, 'TransformOutput': {'S3OutputPath': output_url}, 'TransformResources': {'InstanceType': 'ml.m4.xlarge', 'InstanceCount': 123}}
create_transform_params_fs = {'TransformJobName': job_name, 'ModelName': model_name, 'BatchStrategy': 'MultiRecord', 'TransformInput': {'DataSource': {'FileSystemDataSource': {'DirectoryPath': '/tmp', 'FileSystemAccessMode': 'ro', 'FileSystemId': 'fs-abc', 'FileSystemType': 'FSxLustre'}}}, 'TransformOutput': {'S3OutputPath': output_url}, 'TransformResources': {'InstanceType': 'ml.m4.xlarge', 'InstanceCount': 123}}
create_model_params = {'ModelName': model_name, 'PrimaryContainer': {'Image': image, 'ModelDataUrl': output_url}, 'ExecutionRoleArn': role}
create_endpoint_config_params = {'EndpointConfigName': config_name, 'ProductionVariants': [{'VariantName': 'AllTraffic', 'ModelName': model_name, 'InitialInstanceCount': 1, 'InstanceType': 'ml.c4.xlarge'}]}
...
DESCRIBE_TRAINING_COMPLETED_RETURN = {'TrainingJobStatus': 'Completed', 'ResourceConfig': {'InstanceCount': 1, 'InstanceType': 'ml.c4.xlarge', 'VolumeSizeInGB': 10}, 'TrainingStartTime': datetime(2018, 2, 17, 7, 15, 0, 103000), 'TrainingEndTime': datetime(2018, 2, 17, 7, 19, 34, 953000), 'ResponseMetadata': {'HTTPStatusCode': 200}}
...
message = 'message'
status = 'status'
SECONDARY_STATUS_DESCRIPTION_1 = {'SecondaryStatusTransitions': [{'StatusMessage': message, 'Status': status}]}
SECONDARY_STATUS_DESCRIPTION_2 = {'SecondaryStatusTransitions': [{'StatusMessage': 'different message', 'Status': status}]}
DEFAULT_LOG_STREAMS = {'logStreams': [{'logStreamName': job_name + '/xxxxxxxxx'}]}
LIFECYCLE_LOG_STREAMS = [DEFAULT_LOG_STREAMS, DEFAULT_LOG_STREAMS, DEFAULT_LOG_STREAMS, DEFAULT_LOG_STREAMS, DEFAULT_LOG_STREAMS, DEFAULT_LOG_STREAMS]
DEFAULT_LOG_EVENTS = [{'nextForwardToken': None, 'events': [{'timestamp': 1, 'message': 'hi there #1'}]}, {'nextForwardToken': None, 'events': []}]
STREAM_LOG_EVENTS = [{'nextForwardToken': None, 'events': [{'timestamp': 1, 'message': 'hi there #1'}]}, {'nextForwardToken': None, 'events': []}, {'nextForwardToken': None, 'events': [{'timestamp': 1, 'message': 'hi there #1'}, {'timestamp': 2, 'message': 'hi there #2'}]}, {'nextForwardToken': None, 'events': []}, {'nextForwardToken': None, 'events': [{'timestamp': 2, 'message': 'hi there #2'}, {'timestamp': 2, 'message': 'hi there #2a'}, {'timestamp': 3, 'message': 'hi there #3'}]}, {'nextForwardToken': None, 'events': []}]
test_evaluation_config = {'Image': image, 'Role': role, 'S3Operations': {'S3CreateBucket': [{'Bucket': bucket}], 'S3Upload': [{'Path': path, 'Bucket': bucket, 'Key': key, 'Tar': False}]}}
