...
COLLECTIONS_ABC_ATTRS = frozenset((attr for (mod, attr) in REPLACE_EXACT[3,] if mod == 'collections'))
...
def visit_Attribute(state: State, node: ast.Attribute, parent: ast.AST) -> Iterable[tuple[Offset, TokenFunc]]:
    if False:
        for i in range(10):
            print('nop')
    if isinstance(node.value, ast.Name) and node.value.id == 'collections' and (node.attr in COLLECTIONS_ABC_ATTRS):
