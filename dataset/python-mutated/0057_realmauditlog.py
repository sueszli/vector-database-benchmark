...
def backfill_user_activations_and_deactivations(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    if False:
        while True:
            i = 10
    ...
    for user in UserProfile.objects.all():
        RealmAuditLog.objects.create(realm=user.realm, modified_user=user, event_type='user_created', event_time=user.date_joined, backfilled=False)
    for user in UserProfile.objects.filter(is_active=False):
        RealmAuditLog.objects.create(realm=user.realm, modified_user=user, event_type='user_deactivated', event_time=migration_time, backfilled=True)
