...
parametrize_ipv6 = pytest.mark.parametrize('ipv6', [False, pytest.param(True, marks=binds_ipv6)], ids=['ipv4', 'ipv6'])
...
@asynccontextmanager
async def dtls_echo_server(*, autocancel: bool=True, mtu: int | None=None, ipv6: bool=False) -> AsyncGenerator[tuple[DTLSEndpoint, tuple[str, int]], None]:
    with endpoint(ipv6=ipv6) as server:
        ...
        async with trio.open_nursery() as nursery:
            ...
            async def echo_handler(dtls_channel: DTLSChannel) -> None:
                print(f'echo handler started: server {dtls_channel.endpoint.socket.getsockname()!r} client {dtls_channel.peer_address!r}')
                if mtu is not None:
...
@slow
async def test_handshake_over_terrible_network(autojump_clock: trio.testing.MockClock) -> None:
    HANDSHAKES = 100
    r = random.Random(0)
    fn = FakeNet()
    fn.enable()
    ...
    async with dtls_echo_server() as (_, address):
        async with trio.open_nursery() as nursery:
            ...
            async def route_packet(packet: UDPPacket) -> None:
                while True:
                    op = r.choices(['deliver', 'drop', 'dupe', 'delay'], weights=[0.7, 0.1, 0.1, 0.1])[0]
...
async def test_full_duplex() -> None:
    with endpoint() as server_endpoint, endpoint() as client_endpoint:
        ...
        async with trio.open_nursery() as server_nursery:
            ...
            client = client_endpoint.connect(server_endpoint.socket.getsockname(), client_ctx)
            async with trio.open_nursery() as nursery:
        ...
        with endpoint() as client_endpoint:
            ...
            async with trio.open_nursery() as nursery:
                with pytest.raises(trio.ClosedResourceError):
                    ...
                    async def null_handler(_: object) -> None:
                        pass
...
async def test_server_socket_doesnt_crash_on_garbage(autojump_clock: trio.abc.Clock) -> None:
    fn = FakeNet()
    fn.enable()
    from trio._dtls import ContentType, HandshakeFragment, HandshakeType, ProtocolVersion, Record, encode_handshake_fragment, encode_record
    client_hello = encode_record(Record(content_type=ContentType.handshake, version=ProtocolVersion.DTLS10, epoch_seqno=0, payload=encode_handshake_fragment(HandshakeFragment(msg_type=HandshakeType.client_hello, msg_len=10, msg_seq=0, frag_offset=0, frag_len=10, frag=bytes(10)))))
    ...
    client_hello_corrupt_record_len[11] = 255
    client_hello_fragmented = encode_record(Record(content_type=ContentType.handshake, version=ProtocolVersion.DTLS10, epoch_seqno=0, payload=encode_handshake_fragment(HandshakeFragment(msg_type=HandshakeType.client_hello, msg_len=20, msg_seq=0, frag_offset=0, frag_len=10, frag=bytes(10)))))
    client_hello_trailing_data_in_record = encode_record(Record(content_type=ContentType.handshake, version=ProtocolVersion.DTLS10, epoch_seqno=0, payload=encode_handshake_fragment(HandshakeFragment(msg_type=HandshakeType.client_hello, msg_len=20, msg_seq=0, frag_offset=0, frag_len=10, frag=bytes(10))) + b'\x00'))
    handshake_empty = encode_record(Record(content_type=ContentType.handshake, version=ProtocolVersion.DTLS10, epoch_seqno=0, payload=b''))
    client_hello_truncated_in_cookie = encode_record(Record(content_type=ContentType.handshake, version=ProtocolVersion.DTLS10, epoch_seqno=0, payload=bytes(2 + 32 + 1) + b'\xff'))
    async with dtls_echo_server() as (_, address):
        with trio.socket.socket(type=trio.socket.SOCK_DGRAM) as sock:
            for bad_packet in [b'', b'xyz', client_hello_extended, client_hello_short, client_hello_really_short, client_hello_corrupt_record_len, client_hello_fragmented, client_hello_trailing_data_in_record, handshake_empty, client_hello_truncated_in_cookie]:
...
async def test_client_cancels_handshake_and_starts_new_one(autojump_clock: trio.abc.Clock) -> None:
    fn = FakeNet()
    fn.enable()
...
async def test_initial_retransmit_timeout_configuration(autojump_clock: trio.abc.Clock) -> None:
    fn = FakeNet()
    fn.enable()
    blackholed = True
...
@parametrize_ipv6
async def test_handshake_handles_minimum_network_mtu(ipv6: bool, autojump_clock: trio.abc.Clock) -> None:
    fn = FakeNet()
    fn.enable()
    ...
    async with dtls_echo_server(ipv6=ipv6) as (_, address):
        with endpoint(ipv6=ipv6) as client_endpoint:
            ...
            await client.send(b'xyz')
            assert await client.receive() == b'xyz'
            assert client.get_cleartext_mtu() > 9000
...
async def test_socket_closed_while_processing_clienthello(autojump_clock: trio.abc.Clock) -> None:
    fn = FakeNet()
    fn.enable()
...
async def test_association_replaced_while_handshake_running(autojump_clock: trio.abc.Clock) -> None:
    fn = FakeNet()
    fn.enable()
