...
categorical_features = ['country', 'province', 'region_1', 'region_2', 'variety', 'winery']
target_name = 'points'
...
for (name, categorical_preprocessor) in categorical_preprocessors:
    preprocessor = ColumnTransformer([('numerical', 'passthrough', numerical_features), ('categorical', categorical_preprocessor, categorical_features)])
...
mixed_encoded_preprocessor = ColumnTransformer([('numerical', 'passthrough', numerical_features), ('high_cardinality', TargetEncoder(target_type='continuous'), high_cardinality_features), ('low_cardinality', OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1), low_cardinality_features)], verbose_feature_names_out=False)
mixed_encoded_preprocessor.set_output(transform='pandas')
mixed_pipe = make_pipeline(mixed_encoded_preprocessor, HistGradientBoostingRegressor(random_state=0, max_iter=max_iter, categorical_features=low_cardinality_features))
mixed_pipe
...
for (subset, ax) in zip(['test', 'train'], [ax1, ax2]):
    ...
    ax.bar(x=xticks, height=data[mean], yerr=data[std], width=0.9, color=[name_to_color[name] for name in data.index])
    ax.set(title=f'RMSE ({subset.title()})', xlabel='Encoding Scheme', xticks=xticks, xticklabels=data.index)