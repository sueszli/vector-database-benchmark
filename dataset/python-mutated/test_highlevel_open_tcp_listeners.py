...
import pytest
import trio
from trio import SocketListener, TrioDeprecationWarning, open_tcp_listeners, open_tcp_stream, serve_tcp
...
class FakeSocketFactory(SocketFactory):
    ...
    raise_on_family: dict[AddressFamily, int] = attr.ib(factory=dict)
...
async def test_open_tcp_listeners_multiple_host_cleanup_on_error() -> None:
    ...
    tsocket.set_custom_hostname_resolver(FakeHostnameResolver([(tsocket.AF_INET, '1.1.1.1'), (tsocket.AF_INET, '2.2.2.2'), (tsocket.AF_INET, '3.3.3.3')]))
...
@pytest.mark.parametrize('try_families', [{tsocket.AF_INET}, {tsocket.AF_INET6}, {tsocket.AF_INET, tsocket.AF_INET6}])
@pytest.mark.parametrize('fail_families', [{tsocket.AF_INET}, {tsocket.AF_INET6}, {tsocket.AF_INET, tsocket.AF_INET6}])
async def test_open_tcp_listeners_some_address_families_unavailable(try_families: set[AddressFamily], fail_families: set[AddressFamily]) -> None:
...
async def test_open_tcp_listeners_backlog() -> None:
    ...
    tsocket.set_custom_socket_factory(fsf)
    for (given, expected) in [(None, 65535), (99999999, 65535), (10, 10), (1, 1)]:
...
async def test_open_tcp_listeners_backlog_inf_warning() -> None:
    ...
    with pytest.warns(TrioDeprecationWarning):
        listeners = await open_tcp_listeners(0, backlog=float('inf'))
    assert listeners
    for listener in listeners:
        assert listener.socket.backlog == 65535

async def test_open_tcp_listeners_backlog_float_error() -> None:
    fsf = FakeSocketFactory(99)
    tsocket.set_custom_socket_factory(fsf)
    for should_fail in (0.0, 2.18, 3.14, 9.75):
        with pytest.raises(TypeError, match=f'backlog must be an int or None, not {should_fail!r}'):
