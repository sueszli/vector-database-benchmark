...
def prepare_free_shipping_voucher(e2e_staff_api_client, channel_id, voucher_code, voucher_discount_type, voucher_discount_value, voucher_type, min_amount_spent):
    if False:
        while True:
            i = 10
    ...
    channel_listing = [{'channelId': channel_id, 'discountValue': voucher_discount_value, 'minAmountSpent': min_amount_spent}]
    create_voucher_channel_listing(e2e_staff_api_client, voucher_id, channel_listing)
    return voucher_code

@pytest.mark.e2e
def test_checkout_use_free_shipping_voucher_with_min_spent_amount_0903(e2e_not_logged_api_client, e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_shipping, permission_manage_product_types_and_attributes, permission_manage_discounts):
    if False:
        i = 10
        return i + 15
    ...
    (_product_id, product_variant_id, product_variant_price) = prepare_product(e2e_staff_api_client, warehouse_id, channel_id, variant_price=25)
    voucher_code = prepare_free_shipping_voucher(e2e_staff_api_client, channel_id, voucher_code='FREESHIPPING', voucher_discount_type='PERCENTAGE', voucher_discount_value=100, voucher_type='SHIPPING', min_amount_spent=75)
    ...
    lines_add = [{'variantId': product_variant_id, 'quantity': 2}]
