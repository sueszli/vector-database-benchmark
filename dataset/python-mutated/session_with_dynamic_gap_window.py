...
from pyflink.datastream.window import EventTimeSessionWindows, SessionWindowTimeGapExtractor, TimeWindow
...
if __name__ == '__main__':
    ...
    data_stream = env.from_collection([('hi', 1), ('hi', 2), ('hi', 3), ('hi', 4), ('hi', 8), ('hi', 9), ('hi', 15)], type_info=Types.TUPLE([Types.STRING(), Types.INT()]))
    watermark_strategy = WatermarkStrategy.for_monotonous_timestamps().with_timestamp_assigner(MyTimestampAssigner())
    ds = data_stream.assign_timestamps_and_watermarks(watermark_strategy).key_by(lambda x: x[0], key_type=Types.STRING()).window(EventTimeSessionWindows.with_dynamic_gap(MySessionWindowTimeGapExtractor())).process(CountWindowProcessFunction(), Types.TUPLE([Types.STRING(), Types.INT(), Types.INT(), Types.INT()]))
