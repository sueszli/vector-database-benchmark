...
class capillary_threaded_decoder(gr.hier_block2):
    ...
    def __init__(self, decoder_list_0, input_size, output_size):
        if False:
            for i in range(10):
                print('nop')
        gr.hier_block2.__init__(self, 'Capillary Threaded Decoder', gr.io_signature(1, 1, input_size * 1), gr.io_signature(1, 1, output_size * 1))
        ...
        for i in range(int(math.log(len(decoder_list_0), 2))):
            for j in range(int(math.pow(2, i))):
                self.deinterleaves_0.append(blocks.deinterleave(input_size, fec.get_decoder_input_size(decoder_list_0[0])))
        ...
        for i in range(int(math.log(len(decoder_list_0), 2))):
            for j in range(int(math.pow(2, i))):
                self.interleaves_0.append(blocks.interleave(output_size, fec.get_decoder_output_size(decoder_list_0[0])))
        rootcount = 0
        branchcount = 1
