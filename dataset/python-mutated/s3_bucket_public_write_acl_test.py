...
class Test_s3_bucket_public_write_acl:
    ...
    def test_no_buckets(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3control_client', new=S3Control(audit_info)):
                    from prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl import s3_bucket_public_write_acl
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert len(result) == 1
                    assert result[0].status == 'PASS'
                    assert result[0].status_extended == f'S3 Bucket {bucket_name_us} is not publicly writable.'
    ...
    def test_bucket_public_write_ACL_AllUsers_WRITE(self):
        ...
        s3_client.put_bucket_acl(Bucket=bucket_name_us, AccessControlPolicy={'Grants': [{'Grantee': {'URI': 'http://acs.amazonaws.com/groups/global/AllUsers', 'Type': 'Group'}, 'Permission': 'WRITE'}], 'Owner': bucket_owner})
        from prowler.providers.aws.services.s3.s3_service import S3, S3Control
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert result[0].status_extended == f'S3 Bucket {bucket_name_us} is writable by anyone due to the bucket ACL: AllUsers having the WRITE permission.'
                    assert result[0].resource_id == bucket_name_us
                    assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                    assert result[0].region == AWS_REGION
    ...
    def test_bucket_public_write_ACL_AllUsers_WRITE_ACP(self):
        ...
        s3control_client.put_public_access_block(AccountId=AWS_ACCOUNT_NUMBER, PublicAccessBlockConfiguration={'BlockPublicAcls': False, 'IgnorePublicAcls': False, 'BlockPublicPolicy': False, 'RestrictPublicBuckets': False})
        s3_client.put_public_access_block(Bucket=bucket_name_us, PublicAccessBlockConfiguration={'BlockPublicAcls': False, 'IgnorePublicAcls': False, 'BlockPublicPolicy': False, 'RestrictPublicBuckets': False})
        s3_client.put_bucket_acl(Bucket=bucket_name_us, AccessControlPolicy={'Grants': [{'Grantee': {'URI': 'http://acs.amazonaws.com/groups/global/AllUsers', 'Type': 'Group'}, 'Permission': 'WRITE_ACP'}], 'Owner': bucket_owner})
        from prowler.providers.aws.services.s3.s3_service import S3, S3Control
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert len(result) == 1
                    assert result[0].status == 'FAIL'
                    assert result[0].status_extended == f'S3 Bucket {bucket_name_us} is writable by anyone due to the bucket ACL: AllUsers having the WRITE_ACP permission.'
                    assert result[0].resource_id == bucket_name_us
                    assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                    assert result[0].region == AWS_REGION
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert len(result) == 1
                    assert result[0].status == 'FAIL'
                    assert result[0].status_extended == f'S3 Bucket {bucket_name_us} is writable by anyone due to the bucket ACL: AllUsers having the FULL_CONTROL permission.'
                    assert result[0].resource_id == bucket_name_us
                    assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                    assert result[0].region == AWS_REGION
    ...
    def test_bucket_public_write_ACL_AuthenticatedUsers_WRITE(self):
        ...
        s3control_client.put_public_access_block(AccountId=AWS_ACCOUNT_NUMBER, PublicAccessBlockConfiguration={'BlockPublicAcls': False, 'IgnorePublicAcls': False, 'BlockPublicPolicy': False, 'RestrictPublicBuckets': False})
        s3_client.put_public_access_block(Bucket=bucket_name_us, PublicAccessBlockConfiguration={'BlockPublicAcls': False, 'IgnorePublicAcls': False, 'BlockPublicPolicy': False, 'RestrictPublicBuckets': False})
        s3_client.put_bucket_acl(Bucket=bucket_name_us, AccessControlPolicy={'Grants': [{'Grantee': {'URI': 'http://acs.amazonaws.com/groups/global/AuthenticatedUsers', 'Type': 'Group'}, 'Permission': 'WRITE'}], 'Owner': bucket_owner})
        from prowler.providers.aws.services.s3.s3_service import S3, S3Control
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert len(result) == 1
                    assert result[0].status == 'FAIL'
                    assert result[0].status_extended == f'S3 Bucket {bucket_name_us} is writable by anyone due to the bucket ACL: AuthenticatedUsers having the WRITE permission.'
                    assert result[0].resource_id == bucket_name_us
                    assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                    assert result[0].region == AWS_REGION
    ...
    def test_bucket_public_write_ACL_AuthenticatedUsers_WRITE_ACP(self):
        ...
        s3control_client.put_public_access_block(AccountId=AWS_ACCOUNT_NUMBER, PublicAccessBlockConfiguration={'BlockPublicAcls': False, 'IgnorePublicAcls': False, 'BlockPublicPolicy': False, 'RestrictPublicBuckets': False})
        s3_client.put_public_access_block(Bucket=bucket_name_us, PublicAccessBlockConfiguration={'BlockPublicAcls': False, 'IgnorePublicAcls': False, 'BlockPublicPolicy': False, 'RestrictPublicBuckets': False})
        s3_client.put_bucket_acl(Bucket=bucket_name_us, AccessControlPolicy={'Grants': [{'Grantee': {'URI': 'http://acs.amazonaws.com/groups/global/AuthenticatedUsers', 'Type': 'Group'}, 'Permission': 'WRITE_ACP'}], 'Owner': bucket_owner})
        from prowler.providers.aws.services.s3.s3_service import S3, S3Control
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert len(result) == 1
                    assert result[0].status == 'FAIL'
                    assert result[0].status_extended == f'S3 Bucket {bucket_name_us} is writable by anyone due to the bucket ACL: AuthenticatedUsers having the WRITE_ACP permission.'
                    assert result[0].resource_id == bucket_name_us
                    assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                    assert result[0].region == AWS_REGION
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3_client', new=S3(audit_info)):
                with mock.patch('prowler.providers.aws.services.s3.s3_bucket_public_write_acl.s3_bucket_public_write_acl.s3control_client', new=S3Control(audit_info)):
                    ...
                    assert len(result) == 1
                    assert result[0].status == 'FAIL'
                    assert result[0].status_extended == f'S3 Bucket {bucket_name_us} is writable by anyone due to the bucket ACL: AuthenticatedUsers having the FULL_CONTROL permission.'
                    assert result[0].resource_id == bucket_name_us
                    assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                    assert result[0].region == AWS_REGION