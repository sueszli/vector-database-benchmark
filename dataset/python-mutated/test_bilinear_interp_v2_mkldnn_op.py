...
def bilinear_interp_onednn_np(input, out_h, out_w, out_size=None, actual_shape=None, data_layout='NCHW'):
    if False:
        print('Hello World!')
...
class TestBilinearInterpOneDNNOp(OpTest):
    ...
    def setUp(self):
        ...
        if scale_h > 0 and scale_w > 0:
            ...
            out_h = self.out_h
            out_w = self.out_w
        output_np = bilinear_interp_onednn_np(input_np, out_h, out_w, self.out_size, self.actual_shape, self.data_layout)
        ...
        self.attrs = {'interp_method': self.interp_method, 'out_h': self.out_h, 'out_w': self.out_w, 'scale': self.scale, 'data_layout': self.data_layout, 'use_mkldnn': self.use_onednn}
