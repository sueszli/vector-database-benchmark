...
@pluginmatcher(re.compile('https?://(?:www\\.)?bigo\\.tv/([^/]+)$'))
class Bigo(Plugin):
    _api_url = 'https://www.bigo.tv/OInterface/getVideoParam?bigoId={0}'
    _video_info_schema = validate.Schema({'code': 0, 'msg': 'success', 'data': {'videoSrc': validate.any(None, '', validate.url())}})

    def _get_streams(self):
        if False:
            for i in range(10):
                print('nop')
        res = self.session.http.get(self._api_url.format(self.match.group(1)), allow_redirects=True, headers={'User-Agent': useragents.IPHONE_6})
