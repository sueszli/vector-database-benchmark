...
class FileStoreTo(SimpleDownloader):
    ...
    __version__ = '0.16'
    __status__ = 'testing'
    __pattern__ = 'http://(?:www\\.)?filestore\\.to/\\?d=(?P<ID>\\w+)'
    __config__ = [('enabled', 'bool', 'Activated', True), ('use_premium', 'bool', 'Use premium account if available', True), ('fallback', 'bool', 'Fallback to free download if premium fails', True), ('chk_filesize', 'bool', 'Check file size', True), ('max_wait', 'int', 'Reconnect if waiting time is greater than minutes', 10), ('freeslot_wait', 'int', 'Delay to wait for free slot (seconds)', 600), ('freeslot_attempts', 'int', 'Number of retries to wait for free slot', 15), ('beadheader_retry', 'bool', 'Retry download on HTTP Header 503', True)]
    __description__ = 'FileStore.to downloader plugin'
    __license__ = 'GPLv3'
    __authors__ = [('Walter Purcaro', 'vuolter@gmail.com'), ('stickell', 'l.stickell@yahoo.it'), ('GammaC0de', 'nitzo2001[AT]yahoo[DOT]com')]
    NAME_PATTERN = '<div class="file">(?P<N>.+?)</div>'
    SIZE_PATTERN = '<div class="size">(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)</div>'
    OFFLINE_PATTERN = '>(?:Download-Datei wurde nicht gefunden|Datei nicht gefunden)<'
    TEMP_OFFLINE_PATTERN = '>Der Download ist nicht bereit !<'
    NO_FREESLOTS_PATTERN = '>Leider sind aktuell keine freien Downloadslots für Freeuser verfügbar<'
    WAIT_PATTERN = 'data-wait="(\\d+?)"'
    LINK_PATTERN = 'klicke <a href="(.+?)">hier<'
