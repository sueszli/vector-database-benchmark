...
from azure.communication.jobrouter.models import LongestIdleMode, RouterChannel, RouterJobStatus, RouterWorker, RouterJobOffer, AcceptJobOfferResult, UnassignJobResult, RouterJob, RouterJobAssignment, RouterWorkerState, DistributionPolicy, RouterQueue, CancelJobOptions, CompleteJobOptions, CloseJobOptions, DeclineJobOfferOptions
...
class TestAssignmentScenarioAsync(AsyncRouterRecordedTestCase):
    ...
    async def setup_distribution_policy(self, **kwargs):
        ...
        async with client:
            ...
            policy: DistributionPolicy = DistributionPolicy(name='test', offer_expires_after_seconds=10.0 * 60, mode=LongestIdleMode(min_concurrent_offers=1, max_concurrent_offers=1))
    ...
    async def validate_worker_has_offer(self, worker_id, job_id, **kwargs):
        router_client: JobRouterClient = self.create_client()
        async with router_client:
    ...
    async def validate_worker_deregistration(self, worker_id, **kwargs):
        router_client: JobRouterClient = self.create_client()
        async with router_client:
            router_worker: RouterWorker = await router_client.get_worker(worker_id)
    ...
    async def test_assignment_scenario(self, **kwargs):
        ...
        async with router_client:
            ...
            await self._poll_until_no_exception(self.validate_worker_has_offer, AssertionError, self.get_router_worker_id(), job_identifier)
            ...
            accept_job_offer_result: AcceptJobOfferResult = await router_client.accept_job_offer(worker_id=self.get_router_worker_id(), offer_id=offer_id)
            ...
            with pytest.raises(HttpResponseError) as sre:
                await router_client.decline_job_offer(worker_id=self.get_router_worker_id(), offer_id=offer_id, options=DeclineJobOfferOptions(retry_offer_at=datetime.min))
