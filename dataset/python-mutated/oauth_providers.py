...
class GithubOAuthProvider(OAuthProvider):
    ...
    async def get_token(self, code: str, url: str):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post('https://github.com/login/oauth/access_token', data=payload)
    ...
    async def get_user_info(self, token: str):
        async with httpx.AsyncClient() as client:
            user_response = await client.get('https://api.github.com/user', headers={'Authorization': f'token {token}'})
            user_response.raise_for_status()
            user = user_response.json()
            emails_response = await client.get('https://api.github.com/user/emails', headers={'Authorization': f'token {token}'})
            emails_response.raise_for_status()
            emails = emails_response.json()
            user.update({'emails': emails})
            app_user = AppUser(username=user['login'], image=user['avatar_url'], provider='github')
...
class GoogleOAuthProvider(OAuthProvider):
    ...
    async def get_token(self, code: str, url: str):
        payload = {'client_id': self.client_id, 'client_secret': self.client_secret, 'code': code, 'grant_type': 'authorization_code', 'redirect_uri': url}
        async with httpx.AsyncClient() as client:
            ...
            if not token:
                raise httpx.HTTPStatusError('Failed to get the access token', request=response.request, response=response)
            return token

    async def get_user_info(self, token: str):
        async with httpx.AsyncClient() as client:
            response = await client.get('https://www.googleapis.com/userinfo/v2/me', headers={'Authorization': f'Bearer {token}'})
            response.raise_for_status()
            user = response.json()
            app_user = AppUser(username=user['name'], image=user['picture'], provider='google')
            return (user, app_user)

class AzureADOAuthProvider(OAuthProvider):
    id = 'azure-ad'
    env = ['OAUTH_AZURE_AD_CLIENT_ID', 'OAUTH_AZURE_AD_CLIENT_SECRET', 'OAUTH_AZURE_AD_TENANT_ID']
    authorize_url = f"https://login.microsoftonline.com/{os.environ.get('OAUTH_AZURE_AD_TENANT_ID', '')}/oauth2/v2.0/authorize" if os.environ.get('OAUTH_AZURE_AD_ENABLE_SINGLE_TENANT') else 'https://login.microsoftonline.com/common/oauth2/v2.0/authorize'
    token_url = f"https://login.microsoftonline.com/{os.environ.get('OAUTH_AZURE_AD_TENANT_ID', '')}/oauth2/v2.0/token" if os.environ.get('OAUTH_AZURE_AD_ENABLE_SINGLE_TENANT') else 'https://login.microsoftonline.com/common/oauth2/v2.0/token'
    ...
    async def get_user_info(self, token: str):
        async with httpx.AsyncClient() as client:
            response = await client.get('https://graph.microsoft.com/v1.0/me', headers={'Authorization': f'Bearer {token}'})
            response.raise_for_status()
            user = response.json()
            try:
                photo_response = await client.get('https://graph.microsoft.com/v1.0/me/photos/48x48/$value', headers={'Authorization': f'Bearer {token}'})
                photo_data = await photo_response.aread()
                base64_image = base64.b64encode(photo_data)
                user['image'] = f"data:{photo_response.headers['Content-Type']};base64,{base64_image.decode('utf-8')}"
            except Exception as e:
                pass
            app_user = AppUser(username=user['userPrincipalName'], image=user.get('image', ''), provider='azure-ad')
            return (user, app_user)

class OktaOAuthProvider(OAuthProvider):
    id = 'okta'
    env = ['OAUTH_OKTA_CLIENT_ID', 'OAUTH_OKTA_CLIENT_SECRET', 'OAUTH_OKTA_DOMAIN']
    ...
    async def get_token(self, code: str, url: str):
        payload = {'client_id': self.client_id, 'client_secret': self.client_secret, 'code': code, 'grant_type': 'authorization_code', 'redirect_uri': url}
        async with httpx.AsyncClient() as client:
            response = await client.post(f'{self.domain}/oauth2{self.get_authorization_server_path()}/v1/token', data=payload)
            response.raise_for_status()
    ...
    async def get_user_info(self, token: str):
        async with httpx.AsyncClient() as client:
            response = await client.get(f'{self.domain}/oauth2{self.get_authorization_server_path()}/v1/userinfo', headers={'Authorization': f'Bearer {token}'})
            response.raise_for_status()
            user = response.json()
...
class Auth0OAuthProvider(OAuthProvider):
    ...
    async def get_token(self, code: str, url: str):
        payload = {'client_id': self.client_id, 'client_secret': self.client_secret, 'code': code, 'grant_type': 'authorization_code', 'redirect_uri': url}
        async with httpx.AsyncClient() as client:
            response = await client.post(f'{self.domain}/oauth/token', data=payload)
            response.raise_for_status()
    ...
    async def get_user_info(self, token: str):
        async with httpx.AsyncClient() as client:
            response = await client.get(f'{self.original_domain}/userinfo', headers={'Authorization': f'Bearer {token}'})
            response.raise_for_status()
            user = response.json()
            app_user = AppUser(username=user.get('email'), image=user.get('picture', ''), provider='auth0')
            return (user, app_user)
...
class DescopeOAuthProvider(OAuthProvider):
    ...
    async def get_token(self, code: str, url: str):
        payload = {'client_id': self.client_id, 'client_secret': self.client_secret, 'code': code, 'grant_type': 'authorization_code', 'redirect_uri': url}
        async with httpx.AsyncClient() as client:
            response = await client.post(f'{self.domain}/token', data=payload)
            response.raise_for_status()
            json_content = response.json()
            token = json_content.get('access_token')
            if not token:
    ...
    async def get_user_info(self, token: str):
        async with httpx.AsyncClient() as client:
            response = await client.get(f'{self.domain}/userinfo', headers={'Authorization': f'Bearer {token}'})
            response.raise_for_status()
            user = response.json()
            app_user = AppUser(username=user.get('email'), image='', provider='descope')
            return (user, app_user)
providers = [GithubOAuthProvider(), GoogleOAuthProvider(), AzureADOAuthProvider(), OktaOAuthProvider(), Auth0OAuthProvider(), DescopeOAuthProvider()]
