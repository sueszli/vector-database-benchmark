...
class enterpriseknowledgegraphCallTransformer(cst.CSTTransformer):
    CTRL_PARAMS: Tuple[str] = ('retry', 'timeout', 'metadata')
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'cancel_entity_reconciliation_job': ('name',), 'create_entity_reconciliation_job': ('parent', 'entity_reconciliation_job'), 'delete_entity_reconciliation_job': ('name',), 'get_entity_reconciliation_job': ('name',), 'list_entity_reconciliation_jobs': ('parent', 'filter', 'page_size', 'page_token'), 'lookup': ('parent', 'ids', 'languages'), 'lookup_public_kg': ('parent', 'ids', 'languages'), 'search': ('parent', 'query', 'languages', 'types', 'limit'), 'search_public_kg': ('parent', 'query', 'languages', 'types', 'limit')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=enterpriseknowledgegraphCallTransformer()):
    if False:
        return 10
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Fix up source that uses the enterpriseknowledgegraph client library.\n\nThe existing sources are NOT overwritten but are copied to output_dir with changes made.\n\nNote: This tool operates at a best-effort level at converting positional\n      parameters in client method calls to keyword based parameters.\n      Cases where it WILL FAIL include\n      A) * or ** expansion in a method call.\n      B) Calls via function or method alias (includes free function calls)\n      C) Indirect or dispatched calls (e.g. the method is looked up dynamically)\n\n      These all constitute false negatives. The tool will also detect false\n      positives when an API method shares a name with another method.\n')
