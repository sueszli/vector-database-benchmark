...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-gpu', action='store_true', help='Whether to use GPU for training.')
    parser.add_argument('--epochs', type=int, default=3, help='Number of epochs to train for.')
    parser.add_argument('--global-batch-size', '-b', type=int, default=32, help='Global batch size to use for training.')
    parser.add_argument('--dataset', '-d', type=str, choices=['reddit', 'fake'], default='reddit', help="The dataset to use. Either 'reddit' or 'fake' Defaults to 'reddit'.")
    (args, _) = parser.parse_known_args()
    train_gnn(num_workers=args.num_workers, use_gpu=args.use_gpu, epochs=args.epochs, global_batch_size=args.global_batch_size, dataset=args.dataset)