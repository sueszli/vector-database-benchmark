...
class PageTypeReorderAttributes(BaseReorderAttributesMutation):
    page_type = graphene.Field(PageType, description='Page type from which attributes are reordered.')

    class Arguments:
        page_type_id = graphene.Argument(graphene.ID, required=True, description='ID of a page type.')
        moves = NonNullList(ReorderInput, required=True, description='The list of attribute reordering operations.')
