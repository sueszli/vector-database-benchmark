...
class TestRNNOp(OpTest):
    ...
    def setUp(self):
        ...
        if self.sequence_length is not None:
            ...
            input[9][3:][:] = 0
            input[8][4:][:] = 0
        rnn1 = LSTM(input_size, hidden_size, num_layers=self.num_layers, time_major=True, direction=direction, dropout=self.dropout, dtype=self.dtype)
        ...
        init_c = np.zeros((self.num_layers * self.direction_num, batch_size, hidden_size)).astype(self.dtype)
        ...
        self.attrs = {'dropout_prob': self.dropout, 'is_bidirec': self.is_bidirec, 'input_size': input_size, 'hidden_size': hidden_size, 'num_layers': self.num_layers, 'mode': self.mode, 'is_test': self.is_test}
    ...
    def test_grad(self):
        ...
        if not self.is_test:
            ...
            self.check_grad(set(grad_check_list), ['Out', 'last_hidden', 'last_cell'])
