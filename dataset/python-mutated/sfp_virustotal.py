...
class sfp_virustotal(SpiderFootPlugin):
    meta = {'name': 'VirusTotal', 'summary': 'Obtain information from VirusTotal about identified IP addresses.', 'flags': ['apikey'], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'website': 'https://www.virustotal.com/', 'model': 'FREE_AUTH_LIMITED', 'references': ['https://developers.virustotal.com/reference'], 'apiKeyInstructions': ['Visit https://www.virustotal.com/', 'Register a free account', 'Click on your profile', 'Click on API Key', "The API key is listed under 'API Key'"], 'favIcon': 'https://www.virustotal.com/gui/images/favicon.png', 'logo': 'https://www.virustotal.com/gui/images/logo.svg', 'description': 'Analyze suspicious files and URLs to detect types of malware, automatically share them with the security community.'}}
    opts = {'api_key': '', 'verify': True, 'publicapi': True, 'checkcohosts': True, 'checkaffiliates': True, 'netblocklookup': True, 'maxnetblock': 24, 'subnetlookup': True, 'maxsubnet': 24}
    optdescs = {'api_key': 'VirusTotal API Key.', 'publicapi': 'Are you using a public key? If so SpiderFoot will pause for 15 seconds after each query to avoid VirusTotal dropping requests.', 'checkcohosts': 'Check co-hosted sites?', 'checkaffiliates': 'Check affiliates?', 'netblocklookup': 'Look up all IPs on netblocks deemed to be owned by your target for possible hosts on the same target subdomain/domain?', 'maxnetblock': 'If looking up owned netblocks, the maximum netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'subnetlookup': 'Look up all IPs on subnets which your target is a part of?', 'maxsubnet': 'If looking up subnets, the maximum subnet size to look up all the IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'verify': 'Verify that any hostnames found on the target domain still resolve?'}
    results = None
    errorState = False
