...
from searx.utils import eval_xpath, extract_url, extract_text, eval_xpath_list, eval_xpath_getindex
about = {'website': 'https://play.google.com/', 'wikidata_id': 'Q79576', 'use_official_api': False, 'require_api_key': False, 'results': 'HTML'}
...
def request(query, params):
    ...
    params['url'] = search_url.format(query=urlencode({'q': query}), play_categ=play_categ)
...
def response_movies(resp):
    ...
    for section in eval_xpath(dom, '//c-wiz/section/header/..'):
        ...
        for item in eval_xpath(section, './/a'):
            ...
            img_src = img.get('src')
            results.append({'url': url, 'title': title, 'content': sec_name, 'img_src': img_src, 'metadata': metadata, 'template': 'videos.html'})
    return results
