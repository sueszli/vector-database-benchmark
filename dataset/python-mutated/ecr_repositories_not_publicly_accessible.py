    ...
    def execute(self):
        ...
        for registry in ecr_client.registries.values():
            for repository in registry.repositories:
                ...
                report.status = 'PASS'
                report.status_extended = f'Repository {repository.name} is not publicly accesible.'
                if repository.policy:
                    for statement in repository.policy['Statement']:
                        if statement['Effect'] == 'Allow':
                            if '*' in statement['Principal'] or ('AWS' in statement['Principal'] and '*' in statement['Principal']['AWS']):
                                report.status = 'FAIL'
