...
class GKEOperationTrigger(BaseTrigger):
    ...
    async def run(self) -> AsyncIterator[TriggerEvent]:
        ...
        try:
            while True:
                operation = await hook.get_operation(operation_name=self.operation_name, project_id=self.project_id)
                status = operation.status
                if status == Operation.Status.DONE:
                    yield TriggerEvent({'status': 'success', 'message': 'Operation is successfully ended.', 'operation_name': operation.name})
                    return
                    ...
                    yield TriggerEvent({'status': 'failed', 'message': f'Operation has failed with status: {operation.status}'})
                    return
