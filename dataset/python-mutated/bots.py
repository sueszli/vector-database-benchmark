...
class Notes(BaseModel):
    ...
    records: dict[str, list] = Field(default_factory=dict, description='a list of notes for each topic')
...
async def select_a_meme(query: str) -> dict:
    ...
    try:
        ...
        raise ImportError("The serpapi library is required to use the MemeGenerator tool. Please install it with `pip install 'marvin[serpapi]'`.")
    results = GoogleSearch({'q': query, 'tbm': 'isch', 'api_key': marvin_recipes.settings.google_api_key.get_secret_value() if marvin_recipes.settings.google_api_key else None}).get_dict()
    if 'error' in results:
