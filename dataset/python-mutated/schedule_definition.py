...
from typing import TYPE_CHECKING, Any, Callable, Dict, Iterator, List, Mapping, NamedTuple, Optional, Sequence, Set, TypeVar, Union, cast
import pendulum
...
from ..errors import DagsterInvalidDefinitionError, DagsterInvalidInvocationError, DagsterInvariantViolationError, ScheduleExecutionError, user_code_error_boundary
...
ScheduleEvaluationFunctionReturn: TypeAlias = Union[RunRequest, SkipReason, RunConfig, RunRequestIterator, Sequence[RunRequest]]
RawScheduleEvaluationFunction: TypeAlias = Callable[..., ScheduleEvaluationFunctionReturn]
ScheduleRunConfigFunction: TypeAlias = Union[Callable[['ScheduleEvaluationContext'], RunConfig], Callable[[], RunConfig]]
...
ScheduleExecutionFunction: TypeAlias = Union[Callable[['ScheduleEvaluationContext'], Any], 'DecoratedScheduleFunction']
...
class ScheduleEvaluationContext:
    ...
    """
    __slots__ = ['_instance_ref', '_scheduled_execution_time', '_exit_stack', '_instance', '_log_key', '_logger', '_repository_name', '_resource_defs', '_schedule_name', '_resources_cm', '_resources', '_cm_scope_entered', '_repository_def']
...
@whitelist_for_serdes
class ScheduleExecutionData(NamedTuple('_ScheduleExecutionData', [('run_requests', Optional[Sequence[RunRequest]]), ('skip_message', Optional[str]), ('captured_log_key', Optional[Sequence[str]])])):
...
@deprecated_param(param='environment_vars', breaking_version='2.0', additional_warn_text='It is no longer necessary. Schedules will have access to all environment variables set in the containing environment, and can safely be deleted.')
