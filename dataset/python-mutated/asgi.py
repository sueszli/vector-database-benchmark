...
from .base import JSON, DebuggableGraphQLTransportWSMixin, DebuggableGraphQLWSMixin, HttpClient, Message, Response, ResultOverrideFunction, WebSocketClient

class DebuggableGraphQLTransportWSHandler(DebuggableGraphQLTransportWSMixin, GraphQLTransportWSHandler):
    pass
...
class GraphQLView(BaseGraphQLView):
    ...
    async def get_context(self, request: Union[Request, WebSocket], response: Optional[StarletteResponse]=None) -> object:
...
class AsgiHttpClient(HttpClient):
    ...
    def __init__(self, graphiql: Optional[bool]=None, graphql_ide: Optional[GraphQL_IDE]='graphiql', allow_queries_via_get: bool=True, result_override: ResultOverrideFunction=None):
        if False:
            i = 10
            return i + 15
        view = GraphQLView(schema, graphiql=graphiql, graphql_ide=graphql_ide, allow_queries_via_get=allow_queries_via_get, keep_alive=False)
    ...
    async def _graphql_request(self, method: Literal['get', 'post'], query: Optional[str]=None, variables: Optional[Dict[str, object]]=None, files: Optional[Dict[str, BytesIO]]=None, headers: Optional[Dict[str, str]]=None, **kwargs: Any) -> Response:
        body = self._build_body(query=query, variables=variables, files=files, method=method)
        if method == 'get':
            kwargs['params'] = body
        elif body:
            if files:
        ...
        if files is not None:
            kwargs['files'] = files
        response = getattr(self.client, method)('/graphql', headers=self._get_headers(method=method, headers=headers, files=files), **kwargs)
        return Response(status_code=response.status_code, data=response.content, headers=response.headers)

    async def request(self, url: str, method: Literal['get', 'post', 'patch', 'put', 'delete'], headers: Optional[Dict[str, str]]=None) -> Response:
    ...
    async def post(self, url: str, data: Optional[bytes]=None, json: Optional[JSON]=None, headers: Optional[Dict[str, str]]=None) -> Response:
        response = self.client.post(url, headers=headers, content=data, json=json)
        return Response(status_code=response.status_code, data=response.content, headers=response.headers)

    @contextlib.asynccontextmanager
    async def ws_connect(self, url: str, *, protocols: List[str]) -> AsyncGenerator[WebSocketClient, None]:
        try:
