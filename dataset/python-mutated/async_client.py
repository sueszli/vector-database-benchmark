...
class AsyncClient(base_client.BaseClient):
    ...
    async def connect(self, url, headers={}, auth=None, transports=None, namespaces=None, socketio_path='socket.io', wait=True, wait_timeout=1):
        ...
        if namespaces is None:
            namespaces = list(set(self.handlers.keys()).union(set(self.namespace_handlers.keys())))
            if len(namespaces) == 0:
                namespaces = ['/']
        ...
        try:
            await self.eio.connect(real_url, headers=real_headers, transports=transports, engineio_path=socketio_path)
        except engineio.exceptions.ConnectionError as exc:
            for n in self.connection_namespaces:
                await self._trigger_event('connect_error', n, exc.args[1] if len(exc.args) > 1 else exc.args[0])
        ...
        if wait:
            ...
            if set(self.namespaces) != set(self.connection_namespaces):
                ...
                raise exceptions.ConnectionError('One or more namespaces failed to connect')
        self.connected = True
    ...
    async def emit(self, event, data=None, namespace=None, callback=None):
        ...
        if namespace not in self.namespaces:
            raise exceptions.BadNamespaceError(namespace + ' is not a connected namespace.')
        ...
        if isinstance(data, tuple):
            ...
            data = []
        await self._send_packet(self.packet_class(packet.EVENT, namespace=namespace, data=[event] + data, id=id))
    ...
    async def call(self, event, data=None, namespace=None, timeout=60):
        ...
        return callback_args[0] if len(callback_args[0]) > 1 else callback_args[0][0] if len(callback_args[0]) == 1 else None
    ...
    async def _handle_event(self, namespace, id, data):
        ...
        if id is not None:
            if r is None:
                ...
                data = [r]
            await self._send_packet(self.packet_class(packet.ACK, namespace=namespace, id=id, data=data))
    ...
    async def _handle_reconnect(self):
        ...
        while True:
            ...
            self.logger.info('Connection failed, new attempt in {:.02f} seconds'.format(delay))
            try:
            ...
            attempt_count += 1
            try:
                await self.connect(self.connection_url, headers=self.connection_headers, auth=self.connection_auth, transports=self.connection_transports, namespaces=self.connection_namespaces, socketio_path=self.socketio_path)
    ...
    async def _handle_eio_connect(self):
        ...
        for n in self.connection_namespaces:
            await self._send_packet(self.packet_class(packet.CONNECT, data=real_auth, namespace=n))
