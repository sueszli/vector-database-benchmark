...
class MockManager:
    ...
    def __init__(self, stub_runner, scenario_data, input_mocker):
        ...
        self.table_schema = {'allColumns': [{'name': 'test-column', 'type': 'text'}], 'partitionKeys': [{'name': 'test-column'}]}
        self.tables = [{'keyspaceName': self.scenario_data.scenario.ks_wrapper.ks_name, 'tableName': f'table-{ind}', 'resourceArn': self.table_arn} for ind in range(1, 4)]
    ...
    def setup_stubs(self, error, stop_on, stubber):
        ...
        with self.stub_runner(error, stop_on) as runner:
            if self.table_exists:
                runner.add(stubber.stub_get_table, ks_name, self.table_name, self.table_status, self.table_arn, schema=self.table_schema)
            else:
                runner.add(stubber.stub_get_table, ks_name, self.table_name, self.table_status, self.table_arn, error_code='ResourceNotFoundException')
...
@pytest.mark.parametrize('error, stop_on_index', [('TESTERROR-stub_get_table', 0), ('TESTERROR-stub_create_table', 1), ('TESTERROR-stub_get_table', 2), ('TESTERROR-stub_list_tables', 3)])
