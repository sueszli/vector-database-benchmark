...
from ....core.descriptions import ADDED_IN_38, ADDED_IN_310, DEPRECATED_IN_3X_INPUT, RICH_CONTENT
...
class ProductInput(BaseInputObjectType):
    ...
    charge_taxes = graphene.Boolean(description=f'Determine if taxes are being charged for the product. {DEPRECATED_IN_3X_INPUT} Use `Channel.taxConfiguration` to configure whether tax collection is enabled.')
    ...
    tax_code = graphene.String(description=f'Tax rate for enabled tax gateway. {DEPRECATED_IN_3X_INPUT} Use tax classes to control the tax calculation for a product. If taxCode is provided, Saleor will try to find a tax class with given code (codes are stored in metadata) and assign it. If no tax class is found, it would be created and assigned.')
    ...
    metadata = NonNullList(MetadataInput, description='Fields required to update the product metadata.' + ADDED_IN_38, required=False)
    private_metadata = NonNullList(MetadataInput, description='Fields required to update the product private metadata.' + ADDED_IN_38, required=False)
    external_reference = graphene.String(description='External ID of this product.' + ADDED_IN_310, required=False)
...
class StockInput(BaseInputObjectType):
    ...
    quantity = graphene.Int(required=True, description='Quantity of items available for sell.')
...
class ProductCreate(ModelMutation):
    ...
    class Arguments:
        input = ProductCreateInput(required=True, description='Fields required to create a product.')
