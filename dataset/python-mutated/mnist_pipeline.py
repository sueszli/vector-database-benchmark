...
if __name__ == '__main__':
    ...
    df.show()
    if args.mode == 'train':
        estimator = TFEstimator(main_fun, args).setInputMapping({'image': 'image', 'label': 'label'}).setModelDir(args.model_dir).setExportDir(args.export_dir).setClusterSize(args.cluster_size).setTensorboard(args.tensorboard).setEpochs(args.epochs).setBatchSize(args.batch_size).setGraceSecs(60)
        model = estimator.fit(df)
    else:
        model = TFModel(args).setInputMapping({'image': 'conv2d_input'}).setOutputMapping({'logits': 'prediction'}).setSignatureDefKey('serving_default').setExportDir(args.export_dir).setBatchSize(args.batch_size)
