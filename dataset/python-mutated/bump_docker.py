...
for file in (DOCKERFILE_BASE, DOCKERFILE_LATEXPDF):
    ...
    content = re.sub(f'{re.escape(OPENCONTAINERS_VERSION_PREFIX)} = "{VERSION_PATTERN}"', f'{OPENCONTAINERS_VERSION_PREFIX} = "{VERSION}"', content)
    content = re.sub(f'{re.escape(SPHINX_VERSION_PREFIX)}{VERSION_PATTERN}', f'{SPHINX_VERSION_PREFIX}{VERSION}', content)
    file.write_text(content, encoding='utf-8')

def git(*args):
    if False:
        return 10
    ret = subprocess.run(('git', *args), capture_output=True, cwd=DOCKER_ROOT, check=True, text=True, encoding='utf-8')
    print(ret.stdout)
