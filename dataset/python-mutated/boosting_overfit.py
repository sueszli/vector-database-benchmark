...
class PartialBoostingModel:
    ...
    _UNSUPPORTED_MODEL_ERROR = 'Check is relevant for Boosting models of type {supported_models}, but received model of type {model_type}'
    _NO_MODEL_ERROR = 'Check is relevant only when receiving the model, but predictions/probabilities were received instead. In order to use this check, please pass the model to the run() method.'
    _SUPPORTED_CLASSIFICATION_MODELS = ('AdaBoostClassifier', 'GradientBoostingClassifier', 'LGBMClassifier', 'XGBClassifier', 'CatBoostClassifier')
    _SUPPORTED_REGRESSION_MODELS = ('AdaBoostRegressor', 'GradientBoostingRegressor', 'LGBMRegressor', 'XGBRegressor', 'CatBoostRegressor')
