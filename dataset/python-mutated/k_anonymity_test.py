...
def test_k_anonymity_analysis_single_field(topic_id: str, subscription_id: str, bigquery_project: str, capsys: pytest.CaptureFixture) -> None:
    if False:
        i = 10
        return i + 15
    risk.k_anonymity_analysis(GCLOUD_PROJECT, TABLE_PROJECT, BIGQUERY_DATASET_ID, BIGQUERY_HARMFUL_TABLE_ID, topic_id, subscription_id, [NUMERIC_FIELD])
...
@pytest.mark.flaky(max_runs=3, min_passes=1)
def test_k_anonymity_analysis_multiple_fields(topic_id: str, subscription_id: str, bigquery_project: str, capsys: pytest.CaptureFixture) -> None:
    if False:
        print('Hello World!')
    risk.k_anonymity_analysis(GCLOUD_PROJECT, TABLE_PROJECT, BIGQUERY_DATASET_ID, BIGQUERY_HARMFUL_TABLE_ID, topic_id, subscription_id, [NUMERIC_FIELD, REPEATED_FIELD])
    (out, _) = capsys.readouterr()
    assert 'Quasi-ID values:' in out
    assert 'Class size:' in out
    assert 'Job name:' in out
