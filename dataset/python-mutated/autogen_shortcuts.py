...
HUMAN_NAMES_FOR_FILTERS = {filter_: name for (name, filter_) in KEYBINDING_FILTERS.items()}
...
if __name__ == '__main__':
    ...
    with (dest / 'table.tsv').open('w', encoding='utf-8') as csv:
        for binding in sorted(bindings, key=sort_key):
            sequence = ', '.join([format_prompt_keys(keys) for keys in binding.shortcut.keys_sequence])
            if binding.shortcut.filter == 'always':
                condition_label = '-'
            else:
                condition_label = 'ⓘ'
            csv.write('\t'.join([sequence, sentencize(binding.handler.description) + f' :raw-html:`<br>` `{binding.handler.identifier}`', f':raw-html:`<span title="{html_escape(binding.shortcut.filter)}" style="cursor: help">{condition_label}</span>`']) + '\n')