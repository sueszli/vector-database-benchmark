...
class App:
    ...
    async def start(self):
        ...
        self.process = await asyncio.create_subprocess_exec(str(python), '-m', 'uvicorn', '--port', str(port), '--app-dir', str(self.dir), '--reload', f'app:main', env=dict(H2O_WAVE_APP_ADDRESS=f'http://{_localhost}:{port}'))
