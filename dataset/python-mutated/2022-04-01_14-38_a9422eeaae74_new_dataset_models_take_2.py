...
user_table = sa.Table('ab_user', Base.metadata, sa.Column('id', sa.Integer(), primary_key=True))

class UUIDMixin:
    uuid = sa.Column(UUIDType(binary=True), primary_key=False, unique=True, default=uuid4)
...
class SqlMetric(AuxiliaryColumnsMixin, Base):
    ...
    verbose_name = sa.Column(sa.String(1024))
sqlatable_user_table = sa.Table('sqlatable_user', Base.metadata, sa.Column('id', sa.Integer, primary_key=True), sa.Column('user_id', sa.Integer, sa.ForeignKey('ab_user.id')), sa.Column('table_id', sa.Integer, sa.ForeignKey('tables.id')))
...
table_column_association_table = sa.Table('sl_table_columns', Base.metadata, sa.Column('table_id', sa.ForeignKey('sl_tables.id'), primary_key=True), sa.Column('column_id', sa.ForeignKey('sl_columns.id'), primary_key=True))
dataset_column_association_table = sa.Table('sl_dataset_columns', Base.metadata, sa.Column('dataset_id', sa.ForeignKey('sl_datasets.id'), primary_key=True), sa.Column('column_id', sa.ForeignKey('sl_columns.id'), primary_key=True))
dataset_table_association_table = sa.Table('sl_dataset_tables', Base.metadata, sa.Column('dataset_id', sa.ForeignKey('sl_datasets.id'), primary_key=True), sa.Column('table_id', sa.ForeignKey('sl_tables.id'), primary_key=True))
dataset_user_association_table = sa.Table('sl_dataset_users', Base.metadata, sa.Column('dataset_id', sa.ForeignKey('sl_datasets.id'), primary_key=True), sa.Column('user_id', sa.ForeignKey('ab_user.id'), primary_key=True))
...
class NewTable(AuxiliaryColumnsMixin, Base):
    ...
    external_url = sa.Column(sa.Text, nullable=True)
    extra_json = sa.Column(MediumText(), default='{}')
    database: Database = relationship('Database', backref=backref('new_tables', cascade='all, delete-orphan'), foreign_keys=[database_id])
...
active_table_columns = sa.join(TableColumn, SqlaTable, TableColumn.table_id == SqlaTable.id)
...
new_tables: sa.Table = [NewTable.__table__, NewDataset.__table__, NewColumn.__table__, table_column_association_table, dataset_column_association_table, dataset_table_association_table, dataset_user_association_table]
