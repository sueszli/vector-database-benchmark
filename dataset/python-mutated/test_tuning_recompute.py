...
class TestRecomputePassTuning(unittest.TestCase):
    ...
    def test_recompute_pass(self):
        ...
        gpu_memory_size = round(paddle.device.cuda.get_device_properties(0).total_memory / 1024 / 1024 / 1024)
        dist_strategy = engine._dist_contexts['train'].strategy
        if gpu_memory_size in [16, 32]:
            self.assertGreater(len(dist_strategy.recompute.no_recompute_segments), 0, 'When GPU memory size is 16G or 32G, the length of no_recompute_segments should be greater than 0.')
        elif gpu_memory_size >= 40:
            self.assertEqual(dist_strategy.recompute.enable, False, 'When GPU memory size is greater than 40GB, the recompute strategy should be disable.')
