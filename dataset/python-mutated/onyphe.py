...
class SearchOnyphe:
    ...
    async def do_search(self) -> None:
        ...
        headers = {'User-Agent': Core.get_user_agent(), 'Content-Type': 'application/json', 'Authorization': f'bearer {self.key}'}
                ...
                for result in self.response['results']:
                    try:
                        ...
                        if 'url' in result.keys() and isinstance(result['url'], list):
                            self.totalhosts.update((urlparse(url).netloc for url in result['url'] if urlparse(url).netloc.endswith(self.word)))
                        ...
                        for subdomain_key in ['domain', 'hostname', 'subdomains', 'subject', 'reverse', 'geolocus']:
                            if subdomain_key in result.keys():
                                if subdomain_key == 'subject':
                                    self.totalhosts.update({domain for domain in result[subdomain_key]['altname'] if domain.endswith(self.word)})
                                elif subdomain_key == 'geolocus':
                                    self.totalhosts.update({domain for domain in result[subdomain_key]['domain'] if domain.endswith(self.word)})
                                else:
                                    self.totalhosts.update({domain for domain in result[subdomain_key] if domain.endswith(self.word)})
