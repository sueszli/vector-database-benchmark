...
class AsyncSessionTest(AsyncFixture):
    ...
    async def test_sequence_execute(self, async_session: AsyncSession, metadata, use_scalar):
        ...
        if use_scalar:
            eq_(await async_session.scalar(seq), sync_connection.dialect.default_sequence_base)
        else:
            with expect_deprecated('Using the .execute\\(\\) method to invoke a DefaultGenerator object is deprecated; please use the .scalar\\(\\) method.'):
                eq_(await async_session.execute(seq), sync_connection.dialect.default_sequence_base)
    ...
    async def test_session_close_all_deprecated(self, async_engine):
        ...
        in_(u1, s1)
        in_(u2, s2)
        with expect_deprecated('The AsyncSession.close_all\\(\\) method is deprecated and will be removed in a future release. '):
...
class AsyncSessionQueryTest(AsyncFixture):
    ...
    @async_test
    @testing.combinations({}, dict(execution_options={'logging_token': 'test'}), argnames='kw')
    ...
    async def test_get_one(self, async_session):
        ...
        eq_(u1.name, 'jack')
        eq_(u2.name, 'chuck')
        with testing.expect_raises_message(exc.NoResultFound, 'No row was found when one was required'):
    ...
    async def test_stream_partitions(self, async_session, kw):
        ...
        eq_(assert_result, [self.static.user_address_result[0:3], self.static.user_address_result[3:]])
    ...
    async def test_no_ss_cursor_w_execute(self, async_session, location):
        ...
        with expect_raises_message(async_exc.AsyncMethodRequired, "Can't use the AsyncSession.execute\\(\\) method with a server-side cursor. Use the AsyncSession.stream\\(\\) method for an async streaming result set."):
            if location == 'execute':
                await async_session.execute(stmt, execution_options={'stream_results': True})
            else:
...
class AsyncSessionTransactionTest(AsyncFixture):
    ...
    @async_test
    async def test_interrupt_ctxmanager_connection(self, async_trans_ctx_manager_fixture, async_session):
    ...
    async def test_refresh(self, async_session):
        ...
        async with async_session.begin():
            ...
            await async_session.flush()
            conn = await async_session.connection()
            await conn.execute(update(User).values(name='u2').execution_options(synchronize_session=None))
            eq_(u1.name, 'u1')
    ...
    async def test_merge_loader_options(self, async_session):
        ...
        async with async_session.begin():
            ...
            new_u_merged = await async_session.merge(new_u1, options=[selectinload(User.addresses)])
...
class AsyncCascadesTest(AsyncFixture):
    ...
    async def test_delete_w_cascade(self, async_session):
        ...
        async with async_session.begin():
            ...
            await async_session.delete(u1)
        eq_((await async_session.execute(select(func.count()).select_from(Address))).scalar(), 0)
...
class AsyncORMBehaviorsTest(AsyncFixture):
    ...
    @testing.combinations(('legacy_style', True), ('new_style', False), argnames='_legacy_inactive_history_style', id_='ia')
    @async_test
    async def test_new_style_active_history(self, async_session, one_to_one_fixture, _legacy_inactive_history_style):
        ...
        if _legacy_inactive_history_style:
            ...
            eq_((await async_session.execute(select(func.count()).where(B.id == b1.id).where(B.a_id == None))).scalar(), 1)
...
class AsyncEventTest(AsyncFixture):
    ...
    async def test_no_async_listeners(self, async_session):
        with testing.expect_raises_message(NotImplementedError, 'asynchronous events are not implemented at this time.  Apply synchronous listeners to the AsyncSession.sync_session.'):
    ...
    async def test_sync_before_commit(self, async_session):
        ...
        async with async_session.begin():
            pass
        eq_(canary.mock_calls, [mock.call(async_session.sync_session)])
...
class AsyncProxyTest(AsyncFixture):
    ...
    async def test_get_connection_kws(self, async_session):
        c1 = await async_session.connection(execution_options={'isolation_level': 'AUTOCOMMIT'})
        eq_(c1.sync_connection._execution_options, {'isolation_level': 'AUTOCOMMIT'})
