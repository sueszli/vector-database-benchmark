...
from .exceptions import ConfigEntryAuthFailed, ConfigEntryError, ConfigEntryNotReady, HomeAssistantError
from .helpers import device_registry, entity_registry, storage
from .helpers.debounce import Debouncer
from .helpers.dispatcher import async_dispatcher_send
from .helpers.event import RANDOM_MICROSECOND_MAX, RANDOM_MICROSECOND_MIN, async_call_later
...
DISCOVERY_SOURCES = {SOURCE_BLUETOOTH, SOURCE_DHCP, SOURCE_DISCOVERY, SOURCE_HOMEKIT, SOURCE_IMPORT, SOURCE_INTEGRATION_DISCOVERY, SOURCE_MQTT, SOURCE_SSDP, SOURCE_UNIGNORE, SOURCE_USB, SOURCE_ZEROCONF}
...
class ConfigEntry:
    ...
    __slots__ = ('entry_id', 'version', 'domain', 'title', 'data', 'options', 'unique_id', 'supports_unload', 'supports_remove_device', 'pref_disable_new_entities', 'pref_disable_polling', 'source', 'state', 'disabled_by', '_setup_lock', 'update_listeners', 'reason', '_async_cancel_retry_setup', '_on_unload', 'reload_lock', '_reauth_lock', '_tasks', '_background_tasks', '_integration_for_domain', '_tries', '_setup_again_job')
    ...
    async def async_setup(self, hass: HomeAssistant, *, integration: loader.Integration | None=None) -> None:
        ...
        if self.supports_remove_device is None:
            self.supports_remove_device = await support_remove_from_device(hass, self.domain)
        try:
            component = integration.get_component()
        except ImportError as err:
            _LOGGER.error('Error importing integration %s to set up %s configuration entry: %s', integration.domain, self.domain, err)
            if self.domain == integration.domain:
                self._async_set_state(hass, ConfigEntryState.SETUP_ERROR, 'Import error')
            return
        if self.domain == integration.domain:
            try:
                integration.get_platform('config_flow')
            except ImportError as err:
                _LOGGER.error('Error importing platform config_flow from integration %s to set up %s configuration entry: %s', integration.domain, self.domain, err)
                self._async_set_state(hass, ConfigEntryState.SETUP_ERROR, 'Import error')
                return
        ...
        try:
            ...
            if not isinstance(result, bool):
                _LOGGER.error('%s.async_setup_entry did not return boolean', integration.domain)
                result = False
        except ConfigEntryError as ex:
            error_reason = str(ex) or 'Unknown fatal config entry error'
            _LOGGER.exception('Error setting up entry %s for %s: %s', self.title, self.domain, error_reason)
            ...
            auth_message = f'{auth_base_message}: {message}' if message else auth_base_message
            _LOGGER.warning("Config entry '%s' for %s integration %s", self.title, self.domain, auth_message)
            await self._async_process_on_unload(hass)
            ...
            _LOGGER.debug("Config entry '%s' for %s integration not %s; Retrying in %d seconds", self.title, self.domain, ready_message, wait_time)
            if hass.state == CoreState.running:
                self._async_cancel_retry_setup = async_call_later(hass, wait_time, self._async_get_setup_again_job(hass))
            else:
                self._async_cancel_retry_setup = hass.bus.async_listen_once(EVENT_HOMEASSISTANT_STARTED, functools.partial(self._async_setup_again, hass))
            await self._async_process_on_unload(hass)
            return
        except (asyncio.CancelledError, SystemExit, Exception):
            _LOGGER.exception('Error setting up entry %s for %s', self.title, integration.domain)
            result = False
        if self.domain != integration.domain:
            return
        if result:
    ...
    async def async_unload(self, hass: HomeAssistant, *, integration: loader.Integration | None=None) -> bool:
        ...
        if not supports_unload:
            if integration.domain == self.domain:
                self._async_set_state(hass, ConfigEntryState.FAILED_UNLOAD, 'Unload not supported')
            return False
        try:
            ...
            _LOGGER.exception('Error unloading entry %s for %s', self.title, integration.domain)
            if integration.domain == self.domain:
                self._async_set_state(hass, ConfigEntryState.FAILED_UNLOAD, str(ex) or 'Unknown error')
            return False
    ...
    async def async_remove(self, hass: HomeAssistant) -> None:
        ...
        try:
            ...
            _LOGGER.exception('Error calling entry remove callback %s for %s', self.title, integration.domain)
    ...
    async def async_migrate(self, hass: HomeAssistant) -> bool:
        ...
        if (handler := HANDLERS.get(self.domain)) is None:
            _LOGGER.error('Flow handler not found for entry %s for %s', self.title, self.domain)
            return False
        ...
        if not supports_migrate:
            _LOGGER.error('Migration handler not found for entry %s for %s', self.title, self.domain)
            return False
        try:
            result = await component.async_migrate_entry(hass, self)
            if not isinstance(result, bool):
                _LOGGER.error('%s.async_migrate_entry did not return boolean', self.domain)
                return False
            if result:
                hass.config_entries._async_schedule_save()
            return result
        except Exception:
            _LOGGER.exception('Error migrating entry %s for %s', self.title, self.domain)
            return False
    ...
    async def _async_process_on_unload(self, hass: HomeAssistant) -> None:
        ...
        for task in pending:
            _LOGGER.warning('Unloading %s (%s) config entry. Task %s did not complete in time', self.title, self.domain, task)
    ...
    async def _async_init_reauth(self, hass: HomeAssistant, context: dict[str, Any] | None=None, data: dict[str, Any] | None=None) -> None:
        ...
        async with self._reauth_lock:
            if any(self.async_get_active_flows(hass, {SOURCE_REAUTH})):
                return
            await hass.config_entries.flow.async_init(self.domain, context={'source': SOURCE_REAUTH, 'entry_id': self.entry_id, 'title_placeholders': {'name': self.title}, 'unique_id': self.unique_id} | (context or {}), data=self.data | (data or {}))
    ...
    def async_create_background_task(self, hass: HomeAssistant, target: Coroutine[Any, Any, _R], name: str) -> asyncio.Task[_R]:
        ...
        return task
current_entry: ContextVar[ConfigEntry | None] = ContextVar('current_entry', default=None)
...
class ConfigEntriesFlowManager(data_entry_flow.FlowManager):
    ...
    async def async_init(self, handler: str, *, context: dict[str, Any] | None=None, data: Any=None) -> FlowResult:
        ...
        task = asyncio.create_task(self._async_init(flow_id, handler, context, data), name=f'config entry flow {handler} {flow_id}')
    ...
    async def async_shutdown(self) -> None:
        ...
        for task_list in self._initialize_tasks.values():
            for task in task_list:
                task.cancel('Config entry initialize canceled: Home Assistant is shutting down')
    ...
    async def async_finish_flow(self, flow: data_entry_flow.FlowHandler, result: data_entry_flow.FlowResult) -> data_entry_flow.FlowResult:
        ...
        for progress_flow in self.async_progress_by_handler(flow.handler):
            ...
            if progress_flow['flow_id'] != flow.flow_id and (flow.unique_id and progress_unique_id == flow.unique_id or progress_unique_id == DEFAULT_DISCOVERY_UNIQUE_ID):
        ...
        entry = ConfigEntry(version=result['version'], domain=result['handler'], title=result['title'], data=result['data'], options=result['options'], source=flow.context['source'], unique_id=flow.unique_id)
    ...
    async def async_create_flow(self, handler_key: str, *, context: dict | None=None, data: Any=None) -> ConfigFlow:
        ...
        """
        handler = await _async_get_flow_handler(self.hass, handler_key, self._hass_config)
        if not context or 'source' not in context:
            raise KeyError("Context not set or doesn't have a source set")
        flow = handler()
    ...
    async def async_post_init(self, flow: data_entry_flow.FlowHandler, result: data_entry_flow.FlowResult) -> None:
        ...
        if source in DISCOVERY_SOURCES:
            ...
            persistent_notification.async_create(self.hass, title='Integration requires reconfiguration', message='At least one of your integrations requires reconfiguration to continue functioning. [Check it out](/config/integrations).', notification_id=RECONFIGURE_NOTIFICATION_ID)
...
class ConfigEntries:
    ...
    async def async_add(self, entry: ConfigEntry) -> None:
        ...
        if entry.entry_id in self._entries:
            raise HomeAssistantError(f'An entry with the id {entry.entry_id} already exists.')
    ...
    async def async_remove(self, entry_id: str) -> dict[str, Any]:
        ...
        for progress_flow in self.hass.config_entries.flow.async_progress_by_handler(entry.domain, match_context={'entry_id': entry_id, 'source': SOURCE_REAUTH}):
            if 'flow_id' in progress_flow:
                self.hass.config_entries.flow.async_abort(progress_flow['flow_id'])
        if entry.source == SOURCE_IGNORE:
            self.hass.async_create_task(self.hass.config_entries.flow.async_init(entry.domain, context={'source': SOURCE_UNIGNORE}, data={'unique_id': entry.unique_id}), f'config entry unignore {entry.title} {entry.domain} {entry.unique_id}')
        self._async_dispatch(ConfigEntryChange.REMOVED, entry)
        return {'require_restart': not unload_success}

    async def _async_shutdown(self, event: Event) -> None:
        """Call when Home Assistant is stopping."""
        await asyncio.gather(*(asyncio.create_task(entry.async_shutdown(), name=f'config entry shutdown {entry.title} {entry.domain} {entry.entry_id}') for entry in self._entries.values()))
        await self.flow.async_shutdown()

    async def async_initialize(self) -> None:
        """Initialize config entry config."""
        config = await storage.async_migrator(self.hass, self.hass.config.path(PATH_CONFIG), self._store, old_conf_migrate_func=_old_conf_migrator)
        ...
        for entry in config['entries']:
            ...
            if pref_disable_new_entities is None and 'system_options' in entry:
                pref_disable_new_entities = entry.get('system_options', {}).get('disable_new_entities')
            domain = entry['domain']
            entry_id = entry['entry_id']
            config_entry = ConfigEntry(version=entry['version'], domain=domain, entry_id=entry_id, data=entry['data'], source=entry['source'], title=entry['title'], options=entry.get('options'), unique_id=entry.get('unique_id'), disabled_by=ConfigEntryDisabler(entry['disabled_by']) if entry.get('disabled_by') else None, pref_disable_new_entities=pref_disable_new_entities, pref_disable_polling=entry.get('pref_disable_polling'))
    ...
    async def async_setup(self, entry_id: str) -> bool:
        ...
        if entry.state is not ConfigEntryState.NOT_LOADED:
            raise OperationNotAllowed(f'The config entry {entry.title} ({entry.domain}) with entry_id {entry.entry_id} cannot be setup because is already loaded in the {entry.state} state')
        if entry.domain in self.hass.config.components:
            await entry.async_setup(self.hass)
        else:
            result = await async_setup_component(self.hass, entry.domain, self._hass_config)
            if not result:
                return result
    ...
    async def async_unload(self, entry_id: str) -> bool:
        ...
        if (entry := self.async_get_entry(entry_id)) is None:
            raise UnknownEntry
        if not entry.state.recoverable:
            raise OperationNotAllowed(f'The config entry {entry.title} ({entry.domain}) with entry_id {entry.entry_id} cannot be unloaded because it is not in a recoverable state ({entry.state})')
    ...
    async def async_forward_entry_setups(self, entry: ConfigEntry, platforms: Iterable[Platform | str]) -> None:
        ...
        await asyncio.gather(*(asyncio.create_task(self.async_forward_entry_setup(entry, platform), name=f'config entry forward setup {entry.title} {entry.domain} {entry.entry_id} {platform}') for platform in platforms))
    ...
    async def async_unload_platforms(self, entry: ConfigEntry, platforms: Iterable[Platform | str]) -> bool:
        ...
        return all(await asyncio.gather(*(asyncio.create_task(self.async_forward_entry_unload(entry, platform), name=f'config entry forward unload {entry.title} {entry.domain} {entry.entry_id} {platform}') for platform in platforms)))
...
class ConfigFlow(data_entry_flow.FlowHandler):
    ...
    async def async_set_unique_id(self, unique_id: str | None=None, *, raise_on_progress: bool=True) -> ConfigEntry | None:
        ...
        if raise_on_progress:
            if self._async_in_progress(include_uninitialized=True, match_context={'unique_id': unique_id}):
                raise data_entry_flow.AbortFlow('already_in_progress')
        self.context['unique_id'] = unique_id
        if unique_id != DEFAULT_DISCOVERY_UNIQUE_ID:
            for progress in self._async_in_progress(include_uninitialized=True, match_context={'unique_id': DEFAULT_DISCOVERY_UNIQUE_ID}):
...
class EntityRegistryDisabledHandler:
    ...
    async def _handle_entry_updated(self, event: Event) -> None:
        ...
        if entity_entry is None or entity_entry.config_entry_id is None or entity_entry.disabled_by:
            return
        ...
        self._remove_call_later = async_call_later(self.hass, RELOAD_AFTER_UPDATE_DELAY, HassJob(self._handle_reload, cancel_on_shutdown=True))
    ...
    async def _handle_reload(self, _now: Any) -> None:
        ...
        self.changed = set()
        _LOGGER.info('Reloading configuration entries because disabled_by changed in entity registry: %s', ', '.join(to_reload))
        await asyncio.gather(*(asyncio.create_task(self.hass.config_entries.async_reload(entry_id), name='config entry reload {entry.title} {entry.domain} {entry.entry_id}') for entry_id in to_reload))
...
async def _load_integration(hass: HomeAssistant, domain: str, hass_config: ConfigType) -> None:
    ...
    try:
        ...
        _LOGGER.error('Error occurred loading flow for integration %s: %s', domain, err)
        raise data_entry_flow.UnknownHandler
...
async def _async_get_flow_handler(hass: HomeAssistant, domain: str, hass_config: ConfigType) -> type[ConfigFlow]:
    ...
    if loader.is_component_module_loaded(hass, f'{domain}.config_flow') and (handler := HANDLERS.get(domain)):
        return handler
