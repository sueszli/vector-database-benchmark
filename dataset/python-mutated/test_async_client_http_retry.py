...
class TestAsyncAuditLogsClient_HttpRetries(unittest.TestCase):
    ...
    async def test_http_retries(self):
        ...
        client = AsyncAuditLogsClient(token='xoxp-remote_disconnected', base_url='http://localhost:8888/', retry_handlers=[retry_handler])
        try:
    ...
    async def test_ratelimited(self):
        client = AsyncAuditLogsClient(token='xoxp-ratelimited', base_url='http://localhost:8888/', retry_handlers=[AsyncRateLimitErrorRetryHandler()])
