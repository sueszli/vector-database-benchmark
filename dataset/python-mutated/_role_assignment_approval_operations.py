...
def build_get_by_id_request(approval_id: str, **kwargs: Any) -> HttpRequest:
    ...
    path_format_arguments = {'approvalId': _SERIALIZER.url('approval_id', approval_id, 'str')}
...
class RoleAssignmentApprovalOperations:
    ...
    def get_by_id(self, approval_id: str, **kwargs: Any) -> _models.RoleAssignmentApproval:
        ...
        request = build_get_by_id_request(approval_id=approval_id, api_version=api_version, template_url=self.get_by_id.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = self._client._pipeline.run(request, stream=_stream, **kwargs)
