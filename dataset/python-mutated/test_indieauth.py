...
async def test_verify_redirect_uri() -> None:
    ...
    assert await indieauth.verify_redirect_uri(None, 'http://ex.com', 'http://ex.com/callback')
    with patch.object(indieauth, 'fetch_redirect_uris', return_value=[]):
        assert not await indieauth.verify_redirect_uri(None, 'http://ex.com', 'http://different.com/callback')
        assert not await indieauth.verify_redirect_uri(None, 'http://ex.com', 'https://ex.com/callback')
        assert not await indieauth.verify_redirect_uri(None, 'https://sub1.ex.com', 'https://sub2.ex.com/callback')
...
async def test_find_link_tag_max_size(hass: HomeAssistant, mock_session) -> None:
    """Test finding link tag."""
    text = ''.join(['<link rel="redirect_uri" href="/wine">', '0' * 1024 * 10, '<link rel="redirect_uri" href="/beer">'])
    mock_session.get('http://127.0.0.1:8000', text=text)
    redirect_uris = await indieauth.fetch_redirect_uris(hass, 'http://127.0.0.1:8000')
    assert redirect_uris == ['http://127.0.0.1:8000/wine']

@pytest.mark.parametrize('client_id', ['https://home-assistant.io/android', 'https://home-assistant.io/iOS'])
async def test_verify_redirect_uri_android_ios(client_id) -> None:
    """Test that we verify redirect uri correctly for Android/iOS."""
    with patch.object(indieauth, 'fetch_redirect_uris', return_value=[]):
        assert await indieauth.verify_redirect_uri(None, client_id, 'homeassistant://auth-callback')
        assert not await indieauth.verify_redirect_uri(None, client_id, 'homeassistant://something-else')
        assert not await indieauth.verify_redirect_uri(None, 'https://incorrect.com', 'homeassistant://auth-callback')
        if client_id == 'https://home-assistant.io/android':
            assert await indieauth.verify_redirect_uri(None, client_id, 'https://wear.googleapis.com/3p_auth/io.homeassistant.companion.android')
            assert await indieauth.verify_redirect_uri(None, client_id, 'https://wear.googleapis-cn.com/3p_auth/io.homeassistant.companion.android')
        else:
            assert not await indieauth.verify_redirect_uri(None, client_id, 'https://wear.googleapis.com/3p_auth/io.homeassistant.companion.android')
            assert not await indieauth.verify_redirect_uri(None, client_id, 'https://wear.googleapis-cn.com/3p_auth/io.homeassistant.companion.android')