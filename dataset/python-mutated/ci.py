...
def main(argv: List[str]) -> None:
    ...
    work_orders = tuple((WorkOrder(label, autolabels, timer_args, timeout=TIMEOUT, retries=RETRIES) for (label, autolabels, timer_args) in benchmarks * REPEATS))
    ...
    grouped_results: Dict[str, Dict[str, List[Union[float, int]]]] = {key: {'times': [], 'counts': []} for key in keys}
    ...
    final_results = {'version': version, 'md5': md5.hexdigest(), 'start_time': t0, 'end_time': int(time.time()), 'values': grouped_results}
    if args.destination:
