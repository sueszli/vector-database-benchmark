...
class SearchTomba:
    ...
    async def do_search(self) -> None:
        ...
        is_free = is_free if 'name' in response[0]['data']['pricing'].keys() and response[0]['data']['pricing']['name'].lower() == 'free' else False
        total_requests_avail = response[0]['data']['requests']['domains']['available'] - response[0]['data']['requests']['domains']['used']
        if is_free:
            ...
            if total_requests_avail < total_number_reqs:
                ...
                print('RETURNING current results, If you still wish to run this module despite the current results, please comment out the "if request" line.')
                return
            self.limit = 100
    ...
    async def parse_resp(self, json_resp):
        ...
        domains = list(sorted({source['website_url'] for email in json_resp['data']['emails'] for source in email['sources'] if self.word in source['website_url']}))
