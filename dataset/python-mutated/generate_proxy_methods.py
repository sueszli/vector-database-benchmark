...
class _repr_sym:
    ...
    def __repr__(self) -> str:
        if False:
            ...
            return i + 15
        return self.sym
classes: collections.defaultdict[str, Dict[str, Tuple[Any, ...]]] = collections.defaultdict(dict)
...
entries = ['sqlalchemy.orm.scoping', 'sqlalchemy.ext.asyncio.engine', 'sqlalchemy.ext.asyncio.session', 'sqlalchemy.ext.asyncio.scoping']
if __name__ == '__main__':
    cmd = code_writer_cmd(__file__)
    with cmd.add_arguments() as parser:
        parser.add_argument('--module', choices=entries + ['all'], default='all', help='Which file to generate. Default is to regenerate all files')
