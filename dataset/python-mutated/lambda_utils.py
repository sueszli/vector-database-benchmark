...
if TYPE_CHECKING:
    ...
    from mypy_boto3_lambda.type_defs import DeadLetterConfigTypeDef, EnvironmentTypeDef, EphemeralStorageTypeDef, FileSystemConfigTypeDef, FunctionCodeTypeDef, FunctionConfigurationResponseMetadataTypeDef, ImageConfigTypeDef, TracingConfigTypeDef, VpcConfigTypeDef
LOG = logging.getLogger(__name__)
RUNTIMES_AGGREGATED = {'python': [Runtime.python3_7, Runtime.python3_8, Runtime.python3_9, Runtime.python3_10, Runtime.python3_11], 'nodejs': [Runtime.nodejs14_x, Runtime.nodejs16_x, Runtime.nodejs18_x], 'ruby': [Runtime.ruby2_7, Runtime.ruby3_2], 'java': [Runtime.java8, Runtime.java8_al2, Runtime.java11, Runtime.java17], 'dotnet': [Runtime.dotnet6], 'go': [Runtime.go1_x], 'custom': [Runtime.provided, Runtime.provided_al2]}
HANDLERS = {**dict.fromkeys(RUNTIMES_AGGREGATED.get('python'), 'handler.handler'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('nodejs'), 'index.handler'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('ruby'), 'function.handler'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('java'), 'echo.Handler'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('custom'), 'function.handler'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('go'), 'main'), 'dotnetcore3.1': 'dotnetcore31::dotnetcore31.Function::FunctionHandler', 'dotnet6': 'dotnet6::dotnet6.Function::FunctionHandler'}
PACKAGE_FOR_RUNTIME = {**dict.fromkeys(RUNTIMES_AGGREGATED.get('python'), 'python'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('nodejs'), 'nodejs'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('ruby'), 'ruby'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('java'), 'java'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('custom'), 'provided'), **dict.fromkeys(RUNTIMES_AGGREGATED.get('go'), 'go'), 'dotnet6': 'dotnet6', 'dotnetcore3.1': 'dotnetcore3.1'}
...
lambda_role = {'Version': '2012-10-17', 'Statement': [{'Effect': 'Allow', 'Principal': {'Service': 'lambda.amazonaws.com'}, 'Action': 'sts:AssumeRole'}]}
s3_lambda_permission = {'Version': '2012-10-17', 'Statement': [{'Effect': 'Allow', 'Action': ['sqs:*', 'dynamodb:DescribeStream', 'dynamodb:GetRecords', 'dynamodb:GetShardIterator', 'dynamodb:ListStreams', 'kinesis:DescribeStream', 'kinesis:DescribeStreamSummary', 'kinesis:GetRecords', 'kinesis:GetShardIterator', 'kinesis:ListShards', 'kinesis:ListStreams', 'kinesis:SubscribeToShard', 'logs:CreateLogGroup', 'logs:CreateLogStream', 'logs:PutLogEvents'], 'Resource': ['*']}]}
