...
_intervalTypes = {'hours': lambda interval: relativedelta(hours=interval), 'days': lambda interval: relativedelta(days=interval), 'months': lambda interval: relativedelta(months=interval), 'years': lambda interval: relativedelta(years=interval)}

class MarketingCampaign(models.Model):
    _name = 'marketing.campaign'
    _description = 'Marketing Campaign'
    name = fields.Char('Name', required=True)
    object_id = fields.Many2one('ir.model', 'Resource', required=True, help='Choose the resource on which you want this campaign to be run')
    partner_field_id = fields.Many2one('ir.model.fields', 'Partner Field', domain="[('model_id', '=', object_id), ('ttype', '=', 'many2one'), ('relation', '=', 'res.partner')]", help='The generated workitems will be linked to the partner related to the record. If the record is the partner itself leave this field empty. This is useful for reporting purposes, via the Campaign Analysis or Campaign Follow-up views.')
    unique_field_id = fields.Many2one('ir.model.fields', 'Unique Field', domain="[('model_id', '=', object_id), ('ttype', 'in', ['char','int','many2one','text','selection'])]", help='If set, this field will help segments that work in "no duplicates" mode to avoid selecting similar records twice. Similar records are records that have the same value for this unique field. For example by choosing the "email_from" field for CRM Leads you would prevent sending the same campaign to the same email address again. If not set, the "no duplicates" segments will only avoid selecting the same record again if it entered the campaign previously. Only easily comparable fields like textfields, integers, selections or single relationships may be used.')
    mode = fields.Selection([('test', 'Test Directly'), ('test_realtime', 'Test in Realtime'), ('manual', 'With Manual Confirmation'), ('active', 'Normal')], 'Mode', required=True, default='test', help="Test - It creates and process all the activities directly (without waiting for the delay on transitions) but does not send emails or produce reports. \nTest in Realtime - It creates and processes all the activities directly but does not send emails or produce reports.\nWith Manual Confirmation - the campaigns runs normally, but the user has to \n validate all workitem manually.\nNormal - the campaign runs normally and automatically sends all emails and reports (be very careful with this mode, you're live!)")
    state = fields.Selection([('draft', 'New'), ('running', 'Running'), ('cancelled', 'Cancelled'), ('done', 'Done')], 'Status', copy=False, default='draft')
    activity_ids = fields.One2many('marketing.campaign.activity', 'campaign_id', 'Activities')
    fixed_cost = fields.Float('Fixed Cost', help='Fixed cost for running this campaign. You may also specify variable cost and revenue on each campaign activity. Cost and Revenue statistics are included in Campaign Reporting.', digits=dp.get_precision('Product Price'))
...
class MarketingCampaignSegment(models.Model):
    ...
    ir_filter_id = fields.Many2one('ir.filters', 'Filter', ondelete='restrict', domain=lambda self: [('model_id', '=', self.object_id._name)], help='Filter to select the matching resource records that belong to this segment. New filters can be created and saved using the advanced search on the list view of the Resource. If no filter is set, all records are selected without filtering. The synchronization mode may also add a criterion to the filter.')
    ...
    sync_mode = fields.Selection([('create_date', 'Only records created after last sync'), ('write_date', 'Only records modified after last sync (no duplicates)'), ('all', 'All records (no duplicates)')], 'Synchronization mode', default='create_date', help='Determines an additional criterion to add to the filter when selecting new records to inject in the campaign. "No duplicates" prevents selecting records which have already entered the campaign previously.If the campaign has a "unique field" set, "no duplicates" will also prevent selecting records which have the same value for the unique field as other records that already entered the campaign.')
    state = fields.Selection([('draft', 'New'), ('cancelled', 'Cancelled'), ('running', 'Running'), ('done', 'Done')], 'Status', copy=False, default='draft')
    ...
    date_next_sync = fields.Datetime(compute='_compute_date_next_sync', string='Next Synchronization', help='Next time the synchronization job is scheduled to run automatically')
...
class MarketingCampaignActivity(models.Model):
    ...
    condition = fields.Text('Condition', required=True, default='True', help='Python expression to decide whether the activity can be executed, otherwise it will be deleted or cancelled.The expression may use the following [browsable] variables:\n   - activity: the campaign activity\n   - workitem: the campaign workitem\n   - resource: the resource object this campaign item represents\n   - transitions: list of campaign transitions outgoing from this activity\n...- re: Python regular expression module')
    action_type = fields.Selection([('email', 'Email'), ('report', 'Report'), ('action', 'Custom Action')], 'Type', required=True, oldname='type', default='email', help='The type of action to execute when an item enters this activity, such as:\n- Email: send an email using a predefined email template \n- Report: print an existing Report defined on the resource item and save it into a specific directory \n- Custom Action: execute a predefined action, e.g. to modify the fields of the resource record')
    ...
    variable_cost = fields.Float('Variable Cost', digits=dp.get_precision('Product Price'), help='Set a variable cost if you consider that every campaign item that has reached this point has entailed a certain cost. You can get cost statistics in the Reporting section')
    revenue = fields.Float('Revenue', digits=0, help='Set an expected revenue if you consider that every campaign item that has reached this point has generated a certain revenue. You can get revenue statistics in the Reporting section')
    signal = fields.Char('Signal', help='An activity with a signal can be called programmatically. Be careful, the workitem is always created when a signal is sent')
...
class MarketingCampaignTransition(models.Model):
    ...
    _interval_units = [('hours', 'Hour(s)'), ('days', 'Day(s)'), ('months', 'Month(s)'), ('years', 'Year(s)')]
    ...
    trigger = fields.Selection([('auto', 'Automatic'), ('time', 'Time'), ('cosmetic', 'Cosmetic')], 'Trigger', required=True, default='time', help='How is the destination workitem triggered')
...
class MarketingCampaignWorkitem(models.Model):
    ...
    date = fields.Datetime('Execution Date', readonly=True, default=False, help='If date is not set, this workitem has to be run manually')
    partner_id = fields.Many2one('res.partner', 'Partner', index=1, readonly=True)
    state = fields.Selection([('todo', 'To Do'), ('cancelled', 'Cancelled'), ('exception', 'Exception'), ('done', 'Done')], 'Status', readonly=True, copy=False, default='todo')
