...
class FM(BaseFM):
    ...
    def _update_latents(self, x, g_loss):
        ...
        precomputed_sum = {f: sum((v[j][f] * xj for (j, xj) in x.items())) for f in range(self.n_factors)}
        gradients = {}
        for (j, xj) in x.items():
            gradients[j] = {f: g_loss * (xj * precomputed_sum[f] - v[j][f] * xj ** 2) + l1 * sign(v[j][f]) + l2 * v[j][f] for f in range(self.n_factors)}
        for j in x.keys():
