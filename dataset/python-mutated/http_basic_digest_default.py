...
class Exploit(HTTPClient):
    __info__ = {'name': 'HTTP Basic/Digest Default Creds', 'description': 'Module performs dictionary attack with default credentials against HTTP Basic/Digest Auth service. If valid credentials are found, they are displayed to the user.', 'authors': ('Marcin Bury <marcin[at]threat9.com>', 'Alexander Yakovlev <https://github.com/toxydose>'), 'devices': ('Multiple devices',)}
    ...
    def target_function(self, running, data):
        ...
        while running.is_set():
            try:
                ...
                response = self.http_request(method='GET', path=self.path, auth=auth)
    ...
    def check(self):
        if False:
            i = 10
            return i + 15
        response = self.http_request(method='GET', path=self.path)
        if response is None:
            return False
