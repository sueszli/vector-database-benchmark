...
from gradio.data_classes import Estimation, LogMessage, PredictBody, Progress, ProgressUnit
...
class Queue:
        ...
        while not self.stopped:
            ...
            if events:
                ...
                if self.live_updates:
                    broadcast_live_estimations_task = run_coro_in_background(self.broadcast_estimations)
    ...
    async def broadcast_estimations(self) -> None:
        ...
        await asyncio.gather(*[self.send_estimation(event, estimation, rank) for (rank, event) in enumerate(self.event_queue)])
    ...
    async def send_estimation(self, event: Event, estimation: Estimation, rank: int) -> Estimation:
        ...
        if self.avg_concurrent_process_time is not None:
            estimation.rank_eta = estimation.rank * self.avg_concurrent_process_time + self.avg_process_time
            if None not in self.active_jobs:
    ...
    async def call_prediction(self, events: list[Event], batch: bool):
        ...
        api_name = 'predict'
        fn_index_inferred = route_utils.infer_fn_index(app=app, api_name=api_name, body=body)
        gr_request = route_utils.compile_gr_request(app=app, body=body, fn_index_inferred=fn_index_inferred, username=username, request=None)
        try:
            output = await route_utils.call_process_api(app=app, body=body, gr_request=gr_request, fn_index_inferred=fn_index_inferred)
        ...
        http_response = actual_response_class(output)
        ...
        try:
            ...
            try:
                ...
                for event in awake_events:
                    event.send_message('process_completed', {'output': {'error': None if len(e.args) and e.args[0] is None else str(e)}, 'success': False}, final=True)
            if response and response.get('is_generating', False):
                ...
                while response and response.get('is_generating', False):
                    ...
                    old_err = err
                    for event in awake_events:
                        event.send_message('process_generating', {'output': old_response, 'success': old_response is not None})
                ...
                for event in awake_events:
                    ...
                    event.send_message('process_completed', {'output': {'error': str(relevant_response)} if isinstance(relevant_response, Exception) else relevant_response, 'success': relevant_response and (not isinstance(relevant_response, Exception))}, final=True)
            elif response:
                output = copy.deepcopy(response)
                for (e, event) in enumerate(awake_events):
                    if batch and 'data' in output:
                        output['data'] = list(zip(*response.get('data')))[e]
                    event.send_message('process_completed', {'output': output, 'success': response is not None}, final=True)
            end_time = time.time()
