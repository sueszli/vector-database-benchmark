...
conf = {'spark.network.timeout': '10000000', 'spark.sql.broadcastTimeout': '7200', 'spark.sql.shuffle.partitions': '2000', 'spark.locality.wait': '0s', 'spark.sql.crossJoin.enabled': 'true', 'spark.task.cpus': '1', 'spark.executor.heartbeatInterval': '200s', 'spark.driver.maxResultSize': '40G', 'spark.app.name': 'recsys-val-parquet'}
...
if __name__ == '__main__':
    ...
    start = time()
    val_tbl = FeatureTable.read_csv(args.input_file, delimiter='\x01', names=RecsysSchema().toColumns(), dtype=RecsysSchema().toDtype())
