...
class MaintenanceEquipmentCategory(models.Model):
    ...
    alias_id = fields.Many2one('mail.alias', 'Alias', ondelete='cascade', required=True, help='Email alias for this equipment category. New emails will automatically create new maintenance request for this equipment category.')
...
class MaintenanceEquipment(models.Model):
    ...
    category_id = fields.Many2one('maintenance.equipment.category', string='Equipment Category', track_visibility='onchange', group_expand='_read_group_category_ids')
    ...
    _sql_constraints = [('serial_no', 'unique(serial_no)', 'Another asset already exists with this serial number!')]
...
class MaintenanceRequest(models.Model):
    ...
    stage_id = fields.Many2one('maintenance.stage', string='Stage', track_visibility='onchange', group_expand='_read_group_stage_ids', default=_default_stage)
    priority = fields.Selection([('0', 'Very Low'), ('1', 'Low'), ('2', 'Normal'), ('3', 'High')], string='Priority')
    color = fields.Integer('Color Index')
    close_date = fields.Date('Close Date', help='Date the maintenance was finished. ')
    kanban_state = fields.Selection([('normal', 'In Progress'), ('blocked', 'Blocked'), ('done', 'Ready for next stage')], string='Kanban State', required=True, default='normal', track_visibility='onchange')
