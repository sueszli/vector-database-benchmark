...
@testing.parameterize({'init': 2.0, 'rate': 0.5, 'target': None, 'expect': [2.0, 1.0, 0.5]}, {'init': 2.0, 'rate': 0.5, 'target': 1.2, 'expect': [2.0, 1.2, 1.2]}, {'init': -2.0, 'rate': 0.5, 'target': -1.2, 'expect': [-2.0, -1.2, -1.2]}, {'init': 2.0, 'rate': 2.0, 'target': None, 'expect': [2.0, 4.0, 8.0]}, {'init': 2.0, 'rate': 2.0, 'target': 3.0, 'expect': [2.0, 3.0, 3.0]}, {'init': -2.0, 'rate': 2.0, 'target': -3.0, 'expect': [-2.0, -3.0, -3.0]})
class TestExponentialShift(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.extension = extensions.ExponentialShift('x', self.rate, self.init, self.target, self.optimizer)
        self.interval = 4
    ...
    def test_basic(self):
        if False:
            return 10
        self.optimizer.x = 0
        extension = extensions.ExponentialShift('x', self.rate, init=self.init, target=self.target)
    ...
    def test_without_init(self):
        ...
        extension = extensions.ExponentialShift('x', self.rate, target=self.target)
        self._run_trainer(extension, self.expect)
    ...
    def test_with_optimizer(self):
        ...
        optimizer = mock.Mock()
        optimizer.x = 0
        extension = extensions.ExponentialShift('x', self.rate, init=self.init, target=self.target, optimizer=optimizer)
    ...
    def test_resume(self):
        ...
        new_extension = extensions.ExponentialShift('x', self.rate, self.init, self.target, new_optimizer)
