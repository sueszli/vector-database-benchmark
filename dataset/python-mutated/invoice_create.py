...
class InvoiceCreateInput(BaseInputObjectType):
    ...
    metadata = NonNullList(MetadataInput, description='Fields required to update the invoice metadata.' + ADDED_IN_314, required=False)
    private_metadata = NonNullList(MetadataInput, description='Fields required to update the invoice private metadata.' + ADDED_IN_314, required=False)
...
class InvoiceCreate(ModelMutation):
    ...
    class Arguments:
        order_id = graphene.ID(required=True, description='ID of the order related to invoice.')
        input = InvoiceCreateInput(required=True, description='Fields required when creating an invoice.')
