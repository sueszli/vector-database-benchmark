...
from ....core.notification.mutation_handler import get_external_notification_payload, send_notification
from ....core.notification.validation import validate_and_get_channel, validate_and_get_external_event_type, validate_and_get_payload_params, validate_ids_and_get_model_type_and_pks
...
class ExternalNotificationTriggerInput(graphene.InputObjectType):
    ids = NonNullList(graphene.ID, required=True, description='The list of customers or orders node IDs that will be serialized and included in the notification payload.')
    extra_payload = JSONString(description='Additional payload that will be merged with the one based on the bussines object ID.')
    external_event_type = graphene.String(required=True, description='External event type. This field is passed to a plugin as an event type.')

class ExternalNotificationTrigger(BaseMutation):

    class Arguments:
        input = ExternalNotificationTriggerInput(required=True, description='Input for External Notification Trigger.')
        plugin_id = graphene.String(description='The ID of notification plugin.')
        channel = graphene.String(required=True, description='Channel slug. Saleor will send a notification within a provided channel. Please, make sure that necessary plugins are active.')

    class Meta:
        description = 'Trigger sending a notification with the notify plugin method. Serializes nodes provided as ids parameter and includes this data in the notification payload.' + ADDED_IN_31
