...
@dataclass(frozen=True)
class Prepend:
    x: List[int] = field(default_factory=list, metadata={'merge_policy': Policy.PREPEND})

@dataclass_merge
@dataclass(frozen=True)
class RaiseWhenOverwritten:
    x: Optional[int] = field(default=None, metadata={'merge_policy': Policy.RAISE_WHEN_OVERWRITTEN})
...
@dataclass(frozen=True)
class Custom:
    x: Optional[int] = field(default=None, metadata={'merge_policy': _always_prefer_base})
