...
class CalciteSerializer:
    ...
    """
    _DTYPE_STRINGS = {'int8': 'TINYINT', 'int16': 'SMALLINT', 'int32': 'INTEGER', 'int64': 'BIGINT', 'uint8': 'SMALLINT', 'uint16': 'INTEGER', 'uint32': 'BIGINT', 'uint64': 'BIGINT', 'bool': 'BOOLEAN', 'float32': 'FLOAT', 'float64': 'DOUBLE', 'datetime64': 'TIMESTAMP'}
    _INT_OPTS = {np.int8: ('TINYINT', 3), np.int16: ('SMALLINT', 5), np.int32: ('INTEGER', 10), np.int64: ('BIGINT', 19), np.uint8: ('SMALLINT', 5), np.uint16: ('INTEGER', 10), np.uint32: ('BIGINT', 19), np.uint64: ('BIGINT', 19), int: ('BIGINT', 19)}
    _TIMESTAMP_PRECISION = {'s': 0, 'ms': 3, 'us': 6, 'ns': 9}
    _DTYPE_STRINGS.update({f'datetime64[{u}]': 'TIMESTAMP' for u in _TIMESTAMP_PRECISION})
