...
class Test_cloudtrail_multi_region_enabled:
    ...
    def set_mocked_audit_info(self):
        if False:
            for i in range(10):
                print('nop')
        audit_info = AWS_Audit_Info(session_config=None, original_session=None, audit_session=session.Session(profile_name=None, botocore_session=None), audited_account=AWS_ACCOUNT_NUMBER, audited_account_arn=f'arn:aws:iam::{AWS_ACCOUNT_NUMBER}:root', audited_user_id=None, audited_partition='aws', audited_identity_arn=None, profile=None, profile_region=None, credentials=None, assumed_role_info=None, audited_regions=[AWS_REGION_US_EAST_1, AWS_REGION_EU_WEST_1], organizations_metadata=None, audit_resources=None, mfa_enabled=False, audit_metadata=Audit_Metadata(services_scanned=0, expected_checks=[], completed_checks=0, audit_progress=0))
        return audit_info
    ...
    def test_no_trails(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info):
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_multi_region_enabled.cloudtrail_multi_region_enabled.cloudtrail_client', new=Cloudtrail(current_audit_info)):
                from prowler.providers.aws.services.cloudtrail.cloudtrail_multi_region_enabled.cloudtrail_multi_region_enabled import cloudtrail_multi_region_enabled
                ...
                for report in result:
                    if report.region == AWS_REGION_US_EAST_1:
                        ...
                        assert report.status_extended == 'No CloudTrail trails enabled and logging were found.'
                        assert report.resource_id == AWS_ACCOUNT_NUMBER
                        assert report.resource_arn == f'arn:aws:iam::{AWS_ACCOUNT_NUMBER}:root'
    ...
    def test_various_trails_no_logging(self):
        ...
        cloudtrail_client_eu_west_1 = client('cloudtrail', region_name=AWS_REGION_EU_WEST_1)
        ...
        s3_client_eu_west_1.create_bucket(Bucket=bucket_name_eu, CreateBucketConfiguration={'LocationConstraint': AWS_REGION_EU_WEST_1})
        _ = cloudtrail_client_us_east_1.create_trail(Name=trail_name_us, S3BucketName=bucket_name_us, IsMultiRegionTrail=False)
        _ = cloudtrail_client_eu_west_1.create_trail(Name=trail_name_eu, S3BucketName=bucket_name_eu, IsMultiRegionTrail=False)
        from prowler.providers.aws.services.cloudtrail.cloudtrail_service import Cloudtrail
        current_audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info):
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_multi_region_enabled.cloudtrail_multi_region_enabled.cloudtrail_client', new=Cloudtrail(current_audit_info)):
            ...
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_multi_region_enabled.cloudtrail_multi_region_enabled.cloudtrail_client', new=Cloudtrail(current_audit_info)):
                ...
                for report in result:
                    if report.resource_id == trail_name_us:
                        ...
                        assert search('is not multiregion and it is logging', report.status_extended)
                        ...
                        assert report.status == 'FAIL'
                        assert search('No CloudTrail trails enabled and logging were found.', report.status_extended)
                        assert report.resource_id == AWS_ACCOUNT_NUMBER
                        assert report.resource_arn == f'arn:aws:iam::{AWS_ACCOUNT_NUMBER}:root'
                        assert report.resource_tags == []
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info):
            with mock.patch('prowler.providers.aws.services.cloudtrail.cloudtrail_multi_region_enabled.cloudtrail_multi_region_enabled.cloudtrail_client', new=Cloudtrail(current_audit_info)):
                ...
                for report in result:
                    if report.region == AWS_REGION_US_EAST_1:
                        assert report.status == 'PASS'
                        assert search(f'Trail {trail_name_us} is multiregion and it is logging.', report.status_extended)
                        assert report.resource_id == trail_name_us
                        assert report.resource_arn == trail_us['TrailARN']
                        assert report.resource_tags == []
                    elif report.region == AWS_REGION_EU_WEST_1:
