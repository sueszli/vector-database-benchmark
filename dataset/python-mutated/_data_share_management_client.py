...
from .operations import AccountsOperations, ConsumerInvitationsOperations, ConsumerSourceDataSetsOperations, DataSetMappingsOperations, DataSetsOperations, EmailRegistrationsOperations, InvitationsOperations, Operations, ProviderShareSubscriptionsOperations, ShareSubscriptionsOperations, SharesOperations, SynchronizationSettingsOperations, TriggersOperations
if TYPE_CHECKING:
    from azure.core.credentials_async import AsyncTokenCredential

class DataShareManagementClient:
    ...
    def __init__(self, credential: 'AsyncTokenCredential', subscription_id: str, base_url: str='https://management.azure.com', **kwargs: Any) -> None:
        if False:
            while True:
                i = 10
        self._config = DataShareManagementClientConfiguration(credential=credential, subscription_id=subscription_id, **kwargs)
        ...
        self.data_set_mappings = DataSetMappingsOperations(self._client, self._config, self._serialize, self._deserialize)
        self.email_registrations = EmailRegistrationsOperations(self._client, self._config, self._serialize, self._deserialize)
        ...
        self.provider_share_subscriptions = ProviderShareSubscriptionsOperations(self._client, self._config, self._serialize, self._deserialize)
        self.share_subscriptions = ShareSubscriptionsOperations(self._client, self._config, self._serialize, self._deserialize)
        self.consumer_source_data_sets = ConsumerSourceDataSetsOperations(self._client, self._config, self._serialize, self._deserialize)
        self.synchronization_settings = SynchronizationSettingsOperations(self._client, self._config, self._serialize, self._deserialize)
