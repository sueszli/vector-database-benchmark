...
from streamlit.elements.lib.column_config_utils import _EDITING_COMPATIBILITY_MAPPING, INDEX_IDENTIFIER, ColumnConfigMapping, ColumnConfigMappingInput, ColumnDataKind, _determine_data_kind, _determine_data_kind_via_arrow, _determine_data_kind_via_inferred_type, _determine_data_kind_via_pandas_dtype, apply_data_specific_configs, determine_dataframe_schema, is_type_compatible, process_config_mapping, update_column_config
...
def _get_arrow_schema_field(column: pd.Series) -> pa.Field | None:
    ...
    try:
        ...
        return None
SHARED_DATA_KIND_TEST_CASES = [(pd.Series(['a', 'b', 'c'], dtype=pd.StringDtype()), ColumnDataKind.STRING), (pd.Series([1, 2, -3], dtype='Int64'), ColumnDataKind.INTEGER), (pd.Series([1.1, 2.2, -3.3]), ColumnDataKind.FLOAT), (pd.Series([1, 2.2, 3]), ColumnDataKind.FLOAT), (pd.Series([pd.Timestamp('2000-01-01'), pd.Timestamp('2000-01-02')]), ColumnDataKind.DATETIME), (pd.Series([datetime.datetime(2000, 1, 1), datetime.datetime(2000, 1, 2)]), ColumnDataKind.DATETIME), (pd.Series([pd.Timestamp('2000-01-01', tz='US/Central'), pd.Timestamp('2000-01-02', tz='US/Central')]), ColumnDataKind.DATETIME), (pd.Series([True, False]), ColumnDataKind.BOOLEAN), (pd.Series([pd.Timedelta('1 day'), pd.Timedelta('2 days')]), ColumnDataKind.TIMEDELTA), (pd.Series([np.timedelta64(1, 'D'), np.timedelta64(2, 'D')]), ColumnDataKind.TIMEDELTA)]
