...
def add_edit(tenant_id, project_id, data: schemas.IntegrationNewrelicSchema):
    ...
    s = get(project_id)
    if s is not None:
        return update(tenant_id=tenant_id, project_id=project_id, changes={'applicationId': data.application_id, 'xQueryKey': data.x_query_key, 'region': data.region})
    else:
        return add(tenant_id=tenant_id, project_id=project_id, application_id=data.application_id, x_query_key=data.x_query_key, region=data.region)