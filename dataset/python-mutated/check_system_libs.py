...
syslibs_configure_path = os.path.join(tf_source_path, 'third_party', 'systemlibs', 'syslibs_configure.bzl')
...
if not os.path.isfile(syslibs_configure_path):
    raise ValueError('Could not find syslibs_configure.bzl at %s' % syslibs_configure_path)
...
missing_build_files = [file for file in system_build_files_from_workspace if not os.path.isfile(os.path.join(tf_source_path, file))]
has_error = False
...
if syslibs != syslibs_from_workspace:
    ...
    if additional_syslibs:
        ...
        print('Libs missing in workspace (or superfluous in syslibs_configure): ' + libs)
