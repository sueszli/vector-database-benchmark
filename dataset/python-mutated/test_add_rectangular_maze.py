...
class TestAddRectangularMaze(TestCase):
    ...
    def _write_maze(self, width: int, height: int) -> DisconnectedShape:
        if False:
            while True:
                i = 10
        return DisconnectedShape(RectangularMazeFactory.rectangular_maze(width, height), stroke_color=HexColor('f1cd2e'), line_width=Decimal(1))
    ...
    def test_add_rectangular_maze_001(self):
        ...
        layout.add(self.get_test_header(test_description='This test creates a PDF with a square maze in it.'))
        random.seed(2048)
