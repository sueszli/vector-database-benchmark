...
from typing import Iterable, Mapping, Any, TypeVar, NamedTuple, Sequence, Optional, Union, Tuple, List, Callable
import numpy as np
import scipy.sparse as sp
from AnyQt.QtWidgets import QGraphicsView, QFormLayout, QComboBox, QGroupBox, QMenu, QAction, QSizePolicy
...
from Orange.data import Domain, Table, Variable, DiscreteVariable, ContinuousVariable
...
from Orange.widgets.visualize.utils.heatmap import HeatmapGridWidget, ColorMap, CategoricalColorMap, GradientColorMap
...
ClusteringModelData = [{Qt.DisplayRole: 'None', Qt.ToolTipRole: 'No clustering', ClusteringRole: Clustering.None_}, {Qt.DisplayRole: 'Clustering', Qt.ToolTipRole: 'Apply hierarchical clustering', ClusteringRole: Clustering.Clustering}, {Qt.DisplayRole: 'Clustering (opt. ordering)', Qt.ToolTipRole: 'Apply hierarchical clustering with optimal leaf ordering.', ClusteringRole: Clustering.OrderedClustering}]
ColumnLabelsPosData = [{Qt.DisplayRole: name, Qt.UserRole: value} for (name, value) in [('None', HeatmapGridWidget.NoPosition), ('Top', HeatmapGridWidget.PositionTop), ('Bottom', HeatmapGridWidget.PositionBottom), ('Top and Bottom', HeatmapGridWidget.PositionTop | HeatmapGridWidget.PositionBottom)]]
...
class OWHeatMap(widget.OWWidget):
    ...
    class Error(widget.OWWidget.Error):
        ...
        not_enough_instances_k_means = Msg('Not enough instances for k-means merging')
        not_enough_memory = Msg('Not enough memory to show this data')

    class Warning(widget.OWWidget.Warning):
        empty_clusters = Msg('Empty clusters were removed')
    UserAdviceMessages = [widget.Message('For data with a meaningful mid-point, choose one of diverging palettes.', 'diverging_palette')]
