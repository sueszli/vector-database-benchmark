...
num_batches = 20
num_epochs = 1
x_train = np.random.normal(size=(num_batches * batch_size, *input_shape)).astype(np.float32)
...
def run_keras_custom_training_loop():
    if False:
        while True:
            i = 10
    keras_model = keras.Sequential([layers.Input(shape=input_shape), layers.Dense(64, activation='relu'), layers.Dense(8, activation='relu'), layers.Dense(num_classes), layers.Softmax()])
