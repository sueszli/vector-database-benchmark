...
class WorkflowActivity(models.Model):
    ...
    _order = 'name'
    name = fields.Char('Name', required=True)
    wkf_id = fields.Many2one('workflow', string='Workflow', ondelete='cascade', required=True, index=True)
    split_mode = fields.Selection([('XOR', 'Xor'), ('OR', 'Or'), ('AND', 'And')], string='Split Mode', size=3, required=True, default='XOR')
    join_mode = fields.Selection([('XOR', 'Xor'), ('AND', 'And')], string='Join Mode', size=3, required=True, default='XOR')
    kind = fields.Selection([('dummy', 'Dummy'), ('function', 'Function'), ('subflow', 'Subflow'), ('stopall', 'Stop All')], string='Kind', required=True, default='dummy')
...
class WorkflowTransition(models.Model):
    ...
    signal = fields.Char('Signal (Button Name)', help='When the operation of transition comes from a button pressed in the client form, signal tests the name of the pressed button. If signal is NULL, no button is necessary to validate this transition.')
    group_id = fields.Many2one('res.groups', string='Group Required', help='The group that a user must have to be authorized to validate this transition.')
    condition = fields.Char('Condition', required=True, default='True', help='Expression to be satisfied if we want the transition done.')
    act_from = fields.Many2one('workflow.activity', string='Source Activity', ondelete='cascade', required=True, index=True, help='Source activity. When this activity is over, the condition is tested to determine if we can start the ACT_TO activity.')
    act_to = fields.Many2one('workflow.activity', string='Destination Activity', ondelete='cascade', required=True, index=True, help='The destination activity.')
...
class WorkflowWorkitem(models.Model):
    ...
    _rec_name = 'state'
    _log_access = False
    act_id = fields.Many2one('workflow.activity', string='Activity', ondelete='cascade', required=True, index=True)
    wkf_id = fields.Many2one('workflow', related='act_id.wkf_id', string='Workflow')
    subflow_id = fields.Many2one('workflow.instance', string='Subflow', ondelete='set null', index=True)
    inst_id = fields.Many2one('workflow.instance', string='Instance', ondelete='cascade', required=True, index=True)
...
class WorkflowTriggers(models.Model):
    ...
    instance_id = fields.Many2one('workflow.instance', string='Destination Instance', ondelete='cascade')
    workitem_id = fields.Many2one('workflow.workitem', string='Workitem', ondelete='cascade', required=True)
