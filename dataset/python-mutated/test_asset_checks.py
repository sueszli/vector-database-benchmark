...
import pytest
from dagster import AssetCheckResult, AssetExecutionContext, AssetKey, AssetSelection, ExecuteInProcessResult, materialize
...
test_asset_checks_dbt_project_dir = Path(__file__).joinpath('..', 'dbt_projects', 'test_dagster_asset_checks').resolve()
manifest_path = test_asset_checks_dbt_project_dir.joinpath('manifest.json').resolve()
manifest = json.loads(manifest_path.read_bytes())
dagster_dbt_translator_with_checks = DagsterDbtTranslator(settings=DagsterDbtTranslatorSettings(enable_asset_checks=True))
