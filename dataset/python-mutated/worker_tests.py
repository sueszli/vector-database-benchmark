...
class TestWorker(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retry_job_after_delay_with_fixed_backoff(self):
        ...
        await queue.add('test', {'foo': 'bar'}, {'attempts': 3, 'backoff': {'type': 'fixed', 'delay': 1000}})
    ...
    async def test_retry_job_after_delay_with_custom_backoff(self):
        ...
        worker = Worker(queueName, process1, {'settings': {'backoffStrategy': backoff_strategy}})
        start = round(time.time() * 1000)
        await queue.add('test', {'foo': 'bar'}, {'attempts': 3, 'backoff': {'type': 'custom'}})
        completed_events = Future()
    ...
    async def test_create_children_at_runtime(self):
        ...
        async def parent_process(job: Job, token: str):
            ...
            while step != Step.Finish:
                if step == Step.Initial:
                    await queue.add('child-1', {'foo': 'bar'}, {'parent': {'id': job.id, 'queue': job.queueQualifiedName}})
                    await job.updateData({'step': Step.Second})
                    step = Step.Second
                elif step == Step.Second:
                    await queue.add('child-2', {'foo': 'bar'}, {'parent': {'id': job.id, 'queue': job.queueQualifiedName}})
        ...
        await parent_queue.add('test', {'step': Step.Initial}, {'attempts': 3, 'backoff': 1000})
        completed_events = Future()
