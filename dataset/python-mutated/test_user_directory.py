...
from synapse.storage.databases.main.user_directory import _parse_words_with_icu, _parse_words_with_regex
...
class GetUserDirectoryTables:
    ...
    async def get_users_in_public_rooms(self) -> Set[Tuple[str, str]]:
        ...
        """
        r = cast(List[Tuple[str, str]], await self.store.db_pool.simple_select_list('users_in_public_rooms', None, ('user_id', 'room_id')))
        return set(r)
    ...
    async def get_users_who_share_private_rooms(self) -> Set[Tuple[str, str, str]]:
        ...
        """
        rows = cast(List[Tuple[str, str, str]], await self.store.db_pool.simple_select_list('users_who_share_private_rooms', None, ['user_id', 'other_user_id', 'room_id']))
        return set(rows)
    ...
    async def get_users_in_user_directory(self) -> Set[str]:
        ...
        """
        result = cast(List[Tuple[str]], await self.store.db_pool.simple_select_list('user_directory', None, ['user_id']))
    ...
    async def get_profiles_in_user_directory(self) -> Dict[str, ProfileInfo]:
        ...
        """
        rows = cast(List[Tuple[str, Optional[str], Optional[str]]], await self.store.db_pool.simple_select_list('user_directory', None, ('user_id', 'display_name', 'avatar_url')))
        return {user_id: ProfileInfo(display_name=display_name, avatar_url=avatar_url) for (user_id, display_name, avatar_url) in rows}

    async def get_tables(self) -> Tuple[Set[str], Set[Tuple[str, str]], Set[Tuple[str, str, str]]]:
        """Multiple tests want to inspect these tables, so expose them together."""
        return (await self.get_users_in_user_directory(), await self.get_users_in_public_rooms(), await self.get_users_who_share_private_rooms())
...
class UserDirectoryInitialPopulationTestcase(HomeserverTestCase):
    ...
    """
    servlets = [login.register_servlets, admin.register_servlets, room.register_servlets, register.register_servlets]
...
class UserDirectoryStoreTestCase(HomeserverTestCase):
    ...
    test_search_user_dir_dotted_dotless_i_case_insensitivity.skip = 'not supported'
    ...
    test_search_user_dir_accent_insensitivity.skip = 'not supported yet'
