...
class CountryRateUpdateInput(BaseInputObjectType):
    country_code = CountryCodeEnum(description='Country in which this rate applies.', required=True)
    rate = graphene.Float(description='Tax rate value provided as percentage. Example: provide `23` to represent `23%` tax rate. Provide `null` to remove the particular rate.', required=False)
...
class TaxClassUpdateError(Error):
    ...
    country_codes = NonNullList(graphene.String, description='List of country codes for which the configuration is invalid.', required=True)
...
class TaxClassUpdateInput(BaseInputObjectType):
    ...
    update_country_rates = NonNullList(CountryRateUpdateInput, description='List of country-specific tax rates to create or update for this tax class.')
    remove_country_rates = NonNullList(CountryCodeEnum, description='List of country codes for which to remove the tax class rates. Note: It removes all rates for given country code.')
...
class TaxClassUpdate(ModelMutation):
    ...
    class Arguments:
        ...
        input = TaxClassUpdateInput(description='Fields required to update a tax class.', required=True)
