...
try:
    ...
    try:
        ...
        compat.register()
from pony.orm.dbproviders.postgres import PGSQLBuilder, PGColumn, PGSchema, PGTranslator, PGProvider, PGIntConverter, PGRealConverter, PGDatetimeConverter, PGTimedeltaConverter, PGBlobConverter, PGJsonConverter, PGArrayConverter
...
class CRArrayConverter(PGArrayConverter):
    array_types = {int: ('INT', PGIntConverter), str: ('STRING', dbapiprovider.StrConverter), float: ('DOUBLE PRECISION', PGRealConverter)}
...
class CRProvider(PGProvider):
    ...
    converter_classes = [(NoneType, dbapiprovider.NoneConverter), (bool, dbapiprovider.BoolConverter), (str, dbapiprovider.StrConverter), (int_types, CRIntConverter), (float, PGRealConverter), (Decimal, dbapiprovider.DecimalConverter), (datetime, PGDatetimeConverter), (date, dbapiprovider.DateConverter), (time, dbapiprovider.TimeConverter), (timedelta, CRTimedeltaConverter), (UUID, PGUuidConverter), (buffer, CRBlobConverter), (ormtypes.Json, PGJsonConverter)]
