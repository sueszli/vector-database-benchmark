...
class SolutionsDiscoverabilityOperations:
    ...
    @distributed_trace
    def list(self, farm_beats_solution_ids: Optional[List[str]]=None, farm_beats_solution_names: Optional[List[str]]=None, max_page_size: int=50, **kwargs: Any) -> AsyncIterable['_models.FarmBeatsSolution']:
        if False:
            print('Hello World!')
        ...
        def prepare_request(next_link=None):
            if False:
                for i in range(10):
                    print('nop')
            if not next_link:
                request = build_list_request(farm_beats_solution_ids=farm_beats_solution_ids, farm_beats_solution_names=farm_beats_solution_names, max_page_size=max_page_size, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
    ...
    async def get(self, farm_beats_solution_id: str, **kwargs: Any) -> _models.FarmBeatsSolution:
        ...
        request = build_get_request(farm_beats_solution_id=farm_beats_solution_id, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=False, **kwargs)
        response = pipeline_response.http_response
