...
from .const import API_DIRECTIVE, API_HEADER, CONF_ENDPOINT, CONF_ENTITY_CONFIG, CONF_FILTER, CONF_LOCALE, EVENT_ALEXA_SMART_HOME
...
class SmartHomeView(HomeAssistantView):
    ...
    async def post(self, request: HomeAssistantRequest) -> web.Response | bytes:
        ...
        response = await async_handle_message(hass, self.smart_home_config, message, context=core.Context(user_id=user.id))
...
async def async_handle_message(hass: HomeAssistant, config: AbstractConfig, request: dict[str, Any], context: Context | None=None, enabled: bool=True) -> dict[str, Any]:
    ...
    try:
        if not enabled:
            raise AlexaBridgeUnreachableError('Alexa API not enabled in Home Assistant configuration')
        ...
        if funct_ref:
            ...
            _LOGGER.warning('Unsupported API request %s/%s', directive.namespace, directive.name)
            response = directive.error()
    except AlexaError as err:
        response = directive.error(error_type=str(err.error_type), error_message=err.error_message, payload=err.payload)
    except Exception:
        _LOGGER.exception('Uncaught exception processing Alexa %s/%s request (%s)', directive.namespace, directive.name, directive.entity_id or '-')
        response = directive.error(error_message='Unknown error')
    request_info: dict[str, Any] = {'namespace': directive.namespace, 'name': directive.name}
    if directive.has_endpoint:
        assert directive.entity_id is not None
        request_info['entity_id'] = directive.entity_id
    hass.bus.async_fire(EVENT_ALEXA_SMART_HOME, {'request': request_info, 'response': {'namespace': response.namespace, 'name': response.name}}, context=context)
