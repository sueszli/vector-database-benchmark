...
class upsample_nearest_neighbor(Operation):
    input_spec = InputSpec(x=TensorInputType(), upscale_factor_height=IntInputType(const=True, default=1), upscale_factor_width=IntInputType(const=True, default=1))
...
class upsample_bilinear(Operation):
    input_spec = InputSpec(x=TensorInputType(), scale_factor_height=IntOrFloatInputType(const=True, default=1), scale_factor_width=IntOrFloatInputType(const=True, default=1), align_corners=BoolInputType(const=True, default=True))
...
class resize_bilinear(Operation):
    input_spec = InputSpec(x=TensorInputType(), target_size_height=IntInputType(const=True, default=1), target_size_width=IntInputType(const=True, default=1), sampling_mode=StringInputType(const=True, default='DEFAULT'))
...
class crop_resize(Operation):
    input_spec = InputSpec(x=TensorInputType(), roi=TensorInputType(), target_height=IntInputType(const=True, default=1), target_width=IntInputType(const=True, default=1), normalized_coordinates=BoolInputType(const=True, default=False), spatial_scale=FloatInputType(const=True, default=1.0), box_coordinate_mode=StringInputType(const=True, default='CONRNERS_HEIGHT_FIRST'), sampling_mode=StringInputType(const=True, default='STRICT_ALIGN_CORNERS'))
...
class crop(Operation):
    input_spec = InputSpec(x=TensorInputType(), crop_height=IntTensorInputType(const=True), crop_width=IntTensorInputType(const=True))
