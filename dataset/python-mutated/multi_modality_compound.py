...
@dataclass
class SpeechTextPreTrainCompoundCriterionConfig(LabelSmoothedCrossEntropyCriterionConfig):
    zero_infinity: bool = field(default=False, metadata={'help': 'zero inf loss when source length <= target length'})
    post_process: str = field(default='none', metadata={'help': 'how to post process predictions into words. can be letter, wordpiece, BPE symbols, etc. See fairseq.data.data_utils.post_process() for full list of options'})

@register_criterion('speech_text_pretrain_compound', dataclass=SpeechTextPreTrainCompoundCriterionConfig)
