...
from ray._private.ray_constants import RAY_ADDRESS_ENVIRONMENT_VARIABLE, KV_NAMESPACE_JOB, DEFAULT_DASHBOARD_AGENT_LISTEN_PORT
from ray._private.test_utils import SignalActor, async_wait_for_condition, async_wait_for_condition_async_predicate
from ray.dashboard.modules.job.common import JOB_ID_METADATA_KEY, JOB_NAME_METADATA_KEY
from ray.dashboard.modules.job.job_manager import JobLogStorageClient, JobManager, JobSupervisor, generate_job_id
from ray.dashboard.consts import RAY_JOB_ALLOW_DRIVER_ON_WORKER_NODES_ENV_VAR, RAY_JOB_START_TIMEOUT_SECONDS_ENV_VAR
...
async def test_get_scheduling_strategy(call_ray_start, monkeypatch, resources_specified, tmp_path):
    ...
    await gcs_aio_client.internal_kv_del('head_node_id'.encode(), del_by_prefix=False, namespace=KV_NAMESPACE_JOB)
    strategy = await job_manager._get_scheduling_strategy(resources_specified)
    assert strategy == 'DEFAULT'
    await gcs_aio_client.internal_kv_put('head_node_id'.encode(), '123456'.encode(), True, namespace=KV_NAMESPACE_JOB)
    strategy = await job_manager._get_scheduling_strategy(resources_specified)
...
@pytest.mark.asyncio
@pytest.mark.parametrize('call_ray_start', ['ray start --head --resources={"TestResourceKey":123}'], indirect=True)
async def test_submit_no_ray_address(call_ray_start, tmp_path):
    ...
    job_id = await job_manager.submit_job(entrypoint=f'python -c "{init_ray_no_address_script}" ')
    await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)

@pytest.mark.asyncio
@pytest.mark.parametrize('call_ray_start', ['ray start --head'], indirect=True)
async def test_get_all_job_info(call_ray_start, tmp_path):
    """Test that JobInfo is correctly populated in the GCS get_all_job_info API."""
    address_info = ray.init(address=call_ray_start)
    gcs_aio_client = GcsAioClient(address=address_info['gcs_address'], nums_reconnect_retry=0)
    job_manager = JobManager(gcs_aio_client, tmp_path)
    submission_id = await job_manager.submit_job(entrypoint="python -c 'import ray; ray.init()'")
    await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=submission_id)
    found = False
    for job_table_entry in (await gcs_aio_client.get_all_job_info()).values():
        if job_table_entry.config.metadata.get(JOB_ID_METADATA_KEY) == submission_id:
            ...
            assert job_info.driver_agent_http_address.startswith('http://') and job_info.driver_agent_http_address.endswith(str(DEFAULT_DASHBOARD_AGENT_LISTEN_PORT))
            assert job_info.driver_node_id != ''
    assert found

@pytest.mark.asyncio
@pytest.mark.parametrize('call_ray_start', ['ray start --head'], indirect=True)
async def test_get_all_job_info_with_is_running_tasks(call_ray_start):
    ...
    await async_wait_for_condition_async_predicate(lambda : check_is_running_tasks(job_id, False), timeout=30)
    ray.shutdown()
...
async def _run_hanging_command(job_manager, tmp_dir, start_signal_actor=None):
    ...
    wait_for_file_cmd = f"echo $$ > {pid_file} && until [ -f {tmp_file} ]; do echo 'Waiting...' && sleep 1; done"
    job_id = await job_manager.submit_job(entrypoint=wait_for_file_cmd, _start_signal_actor=start_signal_actor)
    ...
    if start_signal_actor:
        ...
        await async_wait_for_condition_async_predicate(check_job_running, job_manager=job_manager, job_id=job_id)
        await async_wait_for_condition(lambda : 'Waiting...' in job_manager.get_job_logs(job_id))
...
async def test_list_jobs(job_manager: JobManager):
    ...
    metadata = {'foo': 'bar'}
    await job_manager.submit_job(entrypoint='echo hello', submission_id='2', runtime_env=runtime_env, metadata=metadata)
    await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id='1')
    await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id='2')
...
async def test_pass_job_id(job_manager):
    ...
    returned_id = await job_manager.submit_job(entrypoint='echo hello', submission_id=submission_id)
    assert returned_id == submission_id
    await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=submission_id)
    with pytest.raises(ValueError):
        await job_manager.submit_job(entrypoint='echo hello', submission_id=submission_id)

@pytest.mark.asyncio
async def test_simultaneous_submit_job(job_manager):
    """Test that we can submit multiple jobs at once."""
    job_ids = await asyncio.gather(job_manager.submit_job(entrypoint='echo hello'), job_manager.submit_job(entrypoint='echo hello'), job_manager.submit_job(entrypoint='echo hello'))
...
async def test_simultaneous_with_same_id(job_manager):
    ...
    with pytest.raises(ValueError) as excinfo:
        await asyncio.gather(job_manager.submit_job(entrypoint='echo hello', submission_id='1'), job_manager.submit_job(entrypoint='echo hello', submission_id='1'))
...
class TestShellScriptExecution:
    ...
    async def test_submit_with_s3_runtime_env(self, job_manager):
        job_id = await job_manager.submit_job(entrypoint='python script.py', runtime_env={'working_dir': 's3://runtime-env-test/script_runtime_env.zip'})
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
        assert job_manager.get_job_logs(job_id) == 'Executing main() from script.py !!\n'

    async def test_submit_with_file_runtime_env(self, job_manager):
        with tempfile.NamedTemporaryFile(suffix='.zip') as f:
            (filename, _) = urllib.request.urlretrieve('https://runtime-env-test.s3.amazonaws.com/script_runtime_env.zip', filename=f.name)
            job_id = await job_manager.submit_job(entrypoint='python script.py', runtime_env={'working_dir': 'file://' + filename})
            await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
            assert job_manager.get_job_logs(job_id) == 'Executing main() from script.py !!\n'
...
class TestRuntimeEnv:
    ...
    async def test_pass_env_var(self, job_manager):
        ...
        driver script.
        """
        job_id = await job_manager.submit_job(entrypoint='echo $TEST_SUBPROCESS_JOB_CONFIG_ENV_VAR', runtime_env={'env_vars': {'TEST_SUBPROCESS_JOB_CONFIG_ENV_VAR': '233'}})
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
        assert job_manager.get_job_logs(job_id) == '233\n'

    async def test_niceness(self, job_manager):
        job_id = await job_manager.submit_job(entrypoint=f"python {_driver_script_path('check_niceness.py')}")
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
        logs = job_manager.get_job_logs(job_id)
    ...
    async def test_multiple_runtime_envs(self, job_manager):
        job_id_1 = await job_manager.submit_job(entrypoint=f"python {_driver_script_path('print_runtime_env.py')}", runtime_env={'env_vars': {'TEST_SUBPROCESS_JOB_CONFIG_ENV_VAR': 'JOB_1_VAR'}})
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id_1)
        logs = job_manager.get_job_logs(job_id_1)
        assert "{'env_vars': {'TEST_SUBPROCESS_JOB_CONFIG_ENV_VAR': 'JOB_1_VAR'}}" in logs
        job_id_2 = await job_manager.submit_job(entrypoint=f"python {_driver_script_path('print_runtime_env.py')}", runtime_env={'env_vars': {'TEST_SUBPROCESS_JOB_CONFIG_ENV_VAR': 'JOB_2_VAR'}})
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id_2)
        logs = job_manager.get_job_logs(job_id_2)
        assert "{'env_vars': {'TEST_SUBPROCESS_JOB_CONFIG_ENV_VAR': 'JOB_2_VAR'}}" in logs
    ...
    async def test_failed_runtime_env_validation(self, job_manager):
        ...
        job_id = await job_manager.submit_job(entrypoint=run_cmd, runtime_env={'working_dir': 'path_not_exist'})
        data = await job_manager.get_job_info(job_id)
    ...
    async def test_failed_runtime_env_setup(self, job_manager):
        ...
        """
        run_cmd = f"python {_driver_script_path('override_env_var.py')}"
        job_id = await job_manager.submit_job(entrypoint=run_cmd, runtime_env={'working_dir': 's3://does_not_exist.zip'})
        await async_wait_for_condition_async_predicate(check_job_failed, job_manager=job_manager, job_id=job_id)
        data = await job_manager.get_job_info(job_id)
    ...
    async def test_pass_metadata(self, job_manager):
        ...
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
        assert dict_to_str({JOB_NAME_METADATA_KEY: job_id, JOB_ID_METADATA_KEY: job_id}) in job_manager.get_job_logs(job_id)
        job_id = await job_manager.submit_job(entrypoint=print_metadata_cmd, metadata={'key1': 'val1', 'key2': 'val2'})
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
        assert dict_to_str({JOB_NAME_METADATA_KEY: job_id, JOB_ID_METADATA_KEY: job_id, 'key1': 'val1', 'key2': 'val2'}) in job_manager.get_job_logs(job_id)
        job_id = await job_manager.submit_job(entrypoint=print_metadata_cmd, metadata={JOB_NAME_METADATA_KEY: 'custom_name'})
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
        assert dict_to_str({JOB_NAME_METADATA_KEY: 'custom_name', JOB_ID_METADATA_KEY: job_id}) in job_manager.get_job_logs(job_id)

    @pytest.mark.parametrize('env_vars', [None, {}, {'hello': 'world'}])
    @pytest.mark.parametrize('resource_kwarg', [{}, {'entrypoint_num_cpus': 1}, {'entrypoint_num_gpus': 1}, {'entrypoint_memory': 4}, {'entrypoint_resources': {'Custom': 1}}])
    async def test_cuda_visible_devices(self, job_manager, resource_kwarg, env_vars):
        ...
        job_id = await job_manager.submit_job(entrypoint=run_cmd, runtime_env=runtime_env, **resource_kwarg)
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
...
class TestAsyncAPI:
    ...
    async def test_status_and_logs_while_blocking(self, job_manager):
        with tempfile.TemporaryDirectory() as tmp_dir:
            (pid_file, tmp_file, job_id) = await _run_hanging_command(job_manager, tmp_dir)
    ...
    async def test_stop_job(self, job_manager):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            await async_wait_for_condition_async_predicate(check_job_stopped, job_manager=job_manager, job_id=job_id)
            await async_wait_for_condition(lambda : job_manager.stop_job(job_id) is False)
    ...
    async def test_stop_job_in_pending(self, job_manager):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            (pid_file, _, job_id) = await _run_hanging_command(job_manager, tmp_dir, start_signal_actor=start_signal_actor)
            assert not os.path.exists(pid_file), 'driver subprocess should NOT be running while job is still PENDING.'
            assert job_manager.stop_job(job_id) is True
...
class TestTailLogs:
    ...
    async def _tail_and_assert_logs(self, job_id, job_manager, expected_log='', num_iteration=5):
        i = 0
    ...
    async def test_successful_job(self, job_manager):
        ...
        start_signal_actor = SignalActor.remote()
        with tempfile.TemporaryDirectory() as tmp_dir:
            (_, tmp_file, job_id) = await _run_hanging_command(job_manager, tmp_dir, start_signal_actor=start_signal_actor)
            job_status = await job_manager.get_job_status(job_id)
            assert job_status == JobStatus.PENDING
            ray.get(start_signal_actor.send.remote())
            await self._tail_and_assert_logs(job_id, job_manager, expected_log='Waiting...', num_iteration=5)
            with open(tmp_file, 'w') as f:
                print('hello', file=f)
            async for lines in job_manager.tail_job_logs(job_id):
...
@pytest.mark.asyncio
@pytest.mark.parametrize('use_env_var,stop_timeout', [(True, 10), (False, JobSupervisor.DEFAULT_RAY_JOB_STOP_WAIT_TIME_S)])
async def test_stop_job_timeout(job_manager, use_env_var, stop_timeout):
    ...
    entrypoint = 'python -c "\nimport sys\nimport signal\nimport time\ndef handler(*args):\n    print(\'SIGTERM signal handled!\');\nsignal.signal(signal.SIGTERM, handler)\n\nwhile True:\n    print(\'Waiting...\')\n    time.sleep(1)"\n'
    if use_env_var:
        job_id = await job_manager.submit_job(entrypoint=entrypoint, runtime_env={'env_vars': {'RAY_JOB_STOP_WAIT_TIME_S': str(stop_timeout)}})
    else:
        job_id = await job_manager.submit_job(entrypoint=entrypoint)
    await async_wait_for_condition(lambda : 'Waiting...' in job_manager.get_job_logs(job_id))
    assert job_manager.stop_job(job_id) is True
    with pytest.raises(RuntimeError):
        await async_wait_for_condition_async_predicate(check_job_stopped, job_manager=job_manager, job_id=job_id, timeout=stop_timeout - 1)
    await async_wait_for_condition(lambda : 'SIGTERM signal handled!' in job_manager.get_job_logs(job_id))
    await async_wait_for_condition_async_predicate(check_job_stopped, job_manager=job_manager, job_id=job_id, timeout=10)
...
async def test_logs_streaming(job_manager):
    ...
    await async_wait_for_condition(lambda : 'STREAMED' in job_manager.get_job_logs(job_id))
    job_manager.stop_job(job_id)
...
async def test_bootstrap_address(job_manager, monkeypatch):
    ...
    print_ray_address_cmd = 'python -c"import os;import ray;ray.init();print(\'SUCCESS!\');"'
...
async def test_job_runs_with_no_resources_available(job_manager):
    ...
    try:
        ...
        await async_wait_for_condition_async_predicate(check_job_running, job_manager=job_manager, job_id=job_id)
        await async_wait_for_condition(lambda : 'Hanging...' in job_manager.get_job_logs(job_id))
        ray.get(hang_signal_actor.send.remote())
        await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
        await async_wait_for_condition(lambda : 'Success!' in job_manager.get_job_logs(job_id))
    finally:
...
async def test_failed_job_logs_max_char(job_manager):
    ...
    print_large_logs_cmd = 'python -c "print(\'1234567890\'* 2100); raise RuntimeError()"'
    job_id = await job_manager.submit_job(entrypoint=print_large_logs_cmd)
    await async_wait_for_condition_async_predicate(check_job_failed, job_manager=job_manager, job_id=job_id)
    job_info = await job_manager.get_job_info(job_id)
    assert job_info
    assert len(job_info.message) == 20000 + len('Job entrypoint command failed with exit code 1, last available logs (truncated to 20,000 chars):\n')
...
async def test_simultaneous_drivers(job_manager):
    ...
    job_id = await job_manager.submit_job(entrypoint=f"{cmd} & {cmd} && wait && echo 'done'")
    await async_wait_for_condition_async_predicate(check_job_succeeded, job_manager=job_manager, job_id=job_id)
...
async def test_monitor_job_pending(job_manager):
    ...
    start_signal_actor = SignalActor.remote()
    job_id = await job_manager.submit_job(entrypoint="echo 'hello world'", _start_signal_actor=start_signal_actor)
...
async def test_actor_creation_error_not_overwritten(shared_ray_instance, tmp_path):
    ...
    for _ in range(10):
        ...
        job_id = await job_manager.submit_job(entrypoint="doesn't matter", runtime_env={'working_dir': 'path_not_exist'})
        for _ in range(100):
            data = await job_manager.get_job_info(job_id)
            assert data.status == JobStatus.FAILED
            assert 'path_not_exist is not a valid URI' in data.message
