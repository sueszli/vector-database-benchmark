...
excluded_packages = ['azure', 'azure-mgmt']
...
if __name__ == '__main__':
    ...
    if should_run_import_all(pkg_details.name):
        logging.info('Importing all modules from namespace [{0}] to verify dependency'.format(pkg_details.namespace))
        import_script_all = 'from {0} import *'.format(pkg_details.namespace)
        commands = [sys.executable, '-c', import_script_all]
