...
class CreateNewAbility(Ability):
    default_configuration = AbilityConfiguration(location=PluginLocation(storage_format=PluginStorageFormat.INSTALLED_PACKAGE, storage_route='autogpt.core.ability.builtins.CreateNewAbility'))

    def __init__(self, logger: logging.Logger, configuration: AbilityConfiguration):
        if False:
            for i in range(10):
                print('nop')
        self._logger = logger
