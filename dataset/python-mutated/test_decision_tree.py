...
RMSE_CUTOFF = 15
_DEFAULT_OPTIONS_REGRESSION = {'max_depth': 6, 'min_child_weight': 0.1, 'min_loss_reduction': 0.0, 'random_seed': None, 'metric': 'auto'}
...
class DecisionTreeRegressionTest(unittest.TestCase):
    ...
    def test_create(self):
        if False:
            i = 10
            return i + 15
        model = tc.decision_tree_regression.create(self.dtrain, target='label', validation_set=self.dtest, **self.param)
        rmse = model.evaluate(self.dtest, 'rmse')['rmse']
        self.assertTrue(model is not None)
        self.assertLess(rmse, RMSE_CUTOFF)
        dtrain = self.dtrain
        dtrain['label'] = 10
        self.assertRaises(ToolkitError, lambda : tc.decision_tree_regression.create(self.dtrain, target='label_wrong', **self.param))
    ...
    def test_evaluate(self):
        ...
        self.sm_metrics = {'max_error': tc.toolkits.evaluation.max_error(t, p), 'rmse': tc.toolkits.evaluation.rmse(t, p)}
    ...
    def _test_regression_model(self, train, test, rmse_threshold, target='label'):
        if False:
            while True:
                i = 10
        model = tc.decision_tree_regression.create(train, target=target, validation_set=test, **self.param)
    ...
    def test_predict_new_dictionary_key(self):
        ...
        model = tc.decision_tree_regression.create(train, target='label', **self.param)
        y1 = model.predict(test)
        y2 = model.predict(list(test))
        self.assertEqual(list(y1), list(y2))

def test_suite_decision_tree_classifier():
    if False:
        return 10
    '\n    Create a test suite for each test case in the DecisionTreeClassifierTest.\n    '
    testCases = [binary_classification_integer_target, binary_classification_string_target, multiclass_classification_integer_target, multiclass_classification_string_target]
    for t in testCases:
        testcase_members = {}
        testcase_members[t.__name__] = classmethod(t)
        testcase_class = type('DecisionTreeClassifierTest_%s' % t.__name__, (DecisionTreeClassifierTest,), testcase_members)
...
def binary_classification_integer_target(cls):
    ...
    cls.param = {'max_depth': 3, 'min_loss_reduction': 1, 'min_child_weight': 1}
    cls.target = 'label'
    cls.unpacked_features = cls.data.column_names()
    cls.unpacked_features.remove(cls.target)
    cls.features = cls.unpacked_features[:]
    cls.model = tc.decision_tree_classifier.create(cls.dtrain, target=cls.target, validation_set=cls.dtest, **cls.param)
...
def multiclass_classification_integer_target(cls):
    ...
    num_examples_per_class = {c: (cls.dtrain[cls.target] == c).sum() for c in cls.model.classes}
    cls.get_ans['num_examples_per_class'] = lambda x: x == num_examples_per_class
    cls.get_ans['num_trees'] = lambda x: x == 3
...
class DecisionTreeClassifierTest(unittest.TestCase):
    ...
    def test_create(self):
        if False:
            print('Hello World!')
        model = tc.decision_tree_classifier.create(self.dtrain, target='label', validation_set=self.dtest, **self.param)
        self.assertTrue(model is not None)
        self.assertGreater(model.evaluate(self.dtest, 'accuracy')['accuracy'], 0.9)
        dtrain = self.dtrain[:]
        dtrain['label'] = 10
        self.assertRaises(ToolkitError, lambda : tc.decision_tree_classifier.create(self.dtrain, target='label_wrong', **self.param))
    ...
    def test_evaluate(self):
        ...
        self.sm_metrics = {'accuracy': tc.toolkits.evaluation.accuracy(t, c), 'auc': tc.toolkits.evaluation.auc(t, p), 'confusion_matrix': tc.toolkits.evaluation.confusion_matrix(t, c), 'f1_score': tc.toolkits.evaluation.f1_score(t, c), 'log_loss': tc.toolkits.evaluation.log_loss(t, p), 'precision': tc.toolkits.evaluation.precision(t, c), 'recall': tc.toolkits.evaluation.recall(t, c), 'roc_curve': tc.toolkits.evaluation.roc_curve(t, p)}
        model = self.model
    ...
    def _test_classifier_model(self, train, test, accuracy_threshold, target='label'):
        if False:
            return 10
        model = tc.decision_tree_classifier.create(train, target=target, validation_set=test, **self.param)
    ...
    def test_predict_new_dictionary_key(self):
        ...
        (train, test) = new_data.random_split(0.8, seed=1)
        test['dict_color_feature'] = test['dict_color_feature'].apply(lambda x: dict(list(x.items()) + list({'cap-color2': x['cap-color'] + 1}.items())))
        model = tc.decision_tree_classifier.create(train, target='label', **self.param)
        y1 = model.predict(test)
        y2 = model.predict(list(test))
        self.assertEqual(list(y1), list(y2))
...
class TestStringTarget(unittest.TestCase):
    ...
    def test_cat(self):
        ...
        model = tc.decision_tree_classifier.create(sf, 'target')
