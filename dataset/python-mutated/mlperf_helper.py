...
_PREFIX = '(?:{})?:::MLPv([0-9]+).([0-9]+).([0-9]+)'.format(_NCF_PREFIX)
...
LINE_PATTERN = re.compile('^{prefix} {benchmark} {timestamp} {callsite} {tag}(: |$){value}?$'.format(prefix=_PREFIX, benchmark=_BENCHMARK, timestamp=_TIMESTAMP, callsite=_CALLSITE, tag=_TAG, value=_VALUE))
