...
if __name__ == '__main__':
    ...
    if args.endpoint_url:
        ...
        with amazondax.AmazonDaxClient.resource(endpoint_url=args.endpoint_url) as dax:
            (test_start, test_end) = query_test(test_partition_key, test_sort_keys, test_iterations, dyn_resource=dax)
    else:
        print(f'Querying the table {test_iterations} times, using the Boto3 client.')
        (test_start, test_end) = query_test(test_partition_key, test_sort_keys, test_iterations)
