...
import aiohttp
from mage_ai.authentication.oauth.constants import GHE_CLIENT_ID_ENV_VAR, GHE_CLIENT_SECRET_ENV_VAR, OAUTH_PROVIDER_GHE, get_ghe_hostname
...
class GHEProvider(OauthProvider):
    ...
    async def get_access_token_response(self, code: str, **kwargs) -> Dict:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f'{self.hostname}/login/oauth/access_token', headers={'Accept': 'application/json'}, data=dict(client_id=os.getenv(GHE_CLIENT_ID_ENV_VAR), client_secret=os.getenv(GHE_CLIENT_SECRET_ENV_VAR), code=code), timeout=20) as response:
