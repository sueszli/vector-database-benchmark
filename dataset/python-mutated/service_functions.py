...
from .const import ATTR_CONTROLLER_ID, ATTR_IHC_ID, ATTR_VALUE, DOMAIN, IHC_CONTROLLER, IHC_CONTROLLER_INDEX, SERVICE_PULSE, SERVICE_SET_RUNTIME_VALUE_BOOL, SERVICE_SET_RUNTIME_VALUE_FLOAT, SERVICE_SET_RUNTIME_VALUE_INT
from .util import async_pulse, async_set_bool, async_set_float, async_set_int
SET_RUNTIME_VALUE_BOOL_SCHEMA = vol.Schema({vol.Required(ATTR_IHC_ID): cv.positive_int, vol.Required(ATTR_VALUE): cv.boolean, vol.Optional(ATTR_CONTROLLER_ID, default=0): cv.positive_int})
SET_RUNTIME_VALUE_INT_SCHEMA = vol.Schema({vol.Required(ATTR_IHC_ID): cv.positive_int, vol.Required(ATTR_VALUE): vol.Coerce(int), vol.Optional(ATTR_CONTROLLER_ID, default=0): cv.positive_int})
SET_RUNTIME_VALUE_FLOAT_SCHEMA = vol.Schema({vol.Required(ATTR_IHC_ID): cv.positive_int, vol.Required(ATTR_VALUE): vol.Coerce(float), vol.Optional(ATTR_CONTROLLER_ID, default=0): cv.positive_int})
PULSE_SCHEMA = vol.Schema({vol.Required(ATTR_IHC_ID): cv.positive_int, vol.Optional(ATTR_CONTROLLER_ID, default=0): cv.positive_int})
