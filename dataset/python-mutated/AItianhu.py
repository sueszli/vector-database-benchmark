...
class AItianhu(AsyncGeneratorProvider):
    ...
    @classmethod
    async def create_async_generator(cls, model: str, messages: Messages, proxy: str=None, cookies: dict=None, timeout: int=120, **kwargs) -> AsyncResult:
        if not cookies:
    ...
    def params(cls):
        if False:
            while True:
                i = 10
        params = [('model', 'str'), ('messages', 'list[dict[str, str]]'), ('stream', 'bool'), ('proxy', 'str'), ('temperature', 'float'), ('top_p', 'int')]
