...
_is_start = re.compile('\\s* switch \\s* \\( opcode \\)', re.VERBOSE).match
_match_target = re.compile('\\s* (?: TARGET \\s* \\( | case \\s* ) \\s* (\\w+) \\s* [:)]', re.VERBOSE).match
_ignored = re.compile('\\s* PREDICTED[A-Z_]*\\(', re.VERBOSE).match
_is_end = re.compile('\\s* } \\s* /\\* \\s* switch \\s* \\*/', re.VERBOSE).match
_find_pyversion = re.compile('\\#define \\s+ PY_VERSION \\s+ "([^"]+)"', re.VERBOSE).findall
