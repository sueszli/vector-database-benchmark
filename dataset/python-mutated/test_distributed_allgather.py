...
import cupy as cp
import torch
from ray.util.collective.tests.util import create_collective_workers, init_tensors_for_gather_scatter

@pytest.mark.parametrize('tensor_backend', ['cupy', 'torch'])
@pytest.mark.parametrize('array_size', [2, 2 ** 5, 2 ** 10, 2 ** 15, 2 ** 20, [2, 2], [5, 5, 5]])
def test_allgather_different_array_size(ray_start_distributed_2_nodes_4_gpus, array_size, tensor_backend):
    if False:
        while True:
            i = 10
    world_size = 4
