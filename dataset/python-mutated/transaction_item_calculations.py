...
class CancelEvents(BaseEvent):
    ...
PSPReference = str
AUTHORIZATION_EVENTS = [TransactionEventType.AUTHORIZATION_SUCCESS, TransactionEventType.AUTHORIZATION_FAILURE, TransactionEventType.AUTHORIZATION_ADJUSTMENT, TransactionEventType.AUTHORIZATION_REQUEST]

@dataclass
class ActionEventMap:
    without_psp_reference: list[TransactionEvent]
    authorization: dict[PSPReference, AuthorizationEvents] = field(default_factory=lambda : defaultdict(AuthorizationEvents))
    charge: dict[PSPReference, ChargeEvents] = field(default_factory=lambda : defaultdict(ChargeEvents))
    refund: dict[PSPReference, RefundEvents] = field(default_factory=lambda : defaultdict(RefundEvents))
    cancel: dict[PSPReference, CancelEvents] = field(default_factory=lambda : defaultdict(CancelEvents))
