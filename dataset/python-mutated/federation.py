...
from typing import TYPE_CHECKING, Dict, List, Mapping, Optional, Sequence, Tuple, Type, Union
from typing_extensions import Literal
from synapse.api.constants import Direction, EduTypes
from synapse.api.errors import Codes, SynapseError
from synapse.api.room_versions import RoomVersions
from synapse.api.urls import FEDERATION_UNSTABLE_PREFIX, FEDERATION_V2_PREFIX
from synapse.federation.transport.server._base import Authenticator, BaseFederationServlet
from synapse.http.servlet import parse_boolean_from_args, parse_integer_from_args, parse_string_from_args, parse_strings_from_args
...
class BaseFederationServerServlet(BaseFederationServlet):
    ...
    def __init__(self, hs: 'HomeServer', authenticator: Authenticator, ratelimiter: FederationRateLimiter, server_name: str):
        if False:
            while True:
                i = 10
...
class FederationSendServlet(BaseFederationServerServlet):
    ...
    async def on_PUT(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]], transaction_id: str) -> Tuple[int, JsonDict]:
        ...
        try:
            ...
            logger.info('Received txn %s from %s. (PDUs: %d, EDUs: %d)', transaction_id, origin, len(transaction_data.get('pdus', [])), len(transaction_data.get('edus', [])))
            if issue_8631_logger.isEnabledFor(logging.DEBUG):
                DEVICE_UPDATE_EDUS = [EduTypes.DEVICE_LIST_UPDATE, EduTypes.SIGNING_KEY_UPDATE]
                device_list_updates = [edu.get('content', {}) for edu in transaction_data.get('edus', []) if edu.get('edu_type') in DEVICE_UPDATE_EDUS]
                if device_list_updates:
                    issue_8631_logger.debug('received transaction [%s] including device list updates: %s', transaction_id, device_list_updates)
        except Exception as e:
            logger.exception(e)
            return (400, {'error': 'Invalid transaction'})
        (code, response) = await self.handler.on_incoming_transaction(origin, transaction_id, self.server_name, transaction_data)
...
class FederationEventServlet(BaseFederationServerServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], event_id: str) -> Tuple[int, Union[JsonDict, str]]:
...
class FederationStateV1Servlet(BaseFederationServerServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], room_id: str) -> Tuple[int, JsonDict]:
        return await self.handler.on_room_state_request(origin, room_id, parse_string_from_args(query, 'event_id', None, required=True))
...
class FederationStateIdsServlet(BaseFederationServerServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], room_id: str) -> Tuple[int, JsonDict]:
        return await self.handler.on_state_ids_request(origin, room_id, parse_string_from_args(query, 'event_id', None, required=True))
...
class FederationTimestampLookupServlet(BaseFederationServerServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], room_id: str) -> Tuple[int, JsonDict]:
        ...
        direction_str = parse_string_from_args(query, 'dir', allowed_values=['f', 'b'], required=True)
        direction = Direction(direction_str)
        return await self.handler.on_timestamp_to_event_request(origin, room_id, timestamp, direction)

class FederationQueryServlet(BaseFederationServerServlet):
    PATH = '/query/(?P<query_type>[^/]*)'
    CATEGORY = 'Federation requests'

    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], query_type: str) -> Tuple[int, JsonDict]:
...
class FederationMakeJoinServlet(BaseFederationServerServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], room_id: str, user_id: str) -> Tuple[int, JsonDict]:
        """
        Args:
        ...
        result = await self.handler.on_make_join_request(origin, room_id, user_id, supported_versions=supported_versions)
        return (200, result)
...
class FederationV1SendLeaveServlet(BaseFederationServerServlet):
    ...
    async def on_PUT(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]], room_id: str, event_id: str) -> Tuple[int, Tuple[int, JsonDict]]:
...
class FederationV2SendLeaveServlet(BaseFederationServerServlet):
    ...
    async def on_PUT(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]], room_id: str, event_id: str) -> Tuple[int, JsonDict]:
        result = await self.handler.on_send_leave_request(origin, content, room_id)
        return (200, result)
...
class FederationMakeKnockServlet(BaseFederationServerServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], room_id: str, user_id: str) -> Tuple[int, JsonDict]:
        supported_versions = parse_strings_from_args(query, 'ver', required=True, encoding='utf-8')
        result = await self.handler.on_make_knock_request(origin, room_id, user_id, supported_versions=supported_versions)
        return (200, result)
...
class FederationEventAuthServlet(BaseFederationServerServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], room_id: str, event_id: str) -> Tuple[int, JsonDict]:
...
class FederationV2SendJoinServlet(BaseFederationServerServlet):
    ...
    async def on_PUT(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]], room_id: str, event_id: str) -> Tuple[int, JsonDict]:
        ...
        result = await self.handler.on_send_join_request(origin, content, room_id, caller_supports_partial_state=partial_state)
        return (200, result)
...
class FederationV1InviteServlet(BaseFederationServerServlet):
    ...
    async def on_PUT(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]], room_id: str, event_id: str) -> Tuple[int, Tuple[int, JsonDict]]:
        result = await self.handler.on_invite_request(origin, content, room_version_id=RoomVersions.V1.identifier)
        return (200, (200, result))
...
class FederationV2InviteServlet(BaseFederationServerServlet):
    ...
    async def on_PUT(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]], room_id: str, event_id: str) -> Tuple[int, JsonDict]:
        ...
        result = await self.handler.on_invite_request(origin, event, room_version_id=room_version)
...
class FederationThirdPartyInviteExchangeServlet(BaseFederationServerServlet):
    ...
    async def on_PUT(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]], room_id: str) -> Tuple[int, JsonDict]:
...
class FederationClientKeysQueryServlet(BaseFederationServerServlet):
    ...
    async def on_POST(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]]) -> Tuple[int, JsonDict]:
...
class FederationUserDevicesQueryServlet(BaseFederationServerServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], user_id: str) -> Tuple[int, JsonDict]:
...
class FederationClientKeysClaimServlet(BaseFederationServerServlet):
    ...
    async def on_POST(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]]) -> Tuple[int, JsonDict]:
        ...
        response = await self.handler.on_claim_client_keys(key_query, always_include_fallback_keys=False)
        return (200, response)
...
class FederationUnstableClientKeysClaimServlet(BaseFederationServerServlet):
    ...
    async def on_POST(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]]) -> Tuple[int, JsonDict]:
        ...
        response = await self.handler.on_claim_client_keys(key_query, always_include_fallback_keys=True)
        return (200, response)
...
class FederationGetMissingEventsServlet(BaseFederationServerServlet):
    ...
    async def on_POST(self, origin: str, content: JsonDict, query: Dict[bytes, List[bytes]], room_id: str) -> Tuple[int, JsonDict]:
        limit = int(content.get('limit', 10))
        earliest_events = content.get('earliest_events', [])
        latest_events = content.get('latest_events', [])
        result = await self.handler.on_get_missing_events(origin, room_id=room_id, earliest_events=earliest_events, latest_events=latest_events, limit=limit)
        return (200, result)
...
class On3pidBindServlet(BaseFederationServerServlet):
    ...
    async def on_POST(self, origin: Optional[str], content: JsonDict, query: Dict[bytes, List[bytes]]) -> Tuple[int, JsonDict]:
        if 'invites' in content:
            last_exception = None
            for invite in content['invites']:
                try:
                    if 'signed' not in invite or 'token' not in invite['signed']:
                        message = 'Rejecting received notification of third-party invite without signed: %s' % (invite,)
                        logger.info(message)
                        raise SynapseError(400, message)
                    await self.handler.exchange_third_party_invite(invite['sender'], invite['mxid'], invite['room_id'], invite['signed'])
                except Exception as e:
                    last_exception = e
            if last_exception:
...
class FederationVersionServlet(BaseFederationServlet):
    ...
    async def on_GET(self, origin: Optional[str], content: Literal[None], query: Dict[bytes, List[bytes]]) -> Tuple[int, JsonDict]:
        return (200, {'server': {'name': 'Synapse', 'version': SYNAPSE_VERSION}})
...
class FederationRoomHierarchyServlet(BaseFederationServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Mapping[bytes, Sequence[bytes]], room_id: str) -> Tuple[int, JsonDict]:
        suggested_only = parse_boolean_from_args(query, 'suggested_only', default=False)
        return (200, await self.handler.get_federation_hierarchy(origin, room_id, suggested_only))
...
class RoomComplexityServlet(BaseFederationServlet):
    ...
    async def on_GET(self, origin: str, content: Literal[None], query: Dict[bytes, List[bytes]], room_id: str) -> Tuple[int, JsonDict]:
        is_public = await self._store.is_room_world_readable_or_publicly_joinable(room_id)
        if not is_public:
...
class FederationAccountStatusServlet(BaseFederationServerServlet):
    ...
    async def on_POST(self, origin: str, content: JsonDict, query: Mapping[bytes, Sequence[bytes]], room_id: str) -> Tuple[int, JsonDict]:
        if 'user_ids' not in content:
            raise SynapseError(400, "Required parameter 'user_ids' is missing", Codes.MISSING_PARAM)
        (statuses, failures) = await self._account_handler.get_account_statuses(content['user_ids'], allow_remote=False)
