...
for backend in os.listdir(os.path.join(ivy.__path__[0].rpartition(os.path.sep)[0], _backends_subpackage_path.replace('.', os.path.sep))):
    if not backend[0].isalpha():
        continue
    sub_backends_dir = os.path.join(ivy.__path__[0].rpartition(os.path.sep)[0], _backends_subpackage_path.replace('.', os.path.sep), backend, 'sub_backends')
    for sub_backend in os.listdir(sub_backends_dir):
        if not sub_backend[0].isalpha():
            continue
        _sub_backend_dict[sub_backend] = f'{_backends_subpackage_path}.{backend}.sub_backends.{sub_backend}'
        try:
