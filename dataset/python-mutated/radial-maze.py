...
for i in range(n_arms):
    ...
    for t in np.linspace(t0, t1, 25):
        V.append(1.0 * inner_radius * np.array([np.cos(t), np.sin(t)]))
...
collection = PolyCollection(C, closed=True, linewidth=2, facecolors=colors, edgecolors='white')
ax.add_collection(collection)
for i in range(n_arms):
    theta = i / n_arms * 2 * np.pi
    x = (outer_radius + 1) * np.cos(theta)
    y = (outer_radius + 1) * np.sin(theta)
    ax.text(x, y, chr(ord('A') + i), weight='bold', rotation=i / n_arms * 360 - 90, va='center', ha='center', size='xx-large')
