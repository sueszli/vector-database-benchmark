...
class MatrixBandPartBenchmark(test_lib.Benchmark):
    shapes = [(10, 16, 16), (10, 101, 101), (10, 256, 256), (10, 1000, 1000), (10, 1024, 1024), (10, 2048, 2048), (10, 10, 4, 4), (10, 10, 10, 10), (10, 10, 16, 16), (10, 10, 101, 101), (10, 10, 256, 256), (10, 10, 1000, 1000), (10, 10, 1024, 1024), (10, 10, 2048, 2048)]
...
if __name__ == '__main__':
    dtypes = (np.bool_, np.int32, np.int64, np.float16, dtypes_lib.bfloat16.as_numpy_dtype, np.float32, np.float64, np.complex64, np.complex128)
    for dtype in dtypes:
        for batch_shape in ((), (2,), (1, 3, 2)):
            for rows in (1, 2, 7, 23):
                for cols in (1, 2, 7, 23):
                    shape = (rows, cols)
                    name = '%s_%s' % (dtype.__name__, '_'.join(map(str, batch_shape + shape)))
                    _AddTest(MatrixBandPartTest, 'MatrixBandPart', name, _GetMatrixBandPartTest(dtype, batch_shape, shape))
        ...
        for batch_shape in ((), (2,)):
            for rows in (1, 2, 7):
                for cols in (1, 2, 7):
                    shape = (rows, cols)
                    name = '%s_%s' % (dtype.__name__, '_'.join(map(str, batch_shape + shape)))
                    _AddTest(MatrixBandPartGradTest, 'MatrixBandPartGrad', name, _GetMatrixBandPartGradTest(dtype, batch_shape, shape))
    test_lib.main()