...
from fairseq.data import AppendTokenDataset, ConcatDataset, Dictionary, IdDataset, LMContextWindowDataset, MonolingualDataset, NestedDictionaryDataset, NumelDataset, PadDataset, PrependTokenDataset, ResamplingDataset, SortDataset, StripTokenDataset, TokenBlockDataset, TruncatedDictionary, data_utils
...
class MultilingualLanguageModelingConfig(FairseqDataclass):
    ...
    add_bos_token: bool = field(default=False, metadata={'help': 'prepend lang id token <dialect>'})
    max_source_positions: Optional[int] = field(default=None, metadata={'help': 'max number of tokens in the source sequence'})
    max_target_positions: Optional[int] = field(default=None, metadata={'help': 'max number of tokens in the target sequence'})
    pad_to_fixed_length: Optional[bool] = field(default=False, metadata={'help': 'pad to fixed length'})
    pad_to_fixed_bsz: Optional[bool] = field(default=False, metadata={'help': 'boolean to pad to fixed batch size'})
    multilang_sampling_alpha: Optional[float] = field(default=1.0, metadata={'help': 'smoothing alpha for sample rations across multiple datasets'})
    ...
    langs: str = field(default='', metadata={'help': 'comma-separated list of languages (default: all directories in data path)'})
    baseline_model_langs: str = field(default='', metadata={'help': 'comma-separated list of languages in the baseline model (default: none)'})
    baseline_model: str = field(default='', metadata={'help': 'path to the baseline model (default: none)'})
    lang_to_offline_shard_ratio: str = field(default='', metadata={'help': 'absolute path of tsv file location to indicate lang to offline shard ratio.'})
