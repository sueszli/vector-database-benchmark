...
class SpotifyWrapper:
    ...
    async def get_categories(self, ctx: Context=None) -> List[MutableMapping]:
        ...
        with contextlib.suppress(KeyError):
            if result['error']['status'] == 401:
                raise SpotifyFetchError(message=_('The Spotify API key or client secret has not been set properly. \nUse `{prefix}audioset spotifyapi` for instructions.'))
    ...
    async def get_playlist_from_category(self, category: str, ctx: Context=None):
        ...
        return [{'name': c['name'], 'uri': c['uri'], 'url': c.get('external_urls', {}).get('spotify'), 'tracks': c.get('tracks', {}).get('total', 'Unknown')} async for c in AsyncIter(playlists) if c]