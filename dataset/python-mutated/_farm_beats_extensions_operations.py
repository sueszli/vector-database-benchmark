...
class FarmBeatsExtensionsOperations:
    ...
    @distributed_trace
    def list(self, farm_beats_extension_ids: Optional[List[str]]=None, farm_beats_extension_names: Optional[List[str]]=None, extension_categories: Optional[List[str]]=None, publisher_ids: Optional[List[str]]=None, max_page_size: int=50, **kwargs: Any) -> AsyncIterable['_models.FarmBeatsExtension']:
        if False:
            for i in range(10):
                print('nop')
        ...
        def prepare_request(next_link=None):
            if False:
                while True:
                    i = 10
            if not next_link:
                request = build_list_request(farm_beats_extension_ids=farm_beats_extension_ids, farm_beats_extension_names=farm_beats_extension_names, extension_categories=extension_categories, publisher_ids=publisher_ids, max_page_size=max_page_size, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
    ...
    async def get(self, farm_beats_extension_id: str, **kwargs: Any) -> _models.FarmBeatsExtension:
        ...
        request = build_get_request(farm_beats_extension_id=farm_beats_extension_id, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=False, **kwargs)
        response = pipeline_response.http_response
