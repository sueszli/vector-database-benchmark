...
@PIPELINES.register_module(Tasks.nerf_recon_acc, module_name=Pipelines.nerf_recon_acc)
class NeRFReconAccPipeline(Pipeline):
    ...
    def __init__(self, model, data_type='blender', use_mask=True, ckpt_path='', save_mesh=False, n_test_traj_steps=120, test_ray_chunk=1024, device='gpu', **kwargs):
        if False:
            for i in range(10):
                print('nop')
        ...
        model = Model.from_pretrained(model, device=device, model_prefetched=True, invoked_by=Invoke.PIPELINE, data_type=data_type, use_mask=use_mask, ckpt_path=ckpt_path, save_mesh=save_mesh, n_test_traj_steps=n_test_traj_steps, test_ray_chunk=test_ray_chunk) if is_model(model) else model
