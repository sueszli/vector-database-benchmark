...
async def get_modified_files_in_branch_remote(current_git_branch: str, current_git_revision: str, diffed_branch: str='origin/master') -> Set[str]:
...
async def get_modified_files_in_commit_remote(current_git_branch: str, current_git_revision: str) -> Set[str]:
    async with Connection(DAGGER_CONFIG) as dagger_client:
        modified_files = await dagger_client.container().from_('alpine/git:latest').with_workdir('/repo').with_exec(['init']).with_env_variable('CACHEBUSTER', current_git_revision).with_exec(['remote', 'add', '--fetch', '--track', current_git_branch, 'origin', AIRBYTE_REPO_URL]).with_exec(['checkout', '-t', f'origin/{current_git_branch}']).with_exec(['diff-tree', '--no-commit-id', '--name-only', current_git_revision, '-r']).stdout()
    return set(modified_files.split('\n'))
