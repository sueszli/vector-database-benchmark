...
import json
import dataclasses
from typing import Any, Callable, Dict as DictType, Iterable, Iterator, List as ListType, Optional, Pattern, Sequence, Tuple, Union
import yaml
...
from qutebrowser.utils import standarddir, utils, qtutils, urlutils, urlmatch, usertypes, log
from qutebrowser.keyinput import keyutils
from qutebrowser.browser.network import pac

class _SystemProxy:
    pass
SYSTEM_PROXY = _SystemProxy()
...
class ColorSystem(MappingType):
    ...
    MAPPING = {'rgb': (QColor.Spec.Rgb, 'Interpolate in the RGB color system.'), 'hsv': (QColor.Spec.Hsv, 'Interpolate in the HSV color system.'), 'hsl': (QColor.Spec.Hsl, 'Interpolate in the HSL color system.'), 'none': (None, "Don't show a gradient.")}

class IgnoreCase(MappingType):
    """Whether to search case insensitively."""
    MAPPING = {'always': (usertypes.IgnoreCase.always, 'Search case-insensitively.'), 'never': (usertypes.IgnoreCase.never, 'Search case-sensitively.'), 'smart': (usertypes.IgnoreCase.smart, 'Search case-sensitively if there are capital characters.')}
...
class Position(MappingType):
    ...
    MAPPING = {'top': (QTabWidget.TabPosition.North, None), 'bottom': (QTabWidget.TabPosition.South, None), 'left': (QTabWidget.TabPosition.West, None), 'right': (QTabWidget.TabPosition.East, None)}

class TextAlignment(MappingType):
    """Alignment of text."""
    MAPPING = {'left': (Qt.AlignmentFlag.AlignLeft, None), 'right': (Qt.AlignmentFlag.AlignRight, None), 'center': (Qt.AlignmentFlag.AlignCenter, None)}

class ElidePosition(MappingType):
    """Position of ellipsis in truncated text."""
    MAPPING = {'left': (Qt.TextElideMode.ElideLeft, None), 'right': (Qt.TextElideMode.ElideRight, None), 'middle': (Qt.TextElideMode.ElideMiddle, None), 'none': (Qt.TextElideMode.ElideNone, None)}
