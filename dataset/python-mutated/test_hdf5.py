...
unsupported_cols = {name: col for (name, col) in mixin_cols.items() if isinstance(col, np.ndarray) and col.dtype.kind == 'O'}
mixin_cols = {name: col for (name, col) in mixin_cols.items() if name not in unsupported_cols}
ALL_DTYPES = [np.uint8, np.uint16, np.uint32, np.uint64, np.int8, np.int16, np.int32, np.int64, np.float32, np.float64, np.bool_, '|S3']
