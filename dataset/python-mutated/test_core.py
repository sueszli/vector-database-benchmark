...
import jsonschema
import pytest
from airbyte_protocol.models import AirbyteRecordMessage, AirbyteStream, AirbyteTraceMessage, ConfiguredAirbyteCatalog, ConfiguredAirbyteStream, ConnectorSpecification, Status, SyncMode, TraceType, Type
from connector_acceptance_test.base import BaseTest
from connector_acceptance_test.config import BasicReadTestConfig, Config, ConnectionTestConfig, DiscoveryTestConfig, EmptyStreamConfiguration, ExpectedRecordsConfig, IgnoredFieldsConfiguration, SpecTestConfig
...
from connector_acceptance_test.utils.common import build_configured_catalog_from_custom_catalog, build_configured_catalog_from_discovered_catalog_and_empty_streams, find_all_values_for_key_in_schema, find_keyword_schema
from connector_acceptance_test.utils.compare import diff_dicts
from connector_acceptance_test.utils.json_schema_helper import JsonSchemaHelper, flatten_tuples, get_expected_schema_structure, get_object_structure, get_paths_in_connector_config
...
class TestSpec(BaseTest):
    ...
    async def skip_backward_compatibility_tests_fixture(self, inputs: SpecTestConfig, previous_connector_docker_runner: ConnectorRunner, previous_connector_spec: ConnectorSpecification, actual_connector_spec: ConnectorSpecification) -> bool:
    ...
    async def test_image_labels(self, docker_runner: ConnectorRunner, connector_metadata: dict):
        ...
        assert await docker_runner.get_container_label('io.airbyte.name') == connector_metadata['data']['dockerRepository'], 'io.airbyte.name must be equal to dockerRepository in metadata.yaml'
        assert await docker_runner.get_container_label('io.airbyte.version') == connector_metadata['data']['dockerImageTag'], 'io.airbyte.version must be equal to dockerImageTag in metadata.yaml'
...
class TestDiscovery(BaseTest):
    ...
    VALID_TYPE_FORMAT_COMBINATIONS = [({'string'}, 'date'), ({'string'}, 'date-time'), ({'string', 'null'}, 'date'), ({'string', 'null'}, 'date-time')]
    VALID_TYPE_AIRBYTE_TYPE_COMBINATIONS = [({'string'}, 'timestamp_with_timezone'), ({'string'}, 'timestamp_without_timezone'), ({'string', 'null'}, 'timestamp_with_timezone'), ({'integer'}, 'integer'), ({'integer', 'null'}, 'integer'), ({'number'}, 'integer'), ({'number', 'null'}, 'integer')]

    @pytest.fixture()
    async def skip_backward_compatibility_tests_for_version(self, inputs: DiscoveryTestConfig, previous_connector_docker_runner: ConnectorRunner):
        previous_connector_version = await previous_connector_docker_runner.get_container_label('io.airbyte.version')
        if previous_connector_version == inputs.backward_compatibility_tests_config.disable_for_version:
            pytest.skip(f'Backward compatibility tests are disabled for version {previous_connector_version}.')
        return False
    ...
    @pytest.fixture(name='skip_backward_compatibility_tests')
    async def skip_backward_compatibility_tests_fixture(self, skip_backward_compatibility_tests_for_version: bool, previous_connector_docker_runner: ConnectorRunner, discovered_catalog: MutableMapping[str, AirbyteStream], previous_discovered_catalog: MutableMapping[str, AirbyteStream]) -> bool:
...
class TestBasicRead(BaseTest):
    ...
    async def test_read(self, connector_config, configured_catalog, expect_records_config: ExpectedRecordsConfig, should_validate_schema: Boolean, should_validate_data_points: Boolean, should_fail_on_extra_columns: Boolean, empty_streams: Set[EmptyStreamConfiguration], ignored_fields: Optional[Mapping[str, List[IgnoredFieldsConfiguration]]], expected_records_by_stream: MutableMapping[str, List[MutableMapping]], docker_runner: ConnectorRunner, detailed_logger):
        ...
        for (pks, record) in primary_keys_for_records(streams=configured_catalog.streams, records=records):
            for (pk_path, pk_value) in pks.items():
                assert pk_value is not None, f'Primary key subkeys {repr(pk_path)} have null values or not present in {record.stream} stream records.'
        ...
        if expected_records_by_stream:
            self._validate_expected_records(records=records, expected_records_by_stream=expected_records_by_stream, flags=expect_records_config, ignored_fields=ignored_fields, detailed_logger=detailed_logger)
    ...
    async def test_airbyte_trace_message_on_failure(self, connector_config, inputs: BasicReadTestConfig, docker_runner: ConnectorRunner):
        if not inputs.expect_trace_message_on_failure:
            ...
            return
        invalid_configured_catalog = ConfiguredAirbyteCatalog(streams=[ConfiguredAirbyteStream.construct(stream=AirbyteStream(name='__AIRBYTE__stream_that_does_not_exist', json_schema={'type': 'object', 'properties': {'f1': {'type': 'string'}}}, supported_sync_modes=[SyncMode.full_refresh]), sync_mode='INVALID', destination_sync_mode='INVALID')])
