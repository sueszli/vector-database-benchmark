...
class TestPreCheckoutQueryWithoutRequest(TestPreCheckoutQueryBase):
    ...
    async def test_answer(self, monkeypatch, pre_checkout_query):
        ...
        assert check_shortcut_signature(PreCheckoutQuery.answer, Bot.answer_pre_checkout_query, ['pre_checkout_query_id'], [])
        assert await check_shortcut_call(pre_checkout_query.answer, pre_checkout_query.get_bot(), 'answer_pre_checkout_query')
        assert await check_defaults_handling(pre_checkout_query.answer, pre_checkout_query.get_bot())
        monkeypatch.setattr(pre_checkout_query.get_bot(), 'answer_pre_checkout_query', make_assertion)
