...
def dagster_celery_k8s_editable_cm(cwd: str) -> Iterator[None]:
    ...
    with copy_directories(get_core_celery_k8s_dirs() + ['python_modules/dagster-graphql', 'python_modules/dagster-webserver', 'python_modules/libraries/dagster-aws'], cwd):
        yield
CUSTOM_BUILD_CONTEXTMANAGERS: Dict[str, Callable] = {'user-code-example': user_code_example_cm, 'user-code-example-editable': user_code_example_editable_cm, 'dagster-k8s-editable': dagster_k8s_editable_cm, 'dagster-celery-k8s-editable': dagster_celery_k8s_editable_cm}
