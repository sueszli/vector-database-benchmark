...
for dtype in dtypes_to_test:
    for (t_a, t_b, adj_a, adj_b, t_out, conj_out, batched) in itertools.product(*([False, True],) * 7):
        ...
        def create_mat_mul_test_fn(dtype_, t_a_, t_b_, adj_a_, adj_b_, t_out_, conj_out_, batched_):
            ...
            return test_fn
        name = '_testLargeBatchSparseMatrixMatMulGrad_dtype_%s_t_a_%s_t_b_%s_adj_a_%s_adj_b_%s_t_out_%s_conj_out_%s_batched_%s' % (dtype.__name__, t_a, t_b, adj_a, adj_b, t_out, conj_out, batched)
        _add_test(CSRSparseMatrixDenseMatMulGradTest, 'CSRSparseMatrixGradTest', name, create_mat_mul_test_fn(dtype, t_a, t_b, adj_a, adj_b, t_out, conj_out, batched))
