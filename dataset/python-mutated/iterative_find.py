...
class IterativeFinder(AsyncIterator):
    ...
    async def _send_probe(self, peer: 'KademliaPeer'):
        try:
            ...
            log.debug('%s[%x] cancelled probe', type(self).__name__, id(self))
            raise
    ...
    async def _aclose(self, reason='?'):
        log.debug('%s[%x] [%s] shutdown because %s: %i active nodes %i contacted %i produced %i queued', type(self).__name__, id(self), self.key.hex()[:8], reason, len(self.active), len(self.contacted), self.iteration_count, self.iteration_queue.qsize())
        self.running = False
        self.iteration_queue.put_nowait(None)
    ...
    async def aclose(self):
        ...
        log.debug('%s[%x] [%s] async close completed', type(self).__name__, id(self), self.key.hex()[:8])
...
class IterativeNodeFinder(IterativeFinder):
    ...
    async def send_probe(self, peer: 'KademliaPeer') -> FindNodeResponse:
        log.debug('probe %s:%d (%s) for NODE %s', peer.address, peer.udp_port, peer.node_id.hex()[:8] if peer.node_id else '', self.key.hex()[:8])
...
class IterativeValueFinder(IterativeFinder):
    ...
    async def send_probe(self, peer: 'KademliaPeer') -> FindValueResponse:
        log.debug('probe %s:%d (%s) for VALUE %s', peer.address, peer.udp_port, peer.node_id.hex()[:8], self.key.hex()[:8])
        ...
        for compact_addr in parsed.found_compact_addresses:
            try:
                ...
                log.warning('misbehaving peer %s:%i returned invalid peer for blob', peer.address, peer.udp_port)
        ...
        log.debug('probed %s:%i page %i, %i known', peer.address, peer.udp_port, page, already_known + len(parsed.found_compact_addresses))
