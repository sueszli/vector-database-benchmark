...
flags.DEFINE_string('data_dir', None, 'The input data dir at which training and evaluation tf record files will be saved.')
flags.DEFINE_string('meta_data_file_path', None, 'The path in which input meta data will be written.')
flags.DEFINE_enum('dataset', 'ml-20m', ['ml-1m', 'ml-20m'], 'Dataset to be trained/evaluated.')
flags.DEFINE_enum('constructor_type', 'bisection', ['bisection', 'materialized'], 'Strategy to use for generating false negatives. materialized has a precompute that scales badly, but a faster per-epoch construction time and can be faster on very large systems.')
...
flags.DEFINE_integer('train_prebatch_size', 99000, 'Batch size to be used for prebatching the dataset for training.')
flags.DEFINE_integer('eval_prebatch_size', 99000, 'Batch size to be used for prebatching the dataset for training.')
FLAGS = flags.FLAGS
