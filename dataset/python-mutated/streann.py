...
@pluginmatcher(re.compile('https?://ott\\.streann\\.com/s(?:treaming|-secure)/player\\.html'))
@pluginmatcher(re.compile('\n    https?://(?:www\\.)?(?:\n        centroecuador\\.ec\n        |\n        columnaestilos\\.com\n        |\n        crc\\.cr/estaciones/\n        |\n        evtv\\.online/noticias-de-venezuela/\n        |\n        telecuracao\\.com\n    )\n', re.VERBOSE))
@pluginargument('url', metavar='URL', type=str, help='Source URL where the iframe is located, only required for direct URLs of ott.streann.com')
class Streann(Plugin):
    ...
    passphrase_re = re.compile('CryptoJS\\.AES\\.decrypt\\(.*?,\\s*([\'"])(?P<passphrase>(?:(?!\\1).)*)\\1\\s*?\\);')
    _device_id = None
    _domain = None
