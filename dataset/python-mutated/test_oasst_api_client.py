...
async def test_can_post_interaction(oasst_api_client_mocked: OasstApiClient):
    assert await oasst_api_client_mocked.post_interaction(protocol_schema.TextReplyToMessage(type='text_reply_to_message', message_id='123', user_message_id='321', text='This is my reply', lang='en', user=protocol_schema.User(id='123', display_name='lomz', auth_method='discord'))) is not None

@pytest.mark.asyncio
async def test_can_handle_oasst_error_from_api(oasst_api_client_fake_http: OasstApiClient, mock_http_session: MockClientSession):
    response_body = protocol_schema.OasstErrorResponse(error_code=OasstErrorCode.GENERIC_ERROR, message='Some error')
    status_code = 400
    mock_http_session.set_response(mock.AsyncMock(status=status_code, text=mock.AsyncMock(return_value=response_body.json()), json=mock.AsyncMock(return_value=response_body.dict())))
    with pytest.raises(OasstError):
        await oasst_api_client_fake_http.post('/some-path', data={})

@pytest.mark.asyncio
async def test_can_handle_unknown_error_from_api(oasst_api_client_fake_http: OasstApiClient, mock_http_session: MockClientSession):
    response_body = 'Internal Server Error'
    status_code = 500
    mock_http_session.set_response(mock.AsyncMock(status=status_code, text=mock.AsyncMock(return_value=response_body), json=mock.AsyncMock(return_value=None)))
    with pytest.raises(OasstError):
        await oasst_api_client_fake_http.post('/some-path', data={})