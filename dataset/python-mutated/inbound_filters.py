...
FILTER_STAT_KEYS_TO_VALUES = {FilterStatKeys.IP_ADDRESS: TSDBModel.project_total_received_ip_address, FilterStatKeys.RELEASE_VERSION: TSDBModel.project_total_received_release_version, FilterStatKeys.ERROR_MESSAGE: TSDBModel.project_total_received_error_message, FilterStatKeys.BROWSER_EXTENSION: TSDBModel.project_total_received_browser_extensions, FilterStatKeys.LEGACY_BROWSER: TSDBModel.project_total_received_legacy_browsers, FilterStatKeys.LOCALHOST: TSDBModel.project_total_received_localhost, FilterStatKeys.WEB_CRAWLER: TSDBModel.project_total_received_web_crawlers, FilterStatKeys.INVALID_CSP: TSDBModel.project_total_received_invalid_csp, FilterStatKeys.CORS: TSDBModel.project_total_received_cors, FilterStatKeys.DISCARDED_HASH: TSDBModel.project_total_received_discarded, FilterStatKeys.HEALTH_CHECK: TSDBModel.project_total_healthcheck}
...
class _FilterSerializer(serializers.Serializer):
    active = serializers.BooleanField(help_text='Toggle the browser-extensions, localhost, filtered-transaction, or web-crawlers filter on or off.', required=False)
...
_localhost_filter = _FilterSpec(id=FilterStatKeys.LOCALHOST, name='Filter out events coming from localhost', description='This applies to both IPv4 (``127.0.0.1``) and IPv6 (``::1``) addresses.')
_browser_extensions_filter = _FilterSpec(id=FilterStatKeys.BROWSER_EXTENSION, name='Filter out errors known to be caused by browser extensions', description='Certain browser extensions will inject inline scripts and are known to cause errors.')

class _LegacyBrowserFilterSerializer(_FilterSerializer):
    subfilters = serializers.MultipleChoiceField(help_text="\nSpecifies which legacy browser filters should be active. Anything excluded from the list will be\ndisabled. The options are:\n- `ie_pre_9` - Internet Explorer Version 8 and lower\n- `ie9` - Internet Explorer Version 9\n- `ie10` - Internet Explorer Version 10\n- `ie11` - Internet Explorer Version 11\n- `safari_pre_6` - Safari Version 5 and lower\n- `opera_pre_15` - Opera Version 14 and lower\n- `opera_mini_pre_8` - Opera Mini Version 8 and lower\n- `android_pre_4` - Android Version 3 and lower\n- 'edge_pre_79' - Edge Version 18 and lower (non Chromium based)\n", choices=['ie_pre_9', 'ie9', 'ie10', 'ie11', 'opera_pre_15', 'android_pre_4', 'safari_pre_6', 'opera_mini_pre_8', 'edge_pre_79'], required=False)
_legacy_browsers_filter = _FilterSpec(id=FilterStatKeys.LEGACY_BROWSER, name='Filter out known errors from legacy browsers', description='Older browsers often give less accurate information, and while they may report valid issues, the context to understand them is incorrect or missing.', serializer_cls=_LegacyBrowserFilterSerializer)
_web_crawlers_filter = _FilterSpec(id=FilterStatKeys.WEB_CRAWLER, name='Filter out known web crawlers', description='Some crawlers may execute pages in incompatible ways which then cause errors that are unlikely to be seen by a normal user.')
_healthcheck_filter = _FilterSpec(id=FilterStatKeys.HEALTH_CHECK, name='Filter out health check transactions', description='Filter transactions that match most common naming patterns for health checks.', serializer_cls=None, config_name='ignoreTransactions')