...
V = np.array([[50, 23, 20, 7], [45, 33, 12, 10], [35, 43, 10, 12], [54, 23, 15, 8], [65, 23, 7, 5]])
...
L = np.zeros(len(V))
for i in range(4):
    color = lighten_color(cmap(3), 1.0 - i / 4)
    ax.barh(Y, V[:, i], left=L, color=color, label=ratings[i])
    for j in range(len(V)):
        ax.text(L[j] + V[j, i] / 2, Y[j], '%d%%' % V[j, i], zorder=10, ha='center', va='center', color='white', size='x-small')
    L += V[:, i]
