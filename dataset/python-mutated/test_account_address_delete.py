...
def test_customer_delete_own_address(user_api_client, customer_user):
    ...
    assert generate_address_search_document_value(address_obj) not in user.search_document

@freeze_time('2022-05-12 12:00:00')
@patch('saleor.plugins.webhook.plugin.get_webhooks_for_event')
@patch('saleor.plugins.webhook.plugin.trigger_webhooks_async')
def test_customer_delete_address_trigger_webhook(mocked_webhook_trigger, mocked_get_webhooks_for_event, any_webhook, user_api_client, customer_user, settings):
    if False:
        print('Hello World!')
    ...
    mocked_webhook_trigger.assert_called_with(*generate_address_webhook_call_args(address, WebhookEventAsyncType.ADDRESS_DELETED, user_api_client.user, any_webhook))
