...
trainer = AutoTSTrainer(dt_col='datetime', target_col=['AvgRate', 'total'], horizon=1, extra_features_col=None)
look_back = (36, 84)
from bigdl.chronos.autots.deprecated.preprocessing.utils import train_val_test_split
(train_df, val_df, test_df) = train_val_test_split(df, val_ratio=0.1, test_ratio=0.1, look_back=look_back[0])
ts_pipeline = trainer.fit(train_df, val_df, recipe=MTNetGridRandomRecipe(num_rand_samples=1, time_step=[12], long_num=[6], ar_size=[6], cnn_height=[4], cnn_hid_size=[32], training_iteration=1, epochs=20, batch_size=[1024]), metric='mse')
