...
class OrderLineDiscountRemove(OrderDiscountCommon):
    order_line = graphene.Field(OrderLine, description='Order line which has removed discount.')
    order = graphene.Field(Order, description='Order which is related to line which has removed discount.')

    class Arguments:
        order_line_id = graphene.ID(description='ID of a order line to remove its discount', required=True)
