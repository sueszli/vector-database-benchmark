...
class TestConvBiasOneDNNFusePass(PassAutoScanTest):
    ...
    def sample_program_config(self, draw):
        ...
        if draw(st.booleans()):
            ...
            inputs = {'Input': ['input_x'], 'Filter': ['filter'], 'Bias': ['conv_bias']}
            weights = {'filter': TensorConfig(shape=f_shape), 'bias': TensorConfig(shape=bias_shape), 'conv_bias': TensorConfig(shape=conv_bias_shape)}
            use_mkldnn = True
        else:
            inputs = {'Input': ['input_x'], 'Filter': ['filter']}
            weights = {'filter': TensorConfig(shape=f_shape), 'bias': TensorConfig(shape=bias_shape)}
            use_mkldnn = False
        conv2d_op = OpConfig(conv_type, inputs=inputs, outputs={'Output': ['conv2d_out']}, strides=strides, padding_algorithm=padding_algorithm, paddings=padding, groups=groups, dilations=dilations, data_format=data_format, use_mkldnn=use_mkldnn)
    ...
    def test(self):
        if False:
            print('Hello World!')
        self.run_and_statis(quant=False, passes=['conv_bias_mkldnn_fuse_pass'], max_examples=130)
