...
class DebugEndpoint(RESTEndpoint):
    ...
    @docs(tags=['Debug'], summary='Return information about the slots in the tunnel overlay.', responses={200: {'schema': schema(CircuitSlotsResponse={'slots': [schema(CircuitSlot={'random': Integer, 'competing': Integer})]})}})
    async def get_circuit_slots(self, request):
        ...
        return RESTResponse({'slots': {'random': random_slots, 'competing': competing_slots}})

    @docs(tags=['Debug'], summary='Return information about files opened by Tribler.', responses={200: {'schema': schema(OpenFilesResponse={'open_files': [schema(OpenFile={'path': String, 'fd': Integer})]})}})
    async def get_open_files(self, request):
        my_process = psutil.Process()
        return RESTResponse({'open_files': [{'path': open_file.path, 'fd': open_file.fd} for open_file in my_process.open_files()]})

    @docs(tags=['Debug'], summary='Return information about open sockets.', responses={200: {'schema': schema(OpenSocketsResponse={'open_sockets': [schema(OpenSocket={'family': Integer, 'status': String, 'laddr': String, 'raddr': String, 'type': Integer})]})}})
    async def get_open_sockets(self, request):
        my_process = psutil.Process()
        sockets = []
        for open_socket in my_process.connections():
            sockets.append({'family': open_socket.family, 'status': open_socket.status, 'laddr': '%s:%d' % open_socket.laddr if open_socket.laddr else '-', 'raddr': '%s:%d' % open_socket.raddr if open_socket.raddr else '-', 'type': open_socket.type})
        return RESTResponse({'open_sockets': sockets})

    @docs(tags=['Debug'], summary='Return information about running threads.', responses={200: {'schema': schema(ThreadsResponse={'threads': [schema(Thread={'thread_id': Integer, 'thread_name': String, 'frames': [String]})]})}})
    async def get_threads(self, request):
        return RESTResponse({'threads': get_threads_info()})

    @docs(tags=['Debug'], summary='Return information about CPU usage history.', responses={200: {'schema': schema(CPUHistoryResponse={'cpu_history': [schema(CPUHistory={'time': Integer, 'cpu': Float})]})}})
    ...
    @docs(tags=['Debug'], summary='Return information about memory usage history.', responses={200: {'schema': schema(MemoryHistoryResponse={'memory_history': [schema(MemoryHistory={'time': Integer, 'mem': Integer})]})}})
    ...
    @docs(tags=['Debug'], summary='Return a Meliae-compatible dump of the memory contents.', responses={200: {'description': 'The content of the memory dump file'}})
    async def get_memory_dump(self, request):
        ...
        return RESTResponse(content, headers={'Content-Type', 'application/json', 'Content-Disposition', 'attachment; filename=tribler_memory_dump_%s.json' % date_str})

    @docs(tags=['Debug'], summary='Return content of core or gui log file & max_lines requested.', parameters=[{'in': 'query', 'name': 'process', 'description': 'Specifies which log to return', 'enum': ['core', 'gui'], 'type': 'string', 'required': False}, {'in': 'query', 'name': 'max_lines', 'description': 'Maximum number of lines to return from the log file', 'type': 'integer', 'required': False}], responses={200: {'schema': schema(LogFileResponse={'max_lines': Integer, 'content': String})}})
    ...
    @docs(tags=['Debug'], summary='Return information about the state of the profiler.', responses={200: {'schema': schema(ProfilerStateResponse={'state': (String, 'State of the profiler (STARTED or STOPPED)')})}})
    ...
    @docs(tags=['Debug'], summary='Start the profiler.', responses={200: {'schema': schema(StartProfilerResponse={'success': Boolean})}})
    async def start_profiler(self, _):
        self.resource_monitor.profiler.start()
        return RESTResponse({'success': True})

    @docs(tags=['Debug'], summary='Stop the profiler.', responses={200: {'schema': schema(StopProfilerResponse={'success': Boolean})}})
    ...
    @docs(tags=['Debug'], summary='Generates a test exception in the Core process.', responses={200: {'schema': schema(CoreTestException={'success': Boolean})}})
