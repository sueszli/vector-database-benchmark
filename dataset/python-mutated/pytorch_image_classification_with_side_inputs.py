...
def parse_known_args(argv):
    ...
    parser.add_argument('--topic', dest='topic', help='PubSub topic emitting absolute path to the images.Path must be accessible by the pipeline.')
    parser.add_argument('--model_path', '--initial_model_path', dest='model_path', default='gs://apache-beam-samples/run_inference/resnet152.pth', help="Path to the initial model's state_dict. This will be used until the first model update occurs.")
    parser.add_argument('--file_pattern', help='Glob pattern to watch for an update.')
    parser.add_argument('--interval', default=10, type=int, help='Interval used to check for file updates.')
...
def run(argv=None, model_class=None, model_params=None, save_main_session=True, device='CPU', test_pipeline=None) -> PipelineResult:
    ...
    model_handler = KeyedModelHandler(PytorchModelHandlerTensor(state_dict_path=known_args.model_path, model_class=model_class, model_params=model_params, device=device, min_batch_size=10, max_batch_size=100))
    pipeline = test_pipeline
    if not test_pipeline:
        pipeline = beam.Pipeline(options=pipeline_options)
    side_input = pipeline | WatchFilePattern(interval=known_args.interval, file_pattern=known_args.file_pattern)
    filename_value_pair = pipeline | 'ReadImageNamesFromPubSub' >> beam.io.ReadFromPubSub(known_args.topic) | 'DecodeBytes' >> beam.Map(lambda x: x.decode('utf-8')) | 'ReadImageData' >> beam.Map(lambda image_name: read_image(image_file_name=image_name)) | 'PreprocessImages' >> beam.MapTuple(lambda file_name, data: (file_name, preprocess_image(data)))
    predictions = filename_value_pair | 'PyTorchRunInference' >> RunInference(model_handler, model_metadata_pcoll=side_input) | 'ProcessOutput' >> beam.ParDo(PostProcessor())
