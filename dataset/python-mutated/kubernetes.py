...
class AsyncKubernetesHook(KubernetesHook):
    ...
    async def _load_config(self):
        ...
        if kubeconfig_path:
            ...
            self._is_in_cluster = False
            await async_config.load_kube_config(config_file=kubeconfig_path, client_configuration=self.client_configuration, context=cluster_context)
            return async_client.ApiClient()
        if kubeconfig is not None:
            async with aiofiles.tempfile.NamedTemporaryFile() as temp_config:
                self.log.debug('Reading kubernetes configuration file from connection object and writing temporary config file with its content')
                await temp_config.write(kubeconfig.encode())
                await temp_config.flush()
                self._is_in_cluster = False
                await async_config.load_kube_config(config_file=temp_config.name, client_configuration=self.client_configuration, context=cluster_context)
            return async_client.ApiClient()
        self.log.debug(LOADING_KUBE_CONFIG_FILE_RESOURCE.format('default configuration file'))
        await async_config.load_kube_config(client_configuration=self.client_configuration, context=cluster_context)
    ...
    async def get_pod(self, name: str, namespace: str) -> V1Pod:
        ...
        async with self.get_conn() as connection:
            ...
            pod: V1Pod = await v1_api.read_namespaced_pod(name=name, namespace=namespace)
        return pod
    ...
    async def delete_pod(self, name: str, namespace: str):
        ...
        async with self.get_conn() as connection:
            try:
                v1_api = async_client.CoreV1Api(connection)
                await v1_api.delete_namespaced_pod(name=name, namespace=namespace, body=client.V1DeleteOptions())
    ...
    async def read_logs(self, name: str, namespace: str):
        ...
        async with self.get_conn() as connection:
            try:
                v1_api = async_client.CoreV1Api(connection)
                logs = await v1_api.read_namespaced_pod_log(name=name, namespace=namespace, follow=False, timestamps=True)
