...
class TrtConvertLookupTableV2Test(TrtLayerAutoScanTest):
    ...
    def sample_program_configs(self):
        ...
        for dims in [1, 2, 3]:
            self.dims = dims
            ops_config = [{'op_type': 'lookup_table_v2', 'op_inputs': {'Ids': ['indices'], 'W': ['data']}, 'op_outputs': {'Out': ['out_data']}, 'op_attrs': {}}]
    ...
    def sample_predictor_configs(self, program_config) -> (paddle_infer.Config, List[int], float):
        ...
        def generate_dynamic_shape(attrs):
            if False:
                return 10
            if self.dims == 1:
                self.dynamic_shape.min_input_shape = {'indices': [1], 'data': [64, 4]}
                self.dynamic_shape.max_input_shape = {'indices': [16], 'data': [64, 4]}
                self.dynamic_shape.opt_input_shape = {'indices': [8], 'data': [64, 4]}
            elif self.dims == 2:
                self.dynamic_shape.min_input_shape = {'indices': [1, 1], 'data': [64, 4]}
                self.dynamic_shape.max_input_shape = {'indices': [16, 32], 'data': [64, 4]}
                self.dynamic_shape.opt_input_shape = {'indices': [2, 16], 'data': [64, 4]}
            else:
                self.dynamic_shape.min_input_shape = {'indices': [1, 1, 1], 'data': [64, 4]}
                self.dynamic_shape.max_input_shape = {'indices': [16, 16, 16], 'data': [64, 4]}
                self.dynamic_shape.opt_input_shape = {'indices': [2, 8, 8], 'data': [64, 4]}
