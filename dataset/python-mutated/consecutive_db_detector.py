...
from sentry.utils.performance_issues.detectors.utils import get_max_span_duration, get_total_span_duration
from ..base import DetectorType, PerformanceDetector, fingerprint_spans, get_notification_attachment_body, get_span_duration, get_span_evidence_value
...
def join_regexes(regexes: Sequence[str]) -> str:
    if False:
        while True:
            i = 10
    return '(?:' + ')|(?:'.join(regexes) + ')'
CONTAINS_PARAMETER_REGEX = re.compile(join_regexes(["'(?:[^']|'')*?(?:\\\\'.*|'(?!'))", '\\b(?:true|false)\\b', '-?\\b(?:[0-9]+\\.)?[0-9]+(?:[eE][+-]?[0-9]+)?\\b', '\\?|\\$1|%s']))
