...
class TestUploadSpecToCache:
    ...
    @pytest.mark.parametrize('valid_spec, successful_upload', [[True, True], [False, True], [True, False], [False, False]])
    async def test_run(self, mocker, dagger_client, valid_spec, successful_upload, random_connector, publish_context):
        ...
        mocker.patch.object(publish_pipeline, 'upload_to_gcs', mocker.AsyncMock(return_value=(upload_exit_code, 'upload_to_gcs_stdout', 'upload_to_gcs_stderr')))
        if not valid_spec:
            mocker.patch.object(publish_pipeline.UploadSpecToCache, '_get_connector_spec', mocker.Mock(side_effect=publish_pipeline.InvalidSpecOutputError('Invalid spec.')))
        step = publish_pipeline.UploadSpecToCache(publish_context)
        step_result = await step.run(connector_container)
        if valid_spec:
            publish_pipeline.upload_to_gcs.assert_called_once_with(publish_context.dagger_client, mocker.ANY, f"specs/{image_name.replace(':', '/')}/spec.json", publish_context.spec_cache_bucket_name, publish_context.spec_cache_gcs_credentials_secret, flags=['--cache-control="no-cache"'])
    ...
    def test_parse_spec_output_no_spec(self, publish_context):
        ...
        with pytest.raises(publish_pipeline.InvalidSpecOutputError):
            step._parse_spec_output(spec_output)
STEPS_TO_PATCH = [(publish_pipeline, 'MetadataValidation'), (publish_pipeline, 'MetadataUpload'), (publish_pipeline, 'CheckConnectorImageDoesNotExist'), (publish_pipeline, 'UploadSpecToCache'), (publish_pipeline, 'PushConnectorImageToRegistry'), (publish_pipeline, 'PullConnectorImageFromRegistry'), (publish_pipeline.steps, 'run_connector_build')]
...
async def test_run_connector_publish_pipeline_when_failed_validation(mocker, pre_release):
    ...
    assert report.steps_results == context.report.steps_results == [run_metadata_validation.return_value]
...
async def test_run_connector_publish_pipeline_when_image_exists_or_failed(mocker, check_image_exists_status, publish_context):
    ...
    if check_image_exists_status is StepStatus.SKIPPED:
        ...
        assert report.steps_results == publish_context.report.steps_results == [run_metadata_validation.return_value, run_check_connector_image_does_not_exist.return_value, run_upload_spec_to_cache.return_value, run_metadata_upload.return_value]
    if check_image_exists_status is StepStatus.FAILURE:
        run_metadata_upload.assert_not_called()
        assert report.steps_results == publish_context.report.steps_results == [run_metadata_validation.return_value, run_check_connector_image_does_not_exist.return_value]

@pytest.mark.parametrize('pre_release, build_step_status, push_step_status, pull_step_status, upload_to_spec_cache_step_status, metadata_upload_step_status', [(False, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.SUCCESS), (False, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.FAILURE), (False, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.FAILURE, None), (False, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.FAILURE, None, None), (False, StepStatus.SUCCESS, StepStatus.FAILURE, None, None, None), (False, StepStatus.FAILURE, None, None, None, None), (True, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.SUCCESS, StepStatus.SUCCESS)])
async def test_run_connector_publish_pipeline_when_image_does_not_exist(mocker, pre_release, build_step_status, push_step_status, pull_step_status, upload_to_spec_cache_step_status, metadata_upload_step_status):
    ...
    for (module, to_mock) in STEPS_TO_PATCH:
        mocker.patch.object(module, to_mock, return_value=mocker.AsyncMock())
    publish_pipeline.MetadataValidation.return_value.run.return_value = mocker.Mock(name='metadata_validation_result', status=StepStatus.SUCCESS)
    publish_pipeline.CheckConnectorImageDoesNotExist.return_value.run.return_value = mocker.Mock(name='check_connector_image_does_not_exist_result', status=StepStatus.SUCCESS)
    built_connector_platform = mocker.Mock()
    built_connector_platform.values.return_value = ['linux/amd64']
    publish_pipeline.steps.run_connector_build.return_value = mocker.Mock(name='build_connector_for_publish_result', status=build_step_status, output_artifact=built_connector_platform)
    publish_pipeline.PushConnectorImageToRegistry.return_value.run.return_value = mocker.Mock(name='push_connector_image_to_registry_result', status=push_step_status)
    publish_pipeline.PullConnectorImageFromRegistry.return_value.run.return_value = mocker.Mock(name='pull_connector_image_from_registry_result', status=pull_step_status)
    publish_pipeline.UploadSpecToCache.return_value.run.return_value = mocker.Mock(name='upload_spec_to_cache_result', status=upload_to_spec_cache_step_status)
    publish_pipeline.MetadataUpload.return_value.run.return_value = mocker.Mock(name='metadata_upload_result', status=metadata_upload_step_status)
    context = mocker.MagicMock(pre_release=pre_release)
    semaphore = anyio.Semaphore(1)
    report = await publish_pipeline.run_connector_publish_pipeline(context, semaphore)
    steps_to_run = [publish_pipeline.MetadataValidation.return_value.run, publish_pipeline.CheckConnectorImageDoesNotExist.return_value.run, publish_pipeline.steps.run_connector_build, publish_pipeline.PushConnectorImageToRegistry.return_value.run, publish_pipeline.PullConnectorImageFromRegistry.return_value.run]
