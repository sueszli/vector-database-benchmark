...
from .socks import ProxyConnectionError, socksocket, PROXY_TYPES, DEFAULT_PORTS
...
from .netcreds import find_first_cred, find_creds_for_uri, add_cred_for_uri, add_cred
...
class TCPReaderHandler(urllib2.BaseHandler):
    ...
    def _get_stream_data(self, conn, req):
        ...
        if data:
            ...
            code = 404
        return urllib.addinfourl(fp, headers, req.get_full_url(), code=code)
StreamingHTTPSHandler.https_open = lambda self, req: self.do_open(StreamingHTTPSConnection, req, context=self._context)
...
class HTTP(object):
    __slots__ = ('ctx', 'proxy', 'noverify', 'no_proxy_locals', 'no_proxy_for', 'timeout', 'headers', 'follow_redirects')
    ...
    def post(self, url, file=None, data=None, save=None, headers={}, multipart=False, return_url=False, return_headers=False, code=False, params={}):
        ...
        if len(result) == 1:
            ...
            return tuple(result)
from .proxies import find_default_proxy, set_proxy_unavailable, has_wpad, get_proxy_for_address