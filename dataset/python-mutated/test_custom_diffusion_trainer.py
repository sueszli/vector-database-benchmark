...
class TestCustomDiffusionTrainer(unittest.TestCase):

    def setUp(self):
        if False:
            return 10
        print('Testing %s.%s' % (type(self).__name__, self._testMethodName))
        self.train_dataset = MsDataset.load('buptwq/lora-stable-diffusion-finetune-dog', split='train', download_mode=DownloadMode.FORCE_REDOWNLOAD)
        self.eval_dataset = MsDataset.load('buptwq/lora-stable-diffusion-finetune-dog', split='validation', download_mode=DownloadMode.FORCE_REDOWNLOAD)
        self.max_epochs = 5
    ...
    def test_custom_diffusion_train(self):
        ...
        print(f'Custom-diffusion train output: {result}.')
        results_files = os.listdir(self.tmp_dir)
        self.assertIn(f'{trainer.timestamp}.log.json', results_files)
        pipe = pipeline(task=Tasks.text_to_image_synthesis, model=f'{self.tmp_dir}/output')
    ...
    def test_dreambooth_diffusion_eval(self):
        ...
        result = trainer.evaluate()
        print(f'Custom-diffusion eval output: {result}.')
