...
IPTABLES_RULES_DEFAULT_DROP = {'policies': ['-P INPUT DROP', '-P FORWARD DROP', '-P OUTPUT DROP'], 'input': ['-A INPUT -m comment --comment "Drop and log all other incoming traffic" -j LOGNDROP'], 'output': ['-A OUTPUT -m comment --comment "Drop all other outgoing traffic" -j DROP'], 'logndrop': ['-A LOGNDROP -p tcp -m limit --limit 5/min -j LOG --log-tcp-options --log-ip-options --log-uid', '-A LOGNDROP -p udp -m limit --limit 5/min -j LOG --log-ip-options --log-uid', '-A LOGNDROP -p icmp -m limit --limit 5/min -j LOG --log-ip-options --log-uid', '-A LOGNDROP -j DROP']}
