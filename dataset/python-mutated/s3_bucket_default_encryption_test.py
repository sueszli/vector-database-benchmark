...
class Test_s3_bucket_default_encryption:
    ...
    def test_bucket_no_encryption(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_default_encryption.s3_bucket_default_encryption.s3_client', new=S3(audit_info)):
                from prowler.providers.aws.services.s3.s3_bucket_default_encryption.s3_bucket_default_encryption import s3_bucket_default_encryption
                check = s3_bucket_default_encryption()
                result = check.execute()
                assert len(result) == 1
                assert result[0].status == 'FAIL'
                assert search('Server Side Encryption is not configured', result[0].status_extended)
    ...
    def test_bucket_kms_encryption(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_default_encryption.s3_bucket_default_encryption.s3_client', new=S3(audit_info)):
                ...
                assert search('has Server Side Encryption', result[0].status_extended)
                assert result[0].resource_id == bucket_name_us
                assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                assert result[0].region == 'us-east-1'