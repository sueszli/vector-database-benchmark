...
class FlaxRegNetModelTester(unittest.TestCase):
    ...
    def get_config(self):
        if False:
            return 10
        return RegNetConfig(num_channels=self.num_channels, embeddings_size=self.embeddings_size, hidden_sizes=self.hidden_sizes, depths=self.depths, hidden_act=self.hidden_act, num_labels=self.num_labels, image_size=self.image_size)
...
class FlaxRegNetModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_classification_head(self):
        ...
        expected_slice = jnp.array([-0.418, -1.5051, -3.4836])
