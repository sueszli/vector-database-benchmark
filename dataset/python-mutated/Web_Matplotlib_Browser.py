...
def main():
    ...
    col_listbox = [[sg.Listbox(values=fig_dict.keys(), enable_events=True, size=(25, len(fig_dict.keys())), key='-LISTBOX-')], [sg.Exit(size=(5, 2))]]
    layout = [[sg.Text('Matplotlib Plot Browser', font='current 25')], [sg.Text('Choose a plot from the list to see the plot and the source code used to make it.')], [sg.Column(col_listbox, element_justification='c'), sg.Image(key='-IMAGE-'), sg.MLine(size=(40, 20), font='Courier 12', key='-MULTILINE-')]]
    ...
    while True:
        ...
        the_plotting_func = fig_dict[choice]
        window['-MULTILINE-'].update(inspect.getsource(the_plotting_func))
        draw_figure(the_plotting_func(), window['-IMAGE-'])
    window.close()
