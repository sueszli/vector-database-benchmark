...
class TestTransactionalBatchAsync:
    ...
    async def test_invalid_batch_sizes_async(self):
        await self._set_up()
        container = await self.test_database.create_container_if_not_exists(id='invalid_batch_size' + str(uuid.uuid4()), partition_key=PartitionKey(path='/company'))
        try:
    ...
    async def test_batch_create_async(self):
        await self._set_up()
        container = await self.test_database.create_container_if_not_exists(id='batch_create' + str(uuid.uuid4()), partition_key=PartitionKey(path='/company'))
        batch = []
        for i in range(100):
            batch.append(('create', ({'id': 'item' + str(i), 'company': 'Microsoft'},)))
    ...
    async def test_batch_read_async(self):
        await self._set_up()
        container = await self.test_database.create_container_if_not_exists(id='batch_read' + str(uuid.uuid4()), partition_key=PartitionKey(path='/company'))
        batch = []
        for i in range(100):
    ...
    async def test_batch_replace_async(self):
        await self._set_up()
        container = await self.test_database.create_container_if_not_exists(id='batch_replace' + str(uuid.uuid4()), partition_key=PartitionKey(path='/company'))
    ...
    async def test_batch_upsert_async(self):
        await self._set_up()
        container = await self.test_database.create_container_if_not_exists(id='batch_upsert' + str(uuid.uuid4()), partition_key=PartitionKey(path='/company'))
        item_id = str(uuid.uuid4())
    ...
    async def test_batch_patch_async(self):
        await self._set_up()
        container = await self.test_database.create_container_if_not_exists(id='batch_patch' + str(uuid.uuid4()), partition_key=PartitionKey(path='/company'))
        item_id = str(uuid.uuid4())
    ...
    async def test_batch_delete_async(self):
        await self._set_up()
        container = await self.test_database.create_container_if_not_exists(id='batch_delete' + str(uuid.uuid4()), partition_key=PartitionKey(path='/company'))
        create_batch = []
        delete_batch = []
        for i in range(10):
            item_id = str(uuid.uuid4())
    ...
    async def test_batch_lsn_async(self):
        await self._set_up()
        container = await self.test_database.create_container_if_not_exists(id='batch_lsn' + str(uuid.uuid4()), partition_key=PartitionKey(path='/company'))
        ...
        await container.read_item(item='read_item', partition_key='Microsoft')
    ...
    async def test_batch_subpartition(self):
        await self._set_up()
        container = await self.test_database.create_container_if_not_exists(id='batch_subpartition' + str(uuid.uuid4()), partition_key=PartitionKey(path=['/state', '/city', '/zipcode'], kind='MultiHash'))
        item_ids = [str(uuid.uuid4()), str(uuid.uuid4()), str(uuid.uuid4())]
        await container.upsert_item({'id': item_ids[0], 'key': 'value', 'state': 'WA', 'city': 'Redmond', 'zipcode': '98052'})
        await container.upsert_item({'id': item_ids[1], 'key': 'value', 'state': 'WA', 'city': 'Redmond', 'zipcode': '98052'})
        await container.upsert_item({'id': item_ids[2], 'key': 'value', 'state': 'WA', 'city': 'Redmond', 'zipcode': '98052'})
