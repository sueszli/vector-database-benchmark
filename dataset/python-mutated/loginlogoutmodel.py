...
class LoginLogoutBaseModel(BasicModel):
    ...
    def __init__(self, metadata):
        if False:
            return 10
        super().__init__(self.TYPE, metadata.cliid, metadata.sessid)
        self.metadata = metadata.dict_repr()
        self.protocol_versions = {'monitor': MONITOR_CONFIG['PROTO_VERSION'], 'p2p': PROTOCOL_CONST.ID, 'task': PROTOCOL_CONST.ID}
        self.nvgpu = {'is_supported': nvgpu.is_supported()}
