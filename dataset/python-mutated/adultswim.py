...
class AdultSwim(Plugin):
    ...
    app_id_js_url_re = re.compile('<script src="([^"]*asvp\\..*?\\.bundle\\.js)">')
    app_id_re = re.compile('CDN_TOKEN_APP_ID="(.*?)"')
    json_data_re = re.compile('<script id="__NEXT_DATA__" type="application/json">({.*})</script>')
    truncate_url_re = re.compile('(.*)/\\w+/?')
    _api_schema = validate.Schema({'media': {'desktop': {str: {'url': validate.url()}}}}, validate.get('media'), validate.get('desktop'), validate.filter(lambda k, v: k in ['unprotected', 'bulkaes']))
    _stream_data_schema = validate.Schema({'props': {'__REDUX_STATE__': {'streams': [{'id': str, 'stream': str}]}}}, validate.get('props'), validate.get('__REDUX_STATE__'), validate.get('streams'))
    _token_schema = validate.Schema(validate.any({'auth': {'token': str}}, {'auth': {'error': {'message': str}}}), validate.get('auth'))
    _video_data_schema = validate.Schema({'props': {'pageProps': {'__APOLLO_STATE__': {str: {validate.optional('id'): str, validate.optional('slug'): str}}}}}, validate.get('props'), validate.get('pageProps'), validate.get('__APOLLO_STATE__'), validate.filter(lambda k, v: k.startswith('Video:')))
