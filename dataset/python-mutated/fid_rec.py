...
class FID:
    ...
    def evaluate(self, logger, mapping, decoder, encoder, lod):
        ...
        dataset = TFRecordsDataset(self.cfg, logger, rank=0, world_size=1, buffer_size_mb=128, channels=self.cfg.MODEL.CHANNELS, train=True)
        dataset.reset(lod + 2, self.minibatch_size)
...
if __name__ == '__main__':
    gpu_count = 1
    run(sample, get_cfg_defaults(), description='ALAE-fid-reconstruction', default_config='configs/ffhq.yaml', world_size=gpu_count, write_log='metrics/fid_score-reconstruction.txt')