...
SERIALIZABLE_ALLOWLIST = os.path.join(resource_loader.get_data_files_path(), 'tf_serializable_allowlist.txt')
CHECKPOINT_SAVER_ALLOWLIST = os.path.join(resource_loader.get_data_files_path(), 'tf_checkpoint_saver_allowlist.txt')
...
class NotInAllowlistExample(tf.Variable):
    pass
registration.register_tf_checkpoint_saver(name='TestDummySaver', predicate=lambda : False, save_fn=lambda : None, restore_fn=lambda : None)
registration.register_tf_checkpoint_saver(name='NotInAllowlistExample', predicate=lambda : False, save_fn=lambda : None, restore_fn=lambda : None)
