...
def get_mock_provider(valid_api_key, valid_secret_key):
    if False:
        for i in range(10):
            print('nop')
    return Provider(id='provider_id', tenant_id='tenant_id', provider_name='baichuan', provider_type=ProviderType.CUSTOM.value, encrypted_config=json.dumps({'api_key': valid_api_key, 'secret_key': valid_secret_key}), is_valid=True)
...
def get_mock_model(model_name: str, streaming: bool=False):
    ...
    return BaichuanModel(model_provider=model_provider, name=model_name, model_kwargs=model_kwargs, streaming=streaming)
