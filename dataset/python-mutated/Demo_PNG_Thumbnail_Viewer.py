...
for display_index in range(ROWS):
    ...
    buttons.append(row)
col_buttons = [[]]
col = [[sg.Text(png_files[0], size=(80, 3), key='filename')], [sg.Image(data=convert_to_bytes(png_files[0], (500, 500)), key='image')]]
layout = [[sg.Menu(menu)], [sg.Col(buttons), sg.Col([[sg.Slider((len(png_files), 0), default_value=0, size=(38, 20), orientation='v', key='-slider-', change_submits=True)]]), sg.Col(col)]]
window = sg.Window('Image Browser', layout, return_keyboard_events=True, use_default_focus=False, finalize=True)
display_index = 0
while True:
    for x in range(ROWS):
