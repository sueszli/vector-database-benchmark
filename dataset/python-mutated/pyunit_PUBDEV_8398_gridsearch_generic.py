    ...
    def setup_data(self):
        ...
        for lambda_param in self.hyper_parameters['lambda']:
            for alpha_param in self.hyper_parameters['alpha']:
                self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family='gaussian', gam_columns=['C11', 'C12', 'C13'], keep_gam_cols=True, scale=[1, 1, 1], num_knots=[5, 5, 5], alpha=alpha_param, lambda_=lambda_param, bs=[2, 0, 3]))

    def train_models(self):
        if False:
            i = 10
            return i + 15
        self.h2o_model = H2OGridSearch(H2OGeneralizedAdditiveEstimator(family='gaussian', gam_columns=['C11', 'C12', 'C13'], keep_gam_cols=True, scale=[1, 1, 1], num_knots=[5, 5, 5], bs=[2, 0, 3]), self.hyper_parameters)
    ...
    def match_models(self):
        ...
        for model in self.manual_gam_models:
            ...
            for grid_search_model in self.h2o_model.models:
                if grid_search_model.actual_params['alpha'] == alpha and grid_search_model.actual_params['lambda'] == lambda_:
