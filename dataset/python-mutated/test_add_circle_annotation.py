...
class TestAddCircleAnnotation(TestCase):
    ...
    def test_add_circle_annotation(self):
        ...
        layout.add(self.get_test_header(test_description='This test adds a CircleAnnotation to a PDF.'))
        w: Decimal = pdf.get_page(0).get_page_info().get_width()
        h: Decimal = pdf.get_page(0).get_page_info().get_height()
        pdf.get_page(0).add_annotation(CircleAnnotation(bounding_box=Rectangle(w / Decimal(2) - Decimal(32), h / Decimal(2) - Decimal(32), Decimal(64), Decimal(64)), stroke_color=HexColor('0B3954'), fill_color=HexColor('f1cd2e')))
