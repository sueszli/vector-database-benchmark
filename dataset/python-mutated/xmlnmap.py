...
from ivre.active.data import ALIASES_TABLE_ELEMS, add_cert_hostnames, add_hostname, create_ssl_cert, handle_http_content, handle_http_headers
...
SCREENSHOTS_SCRIPTS = {'http-screenshot': screenshot_extract, 'mainframe-screenshot': screenshot_extract, 'rdp-screenshot': screenshot_extract, 'rtsp-screenshot': screenshot_extract, 'vnc-screenshot': screenshot_extract, 'x11-screenshot': screenshot_extract}
_MONGODB_DATABASES_CONVERTS = {'false': False, 'true': True, 'nil': None}
_MONGODB_DATABASES_TYPES = {'totalSize': float, 'totalSizeMb': float, 'empty': lambda x: _MONGODB_DATABASES_CONVERTS.get(x, x), 'sizeOnDisk': float, 'code': lambda x: _MONGODB_DATABASES_CONVERTS.get(x, x) if isinstance(x, str) else float(x), 'ok': lambda x: _MONGODB_DATABASES_CONVERTS.get(x, x) if isinstance(x, str) else float(x)}
...
def add_http_headers_data(script):
    ...
    return result
ADD_TABLE_ELEMS = {'modbus-discover': re.compile('^ *DEVICE IDENTIFICATION: *(?P<deviceid>.*?) *$', re.M), 'ls': add_ls_data, 'mongodb-databases': add_mongodb_databases_data, 'http-headers': add_http_headers_data}
...
def change_http_default_accounts(table):
    ...
    return table
CHANGE_TABLE_ELEMS = {'smb-enum-shares': change_smb_enum_shares, 's7-info': change_s7_info_keys, 'ls': change_ls, 'vulns': change_vulns, 'fcrdns': change_fcrdns, 'rpcinfo': change_rpcinfo, 'ms-sql-info': change_ms_sql_info, 'ssh-hostkey': change_ssh_hostkey, 'http-git': change_http_git, 'http-server-header': change_http_server_header, 'http-default-accounts': change_http_default_accounts}
...
def change_ssh2_enum_algos(out, table):
    ...
    out += '\n'.join(new_out)
    return (out, table)
CHANGE_OUTPUT_TABLE_ELEMS = {'ssh2-enum-algos': change_ssh2_enum_algos, 'ssl-cert': change_ssl_cert}
...
def post_snmp_info(script, _, host):
    ...
    if mac not in cur_macs:
        cur_macs.append(mac)
POST_PROCESS = {'http-headers': post_http_headers, 'ntlm-info': post_ntlm_info, 'smb-os-discovery': post_smb_os_discovery, 'snmp-info': post_snmp_info, 'ssl-cert': post_ssl_cert}
...
def split_smb_os_discovery(script):
    ...
    yield ntlm
SPLIT_SCRIPTS = {'smb-os-discovery': split_smb_os_discovery}
IGNORE_SCRIPTS = {'mcafee-epo-agent': set(['ePO Agent not found']), 'ftp-bounce': set(['no banner']), 'telnet-encryption': set(['\n  ERROR: Failed to send packet: TIMEOUT']), 'http-mobileversion-checker': set(['No mobile version detected.']), 'http-referer-checker': set(["Couldn't find any cross-domain scripts."]), 'http-default-accounts': set(['[ERROR] HTTP request table is empty. This should not happen since we at least made one request.']), 'http-headers': set(['\n  (Request type: GET)\n']), 'http-cisco-anyconnect': set(['\n  ERROR: Not a Cisco ASA or unsupported version']), 'ndmp-fs-info': set(['\n  ERROR: Failed to get filesystem information from server']), 'ndmp-version': set(['\n  ERROR: Failed to get host information from server']), 'ajp-auth': set(['\n  ERROR: Failed to connect to AJP server']), 'ajp-headers': set(['\n  ERROR: Failed to retrieve server headers']), 'ajp-methods': set(['Failed to get a valid response for the OPTION request']), 'ajp-request': set(['\n  ERROR: Failed to retrieve response for request', '\n  ERROR: Failed to connect to AJP server']), 'giop-info': set(['  \n  ERROR: Failed to read Packet.GIOP']), 'rsync-list-modules': set(['\n  ERROR: Failed to connect to rsync server', '\n  ERROR: Failed to retrieve a list of modules']), 'sip-methods': set(['ERROR: Failed to connect to the SIP server.']), 'sip-call-spoof': set(['ERROR: Failed to connect to the SIP server.']), 'rpcap-info': set(['\n  ERROR: EOF']), 'rmi-dumpregistry': set(['Registry listing failed (Handshake failed)']), 'voldemort-info': set(['\n  ERROR: Unsupported protocol']), 'irc-botnet-channels': set(['\n  ERROR: EOF\n']), 'bitcoin-getaddr': set(['\n  ERROR: Failed to extract address information', '\n  ERROR: Failed to extract version information']), 'bitcoin-info': set(['\n  ERROR: Failed to extract version information']), 'drda-info': set(['The response contained no EXCSATRD']), 'rdp-enum-encryption': set(['Received unhandled packet']), 'ldap-search': set(['ERROR: Failed to bind as the anonymous user']), 'mongodb-databases': set(['No Bson data returned']), 'snmp-hh3c-logins': set(['\n  baseoid: 1.3.6.1.4.1.25506.2.12.1.1.1']), 'dns-nsec-enum': set(['\n  No NSEC records found\n']), 'dns-nsec3-enum': set(['\n  DNSSEC NSEC3 not supported\n']), 'http-csrf': set(["Couldn't find any CSRF vulnerabilities."]), 'http-devframework': set(["Couldn't determine the underlying framework or CMS. Try increasing 'httpspider.maxpagecount' value to spider more pages."]), 'http-dombased-xss': set(["Couldn't find any DOM based XSS."]), 'http-drupal-enum': set(['Nothing found amongst the top 100 resources,use --script-args number=<number|all> for deeper analysis)']), 'http-errors': set(["Couldn't find any error pages."]), 'http-feed': set(["Couldn't find any feeds."]), 'http-litespeed-sourcecode-download': set(['Request with null byte did not work. This web server might not be vulnerable', 'Page: /index.php was not found. Try with an existing file.']), 'http-sitemap-generator': set(['\n  Directory structure:\n    /\n      Other: 1\n  Longest directory structure:\n    Depth: 0\n    Dir: /\n  Total files found (by extension):\n    Other: 1\n', '\n  Directory structure:\n  Longest directory structure:\n    Depth: 0\n    Dir: /\n  Total files found (by extension):\n    \n']), 'http-stored-xss': set(["Couldn't find any stored XSS vulnerabilities."]), 'http-wordpress-enum': set(['Nothing found amongst the top 100 resources,use --script-args search-limit=<number|all> for deeper analysis)']), 'http-wordpress-users': set(["[Error] Wordpress installation was not found. We couldn't find wp-login.php"]), 'ssl-date': set(['TLS randomness does not represent time']), 'http-comments-displayer': set(["Couldn't find any comments."]), 'http-jsonp-detection': set(["Couldn't find any JSONP endpoints."]), 'firewalk': set(['None found']), 'ipidseq': set(['Unknown']), 'fcrdns': set(['FAIL (No PTR record)']), 'msrpc-enum': set(['SMB: ERROR: Server disconnected the connection']), 'smb-mbenum': set(['\n  ERROR: Failed to connect to browser service: SMB: ERROR: Server disconnected the connection'])}
IGNORE_SCRIPTS_IDS = set(['http-screenshot', 'mainframe-screenshot', 'rdp-screenshot', 'rtsp-screenshot', 'vnc-screenshot', 'x11-screenshot'])
MSSQL_ERROR = re.compile('^ *(ERROR: )?(No login credentials|TCP: Socket connection failed, Named Pipes: No named pipe for this instance)\\.?$', re.MULTILINE)
IGNORE_SCRIPTS_REGEXP = {'smtp-commands': re.compile('^' + re.escape("Couldn't establish connection on port ") + '[0-9]+$'), 'ms-sql-config': MSSQL_ERROR, 'ms-sql-dump-hashes': MSSQL_ERROR, 'ms-sql-hasdbaccess': MSSQL_ERROR, 'ms-sql-query': MSSQL_ERROR, 'ms-sql-tables': MSSQL_ERROR, 'irc-botnet-channels': re.compile('^' + re.escape('\n  ERROR: Closing Link: ')), 'http-php-version': re.compile('^(Logo query returned unknown hash [0-9a-f]{32}\\\nCredits query returned unknown hash [0-9a-f]{32}|(Logo|Credits) query returned unknown hash [0-9a-f]{32})$'), 'p2p-conficker': re.compile(re.escape('Host is CLEAN or ports are blocked')), 'dns-nsec-enum': re.compile('^' + re.escape("Can't determine domain for host ") + '.*' + re.escape('; use dns-nsec-enum.domains script arg.') + '$'), 'dns-nsec3-enum': re.compile('^' + re.escape("Can't determine domain for host ") + '.*' + re.escape('; use dns-nsec3-enum.domains script arg.') + '$'), 'http-vhosts': re.compile('^\\\n[0-9]+' + re.escape(' names had status ') + '(?:[0-9]{3}|ERROR)'), 'http-fileupload-exploiter': re.compile('^(' + re.escape("\n  \n    Couldn't find a file-type field.") + ')*$')}
IGNORE_SCRIPT_OUTPUTS = set(['Unable to open connection', 'false', 'TIMEOUT', 'ERROR', '\n', '\r\n'])
IGNORE_SCRIPT_OUTPUTS_REGEXP = set([re.compile('d41d8cd98f00b204e9800998ecf8427e', re.IGNORECASE), re.compile('^ *ERROR\\:\\ (Failed\\ to\\ (connect\\ to|receive\\ response\\ from)\\ server|Script\\ execution\\ failed\\ \\(use\\ \\-d\\ to\\ debug\\)|Receiving\\ packet\\:\\ (ERROR|EOF)|Failed\\ to\\ send\\ packet\\:\\ ERROR|ERROR)', re.MULTILINE), re.compile('^ *(SMB|ERROR):.*TIMEOUT', re.MULTILINE)])
MASSCAN_S7_INDEXES = {17: {1: 'module', 6: 'hardware', 7: 'firmware', 129: 'vipa_firmware', 130: 'svn_cpu'}, 28: {1: 'system_name', 2: 'module_name', 3: 'plant', 4: 'copyright', 5: 'module_sn', 6: 'reserved', 7: 'module_type', 8: 'memory_card_sn', 11: 'location'}}
NMAP_S7_INDEXES = {'Module': MASSCAN_S7_INDEXES[17][1], 'Basic Hardware': MASSCAN_S7_INDEXES[17][6], 'Version': 'version', 'System Name': MASSCAN_S7_INDEXES[28][1], 'Module Type': MASSCAN_S7_INDEXES[28][2], 'Serial Number': MASSCAN_S7_INDEXES[28][5], 'Plant Identification': MASSCAN_S7_INDEXES[28][3], 'Copyright': MASSCAN_S7_INDEXES[28][4]}
MASSCAN_SERVICES_NMAP_SCRIPTS = {'http': 'http-headers', 'http.server': 'http-server-header', 'title': 'http-title', 'html': 'http-content', 'ftp': 'banner', 'unknown': 'banner', 'ssh': 'ssh-banner', 'vnc': 'banner', 'imap': 'banner', 'pop': 'banner', 'smtp': 'banner', 'X509': 'ssl-cert', 'X509CA': 'ssl-cacert', 's7comm': 's7-info', 'telnet': 'banner', 'rdp': 'rdp-nla-support'}
MASSCAN_NMAP_SCRIPT_NMAP_PROBES = {'tcp': {'banner': ['NULL'], 'ssh-banner': ['NULL'], 'http-headers': ['GetRequest']}}
MASSCAN_SERVICES_NMAP_SERVICES = {'ftp': 'ftp', 'http': 'http', 'http.server': 'http', 'title': 'http', 'html': 'http', 'ssh': 'ssh', 'vnc': 'vnc', 'imap': 'imap', 'pop': 'pop3', 'smtp': 'smtp', 's7comm': 'iso-tsap', 'telnet': 'telnet', 'rdp': 'ms-wbt-server'}
...
class NmapHandler(ContentHandler):
    ...
    _ssh_key_exchange_data = ['kex_algorithms', 'server_host_key_algorithms', 'encryption_algorithms_client_to_server', 'encryption_algorithms_server_to_client', 'mac_algorithms_client_to_server', 'mac_algorithms_server_to_client', 'compression_algorithms_client_to_server', 'compression_algorithms_server_to_client', 'languages_client_to_server', 'languages_server_to_client']
    _ssh_key_exchange_data_pairs = ['encryption_algorithms', 'mac_algorithms', 'compression_algorithms', 'languages']
