        ...
        if old_major_version == 1:
            ...
            if old_minor_version < 2:
                ...
                for dataset in old_data['datasets']:
                    ...
                    entry = DatasetEntry(created=created, id=dataset['id'], preferred_border_agent_id=None, source=dataset['source'], tlv=dataset['tlv'])
...
class DatasetStore:
    ...
    async def async_load(self) -> None:
        ...
        if data is not None:
            for dataset in data['datasets']:
                created = cast(datetime, dt_util.parse_datetime(dataset['created']))
                datasets[dataset['id']] = DatasetEntry(created=created, id=dataset['id'], preferred_border_agent_id=dataset['preferred_border_agent_id'], source=dataset['source'], tlv=dataset['tlv'])
...
async def async_add_dataset(hass: HomeAssistant, source: str, tlv: str, *, preferred_border_agent_id: str | None=None) -> None:
    """Add a dataset."""
