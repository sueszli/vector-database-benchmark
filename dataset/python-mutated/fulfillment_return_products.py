...
class OrderReturnLineInput(BaseInputObjectType):
    order_line_id = graphene.ID(description='The ID of the order line to return.', name='orderLineId', required=True)
    quantity = graphene.Int(description='The number of items to be returned.', required=True)
    replace = graphene.Boolean(description='Determines, if the line should be added to replace order.', default_value=False)

    class Meta:
        doc_category = DOC_CATEGORY_ORDERS

class OrderReturnFulfillmentLineInput(BaseInputObjectType):
    fulfillment_line_id = graphene.ID(description='The ID of the fulfillment line to return.', name='fulfillmentLineId', required=True)
    quantity = graphene.Int(description='The number of items to be returned.', required=True)
    replace = graphene.Boolean(description='Determines, if the line should be added to replace order.', default_value=False)
...
class OrderReturnProductsInput(BaseInputObjectType):
    order_lines = NonNullList(OrderReturnLineInput, description='List of unfulfilled lines to return.')
    fulfillment_lines = NonNullList(OrderReturnFulfillmentLineInput, description='List of fulfilled lines to return.')
    ...
    refund = graphene.Boolean(description='If true, Saleor will call refund action for all lines.', default_value=False)

    class Meta:
        doc_category = DOC_CATEGORY_ORDERS

class FulfillmentReturnProducts(FulfillmentRefundAndReturnProductBase):
    return_fulfillment = graphene.Field(Fulfillment, description='A return fulfillment.')
    replace_fulfillment = graphene.Field(Fulfillment, description='A replace fulfillment.')
    order = graphene.Field(Order, description='Order which fulfillment was returned.')
    replace_order = graphene.Field(Order, description='A draft order which was created for products with replace flag.')
    ...
    class Arguments:
        order = graphene.ID(description='ID of the order to be returned.', required=True)
        input = OrderReturnProductsInput(required=True, description='Fields required to return products.')
