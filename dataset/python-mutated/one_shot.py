...
from autogpt.core.resource.model_providers.schema import AssistantChatMessageDict, ChatMessage, CompletionModelFunction
from autogpt.core.utils.json_schema import JSONSchema
from autogpt.json_utils.utilities import extract_dict_from_response
from autogpt.prompts.utils import format_numbered_list, indent

class OneShotAgentPromptConfiguration(SystemConfiguration):
    DEFAULT_BODY_TEMPLATE: str = '## Constraints\nYou operate within the following constraints:\n{constraints}\n\n## Resources\nYou can leverage access to the following resources:\n{resources}\n\n## Commands\nThese are the ONLY commands you can use. Any action you perform must be possible through one of these commands:\n{commands}\n\n## Best practices\n{best_practices}'
    DEFAULT_CHOOSE_ACTION_INSTRUCTION: str = 'Determine exactly one command to use next based on the given goals and the progress you have made so far, and respond using the JSON schema specified previously:'
    DEFAULT_RESPONSE_SCHEMA = JSONSchema(type=JSONSchema.Type.OBJECT, properties={'thoughts': JSONSchema(type=JSONSchema.Type.OBJECT, required=True, properties={'observations': JSONSchema(description='Relevant observations from your last action (if any)', type=JSONSchema.Type.STRING, required=False), 'text': JSONSchema(description='Thoughts', type=JSONSchema.Type.STRING, required=True), 'reasoning': JSONSchema(type=JSONSchema.Type.STRING, required=True), 'self_criticism': JSONSchema(description='Constructive self-criticism', type=JSONSchema.Type.STRING, required=True), 'plan': JSONSchema(description='Short markdown-style bullet list that conveys the long-term plan', type=JSONSchema.Type.STRING, required=True), 'speak': JSONSchema(description='Summary of thoughts, to say to user', type=JSONSchema.Type.STRING, required=True)}), 'command': JSONSchema(type=JSONSchema.Type.OBJECT, required=True, properties={'name': JSONSchema(type=JSONSchema.Type.STRING, required=True), 'args': JSONSchema(type=JSONSchema.Type.OBJECT, required=True)})})
    body_template: str = UserConfigurable(default=DEFAULT_BODY_TEMPLATE)
    response_schema: dict = UserConfigurable(default_factory=DEFAULT_RESPONSE_SCHEMA.to_dict)
    choose_action_instruction: str = UserConfigurable(default=DEFAULT_CHOOSE_ACTION_INSTRUCTION)
    use_functions_api: bool = UserConfigurable(default=False)

class OneShotAgentPromptStrategy(PromptStrategy):
    default_configuration: OneShotAgentPromptConfiguration = OneShotAgentPromptConfiguration()
