    ...
    def handleEvent(self, event):
        ...
        for ip in qrylist:
            if self.checkForStop():
                return
            res = self.sf.bingIterate(searchString='ip:' + ip, opts={'timeout': self.opts['_fetchtimeout'], 'useragent': self.opts['_useragent'], 'count': self.opts['pages'], 'api_key': self.opts['api_key']})
            if res is None:
                return
            urls = res['urls']
            for url in urls:
                ...
                if site not in myres and site != ip:
                    if not self.opts['cohostsamedomain']:
                        if self.getTarget().matches(site, includeParents=True):
                            self.debug(f'Skipping {site} because it is on the same domain.')
                            continue
