...
class MgpstrModelTester:
    ...
    def __init__(self, parent, is_training=False, batch_size=13, image_size=(32, 128), patch_size=4, num_channels=3, max_token_length=27, num_character_labels=38, num_bpe_labels=99, num_wordpiece_labels=99, hidden_size=32, num_hidden_layers=2, num_attention_heads=4, mlp_ratio=4.0, patch_embeds_hidden_size=257, output_hidden_states=None):
        if False:
            return 10
        self.parent = parent
    ...
    def create_and_check_model(self, config, pixel_values):
        ...
        self.parent.assertEqual(generated_ids[0][0].shape, (self.batch_size, self.max_token_length, self.num_character_labels))
...
class MgpstrModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    def test_hidden_states_output(self):
        ...
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            self.assertListEqual(list(hidden_states[0].shape[-2:]), [self.model_tester.patch_embeds_hidden_size, self.model_tester.hidden_size])
        (config, inputs_dict) = self.model_tester.prepare_config_and_inputs_for_common()
        for model_class in self.all_model_classes:
...
class MgpstrModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        expected_slice = torch.tensor([[[-39.5397, -44.4024, -36.1844], [-61.4709, -63.8639, -58.3454], [-74.0225, -68.5494, -71.2164]]], device=torch_device)
        self.assertTrue(torch.allclose(outputs.logits[0][:, 1:4, 1:4], expected_slice, atol=0.0001))