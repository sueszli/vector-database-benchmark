...
if cc_pos == -1:
    ...
    asm_cmd = cmd.replace('_op.s', '_op.post.s').replace(' -o ', ' -g -o ')
    run_cmd(asm_cmd)
    s_path = cc_path.replace('_op.cc', '_op.post.s')
    init(cc_path, s_path)
    pass_asm(cc_path, s_path)
else:
    asm_cmd = cmd.replace('_op.so', '_op.post.s').replace('-lstdc++', '').replace('-ldl', '').replace('-shared', '-S').replace(' -o ', ' -g -o ')
    asm_cmd = remove_flags(asm_cmd, ['-l', '-L', '-Wl,', '.lib', '-shared'])
    run_cmd(asm_cmd)
    s_path = cc_path.replace('_op.cc', '_op.post.s')
    init(cc_path, s_path)
    pass_asm(cc_path, s_path)
    asm_cmd = cmd.replace('_op.cc', '_op.s').replace(' -g', '')
