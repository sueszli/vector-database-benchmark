...
class test_random_gam_gridsearch_specific:
    ...
    hyper_parameters = {'lambda': [0, 0.01], 'subspaces': [{'scale': [[1, 1, 1], [2, 2, 2]], 'gam_columns': [['C6', 'C7', 'C8']]}, {'scale': [[1, 1], [2, 2]], 'gam_columns': [['C6', 'C7'], ['C7', 'C8']]}, {'scale': [[1, 1], [2, 2]], 'gam_columns': [['C6', 'C7'], ['C7', 'C8']]}]}
    manual_gam_models = []
    num_grid_models = 0
            ...
            for subspace in self.hyper_parameters['subspaces']:
                for scale in subspace['scale']:
                    for gam_columns in subspace['gam_columns']:
                        self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family='multinomial', gam_columns=gam_columns, keep_gam_cols=True, scale=scale, lambda_=lambda_))

    def train_models(self):
        if False:
            i = 10
            return i + 15
        self.h2o_model = H2OGridSearch(H2OGeneralizedAdditiveEstimator(family='multinomial', keep_gam_cols=True), hyper_params=self.hyper_parameters, search_criteria=self.search_criteria)
