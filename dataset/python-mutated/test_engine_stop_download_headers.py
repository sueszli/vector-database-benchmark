    ...
    def test_crawler(self):
        ...
        for spider in (TestSpider, DictItemsSpider, AttrsItemsSpider, DataClassItemsSpider):
            ...
            with LogCapture() as log:
                yield run.run()
                log.check_present(('scrapy.core.downloader.handlers.http11', 'DEBUG', f'Download stopped for <GET http://localhost:{run.portno}/redirected> from signal handler HeadersReceivedCrawlerRun.headers_received'))
                log.check_present(('scrapy.core.downloader.handlers.http11', 'DEBUG', f'Download stopped for <GET http://localhost:{run.portno}/> from signal handler HeadersReceivedCrawlerRun.headers_received'))
                log.check_present(('scrapy.core.downloader.handlers.http11', 'DEBUG', f'Download stopped for <GET http://localhost:{run.portno}/numbers> from signal handler HeadersReceivedCrawlerRun.headers_received'))
