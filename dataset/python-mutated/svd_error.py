...
if __name__ == '__main__':
    ...
    logging.info('computing corpus * corpus^T')
    ...
    for factors in FACTORS:
        ...
        print('*' * 30, end='')
        print_error('baseline', aat, np.zeros((m, factors)), np.zeros(factors), ideal_fro, ideal_n2)
        if sparsesvd:
            ...
            del corpus_ram
            del vt
            (u, s) = (ut.T.astype(np.float32), s.astype(np.float32) ** 2)
            del ut
        ...
        for power_iters in POWER_ITERS:
            for chunksize in CHUNKSIZE:
                ...
                model = gensim.models.LsiModel(corpus, id2word=id2word, num_topics=factors, chunksize=chunksize, power_iters=power_iters)
                taken = time.time() - taken
                (u, s) = (model.projection.u.astype(np.float32), model.projection.s.astype(np.float32) ** 2)
                del model
                print('incremental SVD for %i factors, %i power iterations, chunksize %i took %s s (spectrum %f .. %f)' % (factors, power_iters, chunksize, taken, s[0], s[-1]))
            ...
            taken = time.time()
            model = gensim.models.LsiModel(corpus, id2word=id2word, num_topics=factors, chunksize=2000, onepass=False, power_iters=power_iters)
            taken = time.time() - taken
            (u, s) = (model.projection.u.astype(np.float32), model.projection.s.astype(np.float32) ** 2)
            del model
            print('multipass SVD for %i factors, %i power iterations took %s s (spectrum %f .. %f)' % (factors, power_iters, taken, s[0], s[-1]))
