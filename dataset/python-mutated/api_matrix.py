...
class MatrixSheet(Sheet):
    help = __doc__
    columns = [Column('room', width=12, getter=lambda c, r: r.room and r.room.display_name), ItemColumn('sender', width=0), Column('sender', width=10, getter=lambda c, r: r.sender.split(':')[0][1:]), Column('timestamp', width=16, type=date, getter=lambda c, r: r and r.origin_server_ts / 1000, fmtstr='%Y-%m-%d %H:%m'), ItemColumn('type', width=15), ItemColumn('content', width=0), ItemColumn('content.body', width=50), ItemColumn('received', type=vlen, width=0), ItemColumn('content.msgtype')]
