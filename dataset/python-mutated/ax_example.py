...
if __name__ == '__main__':
    ...
    algo = AxSearch(parameter_constraints=['x1 + x2 <= 2.0'], outcome_constraints=['l2norm <= 1.25'])
    algo = tune.search.ConcurrencyLimiter(algo, max_concurrent=4)
    scheduler = AsyncHyperBandScheduler()
    tuner = tune.Tuner(easy_objective, run_config=train.RunConfig(name='ax', stop={'timesteps_total': 100}), tune_config=tune.TuneConfig(metric='hartmann6', mode='min', search_alg=algo, scheduler=scheduler, num_samples=10 if args.smoke_test else 50), param_space={'iterations': 100, 'x1': tune.uniform(0.0, 1.0), 'x2': tune.uniform(0.0, 1.0), 'x3': tune.uniform(0.0, 1.0), 'x4': tune.uniform(0.0, 1.0), 'x5': tune.uniform(0.0, 1.0), 'x6': tune.uniform(0.0, 1.0)})
    results = tuner.fit()
