...
class SSDPBase:
    ...
    LISTEN_IP = 'listen_ip'
    TIMEOUT = 'timeout'
    DEFAULTS = {SIGNATURE: '__salt_master_service', PORT: 4520, LISTEN_IP: '0.0.0.0', TIMEOUT: 3, ANSWER: {}}
        ...
        if message.startswith(self.signature):
            try:
                ...
                if self.disable_hidden:
                    self._sendto('{}:E:{}'.format(self.signature, 'Invalid timestamp'), addr)
                return
            if datetime.datetime.fromtimestamp(timestamp) < datetime.datetime.now() - datetime.timedelta(seconds=20):
                if self.disable_hidden:
                    self._sendto('{}:E:{}'.format(self.signature, 'Timestamp is too old'), addr)
                self.log.debug('Received outdated package from %s:%s', *addr)
                return
            self.log.debug('Received "%s" from %s:%s', message, *addr)
            self._sendto(salt.utils.stringutils.to_bytes('{}:@:{}'.format(self.signature, salt.utils.json.dumps(self.answer, _json_module=_json))), addr)
        else:
            if self.disable_hidden:
                self._sendto(salt.utils.stringutils.to_bytes('{}:E:{}'.format(self.signature, 'Invalid packet signature'), addr))
...
class SSDPDiscoveryServer(SSDPBase):
        ...
        if not (local_addr or remote_addr):
            ...
            for (idx, addr) in ((0, local_addr), (1, remote_addr)):
                if addr is not None:
                    ...
                    infos = (yield asyncio.coroutines.From(loop.getaddrinfo(*addr, family=family, type=socket.SOCK_DGRAM, proto=proto, flags=flags)))
                    if not infos:
    ...
    def run(self):
        ...
        if asyncio.ported:
            (transport, protocol) = loop.run_until_complete(SSDPDiscoveryServer.create_datagram_endpoint(loop, protocol, local_addr=(listen_ip, port)))
        else:
            (transport, protocol) = loop.run_until_complete(loop.create_datagram_endpoint(protocol, local_addr=(listen_ip, port), allow_broadcast=True))
        try:
            loop.run_forever()
        finally:
...
class SSDPDiscoveryClient(SSDPBase):
    ...
    def _query(self):
        ...
        query = salt.utils.stringutils.to_bytes('{}{}'.format(self.signature, time.time()))
            ...
            for (addr, descriptions) in response.items():
                for data in descriptions:
                    ...
                    if msg.startswith(self.signature):
                        ...
                        self.log.debug("Service announcement at '%s:%s'. Response: '%s'", addr[0], addr[1], msg)
                        if ':E:' in msg:
                            err = msg.split(':E:')[-1]
                            self.log.error('Error response from the service publisher at %s: %s', addr, err)
                            if 'timestamp' in err:
                                self.log.error('Publisher sent shifted timestamp from %s', addr)
                        else:
                            if addr not in masters:
                                masters[addr] = []
                            masters[addr].append(salt.utils.json.loads(msg.split(':@:')[-1], _json_module=_json))
        return masters