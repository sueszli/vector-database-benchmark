...
class plot_fft_base(object):
    ...
    def __init__(self, datatype, filename, options):
        ...
        self.text_file = figtext(0.1, 0.94, 'File: %s' % filename, weight='heavy', size=self.text_size)
        self.text_file_pos = figtext(0.1, 0.88, 'File Position: ', weight='heavy', size=self.text_size)
        self.text_block = figtext(0.35, 0.88, 'Block Size: %d' % self.block_length, weight='heavy', size=self.text_size)
    ...
    def dofft(self, iq):
        if False:
            for i in range(10):
                print('nop')
        N = len(iq)
        iq_fft = numpy.fft.fftshift(numpy.fft.fft(iq))
    ...
    def make_plots(self):
        ...
        self.draw_time()
        self.plot_fft = self.sp_fft.plot([], 'bo-')
        self.draw_fft()
        draw()
    ...
    def zoom(self, event):
        ...
        if newxlim[0] != curxlim[0] or newxlim[1] != curxlim[1]:
            ...
            self.sp_fft.axis([freq.min(), freq.max(), iq_fft.min() - 10, iq_fft.max() + 10])
            draw()
