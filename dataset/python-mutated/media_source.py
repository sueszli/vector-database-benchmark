...
from homeassistant.components.media_source import BrowseError, BrowseMediaSource, MediaSource, MediaSourceItem, PlayMedia, Unresolvable
...
class SynologyPhotosMediaSource(MediaSource):
    ...
    async def async_browse_media(self, item: MediaSourceItem) -> BrowseMediaSource:
        ...
        return BrowseMediaSource(domain=DOMAIN, identifier=None, media_class=MediaClass.DIRECTORY, media_content_type=MediaClass.IMAGE, title='Synology Photos', can_play=False, can_expand=True, children_media_class=MediaClass.DIRECTORY, children=[*await self._async_build_diskstations(item)])
    ...
    async def _async_build_diskstations(self, item: MediaSourceItem) -> list[BrowseMediaSource]:
        ...
        if not item.identifier:
            ...
            for entry in self.entries:
                ret.append(BrowseMediaSource(domain=DOMAIN, identifier=entry.unique_id, media_class=MediaClass.DIRECTORY, media_content_type=MediaClass.IMAGE, title=f'{entry.title} - {entry.unique_id}', can_play=False, can_expand=True))
            return ret
        ...
        if identifier.album_id is None:
            try:
                ...
                return []
            ret = [BrowseMediaSource(domain=DOMAIN, identifier=f'{item.identifier}/0', media_class=MediaClass.DIRECTORY, media_content_type=MediaClass.IMAGE, title='All images', can_play=False, can_expand=True)]
            for album in albums:
                ret.append(BrowseMediaSource(domain=DOMAIN, identifier=f'{item.identifier}/{album.album_id}', media_class=MediaClass.DIRECTORY, media_content_type=MediaClass.IMAGE, title=album.name, can_play=False, can_expand=True))
            return ret
        album = SynoPhotosAlbum(int(identifier.album_id), '', 0)
        try:
            album_items = await diskstation.api.photos.get_items_from_album(album, 0, 1000)
        except SynologyDSMException:
            return []
        ret = []
        for album_item in album_items:
            ...
            if mime_type.startswith('image/'):
                ...
                ret.append(BrowseMediaSource(domain=DOMAIN, identifier=f'{identifier.unique_id}/{identifier.album_id}/{album_item.thumbnail_cache_key}/{album_item.file_name}', media_class=MediaClass.IMAGE, media_content_type=mime_type, title=album_item.file_name, can_play=True, can_expand=False, thumbnail=await self.async_get_thumbnail(album_item, diskstation)))
        return ret
    ...
    async def async_resolve_media(self, item: MediaSourceItem) -> PlayMedia:
        ...
        return PlayMedia(f'/synology_dsm/{identifier.unique_id}/{identifier.cache_key}/{identifier.file_name}', mime_type)
