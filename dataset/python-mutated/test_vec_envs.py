...
def test_vecenv_terminal_obs(vec_env_class, vec_env_wrapper):
    ...
    for step_num in range(1, max(step_nums) + 1):
        ...
        prev_obs_b = obs_b
    vec_env.close()
SPACES = collections.OrderedDict([('discrete', spaces.Discrete(2)), ('multidiscrete', spaces.MultiDiscrete([2, 3])), ('multibinary', spaces.MultiBinary(3)), ('continuous', spaces.Box(low=np.zeros(2), high=np.ones(2)))])
