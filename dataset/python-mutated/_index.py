...
_compress_getitem_kern = _core.ElementwiseKernel('T d, S ind, int32 minor', 'raw T answer', 'if (ind == minor) atomicAdd(&answer[0], d);', 'cupyx_scipy_sparse_compress_getitem')
_compress_getitem_complex_kern = _core.ElementwiseKernel('T real, T imag, S ind, int32 minor', 'raw T answer_real, raw T answer_imag', '\n    if (ind == minor) {\n    atomicAdd(&answer_real[0], real);\n    atomicAdd(&answer_imag[0], imag);\n    }\n    ', 'cupyx_scipy_sparse_compress_getitem_complex')
