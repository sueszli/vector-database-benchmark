...
from ray.serve._private.common import CONTROL_PLANE_CONCURRENCY_GROUP, DeploymentID, ReplicaTag, ServeComponentType, StreamingHTTPRequest, gRPCRequest
from ray.serve._private.config import DeploymentConfig
from ray.serve._private.constants import DEFAULT_LATENCY_BUCKET_MS, HEALTH_CHECK_METHOD, RAY_SERVE_GAUGE_METRIC_SET_PERIOD_S, RAY_SERVE_REPLICA_AUTOSCALING_METRIC_RECORD_PERIOD_S, RECONFIGURE_METHOD, SERVE_LOGGER_NAME, SERVE_NAMESPACE
from ray.serve._private.http_util import ASGIAppReplicaWrapper, ASGIMessageQueue, ASGIReceiveProxy, Response
from ray.serve._private.logging_utils import access_log_msg, configure_component_cpu_profiler, configure_component_logger, configure_component_memory_profiler, get_component_logger_file_path
from ray.serve._private.router import RequestMetadata
from ray.serve._private.utils import MetricsPusher, merge_dict, parse_import_path, wrap_to_ray_error
...
class RayServeReplica:
    ...
    async def reconfigure(self, deployment_config: DeploymentConfig):
        ...
        if deployment_config.logging_config:
            logging_config = LoggingConfig(**deployment_config.logging_config)
            configure_component_logger(component_type=ServeComponentType.DEPLOYMENT, component_name=self.deployment_id.name, component_id=self.replica_tag, logging_config=logging_config)
    ...
    async def wrap_user_method_call(self, request_metadata: RequestMetadata):
        ...
        method.
        """
        ray.serve.context._serve_request_context.set(ray.serve.context._RequestContext(request_metadata.route, request_metadata.request_id, self.deployment_id.app, request_metadata.multiplexed_model_id))
    ...
    async def call_user_method_with_grpc_unary_stream(self, request_metadata: RequestMetadata, request: gRPCRequest) -> AsyncGenerator[bytes, None]:
    ...
    async def call_user_method_grpc_unary(self, request_metadata: RequestMetadata, request: gRPCRequest) -> bytes:
