...
class AttributeValueCreate(AttributeMixin, ModelMutation):
    ...
    class Arguments:
        attribute_id = graphene.ID(required=True, name='attribute', description='Attribute to which value will be assigned.')
        input = AttributeValueCreateInput(required=True, description='Fields required to create an AttributeValue.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.ATTRIBUTE_VALUE_CREATED, description='An attribute value was created.'), WebhookEventInfo(type=WebhookEventAsyncType.ATTRIBUTE_UPDATED, description='An attribute was updated.')]
