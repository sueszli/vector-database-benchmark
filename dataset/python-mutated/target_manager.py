...
ALL_EXTENSIONS: Collection[FileExtension] = {ext for definition in LANGUAGE.definition_by_id.values() for ext in definition.exts if ext != FileExtension('')}
ECOSYSTEM_TO_LOCKFILES = {Ecosystem(Pypi()): ['Pipfile.lock', 'poetry.lock', 'requirements.txt', 'requirements3.txt'], Ecosystem(Npm()): ['package-lock.json', 'yarn.lock', 'pnpm-lock.yaml'], Ecosystem(Gem()): ['Gemfile.lock'], Ecosystem(Gomod()): ['go.mod'], Ecosystem(Cargo()): ['Cargo.lock'], Ecosystem(Maven()): ['maven_dep_tree.txt', 'gradle.lockfile'], Ecosystem(Composer()): ['composer.lock'], Ecosystem(Nuget()): ['packages.lock.json'], Ecosystem(Pub()): ['pubspec.lock']}
...
class FileTargetingLog:
    ...
    core_failure_lines_by_file: Mapping[Path, Tuple[Optional[int], List[out.RuleId]]] = Factory(dict)
    by_language: Dict[Union[Language, Ecosystem], Set[Path]] = Factory(lambda : defaultdict(set))
