...
class AveragePoolingBasicTest(testing.TestCase, parameterized.TestCase):
    ...
    def test_average_pooling1d(self, pool_size, strides, padding, data_format, input_shape, output_shape):
        if False:
            i = 10
            return i + 15
        self.run_layer_test(layers.AveragePooling1D, init_kwargs={'pool_size': pool_size, 'strides': strides, 'padding': padding, 'data_format': data_format}, input_shape=input_shape, expected_output_shape=output_shape, expected_num_trainable_weights=0, expected_num_non_trainable_weights=0, expected_num_losses=0, supports_masking=False)
    ...
    def test_average_pooling2d(self, pool_size, strides, padding, data_format, input_shape, output_shape):
        if False:
            while True:
                i = 10
        self.run_layer_test(layers.AveragePooling2D, init_kwargs={'pool_size': pool_size, 'strides': strides, 'padding': padding, 'data_format': data_format}, input_shape=input_shape, expected_output_shape=output_shape, expected_num_trainable_weights=0, expected_num_non_trainable_weights=0, expected_num_losses=0, supports_masking=False)
    ...
    def test_average_pooling3d(self, pool_size, strides, padding, data_format, input_shape, output_shape):
        if False:
            print('Hello World!')
        self.run_layer_test(layers.AveragePooling3D, init_kwargs={'pool_size': pool_size, 'strides': strides, 'padding': padding, 'data_format': data_format}, input_shape=input_shape, expected_output_shape=output_shape, expected_num_trainable_weights=0, expected_num_non_trainable_weights=0, expected_num_losses=0, supports_masking=False, run_mixed_precision_check=False)

class AveragePoolingCorrectnessTest(testing.TestCase, parameterized.TestCase):

    @parameterized.parameters((2, 1, 'valid', 'channels_last'), (2, 1, 'valid', 'channels_first'), ((2,), (2,), 'valid', 'channels_last'), ((2,), (2,), 'valid', 'channels_first'))
    def test_average_pooling1d(self, pool_size, strides, padding, data_format):
        if False:
            while True:
                i = 10
        inputs = np.arange(24, dtype='float32').reshape((2, 3, 4))
        layer = layers.AveragePooling1D(pool_size=pool_size, strides=strides, padding=padding, data_format=data_format)
        outputs = layer(inputs)
        expected = np_avgpool1d(inputs, pool_size, strides, padding, data_format)
        self.assertAllClose(outputs, expected)

    @parameterized.parameters((2, 1, 'same', 'channels_last'), (2, 1, 'same', 'channels_first'), ((2,), (2,), 'same', 'channels_last'), ((2,), (2,), 'same', 'channels_first'))
    @pytest.mark.skipif(backend.backend() == 'torch', reason='Same padding in Torch backend produces different results.')
    def test_average_pooling1d_same_padding(self, pool_size, strides, padding, data_format):
        if False:
            return 10
        inputs = np.arange(24, dtype='float32').reshape((2, 3, 4))
        layer = layers.AveragePooling1D(pool_size=pool_size, strides=strides, padding=padding, data_format=data_format)
    ...
    @parameterized.parameters((2, 1, 'valid', 'channels_last'), ((2, 3), (2, 2), 'valid', 'channels_last'))
    def test_average_pooling2d(self, pool_size, strides, padding, data_format):
        if False:
            print('Hello World!')
        inputs = np.arange(16, dtype='float32').reshape((1, 4, 4, 1))
        layer = layers.AveragePooling2D(pool_size=pool_size, strides=strides, padding=padding, data_format=data_format)
        outputs = layer(inputs)
        expected = np_avgpool2d(inputs, pool_size, strides, padding, data_format)
        self.assertAllClose(outputs, expected)

    @parameterized.parameters((2, (2, 1), 'same', 'channels_last'), (2, (2, 1), 'same', 'channels_first'), ((2, 2), (2, 2), 'same', 'channels_last'), ((2, 2), (2, 2), 'same', 'channels_first'))
    @pytest.mark.skipif(backend.backend() == 'torch', reason='Same padding in Torch backend produces different results.')
    def test_average_pooling2d_same_padding(self, pool_size, strides, padding, data_format):
        if False:
            return 10
        inputs = np.arange(16, dtype='float32').reshape((1, 4, 4, 1))
        layer = layers.AveragePooling2D(pool_size=pool_size, strides=strides, padding=padding, data_format=data_format)
    ...
    @parameterized.parameters((2, 1, 'valid', 'channels_last'), (2, 1, 'valid', 'channels_first'), ((2, 3, 2), (2, 2, 1), 'valid', 'channels_last'), ((2, 3, 2), (2, 2, 1), 'valid', 'channels_first'))
    def test_average_pooling3d(self, pool_size, strides, padding, data_format):
        ...
        layer = layers.AveragePooling3D(pool_size=pool_size, strides=strides, padding=padding, data_format=data_format)
        outputs = layer(inputs)
        expected = np_avgpool3d(inputs, pool_size, strides, padding, data_format)
        self.assertAllClose(outputs, expected)

    @parameterized.parameters((2, 1, 'same', 'channels_last'), (2, 1, 'same', 'channels_first'), ((2, 2, 2), (2, 2, 1), 'same', 'channels_last'), ((2, 2, 2), (2, 2, 1), 'same', 'channels_first'))
    @pytest.mark.skipif(backend.backend() == 'torch', reason='Same padding in Torch backend produces different results.')
    def test_average_pooling3d_same_padding(self, pool_size, strides, padding, data_format):
        if False:
            i = 10
            return i + 15
        inputs = np.arange(240, dtype='float32').reshape((2, 3, 4, 5, 2))
