...
class TestSpiderFootHelpers(unittest.TestCase):
    ...
    def test_extractLinksFromHtml_argument_data_containing_malformed_html_with_links_should_return_a_dict_of_urls(self):
        ...
        parse_links = SpiderFootHelpers.extractLinksFromHtml(url, '<!DOCTYPE html><html lang="en-US"><meta charset="UTF-8" /><link rel="pingback" href="http://spiderfoot.net/xmlrpc.php">example html content<unclosed tag><a href="http://spiderfoot.net/path"></a><a href="/relative-path"></a></html>', 'domains')
        self.assertIsInstance(parse_links, dict)
        self.assertEqual(parse_links, {'http://spiderfoot.net/xmlrpc.php': {'source': url, 'original': 'http://spiderfoot.net/xmlrpc.php'}, 'http://spiderfoot.net/path': {'source': url, 'original': 'http://spiderfoot.net/path'}, 'http://spiderfoot.net/relative-path': {'source': url, 'original': '/relative-path'}})
    ...
    def test_extractIbansFromText_should_return_a_list(self):
        ...
        ibans = ['ZZ21PACG0000000123456789', 'VG123456789012345', 'VG21PACG0000000123456111']
        for iban in ibans:
            with self.subTest(iban=iban):
                extract_ibans = SpiderFootHelpers.extractIbansFromText(iban)
                self.assertIsInstance(extract_ibans, list)
