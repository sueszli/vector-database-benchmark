...
from darts.dataprocessing.encoders import FutureCyclicEncoder, PastDatetimeAttributeEncoder
from darts.logging import get_logger
from darts.metrics import mae, rmse
from darts.models import CatBoostModel, LightGBMModel, LinearRegressionModel, NotImportedModule, RandomForest, RegressionModel, XGBModel
...
class TestRegressionModels:
    ...
    QuantileLinearRegressionModel = partialclass(LinearRegressionModel, likelihood='quantile', quantiles=[0.05, 0.5, 0.95], random_state=42)
    PoissonLinearRegressionModel = partialclass(LinearRegressionModel, likelihood='poisson', random_state=42)
    PoissonXGBModel = partialclass(XGBModel, likelihood='poisson', random_state=42, tree_method='exact')
    QuantileXGBModel = partialclass(XGBModel, likelihood='quantile', random_state=42, tree_method='exact')
    models.extend([QuantileLinearRegressionModel, PoissonLinearRegressionModel, PoissonXGBModel, QuantileXGBModel])
    univariate_accuracies = [0.03, 1e-13, 1e-13, 0.8, 0.4, 0.1, 0.5]
    multivariate_accuracies = [0.3, 1e-13, 1e-13, 0.8, 0.4, 0.15, 0.4]
    multivariate_multiseries_accuracies = [0.05, 1e-13, 1e-13, 0.8, 0.4, 0.1, 0.4]
    lgbm_w_categorical_covariates = NotImportedModule
    if lgbm_available:
        QuantileLightGBMModel = partialclass(LightGBMModel, likelihood='quantile', quantiles=[0.05, 0.5, 0.95], random_state=42)
        PoissonLightGBMModel = partialclass(LightGBMModel, likelihood='poisson', random_state=42)
        models += [LightGBMModel, QuantileLightGBMModel, PoissonLightGBMModel]
        lgbm_w_categorical_covariates = LightGBMModel(lags=1, lags_past_covariates=1, lags_future_covariates=[1], output_chunk_length=1, categorical_future_covariates=['fut_cov_promo_mechanism'], categorical_past_covariates=['past_cov_cat_dummy'], categorical_static_covariates=['product_id'])
        univariate_accuracies += [0.3, 0.5, 0.4]
        multivariate_accuracies += [0.4, 0.4, 0.4]
        multivariate_multiseries_accuracies += [0.05, 0.4, 0.4]
    if cb_available:
        QuantileCatBoostModel = partialclass(CatBoostModel, likelihood='quantile', quantiles=[0.05, 0.5, 0.95], random_state=42)
        PoissonCatBoostModel = partialclass(CatBoostModel, likelihood='poisson', random_state=42)
        NormalCatBoostModel = partialclass(CatBoostModel, likelihood='gaussian', random_state=42)
        models += [CatBoostModel, QuantileCatBoostModel, PoissonCatBoostModel, NormalCatBoostModel]
        univariate_accuracies += [0.75, 0.001, 0.1, 1e-05]
        multivariate_accuracies += [0.75, 0.001, 0.15, 1e-05]
        multivariate_multiseries_accuracies += [0.75, 0.001, 0.1, 0.001]
    (target_series, past_covariates, future_covariates) = dummy_timeseries(length=100, n_series=3, comps_target=3, comps_pcov=2, comps_fcov=1, multiseries_offset=10, pcov_offset=0, fcov_offset=0)
...
class TestProbabilisticRegressionModels:
    models_cls_kwargs_errs = [(LinearRegressionModel, {'lags': 2, 'likelihood': 'quantile', 'random_state': 42, 'multi_models': True}, 0.6), (LinearRegressionModel, {'lags': 2, 'likelihood': 'poisson', 'random_state': 42, 'multi_models': True}, 0.6), (XGBModel, {'lags': 2, 'likelihood': 'poisson', 'random_state': 42, 'multi_models': True}, 0.6), (XGBModel, {'lags': 2, 'likelihood': 'quantile', 'quantiles': [0.1, 0.3, 0.5, 0.7, 0.9], 'random_state': 42, 'multi_models': True}, 0.4)]
    if lgbm_available:
        models_cls_kwargs_errs += [(LightGBMModel, {'lags': 2, 'likelihood': 'quantile', 'random_state': 42, 'multi_models': True}, 0.4), (LightGBMModel, {'lags': 2, 'likelihood': 'quantile', 'quantiles': [0.1, 0.3, 0.5, 0.7, 0.9], 'random_state': 42, 'multi_models': True}, 0.4), (LightGBMModel, {'lags': 2, 'likelihood': 'poisson', 'random_state': 42, 'multi_models': True}, 0.6)]
    if cb_available:
        models_cls_kwargs_errs += [(CatBoostModel, {'lags': 2, 'likelihood': 'quantile', 'random_state': 42, 'multi_models': True}, 0.05), (CatBoostModel, {'lags': 2, 'likelihood': 'quantile', 'quantiles': [0.1, 0.3, 0.5, 0.7, 0.9], 'random_state': 42, 'multi_models': True}, 0.05), (CatBoostModel, {'lags': 2, 'likelihood': 'poisson', 'random_state': 42, 'multi_models': True}, 0.6), (CatBoostModel, {'lags': 2, 'likelihood': 'gaussian', 'random_state': 42, 'multi_models': True}, 0.05)]
