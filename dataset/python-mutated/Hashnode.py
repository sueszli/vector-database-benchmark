...
class Hashnode(AsyncGeneratorProvider):
    ...
    @classmethod
    async def create_async_generator(cls, model: str, messages: Messages, search_type: str=SearchTypes.websearch, proxy: str=None, **kwargs) -> AsyncResult:
        ...
        async with ClientSession(headers=headers) as session:
            ...
            if search_type == 'websearch':
                async with session.post(f'{cls.url}/api/ai/rix/search', json={'prompt': prompt}, proxy=proxy) as response:
                    response.raise_for_status()
                    cls._sources = (await response.json())['result']
            data = {'chatId': secrets.token_hex(16).zfill(32), 'history': messages, 'prompt': prompt, 'searchType': search_type, 'urlToScan': None, 'searchResults': cls._sources}
            async with session.post(f'{cls.url}/api/ai/rix/completion', json=data, proxy=proxy) as response:
                response.raise_for_status()
