...
class ServerConfig(Config):
    ...
    keys = [libqtile.config.Key(['mod4'], 'Return', lazy.spawn('xterm')), libqtile.config.Key(['mod4'], 't', lazy.spawn('xterm'), desc='dummy description'), libqtile.config.Key([], 'y', desc='noop'), libqtile.config.KeyChord(['mod4'], 'q', [libqtile.config.KeyChord([], 'q', [libqtile.config.Key([], 'a', lazy.togroup('a'))]), libqtile.config.Key([], 'b', lazy.togroup('b'))], mode='named')]
    mouse = []
    groups = [libqtile.config.Group('a'), libqtile.config.Group('b'), libqtile.config.Group('c')]
    layouts = [libqtile.layout.Stack(num_stacks=1), libqtile.layout.Stack(num_stacks=2), libqtile.layout.Stack(num_stacks=3)]
    floating_layout = libqtile.resources.default_config.floating_layout
    screens = [libqtile.config.Screen(bottom=libqtile.bar.Bar([libqtile.widget.TextBox(name='one')], 20)), libqtile.config.Screen(bottom=libqtile.bar.Bar([libqtile.widget.TextBox(name='two')], 20))]
