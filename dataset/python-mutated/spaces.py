...
class Space(Component):
    ...
    """
    meta = {'strict': False, 'allow_inheritance': True}
    children = fof.ListField(fof.EmbeddedDocumentField(Component), validation=_validate_children)
    orientation = fof.StringField(choices=['horizontal', 'vertical'], default=None)
    active_child = fof.StringField(default=None)
samples_panel = Panel(type='Samples', pinned=True)
default_spaces = Space(children=[samples_panel], active_child=samples_panel.component_id)