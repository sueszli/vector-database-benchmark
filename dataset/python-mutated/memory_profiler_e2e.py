...
class MemoryProfilerModuleEndToEndTest(unittest.TestCase):
    ...
    def setUp(self):
        if False:
            return 10
        program_stats = memory_profiler.MemoryProfiler(_MODULE_FILENAME).run()
    ...
    def testRequest(self):
        ...
        self.assertEqual(stats['codeEvents'][0][4], 'vprof/tests/test_pkg/dummy_module.py')
...
class MemoryProfilerFunctionEndToEndTest(unittest.TestCase):
    ...
    def testRequest(self):
        ...
        self.assertEqual(stats['m']['objectName'], '_func @ %s (function)' % curr_filename)
