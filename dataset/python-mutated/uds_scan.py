...
from typing import Dict, Optional, NamedTuple, List, Type, Any, Iterable, cast, Union, Set, Sequence
_PointOfInterest = NamedTuple('_PointOfInterest', [('memory_address', int), ('direction', bool), ('memorySizeLen', int), ('memoryAddressLen', int), ('memorySize', int)])
...
class UDS_DSCEnumerator(UDS_Enumerator, StateGeneratingServiceEnumerator):
    ...
    _supported_kwargs['scan_range'] = ((list, tuple, range), lambda x: max(x) < 256 and min(x) >= 0)
...
class UDS_ServiceEnumerator(UDS_Enumerator):
    ...
    _supported_kwargs = copy.copy(ServiceEnumerator._supported_kwargs)
    _supported_kwargs.update({'request_length': (int, lambda x: 1 <= x < 5)})
    _supported_kwargs['scan_range'] = ((list, tuple, range), lambda x: max(x) < 256 and min(x) >= 0)
...
class UDS_RDBIEnumerator(UDS_Enumerator):
    ...
    _supported_kwargs = copy.copy(ServiceEnumerator._supported_kwargs)
    _supported_kwargs['scan_range'] = ((list, tuple, range), lambda x: max(x) < 65536 and min(x) >= 0)
...
class UDS_RDBIRandomEnumerator(UDS_RDBIEnumerator):
    ...
    _supported_kwargs.update({'probe_start': (int, lambda x: 0 <= x <= 65535), 'probe_end': (int, lambda x: 0 <= x <= 65535)})
    block_size = 2 ** 6
...
class UDS_SA_XOR_Enumerator(UDS_SAEnumerator, StateGenerator):
    ...
    _transition_function_args = dict()
...
class UDS_RCEnumerator(UDS_Enumerator):
    ...
    _supported_kwargs = copy.copy(ServiceEnumerator._supported_kwargs)
    _supported_kwargs.update({'type_list': (list, lambda x: max(x) < 256 and min(x) >= 0)})
    _supported_kwargs['scan_range'] = ((list, tuple, range), lambda x: max(x) < 65536 and min(x) >= 0)
