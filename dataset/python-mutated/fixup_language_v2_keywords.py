...
class languageCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'analyze_entities': ('document', 'encoding_type'), 'analyze_sentiment': ('document', 'encoding_type'), 'annotate_text': ('document', 'features', 'encoding_type'), 'classify_text': ('document',), 'moderate_text': ('document',)}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=languageCallTransformer()):
    if False:
        while True:
            i = 10
