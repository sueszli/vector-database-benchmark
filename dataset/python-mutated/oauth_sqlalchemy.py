...
installation_store = SQLAlchemyInstallationStore(client_id=client_id, engine=engine, logger=logger)
token_rotator = TokenRotator(client_id=client_id, client_secret=client_secret)
state_store = SQLAlchemyOAuthStateStore(engine=engine, logger=logger, expiration_seconds=300)
try:
