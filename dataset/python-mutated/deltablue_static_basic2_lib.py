...
class Planner(object):
    ...
    def make_plan(self, sources: CheckedList[Constraint]) -> Plan:
        if False:
            print('Hello World!')
        mark = self.new_mark()
        plan = Plan()
    ...
    def extract_plan_from_constraints(self, constraints: CheckedList[Constraint]) -> Plan:
        if False:
            print('Hello World!')
        sources: CheckedList[Constraint] = []
        x = len(constraints)
        i = 0
        while i < x:
            c = constraints[i]
