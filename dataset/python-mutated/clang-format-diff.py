...
def main():
    if False:
        i = 10
        return i + 15
    parser = argparse.ArgumentParser(description='Reformat changed lines in diff. Without -i option just output the diff that would be introduced.')
    parser.add_argument('-i', action='store_true', default=False, help='apply edits to files instead of displaying a diff')
    ...
    parser.add_argument('-iregex', metavar='PATTERN', default='.*\\.(cpp|cc|c\\+\\+|cxx|c|cl|h|hpp|m|mm|inc|js|ts|proto|protodevel|java)', help='custom pattern selecting file paths to reformat (case insensitive, overridden by -regex)')
    parser.add_argument('-sort-includes', action='store_true', default=False, help='let clang-format sort include blocks')
