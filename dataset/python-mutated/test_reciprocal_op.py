...
class TestReciprocalOp(OpTest):
    ...
    def prepare_inputs(self):
        if False:
            i = 10
            return i + 15
        self.inputs = {'x': np.random.random(self.case['x_shape']).astype(self.case['x_dtype'])}
...
class TestReciprocalShape(TestCaseHelper):
    ...
    def init_attrs(self):
        ...
        self.inputs = [{'x_shape': [1024]}, {'x_shape': [512, 256]}, {'x_shape': [128, 64, 32]}, {'x_shape': [16, 8, 4, 2]}, {'x_shape': [16, 8, 4, 2, 1]}, {'x_shape': [1]}, {'x_shape': [1, 1, 1, 1, 1]}]
        self.dtypes = [{'x_dtype': 'float64'}, {'x_dtype': 'float32'}, {'x_dtype': 'float16'}]
        self.attrs = []
