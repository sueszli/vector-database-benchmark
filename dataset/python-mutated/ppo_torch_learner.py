...
from ray.rllib.algorithms.ppo.ppo_learner import LEARNER_RESULTS_KL_KEY, LEARNER_RESULTS_CURR_KL_COEFF_KEY, LEARNER_RESULTS_VF_EXPLAINED_VAR_KEY, LEARNER_RESULTS_VF_LOSS_UNCLIPPED_KEY, PPOLearner, PPOLearnerHyperparameters
...
class PPOTorchLearner(PPOLearner, TorchLearner):
    ...
    def compute_loss_for_module(self, *, module_id: ModuleID, hps: PPOLearnerHyperparameters, batch: NestedDict, fwd_out: Mapping[str, TensorType]) -> TensorType:
        ...
        action_dist_class_exploration = self.module[module_id].unwrapped().get_exploration_action_dist_cls()
        ...
        logp_ratio = torch.exp(curr_action_dist.logp(batch[SampleBatch.ACTIONS]) - batch[SampleBatch.ACTION_LOGP])
        if hps.use_kl_loss:
        ...
        surrogate_loss = torch.min(batch[Postprocessing.ADVANTAGES] * logp_ratio, batch[Postprocessing.ADVANTAGES] * torch.clamp(logp_ratio, 1 - hps.clip_param, 1 + hps.clip_param))
        if hps.use_critic:
