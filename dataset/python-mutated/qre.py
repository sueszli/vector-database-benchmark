...
class Solver(object):
    ...
    def __init__(self, temperature=0.0, proj_grad=True, euclidean=False, cheap=False, lrs=(0.01, 0.1), rnd_init=False, seed=None, **kwargs):
        if False:
            return 10
        'Ctor.'
        del kwargs
    ...
    def compute_gradients(self, params, payoff_matrices):
        ...
        if self.cheap:
            return cheap_gradients(self.random, *params, payoff_matrices, self.num_players, self.temperature, self.proj_grad)
        else:
            return gradients(*params, payoff_matrices, self.num_players, self.temperature, self.proj_grad)
...
def gradients(dist, y, payoff_matrices, num_players, temperature=0.0, proj_grad=True):
    if False:
        for i in range(10):
            print('nop')
...
def cheap_gradients(random, dist, y, payoff_matrices, num_players, temperature=0.0, proj_grad=True):
    if False:
        print('Hello World!')
