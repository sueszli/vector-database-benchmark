...
class WebSocketProtocol(HttpProtocol):
    __slots__ = ('websocket', 'websocket_timeout', 'websocket_max_size', 'websocket_ping_interval', 'websocket_ping_timeout')
    ...
    async def websocket_handshake(self, request, subprotocols: Optional[Sequence[str]]=None):
        try:
            if subprotocols is not None:
                subprotocols = cast(Optional[Sequence[Subprotocol]], list([Subprotocol(subprotocol) for subprotocol in subprotocols]))
            ws_proto = ServerProtocol(max_size=self.websocket_max_size, subprotocols=subprotocols, state=OPEN, logger=logger)
        ...
        self.websocket = WebsocketImplProtocol(ws_proto, ping_interval=self.websocket_ping_interval, ping_timeout=self.websocket_ping_timeout, close_timeout=self.websocket_timeout)
