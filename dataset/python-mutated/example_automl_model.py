...
with DAG(dag_id=DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), catchup=False, user_defined_macros={'get_target_column_spec': get_target_column_spec, 'target': 'Deposit', 'extract_object_id': extract_object_id}, tags=['example', 'automl', 'model']) as dag:
    ...
    move_dataset_file = GCSSynchronizeBucketsOperator(task_id='move_data_to_bucket', source_bucket=RESOURCE_DATA_BUCKET, source_object='automl/datasets/model', destination_bucket=DATA_SAMPLE_GCS_BUCKET_NAME, destination_object='automl', recursive=True)
    ...
    list_columns_spec = AutoMLTablesListColumnSpecsOperator(task_id='list_columns_spec', dataset_id=dataset_id, table_spec_id="{{ extract_object_id(task_instance.xcom_pull('list_tables_spec')[0]) }}", location=GCP_AUTOML_LOCATION, project_id=GCP_PROJECT_ID)
    update = deepcopy(DATASET)
    update['name'] = '{{ task_instance.xcom_pull("create_dataset")["name"] }}'
    update['tables_dataset_metadata']['target_column_spec_id'] = "{{ get_target_column_spec(task_instance.xcom_pull('list_columns_spec'), target) }}"
    update_dataset = AutoMLTablesUpdateDatasetOperator(task_id='update_dataset', dataset=update, location=GCP_AUTOML_LOCATION)
    create_model = AutoMLTrainModelOperator(task_id='create_model', model=MODEL, location=GCP_AUTOML_LOCATION, project_id=GCP_PROJECT_ID)
    model_id = create_model.output['model_id']
    get_model = AutoMLGetModelOperator(task_id='get_model', model_id=model_id, location=GCP_AUTOML_LOCATION, project_id=GCP_PROJECT_ID)
    deploy_model = AutoMLDeployModelOperator(task_id='deploy_model', model_id=model_id, location=GCP_AUTOML_LOCATION, project_id=GCP_PROJECT_ID)
    predict_task = AutoMLPredictOperator(task_id='predict_task', model_id=model_id, payload={'row': {'values': PREDICT_VALUES}}, location=GCP_AUTOML_LOCATION, project_id=GCP_PROJECT_ID)
    batch_predict_task = AutoMLBatchPredictOperator(task_id='batch_predict_task', model_id=model_id, input_config=IMPORT_INPUT_CONFIG, output_config=IMPORT_OUTPUT_CONFIG, location=GCP_AUTOML_LOCATION, project_id=GCP_PROJECT_ID)
    ...
    delete_dataset = AutoMLDeleteDatasetOperator(task_id='delete_dataset', dataset_id=dataset_id, location=GCP_AUTOML_LOCATION, project_id=GCP_PROJECT_ID, trigger_rule=TriggerRule.ALL_DONE)
    ...
    [create_bucket >> move_dataset_file, create_dataset] >> import_dataset >> list_tables_spec >> list_columns_spec >> update_dataset >> create_model >> get_model >> deploy_model >> predict_task >> batch_predict_task >> delete_model >> delete_dataset >> delete_bucket
