...
def test_categorical_risk_analysis_on_string_field(topic_id: str, subscription_id: str, bigquery_project: str, capsys: pytest.CaptureFixture) -> None:
    if False:
        return 10
    risk.categorical_risk_analysis(GCLOUD_PROJECT, TABLE_PROJECT, BIGQUERY_DATASET_ID, BIGQUERY_HARMFUL_TABLE_ID, UNIQUE_FIELD, topic_id, subscription_id)
...
@pytest.mark.flaky(max_runs=3, min_passes=1)
def test_categorical_risk_analysis_on_number_field(topic_id: str, subscription_id: str, bigquery_project: str, capsys: pytest.CaptureFixture) -> None:
    if False:
        while True:
            i = 10
    risk.categorical_risk_analysis(GCLOUD_PROJECT, TABLE_PROJECT, BIGQUERY_DATASET_ID, BIGQUERY_HARMFUL_TABLE_ID, NUMERIC_FIELD, topic_id, subscription_id)
    (out, _) = capsys.readouterr()
    assert 'Most common value occurs' in out
    assert 'Job name:' in out
    for line in str(out).split('\n'):
