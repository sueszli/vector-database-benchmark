...
class test_random_gam_gridsearch_generic:
    ...
    hyper_parameters = {'scale': [[1, 1], [2, 2]], 'gam_columns': [['C11', 'C12'], ['C12', 'C13']], 'bs': [[0, 2], [2, 0]], 'lambda': [0, 0.01]}
    manual_gam_models = []
    num_grid_models = 0
            ...
            for gam_columns in self.hyper_parameters['gam_columns']:
                for lambda_ in self.hyper_parameters['lambda']:
                    for bs in self.hyper_parameters['bs']:
                        self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family='gaussian', scale=scale, gam_columns=gam_columns, bs=bs, keep_gam_cols=True, lambda_=lambda_))
    ...
    def match_models(self):
        ...
        for model in self.manual_gam_models:
            ...
            for grid_search_model in self.h2o_model.models:
                if grid_search_model.actual_params['gam_columns'] == gam_columns and grid_search_model.actual_params['scale'] == scale and (grid_search_model.actual_params['bs'] == bs) and (grid_search_model.actual_params['lambda'] == lambda_):
