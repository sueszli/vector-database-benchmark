...
WSException = (websockets.exceptions.WebSocketException, websockets.exceptions.ConnectionClosedError, fastapi.WebSocketException, fastapi.WebSocketDisconnect, WorkerDisconnectException)
router = fastapi.APIRouter(prefix='/workers', tags=['workers'])
...
async def add_worker_connect_event(session: database.AsyncSession, worker_id: str, worker_info: inference.WorkerInfo):
    event = models.DbWorkerEvent(worker_id=worker_id, event_type=models.WorkerEventType.connect, worker_info=worker_info)
    session.add(event)
...
async def handle_worker(websocket: fastapi.WebSocket, api_key: str=worker_utils.api_key_header, protocol_version: str=worker_utils.protocol_version_header):
    ...
    try:
        ...
        logger.warning(f'handle_worker: e.status_code={e.status_code!r} e.detail={e.detail!r}')
    ...
    logger.info(f'handle_worker: worker_id={worker_id!r}')
    worker_info = await worker_utils.receive_worker_info(websocket)
    logger.info(f'handle_worker: worker_info={worker_info!r}')
    ...
    worker_session = worker_utils.WorkerSession(worker_id=worker_id, worker_info=worker_info)
    ...
    try:
        ...
        logger.info(f'handle_worker: worker_id={worker_id!r} started')
        while True:
            ...
            (done, pending_futures) = await asyncio.wait(pending_futures, timeout=settings.worker_ping_interval, return_when=asyncio.FIRST_COMPLETED)
            ftr: asyncio.Future
            for ftr in done:
                result = ftr.result()
                if result is None:
                    logger.error(f'handle_worker: worker_id={worker_id!r} received None from queue. This should never happen.')
                    raise RuntimeError('Received None from queue. This should never happen.')
                elif isinstance(result, tuple):
                    try:
                        (_, message_id) = result
                        work_request = await initiate_work_for_message(websocket=websocket, work_queue=work_queue, message_id=message_id, worker_id=worker_id, worker_config=worker_config)
                        work_request_map[work_request.id] = WorkRequestContainer(work_request=work_request, message_id=message_id)
                    except chat_schema.MessageCancelledException as e:
                        logger.warning(f'Message was cancelled before work could be initiated: e.message_id={e.message_id!r}')
                    except chat_schema.MessageTimeoutException as e:
                        logger.warning(f'Message timed out before work could be initiated: e.message.id={e.message.id!r}')
                    ...
                    try:
                        ...
                        match worker_response.response_type:
                            ...
                            case 'token':
                                ...
                                await handle_token_response(work_request_map=work_request_map, response=worker_response)
                            case 'generated_text':
                                worker_response = cast(inference.GeneratedTextResponse, worker_response)
                                await handle_generated_text_response(work_request_map=work_request_map, response=worker_response)
                                await _update_session(worker_response.metrics)
                            case 'error':
                                worker_response = cast(inference.ErrorResponse, worker_response)
                                await handle_error_response(work_request_map=work_request_map, response=worker_response)
                                await _update_session(worker_response.metrics)
                            case 'general_error':
                                worker_response = cast(inference.GeneralErrorResponse, worker_response)
                                await handle_general_error_response(response=worker_response)
                                await _update_session(worker_response.metrics)
                            case 'safe_prompt':
                                logger.info('Received safe prompt response')
                                worker_response = cast(inference.SafePromptResponse, worker_response)
                                await handle_safe_prompt_response(response=worker_response, work_request_map=work_request_map)
                            case 'plugin_intermediate':
                                worker_response = cast(inference.PluginIntermediateResponse, worker_response)
                                await handle_plugin_intermediate_response(work_request_map=work_request_map, response=worker_response)
                            case _:
        ...
        for container in work_request_map.values():
            try:
                ...
                if container.num_responses == 0:
                    logger.warning(f'Marking message_id={message_id!r} as pending since no work was done.')
                    ...
                    logger.warning(f'Aborting message_id={message_id!r}')
                    await abort_message(message_id=message_id, error='Aborted due to worker error.')
            except Exception as e:
                logger.exception(f'Error while trying to reset work for message_id={message_id!r}: {str(e)}')
    finally:
...
async def initiate_work_for_message(*, websocket: fastapi.WebSocket, work_queue: queueing.RedisQueue, message_id: str, worker_id: str, worker_config: inference.WorkerConfig) -> inference.WorkRequest:
    async with deps.manual_create_session() as session:
        cr = chat_repository.ChatRepository(session=session)
        message = await cr.start_work(message_id=message_id, worker_id=worker_id, worker_config=worker_config)
        work_request = await worker_utils.build_work_request(session, message.id)
    logger.info(f'Created work_request={work_request!r} with len(work_request.thread.messages)={len(work_request.thread.messages)!r}')
    try:
...
async def handle_token_response(response: inference.TokenResponse, work_request_map: WorkRequestContainerMap):
    work_response_container = get_work_request_container(work_request_map, response.request_id)
    message_queue = queueing.message_queue(deps.redis_client, message_id=work_response_container.message_id)
    await message_queue.enqueue(response.json())
    work_response_container.num_responses += 1

async def handle_plugin_intermediate_response(response: inference.PluginIntermediateResponse, work_request_map: WorkRequestContainerMap):
    work_response_container = get_work_request_container(work_request_map, response.request_id)
    message_queue = queueing.message_queue(deps.redis_client, message_id=work_response_container.message_id)
    await message_queue.enqueue(response.json())
    work_response_container.num_responses += 1

async def handle_generated_text_response(response: inference.GeneratedTextResponse, work_request_map: WorkRequestContainerMap):
    try:
        work_response_container = get_work_request_container(work_request_map, response.request_id)
        message_id = work_response_container.message_id
        async with deps.manual_create_session() as session:
            cr = chat_repository.ChatRepository(session=session)
            message = await cr.complete_work(message_id=message_id, content=response.text, used_plugin=response.used_plugin)
            logger.info(f'Completed work for message_id={message_id!r}')
        message_packet = inference.InternalFinishedMessageResponse(message=message.to_read())
        message_queue = queueing.message_queue(deps.redis_client, message_id=message_id)
...
async def handle_error_response(response: inference.ErrorResponse, work_request_map: WorkRequestContainerMap):
    logger.warning(f'Got error response={response!r}')
    try:
        work_response_container = get_work_request_container(work_request_map, response.request_id)
        message_id = work_response_container.message_id
...
async def handle_general_error_response(response: inference.GeneralErrorResponse):
    logger.warning(f'Got general error response={response!r}')

async def handle_safe_prompt_response(response: inference.SafePromptResponse, work_request_map: WorkRequestContainerMap):
    """
...
async def handle_timeout(message: inference.MessageRead):
    response = inference.InternalErrorResponse(error='Timeout', message=message)
    message_queue = queueing.message_queue(deps.redis_client, message_id=message.id)
    await message_queue.enqueue(response.json())