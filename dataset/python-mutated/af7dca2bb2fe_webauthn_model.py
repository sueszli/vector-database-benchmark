...
def upgrade():
    if False:
        for i in range(10):
            print('nop')
    op.create_table('user_security_keys', sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False), sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=False), sa.Column('credential_id', sa.String, nullable=False), sa.Column('public_key', sa.String, nullable=True), sa.Column('sign_count', sa.Integer, nullable=True), sa.ForeignKeyConstraint(['user_id'], ['users.id'], initially='DEFERRED', deferrable=True), sa.PrimaryKeyConstraint('id'), sa.UniqueConstraint('credential_id'), sa.UniqueConstraint('public_key'))
