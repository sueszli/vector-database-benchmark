...
if __name__ == '__main__':
    ...
    width = 500
    height = 500
    c1 = Canvas(title='Unstructured 2D - 2D colormap', size=(width, height), position=(0, 40), x=loc[:, 0], y=loc[:, 1], u=vec[:, 0], v=vec[:, 1], colormap=create_colormap2d_4dirs(size=128), keys='interactive')
    c2 = Canvas(title='Unstructured 2D - 1D colormap', size=(width, height), position=(width + 20, 40), x=loc[:, 0], y=loc[:, 1], u=vec[:, 0], colormap=create_colormap1d_hot(size=128), keys='interactive')
