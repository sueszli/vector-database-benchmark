...
class NLITest(unittest.TestCase):
    ...
    def test_run_with_model_from_modelhub(self):
        ...
        pipeline_ins = pipeline(task=Tasks.nli, model=model, preprocessor=tokenizer)
    ...
    def test_run_with_model_name(self):
        ...
        with self.regress_tool.monitor_module_single_forward(pipeline_ins.model, 'sbert_nli', compare_fn=IgnoreKeyFn('.*intermediate_act_fn')):
            print(pipeline_ins(input=(self.sentence1, self.sentence2)))
    ...
    def test_fact_checking_model(self):
        if False:
            i = 10
            return i + 15
        pipeline_ins = pipeline(task=Tasks.nli, model=self.model_id_fact_checking, model_revision='v1.0.1')
        print(pipeline_ins(input=(self.sentence1, self.sentence2)))
    ...
    def test_peer_model(self):
        if False:
            print('Hello World!')
        pipeline_ins = pipeline(task=Tasks.nli, model=self.model_id_peer, model_revision='v1.0.0')
