...
if is_tf_available():
    ...
    from transformers.models.roberta.modeling_tf_roberta import TF_ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST, TFRobertaForCausalLM, TFRobertaForMaskedLM, TFRobertaForMultipleChoice, TFRobertaForQuestionAnswering, TFRobertaForSequenceClassification, TFRobertaForTokenClassification, TFRobertaModel
...
class TFRobertaModelTester:
    ...
    def create_and_check_causal_lm_model(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            return 10
        config.is_decoder = True
    ...
    def create_and_check_causal_lm_model_as_decoder(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels, encoder_hidden_states, encoder_attention_mask):
        if False:
            while True:
                i = 10
        config.add_cross_attention = True
    ...
    def create_and_check_causal_lm_model_past(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            for i in range(10):
                print('nop')
        config.is_decoder = True
    ...
    def create_and_check_causal_lm_model_past_with_attn_mask(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            while True:
                i = 10
        config.is_decoder = True
    ...
    def create_and_check_causal_lm_model_past_large_inputs(self, config, input_ids, token_type_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            i = 10
            return i + 15
        config.is_decoder = True
...
class TFRobertaModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TFRobertaModel, TFRobertaForCausalLM, TFRobertaForMaskedLM, TFRobertaForSequenceClassification, TFRobertaForTokenClassification, TFRobertaForQuestionAnswering) if is_tf_available() else ()
    pipeline_model_mapping = {'feature-extraction': TFRobertaModel, 'fill-mask': TFRobertaForMaskedLM, 'question-answering': TFRobertaForQuestionAnswering, 'text-classification': TFRobertaForSequenceClassification, 'text-generation': TFRobertaForCausalLM, 'token-classification': TFRobertaForTokenClassification, 'zero-shot': TFRobertaForSequenceClassification} if is_tf_available() else {}
...
class TFRobertaModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        ...
        expected_slice = tf.constant([[[33.8802, -4.3103, 22.7761], [4.6539, -2.8098, 13.6253], [1.8228, -3.6898, 8.86]]])
    ...
    def test_inference_no_head(self):
        ...
        input_ids = tf.constant([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        output = model(input_ids)[0]
        expected_slice = tf.constant([[[-0.0231, 0.0782, 0.0074], [-0.1854, 0.054, -0.0175], [0.0548, 0.0799, 0.1687]]])
        self.assertTrue(numpy.allclose(output[:, :3, :3].numpy(), expected_slice.numpy(), atol=0.0001))
    ...
    def test_inference_classification_head(self):
        ...
        self.assertTrue(numpy.allclose(output.numpy(), expected_tensor.numpy(), atol=0.0001))