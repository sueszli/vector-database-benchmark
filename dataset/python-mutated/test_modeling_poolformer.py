...
class PoolFormerModelTester:
    ...
    def __init__(self, parent, batch_size=13, image_size=64, num_channels=3, num_encoder_blocks=4, depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1], hidden_sizes=[16, 32, 64, 128], downsampling_rates=[1, 4, 8, 16], is_training=False, use_labels=True, hidden_act='gelu', hidden_dropout_prob=0.1, initializer_range=0.02, num_labels=3, scope=None):
        if False:
            return 10
        self.parent = parent
    ...
    def prepare_config_and_inputs(self):
        ...
        config = PoolFormerConfig(image_size=self.image_size, num_channels=self.num_channels, num_encoder_blocks=self.num_encoder_blocks, depths=self.depths, hidden_sizes=self.hidden_sizes, hidden_act=self.hidden_act, hidden_dropout_prob=self.hidden_dropout_prob, initializer_range=self.initializer_range)
...
class PoolFormerModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': PoolFormerModel, 'image-classification': PoolFormerForImageClassification} if is_torch_available() else {}
