...
from prowler.providers.aws.services.wellarchitected.wellarchitected_service import WellArchitected
...
def mock_make_api_call(self, operation_name, kwarg):
    ...
    if operation_name == 'ListWorkloads':
        return {'WorkloadSummaries': [{'WorkloadId': workload_id, 'WorkloadArn': f'arn:aws:wellarchitected:{AWS_REGION}:{AWS_ACCOUNT_NUMBER}:workload/{workload_id}', 'WorkloadName': 'test', 'Owner': AWS_ACCOUNT_NUMBER, 'UpdatedAt': '2023-06-07T15:40:24+02:00', 'Lenses': ['wellarchitected', 'serverless', 'softwareasaservice'], 'RiskCounts': {'UNANSWERED': 56, 'NOT_APPLICABLE': 4, 'HIGH': 10}, 'ImprovementStatus': 'NOT_APPLICABLE'}]}
...
class Test_WellArchitected_Service:
    ...
    def test__list_workloads__(self):
        ...
        assert wellarchitected.workloads[0].arn == f'arn:aws:wellarchitected:{AWS_REGION}:{AWS_ACCOUNT_NUMBER}:workload/{workload_id}'
        assert wellarchitected.workloads[0].name == 'test'
        assert wellarchitected.workloads[0].region == AWS_REGION
        assert wellarchitected.workloads[0].tags == [{'Key': 'test', 'Value': 'test'}]
        assert wellarchitected.workloads[0].lenses == ['wellarchitected', 'serverless', 'softwareasaservice']
        assert wellarchitected.workloads[0].improvement_status == 'NOT_APPLICABLE'
        assert wellarchitected.workloads[0].risks == {'UNANSWERED': 56, 'NOT_APPLICABLE': 4, 'HIGH': 10}