...
class _CoAPOpt(Packet):
    fields_desc = [BitField('delta', 0, 4), BitField('len', 0, 4), StrLenField('delta_ext', '', length_from=_get_delta_ext_size), StrLenField('len_ext', '', length_from=_get_len_ext_size), StrLenField('opt_val', '', length_from=_get_opt_val_size)]
...
class CoAP(Packet):
    ...
    name = 'CoAP'
    fields_desc = [BitField('ver', 1, 2), BitEnumField('type', 0, 2, {0: 'CON', 1: 'NON', 2: 'ACK', 3: 'RST'}), BitFieldLenField('tkl', None, 4, length_of='token'), ByteEnumField('code', 0, coap_codes), ShortField('msg_id', 0), StrLenField('token', '', length_from=lambda pkt: pkt.tkl), _CoAPOptsField('options', []), _CoAPPaymark('paymark', b'')]
