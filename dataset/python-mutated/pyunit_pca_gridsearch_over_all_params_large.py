...
class Test_PCA_grid_search:
    ...
    test_name = 'pyunit_PCA_gridsearch_over_all_params_large.py'
    sandbox_dir = ''
    x_indices = []
    training1_data = []
    test_failed = 0
    ...
    def test_PCA_grid_search_over_params(self):
        ...
        try:
            ...
            grid_model = H2OGridSearch(H2OPCA(pca_method=self.pca_method), hyper_params=self.final_hyper_params)
            ...
            if self.correct_model_number - self.possible_number_models > 0.9:
                self.test_failed += 1
                print('test_PCA_grid_search_over_params for PCA failed: number of models built by gridsearch: {0} does not equal to all possible combinations of hyper-parameters: {1}'.format(self.correct_model_number, self.possible_number_models))
            else:
                params_dict = dict()
                ...
                for each_model in grid_model:
                    ...
                    grid_model_metrics = sum(each_model._model_json['output']['model_summary'].cell_values[0][1:params_list['k']])
                    manual_model_metrics = sum(manual_model._model_json['output']['model_summary'].cell_values[0][1:params_list['k']])
                    if not (type(grid_model_metrics) == str or type(manual_model_metrics) == str):
                        if abs(grid_model_metrics) > 0 and abs(grid_model_metrics - manual_model_metrics) / grid_model_metrics > self.allowed_diff:
                            print('test_PCA_grid_search_over_params for PCA warning: grid search model mdetric ({0}) and manually built H2O model metric ({1}) differ too much!'.format(grid_model_metrics, manual_model_metrics))
                total_run_time_limits = max(total_run_time_limits, true_run_time_limits) * (1 + self.extra_time_fraction)
                if not manual_run_runtime <= total_run_time_limits:
                    self.test_failed += 1
                    print('test_PCA_grid_search_over_params for PCA failed: time taken to manually build models is {0}.  Maximum allowed time is {1}'.format(manual_run_runtime, total_run_time_limits))
                else:
