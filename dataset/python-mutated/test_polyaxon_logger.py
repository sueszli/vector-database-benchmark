...
import pytest
import torch
from ignite.contrib.handlers.polyaxon_logger import global_step_from_engine, OptimizerParamsHandler, OutputHandler, PolyaxonLogger
...
def test_output_handler_metric_names():
    ...
    wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
    assert mock_logger.log_metrics.call_count == 1
    mock_logger.log_metrics.assert_has_calls([call(step=5, **{'tag/a/0': 0.0, 'tag/a/1': 1.0, 'tag/a/2': 2.0, 'tag/a/3': 3.0})], any_order=True)
...
def test_output_handler_with_global_step_from_engine():
    ...
    wrapper(mock_engine, mock_logger, Events.EPOCH_STARTED)
    assert mock_logger.log_metrics.call_count == 1
    mock_logger.log_metrics.assert_has_calls([call(step=mock_another_engine.state.epoch, **{'tag/loss': mock_engine.state.output})])
...
def test_no_polyaxon_client(no_site_packages):
    if False:
        i = 10
        return i + 15
    with pytest.raises(ModuleNotFoundError, match='This contrib module requires polyaxon'):
        PolyaxonLogger()