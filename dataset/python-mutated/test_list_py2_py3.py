...
def test_fluent_interface():
    if False:
        i = 10
        return i + 15
    provider = providers.List().add_args(1, 2)
    assert provider() == [1, 2]
...
def test_set_args():
    if False:
        while True:
            i = 10
    provider = providers.List().add_args(1, 2).set_args(3, 4)
...
def test_clear_args():
    if False:
        i = 10
        return i + 15
    provider = providers.List().add_args(1, 2).clear_args()
...
def test_repr():
    if False:
        while True:
            i = 10
    provider = providers.List(1, 2)
    assert repr(provider) == '<dependency_injector.providers.List({0}) at {1}>'.format(repr(list(provider.args)), hex(id(provider)))