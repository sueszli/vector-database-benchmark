...
_TAB_STR = ' ' * 4
_FIELD_PATTERN = re.compile('{((?P<fname>[^}:]*)((?P<cname_sep>:(?P<cname>[^}\\(]*))(\\((?P<argstr>[^}]*)\\))?)?)}')
...
class CompiledRouter:
    ...
    """
    __slots__ = ('_ast', '_converter_map', '_converters', '_find', '_finder_src', '_options', '_patterns', '_return_values', '_roots', '_compile_lock')
...
_NO_CHILDREN_ERR = 'Cannot add route with template "{0}". Field name "{1}" uses the converter "{2}" that will consume all the path, making it impossible to match this route.'
