...
SKIP_ANONYMOUS_IN_TF1_REASON = "In v1 graph mode, each self.evaluate call will execute the handle creation op (e.g. AnonymousHashTable) which will create a new table resource unrelated to other self.evaluate calls, so we can't test anonymous resources with self.evaluate ."

@parameterized.named_parameters(((f'_{is_anonymous}', is_anonymous) for is_anonymous in [False, True]))
