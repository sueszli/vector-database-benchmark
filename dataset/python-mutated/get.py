...
async def Username(_id, bearer_token, guest_token):
    ...
    _headers = {'authorization': bearer_token, 'x-guest-token': guest_token}
...
async def User(username, config, conn, user_id=False):
    ...
    _url = 'https://api.twitter.com/graphql/jMaTS-_Ea8vh9rpKggJbCQ/UserByScreenName?variables={}'.format(dict_to_url(_dct))
    _headers = {'authorization': config.Bearer_token, 'x-guest-token': config.Guest_token}
    try:
