...
KNOWN_INVALID_TYPES = {"$['properties']['ingress']['properties']['web']['properties']['precedingPaths']", "$['properties']['ingress']['properties']['web']['properties']['succeedingPaths']", "$['properties']['statsd']['properties']['extraMappings']", "$['properties']['statsd']['properties']['overrideMappings']"}
VENDORED_PATHS = {"$['definitions']['io.k8s"}
...
if fixed_types:
    ...
    print(f'Some types that were known to be invalid have been fixed. Can you update the variable `known_invalid_types` in file {current_file!r}? You just need to delete the following items:')
    print('\n'.join(fixed_types))
