...
from ray._private.runtime_env.packaging import delete_package, download_and_unpack_package, get_local_dir_from_uri, is_jar_uri
...
class JavaJarsPlugin(RuntimeEnvPlugin):
    ...
    async def _download_jars(self, uri: str, logger: Optional[logging.Logger]=default_logger):
        """Download a jar URI."""
        try:
            jar_file = await download_and_unpack_package(uri, self._resources_dir, self._gcs_aio_client, logger=logger)
        except Exception as e:
            raise RuntimeEnvSetupError('Failed to download jar file: {}'.format(e)) from e
