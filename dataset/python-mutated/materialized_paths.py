...
class Node(Base):
    ...
    descendants = relationship('Node', viewonly=True, order_by=path, primaryjoin=remote(foreign(path)).like(path.concat('.%')))
    secondary = select(id.label('id'), func.unnest(cast(func.string_to_array(func.regexp_replace(path, '\\.?\\d+$', ''), '.'), ARRAY(Integer))).label('ancestor_id')).alias()
    ancestors = relationship('Node', viewonly=True, secondary=secondary, primaryjoin=id == secondary.c.id, secondaryjoin=secondary.c.ancestor_id == id, order_by=path)
...
if __name__ == '__main__':
    ...
    print('-' * 80)
    print('create a tree')
    session.add_all([Node(id=1, path='1'), Node(id=2, path='1.2'), Node(id=3, path='1.3'), Node(id=4, path='1.3.4'), Node(id=5, path='1.3.5'), Node(id=6, path='1.3.6'), Node(id=7, path='1.7'), Node(id=8, path='1.7.8'), Node(id=9, path='1.7.9'), Node(id=10, path='1.7.9.10'), Node(id=11, path='1.7.11')])
    session.flush()
