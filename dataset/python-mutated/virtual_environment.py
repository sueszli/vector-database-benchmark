...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run inside the Pyre virtual environment')
    commands = parser.add_subparsers()
    setup = commands.add_parser('setup')
    setup.set_defaults(command=_setup)
    enter = commands.add_parser('enter', help='Enter the virtual environment.Run with `eval $(./scripts/virtual_environment.py enter)`')
    enter.set_defaults(command=_enter)
    leave = commands.add_parser('leave', help='Leave the virtual environment.Run with `eval $(./scripts/virtual_environment.py leave)`')
