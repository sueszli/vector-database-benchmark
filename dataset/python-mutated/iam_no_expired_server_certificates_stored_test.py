...
class Test_iam_no_expired_server_certificates_stored_test:
    ...
    def test_no_certificates(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.iam.iam_no_expired_server_certificates_stored.iam_no_expired_server_certificates_stored.iam_client', new=IAM(audit_info)):
                from prowler.providers.aws.services.iam.iam_no_expired_server_certificates_stored.iam_no_expired_server_certificates_stored import iam_no_expired_server_certificates_stored
    ...
    def test_expired_certificate(self):
        ...
        cert = iam_client.upload_server_certificate(ServerCertificateName='certname', CertificateBody='certbody', PrivateKey='privatekey')['ServerCertificateMetadata']
        from prowler.providers.aws.services.iam.iam_service import IAM
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.iam.iam_no_expired_server_certificates_stored.iam_no_expired_server_certificates_stored.iam_client', new=IAM(audit_info)):
                ...
                assert search('IAM Certificate certname has expired', result[0].status_extended)
