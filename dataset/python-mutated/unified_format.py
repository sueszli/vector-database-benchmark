...
from test import AWS_CREDS, AZURE_CREDS, CA_PEM, CLIENT_PEM, GCP_CREDS, KMIP_CREDS, LOCAL_MASTER_KEY, IntegrationTest, client_context, unittest
from test.utils import CMAPListener, camel_to_snake, camel_to_snake_args, get_pool, parse_collection_options, parse_spec_options, prepare_spec_arguments, rs_or_single_client, single_client, snake_to_camel, wait_until
...
from pymongo.errors import BulkWriteError, ConfigurationError, ConnectionFailure, EncryptionError, InvalidOperation, NotPrimaryError, PyMongoError
...
PLACEHOLDER_MAP = {}
for (provider_name, provider_data) in [('local', {'key': LOCAL_MASTER_KEY}), ('aws', AWS_CREDS), ('azure', AZURE_CREDS), ('gcp', GCP_CREDS), ('kmip', KMIP_CREDS)]:
...
long_types = (Int64,)
unicode_type = str
BSON_TYPE_ALIAS_MAP = {'double': (float,), 'string': (str,), 'object': (abc.Mapping,), 'array': (abc.MutableSequence,), 'binData': binary_types, 'undefined': (type(None),), 'objectId': (ObjectId,), 'bool': (bool,), 'date': (datetime.datetime,), 'null': (type(None),), 'regex': (Regex, RE_TYPE), 'dbPointer': (DBRef,), 'javascript': (unicode_type, Code), 'symbol': (unicode_type,), 'javascriptWithScope': (unicode_type, Code), 'int': (int,), 'long': (Int64,), 'decimal': (Decimal128,), 'maxKey': (MaxKey,), 'minKey': (MinKey,)}
...
_ALL_MIXIN_CLASSES = [UnifiedSpecTestMixinV1]
_SCHEMA_VERSION_MAJOR_TO_MIXIN_CLASS = {KLASS.SCHEMA_VERSION[0]: KLASS for KLASS in _ALL_MIXIN_CLASSES}
