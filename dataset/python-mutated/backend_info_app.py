...
hide_supported_by_all_backends = st.sidebar.selectbox('Operation compatibility', ['Show all', 'Show supported by all backends', 'Hide supported by all backends'], 0)
...
supported_backend_count = sum((getattr(table_expr, backend_name).ifelse(1, 0) for backend_name in current_backend_names))
if hide_supported_by_all_backends == 'Show supported by all backends':
    table_expr = table_expr.filter(supported_backend_count == len(current_backend_names))
...
if all_visible_ops_count:
    coverage = df.sum().sort_values(ascending=False).map(lambda n: f'{n} ({round(100 * n / all_visible_ops_count)}%)').to_frame(name='API Coverage').T
    table = pd.concat([coverage, df.replace({True: '✔', False: '🚫'})]).loc[:, slice(None) if sort_by_coverage else sorted(df.columns)]
    st.dataframe(table)
else:
    st.write('No data')
with st.expander('SQL queries'):
    for sql_query in sql_queries:
        pretty_sql_query = sqlglot.transpile(sql_query, read='duckdb', write='duckdb', pretty=True)[0]
