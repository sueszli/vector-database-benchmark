...
class SonosHouseholdCoordinator:
    ...
    async def _async_setup(self) -> None:
        ...
        self.async_poll = Debouncer[Coroutine[Any, Any, None]](self.hass, _LOGGER, cooldown=3, immediate=False, function=self._async_poll).async_call
    ...
    async def _async_poll(self) -> None:
        ...
        for (uid, speaker) in discovered.items():
            ...
            try:
                ...
                _LOGGER.error('Could not refresh %s: %s', self.class_type, err)
            else:
