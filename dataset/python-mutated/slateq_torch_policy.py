...
from ray.rllib.utils.torch_utils import apply_grad_clipping, concat_multi_gpu_td_errors, convert_to_torch_tensor, huber_loss
...
SlateQTorchPolicy = build_policy_class(name='SlateQTorchPolicy', framework='torch', get_default_config=lambda : rllib_slate_q.slate_q.slateq.SlateQConfig(), before_init=setup_early, after_init=setup_late_mixins, loss_fn=build_slateq_losses, stats_fn=build_slateq_stats, make_model_and_action_dist=build_slateq_model_and_distribution, optimizer_fn=optimizer_fn, action_distribution_fn=action_distribution_fn, extra_grad_process_fn=apply_grad_clipping, extra_learn_fetches_fn=concat_multi_gpu_td_errors, mixins=[TargetNetworkMixin])