...
class XmlPropertyListParser(object):
    ...
    DATETIME_PATTERN = re.compile('(?P<year>\\d\\d\\d\\d)(?:-(?P<month>\\d\\d)(?:-(?P<day>\\d\\d)(?:T(?P<hour>\\d\\d)(?::(?P<minute>\\d\\d)(?::(?P<second>\\d\\d))?)?)?)?)?Z$')
    ...
    START_CALLBACKS = {'plist': _start_plist, 'array': _start_array, 'dict': _start_dict, 'true': _start_true, 'false': _start_false}
    END_CALLBACKS = {'array': _end_array, 'dict': _end_dict}
    PARSE_CALLBACKS = {'key': _parse_key, 'string': _parse_string, 'data': _parse_data, 'date': _parse_date, 'real': _parse_real, 'integer': _parse_integer}
