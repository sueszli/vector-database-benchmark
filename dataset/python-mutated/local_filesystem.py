...
from airflow.exceptions import AirflowException, AirflowFileParseException, ConnectionNotUnique, FileSyntaxError, RemovedInAirflow3Warning
...
def _parse_json_file(file_path: str) -> tuple[dict[str, Any], list[FileSyntaxError]]:
    ...
    if not isinstance(secrets, dict):
        return ({}, [FileSyntaxError(line_no=1, message='The file should contain the object.')])
    return (secrets, [])
FILE_PARSERS = {'env': _parse_env_file, 'json': _parse_json_file, 'yaml': _parse_yaml_file, 'yml': _parse_yaml_file}
