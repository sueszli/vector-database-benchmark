...
chr1_info = (('', None, 1), ('AC30001', None, 1), ('', colors.blue, 2), ('', colors.blue, 1.5), ('', colors.red, 2), ('AC12345', colors.blue, 1), ('', colors.blue, 1), ('', colors.blue, 1), ('', None, 1))
chr2_info = (('', None, 2), ('AC23456', None, 2), ('', colors.blue, 1), ('', colors.blue, 1), ('', colors.red, 1), ('AC00002', colors.blue, 1.5), ('', colors.blue, 1.5), ('', colors.blue, 2), ('', None, 1.5), ('', colors.red, 2), ('', colors.red, 1.5), ('AB0001', None, 1), ('', colors.red, 1))
chr3_info = (('', colors.green, 2), ('', colors.green, 1), ('AD00002', colors.blue, 1), ('', colors.blue, 1), ('', colors.red, 1))
chr4_info = (('', None, 1.5), ('', None, 1), ('AD11111', colors.blue, 2), ('', colors.blue, 1), ('', colors.red, 1.5), ('', colors.blue, 1), ('', colors.blue, 1), ('AC22222', colors.blue, 1))
...
class OrganismGraphicTest(unittest.TestCase):
    ...
    def test_widget(self):
        ...
        self.assertIn(expected_string, properties, f'Unexpected results from dumpProperties: \n {properties}')
        properties = test_widget.getProperties()
        self.assertEqual(properties['label_size'], 6, f'Unexpected results from getProperties: {properties}')
        test_widget.setProperties({'start_x_position': 12})
        self.assertEqual(test_widget.start_x_position, 12, f"setProperties doesn't seem to work right: {test_widget.start_x_position}")
...
class OrganismSubAnnotationsTest(unittest.TestCase):
    ...
    def check_simple_tRNA(self, filename, use_seqfeatures=False):
        ...
        entries = [('Chr I', 'NC_003070', 30432563, f1, colors.red), ('Chr II', 'NC_003071', 19705359, f2, colors.green), ('Chr III', 'NC_003074', 23470805, f3, colors.blue), ('Chr IV', 'NC_003075', 18585042, f4, colors.orange), ('Chr V', 'NC_003076', 26992728, f5, colors.purple)]
        ...
        for (name, acc, length, features, color) in entries:
            if False:
                ...
                print(name)
                print([(int(f.location.start), int(f.location.end), f.strand, f.qualifiers['locus_tag'][0][3:]) for f in features])
                for (i, f) in enumerate(features):
                    f.qualifiers['color'] = [str(i % 16)]
            elif use_seqfeatures:
                features = [SeqFeature(SimpleLocation(start, end, strand), qualifiers={'name': [label], 'color': [color]}) for (start, end, strand, label) in features]
            else:
                features = [(start, end, strand, label, color) for (start, end, strand, label) in features]
            cytobands = []
...
class ChromosomeCountTest(unittest.TestCase):
    ...
    def test_get_segment_info(self):
        ...
        self.assertEqual(seg_info[test_count_num][0], test_count_value, 'Did not set and retrieve counts correctly.')
        self.assertEqual(seg_info[test_label_num][1], test_label_value, 'Did not set and retrieve label correctly.')
