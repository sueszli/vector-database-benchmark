...
class Test_elb_ssl_listeners:
    ...
    def test_elb_no_balancers(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()), mock.patch('prowler.providers.aws.services.elb.elb_ssl_listeners.elb_ssl_listeners.elb_client', new=ELB(self.set_mocked_audit_info())):
            from prowler.providers.aws.services.elb.elb_ssl_listeners.elb_ssl_listeners import elb_ssl_listeners
    ...
    def test_elb_with_HTTPS_listener(self):
        ...
        ec2 = resource('ec2', region_name=AWS_REGION)
        security_group = ec2.create_security_group(GroupName='sg01', Description='Test security group sg01')
        elb.create_load_balancer(LoadBalancerName='my-lb', Listeners=[{'Protocol': 'https', 'LoadBalancerPort': 443, 'InstancePort': 9000}], AvailabilityZones=[f'{AWS_REGION}a'], Scheme='internal', SecurityGroups=[security_group.id])
        from prowler.providers.aws.services.elb.elb_service import ELB
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()), mock.patch('prowler.providers.aws.services.elb.elb_ssl_listeners.elb_ssl_listeners.elb_client', new=ELB(self.set_mocked_audit_info())):
            from prowler.providers.aws.services.elb.elb_ssl_listeners.elb_ssl_listeners import elb_ssl_listeners
            check = elb_ssl_listeners()
