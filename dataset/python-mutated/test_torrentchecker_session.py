...
from tribler.core.components.torrent_checker.torrent_checker.torrentchecker_session import FakeBep33DHTSession, FakeDHTSession, HttpTrackerSession, UdpSocketManager, UdpTrackerSession
...
async def test_httpsession_unicode_err():
    session = HttpTrackerSession('retracker.local', ('retracker.local', 80), '/announce?comment=%26%23%3B%28%2C%29%5B%5D%E3%5B%D4%E8%EB%FC%EC%EE%E2', 5, None)
...
async def test_httpsession_timeout():
    ...
    session = HttpTrackerSession('localhost', ('localhost', port), '/announce', 0.1, None)
    with pytest.raises(ValueError):
        await session.connect_to_tracker()
...
async def test_udpsession_timeout(fake_udp_socket_manager):
    ...
    (transport, _) = await get_event_loop().create_datagram_endpoint(lambda : DatagramProtocol(), local_addr=('127.0.0.1', 0), family=socket.AF_INET)
    (_, port) = transport.get_extra_info('sockname')
    session = UdpTrackerSession('localhost', ('127.0.0.1', port), '/announce', 0.1, None, fake_udp_socket_manager)
    with pytest.raises(ValueError):
        await session.connect_to_tracker()
    transport.close()
...
async def test_failed_unicode():
    session = HttpTrackerSession('localhost', ('localhost', 8475), '/announce', 5, None)
    with pytest.raises(ValueError):
        session._process_scrape_response(bencode({'failure reason': 'é'}))
    await session.cleanup()
