...
class IDownloadCouponScraper(BaseScraper):
    ...
    async def get_links(self):
        ...
        """
        self.current_page += 1
        course_links = await self.get_course_links(f'{self.DOMAIN}/page/{self.current_page}/')
        logger.info(f'Page: {self.current_page} of {self.last_page} scraped from {self.scraper_name}')
    ...
    async def get_course_links(self, url: str) -> List:
        ...
        if text is not None:
            ...
            self.last_page = int(soup.find('ul', class_='page-numbers').find_all('a', class_='page-numbers')[-2].text.replace(',', ''))
            return course_links
