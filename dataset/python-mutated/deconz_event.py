...
from pydeconz.models.sensor.ancillary_control import AncillaryControl, AncillaryControlAction
...
from homeassistant.const import CONF_DEVICE_ID, CONF_EVENT, CONF_ID, CONF_UNIQUE_ID, CONF_XY
...
SUPPORTED_DECONZ_ALARM_EVENTS = {AncillaryControlAction.EMERGENCY, AncillaryControlAction.FIRE, AncillaryControlAction.INVALID_CODE, AncillaryControlAction.PANIC}
SUPPORTED_DECONZ_PRESENCE_EVENTS = {PresenceStatePresenceEvent.ENTER, PresenceStatePresenceEvent.LEAVE, PresenceStatePresenceEvent.ENTER_LEFT, PresenceStatePresenceEvent.RIGHT_LEAVE, PresenceStatePresenceEvent.ENTER_RIGHT, PresenceStatePresenceEvent.LEFT_LEAVE, PresenceStatePresenceEvent.APPROACHING, PresenceStatePresenceEvent.ABSENTING}
RELATIVE_ROTARY_DECONZ_TO_EVENT = {RelativeRotaryEvent.NEW: 'new', RelativeRotaryEvent.REPEAT: 'repeat'}
...
async def async_setup_events(gateway: DeconzGateway) -> None:
    ...
    gateway.register_platform_add_device_callback(async_add_sensor, gateway.api.sensors.switch)
    gateway.register_platform_add_device_callback(async_add_sensor, gateway.api.sensors.ancillary_control)
    gateway.register_platform_add_device_callback(async_add_sensor, gateway.api.sensors.presence)
    gateway.register_platform_add_device_callback(async_add_sensor, gateway.api.sensors.relative_rotary)
...
class DeconzEventBase(DeconzBase):
    ...
    async def async_update_device_registry(self) -> None:
        ...
        entry = device_registry.async_get_or_create(config_entry_id=self.gateway.config_entry.entry_id, **self.device_info)
