...
class SEWDModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'audio-classification': SEWDForSequenceClassification, 'automatic-speech-recognition': SEWDForCTC, 'feature-extraction': SEWDModel} if is_torch_available() else {}
    test_pruning = False
...
class SEWDModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_pretrained_batched(self):
        ...
        expected_outputs_first = torch.tensor([[[-0.1619, 0.6995, 0.4062, -0.1014], [-0.1364, 0.596, 0.0952, -0.0873], [-0.1572, 0.5718, 0.4228, -0.0864], [-0.1325, 0.6823, 0.1387, -0.0871]], [[-0.1296, 0.4008, 0.4952, -0.145], [-0.1152, 0.3693, 0.3037, -0.129], [-0.1194, 0.6074, 0.3531, -0.1466], [-0.1113, 0.3135, 0.2224, -0.1338]]], device=torch_device)
        expected_outputs_last = torch.tensor([[[-0.1577, 0.5108, 0.8553, 0.255], [-0.153, 0.358, 0.6143, 0.2672], [-0.1535, 0.4954, 0.8503, 0.1387], [-0.1572, 0.3363, 0.6217, 0.149]], [[-0.1338, 0.5459, 0.9607, -0.1133], [-0.1502, 0.3738, 0.7313, -0.0986], [-0.0953, 0.4708, 1.0821, -0.0944], [-0.1474, 0.3598, 0.7248, -0.0748]]], device=torch_device)
        expected_output_sum = 54201.0469
        self.assertTrue(torch.allclose(outputs[:, :4, :4], expected_outputs_first, atol=0.001))
        self.assertTrue(torch.allclose(outputs[:, -4:, -4:], expected_outputs_last, atol=0.001))
    ...
    def test_inference_ctc_batched(self):
        ...
        EXPECTED_TRANSCRIPTIONS = ['a man said to the universe sir i exist', "swet covered breon's body trickling into the titlowing closs that was the only garmened he war"]
