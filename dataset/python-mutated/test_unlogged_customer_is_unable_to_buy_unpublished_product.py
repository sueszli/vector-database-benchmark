...
def prepare_unpublished_product(e2e_staff_api_client):
    if False:
        return 10
    ...
    raw_create_product_channel_listing(e2e_staff_api_client, product_id, channel_id, is_published=False, is_available_for_purchase=True)
...
@pytest.mark.e2e
def test_unlogged_customer_is_unable_to_buy_unpublished_product_core_0109(e2e_staff_api_client, e2e_not_logged_api_client, permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes, permission_manage_shipping):
    if False:
        for i in range(10):
            print('nop')
    ...
    (product_variant_id, channel_slug) = prepare_unpublished_product(e2e_staff_api_client)
    lines = [{'variantId': product_variant_id, 'quantity': 1}]
    checkout_data = raw_checkout_create(e2e_not_logged_api_client, lines, channel_slug, email='testEmail@example.com', set_default_billing_address=True)
