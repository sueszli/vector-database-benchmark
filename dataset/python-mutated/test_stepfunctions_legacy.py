...
STATE_MACHINE_MAP = {'Comment': 'Hello Map State', 'StartAt': 'ExampleMapState', 'States': {'ExampleMapState': {'Type': 'Map', 'Iterator': {'StartAt': 'CallLambda', 'States': {'CallLambda': {'Type': 'Task', 'Resource': '__tbd__', 'End': True}}}, 'End': True}}}
...
STATE_MACHINE_INTRINSIC_FUNCS = {'StartAt': 'state0', 'States': {'state0': {'Type': 'Pass', 'Result': {'v1': 1, 'v2': 'v2'}, 'Next': 'state1'}, 'state1': {'Type': 'Pass', 'Parameters': {'lambda_params': {'FunctionName': '__tbd__', 'Payload': {'values.$': 'States.Array($.v1, $.v2)'}}}, 'Next': 'state2'}, 'state2': {'Type': 'Task', 'Resource': 'arn:aws:states:::lambda:invoke', 'Parameters': {'FunctionName.$': '$.lambda_params.FunctionName', 'Payload.$': 'States.StringToJson(States.JsonToString($.lambda_params.Payload))'}, 'Next': 'state3'}, 'state3': {'Type': 'Task', 'Resource': '__tbd__', 'ResultSelector': {'payload.$': '$.Payload'}, 'ResultPath': '$.result_value', 'End': True}}}
...
pytestmark = pytest.mark.skipif(condition=is_not_legacy_provider, reason='Test suite only for legacy provider.')
