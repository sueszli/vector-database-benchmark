...
status_map: dict[str, bool] = {':heavy_check_mark:': True, ':x:': False}
...
if __name__ == '__main__':
    ...
    for test in res:
        ...
        if test.contents and test.contents[0].name == 'skipped':
            ...
            test_results.append(TestResult(test_id=test['classname'] + '::' + test['name'], file=test['file'], line=test['line'], name=test['name'], classname=test['classname'], result=not test.contents))
    ...
    with Path(__file__).resolve().with_name('quarantine_issue_header.md').open() as f:
        header = jinja2.Template(f.read(), autoescape=True, undefined=StrictUndefined).render(DATE_UTC_NOW=datetime.utcnow())
    quarantined_issue.edit(title=None, body=f'{header}\n\n{table}', state='open' if test_results else 'closed')