...
from ..descriptions import ADDED_IN_311, ADDED_IN_314, DEPRECATED_IN_3X_INPUT, PREVIEW_FEATURE
...
GLOBAL_ID_FILTERS = {models.AutoField: {'filter_class': GlobalIDFilter}, models.OneToOneField: {'filter_class': GlobalIDFilter}, models.ForeignKey: {'filter_class': GlobalIDFilter}, models.ManyToManyField: {'filter_class': GlobalIDMultipleChoiceFilter}, models.ManyToOneRel: {'filter_class': GlobalIDMultipleChoiceFilter}, models.ManyToManyRel: {'filter_class': GlobalIDMultipleChoiceFilter}}

class GraphQLFilterSetMixin(BaseFilterSet):
    FILTER_DEFAULTS = dict(itertools.chain(FILTER_FOR_DBFIELD_DEFAULTS.items(), GLOBAL_ID_FILTERS.items()))
...
class ChannelFilterInputObjectType(FilterInputObjectType):
    channel = Argument(String, description=f'Specifies the channel by which the data should be filtered. {DEPRECATED_IN_3X_INPUT} Use root-level channel argument instead.')
...
class StringFilterInput(graphene.InputObjectType):
    ...
    one_of = NonNullList(graphene.String, description=FilterInputDescriptions.ONE_OF, required=False)

    class Meta:
        description = 'Define the filtering options for string fields.' + ADDED_IN_311 + PREVIEW_FEATURE

class IntFilterInput(graphene.InputObjectType):
    eq = graphene.Int(description=FilterInputDescriptions.EQ, required=False)
    one_of = NonNullList(graphene.Int, description=FilterInputDescriptions.ONE_OF, required=False)
    range = IntRangeInput(description=FilterInputDescriptions.RANGE, required=False)

    class Meta:
        description = 'Define the filtering options for integer fields.' + ADDED_IN_311 + PREVIEW_FEATURE

class DecimalFilterInput(graphene.InputObjectType):
    eq = Decimal(description=FilterInputDescriptions.EQ, required=False)
    one_of = NonNullList(Decimal, description=FilterInputDescriptions.ONE_OF, required=False)
    range = DecimalRangeInput(description=FilterInputDescriptions.RANGE, required=False)

    class Meta:
        description = 'Define the filtering options for decimal fields.' + ADDED_IN_314 + PREVIEW_FEATURE

class DateFilterInput(graphene.InputObjectType):
    eq = Date(description=FilterInputDescriptions.EQ, required=False)
    one_of = NonNullList(Date, description=FilterInputDescriptions.ONE_OF, required=False)
    range = DateRangeInput(description=FilterInputDescriptions.RANGE, required=False)

    class Meta:
        description = 'Define the filtering options for date fields.' + ADDED_IN_311 + PREVIEW_FEATURE
...
class DateTimeFilterInput(graphene.InputObjectType):
    ...
    one_of = NonNullList(graphene.DateTime, description=FilterInputDescriptions.ONE_OF, required=False)
    range = DateTimeRangeInput(description=FilterInputDescriptions.RANGE, required=False)

    class Meta:
        description = 'Define the filtering options for date time fields.' + ADDED_IN_311 + PREVIEW_FEATURE

class GlobalIDFilterInput(graphene.InputObjectType):
    eq = graphene.ID(description=FilterInputDescriptions.EQ, required=False)
    one_of = NonNullList(graphene.ID, description=FilterInputDescriptions.ONE_OF, required=False)

    class Meta:
        description = 'Define the filtering options for foreign key fields.' + ADDED_IN_314 + PREVIEW_FEATURE