...
def word_sequence_missing_words(tree):
    ...
    for (idx, label) in enumerate(indices):
        if label != idx + 1:
            return True
    return False
WORD_TO_PHRASE = {'art': 'advp', 'adj': 'adjp', 'adv': 'advp', 'conj': 'cp', 'intj': 'fcl', 'n': 'np', 'num': 'np', 'pron': 'np', 'prop': 'np', 'prp': 'pp', 'v': 'vp'}
...
REMAP_LABELS = {'adj': 'adjp', 'adv': 'advp', 'intj': 'fcl', 'n': 'np', 'num': 'np', 'prp': 'pp'}
