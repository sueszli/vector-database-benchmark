...
class TestCallbackDataFilter:
    ...
    @pytest.mark.parametrize('query,rule,result', [['test', F.foo == 'test', False], ['test:spam:42', F.foo == 'test', False], ['test:test:42', F.foo == 'test', {'callback_data': MyCallback(foo='test', bar=42)}], ['test:test:42', None, {'callback_data': MyCallback(foo='test', bar=42)}], ['test:test:777', None, {'callback_data': MyCallback(foo='test', bar=777)}], ['spam:test:777', None, False], ['test:test:', F.foo == 'test', False], ['test:test:', None, False]])
    async def test_call(self, query, rule, result):
        callback_query = CallbackQuery(id='1', from_user=User(id=42, is_bot=False, first_name='test'), data=query, chat_instance='test')
