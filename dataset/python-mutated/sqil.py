...
from ding.framework.middleware import OffPolicyLearner, StepCollector, interaction_evaluator, eps_greedy_handler, CkptSaver, eps_greedy_masker, sqil_data_pusher
...
def main():
    ...
    with task.start(async_mode=False, ctx=OnlineRLContext()):
        collector_env = BaseEnvManagerV2(env_fn=[lambda : DingEnvWrapper(gym.make('CartPole-v0')) for _ in range(cfg.env.collector_env_num)], cfg=cfg.env.manager)
        expert_collector_env = BaseEnvManagerV2(env_fn=[lambda : DingEnvWrapper(gym.make('CartPole-v0')) for _ in range(cfg.env.collector_env_num)], cfg=cfg.env.manager)
        evaluator_env = BaseEnvManagerV2(env_fn=[lambda : DingEnvWrapper(gym.make('CartPole-v0')) for _ in range(cfg.env.evaluator_env_num)], cfg=cfg.env.manager)
        ...
        task.use(StepCollector(cfg, expert_policy.collect_mode, expert_collector_env))
