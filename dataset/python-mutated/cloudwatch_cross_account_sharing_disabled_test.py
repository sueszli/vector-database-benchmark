...
class Test_cloudwatch_cross_account_sharing_disabled:
    ...
    def test_cloudwatch_without_cross_account_role(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.cloudwatch.cloudwatch_cross_account_sharing_disabled.cloudwatch_cross_account_sharing_disabled.iam_client', new=IAM(current_audit_info)):
            from prowler.providers.aws.services.cloudwatch.cloudwatch_cross_account_sharing_disabled.cloudwatch_cross_account_sharing_disabled import cloudwatch_cross_account_sharing_disabled
            check = cloudwatch_cross_account_sharing_disabled()
            result = check.execute()
            assert len(result) == 1
            assert result[0].status == 'PASS'
            assert result[0].status_extended == "CloudWatch doesn't allow cross-account sharing."
    ...
    def test_cloudwatch_log_group_with_cross_account_role(self):
        ...
        iam_client.create_role(RoleName='CloudWatch-CrossAccountSharingRole', AssumeRolePolicyDocument='{}')
        from prowler.providers.aws.services.iam.iam_service import IAM
        current_audit_info = self.set_mocked_audit_info()
        from prowler.providers.common.models import Audit_Metadata
        current_audit_info.audit_metadata = Audit_Metadata(services_scanned=0, expected_checks=['cloudwatch_log_group_no_secrets_in_logs'], completed_checks=0, audit_progress=0)
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.cloudwatch.cloudwatch_cross_account_sharing_disabled.cloudwatch_cross_account_sharing_disabled.iam_client', new=IAM(current_audit_info)):
            ...
            assert result[0].status_extended == 'CloudWatch has allowed cross-account sharing.'
