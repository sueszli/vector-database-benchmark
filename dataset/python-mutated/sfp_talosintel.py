...
class sfp_talosintel(SpiderFootPlugin):
    meta = {'name': 'Talos Intelligence', 'summary': 'Check if a netblock or IP address is malicious according to TalosIntelligence.', 'flags': [], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'website': 'https://talosintelligence.com/', 'model': 'FREE_NOAUTH_UNLIMITED', 'references': ['https://talosintelligence.com/vulnerability_info', 'https://talosintelligence.com/reputation'], 'favIcon': 'https://talosintelligence.com/assets/favicons/favicon-49c9b25776778ff43873cf5ebde2e1ffcd0747ad1042ac5a5306cdde3ffca8cd.ico', 'logo': 'https://talosintelligence.com/assets/favicons/favicon-49c9b25776778ff43873cf5ebde2e1ffcd0747ad1042ac5a5306cdde3ffca8cd.ico', 'description': 'Cisco Talos Incident Response provides a full suite of proactive and reactive services to help you prepare, respond and recover from a breach. With Talos IR, you have direct access to the same threat intelligence available to Cisco and world-class emergency response capabilities — in addition to more than 350 threat researchers for questions and analysis. Let our experts work with you to evaluate existing plans, develop a new plan, and provide rapid assistance when you need it most.'}}
    opts = {'checkaffiliates': True, 'cacheperiod': 18, 'checknetblocks': True, 'checksubnets': True}
    optdescs = {'checkaffiliates': 'Apply checks to affiliates?', 'cacheperiod': 'Hours to cache list data before re-fetching.', 'checknetblocks': 'Report if any malicious IPs are found within owned netblocks?', 'checksubnets': 'Check if any malicious IPs are found within the same subnet of the target?'}
    results = None
    errorState = False
    ...
    def retrieveBlacklist(self):
        ...
        res = self.sf.fetchUrl('https://snort.org/downloads/ip-block-list', timeout=self.opts['_fetchtimeout'], useragent=self.opts['_useragent'])
        if res['code'] != '200':
