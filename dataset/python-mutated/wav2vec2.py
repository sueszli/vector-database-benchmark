...
class Wav2Vec2Config(FairseqDataclass):
    extractor_mode: EXTRACTOR_MODE_CHOICES = field(default='default', metadata={'help': 'mode for feature extractor. default has a single group norm with d groups in the first conv block, whereas layer_norm has layer norms in every block (meant to use with normalize=True)'})
    ...
    encoder_layerdrop: float = field(default=0.0, metadata={'help': 'probability of dropping a tarnsformer layer'})
    ...
    final_dim: int = field(default=0, metadata={'help': 'project final representations and targets to this many dimensions.set to encoder_embed_dim is <= 0'})
    layer_norm_first: bool = field(default=False, metadata={'help': 'apply layernorm first in the transformer'})
    conv_feature_layers: str = field(default='[(512, 10, 5)] + [(512, 3, 2)] * 4 + [(512,2,2)] + [(512,2,2)]', metadata={'help': 'string describing convolutional feature extraction layers in form of a python list that contains [(dim, kernel_size, stride), ...]'})
    ...
    quantize_targets: bool = field(default=False, metadata={'help': 'use quantized targets'})
    quantize_input: bool = field(default=False, metadata={'help': 'use quantized inputs'})
    same_quantizer: bool = field(default=False, metadata={'help': 'use same quantizer for inputs and targets'})
    ...
    quantizer_depth: int = field(default=1, metadata={'help': 'number of quantizer layers'})
    quantizer_factor: int = field(default=3, metadata={'help': 'dimensionality increase for inner quantizer layers (if depth > 1)'})
    latent_vars: int = field(default=320, metadata={'help': 'number of latent variables V in each group of the codebook'})
    latent_groups: int = field(default=2, metadata={'help': 'number of groups G of latent variables in the codebook'})
    latent_dim: int = field(default=0, metadata={'help': 'if > 0, uses this dimensionality for latent variables. otherwise uses final_dim / latent_groups'})
    ...
    mask_other: float = field(default=0, metadata={'help': 'secondary mask argument (used for more complex distributions), see help in compute_mask_indices'})
    no_mask_overlap: bool = field(default=False, metadata={'help': 'whether to allow masks to overlap'})
    mask_min_space: int = field(default=1, metadata={'help': 'min space between spans (if no overlap is enabled)'})
    require_same_masks: bool = field(default=True, metadata={'help': 'whether to number of masked timesteps must be the same across all examples in a batch'})
    mask_dropout: float = field(default=0.0, metadata={'help': 'percent of masks to unmask for each sample'})
    ...
    mask_channel_selection: MASKING_DISTRIBUTION_CHOICES = field(default='static', metadata={'help': 'how to choose mask length for channel masking'})
    mask_channel_other: float = field(default=0, metadata={'help': 'secondary mask argument (used for more complex distributions), see help in compute_mask_indicesh'})
    no_mask_channel_overlap: bool = field(default=False, metadata={'help': 'whether to allow channel masks to overlap'})
    mask_channel_min_space: int = field(default=1, metadata={'help': 'min space between spans (if no overlap is enabled)'})
    num_negatives: int = field(default=100, metadata={'help': 'number of negative examples from the same sample'})
    negatives_from_everywhere: bool = field(default=False, metadata={'help': 'sample negatives from everywhere, not just masked states'})
    ...
    pos_conv_depth: int = field(default=1, metadata={'help': 'depth of positional encoder network'})
    latent_temp: Tuple[float, float, float] = field(default=(2, 0.5, 0.999995), metadata={'help': 'temperature for latent variable sampling. can be tuple of 3 values (start, end, decay)'})
    max_positions: int = field(default=100000, metadata={'help': 'Max positions'})
    checkpoint_activations: bool = field(default=False, metadata={'help': 'recompute activations and save memory for extra compute'})
    required_seq_len_multiple: int = field(default=2, metadata={'help': 'pad the input to encoder such that the sequence length is divisible by multiple'})
    crop_seq_to_multiple: int = field(default=1, metadata={'help': 'crop convolutional feature extractor output such that the sequence length is divisible by multiple'})
    depthwise_conv_kernel_size: int = field(default=31, metadata={'help': 'depthwise-conv-kernel-size for convolution in conformer layer'})
    attn_type: str = field(default='', metadata={'help': 'if espnet use ESPNET MHA'})
    pos_enc_type: str = field(default='abs', metadata={'help': 'Positional encoding type to use in conformer'})
    fp16: bool = field(default=False, metadata={'help': 'If fp16 is being used'})
    adp_num: int = field(default=-1)
    adp_dim: int = field(default=64)
    adp_act_fn: str = field(default='relu')
    adp_trf_idx: str = field(default='all')
