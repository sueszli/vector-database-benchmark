...
backend = 'loky'
random_vector = Parallel(n_jobs=2, backend=backend)((delayed(stochastic_function)(10) for _ in range(n_vectors)))
...
if mp.get_start_method() != 'spawn':
    random_vector = Parallel(n_jobs=2, backend=backend)((delayed(stochastic_function_seeded)(10, None) for _ in range(n_vectors)))
    print_vector(random_vector, backend)
if mp.get_start_method() != 'spawn':
    seed = 42
    random_vector = Parallel(n_jobs=2, backend=backend)((delayed(stochastic_function_seeded)(10, [i, seed]) for i in range(n_vectors)))
    print_vector(random_vector, backend)
    random_vector = Parallel(n_jobs=2, backend=backend)((delayed(stochastic_function_seeded)(10, [i, seed]) for i in range(n_vectors)))
    print_vector(random_vector, backend)