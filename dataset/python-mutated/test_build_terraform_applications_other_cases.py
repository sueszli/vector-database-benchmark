...
from tests.integration.buildcmd.test_build_terraform_applications import BuildTerraformApplicationIntegBase
...
@skipIf(not RUN_BY_CANARY and (not CI_OVERRIDE), 'Skip Terraform test cases unless running in CI')
class TestInvalidBuildTerraformApplicationsWithZipBasedLambdaFunctionAndS3BackendNoS3Config(BuildTerraformApplicationIntegBase):
    terraform_application = Path('terraform/zip_based_lambda_functions_s3_backend') if not IS_WINDOWS else Path('terraform/zip_based_lambda_functions_s3_backend_windows')
...
@skipIf(not RUN_BY_CANARY and (not CI_OVERRIDE), 'Skip Terraform test cases unless running in CI')
@parameterized_class([{'app': 'conditional_layers'}, {'app': 'conditional_layers_null'}, {'app': 'one_lambda_function_linked_to_two_layers'}, {'app': 'lambda_function_referencing_local_var_layer'}])
...
@skipIf(not RUN_BY_CANARY and (not CI_OVERRIDE), 'Skip Terraform test cases unless running in CI')
@parameterized_class(('build_in_container',), [(False,), (True,)])
class TestBuildTerraformNestedDirectories(BuildTerraformApplicationIntegBase):
    terraform_application = Path('terraform/application_outside_root_directory') if not IS_WINDOWS else Path('terraform/application_outside_root_directory_windows')
    functions = [('aws_lambda_function.function1', 'hello world 1'), ('module.function2.aws_lambda_function.this', 'hello world 1')]
...
class TestBuildTerraformApplicationsNestedDirectoriesGetParametersFromSamConfig(BuildTerraformApplicationIntegBase):
    terraform_application = Path('terraform/application_outside_root_directory') if not IS_WINDOWS else Path('terraform/application_outside_root_directory_windows')
    functions = [('aws_lambda_function.function1', 'hello world 1')]
