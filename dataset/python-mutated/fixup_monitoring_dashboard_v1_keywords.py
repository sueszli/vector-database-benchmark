...
class monitoring_dashboardCallTransformer(cst.CSTTransformer):
    CTRL_PARAMS: Tuple[str] = ('retry', 'timeout', 'metadata')
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_dashboard': ('parent', 'dashboard', 'validate_only'), 'delete_dashboard': ('name',), 'get_dashboard': ('name',), 'list_dashboards': ('parent', 'page_size', 'page_token'), 'update_dashboard': ('dashboard', 'validate_only')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=monitoring_dashboardCallTransformer()):
    if False:
        i = 10
        return i + 15
