...
async def can_render(datasette, columns, rows, sql, query_name, database, table, request, view_name):
    datasette._can_render_saw = {'datasette': datasette, 'columns': columns, 'rows': rows, 'sql': sql, 'query_name': query_name, 'database': database, 'table': table, 'request': request, 'view_name': view_name}
...
async def render_test_all_parameters(datasette, columns, rows, sql, query_name, database, table, request, view_name, data):
    ...
    return {'body': json.dumps({'datasette': datasette, 'columns': columns, 'rows': rows, 'sql': sql, 'query_name': query_name, 'database': database, 'table': table, 'request': request, 'view_name': view_name, '1+1': result.first()[0]}, default=repr), 'content_type': request.args.get('content_type', 'text/plain'), 'status_code': int(request.args.get('status_code', 200)), 'headers': headers}
