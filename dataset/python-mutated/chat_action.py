...
class ChatActionSender:
    ...
    async def _worker(self) -> None:
        logger.debug('Started chat action %r sender in chat_id=%s via bot id=%d', self.action, self.chat_id, self.bot.id)
        try:
            counter = 0
            await self._wait(self.initial_sleep)
            while not self._close_event.is_set():
                start = time.monotonic()
                logger.debug('Sent chat action %r to chat_id=%s via bot %d (already sent actions %d)', self.action, self.chat_id, self.bot.id, counter)
                await self.bot.send_chat_action(chat_id=self.chat_id, action=self.action, message_thread_id=self.message_thread_id)
                counter += 1
                interval = self.interval - (time.monotonic() - start)
                await self._wait(interval)
        finally:
            logger.debug('Finished chat action %r sender in chat_id=%s via bot id=%d', self.action, self.chat_id, self.bot.id)
...
class ChatActionMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler: Callable[[TelegramObject, Dict[str, Any]], Awaitable[Any]], event: TelegramObject, data: Dict[str, Any]) -> Any:
        ...
        kwargs['message_thread_id'] = event.message_thread_id if isinstance(event, Message) and event.is_topic_message else None
