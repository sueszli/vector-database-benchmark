...
parser.add_argument('--experiment-dir', type=str, help='Experiment dir in which all sub-runs (seeds) are located (as sub-dirs). Each sub0-run dir must contain the files: params.json and progress.csv.')
parser.add_argument('--output-dir', type=str, help='The output dir, in which the cleaned up output will be placed.')
parser.add_argument('--results-filter', type=str, help='comma-separated list of csv fields to exclude.', default='experiment_id,pid,hostname,node_ip,trial_id,hist_stats/episode_reward,hist_stats/episode_lengths,experiment_tag')
parser.add_argument('--results-max-size', type=int, help='the max. size of the final results.csv file (in kb). Will erase every nth line in the original input to reach that goal. Use 0 for no limit (default=100).', default=100)
...
if __name__ == '__main__':
    ...
    for (i, sub_run) in enumerate(sorted(exp_dir)):
        ...
        if os.path.isdir(abspath) and re.search('^(\\w+?)_(\\w+?-v\\d+)(_\\d+)', sub_run):
            cleaned_up_out = re.sub('^(\\w+?)_(\\w+?-v\\d+)(_\\d+)(_.+)?(_\\d{4}-\\d{2}-\\d{2})_\\d{2}-\\d{2}-\\w+', '{:02}_\\1_\\2\\4\\5'.format(i), sub_run)
            cleaned_up_out = re.sub('^(.+)env=\\w+?-v\\d+,?(.+)', '\\1\\2', cleaned_up_out)
