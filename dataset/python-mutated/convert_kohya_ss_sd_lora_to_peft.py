...
if __name__ == '__main__':
    ...
    parser.add_argument('--kohya_lora_path', default=None, type=str, required=True, help='Path to kohya_ss trained LoRA')
    ...
    models_keys = {}
    for (model, model_key, model_name) in [(text_encoder, LORA_PREFIX_TEXT_ENCODER, 'text_encoder'), (unet, LORA_PREFIX_UNET, 'unet')]:
        models_keys.update({f'{model_key}.{peft_key}'.replace('.', '_'): peft_key for peft_key in (x[0] for x in model.named_modules())})
    lora_info: Dict[str, Dict[str, LoRAInfo]] = {'text_encoder': {}, 'unet': {}}
