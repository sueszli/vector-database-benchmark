...
async def test_wireguard(tdata, monkeypatch, caplog):
    ...
    with taddons.context(Proxyserver()):
        ...
        proc = await asyncio.create_subprocess_exec(test_client_path, str(port), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
...
async def test_multiple_local_redirectors(patched_local_redirector):
    ...
    with taddons.context():
        ...
        with pytest.raises(RuntimeError, match='Cannot spawn more than one local redirector'):
            await inst2.start()
