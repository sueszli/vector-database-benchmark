...
from typing import TYPE_CHECKING, Union, Tuple, List, Optional, Iterable, Sequence, Dict, Set, Literal
import aiohttp
import discord
from babel import Locale as BabelLocale, UnknownLocaleError
from redbot.core.data_manager import storage_type
from . import __version__, version_info as red_version_info, commands, errors, i18n, bank, modlog
...
from .utils.chat_formatting import box, escape, humanize_list, humanize_number, humanize_timedelta, inline, pagify, warning
from .commands import CommandConverter, CogConverter
from .commands.requires import PrivilegeLevel
from .commands.help import HelpMenuSetting
_entities = {'*': '&midast;', '\\': '&bsol;', '`': '&grave;', '!': '&excl;', '{': '&lcub;', '[': '&lsqb;', '_': '&UnderBar;', '(': '&lpar;', '#': '&num;', '.': '&period;', '+': '&plus;', '}': '&rcub;', ']': '&rsqb;', ')': '&rpar;'}
...
class CoreLogic:
        ...
        async for (spec, name) in AsyncIter(pkg_specs, steps=10):
            try:
                ...
                if isinstance(e, commands.CommandRegistrationError):
                    if e.alias_conflict:
                        error_message = _('Alias {alias_name} is already an existing command or alias in one of the loaded cogs.').format(alias_name=inline(e.name))
                    else:
                        error_message = _('Command {command_name} is already an existing command or alias in one of the loaded cogs.').format(command_name=inline(e.name))
                ...
                try:
                    ...
                    log.exception("Shared library check failed, if you're not using modified Downloader, report this issue.")
                    maybe_repo = None
                if maybe_repo is not None:
                    repos_with_shared_libs.add(maybe_repo.name)
        return {'loaded_packages': loaded_packages, 'failed_packages': failed_packages, 'invalid_pkg_names': invalid_pkg_names, 'notfound_packages': notfound_packages, 'alreadyloaded_packages': alreadyloaded_packages, 'failed_with_reason_packages': failed_with_reason_packages, 'repos_with_shared_libs': list(repos_with_shared_libs)}
    ...
    async def _reload(self, pkg_names: Sequence[str]) -> Dict[str, Union[List[str], Dict[str, str]]]:
        """
...
class Core(commands.commands._RuleDropper, commands.Cog, CoreLogic):
    ...
    async def info(self, ctx: commands.Context):
        ...
        if embed_links:
            ...
            about = _('This bot is an instance of [Red, an open source Discord bot]({}) created by [Twentysix]({}) and [improved by many]({}).\n\nRed is backed by a passionate community who contributes and creates content for everyone to enjoy. [Join us today]({}) and help us improve!\n\n(c) Cog Creators').format(red_repo, author_repo, contributors_url, support_server_url)
            embed = discord.Embed(color=await ctx.embed_colour())
            embed.add_field(name=_('Instance owned by team') if app_info.team else _('Instance owned by'), value=str(owner))
            embed.add_field(name='Python', value=python_version)
            embed.add_field(name='discord.py', value=dpy_version)
            embed.add_field(name=_('Red version'), value=red_version)
            if outdated in (True, None):
                if outdated is True:
                    outdated_value = _('Yes, {version} is available.').format(version=str(pypi_version))
                else:
            ...
            embed.set_footer(text=_('Bringing joy since 02 Jan 2016 (over {} days ago!)').format(days_since))
            await ctx.send(embed=embed)
        else:
            python_version = '{}.{}.{}'.format(*sys.version_info[:3])
            dpy_version = '{}'.format(discord.__version__)
            red_version = '{}'.format(__version__)
            about = _('This bot is an instance of Red, an open source Discord bot (1) created by Twentysix (2) and improved by many (3).\n\nRed is backed by a passionate community who contributes and creates content for everyone to enjoy. Join us today (4) and help us improve!\n\n(c) Cog Creators')
            about = box(about)
            if app_info.team:
                extras = _('Instance owned by team: [{owner}]\nPython:                 [{python_version}] (5)\ndiscord.py:             [{dpy_version}] (6)\nRed version:            [{red_version}] (7)\n').format(owner=owner, python_version=python_version, dpy_version=dpy_version, red_version=red_version)
            else:
                extras = _('Instance owned by: [{owner}]\nPython:            [{python_version}] (5)\ndiscord.py:        [{dpy_version}] (6)\nRed version:       [{red_version}] (7)\n').format(owner=owner, python_version=python_version, dpy_version=dpy_version, red_version=red_version)
            if outdated in (True, None):
                if outdated is True:
                    outdated_value = _('Yes, {version} is available.').format(version=str(pypi_version))
                else:
            ...
            refs = _('**References**\n1. <{}>\n2. <{}>\n3. <{}>\n4. <{}>\n5. <{}>\n6. <{}>\n7. <{}>\n').format(red_repo, author_repo, contributors_url, support_server_url, python_url, dpy_repo, red_pypi)
            await ctx.send(refs)
    ...
    async def uptime(self, ctx: commands.Context):
        ...
        await ctx.send(_('I have been up for: **{time_quantity}** (since {timestamp})').format(time_quantity=uptime_str, timestamp=discord.utils.format_dt(uptime, 'f')))
    ...
    async def mydata_whatdata(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        await ctx.send(_("This bot stores some data about users as necessary to function. This is mostly the ID your user is assigned by Discord, linked to a handful of things depending on what you interact with in the bot. There are a few commands which store it to keep track of who created something. (such as playlists) For full details about this as well as more in depth details of what is stored and why, see {link}.\n\nAdditionally, 3rd party addons loaded by the bot's owner may or may not store additional things. You can use `{prefix}mydata 3rdparty` to view the statements provided by each 3rd-party addition.").format(link=link, prefix=ctx.clean_prefix))
    ...
    async def mydata_3rd_party(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        statements = {ext_name: getattr(ext, '__red_end_user_data_statement__', None) for (ext_name, ext) in ctx.bot.extensions.items() if not (ext.__package__ and ext.__package__.startswith('redbot.'))}
        if not statements:
            return await ctx.send(_('This instance does not appear to have any 3rd-party extensions loaded.'))
        parts = []
        ...
        await ctx.send(_("Here's a generated page with the statements provided by 3rd-party extensions."), file=discord.File(fp, filename='3rd-party.html'))
    ...
    async def get_serious_confirmation(self, ctx: commands.Context, prompt: str) -> bool:
        ...
        try:
            message = await ctx.bot.wait_for('message', check=lambda m: m.channel.id == ctx.channel.id and m.author.id == ctx.author.id, timeout=30)
    ...
    async def mydata_forgetme(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        if ctx.assume_yes:
            ctx.command.reset_cooldown(ctx)
            return await ctx.send(_('This command ({command}) does not support non-interactive usage.').format(command=ctx.command.qualified_name))
        if not await self.get_serious_confirmation(ctx, _('This will cause the bot to get rid of and/or disassociate data from you. It will not get rid of operational data such as modlog entries, warnings, or mutes. If you are sure this is what you want, please respond with the following:')):
            ctx.command.reset_cooldown(ctx)
            return
        ...
        if await ctx.bot._config.datarequests.user_requests_are_strict():
            ...
            requester = 'user'
        results = await self.bot.handle_data_deletion_request(requester=requester, user_id=ctx.author.id)
        if results.failed_cogs and results.failed_modules:
            await ctx.send(_('I tried to delete all non-operational data about you (that I know how to delete) {mention}, however the following modules errored: {modules}. Additionally, the following cogs errored: {cogs}.\nPlease contact the owner of this bot to address this.\nNote: Outside of these failures, data should have been deleted.').format(mention=ctx.author.mention, cogs=humanize_list(results.failed_cogs), modules=humanize_list(results.failed_modules)))
        elif results.failed_cogs:
            await ctx.send(_('I tried to delete all non-operational data about you (that I know how to delete) {mention}, however the following cogs errored: {cogs}.\nPlease contact the owner of this bot to address this.\nNote: Outside of these failures, data should have been deleted.').format(mention=ctx.author.mention, cogs=humanize_list(results.failed_cogs)))
        elif results.failed_modules:
            await ctx.send(_('I tried to delete all non-operational data about you (that I know how to delete) {mention}, however the following modules errored: {modules}.\nPlease contact the owner of this bot to address this.\nNote: Outside of these failures, data should have been deleted.').format(mention=ctx.author.mention, modules=humanize_list(results.failed_modules)))
        else:
            await ctx.send(_("I've deleted any non-operational data about you (that I know how to delete) {mention}").format(mention=ctx.author.mention))
        if results.unhandled:
            await ctx.send(_('{mention} The following cogs did not handle deletion:\n{cogs}.').format(mention=ctx.author.mention, cogs=humanize_list(results.unhandled)))
    ...
    async def mydata_getdata(self, ctx: commands.Context):
        ...
        await ctx.send(_("This command doesn't do anything yet, but we're working on adding support for this."))
    ...
    async def mydata_owner_allow_user_deletions(self, ctx):
        ...
        **Example:**
        ...
        await ctx.send(_('User can delete their own data. This will not include operational data such as blocked users.'))
    ...
    async def mydata_owner_user_deletion_level(self, ctx, level: int):
        ...
        **Arguments:**
        ...
        if level == 1:
            ...
            await ctx.send(_('Cogs will be instructed to remove all non operational data upon a user request.'))
        elif level == 0:
            await ctx.bot._config.datarequests.user_requests_are_strict.set(False)
            await ctx.send(_('Cogs will be informed a user has made a data deletion request, and the details of what to delete will be left to the discretion of the cog author.'))
        else:
            await ctx.send_help()
    ...
    async def mydata_discord_deletion_request(self, ctx, user_id: int):
        ...
        **Arguments:**
        ...
        """
        if not await self.get_serious_confirmation(ctx, _('This will cause the bot to get rid of or disassociate all data from the specified user ID. You should not use this unless Discord has specifically requested this with regard to a deleted user. This will remove the user from various anti-abuse measures. If you are processing a manual request from a user, you may want `{prefix}{command_name}` instead.\n\nIf you are sure this is what you intend to do please respond with the following:').format(prefix=ctx.clean_prefix, command_name='mydata ownermanagement deleteforuser')):
            return
        results = await self.bot.handle_data_deletion_request(requester='discord_deleted_user', user_id=user_id)
        if results.failed_cogs and results.failed_modules:
            await ctx.send(_('I tried to delete all data about that user, (that I know how to delete) however the following modules errored: {modules}. Additionally, the following cogs errored: {cogs}\nPlease check your logs and contact the creators of these cogs and modules.\nNote: Outside of these failures, data should have been deleted.').format(cogs=humanize_list(results.failed_cogs), modules=humanize_list(results.failed_modules)))
        elif results.failed_cogs:
            await ctx.send(_('I tried to delete all data about that user, (that I know how to delete) however the following cogs errored: {cogs}.\nPlease check your logs and contact the creators of these cogs and modules.\nNote: Outside of these failures, data should have been deleted.').format(cogs=humanize_list(results.failed_cogs)))
        elif results.failed_modules:
            await ctx.send(_('I tried to delete all data about that user, (that I know how to delete) however the following modules errored: {modules}.\nPlease check your logs and contact the creators of these cogs and modules.\nNote: Outside of these failures, data should have been deleted.').format(modules=humanize_list(results.failed_modules)))
        else:
    ...
    async def mydata_user_deletion_request_by_owner(self, ctx, user_id: int):
        ...
        **Arguments:**
        - `<user_id>` - The id of the user whose data would be deleted.
        """
        if not await self.get_serious_confirmation(ctx, _("This will cause the bot to get rid of or disassociate a lot of non-operational data from the specified user. Users have access to different command for this unless they can't interact with the bot at all. This is a mostly safe operation, but you should not use it unless processing a request from this user as it may impact their usage of the bot. \n\nIf you are sure this is what you intend to do please respond with the following:")):
            return
        if await ctx.bot._config.datarequests.user_requests_are_strict():
            requester = 'user_strict'
        else:
        ...
        if results.failed_cogs and results.failed_modules:
            await ctx.send(_('I tried to delete all non-operational data about that user, (that I know how to delete) however the following modules errored: {modules}. Additionally, the following cogs errored: {cogs}\nPlease check your logs and contact the creators of these cogs and modules.\nNote: Outside of these failures, data should have been deleted.').format(cogs=humanize_list(results.failed_cogs), modules=humanize_list(results.failed_modules)))
        elif results.failed_cogs:
            await ctx.send(_('I tried to delete all non-operational data about that user, (that I know how to delete) however the following cogs errored: {cogs}.\nPlease check your logs and contact the creators of these cogs and modules.\nNote: Outside of these failures, data should have been deleted.').format(cogs=humanize_list(results.failed_cogs)))
        elif results.failed_modules:
            await ctx.send(_('I tried to delete all non-operational data about that user, (that I know how to delete) however the following modules errored: {modules}.\nPlease check your logs and contact the creators of these cogs and modules.\nNote: Outside of these failures, data should have been deleted.').format(modules=humanize_list(results.failed_modules)))
        else:
            await ctx.send(_("I've deleted all non-operational data about that user that I know how to delete."))
        if results.unhandled:
            await ctx.send(_('{mention} The following cogs did not handle deletion:\n{cogs}.').format(mention=ctx.author.mention, cogs=humanize_list(results.unhandled)))
    ...
    async def mydata_user_deletion_by_owner(self, ctx, user_id: int):
        ...
        **Arguments:**
        - `<user_id>` - The id of the user whose data would be deleted.
        """
        if not await self.get_serious_confirmation(ctx, _('This will cause the bot to get rid of or disassociate a lot of data about the specified user. This may include more than just end user data, including anti abuse records.\n\nIf you are sure this is what you intend to do please respond with the following:')):
            return
    ...
    async def embedset_showsettings(self, ctx: commands.Context, command: CommandConverter=None) -> None:
        ...
        **Arguments:**
        ...
        if command_name is not None:
            ...
            text += _('Global command setting for {command} command: {value}\n').format(command=inline(command_name), value=global_command_setting)
        if ctx.guild:
            ...
            if command_name is not None:
                ...
                text += _('Server command setting for {command} command: {value}\n').format(command=inline(command_name), value=command_setting)
        if ctx.channel:
    ...
    async def embedset_guild(self, ctx: commands.Context, enabled: bool=None):
        ...
        **Arguments:**
        ...
        await ctx.send(_('Embeds are now enabled for this guild.') if enabled else _('Embeds are now disabled for this guild.'))
    ...
    async def embedset_command_global(self, ctx: commands.Context, command: CommandConverter, enabled: bool=None):
        ...
        **Arguments:**
        ...
        if enabled:
            await ctx.send(_('Embeds are now enabled for {command_name} command.').format(command_name=inline(command_name)))
        else:
            await ctx.send(_('Embeds are now disabled for {command_name} command.').format(command_name=inline(command_name)))
    ...
    @commands.guildowner_or_permissions(administrator=True)
    @commands.guild_only()
    async def embedset_channel(self, ctx: commands.Context, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.ForumChannel], enabled: bool=None):
        """
        ...
        **Arguments:**
        ...
        await ctx.send(_('Embeds are now {} for this channel.').format(_('enabled') if enabled else _('disabled')))
    ...
    async def embedset_user(self, ctx: commands.Context, enabled: bool=None):
        ...
        **Arguments:**
        ...
        await ctx.send(_('Embeds are now enabled for you in DMs.') if enabled else _('Embeds are now disabled for you in DMs.'))
    ...
    async def traceback(self, ctx: commands.Context, public: bool=False):
        ...
        **Arguments:**
        ...
        if self.bot._last_exception:
            try:
                await self.bot.send_interactive(channel, pagify(self.bot._last_exception, shorten_by=10), user=ctx.author, box_lang='py')
    ...
    async def invite(self, ctx):
        ...
        **Example:**
        ...
        if (admin := self.bot.get_cog('Admin')) and await admin.config.serverlocked():
            message += '\n\n' + warning(_('This bot is currently **serverlocked**, meaning that it is locked to its current servers and will leave any server it joins.'))
        try:
    ...
    async def public(self, ctx, confirm: bool=False):
        ...
        **Arguments:**
        ...
        if not app_info.bot_public:
            await ctx.send('I am not a public bot. That means that nobody except you can invite me on new servers.\n\nYou can change this by ticking `Public bot` in your token settings: https://discord.com/developers/applications/{0}/bot'.format(self.bot.user.id))
            return
        if not confirm:
            await ctx.send("You're about to make the `{0}invite` command public. All users will be able to invite me on their server.\n\nIf you agree, you can type `{0}inviteset public yes`.".format(ctx.clean_prefix))
        else:
    ...
    async def commandscope(self, ctx: commands.Context):
        ...
        if enabled is True:
            ...
            await ctx.send(_('The `applications.commands` scope has been removed from the invite URL.'))
    ...
    async def leave(self, ctx: commands.Context, *servers: GuildConverter):
        ...
        **Arguments:**
        ...
        if leaving_local_guild:
            guilds = (ctx.guild,)
            msg = _("You haven't passed any server ID. Do you want me to leave this server?") + ' (yes/no)'
        elif number > 1:
            msg = _('Are you sure you want me to leave these servers?') + ' (yes/no):\n' + '\n'.join((f'- {guild.name} (`{guild.id}`)' for guild in guilds))
        else:
            msg = _('Are you sure you want me to leave this server?') + ' (yes/no):\n' + f'- {guilds[0].name} (`{guilds[0].id}`)'
        for guild in guilds:
            if guild.owner.id == ctx.me.id:
                return await ctx.send(_('I cannot leave the server `{server_name}`: I am the owner of it.').format(server_name=guild.name))
        ...
        **Arguments:**
        ...
        if (already_loaded := outcomes['alreadyloaded_packages']):
            if len(already_loaded) == 1:
                ...
                formed = _('The following packages are already loaded: {packs}').format(packs=humanize_list([inline(package) for package in already_loaded]))
            output.append(formed)
        if (failed := outcomes['failed_packages']):
            if len(failed) == 1:
                formed = _('Failed to load the following package: {pack}.\nCheck your console or logs for details.').format(pack=inline(failed[0]))
            else:
                formed = _('Failed to load the following packages: {packs}\nCheck your console or logs for details.').format(packs=humanize_list([inline(package) for package in failed]))
            output.append(formed)
        if (invalid_pkg_names := outcomes['invalid_pkg_names']):
            if len(invalid_pkg_names) == 1:
                formed = _('The following name is not a valid package name: {pack}\nPackage names cannot start with a number and can only contain ascii numbers, letters, and underscores.').format(pack=inline(invalid_pkg_names[0]))
            else:
                formed = _('The following names are not valid package names: {packs}\nPackage names cannot start with a number and can only contain ascii numbers, letters, and underscores.').format(packs=humanize_list([inline(package) for package in invalid_pkg_names]))
            output.append(formed)
        if (not_found := outcomes['notfound_packages']):
            if len(not_found) == 1:
                formed = _('The following package was not found in any cog path: {pack}.').format(pack=inline(not_found[0]))
            else:
                formed = _('The following packages were not found in any cog path: {packs}').format(packs=humanize_list([inline(package) for package in not_found]))
            output.append(formed)
        if (failed_with_reason := outcomes['failed_with_reason_packages']):
            ...
            if len(failed_with_reason) == 1:
                formed = _('This package could not be loaded for the following reason:\n\n{reason}').format(reason=reasons)
            else:
                formed = _('These packages could not be loaded for the following reasons:\n\n{reasons}').format(reasons=reasons)
            output.append(formed)
        if (repos_with_shared_libs := outcomes['repos_with_shared_libs']):
            if len(repos_with_shared_libs) == 1:
                formed = _('**WARNING**: The following repo is using shared libs which are marked for removal in the future: {repo}.\nYou should inform maintainer of the repo about this message.').format(repo=inline(repos_with_shared_libs.pop()))
            else:
                formed = _('**WARNING**: The following repos are using shared libs which are marked for removal in the future: {repos}.\nYou should inform maintainers of these repos about this message.').format(repos=humanize_list([inline(repo) for repo in repos_with_shared_libs]))
            output.append(formed)
        if output:
            total_message = '\n\n'.join(output)
        ...
        **Arguments:**
        ...
        if (unloaded := outcomes['unloaded_packages']):
            if len(unloaded) == 1:
                ...
                formed = _('The following packages were unloaded: {packs}.').format(packs=humanize_list([inline(package) for package in unloaded]))
            output.append(formed)
        if (failed := outcomes['notloaded_packages']):
            if len(failed) == 1:
                formed = _('The following package was not loaded: {pack}.').format(pack=inline(failed[0]))
            else:
                formed = _('The following packages were not loaded: {packs}.').format(packs=humanize_list([inline(package) for package in failed]))
            output.append(formed)
        if output:
            total_message = '\n\n'.join(output)
    ...
    async def reload(self, ctx: commands.Context, *cogs: str):
        ...
        **Arguments:**
        ...
        if (failed := outcomes['failed_packages']):
            if len(failed) == 1:
                formed = _('Failed to reload the following package: {pack}.\nCheck your console or logs for details.').format(pack=inline(failed[0]))
            else:
                formed = _('Failed to reload the following packages: {packs}\nCheck your console or logs for details.').format(packs=humanize_list([inline(package) for package in failed]))
            output.append(formed)
        if (invalid_pkg_names := outcomes['invalid_pkg_names']):
            if len(invalid_pkg_names) == 1:
                formed = _('The following name is not a valid package name: {pack}\nPackage names cannot start with a number and can only contain ascii numbers, letters, and underscores.').format(pack=inline(invalid_pkg_names[0]))
        ...
        if (failed_with_reason := outcomes['failed_with_reason_packages']):
            reasons = '\n'.join([f'`{x}`: {y}' for (x, y) in failed_with_reason.items()])
            if len(failed_with_reason) == 1:
                formed = _('This package could not be reloaded for the following reason:\n\n{reason}').format(reason=reasons)
            else:
                formed = _('These packages could not be reloaded for the following reasons:\n\n{reasons}').format(reasons=reasons)
            output.append(formed)
        if (repos_with_shared_libs := outcomes['repos_with_shared_libs']):
            if len(repos_with_shared_libs) == 1:
                formed = _('**WARNING**: The following repo is using shared libs which are marked for removal in the future: {repo}.\nYou should inform maintainers of these repos about this message.').format(repo=inline(repos_with_shared_libs.pop()))
            else:
                formed = _('**WARNING**: The following repos are using shared libs which are marked for removal in the future: {repos}.\nYou should inform maintainers of these repos about this message.').format(repos=humanize_list([inline(repo) for repo in repos_with_shared_libs]))
            output.append(formed)
        if output:
    ...
    async def slash_enable(self, ctx: commands.Context, command_name: str, command_type: Literal['slash', 'message', 'user']='slash'):
        ...
        **Arguments:**
        ...
        if key not in command_list:
            await ctx.send(_('That application command could not be found. Use `{prefix}slash list` to see all application commands. You may need to double check the command type.').format(prefix=ctx.prefix))
            return
        try:
    ...
    async def slash_disable(self, ctx: commands.Context, command_name: str, command_type: Literal['slash', 'message', 'user']='slash'):
        ...
        **Arguments:**
        ...
        if existing is not None and existing.extras.get('red_force_enable', False):
            await ctx.send(_('That application command has been set as required for the cog to function by the author, and cannot be disabled. The cog must be unloaded to remove the command.'))
            return
        current_settings = await self.bot.list_enabled_app_commands()
        current_settings = current_settings[command_type]
    ...
    async def slash_enablecog(self, ctx: commands.Context, cog_name: str):
        ...
        **Arguments:**
        ...
        if not (to_add_slash or to_add_message or to_add_user):
            await ctx.send(_("Couldn't find any disabled commands from the cog `{cog_name}`. Use `{prefix}slash list` to see all cogs with application commands.").format(cog_name=cog_name, prefix=ctx.prefix))
            return
        SLASH_CAP = 100
        CONTEXT_CAP = 5
        ...
        if total_slash > SLASH_CAP:
            await ctx.send(_('Enabling all application commands from that cog would enable a total of {count} commands, exceeding the {cap} command limit for slash commands. Disable some commands first.').format(count=total_slash, cap=SLASH_CAP))
            return
        if total_message > CONTEXT_CAP:
            await ctx.send(_('Enabling all application commands from that cog would enable a total of {count} commands, exceeding the {cap} command limit for message commands. Disable some commands first.').format(count=total_message, cap=CONTEXT_CAP))
            return
        if total_user > CONTEXT_CAP:
            await ctx.send(_('Enabling all application commands from that cog would enable a total of {count} commands, exceeding the {cap} command limit for user commands. Disable some commands first.').format(count=total_user, cap=CONTEXT_CAP))
            return
        ...
        **Arguments:**
        ...
        for (key, com) in self.bot.tree._context_menus.items():
            if self._is_submodule(cog_name, com.module):
                ...
                removed.append(name)
        if not removed:
            await ctx.send(_("Couldn't find any enabled commands from the `{cog_name}` cog. Use `{prefix}slash list` to see all cogs with application commands.").format(cog_name=cog_name, prefix=ctx.prefix))
            return
        await self.bot.tree.red_check_enabled()
        formatted_names = humanize_list([inline(name) for name in removed])
        await ctx.send(_('Disabled {count} commands from `{cog_name}`:\n{names}').format(count=len(removed), cog_name=cog_name, names=formatted_names))
        ...
        **Arguments:**
        ...
        async with ctx.typing():
            try:
                ...
                if not guild:
                    raise e
                await ctx.send(_('I need the `applications.commands` scope in this server to be able to do that. You can tell the bot to add that scope to invite links using `{prefix}inviteset commandscope`, and can then run `{prefix}invite` to get an invite that will give the bot the scope. You do not need to kick the bot to enable the scope, just use that invite to re-auth the bot with the scope enabled.').format(prefix=ctx.prefix))
                return
            except Exception as e:
                raise e
    ...
    async def bankset_showsettings(self, ctx: commands.Context):
        ...
        settings = _('Bank settings:\n\nBank name: {bank_name}\nBank scope: {bank_scope}\nCurrency: {currency_name}\nDefault balance: {default_balance}\nMaximum allowed balance: {maximum_bal}\n').format(bank_name=bank_name, bank_scope=bank_scope, currency_name=currency_name, default_balance=humanize_number(default_balance), maximum_bal=humanize_number(max_balance))
    ...
    async def bankset_toggleglobal(self, ctx: commands.Context, confirm: bool=False):
        ...
        if confirm is False:
            await ctx.send(_("This will toggle the bank to be {banktype}, deleting all accounts in the process! If you're sure, type `{command}`").format(banktype=word, command=f'{ctx.clean_prefix}bankset toggleglobal yes'))
        else:
    ...
    async def bankset_maxbal(self, ctx: commands.Context, *, amount: int):
        ...
        try:
            ...
            return await ctx.send(_('Amount must be greater than zero and less than {max}.').format(max=humanize_number(bank._MAX_BALANCE)))
        await ctx.send(_('Maximum balance has been set to: {amount}').format(amount=humanize_number(amount)))
    ...
    async def bankset_registeramount(self, ctx: commands.Context, creds: int):
        ...
        **Arguments**
        ...
        try:
            ...
            return await ctx.send(_('Amount must be greater than or equal to zero and less than {maxbal}.').format(maxbal=humanize_number(max_balance)))
        await ctx.send(_('Registering an account will now give {num} {currency}.').format(num=humanize_number(creds), currency=credits_name))
    ...
    async def bankset_reset(self, ctx, confirmation: bool=False):
        ...
        **Arguments**
        ...
        if confirmation is False:
            await ctx.send(_("This will delete all bank accounts for {scope}.\nIf you're sure, type `{prefix}bankset reset yes`").format(scope=self.bot.user.display_name if await bank.is_global() else _('this server'), prefix=ctx.clean_prefix))
        else:
            await bank.wipe_bank(guild=ctx.guild)
            await ctx.send(_('All bank accounts for {scope} have been deleted.').format(scope=self.bot.user.display_name if await bank.is_global() else _('this server')))
    ...
    async def bankset_prune_local(self, ctx, confirmation: bool=False):
        ...
        **Arguments**
        ...
        if confirmation is False:
            await ctx.send(_("This will delete all bank accounts for users no longer in this server.\nIf you're sure, type `{prefix}bankset prune local yes`").format(prefix=ctx.clean_prefix))
        else:
            await bank.bank_prune(self.bot, guild=ctx.guild)
            await ctx.send(_('Bank accounts for users no longer in this server have been deleted.'))
    ...
    async def bankset_prune_global(self, ctx, confirmation: bool=False):
        ...
        **Arguments**
        ...
        if confirmation is False:
            await ctx.send(_("This will delete all bank accounts for users who no longer share a server with the bot.\nIf you're sure, type `{prefix}bankset prune global yes`").format(prefix=ctx.clean_prefix))
        else:
            await bank.bank_prune(self.bot)
            await ctx.send(_('Bank accounts for users who no longer share a server with the bot have been pruned.'))

    @bankset_prune.command(name='user', usage='<user> [confirmation=False]')
    async def bankset_prune_user(self, ctx, member_or_id: Union[discord.Member, RawUserIdConverter], confirmation: bool=False):
        ...
        **Arguments**
        ...
        try:
            ...
            uid = member_or_id
        if confirmation is False:
            await ctx.send(_("This will delete {name}'s bank account.\nIf you're sure, type `{prefix}bankset prune user {id} yes`").format(prefix=ctx.clean_prefix, id=uid, name=name))
        else:
    ...
    @commands.guild_only()
    async def modlogset_modlog(self, ctx: commands.Context, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel]=None):
        ...
        if channel:
            if channel.permissions_for(guild.me).send_messages:
                ...
                await ctx.send(_('Mod events will be sent to {channel}.').format(channel=channel.mention))
            else:
                await ctx.send(_('I do not have permissions to send messages in {channel}!').format(channel=channel.mention))
        else:
            try:
    ...
    async def modlogset_cases(self, ctx: commands.Context, action: str=None):
        ...
        if not casetype:
            ...
            await ctx.send(_('Case creation for {action_name} actions is now {enabled}.').format(action_name=action, enabled=_('enabled') if not enabled else _('disabled')))
    ...
    async def modlogset_resetcases(self, ctx: commands.Context):
        ...
        guild = ctx.guild
        await ctx.send(_('Are you sure you would like to reset all modlog cases in this server?') + ' (yes/no)')
        try:
    ...
    async def _set_bot_description(self, ctx: commands.Context, *, description: str=''):
        ...
        **Arguments:**
        ...
        if not description:
            ...
            await ctx.send(_('This description is too long to properly display. Please try again with below 250 characters.'))
        else:
    ...
    async def _set_bot_avatar(self, ctx: commands.Context, url: str=None):
        ...
        **Arguments:**
        ...
        try:
            ...
            await ctx.send(_('Failed. Remember that you can edit my avatar up to two times a hour. The URL or attachment must be a valid image in either JPG or PNG format.'))
        except ValueError:
        ...
        **Arguments:**
        ...
        try:
            if self.bot.user.public_flags.verified_bot:
                await ctx.send(_('The username of a verified bot cannot be manually changed. Please contact Discord support to change it.'))
                return
            if len(username) > 32:
            ...
            await ctx.send(_('Changing the username timed out. Remember that you can only do it up to 2 times an hour. Use nicknames if you need frequent changes: {command}').format(command=inline(f'{ctx.clean_prefix}set bot nickname')))
        except discord.HTTPException as e:
            if e.code == 50035:
                error_string = e.text.split('\n')[1]
                await ctx.send(_('Failed to change the username. Discord returned the following error:\n{error_message}').format(error_message=inline(error_string)))
            else:
                log.error('Unexpected error occurred when trying to change the username.', exc_info=e)
    ...
    @_set_status.command(name='streaming', aliases=['stream', 'twitch'], usage='[(<streamer> <stream_title>)]')
    @commands.bot_in_a_guild()
    @commands.is_owner()
    async def _set_status_stream(self, ctx: commands.Context, streamer: commands.Range[str, 1, 489]=None, *, stream_title: commands.Range[str, 1, 128]=None):
    ...
    @commands.bot_in_a_guild()
    @commands.is_owner()
    async def _set_status_game(self, ctx: commands.Context, *, game: commands.Range[str, 1, 128]=None):
    ...
    @commands.bot_in_a_guild()
    @commands.is_owner()
    async def _set_status_listening(self, ctx: commands.Context, *, listening: commands.Range[str, 1, 128]=None):
    ...
    @commands.bot_in_a_guild()
    @commands.is_owner()
    async def _set_status_watching(self, ctx: commands.Context, *, watching: commands.Range[str, 1, 128]=None):
    ...
    @commands.bot_in_a_guild()
    @commands.is_owner()
    async def _set_status_competing(self, ctx: commands.Context, *, competing: commands.Range[str, 1, 128]=None):
    ...
    @_set_roles.command(name='removeadminrole', aliases=['remadmindrole', 'deladminrole', 'deleteadminrole'])
    @commands.guildowner()
    @commands.guild_only()
    ...
    @_set_roles.command(name='removemodrole', aliases=['remmodrole', 'delmodrole', 'deletemodrole'])
    @commands.guildowner()
    @commands.guild_only()
    ...
    async def _set_locale_global(self, ctx: commands.Context, language_code: str):
        ...
        **Arguments:**
        ...
        if locale.territory is None:
            await ctx.send(_('Invalid format - language code has to include country code, e.g. `en-US`'))
            return
    ...
    async def _set_regional_format_global(self, ctx: commands.Context, language_code: str):
        ...
        **Arguments:**
        ...
        await ctx.send(_('Global regional formatting will now be based on `{language_code}` locale.').format(language_code=standardized_locale_name))
    ...
    async def _set_regional_format_local(self, ctx: commands.Context, language_code: str):
        ...
        **Arguments:**
        ...
        await ctx.send(_('Regional formatting will now be based on `{language_code}` locale.').format(language_code=standardized_locale_name))

    @_set.group(name='api', invoke_without_command=True)
    @commands.is_owner()
    async def _set_api(self, ctx: commands.Context, service: Optional[str]=None, *, tokens: Optional[TokenConverter]=None):
        """
        ...
        **Arguments:**
        ...
        if services:
            ...
            if len(services) > 1:
                ...
                msg = _('Service deleted successfully: {service_name}').format(service_name=services[0])
            await ctx.send(msg)
        else:
    ...
    async def _set_ownernotifications_adddestination(self, ctx: commands.Context, *, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel]):
        """
    ...
    @_set_ownernotifications.command(name='removedestination', aliases=['remdestination', 'deletedestination', 'deldestination'])
    async def _set_ownernotifications_removedestination(self, ctx: commands.Context, *, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, int]):
        """
    ...
    async def _set_showsettings(self, ctx: commands.Context, server: discord.Guild=None):
        ...
        if server:
            ...
            guild_regional_format = await i18n.get_regional_format_from_guild(self.bot, server) or guild_locale
            guild_settings = _('Admin roles: {admin}\nMod roles: {mod}\nLocale: {guild_locale}\nRegional format: {guild_regional_format}\n').format(admin=admin_roles_str, mod=mod_roles_str, guild_locale=guild_locale, guild_regional_format=guild_regional_format)
        else:
            guild_settings = ''
        ...
        settings = _('{bot_name} Settings:\n\nPrefixes: {prefixes}\n{guild_settings}Global locale: {locale}\nGlobal regional format: {regional_format}\nDefault embed colour: {colour}').format(bot_name=ctx.bot.user.display_name, prefixes=prefix_string, guild_settings=guild_settings, locale=locale, regional_format=regional_format, colour=colour)
    ...
    async def _set_deletedelay(self, ctx: commands.Context, time: int=None):
        ...
        **Arguments:**
        ...
        if time is not None:
            ...
            if delay != -1:
                await ctx.send(_('Bot will delete command messages after {num} seconds. Set this value to -1 to stop deleting messages').format(num=delay))
            else:
    ...
    async def _set_usebotcolour(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        await ctx.send(_('The bot {} use its configured color for embeds.').format(_('will not') if not current_setting else _('will')))
    ...
    async def _set_serverfuzzy(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        await ctx.send(_('Fuzzy command search has been {} for this server.').format(_('disabled') if current_setting else _('enabled')))
    ...
    async def _set_fuzzy(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        await ctx.send(_('Fuzzy command search has been {} in DMs.').format(_('disabled') if current_setting else _('enabled')))
    ...
    async def _set_prefix(self, ctx: commands.Context, *prefixes: str):
        ...
        **Arguments:**
        ...
        if any((prefix.startswith('/') for prefix in prefixes)):
            await ctx.send(_("Prefixes cannot start with '/', as it conflicts with Discord's slash commands."))
            return
        if any((len(x) > MAX_PREFIX_LENGTH for x in prefixes)):
            await ctx.send(_('Warning: A prefix is above the recommended length (25 characters).\nDo you want to continue?') + ' (yes/no)')
            pred = MessagePredicate.yes_or_no(ctx)
            try:
                await self.bot.wait_for('message', check=pred, timeout=30)
            except asyncio.TimeoutError:
    ...
    @commands.admin_or_permissions(manage_guild=True)
    async def _set_serverprefix(self, ctx: commands.Context, server: Optional[discord.Guild], *prefixes: str):
        """
    ...
    async def helpset_showsettings(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        if type(ctx.bot._help_formatter) is commands.help.RedHelpFormatter:
            ...
            message = _('Warning: The default formatter is not in use, these settings may not apply.')
    ...
    async def helpset_resetformatter(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        await ctx.send(_('The help formatter has been reset. This will not prevent cogs from modifying help, you may need to remove a cog if this has been an issue.'))
    ...
    async def helpset_resetsettings(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        await ctx.send(_('The help settings have been reset to their defaults. This may not have an impact when using 3rd party help formatters.'))

    @helpset.command(name='usemenus')
    async def helpset_usemenus(self, ctx: commands.Context, use_menus: Literal['buttons', 'reactions', 'select', 'selectonly', 'disable']):
        """
    ...
    async def helpset_verifyexists(self, ctx: commands.Context, verify: bool=None):
        ...
        **Arguments:**
        ...
        if verify:
            ...
            await ctx.send(_('Help will only verify the existence of help topics via fuzzy help (if enabled).'))
    ...
    async def helpset_tagline(self, ctx: commands.Context, *, tagline: str=None):
        ...
        **Arguments:**
        ...
        if len(tagline) > 2048:
            await ctx.send(_('Your tagline is too long! Please shorten it to be no more than 2048 characters long.'))
            return
    ...
    async def contact(self, ctx: commands.Context, *, message: str):
        ...
        **Arguments:**
        ...
        prefix = re.sub(f'<@!?{ctx.me.id}>', f'@{ctx.me.name}'.replace('\\', '\\\\'), prefixes[0])
        ...
        for destination in destinations:
            ...
            if await ctx.bot.embed_requested(destination, command=ctx.command):
                ...
                try:
                    ...
                    log.exception(f'An unexpected error happened while attempting to send contact to {destination}({destination.id})')
                else:
                    successful = True
            else:
    ...
    async def dm(self, ctx: commands.Context, user_id: int, *, message: str):
        ...
        **Arguments:**
        ...
        if destination is None or destination.bot:
            await ctx.send(_('Invalid ID, user not found, or user is a bot. You can only send messages to people I share a server with.'))
            return
        prefixes = await ctx.bot.get_valid_prefixes()
        prefix = re.sub(f'<@!?{ctx.me.id}>', f'@{ctx.me.name}'.replace('\\', '\\\\'), prefixes[0])
    ...
    @commands.is_owner()
    @commands.command()
    async def diagnoseissues(self, ctx: commands.Context, channel: Optional[Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.Thread]]=commands.CurrentChannel, member: Union[discord.Member, discord.User]=commands.param(), *, command_name: str) -> None:
        """
        ...
        **Arguments:**
        ...
        """
        if ctx.guild is None:
            await ctx.send(_('A text channel, voice channel, stage channel, or thread needs to be passed when using this command in DMs.'))
            return
        ...
        if not can_user_send_messages_in(member, channel):
            await ctx.send(_("Don't try to fool me, the given member can't access the {channel} channel!").format(channel=channel.mention))
            return
        ...
        **Arguments:**
        ...
        if not (ctx.guild.owner == ctx.author or await self.bot.is_owner(ctx.author)):
            ...
            if ids.isdisjoint(theoretical_whitelist):
                return await ctx.send(_('I cannot allow you to do this, as it would remove your ability to run commands, please ensure to add yourself to the allowlist first.'))
        ...
        **Arguments:**
        ...
        if not (ctx.guild.owner == ctx.author or await self.bot.is_owner(ctx.author)):
            ...
            if theoretical_whitelist and ids.isdisjoint(theoretical_whitelist):
                return await ctx.send(_('I cannot allow you to do this, as it would remove your ability to run commands.'))
    ...
    async def command_disable_cog(self, ctx: commands.Context, *, cog: CogConverter):
        ...
        **Arguments:**
        ...
        if await self.bot._disabled_cog_cache.disable_cog_in_guild(cogname, ctx.guild.id):
            ...
            await ctx.send(_('{cogname} was already disabled (nothing to do).').format(cogname=cogname))
    ...
    async def command_enable_cog(self, ctx: commands.Context, *, cogname: str):
        ...
        **Arguments:**
        ...
        if await self.bot._disabled_cog_cache.enable_cog_in_guild(cogname, ctx.guild.id):
            ...
            await ctx.send(_('{cogname} was not disabled (nothing to do).').format(cogname=cogname))
    ...
    async def command_list_disabled_cogs(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        """
        disabled = [cog.qualified_name for cog in self.bot.cogs.values() if await self.bot._disabled_cog_cache.cog_disabled_in_guild(cog.qualified_name, ctx.guild.id)]
        if disabled:
    ...
    async def list_disabled_global(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        if len(disabled_list) > 1:
            header = _('{} commands are disabled globally.\n').format(humanize_number(len(disabled_list)))
        else:
    ...
    async def list_disabled_guild(self, ctx: commands.Context):
        ...
        **Example:**
        ...
        if len(disabled_list) > 1:
            header = _('{} commands are disabled in {}.\n').format(humanize_number(len(disabled_list)), ctx.guild)
        else:
    ...
    async def command_disable_global(self, ctx: commands.Context, *, command: CommandConverter):
        ...
        **Arguments:**
        ...
        if self.command_manager in command.parents or self.command_manager == command:
            await ctx.send(_('The command to disable cannot be `command` or any of its subcommands.'))
            return
        if isinstance(command, commands.commands._RuleDropper):
            await ctx.send(_('This command is designated as being always available and cannot be disabled.'))
            return
    ...
    async def autoimmune_add(self, ctx: commands.Context, *, user_or_role: Union[discord.Member, discord.Role]):
        """
    ...
    async def autoimmune_remove(self, ctx: commands.Context, *, user_or_role: Union[discord.Member, discord.Role]):
        """
    ...
    async def ignore_channel(self, ctx: commands.Context, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.ForumChannel, discord.CategoryChannel, discord.Thread]=commands.CurrentChannel):
        """
    ...
    async def unignore_channel(self, ctx: commands.Context, channel: Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.ForumChannel, discord.CategoryChannel, discord.Thread]=commands.CurrentChannel):
        """
    ...
    async def count_ignored(self, ctx: commands.Context):
        ...
        channels: List[Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.ForumChannel]] = []
        threads: List[discord.Thread] = []
        if await self.bot._ignored_cache.get_ignored_guild(ctx.guild):
            return _('This server is currently being ignored.')
        for channel in itertools.chain(ctx.guild.text_channels, ctx.guild.voice_channels, ctx.guild.stage_channels, ctx.guild.forums):
        ...
        cat_str = humanize_list([c.name for c in category_channels]) if category_channels else _('None')
        ...
        msg = _('Currently ignored categories: {categories}\nChannels: {channels}\nThreads (excluding archived):{threads}').format(categories=cat_str, channels=chan_str, threads=thread_str)
        return msg

    @commands.cooldown(1, 180, lambda ctx: (ctx.message.channel.id, ctx.message.author.id))
    @commands.command(cls=commands.commands._AlwaysAvailableCommand, name='licenseinfo', aliases=['licenceinfo'], i18n=_)
    async def license_info_command(self, ctx):
        """
        Get info about Red's licenses.
        """
        message = 'This bot is an instance of Red-DiscordBot (hereinafter referred to as Red).\nRed is a free and open source application made available to the public and licensed under the GNU GPLv3. The full text of this license is available to you at <https://github.com/Cog-Creators/Red-DiscordBot/blob/V3/develop/LICENSE>.'
