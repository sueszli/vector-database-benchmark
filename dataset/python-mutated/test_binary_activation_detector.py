...
def get_classifier(framework):
    ...
    def _get_classifier():
        ...
        if framework in ('keras', 'kerastf'):
            ...
            model = Sequential()
            model.add(layers.MaxPooling2D(pool_size=(2, 2), input_shape=(22, 22, 1)))
            ...
            classifier = TensorFlowV2Classifier(model, nb_classes=2, input_shape=(22, 22, 1), loss_object=loss_object, optimizer=optimizer)
        elif framework == 'pytorch':
            import torch
            model = torch.nn.Sequential(torch.nn.MaxPool2d(2, 2), torch.nn.Flatten(), torch.nn.Linear(121, 2))
            criterion = torch.nn.CrossEntropyLoss()
            optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
            classifier = PyTorchClassifier(model, loss=criterion, optimizer=optimizer, input_shape=(1, 22, 22), nb_classes=2)
        else:
            classifier = None
        return classifier
...
def test_binary_activation_detector(art_warning, get_default_mnist_subset, image_dl_estimator, get_classifier):
    if False:
        print('Hello World!')
    ...
    try:
        detector = BinaryActivationDetector(classifier=classifier, detector=detector_classifier, layer=0)
