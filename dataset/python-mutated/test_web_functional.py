...
import aiohttp
from aiohttp import FormData, HttpVersion, HttpVersion10, HttpVersion11, TraceConfig, multipart, web
...
async def test_handler_returns_not_response(aiohttp_server: Any, aiohttp_client: Any) -> None:
    ...
    with pytest.raises(aiohttp.ServerDisconnectedError):
        await client.get('/')
    logger.exception.assert_called_with('Unhandled runtime exception', exc_info=mock.ANY)
...
async def test_files_upload_with_same_key(aiohttp_client: Any) -> None:
    ...
    client = await aiohttp_client(app)
    data = FormData()
    data.add_field('file', b'binary data 1', content_type='image/jpeg', filename='test1.jpeg')
    data.add_field('file', b'binary data 2', content_type='image/jpeg', filename='test2.jpeg')
    resp = await client.post('/', data=data)
    assert 200 == resp.status
    await resp.release()
...
async def test_empty_content_for_query_without_body(method: Any, aiohttp_client: Any) -> None:
...
async def test_response_with_async_gen_no_params(aiohttp_client: Any, fname: Any) -> None:
    with fname.open('rb') as f:
        data = f.read()
    data_size = len(data)
...
async def test_response_with_file(aiohttp_client: Any, fname: Any) -> None:
    ...
    assert resp_data == data
    assert resp.headers.get('Content-Type') in ('application/octet-stream', 'text/x-python', 'text/plain')
    assert resp.headers.get('Content-Length') == str(len(resp_data))
...
async def test_response_with_file_ctype(aiohttp_client: Any, fname: Any) -> None:
    ...
    async def handler(request):
        nonlocal outer_file_descriptor
        outer_file_descriptor = fname.open('rb')
        return web.Response(body=outer_file_descriptor, headers={'content-type': 'text/binary'})
    app = web.Application()
    app.router.add_get('/', handler)
    client = await aiohttp_client(app)
    resp = await client.get('/')
...
@pytest.mark.parametrize('compressor,encoding', [(zlib.compressobj(wbits=16 + zlib.MAX_WBITS), 'gzip'), (zlib.compressobj(wbits=zlib.MAX_WBITS), 'deflate'), (zlib.compressobj(wbits=-zlib.MAX_WBITS), 'deflate')])
async def test_response_with_precompressed_body(aiohttp_client: Any, compressor: Any, encoding: Any) -> None:
...
async def test_subapp_reverse_variable_url(aiohttp_client: Any) -> None:
    ...
    async def handler(request):
        raise web.HTTPMovedPermanently(location=subapp.router['name'].url_for(part='final'))
...
async def test_subapp_reverse_static_url(aiohttp_client: Any) -> None:
    ...
    async def handler(request):
        raise web.HTTPMovedPermanently(location=subapp.router['name'].url_for(filename=fname))
    app = web.Application()
    subapp = web.Application()
    subapp.router.add_get('/to', handler)
    here = pathlib.Path(__file__).parent
...
async def test_old_style_subapp_middlewares(aiohttp_client: Any) -> None:
    ...
    assert resp.status == 200
    assert [(1, 'app'), (1, 'subapp1'), (1, 'subapp2'), (2, 'subapp2'), (2, 'subapp1'), (2, 'app')] == order
    await resp.release()
...
@pytest.mark.parametrize('route,expected,middlewares', [('/sub/', ['A: root', 'C: sub', 'D: sub'], 'AC'), ('/', ['A: root', 'B: root'], 'AC'), ('/sub/', ['A: root', 'D: sub'], 'A'), ('/', ['A: root', 'B: root'], 'A'), ('/sub/', ['C: sub', 'D: sub'], 'C'), ('/', ['B: root'], 'C'), ('/sub/', ['D: sub'], ''), ('/', ['B: root'], '')])
async def test_subapp_middleware_context(aiohttp_client: Any, route: Any, expected: Any, middlewares: Any):
    values = []
...
async def test_post_max_client_size(aiohttp_client: Any) -> None:
    ...
    with io.BytesIO(b'test') as file_handle:
        ...
        assert 413 == resp.status
        resp_text = await resp.text()
        assert 'Maximum request body size 10 exceeded, actual body size 1024' in resp_text
        data['file'].close()
        await resp.release()
...
async def test_await(aiohttp_server: Any) -> None:
    ...
    async def handler(request):
        ...
        await resp.prepare(request)
        with pytest.deprecated_call(match='^drain method is deprecated, use await resp\\.write\\(\\)$'):
            await resp.drain()
...
async def test_context_manager_close_on_release(aiohttp_server: Any, mocker: Any) -> None:
    ...
    class FakeResolver:
        ...
        async def resolve(self, host, port=0, family=socket.AF_INET):
            ...
            if fake_port is not None:
                return [{'hostname': host, 'host': self._LOCAL_HOST[family], 'port': fake_port, 'family': socket.AF_INET, 'proto': 0, 'flags': socket.AI_NUMERICHOST}]
            else:
...
@pytest.mark.parametrize('auto_decompress,len_of', [(True, 'uncompressed'), (False, 'compressed')])
async def test_auto_decompress(aiohttp_client: Any, auto_decompress: bool, len_of: str) -> None:
...
async def test_response_101_204_no_content_length_http11(status: Any, aiohttp_client: Any) -> None:
...
async def test_no_body_for_1xx_204_304_responses(aiohttp_client: Any, status: int, version: HttpVersion) -> None:
