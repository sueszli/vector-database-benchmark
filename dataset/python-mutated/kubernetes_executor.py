...
try:
    from airflow.cli.cli_config import ARG_DAG_ID, ARG_EXECUTION_DATE, ARG_OUTPUT_PATH, ARG_SUBDIR, ARG_VERBOSE, ActionCommand, Arg, GroupCommand, lazy_load_command, positive_int
except ImportError:
    try:
    ...
    if packaging.version.parse(base_version) < packaging.version.parse('2.7.0'):
        raise AirflowOptionalProviderFeatureException(f"Kubernetes Executor from CNCF Provider should only be used with Airflow 2.7.0+.\nThis is Airflow {airflow_version} and Kubernetes and CeleryKubernetesExecutor are available in the 'airflow.executors' package. You should not use the provider's executors in this version of Airflow.")
    raise
...
if TYPE_CHECKING:
    ...
    from airflow.providers.cncf.kubernetes.executors.kubernetes_executor_types import KubernetesJobType, KubernetesResultsType
