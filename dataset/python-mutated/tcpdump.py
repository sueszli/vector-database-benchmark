...
async def once_tcpdump(interface, src_ips, src_ports, dest_ips, dest_ports, display, stop_event):
    ...
    while not stop_event.is_set():
        ...
        if not green_light:
            continue
        results = [f'[{interface}][{local_now_display()}] {src_ip}:{src_port} -> {dest_ip}:{dest_port} ({packet_type}, {packet_len} bytes)', f'\tFlags: {flags} Seq: {seq}, Ack: {ack}', f'\tData: {data}']
        for r in results:
            await display(r)
...
async def verbose_tcpdump(interfaces, src_ips, src_ports, dest_ips, dest_ports, display=None):
    ...
    summary = [f'[Summary] Tcpdump filter info: ', f'Interface: [{list_show(valid_interface)}]', f'Source address: [{list_show(src_ips)}]', f'source port: [{list_show(src_ports)}]', f'Destination address: [{list_show(dest_ips)}]', f'Destination port: [{list_show(dest_ports)}]']
    for s in summary:
        await display(s)
    params = [src_ips, src_ports, dest_ips, dest_ports, display, stop_event]
    tasks = [asyncio.create_task(once_tcpdump(i, *params)) for i in valid_interface]
