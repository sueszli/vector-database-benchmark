...
async def test_SocketListener() -> None:
    ...
    with tsocket.socket(type=tsocket.SOCK_DGRAM) as s:
        ...
        with pytest.raises(ValueError) as excinfo:
            SocketListener(s)
        excinfo.match('.*SOCK_STREAM')
    if sys.platform != 'darwin':
        with tsocket.socket() as s:
            await s.bind(('127.0.0.1', 0))
            with pytest.raises(ValueError) as excinfo:
                SocketListener(s)
            excinfo.match('.*listen')
...
async def test_SocketListener_accept_errors() -> None:
    ...
    fake_listen_sock = FakeSocket([OSError(errno.ECONNABORTED, 'Connection aborted'), OSError(errno.EPERM, 'Permission denied'), OSError(errno.EPROTO, 'Bad protocol'), fake_server_sock, OSError(errno.EMFILE, 'Out of file descriptors'), OSError(errno.EFAULT, 'attempt to write to read-only memory'), OSError(errno.ENOBUFS, 'out of buffers'), fake_server_sock])
