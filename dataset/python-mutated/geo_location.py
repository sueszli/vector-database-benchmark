...
ICONS = {'DR': 'mdi:water-off', 'EQ': 'mdi:pulse', 'FL': 'mdi:home-flood', 'TC': 'mdi:weather-hurricane', 'TS': 'mdi:waves', 'VO': 'mdi:image-filter-hdr'}
PARALLEL_UPDATES = 0
SOURCE = 'gdacs'
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    manager.listeners.append(async_dispatcher_connect(hass, manager.async_event_new_entity(), async_add_geolocation))
...
class GdacsEvent(GeolocationEvent):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self._remove_signal_delete = async_dispatcher_connect(self.hass, f'gdacs_delete_{self._external_id}', self._delete_callback)
        self._remove_signal_update = async_dispatcher_connect(self.hass, f'gdacs_update_{self._external_id}', self._update_callback)
