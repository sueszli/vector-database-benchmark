...
class ProductTypeInput(BaseInputObjectType):
    ...
    has_variants = graphene.Boolean(description='Determines if product of this type has multiple variants. This option mainly simplifies product management in the dashboard. There is always at least one variant created under the hood.')
    product_attributes = NonNullList(graphene.ID, description='List of attributes shared among all product variants.', name='productAttributes')
    variant_attributes = NonNullList(graphene.ID, description='List of attributes used to distinguish between different variants of a product.', name='variantAttributes')
    is_shipping_required = graphene.Boolean(description='Determines if shipping is required for products of this variant.')
    is_digital = graphene.Boolean(description='Determines if products are digital.', required=False)
    weight = WeightScalar(description='Weight of the ProductType items.')
    tax_code = graphene.String(description=f'Tax rate for enabled tax gateway. {DEPRECATED_IN_3X_INPUT}. Use tax classes to control the tax calculation for a product type. If taxCode is provided, Saleor will try to find a tax class with given code (codes are stored in metadata) and assign it. If no tax class is found, it would be created and assigned.')
    tax_class = graphene.ID(description="ID of a tax class to assign to this product type. All products of this product type would use this tax class, unless it's overridden in the `Product` type.", required=False)
...
class ProductTypeCreate(ModelMutation):
    ...
    class Arguments:
        input = ProductTypeInput(required=True, description='Fields required to create a product type.')
