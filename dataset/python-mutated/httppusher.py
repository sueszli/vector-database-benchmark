...
http_push_processed_counter = Counter('synapse_http_httppusher_http_pushes_processed', 'Number of push notifications successfully sent')
http_push_failed_counter = Counter('synapse_http_httppusher_http_pushes_failed', 'Number of push notifications which failed')
http_badges_processed_counter = Counter('synapse_http_httppusher_badge_updates_processed', 'Number of badge updates successfully sent')
http_badges_failed_counter = Counter('synapse_http_httppusher_badge_updates_failed', 'Number of badge updates which failed')
...
class HttpPusher(Pusher):
    ...
    async def _update_badge(self) -> None:
        badge = await push_tools.get_badge_count(self.hs.get_datastores().main, self.user_id, group_by_room=self._group_unread_count_by_room)
    ...
    async def _unsafe_process(self) -> None:
        ...
        run once per pusher.
        """
        unprocessed = await self.store.get_unread_push_actions_for_user_in_range_for_http(self.user_id, self.last_stream_ordering, self.max_stream_ordering)
        logger.info('Processing %i unprocessed push actions for %s starting at stream_ordering %s', len(unprocessed), self.name, self.last_stream_ordering)
        for push_action in unprocessed:
            with opentracing.start_active_span('http-push', tags={'authenticated_entity': self.user_id, 'event_id': push_action.event_id, 'app_id': self.app_id, 'app_display_name': self.app_display_name}):
                processed = await self._process_one(push_action)
            if processed:
                http_push_processed_counter.inc()
                self.backoff_delay = HttpPusher.INITIAL_BACKOFF_SEC
                self.last_stream_ordering = push_action.stream_ordering
                pusher_still_exists = await self.store.update_pusher_last_stream_ordering_and_success(self.app_id, self.pushkey, self.user_id, self.last_stream_ordering, self.clock.time_msec())
                if not pusher_still_exists:
                    self.on_stop()
                    return
                if self.failing_since:
                    self.failing_since = None
                    await self.store.update_pusher_failing_since(self.app_id, self.pushkey, self.user_id, self.failing_since)
            else:
                http_push_failed_counter.inc()
                if not self.failing_since:
                    self.failing_since = self.clock.time_msec()
                    await self.store.update_pusher_failing_since(self.app_id, self.pushkey, self.user_id, self.failing_since)
                if self.failing_since and self.failing_since < self.clock.time_msec() - HttpPusher.GIVE_UP_AFTER_MS:
                    logger.warning('Giving up on a notification to user %s, pushkey %s', self.user_id, self.pushkey)
                    self.backoff_delay = HttpPusher.INITIAL_BACKOFF_SEC
                    self.last_stream_ordering = push_action.stream_ordering
                    await self.store.update_pusher_last_stream_ordering(self.app_id, self.pushkey, self.user_id, self.last_stream_ordering)
                    self.failing_since = None
                    await self.store.update_pusher_failing_since(self.app_id, self.pushkey, self.user_id, self.failing_since)
                else:
                    logger.info('Push failed: delaying for %ds', self.backoff_delay)
                    self.timed_call = self.hs.get_reactor().callLater(self.backoff_delay, self.on_timer)
    ...
    async def dispatch_push(self, content: JsonDict, tweaks: Optional[JsonMapping]=None, default_payload: Optional[JsonMapping]=None) -> Union[bool, List[str]]:
        ...
        device = {'app_id': self.app_id, 'pushkey': self.pushkey, 'pushkey_ts': int(self.pushkey_ts / 1000), 'data': data}
        if tweaks:
            device['tweaks'] = tweaks
        content['devices'] = [device]
        try:
            resp = await self.http_client.post_json_get_json(self.url, {'notification': content})
        except Exception as e:
            logger.warning('Failed to push data to %s: %s %s', self.name, type(e), e)
            return False
        rejected = []
        if 'rejected' in resp:
    ...
    async def dispatch_push_event(self, event: EventBase, tweaks: JsonMapping, badge: int) -> Union[bool, List[str]]:
        ...
        """
        priority = 'low'
        if event.type == EventTypes.Encrypted or tweaks.get('highlight') or tweaks.get('sound'):
            priority = 'high'
        assert self.data is not None
        if self.data.get('format') == 'event_id_only':
            content: JsonDict = {'event_id': event.event_id, 'room_id': event.room_id, 'counts': {'unread': badge}, 'prio': priority}
            tweaks = {}
        else:
            ctx = await push_tools.get_context_for_event(self._storage_controllers, event, self.user_id)
            content = {'id': event.event_id, 'event_id': event.event_id, 'room_id': event.room_id, 'type': event.type, 'sender': event.user_id, 'prio': priority, 'counts': {'unread': badge}}
    ...
    async def _send_badge(self, badge: int) -> None:
        ...
        d = {'notification': {'id': '', 'type': None, 'sender': '', 'counts': {'unread': badge}, 'devices': [{'app_id': self.app_id, 'pushkey': self.pushkey, 'pushkey_ts': int(self.pushkey_ts / 1000), 'data': self.data_minus_url}]}}
        try:
            await self.http_client.post_json_get_json(self.url, d)
            http_badges_processed_counter.inc()
        except Exception as e:
            logger.warning('Failed to send badge count to %s: %s %s', self.name, type(e), e)
