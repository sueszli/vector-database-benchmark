...
async def _read_headers(input_channel: connections.AsyncTextReader) -> List[str]:
    headers = []
...
async def _try_read_json_rpc(input_channel: connections.AsyncTextReader) -> json_rpc.Request:
    """
    ...
    try:
        ...
        if len(error.partial) == 0:
            raise ReadChannelClosedError('Trying to read from a closed input channel') from None
        else:
            raise json_rpc.ParseError(str(error)) from None

async def read_json_rpc(input_channel: connections.AsyncTextReader) -> json_rpc.Request:
    """
...
async def write_json_rpc(output_channel: connections.AsyncTextWriter, response: json_rpc.JSONRPC) -> None:
    """
    Asynchronously write a JSON-RPC response to the given output channel.
    """
    await output_channel.write(json_rpc_payload(response))

async def write_json_rpc_ignore_connection_error(output_channel: connections.AsyncTextWriter, response: json_rpc.JSONRPC) -> None:
    """
...
@dataclasses.dataclass(frozen=True)
class WorkspaceDidChangeConfigurationParameters(json_mixins.CamlCaseAndExcludeJsonMixin):
...
class CallHierarchyIncomingCall(json_mixins.CamlCaseAndExcludeJsonMixin):
    from_: CallHierarchyItem = field(metadata=dataclasses_json.config(field_name='from'))
