...
class InstrumentedAsyncServer:
    ...
    async def admin_connect(self, sid, environ, client_auth):
        ...
        if self.auth:
            ...
            elif asyncio.iscoroutinefunction(self.auth):
        ...
        async def config(sid):
            ...
            if not self.read_only:
                features += ['EMIT', 'JOIN', 'LEAVE', 'DISCONNECT', 'MJOIN', 'MLEAVE', 'MDISCONNECT']
            if self.mode == 'development':
                features.append('ALL_EVENTS')
            await self.sio.emit('config', {'supportedFeatures': features}, to=sid, namespace=self.admin_namespace)
            if self.mode == 'development':
                all_sockets = []
                ...
                await self.sio.emit('all_sockets', all_sockets, to=sid, namespace=self.admin_namespace)
    ...
    async def _connect(self, eio_sid, namespace):
        ...
        await self.sio.emit('socket_connected', (serialized_socket, datetime.utcfromtimestamp(t).isoformat() + 'Z'), namespace=self.admin_namespace)
        return sid

    async def _disconnect(self, sid, namespace, **kwargs):
        del self.sio.manager._timestamps[sid]
        await self.sio.emit('socket_disconnected', (namespace, sid, 'N/A', datetime.utcnow().isoformat() + 'Z'), namespace=self.admin_namespace)
        return await self.sio.manager.__disconnect(sid, namespace, **kwargs)

    async def _check_for_upgrade(self, eio_sid, sid, namespace):
        for _ in range(5):
            await self.sio.sleep(5)
            try:
                if self.sio.eio._get_socket(eio_sid).upgraded:
                    await self.sio.emit('socket_updated', {'id': sid, 'nsp': namespace, 'transport': 'websocket'}, namespace=self.admin_namespace)
                    break
            except KeyError:
                pass
    ...
    async def _emit(self, event, data, namespace, room=None, skip_sid=None, callback=None, **kwargs):
        ret = await self.sio.manager.__emit(event, data, namespace, room=room, skip_sid=skip_sid, callback=callback, **kwargs)
        if namespace != self.admin_namespace:
            ...
            for (sid, _) in self.sio.manager.get_participants(namespace, room):
                if sid not in skip_sid:
                    await self.sio.emit('event_sent', (namespace, sid, event_data, datetime.utcnow().isoformat() + 'Z'), namespace=self.admin_namespace)
        return ret

    async def _handle_event_internal(self, server, sid, eio_sid, data, namespace, id):
        ret = await self.sio.__handle_event_internal(server, sid, eio_sid, data, namespace, id)
        await self.sio.emit('event_received', (namespace, sid, data, datetime.utcnow().isoformat() + 'Z'), namespace=self.admin_namespace)
        return ret
