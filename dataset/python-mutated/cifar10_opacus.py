    ...
    for (i, (images, target)) in enumerate(tqdm(train_loader)):
        ...
        if i % args.print_freq == 0:
            if not args.disable_dp:
                (epsilon, best_alpha) = privacy_engine.accountant.get_privacy_spent(delta=args.delta, alphas=[1 + x / 10.0 for x in range(1, 100)] + list(range(12, 64)))
                print(f'\tTrain Epoch: {epoch} \tLoss: {np.mean(losses):.6f} Acc@1: {np.mean(top1_acc):.6f} (ε = {epsilon:.2f}, δ = {args.delta}) for α = {best_alpha}')
            else:
...
def main():
    ...
    if not args.disable_dp:
        ...
        (model, optimizer, train_loader) = privacy_engine.make_private(module=model, optimizer=optimizer, data_loader=train_loader, noise_multiplier=args.sigma, max_grad_norm=max_grad_norm, clipping=clipping)
    accuracy_per_epoch = []
    time_per_epoch = []
...
def parse_args():
    ...
    parser.add_argument('--device', type=str, default='cuda', help='Device on which to run the code.')
