...
class CategoricalPassthroughEncoderConfig(BaseEncoderConfig):
    ...
    type: str = schema_utils.ProtectedString('passthrough', description=ENCODER_METADATA['PassthroughEncoder']['type'].long_description)
...
class CategoricalEmbedConfig(BaseEncoderConfig):
    ...
    type: str = schema_utils.ProtectedString('dense', description=ENCODER_METADATA['CategoricalEmbed']['type'].long_description)
...
class CategoricalSparseConfig(BaseEncoderConfig):
    ...
    type: str = schema_utils.ProtectedString('sparse', description=ENCODER_METADATA['CategoricalSparse']['type'].long_description)
    dropout: float = common_fields.DropoutField()
    vocab: List[str] = common_fields.VocabField()
    embedding_initializer: str = common_fields.EmbeddingInitializerField()
    embeddings_on_cpu: bool = common_fields.EmbeddingsOnCPUField()
...
class CategoricalOneHotEncoderConfig(BaseEncoderConfig):
    ...
    encoder."""
    type: str = schema_utils.ProtectedString('onehot', description='Type of encoder.')
    vocab: List[str] = common_fields.VocabField()
