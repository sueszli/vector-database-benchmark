...
class rds_snapshots_public_access(Check):
    ...
    def execute(self):
        ...
        for db_snap in rds_client.db_snapshots:
            ...
            if db_snap.public:
                report.status = 'FAIL'
                report.status_extended = f'RDS Instance Snapshot {db_snap.id} is public.'
            else:
                report.status = 'PASS'
                report.status_extended = f'RDS Instance Snapshot {db_snap.id} is not shared.'
        ...
        for db_snap in rds_client.db_cluster_snapshots:
            ...
            if db_snap.public:
                ...
                report.status = 'PASS'
                report.status_extended = f'RDS Cluster Snapshot {db_snap.id} is not shared.'
            findings.append(report)
        return findings