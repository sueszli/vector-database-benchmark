...
with DAG(DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), catchup=False, tags=['example', 'gdrive']) as dag:
    ...
    upload_single_file = LocalFilesystemToGoogleDriveOperator(gcp_conn_id=CONNECTION_ID, task_id='upload_single_file', local_paths=SINGLE_FILE_LOCAL_PATHS, drive_folder=DRIVE_FOLDER)
    upload_multiple_files = LocalFilesystemToGoogleDriveOperator(gcp_conn_id=CONNECTION_ID, task_id='upload_multiple_files', local_paths=MULTIPLE_FILES_LOCAL_PATHS, drive_folder=DRIVE_FOLDER, ignore_if_missing=True)
    ...
    def remove_files_from_drive():
        ...
        root_path = service.files().list(q=f"name = '{DRIVE_FOLDER}' and mimeType = 'application/vnd.google-apps.folder'").execute()
    ...
    create_temp_gcp_connection_task >> upload_single_file >> upload_multiple_files >> remove_files_from_drive_task >> delete_temp_gcp_connection_task
