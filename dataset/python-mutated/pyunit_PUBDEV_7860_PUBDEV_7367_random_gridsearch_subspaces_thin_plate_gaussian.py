...
class test_random_gam_gridsearch_specific:
    ...
    hyper_parameters = {'lambda': [100, 200], 'subspaces': [{'scale': [[1, 1, 1], [2, 2, 2]], 'bs': [[1, 1, 1]], 'gam_columns': [[['C11', 'C12'], ['C13', 'C14'], ['C15', 'C16']]]}, {'scale': [[1, 1], [2, 2]], 'bs': [[1, 1]], 'gam_columns': [[['C11'], ['C14']]]}]}
    manual_gam_models = []
    num_grid_models = 0
                ...
                for scale in subspace['scale']:
                    for gam_columns in subspace['gam_columns']:
                        for bsVal in subspace['bs']:
                            ...
                            self.manual_gam_models.append(H2OGeneralizedAdditiveEstimator(family='gaussian', gam_columns=gam_columns, scale=scale, bs=bsVal, lambda_=lambda_, seed=1234))

    def train_models(self):
        if False:
            i = 10
            return i + 15
        self.h2o_model = H2OGridSearch(H2OGeneralizedAdditiveEstimator(family='gaussian', keep_gam_cols=True, seed=1234), hyper_params=self.hyper_parameters, search_criteria=self.search_criteria)
