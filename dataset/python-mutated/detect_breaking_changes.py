...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--target', dest='target_package', help='The target package directory on disk. The target module passed to will be <target_package>/azure.', required=True)
    parser.add_argument('-m', '--module', dest='target_module', help='The target module. The target module passed will be the top most module in the package')
    parser.add_argument('-v', '--in-venv', dest='in_venv', help='Check if we are in the newly created venv.', default=False)
    parser.add_argument('-s', '--stable_version', dest='stable_version', help='The stable version of the target package, if it exists on PyPi.', default=None)
    ...
    if package_name not in RUN_BREAKING_CHANGES_PACKAGES:
        _LOGGER.info(f'{package_name} opted out of breaking changes checks. See http://aka.ms/azsdk/breaking-changes-tool to opt-in.')
        exit(0)
