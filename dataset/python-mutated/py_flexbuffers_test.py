...
GOLD_FLEXBUFFER_OBJ = {'bar': [1, 2, 3], 'bar3': [1, 2, 3], 'bool': True, 'bools': [True, False, True, False], 'foo': 100.0, 'mymap': {'foo': 'Fred'}, 'vec': [-100, 'Fred', 4.0, b'M', False, 4.0]}
...
def encode_type(type_, value, byte_width=None):
    ...
    if byte_width:
        ...
        add(value)
    return fbb.Finish()
INT_MIN_MAX_VALUES = (min_value(Type.INT, 1), max_value(Type.INT, 1), min_value(Type.INT, 2), max_value(Type.INT, 2), min_value(Type.INT, 4), max_value(Type.INT, 4), min_value(Type.INT, 8), max_value(Type.INT, 8))
UINT_MIN_MAX_VALUES = (0, max_value(Type.UINT, 1), max_value(Type.UINT, 2), max_value(Type.UINT, 4), max_value(Type.UINT, 8))
