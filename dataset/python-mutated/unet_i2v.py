...
class Img2VidSDUNet(nn.Module):
    ...
    def __init__(self, in_dim=7, dim=512, y_dim=512, num_tokens=4, context_dim=512, out_dim=6, dim_mult=[1, 2, 3, 4], num_heads=None, head_dim=64, num_res_blocks=3, attn_scales=[1 / 2, 1 / 4, 1 / 8], use_scale_shift_norm=True, dropout=0.1, default_fps=8, temporal_attn_times=1, temporal_attention=True, use_checkpoint=False, use_image_dataset=False, use_sim_mask=False, training=True, inpainting=True, **kwargs):
        ...
        self.context_embedding = nn.Sequential(nn.Linear(y_dim, embed_dim), nn.SiLU(), nn.Linear(embed_dim, context_dim * self.num_tokens))
    ...
    def forward(self, x, t, y, fps=None, video_mask=None, focus_present_mask=None, prob_focus_present=0.0, mask_last_frame_num=0, **kwargs):
        if False:
            for i in range(10):
                print('nop')
        ...
        self.batch = batch
        if fps is None:
            fps = torch.tensor([cfg.default_fps] * batch, dtype=torch.long, device=device)
