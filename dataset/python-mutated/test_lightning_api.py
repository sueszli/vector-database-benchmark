...
from lightning.app.core.api import UIRefresher, fastapi_service, global_app_state_store, register_global_routes, start_server
...
async def test_start_server(x_lightning_type, monkeypatch):
    ...
    ui_refresher = start_server(publish_state_queue, change_state_queue, api_response_queue, has_started_queue=has_started_queue, uvicorn_run=False, spec=spec)
    ...
    async with AsyncClient(app=fastapi_service, base_url='http://test') as client:
        ...
        with pytest.raises(Exception, match='X-Lightning-Session-ID'):
            await client.post('/api/v1/state', headers={'X-Lightning-Session-UUID': headers['X-Lightning-Session-UUID']})
        response = await client.post('/api/v1/state', json={'stage': 'running'}, headers=headers)
        assert change_state_queue._queue[0].to_dict() == {'values_changed': {"root['app_state']['stage']": {'new_value': 'running'}}}
        assert response.status_code == 200
        response = await client.get('/api/v1/layout')
        assert json.loads(response.json()) == [{'name': 'main_1', 'content': 'https://te', 'target': 'https://te'}, {'name': 'main_2', 'content': 'https://te'}, {'name': 'main_3', 'content': 'https://te'}, {'name': 'main_4', 'content': 'https://te'}]
        ...
        assert change_state_queue._queue[1].to_dict() == {'values_changed': {"root['vars']['counter']": {'new_value': 1}}}
        assert response.status_code == 200
        response = await client.post('/api/v1/delta', json={'delta': {'values_changed': {"root['flows']['video_search']['vars']['should_process']": {'new_value': True}}}}, headers=headers)
        assert change_state_queue._queue[2].to_dict() == {'values_changed': {"root['flows']['video_search']['vars']['should_process']": {'new_value': True}}}
        assert response.status_code == 200
...
@pytest.mark.parametrize(('path', 'expected_status_code'), [('/api/v1', 404), ('/api/v1/asdf', 404), ('/api/asdf', 404), ('/api', 404)])
@pytest.mark.anyio()
...
@pytest.mark.skipif(check_if_redis_running(), reason='this is testing the failure condition for which the redis should not run')
@pytest.mark.anyio()
async def test_health_endpoint_failure(monkeypatch):
    monkeypatch.setenv('LIGHTNING_APP_STATE_URL', 'http://someurl')
    monkeypatch.setitem(os.environ, 'LIGHTNING_CLOUD_QUEUE_TYPE', 'redis')
    async with AsyncClient(app=fastapi_service, base_url='http://test') as client:
        response = await client.get('/healthz')
        assert response.status_code == 500

@pytest.mark.parametrize(('path', 'expected_status_code'), [('/', 200), ('/asdf', 200), ('/view/component_a', 200)])
@pytest.mark.anyio()
...
async def test_get_annotations(tmpdir):
    ...
    try:
        ...
        api_response_queue = _MockQueue('api_response_queue')
        spec = extract_metadata_from_app(app)
        start_server(None, change_state_queue, api_response_queue, has_started_queue=has_started_queue, uvicorn_run=False, spec=spec)
        async with AsyncClient(app=fastapi_service, base_url='http://test') as client:
