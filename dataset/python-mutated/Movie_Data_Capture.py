        ...
        while True:
            try:
                ...
                if all((isinstance(v, int) for v in 分析结果元组)):
                    ...
                    print(f'All movies:{扫描电影数}  processed:{已处理}  successes:{完成数}  remain:{剩余个数}' + '  Elapsed time {}'.format(period(总用时, '{d} day {h}:{m:02}:{s:02}') if 总用时.days == 1 else period(总用时, '{d} days {h}:{m:02}:{s:02}') if 总用时.days > 1 else period(总用时, '{h}:{m:02}:{s:02}')))
                    if 剩余个数 == 0:
                        break
