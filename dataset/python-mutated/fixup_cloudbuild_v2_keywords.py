...
class cloudbuildCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'batch_create_repositories': ('parent', 'requests'), 'create_connection': ('parent', 'connection', 'connection_id'), 'create_repository': ('parent', 'repository', 'repository_id'), 'delete_connection': ('name', 'etag', 'validate_only'), 'delete_repository': ('name', 'etag', 'validate_only'), 'fetch_git_refs': ('repository', 'ref_type'), 'fetch_linkable_repositories': ('connection', 'page_size', 'page_token'), 'fetch_read_token': ('repository',), 'fetch_read_write_token': ('repository',), 'get_connection': ('name',), 'get_repository': ('name',), 'list_connections': ('parent', 'page_size', 'page_token'), 'list_repositories': ('parent', 'page_size', 'page_token', 'filter'), 'update_connection': ('connection', 'update_mask', 'allow_missing', 'etag')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=cloudbuildCallTransformer()):
    if False:
        while True:
            i = 10
