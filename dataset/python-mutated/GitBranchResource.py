...
class GitBranchResource(GenericResource):
    ...
    async def member(self, pk, user, **kwargs):
        ...
        if 'with_basic_details' == display_format:
            return self(dict(files={}, is_git_integration_enabled=preferences.is_git_integration_enabled(), modified_files=[], name=git_manager.current_branch, staged_files=[], sync_config=get_preferences().sync_config, untracked_files=[]), user, **kwargs)
        ...
        return self(dict(files={}, is_git_integration_enabled=preferences.is_git_integration_enabled(), modified_files=modified_files, name=git_manager.current_branch, staged_files=staged_files, sync_config=get_preferences().sync_config, untracked_files=untracked_files), user, **kwargs)
    ...
    async def update(self, payload, **kwargs):
        ...
        if action_type == 'status':
            ...
            untracked_files = await git_manager.untracked_files()
            modified_files = git_manager.modified_files
            self.model = dict(name=git_manager.current_branch, status=status, untracked_files=untracked_files, modified_files=modified_files)
        elif action_type == 'commit':
            if not message:
                error = ApiError.RESOURCE_ERROR
                error.update({'message': 'Message is empty, please add a message for your commit.'})
            ...
            if push and 'remote' in push and ('branch' in push):
                ...
                try:
                    ...
                    if token:
                        custom_progress = api.push(remote.name, url, push['branch'], token)
                        ...
                        self.model['error'] = 'Please authenticate with GitHub before trying to push.'
            ...
            if pull and 'remote' in pull:
                ...
                try:
                    ...
                    if token:
                        ...
                        if branch_name:
                            custom_progress = api.pull(remote.name, url, pull.get('branch'), token)
                        else:
                            custom_progress = api.fetch(remote.name, url, token)
                        ...
                        self.model['error'] = 'Please authenticate with GitHub before trying to pull.'
                except GitCommandError as err:
                    self.model['error'] = str(err)
            else:
                git_manager.pull()
            ...
            if fetch and 'remote' in fetch:
                ...
                try:
                    ...
                    if token:
                        ...
                        self.model['error'] = 'Please authenticate with GitHub before trying to fetch.'
                except GitCommandError as err:
                    self.model['error'] = str(err)
            else:
            ...
            if files and len(files) >= 1:
                ...
                if reset and 'remote' in reset:
                    ...
                    try:
                        ...
                        url = list(remote.urls)[0]
                        if token:
                            api.reset_hard(remote.name, url, branch_name, token)
                        else:
                            self.model['error'] = 'Please authenticate with GitHub before trying to pull.'
                    except GitCommandError as err:
                        self.model['error'] = str(err)
            ...
            if clone and 'remote' in clone:
                ...
                try:
                    ...
                    url = list(remote.urls)[0]
                    if token:
                        api.clone(remote.name, url, token)
                    else:
                        self.model['error'] = 'Please authenticate with GitHub before trying to pull.'
                except GitCommandError as err:
                    self.model['error'] = str(err)
            ...
            if not remote:
                error = ApiError.RESOURCE_ERROR
                error.update({'message': 'Remote is empty, please add a remote.'})
                raise ApiError(error)
            args = []
            keys = ['name']
            ...
            for key in keys:
                ...
                if not val:
                    error = ApiError.RESOURCE_ERROR
                    error.update({'message': f'Remote is missing {key}.'})
                    raise ApiError(error)
                args.append(val)
            if action_type == 'add_remote':
                if remote.get('name') == REMOTE_NAME:
                    error = ApiError.RESOURCE_ERROR
                    error.update({'message': f'Remote name {REMOTE_NAME} is reserved, ' + 'please select a different name.'})
                    raise ApiError(error)
                git_manager.add_remote(*args)
            elif action_type == 'remove_remote':
                git_manager.remove_remote(*args)
        elif action_type in ['delete', 'merge', 'rebase']:
            data = payload.get('delete', None) or payload.get('merge', None) or payload.get('rebase', None)
            if data and 'base_branch' in data:
                ...
                error = ApiError.RESOURCE_ERROR
                error.update({'message': 'Please select a base branch to ' + f'{action_type} into the current branch.'})
                raise ApiError(error)
        return self

    async def files(self, modified_files: List[str], staged_files: List[str], untracked_files: List[str], limit: int=None) -> Dict:
        arr = []
