...
from ray._private.test_utils import enable_external_redis, find_free_port, generate_system_config_map, async_wait_for_condition_async_predicate
...
async def test_kv_basic_aio(ray_start_regular):
    gcs_client = gcs_utils.GcsAioClient(address=ray._private.worker.global_worker.gcs_client.address)
    ...
    assert await gcs_client.internal_kv_multi_get([b'A', b'B'], b'NS') == {b'A': b'B', b'B': b'C'}
    assert await gcs_client.internal_kv_multi_get([b'A', b'B', b'C'], b'NS') == {b'A': b'B', b'B': b'C'}
...
@pytest.mark.asyncio
@pytest.mark.parametrize('ray_start_cluster', [generate_system_config_map(health_check_initial_delay_ms=0, health_check_period_ms=1000, health_check_failure_threshold=2)], indirect=['ray_start_cluster'])
async def test_check_liveness(monkeypatch, ray_start_cluster):
    ...
    node_manager_addresses = [f'{n.raylet_ip_address}:{n.node_manager_port}' for n in [h, n1, n2]]
    ...
    await async_wait_for_condition_async_predicate(check, expect_liveness=[True, False, True])
    ...
    await async_wait_for_condition_async_predicate(check, expect_liveness=[True, False, False])
...
async def test_gcs_aio_client_is_async(ray_start_regular):
    ...
    with async_timeout.timeout(3):
        (none, result) = await asyncio.gather(asyncio.sleep(2), gcs_client.internal_kv_get(b'A', b'NS', timeout=2))
        assert result == b'B'
    await gcs_client.internal_kv_keys(b'A', b'NS', timeout=2)
    await gcs_client.internal_kv_del(b'A', True, b'NS', timeout=2)
