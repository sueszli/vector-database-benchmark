...
from ..cost import Reshape2GradOpCost, Reshape2OpCost, build_comp_costs_from_descs, build_comp_desc_from_dist_op, build_dp_costs
from ..utils import compute_compatible_and_update_dim_mapping, get_dist_tensor_spec, is_dim_shard, set_dist_op_desc_original_id
from .common import DistributedOperatorImpl, DistributedOperatorImplContainer, is_parameter_related, merge_forward_backward_dims_mapping, register_distributed_operator_impl, register_distributed_operator_impl_container, update_op_dims_mapping
...
class DistributedReshape2(DistributedOperatorImplContainer):
    ...
    def update_dims_mapping(dist_op):
        ...
        assert dist_op.serial_op.type == 'reshape2', f'{dist_op.serial_op.type} is not supported by dist reshape yet.'
...
class DistributedReshapeImpl0(DistributedOperatorImpl):
    ...
    def calc_fwd_cost(self, dist_op, ctx, cluster):
        ...
        cost_mapping = build_comp_costs_from_descs(Reshape2OpCost, ctx, processes, desc_mapping, cluster)
    ...
    def calc_bwd_cost(self, dist_op, ctx, cluster):
        ...
        cost_mapping = build_comp_costs_from_descs(Reshape2GradOpCost, ctx, processes, desc_mapping, cluster)
        res.append(cost_mapping)
    ...
    def backward(ctx, *args, **kwargs):
        if False:
            while True:
                i = 10
        DistributedDefaultImpl0.backward(ctx, *args, **kwargs)
register_distributed_operator_impl('reshape2', DistributedReshapeImpl0('add_one_dim_back'))
register_distributed_operator_impl('reshape2', DistributedReshapeImpl1('remove_one_dim_back'))
register_distributed_operator_impl('reshape2', DistributedReshapeImpl2('same_dim_shape'))