...
from paddle.nn.functional.flash_attention import flash_attention, flash_attn_unpadded, scaled_dot_product_attention
...
def attention_naive_with_mask(q, k, v, attn_bias):
    ...
    o = paddle.matmul(p, vt)
    return paddle.transpose(o, [0, 2, 1, 3])
is_sm8x = core.is_compiled_with_cuda() and paddle.device.cuda.get_device_capability()[0] == 8 and (paddle.device.cuda.get_device_capability()[1] >= 0)
is_sm90 = core.is_compiled_with_cuda() and paddle.device.cuda.get_device_capability()[0] == 9 and (paddle.device.cuda.get_device_capability()[1] == 0)
is_sm_supported = is_sm8x or is_sm90

@unittest.skipIf(not core.is_compiled_with_cuda() or get_cuda_version() < 11040 or (not is_sm_supported), "core is not compiled with CUDA and cuda version need larger than or equal to 11.4and device's compute capability must be 8.x or 90")
...
@unittest.skipIf(not core.is_compiled_with_cuda() or get_cuda_version() < 11040 or (not is_sm_supported), "core is not compiled with CUDA and cuda version need larger than or equal to 11.4and device's compute capability must be 7.5 or 8.x")
