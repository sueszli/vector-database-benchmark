...
class TestBlocksMethods:
    ...
    async def test_dict_inputs_in_config(self):
        ...
        result = await demo.process_api(inputs=['huggy', 'face'], fn_index=0, state=None)
...
class TestBlocksPostprocessing:
    ...
    async def test_blocks_update_dict_does_not_postprocess_value_if_postprocessing_false(self):
        ...
        output = await demo.process_api(0, ['test'], state=None)
        assert output['data'][0] == {'__type__': 'update', 'value': media_data.BASE64_IMAGE}

    @pytest.mark.asyncio
    async def test_blocks_update_interactive(self):
        ...
        for fn_index in range(2):
            ...
            assert output['data'][0] == {'__type__': 'update', 'interactive': True}
...
class TestStateHolder:
        ...
        with gr.Blocks() as demo:
            ...
            def run(x, s):
                if False:
                    print('Hello World!')
                return (s, s + 1)
            num.submit(run, inputs=[num, state], outputs=[num, state])
        (app, _, _) = demo.launch(prevent_thread_lock=True, state_session_capacity=2)
        client = TestClient(app)
        session_1 = client.post('/api/predict/', json={'data': [1, None], 'session_hash': '1', 'fn_index': 0})
        assert session_1.json()['data'][0] == 0
        session_2 = client.post('/api/predict/', json={'data': [1, None], 'session_hash': '2', 'fn_index': 0})
        assert session_2.json()['data'][0] == 0
        session_1 = client.post('/api/predict/', json={'data': [1, None], 'session_hash': '1', 'fn_index': 0})
        assert session_1.json()['data'][0] == 1
        session_2 = client.post('/api/predict/', json={'data': [1, None], 'session_hash': '2', 'fn_index': 0})
        assert session_2.json()['data'][0] == 1
        session_3 = client.post('/api/predict/', json={'data': [1, None], 'session_hash': '3', 'fn_index': 0})
    ...
    async def test_updates_stored_up_to_capacity(self):
        with gr.Blocks() as demo:
            ...
            num.submit(run, inputs=[min, num], outputs=[min, num])
        (app, _, _) = demo.launch(prevent_thread_lock=True, state_session_capacity=2)
        client = TestClient(app)
        session_1 = client.post('/api/predict/', json={'data': [5, 5], 'session_hash': '1', 'fn_index': 0})
        assert session_1.json()['data'][0] == 5
        session_1 = client.post('/api/predict/', json={'data': [2, 2], 'session_hash': '1', 'fn_index': 0})
        assert 'error' in session_1.json()
        session_2 = client.post('/api/predict/', json={'data': [5, 5], 'session_hash': '2', 'fn_index': 0})
        assert session_2.json()['data'][0] == 5
        session_3 = client.post('/api/predict/', json={'data': [5, 5], 'session_hash': '3', 'fn_index': 0})
        assert session_3.json()['data'][0] == 5
        session_1 = client.post('/api/predict/', json={'data': [2, 2], 'session_hash': '1', 'fn_index': 0})
        assert 'error' not in session_1.json()
...
class TestCallFunction:
    ...
    async def test_call_generator(self):
        ...
        with gr.Blocks() as demo:
            ...
            out = gr.Number()
            btn = gr.Button()
            btn.click(generator, inputs=inp, outputs=out)
        demo.queue()
    ...
    async def test_call_both_generator_and_function(self):
        ...
        with gr.Blocks() as demo:
            ...
            btn.click(generator, inputs=inp, outputs=[out1, out2])
        demo.queue()
...
class TestBatchProcessing:
    ...
    async def test_functions_multiple_parameters(self):
        ...
        with gr.Blocks() as demo:
            ...
            btn2.click(batch_fn, inputs=[text1, leng], outputs=[text1, bigger], batch=True)
        output = await demo.call_function(0, ['Adam', 'Yahya'])
        assert output['prediction'] is False
        output = demo('Abubakar', 'Abid')
        assert output
        output = await demo.call_function(1, [['Adam', 'Mary'], [3, 5]])
        assert output['prediction'] == (['Ada', 'Mary'], [True, False])
...
class TestUpdate:
    ...
    async def test_accordion_update(self):
        with gr.Blocks() as demo:
            ...
            open_btn.click(lambda : gr.Accordion(open=True, label='Open Accordion'), inputs=None, outputs=[accordion])
            close_btn.click(lambda : gr.Accordion(open=False, label='Closed Accordion'), inputs=None, outputs=[accordion])
        result = await demo.process_api(fn_index=0, inputs=[None], request=None, state=None)
        assert result['data'][0] == {'open': True, 'label': 'Open Accordion', '__type__': 'update'}
        result = await demo.process_api(fn_index=1, inputs=[None], request=None, state=None)
        assert result['data'][0] == {'open': False, 'label': 'Closed Accordion', '__type__': 'update'}
