...
class TestIngressWeb:
    ...
    def test_should_pass_validation_with_just_ingress_enabled_v1(self):
        if False:
            for i in range(10):
                print('nop')
        render_chart(values={'ingress': {'web': {'enabled': True}}}, show_only=['templates/webserver/webserver-ingress.yaml'])

    def test_should_pass_validation_with_just_ingress_enabled_v1beta1(self):
        if False:
            print('Hello World!')
        render_chart(values={'ingress': {'web': {'enabled': True}}}, show_only=['templates/webserver/webserver-ingress.yaml'], kubernetes_version='1.16.0')

    def test_should_allow_more_than_one_annotation(self):
        if False:
            print('Hello World!')
        docs = render_chart(values={'ingress': {'web': {'enabled': True, 'annotations': {'aa': 'bb', 'cc': 'dd'}}}}, show_only=['templates/webserver/webserver-ingress.yaml'])
        assert {'aa': 'bb', 'cc': 'dd'} == jmespath.search('metadata.annotations', docs[0])

    def test_should_set_ingress_class_name(self):
        if False:
            print('Hello World!')
        docs = render_chart(values={'ingress': {'web': {'enabled': True, 'ingressClassName': 'foo'}}}, show_only=['templates/webserver/webserver-ingress.yaml'])
        assert 'foo' == jmespath.search('spec.ingressClassName', docs[0])

    def test_should_ingress_hosts_objs_have_priority_over_host(self):
        if False:
            return 10
        docs = render_chart(values={'ingress': {'web': {'enabled': True, 'tls': {'enabled': True, 'secretName': 'oldsecret'}, 'hosts': [{'name': '*.a-host', 'tls': {'enabled': True, 'secretName': 'newsecret1'}}, {'name': 'b-host', 'tls': {'enabled': True, 'secretName': 'newsecret2'}}, {'name': 'c-host', 'tls': {'enabled': True, 'secretName': 'newsecret1'}}, {'name': 'd-host', 'tls': {'enabled': False, 'secretName': ''}}, {'name': 'e-host'}], 'host': 'old-host'}}}, show_only=['templates/webserver/webserver-ingress.yaml'])
    ...
    def test_should_ingress_hosts_strs_have_priority_over_host(self):
        if False:
            i = 10
            return i + 15
        docs = render_chart(values={'ingress': {'web': {'enabled': True, 'tls': {'enabled': True, 'secretName': 'secret'}, 'hosts': ['*.a-host', 'b-host', 'c-host', 'd-host'], 'host': 'old-host'}}}, show_only=['templates/webserver/webserver-ingress.yaml'])
    ...
    def test_should_ingress_deprecated_host_and_top_level_tls_still_work(self):
        if False:
            while True:
                i = 10
        docs = render_chart(values={'ingress': {'web': {'enabled': True, 'tls': {'enabled': True, 'secretName': 'supersecret'}, 'host': 'old-host'}}}, show_only=['templates/webserver/webserver-ingress.yaml'])
        assert ['old-host'] == jmespath.search('spec.rules[*].host', docs[0]) == jmespath.search('spec.tls[0].hosts', docs[0])

    def test_should_ingress_host_entry_not_exist(self):
        if False:
            print('Hello World!')
        docs = render_chart(values={'ingress': {'web': {'enabled': True}}}, show_only=['templates/webserver/webserver-ingress.yaml'])
        assert not jmespath.search('spec.rules[*].host', docs[0])

    @pytest.mark.parametrize('global_value, web_value, expected', [(None, None, False), (None, False, False), (None, True, True), (False, None, False), (True, None, True), (False, True, True), (True, False, True)])
    ...
    def test_should_add_component_specific_labels(self):
        if False:
            print('Hello World!')
        docs = render_chart(values={'ingress': {'enabled': True}, 'webserver': {'labels': {'test_label': 'test_label_value'}}}, show_only=['templates/webserver/webserver-ingress.yaml'])
