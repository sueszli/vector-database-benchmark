...
if __name__ == '__main__':
    ...
    if opt.cluster_mode == 'yarn':
        init_orca_context(cluster_mode='yarn-client', num_nodes=opt.num_workers, cores=opt.cores, init_ray_on_spark=True)
    else:
    ...
    df.rename(columns={' FIPS': 'FIPS', 'Age-Adjusted Incidence Rate(Ê) - cases per 100,000': 'Age-Adjusted Incidence Rate', 'Recent 5-Year Trend (\x88) in Incidence Rates': 'Recent 5-Year Trend'}, inplace=True)
    rows = [i for (i, x) in df.iterrows() if df.iat[i, 2].find('_') != -1 or df.iat[i, 2].find('#') != -1 or df.iat[i, 6].find('*') != -1]
    df.drop(rows, inplace=True)
    df.reset_index(drop=True, inplace=True)
    num_rows = 1500
    df = df[0:num_rows]
    df['Lower 95% Confidence Interval'] = df['Lower 95% Confidence Interval'].astype('float')
    df['Upper 95% Confidence Interval'] = df['Upper 95% Confidence Interval'].astype('float')
    df['Average Annual Count'] = df['Average Annual Count'].astype('float')
    df['Recent 5-Year Trend'] = df['Recent 5-Year Trend'].astype('float')
    df['Age-Adjusted Incidence Rate'] = df['Age-Adjusted Incidence Rate'].astype('float')
    ...
    auto_xgb_reg.fit(data=(X_train, y_train), validation_data=(X_val, y_val), metric='rmse', n_sampling=num_rand_samples, search_space=search_space, search_alg=search_alg, search_alg_params=None, scheduler=scheduler, scheduler_params=scheduler_params)
