...
class DetrModelTester:
    ...
    def __init__(self, parent, batch_size=8, is_training=True, use_labels=True, hidden_size=32, num_hidden_layers=2, num_attention_heads=8, intermediate_size=4, hidden_act='gelu', hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, num_queries=12, num_channels=3, min_size=200, max_size=200, n_targets=8, num_labels=91):
        if False:
            i = 10
            return i + 15
        self.parent = parent
...
class DetrModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (DetrModel, DetrForObjectDetection, DetrForSegmentation) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': DetrModel, 'image-segmentation': DetrForSegmentation, 'object-detection': DetrForObjectDetection} if is_torch_available() else {}
        ...
        if return_labels:
            if model_class.__name__ in ['DetrForObjectDetection', 'DetrForSegmentation']:
                ...
                for i in range(self.model_tester.batch_size):
                    ...
                    target['masks'] = torch.ones(self.model_tester.n_targets, self.model_tester.min_size, self.model_tester.max_size, device=torch_device, dtype=torch.float)
                    labels.append(target)
    ...
    def test_initialization(self):
        ...
        configs_no_init.init_xavier_std = 1000000000.0
        for model_class in self.all_model_classes:
            model = model_class(config=configs_no_init)
            for (name, param) in model.named_parameters():
                if param.requires_grad:
                    if 'bbox_attention' in name and 'bias' not in name:
                        self.assertLess(100000, abs(param.data.max().item()), msg=f'Parameter {name} of model {model_class} seems not properly initialized')
                    else:
...
class DetrModelIntegrationTestsTimmBackbone(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor([[0.0616, -0.5146, -0.4032], [-0.7629, -0.4934, -1.7153], [-0.4768, -0.6403, -0.7826]]).to(torch_device)
    ...
    def test_inference_object_detection_head(self):
        ...
        expected_slice_logits = torch.tensor([[-19.1194, -0.0893, -11.0154], [-17.364, -1.8035, -14.0219], [-20.0461, -0.5837, -11.106]]).to(torch_device)
        self.assertTrue(torch.allclose(outputs.logits[0, :3, :3], expected_slice_logits, atol=0.0001))
        expected_shape_boxes = torch.Size((1, model.config.num_queries, 4))
        self.assertEqual(outputs.pred_boxes.shape, expected_shape_boxes)
        expected_slice_boxes = torch.tensor([[0.4433, 0.5302, 0.8853], [0.5494, 0.2517, 0.0529], [0.4998, 0.536, 0.9956]]).to(torch_device)
        ...
        expected_scores = torch.tensor([0.9982, 0.996, 0.9955, 0.9988, 0.9987]).to(torch_device)
    ...
    def test_inference_panoptic_segmentation_head(self):
        ...
        expected_shape_logits = torch.Size((1, model.config.num_queries, model.config.num_labels + 1))
        self.assertEqual(outputs.logits.shape, expected_shape_logits)
        expected_slice_logits = torch.tensor([[-18.1565, -1.7568, -13.5029], [-16.8888, -1.4138, -14.1028], [-17.5709, -2.508, -11.8654]]).to(torch_device)
        self.assertTrue(torch.allclose(outputs.logits[0, :3, :3], expected_slice_logits, atol=0.0001))
        expected_shape_boxes = torch.Size((1, model.config.num_queries, 4))
        self.assertEqual(outputs.pred_boxes.shape, expected_shape_boxes)
        expected_slice_boxes = torch.tensor([[0.5344, 0.1789, 0.9285], [0.442, 0.0572, 0.0875], [0.663, 0.6887, 0.1017]]).to(torch_device)
        self.assertTrue(torch.allclose(outputs.pred_boxes[0, :3, :3], expected_slice_boxes, atol=0.0001))
        ...
        expected_slice_masks = torch.tensor([[-7.7558, -10.8788, -11.9797], [-11.8881, -16.4329, -17.7451], [-14.7316, -19.7383, -20.3004]]).to(torch_device)
        self.assertTrue(torch.allclose(outputs.pred_masks[0, 0, :3, :3], expected_slice_masks, atol=0.001))
        results = image_processor.post_process_panoptic_segmentation(outputs, threshold=0.3, target_sizes=[image.size[::-1]])[0]
        expected_shape = torch.Size([480, 640])
        expected_slice_segmentation = torch.tensor([[4, 4, 4], [4, 4, 4], [4, 4, 4]], dtype=torch.int32).to(torch_device)
        ...
        self.assertTrue(number_of_unique_segments, expected_number_of_segments + 1)
        self.assertTrue(results['segmentation'].shape, expected_shape)
        self.assertTrue(torch.allclose(results['segmentation'][:3, :3], expected_slice_segmentation, atol=0.0001))
...
class DetrModelIntegrationTests(unittest.TestCase):
    ...
    def default_image_processor(self):
        if False:
            for i in range(10):
                print('nop')
        return DetrImageProcessor.from_pretrained('facebook/detr-resnet-50', revision='no_timm') if is_vision_available() else None
