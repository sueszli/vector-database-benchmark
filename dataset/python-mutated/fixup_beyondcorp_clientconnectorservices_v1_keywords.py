...
class beyondcorp_clientconnectorservicesCallTransformer(cst.CSTTransformer):
    CTRL_PARAMS: Tuple[str] = ('retry', 'timeout', 'metadata')
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_client_connector_service': ('parent', 'client_connector_service', 'client_connector_service_id', 'request_id', 'validate_only'), 'delete_client_connector_service': ('name', 'request_id', 'validate_only'), 'get_client_connector_service': ('name',), 'list_client_connector_services': ('parent', 'page_size', 'page_token', 'filter', 'order_by'), 'update_client_connector_service': ('update_mask', 'client_connector_service', 'request_id', 'validate_only', 'allow_missing')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=beyondcorp_clientconnectorservicesCallTransformer()):
    if False:
        while True:
            i = 10
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Fix up source that uses the beyondcorp_clientconnectorservices client library.\n\nThe existing sources are NOT overwritten but are copied to output_dir with changes made.\n\nNote: This tool operates at a best-effort level at converting positional\n      parameters in client method calls to keyword based parameters.\n      Cases where it WILL FAIL include\n      A) * or ** expansion in a method call.\n      B) Calls via function or method alias (includes free function calls)\n      C) Indirect or dispatched calls (e.g. the method is looked up dynamically)\n\n      These all constitute false negatives. The tool will also detect false\n      positives when an API method shares a name with another method.\n')
