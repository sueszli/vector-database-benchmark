...
async def async_handle_hover_request(socket_path: Path, hover_request: HoverRequest) -> Union[HoverResponse, ErrorResponse]:
    raw_request = json.dumps(['Query', hover_request.to_json()])
    response = await daemon_connection.attempt_send_async_raw_request(socket_path, raw_request)
    if isinstance(response, daemon_connection.DaemonConnectionFailure):
        return ErrorResponse(message=response.error_message)
    response = parse_raw_response(response, expected_response_kind='Hover', response_type=HoverResponse)
    if isinstance(response, ErrorResponse):
        return response
    return HoverResponse(response.contents)

async def async_handle_definition_request(socket_path: Path, definition_request: LocationOfDefinitionRequest) -> Union[LocationOfDefinitionResponse, ErrorResponse]:
    raw_request = json.dumps(['Query', definition_request.to_json()])
    response = await daemon_connection.attempt_send_async_raw_request(socket_path, raw_request)
    if isinstance(response, daemon_connection.DaemonConnectionFailure):
        return ErrorResponse(message=response.error_message)
    return parse_raw_response(response, expected_response_kind='LocationOfDefinition', response_type=LocationOfDefinitionResponse)

async def async_handle_type_errors_request(socket_path: Path, type_errors_request: TypeErrorsRequest) -> Union[TypeErrorsResponse, ErrorResponse]:
    raw_request = json.dumps(['Query', type_errors_request.to_json()])
    response = await daemon_connection.attempt_send_async_raw_request(socket_path, raw_request)
    if isinstance(response, daemon_connection.DaemonConnectionFailure):
        return ErrorResponse(message=response.error_message)
    return parse_raw_response(response, expected_response_kind='TypeErrors', response_type=TypeErrorsResponse)

async def async_handle_completion_request(socket_path: Path, completion_request: lsp.CompletionRequest) -> Union[PyreCompletionsResponse, ErrorResponse]:
    raw_request = json.dumps(['Query', completion_request.to_json()])
    response = await daemon_connection.attempt_send_async_raw_request(socket_path, raw_request)
    if isinstance(response, daemon_connection.DaemonConnectionFailure):
        return ErrorResponse(message=response.error_message)
    return parse_raw_response(response, expected_response_kind='Completion', response_type=PyreCompletionsResponse)

async def async_handle_register_client(socket_path: Path, register_client: RegisterClient) -> Union[str, daemon_connection.DaemonConnectionFailure]:
    raw_command = json.dumps(['Command', register_client.to_json()])
    response = await daemon_connection.attempt_send_async_raw_request(socket_path, raw_command)
    return response

async def async_handle_dispose_client(socket_path: Path, dispose_client: DisposeClient) -> Union[str, daemon_connection.DaemonConnectionFailure]:
    raw_command = json.dumps(['Command', dispose_client.to_json()])
    response = await daemon_connection.attempt_send_async_raw_request(socket_path, raw_command)
    return response

async def async_handle_local_update(socket_path: Path, local_update: LocalUpdate) -> Union[str, daemon_connection.DaemonConnectionFailure]:
    raw_command = json.dumps(['Command', local_update.to_json()])
    response = await daemon_connection.attempt_send_async_raw_request(socket_path, raw_command)
    return response

async def async_handle_file_opened(socket_path: Path, file_opened: FileOpened) -> Union[str, daemon_connection.DaemonConnectionFailure]:
    raw_command = json.dumps(['Command', file_opened.to_json()])
    response = await daemon_connection.attempt_send_async_raw_request(socket_path, raw_command)
    return response

async def async_handle_file_closed(socket_path: Path, file_closed: FileClosed) -> Union[str, daemon_connection.DaemonConnectionFailure]:
    raw_command = json.dumps(['Command', file_closed.to_json()])
    response = await daemon_connection.attempt_send_async_raw_request(socket_path, raw_command)
    return response

async def async_handle_superclasses(socket_path: Path, superclasses: SuperclassesRequest) -> Union[str, daemon_connection.DaemonConnectionFailure]:
