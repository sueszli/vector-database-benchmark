...
from ..utils import _get_comm_group, _get_corresponding_rank, compute_compatible_and_update_dim_mapping, is_dim_replicate, is_dim_shard
...
register_distributed_operator_impl_container(DistributedFusedFeedForward('fused_feedforward'))
...
class DistributedFusedFeedForwardImpl(DistributedOperatorImpl):
    ...
    def is_input_compatible(self, dist_op):
        ...
        linear1_weight_dims_mapping = op_dist_attr.get_input_dims_mapping(linear1_weight)
        linear1_bias_dims_mapping = op_dist_attr.get_input_dims_mapping(linear1_bias)
        linear2_weight_dims_mapping = op_dist_attr.get_input_dims_mapping(linear2_weight)
        linear2_bias_dims_mapping = op_dist_attr.get_input_dims_mapping(linear2_bias)
        for mapping in x_dims_mapping[1:-1]:
            if is_dim_shard(mapping):
                return False
        if is_dim_shard(linear1_weight_dims_mapping[-2]) or is_dim_replicate(linear1_weight_dims_mapping[-1]):
            return False
        if is_dim_replicate(linear1_bias_dims_mapping[-1]):
            return False
        if is_dim_replicate(linear2_weight_dims_mapping[-2]) or is_dim_shard(linear2_weight_dims_mapping[-1]):
            return False
    ...
    def forward(ctx, *args, **kwargs):
        ...
        linear1_weight_col_dim_mapping = op_dist_attr.get_input_dims_mapping(linear1_weight)[-1]
        assert linear1_weight_col_dim_mapping >= 0, "col_parallel_matmul's row should be divided by a specific mesh axis, but got [{}]".format(linear1_weight_col_dim_mapping)
    ...
    def backward(ctx, *args, **kwargs):
        ...
        linear2_weight_col_dim_mapping = op_dist_attr.get_input_dims_mapping(linear2_weight)[-1]
        assert linear2_weight_col_dim_mapping >= 0, "col_parallel_matmul's row should be divided by a specific mesh axis, but got [{}]".format(linear2_weight_col_dim_mapping)
        process_mesh_shape = op_dist_attr.process_mesh.shape
        process_mesh_group = op_dist_attr.process_mesh.process_ids
        ...
        new_op._set_attr('ring_id', int(group.id))
register_distributed_operator_impl('fused_feedforward', DistributedFusedFeedForwardImpl('tensor_parallel'))