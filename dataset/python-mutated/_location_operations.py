...
class LocationOperations:
        ...
        def prepare_request(next_link=None):
            if False:
                while True:
                    i = 10
            if not next_link:
                request = build_list_usage_request(location=location, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.list_usage.metadata['url'], headers=_headers, params=_params)
    ...
    @distributed_trace
    def list_cached_images(self, location: str, **kwargs: Any) -> AsyncIterable['_models.CachedImages']:
        if False:
            return 10
        ...
        def prepare_request(next_link=None):
            if False:
                print('Hello World!')
            if not next_link:
                request = build_list_cached_images_request(location=location, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.list_cached_images.metadata['url'], headers=_headers, params=_params)
                request = _convert_request(request)
                request.url = self._client.format_url(request.url)
            else:
                _parsed_next_link = urllib.parse.urlparse(next_link)
    ...
    @distributed_trace
    def list_capabilities(self, location: str, **kwargs: Any) -> AsyncIterable['_models.Capabilities']:
        if False:
            while True:
                i = 10
        ...
        def prepare_request(next_link=None):
            if False:
                while True:
                    i = 10
            if not next_link:
                request = build_list_capabilities_request(location=location, subscription_id=self._config.subscription_id, api_version=api_version, template_url=self.list_capabilities.metadata['url'], headers=_headers, params=_params)
                request = _convert_request(request)
                request.url = self._client.format_url(request.url)
            else:
                _parsed_next_link = urllib.parse.urlparse(next_link)
