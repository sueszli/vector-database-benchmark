...
if __name__ == '__main__':
    ...
    x = x_re + x_im * 1j
    if 1:
        (y_python_gen_calc, y_python_table, y_python_raw_calc, y_cpp_table, y_cpp_raw_calc, constel, code, c) = test_qpsk(index, x, prec)
    else:
        (y_python_gen_calc, y_python_table, y_python_raw_calc, y_cpp_table, y_cpp_raw_calc, constel, code, c) = test_qam16(index, x, prec)
    ...
    sp1.plot([c.real for c in constel], [c.imag for c in constel], 'bo')
    ...
    for (i, c) in enumerate(constel):
        sp1.text(1.2 * c.real, 1.2 * c.imag, bin(code[i])[2:].zfill(fill), ha='center', va='center', size=18)
    pyplot.show()