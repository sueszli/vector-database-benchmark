...
async def main(host: str, port: int, db_file_path: str, bootstrap_node: Optional[str], prometheus_port: int, export: bool):
    ...
    await storage.open()
    node = Node(loop, PeerManager(loop), node_id, port, port, 3333, None, storage=storage, is_bootstrap_node=True)
    ...
    while True:
        ...
        log.info('Known peers: %d. Storing contact information for %d blobs from %d peers.', len(node.protocol.routing_table.get_peers()), len(node.protocol.data_store), len(node.protocol.data_store.get_storing_contacts()))
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Starts a single DHT node, which then can be used as a seed node or just a contributing node.')
    ...
    parser.add_argument('--bootstrap_node', default=None, type=str, help='Node to connect for bootstraping this node. Leave unset to use the default ones. Format: host:port Example: lbrynet1.lbry.com:4444')
