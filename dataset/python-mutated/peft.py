...
class LoraPostprocessorConfig(schema_utils.BaseMarshmallowConfig):
    ...
    merge_adapter_into_base_model: bool = schema_utils.Boolean(default=False, description='Instructs whether or not the fine-tuned LoRA weights are to be merged into the base LLM model so\nthat the complete fine-tuned model is available to be used and/or persisted, and then reused upon loading as a single\nmodel (rather than having to load base and fine-tuned models separately).')
    progressbar: bool = schema_utils.Boolean(default=False, description='Instructs whether or not to show a progress bar indicating the unload and merge process.')
...
class BaseAdapterConfig(schema_utils.BaseMarshmallowConfig, ABC):
    type: str
    pretrained_adapter_weights: Optional[str] = schema_utils.String(default=None, description='Path to pretrained weights.', allow_none=True)
...
class LoraConfig(BaseAdapterConfig):
    type: str = schema_utils.ProtectedString('lora', description=LLM_METADATA['adapter']['lora']['type'].long_description)
    r: int = schema_utils.PositiveInteger(default=8, description='Lora attention dimension.', parameter_metadata=LLM_METADATA['adapter']['lora']['r'])
    alpha: int = schema_utils.PositiveInteger(default=16, description='The alpha parameter for Lora scaling.', parameter_metadata=LLM_METADATA['adapter']['lora']['alpha'])
    dropout: float = schema_utils.NonNegativeFloat(default=0.05, description='The dropout probability for Lora layers.', parameter_metadata=LLM_METADATA['adapter']['lora']['dropout'])
    bias_type: str = schema_utils.StringOptions(options=['none', 'all', 'lora_only'], default='none', description='Bias type for Lora.')
...
class BasePromptLearningConfig(BaseAdapterConfig):
    ...
    """
    num_virtual_tokens: int = schema_utils.PositiveInteger(default=8, description='Number of virtual tokens to add to the prompt. Virtual tokens are used to control the behavior of  the model during inference. ', parameter_metadata=LLM_METADATA['adapter']['prompt_learning']['num_virtual_tokens'])
    token_dim: Optional[int] = schema_utils.PositiveInteger(default=None, allow_none=True, description='The hidden embedding dimension of the base transformer model.')
    num_transformer_submodules: Optional[int] = schema_utils.PositiveInteger(default=None, allow_none=True, description='The number of transformer submodules in the base transformer model.')
    num_attention_heads: Optional[int] = schema_utils.PositiveInteger(default=None, allow_none=True, description='The number of attention heads in the base transformer model.')
    num_layers: Optional[int] = schema_utils.PositiveInteger(default=None, allow_none=True, description='The number of layers in the base transformer model.')
...
class AdaloraConfig(LoraConfig):
    ...
    type: str = schema_utils.ProtectedString('adalora', description=LLM_METADATA['adapter']['adalora']['type'].long_description)
    target_r: int = schema_utils.PositiveInteger(default=8, description='Target Lora Matrix Dimension. The target average rank of incremental matrix.')
    init_r: int = schema_utils.PositiveInteger(default=12, description='Initial Lora Matrix Dimension. The initial rank for each incremental matrix.')
    tinit: int = schema_utils.NonNegativeInteger(default=0, description='The steps of initial fine-tuning warmup.')
    tfinal: int = schema_utils.NonNegativeInteger(default=0, description='The steps of final fine-tuning warmup.')
    delta_t: int = schema_utils.NonNegativeInteger(default=1, description='The time internval between two budget allocations. The step interval of rank allocation.')
    beta1: float = schema_utils.FloatRange(default=0.85, min=0.0, max=1.0, description='The hyperparameter of EMA for sensitivity smoothing.')
    beta2: float = schema_utils.FloatRange(default=0.85, min=0.0, max=1.0, description=' The hyperparameter of EMA for undertainty quantification.')
    orth_reg_weight: float = schema_utils.FloatRange(default=0.5, min=0.0, max=1.0, description='The coefficient of orthogonality regularization.')
    total_step: Optional[int] = schema_utils.PositiveInteger(default=None, allow_none=True, description='The total training steps that should be specified before training.')
    rank_pattern: Optional[dict] = schema_utils.Dict(default=None, allow_none=True, description='The allocated rank for each weight matrix by RankAllocator.')
...
class AdaptionPromptConfig(BaseAdapterConfig):
    ...
    type: str = schema_utils.ProtectedString('adaption_prompt', description=LLM_METADATA['adapter']['adaption_prompt']['type'].long_description)
    adapter_len: int = schema_utils.PositiveInteger(default=4, description='Number of adapter tokens to insert.', parameter_metadata=LLM_METADATA['adapter']['adaption_prompt']['adapter_len'])
    adapter_layers: int = schema_utils.PositiveInteger(default=1, allow_none=False, description='Number of adapter layers to insert (from the top).', parameter_metadata=LLM_METADATA['adapter']['adaption_prompt']['adapter_layers'])
