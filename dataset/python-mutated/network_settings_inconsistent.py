...
async def warn_on_inconsistent_network_settings(hass: HomeAssistant, config_entry: ConfigEntry, old_state: NetworkBackup, new_state: NetworkBackup) -> None:
    """Create a repair if the network settings are inconsistent with the last backup."""
    ir.async_create_issue(hass, domain=DOMAIN, issue_id=ISSUE_INCONSISTENT_NETWORK_SETTINGS, is_fixable=True, severity=ir.IssueSeverity.ERROR, translation_key=ISSUE_INCONSISTENT_NETWORK_SETTINGS, data={'config_entry_id': config_entry.entry_id, 'old_state': old_state.as_dict(), 'new_state': new_state.as_dict()})
...
class NetworkSettingsInconsistentFlow(RepairsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, str] | None=None) -> FlowResult:
        ...
        return self.async_show_menu(step_id='init', menu_options=['restore_old_settings', 'use_new_settings'], description_placeholders={'diff': _format_settings_diff(self._old_state, self._new_state)})

    async def async_step_use_new_settings(self, user_input: dict[str, str] | None=None) -> FlowResult:
    ...
    async def async_step_restore_old_settings(self, user_input: dict[str, str] | None=None) -> FlowResult:
