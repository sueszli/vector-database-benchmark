...
from typing import TYPE_CHECKING, List, Mapping, Optional, Sequence, Tuple, Type, TypeVar, overload
...
class ResolveRoomIdMixin:
    ...
    async def resolve_room_id(self, room_identifier: str, remote_room_hosts: Optional[List[str]]=None) -> Tuple[str, Optional[List[str]]]:
        """
        ...
        if RoomID.is_valid(room_identifier):
            ...
            raise SynapseError(HTTPStatus.BAD_REQUEST, '%s was not legal room ID or room alias' % (room_identifier,))
        if not resolved_room_id:
            raise SynapseError(HTTPStatus.BAD_REQUEST, 'Unknown room ID or room alias %s' % room_identifier)
