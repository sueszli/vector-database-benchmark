...
DataflowRunner:
$ python addprefix.py       --runner DataflowRunner       --temp_location $TEMP_LOCATION       --project $GCP_PROJECT       --region $GCP_REGION       --job_name $JOB_NAME       --num_workers $NUM_WORKERS       --input "gs://dataflow-samples/shakespeare/kinglear.txt"       --output "gs://$GCS_BUCKET/javaprefix/output"       --expansion_service_port <PORT>
"""
import logging
import re
import typing
...
def run(input_path, output_path, expansion_service_port, pipeline_args):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        java_output = input | 'JavaPrefix' >> beam.ExternalTransform('beam:transform:org.apache.beam:javaprefix:v1', ImplicitSchemaPayloadBuilder({'prefix': 'java:'}), 'localhost:%s' % expansion_service_port)
