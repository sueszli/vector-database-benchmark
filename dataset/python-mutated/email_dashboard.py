...
class EmailDashboardPage(base.BaseHandler[Dict[str, str], Dict[str, str]]):
...
class EmailDashboardResultPage(base.BaseHandler[EmailDashboardResultPageNormalizedPayloadDict, Dict[str, str]]):
    """Handler for email dashboard result page."""
    URL_PATH_ARGS_SCHEMAS = {'query_id': {'schema': {'type': 'basestring'}}}
    HANDLER_ARGS_SCHEMAS = {'GET': {}, 'POST': {'email_subject': {'schema': {'type': 'basestring'}}, 'email_body': {'schema': {'type': 'basestring'}}, 'email_intent': {'schema': {'type': 'basestring'}}, 'max_recipients': {'schema': {'type': 'int'}, 'default_value': None}}}
