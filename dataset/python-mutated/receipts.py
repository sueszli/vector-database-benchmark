...
from synapse.types import JsonDict, JsonMapping, MultiWriterStreamToken, ReadReceipt, StreamKeyType, UserID, get_domain_from_id
if TYPE_CHECKING:
...
class ReceiptsHandler:
    ...
    async def _received_remote_receipt(self, origin: str, content: JsonDict) -> None:
        ...
        for (room_id, room_values) in content.items():
            is_in_room = await self.event_auth_handler.is_host_in_room(room_id, self.server_name)
            if not is_in_room:
                logger.info("Ignoring receipt for room %r from server %s as we're not in the room", room_id, origin)
                continue
            domains = await self._storage_controllers.state.get_current_hosts_in_room_or_partial_state_approximation(room_id)
            if origin not in domains:
                logger.info("Ignoring receipt for room %r from server %s as they're not in the room", room_id, origin)
                continue
            for (receipt_type, users) in room_values.items():
                for (user_id, user_values) in users.items():
                    if get_domain_from_id(user_id) != origin:
                        logger.info('Received receipt for user %r from server %s, ignoring', user_id, origin)
                        continue
                    data = user_values.get('data', {})
                    thread_id = data.get('thread_id')
                    if not isinstance(thread_id, str):
                        thread_id = None
                    receipts.append(ReadReceipt(room_id=room_id, receipt_type=receipt_type, user_id=user_id, event_ids=user_values['event_ids'], thread_id=thread_id, data=data))
    ...
    async def _handle_new_receipts(self, receipts: List[ReadReceipt]) -> bool:
        ...
        for receipt in receipts:
            stream_id = await self.store.insert_receipt(receipt.room_id, receipt.receipt_type, receipt.user_id, receipt.event_ids, receipt.thread_id, receipt.data)
        ...
        self.notifier.on_new_event(StreamKeyType.RECEIPT, max_batch_id, rooms=affected_room_ids)
        await self.hs.get_pusherpool().on_new_receipts({r.user_id for r in receipts_persisted})
        return True

    async def received_client_receipt(self, room_id: str, receipt_type: str, user_id: UserID, event_id: str, thread_id: Optional[str]) -> None:
        """Called when a client tells us a local user has read up to the given
        event_id in the room.
        """
        if not await self.event_handler.get_event(user_id, room_id, event_id):
            return
        receipt = ReadReceipt(room_id=room_id, receipt_type=receipt_type, user_id=user_id.to_string(), event_ids=[event_id], thread_id=thread_id, data={'ts': int(self.clock.time_msec())})
...
class ReceiptEventSource(EventSource[MultiWriterStreamToken, JsonMapping]):
    ...
    async def get_new_events(self, user: UserID, from_key: MultiWriterStreamToken, limit: int, room_ids: Iterable[str], is_guest: bool, explicit_room_id: Optional[str]=None) -> Tuple[List[JsonMapping], MultiWriterStreamToken]:
        to_key = self.get_current_key()
        if from_key == to_key:
            return ([], to_key)
        events = await self.store.get_linearized_receipts_for_rooms(room_ids, from_key=from_key, to_key=to_key)
        events = ReceiptEventSource.filter_out_private_receipts(events, user.to_string())
        return (events, to_key)

    async def get_new_events_as(self, from_key: MultiWriterStreamToken, to_key: MultiWriterStreamToken, service: ApplicationService) -> Tuple[List[JsonMapping], MultiWriterStreamToken]:
        ...
        if from_key == to_key:
            return ([], to_key)
        rooms_to_events = await self.store.get_linearized_receipts_for_all_rooms(from_key=from_key, to_key=to_key)
        events = []
