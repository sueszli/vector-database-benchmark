...
class GraphQLWSConsumer(ChannelsWSConsumer):
    ...
    async def connect(self) -> None:
        ...
        if preferred_protocol == GRAPHQL_TRANSPORT_WS_PROTOCOL:
            self._handler = self.graphql_transport_ws_handler_class(schema=self.schema, debug=self.debug, connection_init_wait_timeout=self.connection_init_wait_timeout, get_context=self.get_context, get_root_value=self.get_root_value, ws=self)
        elif preferred_protocol == GRAPHQL_WS_PROTOCOL:
            self._handler = self.graphql_ws_handler_class(schema=self.schema, debug=self.debug, keep_alive=self.keep_alive, keep_alive_interval=self.keep_alive_interval, get_context=self.get_context, get_root_value=self.get_root_value, ws=self)
        else:
    ...
    async def get_context(self, request: ChannelsConsumer, connection_params: Any) -> Context:
        return {'request': request, 'connection_params': connection_params, 'ws': request}