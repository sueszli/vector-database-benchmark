...
RESULT_STORE_LINK_RE = re.compile('^INFO: Streaming build results to: (https://[\\w./\\-]+)')
FAILED_BUILD_LINE = 'FAILED: Build did NOT complete successfully'
BUILD_STATUS_LINE = 'INFO: Build'
TESTS_FAILED_RE = re.compile('^INFO: Build completed, \\d+ tests? FAILED')
BAZEL_COMMAND_RE = re.compile('(^| )(?P<command>bazel (.*? )?(?P<type>test|build) .+)')
