...
async def get(ctx: lightbulb.SlashContext) -> None:
    ...
    async with conn.cursor() as cursor:
        ...
        await ctx.respond(f"**Guild Settings**\n`log_channel`: {(mention(guild_settings.log_channel_id, 'channel') if guild_settings.log_channel_id else 'not set')}\n")
...
async def log_channel(ctx: lightbulb.SlashContext) -> None:
    ...
    conn: Connection = ctx.bot.d.db
    assert ctx.guild_id is not None
    assert isinstance(channel, hikari.InteractionChannel)
    ...
    async with conn.cursor() as cursor:
        await cursor.execute('INSERT OR REPLACE INTO guild_settings (guild_id, log_channel_id) VALUES (?, ?)', (ctx.guild_id, channel.id))
    await conn.commit()
