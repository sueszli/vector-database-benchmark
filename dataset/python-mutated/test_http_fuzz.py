...
request_lines = sampled_from([b'GET / HTTP/1.1', b'GET http://example.com/ HTTP/1.1', b'CONNECT example.com:443 HTTP/1.1', b'HEAD /foo HTTP/0.9'])
response_lines = sampled_from([b'HTTP/1.1 200 OK', b'HTTP/1.1 100 Continue', b'HTTP/0.9 204 No Content', b'HEAD /foo HTTP/0.9'])
headers = lists(sampled_from([b'Host: example.com', b'Content-Length: 5', b'Expect: 100-continue', b'Transfer-Encoding: chunked', b'Connection: close', b'']))
...
def test_fuzz_h1_response(data):
    ...
    for chunk in data:
        for _ in playbook.layer.handle_event(events.DataReceived(server(), chunk)):
            pass
h2_flags = sets(sampled_from(['END_STREAM', 'END_HEADERS']))
