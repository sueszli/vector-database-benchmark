...
class AutoProjectedGradientDescent(EvasionAttack):
    ...
    | Paper link: https://arxiv.org/abs/2003.01690
    """
    attack_params = EvasionAttack.attack_params + ['norm', 'eps', 'eps_step', 'max_iter', 'targeted', 'nb_random_init', 'batch_size', 'loss_type', 'verbose']
    ...
    def __init__(self, estimator: 'CLASSIFIER_LOSS_GRADIENTS_TYPE', norm: Union[int, float, str]=np.inf, eps: float=0.3, eps_step: float=0.1, max_iter: int=100, targeted: bool=False, nb_random_init: int=5, batch_size: int=32, loss_type: Optional[str]=None, verbose: bool=True):
        ...
        if loss_type is None:
            ...
            if loss_type == 'cross_entropy':
                ...
                raise NotImplementedError()
            estimator_apgd = PyTorchClassifier(model=estimator.model, loss=_loss_object_pt, input_shape=estimator.input_shape, nb_classes=estimator.nb_classes, optimizer=None, channels_first=estimator.channels_first, clip_values=estimator.clip_values, preprocessing_defences=estimator.preprocessing_defences, postprocessing_defences=estimator.postprocessing_defences, preprocessing=estimator.preprocessing, device_type=str(estimator._device))
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray]=None, **kwargs) -> np.ndarray:
        ...
        for _ in trange(max(1, self.nb_random_init), desc='AutoPGD - restart', disable=not self.verbose):
            ...
            for batch_id in trange(int(np.ceil(x_robust.shape[0] / float(self.batch_size))), desc='AutoPGD - batch', leave=False, disable=not self.verbose):
