...
ERROR_MESSAGE = "{path}:{lineno}:{col_offset}: Don't use pd.array in core, import array as pd_array instead\n"
...
class Visitor(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        if False:
            return 10
        if node.module is not None and node.module.startswith('pandas') and any((i.name == 'array' and i.asname != 'pd_array' for i in node.names)):
    ...
    def visit_Attribute(self, node: ast.Attribute) -> None:
        if False:
            for i in range(10):
                print('nop')
        if isinstance(node.value, ast.Name) and node.value.id == 'pd' and (node.attr == 'array'):
            msg = ERROR_MESSAGE.format(path=self.path, lineno=node.lineno, col_offset=node.col_offset)
            sys.stdout.write(msg)
            sys.exit(1)
        super().generic_visit(node)
