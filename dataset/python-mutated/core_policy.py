...
from prefect.server.orchestration.rules import ALL_ORCHESTRATION_STATES, TERMINAL_STATES, BaseOrchestrationRule, BaseUniversalTransform, FlowOrchestrationContext, OrchestrationContext, TaskOrchestrationContext
...
class CoreFlowPolicy(BaseOrchestrationPolicy):
    ...
    def priority():
        if False:
            i = 10
            return i + 15
        return [HandleFlowTerminalStateTransitions, EnforceCancellingToCancelledTransition, BypassCancellingScheduledFlowRuns, PreventPendingTransitions, HandlePausingFlows, HandleResumingPausedFlows, CopyScheduledTime, WaitForScheduledTime, RetryFailedFlows]
...
class CoreTaskPolicy(BaseOrchestrationPolicy):
    ...
    def priority():
        if False:
            for i in range(10):
                print('nop')
        return [CacheRetrieval, HandleTaskTerminalStateTransitions, PreventRunningTasksFromStoppedFlows, SecureTaskConcurrencySlots, CopyScheduledTime, WaitForScheduledTime, RetryFailedTasks, RenameReruns, UpdateFlowRunTrackerOnTasks, CacheInsertion, ReleaseTaskConcurrencySlots]
...
class MinimalTaskPolicy(BaseOrchestrationPolicy):
    ...
    def priority():
        if False:
            print('Hello World!')
        return [ReleaseTaskConcurrencySlots]
...
class SecureTaskConcurrencySlots(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], validated_state: Optional[states.State], context: TaskOrchestrationContext) -> None:
        self._applied_limits = []
        filtered_limits = await concurrency_limits.filter_concurrency_limits_for_orchestration(context.session, tags=context.run.tags)
        ...
        for (tag, cl) in run_limits.items():
            ...
            if limit == 0:
                ...
                await self.abort_transition(reason=f'The concurrency limit on tag "{tag}" is 0 and will deadlock if the task tries to run again.')
                ...
                for stale_tag in self._applied_limits:
                    ...
                    active_slots.discard(str(context.run.id))
                    stale_limit.active_slots = list(active_slots)
                await self.delay_transition(PREFECT_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS.value(), f'Concurrency limit for the {tag} tag has been reached')
            else:
    ...
    async def cleanup(self, initial_state: Optional[states.State], validated_state: Optional[states.State], context: OrchestrationContext) -> None:
...
class ReleaseTaskConcurrencySlots(BaseUniversalTransform):
    ...
    async def after_transition(self, context: OrchestrationContext):
        if self.nullified_transition():
            return
        if context.validated_state and context.validated_state.type not in [states.StateType.RUNNING, states.StateType.CANCELLING]:
            filtered_limits = await concurrency_limits.filter_concurrency_limits_for_orchestration(context.session, tags=context.run.tags)
            run_limits = {limit.tag: limit for limit in filtered_limits}
            for (tag, cl) in run_limits.items():
                active_slots = set(cl.active_slots)
...
class CacheInsertion(BaseOrchestrationRule):
    ...
    @inject_db
    async def after_transition(self, initial_state: Optional[states.State], validated_state: Optional[states.State], context: TaskOrchestrationContext, db: PrefectDBInterface) -> None:
        if not validated_state or not context.session:
            return
        cache_key = validated_state.state_details.cache_key
        if cache_key:
            new_cache_item = db.TaskRunStateCache(cache_key=cache_key, cache_expiration=validated_state.state_details.cache_expiration, task_run_state_id=validated_state.id)
...
class CacheRetrieval(BaseOrchestrationRule):
    ...
    @inject_db
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: TaskOrchestrationContext, db: PrefectDBInterface) -> None:
        cache_key = proposed_state.state_details.cache_key
        if cache_key and (not proposed_state.state_details.refresh_cache):
            cached_state_id = select(db.TaskRunStateCache.task_run_state_id).where(sa.and_(db.TaskRunStateCache.cache_key == cache_key, sa.or_(db.TaskRunStateCache.cache_expiration.is_(None), db.TaskRunStateCache.cache_expiration > pendulum.now('utc')))).order_by(db.TaskRunStateCache.created.desc()).limit(1).scalar_subquery()
            ...
            if cached_state:
                ...
                await self.reject_transition(state=new_state, reason='Retrieved state from cache')
...
class RetryFailedFlows(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: FlowOrchestrationContext) -> None:
        run_settings = context.run_settings
        run_count = context.run.run_count
        if run_settings.retries is None or run_count > run_settings.retries:
            return
        scheduled_start_time = pendulum.now('UTC').add(seconds=run_settings.retry_delay or 0)
        api_version = context.parameters.get('api-version', None)
        if api_version and api_version < Version('0.8.3'):
            failed_task_runs = await models.task_runs.read_task_runs(context.session, flow_run_filter=filters.FlowRunFilter(id={'any_': [context.run.id]}), task_run_filter=filters.TaskRunFilter(state={'type': {'any_': ['FAILED']}}))
            for run in failed_task_runs:
                await models.task_runs.set_task_run_state(context.session, run.id, state=states.AwaitingRetry(scheduled_time=scheduled_start_time), force=True)
                run.run_count = 0
        api_version = context.parameters.get('api-version', None)
        ...
        retry_state = states.AwaitingRetry(scheduled_time=scheduled_start_time, message=proposed_state.message, data=proposed_state.data)
...
class RetryFailedTasks(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: TaskOrchestrationContext) -> None:
        run_settings = context.run_settings
        run_count = context.run.run_count
        ...
        if run_settings.retries is not None and run_count <= run_settings.retries:
            retry_state = states.AwaitingRetry(scheduled_time=pendulum.now('UTC').add(seconds=delay), message=proposed_state.message, data=proposed_state.data)
            await self.reject_transition(state=retry_state, reason='Retrying')
...
class CopyScheduledTime(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: OrchestrationContext) -> None:
        if not proposed_state.state_details.scheduled_time:
            proposed_state.state_details.scheduled_time = initial_state.state_details.scheduled_time
...
class WaitForScheduledTime(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: OrchestrationContext) -> None:
        ...
        delay_seconds += round(delay.microseconds / 1000000.0)
        if delay_seconds > 0:
            await self.delay_transition(delay_seconds, reason='Scheduled time is in the future')
...
class HandlePausingFlows(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: TaskOrchestrationContext) -> None:
        ...
        if not initial_state.is_running():
            await self.reject_transition(state=None, reason='Cannot pause flows that are not currently running.')
            return
        self.key = proposed_state.state_details.pause_key
        if self.key is None:
            self.key = str(uuid4())
        if self.key in context.run.empirical_policy.pause_keys:
            await self.reject_transition(state=None, reason='This pause has already fired.')
            return
        if proposed_state.state_details.pause_reschedule:
            if context.run.parent_task_run_id:
                await self.abort_transition(reason='Cannot pause subflows with the reschedule option.')
                return
            if context.run.deployment_id is None:
                await self.abort_transition(reason='Cannot pause flows without a deployment with the reschedule option.')
                return

    async def after_transition(self, initial_state: Optional[states.State], validated_state: Optional[states.State], context: TaskOrchestrationContext) -> None:
        updated_policy = context.run.empirical_policy.dict()
...
class HandleResumingPausedFlows(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: TaskOrchestrationContext) -> None:
        if not (proposed_state.is_running() or proposed_state.is_scheduled() or proposed_state.is_final()):
            await self.reject_transition(state=None, reason=f'This run cannot transition to the {proposed_state.type} state from the {initial_state.type} state.')
            return
        if initial_state.state_details.pause_reschedule:
            if not context.run.deployment_id:
                await self.reject_transition(state=None, reason='Cannot reschedule a paused flow run without a deployment.')
                return
        pause_timeout = initial_state.state_details.pause_timeout
        if pause_timeout and pause_timeout < pendulum.now('UTC'):
            pause_timeout_failure = states.Failed(message='The flow was paused and never resumed.')
            await self.reject_transition(state=pause_timeout_failure, reason='The flow run pause has timed out and can no longer resume.')
            return
    ...
    async def after_transition(self, initial_state: Optional[states.State], validated_state: Optional[states.State], context: TaskOrchestrationContext) -> None:
        if context.run.flow_run_id is not None:
            ...
            if self.flow_run:
                ...
                raise ObjectNotFoundError(f'Unable to read flow run associated with task run: {context.run.id}, this flow run might have been deleted')
...
class HandleTaskTerminalStateTransitions(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: TaskOrchestrationContext) -> None:
        ...
        if proposed_state.type in {StateType.CANCELLING, StateType.PAUSED, StateType.CRASHED}:
            await self.abort_transition(f'Run is already {initial_state.type.value}.')
            return
        if initial_state.is_completed() and initial_state.data and (initial_state.data.get('type') != 'unpersisted'):
    ...
    async def cleanup(self, initial_state: Optional[states.State], validated_state: Optional[states.State], context: OrchestrationContext):
...
class HandleFlowTerminalStateTransitions(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: FlowOrchestrationContext) -> None:
        ...
        if proposed_state.type in {StateType.CANCELLING, StateType.PAUSED, StateType.CRASHED}:
            await self.abort_transition(f'Run is already in terminal state {initial_state.type.value}.')
            return
        if initial_state.is_completed() and (not proposed_state.is_final()) and initial_state.data and (initial_state.data.get('type') != 'unpersisted'):
        ...
        if proposed_state.is_scheduled() and (not context.run.deployment_id):
            await self.abort_transition('Cannot reschedule a run without an associated deployment.')
            return
        if not proposed_state.is_final():
            api_version = context.parameters.get('api-version', None)
            if api_version is None or api_version >= Version('0.8.4'):
...
class PreventPendingTransitions(BaseOrchestrationRule):
    ...
    """
    FROM_STATES = [StateType.PENDING, StateType.CANCELLING, StateType.RUNNING, StateType.CANCELLED]
    TO_STATES = [StateType.PENDING]

    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: OrchestrationContext) -> None:
        await self.abort_transition(reason=f'This run is in a {initial_state.type.name} state and cannot transition to a PENDING state.')
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: TaskOrchestrationContext) -> None:
        ...
        if flow_run is not None:
            if flow_run.state is None:
                await self.abort_transition(reason='The enclosing flow must be running to begin task execution.')
            elif flow_run.state.type == StateType.PAUSED:
                await self.reject_transition(state=states.Paused(name='NotReady'), reason=f'The flow is paused, new tasks can execute after resuming flow run: {flow_run.id}.')
...
class EnforceCancellingToCancelledTransition(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: TaskOrchestrationContext) -> None:
        await self.reject_transition(state=None, reason='Cannot transition flows that are cancelling to a state other than Cancelled.')
        return
...
class BypassCancellingScheduledFlowRuns(BaseOrchestrationRule):
    ...
    async def before_transition(self, initial_state: Optional[states.State], proposed_state: Optional[states.State], context: FlowOrchestrationContext) -> None:
        if not context.run.infrastructure_pid:
            await self.reject_transition(state=states.Cancelled(), reason='Scheduled flow run has no infrastructure to terminate.')