...
def upgrade() -> None:
    if False:
        return 10
    op.add_column('user', sa.Column('streak_last_day_date', sa.DateTime(timezone=True), server_default=sa.text('CURRENT_TIMESTAMP'), autoincrement=False, nullable=True))
    op.add_column('user', sa.Column('streak_days', sa.INTEGER(), autoincrement=False, nullable=True))
    op.add_column('user', sa.Column('last_activity_date', sa.DateTime(timezone=True), autoincrement=False, nullable=True))
