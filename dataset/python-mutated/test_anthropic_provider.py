...
def mock_chat_generate(messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any):
    if False:
        while True:
            i = 10
    return ChatResult(generations=[ChatGeneration(message=AIMessage(content='answer'))])

def mock_chat_generate_invalid(messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any):
    if False:
        return 10
    raise anthropic.APIStatusError('Invalid credentials', request=httpx._models.Request(method='POST', url='https://api.anthropic.com/v1/completions'), response=httpx._models.Response(status_code=401), body=None)
...
def test_get_credentials_obfuscated(mock_decrypt):
    if False:
        for i in range(10):
            print('nop')
    api_key = 'valid_key'
    provider = Provider(id='provider_id', tenant_id='tenant_id', provider_name=PROVIDER_NAME, provider_type=ProviderType.CUSTOM.value, encrypted_config=json.dumps({VALIDATE_CREDENTIAL_KEY: f'encrypted_{api_key}'}), is_valid=True)
    model_provider = MODEL_PROVIDER_CLASS(provider=provider)
