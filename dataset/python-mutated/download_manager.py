...
from tribler.core.utilities.rest_utils import FILE_SCHEME, HTTPS_SCHEME, HTTP_SCHEME, MAGNET_SCHEME, path_to_url, scheme_from_url, url_to_path
...
DEFAULT_DHT_ROUTERS = [('dht.libtorrent.org', 25401), ('router.bittorrent.com', 6881), ('router.utorrent.com', 6881), ('dht.transmissionbt.com', 6881)]
DEFAULT_LT_EXTENSIONS = [lt.create_ut_metadata_plugin, lt.create_ut_pex_plugin, lt.create_smart_ban_plugin]
...
class DownloadManager(TaskManager):
    ...
    async def _start(self):
        ...
        if self.gui_test_mode:
            from tribler.core.tests.tools.common import TORRENT_WITH_DIRS
    ...
    async def get_metainfo(self, infohash: bytes, timeout: float=30, hops: Optional[int]=None, url: Optional[str]=None, raise_errors: bool=False) -> Optional[Dict]:
        """
    ...
    async def start_download(self, torrent_file=None, tdef=None, config: DownloadConfig=None, checkpoint_disabled=False, hidden=False) -> Download:
        ...
        download = Download(tdef=tdef, config=config, download_defaults=self.download_defaults, checkpoint_disabled=checkpoint_disabled, hidden=hidden or config.get_bootstrap_download(), notifier=self.notifier, state_dir=self.state_dir, download_manager=self, dummy=self.dummy_mode)
            ...
            if self.state_cb_count % 5 == 0 and download.config.get_hops() == 0 and self.notifier:
                for peer in download.get_peerlist():
                    if str(peer['extended_version']).startswith('Tribler'):
                        self.notifier[notifications.tribler_torrent_peer_update](unhexlify(peer['id']), infohash, peer['dtotal'])
    ...
    async def load_checkpoint(self, filename):
        ...
        if not isinstance(metainfo, dict):
            self._logger.error('Could not resume checkpoint %s; metainfo is not dict %s %s', filename, type(metainfo), repr(metainfo))
            return
        try:
            url = metainfo.get(b'url', None)
            url = url.decode('utf-8') if url else url
            tdef = TorrentDefNoMetainfo(metainfo[b'infohash'], metainfo[b'name'], url) if b'infohash' in metainfo else TorrentDef.load_from_dict(metainfo)
    ...
    async def create_torrent_file(file_path_list, params=None):
        ...
        """
        return await asyncio.get_event_loop().run_in_executor(None, torrent_utils.create_torrent_file, file_path_list, params or {})
