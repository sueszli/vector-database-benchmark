            ...
            for index in [[1, 4], [4, 8]]:
                for axis in [0, 1, 2, 3]:
                    for overwrite in [True, False]:
                        for input in [{'X': ['input_data'], 'Index': ['index_data']}, {'X': ['input_data'], 'Index': ['index_data'], 'Axis': ['axis_data']}]:
                            for index_type_int32 in [True, False]:
                                ...
                                ops_config = [{'op_type': 'gather', 'op_inputs': input, 'op_outputs': {'Out': ['output_data']}, 'op_attrs': dics[0]}]
                                ops = self.generate_op_config(ops_config)
                                program_config = ProgramConfig(ops=ops, weights={}, inputs={'input_data': TensorConfig(data_gen=partial(generate_input1, shape)), 'index_data': TensorConfig(data_gen=partial(generate_input2 if index_type_int32 else generate_input4, index))} if len(input) == 2 else {'input_data': TensorConfig(data_gen=partial(generate_input1, shape)), 'index_data': TensorConfig(data_gen=partial(generate_input2, index)), 'axis_data': TensorConfig(data_gen=partial(generate_input3, axis))}, outputs=['output_data'])
        ...
        if ver[0] * 1000 + ver[1] * 100 + ver[0] * 10 < 7000:
            ...
            def teller1(program_config, predictor_config):
                ...
                if len(self.dynamic_shape.min_input_shape) != 0:
                    inputs = program_config.inputs
                    if len(inputs['input_data'].shape) == 1 or len(inputs['index_data'].shape) == 1:
                        return True
                return False
            self.add_skip_case(teller1, SkipReasons.TRT_NOT_SUPPORT, "Need to repair the case: trt reshape out failed for dynamic shape mode when inputs' dims==1. under trt7.0 ")
