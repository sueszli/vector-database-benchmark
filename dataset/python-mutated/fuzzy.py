...
class Matcher:
    ...
    def __init__(self, query: str, *, match_style: Style | None=None, case_sensitive: bool=False) -> None:
        if False:
            return 10
        ...
        self._query_regex = compile('.*?'.join((f'({escape(character)})' for character in query)), flags=0 if case_sensitive else IGNORECASE)
...
if __name__ == '__main__':
    ...
    print(Rule())
    candidates = ('foo.bar', ' foo.bar ', 'Hello foo.bar world', 'f o o . b a r', 'f o o .bar', 'foo. b a r', 'Lots of text before the foo.bar', 'foo.bar up front and then lots of text afterwards', 'This has an o in it but does not have a match', "Let's find one obvious match. But blat around always roughly.")
    results = sorted([(matcher.match(candidate), matcher.highlight(candidate)) for candidate in candidates], key=lambda pair: pair[0], reverse=True)
    for (score, highlight) in results:
        print(f"{score:.15f} '", highlight, "'", sep='')
    print(Rule())
    RUNS = 5
    candidates = [''.join(permutation) for permutation in permutations(ascii_lowercase[:10])]
