...
class VariantMediaAssign(BaseMutation):
    ...
    class Arguments:
        media_id = graphene.ID(required=True, description='ID of a product media to assign to a variant.')
    ...
    def perform_mutation(cls, _root, info: ResolveInfo, /, *, media_id, variant_id):
        if False:
            return 10
        media = cls.get_node_or_error(info, media_id, field='media_id', only_type=ProductMedia)
        ...
        with traced_atomic_transaction():
            if media and variant:
                ...
                if media_belongs_to_product:
                    ...
                    if not created:
                        raise ValidationError({'media_id': ValidationError('This media is already assigned', code=ProductErrorCode.MEDIA_ALREADY_ASSIGNED.value)})
                else:
                    raise ValidationError({'media_id': ValidationError("This media doesn't belong to that product.", code=ProductErrorCode.NOT_PRODUCTS_IMAGE.value)})
