...
from mage_ai.data_preparation.models.triggers import ScheduleInterval, ScheduleStatus, ScheduleType
from mage_ai.orchestration.db.models.schedules import BlockRun, EventMatcher, PipelineRun, PipelineSchedule
...
build_list_endpoint_tests(PipelineScheduleAPIEndpointTest, list_count=3, resource='pipeline_schedules', result_keys_to_compare=['created_at', 'description', 'event_matchers', 'global_data_product_uuid', 'id', 'last_pipeline_run_status', 'name', 'next_pipeline_run_date', 'pipeline_runs_count', 'pipeline_uuid', 'repo_path', 'schedule_interval', 'schedule_type', 'settings', 'sla', 'start_time', 'status', 'tags', 'token', 'updated_at', 'variables'])
build_list_endpoint_tests(PipelineScheduleAPIEndpointTest, list_count=2, resource='pipeline_schedules', build_query=lambda _self: {'schedule_type[]': [','.join([e.value for e in [ScheduleType.EVENT, ScheduleType.TIME]])]}, result_keys_to_compare=['created_at', 'description', 'event_matchers', 'global_data_product_uuid', 'id', 'last_pipeline_run_status', 'name', 'next_pipeline_run_date', 'pipeline_runs_count', 'pipeline_uuid', 'repo_path', 'schedule_interval', 'schedule_type', 'settings', 'sla', 'start_time', 'status', 'tags', 'token', 'updated_at', 'variables'])
build_list_endpoint_tests(PipelineScheduleAPIEndpointTest, list_count=1, resource='pipeline_schedules', resource_parent='pipeline', get_resource_parent_id=lambda self: self.pipeline2.uuid, result_keys_to_compare=['created_at', 'description', 'event_matchers', 'global_data_product_uuid', 'id', 'last_pipeline_run_status', 'name', 'next_pipeline_run_date', 'pipeline_runs_count', 'pipeline_uuid', 'repo_path', 'schedule_interval', 'schedule_type', 'settings', 'sla', 'start_time', 'status', 'tags', 'token', 'updated_at', 'variables'])
build_create_endpoint_tests(PipelineScheduleAPIEndpointTest, resource='pipeline_schedules', resource_parent='pipeline', get_resource_parent_id=lambda self: self.pipeline.uuid, assert_before_create_count=lambda self: len(PipelineSchedule.query.filter(PipelineSchedule.pipeline_uuid == self.pipeline.uuid).all()) == 2, assert_after_create_count=lambda self: len(PipelineSchedule.query.filter(PipelineSchedule.pipeline_uuid == self.pipeline.uuid).all()) == 3, build_payload=lambda self: dict(description=self.faker.text(), name=self.faker.unique.name(), schedule_interval=ScheduleInterval.HOURLY, schedule_type=ScheduleType.TIME, start_time=datetime.utcnow(), status=ScheduleStatus.ACTIVE, variables=dict(fire=1)))
build_detail_endpoint_tests(PipelineScheduleAPIEndpointTest, resource='pipeline_schedule', get_resource_id=lambda self: self.pipeline_schedule.id, result_keys_to_compare=['created_at', 'description', 'event_matchers', 'global_data_product_uuid', 'id', 'name', 'next_pipeline_run_date', 'pipeline_uuid', 'repo_path', 'schedule_interval', 'schedule_type', 'settings', 'sla', 'start_time', 'status', 'tags', 'token', 'updated_at', 'variables'])
...
build_update_endpoint_tests(PipelineScheduleAPIEndpointTest, resource='pipeline_schedule', get_resource_id=lambda self: self.pipeline_schedule.id, build_payload=_build_payload_update, get_model_before_update=lambda self: PipelineSchedule.query.get(self.pipeline_schedule.id), assert_after_update=_assert_after_update)
build_delete_endpoint_tests(PipelineScheduleAPIEndpointTest, resource='pipeline_schedule', get_resource_id=lambda self: self.pipeline_schedule.id, assert_before_delete_count=lambda self: len(PipelineSchedule.query.filter(PipelineSchedule.id == self.pipeline_schedule.id).all()) == 1, assert_after_delete_count=lambda self: len(PipelineSchedule.query.filter(PipelineSchedule.id == self.pipeline_schedule.id).all()) == 0)