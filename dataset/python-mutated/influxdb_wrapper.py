...
range_bucket_mappings = {'24h': settings.INFLUXDB_BUCKET + '_downsampled_15m', '7d': settings.INFLUXDB_BUCKET + '_downsampled_15m', '30d': settings.INFLUXDB_BUCKET + '_downsampled_1h'}
retries = Retry(connect=3, read=3, redirect=3)
influxdb_client = InfluxDBClient(url=url, token=token, org=influx_org, retries=retries, timeout=3000)
DEFAULT_DROP_COLUMNS = ('organisation', 'organisation_id', 'type', 'project', 'project_id', 'environment', 'environment_id', 'host')
