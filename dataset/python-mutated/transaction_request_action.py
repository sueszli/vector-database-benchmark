...
from .....payment.gateway import request_cancelation_action, request_charge_action, request_refund_action
...
class TransactionRequestAction(BaseMutation):
    ...
    class Arguments:
        ...
        action_type = graphene.Argument(TransactionActionEnum, required=True, description='Determines the action type.')
        amount = PositiveDecimal(description='Transaction request amount. If empty for refund or capture, maximal possible amount will be used.')

    class Meta:
        description = 'Request an action for payment transaction.' + ADDED_IN_34 + PREVIEW_FEATURE
