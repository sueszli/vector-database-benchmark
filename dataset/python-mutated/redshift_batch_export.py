...
from posthog.temporal.workflows.postgres_batch_export import PostgresInsertInputs, create_table_in_postgres, postgres_connection

async def insert_records_to_redshift(records: collections.abc.Iterator[dict[str, typing.Any]], redshift_connection: psycopg.AsyncConnection, schema: str, table: str, batch_size: int=100):
    ...
    pre_query = sql.SQL('INSERT INTO {table} ({fields}) VALUES').format(table=sql.Identifier(schema, table), fields=sql.SQL(', ').join(map(sql.Identifier, columns)))
...
async def async_client_cursor_from_connection(psycopg_connection: psycopg.AsyncConnection) -> typing.AsyncIterator[psycopg.AsyncClientCursor]:
...
async def insert_into_redshift_activity(inputs: RedshiftInsertInputs):
    ...
    async with get_client() as client:
        ...
        def map_to_record(row: dict) -> dict:
            ...
            return {key: json.dumps(row[key]) if key in json_columns and row[key] is not None else row[key] for key in schema_columns}
        async with postgres_connection(inputs) as connection:
            await insert_records_to_redshift((map_to_record(result) for result in results_iterator), connection, inputs.schema, inputs.table_name)
...
class RedshiftBatchExportWorkflow(PostHogWorkflow):
    ...
    async def run(self, inputs: RedshiftBatchExportInputs):
        ...
        insert_inputs = RedshiftInsertInputs(team_id=inputs.team_id, user=inputs.user, password=inputs.password, host=inputs.host, port=inputs.port, database=inputs.database, schema=inputs.schema, table_name=inputs.table_name, has_self_signed_cert=inputs.has_self_signed_cert, data_interval_start=data_interval_start.isoformat(), data_interval_end=data_interval_end.isoformat(), exclude_events=inputs.exclude_events, include_events=inputs.include_events, properties_data_type=inputs.properties_data_type)
        await execute_batch_export_insert_activity(insert_into_redshift_activity, insert_inputs, non_retryable_error_types=[], update_inputs=update_inputs, heartbeat_timeout_seconds=None)