...
async def test_input_changed_message_validation_failure():
    ...
    async with app.run_test() as pilot:
        ...
        assert app.messages[0].validation_result == ValidationResult.failure(failures=[Number.NotInRange(value='8', validator=app.validator, description='Must be between 1 and 5.')])
...
async def test_input_submitted_message_validation_failure():
    ...
    async with app.run_test() as pilot:
        ...
        assert app.messages[1].validation_result == ValidationResult.failure(failures=[Number.NotInRange(value='8', validator=app.validator, description='Must be between 1 and 5.')])
...
@pytest.mark.parametrize('validate_on', [set(), {'blur'}, {'submitted'}, {'blur', 'submitted'}, {'fried', 'garbage'}])
...
@pytest.mark.parametrize('validate_on', [set(), {'blur'}, {'changed'}, {'blur', 'changed'}, {'fried', 'garbage'}])
...
@pytest.mark.parametrize('validate_on', [set(), {'submitted'}, {'changed'}, {'submitted', 'changed'}, {'fried', 'garbage'}])
