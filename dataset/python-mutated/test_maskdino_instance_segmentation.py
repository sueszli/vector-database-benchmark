...
class MaskDINOInstanceSegmentationTest(unittest.TestCase):
    ...
    def test_run_with_model_from_modelhub(self):
        ...
        pipeline_ins = pipeline(task=Tasks.image_segmentation, model=model, preprocessor=None)
        print(pipeline_ins(input=self.image)[OutputKeys.LABELS])
    ...
    def test_run_by_direct_model_download(self):
        ...
        pipeline1 = MaskDINOInstanceSegmentationPipeline(model, preprocessor=None)
        pipeline2 = pipeline(Tasks.image_segmentation, model=model, preprocessor=None)
