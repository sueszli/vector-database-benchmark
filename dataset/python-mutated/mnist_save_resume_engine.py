...
try:
    ...
    try:
        ...
        raise ModuleNotFoundError('This module requires either tensorboardX or torch >= 1.2.0. You may install tensorboardX with command: \n pip install tensorboardX \nor upgrade PyTorch using your package manager of choice (pip or conda).')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='/tmp/mnist_save_resume', help='log directory for Tensorboard log output')
    parser.add_argument('--checkpoint_every', type=int, default=1, help='Checkpoint training every X epochs')
    parser.add_argument('--resume_from', type=str, default=None, help='Path to the checkpoint .pt file to resume training from')
    parser.add_argument('--crash_iteration', type=int, default=-1, help='Iteration at which to raise an exception')
    parser.add_argument('--deterministic', action='store_true', help='Deterministic training with dataflow synchronization')
    args = parser.parse_args()
    run(args.batch_size, args.val_batch_size, args.epochs, args.lr, args.momentum, args.log_interval, args.log_dir, args.checkpoint_every, args.resume_from, args.crash_iteration, args.deterministic)