...
st.markdown('Opyrator enables you to instantly to turn your python functions into microservices with auto-generated HTTP API, interactive UI, and more.' + ' You can explore some examples below 👇')
...
with st.beta_expander('Export this Opyrator'):
    st.markdown('Opyrator provides capabilities to seamlessly export your services into portable, shareable, and executable files or Docker images.')
    (col1, col2) = st.beta_columns([1, 2])
    with col1:
        open_zip_export_feature = st.button('📦 Export to ZIP')
    with col2:
        st.code(f'opyrator export app:{selected_demo} my-opyrator.zip', language='plain')
    (col1, col2) = st.beta_columns([1, 2])
    with col1:
        open_docker_export_feature = st.button('🐳 Export to Docker')
    with col2:
        st.code(f'opyrator export app:{selected_demo} --format docker my-opyrator:latest', language='plain')
if open_ui:
