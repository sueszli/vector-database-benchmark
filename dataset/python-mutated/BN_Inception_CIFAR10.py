...
def create_image_mb_source(map_file, mean_file, is_training, total_number_of_samples):
    ...
    if is_training:
        ...
        transforms += [xforms.crop(crop_type='center', crop_size=IMAGE_WIDTH)]
...
def bn_inception_train_and_eval(train_data, test_data, mean_data, minibatch_size=128, epoch_size=50000, max_epochs=200, restore=True, log_to_file=None, num_mbs_per_log=100, gen_heartbeat=False, profiler_dir=None, testing_parameters=(10000, 128)):
    if False:
        while True:
            i = 10
    _cntk_py.set_computation_network_trace_level(0)
    progress_writers = [ProgressPrinter(freq=num_mbs_per_log, tag='Training', log_to_file=log_to_file, gen_heartbeat=gen_heartbeat, num_epochs=max_epochs)]
    ...
    return train_and_test(network, trainer, train_source, test_source, max_epochs, minibatch_size, epoch_size, restore, profiler_dir, progress_writers[0], testing_parameters)
if __name__ == '__main__':
    ...
    os.chdir(data_path)
    bn_inception_train_and_eval(train_data, test_data, mean_data, minibatch_size=args['minibatch_size'], epoch_size=args['epoch_size'], max_epochs=args['num_epochs'], restore=not args['restart'], log_to_file=args['logdir'], num_mbs_per_log=100, gen_heartbeat=True, profiler_dir=args['profilerdir'])