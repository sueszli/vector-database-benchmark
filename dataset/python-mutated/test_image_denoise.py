...
class ImageDenoiseTest(unittest.TestCase):
    ...
    def test_run_by_direct_model_download(self):
        ...
        denoise_img = pipeline(input=self.demo_image_path)[OutputKeys.OUTPUT_IMG]
    ...
    def test_run_with_model_from_modelhub(self):
        ...
        denoise_img = pipeline_ins(input=self.demo_image_path)[OutputKeys.OUTPUT_IMG]
        (h, w) = denoise_img.shape[:2]
        print('pipeline: the shape of output_img is {}x{}'.format(h, w))
    ...
    def test_run_with_model_name(self):
        if False:
            while True:
                i = 10
        pipeline_ins = pipeline(task=Tasks.image_denoising, model=self.model_id)
        denoise_img = pipeline_ins(input=self.demo_image_path)[OutputKeys.OUTPUT_IMG]
        (h, w) = denoise_img.shape[:2]
        print('pipeline: the shape of output_img is {}x{}'.format(h, w))
