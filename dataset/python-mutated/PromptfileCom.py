...
class PromptfileCom(SimpleDownloader):
    ...
    __version__ = '0.19'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?promptfile\\.com/'
    ...
    INFO_PATTERN = '<span style=".+?" title=".+?">(?P<N>.*?) \\((?P<S>[\\d.,]+) (?P<U>[\\w^_]+)\\)</span>'
    OFFLINE_PATTERN = '<span style=".+?" title="File Not Found">File Not Found</span>'
    CHASH_PATTERN = 'input.+"([a-z\\d]{10,})".+"([a-z\\d]{10,})"'
    MODIFY_PATTERN = '\\$\\("#chash"\\)\\.val\\("(.+)"\\+\\$\\("#chash"\\)'
    LINK_FREE_PATTERN = '<a href="(http://www\\.promptfile\\.com/file/[^"]+)'
