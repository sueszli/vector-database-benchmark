...
class AdamaxTest(testing.TestCase):
    ...
    def test_config(self):
        if False:
            for i in range(10):
                print('nop')
        optimizer = Adamax(learning_rate=0.5, beta_1=0.8, beta_2=0.95, epsilon=1e-05)
    ...
    def test_single_step(self):
        ...
        self.assertAllClose(vars, [0.5, 1.5, 2.5, 3.5], rtol=0.0001, atol=0.0001)
    ...
    def test_correctness_with_golden(self):
        if False:
            while True:
                i = 10
        optimizer = Adamax(learning_rate=0.2, beta_1=0.85, beta_2=0.95, epsilon=1e-06)
        x = backend.Variable(np.ones([10]))
        grads = ops.arange(0.1, 1.1, 0.1)
        first_grads = ops.full((10,), 0.01)
        golden = np.array([[0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8], [0.6827, 0.6873, 0.6888, 0.6896, 0.6901, 0.6904, 0.6906, 0.6908, 0.6909, 0.691], [0.5333, 0.5407, 0.5431, 0.5444, 0.5451, 0.5456, 0.546, 0.5462, 0.5464, 0.5466], [0.368, 0.3773, 0.3804, 0.382, 0.3829, 0.3835, 0.384, 0.3843, 0.3846, 0.3848], [0.1933, 0.204, 0.2076, 0.2094, 0.2105, 0.2112, 0.2117, 0.2121, 0.2124, 0.2126]])
