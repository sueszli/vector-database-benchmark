...
async def test_skips_for_websocket() -> None:
    ...
    async def websocket_handler(socket: WebSocket) -> None:
        ...
        await socket.close()
    with create_test_client(route_handlers=[websocket_handler], compression_config=CompressionConfig(backend='brotli', brotli_gzip_fallback=False)).websocket_connect('/') as ws:
...
@pytest.mark.parametrize('backend, compression_encoding', (('brotli', CompressionEncoding.BROTLI), ('gzip', CompressionEncoding.GZIP)))
async def test_compression_streaming_response_emitted_messages(backend: Literal['gzip', 'brotli'], compression_encoding: Literal[CompressionEncoding.BROTLI, CompressionEncoding.GZIP], create_scope: Callable[..., Scope], mock_asgi_app: ASGIApp) -> None:
    mock = MagicMock()

    async def fake_send(message: Message) -> None:
        mock(message)
    wrapped_send = CompressionMiddleware(mock_asgi_app, CompressionConfig(backend=backend)).create_compression_send_wrapper(fake_send, compression_encoding, create_scope())
