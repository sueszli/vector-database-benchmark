...
industry_category_df = DataAPI.EquIndustryGet(secID=u'', ticker=u'', industryVersionCD=u'010303', industry=u'', industryID=u'', industryID1=u'', industryID2=u'', industryID3=u'', intoDate=u'20210101', equTypeID=u'', field=u'', pandas='1')
industry_category_df = industry_category_df.drop_duplicates(['ticker'])
industry_category_df = industry_category_df[industry_category_df['ticker'].str.startswith(('3', '6', '0'))]
...
market_df = DataAPI.MktEqudGet(secID=u'', ticker=ticker_list, tradeDate=u'', beginDate=beginDate, endDate=endDate, isOpen='', field=u'tradeDate,ticker,marketValue,closePrice', pandas='1')
d1 = market_df.set_index(['tradeDate', 'ticker']).unstack()['marketValue']
lcap_ = np.log(d1)
labeled_lcap_ = lcap_.apply(pd.qcut, axis=1, q=5, labels=False)
...
for i in range(len(trade_calendar) - 2):
    ...
    train_data = pd.DataFrame({'labeled_industry': industry_df_.loc[train_x_time], 'labeled_lcap': labeled_lcap_.loc[train_x_time] + 200, 'labeled_mom': mom_result.loc[train_x_time] + 300, 'labeled_pct_change': label_pct_change.loc[train_y_time]})
    ...
    predict_data = pd.DataFrame({'labeled_industry': industry_df_.loc[predict_x_time], 'labeled_lcap': labeled_lcap_.loc[predict_x_time] + 200, 'labeled_mom': mom_result.loc[predict_x_time] + 300, 'pct_change': label_pct_change.loc[predict_y_time]})
