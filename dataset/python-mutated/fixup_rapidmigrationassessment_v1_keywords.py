...
class rapidmigrationassessmentCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_annotation': ('parent', 'annotation', 'request_id'), 'create_collector': ('parent', 'collector_id', 'collector', 'request_id'), 'delete_collector': ('name', 'request_id'), 'get_annotation': ('name',), 'get_collector': ('name',), 'list_collectors': ('parent', 'page_size', 'page_token', 'filter', 'order_by'), 'pause_collector': ('name', 'request_id'), 'register_collector': ('name', 'request_id'), 'resume_collector': ('name', 'request_id'), 'update_collector': ('update_mask', 'collector', 'request_id')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=rapidmigrationassessmentCallTransformer()):
    if False:
        while True:
            i = 10
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Fix up source that uses the rapidmigrationassessment client library.\n\nThe existing sources are NOT overwritten but are copied to output_dir with changes made.\n\nNote: This tool operates at a best-effort level at converting positional\n      parameters in client method calls to keyword based parameters.\n      Cases where it WILL FAIL include\n      A) * or ** expansion in a method call.\n      B) Calls via function or method alias (includes free function calls)\n      C) Indirect or dispatched calls (e.g. the method is looked up dynamically)\n\n      These all constitute false negatives. The tool will also detect false\n      positives when an API method shares a name with another method.\n')
