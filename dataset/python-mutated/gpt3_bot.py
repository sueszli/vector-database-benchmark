...
class GPT3Bot(ChatHistoryMixin, BaseBot):
    ...
    def from_config(cls, config):
        if False:
            for i in range(10):
                print('nop')
        return cls(openai_key=config.openai_key, api_base=config.api_base, proxy=config.proxy)

    async def ask(self, query, **options):
        data = {'prompt': query, 'model': 'text-davinci-003', 'max_tokens': 1024, 'temperature': 1, 'top_p': 1, **options}
    ...
    async def ask_stream(self, query, **options):
        data = {'prompt': query, 'model': 'text-davinci-003', 'max_tokens': 1024, 'temperature': 1, 'top_p': 1, 'stream': True, **options}
        async with httpx.AsyncClient(trust_env=True, proxies=self.proxy) as sess:
            client = openai.AsyncOpenAI(api_key=self.openai_key, http_client=sess, base_url=self.api_base)
            try:
                completion = await client.completions.create(**data)
