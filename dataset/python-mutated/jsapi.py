...
class WeChatJSAPI(BaseWeChatAPI):
    ...
    def get_jsapi_signature(self, noncestr, ticket, timestamp, url):
        ...
        data = [f'noncestr={noncestr}', f'jsapi_ticket={ticket}', f'timestamp={timestamp}', f'url={url}']
    ...
    def get_jsapi_add_card_params(self, card_id: str, code: str='', openid: str='', fixed_begintimestamp: Optional[int]=None, outer_str: str='', nonce_str: str='', timestamp: int=0, card_ticket: str='') -> JsApiCardExt:
        if False:
            i = 10
            return i + 15
        ...
        card_signature_dict = {'nonce_str': nonce_str, 'api_ticket': card_ticket, 'timestamp': str(timestamp), 'code': code, 'openid': openid, 'card_id': card_id}
        list_before_sign = sorted([str(x) for x in card_signature_dict.values()])
        str_to_sign = ''.join(list_before_sign).encode()
        card_ext = JsApiCardExt(code=code, openid=openid, timestamp=str(timestamp), fixed_begintimestamp=fixed_begintimestamp, outer_str=outer_str, nonce_str=nonce_str, signature=hashlib.sha1(str_to_sign).hexdigest())
        return card_ext