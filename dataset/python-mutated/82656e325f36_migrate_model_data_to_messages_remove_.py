...
def upgrade() -> None:
    ...
    op.add_column('message', sa.Column('message_type', sa.String(), nullable=False, server_default='content'))
    ...
    op.execute('UPDATE message SET provider = (SELECT provider FROM conversation WHERE conversation.id = message.conversation_id), model = (SELECT model FROM conversation WHERE conversation.id = message.conversation_id), preset = (SELECT preset FROM conversation WHERE conversation.id = message.conversation_id);')
