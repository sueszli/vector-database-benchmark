...
from Orange.regression.linear import LassoRegressionLearner, LinearRegressionLearner, RidgeRegressionLearner, ElasticNetLearner
...
class OWLinearRegression(OWBaseLearner):
    ...
    description = 'A linear regression algorithm with optional L1 (LASSO), L2 (ridge) or L1L2 (elastic net) regularization.'
    ...
    REGULARIZATION_TYPES = ['No regularization', 'Ridge regression (L2)', 'Lasso regression (L1)', 'Elastic net regression']
    ...
    alphas = list(chain([x / 10000 for x in range(1, 10)], [x / 1000 for x in range(1, 20)], [x / 100 for x in range(2, 20)], [x / 10 for x in range(2, 9)], range(1, 20), range(20, 100, 5), range(100, 1001, 100)))
