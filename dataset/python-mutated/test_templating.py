...
from prefect.utilities.templating import PlaceholderType, apply_values, find_placeholders, resolve_block_document_references, resolve_variables
...
class TestResolveBlockDocumentReferences:
    ...
    async def test_resolve_block_document_references_with_no_block_document_references(self):
        assert await resolve_block_document_references({'key': 'value'}) == {'key': 'value'}

    async def test_resolve_block_document_references_with_one_block_document_reference(self, prefect_client, block_document_id):
        assert {'key': {'a': 1, 'b': 'hello'}} == await resolve_block_document_references({'key': {'$ref': {'block_document_id': block_document_id}}}, client=prefect_client)

    async def test_resolve_block_document_references_with_nested_block_document_references(self, prefect_client, block_document_id):
        template = {'key': {'nested_key': {'$ref': {'block_document_id': block_document_id}}, 'other_nested_key': {'$ref': {'block_document_id': block_document_id}}}}
        block_document = await prefect_client.read_block_document(block_document_id)
        result = await resolve_block_document_references(template, client=prefect_client)
        assert result == {'key': {'nested_key': block_document.data, 'other_nested_key': block_document.data}}

    async def test_resolve_block_document_references_with_list_of_block_document_references(self, prefect_client, block_document_id):
        template = [{'$ref': {'block_document_id': block_document_id}}]
        block_document = await prefect_client.read_block_document(block_document_id)
        result = await resolve_block_document_references(template, client=prefect_client)
        assert result == [block_document.data]

    async def test_resolve_block_document_references_with_dot_delimited_syntax(self, prefect_client, block_document_id):
        template = {'key': '{{ prefect.blocks.arbitraryblock.arbitrary-block }}'}
        block_document = await prefect_client.read_block_document(block_document_id)
        result = await resolve_block_document_references(template, client=prefect_client)
        assert result == {'key': block_document.data}

    async def test_resolve_block_document_references_raises_on_multiple_placeholders(self, prefect_client):
        template = {'key': '{{ prefect.blocks.arbitraryblock.arbitrary-block }} {{ another_placeholder }}'}
        with pytest.raises(ValueError, match='Only a single block placeholder is allowed in a string and no surrounding text is allowed.'):
            await resolve_block_document_references(template, client=prefect_client)

    async def test_resolve_block_document_references_raises_on_extra_text(self, prefect_client):
        template = {'key': '{{ prefect.blocks.arbitraryblock.arbitrary-block }} extra text'}
        with pytest.raises(ValueError, match='Only a single block placeholder is allowed in a string and no surrounding text is allowed.'):
            await resolve_block_document_references(template, client=prefect_client)

    async def test_resolve_block_document_references_does_not_change_standard_placeholders(self):
    ...
    async def test_resolve_block_document_unpacks_system_blocks(self):
        ...
        template = {'json': '{{ prefect.blocks.json.json-block }}', 'secret': '{{ prefect.blocks.secret.secret-block }}', 'datetime': '{{ prefect.blocks.date-time.datetime-block }}', 'string': '{{ prefect.blocks.string.string-block }}'}
        result = await resolve_block_document_references(template)
        assert result == {'json': {'key': 'value'}, 'secret': 'N1nj4C0d3rP@ssw0rd!', 'datetime': '2020-01-01T00:00:00', 'string': 'hello'}
...
class TestResolveVariables:
    ...
    async def test_resolve_string_with_standard_placeholder(self, variables, prefect_client: PrefectClient):
        template = 'This is a string with a placeholder: {{ prefect.variables.test_variable_1 }}.'
        expected = 'This is a string with a placeholder: test_value_1.'
        result = await resolve_variables(template, client=prefect_client)
        assert result == expected

    async def test_resolve_string_with_multiple_standard_placeholders(self, variables, prefect_client: PrefectClient):
        template = '{{ prefect.variables.test_variable_1}} - {{ prefect.variables.test_variable_2 }}'
        expected = 'test_value_1 - test_value_2'
        result = await resolve_variables(template, client=prefect_client)
        assert result == expected

    async def test_resolve_dict(self, variables, prefect_client: PrefectClient):
        template: Dict[str, Any] = {'key1': 'value1', 'key2': '{{ prefect.variables.test_variable_1}}'}
        expected = {'key1': 'value1', 'key2': 'test_value_1'}
        result = await resolve_variables(template, client=prefect_client)
        assert result == expected

    async def test_resolve_nested_dict(self, variables, prefect_client: PrefectClient):
        template: Dict[str, Any] = {'key1': 'value1', 'key2': '{{ prefect.variables.test_variable_1}}', 'key3': {'key4': '{{ prefect.variables.test_variable_2}}'}}
        expected = {'key1': 'value1', 'key2': 'test_value_1', 'key3': {'key4': 'test_value_2'}}
        result = await resolve_variables(template, client=prefect_client)
        assert result == expected
    ...
    async def test_resolve_does_not_template_other_placeholder_types(self, prefect_client: PrefectClient):
        template = {'key': '{{ another_placeholder }}', 'key2': '{{ prefect.blocks.arbitraryblock.arbitrary-block }}', 'key3': '{{ $another_placeholder }}'}
        result = await resolve_variables(template, client=prefect_client)
        assert result == template

    async def test_resolve_clears_placeholder_for_missing_variable(self, prefect_client: PrefectClient):
        template = '{{ prefect.variables.missing_variable }}'
        result = await resolve_variables(template, client=prefect_client)
        assert result == ''

    async def test_resolve_clears_placeholders_for_missing_variables(self, prefect_client: PrefectClient):
        template = '{{ prefect.variables.missing_variable_1 }} - {{ prefect.variables.missing_variable_2 }}'
        result = await resolve_variables(template, client=prefect_client)
        assert result == ' - '