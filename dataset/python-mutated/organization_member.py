...
class OrganizationMemberViewSet(StructuredViewSetMixin, mixins.DestroyModelMixin, mixins.UpdateModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    serializer_class = OrganizationMemberSerializer
    permission_classes = [IsAuthenticated, OrganizationMemberPermissions, OrganizationMemberObjectPermissions]
    queryset = OrganizationMembership.objects.order_by('user__first_name', '-joined_at').exclude(user__email__endswith=INTERNAL_BOT_EMAIL_SUFFIX).filter(user__is_active=True).select_related('user').prefetch_related(Prefetch('user__totpdevice_set', queryset=TOTPDevice.objects.filter(name='default')), Prefetch('user__social_auth', queryset=UserSocialAuth.objects.all()))
