...
if __name__ == '__main__':
    ...
    config = get_trainable_cls(args.run).get_default_config().environment('ray.rllib.examples.env.random_env.RandomEnv', env_config={'config': {'observation_space': Dict({'a': Discrete(2), 'b': Dict({'ba': Discrete(3), 'bb': Box(-1.0, 1.0, (2, 3), dtype=np.float32)}), 'c': Tuple((MultiDiscrete([2, 3]), Discrete(2))), 'd': Box(-1.0, 1.0, (2,), dtype=np.int32)})}}).framework(args.framework).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', 0))).experimental(_disable_preprocessor_api=True)
