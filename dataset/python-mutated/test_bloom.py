...
async def test_bf_insert(decoded_r: redis.Redis):
    ...
    assert_resp_response(decoded_r, 2, info.get('insertedNum'), info.get('Number of items inserted'))
    assert_resp_response(decoded_r, 1000, info.get('capacity'), info.get('Capacity'))
...
async def test_bf_info(decoded_r: redis.Redis):
    ...
    assert_resp_response(decoded_r, None, info.get('expansionRate'), info.get('Expansion rate'))
    await decoded_r.bf().create('expanding', '0.0001', '1000', expansion=expansion)
    info = await decoded_r.bf().info('expanding')
    assert_resp_response(decoded_r, 4, info.get('expansionRate'), info.get('Expansion rate'))
    try:
        await decoded_r.bf().create('myBloom', '0.0001', '1000', expansion=expansion, noScale=True)
        assert False
    except RedisError:
        assert True
...
async def test_cf_add_and_insert(decoded_r: redis.Redis):
    ...
    assert_resp_response(decoded_r, 5, info.get('insertedNum'), info.get('Number of items inserted'))
    assert_resp_response(decoded_r, 0, info.get('deletedNum'), info.get('Number of items deleted'))
    assert_resp_response(decoded_r, 1, info.get('filterNum'), info.get('Number of filters'))
...
async def test_topk(decoded_r: redis.Redis):
    ...
    assert [None, None, None, 'A', 'C', 'D', None, None, 'E', None, 'B', 'C', None, None, None, 'D', None] == await decoded_r.topk().add('topk', 'A', 'B', 'C', 'D', 'E', 'A', 'A', 'B', 'C', 'G', 'D', 'B', 'D', 'A', 'E', 'E', 1)
    assert [1, 1, 0, 0, 1, 0, 0] == await decoded_r.topk().query('topk', 'A', 'B', 'C', 'D', 'E', 'F', 'G')
    with pytest.deprecated_call():
        assert [4, 3, 2, 3, 3, 0, 1] == await decoded_r.topk().count('topk', 'A', 'B', 'C', 'D', 'E', 'F', 'G')
    assert await decoded_r.topk().reserve('topklist', 3, 50, 3, 0.9)
    assert await decoded_r.topk().add('topklist', 'A', 'B', 'C', 'D', 'E', 'A', 'A', 'B', 'C', 'G', 'D', 'B', 'D', 'A', 'E', 'E')
...
async def test_topk_incrby(decoded_r: redis.Redis):
    ...
    assert [None, None, None] == await decoded_r.topk().incrby('topk', ['bar', 'baz', '42'], [3, 6, 2])
    res = await decoded_r.topk().incrby('topk', ['42', 'xyzzy'], [8, 4])
    assert [None, 'bar'] == res
    with pytest.deprecated_call():
        assert [3, 6, 10, 4, 0] == await decoded_r.topk().count('topk', 'bar', 'baz', '42', 'xyzzy', 4)
...
async def test_tdigest_merge(decoded_r: redis.Redis):
    ...
    assert await decoded_r.tdigest().merge('to-tDigest', 2, 'from-override', 'from-override-2', override=True)
...
async def test_tdigest_quantile(decoded_r: redis.Redis):
    ...
    assert await decoded_r.tdigest().max('tDigest') == (await decoded_r.tdigest().quantile('tDigest', 1))[0]
    assert await decoded_r.tdigest().min('tDigest') == (await decoded_r.tdigest().quantile('tDigest', 0.0))[0]
