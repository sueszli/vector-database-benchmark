...
VALIDATE_CREDENTIAL = {'model_version': 'fake-version', 'replicate_api_token': 'valid_key'}
...
def version_effect(id: str):
    ...
    mock_version.openapi_schema = {'components': {'schemas': {'Output': {'items': {'type': 'string'}}}}}
    return mock_version
...
def test_is_credentials_valid_or_raise_invalid():
    ...
    with pytest.raises(CredentialsValidateFailedError):
        MODEL_PROVIDER_CLASS.is_model_credentials_valid_or_raise(model_name='test_model_name', model_type=ModelType.TEXT_GENERATION, credentials={'replicate_api_token': 'invalid_key'})
