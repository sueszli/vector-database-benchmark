...
class XCLIPVisionModelTester:
    ...
    def __init__(self, parent, batch_size=8, image_size=30, patch_size=2, num_channels=3, num_frames=8, is_training=True, hidden_size=32, num_hidden_layers=2, num_attention_heads=4, intermediate_size=37, mit_hidden_size=64, dropout=0.1, attention_dropout=0.1, initializer_range=0.02, scope=None):
        if False:
            return 10
        self.parent = parent
    ...
    def get_config(self):
        if False:
            while True:
                i = 10
        return XCLIPVisionConfig(image_size=self.image_size, patch_size=self.patch_size, num_channels=self.num_channels, num_frames=self.num_frames, hidden_size=self.hidden_size, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, mit_hidden_size=self.mit_hidden_size, dropout=self.dropout, attention_dropout=self.attention_dropout, initializer_range=self.initializer_range)
    ...
    def create_and_check_model(self, config, pixel_values):
        ...
        self.parent.assertEqual(result.last_hidden_state.shape, (self.batch_size * self.num_frames, num_patches + 1, self.hidden_size))
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            with torch.no_grad():
                outputs = model(**self._prepare_for_class(inputs_dict, model_class))
            self.assertEqual(len(outputs.attentions), self.model_tester.num_hidden_layers)
            self.assertListEqual(list(outputs.attentions[0].shape[-3:]), [self.model_tester.num_attention_heads, encoder_seq_length, encoder_seq_length])
            out_len = len(outputs)
            inputs_dict['output_attentions'] = True
...
class XCLIPTextModelTester:
    ...
    def __init__(self, parent, batch_size=8, seq_length=7, is_training=True, use_input_mask=True, use_labels=True, vocab_size=99, hidden_size=32, num_hidden_layers=2, num_attention_heads=4, intermediate_size=37, dropout=0.1, attention_dropout=0.1, max_position_embeddings=512, initializer_range=0.02, scope=None):
        if False:
            while True:
                i = 10
        self.parent = parent
...
class XCLIPModelTester:
    ...
    def __init__(self, parent, text_kwargs=None, vision_kwargs=None, projection_dim=64, mit_hidden_size=64, is_training=True):
        if False:
            for i in range(10):
                print('nop')
    ...
    def prepare_config_and_inputs(self):
        ...
        pixel_values = floats_tensor([self.vision_model_tester.batch_size, self.vision_model_tester.num_frames, self.vision_model_tester.num_channels, self.vision_model_tester.image_size, self.vision_model_tester.image_size])
        config = self.get_config()
    ...
    def get_config(self):
        if False:
            i = 10
            return i + 15
        return XCLIPConfig.from_text_vision_configs(self.text_model_tester.get_config(), self.vision_model_tester.get_config(), projection_dim=self.projection_dim)
    ...
    def create_and_check_model(self, config, input_ids, attention_mask, pixel_values):
        ...
        self.parent.assertEqual(result.logits_per_video.shape, (self.vision_model_tester.batch_size, self.text_model_tester.batch_size))
...
def prepare_video():
    if False:
        for i in range(10):
            print('nop')
    file = hf_hub_download(repo_id='hf-internal-testing/spaghetti-video', filename='eating_spaghetti_8_frames.npy', repo_type='dataset')
    video = np.load(file)
    return list(video)
...
class XCLIPModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        video = prepare_video()
        inputs = processor(text=['playing sports', 'eating spaghetti', 'go shopping'], videos=video, return_tensors='pt', padding=True).to(torch_device)
        with torch.no_grad():
            outputs = model(**inputs)
        self.assertEqual(outputs.logits_per_video.shape, torch.Size((inputs.pixel_values.shape[0], inputs.input_ids.shape[0])))
        ...
        self.assertTrue(torch.allclose(outputs.logits_per_video, expected_logits, atol=0.001))