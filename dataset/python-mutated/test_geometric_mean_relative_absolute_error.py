...
def test_zero_sample():
    ...
    with pytest.raises(NotComputableError, match='GeometricMeanRelativeAbsoluteError must have at least one example before it can be computed'):
        m.compute()
...
def _test_distrib_compute(device):
    ...
    def _test(metric_device):
        ...
        np_gmrae = np.exp(np.log(np.abs(np_y - np_y_pred) / np.abs(np_y - np_y.mean())).mean())
        assert m.compute() == pytest.approx(np_gmrae, rel=0.0001)
    for i in range(3):
...
def _test_distrib_integration(device):
    ...
    def _test(n_epochs, metric_device):
        ...
        np_gmrae = np.exp(np.log(np.abs(np_y - np_y_pred) / np.abs(np_y - np_y.mean())).mean())
        assert pytest.approx(res, rel=0.0001) == np_gmrae
    metric_devices = ['cpu']
    if device.type != 'xla':
