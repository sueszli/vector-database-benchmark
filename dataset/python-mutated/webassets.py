...
class LessImportRewrite(UrlRewriter):
    ...
    patterns = {'import_rewrite': re.compile('(@import(\\s+\\(.*\\))?\\s+)\\"(.*)\\";')}
...
class SourceMapRewrite(UrlRewriter):
    ...
    patterns = {'url_rewrite': re.compile('(//#\\s+sourceMappingURL=)(.*)')}
...
class SourceMapRemove(PatternRewriter):
    ...
    patterns = {'sourcemap_remove': re.compile('(//#\\s+sourceMappingURL=)(.*)')}
...
class RJSMinExtended(Filter):
    ...
    options = {'keep_bang_comments': 'RJSMIN_KEEP_BANG_COMMENTS'}
