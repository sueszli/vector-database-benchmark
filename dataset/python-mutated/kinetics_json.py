...
def convert_csv_to_dict(csv_path, subset):
    ...
    for i in range(data.shape[0]):
        row = data.iloc[i, :]
        basename = '%s_%s_%s' % (row['youtube_id'], '%06d' % row['time_start'], '%06d' % row['time_end'])
...
def convert_kinetics_csv_to_json(train_csv_path, val_csv_path, test_csv_path, video_dir_path, video_type, dst_json_path):
    if False:
        while True:
            i = 10
...
if __name__ == '__main__':
    ...
    parser.add_argument('dir_path', default=None, type=Path, help='Directory path including kinetics_train.csv, kinetics_val.csv, (kinetics_test.csv (optional))')
    parser.add_argument('n_classes', default=700, type=int, help='400, 600, or 700 (Kinetics-400, Kinetics-600, or Kinetics-700)')
    parser.add_argument('video_path', default=None, type=Path, help='Path of video directory (jpg or hdf5).Using to get n_frames of each video.')
    parser.add_argument('video_type', default='jpg', type=str, help='jpg or hdf5')
    parser.add_argument('dst_path', default=None, type=Path, help='Path of dst json file.')
    ...
    convert_kinetics_csv_to_json(train_csv_path, val_csv_path, test_csv_path, args.video_path, args.video_type, args.dst_path)