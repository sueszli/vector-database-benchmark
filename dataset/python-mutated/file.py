...
class TOMLFile(BaseTOMLFile):
    ...
    def __getattr__(self, item: str) -> Any:
        if False:
            i = 10
            return i + 15
        warnings.warn('`__getattr__` will be removed from the `TOMLFile` in a future release.\n\nInstead of accessing properties of the underlying `Path` as `tomlfile.whatever`, prefer `tomlfile.path.whatever`.', DeprecationWarning, stacklevel=2)
