...
import sys
import time
IDLE_RE = re.compile('^\\(idle\\) \\d+$')
FLAMEGRAPH_RE = re.compile('^((?:[^:]+:[^:]+:\\d+)(?:;[^:]+:[^:]+:\\d+)*) (\\d+)$')
FLAMEGRAPH_NONUMBER_RE = re.compile('^((?:[^:]+:[^:]+)(?:;[^:]+:[^:]+)*) (\\d+)$')
TS_IDLE_RE = re.compile('\\(idle\\)')
TS_FLAMEGRAPH_RE = re.compile('[^[^\\d]+\\d+;]*')
TS_RE = re.compile('\\d+')
SLEEP_A_RE = re.compile('.*:sleep_a:.*')
SLEEP_B_RE = re.compile('.*:sleep_b:.*')
