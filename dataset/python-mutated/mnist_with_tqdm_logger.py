...
def run(train_batch_size, val_batch_size, epochs, lr, momentum, display_gpu_info):
    ...
    def log_validation_results(engine):
        ...
        avg_accuracy = metrics['accuracy']
        avg_nll = metrics['nll']
        pbar.log_message(f'Validation Results - Epoch: {engine.state.epoch} Avg accuracy: {avg_accuracy:.2f} Avg loss: {avg_nll:.2f}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--display_gpu_info', action='store_true', help='Display gpu usage info. This needs python 3.X and pynvml package')
