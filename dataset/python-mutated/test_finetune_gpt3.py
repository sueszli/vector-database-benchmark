...
class TestFinetuneTextGeneration(DistributedTestCase):
    ...
    def test_finetune_dureader(self):
        ...
        def cfg_modify_fn(cfg):
            ...
            cfg.train.optimizer = {'type': 'AdamW', 'lr': 0.0001}
    ...
    def test_multi_finetune_portry(self):
        if False:
            i = 10
            return i + 15
        self.start(finetune_poetry, num_gpus=4, work_dir=self.tmp_dir, dp_tp=True)
...
def finetune_poetry(dp_tp=False):
    ...
    def cfg_modify_fn(cfg):
        ...
        cfg.train.hooks.append({'type': 'EvaluationHook', 'by_epoch': True, 'interval': 1})
        cfg.evaluation.dataloader = {'batch_size_per_gpu': 8, 'workers_per_gpu': 1}
