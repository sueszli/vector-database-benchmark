...
from airbyte_cdk.sources.file_based.exceptions import FileBasedSourceError, InvalidSchemaError, MissingSchemaError, NoFilesMatchingError, RecordParseError, SchemaInferenceError, StopSyncPerValidationPolicy
...
class DefaultFileBasedStream(AbstractFileBasedStream, IncrementalMixin):
    ...
    async def _infer_file_schema(self, file: RemoteFile) -> SchemaType:
        try:
            ...
            raise SchemaInferenceError(FileBasedSourceError.SCHEMA_INFERENCE_ERROR, file=file.uri, format=str(self.config.format), stream=self.name) from exc