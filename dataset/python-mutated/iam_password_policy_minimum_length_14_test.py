...
class Test_iam_password_policy_minimum_length_14:
    ...
    def test_iam_password_policy_minimum_length_equal_14(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_password_policy_minimum_length_14.iam_password_policy_minimum_length_14.iam_client', new=IAM(current_audit_info)):
            from prowler.providers.aws.services.iam.iam_password_policy_minimum_length_14.iam_password_policy_minimum_length_14 import iam_password_policy_minimum_length_14
            check = iam_password_policy_minimum_length_14()
            result = check.execute()
            assert len(result) == 1
            assert result[0].status == 'PASS'
            assert search('IAM password policy requires minimum length of 14 characters.', result[0].status_extended)
    ...
    def test_iam_password_policy_minimum_length_less_14(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_password_policy_minimum_length_14.iam_password_policy_minimum_length_14.iam_client', new=IAM(current_audit_info)):
            ...
            assert search('IAM password policy does not require minimum length of 14 characters.', result[0].status_extended)
            assert result[0].resource_id == AWS_ACCOUNT_NUMBER
            assert result[0].resource_arn == AWS_ACCOUNT_ARN
            assert result[0].region == AWS_REGION