...
from coconut.constants import PYPY, PURE_PYTHON, use_fast_pyparsing_reprs, use_packrat_parser, packrat_cache_size, default_whitespace_chars, varchars, min_versions, pure_python_env_var, enable_pyparsing_warnings, use_left_recursion_if_available, get_bool_env_var, use_computation_graph_env_var, use_incremental_if_available, incremental_cache_size, never_clear_incremental_cache, warn_on_multiline_regex
from coconut.util import get_clock_time
from coconut.util import ver_str_to_tuple, ver_tuple_to_str, get_next_version
try:
    if PURE_PYTHON:
...
if cur_ver is None or cur_ver < min_ver:
    raise ImportError('This version of Coconut requires pyparsing/cPyparsing version >= ' + min_ver_str + ('; got ' + PYPARSING_INFO if PYPARSING_INFO is not None else '') + " (run '{python} -m pip install --upgrade {package}' to fix)".format(python=sys.executable, package=PYPARSING_PACKAGE))
elif cur_ver >= max_ver:
    warn('This version of Coconut was built for pyparsing/cPyparsing versions < ' + max_ver_str + ('; got ' + PYPARSING_INFO if PYPARSING_INFO is not None else '') + " (run '{python} -m pip install {package}<{max_ver}' to fix)".format(python=sys.executable, package=PYPARSING_PACKAGE, max_ver=max_ver_str))
MODERN_PYPARSING = cur_ver >= (3,)
if MODERN_PYPARSING:
    warn('This version of Coconut is not built for pyparsing v3; some syntax features WILL NOT WORK' + " (run either '{python} -m pip install cPyparsing<{max_ver}' or '{python} -m pip install pyparsing<{max_ver}' to fix)".format(python=sys.executable, max_ver=max_ver_str))
if PYPARSING_PACKAGE != 'cPyparsing':
    ...
    raise ImportError('This version of Coconut requires cPyparsing>=' + ver_tuple_to_str(min_versions['cPyparsing']) + '; got cPyparsing==' + __version__ + " (run '{python} -m pip install --upgrade cPyparsing' to fix)".format(python=sys.executable))
if hasattr(ParserElement, 'enableIncremental'):
    SUPPORTS_INCREMENTAL = sys.version_info >= (3, 8)
else:
...
USE_COMPUTATION_GRAPH = get_bool_env_var(use_computation_graph_env_var, default=not MODERN_PYPARSING and (not PYPY))
...
if not hasattr(_pyparsing, 'python_quoted_string'):
    import re as _re
    python_quoted_string = _pyparsing.Combine((_pyparsing.Regex('"""(?:[^"\\\\]|""(?!")|"(?!"")|\\\\.)*', flags=_re.MULTILINE) + '"""').setName('multiline double quoted string') ^ (_pyparsing.Regex("'''(?:[^'\\\\]|''(?!')|'(?!'')|\\\\.)*", flags=_re.MULTILINE) + "'''").setName('multiline single quoted string') ^ (_pyparsing.Regex('"(?:[^"\\n\\r\\\\]|(?:\\\\")|(?:\\\\(?:[^x]|x[0-9a-fA-F]+)))*') + '"').setName('double quoted string') ^ (_pyparsing.Regex("'(?:[^'\\n\\r\\\\]|(?:\\\\')|(?:\\\\(?:[^x]|x[0-9a-fA-F]+)))*") + "'").setName('single quoted string')).setName('Python quoted string')
