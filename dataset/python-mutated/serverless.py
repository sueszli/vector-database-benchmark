...
from .awslambda import Cors, DestinationConfig, Environment, EphemeralStorage, FileSystemConfig, FilterCriteria, ImageConfig, ProvisionedConcurrencyConfiguration, RuntimeManagementConfig, SourceAccessConfiguration, VPCConfig, validate_memory_size, validate_package_type
...
from .validators import boolean, double, exactly_one, integer, integer_range, mutually_exclusive, positive_integer
try:
...
class DeadLetterQueue(AWSProperty):
    props: PropsDictType = {'Type': (str, False), 'TargetArn': (str, False)}
...
class S3Location(AWSProperty):
    props: PropsDictType = {'Bucket': (str, True), 'Key': (str, True), 'Version': (str, False)}

class Hooks(AWSProperty):
    props: PropsDictType = {'PreTraffic': (str, False), 'PostTraffic': (str, False)}

class DeploymentPreference(AWSProperty):
    props: PropsDictType = {'Type': (str, True), 'Alarms': (list, False), 'Hooks': (Hooks, False), 'Enabled': (bool, False), 'Role': (str, False)}

class EventInvokeDestination(AWSProperty):
    props: PropsDictType = {'Destination': (str, False), 'Type': (str, False)}
...
class DestinationConfiguration(AWSProperty):
    props: PropsDictType = {'OnFailure': (OnFailure, False), 'OnSuccess': (OnSuccess, False)}

class EventInvokeConfiguration(AWSProperty):
    props: PropsDictType = {'DestinationConfig': (DestinationConfiguration, False), 'MaximumEventAgeInSeconds': (integer, False), 'MaximumRetryAttempts': (integer, False)}
...
class CognitoAuthIdentity(AWSProperty):
    props: PropsDictType = {'Header': (str, False), 'ValidationExpression': (str, False)}

class LambdaTokenAuthIdentity(AWSProperty):
    props: PropsDictType = {'Header': (str, False), 'ValidationExpression': (str, False), 'ReauthorizeEvery': (str, False)}

class LambdaRequestAuthIdentity(AWSProperty):
    props: PropsDictType = {'Headers': ([str], False), 'QueryStrings': ([str], False), 'StageVariables': ([str], False), 'Context': ([str], False), 'ReauthorizeEvery': (str, False)}

class CognitoAuth(AWSProperty):
    props: PropsDictType = {'UserPoolArn': (str, False), 'Identity': (CognitoAuthIdentity, False)}

class LambdaTokenAuth(AWSProperty):
    props: PropsDictType = {'FunctionPayloadType': (str, False), 'FunctionArn': (str, False), 'FunctionInvokeRole': (str, False), 'Identity': (LambdaTokenAuthIdentity, False)}

class LambdaRequestAuth(AWSProperty):
    props: PropsDictType = {'FunctionPayloadType': (str, False), 'FunctionArn': (str, False), 'FunctionInvokeRole': (str, False), 'Identity': (LambdaRequestAuthIdentity, False)}

class Authorizers(AWSProperty):
    props: PropsDictType = {'DefaultAuthorizer': (str, False), 'CognitoAuth': (CognitoAuth, False), 'LambdaTokenAuth': (LambdaTokenAuth, False), 'LambdaRequestAuth': (LambdaRequestAuth, False)}

class ResourcePolicyStatement(AWSProperty):
    props: PropsDictType = {'AwsAccountBlacklist': (list, False), 'AwsAccountWhitelist': (list, False), 'CustomStatements': (list, False), 'IpRangeBlacklist': (list, False), 'IpRangeWhitelist': (list, False), 'SourceVpcBlacklist': (list, False), 'SourceVpcWhitelist': (list, False)}

class Auth(AWSProperty):
    props: PropsDictType = {'AddDefaultAuthorizerToCorsPreflight': (bool, False), 'ApiKeyRequired': (bool, False), 'Authorizers': (Authorizers, False), 'DefaultAuthorizer': (str, False), 'InvokeRole': (str, False), 'ResourcePolicy': (ResourcePolicyStatement, False)}

class Cors(AWSProperty):
    props: PropsDictType = {'AllowCredentials': (str, False), 'AllowHeaders': (str, False), 'AllowMethods': (str, False), 'AllowOrigin': (str, True), 'MaxAge': (str, False)}

class Route53(AWSProperty):
    props: PropsDictType = {'DistributionDomainName': (str, False), 'EvaluateTargetHealth': (bool, False), 'HostedZoneId': (str, False), 'HostedZoneName': (str, False), 'IpV6': (bool, False)}
...
class Domain(AWSProperty):
    props: PropsDictType = {'BasePath': (list, False), 'CertificateArn': (str, True), 'DomainName': (str, True), 'EndpointConfiguration': (str, False), 'Route53': (Route53, False)}
...
class OAuth2Authorizer(AWSProperty):
    props: PropsDictType = {'AuthorizationScopes': (list, False), 'IdentitySource': (str, False), 'JwtConfiguration': (dict, False)}

class LambdaAuthorizationIdentity(AWSProperty):
    props: PropsDictType = {'Context': (list, False), 'Headers': (list, False), 'QueryStrings': (list, False), 'ReauthorizeEvery': (integer, False), 'StageVariables': (list, False)}

class LambdaAuthorizer(AWSProperty):
    props: PropsDictType = {'AuthorizerPayloadFormatVersion': (str, True), 'EnableSimpleResponses': (boolean, False), 'FunctionArn': (str, True), 'FunctionInvokeRole': (str, False), 'Identity': (LambdaAuthorizationIdentity, False)}

class HttpApiAuth(AWSProperty):
    props: PropsDictType = {'Authorizers': ((OAuth2Authorizer, LambdaAuthorizer), False), 'DefaultAuthorizer': (str, False)}

class HttpApiCorsConfiguration(AWSProperty):
    props: PropsDictType = {'AllowCredentials': (boolean, False), 'AllowHeaders': (list, False), 'AllowMethods': (list, False), 'AllowOrigins': (list, False), 'ExposeHeaders': (list, False), 'MaxAge': (integer, False)}
...
class PrimaryKey(AWSProperty):
    props: PropsDictType = {'Name': (str, False), 'Type': (primary_key_type_validator, False)}

class SimpleTable(AWSObject):
    resource_type = 'AWS::Serverless::SimpleTable'
    props: PropsDictType = {'PrimaryKey': (PrimaryKey, False), 'ProvisionedThroughput': (ProvisionedThroughput, False), 'SSESpecification': (SSESpecification, False), 'Tags': (dict, False), 'TableName': (str, False)}

class LayerVersion(AWSObject):
    resource_type = 'AWS::Serverless::LayerVersion'
    props: PropsDictType = {'CompatibleArchitectures': ([str], False), 'CompatibleRuntimes': ([str], False), 'ContentUri': ((S3Location, str), True), 'Description': (str, False), 'LayerName': (str, False), 'LicenseInfo': (str, False), 'RetentionPolicy': (str, False)}

class S3Event(AWSObject):
    resource_type = 'S3'
    props: PropsDictType = {'Bucket': (str, True), 'Events': (list, True), 'Filter': (Filter, False)}

class SNSEvent(AWSObject):
    resource_type = 'SNS'
    props: PropsDictType = {'FilterPolicy': (dict, False), 'Region': (str, False), 'SqsSubscription': (bool, False), 'Topic': (str, True)}
...
class KinesisEvent(AWSObject):
    ...
    props: PropsDictType = {'Stream': (str, True), 'StartingPosition': (starting_position_validator, True), 'BatchSize': (positive_integer, False), 'BisectBatchOnFunctionError': (bool, False), 'DestinationConfig': (DestinationConfig, False), 'Enabled': (bool, False), 'MaximumBatchingWindowInSeconds': (positive_integer, False), 'MaximumRecordAgeInSeconds': (integer_range(60, 604800), False), 'MaximumRetryAttempts': (positive_integer, False), 'ParallelizationFactor': (integer_range(1, 10), False)}

class DynamoDBEvent(AWSObject):
    resource_type = 'DynamoDB'
    props: PropsDictType = {'Stream': (str, True), 'StartingPosition': (starting_position_validator, True), 'BatchSize': (positive_integer, False)}

class ApiFunctionAuth(AWSProperty):
    props: PropsDictType = {'ApiKeyRequired': (bool, False), 'AuthorizationScopes': (list, False), 'Authorizer': (str, False), 'InvokeRole': (str, False), 'ResourcePolicy': (ResourcePolicyStatement, False)}

class RequestModel(AWSProperty):
    props: PropsDictType = {'Model': (str, True), 'Required': (bool, False), 'ValidateBody': (bool, False), 'ValidateParameters': (bool, False)}
...
class ApiEvent(AWSObject):
    resource_type = 'Api'
    props: PropsDictType = {'Auth': (api_function_auth_validator, False), 'Path': (str, True), 'Method': (str, True), 'RequestModel': (RequestModel, False), 'RequestParameters': (str, False), 'RestApiId': (str, False)}

class ScheduleEvent(AWSObject):
    resource_type = 'Schedule'
    props: PropsDictType = {'Schedule': (str, True), 'Input': (str, False), 'Description': (str, False), 'Enabled': (bool, False), 'Name': (str, False)}

class DeadLetterConfig(AWSProperty):
    props: PropsDictType = {'Arn': (str, False), 'QueueLogicalId': (str, False), 'Type': (str, False)}

class FlexibleTimeWindow(AWSProperty):
    props: PropsDictType = {'MaximumWindowInMinutes': (double, False), 'Mode': (str, True)}

class RetryPolicy(AWSProperty):
    props: PropsDictType = {'MaximumEventAgeInSeconds': (double, False), 'MaximumRetryAttempts': (double, False)}
...
class CloudWatchEvent(AWSObject):
    ...
    props: PropsDictType = {'Pattern': (dict, True), 'Input': (str, False), 'InputPath': (str, False)}

class IoTRuleEvent(AWSObject):
    resource_type = 'IoTRule'
    props: PropsDictType = {'Sql': (str, True), 'AwsIotSqlVersion': (str, False)}
...
class MSKEvent(AWSObject):
    resource_type = 'MSK'
    props: PropsDictType = {'ConsumerGroupId': (str, False), 'FilterCriteria': (FilterCriteria, False), 'MaximumBatchingWindowInSeconds': (integer, False), 'SourceAccessConfigurations': ([SourceAccessConfiguration], False), 'StartingPosition': (str, True), 'StartingPositionTimestamp': (double, False), 'Stream': (str, True), 'Topics': ([str], True)}

class SelfManagedKafkaEvent(AWSObject):
    resource_type = 'SelfManagedKafka'
    props: PropsDictType = {'BatchSize': (integer, False), 'ConsumerGroupId': (str, False), 'Enabled': (bool, False), 'FilterCriteria': (FilterCriteria, False), 'KafkaBootstrapServers': ([str], False), 'SourceAccessConfigurations': ([SourceAccessConfiguration], True), 'Topics': ([str], True)}

class MQEvent(AWSObject):
    resource_type = 'MQ'
    props: PropsDictType = {'BatchSize': (integer, False), 'Broker': (str, True), 'DynamicPolicyName': (bool, False), 'Enabled': (bool, False), 'FilterCriteria': (FilterCriteria, False), 'MaximumBatchingWindowInSeconds': (integer, False), 'Queues': ([str], True), 'SecretsManagerKmsKeyId': (str, False), 'SourceAccessConfigurations': ([str], True)}

class DocumentDBEvent(AWSObject):
    resource_type = 'DocumentDB'
    props: PropsDictType = {'BatchSize': (integer, False), 'Cluster': (str, True), 'CollectionName': (str, False), 'DatabaseName': (str, True), 'Enabled': (bool, False), 'FilterCriteria': (FilterCriteria, False), 'FullDocument': (str, False), 'MaximumBatchingWindowInSeconds': (integer, False), 'SecretsManagerKmsKeyId': (str, False), 'SourceAccessConfigurations': ([str], True), 'StartingPosition': (str, True), 'StartingPositionTimestamp': (double, False)}
...
class EventBridgeRuleEvent(AWSObject):
    ...
    props: PropsDictType = {'DeadLetterConfig': (DeadLetterConfig, False), 'EventBusName': (str, False), 'Input': (str, False), 'InputPath': (str, False), 'Name': (str, False), 'Pattern': (dict, True), 'RetryPolicy': (RetryPolicy, False), 'State': (str, False), 'Target': (Target, False)}

class ApplicationLocation(AWSProperty):
    props: PropsDictType = {'ApplicationId': (str, True), 'SemanticVersion': (str, True)}

class Application(AWSObject):
    resource_type = 'AWS::Serverless::Application'
    props: PropsDictType = {'Location': ((ApplicationLocation, str), True), 'NotificationARNs': ([str], False), 'Parameters': (dict, False), 'Tags': (dict, False), 'TimeoutInMinutes': (positive_integer, False)}
...
class StateMachine(AWSObject):
    ...
    props: PropsDictType = {'Definition': (dict, False), 'DefinitionUri': ((S3Location, str), False), 'DefinitionSubstitutions': (dict, False), 'Events': (dict, False), 'Logging': (LoggingConfiguration, False), 'Name': (str, False), 'PermissionsBoundary': (str, False), 'Policies': (policytypes, False), 'Role': (str, False), 'Tags': (dict, False), 'TracingConfiguration': (TracingConfiguration, False), 'Type': (str, False)}
...
class HttpApiGlobals(AWSProperty):
    props: PropsDictType = {'AccessLogSettings': (AccessLogSettings, False), 'Auth': (HttpApiAuth, False), 'StageVariables': (dict, False), 'Tags': (dict, False)}

class SimpleTableGlobals(AWSProperty):
    props: PropsDictType = {'SSESpecification': (SSESpecification, False)}
...
class Globals(AWSProperty):
    ...
    """
    props: PropsDictType = {'Api': (ApiGlobals, False), 'Function': (FunctionGlobals, False), 'HttpApi': (HttpApiGlobals, False), 'SimpleTable': (SimpleTableGlobals, False)}