...
class BlobExchangeTestBase(AsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.server_config = Config(data_dir=self.server_dir, download_dir=self.server_dir, wallet=self.server_dir, save_files=True, fixed_peers=[])
        ...
        self.client_config = Config(data_dir=self.client_dir, download_dir=self.client_dir, wallet=self.client_wallet_dir, save_files=True, fixed_peers=[], tracker_servers=[])
...
class TestBlobExchange(BlobExchangeTestBase):
    ...
    async def _test_transfer_blob(self, blob_hash: str):
        ...
        (downloaded, transport) = await request_blob(self.loop, client_blob, self.server_from_client.address, self.server_from_client.tcp_port, 2, 3)
    ...
    async def test_host_same_blob_to_multiple_peers_at_once(self):
        ...
        second_client_blob_manager = BlobManager(self.loop, second_client_dir, second_client_storage, second_client_conf)
        ...
        await asyncio.gather(request_blob(self.loop, second_client_blob, server_from_second_client.address, server_from_second_client.tcp_port, 2, 3), self._test_transfer_blob(blob_hash))
    ...
    async def test_host_different_blobs_to_multiple_peers_at_once(self):
        ...
        await asyncio.gather(request_blob(self.loop, second_client_blob, server_from_second_client.address, server_from_second_client.tcp_port, 2, 3), self._test_transfer_blob(sd_hash), second_client_blob.verified.wait())
        self.assertTrue(second_client_blob.get_is_verified())
    ...
    async def test_idle_timeout(self):
        ...
        await self._add_blob_to_server(blob_hash, mock_blob_bytes)
        client_blob = self.client_blob_manager.get_blob(blob_hash)
        (downloaded, protocol) = await request_blob(self.loop, client_blob, self.server_from_client.address, self.server_from_client.tcp_port, 2, 3)
        ...
        (downloaded, protocol2) = await request_blob(self.loop, client_blob, self.server_from_client.address, self.server_from_client.tcp_port, 2, 3, connected_protocol=protocol)
    ...
    async def test_transfer_timeout(self):
        ...
        with self.assertRaises(asyncio.CancelledError):
            await request_blob(self.loop, client_blob, self.server_from_client.address, self.server_from_client.tcp_port, 2, 3)
    ...
    async def test_download_blob_using_jsonrpc_blob_get(self):
        ...
        with mock.patch('lbry.extras.daemon.componentmanager.ComponentManager.all_components_running', return_value=True):
            with mock.patch('lbry.extras.daemon.daemon.Daemon.dht_node', new_callable=mock.PropertyMock) as daemon_mock_dht:
                with mock.patch('lbry.extras.daemon.daemon.Daemon.blob_manager', new_callable=mock.PropertyMock) as daemon_mock_blob_manager:
