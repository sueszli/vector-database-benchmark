...
class BasePresenter:
    ...
    async def present_resource(self, resource, user, **kwargs):
        ...
        async def present_lambda(r):
            if r and inspect.isawaitable(r):
                r = await r
            results = r.__class__.presenter_class()(r, user, **kwargs).present(**kwargs)
    ...
    async def present_model(self, model, resource_class, user, **kwargs):
        if model:
            return await self.present_resource(resource_class(model, user, **kwargs), user, **kwargs)

    @classmethod
    async def present_models(self, models, resource_class, user, **kwargs):
        return await self.present_resource(resource_class.build_result_set(models, user, **kwargs), user, **kwargs)
    ...
    async def present(self, present_options_by_key: Dict=None, **kwargs):
        ...
        async def _build(obj, key, object_to_present=object_to_present, present_options_by_key=present_options_by_key):
            ...
            if issubclass(value.__class__, list) or issubclass(value.__class__, UserList):
                obj[key] = [await self.__transform_value(key, v, **merge_dict(kwargs, options_for_key or {})) for v in value]
            else:
                obj[key] = await self.__transform_value(key, value, **merge_dict(kwargs, options_for_key or {}))
            return obj
    ...
    async def __transform_value(self, key, value, **kwargs):
        ...
        if issubclass(value.__class__, BaseModel):
            ...
            resource_class = getattr(importlib.import_module(f'mage_ai.api.resources.{resource_class_name}'), resource_class_name)
        ...
        if isinstance(value, datetime):
            ...
            data = await value.presenter_class().present_resource(value, self.current_user, **merge_dict(kwargs, opts))
            if not kwargs.get('ignore_permissions'):
                ...
                if parser_class:
                    parser = parser_class(resource=value, current_user=self.current_user, policy=policy, **opts)
                    data = await parser.parse_read_attributes_and_authorize(data, _build_authorize_attributes, **opts)
                    ...
                    await policy.authorize_attributes(READ, data.keys(), **opts)
            return data
        else:
            return value
