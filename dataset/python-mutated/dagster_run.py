...
from typing import TYPE_CHECKING, AbstractSet, Any, Dict, List, Mapping, NamedTuple, Optional, Sequence, Union
...
from .tags import BACKFILL_ID_TAG, REPOSITORY_LABEL_TAG, RESUME_RETRY_TAG, SCHEDULE_NAME_TAG, SENSOR_NAME_TAG
if TYPE_CHECKING:
...
class DagsterRunStatus(Enum):
    ...
    CANCELING = 'CANCELING'
    CANCELED = 'CANCELED'
IN_PROGRESS_RUN_STATUSES = [DagsterRunStatus.STARTING, DagsterRunStatus.STARTED, DagsterRunStatus.CANCELING]
NON_IN_PROGRESS_RUN_STATUSES = [DagsterRunStatus.QUEUED, DagsterRunStatus.NOT_STARTED, DagsterRunStatus.SUCCESS, DagsterRunStatus.FAILURE, DagsterRunStatus.MANAGED, DagsterRunStatus.CANCELED]
FINISHED_STATUSES = [DagsterRunStatus.SUCCESS, DagsterRunStatus.FAILURE, DagsterRunStatus.CANCELED]
...
class DagsterRunStatsSnapshot(NamedTuple('_DagsterRunStatsSnapshot', [('run_id', str), ('steps_succeeded', int), ('steps_failed', int), ('materializations', int), ('expectations', int), ('enqueued_time', Optional[float]), ('launch_time', Optional[float]), ('start_time', Optional[float]), ('end_time', Optional[float])])):
...
@whitelist_for_serdes(serializer=DagsterRunSerializer, storage_name='PipelineRun', old_fields={'mode': None}, storage_field_names={'job_name': 'pipeline_name', 'job_snapshot_id': 'pipeline_snapshot_id', 'external_job_origin': 'external_pipeline_origin', 'job_code_origin': 'pipeline_code_origin', 'op_selection': 'solid_selection', 'resolved_op_selection': 'solids_to_execute'})
class DagsterRun(NamedTuple('_DagsterRun', [('job_name', PublicAttr[str]), ('run_id', str), ('run_config', Mapping[str, object]), ('asset_selection', Optional[AbstractSet[AssetKey]]), ('asset_check_selection', Optional[AbstractSet[AssetCheckKey]]), ('op_selection', Optional[Sequence[str]]), ('resolved_op_selection', Optional[AbstractSet[str]]), ('step_keys_to_execute', Optional[Sequence[str]]), ('status', DagsterRunStatus), ('tags', Mapping[str, str]), ('root_run_id', Optional[str]), ('parent_run_id', Optional[str]), ('job_snapshot_id', Optional[str]), ('execution_plan_snapshot_id', Optional[str]), ('external_job_origin', Optional['ExternalJobOrigin']), ('job_code_origin', Optional[JobPythonOrigin]), ('has_repository_load_data', bool)])):
...
class RunsFilter(NamedTuple('_RunsFilter', [('run_ids', Sequence[str]), ('job_name', Optional[str]), ('statuses', Sequence[DagsterRunStatus]), ('tags', Mapping[str, Union[str, Sequence[str]]]), ('snapshot_id', Optional[str]), ('updated_after', Optional[datetime]), ('updated_before', Optional[datetime]), ('created_after', Optional[datetime]), ('created_before', Optional[datetime])])):
...
class RunRecord(NamedTuple('_RunRecord', [('storage_id', int), ('dagster_run', DagsterRun), ('create_timestamp', datetime), ('update_timestamp', datetime), ('start_time', Optional[float]), ('end_time', Optional[float])])):
...
class RunPartitionData(NamedTuple('_RunPartitionData', [('run_id', str), ('partition', str), ('status', DagsterRunStatus), ('start_time', Optional[float]), ('end_time', Optional[float])])):
...
@whitelist_for_serdes
class ExecutionSelector(NamedTuple('_ExecutionSelector', [('name', str), ('solid_subset', Optional[Sequence[str]])])):
