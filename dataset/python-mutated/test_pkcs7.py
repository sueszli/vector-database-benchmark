...
@pytest.mark.supported(only_if=lambda backend: backend.pkcs7_supported(), skip_message='Requires OpenSSL with PKCS7 support')
...
class TestPKCS7Builder:
    ...
    def test_rsa_pkcs_padding_options(self, pad, backend):
        ...
        if isinstance(pad, padding.PSS):
            ...
            assert sig.count(b'\x06\t*\x86H\x86\xf7\r\x01\x01\x08') == 1
        else:
            assert sig.count(b'\x06\t*\x86H\x86\xf7\r\x01\x01\r') == 1
