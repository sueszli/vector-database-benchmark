...
class TurbobitNet(SimpleDownloader):
    ...
    __version__ = '0.51'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:(?:www|m)\\.)?(?:(?:trbbt|turbo(?:beet|bite?))\\.net|(?:tourbobit|turbobi(?:tn?|f))\\.com|turbo?\\.(?:to|cc)|turb\\.pw)/(?:download/free/)?(?P<ID>\\w+)'
    ...
    __license__ = 'GPLv3'
    __authors__ = [('zoidberg', 'zoidberg@mujmail.cz'), ('prOq', None), ('GammaC0de', 'nitzo2001[AT]yahoo[DOT]com')]
    URL_REPLACEMENTS = [(__pattern__ + '.*', 'https://turbobit.net/\\g<ID>.html')]
    SIZE_REPLACEMENTS = [(' ', '')]
    COOKIES = [('turbobit.net', 'user_lang', 'en')]
    INFO_PATTERN = '<title>\\s*Download file (?P<N>.+?) \\((?P<S>[\\d.,]+) (?P<U>[\\w^_]+)\\)'
    OFFLINE_PATTERN = "<h2>File Not Found</h2>|html\\(\\'File (?:was )?not found"
    TEMP_OFFLINE_PATTERN = '^unmatchable$'
    LINK_FREE_PATTERN = '(/download/redirect/[^"\\\']+)'
    LINK_PREMIUM_PATTERN = '<a href=[\\\'"](.+?/download/redirect/[^"\\\']+)'
    LIMIT_WAIT_PATTERN = "<div id=\\'timeout\\'>(\\d+)<"

    def handle_free(self, pyfile):
        if False:
            while True:
                i = 10
        self.free_url = 'https://turbobit.net/download/free/{}'.format(self.info['pattern']['ID'])
        ...
        self.data = self.load('https://turbobit.net/download/getLinkTimeout/{}'.format(self.info['pattern']['ID']), ref=self.free_url)
        self.req.http.c.setopt(pycurl.HTTPHEADER, ['X-Requested-With:'])
        if '/download/started/' in self.data:
            self.data = self.load('https://turbobit.net/download/started/{}'.format(self.info['pattern']['ID']))
