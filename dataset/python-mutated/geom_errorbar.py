...
class geom_errorbar(geom):
    ...
    DEFAULT_PARAMS = {'stat': 'identity', 'position': 'identity', 'na_rm': False, 'width': 0.5}
    ...
    def draw_group(data: pd.DataFrame, panel_params: panel_view, coord: Coord, ax: Axes, **params: Any):
        if False:
            while True:
                i = 10
        f = np.hstack
        bars = pd.DataFrame({'x': f([data['xmin'], data['xmin'], data['x']]), 'xend': f([data['xmax'], data['xmax'], data['x']]), 'y': f([data['ymin'], data['ymax'], data['ymax']]), 'yend': f([data['ymin'], data['ymax'], data['ymin']])})
