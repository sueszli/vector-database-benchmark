...
from sentry.incidents.logic import CRITICAL_TRIGGER_LABEL, WARNING_TRIGGER_LABEL, ChannelLookupTimeoutError, check_aggregate_column_support, create_alert_rule, delete_alert_rule_trigger, query_datasets_to_type, translate_aggregate_field, update_alert_rule
from sentry.incidents.models import AlertRule, AlertRuleThresholdType, AlertRuleTrigger
from sentry.snuba.dataset import Dataset
from sentry.snuba.entity_subscription import ENTITY_TIME_COLUMNS, get_entity_key_from_query_builder, get_entity_subscription
from sentry.snuba.models import QuerySubscription, SnubaQuery, SnubaQueryEventType
from sentry.snuba.tasks import build_query_builder
from . import CRASH_RATE_ALERTS_ALLOWED_TIME_WINDOWS, QUERY_TYPE_VALID_DATASETS, QUERY_TYPE_VALID_EVENT_TYPES, UNSUPPORTED_QUERIES
...
class AlertRuleSerializer(CamelSnakeModelSerializer):
    ...
    excluded_projects = serializers.ListField(child=ProjectField(scope='project:read'), required=False)
    ...
    time_window = serializers.IntegerField(required=True, min_value=1, max_value=int(timedelta(days=1).total_seconds() / 60))
    threshold_period = serializers.IntegerField(default=1, min_value=1, max_value=20)
    comparison_delta = serializers.IntegerField(required=False, min_value=1, max_value=int(timedelta(days=89).total_seconds() / 60), allow_null=True)
    aggregate = serializers.CharField(required=True, min_length=1)
    owner = ActorField(required=False, allow_null=True, as_actor=True)

    class Meta:
        model = AlertRule
        fields = ['name', 'owner', 'query_type', 'dataset', 'query', 'time_window', 'environment', 'threshold_type', 'resolve_threshold', 'threshold_period', 'comparison_delta', 'aggregate', 'projects', 'include_all_projects', 'excluded_projects', 'triggers', 'event_types']
        extra_kwargs = {'name': {'min_length': 1, 'max_length': 64}, 'include_all_projects': {'default': False}, 'threshold_type': {'required': True}, 'resolve_threshold': {'required': False}}
    threshold_translators = {AlertRuleThresholdType.ABOVE: lambda threshold: threshold + 100, AlertRuleThresholdType.BELOW: lambda threshold: 100 - threshold}
