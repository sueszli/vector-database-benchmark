...
class TestRoomsClientAsync(ACSRoomsTestCase):
    ...
    async def test_add_or_update_participant_async(self):
        ...
        expected_participants = [RoomParticipant(communication_identifier=CommunicationUserIdentifier(self.id1), role=ParticipantRole.CONSUMER), RoomParticipant(communication_identifier=CommunicationUserIdentifier(self.id2), role=ParticipantRole.CONSUMER), RoomParticipant(communication_identifier=CommunicationUserIdentifier(self.id3), role=ParticipantRole.ATTENDEE)]
        async with self.rooms_client:
    ...
    async def test_add_or_update_participants_wrongRoleName_async(self):
        async with self.rooms_client:
            create_response = await self.rooms_client.create_room()
            participants = [RoomParticipant(communication_identifier=CommunicationUserIdentifier('chris'), role='kafka'), self.users['john']]
            with pytest.raises(HttpResponseError) as ex:
                await self.rooms_client.add_or_update_participants(room_id=create_response.id, participants=participants)
                assert str(ex.value.status_code) == '400'
                assert ex.value.message is not None
    ...
    async def test_remove_participant_wrong_mri_async(self):
        async with self.rooms_client:
            create_response = await self.rooms_client.create_room()
            participants = [RoomParticipant(communication_identifier=CommunicationUserIdentifier('wrong_mri'), role=ParticipantRole.ATTENDEE)]
            with pytest.raises(HttpResponseError) as ex:
