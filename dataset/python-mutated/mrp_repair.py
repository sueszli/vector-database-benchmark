...
class Repair(models.Model):
    ...
    def _default_stock_location(self):
        ...
        return False
    name = fields.Char('Repair Reference', default=lambda self: self.env['ir.sequence'].next_by_code('mrp.repair'), copy=False, required=True, states={'confirmed': [('readonly', True)]})
    product_id = fields.Many2one('product.product', string='Product to Repair', readonly=True, required=True, states={'draft': [('readonly', False)]})
    product_qty = fields.Float('Product Quantity', default=1.0, digits=dp.get_precision('Product Unit of Measure'), readonly=True, required=True, states={'draft': [('readonly', False)]})
    product_uom = fields.Many2one('product.uom', 'Product Unit of Measure', readonly=True, required=True, states={'draft': [('readonly', False)]})
    partner_id = fields.Many2one('res.partner', 'Partner', index=True, states={'confirmed': [('readonly', True)]}, help='Choose partner for whom the order will be invoiced and delivered.')
    ...
    location_id = fields.Many2one('stock.location', 'Current Location', default=_default_stock_location, index=True, readonly=True, required=True, states={'draft': [('readonly', False)], 'confirmed': [('readonly', True)]})
    ...
    operations = fields.One2many('mrp.repair.line', 'repair_id', 'Operation Lines', copy=True, readonly=True, states={'draft': [('readonly', False)]})
    ...
    move_id = fields.Many2one('stock.move', 'Move', copy=False, readonly=True, track_visibility='onchange', help='Move created by the repair order')
    fees_lines = fields.One2many('mrp.repair.fee', 'repair_id', 'Fees', copy=True, readonly=True, states={'draft': [('readonly', False)]})
    internal_notes = fields.Text('Internal Notes')
    quotation_notes = fields.Text('Quotation Notes')
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env['res.company']._company_default_get('mrp.repair'))
...
class RepairLine(models.Model):
    ...
    repair_id = fields.Many2one('mrp.repair', 'Repair Order Reference', index=True, ondelete='cascade')
...
class RepairFee(models.Model):
    ...
    repair_id = fields.Many2one('mrp.repair', 'Repair Order Reference', index=True, ondelete='cascade', required=True)
