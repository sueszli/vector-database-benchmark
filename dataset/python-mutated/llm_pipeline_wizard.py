...
from mage_ai.data_preparation.models.constants import NON_PIPELINE_EXECUTABLE_BLOCK_TYPES, AIMode, BlockLanguage, BlockType, PipelineType
from mage_ai.data_preparation.models.pipeline import Pipeline
from mage_ai.data_preparation.repo_manager import get_repo_config, get_repo_path
from mage_ai.data_preparation.templates.template import fetch_template_source, fetch_transformer_default_template, is_default_transformer_template
from mage_ai.io.base import DataSource
from mage_ai.orchestration.ai.config import AIConfig
from mage_ai.server.logger import Logger
logger = Logger().new_server_logger(__name__)
BLOCK_LANGUAGE_TO_FILE_TYPE_VARIABLE = {BlockLanguage.MARKDOWN: 'markdown script', BlockLanguage.PYTHON: 'python script', BlockLanguage.R: 'R script', BlockLanguage.SQL: 'sql script', BlockLanguage.YAML: 'yaml configuration'}
BLOCK_TYPE_TO_PURPOSE_VARIABLE = {BlockType.DATA_LOADER: 'load data from a specific data source.', BlockType.DATA_EXPORTER: 'export data to a specific data source.', BlockType.TRANSFORMER: 'transform data from one format into different format.'}
...
BLOCK_SPLIT_PATTERN = 'BLOCK\\s+(\\w+):\\s+function:\\s+(.*?)\\.\\s+upstream:\\s*(.*?)$'
TRANSFORMERS_FOLDER = 'transformers'
CLASSIFICATION_FUNCTION_NAME = 'classify_description'
TEMPLATE_CLASSIFICATION_FUNCTION = [{'name': CLASSIFICATION_FUNCTION_NAME, 'description': 'Classify the code description provided into following properties.', 'parameters': {'type': 'object', 'properties': {BlockType.__name__: {'type': 'string', 'description': 'Type of the code block. It either loads data from a source, export data to a source or transform data from one format to another.', 'enum': [f'{BlockType.__name__}__data_exporter', f'{BlockType.__name__}__data_loader', f'{BlockType.__name__}__transformer']}, BlockLanguage.__name__: {'type': 'string', 'description': f'Programming language of the code block. Default value is {BlockLanguage.__name__}__python.', 'enum': [f'{BlockLanguage.__name__}__{type.name.lower()}' for type in BlockLanguage]}, PipelineType.__name__: {'type': 'string', 'description': f'Type of pipeline to build. Default value is {PipelineType.__name__}__python if pipeline type is not mentioned in the description.', 'enum': [f'{PipelineType.__name__}__{type.name.lower()}' for type in PipelineType]}, ActionType.__name__: {'type': 'string', 'description': f'If {BlockType.__name__} is transformer, {ActionType.__name__} specifies what kind of action the code performs.', 'enum': [f'{ActionType.__name__}__{type.name.lower()}' for type in ActionType]}, DataSource.__name__: {'type': 'string', 'description': f'If {BlockType.__name__} is data_loader or data_exporter, {DataSource.__name__} field specify where the data loads from or exports to.', 'enum': [f'{DataSource.__name__}__{type.name.lower()}' for type in DataSource]}}, 'required': [BlockType.__name__, BlockLanguage.__name__, PipelineType.__name__]}}]
...
class LLMPipelineWizard:
    ...
    async def __async_llm_call(self, variable_values: Dict[str, str], prompt_template: str, is_json_response: bool=True):
        ...
        """
        filled_prompt = PromptTemplate(input_variables=list(variable_values.keys()), template=prompt_template)
    ...
    async def __async_create_customized_code_in_block(self, block_code: str, block_language: str, code_description: str) -> str:
        """
        ...
        if block_language == BlockLanguage.PYTHON:
            customized_logic = await self.client.inference_with_prompt(variable_values, PROMPT_FOR_CUSTOMIZED_CODE_IN_PYTHON)
            if 'action_code' in customized_logic.keys() and customized_logic.get('action_code') and ('null' != customized_logic.get('action_code')):
                block_code = block_code.replace("action_code=''", f"action_code='{customized_logic.get('action_code')}'")
            if 'arguments' in customized_logic.keys():
                block_code = block_code.replace('arguments=[]', f"arguments={customized_logic.get('arguments')}")
        elif block_language == BlockLanguage.SQL:
            customized_logic = await self.client.inference_with_prompt(variable_values, PROMPT_FOR_CUSTOMIZED_CODE_IN_SQL)
            if 'sql_code' in customized_logic.keys():
                block_code = f"{block_code}\n{customized_logic.get('sql_code')}"
        return block_code

    async def async_generate_block_with_description(self, block_description: str, upstream_blocks: List[str]=None) -> dict:
        ...
        if is_default_transformer_template(config):
            customized_logic = await self.client.inference_with_prompt(variable_values, PROMPT_FOR_CUSTOMIZED_CODE_WITH_BASE_TEMPLATE)
            if 'code' in customized_logic.keys():
                config['existing_code'] = customized_logic.get('code')
            block_code = fetch_template_source(block_type=block_type, config=config, language=block_language, pipeline_type=pipeline_type)
        else:
            try:
                block_code_template = fetch_template_source(block_type=block_type, config=config, language=block_language, pipeline_type=pipeline_type)
                block_code = await self.__async_create_customized_code_in_block(block_code_template, block_language, block_description)
            except TemplateNotFound:
                customized_logic = await self.client.inference_with_prompt(variable_values, PROMPT_FOR_CUSTOMIZED_CODE_WITH_BASE_TEMPLATE)
                if 'code' in customized_logic.keys():
                    block_code = fetch_transformer_default_template(customized_logic.get('code'))
        return dict(block_type=block_type, configuration=config, content=block_code, language=block_language, upstream_blocks=upstream_blocks)

    async def __async_generate_blocks(self, block_dict: dict, block_id: int, block_description: str, upstream_blocks: [str]) -> dict:
    ...
    async def async_generate_pipeline_from_description(self, pipeline_description: str) -> dict:
        ...
        splited_block_descriptions = await self.client.inference_with_prompt(variable_values, PROMPT_TO_SPLIT_BLOCKS, is_json_response=False)
        blocks = {}
        block_tasks = []
        for line in splited_block_descriptions.strip().split('\n'):
            if line.startswith('BLOCK') and ':' in line:
                ...
                if match:
                    ...
                    block_tasks.append(self.__async_generate_blocks(blocks, block_id, block_description, upstream_blocks))
    ...
    async def async_generate_comment_for_block(self, block_content: str) -> str:
        ...
        function_comments = await self.client.inference_with_prompt(variable_values, PROMPT_FOR_FUNCTION_COMMENT)
        return self.__insert_comments_in_functions(block_content, function_comments)

    async def async_generate_doc_for_pipeline(self, pipeline_uuid: str, project_path: str=None, print_block_doc: bool=False) -> dict:
        pipeline = Pipeline.get(uuid=pipeline_uuid, repo_path=project_path or get_repo_path())
        async_block_docs = []
        ...
        pipeline_doc = await self.client.inference_with_prompt(variable_values, PROMPT_FOR_SUMMARIZE_BLOCK_DOC, is_json_response=False)
        return dict(block_docs=block_docs, pipeline_doc=pipeline_doc)

    async def async_generate_doc_for_block(self, pipeline_uuid: str, block_uuid: str, project_path: str=None) -> str:
        pipeline = Pipeline.get(uuid=pipeline_uuid, repo_path=project_path or get_repo_path())
        return await self.__async_generate_doc_for_block(pipeline.get_block(block_uuid))

    async def __async_generate_doc_for_block(self, block: Block) -> str:
        add_on_prompt = ''
        if block.type == BlockType.TRANSFORMER:
            add_on_prompt = 'Focus on the customized business logic in execute_transformer_actionfunction.'
        variable_values = dict()
        variable_values['block_content'] = re.sub('\\s*\\n*\\s*@test.*', '', block.content, flags=re.DOTALL)
        ...
        return await self.client.inference_with_prompt(variable_values, PROMPT_FOR_BLOCK, is_json_response=False)