...
new_start = True
size = (30, 30)
im = ['', blank, bomb_img, flag, '']
color = [('grey', 'grey'), ('black', '#22B14C'), ('black', '#22B14C'), ('black', '#22B14C'), ('black', 'grey')]
...
layout = [[button1('New Game', key='-New Game-'), button1('Game Over', key='-Game Over-'), button1('Target:' + str(all), key='-Target-'), button1('Bomb:0', key='-Count-Bomb-'), button1('Flag:0', key='-Count-Flag-')]] + [[button2(x, y) for x in range(width)] for y in range(height)]
window = sg.Window('MineSweeper', layout=layout, finalize=True, use_ttk_buttons=False)
binding_all()
show_blank()
new_start = True
message = False
while True:
    ...
    if event == None or event == '-Game Over-':
        break
    elif event == '-New Game-':
        new_start = True
