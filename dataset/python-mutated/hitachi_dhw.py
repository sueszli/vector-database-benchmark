...
from homeassistant.components.water_heater import STATE_HIGH_DEMAND, WaterHeaterEntity, WaterHeaterEntityFeature
from homeassistant.const import ATTR_TEMPERATURE, PRECISION_WHOLE, STATE_OFF, STATE_ON, UnitOfTemperature
from ..entity import OverkizEntity
OVERKIZ_TO_OPERATION_MODE: dict[str, str] = {OverkizCommandParam.STANDARD: STATE_ON, OverkizCommandParam.HIGH_DEMAND: STATE_HIGH_DEMAND, OverkizCommandParam.STOP: STATE_OFF}
...
class HitachiDHW(OverkizEntity, WaterHeaterEntity):
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        await self.executor.async_execute_command(OverkizCommand.SET_CONTROL_DHW_SETTING_TEMPERATURE, int(kwargs[ATTR_TEMPERATURE]))
    ...
    async def async_set_operation_mode(self, operation_mode: str) -> None:
        ...
        if operation_mode == OverkizCommandParam.OFF:
            return await self.executor.async_execute_command(OverkizCommand.SET_CONTROL_DHW, OverkizCommandParam.STOP)
        if self.current_operation == OverkizCommandParam.OFF:
            await self.executor.async_execute_command(OverkizCommand.SET_CONTROL_DHW, OverkizCommandParam.ON)
        await self.executor.async_execute_command(OverkizCommand.SET_DHW_MODE, OPERATION_MODE_TO_OVERKIZ[operation_mode])