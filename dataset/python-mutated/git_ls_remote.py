...
def _process(proc_data: List[JSONDictType]) -> JSONDictType:
    ...
    for item in proc_data:
        new_dict.update({item['reference']: item['commit']})
    return new_dict

def parse(data: str, raw: bool=False, quiet: bool=False) -> Union[JSONDictType, List[JSONDictType]]:
    if False:
        for i in range(10):
            print('nop')
    ...
    if jc.utils.has_data(data):
        for line in filter(None, data.splitlines()):
            ...
            output_line = {'reference': reference, 'commit': commit}
