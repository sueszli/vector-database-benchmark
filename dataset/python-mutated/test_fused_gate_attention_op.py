...
@unittest.skipIf(not core.is_compiled_with_cuda(), 'Paddle is not compiled with CUDA')
...
@unittest.skipIf(not core.is_compiled_with_cuda() or get_cuda_version() < 11000 or paddle.device.cuda.get_device_capability()[0] < 8, 'core is not compiled with CUDA and cuda version need larger than or equal to 11.3')
