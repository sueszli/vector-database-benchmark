...
@pluginargument('username', requires=['password'], metavar='USERNAME', help='The username used to register with bbc.co.uk.')
@pluginargument('password', prompt='Enter bbc.co.uk account password', sensitive=True, metavar='PASSWORD', help='A bbc.co.uk account password to use with --bbciplayer-username.')
@pluginargument('hd', action='store_true', help='Prefer HD streams over local SD streams, some live programmes may not be broadcast in HD.')
class BBCiPlayer(Plugin):
    ...
    """
    mediator_re = re.compile('window\\.__IPLAYER_REDUX_STATE__\\s*=\\s*({.*?});', re.DOTALL)
    state_re = re.compile('window.__IPLAYER_REDUX_STATE__\\s*=\\s*({.*?});</script>')
    account_locals_re = re.compile('window.bbcAccount.locals\\s*=\\s*({.*?});')
    ...
    mediator_schema = validate.Schema({'versions': [{'id': str}]}, validate.get('versions'), validate.get(0), validate.get('id'))
    mediaselector_schema = validate.Schema(validate.parse_json(), {'media': [{'connection': validate.all([{validate.optional('href'): validate.url(), validate.optional('transferFormat'): str}], validate.filter(lambda c: c.get('href'))), 'kind': str}]}, validate.get('media'), validate.filter(lambda x: x['kind'] == 'video'))
