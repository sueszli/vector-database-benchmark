...
PartitionClass = FactoryDispatcher.get_factory().io_cls.frame_cls._partition_mgr_cls._partition_class
if Engine.get() == 'Ray':
    ...
    is_future = lambda obj: isinstance(obj, ObjectIDType)
elif Engine.get() == 'Dask':
    from distributed import Future
    from modin.core.execution.dask.common import DaskWrapper
    put_func = lambda obj: DaskWrapper.put(obj, hash=False)
    get_func = DaskWrapper.materialize
    is_future = lambda obj: isinstance(obj, Future)
    ...
    put_func = lambda x: x
    get_func = lambda x: x
    is_future = lambda obj: isinstance(obj, object)
else:
    raise NotImplementedError(f"'{Engine.get()}' engine is not supported by these test suites")
NPartitions.put(4)
pd.DataFrame([])
