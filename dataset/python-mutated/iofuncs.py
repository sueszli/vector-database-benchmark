...
import glob
import pickle
from spyder_kernels.utils.lazymodules import FakeObject, numpy as np, pandas as pd, PIL, scipy as sp
...
if sys.byteorder == 'little':
    ...
    _ENDIAN = '>'
DTYPES = {'1': ('|b1', None), 'L': ('|u1', None), 'I': ('%si4' % _ENDIAN, None), 'F': ('%sf4' % _ENDIAN, None), 'I;16': ('|u2', None), 'I;16S': ('%si2' % _ENDIAN, None), 'P': ('|u1', None), 'RGB': ('|u1', 3), 'RGBX': ('|u1', 4), 'RGBA': ('|u1', 4), 'CMYK': ('|u1', 4), 'YCbCr': ('|u1', 4)}
...
if __name__ == '__main__':
    ...
    example = {'str': 'kjkj kj k j j kj k jkj', 'unicode': u'éù', 'list': [1, 3, [4, 5, 6], 'kjkj', None], 'tuple': ([1, testdate, testdict], 'kjkj', None), 'dict': testdict, 'float': 1.2233, 'array': np.random.rand(4000, 400), 'empty_array': np.array([]), 'date': testdate, 'datetime': datetime.datetime(1945, 5, 8)}
    import time
    t0 = time.time()
