...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, default=None, help='Input file path. Accept SavedModel, MetaGraphDef, and GraphDef in either binary or text format.')
    parser.add_argument('--fetch', type=str, default=None, help='The names of the fetch node delimited by comma.')
    parser.add_argument('--rewriter_config', type=str, default=None, help='Configuration for the grappler optimizers, described as a RewriterConfig protocol buffer. Usage example 1: --rewriter_config=\'optimize_tensor_layout: true disable_model_pruning: true\'. Usage example 2: --rewriter_config=\'optimizers: "constfold" optimizers: "layout"\'')
    parser.add_argument('--per_node_report', action='store_true', help='Generate per-node report. By default the report contains stats aggregated on a per op type basis, per_node_report adds results for each individual node to the report.')
    parser.add_argument('--memory_report', action='store_true', help='Generate memory usage report.')
    parser.add_argument('--verbose', action='store_true', help='Generate verbose reports. By default, succinct reports are used.')
