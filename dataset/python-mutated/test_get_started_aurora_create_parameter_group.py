...
class MockManager:
    ...
    def setup_stubs(self, error, stop_on, stubber):
        ...
        with self.stub_runner(error, stop_on) as runner:
            runner.add(stubber.stub_describe_db_cluster_parameter_groups, self.group_name, [], error_code='DBParameterGroupNotFound')
...
def test_create_parameter_group_exists(mock_mgr, capsys):
    if False:
        i = 10
        return i + 15
    mock_mgr.cluster_data.stubber.stub_describe_db_cluster_parameter_groups(mock_mgr.group_name, [mock_mgr.group])
...
@pytest.mark.parametrize('error, stop_on_index', [('TESTERROR-describe_db_cluster_parameter_groups', 0), ('TESTERROR-describe_db_engine_versions', 1), ('TESTERROR-create_db_cluster_parameter_group', 2), ('TESTERROR-describe_db_cluster_parameter_groups', 3)])
