...
class TrustViewEndpoint(RESTEndpoint):
    ...
    @docs(tags=['TrustGraph'], summary='Return the trust graph.', parameters=[], responses={200: {'schema': schema(GraphResponse={'root_public_key': String, 'graph': schema(Graph={'node': schema(Node={'id': Integer, 'key': String, 'pos': [Float], 'sequence_number': Integer, 'total_up': Integer, 'total_down': Integer}), 'edge': List(List(Integer))}), 'bootstrap': schema(Bootstrap={'download': Integer, 'upload': Integer, 'progress': Float}), 'num_tx': Integer})}})
    async def get_view(self, request):
        ...
        return RESTResponse({'root_public_key': hexlify(self.bandwidth_db.my_pub_key), 'graph': graph_data, 'bootstrap': 0, 'num_tx': len(graph_data['edge'])})