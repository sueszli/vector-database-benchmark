...
class sfp_binstring(SpiderFootPlugin):
    meta = {'name': 'Binary String Extractor', 'summary': 'Attempt to identify strings in binary content.', 'flags': ['errorprone'], 'useCases': ['Footprint'], 'categories': ['Content Analysis']}
    opts = {'minwordsize': 5, 'maxwords': 100, 'maxfilesize': 1000000, 'usedict': True, 'fileexts': ['png', 'gif', 'jpg', 'jpeg', 'tiff', 'tif', 'ico', 'flv', 'mp4', 'mp3', 'avi', 'mpg', 'mpeg', 'dat', 'mov', 'swf', 'exe', 'bin'], 'filterchars': '#}{|%^&*()=+,;[]~'}
    optdescs = {'minwordsize': 'Upon finding a string in a binary, ensure it is at least this length. Helps weed out false positives.', 'usedict': 'Use the dictionary to further reduce false positives - any string found must contain a word from the dictionary (can be very slow, especially for larger files).', 'fileexts': 'File types to fetch and analyse.', 'maxfilesize': 'Maximum file size in bytes to download for analysis.', 'maxwords': 'Stop reporting strings from a single binary after this many are found.', 'filterchars': 'Ignore strings with these characters, as they may just be garbage ASCII.'}
    results = list()
    d = None
    n = None
    fq = None
