...
from scapy.asn1fields import ASN1F_INTEGER, ASN1F_IPADDRESS, ASN1F_OID, ASN1F_SEQUENCE, ASN1F_SEQUENCE_OF, ASN1F_STRING, ASN1F_TIME_TICKS, ASN1F_enum_INTEGER, ASN1F_field, ASN1F_CHOICE, ASN1F_optional, ASN1F_NULL
...
SNMP_error = {0: 'no_error', 1: 'too_big', 2: 'no_such_name', 3: 'bad_value', 4: 'read_only', 5: 'generic_error', 6: 'no_access', 7: 'wrong_type', 8: 'wrong_length', 9: 'wrong_encoding', 10: 'wrong_value', 11: 'no_creation', 12: 'inconsistent_value', 13: 'resource_unavailable', 14: 'commit_failed', 15: 'undo_failed', 16: 'authorization_error', 17: 'not_writable', 18: 'inconsistent_name'}
SNMP_trap_types = {0: 'cold_start', 1: 'warm_start', 2: 'link_down', 3: 'link_up', 4: 'auth_failure', 5: 'egp_neigh_loss', 6: 'enterprise_specific'}

class SNMPvarbind(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SEQUENCE(ASN1F_OID('oid', '1.3'), ASN1F_optional(ASN1F_field('value', ASN1_NULL(0))), ASN1F_optional(ASN1F_NULL('noSuchObject', None, implicit_tag=128)), ASN1F_optional(ASN1F_NULL('noSuchInstance', None, implicit_tag=129)), ASN1F_optional(ASN1F_NULL('endOfMibView', None, implicit_tag=130)))

class SNMPget(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SNMP_PDU_GET(ASN1F_INTEGER('id', 0), ASN1F_enum_INTEGER('error', 0, SNMP_error), ASN1F_INTEGER('error_index', 0), ASN1F_SEQUENCE_OF('varbindlist', [], SNMPvarbind))

class SNMPnext(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SNMP_PDU_NEXT(ASN1F_INTEGER('id', 0), ASN1F_enum_INTEGER('error', 0, SNMP_error), ASN1F_INTEGER('error_index', 0), ASN1F_SEQUENCE_OF('varbindlist', [], SNMPvarbind))

class SNMPresponse(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SNMP_PDU_RESPONSE(ASN1F_INTEGER('id', 0), ASN1F_enum_INTEGER('error', 0, SNMP_error), ASN1F_INTEGER('error_index', 0), ASN1F_SEQUENCE_OF('varbindlist', [], SNMPvarbind))

class SNMPset(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SNMP_PDU_SET(ASN1F_INTEGER('id', 0), ASN1F_enum_INTEGER('error', 0, SNMP_error), ASN1F_INTEGER('error_index', 0), ASN1F_SEQUENCE_OF('varbindlist', [], SNMPvarbind))

class SNMPtrapv1(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SNMP_PDU_TRAPv1(ASN1F_OID('enterprise', '1.3'), ASN1F_IPADDRESS('agent_addr', '0.0.0.0'), ASN1F_enum_INTEGER('generic_trap', 0, SNMP_trap_types), ASN1F_INTEGER('specific_trap', 0), ASN1F_TIME_TICKS('time_stamp', IntAutoTime()), ASN1F_SEQUENCE_OF('varbindlist', [], SNMPvarbind))

class SNMPbulk(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SNMP_PDU_BULK(ASN1F_INTEGER('id', 0), ASN1F_INTEGER('non_repeaters', 0), ASN1F_INTEGER('max_repetitions', 0), ASN1F_SEQUENCE_OF('varbindlist', [], SNMPvarbind))

class SNMPinform(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SNMP_PDU_INFORM(ASN1F_INTEGER('id', 0), ASN1F_enum_INTEGER('error', 0, SNMP_error), ASN1F_INTEGER('error_index', 0), ASN1F_SEQUENCE_OF('varbindlist', [], SNMPvarbind))

class SNMPtrapv2(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SNMP_PDU_TRAPv2(ASN1F_INTEGER('id', 0), ASN1F_enum_INTEGER('error', 0, SNMP_error), ASN1F_INTEGER('error_index', 0), ASN1F_SEQUENCE_OF('varbindlist', [], SNMPvarbind))

class SNMP(ASN1_Packet):
    ASN1_codec = ASN1_Codecs.BER
    ASN1_root = ASN1F_SEQUENCE(ASN1F_enum_INTEGER('version', 1, {0: 'v1', 1: 'v2c', 2: 'v2', 3: 'v3'}), ASN1F_STRING('community', 'public'), ASN1F_CHOICE('PDU', SNMPget(), SNMPget, SNMPnext, SNMPresponse, SNMPset, SNMPtrapv1, SNMPbulk, SNMPinform, SNMPtrapv2))
