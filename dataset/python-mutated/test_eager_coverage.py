...
from test_dali_cpu_only_utils import pipeline_arithm_ops_cpu, setup_test_nemo_asr_reader_cpu, setup_test_numpy_reader_cpu
...
data = [[rng.integers(0, 255, size=sample_shape, dtype=np.uint8) for _ in range(batch_size)] for _ in range(data_size)]
audio_data = [[rng.random(size=[200], dtype=np.float32) for _ in range(batch_size)] for _ in range(data_size)]
flat_data = [[rng.integers(0, 255, size=[200], dtype=np.uint8) for _ in range(batch_size)] for _ in range(data_size)]
...
tested_methods = ['decoders.image', 'rotate', 'brightness_contrast', 'hue', 'brightness', 'contrast', 'hsv', 'color_twist', 'saturation', 'shapes', 'crop', 'color_space_conversion', 'cast', 'resize', 'per_frame', 'gaussian_blur', 'laplacian', 'crop_mirror_normalize', 'flip', 'jpeg_compression_distortion', 'decoders.image_crop', 'reshape', 'reinterpret', 'water', 'sphere', 'erase', 'expand_dims', 'coord_transform', 'grid_mask', 'multi_paste', 'nonsilent_region', 'preemphasis_filter', 'power_spectrum', 'spectrogram', 'mel_filter_bank', 'to_decibels', 'audio_resample', 'mfcc', 'one_hot', 'transpose', 'decoders.audio', 'coord_flip', 'bb_flip', 'warp_affine', 'normalize', 'lookup_table', 'slice', 'decoders.image_slice', 'pad', 'readers.file', 'readers.mxnet', 'readers.webdataset', 'readers.coco', 'readers.caffe', 'readers.caffe2', 'readers.nemo_asr', 'experimental.readers.video', 'copy', 'element_extract', 'bbox_paste', 'sequence_rearrange', 'box_encoder', 'readers.numpy', 'constant', 'dump_image', 'readers.sequence', 'transforms.translation', 'transforms.scale', 'transforms.rotation', 'transforms.shear', 'transforms.crop', 'transforms.combine', 'reductions.min', 'reductions.max', 'reductions.sum', 'segmentation.select_masks', 'reductions.mean', 'reductions.mean_square', 'reductions.rms', 'reductions.std_dev', 'reductions.variance', 'cat', 'stack', 'permute_batch', 'squeeze', 'peek_image_shape', 'subscript_dim_check', 'get_property', 'tensor_subscript', 'arithmetic_generic_op', 'decoders.image_random_crop', 'noise.gaussian', 'noise.salt_and_pepper', 'noise.shot', 'segmentation.random_mask_pixel', 'segmentation.random_object_bbox', 'roi_random_crop', 'random_bbox_crop', 'random_resized_crop', 'resize_crop_mirror', 'random.coin_flip', 'random.normal', 'random.uniform', 'batch_permutation']
excluded_methods = ['hidden.*', 'jitter', 'video_reader', 'video_reader_resize', 'readers.video', 'readers.video_resize', 'optical_flow', 'paste', 'experimental.inflate']
