...
re_novar = re.compile('^(SRC|TGT)\\W+.*?$')
reg_act = re.compile('(?P<backslash>\\\\)|(?P<dollar>\\$\\$)|(?P<subst>\\$\\{(?P<var>\\w+)(?P<code>.*?)\\})', re.M)
...
reg_act_noshell = re.compile('(?P<space>\\s+)|(?P<subst>\\$\\{(?P<var>\\w+)(?P<code>.*?)\\})|(?P<text>([^$ \\t\\n\\r\\f\\v]|\\$\\$)+)', re.M)
