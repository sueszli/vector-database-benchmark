...
class GroupSnooze(Model):
    ...
    def test_user_rates(self):
        ...
        if rate >= self.user_count:
            return False
        return True
post_save.connect(lambda instance, **kwargs: cache.set(GroupSnooze.get_cache_key(instance.group_id), instance, 3600), sender=GroupSnooze, weak=False)
post_delete.connect(lambda instance, **kwargs: cache.set(GroupSnooze.get_cache_key(instance.group_id), False, 3600), sender=GroupSnooze, weak=False)