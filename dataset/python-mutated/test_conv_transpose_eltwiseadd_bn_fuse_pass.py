...
class TestConvTransposeEltwiseaddBnFusePass(PassAutoScanTest):
    ...
    def test(self):
        if False:
            for i in range(10):
                print('nop')
        self.run_and_statis(quant=False, max_examples=150, max_duration=250, passes=['conv_transpose_eltwiseadd_bn_fuse_pass'])
    ...
    def sample_program_config(self, draw):
        ...
        elementwise_op = OpConfig(type='elementwise_add', inputs={'X': ['conv2d_Out'], 'Y': ['elementwise_add_Y']}, outputs={'Out': ['elementwise_add_Out']}, attrs={'axis': 1})
        batch_norm_op = OpConfig(type='batch_norm', inputs={'X': ['elementwise_add_Out'], 'Scale': ['batch_norm_Scale'], 'Bias': ['batch_norm_Bias'], 'Mean': ['batch_norm_Mean'], 'Variance': ['batch_norm_Variance']}, outputs={'Y': ['batch_norm_Y'], 'MeanOut': ['batch_norm_Mean'], 'VarianceOut': ['batch_norm_Variance'], 'SavedMean': ['batch_norm_SavedMean'], 'SavedVariance': ['batch_norm_SavedVariance'], 'ReserveSpace': ['batch_norm_ReserveSpace']}, attrs={'epsilon': random_epsilon, 'is_test': True, 'trainable_statistics': False, 'data_layout': random_data_layout, 'use_mkldnn': random_use_mkldnn})
        model_net = [conv2d_op, elementwise_op, batch_norm_op]
        program_config = ProgramConfig(ops=model_net, inputs={'conv2d_Input': TensorConfig(data_gen=generate_conv2d_Input)}, weights={'conv2d_Filter': TensorConfig(data_gen=generate_conv2d_Filter), 'elementwise_add_Y': TensorConfig(data_gen=generate_elementwise_add_Y), 'batch_norm_Scale': TensorConfig(data_gen=generate_batch_norm_Scale), 'batch_norm_Bias': TensorConfig(data_gen=generate_batch_norm_Bias), 'batch_norm_Mean': TensorConfig(data_gen=generate_batch_norm_Mean), 'batch_norm_Variance': TensorConfig(data_gen=generate_batch_norm_Variance)}, outputs=['batch_norm_Y'])
