...
from ..sql_statements import LAVALINK_CREATE_INDEX, LAVALINK_CREATE_TABLE, LAVALINK_DELETE_OLD_ENTRIES, LAVALINK_FETCH_ALL_ENTRIES_GLOBAL, LAVALINK_QUERY, LAVALINK_QUERY_ALL, LAVALINK_QUERY_LAST_FETCHED_RANDOM, LAVALINK_UPDATE, LAVALINK_UPSERT, SPOTIFY_CREATE_INDEX, SPOTIFY_CREATE_TABLE, SPOTIFY_DELETE_OLD_ENTRIES, SPOTIFY_QUERY, SPOTIFY_QUERY_ALL, SPOTIFY_QUERY_LAST_FETCHED_RANDOM, SPOTIFY_UPDATE, SPOTIFY_UPSERT, YOUTUBE_CREATE_INDEX, YOUTUBE_CREATE_TABLE, YOUTUBE_DELETE_OLD_ENTRIES, YOUTUBE_QUERY, YOUTUBE_QUERY_ALL, YOUTUBE_QUERY_LAST_FETCHED_RANDOM, YOUTUBE_UPDATE, YOUTUBE_UPSERT, PRAGMA_FETCH_user_version, PRAGMA_SET_journal_mode, PRAGMA_SET_read_uncommitted, PRAGMA_SET_temp_store, PRAGMA_SET_user_version
from .api_utils import LavalinkCacheFetchForGlobalResult, LavalinkCacheFetchResult, SpotifyCacheFetchResult, YouTubeCacheFetchResult
if TYPE_CHECKING:
    from .. import Audio
...
class BaseWrapper:
    ...
    async def _fetch_one(self, values: MutableMapping) -> Optional[Union[LavalinkCacheFetchResult, SpotifyCacheFetchResult, YouTubeCacheFetchResult]]:
        ...
        row = None
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            for future in concurrent.futures.as_completed([executor.submit(self.database.cursor().execute, self.statement.get_one, values)]):
                try:
                    row_result = future.result()
    ...
    async def _fetch_all(self, values: MutableMapping) -> List[Union[LavalinkCacheFetchResult, SpotifyCacheFetchResult, YouTubeCacheFetchResult]]:
        ...
        if self.fetch_result is None:
            return []
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            for future in concurrent.futures.as_completed([executor.submit(self.database.cursor().execute, self.statement.get_all, values)]):
                try:
                    row_result = future.result()
                except Exception as exc:
                    log.verbose('Failed to completed fetch from database', exc_info=exc)
    ...
    async def _fetch_random(self, values: MutableMapping) -> Optional[Union[LavalinkCacheFetchResult, SpotifyCacheFetchResult, YouTubeCacheFetchResult]]:
        """Get a random entry from the local cache"""
        row = None
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            for future in concurrent.futures.as_completed([executor.submit(self.database.cursor().execute, self.statement.get_random, values)]):
                try:
                    row_result = future.result()
...
class YouTubeTableWrapper(BaseWrapper):
    ...
    async def fetch_one(self, values: MutableMapping) -> Tuple[Optional[str], Optional[datetime.datetime]]:
...
class LavalinkTableWrapper(BaseWrapper):
    ...
    async def fetch_one(self, values: MutableMapping) -> Tuple[Optional[MutableMapping], Optional[datetime.datetime]]:
    ...
    async def fetch_all_for_global(self) -> List[LavalinkCacheFetchForGlobalResult]:
        ...
        if self.fetch_for_global is None:
            return []
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            for future in concurrent.futures.as_completed([executor.submit(self.database.cursor().execute, self.statement.get_all_global)]):
                try:
                    row_result = future.result()
                except Exception as exc:
                    log.verbose('Failed to completed fetch from database', exc_info=exc)
