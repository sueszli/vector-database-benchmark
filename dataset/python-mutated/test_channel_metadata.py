...
from tribler.core.components.metadata_store.db.orm_bindings.channel_metadata import CHANNEL_DIR_NAME_LENGTH, MetadataCompressor, entries_to_chunk
from tribler.core.components.metadata_store.db.orm_bindings.channel_node import COMMITTED, NEW, TODELETE, UPDATED
from tribler.core.components.metadata_store.db.serialization import CHANNEL_TORRENT, COLLECTION_NODE, REGULAR_TORRENT, int2time
...
async def test_consolidate_channel_torrent(torrent_template, metadata_store):
    ...
    with db_session:
        ...
        channel.commit_channel_torrent()
        metadata_store.TorrentMetadata.from_dict(dict(torrent_template, public_key=channel.public_key, origin_id=channel.id_, status=NEW))
        channel.commit_channel_torrent()
        torrent_entry.soft_delete()
        channel.commit_channel_torrent()
        assert len(channel.contents_list) == 1
        assert len(os.listdir(my_dir)) == 3
        torrent3 = metadata_store.TorrentMetadata(public_key=channel.public_key, origin_id=channel.id_, status=NEW, infohash=random_infohash())
        channel.commit_channel_torrent()
