...
class TestStateVSIsolation:
    ...
    async def test_issue(self, bot: MockedBot):
        ...
        async def handle_all(message: Message):
            stack.append('all')
        await asyncio.gather(*(dispatcher.feed_update(bot, update) for update in [create_message_update(index=1, text='/test'), create_message_update(index=2, text='foo'), create_message_update(index=3, text='bar'), create_message_update(index=4, text='baz')]))
        assert first == 1
        assert second == 1
        assert third == 1
...
def create_message_update(index: int, text: str):
    if False:
        print('Hello World!')
    return Update(update_id=index, message=Message(message_id=index, date=datetime.now(), chat=Chat(id=42, type='private'), from_user=User(id=42, is_bot=False, first_name='Test', username='test'), text=text))