...
class PrinterStateConnection(octoprint.vendor.sockjs.tornado.SockJSConnection, octoprint.printer.PrinterCallback, LoginStatusListener, GroupChangeListener):
    _event_permissions = {Events.USER_LOGGED_IN: [Permissions.ADMIN], Events.USER_LOGGED_OUT: [Permissions.ADMIN], '*': []}
    _event_payload_processors = {Events.CLIENT_OPENED: [lambda user, payload: payload if user.has_permission(Permissions.ADMIN) else {}], Events.CLIENT_AUTHED: [lambda user, payload: payload if user.has_permission(Permissions.ADMIN) else {}], '*': []}
    _emit_permissions = {'connected': [], 'reauthRequired': [], 'plugin': lambda payload: [] if payload.get('plugin') in ('backup', 'softwareupdate') and settings().getBoolean(['server', 'firstRun']) else [Permissions.STATUS], '*': [Permissions.STATUS]}
