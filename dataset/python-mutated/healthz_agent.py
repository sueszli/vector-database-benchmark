...
class HealthzAgent(dashboard_utils.DashboardAgentModule):
    ...
    def __init__(self, dashboard_agent):
        ...
        self._health_checker = HealthChecker(dashboard_agent.gcs_aio_client, f'{dashboard_agent.ip}:{dashboard_agent.node_manager_port}')
    ...
    async def health_check(self, req: Request) -> Response:
        try:
            ...
            if e.rpc_code not in (ray._raylet.GRPC_STATUS_CODE_UNAVAILABLE, ray._raylet.GRPC_STATUS_CODE_UNKNOWN, ray._raylet.GRPC_STATUS_CODE_DEADLINE_EXCEEDED):
                return Response(status=503, text=f'Health check failed due to: {e}')
        return Response(text='success', content_type='application/text')
