...
from dagster._core.storage.db_io_manager import DbClient, DbIOManager, DbTypeHandler, TablePartitionDimension, TableSlice
...
class SnowflakeIOManager(ConfigurableIOManagerFactory):
    ...
    account: str = Field(description='Your Snowflake account name. For more details, see the `Snowflake documentation. <https://docs.snowflake.com/developer-guide/python-connector/python-connector-api>`__')
    user: str = Field(description='User login name.')
    schema_: Optional[str] = Field(default=None, alias='schema', description='Name of the schema to use.')
    ...
    private_key: Optional[str] = Field(default=None, description='Raw private key to use. See the `Snowflake documentation <https://docs.snowflake.com/en/user-guide/key-pair-auth.html>`__ for details. To avoid issues with newlines in the keys, you can base64 encode the key. You can retrieve the base64 encoded key with this shell command: cat rsa_key.p8 | base64')
    private_key_path: Optional[str] = Field(default=None, description='Path to the private key. See the `Snowflake documentation <https://docs.snowflake.com/en/user-guide/key-pair-auth.html>`__ for details.')
    private_key_password: Optional[str] = Field(default=None, description='The password of the private key. See the `Snowflake documentation <https://docs.snowflake.com/en/user-guide/key-pair-auth.html>`__ for details. Required for both private_key and private_key_path if the private key is encrypted. For unencrypted keys, this config can be omitted or set to None.')
    store_timestamps_as_strings: bool = Field(default=False, description='If using Pandas DataFrames, whether to convert time data to strings. If True, time data will be converted to strings when storing the DataFrame and converted back to time data when loading the DataFrame. If False, time data without a timezone will be set to UTC timezone to avoid a Snowflake bug. Defaults to False.')
    authenticator: Optional[str] = Field(default=None, description='Optional parameter to specify the authentication mechanism to use.')
