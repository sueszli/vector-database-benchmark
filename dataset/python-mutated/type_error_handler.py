...
async def _publish_diagnostics(output_channel: connections.AsyncTextWriter, path: Path, diagnostics: Sequence[lsp.Diagnostic]) -> None:
    LOG.debug(f'Publish diagnostics for {path}: {diagnostics}')
    await lsp.write_json_rpc(output_channel, json_rpc.Request(method='textDocument/publishDiagnostics', parameters=json_rpc.ByNameParameters({'uri': lsp.DocumentUri.from_file_path(path).unparse(), 'diagnostics': [diagnostic.to_dict() for diagnostic in diagnostics]})))
...
class ClientTypeErrorHandler:
    ...
    async def show_overlay_type_errors(self, path: Path, type_errors: Sequence[error.Error]) -> None:
        LOG.info(f'Refreshing type errors at path {path}. Total number of type errors is {len(type_errors)}.')
