...
class Test_organizations_delegated_administrators:
    ...
    def test_no_organization(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.organizations.organizations_delegated_administrators.organizations_delegated_administrators.organizations_client', new=Organizations(audit_info)):
                from prowler.providers.aws.services.organizations.organizations_delegated_administrators.organizations_delegated_administrators import organizations_delegated_administrators
    ...
    def test_organization_no_delegations(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.organizations.organizations_delegated_administrators.organizations_delegated_administrators.organizations_client', new=Organizations(audit_info)):
                ...
                assert search('No Delegated Administrators', result[0].status_extended)
    ...
    def test_organization_trusted_delegated(self):
        ...
        conn.register_delegated_administrator(AccountId=account['CreateAccountStatus']['AccountId'], ServicePrincipal='config-multiaccountsetup.amazonaws.com')
        audit_info.audit_config = {'organizations_trusted_delegated_administrators': [account['CreateAccountStatus']['AccountId']]}
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.organizations.organizations_delegated_administrators.organizations_delegated_administrators.organizations_client', new=Organizations(audit_info)):
                from prowler.providers.aws.services.organizations.organizations_delegated_administrators.organizations_delegated_administrators import organizations_delegated_administrators
                check = organizations_delegated_administrators()
                ...
                assert result[0].resource_id == response['Organization']['Id']
                assert result[0].resource_arn == response['Organization']['Arn']
                assert search('Trusted Delegated Administrator', result[0].status_extended)
                assert result[0].region == AWS_REGION
    ...
    def test_organization_untrusted_delegated(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.organizations.organizations_delegated_administrators.organizations_delegated_administrators.organizations_client', new=Organizations(audit_info)):
                ...
                assert result[0].resource_id == response['Organization']['Id']
                assert result[0].resource_arn == response['Organization']['Arn']
                assert search('Untrusted Delegated Administrator', result[0].status_extended)
                assert result[0].region == AWS_REGION