...
if TYPE_CHECKING:
    ...
    from litestar.types import ASGIApp, LifeSpanReceive, LifeSpanSend, LifeSpanShutdownCompleteEvent, LifeSpanShutdownFailedEvent, LifeSpanStartupCompleteEvent, LifeSpanStartupFailedEvent, Method, Receive, RouteHandlerType, Scope, Send
...
class ASGIRouter:
    ...
    """
    __slots__ = ('_mount_paths_regex', '_mount_routes', '_plain_routes', '_registered_routes', '_static_routes', 'app', 'root_route_map_node', 'route_handler_index', 'route_mapping')
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        (asgi_app, scope['route_handler'], scope['path'], scope['path_params']) = self.handle_routing(path=normalized_path, method=scope.get('method'))
