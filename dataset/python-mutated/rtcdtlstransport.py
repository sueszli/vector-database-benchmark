...
from .rtp import AnyRtcpPacket, RtcpByePacket, RtcpPacket, RtcpPsfbPacket, RtcpRrPacket, RtcpRtpfbPacket, RtcpSrPacket, RtpPacket, is_rtcp
...
class RTCDtlsTransport(AsyncIOEventEmitter):
    ...
    async def start(self, remoteParameters: RTCDtlsParameters) -> None:
        ...
        if not fingerprint_is_valid:
            ...
            self._set_state(State.FAILED)
            return
        view = self.ssl.export_keying_material(b'EXTRACTOR-dtls_srtp', 2 * (SRTP_KEY_LEN + SRTP_SALT_LEN))
        if self._role == 'server':
