...
from prefect.serializers import CompressedSerializer, JSONSerializer, PickleSerializer, Serializer
from prefect.server import schemas
from prefect.settings import PREFECT_DEFAULT_RESULT_STORAGE_BLOCK, PREFECT_HOME, PREFECT_RESULTS_PERSIST_BY_DEFAULT, temporary_settings
from prefect.states import Cancelled, Completed, Failed
from prefect.testing.utilities import assert_blocks_equal, assert_uses_result_serializer, assert_uses_result_storage
...
async def test_flow_with_unpersisted_result(prefect_client, persist_result):
    ...
    api_state = (await prefect_client.read_flow_run(state.state_details.flow_run_id)).state
...
async def test_flow_literal_result_is_available_but_not_serialized_or_persisted(prefect_client, value):
...
@pytest.mark.parametrize('serializer', ['json', 'pickle', JSONSerializer(), PickleSerializer(), MyIntSerializer(), 'int-custom', 'compressed/pickle', 'compressed/json', CompressedSerializer(serializer=MyIntSerializer())])
...
async def test_child_flow_persisted_result_due_to_parent_feature(prefect_client, options):
    ...
    api_state = (await prefect_client.read_flow_run(child_state.state_details.flow_run_id)).state
    assert await api_state.result() == 1
...
async def test_root_flow_default_remote_storage():
    ...
    with temporary_settings({PREFECT_RESULTS_PERSIST_BY_DEFAULT: True, PREFECT_DEFAULT_RESULT_STORAGE_BLOCK: 'local-file-system/my-result-storage'}):
...
async def test_root_flow_default_remote_storage_saves_correct_result():
    ...
    saved_python_result = pickle.loads(base64.b64decode(PersistedResultBlob.parse_raw(result_bytes).data))
...
async def test_root_flow_nonexistent_default_storage_block_fails():
    ...
    async def foo():
        ...
        return result_fac.storage_block
    with temporary_settings({PREFECT_RESULTS_PERSIST_BY_DEFAULT: True, PREFECT_DEFAULT_RESULT_STORAGE_BLOCK: 'local-file-system/my-result-storage'}):
        with pytest.raises(ValueError, match='Unable to find block document named my-result-storage'):
            await foo()
...
async def test_root_flow_explicit_result_storage_settings_overrides_default():
    ...
    await LocalFileSystem(basepath='~/.prefect/other-results').save('default-result-storage')

    @flow(result_storage=await LocalFileSystem.load('explicit-storage'))
    async def foo():
        return get_run_context().result_factory.storage_block
    with temporary_settings({PREFECT_RESULTS_PERSIST_BY_DEFAULT: True, PREFECT_DEFAULT_RESULT_STORAGE_BLOCK: 'local-file-system/default-result-storage'}):
        result = await foo()
