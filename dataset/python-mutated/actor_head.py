...
class ActorHead(dashboard_utils.DashboardHeadModule):
    ...
    async def _update_actors(self):
        while True:
            try:
                ...
                await asyncio.sleep(actor_consts.RETRY_GET_ALL_ACTOR_INFO_INTERVAL_SECONDS)
        state_keys = ('state', 'address', 'numRestarts', 'timestamp', 'pid', 'exitDetail', 'startTime', 'endTime', 'reprName')
        ...
        while True:
            try:
                ...
                logger.debug(f'Processing takes {elapsed}. Total process: {len(published)}')
                if self.accumulative_event_processing_s > 0:
                    logger.debug(f'Processing throughput: {self.total_published_events / self.accumulative_event_processing_s} / s')
                ...
                if len(DataSource.actors) > MAX_ACTORS_TO_CACHE:
                    ...
                    while len(DataSource.actors) > MAX_ACTORS_TO_CACHE:
                        if not self.dead_actors_queue:
                            logger.warning(f'More than {MAX_ACTORS_TO_CACHE} live actors are cached')
                            break
    ...
    async def get_all_actors(self, req) -> aiohttp.web.Response:
        return rest_response(success=True, message='All actors fetched.', actors=DataSource.actors, convert_google_style=False)
    ...
    async def get_actor(self, req) -> aiohttp.web.Response:
        ...
        return dashboard_optional_utils.rest_response(success=True, message='Actor details fetched.', detail=actors[actor_id])
