...
from airflow.utils.log.trigger_handler import DropTriggerLogsFilter, LocalQueueHandler, TriggererHandlerWrapper, TriggerMetadataFilter, ctx_indiv_trigger, ctx_task_instance, ctx_trigger_end, ctx_trigger_id
...
class TriggerRunner(threading.Thread, LoggingMixin):
    ...
    async def create_triggers(self):
        ...
        while self.to_create:
            ...
            if trigger_id not in self.triggers:
                ...
                self.triggers[trigger_id] = {'task': asyncio.create_task(self.run_trigger(trigger_id, trigger_instance)), 'name': f'{ti.dag_id}/{ti.run_id}/{ti.task_id}/{ti.map_index}/{ti.try_number} (ID {trigger_id})', 'events': 0}
            else:
            ...
            if details['task'].done():
                ...
                try:
                    ...
                    if isinstance(result, TriggerEvent):
                        self.log.error('Trigger %s returned a TriggerEvent rather than yielding it', details['name'])
                if details['events'] == 0:
                    self.log.error('Trigger %s exited without sending an event. Dependent tasks will be failed.', details['name'])
    ...
    async def block_watchdog(self):
        ...
        while not self.stop:
            ...
            if time_elapsed > 0.2:
                self.log.info("Triggerer's async thread was blocked for %.2f seconds, likely by a badly-written trigger. Set PYTHONASYNCIODEBUG=1 to get more information on overrunning coroutines.", time_elapsed)
