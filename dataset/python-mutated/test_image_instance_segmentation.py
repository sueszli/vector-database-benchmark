...
from modelscope.models.cv.image_instance_segmentation import CascadeMaskRCNNSwinModel
...
class ImageInstanceSegmentationTest(unittest.TestCase):
    ...
    def test_run_with_model_from_modelhub(self):
        ...
        pipeline_ins = pipeline(task=Tasks.image_segmentation, model=model, preprocessor=preprocessor)
    ...
    def test_run_with_model_name(self):
        if False:
            print('Hello World!')
        pipeline_ins = pipeline(task=Tasks.image_segmentation, model=self.model_id)
        print(pipeline_ins(input=self.image)[OutputKeys.LABELS])
    ...
    def test_run_by_direct_model_download(self):
        ...
        pipeline1 = ImageInstanceSegmentationPipeline(model, preprocessor=preprocessor)
        pipeline2 = pipeline(Tasks.image_segmentation, model=model, preprocessor=preprocessor)
