...
def test_mixup_image_data(art_warning, image_batch, alpha, probability):
    if False:
        while True:
            i = 10
    try:
        mixup = CutMix(num_classes=10, alpha=alpha, probability=probability)
...
def test_check_params(art_warning):
    ...
    try:
        with pytest.raises(ValueError):
            _ = CutMix(num_classes=0)
        with pytest.raises(ValueError):
            _ = CutMix(num_classes=10, alpha=0)
        with pytest.raises(ValueError):
            _ = CutMix(num_classes=10, alpha=-1)
        with pytest.raises(ValueError):
            _ = CutMix(num_classes=10, probability=1.1)
        with pytest.raises(ValueError):
            _ = CutMix(num_classes=10, probability=-0.1)
    except ARTTestException as e:
        art_warning(e)