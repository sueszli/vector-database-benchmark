...
try:
    ...
    batchid = 1
func = {1: np.cos, 2: np.sin, 3: np.tan}[batchid]
N = 200
x = np.linspace(0, 4 * np.pi, N)
y = func(x)
source = ColumnDataSource(data=dict(x=x, y=y))
plot = figure(height=400, width=400, title='my wave', tools='crosshair,pan,reset,save,wheel_zoom', x_range=[0, 4 * np.pi], y_range=[-2.5, 2.5])
