...
class ConversationWriter:
    ...
    async def queue(self, conversation_id: str, conversation_history: List[ConversationTurn], meta_data=None):
        formatted_conversation = {'conversation_id': conversation_id, 'conversation': [turn.to_annotation_format(turn_number=turn_number) for (turn_number, turn) in enumerate(conversation_history)]}
        if meta_data:
