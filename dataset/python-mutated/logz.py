...
DEBUG_FORMATTER = Formatter('[%(levelname)s] [%(asctime)s.%(msecs)03d] %(process)d %(name)s:%(funcName)s(%(lineno)d):\n%(message)s\n', '%Y-%m-%d %H:%M:%S')
INFO_FORMATTER = Formatter('[%(levelname)s] [%(asctime)s.%(msecs)03d] %(process)d %(name)s(%(lineno)d): %(message)s\n', '%Y-%m-%d %H:%M:%S')
...
request_header_sort_dict = {'Host': '\x00\x00', 'User-Agent': '\x00\x01'}
...
response_header_sort_dict = {'Content-Length': '~~a', 'Connection': '~~b'}
