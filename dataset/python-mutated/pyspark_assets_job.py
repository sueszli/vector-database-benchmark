...
if __name__ == '__main__':
    run_config = {'ops': {'get_max_temp_per_station': {'config': {'temperature_file': 'temperature.csv', 'version_salt': 'foo'}}, 'get_consolidated_location': {'config': {'station_file': 'stations.csv', 'version_salt': 'foo'}}, 'combine_dfs': {'config': {'version_salt': 'foo'}}, 'pretty_output': {'config': {'version_salt': 'foo'}}}, 'resources': {'source_data_dir': {'config': {'dir': 'asset_job_files'}}, 'savedir': {'config': {'dir': 'asset_job_files'}}}}
    pyspark_assets_config_job = pyspark_assets.to_job(resource_defs={'source_data_dir': source_data_dir, 'savedir': savedir}, config=run_config)
