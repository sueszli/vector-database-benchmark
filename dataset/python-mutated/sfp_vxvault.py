...
class sfp_vxvault(SpiderFootPlugin):
    meta = {'name': 'VXVault.net', 'summary': 'Check if a domain or IP address is malicious according to VXVault.net.', 'flags': [], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'website': 'http://vxvault.net/', 'model': 'FREE_NOAUTH_UNLIMITED', 'references': ['http://vxvault.net/URL_List.php', 'https://github.com/InfectedPacket/VxVault'], 'favIcon': '', 'logo': 'http://vxvault.net/header.png', 'description': 'VxVault is a malware management program to automatically download and classify malware samples. VxVault downloads malware samples from links from online sources such as webpages or RSS feeds, downloads them and attempts to identify the malware using VirusTotal. It then sort the malware onto a local file system and into a SQLite database.'}}
    opts = {'checkaffiliates': True, 'checkcohosts': True, 'cacheperiod': 18}
    optdescs = {'checkaffiliates': 'Apply checks to affiliates?', 'checkcohosts': "Apply checks to sites found to be co-hosted on the target's IP?", 'cacheperiod': 'Hours to cache list data before re-fetching.'}
    results = None
    errorState = False
    ...
    def watchedEvents(self):
        if False:
            for i in range(10):
                print('nop')
        return ['INTERNET_NAME', 'IP_ADDRESS', 'IPV6_ADDRESS', 'AFFILIATE_IPADDR', 'AFFILIATE_IPV6_ADDRESS', 'AFFILIATE_INTERNET_NAME', 'CO_HOSTED_SITE']
    ...
    def retrieveBlacklist(self):
        ...
        res = self.sf.fetchUrl('http://vxvault.net/URL_List.php', timeout=10, useragent=self.opts['_useragent'])
        if res['code'] != '200':
