...
class Dinov2ModelTester:
    ...
    def create_and_check_backbone(self, config, pixel_values, labels):
        ...
        self.parent.assertListEqual(list(result.feature_maps[0].shape), [self.batch_size, model.channels[0], expected_size, expected_size])
        ...
        result = model(pixel_values)
        self.parent.assertEqual(len(result.feature_maps), 1)
        self.parent.assertListEqual(list(result.feature_maps[0].shape), [self.batch_size, self.seq_length, self.hidden_size])
...
class Dinov2ModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    """
    all_model_classes = (Dinov2Model, Dinov2ForImageClassification, Dinov2Backbone) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': Dinov2Model, 'image-classification': Dinov2ForImageClassification} if is_torch_available() else {}
    fx_compatible = False
    test_pruning = False
...
class Dinov2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor([[-2.1747, -0.4729, 1.0936], [-3.278, -0.8269, -0.921], [-2.9129, 1.1284, -0.7306]], device=torch_device)
