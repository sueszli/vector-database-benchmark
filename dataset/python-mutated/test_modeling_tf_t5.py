...
class TFT5ModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        config = T5Config(vocab_size=self.vocab_size, n_positions=self.n_positions, d_model=self.hidden_size, d_ff=self.d_ff, d_kv=self.hidden_size // self.num_attention_heads, num_layers=self.num_hidden_layers, num_heads=self.num_attention_heads, relative_attention_num_buckets=self.relative_attention_num_buckets, dropout_rate=self.dropout_rate, initializer_factor=self.initializer_factor, eos_token_id=self.eos_token_id, bos_token_id=self.pad_token_id, pad_token_id=self.pad_token_id, decoder_start_token_id=self.pad_token_id)
    ...
    def create_and_check_t5_decoder_model_attention_mask_past(self, config, input_ids, decoder_input_ids, attention_mask):
        if False:
            print('Hello World!')
        model = TFT5Model(config=config).get_decoder()
    ...
    def create_and_check_t5_decoder_model_past_large_inputs(self, config, input_ids, decoder_input_ids, attention_mask):
        ...
        output_from_past = model(next_tokens, attention_mask=next_attention_mask, past_key_values=outputs.past_key_values)[0]
...
class TFT5ModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'conversational': TFT5ForConditionalGeneration, 'feature-extraction': TFT5Model, 'summarization': TFT5ForConditionalGeneration, 'text2text-generation': TFT5ForConditionalGeneration, 'translation': TFT5ForConditionalGeneration} if is_tf_available() else {}
    test_onnx = False
...
class TFT5GenerationIntegrationTests(unittest.TestCase):
    ...
    def test_greedy_xla_generate_simple(self):
        ...
        sentences = ['Translate English to German: Today is a beautiful day.', 'Translate English to German: I have four cats, three dogs, two birds, and a horse.']
        ...
        expected_output_string = ['Heute ist ein schöner Tag.', 'Ich habe vier Katzen, drei Hunde, zwei Vögel und ein Pferd.']
    ...
    def test_greedy_generate(self):
        ...
        input_ids = tokenizer(sentences, return_tensors='tf', padding=True).input_ids
        generation_kwargs = {'bad_words_ids': [tokenizer('my').input_ids, tokenizer('ein schöner').input_ids], 'no_repeat_ngram_size': 3, 'do_sample': False, 'repetition_penalty': 2.2}
    ...
    def test_sample_generate(self):
        ...
        input_ids = tokenizer(sentences, return_tensors='tf', padding=True).input_ids
        generation_kwargs = {'do_sample': True, 'bad_words_ids': [tokenizer('my').input_ids, tokenizer('ein schöner').input_ids], 'no_repeat_ngram_size': 3, 'repetition_penalty': 2.2, 'temperature': 0.8, 'top_k': 500, 'top_p': 0.9, 'seed': [20, 0]}
        with tf.device(':/CPU:0'):
            output_ids = model.generate(input_ids, **generation_kwargs)
        output_strings = tokenizer.batch_decode(output_ids, skip_special_tokens=True)
    ...
    def test_beam_search_xla_generate_simple(self):
        ...
        sentences = [model.config.prefix + 'Today is a beautiful day.', model.config.prefix + 'I have four cats, three dogs, two birds, and a horse.']
        input_ids = tokenizer(sentences, return_tensors='tf', padding=True).input_ids
        xla_generate = tf.function(model.generate, jit_compile=True)
        ...
        output_strings = tokenizer.batch_decode(output_ids, skip_special_tokens=True)
        output_strings_xla = tokenizer.batch_decode(output_ids_xla, skip_special_tokens=True)
        expected_output_string = ["Aujourd'hui est une belle journée.", "J'ai quatre chats, trois chiens, deux oiseaux et un cheval."]
        self.assertListEqual(expected_output_string, output_strings)
        self.assertListEqual(expected_output_string, output_strings_xla)
    ...
    def test_beam_search_generate(self):
        ...
        sentences = ['I really love my', 'Translate English to German: the transformers are truly amazing']
        input_ids = tokenizer(sentences, return_tensors='tf', padding=True).input_ids
        generation_kwargs = {'bad_words_ids': [tokenizer('my').input_ids, tokenizer('ein schöner').input_ids], 'no_repeat_ngram_size': 3, 'do_sample': False, 'repetition_penalty': 2.2, 'num_beams': 4}
        output_ids = model.generate(input_ids, **generation_kwargs)
        output_strings = tokenizer.batch_decode(output_ids, skip_special_tokens=True)
...
class TFT5ModelIntegrationTests(unittest.TestCase):
    ...
    def test_summarization(self):
        ...
        dct = tok([model.config.prefix + x for x in [FRANCE_ARTICLE, SHORTER_ARTICLE, IRAN_ARTICLE, ARTICLE_SUBWAY]], max_length=512, padding='max_length', truncation=True, return_tensors='tf')
        self.assertEqual(512, dct['input_ids'].shape[1])
        hypotheses_batch = model.generate(input_ids=dct['input_ids'], attention_mask=dct['attention_mask'], num_beams=4, length_penalty=2.0, max_length=142, min_length=56, no_repeat_ngram_size=3, do_sample=False, early_stopping=True)
    ...
    def test_translation_en_to_de(self):
        ...
        output = model.generate(input_ids=input_ids, num_beams=4, length_penalty=2.0, max_length=50, no_repeat_ngram_size=3, do_sample=False, early_stopping=True)
