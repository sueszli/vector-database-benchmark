...
OFFSET_BOOTLOADER_DEFAULT = 4096
OFFSET_PARTITIONS_DEFAULT = 32768
...
offset_bootloader = load_sdkconfig_hex_value(arg_sdkconfig, 'BOOTLOADER_OFFSET_IN_FLASH', OFFSET_BOOTLOADER_DEFAULT)
offset_partitions = load_sdkconfig_hex_value(arg_sdkconfig, 'PARTITION_TABLE_OFFSET', OFFSET_PARTITIONS_DEFAULT)
...
files_in = [('bootloader', offset_bootloader, max_size_bootloader, arg_bootloader_bin), ('partitions', offset_partitions, max_size_partitions, arg_partitions_bin), ('application', offset_application, max_size_application, arg_application_bin)]
...
with open(file_out, 'wb') as fout:
    for (name, offset, max_size, file_in) in files_in:
        ...
        with open(file_in, 'rb') as fin:
            ...
            fout.write(data)
            cur_offset += len(data)
            print('%-12s@0x%06x % 8d  (% 8d remaining)' % (name, offset, len(data), max_size - len(data)))
            if len(data) > max_size:
                print('ERROR: %s overflows allocated space of %d bytes by %d bytes' % (name, max_size, len(data) - max_size))
                sys.exit(1)
