...
try:
    ...
    raise ImportError('In order to run this example you must have the libraries: ' + '`tornado` and `requests` installed.')
...
class BayesianOptimizationHandler(RequestHandler):
    ...
    _bo = BayesianOptimization(f=black_box_function, pbounds={'x': (-4, 4), 'y': (-3, 3)})
...
if __name__ == '__main__':
    ...
    optimizers_config = [{'name': 'optimizer 1', 'colour': Colours.red}, {'name': 'optimizer 2', 'colour': Colours.green}, {'name': 'optimizer 3', 'colour': Colours.blue}]
    app_thread = threading.Thread(target=run_optimization_app)
    app_thread.daemon = True
    app_thread.start()
    targets = (run_optimizer, run_optimizer, run_optimizer)
    optimizer_threads = []
