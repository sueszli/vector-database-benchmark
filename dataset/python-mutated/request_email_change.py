...
class RequestEmailChange(BaseMutation):
    ...
    class Arguments:
        ...
        redirect_url = graphene.String(required=True, description='URL of a view where users should be redirected to update the email address. URL in RFC 1808 format.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.NOTIFY_USER, description='A notification for account email change.'), WebhookEventInfo(type=WebhookEventAsyncType.ACCOUNT_CHANGE_EMAIL_REQUESTED, description='An account email change was requested.')]
