...
import numpy as np
from AnyQt.QtCore import QItemSelection, QItemSelectionRange, QItemSelectionModel, Qt
...
from Orange.widgets.data.owfeaturestatistics import OWFeatureStatistics
...
continuous_all_missing = VarDataPair(ContinuousVariable('continuous_all_missing'), np.array([np.nan] * 5, dtype=float))
continuous_same = VarDataPair(ContinuousVariable('continuous_same'), np.array([3] * 5, dtype=float))
continuous = [continuous_full, continuous_missing, continuous_all_missing, continuous_same]
...
rgb_all_missing = VarDataPair(DiscreteVariable('rgb_all_missing', values=('r', 'g', 'b')), np.array([np.nan] * 5, dtype=float))
rgb_bins_missing = VarDataPair(DiscreteVariable('rgb_bins_missing', values=('r', 'g', 'b')), np.array([np.nan, 1, 1, 1, np.nan], dtype=float))
rgb_same = VarDataPair(DiscreteVariable('rgb_same', values=('r', 'g', 'b')), np.array([2] * 5, dtype=float))
rgb = [rgb_full, rgb_missing, rgb_all_missing, rgb_bins_missing, rgb_same]
...
ints_all_missing = VarDataPair(DiscreteVariable('ints_all_missing', values=('2', '3', '4')), np.array([np.nan] * 5, dtype=float))
ints_bins_missing = VarDataPair(DiscreteVariable('ints_bins_missing', values=('2', '3', '4')), np.array([np.nan, 1, 1, 1, np.nan], dtype=float))
ints_same = VarDataPair(DiscreteVariable('ints_same', values=('2', '3', '4')), np.array([0] * 5, dtype=float))
ints = [ints_full, ints_missing, ints_all_missing, ints_bins_missing, ints_same]
...
time_full = VarDataPair(TimeVariable('time_full', have_date=True, have_time=True), np.array(_to_timestamps([2000, 2001, 2002, 2003, 2004]), dtype=float))
time_missing = VarDataPair(TimeVariable('time_missing'), np.array(_to_timestamps([2000, np.nan, 2001, 2003, 2004]), dtype=float))
time_all_missing = VarDataPair(TimeVariable('time_all_missing'), np.array(_to_timestamps([np.nan] * 5), dtype=float))
time_same = VarDataPair(TimeVariable('time_same', have_date=True, have_time=True), np.array(_to_timestamps([2004] * 5), dtype=float))
time_negative = VarDataPair(TimeVariable('time_negative', have_date=True, have_time=True), np.array([0, -1, 24 * 60 * 60], dtype=float))
time = [time_full, time_missing, time_all_missing, time_same, time_negative]
...
string_all_missing = VarDataPair(StringVariable('string_all_missing'), np.array([StringVariable.Unknown] * 5, dtype=object))
string_same = VarDataPair(StringVariable('string_same'), np.array(['a'] * 5, dtype=object))
