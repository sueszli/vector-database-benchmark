...
def ext_pillar(minion_id, pillar, url, with_grains=False, header_dict=None, auth=None, username=None, password=None):
    if False:
        for i in range(10):
            print('nop')
    ...
    data = __salt__['http.query'](url=url, decode=True, decode_type='yaml', header_dict=header_dict, auth=auth, username=username, password=password)
    if 'dict' in data:
        return data['dict']
