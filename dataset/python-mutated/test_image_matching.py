...
class ImageMatchingTest(unittest.TestCase):
    ...
    def test_image_matching(self):
        if False:
            for i in range(10):
                print('nop')
        input_location = [['data/test/images/image_matching1.jpg', 'data/test/images/image_matching2.jpg']]
        estimator = pipeline(Tasks.image_matching, model=self.model_id)
        result = estimator(input_location)
        (kpts0, kpts1, conf) = result[0][OutputKeys.MATCHES]
        match_pair_visualization(input_location[0][0], input_location[0][1], kpts0, kpts1, conf, output_filename='quadtree_match.png')
