...
import chainerx
_parameterize = testing.parameterize(*testing.product_dict(testing.product({'test': [True, False], 'size': ['skip', 'explicit'], 'dtype': [numpy.float16, numpy.float32, numpy.float64, chainer.mixed16]}), testing.product({'ndim': [0, 1, 2, 3]}) + [{'input_shape': (5, 4, 3, 2), 'axis': (0, 2, 3)}, {'input_shape': (5, 4), 'axis': 0}, {'input_shape': (5, 4, 3), 'axis': (0, 1)}]))
_inject_backend_tests = testing.inject_backend_tests(None, [{}, {'use_ideep': 'always'}] + testing.product({'use_cuda': [True], 'use_cudnn': ['never', 'always'], 'cuda_device': [0, 1]}) + [{'use_chainerx': True, 'chainerx_device': 'native:0'}, {'use_chainerx': True, 'chainerx_device': 'cuda:0'}, {'use_chainerx': True, 'chainerx_device': 'cuda:1'}])
...
@testing.inject_backend_tests(None, testing.product({'use_cuda': [True], 'use_cudnn': ['never', 'always'], 'cuda_device': [0]}))
@_parameterize
@attr.multi_gpu(2)
class BatchNormalizationMultiGpuTest(BatchNormalizationTestBase, testing.LinkTestCase):
...
@testing.parameterize(*testing.product_dict(testing.product({'dtype': [numpy.float16, numpy.float32, numpy.float64]})))
@_inject_backend_tests
class BatchNormalizationInitializersTest(BatchNormalizationTestBase, testing.LinkInitializersTestCase):
    test = False
    size = 'skip'
    ndim = 1
    input_shape = (5, 4)
...
@testing.parameterize({'nx': 10, 'ny': 10, 'eps': 2e-05}, {'nx': 10, 'ny': 10, 'eps': 0.1})
...
@testing.parameterize(*testing.product({'test': [True, False], 'ndim': [0, 1, 2, 3]}))
...
@testing.parameterize(*testing.product({'size': [3, (2, 3)]}))
...
@testing.parameterize(*testing.product({'dtype': [numpy.float32, numpy.float16, chainer.mixed16]}))
...
@testing.parameterize(*testing.product({'shape': [(2, 4), (2, 5, 3, 4)]}))
...
@testing.parameterize({'shape': (5, 4, 3, 2), 'axis': (0, 2, 3)}, {'shape': (5, 4), 'axis': 0}, {'shape': (5, 4, 3), 'axis': (0, 1)})
...
@testing.parameterize(*testing.product({'x_shape,bn_kwargs': [((4, 3), {'axis': (0,)}), ((4, 3), {'size': (3,)})]}))
...
@testing.inject_backend_tests(['test_param_layout_to_device', 'test_forward'], [{}] + testing.product({'use_cuda': [True], 'use_cudnn': ['never', 'always']}))
