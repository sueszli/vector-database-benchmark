...
class SearchZoomEye:
    ...
    async def fetch_subdomains(self) -> None:
        ...
        response = await AsyncFetcher.fetch_all([subdomain_search_endpoint + 'page=1'], json=True, proxy=self.proxy, headers=headers)
        resp = response[0]
        if resp['status'] != 200:
            return
        total = resp['total']
        ...
        for i in range(2, self.limit):
            response = await AsyncFetcher.fetch_all([subdomain_search_endpoint + f'page={i}'], json=True, proxy=self.proxy, headers=headers)
            resp = response[0]
            if resp['status'] != 200:
                return
    ...
    async def do_search(self) -> None:
        ...
        params = (('query', f'site:{self.word}'), ('page', '1'))
        ...
        if cur_page == -1:
            ...
            for num in range(2, self.limit):
                params = (('query', f'site:{self.word}'), ('page', f'{num}'))
                response = await AsyncFetcher.fetch_all([self.baseurl], json=True, proxy=self.proxy, headers=headers, params=params)
                resp = response[0]
        ...
        for match in matches:
            try:
                ...
                if 'portinfo' in match.keys():
                    ...
                    iurls = {str(iurl.group(1)).replace('"', '') for iurl in re.finditer(self.iurl_regex, match['portinfo']['banner']) if self.word in str(iurl.group(1))}
