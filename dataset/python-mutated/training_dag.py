...
dag = DAG('first_test', default_args={'retries': 1, 'retry_delay': timedelta(minutes=3)}, start_date=pendulum.datetime(2015, 12, 1, tz='UTC'), description='My first test', schedule=config('crons_train', default='@weekly'), catchup=False)
with dag:
    split = PythonOperator(task_id='Split_Create_and_Retrain', provide_context=True, python_callable=split_training, do_xcom_push=True)
    select_vp = PythonOperator(task_id='Select_Valid_Projects', provide_context=True, python_callable=select_from_db, do_xcom_push=True)
    dag_split1 = ShortCircuitOperator(task_id='Create_Condition', python_callable=continue_new)
    dag_split2 = ShortCircuitOperator(task_id='Retrain_Condition', python_callable=continue_old)
    new_models = BashOperator(task_id='Create_Models', bash_command=f'python {_work_dir}/main.py ' + "--projects {{task_instance.xcom_pull(task_ids='Split_Create_and_Retrain', key='new_project_data')}} " + "--tenants {{task_instance.xcom_pull(task_ids='Split_Create_and_Retrain', key='new_tenant_data')}}")
    old_models = BashOperator(task_id='Retrain_Models', bash_command=f'python {_work_dir}/main.py ' + "--projects {{task_instance.xcom_pull(task_ids='Split_Create_and_Retrain', key='old_project_data')}} " + "--tenants {{task_instance.xcom_pull(task_ids='Split_Create_and_Retrain', key='old_tenant_data')}}")
