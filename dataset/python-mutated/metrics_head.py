...
from ray.dashboard.modules.metrics.grafana_dashboard_factory import generate_default_grafana_dashboard, generate_serve_grafana_dashboard, generate_serve_deployment_grafana_dashboard, generate_data_grafana_dashboard
...
PROMETHEUS_CONFIG_INPUT_PATH = os.path.join(METRICS_INPUT_ROOT, 'prometheus', 'prometheus.yml')
...
class MetricsHead(dashboard_utils.DashboardHeadModule):
    ...
    async def grafana_health(self, req) -> aiohttp.web.Response:
        ...
        if self.grafana_host == GRAFANA_HOST_DISABLED_VALUE:
            return dashboard_optional_utils.rest_response(success=True, message='Grafana disabled', grafana_host=GRAFANA_HOST_DISABLED_VALUE)
        grafana_iframe_host = os.environ.get(GRAFANA_IFRAME_HOST_ENV_VAR, self.grafana_host)
        path = f'{self.grafana_host}/{GRAFANA_HEALTHCHECK_PATH}'
        try:
            async with self._session.get(path) as resp:
                if resp.status != 200:
                    return dashboard_optional_utils.rest_response(success=False, message='Grafana healtcheck failed', status=resp.status)
                json = await resp.json()
                if json['database'] != 'ok':
                    return dashboard_optional_utils.rest_response(success=False, message='Grafana healtcheck failed. Database not ok.', status=resp.status, json=json)
                return dashboard_optional_utils.rest_response(success=True, message='Grafana running', grafana_host=grafana_iframe_host, session_name=self._session_name, dashboard_uids=self._dashboard_uids, dashboard_datasource=self._prometheus_name)
        except Exception as e:
            logger.debug('Error fetching grafana endpoint. Is grafana running?', exc_info=e)
            return dashboard_optional_utils.rest_response(success=False, message='Grafana healtcheck failed', exception=str(e))
    ...
    async def prometheus_health(self, req):
        try:
            ...
            async with self._session.get(path) as resp:
                if resp.status != 200:
                    return dashboard_optional_utils.rest_response(success=False, message='prometheus healthcheck failed.', status=resp.status)
                text = await resp.text()
                if 'Prometheus' not in text:
                    return dashboard_optional_utils.rest_response(success=False, message='prometheus healthcheck failed.', status=resp.status, text=text)
                return dashboard_optional_utils.rest_response(success=True, message='prometheus running')
        except Exception as e:
            logger.debug('Error fetching prometheus endpoint. Is prometheus running?', exc_info=e)
            return dashboard_optional_utils.rest_response(success=False, message='prometheus healthcheck failed.', reason=str(e))
    ...
    async def record_dashboard_metrics(self):
        ...
        self._dashboard_head.metrics.metrics_dashboard_cpu.labels(ip=self._ip, pid=self._pid, Component=self._component, SessionName=self._session_name).set(float(dashboard_proc.cpu_percent()) * 100)
        self._dashboard_head.metrics.metrics_dashboard_mem.labels(ip=self._ip, pid=self._pid, Component=self._component, SessionName=self._session_name).set(float(dashboard_proc.memory_full_info().uss) / 1000000.0)
    ...
    async def run(self, server):
        ...
        logger.info(f'Generated prometheus and grafana configurations in: {self._metrics_root}')
