...
from ..utils import urljoin, traverse_obj, int_or_none, mimetype2ext, clean_html, url_or_none, unified_timestamp, str_or_none

class PRXBaseIE(InfoExtractor):
    PRX_BASE_URL_RE = 'https?://(?:(?:beta|listen)\\.)?prx.org/%s'

    def _call_api(self, item_id, path, query=None, fatal=True, note='Downloading CMS API JSON'):
        if False:
            while True:
                i = 10
        return self._download_json(urljoin('https://cms.prx.org/api/v1/', path), item_id, query=query, fatal=fatal, note=note)
    ...
    def _extract_image(cls, image_response):
        ...
        if not isinstance(image_response, dict):
            return
        return {'id': str_or_none(image_response.get('id')), 'filesize': image_response.get('size'), 'width': image_response.get('width'), 'height': image_response.get('height'), 'url': cls._extract_file_link(image_response)}
    ...
    def _extract_base_info(cls, response):
        ...
        return {'id': item_id, 'title': response.get('title') or item_id, 'thumbnails': [thumbnail_dict] if thumbnail_dict else None, 'description': description, 'release_timestamp': unified_timestamp(response.get('releasedAt')), 'timestamp': unified_timestamp(response.get('createdAt')), 'modified_timestamp': unified_timestamp(response.get('updatedAt')), 'duration': int_or_none(response.get('duration')), 'tags': response.get('tags'), 'episode_number': int_or_none(response.get('episodeIdentifier')), 'season_number': int_or_none(response.get('seasonIdentifier'))}
    ...
    def _extract_series_info(cls, series_response):
        ...
        return {**base_info, 'channel_id': account_info.get('channel_id'), 'channel_url': account_info.get('channel_url'), 'channel': account_info.get('channel'), 'series': base_info.get('title'), 'series_id': base_info.get('id')}
    ...
    def _extract_account_info(cls, account_response):
        ...
        return {**base_info, 'title': name, 'channel_id': base_info.get('id'), 'channel_url': 'https://beta.prx.org/accounts/%s' % base_info.get('id'), 'channel': name}
    ...
    def _extract_story_info(cls, story_response):
        ...
        return {**base_info, 'series': series.get('series'), 'series_id': series.get('series_id'), 'channel_id': account.get('channel_id'), 'channel_url': account.get('channel_url'), 'channel': account.get('channel')}
    ...
    def _entries(self, item_id, endpoint, entry_func, query=None):
        ...
        for page in itertools.count(1):
            response = self._call_api(f'{item_id}: page {page}', endpoint, query={**(query or {}), 'page': page, 'per': 100})
    ...
    def _story_playlist_entry(self, response):
        ...
        if not story:
            return
        story.update({'_type': 'url', 'url': 'https://beta.prx.org/stories/%s' % story['id'], 'ie_key': PRXStoryIE.ie_key()})
        return story
...
class PRXStoryIE(PRXBaseIE):
    ...
    def _extract_story(self, story_response):
        ...
        if len(audio_pieces) == 1:
            return {'formats': audio_pieces, **info}
        entries = [{**info, 'id': '%s_part%d' % (info['id'], idx + 1), 'formats': [fmt]} for (idx, fmt) in enumerate(audio_pieces)]
        return {'_type': 'multi_video', 'entries': entries, **info}
...
class PRXSeriesIE(PRXBaseIE):
    _VALID_URL = PRXBaseIE.PRX_BASE_URL_RE % 'series/(?P<id>\\d+)'
    _TESTS = [{'url': 'https://beta.prx.org/series/36252', 'info_dict': {'id': '36252', 'title': 'Outside/In', 'thumbnails': 'count:1', 'description': 'md5:a6bedc5f810777bcb09ab30ff9059114', 'timestamp': 1470684964, 'modified_timestamp': 1582308830, 'channel_id': '206', 'channel_url': 'https://beta.prx.org/accounts/206', 'channel': 'New Hampshire Public Radio', 'series': 'Outside/In', 'series_id': '36252'}, 'playlist_mincount': 39}, {'url': 'https://beta.prx.org/series/25038', 'info_dict': {'id': '25038', 'title': '25038', 'timestamp': 1207612800, 'modified_timestamp': 1207612800, 'channel_id': '206', 'channel_url': 'https://beta.prx.org/accounts/206', 'channel': 'New Hampshire Public Radio', 'series': '25038', 'series_id': '25038'}, 'playlist_count': 0}]

    def _extract_series(self, series_response):
        if False:
            return 10
        info = self._extract_series_info(series_response)
        return {'_type': 'playlist', 'entries': self._entries(info['id'], 'series/%s/stories' % info['id'], self._story_playlist_entry), **info}
...
class PRXAccountIE(PRXBaseIE):
    _VALID_URL = PRXBaseIE.PRX_BASE_URL_RE % 'accounts/(?P<id>\\d+)'
    _TESTS = [{'url': 'https://beta.prx.org/accounts/206', 'info_dict': {'id': '206', 'title': 'New Hampshire Public Radio', 'description': 'md5:277f2395301d0aca563c80c70a18ee0a', 'channel_id': '206', 'channel_url': 'https://beta.prx.org/accounts/206', 'channel': 'New Hampshire Public Radio', 'thumbnails': 'count:1'}, 'playlist_mincount': 380}]
    ...
    def _extract_account(self, account_response):
        ...
        return {'_type': 'playlist', 'entries': itertools.chain(series, stories), **info}
