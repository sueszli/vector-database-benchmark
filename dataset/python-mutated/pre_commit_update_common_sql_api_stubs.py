...
if __name__ == '__main__':
    ...
    subprocess.run(['stubgen', *[os.fspath(path) for path in COMMON_SQL_ROOT.rglob('*.py')]], cwd=AIRFLOW_SOURCES_ROOT_PATH)
    ...
    for target_path in COMMON_SQL_ROOT.rglob('*.pyi'):
        ...
        if not generated_path.exists():
            console.print(f'[red]The {target_path} file is missing in generated files:. This is treated as breaking change.')
            total_removals += 1
            if _force_override:
                console.print(f'[yellow]The file {target_path} has been removed as changes are force-overridden')
                target_path.unlink()
    ...
    if total_removals:
        if not _force_override:
            ...
            console.print(f'\n[bright_blue]As you can see above, there are changes in the common.sql API:[/]\n\n[bright_blue]* additions: {total_additions}[/]\n[bright_blue]* removals: {total_removals}[/]\n')
