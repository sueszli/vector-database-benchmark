...
class LearnerItemModel(QStandardItemModel):
    LEARNERS = [(GBLearner, '', ''), (XGBLearner, 'Extreme Gradient Boosting (xgboost)', 'xgboost'), (XGBRFLearner, 'Extreme Gradient Boosting Random Forest (xgboost)', 'xgboost'), (CatGBLearner, 'Gradient Boosting (catboost)', 'catboost')]
...
class RegEditor(BaseEditor):
    LAMBDAS = list(chain([x / 10000 for x in range(1, 10)], [x / 1000 for x in range(1, 20)], [x / 100 for x in range(2, 20)], [x / 10 for x in range(2, 9)], range(1, 20), range(20, 100, 5), range(100, 1001, 100)))
