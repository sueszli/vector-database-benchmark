...
CLIPSEG_PRETRAINED_CONFIG_ARCHIVE_MAP = {'CIDAS/clipseg-rd64': 'https://huggingface.co/CIDAS/clipseg-rd64/resolve/main/config.json'}
            ...
            for (key, value) in _text_config_dict.items():
                if key in text_config and value != text_config[key] and (key not in ['transformers_version']):
                    if key in text_config_dict:
                        ...
                        message = f'`text_config_dict` is provided which will be used to initialize `CLIPSegTextConfig`. The value `text_config["{key}"]` will be overriden.'
            ...
            for (key, value) in _vision_config_dict.items():
                if key in vision_config and value != vision_config[key] and (key not in ['transformers_version']):
                    if key in vision_config_dict:
                        ...
                        message = f'`vision_config_dict` is provided which will be used to initialize `CLIPSegVisionConfig`. The value `vision_config["{key}"]` will be overriden.'
                    logger.warning(message)
