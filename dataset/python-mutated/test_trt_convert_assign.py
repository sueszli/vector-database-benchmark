...
class TrtConvertAssignTest(TrtLayerAutoScanTest):
    ...
    def sample_program_configs(self):
        ...
        for dims in [0, 1, 4]:
            self.dims = dims
            for dtype in [np.bool_, np.int32, np.float32, np.int64]:
                self.has_bool_dtype = dtype == np.bool_
                ops_config = [{'op_type': 'assign', 'op_inputs': {'X': ['input_data']}, 'op_outputs': {'Out': ['assign_output_data0']}, 'op_attrs': {}}, {'op_type': 'assign', 'op_inputs': {'X': ['assign_output_data0']}, 'op_outputs': {'Out': ['assign_output_data1']}, 'op_attrs': {}}]
                ops = self.generate_op_config(ops_config)
                program_config = ProgramConfig(ops=ops, weights={}, inputs={'input_data': TensorConfig(data_gen=partial(generate_input, dtype))}, outputs=['assign_output_data1'])
