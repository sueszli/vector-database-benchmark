from typing import Any, Optional
from flet_core.control import Control, OptionalNumber
from flet_core.ref import Ref

class Semantics(Control):
    """
    A control that annotates the control tree with a description of the meaning of the widgets.

    Used by accessibility tools, search engines, and other semantic analysis software to determine the meaning of the application.

    -----

    Online docs: https://flet.dev/docs/controls/semantics
    """

    def __init__(self, content: Optional[Control]=None, ref: Optional[Ref]=None, visible: Optional[bool]=None, disabled: Optional[bool]=None, data: Any=None, label: Optional[str]=None):
        if False:
            while True:
                i = 10
        Control.__init__(self, ref=ref, visible=visible, disabled=disabled, data=data)
        self.content = content
        self.label = label

    def _get_control_name(self):
        if False:
            i = 10
            return i + 15
        return 'semantics'

    def _get_children(self):
        if False:
            i = 10
            return i + 15
        children = []
        if self.__content is not None:
            self.__content._set_attr_internal('n', 'content')
            children.append(self.__content)
        return children

    @property
    def label(self):
        if False:
            for i in range(10):
                print('nop')
        return self._get_attr('label')

    @label.setter
    def label(self, value):
        if False:
            i = 10
            return i + 15
        self._set_attr('label', value)

    @property
    def content(self) -> Optional[Control]:
        if False:
            while True:
                i = 10
        return self.__content

    @content.setter
    def content(self, value: Optional[Control]):
        if False:
            i = 10
            return i + 15
        self.__content = value