        ...
        if run is None:
            for step in flow:
                ...
                for task_pathspec in cli_check_dict:
                    ...
                    impc_ne = impc_ne[0]
                    checker.assert_card(step.name, task_id, impc_ne['type'], '%s' % cards_info['pathspec'], card_hash=impc_ne['hash'], exact_match=True)
                    checker.assert_card(step.name, task_id, impc_e['type'], '%d' % random_number, card_hash=impc_e['hash'], exact_match=True)
        else:
            for step in flow:
                if step.name != 'start':
                    continue
                ...
                for task_id in meta_check_dict:
                    ...
                    checker.assert_card(step.name, task_id, impc_ne['type'], '%s' % task_pathspec, card_hash=impc_ne['hash'], exact_match=True)
                    checker.assert_card(step.name, task_id, impc_e['type'], '%d' % random_number, card_hash=impc_e['hash'], exact_match=True)