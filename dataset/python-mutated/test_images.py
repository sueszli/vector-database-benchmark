...
class TestAsyncImages:
    ...
    async def test_method_create_variation(self, client: AsyncOpenAI) -> None:
        image = await client.images.create_variation(image=b'raw file contents')
        assert_matches_type(ImagesResponse, image, path=['response'])

    @parametrize
    async def test_method_create_variation_with_all_params(self, client: AsyncOpenAI) -> None:
        image = await client.images.create_variation(image=b'raw file contents', model='dall-e-2', n=1, response_format='url', size='1024x1024', user='user-1234')
        assert_matches_type(ImagesResponse, image, path=['response'])

    @parametrize
    async def test_raw_response_create_variation(self, client: AsyncOpenAI) -> None:
        response = await client.images.with_raw_response.create_variation(image=b'raw file contents')
        assert response.http_request.headers.get('X-Stainless-Lang') == 'python'
        image = response.parse()
        assert_matches_type(ImagesResponse, image, path=['response'])
    ...
    async def test_method_edit(self, client: AsyncOpenAI) -> None:
        image = await client.images.edit(image=b'raw file contents', prompt='A cute baby sea otter wearing a beret')
        assert_matches_type(ImagesResponse, image, path=['response'])

    @parametrize
    async def test_method_edit_with_all_params(self, client: AsyncOpenAI) -> None:
        image = await client.images.edit(image=b'raw file contents', prompt='A cute baby sea otter wearing a beret', mask=b'raw file contents', model='dall-e-2', n=1, response_format='url', size='1024x1024', user='user-1234')
        assert_matches_type(ImagesResponse, image, path=['response'])

    @parametrize
    async def test_raw_response_edit(self, client: AsyncOpenAI) -> None:
        response = await client.images.with_raw_response.edit(image=b'raw file contents', prompt='A cute baby sea otter wearing a beret')
        assert response.http_request.headers.get('X-Stainless-Lang') == 'python'
        image = response.parse()
        assert_matches_type(ImagesResponse, image, path=['response'])
    ...
    async def test_method_generate(self, client: AsyncOpenAI) -> None:
        image = await client.images.generate(prompt='A cute baby sea otter')
        assert_matches_type(ImagesResponse, image, path=['response'])

    @parametrize
    async def test_method_generate_with_all_params(self, client: AsyncOpenAI) -> None:
        image = await client.images.generate(prompt='A cute baby sea otter', model='dall-e-3', n=1, quality='standard', response_format='url', size='1024x1024', style='vivid', user='user-1234')
        assert_matches_type(ImagesResponse, image, path=['response'])

    @parametrize
    async def test_raw_response_generate(self, client: AsyncOpenAI) -> None:
        response = await client.images.with_raw_response.generate(prompt='A cute baby sea otter')
        assert response.http_request.headers.get('X-Stainless-Lang') == 'python'
        image = response.parse()
        assert_matches_type(ImagesResponse, image, path=['response'])