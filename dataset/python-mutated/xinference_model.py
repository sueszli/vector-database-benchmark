...
class XinferenceModel(BaseLLM):
    ...
    def _init_client(self) -> Any:
        ...
        client = XinferenceLLM(server_url=self.credentials['server_url'], model_uid=self.credentials['model_uid'])
    ...
    def _run(self, messages: List[PromptMessage], stop: Optional[List[str]]=None, callbacks: Callbacks=None, **kwargs) -> LLMResult:
        ...
        return self._client.generate([prompts], stop, callbacks, generate_config={'stop': stop, 'stream': self.streaming, **self.provider_model_kwargs})
