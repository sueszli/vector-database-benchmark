...
class ShippingZoneUpdateInput(ShippingZoneCreateInput):
    remove_warehouses = NonNullList(graphene.ID, description='List of warehouses to unassign from a shipping zone')
    remove_channels = NonNullList(graphene.ID, description='List of channels to unassign from the shipping zone.')
...
class ShippingZoneUpdate(ShippingZoneMixin, ModelMutation):
    ...
    class Arguments:
        ...
        input = ShippingZoneUpdateInput(description='Fields required to update a shipping zone.', required=True)
