...
class PytestStep(Step, ABC):
    ...
    async def _run(self, connector_under_test: Container) -> StepResult:
        ...
        test_environment = await self.install_testing_environment(connector_under_test, test_config_file_name, test_config_file, self.extra_dependencies_names)
    ...
    async def install_testing_environment(self, built_connector_container: Container, test_config_file_name: str, test_config_file: File, extra_dependencies_names: Iterable[str]) -> Callable:
        ...
        container_with_test_deps = await pipelines.dagger.actions.python.common.with_python_connector_installed(self.context, built_connector_container.with_entrypoint([]), str(self.context.connector.code_directory), additional_dependency_groups=extra_dependencies_names)
        if self.common_test_dependencies:
            container_with_test_deps = container_with_test_deps.with_exec(['pip', 'install', f"{' '.join(self.common_test_dependencies)}"], skip_entrypoint=True)
        return container_with_test_deps.with_mounted_file(test_config_file_name, test_config_file).with_(secret_mounting_function).with_env_variable('PYTHONPATH', '.')
...
async def run_all_tests(context: ConnectorContext) -> List[StepResult]:
    ...
    async with asyncer.create_task_group() as task_group:
        tasks = [task_group.soonify(IntegrationTests(context).run)(connector_container), task_group.soonify(AcceptanceTests(context, context.concurrent_cat).run)(connector_container), task_group.soonify(CheckBaseImageIsUsed(context).run)()]
