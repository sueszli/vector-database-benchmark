...
@command('read_webpage', 'Read a webpage, and extract specific information from it if a question is specified. If you are looking to extract specific information from the webpage, you should specify a question.', {'url': JSONSchema(type=JSONSchema.Type.STRING, description='The URL to visit', required=True), 'question': JSONSchema(type=JSONSchema.Type.STRING, description='A question that you want to answer using the content of the webpage.', required=False)})
@validate_url
async def read_webpage(url: str, agent: Agent, question: str='') -> str:
    ...
    try:
        ...
        elif agent.llm_provider.count_tokens(text, agent.llm.name) > TOKENS_TO_TRIGGER_SUMMARY:
            text = await summarize_memorize_webpage(url, text, question or None, agent, driver)
        ...
        return (f"Page content{(' (summary)' if summarized else '')}:" if return_literal_content else 'Answer gathered from webpage:') + f' {text_fmt}\n\nLinks:\n{links_fmt}'
    except WebDriverException as e:
        msg = e.msg.split('\n')[0]
        if 'net::' in msg:
            raise BrowsingError(f'A networking error occurred while trying to load the page: ' + re.sub('^unknown error: ', '', msg))
...
async def summarize_memorize_webpage(url: str, text: str, question: str | None, agent: Agent, driver: Optional[WebDriver]=None) -> str:
    ...
    (summary, _) = await summarize_text(text, question=question, llm_provider=agent.llm_provider, config=agent.legacy_config)
    return summary