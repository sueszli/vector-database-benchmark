...
class TestHeatmap:
    ...
    default_kws = dict(vmin=None, vmax=None, cmap=None, center=None, robust=False, annot=False, fmt='.2f', annot_kws=None, cbar=True, cbar_kws=None, mask=None)
...
class TestDendrogram:
    ...
    default_kws = dict(linkage=None, metric='euclidean', method='single', axis=1, label=True, rotate=False)
    ...
    if not _no_scipy:
        if _no_fastcluster:
            ...
            x_norm_linkage = fastcluster.linkage_vector(x_norm.T, metric='euclidean', method='single')
        x_norm_dendrogram = hierarchy.dendrogram(x_norm_linkage, no_plot=True, color_threshold=-np.inf)
...
class TestClustermap:
    ...
    letters = pd.Series(['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'], name='letters')
    df_norm = pd.DataFrame(x_norm, columns=letters)
    default_kws = dict(pivot_kws=None, z_score=None, standard_scale=None, figsize=(10, 10), row_colors=None, col_colors=None, dendrogram_ratio=0.2, colors_ratio=0.03, cbar_pos=(0, 0.8, 0.05, 0.2))
    default_plot_kws = dict(metric='euclidean', method='average', colorbar_kws=None, row_cluster=True, col_cluster=True, row_linkage=None, col_linkage=None, tree_kws=None)
