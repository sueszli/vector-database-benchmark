...
class TestDistributeFpnProposalsAPI(unittest.TestCase):
    ...
    def test_dygraph_with_static(self):
        ...
        (multi_rois, restore_ind, rois_num_per_level) = paddle.vision.ops.distribute_fpn_proposals(fpn_rois=rois, min_level=2, max_level=5, refer_level=4, refer_scale=224, rois_num=rois_num)
        fetch_list = multi_rois + [restore_ind] + rois_num_per_level
        exe = paddle.static.Executor()
        output_stat = exe.run(paddle.static.default_main_program(), feed={'rois': self.rois_np, 'rois_num': self.rois_num_np}, fetch_list=fetch_list, return_numpy=False)
        output_stat_np = []
        ...
        (multi_rois_dy, restore_ind_dy, rois_num_per_level_dy) = paddle.vision.ops.distribute_fpn_proposals(fpn_rois=rois_dy, min_level=2, max_level=5, refer_level=4, refer_scale=224, rois_num=rois_num_dy)
