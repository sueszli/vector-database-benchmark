...
class apigeeconnectCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'egress': ('id', 'http_response', 'status', 'project', 'trace_id', 'endpoint', 'name'), 'list_connections': ('parent', 'page_size', 'page_token')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=apigeeconnectCallTransformer()):
    if False:
        i = 10
        return i + 15
