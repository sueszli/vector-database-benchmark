...
class ApolloSearchSchema(BaseModel):
    person_titles: list[str] = Field(..., description='The titles of the people to search for.')
    page: int = Field(1, description='The page of results to retrieve. Default value is 1.')
    per_page: int = Field(25, description='The number of results to retrieve per page. Default value is 25.')
    num_of_employees: list[int] = Field([], description='The number of employees to filter by in format [start_range, end_range]. Default value is empty array.')
    organization_domains: str = Field('', description='The organization domains to search within. It is optional field.')
    person_location: str = Field('', description='Region country/state/city filter to search for. It is optional field.')
...
class ApolloSearchTool(BaseTool):
    ...
    """
    name = 'ApolloSearch'
    description = 'A tool for performing a Apollo search and extracting people data.Input should include API key, organization domains, page number, and person titles.'
