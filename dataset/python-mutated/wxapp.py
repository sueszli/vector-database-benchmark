...
class wxapp:
    ...
    def check_app_login(self, get):
        ...
        if not btapp_info:
            return public.returnMsg(False, '未绑定')
        if not btapp_info['open']:
            return public.returnMsg(False, '未开启API')
        if not 'apps' in btapp_info:
            return public.returnMsg(False, '未绑定手机')
        if not btapp_info['apps']:
            return public.returnMsg(False, '未绑定手机')
        try:
            session_id = public.get_session_id()
            if not os.path.exists(self.app_path + 'app_login_check.pl'):
                return public.returnMsg(False, '等待APP扫码登录1')
            ...
            if len(session_id) != 64:
                return public.returnMsg(False, '等待APP扫码登录2')
            ...
            if time.time() - float(init_time) > 60:
                return public.returnMsg(False, '当前二维码失效')
            import uuid
            if status != uuid.UUID(int=uuid.getnode()).hex[-12:]:
                return public.returnMsg(False, '当前二维码失效222')
            cache.delete(session_id)
            cache.delete(tid)
