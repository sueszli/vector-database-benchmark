...
def get_num_issues():
    ...
    find_first_issue = re.compile('issue-id-\\d+')
...
def system_tray():
    ...
    while True:
        ...
        if seconds % 12 == 0:
            (issues, first_issue) = get_num_issues()
            menu_def = ['root', ['{} Issues'.format(issues), '{} First Issue'.format(first_issue), '---', '&View Issues Online', '&Pull Request', '&Discord', '---', '&Run GUI', '&Refresh', 'E&xit']]
