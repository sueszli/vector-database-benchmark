...
async def serve(q: Q):
    if not q.client.initialized:
        ...
        q.page['meta'] = ui.meta_card(box='', theme='custom', layouts=[ui.layout(breakpoint='xs', zones=[ui.zone('mobile_content'), ui.zone('footer')]), ui.layout(breakpoint='817px', zones=[ui.zone('header'), ui.zone('content', direction=ui.ZoneDirection.ROW, zones=[ui.zone('colors', size='30%'), ui.zone('preview', size='70%')]), ui.zone('footer')])])
        q.page['header'] = ui.header_card(box='header', title='Theme generator', subtitle='Color your app easily', icon='Color', icon_color='$card')
        q.page['mobile_header'] = ui.header_card(box='mobile_content', icon='Color', title='Theme generator', subtitle='Color your app easily', items=[ui.button(name='show_side_panel', label=' ', icon='Color')])
        q.client.color_items = [ui.color_picker(name='primary', label='Primary', trigger=True, alpha=False, inline=True, value=q.client.primary), ui.color_picker(name='text', label='Text', trigger=True, alpha=False, inline=True, value=q.client.text), ui.color_picker(name='card', label='Card', trigger=True, alpha=False, inline=True, value=q.client.card), ui.color_picker(name='page', label='Page', trigger=True, alpha=False, inline=True, value=q.client.page), ui.text_xl('Check contrast'), ui.message_bar(name='text_card', type='success', text='Contrast between **text** and **card** is great!'), ui.message_bar(name='card_primary', type='success', text='Contrast between **card** and **primary** is great!'), ui.message_bar(name='text_page', type='success', text='Contrast between **text** and **page** is great!'), ui.message_bar(name='page_primary', type='success', text='Contrast between **page** and **primary** is great!'), ui.text_xl('Copy code'), ui.text(name='code', content=get_theme_code(q))]
        ...
        q.client.themes = [ui.theme(name='custom', text=q.client.text, card=q.client.card, page=q.client.page, primary=q.client.primary)]
        q.client.initialized = True
    if q.args.show_side_panel:
        q.page['meta'].side_panel = ui.side_panel(title='Adjust theme colors', items=q.client.color_items, closable=True, width='min(75%, 420px)')
    if q.args.primary:
