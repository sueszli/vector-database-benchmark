...
def rf_mean_residual_deviance():
    ...
    rf.train(x=predictors, y=response_col, training_frame=train, validation_frame=valid)
    rf_mrd = rf.mean_residual_deviance(train=True, valid=True, xval=True)
    assert isinstance(rf_mrd['train'], float), 'Expected training mean residual deviance to be a float, but got {0}'.format(type(rf_mrd['train']))
    assert isinstance(rf_mrd['valid'], float), 'Expected validation mean residual deviance to be a float, but got {0}'.format(type(rf_mrd['valid']))
