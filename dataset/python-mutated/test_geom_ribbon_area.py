import numpy as np
import pandas as pd
from plotnine import aes, after_stat, coord_flip, facet_wrap, geom_area, geom_line, geom_ribbon, ggplot, scale_x_continuous
n = 4
m = 100
width = 2 * np.pi
x = np.linspace(0, width, m)
data = pd.DataFrame({'x': np.tile(x, n), 'ymin': np.hstack([np.sin(x) + 2 * i for i in range(n)]), 'ymax': np.hstack([np.sin(x) + 2 * i + 1 for i in range(n)]), 'z': np.repeat(range(n), m)})
...
class TestOutlineType:
    x = np.arange(10)
    d = 5
    data = pd.DataFrame({'x': x, 'y1': x, 'y2': x + 2 * d, 'y3': x + 4 * d, 'y4': x + 6 * d})
    p = ggplot(data, aes('x', ymax=after_stat('ymin + d'))) + geom_ribbon(aes(ymin='y1'), size=1, fill='bisque', color='orange', outline_type='upper') + geom_ribbon(aes(ymin='y2'), size=1, fill='khaki', color='darkkhaki', outline_type='lower') + geom_ribbon(aes(ymin='y3'), size=1, fill='plum', color='purple', outline_type='both') + geom_ribbon(aes(ymin='y4'), size=1, fill='lightblue', color='cadetblue', outline_type='full')
