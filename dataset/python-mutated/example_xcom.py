...
with DAG('example_xcom', schedule='@once', start_date=pendulum.datetime(2021, 1, 1, tz='UTC'), catchup=False, tags=['example']) as dag:
    bash_push = BashOperator(task_id='bash_push', bash_command='echo "bash_push demo"  && echo "Manually set xcom value {{ ti.xcom_push(key="manually_pushed_value", value="manually_pushed_value") }}" && echo "value_by_return"')
    bash_pull = BashOperator(task_id='bash_pull', bash_command=f'''echo "bash pull demo" && echo "The xcom pushed manually is {XComArg(bash_push, key='manually_pushed_value')}" && echo "The returned_value xcom is {XComArg(bash_push)}" && echo "finished"''', do_xcom_push=False)
