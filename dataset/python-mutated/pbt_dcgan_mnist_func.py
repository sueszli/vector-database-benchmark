...
import numpy as np
from ray.tune.examples.pbt_dcgan_mnist.common import beta1, MODEL_PATH, demo_gan, get_data_loader, plot_images, train_func, weights_init, Discriminator, Generator, Net
...
if __name__ == '__main__':
    ...
    scheduler = PopulationBasedTraining(perturbation_interval=5, hyperparam_mutations={'netG_lr': lambda : np.random.uniform(0.01, 1e-05), 'netD_lr': lambda : np.random.uniform(0.01, 1e-05)})
    tune_iter = 5 if args.smoke_test else 300
    tuner = tune.Tuner(dcgan_train, run_config=train.RunConfig(name='pbt_dcgan_mnist', stop={'training_iteration': tune_iter}, verbose=1), tune_config=tune.TuneConfig(metric='is_score', mode='max', num_samples=8, scheduler=scheduler), param_space={'netG_lr': tune.choice([0.0001, 0.0002, 0.0005]), 'netD_lr': tune.choice([0.0001, 0.0002, 0.0005]), 'mnist_model_ref': mnist_model_ref})
    results = tuner.fit()
    if not args.smoke_test:
