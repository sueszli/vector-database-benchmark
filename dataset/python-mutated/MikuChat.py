...
class MikuChat(AsyncGeneratorProvider):
    ...
    @classmethod
    async def create_async_generator(cls, model: str, messages: list[dict[str, str]], **kwargs) -> AsyncGenerator:
        if not model:
            model = 'gpt-3.5-turbo'
        headers = {'authority': 'api.catgpt.cc', 'accept': 'application/json', 'origin': cls.url, 'referer': f'{cls.url}/chat/', 'x-app-version': 'undefined', 'x-date': get_datetime(), 'x-fingerprint': get_fingerprint(), 'x-platform': 'web'}
        async with StreamSession(headers=headers, impersonate='chrome107') as session:
            data = {'model': model, 'top_p': 0.8, 'temperature': 0.5, 'presence_penalty': 1, 'frequency_penalty': 0, 'max_tokens': 2000, 'stream': True, 'messages': messages}
