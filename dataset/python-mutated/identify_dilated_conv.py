...
def make_identify_dilated_conv_tests(options):
    ...
    test_parameters = [{'input_shape': [[1, 3, 4, 3], [4, 6, 6, 1]], 'filter_shape': [[1, 1], [2, 3], [3, 3]], 'strides': [[1, 1, 1, 1], [1, 2, 3, 1]], 'dilations': [[1, 3, 2, 1], [1, 2, 2, 1], [1, 2, 1, 1]], 'padding': ['VALID', 'SAME'], 'data_format': ['NHWC'], 'constant_filter': [True, False], 'channel_multiplier': [1, 2]}]
    ...
    def build_graph(parameters):
        ...
        out = tf.nn.convolution(input=input_tensor, filters=filter_input, strides=parameters['strides'], dilations=parameters['dilations'], padding=parameters['padding'], data_format=parameters['data_format'])
    ...
    make_zip_of_tests(options, test_parameters, build_graph, build_inputs, expected_tf_failures=168)