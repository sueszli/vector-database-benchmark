...
async def test_config_schema():
    """Test CONFIG_SCHEMA."""
    tn_auth.CONFIG_SCHEMA({'type': 'trusted_networks', 'trusted_networks': ['192.168.0.1'], 'trusted_users': {'192.168.0.1': ['a1ab982744b64757bf80515589258924', {'group': 'system-group'}]}})
    with pytest.raises(vol.Invalid):
        tn_auth.CONFIG_SCHEMA({'type': 'trusted_networks', 'trusted_networks': ['192.168.0.1'], 'trusted_users': {'192.168.0.1': ['abcde']}})
...
async def test_validate_access_proxy(hass: HomeAssistant, provider) -> None:
    ...
    await async_setup_component(hass, 'http', {'http': {CONF_TRUSTED_PROXIES: ['192.168.128.0/31', 'fd00::1'], CONF_USE_X_FORWARDED_FOR: True}})
...
async def test_trusted_users_login(manager_with_user, provider_with_user) -> None:
    ...
    flow = await provider_with_user.async_login_flow({'ip_address': ip_address('127.0.0.1')})
    step = await flow.async_step_init()
    assert step['type'] == FlowResultType.ABORT
    assert step['reason'] == 'not_allowed'
    flow = await provider_with_user.async_login_flow({'ip_address': ip_address('192.168.0.1')})
    step = await flow.async_step_init()
    assert step['step_id'] == 'init'
    schema = step['data_schema']
    assert schema({'user': owner.id})
    ...
    flow = await provider_with_user.async_login_flow({'ip_address': ip_address('192.168.128.1')})
    step = await flow.async_step_init()
    assert step['step_id'] == 'init'
    schema = step['data_schema']
    assert schema({'user': user.id})
    ...
    with pytest.raises(vol.Invalid):
        assert schema({'user': sys_user.id})
    flow = await provider_with_user.async_login_flow({'ip_address': ip_address('fd00::1')})
    step = await flow.async_step_init()
    assert step['step_id'] == 'init'
    schema = step['data_schema']
    with pytest.raises(vol.Invalid):
...
async def test_trusted_group_login(manager_with_user, provider_with_user) -> None:
    ...
    owner = await manager_with_user.async_create_user('test-owner')
    user = await manager_with_user.async_create_user('test-user')
    await manager_with_user.async_update_user(user, group_ids=[auth.const.GROUP_ID_USER])
    config = provider_with_user.config['trusted_users']
    assert ip_network('192.168.0.1') in config
    config[ip_network('192.168.0.1')] = [{'group': [auth.const.GROUP_ID_USER]}]
    assert ip_network('192.168.128.0/24') in config
    config[ip_network('192.168.128.0/24')] = [owner.id, {'group': [auth.const.GROUP_ID_USER]}]
    flow = await provider_with_user.async_login_flow({'ip_address': ip_address('127.0.0.1')})
    step = await flow.async_step_init()
    assert step['type'] == FlowResultType.ABORT
    assert step['reason'] == 'not_allowed'
...
async def test_bypass_login_flow(manager_bypass_login, provider_bypass_login) -> None:
    ...
    flow = await provider_bypass_login.async_login_flow({'ip_address': ip_address('127.0.0.1')})
    step = await flow.async_step_init()
    assert step['type'] == FlowResultType.ABORT
    assert step['reason'] == 'not_allowed'
    flow = await provider_bypass_login.async_login_flow({'ip_address': ip_address('192.168.0.1')})
    step = await flow.async_step_init()
    assert step['type'] == FlowResultType.CREATE_ENTRY
