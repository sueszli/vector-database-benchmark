    ...
    def ops(self):
        if False:
            for i in range(10):
                print('nop')
        '\n        operation\n        '
        emb = paddle.nn.Embedding(self.dict_size, self.emb_dim, sparse=True, padding_idx=self.padding_idx, weight_attr=paddle.ParamAttr(name=self.name, initializer=paddle.nn.initializer.XavierUniform()))
        return emb
...
class FCLayer:
    ...
    def ops(self):
        if False:
            return 10
        '\n        operation\n        '
        fc = FC(size=self.fc_dim, param_attr=paddle.ParamAttr(name='%s.w' % self.name), bias_attr=paddle.ParamAttr(name='%s.b' % self.name), act=self.act)
        return fc
...
class BOW(paddle.nn.Layer):
    ...
    def __init__(self, conf_dict):
        ...
        self.bow_layer = paddle.nn.Linear(in_features=self.bow_dim, out_features=self.bow_dim)
