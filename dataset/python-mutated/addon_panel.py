...
async def async_setup_addon_panel(hass: HomeAssistant, hassio):
    ...
    for (addon, data) in panels.items():
        if not data[ATTR_ENABLE]:
            continue
        jobs.append(asyncio.create_task(_register_panel(hass, addon, data), name=f'register panel {addon}'))
    if jobs:
...
async def _register_panel(hass, addon, data):
    ...
    await panel_custom.async_register_panel(hass, frontend_url_path=addon, webcomponent_name='hassio-main', sidebar_title=data[ATTR_TITLE], sidebar_icon=data[ATTR_ICON], js_url='/api/hassio/app/entrypoint.js', embed_iframe=True, require_admin=data[ATTR_ADMIN], config={'ingress': addon})