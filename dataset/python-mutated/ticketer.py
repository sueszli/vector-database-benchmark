...
if __name__ == '__main__':
    ...
    parser.add_argument('-spn', action='store', help='SPN (service/server) of the target service the silver ticket will be generated for. if omitted, golden ticket will be created')
    parser.add_argument('-request', action='store_true', default=False, help='Requests ticket to domain and clones it changing only the supplied information. It requires specifying -user')
    parser.add_argument('-domain', action='store', required=True, help='the fully qualified domain name (e.g. contoso.com)')
    parser.add_argument('-domain-sid', action='store', required=True, help='Domain SID of the target domain the ticker will be generated for')
    ...
    parser.add_argument('-old-pac', action='store_true', help='Use the old PAC structure to create your ticket (exclude PAC_ATTRIBUTES_INFO and PAC_REQUESTOR')
    parser.add_argument('-duration', action='store', default='87600', help='Amount of hours till the ticket expires (default = 24*365*10)')
    ...
    parser.add_argument('-impersonate', action='store', help='Sapphire ticket. target username that will be impersonated (through S4U2Self+U2U) for querying the ST and extracting the PAC, which will be included in the new ticket')
    if len(sys.argv) == 1:
        parser.print_help()
        print('\nExamples: ')
    ...
    if options.impersonate:
        missing_params = [param_name for (param, param_name) in zip([options.request, options.aesKey, options.nthash, options.domain, options.user, options.password, options.domain_sid, options.user_id], ['-request', '-aesKey', '-nthash', '-domain', '-user', '-password', '-domain-sid', '-user-id']) if param is None or (param_name == '-request' and (not param))]
        if missing_params:
