...
@PIPELINES.register_module(Tasks.speech_separation, module_name=Pipelines.funasr_speech_separation)
class SeparationPipeline(Pipeline):
    ...
    def __init__(self, model: Union[Model, str]=None, ngpu: int=1, **kwargs):
        ...
        self.funasr_infer_modelscope = ss_inference_launch.inference_launch(mode=self.cmd['mode'], batch_size=self.cmd['batch_size'], ngpu=ngpu, log_level=self.cmd['log_level'], ss_infer_config=self.cmd['ss_infer_config'], ss_model_file=self.cmd['ss_model_file'], output_dir=self.cmd['output_dir'], dtype=self.cmd['dtype'], seed=self.cmd['seed'], num_workers=self.cmd['num_workers'], num_spks=self.cmd['num_spks'], param_dict=self.cmd['param_dict'], **kwargs)
    ...
    def get_cmd(self, config_path, extra_args, model_path) -> Dict[str, Any]:
        ...
        ss_model_path = os.path.join(model_dir, model_cfg['model']['model_config']['ss_model_name'])
        ss_model_config = os.path.join(model_dir, model_cfg['model']['model_config']['ss_model_config'])
        ...
        cmd = {'mode': mode, 'batch_size': 1, 'ngpu': 1, 'log_level': 'ERROR', 'ss_infer_config': ss_model_config, 'ss_model_file': ss_model_path, 'output_dir': None, 'dtype': 'float32', 'seed': 0, 'num_workers': 0, 'num_spks': 2, 'param_dict': None, 'fs': {'model_fs': None, 'audio_fs': None}}
    ...
    def run_inference(self, cmd, **kwargs):
        ...
        if self.framework == Frameworks.torch:
            ss_result = self.funasr_infer_modelscope(data_path_and_name_and_type=cmd['name_and_type'], raw_inputs=cmd['raw_inputs'], output_dir_v2=cmd['output_dir'], fs=cmd['fs'], param_dict=cmd['param_dict'], **kwargs)
        else:
