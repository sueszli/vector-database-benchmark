...
class Erf(function_node.FunctionNode):
    ...
    def forward_cpu(self, x):
        ...
        if _erf_cpu is None:
            try:
                ...
                warnings.warn('SciPy is not available. Forward computation of erf in CPU can be slow without SciPy.', chainer.warnings.PerformanceWarning)
    ...
    def forward_gpu(self, x):
        if False:
            print('Hello World!')
        self.retain_inputs((0,))
        return (cuda.elementwise('T x', 'T y', 'y = erf(x)', 'elementwise_erf')(x[0]),)
