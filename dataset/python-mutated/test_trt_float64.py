                ...
                for ends in [[2, 2], [5, 5], [1, -1]]:
                    for decrease_axis in [[], [1], [2], [-1], [-100]]:
                        for infer_flags in [[-1]]:
                            ...
                            ops_config = [{'op_type': 'slice', 'op_inputs': {'Input': ['input_data']}, 'op_outputs': {'Out': ['slice_output_data']}, 'op_attrs': dics[0], 'outputs_dtype': {'slice_output_data': np.float64}}]
    ...
    def sample_program_configs(self):
        ...
        def generate_input(shape, op_type):
            if False:
                while True:
                    i = 10
            return np.random.randint(low=1, high=10000, size=shape).astype(np.float64)
        for shape in [[2, 32, 16], [1, 8, 16, 32]]:
            for op_type in ['elementwise_add', 'elementwise_mul', 'elementwise_sub']:
                for axis in [0, -1]:
                    self.dims = len(shape)
                    dics = [{'axis': axis}]
                    ops_config = [{'op_type': op_type, 'op_inputs': {'X': ['input_data1'], 'Y': ['input_data2']}, 'op_outputs': {'Out': ['output_data']}, 'op_attrs': dics[0], 'outputs_dtype': {'slice_output_data': np.float64}}]
                    ops = self.generate_op_config(ops_config)
