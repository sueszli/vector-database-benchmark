...
class GetGameHighScores(TelegramMethod[List[GameHighScore]]):
    ...
    if TYPE_CHECKING:
        ...
        def __init__(__pydantic__self__, *, user_id: int, chat_id: Optional[int]=None, message_id: Optional[int]=None, inline_message_id: Optional[str]=None, **__pydantic_kwargs: Any) -> None:
            if False:
                for i in range(10):
                    print('nop')
            super().__init__(user_id=user_id, chat_id=chat_id, message_id=message_id, inline_message_id=inline_message_id, **__pydantic_kwargs)