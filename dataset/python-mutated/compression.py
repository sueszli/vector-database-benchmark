...
import deeplake
from deeplake.util.exceptions import SampleCompressionError, SampleDecompressionError, SampleReadError, UnsupportedCompressionError, CorruptedSampleError
from deeplake.util.object_3d.read_3d_data import read_3d_data
from deeplake.compression import get_compression_type, BYTE_COMPRESSION, VIDEO_COMPRESSION, AUDIO_COMPRESSION, POINT_CLOUD_COMPRESSION, MESH_COMPRESSION, NIFTI_COMPRESSION
...
if sys.byteorder == 'little':
    ...
    _NATIVE_INT32 = '>i4'
    _NATIVE_FLOAT32 = '>f4'
DIMS_RE = re.compile(b' ([0-9]+)x([0-9]+)')
FPS_RE = re.compile(b' ([0-9]+) fps,')
DURATION_RE = re.compile(b'Duration: ([0-9:.]+),')
INFO_RE = re.compile(b'([a-z]+)=([0-9./]+)')
_JPEG_SOFS = [b'\xff\xc0', b'\xff\xc2', b'\xff\xc1', b'\xff\xc3', b'\xff\xc5', b'\xff\xc6', b'\xff\xc7', b'\xff\xc9', b'\xff\xca', b'\xff\xcb', b'\xff\xcd', b'\xff\xce', b'\xff\xcf', b'\xff\xde', b'\xff\xcc', b'\xff\xdc', b'\xff\xdd', b'\xff\xdf', *map(lambda x: x.to_bytes(2, 'big'), range(65504, 65520)), b'\xff\xdb', b'\xff\xfe', b'\xff\xda']
