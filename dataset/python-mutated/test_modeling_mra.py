...
if is_torch_available():
    import torch
    from transformers import MraForMaskedLM, MraForMultipleChoice, MraForQuestionAnswering, MraForSequenceClassification, MraForTokenClassification, MraModel
    from transformers.models.mra.modeling_mra import MRA_PRETRAINED_MODEL_ARCHIVE_LIST

class MraModelTester:

    def __init__(self, parent, batch_size=2, seq_length=8, is_training=True, use_input_mask=True, use_token_type_ids=True, use_labels=True, vocab_size=99, hidden_size=16, num_hidden_layers=2, num_attention_heads=2, intermediate_size=36, hidden_act='gelu', hidden_dropout_prob=0.0, attention_probs_dropout_prob=0.0, max_position_embeddings=512, type_vocab_size=16, type_sequence_label_size=2, initializer_range=0.02, num_labels=3, num_choices=4, scope=None):
        if False:
            print('Hello World!')
        self.parent = parent
    ...
    def get_config(self):
        if False:
            i = 10
            return i + 15
        return MraConfig(vocab_size=self.vocab_size, hidden_size=self.hidden_size, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, hidden_act=self.hidden_act, hidden_dropout_prob=self.hidden_dropout_prob, attention_probs_dropout_prob=self.attention_probs_dropout_prob, max_position_embeddings=self.max_position_embeddings, type_vocab_size=self.type_vocab_size, is_decoder=False, initializer_range=self.initializer_range)
...
class MraModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (MraModel, MraForMaskedLM, MraForMultipleChoice, MraForQuestionAnswering, MraForSequenceClassification, MraForTokenClassification) if is_torch_available() else ()
    test_pruning = False
    test_headmasking = False
    test_torchscript = False
    has_attentions = False
    all_generative_model_classes = ()
    pipeline_model_mapping = {'feature-extraction': MraModel, 'fill-mask': MraForMaskedLM, 'question-answering': MraForQuestionAnswering, 'text-classification': MraForSequenceClassification, 'token-classification': MraForTokenClassification, 'zero-shot': MraForSequenceClassification} if is_torch_available() else {}
...
class MraModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor([[[-0.014, 0.083, -0.0381], [0.1546, 0.1402, 0.022], [0.1162, 0.0851, 0.0165]]])
    ...
    def test_inference_masked_lm(self):
        ...
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[[9.2595, -3.6038, 11.8819], [9.3869, -3.2693, 11.0956], [11.8524, -3.4938, 13.121]]])
        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=0.0001))
    ...
    def test_inference_masked_lm_long_input(self):
        ...
        self.assertEqual(output.shape, expected_shape)
        expected_slice = torch.tensor([[[5.4789, -2.3564, 7.5064], [7.9067, -1.3369, 9.9668], [9.0712, -1.8106, 7.038]]])
        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=0.0001))