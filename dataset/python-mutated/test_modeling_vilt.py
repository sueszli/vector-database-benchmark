...
if is_torch_available():
    import torch
    from transformers import MODEL_MAPPING, ViltForImageAndTextRetrieval, ViltForImagesAndTextClassification, ViltForMaskedLM, ViltForQuestionAnswering, ViltForTokenClassification, ViltModel
...
class ViltModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (ViltModel, ViltForQuestionAnswering, ViltForImageAndTextRetrieval, ViltForMaskedLM, ViltForTokenClassification) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': ViltModel, 'visual-question-answering': ViltForQuestionAnswering} if is_torch_available() else {}
    test_pruning = False
