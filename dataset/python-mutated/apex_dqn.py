...
class ApexDQNConfig(DQNConfig):
    ...
    def __init__(self, algo_class=None):
        ...
        self.optimizer = merge_dicts(DQNConfig().optimizer, {'max_weight_sync_delay': 400, 'num_replay_buffer_shards': 4, 'debug': False})
        self.n_step = 3
        ...
        self.replay_buffer_config = {'no_local_replay_buffer': True, 'type': 'MultiAgentPrioritizedReplayBuffer', 'capacity': 2000000, 'prioritized_replay_alpha': 0.6, 'prioritized_replay_beta': 0.4, 'prioritized_replay_eps': 1e-06, 'replay_buffer_shards_colocated_with_driver': True, 'worker_side_prioritization': True, 'prioritized_replay': DEPRECATED_VALUE}
        self.num_rollout_workers = 32
        self.rollout_fragment_length = 50
        self.exploration_config = {'type': 'PerWorkerEpsilonGreedy'}
        self.num_gpus = 1
        self.min_time_s_per_iteration = 30
        self.min_sample_timesteps_per_iteration = 25000

    def training(self, *, max_requests_in_flight_per_replay_worker: Optional[int]=NotProvided, timeout_s_sampler_manager: Optional[float]=NotProvided, timeout_s_replay_manager: Optional[float]=NotProvided, **kwargs) -> 'ApexDQNConfig':
        if False:
            while True:
                i = 10
