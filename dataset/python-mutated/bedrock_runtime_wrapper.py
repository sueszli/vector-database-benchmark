...
class BedrockRuntimeWrapper:
    ...
    async def invoke_model_with_response_stream(self, prompt):
        ...
        try:
            enclosed_prompt = 'Human: ' + prompt + '\n\nAssistant:'
            body = {'prompt': enclosed_prompt, 'max_tokens_to_sample': 1024, 'temperature': 0.5, 'stop_sequences': ['\n\nHuman:']}
            response = self.bedrock_runtime_client.invoke_model_with_response_stream(modelId='anthropic.claude-v2', body=json.dumps(body))
