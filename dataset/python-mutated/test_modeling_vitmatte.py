...
class VitMatteModelTester:
    ...
    def __init__(self, parent, batch_size=13, image_size=32, patch_size=16, num_channels=4, is_training=True, use_labels=False, hidden_size=2, num_hidden_layers=2, num_attention_heads=2, hidden_act='gelu', type_sequence_label_size=10, initializer_range=0.02, scope=None, out_features=['stage1'], fusion_hidden_sizes=[128, 64, 32, 16]):
        if False:
            i = 10
            return i + 15
        self.parent = parent
    ...
    def get_backbone_config(self):
        if False:
            return 10
        return VitDetConfig(image_size=self.image_size, patch_size=self.patch_size, num_channels=self.num_channels, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, hidden_size=self.hidden_size, is_training=self.is_training, hidden_act=self.hidden_act, out_features=self.out_features)

    def get_config(self):
        if False:
            for i in range(10):
                print('nop')
        return VitMatteConfig(backbone_config=self.get_backbone_config(), hidden_size=self.hidden_size, fusion_hidden_sizes=self.fusion_hidden_sizes)
...
class VitMatteModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    def test_hidden_states_output(self):
        ...
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            self.assertListEqual(list(hidden_states[0].shape[-2:]), [2, 2])
...
class VitMatteModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        expected_slice = torch.tensor([[0.9977, 0.9987, 0.999], [0.998, 0.9998, 0.9998], [0.9983, 0.9998, 0.9998]], device=torch_device)
        self.assertTrue(torch.allclose(alphas[0, 0, :3, :3], expected_slice, atol=0.0001))