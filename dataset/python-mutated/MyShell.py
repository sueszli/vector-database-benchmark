...
class MyShell(AsyncGeneratorProvider):
    ...
    async def create_async_generator(cls, model: str, messages: Messages, proxy: str=None, timeout: int=120, **kwargs) -> AsyncResult:
        ...
        headers = {'User-Agent': user_agent, 'Myshell-Service-Name': 'organics-api', 'Visitor-Id': generate_visitor_id(user_agent)}
        async with StreamSession(impersonate='chrome107', proxies={'https': proxy}, timeout=timeout, headers=headers) as session:
            prompt = format_prompt(messages)
            data = {'botId': '1', 'conversation_scenario': 3, 'message': prompt, 'messageType': 1}
