...
from ...modeling_tf_utils import TFMaskedLanguageModelingLoss, TFModelInputType, TFPreTrainedModel, TFQuestionAnsweringLoss, TFSequenceClassificationLoss, TFTokenClassificationLoss, get_initializer, unpack_inputs
...
TF_DEBERTA_PRETRAINED_MODEL_ARCHIVE_LIST = ['kamalkraj/deberta-base']
...
class TFDebertaDisentangledSelfAttention(tf.keras.layers.Layer):
    ...
    def __init__(self, config: DebertaConfig, **kwargs):
        ...
        self.in_proj = tf.keras.layers.Dense(self.all_head_size * 3, kernel_initializer=get_initializer(config.initializer_range), name='in_proj', use_bias=False)
        self.pos_att_type = config.pos_att_type if config.pos_att_type is not None else []
        self.relative_attention = getattr(config, 'relative_attention', False)
        self.talking_head = getattr(config, 'talking_head', False)
        if self.talking_head:
            self.head_logits_proj = tf.keras.layers.Dense(self.num_attention_heads, kernel_initializer=get_initializer(config.initializer_range), name='head_logits_proj', use_bias=False)
            self.head_weights_proj = tf.keras.layers.Dense(self.num_attention_heads, kernel_initializer=get_initializer(config.initializer_range), name='head_weights_proj', use_bias=False)
        ...
        if self.relative_attention:
            ...
            if 'c2p' in self.pos_att_type:
                self.pos_proj = tf.keras.layers.Dense(self.all_head_size, kernel_initializer=get_initializer(config.initializer_range), name='pos_proj', use_bias=False)
            if 'p2c' in self.pos_att_type:
                self.pos_q_proj = tf.keras.layers.Dense(self.all_head_size, kernel_initializer=get_initializer(config.initializer_range), name='pos_q_proj')
    ...
    def build(self, input_shape):
        if False:
            i = 10
            return i + 15
        self.q_bias = self.add_weight(name='q_bias', shape=self.all_head_size, initializer=tf.keras.initializers.Zeros())
        self.v_bias = self.add_weight(name='v_bias', shape=self.all_head_size, initializer=tf.keras.initializers.Zeros())
        return super().build(input_shape)
