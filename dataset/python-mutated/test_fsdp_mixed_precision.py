...
from torch.distributed.fsdp import BackwardPrefetch, CPUOffload, FullyShardedDataParallel as FSDP, MixedPrecision, ShardingStrategy
...
from torch.testing._internal.common_fsdp import CUDAInitMode, FSDPInitMode, FSDPTest, subtest_name, TransformerWithSharedParams
...
if TEST_WITH_DEV_DBG_ASAN:
    ...
    sys.exit(0)
default_mp = MixedPrecision(param_dtype=torch.float16, buffer_dtype=torch.float16, reduce_dtype=torch.float16)
...
if nccl_supports_bf16:
    mp_diff_buffer_and_reduce = MixedPrecision(param_dtype=torch.float16, buffer_dtype=torch.bfloat16, reduce_dtype=torch.float32)
...
configs = list(product(mp_configs, cpu_offload_config, full_precision_param_dtype_config, enable_sharded_grad_scaler))
test_name_mapping = {str(CPUOffload(offload_params=True)): 'offload_true', str(CPUOffload(offload_params=False)): 'offload_false', str(default_mp): 'mp_fp16', str(mp_only_reduce): 'mp_only_reduce', str(mp_only_param_and_buf): 'mp_only_param_and_buf', str(mp_no_mixed_precision): 'mp_no_mp', str(torch.float32): 'fp32', str(torch.float64): 'fp64', 'enable_sharded_grad_scaler': 'enable_sharded_grad_scaler'}
if nccl_supports_bf16:
    test_name_mapping.update({str(mp_diff_buffer_and_reduce): 'mp_diff_buffer_reduce'})
