...
class TestPyCmaSampler:
    ...
    def test_init_cma_opts() -> None:
        if False:
            i = 10
            return i + 15
        sampler = optuna.integration.PyCmaSampler(x0={'x': 0, 'y': 0}, sigma0=0.1, cma_stds={'x': 1, 'y': 1}, seed=1, cma_opts={'popsize': 5})
        study = optuna.create_study(sampler=sampler)
        with patch('optuna.integration.cma._Optimizer') as mock_obj:
            mock_obj.ask.return_value = {'x': -1, 'y': -1}
            study.optimize(lambda t: t.suggest_int('x', -1, 1) + t.suggest_int('y', -1, 1), n_trials=2)
            assert mock_obj.mock_calls[0] == call({'x': IntDistribution(low=-1, high=1), 'y': IntDistribution(low=-1, high=1)}, {'x': 0, 'y': 0}, 0.1, {'x': 1, 'y': 1}, {'popsize': 5, 'seed': 1, 'verbose': -2})
    ...
    def test_warn_independent_sampling(capsys: _pytest.capture.CaptureFixture) -> None:
        ...
        sampler = optuna.integration.PyCmaSampler(warn_independent_sampling=True, n_startup_trials=0)
        study = optuna.create_study(sampler=sampler)
    ...
    def test_sample_relative_n_startup_trials() -> None:
        if False:
            while True:
                i = 10
        independent_sampler = optuna.samplers.RandomSampler()
        sampler = optuna.integration.PyCmaSampler(n_startup_trials=2, independent_sampler=independent_sampler)
        study = optuna.create_study(sampler=sampler)
        with patch.object(independent_sampler, 'sample_independent', wraps=independent_sampler.sample_independent) as mock_independent, patch.object(sampler, 'sample_relative', wraps=sampler.sample_relative) as mock_relative:
            ...
            assert mock_independent.call_count == 4
...
class TestOptimizer:
    ...
    def search_space() -> Dict[str, BaseDistribution]:
        if False:
            while True:
                i = 10
        return {'c': CategoricalDistribution(('a', 'b')), 'd': FloatDistribution(-1, 9, step=2), 'i': IntDistribution(-1, 1), 'ii': IntDistribution(-1, 3, step=2), 'il': IntDistribution(2, 16, log=True), 'l': FloatDistribution(0.001, 0.1, log=True), 'u': FloatDistribution(-2, 2)}
    ...
    def x0() -> Dict[str, Any]:
        if False:
            i = 10
            return i + 15
        return {'c': 'a', 'd': -1, 'i': -1, 'ii': -1, 'il': 2, 'l': 0.001, 'u': -2}
    ...
    def test_init(search_space: Dict[str, BaseDistribution], x0: Dict[str, Any]) -> None:
        ...
        with patch('cma.CMAEvolutionStrategy') as mock_obj:
            optuna.integration.cma._Optimizer(search_space, x0, 0.2, None, {'popsize': 5, 'seed': 1})
            assert mock_obj.mock_calls[0] == call([0, 0, -1, -1, math.log(2), math.log(0.001), -2], 0.2, {'BoundaryHandler': cma.BoundTransform, 'bounds': [[-0.5, -1.0, -1.5, -2.0, math.log(1.5), math.log(0.001), -2], [1.5, 11.0, 1.5, 4.0, math.log(16.5), math.log(0.1) - eps, 2 - eps]], 'popsize': 5, 'seed': 1})
    ...
    def test_init_with_unsupported_distribution() -> None:
        if False:
            while True:
                i = 10
        with pytest.raises(NotImplementedError):
            optuna.integration.cma._Optimizer({'x': UnsupportedDistribution()}, {'x': 0}, 0.2, None, {})

    @staticmethod
    @pytest.mark.parametrize('direction', [StudyDirection.MINIMIZE, StudyDirection.MAXIMIZE])
    def test_tell(search_space: Dict[str, BaseDistribution], x0: Dict[str, Any], direction: StudyDirection) -> None:
        if False:
            for i in range(10):
                print('nop')
        optimizer = optuna.integration.cma._Optimizer(search_space, x0, 0.2, None, {'popsize': 3, 'seed': 1})
    ...
    def test_tell_filter_by_state(search_space: Dict[str, BaseDistribution], x0: Dict[str, Any], state: TrialState) -> None:
        if False:
            for i in range(10):
                print('nop')
        optimizer = optuna.integration.cma._Optimizer(search_space, x0, 0.2, None, {'popsize': 2, 'seed': 1})
        trials = [_create_frozen_trial(x0, search_space)]
    ...
    @staticmethod
    def test_tell_filter_by_distribution(search_space: Dict[str, BaseDistribution], x0: Dict[str, Any]) -> None:
        if False:
            i = 10
            return i + 15
        optimizer = optuna.integration.cma._Optimizer(search_space, x0, 0.2, None, {'popsize': 2, 'seed': 1})
    ...
    def test_is_compatible(search_space: Dict[str, BaseDistribution], x0: Dict[str, Any]) -> None:
        ...
        assert optimizer._is_compatible(trial)
        trial = _create_frozen_trial(dict(x0, unknown=7), dict(search_space, unknown=FloatDistribution(0, 10)))
        assert optimizer._is_compatible(trial)
        param = dict(x0)
        del param['u']
        ...
        trial = _create_frozen_trial(dict(x0, u=20), dict(search_space, u=FloatDistribution(-100, 100)))
        assert not optimizer._is_compatible(trial)
        trial = _create_frozen_trial(x0, dict(search_space, u=IntDistribution(-2, 2)))
        with pytest.raises(ValueError):
            optimizer._is_compatible(trial)

def _create_frozen_trial(params: Dict[str, Any], param_distributions: Dict[str, BaseDistribution], state: TrialState=TrialState.COMPLETE, number: int=0) -> FrozenTrial:
    if False:
        while True:
            i = 10
    return FrozenTrial(number=number, value=1.0, state=state, user_attrs={}, system_attrs={}, params=params, distributions=param_distributions, intermediate_values={}, datetime_start=None, datetime_complete=None, trial_id=number)