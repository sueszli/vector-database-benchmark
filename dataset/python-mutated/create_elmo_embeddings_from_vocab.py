...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate CNN representations for a vocabulary using ELMo', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--vocab_path', type=str, help='A path to a vocabulary file to generate representations for.')
    parser.add_argument('--elmo_config', type=str, help='The path to a directory containing an ELMo config file.')
    parser.add_argument('--elmo_weights', type=str, help='The path to a directory containing an ELMo weight file.')
    parser.add_argument('--output_dir', type=str, help='The output directory to store the serialised embeddings.')
    ...
    parser.add_argument('--use_custom_oov_token', type=bool, default=False, help='AllenNLP requires a particular OOV token.To generate embeddings with a custom OOV token,add this flag.')
    args = parser.parse_args()
    main(args.vocab_path, args.elmo_config, args.elmo_weights, args.output_dir, args.batch_size, args.device, args.use_custom_oov_token)