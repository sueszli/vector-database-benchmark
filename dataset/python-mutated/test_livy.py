...
VALID_SESSION_ID_TEST_CASES = [pytest.param(BATCH_ID, id='integer'), pytest.param(str(BATCH_ID), id='integer as string')]
INVALID_SESSION_ID_TEST_CASES = [pytest.param(None, id='none'), pytest.param('forty two', id='invalid string'), pytest.param({'a': 'b'}, id='dictionary')]
...
class TestLivyAsyncHook:
    ...
    async def test_get_batch_state_running(self, mock_run_method):
        ...
        assert state == {'batch_state': BatchState.RUNNING, 'response': 'successfully fetched the batch state.', 'status': 'success'}
    ...
    async def test_dump_batch_logs_success(self, mock_get_batch_logs):
        ...
        mock_get_batch_logs.return_value = {'status': 'success', 'response': {'id': 1, 'log': ['mock_log_1', 'mock_log_2', 'mock_log_3']}}
        hook = LivyAsyncHook(livy_conn_id=LIVY_CONN_ID)
    ...
    async def test_dump_batch_logs_error(self, mock_get_batch_logs):
        ...
        mock_get_batch_logs.return_value = {'status': 'error', 'response': {'id': 1, 'log': ['mock_log_1', 'mock_log_2']}}
        hook = LivyAsyncHook(livy_conn_id=LIVY_CONN_ID)
        log_dump = await hook.dump_batch_logs(BATCH_ID)
    ...
    async def test_do_api_call_async_post_method_with_success(self, mock_get_connection, mock_session):
        ...
        mock_session.return_value.__aenter__.return_value.post.return_value.json = AsyncMock(return_value={'status': 'success'})
    ...
    async def test_do_api_call_async_get_method_with_success(self, mock_get_connection, mock_session):
        ...
        mock_session.return_value.__aenter__.return_value.get.return_value.json = AsyncMock(return_value={'status': 'success'})
        GET_RUN_ENDPOINT = 'api/jobs/runs/get'
        hook = LivyAsyncHook(livy_conn_id=LIVY_CONN_ID)
        hook.method = 'GET'
        hook.http_conn_id = mock_get_connection
    ...
    async def test_do_api_call_async_patch_method_with_success(self, mock_get_connection, mock_session):
        ...
        mock_session.return_value.__aenter__.return_value.patch.return_value.json = AsyncMock(return_value={'status': 'success'})
        GET_RUN_ENDPOINT = 'api/jobs/runs/get'
        hook = LivyAsyncHook(livy_conn_id=LIVY_CONN_ID)
        hook.method = 'PATCH'
        hook.http_conn_id = mock_get_connection
    ...
    async def test_do_api_call_async_with_client_response_error(self, mock_get_connection, mock_session):
        ...
        mock_session.return_value.__aexit__.return_value = mock_fun
        mock_session.return_value.__aenter__.return_value.patch = AsyncMock()
        mock_session.return_value.__aenter__.return_value.patch.return_value.json.side_effect = ClientResponseError(request_info=RequestInfo(url='example.com', method='PATCH', headers=multidict.CIMultiDict()), status=500, history=[])
        GET_RUN_ENDPOINT = ''
