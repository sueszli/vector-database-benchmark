...
if __name__ == '__main__':
    ...
    if is_regeneration_needed():
        ...
        if shutil.which('breeze') is None:
            ...
            console.print('[yellow]The `breeze` command is not on path. Skipping regeneration of images and consistency check.')
        ...
        res = subprocess.run(['breeze', 'setup', 'regenerate-command-images'], check=False)
        if res.returncode != 0:
            console.print('\n[red]Breeze command configuration has changed.\n')
