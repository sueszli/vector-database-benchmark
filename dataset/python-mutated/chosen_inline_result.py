...
class ChosenInlineResult(TelegramObject):
    ...
    **Note:** It is necessary to enable `inline feedback <https://core.telegram.org/bots/inline#collecting-feedback>`_ via `@BotFather <https://t.me/botfather>`_ in order to receive these objects in updates.
    ...
    if TYPE_CHECKING:
        ...
        def __init__(__pydantic__self__, *, result_id: str, from_user: User, query: str, location: Optional[Location]=None, inline_message_id: Optional[str]=None, **__pydantic_kwargs: Any) -> None:
            if False:
                for i in range(10):
                    print('nop')
            super().__init__(result_id=result_id, from_user=from_user, query=query, location=location, inline_message_id=inline_message_id, **__pydantic_kwargs)