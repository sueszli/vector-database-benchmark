...
class GraphQLView(BaseGraphQLView):
    ...
    async def get_root_value(self, request: web.Request) -> Query:
        await super().get_root_value(request)
        return Query()

    async def process_result(self, request: web.Request, result: ExecutionResult) -> GraphQLHTTPResponse:
...
class AioHttpClient(HttpClient):
    ...
    async def _graphql_request(self, method: Literal['get', 'post'], query: Optional[str]=None, variables: Optional[Dict[str, object]]=None, files: Optional[Dict[str, BytesIO]]=None, headers: Optional[Dict[str, str]]=None, **kwargs: Any) -> Response:
        async with TestClient(TestServer(self.app)) as client:
            ...
            response = await getattr(client, method)('/graphql', headers=self._get_headers(method=method, headers=headers, files=files), **kwargs)
            return Response(status_code=response.status, data=(await response.text()).encode(), headers=response.headers)
    ...
    async def post(self, url: str, data: Optional[bytes]=None, json: Optional[JSON]=None, headers: Optional[Dict[str, str]]=None) -> Response:
        async with TestClient(TestServer(self.app)) as client:
            response = await client.post('/graphql', headers=headers, data=data, json=json)
            return Response(status_code=response.status, data=(await response.text()).encode(), headers=response.headers)
