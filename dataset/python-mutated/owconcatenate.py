...
class OWConcatenate(widget.OWWidget):
    ...
    class Inputs:
        ...
        additional_data = MultiInput('Additional Data', Orange.data.Table, default=True)
    ...
    class Error(widget.OWWidget.Error):
        ...
        incompatible_domains = Msg('Ignoring column names requires matching column types')

    class Warning(widget.OWWidget.Warning):
        renamed_variables = Msg('Variables with duplicated names have been renamed.')
        unmergeable_attributes = Msg('Some variables may not be concatenated correctly due to attributes difference ({}).')
    merge_type: int
    ...
    domain_opts = ('all variables that appear in input tables', 'only variables that appear in all tables')
...
if __name__ == '__main__':
    WidgetPreview(OWConcatenate).run(insert_more_data=[(0, Orange.data.Table('iris')), (1, Orange.data.Table('zoo'))])