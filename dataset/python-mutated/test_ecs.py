...
from airflow.providers.amazon.aws.operators.ecs import DEFAULT_CONN_ID, EcsBaseOperator, EcsCreateClusterOperator, EcsDeleteClusterOperator, EcsDeregisterTaskDefinitionOperator, EcsRegisterTaskDefinitionOperator, EcsRunTaskOperator
...
TASK_DEFINITION_CONFIG = {'family': 'family_name', 'register_task_kwargs': {'cpu': '256', 'memory': '512', 'networkMode': 'awsvpc'}, 'container_definitions': [{'name': CONTAINER_NAME, 'image': 'ubuntu', 'workingDirectory': '/usr/bin', 'entryPoint': ['sh', '-c'], 'command': ['ls']}]}
RESPONSE_WITHOUT_FAILURES = {'failures': [], 'tasks': [{'containers': [{'containerArn': f'arn:aws:ecs:us-east-1:012345678910:container/{CONTAINER_NAME}', 'lastStatus': 'PENDING', 'name': 'wordpress', 'taskArn': f'arn:aws:ecs:us-east-1:012345678910:task/{TASK_ID}'}], 'desiredStatus': 'RUNNING', 'lastStatus': 'PENDING', 'taskArn': f'arn:aws:ecs:us-east-1:012345678910:task/{TASK_ID}', 'taskDefinitionArn': 'arn:aws:ecs:us-east-1:012345678910:task-definition/hello_world:11'}]}
WAITERS_TEST_CASES = [pytest.param(None, None, id='default-values'), pytest.param(3.14, None, id='set-delay-only'), pytest.param(None, 42, id='set-max-attempts-only'), pytest.param(2.71828, 9000, id='user-defined')]
