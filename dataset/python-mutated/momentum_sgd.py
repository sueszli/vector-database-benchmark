...
class MomentumSGDRule(optimizer.UpdateRule):
    ...
    def __init__(self, parent_hyperparam=None, lr=None, momentum=None):
        if False:
            i = 10
            return i + 15
        super(MomentumSGDRule, self).__init__(parent_hyperparam or _default_hyperparam)
        if lr is not None:
    ...
    def update_core_gpu(self, param):
        ...
        if MomentumSGDRule._kernel is None:
            MomentumSGDRule._kernel = cuda.elementwise('T grad, T lr, T momentum', 'T param, T v', 'v = momentum * v - lr * grad;\n                   param += v;', 'momentum_sgd')
        MomentumSGDRule._kernel(grad, self.hyperparam.lr, self.hyperparam.momentum, param.data, self.state['v'])
