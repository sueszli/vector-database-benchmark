...
if is_torch_available():
    import torch
    from transformers import RobertaForCausalLM, RobertaForMaskedLM, RobertaForMultipleChoice, RobertaForQuestionAnswering, RobertaForSequenceClassification, RobertaForTokenClassification, RobertaModel
    from transformers.models.roberta.modeling_roberta import ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST, RobertaEmbeddings, create_position_ids_from_input_ids
...
class RobertaModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (RobertaForCausalLM, RobertaForMaskedLM, RobertaModel, RobertaForSequenceClassification, RobertaForTokenClassification, RobertaForMultipleChoice, RobertaForQuestionAnswering) if is_torch_available() else ()
    all_generative_model_classes = (RobertaForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': RobertaModel, 'fill-mask': RobertaForMaskedLM, 'question-answering': RobertaForQuestionAnswering, 'text-classification': RobertaForSequenceClassification, 'text-generation': RobertaForCausalLM, 'token-classification': RobertaForTokenClassification, 'zero-shot': RobertaForSequenceClassification} if is_torch_available() else {}
    fx_compatible = True
...
class RobertaModelIntegrationTest(TestCasePlus):
    ...
    def test_inference_no_head(self):
        ...
        with torch.no_grad():
            output = model(input_ids)[0]
        expected_slice = torch.tensor([[[-0.0231, 0.0782, 0.0074], [-0.1854, 0.054, -0.0175], [0.0548, 0.0799, 0.1687]]])
        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=0.0001))
