...
def upgrade():
    ...
    op.alter_column('revision', 'id', type_=sa.UnicodeText, server_default=None)
    for table in foreign_tables:
        op.add_column(table, sa.Column('revision_id', sa.UnicodeText, sa.ForeignKey('revision.id')))
...
def downgrade():
    ...
    op.add_column('revision', sa.Column('id', sa.Integer, primary_key=True, nullable=False, unique=True))
    for table in foreign_tables:
        op.add_column(table, sa.Column('revision_id', sa.Integer, sa.ForeignKey('revision.id')))