...
async def test_radio_sets_toggle():
    ...
    async with RadioSetApp().run_test() as pilot:
        ...
        assert pilot.app.events_received == [('from_buttons', 0, [True, False, False]), ('from_strings', 2, [False, False, True])]
...
async def test_radioset_inner_navigation():
    ...
    async with RadioSetApp().run_test() as pilot:
        for (key, landing) in (('down', 1), ('up', 0), ('right', 1), ('left', 0), ('up', 2), ('down', 0)):
            await pilot.press(key, 'enter')
            assert pilot.app.query_one('#from_buttons', RadioSet).pressed_button == pilot.app.query_one('#from_buttons').children[landing]
    async with RadioSetApp().run_test() as pilot:
