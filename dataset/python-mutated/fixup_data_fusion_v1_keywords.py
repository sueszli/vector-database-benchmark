...
class data_fusionCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_instance': ('parent', 'instance_id', 'instance'), 'delete_instance': ('name',), 'get_instance': ('name',), 'list_available_versions': ('parent', 'page_size', 'page_token', 'latest_patch_only'), 'list_instances': ('parent', 'page_size', 'page_token', 'filter', 'order_by'), 'restart_instance': ('name',), 'update_instance': ('instance', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=data_fusionCallTransformer()):
    if False:
        while True:
            i = 10
