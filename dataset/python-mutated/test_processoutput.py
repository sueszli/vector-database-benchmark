...
async def test_onoutput(event_loop: asyncio.BaseEventLoop, processoutput: FakeProcessOutput, mock_process: Mock, returnvalue):
    ...
    assert not processoutput.onexit.called
    assert processoutput.onstdout.call_args_list == [call(0, 'foo'), call(1, 'bar'), call(2, 'baz'), call(3, 'OOF'), call(4, 'RAB')]
    assert processoutput.onstderr.call_args_list == [call(0, 'FOO'), call(1, 'BAR')]
    assert mock_process.kill.called
