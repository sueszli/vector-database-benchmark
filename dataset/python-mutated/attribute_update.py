...
class AttributeUpdateInput(BaseInputObjectType):
    ...
    remove_values = NonNullList(graphene.ID, name='removeValues', description='IDs of values to be removed from this attribute.')
    add_values = NonNullList(AttributeValueUpdateInput, name='addValues', description='New values to be created for this attribute.')
...
class AttributeUpdate(AttributeMixin, ModelWithExtRefMutation):
    ...
    class Arguments:
        ...
        external_reference = graphene.String(required=False, description=f'External ID of an attribute to update. {ADDED_IN_310}')
        input = AttributeUpdateInput(required=True, description='Fields required to update an attribute.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.ATTRIBUTE_UPDATED, description='An attribute was updated.')]
