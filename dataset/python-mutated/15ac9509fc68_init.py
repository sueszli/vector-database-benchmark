...
def upgrade() -> None:
    ...
    op.create_table('sources', sa.Column('id', sa.Integer(), nullable=False), sa.Column('filesystem_id', sa.String(length=96), nullable=True), sa.Column('journalist_designation', sa.String(length=255), nullable=False), sa.Column('flagged', sa.Boolean(), nullable=True), sa.Column('last_updated', sa.DateTime(), nullable=True), sa.Column('pending', sa.Boolean(), nullable=True), sa.Column('interaction_count', sa.Integer(), nullable=False), sa.PrimaryKeyConstraint('id'), sa.UniqueConstraint('filesystem_id'))
    op.create_table('journalist_login_attempt', sa.Column('id', sa.Integer(), nullable=False), sa.Column('timestamp', sa.DateTime(), nullable=True), sa.Column('journalist_id', sa.Integer(), nullable=True), sa.ForeignKeyConstraint(['journalist_id'], ['journalists.id']), sa.PrimaryKeyConstraint('id'))
    ...
    op.create_table('source_stars', sa.Column('id', sa.Integer(), nullable=False), sa.Column('source_id', sa.Integer(), nullable=True), sa.Column('starred', sa.Boolean(), nullable=True), sa.ForeignKeyConstraint(['source_id'], ['sources.id']), sa.PrimaryKeyConstraint('id'))
    op.create_table('submissions', sa.Column('id', sa.Integer(), nullable=False), sa.Column('source_id', sa.Integer(), nullable=True), sa.Column('filename', sa.String(length=255), nullable=False), sa.Column('size', sa.Integer(), nullable=False), sa.Column('downloaded', sa.Boolean(), nullable=True), sa.ForeignKeyConstraint(['source_id'], ['sources.id']), sa.PrimaryKeyConstraint('id'))
