...
if runtime.exists():
    ...
    st.number_input('number input 9 (on_change)', key='number_input_9', on_change=on_change)
    st.write('number input 9 (on_change) - value: ', st.session_state.number_input_9)
    st.write('number input 9 (on_change) - changed:', st.session_state.get('number_input_changed') is True)
[col1, col2, col3, col4, col5, col6] = st.columns(6)
with col1:
    v10 = st.number_input('number input 10 (small width)', max_value=10)
    st.write('number input 10 (small width) - value: ', v10)
v11 = st.number_input('number input 11 (value=None)', value=None, placeholder='Type a number...')
...
v12 = st.number_input('number input 12 (value from state & min=1)', value=None, min_value=1, key='number_input_12')
