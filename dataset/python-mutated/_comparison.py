...
import contextlib
import warnings
from typing import Any, Callable, Collection, Dict, List, NoReturn, Optional, Sequence, Tuple, Type, Union
import torch
try:
    import numpy as np
...
_DTYPE_PRECISIONS = {torch.float16: (0.001, 1e-05), torch.bfloat16: (0.016, 1e-05), torch.float32: (1.3e-06, 1e-05), torch.float64: (1e-07, 1e-07), torch.complex32: (0.001, 1e-05), torch.complex64: (1.3e-06, 1e-05), torch.complex128: (1e-07, 1e-07)}
_DTYPE_PRECISIONS.update({dtype: _DTYPE_PRECISIONS[torch.float32] for dtype in (torch.quint8, torch.quint2x4, torch.quint4x2, torch.qint8, torch.qint32)})
...
class NumberPair(Pair):
    ...
    +------------------+-------------------------------+
    """
    _TYPE_TO_DTYPE = {int: torch.int64, float: torch.float64, complex: torch.complex128}
