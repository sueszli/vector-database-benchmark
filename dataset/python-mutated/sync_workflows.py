...
async def sync_workflow_runs():
    ...
    if not GH_NUMBER_OF_WORKFLOW_RUNS_TO_FETCH or not GH_NUMBER_OF_WORKFLOW_RUNS_TO_FETCH.isdigit():
        raise ValueError('The number of workflow runs to fetch is not specified or not an integer')
...
async def fetch_workflow_data():
    ...
    headers = {'Authorization': get_token()}
    page = 1
    number_of_entries_per_page = 100
    ...
    for completed_task in asyncio.as_completed(workflow_tasks):
        ...
        for workflow in workflows:
            ...
            page = 1
            pages_to_fetch = math.ceil(int(GH_NUMBER_OF_WORKFLOW_RUNS_TO_FETCH) / number_of_entries_per_page)
            while pages_to_fetch >= page:
                params = {'branch': 'master', 'page': page, 'per_page': number_of_entries_per_page, 'exclude_pull_requests': 'true'}
    ...
    for completed_task in asyncio.as_completed(workflow_run_tasks):
        ...
        if workflow_runs:
            ...
            if not workflow:
                ...
                result = re.search('(workflows\\/.*)$', workflow_path)
                if result:
    ...
    page = math.ceil(int(GH_NUMBER_OF_WORKFLOW_RUNS_TO_FETCH) / number_of_entries_per_page) + 1
    while workflow_ids_to_fetch_extra_runs:
        ...
        for completed_task in asyncio.as_completed(extra_workflow_runs_tasks):
            ...
            if workflow_runs:
                ...
                number_of_runs_to_add = int(GH_NUMBER_OF_WORKFLOW_RUNS_TO_FETCH) - len(workflow.runs)
