...
class GrapheneSensorDryRunMutation(graphene.Mutation):
    ...
    @capture_error
    def mutate(self, graphene_info: 'ResolveInfo', selector_data: Mapping[str, Any], cursor: Optional[str]):
        if False:
            return 10
        return GrapheneDryRunInstigationTick(SensorSelector.from_graphql_input(selector_data), timestamp=None, cursor=cursor)