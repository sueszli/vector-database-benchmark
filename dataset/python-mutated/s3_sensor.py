...
class S3KeySensorDecorator(AirflowSensorDecorator):
    ...
    defaults = dict(**AirflowSensorDecorator.defaults, bucket_key=None, bucket_name=None, wildcard_match=False, aws_conn_id=None, verify=None)

    def validate(self):
        if False:
            print('Hello World!')
        if self.attributes['bucket_key'] is None:
            raise AirflowException('`bucket_key` for `@%s`cannot be empty.' % self.name)
        super().validate()