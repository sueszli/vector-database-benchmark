...
class TestFullRefresh(BaseTest):
    ...
    async def test_sequential_reads(self, connector_config: SecretDict, configured_catalog: ConfiguredAirbyteCatalog, ignored_fields: Optional[Mapping[str, List[IgnoredFieldsConfiguration]]], docker_runner: ConnectorRunner, detailed_logger: Logger):
        configured_catalog = full_refresh_only_catalog(configured_catalog)
        output_1 = await docker_runner.call_read(connector_config, configured_catalog, enable_caching=False)
