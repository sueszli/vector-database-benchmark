...
from airflow.providers.google.cloud.operators.dataflow import CheckJobRunning, DataflowCreateJavaJobOperator, DataflowCreatePythonJobOperator, DataflowStartFlexTemplateOperator, DataflowStartSqlJobOperator, DataflowStopJobOperator, DataflowTemplatedJobStartOperator
...
PARAMETERS = {'inputFile': 'gs://dataflow-samples/shakespeare/kinglear.txt', 'output': 'gs://test/output/my_output'}
...
PY_OPTIONS = ['-m']
DEFAULT_OPTIONS_PYTHON = DEFAULT_OPTIONS_JAVA = {'project': 'test', 'stagingLocation': 'gs://test/staging'}
DEFAULT_OPTIONS_TEMPLATE = {'project': 'test', 'stagingLocation': 'gs://test/staging', 'tempLocation': 'gs://test/temp', 'zone': 'us-central1-f'}
ADDITIONAL_OPTIONS = {'output': 'gs://test/output', 'labels': {'foo': 'bar'}}
TEST_VERSION = f"v{version.replace('.', '-').replace('+', '-')}"
EXPECTED_ADDITIONAL_OPTIONS = {'output': 'gs://test/output', 'labels': {'foo': 'bar', 'airflow-version': TEST_VERSION}}
POLL_SLEEP = 30
GCS_HOOK_STRING = 'airflow.providers.google.cloud.operators.dataflow.{}'
TEST_FLEX_PARAMETERS = {'containerSpecGcsPath': 'gs://test-bucket/test-file', 'jobName': 'test-job-name', 'parameters': {'inputSubscription': 'test-subscription', 'outputTable': 'test-project:test-dataset.streaming_beam_sql'}}
TEST_LOCATION = 'custom-location'
TEST_REGION = 'custom-region'
TEST_PROJECT = 'test-project'
TEST_SQL_JOB_NAME = 'test-sql-job-name'
TEST_DATASET = 'test-dataset'
TEST_SQL_OPTIONS = {'bigquery-project': TEST_PROJECT, 'bigquery-dataset': TEST_DATASET, 'bigquery-table': 'beam_output', 'bigquery-write-disposition': 'write-truncate'}
