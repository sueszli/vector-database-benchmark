...
class NewUserConsentResource(DirectServeHtmlResource):
    ...
    async def _async_render_GET(self, request: Request) -> None:
        ...
        if not session.chosen_localpart:
            ...
            self._sso_handler.render_error(request, 'no_user', 'No user name has been selected.', code=400)
            return
        user_id = UserID(session.chosen_localpart, self._server_name)
        user_profile = {'display_name': session.display_name}
        template_params = {'user_id': user_id.to_string(), 'user_profile': user_profile, 'consent_version': self._consent_version, 'terms_url': '/_matrix/consent?v=%s' % (self._consent_version,)}
    ...
    async def _async_render_POST(self, request: SynapseRequest) -> None:
        ...
        try:
            ...
            return
        await self._sso_handler.handle_terms_accepted(request, session_id, accepted_version)