...
CURSOR_DESCRIPTION = (('some_str', oracledb.DB_TYPE_VARCHAR, None, None, None, None, None), ('some_num', oracledb.DB_TYPE_NUMBER, None, None, None, None, None))
NDJSON_LINES = [b'{"some_num": 42, "some_str": "mock_row_content_1"}\n', b'{"some_num": 43, "some_str": "mock_row_content_2"}\n', b'{"some_num": 44, "some_str": "mock_row_content_3"}\n']
SCHEMA_FILENAME = 'schema_test.json'
SCHEMA_JSON = [b'[{"mode": "NULLABLE", "name": "some_str", "type": "STRING"}, ', b'{"mode": "NULLABLE", "name": "some_num", "type": "NUMERIC"}]']
...
class TestOracleToGoogleCloudStorageOperator:
    ...
    def test_exec_success_json(self, gcs_hook_mock_class, oracle_hook_mock_class):
        ...
        op = OracleToGCSOperator(task_id=TASK_ID, oracle_conn_id=ORACLE_CONN_ID, sql=SQL, bucket=BUCKET, filename=JSON_FILENAME)
    ...
    def test_file_splitting(self, gcs_hook_mock_class, oracle_hook_mock_class):
        ...
        gcs_hook_mock.upload.side_effect = _assert_upload
        op = OracleToGCSOperator(task_id=TASK_ID, sql=SQL, bucket=BUCKET, filename=JSON_FILENAME, approx_max_file_size_bytes=len(expected_upload[JSON_FILENAME.format(0)]))
        op.execute(None)
    ...
    def test_schema_file(self, gcs_hook_mock_class, oracle_hook_mock_class):
        ...
        gcs_hook_mock.upload.side_effect = _assert_upload
        op = OracleToGCSOperator(task_id=TASK_ID, sql=SQL, bucket=BUCKET, filename=JSON_FILENAME, schema_filename=SCHEMA_FILENAME)
        op.execute(None)
