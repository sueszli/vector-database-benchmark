...
from posthog.temporal.workflows.batch_exports import CreateBatchExportRunInputs, UpdateBatchExportRunStatusInputs, create_export_run, update_export_run_status
...
async def test_create_export_run(activity_environment, team, batch_export):
    ...
    inputs = CreateBatchExportRunInputs(team_id=team.id, batch_export_id=str(batch_export.id), data_interval_start=start.isoformat(), data_interval_end=end.isoformat())
...
async def test_update_export_run_status(activity_environment, team, batch_export):
    ...
    update_inputs = UpdateBatchExportRunStatusInputs(id=str(run_id), status='Completed', team_id=inputs.team_id)
