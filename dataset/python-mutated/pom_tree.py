...
dep = regex('([^:\n]+:[^:\n]+):[^:\n]+:[^:\n]+:([^:\n]+):[^:\n]+', flags=0, group=(1, 2)) | regex('([^:\n]+:[^:\n]+):[^:\n]+:([^:\n]+):[^:\n]+', flags=0, group=(1, 2))
tree_line = mark_line(regex('((\\|  )|(   ))*').bind(lambda depth: regex('(\\+- )|(\\\\- )') >> dep.map(lambda d: {'line_number': 0, 'depth': len(depth) // 3, 'transitivity': Transitivity(Transitive() if len(depth) // 3 > 0 else Direct()), 'children': [], 'package': d[0], 'version': d[1]}) | consume_line))
