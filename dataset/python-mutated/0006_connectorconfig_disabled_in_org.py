...
def backwards_migrate_disabled_config(apps, schema_editor):
    ...
    for config in ConnectorConfig.objects.all():
        for org in config.disabled_in_organizations.all():
            OrganizationPluginState.objects.create(plugin_name=config.name, type='2', organization=org, disabled=True)

class Migration(migrations.Migration):
    dependencies = [('certego_saas_organization', '0001_initial'), ('connectors_manager', '0005_auto_20230301_1415')]
    operations = [migrations.AddField(model_name='connectorconfig', name='disabled_in_organizations', field=models.ManyToManyField(blank=True, related_name='connectors_manager_connectorconfig_disabled', to='certego_saas_organization.Organization')), migrations.RunPython(migrate_disabled_config, backwards_migrate_disabled_config)]