...
from homeassistant.const import ATTR_ENTITY_ID, ATTR_NAME, CONF_FRIENDLY_NAME, CONF_IP_ADDRESS, CONF_PASSWORD, CONF_PORT, CONF_USERNAME, STATE_UNKNOWN
...
MOCK_FACE = {'confidence': 0.5812028911604818, 'id': 'john.jpg', 'matched': True, 'name': 'John Lennon', 'rect': {'height': 75, 'left': 63, 'top': 262, 'width': 74}}
MOCK_FILE_PATH = '/images/mock.jpg'
MOCK_HEALTH = {'success': True, 'hostname': 'b893cc4f7fd6', 'metadata': {'boxname': 'facebox', 'build': 'development'}, 'errors': []}
MOCK_JSON = {'facesCount': 1, 'success': True, 'faces': [MOCK_FACE]}
MOCK_NAME = 'mock_name'
MOCK_USERNAME = 'mock_username'
MOCK_PASSWORD = 'mock_password'
PARSED_FACES = [{fb.FACEBOX_NAME: 'John Lennon', fb.ATTR_IMAGE_ID: 'john.jpg', fb.ATTR_CONFIDENCE: 58.12, fb.ATTR_MATCHED: True, fb.ATTR_BOUNDING_BOX: {'height': 75, 'left': 63, 'top': 262, 'width': 74}}]
MATCHED_FACES = {'John Lennon': 58.12}
VALID_ENTITY_ID = 'image_processing.facebox_demo_camera'
VALID_CONFIG = {ip.DOMAIN: {'platform': 'facebox', CONF_IP_ADDRESS: MOCK_IP, CONF_PORT: MOCK_PORT, ip.CONF_SOURCE: {ip.CONF_ENTITY_ID: 'camera.demo_camera'}}, 'camera': {'platform': 'demo'}}
...
async def test_process_image(hass: HomeAssistant, mock_healthybox, mock_image) -> None:
    ...
    PARSED_FACES[0][ATTR_ENTITY_ID] = VALID_ENTITY_ID
    ...
    assert face_events[0].data[fb.ATTR_CONFIDENCE] == PARSED_FACES[0][fb.ATTR_CONFIDENCE]
    ...
    assert face_events[0].data[fb.ATTR_BOUNDING_BOX] == PARSED_FACES[0][fb.ATTR_BOUNDING_BOX]

async def test_process_image_errors(hass: HomeAssistant, mock_healthybox, mock_image, caplog: pytest.LogCaptureFixture) -> None:
...
async def test_teach_service(hass: HomeAssistant, mock_healthybox, mock_image, mock_isfile, mock_open_file, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with requests_mock.Mocker() as mock_req:
        ...
        data = {ATTR_ENTITY_ID: VALID_ENTITY_ID, ATTR_NAME: MOCK_NAME, fb.FILE_PATH: MOCK_FILE_PATH}
        await hass.services.async_call(fb.DOMAIN, fb.SERVICE_TEACH_FACE, service_data=data)
        await hass.async_block_till_done()
    with requests_mock.Mocker() as mock_req:
        url = f'http://{MOCK_IP}:{MOCK_PORT}/facebox/teach'
