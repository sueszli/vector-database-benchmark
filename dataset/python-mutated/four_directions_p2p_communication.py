...
def initialize_p2p_groups(hcg, use_cache=True, enable_partial_send_recv=True, enable_timer=False):
    if False:
        print('Hello World!')
    ...
    debug_str = 'P2pInfo: send_next_group: {}, send_prev_group: {}, recv_next_group: {}, recv_prev_group: {}'.format(repr(send_next_group), repr(send_prev_group), repr(recv_next_group), repr(recv_prev_group))
...
def _partial_send_op(tensor, group, use_calc_stream, ring_id, dst, nranks, rank_id):
    if False:
        i = 10
        return i + 15
    ...
    if framework.in_dynamic_mode():
        ...
        comm_op = group.process_group.send_partial_on_calc_stream if use_calc_stream else group.process_group.send_partial
...
def send_partial(tensor, dst=0, nranks=1, rank_id=0, group=None, use_calc_stream=True):
    ...
    dst_rank = _hcg._get_p2p_next_rank() if dst == 1 else _hcg._get_p2p_prev_rank()
    if _is_valid_send_recv_partial(tensor, nranks):
        return _partial_send_op(tensor, group, use_calc_stream, ring_id, dst_rank, nranks, rank_id)
    else:
        send_op = paddle.distributed.isend
        return send_op(tensor.detach(), dst=dst_rank, group=group)

def _partial_recv_op(tensor, group, use_calc_stream, ring_id, src, nranks, rank_id):
    if False:
        i = 10
        return i + 15
    src_rank_in_group = src if group is None else group.get_group_rank(src)
    group = paddle.distributed.collective._get_default_group() if group is None else group
    comm_op = group.process_group.recv_partial_on_calc_stream if use_calc_stream else group.process_group.recv_partial
...
def recv_partial(tensor, src=0, nranks=1, rank_id=0, group=None, use_calc_stream=True):
    ...
    if group is not None and (not group.is_member()):
        return
    ring_id = 0 if group is None else group.id
    src_rank = _hcg._get_p2p_prev_rank() if src == 0 else _hcg._get_p2p_next_rank()
    if _is_valid_send_recv_partial(tensor, nranks):
        return _partial_recv_op(tensor, group, use_calc_stream, ring_id, src_rank, nranks, rank_id)
    else:
        if use_calc_stream:
...
def _p2p_helper(tensor_send_next, tensor_send_prev, recv_prev, recv_next, sync_recv=True):
    if False:
        while True:
            i = 10
    global _hcg
        ...
        if tensor_recv_prev is not None:
            if isinstance(tensor_recv_prev, tuple):
                for d in tensor_recv_prev:
                    task = recv_partial(d, src=0, nranks=mp_degree, rank_id=mp_rank, group=_hcg.recv_prev_group, use_calc_stream=sync_recv)
                    if sync_recv:
                        allgather_partial(d, nranks=mp_degree, rank_id=mp_rank, group=mp_group, use_calc_stream=True)
                    else:
                        tasks.append(task)
            else:
                task = recv_partial(tensor_recv_prev, src=0, nranks=mp_degree, rank_id=mp_rank, group=_hcg.recv_prev_group, use_calc_stream=sync_recv)
                if sync_recv:
                    allgather_partial(tensor_recv_prev, nranks=mp_degree, rank_id=mp_rank, group=mp_group, use_calc_stream=True)
                else:
                    tasks.append(task)
        if tensor_send_next is not None:
            if isinstance(tensor_send_next, tuple):
                for d in tensor_send_next:
                    ...
                    send_partial(d, dst=1, nranks=mp_degree, rank_id=mp_rank, group=_hcg.send_next_group, use_calc_stream=False)
            else:
                paddle.distributed.wait(tensor_send_next, use_calc_stream=True)
                send_partial(tensor_send_next, dst=1, nranks=mp_degree, rank_id=mp_rank, group=_hcg.send_next_group, use_calc_stream=False)
        if tensor_recv_next is not None:
            if isinstance(tensor_recv_next, tuple):
                for d in tensor_recv_next:
                    task = recv_partial(d, src=1, nranks=mp_degree, rank_id=mp_rank, group=_hcg.recv_next_group, use_calc_stream=sync_recv)
                    if sync_recv:
                        allgather_partial(d, nranks=mp_degree, rank_id=mp_rank, group=mp_group, use_calc_stream=True)
                    else:
                        tasks.append(task)
            else:
                task = recv_partial(tensor_recv_next, src=1, nranks=mp_degree, rank_id=mp_rank, group=_hcg.recv_next_group, use_calc_stream=sync_recv)
                if sync_recv:
                    allgather_partial(tensor_recv_next, nranks=mp_degree, rank_id=mp_rank, group=mp_group, use_calc_stream=True)
                else:
                    tasks.append(task)
        if tensor_send_prev is not None:
            if isinstance(tensor_send_prev, tuple):
                for d in tensor_send_prev:
                    paddle.distributed.wait(d, use_calc_stream=True)
                    send_partial(d, dst=0, nranks=mp_degree, rank_id=mp_rank, group=_hcg.send_prev_group, use_calc_stream=False)
            else:
                paddle.distributed.wait(tensor_send_prev, use_calc_stream=True)
                send_partial(tensor_send_prev, dst=0, nranks=mp_degree, rank_id=mp_rank, group=_hcg.send_prev_group, use_calc_stream=False)
    else:
...
def recv_forward(pp_first_stage, sync_recv=True):
    ...
    if pp_first_stage:
        ...
        (input_tensor, _) = _p2p_helper(tensor_send_next=None, tensor_send_prev=None, recv_prev=True, recv_next=False, sync_recv=sync_recv)
    if _timers is not None:
...
def recv_backward(pp_last_stage, sync_recv=True):
    ...
    if pp_last_stage:
        ...
        (_, output_tensor_grad) = _p2p_helper(tensor_send_next=None, tensor_send_prev=None, recv_prev=False, recv_next=True, sync_recv=sync_recv)
    if _timers is not None:
...
def send_forward(output_tensor, pp_last_stage):
    ...
    if not pp_last_stage:
        ...
        _p2p_helper(tensor_send_next=output_tensor, tensor_send_prev=None, recv_prev=False, recv_next=False)
    if _timers is not None:
...
def send_backward(input_tensor_grad, pp_first_stage):
    ...
    if not pp_first_stage:
        _p2p_helper(tensor_send_next=None, tensor_send_prev=input_tensor_grad, recv_prev=False, recv_next=False)
    if _timers is not None:
...
def send_forward_recv_backward(output_tensor, pp_last_stage):
    ...
    if pp_last_stage:
        ...
        (_, output_tensor_grad) = _p2p_helper(tensor_send_next=output_tensor, tensor_send_prev=None, recv_prev=False, recv_next=True)
    if _timers is not None:
...
def send_backward_recv_forward(input_tensor_grad, pp_first_stage):
    ...
    if pp_first_stage:
        ...
        (input_tensor, _) = _p2p_helper(tensor_send_next=None, tensor_send_prev=input_tensor_grad, recv_prev=True, recv_next=False)
    if _timers is not None:
        _timers('send_backward_recv_forward').stop()
    return input_tensor

def send_forward_backward_recv_forward_backward(output_tensor, input_tensor_grad, recv_prev, recv_next):
    if False:
        while True:
            i = 10
    global _timers
    ...
    if recv_prev and (not _send_recv_meta.has_recv_meta):
        _send_recv_meta.recv_meta(_hcg.recv_prev_group)
        _send_recv_meta.has_recv_meta = _use_cache
    (input_tensor, output_tensor_grad) = _p2p_helper(tensor_send_next=output_tensor, tensor_send_prev=input_tensor_grad, recv_prev=recv_prev, recv_next=recv_next, sync_recv=False)
    if _timers is not None:
...
def send_forward_recv_forward(output_tensor, recv_prev):
    ...
    if recv_prev and (not _send_recv_meta.has_recv_meta):
        _send_recv_meta.recv_meta(_hcg.recv_prev_group)
        _send_recv_meta.has_recv_meta = _use_cache
    (input_tensor, _) = _p2p_helper(tensor_send_next=output_tensor, tensor_send_prev=None, recv_prev=recv_prev, recv_next=False, sync_recv=False)
    if _timers is not None:
...
def send_backward_recv_backward(input_tensor_grad, recv_next):
    ...
    (_, output_tensor_grad) = _p2p_helper(tensor_send_next=None, tensor_send_prev=input_tensor_grad, recv_prev=False, recv_next=recv_next, sync_recv=False)
    if _timers is not None:
