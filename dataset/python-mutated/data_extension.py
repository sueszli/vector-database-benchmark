...
class DataExtension(object):
    ...
    async def _md_recv(self, pack):
        ...
        for d in pack.get('data', []):
            ...
            if d.get('quotes', None):
                _simple_merge_diff(result=self._data['quotes'], diff=d['quotes'])
            if d.get('trade', None):
                _simple_merge_diff_and_collect_paths(result=self._data['trade'], diff=d['trade'], path=('trade',), diff_paths=self._diffs_paths, prototype=self._prototype['trade'])
    ...
    async def _send_diff(self):
        if self._datetime_state.data_ready and self._pending_peek and self._diffs:
            ...
            rtn_data = {'aid': 'rtn_data', 'data': self._diffs + [ext_diff]}
            self._diffs = []
            self._pending_peek = False
