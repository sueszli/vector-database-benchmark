...
def upgrade():
    if False:
        i = 10
        return i + 15
    op.create_table('events', sa.Column('uuid', postgresql.UUID(), nullable=False), sa.Column('type', sa.String(length=50), nullable=True), sa.Column('timestamp', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False), sa.ForeignKeyConstraint(['type'], ['event_types.name'], name=op.f('fk_events_type_event_types'), ondelete='CASCADE'), sa.PrimaryKeyConstraint('uuid', name=op.f('pk_events')))
