...
args.filename.close()
skip = {'fib_L261', 'check-memory-zero_L87', 'check-memory-zero_L89', 'check-memory-zero_L91', 'check-memory-zero_L93', 'check-memory-zero_L95', 'check-memory-zero_L97', 'odd_L269', 'fib-i64_L527'}
    ...
    if d['type'] == 'action':
        if d['action']['type'] == 'invoke':
            if isinstance(current_module, int):
                modules[current_module].add_test(d['action']['field'], d['line'], convert_args(d['action']['args']), convert_rets(d['expected']), 'action')
        else:
        ...
        if d['action']['type'] == 'invoke':
            if isinstance(current_module, int):
                modules[current_module].add_test(d['action']['field'], d['line'], convert_args(d['action']['args']), convert_rets(d['expected']))
        else:
        ...
        if d['action']['type'] == 'invoke':
            if isinstance(current_module, int):
                modules[current_module].add_test(d['action']['field'], d['line'], convert_args(d['action']['args']), convert_rets(d['expected']), 'assert_trap')
        else:
