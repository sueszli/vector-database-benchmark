...
from homeassistant.components import climate, cover, emulated_hue, fan, http, humidifier, light, media_player, script
from homeassistant.components.emulated_hue import Config, hue_api
from homeassistant.components.emulated_hue.hue_api import HUE_API_STATE_BRI, HUE_API_STATE_BRI_MAX, HUE_API_STATE_CT, HUE_API_STATE_HUE, HUE_API_STATE_ON, HUE_API_STATE_SAT, HUE_API_STATE_TRANSITION, HUE_API_STATE_XY, HUE_API_USERNAME, HueAllGroupsStateView, HueAllLightsStateView, HueConfigView, HueFullStateView, HueGroupView, HueOneLightChangeView, HueOneLightStateView, HueUsernameView, _remote_is_allowed
from homeassistant.const import ATTR_ENTITY_ID, ATTR_SUPPORTED_FEATURES, CONTENT_TYPE_JSON, SERVICE_TURN_OFF, SERVICE_TURN_ON, STATE_OFF, STATE_ON
from homeassistant.core import HomeAssistant, callback
from homeassistant.helpers.typing import ConfigType
import homeassistant.util.dt as dt_util
from tests.common import async_fire_time_changed, async_mock_service, get_test_instance_port
...
ENTITY_IDS_BY_NUMBER = {'1': 'light.ceiling_lights', '2': 'light.bed_light', '3': 'script.set_kitchen_light', '4': 'light.kitchen_lights', '5': 'media_player.living_room', '6': 'media_player.bedroom', '7': 'media_player.walkman', '8': 'media_player.lounge_room', '9': 'fan.living_room_fan', '10': 'fan.ceiling_fan', '11': 'fan.percentage_full_fan', '12': 'fan.percentage_limited_fan', '13': 'fan.preset_only_limited_fan', '14': 'cover.living_room_window', '15': 'climate.hvac', '16': 'climate.heatpump', '17': 'climate.ecobee', '18': 'light.no_brightness', '19': 'humidifier.humidifier', '20': 'humidifier.dehumidifier', '21': 'humidifier.hygrostat', '22': 'scene.light_on', '23': 'scene.light_off', '24': 'media_player.kitchen', '25': 'light.office_rgbw_lights', '26': 'light.living_room_rgbww_lights'}
...
async def _async_setup_emulated_hue(hass: HomeAssistant, conf: ConfigType) -> None:
    ...
    with patch_upnp():
        await setup.async_setup_component(hass, emulated_hue.DOMAIN, {emulated_hue.DOMAIN: conf})
        await hass.async_block_till_done()

@pytest.fixture
async def base_setup(hass):
    """Set up homeassistant and http."""
    await asyncio.gather(setup.async_setup_component(hass, 'homeassistant', {}), setup.async_setup_component(hass, http.DOMAIN, {http.DOMAIN: {http.CONF_SERVER_PORT: HTTP_SERVER_PORT}}))

@pytest.fixture(autouse=True)
async def wanted_platforms_only() -> None:
    """Enable only the wanted demo platforms."""
    with patch('homeassistant.components.demo.COMPONENTS_WITH_CONFIG_ENTRY_DEMO_PLATFORM', [const.Platform.CLIMATE, const.Platform.COVER, const.Platform.FAN, const.Platform.HUMIDIFIER, const.Platform.LIGHT, const.Platform.MEDIA_PLAYER]):
        yield

@pytest.fixture
async def demo_setup(hass, wanted_platforms_only):
    """Fixture to setup demo platforms."""
    setups = [setup.async_setup_component(hass, 'homeassistant', {}), setup.async_setup_component(hass, http.DOMAIN, {http.DOMAIN: {http.CONF_SERVER_PORT: HTTP_SERVER_PORT}}), setup.async_setup_component(hass, 'demo', {}), setup.async_setup_component(hass, script.DOMAIN, {'script': {'set_kitchen_light': {'sequence': [{'service_template': 'light.turn_{{ requested_state }}', 'data_template': {'entity_id': 'light.kitchen_lights', 'brightness': '{{ requested_level }}'}}]}}}), setup.async_setup_component(hass, 'scene', {'scene': [{'id': 'light_on', 'name': 'Light on', 'entities': {'light.kitchen_lights': {'state': 'on'}}}, {'id': 'light_off', 'name': 'Light off', 'entities': {'light.kitchen_lights': {'state': 'off'}}}]})]
    await asyncio.gather(*setups)

@pytest.fixture
async def hass_hue(hass, base_setup, demo_setup):
    """Set up a Home Assistant instance for these tests."""
    await _async_setup_emulated_hue(hass, {emulated_hue.CONF_LISTEN_PORT: BRIDGE_SERVER_PORT, emulated_hue.CONF_EXPOSE_BY_DEFAULT: True})
...
async def hue_client(hass_hue, hass_client_no_auth):
    ...
    _mock_hue_endpoints(hass_hue, {emulated_hue.CONF_ENTITIES: {'light.bed_light': {emulated_hue.CONF_ENTITY_HIDDEN: True}, 'light.kitchen_lights': {emulated_hue.CONF_ENTITY_HIDDEN: True}, 'light.entrance_color_white_lights': {emulated_hue.CONF_ENTITY_HIDDEN: True}, 'fan.ceiling_fan': {emulated_hue.CONF_ENTITY_HIDDEN: True}, 'script.set_kitchen_light': {emulated_hue.CONF_ENTITY_HIDDEN: False}, 'cover.living_room_window': {emulated_hue.CONF_ENTITY_HIDDEN: False}, 'climate.hvac': {emulated_hue.CONF_ENTITY_HIDDEN: False}, 'climate.heatpump': {emulated_hue.CONF_ENTITY_HIDDEN: False}, 'humidifier.humidifier': {emulated_hue.CONF_ENTITY_HIDDEN: False}, 'humidifier.dehumidifier': {emulated_hue.CONF_ENTITY_HIDDEN: False}, 'climate.nosetting': {}, 'scene.light_on': {emulated_hue.CONF_ENTITY_HIDDEN: False}, 'scene.light_off': {emulated_hue.CONF_ENTITY_HIDDEN: False}}}, ENTITY_IDS_BY_NUMBER)
...
async def test_discover_lights(hass: HomeAssistant, hue_client) -> None:
    ...
    assert '00:2f:d2:31:ce:c5:55:cc-ee' in devices
    assert '00:b6:14:77:34:b7:bb:06-e8' not in devices
    assert '00:95:b7:51:16:58:6c:c0-c5' in devices
    assert '00:64:7b:e4:96:c3:fe:90-c3' not in devices
    assert '00:7e:8a:42:35:66:db:86-c5' in devices
    assert '00:05:44:c2:d6:0a:e5:17-b7' in devices
    assert '00:f3:5f:fa:31:f3:32:21-a8' in devices
    assert '00:b4:06:2e:91:95:23:97-fb' in devices
    assert '00:b2:bd:f9:2c:ad:22:ae-58' in devices
    assert '00:77:4c:8a:23:7d:27:4b-7f' not in devices
    assert '00:02:53:b9:d5:1a:b3:67-b2' in devices
    assert '00:42:03:fe:97:58:2d:b1-50' in devices
    assert '00:7b:2a:c7:08:d6:66:bf-80' in devices
    assert '00:57:77:a1:6a:8e:ef:b3-6c' not in devices
    assert '00:18:7c:7e:78:0e:cd:86-ae' in devices
    assert '00:78:eb:f8:d5:0c:14:85-e7' in devices
    assert '00:67:19:bd:ea:e4:2d:ef-22' in devices
    assert '00:61:bf:ab:08:b1:a6:18-43' not in devices
    assert '00:62:5c:3e:df:58:40:01-43' in devices
    assert '00:1c:72:08:ed:09:e7:89-77' in devices
    hass.states.async_remove('light.ceiling_lights')
    await hass.async_block_till_done()
    result_json = await async_get_lights(hue_client)
    assert '1' not in result_json
    devices = {val['uniqueid'] for val in result_json.values()}
    assert '00:2f:d2:31:ce:c5:55:cc-ee' not in devices
    hass.states.async_set('light.ceiling_lights', STATE_ON)
    await hass.async_block_till_done()
    result_json = await async_get_lights(hue_client)
    device = result_json['1']
    assert device['uniqueid'] == '00:2f:d2:31:ce:c5:55:cc-ee'
...
async def test_light_without_brightness_supported(hass_hue, hue_client) -> None:
    ...
    light_without_brightness_json = await perform_get_light_state(hue_client, 'light.no_brightness', HTTPStatus.OK)
...
async def test_lights_all_dimmable(hass: HomeAssistant, hass_client_no_auth: ClientSessionGenerator) -> None:
    """Test CONF_LIGHTS_ALL_DIMMABLE."""
    hass.states.async_set('light.no_brightness', 'on', {})
    await setup.async_setup_component(hass, http.DOMAIN, {http.DOMAIN: {http.CONF_SERVER_PORT: HTTP_SERVER_PORT}})
    await hass.async_block_till_done()
    hue_config = {emulated_hue.CONF_LISTEN_PORT: BRIDGE_SERVER_PORT, emulated_hue.CONF_EXPOSE_BY_DEFAULT: True, emulated_hue.CONF_LIGHTS_ALL_DIMMABLE: True}
    await _async_setup_emulated_hue(hass, hue_config)
    _mock_hue_endpoints(hass, hue_config, ENTITY_IDS_BY_NUMBER)
    client = await hass_client_no_auth()
    light_without_brightness_json = await perform_get_light_state(client, 'light.no_brightness', HTTPStatus.OK)
    assert light_without_brightness_json['state'][HUE_API_STATE_ON] is True
    assert light_without_brightness_json['type'] == 'Dimmable light'
    assert light_without_brightness_json['state'][HUE_API_STATE_BRI] == HUE_API_STATE_BRI_MAX
...
async def test_light_without_brightness_can_be_turned_off(hass_hue, hue_client) -> None:
    ...
    hass_hue.services.async_register(light.DOMAIN, SERVICE_TURN_OFF, mock_service_call, schema=None)
    no_brightness_result = await perform_put_light_state(hass_hue, hue_client, 'light.no_brightness', False)
...
async def test_light_without_brightness_can_be_turned_on(hass_hue, hue_client) -> None:
    ...
    def mock_service_call(call):
        ...
        hass_hue.states.async_set('light.no_brightness', 'on', {})
    hass_hue.services.async_register(light.DOMAIN, SERVICE_TURN_ON, mock_service_call, schema=None)
    no_brightness_result = await perform_put_light_state(hass_hue, hue_client, 'light.no_brightness', True, brightness=0)
    no_brightness_result_json = await no_brightness_result.json()
    assert no_brightness_result.status == HTTPStatus.OK
    assert CONTENT_TYPE_JSON in no_brightness_result.headers['content-type']
    assert len(no_brightness_result_json) == 1
...
@pytest.mark.parametrize(('state', 'is_reachable'), [(const.STATE_UNAVAILABLE, False), (const.STATE_OK, True), (const.STATE_UNKNOWN, True)])
...
async def test_get_light_state(hass_hue, hue_client) -> None:
    ...
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.ceiling_lights', light.ATTR_BRIGHTNESS: 127, light.ATTR_RGB_COLOR: (1, 2, 7)}, blocking=True)
    office_json = await perform_get_light_state(hue_client, 'light.ceiling_lights', HTTPStatus.OK)
    ...
    assert result_json[ENTITY_NUMBERS_BY_ID['light.ceiling_lights']]['state'][HUE_API_STATE_BRI] == 127
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_OFF, {const.ATTR_ENTITY_ID: 'light.ceiling_lights'}, blocking=True)
    office_json = await perform_get_light_state(hue_client, 'light.ceiling_lights', HTTPStatus.OK)
    ...
    await perform_get_light_state(hue_client, 'light.bed_light', HTTPStatus.UNAUTHORIZED)
    await perform_get_light_state(hue_client, 'light.kitchen_lights', HTTPStatus.UNAUTHORIZED)
...
async def test_put_light_state(hass: HomeAssistant, hass_hue, hue_client) -> None:
    ...
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.ceiling_lights', light.ATTR_BRIGHTNESS: 153}, blocking=True)
    ...
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, hue=4369, saturation=127, brightness=128)
    assert hass.states.get('light.ceiling_lights').attributes[light.ATTR_BRIGHTNESS] == 129
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, hue=4369, saturation=127, brightness=123)
    assert hass.states.get('light.ceiling_lights').attributes[light.ATTR_BRIGHTNESS] == 123
    ceiling_json = await perform_get_light_state(hue_client, 'light.ceiling_lights', HTTPStatus.OK)
    ...
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, hue=4369, saturation=127, brightness=255)
    ceiling_json = await perform_get_light_state(hue_client, 'light.ceiling_lights', HTTPStatus.OK)
    assert ceiling_json['state'][HUE_API_STATE_BRI] == 254
    assert ceiling_json['state'][HUE_API_STATE_HUE] == 4369
    assert ceiling_json['state'][HUE_API_STATE_SAT] == 127
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, brightness=100, xy=(0.488, 0.48))
    ceiling_json = await perform_get_light_state(hue_client, 'light.ceiling_lights', HTTPStatus.OK)
    assert ceiling_json['state'][HUE_API_STATE_BRI] == 100
    assert hass.states.get('light.ceiling_lights').attributes[light.ATTR_XY_COLOR] == (0.488, 0.48)
    ceiling_result = await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', False)
    ...
    bedroom_result = await perform_put_light_state(hass_hue, hue_client, 'light.bed_light', True)
    assert bedroom_result.status == HTTPStatus.UNAUTHORIZED
    kitchen_result = await perform_put_light_state(hass_hue, hue_client, 'light.kitchen_lights', True)
    assert kitchen_result.status == HTTPStatus.UNAUTHORIZED
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.ceiling_lights', light.ATTR_COLOR_TEMP: 20}, blocking=True)
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, color_temp=50)
    assert hass_hue.states.get('light.ceiling_lights').attributes[light.ATTR_COLOR_TEMP] == 50
    ...
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, brightness=99, xy=(0.488, 0.48), transitiontime=60)
    await hass.async_block_till_done()
...
async def test_put_light_state_script(hass: HomeAssistant, hass_hue, hue_client) -> None:
    """Test the setting of script variables."""
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_OFF, {const.ATTR_ENTITY_ID: 'light.kitchen_lights'}, blocking=True)
    level = 23
    brightness = round(level * 254 / 100)
    script_result = await perform_put_light_state(hass_hue, hue_client, 'script.set_kitchen_light', True, brightness)
    ...
    assert hass.states.get('light.kitchen_lights').attributes[light.ATTR_BRIGHTNESS] == 23
...
async def test_put_light_state_climate_set_temperature(hass_hue, hue_client) -> None:
    ...
    hvac_result = await perform_put_light_state(hass_hue, hue_client, 'climate.hvac', True, brightness)
    ...
    ecobee_result = await perform_put_light_state(hass_hue, hue_client, 'climate.ecobee', True)
...
async def test_put_light_state_humidifier_set_humidity(hass_hue, hue_client) -> None:
    ...
    await hass_hue.services.async_call(humidifier.DOMAIN, const.SERVICE_TURN_OFF, {const.ATTR_ENTITY_ID: 'humidifier.humidifier'}, blocking=True)
    brightness = 19
    humidity = round(brightness / 254 * 100)
    humidifier_result = await perform_put_light_state(hass_hue, hue_client, 'humidifier.humidifier', True, brightness)
    ...
    hygrostat_result = await perform_put_light_state(hass_hue, hue_client, 'humidifier.hygrostat', True)
...
async def test_put_light_state_media_player(hass_hue, hue_client) -> None:
    ...
    await hass_hue.services.async_call(media_player.DOMAIN, const.SERVICE_TURN_OFF, {const.ATTR_ENTITY_ID: 'media_player.walkman'}, blocking=True)
    level = 0.25
    brightness = round(level * 254)
    mp_result = await perform_put_light_state(hass_hue, hue_client, 'media_player.walkman', True, brightness)
...
async def test_open_cover_without_position(hass_hue, hue_client) -> None:
    ...
    await hass_hue.services.async_call(cover.DOMAIN, const.SERVICE_CLOSE_COVER, {const.ATTR_ENTITY_ID: cover_id}, blocking=True)
...
async def test_set_position_cover(hass_hue, hue_client) -> None:
    ...
    cover_result = await perform_put_light_state(hass_hue, hue_client, cover_id, False, brightness)
    assert cover_result.status == HTTPStatus.OK
    assert CONTENT_TYPE_JSON in cover_result.headers['content-type']
    cover_result_json = await cover_result.json()
...
async def test_put_light_state_fan(hass_hue, hue_client) -> None:
    ...
    await hass_hue.services.async_call(fan.DOMAIN, const.SERVICE_TURN_OFF, {const.ATTR_ENTITY_ID: 'fan.living_room_fan'}, blocking=True)
    level = 43
    brightness = round(level * 254 / 100)
    fan_result = await perform_put_light_state(hass_hue, hue_client, 'fan.living_room_fan', True, brightness)
    ...
    await perform_put_light_state(hass_hue, hue_client, 'fan.living_room_fan', True, brightness=0)
    assert hass_hue.states.get('fan.living_room_fan').state == STATE_OFF
    await perform_put_light_state(hass_hue, hue_client, 'fan.living_room_fan', True, brightness=round(33 * 254 / 100))
    assert hass_hue.states.get('fan.living_room_fan').attributes[fan.ATTR_PERCENTAGE] == 33
    with patch.object(hue_api, 'STATE_CACHED_TIMEOUT', 1e-06):
        await asyncio.sleep(1e-06)
        fan_json = await perform_get_light_state(hue_client, 'fan.living_room_fan', HTTPStatus.OK)
        assert round(fan_json['state'][HUE_API_STATE_BRI] * 100 / 254) == 33
    await perform_put_light_state(hass_hue, hue_client, 'fan.living_room_fan', True, brightness=round(66 * 254 / 100))
    assert hass_hue.states.get('fan.living_room_fan').attributes[fan.ATTR_PERCENTAGE] == 66
    with patch.object(hue_api, 'STATE_CACHED_TIMEOUT', 1e-06):
        await asyncio.sleep(1e-06)
        fan_json = await perform_get_light_state(hue_client, 'fan.living_room_fan', HTTPStatus.OK)
        assert round(fan_json['state'][HUE_API_STATE_BRI] * 100 / 254) == 66
    await perform_put_light_state(hass_hue, hue_client, 'fan.living_room_fan', True, brightness=round(100 * 254 / 100))
    assert hass_hue.states.get('fan.living_room_fan').attributes[fan.ATTR_PERCENTAGE] == 100
    with patch.object(hue_api, 'STATE_CACHED_TIMEOUT', 1e-06):
        await asyncio.sleep(1e-06)
        fan_json = await perform_get_light_state(hue_client, 'fan.living_room_fan', HTTPStatus.OK)
...
async def test_put_with_form_urlencoded_content_type(hass_hue, hue_client) -> None:
    ...
    await perform_put_test_on_ceiling_lights(hass_hue, hue_client, 'application/x-www-form-urlencoded')
    data = {'key1': 'value1', 'key2': 'value2'}
    result = await hue_client.put(f'/api/username/lights/{entity_number}/state', headers={'content-type': 'application/x-www-form-urlencoded'}, data=data)
...
async def test_allowed_methods(hue_client) -> None:
    ...
    result = await hue_client.get('/api/username/lights/ENTITY_NUMBERS_BY_ID[light.ceiling_lights]/state')
    assert result.status == HTTPStatus.METHOD_NOT_ALLOWED
    result = await hue_client.put('/api/username/lights/ENTITY_NUMBERS_BY_ID[light.ceiling_lights]')
    assert result.status == HTTPStatus.METHOD_NOT_ALLOWED
...
async def test_proper_put_state_request(hue_client) -> None:
    ...
    result = await hue_client.put('/api/username/lights/{}/state'.format(ENTITY_NUMBERS_BY_ID['light.ceiling_lights']), data=json.dumps({HUE_API_STATE_ON: 1234}))
    assert result.status == HTTPStatus.BAD_REQUEST
    result = await hue_client.put('/api/username/lights/{}/state'.format(ENTITY_NUMBERS_BY_ID['light.ceiling_lights']), data=json.dumps({HUE_API_STATE_ON: True, HUE_API_STATE_BRI: 'Hello world!'}))
    assert result.status == HTTPStatus.BAD_REQUEST
...
async def perform_put_test_on_ceiling_lights(hass_hue, hue_client, content_type=CONTENT_TYPE_JSON):
    """Test the setting of a light."""
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_OFF, {const.ATTR_ENTITY_ID: 'light.ceiling_lights'}, blocking=True)
    ceiling_lights = hass_hue.states.get('light.ceiling_lights')
    assert ceiling_lights.state == STATE_OFF
    office_result = await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, 56, content_type)
...
async def perform_get_light_state(client, entity_id, expected_status):
    ...
    return await perform_get_light_state_by_number(client, entity_number, expected_status)

async def perform_put_light_state(hass_hue, client, entity_id, is_on, brightness=None, content_type=CONTENT_TYPE_JSON, hue=None, saturation=None, color_temp=None, with_state=True, xy=None, transitiontime=None):
    ...
    entity_number = ENTITY_NUMBERS_BY_ID[entity_id]
    result = await client.put(f'/api/username/lights/{entity_number}/state', headers=req_headers, data=json.dumps(data).encode())
    await hass_hue.async_block_till_done()
    return result

async def test_external_ip_blocked(hue_client) -> None:
    """Test external IP blocked."""
    getUrls = ['/api/username/groups', '/api/username', '/api/username/config', '/api/username/lights', '/api/username/lights/light.ceiling_lights']
    postUrls = ['/api']
    putUrls = ['/api/username/lights/light.ceiling_lights/state', '/api/username/groups/0/action']
    with patch('homeassistant.components.emulated_hue.hue_api.ip_address', return_value=ip_address('45.45.45.45')):
...
async def test_put_then_get_cached_properly(hass: HomeAssistant, hass_hue, hue_client) -> None:
    """Test the setting of light states and an immediate readback reads the same values."""
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.ceiling_lights', light.ATTR_BRIGHTNESS: 153}, blocking=True)
    ceiling_lights = hass_hue.states.get('light.ceiling_lights')
    assert ceiling_lights.state == STATE_ON
    assert ceiling_lights.attributes[light.ATTR_BRIGHTNESS] == 153
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, hue=4369, saturation=127, brightness=254)
    assert hass.states.get('light.ceiling_lights').attributes[light.ATTR_BRIGHTNESS] == 255
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.ceiling_lights', light.ATTR_BRIGHTNESS: 153}, blocking=True)
    ceiling_json = await perform_get_light_state(hue_client, 'light.ceiling_lights', HTTPStatus.OK)
    assert ceiling_json['state'][HUE_API_STATE_HUE] == 4369
    assert ceiling_json['state'][HUE_API_STATE_SAT] == 127
    ...
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.ceiling_lights'}, blocking=True)
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, hue=4369, saturation=127, brightness=254)
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.ceiling_lights', light.ATTR_BRIGHTNESS: 127, light.ATTR_RGB_COLOR: (1, 2, 7)}, blocking=True)
    ceiling_json = await perform_get_light_state(hue_client, 'light.ceiling_lights', HTTPStatus.OK)
    assert ceiling_json['state'][HUE_API_STATE_HUE] == 4369
...
async def test_put_than_get_when_service_call_fails(hass: HomeAssistant, hass_hue, hue_client) -> None:
    ...
    ceiling_lights = hass_hue.states.get('light.ceiling_lights')
    assert ceiling_lights.state == STATE_OFF
    with patch.object(hue_api, 'STATE_CHANGE_WAIT_TIMEOUT', 1e-06):
        await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, hue=4369, saturation=127, brightness=254)
...
async def test_put_light_state_scene(hass: HomeAssistant, hass_hue, hue_client) -> None:
    ...
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_OFF, {const.ATTR_ENTITY_ID: 'light.kitchen_lights'}, blocking=True)
    scene_result = await perform_put_light_state(hass_hue, hue_client, 'scene.light_on', True)
    ...
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.kitchen_lights', light.ATTR_BRIGHTNESS: 127}, blocking=True)
    await perform_put_light_state(hass_hue, hue_client, 'scene.light_on', True, brightness=254)
    assert hass_hue.states.get('light.kitchen_lights').state == STATE_ON
    assert hass_hue.states.get('light.kitchen_lights').attributes[light.ATTR_BRIGHTNESS] == 127
...
async def test_only_change_contrast(hass: HomeAssistant, hass_hue, hue_client) -> None:
    ...
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_OFF, {const.ATTR_ENTITY_ID: 'light.ceiling_lights'}, blocking=True)
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, brightness=254, with_state=False)
    ceiling_lights = hass_hue.states.get('light.ceiling_lights')
    assert ceiling_lights.state == STATE_ON
    assert ceiling_lights.attributes[light.ATTR_BRIGHTNESS] == 255

async def test_only_change_hue_or_saturation(hass: HomeAssistant, hass_hue, hue_client) -> None:
    """Test setting either the hue or the saturation but not both."""
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.ceiling_lights', light.ATTR_HS_COLOR: (10, 10)}, blocking=True)
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, hue=4369)
    assert hass_hue.states.get('light.ceiling_lights').attributes[light.ATTR_HS_COLOR] == (24, 0)
    await hass_hue.services.async_call(light.DOMAIN, const.SERVICE_TURN_ON, {const.ATTR_ENTITY_ID: 'light.ceiling_lights', light.ATTR_HS_COLOR: (10, 10)}, blocking=True)
    await perform_put_light_state(hass_hue, hue_client, 'light.ceiling_lights', True, saturation=10)
    assert hass_hue.states.get('light.ceiling_lights').attributes[light.ATTR_HS_COLOR] == (0, 3)

async def test_specificly_exposed_entities(hass: HomeAssistant, base_setup, hass_client_no_auth: ClientSessionGenerator) -> None:
    """Test specific entities with expose by default off."""
    conf = {emulated_hue.CONF_LISTEN_PORT: BRIDGE_SERVER_PORT, emulated_hue.CONF_EXPOSE_BY_DEFAULT: False, emulated_hue.CONF_ENTITIES: {'light.exposed': {emulated_hue.CONF_ENTITY_HIDDEN: False}}}
...
async def test_get_light_state_when_none(hass_hue: HomeAssistant, hue_client) -> None:
    ...
    hass_hue.states.async_set('light.ceiling_lights', STATE_ON, {light.ATTR_BRIGHTNESS: None, light.ATTR_RGB_COLOR: None, light.ATTR_HS_COLOR: None, light.ATTR_COLOR_TEMP: None, light.ATTR_XY_COLOR: None, light.ATTR_SUPPORTED_COLOR_MODES: [light.COLOR_MODE_COLOR_TEMP, light.COLOR_MODE_HS, light.COLOR_MODE_XY], light.ATTR_COLOR_MODE: light.COLOR_MODE_XY})
    light_json = await perform_get_light_state(hue_client, 'light.ceiling_lights', HTTPStatus.OK)
    ...
    hass_hue.states.async_set('light.ceiling_lights', STATE_OFF, {light.ATTR_BRIGHTNESS: None, light.ATTR_RGB_COLOR: None, light.ATTR_HS_COLOR: None, light.ATTR_COLOR_TEMP: None, light.ATTR_XY_COLOR: None, light.ATTR_SUPPORTED_COLOR_MODES: [light.COLOR_MODE_COLOR_TEMP, light.COLOR_MODE_HS, light.COLOR_MODE_XY], light.ATTR_COLOR_MODE: light.COLOR_MODE_XY})
    light_json = await perform_get_light_state(hue_client, 'light.ceiling_lights', HTTPStatus.OK)
    state = light_json['state']
