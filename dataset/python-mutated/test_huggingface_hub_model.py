...
def get_mock_model(model_name, huggingfacehub_api_type, mocker):
    ...
    credentials = {'huggingfacehub_api_type': huggingfacehub_api_type, 'huggingfacehub_api_token': valid_api_key}
    if huggingfacehub_api_type == 'inference_endpoints':
        credentials['huggingfacehub_endpoint_url'] = endpoint_url
    mock_query = MagicMock()
    mock_query.filter.return_value.first.return_value = ProviderModel(provider_name='huggingface_hub', model_name=model_name, model_type=ModelType.TEXT_GENERATION.value, encrypted_config=json.dumps(credentials), is_valid=True)
    mocker.patch('extensions.ext_database.db.session.query', return_value=mock_query)
    return HuggingfaceHubModel(model_provider=model_provider, name=model_name, model_kwargs=model_kwargs)
...
def test_hosted_inference_api_get_num_tokens(mock_decrypt, mock_model_info, mocker):
    ...
    model = get_mock_model('tiiuae/falcon-40b', 'hosted_inference_api', mocker)
...
def test_inference_endpoints_get_num_tokens(mock_decrypt, mock_model_info, mocker):
    ...
    mock_model_info.return_value = MagicMock(pipeline_tag='text2text-generation')
    mocker.patch('langchain.llms.huggingface_hub.HuggingFaceHub._call', return_value='abc')
    model = get_mock_model('', 'inference_endpoints', mocker)
    rst = model.get_num_tokens([PromptMessage(type=MessageType.USER, content='Who is your manufacturer?')])
    assert rst == 5
...
def test_hosted_inference_api_run(mock_decrypt, mocker):
    ...
    model = get_mock_model('google/flan-t5-base', 'hosted_inference_api', mocker)
    rst = model.run([PromptMessage(content='Human: Are you Really Human? you MUST only answer `y` or `n`? \nAssistant: ')], stop=['\nHuman:'])
...
def test_inference_endpoints_run(mock_decrypt, mocker):
    ...
    mocker.patch('core.model_providers.providers.base.BaseModelProvider.update_last_used', return_value=None)
    model = get_mock_model('', 'inference_endpoints', mocker)
    rst = model.run([PromptMessage(content='Answer the following yes/no question. Can you write a whole Haiku in a single tweet?')])
    assert len(rst.content) > 0