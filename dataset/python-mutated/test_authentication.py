...
async def websocket_endpoint(websocket):
    ...
    await websocket.send_json({'authenticated': websocket.user.is_authenticated, 'user': websocket.user.display_name})
...
async def websocket_endpoint_decorated(websocket, additional):
    await websocket.accept()
    await websocket.send_json({'authenticated': websocket.user.is_authenticated, 'user': websocket.user.display_name, 'additional': additional})
app = Starlette(middleware=[Middleware(AuthenticationMiddleware, backend=BasicAuth())], routes=[Route('/', endpoint=homepage), Route('/dashboard', endpoint=dashboard), Route('/admin', endpoint=admin), Route('/dashboard/sync', endpoint=dashboard_sync), Route('/dashboard/class', endpoint=Dashboard), Route('/admin/sync', endpoint=admin_sync), Route('/dashboard/decorated', endpoint=decorated_async), Route('/dashboard/decorated/sync', endpoint=decorated_sync), WebSocketRoute('/ws', endpoint=websocket_endpoint), WebSocketRoute('/ws/decorated', endpoint=websocket_endpoint_decorated)])
...
def control_panel(request):
    if False:
        print('Hello World!')
    return JSONResponse({'authenticated': request.user.is_authenticated, 'user': request.user.display_name})
other_app = Starlette(routes=[Route('/control-panel', control_panel)], middleware=[Middleware(AuthenticationMiddleware, backend=BasicAuth(), on_error=on_auth_error)])
