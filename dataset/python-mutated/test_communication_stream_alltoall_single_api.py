...
class TestCommunicationStreamAllToAllSingleAPI(test_base.CommunicationTestDistBase):
    ...
    def test_alltoall_single_stream(self):
        ...
        for envs in envs_list:
            if eval(envs['use_calc_stream']) and (not eval(envs['sync_op'])):
                continue
            self.run_test_case('communication_stream_alltoall_single_api_dygraph.py', user_defined_envs=envs)
