...
from examples.speech_recognition.kaldi.kaldi_decoder import KaldiDecoder, KaldiDecoderConfig
...
class UnpairedAudioTextConfig(FairseqDataclass):
    ...
    labels: Optional[str] = field(default=None, metadata={'help': 'extension of the label file to load, used for fine-tuning'})
    aux_target_postfix: Optional[str] = field(default=None, metadata={'help': 'auxaliry target filename extension'})
    unfiltered: bool = field(default=False, metadata={'help': 'load data with _unfiltered suffix'})
    ctc_eval: bool = field(default=False, metadata={'help': 'eval UER as if computed by CTC'})
    sort_by_length: bool = field(default=True, metadata={'help': 'sort examples by length of audio timesteps'})
    ...
    uppercase: Optional[bool] = field(default=False, metadata={'help': 'uppercase for LM score computation'})
    skipwords: Optional[str] = field(default='', metadata={'help': 'comma-separated words to be removed for LM score computation'})
    kenlm_path: Optional[str] = None
