...
class MrpWorkcenter(models.Model):
    ...
    capacity = fields.Float('Capacity', default=1.0, oldname='capacity_per_cycle', help='Number of pieces that can be produced in parallel.')
    sequence = fields.Integer('Sequence', default=1, required=True, help='Gives the sequence order when displaying a list of work centers.')
    ...
    working_state = fields.Selection([('normal', 'Normal'), ('blocked', 'Blocked'), ('done', 'In Progress')], 'Status', compute='_compute_working_state', store=True)
    blocked_time = fields.Float('Blocked Time', compute='_compute_blocked_time', help='Blocked hour(s) over the last month', digits=(16, 2))
    productive_time = fields.Float('Productive Time', compute='_compute_productive_time', help='Productive hour(s) over the last month', digits=(16, 2))
...
class MrpWorkcenterProductivityLoss(models.Model):
    ...
    loss_type = fields.Selection([('availability', 'Availability'), ('performance', 'Performance'), ('quality', 'Quality'), ('productive', 'Productive')], 'Effectiveness Category', default='availability', required=True)
...
class MrpWorkcenterProductivity(models.Model):
    ...
    loss_id = fields.Many2one('mrp.workcenter.productivity.loss', 'Loss Reason', ondelete='restrict', required=True)
    loss_type = fields.Selection('Effectiveness', related='loss_id.loss_type', store=True)
