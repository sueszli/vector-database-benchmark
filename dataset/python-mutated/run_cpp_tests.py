...
def run_in_docker(cmd, workdir='/build'):
    ...
    if not isinstance(cmd, list):
        cmd = [cmd]
    subprocess.check_call(['docker', 'run', '--rm', '-m=4g', '--mount', 'type=bind,source=' + WORKSPACE + ',target=/build,consistency=delegated', '-w=%s' % workdir, TC_BUILD_IMAGE_COS6] + cmd)
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run cxxtests (optionally caching those that have not been modified).')
    ...
    parser.add_argument('--cache-file', type=str, default='cache_for_cxxtests.pkl', help='Filename where the cache should be saved')
    parser.add_argument('-j', type=int, default=4, help='Number of processes to use for ctest command.')
    parser.add_argument('--dry-run', action='store_true', help='If present, the ctest command is printed rather than run.')
    parser.add_argument('--docker', action='store_true', help='Run the C++ tests inside of Docker on Centos 6.')
    ...
    cmd = ['ctest', '--output-on-failure', '-j', str(args.j)]
    if args.schedule_random:
        cmd.append('--schedule-random')
    cmd += ['-R', '({})'.format('|'.join(runtests))]
    if args.dry_run:
