    ...
    def test_prowler_finding(self):
        ...
        with open(get_unit_tests_path() + sample_path('prowler-output.asff.json')) as test_file:
            ...
            with self.subTest(i=0):
                ...
                self.assertEqual('prowler-accessanalyzer_enabled-123456789012-ap-northeast-1-fb33278bd', finding.unique_id_from_tool)
            with self.subTest(i=300):
                finding = findings[300]
                self.assertIn('Ensure no security groups allow ingress from 0.0.0.0/0 or ::/0 to Elasticsearch/Kibana ports', finding.title)
                self.assertIn('Security group default (sg-3965844c) has not Elasticsearch/Kibana ports 9200, 9300 and 5601 open to the Internet', finding.description)
                self.assertEqual(datetime.date(2023, 3, 18), finding.date.date())
                self.assertEqual('High', finding.severity)
                self.assertTrue(finding.active)
                self.assertEqual('prowler-ec2_securitygroup_allow_ingress_from_internet_to_tcp_port_elasticsearch_kibana_9200_9300_5601-123456789012-sa-east-1-49c27aa6d', finding.unique_id_from_tool)
            with self.subTest(i=700):
                ...
                self.assertTrue(finding.active)
                self.assertEqual('prowler-securityhub_enabled-123456789012-ap-southeast-1-d053c2dfc', finding.unique_id_from_tool)

    def test_guardduty_finding(self):
        if False:
            for i in range(10):
                print('nop')
        with open(get_unit_tests_path() + sample_path('guardduty/Unusual Behaviors-User-Persistence IAMUser-NetworkPermissions.json')) as test_file:
            parser = AsffParser()
            findings = parser.get_findings(test_file, Test())
            ...
            with self.subTest(i=0):
                ...
                self.assertIn('APIs commonly used to change the network access permissions for security groups, routes and ACLs, was invoked by IAM principal GeneratedFindingUserName', finding.description)
                self.assertEqual(datetime.date(2020, 11, 11), finding.date.date())
                self.assertEqual('Medium', finding.severity)
                self.assertTrue(finding.active)
                self.assertEqual('arn:aws:guardduty:eu-west-1:123456789012:detector/cab6a714deb3b739eaddacbdfd5ef2f2/finding/d6badb90e557d4bd811488a53ca89895', finding.unique_id_from_tool)