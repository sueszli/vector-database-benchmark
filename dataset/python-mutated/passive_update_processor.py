...
from homeassistant.const import ATTR_CONNECTIONS, ATTR_IDENTIFIERS, ATTR_NAME, CONF_ENTITY_CATEGORY, EVENT_HOMEASSISTANT_STOP, EntityCategory
...
async def async_setup(hass: HomeAssistant) -> None:
    ...
    storage: Store[dict[str, dict[str, RestoredPassiveBluetoothDataUpdate]]] = Store(hass, STORAGE_VERSION, STORAGE_KEY)
    coordinators: set[PassiveBluetoothProcessorCoordinator] = set()
    all_restore_data: dict[str, dict[str, RestoredPassiveBluetoothDataUpdate]] = await storage.async_load() or {}
    hass.data[PASSIVE_UPDATE_PROCESSOR] = PassiveBluetoothProcessorData(coordinators, all_restore_data)

    async def _async_save_processor_data(_: Any) -> None:
        """Save the processor data."""
        await storage.async_save({coordinator.restore_key: coordinator.async_get_restore_data() for coordinator in coordinators if coordinator.restore_key})
    cancel_interval = async_track_time_interval(hass, _async_save_processor_data, STORAGE_SAVE_INTERVAL)
    ...
    hass.bus.async_listen_once(EVENT_HOMEASSISTANT_STOP, _async_save_processor_data_at_stop)
...
class PassiveBluetoothProcessorEntity(Entity, Generic[_PassiveBluetoothDataProcessorT]):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self.async_on_remove(self.processor.async_add_entity_key_listener(self._handle_processor_update, self.entity_key))
