...
def _get_default_args(estimator_cls):
    if False:
        while True:
            i = 10
    defaults = dict(H2OCoxProportionalHazardsClassifier=dict(), H2ODeepLearningClassifier=dict(seed=seed, reproducible=True), H2OGeneralizedAdditiveClassifier=dict(family='binomial', seed=seed, gam_columns=['C1']), H2OGeneralizedLinearClassifier=dict(family='binomial', seed=seed))
...
def _get_custom_behaviour(estimator_cls):
    if False:
        i = 10
        return i + 15
    custom = dict(H2ODeepLearningClassifier=dict(allow_verbose=True), H2OGradientBoostingClassifier=dict(allow_verbose=True), H2ORandomForestClassifier=dict(allow_verbose=True))
...
def test_estimator_with_h2o_frames(estimator_cls):
    ...
    if _get_custom_behaviour(estimator_cls).get('allow_verbose', False):
        estimator.fit(data.X_train, data.y_train, verbose=True)
    else:
...
failing = ['H2OStackedEnsembleClassifier', 'H2OUpliftRandomForestClassifier']
classifiers = [cls for (name, cls) in inspect.getmembers(h2o.sklearn, inspect.isclass) if name.endswith('Classifier') and name not in ['H2OAutoMLClassifier'] + failing]
