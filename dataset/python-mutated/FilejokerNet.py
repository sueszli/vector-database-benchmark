...
class FilejokerNet(XFSDownloader):
    ...
    __version__ = '0.16'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?filejoker\\.net/(?:file/)?(?P<ID>\\w{12})'
    ...
    ERROR_PATTERN = 'Wrong Captcha|Session expired|Your download has not finished yet'
    PREMIUM_ONLY_PATTERN = 'Free Members can download files no bigger'
    WAIT_PATTERN = '<span id="count" class="alert-success">([\\w ]+?)</span>\\s*seconds'
    DL_LIMIT_PATTERN = 'Wait [\\w ]+? to download for free.'
    TEMP_OFFLINE_PATTERN = 'Your download has not finished yet'
    INFO_PATTERN = '<div class="name-size"><span>(?P<N>.+?)</span> <p>(?:\\()?(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)(?:\\()?</p></div>'
    SIZE_REPLACEMENTS = [('Kb', 'KiB'), ('Mb', 'MiB'), ('Gb', 'GiB')]
    LINK_PATTERN = '<div class=".*premium-download">\\s+<a href="(.+?)"'
    ...
    FORM_INPUTS_MAP = {'op': re.compile('^download')}
