...
from aiogram.exceptions import ClientDecodeError, RestartingTelegram, TelegramAPIError, TelegramBadRequest, TelegramConflictError, TelegramEntityTooLarge, TelegramForbiddenError, TelegramMigrateToChat, TelegramNotFound, TelegramRetryAfter, TelegramServerError, TelegramUnauthorizedError
...
class CustomSession(BaseSession):
    ...
    async def make_request(self, token: str, method: TelegramMethod[TelegramType], timeout: Optional[int]=UNSET_PARSE_MODE) -> None:
        assert isinstance(token, str)
        assert isinstance(method, TelegramMethod)

    async def stream_content(self, url: str, headers: Optional[Dict[str, Any]]=None, timeout: int=30, chunk_size: int=65536, raise_for_status: bool=True) -> AsyncGenerator[bytes, None]:
...
class TestBaseSession:
    ...
    async def test_stream_content(self):
        session = CustomSession()
        stream = session.stream_content('https://www.python.org/static/img/python-logo.png', headers={}, timeout=5, chunk_size=65536, raise_for_status=True)
    ...
    async def test_context_manager(self):
        ...
        with patch('tests.test_api.test_client.test_session.test_base_session.CustomSession.close', new_callable=AsyncMock) as mocked_close:
