...
if __name__ == '__main__':
    (cluster_config, retries, no_config_cache, num_expected_nodes, docker_override, wheel_override) = check_arguments()
    ...
    config_yaml['cluster_name'] = config_yaml['cluster_name'] + '-' + str(int(time.time()))
    print('======================================')
    ...
    if provider_type == 'aws':
        ...
        account_email = subprocess.run(['gcloud', 'config', 'get-value', 'account'], stdout=subprocess.PIPE, check=True).stdout.decode('utf-8').strip()
        print('Active account email:', account_email)
        project_id = subprocess.run(['gcloud', 'config', 'get-value', 'project'], stdout=subprocess.PIPE, check=True).stdout.decode('utf-8').strip()
        print(f"Injecting GCP project '{project_id}' into cluster configuration file...")
