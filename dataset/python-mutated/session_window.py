...
def session_window_demo():
    ...
    ds = env.from_collection(collection=[(Instant.of_epoch_milli(1000), 'Alice', 110.1), (Instant.of_epoch_milli(4000), 'Bob', 30.2), (Instant.of_epoch_milli(3000), 'Alice', 20.0), (Instant.of_epoch_milli(2000), 'Bob', 53.1), (Instant.of_epoch_milli(8000), 'Bob', 16.1), (Instant.of_epoch_milli(10000), 'Alice', 20.1)], type_info=Types.ROW([Types.INSTANT(), Types.STRING(), Types.FLOAT()]))
    ...
    table = table.window(Session.with_gap(lit(5).seconds).on(col('ts')).alias('w')).group_by(col('name'), col('w')).select(col('name'), col('price').sum, col('w').start, col('w').end)
