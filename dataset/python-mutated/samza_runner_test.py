...
class SamzaRunnerTest(portable_runner_test.PortableRunnerTest):
    ...
    def parse_options(self, request):
        ...
        parser.add_argument('--samza_job_server_jar', help='Job server jar to submit jobs.', action='store')
        ...
        self.set_samza_job_server_jar(known_args.samza_job_server_jar or job_server.JavaJarJobServer.path_to_beam_jar(':runners:samza:job-server:shadowJar'))
    ...
    def _subprocess_command(cls, job_port, expansion_port):
        ...
        try:
            return ['java', '-jar', cls.samza_job_server_jar, '--artifacts-dir', tmp_dir, '--job-port', str(job_port), '--artifact-port', '0', '--expansion-port', str(expansion_port)]
        finally:
            rmtree(tmp_dir)
