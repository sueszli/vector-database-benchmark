...
class PlayersSheet(GalconSheet):
    columns = [ColumnAttr('name'), ColumnAttr('ready')]
    colorizers = [RowColorizer(8, None, lambda s, c, r, v: r and r.color)]
...
class PlanetsSheet(GalconSheet):
    columns = [ColumnAttr('name'), ColumnAttr('prod', type=int), ColumnAttr('killpct', type=int, fmtstr='%s%%'), ColumnAttr('ownername'), ColumnAttr('nships', type=int), Column('turns_to_marked', type=int, getter=lambda col, row: distance_turns(row, col.sheet.marked_planet)), ColumnAttr('x', type=int, width=4), ColumnAttr('y', type=int, width=4)]
    colorizers = [RowColorizer(5, None, lambda sheet, col, row, value: row and vd.g_client.Players.get_player_color(row.ownername)), RowColorizer(5, 'color_dest_planet', lambda sheet, c, row, v: row is sheet.marked_planet)]
    marked_planet = None
...
class QueuedDeploymentsSheet(GalconSheet):
    columns = [ColumnItem('src_turn', 'launch_turn', type=int), ColumnItem('dest_turn', 'dest_turn', type=int), ColumnItem('src', 'launch_planet_name'), ColumnItem('dest', 'dest_planet_name'), ColumnItem('nrequested', 'nships_requested', type=int), ColumnItem('ndeployed', 'nships_deployed', type=int), ColumnItem('result', 'result')]
    colorizers = [CellColorizer(5, 'red bold', lambda s, c, r, v: c and s.colorIncomplete(c, r, v))]
...
class HistoricalDeploymentsSheet(GalconSheet):
    columns = [ColumnItem('player', 'launch_player_name'), ColumnItem('src_turn', 'launch_turn', type=int), ColumnItem('dest_turn', 'dest_turn', type=int), ColumnItem('src', 'launch_planet_name'), ColumnItem('dest', 'dest_planet_name'), ColumnItem('nships', 'nships_deployed', type=int), ColumnItem('killpct', 'killpct', type=int, fmtstr='%s%%')]
    colorizers = [RowColorizer(9, None, lambda s, c, r, v: r and vd.g_client.Players.get_player_color(r['launch_player_name']))]
...
class EventsSheet(GalconSheet):
    columns = [ColumnItem('turn', 'turn', type=int), ColumnItem('event', 'event', width=80)]
...
class MapSheet(GalconSheet):
    ...
    colorizers = [CellColor(9, None, lambda s, c, r, v: vd.g_client.Players.get_player_color(r[c.x].ownername) if r[c.x] else None), CellColor(5, 'color_dest_planet', lambda s, c, r, v: r[c.x] and r[c.x] is vd.g_client.Planets.marked_planet), CellColor(2, None, lambda s, c, r, v: s.colorSpace(c, r, v))]
...
class GameOptionsSheet(GalconSheet):
    columns = [ColumnItem('option', 0), ColumnItem('value', 1)]
