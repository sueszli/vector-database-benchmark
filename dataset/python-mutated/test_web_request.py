...
async def test_call_POST_on_weird_content_type() -> None:
    req = make_mocked_request('POST', '/', headers=CIMultiDict({'CONTENT-TYPE': 'something/weird'}))
    ret = await req.post()
    assert CIMultiDict() == ret
...
async def test_multipart_formdata(protocol: Any) -> None:
    ...
    content_type = 'multipart/form-data; boundary=---------------------------326931944431359'
    payload.feed_eof()
    req = make_mocked_request('POST', '/', headers={'CONTENT-TYPE': content_type}, payload=payload)
    result = await req.post()
    assert dict(result) == {'a': 'b', 'c': 'd'}

async def test_multipart_formdata_file(protocol: Any) -> None:
    payload = StreamReader(protocol, 2 ** 16, loop=asyncio.get_event_loop())
    payload.feed_data(b'-----------------------------326931944431359\r\nContent-Disposition: form-data; name="a_file"; filename="binary"\r\n\r\n\x0cf\r\n-----------------------------326931944431359--\r\n')
    content_type = 'multipart/form-data; boundary=---------------------------326931944431359'
    payload.feed_eof()
    req = make_mocked_request('POST', '/', headers={'CONTENT-TYPE': content_type}, payload=payload)
    result = await req.post()
...
async def test_json_invalid_content_type(aiohttp_client: Any) -> None:
    ...
    async with client.post('/', json=json_data, headers=headers) as resp:
        ...
        assert resp_text == 'Attempt to decode JSON with unexpected mimetype: text/plain'
...
@pytest.mark.xfail(raises=ServerDisconnectedError, reason='see https://github.com/aio-libs/aiohttp/issues/4572')
