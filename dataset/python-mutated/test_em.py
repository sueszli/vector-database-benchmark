...
def model(detections, args):
    ...
    with pyro.plate('objects_plate', max_num_objects):
        ...
        with poutine.mask(mask=exists.bool()):
            pyro.sample('objects', dist.Normal(0.0, 1.0), obs=objects)
    ...
    with pyro.plate('detections_plate', num_detections):
        ...
        fake_dist = dist.Normal(0.0, 1.0)
        pyro.sample('detections', dist.MaskedMixture(is_fake, real_dist, fake_dist), obs=detections)

def compute_exists_logits(objects, args):
    if False:
        return 10
    p_exists = args.expected_num_objects / args.max_num_objects
    real_part = dist.Normal(0.0, 1.0).log_prob(objects)
...
def compute_assign_logits(objects, detections, noise_scale, args):
    ...
    fake_part = dist.Normal(0.0, 1.0).log_prob(detections)
...
def guide(detections, args):
    ...
    with pyro.plate('objects_plate', max_num_objects):
        pyro.sample('exists', assignment.exists_dist, infer={'enumerate': 'parallel'})
    with pyro.plate('detections_plate', num_detections):
...
def test_em(assignment_grad):
    ...
    pyro.param('noise_scale', torch.tensor(args.init_noise_scale), constraint=constraints.positive)
    ...
    for step in range(10):
        ...
        newton.step(loss, {'objects_loc': objects_loc})
...
def test_em_nested_in_svi(assignment_grad):
    ...
    elbo = TraceEnum_ELBO(max_plate_nesting=2)
    newton = Newton(trust_radii={'objects_loc': 1.0})
    svi = SVI(poutine.block(model, hide=['objects_loc']), poutine.block(guide, hide=['objects_loc']), optim, elbo)
    for svi_step in range(50):
        for em_step in range(2):
            objects_loc = pyro.param('objects_loc').detach_().requires_grad_()
            assert pyro.param('objects_loc').grad_fn is None
            loss = elbo.differentiable_loss(model, guide, detections, args)
            updated = newton.get_step(loss, {'objects_loc': objects_loc})
        ...
        logger.debug('step {: >2d}, loss = {:0.6f}, noise_scale = {:0.6f}'.format(svi_step, loss, pyro.param('noise_scale').item()))
...
def test_svi_multi():
    ...
    adam = Adam({'lr': 0.1})
    newton = Newton(trust_radii={'objects_loc': 1.0})
    optim = MixedMultiOptimizer([(['noise_scale'], adam), (['objects_loc'], newton)])
    for svi_step in range(50):
        with poutine.trace(param_only=True) as param_capture:
            loss = elbo.differentiable_loss(model, guide, detections, args)
        params = {name: pyro.param(name).unconstrained() for name in param_capture.trace.nodes.keys()}
        optim.step(loss, params)
        logger.debug('step {: >2d}, loss = {:0.6f}, noise_scale = {:0.6f}'.format(svi_step, loss.item(), pyro.param('noise_scale').item()))