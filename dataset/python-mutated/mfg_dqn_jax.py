...
FLAGS = flags.FLAGS
flags.DEFINE_string('game_name', 'python_mfg_predator_prey', 'Name of the game.')
flags.DEFINE_string('env_setting', None, 'Name of the game settings. If None, the game name will be used.')
flags.DEFINE_integer('num_train_episodes', int(20000000.0), 'Number of training episodes.')
flags.DEFINE_integer('eval_every', 10000, 'Episode frequency at which the agents are evaluated.')
flags.DEFINE_list('hidden_layers_sizes', [128, 128], 'Number of hidden units in the avg-net and Q-net.')
flags.DEFINE_integer('replay_buffer_capacity', int(200000.0), 'Size of the replay buffer.')
...
flags.DEFINE_integer('epsilon_decay_duration', int(20000000.0), 'Number of game steps over which epsilon is decayed.')
...
flags.DEFINE_string('checkpoint_dir', '/tmp/dqn_test', 'Directory to save/load the agent.')
flags.DEFINE_string('logdir', None, 'Logging dir to use for TF summary files. If None, the metrics will only be logged to stderr.')
...
def main(unused_argv):
    ...
    for ep in range(1, FLAGS.num_train_episodes + 1):
        if ep % FLAGS.eval_every == 0:
            writer.write_scalars(ep, {f'agent{i}/loss': float(agent.loss) for (i, agent) in enumerate(agents)})
            initial_states = game.new_initial_states()
            nash_conv_obj = nash_conv.NashConv(game, uniform_policy)
            writer.write_scalars(ep, {f'exact_br/{state}': value for (state, value) in zip(initial_states, nash_conv_obj.br_values())})
            pi_value = policy_value.PolicyValue(game, mfg_dist, joint_avg_policy)
            writer.write_scalars(ep, {f'dqn_br/{state}': pi_value.eval_state(state) for state in initial_states})
            if FLAGS.use_checkpoints:
                for agent in agents:
