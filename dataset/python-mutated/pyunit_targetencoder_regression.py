...
def test_regression_with_none():
    ...
    print(encoded)
    col_te_golden = [46.1981, 26.14816, 46.1981, 26.14816, 46.19809]
...
def test_regression_with_kfold():
    ...
    encoded = te.transform(ds.train, as_training=True)
    print(encoded)
    col_te_golden = [45.05591, 24.68419, 45.0065, 27.6513, 45.0065]
    col_te = encoded['sex_te'].head(5).as_data_frame().values.reshape(-1).tolist()
    assert_allclose(col_te, col_te_golden, atol=1e-05)
...
def test_regression_with_loo():
    ...
    assert golden.names == encoded.names
    assert pu.compare_frames(golden, encoded, 0, tol_numeric=1e-05)
pu.run_tests([test_regression_with_none, test_regression_with_kfold, test_regression_with_loo])