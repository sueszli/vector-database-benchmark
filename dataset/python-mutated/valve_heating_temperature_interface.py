...
from homeassistant.components.climate import PRESET_AWAY, PRESET_COMFORT, PRESET_ECO, PRESET_NONE, ClimateEntity, ClimateEntityFeature, HVACAction, HVACMode, UnitOfTemperature
...
class ValveHeatingTemperatureInterface(OverkizEntity, ClimateEntity):
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        await self.executor.async_execute_command(OverkizCommand.SET_DEROGATION, float(temperature), OverkizCommandParam.FURTHER_NOTICE)
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        ...
        if preset_mode == PRESET_MANUAL:
            if (current_temperature := self.current_temperature):
                await self.executor.async_execute_command(OverkizCommand.SET_DEROGATION, current_temperature, OverkizCommandParam.FURTHER_NOTICE)
        else:
            await self.executor.async_execute_command(OverkizCommand.SET_DEROGATION, PRESET_MODE_TO_OVERKIZ[preset_mode], OverkizCommandParam.FURTHER_NOTICE)