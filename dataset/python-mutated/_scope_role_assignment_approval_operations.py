...
class ScopeRoleAssignmentApprovalOperations:
    ...
    @distributed_trace
    def list(self, scope: str, filter: Optional[str]=None, **kwargs: Any) -> Iterable['_models.RoleAssignmentApproval']:
        if False:
            print('Hello World!')
    ...
    def get_by_id(self, approval_id: str, scope: str, **kwargs: Any) -> _models.RoleAssignmentApproval:
        ...
        request = build_get_by_id_request(approval_id=approval_id, scope=scope, api_version=api_version, template_url=self.get_by_id.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = self._client._pipeline.run(request, stream=_stream, **kwargs)
