...
SCHED_S_TASK_NOT_SCHEDULED = 267009
...
class TASK_USER_CRED(NDRSTRUCT):
    structure = (('userId', LPWSTR), ('password', LPWSTR), ('flags', DWORD))
...
class TASK_XML_ERROR_INFO(NDRSTRUCT):
    structure = (('line', DWORD), ('column', DWORD), ('node', LPWSTR), ('value', LPWSTR))

class PTASK_XML_ERROR_INFO(NDRPOINTER):
    referent = (('Data', TASK_XML_ERROR_INFO),)

class FIXDLEN_DATA(Structure):
    structure = (('Product Version', '<H=0'), ('File Version', '<H=0'), ('Job uuid', '16s="'), ('App Name Len Offset', '<H=0'), ('Trigger Offset', '<H=0'), ('Error Retry Count', '<H=0'), ('Error Retry Interval', '<H=0'), ('Idle Deadline', '<H=0'), ('Idle Wait', '<H=0'), ('Priority', '<L=0'), ('Maximum Run Time', '<L=0'), ('Exit Code', '<L=0'), ('Status', '<L=0'), ('Flags', '<L=0'))

class TRIGGERS(Structure):
    structure = (('Trigger Size', '<H=0'), ('Reserved1', '<H=0'), ('Begin Year', '<H=0'), ('Begin Month', '<H=0'), ('Begin Day', '<H=0'), ('End Year', '<H=0'), ('End Month', '<H=0'), ('End Day', '<H=0'), ('Start Hour', '<H=0'), ('Start Minute', '<H=0'), ('Minutes Duration', '<L=0'), ('Minutes Interval', '<L=0'), ('Flags', '<L=0'), ('Trigger Type', '<L=0'), ('TriggerSpecific0', '<H=0'), ('TriggerSpecific1', '<H=0'), ('TriggerSpecific2', '<H=0'), ('Padding', '<H=0'), ('Reserved2', '<H=0'), ('Reserved3', '<H=0'))

class WEEKLY(Structure):
    structure = (('Trigger Type', '<L=0'), ('Weeks Interval', '<H=0'), ('DaysOfTheWeek', '<H=0'), ('Unused', '<H=0'), ('Padding', '<H=0'))

class MONTHLYDATE(Structure):
    structure = (('Trigger Type', '<L=0'), ('Days', '<L=0'), ('Months', '<H=0'), ('Padding', '<H=0'))

class MONTHLYDOW(Structure):
    structure = (('Trigger Type', '<L=0'), ('WhichWeek', '<H=0'), ('DaysOfTheWeek', '<H=0'), ('Months', '<H=0'), ('Padding', '<H=0'), ('Reserved2', '<H=0'), ('Reserved3', '<H=0'))

class JOB_SIGNATURE(Structure):
    structure = (('SignatureVersion', '<HH0'), ('MinClientVersion', '<H=0'), ('Signature', '64s="'))
...
class SchRpcHighestVersionResponse(NDRCALL):
    structure = (('pVersion', DWORD), ('ErrorCode', ULONG))

class SchRpcRegisterTask(NDRCALL):
    opnum = 1
    structure = (('path', LPWSTR), ('xml', WSTR), ('flags', DWORD), ('sddl', LPWSTR), ('logonType', DWORD), ('cCreds', DWORD), ('pCreds', LPTASK_USER_CRED_ARRAY))

class SchRpcRegisterTaskResponse(NDRCALL):
    structure = (('pActualPath', LPWSTR), ('pErrorInfo', PTASK_XML_ERROR_INFO), ('ErrorCode', ULONG))

class SchRpcRetrieveTask(NDRCALL):
    opnum = 2
    structure = (('path', WSTR), ('lpcwszLanguagesBuffer', WSTR), ('pulNumLanguages', DWORD))

class SchRpcRetrieveTaskResponse(NDRCALL):
    structure = (('pXml', LPWSTR), ('ErrorCode', ULONG))

class SchRpcCreateFolder(NDRCALL):
    opnum = 3
    structure = (('path', WSTR), ('sddl', LPWSTR), ('flags', DWORD))
...
class SchRpcSetSecurity(NDRCALL):
    opnum = 4
    structure = (('path', WSTR), ('sddl', WSTR), ('flags', DWORD))
...
class SchRpcGetSecurity(NDRCALL):
    opnum = 5
    structure = (('path', WSTR), ('securityInformation', DWORD))

class SchRpcGetSecurityResponse(NDRCALL):
    structure = (('sddl', LPWSTR), ('ErrorCode', ULONG))

class SchRpcEnumFolders(NDRCALL):
    opnum = 6
    structure = (('path', WSTR), ('flags', DWORD), ('startIndex', DWORD), ('cRequested', DWORD))

class SchRpcEnumFoldersResponse(NDRCALL):
    structure = (('startIndex', DWORD), ('pcNames', DWORD), ('pNames', PTASK_NAMES_ARRAY), ('ErrorCode', ULONG))
...
class SchRpcEnumInstances(NDRCALL):
    opnum = 8
    structure = (('path', LPWSTR), ('flags', DWORD))

class SchRpcEnumInstancesResponse(NDRCALL):
    structure = (('pcGuids', DWORD), ('pGuids', PGUID_ARRAY), ('ErrorCode', ULONG))
...
class SchRpcGetInstanceInfoResponse(NDRCALL):
    structure = (('pPath', LPWSTR), ('pState', DWORD), ('pCurrentAction', LPWSTR), ('pInfo', LPWSTR), ('pcGroupInstances', DWORD), ('pGroupInstances', PGUID_ARRAY), ('pEnginePID', DWORD), ('ErrorCode', ULONG))

class SchRpcStopInstance(NDRCALL):
    opnum = 10
    structure = (('guid', GUID), ('flags', DWORD))
...
class SchRpcRun(NDRCALL):
    opnum = 12
    structure = (('path', WSTR), ('cArgs', DWORD), ('pArgs', PWSTR_ARRAY), ('flags', DWORD), ('sessionId', DWORD), ('user', LPWSTR))

class SchRpcRunResponse(NDRCALL):
    structure = (('pGuid', GUID), ('ErrorCode', ULONG))

class SchRpcDelete(NDRCALL):
    opnum = 13
    structure = (('path', WSTR), ('flags', DWORD))
...
class SchRpcRename(NDRCALL):
    opnum = 14
    structure = (('path', WSTR), ('newName', WSTR), ('flags', DWORD))
...
class SchRpcScheduledRuntimes(NDRCALL):
    opnum = 15
    structure = (('path', WSTR), ('start', PSYSTEMTIME), ('end', PSYSTEMTIME), ('flags', DWORD), ('cRequested', DWORD))

class SchRpcScheduledRuntimesResponse(NDRCALL):
    structure = (('pcRuntimes', DWORD), ('pRuntimes', PSYSTEMTIME_ARRAY), ('ErrorCode', ULONG))
...
class SchRpcGetLastRunInfoResponse(NDRCALL):
    structure = (('pLastRuntime', SYSTEMTIME), ('pLastReturnCode', DWORD), ('ErrorCode', ULONG))
...
class SchRpcGetTaskInfoResponse(NDRCALL):
    structure = (('pEnabled', DWORD), ('pState', DWORD), ('ErrorCode', ULONG))
...
class SchRpcGetNumberOfMissedRunsResponse(NDRCALL):
    structure = (('pNumberOfMissedRuns', DWORD), ('ErrorCode', ULONG))

class SchRpcEnableTask(NDRCALL):
    opnum = 19
    structure = (('path', WSTR), ('enabled', DWORD))

class SchRpcEnableTaskResponse(NDRCALL):
    structure = (('ErrorCode', ULONG),)
OPNUMS = {0: (SchRpcHighestVersion, SchRpcHighestVersionResponse), 1: (SchRpcRegisterTask, SchRpcRegisterTaskResponse), 2: (SchRpcRetrieveTask, SchRpcRetrieveTaskResponse), 3: (SchRpcCreateFolder, SchRpcCreateFolderResponse), 4: (SchRpcSetSecurity, SchRpcSetSecurityResponse), 5: (SchRpcGetSecurity, SchRpcGetSecurityResponse), 6: (SchRpcEnumFolders, SchRpcEnumFoldersResponse), 7: (SchRpcEnumTasks, SchRpcEnumTasksResponse), 8: (SchRpcEnumInstances, SchRpcEnumInstancesResponse), 9: (SchRpcGetInstanceInfo, SchRpcGetInstanceInfoResponse), 10: (SchRpcStopInstance, SchRpcStopInstanceResponse), 11: (SchRpcStop, SchRpcStopResponse), 12: (SchRpcRun, SchRpcRunResponse), 13: (SchRpcDelete, SchRpcDeleteResponse), 14: (SchRpcRename, SchRpcRenameResponse), 15: (SchRpcScheduledRuntimes, SchRpcScheduledRuntimesResponse), 16: (SchRpcGetLastRunInfo, SchRpcGetLastRunInfoResponse), 17: (SchRpcGetTaskInfo, SchRpcGetTaskInfoResponse), 18: (SchRpcGetNumberOfMissedRuns, SchRpcGetNumberOfMissedRunsResponse), 19: (SchRpcEnableTask, SchRpcEnableTaskResponse)}
