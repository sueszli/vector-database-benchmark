...
def test_set_encoder(vocab: List[str], embedding_size: int, representation: str, num_fc_layers: int, dropout: float):
    ...
    bag_encoder = BagEmbedWeightedEncoder(vocab=vocab, representation=representation, embedding_size=embedding_size, num_fc_layers=num_fc_layers, dropout=dropout).to(DEVICE)
    ...
    if dropout == 0:
        ...
        assert upc == tpc or upc == 0, f'Not all parameterss updated.  Parameters not updated: {not_updated}.\nModule: {bag_encoder}'