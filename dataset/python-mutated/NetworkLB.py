...
def main():
    ...
    template.set_description('AWS CloudFormation Sample Template: NLB with 1 EC2 instance')
    AddAMI(template)
    ...
    NetworkLB = template.add_resource(elb.LoadBalancer('NetworkLB', Name='NetworkLB', Scheme='internet-facing', SubnetMappings=[elb.SubnetMapping(AllocationId=GetAtt(eipA, 'AllocationId'), SubnetId=Ref(subnetA)), elb.SubnetMapping(AllocationId=GetAtt(eipB, 'AllocationId'), SubnetId=Ref(subnetB))], Type='network'))
    ...
    template.add_resource(elb.Listener('Listener', Port='80', Protocol='HTTP', LoadBalancerArn=Ref(NetworkLB), DefaultActions=[elb.Action(Type='forward', TargetGroupArn=Ref(TargetGroupWeb))]))
    template.add_output(Output('URL', Description='URL of the sample website', Value=Join('', ['http://', GetAtt(NetworkLB, 'DNSName')])))
