...
def coalesce_tensor_eager_api(Input, datatype=core.VarDesc.VarType.FP32, copy_data=False, set_constant=False, persist_output=False, constant=0.0, use_align=True, align_size=-1, user_defined_size_of_dtype=-1, concated_shapes=[], concated_ranks=[]):
    if False:
        while True:
            i = 10
    if datatype == int(core.VarDesc.VarType.FP32):
        datatype = core.VarDesc.VarType.FP32
    return paddle._C_ops.coalesce_tensor(Input, datatype, copy_data, set_constant, persist_output, constant, use_align, align_size, user_defined_size_of_dtype, concated_shapes, concated_ranks)
...
class TestAllocContinuousSpace(OpTest):
    ...
    def setUp(self):
        ...
        (self.Outputs, self.FusedOutput) = self.init_output(self.Inputs, self.attrs['set_constant'], self.attrs['constant'])
    ...
    def verify_output(self, place):
        ...
        with base.dygraph.base.guard(place=place):
            tensor_input = [base.dygraph.base.to_variable(value=data[1]) for data in self.inputs['Input']]
            (eager_outputs, eager_fused_output) = coalesce_tensor_eager_api(tensor_input, datatype=self.attrs['dtype'], copy_data=self.attrs['copy_data'] if 'copy_data' in self.attrs else False, set_constant=self.attrs['set_constant'] if 'set_constant' in self.attrs else False, persist_output=False, constant=self.attrs['constant'] if 'constant' in self.attrs else 0.0, use_align=True, align_size=-1, user_defined_size_of_dtype=self.attrs['user_defined_size_of_dtype'] if 'user_defined_size_of_dtype' in self.attrs else -1, concated_shapes=[], concated_ranks=[])
            for (idx, (expected, eager_output)) in enumerate(zip(self.outputs['Output'], eager_outputs)):
                np.testing.assert_allclose(expected[1], eager_output, atol=1e-05, err_msg=f'not equal {idx}')
            np.testing.assert_allclose(self.outputs['FusedOutput'], eager_fused_output, atol=1e-05, err_msg='not equal fusedoutput')

    def test_check_output(self):
        if False:
            return 10
        self.check_output_with_place(place=core.CUDAPlace(0), no_check_set=['FusedOutput'], atol=1e-05, check_dygraph=False)
