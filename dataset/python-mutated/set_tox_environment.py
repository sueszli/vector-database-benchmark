...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This script is used to resolve a set of arguments (that correspond to devops runtime variables) and determine which tox environments should be run for the current job. ' + 'When running against a specific service directory, attempts to find entire analysis steps that can be skipped. EG if pylint is disabled for every package in a given service directory, that ' + 'step should never actually run.')
    parser.add_argument('glob_string', nargs='?', help='A comma separated list of glob strings that will target the top level directories that contain packages. Examples: All == "azure-*", Single = "azure-keyvault"')
    parser.add_argument('-t', '--team-project', dest='team_project', help='', required=True)
    parser.add_argument('-o', '--override', dest='override_set', help='If you have a set of tox environments that should override the defaults, provide it here. In CI this is runtime variable $(Run.ToxCustomEnvs). EG: "whl,sdist".')
    parser.add_argument('-u', '--unsupported', dest='unsupported', help='A list of unsupported environments. EG: "pylint,sdist"')
    parser.add_argument('-s', '--service', dest='service', help='If provided, activates secondary capability of this script. If each package in service "A" skips check "pylint", that entire STEP will be skipped.' + 'If a check is only individually skipped, but still active in other packages of the service, then the step will run and the individual skip logic will apply.')
