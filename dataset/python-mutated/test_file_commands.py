...
class FileCommands(CommandTestCase):
    ...
    async def initialize_torrent(self, tx_to_update=None):
        ...
        if not tx_to_update:
            ...
            tx = await Transaction.claim_create('torrent', claim, 1, address, [self.account], self.account)
        else:
            claim = tx_to_update.outputs[0].claim
            claim.stream.update(bt_infohash=btih)
            tx = await Transaction.claim_update(tx_to_update.outputs[0], claim, 1, address, [self.account], self.account)
        ...
        self.client_session.wait_start = False
    ...
    async def test_file_reflect(self):
        ...
        all_except_sd = [blob_hash for blob_hash in self.server.blob_manager.completed_blob_hashes if blob_hash != sd_hash]
    ...
    async def test_tracker_discovery(self):
        ...
        self.assertEqual([{'address': '127.0.0.1', 'node_id': None, 'tcp_port': 5567, 'udp_port': None}, {'address': '127.0.0.1', 'node_id': None, 'tcp_port': 4444, 'udp_port': None}], (await self.daemon.jsonrpc_peer_list(sd_hash))['items'])
    ...
    async def _purge_file(self, claim_name, full_path):
        self.assertTrue(await self.daemon.jsonrpc_file_delete(claim_name=claim_name, delete_from_download_dir=True))
        self.assertItemCount(await self.daemon.jsonrpc_file_list(), 0)
    ...
    async def test_publish_with_illegal_chars(self):
        ...
        full_path = (await self.daemon.jsonrpc_get('lbry://' + claim_name, file_name=custom_name, save_file=True)).full_path
    ...
    async def test_file_list_fields(self):
        ...
        self.assertEqual(file_list[0]['timestamp'], self.ledger.headers.estimated_timestamp(file_list[0]['height']))
    ...
    async def test_incomplete_downloads_erases_output_file_on_stop(self):
        ...
        sd_hash = tx['outputs'][0]['value']['source']['sd_hash']
        file_info = (await self.file_list())[0]
        blobs = await self.daemon.storage.get_blobs_for_stream(await self.daemon.storage.get_stream_hash_for_sd_hash(sd_hash))
        await self.daemon.jsonrpc_file_delete(claim_name='foo')
        self.assertEqual(5, len(blobs))
        all_except_sd_and_head = [blob.blob_hash for blob in blobs[1:-1]]
    ...
    async def test_paid_download(self):
        ...
        await self.stream_create('expensive', '0.01', data=b'pay me if you can', fee_currency='LBC', fee_amount='11.0', fee_address=target_address, claim_address=target_address)
        ...
        self.assertItemCount(await self.daemon.jsonrpc_file_list(), 0)
        await self.stream_create('maxkey', '0.01', data=b'no pay me, no', fee_currency='LBC', fee_amount='111.0', fee_address=target_address, claim_address=target_address)
        await self.daemon.jsonrpc_file_delete(claim_name='maxkey')
        response = await self.out(self.daemon.jsonrpc_get('lbry://maxkey'))
        self.assertItemCount(await self.daemon.jsonrpc_file_list(), 0)
        self.assertEqual(response['error'], 'Purchase price of 111.0 LBC exceeds maximum configured price of 100.0 LBC (50.0 USD).')
        await self.stream_create('icanpay', '0.01', data=b'I got the power!', fee_currency='LBC', fee_amount='1.0', fee_address=target_address, claim_address=target_address)
        ...
        self.assertEqual(float(await self.blockchain.get_balance()), starting_balance + block_reward_and_claim_fee)
        await self.daemon.file_manager.stop()
        await self.daemon.file_manager.start()
        self.assertItemCount(await self.daemon.jsonrpc_file_list(), 1)
        self.assertEqual((await self.daemon.jsonrpc_file_list())['items'][0].content_fee.raw, raw_content_fee)
        await self.daemon.jsonrpc_file_delete(claim_name='icanpay')
        tx = await self.stream_create('nofeeaddress', '0.01', data=b'free stuff?')
        await self.__raw_value_update_no_fee_address(tx, fee_amount='2.0', fee_currency='LBC', claim_address=target_address)
    ...
    async def test_null_max_key_fee(self):
        ...
        await self.stream_create('somename', '0.5', data=b'Yes, please', fee_currency='LBC', fee_amount='1.0', fee_address=target_address, claim_address=target_address)
    ...
    async def test_null_fee(self):
        target_address = await self.blockchain.get_raw_change_address()
        tx = await self.stream_create('nullfee', '0.01', data=b'no pay me, no', fee_currency='LBC', fee_address=target_address, fee_amount='1.0')
        ...
        self.assertDictEqual(response.stream_claim_info.claim.stream.to_dict()['fee'], {'currency': 'LBC', 'address': target_address})
        await self.daemon.jsonrpc_file_delete(claim_name='nullfee')

    async def __raw_value_update_no_fee_address(self, tx, claim_address, **kwargs):
        tx = await self.daemon.jsonrpc_stream_update(self.get_claim_id(tx), preview=True, claim_address=claim_address, **kwargs)
    ...
    async def __raw_value_update_no_fee_amount(self, tx, claim_address):
        tx = await self.daemon.jsonrpc_stream_update(self.get_claim_id(tx), preview=True, fee_currency='LBC', fee_amount='1.0', fee_address=claim_address, claim_address=claim_address)
...
class DiskSpaceManagement(CommandTestCase):
    ...
    async def get_referenced_blobs(self, tx):
        ...
        return (tx['outputs'][0]['value']['source']['sd_hash'], set(await self.blob_list(stream_hash=stream_hash)))
    ...
    async def test_file_management(self):
        ...
        (sd_hash1, blobs1) = await self.get_referenced_blobs(await self.stream_create('foo1', '0.01', data=('0' * 2 * 1024 * 1024).encode()))
        (sd_hash2, blobs2) = await self.get_referenced_blobs(await self.stream_create('foo2', '0.01', data=('0' * 3 * 1024 * 1024).encode()))
        (sd_hash3, blobs3) = await self.get_referenced_blobs(await self.stream_create('foo3', '0.01', data=('0' * 3 * 1024 * 1024).encode()))
        (sd_hash4, blobs4) = await self.get_referenced_blobs(await self.stream_create('foo4', '0.01', data=('0' * 2 * 1024 * 1024).encode()))
...
class TestBackgroundDownloaderComponent(CommandTestCase):
    ...
    async def get_blobs_from_sd_blob(self, sd_blob):
        descriptor = await StreamDescriptor.from_stream_descriptor_blob(asyncio.get_running_loop(), self.daemon.blob_manager.blob_dir, sd_blob)
    ...
    async def test_download(self):
        ...
        await background_downloader.download_blobs(content1)
        await self.daemon.storage.db.execute_fetchall('update blob set added_on=0')
