...
class SerializedDagModel(Base):
    ...
    * ``[core] compress_serialized_dags``:
    ...
    dag_runs = relationship(DagRun, primaryjoin=dag_id == foreign(DagRun.dag_id), backref=backref('serialized_dag', uselist=False, innerjoin=True))
    dag_model = relationship(DagModel, primaryjoin=dag_id == DagModel.dag_id, foreign_keys=dag_id, uselist=False, innerjoin=True, backref=backref('serialized_dag', uselist=False, innerjoin=True))
    load_op_links = True
