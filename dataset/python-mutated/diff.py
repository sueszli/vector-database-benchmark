...
class GitDiffSheet(GitSheet):
    columns = [ItemColumn('a_fn', width=0), ItemColumn('fn', 'b_fn', width=30, hoffset=-28), ItemColumn('a_lineno', type=int, width=0), ItemColumn('lineno', 'b_lineno', type=int, width=8), Column('count', width=10, getter=lambda c, r: c.sheet.hunkCount(r)), ItemColumn('context'), ItemColumn('lines', type=''.join)]
...
class HunkViewer(GitSheet):
    colorizers = [RowColorizer(4, 'color_git_hunk_add', lambda s, c, r, v: r and r.old != r.new and (r.old is None)), RowColorizer(4, 'color_git_hunk_del', lambda s, c, r, v: r and r.old != r.new and (r.new is None)), RowColorizer(5, 'color_git_hunk_diff', lambda s, c, r, v: r and r.old != r.new and (r.new is not None) and (r.old is not None))]
    columns = [ItemColumn('1', 'old', width=40), ItemColumn('2', 'new', width=40)]
...
vd.addGlobals(GitDiffSheet=GitDiffSheet, HunkViewer=HunkViewer)