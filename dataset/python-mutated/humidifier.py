...
from homeassistant.components.humidifier import ATTR_ACTION, ATTR_CURRENT_HUMIDITY, ATTR_HUMIDITY, ATTR_MODE, DEFAULT_MAX_HUMIDITY, DEFAULT_MIN_HUMIDITY, HumidifierAction, HumidifierDeviceClass, HumidifierEntity, HumidifierEntityFeature
from homeassistant.config_entries import ConfigEntry
from homeassistant.const import CONF_NAME, CONF_OPTIMISTIC, CONF_PAYLOAD_OFF, CONF_PAYLOAD_ON, CONF_STATE
...
from .const import CONF_ACTION_TEMPLATE, CONF_ACTION_TOPIC, CONF_COMMAND_TEMPLATE, CONF_COMMAND_TOPIC, CONF_CURRENT_HUMIDITY_TEMPLATE, CONF_CURRENT_HUMIDITY_TOPIC, CONF_ENCODING, CONF_QOS, CONF_RETAIN, CONF_STATE_TOPIC, CONF_STATE_VALUE_TEMPLATE, PAYLOAD_NONE
from .debug_info import log_messages
from .mixins import MQTT_ENTITY_COMMON_SCHEMA, MqttEntity, async_setup_entity_entry_helper, write_state_on_attr_change
from .models import MqttCommandTemplate, MqttValueTemplate, PublishPayloadType, ReceiveMessage, ReceivePayloadType
...
MQTT_HUMIDIFIER_ATTRIBUTES_BLOCKED = frozenset({humidifier.ATTR_HUMIDITY, humidifier.ATTR_MAX_HUMIDITY, humidifier.ATTR_MIN_HUMIDITY, humidifier.ATTR_MODE, humidifier.ATTR_AVAILABLE_MODES})
...
def valid_humidity_range_configuration(config: ConfigType) -> ConfigType:
    ...
    return config
_PLATFORM_SCHEMA_BASE = MQTT_RW_SCHEMA.extend({vol.Optional(CONF_ACTION_TEMPLATE): cv.template, vol.Optional(CONF_ACTION_TOPIC): valid_subscribe_topic, vol.Inclusive(CONF_AVAILABLE_MODES_LIST, 'available_modes', default=[]): cv.ensure_list, vol.Inclusive(CONF_MODE_COMMAND_TOPIC, 'available_modes'): valid_publish_topic, vol.Optional(CONF_COMMAND_TEMPLATE): cv.template, vol.Optional(CONF_CURRENT_HUMIDITY_TEMPLATE): cv.template, vol.Optional(CONF_CURRENT_HUMIDITY_TOPIC): valid_subscribe_topic, vol.Optional(CONF_DEVICE_CLASS, default=HumidifierDeviceClass.HUMIDIFIER): vol.In([HumidifierDeviceClass.HUMIDIFIER, HumidifierDeviceClass.DEHUMIDIFIER, None]), vol.Optional(CONF_MODE_COMMAND_TEMPLATE): cv.template, vol.Optional(CONF_MODE_STATE_TOPIC): valid_subscribe_topic, vol.Optional(CONF_MODE_STATE_TEMPLATE): cv.template, vol.Optional(CONF_NAME): vol.Any(cv.string, None), vol.Optional(CONF_PAYLOAD_OFF, default=DEFAULT_PAYLOAD_OFF): cv.string, vol.Optional(CONF_PAYLOAD_ON, default=DEFAULT_PAYLOAD_ON): cv.string, vol.Optional(CONF_STATE_VALUE_TEMPLATE): cv.template, vol.Required(CONF_TARGET_HUMIDITY_COMMAND_TOPIC): valid_publish_topic, vol.Optional(CONF_TARGET_HUMIDITY_COMMAND_TEMPLATE): cv.template, vol.Optional(CONF_TARGET_HUMIDITY_MAX, default=DEFAULT_MAX_HUMIDITY): cv.positive_int, vol.Optional(CONF_TARGET_HUMIDITY_MIN, default=DEFAULT_MIN_HUMIDITY): cv.positive_int, vol.Optional(CONF_TARGET_HUMIDITY_STATE_TEMPLATE): cv.template, vol.Optional(CONF_TARGET_HUMIDITY_STATE_TOPIC): valid_subscribe_topic, vol.Optional(CONF_PAYLOAD_RESET_HUMIDITY, default=DEFAULT_PAYLOAD_RESET): cv.string, vol.Optional(CONF_PAYLOAD_RESET_MODE, default=DEFAULT_PAYLOAD_RESET): cv.string}).extend(MQTT_ENTITY_COMMON_SCHEMA.schema)
PLATFORM_SCHEMA_MODERN = vol.All(_PLATFORM_SCHEMA_BASE, valid_humidity_range_configuration, valid_mode_configuration)
DISCOVERY_SCHEMA = vol.All(_PLATFORM_SCHEMA_BASE.extend({}, extra=vol.REMOVE_EXTRA), valid_humidity_range_configuration, valid_mode_configuration)
TOPICS = (CONF_ACTION_TOPIC, CONF_STATE_TOPIC, CONF_COMMAND_TOPIC, CONF_CURRENT_HUMIDITY_TOPIC, CONF_TARGET_HUMIDITY_STATE_TOPIC, CONF_TARGET_HUMIDITY_COMMAND_TOPIC, CONF_MODE_STATE_TOPIC, CONF_MODE_COMMAND_TOPIC)

async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    """Set up MQTT humidifier through YAML and through MQTT discovery."""
    await async_setup_entity_entry_helper(hass, config_entry, MqttHumidifier, humidifier.DOMAIN, async_add_entities, DISCOVERY_SCHEMA, PLATFORM_SCHEMA_MODERN)
...
class MqttHumidifier(MqttEntity, HumidifierEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        await self.async_publish(self._topic[CONF_COMMAND_TOPIC], mqtt_payload, self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
        if self._optimistic:
            self._attr_is_on = True
    ...
    async def async_set_humidity(self, humidity: int) -> None:
        ...
        await self.async_publish(self._topic[CONF_TARGET_HUMIDITY_COMMAND_TOPIC], mqtt_payload, self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
    ...
    async def async_set_mode(self, mode: str) -> None:
        ...
        await self.async_publish(self._topic[CONF_MODE_COMMAND_TOPIC], mqtt_payload, self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
