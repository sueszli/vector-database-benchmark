...
_SPATIAL_BIAS = 0
_DEFAULT_PARAMS = {'size': _SIZE, 'horizon': _HORIZON, 'dt': _DELTA_T, 'n_actions_per_side': _N_ACTIONS_PER_SIDE, 'volatility': _VOLATILITY, 'mean_revert': _MEAN_REVERT, 'cross_q': _CROSS_Q, 'kappa': _KAPPA, 'terminal_cost': _TERMINAL_COST, 'spatial_bias': _SPATIAL_BIAS}
_GAME_TYPE = pyspiel.GameType(short_name='mean_field_lin_quad', long_name='Mean-Field Linear Quadratic Game', dynamics=pyspiel.GameType.Dynamics.MEAN_FIELD, chance_mode=pyspiel.GameType.ChanceMode.EXPLICIT_STOCHASTIC, information=pyspiel.GameType.Information.PERFECT_INFORMATION, utility=pyspiel.GameType.Utility.GENERAL_SUM, reward_model=pyspiel.GameType.RewardModel.REWARDS, max_num_players=_NUM_PLAYERS, min_num_players=_NUM_PLAYERS, provides_information_state_string=True, provides_information_state_tensor=False, provides_observation_string=True, provides_observation_tensor=True, parameter_specification=_DEFAULT_PARAMS)
