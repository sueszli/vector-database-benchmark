from importlib import machinery
import sys
import types
import unittest
import warnings
from .. import util

class SpecLoaderMock:

    def find_spec(self, fullname, path=None, target=None):
        if False:
            for i in range(10):
                print('nop')
        return machinery.ModuleSpec(fullname, self)

    def create_module(self, spec):
        if False:
            for i in range(10):
                print('nop')
        return None

    def exec_module(self, module):
        if False:
            while True:
                i = 10
        pass

class SpecLoaderAttributeTests:

    def test___loader__(self):
        if False:
            print('Hello World!')
        loader = SpecLoaderMock()
        with util.uncache('blah'), util.import_state(meta_path=[loader]):
            module = self.__import__('blah')
        self.assertEqual(loader, module.__loader__)
(Frozen_SpecTests, Source_SpecTests) = util.test_both(SpecLoaderAttributeTests, __import__=util.__import__)

class LoaderMock:

    def find_module(self, fullname, path=None):
        if False:
            print('Hello World!')
        return self

    def load_module(self, fullname):
        if False:
            return 10
        sys.modules[fullname] = self.module
        return self.module

class LoaderAttributeTests:

    def test___loader___missing(self):
        if False:
            print('Hello World!')
        with warnings.catch_warnings():
            warnings.simplefilter('ignore', ImportWarning)
            module = types.ModuleType('blah')
            try:
                del module.__loader__
            except AttributeError:
                pass
            loader = LoaderMock()
            loader.module = module
            with util.uncache('blah'), util.import_state(meta_path=[loader]):
                module = self.__import__('blah')
            self.assertEqual(loader, module.__loader__)

    def test___loader___is_None(self):
        if False:
            i = 10
            return i + 15
        with warnings.catch_warnings():
            warnings.simplefilter('ignore', ImportWarning)
            module = types.ModuleType('blah')
            module.__loader__ = None
            loader = LoaderMock()
            loader.module = module
            with util.uncache('blah'), util.import_state(meta_path=[loader]):
                returned_module = self.__import__('blah')
            self.assertEqual(loader, module.__loader__)
(Frozen_Tests, Source_Tests) = util.test_both(LoaderAttributeTests, __import__=util.__import__)
if __name__ == '__main__':
    unittest.main()