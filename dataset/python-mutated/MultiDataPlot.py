...
values = {'None (replaced by integer indices)': None, 'Single curve values': sortedRandint(0, 20, 15), 'container of (optionally) mixed-size curve values': [sortedRandint(0, 20, 15), *[sortedRandint(0, 20, 15) for _ in range(4)]], '2D matrix': np.row_stack([sortedRandint(20, 40, 15) for _ in range(6)])}
...
topParam = interact(next_plot, symbolBrush=dict(type='color'), symbol=dict(type='list', values=name_list), xtype=xytype, ytype=xytype)
tree = ParameterTree()
