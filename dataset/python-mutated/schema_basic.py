...
from homeassistant.components.light import ATTR_BRIGHTNESS, ATTR_COLOR_MODE, ATTR_COLOR_TEMP, ATTR_EFFECT, ATTR_EFFECT_LIST, ATTR_HS_COLOR, ATTR_MAX_MIREDS, ATTR_MIN_MIREDS, ATTR_RGB_COLOR, ATTR_RGBW_COLOR, ATTR_RGBWW_COLOR, ATTR_SUPPORTED_COLOR_MODES, ATTR_WHITE, ATTR_XY_COLOR, ENTITY_ID_FORMAT, ColorMode, LightEntity, LightEntityFeature, valid_supported_color_modes
...
from ..const import CONF_COMMAND_TOPIC, CONF_ENCODING, CONF_QOS, CONF_RETAIN, CONF_STATE_TOPIC, CONF_STATE_VALUE_TEMPLATE, PAYLOAD_NONE
from ..debug_info import log_messages
from ..mixins import MQTT_ENTITY_COMMON_SCHEMA, MqttEntity, write_state_on_attr_change
from ..models import MessageCallbackType, MqttCommandTemplate, MqttValueTemplate, PayloadSentinel, PublishPayloadType, ReceiveMessage, ReceivePayloadType, TemplateVarsType
...
MQTT_LIGHT_ATTRIBUTES_BLOCKED = frozenset({ATTR_COLOR_MODE, ATTR_BRIGHTNESS, ATTR_COLOR_TEMP, ATTR_EFFECT, ATTR_EFFECT_LIST, ATTR_HS_COLOR, ATTR_MAX_MIREDS, ATTR_MIN_MIREDS, ATTR_RGB_COLOR, ATTR_RGBW_COLOR, ATTR_RGBWW_COLOR, ATTR_SUPPORTED_COLOR_MODES, ATTR_XY_COLOR})
...
class MqttLight(MqttEntity, LightEntity, RestoreEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        async def publish(topic: str, payload: PublishPayloadType) -> None:
            ...
            await self.async_publish(str(self._topic[topic]), payload, self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
        ...
        if ATTR_BRIGHTNESS in kwargs and self._topic[CONF_BRIGHTNESS_COMMAND_TOPIC] is not None:
            ...
            await publish(CONF_RGBWW_COMMAND_TOPIC, rgbww_s)
            should_update |= set_optimistic(ATTR_BRIGHTNESS, kwargs[ATTR_BRIGHTNESS])
        if ATTR_COLOR_TEMP in kwargs and self._topic[CONF_COLOR_TEMP_COMMAND_TOPIC] is not None:
            ...
            should_update |= set_optimistic(ATTR_COLOR_TEMP, kwargs[ATTR_COLOR_TEMP], ColorMode.COLOR_TEMP)
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        This method is a coroutine.
        """
        await self.async_publish(str(self._topic[CONF_COMMAND_TOPIC]), self._payload['off'], self._config[CONF_QOS], self._config[CONF_RETAIN], self._config[CONF_ENCODING])
        if self._optimistic:
            self._attr_is_on = False
            self.async_write_ha_state()