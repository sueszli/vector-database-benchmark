...
from typing import TYPE_CHECKING, Any, Callable, Generic, Optional, Tuple, Type, TypeVar, Union
...
from prefect.settings import PREFECT_DEFAULT_RESULT_STORAGE_BLOCK, PREFECT_LOCAL_STORAGE_PATH, PREFECT_RESULTS_DEFAULT_SERIALIZER, PREFECT_RESULTS_PERSIST_BY_DEFAULT
...
async def get_default_result_storage() -> ResultStorage:
    ...
    """
    return await Block.load(PREFECT_DEFAULT_RESULT_STORAGE_BLOCK.value()) if PREFECT_DEFAULT_RESULT_STORAGE_BLOCK.value() is not None else LocalFileSystem(basepath=PREFECT_LOCAL_STORAGE_PATH.value())
...
class ResultFactory(pydantic.BaseModel):
    ...
    @classmethod
    @inject_client
    async def from_flow(cls: Type[Self], flow: 'Flow', client: 'PrefectClient'=None) -> Self:
        """
        ...
        if ctx:
            return await cls.from_settings(result_storage=flow.result_storage or ctx.result_factory.storage_block, result_serializer=flow.result_serializer or ctx.result_factory.serializer, persist_result=flow.persist_result if flow.persist_result is not None else flow_features_require_result_persistence(flow) or flow_features_require_child_result_persistence(ctx.flow) or get_default_persist_setting(), cache_result_in_memory=flow.cache_result_in_memory, storage_key_fn=DEFAULT_STORAGE_KEY_FN, client=client)
        else:
            return await cls.default_factory(client=client, result_storage=flow.result_storage, result_serializer=flow.result_serializer, persist_result=flow.persist_result if flow.persist_result is not None else flow_features_require_result_persistence(flow) or get_default_persist_setting(), cache_result_in_memory=flow.cache_result_in_memory, storage_key_fn=DEFAULT_STORAGE_KEY_FN)

    @classmethod
    @inject_client
    async def from_task(cls: Type[Self], task: 'Task', client: 'PrefectClient'=None) -> Self:
        """
        Create a new result factory for a task.
        """
        from prefect.context import FlowRunContext
        ctx = FlowRunContext.get()
        result_storage = task.result_storage or (ctx.result_factory.storage_block if ctx and ctx.result_factory else await get_default_result_storage())
        result_serializer = task.result_serializer or (ctx.result_factory.serializer if ctx and ctx.result_factory else get_default_result_serializer())
        persist_result = task.persist_result if task.persist_result is not None else (flow_features_require_child_result_persistence(ctx.flow) if ctx else False) or task_features_require_result_persistence(task) or get_default_persist_setting()
        cache_result_in_memory = task.cache_result_in_memory
        return await cls.from_settings(result_storage=result_storage, result_serializer=result_serializer, persist_result=persist_result, cache_result_in_memory=cache_result_in_memory, client=client, storage_key_fn=partial(_format_user_supplied_storage_key, task.result_storage_key) if task.result_storage_key is not None else DEFAULT_STORAGE_KEY_FN)

    @classmethod
    @inject_client
    async def from_settings(cls: Type[Self], result_storage: ResultStorage, result_serializer: ResultSerializer, persist_result: bool, cache_result_in_memory: bool, storage_key_fn: Callable[[], str], client: 'PrefectClient') -> Self:
        (storage_block_id, storage_block) = await cls.resolve_storage_block(result_storage, client=client, persist_result=persist_result)
        serializer = cls.resolve_serializer(result_serializer)
        return cls(storage_block=storage_block, storage_block_id=storage_block_id, serializer=serializer, persist_result=persist_result, cache_result_in_memory=cache_result_in_memory, storage_key_fn=storage_key_fn)

    @staticmethod
    async def resolve_storage_block(result_storage: ResultStorage, client: 'PrefectClient', persist_result: bool=True) -> Tuple[Optional[uuid.UUID], WritableFileSystem]:
        """
        ...
        if isinstance(result_storage, Block):
            ...
            if storage_block._block_document_id is not None:
                ...
                storage_block_id = await storage_block._save(is_anonymous=True, overwrite=True, client=client)
            else:
    ...
    async def create_result(self, obj: R) -> Union[R, 'BaseResult[R]']:
        ...
        return await PersistedResult.create(obj, storage_block=self.storage_block, storage_block_id=self.storage_block_id, storage_key_fn=self.storage_key_fn, serializer=self.serializer, cache_object=should_cache_object)
...
class BaseResult(pydantic.BaseModel, abc.ABC, Generic[R]):
    ...
    @sync_compatible
    async def create(cls: 'Type[BaseResult[R]]', obj: R, **kwargs: Any) -> 'BaseResult[R]':
        ...
...
class UnpersistedResult(BaseResult):
    ...
    @classmethod
    @sync_compatible
    async def create(cls: 'Type[UnpersistedResult]', obj: R, cache_object: bool=True) -> 'UnpersistedResult[R]':
        description = f'Unpersisted result of type `{type(obj).__name__}`'
        result = cls(artifact_type='result', artifact_description=description)
        if cache_object:
...
class LiteralResult(BaseResult):
    ...
    @classmethod
    @sync_compatible
    async def create(cls: 'Type[LiteralResult]', obj: R) -> 'LiteralResult[R]':
        if type(obj) not in LITERAL_TYPES:
            raise TypeError(f"Unsupported type {type(obj).__name__!r} for result literal. Expected one of: {', '.join((type_.__name__ for type_ in LITERAL_TYPES))}")
...
class PersistedResult(BaseResult):
    ...
    async def _read_blob(self, client: 'PrefectClient') -> 'PersistedResultBlob':
        assert self.storage_block_id is not None, 'Unexpected storage block ID. Was it persisted?'
    ...
    @classmethod
    @sync_compatible
    async def create(cls: 'Type[PersistedResult]', obj: R, storage_block: WritableFileSystem, storage_block_id: uuid.UUID, storage_key_fn: Callable[[], str], serializer: Serializer, cache_object: bool=True) -> 'PersistedResult[R]':
        """
        ...
        """
        assert storage_block_id is not None, 'Unexpected storage block ID. Was it persisted?'
        data = serializer.dumps(obj)
        blob = PersistedResultBlob(serializer=serializer, data=data)
        key = storage_key_fn()
        if not isinstance(key, str):
            raise TypeError(f"Expected type 'str' for result storage key; got value {key!r}")
        ...
        result = cls(serializer_type=serializer.type, storage_block_id=storage_block_id, storage_key=key, artifact_type='result', artifact_description=description)
        if cache_object:
            result._cache_object(obj)
