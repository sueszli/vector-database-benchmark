...
class EventReportsRestServlet(RestServlet):
    ...
    async def on_GET(self, request: SynapseRequest) -> Tuple[int, JsonDict]:
        ...
        if start < 0:
            raise SynapseError(HTTPStatus.BAD_REQUEST, 'The start parameter must be a positive integer.', errcode=Codes.INVALID_PARAM)
        if limit < 0:
            raise SynapseError(HTTPStatus.BAD_REQUEST, 'The limit parameter must be a positive integer.', errcode=Codes.INVALID_PARAM)
        (event_reports, total) = await self._store.get_event_reports_paginate(start, limit, direction, user_id, room_id)
...
class EventReportDetailRestServlet(RestServlet):
    ...
    async def on_GET(self, request: SynapseRequest, report_id: str) -> Tuple[int, JsonDict]:
        await assert_requester_is_admin(self._auth, request)
    ...
    async def on_DELETE(self, request: SynapseRequest, report_id: str) -> Tuple[int, JsonDict]:
        await assert_requester_is_admin(self._auth, request)
        message = 'The report_id parameter must be a string representing a positive integer.'
        try:
            resolved_report_id = int(report_id)
