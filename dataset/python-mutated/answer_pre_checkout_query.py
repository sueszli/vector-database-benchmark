...
class AnswerPreCheckoutQuery(TelegramMethod[bool]):
    ...
    if TYPE_CHECKING:
        ...
        def __init__(__pydantic__self__, *, pre_checkout_query_id: str, ok: bool, error_message: Optional[str]=None, **__pydantic_kwargs: Any) -> None:
            if False:
                for i in range(10):
                    print('nop')
            super().__init__(pre_checkout_query_id=pre_checkout_query_id, ok=ok, error_message=error_message, **__pydantic_kwargs)