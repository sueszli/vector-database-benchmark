...
from dizoo.tabmwp.envs.utils import create_example_from_pid, build_prompt, get_gpt3_output, calc_rwkv, calc_internlm, extract_prediction, normalize_answer, load_data
...
if __name__ == '__main__':
    ...
    env_cfg = EasyDict(dict(cand_number=16, train_number=20, engine='text-davinci-002', temperature=0.0, max_tokens=512, top_p=1.0, frequency_penalty=0.0, presence_penalty=0.0, option_inds=['A', 'B', 'C', 'D', 'E', 'F'], api_key='xxx', prompt_format='TQ-A', enable_replay=True, seed=0))
    env = TabMWP(env_cfg)
    env.seed(0)
    env.reset()
