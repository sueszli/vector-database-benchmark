...
class ErfInv(function_node.FunctionNode):
    ...
    def forward_cpu(self, x):
        ...
        if not available_cpu:
            raise ImportError('SciPy is not available. Forward computation of erfinv in CPU cannot be done. ' + str(_import_error))
    ...
    def forward_gpu(self, x):
        if False:
            return 10
        self.retain_outputs((0,))
        return (cuda.elementwise('T x', 'T y', 'y = erfinv(x)', 'elementwise_erfinv')(x[0]),)
