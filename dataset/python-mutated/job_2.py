...
trips_schema = pa.DataFrameSchema(columns={'bike_id': pa.Column(int, checks=pa.Check.ge(0)), 'start_time': pa.Column(pd.Timestamp, checks=pa.Check.ge(MIN_DATE)), 'end_time': pa.Column(pd.Timestamp, checks=pa.Check.ge(MIN_DATE))})
TripsDataFrame = pandera_schema_to_dagster_type(trips_schema, 'TripsDataFrame', 'DataFrame type for e-bike trips.')
...
def load_trips(context: OpExecutionContext):
    if False:
        for i in range(10):
            print('nop')
    df = pd.read_csv('./ebike_trips.csv', parse_dates=['start_time', 'end_time'])
