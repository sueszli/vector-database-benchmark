...
parser.add_argument('--version', action='version', version='%(prog)s Version 1.0', help='Print debug information.')
parser.add_argument('-v', '--View', dest='View', nargs='+', help="View each FV and the named files within each FV: '-v inputfile outputfile, inputfiletype(.Fd/.Fv/.ffs/.sec)'")
parser.add_argument('-d', '--Delete', dest='Delete', nargs='+', help="Delete a Ffs from FV: '-d inputfile TargetFvName(Optional) TargetFfsName outputfile                        If not given TargetFvName, all the existed target Ffs will be deleted'")
parser.add_argument('-e', '--Extract', dest='Extract', nargs='+', help="Extract a Ffs Info: '-e inputfile TargetFvName(Optional) TargetFfsName outputfile                        If not given TargetFvName, the first found target Ffs will be extracted.                        If only given TargetFvName, not given TargetFfsName, the TargetFv will be extracted to output file'")
parser.add_argument('-a', '--Add', dest='Add', nargs='+', help="Add a Ffs into a FV:'-a inputfile TargetFvName newffsfile outputfile'")
parser.add_argument('-r', '--Replace', dest='Replace', nargs='+', help="Replace a Ffs in a FV: '-r inputfile TargetFvName(Optional) TargetFfsName newffsfile outputfile                        If not given TargetFvName, all the existed target Ffs will be replaced with new Ffs file)'")
parser.add_argument('-l', '--LayoutFileName', dest='LayoutFileName', nargs='+', help="The output file which saves Binary layout: '-l xxx.txt'/'-l xxx.json'                        If only provide file format as 'txt',                         the file will be generated with default name (Layout_'InputFileName'.txt).                         Currently supports two formats: json, txt. More formats will be added in the future")
parser.add_argument('-c', '--ConfigFilePath', dest='ConfigFilePath', nargs='+', help="Provide the target FmmtConf.ini file path: '-c C:\\Code\\FmmtConf.ini'                         FmmtConf file saves the target guidtool used in compress/uncompress process.                        If do not provide, FMMT tool will search the inputfile folder for FmmtConf.ini firstly, if not found,                        the FmmtConf.ini saved in FMMT tool's folder will be used as default.")
parser.add_argument('-s', '--ShrinkFv', dest='ShrinkFv', nargs='+', help="Shrink the Fv file: '-s InputFvfile OutputFvfile")
