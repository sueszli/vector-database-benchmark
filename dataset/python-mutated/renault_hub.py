...
from homeassistant.const import ATTR_IDENTIFIERS, ATTR_MANUFACTURER, ATTR_MODEL, ATTR_NAME, ATTR_SW_VERSION
...
class RenaultHub:
    ...
    async def async_initialise(self, config_entry: ConfigEntry) -> None:
        ...
        if vehicles.vehicleLinks:
            if any((vehicle_link.vehicleDetails is None for vehicle_link in vehicles.vehicleLinks)):
                raise ConfigEntryNotReady('Failed to retrieve vehicle details from Renault servers')
            device_registry = dr.async_get(self._hass)
            await asyncio.gather(*(self.async_initialise_vehicle(vehicle_link, self._account, scan_interval, config_entry, device_registry) for vehicle_link in vehicles.vehicleLinks))
    ...
    async def async_initialise_vehicle(self, vehicle_link: KamereonVehiclesLink, renault_account: RenaultAccount, scan_interval: timedelta, config_entry: ConfigEntry, device_registry: dr.DeviceRegistry) -> None:
        ...
        vehicle = RenaultVehicleProxy(hass=self._hass, vehicle=await renault_account.get_api_vehicle(vehicle_link.vin), details=vehicle_link.vehicleDetails, scan_interval=scan_interval)
        await vehicle.async_initialise()
        device_registry.async_get_or_create(config_entry_id=config_entry.entry_id, identifiers=vehicle.device_info[ATTR_IDENTIFIERS], manufacturer=vehicle.device_info[ATTR_MANUFACTURER], name=vehicle.device_info[ATTR_NAME], model=vehicle.device_info[ATTR_MODEL], sw_version=vehicle.device_info[ATTR_SW_VERSION])
