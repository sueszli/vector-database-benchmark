...
from aioesphomeapi import COMPONENT_TYPE_TO_INFO, AlarmControlPanelInfo, APIClient, APIVersion, BinarySensorInfo, CameraInfo, CameraState, ClimateInfo, CoverInfo, DeviceInfo, EntityInfo, EntityState, FanInfo, LightInfo, LockInfo, MediaPlayerInfo, NumberInfo, SelectInfo, SensorInfo, SensorState, SwitchInfo, TextInfo, TextSensorInfo, UserService, build_unique_id
...
INFO_TYPE_TO_PLATFORM: dict[type[EntityInfo], Platform] = {AlarmControlPanelInfo: Platform.ALARM_CONTROL_PANEL, BinarySensorInfo: Platform.BINARY_SENSOR, ButtonInfo: Platform.BUTTON, CameraInfo: Platform.CAMERA, ClimateInfo: Platform.CLIMATE, CoverInfo: Platform.COVER, FanInfo: Platform.FAN, LightInfo: Platform.LIGHT, LockInfo: Platform.LOCK, MediaPlayerInfo: Platform.MEDIA_PLAYER, NumberInfo: Platform.NUMBER, SelectInfo: Platform.SELECT, SensorInfo: Platform.SENSOR, SwitchInfo: Platform.SWITCH, TextInfo: Platform.TEXT, TextSensorInfo: Platform.SENSOR}
...
class RuntimeEntryData:
    ...
    state_subscriptions: dict[tuple[type[EntityState], int], Callable[[], None]] = field(default_factory=dict)
    ...
    assist_pipeline_update_callbacks: list[Callable[[], None]] = field(default_factory=list)
    ...
    entity_info_key_remove_callbacks: dict[tuple[type[EntityInfo], int], list[Callable[[], Coroutine[Any, Any, None]]]] = field(default_factory=dict)
    ...
    async def async_update_static_infos(self, hass: HomeAssistant, entry: ConfigEntry, infos: list[EntityInfo], mac: str) -> None:
        ...
        for info in infos:
            ...
            if (old_unique_id := info.unique_id) and (old_entry := registry_get_entity(platform, DOMAIN, old_unique_id)) and ((new_unique_id := build_unique_id(mac, info)) != old_unique_id) and (not registry_get_entity(platform, DOMAIN, new_unique_id)):
    ...
    async def async_load_from_store(self) -> tuple[list[EntityInfo], list[UserService]]:
        ...
        services = [UserService.from_dict(service) for service in restored.pop('services', [])]
    ...
    async def async_save_to_store(self) -> None:
        ...
        store_data: StoreData = {'device_info': self.device_info.to_dict(), 'services': [], 'api_version': self.api_version.to_dict()}
        ...
        store_data['services'] = [service.to_dict() for service in self.services.values()]
