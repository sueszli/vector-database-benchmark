...
class Test_apigateway_restapi_logging_enabled:
    ...
    def test_apigateway_no_rest_apis(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.apigateway.apigateway_restapi_logging_enabled.apigateway_restapi_logging_enabled.apigateway_client', new=APIGateway(current_audit_info)):
            from prowler.providers.aws.services.apigateway.apigateway_restapi_logging_enabled.apigateway_restapi_logging_enabled import apigateway_restapi_logging_enabled
    ...
    def test_apigateway_one_rest_api_with_logging(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.apigateway.apigateway_restapi_logging_enabled.apigateway_restapi_logging_enabled.apigateway_client', new=APIGateway(current_audit_info)):
            ...
            assert len(result) == 1
            assert result[0].status_extended == f"API Gateway test-rest-api ID {rest_api['id']} in stage test has logging enabled."
    ...
    def test_apigateway_one_rest_api_without_logging(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.apigateway.apigateway_restapi_logging_enabled.apigateway_restapi_logging_enabled.apigateway_client', new=APIGateway(current_audit_info)):
            ...
            assert len(result) == 1
            assert result[0].status_extended == f"API Gateway test-rest-api ID {rest_api['id']} in stage test has logging disabled."
            assert result[0].resource_id == 'test-rest-api'
            assert result[0].resource_arn == f"arn:{current_audit_info.audited_partition}:apigateway:{AWS_REGION}::/restapis/{rest_api['id']}/stages/test"
            assert result[0].region == AWS_REGION
            assert result[0].resource_tags == [None]