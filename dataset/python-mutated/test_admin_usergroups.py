...
import os
import unittest
from integration_tests.env_variable_names import SLACK_SDK_TEST_GRID_WORKSPACE_ADMIN_USER_TOKEN, SLACK_SDK_TEST_GRID_ORG_ADMIN_USER_TOKEN, SLACK_SDK_TEST_GRID_IDP_USERGROUP_ID, SLACK_SDK_TEST_GRID_TEAM_ID
...
class TestWebClient(unittest.TestCase):
    ...
    def test_sync(self):
        ...
        list_channels = client.admin_usergroups_listChannels(team_id=self.team_id, usergroup_id=self.idp_usergroup_id)
        self.assertIsNotNone(list_channels)
        add_teams = client.admin_usergroups_addTeams(usergroup_id=self.idp_usergroup_id, team_ids=self.team_id)
        self.assertIsNotNone(add_teams)
        add_channels = client.admin_usergroups_addChannels(team_id=self.team_id, usergroup_id=self.idp_usergroup_id, channel_ids=self.channel_ids)
    ...
    async def test_async(self):
        ...
        list_channels = await client.admin_usergroups_listChannels(team_id=self.team_id, usergroup_id=self.idp_usergroup_id)
        self.assertIsNotNone(list_channels)
        add_teams = await client.admin_usergroups_addTeams(usergroup_id=self.idp_usergroup_id, team_ids=self.team_id)
        self.assertIsNotNone(add_teams)
        add_channels = await client.admin_usergroups_addChannels(team_id=self.team_id, usergroup_id=self.idp_usergroup_id, channel_ids=self.channel_ids)
        self.assertIsNotNone(add_channels)
