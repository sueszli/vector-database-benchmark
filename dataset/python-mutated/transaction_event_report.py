...
from .....payment.utils import authorization_success_already_exists, create_failed_transaction_event, get_already_existing_event
...
class TransactionEventReport(ModelMutation):
    already_processed = graphene.Boolean(description="Defines if the reported event hasn't been processed earlier.")
    transaction = graphene.Field(TransactionItem, description='The transaction related to the reported event.')
    transaction_event = graphene.Field(TransactionEvent, description='The event assigned to this report. if `alreadyProcessed` is set to `true`, the previously processed event will be returned.')

    class Arguments:
        id = graphene.ID(description='The ID of the transaction.', required=True)
        psp_reference = graphene.String(description='PSP Reference of the event to report.', required=True)
        type = graphene.Argument(TransactionEventTypeEnum, required=True, description='Current status of the event to report.')
        amount = PositiveDecimal(description='The amount of the event to report.', required=True)
        time = graphene.DateTime(description='The time of the event to report. If not provide, the current time will be used.')
        external_url = graphene.String(description='The url that will allow to redirect user to payment provider page with event details.')
        message = graphene.String(description='The message related to the event.')
        available_actions = graphene.List(graphene.NonNull(TransactionActionEnum), description='List of all possible actions for the transaction')

    class Meta:
        description = 'Report the event for the transaction.' + ADDED_IN_313 + PREVIEW_FEATURE + '\n\nRequires the following permissions: ' + f'{AuthorizationFilters.OWNER.name} ' + f'and {PaymentPermissions.HANDLE_PAYMENTS.name} for apps, {PaymentPermissions.HANDLE_PAYMENTS.name} for staff users. Staff user cannot update a transaction that is owned by the app.'
