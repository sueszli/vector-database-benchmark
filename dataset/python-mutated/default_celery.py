...
DEFAULT_CELERY_CONFIG = {'accept_content': ['json'], 'event_serializer': 'json', 'worker_prefetch_multiplier': conf.getint('celery', 'worker_prefetch_multiplier', fallback=1), 'task_acks_late': True, 'task_default_queue': conf.get('operators', 'DEFAULT_QUEUE'), 'task_default_exchange': conf.get('operators', 'DEFAULT_QUEUE'), 'task_track_started': conf.getboolean('celery', 'task_track_started', fallback=True), 'broker_url': broker_url, 'broker_transport_options': broker_transport_options_for_celery, 'result_backend': result_backend, 'database_engine_options': conf.getjson('celery', 'result_backend_sqlalchemy_engine_options', fallback={}), 'worker_concurrency': conf.getint('celery', 'WORKER_CONCURRENCY', fallback=16), 'worker_enable_remote_control': conf.getboolean('celery', 'worker_enable_remote_control', fallback=True)}
...
try:
    if celery_ssl_active:
        if broker_url and 'amqp://' in broker_url:
            broker_use_ssl = {'keyfile': conf.get('celery', 'SSL_KEY'), 'certfile': conf.get('celery', 'SSL_CERT'), 'ca_certs': conf.get('celery', 'SSL_CACERT'), 'cert_reqs': ssl.CERT_REQUIRED}
    ...
    raise AirflowException(f'Exception: There was an unknown Celery SSL Error. Please ensure you want to use SSL and/or have all necessary certs and key ({e}).')
match_not_recommended_backend = re2.search('rediss?://|amqp://|rpc://', result_backend)
if match_not_recommended_backend:
    log.warning('You have configured a result_backend using the protocol `%s`, it is highly recommended to use an alternative result_backend (i.e. a database).', match_not_recommended_backend.group(0).strip('://'))