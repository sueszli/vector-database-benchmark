...
def create_model(data, rank):
    ...
    if rank is not None:
        ...
        result = paddle.distributed.split(data, size=(IN_SIZE, OUT_SIZE), operation='linear', axis=1, num_partitions=MODEL_PARALLEL_SIZE, weight_attr=weight_attr, bias_attr=bias_attr)
    else:
        (weight_attr, bias_attr) = get_param_attr(np_weight, np_bias)
        result = paddle.static.nn.fc(data, size=OUT_SIZE, weight_attr=weight_attr, bias_attr=bias_attr)
