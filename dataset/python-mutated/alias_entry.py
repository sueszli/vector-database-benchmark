...
class AliasCache:
    ...
    async def get_alias(self, guild: Optional[discord.Guild], alias_name: str) -> Optional[AliasEntry]:
        ...
        if self._cache_enabled:
            ...
            if guild:
                server_aliases = [AliasEntry.from_json(d) for d in await self.config.guild(guild.id).entries()]
    ...
    async def add_alias(self, ctx: commands.Context, alias_name: str, command: str, global_: bool=False) -> AliasEntry:
    ...
    async def edit_alias(self, ctx: commands.Context, alias_name: str, command: str, global_: bool=False) -> bool:
        command = self.format_command_for_alias(command)
        if global_:
            settings = self.config
        else:
    ...
    async def delete_alias(self, ctx: commands.Context, alias_name: str, global_: bool=False) -> bool:
        if global_:
            settings = self.config
        else:
            settings = self.config.guild(ctx.guild)
