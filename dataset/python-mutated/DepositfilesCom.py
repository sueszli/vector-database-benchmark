...
class DepositfilesCom(SimpleDownloader):
    ...
    __version__ = '0.65'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?(depositfiles\\.com|dfiles\\.(eu|ru))(/\\w{1,3})?/files/(?P<ID>\\w+)'
    ...
    __license__ = 'GPLv3'
    __authors__ = [('spoob', 'spoob@pyload.net'), ('zoidberg', 'zoidberg@mujmail.cz'), ('Walter Purcaro', 'vuolter@gmail.com'), ('GammaC0de', 'nitzo2001[AT}yahoo[DOT]com')]
    NAME_PATTERN = '<script type="text/javascript">eval\\( unescape\\(\\\'(?P<N>.*?)\\\''
    SIZE_PATTERN = '>File size: <b>(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)</b>'
    OFFLINE_PATTERN = '<span class="html_download_api-not_exists"></span>'
    TEMP_OFFLINE_PATTERN = '^unmatchable$'
    NAME_REPLACEMENTS = [('\\%u([0-9A-Fa-f]{4})', lambda m: chr(int(m.group(1), 16))), ('.*<b title="(?P<N>.+?)".*', '\\g<N>')]
    URL_REPLACEMENTS = [(__pattern__ + '.*', 'https://depositfiles.com/files/\\g<ID>')]
    COOKIES = [('depositfiles.com', 'lang_current', 'en')]
    WAIT_PATTERN = '(?:download_waiter_remain">|html_download_api-limit_interval">|>Please wait|>Try in).+'
    ERROR_PATTER = 'File is checked, please try again in a minute'
    LINK_FREE_PATTERN = '<form id="downloader_file_form" action="(https?://.+?)" method="post"'
    LINK_PREMIUM_PATTERN = 'class="repeat"><a href="(.+?)"'
    LINK_MIRROR_PATTERN = 'class="repeat_mirror"><a href="(.+?)"'
