...
from ...utils import checkout_complete, checkout_create, checkout_delivery_method_update, checkout_dummy_payment_create, raw_checkout_add_promo_code

def prepare_voucher(e2e_staff_api_client, channel_id, voucher_code, voucher_discount_type, voucher_discount_value, voucher_type, start_date):
    if False:
        for i in range(10):
            print('nop')
    input = {'code': voucher_code, 'discountValueType': voucher_discount_type, 'type': voucher_type, 'usageLimit': 1, 'singleUse': True, 'startDate': start_date}
...
@pytest.mark.e2e
def test_checkout_unable_to_use_not_started_voucher_CORE_0920(e2e_not_logged_api_client, e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_shipping, permission_manage_product_types_and_attributes, permission_manage_discounts, permission_manage_checkouts):
    if False:
        i = 10
        return i + 15
    ...
    (voucher_code, voucher_id) = prepare_voucher(e2e_staff_api_client, channel_id, voucher_code='percentage_voucher', voucher_discount_type='PERCENTAGE', voucher_discount_value=15, voucher_type='ENTIRE_ORDER', start_date=date_in_the_future)
