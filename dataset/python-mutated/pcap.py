...
protocols = collections.defaultdict(dict)
_flags = collections.defaultdict(dict)
...
def load_oui(url):
    ...
    for r in vsoui.rows:
        if r.prefix.endswith('/36'):
            prefix = r.prefix[:13]
        elif r.prefix.endswith('/28'):
            prefix = r.prefix[:10]
        else:
            prefix = r.prefix[:8]
        try:
...
class TCPSheet(IPSheet):
    columns = IPSheet.columns + [ColumnAttr('tcp_srcport', 'ip.tcp.sport', type=int, width=8, helpstr='TCP Source Port'), ColumnAttr('tcp_dstport', 'ip.tcp.dport', type=int, width=8, helpstr='TCP Dest Port'), ColumnAttr('tcp_opts', 'ip.tcp.opts', width=0), ColumnAttr('tcp_flags', 'ip.tcp.flags', type=FlagGetter('tcp'), helpstr='TCP Flags')]
...
class UDPSheet(IPSheet):
    columns = IPSheet.columns + [ColumnAttr('udp_srcport', 'ip.udp.sport', type=int, width=8, helpstr='UDP Source Port'), ColumnAttr('udp_dstport', 'ip.udp.dport', type=int, width=8, helpstr='UDP Dest Port'), ColumnAttr('ip.udp.data', type=vlen, width=0), ColumnAttr('ip.udp.ulen', type=int, width=0)]
...
class PcapFlowsSheet(Sheet):
    rowtype = 'netflows'
    _rowtype = flowtype
    columns = [ColumnAttr('transport', type=str), Column('src', type=str, getter=lambda col, row: row.src), ColumnAttr('sport', type=int), Column('dst', type=str, getter=lambda col, row: row.dst), ColumnAttr('dport', type=int), Column('service', type=str, width=8, getter=lambda col, row: getService(getTuple(row.packets[0]))), ColumnAttr('packets', type=vlen), Column('connect_latency_ms', type=float, getter=lambda col, row: col.sheet.latency[getTuple(row.packets[0])])]
