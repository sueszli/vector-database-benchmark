...
from sentry.sentry_metrics.consumers.indexer.common import BatchMessages, IndexerOutputMessageBatch, MetricsBatchBuilder
from sentry.sentry_metrics.consumers.indexer.processing import MessageProcessor
from sentry.sentry_metrics.indexer.limiters.cardinality import TimeseriesCardinalityLimiter, cardinality_limiter_factory
from sentry.sentry_metrics.indexer.mock import MockIndexer, RawSimpleIndexer
from sentry.sentry_metrics.use_case_id_registry import UseCaseID
from sentry.utils import json
logger = logging.getLogger(__name__)
pytestmark = pytest.mark.sentry_metrics
MESSAGE_PROCESSOR = MessageProcessor(get_ingest_config(UseCaseKey.PERFORMANCE, IndexerStorage.POSTGRES))
...
counter_payloads: list[dict[str, Any]] = [{'name': f'c:{use_case.value}/alert@none', 'tags': {'environment': 'production', 'session.status': 'init'}, 'timestamp': ts, 'type': b'c', 'value': 1.0, 'org_id': 1, 'project_id': 3, 'retention_days': 90} for use_case in UseCaseID if use_case is not UseCaseID.SESSIONS]
distribution_payloads: list[dict[str, Any]] = [{'name': f'd:{use_case.value}/alert@none', 'tags': {'environment': 'production', 'session.status': 'healthy'}, 'timestamp': ts, 'type': b'd', 'value': [4, 5, 6], 'org_id': 1, 'project_id': 3, 'retention_days': 90} for use_case in UseCaseID if use_case is not UseCaseID.SESSIONS]
set_payloads: list[dict[str, Any]] = [{'name': f's:{use_case.value}/alert@none', 'tags': {'environment': 'production', 'session.status': 'errored'}, 'timestamp': ts, 'type': b's', 'value': [3], 'org_id': 1, 'project_id': 3, 'retention_days': 90} for use_case in UseCaseID if use_case is not UseCaseID.SESSIONS]

def __translated_payload(payload, indexer=None) -> Dict[str, Union[str, int, List[int], MutableMapping[int, int]]]:
    if False:
        for i in range(10):
            print('nop')
    ...
    new_tags = {indexer.resolve(use_case_id=use_case_id, org_id=org_id, string=k): v for (k, v) in payload['tags'].items()}
    agg_option = get_aggregation_option(payload['name'])
    if agg_option:
        payload['aggregation_option'] = agg_option
    payload['metric_id'] = indexer.resolve(use_case_id=use_case_id, org_id=org_id, string=payload['name'])
...
invalid_payloads = [({'name': 'c:transactions/alert@none', 'tags': {'environment' * 21: 'production', 'session.status': 'errored'}, 'timestamp': ts, 'type': 's', 'value': [3], 'org_id': 1, 'project_id': 3, 'retention_days': 90}, 'invalid_tags', True), ({'name': 'c:transactions/alert@none' * 21, 'tags': {'environment': 'production', 'session.status': 'errored'}, 'timestamp': ts, 'type': 's', 'value': [3], 'org_id': 1, 'project_id': 3, 'retention_days': 90}, 'invalid_metric_name', True), (b'invalid_json_payload', 'invalid_json', False)]
...
def test_process_messages_invalid_messages(invalid_payload, error_text, format_payload, caplog) -> None:
    ...
    message_batch = [Message(BrokerValue(KafkaPayload(None, json.dumps(counter_payloads[0]).encode('utf-8'), []), Partition(Topic('topic'), 0), 0, BROKER_TIMESTAMP)), Message(BrokerValue(KafkaPayload(None, formatted_payload, []), Partition(Topic('topic'), 0), 1, BROKER_TIMESTAMP))]
    last = message_batch[-1]
    outer_message = Message(Value(message_batch, last.committable))
    with caplog.at_level(logging.ERROR):
        new_batch = MESSAGE_PROCESSOR.process_messages(outer_message=outer_message)
    expected_msg = message_batch[0]
    expected_new_batch = [Message(Value(KafkaPayload(None, json.dumps(__translated_payload(counter_payloads[0])).encode('utf-8'), [('metric_type', b'c')]), expected_msg.committable)), Message(Value(InvalidMessage(Partition(Topic('topic'), 0), 1), message_batch[1].committable))]
    compare_message_batches_ignoring_metadata(new_batch, expected_new_batch)
...
def test_process_messages_rate_limited(caplog, settings) -> None:
    ...
    message_batch = [Message(BrokerValue(KafkaPayload(None, json.dumps(counter_payloads[0]).encode('utf-8'), []), Partition(Topic('topic'), 0), 0, BROKER_TIMESTAMP)), Message(BrokerValue(KafkaPayload(None, json.dumps(rate_limited_payload).encode('utf-8'), []), Partition(Topic('topic'), 0), 1, BROKER_TIMESTAMP))]
    last = message_batch[-1]
    outer_message = Message(Value(message_batch, last.committable))
    message_processor = MessageProcessor(get_ingest_config(UseCaseKey.PERFORMANCE, IndexerStorage.MOCK))
    ...
    expected_new_batch = [Message(BrokerValue(KafkaPayload(None, json.dumps(__translated_payload(counter_payloads[0], message_processor._indexer)).encode('utf-8'), [('metric_type', b'c')]), expected_msg.value.partition, expected_msg.value.offset, expected_msg.value.timestamp))]
    compare_message_batches_ignoring_metadata(new_batch, expected_new_batch)
...
def test_process_messages_cardinality_limited(caplog, settings, monkeypatch, set_sentry_option) -> None:
    ...
    settings.SENTRY_METRICS_INDEXER_DEBUG_LOG_SAMPLE_RATE = 1.0
    with set_sentry_option('sentry-metrics.cardinality-limiter.limits.performance.per-org', [{'window_seconds': 3600, 'granularity_seconds': 60, 'limit': 0}]), set_sentry_option('sentry-metrics.cardinality-limiter.orgs-rollout-rate', 1.0):
            ...
            def use_quotas(self, grants, timestamp):
                if False:
                    while True:
                        i = 10
                pass
        monkeypatch.setitem(cardinality_limiter_factory.rate_limiters, 'performance', TimeseriesCardinalityLimiter('performance', MockCardinalityLimiter()))
        message_payloads = counter_payloads + distribution_payloads + set_payloads
