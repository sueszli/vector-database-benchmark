...
from keras.backend.common.backend_utils import compute_conv_transpose_output_shape
...
class BaseConvTranspose(Layer):
    ...
    def __init__(self, rank, filters, kernel_size, strides=1, padding='valid', output_padding=None, data_format=None, dilation_rate=1, activation=None, use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros', kernel_regularizer=None, bias_regularizer=None, activity_regularizer=None, kernel_constraint=None, bias_constraint=None, trainable=True, name=None, **kwargs):
        if False:
            for i in range(10):
                print('nop')
        ...
        if output_padding is None:
            ...
            self.output_padding = standardize_tuple(output_padding, rank, 'output_padding')
    ...
    def call(self, inputs):
        if False:
            for i in range(10):
                print('nop')
        outputs = ops.conv_transpose(inputs, self.kernel, strides=list(self.strides), padding=self.padding, output_padding=self.output_padding, dilation_rate=self.dilation_rate, data_format=self.data_format)
        if self.use_bias:
            if self.data_format == 'channels_last':
    ...
    def compute_output_shape(self, input_shape):
        if False:
            i = 10
            return i + 15
        return compute_conv_transpose_output_shape(input_shape, self.kernel_size, self.filters, strides=self.strides, padding=self.padding, output_padding=self.output_padding, data_format=self.data_format, dilation_rate=self.dilation_rate)
