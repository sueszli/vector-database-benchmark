...
class sfp_trumail(SpiderFootPlugin):
    meta = {'name': 'Trumail', 'summary': 'Check whether an email is disposable', 'flags': [], 'useCases': ['Footprint', 'Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'website': 'https://trumail.io/', 'model': 'FREE_NOAUTH_UNLIMITED', 'references': ['https://trumail.io/documentation'], 'favIcon': 'https://trumail.io/assets/favicon-32x32.png', 'logo': 'https://trumail.io/assets/images/trumail100.png', 'description': "Trumail is a product that was built with the intention of providing an easy to use API to software professionals who value a quality audience. Your apps registration workflow is one of the most important and complex parts of your software and it's very important that you filter user credentials in a way that allows for future use. Invalid user credentials, particularly email addresses, should be deemed valid and deliverable at the time of signup - That's where we come in."}}
    opts = {}
    optdescs = {}
    results = None
    errorState = False
    ...
    def queryEmailAddr(self, qry):
        if False:
            for i in range(10):
                print('nop')
        res = self.sf.fetchUrl(f'https://api.trumail.io/v2/lookups/json?email={qry}', timeout=self.opts['_fetchtimeout'], useragent='SpiderFoot')
