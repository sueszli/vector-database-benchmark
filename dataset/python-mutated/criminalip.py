...
class SearchCriminalIP:
    ...
    async def do_search(self) -> None:
        ...
        response = await AsyncFetcher.post_fetch(url, json=True, headers={'User-Agent': user_agent, 'x-api-key': f'{self.key}'}, data=data, proxy=self.proxy)
        if 'status' in response.keys():
            ...
            if status != 200:
                ...
                while scan_percentage != 100:
                    ...
                    status_response = await AsyncFetcher.fetch_all([status_url], json=True, headers={'User-Agent': user_agent, 'x-api-key': f'{self.key}'}, proxy=self.proxy)
                ...
                scan_response = await AsyncFetcher.fetch_all([report_url], json=True, headers={'User-Agent': user_agent, 'x-api-key': f'{self.key}'}, proxy=self.proxy)
