...
from helper import find_examples, load_supported_categories, validate_examples_for_duplicates_by_name, validate_examples_for_conflicting_datasets
from logger import setup_logger
parser = argparse.ArgumentParser(description='CI/CD Steps for Playground objects')
parser.add_argument('--step', dest='step', required=True, help='CI step to verify all beam examples/tests/katas. CD step to save all beam examples/tests/katas and their outputs on the GCD', choices=[Config.CI_STEP_NAME, Config.CD_STEP_NAME])
parser.add_argument('--namespace', dest='namespace', help=f'Datastore namespace to use when saving data (default: {Config.DEFAULT_NAMESPACE})', default=Config.DEFAULT_NAMESPACE)
parser.add_argument('--datastore-project', dest='datastore_project', help='Datastore project to use when saving data (CD step only)', default=None)
parser.add_argument('--sdk', dest='sdk', required=True, help='Supported SDKs', choices=Config.SUPPORTED_SDK)
parser.add_argument('--origin', type=Origin, required=True, help='ORIGIN field of pg_examples/pg_snippets', choices=[o.value for o in [Origin.PG_EXAMPLES, Origin.PG_BEAMDOC, Origin.TB_EXAMPLES]])
parser.add_argument('--subdirs', default=[], nargs='+', required=True, help='limit sub directories to walk through, relative to BEAM_ROOT_DIR')
