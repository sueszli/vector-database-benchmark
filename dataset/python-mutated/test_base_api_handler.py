...
async def test_accept_content_type(http_client, content_type_map, subtests, io_loop):
    ...
    with subtests.test('Request application/json'):
        response = await http_client.fetch('/', headers={'Accept': content_type_map['json']})
        assert response.headers['Content-Type'] == content_type_map['json']
        assert isinstance(salt.utils.json.loads(response.body), dict)
    with subtests.test('Request application/x-yaml'):
        response = await http_client.fetch('/', headers={'Accept': content_type_map['yaml']})
...
async def test_token(http_client):
    ...
    token = salt.utils.json.loads(response.body)['token']
    assert token == 'foo'
    response = await http_client.fetch('/', headers={'Cookie': '{}=foo'.format(saltnado.AUTH_COOKIE_NAME)})
    token = salt.utils.json.loads(response.body)['token']
    assert token == 'foo'
    response = await http_client.fetch('/', headers={saltnado.AUTH_TOKEN_HEADER: 'foo', 'Cookie': '{}=bar'.format(saltnado.AUTH_COOKIE_NAME)})
    token = salt.utils.json.loads(response.body)['token']
    assert token == 'foo'
...
async def test_deserialize(http_client, content_type_map, subtests):
    ...
    """
    valid_lowstate = [{'client': 'local', 'tgt': '*', 'fun': 'test.fib', 'arg': ['10']}, {'client': 'runner', 'fun': 'jobs.lookup_jid', 'jid': '20130603122505459265'}]
    with subtests.test('send as JSON'):
        response = await http_client.fetch('/', method='POST', body=salt.utils.json.dumps(valid_lowstate), headers={'Content-Type': content_type_map['json']})
    ...
    with subtests.test('send yaml as json (should break)'):
        with pytest.raises(HTTPError) as exc:
            await http_client.fetch('/', method='POST', body=salt.utils.yaml.safe_dump(valid_lowstate), headers={'Content-Type': content_type_map['json']})
        assert exc.value.code == 400
    with subtests.test('send as yaml'):
        response = await http_client.fetch('/', method='POST', body=salt.utils.yaml.safe_dump(valid_lowstate), headers={'Content-Type': content_type_map['yaml']})
        assert valid_lowstate == salt.utils.json.loads(response.body)['lowstate']
    with subtests.test('send json as yaml (works since yaml is a superset of json)'):
        response = await http_client.fetch('/', method='POST', body=salt.utils.json.dumps(valid_lowstate), headers={'Content-Type': content_type_map['yaml']})
        assert valid_lowstate == salt.utils.json.loads(response.body)['lowstate']
    with subtests.test('send json as text/plain'):
        response = await http_client.fetch('/', method='POST', body=salt.utils.json.dumps(valid_lowstate), headers={'Content-Type': content_type_map['text']})
        assert valid_lowstate == salt.utils.json.loads(response.body)['lowstate']
    with subtests.test('send form-urlencoded'):
        form_lowstate = (('client', 'local'), ('tgt', '*'), ('fun', 'test.fib'), ('arg', '10'), ('arg', 'foo'))
        response = await http_client.fetch('/', method='POST', body=urllib.parse.urlencode(form_lowstate), headers={'Content-Type': content_type_map['form']})
    ...
    with subtests.test('Send json with utf8 charset'):
        response = await http_client.fetch('/', method='POST', body=salt.utils.json.dumps(valid_lowstate), headers={'Content-Type': content_type_map['json-utf8']})
        assert valid_lowstate == salt.utils.json.loads(response.body)['lowstate']
...
async def test_get_lowstate(http_client, content_type_map, subtests):
    ...
    with subtests.test('Case 1. dictionary type of lowstate'):
        ...
        response = await http_client.fetch('/', method='POST', body=salt.utils.json.dumps(request_lowstate), headers={'Content-Type': content_type_map['json']})
        assert valid_lowstate == salt.utils.json.loads(response.body)['lowstate']
    ...
    with subtests.test('Case 3. Combine Case 1 and Case 2.'):
        ...
        assert valid_lowstate == salt.utils.json.loads(response.body)['lowstate']
    with subtests.test('send as yaml'):
        response = await http_client.fetch('/', method='POST', body=salt.utils.yaml.safe_dump(request_lowstate), headers={'Content-Type': content_type_map['yaml']})
        assert valid_lowstate == salt.utils.json.loads(response.body)['lowstate']
    with subtests.test('send as plain text'):
        response = await http_client.fetch('/', method='POST', body=salt.utils.json.dumps(request_lowstate), headers={'Content-Type': content_type_map['text']})
        assert valid_lowstate == salt.utils.json.loads(response.body)['lowstate']
    with subtests.test('send as form-urlencoded'):
        request_form_lowstate = (('client', 'local'), ('tgt', '*'), ('fun', 'test.fib'), ('arg', '10'))
        response = await http_client.fetch('/', method='POST', body=urllib.parse.urlencode(request_form_lowstate), headers={'Content-Type': content_type_map['form']})
        assert valid_lowstate == salt.utils.json.loads(response.body)['lowstate']
...
async def test_cors_origin_single(http_client, app, subtests):
    ...
    with subtests.test('Example.foo is an authorized origin'):
        response = await http_client.fetch('/', headers={'Origin': 'http://example.foo'})
    ...
    with subtests.test('Example2.foo is not an authorized origin'):
        response = await http_client.fetch('/', headers={'Origin': 'http://example2.foo'})
...
async def test_cors_preflight_request(http_client, app):
    ...
    preflight_headers = {'Access-Control-Request-Headers': request_headers, 'Access-Control-Request-Method': 'GET'}
...
async def test_cors_origin_url_with_arguments(io_loop, app, http_client):
    ...
    request_headers = 'X-Auth-Token, accept, content-type'
    preflight_headers = {'Access-Control-Request-Headers': request_headers, 'Access-Control-Request-Method': 'GET'}
    response = await http_client.fetch('/1234567890', method='OPTIONS', headers=preflight_headers)
    headers = response.headers
    assert response.code == 204
