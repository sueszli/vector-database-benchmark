...
class cloudwatch_log_metric_filter_and_alarm_for_cloudtrail_configuration_changes_enabled(Check):

    def execute(self):
        if False:
            for i in range(10):
                print('nop')
        pattern = '\\$\\.eventName\\s*=\\s*.?CreateTrail.+\\$\\.eventName\\s*=\\s*.?UpdateTrail.+\\$\\.eventName\\s*=\\s*.?DeleteTrail.+\\$\\.eventName\\s*=\\s*.?StartLogging.+\\$\\.eventName\\s*=\\s*.?StopLogging.?'
        findings = []
