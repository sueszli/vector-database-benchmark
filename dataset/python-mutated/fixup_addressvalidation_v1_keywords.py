...
class addressvalidationCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'provide_validation_feedback': ('conclusion', 'response_id'), 'validate_address': ('address', 'previous_response_id', 'enable_usps_cass')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=addressvalidationCallTransformer()):
    if False:
        return 10
