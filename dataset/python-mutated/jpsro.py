...
import pyspiel
DEFAULT_ECOS_SOLVER_KWARGS = dict(solver='ECOS', max_iters=100000000, abstol=1e-07, reltol=1e-07, feastol=1e-07, abstol_inacc=1e-07, reltol_inacc=1e-07, feastol_inacc=1e-07, verbose=False)
DEFAULT_OSQP_SOLVER_KWARGS = dict(solver='OSQP', max_iter=1000000000, eps_abs=1e-08, eps_rel=1e-08, eps_prim_inf=1e-08, eps_dual_inf=1e-08, polish_refine_iter=100, check_termination=1000, sigma=1e-07, delta=1e-07, verbose=False)
DEFAULT_CVXOPT_SOLVER_KWARGS = dict(solver='CVXOPT', maxiters=200000, abstol=5e-08, reltol=5e-08, feastol=5e-08, refinement=10, verbose=False)
...
META_SOLVERS = ('uni', 'undominated_uni', 'rj', 'undominated_rj', 'rd', 'undominated_rd', 'prd', 'alpharank', 'mgce', 'min_epsilon_mgce', 'approx_mgce', 'rmwce', 'mwce', 'rvce', 'mgcce', 'min_epsilon_mgcce', 'approx_mgcce', 'rmwcce', 'mwcce', 'rvcce')
...
def _rvcce(meta_game, per_player_repeats, ignore_repeats=False):
    ...
    dist = np.reshape(x, meta_game.shape[1:])
    return (dist, dict())
FLAG_TO_FUNC = dict(uni=_uni, undominated_uni=_undominated_uni, rj=_rj, undominated_rj=_undominated_rj, rd=_rd, undominated_rd=_undominated_rd, prd=_prd, alpharank=_alpharank, mgce=_mgce, min_epsilon_mgce=_min_epsilon_mgce, approx_mgce=_approx_mgce, rmwce=_rmwce, mwce=_mwce, rvce=_rvce, mgcce=_mgcce, min_epsilon_mgcce=_min_epsilon_mgcce, approx_mgcce=_approx_mgcce, rmwcce=_rmwcce, mwcce=_mwcce, rvcce=_rvcce)
