...
class TwoTensorsSpecVariableSerialize(TwoTensorsSpec):
    ...
    def _serialize(self):
        ...
        return (self.x_shape, self.x_dtype, self.y_shape, self.y_dtype, self.color)
type_spec.register_type_spec_from_value_converter(TwoTensors, TwoTensorsSpec.from_value)
...
type_spec.register_type_spec_from_value_converter(TwoComposites, TwoCompositesSpec.from_value)
...
class NestOfTensorsSpec(type_spec.TypeSpec):
    ...
    def _deserialize(cls, spec):
        if False:
            print('Hello World!')
        return cls(spec)
type_spec.register_type_spec_from_value_converter(NestOfTensors, NestOfTensorsSpec.from_value)
...
_TestNamedTupleDifferentField = collections.namedtuple('DifferentField', ['a', 'c'])
...
class BatchableTypeSpecTest(test_util.TensorFlowTestCase, parameterized.TestCase):
