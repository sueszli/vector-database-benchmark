...
from typing import Any, ByteString, Callable, Dict, Generator, Iterable, Iterator, List, NoReturn, Optional, Sequence, Set, Tuple, Type, Union
...
try:
    ...
    sys.exit(0)
GDB_MIN_VERSION = (8, 0)
GDB_VERSION = tuple(map(int, re.search('(\\d+)[^\\d]+(\\d+)', gdb.VERSION).groups()))
...
BP_GLYPH = '●'
GEF_PROMPT = 'gef➤  '
GEF_PROMPT_ON = f'\x01\x1b[1;32m\x02{GEF_PROMPT}\x01\x1b[0m\x02'
GEF_PROMPT_OFF = f'\x01\x1b[1;31m\x02{GEF_PROMPT}\x01\x1b[0m\x02'
PATTERN_LIBC_VERSION = re.compile(b'glibc (\\d+)\\.(\\d+)')
gef: 'Gef'
...
class Color:
    ...
    colors = {'normal': '\x1b[0m', 'gray': '\x1b[1;38;5;240m', 'light_gray': '\x1b[0;37m', 'red': '\x1b[31m', 'green': '\x1b[32m', 'yellow': '\x1b[33m', 'blue': '\x1b[34m', 'pink': '\x1b[35m', 'cyan': '\x1b[36m', 'bold': '\x1b[1m', 'underline': '\x1b[4m', 'underline_off': '\x1b[24m', 'highlight': '\x1b[3m', 'highlight_off': '\x1b[23m', 'blink': '\x1b[5m', 'blink_off': '\x1b[25m'}
...
class Phdr:
    ...
    class Type(enum.IntEnum):
        ...
        PT_HISUNW = PT_HIOS = 1879048191
        PT_LOPROC = 1879048192
        PT_ARM_EIDX = 1879048193
        PT_MIPS_ABIFLAGS = 1879048195
        PT_HIPROC = 2147483647
        UNKNOWN_PHDR = 4294967295
...
class Shdr:
    ...
    class Flags(enum.IntFlag):
        ...
        TLS = 1024
        COMPRESSED = 2048
        RELA_LIVEPATCH = 1048576
        RO_AFTER_INIT = 2097152
        ORDERED = 1073741824
        EXCLUDE = 2147483648
        UNKNOWN_FLAG = 4294967295
...
class RISCV(Architecture):
    ...
    all_registers = ('$zero', '$ra', '$sp', '$gp', '$tp', '$t0', '$t1', '$t2', '$fp', '$s1', '$a0', '$a1', '$a2', '$a3', '$a4', '$a5', '$a6', '$a7', '$s2', '$s3', '$s4', '$s5', '$s6', '$s7', '$s8', '$s9', '$s10', '$s11', '$t3', '$t4', '$t5', '$t6')
    return_register = '$a0'
...
class ARM(Architecture):
    ...
    arch = 'ARM'
    all_registers = ('$r0', '$r1', '$r2', '$r3', '$r4', '$r5', '$r6', '$r7', '$r8', '$r9', '$r10', '$r11', '$r12', '$sp', '$lr', '$pc', '$cpsr')
    nop_insn = b'\x00\xf0 \xe3'
    return_register = '$r0'
    flag_register: str = '$cpsr'
    flags_table = {31: 'negative', 30: 'zero', 29: 'carry', 28: 'overflow', 7: 'interrupt', 6: 'fast', 5: 'thumb'}
...
class AARCH64(ARM):
    ...
    arch = 'ARM64'
    mode: str = ''
    all_registers = ('$x0', '$x1', '$x2', '$x3', '$x4', '$x5', '$x6', '$x7', '$x8', '$x9', '$x10', '$x11', '$x12', '$x13', '$x14', '$x15', '$x16', '$x17', '$x18', '$x19', '$x20', '$x21', '$x22', '$x23', '$x24', '$x25', '$x26', '$x27', '$x28', '$x29', '$x30', '$sp', '$pc', '$cpsr', '$fpsr', '$fpcr')
    return_register = '$x0'
    flag_register = '$cpsr'
    flags_table = {31: 'negative', 30: 'zero', 29: 'carry', 28: 'overflow', 7: 'interrupt', 9: 'endian', 6: 'fast', 5: 't32', 4: 'm[4]'}
    nop_insn = b'\x1f \x03\xd5'
...
class X86(Architecture):
    ...
    flags_table = {6: 'zero', 0: 'carry', 2: 'parity', 4: 'adjust', 7: 'sign', 8: 'trap', 9: 'interrupt', 10: 'direction', 11: 'overflow', 16: 'resume', 17: 'virtualx86', 21: 'identification'}
...
class X86_64(X86):
    ...
    arch = 'X86'
    mode = '64'
    gpr_registers = ('$rax', '$rbx', '$rcx', '$rdx', '$rsp', '$rbp', '$rsi', '$rdi', '$rip', '$r8', '$r9', '$r10', '$r11', '$r12', '$r13', '$r14', '$r15')
...
class PowerPC(Architecture):
    ...
    arch = 'PPC'
    mode = 'PPC32'
    all_registers = ('$r0', '$r1', '$r2', '$r3', '$r4', '$r5', '$r6', '$r7', '$r8', '$r9', '$r10', '$r11', '$r12', '$r13', '$r14', '$r15', '$r16', '$r17', '$r18', '$r19', '$r20', '$r21', '$r22', '$r23', '$r24', '$r25', '$r26', '$r27', '$r28', '$r29', '$r30', '$r31', '$pc', '$msr', '$cr', '$lr', '$ctr', '$xer', '$trap')
    instruction_length = 4
    nop_insn = b'`\x00\x00\x00'
    return_register = '$r0'
    flag_register: str = '$cr'
    flags_table = {3: 'negative[0]', 2: 'positive[0]', 1: 'equal[0]', 0: 'overflow[0]', 31: 'less[7]', 30: 'greater[7]', 29: 'equal[7]', 28: 'overflow[7]'}
...
class SPARC(Architecture):
    ...
    arch = 'SPARC'
    mode = ''
    all_registers = ('$g0', '$g1', '$g2', '$g3', '$g4', '$g5', '$g6', '$g7', '$o0', '$o1', '$o2', '$o3', '$o4', '$o5', '$o7', '$l0', '$l1', '$l2', '$l3', '$l4', '$l5', '$l6', '$l7', '$i0', '$i1', '$i2', '$i3', '$i4', '$i5', '$i7', '$pc', '$npc', '$sp ', '$fp ', '$psr')
    instruction_length = 4
    nop_insn = b'\x00\x00\x00\x00'
    return_register = '$i0'
    flag_register: str = '$psr'
    flags_table = {23: 'negative', 22: 'zero', 21: 'overflow', 20: 'carry', 7: 'supervisor', 5: 'trap'}
...
class SPARC64(SPARC):
    ...
    arch = 'SPARC'
    mode = 'V9'
    all_registers = ['$g0', '$g1', '$g2', '$g3', '$g4', '$g5', '$g6', '$g7', '$o0', '$o1', '$o2', '$o3', '$o4', '$o5', '$o7', '$l0', '$l1', '$l2', '$l3', '$l4', '$l5', '$l6', '$l7', '$i0', '$i1', '$i2', '$i3', '$i4', '$i5', '$i7', '$pc', '$npc', '$sp', '$fp', '$state']
    flag_register = '$state'
    flags_table = {35: 'negative', 34: 'zero', 33: 'overflow', 32: 'carry'}
...
class MIPS(Architecture):
    ...
    arch = 'MIPS'
    mode = 'MIPS32'
    all_registers = ('$zero', '$at', '$v0', '$v1', '$a0', '$a1', '$a2', '$a3', '$t0', '$t1', '$t2', '$t3', '$t4', '$t5', '$t6', '$t7', '$s0', '$s1', '$s2', '$s3', '$s4', '$s5', '$s6', '$s7', '$t8', '$t9', '$k0', '$k1', '$s8', '$pc', '$sp', '$hi', '$lo', '$fir', '$ra', '$gp')
    instruction_length = 4
    _ptrsize = 4
    nop_insn = b'\x00\x00\x00\x00'
    return_register = '$v0'
...
class PrintFormatCommand(GenericCommand):
    ...
    _aliases_ = ['pf']
    _syntax_ = f"{_cmdline_} [--lang LANG] [--bitlen SIZE] [(--length,-l) LENGTH] [--clip] LOCATION\t--lang LANG specifies the output format for programming language (available: {valid_formats!s}, default 'py').\t--bitlen SIZE specifies size of bit (possible values: {valid_bitness!s}, default is 8).\t--length LENGTH specifies length of array (default is 256).\t--clip The output data will be copied to clipboard\tLOCATION specifies where the address of bytes is stored."
...
class SearchPatternCommand(GenericCommand):
    ...
    _aliases_ = ['grep', 'xref']
    _example_ = [f'{_cmdline_} AAAAAAAA', f'{_cmdline_} 0x555555554000 little stack', f'{_cmdline_} AAAA 0x600000-0x601000', f'{_cmdline_} --regex 0x401000 0x401500 ([\\\\x20-\\\\x7E]{{2,}})(?=\\\\x00)   <-- It matchs null-end-printable(from x20-x7e) C strings (min size 2 bytes)']
...
class RemoteCommand(GenericCommand):
    ...
    _example_ = [f'{_cmdline_} localhost 1234', f'{_cmdline_} --pid 6789 localhost 1234', f'{_cmdline_} --qemu-user --qemu-binary /bin/debugme localhost 4444 ']
...
class SkipiCommand(GenericCommand):
    ...
    _example_ = [f'{_cmdline_}', f'{_cmdline_} --n 3', f'{_cmdline_} 0x69696969', f'{_cmdline_} 0x69696969 --n 6']
...
class NopCommand(GenericCommand):
    ...
    _example_ = [f'{_cmdline_}', f'{_cmdline_} $pc+3', f'{_cmdline_} --i 2 $pc+3', f'{_cmdline_} --b', f'{_cmdline_} --b $pc+3', f'{_cmdline_} --f --b --i 2 $pc+3{_cmdline_} --n --i 2 $pc+3']
...
class StubCommand(GenericCommand):
    ...
    _cmdline_ = 'stub'
    _syntax_ = f'{_cmdline_} [--retval RETVAL] [address]\taddress\taddress/symbol to stub out\t--retval RETVAL\tSet the return value'
...
class PatchCommand(GenericCommand):
    ...
    _cmdline_ = 'patch'
    _syntax_ = f'{_cmdline_} (qword|dword|word|byte) LOCATION VALUES\n{_cmdline_} string LOCATION "double-escaped string"'
    SUPPORTED_SIZES = {'qword': (8, 'Q'), 'dword': (4, 'L'), 'word': (2, 'H'), 'byte': (1, 'B')}
...
class PatternSearchCommand(GenericCommand):
    ...
    _example_ = [f'{_cmdline_} $pc', f'{_cmdline_} 0x61616164', f'{_cmdline_} aaab']
...
if __name__ == '__main__':
    ...
    if GDB_VERSION < GDB_MIN_VERSION or PYTHON_VERSION < PYTHON_MIN_VERSION:
        err(f"You're using an old version of GDB. GEF will not work correctly. Consider updating to GDB {'.'.join(map(str, GDB_MIN_VERSION))} or higher (with Python {'.'.join(map(str, PYTHON_MIN_VERSION))} or higher).")
        exit(1)
    try:
        pyenv = which('pyenv')
        ...
        site_packages_dir = os.path.join(PYENV_ROOT, 'versions', PYENV_VERSION, 'lib', f'python{PYENV_VERSION[:3]}', 'site-packages')
    ...
    if PREFIX != sys.base_prefix:
        SITE_PACKAGES_DIRS = subprocess.check_output([PYTHONBIN, '-c', 'import os, sys;print(os.linesep.join(sys.path).strip())']).decode('utf-8').split()
        sys.path.extend(SITE_PACKAGES_DIRS)
    gdb_initial_settings = ('set confirm off', 'set verbose off', 'set pagination off', 'set print elements 0', 'set history save on', f"set history filename {os.getenv('GDBHISTFILE', '~/.gdb_history')}", 'set output-radix 0x10', 'set print pretty on', 'set disassembly-flavor intel', 'handle SIGALRM print nopass')
