...
class PosSession(models.Model):
    _name = 'pos.session'
    _order = 'id desc'
    POS_SESSION_STATE = [('opening_control', 'Opening Control'), ('opened', 'In Progress'), ('closing_control', 'Closing Control'), ('closed', 'Closed & Posted')]
    ...
    config_id = fields.Many2one('pos.config', string='Point of Sale', help='The physical point of sale you will use.', required=True, index=True)
    ...
    state = fields.Selection(POS_SESSION_STATE, string='Status', required=True, readonly=True, index=True, copy=False, default='opening_control')
    ...
    cash_register_balance_end_real = fields.Monetary(related='cash_register_id.balance_end_real', string='Ending Balance', help='Total of closing cash control lines.', readonly=True)
    cash_register_balance_start = fields.Monetary(related='cash_register_id.balance_start', string='Starting Balance', help='Total of opening cash control lines.', readonly=True)
    cash_register_total_entry_encoding = fields.Monetary(related='cash_register_id.total_entry_encoding', string='Total Cash Transaction', readonly=True, help='Total of all paid sale orders')
    cash_register_balance_end = fields.Monetary(related='cash_register_id.balance_end', digits=0, string='Theoretical Closing Balance', help='Sum of opening balance and transactions.', readonly=True)
    cash_register_difference = fields.Monetary(related='cash_register_id.difference', string='Difference', help='Difference between the theoretical closing balance and the real closing balance.', readonly=True)
    journal_ids = fields.Many2many('account.journal', related='config_id.journal_ids', readonly=True, string='Available Payment Methods')
