...
def create_config(configs, _type: str, Parameter, PluginConfig):
    ...
    for config in configs:
        ...
        for (secret_name, secret_values) in config.secrets.items():
            secret = Parameter(analyzer_config=config, name=secret_name, type=secret_values['type'], description=secret_values['description'], is_secret=True, required=secret_values['required'])
            secret.full_clean()
            secret.save()
    ...
    for config in AnalyzerConfig.objects.all():
        ...
        for parameter in config.parameters.all():
            if parameter.is_secret:
                ...
                try:
                    value = PluginConfig.objects.get(plugin_name=config.name, attribute=parameter.name, config_type='2', owner__isnull=True, organization=None, type='1')
                ...
                value = PluginConfig.objects.get(plugin_name=config.name, attribute=parameter.name, config_type='1', owner__isnull=True, organization=None, type='1')
...
class Migration(migrations.Migration):
    dependencies = [('analyzers_manager', '0022_otx_check_hash_timeout'), ('api_app', '0027_parameter')]
    operations = [migrations.RunPython(migrate, reverse_migrate), migrations.RemoveField(model_name='analyzerconfig', name='params'), migrations.RemoveField(model_name='analyzerconfig', name='secrets')]