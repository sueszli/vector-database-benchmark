...
class Hashtag:
    ...
    async def info(self, **kwargs) -> dict:
        ...
        """
        if not self.name:
            raise TypeError('You must provide the name when creating this class to use this method.')
        url_params = {'challengeName': self.name, 'msToken': kwargs.get('ms_token')}
        resp = await self.parent.make_request(url='https://www.tiktok.com/api/challenge/detail/', params=url_params, headers=kwargs.get('headers'), session_index=kwargs.get('session_index'))
        if resp is None:
    ...
    async def videos(self, count=30, cursor=0, **kwargs) -> Iterator[Video]:
        ...
        found = 0
        while found < count:
            params = {'challengeID': self.id, 'count': 30, 'cursor': cursor}
            resp = await self.parent.make_request(url='https://www.tiktok.com/api/challenge/item_list/', params=params, headers=kwargs.get('headers'), session_index=kwargs.get('session_index'))
            if resp is None:
                raise InvalidResponseException(resp, 'TikTok returned an invalid response.')
