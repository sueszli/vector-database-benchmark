...
from airflow_breeze.utils.common_options import option_debug_resources, option_dry_run, option_include_success_outputs, option_parallelism, option_python, option_python_versions, option_run_in_parallel, option_skip_cleanup, option_verbose
from airflow_breeze.utils.console import Output, get_console
from airflow_breeze.utils.custom_param_types import CacheableChoice, CacheableDefault
from airflow_breeze.utils.kubernetes_utils import CHART_PATH, K8S_CLUSTERS_PATH, SCRIPTS_CI_KUBERNETES_PATH, KubernetesPythonVersion, create_virtualenv, get_config_folder, get_k8s_env, get_kind_cluster_config_path, get_kind_cluster_name, get_kubeconfig_file, get_kubectl_cluster_name, get_kubernetes_python_combos, make_sure_kubernetes_tools_are_installed, print_cluster_urls, run_command_with_k8s_env, set_random_cluster_ports
from airflow_breeze.utils.parallel import DockerBuildxProgressMatcher, GenericRegexpProgressMatcher, check_async_run_results, run_with_pool
from airflow_breeze.utils.recording import generating_command_images
from airflow_breeze.utils.run_utils import RunCommandResult, check_if_image_exists, run_command
PARALLEL_PYTEST_ARGS = ['--verbosity=0', '--strict-markers', '--durations=100', '--maxfail=50', '--color=yes', '--timeouts-order', 'moi', '--setup-timeout=300', '--execution-timeout=300', '--teardown-timeout=300', '-rfEX']
...
def kubernetes_group():
    if False:
        ...
        return i + 15
    pass
option_executor = click.option('--executor', help='Executor to use for a kubernetes cluster.', type=CacheableChoice(ALLOWED_EXECUTORS), show_default=True, default=CacheableDefault(ALLOWED_EXECUTORS[0]), envvar='EXECUTOR')
option_kubernetes_version = click.option('--kubernetes-version', help='Kubernetes version used to create the KinD cluster of.', type=CacheableChoice(ALLOWED_KUBERNETES_VERSIONS), show_default=True, default=CacheableDefault(ALLOWED_KUBERNETES_VERSIONS[0]), envvar='KUBERNETES_VERSION')
...
option_wait_time_in_seconds = click.option('--wait-time-in-seconds', help='Wait for Airflow webserver for specified number of seconds.', type=click.IntRange(0), default=120, envvar='WAIT_TIME_IN_SECONDS')
option_wait_time_in_seconds_0_default = click.option('--wait-time-in-seconds', help='Wait for Airflow webserver for specified number of seconds.', type=click.IntRange(0), default=0, envvar='WAIT_TIME_IN_SECONDS')
option_force_recreate_cluster = click.option('--force-recreate-cluster', help='Force recreation of the cluster even if it is already created.', is_flag=True, envvar='FORCE_RECREATE_CLUSTER')
option_force_venv_setup = click.option('--force-venv-setup', help='Force recreation of the virtualenv.', is_flag=True, envvar='FORCE_VENV_SETUP')
option_use_standard_naming = click.option('--use-standard-naming', help='Use standard naming.', is_flag=True, envvar='USE_STANDARD_NAMING')
option_rebuild_base_image = click.option('--rebuild-base-image', help='Rebuilds base Airflow image before building K8S image.', is_flag=True, envvar='REBUILD_BASE_IMAGE')
option_kubernetes_versions = click.option('--kubernetes-versions', help='Kubernetes versions used to run in parallel (space separated).', type=str, show_default=True, default=' '.join(ALLOWED_KUBERNETES_VERSIONS), envvar='KUBERNETES_VERSIONS')
option_upgrade = click.option('--upgrade', help='Upgrade Helm Chart rather than installing it.', is_flag=True, envvar='UPGRADE')
option_parallelism_cluster = click.option('--parallelism', help='Maximum number of processes to use while running the operation in parallel for cluster operations.', type=click.IntRange(1, max(1, (mp.cpu_count() + 1) // 3) if not generating_command_images() else 4), default=max(1, (mp.cpu_count() + 1) // 3) if not generating_command_images() else 2, envvar='PARALLELISM', show_default=True)
option_all = click.option('--all', help='Apply it to all created clusters', is_flag=True, envvar='ALL')
K8S_CLUSTER_CREATE_PROGRESS_REGEXP = '.*airflow-python-[0-9.]+-v[0-9.].*|.*Connecting to localhost.*'
K8S_UPLOAD_PROGRESS_REGEXP = '.*airflow-python-[0-9.]+-v[0-9.].*'
K8S_CONFIGURE_CLUSTER_PROGRESS_REGEXP = '.*airflow-python-[0-9.]+-v[0-9.].*'
K8S_DEPLOY_PROGRESS_REGEXP = '.*airflow-python-[0-9.]+-v[0-9.].*'
K8S_TEST_PROGRESS_REGEXP = '.*airflow-python-[0-9.]+-v[0-9.].*|^kubernetes_tests/.*'
PREVIOUS_LINE_K8S_TEST_REGEXP = '^kubernetes_tests/.*'
COMPLETE_TEST_REGEXP = '\\s*#(\\d*) |.*airflow-python-[0-9.]+-v[0-9.].*|.*Connecting to localhost.*|^kubernetes_tests/.*|.*Error during running tests.*|.*Successfully run tests.*'
