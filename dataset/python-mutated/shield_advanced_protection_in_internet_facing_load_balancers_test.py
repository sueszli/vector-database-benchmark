...
class Test_shield_advanced_protection_in_internet_facing_load_balancers:
    ...
    def test_no_shield_not_active(self):
        ...
        with mock.patch('prowler.providers.aws.services.shield.shield_service.Shield', new=shield_client), mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()), mock.patch('prowler.providers.aws.services.shield.shield_advanced_protection_in_internet_facing_load_balancers.shield_advanced_protection_in_internet_facing_load_balancers.elbv2_client', new=ELBv2(self.set_mocked_audit_info())):
            from prowler.providers.aws.services.shield.shield_advanced_protection_in_internet_facing_load_balancers.shield_advanced_protection_in_internet_facing_load_balancers import shield_advanced_protection_in_internet_facing_load_balancers
    ...
    def test_shield_enabled_elbv2_internet_facing_protected(self):
        ...
        lb_name = 'my-lb'
        lb = conn.create_load_balancer(Name=lb_name, Subnets=[subnet1.id, subnet2.id], SecurityGroups=[security_group.id], Scheme='internet-facing', Type='application')['LoadBalancers'][0]
        lb_arn = lb['LoadBalancerArn']
        shield_client = mock.MagicMock
        shield_client.enabled = True
        shield_client.region = AWS_REGION
        protection_id = 'test-protection'
        shield_client.protections = {protection_id: Protection(id=protection_id, name='', resource_arn=lb_arn, protection_arn='', region=AWS_REGION)}
        from prowler.providers.aws.services.elbv2.elbv2_service import ELBv2
        with mock.patch('prowler.providers.aws.services.shield.shield_service.Shield', new=shield_client), mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()), mock.patch('prowler.providers.aws.services.shield.shield_advanced_protection_in_internet_facing_load_balancers.shield_advanced_protection_in_internet_facing_load_balancers.elbv2_client', new=ELBv2(self.set_mocked_audit_info())):
            from prowler.providers.aws.services.shield.shield_advanced_protection_in_internet_facing_load_balancers.shield_advanced_protection_in_internet_facing_load_balancers import shield_advanced_protection_in_internet_facing_load_balancers
            check = shield_advanced_protection_in_internet_facing_load_balancers()
            ...
            assert result[0].status_extended == f'ELBv2 ALB {lb_name} is protected by AWS Shield Advanced.'
    ...
    def test_shield_enabled_elbv2_internal_protected(self):
        ...
        subnet2 = ec2.create_subnet(VpcId=vpc.id, CidrBlock='172.28.7.0/26', AvailabilityZone=f'{AWS_REGION}b')
        lb_name = 'my-lb'
        lb = conn.create_load_balancer(Name=lb_name, Subnets=[subnet1.id, subnet2.id], SecurityGroups=[security_group.id], Scheme='internal', Type='application')['LoadBalancers'][0]
        lb_arn = lb['LoadBalancerArn']
        shield_client = mock.MagicMock
        shield_client.enabled = True
        shield_client.region = AWS_REGION
    ...
    def test_shield_enabled_elbv2_internet_facing_not_protected(self):
        ...
        with mock.patch('prowler.providers.aws.services.shield.shield_service.Shield', new=shield_client), mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()), mock.patch('prowler.providers.aws.services.shield.shield_advanced_protection_in_internet_facing_load_balancers.shield_advanced_protection_in_internet_facing_load_balancers.elbv2_client', new=ELBv2(self.set_mocked_audit_info())):
            ...
            assert result[0].status_extended == f'ELBv2 ALB {lb_name} is not protected by AWS Shield Advanced.'
