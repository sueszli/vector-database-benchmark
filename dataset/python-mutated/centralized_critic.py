...
parser.add_argument('--stop-reward', type=float, default=7.99, help='Reward at which we stop training.')
...
if __name__ == '__main__':
    ...
    ModelCatalog.register_custom_model('cc_model', TorchCentralizedCriticModel if args.framework == 'torch' else CentralizedCriticModel)
    config = PPOConfig().experimental(_enable_new_api_stack=False).environment(TwoStepGame).framework(args.framework).rollouts(batch_mode='complete_episodes', num_rollout_workers=0).training(model={'custom_model': 'cc_model'}).multi_agent(policies={'pol1': (None, Discrete(6), TwoStepGame.action_space, PPOConfig.overrides(framework_str=args.framework)), 'pol2': (None, Discrete(6), TwoStepGame.action_space, PPOConfig.overrides(framework_str=args.framework))}, policy_mapping_fn=lambda agent_id, episode, worker, **kwargs: 'pol1' if agent_id == 0 else 'pol2').resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    stop = {'training_iteration': args.stop_iters, 'timesteps_total': args.stop_timesteps, 'episode_reward_mean': args.stop_reward}
    tuner = tune.Tuner(CentralizedCritic, param_space=config.to_dict(), run_config=air.RunConfig(stop=stop, verbose=1))
    results = tuner.fit()
    if args.as_test:
