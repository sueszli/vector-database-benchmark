...
from homeassistant.components.homekit.const import ATTR_KEY_NAME, ATTR_VALUE, CHAR_REMOTE_KEY, CONF_FEATURE_LIST, EVENT_HOMEKIT_TV_REMOTE_KEY_PRESSED, FEATURE_ON_OFF, FEATURE_PLAY_PAUSE, FEATURE_PLAY_STOP, FEATURE_TOGGLE_MUTE, KEY_ARROW_RIGHT
from homeassistant.components.homekit.type_media_players import MediaPlayer, ReceiverMediaPlayer, TelevisionMediaPlayer
from homeassistant.components.media_player import ATTR_INPUT_SOURCE, ATTR_INPUT_SOURCE_LIST, ATTR_MEDIA_VOLUME_LEVEL, ATTR_MEDIA_VOLUME_MUTED, DOMAIN, MediaPlayerDeviceClass
from homeassistant.const import ATTR_DEVICE_CLASS, ATTR_ENTITY_ID, ATTR_SUPPORTED_FEATURES, EVENT_HOMEASSISTANT_START, STATE_IDLE, STATE_OFF, STATE_ON, STATE_PAUSED, STATE_PLAYING, STATE_STANDBY
...
async def test_media_player_set_state(hass: HomeAssistant, hk_driver, events) -> None:
    ...
    config = {CONF_FEATURE_LIST: {FEATURE_ON_OFF: None, FEATURE_PLAY_PAUSE: None, FEATURE_PLAY_STOP: None, FEATURE_TOGGLE_MUTE: None}}
    entity_id = 'media_player.test'
    base_attrs = {ATTR_SUPPORTED_FEATURES: 20873, ATTR_MEDIA_VOLUME_MUTED: False}
    hass.states.async_set(entity_id, None, base_attrs)
    await hass.async_block_till_done()
    acc = MediaPlayer(hass, hk_driver, 'MediaPlayer', entity_id, 2, config)
    await acc.run()
    await hass.async_block_till_done()
    assert acc.aid == 2
    assert acc.category == 8
    ...
    hass.states.async_set(entity_id, STATE_ON, {**base_attrs, ATTR_MEDIA_VOLUME_MUTED: True})
    await hass.async_block_till_done()
...
async def test_media_player_television(hass: HomeAssistant, hk_driver, events, caplog: pytest.LogCaptureFixture) -> None:
    """Test if television accessory and HA are updated accordingly."""
    entity_id = 'media_player.television'
    base_attrs = {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 3469, ATTR_MEDIA_VOLUME_MUTED: False, ATTR_INPUT_SOURCE_LIST: ['HDMI 1', 'HDMI 2', 'HDMI 3', 'HDMI 4']}
    hass.states.async_set(entity_id, None, base_attrs)
    await hass.async_block_till_done()
    ...
    await hass.async_block_till_done()
    assert acc.char_active.value == 0
    hass.states.async_set(entity_id, STATE_ON, {**base_attrs, ATTR_INPUT_SOURCE: 'HDMI 2'})
    await hass.async_block_till_done()
    assert acc.char_input_source.value == 1
    hass.states.async_set(entity_id, STATE_ON, {**base_attrs, ATTR_INPUT_SOURCE: 'HDMI 3'})
    await hass.async_block_till_done()
    assert acc.char_input_source.value == 2
    hass.states.async_set(entity_id, STATE_ON, {**base_attrs, ATTR_INPUT_SOURCE: 'HDMI 5'})
    await hass.async_block_till_done()
    assert acc.char_input_source.value == 0
...
async def test_media_player_television_basic(hass: HomeAssistant, hk_driver, events, caplog: pytest.LogCaptureFixture) -> None:
    """Test if basic television accessory and HA are updated accordingly."""
    entity_id = 'media_player.television'
    base_attrs = {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 384}
    hass.states.async_set(entity_id, None, base_attrs)
    await hass.async_block_till_done()
    acc = TelevisionMediaPlayer(hass, hk_driver, 'MediaPlayer', entity_id, 2, None)
    await acc.run()
...
async def test_media_player_television_supports_source_select_no_sources(hass: HomeAssistant, hk_driver, events, caplog: pytest.LogCaptureFixture) -> None:
    """Test if basic tv that supports source select but is missing a source list."""
    entity_id = 'media_player.television'
    hass.states.async_set(entity_id, None, {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 3469})
    await hass.async_block_till_done()
    acc = TelevisionMediaPlayer(hass, hk_driver, 'MediaPlayer', entity_id, 2, None)
    await acc.run()
    await hass.async_block_till_done()
...
async def test_tv_restore(hass: HomeAssistant, entity_registry: er.EntityRegistry, hk_driver, events) -> None:
    """Test setting up an entity from state in the event registry."""
    hass.state = CoreState.not_running
    entity_registry.async_get_or_create('media_player', 'generic', '1234', suggested_object_id='simple', original_device_class=MediaPlayerDeviceClass.TV)
    entity_registry.async_get_or_create('media_player', 'generic', '9012', suggested_object_id='all_info_set', capabilities={ATTR_INPUT_SOURCE_LIST: ['HDMI 1', 'HDMI 2', 'HDMI 3', 'HDMI 4']}, supported_features=3469, original_device_class=MediaPlayerDeviceClass.TV)
    hass.bus.async_fire(EVENT_HOMEASSISTANT_START, {})
    await hass.async_block_till_done()
    acc = TelevisionMediaPlayer(hass, hk_driver, 'MediaPlayer', 'media_player.simple', 2, None)
    assert acc.category == 31
    assert acc.chars_tv == [CHAR_REMOTE_KEY]
    assert acc.chars_speaker == []
    assert acc.support_select_source is False
    assert not hasattr(acc, 'char_input_source')
    acc = TelevisionMediaPlayer(hass, hk_driver, 'MediaPlayer', 'media_player.all_info_set', 3, None)
    assert acc.category == 31
    assert acc.chars_tv == [CHAR_REMOTE_KEY]
    assert acc.chars_speaker == ['Name', 'Active', 'VolumeControlType', 'VolumeSelector', 'Volume']
    assert acc.support_select_source is True
    assert acc.char_input_source is not None

async def test_media_player_television_max_sources(hass: HomeAssistant, hk_driver, events, caplog: pytest.LogCaptureFixture) -> None:
    """Test if television accessory that reaches the maximum number of sources."""
    entity_id = 'media_player.television'
    sources = [f'HDMI {i}' for i in range(1, 101)]
    hass.states.async_set(entity_id, None, {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 3469, ATTR_MEDIA_VOLUME_MUTED: False, ATTR_INPUT_SOURCE: 'HDMI 3', ATTR_INPUT_SOURCE_LIST: sources})
    await hass.async_block_till_done()
    acc = TelevisionMediaPlayer(hass, hk_driver, 'MediaPlayer', entity_id, 2, None)
    await acc.run()
    await hass.async_block_till_done()
    ...
    assert acc.char_input_source.value == 2
    assert acc.char_mute.value is False
    hass.states.async_set(entity_id, None, {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 3469, ATTR_MEDIA_VOLUME_MUTED: False, ATTR_INPUT_SOURCE: 'HDMI 90', ATTR_INPUT_SOURCE_LIST: sources})
    await hass.async_block_till_done()
    assert acc.char_input_source.value == 89
    hass.states.async_set(entity_id, None, {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 3469, ATTR_MEDIA_VOLUME_MUTED: False, ATTR_INPUT_SOURCE: 'HDMI 91', ATTR_INPUT_SOURCE_LIST: sources})
    await hass.async_block_till_done()
    assert acc.char_input_source.value == 0

async def test_media_player_television_duplicate_sources(hass: HomeAssistant, hk_driver, events, caplog: pytest.LogCaptureFixture) -> None:
    """Test if television accessory with duplicate sources."""
    entity_id = 'media_player.television'
    sources = ['MUSIC', 'HDMI', 'SCREEN MIRRORING', 'HDMI', 'MUSIC']
    hass.states.async_set(entity_id, None, {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 3469, ATTR_MEDIA_VOLUME_MUTED: False, ATTR_INPUT_SOURCE: 'HDMI', ATTR_INPUT_SOURCE_LIST: sources})
    await hass.async_block_till_done()
    acc = TelevisionMediaPlayer(hass, hk_driver, 'MediaPlayer', entity_id, 2, None)
    await acc.run()
    await hass.async_block_till_done()
    ...
    assert acc.char_input_source.value == 1
    assert acc.char_mute.value is False
    hass.states.async_set(entity_id, None, {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 3469, ATTR_MEDIA_VOLUME_MUTED: False, ATTR_INPUT_SOURCE: 'MUSIC', ATTR_INPUT_SOURCE_LIST: sources})
    await hass.async_block_till_done()
    assert acc.char_input_source.value == 0

async def test_media_player_television_unsafe_chars(hass: HomeAssistant, hk_driver, events, caplog: pytest.LogCaptureFixture) -> None:
    """Test if television accessory with unsafe characters."""
    entity_id = 'media_player.television'
    sources = ['MUSIC', 'HDMI 3/ARC', 'SCREEN MIRRORING', 'HDMI 2/MHL', 'HDMI', 'MUSIC']
    hass.states.async_set(entity_id, None, {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 3469, ATTR_MEDIA_VOLUME_MUTED: False, ATTR_INPUT_SOURCE: 'HDMI 2/MHL', ATTR_INPUT_SOURCE_LIST: sources})
    await hass.async_block_till_done()
    acc = TelevisionMediaPlayer(hass, hk_driver, 'MediaPlayer', entity_id, 2, None)
    await acc.run()
    await hass.async_block_till_done()
    ...
    assert acc.char_mute.value is False
    hass.states.async_set(entity_id, None, {ATTR_DEVICE_CLASS: MediaPlayerDeviceClass.TV, ATTR_SUPPORTED_FEATURES: 3469, ATTR_MEDIA_VOLUME_MUTED: False, ATTR_INPUT_SOURCE: 'HDMI 3/ARC', ATTR_INPUT_SOURCE_LIST: sources})
    await hass.async_block_till_done()
    assert acc.char_input_source.value == 1
    call_select_source = async_mock_service(hass, DOMAIN, 'select_source')
...
async def test_media_player_receiver(hass: HomeAssistant, hk_driver: HomeDriver, caplog: pytest.LogCaptureFixture) -> None:
    """Test if television accessory with unsafe characters."""
