...
from torch.distributed.algorithms.ddp_comm_hooks import post_localSGD_hook as post_localSGD, powerSGD_hook as powerSGD, default_hooks as default, quantization as quantization_hooks
...
from torch.distributed.utils import _verify_param_shape_across_processes, _sync_module_states
from torch.nn.parallel import DistributedDataParallel
from torch.nn.parallel.distributed import _dump_DDP_relevant_env_vars, _MixedPrecision
from torch.testing._internal.common_distributed import MultiProcessTestCase, TEST_SKIPS, init_multigpu_helper, initialize_temp_directories, cleanup_temp_dir, simple_sparse_reduce_tests, skip_if_rocm, skip_if_small_worldsize, skip_if_odd_worldsize, skip_if_lt_x_gpu, nccl_skip_if_lt_x_gpu, skip_if_no_gpu, require_n_gpus_for_nccl_backend, requires_nccl_version, captured_output, with_nccl_blocking_wait, with_dist_debug_levels, verify_ddp_error_logged, DistTestCases
from torch.testing._internal.common_utils import instantiate_parametrized_tests, IS_MACOS, IS_WINDOWS, FILE_SCHEMA, IS_FBCODE, NO_MULTIPROCESSING_SPAWN, IS_SANDCASTLE, skip_but_pass_in_sandcastle, skip_but_pass_in_sandcastle_if
...
COLLECTIVES_OBJECT_TEST_LIST = [{'key1': 3, 'key2': 4, 'key3': {'nested': True}}, f, foo_cpu_tensor, 'foo', [1, 2, True, 'string', [4, 5, 'nested']]]
PROFILING_SUPPORTED_BACKENDS = [dist.Backend.NCCL, dist.Backend.GLOO, dist.Backend.MPI, dist.Backend.UCC]
CUDA_PROFILING_SUPPORTED_BACKENDS = [dist.Backend.GLOO, dist.Backend.MPI, dist.Backend.NCCL, dist.Backend.UCC]
SEND_RECV_PROFILING_SUPPORTED_BACKENDS = [dist.Backend.MPI, dist.Backend.GLOO, dist.Backend.NCCL, dist.Backend.UCC]
EXPECTED_FIELDS = ('a', 'b')
...
CUSTOM_PG_TIMEOUT = {'test_ddp_uneven_inputs': 300, 'test_ddp_model_diff_across_ranks': 5, 'test_ddp_has_finalized': 5}
