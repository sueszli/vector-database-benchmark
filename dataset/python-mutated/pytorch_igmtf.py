...
class IGMTF(Model):
    ...
    def __init__(self, d_feat=6, hidden_size=64, num_layers=2, dropout=0.0, n_epochs=200, lr=0.001, metric='', early_stop=20, loss='mse', base_model='GRU', model_path=None, optimizer='adam', GPU=0, seed=None, **kwargs):
        if False:
            i = 10
            return i + 15
        ...
        self.seed = seed
        self.logger.info('IGMTF parameters setting:\nd_feat : {}\nhidden_size : {}\nnum_layers : {}\ndropout : {}\nn_epochs : {}\nlr : {}\nmetric : {}\nearly_stop : {}\noptimizer : {}\nloss_type : {}\nbase_model : {}\nmodel_path : {}\nvisible_GPU : {}\nuse_GPU : {}\nseed : {}'.format(d_feat, hidden_size, num_layers, dropout, n_epochs, lr, metric, early_stop, optimizer.lower(), loss, base_model, model_path, GPU, self.use_gpu, seed))
        if self.seed is not None:
            np.random.seed(self.seed)
            torch.manual_seed(self.seed)
        self.igmtf_model = IGMTFModel(d_feat=self.d_feat, hidden_size=self.hidden_size, num_layers=self.num_layers, dropout=self.dropout, base_model=self.base_model)
