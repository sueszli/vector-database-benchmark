...
class sfp_intelx(SpiderFootPlugin):
    ...
    opts = {'api_key': '', 'base_url': '2.intelx.io', 'checkcohosts': False, 'checkaffiliates': False, 'netblocklookup': False, 'maxnetblock': 24, 'subnetlookup': False, 'maxsubnet': 24, 'maxage': 90}
    optdescs = {'api_key': 'IntelligenceX API key.', 'base_url': 'API URL, as provided in your IntelligenceX account settings.', 'checkcohosts': 'Check co-hosted sites?', 'checkaffiliates': 'Check affiliates?', 'netblocklookup': 'Look up all IPs on netblocks deemed to be owned by your target for possible hosts on the same target subdomain/domain?', 'maxnetblock': 'If looking up owned netblocks, the maximum netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'subnetlookup': 'Look up all IPs on subnets which your target is a part of?', 'maxsubnet': 'If looking up subnets, the maximum subnet size to look up all the IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'maxage': 'Maximum age (in days) of results to be considered valid. 0 = unlimited.'}
    results = None
    errorState = False
