...
from frappe.model import child_table_fields, data_field_options, default_fields, no_value_fields, table_fields
...
DEPENDS_ON_PATTERN = re.compile('[\\w\\.:_]+\\s*={1}\\s*[\\w\\.@\\\'"]+')
ILLEGAL_FIELDNAME_PATTERN = re.compile('[\'",./%@()<>{}]')
WHITESPACE_PADDING_PATTERN = re.compile('^[ \\t\\n\\r]+|[ \\t\\n\\r]+$', flags=re.ASCII)
START_WITH_LETTERS_PATTERN = re.compile('^(?![\\W])[^\\d_\\s][\\w -]+$', flags=re.ASCII)
