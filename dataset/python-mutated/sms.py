...
__all__ = ['BaseSMSSettingSerializer', 'SMSSettingSerializer', 'AlibabaSMSSettingSerializer', 'TencentSMSSettingSerializer', 'HuaweiSMSSettingSerializer', 'CMPP2SMSSettingSerializer', 'CustomSMSSettingSerializer']

class SMSSettingSerializer(serializers.Serializer):
    SMS_ENABLED = serializers.BooleanField(default=False, label=_('Enable SMS'))
    SMS_BACKEND = serializers.ChoiceField(choices=BACKENDS.choices, default=BACKENDS.ALIBABA, label=_('SMS provider / Protocol'))
    SMS_CODE_LENGTH = serializers.IntegerField(default=4, min_value=4, max_value=16, label=_('SMS code length'))
...
class BaseSMSSettingSerializer(serializers.Serializer):
    PREFIX_TITLE = _('SMS')
    SMS_TEST_PHONE = PhoneField(validators=[PhoneValidator()], required=False, allow_blank=True, allow_null=True, label=_('Test phone'))
...
class AlibabaSMSSettingSerializer(BaseSMSSettingSerializer):
    ...
    ALIBABA_ACCESS_KEY_SECRET = EncryptedField(max_length=256, required=False, label='Access Key Secret')
...
class CMPP2SMSSettingSerializer(BaseSMSSettingSerializer):
    ...
    CMPP2_VERIFY_TEMPLATE_CODE = serializers.CharField(max_length=69, required=True, label=_('Template'), help_text=_('Template need contain {code} and Signature + template length does not exceed 67 words. For example, your verification code is {code}, which is valid for 5 minutes. Please do not disclose it to others.'))
...
class CustomSMSSettingSerializer(BaseSMSSettingSerializer):
    ...
    CUSTOM_SMS_API_PARAMS = serializers.JSONField(label=_('Parameters'), default={'phone_number': '{phone_number}', 'code': '{code}'})
    CUSTOM_SMS_REQUEST_METHOD = serializers.ChoiceField(default=RequestType.get, choices=RequestType.choices, label=_('Request method'))
