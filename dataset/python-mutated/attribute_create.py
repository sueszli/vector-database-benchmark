...
class AttributeValueInput(BaseInputObjectType):
    ...
    rich_text = JSONString(description=AttributeValueDescriptions.RICH_TEXT + DEPRECATED_IN_3X_INPUT + "The rich text attribute hasn't got predefined value, so can be specified only from instance that supports the given attribute.")
    plain_text = graphene.String(description=AttributeValueDescriptions.PLAIN_TEXT + DEPRECATED_IN_3X_INPUT + "The plain text attribute hasn't got predefined value, so can be specified only from instance that supports the given attribute.")
    file_url = graphene.String(required=False, description='URL of the file attribute. Every time, a new value is created.')
    content_type = graphene.String(required=False, description='File content type.')
    external_reference = graphene.String(description='External ID of this attribute value.' + ADDED_IN_310, required=False)
...
class AttributeCreateInput(BaseInputObjectType):
    ...
    values = NonNullList(AttributeValueCreateInput, description=AttributeDescriptions.VALUES)
    value_required = graphene.Boolean(description=AttributeDescriptions.VALUE_REQUIRED)
    is_variant_only = graphene.Boolean(required=False, description=AttributeDescriptions.IS_VARIANT_ONLY)
    visible_in_storefront = graphene.Boolean(description=AttributeDescriptions.VISIBLE_IN_STOREFRONT)
    filterable_in_storefront = graphene.Boolean(description=AttributeDescriptions.FILTERABLE_IN_STOREFRONT + DEPRECATED_IN_3X_INPUT)
    filterable_in_dashboard = graphene.Boolean(description=AttributeDescriptions.FILTERABLE_IN_DASHBOARD)
    storefront_search_position = graphene.Int(required=False, description=AttributeDescriptions.STOREFRONT_SEARCH_POSITION + DEPRECATED_IN_3X_INPUT)
    available_in_grid = graphene.Boolean(required=False, description=AttributeDescriptions.AVAILABLE_IN_GRID + DEPRECATED_IN_3X_INPUT)
    external_reference = graphene.String(description='External ID of this attribute.' + ADDED_IN_310, required=False)
...
class AttributeCreate(AttributeMixin, ModelMutation):
    ...
    class Arguments:
        input = AttributeCreateInput(required=True, description='Fields required to create an attribute.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.ATTRIBUTE_CREATED, description='An attribute was created.')]
