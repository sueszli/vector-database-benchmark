...
class containeranalysisCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'get_iam_policy': ('resource', 'options'), 'get_vulnerability_occurrences_summary': ('parent', 'filter'), 'set_iam_policy': ('resource', 'policy', 'update_mask'), 'test_iam_permissions': ('resource', 'permissions')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=containeranalysisCallTransformer()):
    if False:
        while True:
            i = 10
