...
def get_template_from_response(response, id):
    ...
    for template in response.json()['results']:
        if template['id'] == str(id):
            return template
    return None
variable_template = {'template_name': 'Sign up conversion template with variables', 'dashboard_description': 'Use this template to see how many users sign up after visiting your pricing page.', 'dashboard_filters': {}, 'tiles': [{'name': 'Website Unique Users (Total)', 'type': 'INSIGHT', 'color': 'blue', 'filters': {'events': ['{VARIABLE_1}'], 'compare': True, 'display': 'BoldNumber', 'insight': 'TRENDS', 'interval': 'day', 'date_from': '-30d'}, 'layouts': {'sm': {'h': 5, 'i': '21', 'w': 6, 'x': 0, 'y': 0, 'minH': 5, 'minW': 3}, 'xs': {'h': 5, 'i': '21', 'w': 1, 'x': 0, 'y': 0, 'minH': 5, 'minW': 1}}, 'description': 'Shows the number of unique users that use your app every day.'}], 'tags': ['popular'], 'variables': [{'id': 'VARIABLE_1', 'name': 'Page view on your website', 'description': 'The event that is triggered when a user visits a page on your site', 'type': 'event', 'default': {'id': '$pageview', 'math': 'dau', 'type': 'events'}, 'required': True}, {'id': 'VARIABLE_2', 'name': 'Sign up event', 'description': 'The event that is triggered when a user signs up', 'type': 'event', 'default': {'id': '$autocapture', 'math': 'dau', 'type': 'events'}, 'required': False}]}
