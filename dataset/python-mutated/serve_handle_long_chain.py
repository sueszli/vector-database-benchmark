...
from serve_test_cluster_utils import setup_local_single_node_cluster, setup_anyscale_cluster
from serve_test_utils import save_test_results
from benchmark_utils import benchmark_throughput_tps, benchmark_latency_ms
DEFAULT_CHAIN_LENGTH = 4
DEFAULT_NUM_REQUESTS_PER_CLIENT = 20
...
class Node:
    ...
    def __init__(self, id: int, prev_node=None, init_delay_secs=0, compute_delay_secs=0):
        if False:
            i = 10
            return i + 15
...
def construct_long_chain_graph_with_pure_handle(chain_length, init_delay_secs=0, compute_delay_secs=0):
    if False:
        print('Hello World!')
    prev_node = None
    for id in range(chain_length):
        prev_node = Node.options(name=str(id)).bind(id, prev_node, init_delay_secs, compute_delay_secs)
...
def main(chain_length: Optional[int], init_delay_secs: Optional[int], compute_delay_secs: Optional[int], num_requests_per_client: Optional[int], num_clients: Optional[int], throughput_trial_duration_secs: Optional[int], local_test: Optional[bool]):
    ...
    handle = construct_long_chain_graph_with_pure_handle(chain_length, init_delay_secs=init_delay_secs, compute_delay_secs=compute_delay_secs)
    assert handle.predict.remote(0).result() == chain_length
    (throughput_mean_tps, throughput_std_tps) = asyncio.run(benchmark_throughput_tps(handle, chain_length, duration_secs=throughput_trial_duration_secs, num_clients=num_clients))
    (latency_mean_ms, latency_std_ms) = asyncio.run(benchmark_latency_ms(handle, chain_length, num_requests=num_requests_per_client, num_clients=num_clients))
