...
class PluginUpdateInput(graphene.InputObjectType):
    active = graphene.Boolean(required=False, description='Indicates whether the plugin should be enabled.')
    configuration = NonNullList(ConfigurationItemInput, required=False, description='Configuration of the plugin.')
...
class PluginUpdate(BaseMutation):
    ...
    class Arguments:
        ...
        channel_id = graphene.ID(required=False, description='ID of a channel for which the data should be modified.')
        input = PluginUpdateInput(description='Fields required to update a plugin configuration.', required=True)
