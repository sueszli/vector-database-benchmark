...
def _get_snowflake_options(config, table_slice: TableSlice) -> Mapping[str, str]:
    if False:
        i = 10
        return i + 15
    check.invariant(config.get('warehouse', None) is not None, 'Missing config: Warehouse is required when using PySpark with the Snowflake I/O manager.')
    conf = {'sfURL': f"{config['account']}.snowflakecomputing.com", 'sfUser': config['user'], 'sfPassword': config['password'], 'sfDatabase': config['database'], 'sfSchema': table_slice.schema, 'sfWarehouse': config['warehouse']}
    return conf
...
class SnowflakePySparkTypeHandler(DbTypeHandler[DataFrame]):
    ...
    def handle_output(self, context: OutputContext, table_slice: TableSlice, obj: DataFrame, _) -> Mapping[str, RawMetadataValue]:
        ...
        with_uppercase_cols.write.format(SNOWFLAKE_CONNECTOR).options(**options).option('dbtable', table_slice.table).mode('append').save()
    ...
    def load_input(self, context: InputContext, table_slice: TableSlice, _) -> DataFrame:
        ...
        df = spark.read.format(SNOWFLAKE_CONNECTOR).options(**options).option('query', SnowflakeDbClient.get_select_statement(table_slice)).load()
    ...
    def supported_types(self):
        if False:
            print('Hello World!')
        return [DataFrame]
snowflake_pyspark_io_manager = build_snowflake_io_manager([SnowflakePySparkTypeHandler()], default_load_type=DataFrame)
