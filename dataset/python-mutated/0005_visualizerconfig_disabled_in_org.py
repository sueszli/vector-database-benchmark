...
def backwards_migrate_disabled_config(apps, schema_editor):
    ...
    for config in VisualizerConfig.objects.all():
        for org in config.disabled_in_organizations.all():
            OrganizationPluginState.objects.create(plugin_name=config.name, type='3', organization=org, disabled=True)

class Migration(migrations.Migration):
    dependencies = [('certego_saas_organization', '0001_initial'), ('visualizers_manager', '0004_alter_visualizerreport_report')]
    operations = [migrations.AddField(model_name='visualizerconfig', name='disabled_in_organizations', field=models.ManyToManyField(blank=True, related_name='visualizers_manager_visualizerconfig_disabled', to='certego_saas_organization.Organization')), migrations.RunPython(migrate_disabled_config, backwards_migrate_disabled_config)]