...
class DisplayFixture:
    ...
    def __init__(self, backend: BackendFixture | None=None, driver: Browser | BrowserContext | Page | None=None, url_prefix: str='') -> None:
        if False:
            while True:
                i = 10
    ...
    async def show(self, component: RootComponentConstructor) -> None:
        self.backend.mount(component)
        await self.goto('/')
        await self.root_element()

    async def goto(self, path: str, query: Any | None=None, add_url_prefix: bool=True) -> None:
        await self.page.goto(self.backend.url(f'{self.url_prefix}{path}' if add_url_prefix else path, query))
