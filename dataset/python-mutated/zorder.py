...
for (i, text) in enumerate(['Figure (background)', 'Axes (spines, ticks & labels)', 'Patches (zorder=1)', 'Lines (zorder=2)', 'Text (zorder=3)', 'Inset axes & legend (zorder=5)']):
    ...
    ax.add_patch(p)
    art3d.pathpatch_2d_to_3d(p, z=i, zdir='z')
    text3d(ax, (-0.25, 0.25, i), text, zdir='z', size=0.5, angle=np.pi / 2, ec='none', fc='k')
