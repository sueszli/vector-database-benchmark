...
from fairseq.data import ConcatDataset, ConcatSentencesDataset, data_utils, Dictionary, IdDataset, indexed_dataset, NestedDictionaryDataset, NumSamplesDataset, NumelDataset, PrependTokenDataset, RawLabelDataset, RightPadDataset, SortDataset, TruncateDataset, TokenBlockDataset
...
class DiscriminativeRerankingNMTConfig(FairseqDataclass):
    ...
    num_data_splits: int = field(default=1, metadata={'help': 'total number of data splits'})
    no_shuffle: bool = field(default=False, metadata={'help': 'do not shuffle training data'})
    max_positions: int = field(default=512, metadata={'help': 'number of positional embeddings to learn'})
    include_src: bool = field(default=False, metadata={'help': 'include source sentence'})
    mt_beam: int = field(default=50, metadata={'help': 'beam size of input hypotheses'})
    eval_target_metric: bool = field(default=False, metadata={'help': 'evaluation with the target metric during validation'})
    target_metric: TARGET_METRIC_CHOICES = field(default='bleu', metadata={'help': 'name of the target metric to optimize for'})
    train_subset: str = field(default=II('dataset.train_subset'), metadata={'help': 'data subset to use for training (e.g. train, valid, test)'})
    seed: int = field(default=II('common.seed'), metadata={'help': 'pseudo random number generator seed'})
...
@register_task('discriminative_reranking_nmt', dataclass=DiscriminativeRerankingNMTConfig)
