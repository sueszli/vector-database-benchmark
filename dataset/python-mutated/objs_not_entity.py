...
class QuoteList(list):
    ...
    async def _ensure_quotes(self):
        ...
        self._api._auth._has_md_grants([q._path[-1] for q in self])
        ...
        if need_quotes - self._api._requests['quotes'] != set():
            ...
            self._api._send_pack({'aid': 'subscribe_quote', 'ins_list': ','.join(self._api._requests['quotes'])})
...
class SymbolList(list):
    ...
    async def _query_graphql(self):
        ...
        if isinstance(self._api._backtest, TqBacktest):
            self._api._send_pack({'aid': 'ins_query', 'query_id': self._query_id, 'query': ''})
        return self
...
class TqSymbolDataFrame(DataFrame):
    ...
    async def async_update(self):
        ...
        self.__dict__['_api']._send_pack({'aid': 'ins_query', 'query_id': query_id, 'query': op.__to_graphql__()})
            ...
            async for _ in update_chan:
                ...
                if query_result:
                    ...
                    if self.__dict__['_backtest_timestamp']:
                        ...
                        self.__dict__['_api']._send_pack({'aid': 'ins_query', 'query_id': query_id, 'query': ''})
                    return self
    ...
    async def _get_ranking_data(self, ranking_id):
        async with aiohttp.ClientSession(headers=self.__dict__['_api']._base_headers) as session:
            ...
            async with session.get(url, params=self.__dict__['_params']) as response:
                ...
                await self.__dict__['_api']._ws_md_recv_chan.send({'aid': 'rtn_data', 'data': [{'_symbol_rankings': {ranking_id: content}, '_symbol_rankings_finished': {ranking_id: True}}]})

    async def async_update(self):
        await self.__dict__['_api']._ensure_symbol_async(self.__dict__['_symbol'])
        ranking_id = _generate_uuid('PYSDK_rank')
        self.__dict__['_api'].create_task(self._get_ranking_data(ranking_id), _caller_api=True)
        ...
        async with self.__dict__['_api'].register_update_notify(symbol_rankings_finished) as update_chan:
            async for _ in update_chan:
                ...
                await self.__dict__['_api']._ws_md_recv_chan.send({'aid': 'rtn_data', 'data': [{'_symbol_rankings': {ranking_id: None}, '_symbol_rankings_finished': {ranking_id: None}}]})
                return self
