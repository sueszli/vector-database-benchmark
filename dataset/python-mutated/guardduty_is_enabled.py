    ...
    def execute(self):
        ...
        for detector in guardduty_client.detectors:
            ...
            if not detector.enabled_in_account:
                ...
                report.status = 'FAIL'
                report.status_extended = f'GuardDuty detector {detector.id} not configured.'
            elif not detector.status:
                report.status = 'FAIL'
                report.status_extended = f'GuardDuty detector {detector.id} configured but suspended.'
            if report.status == 'FAIL' and (guardduty_client.audit_config.get('allowlist_non_default_regions', False) and (not detector.region == guardduty_client.region)):
