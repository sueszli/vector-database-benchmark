...
class Trivia(commands.Cog):
    ...
    async def triviaset_stopafter(self, ctx: commands.Context, seconds: finite_float):
        ...
        await ctx.send(_('Done. Trivia sessions will now time out after {num} seconds of no responses.').format(num=seconds))
    ...
    async def triviaset_allowoverride(self, ctx: commands.Context, enabled: bool):
        ...
        if enabled:
            await ctx.send(_('Done. Trivia lists can now override the trivia settings for this server.'))
        else:
            await ctx.send(_('Done. Trivia lists can no longer override the trivia settings for this server.'))
    ...
    async def custom_trivia_list(self, ctx: commands.Context):
        ...
        if not personal_lists:
            ...
            return
        if await ctx.embed_requested():
            await ctx.send(embed=discord.Embed(title=_('Uploaded trivia lists'), colour=await ctx.embed_colour(), description=', '.join(sorted(personal_lists))))
        else:
    ...
    async def trivia_upload(self, ctx: commands.Context):
        ...
        try:
            ...
            await ctx.send(_('There was an error parsing the trivia list. See logs for more info.'))
            LOG.exception('Custom Trivia file %s failed to upload', parsedfile.filename)
        except schema.SchemaError as exc:
            await ctx.send(_('The custom trivia list was not saved. The file does not follow the proper data format.\n{schema_error}').format(schema_error=box(format_schema_error(exc))))
    ...
    async def trivia(self, ctx: commands.Context, *categories: str):
        ...
        for category in reversed(categories):
            try:
                ...
                await ctx.send(_('Invalid category `{name}`. See `{prefix}trivia list` for a list of trivia categories.').format(name=category, prefix=ctx.clean_prefix))
            except InvalidListError:
                await ctx.send(_('There was an error parsing the trivia list for the `{name}` category. It may be formatted incorrectly.').format(name=category))
            else:
            ...
            return
        if not trivia_dict:
            await ctx.send(_('The trivia list was parsed successfully, however it appears to be empty!'))
            return
    ...
    async def trivia_list(self, ctx: commands.Context):
        ...
        if await ctx.embed_requested():
            await ctx.send(embed=discord.Embed(title=_('Available trivia lists'), colour=await ctx.embed_colour(), description=', '.join(sorted(lists))))
        else:
    ...
    @trivia.group(name='leaderboard', aliases=['lboard'], autohelp=False, invoke_without_command=True)
    ...
    @commands.guild_only()
    async def trivia_leaderboard_server(self, ctx: commands.Context, sort_by: str='wins', top: int=10):
        ...
        if key is None:
            await ctx.send(_('Unknown field `{field_name}`, see `{prefix}help trivia leaderboard server` for valid fields to sort by.').format(field_name=sort_by, prefix=ctx.clean_prefix))
            return
        guild = ctx.guild
    ...
    async def trivia_leaderboard_global(self, ctx: commands.Context, sort_by: str='wins', top: int=10):
    ...
    async def _save_trivia_list(self, ctx: commands.Context, attachment: discord.Attachment) -> None:
        ...
        if filename in self.trivia.all_commands or any((filename == item.stem for item in get_core_lists())):
        ...
        if file.exists():
            overwrite_message = _('{filename} already exists. Do you wish to overwrite?').format(filename=filename)
