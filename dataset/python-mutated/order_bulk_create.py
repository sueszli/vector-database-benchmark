...
from ....order import FulfillmentStatus, OrderEvents, OrderOrigin, OrderStatus, StockUpdatePolicy
...
from ...payment.mutations.transaction.transaction_create import TransactionCreate, TransactionCreateInput
from ...payment.utils import metadata_contains_empty_key
from ...plugins.dataloaders import get_plugin_manager_promise
from ..enums import OrderStatusEnum, StockUpdatePolicyEnum
from ..mutations.order_discount_common import OrderDiscountCommon, OrderDiscountCommonInput
...
class ModelIdentifiers:
    ...
    variant_external_references: ModelIdentifier = ModelIdentifier(model='ProductVariant')
...
class OrderBulkCreateUserInput(BaseInputObjectType):
    ...
    external_reference = graphene.String(description='Customer external ID associated with the order.')

    class Meta:
        doc_category = DOC_CATEGORY_ORDERS

class OrderBulkCreateInvoiceInput(BaseInputObjectType):
    created_at = graphene.DateTime(required=True, description='The date, when the invoice was created.')
    ...
    private_metadata = NonNullList(MetadataInput, description='Private metadata of the invoice.')
...
class OrderBulkCreateDeliveryMethodInput(BaseInputObjectType):
    ...
    shipping_method_name = graphene.String(description='The name of the shipping method.')
    shipping_price = graphene.Field(TaxedMoneyInput, description='The price of the shipping.')
    ...
    shipping_tax_class_metadata = NonNullList(MetadataInput, description='Metadata of the tax class.')
    shipping_tax_class_private_metadata = NonNullList(MetadataInput, description='Private metadata of the tax class.')

    class Meta:
        doc_category = DOC_CATEGORY_ORDERS

class OrderBulkCreateNoteInput(BaseInputObjectType):
    message = graphene.String(required=True, description=f'Note message. Max characters: {MAX_NOTE_LENGTH}.')
    ...
    user_external_reference = graphene.ID(description='The user external ID associated with the message.')
...
class OrderBulkCreateFulfillmentLineInput(BaseInputObjectType):
    ...
    variant_external_reference = graphene.String(description='The external ID of the product variant.')
    quantity = graphene.Int(description='The number of line items to be fulfilled from given warehouse.', required=True)
    warehouse = graphene.ID(description='ID of the warehouse from which the item will be fulfilled.', required=True)
    order_line_index = graphene.Int(required=True, description='0-based index of order line, which the fulfillment line refers to.')
...
class OrderBulkCreateFulfillmentInput(BaseInputObjectType):
    ...
    lines = NonNullList(OrderBulkCreateFulfillmentLineInput, description='List of items informing how to fulfill the order.')
...
class OrderBulkCreateOrderLineInput(BaseInputObjectType):
    ...
    translated_variant_name = graphene.String(description='Translation of the product variant name.')
    translated_product_name = graphene.String(description='Translation of the product name.')
    created_at = graphene.DateTime(required=True, description='The date, when the order line was created.')
    is_shipping_required = graphene.Boolean(required=True, description='Determines whether shipping of the order line items is required.')
    is_gift_card = graphene.Boolean(required=True, description='Gift card flag.')
    quantity = graphene.Int(required=True, description='Number of items in the order line')
    total_price = graphene.Field(TaxedMoneyInput, required=True, description='Price of the order line.')
    undiscounted_total_price = graphene.Field(TaxedMoneyInput, required=True, description='Price of the order line excluding applied discount.')
    warehouse = graphene.ID(required=True, description='The ID of the warehouse, where the line will be allocated.')
    metadata = NonNullList(MetadataInput, description='Metadata of the order line.')
    private_metadata = NonNullList(MetadataInput, description='Private metadata of the order line.')
    ...
    tax_class_metadata = NonNullList(MetadataInput, description='Metadata of the tax class.')
    tax_class_private_metadata = NonNullList(MetadataInput, description='Private metadata of the tax class.')
...
class OrderBulkCreateInput(BaseInputObjectType):
    ...
    channel = graphene.String(required=True, description='Slug of the channel associated with the order.')
    created_at = graphene.DateTime(required=True, description='The date, when the order was inserted to Saleor database.')
    status = OrderStatusEnum(description='Status of the order.')
    user = graphene.Field(OrderBulkCreateUserInput, required=True, description='Customer associated with the order.')
    billing_address = graphene.Field(AddressInput, required=True, description='Billing address of the customer.')
    shipping_address = graphene.Field(AddressInput, description='Shipping address of the customer.')
    ...
    private_metadata = NonNullList(MetadataInput, description='Private metadata of the order.')
    customer_note = graphene.String(description='Note about customer.')
    notes = NonNullList(OrderBulkCreateNoteInput, description='Notes related to the order.')
    language_code = graphene.Argument(LanguageCodeEnum, required=True, description='Order language code.')
    display_gross_prices = graphene.Boolean(description='Determines whether displayed prices should include taxes.')
    weight = WeightScalar(description='Weight of the order in kg.')
    redirect_url = graphene.String(description='URL of a view, where users should be redirected to see the order details.')
    lines = NonNullList(OrderBulkCreateOrderLineInput, required=True, description='List of order lines.')
    delivery_method = graphene.Field(OrderBulkCreateDeliveryMethodInput, description='The delivery method selected for this order.')
    gift_cards = NonNullList(graphene.String, description='List of gift card codes associated with the order.')
    voucher = graphene.String(description='Code of a voucher associated with the order.\n\nDEPRECATED: this field will be removed in Saleor 3.19. Use `voucherCode` instead.')
    voucher_code = graphene.String(description='Code of a voucher associated with the order.' + ADDED_IN_318)
    discounts = NonNullList(OrderDiscountCommonInput, description='List of discounts.')
    fulfillments = NonNullList(OrderBulkCreateFulfillmentInput, description='Fulfillments of the order.')
    transactions = NonNullList(TransactionCreateInput, description='Transactions related to the order.')
    invoices = NonNullList(OrderBulkCreateInvoiceInput, description='Invoices related to the order.')
...
class OrderBulkCreateResult(BaseObjectType):
    ...
    errors = NonNullList(OrderBulkCreateError, description='List of errors occurred on create attempt.')

    class Meta:
        doc_category = DOC_CATEGORY_ORDERS

class OrderBulkCreate(BaseMutation, I18nMixin):
    count = graphene.Int(required=True, default_value=0, description='Returns how many objects were created.')
    results = NonNullList(OrderBulkCreateResult, required=True, default_value=[], description='List of the created orders.')

    class Arguments:
        orders = NonNullList(OrderBulkCreateInput, required=True, description=f'Input list of orders to create. Orders limit: {MAX_ORDERS}.')
        error_policy = ErrorPolicyEnum(required=False, description='Policies of error handling. DEFAULT: ' + ErrorPolicyEnum.REJECT_EVERYTHING.name)
        stock_update_policy = StockUpdatePolicyEnum(required=False, description='Determine how stock should be updated, while processing the order. DEFAULT: UPDATE - Only do update, if there is enough stocks.')
