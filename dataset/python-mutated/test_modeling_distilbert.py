...
if is_torch_available():
    import torch
    from transformers import DISTILBERT_PRETRAINED_MODEL_ARCHIVE_LIST, DistilBertForMaskedLM, DistilBertForMultipleChoice, DistilBertForQuestionAnswering, DistilBertForSequenceClassification, DistilBertForTokenClassification, DistilBertModel
...
class DistilBertModelTester(object):
    ...
    def get_config(self):
        if False:
            for i in range(10):
                print('nop')
        return DistilBertConfig(vocab_size=self.vocab_size, dim=self.hidden_size, n_layers=self.num_hidden_layers, n_heads=self.num_attention_heads, hidden_dim=self.intermediate_size, hidden_act=self.hidden_act, dropout=self.hidden_dropout_prob, attention_dropout=self.attention_probs_dropout_prob, max_position_embeddings=self.max_position_embeddings, initializer_range=self.initializer_range)
...
class DistilBertModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (DistilBertModel, DistilBertForMaskedLM, DistilBertForMultipleChoice, DistilBertForQuestionAnswering, DistilBertForSequenceClassification, DistilBertForTokenClassification) if is_torch_available() else None
    ...
    def test_flash_attn_2_inference(self):
        ...
        import torch
        for model_class in self.all_model_classes:
            dummy_input = torch.LongTensor([[1, 2, 3, 4], [1, 2, 8, 9], [1, 2, 11, 12], [1, 2, 13, 14]]).to(torch_device)
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                self.assertTrue(torch.allclose(logits_fa, logits, atol=0.04, rtol=0.04))
                ...
                self.assertTrue(torch.allclose(logits_fa[1:], logits[1:], atol=0.04, rtol=0.04))
...
class DistilBertModelIntergrationTest(unittest.TestCase):
    ...
    def test_inference_no_head_absolute_embedding(self):
        ...
        expected_slice = torch.tensor([[[-0.1639, 0.3299, 0.1648], [-0.1746, 0.3289, 0.171], [-0.1884, 0.3357, 0.181]]])
