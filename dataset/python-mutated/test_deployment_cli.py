...
class TestUpdatingDeployments:
    ...
    async def flojo(self, prefect_client):
        ...
        deployment_id = await prefect_client.create_deployment(flow_id=flow_id, name='test-deployment', version='git-commit-hash', manifest_path='path/file.json', schedule=old_record, parameters={'foo': 'bar'}, tags=['foo', 'bar'], parameter_openapi_schema={})
...
class TestDeploymentRun:
    ...
    @pytest.mark.parametrize('given,expected', [('foo', 'foo'), ('"foo"', 'foo'), (1, 1), ('["one", "two"]', ['one', 'two']), ('{"key": "val"}', {'key': 'val'}), ('["one", 2]', ['one', 2]), ('{"key": 2}', {'key': 2})])
    async def test_passes_parameters_to_flow_run(self, deployment, deployment_name, prefect_client: PrefectClient, given, expected):
        """
        This test ensures the parameters are set on the created flow run and that
        data types are cast correctly.
        """
        await run_sync_in_worker_thread(invoke_and_assert, ['deployment', 'run', deployment_name, '--param', f'name={given}'])
        flow_runs = await prefect_client.read_flow_runs(deployment_filter=DeploymentFilter(id=DeploymentFilterId(any_=[deployment.id])))
        assert len(flow_runs) == 1
        flow_run = flow_runs[0]
        assert flow_run.parameters == {'name': expected}

    async def test_passes_parameters_from_stdin_to_flow_run(self, deployment, deployment_name, prefect_client):
        await run_sync_in_worker_thread(invoke_and_assert, ['deployment', 'run', deployment_name, '--params', '-'], json.dumps({'name': 'foo'}))
        flow_runs = await prefect_client.read_flow_runs(deployment_filter=DeploymentFilter(id=DeploymentFilterId(any_=[deployment.id])))
        assert len(flow_runs) == 1
        flow_run = flow_runs[0]
        assert flow_run.parameters == {'name': 'foo'}

    async def test_passes_parameters_from_dict_to_flow_run(self, deployment, deployment_name, prefect_client):
        await run_sync_in_worker_thread(invoke_and_assert, ['deployment', 'run', deployment_name, '--params', json.dumps({'name': 'foo'})])
        flow_runs = await prefect_client.read_flow_runs(deployment_filter=DeploymentFilter(id=DeploymentFilterId(any_=[deployment.id])))
        assert len(flow_runs) == 1
        flow_run = flow_runs[0]
