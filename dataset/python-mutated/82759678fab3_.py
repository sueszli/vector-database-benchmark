...
def upgrade():
    if False:
        i = 10
        return i + 15
    op.create_table('jupyter_image_builds', sa.Column('uuid', sa.String(length=36), nullable=False), sa.Column('requested_time', sa.DateTime(), nullable=False), sa.Column('started_time', sa.DateTime(), nullable=True), sa.Column('finished_time', sa.DateTime(), nullable=True), sa.Column('status', sa.String(length=15), nullable=True), sa.PrimaryKeyConstraint('uuid', name=op.f('pk_jupyter_image_builds')))
...
def downgrade():
    if False:
        i = 10
        return i + 15
    op.create_table('jupyter_builds', sa.Column('uuid', sa.VARCHAR(length=36), autoincrement=False, nullable=False), sa.Column('requested_time', postgresql.TIMESTAMP(), autoincrement=False, nullable=False), sa.Column('started_time', postgresql.TIMESTAMP(), autoincrement=False, nullable=True), sa.Column('finished_time', postgresql.TIMESTAMP(), autoincrement=False, nullable=True), sa.Column('status', sa.VARCHAR(length=15), autoincrement=False, nullable=True), sa.PrimaryKeyConstraint('uuid', name='pk_jupyter_builds'))
