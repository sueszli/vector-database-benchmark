...
class TestCallbackAnswerMiddleware:
    ...
    @pytest.mark.parametrize('properties,expected_stack', [[{'answered': False}, ['handler', 'answer']], [{'answered': True}, ['answer', 'handler']], [{'disabled': True}, ['handler']]])
    async def test_call(self, properties, expected_stack):
        ...
        async def answer(*args, **kwargs):
            stack.append('answer')
        middleware = CallbackAnswerMiddleware()
        with patch('aiogram.utils.callback_answer.CallbackAnswerMiddleware.construct_callback_answer', new_callable=MagicMock, side_effect=lambda **kwargs: CallbackAnswer(**{'answered': False, **properties})), patch('aiogram.utils.callback_answer.CallbackAnswerMiddleware.answer', new=answer):
