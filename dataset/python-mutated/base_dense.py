...
class Dense(Layer):
    ...
    def __init__(self, n_units, act=None, W_init=tl.initializers.truncated_normal(stddev=0.05), b_init=tl.initializers.constant(value=0.0), in_channels=None, name=None):
        if False:
            i = 10
            return i + 15
        ...
        if self.in_channels is not None:
            ...
            self._built = True
        logging.info('Dense  %s: %d %s' % (self.name, self.n_units, self.act.__name__ if self.act is not None else 'No Activation'))
