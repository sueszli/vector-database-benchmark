...
from ._types import NOT_GIVEN, Body, Omit, Query, ModelT, Headers, Timeout, NotGiven, ResponseT, Transport, AnyMapping, PostParser, ProxiesTypes, RequestFiles, AsyncTransport, RequestOptions, UnknownResponse, ModelBuilderProtocol, BinaryResponseContent
from ._utils import is_dict, is_given, is_mapping
from ._compat import model_copy, model_dump
from ._models import GenericModel, FinalRequestOptions, validate_type, construct_type
from ._response import APIResponse
from ._constants import DEFAULT_LIMITS, DEFAULT_TIMEOUT, DEFAULT_MAX_RETRIES, RAW_RESPONSE_HEADER
...
class AsyncAPIClient(BaseClient[httpx.AsyncClient, AsyncStream[Any]]):
    ...
    async def _retry_request(self, options: FinalRequestOptions, cast_to: Type[ResponseT], remaining_retries: int, response_headers: Optional[httpx.Headers]=None, *, stream: bool, stream_cls: type[_AsyncStreamT] | None) -> ResponseT | _AsyncStreamT:
        remaining = remaining_retries - 1
        timeout = self._calculate_retry_timeout(remaining, options, response_headers)
        log.info('Retrying request to %s in %f seconds', options.url, timeout)
...
class HttpxBinaryResponseContent(BinaryResponseContent):
    ...
    @override
    async def astream_to_file(self, file: str | os.PathLike[str], *, chunk_size: int | None=None) -> None:
