...
class PyShortcut:
    ...
    def __init__(self):
        if False:
            i = 10
            return i + 15
        self._base = pythoncom.CoCreateInstance(shell.CLSID_ShellLink, None, pythoncom.CLSCTX_INPROC_SERVER, shell.IID_IShellLink)
...
if __name__ == '__main__':
    ...
    if os.path.exists(file):
        shortcut.load(file)
        print('Shortcut in file %s to file:\n\t%s\nArguments:\n\t%s\nDescription:\n\t%s\nWorking Directory:\n\t%s\nItemIDs:\n\t<skipped>' % (file, shortcut.GetPath(shell.SLGP_SHORTPATH)[0], shortcut.GetArguments(), shortcut.GetDescription(), shortcut.GetWorkingDirectory()))
    else:
        if len(sys.argv) < 3:
