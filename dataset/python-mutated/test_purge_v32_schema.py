...
from tests.components.recorder.db_schema_32 import EventData, Events, RecorderRuns, StateAttributes, States, StatisticsRuns, StatisticsShortTerm
...
async def test_purge_old_states(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    ...
    with session_scope(hass=hass) as session:
        ...
        finished = purge_old_data(instance, purge_before, states_batch_size=1, events_batch_size=1, repack=False)
        assert not finished
...
async def test_purge_old_states_encouters_database_corruption(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant, recorder_db_url: str) -> None:
...
async def test_purge_old_states_encounters_temporary_mysql_error(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with patch('homeassistant.components.recorder.util.time.sleep') as sleep_mock, patch('homeassistant.components.recorder.purge._purge_old_recorder_runs', side_effect=[mysql_exception, None]), patch.object(instance.engine.dialect, 'name', 'mysql'):
        await hass.services.async_call(recorder.DOMAIN, SERVICE_PURGE, {'keep_days': 0})
        await hass.async_block_till_done()
        await async_wait_recording_done(hass)
        await async_wait_recording_done(hass)
...
async def test_purge_old_states_encounters_operational_error(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant, caplog: pytest.LogCaptureFixture) -> None:
...
async def test_purge_old_events(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    ...
    with session_scope(hass=hass) as session:
        ...
        purge_before = dt_util.utcnow() - timedelta(days=4)
        finished = purge_old_data(instance, purge_before, repack=False, events_batch_size=1, states_batch_size=1)
        assert not finished
...
async def test_purge_old_recorder_runs(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
...
async def test_purge_old_statistics_runs(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
...
async def test_purge_method(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant, caplog: pytest.LogCaptureFixture, use_sqlite: bool) -> None:
    """Test purge method."""
...
@pytest.mark.parametrize('use_sqlite', (True, False), indirect=True)
async def test_purge_edge_case(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant, use_sqlite: bool) -> None:
    ...
    async def _add_db_entries(hass: HomeAssistant, timestamp: datetime) -> None:
        with session_scope(hass=hass) as session:
            session.add(Events(event_id=1001, event_type='EVENT_TEST_PURGE', event_data='{}', origin='LOCAL', time_fired_ts=dt_util.utc_to_timestamp(timestamp)))
            session.add(States(entity_id='test.recorder2', state='purgeme', attributes='{}', last_changed_ts=dt_util.utc_to_timestamp(timestamp), last_updated_ts=dt_util.utc_to_timestamp(timestamp), event_id=1001, attributes_id=1002))
...
async def test_purge_cutoff_date(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    ...
    async def _add_db_entries(hass: HomeAssistant, cutoff: datetime, rows: int) -> None:
        ...
        with session_scope(hass=hass) as session:
            session.add(Events(event_id=1000, event_type='KEEP', event_data='{}', origin='LOCAL', time_fired_ts=dt_util.utc_to_timestamp(timestamp_keep)))
            session.add(States(entity_id='test.cutoff', state='keep', attributes='{}', last_changed_ts=dt_util.utc_to_timestamp(timestamp_keep), last_updated_ts=dt_util.utc_to_timestamp(timestamp_keep), event_id=1000, attributes_id=1000))
            session.add(StateAttributes(shared_attrs='{}', hash=1234, attributes_id=1000))
            for row in range(1, rows):
                session.add(Events(event_id=1000 + row, event_type='PURGE', event_data='{}', origin='LOCAL', time_fired_ts=dt_util.utc_to_timestamp(timestamp_purge)))
                session.add(States(entity_id='test.cutoff', state='purge', attributes='{}', last_changed_ts=dt_util.utc_to_timestamp(timestamp_purge), last_updated_ts=dt_util.utc_to_timestamp(timestamp_purge), event_id=1000 + row, attributes_id=1000 + row))
    ...
    with session_scope(hass=hass) as session:
        for _ in range(iterations):
            for event_id in range(6):
                ...
                session.add(Events(event_type=event_type, event_data=json.dumps(event_data), origin='LOCAL', time_fired_ts=dt_util.utc_to_timestamp(timestamp)))
    ...
    with session_scope(hass=hass) as session:
        for _ in range(iterations):
            for event_id in range(6):
                ...
                session.add(Events(event_type=event_type, origin='LOCAL', time_fired_ts=dt_util.utc_to_timestamp(timestamp), event_data_rel=event_data))
...
async def test_purge_many_old_events(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    """Test deleting old events."""
    instance = await async_setup_recorder_instance(hass)
    await async_attach_db_engine(hass)
    old_events_count = 5
    with patch.object(instance, 'max_bind_vars', old_events_count), patch.object(instance.database_engine, 'max_bind_vars', old_events_count):
        await _add_test_events(hass, old_events_count)
        with session_scope(hass=hass) as session:
            events = session.query(Events).filter(Events.event_type.like('EVENT_TEST%'))
            assert events.count() == old_events_count * 6
            purge_before = dt_util.utcnow() - timedelta(days=4)
            finished = purge_old_data(instance, purge_before, repack=False, states_batch_size=3, events_batch_size=3)
            assert not finished
            assert events.count() == old_events_count * 3
            finished = purge_old_data(instance, purge_before, repack=False, states_batch_size=3, events_batch_size=3)
            assert finished
            assert events.count() == old_events_count * 2
            finished = purge_old_data(instance, dt_util.utcnow(), repack=False, states_batch_size=20, events_batch_size=20)
            assert finished
            assert events.count() == 0

async def test_purge_can_mix_legacy_and_new_format(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    ...
    with session_scope(hass=hass) as session:
        ...
        finished = purge_old_data(instance, purge_before, repack=False, events_batch_size=100, states_batch_size=100)
        assert finished
        assert states_with_event_id.count() == 0
        assert states_without_event_id.count() == 1
        _add_state_without_event_linkage(session, 'switch.random', 'on', eleven_days_ago)
...
async def test_purge_can_mix_legacy_and_new_format_with_detached_state(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant, recorder_db_url: str) -> None:
...
async def test_purge_entities_keep_days(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant) -> None:
    ...
    await async_recorder_block_till_done(hass)
    states = await instance.async_add_executor_job(get_significant_states, hass, one_month_ago, None, ['sensor.keep', 'sensor.purge'])
    assert len(states['sensor.keep']) == 2
    assert len(states['sensor.purge']) == 3
    await hass.services.async_call(recorder.DOMAIN, SERVICE_PURGE_ENTITIES, {'entity_id': 'sensor.purge', 'keep_days': 1})
    await async_recorder_block_till_done(hass)
    await async_wait_purge_done(hass)
    states = await instance.async_add_executor_job(get_significant_states, hass, one_month_ago, None, ['sensor.keep', 'sensor.purge'])
    assert len(states['sensor.keep']) == 2
    assert len(states['sensor.purge']) == 1
    await hass.services.async_call(recorder.DOMAIN, SERVICE_PURGE_ENTITIES, {'entity_id': 'sensor.purge'})
    await async_recorder_block_till_done(hass)
    await async_wait_purge_done(hass)
    states = await instance.async_add_executor_job(get_significant_states, hass, one_month_ago, None, ['sensor.keep', 'sensor.purge'])
    assert len(states['sensor.keep']) == 2
