...
class NdArray(BaseModel):
    ...
    array: Union[_1DArray, _2DArray, _3DArray, _4DArray] = Field(..., description='The array content as a nested list. You can pass in 1D to 4D array as nested list, or flatten them. When you flatten the array, you can use the `shape` parameter to perform reshaping.')
    shape: Optional[List[int]] = Field(default=None, description='The shape of the array. If present, the array will be reshaped.')
    dtype: Optional[str] = Field(default=None, description='The numpy dtype of the array. If present, the array will be cast by `astype`.')
...
@Deprecated(DAG_DEPRECATION_MESSAGE)
async def starlette_request(request: starlette.requests.Request) -> starlette.requests.Request:
