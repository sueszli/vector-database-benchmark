...
class LocalSource(MediaSource):
    ...
    async def async_browse_media(self, item: MediaSourceItem) -> BrowseMediaSource:
        ...
        result = await self.hass.async_add_executor_job(self._browse_media, source_dir_id, location)
        return result
...
class UploadMediaView(http.HomeAssistantView):
    ...
    async def post(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            raise web.HTTPBadRequest() from err
        try:
            await self.hass.async_add_executor_job(self._move_file, self.source.async_full_path(source_dir_id, location), uploaded_file)
        except ValueError as err:
            LOGGER.error('Moving upload failed: %s', err)
            raise web.HTTPBadRequest() from err
        return self.json({'media_content_id': f"{data['media_content_id']}/{uploaded_file.filename}"})
...
@websocket_api.websocket_command({vol.Required('type'): 'media_source/local_source/remove', vol.Required('media_content_id'): str})
