...
from sqlglot.dialects.dialect import Dialect, any_value_to_max_sql, generatedasidentitycolumnconstraint_sql, max_or_greatest, min_or_least, parse_date_delta, rename_func, timestrtotime_sql, ts_or_ds_to_date_sql
...
class TSQL(Dialect):
    ...
    TYPED_DIVISION = True
    TIME_MAPPING = {'year': '%Y', 'qq': '%q', 'q': '%q', 'quarter': '%q', 'dayofyear': '%j', 'day': '%d', 'dy': '%d', 'y': '%Y', 'week': '%W', 'ww': '%W', 'wk': '%W', 'hour': '%h', 'hh': '%I', 'minute': '%M', 'mi': '%M', 'n': '%M', 'second': '%S', 'ss': '%S', 's': '%-S', 'millisecond': '%f', 'ms': '%f', 'weekday': '%W', 'dw': '%W', 'month': '%m', 'mm': '%M', 'm': '%-M', 'Y': '%Y', 'YYYY': '%Y', 'YY': '%y', 'MMMM': '%B', 'MMM': '%b', 'MM': '%m', 'M': '%-m', 'dddd': '%A', 'dd': '%d', 'd': '%-d', 'HH': '%H', 'H': '%-H', 'h': '%-I', 'S': '%f', 'yyyy': '%Y', 'yy': '%y'}
    ...
    class Tokenizer(tokens.Tokenizer):
        ...
        QUOTES = ["'", '"']
        HEX_STRINGS = [('0x', ''), ('0X', '')]
        KEYWORDS = {**tokens.Tokenizer.KEYWORDS, 'DATETIME2': TokenType.DATETIME, 'DATETIMEOFFSET': TokenType.TIMESTAMPTZ, 'DECLARE': TokenType.COMMAND, 'IMAGE': TokenType.IMAGE, 'MONEY': TokenType.MONEY, 'NTEXT': TokenType.TEXT, 'NVARCHAR(MAX)': TokenType.TEXT, 'PRINT': TokenType.COMMAND, 'PROC': TokenType.PROCEDURE, 'REAL': TokenType.FLOAT, 'ROWVERSION': TokenType.ROWVERSION, 'SMALLDATETIME': TokenType.DATETIME, 'SMALLMONEY': TokenType.SMALLMONEY, 'SQL_VARIANT': TokenType.VARIANT, 'TOP': TokenType.TOP, 'UNIQUEIDENTIFIER': TokenType.UNIQUEIDENTIFIER, 'UPDATE STATISTICS': TokenType.COMMAND, 'VARCHAR(MAX)': TokenType.TEXT, 'XML': TokenType.XML, 'OUTPUT': TokenType.RETURNING, 'SYSTEM_USER': TokenType.CURRENT_USER, 'FOR SYSTEM_TIME': TokenType.TIMESTAMP_SNAPSHOT}

    class Parser(parser.Parser):
        SET_REQUIRES_ASSIGNMENT_DELIMITER = False
        FUNCTIONS = {**parser.Parser.FUNCTIONS, 'CHARINDEX': lambda args: exp.StrPosition(this=seq_get(args, 1), substr=seq_get(args, 0), position=seq_get(args, 2)), 'DATEADD': parse_date_delta(exp.DateAdd, unit_mapping=DATE_DELTA_INTERVAL), 'DATEDIFF': _parse_date_delta(exp.DateDiff, unit_mapping=DATE_DELTA_INTERVAL), 'DATENAME': _format_time_lambda(exp.TimeToStr, full_format_mapping=True), 'DATEPART': _format_time_lambda(exp.TimeToStr), 'EOMONTH': _parse_eomonth, 'FORMAT': _parse_format, 'GETDATE': exp.CurrentTimestamp.from_arg_list, 'HASHBYTES': _parse_hashbytes, 'IIF': exp.If.from_arg_list, 'ISNULL': exp.Coalesce.from_arg_list, 'JSON_VALUE': exp.JSONExtractScalar.from_arg_list, 'LEN': exp.Length.from_arg_list, 'REPLICATE': exp.Repeat.from_arg_list, 'SQUARE': lambda args: exp.Pow(this=seq_get(args, 0), expression=exp.Literal.number(2)), 'SYSDATETIME': exp.CurrentTimestamp.from_arg_list, 'SUSER_NAME': exp.CurrentUser.from_arg_list, 'SUSER_SNAME': exp.CurrentUser.from_arg_list, 'SYSTEM_USER': exp.CurrentUser.from_arg_list}
        JOIN_HINTS = {'LOOP', 'HASH', 'MERGE', 'REMOTE'}
        VAR_LENGTH_DATATYPES = {DataType.Type.NVARCHAR, DataType.Type.VARCHAR, DataType.Type.CHAR, DataType.Type.NCHAR}
    ...
    class Generator(generator.Generator):
        ...
        TRANSFORMS = {**generator.Generator.TRANSFORMS, exp.AnyValue: any_value_to_max_sql, exp.AutoIncrementColumnConstraint: lambda *_: 'IDENTITY', exp.DateAdd: generate_date_delta_with_unit_sql, exp.DateDiff: generate_date_delta_with_unit_sql, exp.CTE: transforms.preprocess([qualify_derived_table_outputs]), exp.CurrentDate: rename_func('GETDATE'), exp.CurrentTimestamp: rename_func('GETDATE'), exp.Extract: rename_func('DATEPART'), exp.GeneratedAsIdentityColumnConstraint: generatedasidentitycolumnconstraint_sql, exp.GroupConcat: _string_agg_sql, exp.If: rename_func('IIF'), exp.Length: rename_func('LEN'), exp.Max: max_or_greatest, exp.MD5: lambda self, e: self.func('HASHBYTES', exp.Literal.string('MD5'), e.this), exp.Min: min_or_least, exp.NumberToStr: _format_sql, exp.Select: transforms.preprocess([transforms.eliminate_distinct_on, transforms.eliminate_semi_and_anti_joins, transforms.eliminate_qualify]), exp.Subquery: transforms.preprocess([qualify_derived_table_outputs]), exp.SHA: lambda self, e: self.func('HASHBYTES', exp.Literal.string('SHA1'), e.this), exp.SHA2: lambda self, e: self.func('HASHBYTES', exp.Literal.string(f"SHA2_{e.args.get('length', 256)}"), e.this), exp.TemporaryProperty: lambda self, e: '', exp.TimeStrToTime: timestrtotime_sql, exp.TimeToStr: _format_sql, exp.TsOrDsToDate: ts_or_ds_to_date_sql('tsql')}
