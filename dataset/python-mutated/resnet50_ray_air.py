...
from tf_utils import DEFAULT_IMAGE_SIZE, NUM_CHANNELS, preprocess_image, build_tf_dataset
from metric_utils import determine_if_memory_monitor_is_enabled_in_latest_session, get_ray_spilled_and_restored_mb, MaxMemoryUtilizationTracker
...
APPROX_PREPROCESS_IMAGE_BYTES = 6 * 100000.0
...
def build_dataset(data_root, num_images_per_epoch, num_images_per_input_file, batch_size, read_from_images=True):
    ...
    return ds
FIELDS = ['data_loader', 'train_sleep_time_ms', 'num_cpu_nodes', 'num_epochs', 'num_images_per_epoch', 'num_images_per_input_file', 'num_files', 'batch_size', 'shuffle_buffer_size', 'ray_mem_monitor_enabled', 'ray_spilled_mb', 'ray_restored_mb', 'min_available_mb', 'time_total_s', 'tput_images_per_s', 'all_epoch_times_s', 'all_throughputs_imgs_s']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-root', default=None, type=str, help='Directory path with TFRecords. Filenames should start with "train".')
    ...
    parser.add_argument('--num-images-per-input-file', default=1, type=int, help='Estimated number of images per input TFRecord file. Used to determine how many files to load.If you receive an error about too few rows, lower this value.')
    parser.add_argument('--num-images-per-epoch', default=100, type=int)
    parser.add_argument('--num-epochs', default=2, type=int)
    parser.add_argument('--batch-size', default=1, type=int)
    parser.add_argument('--train-sleep-time-ms', default=-1, type=int, help='If set to >= 0, use an empty trainer that sleeps this many ms per batch.')
    parser.add_argument('--shuffle-buffer-size', default=0, type=int, help="Size of each Train worker's local shuffle buffer. Default value taken from MLPerf reference implementation.")
    parser.add_argument('--trainer-resources-cpu', default=1, type=int, help='CPU resources requested per trainer instance. Defaults to 1.')
    parser.add_argument('--tune-trials', default=0, type=int, help='Number of Tune trials to run. Defaults to 0, which disables Tune and executes a Trainer instance directly.')
    ...
    if args.use_tf_data or args.use_ray_data or args.use_torch:
        assert args.data_root is not None, '--use-tf-data, --use-ray-data, and --use-torch require a --data-root directory for TFRecord files'
    ...
    if args.synthetic_data:
        ...
        datasets['train'] = build_dataset(args.data_root, args.num_images_per_epoch, args.num_images_per_input_file, args.batch_size, args.from_images)
        train_loop_config['data_loader'] = RAY_DATA
    trainer = TensorflowTrainer(train_loop_for_worker, scaling_config=ScalingConfig(num_workers=1, use_gpu=args.use_gpu, trainer_resources={'CPU': args.trainer_resources_cpu}), datasets=datasets, dataset_config=ray.train.DataConfig(execution_options=options), train_loop_config=train_loop_config)
    tuner = None
    if args.tune_trials > 0:
        tuner = Tuner(trainer, param_space={'train_loop_config': {'random_var': tune.grid_search(range(1, args.tune_trials + 1))}}, tune_config=tune.TuneConfig(metric='time_total_s', mode='max', num_samples=1))
    result = {}
    exc = None
    ...
    if args.from_images:
        ...
        result['num_files'] = len(get_tfrecords_filenames(train_loop_config['data_root'], train_loop_config['num_images_per_epoch'], train_loop_config['num_images_per_input_file']))
    try:
