...
from golem.network.hyperdrive.client import HyperdriveAsyncClient, HyperdriveClient, HyperdriveClientOptions
from tests.factories.hyperdrive import hyperdrive_client_kwargs
response = {'id': str(uuid.uuid4()), 'version': '0.2.4', 'files': ['file1', 'file2'], 'hash': str(uuid.uuid4()), 'addresses': dict(TCP=dict(address='0.0.0.0', port=3282))}
response_str = json.dumps(response)

@mock.patch('golem.network.hyperdrive.client.requests.post', return_value=mock.Mock(text=response_str, content=response_str.encode()))
