...
from sklearn.metrics._pairwise_distances_reduction import ArgKmin, ArgKminClassMode, BaseDistancesReductionDispatcher, RadiusNeighbors, RadiusNeighborsClassMode, sqeuclidean_row_norms
from sklearn.utils._testing import assert_allclose, assert_array_equal, create_memmap_backed_data
from sklearn.utils.fixes import CSR_CONTAINERS
CDIST_PAIRWISE_DISTANCES_REDUCTION_COMMON_METRICS = ['braycurtis', 'canberra', 'chebyshev', 'cityblock', 'euclidean', 'minkowski', 'seuclidean']
...
def assert_compatible_radius_results(neighbors_dists_a, neighbors_dists_b, neighbors_indices_a, neighbors_indices_b, radius, check_sorted=True, rtol=1e-05, atol=1e-06):
    ...
    for query_idx in range(n_queries):
        ...
        assert_no_missing_neighbors(query_idx, dist_row_a, dist_row_b, indices_row_a, indices_row_b, threshold)
FLOAT32_TOLS = {'atol': 1e-07, 'rtol': 1e-05}
FLOAT64_TOLS = {'atol': 1e-09, 'rtol': 1e-07}
ASSERT_RESULT = {(ArgKmin, np.float64): partial(assert_compatible_argkmin_results, **FLOAT64_TOLS), (ArgKmin, np.float32): partial(assert_compatible_argkmin_results, **FLOAT32_TOLS), (RadiusNeighbors, np.float64): partial(assert_compatible_radius_results, **FLOAT64_TOLS), (RadiusNeighbors, np.float32): partial(assert_compatible_radius_results, **FLOAT32_TOLS)}
