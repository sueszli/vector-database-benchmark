...
import socket
import sys
from typing import TYPE_CHECKING, Any, Iterable, NoReturn, TypeVar, Union, overload
import attr
import trio
...
class FakeSocket(trio.socket.SocketType, metaclass=NoPublicConstructor):
    ...
    async def _resolve_address_nocp(self, address: object, *, local: bool) -> tuple[str, int]:
        return await trio._socket._resolve_address_nocp(self.type, self.family, self.proto, address=address, ipv6_v6only=False, local=local)
    ...
    async def _sendmsg(self, buffers: Iterable[Buffer], ancdata: Iterable[tuple[int, int, Buffer]]=(), flags: int=0, address: Any | None=None) -> int:
        self._check_closed()
        await trio.lowlevel.checkpoint()
        ...
        packet = UDPPacket(source=self._binding.local, destination=destination, payload=payload)
        self._fake_net.send_packet(packet)
        return len(payload)
    if sys.platform != 'win32' or (not TYPE_CHECKING and hasattr(socket.socket, 'sendmsg')):
        sendmsg = _sendmsg

    async def _recvmsg_into(self, buffers: Iterable[Buffer], ancbufsize: int=0, flags: int=0) -> tuple[int, list[tuple[int, int, bytes]], int, Any]:
        if ancbufsize != 0:
            raise NotImplementedError("FakeNet doesn't support ancillary data")
        if flags != 0:
            raise NotImplementedError("FakeNet doesn't support any recv flags")
        if self._binding is None:
            raise NotImplementedError('The code will most likely hang if you try to receive on a fakesocket without a binding. If that is not the case, or you explicitly want to test that, remove this warning.')
        self._check_closed()
    ...
    @overload
    async def sendto(self, __data: Buffer, __flags: int, __address: tuple[object, ...] | str | None | Buffer) -> int:
        ...
    ...
    async def recvfrom_into(self, buf: Buffer, nbytes: int=0, flags: int=0) -> tuple[int, Any]:
        if nbytes != 0 and nbytes != memoryview(buf).nbytes:
            raise NotImplementedError('partial recvfrom_into')
        (got_nbytes, ancdata, msg_flags, address) = await self._recvmsg_into([buf], 0, flags)
        return (got_nbytes, address)

    async def _recvmsg(self, bufsize: int, ancbufsize: int=0, flags: int=0) -> tuple[bytes, list[tuple[int, int, bytes]], int, Any]:
        buf = bytearray(bufsize)
        (got_nbytes, ancdata, msg_flags, address) = await self._recvmsg_into([buf], ancbufsize, flags)
