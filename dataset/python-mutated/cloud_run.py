...
class CloudRunJobFinishedTrigger(BaseTrigger):
    ...
    def __init__(self, operation_name: str, job_name: str, project_id: str | None, location: str=DEFAULT_BATCH_LOCATION, gcp_conn_id: str='google_cloud_default', impersonation_chain: str | Sequence[str] | None=None, polling_period_seconds: float=10, timeout: float | None=None):
        if False:
            for i in range(10):
                print('nop')
        super().__init__()
    ...
    async def run(self) -> AsyncIterator[TriggerEvent]:
        ...
        while timeout is None or timeout > 0:
            ...
            if operation.done:
                if operation.response is not None:
                    yield TriggerEvent({'status': RunJobStatus.SUCCESS, 'job_name': self.job_name})
                else:
                    yield TriggerEvent({'status': RunJobStatus.FAIL, 'operation_error_code': operation.error.code, 'operation_error_message': operation.error.message, 'job_name': self.job_name})
        ...
        yield TriggerEvent({'status': RunJobStatus.TIMEOUT, 'job_name': self.job_name})

    def _get_async_hook(self) -> CloudRunAsyncHook:
        if False:
            for i in range(10):
                print('nop')
        return CloudRunAsyncHook(gcp_conn_id=self.gcp_conn_id, impersonation_chain=self.impersonation_chain)