...
class TetsBaseSinger(BaseSingerSource):
    tap_cmd = ''
USER_STREAM = {'type': 'SCHEMA', 'stream': 'users', 'schema': {'properties': {'id': {'type': 'integer'}, 'name': {'type': 'string'}, 'updated_at': {'type': 'string', 'format': 'date-time'}}}, 'key_properties': ['id'], 'bookmark_properties': ['updated_at']}
ROLES_STREAM = {'type': 'SCHEMA', 'stream': 'roles', 'schema': {'properties': {'name': {'type': 'string'}}}, 'key_properties': ['name'], 'bookmark_properties': ['updated_at'], 'metadata': [{'metadata': {'inclusion': 'available', 'table-key-properties': ['id'], 'selected': True, 'valid-replication-keys': ['name'], 'schema-name': 'roles'}, 'breadcrumb': []}]}
