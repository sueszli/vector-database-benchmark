    ...
    def makedict(**kw):
        if False:
            while True:
                i = 10
        return kw
    default_settings = makedict(chdir=None, config_file=None, initial_commands=[], key_location='bottom left', orig_cwd=os.getcwd(), outdir=None, prefix='', python='"%s"' % sys.executable, redirect=redirect_to_file, scons=None, scons_flags='--debug=count --debug=memory --debug=time --debug=memoizer', scons_lib_dir=None, scons_wrapper=None, startup_targets='--help', subdir=None, subversion_url=None, svn='svn', svn_co_flag='-q', tar='tar', targets='', targets0=None, targets1=None, targets2=None, title=None, unzip='unzip', verbose=False, vertical_bars=[], unpack_map={'.tar.gz': (untar, '%(tar)s xzf %%s'), '.tgz': (untar, '%(tar)s xzf %%s'), '.tar': (untar, '%(tar)s xf %%s'), '.zip': (unzip, '%(unzip)s %%s')})
    run_titles = ['Startup', 'Full build', 'Up-to-date build']
    run_commands = ['%(python)s %(scons_wrapper)s %(scons_flags)s --profile=%(prof0)s %(targets0)s', '%(python)s %(scons_wrapper)s %(scons_flags)s --profile=%(prof1)s %(targets1)s', '%(python)s %(scons_wrapper)s %(scons_flags)s --profile=%(prof2)s %(targets2)s']
    stages = ['pre-read', 'post-read', 'pre-build', 'post-build']
    stage_strings = {'pre-read': 'Memory before reading SConscript files:', 'post-read': 'Memory after reading SConscript files:', 'pre-build': 'Memory before building targets:', 'post-build': 'Memory after building targets:'}
    memory_string_all = 'Memory '
    default_stage = stages[-1]
    time_strings = {'total': 'Total build time', 'SConscripts': 'Total SConscript file execution time', 'SCons': 'Total SCons execution time', 'commands': 'Total command execution time'}
