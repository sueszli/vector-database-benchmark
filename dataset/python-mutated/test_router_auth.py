...
class TestLogin:
    ...
    async def test_empty_body(self, path, test_app_client: Tuple[httpx.AsyncClient, bool], user_manager):
        (client, _) = test_app_client
        response = await client.post(path, data={})
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
        assert user_manager.on_after_login.called is False

    async def test_missing_username(self, path, test_app_client: Tuple[httpx.AsyncClient, bool], user_manager):
        (client, _) = test_app_client
    ...
    async def test_missing_password(self, path, test_app_client: Tuple[httpx.AsyncClient, bool], user_manager):
        (client, _) = test_app_client
    ...
    async def test_not_existing_user(self, path, test_app_client: Tuple[httpx.AsyncClient, bool], user_manager):
        (client, _) = test_app_client
    ...
    async def test_wrong_password(self, path, test_app_client: Tuple[httpx.AsyncClient, bool], user_manager):
        (client, _) = test_app_client
    ...
    async def test_valid_credentials_unverified(self, path, email, test_app_client: Tuple[httpx.AsyncClient, bool], user_manager, user: UserModel):
        ...
        if requires_verification:
            ...
            assert response.json() == {'access_token': str(user.id), 'token_type': 'bearer'}
            assert user_manager.on_after_login.called is True

    @pytest.mark.parametrize('email', ['lake.lady@camelot.bt', 'Lake.Lady@camelot.bt'])
    async def test_valid_credentials_verified(self, path, email, test_app_client: Tuple[httpx.AsyncClient, bool], user_manager, verified_user: UserModel):
        (client, _) = test_app_client
        data = {'username': email, 'password': 'excalibur'}
        response = await client.post(path, data=data)
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == {'access_token': str(verified_user.id), 'token_type': 'bearer'}
        assert user_manager.on_after_login.called is True
        (args, kwargs) = user_manager.on_after_login.call_args
        assert len(args) == 3
        assert all((x is not None for x in args))

    async def test_inactive_user(self, path, test_app_client: Tuple[httpx.AsyncClient, bool], user_manager):
        (client, _) = test_app_client
...
class TestLogout:
    ...
    async def test_missing_token(self, path, test_app_client: Tuple[httpx.AsyncClient, bool]):
        (client, _) = test_app_client
        response = await client.post(path)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    async def test_valid_credentials_unverified(self, mocker, path, test_app_client: Tuple[httpx.AsyncClient, bool], user: UserModel):
        (client, requires_verification) = test_app_client
        response = await client.post(path, headers={'Authorization': f'Bearer {user.id}'})
        if requires_verification:
            assert response.status_code == status.HTTP_403_FORBIDDEN
        else:
            assert response.status_code == status.HTTP_200_OK

    async def test_valid_credentials_verified(self, mocker, path, test_app_client: Tuple[httpx.AsyncClient, bool], verified_user: UserModel):
        (client, _) = test_app_client
        response = await client.post(path, headers={'Authorization': f'Bearer {verified_user.id}'})
        assert response.status_code == status.HTTP_200_OK
