...
class MobileNetV1ModelTester:
    ...
    def __init__(self, parent, batch_size=13, num_channels=3, image_size=32, depth_multiplier=0.25, min_depth=8, tf_padding=True, last_hidden_size=1024, output_stride=32, hidden_act='relu6', classifier_dropout_prob=0.1, initializer_range=0.02, is_training=True, use_labels=True, num_labels=10, scope=None):
        if False:
            i = 10
            return i + 15
        self.parent = parent
    ...
    def get_config(self):
        if False:
            while True:
                i = 10
        return MobileNetV1Config(num_channels=self.num_channels, image_size=self.image_size, depth_multiplier=self.depth_multiplier, min_depth=self.min_depth, tf_padding=self.tf_padding, hidden_act=self.hidden_act, classifier_dropout_prob=self.classifier_dropout_prob, initializer_range=self.initializer_range)
...
class MobileNetV1ModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'feature-extraction': MobileNetV1Model, 'image-classification': MobileNetV1ForImageClassification} if is_torch_available() else {}
    test_pruning = False
...
class MobileNetV1ModelIntegrationTest(unittest.TestCase):
    ...
    def default_image_processor(self):
        if False:
            i = 10
            return i + 15
        return MobileNetV1ImageProcessor.from_pretrained('google/mobilenet_v1_1.0_224') if is_vision_available() else None
