...
parser.add_argument('--stop-reward', type=float, default=90.0, help='Reward at which we stop training.')
...
if __name__ == '__main__':
    ...
    config = PPOConfig().environment(args.env, env_config={'repeat_delay': 2}).framework(args.framework).rollouts(num_rollout_workers=0, num_envs_per_worker=20).training(model={'vf_share_layers': False, 'use_lstm': True, 'lstm_cell_size': 256, 'fcnet_hiddens': [256]}, gamma=0.9, entropy_coeff=0.001, vf_loss_coeff=1e-05).rl_module(rl_module_spec=SingleAgentRLModuleSpec(catalog_class=CustomPPOCatalog)).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
