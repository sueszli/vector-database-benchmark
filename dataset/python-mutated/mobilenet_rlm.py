...
config = PPOConfig().experimental(_enable_new_api_stack=True).rl_module(rl_module_spec=SingleAgentRLModuleSpec(module_class=MobileNetTorchPPORLModule)).environment(RandomEnv, env_config={'action_space': gym.spaces.Discrete(2), 'observation_space': gym.spaces.Box(0.0, 1.0, shape=MOBILENET_INPUT_SHAPE, dtype=np.float32)}).rollouts(num_rollout_workers=0).training(train_batch_size=32, sgd_minibatch_size=16, num_sgd_iter=1)
config.build().train()