...
class TransactionWorkerStore(CacheInvalidationWorkerStore):
    ...
    async def _cleanup_transactions(self) -> None:
        ...
        await self.db_pool.runInteraction('_cleanup_transactions', _cleanup_transactions_txn)

    async def get_received_txn_response(self, transaction_id: str, origin: str) -> Optional[Tuple[int, JsonDict]]:
        ...
        Returns:
            ...
            2-tuple of (int, dict)
        """
        return await self.db_pool.runInteraction('get_received_txn_response', self._get_received_txn_response, transaction_id, origin)
    ...
    async def set_received_txn_response(self, transaction_id: str, origin: str, code: int, response_dict: JsonDict) -> None:
        ...
        """
        await self.db_pool.simple_upsert(table='received_transactions', keyvalues={'transaction_id': transaction_id, 'origin': origin}, values={}, insertion_values={'response_code': code, 'response_json': db_binary_type(encode_canonical_json(response_dict)), 'ts': self._clock.time_msec()}, desc='set_received_txn_response')

    @cached(max_entries=10000)
    async def get_destination_retry_timings(self, destination: str) -> Optional[DestinationRetryTimings]:
        ...
        """
        result = await self.db_pool.runInteraction('get_destination_retry_timings', self._get_destination_retry_timings, destination)
        return result
    ...
    @cachedList(cached_method_name='get_destination_retry_timings', list_name='destinations')
    async def get_destination_retry_timings_batch(self, destinations: StrCollection) -> Mapping[str, Optional[DestinationRetryTimings]]:
        rows = cast(List[Tuple[str, Optional[int], Optional[int], Optional[int]]], await self.db_pool.simple_select_many_batch(table='destinations', iterable=destinations, column='destination', retcols=('destination', 'failure_ts', 'retry_last_ts', 'retry_interval'), desc='get_destination_retry_timings_batch'))
        return {destination: DestinationRetryTimings(failure_ts, retry_last_ts, retry_interval) for (destination, failure_ts, retry_last_ts, retry_interval) in rows if retry_last_ts and failure_ts and retry_interval}

    async def set_destination_retry_timings(self, destination: str, failure_ts: Optional[int], retry_last_ts: int, retry_interval: int) -> None:
        ...
        """
        await self.db_pool.runInteraction('set_destination_retry_timings', self._set_destination_retry_timings_txn, destination, failure_ts, retry_last_ts, retry_interval, db_autocommit=True)
    ...
    async def store_destination_rooms_entries(self, destinations: Iterable[str], room_id: str, stream_ordering: int) -> None:
        """
        ...
        """
        await self.db_pool.simple_upsert_many(table='destinations', key_names=('destination',), key_values=[(d,) for d in destinations], value_names=[], value_values=[], desc='store_destination_rooms_entries_dests')
        rows = [(destination, room_id) for destination in destinations]
        await self.db_pool.simple_upsert_many(table='destination_rooms', key_names=('destination', 'room_id'), key_values=rows, value_names=['stream_ordering'], value_values=[(stream_ordering,)] * len(rows), desc='store_destination_rooms_entries_rooms')

    async def get_destination_last_successful_stream_ordering(self, destination: str) -> Optional[int]:
        """
        ...
        """
        return await self.db_pool.simple_select_one_onecol('destinations', {'destination': destination}, 'last_successful_stream_ordering', allow_none=True, desc='get_last_successful_stream_ordering')

    async def set_destination_last_successful_stream_ordering(self, destination: str, last_successful_stream_ordering: int) -> None:
        """
        ...
        """
        await self.db_pool.simple_upsert('destinations', keyvalues={'destination': destination}, values={'last_successful_stream_ordering': last_successful_stream_ordering}, desc='set_last_successful_stream_ordering')

    async def get_catch_up_room_event_ids(self, destination: str, last_successful_stream_ordering: int) -> List[str]:
        """
        ...
        Returns:
            list of event_ids
        """
        return await self.db_pool.runInteraction('get_catch_up_room_event_ids', self._get_catch_up_room_event_ids_txn, destination, last_successful_stream_ordering)
    ...
    async def get_catch_up_outstanding_destinations(self, after_destination: Optional[str]) -> List[str]:
        """
        ...
        return await self.db_pool.runInteraction('get_catch_up_outstanding_destinations', self._get_catch_up_outstanding_destinations_txn, time, after_destination)
    ...
    async def get_destinations_paginate(self, start: int, limit: int, destination: Optional[str]=None, order_by: str=DestinationSortOrder.DESTINATION.value, direction: Direction=Direction.FORWARDS) -> Tuple[List[Tuple[str, Optional[int], Optional[int], Optional[int], Optional[int]]], int]:
        ...
        return await self.db_pool.runInteraction('get_destinations_paginate_txn', get_destinations_paginate_txn)

    async def get_destination_rooms_paginate(self, destination: str, start: int, limit: int, direction: Direction=Direction.FORWARDS) -> Tuple[List[Tuple[str, int]], int]:
        ...
        def get_destination_rooms_paginate_txn(txn: LoggingTransaction) -> Tuple[List[Tuple[str, int]], int]:
            ...
            return (rooms, count)
        return await self.db_pool.runInteraction('get_destination_rooms_paginate_txn', get_destination_rooms_paginate_txn)

    async def is_destination_known(self, destination: str) -> bool:
        """Check if a destination is known to the server."""
        result = await self.db_pool.simple_select_one_onecol(table='destinations', keyvalues={'destination': destination}, retcol='1', allow_none=True, desc='is_destination_known')
        return bool(result)