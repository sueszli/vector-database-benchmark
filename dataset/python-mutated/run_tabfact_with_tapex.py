...
import transformers
from transformers import AutoConfig, BartForSequenceClassification, DataCollatorWithPadding, EvalPrediction, HfArgumentParser, TapexTokenizer, Trainer, TrainingArguments, default_data_collator, set_seed
...
class DataTrainingArguments:
    ...
    the command line.
    """
    dataset_name: Optional[str] = field(default='tab_fact', metadata={'help': 'The name of the dataset to use (via the datasets library).'})
    dataset_config_name: Optional[str] = field(default='tab_fact', metadata={'help': 'The configuration name of the dataset to use (via the datasets library).'})
    max_seq_length: int = field(default=1024, metadata={'help': 'The maximum total input sequence length after tokenization. Sequences longer than this will be truncated, sequences shorter will be padded.'})
