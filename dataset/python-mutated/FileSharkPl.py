...
class FileSharkPl(SimpleDownloader):
    ...
    __version__ = '0.22'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?fileshark\\.pl/pobierz/\\d+/\\w+'
    ...
    NAME_PATTERN = '<h2 class="name-file">(?P<N>.+?)</h2>'
    SIZE_PATTERN = '<p class="size-file">(.*?)<strong>(?P<S>\\d+\\.?\\d*)\\s(?P<U>\\w+)</strong></p>'
    OFFLINE_PATTERN = '(P|p)lik zosta. (usuni.ty|przeniesiony)'
    LINK_FREE_PATTERN = '<a rel="nofollow" href="(.*?)" class="btn-upload-free">'
    LINK_PREMIUM_PATTERN = '<a rel="nofollow" href="(.*?)" class="btn-upload-premium">'
    WAIT_PATTERN = 'var timeToDownload = (\\d+);'
    ERROR_PATTERN = '<p class="lead text-center alert alert-warning">(.*?)</p>'
    IP_ERROR_PATTERN = 'Strona jest dost.pna wy..cznie dla u.ytkownik.w znajduj.cych si. na terenie Polski'
    SLOT_ERROR_PATTERN = 'Osi.gni.to maksymaln. liczb. .ci.ganych jednocze.nie plik.w\\.'
    CAPTCHA_PATTERN = '<img src="data:image/jpeg;base64,(.*?)" title="captcha"'
    TOKEN_PATTERN = 'name="form\\[_token\\]" value="(.*?)" />'
