...
@pytest.mark.parametrize('parametrize_skip_or_fail_return', [(PARAMETRIZE_ACTION, 'parametrize reason'), (SKIP_ACTION, 'skip reason'), (FAIL_ACTION, 'fail_reason')])
def test_pytest_generate_tests(mocker, parametrize_skip_or_fail_return):
    if False:
        return 10
    test_config = config.Config(connector_image='foo', acceptance_tests=config.AcceptanceTestConfigurations(spec=config.GenericTestConfig(tests=[config.SpecTestConfig()])))
    ...
    metafunc_mock = mocker.Mock(fixturenames=['inputs'], function=mocker.Mock(__name__='test_function'), cls=mocker.Mock(config_key=mocker.Mock(return_value='spec'), __name__='MyTest'))
