...
@testing.parameterize({'mode': tuple}, {'mode': dict}, {'mode': None})
class TestDelegateDataset(unittest.TestCase):
    ...
    def test_delegate_dataset(self):
        if False:
            for i in range(10):
                print('nop')
        dataset = tabular.DelegateDataset(dummy_dataset.DummyDataset(mode=self.mode))
        ...
        self.assertEqual(dataset.get_example(3), dataset.dataset.get_example(3))
