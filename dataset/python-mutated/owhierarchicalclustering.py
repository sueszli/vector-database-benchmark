...
import numpy as np
from AnyQt.QtWidgets import QGraphicsWidget, QGraphicsScene, QGridLayout, QSizePolicy, QAction, QComboBox, QGraphicsGridLayout, QGraphicsSceneMouseEvent, QLabel
from AnyQt.QtGui import QPen, QFont, QKeySequence, QPainterPath, QColor, QFontMetrics
from AnyQt.QtCore import Qt, QObject, QSize, QPointF, QRectF, QLineF, QEvent, QModelIndex
...
import Orange.misc
from Orange.clustering.hierarchical import postorder, preorder, Tree, tree_from_linkage, dist_matrix_linkage, leaves, prune, top_clusters
from Orange.data.util import get_unique_names
from Orange.widgets import widget, gui, settings
from Orange.widgets.utils import itemmodels, combobox
from Orange.widgets.utils.annotated_data import lazy_annotated_table, ANNOTATED_DATA_SIGNAL_NAME, domain_with_annotation_column, add_columns, create_annotated_table
...
class OWHierarchicalClustering(widget.OWWidget):
    ...
    description = 'Display a dendrogram of a hierarchical clustering constructed from the input distance matrix.'
    ...
    color_by: Union[DiscreteVariable, ContinuousVariable, None] = settings.ContextSetting(None)
    ...
    class Warning(widget.OWWidget.Warning):
        subset_on_no_table = Msg('Unused data subset: distances do not refer to data instances')
        subset_not_subset = Msg('Some data from the subset does not appear in distance matrix')
        subset_wrong = Msg('Subset data refers to a different table')
        pruning_disables_colors = Msg("Pruned cluster doesn't show colors and indicate subset")
    __pending_selection_restore = None
