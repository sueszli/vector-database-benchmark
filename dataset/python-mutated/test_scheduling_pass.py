...
class TestSchedulingPass(QiskitTestCase):
    ...
    def test_active_reset_circuit(self, write_lat, cond_lat):
        ...
        actual_asap = PassManager(ASAPSchedule(durations, clbit_write_latency=write_lat, conditional_latency=cond_lat)).run(qc)
        actual_alap = PassManager(ALAPSchedule(durations, clbit_write_latency=write_lat, conditional_latency=cond_lat)).run(qc)
    ...
    def test_random_complicated_circuit(self):
        ...
        actual_asap = PassManager(ASAPSchedule(durations, clbit_write_latency=100, conditional_latency=200)).run(qc)
        actual_alap = PassManager(ALAPSchedule(durations, clbit_write_latency=100, conditional_latency=200)).run(qc)
        expected_asap = QuantumCircuit(3, 1)
    ...
    def test_dd_respect_target_instruction_constraints(self):
        ...
        pm_xx = PassManager([ALAPSchedule(target=target), DynamicalDecoupling(durations=None, dd_sequence=[XGate(), XGate()], target=target)])
        scheduled = pm_xx.run(qc)
        self.assertEqual(qc, scheduled)
        with self.assertRaises(TranspilerError):
            PassManager([ALAPSchedule(target=target), DynamicalDecoupling(durations=None, dd_sequence=[XGate(), YGate(), XGate(), YGate()], target=target)])
