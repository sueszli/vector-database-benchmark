...
SAVE_PREVIEWS = True
FIXED_SEED = 8964
flags.DEFINE_string('output_dir', None, 'Directory to store predictions. Assumes that regular inference has been executed before and results were stored in this folder.')
flags.DEFINE_string('data_dir', None, 'Folder pointing to preprocessed triplets to fine-tune on.')
flags.DEFINE_string('triplet_list_file', None, 'Text file containing paths to image files to process. Paths should be relative with respect to the list file location. Every line should be of the form [input_folder_name] [input_frame_num] [output_path], where [output_path] is optional to specify a different path to store the prediction.')
flags.DEFINE_string('triplet_list_file_remains', None, 'Optional text file containing relative paths to image files which should not be fine-tuned, e.g. because of missing adjacent frames. For all files listed, the static prediction will be copied instead. File can be empty. If not, every line should be of the form [input_folder_name] [input_frame_num] [output_path], where [output_path] is optional to specify a different path to take and store the unrefined prediction from/to.')
...
flags.DEFINE_float('size_constraint_weight', 0.0005, 'Weight of the object size constraint loss. Use only with motion handling.')
...
flags.DEFINE_enum('architecture', nets.RESNET, nets.ARCHITECTURES, 'Defines the architecture to use for the depth prediction network. Defaults to ResNet-based encoder and accompanying decoder.')
flags.DEFINE_boolean('imagenet_norm', True, 'Whether to normalize the input images channel-wise so that they match the distribution most ImageNet-models were trained on.')
flags.DEFINE_float('weight_reg', 0.05, 'The amount of weight regularization to apply. This has no effect on the ResNet-based encoder architecture.')
flags.DEFINE_boolean('exhaustive_mode', False, 'Whether to exhaustively warp from any frame to any other instead of just considering adjacent frames. Where necessary, multiple egomotion estimates will be applied. Does not have an effect if compute_minimum_loss is enabled.')
flags.DEFINE_boolean('random_scale_crop', False, 'Whether to apply random image scaling and center cropping during training.')
flags.DEFINE_bool('depth_upsampling', True, 'Whether to apply depth upsampling of lower-scale representations before warping to compute reconstruction loss on full-resolution image.')
flags.DEFINE_bool('depth_normalization', True, 'Whether to apply depth normalization, that is, normalizing inverse depth prediction maps by their mean to avoid degeneration towards small values.')
flags.DEFINE_bool('compute_minimum_loss', True, 'Whether to take the element-wise minimum of the reconstruction/SSIM error in order to avoid overly penalizing dis-occlusion effects.')
flags.DEFINE_bool('use_skip', True, 'Whether to use skip connections in the encoder-decoder architecture.')
flags.DEFINE_bool('joint_encoder', False, 'Whether to share parameters between the depth and egomotion networks by using a joint encoder architecture. The egomotion network is then operating only on the hidden representation provided by the joint encoder.')
flags.DEFINE_float('egomotion_threshold', 0.01, 'Minimum egomotion magnitude to apply finetuning. If lower, just forwards the ordinary prediction.')
flags.DEFINE_integer('num_steps', 20, 'Number of optimization steps to run.')
flags.DEFINE_boolean('handle_motion', True, 'Whether the checkpoint was trained with motion handling.')
flags.DEFINE_bool('flip', False, 'Whether images should be flipped as well as resulting predictions (for test-time augmentation). This currently applies to the depth network only.')
FLAGS = flags.FLAGS
