...
class TestFormatToQido(unittest.TestCase):
    valid_pubsub_string = 'projects/PROJECT_ID/locations/LOCATION/datasets/DATASET_ID/dicomStores/DICOM_STORE_ID/dicomWeb/studies/STUDY_UID/series/SERIES_UID/instances/INSTANCE_UID'
    expected_valid_pubsub_dict = {'result': {'project_id': 'PROJECT_ID', 'region': 'LOCATION', 'dataset_id': 'DATASET_ID', 'dicom_store_id': 'DICOM_STORE_ID', 'search_type': 'instances', 'params': {'StudyInstanceUID': 'STUDY_UID', 'SeriesInstanceUID': 'SERIES_UID', 'SOPInstanceUID': 'INSTANCE_UID'}}, 'input': valid_pubsub_string, 'success': True}
    invalid_pubsub_string = 'this is not a valid pubsub message'
    expected_invalid_pubsub_dict = {'result': {}, 'input': 'this is not a valid pubsub message', 'success': False}
