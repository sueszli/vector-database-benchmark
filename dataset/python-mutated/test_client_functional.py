...
async def test_keepalive_after_head_requests_success(aiohttp_client: Any) -> None:
    ...
    connector = aiohttp.TCPConnector(limit=1)
    client = await aiohttp_client(app, connector=connector, trace_configs=[trace_config])
...
async def test_keepalive_after_empty_body_status(aiohttp_client: Any, status: int) -> None:
...
@pytest.mark.parametrize('status', (101, 204, 304))
async def test_keepalive_after_empty_body_status_stream_response(aiohttp_client: Any, status: int) -> None:
...
async def test_ssl_client(aiohttp_server: Any, ssl_ctx: Any, aiohttp_client: Any, client_ssl_ctx: Any) -> None:
...
async def test_tcp_connector_fingerprint_ok(aiohttp_server: Any, aiohttp_client: Any, ssl_ctx: Any, tls_certificate_fingerprint_sha256: Any):
...
async def test_tcp_connector_fingerprint_fail(aiohttp_server: Any, aiohttp_client: Any, ssl_ctx: Any, tls_certificate_fingerprint_sha256: Any):
...
async def test_raw_headers(aiohttp_client: Any) -> None:
    ...
    async with client.get('/') as resp:
        ...
        assert raw_headers == ((b'Content-Length', b'0'), (b'Content-Type', b'application/octet-stream'), (b'Date', mock.ANY), (b'Server', mock.ANY))
...
async def test_empty_header_values(aiohttp_client: Any) -> None:
    ...
    async with client.get('/') as resp:
        assert resp.status == 200
        raw_headers = tuple(((bytes(h), bytes(v)) for (h, v) in resp.raw_headers))
        assert raw_headers == ((b'X-Empty', b''), (b'Content-Length', b'0'), (b'Content-Type', b'application/octet-stream'), (b'Date', mock.ANY), (b'Server', mock.ANY))
...
async def test_timeout_on_conn_reading_headers(aiohttp_client: Any, mocker: Any) -> None:
...
async def test_timeout_on_session_read_timeout(aiohttp_client: Any, mocker: Any) -> None:
    ...
    app.router.add_route('GET', '/', handler)
    conn = aiohttp.TCPConnector()
    client = await aiohttp_client(app, connector=conn, timeout=aiohttp.ClientTimeout(sock_read=0.01))
    with pytest.raises(asyncio.TimeoutError):
        await client.get('/')
...
async def test_HTTP_302_REDIRECT_POST_with_content_length_hdr(aiohttp_client: Any) -> None:
    ...
    app.router.add_post('/redirect', redirect)
    client = await aiohttp_client(app)
    resp = await client.post('/redirect', data=data, headers={'Content-Length': str(len(data))})
    assert 200 == resp.status
    assert 1 == len(resp.history)
    txt = await resp.text()
    assert txt == 'GET'
...
async def test_HTTP_302_max_redirects(aiohttp_client: Any) -> None:
    ...
    app = web.Application()
    app.router.add_get('/', handler)
    app.router.add_get('/redirect/{count:\\d+}', redirect)
    client = await aiohttp_client(app)
...
async def test_HTTP_200_GET_WITH_PARAMS(aiohttp_client: Any) -> None:
    ...
    async def handler(request):
        return web.Response(text='&'.join((k + '=' + v for (k, v) in request.query.items())))
    app = web.Application()
    app.router.add_get('/', handler)
    client = await aiohttp_client(app)
...
async def test_POST_DATA_with_content_type_context_transfer_encoding(aiohttp_client: Any):
...
async def test_POST_FILES_DEFLATE(aiohttp_client: Any, fname: Any) -> None:
    ...
    client = await aiohttp_client(app)
    with fname.open('rb') as f:
        async with client.post('/', data={'some': f}, chunked=True, compress='deflate') as resp:
            assert 200 == resp.status
...
async def test_POST_FILES_SINGLE(aiohttp_client: Any, fname: Any) -> None:
    ...
    async def handler(request):
        ...
        with fname.open('rb') as f:
            ...
            assert content == data
        assert request.content_type in ['text/plain', 'application/octet-stream', 'text/x-python']
...
async def test_POST_FILES_SINGLE_content_disposition(aiohttp_client: Any, fname: Any) -> None:
    ...
    async def handler(request):
        ...
        with fname.open('rb') as f:
            ...
            assert content == data
        assert request.content_type in ['text/plain', 'application/octet-stream', 'text/x-python']
        assert request.headers['content-disposition'] == 'inline; filename="conftest.py"'
        return web.Response()
    app = web.Application()
    app.router.add_post('/', handler)
    client = await aiohttp_client(app)
...
async def test_POST_FILES_SINGLE_BINARY(aiohttp_client: Any, fname: Any) -> None:
    ...
    async def handler(request):
        ...
        with fname.open('rb') as f:
            content = f.read()
        assert content == data
        assert request.content_type in ['application/pgp-keys', 'text/plain', 'text/x-python', 'application/octet-stream']
        return web.Response()
    app = web.Application()
    app.router.add_post('/', handler)
    client = await aiohttp_client(app)
...
async def test_POST_FILES_IO_WITH_PARAMS(aiohttp_client: Any) -> None:
    ...
    client = await aiohttp_client(app)
    with io.BytesIO(b'data') as file_handle:
        async with client.post('/', data=(('test', 'true'), MultiDict([('q', 't1'), ('q', 't2')]), file_handle)) as resp:
            assert 200 == resp.status
...
async def test_POST_FILES_WITH_DATA(aiohttp_client: Any, fname: Any) -> None:
    ...
    async def handler(request):
        data = await request.post()
        assert data['test'] == 'true'
        assert data['some'].content_type in ['text/x-python', 'text/plain', 'application/octet-stream']
        assert data['some'].filename == fname.name
        with fname.open('rb') as f:
    ...
    async def gen(fname):
        with fname.open('rb') as f:
            ...
            while data:
                yield data
                data = f.read(100)
    async with client.post('/', data=gen(fname), headers={'Content-Length': str(data_size)}) as resp:
        assert 200 == resp.status
...
async def test_set_cookies(aiohttp_client: Any) -> None:
    ...
    async def handler(request):
        ...
        ret.headers.add('Set-Cookie', 'ISAWPLB{A7F52349-3531-4DA9-8776-F74BC6F4F1BB}={925EC0B8-CB17-4BEB-8A35-1033813B0523}; HttpOnly; Path=/')
        return ret
    app = web.Application()
    app.router.add_get('/', handler)
    client = await aiohttp_client(app)
...
async def test_set_cookies_expired(aiohttp_client: Any) -> None:
    ...
    async def handler(request):
        ...
        ret.set_cookie('c1', 'cookie1')
        ret.set_cookie('c2', 'cookie2')
        ret.headers.add('Set-Cookie', 'c3=cookie3; HttpOnly; Path=/ Expires=Tue, 1 Jan 1980 12:00:00 GMT; ')
        return ret
    app = web.Application()
    app.router.add_get('/', handler)
    client = await aiohttp_client(app)
...
async def test_set_cookies_max_age_overflow(aiohttp_client: Any) -> None:
    ...
    async def handler(request):
        ret = web.Response()
        ret.headers.add('Set-Cookie', 'overflow=overflow; HttpOnly; Path=/ Max-Age=' + str(overflow) + '; ')
        return ret
    overflow = int(datetime.datetime.max.replace(tzinfo=datetime.timezone.utc).timestamp())
    empty = None
    try:
        empty = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=overflow)
...
async def test_custom_headers(aiohttp_client: Any) -> None:
    ...
    app.router.add_post('/', handler)
    client = await aiohttp_client(app)
    async with client.post('/', headers={'Content-Type': 'application/json', 'x-api-key': 'foo'}) as resp:
        assert resp.status == 200
...
@pytest.mark.parametrize(('status', 'expected_ok'), ((200, True), (201, True), (301, True), (400, False), (403, False), (500, False)))
async def test_ok_from_status(aiohttp_client: Any, status: Any, expected_ok: Any) -> None:
...
async def test_creds_in_auth_and_url() -> None:
    ...
    try:
        with pytest.raises(ValueError):
            await session.get('http://user:pass@example.com', auth=aiohttp.BasicAuth('user2', 'pass2'))
    finally:
        await session.close()
...
@pytest.mark.parametrize(['url_from', 'url_to', 'is_drop_header_expected'], [['http://host1.com/path1', 'http://host2.com/path2', True], ['http://host1.com/path1', 'https://host1.com/path1', False], ['https://host1.com/path1', 'http://host1.com/path2', True]], ids=('entirely different hosts', 'http -> https', 'https -> http'))
async def test_drop_auth_on_redirect_to_other_host(create_server_for_url_and_handler: Any, url_from: str, url_to: str, is_drop_header_expected: bool) -> None:
    ...
    assert url_from.host != url_to.host or server_from.scheme != server_to.scheme, 'Invalid test case, host or scheme must differ'
    protocol_port_map = {'http': 80, 'https': 443}
    etc_hosts = {(url_from.host, protocol_port_map[server_from.scheme]): server_from, (url_to.host, protocol_port_map[server_to.scheme]): server_to}

    class FakeResolver(AbstractResolver):

        async def resolve(self, host, port=0, family=socket.AF_INET):
            server = etc_hosts[host, port]
            return [{'hostname': host, 'host': server.host, 'port': server.port, 'family': socket.AF_INET, 'proto': 0, 'flags': socket.AI_NUMERICHOST}]
    ...
    async with aiohttp.ClientSession(connector=connector) as client:
        ...
        assert resp.status == 200
        resp = await client.get(url_from, headers={'Authorization': 'Basic dXNlcjpwYXNz'})
        assert resp.status == 200
...
async def test_aiohttp_request_ctx_manager_close_sess_on_error(ssl_ctx: Any, aiohttp_server: Any) -> None:
...
async def test_aiohttp_request_coroutine(aiohttp_server: Any) -> None:
    ...
    with pytest.raises(TypeError, match="^object _SessionRequestContextManager can't be used in 'await' expression$"):
        await not_an_awaitable
    await not_an_awaitable._coro
    await server.close()
...
async def test_session_headers_merge(aiohttp_client: Any) -> None:
    ...
    app = web.Application()
    app.router.add_get('/', handler)
    client = await aiohttp_client(app, headers=[('X-Real-IP', '192.168.0.1'), ('X-Sent-By', 'requests')])
...
async def test_multidict_headers(aiohttp_client: Any) -> None:
    ...
    client = await aiohttp_client(app)
    data = b'sample data'
    async with client.post('/', data=data, headers=MultiDict({'Content-Length': str(len(data))})) as r:
        assert r.status == 200
...
async def test_error_in_performing_request(ssl_ctx: Any, aiohttp_client: Any, aiohttp_server: Any):
