...
class CodingSchema(BaseModel):
    code_description: str = Field(..., description='Description of the coding task')
...
class CodingTool(BaseTool):
    ...
    name = 'CodingTool'
    description = 'You will get instructions for code to write. You will write a very long answer. Make sure that every detail of the architecture is, in the end, implemented as code. Think step by step and reason yourself to the right decisions to make sure we get it right. You will first lay out the names of the core classes, functions, methods that will be necessary, as well as a quick comment on their purpose. Then you will output the content of each file including each function and class and ALL code.'
    ...
    def _execute(self, code_description: str) -> str:
        ...
        for match in matches:
            file_name = re.sub('[<>"|?*]', '', match.group(1))
