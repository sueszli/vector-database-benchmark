...
input_uris = ['https://serve-resnet-benchmark-data.s3.us-west-1.amazonaws.com/000000000019.jpeg', 'https://serve-resnet-benchmark-data.s3.us-west-1.amazonaws.com/000000000128.jpeg', 'https://serve-resnet-benchmark-data.s3.us-west-1.amazonaws.com/000000000171.jpeg', 'https://serve-resnet-benchmark-data.s3.us-west-1.amazonaws.com/000000000184.jpeg', 'https://serve-resnet-benchmark-data.s3.us-west-1.amazonaws.com/000000000300.jpeg', 'https://serve-resnet-benchmark-data.s3.us-west-1.amazonaws.com/000000000311.jpeg', 'https://serve-resnet-benchmark-data.s3.us-west-1.amazonaws.com/000000000333.jpeg', 'https://serve-resnet-benchmark-data.s3.us-west-1.amazonaws.com/000000000416.jpeg']
...
class ImageObjectioner:
    ...
    async def predict(self, uris: List[str]):
        ...
        image_tensors = [tensor for item_tensors in image_tensors_lists for tensor in item_tensors]
...
async def measure_http_throughput_tps(data_size: int=8, requests_sent: int=8):
    ...
    async def fetch(session):
        async with session.get('http://localhost:8000/', json=input_uris * int(data_size / len(input_uris))) as response:
...
async def trial(measure_func, data_size: int=8, num_clients: int=1):
    ...
    result_stats_list = await asyncio.gather(*[client_task(data_size) for client_task in client_tasks])
