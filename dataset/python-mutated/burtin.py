...
GRAM = dict([('negative', '#e69584'), ('positive', '#aeaeb8')])
...
micmin = sqrt(log(0.001 * 10000.0))
micmax = sqrt(log(1000 * 10000.0))

def scale(mic):
    if False:
        for i in range(10):
            print('nop')
    return -sqrt(log(mic * 10000.0)) + (micmin + micmax)
...
p.text(0, radii, ['0.001', '0.01', '0.1', '1', '10', '100', '1000'], text_font_size='12px', anchor='center')
small_angle = big_angle / 7
for (i, drug) in enumerate(DRUGS):
    start = angles + (5 - 2 * i) * small_angle
    end = angles + (6 - 2 * i) * small_angle
    p.annular_wedge(0, 0, micmin, scale(df[drug]), start, end, color=COLORS[i], line_color=None, legend_label=drug)
r = radii[0] * 1.1
...
legend = Legend(items=[LegendItem(label='Gram-positive', renderers=[br], index=10), LegendItem(label='Gram-negative', renderers=[br], index=0)], location='bottom', orientation='horizontal', background_fill_alpha=0)
