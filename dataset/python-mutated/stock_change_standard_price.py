...
class StockChangeStandardPrice(models.TransientModel):
    ...
    new_price = fields.Float('Price', digits=dp.get_precision('Product Price'), required=True, help='If cost price is increased, stock variation account will be debited and stock output account will be credited with the value = (difference of amount * quantity available).\nIf cost price is decreased, stock variation account will be creadited and stock input account will be debited.')
    counterpart_account_id = fields.Many2one('account.account', string='Counter-Part Account', domain=[('deprecated', '=', False)], required=True)
