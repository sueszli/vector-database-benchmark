...
async def artifact(session):
    artifact_schema = schemas.core.Artifact(key='voltaic', data={'a': 1}, type='table', description='opens many doors')
    model = (await models.artifacts.create_artifact(session=session, artifact=artifact_schema),)
    await session.commit()
    return model[0]

@pytest.fixture
async def artifact_null_field(session):
    artifact_schema = schemas.core.Artifact(key='voltaic', data=1, metadata_={'description': 'opens many doors'})
    model = (await models.artifacts.create_artifact(session=session, artifact=artifact_schema),)
    await session.commit()
    return model[0]
...
async def artifacts(session):
    model1 = await models.artifacts.create_artifact(session=session, artifact=schemas.core.Artifact(key='voltaic', data=1, description='opens many doors', type='table'))
    model2 = await models.artifacts.create_artifact(session=session, artifact=schemas.core.Artifact(key='voltaic', data=2, description='opens many doors', type='table'))
    model3 = await models.artifacts.create_artifact(session=session, artifact=schemas.core.Artifact(key='lotus', data=3, description='opens many doors', type='markdown'))
    await session.commit()
