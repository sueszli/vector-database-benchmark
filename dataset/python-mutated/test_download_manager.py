...
async def fake_dlmgr(tmp_path_factory):
    ...
    dlmgr = DownloadManager(config=config, state_dir=tmp_path_factory.mktemp('state_dir'), notifier=MagicMock(), peer_mid=b'0000')
...
async def test_start_download(fake_dlmgr):
    ...
    mock_alert = type('add_torrent_alert', (object,), dict(handle=mock_handle, error=mock_error, category=lambda _: None))()
    mock_ltsession = MagicMock()
    mock_ltsession.get_torrents = lambda : []
    mock_ltsession.async_add_torrent = lambda _: fake_dlmgr.register_task('post_alert', fake_dlmgr.process_alert, mock_alert, delay=0.1)
