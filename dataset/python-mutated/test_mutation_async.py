...
async def test_node_query_promise():
    executed = await client.execute_async('mutation a { sayPromise(input: {what:"hello", clientMutationId:"1"}) { phrase } }')
...
async def test_edge_query():
    executed = await client.execute_async('mutation a { other(input: {clientMutationId:"1"}) { clientMutationId, myNodeEdge { cursor node { name }} } }')
    assert isinstance(executed, dict)
    assert 'errors' not in executed
    assert executed['data'] == {'other': {'clientMutationId': '1', 'myNodeEdge': {'cursor': '1', 'node': {'name': 'name'}}}}