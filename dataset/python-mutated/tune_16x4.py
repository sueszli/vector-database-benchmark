...
def train_wrapper(config, ray_params):
    if False:
        i = 10
        return i + 15
    train_ray(path='/data/classification.parquet', num_workers=None, num_boost_rounds=100, num_files=64, regression=False, use_gpu=False, ray_params=ray_params, lightgbm_params=config)
if __name__ == '__main__':
    ...
    ray_params = RayParams(elastic_training=False, max_actor_restarts=2, num_actors=4, cpus_per_actor=2, gpus_per_actor=0)
    start = time.time()
    analysis = tune.run(tune.with_parameters(train_wrapper, ray_params=ray_params), config=search_space, num_samples=16, resources_per_trial=ray_params.get_tune_resources())
