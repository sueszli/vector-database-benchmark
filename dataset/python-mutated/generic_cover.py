...
from homeassistant.components.cover import ATTR_TILT_POSITION, CoverEntity, CoverEntityFeature
from ..entity import OverkizEntity
ATTR_OBSTRUCTION_DETECTED = 'obstruction-detected'
COMMANDS_STOP: list[OverkizCommand] = [OverkizCommand.STOP, OverkizCommand.MY]
COMMANDS_STOP_TILT: list[OverkizCommand] = [OverkizCommand.STOP, OverkizCommand.MY]
COMMANDS_OPEN: list[OverkizCommand] = [OverkizCommand.OPEN, OverkizCommand.UP]
COMMANDS_OPEN_TILT: list[OverkizCommand] = [OverkizCommand.OPEN_SLATS]
COMMANDS_CLOSE: list[OverkizCommand] = [OverkizCommand.CLOSE, OverkizCommand.DOWN]
...
class OverkizGenericCover(OverkizEntity, CoverEntity):
    ...
    async def async_set_cover_tilt_position(self, **kwargs: Any) -> None:
        ...
        if (command := self.executor.select_command(*COMMANDS_SET_TILT_POSITION)):
            await self.executor.async_execute_command(command, 100 - kwargs[ATTR_TILT_POSITION])
