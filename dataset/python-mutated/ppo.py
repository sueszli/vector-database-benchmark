...
class PPOAgent(nn.Module):
    ...
    def __init__(self, num_actions, observation_shape, device):
        if False:
            return 10
        super().__init__()
        self.critic = nn.Sequential(layer_init(nn.Linear(np.array(observation_shape).prod(), 64)), nn.Tanh(), layer_init(nn.Linear(64, 64)), nn.Tanh(), layer_init(nn.Linear(64, 1), std=1.0))
        self.actor = nn.Sequential(layer_init(nn.Linear(np.array(observation_shape).prod(), 64)), nn.Tanh(), layer_init(nn.Linear(64, 64)), nn.Tanh(), layer_init(nn.Linear(64, num_actions), std=0.01))
        self.device = device
...
class PPOAtariAgent(nn.Module):
    ...
    def __init__(self, num_actions, observation_shape, device):
        ...
        self.network = nn.Sequential(layer_init(nn.Conv2d(4, 32, 8, stride=4)), nn.ReLU(), layer_init(nn.Conv2d(32, 64, 4, stride=2)), nn.ReLU(), layer_init(nn.Conv2d(64, 64, 3, stride=1)), nn.ReLU(), nn.Flatten(), layer_init(nn.Linear(64 * 7 * 7, 512)), nn.ReLU())
...
class PPO(nn.Module):
  ...
  """
    ...
    def __init__(self, input_shape, num_actions, num_players, player_id=0, num_envs=1, steps_per_batch=128, num_minibatches=4, update_epochs=4, learning_rate=0.00025, gae=True, gamma=0.99, gae_lambda=0.95, normalize_advantages=True, clip_coef=0.2, clip_vloss=True, entropy_coef=0.01, value_coef=0.5, max_grad_norm=0.5, target_kl=None, device='cpu', writer=None, agent_fn=PPOAtariAgent):
        if False:
            while True:
                i = 10
        super().__init__()
        ...
        self.optimizer = optim.Adam(self.parameters(), lr=self.learning_rate, eps=1e-05)
    ...
    def step(self, time_step, is_evaluation=False):
        ...
        if is_evaluation:
            with torch.no_grad():
                legal_actions_mask = legal_actions_to_mask([ts.observations['legal_actions'][self.player_id] for ts in time_step], self.num_actions).to(self.device)
                obs = torch.Tensor(np.array([np.reshape(ts.observations['info_state'][self.player_id], self.input_shape) for ts in time_step])).to(self.device)
    ...
    def learn(self, time_step):
        if False:
            print('Hello World!')
        next_obs = torch.Tensor(np.array([np.reshape(ts.observations['info_state'][self.player_id], self.input_shape) for ts in time_step])).to(self.device)
        with torch.no_grad():
