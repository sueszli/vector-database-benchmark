...
class StartUpTasks(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def initialize(self) -> None:
        ...
        try:
            self.db_conn = APSWConnectionWrapper(str(cog_data_path(self.bot.get_cog('Audio')) / 'Audio.db'))
            self.api_interface = AudioAPIInterface(self.bot, self.config, self.session, self.db_conn, self.bot.get_cog('Audio'))
            ...
            await self.data_schema_migration(from_version=await self.config.schema_version(), to_version=_SCHEMA_VERSION)
    ...
    async def restore_players(self):
        ...
        try:
            ...
            log.error('Restoring player task aborted due to a timeout waiting for Lavalink node to be ready.')
        ...
        for (guild_id, track_data) in itertools.groupby(tracks_to_restore, key=lambda x: x.guild_id):
            ...
            try:
                ...
                if not guild:
                    log.verbose('Skipping player restore - Bot is no longer in Guild (%s)', guild_id)
                    continue
                persist_cache = self._persist_queue_cache.setdefault(guild_id, await self.config.guild(guild).persist_queue())
                if not persist_cache:
                    log.verbose('Skipping player restore - Guild (%s) does not have a persist cache', guild_id)
                ...
                if player is None:
                    while tries < 5 and vc is not None:
                        try:
                            (notify_channel_id, vc_id) = metadata.pop(guild_id, (None, track_data[-1].room_id))
                ...
                if tries >= 5 or vc is None or player is None:
                    if tries >= 5:
                        log.verbose('Skipping player restore - Guild (%s), 5 attempts to restore player failed.', guild_id)
                    elif vc is None:
                        log.verbose('Skipping player restore - Guild (%s), VC (%s) does not exist.', guild_id, vc_id)
                    else:
                        log.verbose('Skipping player restore - Guild (%s), Unable to create player for VC (%s).', guild_id, vc_id)
                    await self.api_interface.persistent_queue_api.drop(guild_id)
                    continue
            ...
            if player is None:
                ...
                if not player.is_playing:
                    ...
                    try:
                        ...
                        notify_channel = guild.get_channel_or_thread(notify_channel)
                        if notify_channel:
                            await self.send_embed_msg(notify_channel, title=_('Unable to Get Track'), description=_("I'm unable to get a track from the Lavalink node at the moment, try again in a few minutes."))
                        return
        del metadata
        del all_guilds
