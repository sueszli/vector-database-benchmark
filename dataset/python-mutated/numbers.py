...
from hypothesis.internal.floats import float_of, float_to_int, int_to_float, is_negative, make_float_clamper, next_down, next_down_normal, next_up, next_up_normal, width_smallest_normals
...
NASTY_FLOATS = sorted([0.0, 0.5, 1.1, 1.5, 1.9, 1.0 / 3, 10000000.0, 1e-05, 1.175494351e-38, next_up(0.0), float_info.min, float_info.max, 3.402823466e+38, 9007199254740992, 1 - 1e-05, 2 + 1e-05, 1.192092896e-07, 2.220446049250313e-16] + [2.0 ** (-n) for n in (24, 14, 149, 126)] + [float_info.min / n for n in (2, 10, 1000, 100000)] + [math.inf, math.nan] * 5 + [SIGNALING_NAN], key=flt.float_to_lex)
