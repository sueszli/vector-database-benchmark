...
from typing import TYPE_CHECKING, Collection, Dict, List, Mapping, Optional, Tuple, Union, cast
import attr
from synapse.api.constants import MAIN_TIMELINE, ReceiptTypes
from synapse.metrics.background_process_metrics import wrap_as_background_process
from synapse.storage._base import SQLBaseStore, db_to_json, make_in_list_sql_clause
from synapse.storage.database import DatabasePool, LoggingDatabaseConnection, LoggingTransaction, PostgresEngine
...
DEFAULT_NOTIF_ACTION: List[Union[dict, str]] = ['notify', {'set_tweak': 'highlight', 'value': False}]
DEFAULT_HIGHLIGHT_ACTION: List[Union[dict, str]] = ['notify', {'set_tweak': 'sound', 'value': 'default'}, {'set_tweak': 'highlight'}]
...
class EventPushActionsWorkerStore(ReceiptsWorkerStore, StreamWorkerStore, SQLBaseStore):
    ...
    async def _background_drop_null_thread_id_indexes(self, progress: JsonDict, batch_size: int) -> int:
        """
        ...
        def drop_null_thread_id_indexes_txn(txn: LoggingTransaction) -> None:
            ...
            logger.debug('[SQL] %s', sql)
            txn.execute(sql)
        await self.db_pool.runInteraction('drop_null_thread_id_indexes_txn', drop_null_thread_id_indexes_txn)
        await self.db_pool.updates._end_background_update('event_push_drop_null_thread_id_indexes')
        return 0
    ...
    async def get_unread_counts_by_room_for_user(self, user_id: str) -> Dict[str, int]:
        ...
        """
        return await self.db_pool.runInteraction('get_unread_counts_by_room_for_user', self._get_unread_counts_by_room_for_user_txn, user_id)
    ...
    async def get_unread_event_push_actions_by_room_for_user(self, room_id: str, user_id: str) -> RoomNotifCounts:
        ...
        Returns
            ...
            and threads.
        """
        return await self.db_pool.runInteraction('get_unread_event_push_actions_by_room', self._get_unread_counts_by_receipt_txn, room_id, user_id)
    ...
    async def get_push_action_users_in_range(self, min_stream_ordering: int, max_stream_ordering: int) -> List[str]:
    ...
    async def get_unread_push_actions_for_user_in_range_for_http(self, user_id: str, min_stream_ordering: int, max_stream_ordering: int, limit: int=20) -> List[HttpPushAction]:
        ...
        """
        receipts_by_room = await self.db_pool.runInteraction('get_unread_push_actions_for_user_in_range_http_receipts', self._get_receipts_by_room_txn, user_id=user_id)
        ...
        push_actions = await self.db_pool.runInteraction('get_unread_push_actions_for_user_in_range_http', get_push_actions_txn)
        notifs = [HttpPushAction(event_id=event_id, room_id=room_id, stream_ordering=stream_ordering, actions=_deserialize_action(actions, highlight)) for (event_id, room_id, thread_id, stream_ordering, actions, highlight) in push_actions if receipts_by_room.get(room_id, MISSING_ROOM_RECEIPT).is_unread(thread_id, stream_ordering)]
        notifs.sort(key=lambda r: r.stream_ordering)
        return notifs[:limit]

    async def get_unread_push_actions_for_user_in_range_for_email(self, user_id: str, min_stream_ordering: int, max_stream_ordering: int, limit: int=20) -> List[EmailPushAction]:
        """Get a list of the most recent unread push actions for a given user,
        ...
        Returns:
            ...
            The list will have between 0~limit entries.
        """
        receipts_by_room = await self.db_pool.runInteraction('get_unread_push_actions_for_user_in_range_email_receipts', self._get_receipts_by_room_txn, user_id=user_id)
        ...
        push_actions = await self.db_pool.runInteraction('get_unread_push_actions_for_user_in_range_email', get_push_actions_txn)
        notifs = [EmailPushAction(event_id=event_id, room_id=room_id, stream_ordering=stream_ordering, actions=_deserialize_action(actions, highlight), received_ts=received_ts) for (event_id, room_id, thread_id, stream_ordering, actions, highlight, received_ts) in push_actions if receipts_by_room.get(room_id, MISSING_ROOM_RECEIPT).is_unread(thread_id, stream_ordering)]
        notifs.sort(key=lambda r: -(r.received_ts or 0))
        return notifs[:limit]

    async def get_if_maybe_push_in_range_for_user(self, user_id: str, min_stream_ordering: int) -> bool:
        ...
        return await self.db_pool.runInteraction('get_if_maybe_push_in_range_for_user', _get_if_maybe_push_in_range_for_user_txn)

    async def add_push_actions_to_staging(self, event_id: str, user_id_actions: Dict[str, Collection[Union[Mapping, str]]], count_as_unread: bool, thread_id: str) -> None:
        ...
        await self.db_pool.simple_insert_many('event_push_actions_staging', keys=('event_id', 'user_id', 'actions', 'notif', 'highlight', 'unread', 'thread_id', 'inserted_ts'), values=[_gen_entry(user_id, actions) for (user_id, actions) in user_id_actions.items()], desc='add_push_actions_to_staging')
    ...
    async def remove_push_actions_from_staging(self, event_id: str) -> None:
        ...
        """
        try:
            await self.db_pool.simple_delete(table='event_push_actions_staging', keyvalues={'event_id': event_id}, desc='remove_push_actions_from_staging')
        except Exception:
            logger.exception('Error removing push actions after event persistence failure')

    @wrap_as_background_process('event_push_action_stream_orderings')
    async def _find_stream_orderings_for_times(self) -> None:
        await self.db_pool.runInteraction('_find_stream_orderings_for_times', self._find_stream_orderings_for_times_txn)
    ...
    async def find_first_stream_ordering_after_ts(self, ts: int) -> int:
        ...
        """
        return await self.db_pool.runInteraction('_find_first_stream_ordering_after_ts_txn', self._find_first_stream_ordering_after_ts_txn, ts)
    ...
    async def get_time_of_last_push_action_before(self, stream_ordering: int) -> Optional[int]:
        ...
        result = await self.db_pool.runInteraction('get_time_of_last_push_action_before', f)
    ...
    async def _rotate_notifs(self) -> None:
        ...
        try:
            while True:
                ...
                caught_up = await self.db_pool.runInteraction('_handle_new_receipts_for_notifs_txn', self._handle_new_receipts_for_notifs_txn)
                if caught_up:
                    break
            while True:
                logger.info('Rotating notifications')
                caught_up = await self.db_pool.runInteraction('_rotate_notifs', self._rotate_notifs_txn)
                if caught_up:
                    break
    ...
    async def _remove_old_push_actions_that_have_rotated(self) -> None:
        ...
        1 day ago).
        """
        rotated_upto_stream_ordering = await self.db_pool.simple_select_one_onecol(table='event_push_summary_stream_ordering', keyvalues={}, retcol='stream_ordering')
        max_stream_ordering_to_delete = min(rotated_upto_stream_ordering, self.stream_ordering_day_ago)
        ...
        while True:
            done = await self.db_pool.runInteraction('_remove_old_push_actions_that_have_rotated', remove_old_push_actions_that_have_rotated_txn)
            if done:
                break
    ...
    async def _clear_old_push_actions_staging(self) -> None:
        ...
        while True:
            deleted = await self.db_pool.runInteraction('_clear_old_push_actions_staging', _clear_old_push_actions_staging_txn)
            if not deleted:
                return
            await self._clock.sleep(1.0)

    async def get_push_actions_for_user(self, user_id: str, before: Optional[str]=None, limit: int=50, only_highlight: bool=False) -> List[UserPushAction]:
        ...
        return [UserPushAction(event_id=row[0], room_id=row[1], stream_ordering=row[2], actions=_deserialize_action(row[4], row[5]), received_ts=row[7], topological_ordering=row[3], highlight=row[5], profile_tag=row[6]) for row in push_actions]
