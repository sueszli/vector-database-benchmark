...
_ENV_SETTING = flags.DEFINE_string('env_setting', 'mfg_crowd_modelling_2d', 'Name of the game settings. If None, the game name will be used.')
...
_NUM_ITERATIONS = flags.DEFINE_integer('num_iterations', 100, 'Number of iterations.')
...
_LEARN_EVERY = flags.DEFINE_integer('learn_every', 40, 'Number of steps between learning updates.')
_NUM_DQN_EPISODES_PER_ITERATION = flags.DEFINE_integer('num_dqn_episodes_per_iteration', 3000, 'Number of DQN training episodes for each iteration.')
_EPSILON_DECAY_DURATION = flags.DEFINE_integer('epsilon_decay_duration', int(20000000.0), 'Number of game steps over which epsilon is decayed.')
_EPSILON_START = flags.DEFINE_float('epsilon_start', 0.1, 'Starting exploration parameter.')
_EPSILON_END = flags.DEFINE_float('epsilon_end', 0.1, 'Final exploration parameter.')
...
_HIDDEN_LAYERS_SIZES = flags.DEFINE_list('hidden_layers_sizes', ['128', '128'], 'Number of hidden units in the Q-net.')
...
_REPLAY_BUFFER_CAPACITY = flags.DEFINE_integer('replay_buffer_capacity', 5000, 'Size of the replay buffer.')
_MIN_BUFFER_SIZE_TO_LEARN = flags.DEFINE_integer('min_buffer_size_to_learn', 200, 'Number of samples in buffer before learning begins.')
_OPTIMIZER = flags.DEFINE_enum('optimizer', 'adam', ['sgd', 'adam'], 'Optimizer.')
_LEARNING_RATE = flags.DEFINE_float('learning_rate', 0.001, 'Learning rate for inner rl agent.')
_LOSS = flags.DEFINE_enum('loss', 'mse', ['mse', 'huber'], 'Loss function.')
_HUBER_LOSS_PARAMETER = flags.DEFINE_float('huber_loss_parameter', 1.0, 'Parameter for Huber loss.')
_GRADIENT_CLIPPING = flags.DEFINE_float('gradient_clipping', 40, 'Value to clip the gradient to.')
_AVG_POL_BATCH_SIZE = flags.DEFINE_integer('avg_pol_batch_size', 128, 'Number of transitions to sample at each learning step.')
_AVG_POL_NUM_TRAINING_STEPS_PER_ITERATION = flags.DEFINE_integer('avg_pol_num_training_steps_per_iteration', 2000, 'Number of steps for average policy at each FP iteration.')
_AVG_POL_NUM_EPISODES_PER_ITERATION = flags.DEFINE_integer('avg_pol_num_episodes_per_iteration', 100, 'Number of samples to store at each FP iteration.')
_AVG_POL_HIDDEN_LAYERS_SIZES = flags.DEFINE_list('avg_pol_hidden_layers_sizes', ['128', '128'], 'Number of hidden units in the avg-net and Q-net.')
_AVG_POL_RESERVOIR_BUFFER_CAPACITY = flags.DEFINE_integer('avg_pol_reservoir_buffer_capacity', 100000000, 'Size of the reservoir buffer.')
_AVG_POL_MIN_BUFFER_SIZE_TO_LEARN = flags.DEFINE_integer('avg_pol_min_buffer_size_to_learn', 100, 'Number of samples in buffer before learning begins.')
_AVG_POL_OPTIMIZER = flags.DEFINE_enum('avg_pol_optimizer', 'sgd', ['sgd', 'adam'], 'Optimizer.')
_AVG_POL_LEARNING_RATE = flags.DEFINE_float('avg_pol_learning_rate', 0.01, 'Learning rate for inner rl agent.')
_AVG_GRADIENT_CLIPPING = flags.DEFINE_float('avg_gradient_clipping', 100, 'Value to clip the gradient to.')
_AVG_POL_TAU = flags.DEFINE_float('avg_pol_tau', 10.0, 'Temperature for softmax in policy.')
