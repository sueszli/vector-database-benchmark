...
class Sagittarius(coord.BaseCoordinateFrame):
    ...
    frame_specific_representation_info = {coord.SphericalRepresentation: [coord.RepresentationMapping('lon', 'Lambda'), coord.RepresentationMapping('lat', 'Beta'), coord.RepresentationMapping('distance', 'distance')]}
SGR_PHI = (180 + 3.75) * u.degree
SGR_THETA = (90 - 13.46) * u.degree
SGR_PSI = (180 + 14.111534) * u.degree
SGR_MATRIX = np.diag([1.0, 1.0, -1.0]) @ rotation_matrix(SGR_PSI, 'z') @ rotation_matrix(SGR_THETA, 'x') @ rotation_matrix(SGR_PHI, 'z')
...
print(sgr)
sgr = coord.SkyCoord(Lambda=np.linspace(0, 2 * np.pi, 128) * u.radian, Beta=np.zeros(128) * u.radian, frame='sagittarius')
icrs = sgr.transform_to(coord.ICRS)
print(icrs)
(fig, axes) = plt.subplots(2, 1, figsize=(8, 10), subplot_kw={'projection': 'aitoff'})
axes[0].set_title('Sagittarius')
axes[0].plot(sgr.Lambda.wrap_at(180 * u.deg).radian, sgr.Beta.radian, linestyle='none', marker='.')
axes[1].set_title('ICRS')
axes[1].plot(icrs.ra.wrap_at(180 * u.deg).radian, icrs.dec.radian, linestyle='none', marker='.')
plt.show()
sgr = coord.SkyCoord(Lambda=np.linspace(0, 2 * np.pi, 128) * u.radian, Beta=np.zeros(128) * u.radian, pm_Lambda_cosBeta=np.random.uniform(-5, 5, 128) * u.mas / u.yr, pm_Beta=np.zeros(128) * u.mas / u.yr, frame='sagittarius')
icrs = sgr.transform_to(coord.ICRS)
print(icrs)
(fig, axes) = plt.subplots(3, 1, figsize=(8, 10), sharex=True)
axes[0].set_title('Sagittarius')
axes[0].plot(sgr.Lambda.degree, sgr.pm_Lambda_cosBeta.value, linestyle='none', marker='.')
axes[0].set_xlabel('$\\Lambda$ [deg]')
axes[0].set_ylabel(f"$\\mu_\\Lambda \\, \\cos B$ [{sgr.pm_Lambda_cosBeta.unit.to_string('latex_inline')}]")
axes[1].set_title('ICRS')
axes[1].plot(icrs.ra.degree, icrs.pm_ra_cosdec.value, linestyle='none', marker='.')
axes[1].set_ylabel(f"$\\mu_\\alpha \\, \\cos\\delta$ [{icrs.pm_ra_cosdec.unit.to_string('latex_inline')}]")
axes[2].set_title('ICRS')
axes[2].plot(icrs.ra.degree, icrs.pm_dec.value, linestyle='none', marker='.')
axes[2].set_xlabel('RA [deg]')
axes[2].set_ylabel(f"$\\mu_\\delta$ [{icrs.pm_dec.unit.to_string('latex_inline')}]")
plt.show()