...
async def test_huawei_lte_wifi_guest_network_config_entry_when_network_is_not_present(client, hass: HomeAssistant, entity_registry: er.EntityRegistry) -> None:
...
@patch('homeassistant.components.huawei_lte.Connection', MagicMock())
@patch('homeassistant.components.huawei_lte.Client', return_value=magic_client({'Ssids': {'Ssid': [{'wifiisguestnetwork': '1', 'WifiEnable': '0'}]}}))
async def test_huawei_lte_wifi_guest_network_config_entry_when_network_is_present(client, hass: HomeAssistant, entity_registry: er.EntityRegistry) -> None:
...
async def test_turn_on_switch_wifi_guest_network(client, hass: HomeAssistant) -> None:
    ...
    client.return_value = magic_client({'Ssids': {'Ssid': [{'wifiisguestnetwork': '1', 'WifiEnable': '0'}]}})
    huawei_lte = MockConfigEntry(domain=DOMAIN, data={CONF_URL: 'http://huawei-lte'})
    huawei_lte.add_to_hass(hass)
    await hass.config_entries.async_setup(huawei_lte.entry_id)
    await hass.async_block_till_done()
    await hass.services.async_call(SWITCH_DOMAIN, SERVICE_TURN_ON, {ATTR_ENTITY_ID: SWITCH_WIFI_GUEST_NETWORK}, blocking=True)
    await hass.async_block_till_done()
...
async def test_turn_off_switch_wifi_guest_network(client, hass: HomeAssistant) -> None:
    ...
    client.return_value = magic_client({'Ssids': {'Ssid': [{'wifiisguestnetwork': '1', 'WifiEnable': '1'}]}})
    huawei_lte = MockConfigEntry(domain=DOMAIN, data={CONF_URL: 'http://huawei-lte'})
    huawei_lte.add_to_hass(hass)
    await hass.config_entries.async_setup(huawei_lte.entry_id)
    await hass.async_block_till_done()
    await hass.services.async_call(SWITCH_DOMAIN, SERVICE_TURN_OFF, {ATTR_ENTITY_ID: SWITCH_WIFI_GUEST_NETWORK}, blocking=True)
    await hass.async_block_till_done()
...
@patch('homeassistant.components.huawei_lte.Connection', MagicMock())
@patch('homeassistant.components.huawei_lte.Client', return_value=magic_client({'Ssids': {'Ssid': 'str'}}))
async def test_huawei_lte_wifi_guest_network_config_entry_when_ssid_is_str(client, hass: HomeAssistant, entity_registry: er.EntityRegistry) -> None:
...
@patch('homeassistant.components.huawei_lte.Connection', MagicMock())
@patch('homeassistant.components.huawei_lte.Client', return_value=magic_client({'Ssids': {'Ssid': None}}))
async def test_huawei_lte_wifi_guest_network_config_entry_when_ssid_is_none(client, hass: HomeAssistant, entity_registry: er.EntityRegistry) -> None:
