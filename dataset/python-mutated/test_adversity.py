...
@unittest.skipIf(platform.system() == 'Windows', 'not compatible with ProactorEventLoop which is default in Python 3.8+')
    ...
    async def test_pool_handles_abrupt_connection_loss(self):
        ...
        def kill_connectivity():
            if False:
                print('Hello World!')
            self.proxy.trigger_connectivity_loss()
        new_pool = self.create_pool(database='postgres', min_size=pool_size, max_size=pool_size, timeout=cmd_timeout, command_timeout=cmd_timeout)
