...
DATASET = {'display_name': DATASET_NAME, 'image_classification_dataset_metadata': {'classification_type': 'MULTILABEL'}}
...
MODEL = {'display_name': MODEL_NAME, 'image_classification_model_metadata': {'train_budget': 1}}
...
with DAG(DAG_ID, schedule='@once', start_date=datetime(2021, 1, 1), catchup=False, user_defined_macros={'extract_object_id': extract_object_id}, tags=['example', 'automl', 'vision-clss']) as dag:
    ...
    copy_folder_tasks = [GCSToGCSOperator(task_id=f'copy_dataset_folder_{folder}', source_bucket=RESOURCE_DATA_BUCKET, source_object=f'automl/datasets/vision/{folder}', destination_bucket=DATA_SAMPLE_GCS_BUCKET_NAME, destination_object=f'automl/{folder}') for folder in ('cirrus', 'cumulonimbus', 'cumulus')]
    create_dataset_task = AutoMLCreateDatasetOperator(task_id='create_dataset_task', dataset=DATASET, location=GCP_AUTOML_LOCATION)
    dataset_id = cast(str, XComArg(create_dataset_task, key='dataset_id'))
    import_dataset_task = AutoMLImportDataOperator(task_id='import_dataset_task', dataset_id=dataset_id, location=GCP_AUTOML_LOCATION, input_config=IMPORT_INPUT_CONFIG)
    ...
    delete_model = AutoMLDeleteModelOperator(task_id='delete_model', model_id=model_id, location=GCP_AUTOML_LOCATION, project_id=GCP_PROJECT_ID, trigger_rule=TriggerRule.ALL_DONE)
    ...
    create_bucket >> upload_csv_file_to_gcs_task >> copy_folder_tasks >> create_dataset_task >> import_dataset_task >> create_model >> delete_model >> delete_dataset >> delete_bucket
