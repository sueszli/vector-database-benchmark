...
class STTrainingArguments:
    ...
    do_filter_by_confidence: Optional[bool] = dataclasses.field(default=False, metadata={'help': 'Whether to filter the pseudo-labeled data based on the confidence score.'})
    do_filter_by_val_performance: Optional[bool] = dataclasses.field(default=False, metadata={'help': 'Whether to filter the pseudo-labeled data based on the validation performance.'})
    finetune_on_labeled_data: Optional[bool] = dataclasses.field(default=False, metadata={'help': 'Whether to fine-tune on labeled data after pseudo training.'})
    confidence_threshold: Optional[float] = dataclasses.field(default=0.0, metadata={'help': 'Confidence threshold for pseudo-labeled data filtering.'})
    max_selftrain_iterations: Optional[int] = dataclasses.field(default=100, metadata={'help': 'Number of evaluation calls with no improvement after which training will be stopped.'})
    seed: Optional[int] = dataclasses.field(default=None, metadata={'help': 'Random seed for initialization.'})
