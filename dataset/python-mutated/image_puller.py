...
async def _notify_orchest_api_of_env_image_pull(session: aiohttp.ClientSession, image: str) -> None:
    ...
    endpoint = f'http://orchest-api/api/environment-images/{proj_uuid}/{env_uuid}/{tag}/node/{CONFIG_CLASS.CLUSTER_NODE}'
    async with session.put(endpoint) as response:
        if response.status != 200:
            raise Exception(f'Failed to PUT node pull of {image} to the orchest-api.')

async def _notify_orchest_api_of_jupyter_image_pull(session: aiohttp.ClientSession, image: str) -> None:
    tag = _utils.jupyter_image_name_to_tag(image)
    endpoint = f'http://orchest-api/api/ctl/jupyter-images/{tag}/node/{CONFIG_CLASS.CLUSTER_NODE}'
    if tag is None:
        raise ValueError(f'Unexpected image without tag: {image}.')
    async with session.put(endpoint) as response:
        if response.status != 200:
...
class ImagePuller(object):
    ...
    async def _enqueue_pre_pull_orchest_images(self, session: aiohttp.ClientSession, queue: asyncio.Queue):
    ...
    async def _enqueue_active_environment_images(self, session: aiohttp.ClientSession, queue: asyncio.Queue):
        endpoint = f'{self.orchest_api_host}/api/environment-images/active?stored_in_registry=true'
        async with session.get(endpoint) as response:
            response_json = await response.json()
            active_images = response_json['active_environment_images']
        endpoint = f'{self.orchest_api_host}/api/environment-images/active?stored_in_registry=true&not_in_node={CONFIG_CLASS.CLUSTER_NODE}'
        async with session.get(endpoint) as response:
            response_json = await response.json()
            images_to_notify_api_about_pull = set(response_json['active_environment_images'])
        for image in active_images:
            await queue.put((image, image in images_to_notify_api_about_pull))

    async def _enqueue_active_jupyter_images(self, session: aiohttp.ClientSession, queue: asyncio.Queue):
        endpoint = f'{self.orchest_api_host}/api/ctl/active-custom-jupyter-images?stored_in_registry=true'
        async with session.get(endpoint) as response:
            response_json = await response.json()
            active_images = response_json['active_custom_jupyter_images']
        endpoint = f'{self.orchest_api_host}/api/ctl/active-custom-jupyter-images?stored_in_registry=true&not_in_node={CONFIG_CLASS.CLUSTER_NODE}'
        async with session.get(endpoint) as response:
            response_json = await response.json()
            images_to_notify_api_about_pull = set(response_json['active_custom_jupyter_images'])
        for image in active_images:
            await queue.put((image, image in images_to_notify_api_about_pull))
        ...
        async with aiohttp.ClientSession(trust_env=True) as session:
            while True:
                try:
                    ...
                    self.logger.error(f"Attempt '{self.interval}' to get active imagesencountered an exception. Exception was: {ex}.")
    ...
    async def pull_image(self, queue: asyncio.Queue):
        ...
        while True:
            ...
            should_pull = self.policy == Policy.Always or (self.policy == Policy.IfNotPresent and (not await self.container_runtime.image_exists(image_name)))
            for retry in range(self.num_retries):
                try:
                    if should_pull:
                        self.logger.info(f"Pulling image '{image_name}'...")
                        pulled_successfully = await self.container_runtime.pull_image(image_name)
                    should_notify_orchest_api = should_notify_orchest_api and (not should_pull or pulled_successfully)
                    if should_notify_orchest_api:
                        async with aiohttp.ClientSession(trust_env=True) as session:
                            await self.notify_orchest_api_of_image_pull(session, image_name)
                        self.logger.info(f"Notified orchest-api of pull of '{image_name}'")
                    if should_pull:
                    ...
                    self.logger.info(f'{image_name} is already being pulled, skipping task.')
                    break
                except Exception as ex:
                    self.logger.warning(f"Attempt {retry} to pull image '{image_name}' and notify the orchest-api failed with exception - retrying. Exception was: {ex}.")
            queue.task_done()

    async def notify_orchest_api_of_image_pull(self, session: aiohttp.ClientSession, image: str) -> None:
        if 'orchest-env' in image:
            ...
            self.logger.info('Not an environment or jupyter image, not notifying the orchest-api.')
    ...
    async def run(self):
        try:
            ...
            pullers = [asyncio.create_task(self.pull_image(queue)) for _ in range(self.threadiness)]
