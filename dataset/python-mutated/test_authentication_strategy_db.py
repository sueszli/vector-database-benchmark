...
import pytest
from fastapi_users.authentication.strategy import AccessTokenDatabase, AccessTokenProtocol, DatabaseStrategy
...
class TestReadToken:
    ...
    @pytest.mark.asyncio
    async def test_missing_token(self, database_strategy: DatabaseStrategy[UserModel, IDType, AccessTokenModel], user_manager):
        authenticated_user = await database_strategy.read_token(None, user_manager)
        assert authenticated_user is None

    @pytest.mark.asyncio
    async def test_invalid_token(self, database_strategy: DatabaseStrategy[UserModel, IDType, AccessTokenModel], user_manager):
        authenticated_user = await database_strategy.read_token('TOKEN', user_manager)
        assert authenticated_user is None

    @pytest.mark.asyncio
    async def test_valid_token_not_existing_user(self, database_strategy: DatabaseStrategy[UserModel, IDType, AccessTokenModel], access_token_database: AccessTokenDatabaseMock, user_manager):
        await access_token_database.create({'token': 'TOKEN', 'user_id': uuid.UUID('d35d213e-f3d8-4f08-954a-7e0d1bea286f')})
        authenticated_user = await database_strategy.read_token('TOKEN', user_manager)
        assert authenticated_user is None

    @pytest.mark.asyncio
    async def test_valid_token(self, database_strategy: DatabaseStrategy[UserModel, IDType, AccessTokenModel], access_token_database: AccessTokenDatabaseMock, user_manager, user: UserModel):
...
@pytest.mark.authentication
@pytest.mark.asyncio
async def test_write_token(database_strategy: DatabaseStrategy[UserModel, IDType, AccessTokenModel], access_token_database: AccessTokenDatabaseMock, user: UserModel):
...
@pytest.mark.authentication
@pytest.mark.asyncio
async def test_destroy_token(database_strategy: DatabaseStrategy[UserModel, IDType, AccessTokenModel], access_token_database: AccessTokenDatabaseMock, user: UserModel):
    await access_token_database.create({'token': 'TOKEN', 'user_id': user.id})
