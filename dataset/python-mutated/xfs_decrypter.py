...
class XFSDecrypter(SimpleDecrypter):
    ...
    PLUGIN_DOMAIN = None
    URL_REPLACEMENTS = [('&?per_page=\\d+', ''), ('[?/&]+$', ''), ('(.+/[^?]+)$', '\\1?'), ('$', '&per_page=10000')]
    NAME_PATTERN = '<[Tt]itle>.*?\\: (?P<N>.+) folder</[Tt]itle>'
    LINK_PATTERN = '<(?:td|TD).*?>\\s*(?:<.+>\\s*)?<a href="(.+?)".*?>.+?(?:</a>)?\\s*(?:<.+>\\s*)?</(?:td|TD)>'
    OFFLINE_PATTERN = '>\\s*(No such user|\\w+ (Not Found|file (was|has been) removed|no longer available))'
