...
class Test_elbv2_ssl_listeners:
    ...
    def test_elb_no_balancers(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()), mock.patch('prowler.providers.aws.services.elbv2.elbv2_ssl_listeners.elbv2_ssl_listeners.elbv2_client', new=ELBv2(self.set_mocked_audit_info())):
            from prowler.providers.aws.services.elbv2.elbv2_ssl_listeners.elbv2_ssl_listeners import elbv2_ssl_listeners
    ...
    def test_elbv2_with_HTTP_listener(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()), mock.patch('prowler.providers.aws.services.elbv2.elbv2_ssl_listeners.elbv2_ssl_listeners.elbv2_client', new=ELBv2(self.set_mocked_audit_info())):
            ...
            assert search('has non-encrypted listeners', result[0].status_extended)
    ...
    def test_elbv2_with_HTTPS_listener(self):
        ...
        target_group = response.get('TargetGroups')[0]
        target_group_arn = target_group['TargetGroupArn']
        response = conn.create_listener(LoadBalancerArn=lb['LoadBalancerArn'], Protocol='HTTPS', DefaultActions=[{'Type': 'forward', 'TargetGroupArn': target_group_arn}])
        from prowler.providers.aws.services.elbv2.elbv2_service import ELBv2
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()), mock.patch('prowler.providers.aws.services.elbv2.elbv2_ssl_listeners.elbv2_ssl_listeners.elbv2_client', new=ELBv2(self.set_mocked_audit_info())):
            from prowler.providers.aws.services.elbv2.elbv2_ssl_listeners.elbv2_ssl_listeners import elbv2_ssl_listeners
            check = elbv2_ssl_listeners()
            ...
            assert search('has HTTPS listeners only', result[0].status_extended)
            assert result[0].resource_id == 'my-lb'
            assert result[0].resource_arn == lb['LoadBalancerArn']
    ...
    def test_elbv2_with_HTTPS_redirection(self):
        ...
        subnet2 = ec2.create_subnet(VpcId=vpc.id, CidrBlock='172.28.7.0/26', AvailabilityZone=f'{AWS_REGION}b')
        lb = conn.create_load_balancer(Name='my-lb', Subnets=[subnet1.id, subnet2.id], SecurityGroups=[security_group.id], Scheme='internal')['LoadBalancers'][0]
        conn.create_listener(LoadBalancerArn=lb['LoadBalancerArn'], Protocol='HTTP', DefaultActions=[{'Type': 'redirect', 'RedirectConfig': {'Protocol': 'HTTPS', 'Port': '443', 'StatusCode': 'HTTP_301'}}])
        from prowler.providers.aws.services.elbv2.elbv2_service import ELBv2
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=self.set_mocked_audit_info()), mock.patch('prowler.providers.aws.services.elbv2.elbv2_ssl_listeners.elbv2_ssl_listeners.elbv2_client', new=ELBv2(self.set_mocked_audit_info())):
            from prowler.providers.aws.services.elbv2.elbv2_ssl_listeners.elbv2_ssl_listeners import elbv2_ssl_listeners
            check = elbv2_ssl_listeners()
            ...
            assert len(result) == 1
            assert result[0].status == 'PASS'
            assert search('has HTTP listener but it redirects to HTTPS', result[0].status_extended)
            assert result[0].resource_id == 'my-lb'
            assert result[0].resource_arn == lb['LoadBalancerArn']