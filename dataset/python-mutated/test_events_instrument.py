...
from prefect.events.instrument import ResourceTuple, instrument_method_calls_on_class_instances
...
class InstrumentedClass:
    ...
    def _event_method_called_resources(self) -> ResourceTuple:
        if False:
            for i in range(10):
                print('nop')
        return ({'prefect.resource.id': 'prefect.instrumented-class'}, [{'prefect.resource.id': 'prefect.class.InstrumentedClass', 'prefect.resource.role': 'class'}])
...
async def test_instruments_methods(asserting_events_worker: EventsWorker, reset_worker_events):
...
async def test_handles_method_failure(asserting_events_worker: EventsWorker, reset_worker_events):
    instance = InstrumentedClass()
    try:
        instance.sync_fail()
    except Exception:
...
async def test_ignores_excluded_and_private_methods(asserting_events_worker: EventsWorker, reset_worker_events):
    instance = InstrumentedClass()
...
async def test_instrument_idempotent(asserting_events_worker: EventsWorker, reset_worker_events):
...
async def test_skip_event_no_resources(asserting_events_worker: EventsWorker, reset_worker_events):
