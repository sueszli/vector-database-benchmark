...
Example usage:
    python create_live_config.py --project_id <project-id> --location <location>         --live_config_id <live-config-id> --live_stream_uri <uri> --ad_tag_uri <uri>         --slate_id <id>
"""
import argparse
...
if __name__ == '__main__':
    ...
    parser.add_argument('--location', help='The location in which to create the live config.', default='us-central1')
    parser.add_argument('--live_config_id', help='The user-defined live config ID.', required=True)
    parser.add_argument('--live_stream_uri', help='The uri of the livestream to stitch (.mpd or .m3u8 file) in double quotes.', required=True)
    parser.add_argument('--ad_tag_uri', help='Uri of the ad tag in double quotes.', required=True)
    parser.add_argument('--slate_id', help='The user-defined slate ID of the default slate.', required=True)
    args = parser.parse_args()
    create_live_config(args.project_id, args.location, args.live_config_id, args.live_stream_uri, args.ad_tag_uri, args.slate_id)