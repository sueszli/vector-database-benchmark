...
mapVTToTypeString = {pythoncom.VT_I2: 'int', pythoncom.VT_I4: 'int', pythoncom.VT_R4: 'float', pythoncom.VT_R8: 'float', pythoncom.VT_BSTR: 'str', pythoncom.VT_BOOL: 'int', pythoncom.VT_VARIANT: 'type', pythoncom.VT_I1: 'int', pythoncom.VT_UI1: 'int', pythoncom.VT_UI2: 'int', pythoncom.VT_UI4: 'int', pythoncom.VT_I8: 'int', pythoncom.VT_UI8: 'int', pythoncom.VT_INT: 'int', pythoncom.VT_DATE: 'datetime.date', pythoncom.VT_UINT: 'int'}
    ...
    def WriteClassBody(self, generator):
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                if resultName:
                    print(f"\t\t# Property '{key}' is an object of type '{resultName}'", file=stream)
                lkey = key.lower()
                details = entry.desc
                ...
                if lkey in specialItems and specialItems[lkey] is None:
                    ...
                    if details.memid == pythoncom.DISPID_NEWENUM:
                        continue
                print(f'\t\t"{build.MakePublicAttributeName(key)}": {mapEntry},', file=stream)
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                if entry.GetResultName():
                    print("\t\t# Method '{}' returns object of type '{}'".format(key, entry.GetResultName()), file=stream)
                details = entry.desc
        ...
        if specialItems['value']:
            ...
            print(f"\t# Default {typename} for this class is '{entry.names[0]}'", file=stream)
            for line in ret:
                print(line, file=stream)
        ...
        if specialItems['item']:
            ...
            print('\t\treturn self._get_good_object_(self._oleobj_.Invoke(*(%d, LCID, %d, 1, key)), "Item", %s)' % (entry.desc.memid, invoketype, resultCLSID), file=stream)
    ...
    def WriteClass(self, generator):
        ...
        if generator.generate_type == GEN_DEMAND_CHILD:
            ...
            for ref in referenced_items:
                print(f"__import__('{generator.base_mod_name}.{ref.python_name}')", file=stream)
                print("{} = sys.modules['{}.{}'].{}".format(ref.python_name, generator.base_mod_name, ref.python_name, ref.python_name), file=stream)
                ref.bWritten = 1
        try:
            progId = pythoncom.ProgIDFromCLSID(self.clsid)
...
class Generator:
    ...
    def do_gen_file_header(self):
        ...
        print('# By python version {}'.format(sys.version.replace('\n', '-')), file=self.file)
        if self.sourceFilename:
            print(f"# From type library '{os.path.split(self.sourceFilename)[1]}'", file=self.file)
    ...
    def do_generate(self):
        ...
        for record in recordItems.values():
            if record.clsid == pythoncom.IID_NULL:
                print("\t###{}: {}, # Record disabled because it doesn't have a non-null GUID".format(repr(record.doc[0]), repr(str(record.clsid))), file=stream)
            else:
                print(f'\t{repr(record.doc[0])}: {repr(str(record.clsid))},', file=stream)
        print('}', file=stream)
        print(file=stream)
        if self.generate_type == GEN_FULL:
            ...
            for item in oleItems.values():
                if item is not None and item.bWritten:
                    print(f"\t'{str(item.clsid)}' : {item.python_name},", file=stream)
            print('}', file=stream)
            ...
            for item in oleItems.values():
                if item is not None:
                    print(f"\t'{str(item.clsid)}' : {repr(item.python_name)},", file=stream)
            print('}', file=stream)
            print('VTablesToClassMap = {}', file=stream)
            print('VTablesToPackageMap = {', file=stream)
            for item in vtableItems.values():
    ...
    def generate_child(self, child, dir):
        ...
        major = la[3]
        minor = la[4]
        self.base_mod_name = 'win32com.gen_py.' + str(clsid)[1:-1] + f'x{lcid}x{major}x{minor}'
        try:
            oleItems = {}
            vtableItems = {}
                ...
                for type_info_tuple in infos:
                    ...
                    if infotype in [pythoncom.TKIND_INTERFACE, pythoncom.TKIND_DISPATCH]:
                        if build.MakePublicAttributeName(doc[0]) == child:
                            ...
                            if vtableItem is not None:
                                vtableItems[clsid] = vtableItem
            assert found, f"Cant find the '{child}' interface in the CoClasses, or the interfaces"
            items = {}
    ...
    def do_gen_child_item(self, oleitem):
        ...
        oleitem.WriteClass(self)
        if oleitem.bWritten:
            print('win32com.client.CLSIDToClass.RegisterCLSID( "{}", {} )'.format(oleitem.clsid, oleitem.python_name), file=self.file)
