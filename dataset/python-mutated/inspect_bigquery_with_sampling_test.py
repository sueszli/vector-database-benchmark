...
def test_inspect_bigquery_with_sampling(subscriber_client: MagicMock, dlp_client: MagicMock, capsys: pytest.CaptureFixture) -> None:
    if False:
        print('Hello World!')
    mock_dlp_instance = dlp_client.return_value
    mock_subscriber_instance = subscriber_client.return_value
    mock_job_and_subscriber(mock_dlp_instance, mock_subscriber_instance, f'projects/{GCLOUD_PROJECT}/dlpJobs/test_job', 'PERSON_NAME', random.randint(0, 1000))
    inspect_content.inspect_bigquery_table_with_sampling(GCLOUD_PROJECT, 'topic_id', 'subscription_id', timeout=1)
