...
def test_encrypt_and_insert_data(pool: sqlalchemy.engine.Engine, env_aead: tink.aead.KmsEnvelopeAead) -> None:
    if False:
        while True:
            i = 10
    ...
    decrypted_emails = []
    with pool.connect() as conn:
        results = conn.execute(f'SELECT team, time_cast, voter_email FROM {table_name} ORDER BY time_cast DESC LIMIT 5').fetchall()
        for row in results:
            team = row[0]
