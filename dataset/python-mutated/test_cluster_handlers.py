...
async def test_in_cluster_handler_config(cluster_id, bind_count, attrs, endpoint, zigpy_device_mock, zha_gateway) -> None:
    """Test ZHA core cluster handler configuration for input clusters."""
    zigpy_dev = zigpy_device_mock({1: {SIG_EP_INPUT: [cluster_id], SIG_EP_OUTPUT: [], SIG_EP_TYPE: 4660}}, '00:11:22:33:44:55:66:77', 'test manufacturer', 'test model')
    cluster = zigpy_dev.endpoints[1].in_clusters[cluster_id]
    cluster_handler_class = registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.get(cluster_id, cluster_handlers.ClusterHandler)
    ...
    reported_attrs = {a for a in attrs for attr in cluster.configure_reporting_multiple.call_args_list for attrs in attr[0][0]}
    assert set(attrs) == reported_attrs

@pytest.mark.parametrize(('cluster_id', 'bind_count'), [(0, 0), (1, 1), (2, 1), (3, 0), (4, 0), (5, 1), (6, 1), (7, 1), (8, 1), (9, 1), (21, 1), (22, 1), (25, 0), (26, 1), (27, 1), (32, 1), (33, 0), (257, 1), (514, 1), (768, 1), (1024, 1), (1026, 1), (1027, 1), (1029, 1), (1030, 1), (1794, 1), (2820, 1)])
async def test_out_cluster_handler_config(cluster_id, bind_count, endpoint, zigpy_device_mock, zha_gateway) -> None:
    """Test ZHA core cluster handler configuration for output clusters."""
    zigpy_dev = zigpy_device_mock({1: {SIG_EP_OUTPUT: [cluster_id], SIG_EP_INPUT: [], SIG_EP_TYPE: 4660}}, '00:11:22:33:44:55:66:77', 'test manufacturer', 'test model')
...
async def test_ep_cluster_handlers_configure(cluster_handler) -> None:
    ...
    with mock.patch.dict(endpoint.claimed_cluster_handlers, claimed, clear=True), mock.patch.dict(endpoint.client_cluster_handlers, client_handlers, clear=True):
...
async def test_poll_control_configure(poll_control_ch) -> None:
    ...
    assert poll_control_ch.cluster.write_attributes.call_args[0][0] == {'checkin_interval': poll_control_ch.CHECKIN_INTERVAL}
...
async def test_poll_control_checkin_response(poll_control_ch) -> None:
    ...
    assert cluster.endpoint.request.call_args_list[0][0][0] == 32
    assert cluster.endpoint.request.call_args_list[1][0][0] == 32

async def test_poll_control_cluster_command(hass: HomeAssistant, poll_control_device) -> None:
    ...
    with mock.patch.object(poll_control_ch, 'check_in_response', checkin_mock):
        ...
        assert not events
        cluster.handle_message(hdr, [mock.sentinel.args, mock.sentinel.args2, mock.sentinel.args3])
...
async def test_poll_control_ignore_list(hass: HomeAssistant, poll_control_device) -> None:
...
async def test_zll_device_groups(zigpy_zll_device, endpoint, zigpy_coordinator_device) -> None:
    ...
    get_group_identifiers_rsp = zigpy.zcl.clusters.lightlink.LightLink.commands_by_name['get_group_identifiers_rsp'].schema
    with patch.object(cluster, 'command', AsyncMock(return_value=get_group_identifiers_rsp(total=0, start_index=0, group_info_records=[]))) as cmd_mock:
        await cluster_handler.async_configure()
        assert cmd_mock.await_count == 1
        assert cluster.server_commands[cmd_mock.await_args[0][0]].name == 'get_group_identifiers'
        assert cluster.bind.call_count == 0
        assert zigpy_coordinator_device.add_to_group.await_count == 1
        assert zigpy_coordinator_device.add_to_group.await_args[0][0] == 0
    zigpy_coordinator_device.add_to_group.reset_mock()
    group_1 = zigpy.zcl.clusters.lightlink.GroupInfoRecord(43981, 0)
    group_2 = zigpy.zcl.clusters.lightlink.GroupInfoRecord(43707, 0)
    with patch.object(cluster, 'command', AsyncMock(return_value=get_group_identifiers_rsp(total=2, start_index=0, group_info_records=[group_1, group_2]))) as cmd_mock:
        await cluster_handler.async_configure()
        assert cmd_mock.await_count == 1
        assert cluster.server_commands[cmd_mock.await_args[0][0]].name == 'get_group_identifiers'
        assert cluster.bind.call_count == 0
        assert zigpy_coordinator_device.add_to_group.await_count == 2
        assert zigpy_coordinator_device.add_to_group.await_args_list[0][0][0] == group_1.group_id
        assert zigpy_coordinator_device.add_to_group.await_args_list[1][0][0] == group_2.group_id

@mock.patch('homeassistant.components.zha.core.discovery.PROBE.discover_entities', mock.MagicMock())
async def test_cluster_no_ep_attribute(zha_device_mock: Callable[..., ZHADevice]) -> None:
    """Test cluster handlers for clusters without ep_attribute."""
    zha_device = zha_device_mock({1: {SIG_EP_INPUT: [1070], SIG_EP_OUTPUT: [], SIG_EP_TYPE: 4660}})
...
async def test_configure_reporting(hass: HomeAssistant, endpoint) -> None:
    ...
    class TestZigbeeClusterHandler(cluster_handlers.ClusterHandler):
        BIND = True
        REPORT_CONFIG = (cluster_handlers.AttrReportConfig(attr='current_x', config=(1, 60, 1)), cluster_handlers.AttrReportConfig(attr='current_hue', config=(1, 60, 2)), cluster_handlers.AttrReportConfig(attr='color_temperature', config=(1, 60, 3)), cluster_handlers.AttrReportConfig(attr='current_y', config=(1, 60, 4)))
    ...
    cluster.bind = AsyncMock(spec_set=cluster.bind, return_value=[zdo_t.Status.SUCCESS])
    cluster.configure_reporting_multiple = AsyncMock(spec_set=cluster.configure_reporting_multiple, return_value=[foundation.ConfigureReportingResponseRecord(status=foundation.Status.SUCCESS)])
    cluster_handler = TestZigbeeClusterHandler(cluster, endpoint)
    await cluster_handler.async_configure()
    assert cluster.configure_reporting_multiple.mock_calls == [mock.call({'current_x': (1, 60, 1), 'current_hue': (1, 60, 2), 'color_temperature': (1, 60, 3)}), mock.call({'current_y': (1, 60, 4)})]
...
async def test_invalid_cluster_handler(hass: HomeAssistant, caplog) -> None:
    ...
    class TestZigbeeClusterHandler(cluster_handlers.ClusterHandler):
        REPORT_CONFIG = (cluster_handlers.AttrReportConfig(attr='missing_attr', config=(1, 60, 1)),)
    ...
    with patch.dict(registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY, {cluster.cluster_id: TestZigbeeClusterHandler}), caplog.at_level(logging.WARNING):
        zha_endpoint.add_all_cluster_handlers()
    assert 'missing_attr' in caplog.text

@pytest.mark.parametrize(('side_effect', 'expected_error'), [(zigpy.exceptions.ZigbeeException(), 'Failed to send request'), (zigpy.exceptions.ZigbeeException('Zigbee exception'), 'Failed to send request: Zigbee exception'), (asyncio.TimeoutError(), 'Failed to send request: device did not respond')])
async def test_retry_request(side_effect: Exception | None, expected_error: str | None) -> None:
