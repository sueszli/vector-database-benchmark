...
class TestPool1D_API(unittest.TestCase):
    ...
    def check_adaptive_avg_dygraph_results(self, place):
        ...
        with base.dygraph.guard(place):
            ...
            result_np = avg_pool1D_forward_naive(input_np, ksize=[16], strides=[0], paddings=[0], adaptive=True)
            np.testing.assert_allclose(result.numpy(), result_np, rtol=1e-05)
            ada_max_pool1d_dg = paddle.nn.layer.AdaptiveAvgPool1D(output_size=16)
            result = ada_max_pool1d_dg(input)
            np.testing.assert_allclose(result.numpy(), result_np, rtol=1e-05)
            result = paddle.nn.functional.common.interpolate(input, mode='area', size=16)
            np.testing.assert_allclose(result.numpy(), result_np, rtol=1e-05)
    ...
    def check_adaptive_avg_static_results(self, place):
        ...
        with base.program_guard(base.Program(), base.Program()):
            ...
            result = F.adaptive_avg_pool1d(input, output_size=16)
            input_np = np.random.random([2, 3, 32]).astype('float32')
            result_np = avg_pool1D_forward_naive(input_np, ksize=[16], strides=[2], paddings=[0], adaptive=True)
