...
class Resolver(dns.resolver.BaseResolver):
    ...
    async def resolve(self, qname: Union[dns.name.Name, str], rdtype: Union[dns.rdatatype.RdataType, str]=dns.rdatatype.A, rdclass: Union[dns.rdataclass.RdataClass, str]=dns.rdataclass.IN, tcp: bool=False, source: Optional[str]=None, raise_on_no_answer: bool=True, source_port: int=0, lifetime: Optional[float]=None, search: Optional[bool]=None, backend: Optional[dns.asyncbackend.Backend]=None) -> dns.resolver.Answer:
        ...
        *backend*, a ``dns.asyncbackend.Backend``, or ``None``.  If ``None``,
        ...
        while True:
            ...
            while not done:
                ...
                try:
                    response = await nameserver.async_query(request, timeout=timeout, source=source, source_port=source_port, max_size=tcp, backend=backend)
    ...
    async def resolve_name(self, name: Union[dns.name.Name, str], family: int=socket.AF_UNSPEC, **kwargs: Any) -> dns.resolver.HostAnswers:
        ...
        *family*, an ``int``, the address family.  If socket.AF_UNSPEC
        ...
        start = time.time()
        v6 = await self.resolve(name, dns.rdatatype.AAAA, raise_on_no_answer=False, lifetime=self._compute_timeout(start, lifetime), **modified_kwargs)
        name = v6.qname
        v4 = await self.resolve(name, dns.rdatatype.A, raise_on_no_answer=False, lifetime=self._compute_timeout(start, lifetime), **modified_kwargs)
...
async def resolve(qname: Union[dns.name.Name, str], rdtype: Union[dns.rdatatype.RdataType, str]=dns.rdatatype.A, rdclass: Union[dns.rdataclass.RdataClass, str]=dns.rdataclass.IN, tcp: bool=False, source: Optional[str]=None, raise_on_no_answer: bool=True, source_port: int=0, lifetime: Optional[float]=None, search: Optional[bool]=None, backend: Optional[dns.asyncbackend.Backend]=None) -> dns.resolver.Answer:
    """Query nameservers asynchronously to find the answer to the question.
    ...
    """
    return await get_default_resolver().resolve(qname, rdtype, rdclass, tcp, source, raise_on_no_answer, source_port, lifetime, search, backend)
...
async def resolve_name(name: Union[dns.name.Name, str], family: int=socket.AF_UNSPEC, **kwargs: Any) -> dns.resolver.HostAnswers:
...
async def zone_for_name(name: Union[dns.name.Name, str], rdclass: dns.rdataclass.RdataClass=dns.rdataclass.IN, tcp: bool=False, resolver: Optional[Resolver]=None, backend: Optional[dns.asyncbackend.Backend]=None) -> dns.name.Name:
...
async def make_resolver_at(where: Union[dns.name.Name, str], port: int=53, family: int=socket.AF_UNSPEC, resolver: Optional[Resolver]=None) -> Resolver:
...
async def resolve_at(where: Union[dns.name.Name, str], qname: Union[dns.name.Name, str], rdtype: Union[dns.rdatatype.RdataType, str]=dns.rdatatype.A, rdclass: Union[dns.rdataclass.RdataClass, str]=dns.rdataclass.IN, tcp: bool=False, source: Optional[str]=None, raise_on_no_answer: bool=True, source_port: int=0, lifetime: Optional[float]=None, search: Optional[bool]=None, backend: Optional[dns.asyncbackend.Backend]=None, port: int=53, family: int=socket.AF_UNSPEC, resolver: Optional[Resolver]=None) -> dns.resolver.Answer:
    ...
    return await res.resolve(qname, rdtype, rdclass, tcp, source, raise_on_no_answer, source_port, lifetime, search, backend)