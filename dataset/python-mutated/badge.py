...
class GamificationBadge(models.Model):
    ...
    rule_auth = fields.Selection([('everyone', 'Everyone'), ('users', 'A selected list of users'), ('having', 'People having some badges'), ('nobody', 'No one, assigned through challenges')], default='everyone', string='Allowance to Grant', help='Who can grant this badge', required=True)
    rule_auth_user_ids = fields.Many2many('res.users', 'rel_badge_auth_users', string='Authorized Users', help='Only these people can give this badge')
    rule_auth_badge_ids = fields.Many2many('gamification.badge', 'gamification_badge_rule_badge_rel', 'badge1_id', 'badge2_id', string='Required Badges', help='Only the people having these badges can give this badge')
    ...
    goal_definition_ids = fields.Many2many('gamification.goal.definition', 'badge_unlocked_definition_rel', string='Rewarded by', help='The users that have succeeded theses goals will receive automatically the badge.')
    owner_ids = fields.One2many('gamification.badge.user', 'badge_id', string='Owners', help='The list of instances of this badge granted to users')
    ...
    unique_owner_ids = fields.Many2many('res.users', string='Unique Owners', compute='_get_owners_info', help='The list of unique users having received this badge.')
    stat_this_month = fields.Integer('Monthly total', compute='_get_badge_user_stats', help='The number of time this badge has been received this month.')
    stat_my = fields.Integer('My Total', compute='_get_badge_user_stats', help='The number of time the current user has received this badge.')
    stat_my_this_month = fields.Integer('My Monthly Total', compute='_get_badge_user_stats', help='The number of time the current user has received this badge this month.')
    stat_my_monthly_sending = fields.Integer('My Monthly Sending Total', compute='_get_badge_user_stats', help='The number of time the current user has sent this badge this month.')
    remaining_sending = fields.Integer('Remaining Sending Allowed', compute='_remaining_sending_calc', help='If a maximum is set')
