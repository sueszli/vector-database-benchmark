...
SimpleRow = typing.NamedTuple('SimpleRow', [('id', int), ('str', str), ('flt', float)])
...
Shopper = typing.NamedTuple('Shopper', [('shopper', str), ('cart', typing.Mapping[str, int])])
coders.registry.register_coder(Shopper, coders.RowCoder)

@pytest.mark.xlang_sql_expansion_service
@unittest.skipIf(TestPipeline().get_pipeline_options().view_as(StandardOptions).runner is None, 'Must be run with a runner that supports staging java artifacts.')
