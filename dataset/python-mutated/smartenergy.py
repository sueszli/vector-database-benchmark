...
from ..const import REPORT_CONFIG_ASAP, REPORT_CONFIG_DEFAULT, REPORT_CONFIG_OP, SIGNAL_ATTR_UPDATED
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(smartenergy.DeviceManagement.cluster_id)
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(smartenergy.EnergyManagement.cluster_id)
...
@registries.ZIGBEE_CLUSTER_HANDLER_REGISTRY.register(smartenergy.KeyEstablishment.cluster_id)
...
class Metering(ClusterHandler):
    ...
    async def async_initialize_cluster_handler_specific(self, from_cache: bool) -> None:
        ...
        fmting = self.cluster.get('demand_formatting', 249)
        self._format_spec = self.get_formatting(fmting)
        fmting = self.cluster.get('summation_formatting', 249)
