...
if __name__ == '__main__':
    ...
    tuner = tune.Tuner('BanditLinTS', param_space=config.to_dict(), run_config=air.RunConfig(stop={'training_iteration': training_iterations}, checkpoint_config=air.CheckpointConfig(checkpoint_at_end=True)), tune_config=tune.TuneConfig(num_samples=1))
    results = tuner.fit()
    print('The trials took', time.time() - start_time, 'seconds\n')
    result = results.get_best_result()
    x = result.metrics_dataframe.groupby('agent_timesteps_total')['episode_reward_mean'].aggregate(['mean', 'max', 'min', 'std'])
