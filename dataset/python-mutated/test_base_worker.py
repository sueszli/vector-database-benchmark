...
from prefect.exceptions import CrashedRun, InfrastructureNotAvailable, InfrastructureNotFound, ObjectNotFound
from prefect.flows import flow
from prefect.server import models
from prefect.server.schemas.core import Flow
from prefect.server.schemas.responses import DeploymentResponse
from prefect.server.schemas.states import StateType
from prefect.settings import PREFECT_EXPERIMENTAL_ENABLE_ENHANCED_CANCELLATION, PREFECT_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION, PREFECT_WORKER_PREFETCH_SECONDS, get_current_settings, temporary_settings
...
class WorkerTestImpl(BaseWorker):
    ...
    async def kill_infrastructure(self, infrastructure_pid: str, grace_seconds: int=30, configuration: Optional[BaseJobConfiguration]=None):
        pass
...
async def ensure_default_agent_pool_exists(session):
    ...
    if default_work_pool is None:
        await models.workers.create_work_pool(session=session, work_pool=schemas.actions.WorkPoolCreate(name=models.workers.DEFAULT_AGENT_WORK_POOL_NAME, type='prefect-agent'))
...
async def test_worker_creates_work_pool_by_default_during_sync(prefect_client: PrefectClient):
...
async def test_worker_does_not_creates_work_pool_when_create_pool_is_false(prefect_client: PrefectClient):
    with pytest.raises(ObjectNotFound):
        await prefect_client.read_work_pool('test-work-pool')
...
async def test_worker_sends_heartbeat_messages(prefect_client: PrefectClient):
    async with WorkerTestImpl(name='test', work_pool_name='test-work-pool') as worker:
        await worker.sync_with_backend()
...
async def test_worker_with_work_pool(prefect_client: PrefectClient, worker_deployment_wq1, work_pool):
...
async def test_worker_with_work_pool_and_work_queue(prefect_client: PrefectClient, worker_deployment_wq1, worker_deployment_wq_2, work_queue_1, work_pool):
...
async def test_worker_with_work_pool_and_limit(prefect_client: PrefectClient, worker_deployment_wq1, work_pool):
...
async def test_worker_calls_run_with_expected_arguments(prefect_client: PrefectClient, worker_deployment_wq1, work_pool, monkeypatch):
    run_mock = AsyncMock()
...
async def test_worker_warns_when_running_a_flow_run_with_a_storage_block(prefect_client: PrefectClient, deployment, work_pool, caplog):
...
async def test_base_worker_gets_job_configuration_when_syncing_with_backend_with_just_job_config(session, client):
    ...
    expected_job_template = {'job_configuration': {'command': '{{ command }}', 'env': '{{ env }}', 'labels': '{{ labels }}', 'name': '{{ name }}', 'other': '{{ other }}'}, 'variables': {'properties': {'command': {'type': 'string', 'title': 'Command', 'description': 'The command to use when starting a flow run. In most cases, this should be left blank and the command will be automatically generated by the worker.'}, 'env': {'title': 'Environment Variables', 'type': 'object', 'additionalProperties': {'type': 'string'}, 'description': 'Environment variables to set when starting a flow run.'}, 'labels': {'title': 'Labels', 'type': 'object', 'additionalProperties': {'type': 'string'}, 'description': 'Labels applied to infrastructure created by the worker using this job configuration.'}, 'name': {'type': 'string', 'title': 'Name', 'description': 'Name given to infrastructure created by the worker using this job configuration.'}, 'other': {'type': 'string', 'title': 'Other'}}, 'type': 'object'}}
    pool_name = 'test-pool'
...
async def test_job_configuration_from_template_and_overrides_with_nested_variables():
    ...
    assert config.dict() == {'command': None, 'env': {}, 'labels': {}, 'name': None, 'config': {'var1': 'woof!', 'var2': 42}}
...
async def test_job_configuration_from_template_and_overrides_with_hard_coded_primitives():
    ...
    assert config.dict() == {'command': None, 'env': {}, 'labels': {}, 'name': None, 'config': {'var1': 1, 'var2': 1.1, 'var3': True}}
...
async def test_job_configuration_from_template_overrides_with_block():
    ...
    assert config.dict() == {'command': None, 'env': {}, 'labels': {}, 'name': None, 'var1': 'woof!', 'arbitrary_block': {'a': 1, 'b': 'hello', 'block_type_slug': 'arbitraryblock'}}
...
async def test_job_configuration_from_template_overrides_with_remote_variables():
    ...
    assert config.dict() == {'command': None, 'env': {'MY_ENV_VAR': 'test_value_2'}, 'labels': {}, 'name': None, 'var1': 'test_value_1'}
...
async def test_job_configuration_from_template_and_overrides_with_variables_in_a_list():
    ...
    config = await ArbitraryJobConfiguration.from_template_and_values(base_job_template=template, values={'var1': 'woof!'})
    assert config.dict() == {'command': None, 'env': {}, 'labels': {}, 'name': None, 'config': ['woof!', 42]}
...
class TestCancellation:
    ...
    async def test_worker_cancel_run_called_for_cancelling_run(self, prefect_client: PrefectClient, worker_deployment_wq1, cancelling_constructor, work_pool):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        async with WorkerTestImpl(work_pool_name=work_pool.name) as worker:
            await worker.sync_with_backend()
    ...
    async def test_worker_cancel_run_not_called_for_other_states(self, prefect_client: PrefectClient, worker_deployment_wq1, state, work_pool):
        await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=state)
        async with WorkerTestImpl(work_pool_name=work_pool.name) as worker:
            await worker.sync_with_backend()
            worker.cancel_run = AsyncMock()
            await worker.check_for_cancelled_flow_runs()
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_called_for_cancelling_run_with_multiple_work_queues(self, prefect_client: PrefectClient, worker_deployment_wq1, cancelling_constructor, work_pool, work_queue_1, work_queue_2):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        async with WorkerTestImpl(work_pool_name=work_pool.name, work_queues=[work_queue_1.name, work_queue_2.name]) as worker:
            await worker.sync_with_backend()
            worker.cancel_run = AsyncMock()
            await worker.check_for_cancelled_flow_runs()
        worker.cancel_run.assert_awaited_once_with(flow_run)
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_not_called_for_same_queue_names_in_different_work_pool(self, prefect_client: PrefectClient, deployment, cancelling_constructor, work_pool, work_queue_1, work_queue_2):
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_not_called_for_other_work_queues(self, prefect_client: PrefectClient, worker_deployment_wq1, cancelling_constructor, work_pool):
        await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        async with WorkerTestImpl(work_pool_name=work_pool.name, work_queues=[f'not-{worker_deployment_wq1.work_queue_name}'], prefetch_seconds=10) as worker:
            await worker.sync_with_backend()
            worker.cancel_run = AsyncMock()
            await worker.check_for_cancelled_flow_runs()
        worker.cancel_run.assert_not_called()
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_kills_run_with_infrastructure_pid(self, prefect_client: PrefectClient, worker_deployment_wq1, cancelling_constructor, work_pool):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        await prefect_client.update_flow_run(flow_run.id, infrastructure_pid='test')
        async with WorkerTestImpl(work_pool_name=work_pool.name, prefetch_seconds=10) as worker:
            await worker.sync_with_backend()
            worker.kill_infrastructure = AsyncMock()
            await worker.check_for_cancelled_flow_runs()
            configuration = await worker._get_configuration(flow_run)
        worker.kill_infrastructure.assert_awaited_once_with(infrastructure_pid='test', configuration=configuration)

    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_with_missing_infrastructure_pid(self, prefect_client: PrefectClient, worker_deployment_wq1, caplog, cancelling_constructor, work_pool):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        async with WorkerTestImpl(work_pool_name=work_pool.name, prefetch_seconds=10) as worker:
            await worker.sync_with_backend()
            worker.kill_infrastructure = AsyncMock()
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_updates_state_type(self, prefect_client: PrefectClient, worker_deployment_wq1, cancelling_constructor, work_pool):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        await prefect_client.update_flow_run(flow_run.id, infrastructure_pid='test')
        async with WorkerTestImpl(work_pool_name=work_pool.name, prefetch_seconds=10) as worker:
            await worker.sync_with_backend()
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_handles_missing_deployment(self, prefect_client: PrefectClient, worker_deployment_wq1, cancelling_constructor, work_pool):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_preserves_other_state_properties(self, prefect_client: PrefectClient, worker_deployment_wq1, cancelling_constructor, work_pool):
        expected_changed_fields = {'type', 'name', 'timestamp', 'id'}
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor(message='test'))
        await prefect_client.update_flow_run(flow_run.id, infrastructure_pid='test')
        async with WorkerTestImpl(work_pool_name=work_pool.name, prefetch_seconds=10) as worker:
            await worker.sync_with_backend()
            await worker.check_for_cancelled_flow_runs()
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_with_infrastructure_not_available_during_kill(self, prefect_client: PrefectClient, worker_deployment_wq1, caplog, cancelling_constructor, work_pool):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        await prefect_client.update_flow_run(flow_run.id, infrastructure_pid='test')
        async with WorkerTestImpl(work_pool_name=work_pool.name, prefetch_seconds=10) as worker:
            await worker.sync_with_backend()
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_with_infrastructure_not_found_during_kill(self, prefect_client: PrefectClient, worker_deployment_wq1, caplog, cancelling_constructor, work_pool):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        await prefect_client.update_flow_run(flow_run.id, infrastructure_pid='test')
        async with WorkerTestImpl(work_pool_name=work_pool.name, prefetch_seconds=10) as worker:
            await worker.sync_with_backend()
    ...
    @pytest.mark.flaky(max_runs=3)
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_with_unknown_error_during_kill(self, prefect_client: PrefectClient, worker_deployment_wq1, caplog, cancelling_constructor, work_pool):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        await prefect_client.update_flow_run(flow_run.id, infrastructure_pid='test')
        async with WorkerTestImpl(work_pool_name=work_pool.name, prefetch_seconds=10) as worker:
            await worker.sync_with_backend()
            ...
            await worker.check_for_cancelled_flow_runs()
            configuration = await worker._get_configuration(flow_run)
        worker.kill_infrastructure.assert_has_awaits([call(infrastructure_pid='test', configuration=configuration), call(infrastructure_pid='test', configuration=configuration)])
        post_flow_run = await prefect_client.read_flow_run(flow_run.id)
        assert post_flow_run.state.name == 'Cancelling'
    ...
    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_without_infrastructure_support_for_kill(self, prefect_client: PrefectClient, worker_deployment_wq1, caplog, cancelling_constructor, work_pool):
        ...
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        await prefect_client.update_flow_run(flow_run.id, infrastructure_pid='test')
        async with WorkerNoKill(work_pool_name=work_pool.name, prefetch_seconds=10) as worker:
            await worker.sync_with_backend()
            await worker.check_for_cancelled_flow_runs()
        post_flow_run = await prefect_client.read_flow_run(flow_run.id)
        assert post_flow_run.state.name == 'Cancelling'
        assert f'Worker type {worker_type!r} does not support killing created infrastructure.' in caplog.text
        assert 'Cancellation cannot be guaranteed.' in caplog.text

    @pytest.mark.parametrize('cancelling_constructor', [legacy_named_cancelling_state, Cancelling])
    async def test_worker_cancel_run_skips_with_runner(self, prefect_client: PrefectClient, worker_deployment_wq1, caplog, cancelling_constructor, enable_enhanced_cancellation, work_pool):
        flow_run = await prefect_client.create_flow_run_from_deployment(worker_deployment_wq1.id, state=cancelling_constructor())
        async with WorkerTestImpl(work_pool_name=work_pool.name) as worker:
            await worker.sync_with_backend()
            await worker.check_for_cancelled_flow_runs()
...
async def test_get_flow_run_logger(prefect_client: PrefectClient, worker_deployment_wq1, work_pool):
    ...
    async with WorkerTestImpl(name='test', work_pool_name='test-work-pool', create_pool_if_not_found=False) as worker:
        ...
        assert logger.extra == {'flow_run_name': flow_run.name, 'flow_run_id': str(flow_run.id), 'flow_name': '<unknown>', 'worker_name': 'test', 'work_pool_name': 'test-work-pool', 'work_pool_id': str(work_pool.id)}

class TestInfrastructureIntegration:

    async def test_agent_crashes_flow_if_infrastructure_submission_fails(self, prefect_client: PrefectClient, worker_deployment_infra_wq1, work_pool, monkeypatch):
