    ...
    def execute(self):
        ...
        for web_acl in wafv2_client.web_acls:
            ...
            if web_acl.logging_enabled:
                report.status = 'PASS'
                report.status_extended = f'AWS WAFv2 Web ACL {web_acl.id} has logging enabled.'
            else:
                report.status = 'FAIL'
                report.status_extended = f'AWS WAFv2 Web ACL {web_acl.id} does not have logging enabled.'
