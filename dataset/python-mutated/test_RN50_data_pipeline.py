...
parser.add_argument('--prefetch', default=2, type=int, metavar='N', help='prefetch queue depth (default: 2)')
parser.add_argument('--separate_queue', action='store_true', help='Use separate queues executor')
parser.add_argument('--cpu_size', default=2, type=int, metavar='N', help='cpu prefetch queue depth (default: 2)')
parser.add_argument('--gpu_size', default=2, type=int, metavar='N', help='gpu prefetch queue depth (default: 2)')
...
parser.add_argument('-i', '--iters', default=-1, type=int, metavar='N', help='Number of iterations to run (default: -1 - whole data set)')
parser.add_argument('--epochs', default=2, type=int, metavar='N', help='Number of epochs to run')
parser.add_argument('--decoder_type', default='', type=str, metavar='N', help='roi, cached (default: regular nvjpeg)')
...
parser.add_argument('--reader_queue_depth', default=1, type=int, metavar='N', help='prefetch queue depth (default: 1)')
parser.add_argument('--read_shuffle', action='store_true', help='Shuffle data when reading')
parser.add_argument('--disable_mmap', action='store_true', help='Disable mmap for DALI readers. Used for network filesystem tests.')
parser.add_argument('-s', '--small', action='store_true', help='use small dataset, DALI_EXTRA_PATH needs to be set')
parser.add_argument('--number_of_shards', default=None, type=int, metavar='N', help='Number of shards in the dataset')
parser.add_argument('--assign_gpu', default=None, type=int, metavar='N', help='Assign a given GPU. Cannot be used with --gpus')
parser.add_argument('--assign_shard', default=0, type=int, metavar='N', help='Assign a given shard id. If used with --gpus, it assigns the first GPU to this id and next GPUs get consecutive ids')
parser.add_argument('--simulate_N_gpus', default=None, type=int, metavar='N', help='Used to simulate small shard as it would be in a multi gpu setup with this number of gpus. If provided, each gpu will see a shard size as if we were in a multi gpu setup with this number of gpus', dest='number_of_shards')
parser.add_argument('--remove_default_pipeline_paths', action='store_true', help='For all data pipeline types, remove the default values')
parser.add_argument('--file_read_pipeline_paths', default=None, type=str, metavar='N', help='Add custom FileReadPipeline paths. Separate multiple paths by commas')
parser.add_argument('--mxnet_reader_pipeline_paths', default=None, type=str, metavar='N', help='Add custom MXNetReaderPipeline paths. For a given path, a .rec and .idx extension will be appended. Separate multiple paths by commas')
parser.add_argument('--caffe_read_pipeline_paths', default=None, type=str, metavar='N', help='Add custom CaffeReadPipeline paths. Separate multiple paths by commas')
parser.add_argument('--caffe2_read_pipeline_paths', default=None, type=str, metavar='N', help='Add custom Caffe2ReadPipeline paths. Separate multiple paths by commas')
parser.add_argument('--tfrecord_pipeline_paths', default=None, type=str, metavar='N', help='Add custom TFRecordPipeline paths. For a given path, a second path with an .idx extension will be added for the required idx file(s). Separate multiple paths by commas')
parser.add_argument('--webdataset_pipeline_paths', default=None, type=str, metavar='N', help='Add custom WebdatasetPipeline paths. For a given path, a second path with an .idx extension will be added for the required idx file(s). Separate multiple paths by commas')
parser.add_argument('--system_id', default='localhost', type=str, metavar='N', help='Add a system id to denote a unique identifier for the performance output. Defaults to localhost')
    ...
    for (i, data_set) in enumerate(test_data[pipe_name]):
        ...
        for i in range(args.epochs):
            ...
            for j in range(iters):
                ...
                if j % LOG_INTERVAL == 0:
                    print(f'System {SYS_ID}, GPU {GPU_ID}, run {i}:  {pipe_name.__name__} {j + 1}/ {iters}, avg time: {data_time.avg} [s], worst time: {data_time.max_val} [s], speed: {N * BATCH_SIZE / data_time.avg} [img/s]')
                end = time.time()
