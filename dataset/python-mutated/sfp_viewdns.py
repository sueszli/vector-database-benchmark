...
class sfp_viewdns(SpiderFootPlugin):
    meta = {'name': 'ViewDNS.info', 'summary': 'Identify co-hosted websites and perform reverse Whois lookups using ViewDNS.info.', 'flags': ['apikey'], 'useCases': ['Footprint', 'Investigate', 'Passive'], 'categories': ['Search Engines'], 'dataSource': {'website': 'https://viewdns.info/', 'model': 'FREE_AUTH_LIMITED', 'references': ['https://viewdns.info/api/docs', 'https://viewdns.info/api/'], 'apiKeyInstructions': ['Visit https://viewdns.info/api', 'Select a plan', 'Register an account', 'Navigate to https://viewdns.info/api/dashboard/', "The API key is listed under 'API Key'"], 'favIcon': 'https://viewdns.info/apple-touch-icon.png', 'logo': 'https://viewdns.info/images/viewdns_logo.gif', 'description': 'The ViewDNS.info API allows webmasters to integrate the tools provided by ViewDNS.info into their own sites in a simple and effective manner.'}}
    opts = {'api_key': '', 'verify': True, 'maxcohost': 100}
    optdescs = {'api_key': 'ViewDNS.info API key.', 'verify': 'Verify co-hosts are valid by checking if they still resolve to the shared IP.', 'maxcohost': 'Stop reporting co-hosted sites after this many are found, as it would likely indicate web hosting.'}
    results = None
    errorState = False
    accum = list()
    cohostcount = 0
