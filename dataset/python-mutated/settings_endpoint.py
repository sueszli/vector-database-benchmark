...
class SettingsEndpoint(RESTEndpoint):
    ...
    def setup_routes(self):
        if False:
            while True:
                i = 10
        self.app.add_routes([web.get('', self.get_settings), web.post('', self.update_settings)])

    @docs(tags=['General'], summary='Return all the session settings that can be found in Tribler.', responses={200: {'schema': schema(GetTriblerSettingsResponse={})}}, description='This endpoint returns all the session settings that can be found in Tribler.\n\n It also returns the runtime-determined ports')
    async def get_settings(self, request):
        self._logger.info(f'Get settings. Request: {request}')
        return RESTResponse({'settings': self.config.dict(), 'ports': list(default_network_utils.ports_in_use)})

    @docs(tags=['General'], summary='Update Tribler settings.', responses={200: {'schema': schema(UpdateTriblerSettingsResponse={'modified': Boolean})}})
