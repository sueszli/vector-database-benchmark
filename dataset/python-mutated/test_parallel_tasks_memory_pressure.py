...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-tasks', help='number of tasks to process in total', default='20', type=int)
    parser.add_argument('--mem-pct-per-task', help="memory to allocate per task as a fraction of the node's available memory", default='0.45', type=float)
    args = parser.parse_args()
    cpu_per_task = 1
    bytes_per_task = get_additional_bytes_to_reach_memory_usage_pct(args.mem_pct_per_task)
    start = time.time()
    task_refs = [allocate_memory.options(num_cpus=cpu_per_task).remote(total_allocate_bytes=bytes_per_task, allocate_interval_s=1) for _ in range(args.num_tasks)]
