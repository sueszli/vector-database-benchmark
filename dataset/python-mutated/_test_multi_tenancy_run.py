...
trainer = DataParallelTrainer(train_loop_per_worker=train_func, train_loop_config={'fixed': FIXED_VAL}, scaling_config=train.ScalingConfig(num_workers=1, trainer_resources={'CPU': 0}, resources_per_worker={'CPU': 2}))
tuner = tune.Tuner(trainer, param_space={'train_loop_config': {'param': tune.grid_search(VALS), 'delete_marker': DELETE_TRIAL_MARKER, 'hang_marker': HANG_TRIAL_MARKER}}, tune_config=tune.TuneConfig(search_alg=BasicVariantGenerator(max_concurrent=1)))
results = tuner.fit()
