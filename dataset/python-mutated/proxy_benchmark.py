...
async def trial(num_replicas: int, max_concurrent_queries: int, data_size: int, num_clients: int, proxy: RequestProtocol) -> Dict[str, float]:
    data = [random() for _ in range(data_size)]
    app = build_app(num_replicas=num_replicas, max_concurrent_queries=max_concurrent_queries, data_size=data_size)
    serve.run(app)
    ...
    trial_key_base = f'proxy:{proxy}/num_client:{num_clients}/replica:{num_replicas}/concurrent_queries:{max_concurrent_queries}/data_size:{data_size}'
    (tps_mean, tps_sdt) = await get_query_tps(trial_key_base, client_do_queries)
    (latency_ms_mean, latency_ms_std) = await get_query_latencies(trial_key_base, client_time_queries)
    results = {'proxy': proxy.value, 'num_client': num_clients, 'replica': num_replicas, 'concurrent_queries': max_concurrent_queries, 'data_size': data_size, 'tps_mean': tps_mean, 'tps_sdt': tps_sdt, 'latency_ms_mean': latency_ms_mean, 'latency_ms_std': latency_ms_std}
    return results
...
async def main():
    ...
    for num_replicas in [1, 8]:
        for max_concurrent_queries in [1, 10000]:
            for data_size in [1, 100, 10000]:
                for num_clients in [1, 8]:
                    for proxy in [RequestProtocol.GRPC, RequestProtocol.HTTP]:
                        results.append(await trial(num_replicas=num_replicas, max_concurrent_queries=max_concurrent_queries, data_size=data_size, num_clients=num_clients, proxy=proxy))
    print(f'Total time: {time.time() - start_time}s')
    print('results', results)
    df = pd.DataFrame.from_dict(results)
    df = df.sort_values(by=['proxy', 'num_client', 'replica', 'concurrent_queries', 'data_size'])
...
if __name__ == '__main__':
    ray.init()
    grpc_port = 9000
    grpc_servicer_functions = ['ray.serve.generated.serve_pb2_grpc.add_RayServeBenchmarkServiceServicer_to_server']
    serve.start(grpc_options=gRPCOptions(port=grpc_port, grpc_servicer_functions=grpc_servicer_functions))
