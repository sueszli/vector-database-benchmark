...
class PurchaseOrder(models.Model):
    ...
    origin = fields.Char('Source Document', copy=False, help='Reference of the document that generated this purchase order request (e.g. a sale order or an internal procurement request)')
    partner_ref = fields.Char('Vendor Reference', copy=False, help="Reference of the sales order or bid sent by the vendor. It's used to do the matching when you receive the products as this reference is usually written on the delivery order sent by your vendor.")
    ...
    dest_address_id = fields.Many2one('res.partner', string='Drop Ship Address', states=READONLY_STATES, help='Put an address if you want to deliver directly from the vendor to the customer. Otherwise, keep empty to deliver to your own company.')
    ...
    state = fields.Selection([('draft', 'RFQ'), ('sent', 'RFQ Sent'), ('to approve', 'To Approve'), ('purchase', 'Purchase Order'), ('done', 'Locked'), ('cancel', 'Cancelled')], string='Status', readonly=True, index=True, copy=False, default='draft', track_visibility='onchange')
    ...
    invoice_status = fields.Selection([('no', 'Nothing to Bill'), ('to invoice', 'Waiting Bills'), ('invoiced', 'Bills Received')], string='Billing Status', compute='_get_invoiced', store=True, readonly=True, copy=False, default='no')
    ...
    picking_type_id = fields.Many2one('stock.picking.type', 'Deliver To', states=READONLY_STATES, required=True, default=_default_picking_type, help='This will determine picking type of incoming shipment')
...
class ProductTemplate(models.Model):
    ...
    def _purchase_count(self):
        ...
        return True
    property_account_creditor_price_difference = fields.Many2one('account.account', string='Price Difference Account', company_dependent=True, help='This account will be used to value price difference between purchase price and cost price.')
    purchase_count = fields.Integer(compute='_purchase_count', string='# Purchases')
    purchase_method = fields.Selection([('purchase', 'On ordered quantities'), ('receive', 'On received quantities')], string='Control Purchase Bills', help='On ordered quantities: control bills based on ordered quantities.\nOn received quantities: control bills based on received quantity.', default='receive')
...
class ProductCategory(models.Model):
    ...
    property_account_creditor_price_difference_categ = fields.Many2one('account.account', string='Price Difference Account', company_dependent=True, help='This account will be used to value price difference between purchase price and accounting cost.')
