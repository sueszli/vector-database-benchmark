...
import pyspiel
FLAGS = flags.FLAGS
flags.DEFINE_enum('sampling', 'outcome', ['external', 'outcome'], 'Sampling for the MCCFR solver')
...
def main(_):
    ...
    if FLAGS.sampling == 'external':
        cfr_solver = external_mccfr.ExternalSamplingSolver(game, external_mccfr.AverageType.SIMPLE)
    else:
