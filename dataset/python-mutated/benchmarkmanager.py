...
class AppBenchmarkManager:
    ...
    async def get(self, env_id: 'EnvId', env_prereq_dict: Dict[str, Any]) -> AppBenchmark:
        ...
        try:
            ...
            pass
        if self._computing:
            raise ComputationInProgress(comp_type=ComputationType.BENCHMARK, comp_id=self._computing)
        try:
    ...
    async def _run_benchmark(self, env_id: 'EnvId', env_prereq_dict: Dict[str, Any]) -> float:
        env = self._env_manager.environment(env_id)
        prereq_hash = hash_prereq_dict(env_prereq_dict)
        shared_dir = self._root_path / prereq_hash
        shared_dir.mkdir(parents=True, exist_ok=True)
        task_api_service = EnvironmentTaskApiService(env=env, payload_builder=self._env_manager.payload_builder(env_id), prereq=env.parse_prerequisites(env_prereq_dict), shared_dir=shared_dir)
        try:
