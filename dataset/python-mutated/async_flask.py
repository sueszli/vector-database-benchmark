...
class GraphQLView(BaseAsyncGraphQLView):
    ...
    async def get_context(self, request: FlaskRequest, response: FlaskResponse) -> Dict[str, object]:
        context = await super().get_context(request, response)
        return get_context(context)

    async def process_result(self, request: FlaskRequest, result: ExecutionResult) -> GraphQLHTTPResponse:
