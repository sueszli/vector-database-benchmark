...
class Cmus(base.ThreadPoolText):
    ...
    """
    defaults = [('format', '{play_icon}{artist} - {title}', 'Format of playback info.'), ('play_icon', '♫ ', 'Icon to display, if chosen.'), ('play_color', '00ff00', 'Text colour when playing.'), ('noplay_color', 'cecece', 'Text colour when not playing.'), ('update_interval', 0.5, 'Update Time in seconds.')]
    ...
    def __init__(self, **config):
        ...
        self.status = ''
        self.local = None
        self.add_callbacks({'Button1': self.play, 'Button4': partial(subprocess.Popen, ['cmus-remote', '-n']), 'Button5': partial(subprocess.Popen, ['cmus-remote', '-r'])})
    ...
    def get_info(self):
        ...
        if output.startswith('status'):
            ...
            info = {'status': '', 'file': '', 'artist': '', 'album': '', 'albumartist': '', 'composer': '', 'comment': '', 'date': '', 'discnumber': '', 'genre': '', 'title': '', 'tracknumber': '', 'stream': '', 'play_icon': self.play_icon}
            for line in output:
                for data in info:
                    match = data + ' '
                    if match in line:
