...
async def test_login(http_client, content_type_map, auth_creds, subtests, client_config):
    """
    Test valid logins
    """
    with subtests.test('Test in form encoded'):
        response = await http_client.fetch('/login', method='POST', body=urllib.parse.urlencode(auth_creds), headers={'Content-Type': content_type_map['form']})
        ...
        assert 'token' in response_obj
        assert response_obj['user'] == auth_creds['username']
        assert response_obj['eauth'] == auth_creds['eauth']
    with subtests.test('Test in JSON'):
        response = await http_client.fetch('/login', method='POST', body=salt.utils.json.dumps(auth_creds), headers={'Content-Type': content_type_map['json']})
        cookies = response.headers['Set-Cookie']
        assert response.code == 200
        response_obj = salt.utils.json.loads(response.body)['return'][0]
        token = response_obj['token']
    ...
    with subtests.test('Test in YAML'):
        response = await http_client.fetch('/login', method='POST', body=salt.utils.yaml.safe_dump(auth_creds), headers={'Content-Type': content_type_map['yaml']})
        cookies = response.headers['Set-Cookie']
        assert response.code == 200
        response_obj = salt.utils.json.loads(response.body)['return'][0]
        token = response_obj['token']
...
async def test_login_missing_password(http_client, auth_creds, content_type_map):
    ...
    with pytest.raises(HTTPError) as exc:
        await http_client.fetch('/login', method='POST', body=urllib.parse.urlencode(bad_creds), headers={'Content-Type': content_type_map['form']})
...
async def test_login_invalid_data_structure(http_client, content_type_map, auth_creds):
    ...
    """
    with pytest.raises(HTTPError) as exc:
        await http_client.fetch('/login', method='POST', body=salt.utils.json.dumps(auth_creds), headers={'Content-Type': content_type_map['form']})
    assert exc.value.code == 400
    with pytest.raises(HTTPError) as exc:
        await http_client.fetch('/login', method='POST', body=salt.utils.json.dumps(42), headers={'Content-Type': content_type_map['form']})
    assert exc.value.code == 400
    with pytest.raises(HTTPError) as exc:
        await http_client.fetch('/login', method='POST', body=salt.utils.json.dumps('mystring42'), headers={'Content-Type': content_type_map['form']})
    assert exc.value.code == 400