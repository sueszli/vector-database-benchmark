...
class texttospeechCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'list_voices': ('language_code',), 'synthesize_long_audio': ('input', 'audio_config', 'output_gcs_uri', 'voice', 'parent'), 'synthesize_speech': ('input', 'voice', 'audio_config', 'enable_time_pointing')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=texttospeechCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
