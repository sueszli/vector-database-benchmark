...
async def test_telegram_profit_handle(default_conf_usdt, update, ticker_usdt, ticker_sell_up, fee, limit_sell_order_usdt, mocker) -> None:
    mocker.patch('freqtrade.rpc.rpc.CryptoToFiatConverter._find_price', return_value=1.1)
...
async def test_telegram_forceexit_handle(default_conf, update, ticker, fee, ticker_sell_up, mocker) -> None:
    mocker.patch('freqtrade.rpc.rpc.CryptoToFiatConverter._find_price', return_value=15000.0)
    ...
    assert {'type': RPCMessageType.EXIT, 'trade_id': 1, 'exchange': 'Binance', 'pair': 'ETH/BTC', 'gain': 'profit', 'leverage': 1.0, 'limit': 1.173e-05, 'order_rate': 1.173e-05, 'amount': 91.07468123, 'order_type': 'limit', 'open_rate': 1.098e-05, 'current_rate': 1.173e-05, 'direction': 'Long', 'profit_amount': 6.314e-05, 'profit_ratio': 0.0629778, 'stake_currency': 'BTC', 'base_currency': 'ETH', 'fiat_currency': 'USD', 'buy_tag': ANY, 'enter_tag': ANY, 'sell_reason': ExitType.FORCE_EXIT.value, 'exit_reason': ExitType.FORCE_EXIT.value, 'open_date': ANY, 'close_date': ANY, 'close_rate': ANY, 'stake_amount': 0.0009999999999054, 'sub_trade': False, 'cumulative_profit': 0.0} == last_msg

async def test_telegram_force_exit_down_handle(default_conf, update, ticker, fee, ticker_sell_down, mocker) -> None:
...
async def test_telegram_entry_tag_performance_handle(default_conf_usdt, update, ticker, fee, mocker) -> None:
    mocker.patch.multiple(EXMS, fetch_ticker=ticker, get_fee=fee)
    (telegram, freqtradebot, msg_mock) = get_telegram_testobject(mocker, default_conf_usdt)
    patch_get_signal(freqtradebot)
    create_mock_trades_usdt(fee)
...
async def test_telegram_exit_reason_performance_handle(default_conf_usdt, update, ticker, fee, mocker) -> None:
    mocker.patch.multiple(EXMS, fetch_ticker=ticker, get_fee=fee)
    (telegram, freqtradebot, msg_mock) = get_telegram_testobject(mocker, default_conf_usdt)
    patch_get_signal(freqtradebot)
    create_mock_trades_usdt(fee)
...
async def test_telegram_mix_tag_performance_handle(default_conf_usdt, update, ticker, fee, mocker) -> None:
    mocker.patch.multiple(EXMS, fetch_ticker=ticker, get_fee=fee)
    (telegram, freqtradebot, msg_mock) = get_telegram_testobject(mocker, default_conf_usdt)
    patch_get_signal(freqtradebot)
    create_mock_trades_usdt(fee)
    ...
    assert '<code>TEST3 roi\t2.842 USDT (10.00%) (1)</code>' in msg_mock.call_args_list[0][0][0]
    context.args = ['XRP/USDT']
    await telegram._mix_tag_performance(update=update, context=context)
    assert msg_mock.call_count == 2
    msg_mock.reset_mock()
...
@pytest.mark.parametrize('is_short,regex_pattern', [(True, 'just now[ ]*XRP\\/BTC \\(#3\\)  -1.00% \\('), (False, 'just now[ ]*XRP\\/BTC \\(#3\\)  1.00% \\(')])
