...
class Submitter:
    ...
    async def site_self_check(self, site, semaphore, silent=False):
        ...
        for (username, status) in check_data:
            results_dict = await maigret(username=username, site_dict={site.name: site}, proxy=self.args.proxy, logger=self.logger, cookies=self.args.cookie_file, timeout=30, id_type=site.type, forced=True, no_progressbar=True)
            ...
            if site_status != status:
                if site_status == QueryStatus.UNKNOWN:
                    ...
                    self.logger.warning("Error while searching '%s' in %s: %s, %s, check type %s", username, site.name, result.context, msgs, etype)
                    if status == QueryStatus.CLAIMED:
                        changes['disabled'] = True
                elif status == QueryStatus.CLAIMED:
                    self.logger.warning(f'Not found `{username}` in {site.name}, must be claimed')
                    self.logger.info(results_dict[site.name])
                    changes['disabled'] = True
                else:
                    self.logger.warning(f'Found `{username}` in {site.name}, must be available')
                    self.logger.info(results_dict[site.name])
                    changes['disabled'] = True
        ...
        for engine in self.db.engines:
            ...
            if strs_to_check and resp_text:
                ...
                if all_strs_in_response:
                    ...
                    add_fields = self.generate_additional_fields_dialog(engine, url_exists)
                    for u in usernames_to_check:
                        site_data = {'urlMain': url_mainpage, 'name': url_mainpage.split('//')[1], 'engine': engine_name, 'usernameClaimed': u, 'usernameUnclaimed': 'noonewouldeverusethis7', **add_fields}
                        self.logger.info(site_data)
                        maigret_site = MaigretSite(url_mainpage.split('/')[-1], site_data)
                        maigret_site.update_from_engine(self.db.engines_dict[engine_name])
    ...
    async def check_features_manually(self, url_exists, url_mainpage, cookie_file, redirects=False):
        ...
        exists_resp = await self.session.get(url_exists, headers=headers, allow_redirects=redirects)
        ...
        non_exists_resp = await self.session.get(url_not_exists, headers=headers, allow_redirects=redirects)
        ...
        top_features_count = int(input(f'Specify count of features to extract [default {self.TOP_FEATURES}]: ') or self.TOP_FEATURES)
        match_fun = get_match_ratio(self.settings.presence_strings)
        presence_list = sorted(a_minus_b, key=match_fun, reverse=True)[:top_features_count]
        ...
        absence_list = sorted(b_minus_a, key=match_fun, reverse=True)[:top_features_count]
        print('Detected text features of non-existing account: ' + ', '.join(absence_list))
        features = input('If features was not detected correctly, write it manually: ')
        if features:
            absence_list = list(map(str.strip, features.split(',')))
        site_data = {'absenceStrs': absence_list, 'presenseStrs': presence_list, 'url': url_user, 'urlMain': url_mainpage, 'usernameClaimed': supposed_username, 'usernameUnclaimed': non_exist_username, 'checkType': 'message'}
    ...
    async def dialog(self, url_exists, cookie_file):
        ...
        matched_sites = list(filter(lambda x: domain_raw in x.url_main + x.url, self.db.sites))
        if matched_sites:
        ...
        if not sites:
            ...
            sites = [await self.check_features_manually(url_exists, url_mainpage, cookie_file, redirects)]
        ...
        if not found:
            ...
            return False
        elif input(f'Site {chosen_site.name} successfully checked. Do you want to save it in the Maigret DB? [Yn] ').lower().strip('y'):
            return False
        if self.args.verbose:
