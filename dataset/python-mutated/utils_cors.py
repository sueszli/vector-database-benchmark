...
CORS_ALLOWED_TRACING_HEADERS = ('traceparent', 'request-id', 'request-context', 'x-amzn-trace-id', 'x-cloud-trace-context', 'Sentry-Trace', 'Baggage')
...
def cors_response(request, response):
    ...
    response['Access-Control-Allow-Headers'] = 'X-Requested-With,Content-Type' + (',' + ','.join(allow_headers) if len(allow_headers) > 0 else '')
    return response