...
from ding.framework.middleware import CkptSaver, OffPolicyLearner, StepCollector, data_pusher, eps_greedy_handler, interaction_evaluator, online_logger
...
def main():
    ...
    ding_init(cfg)
    collector_env = BaseEnvManagerV2(env_fn=[lambda : CliffWalkingEnv(cfg.env) for _ in range(cfg.env.collector_env_num)], cfg=cfg.env.manager)
    evaluator_env = BaseEnvManagerV2(env_fn=[lambda : CliffWalkingEnv(cfg.env) for _ in range(cfg.env.collector_env_num)], cfg=cfg.env.manager)
