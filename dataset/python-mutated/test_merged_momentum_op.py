...
def run_momentum_op(params, grads, velocitys, master_params, learning_rate, place, multi_precision, mu=0.9, rescale_grad=0.01, use_merged=False):
    if False:
        while True:
            i = 10
    ...
    with paddle.static.program_guard(main, startup):
        ...
        attrs = {'mu': mu, 'multi_precision': multi_precision, 'rescale_grad': rescale_grad}
...
def run_momentum_op2(params, grads, velocitys, master_params, learning_rate, place, multi_precision, mu=0.9, rescale_grad=0.01, use_merged=False, use_nesterov=True):
    if False:
        print('Hello World!')
    assert len(params) == len(grads)
    assert len(params) == len(velocitys)
...
class TestMergedMomentum(unittest.TestCase):
    ...
    def prepare_data(self, shapes, multi_precision, seed, place):
        ...
        np.random.seed(seed)
        mp_dtype = np.float32
        dtype = np.float16 if multi_precision and isinstance(place, paddle.CUDAPlace) else np.float32
    ...
    def check_with_place(self, place, multi_precision):
        if False:
            for i in range(10):
                print('nop')
        (params, grads, velocitys, master_params, learning_rate) = self.prepare_data(self.shapes, multi_precision, self.seed, place)
        ...
        def run_op(use_merged):
            ...
            return run_momentum_op(params, grads, velocitys, master_params, learning_rate, place, multi_precision, rescale_grad=rescale_grad, use_merged=use_merged)
        outs1 = run_op(True)
        outs2 = run_op(False)
        ...
        def run_op(use_nesterov, use_merged):
            if False:
                while True:
                    i = 10
            rescale_grad = 1.0 if isinstance(place, paddle.CPUPlace) else 0.01
            return run_momentum_op2(params, grads, velocitys, master_params, learning_rate, place, multi_precision, rescale_grad=rescale_grad, use_merged=use_merged, use_nesterov=use_nesterov)
