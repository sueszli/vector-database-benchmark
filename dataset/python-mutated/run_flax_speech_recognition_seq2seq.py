...
import transformers
from transformers import AutoConfig, AutoFeatureExtractor, AutoProcessor, AutoTokenizer, FlaxAutoModelForSpeechSeq2Seq, HfArgumentParser, Seq2SeqTrainingArguments, is_tensorboard_available
...
class ModelArguments:
    ...
    num_beams: Optional[int] = field(default=None, metadata={'help': 'Number of beams to use for evaluation. This argument will be passed to `model.generate`, which is used during evaluation.'})
...
class DataTrainingArguments:
    ...
    max_duration_in_seconds: float = field(default=20.0, metadata={'help': 'Filter audio files that are longer than `max_duration_in_seconds` seconds'})
    ...
    max_label_length: float = field(default=128, metadata={'help': 'Truncate transcriptions that are longer `max_eval_length` tokens.'})
    pad_input_to_multiple_of: Optional[int] = field(default=None, metadata={'help': 'If set will pad the input sequence to a multiple of the provided value. This is important to avoid triggering recompilations on TPU. If unspecified, will default to padding the inputs to max length.'})
    pad_target_to_multiple_of: Optional[int] = field(default=None, metadata={'help': 'If set will pad the target sequence to a multiple of the provided value. This is important to avoid triggering recompilations on TPU. If unspecified, will default to padding the targets to max length.'})
    ...
    eval_split_name: str = field(default='validation', metadata={'help': "The name of the evaluation data set split to use (via the datasets library). Defaults to 'validation'"})
