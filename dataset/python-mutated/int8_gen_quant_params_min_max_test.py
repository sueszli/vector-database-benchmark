...
class TestInt8GenQuantParamsMinMaxOperator(hu.HypothesisTestCase):
    ...
    @given(n=st.integers(10, 10), m=st.integers(10, 10), preserve_sparsity=st.booleans(), rnd_seed=st.integers(1, 5), **hu.gcs_cpu_only)
    def test_int8_gen_quant_params_min_max_op(self, n, m, preserve_sparsity, rnd_seed, gc, dc):
        if False:
            while True:
                i = 10
        ...
        X_qparam = dnnlowp_pybind11.ChooseStaticQuantizationParams(np.min(X), np.max(X), hist, preserve_sparsity, 8, 'MIN_MAX_QUANTIZATION')
        ...
        dnnlowp_pybind11.CreateInt8QuantSchemeBlob('quant_scheme', 'MIN_MAX_QUANTIZATION', preserve_sparsity)
        ...
        gen_quant_params_op = core.CreateOperator('Int8GenQuantParamsMinMax', ['X_min', 'X_max', 'quant_scheme'], ['quant_param'], device_option=gc)
        ...
        (shapes, types) = workspace.InferShapesAndTypes([gen_quant_params_net], blob_dimensions={'X': [n, m], 'X_min': [1], 'X_max': [1], 'quant_scheme': [1]}, blob_types={'X': core.DataType.FLOAT, 'X_min': core.DataType.FLOAT, 'X_max': core.DataType.FLOAT, 'quant_scheme': core.DataType.STRING})
