...
domains = {'gpt-3.5-turbo': 'aitianhu.space', 'gpt-4': 'aitianhu.website'}
...
class AItianhuSpace(AsyncGeneratorProvider):
    ...
    @classmethod
    async def create_async_generator(cls, model: str, messages: Messages, proxy: str=None, domain: str=None, cookies: dict=None, timeout: int=10, **kwargs) -> AsyncResult:
        if not model:
            model = 'gpt-3.5-turbo'
        ...
        url = f'https://{domain}'
        async with StreamSession(proxies={'https': proxy}, cookies=cookies, timeout=timeout, impersonate='chrome110', verify=False) as session:
            ...
            headers = {'Authority': url, 'Accept': 'application/json, text/plain, */*', 'Origin': url, 'Referer': f'{url}/'}
