...
from modelscope.hub.constants import MODELSCOPE_SDK_DEBUG, Licenses, ModelVisibility
from modelscope.hub.errors import NotExistError
from modelscope.hub.file_download import model_file_download
from modelscope.hub.repository import Repository
from modelscope.hub.snapshot_download import snapshot_download
from modelscope.utils.logger import get_logger
from modelscope.utils.test_utils import TEST_ACCESS_TOKEN1, TEST_MODEL_CHINESE_NAME, TEST_MODEL_ORG
logger = get_logger()
...
class HubRevisionTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.modified_environ = {k: v for (k, v) in os.environ.items() if k not in names_to_remove}
    ...
    def test_dev_mode_default_master(self):
        ...
        with mock.patch.dict(os.environ, self.modified_environ, clear=True):
            ...
            with tempfile.TemporaryDirectory() as temp_cache_dir:
                file_path = model_file_download(self.model_id, download_model_file_name, cache_dir=temp_cache_dir)
    ...
    def test_dev_mode_specify_branch(self):
        ...
        with mock.patch.dict(os.environ, self.modified_environ, clear=True):
            ...
            with tempfile.TemporaryDirectory() as temp_cache_dir:
                snapshot_path = snapshot_download(self.model_id, revision=branch_name, cache_dir=temp_cache_dir)
                assert os.path.exists(os.path.join(snapshot_path, download_model_file_name))
            with tempfile.TemporaryDirectory() as temp_cache_dir:
                file_path = model_file_download(self.model_id, download_model_file_name, revision=branch_name, cache_dir=temp_cache_dir)
                assert os.path.exists(file_path)
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as temp_cache_dir:
                    ...
                    with self.assertRaises(NotExistError):
                        model_file_download(self.model_id, download_model_file_name2, cache_dir=temp_cache_dir)
                version.__release_datetime__ = t2
                logger.info('Setting __release_datetime__ to: %s' % t2)
                with tempfile.TemporaryDirectory() as temp_cache_dir:
                    file_path = model_file_download(self.model_id, download_model_file_name, cache_dir=temp_cache_dir)
                    assert os.path.exists(file_path)
                    file_path = model_file_download(self.model_id, download_model_file_name2, cache_dir=temp_cache_dir)
                    assert os.path.exists(file_path)
            finally:
                version.__release_datetime__ = release_datetime_backup
