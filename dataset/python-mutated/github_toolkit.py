...
class GitHubToolkit(BaseToolkit, ABC):
    ...
    def get_tools(self) -> List[BaseTool]:
        if False:
            for i in range(10):
                print('nop')
        return [GithubAddFileTool(), GithubDeleteFileTool(), GithubRepoSearchTool(), GithubReviewPullRequest(), GithubFetchPullRequest()]

    def get_env_keys(self) -> List[ToolConfiguration]:
        if False:
            while True:
                i = 10
        return [ToolConfiguration(key='GITHUB_ACCESS_TOKEN', key_type=ToolConfigKeyType.STRING, is_required=True, is_secret=True), ToolConfiguration(key='GITHUB_USERNAME', key_type=ToolConfigKeyType.STRING, is_required=True, is_secret=False)]