...
if gpuarray.pygpu_activated:
    raise ImportError("cuda_convnet is not supported under Theano's new GPU backend. Please either use the ordinary convolutional and pooling layers (they are usually faster), or use Theano's old GPU backend (available up to Theano 0.9).")
from theano.sandbox.cuda.basic_ops import gpu_contiguous
from pylearn2.sandbox.cuda_convnet.filter_acts import FilterActs
__all__ = ['Conv2DCCLayer', 'MaxPool2DCCLayer', 'ShuffleBC01ToC01BLayer', 'bc01_to_c01b', 'ShuffleC01BToBC01Layer', 'c01b_to_bc01', 'NINLayer_c01b']
if not theano.sandbox.cuda.cuda_enabled:
    raise ImportError('requires GPU support -- see http://lasagne.readthedocs.org/en/latest/user/installation.html#gpu-support')
if theano.config.floatX == 'float64':
    warnings.warn('You are using a GPU layer with Theano configured for double precision (floatX=float64). Depending on your Theano version and GPU, this may be slow or unsupported. We recommend to configure Theano for single precision (floatX=float32); see http://lasagne.readthedocs.org/en/latest/user/installation.html#gpu-support.')
...
bc01_to_c01b = ShuffleBC01ToC01BLayer
...
c01b_to_bc01 = ShuffleC01BToBC01Layer
