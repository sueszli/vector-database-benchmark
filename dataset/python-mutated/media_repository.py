...
from synapse.api.errors import Codes, FederationDeniedError, HttpResponseException, NotFoundError, RequestSendFailed, SynapseError, cs_error
...
MEDIA_RETENTION_CHECK_PERIOD_MS = 60 * 60 * 1000
...
class MediaRepository:
    ...
    async def _update_recently_accessed(self) -> None:
        ...
        await self.store.update_cached_last_access_time(local_media, remote_media, self.clock.time_msec())
    ...
    async def create_media_id(self, auth_user: UserID) -> Tuple[str, int]:
        ...
        await self.store.store_local_media_id(media_id=media_id, time_now_ms=now, user_id=auth_user)
    ...
    async def reached_pending_media_limit(self, auth_user: UserID) -> Tuple[bool, int]:
        ...
        """
        (pending, first_expiration_ts) = await self.store.count_pending_media(user_id=auth_user)
    ...
    async def verify_can_upload(self, media_id: str, auth_user: UserID) -> None:
        ...
        * the media ID has not expired
        ...
        if media.user_id != auth_user.to_string():
            raise SynapseError(403, 'Only the creator of the media ID can upload to it', errcode=Codes.FORBIDDEN)
        if media.media_length is not None:
            raise SynapseError(409, 'Media ID already has content', errcode=Codes.CANNOT_OVERWRITE_MEDIA)
    ...
    @trace
    async def update_content(self, media_id: str, media_type: str, upload_name: Optional[str], content: IO, content_length: int, auth_user: UserID) -> None:
        ...
        await self.store.update_local_media(media_id=media_id, media_type=media_type, upload_name=upload_name, media_length=content_length, user_id=auth_user)
        try:
    ...
    @trace
    async def create_content(self, media_type: str, upload_name: Optional[str], content: IO, content_length: int, auth_user: UserID) -> MXCUri:
        ...
        fname = await self.media_storage.store_file(content, file_info)
        logger.info('Stored local media in file %r', fname)
        await self.store.store_local_media(media_id=media_id, media_type=media_type, time_now_ms=self.clock.time_msec(), upload_name=upload_name, media_length=content_length, user_id=auth_user)
        try:
            await self._generate_thumbnails(None, media_id, media_id, media_type)
        except Exception as e:
            logger.info('Failed to generate thumbnails: %s', e)
    ...
    async def get_local_media_info(self, request: SynapseRequest, media_id: str, max_timeout_ms: int) -> Optional[LocalMedia]:
    ...
    async def get_local_media(self, request: SynapseRequest, media_id: str, name: Optional[str], max_timeout_ms: int) -> None:
        ...
        await respond_with_responder(request, responder, media_type, media_length, upload_name)

    async def get_remote_media(self, request: SynapseRequest, server_name: str, media_id: str, name: Optional[str], max_timeout_ms: int) -> None:
        ...
        async with self.remote_media_linearizer.queue(key):
            (responder, media_info) = await self._get_remote_media_impl(server_name, media_id, max_timeout_ms)
        if responder and media_info:
            upload_name = name if name else media_info.upload_name
            await respond_with_responder(request, responder, media_info.media_type, media_info.media_length, upload_name)
        else:
            respond_404(request)

    async def get_remote_media_info(self, server_name: str, media_id: str, max_timeout_ms: int) -> RemoteMedia:
    ...
    async def _get_remote_media_impl(self, server_name: str, media_id: str, max_timeout_ms: int) -> Tuple[Optional[Responder], RemoteMedia]:
        ...
        try:
            media_info = await self._download_remote_file(server_name, media_id, max_timeout_ms)
        except SynapseError:
            raise
        except Exception as e:
            media_info = await self.store.get_cached_remote_media(server_name, media_id)
        ...
        if not media_info.media_type:
            media_info = attr.evolve(media_info, media_type='application/octet-stream')
        file_info = FileInfo(server_name, file_id)
        await self._generate_thumbnails(server_name, media_id, file_id, media_info.media_type)
        responder = await self.media_storage.fetch_media(file_info)
        return (responder, media_info)

    async def _download_remote_file(self, server_name: str, media_id: str, max_timeout_ms: int) -> RemoteMedia:
        ...
        with self.media_storage.store_into_file(file_info) as (f, fname, finish):
            request_path = '/'.join(('/_matrix/media/r0/download', server_name, media_id))
            try:
                (length, headers) = await self.client.get_file(server_name, request_path, output_stream=f, max_size=self.max_upload_size, args={'allow_remote': 'false', 'timeout_ms': str(max_timeout_ms)})
            except RequestSendFailed as e:
                logger.warning('Request failed fetching remote media %s/%s: %r', server_name, media_id, e)
                raise SynapseError(502, 'Failed to fetch remote media')
            except HttpResponseException as e:
                logger.warning('HTTP error fetching remote media %s/%s: %s', server_name, media_id, e.response)
                if e.code == twisted.web.http.NOT_FOUND:
                    raise e.to_synapse_error()
                raise SynapseError(502, 'Failed to fetch remote media')
            except SynapseError:
                logger.warning('Failed to fetch remote media %s/%s', server_name, media_id)
                raise
            except NotRetryingDestination:
                logger.warning('Not retrying destination %r', server_name)
                raise SynapseError(502, 'Failed to fetch remote media')
            except Exception:
                logger.exception('Failed to fetch remote media %s/%s', server_name, media_id)
                raise SynapseError(502, 'Failed to fetch remote media')
            await finish()
            ...
            await self.store.store_cached_remote_media(origin=server_name, media_id=media_id, media_type=media_type, time_now_ms=time_now_ms, upload_name=upload_name, media_length=length, filesystem_id=file_id)
        logger.info('Stored remote media in file %r', fname)
        return RemoteMedia(media_origin=server_name, media_id=media_id, media_type=media_type, media_length=length, upload_name=upload_name, created_ts=time_now_ms, filesystem_id=file_id, last_access_ts=time_now_ms, quarantined_by=None)
    ...
    async def generate_local_exact_thumbnail(self, media_id: str, t_width: int, t_height: int, t_method: str, t_type: str, url_cache: bool) -> Optional[str]:
        input_path = await self.media_storage.ensure_media_is_in_local_cache(FileInfo(None, media_id, url_cache=url_cache))
        try:
            thumbnailer = Thumbnailer(input_path)
        except ThumbnailError as e:
            logger.warning('Unable to generate a thumbnail for local media %s using a method of %s and type of %s: %s', media_id, t_method, t_type, e)
            return None
        with thumbnailer:
            t_byte_source = await defer_to_thread(self.hs.get_reactor(), self._generate_thumbnail, thumbnailer, t_width, t_height, t_method, t_type)
        if t_byte_source:
            try:
                file_info = FileInfo(server_name=None, file_id=media_id, url_cache=url_cache, thumbnail=ThumbnailInfo(width=t_width, height=t_height, method=t_method, type=t_type, length=t_byte_source.tell()))
                output_path = await self.media_storage.store_file(t_byte_source, file_info)
            finally:
                t_byte_source.close()
            logger.info('Stored thumbnail in file %r', output_path)
            t_len = os.path.getsize(output_path)
            await self.store.store_local_thumbnail(media_id, t_width, t_height, t_type, t_method, t_len)
            return output_path
        return None

    async def generate_remote_exact_thumbnail(self, server_name: str, file_id: str, media_id: str, t_width: int, t_height: int, t_method: str, t_type: str) -> Optional[str]:
        input_path = await self.media_storage.ensure_media_is_in_local_cache(FileInfo(server_name, file_id))
        try:
            thumbnailer = Thumbnailer(input_path)
        except ThumbnailError as e:
            logger.warning('Unable to generate a thumbnail for remote media %s from %s using a method of %s and type of %s: %s', media_id, server_name, t_method, t_type, e)
            return None
        with thumbnailer:
            t_byte_source = await defer_to_thread(self.hs.get_reactor(), self._generate_thumbnail, thumbnailer, t_width, t_height, t_method, t_type)
        if t_byte_source:
            try:
                file_info = FileInfo(server_name=server_name, file_id=file_id, thumbnail=ThumbnailInfo(width=t_width, height=t_height, method=t_method, type=t_type, length=t_byte_source.tell()))
                output_path = await self.media_storage.store_file(t_byte_source, file_info)
            finally:
                t_byte_source.close()
            logger.info('Stored thumbnail in file %r', output_path)
            t_len = os.path.getsize(output_path)
            await self.store.store_remote_media_thumbnail(server_name, media_id, file_id, t_width, t_height, t_type, t_method, t_len)
            return output_path
        return None

    @trace
    async def _generate_thumbnails(self, server_name: Optional[str], media_id: str, file_id: str, media_type: str, url_cache: bool=False) -> Optional[dict]:
        ...
        if not requirements:
            return None
        input_path = await self.media_storage.ensure_media_is_in_local_cache(FileInfo(server_name, file_id, url_cache=url_cache))
        try:
            thumbnailer = Thumbnailer(input_path)
        except ThumbnailError as e:
            logger.warning('Unable to generate thumbnails for remote media %s from %s of type %s: %s', media_id, server_name, media_type, e)
            return None
        with thumbnailer:
            m_width = thumbnailer.width
            m_height = thumbnailer.height
            if m_width * m_height >= self.max_image_pixels:
                ...
                return None
            if thumbnailer.transpose_method is not None:
                (m_width, m_height) = await defer_to_thread(self.hs.get_reactor(), thumbnailer.transpose)
            thumbnails: Dict[Tuple[int, int, str], str] = {}
            for requirement in requirements:
                if requirement.method == 'crop':
                    thumbnails.setdefault((requirement.width, requirement.height, requirement.media_type), requirement.method)
                elif requirement.method == 'scale':
                    (t_width, t_height) = thumbnailer.aspect(requirement.width, requirement.height)
                    t_width = min(m_width, t_width)
                    t_height = min(m_height, t_height)
                    thumbnails[t_width, t_height, requirement.media_type] = requirement.method
            for ((t_width, t_height, t_type), t_method) in thumbnails.items():
                if t_method == 'crop':
                    t_byte_source = await defer_to_thread(self.hs.get_reactor(), thumbnailer.crop, t_width, t_height, t_type)
                elif t_method == 'scale':
                    t_byte_source = await defer_to_thread(self.hs.get_reactor(), thumbnailer.scale, t_width, t_height, t_type)
                else:
                    logger.error('Unrecognized method: %r', t_method)
                    continue
                if not t_byte_source:
                    continue
                file_info = FileInfo(server_name=server_name, file_id=file_id, url_cache=url_cache, thumbnail=ThumbnailInfo(width=t_width, height=t_height, method=t_method, type=t_type, length=t_byte_source.tell()))
                with self.media_storage.store_into_file(file_info) as (f, fname, finish):
                    try:
                    ...
                    if server_name:
                        try:
                            ...
                            thumbnail_exists = await self.store.get_remote_media_thumbnail(server_name, media_id, t_width, t_height, t_type)
    ...
    async def _apply_media_retention_rules(self) -> None:
        ...
        if self._media_retention_remote_media_lifetime_ms is not None:
            remote_media_threshold_timestamp_ms = self.clock.time_msec() - self._media_retention_remote_media_lifetime_ms
            logger.info(f'Purging remote media last accessed before {remote_media_threshold_timestamp_ms}')
            await self.delete_old_remote_media(before_ts=remote_media_threshold_timestamp_ms)
        if self._media_retention_local_media_lifetime_ms is not None:
            local_media_threshold_timestamp_ms = self.clock.time_msec() - self._media_retention_local_media_lifetime_ms
            logger.info(f'Purging local media last accessed before {local_media_threshold_timestamp_ms}')
            await self.delete_old_local_media(before_ts=local_media_threshold_timestamp_ms, keep_profiles=True, delete_quarantined_media=False, delete_protected_media=False)

    async def delete_old_remote_media(self, before_ts: int) -> Dict[str, int]:
        old_media = await self.store.get_remote_media_ids(before_ts, include_quarantined_media=False)
        deleted = 0
    ...
    async def delete_local_media_ids(self, media_ids: List[str]) -> Tuple[List[str], int]:
        """
    ...
    async def delete_old_local_media(self, before_ts: int, size_gt: int=0, keep_profiles: bool=True, delete_quarantined_media: bool=False, delete_protected_media: bool=False) -> Tuple[List[str], int]:
        """
        ...
        Returns:
            A tuple of (list of deleted media IDs, total deleted media IDs).
        """
        old_media = await self.store.get_local_media_ids(before_ts, size_gt, keep_profiles, include_quarantined_media=delete_quarantined_media, include_protected_media=delete_protected_media)
        return await self._remove_local_media_from_disk(old_media)

    async def _remove_local_media_from_disk(self, media_ids: List[str]) -> Tuple[List[str], int]:
        """
