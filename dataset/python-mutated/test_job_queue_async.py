...
from azure.communication.jobrouter.models import RoundRobinMode, DistributionPolicy, RouterQueue
...
class TestJobQueueAsync(AsyncRouterRecordedTestCase):
    ...
    async def test_create_queue(self):
        ...
        router_client: JobRouterAdministrationClient = self.create_admin_client()
        async with router_client:
            job_queue: RouterQueue = RouterQueue(name=dp_identifier, labels=queue_labels, distribution_policy_id=self.get_distribution_policy_id())
    ...
    async def test_list_queues(self):
        ...
        async with router_client:
            for identifier in dp_identifiers:
                job_queue: RouterQueue = RouterQueue(name=identifier, labels=queue_labels, distribution_policy_id=self.get_distribution_policy_id())
