...
_IS_2BYTE_UNICODE = sys.maxunicode == 65535
...
(_parse_escape_sequences_raw, _parse_escape_sequences) = [re.compile((b'(\\\\(?:' + (b'\\\\?' if is_raw else b'[\\\\abfnrtv"\\\'{]|[0-7]{2,3}|N\\{[^}]*\\}|x[0-9a-fA-F]{2}|u[0-9a-fA-F]{4}|U[0-9a-fA-F]{8}|[NxuU]|') + b')?|\\{\\{?|\\}\\}?|[^\\\\{}]+)').decode('us-ascii')).match for is_raw in (True, False)]
...
special_basic_c_types = cython.declare(dict, {'Py_UNICODE': (0, 0), 'Py_UCS4': (0, 0), 'Py_hash_t': (2, 0), 'Py_ssize_t': (2, 0), 'ssize_t': (2, 0), 'size_t': (0, 0), 'ptrdiff_t': (2, 0), 'Py_tss_t': (1, 0)})
sign_and_longness_words = cython.declare(frozenset, frozenset(('short', 'long', 'signed', 'unsigned')))
base_type_start_words = cython.declare(frozenset, basic_c_type_names | sign_and_longness_words | frozenset(special_basic_c_types))
...
supported_overloaded_operators = cython.declare(frozenset, frozenset(('+', '-', '*', '/', '%', '++', '--', '~', '|', '&', '^', '<<', '>>', ',', '==', '!=', '>=', '>', '<=', '<', '[]', '()', '!', '=', 'bool')))
...
def p_code(s, level=None, ctx=Ctx):
    ...
    return body
_match_compiler_directive_comment = cython.declare(object, re.compile('^#\\s*cython\\s*:\\s*((\\w|[.])+\\s*=.*)$').match)
