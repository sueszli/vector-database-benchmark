...
class EXPORTGIS_OT_shapefile(Operator, ExportHelper):
    ...
    exportType: EnumProperty(name='Feature type', description='Select feature type', items=[('POINTZ', 'Point', ''), ('POLYLINEZ', 'Line', ''), ('POLYGONZ', 'Polygon', '')])
    objectsSource: EnumProperty(name='Objects', description='Objects to export', items=[('COLLEC', 'Collection', 'Export a collection of objects'), ('SELECTED', 'Selected objects', 'Export the current selection')], default='SELECTED')

    def listCollections(self, context):
        if False:
            return 10
        return [(c.name, c.name, 'Collection') for c in bpy.data.collections]
    selectedColl: EnumProperty(name='Collection', description='Select the collection to export', items=listCollections)
    mode: EnumProperty(name='Mode', description='Select the export strategy', items=[('OBJ2FEAT', 'Objects to features', 'Create one multipart feature per object'), ('MESH2FEAT', 'Mesh to features', 'Decompose mesh primitives to separate features')], default='OBJ2FEAT')
