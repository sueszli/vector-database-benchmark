...
REPO_NAME = 'prefect'
DEFAULT_TAG = 'preview'
TOKEN_REGEX = re.compile('Token:\\s(.*)')
ENTRY_REGEX = re.compile('^\\* (.*) by @(.*) in (.*)$', re.MULTILINE)
...
if __name__ == '__main__':
    generate_release_notes(REPO_ORG, REPO_NAME, tag_name=sys.argv[1] if len(sys.argv) > 1 else DEFAULT_TAG, target_commit=sys.argv[2] if len(sys.argv) > 2 else 'main', previous_tag=sys.argv[3] if len(sys.argv) > 3 else None, github_token=get_github_token())