...
from homeassistant.components.light import ATTR_COLOR_MODE, ATTR_COLOR_TEMP, ATTR_SUPPORTED_COLOR_MODES, DOMAIN as LIGHT_DOMAIN, ColorMode
...
async def test_async_update_locked_callback_and_update(hass: HomeAssistant, pywemo_bridge_light, wemo_entity, pywemo_device) -> None:
    """Test that a callback and a state update request can't both happen at the same time."""
    await entity_test_helpers.test_async_update_locked_callback_and_update(hass, pywemo_device, wemo_entity)

async def test_async_update_locked_multiple_updates(hass: HomeAssistant, pywemo_bridge_light, wemo_entity, pywemo_device) -> None:
    """Test that two state updates do not proceed at the same time."""
    await entity_test_helpers.test_async_update_locked_multiple_updates(hass, pywemo_device, wemo_entity)

async def test_async_update_locked_multiple_callbacks(hass: HomeAssistant, pywemo_bridge_light, wemo_entity, pywemo_device) -> None:
    """Test that two device callback state updates do not proceed at the same time."""
    await entity_test_helpers.test_async_update_locked_multiple_callbacks(hass, pywemo_device, wemo_entity)

async def test_available_after_update(hass: HomeAssistant, pywemo_registry, pywemo_device, pywemo_bridge_light, wemo_entity) -> None:
    ...
    await entity_test_helpers.test_avaliable_after_update(hass, pywemo_registry, pywemo_device, wemo_entity, LIGHT_DOMAIN)

async def test_turn_off_state(hass: HomeAssistant, pywemo_bridge_light, wemo_entity) -> None:
...
async def test_light_update_entity(hass: HomeAssistant, pywemo_registry, pywemo_bridge_light, wemo_entity) -> None:
    """Verify that the light performs state updates."""
    await async_setup_component(hass, HA_DOMAIN, {})
    pywemo_bridge_light.state['onoff'] = 1
    pywemo_bridge_light.state['temperature_mireds'] = 432
    await hass.services.async_call(HA_DOMAIN, SERVICE_UPDATE_ENTITY, {ATTR_ENTITY_ID: [wemo_entity.entity_id]}, blocking=True)
