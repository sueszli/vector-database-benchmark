...
class UloziskoSk(SimpleDownloader):
    ...
    __version__ = '0.31'
    __status__ = 'testing'
    __pattern__ = 'http://(?:www\\.)?ulozisko\\.sk/.+'
    ...
    NAME_PATTERN = '<div class="down1">(?P<N>.+?)</div>'
    SIZE_PATTERN = 'Veľkosť súboru: <strong>(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)</strong><br />'
    OFFLINE_PATTERN = '<span class = "red">Zadaný súbor neexistuje z jedného z nasledujúcich dôvodov:</span>'
    LINK_FREE_PATTERN = '<form name = "formular" action = "(.+?)" method = "post">'
    ID_PATTERN = '<input type = "hidden" name = "id" value = "(.+?)" />'
    CAPTCHA_PATTERN = '<img src="(/obrazky/obrazky\\.php\\?fid=.+?)" alt="" />'
    IMG_PATTERN = '<strong>PRE ZVÄČŠENIE KLIKNITE NA OBRÁZOK</strong><br /><a href = "(.+?)">'
