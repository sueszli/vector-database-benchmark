...
def provider(hass, store):
    if False:
        while True:
            i = 10
    'Mock provider.'
    return legacy_api_password.LegacyApiPasswordAuthProvider(hass, store, {'type': 'legacy_api_password', 'api_password': 'test-password'})
...
async def test_login_flow_works(hass: HomeAssistant, manager) -> None:
    ...
    result = await manager.login_flow.async_configure(flow_id=result['flow_id'], user_input={'password': 'not-hello'})
    assert result['type'] == data_entry_flow.FlowResultType.FORM
    assert result['errors']['base'] == 'invalid_auth'
    result = await manager.login_flow.async_configure(flow_id=result['flow_id'], user_input={'password': 'test-password'})
