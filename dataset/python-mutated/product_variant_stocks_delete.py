...
from ...warehouse.dataloaders import StocksWithAvailableQuantityByProductVariantIdCountryCodeAndChannelLoader
from ...warehouse.types import Warehouse
from ..types import ProductVariant

class ProductVariantStocksDelete(BaseMutation):
    product_variant = graphene.Field(ProductVariant, description='Updated product variant.')

    class Arguments:
        variant_id = graphene.ID(required=False, description='ID of product variant for which stocks will be deleted.')
        sku = graphene.String(required=False, description='SKU of product variant for which stocks will be deleted.')
        warehouse_ids = NonNullList(graphene.ID, description='Input list of warehouse IDs.')
