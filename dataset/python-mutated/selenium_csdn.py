...
class Api(object):
    ...
    async def crawl(self):
        self.browser = await launch({'headless': False, 'userDataDir': cache_dir, 'defaultViewport': {'width': 1440, 'height': 1000}, 'args': ['--no-sandbox']})
        self.page = await self.browser.newPage()
        await self.page.goto(self.url)
        codes = ('() =>{ Object.defineProperties(navigator,{ webdriver:{ get: () => false } }) }', '() =>{ window.navigator.chrome = { runtime: {},  }; }', "() =>{ Object.defineProperty(navigator, 'languages', { get: () => ['en-US', 'en'] }); }", "() =>{ Object.defineProperty(navigator, 'plugins', { get: () => [1, 2, 3, 4, 5,6], }); }")
        for code in codes:
