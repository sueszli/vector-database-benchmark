...
from pycaret.internal.memory import fast_hash
from pycaret.regression import RegressionExperiment
pytestmark = [pytest.mark.benchmark]
data_df = get_data(verbose=False)
supervised_datasets_df = data_df.copy()
supervised_datasets_df['Items'] = supervised_datasets_df['# Instances'] * supervised_datasets_df['# Attributes']
supervised_datasets_df = supervised_datasets_df[supervised_datasets_df['Items'] >= 30000].sort_values('Items', ascending=True)
supervised_datasets_df = supervised_datasets_df[supervised_datasets_df['Default Task'].str.startswith('Classification') | supervised_datasets_df['Default Task'].str.startswith('Regression')]
supervised_datasets = supervised_datasets_df[['Dataset', 'Default Task', 'Target Variable 1']].iterrows()
