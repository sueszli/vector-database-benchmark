...
if __name__ == '__main__':
    ...
    config = get_trainable_cls(args.run).get_default_config().environment('CartPole-v1' if args.run not in ['DDPG', 'TD3'] else 'Pendulum-v1').framework(args.framework).debugging(logger_config={'type': MyPrintLogger, 'prefix': 'ABC'}).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    stop = {'training_iteration': args.stop_iters, 'timesteps_total': args.stop_timesteps, 'episode_reward_mean': args.stop_reward}
    tuner = tune.Tuner(args.run, param_space=config.to_dict(), run_config=air.RunConfig(stop=stop, verbose=2, callbacks=[LegacyLoggerCallback(MyPrintLogger)]))
    results = tuner.fit()
    if args.as_test:
