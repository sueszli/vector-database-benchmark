...
class TestAsyncSteps:
    ...
    async def test_method_retrieve(self, client: AsyncOpenAI) -> None:
        step = await client.beta.threads.runs.steps.retrieve('string', thread_id='string', run_id='string')
        assert_matches_type(RunStep, step, path=['response'])

    @parametrize
    async def test_raw_response_retrieve(self, client: AsyncOpenAI) -> None:
        response = await client.beta.threads.runs.steps.with_raw_response.retrieve('string', thread_id='string', run_id='string')
        assert response.http_request.headers.get('X-Stainless-Lang') == 'python'
        step = response.parse()
        assert_matches_type(RunStep, step, path=['response'])
    ...
    async def test_method_list(self, client: AsyncOpenAI) -> None:
        step = await client.beta.threads.runs.steps.list('string', thread_id='string')
    ...
    async def test_method_list_with_all_params(self, client: AsyncOpenAI) -> None:
        step = await client.beta.threads.runs.steps.list('string', thread_id='string', after='string', before='string', limit=0, order='asc')
        assert_matches_type(AsyncCursorPage[RunStep], step, path=['response'])

    @parametrize
    async def test_raw_response_list(self, client: AsyncOpenAI) -> None:
        response = await client.beta.threads.runs.steps.with_raw_response.list('string', thread_id='string')
        assert response.http_request.headers.get('X-Stainless-Lang') == 'python'
        step = response.parse()
        assert_matches_type(AsyncCursorPage[RunStep], step, path=['response'])