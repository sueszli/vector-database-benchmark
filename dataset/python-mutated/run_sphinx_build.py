...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run sphinx-build against target folder. Zips and moves resulting files to a root location as well.')
    parser.add_argument('-w', '--workingdir', dest='working_directory', help='The unzipped package directory on disk. Usually {distdir}/unzipped/', required=True)
    parser.add_argument('-o', '--outputdir', dest='output_directory', help='The output location for the generated site. Usually {distdir}/site', required=True)
    parser.add_argument('-r', '--root', dest='package_root', help='', required=True)
    parser.add_argument('--inci', dest='in_ci', action='store_true', default=False)
