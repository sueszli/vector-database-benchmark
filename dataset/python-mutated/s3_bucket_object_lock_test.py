...
class Test_s3_bucket_object_lock:
    ...
    def test_no_buckets(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_object_lock.s3_bucket_object_lock.s3_client', new=S3(audit_info)):
                from prowler.providers.aws.services.s3.s3_bucket_object_lock.s3_bucket_object_lock import s3_bucket_object_lock
    ...
    def test_bucket_no_object_lock(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_object_lock.s3_bucket_object_lock.s3_client', new=S3(audit_info)):
                ...
                assert search('Object Lock disabled', result[0].status_extended)
    ...
    def test_bucket_object_lock_enabled(self):
        ...
        s3_client_us_east_1 = client('s3', region_name='us-east-1')
        bucket_name_us = 'bucket_test_us'
        s3_client_us_east_1.create_bucket(Bucket=bucket_name_us, ObjectOwnership='BucketOwnerEnforced', ObjectLockEnabledForBucket=True)
        from prowler.providers.aws.services.s3.s3_service import S3
        audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=audit_info):
            with mock.patch('prowler.providers.aws.services.s3.s3_bucket_object_lock.s3_bucket_object_lock.s3_client', new=S3(audit_info)):
                ...
                assert search('Object Lock enabled', result[0].status_extended)
                assert result[0].resource_id == bucket_name_us
                assert result[0].resource_arn == f'arn:{audit_info.audited_partition}:s3:::{bucket_name_us}'
                assert result[0].region == 'us-east-1'
                assert result[0].resource_tags == []