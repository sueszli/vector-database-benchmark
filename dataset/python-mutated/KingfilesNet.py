...
class KingfilesNet(SimpleDownloader):
    ...
    __version__ = '0.13'
    __status__ = 'testing'
    __pattern__ = 'http://(?:www\\.)?kingfiles\\.net/(?P<ID>\\w{12})'
    ...
    __license__ = 'GPLv3'
    __authors__ = [('zapp-brannigan', 'fuerst.reinje@web.de'), ('Walter Purcaro', 'vuolter@gmail.com')]
    NAME_PATTERN = 'name="fname" value="(?P<N>.+?)">'
    SIZE_PATTERN = '>Size: .+?">(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)'
    OFFLINE_PATTERN = '>(File Not Found</b><br><br>|File Not Found</h2>)'
    RAND_ID_PATTERN = 'type=\\"hidden\\" name=\\"rand\\" value=\\"(.+)\\">'
    LINK_FREE_PATTERN = "var download_url = \\'(.+)\\';"
    ...
    def handle_free(self, pyfile):
        if False:
            while True:
                i = 10
        post_data = {'op': 'download1', 'usr_login': '', 'id': self.info['pattern']['ID'], 'fname': pyfile.name, 'referer': '', 'method_free': '+'}
        ...
        post_data = {'op': 'download2', 'id': self.info['pattern']['ID'], 'rand': rand, 'referer': pyfile.url, 'method_free': '+', 'method_premium': '', 'adcopy_response': response, 'adcopy_challenge': challenge, 'down_direct': '1'}
        self.data = self.load(pyfile.url, post=post_data)
