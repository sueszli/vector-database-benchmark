...
class ChannelsEndpoint(MetadataEndpointBase):
    ...
    @docs(tags=['Metadata'], summary='Get a list of all channels known to the system.', responses={200: {'schema': schema(GetChannelsResponse={'results': [ChannelSchema], 'first': Integer(), 'last': Integer(), 'sort_by': String(), 'sort_desc': Integer(), 'total': Integer()})}})
                ...
                if channel.subscribed:
                    if channel_dict['state'] == CHANNEL_STATE.UPDATING.value:
                        try:
                            ...
                            self._logger.error('Error %s when calculating channel update progress. Channel data: %s-%i %i/%i', e, hexlify(channel.public_key), channel.id_, channel.start_timestamp, channel.local_version)
                    elif channel_dict['state'] == CHANNEL_STATE.METAINFO_LOOKUP.value:
                        if not self.download_manager.metainfo_requests.get(bytes(channel.infohash)) and self.download_manager.download_exists(bytes(channel.infohash)):
                            channel_dict['state'] = CHANNEL_STATE.DOWNLOADING.value
                channels_list.append(channel_dict)
        response_dict = {'results': channels_list, 'first': sanitized['first'], 'last': sanitized['last'], 'sort_by': sanitized['sort_by'], 'sort_desc': int(sanitized['sort_desc'])}
        if total is not None:
            response_dict.update({'total': total})
        return RESTResponse(response_dict)

    @docs(tags=['Metadata'], summary="Get a list of the channel's contents (torrents/channels/etc.).", responses={200: {'schema': schema(GetChannelContentsResponse={'results': [MetadataSchema], 'first': Integer(), 'last': Integer(), 'sort_by': String(), 'sort_desc': Integer(), 'total': Integer()})}})
    async def get_channel_contents(self, request):
        ...
        response_dict = {'results': contents_list, 'first': sanitized['first'], 'last': sanitized['last'], 'sort_by': sanitized['sort_by'], 'sort_desc': int(sanitized['sort_desc'])}
        if total is not None:
            response_dict.update({'total': total})
        return RESTResponse(response_dict)
    ...
    async def get_channel_description(self, request):
        (channel_pk, channel_id) = self.get_channel_from_request(request)
        with db_session:
            channel_description = self.mds.ChannelDescription.select(lambda g: g.public_key == channel_pk and g.origin_id == channel_id).first()
    ...
    async def put_channel_description(self, request):
        ...
        with db_session:
            ...
            if channel_description is not None:
                ...
                channel_description = self.mds.ChannelDescription(public_key=channel_pk, origin_id=channel_id, json_text=updated_json_text, status=NEW)
        return RESTResponse(json.loads(channel_description.json_text))

    async def get_channel_thumbnail(self, request):
        (channel_pk, channel_id) = self.get_channel_from_request(request)
        with db_session:
            obj = self.mds.ChannelThumbnail.select(lambda g: g.public_key == channel_pk and g.origin_id == channel_id).first()
    ...
    @docs(tags=['Metadata'], summary='Create a copy of an entry/entries from another channel.', parameters=[{'in': 'body', 'name': 'entries', 'description': 'List of entries to copy', 'example': [{'public_key': '1234567890', 'id': 123}], 'required': True}], responses={200: {'description': 'Returns a list of copied content'}, HTTP_NOT_FOUND: {'schema': HandledErrorSchema, 'example': {'error': 'Target channel not found'}}, HTTP_BAD_REQUEST: {'schema': HandledErrorSchema, 'example': {'error': 'Source entry not found'}}})
    ...
    @docs(tags=['Metadata'], summary='Create a new channel entry in the given channel.', responses={200: {'description': 'Returns the newly created channel', 'schema': schema(CreateChannelResponse={'results': [ChannelSchema]})}})
    ...
    @docs(tags=['Metadata'], summary='Create a new collection entry in the given channel.', responses={200: {'description': 'Returns the newly created collection', 'schema': schema(CreateCollectionResponse={'results': [ChannelSchema]})}})
    ...
    @docs(tags=['Metadata'], summary='Add a torrent file to your own channel.', responses={200: {'schema': schema(AddTorrentToChannelResponse={'added': (Integer, 'Number of torrent that were added to the channel')})}, HTTP_NOT_FOUND: {'schema': HandledErrorSchema, 'example': {'error': 'Unknown channel'}}, HTTP_BAD_REQUEST: {'schema': HandledErrorSchema, 'example': {'error': 'unknown uri type'}}})
    @json_schema(schema(AddTorrentToChannelRequest={'torrent': (String, 'Base64-encoded torrent file'), 'uri': (String, 'Add a torrent from a magnet link or URL'), 'torrents_dir': (String, 'Add all .torrent files from a chosen directory'), 'recursive': (Boolean, 'Toggle recursive scanning of the chosen directory for .torrent files'), 'description': (String, 'Description for the torrent')}))
    async def add_torrent_to_channel(self, request):
        ...
        if parameters.get('recursive'):
            ...
            if not torrents_dir:
                return RESTResponse({'error': 'the torrents_dir parameter should be provided when the recursive parameter is set'}, status=HTTP_BAD_REQUEST)
        if torrents_dir:
    ...
    @docs(tags=['Metadata'], summary='Commit a channel.', responses={200: {'schema': schema(CommitResponse={'success': Boolean()})}})
    ...
    @docs(tags=['Metadata'], summary='Get the list of most popular torrents. Functions as a pseudo-channel.', responses={200: {'schema': schema(GetPopularTorrentsResponse={'results': [TorrentSchema], 'first': Integer(), 'last': Integer()})}})
    async def get_popular_torrents_channel(self, request):
        ...
        self.add_download_progress_to_metadata_list(contents_list)
        self.add_statements_to_metadata_list(contents_list, hide_xxx=sanitized['hide_xxx'])
        response_dict = {'results': contents_list, 'first': sanitized['first'], 'last': sanitized['last']}
        return RESTResponse(response_dict)