        ...
        if isinstance(p.get_supported()[0], tuple):
            ...
            for py_ver in [tag for tag in p.get_supported() if tag.abi != 'none' and tag.platform != 'any']:
                ...
                if ret:
                    return ret
        return ''
all_packages = [PlainPackage('numpy', ['>=1.17,<1.24']), PlainPackage('opencv-python', [PckgVer('4.5.4.60', dependencies=['numpy<1.24'])]), CudaPackage('cupy', {'118': [PckgVer('12.2.0', python_min_ver='3.8', dependencies=['numpy<1.24'])]}, 'cupy-cuda11x'), CudaPackage('tensorflow-gpu', {'110': [PckgVer('2.12.1', python_min_ver='3.8', alias='tensorflow', dependencies=['protobuf<4', 'numpy<1.24', 'urllib3<2.0', 'typing_extensions<4.6']), PckgVer('2.13.0', python_min_ver='3.8', alias='tensorflow', dependencies=['protobuf<4', 'numpy<1.24', 'urllib3<2.0', 'typing_extensions<4.6'])]}), CudaPackageExtraIndex('torch', {'118': [PckgVer('2.1.0', python_min_ver='3.8', dependencies=['numpy<1.24'])]}, extra_index='https://download.pytorch.org/whl/cu{cuda_v}/'), CudaPackageExtraIndex('torchvision', {'118': [PckgVer('0.16.0', python_min_ver='3.8', dependencies=['numpy<1.24'])]}, extra_index='https://download.pytorch.org/whl/cu{cuda_v}/'), CudaPackageExtraIndex('paddlepaddle-gpu', {'110': [PckgVer('2.5.0.post118', dependencies=['protobuf<4', 'numpy<1.24'])]}, links_index='https://www.paddlepaddle.org.cn/whl/linux/mkl/avx/stable.html'), CudaPackageExtraIndex('jax', {'118': [PckgVer('0.4.13', python_min_ver='3.8', dependencies=['jaxlib'])]}, name='jax[cuda{cuda_v[0]}{cuda_v[1]}_local]', links_index='https://storage.googleapis.com/jax-releases/jax_cuda_releases.html'), CudaPackage('numba', {'110': [PckgVer('0.57.0', python_min_ver='3.8', dependencies=['numpy<1.24'])]})]
...
parser.add_argument('--links_index', '-k', dest='links_index', help='return relevant link indices list for pip', action='store_true', default=False)
