...
class ProductMediaUpdate(BaseMutation):
    ...
    class Arguments:
        ...
        input = ProductMediaUpdateInput(required=True, description='Fields required to update a product media.')
    ...
    def perform_mutation(cls, _root, info: ResolveInfo, /, *, id, input):
        ...
        product = models.Product.objects.prefetched_for_webhook().get(pk=media.product_id)
        alt = input.get('alt')
        if alt is not None:
            media.alt = alt
