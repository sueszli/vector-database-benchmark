...
dpgmm = mixture.BayesianGaussianMixture(n_components=10, covariance_type='full', weight_concentration_prior=0.01, weight_concentration_prior_type='dirichlet_process', mean_precision_prior=0.01, covariance_prior=1.0 * np.eye(2), init_params='random', max_iter=100, random_state=2).fit(X)
plot_results(X, dpgmm.predict(X), dpgmm.means_, dpgmm.covariances_, 1, 'Bayesian Gaussian mixture models with a Dirichlet process prior for $\\gamma_0=0.01$.')
(X_s, y_s) = dpgmm.sample(n_samples=2000)
plot_samples(X_s, y_s, dpgmm.n_components, 0, 'Gaussian mixture with a Dirichlet process prior for $\\gamma_0=0.01$ sampled with $2000$ samples.')
dpgmm = mixture.BayesianGaussianMixture(n_components=10, covariance_type='full', weight_concentration_prior=100.0, weight_concentration_prior_type='dirichlet_process', mean_precision_prior=0.01, covariance_prior=1.0 * np.eye(2), init_params='kmeans', max_iter=100, random_state=2).fit(X)
plot_results(X, dpgmm.predict(X), dpgmm.means_, dpgmm.covariances_, 2, 'Bayesian Gaussian mixture models with a Dirichlet process prior for $\\gamma_0=100$')
(X_s, y_s) = dpgmm.sample(n_samples=2000)
plot_samples(X_s, y_s, dpgmm.n_components, 1, 'Gaussian mixture with a Dirichlet process prior for $\\gamma_0=100$ sampled with $2000$ samples.')
plt.show()