...
import transformers
from transformers import AutoConfig, AutoFeatureExtractor, AutoModelForAudioClassification, HfArgumentParser, Trainer, TrainingArguments, set_seed
...
class DataTrainingArguments:
    ...
    eval_file: Optional[str] = field(default=None, metadata={'help': 'A file containing the validation audio paths and labels.'})
    ...
    eval_split_name: str = field(default='validation', metadata={'help': "The name of the training data set split to use (via the datasets library). Defaults to 'validation'"})
    ...
    max_length_seconds: float = field(default=20, metadata={'help': 'Audio clips will be randomly cut to this length during training if the value is set.'})
