...
try:
    from airflow.cli.cli_config import ARG_DAEMON, ARG_LOG_FILE, ARG_PID, ARG_SKIP_SERVE_LOGS, ARG_STDERR, ARG_STDOUT, ARG_VERBOSE, ActionCommand, Arg, GroupCommand, lazy_load_command
except ImportError:
    try:
    ...
    if packaging.version.parse(base_version) < packaging.version.parse('2.7.0'):
        raise AirflowOptionalProviderFeatureException(f"Celery Executor from Celery Provider should only be used with Airflow 2.7.0+.\nThis is Airflow {airflow_version} and Celery and CeleryKubernetesExecutor are available in the 'airflow.executors' package. You should not use the provider's executors in this version of Airflow.")
    raise
...
ARG_FLOWER_HOSTNAME = Arg(('-H', '--hostname'), default=conf.get('celery', 'FLOWER_HOST'), help='Set the hostname on which to run the server')
ARG_FLOWER_PORT = Arg(('-p', '--port'), default=conf.getint('celery', 'FLOWER_PORT'), type=int, help='The port on which to run the server')
ARG_FLOWER_CONF = Arg(('-c', '--flower-conf'), help='Configuration file for flower')
ARG_FLOWER_URL_PREFIX = Arg(('-u', '--url-prefix'), default=conf.get('celery', 'FLOWER_URL_PREFIX'), help='URL prefix for Flower')
ARG_FLOWER_BASIC_AUTH = Arg(('-A', '--basic-auth'), default=conf.get('celery', 'FLOWER_BASIC_AUTH'), help='Securing Flower with Basic Authentication. Accepts user:password pairs separated by a comma. Example: flower_basic_auth = user1:password1,user2:password2')
ARG_AUTOSCALE = Arg(('-a', '--autoscale'), help='Minimum and Maximum number of worker to autoscale')
ARG_QUEUES = Arg(('-q', '--queues'), help='Comma delimited list of queues to serve', default=conf.get('operators', 'DEFAULT_QUEUE'))
ARG_CONCURRENCY = Arg(('-c', '--concurrency'), type=int, help='The number of worker processes', default=conf.getint('celery', 'worker_concurrency'))
ARG_CELERY_HOSTNAME = Arg(('-H', '--celery-hostname'), help='Set the hostname of celery worker if you have multiple workers on a single machine')
ARG_UMASK = Arg(('-u', '--umask'), help='Set the umask of celery worker in daemon mode')
