...
SCHEMA = {'properties': {'ID': {'type': ['null', 'string']}}, 'type': 'object'}
SCHEMA_NAME = 'test'
STREAM = 'oracletest'
...
class OracleDBDestinationTests(unittest.TestCase, SQLDestinationMixin):
    config = {'host': 'host', 'password': 'password', 'user': 'user', 'port': 'port', 'service': 'service', 'lower_case': False}
    conn_class_path = 'mage_integrations.destinations.oracledb.OracleDBConnection'
    destination_class = OracleDB
    expected_conn_class_kwargs = dict(host='host', password='password', user='user', port='port', service='service')
    expected_template_config = {'config': {'host': '', 'port': '1521', 'service': '', 'password': '', 'user': '', 'database': ''}}
