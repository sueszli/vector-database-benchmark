...
if is_tf_available():
    ...
    from transformers.models.mpnet.modeling_tf_mpnet import TFMPNetForMaskedLM, TFMPNetForMultipleChoice, TFMPNetForQuestionAnswering, TFMPNetForSequenceClassification, TFMPNetForTokenClassification, TFMPNetModel
...
class TFMPNetModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        config = MPNetConfig(vocab_size=self.vocab_size, hidden_size=self.hidden_size, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, hidden_act=self.hidden_act, hidden_dropout_prob=self.hidden_dropout_prob, attention_probs_dropout_prob=self.attention_probs_dropout_prob, max_position_embeddings=self.max_position_embeddings, initializer_range=self.initializer_range)
        return (config, input_ids, input_mask, sequence_labels, token_labels, choice_labels)

    def create_and_check_mpnet_model(self, config, input_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            while True:
                i = 10
    ...
    def create_and_check_mpnet_for_masked_lm(self, config, input_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            while True:
                i = 10
    ...
    def create_and_check_mpnet_for_question_answering(self, config, input_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            print('Hello World!')
    ...
    def create_and_check_mpnet_for_sequence_classification(self, config, input_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            return 10
    ...
    def create_and_check_mpnet_for_multiple_choice(self, config, input_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            for i in range(10):
                print('nop')
        ...
        inputs = {'input_ids': multiple_choice_inputs_ids, 'attention_mask': multiple_choice_input_mask}
        result = model(inputs)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.num_choices))

    def create_and_check_mpnet_for_token_classification(self, config, input_ids, input_mask, sequence_labels, token_labels, choice_labels):
        if False:
            while True:
                i = 10
        config.num_labels = self.num_labels
...
class TFMPNetModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TFMPNetForMaskedLM, TFMPNetForMultipleChoice, TFMPNetForQuestionAnswering, TFMPNetForSequenceClassification, TFMPNetForTokenClassification, TFMPNetModel) if is_tf_available() else ()
    pipeline_model_mapping = {'feature-extraction': TFMPNetModel, 'fill-mask': TFMPNetForMaskedLM, 'question-answering': TFMPNetForQuestionAnswering, 'text-classification': TFMPNetForSequenceClassification, 'token-classification': TFMPNetForTokenClassification, 'zero-shot': TFMPNetForSequenceClassification} if is_tf_available() else {}
...
class TFMPNetModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        ...
        expected_slice = tf.constant([[[-0.1067172, 0.08216473, 0.0024543], [-0.03465879, 0.8354118, -0.03252288], [-0.06569476, -0.12424111, -0.0494436]]])
