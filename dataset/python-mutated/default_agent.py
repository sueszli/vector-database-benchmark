...
from hassil.intents import Intents, ResponseType, SlotList, TextSlotList, WildcardSlotList
...
from homeassistant.helpers import area_registry as ar, device_registry as dr, entity_registry as er, intent, start, template, translation
from homeassistant.helpers.event import EventStateChangedData, async_track_state_added_domain
...
class DefaultAgent(AbstractConversationAgent):
    ...
    async def async_initialize(self, config_intents):
        ...
        self.hass.bus.async_listen(ar.EVENT_AREA_REGISTRY_UPDATED, self._async_handle_area_registry_changed, run_immediately=True)
        self.hass.bus.async_listen(er.EVENT_ENTITY_REGISTRY_UPDATED, self._async_handle_entity_registry_changed, run_immediately=True)
        self.hass.bus.async_listen(core.EVENT_STATE_CHANGED, self._async_handle_state_changed, run_immediately=True)
        async_listen_entity_updates(self.hass, DOMAIN, self._async_exposed_entities_updated)

    async def async_recognize(self, user_input: ConversationInput) -> RecognizeResult | None:
        """Recognize intent from user input."""
        language = user_input.language or self.hass.config.language
        lang_intents = self._lang_intents.get(language)
        if lang_intents is None or lang_intents.loaded_components - self.hass.config.components:
        ...
        result = await self.hass.async_add_executor_job(self._recognize, user_input, lang_intents, slot_lists)
        return result
    ...
    async def async_process(self, user_input: ConversationInput) -> ConversationResult:
        ...
        if result is None:
            ...
            return _make_error_result(language, intent.IntentResponseErrorCode.NO_INTENT_MATCH, self._get_error_text(ResponseType.NO_INTENT, lang_intents), conversation_id)
        assert lang_intents is not None
        try:
            intent_response = await intent.async_handle(self.hass, DOMAIN, result.intent.name, {entity.name: {'value': entity.value} for entity in result.entities_list}, user_input.text, user_input.context, language, assistant=DOMAIN)
        except intent.IntentHandleError:
            _LOGGER.exception('Intent handling error')
            return _make_error_result(language, intent.IntentResponseErrorCode.FAILED_TO_HANDLE, self._get_error_text(ResponseType.HANDLE_ERROR, lang_intents), conversation_id)
        except intent.IntentUnexpectedError:
            _LOGGER.exception('Unexpected intent error')
            return _make_error_result(language, intent.IntentResponseErrorCode.UNKNOWN, self._get_error_text(ResponseType.HANDLE_ERROR, lang_intents), conversation_id)
    ...
    async def _build_speech(self, language: str, response_template: template.Template, intent_response: intent.IntentResponse, recognize_result: RecognizeResult) -> str:
        ...
        translations = await translation.async_get_translations(self.hass, language, 'entity_component', domains)
        ...
        if intent_response.matched_states:
            ...
            state1 = unmatched[0]
        speech = response_template.async_render({'slots': {entity_name: entity_value.text or entity_value.value for (entity_name, entity_value) in recognize_result.entities.items()}, 'state': template.TemplateState(self.hass, state1) if state1 is not None else None, 'query': {'matched': [template.TemplateState(self.hass, state) for state in matched], 'unmatched': [template.TemplateState(self.hass, state) for state in unmatched]}})
    ...
    async def async_get_or_load_intents(self, language: str) -> LanguageIntents | None:
        ...
        async with self._lang_lock[language]:
            return await self.hass.async_add_executor_job(self._get_or_load_intents, language, hass_components)
    ...
    async def _match_triggers(self, sentence: str) -> ConversationResult | None:
        ...
        if not matched_triggers:
            return None
        _LOGGER.debug("'%s' matched %s trigger(s): %s", sentence, len(matched_triggers), list(matched_triggers))
        trigger_responses = await asyncio.gather(*(self._trigger_sentences[trigger_id].callback(sentence, result) for (trigger_id, result) in matched_triggers.items()))
