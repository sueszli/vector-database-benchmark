...
class TypeDeclUnit(Node):
  ...
  """
    ...
    def _InitCache(self):
        if False:
            print('Hello World!')
        self.PopulateLookupCache(self.constants, self.functions, self.classes, self.aliases)
...
class ClassType(Node, Type):
    ...
    def __repr__(self):
        if False:
            for i in range(10):
                print('nop')
        return '{type}{cls}({name})'.format(type=type(self).__name__, name=self.name, cls='<unresolved>' if self.cls is None else '')
...
def _FlattenTypes(type_list) -> Tuple[Type, ...]:
    ...
    assert type_list
    flattened = itertools.chain.from_iterable((t.type_list if isinstance(t, _SetOfTypes) else [t] for t in type_list))
...
def IsContainer(t: Class) -> bool:
    ...
    for p in t.bases:
        if isinstance(p, GenericType):
            base = p.base_type
            if isinstance(base, ClassType) and IsContainer(base.cls) or isinstance(base, LateType):
                return True
    return False
...
def AliasMethod(func, from_constant):
    ...
    if func.kind == MethodKind.STATICMETHOD or (func.kind == MethodKind.METHOD and (not from_constant)):
        return new_func
    return new_func.Replace(signatures=tuple((s.Replace(params=s.params[1:]) for s in new_func.signatures)))
...
def LookupItemRecursive(module: TypeDeclUnit, name: str) -> Node:
    ...
    for part in parts:
        ...
        if found:
            seen = {found}
            while isinstance(found, Alias) and isinstance(found.type, NamedType) and found.type.name.startswith(f'{item.name}.'):
