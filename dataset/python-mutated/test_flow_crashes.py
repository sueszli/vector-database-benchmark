...
async def test_anyio_cancellation_crashes_flow(prefect_client):
    ...
    await assert_flow_run_crashed(flow_run, expected_message='Execution was cancelled by the runtime environment')
...
async def test_anyio_cancellation_crashes_parent_and_child_flow(prefect_client):
    ...
    await assert_flow_run_crashed(child_flow_run, expected_message='Execution was cancelled by the runtime environment')
    parent_flow_run = await prefect_client.read_flow_run(parent_flow_run_id)
    await assert_flow_run_crashed(parent_flow_run, expected_message='Execution was cancelled by the runtime environment')
...
async def test_interrupt_crashes_flow(prefect_client, interrupt_type):
    ...
    with pytest.raises(interrupt_type):
        await my_flow()
    flow_run = await prefect_client.read_flow_run(flow_run_id)
    await assert_flow_run_crashed(flow_run, expected_message='Execution was aborted')

@pytest.mark.filterwarnings('ignore::pytest.PytestUnhandledThreadExceptionWarning')
@pytest.mark.parametrize('interrupt_type', [KeyboardInterrupt, SystemExit])
async def test_interrupt_in_child_crashes_parent_and_child_flow(prefect_client, interrupt_type):
    child_flow_run_id = parent_flow_run_id = None
    ...
    with pytest.raises(interrupt_type):
        await parent_flow()
    parent_flow_run = await prefect_client.read_flow_run(parent_flow_run_id)
    await assert_flow_run_crashed(parent_flow_run, expected_message='Execution was aborted')
    child_flow_run = await prefect_client.read_flow_run(child_flow_run_id)
    await assert_flow_run_crashed(child_flow_run, expected_message='Execution was aborted')
...
async def test_sigterm_crashes_flow(prefect_client, mock_sigterm_handler):
    ...
    with pytest.raises(prefect.exceptions.TerminationSignal):
        await my_flow()
    flow_run = await prefect_client.read_flow_run(flow_run_id)
    await assert_flow_run_crashed(flow_run, expected_message='Execution was aborted by a termination signal')
