...
from strawberry.subscriptions.protocols.graphql_transport_ws.types import CompleteMessage, ConnectionAckMessage, ConnectionInitMessage, ErrorMessage, NextMessage, PingMessage, PongMessage, SubscribeMessage, SubscribeMessagePayload
...
async def ws_raw(http_client: HttpClient) -> AsyncGenerator[WebSocketClient, None]:
    async with http_client.ws_connect('/graphql', protocols=[GRAPHQL_TRANSPORT_WS_PROTOCOL]) as ws:
        yield ws
    await ws.close()
    assert ws.closed
...
async def test_connection_init_timeout(request: Any, http_client_class: Type[HttpClient]):
    with contextlib.suppress(ImportError):
        from tests.http.clients.aiohttp import AioHttpClient
        if http_client_class == AioHttpClient:
            pytest.skip("Closing a AIOHTTP WebSocket from a task currently doesn't work as expected")
    test_client = http_client_class()
    test_client.create_app(connection_init_wait_timeout=timedelta(seconds=0))
    async with test_client.ws_connect('/graphql', protocols=[GRAPHQL_TRANSPORT_WS_PROTOCOL]) as ws:
        data = await ws.receive(timeout=2)
        assert ws.closed
...
@pytest.mark.flaky
async def test_connection_init_timeout_cancellation(ws_raw: WebSocketClient):
    ws = ws_raw
    await ws.send_json(ConnectionInitMessage().as_dict())
    response = await ws.receive_json()
    assert response == ConnectionAckMessage().as_dict()
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { debug { isConnectionInitTimeoutTaskDone } }')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub1', payload={'data': {'debug': {'isConnectionInitTimeoutTaskDone': True}}}).as_dict()

async def test_close_twice(mocker: MockerFixture, request: Any, http_client_class: Type[HttpClient]):
    test_client = http_client_class()
    test_client.create_app(connection_init_wait_timeout=timedelta(seconds=0.25))
    async with test_client.ws_connect('/graphql', protocols=[GRAPHQL_TRANSPORT_WS_PROTOCOL]) as ws:
        transport_close = mocker.patch.object(ws, 'close')
        await ws.send_json(ConnectionInitMessage(payload='invalid value').as_dict())
        await asyncio.sleep(0)
        for t in asyncio.all_tasks():
            if t.get_coro().__qualname__ == 'BaseGraphQLTransportWSHandler.handle_connection_init_timeout':
...
async def test_server_sent_ping(ws: WebSocketClient):
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { requestPing }')).as_dict())
    response = await ws.receive_json()
    assert response == PingMessage().as_dict()
    await ws.send_json(PongMessage().as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub1', payload={'data': {'requestPing': True}}).as_dict()
    response = await ws.receive_json()
    assert response == CompleteMessage(id='sub1').as_dict()

async def test_unauthorized_subscriptions(ws_raw: WebSocketClient):
    ws = ws_raw
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { echo(message: "Hi") }')).as_dict())
    data = await ws.receive(timeout=2)
    assert ws.closed
...
async def test_duplicated_operation_ids(ws: WebSocketClient):
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { echo(message: "Hi", delay: 5) }')).as_dict())
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { echo(message: "Hi", delay: 5) }')).as_dict())
    data = await ws.receive(timeout=2)
    assert ws.closed
...
async def test_subscription_syntax_error(ws: WebSocketClient):
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { INVALID_SYNTAX ')).as_dict())
    data = await ws.receive(timeout=2)
    assert ws.closed
    assert ws.close_code == 4400
...
async def test_subscription_field_errors(ws: WebSocketClient):
    ...
    with patch.object(Schema, 'process_errors', process_errors):
        await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { notASubscriptionField }')).as_dict())
        response = await ws.receive_json()
        ...
        assert response['payload'][0]['message'] == "The subscription field 'notASubscriptionField' is not defined."
        process_errors.assert_called_once()

async def test_subscription_cancellation(ws: WebSocketClient):
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { echo(message: "Hi", delay: 99) }')).as_dict())
    await ws.send_json(SubscribeMessage(id='sub2', payload=SubscribeMessagePayload(query='subscription { debug { numActiveResultHandlers } }')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub2', payload={'data': {'debug': {'numActiveResultHandlers': 2}}}).as_dict()
    response = await ws.receive_json()
    assert response == CompleteMessage(id='sub2').as_dict()
    await ws.send_json(CompleteMessage(id='sub1').as_dict())
    await ws.send_json(SubscribeMessage(id='sub3', payload=SubscribeMessagePayload(query='subscription { debug { numActiveResultHandlers } }')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub3', payload={'data': {'debug': {'numActiveResultHandlers': 1}}}).as_dict()
    response = await ws.receive_json()
    assert response == CompleteMessage(id='sub3').as_dict()

async def test_subscription_errors(ws: WebSocketClient):
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { error(message: "TEST ERR") }')).as_dict())
    response = await ws.receive_json()
...
async def test_operation_error_no_complete(ws: WebSocketClient):
    ...
    """
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='query { error(message: "TEST ERR") }')).as_dict())
    response = await ws.receive_json()
    assert response['type'] == ErrorMessage.type
    assert response['id'] == 'sub1'
    await ws.send_json(SubscribeMessage(id='sub2', payload=SubscribeMessagePayload(query='query { error(message: "TEST ERR") }')).as_dict())
    response = await ws.receive_json()
    assert response['type'] == ErrorMessage.type
...
async def test_subscription_exceptions(ws: WebSocketClient):
    process_errors = Mock()
    with patch.object(Schema, 'process_errors', process_errors):
        await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { exception(message: "TEST EXC") }')).as_dict())
        response = await ws.receive_json()
        assert response['type'] == ErrorMessage.type
        assert response['id'] == 'sub1'
        assert len(response['payload']) == 1
...
async def test_single_result_query_operation(ws: WebSocketClient):
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='query { hello }')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub1', payload={'data': {'hello': 'Hello world'}}).as_dict()
    response = await ws.receive_json()
    assert response == CompleteMessage(id='sub1').as_dict()
...
async def test_single_result_query_operation_async(ws: WebSocketClient):
    ...
    async delay
    """
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='query { asyncHello(name: "Dolly", delay:0.01)}')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub1', payload={'data': {'asyncHello': 'Hello Dolly'}}).as_dict()
    response = await ws.receive_json()
    assert response == CompleteMessage(id='sub1').as_dict()
...
async def test_single_result_query_operation_overlapped(ws: WebSocketClient):
    ...
    """
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='query { asyncHello(name: "Dolly", delay:1)}')).as_dict())
    await ws.send_json(SubscribeMessage(id='sub2', payload=SubscribeMessagePayload(query='query { asyncHello(name: "Dolly", delay:0)}')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub2', payload={'data': {'asyncHello': 'Hello Dolly'}}).as_dict()
    response = await ws.receive_json()
    assert response == CompleteMessage(id='sub2').as_dict()

async def test_single_result_mutation_operation(ws: WebSocketClient):
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='mutation { hello }')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub1', payload={'data': {'hello': 'strawberry'}}).as_dict()
    response = await ws.receive_json()
    assert response == CompleteMessage(id='sub1').as_dict()

async def test_single_result_operation_selection(ws: WebSocketClient):
    query = '\n        query Query1 {\n            hello\n        }\n        query Query2 {\n            hello(name: "Strawberry")\n        }\n    '
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query=query, operationName='Query2')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub1', payload={'data': {'hello': 'Hello Strawberry'}}).as_dict()
    response = await ws.receive_json()
    assert response == CompleteMessage(id='sub1').as_dict()
...
async def test_single_result_operation_error(ws: WebSocketClient):
    process_errors = Mock()
    with patch.object(Schema, 'process_errors', process_errors):
        await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='query { alwaysFail }')).as_dict())
        response = await ws.receive_json()
        assert response['type'] == ErrorMessage.type
        assert response['id'] == 'sub1'
        assert len(response['payload']) == 1
...
async def test_single_result_operation_exception(ws: WebSocketClient):
    ...
    process_errors = Mock()
    with patch.object(Schema, 'process_errors', process_errors):
        await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='query { exception(message: "bummer") }')).as_dict())
        response = await ws.receive_json()
        assert response['type'] == ErrorMessage.type
        assert response['id'] == 'sub1'
        assert len(response['payload']) == 1
...
async def test_single_result_duplicate_ids_query(ws: WebSocketClient):
    ...
    """
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='query { asyncHello(name: "Hi", delay: 5) }')).as_dict())
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='query { hello }')).as_dict())
    data = await ws.receive(timeout=2)
    assert ws.closed
    assert ws.close_code == 4409
...
async def test_injects_connection_params(ws_raw: WebSocketClient):
    ...
    response = await ws.receive_json()
    assert response == ConnectionAckMessage().as_dict()
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { connectionParams }')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub1', payload={'data': {'connectionParams': 'rocks'}}).as_dict()
    await ws.send_json(CompleteMessage(id='sub1').as_dict())
...
@pytest.mark.parametrize('payload', [[], 'invalid value', 1])
async def test_rejects_connection_params_with_wrong_type(payload: Any, ws_raw: WebSocketClient):
    ws = ws_raw
...
async def test_subsciption_cancel_finalization_delay(ws: WebSocketClient):
    delay = 0.1
    await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query=f'subscription {{ longFinalizer(delay: {delay}) }}')).as_dict())
    response = await ws.receive_json()
    assert response == NextMessage(id='sub1', payload={'data': {'longFinalizer': 'hello'}}).as_dict()
    start = time.time()
    await ws.send_json(CompleteMessage(id='sub1').as_dict())
    await ws.send_json(SubscribeMessage(id='sub2', payload=SubscribeMessagePayload(query='query { hello }')).as_dict())
    while True:
        response = await ws.receive_json()
...
async def test_subscription_errors_continue(ws: WebSocketClient):
    ...
    process_errors = Mock()
    with patch.object(Schema, 'process_errors', process_errors):
        await ws.send_json(SubscribeMessage(id='sub1', payload=SubscribeMessagePayload(query='subscription { flavorsInvalid }')).as_dict())
        response = await ws.receive_json()
        assert response['type'] == NextMessage.type
        assert response['id'] == 'sub1'
