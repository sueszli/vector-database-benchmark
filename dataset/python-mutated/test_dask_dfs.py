...
def test_single_table_dask_entityset():
    ...
    dask_es.add_dataframe(dataframe_name='data', dataframe=values_dd, index='id', logical_types=ltypes)
    (dask_fm, _) = dfs(entityset=dask_es, target_dataframe_name='data', trans_primitives=primitives_list)
...
def test_single_table_dask_entityset_ids_not_sorted():
    ...
    (fm, _) = dfs(entityset=pd_es, target_dataframe_name='data', trans_primitives=primitives_list)
    dask_fm = dask_fm.compute().astype({'id': 'int64'})
    pd.testing.assert_frame_equal(fm, dask_fm.set_index('id').loc[fm.index], check_dtype=False)
...
def test_single_table_dask_entityset_with_instance_ids():
    ...
    ltypes = {'values': Integer, 'dates': Datetime, 'strings': NaturalLanguage}
    dask_es.add_dataframe(dataframe_name='data', dataframe=values_dd, index='id', logical_types=ltypes)
    (dask_fm, _) = dfs(entityset=dask_es, target_dataframe_name='data', trans_primitives=primitives_list, instance_ids=instance_ids)
    pd_es = EntitySet(id='pd_es')
    pd_es.add_dataframe(dataframe_name='data', dataframe=df, index='id', logical_types=ltypes)
...
def test_single_table_dask_entityset_single_cutoff_time():
    ...
    ltypes = {'values': Integer, 'dates': Datetime, 'strings': NaturalLanguage}
    dask_es.add_dataframe(dataframe_name='data', dataframe=values_dd, index='id', logical_types=ltypes)
    (dask_fm, _) = dfs(entityset=dask_es, target_dataframe_name='data', trans_primitives=primitives_list, cutoff_time=pd.Timestamp('2019-01-05 04:00'))
    pd_es = EntitySet(id='pd_es')
    pd_es.add_dataframe(dataframe_name='data', dataframe=df, index='id', logical_types=ltypes)
...
def test_single_table_dask_entityset_cutoff_time_df():
    ...
    dask_es.add_dataframe(dataframe_name='data', dataframe=values_dd, index='id', time_index='dates', logical_types=ltypes)
    ids = [0, 1, 2, 0]
    ...
    (dask_fm, _) = dfs(entityset=dask_es, target_dataframe_name='data', trans_primitives=primitives_list, cutoff_time=cutoff_times)
    pd_es = EntitySet(id='pd_es')
...
def test_single_table_dask_entityset_dates_not_sorted():
    ...
    dask_es.add_dataframe(dataframe_name='data', dataframe=values_dd, index='id', time_index='dates', logical_types=ltypes)
    (dask_fm, _) = dfs(entityset=dask_es, target_dataframe_name='data', trans_primitives=primitives_list, max_depth=1)
    pd_es = EntitySet(id='pd_es')
    pd_es.add_dataframe(dataframe_name='data', dataframe=df, index='id', time_index='dates', logical_types=ltypes)
...
def test_dask_entityset_secondary_time_index():
    ...
    pd_es.add_dataframe(dataframe_name='logs', dataframe=log_df, index='id', time_index='scheduled_time', secondary_time_index={'arrival_time': ['departure_time', 'delay']}, logical_types=log_ltypes)
    dask_es.add_dataframe(dataframe_name='logs', dataframe=log_dask, index='id', logical_types=log_ltypes, semantic_tags={'flight_id': 'foreign_key'}, time_index='scheduled_time', secondary_time_index={'arrival_time': ['departure_time', 'delay']})
    ...
    (dask_fm, _) = dfs(entityset=dask_es, target_dataframe_name='logs', cutoff_time=cutoff_df, agg_primitives=['max'], trans_primitives=['month'])
