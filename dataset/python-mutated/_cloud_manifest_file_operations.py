...
def build_get_request(verification_version: str, *, version_creation_date: Optional[str]=None, **kwargs: Any) -> HttpRequest:
    if False:
        for i in range(10):
            print('nop')
    _headers = case_insensitive_dict(kwargs.pop('headers', {}) or {})
    ...
    path_format_arguments = {'verificationVersion': _SERIALIZER.url('verification_version', verification_version, 'str')}
...
class CloudManifestFileOperations:
    ...
    @distributed_trace
    def get(self, verification_version: str, version_creation_date: Optional[str]=None, **kwargs: Any) -> _models.CloudManifestFileResponse:
        if False:
            i = 10
            return i + 15
        ...
        api_version: Literal['2022-06-01'] = kwargs.pop('api_version', _params.pop('api-version', self._config.api_version))
        cls: ClsType[_models.CloudManifestFileResponse] = kwargs.pop('cls', None)
        request = build_get_request(verification_version=verification_version, version_creation_date=version_creation_date, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        pipeline_response: PipelineResponse = self._client._pipeline.run(request, stream=False, **kwargs)
        response = pipeline_response.http_response
