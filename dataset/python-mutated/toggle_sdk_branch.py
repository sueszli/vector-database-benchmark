...
def update_sdk_dependencies(cargo: TOMLDocument, branch: Union['main', 'next']) -> None:
    ...
    for name in dependencies:
        if isinstance(name, str):
            ...
            if not isinstance(dependency, str) and dependency.get('git', None) == SDK_ORIGIN and (dependency.get('branch', None) is not None):
                logging.debug(f"Found SDK crate {name} currently on {dependency['branch']}")
...
arg_parser.add_argument('--root', default='../', help='Root path for rust_dev_preview cargos. Default ../ assumes running this script from its location in tools. Update single examples by setting root to their folder directly.')
arg_parser.add_argument('--branch', default='main', choices=['main', 'next'], help='Branch to use')
arg_parser.add_argument('--dry-run', dest='dry_run', action='store_true', default=False, help="Don't write updated Cargo files.")
arg_parser.add_argument('--verbose', action='store_true', default=False, help='Write verbose logging')
