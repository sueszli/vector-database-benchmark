...
class AsyncQuerySetTest(TestCase):
    ...
    def setUpTestData(cls):
        if False:
            while True:
                i = 10
        cls.s1 = SimpleModel.objects.create(field=1, created=datetime(2022, 1, 1, 0, 0, 0))
        cls.s2 = SimpleModel.objects.create(field=2, created=datetime(2022, 1, 1, 0, 0, 1))
        cls.s3 = SimpleModel.objects.create(field=3, created=datetime(2022, 1, 1, 0, 0, 2))
    ...
    async def test_aiterator_prefetch_related(self):
        results = []
        async for s in SimpleModel.objects.prefetch_related(Prefetch('relatedmodel_set', to_attr='prefetched_relatedmodel')).aiterator():
    ...
    async def test_acount_cached_result(self):
        ...
        self.assertEqual(count, 3)
        await sync_to_async(SimpleModel.objects.create)(field=4, created=datetime(2022, 1, 1, 0, 0, 0))
        count = await qs.acount()
        self.assertEqual(count, 3)
    ...
    async def test_aupdate_or_create(self):
        (instance, created) = await SimpleModel.objects.aupdate_or_create(id=self.s1.id, defaults={'field': 2})
        self.assertEqual(instance, self.s1)
        ...
        self.assertEqual(await SimpleModel.objects.acount(), 4)
        self.assertIs(created, True)
        (instance, created) = await SimpleModel.objects.aupdate_or_create(field=5, defaults={'field': 7}, create_defaults={'field': 6})
    ...
    async def test_update_conflicts_unique_field_unsupported(self):
        msg = 'This database backend does not support updating conflicts with specifying unique fields that can trigger the upsert.'
        with self.assertRaisesMessage(NotSupportedError, msg):
            await SimpleModel.objects.abulk_create([SimpleModel(field=1), SimpleModel(field=2)], update_conflicts=True, update_fields=['field'], unique_fields=['created'])
    ...
    async def test_abulk_update(self):
        ...
        self.assertCountEqual(qs, [(self.s1.pk, 10), (self.s2.pk, 20), (self.s3.pk, 30)])

    async def test_ain_bulk(self):
        res = await SimpleModel.objects.ain_bulk()
        self.assertEqual(res, {self.s1.pk: self.s1, self.s2.pk: self.s2, self.s3.pk: self.s3})
    ...
    async def test_aexplain(self):
        supported_formats = await sync_to_async(self._get_db_feature)(connection, 'supported_explain_formats')
