...
from ray._private.test_utils import chdir, format_web_url, wait_until_server_available, wait_for_condition, run_string_as_driver_nonblocking, get_current_unused_port, async_wait_for_condition_async_predicate
from ray.dashboard.modules.job.common import JobSubmitRequest, validate_request_type, JOB_ACTOR_NAME_TEMPLATE, SUPERVISOR_ACTOR_RAY_NAMESPACE
from ray.dashboard.tests.conftest import *
from ray.runtime_env.runtime_env import RuntimeEnv, RuntimeEnvConfig
from ray.util.state import get_node, list_nodes, list_actors
...
async def test_submit_job(job_sdk_client, runtime_env_option, monkeypatch):
    ...
    request = validate_request_type({'runtime_env': runtime_env, 'entrypoint': runtime_env_option['entrypoint']}, JobSubmitRequest)
    submit_result = await agent_client.submit_job_internal(request)
    job_id = submit_result.submission_id
    try:
        job_start_time = time.time()
        wait_for_condition(partial(_check_job, client=head_client, job_id=job_id, status=JobStatus.SUCCEEDED), timeout=300)
...
async def test_timeout(job_sdk_client):
    (agent_client, head_client) = job_sdk_client
    runtime_env = RuntimeEnv(pip={'packages': ['tensorflow', 'requests', 'botocore', 'torch'], 'pip_check': False, 'pip_version': "==22.0.2;python_version=='3.8.11'"}, config=RuntimeEnvConfig(setup_timeout_seconds=1)).to_dict()
    request = validate_request_type({'runtime_env': runtime_env, 'entrypoint': 'echo hello'}, JobSubmitRequest)
    submit_result = await agent_client.submit_job_internal(request)
    job_id = submit_result.submission_id
    wait_for_condition(partial(_check_job, client=head_client, job_id=job_id, status=JobStatus.FAILED), timeout=10)
...
async def test_stop_long_running_job(job_sdk_client):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        request = validate_request_type({'runtime_env': runtime_env, 'entrypoint': 'python test_script.py'}, JobSubmitRequest)
        submit_result = await agent_client.submit_job_internal(request)
        job_id = submit_result.submission_id
        resp = await agent_client.stop_job_internal(job_id)
        assert resp.stopped is True
        wait_for_condition(partial(_check_job, client=head_client, job_id=job_id, status=JobStatus.STOPPED), timeout=10)

@pytest.mark.asyncio
async def test_tail_job_logs_with_echo(job_sdk_client):
    (agent_client, head_client) = job_sdk_client
    runtime_env = RuntimeEnv().to_dict()
    entrypoint = 'python -c "import time; [(print(\'Hello\', i), time.sleep(0.1)) for i in range(100)]"'
    request = validate_request_type({'runtime_env': runtime_env, 'entrypoint': entrypoint}, JobSubmitRequest)
    submit_result = await agent_client.submit_job_internal(request)
    job_id = submit_result.submission_id
    i = 0
    async for lines in agent_client.tail_job_logs(job_id):
        ...
        for line in lines.strip().split('\n'):
            ...
            i += 1
    wait_for_condition(partial(_check_job, client=head_client, job_id=job_id, status=JobStatus.SUCCEEDED), timeout=10)

@pytest.mark.asyncio
@pytest.mark.parametrize('ray_start_cluster_head', [{'include_dashboard': True, 'dashboard_agent_listen_port': DEFAULT_DASHBOARD_AGENT_LISTEN_PORT}], indirect=True)
async def test_job_log_in_multiple_node(make_sure_dashboard_http_port_unused, enable_test_module, disable_aiohttp_cache, ray_start_cluster_head):
    cluster = ray_start_cluster_head
    assert wait_until_server_available(cluster.webui_url) is True
    webui_url = cluster.webui_url
    webui_url = format_web_url(webui_url)
    cluster.add_node(dashboard_agent_listen_port=DEFAULT_DASHBOARD_AGENT_LISTEN_PORT + 1)
    cluster.add_node(dashboard_agent_listen_port=DEFAULT_DASHBOARD_AGENT_LISTEN_PORT + 2)
    ...
    job_check_status = []
    JOB_NUM = 10
    job_agent_ports = [DEFAULT_DASHBOARD_AGENT_LISTEN_PORT, DEFAULT_DASHBOARD_AGENT_LISTEN_PORT + 1, DEFAULT_DASHBOARD_AGENT_LISTEN_PORT + 2]
    for index in range(JOB_NUM):
        runtime_env = RuntimeEnv().to_dict()
        request = validate_request_type({'runtime_env': runtime_env, 'entrypoint': f'while true; do echo hello index-{index} && sleep 3600; done'}, JobSubmitRequest)
        ...
        for (index, job_id) in enumerate(job_ids):
            ...
            for agent_port in job_agent_ports:
                if f'--listen-port={agent_port}' in ' '.join(node_info['cmdline']):
                    break
            assert f'--listen-port={agent_port}' in ' '.join(node_info['cmdline']), f'port: {agent_port}'
...
async def test_non_default_dashboard_agent_http_port(tmp_path):
    ...
    """
    import subprocess
    cmd = f'ray start --head --dashboard-agent-listen-port {get_current_unused_port()}'
