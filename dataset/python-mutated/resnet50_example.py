...
@serve.deployment(ray_actor_options={'num_cpus': 1}, max_concurrent_queries=5, autoscaling_config={'target_num_ongoing_requests_per_replica': 1, 'min_replicas': 0, 'initial_replicas': 0, 'max_replicas': 200})
class Model:
    ...
    def __init__(self):
        if False:
            while True:
                i = 10
        self.resnet50 = models.resnet50(weights=ResNet50_Weights.DEFAULT).eval().to('cpu')
...
import requests
serve.run(app)
resp = requests.post('http://localhost:8000/', json={'uri': 'https://serve-resnet-benchmark-data.s3.us-west-1.amazonaws.com/000000000019.jpeg'})
assert resp.text == 'ox'