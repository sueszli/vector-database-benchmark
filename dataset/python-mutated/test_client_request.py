...
from aiohttp.client_reqrep import ClientRequest, ClientResponse, Fingerprint, _gen_default_accept_encoding
...
async def test_content_type_auto_header_form(loop: Any, conn: Any) -> None:
    req = ClientRequest('post', URL('http://python.org'), data={'hey': 'you'}, loop=loop)
    resp = await req.send(conn)
...
async def test_content_type_skip_auto_header_bytes(loop: Any, conn: Any) -> None:
    req = ClientRequest('post', URL('http://python.org'), data=b'hey you', skip_auto_headers={'Content-Type'}, loop=loop)
    resp = await req.send(conn)
    assert 'CONTENT-TYPE' not in req.headers
    resp.close()

async def test_content_type_skip_auto_header_form(loop: Any, conn: Any) -> None:
    req = ClientRequest('post', URL('http://python.org'), data={'hey': 'you'}, loop=loop, skip_auto_headers={'Content-Type'})
    resp = await req.send(conn)
    assert 'CONTENT-TYPE' not in req.headers
    resp.close()

async def test_content_type_auto_header_content_length_no_skip(loop: Any, conn: Any) -> None:
    with io.BytesIO(b'hey') as file_handle:
        req = ClientRequest('post', URL('http://python.org'), data=file_handle, skip_auto_headers={'Content-Length'}, loop=loop)
        resp = await req.send(conn)
        assert req.headers.get('CONTENT-LENGTH') == '3'
        resp.close()

async def test_urlencoded_formdata_charset(loop: Any, conn: Any) -> None:
    req = ClientRequest('post', URL('http://python.org'), data=aiohttp.FormData({'hey': 'you'}, charset='koi8-r'), loop=loop)
    await req.send(conn)
    assert 'application/x-www-form-urlencoded; charset=koi8-r' == req.headers.get('CONTENT-TYPE')
...
async def test_formdata_boundary_from_headers(loop: Any, conn: Any) -> None:
    ...
    with file_path.open('rb') as f:
        req = ClientRequest('post', URL('http://python.org'), data={'aiohttp.png': f}, headers={'Content-Type': f'multipart/form-data; boundary={boundary}'}, loop=loop)
        await req.send(conn)
...
async def test_post_data(loop: Any, conn: Any) -> None:
    for meth in ClientRequest.POST_METHODS:
        req = ClientRequest(meth, URL('http://python.org/'), data={'life': '42'}, loop=loop)
        resp = await req.send(conn)
        assert '/' == req.url.path
...
async def test_pass_falsy_data_file(loop: Any, tmp_path: Any) -> None:
    ...
    req = ClientRequest('post', URL('http://python.org/'), data=testfile, skip_auto_headers=skip, loop=loop)
...
async def test_bytes_data(loop: Any, conn: Any) -> None:
    for meth in ClientRequest.POST_METHODS:
        req = ClientRequest(meth, URL('http://python.org/'), data=b'binary data', loop=loop)
        resp = await req.send(conn)
        assert '/' == req.url.path
...
async def test_content_encoding(loop: Any, conn: Any) -> None:
    req = ClientRequest('post', URL('http://python.org/'), data='foo', compress='deflate', loop=loop)
...
async def test_content_encoding_dont_set_headers_if_no_body(loop: Any, conn: Any) -> None:
    req = ClientRequest('post', URL('http://python.org/'), compress='deflate', loop=loop)
...
async def test_content_encoding_header(loop: Any, conn: Any) -> None:
    req = ClientRequest('post', URL('http://python.org/'), data='foo', headers={'Content-Encoding': 'deflate'}, loop=loop)
    with mock.patch('aiohttp.client_reqrep.StreamWriter') as m_writer:
        m_writer.return_value.write_headers = make_mocked_coro()
        resp = await req.send(conn)
...
async def test_compress_and_content_encoding(loop: Any, conn: Any) -> None:
    with pytest.raises(ValueError):
        ClientRequest('post', URL('http://python.org/'), data='foo', headers={'content-encoding': 'deflate'}, compress='deflate', loop=loop)

async def test_chunked(loop: Any, conn: Any) -> None:
    req = ClientRequest('post', URL('http://python.org/'), headers={'TRANSFER-ENCODING': 'gzip'}, loop=loop)
    resp = await req.send(conn)
...
async def test_chunked2(loop: Any, conn: Any) -> None:
    req = ClientRequest('post', URL('http://python.org/'), headers={'Transfer-encoding': 'chunked'}, loop=loop)
    resp = await req.send(conn)
...
async def test_chunked_length(loop: Any, conn: Any) -> None:
    with pytest.raises(ValueError):
        ClientRequest('post', URL('http://python.org/'), headers={'CONTENT-LENGTH': '1000'}, chunked=True, loop=loop)

async def test_chunked_transfer_encoding(loop: Any, conn: Any) -> None:
    with pytest.raises(ValueError):
        ClientRequest('post', URL('http://python.org/'), headers={'TRANSFER-ENCODING': 'chunked'}, chunked=True, loop=loop)
...
async def test_precompressed_data_stays_intact(loop: Any) -> None:
    ...
    req = ClientRequest('post', URL('http://python.org/'), data=data, headers={'CONTENT-ENCODING': 'deflate'}, compress=False, loop=loop)
...
async def test_file_upload_force_chunked(loop: Any) -> None:
    file_path = pathlib.Path(__file__).parent / 'aiohttp.png'
    with file_path.open('rb') as f:
        req = ClientRequest('post', URL('http://python.org/'), data=f, chunked=True, loop=loop)
        assert req.chunked
...
async def test_data_file(loop: Any, buf: Any, conn: Any) -> None:
    with io.BufferedReader(io.BytesIO(b'*' * 2)) as file_handle:
        req = ClientRequest('POST', URL('http://python.org/'), data=file_handle, loop=loop)
        assert req.chunked
...
async def test_data_stream_continue(loop: Any, buf: Any, conn: Any) -> None:
    ...
    async def gen():
        yield b'binary data'
        yield b' result'
    req = ClientRequest('POST', URL('http://python.org/'), data=gen(), expect100=True, loop=loop)
    assert req.chunked
...
async def test_bad_version(loop: Any, conn: Any) -> None:
    req = ClientRequest('GET', URL('http://python.org'), loop=loop, headers={'Connection': 'Close'}, version=('1', '1\r\nInjected-Header: not allowed'))
        ...
        def read(self, decode=False):
            if False:
                while True:
                    i = 10
            return 'customized!'
    req = ClientRequest('GET', URL('http://python.org/'), response_class=CustomResponse, loop=loop)
    resp = await req.send(conn)
...
async def test_custom_req_rep(loop: Any, create_mocked_conn: Any) -> None:
    ...
    class CustomRequest(ClientRequest):
        ...
        async def send(self, conn):
            resp = self.response_class(self.method, self.url, writer=self._writer, continue100=self._continue, timer=self._timer, request_info=self.request_info, traces=self._traces, loop=self.loop, session=self._session)
            self.response = resp
            nonlocal called
            called = True
            return resp
    ...
    session = aiohttp.ClientSession(request_class=CustomRequest, response_class=CustomResponse, connector=connector)
