...
def test_reduce_different_name(ray_start_distributed_2_nodes_4_gpus, group_name, dst_rank):
    if False:
        return 10
    world_size = 4
...
@pytest.mark.parametrize('dst_rank', [0, 1, 2, 3])
def test_reduce_different_array_size(ray_start_distributed_2_nodes_4_gpus, array_size, dst_rank):
    if False:
        return 10
    world_size = 4
...
def test_reduce_different_op(ray_start_distributed_2_nodes_4_gpus, dst_rank):
    ...
    results = ray.get([a.do_reduce.remote(dst_rank=dst_rank, op=ReduceOp.PRODUCT) for a in actors])
    for i in range(world_size):
        if i == dst_rank:
    ...
    ray.wait([a.set_buffer.remote(cp.ones(10, dtype=cp.float32) * (i + 2)) for (i, a) in enumerate(actors)])
    results = ray.get([a.do_reduce.remote(dst_rank=dst_rank, op=ReduceOp.MIN) for a in actors])
    for i in range(world_size):
        if i == dst_rank:
            ...
            assert (results[i] == cp.ones((10,), dtype=cp.float32) * (i + 2)).all()
    ray.wait([a.set_buffer.remote(cp.ones(10, dtype=cp.float32) * (i + 2)) for (i, a) in enumerate(actors)])
    results = ray.get([a.do_reduce.remote(dst_rank=dst_rank, op=ReduceOp.MAX) for a in actors])
    for i in range(world_size):
        if i == dst_rank:
