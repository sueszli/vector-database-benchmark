...
from aiogram.webhook.aiohttp_server import SimpleRequestHandler, TokenBasedRequestHandler, ip_filter_middleware, setup_application
...
class TestAiohttpServer:
    ...
    async def test_middleware(self, aiohttp_client):
        ...
        resp = await client.post('/webhook', headers={'X-Forwarded-For': '149.154.167.220,10.111.0.2'})
        assert resp.status == 200

class TestSimpleRequestHandler:

    async def make_reqest(self, client: TestClient, text: str='test'):
        return await client.post('/webhook', json={'update_id': 0, 'message': {'message_id': 0, 'from': {'id': 42, 'first_name': 'Test', 'is_bot': False}, 'chat': {'id': 42, 'is_bot': False, 'type': 'private'}, 'date': int(time.time()), 'text': text}})
    ...
    async def test_reply_into_webhook_file(self, bot: MockedBot, aiohttp_client):
        ...
        handler = SimpleRequestHandler(dispatcher=dp, bot=bot, handle_in_background=False)
    ...
    async def test_reply_into_webhook_background(self, bot: MockedBot, aiohttp_client):
        ...
        def handle_message(msg: Message):
            ...
            handler_event.set()
            return msg.answer(text='PASS')
        handler = SimpleRequestHandler(dispatcher=dp, bot=bot, handle_in_background=True)
        handler.register(app, path='/webhook')
        client: TestClient = await aiohttp_client(app)
        with patch('aiogram.dispatcher.dispatcher.Dispatcher.silent_call_request', new_callable=AsyncMock) as mocked_silent_call_request:
            handler_event.clear()
            resp = await self.make_reqest(client=client)
            assert resp.status == 200
...
class TestTokenBasedRequestHandler:
    ...
    async def test_resolve_bot(self):
        ...
        assert bot1.id == 42
        bot2 = await handler.resolve_bot(request=FakeRequest(match_info={'bot_token': '1337:TEST'}))
        assert bot2.id == 1337
        bot3 = await handler.resolve_bot(request=FakeRequest(match_info={'bot_token': '1337:TEST'}))
        assert bot3.id == 1337
        assert bot2 == bot3
