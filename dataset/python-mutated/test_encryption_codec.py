...
async def test_payloads_are_encrypted():
    ...
    client = await Client.connect(f'{settings.TEMPORAL_HOST}:{settings.TEMPORAL_PORT}', namespace=settings.TEMPORAL_NAMESPACE, data_converter=dataclasses.replace(temporalio.converter.default(), payload_codec=codec))
    workflow_id = uuid.uuid4()
    input_str = str(uuid.uuid4())
    no_op_result_str = f'OK - {input_str}'
    inputs = NoOpInputs(arg=input_str, batch_export_id='123', team_id=1)
    expected_results = (no_op_result_str, {'arg': input_str}, dataclasses.asdict(inputs))
    async with Worker(client, task_queue=settings.TEMPORAL_TASK_QUEUE, workflows=[NoOpWorkflow], activities=[noop_activity], workflow_runner=UnsandboxedWorkflowRunner()) as worker:
        handle = await client.start_workflow(NoOpWorkflow.run, inputs, id=f'workflow-{workflow_id}', task_queue=worker.task_queue)
