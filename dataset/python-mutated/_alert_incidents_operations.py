...
def build_get_request(scope: str, alert_id: str, alert_incident_id: str, **kwargs: Any) -> HttpRequest:
    ...
    _url = kwargs.pop('template_url', '/{scope}/providers/Microsoft.Authorization/roleManagementAlerts/{alertId}/alertIncidents/{alertIncidentId}')
    path_format_arguments = {'scope': _SERIALIZER.url('scope', scope, 'str', skip_quote=True), 'alertId': _SERIALIZER.url('alert_id', alert_id, 'str', skip_quote=True), 'alertIncidentId': _SERIALIZER.url('alert_incident_id', alert_incident_id, 'str', skip_quote=True)}
...
def build_list_for_scope_request(scope: str, alert_id: str, **kwargs: Any) -> HttpRequest:
    ...
    api_version: str = kwargs.pop('api_version', _params.pop('api-version', '2022-08-01-preview'))
    accept = _headers.pop('Accept', 'application/json')
    _url = kwargs.pop('template_url', '/{scope}/providers/Microsoft.Authorization/roleManagementAlerts/{alertId}/alertIncidents')
...
def build_remediate_request(scope: str, alert_id: str, alert_incident_id: str, **kwargs: Any) -> HttpRequest:
    ...
    api_version: str = kwargs.pop('api_version', _params.pop('api-version', '2022-08-01-preview'))
    accept = _headers.pop('Accept', 'application/json')
    _url = kwargs.pop('template_url', '/{scope}/providers/Microsoft.Authorization/roleManagementAlerts/{alertId}/alertIncidents/{alertIncidentId}/remediate')
    path_format_arguments = {'scope': _SERIALIZER.url('scope', scope, 'str', skip_quote=True), 'alertId': _SERIALIZER.url('alert_id', alert_id, 'str', skip_quote=True), 'alertIncidentId': _SERIALIZER.url('alert_incident_id', alert_incident_id, 'str', skip_quote=True)}
    _url: str = _format_url_section(_url, **path_format_arguments)
    _params['api-version'] = _SERIALIZER.query('api_version', api_version, 'str')
    _headers['Accept'] = _SERIALIZER.header('accept', accept, 'str')
...
class AlertIncidentsOperations:
    ...
    def get(self, scope: str, alert_id: str, alert_incident_id: str, **kwargs: Any) -> _models.AlertIncident:
        ...
        request = build_get_request(scope=scope, alert_id=alert_id, alert_incident_id=alert_incident_id, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        ...
        return deserialized
    get.metadata = {'url': '/{scope}/providers/Microsoft.Authorization/roleManagementAlerts/{alertId}/alertIncidents/{alertIncidentId}'}
    ...
    def list_for_scope(self, scope: str, alert_id: str, **kwargs: Any) -> Iterable['_models.AlertIncident']:
        ...
        def prepare_request(next_link=None):
            if False:
                print('Hello World!')
            if not next_link:
                request = build_list_for_scope_request(scope=scope, alert_id=alert_id, api_version=api_version, template_url=self.list_for_scope.metadata['url'], headers=_headers, params=_params)
                request = _convert_request(request)
                request.url = self._client.format_url(request.url)
            else:
    ...
    list_for_scope.metadata = {'url': '/{scope}/providers/Microsoft.Authorization/roleManagementAlerts/{alertId}/alertIncidents'}

    @distributed_trace
    def remediate(self, scope: str, alert_id: str, alert_incident_id: str, **kwargs: Any) -> None:
        if False:
            print('Hello World!')
        ...
        request = build_remediate_request(scope=scope, alert_id=alert_id, alert_incident_id=alert_incident_id, api_version=api_version, template_url=self.remediate.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = self._client._pipeline.run(request, stream=_stream, **kwargs)
    ...
    remediate.metadata = {'url': '/{scope}/providers/Microsoft.Authorization/roleManagementAlerts/{alertId}/alertIncidents/{alertIncidentId}/remediate'}