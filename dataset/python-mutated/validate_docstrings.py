...
IGNORE_VALIDATION = {'Styler.env', 'Styler.template_html', 'Styler.template_html_style', 'Styler.template_html_table', 'Styler.template_latex', 'Styler.template_string', 'Styler.loader', 'errors.InvalidComparison', 'errors.LossySetitemError', 'errors.NoBufferPresent', 'errors.IncompatibilityWarning', 'errors.PyperclipException', 'errors.PyperclipWindowsException'}
PRIVATE_CLASSES = ['NDFrame', 'IndexOpsMixin']
ERROR_MSGS = {'GL04': 'Private classes ({mentioned_private_classes}) should not be mentioned in public docstrings', 'GL05': "Use 'array-like' rather than 'array_like' in docstrings.", 'SA05': '{reference_name} in `See Also` section does not need `pandas` prefix, use {right_reference} instead.', 'EX02': 'Examples do not pass tests:\n{doctest_log}', 'EX03': 'flake8 error: line {line_number}, col {col_number}: {error_code} {error_message}', 'EX04': 'Do not import {imported_library}, as it is imported automatically for the examples (numpy as np, pandas as pd)'}
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--ignore_functions', nargs='*', help='function or method to not validate (e.g. pandas.DataFrame.head). Inverse of the `function` argument.')
    args = argparser.parse_args()
    sys.exit(main(args.function, args.prefix, args.errors.split(',') if args.errors else None, args.format, args.ignore_deprecated, args.ignore_functions))