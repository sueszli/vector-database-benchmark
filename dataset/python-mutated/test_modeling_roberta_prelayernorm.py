...
if is_torch_available():
    import torch
    from transformers import RobertaPreLayerNormForCausalLM, RobertaPreLayerNormForMaskedLM, RobertaPreLayerNormForMultipleChoice, RobertaPreLayerNormForQuestionAnswering, RobertaPreLayerNormForSequenceClassification, RobertaPreLayerNormForTokenClassification, RobertaPreLayerNormModel
    from transformers.models.roberta_prelayernorm.modeling_roberta_prelayernorm import ROBERTA_PRELAYERNORM_PRETRAINED_MODEL_ARCHIVE_LIST, RobertaPreLayerNormEmbeddings, create_position_ids_from_input_ids
...
class RobertaPreLayerNormModelTester:
    ...
    def get_config(self):
        if False:
            while True:
                i = 10
        return RobertaPreLayerNormConfig(vocab_size=self.vocab_size, hidden_size=self.hidden_size, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, hidden_act=self.hidden_act, hidden_dropout_prob=self.hidden_dropout_prob, attention_probs_dropout_prob=self.attention_probs_dropout_prob, max_position_embeddings=self.max_position_embeddings, type_vocab_size=self.type_vocab_size, initializer_range=self.initializer_range)
...
class RobertaPreLayerNormModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (RobertaPreLayerNormForCausalLM, RobertaPreLayerNormForMaskedLM, RobertaPreLayerNormModel, RobertaPreLayerNormForSequenceClassification, RobertaPreLayerNormForTokenClassification, RobertaPreLayerNormForMultipleChoice, RobertaPreLayerNormForQuestionAnswering) if is_torch_available() else ()
    all_generative_model_classes = (RobertaPreLayerNormForCausalLM,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': RobertaPreLayerNormModel, 'fill-mask': RobertaPreLayerNormForMaskedLM, 'question-answering': RobertaPreLayerNormForQuestionAnswering, 'text-classification': RobertaPreLayerNormForSequenceClassification, 'text-generation': RobertaPreLayerNormForCausalLM, 'token-classification': RobertaPreLayerNormForTokenClassification, 'zero-shot': RobertaPreLayerNormForSequenceClassification} if is_torch_available() else {}
    fx_compatible = False
...
class RobertaPreLayerNormModelIntegrationTest(TestCasePlus):
    ...
    def test_inference_masked_lm(self):
        ...
        EXPECTED_SLICE = torch.tensor([[[40.488, 18.0199, -5.2367], [-1.8877, -4.0885, 10.7085], [-2.2613, -5.611, 7.2665]]])
    ...
    def test_inference_no_head(self):
        ...
        with torch.no_grad():
            output = model(input_ids)[0]
        EXPECTED_SLICE = torch.tensor([[[0.0208, -0.0356, 0.0237], [-0.1569, -0.0411, -0.2626], [0.1879, 0.0125, -0.0089]]])
        self.assertTrue(torch.allclose(output[:, :3, :3], EXPECTED_SLICE, atol=0.0001))