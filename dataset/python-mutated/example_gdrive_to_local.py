...
with DAG(DAG_ID, start_date=datetime(2021, 1, 1), schedule='@once', catchup=False, tags=['example', 'gdrive']) as dag:
    ...
    detect_file = GoogleDriveFileExistenceSensor(task_id='detect_file', gcp_conn_id=CONNECTION_ID, folder_id='', file_name=DRIVE_FILE_NAME)
    download_from_gdrive_to_local = GoogleDriveToLocalOperator(task_id='download_from_gdrive_to_local', gcp_conn_id=CONNECTION_ID, folder_id='', file_name=DRIVE_FILE_NAME, output_file=OUTPUT_FILE)
    ...
    [create_bucket >> upload_file >> copy_single_file, create_temp_gcp_connection_task] >> detect_file >> download_from_gdrive_to_local >> remove_file_from_drive_task >> [delete_bucket, delete_temp_gcp_connection_task]
