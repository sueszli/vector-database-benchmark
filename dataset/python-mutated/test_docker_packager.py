...
from prefect.utilities.dockerutils import get_prefect_image_name, silence_docker_warnings
from . import howdy
pytestmark = pytest.mark.skip(reason='These tests are incredibly brittle and causing noise.')
...
async def test_packaging_a_flow_to_local_docker_daemon(prefect_base_image: str):
    packager = DockerPackager(base_image=prefect_base_image, python_environment=PythonEnvironment(python_version='3.9', pip_requirements=['requests==2.28.0']))
...
async def test_packaging_a_flow_to_registry(prefect_base_image: str, registry: str):
    packager = DockerPackager(base_image=prefect_base_image, python_environment=PythonEnvironment(python_version='3.9', pip_requirements=['requests==2.28.0']), registry_url=registry)
    manifest = await packager.package(howdy)
    assert isinstance(manifest, DockerPackageManifest)
...
@pytest.mark.service('docker')
async def test_packaging_a_flow_to_registry_without_scheme(prefect_base_image: str, registry: str):
    packager = DockerPackager(base_image=prefect_base_image, python_environment=PythonEnvironment(python_version='3.9', pip_requirements=['requests==2.28.0']), registry_url='localhost:5555')
    manifest = await packager.package(howdy)
    assert isinstance(manifest, DockerPackageManifest)
    assert manifest.image.startswith('localhost:5555/howdy:')
    assert manifest.image_flow_location == '/flow.py'
...
async def test_unpackaging_outside_container(howdy_context: Path):
    packager = DockerPackager(dockerfile=howdy_context / 'Dockerfile', image_flow_location=str(howdy_context / 'howdy.py'))
    manifest = await packager.package(howdy)
...
async def test_unpackaging_inside_container(prefect_base_image: str, docker: DockerClient):
    packager = DockerPackager(base_image=prefect_base_image, python_environment=PythonEnvironment(python_version='3.9', pip_requirements=['requests==2.28.0']))
    manifest = await packager.package(howdy)
...
async def test_custom_dockerfile_unpackaging(howdy_context: Path, docker: DockerClient):
    packager = DockerPackager(dockerfile=howdy_context / 'Dockerfile', image_flow_location='/howdy.py')
    manifest = await packager.package(howdy)
    assert_unpackaged_flow_works(docker, manifest)
