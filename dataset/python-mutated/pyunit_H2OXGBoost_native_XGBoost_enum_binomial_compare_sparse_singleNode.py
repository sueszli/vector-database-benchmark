...
def comparison_test():
    ...
    if len(ret.nodes) == 1:
        ...
        trainFile = pyunit_utils.genTrainFrame(nrows, 0, enumCols=enumCols, enumFactors=factorL, miscfrac=0.1, randseed=dataSeed)
        print(trainFile)
        myX = trainFile.names
        y = 'response'
        myX.remove(y)
        ...
        pyunit_utils.summarize_metrics_binomial(h2o_metrics, xgboost_metrics, ['auc', 'aucpr'], tolerance=0.001)
    else:
