...
from flask import Blueprint, Flask, Request, copy_current_request_context, request, send_file
from flask_cors import CORS
from flask_sock import Sock
from simple_websocket import Server as WebSocket
from werkzeug.serving import BaseWSGIServer, make_server
import reactpy
from reactpy.backend._common import ASSETS_PATH, MODULES_PATH, PATH_PREFIX, STREAM_PATH, CommonOptions, read_client_index_html, safe_client_build_dir_path, safe_web_modules_dir_path
...
def configure(app: Flask, component: RootComponentConstructor, options: Options | None=None) -> None:
    ...
    spa_bp = Blueprint(f'reactpy_spa_{id(app)}', __name__, url_prefix=options.url_prefix)
...
async def serve_development_app(app: Flask, host: str, port: int, started: asyncio.Event | None=None) -> None:
...
def _dispatch_in_thread(websocket: WebSocket, path: str, component: ComponentType, send: Callable[[Any], None], recv: Callable[[], Any | None]) -> NoReturn:
    if False:
        return 10
    dispatch_thread_info_created = ThreadEvent()
    dispatch_thread_info_ref: reactpy.Ref[_DispatcherThreadInfo | None] = reactpy.Ref(None)
    ...
    def run_dispatcher() -> None:
        ...
        async def main() -> None:
            ...
            await serve_layout(reactpy.Layout(ConnectionContext(component, value=Connection(scope=request.environ, location=Location(pathname=f'/{path}', search=f'?{search}' if search else ''), carrier=_FlaskCarrier(request, websocket)))), send_coro, async_recv_queue.get)
        main_future = asyncio.ensure_future(main(), loop=loop)
        dispatch_thread_info_ref.current = _DispatcherThreadInfo(dispatch_loop=loop, dispatch_future=main_future, thread_send_queue=thread_send_queue, async_recv_queue=async_recv_queue)
    ...
    try:
        while True:
            value = recv()
            dispatch_thread_info.dispatch_loop.call_soon_threadsafe(dispatch_thread_info.async_recv_queue.put_nowait, value)
    finally:
        dispatch_thread_info.dispatch_loop.call_soon_threadsafe(dispatch_thread_info.dispatch_future.cancel)
