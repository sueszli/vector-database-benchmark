...
class SelectionListApp(App[None]):
    ...
    def _record(self, event: OptionList.OptionMessage | SelectionList.SelectionMessage | SelectionList.SelectedChanged) -> None:
        if False:
            print('Hello World!')
        assert event.control == self.query_one(SelectionList)
        self.messages.append((event.__class__.__name__, event.selection_index if isinstance(event, SelectionList.SelectionMessage) else None))
...
async def test_toggle() -> None:
    ...
    async with SelectionListApp().run_test() as pilot:
        ...
        await pilot.pause()
        assert pilot.app.messages == [('SelectionHighlighted', 0), ('SelectedChanged', None)]
...
async def test_toggle_via_user() -> None:
    ...
    async with SelectionListApp().run_test() as pilot:
        ...
        await pilot.pause()
        assert pilot.app.messages == [('SelectionHighlighted', 0), ('SelectedChanged', None), ('SelectionToggled', 0)]
...
async def test_deselect() -> None:
    ...
    async with SelectionListApp().run_test() as pilot:
        ...
        await pilot.pause()
        assert pilot.app.messages == [('SelectionHighlighted', 0), ('SelectedChanged', None), ('SelectedChanged', None)]
