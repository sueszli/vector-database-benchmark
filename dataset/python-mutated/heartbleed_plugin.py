...
from sslyze.plugins.plugin_base import ScanCommandResult, ScanCommandImplementation, ScanJob, ScanCommandExtraArgument, ScanCommandWrongUsageError, ScanCommandCliConnector, ScanJobResult
...
class _HeartbleedCliConnector(ScanCommandCliConnector[HeartbleedScanResult, None]):
    ...
    def result_to_console_output(cls, result: HeartbleedScanResult) -> List[str]:
        ...
        heartbleed_txt = 'VULNERABLE - Server is vulnerable to Heartbleed' if result.is_vulnerable_to_heartbleed else 'OK - Not vulnerable to Heartbleed'
...
class HeartbleedImplementation(ScanCommandImplementation[HeartbleedScanResult, None]):
    ...
    @classmethod
    def result_for_completed_scan_jobs(cls, server_info: ServerConnectivityInfo, scan_job_results: List[ScanJobResult]) -> HeartbleedScanResult:
        if False:
            while True:
                i = 10
...
def _do_handshake_with_heartbleed(self):
    if False:
        i = 10
        return i + 15
    ...
    try:
        ...
        while len_to_read:
            ...
            len_to_read = self._network_bio.pending()
    payload = TlsHeartbeatRequestRecord.from_parameters(tls_version=tls_parser.record_protocol.TlsVersionEnum[self._ssl_version.name], heartbeat_data=b'\x01' * 16381).to_bytes()
    payload += TlsHeartbeatRequestRecord.from_parameters(tls_parser.record_protocol.TlsVersionEnum[self._ssl_version.name], heartbeat_data=b'\x01\x00\x00').to_bytes()
