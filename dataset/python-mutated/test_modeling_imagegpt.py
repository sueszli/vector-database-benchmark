...
if is_torch_available():
    import torch
    from transformers import IMAGEGPT_PRETRAINED_MODEL_ARCHIVE_LIST, ImageGPTForCausalImageModeling, ImageGPTForImageClassification, ImageGPTModel
...
class ImageGPTModelTester:
    ...
    def prepare_config_and_inputs_for_decoder(self):
        if False:
            for i in range(10):
                print('nop')
        (config, pixel_values, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, token_labels, choice_labels) = self.prepare_config_and_inputs()
        encoder_hidden_states = floats_tensor([self.batch_size, self.seq_length, self.hidden_size])
        encoder_attention_mask = ids_tensor([self.batch_size, self.seq_length], vocab_size=2)
        return (config, pixel_values, input_mask, head_mask, token_type_ids, sequence_labels, token_labels, choice_labels, encoder_hidden_states, encoder_attention_mask)
    ...
    def create_and_check_imagegpt_for_image_classification(self, config, pixel_values, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, *args):
        if False:
            i = 10
            return i + 15
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        (config, pixel_values, input_mask, head_mask, token_type_ids, mc_token_ids, sequence_labels, token_labels, choice_labels) = config_and_inputs
        inputs_dict = {'pixel_values': pixel_values, 'token_type_ids': token_type_ids, 'head_mask': head_mask}
        return (config, inputs_dict)

@require_torch
class ImageGPTModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (ImageGPTForCausalImageModeling, ImageGPTForImageClassification, ImageGPTModel) if is_torch_available() else ()
    all_generative_model_classes = (ImageGPTForCausalImageModeling,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': ImageGPTModel, 'image-classification': ImageGPTForImageClassification} if is_torch_available() else {}
...
class ImageGPTModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_causal_lm_head(self):
        ...
        expected_slice = torch.tensor([[2.3445, 2.6889, 2.7313], [1.053, 1.2416, 0.5699], [0.2205, 0.7749, 0.3953]]).to(torch_device)
