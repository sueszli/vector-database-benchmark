...
PNG_DATA = unhexlify('89504e470d0a1a0a0000000d4948445200000001000000010100000000376ef9240000001049444154789c626001000000ffff03000006000557bfabd40000000049454e44ae426082')
...
async def test_get_channels(rest_api, add_fake_torrents_channels, add_subscribed_and_not_downloaded_channel, mock_dlmgr, metadata_store):
    """
...
async def test_create_channel(rest_api, metadata_store):
    ...
    await do_request(rest_api, 'channels/mychannel/0/channels', request_type='POST', post_data={'name': 'foobar'}, expected_code=200)
    with db_session:
...
async def test_get_channel_description(rest_api, metadata_store):
    ...
    with db_session:
        ...
        channel_description = metadata_store.ChannelDescription(origin_id=chan.id_, json_text=json.dumps({'description_text': descr_txt}))
    response_dict = await do_request(rest_api, f'channels/{hexlify(chan.public_key)}/{chan.id_}/description', expected_code=200)
...
async def test_put_new_channel_description(rest_api, metadata_store):
    ...
    with db_session:
        chan = metadata_store.ChannelMetadata.create_channel(title='bla')
    response_dict = await do_request(rest_api, f'channels/{hexlify(chan.public_key)}/{chan.id_}/description', request_type='PUT', post_data={'description_text': new_descr}, expected_code=200)
    assert response_dict == {'description_text': new_descr}
    updated_descr = 'foobar'
    response_dict = await do_request(rest_api, f'channels/{hexlify(chan.public_key)}/{chan.id_}/description', request_type='PUT', post_data={'description_text': updated_descr}, expected_code=200)
...
async def test_get_channel_contents_by_type(metadata_store, my_channel, mock_dlmgr_get_download, rest_api):
    ...
    json_dict = await do_request(rest_api, 'channels/%s/%d?metadata_type=%d&metadata_type=%d' % (hexlify(my_channel.public_key), my_channel.id_, COLLECTION_NODE, REGULAR_TORRENT), expected_code=200)
    assert len(json_dict['results']) == 10
    assert 'status' in json_dict['results'][0]
...
async def test_add_torrents_no_channel(metadata_store, my_channel, rest_api):
    ...
    with db_session:
        ...
        my_chan.delete()
    await do_request(rest_api, f'channels/{hexlify(my_channel.public_key)}/{my_channel.id_}/torrents', request_type='PUT', expected_code=404)
...
async def test_add_torrents_no_dir(my_channel, rest_api):
    ...
    await do_request(rest_api, f'channels/{hexlify(my_channel.public_key)}/{my_channel.id_}/torrents', request_type='PUT', post_data=post_params, expected_code=400)
...
async def test_add_torrents_from_dir(my_channel, state_dir, rest_api):
    ...
    await do_request(rest_api, f'channels/{hexlify(my_channel.public_key)}/{my_channel.id_}/torrents', request_type='PUT', post_data=post_params)
...
async def test_add_invalid_torrent(my_channel, rest_api):
    ...
    await do_request(rest_api, f'channels/{hexlify(my_channel.public_key)}/{my_channel.id_}/torrents', request_type='PUT', post_data=post_params, expected_code=500)
...
async def test_add_torrent_duplicate(my_channel, rest_api):
    ...
    with open(TORRENT_UBUNTU_FILE, 'rb') as torrent_file:
        ...
        await do_request(rest_api, f'channels/{hexlify(my_channel.public_key)}/{my_channel.id_}/torrents', request_type='PUT', post_data=post_params, expected_code=200)
...
async def test_add_torrent_from_magnet_error(my_channel, mock_dlmgr, rest_api):
    ...
    response = await do_request(rest_api, f'channels/{hexlify(my_channel.public_key)}/{my_channel.id_}/torrents', request_type='PUT', post_data=post_params, expected_code=400)
...
async def test_get_torrents_ffa_channel(my_channel, mock_dlmgr_get_download, rest_api, metadata_store):
    ...
    with db_session:
        ...
        metadata_store.TorrentMetadata(public_key=b'', id_=333333, origin_id=channel.id_, title='torrent', infohash=random_infohash())
...
async def test_put_channel_thumbnail(rest_api, metadata_store):
    ...
    with db_session:
        chan = metadata_store.ChannelMetadata.create_channel(title='bla')
    await do_request(rest_api, f'channels/{hexlify(chan.public_key)}/{chan.id_}/thumbnail', request_type='PUT', headers={'Content-Type': 'image/png'}, json_response=False, post_data=PNG_DATA, expected_code=201)
    with db_session:
    ...
    await do_request(rest_api, f'channels/{hexlify(chan.public_key)}/{chan.id_}/thumbnail', request_type='PUT', headers={'Content-Type': 'image/foo'}, json_response=False, post_data=b'ffff', expected_code=201)
    with db_session:
        obj = metadata_store.ChannelThumbnail.get(public_key=chan.public_key, origin_id=chan.id_)
...
async def test_get_channel_thumbnail(rest_api, metadata_store):
    ...
    with db_session:
        chan = metadata_store.ChannelMetadata.create_channel(title='bla')
        metadata_store.ChannelThumbnail(public_key=chan.public_key, origin_id=chan.id_, binary_data=PNG_DATA, data_type='image/png')
...
async def test_get_my_channel_tags(metadata_store, mock_dlmgr_get_download, my_channel, rest_api):
    """
    Test whether tags are correctly returned over the REST API
    """
    with db_session:
        json_dict = await do_request(rest_api, 'channels/%s/%d?metadata_type=%d' % (hexlify(my_channel.public_key), my_channel.id_, REGULAR_TORRENT), expected_code=200)
    assert len(json_dict['results']) == 9
    for item in json_dict['results']:
        assert len(item['statements']) >= 2

async def test_get_my_channel_tags_xxx(metadata_store, tribler_db, mock_dlmgr_get_download, my_channel, rest_api):
    """
    ...
    json_dict = await do_request(rest_api, 'channels/%s/%d?metadata_type=%d&hide_xxx=1' % (hexlify(my_channel.public_key), chan.id_, REGULAR_TORRENT), expected_code=200)
    assert len(json_dict['results']) == 1
    print(json_dict)
