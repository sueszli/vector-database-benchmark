...
class PServeCommand:
    description = "    This command serves a web application that uses a PasteDeploy\n    configuration file for the server and application.\n\n    You can also include variable assignments like 'http_port=8080'\n    and then use %(http_port)s in your config files.\n    "
    default_verbosity = 1
    ...
    parser.add_argument('-n', '--app-name', dest='app_name', metavar='NAME', help='Load the named application (default main)')
    parser.add_argument('-s', '--server', dest='server', metavar='SERVER_TYPE', help='Use the named server.')
    parser.add_argument('--server-name', dest='server_name', metavar='SECTION_NAME', help='Use the named server as defined in the configuration file (default: main)')
    parser.add_argument('--reload', dest='reload', action='store_true', help='Use auto-restart file monitor')
    parser.add_argument('--reload-interval', dest='reload_interval', default=1, help='Seconds between checking files (low number can cause significant CPU usage)')
    parser.add_argument('-b', '--browser', dest='browser', action='store_true', help="Open a web browser to the server url. The server url is determined from the 'open_url' setting in the 'pserve' section of the configuration file.")
    parser.add_argument('-v', '--verbose', default=default_verbosity, dest='verbose', action='count', help='Set verbose level (default ' + str(default_verbosity) + ')')
    parser.add_argument('-q', '--quiet', action='store_const', const=0, dest='verbose', help='Suppress verbose output')
