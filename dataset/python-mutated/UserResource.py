...
class UserResource(DatabaseResource):
    ...
    async def create(self, payload, user, **kwargs):
        ...
        if username:
            ...
            if user:
                error.update({'message': f'Account with same username is already taken: {username}.'})
                raise ApiError(error)
        if len(password) < 8:
        ...
        resource = super().create(extract(payload, ['avatar', 'email', 'first_name', 'last_name', 'password_hash', 'password_salt', 'roles', 'roles_new', 'username']), user, **kwargs)
...
def __select_roles(resource, arr):
    if False:
        while True:
            i = 10
    return [r for r in arr if r.user_id == resource.id]
UserResource.register_collective_loader('permissions', load=__load_permissions, select=__select_permissions)
UserResource.register_collective_loader('roles_new', load=__load_roles, select=__select_roles)