...
def feather_data_set(filepath_feather, load_args, fs_args):
    if False:
        for i in range(10):
            print('nop')
    return FeatherDataSet(filepath=filepath_feather, load_args=load_args, fs_args=fs_args)
...
def versioned_feather_data_set(filepath_feather, load_version, save_version):
    if False:
        i = 10
        return i + 15
    return FeatherDataSet(filepath=filepath_feather, version=Version(load_version, save_version))
...
class TestFeatherDataSet:
    ...
    def test_load_missing_file(self, feather_data_set):
        ...
        pattern = 'Failed while loading data from data set FeatherDataSet\\(.*\\)'
...
class TestFeatherDataSetVersioned:
    ...
    def test_version_str_repr(self, load_version, save_version):
        ...
        ds_versioned = FeatherDataSet(filepath=filepath, version=Version(load_version, save_version))
    ...
    def test_no_versions(self, versioned_feather_data_set):
        ...
        pattern = 'Did not find any versions for FeatherDataSet\\(.+\\)'
        with pytest.raises(DatasetError, match=pattern):
    ...
    def test_prevent_overwrite(self, versioned_feather_data_set, dummy_dataframe):
        ...
        versioned_feather_data_set.save(dummy_dataframe)
        pattern = "Save path \\'.+\\' for FeatherDataSet\\(.+\\) must not exist if versioning is enabled\\."
        with pytest.raises(DatasetError, match=pattern):
            versioned_feather_data_set.save(dummy_dataframe)
    ...
    def test_save_version_warning(self, versioned_feather_data_set, load_version, save_version, dummy_dataframe):
        if False:
            return 10
        'Check the warning when saving to the path that differs from\n        the subsequent load path.'
        pattern = f"Save version '{save_version}' did not match load version '{load_version}' for FeatherDataSet\\(.+\\)"
    ...
    def test_http_filesystem_no_versioning(self):
        ...
        with pytest.raises(DatasetError, match=pattern):
            FeatherDataSet(filepath='https://example.com/file.feather', version=Version(None, None))

    def test_versioning_existing_dataset(self, feather_data_set, versioned_feather_data_set, dummy_dataframe):
        if False:
            for i in range(10):
                print('nop')
        ...
        pattern = f'(?=.*file with the same name already exists in the directory)(?=.*{versioned_feather_data_set._filepath.parent.as_posix()})'
        with pytest.raises(DatasetError, match=pattern):
            versioned_feather_data_set.save(dummy_dataframe)
