...
from mage_ai.data_preparation.models.block.dbt.utils import get_source_name, get_source_table_name_for_block
...
class DBTBlockSQL(DBTBlock):
    ...
    async def metadata_async(self) -> Dict[str, Any]:
        ...
        projects = {project_name: {'target': target, 'targets': targets}}
        node_type = self.__node_type
        return {'dbt': {'block': {'snapshot': node_type == 'snapshot'}, 'project': project_name, 'projects': projects}}
