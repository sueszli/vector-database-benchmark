...
class CloudBatchJobFinishedTrigger(BaseTrigger):
    ...
    def __init__(self, job_name: str, project_id: str | None, location: str=DEFAULT_BATCH_LOCATION, gcp_conn_id: str='google_cloud_default', impersonation_chain: str | Sequence[str] | None=None, polling_period_seconds: float=10, timeout: float | None=None):
        if False:
            while True:
                i = 10
        super().__init__()
    ...
    async def run(self) -> AsyncIterator[TriggerEvent]:
        ...
        try:
            while timeout is None or timeout > 0:
                ...
                if status == JobStatus.State.SUCCEEDED:
                    yield TriggerEvent({'job_name': self.job_name, 'status': 'success', 'message': 'Job completed'})
                    return
                elif status == JobStatus.State.FAILED:
                    yield TriggerEvent({'job_name': self.job_name, 'status': 'error', 'message': f'Batch job with name {self.job_name} has failed its execution'})
                    return
                elif status == JobStatus.State.DELETION_IN_PROGRESS:
                    yield TriggerEvent({'job_name': self.job_name, 'status': 'deleted', 'message': f'Batch job with name {self.job_name} is being deleted'})
                    return
                else:
        ...
        yield TriggerEvent({'job_name': self.job_name, 'status': 'timed out', 'message': f'Batch job with name {self.job_name} timed out'})

    def _get_async_hook(self) -> CloudBatchAsyncHook:
        if False:
            for i in range(10):
                print('nop')
        return CloudBatchAsyncHook(gcp_conn_id=self.gcp_conn_id, impersonation_chain=self.impersonation_chain)