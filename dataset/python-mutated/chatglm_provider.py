...
class ChatGLMProvider(BaseModelProvider):
    ...
    def _get_fixed_model_list(self, model_type: ModelType) -> list[dict]:
        ...
        if model_type == ModelType.TEXT_GENERATION:
            return [{'id': 'chatglm2-6b', 'name': 'ChatGLM2-6B', 'mode': ModelMode.COMPLETION.value}, {'id': 'chatglm-6b', 'name': 'ChatGLM-6B', 'mode': ModelMode.COMPLETION.value}]
        else:
            return []
    ...
    def is_provider_credentials_valid_or_raise(cls, credentials: dict):
        ...
        try:
            credential_kwargs = {'endpoint_url': credentials['api_base']}
            llm = ChatGLM(max_token=10, **credential_kwargs)
            llm('ping')
    ...
    def get_provider_credentials(self, obfuscated: bool=False) -> dict:
        ...
        if self.provider.provider_type == ProviderType.CUSTOM.value:
            try:
                ...
                credentials = {'api_base': None}
            if credentials['api_base']:
                credentials['api_base'] = encrypter.decrypt_token(self.provider.tenant_id, credentials['api_base'])
                if obfuscated:
