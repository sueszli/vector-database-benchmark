...
class TestQueryAsync:
    ...
    async def test_first_and_last_slashes_trimmed_for_query_string_async(self):
        await self._set_up()
        created_collection = await self.created_db.create_container_if_not_exists(str(uuid.uuid4()), PartitionKey(path='/pk'))
    ...
    async def test_query_change_feed_with_pk_async(self):
        await self._set_up()
        created_collection = await self.created_db.create_container_if_not_exists('change_feed_test_' + str(uuid.uuid4()), PartitionKey(path='/pk'))
        partition_key = 'pk'
        ...
        query_iterable = created_collection.query_items_change_feed(is_start_from_beginning=True, partition_key=partition_key)
        iter_list = [item async for item in query_iterable]
        assert len(iter_list) == 0
        if 'Etag' in created_collection.client_connection.last_response_headers:
        ...
        for pageSize in [2, 100]:
            query_iterable = created_collection.query_items_change_feed(continuation=continuation2, max_item_count=pageSize, partition_key=partition_key)
        ...
        if 'Etag' in created_collection.client_connection.last_response_headers:
            ...
            pytest.fail('No Etag or etag found in last response headers')
        query_iterable = created_collection.query_items_change_feed(continuation=continuation3, is_start_from_beginning=True, partition_key=partition_key)
        iter_list = [item async for item in query_iterable]
        assert len(iter_list) == 0
    ...
    async def test_query_change_feed_with_pk_range_id_async(self):
        await self._set_up()
        created_collection = await self.created_db.create_container_if_not_exists('cf_test_' + str(uuid.uuid4()), PartitionKey(path='/pk'))
        ...
        if 'Etag' in created_collection.client_connection.last_response_headers:
            ...
            pytest.fail('No Etag or etag found in last response headers')
        query_iterable = created_collection.query_items_change_feed(is_start_from_beginning=True, **partition_param)
        iter_list = [item async for item in query_iterable]
        assert len(iter_list) == 0
        if 'Etag' in created_collection.client_connection.last_response_headers:
            continuation1 = created_collection.client_connection.last_response_headers['Etag']
        ...
        await created_collection.create_item(body=document_definition)
        for pageSize in [2, 100]:
            query_iterable = created_collection.query_items_change_feed(continuation=continuation2, max_item_count=pageSize, **partition_param)
            it = query_iterable.__aiter__()
            expected_ids = 'doc2.doc3.'
            actual_ids = ''
            async for item in it:
        ...
        if 'Etag' in created_collection.client_connection.last_response_headers:
            ...
            pytest.fail('No Etag or etag found in last response headers')
        query_iterable = created_collection.query_items_change_feed(continuation=continuation3, is_start_from_beginning=True, **partition_param)
        iter_list = [item async for item in query_iterable]
        assert len(iter_list) == 0
    ...
    async def test_populate_query_metrics_async(self):
        await self._set_up()
        created_collection = await self.created_db.create_container_if_not_exists('query_metrics_test' + str(uuid.uuid4()), PartitionKey(path='/pk'))
        doc_id = 'MyId' + str(uuid.uuid4())
        document_definition = {'pk': 'pk', 'id': doc_id}
        await created_collection.create_item(body=document_definition)
        query = 'SELECT * from c'
    ...
    async def test_populate_index_metrics(self):
        await self._set_up()
        created_collection = await self.created_db.create_container_if_not_exists('index_metrics_test' + str(uuid.uuid4()), PartitionKey(path='/pk'))
        doc_id = 'MyId' + str(uuid.uuid4())
        document_definition = {'pk': 'pk', 'id': doc_id}
        await created_collection.create_item(body=document_definition)
        query = 'SELECT * from c'
        ...
        expected_index_metrics = {'UtilizedSingleIndexes': [{'FilterExpression': '', 'IndexSpec': '/pk/?', 'FilterPreciseSet': True, 'IndexPreciseSet': True, 'IndexImpactScore': 'High'}], 'PotentialSingleIndexes': [], 'UtilizedCompositeIndexes': [], 'PotentialCompositeIndexes': []}
    ...
    async def test_get_query_plan_through_gateway_async(self):
        await self._set_up()
        created_collection = await self.created_db.create_container_if_not_exists(str(uuid.uuid4()), PartitionKey(path='/pk'))
        await self._validate_query_plan(query='Select top 10 value count(c.id) from c', container_link=created_collection.container_link, top=10, order_by=[], aggregate=['Count'], select_value=True, offset=None, limit=None, distinct=_DistinctType.NoneType)
        await self._validate_query_plan(query='Select * from c order by c._ts offset 5 limit 10', container_link=created_collection.container_link, top=None, order_by=['Ascending'], aggregate=[], select_value=False, offset=5, limit=10, distinct=_DistinctType.NoneType)
        await self._validate_query_plan(query='Select distinct value c.id from c order by c.id', container_link=created_collection.container_link, top=None, order_by=['Ascending'], aggregate=[], select_value=True, offset=None, limit=None, distinct=_DistinctType.Ordered)

    async def _validate_query_plan(self, query, container_link, top, order_by, aggregate, select_value, offset, limit, distinct):
    ...
    async def test_offset_limit_async(self):
        await self._set_up()
        created_collection = await self.created_db.create_container_if_not_exists('offset_limit_' + str(uuid.uuid4()), PartitionKey(path='/pk'))
        values = []
        for i in range(10):
        ...
        await self._validate_distinct_offset_limit(created_collection=created_collection, query='SELECT DISTINCT c["value"] from c ORDER BY c.pk OFFSET 0 LIMIT 2', results=[0, 1])
        await self._validate_distinct_offset_limit(created_collection=created_collection, query='SELECT DISTINCT c["value"] from c ORDER BY c.pk OFFSET 2 LIMIT 2', results=[2, 3])
        await self._validate_distinct_offset_limit(created_collection=created_collection, query='SELECT DISTINCT c["value"] from c ORDER BY c.pk OFFSET 4 LIMIT 3', results=[])
        await self._validate_offset_limit(created_collection=created_collection, query='SELECT * from c ORDER BY c.pk OFFSET 0 LIMIT 5', results=values[:5])
        await self._validate_offset_limit(created_collection=created_collection, query='SELECT * from c ORDER BY c.pk OFFSET 5 LIMIT 10', results=values[5:])
        await self._validate_offset_limit(created_collection=created_collection, query='SELECT * from c ORDER BY c.pk OFFSET 10 LIMIT 5', results=[])
        await self._validate_offset_limit(created_collection=created_collection, query='SELECT * from c ORDER BY c.pk OFFSET 100 LIMIT 1', results=[])
    ...
    async def test_distinct_async(self):
        ...
        created_collection = await created_database.create_container(id='collection with composite index ' + str(uuid.uuid4()), partition_key=PartitionKey(path='/pk', kind='Hash'), indexing_policy={'compositeIndexes': [[{'path': '/' + pk_field, 'order': 'ascending'}, {'path': '/' + distinct_field, 'order': 'ascending'}], [{'path': '/' + distinct_field, 'order': 'ascending'}, {'path': '/' + pk_field, 'order': 'ascending'}]]})
        documents = []
        for i in range(5):
            j = i
            while j > i - 5:
        ...
        await self._validate_distinct(created_collection=created_collection, query='SELECT distinct c.%s from c ORDER BY c.%s' % (distinct_field, distinct_field), results=self._get_distinct_docs(self._get_order_by_docs(padded_docs, distinct_field, None), distinct_field, None, True), is_select=False, fields=[distinct_field])
        await self._validate_distinct(created_collection=created_collection, query='SELECT distinct c.%s, c.%s from c ORDER BY c.%s, c.%s' % (distinct_field, pk_field, pk_field, distinct_field), results=self._get_distinct_docs(self._get_order_by_docs(padded_docs, pk_field, distinct_field), distinct_field, pk_field, True), is_select=False, fields=[distinct_field, pk_field])
        ...
        await self._validate_distinct(created_collection=created_collection, query='SELECT distinct value c.%s from c ORDER BY c.%s' % (distinct_field, distinct_field), results=self._get_distinct_docs(self._get_order_by_docs(padded_docs, distinct_field, None), distinct_field, None, True), is_select=False, fields=[distinct_field])
        await self._validate_distinct(created_collection=created_collection, query='SELECT distinct c.%s from c' % distinct_field, results=self._get_distinct_docs(padded_docs, distinct_field, None, False), is_select=True, fields=[distinct_field])
        await self._validate_distinct(created_collection=created_collection, query='SELECT distinct c.%s, c.%s from c' % (distinct_field, pk_field), results=self._get_distinct_docs(padded_docs, distinct_field, pk_field, False), is_select=True, fields=[distinct_field, pk_field])
        await self._validate_distinct(created_collection=created_collection, query='SELECT distinct value c.%s from c' % distinct_field, results=self._get_distinct_docs(padded_docs, distinct_field, None, True), is_select=True, fields=[distinct_field])
        await self._validate_distinct(created_collection=created_collection, query='SELECT distinct c.%s from c ORDER BY c.%s' % (different_field, different_field), results=[], is_select=True, fields=[different_field])
        await self._validate_distinct(created_collection=created_collection, query='SELECT distinct c.%s from c' % different_field, results=['None'], is_select=True, fields=[different_field])
    ...
    async def test_distinct_on_different_types_and_field_orders_async(self):
        await self._set_up()
        created_collection = await self.created_db.create_container_if_not_exists(str(uuid.uuid4()), PartitionKey(path='/id'))
        payloads = [{'id': str(uuid.uuid4()), 'f1': 1, 'f2': 'value', 'f3': 100000000000000000, 'f4': [1, 2, '3'], 'f5': {'f6': {'f7': 2}}}, {'id': str(uuid.uuid4()), 'f2': "'value", 'f4': [1.0, 2, '3'], 'f5': {'f6': {'f7': 2.0}}, 'f1': 1.0, 'f3': 1e+17}, {'id': str(uuid.uuid4()), 'f3': 1e+17, 'f5': {'f6': {'f7': 2}}, 'f2': "'value", 'f1': 1, 'f4': [1, 2.0, '3']}]
        for pay in payloads:
            await created_collection.create_item(pay)
        await self._validate_distinct_on_different_types_and_field_orders(collection=created_collection, query='Select distinct value c.f1 from c', expected_results=[1])
        await self._validate_distinct_on_different_types_and_field_orders(collection=created_collection, query='Select distinct value c.f2 from c', expected_results=['value', "'value"])
        await self._validate_distinct_on_different_types_and_field_orders(collection=created_collection, query='Select distinct value c.f2 from c order by c.f2', expected_results=['value', "'value"])
        await self._validate_distinct_on_different_types_and_field_orders(collection=created_collection, query='Select distinct value c.f3 from c', expected_results=[100000000000000000])
        await self._validate_distinct_on_different_types_and_field_orders(collection=created_collection, query='Select distinct value c.f4 from c', expected_results=[[1, 2, '3']])
        await self._validate_distinct_on_different_types_and_field_orders(collection=created_collection, query='Select distinct value c.f5.f6 from c', expected_results=[{'f7': 2}])
        await self._validate_distinct_on_different_types_and_field_orders(collection=created_collection, query='Select distinct c.f1, c.f2, c.f3 from c', expected_results=[{'f1': 1, 'f2': 'value', 'f3': 100000000000000000}, {'f1': 1.0, 'f2': "'value", 'f3': 1e+17}])
        await self._validate_distinct_on_different_types_and_field_orders(collection=created_collection, query='Select distinct c.f1, c.f2, c.f3 from c order by c.f1', expected_results=[{'f1': 1, 'f2': 'value', 'f3': 100000000000000000}, {'f1': 1.0, 'f2': "'value", 'f3': 1e+17}])
    ...
    async def test_value_max_query_async(self):
        await self._set_up()
        container = await self.created_db.create_container_if_not_exists(str(uuid.uuid4()), PartitionKey(path='/id'))
        await container.create_item({'id': str(uuid.uuid4()), 'isComplete': True, 'version': 3, 'lookupVersion': 'console_version'})
        await container.create_item({'id': str(uuid.uuid4()), 'isComplete': True, 'version': 2, 'lookupVersion': 'console_version'})
        query = 'Select value max(c.version) FROM c where c.isComplete = true and c.lookupVersion = @lookupVersion'
        query_results = container.query_items(query, parameters=[{'name': '@lookupVersion', 'value': 'console_version'}])
    ...
    async def test_continuation_token_size_limit_query_async(self):
        await self._set_up()
        container = await self.created_db.create_container_if_not_exists(str(uuid.uuid4()), PartitionKey(path='/pk'))
        for i in range(1, 1000):
            await container.create_item(body=dict(pk='123', id=str(i), some_value=str(i % 3)))
        query = "Select * from c where c.some_value='2'"
        response_query = container.query_items(query, partition_key='123', max_item_count=100, continuation_token_limit=1)
