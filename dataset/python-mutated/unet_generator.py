...
class DiffusionGenerator(nn.Module):
    ...
    def __init__(self, in_dim=3, dim=512, text_dim=1024, context_dim=512, out_dim=6, dim_mult=[1, 2, 3, 4], num_heads=None, head_dim=64, num_res_blocks=3, attn_scales=[1 / 2, 1 / 4, 1 / 8], resblock_resample=True, use_scale_shift_norm=True, dropout=0.0):
        if False:
            print('Hello World!')
        embed_dim = dim * 4
    ...
    def forward(self, x, t, y, context, mask=None):
        if False:
            print('Hello World!')
        e = self.time_embedding(sinusoidal_embedding(t, self.dim)) + self.pool_embedding(y)
