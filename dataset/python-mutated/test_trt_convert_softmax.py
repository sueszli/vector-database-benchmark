        ...
        for dims in [0, 1, 2, 3, 4]:
            for batch in [1, 2, 4]:
                for axis in [-1, 0, 1, 2, 3]:
                    self.dims = dims
                    dics = [{'axis': axis}, {}]
                    ops_config = [{'op_type': 'softmax', 'op_inputs': {'X': ['softmax_input']}, 'op_outputs': {'Out': ['softmax_out']}, 'op_attrs': dics[0]}]
                    ops = self.generate_op_config(ops_config)
                    program_config = ProgramConfig(ops=ops, weights={}, inputs={'softmax_input': TensorConfig(data_gen=partial(generate_input1, dics, batch))}, outputs=['softmax_out'])
    ...
    def sample_predictor_configs(self, program_config) -> (paddle_infer.Config, List[int], float):
        ...
        def generate_dynamic_shape(attrs):
            if False:
                return 10
            if self.dims == 4:
                self.dynamic_shape.min_input_shape = {'softmax_input': [1, 3, 24, 24]}
                self.dynamic_shape.max_input_shape = {'softmax_input': [4, 3, 48, 48]}
                self.dynamic_shape.opt_input_shape = {'softmax_input': [1, 3, 24, 48]}
            elif self.dims == 3:
