...
def test_rankWithGroupBy():
    ...
    rankedFrame = train.rank_within_group_by(groupCols, sortCols, sortDirs) if randint(1, 10) > 5 else train.rank_within_group_by(groupCols, sortCols, new_col_name=newColName)
    rankedFrame.summary()
...
def generate_trainingFrame():
    if False:
        while True:
            i = 10
    nrows = 1000000
    temp = pyunit_utils.random_dataset_int_only(nrows, 1, 1, misFrac=0.01)
