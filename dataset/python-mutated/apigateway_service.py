            ...
            for page in get_rest_apis_paginator.paginate():
                for apigw in page['items']:
                    ...
                    if not self.audit_resources or is_resource_filtered(arn, self.audit_resources):
                        self.rest_apis.append(RestAPI(id=apigw['id'], arn=arn, region=regional_client.region, name=apigw.get('name', ''), tags=[apigw.get('tags')]))
    ...
    def __get_authorizers__(self):
        ...
        try:
            for rest_api in self.rest_apis:
                ...
                authorizers = regional_client.get_authorizers(restApiId=rest_api.id)['items']
                if authorizers:
        ...
        try:
            for rest_api in self.rest_apis:
                ...
                for stage in stages['item']:
                    ...
                    rest_api.stages.append(Stage(name=stage['stageName'], arn=arn, logging=logging, client_certificate=client_certificate, waf=waf, tags=[stage.get('tags')]))
        except Exception as error:
            logger.error(f'{error.__class__.__name__}: {error}')
