...
class DetectionOutput(Layer):
    ...
    def __init__(self, num_classes, nms_threshold=0.45, nms_topk=400, topk=200, threshold=0.01, name=None):
        if False:
            while True:
                i = 10
    ...
    def fprop(self, inputs, inference=True):
        ...
        if self.be.is_mkl():
            return self.be.detectionOutput_fprop(conf_view, loc_view, self.detections, prior_boxes, self.proposals, self.nms_topk, self.topk, self.threshold, self.nms_threshold)
        ...
        for k in range(self.be.bsz):
            ...
            for c in range(self.num_classes):
                ...
                if len(top_N_ind) > 0:
                    ...
                    dets = np.append(self.detections[keep, :].get(), c * np.ones((len(keep), 1)), axis=1)
...
class PriorBox(Layer):
    ...
    def __init__(self, min_sizes, max_sizes, step=None, aspect_ratios=[2, 3], img_shape=(300, 300), flip=True, clip=False, variance=[0.1, 0.1, 0.2, 0.2], offset=0.5, name=None):
        if False:
            while True:
                i = 10
    ...
    def fprop(self, inputs, inference=True):
        ...
        def gen_box(center, box_size, image_size, variance, clip):
            if False:
                return 10
            box_ = [None] * 4
            box_[0] = (center[0] - box_size[0] * 0.5) / image_size[0]
            box_[1] = (center[1] - box_size[1] * 0.5) / image_size[1]
            box_[2] = (center[0] + box_size[0] * 0.5) / image_size[0]
            box_[3] = (center[1] + box_size[1] * 0.5) / image_size[1]
            if clip:
                for ind in range(4):
        ...
        for h in range(self.layer_height):
            ...
            for w in range(self.layer_width):
                ...
                for (min_size, max_size) in zip(self.min_sizes, self.max_sizes):
                    ...
                    self.prior_boxes += gen_box(center, box_shape, img_shape, self.variance, self.clip)
                    sz_ = math.sqrt(min_size * max_size)
                    box_shape = [sz_, sz_]
                    self.prior_boxes += gen_box(center, box_shape, img_shape, self.variance, self.clip)
                    for ar in self.aspect_ratios:
                        assert np.abs(ar - 1.0) > 1e-06
