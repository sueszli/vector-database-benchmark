...
from ...utils import checkout_add_promo_code, checkout_create, checkout_delivery_method_update, checkout_dummy_payment_create, raw_checkout_complete

def prepare_voucher(e2e_staff_api_client, channel_id, voucher_code, voucher_discount_type, voucher_discount_value, voucher_type):
    if False:
        while True:
            i = 10
...
@pytest.mark.e2e
def test_checkout_voucher_is_still_active_when_checkout_fails_core_0918(e2e_logged_api_client, e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_shipping, permission_manage_product_types_and_attributes, permission_manage_discounts, permission_manage_checkouts):
    if False:
        for i in range(10):
            print('nop')
    ...
    (voucher_code, voucher_id) = prepare_voucher(e2e_staff_api_client, channel_id, voucher_code='PERCENTAGE_VOUCHER', voucher_discount_type='PERCENTAGE', voucher_discount_value=10, voucher_type='ENTIRE_ORDER')
    ...
    data = checkout_add_promo_code(e2e_logged_api_client, checkout_id, voucher_code)
    ...
    data = product_variant_stock_update(e2e_staff_api_client, warehouse_id, 1, product_variant_id)
