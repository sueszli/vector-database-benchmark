...
class ReservationRecommendationDetailsOperations:
    ...
    async def get(self, resource_scope: str, scope: Union[str, _models.Scope], region: str, term: Union[str, _models.Term], look_back_period: Union[str, _models.LookBackPeriod], product: str, **kwargs: Any) -> Optional[_models.ReservationRecommendationDetailsModel]:
        ...
        request = build_get_request(resource_scope=resource_scope, scope=scope, region=region, term=term, look_back_period=look_back_period, product=product, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
