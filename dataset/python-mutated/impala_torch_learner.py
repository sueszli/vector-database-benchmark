...
from ray.rllib.algorithms.impala.torch.vtrace_torch_v2 import vtrace_torch, make_time_major
...
class ImpalaTorchLearner(ImpalaLearner, TorchLearner):
    ...
    def compute_loss_for_module(self, *, module_id: ModuleID, hps: ImpalaLearnerHyperparameters, batch: NestedDict, fwd_out: Mapping[str, TensorType]) -> TensorType:
        ...
        discounts_time_major = (1.0 - make_time_major(batch[SampleBatch.TERMINATEDS], trajectory_len=hps.rollout_frag_or_episode_len, recurrent_seq_len=hps.recurrent_seq_len).type(dtype=torch.float32)) * hps.discount_factor
        device = behaviour_actions_logp_time_major[0].device
        (vtrace_adjusted_target_values, pg_advantages) = vtrace_torch(target_action_log_probs=target_actions_logp_time_major, behaviour_action_log_probs=behaviour_actions_logp_time_major, discounts=discounts_time_major, rewards=rewards_time_major, values=values_time_major, bootstrap_value=bootstrap_value, clip_rho_threshold=hps.vtrace_clip_rho_threshold, clip_pg_rho_threshold=hps.vtrace_clip_pg_rho_threshold)
        batch_size = convert_to_torch_tensor(target_actions_logp_time_major.shape[-1]).float().to(device)
