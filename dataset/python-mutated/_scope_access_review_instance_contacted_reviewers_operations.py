...
def build_list_request(scope: str, schedule_definition_id: str, id: str, **kwargs: Any) -> HttpRequest:
    ...
    _url = kwargs.pop('template_url', '/{scope}/providers/Microsoft.Authorization/accessReviewScheduleDefinitions/{scheduleDefinitionId}/instances/{id}/contactedReviewers')
...
class ScopeAccessReviewInstanceContactedReviewersOperations:
    ...
    @distributed_trace
    def list(self, scope: str, schedule_definition_id: str, id: str, **kwargs: Any) -> Iterable['_models.AccessReviewContactedReviewer']:
        if False:
            return 10
        ...
        def prepare_request(next_link=None):
            if False:
                ...
                return i + 15
            if not next_link:
                request = build_list_request(scope=scope, schedule_definition_id=schedule_definition_id, id=id, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
