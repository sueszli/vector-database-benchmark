...
class AirlinePassengers(ChangePointFileDataset):
    ...
    def __init__(self):
        if False:
            while True:
                i = 10
        super().__init__(annotations={'6': [299], '7': [], '8': [302], '9': [326, 382], '10': [296]}, filename='airline_passengers.csv', task=datasets.base.REG, n_samples=468, n_features=1)

    def __iter__(self):
        if False:
            print('Hello World!')
        return stream.iter_csv(self.path, target='Total Passengers', converters={'Total Passengers': int}, parse_dates={'date': '%Y-%b'})