...
if is_torch_available():
    import torch
    from transformers import MvpForCausalLM, MvpForConditionalGeneration, MvpForQuestionAnswering, MvpForSequenceClassification, MvpModel, MvpTokenizer
...
class MvpModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (MvpModel, MvpForConditionalGeneration, MvpForSequenceClassification, MvpForQuestionAnswering) if is_torch_available() else ()
    all_generative_model_classes = (MvpForConditionalGeneration,) if is_torch_available() else ()
    pipeline_model_mapping = {'conversational': MvpForConditionalGeneration, 'feature-extraction': MvpModel, 'fill-mask': MvpForConditionalGeneration, 'question-answering': MvpForQuestionAnswering, 'summarization': MvpForConditionalGeneration, 'text-classification': MvpForSequenceClassification, 'text-generation': MvpForCausalLM, 'text2text-generation': MvpForConditionalGeneration, 'translation': MvpForConditionalGeneration, 'zero-shot': MvpForSequenceClassification} if is_torch_available() else {}
