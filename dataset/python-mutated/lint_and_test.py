...
async def async_exec(*args, display=False):
    ...
    if not display:
        ...
        (stdout, _) = await asyncio.gather(read_stream(proc.stdout, sys.stdout.write), read_stream(proc.stderr, sys.stderr.write))
...
async def lint(files):
    ...
    res = sorted(itertools.chain(*await asyncio.gather(pylint(files), ruff(files))), key=lambda item: item.file)
    if res:
        print('Lint errors:')
    else:
...
async def main():
    ...
    files = await git()
    if not files:
        print('No changed files found. Please ensure you have added your changes with git add & git commit')
        return
    ...
    if not test_files:
        ...
        return
    (code, _) = await async_exec('python3', '-m', 'pytest', '-vv', '--force-sugar', '--', *test_files, display=True)
    print('=============================')
    if code == 0:
