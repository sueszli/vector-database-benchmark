...
def _scalar_binop(op):
    ...
    return binop
for op in ['__add__', '__sub__', '__mul__', '__div__', '__truediv__', '__floordiv__', '__mod__', '__divmod__', '__pow__', '__and__', '__or__']:
...
SECTION_ORDER = ['Parameters', 'Returns', 'Raises', BEAM_SPECIFIC, 'See Also', 'Notes', 'Examples']
EXAMPLES_DISCLAIMER = '**NOTE:** These examples are pulled directly from the pandas documentation for convenience. Usage of the Beam DataFrame API will look different because it is a deferred API.'
EXAMPLES_DIFFERENCES = EXAMPLES_DISCLAIMER + f' In addition, some arguments shown here may not be supported, see **{BEAM_SPECIFIC!r}** for details.'
