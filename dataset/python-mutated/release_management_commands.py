...
from airflow_breeze.global_constants import ALLOWED_PLATFORMS, APACHE_AIRFLOW_GITHUB_REPOSITORY, CURRENT_PYTHON_MAJOR_MINOR_VERSIONS, DEFAULT_PYTHON_MAJOR_MINOR_VERSION, MOUNT_ALL, MOUNT_SELECTED, MULTI_PLATFORM
from airflow_breeze.params.shell_params import ShellParams
from airflow_breeze.utils.add_back_references import start_generating_back_references
from airflow_breeze.utils.ci_group import ci_group
from airflow_breeze.utils.common_options import argument_doc_packages, argument_provider_packages, option_airflow_constraints_mode_ci, option_airflow_constraints_mode_update, option_airflow_constraints_reference, option_airflow_extras, option_airflow_site_directory, option_answer, option_commit_sha, option_debug_resources, option_dry_run, option_github_repository, option_historical_python_version, option_image_tag_for_running, option_include_success_outputs, option_install_selected_providers, option_installation_package_format, option_package_format, option_parallelism, option_python, option_python_versions, option_run_in_parallel, option_skip_cleanup, option_skip_constraints, option_use_airflow_version, option_use_packages_from_dist, option_verbose, option_version_suffix_for_pypi
from airflow_breeze.utils.confirm import Answer, user_confirm
from airflow_breeze.utils.console import MessageType, Output, get_console
from airflow_breeze.utils.custom_param_types import BetterChoice
from airflow_breeze.utils.docker_command_utils import check_remote_ghcr_io_commands, get_env_variables_for_docker_commands, get_extra_docker_flags, perform_environment_checks
from airflow_breeze.utils.github import download_constraints_file, get_active_airflow_versions
from airflow_breeze.utils.packages import expand_all_provider_packages, find_matching_long_package_names, get_available_packages, get_provider_details, get_provider_packages_metadata, get_removed_provider_ids
from airflow_breeze.utils.parallel import GenericRegexpProgressMatcher, SummarizeAfter, check_async_run_results, run_with_pool
from airflow_breeze.utils.path_utils import AIRFLOW_SOURCES_ROOT, CONSTRAINTS_CACHE_DIR, DIST_DIR, PROVIDER_METADATA_JSON_FILE_PATH, cleanup_python_generated_files
from airflow_breeze.utils.provider_dependencies import DEPENDENCIES, generate_providers_metadata_for_package, get_related_providers
from airflow_breeze.utils.publish_docs_builder import PublishDocsBuilder
from airflow_breeze.utils.python_versions import get_python_version_list
from airflow_breeze.utils.run_utils import RunCommandResult, assert_pre_commit_installed, run_command, run_compile_www_assets
from airflow_breeze.utils.shared_options import get_dry_run, get_verbose
option_debug_release_management = click.option('--debug', is_flag=True, help='Drop user in shell instead of running the command. Useful for debugging.', envvar='DEBUG')
...
CONSTRAINT_PROGRESS_MATCHER = 'Found|Uninstalling|uninstalled|Collecting|Downloading|eta|Running|Installing|built|Attempting'
...
SDIST_FILENAME_PATTERN = re.compile(f'{SDIST_FILENAME_PREFIX}(.*)-[0-9].*\\.tar\\.gz')
WHEEL_FILENAME_PATTERN = re.compile(f'{WHEEL_FILENAME_PREFIX}(.*)-[0-9].*\\.whl')
...
SDIST_INSTALL_PROGRESS_REGEXP = 'Processing .*|Requirement already satisfied:.*|  Created wheel.*'
...
PUBLISHING_DOCS_PROGRESS_MATCHER = 'Publishing docs|Copy directory'
...
MATCH_CONSTRAINTS_FILE_REGEX = re.compile('constraints-(.*)-python-(.*).txt')
