...
from .const import ISSUE_KEY_SYSTEM_DOCKER_CONFIG, PLACEHOLDER_KEY_COMPONENTS, PLACEHOLDER_KEY_REFERENCE, SupervisorIssueContext
from .handler import HassioAPIError, async_apply_suggestion
from .issues import Issue, Suggestion
SUGGESTION_CONFIRMATION_REQUIRED = {'system_execute_reboot'}
EXTRA_PLACEHOLDERS = {'issue_mount_mount_failed': {'storage_url': '/config/storage'}}
...
class SupervisorIssueRepairFlow(RepairsFlow):
    ...
    async def async_step_init(self, _: None=None) -> FlowResult:
        ...
        for suggestion in self.issue.suggestions:
            setattr(self, f'async_step_{suggestion.key}', MethodType(self._async_step(suggestion), self))
    ...
    async def async_step_fix_menu(self, _: None=None) -> FlowResult:
        """Show the fix menu."""
        assert self.issue
        return self.async_show_menu(step_id='fix_menu', menu_options=[suggestion.key for suggestion in self.issue.suggestions], description_placeholders=self.description_placeholders)

    async def _async_step_apply_suggestion(self, suggestion: Suggestion, confirmed: bool=False) -> FlowResult:
