    ...
    def __init__(self, parent, list_of_links, callback, persepolis_setting):
        ...
        for dict in self.list_of_links:
            ...
            if file_name == '***':
                ...
                self.parent.threadPool[len(self.parent.threadPool) - 1].QUEUESPIDERRETURNEDFILENAME.connect(partial(self.parent.queueSpiderCallBack, child=self, row_number=len(self.list_of_links) - k))
            k = k + 1
    ...
    def okButtonPressed(self, button):
        ...
        for row in range(self.links_table.rowCount()):
            ...
            if item.checkState() == 2:
                self.add_link_dictionary_list.append(deepcopy(dict))
