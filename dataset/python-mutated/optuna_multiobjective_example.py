...
def easy_objective(config):
    ...
    for step in range(config['steps']):
        ...
        train.report({'iterations': step, 'loss': intermediate_score, 'gain': intermediate_score * width})
        time.sleep(0.1)
...
def run_optuna_tune(smoke_test=False):
    ...
    tuner = tune.Tuner(easy_objective, tune_config=tune.TuneConfig(search_alg=algo, num_samples=10 if smoke_test else 100), param_space={'steps': 100, 'width': tune.uniform(0, 20), 'height': tune.uniform(-100, 100), 'activation': tune.choice(['relu', 'tanh'])})
    results = tuner.fit()
