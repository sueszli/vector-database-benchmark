...
class SrvResolver:
    ...
    async def resolve_service(self, service_name: bytes) -> List[Server]:
        ...
        try:
            (answers, _, _) = await make_deferred_yieldable(self._dns_client.lookupService(service_name))
        except DNSNameError:
            return []
            ...
            cache_entry = self._cache.get(service_name, None)
            if cache_entry:
                logger.warning('Failed to resolve %r, falling back to cache. %r', service_name, e)
        ...
        for answer in answers:
            ...
            servers.append(Server(host=payload.target.name, port=payload.port, priority=payload.priority, weight=payload.weight, expires=now + answer.ttl))
