...
if is_torch_available():
    import torch
    from transformers import AutoModelForSequenceClassification, BartForCausalLM, BartForConditionalGeneration, BartForQuestionAnswering, BartForSequenceClassification, BartModel, BartTokenizer, pipeline
    from transformers.models.bart.modeling_bart import BartDecoder, BartEncoder, shift_tokens_right

def prepare_bart_inputs_dict(config, input_ids, decoder_input_ids=None, attention_mask=None, decoder_attention_mask=None, head_mask=None, decoder_head_mask=None, cross_attn_head_mask=None):
    if False:
        while True:
            i = 10
...
class BartHeadTests(unittest.TestCase):
    ...
    def _get_config_and_data(self):
        ...
        config = BartConfig(vocab_size=self.vocab_size, d_model=24, encoder_layers=2, decoder_layers=2, encoder_attention_heads=2, decoder_attention_heads=2, encoder_ffn_dim=32, decoder_ffn_dim=32, max_position_embeddings=48, eos_token_id=2, pad_token_id=1, bos_token_id=0)
    ...
    def test_lm_uneven_forward(self):
        if False:
            return 10
        config = BartConfig(vocab_size=self.vocab_size, d_model=14, encoder_layers=2, decoder_layers=2, encoder_attention_heads=2, decoder_attention_heads=2, encoder_ffn_dim=8, decoder_ffn_dim=8, max_position_embeddings=48)
...
class BartModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (BartModel, BartForConditionalGeneration, BartForSequenceClassification, BartForQuestionAnswering) if is_torch_available() else ()
...
class FastIntegrationTests(unittest.TestCase):
    ...
    def test_xsum_1_1_batch_generation(self):
        ...
        assert result[1] == ' An investigation into the crash that killed at least 10 people in the French capital has been released by the French police investigating the crash.'
    ...
    def test_encoder_equiv(self):
        ...
        expected = [[-0.0828, -0.0251, -0.0674], [0.1277, 0.3311, -0.0255], [0.2613, -0.084, -0.2763]]
        assert_tensors_close(features[0, :3, :3], torch.tensor(expected), atol=0.001)
...
class BartModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor([[0.7144, 0.8143, -1.2813], [0.7144, 0.8143, -1.2813], [-0.0467, 2.5911, -2.1845]], device=torch_device)
    ...
    def test_mnli_inference(self):
        ...
        model = AutoModelForSequenceClassification.from_pretrained('facebook/bart-large-mnli').to(torch_device)
        attention_mask = input_ids.ne(model.config.pad_token_id)
        with torch.no_grad():
        ...
        assert_tensors_close(batched_logits[1], logits2, atol=0.001)
        assert_tensors_close(expected_slice, logits_arr, atol=0.001)
    ...
    def test_xsum_summarization_same_as_fairseq(self):
        ...
        dct = tok.batch_encode_plus([PGE_ARTICLE], max_length=1024, padding='max_length', truncation=True, return_tensors='pt').to(torch_device)
        hypotheses_batch = model.generate(input_ids=dct['input_ids'], attention_mask=dct['attention_mask'], num_beams=2, max_length=62, min_length=11, length_penalty=1.0, no_repeat_ngram_size=3, early_stopping=True, decoder_start_token_id=model.config.eos_token_id)
    ...
    def test_contrastive_search_bart(self):
        ...
        self.assertListEqual(generated_text, ['Liana Barrientos, 39, pleaded not guilty to charges related to false marriage statements. Prosecutors say she married at least 10 times, sometimes within two weeks of each other. She is accused of being part of an immigration scam to get permanent residency. If convicted, she faces up to four years in'])
    ...
    def test_decoder_attention_mask(self):
        if False:
            while True:
                i = 10
        model = BartForConditionalGeneration.from_pretrained('facebook/bart-large', forced_bos_token_id=0).to(torch_device)
        ...
        padding_size = 3
        decoder_input_ids = torch.tensor([[model.config.decoder_start_token_id] + padding_size * [model.config.pad_token_id] + [model.config.bos_token_id]], dtype=torch.long, device=torch_device)
        decoder_attention_mask = torch.where(decoder_input_ids == model.config.pad_token_id, 0, 1).to(torch_device)
        generated_ids = model.generate(input_ids=input_ids, use_cache=False, max_new_tokens=20, decoder_input_ids=decoder_input_ids, decoder_attention_mask=decoder_attention_mask)
