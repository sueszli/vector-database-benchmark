...
class MinimaxProvider(BaseModelProvider):
    ...
    def _get_fixed_model_list(self, model_type: ModelType) -> list[dict]:
        ...
        if model_type == ModelType.TEXT_GENERATION:
            return [{'id': 'abab5.5-chat', 'name': 'abab5.5-chat', 'mode': ModelMode.COMPLETION.value}, {'id': 'abab5-chat', 'name': 'abab5-chat', 'mode': ModelMode.COMPLETION.value}]
        elif model_type == ModelType.EMBEDDINGS:
            return [{'id': 'embo-01', 'name': 'embo-01'}]
        else:
            return []
    ...
    def get_model_parameter_rules(self, model_name: str, model_type: ModelType) -> ModelKwargsRules:
        ...
        model_max_tokens = {'abab5.5-chat': 16384, 'abab5-chat': 6144}
        return ModelKwargsRules(temperature=KwargRule[float](min=0.01, max=1, default=0.9, precision=2), top_p=KwargRule[float](min=0, max=1, default=0.95, precision=2), presence_penalty=KwargRule[float](enabled=False), frequency_penalty=KwargRule[float](enabled=False), max_tokens=KwargRule[int](min=10, max=model_max_tokens.get(model_name, 6144), default=1024, precision=0))
    ...
    def is_provider_credentials_valid_or_raise(cls, credentials: dict):
        ...
        try:
            credential_kwargs = {'minimax_group_id': credentials['minimax_group_id'], 'minimax_api_key': credentials['minimax_api_key']}
            llm = MinimaxChatLLM(model='abab5.5-chat', max_tokens=10, temperature=0.01, **credential_kwargs)
    ...
    def get_provider_credentials(self, obfuscated: bool=False) -> dict:
        ...
        if self.provider.provider_type == ProviderType.CUSTOM.value or (self.provider.provider_type == ProviderType.SYSTEM.value and self.provider.quota_type == ProviderQuotaType.FREE.value):
            try:
                ...
                credentials = {'minimax_group_id': None, 'minimax_api_key': None}
            if credentials['minimax_api_key']:
                credentials['minimax_api_key'] = encrypter.decrypt_token(self.provider.tenant_id, credentials['minimax_api_key'])
                if obfuscated:
