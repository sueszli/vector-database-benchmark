...
def upgrade():
    if False:
        for i in range(10):
            print('nop')
    op.add_column('releases', sa.Column('uploader_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.create_foreign_key(None, 'releases', 'accounts_user', ['uploader_id'], ['id'], onupdate='CASCADE', ondelete='SET NULL')
