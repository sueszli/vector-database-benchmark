...
class TestStringCommandHandler:
    ...
    async def callback(self, update, context):
        self.test_flag = isinstance(context, CallbackContext) and isinstance(context.bot, Bot) and isinstance(update, str) and isinstance(context.update_queue, asyncio.Queue) and isinstance(context.job_queue, JobQueue) and (context.user_data is None) and (context.chat_data is None) and isinstance(context.bot_data, dict)
