...
class ImLivechatChannel(models.Model):
    ...
    button_text = fields.Char('Text of the Button', default='Have a Question? Chat with us.', help='Default text displayed on the Livechat Support Button')
    default_message = fields.Char('Welcome Message', default='How may I help you?', help="This is an automated 'welcome' message that your visitor will see when they initiate a new conversation.")
    input_placeholder = fields.Char('Chat Input Placeholder')
    web_page = fields.Char('Web Page', compute='_compute_web_page_link', store=False, readonly=True, help='URL to a static page where you client can discuss with the operator of the channel.')
    are_you_inside = fields.Boolean(string='Are you inside the matrix?', compute='_are_you_inside', store=False, readonly=True)
    ...
    rating_percentage_satisfaction = fields.Integer('% Happy', compute='_compute_percentage_satisfaction', store=False, default=-1, help='Percentage of happy ratings over the past 7 days')
    image = fields.Binary('Image', default=_default_image, attachment=True, help='This field holds the image used as photo for the group, limited to 1024x1024px.')
    image_medium = fields.Binary('Medium', attachment=True, help='Medium-sized photo of the group. It is automatically resized as a 128x128px image, with aspect ratio preserved. Use this field in form views or some kanban views.')
    image_small = fields.Binary('Thumbnail', attachment=True, help='Small-sized photo of the group. It is automatically resized as a 64x64px image, with aspect ratio preserved. Use this field anywhere a small image is required.')
...
class ImLivechatChannelRule(models.Model):
    ...
    _order = 'sequence asc'
    regex_url = fields.Char('URL Regex', help='Regular expression specifying the web pages this rule will be applied on.')
    action = fields.Selection([('display_button', 'Display the button'), ('auto_popup', 'Auto popup'), ('hide_button', 'Hide the button')], string='Action', required=True, default='display_button', help="* 'Display the button' displays the chat button on the pages.\n* 'Auto popup' displays the button and automatically open the conversation pane.\n* 'Hide the button' hides the chat button on the pages.")
    auto_popup_timer = fields.Integer('Auto popup timer', default=0, help="Delay (in seconds) to automatically open the conversation window. Note: the selected action must be 'Auto popup' otherwise this parameter will not be taken into account.")
    channel_id = fields.Many2one('im_livechat.channel', 'Channel', help='The channel of the rule')
    country_ids = fields.Many2many('res.country', 'im_livechat_channel_country_rel', 'channel_id', 'country_id', 'Country', help="The rule will only be applied for these countries. Example: if you select 'Belgium' and 'United States' and that you set the action to 'Hide Button', the chat button will be hidden on the specified URL from the visitors located in these 2 countries. This feature requires GeoIP installed on your server.")
    sequence = fields.Integer('Matching order', default=10, help='Given the order to find a matching rule. If 2 rules are matching for the given url/country, the one with the lowest sequence will be chosen.')
