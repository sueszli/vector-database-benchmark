...
wgs_84_wkt = 'GEOGCS["GCS_WGS_1984",DATUM["WGS_1984",SPHEROID["WGS_1984",6378137,298.257223563]],PRIMEM["Greenwich",0],UNIT["Degree",0.017453292519943295]]'
wgs_84_wkt_regex = '^GEOGCS\\["(GCS_)?WGS[ _](19)?84".*$'
datetime_format = '%Y-%m-%dT%H:%M:%S'
ds_list = (TestDS('test_point', nfeat=5, nfld=3, geom='POINT', gtype=1, driver='ESRI Shapefile', fields={'dbl': OFTReal, 'int': OFTInteger, 'str': OFTString}, extent=(-1.35011, 0.166623, -0.524093, 0.824508), srs_wkt=wgs_84_wkt, field_values={'dbl': [float(i) for i in range(1, 6)], 'int': list(range(1, 6)), 'str': [str(i) for i in range(1, 6)]}, fids=range(5)), TestDS('test_vrt', ext='vrt', nfeat=3, nfld=3, geom='POINT', gtype='Point25D', driver='OGR_VRT', fields={'POINT_X': OFTString, 'POINT_Y': OFTString, 'NUM': OFTString}, extent=(1.0, 2.0, 100.0, 523.5), field_values={'POINT_X': ['1.0', '5.0', '100.0'], 'POINT_Y': ['2.0', '23.0', '523.5'], 'NUM': ['5', '17', '23']}, fids=range(1, 4)), TestDS('test_poly', nfeat=3, nfld=3, geom='POLYGON', gtype=3, driver='ESRI Shapefile', fields={'float': OFTReal, 'int': OFTInteger, 'str': OFTString}, extent=(-1.01513, -0.558245, 0.161876, 0.839637), srs_wkt=wgs_84_wkt), TestDS('has_nulls', nfeat=3, nfld=6, geom='POLYGON', gtype=3, driver='GeoJSON', ext='geojson', fields={'uuid': OFTString, 'name': OFTString, 'num': OFTReal, 'integer': OFTInteger, 'datetime': OFTDateTime, 'boolean': OFTInteger}, extent=(-75.2742, 39.846504, -74.959717, 40.11904), field_values={'uuid': ['1378c26f-cbe6-44b0-929f-eb330d4991f5', 'fa2ba67c-a135-4338-b924-a9622b5d869f', '4494c1f3-55ab-4256-b365-12115cb388d5'], 'name': ['Philadelphia', None, 'north'], 'num': [1.001, None, 0.0], 'integer': [5, None, 8], 'boolean': [True, None, False], 'datetime': [datetime.strptime('1994-08-14T11:32:14', datetime_format), None, datetime.strptime('2018-11-29T03:02:52', datetime_format)]}, fids=range(3)))
bad_ds = (TestDS('foo'),)
