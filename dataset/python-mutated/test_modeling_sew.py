...
if is_torch_available():
    import torch
    from transformers import SEWForCTC, SEWForSequenceClassification, SEWModel, Wav2Vec2FeatureExtractor, Wav2Vec2Processor
    from transformers.models.hubert.modeling_hubert import _compute_mask_indices

class SEWModelTester:

    def __init__(self, parent, batch_size=13, seq_length=1024, is_training=False, hidden_size=32, feat_extract_norm='group', feat_extract_dropout=0.0, feat_extract_activation='gelu', conv_dim=(64, 32, 32), conv_stride=(5, 2, 1), conv_kernel=(10, 3, 1), conv_bias=False, num_conv_pos_embeddings=31, num_conv_pos_embedding_groups=2, squeeze_factor=2, num_hidden_layers=2, num_attention_heads=2, hidden_dropout=0.1, intermediate_size=20, layer_norm_eps=1e-05, hidden_act='gelu', initializer_range=0.02, vocab_size=32, do_stable_layer_norm=False, scope=None):
        if False:
            print('Hello World!')
        self.parent = parent
    ...
    def get_config(self):
        if False:
            print('Hello World!')
        return SEWConfig(hidden_size=self.hidden_size, feat_extract_norm=self.feat_extract_norm, feat_extract_dropout=self.feat_extract_dropout, feat_extract_activation=self.feat_extract_activation, conv_dim=self.conv_dim, conv_stride=self.conv_stride, conv_kernel=self.conv_kernel, conv_bias=self.conv_bias, num_conv_pos_embeddings=self.num_conv_pos_embeddings, num_conv_pos_embedding_groups=self.num_conv_pos_embedding_groups, squeeze_factor=self.squeeze_factor, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, hidden_dropout=self.hidden_dropout, intermediate_size=self.intermediate_size, layer_norm_eps=self.layer_norm_eps, hidden_act=self.hidden_act, initializer_range=self.initializer_range, vocab_size=self.vocab_size)
...
class SEWModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = {'audio-classification': SEWForSequenceClassification, 'automatic-speech-recognition': SEWForCTC, 'feature-extraction': SEWModel} if is_torch_available() else {}
    test_pruning = False
    ...
    def test_initialization(self):
        ...
        for model_class in self.all_model_classes:
            ...
            for (name, param) in model.named_parameters():
                uniform_init_parms = ['conv.weight', 'masked_spec_embed', 'quantizer.weight_proj.weight']
...
class SEWModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_pretrained_batched(self):
        ...
        expected_outputs_first = torch.tensor([[[0.1509, 0.5372, 0.3061, -0.1694], [-0.17, 0.5764, 0.2753, -0.1299], [0.1281, 0.7949, 0.2342, -0.1624], [-0.1627, 0.671, 0.2215, -0.1317]], [[0.0408, 1.4355, 0.8605, -0.0968], [0.0393, 1.2368, 0.6826, 0.0364], [-0.1269, 1.9215, 1.1677, -0.1297], [-0.1654, 1.6524, 0.6877, -0.0196]]], device=torch_device)
        expected_outputs_last = torch.tensor([[[1.3379, -0.145, -0.15, -0.0515], [0.8364, -0.168, -0.1248, -0.0689], [1.2791, -0.1507, -0.1523, -0.0564], [0.8208, -0.169, -0.1199, -0.0751]], [[0.6959, -0.0861, -0.1235, -0.0861], [0.47, -0.1686, -0.1141, -0.1199], [1.0776, -0.1137, -0.0124, -0.0472], [0.5774, -0.1675, -0.0376, -0.0823]]], device=torch_device)
    ...
    def test_inference_ctc_batched(self):
        ...
        EXPECTED_TRANSCRIPTIONS = ['a man said to the universe sir i exist', "swet covered brian's body trickling into the tightloine closs hat was the only garment he wore"]
