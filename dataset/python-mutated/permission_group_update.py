...
from ....account.utils import can_user_manage_group_channels, can_user_manage_group_permissions, get_not_manageable_permissions_after_removing_perms_from_group, get_not_manageable_permissions_after_removing_users_from_group, get_out_of_scope_users
...
class PermissionGroupUpdateInput(PermissionGroupInput):
    ...
    remove_permissions = NonNullList(PermissionEnum, description='List of permission code names to unassign from this group.', required=False)
    remove_users = NonNullList(graphene.ID, description='List of users to unassign from this group.', required=False)
    remove_channels = NonNullList(graphene.ID, description='List of channels to unassign from this group.' + ADDED_IN_314 + PREVIEW_FEATURE)
    restricted_access_to_channels = graphene.Boolean(description='Determine if the group has restricted access to channels.' + ADDED_IN_314 + PREVIEW_FEATURE, required=False)
...
class PermissionGroupUpdate(PermissionGroupCreate):
    ...
    class Arguments:
        ...
        input = PermissionGroupUpdateInput(description='Input fields to create permission group.', required=True)
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.PERMISSION_GROUP_UPDATED)]
