...
class OrganizationIssuesResolvedInReleaseEndpointTest(APITestCase, SnubaTestCase):
    ...
    def build_grouplink(self, group=None):
        ...
        commit_2 = Commit.objects.create(organization_id=self.org.id, repository_id=repo.id, key=uuid1().hex)
        ReleaseCommit.objects.create(organization_id=self.org.id, release=self.release, commit=commit, order=commit.id)
        ReleaseCommit.objects.create(organization_id=self.org.id, release=self.release, commit=commit_2, order=commit_2.id)
    ...
    def test_multiple_projects(self):
        ...
        self.run_test([self.group, self.group_2], project_ids=[self.group.project_id, self.group_2.project_id])
    ...
    def test_multiple_envs_projects(self):
        ...
        self.build_grouplink()
        self.build_grouplink(self.group_2)
        self.run_test([self.group], project_ids=[self.group.project_id], environment_names=[self.environment.name, self.environment_2.name])