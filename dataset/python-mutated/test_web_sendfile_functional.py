...
async def test_static_file_ok(aiohttp_client: Any, app_with_static_route: web.Application) -> None:
...
async def test_zero_bytes_file_mocked_native_sendfile(aiohttp_client: Any, loop_with_mocked_native_sendfile: Any) -> None:
    filepath = pathlib.Path(__file__).parent / 'data.zero_bytes'
...
async def test_static_file_ok_string_path(aiohttp_client: Any, app_with_static_route: web.Application) -> None:
    client = await aiohttp_client(app_with_static_route)
    resp = await client.get('/')
    assert resp.status == 200
    txt = await resp.text()
...
async def test_static_file_custom_content_type(aiohttp_client: Any, sender: Any) -> None:
...
async def test_static_file_custom_content_type_compress(aiohttp_client: Any, sender: Any):
...
async def test_static_file_with_content_encoding(aiohttp_client: Any, sender: Any) -> None:
    filepath = pathlib.Path(__file__).parent / 'hello.txt.gz'
...
async def test_static_file_if_modified_since(aiohttp_client: Any, app_with_static_route: web.Application) -> None:
    client = await aiohttp_client(app_with_static_route)
    resp = await client.get('/')
    assert 200 == resp.status
...
async def test_static_file_if_modified_since_past_date(aiohttp_client: Any, app_with_static_route: web.Application) -> None:
    client = await aiohttp_client(app_with_static_route)
...
async def test_static_file_if_modified_since_invalid_date(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
...
async def test_static_file_if_modified_since_future_date(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
...
async def test_static_file_if_match(aiohttp_client: Any, app_with_static_route: web.Application, if_unmodified_since: str) -> None:
    client = await aiohttp_client(app_with_static_route)
    resp = await client.get('/')
    assert 200 == resp.status
...
@pytest.mark.parametrize('if_unmodified_since', ('', 'Fri, 31 Dec 0000 23:59:59 GMT'))
@pytest.mark.parametrize('etags,expected_status', [(('*',), 200), (('"example-tag"', 'W/"weak-tag"'), 412)])
async def test_static_file_if_match_custom_tags(aiohttp_client: Any, app_with_static_route: web.Application, if_unmodified_since: str, etags: Iterable[str], expected_status: Iterable[int]) -> None:
    client = await aiohttp_client(app_with_static_route)
    if_match = ', '.join(etags)
...
@pytest.mark.parametrize('additional_etags', ((), ('"some-other-strong-etag"', 'W/"weak-tag"', 'invalid-tag')))
async def test_static_file_if_none_match(aiohttp_client: Any, app_with_static_route: web.Application, if_modified_since: str, additional_etags: Iterable[str]) -> None:
    client = await aiohttp_client(app_with_static_route)
    resp = await client.get('/')
    assert 200 == resp.status
    original_etag = resp.headers.get('ETag')
    ...
    resp = await client.get('/', headers={'If-None-Match': etag, 'If-Modified-Since': if_modified_since})
    body = await resp.read()
    assert 304 == resp.status
    assert resp.headers.get('Content-Length') is None
...
async def test_static_file_if_none_match_star(aiohttp_client: Any, app_with_static_route: web.Application) -> None:
    client = await aiohttp_client(app_with_static_route)
...
async def test_static_file_ssl(aiohttp_server: Any, ssl_ctx: Any, aiohttp_client: Any, client_ssl_ctx: Any) -> None:
...
async def test_static_file_huge(aiohttp_client: Any, tmp_path: Any) -> None:
    ...
    with file_path.open('wb') as f:
        for i in range(1024 * 20):
            f.write((chr(i % 64 + 32) * 1024).encode())
...
async def test_static_file_range(aiohttp_client: Any, sender: Any) -> None:
    ...
    with filepath.open('rb') as f:
        content = f.read()
    responses = await asyncio.gather(client.get('/', headers={'Range': 'bytes=0-999'}), client.get('/', headers={'Range': 'bytes=1000-1999'}), client.get('/', headers={'Range': 'bytes=2000-'}))
    assert len(responses) == 3
    assert responses[0].status == 206, "failed 'bytes=0-999': %s" % responses[0].reason
    assert responses[0].headers['Content-Range'] == 'bytes 0-999/{}'.format(filesize), 'failed: Content-Range Error'
    assert responses[1].status == 206, "failed 'bytes=1000-1999': %s" % responses[1].reason
    assert responses[1].headers['Content-Range'] == 'bytes 1000-1999/{}'.format(filesize), 'failed: Content-Range Error'
    assert responses[2].status == 206, "failed 'bytes=2000-': %s" % responses[2].reason
    assert responses[2].headers['Content-Range'] == 'bytes 2000-{}/{}'.format(filesize - 1, filesize), 'failed: Content-Range Error'
    body = await asyncio.gather(*(resp.read() for resp in responses))
    assert len(body[0]) == 1000, "failed 'bytes=0-999', received %d bytes" % len(body[0])
    assert len(body[1]) == 1000, "failed 'bytes=1000-1999', received %d bytes" % len(body[1])
    responses[0].close()
    responses[1].close()
    responses[2].close()
...
async def test_static_file_range_end_bigger_than_size(aiohttp_client: Any, sender: Any):
    ...
    with filepath.open('rb') as f:
        ...
        assert response.headers['Content-Range'] == 'bytes 54000-54996/54997', 'failed: Content-Range Error'
...
async def test_static_file_range_beyond_eof(aiohttp_client: Any, sender: Any) -> None:
    ...
    assert response.status == 416, "failed 'bytes=1000000-1200000': %s" % response.reason
    await response.release()
    await client.close()
...
async def test_static_file_range_tail(aiohttp_client: Any, sender: Any) -> None:
    ...
    assert resp.headers['Content-Range'] == 'bytes 54497-54996/54997', 'failed: Content-Range Error'
    ...
    assert resp2.headers['Content-Range'] == 'bytes 0-54996/54997', 'failed: Content-Range Error'
    await resp2.release()
    await client.close()
...
async def test_static_file_if_unmodified_since_past_with_range(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'Mon, 1 Jan 1990 01:01:01 GMT'
    resp = await client.get('/', headers={'If-Unmodified-Since': lastmod, 'Range': 'bytes=2-'})
    assert 412 == resp.status
    resp.close()
    await resp.release()
    await client.close()

async def test_static_file_if_unmodified_since_future_with_range(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'Fri, 31 Dec 9999 23:59:59 GMT'
    resp = await client.get('/', headers={'If-Unmodified-Since': lastmod, 'Range': 'bytes=2-'})
...
async def test_static_file_if_range_past_with_range(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'Mon, 1 Jan 1990 01:01:01 GMT'
...
async def test_static_file_if_range_future_with_range(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'Fri, 31 Dec 9999 23:59:59 GMT'
    resp = await client.get('/', headers={'If-Range': lastmod, 'Range': 'bytes=2-'})
    assert 206 == resp.status
...
async def test_static_file_if_unmodified_since_past_without_range(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'Mon, 1 Jan 1990 01:01:01 GMT'
...
async def test_static_file_if_unmodified_since_future_without_range(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'Fri, 31 Dec 9999 23:59:59 GMT'
    resp = await client.get('/', headers={'If-Unmodified-Since': lastmod})
    assert 200 == resp.status
...
async def test_static_file_if_range_past_without_range(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'Mon, 1 Jan 1990 01:01:01 GMT'
...
async def test_static_file_if_range_future_without_range(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'Fri, 31 Dec 9999 23:59:59 GMT'
    resp = await client.get('/', headers={'If-Range': lastmod})
    assert 200 == resp.status
...
async def test_static_file_if_unmodified_since_invalid_date(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'not a valid HTTP-date'
    resp = await client.get('/', headers={'If-Unmodified-Since': lastmod})
    assert 200 == resp.status
...
async def test_static_file_if_range_invalid_date(aiohttp_client: Any, app_with_static_route: web.Application):
    client = await aiohttp_client(app_with_static_route)
    lastmod = 'not a valid HTTP-date'
    resp = await client.get('/', headers={'If-Range': lastmod})
    assert 200 == resp.status
