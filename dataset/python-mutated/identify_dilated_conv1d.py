...
def make_identify_dilated_conv1d_tests(options):
    ...
    test_parameters = [{'input_shape': [[1, 3, 3], [4, 6, 1]], 'filter_size': [1, 2, 3], 'stride': [1, 2], 'dilations': [1, 2, 3], 'padding': ['VALID', 'SAME'], 'num_filters': [1, 2]}]
    ...
    def build_graph(parameters):
        ...
        out = tf.nn.conv1d(input=input_tensor, filters=filter_input, stride=parameters['stride'], dilations=parameters['dilations'], padding=parameters['padding'])
    ...
    def build_inputs(parameters, sess, inputs, outputs):
        if False:
            ...
            return i + 15
        (input_shape, filter_shape) = get_tensor_shapes(parameters)
        values = [create_tensor_data(np.float32, input_shape, min_value=-1, max_value=1), create_tensor_data(np.float32, filter_shape)]
