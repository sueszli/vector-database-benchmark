...
class JupyterHubLoginHandler(web.RequestHandler):
    ...
    async def token_for_code(self, code):
        ...
        params = dict(client_id=self.settings['client_id'], client_secret=self.settings['api_token'], grant_type='authorization_code', code=code, redirect_uri=self.settings['redirect_uri'])
        req = HTTPRequest(self.settings['token_url'], method='POST', body=urlencode(params).encode('utf8'), headers={'Content-Type': 'application/x-www-form-urlencoded'})
    ...
    async def get(self):
        ...
        if code:
            ...
            self.redirect(url_concat(self.settings['authorize_url'], dict(redirect_uri=self.settings['redirect_uri'], client_id=self.settings['client_id'], response_type='code')))
...
class WhoAmIHandler(web.RequestHandler):
    ...
    async def user_for_token(self, token):
        ...
        req = HTTPRequest(self.settings['user_url'], headers={'Authorization': f'token {token}'})
        response = await AsyncHTTPClient().fetch(req)
