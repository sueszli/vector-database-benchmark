...
special_types = set()
IntegerValue = [int(0), long(1)] + [_dt(0) for _dt in np.sctypes['int'] + np.sctypes['uint'] + [np.bool, bool, np.bool_]]
...
StringValue = [str('bork'), unicode('bork'), b'bork', b''] + [_dt('bork') for _dt in [np.unicode, np.unicode_, str, unicode, np.str, np.str_, np.string_]] + [str(''), unicode('')] + [_dt('') for _dt in [np.unicode, np.unicode_, str, unicode, np.str, np.str_, np.string_]]
special_types.add(id(StringValue))
DictValue = [{'a': 12}, dict()]
special_types.add(id(DictValue))
DatetimeValue = [datetime.date(2000, 6, 12), datetime.date(1100, 1, 1), datetime.datetime(2000, 6, 12)]
...
FloatSequence = [[0.5, 1.5, 2.5], (0.5, 1.5, 2.5), {0.5, 1.5, 2.5}, frozenset([0.5, 1.5, 2.5])] + [array.array(c, [0.5, 1.5, 2.5]) for c in 'fd']
special_types.add(id(FloatSequence))
FloatSequenceWithNAN = [[0.5, 1.5, 2.5, nan], (0.5, 1.5, 2.5, nan), {0.5, 1.5, 2.5, nan}, frozenset([0.5, 1.5, 2.5, nan])] + [array.array(c, [0.5, 1.5, 2.5, nan]) for c in 'fd']
...
IntegerSequence = [[int(i) for i in range(3)], [long(i) for i in range(3)], tuple(range(3)), tuple((long(i) for i in range(3))), set(range(3)), frozenset(range(3))] + [array.array(c, range(3)) for c in 'bBhHiIlL']
special_types.add(id(IntegerSequence))
IntegerSequenceWithNAN = [[int(i) for i in range(3)] + [nan], [long(i) for i in range(3)] + [nan], tuple(range(3)) + (nan,), tuple((long(i) for i in range(3))) + (nan,), set([long(i) for i in range(3)] + [nan]), frozenset([long(i) for i in range(3)] + [nan])]
special_types.add(id(IntegerSequenceWithNAN))
IntegerSequenceWithNone = [[int(i) for i in range(3)] + [None], [long(i) for i in range(3)] + [None], tuple(range(3)) + (None,), tuple((long(i) for i in range(3))) + (None,), set([long(i) for i in range(3)] + [None]), frozenset([long(i) for i in range(3)] + [None])]
...
BooleanSequence = [list((i % 2 == 0 for i in range(3))), tuple((i % 2 == 0 for i in range(3))), set([True]), set([False]), set([True, False])]
special_types.add(id(BooleanSequence))
StringSequence = [list((str(i) for i in range(3))), tuple((str(i) for i in range(3))), set((str(i) for i in range(3))), frozenset((str(i) for i in range(3)))]
special_types.add(id(StringSequence))
AnySequence = EmptySequence + BooleanSequence + StringSequence + IntegerSequence + IntegerSequenceWithNone + IntegerSequenceWithNAN + FloatSequence + FloatSequenceWithNone + FloatSequenceWithNAN + EmptyArray
