...
__all__ = ['SeparableConv1d', 'SeparableConv2d']
...
class SeparableConv1d(Layer):
    ...
    def __init__(self, n_filter=100, filter_size=3, strides=1, act=None, padding='valid', data_format='channels_last', dilation_rate=1, depth_multiplier=1, depthwise_init=None, pointwise_init=None, b_init=tl.initializers.constant(value=0.0), in_channels=None, name=None):
        if False:
            i = 10
            return i + 15
        ...
        logging.info('SeparableConv1d  %s: n_filter: %d filter_size: %s strides: %s depth_multiplier: %d act: %s' % (self.name, n_filter, str(filter_size), str(strides), depth_multiplier, self.act.__name__ if self.act is not None else 'No Activation'))
    ...
    def __repr__(self):
        ...
        s = '{classname}(in_channels={in_channels}, out_channels={n_filter}, kernel_size={filter_size}, stride={strides}, padding={padding}'
...
class SeparableConv2d(Layer):
    ...
    def __init__(self, n_filter=100, filter_size=(3, 3), strides=(1, 1), act=None, padding='valid', data_format='channels_last', dilation_rate=(1, 1), depth_multiplier=1, depthwise_init=None, pointwise_init=None, b_init=tl.initializers.constant(value=0.0), in_channels=None, name=None):
        if False:
            return 10
        super().__init__(name, act=act)
        self.n_filter = n_filter
        ...
        self.b_init = b_init
        self.in_channels = in_channels
        logging.info('SeparableConv2d  %s: n_filter: %d filter_size: %s filter_size: %s depth_multiplier: %d act: %s' % (self.name, n_filter, str(filter_size), str(strides), depth_multiplier, self.act.__name__ if self.act is not None else 'No Activation'))
    ...
    def build(self, inputs_shape):
        if False:
            return 10
        self.layer = tf.keras.layers.SeparableConv2D(filters=self.n_filter, kernel_size=self.filter_size, strides=self.strides, padding=self.padding, data_format=self.data_format, dilation_rate=self.dilation_rate, depth_multiplier=self.depth_multiplier, activation=self.act, use_bias=True if self.b_init is not None else False, depthwise_initializer=self.depthwise_init, pointwise_initializer=self.pointwise_init, bias_initializer=self.b_init, trainable=True, name=self.name)
        if self.data_format == 'channels_first':
            self.in_channels = inputs_shape[1]
        else:
            self.in_channels = inputs_shape[-1]
