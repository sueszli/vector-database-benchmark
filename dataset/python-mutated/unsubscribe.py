...
class UnsubscribeResource(DirectServeHtmlResource):
    ...
    async def _async_render_GET(self, request: SynapseRequest) -> None:
        ...
        user_id = self.macaroon_generator.verify_delete_pusher_token(token, app_id, pushkey)
        try:
            await self.pusher_pool.remove_pusher(app_id=app_id, pushkey=pushkey, user_id=user_id)
        except StoreError as se:
            if se.code != 404:
                raise
        self.notifier.on_new_replication_data()
        respond_with_html_bytes(request, 200, UnsubscribeResource.SUCCESS_HTML)
