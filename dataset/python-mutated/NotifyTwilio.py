...
class NotifyTwilio(NotifyBase):
    ...
    notify_url = 'https://api.twilio.com/2010-04-01/Accounts/{sid}/Messages.json'
    body_maxlen = 160
    title_maxlen = 0
    templates = ('{schema}://{account_sid}:{auth_token}@{from_phone}', '{schema}://{account_sid}:{auth_token}@{from_phone}/{targets}')
    template_tokens = dict(NotifyBase.template_tokens, **{'account_sid': {'name': _('Account SID'), 'type': 'string', 'private': True, 'required': True, 'regex': ('^AC[a-f0-9]+$', 'i')}, 'auth_token': {'name': _('Auth Token'), 'type': 'string', 'private': True, 'required': True, 'regex': ('^[a-z0-9]+$', 'i')}, 'from_phone': {'name': _('From Phone No'), 'type': 'string', 'required': True, 'regex': ('^\\+?[0-9\\s)(+-]+$', 'i'), 'map_to': 'source'}, 'target_phone': {'name': _('Target Phone No'), 'type': 'string', 'prefix': '+', 'regex': ('^[0-9\\s)(+-]+$', 'i'), 'map_to': 'targets'}, 'short_code': {'name': _('Target Short Code'), 'type': 'string', 'regex': ('^[0-9]{5,6}$', 'i'), 'map_to': 'targets'}, 'targets': {'name': _('Targets'), 'type': 'list:string'}})
    template_args = dict(NotifyBase.template_args, **{'to': {'alias_of': 'targets'}, 'from': {'alias_of': 'from_phone'}, 'sid': {'alias_of': 'account_sid'}, 'token': {'alias_of': 'auth_token'}, 'apikey': {'name': _('API Key'), 'type': 'string', 'private': True, 'regex': ('^SK[a-f0-9]+$', 'i')}})
