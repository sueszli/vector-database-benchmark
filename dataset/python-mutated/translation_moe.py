...
class TranslationMoEConfig(TranslationConfig):
    method: METHOD_CHOICES = field(default='hMoEup', metadata={'help': 'MoE method'})
    num_experts: int = field(default=3, metadata={'help': 'number of experts'})
    mean_pool_gating_network: bool = field(default=False, metadata={'help': 'use a simple mean-pooling gating network'})
    mean_pool_gating_network_dropout: float = field(default=0, metadata={'help': 'dropout for mean-pooling gating network'})
    mean_pool_gating_network_encoder_dim: int = field(default=0, metadata={'help': 'encoder output dim for mean-pooling gating network'})
    gen_expert: int = field(default=0, metadata={'help': 'which expert to use for generation'})
