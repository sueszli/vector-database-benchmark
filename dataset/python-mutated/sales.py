...
from ..dataloaders import PredicateByPromotionIdLoader, PromotionRulesByPromotionIdAndChannelSlugLoader, PromotionRulesByPromotionIdLoader, SaleChannelListingByPromotionIdLoader
...
class SaleChannelListing(BaseObjectType):
    ...
    channel = graphene.Field(Channel, required=True, description='The channel in which the sale is available.')
    discount_value = graphene.Float(required=True, description='The value of the discount applied to the sale in the channel.')
    currency = graphene.String(required=True, description='The currency in which the discount value is specified.')

    class Meta:
        description = 'Represents sale channel listing.' + DEPRECATED_IN_3X_TYPE + ' Use `PromotionRule` type instead.'
...
class Sale(ChannelContextTypeWithMetadata, ModelObjectType[models.Promotion]):
    ...
    start_date = graphene.DateTime(required=True, description='The start date and time of the sale.')
    end_date = graphene.DateTime(description='The end date and time of the sale.')
    created = graphene.DateTime(required=True, description='The date and time when the sale was created.')
    updated_at = graphene.DateTime(required=True, description='The date and time when the sale was updated.')
    categories = ConnectionField(CategoryCountableConnection, description='List of categories this sale applies to.')
    collections = ConnectionField(CollectionCountableConnection, description='List of collections this sale applies to.', permissions=[DiscountPermissions.MANAGE_DISCOUNTS])
    products = ConnectionField(ProductCountableConnection, description='List of products this sale applies to.', permissions=[DiscountPermissions.MANAGE_DISCOUNTS])
    variants = ConnectionField(ProductVariantCountableConnection, description='List of product variants this sale applies to.' + ADDED_IN_31, permissions=[DiscountPermissions.MANAGE_DISCOUNTS])
    translation = TranslationField(SaleTranslation, type_name='sale', resolver=ChannelContextType.resolve_translation)
    channel_listings = PermissionsField(NonNullList(SaleChannelListing), description='List of channels available for the sale.', permissions=[DiscountPermissions.MANAGE_DISCOUNTS])
    ...
    class Meta:
        ...
        description = 'Sales allow creating discounts for categories, collections or products and are visible to all the customers.' + DEPRECATED_IN_3X_TYPE + ' Use `Promotion` type instead.'
