...
def pattern_recognition(candles: np.ndarray, pattern_type: str, penetration: int=0, sequential: bool=False) -> Union[int, np.ndarray]:
    if False:
        return 10
    ...
    if pattern_type == 'CDL2CROWS':
        ...
        res = talib.CDLABANDONEDBABY(candles[:, 1], candles[:, 3], candles[:, 4], candles[:, 2], penetration=penetration)
        ...
        res = talib.CDLDARKCLOUDCOVER(candles[:, 1], candles[:, 3], candles[:, 4], candles[:, 2], penetration=penetration)
        ...
        res = talib.CDLEVENINGDOJISTAR(candles[:, 1], candles[:, 3], candles[:, 4], candles[:, 2], penetration=penetration)
        ...
        res = talib.CDLMORNINGDOJISTAR(candles[:, 1], candles[:, 3], candles[:, 4], candles[:, 2], penetration=penetration)
