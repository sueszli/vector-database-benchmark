...
class AutomaticSpeechRecognitionTest(unittest.TestCase):
    ...
    def test_run_with_pcm(self):
        ...
        rec_result = self.run_pipeline(model_id=self.am_tf_model_id, audio_in=audio, sr=sr)
        self.check_result('test_run_with_pcm_tf', rec_result)
        logger.info('Run ASR test with wav data (pytorch)...')
        rec_result = self.run_pipeline(model_id=self.am_pytorch_model_id, audio_in=audio, sr=sr)
    ...
    def test_run_with_wav(self):
        ...
        rec_result = self.run_pipeline(model_id=self.am_tf_model_id, audio_in=wav_file_path)
        self.check_result('test_run_with_wav_tf', rec_result)
        logger.info('Run ASR test with waveform file (pytorch)...')
        rec_result = self.run_pipeline(model_id=self.am_pytorch_model_id, audio_in=wav_file_path)
    ...
    def test_run_with_url(self):
        ...
        rec_result = self.run_pipeline(model_id=self.am_tf_model_id, audio_in=URL_FILE)
        self.check_result('test_run_with_url_tf', rec_result)
        logger.info('Run ASR test with url file (pytorch)...')
        rec_result = self.run_pipeline(model_id=self.am_pytorch_model_id, audio_in=URL_FILE)
    ...
    def test_run_with_funasr(self):
        ...
        rec_result = self.run_pipeline(model_id=self.am_funasr_model_id, audio_in=URL_FILE)
    ...
    def test_run_with_wav_dataset_pytorch(self):
        ...
        dataset_path = download_and_untar(os.path.join(self.workspace, LITTLE_TESTSETS_FILE), LITTLE_TESTSETS_URL, self.workspace)
        dataset_path = os.path.join(dataset_path, 'wav', 'test')
        logger.info('Run ASR test with waveform dataset (tensorflow)...')
        rec_result = self.run_pipeline(model_id=self.am_tf_model_id, audio_in=dataset_path)
        self.check_result('test_run_with_wav_dataset_tf', rec_result)
        logger.info('Run ASR test with waveform dataset (pytorch)...')
        rec_result = self.run_pipeline(model_id=self.am_pytorch_model_id, audio_in=dataset_path)
    ...
    def test_run_with_all_models(self):
        ...
        for item in self.all_models_info:
            ...
            rec_result = self.run_pipeline(model_id=model_id, audio_in=wav_path)
            if rec_result.__contains__(OutputKeys.TEXT):
                logger.info(ColorCodes.MAGENTA + str(item['model_id']) + ' ' + ColorCodes.YELLOW + str(rec_result[OutputKeys.TEXT]) + ColorCodes.END)
            else:
                logger.info(ColorCodes.MAGENTA + str(rec_result) + ColorCodes.END)
