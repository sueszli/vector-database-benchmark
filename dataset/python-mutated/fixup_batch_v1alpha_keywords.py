...
class batchCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_job': ('parent', 'job', 'job_id', 'request_id'), 'delete_job': ('name', 'reason', 'request_id'), 'get_job': ('name',), 'get_task': ('name',), 'list_jobs': ('parent', 'filter', 'order_by', 'page_size', 'page_token'), 'list_tasks': ('parent', 'filter', 'order_by', 'page_size', 'page_token')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=batchCallTransformer()):
    if False:
        while True:
            i = 10
