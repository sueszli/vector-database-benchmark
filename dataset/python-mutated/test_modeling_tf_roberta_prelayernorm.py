...
class TFRobertaPreLayerNormModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TFRobertaPreLayerNormModel, TFRobertaPreLayerNormForCausalLM, TFRobertaPreLayerNormForMaskedLM, TFRobertaPreLayerNormForSequenceClassification, TFRobertaPreLayerNormForTokenClassification, TFRobertaPreLayerNormForQuestionAnswering) if is_tf_available() else ()
    pipeline_model_mapping = {'feature-extraction': TFRobertaPreLayerNormModel, 'fill-mask': TFRobertaPreLayerNormForMaskedLM, 'question-answering': TFRobertaPreLayerNormForQuestionAnswering, 'text-classification': TFRobertaPreLayerNormForSequenceClassification, 'text-generation': TFRobertaPreLayerNormForCausalLM, 'token-classification': TFRobertaPreLayerNormForTokenClassification, 'zero-shot': TFRobertaPreLayerNormForSequenceClassification} if is_tf_available() else {}
...
class TFRobertaPreLayerNormModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_masked_lm(self):
        ...
        EXPECTED_SLICE = tf.constant([[[40.488, 18.0199, -5.2367], [-1.8877, -4.0885, 10.7085], [-2.2613, -5.611, 7.2665]]])
    ...
    def test_inference_no_head(self):
        ...
        input_ids = tf.constant([[0, 31414, 232, 328, 740, 1140, 12695, 69, 46078, 1588, 2]])
        output = model(input_ids)[0]
        EXPECTED_SLICE = tf.constant([[[0.0208, -0.0356, 0.0237], [-0.1569, -0.0411, -0.2626], [0.1879, 0.0125, -0.0089]]])
        self.assertTrue(numpy.allclose(output[:, :3, :3].numpy(), EXPECTED_SLICE.numpy(), atol=0.0001))