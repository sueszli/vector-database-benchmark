...
from zigpy.config import CONF_DATABASE, CONF_DEVICE, CONF_DEVICE_PATH, CONF_NWK_BACKUP_ENABLED
from zigpy.exceptions import NetworkNotFormed
from homeassistant import config_entries
from homeassistant.components import usb
from homeassistant.core import HomeAssistant
from . import repairs
from .core.const import CONF_RADIO_TYPE, CONF_ZIGPY, DEFAULT_DATABASE_NAME, EZSP_OVERWRITE_EUI64, RadioType
from .core.helpers import get_zha_data
AUTOPROBE_RADIOS = (RadioType.ezsp, RadioType.znp, RadioType.deconz, RadioType.zigate)
RECOMMENDED_RADIOS = (RadioType.ezsp, RadioType.znp, RadioType.deconz)
CONNECT_DELAY_S = 1.0
RETRY_DELAY_S = 1.0
BACKUP_RETRIES = 5
MIGRATION_RETRIES = 100
HARDWARE_DISCOVERY_SCHEMA = vol.Schema({vol.Required('name'): str, vol.Required('port'): dict, vol.Required('radio_type'): str})
HARDWARE_MIGRATION_SCHEMA = vol.Schema({vol.Required('new_discovery_info'): HARDWARE_DISCOVERY_SCHEMA, vol.Required('old_discovery_info'): vol.Schema({vol.Exclusive('hw', 'discovery'): HARDWARE_DISCOVERY_SCHEMA, vol.Exclusive('usb', 'discovery'): usb.UsbServiceInfo})})
...
class ZhaRadioManager:
    ...
    async def connect_zigpy_app(self) -> ControllerApplication:
        ...
        database_path = config.get(CONF_DATABASE, self.hass.config.path(DEFAULT_DATABASE_NAME))
        ...
        app = await self.radio_type.controller.new(app_config, auto_form=False, start_radio=False)
        try:
            yield app
        finally:
            await app.shutdown()
    ...
    async def restore_backup(self, backup: zigpy.backups.NetworkBackup, **kwargs: Any) -> None:
        """Restore the provided network backup, passing through kwargs."""
        if self.current_settings is not None and self.current_settings.supersedes(self.chosen_backup):
            return
    ...
    async def detect_radio_type(self) -> ProbeResult:
        ...
        for radio in AUTOPROBE_RADIOS:
            ...
            dev_config = radio.controller.SCHEMA_DEVICE({CONF_DEVICE_PATH: self.device_path})
        ...
        with suppress(repairs.wrong_silabs_firmware.AlreadyRunningEZSP):
            if await repairs.wrong_silabs_firmware.warn_on_wrong_silabs_firmware(self.hass, self.device_path):
                return ProbeResult.WRONG_FIRMWARE_INSTALLED
        return ProbeResult.PROBING_FAILED

    async def async_load_network_settings(self, *, create_backup: bool=False) -> zigpy.backups.NetworkBackup | None:
        ...
        async with self.connect_zigpy_app() as app:
            ...
            try:
                ...
                self.current_settings = zigpy.backups.NetworkBackup(network_info=app.state.network_info, node_info=app.state.node_info)
                if create_backup:
    ...
    async def async_restore_backup_step_1(self) -> bool:
        ...
        if self.current_settings.node_info.ieee == self.chosen_backup.node_info.ieee or metadata['can_rewrite_custom_eui64'] or (not metadata['can_burn_userdata_custom_eui64']):
            await self.restore_backup(self.chosen_backup)
            return False
        return True
...
class ZhaMultiPANMigrationHelper:
    ...
    async def async_initiate_migration(self, data: dict[str, Any]) -> bool:
        ...
        new_radio_type = ZhaRadioManager.parse_radio_type(migration_data['new_discovery_info']['radio_type'])
        new_device_settings = new_radio_type.controller.SCHEMA_DEVICE(migration_data['new_discovery_info']['port'])
        if 'hw' in migration_data['old_discovery_info']:
            ...
            old_device_path = await self._hass.async_add_executor_job(usb.get_serial_by_id, device)
        ...
        for retry in range(BACKUP_RETRIES):
            try:
                ...
                if retry >= BACKUP_RETRIES - 1:
                    raise
                _LOGGER.debug('Failed to create backup %r, retrying in %s seconds', err, RETRY_DELAY_S)
        ...
        self._hass.config_entries.async_update_entry(entry=self._config_entry, data={CONF_DEVICE: device_settings, CONF_RADIO_TYPE: self._radio_mgr.radio_type.name}, options=self._config_entry.options, title=name)
        return True
        ...
        for retry in range(MIGRATION_RETRIES):
            try:
                ...
                if retry >= MIGRATION_RETRIES - 1:
                    raise
                _LOGGER.debug('Failed to restore backup %r, retrying in %s seconds', err, RETRY_DELAY_S)
            await asyncio.sleep(RETRY_DELAY_S)
