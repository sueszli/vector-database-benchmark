...
from transformers.testing_utils import is_pt_tf_cross_test, require_tensorflow_probability, require_tf, require_vision, slow
...
class TFGroupViTVisionModelTester:
    ...
    def __init__(self, parent, batch_size=12, image_size=30, patch_size=2, num_channels=3, is_training=True, hidden_size=32, depths=[6, 3, 3], num_group_tokens=[64, 8, 0], num_output_groups=[64, 8, 8], num_attention_heads=4, intermediate_size=37, dropout=0.1, attention_dropout=0.1, initializer_range=0.02, scope=None):
        if False:
            for i in range(10):
                print('nop')
        self.parent = parent
    ...
    def create_and_check_model(self, config, pixel_values):
        ...
        self.parent.assertEqual(result.last_hidden_state.shape, (self.batch_size, self.num_output_groups[-1], self.hidden_size))
...
class TFGroupViTVisionModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def check_pt_tf_outputs(self, tf_outputs, pt_outputs, model_class, tol=0.0001, name='outputs', attributes=None):
        if False:
            return 10
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=GroupViTVisionConfig, has_text_modality=False, hidden_size=37)
        ...
        for model_class in self.all_model_classes:
            ...
            for (i, self_attn) in enumerate(self_attentions):
                if self_attn is None:
                    continue
                self.assertListEqual(list(self_attentions[i].shape[-2:]), [self.model_tester.num_output_groups[i], self.model_tester.num_output_groups[i - 1] if i > 0 else seq_len])
    ...
    @unittest.skip('TFGroupViTVisionModel does not convert `hidden_states` and `attentions` to tensors as they are all of different dimensions, and we get `Got a non-Tensor value` error when saving the model.')
    @slow
...
class TFGroupViTModelTester:
    ...
    def get_config(self):
        if False:
            print('Hello World!')
        return GroupViTConfig.from_text_vision_configs(self.text_model_tester.get_config(), self.vision_model_tester.get_config(), projection_dim=64)
