...
def add_default_shipping(apps, schema_editor):
    ...
    if not ShippingZone.objects.all().exists() and settings.POPULATE_DEFAULTS:
        ...
        shipping_method = ShippingMethod.objects.create(name='Default', type=ShippingMethodType.PRICE_BASED, shipping_zone=shipping_zone)
        if warehouse:
            shipping_zone.warehouses.add(warehouse)
        if channel:
            shipping_zone.channels.add(channel)
            shipping_method.channel_listings.create(channel=channel, currency=channel.currency_code, price_amount=0)

class Migration(migrations.Migration):
    dependencies = [('channel', '0004_create_default_channel'), ('shipping', '0032_shippingmethod_tax_class'), ('warehouse', '0031_create_default_warehouse')]
