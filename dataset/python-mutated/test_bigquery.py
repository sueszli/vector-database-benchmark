...
VIEW_DEFINITION = {'query': f'SELECT * FROM `{TEST_DATASET}.{TEST_TABLE_ID}`', 'useLegacySql': False}
MATERIALIZED_VIEW_DEFINITION = {'query': f'SELECT product, SUM(amount) FROM `{TEST_DATASET}.{TEST_TABLE_ID}` GROUP BY product', 'enableRefresh': True, 'refreshIntervalMs': 2000000}
...
@pytest.mark.parametrize('operator_class, kwargs', [(BigQueryCheckOperator, dict(sql='Select * from test_table')), (BigQueryValueCheckOperator, dict(sql='Select * from test_table', pass_value=95)), (BigQueryIntervalCheckOperator, dict(table=TEST_TABLE_ID, metrics_thresholds={'COUNT(*)': 1.5}))])
