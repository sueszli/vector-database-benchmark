...
SEVERITY_MATCH_CHOICES = {MatchType.GREATER_OR_EQUAL: 'greater than or equal to', MatchType.LESS_OR_EQUAL: 'less than or equal to'}
...
class IssueSeverityFilter(EventFilter):
    ...
    form_fields = {'value': {'type': 'number', 'placeholder': 0.5}, 'match': {'type': 'choice', 'choices': list(SEVERITY_MATCH_CHOICES.items())}}
    ...
    def _passes(self, group: Optional[Group]) -> bool:
        if False:
            while True:
                i = 10
        has_issue_severity_alerts = features.has('projects:first-event-severity-alerting', self.project)
