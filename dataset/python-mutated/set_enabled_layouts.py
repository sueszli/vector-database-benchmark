...
class SetEnabledLayouts(RemoteCommand):
    ...
    desc = 'Set the enabled layouts in the specified tabs (or the active tab if not specified). You can use special match value :code:`all` to set the enabled layouts in all tabs. If the current layout of the tab is not included in the enabled layouts, its layout is changed to the first enabled layout.'
    options_spec = MATCH_TAB_OPTION + '\n\n\n--configured\ntype=bool-set\nChange the default enabled layout value so that the new value takes effect for all newly created tabs\nas well.\n'
    args = RemoteCommand.Args(spec='LAYOUT ...', minimum_count=1, json_field='layouts', completion=RemoteCommand.CompletionSpec.from_string('type:keyword group:"Layout" kwds:' + ','.join(layout_names())), args_choices=layout_names)
