...
class Job(Generic[CCT]):
    ...
    Attributes:
        ...
        user_id (:obj:`int`): Optional. User id of the user that this job is associated with.
            ...
            .. versionadded:: 20.0
    """
    __slots__ = ('callback', 'data', 'name', '_removed', '_enabled', '_job', 'chat_id', 'user_id')
    ...
    async def run(self, application: 'Application[Any, CCT, Any, Any, Any, JobQueue[CCT]]') -> None:
    ...
    async def _run(self, application: 'Application[Any, CCT, Any, Any, Any, JobQueue[CCT]]') -> None:
        try:
            context = application.context_types.context.from_job(self, application)
            await context.refresh_data()
            await self.callback(context)
        except Exception as exc:
            await application.create_task(application.process_error(None, exc, job=self), name=f'Job:{self.id}:run:process_error')
        finally:
