...
class TestFleetExecutor(unittest.TestCase):
    ...
    def run_fleet_executor(self, place, x_data, y_data):
        ...
        with base.program_guard(empty_program, empty_program):
            ...
            opt.minimize(loss)
        task_node = TaskNode(program=empty_program.clone(), rank=0, node_type='Compute', max_run_times=1, lazy_initialize=True)
        empty_program._pipeline_opt = {'fleet_opt': {'tasks': [task_node], 'task_id_to_rank': {task_node.task_id(): 0}}, 'section_program': empty_program}
