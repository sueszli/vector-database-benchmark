...
def comparison_test_dense():
    ...
    if len(ret.nodes) == 1:
        ...
        ntrees = 17
        maxdepth = 5
        h2oParamsD = {'ntrees': ntrees, 'max_depth': maxdepth, 'seed': runSeed, 'learn_rate': 0.7, 'col_sample_rate_per_tree': 0.9, 'min_rows': 5, 'score_tree_interval': ntrees + 1, 'dmatrix_type': 'sparse', 'tree_method': 'exact', 'backend': 'cpu'}
        nativeParam = {'colsample_bytree': h2oParamsD['col_sample_rate_per_tree'], 'tree_method': 'exact', 'seed': h2oParamsD['seed'], 'booster': 'gbtree', 'objective': 'binary:logistic', 'eta': h2oParamsD['learn_rate'], 'grow_policy': 'depthwise', 'alpha': 0.0, 'subsample': 1.0, 'colsample_bylevel': 1.0, 'max_delta_step': 0.0, 'min_child_weight': h2oParamsD['min_rows'], 'gamma': 0.0, 'max_depth': h2oParamsD['max_depth'], 'eval_metric': ['auc', 'aucpr']}
        nrows = 10000
        ncols = 10
        factorL = 11
        numCols = 5
        ...
        responseL = 2
        trainFile = pyunit_utils.genTrainFrame(nrows, numCols, enumCols=enumCols, enumFactors=factorL, miscfrac=0.5, responseLevel=responseL, randseed=dataSeed)
        myX = trainFile.names
        y = 'response'
        myX.remove(y)
        ...
        nativeModel = xgb.train(params=nativeParam, dtrain=nativeTrain, num_boost_round=nrounds, evals=watch_list, verbose_eval=True, evals_result=evals_result)
        modelsfound = False
        ...
        pyunit_utils.summarizeResult_binomial(h2oPredictD, nativePred, h2oTrainTimeD, nativeTrainTime, h2oPredictTimeD, nativeScoreTime, tolerance=testTol)
        ...
        pyunit_utils.summarize_metrics_binomial(h2o_metrics, xgboost_metrics, ['auc', 'aucpr'], tolerance=0.0005)
    else:
