...
from homeassistant.components.climate import PRESET_COMFORT, PRESET_ECO, PRESET_NONE, ClimateEntity, ClimateEntityFeature, HVACMode
...
OVERKIZ_TO_HVAC_MODES: dict[str, HVACMode] = {OverkizCommandParam.ON: HVACMode.HEAT, OverkizCommandParam.COMFORT: HVACMode.HEAT, OverkizCommandParam.OFF: HVACMode.OFF}
HVAC_MODES_TO_OVERKIZ = {v: k for (k, v) in OVERKIZ_TO_HVAC_MODES.items()}
OVERKIZ_TO_PRESET_MODES: dict[str, str] = {OverkizCommandParam.OFF: PRESET_NONE, OverkizCommandParam.FROSTPROTECTION: PRESET_FROST_PROTECTION, OverkizCommandParam.ECO: PRESET_ECO, OverkizCommandParam.COMFORT: PRESET_COMFORT, OverkizCommandParam.COMFORT_1: PRESET_COMFORT1, OverkizCommandParam.COMFORT_2: PRESET_COMFORT2}
...
class AtlanticElectricalHeater(OverkizEntity, ClimateEntity):
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        await self.executor.async_execute_command(OverkizCommand.SET_HEATING_LEVEL, HVAC_MODES_TO_OVERKIZ[hvac_mode])
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        """Set new preset mode."""
        await self.executor.async_execute_command(OverkizCommand.SET_HEATING_LEVEL, PRESET_MODES_TO_OVERKIZ[preset_mode])