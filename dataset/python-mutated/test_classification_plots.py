...
def test_plot():
    ...
    pycaret.classification.setup(data, target='Purchase', log_experiment=True, log_plots=True, html=False, session_id=123, fold=2, n_jobs=1)
    ...
    for model in models:
        for plot in available_shap:
            ...
            pycaret.classification.interpret_model(model, plot=plot, X_new_sample=data.drop('Purchase', axis=1).iloc[:10])
    assert 1 == 1
