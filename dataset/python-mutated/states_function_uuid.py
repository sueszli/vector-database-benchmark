...
class StatesFunctionUUID(StatesFunction):
    ...
    def __init__(self, arg_list: FunctionArgumentList):
        if False:
            return 10
        super().__init__(states_name=StatesFunctionName(function_type=StatesFunctionNameType.UUID), arg_list=arg_list)
        if len(arg_list.arg_list) != 0:
            raise ValueError(f"Expected no arguments for function type '{type(self)}', but got: '{arg_list}'.")
