...
from litestar.repository.testing.generic_mock_repository import GenericAsyncMockRepository, GenericSyncMockRepository
...
async def test_repo_raises_conflict_if_add_with_id(authors: list[UUIDAuthor], author_repository: AuthorRepository) -> None:
    """Test mock repo raises conflict if add identified entity."""
    with pytest.raises(ConflictError):
        await maybe_async(author_repository.add(authors[0]))

async def test_repo_raises_conflict_if_add_many_with_id(authors: list[UUIDAuthor], author_repository: AuthorRepository) -> None:
    """Test mock repo raises conflict if add identified entity."""
    with pytest.raises(ConflictError):
...
async def test_sets_created_updated_on_add(repository_type: type[GenericAsyncMockRepository], audit_model_type: AuditModelType) -> None:
    ...
    instance = await maybe_async(repository_type[audit_model_type]().add(instance))
...
async def test_does_not_set_created_updated(repository_type: type[GenericAsyncMockRepository], model_type: ModelType) -> None:
    """Test that the repository does not update the 'updated' timestamps when
    appropriate."""
    instance = model_type()
    repo = repository_type[model_type]()
    assert 'created_at' not in vars(instance)
    assert 'updated_at' not in vars(instance)
...
async def test_add(repository_type: type[GenericAsyncMockRepository], model_type: ModelType) -> None:
    ...
    instance = model_type()
    inserted_uuid_instance = await maybe_async(repository_type[model_type]().add(instance))
...
async def test_add_many(repository_type: type[GenericAsyncMockRepository], model_type: ModelType) -> None:
    ...
    inserted_uuid_instances = await maybe_async(repository_type[model_type]().add_many(instances))
    assert len(instances) == len(inserted_uuid_instances)

async def test_update(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
    """Test that the repository update method works correctly."""
    Model = create_audit_model_type({'random_column': Mapped[str]})
    mock_repo = repository_type[Model]()
...
async def test_update_many(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
    """Test that the repository add_many method works correctly."""
    Model = create_audit_model_type({'random_column': Mapped[str]})
    mock_repo = repository_type[Model]()
...
async def test_upsert(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
...
async def test_upsert_many(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
    """Test that the repository upsert method works correctly."""
    Model = create_audit_model_type({'random_column': Mapped[str]})
    mock_repo = repository_type[Model]()
    instance = await maybe_async(mock_repo.upsert(Model(random_column='A')))
...
async def test_list(repository_type: type[GenericAsyncMockRepository], audit_model_type: AuditModelType) -> None:
    ...
    mock_repo = repository_type[audit_model_type]()
...
async def test_list_and_count(repository_type: type[GenericAsyncMockRepository], audit_model_type: AuditModelType) -> None:
...
async def test_exists(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
...
async def test_exists_with_filter(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
...
async def test_get(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
...
async def test_get_one(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
...
async def test_get_one_or_none(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
...
async def test_get_or_create(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
    """Test that the repository get_or_create returns a model record correctly."""
    Model = create_audit_model_type({'random_column': Mapped[str], 'cool_attribute': mapped_column(String, nullable=True)})
...
async def test_get_or_create_match_fields(repository_type: type[GenericAsyncMockRepository], create_audit_model_type: CreateAuditModelFixture) -> None:
    """Test that the repository get_or_create returns a model record correctly."""
    Model = create_audit_model_type({'random_column': Mapped[str], 'cool_attribute': mapped_column(String, nullable=True)})
    instances = [Model(random_column='value 1', cool_attribute='yep'), Model(random_column='value 2')]
    mock_repo = repository_type[Model]()
    inserted_instances = await maybe_async(mock_repo.add_many(instances))
    (fetched_instance, fetched_created) = await maybe_async(mock_repo.get_or_create(match_fields=['random_column'], random_column='value 1', cool_attribute='other thing'))
    assert await maybe_async(mock_repo.count()) == 2
    assert inserted_instances[0] == fetched_instance
    assert fetched_created is False