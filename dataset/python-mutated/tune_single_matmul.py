...
_QUANTIZED_LHS = flags.DEFINE_enum_class('quantized_lhs', QuantizedInputType.BFLOAT16, QuantizedInputType, 'Type to use for LHS quantization')
_QUANTIZED_RHS = flags.DEFINE_enum_class('quantized_rhs', QuantizedInputType.BFLOAT16, QuantizedInputType, 'Type to use for RHS quantization')
_TILINGS_M = flags.DEFINE_string('tilings_m', '32, 64, 128, 256', 'Tilings to try for M')
_TILINGS_N = flags.DEFINE_string('tilings_n', '32, 64, 128, 256', 'Tilings to try for N')
_TILINGS_K = flags.DEFINE_string('tilings_k', '32, 64, 128, 256, 512', 'Tilings to try for K')
_LHS_LAYOUTS = flags.DEFINE_string('lhs_layouts', 'row_major', 'Layouts to try for LHS')
_RHS_LAYOUTS = flags.DEFINE_string('rhs_layouts', 'row_major', 'Layouts to try for RHS')
_RESULT_LAYOUTS = flags.DEFINE_string('result_layouts', 'row_major', 'Layouts to try for the result')
_NUM_STAGES = flags.DEFINE_string('num_stages', '1,2,3', 'Number of stages to try')
_NUM_WARPS = flags.DEFINE_string('num_warps', '4,8', 'Number of warps to try')
_SPLIT_KS = flags.DEFINE_string('split_ks', '1,2,3,4,5', 'Number of split_k values to try')
_DEBUG = flags.DEFINE_bool('debug', False, 'Print debug information')
_APPEND_TO_CSV = flags.DEFINE_string('append_to_csv', None, 'If set, appends the best tiling to the CSV file passed')
