...
ignore_settings: Set[str] = {'id', 'tabledir', 'reply_to', 'broker_consumer', 'broker_producer'}
builtin_locals: Dict[str, Any] = {'aiohttp': aiohttp, 'app': app, 'datetime': datetime, 'datadir': app.conf.datadir, 'faust': faust, 'logging': logging, 'mode': mode, 'socket': socket, 'timedelta': timedelta, 'web_host': socket.gethostname(), 'web_port': 6066, 'VERSION': faust.__version__, 'uuid': uuid, 'URL': URL}
RE_REF = re.compile('^:(\\w+):`')
