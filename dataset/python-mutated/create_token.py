...
class CreateToken(BaseMutation):
    ...
    class Arguments:
        ...
        audience = graphene.String(required=False, description='The audience that will be included to JWT tokens with prefix `custom:`.' + ADDED_IN_38)
    ...
    refresh_token = graphene.String(description='JWT refresh token, required to re-generate access token.')
    csrf_token = graphene.String(description='CSRF token required to re-generate access token.')
