...
class DeploymentFunctionExecutorNode(DAGNode):
    ...
    def __init__(self, deployment_function_handle: Union[RayServeSyncHandle, RayServeHandle], func_args, func_kwargs, other_args_to_resolve=None):
        if False:
            return 10
        super().__init__(func_args, func_kwargs, {}, other_args_to_resolve=other_args_to_resolve)
        self._deployment_function_handle = deployment_function_handle

    def _copy_impl(self, new_args: List[Any], new_kwargs: Dict[str, Any], new_options: Dict[str, Any], new_other_args_to_resolve: Dict[str, Any]) -> 'DeploymentFunctionExecutorNode':
        if False:
            return 10
        return DeploymentFunctionExecutorNode(self._deployment_function_handle, new_args, new_kwargs, new_other_args_to_resolve)
    ...
    def _execute_impl(self, *args, **kwargs) -> ObjectRef:
        ...
        return self._deployment_function_handle.options(use_new_handle_api=True).remote(*self._bound_args, **self._bound_kwargs)._to_object_ref_sync(_allow_running_in_asyncio_loop=True)
    ...
    def to_json(self) -> Dict[str, Any]:
        if False:
            for i in range(10):
                print('nop')
        return {DAGNODE_TYPE_KEY: DeploymentFunctionExecutorNode.__name__, 'deployment_function_handle': self._deployment_function_handle, 'args': self.get_args(), 'kwargs': self.get_kwargs(), 'other_args_to_resolve': self.get_other_args_to_resolve(), 'uuid': self.get_stable_uuid()}
    ...
    def from_json(cls, input_json):
        ...
        node = cls(input_json['deployment_function_handle'], input_json['args'], input_json['kwargs'], other_args_to_resolve=input_json['other_args_to_resolve'])
