...
class SimpleNet(Layer):
    ...
    def __init__(self, hidden_size, vocab_size, num_steps=20, init_scale=0.1, is_sparse=False, dtype='float64'):
        if False:
            i = 10
            return i + 15
        super().__init__()
        ...
        self.embedding = Embedding(self.vocab_size, self.hidden_size, sparse=True, weight_attr=paddle.ParamAttr(initializer=paddle.nn.initializer.Uniform(low=-init_scale, high=init_scale)))
        ...
        self.tmp = self.create_parameter(attr=paddle.ParamAttr(), shape=[self.hidden_size, self.vocab_size], dtype=dtype, default_initializer=paddle.nn.initializer.Uniform(low=-self.init_scale, high=self.init_scale))
