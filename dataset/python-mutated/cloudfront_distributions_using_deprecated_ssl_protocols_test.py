...
class Test_cloudfront_distributions_using_deprecated_ssl_protocols:
    ...
    def test_no_distributions(self):
        ...
        with mock.patch('prowler.providers.aws.services.cloudfront.cloudfront_service.CloudFront', new=cloudfront_client):
            from prowler.providers.aws.services.cloudfront.cloudfront_distributions_using_deprecated_ssl_protocols.cloudfront_distributions_using_deprecated_ssl_protocols import cloudfront_distributions_using_deprecated_ssl_protocols
    ...
    def test_one_distribution_using_deprecated_ssl_protocols(self):
        if False:
            ...
            return i + 15
        cloudfront_client = mock.MagicMock
        cloudfront_client.distributions = {'DISTRIBUTION_ID': Distribution(arn=DISTRIBUTION_ARN, id=DISTRIBUTION_ID, region=REGION, origins=[{'Id': 'string', 'DomainName': 'string', 'OriginPath': 'string', 'CustomHeaders': {'Quantity': 123, 'Items': [{'HeaderName': 'string', 'HeaderValue': 'string'}]}, 'S3OriginConfig': {'OriginAccessIdentity': 'string'}, 'CustomOriginConfig': {'HTTPPort': 123, 'HTTPSPort': 123, 'OriginProtocolPolicy': 'https-only', 'OriginSslProtocols': {'Quantity': 123, 'Items': ['SSLv3']}, 'OriginReadTimeout': 123, 'OriginKeepaliveTimeout': 123}, 'ConnectionAttempts': 123, 'ConnectionTimeout': 123, 'OriginShield': {'Enabled': False, 'OriginShieldRegion': 'string'}, 'OriginAccessControlId': 'string'}])}
        with mock.patch('prowler.providers.aws.services.cloudfront.cloudfront_service.CloudFront', new=cloudfront_client):
            from prowler.providers.aws.services.cloudfront.cloudfront_distributions_using_deprecated_ssl_protocols.cloudfront_distributions_using_deprecated_ssl_protocols import cloudfront_distributions_using_deprecated_ssl_protocols
            check = cloudfront_distributions_using_deprecated_ssl_protocols()
            result = check.execute()
            ...
            assert result[0].status_extended == f'CloudFront Distribution {DISTRIBUTION_ID} is using a deprecated SSL protocol.'
    ...
    def test_one_distribution_not_using_deprecated_ssl_protocols(self):
        ...
        with mock.patch('prowler.providers.aws.services.cloudfront.cloudfront_service.CloudFront', new=cloudfront_client):
            ...
            assert result[0].status_extended == f'CloudFront Distribution {DISTRIBUTION_ID} is not using a deprecated SSL protocol.'
            assert result[0].resource_tags == []