...
class Mutes(VoiceMutes, commands.Cog, metaclass=CompositeMetaClass):
    ...
    async def red_delete_data_for_user(self, *, requester: Literal['discord_deleted_user', 'owner', 'user', 'user_strict'], user_id: int):
        ...
        if self._ready_raised:
            raise RuntimeError("Mutes cog is in a bad state, can't proceed with data deletion request.")
    ...
    async def _schema_0_to_1(self):
        ...
        if not all_channels:
            return
        start = datetime.now()
        log.info('Config conversion to schema_version 1 started. This may take a while to proceed...')
        ...
        log.info('Config conversion to schema_version 1 done. It took %s to proceed.', datetime.now() - start)
    ...
    async def cog_before_invoke(self, ctx: commands.Context):
        if not self._ready.is_set():
            async with ctx.typing():
                await self._ready.wait()
        if self._ready_raised:
            await ctx.send("There was an error during Mutes's initialization. Check logs for more information.")
        ...
        for g_id in self._server_mutes:
            ...
            for u_id in self._server_mutes[guild.id]:
                if self._server_mutes[guild.id][u_id]['until'] is None:
                    continue
                time_to_unmute = self._server_mutes[guild.id][u_id]['until'] - datetime.now(timezone.utc).timestamp()
                if time_to_unmute < 60.0:
                    ...
                    self._unmute_tasks[task_name] = asyncio.create_task(self._auto_unmute_user(guild, self._server_mutes[guild.id][u_id]))
    ...
    async def _auto_unmute_user(self, guild: discord.Guild, data: dict):
        ...
        if success['success']:
            await modlog.create_case(self.bot, guild, datetime.now(timezone.utc), 'sunmute', member, author, _('Automatic unmute'), until=None)
            await self._send_dm_notification(member, author, guild, _('Server unmute'), _('Automatic unmute'))
        else:
            ...
            if not notification_channel.permissions_for(guild.me).send_messages:
                return
            error_msg = _('I am unable to unmute {user} for the following reason:\n{reason}').format(user=member, reason=success['reason'])
            try:
    ...
    async def _handle_channel_unmutes(self):
        ...
        for (c_id, c_data) in self._channel_mutes.items():
            for u_id in self._channel_mutes[c_id]:
                if not self._channel_mutes[c_id][u_id] or not self._channel_mutes[c_id][u_id]['until']:
                    continue
                guild = self.bot.get_guild(self._channel_mutes[c_id][u_id]['guild'])
                if guild is None or await self.bot.cog_disabled_in_guild(self, guild):
                    continue
                time_to_unmute = self._channel_mutes[c_id][u_id]['until'] - datetime.now(timezone.utc).timestamp()
                if time_to_unmute < 60.0:
        ...
        for (guild, users) in multiple_mutes.items():
            ...
            for (user, channels) in users.items():
                if len(channels) > 1:
                    ...
                    self._unmute_tasks[task_name] = asyncio.create_task(self._auto_channel_unmute_user_multi(member, guild, channels))
                else:
                    for (channel, mute_data) in channels.items():
                        ...
                        if (guild_channel := guild.get_channel(channel)):
                            self._unmute_tasks[task_name] = asyncio.create_task(self._auto_channel_unmute_user(guild_channel, mute_data))
        del multiple_mutes

    async def _auto_channel_unmute_user_multi(self, member: discord.Member, guild: discord.Guild, channels: Dict[int, dict]):
        ...
        for (channel, mute_data) in channels.items():
            ...
            tasks.append(self._auto_channel_unmute_user(guild.get_channel(channel), mute_data, False))
        ...
        await modlog.create_case(self.bot, guild, datetime.now(timezone.utc), 'sunmute', member, author, modlog_reason, until=None)
        await self._send_dm_notification(member, author, guild, _('Server unmute'), _('Automatic unmute'))
    ...
    async def _auto_channel_unmute_user(self, channel: discord.abc.GuildChannel, data: dict, create_case: bool=True) -> Optional[Tuple[discord.Member, discord.abc.GuildChannel, str]]:
        ...
        if not member:
            ...
            return None
        success = await self.channel_unmute_user(channel.guild, channel, author, member, _('Automatic unmute'))
        async with self.config.channel(channel).muted_users() as muted_users:
            if str(member.id) in muted_users:
                del muted_users[str(member.id)]
        if success['success']:
            if create_case:
                ...
                await modlog.create_case(self.bot, channel.guild, datetime.now(timezone.utc), unmute_type, member, channel.guild.me, _('Automatic unmute'), until=None, channel=channel)
                await self._send_dm_notification(member, author, channel.guild, notification_title, _('Automatic unmute'))
            return None
        else:
            error_msg = _('I am unable to unmute {user} in {channel} for the following reason:\n{reason}').format(user=member, channel=channel.mention, reason=success['reason'])
            if create_case:
    ...
    async def _send_dm_notification(self, user: Union[discord.User, discord.Member], moderator: Optional[Union[discord.User, discord.Member]], guild: discord.Guild, mute_type: str, reason: Optional[str], duration=None):
        if user.bot:
            return
        ...
        if await self.bot.embed_requested(user):
            em = discord.Embed(title=title, description=reason, color=await self.bot.get_embed_color(user))
            ...
            message += f"\n{bold(_('Until:'))} {until_str}\n{bold(_('Duration:'))} {duration_str}" if duration else ''
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        if mute_role in roles_removed:
            ...
            if after.id in self._server_mutes[guild.id]:
                await modlog.create_case(self.bot, guild, datetime.now(timezone.utc), 'sunmute', after, None, _('Manually removed mute role'))
                del self._server_mutes[guild.id][after.id]
                should_save = True
                await self._send_dm_notification(after, None, guild, _('Server unmute'), _('Manually removed mute role'))
        elif mute_role in roles_added:
            if guild.id not in self._server_mutes:
                self._server_mutes[guild.id] = {}
            if after.id not in self._server_mutes[guild.id]:
                await modlog.create_case(self.bot, guild, datetime.now(timezone.utc), 'smute', after, None, _('Manually applied mute role'))
                self._server_mutes[guild.id][after.id] = {'author': None, 'member': after.id, 'until': None}
                should_save = True
                await self._send_dm_notification(after, None, guild, _('Server mute'), _('Manually applied mute role'))
        if should_save:
    ...
    async def on_guild_channel_update(self, before: discord.abc.GuildChannel, after: discord.abc.GuildChannel):
        ...
        if after.id in self._channel_mutes:
            before_perms: Dict[int, Dict[str, Optional[bool]]] = {o.id: {name: attr for (name, attr) in p} for (o, p) in before.overwrites.items()}
            after_perms: Dict[int, Dict[str, Optional[bool]]] = {o.id: {name: attr for (name, attr) in p} for (o, p) in after.overwrites.items()}
            to_del: List[int] = []
            for (user_id, mute_data) in self._channel_mutes[after.id].items():
                ...
                if user_id in after_perms:
                    ...
                    if not voice_mute:
                        perms_to_check.extend(('send_messages', 'send_messages_in_threads', 'create_public_threads', 'create_private_threads'))
                ...
                if user_id in before_perms and (user_id not in after_perms or unmuted):
                    ...
                    if send_dm_notification:
                        await self._send_dm_notification(user, None, after.guild, notification_title, _('Manually removed channel overwrites'))
                    await modlog.create_case(self.bot, after.guild, datetime.now(timezone.utc), unmute_type, user, None, _('Manually removed channel overwrites'), until=None, channel=after)
        ...
        if guild.id in self._server_mutes:
            if member.id in self._server_mutes[guild.id]:
                ...
                if self._server_mutes[guild.id][member.id]['until']:
                    ...
                    until = None
                await self.mute_user(guild, guild.me, member, until, _('Previously muted in this server.'))
    ...
    async def showmoderator(self, ctx, true_or_false: bool):
        ...
        if true_or_false:
            await ctx.send(_('I will include the name of the moderator who issued the mute when sending a DM to a user.'))
        else:
            await ctx.send(_('I will not include the name of the moderator who issued the mute when sending a DM to a user.'))
    ...
    async def show_mutes_settings(self, ctx: commands.Context):
        ...
        msg = _('Mute Role: {role}\nNotification Channel: {channel}\nDefault Time: {time}\nSend DM: {dm}\nShow moderator: {show_mod}').format(role=mute_role.mention if mute_role else _('None'), channel=notification_channel.mention if notification_channel else _('None'), time=humanize_timedelta(timedelta=default_time) if default_time else _('None'), dm=data['dm'], show_mod=data['show_mod'])
        await ctx.maybe_send_embed(msg)

    @muteset.command(name='notification')
    @commands.admin_or_permissions(manage_channels=True)
    async def notification_channel_set(self, ctx: commands.Context, channel: Optional[Union[discord.TextChannel, discord.VoiceChannel, discord.StageChannel]]=None):
        """
        ...
        if channel is None:
            ...
            await ctx.send(_('I will post unmute issues in {channel}.').format(channel=channel.mention))
    ...
    async def mute_role(self, ctx: commands.Context, *, role: discord.Role=None):
        ...
        if not role:
            ...
            if role >= ctx.author.top_role:
                await ctx.send(_("You can't set this role as it is not lower than you in the role hierarchy."))
                return
        ...
        if not await self.config.guild(ctx.guild).notification_channel():
            ...
            await ctx.send(_("No notification channel has been setup, use {command_1} to be updated when there's an issue in automatic unmutes.").format(command_1=command_1))
    ...
    async def make_mute_role(self, ctx: commands.Context, *, name: str):
        ...
        if await self.config.guild(ctx.guild).mute_role():
            ...
            return await ctx.send(_('There is already a mute role setup in this server. Please remove it with {command} before trying to create a new one.').format(command=command))
        async with ctx.typing():
            perms = discord.Permissions()
            perms.update(send_messages=False, send_messages_in_threads=False, create_public_threads=False, create_private_threads=False, use_application_commands=False, speak=False, add_reactions=False)
            try:
                role = await ctx.guild.create_role(name=name, permissions=perms, reason=_('Mute role setup'))
                await self.config.guild(ctx.guild).mute_role.set(role.id)
            except discord.errors.Forbidden:
            ...
            if any(errors):
                msg = _('I could not set overwrites for the following channels: {channels}').format(channels=humanize_list([i for i in errors if i]))
    ...
    async def _set_mute_role_overwrites(self, role: discord.Role, channel: discord.abc.GuildChannel) -> Optional[str]:
        """
    ...
    async def default_mute_time(self, ctx: commands.Context, *, time: Optional[MuteTime]=None):
        ...
        if not time:
            ...
            await ctx.send(_('Default mute time set to {time}.').format(time=humanize_timedelta(timedelta=data)))
    ...
    async def _check_for_mute_role(self, ctx: commands.Context) -> bool:
        ...
        msg = _('This server does not have a mute role setup.  You can setup a mute role with {command_1} or {command_2} if you just want a basic role created setup.\n\n').format(command_1=inline(command_1), command_2=inline(command_2))
        ...
        if mute_role or sent_instructions:
            ...
            msg += _("Channel overwrites for muting users can get expensive on Discord's API as such we recommend that you have an admin setup a mute role instead. Channel overwrites will also not re-apply on guild join, so a user who has been muted may leave and re-join and no longer be muted. Role mutes do not have this issue.\n\nAre you sure you want to continue with channel overwrites? ")
            can_react = can_user_react_in(ctx.me, ctx.channel)
            if can_react:
                msg += _('Reacting with ✅ will continue the mute with overwrites and stop this message from appearing again, Reacting with ❎ will end the mute attempt.')
            else:
                msg += _('Saying {response_1} will continue the mute with overwrites and stop this message from appearing again, saying {response_2} will end the mute attempt.').format(response_1=inline('yes'), response_2=inline('no'))
        ...
        if ctx.guild.id in self._server_mutes:
            ...
            if mutes_data:
                ...
                for (user_id, mutes) in mutes_data.items():
                    ...
                    if mutes['until']:
                        time_left = timedelta(seconds=mutes['until'] - datetime.now(timezone.utc).timestamp())
    ...
    @commands.guild_only()
    @commands.mod_or_permissions(manage_roles=True)
    async def mute(self, ctx: commands.Context, users: commands.Greedy[discord.Member], *, time_and_reason: MuteTime={}):
        """Mute users.
        ...
        async with ctx.typing():
            ...
            if duration:
                ...
                if default_duration:
                    ...
                    time = _(' for {duration}').format(duration=humanize_timedelta(timedelta=timedelta(seconds=default_duration)))
            ...
            for user in users:
                ...
                if success['success']:
                    ...
                    await modlog.create_case(self.bot, guild, ctx.message.created_at, 'smute', user, author, reason, until=until, channel=None)
                    await self._send_dm_notification(user, author, guild, _('Server mute'), reason, duration)
                else:
                    issue_list.append(success)
        if success_list:
            ...
            await ctx.send(msg.format(users=humanize_list([f'{u}' for u in success_list]), time=time))
        if issue_list:
    ...
    async def handle_issues(self, ctx: commands.Context, issue_list: List[dict]) -> None:
        ...
        """
        message = _('Some users could not be properly muted. Would you like to see who, where, and why?')
        can_react = can_user_react_in(ctx.me, ctx.channel)
        if not can_react:
            message += ' (y/n)'
    ...
    @commands.command(name='mutechannel', aliases=['channelmute'], usage='<users...> [time_and_reason]')
    @commands.mod_or_permissions(manage_roles=True)
    @commands.bot_has_guild_permissions(manage_permissions=True)
    async def channel_mute(self, ctx: commands.Context, users: commands.Greedy[discord.Member], *, time_and_reason: MuteTime={}):
        ...
        async with ctx.typing():
            ...
            success_list = []
            for user in users:
                success = await self.channel_mute_user(guild, channel, author, user, until, audit_reason)
                if success['success']:
                    success_list.append(user)
                    await modlog.create_case(self.bot, guild, ctx.message.created_at, 'cmute', user, author, reason, until=until, channel=channel)
                    await self._send_dm_notification(user, author, guild, _('Channel mute'), reason, duration)
    ...
    @commands.guild_only()
    @commands.mod_or_permissions(manage_roles=True)
    async def unmute(self, ctx: commands.Context, users: commands.Greedy[discord.Member], *, reason: Optional[str]=None):
        """Unmute users.
        ...
        async with ctx.typing():
            ...
            for user in users:
                ...
                if success['success']:
                    ...
                    await self._send_dm_notification(user, author, guild, _('Server unmute'), reason)
                else:
                    issue_list.append(success)
        self._channel_mute_events[guild.id].set()
        if success_list:
            ...
            await ctx.send(_('{users} unmuted in this server.').format(users=humanize_list([f'{u}' for u in success_list])))
        if issue_list:
            await self.handle_issues(ctx, issue_list)
    ...
    @commands.bot_has_guild_permissions(manage_permissions=True)
    async def unmute_channel(self, ctx: commands.Context, users: commands.Greedy[discord.Member], *, reason: Optional[str]=None):
        ...
        async with ctx.typing():
            ...
            for user in users:
                ...
                if success['success']:
                    success_list.append(user)
                    await modlog.create_case(self.bot, guild, ctx.message.created_at, 'cunmute', user, author, reason, until=None, channel=channel)
                    await self._send_dm_notification(user, author, guild, _('Channel unmute'), reason)
                else:
                    issue_list.append((user, success['reason']))
        if success_list:
            ...
            await ctx.send(_('{users} unmuted in this channel.').format(users=humanize_list([f'{u}' for u in success_list])))
        if issue_list:
    ...
    async def mute_user(self, guild: discord.Guild, author: discord.Member, user: discord.Member, until: Optional[datetime]=None, reason: Optional[str]=None) -> Dict[str, Optional[Union[List[Tuple[discord.abc.GuildChannel, str]], discord.Member, bool, str]]]:
        """
        Handles muting users
        """
        permissions = user.guild_permissions
        ret: Dict[str, Union[bool, Optional[str], List[Tuple[discord.abc.GuildChannel, str]], discord.Member]] = {'success': False, 'reason': None, 'channels': [], 'user': user}
        ...
        if mute_role:
            ...
            if guild.id not in self._server_mutes:
                self._server_mutes[guild.id] = {}
            self._server_mutes[guild.id][user.id] = {'author': author.id, 'member': user.id, 'until': until.timestamp() if until else None}
            try:
    ...
    async def unmute_user(self, guild: discord.Guild, author: discord.Member, user: discord.Member, reason: Optional[str]=None) -> Dict[str, Union[bool, Optional[str], List[Tuple[discord.abc.GuildChannel, str]], discord.Member]]:
        """
    ...
    async def channel_mute_user(self, guild: discord.Guild, channel: discord.abc.GuildChannel, author: discord.Member, user: discord.Member, until: Optional[datetime]=None, reason: Optional[str]=None, *, voice_mute: bool=False) -> Dict[str, Optional[Union[discord.abc.GuildChannel, str, bool]]]:
        """Mutes the specified user in the specified channel"""
        overwrites = channel.overwrites_for(user)
        permissions = channel.permissions_for(user)
        if permissions.administrator:
            return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['is_admin'])}
        move_channel = False
        send_reason = None
        ...
        if not await self.is_allowed_by_hierarchy(guild, author, user):
            return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['hierarchy_problem'])}
        ...
        is_mute_upgrade = current_mute is not None and (not voice_mute) and current_mute.get('voice_mute', False)
        if current_mute is not None and (not is_mute_upgrade):
            return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['already_muted'])}
        new_overs: Dict[str, Optional[bool]] = {'speak': False}
        if not voice_mute:
            new_overs.update(send_messages=False, send_messages_in_threads=False, create_public_threads=False, create_private_threads=False, use_application_commands=False, add_reactions=False)
        ...
        if not channel.permissions_for(guild.me).manage_permissions:
            return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['permissions_issue_channel'])}
        self._channel_mutes[channel.id][user.id] = {'author': author.id, 'guild': guild.id, 'member': user.id, 'until': until.timestamp() if until else None, 'voice_mute': voice_mute}
        try:
            ...
            if e.code == 10003:
                if channel.id in self._channel_mutes and user.id in self._channel_mutes[channel.id]:
                    del self._channel_mutes[channel.id][user.id]
                return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['unknown_channel'])}
            elif e.code == 10009:
                if channel.id in self._channel_mutes and user.id in self._channel_mutes[channel.id]:
                    del self._channel_mutes[channel.id][user.id]
                return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['left_guild'])}
        except discord.Forbidden:
            return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['permissions_issue_channel'])}
        if move_channel:
            try:
                ...
                return {'success': True, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['voice_mute_permission']), 'old_overs': old_overs}
        return {'success': True, 'channel': channel, 'old_overs': old_overs, 'reason': send_reason}

    async def channel_unmute_user(self, guild: discord.Guild, channel: discord.abc.GuildChannel, author: discord.Member, user: discord.Member, reason: Optional[str]=None, *, voice_mute: bool=False) -> Dict[str, Optional[Union[discord.abc.GuildChannel, str, bool]]]:
        ...
        if channel.id in perms_cache:
            ...
            old_values = {'send_messages': None, 'send_messages_in_threads': None, 'create_public_threads': None, 'create_private_threads': None, 'use_application_commands': None, 'add_reactions': None, 'speak': None}
        if user.voice and user.voice.channel == channel:
            if channel.permissions_for(guild.me).move_members:
                move_channel = True
        if not await self.is_allowed_by_hierarchy(guild, author, user):
        ...
        if channel.id in self._channel_mutes and user.id in self._channel_mutes[channel.id]:
            ...
            return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['already_unmuted'])}
        if not current_mute.get('voice_mute', False) and voice_mute:
            return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['is_not_voice_mute']).format(command=inline('unmutechannel'))}
        if not channel.permissions_for(guild.me).manage_permissions:
            return {'success': False, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['permissions_issue_channel'])}
        try:
            if overwrites.is_empty():
                await channel.set_permissions(user, overwrite=cast(discord.PermissionOverwrite, None), reason=reason)
            else:
                await channel.set_permissions(user, overwrite=overwrites, reason=reason)
            async with self.config.channel(channel).muted_users() as muted_users:
        ...
        if move_channel:
            try:
                ...
                return {'success': True, 'channel': channel, 'reason': _(MUTE_UNMUTE_ISSUES['voice_mute_permission'])}
