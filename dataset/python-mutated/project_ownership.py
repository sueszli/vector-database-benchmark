...
HIGHER_MAX_RAW_LENGTH = 200000

class ProjectOwnershipRequestSerializer(serializers.Serializer):
    raw = serializers.CharField(required=False, allow_blank=True, help_text='Raw input for ownership configuration. See the [Ownership Rules Documentation](/product/issues/ownership-rules/) to learn more.')
    fallthrough = serializers.BooleanField(required=False, help_text='A boolean determining who to assign ownership to when an ownership rule has no match. If set to `True`, all project members are made owners. Otherwise, no owners are set.')
    autoAssignment = serializers.CharField(required=False, allow_blank=False, help_text='Auto-assignment settings. The available options are:\n- Auto Assign to Issue Owner\n- Auto Assign to Suspect Commits\n- Turn off Auto-Assignment')
    codeownersAutoSync = serializers.BooleanField(required=False, default=True, help_text='Set to `True` to sync issue owners with CODEOWNERS updates in a release.')
...
class ProjectOwnershipEndpoint(ProjectEndpoint):
    ...
    publish_status = {'GET': ApiPublishStatus.PUBLIC, 'PUT': ApiPublishStatus.PUBLIC}
