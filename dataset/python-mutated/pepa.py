...
if __name__ == '__main__':
    ...
    if args.query_api:
        ...
        if not request.ok:
            raise RuntimeError('Failed to authenticate to SaltStack REST API: {}'.format(request.text))
    ...
    result = ext_pillar(args.hostname, __pillar__, __opts__['ext_pillar'][loc]['pepa']['resource'], __opts__['ext_pillar'][loc]['pepa']['sequence'], ex_subkey, ex_subkey_only)
    if __opts__['pepa_validate']:
    ...
    try:
        ...
        if not args.no_color:
            try:
                ...
                print(pygments.highlight(salt.utils.yaml.safe_dump(result), pygments.lexers.YamlLexer(), pygments.formatters.TerminalFormatter()))
            except ImportError:
