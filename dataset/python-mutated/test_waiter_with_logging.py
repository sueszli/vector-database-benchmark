...
class TestWaiter:
    ...
    async def test_async_wait(self, caplog):
        ...
        mock_waiter.wait.side_effect = [error, error, True]
        await async_wait(waiter=mock_waiter, waiter_delay=0, waiter_max_attempts=456, args={'test_arg': 'test_value'}, failure_message='test failure message', status_message='test status message', status_args=['Status.State'])
        mock_waiter.wait.assert_called_with(**{'test_arg': 'test_value'}, WaiterConfig={'MaxAttempts': 1})
        assert mock_waiter.wait.call_count == 3
