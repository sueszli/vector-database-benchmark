...
from tests.components.recorder.common import assert_dict_of_states_equal_without_context_and_last_changed, assert_multiple_states_equal_without_context, assert_multiple_states_equal_without_context_and_last_changed, assert_states_equal_without_context, async_recorder_block_till_done, async_wait_recording_done, old_db_schema, wait_recording_done
...
async def test_fetch_period_api(recorder_mock: Recorder, hass: HomeAssistant, hass_client: ClientSessionGenerator) -> None:
    """Test the fetch period view for history."""
    await async_setup_component(hass, 'history', {})
    instance = recorder.get_instance(hass)
    with patch.object(instance.states_meta_manager, 'active', False):
        client = await hass_client()
        response = await client.get(f'/api/history/period/{dt_util.utcnow().isoformat()}?filter_entity_id=sensor.power')
        assert response.status == HTTPStatus.OK

async def test_fetch_period_api_with_minimal_response(recorder_mock: Recorder, hass: HomeAssistant, hass_client: ClientSessionGenerator) -> None:
    ...
    with patch.object(instance.states_meta_manager, 'active', False):
        ...
        await async_wait_recording_done(hass)
        client = await hass_client()
        response = await client.get(f'/api/history/period/{now.isoformat()}?filter_entity_id=sensor.power&minimal_response&no_attributes')
        assert response.status == HTTPStatus.OK
        ...
        assert state_list[2]['last_changed'] == json.dumps(process_timestamp(last_changed), cls=JSONEncoder).replace('"', '')

async def test_fetch_period_api_with_no_timestamp(recorder_mock: Recorder, hass: HomeAssistant, hass_client: ClientSessionGenerator) -> None:
...
async def test_fetch_period_api_with_include_order(recorder_mock: Recorder, hass: HomeAssistant, hass_client: ClientSessionGenerator) -> None:
    """Test the fetch period view for history."""
    await async_setup_component(hass, 'history', {'history': {'use_include_order': True, 'include': {'entities': ['light.kitchen']}}})
    instance = recorder.get_instance(hass)
    with patch.object(instance.states_meta_manager, 'active', False):
        client = await hass_client()
        response = await client.get(f'/api/history/period/{dt_util.utcnow().isoformat()}', params={'filter_entity_id': 'non.existing,something.else'})
        assert response.status == HTTPStatus.OK

async def test_entity_ids_limit_via_api(recorder_mock: Recorder, hass: HomeAssistant, hass_client: ClientSessionGenerator) -> None:
    ...
    with patch.object(instance.states_meta_manager, 'active', False):
        ...
        await async_wait_recording_done(hass)
        client = await hass_client()
        response = await client.get(f'/api/history/period/{dt_util.utcnow().isoformat()}?filter_entity_id=light.kitchen,light.cow')
        assert response.status == HTTPStatus.OK
        response_json = await response.json()
...
async def test_entity_ids_limit_via_api_with_skip_initial_state(recorder_mock: Recorder, hass: HomeAssistant, hass_client: ClientSessionGenerator) -> None:
    ...
    with patch.object(instance.states_meta_manager, 'active', False):
        ...
        await async_wait_recording_done(hass)
        client = await hass_client()
        response = await client.get(f'/api/history/period/{dt_util.utcnow().isoformat()}?filter_entity_id=light.kitchen,light.cow&skip_initial_state')
        assert response.status == HTTPStatus.OK
        response_json = await response.json()
        assert len(response_json) == 0
        when = dt_util.utcnow() - timedelta(minutes=1)
        response = await client.get(f'/api/history/period/{when.isoformat()}?filter_entity_id=light.kitchen,light.cow&skip_initial_state')
        assert response.status == HTTPStatus.OK
        response_json = await response.json()
        assert len(response_json) == 2
        assert response_json[0][0]['entity_id'] == 'light.kitchen'
...
async def test_history_during_period(recorder_mock: Recorder, hass: HomeAssistant, hass_ws_client: WebSocketGenerator) -> None:
    ...
    with patch.object(instance.states_meta_manager, 'active', False):
        ...
        await client.send_json({'id': 1, 'type': 'history/history_during_period', 'start_time': now.isoformat(), 'end_time': now.isoformat(), 'entity_ids': ['sensor.test'], 'include_start_time_state': True, 'significant_changes_only': False, 'no_attributes': True})
        response = await client.receive_json()
        assert response['success']
        assert response['result'] == {}
        await client.send_json({'id': 2, 'type': 'history/history_during_period', 'start_time': now.isoformat(), 'entity_ids': ['sensor.test'], 'include_start_time_state': True, 'significant_changes_only': False, 'no_attributes': True, 'minimal_response': True})
        response = await client.receive_json()
        assert response['success']
        ...
        assert 'lc' not in sensor_test_history[1]
        assert sensor_test_history[2]['s'] == 'on'
        assert 'a' not in sensor_test_history[2]
        await client.send_json({'id': 3, 'type': 'history/history_during_period', 'start_time': now.isoformat(), 'entity_ids': ['sensor.test'], 'include_start_time_state': True, 'significant_changes_only': False, 'no_attributes': False})
        response = await client.receive_json()
        assert response['success']
        ...
        await client.send_json({'id': 4, 'type': 'history/history_during_period', 'start_time': now.isoformat(), 'entity_ids': ['sensor.test'], 'include_start_time_state': True, 'significant_changes_only': True, 'no_attributes': False})
        response = await client.receive_json()
        assert response['success']
...
async def test_history_during_period_impossible_conditions(recorder_mock: Recorder, hass: HomeAssistant, hass_ws_client: WebSocketGenerator) -> None:
    ...
    with patch.object(instance.states_meta_manager, 'active', False):
        ...
        client = await hass_ws_client()
        await client.send_json({'id': 1, 'type': 'history/history_during_period', 'start_time': after.isoformat(), 'end_time': after.isoformat(), 'entity_ids': ['sensor.test'], 'include_start_time_state': False, 'significant_changes_only': False, 'no_attributes': True})
        response = await client.receive_json()
        assert response['success']
        assert response['id'] == 1
        assert response['result'] == {}
        future = dt_util.utcnow() + timedelta(hours=10)
        await client.send_json({'id': 2, 'type': 'history/history_during_period', 'start_time': future.isoformat(), 'entity_ids': ['sensor.test'], 'include_start_time_state': True, 'significant_changes_only': True, 'no_attributes': True})
        response = await client.receive_json()
        assert response['success']
        assert response['id'] == 2
        assert response['result'] == {}

@pytest.mark.parametrize('time_zone', ['UTC', 'Europe/Berlin', 'America/Chicago', 'US/Hawaii'])
async def test_history_during_period_significant_domain(time_zone, recorder_mock: Recorder, hass: HomeAssistant, hass_ws_client: WebSocketGenerator) -> None:
    ...
    with patch.object(instance.states_meta_manager, 'active', False):
        ...
        await async_wait_recording_done(hass)
        client = await hass_ws_client()
        await client.send_json({'id': 1, 'type': 'history/history_during_period', 'start_time': now.isoformat(), 'end_time': now.isoformat(), 'entity_ids': ['climate.test'], 'include_start_time_state': True, 'significant_changes_only': False, 'no_attributes': True})
        response = await client.receive_json()
        assert response['success']
        assert response['result'] == {}
        await client.send_json({'id': 2, 'type': 'history/history_during_period', 'start_time': now.isoformat(), 'entity_ids': ['climate.test'], 'include_start_time_state': True, 'significant_changes_only': False, 'no_attributes': True, 'minimal_response': True})
        response = await client.receive_json()
        assert response['success']
        assert response['id'] == 2
        ...
        await client.send_json({'id': 3, 'type': 'history/history_during_period', 'start_time': now.isoformat(), 'entity_ids': ['climate.test'], 'include_start_time_state': True, 'significant_changes_only': False, 'no_attributes': False})
        response = await client.receive_json()
        assert response['success']
        assert response['id'] == 3
        sensor_test_history = response['result']['climate.test']
        ...
        await client.send_json({'id': 4, 'type': 'history/history_during_period', 'start_time': now.isoformat(), 'entity_ids': ['climate.test'], 'include_start_time_state': True, 'significant_changes_only': True, 'no_attributes': False})
        response = await client.receive_json()
        assert response['success']
        assert response['id'] == 4
        sensor_test_history = response['result']['climate.test']
        ...
        await client.send_json({'id': 5, 'type': 'history/history_during_period', 'start_time': later.isoformat(), 'entity_ids': ['climate.test'], 'include_start_time_state': True, 'significant_changes_only': True, 'no_attributes': False})
        response = await client.receive_json()
        assert response['success']
...
async def test_history_during_period_bad_start_time(recorder_mock: Recorder, hass: HomeAssistant, hass_ws_client: WebSocketGenerator) -> None:
    """Test history_during_period bad state time."""
    await async_setup_component(hass, 'history', {'history': {}})
    instance = recorder.get_instance(hass)
    with patch.object(instance.states_meta_manager, 'active', False):
        client = await hass_ws_client()
        await client.send_json({'id': 1, 'type': 'history/history_during_period', 'entity_ids': ['sensor.pet'], 'start_time': 'cats'})
        response = await client.receive_json()
        assert not response['success']
        assert response['error']['code'] == 'invalid_start_time'

async def test_history_during_period_bad_end_time(recorder_mock: Recorder, hass: HomeAssistant, hass_ws_client: WebSocketGenerator) -> None:
    ...
    with patch.object(instance.states_meta_manager, 'active', False):
        client = await hass_ws_client()
        await client.send_json({'id': 1, 'type': 'history/history_during_period', 'entity_ids': ['sensor.pet'], 'start_time': now.isoformat(), 'end_time': 'dogs'})
        response = await client.receive_json()
        assert not response['success']
