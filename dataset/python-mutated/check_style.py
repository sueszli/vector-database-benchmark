...
import time
import sys
PYTHON_FORMAT_DIRS = ['examples', 'docs', 'python', 'util']
JUPYTER_FORMAT_DIRS = ['examples']
CPP_FORMAT_DIRS = ['cpp', 'examples', 'docs/_static']
if sys.version_info < (3, 6):
    raise RuntimeError(f'Requires Python 3.6+, currently using Python {sys.version_info.major}.{sys.version_info.minor}.')
try:
    import yapf
except ImportError:
    raise ImportError('yapf not found. Install with `pip install yapf==0.30.0`.')
if yapf.__version__ != '0.30.0':
    raise RuntimeError('yapf 0.30.0 required. Install with `pip install yapf==0.30.0`.')
try:
    import nbformat
except ImportError:
    raise ImportError('nbformat not found. Install with `pip install nbformat`.')
