...
async def test_constructor_with_expired(cookies_to_send_with_expired: Any, cookies_to_receive: Any) -> None:
    jar = CookieJar()
...
async def test_save_load(tmp_path: Any, loop: Any, cookies_to_send: Any, cookies_to_receive: Any) -> None:
...
async def test_update_cookie_with_unicode_domain(loop: Any) -> None:
    cookies = ('idna-domain-first=first; Domain=xn--9caa.com; Path=/;', 'idna-domain-second=second; Domain=xn--9caa.com; Path=/;')
    jar = CookieJar()
...
async def test_filter_cookie_with_unicode_domain(loop: Any) -> None:
    jar = CookieJar()
    jar.update_cookies(SimpleCookie('idna-domain-first=first; Domain=xn--9caa.com; Path=/; '))
...
async def test_filter_cookies_str_deprecated(loop: Any) -> None:
    jar = CookieJar()
    with pytest.deprecated_call(match="The method accepts yarl.URL instances only, got <class 'str'>"):
        jar.filter_cookies('http://éé.com')

async def test_domain_filter_ip_cookie_send(loop: Any) -> None:
    jar = CookieJar()
    cookies = SimpleCookie('shared-cookie=first; domain-cookie=second; Domain=example.com; subdomain1-cookie=third; Domain=test1.example.com; subdomain2-cookie=fourth; Domain=test2.example.com; dotted-domain-cookie=fifth; Domain=.example.com; different-domain-cookie=sixth; Domain=different.org; secure-cookie=seventh; Domain=secure.com; Secure; no-path-cookie=eighth; Domain=pathtest.com; path1-cookie=ninth; Domain=pathtest.com; Path=/; path2-cookie=tenth; Domain=pathtest.com; Path=/one; path3-cookie=eleventh; Domain=pathtest.com; Path=/one/two; path4-cookie=twelfth; Domain=pathtest.com; Path=/one/two/; expires-cookie=thirteenth; Domain=expirestest.com; Path=/; Expires=Tue, 1 Jan 1980 12:00:00 GMT; max-age-cookie=fourteenth; Domain=maxagetest.com; Path=/; Max-Age=60; invalid-max-age-cookie=fifteenth; Domain=invalid-values.com;  Max-Age=string; invalid-expires-cookie=sixteenth; Domain=invalid-values.com;  Expires=string;')
...
@pytest.mark.parametrize(('cookies', 'expected', 'quote_bool'), [('shared-cookie=first; ip-cookie=second; Domain=127.0.0.1;', 'Cookie: ip-cookie=second\r\nCookie: shared-cookie=first', True), ('ip-cookie="second"; Domain=127.0.0.1;', 'Cookie: ip-cookie="second"', True), ('custom-cookie=value/one;', 'Cookie: custom-cookie="value/one"', True), ('custom-cookie=value1;', 'Cookie: custom-cookie=value1', True), ('custom-cookie=value/one;', 'Cookie: custom-cookie=value/one', False)], ids=('IP domain preserved', 'no shared cookie', 'quoted cookie with special char', 'quoted cookie w/o special char', 'unquoted cookie with special char'))
async def test_quotes_correctly_based_on_input(loop: Any, cookies: Any, expected: Any, quote_bool: Any) -> None:
...
async def test_ignore_domain_ending_with_dot(loop: Any) -> None:
    ...
    jar.update_cookies(SimpleCookie('cookie=val; Domain=example.com.;'), URL('http://www.example.com'))
...
@pytest.mark.skipif(sys.implementation.name != 'cpython', reason='time_machine leverages CPython specific pointers https://github.com/adamchainz/time-machine/issues/305')
...
@pytest.mark.parametrize('url', ['http://127.0.0.1/index.html', URL('http://127.0.0.1/index.html'), ['http://127.0.0.1/index.html'], [URL('http://127.0.0.1/index.html')]])
...
async def test_treat_as_secure_origin() -> None:
    ...
    secure_cookie = SimpleCookie('cookie-key=cookie-value; HttpOnly; Path=/; Secure')
    jar.update_cookies(secure_cookie, endpoint)
    assert len(jar) == 1
