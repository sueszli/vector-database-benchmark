...
class TestCase(unittest.TestCase):
    ...
    def get_config(self, config_class: type[SomeConfig], cfg: dict[str, Any], warnings: dict[str, str]={}, config_file_path=None) -> SomeConfig:
        if False:
            while True:
                i = 10
...
class ListOfItemsTest(TestCase):
    ...
    def test_warning(self) -> None:
        ...
        self.get_config(Schema, {'option': ['a']}, warnings=dict(option="The configuration option 'option[0]' has been deprecated and will be removed in a future release."))
...
class ExtraScriptsTest(TestCase):
    ...
    def test_js_async(self) -> None:
        ...
        self.assertEqual(conf.option, ['foo.js', {'path': 'bar.js', 'type': '', 'defer': False, 'async': True}])
    ...
    def test_mjs(self) -> None:
        ...
        class Schema(Config):
            option = c.ListOfItems(c.ExtraScript(), default=[])
        conf = self.get_config(Schema, {'option': ['foo.mjs', {'path': 'bar.js', 'type': 'module'}]})
        assert_type(conf.option, List[Union[c.ExtraScriptValue, str]])
        self.assertEqual(len(conf.option), 2)
        self.assertIsInstance(conf.option[0], c.ExtraScriptValue)
        self.assertEqual(conf.option, [{'path': 'foo.mjs', 'type': 'module', 'defer': False, 'async': False}, {'path': 'bar.js', 'type': 'module', 'defer': False, 'async': False}])
    ...
    def test_wrong_type(self) -> None:
        ...
        class Schema(Config):
            option = c.ListOfItems(c.ExtraScript(), default=[])
        with self.expect_error(option="The configuration is invalid. Expected a key-value mapping (dict) but received: <class 'int'>"):
            self.get_config(Schema, {'option': [1]})
    ...
    def test_unknown_key(self) -> None:
        ...
        class Schema(Config):
            option = c.ListOfItems(c.ExtraScript(), default=[])
        conf = self.get_config(Schema, {'option': [{'path': 'foo.js', 'foo': 'bar'}]}, warnings=dict(option="Sub-option 'foo': Unrecognised configuration name: foo"))
        self.assertEqual(conf.option, [{'path': 'foo.js', 'type': '', 'defer': False, 'async': False, 'foo': 'bar'}])
...
class DictOfItemsTest(TestCase):
    ...
    def test_post_validation_error(self) -> None:
        ...
        with self.expect_error(option="'asdf' is not a valid port"):
            self.get_config(Schema, {'option': {'ip_foo': 'localhost:8000', 'ip_bar': '1.2.3.4:asdf'}})
...
class ThemeTest(TestCase):
    ...
    def test_uninstalled_theme_as_string(self) -> None:
        ...
        with self.expect_error(theme=re.compile("Unrecognised theme name: 'mkdocs2'. The available installed themes are: .+")):
...
class NestedSubConfigTest(TestCase):
    ...
    def defaults(self):
        if False:
            print('Hello World!')
        return {'nav': {'omitted_files': logging.INFO, 'not_found': logging.WARNING, 'absolute_links': logging.INFO}, 'links': {'not_found': logging.WARNING, 'absolute_links': logging.INFO, 'unrecognized_links': logging.INFO}}
    ...
    def test_unspecified(self) -> None:
        ...
        for cfg in ({}, {'validation': {}}):
            with self.subTest(cfg):
                conf = self.get_config(self.Schema, {})
                self.assertEqual(conf.validation, self.defaults())

    def test_sets_nested_and_not_nested(self) -> None:
        if False:
            return 10
        conf = self.get_config(self.Schema, {'validation': {'not_found': 'ignore', 'links': {'absolute_links': 'warn'}}})
    ...
    def test_sets_nested_different(self) -> None:
        if False:
            for i in range(10):
                print('nop')
        conf = self.get_config(self.Schema, {'validation': {'not_found': 'ignore', 'links': {'not_found': 'warn'}}})
        expected = self.defaults()
        expected['nav']['not_found'] = logging.DEBUG
    ...
    def test_sets_only_one_nested(self) -> None:
        if False:
            return 10
        conf = self.get_config(self.Schema, {'validation': {'omitted_files': 'ignore'}})
        expected = self.defaults()
    ...
    def test_sets_nested_not_dict(self) -> None:
        if False:
            return 10
        with self.expect_error(validation="Sub-option 'links': Sub-option 'unrecognized_links': Expected a string, but a <class 'list'> was given."):
            self.get_config(self.Schema, {'validation': {'unrecognized_links': [], 'links': {'absolute_links': 'warn'}}})

    def test_wrong_key_nested(self) -> None:
        if False:
            return 10
        conf = self.get_config(self.Schema, {'validation': {'foo': 'warn', 'not_found': 'warn'}}, warnings=dict(validation="Sub-option 'foo': Unrecognised configuration name: foo"))
        expected = self.defaults()
    ...
    def test_wrong_type_nested(self) -> None:
        if False:
            while True:
                i = 10
        with self.expect_error(validation="Sub-option 'nav': Sub-option 'omitted_files': Expected one of ['warn', 'info', 'ignore'], got 'hi'"):
            self.get_config(self.Schema, {'validation': {'omitted_files': 'hi'}})
...
@mock.patch('mkdocs.plugins.entry_points', mock.Mock(return_value=[FakeEntryPoint('sample', FakePlugin), FakeEntryPoint('sample2', FakePlugin2), FakeEntryPoint('sample-e', EnabledPlugin), FakeEntryPoint('readthedocs/sub_plugin', ThemePlugin), FakeEntryPoint('overridden', FakePlugin2), FakeEntryPoint('readthedocs/overridden', ThemePlugin2)]))
class PluginsTest(TestCase):
    ...
    def test_plugin_config_with_options(self) -> None:
        ...
        class Schema(Config):
            plugins = c.Plugins()
        cfg = {'plugins': [{'sample': {'foo': 'foo value', 'bar': 42}}]}
        conf = self.get_config(Schema, cfg)
        self.assertIsInstance(conf.plugins, PluginCollection)
        self.assertIn('sample', conf.plugins)
        self.assertIsInstance(conf.plugins['sample'], BasePlugin)
        expected = {'foo': 'foo value', 'bar': 42, 'dir': None}
    ...
    def test_plugin_config_enabled_for_any_plugin(self) -> None:
        ...
        with self.expect_error(plugins="Plugin 'sample' option 'enabled': Expected boolean but received: <class 'int'>"):
            self.get_config(Schema, cfg)
    ...
    def test_plugin_config_enabled_for_plugin_with_setting(self) -> None:
        ...
        with self.expect_error(plugins="Plugin 'sample-e' option 'enabled': Expected type: <class 'bool'> but received: <class 'int'>"):
            self.get_config(Schema, cfg)
    ...
    def test_plugin_config_with_multiple_instances(self) -> None:
        ...
        class Schema(Config):
            theme = c.Theme(default='mkdocs')
            plugins = c.Plugins(theme_key='theme')
        cfg = {'plugins': [{'sample2': {'foo': 'foo value', 'bar': 42}}, {'sample2': {'foo': 'foo2 value'}}]}
        conf = self.get_config(Schema, cfg)
        self.assertEqual(set(conf.plugins), {'sample2', 'sample2 #2'})
    ...
    def test_plugin_config_with_multiple_instances_and_warning(self) -> None:
        ...
        class Schema(Config):
            theme = c.Theme(default='mkdocs')
            plugins = c.Plugins(theme_key='theme')
        test_cfgs: list[dict[str, Any]] = [{'theme': 'readthedocs', 'plugins': [{'sub_plugin': {}}, {'sample2': {}}, {'sub_plugin': {}}, 'sample2']}, {'theme': 'readthedocs', 'plugins': ['sub_plugin', 'sample2', 'sample2', 'sub_plugin']}]
        for cfg in test_cfgs:
            conf = self.get_config(Schema, cfg, warnings=dict(plugins="Plugin 'readthedocs/sub_plugin' was specified multiple times - this is likely a mistake, because the plugin doesn't declare `supports_multiple_instances`."))
            self.assertEqual(set(conf.plugins), {'readthedocs/sub_plugin', 'readthedocs/sub_plugin #2', 'sample2', 'sample2 #2'})
    ...
    def test_plugin_config_multivalue_dict(self) -> None:
        ...
        class Schema(Config):
            plugins = c.Plugins()
        cfg = {'plugins': [{'sample': {'foo': 'foo value', 'bar': 42}, 'extra_key': 'baz'}]}
    ...
    def test_plugin_config_not_string_or_dict(self) -> None:
        ...
        class Schema(Config):
            plugins = c.Plugins()
        cfg = {'plugins': [('not a string or dict',)]}
    ...
    def test_plugin_config_sub_error(self) -> None:
        ...
        class Schema(Config):
            plugins = c.Plugins(default=['sample'])
        cfg = {'plugins': {'sample': {'bar': 'not an int'}}}
        with self.expect_error(plugins="Plugin 'sample' option 'bar': Expected type: <class 'int'> but received: <class 'str'>"):
            self.get_config(Schema, cfg)
    ...
    def test_plugin_config_sub_warning(self) -> None:
        ...
        class Schema(Config):
            plugins = c.Plugins()
        cfg = {'plugins': {'sample2': {'depr': 'deprecated value'}}}
        conf = self.get_config(Schema, cfg, warnings=dict(plugins="Plugin 'sample2' option 'depr': The configuration option 'depr' has been deprecated and will be removed in a future release."))
        self.assertIsInstance(conf.plugins, PluginCollection)
