...
async def run_and_stream_logs(metrics_actor_name, metrics_actor_namespace, test: TestConfiguration):
    """
    ...
    for _ in range(test.num_jobs):
        ...
        if not test.with_runtime_env:
            ...
            job_id = client.submit_job(entrypoint=entrypoint, runtime_env={'working_dir': './'})
            try:
...
if __name__ == '__main__':
    args = parse_args()
    sys.exit(main(args.num_cpus_in_cluster, args.num_gpus_in_cluster, args.num_tasks_or_actors_per_run, args.num_measurements_per_configuration))