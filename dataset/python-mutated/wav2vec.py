...
from fairseq.modules import Fp32GroupNorm, Fp32LayerNorm, GumbelVectorQuantizer, KmeansVectorQuantizer, TransposeLast
...
class Wav2VecConfig(FairseqDataclass):
    prediction_steps: int = field(default=12, metadata={'help': 'number of steps ahead to predict'})
    sample_distance: Optional[int] = field(default=None, metadata={'help': 'sample distance from target. does not work properly with cross-sampling'})
    cross_sample_negatives: int = field(default=0, metadata={'help': 'num of cross sampled negatives'})
    num_negatives: int = field(default=10, metadata={'help': 'num of sampled negatives'})
    conv_feature_layers: str = field(default='[(512, 10, 5), (512, 8, 4), (512, 4, 2), (512, 4, 2), (512, 4, 2), (512, 1, 1), (512, 1, 1), (512, 1, 1)]', metadata={'help': 'convolutional feature extraction layers [(dim, kernel_size, stride), ...]'})
    conv_aggregator_layers: str = field(default='[(512, 2, 1), (512, 3, 1), (512, 4, 1), (512, 5, 1), (512, 6, 1), (512, 7, 1), (512, 8, 1), (512, 9, 1), (512, 10, 1), (512, 11, 1), (512, 12, 1), (512, 13, 1)]', metadata={'help': 'convolutional aggregator layers [(dim, kernel_size, stride), ...]'})
    dropout: float = field(default=0.0, metadata={'help': 'dropout to apply within the model'})
    dropout_features: float = field(default=0.0, metadata={'help': 'dropout to apply to the features'})
    dropout_agg: float = field(default=0.0, metadata={'help': 'dropout to apply after aggregation step'})
    aggregator: AGGREGATOR_CHOICES = field(default='cnn', metadata={'help': 'type of aggregator to use'})
    gru_dim: int = field(default=512, metadata={'help': 'GRU dimensionality'})
    no_conv_bias: bool = field(default=False, metadata={'help': 'if set, does not learn bias for conv layers'})
    agg_zero_pad: bool = field(default=False, metadata={'help': 'if set, zero pads in aggregator instead of repl pad'})
    skip_connections_feat: bool = field(default=False, metadata={'help': 'if set, adds skip connections to the feature extractor'})
    skip_connections_agg: bool = field(default=True, metadata={'help': 'if set, adds skip connections to the aggregator'})
    residual_scale: float = field(default=0.5, metadata={'help': 'scales residual by sqrt(value)'})
    log_compression: bool = field(default=True, metadata={'help': 'if set, adds a log compression to feature extractor'})
    balanced_classes: bool = field(default=False, metadata={'help': 'if set, loss is scaled to balance for number of negatives'})
    project_features: PROJECT_FEATURES_CHOICES = field(default='none', metadata={'help': 'if not none, features are projected using the (same or new) aggregator'})
    non_affine_group_norm: bool = field(default=False, metadata={'help': 'if set, group norm is not affine'})
    offset: str = field(default='auto', metadata={'help': "if set to 'auto', it is computed automatically from the receptive field, else set to int value"})
    activation: ACTIVATION_CHOICES = field(default='relu', metadata={'help': "if set to 'auto', it is computed automatically from the receptive field, else set to int value"})
    vq_type: VQ_TYPE_CHOICES = field(default='none', metadata={'help': 'which type of quantizer to use'})
    vq_vars: int = field(default=320, metadata={'help': 'project to this many vector quantized variables per group'})
    vq_groups: int = field(default=2, metadata={'help': 'number of groups of latent variables'})
    vq_dim: int = field(default=0, metadata={'help': 'uses this dimensionality for quantized vectors. 0 to use model dim // groups'})
    vq_depth: int = field(default=1, metadata={'help': 'number of layers for vq weight projection'})
    combine_groups: bool = field(default=False, metadata={'help': 'if set, variables are shared among groups'})
    vq_temp: Tuple[float, float, float] = field(default=(2.0, 0.5, 0.999995), metadata={'help': 'temperature for latent variable sampling with gumbel softmax. should be a tuple of 3 values (start, end, decay)'})
    vq_gamma: float = field(default=0.25, metadata={'help': 'gamma parameter for kmeans style vector quantization'})
