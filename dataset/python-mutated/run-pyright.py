...
parser = argparse.ArgumentParser(prog='run-pyright', description='Run pyright for every specified pyright environment and print the merged results.\n\nBy default, the venv for each pyright environment is built using `requirements-pinned.txt`. This speeds up venv construction on a development machine and in CI. Occasionally, these pinned requirements will need to be updated. To do this, pass `--update-pins`. This will cause the venv to be rebuilt with the looser requirements specified in `requirements.txt`, and `requirements-pinned.txt` updated with the resulting dependency list.')
parser.add_argument('--all', action='store_true', default=False, help='Run pyright for all environments. Environments are discovered by looking for directories at `pyright/envs/*`.')
parser.add_argument('--unannotated', action='store_true', default=False, help='Analyze unannotated functions. This is not currently used in CI.')
parser.add_argument('--diff', action='store_true', default=False, help='Run pyright on the diff between the working tree and master.')
parser.add_argument('--env', '-e', type=str, action='append', default=[], help='Names of pyright environment to run. Must be a directory in pyright/envs. Can be passed multiple times.')
parser.add_argument('--json', action='store_true', default=False, help='Output results in JSON format.')
parser.add_argument('--rebuild', '-r', action='store_true', default=False, help='Force rebuild of virtual environment.')
parser.add_argument('--update-pins', action='store_true', default=False, help='Update `requirements-pinned.txt` for selected environments (implies `--rebuild`). The virtual env for the selected environment will be rebuilt using the abstract requirements specified in `requirements.txt`. After the environment is built, the full dependency list will be extracted with `pip freeze` and written to `requirements-pinned.txt` (this is what is used in CI and for building the default venv).')
parser.add_argument('paths', type=str, nargs='*', help='Path to directories or python files to target with pyright.')
...
EXTRA_PIP_INSTALL_ARGS: Final = ['--find-links=https://github.com/dagster-io/build-grpcio/wiki/Wheels']
...
if __name__ == '__main__':
    ...
    run_results = [run_pyright(env, paths=env_path_map[env], rebuild=params['rebuild'], unannotated=params['unannotated'], pinned_deps=params['update_pins']) for env in env_path_map]
