...
class DwPwConv(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=1, bias=False):
        if False:
            for i in range(10):
                print('nop')
        super(DwPwConv, self).__init__()
        self.depthwise = nn.Conv2d(in_planes, in_planes, kernel_size, stride, padding, groups=in_planes, bias=bias)
        self.bn1 = nn.BatchNorm2d(in_planes)
        self.relu1 = nn.ReLU(inplace=True)
        self.pointwise = nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=1, padding=0, groups=1, bias=bias)
...
class DwPwConvTranspose(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride):
        ...
        self.depthwise = nn.ConvTranspose2d(in_planes, in_planes, kernel_size, stride, groups=in_planes)
        self.bn1 = nn.BatchNorm2d(in_planes)
        self.relu1 = nn.ReLU(inplace=True)
        self.pointwise = nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=1, padding=0, groups=1)
...
class LightSegDetector(nn.Module):
    ...
    def __init__(self, in_channels=[64, 128, 256, 512], inner_channels=256, k=10, bias=False, adaptive=False, smooth=False, serial=False, dw_kernel_size=3, dw_padding=1, *args, **kwargs):
        if False:
            print('Hello World!')
...
class DBNasModel(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.backbone = CompactDetBackbone(width_stages=[32, 64, 96, 128], input_channel=32, **kwargs)
        self.decoder = LightSegDetector(in_channels=[32, 64, 96, 128], adaptive=True, k=50, inner_channels=64, dw_kernel_size=5, dw_padding=2, **kwargs)
...
class DBModel_v2(nn.Module):
    ...
    def __init__(self, device, distributed: bool=False, local_rank: int=0, *args, **kwargs):
        if False:
            print('Hello World!')
        '\n        DBNet-resnet18 model without deformable conv,\n        paper reference: https://arxiv.org/pdf/1911.08947.pdf\n        '
