...
async def test_flash_briefing_date_from_str(alexa_client) -> None:
    ...
    data = await req.json()
    assert isinstance(datetime.datetime.strptime(data[0].get(const.ATTR_UPDATE_DATE), const.DATE_FORMAT), datetime.datetime)

async def test_flash_briefing_valid(alexa_client) -> None:
    """Test the response is valid."""
    data = [{'titleText': 'NPR', 'redirectionURL': 'https://npr.org', 'streamUrl': NPR_NEWS_MP3_URL, 'mainText': '', 'uid': 'uuid', 'updateDate': '2016-10-10T19:51:42.0Z'}]
    req = await _flash_briefing_req(alexa_client, 'news_audio')
    assert req.status == HTTPStatus.OK
    json = await req.json()
    assert isinstance(datetime.datetime.strptime(json[0].get(const.ATTR_UPDATE_DATE), const.DATE_FORMAT), datetime.datetime)
