...
class TestAddCheckBox(TestCase):
    ...
    def test_add_orderedlist_of_checkboxes(self):
        ...
        pdf.add_page(page)
        page_layout: PageLayout = SingleColumnLayout(page)
        page_layout.add(self.get_test_header('This test creates a PDF with an OrderedList of Checkboxes in it.'))
    ...
    def test_add_unorderedlist_of_checkboxes(self):
        ...
        pdf.add_page(page)
        page_layout: PageLayout = SingleColumnLayout(page)
        page_layout.add(self.get_test_header('This test creates a PDF with an UnorderedList of Checkboxes in it.'))
    ...
    def test_add_table_of_checkboxes(self):
        ...
        pdf.add_page(page)
        page_layout: PageLayout = SingleColumnLayout(page)
        page_layout.add(self.get_test_header('This test creates a PDF with an Table of Checkboxes in it.'))
        page_layout.add(FlexibleColumnWidthTable(number_of_columns=3, number_of_rows=3).add(CheckBox()).add(CheckBox()).add(CheckBox()).add(CheckBox()).add(CheckBox()).add(CheckBox()).add(CheckBox()).add(CheckBox()).add(CheckBox()))
    ...
    def test_add_checkbox_using_borders(self):
        ...
        page_layout: PageLayout = SingleColumnLayout(page)
        page_layout.add(self.get_test_header('This test creates a PDF with a Checkbox in it.'))
        page_layout.add(CheckBox(border_top=True, border_right=True, border_bottom=True, border_left=True, border_color=HexColor('56cbf9'), border_radius_top_left=Decimal(10), border_radius_top_right=Decimal(10), border_radius_bottom_right=Decimal(10)))
