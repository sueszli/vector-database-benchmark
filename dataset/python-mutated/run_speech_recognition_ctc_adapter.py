...
class DataTrainingArguments:
    ...
    target_language: Optional[str] = field(metadata={'help': "The target language on which the adapter attention layers should be trained on in ISO 693-3 code, e.g. `tur` for Turkish Wav2Vec2's MMS ISO codes can be looked up here: https://dl.fbaipublicfiles.com/mms/misc/language_coverage_mms.html If you are not training the adapter layers on a language, simply choose another accronym that fits your data."})
    ...
    overwrite_lang_vocab: bool = field(default=False, metadata={'help': 'If :obj:`True`, will overwrite existing `target_language` vocabulary of tokenizer.'})
...
def main():
    ...
    if training_args.do_train:
        raw_datasets['train'] = load_dataset(data_args.dataset_name, data_args.dataset_config_name, split=data_args.train_split_name, token=data_args.token)
    ...
    if training_args.do_eval:
        raw_datasets['eval'] = load_dataset(data_args.dataset_name, data_args.dataset_config_name, split=data_args.eval_split_name, token=data_args.token)
    ...
    config = AutoConfig.from_pretrained(model_args.model_name_or_path, cache_dir=model_args.cache_dir, token=data_args.token, trust_remote_code=data_args.trust_remote_code)
    tokenizer_name_or_path = model_args.tokenizer_name_or_path
    tokenizer_kwargs = {}
    vocab_dict = {}
    if tokenizer_name_or_path is not None:
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_name_or_path, token=data_args.token, trust_remote_code=data_args.trust_remote_code)
        ...
        if data_args.target_language in tokenizer.vocab and (not data_args.overwrite_lang_vocab):
            logger.info(f"Adapter language already exists. Skipping vocabulary creating. If you want to create a new vocabulary for {data_args.target_language} make sure to add '--overwrite_lang_vocab'")
        else:
    ...
    if tokenizer_name_or_path is None:
        ...
        with training_args.main_process_first(desc='dataset map vocabulary creation'):
            if not os.path.isfile(vocab_file):
                ...
                lang_dict = create_vocabulary_from_data(raw_datasets, word_delimiter_token=word_delimiter_token, unk_token=unk_token, pad_token=pad_token)
                if data_args.target_language is not None:
                    vocab_dict[data_args.target_language] = lang_dict
                with open(vocab_file, 'w') as file:
                    json.dump(vocab_dict, file)
        tokenizer_kwargs = {'config': config if config.tokenizer_class is not None else None, 'tokenizer_type': config.model_type if config.tokenizer_class is None else None, 'unk_token': unk_token, 'pad_token': pad_token, 'word_delimiter_token': word_delimiter_token, 'target_lang': data_args.target_language}
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_name_or_path, token=data_args.token, trust_remote_code=data_args.trust_remote_code, **tokenizer_kwargs)
    feature_extractor = AutoFeatureExtractor.from_pretrained(model_args.model_name_or_path, cache_dir=model_args.cache_dir, token=data_args.token, trust_remote_code=data_args.trust_remote_code)
    config.update({'final_dropout': model_args.final_dropout, 'mask_time_prob': model_args.mask_time_prob, 'mask_time_length': model_args.mask_time_length, 'mask_feature_prob': model_args.mask_feature_prob, 'mask_feature_length': model_args.mask_feature_length, 'gradient_checkpointing': training_args.gradient_checkpointing, 'layerdrop': model_args.layerdrop, 'ctc_loss_reduction': model_args.ctc_loss_reduction, 'pad_token_id': tokenizer.pad_token_id, 'vocab_size': len(tokenizer), 'adapter_attn_dim': model_args.adapter_attn_dim})
    model = AutoModelForCTC.from_pretrained(model_args.model_name_or_path, cache_dir=model_args.cache_dir, config=config, token=data_args.token, trust_remote_code=data_args.trust_remote_code, ignore_mismatched_sizes=True)
    ...
    trainer = Trainer(model=model, data_collator=data_collator, args=training_args, compute_metrics=compute_metrics, train_dataset=vectorized_datasets['train'] if training_args.do_train else None, eval_dataset=vectorized_datasets['eval'] if training_args.do_eval else None, tokenizer=processor)
    if training_args.do_train:
    ...
    kwargs = {'finetuned_from': model_args.model_name_or_path, 'tasks': 'automatic-speech-recognition', 'tags': ['automatic-speech-recognition', data_args.dataset_name, 'mms'], 'dataset_args': f'Config: {config_name}, Training split: {data_args.train_split_name}, Eval split: {data_args.eval_split_name}', 'dataset': f'{data_args.dataset_name.upper()} - {config_name.upper()}'}
