...
RAND_LOW = 0
RAND_HIGH = 100
test_data = {'int_data': {'col{}'.format(int((i - NCOLS / 2) % NCOLS + 1)): random_state.randint(RAND_LOW, RAND_HIGH, size=NROWS) for i in range(NCOLS)}, 'float_nan_data': {'col{}'.format(int((i - NCOLS / 2) % NCOLS + 1)): [x if j % 4 == 0 and i > NCOLS // 2 or (j != i and i <= NCOLS // 2) else np.NaN for (j, x) in enumerate(random_state.uniform(RAND_LOW, RAND_HIGH, size=NROWS))] for i in range(NCOLS)}}
test_data['int_data']['index'] = test_data['int_data'].pop('col{}'.format(int(NCOLS / 2)))
