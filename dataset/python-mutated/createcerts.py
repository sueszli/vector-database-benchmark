...
from cryptography.x509 import BasicConstraints, CertificateBuilder, DNSName, ExtendedKeyUsage, IPAddress, KeyUsage, Name, NameAttribute, SubjectAlternativeName, random_serial_number
...
cert = selfsigned(key, 'sanic.example', ['sanic.example', 'www.sanic.example', '*.sanic.test', '2001:db8::541c'])
writefiles(key, cert)
key = ec.generate_private_key(ec.SECP256R1)
cert = selfsigned(key, 'localhost', ['localhost', '127.0.0.1', '::1'])
writefiles(key, cert)