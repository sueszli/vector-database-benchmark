...
class DatabaseProxy(object):
    ...
    * `_users`:	The addressable path of the users resource.
    ...
    @distributed_trace_async
    async def create_container(self, id: str, partition_key: PartitionKey, **kwargs: Any) -> ContainerProxy:
        ...
        if indexing_policy is not None:
            if indexing_policy.get('indexingMode') is IndexingMode.Lazy:
                warnings.warn('Lazy indexing mode has been deprecated. Mode will be set to consistent indexing by the backend.', DeprecationWarning)
        ...
        data = await self.client_connection.CreateContainer(database_link=self.database_link, collection=definition, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, data)
        return ContainerProxy(self.client_connection, self.database_link, data['id'], properties=data)

    @distributed_trace_async
    async def create_container_if_not_exists(self, id: str, partition_key: PartitionKey, **kwargs: Any) -> ContainerProxy:
        ...
        try:
            ...
            return await self.create_container(id=id, partition_key=partition_key, indexing_policy=indexing_policy, default_ttl=default_ttl, offer_throughput=offer_throughput, unique_key_policy=unique_key_policy, conflict_resolution_policy=conflict_resolution_policy, analytical_storage_ttl=analytical_storage_ttl)
    ...
    @distributed_trace_async
    async def replace_container(self, container: Union[str, ContainerProxy, Dict[str, Any]], partition_key: PartitionKey, **kwargs: Any) -> ContainerProxy:
        ...
        conflict_resolution_policy = kwargs.pop('conflict_resolution_policy', None)
        analytical_storage_ttl = kwargs.pop('analytical_storage_ttl', None)
        parameters = {key: value for (key, value) in {'id': container_id, 'partitionKey': partition_key, 'indexingPolicy': indexing_policy, 'defaultTtl': default_ttl, 'conflictResolutionPolicy': conflict_resolution_policy, 'analyticalStorageTtl': analytical_storage_ttl}.items() if value is not None}
        container_properties = await self.client_connection.ReplaceContainer(container_link, collection=parameters, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, container_properties)
        return ContainerProxy(self.client_connection, self.database_link, container_properties['id'], properties=container_properties)

    @distributed_trace_async
    async def delete_container(self, container: Union[str, ContainerProxy, Dict[str, Any]], **kwargs: Any) -> None:
        """Delete a container.
    ...
    async def create_user(self, body: Dict[str, Any], **kwargs: Any) -> UserProxy:
        ...
        request_options = _build_options(kwargs)
        response_hook = kwargs.pop('response_hook', None)
        user = await self.client_connection.CreateUser(database_link=self.database_link, user=body, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, user)
        return UserProxy(client_connection=self.client_connection, id=user['id'], database_link=self.database_link, properties=user)
    ...
    async def upsert_user(self, body: Dict[str, Any], **kwargs: Any) -> UserProxy:
        ...
        request_options = _build_options(kwargs)
        response_hook = kwargs.pop('response_hook', None)
        user = await self.client_connection.UpsertUser(database_link=self.database_link, user=body, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, user)
        return UserProxy(client_connection=self.client_connection, id=user['id'], database_link=self.database_link, properties=user)

    @distributed_trace_async
    async def replace_user(self, user: Union[str, UserProxy, Dict[str, Any]], body: Dict[str, Any], **kwargs: Any) -> UserProxy:
        ...
        request_options = _build_options(kwargs)
        response_hook = kwargs.pop('response_hook', None)
        replaced_user: Dict[str, Any] = await self.client_connection.ReplaceUser(user_link=self._get_user_link(user), user=body, options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, replaced_user)
        return UserProxy(client_connection=self.client_connection, id=replaced_user['id'], database_link=self.database_link, properties=replaced_user)
    ...
    async def delete_user(self, user: Union[str, UserProxy, Dict[str, Any]], **kwargs: Any) -> None:
        ...
        request_options = _build_options(kwargs)
        response_hook = kwargs.pop('response_hook', None)
        result = await self.client_connection.DeleteUser(user_link=self._get_user_link(user), options=request_options, **kwargs)
        if response_hook:
            response_hook(self.client_connection.last_response_headers, result)
    ...
    async def get_throughput(self, **kwargs: Any) -> ThroughputProperties:
        ...
        if len(throughput_properties) == 0:
            raise CosmosResourceNotFoundError(status_code=StatusCodes.NOT_FOUND, message='Could not find ThroughputProperties for database ' + self.database_link)
        if response_hook:
    ...
    async def replace_throughput(self, throughput: Union[int, ThroughputProperties], **kwargs: Any) -> ThroughputProperties:
        ...
        throughput_properties = [throughput async for throughput in self.client_connection.QueryOffers(query_spec, **kwargs)]
        if len(throughput_properties) == 0:
            raise CosmosResourceNotFoundError(status_code=StatusCodes.NOT_FOUND, message='Could not find Offer for database ' + self.database_link)
