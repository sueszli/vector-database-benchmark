"""Converts videos to training, validation, test, and debug tfrecords on cns.
...
tf.app.flags.DEFINE_string('view_pattern', '_view[_]*[0]+[.].*', 'view regexp pattern for first view')
tf.app.flags.DEFINE_string('input_dir', '', 'input data path')
tf.app.flags.DEFINE_integer('resize_min_edge', 0, 'resize the smallest edge to this size.')
tf.app.flags.DEFINE_integer('rotate', 0, 'rotate the image in degrees.')
tf.app.flags.DEFINE_string('rotate_if_matching', None, 'rotate only if video path matches regexp.')
tf.app.flags.DEFINE_string('output_dir', '', 'output directory for the dataset')
tf.app.flags.DEFINE_integer('max_per_shard', -1, 'max # of frames per data chunk')
...
tf.app.flags.DEFINE_integer('max_views_discrepancy', 100, 'Maximum length difference (in frames) allowed between views')
