...
class UnivnetConfig(BaseGANVocoderConfig):
    ...
    generator_model_params: Dict = field(default_factory=lambda : {'in_channels': 64, 'out_channels': 1, 'hidden_channels': 32, 'cond_channels': 80, 'upsample_factors': [8, 8, 4], 'lvc_layers_each_block': 4, 'lvc_kernel_size': 3, 'kpnet_hidden_channels': 64, 'kpnet_conv_size': 3, 'dropout': 0.0})
    ...
    stft_loss_params: Dict = field(default_factory=lambda : {'n_ffts': [1024, 2048, 512], 'hop_lengths': [120, 240, 50], 'win_lengths': [600, 1200, 240]})
    subband_stft_loss_weight: float = 0
    mse_G_loss_weight: float = 1
    hinge_G_loss_weight: float = 0
    feat_match_loss_weight: float = 0
    l1_spec_loss_weight: float = 0
    l1_spec_loss_params: Dict = field(default_factory=lambda : {'use_mel': True, 'sample_rate': 22050, 'n_fft': 1024, 'hop_length': 256, 'win_length': 1024, 'n_mels': 80, 'mel_fmin': 0.0, 'mel_fmax': None})
    lr_gen: float = 0.0001
