...
@unittest.skipIf(paddle.is_compiled_with_rocm(), "ROCm doesn't support fp64 LayerNormOpByOp currently")
...
@unittest.skipIf(not core.is_compiled_with_cuda() or paddle.is_compiled_with_rocm() or (not core.is_bfloat16_supported(core.CUDAPlace(0))), 'core is not compiled with CUDA or not support the bfloat16')
...
@unittest.skipIf(not core.is_compiled_with_cuda(), 'core is not compiled with CUDA or not support the float16')
...
@unittest.skipIf(not core.is_compiled_with_cuda() or paddle.is_compiled_with_rocm(), 'core is not compiled with CUDA or not support the FastMath')
