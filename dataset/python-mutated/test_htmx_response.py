...
from litestar.contrib.htmx.response import ClientRedirect, ClientRefresh, HTMXTemplate, HXLocation, HXStopPolling, PushUrl, ReplaceUrl, Reswap, Retarget, TriggerEvent
...
async def test_hx_location_response_with_all_parameters() -> None:
    ...
    with create_test_client(route_handlers=[handler], request_class=HTMXRequest) as client:
        ...
        assert response.status_code == HTTP_200_OK
        assert 'Location' not in response.headers
        assert spec == '{"path":"/contact-us","source":"#button","event":"click","target":"#content","swap":"innerHTML","values":{"action":"true"},"hx_headers":{"attribute":"value"}}'
