...
from ....webhook.validators import HEADERS_LENGTH_LIMIT, HEADERS_NUMBER_LIMIT, custom_headers_validator
...
class WebhookCreateInput(BaseInputObjectType):
    ...
    events = NonNullList(enums.WebhookEventTypeEnum, description=f'The events that webhook wants to subscribe. {DEPRECATED_IN_3X_INPUT} Use `asyncEvents` or `syncEvents` instead.')
    async_events = NonNullList(enums.WebhookEventTypeAsyncEnum, description='The asynchronous events that webhook wants to subscribe.')
    sync_events = NonNullList(enums.WebhookEventTypeSyncEnum, description='The synchronous events that webhook wants to subscribe.')
    ...
    secret_key = graphene.String(description=f'The secret key used to create a hash signature with each payload.{DEPRECATED_IN_3X_INPUT} As of Saleor 3.5, webhook payloads default to signing using a verifiable JWS.', required=False)
...
class WebhookCreate(ModelMutation, NotifyUserEventValidationMixin):
    ...
    class Arguments:
        input = WebhookCreateInput(description='Fields required to create a webhook.', required=True)
