...
_attached_packages = []
TRACKING_RE = re.compile('(tracking url|the url to track the job):\\s+(?P<url>.+)$')

class hadoop(luigi.task.Config):
    pool = luigi.OptionalParameter(default=None, description='Hadoop pool so use for Hadoop tasks. To specify pools per tasks, see BaseHadoopJobTask.pool')
