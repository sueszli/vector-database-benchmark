...
if is_tf_available():
    ...
    from transformers import TF_LAYOUTLMV3_PRETRAINED_MODEL_ARCHIVE_LIST, TF_MODEL_FOR_MULTIPLE_CHOICE_MAPPING, TF_MODEL_FOR_QUESTION_ANSWERING_MAPPING, TF_MODEL_FOR_SEQUENCE_CLASSIFICATION_MAPPING, TF_MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING, LayoutLMv3Config, TFLayoutLMv3ForQuestionAnswering, TFLayoutLMv3ForSequenceClassification, TFLayoutLMv3ForTokenClassification, TFLayoutLMv3Model
...
class TFLayoutLMv3ModelTester:
    ...
    def create_and_check_for_sequence_classification(self, config, input_ids, bbox, pixel_values, token_type_ids, input_mask, sequence_labels):
        if False:
            while True:
                i = 10
        config.num_labels = self.num_labels
        model = TFLayoutLMv3ForSequenceClassification(config=config)
        result = model(input_ids, bbox=bbox, pixel_values=pixel_values, attention_mask=input_mask, token_type_ids=token_type_ids, labels=sequence_labels, training=False)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.num_labels))

    def create_and_check_for_token_classification(self, config, input_ids, bbox, pixel_values, token_type_ids, input_mask, token_labels):
        if False:
            print('Hello World!')
        config.num_labels = self.num_labels
        model = TFLayoutLMv3ForTokenClassification(config=config)
        result = model(input_ids, bbox=bbox, pixel_values=pixel_values, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels, training=False)
        self.parent.assertEqual(result.logits.shape, (self.batch_size, self.text_seq_length, self.num_labels))

    def create_and_check_for_question_answering(self, config, input_ids, bbox, pixel_values, token_type_ids, input_mask, sequence_labels):
        if False:
            while True:
                i = 10
        config.num_labels = 2
        model = TFLayoutLMv3ForQuestionAnswering(config=config)
        result = model(input_ids, bbox=bbox, pixel_values=pixel_values, attention_mask=input_mask, token_type_ids=token_type_ids, start_positions=sequence_labels, end_positions=sequence_labels, training=False)
...
class TFLayoutLMv3ModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TFLayoutLMv3Model, TFLayoutLMv3ForQuestionAnswering, TFLayoutLMv3ForSequenceClassification, TFLayoutLMv3ForTokenClassification) if is_tf_available() else ()
    pipeline_model_mapping = {'document-question-answering': TFLayoutLMv3ForQuestionAnswering, 'feature-extraction': TFLayoutLMv3Model} if is_tf_available() else {}
    test_pruning = False
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False) -> dict:
        ...
        if return_labels:
            if model_class in get_values(TF_MODEL_FOR_MULTIPLE_CHOICE_MAPPING):
                ...
                inputs_dict['labels'] = tf.zeros((self.model_tester.batch_size, self.model_tester.text_seq_length), dtype=tf.int32)
        return inputs_dict
...
class TFLayoutLMv3ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = tf.constant([[-0.0529, 0.3618, 0.1632], [-0.1587, -0.1667, -0.04], [-0.1557, -0.1671, -0.0505]])
        self.assertTrue(np.allclose(outputs.last_hidden_state[0, :3, :3], expected_slice, atol=0.0001))