...
class UnistraIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        files = set(re.findall('file\\s*:\\s*"(/[^"]+)"', webpage))
        quality = qualities(['SD', 'HD'])
        formats = []
        for file_path in files:
            format_id = 'HD' if file_path.endswith('-HD.mp4') else 'SD'
            formats.append({'url': 'http://vod-flash.u-strasbg.fr:8080%s' % file_path, 'format_id': format_id, 'quality': quality(format_id)})
        title = self._html_search_regex('<title>UTV - (.*?)</', webpage, 'title')
        description = self._html_search_regex('<meta name="Description" content="(.*?)"', webpage, 'description', flags=re.DOTALL)
