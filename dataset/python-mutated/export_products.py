...
class ExportInfoInput(BaseInputObjectType):
    attributes = NonNullList(graphene.ID, description='List of attribute ids witch should be exported.')
    warehouses = NonNullList(graphene.ID, description='List of warehouse ids witch should be exported.')
    channels = NonNullList(graphene.ID, description='List of channels ids which should be exported.')
    fields = NonNullList(ProductFieldEnum, description='List of product fields witch should be exported.')

    class Meta:
        doc_category = DOC_CATEGORY_PRODUCTS

class ExportProductsInput(BaseInputObjectType):
    scope = ExportScope(description='Determine which products should be exported.', required=True)
    filter = ProductFilterInput(description='Filtering options for products.', required=False)
    ids = NonNullList(graphene.ID, description='List of products IDs to export.', required=False)
    export_info = ExportInfoInput(description='Input with info about fields which should be exported.', required=False)
...
class ExportProducts(BaseExportMutation):
    ...
    class Arguments:
        input = ExportProductsInput(required=True, description='Fields required to export product data.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.NOTIFY_USER, description='A notification for the exported file.'), WebhookEventInfo(type=WebhookEventAsyncType.PRODUCT_EXPORT_COMPLETED, description='A notification for the exported file.')]
