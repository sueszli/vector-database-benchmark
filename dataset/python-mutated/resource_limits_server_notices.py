...
from synapse.api.constants import EventTypes, LimitBlockingTypes, ServerNoticeLimitReached, ServerNoticeMsgType
...
class ResourceLimitsServerNotices:
    ...
    async def maybe_send_server_notice_to_user(self, user_id: str) -> None:
        ...
        if timestamp is None:
            return
        room_id = await self._server_notices_manager.maybe_get_notice_room_for_user(user_id)
        ...
        try:
            if limit_type == LimitBlockingTypes.MONTHLY_ACTIVE_USER and (not self._config.server.mau_limit_alerting):
            ...
            if currently_blocked and (not limit_msg):
                ...
                await self._apply_limit_block_notification(user_id, limit_msg, limit_type)
        except SynapseError as e:
            logger.error('Error sending resource limits server notice: %s', e)

    async def _remove_limit_block_notification(self, user_id: str, ref_events: List[str]) -> None:
        ...
        await self._server_notices_manager.send_notice(user_id, content, EventTypes.Pinned, '')

    async def _apply_limit_block_notification(self, user_id: str, event_body: str, event_limit_type: str) -> None:
        ...
        """
        content = {'body': event_body, 'msgtype': ServerNoticeMsgType, 'server_notice_type': ServerNoticeLimitReached, 'admin_contact': self._config.server.admin_contact, 'limit_type': event_limit_type}
        event = await self._server_notices_manager.send_notice(user_id, content, EventTypes.Message)
    ...
    async def _is_room_currently_blocked(self, room_id: str) -> Tuple[bool, List[str]]:
        ...
        try:
            pinned_state_event = await self._storage_controllers.state.get_current_state_event(room_id, event_type=EventTypes.Pinned, state_key='')
        except AuthError:
            pass
