...
from homeassistant.components.climate import PRESET_AWAY, PRESET_COMFORT, PRESET_ECO, PRESET_NONE, ClimateEntity, ClimateEntityFeature, HVACAction, HVACMode
from homeassistant.const import ATTR_TEMPERATURE, UnitOfTemperature
from ..coordinator import OverkizDataUpdateCoordinator
from ..entity import OverkizEntity
OVERKIZ_TO_PRESET_MODES: dict[str, str] = {OverkizCommandParam.SECURED: PRESET_AWAY, OverkizCommandParam.ECO: PRESET_ECO, OverkizCommandParam.COMFORT: PRESET_COMFORT, OverkizCommandParam.FREE: PRESET_NONE}
PRESET_MODES_TO_OVERKIZ = {v: k for (k, v) in OVERKIZ_TO_PRESET_MODES.items()}
OVERKIZ_TO_HVAC_MODES: dict[str, HVACMode] = {OverkizCommandParam.AUTO: HVACMode.AUTO, OverkizCommandParam.MANU: HVACMode.HEAT_COOL}
HVAC_MODES_TO_OVERKIZ = {v: k for (k, v) in OVERKIZ_TO_HVAC_MODES.items()}
OVERKIZ_TO_HVAC_ACTION: dict[str, HVACAction] = {OverkizCommandParam.COOLING: HVACAction.COOLING, OverkizCommandParam.HEATING: HVACAction.HEATING}
MAP_PRESET_TEMPERATURES: dict[str, str] = {PRESET_COMFORT: OverkizState.CORE_COMFORT_ROOM_TEMPERATURE, PRESET_ECO: OverkizState.CORE_ECO_ROOM_TEMPERATURE, PRESET_AWAY: OverkizState.CORE_SECURED_POSITION_TEMPERATURE}
SETPOINT_MODE_TO_OVERKIZ_COMMAND: dict[str, str] = {OverkizCommandParam.COMFORT: OverkizCommand.SET_COMFORT_TEMPERATURE, OverkizCommandParam.ECO: OverkizCommand.SET_ECO_TEMPERATURE, OverkizCommandParam.SECURED: OverkizCommand.SET_SECURED_POSITION_TEMPERATURE}
...
class SomfyHeatingTemperatureInterface(OverkizEntity, ClimateEntity):
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        await self.executor.async_execute_command(OverkizCommand.SET_ACTIVE_MODE, HVAC_MODES_TO_OVERKIZ[hvac_mode])
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        """Set new preset mode."""
        await self.executor.async_execute_command(OverkizCommand.SET_MANU_AND_SET_POINT_MODES, PRESET_MODES_TO_OVERKIZ[preset_mode])
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        if (mode := self.device.states[OverkizState.OVP_HEATING_TEMPERATURE_INTERFACE_SETPOINT_MODE]) and mode.value_as_str:
            return await self.executor.async_execute_command(SETPOINT_MODE_TO_OVERKIZ_COMMAND[mode.value_as_str], temperature)