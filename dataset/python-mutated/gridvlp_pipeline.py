...
from modelscope.utils.constant import DEFAULT_MODEL_REVISION, Frameworks, Invoke, Tasks
...
class Config:
    SCALE = 1 / 255.0
    MEAN = np.require([0.485, 0.456, 0.406], dtype=np.float32)[:, np.newaxis, np.newaxis]
    STD = np.require([0.229, 0.224, 0.225], dtype=np.float32)[:, np.newaxis, np.newaxis]
...
@PIPELINES.register_module(Tasks.multi_modal_embedding, module_name=Pipelines.gridvlp_multi_modal_embedding)
