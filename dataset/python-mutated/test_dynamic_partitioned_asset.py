...
from docs_snippets.concepts.partitions_schedules_sensors.dynamic_partitioned_asset import image_sensor, images
DIR = 'images'
...
def test():
    ...
    with tempfile.TemporaryDirectory() as tmpdir_path:
        ...
        mockenv = mock.patch.dict(os.environ, {'MY_DIRECTORY': os.path.join(os.path.join(tmpdir_path, DIR))})
        try:
            mockenv.start()
            with instance_for_test() as instance:
                with build_sensor_context(instance=instance) as context:
                    ...
                    assert result.dynamic_partitions_requests[0].partition_keys == [partition_key]
        finally:
            mockenv.stop()