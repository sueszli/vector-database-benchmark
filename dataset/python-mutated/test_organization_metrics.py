...
from sentry.testutils.cases import APITestCase, MetricsAPIBaseTestCase, OrganizationMetricMetaIntegrationTestCase
from sentry.testutils.silo import assume_test_silo_mode, region_silo_test
from sentry.testutils.skips import requires_snuba
pytestmark = [pytest.mark.sentry_metrics, requires_snuba]
MOCKED_DERIVED_METRICS = copy.deepcopy(DERIVED_METRICS)
MOCKED_DERIVED_METRICS.update({'crash_free_fake': SingularEntityDerivedMetric(metric_mri='crash_free_fake', metrics=[SessionMRI.CRASHED.value, SessionMRI.ERRORED_SET.value], unit='percentage', snql=lambda crashed_count, errored_set, entity, metric_ids, alias=None: complement(division_float(crashed_count, errored_set, alias=alias), alias='crash_free_fake'))})
...
class OrganizationMetricsPermissionTest(APITestCase):
    endpoints = (('sentry-api-0-organization-metrics-index',), ('sentry-api-0-organization-metric-details', 'foo'), ('sentry-api-0-organization-metrics-tags',), ('sentry-api-0-organization-metrics-tag-details', 'foo'), ('sentry-api-0-organization-metrics-data',))
