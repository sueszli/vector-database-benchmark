...
plot_figure = figure(title='Multi-Select', height=450, width=600, tools='save,reset', toolbar_location='below')
plot_figure.scatter('x', 'y', color='label', source=source, size=10)
multi_select = MultiSelect(title='Filter Plot by color:', value=['Red', 'Orange'], options=[('Red', 'Red'), ('Orange', 'Orange')])

def multiselect_click(attr, old, new):
    if False:
        for i in range(10):
            print('nop')
    active_mselect = multi_select.value
    selected_df = df[df['label'].isin(active_mselect)]
