...
class shellCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'add_public_key': ('environment', 'key'), 'authorize_environment': ('name', 'access_token', 'id_token', 'expire_time'), 'get_environment': ('name',), 'remove_public_key': ('environment', 'key'), 'start_environment': ('name', 'access_token', 'public_keys')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=shellCallTransformer()):
    if False:
        while True:
            i = 10
