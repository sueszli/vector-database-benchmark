...
def model():
    ...
    starts = [0.2, 0.8]
    ends = [0.1, 0.1]
    edges = [[d1, d1, 0.1], [d1, d2, 0.8], [d2, d1, 0.3], [d2, d2, 0.6]]
    model = SparseHMM(distributions=[d1, d2], edges=edges, starts=starts, ends=ends, random_state=0)
    return model
...
def test_initialization_raises():
    ...
    assert_raises(ValueError, SparseHMM, d, edges=[[d1, 0.1, 0.1], [d1, 0.1, 0.1]])
...
def test_reset_cache(model, X):
    if False:
        ...
        return i + 15
    model.summarize(X)
    assert_array_almost_equal(model._xw_sum, [0.0002666848, 1.895243, 2.635099, 3.469392], 4)
...
def test_sample_length(model):
    ...
    ends = [0.1, 0.1]
    edges = [[d1, d1, 0.1], [d1, d2, 0.8], [d2, d1, 0.3], [d2, d2, 0.6]]
    model = SparseHMM(distributions=[d1, d2], edges=edges, starts=starts, ends=ends, sample_length=3, random_state=0)
    X = model.sample(25)
...
def test_sample_paths(model):
    ...
    starts = [0.2, 0.8]
    ends = [0.1, 0.1]
    edges = [[d1, d1, 0.4], [d1, d2, 0.5], [d2, d1, 0.6], [d2, d2, 0.3]]
    model = SparseHMM(distributions=[d1, d2], edges=edges, starts=starts, ends=ends, return_sample_paths=True, random_state=0)
...
def test_partial_summarize(model, X):
    ...
    model.summarize(X[:1])
    assert_array_almost_equal(model._xw_sum, [0.0002635347, 0.143041, 0.8828942, 2.973804], 4)
...
def test_summarize_weighted(model, X, w):
    ...
    assert_array_almost_equal(model._xw_sum, [0.0002707798, 4.173105, 4.912965, 4.113657], 4)
    ...
    assert_array_almost_equal(d1._xw_sum, [8.834015, 5.17916, 0.006910709], 4)
...
def test_from_summaries(model, X):
    ...
    assert_array_almost_equal(model.edges, [-9.248932, -0.380141, -1.009072, -0.734015], 4)
...
def test_from_summaries_weighted(model, X, w):
    ...
    assert_array_almost_equal(model.edges, [-9.833524, -0.190658, -0.846142, -1.023709], 4)
    assert_array_almost_equal(d1.scales, [1.749434, 1.025649, 0.001368553], 4)
    assert_array_almost_equal(d1._w_sum, [0.0, 0.0, 0.0])
    assert_array_almost_equal(d1._xw_sum, [0.0, 0.0, 0.0])
...
def test_from_summaries_inertia(X):
    ...
    d2 = Exponential([1.5, 3.1, 2.2])
    edges = [[d1, d1, 0.1], [d1, d2, 0.8], [d2, d1, 0.3], [d2, d2, 0.6]]
    model = SparseHMM(distributions=[d1, d2], edges=edges, starts=[0.2, 0.8], ends=[0.1, 0.1], inertia=0.3)
    model.summarize(X)
    model.from_summaries()
    assert_array_almost_equal(model.starts, [-2.362523, -0.116391], 4)
    assert_array_almost_equal(model.ends, [-1.496878, -1.99371], 4)
    assert_array_almost_equal(model.edges, [-7.165028, -0.333042, -1.067542, -0.667058], 4)
    ...
    edges = [[d1, d1, 0.1], [d1, d2, 0.8], [d2, d1, 0.3], [d2, d2, 0.6]]
    model = SparseHMM(distributions=[d1, d2], edges=edges, starts=[0.2, 0.8], ends=[0.1, 0.1], inertia=0.0)
    model.summarize(X)
    model.from_summaries()
...
def test_from_summaries_weighted_inertia(X, w):
    ...
    assert_array_almost_equal(model.edges, [-7.574241, -0.200404, -0.953492, -0.869843], 4)
...
def test_from_summaries_frozen(model, X):
    ...
    d2 = Exponential([1.5, 3.1, 2.2])
    edges = [[d1, d1, 0.1], [d1, d2, 0.8], [d2, d1, 0.3], [d2, d2, 0.6]]
    model = SparseHMM(distributions=[d1, d2], edges=edges, starts=[0.2, 0.8], ends=[0.1, 0.1], frozen=True)
    model.summarize(X)
    model.from_summaries()
...
def test_fit(X):
    ...
    d2 = Exponential([1.5, 3.1, 2.2])
    edges = [[d1, d1, 0.1], [d1, d2, 0.8], [d2, d1, 0.3], [d2, d2, 0.6]]
    model = SparseHMM(distributions=[d1, d2], edges=edges, starts=[0.2, 0.8], ends=[0.1, 0.1], max_iter=1)
    model.fit(X)
    assert_array_almost_equal(model.starts, [-14.89857, -3.385568e-07], 4)
    assert_array_almost_equal(model.ends, [-1.110725, -1.609444])
    assert_array_almost_equal(model.edges, [-23.442364, -0.399464, -11.60755, -0.223153])
    ...
    d2 = Exponential([1.5, 3.1, 2.2])
    edges = [[d1, d1, 0.1], [d1, d2, 0.8], [d2, d1, 0.3], [d2, d2, 0.6]]
    model = SparseHMM(distributions=[d1, d2], edges=edges, starts=[0.2, 0.8], ends=[0.1, 0.1], max_iter=5)
    model.fit(X)
    assert_array_almost_equal(model.starts, [-15.45504, -1.940718e-07], 4)
    assert_array_almost_equal(model.ends, [-0.758036, -1.609449])
    assert_array_almost_equal(model.edges, [-23.906055, -0.632214, -11.732577, -0.223151])
...
def test_masked_forward_backward(model, X, X_masked):
    ...
    assert_array_almost_equal(t, [[0.0002635347, 0.143041, 0.8828942, 2.973804], [3.150036e-06, 1.752202, 1.752205, 0.4955886]], 4)
    ...
    assert_array_almost_equal(t, [[0.041682, 0.643657, 1.405594, 1.90907], [0.092079, 1.618498, 1.767671, 0.521754]], 4)
...
def test_masked_ones_from_summaries(model, X, w):
    ...
    assert_array_almost_equal(model.edges, [-9.833523, -0.190658, -0.846143, -1.023708], 4)
