...
TaxExemptionManageErrorCode = graphene.Enum.from_enum(error_codes.TaxExemptionManageErrorCode)
...
class TaxExemptionManage(BaseMutation):
    ...
    class Arguments:
        id = graphene.ID(description='ID of the Checkout or Order object.', required=True)
        tax_exemption = graphene.Boolean(description='Determines if a taxes should be exempt.', required=True)

    class Meta:
        description = "Exempt checkout or order from charging the taxes. When tax exemption is enabled, taxes won't be charged for the checkout or order. Taxes may still be calculated in cases when product prices are entered with the tax included and the net price needs to be known." + ADDED_IN_38
        doc_category = DOC_CATEGORY_TAXES
