...
from strawberry.subscriptions.protocols.graphql_ws import GQL_COMPLETE, GQL_CONNECTION_ACK, GQL_CONNECTION_ERROR, GQL_CONNECTION_INIT, GQL_CONNECTION_KEEP_ALIVE, GQL_CONNECTION_TERMINATE, GQL_DATA, GQL_ERROR, GQL_START, GQL_STOP
if TYPE_CHECKING:
...
async def ws_raw(http_client: HttpClient) -> AsyncGenerator[WebSocketClient, None]:
    async with http_client.ws_connect('/graphql', protocols=[GRAPHQL_WS_PROTOCOL]) as ws:
        yield ws
    await ws.close()
    assert ws.closed
...
async def test_simple_subscription(ws: WebSocketClient):
    await ws.send_json({'type': GQL_START, 'id': 'demo', 'payload': {'query': 'subscription { echo(message: "Hi") }'}})
    response = await ws.receive_json()
...
async def test_sends_keep_alive(aiohttp_app_client: HttpClient):
    ...
    async with aiohttp_app_client.ws_connect('/graphql', protocols=[GRAPHQL_WS_PROTOCOL]) as ws:
        await ws.send_json({'type': GQL_CONNECTION_INIT})
        await ws.send_json({'type': GQL_START, 'id': 'demo', 'payload': {'query': 'subscription { echo(message: "Hi", delay: 0.15) }'}})
        response = await ws.receive_json()
        assert response['type'] == GQL_CONNECTION_ACK
        keepalive_count = 0
        while True:
...
async def test_subscription_cancellation(ws: WebSocketClient):
    await ws.send_json({'type': GQL_START, 'id': 'demo', 'payload': {'query': 'subscription { echo(message: "Hi", delay: 99) }'}})
    await ws.send_json({'type': GQL_START, 'id': 'debug1', 'payload': {'query': 'subscription { debug { numActiveResultHandlers } }'}})
    response = await ws.receive_json()
    assert response['type'] == GQL_DATA
    ...
    assert response['type'] == GQL_COMPLETE
    assert response['id'] == 'demo'
    await ws.send_json({'type': GQL_START, 'id': 'debug2', 'payload': {'query': 'subscription { debug { numActiveResultHandlers} }'}})
    response = await ws.receive_json()
    assert response['type'] == GQL_DATA
...
async def test_subscription_errors(ws: WebSocketClient):
    await ws.send_json({'type': GQL_START, 'id': 'demo', 'payload': {'query': 'subscription { error(message: "TEST ERR") }'}})
    response = await ws.receive_json()
    assert response['type'] == GQL_DATA
    assert response['id'] == 'demo'
...
async def test_subscription_exceptions(ws: WebSocketClient):
    await ws.send_json({'type': GQL_START, 'id': 'demo', 'payload': {'query': 'subscription { exception(message: "TEST EXC") }'}})
    response = await ws.receive_json()
    assert response['type'] == GQL_DATA
    assert response['id'] == 'demo'
    assert response['payload']['data'] is None
...
async def test_subscription_field_error(ws: WebSocketClient):
    await ws.send_json({'type': GQL_START, 'id': 'invalid-field', 'payload': {'query': 'subscription { notASubscriptionField }'}})
    response = await ws.receive_json()
    assert response['type'] == GQL_ERROR
    assert response['id'] == 'invalid-field'
    assert response['payload'] == {'locations': [{'line': 1, 'column': 16}], 'message': "The subscription field 'notASubscriptionField' is not defined."}

async def test_subscription_syntax_error(ws: WebSocketClient):
    await ws.send_json({'type': GQL_START, 'id': 'syntax-error', 'payload': {'query': 'subscription { example '}})
    response = await ws.receive_json()
    assert response['type'] == GQL_ERROR
    assert response['id'] == 'syntax-error'
    assert response['payload'] == {'locations': [{'line': 1, 'column': 24}], 'message': 'Syntax Error: Expected Name, found <EOF>.'}
...
async def test_custom_context(ws: WebSocketClient):
    await ws.send_json({'type': GQL_START, 'id': 'demo', 'payload': {'query': 'subscription { context }'}})
    response = await ws.receive_json()
    assert response['type'] == GQL_DATA
    assert response['id'] == 'demo'
...
async def test_resolving_enums(ws: WebSocketClient):
    await ws.send_json({'type': GQL_START, 'id': 'demo', 'payload': {'query': 'subscription { flavors }'}})
    response = await ws.receive_json()
    assert response['type'] == GQL_DATA
    assert response['id'] == 'demo'
...
async def test_task_cancellation_separation(aiohttp_app_client: HttpClient):
    ...
    def get_result_handler_tasks():
        if False:
            for i in range(10):
                print('nop')
        return [task for task in asyncio.all_tasks() if 'BaseGraphQLWSHandler.handle_async_results' in repr(task)]
    connection1 = aiohttp_app_client.ws_connect('/graphql', protocols=[GRAPHQL_WS_PROTOCOL])
    connection2 = aiohttp_app_client.ws_connect('/graphql', protocols=[GRAPHQL_WS_PROTOCOL])
    async with connection1 as ws1, connection2 as ws2:
        start_payload = {'type': GQL_START, 'id': 'demo', 'payload': {'query': 'subscription { infinity(message: "Hi") }'}}
        if aio:
        ...
        if aio:
            assert len(get_result_handler_tasks()) == 0
        await ws1.send_json({'type': GQL_START, 'id': 'debug1', 'payload': {'query': 'subscription { debug { numActiveResultHandlers } }'}})
...
async def test_injects_connection_params(aiohttp_app_client: HttpClient):
    async with aiohttp_app_client.ws_connect('/graphql', protocols=[GRAPHQL_WS_PROTOCOL]) as ws:
        await ws.send_json({'type': GQL_CONNECTION_INIT, 'id': 'demo', 'payload': {'strawberry': 'rocks'}})
        await ws.send_json({'type': GQL_START, 'id': 'demo', 'payload': {'query': 'subscription { connectionParams }'}})
        response = await ws.receive_json()
        assert response['type'] == GQL_CONNECTION_ACK
        response = await ws.receive_json()
        assert response['type'] == GQL_DATA
...
async def test_rejects_connection_params(aiohttp_app_client: HttpClient):
    async with aiohttp_app_client.ws_connect('/graphql', protocols=[GRAPHQL_WS_PROTOCOL]) as ws:
        await ws.send_json({'type': GQL_CONNECTION_INIT, 'id': 'demo', 'payload': 'gonna fail'})
        response = await ws.receive_json()
