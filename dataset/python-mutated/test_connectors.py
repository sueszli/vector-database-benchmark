...
@pytest.mark.parametrize('command, command_args', [(connectors_test_command.test, []), (connectors_publish_command.publish, ['--spec-cache-gcs-credentials', 'test', '--spec-cache-bucket-name', 'test', '--metadata-service-gcs-credentials', 'test', '--metadata-service-bucket-name', 'test']), (connectors_build_command.build, [])])
@pytest.mark.anyio
async def test_commands_do_not_override_connector_selection(mocker, runner: CliRunner, click_context_obj: dict, command: Callable, command_args: list):
    """
