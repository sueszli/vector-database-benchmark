...
@pytest.mark.e2e
def test_should_not_be_able_to_login_with_invalid_credentials_core_1506(e2e_not_logged_api_client, e2e_staff_api_client, permission_manage_channels, permission_manage_settings):
    if False:
        for i in range(10):
            print('nop')
    ...
    login_data = raw_token_create(e2e_not_logged_api_client, user_email, invalid_password)
    error = login_data['data']['tokenCreate']['errors'][0]
    assert error['field'] == 'email'
    assert error['message'] == 'Please, enter valid credentials'
    assert error['code'] == 'INVALID_CREDENTIALS'
    invalid_email = 'invalid@email.com'
    login_data = raw_token_create(e2e_not_logged_api_client, invalid_email, user_password)
    error = login_data['data']['tokenCreate']['errors'][0]
    assert error['field'] == 'email'
    assert error['message'] == 'Please, enter valid credentials'
    assert error['code'] == 'INVALID_CREDENTIALS'