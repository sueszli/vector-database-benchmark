...
def test_infogram_breast_cancer_cv_fold_column():
    ...
    test = splits[1]
    n_fold = 5
    infogram_model_cv_valid = H2OInfogram(seed=12345, top_n_features=50, nfolds=n_fold, fold_assignment='modulo')
    ...
    infogram_model_cv_valid.plot(train=True, valid=True, xval=True, title='infogram from training/validation/cv holdout dataset 1', server=True)
