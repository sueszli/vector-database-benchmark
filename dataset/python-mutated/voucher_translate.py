...
class VoucherTranslate(BaseTranslateMutation):
    ...
    class Arguments:
        id = graphene.ID(required=True, description='Voucher ID or VoucherTranslatableContent ID.')
        language_code = graphene.Argument(LanguageCodeEnum, required=True, description='Translation language code.')
        input = NameTranslationInput(required=True, description='Fields required to update voucher translations.')
