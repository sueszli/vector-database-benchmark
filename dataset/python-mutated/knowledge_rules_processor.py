...
class KnowledgeRulesProcessor(TaskManager):
    ...
    async def process_batch(self) -> int:
        ...
        message = f'[Batch] Processed: {processed} titles. Added: {added} statements. Duration: {duration:.3f} seconds. Estimated time for full processing: {calculate_eta()}'
    ...
    async def process_torrent_title(self, infohash: Optional[bytes]=None, title: Optional[str]=None) -> int:
        ...
        if (tags := set(extract_only_valid_tags(title, rules=general_rules))):
            self.save_statements(subject_type=ResourceType.TORRENT, subject=infohash_str, predicate=ResourceType.TAG, objects=tags)
        if (content_items := set(extract_only_valid_tags(title, rules=content_items_rules))):
            self.save_statements(subject_type=ResourceType.TORRENT, subject=infohash_str, predicate=ResourceType.CONTENT_ITEM, objects=content_items)
