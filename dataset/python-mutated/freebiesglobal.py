...
class FreebiesglobalScraper(BaseScraper):
    ...
    async def run(self) -> List:
        ...
        logger.info(f'Page: {self.current_page} of {self.last_page} scraped from freebiesglobal.com')
    ...
    async def get_links(self) -> List:
        ...
        self.current_page += 1
        coupons_data = await http_get(f'{self.DOMAIN}/dealstore/udemy/page/{self.current_page}')
    ...
    def _get_last_page(soup: BeautifulSoup) -> int:
        ...
        return max([int(i.text) for i in soup.find('ul', class_='page-numbers').find_all('li') if i.text.isdigit()])