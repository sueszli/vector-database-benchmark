...
def upgrade():
    if False:
        for i in range(10):
            print('nop')
    op.add_column('projects', sa.Column('owners_require_2fa', sa.Boolean(), server_default=sa.text('false'), nullable=False))
    op.add_column('projects', sa.Column('pypi_mandates_2fa', sa.Boolean(), server_default=sa.text('false'), nullable=False))
