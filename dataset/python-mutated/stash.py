...
class StashManager:
    ...
    async def do_init(self) -> None:
        async with aiosqlite.connect(self.db) as db:
            await db.execute('CREATE TABLE IF NOT EXISTS results (domain text, resource text, type text, find_date date, source text)')
            await db.commit()
    ...
    async def store(self, domain, resource, res_type, source) -> None:
        ...
        try:
            async with aiosqlite.connect(self.db, timeout=30) as db:
                await db.execute('INSERT INTO results (domain,resource, type, find_date, source) VALUES (?,?,?,?,?)', (self.domain, self.resource, self.type, self.date, self.source))
                await db.commit()
    ...
    async def store_all(self, domain, all, res_type, source) -> None:
        ...
        async with aiosqlite.connect(self.db, timeout=30) as db:
            try:
                await db.executemany('INSERT INTO results (domain,resource, type, find_date, source) VALUES (?,?,?,?,?)', master_list)
                await db.commit()
            except Exception as e:
                print(e)
    ...
    async def getscanhistorydomain(self, domain):
        try:
            async with aiosqlite.connect(self.db, timeout=30) as conn:
                ...
                for date in dates:
                    cursor = await conn.execute('SELECT COUNT(*) from results WHERE domain=? AND type="host" AND find_date=?', (domain, date[0]))
                    counthost = await cursor.fetchone()
                    cursor = await conn.execute('SELECT COUNT(*) from results WHERE domain=? AND type="email" AND find_date=?', (domain, date[0]))
                    countemail = await cursor.fetchone()
                    cursor = await conn.execute('SELECT COUNT(*) from results WHERE domain=? AND type="ip" AND find_date=?', (domain, date[0]))
                    countip = await cursor.fetchone()
                    cursor = await conn.execute('SELECT COUNT(*) from results WHERE domain=? AND type="vhost" AND find_date=?', (domain, date[0]))
                    countvhost = await cursor.fetchone()
                    cursor = await conn.execute('SELECT COUNT(*) from results WHERE domain=? AND type="shodan" AND find_date=?', (domain, date[0]))
                    countshodan = await cursor.fetchone()
                    results = {'date': str(date[0]), 'hosts': str(counthost[0]), 'email': str(countemail[0]), 'ip': str(countip[0]), 'vhost': str(countvhost[0]), 'shodan': str(countshodan[0])}
