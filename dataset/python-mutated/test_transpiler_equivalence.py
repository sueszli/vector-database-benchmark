...
from qiskit.providers.fake_provider import FakeProvider, FakeOpenPulse2Q, FakeOpenPulse3Q, FakeYorktown, FakeTenerife, FakeOurense, FakeVigo, FakeMelbourne, FakeRueschlikon, FakeTokyo, FakePoughkeepsie, FakeAlmaden, FakeSingapore, FakeJohannesburg, FakeBoeblingen, FakeRochester, FakeBurlington, FakeCambridge, FakeCambridgeAlternativeBasis, FakeEssex, FakeLondon, FakeQasmSimulator, FakeArmonk, FakeRome, FakeSantiago, FakeSydney, FakeToronto, FakeValencia
from qiskit.test.base import dicts_almost_equal
from qiskit.circuit.library.standard_gates import *
default_profile = 'transpiler_equivalence'
settings.register_profile(default_profile, report_multiple_bugs=False, max_examples=200, deadline=None, suppress_health_check=[HealthCheck.filter_too_much])
settings.load_profile(os.getenv('HYPOTHESIS_PROFILE', default_profile))
BASE_INSTRUCTIONS = {(1, 0, 0): [HGate, IGate, SGate, SdgGate, TGate, TdgGate, XGate, YGate, ZGate, Reset], (2, 0, 0): [CXGate, CYGate, CZGate, SwapGate], (3, 0, 0): [CCXGate, CSwapGate], (1, 0, 1): [PhaseGate, RXGate, RYGate, RZGate], (1, 0, 3): [UGate], (2, 0, 1): [RZZGate, CPhaseGate], (2, 0, 4): [CUGate], (1, 1, 0): [Measure]}
...
layout_methods = _getenv_list('QISKIT_RANDOMIZED_TEST_LAYOUT_METHODS') or [None, 'trivial', 'dense', 'noise_adaptive', 'sabre']
routing_methods = _getenv_list('QISKIT_RANDOMIZED_TEST_ROUTING_METHODS') or [None, 'basic', 'stochastic', 'lookahead', 'sabre']
scheduling_methods = _getenv_list('QISKIT_RANDOMIZED_TEST_SCHEDULING_METHODS') or [None, 'alap', 'asap']
backend_needs_durations = _strtobool(os.getenv('QISKIT_RANDOMIZED_TEST_BACKEND_NEEDS_DURATIONS', 'False'))
