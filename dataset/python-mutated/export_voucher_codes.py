...
class ExportVoucherCodesInput(BaseInputObjectType):
    voucher_id = graphene.GlobalID(required=False, description='The ID of the voucher. If provided, exports all codes belonging to the voucher.')
    ids = NonNullList(graphene.ID, description='List of voucher code IDs to export.', required=False)
...
class ExportVoucherCodes(BaseExportMutation):
    ...
    class Arguments:
        input = ExportVoucherCodesInput(required=True, description='Fields required to export voucher codes.')

    class Meta:
        description = 'Export voucher codes to csv/xlsx file.' + ADDED_IN_318 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_DISCOUNTS
        permissions = (DiscountPermissions.MANAGE_DISCOUNTS,)
        error_type_class = ExportError
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.VOUCHER_CODE_EXPORT_COMPLETED, description='A notification for the exported file.')]
