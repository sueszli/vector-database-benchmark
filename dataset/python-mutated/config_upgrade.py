...
from picard.const import DEFAULT_FILE_NAMING_FORMAT, DEFAULT_SCRIPT_NAME
...
OLD_DEFAULT_FILE_NAMING_FORMAT_v1_3 = '$if2(%albumartist%,%artist%)/$if($ne(%albumartist%,),%album%/)$if($gt(%totaldiscs%,1),%discnumber%-,)$if($ne(%albumartist%,),$num(%tracknumber%,2) ,)$if(%_multiartist%,%artist% - ,)%title%'
...
def upgrade_to_v2_2_0_dev_3(config):
    ...
    if old_opt in _s:
        if _s[old_opt]:
            ...
            _s[new_opt] = '\n'.join(tags)
        _s.remove(old_opt)
OLD_DEFAULT_FILE_NAMING_FORMAT_v2_1 = '$if2(%albumartist%,%artist%)/$if($ne(%albumartist%,),%album%/,)$if($gt(%totaldiscs%,1),%discnumber%-,)$if($ne(%albumartist%,),$num(%tracknumber%,2) ,)$if(%_multiartist%,%artist% - ,)%title%'
