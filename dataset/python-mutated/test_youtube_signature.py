...
make_sig_test = t_factory('signature', signature, re.compile('.*(?:-|/player/)(?P<id>[a-zA-Z0-9_-]+)(?:/.+\\.js|(?:/watch_as3|/html5player)?\\.[a-z]+)$'))
for test_spec in _SIG_TESTS:
    make_sig_test(*test_spec)
make_nsig_test = t_factory('nsig', n_sig, re.compile('.+/player/(?P<id>[a-zA-Z0-9_-]+)/.+.js$'))
