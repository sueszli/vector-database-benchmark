...
class TestBatchPredictionJobWithDefaultProjectIdHook:
    ...
    def test_delete_batch_prediction_job(self, mock_client) -> None:
        if False:
            return 10
        self.hook.delete_batch_prediction_job(project_id=TEST_PROJECT_ID, region=TEST_REGION, batch_prediction_job=TEST_BATCH_PREDICTION_JOB)
        mock_client.assert_called_once_with(TEST_REGION)
        mock_client.return_value.delete_batch_prediction_job.assert_called_once_with(request=dict(name=mock_client.return_value.batch_prediction_job_path.return_value), metadata=(), retry=DEFAULT, timeout=None)
        mock_client.return_value.batch_prediction_job_path.assert_called_once_with(TEST_PROJECT_ID, TEST_REGION, TEST_BATCH_PREDICTION_JOB)
    ...
    def test_get_batch_prediction_job(self, mock_client) -> None:
        if False:
            while True:
                i = 10
        self.hook.get_batch_prediction_job(project_id=TEST_PROJECT_ID, region=TEST_REGION, batch_prediction_job=TEST_BATCH_PREDICTION_JOB)
        mock_client.assert_called_once_with(TEST_REGION)
        mock_client.return_value.get_batch_prediction_job.assert_called_once_with(request=dict(name=mock_client.return_value.batch_prediction_job_path.return_value), metadata=(), retry=DEFAULT, timeout=None)
        mock_client.return_value.batch_prediction_job_path.assert_called_once_with(TEST_PROJECT_ID, TEST_REGION, TEST_BATCH_PREDICTION_JOB)
    ...
    def test_list_batch_prediction_jobs(self, mock_client) -> None:
        if False:
            i = 10
            return i + 15
        self.hook.list_batch_prediction_jobs(project_id=TEST_PROJECT_ID, region=TEST_REGION)
        mock_client.assert_called_once_with(TEST_REGION)
        mock_client.return_value.list_batch_prediction_jobs.assert_called_once_with(request=dict(parent=mock_client.return_value.common_location_path.return_value, filter=None, page_size=None, page_token=None, read_mask=None), metadata=(), retry=DEFAULT, timeout=None)
