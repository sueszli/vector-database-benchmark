...
@pluginmatcher(re.compile('https?://(?:www\\.)?yupptv\\.com'))
@pluginargument('boxid', requires=['yuppflixtoken'], sensitive=True, metavar='BOXID', help="The yupptv.com boxid that's used in the BoxId cookie.")
@pluginargument('yuppflixtoken', sensitive=True, metavar='YUPPFLIXTOKEN', help="The yupptv.com yuppflixtoken that's used in the YuppflixToken cookie.")
@pluginargument('purge-credentials', action='store_true', help='Purge cached YuppTV credentials to initiate a new session and reauthenticate.')
class YuppTV(Plugin):
    _m3u8_re = re.compile('[\'"](http.+\\.m3u8.*?)[\'"]')
