...
class Lenia:
    ...
    def random_world_and_params(self, is_reseed=False, is_fill=False):
        ...
        p['m'] = round(np.random.rand() * (0.5 - 0.1) + 0.1, 2)
        ...
        if is_reimage:
            ...
            if show_arr is not None:
                ...
                if self.show_what == 0:
                    ...
                    self.draw_world(self.automaton.potential, 0, 2 * self.world.params['m'], is_shift=True, is_shift_zero=True, markers=['arrow', 'scale', 'grid', 'colormap', 'params'])
                    ...
                    self.draw_world(self.automaton.kernel, 0, 1, markers=['scale', 'fixgrid', 'colormap', 'params'])
                elif self.show_what == 4:
                    self.draw_world(self.automaton.fftshift(np.log(np.abs(self.automaton.world_FFT))), 0, 5, markers=['colormap', 'params'])
                elif self.show_what == 5:
                    self.draw_world(self.automaton.fftshift(np.log(np.abs(self.automaton.potential_FFT))), -20, 5, markers=['colormap', 'params'])
                elif self.show_what == 6:
                    self.draw_world(grad, 0, 1, is_shift=True, markers=['arrow', 'scale', 'grid', 'colormap', 'params'])
                elif self.show_what == 7:
                    self.draw_world(self.automaton.change, -change_range, change_range, is_shift=True, markers=['arrow', 'scale', 'grid', 'colormap', 'params'])
    ...
    def draw_params(self, buffer, markers=[]):
        ...
        if self.is_draw_params and 'params' in markers and (self.markers_mode in [0, 1, 4, 5]) and (self.polar_mode in [0, 1]):
            ...
            for (d1, d2, d3) in [(d1, d2, d3) for d1 in r for d2 in r for d3 in r if d1 == s or d2 == s or d3 == s]:
                ...
                if len(b) > 3:
                    c = 253
                elif b[0] == Fraction(d1, s) and b[1] == Fraction(d2, s) and (b[2] == Fraction(d3, s)):
                    c = 255
                elif (d1, d2, d3).count(s) >= 2:
                    c = 254
                else:
                    c = 253
                buffer[y + my, x * 2 + mx:x * 2 + 2 + mx] = c
            (mx, my) = (2, SIZEY - 53)
            for pm in range(51):
    ...
    def toggle_auto_rotate_from_sampling(self):
        ...
        if self.auto_rotate_mode not in [3]:
            ...
            self.samp_freq = 1
            self.is_auto_center = False
    SHIFT_KEYS = {'asciitilde': 'quoteleft', 'exclam': '1', 'at': '2', 'numbersign': '3', 'dollar': '4', 'percent': '5', 'asciicircum': '6', 'ampersand': '7', 'asterisk': '8', 'parenleft': '9', 'parenright': '0', 'underscore': '-', 'plus': 'equal', 'braceleft': 'bracketleft', 'braceright': 'bracketright', 'bar': 'backslash', 'colon': 'semicolon', 'quotedbl': 'quoteright', 'less': 'comma', 'greater': 'period', 'question': 'slash'}
    ...
    if DIM == 2:
        ANIMAL_KEY_LIST = {'1': 'O2u', '2': 'OG2g', '3': 'OV2u', '4': 'P4ap', '5': 'PS3ap', '6': '2S2:2', '7': '1P4oal', '8': '2PG1v', '9': '3H3t', '0': '~gldr', 's+1': '3GH2n', 's+2': '3GG2g', 's+3': 'K5s', 's+4': 'K7a', 's+5': 'K9a', 's+6': '3R5t:2', 's+7': '3R6s:4', 's+8': '2D10rb', 's+9': '4F14x9xR5r', 's+0': '~ggun', 'c+1': '4Q5m:2', 'c+2': '3ECv', 'c+3': '2L4m:2', 'c+4': '3R6n', 'c+5': '4D10v:2', 'c+6': '', 'c+7': '', 'c+8': '', 'c+9': '', 'c+0': 'bbug', 'a+1': '2F5l', 'a+2': '3L6v', 'a+3': '2L3i', 'a+4': '3F8t', 'a+5': '4R5n', 'a+6': '3R5r:2', 'a+7': '3R5t:2', 'a+8': '2D6s', 'a+9': '3R6i', 'a+0': '4D10v:2', 's+a+1': 'P4cl', 's+a+2': 'P4cs', 's+a+3': 'P4cp', 's+a+4': 'P4sp', 's+a+5': 'P4cf', 's+a+6': '2PG1v', 's+a+7': 'OG2g', 's+a+8': 'OV2u', 's+a+9': 'OG2r', 's+a+0': '3GG2g'}
    elif DIM == 3:
    ...
    def process_key(self, k):
        ...
        if k in ['escape']:
            ...
            self.show_what = (self.show_what + inc_or_dec) % 4
        elif k in ['c+tab', 's+c+tab']:
            self.show_what = (self.show_what + inc_or_dec) % 8
            ...
            self.world.params['kn'] = (self.world.params['kn'] + inc_or_dec - 1) % len(self.automaton.kernel_core) + 1
            self.automaton.calc_kernel()
            self.info_type = 'kn'
        elif k in ['c+u', 's+c+u']:
            self.world.params['gn'] = (self.world.params['gn'] + inc_or_dec - 1) % len(self.automaton.growth_func) + 1
            self.info_type = 'gn'
        elif k in ['c+i']:
            ...
            self.load_animal_id(self.world, self.animal_id)
            self.world.params['kn'] = self.world.params['gn'] = 2
            self.automaton.calc_kernel()
            self.info_type = 'animal'
        elif k in ['c+b']:
    ...
    def get_value_text(self, name):
        ...
        if name == 'animal':
            ...
            return ['Exponential', 'Polynomial', 'Step', 'Staircase'][self.world.params.get('kn') - 1]
        elif name == 'gn':
            return ['Exponential', 'Polynomial', 'Step'][self.world.params.get('gn') - 1]
        elif name == 'colormap_id':
            return ['Vivid blue/red (Jet)', "Paul Tol's Rainbow", 'Vivid green/purple', 'Vivid red/green', 'Pale blue/red', 'Pale green/purple', 'Pale yellow/green', 'White/black', 'Black/white'][self.colormap_id]
        elif name == 'show_what':
    ...
    def create_menu(self):
        ...
        self.menu.add_cascade(label='Edit', menu=self.create_submenu(self.menu, ['|Clear|Backspace', '|Random|N', '|*Random (last seed)|s+N', '|Random cells & params|M', None, '|Flip vertically|Equal|=', '|Flip horizontally|s+Equal|+', '|Mirror horizontally|c+Equal|c+=', '|Mirror flip|s+c+Equal|c++', None, '|Copy|c+C', '|*Copy as CSV|c+X', '|Paste|c+V', None, '^is_auto_load|*Auto put mode (place/paste/random)|c+Z', '^is_layered|*Layer mode|s+c+X']))
