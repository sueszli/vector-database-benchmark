...
from ray.dashboard.modules.event.event_utils import parse_event_strings, monitor_events
...
class EventHead(dashboard_utils.DashboardHeadModule, event_pb2_grpc.ReportEventServiceServicer):
    ...
    async def _periodic_state_print(self):
        ...
        return {'total_events_received': self.total_events_received, 'Total_requests_received': self.total_report_events_count, 'total_uptime': elapsed}
    ...
    async def get_event(self, req) -> aiohttp.web.Response:
        ...
        if job_id is None:
            all_events = {job_id: list(job_events.values()) for (job_id, job_events) in DataSource.events.items()}
            return dashboard_optional_utils.rest_response(success=True, message='All events fetched.', events=all_events)
        job_events = DataSource.events.get(job_id, {})
        return dashboard_optional_utils.rest_response(success=True, message='Job events fetched.', job_id=job_id, events=list(job_events.values()))

    async def run(self, server):
        event_pb2_grpc.add_ReportEventServiceServicer_to_server(self, server)
        self._monitor = monitor_events(self._event_dir, lambda data: self._update_events(parse_event_strings(data)), self.monitor_thread_pool_executor)
