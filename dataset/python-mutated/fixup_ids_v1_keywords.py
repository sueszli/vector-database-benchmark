...
class idsCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_endpoint': ('parent', 'endpoint_id', 'endpoint', 'request_id'), 'delete_endpoint': ('name', 'request_id'), 'get_endpoint': ('name',), 'list_endpoints': ('parent', 'page_size', 'page_token', 'filter', 'order_by')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=idsCallTransformer()):
    if False:
        while True:
            i = 10
