...
class WeightOnlyLinearTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (self.weight, self.weight_scale) = Q.weight_quantize(self.weight, algo='weight_only_int8' if self.weight_dtype == 'int8' else 'weight_only_int4')
    ...
    def get_weight_only_linear_out(self):
        if False:
            while True:
                i = 10
        out = Q.weight_only_linear(self.x, self.weight, bias=self.bias, weight_scale=self.weight_scale, weight_dtype=self.weight_dtype)
        return out.numpy()
    ...
    def get_weight_only_linear_out_static(self):
        ...
        with base.program_guard(main, start):
            ...
            out = Q.weight_only_linear(x, weight, bias, weight_scale, self.weight_dtype)
...
class WeightOnlyLinearBackwardAndWeightDequantizeTestCase(unittest.TestCase):
    ...
    def test_weightonly_linear_backward(self):
        ...
        (quant_weight, quant_scale) = Q.weight_quantize(x=weight, algo='weight_only_int8')
        dequant_weight = Q.weight_dequantize(quant_weight, quant_scale)
        np.testing.assert_allclose(weight, dequant_weight, rtol=0.01, atol=0.01)
        quant_out = Q.weight_only_linear(x=quant_x, weight=quant_weight, weight_scale=quant_scale, weight_dtype='int8')
        out = paddle.matmul(x=x, y=weight)
        np.testing.assert_allclose(quant_out, out, rtol=0.001, atol=0.001)
        quant_out.backward()
        out.backward()
        np.testing.assert_allclose(quant_x.grad, x.grad, rtol=0.001, atol=0.001)
