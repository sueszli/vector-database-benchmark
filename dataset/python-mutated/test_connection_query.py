...
async def check(args, letters, has_previous_page=False, has_next_page=False):
    ...
    expected_page_info = {'hasPreviousPage': has_previous_page, 'hasNextPage': has_next_page, 'endCursor': expected_edges[-1]['cursor'] if expected_edges else None, 'startCursor': expected_edges[0]['cursor'] if expected_edges else None}
    assert not result.errors
    assert result.data == {'letters': {'edges': expected_edges, 'pageInfo': expected_page_info}}
...
async def test_respects_first_and_after_and_before_too_few():
    await check(f'''first: 2, after: "{cursor_for('A')}", before: "{cursor_for('E')}"''', 'BC', has_next_page=True)

@mark.asyncio
async def test_respects_first_and_after_and_before_too_many():
    await check(f'''first: 4, after: "{cursor_for('A')}", before: "{cursor_for('E')}"''', 'BCD')

@mark.asyncio
async def test_respects_first_and_after_and_before_exactly_right():
    await check(f'''first: 3, after: "{cursor_for('A')}", before: "{cursor_for('E')}"''', 'BCD')

@mark.asyncio
async def test_respects_last_and_after_and_before_too_few():
    await check(f'''last: 2, after: "{cursor_for('A')}", before: "{cursor_for('E')}"''', 'CD', has_previous_page=True)

@mark.asyncio
async def test_respects_last_and_after_and_before_too_many():
    await check(f'''last: 4, after: "{cursor_for('A')}", before: "{cursor_for('E')}"''', 'BCD')

@mark.asyncio
async def test_respects_last_and_after_and_before_exactly_right():
    await check(f'''last: 3, after: "{cursor_for('A')}", before: "{cursor_for('E')}"''', 'BCD')
...
async def test_returns_all_elements_if_cursors_are_on_the_outside():
    await check(f'''before: "{base64('arrayconnection:%s' % 6)}" after: "{base64('arrayconnection:%s' % -1)}"''', 'ABCDE')

@mark.asyncio
async def test_returns_no_elements_if_cursors_cross():
    await check(f'''before: "{base64('arrayconnection:%s' % 2)}" after: "{base64('arrayconnection:%s' % 4)}"''', '')

@mark.asyncio
async def test_connection_type_nodes():
    result = await schema.execute_async('\n    {\n        connectionLetters {\n            edges {\n                node {\n                    id\n                    letter\n                }\n                cursor\n            }\n            pageInfo {\n                hasPreviousPage\n                hasNextPage\n            }\n        }\n    }\n    ')
    assert not result.errors
    assert result.data == {'connectionLetters': {'edges': [{'node': {'id': 'TGV0dGVyOjA=', 'letter': 'A'}, 'cursor': 'a-cursor'}], 'pageInfo': {'hasPreviousPage': False, 'hasNextPage': True}}}
