...
class TestSimpleNetWithRecomputeForSemiAutoParallel(TestSimpleNetForSemiAutoParallel):
    ...
    def init_single_card_net_result(self):
        ...
        (self.base_loss, self.base_parameters) = self.run_dynamic_recompute(DemoNet('recompute_demo', is_recompute=True))

    def test_dp_demo_net(self):
        if False:
            print('Hello World!')
        self.set_random_seed(self._seed)
        (self.dp_loss, self.dp_parameters) = self.run_dynamic_recompute(DemoNet('recompute_dp_demo', is_recompute=True), shard_input=True)
    ...
    def test_mp_demo_net(self):
        if False:
            while True:
                i = 10
        self.set_random_seed(self._seed)
        mp_layer = dist.shard_layer(DemoNet('recompute_mp_demo', is_recompute=True), self._mesh, self.shard_fn)
        (self.mp_loss, self.mp_parameters) = self.run_dynamic_recompute(mp_layer)
