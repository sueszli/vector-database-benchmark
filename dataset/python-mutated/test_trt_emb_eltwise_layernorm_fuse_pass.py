...
class TestEmbeddingEltwiseLayerNormFusePass(PassAutoScanTest):
    ...
    def sample_program_config(self, draw):
        ...
        def generate_weight1(attrs):
            if False:
                for i in range(10):
                    print('nop')
            return np.random.uniform(0.005, 0.005, attrs['weight_size']).astype(np.float32)

        def generate_weight2(attrs):
            if False:
                i = 10
                return i + 15
            return np.random.uniform(1, 1, attrs[3]['weight_size'][1]).astype(np.float32)
    ...
    def sample_predictor_configs(self, program_config):
        ...
        config.enable_tensorrt_engine(max_batch_size=4, workspace_size=1 << 30, min_subgraph_size=0, precision_mode=paddle_infer.PrecisionType.Half, use_static=False, use_calib_mode=False)
        if program_config.ops[0].type == 'lookup_table':
            config.set_trt_dynamic_shape_info({'input_data1': [1, 128, 1], 'input_data2': [1, 128, 1], 'input_data3': [1, 128, 1]}, {'input_data1': [4, 128, 1], 'input_data2': [4, 128, 1], 'input_data3': [4, 128, 1]}, {'input_data1': [2, 128, 1], 'input_data2': [2, 128, 1], 'input_data3': [2, 128, 1]})
        else:
            config.set_trt_dynamic_shape_info({'input_data1': [1, 128], 'input_data2': [1, 128], 'input_data3': [1, 128]}, {'input_data1': [4, 128], 'input_data2': [4, 128], 'input_data3': [4, 128]}, {'input_data1': [2, 128], 'input_data2': [2, 128], 'input_data3': [2, 128]})
    ...
    def test(self):
        if False:
            return 10
        self.run_and_statis(quant=False, max_examples=50, passes=['trt_embedding_eltwise_layernorm_fuse_pass'], min_success_num=0)
...
class TestEmbeddingEltwiseLayerNormFusePassNoBroadcast(PassAutoScanTest):
    ...
    def sample_predictor_configs(self, program_config):
        ...
        config.enable_tensorrt_engine(max_batch_size=4, workspace_size=1 << 30, min_subgraph_size=0, precision_mode=paddle_infer.PrecisionType.Half, use_static=False, use_calib_mode=False)
        if program_config.ops[0].type == 'lookup_table':
            config.set_trt_dynamic_shape_info({'embedding_output1': [1, 128, 384], 'embedding_output2': [1, 128, 384], 'embedding_output3': [1, 1, 384]}, {'embedding_output1': [4, 128, 384], 'embedding_output2': [4, 128, 384], 'embedding_output3': [4, 1, 384]}, {'embedding_output1': [2, 128, 384], 'embedding_output2': [2, 128, 384], 'embedding_output3': [2, 1, 384]})
