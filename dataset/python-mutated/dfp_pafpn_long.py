...
from modelscope.models.cv.stream_yolo.models.network_blocks import BaseConv, DWConv

class DFPPAFPNLONG(nn.Module):

    def __init__(self, depth=1.0, width=1.0, in_features=('dark3', 'dark4', 'dark5'), in_channels=[256, 512, 1024], depthwise=False, act='silu', frame_num=2, with_short_cut=True, merge_form='pure_concat', out_channels=[((64, 128, 256), 1)]):
        if False:
            print('Hello World!')
        super().__init__()
    ...
    def off_forward(self, input, backbone_neck):
        ...
        if self.with_short_cut:
            if self.merge_form == 'pure_concat':
                ...
                raise Exception('merge_form must be in ["pure_concat", "add"].')
