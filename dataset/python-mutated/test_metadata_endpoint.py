...
async def torrent_checker(mock_dlmgr, metadata_store):
    ...
    notifier = MagicMock()
    torrent_checker = TorrentChecker(config=config, download_manager=mock_dlmgr, tracker_manager=tracker_manager, metadata_store=metadata_store, notifier=notifier, socks_listen_ports=[2000, 3000])
...
async def test_update_multiple_metadata_entries(metadata_store, add_fake_torrents_channels, rest_api):
    ...
    await do_request(rest_api, 'metadata', post_data=[{'public_key': hexlify(b'1' * 64), 'id': 111}], expected_code=404, request_type='PATCH')
    with db_session:
    ...
    NEW_NAME1 = 'updated1'
    NEW_NAME2 = 'updated2'
    patch_data = [{'public_key': hexlify(md1.public_key), 'id': md1.id_, 'title': NEW_NAME1}, {'public_key': hexlify(md2.public_key), 'id': md2.id_, 'title': NEW_NAME2, 'subscribed': 1}]
...
async def test_delete_multiple_metadata_entries(rest_api, metadata_store):
    ...
    patch_data = [{'public_key': hexlify(md1.public_key), 'id': md1.id_}, {'public_key': hexlify(md2.public_key), 'id': md2.id_}]
...
async def test_update_entry_status_and_name(metadata_store, rest_api):
    ...
    await do_request(rest_api, 'metadata/%s/%i' % (hexlify(chan.public_key), chan.id_), request_type='PATCH', post_data=patch_params, expected_code=400)
...
async def test_update_entry(rest_api, metadata_store):
    ...
    result = await do_request(rest_api, 'metadata/%s/%i' % (hexlify(chan.public_key), chan.id_), request_type='PATCH', post_data=patch_params, expected_code=200)
...
async def test_get_entry(rest_api, metadata_store):
    ...
    """
    for (md_type, kwargs) in ((metadata_store.TorrentMetadata, {'title': 'bla', 'infohash': random_infohash(), 'tracker_info': 'http://sometracker.local/announce'}), (metadata_store.ChannelDescription, {'text': json.dumps({'description_text': '*{{}bla <\\> [)]// /ee2323㋛㋛㋛  ', 'channel_thumbnail': 'ffffff.jpg'})})):
        with db_session:
            md = md_type(**kwargs)
            md.status = COMMITTED
        await do_request(rest_api, 'metadata/%s/%i' % (hexlify(md.public_key), md.id_), expected_json=md.to_simple_dict())
