...
if __name__ == '__main__':
    ...
    config = ppo.PPOConfig().environment(ActionMaskEnv, env_config={'action_space': Discrete(100), 'observation_space': Box(-1.0, 1.0, (5,))}).experimental(_enable_new_api_stack=True, _disable_preprocessor_api=True).framework(args.framework).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0'))).rl_module(rl_module_spec=rlm_spec)
    algo = config.build()
