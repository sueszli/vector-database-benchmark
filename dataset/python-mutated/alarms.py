...
class SonosAlarms(SonosHouseholdCoordinator):
    ...
    async def async_update_entities(self, soco: SoCo, update_id: int | None=None) -> None:
        """Create and update alarms entities, return success."""
        updated = await self.hass.async_add_executor_job(self.update_cache, soco, update_id)
        if not updated:
            return
        for (alarm_id, alarm) in self.alarms.alarms.items():
            ...
            if speaker:
                async_dispatcher_send(self.hass, SONOS_CREATE_ALARM, speaker, [alarm_id])
        async_dispatcher_send(self.hass, f'{SONOS_ALARMS_UPDATED}-{self.household_id}')

    async def async_process_event(self, event: SonosEvent, speaker: SonosSpeaker) -> None:
