...
class InlineQueryResultAnimation(InlineQueryResult):
    ...
    async def write(self, client: 'pyrogram.Client'):
        animation = raw.types.InputWebDocument(url=self.animation_url, size=0, mime_type='image/gif', attributes=[raw.types.DocumentAttributeVideo(w=self.animation_width, h=self.animation_height, duration=self.animation_duration)])
        if self.thumb_url is None:
            thumb = animation
        else:
            thumb = raw.types.InputWebDocument(url=self.thumb_url, size=0, mime_type=self.thumb_mime_type, attributes=[])
        (message, entities) = (await utils.parse_text_entities(client, self.caption, self.parse_mode, self.caption_entities)).values()
        return raw.types.InputBotInlineResult(id=self.id, type=self.type, title=self.title, thumb=thumb, content=animation, send_message=self.input_message_content.write(client, self.reply_markup) if self.input_message_content else raw.types.InputBotInlineMessageMediaAuto(reply_markup=await self.reply_markup.write(client) if self.reply_markup else None, message=message, entities=entities))