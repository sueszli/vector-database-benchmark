...
from ...operations._access_review_instances_assigned_for_my_approval_operations import build_get_by_id_request, build_list_request
T = TypeVar('T')
...
class AccessReviewInstancesAssignedForMyApprovalOperations:
    ...
    @distributed_trace
    def list(self, schedule_definition_id: str, filter: Optional[str]=None, **kwargs: Any) -> AsyncIterable['_models.AccessReviewInstance']:
        if False:
            i = 10
            return i + 15
        ...
        def prepare_request(next_link=None):
            if False:
                print('Hello World!')
            if not next_link:
                request = build_list_request(schedule_definition_id=schedule_definition_id, filter=filter, api_version=api_version, template_url=self.list.metadata['url'], headers=_headers, params=_params)
    ...
    async def get_by_id(self, schedule_definition_id: str, id: str, **kwargs: Any) -> _models.AccessReviewInstance:
        ...
        request = build_get_by_id_request(schedule_definition_id=schedule_definition_id, id=id, api_version=api_version, template_url=self.get_by_id.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=_stream, **kwargs)
