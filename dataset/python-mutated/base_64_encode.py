...
class Base64Encode(StatesFunction):
    ...
    def __init__(self, arg_list: FunctionArgumentList):
        if False:
            print('Hello World!')
        super().__init__(states_name=StatesFunctionName(function_type=StatesFunctionNameType.Base64Encode), arg_list=arg_list)
        if arg_list.size != 1:
    ...
    def _eval_body(self, env: Environment) -> None:
        ...
        if len(string) > self.MAX_INPUT_CHAR_LEN:
            raise ValueError(f"Maximum input string for function type '{type(self)}' is '{self.MAX_INPUT_CHAR_LEN}', but got '{len(string)}'.")
