...
def test_gam_model_predict():
    ...
    h2o_model_no_early_stop = H2OGeneralizedAdditiveEstimator(family='binomial', gam_columns=['C11'], scale=[0.0001], score_each_iteration=True, bs=[2])
    ...
    for ind in range(len(early_stop_metrics)):
        ...
        h2o_model = H2OGeneralizedAdditiveEstimator(family='binomial', gam_columns=['C11'], scale=[0.0001], stopping_rounds=max_stopping_rounds, score_each_iteration=True, stopping_metric=early_stop_metrics[ind], stopping_tolerance=max_tolerance, bs=[2])
        h2o_model.train(x=list(range(0, 20)), y='C21', training_frame=train, validation_frame=test)
        metric_list1 = pyunit_utils.extract_field_from_twoDimTable(h2o_model._model_json['output']['glm_scoring_history'].col_header, h2o_model._model_json['output']['glm_scoring_history'].cell_values, early_stop_valid_metrics[ind])
    ...
    try:
        h2o_model = H2OGeneralizedAdditiveEstimator(family='binomial', gam_columns=['C11'], scale=[0.0001], stopping_rounds=max_stopping_rounds, score_each_iteration=True, stopping_metric=early_stop_metrics[ind], bs=[2], stopping_tolerance=max_tolerance, lambda_search=True, nlambdas=3)
