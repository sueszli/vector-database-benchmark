...
class Test_iam_user_mfa_enabled_console_access_test:
    ...
    def test_root_user_not_password_console_enabled(self):
        ...
        current_audit_info = self.set_mocked_audit_info()
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_user_mfa_enabled_console_access.iam_user_mfa_enabled_console_access.iam_client', new=IAM(current_audit_info)) as service_client:
            from prowler.providers.aws.services.iam.iam_user_mfa_enabled_console_access.iam_user_mfa_enabled_console_access import iam_user_mfa_enabled_console_access
            service_client.credential_report[0]['password_enabled'] = 'not_supported'
            check = iam_user_mfa_enabled_console_access()
            result = check.execute()
            assert result[0].status == 'PASS'
            assert result[0].status_extended == f'User {user} does not have Console Password enabled.'
    ...
    def test_user_password_console_and_mfa_enabled(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_user_mfa_enabled_console_access.iam_user_mfa_enabled_console_access.iam_client', new=IAM(current_audit_info)) as service_client:
            ...
            result = check.execute()
            assert result[0].status == 'PASS'
            assert result[0].status_extended == f'User {user} has Console Password enabled and MFA enabled.'
            assert result[0].resource_id == user
            assert result[0].resource_arn == arn
    ...
    def test_user_password_console_enabled_and_mfa_not_enabled(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_user_mfa_enabled_console_access.iam_user_mfa_enabled_console_access.iam_client', new=IAM(current_audit_info)) as service_client:
            ...
            assert result[0].status_extended == f'User {user} has Console Password enabled but MFA disabled.'
            assert result[0].resource_id == user
            assert result[0].resource_arn == arn