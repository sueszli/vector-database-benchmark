...
from mage_ai.api.utils import has_at_least_admin_role, has_at_least_editor_role, has_at_least_editor_role_and_notebook_edit_access, has_at_least_editor_role_and_pipeline_edit_access, has_at_least_viewer_role, is_owner
...
from mage_ai.settings import DISABLE_NOTEBOOK_EDIT_ACCESS, REQUIRE_USER_AUTHENTICATION, REQUIRE_USER_PERMISSIONS
...
class BasePolicy(UserPermissionMixIn, ResultSetMixIn):
    ...
    async def authorize_action(self, action):
        if not REQUIRE_USER_AUTHENTICATION and (not DISABLE_NOTEBOOK_EDIT_ACCESS) and (action in [OperationType.CREATE, OperationType.DELETE, OperationType.UPDATE]):
            return True
        ...
        if config:
            ...
            if conditions and len(conditions) >= 1:
                ...
                if config_override:
                    ...
                    if override_rules:
                        override_permission_conditions = [rule.get('override_permission_condition') for rule in override_rules if rule.get('override_permission_condition')]
                await self.__validate_condition(action, conditions, operation=action, override_permission_conditions=override_permission_conditions)
        else:
            error = ApiError.UNAUTHORIZED_ACCESS
            error.update({'message': 'The {} action is disabled for {}.'.format(action, self.resource_name())})
    ...
    async def authorize_attribute(self, read_or_write, attrb, **kwargs):
        ...
        if not DISABLE_NOTEBOOK_EDIT_ACCESS and REQUIRE_USER_PERMISSIONS and self.is_owner():
            return True
        api_operation_action = self.options.get('api_operation_action', kwargs.get('api_operation_action', OperationType.ALL))
        ...
        if orig_config_override:
            ...
            override_rules = config_override_scope.get(api_operation_action) or config_override_scope.get(OperationType.ALL)
            if override_rules:
                override_permission_conditions = [rule.get('override_permission_condition') for rule in override_rules if rule.get('override_permission_condition')]
        if rules is None:
            error = ApiError.UNAUTHORIZED_ACCESS
            error.update({'message': '{} of {} on {} is disabled for {}.'.format(read_or_write, attrb, api_operation_action, self.__class__.resource_name())})
            raise ApiError(error)
        conditions = [rule.get('condition') for rule in rules if rule.get('condition')]
        if conditions and len(conditions) >= 1:
            await self.__validate_condition(attrb, conditions, attribute_operation=attribute_operation, operation=api_operation_action, override_permission_conditions=override_permission_conditions, **kwargs)
    ...
    async def authorize_query(self, query, **kwargs):
        if not DISABLE_NOTEBOOK_EDIT_ACCESS and (not REQUIRE_USER_AUTHENTICATION or self.is_owner()):
            return True
        if not DISABLE_NOTEBOOK_EDIT_ACCESS and REQUIRE_USER_PERMISSIONS and self.is_owner():
            return True
        ...
        for (key, value) in query_filtered.items():
            if key == settings.QUERY_API_KEY:
                continue
            error_message = f'Query parameter {key} of value {value} is not permitted on {api_operation_action} operation for {self.__class__.resource_name()}.'
            orig_config = self.__class__.query_rule(key) or self.__class__.query_rule(AttributeType.ALL)
            rules = None
            if orig_config:
                ...
                if rules is None:
                    rules = config_scope.get(OperationType.ALL)
            orig_config_override = self.__class__.override_permission_query_rule(key) or self.__class__.override_permission_query_rule(AttributeType.ALL)
            override_permission_conditions = None
            if orig_config_override:
            ...
            if rules is None:
                error = ApiError.UNAUTHORIZED_ACCESS
                error.update({'message': error_message})
                raise ApiError(error)
            conditions = [rule.get('condition') for rule in rules if rule.get('condition')]
            if conditions and len(conditions) >= 1:
                await self.__validate_condition(key, conditions, message=error_message, operation=api_operation_action, override_permission_conditions=override_permission_conditions)
    ...
    async def __validate_condition(self, action, conditions: List[Callable[['BasePolicy'], None]], attribute_operation: AttributeOperationType=None, operation: OperationType=None, override_permission_conditions: List[Callable[['BasePolicy'], None]]=None, **kwargs):
        ...
        if not validation:
            ...
            error.update({'message': kwargs.get('message', f'{message}, failed condition.')})
            increment('api_error.unauthorized_access', tags={'action': action, 'attribute_operation': attribute_operation, 'policy_class_name': self.__class__.__name__, 'id': self.resource.id if hasattr(self.resource, 'id') else None, 'operation': operation})
            if settings.DEBUG:
                error['debug'] = {'action': action, 'attribute_operation': attribute_operation, 'operation': operation}
            raise ApiError(error)
    ...
    async def __validate_scopes(self, val, scopes):
        ...
        if OauthScope.CLIENT_ALL in scopes:
            ...
            error.update({'message': 'Private access for {} only, invalid scope.'.format(val)})
            if settings.DEBUG:
                error['debug'] = {'class': self.__class__.__name__, 'scopes': scopes}
            raise ApiError(error)
        elif self.current_user and OauthScope.CLIENT_PRIVATE not in scopes:
            error.update({'message': 'Public access for {} only, invalid scope.'.format(val)})
            if settings.DEBUG:
                error['debug'] = {'scopes': scopes}
            raise ApiError(error)
