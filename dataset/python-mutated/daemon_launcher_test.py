...
from ...client import configuration as configuration_module, daemon_socket, frontend_configuration, identifiers
from ...client.commands import initialization
from ...client.tests import setup
from ..daemon_launcher import _start_server, PyreServerStarterBase, RegistrationFailure, RegitrationSuccess, StartedServerInfo, StartFailure
...
class DaemonLauncherTest(unittest.TestCase):
    ...
    async def test_successful_start(self) -> None:
        ...
        client_id = 'test'
        result = await _start_server(configuration, SuccessfulPyreServerStarter(client_id))
        assert isinstance(result, StartedServerInfo)
        self.assertEqual(result, StartedServerInfo(daemon_socket.get_socket_path(configuration.get_project_identifier(), flavor=identifiers.PyreFlavor.CODE_NAVIGATION), client_id=client_id))

    @setup.async_test
    async def test_failed_start(self) -> None:
        result = await _start_server(FakeFrontendConfiguration(), FailedPyreServerStarter())
    ...
    async def test_already_started(self) -> None:
        configuration = FakeFrontendConfiguration()
        client_id = 'test'
        result = await _start_server(configuration, AlreadyStartedServerStarter(client_id))
        self.assertEqual(result, StartedServerInfo(daemon_socket.get_socket_path(configuration.get_project_identifier(), flavor=identifiers.PyreFlavor.CODE_NAVIGATION), client_id=client_id))

    @setup.async_test
    async def test_failed_registration(self) -> None:
        error_message = 'test message'
        result = await _start_server(FakeFrontendConfiguration(), AlwaysFailsRegistrationPyreServerStarter(error_message))
