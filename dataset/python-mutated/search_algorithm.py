...
import numpy as np
distance_metric_map = {'l2': lambda a, b: np.linalg.norm(a - b, axis=1, ord=2), 'l1': lambda a, b: np.linalg.norm(a - b, axis=1, ord=1), 'max': lambda a, b: np.linalg.norm(a - b, axis=1, ord=np.inf), 'cos': lambda a, b: np.dot(a, b.T) / (np.linalg.norm(a) * np.linalg.norm(b, axis=1))}
