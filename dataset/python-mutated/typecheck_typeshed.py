...
parser.add_argument('dir', choices=DIRECTORIES_TO_TEST + (EMPTY,), nargs='*', action='extend', help=f'Test only these top-level typeshed directories (defaults to {DIRECTORIES_TO_TEST!r})')
parser.add_argument('--platform', choices=SUPPORTED_PLATFORMS, nargs='*', action='extend', help='Run mypy for certain OS platforms (defaults to sys.platform)')
parser.add_argument('-p', '--python-version', choices=SUPPORTED_VERSIONS, nargs='*', action='extend', help=f'Run mypy for certain Python versions (defaults to {LOWEST_SUPPORTED_VERSION!r})')
