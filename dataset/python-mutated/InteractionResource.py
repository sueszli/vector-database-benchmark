...
class InteractionResource(GenericResource):
    ...
    async def collection(self, query, meta, user, **kwargs):
        ...
        if pipeline_interaction:
            ...
            interactions += await asyncio.gather(*[load_interaction(uuid) for uuid in interaction_uuids])
    ...
    async def create(self, payload, user, **kwargs):
        pipeline_interaction = kwargs.get('parent_model')
        uuid = clean_name(payload.get('uuid') or '', allow_characters=['.', os.path.sep])
        interaction = Interaction(uuid=uuid, pipeline=pipeline_interaction.pipeline if pipeline_interaction else None)
        ...
        if 'content' in payload:
            ...
            payload_update['content_parsed'] = extract(payload, ['inputs', 'layout', 'variables'])
        await interaction.update(**payload_update)
        block_uuid = payload.get('block_uuid')
        if block_uuid and pipeline_interaction:
            await pipeline_interaction.add_interaction_to_block(block_uuid, interaction)
