...
class TestCustomJobWithDefaultProjectIdHook:
    ...
    def test_cancel_pipeline_job(self, mock_client) -> None:
        if False:
            print('Hello World!')
        self.hook.cancel_pipeline_job(project_id=TEST_PROJECT_ID, region=TEST_REGION, pipeline_job=TEST_PIPELINE_JOB_ID)
        mock_client.assert_called_once_with(TEST_REGION)
        mock_client.return_value.cancel_pipeline_job.assert_called_once_with(request=dict(name=mock_client.return_value.pipeline_job_path.return_value), metadata=(), retry=DEFAULT, timeout=None)
    ...
    def test_cancel_training_pipeline(self, mock_client) -> None:
        if False:
            return 10
        self.hook.cancel_training_pipeline(project_id=TEST_PROJECT_ID, region=TEST_REGION, training_pipeline=TEST_TRAINING_PIPELINE_NAME)
        mock_client.assert_called_once_with(TEST_REGION)
        mock_client.return_value.cancel_training_pipeline.assert_called_once_with(request=dict(name=mock_client.return_value.training_pipeline_path.return_value), metadata=(), retry=DEFAULT, timeout=None)
    ...
    def test_create_training_pipeline(self, mock_client) -> None:
        if False:
            print('Hello World!')
        self.hook.create_training_pipeline(project_id=TEST_PROJECT_ID, region=TEST_REGION, training_pipeline=TEST_TRAINING_PIPELINE)
        mock_client.assert_called_once_with(TEST_REGION)
        mock_client.return_value.create_training_pipeline.assert_called_once_with(request=dict(parent=mock_client.return_value.common_location_path.return_value, training_pipeline=TEST_TRAINING_PIPELINE), metadata=(), retry=DEFAULT, timeout=None)
        mock_client.return_value.common_location_path.assert_called_once_with(TEST_PROJECT_ID, TEST_REGION)
    ...
    def test_delete_pipeline_job(self, mock_client) -> None:
        if False:
            i = 10
            return i + 15
        self.hook.delete_pipeline_job(project_id=TEST_PROJECT_ID, region=TEST_REGION, pipeline_job=TEST_PIPELINE_JOB_ID)
        mock_client.assert_called_once_with(TEST_REGION)
    ...
    def test_get_pipeline_job(self, mock_client) -> None:
        if False:
            i = 10
            return i + 15
        self.hook.get_pipeline_job(project_id=TEST_PROJECT_ID, region=TEST_REGION, pipeline_job=TEST_PIPELINE_JOB_ID)
        mock_client.assert_called_once_with(TEST_REGION)
