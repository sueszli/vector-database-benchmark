...
class DetrImageProcessingTester(unittest.TestCase):
    ...
    def __init__(self, parent, batch_size=7, num_channels=3, min_resolution=30, max_resolution=400, do_resize=True, size=None, do_rescale=True, rescale_factor=1 / 255, do_normalize=True, image_mean=[0.5, 0.5, 0.5], image_std=[0.5, 0.5, 0.5], do_pad=True):
        if False:
            i = 10
            return i + 15
    ...
    def prepare_image_processor_dict(self):
        if False:
            i = 10
            return i + 15
        return {'do_resize': self.do_resize, 'size': self.size, 'do_rescale': self.do_rescale, 'rescale_factor': self.rescale_factor, 'do_normalize': self.do_normalize, 'image_mean': self.image_mean, 'image_std': self.image_std, 'do_pad': self.do_pad}
