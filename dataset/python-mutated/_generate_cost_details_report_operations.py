...
from ...operations._generate_cost_details_report_operations import build_create_operation_request, build_get_operation_results_request
T = TypeVar('T')
...
class GenerateCostDetailsReportOperations:
    ...
    async def _create_operation_initial(self, scope: str, parameters: Union[_models.GenerateCostDetailsReportRequestDefinition, IO], **kwargs: Any) -> Optional[_models.CostDetailsOperationResults]:
        ...
        request = build_create_operation_request(scope=scope, api_version=api_version, content_type=content_type, json=_json, content=_content, template_url=self._create_operation_initial.metadata['url'], headers=_headers, params=_params)
        ...
        if response.status_code not in [200, 202, 204]:
            ...
            error = self._deserialize.failsafe_deserialize(_models.GenerateCostDetailsReportErrorResponse, pipeline_response)
        ...
        return deserialized
    _create_operation_initial.metadata = {'url': '/{scope}/providers/Microsoft.CostManagement/generateCostDetailsReport'}

    @overload
    async def begin_create_operation(self, scope: str, parameters: _models.GenerateCostDetailsReportRequestDefinition, *, content_type: str='application/json', **kwargs: Any) -> AsyncLROPoller[_models.CostDetailsOperationResults]:
    ...
    @overload
    async def begin_create_operation(self, scope: str, parameters: IO, *, content_type: str='application/json', **kwargs: Any) -> AsyncLROPoller[_models.CostDetailsOperationResults]:
        """This API is the replacement for all previously release Usage Details APIs. Request to generate
        a cost details report for the provided date range, billing period (Only enterprise customers)
        or Invoice Id asynchronously at a certain scope. The initial call to request a report will
        return a 202 with a 'Location' and 'Retry-After' header. The 'Location' header will provide the
    ...
    async def begin_create_operation(self, scope: str, parameters: Union[_models.GenerateCostDetailsReportRequestDefinition, IO], **kwargs: Any) -> AsyncLROPoller[_models.CostDetailsOperationResults]:
        """This API is the replacement for all previously release Usage Details APIs. Request to generate
        a cost details report for the provided date range, billing period (Only enterprise customers)
        or Invoice Id asynchronously at a certain scope. The initial call to request a report will
        return a 202 with a 'Location' and 'Retry-After' header. The 'Location' header will provide the
        ...
        if cont_token is None:
            raw_result = await self._create_operation_initial(scope=scope, parameters=parameters, api_version=api_version, content_type=content_type, cls=lambda x, y, z: x, headers=_headers, params=_params, **kwargs)
    ...
    async def _get_operation_results_initial(self, scope: str, operation_id: str, **kwargs: Any) -> Optional[_models.CostDetailsOperationResults]:
        error_map = {401: ClientAuthenticationError, 404: ResourceNotFoundError, 409: ResourceExistsError, 304: ResourceNotModifiedError}
        error_map.update(kwargs.pop('error_map', {}) or {})
        ...
        api_version: str = kwargs.pop('api_version', _params.pop('api-version', self._config.api_version))
        cls: ClsType[Optional[_models.CostDetailsOperationResults]] = kwargs.pop('cls', None)
        request = build_get_operation_results_request(scope=scope, operation_id=operation_id, api_version=api_version, template_url=self._get_operation_results_initial.metadata['url'], headers=_headers, params=_params)
        request = _convert_request(request)
        request.url = self._client.format_url(request.url)
        _stream = False
        pipeline_response: PipelineResponse = await self._client._pipeline.run(request, stream=_stream, **kwargs)
        ...
        if cls:
            return cls(pipeline_response, deserialized, {})
        return deserialized
    _get_operation_results_initial.metadata = {'url': '/{scope}/providers/Microsoft.CostManagement/costDetailsOperationResults/{operationId}'}

    @distributed_trace_async
    async def begin_get_operation_results(self, scope: str, operation_id: str, **kwargs: Any) -> AsyncLROPoller[_models.CostDetailsOperationResults]:
        ...
        cont_token: Optional[str] = kwargs.pop('continuation_token', None)
        if cont_token is None:
            raw_result = await self._get_operation_results_initial(scope=scope, operation_id=operation_id, api_version=api_version, cls=lambda x, y, z: x, headers=_headers, params=_params, **kwargs)
        kwargs.pop('error_map', None)
        ...
        if cont_token:
            return AsyncLROPoller.from_continuation_token(polling_method=polling_method, continuation_token=cont_token, client=self._client, deserialization_callback=get_long_running_output)
        return AsyncLROPoller(self._client, raw_result, get_long_running_output, polling_method)
    begin_get_operation_results.metadata = {'url': '/{scope}/providers/Microsoft.CostManagement/costDetailsOperationResults/{operationId}'}