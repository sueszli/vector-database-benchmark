...
def test_l_diversity_analysis_single_field(topic_id: str, subscription_id: str, bigquery_project: str, capsys: pytest.CaptureFixture) -> None:
    if False:
        return 10
    risk.l_diversity_analysis(GCLOUD_PROJECT, TABLE_PROJECT, BIGQUERY_DATASET_ID, BIGQUERY_HARMFUL_TABLE_ID, topic_id, subscription_id, UNIQUE_FIELD, [NUMERIC_FIELD])
...
@pytest.mark.flaky(max_runs=3, min_passes=1)
def test_l_diversity_analysis_multiple_field(topic_id: str, subscription_id: str, bigquery_project: str, capsys: pytest.CaptureFixture) -> None:
    if False:
        for i in range(10):
            print('nop')
    risk.l_diversity_analysis(GCLOUD_PROJECT, TABLE_PROJECT, BIGQUERY_DATASET_ID, BIGQUERY_HARMFUL_TABLE_ID, topic_id, subscription_id, UNIQUE_FIELD, [NUMERIC_FIELD, REPEATED_FIELD])
    (out, _) = capsys.readouterr()
    assert 'Quasi-ID values:' in out
    assert 'Class size:' in out
    assert 'Sensitive value' in out
