...
from prefect.utilities.visualization import TaskVizTracker, VisualizationUnsupportedError, VizTask, _track_viz_task, get_task_viz_tracker
...
class TestTaskVizTracker:
    ...
    @pytest.mark.parametrize('trackable', [('my_return_value', True), ([1, 2, 3], True), (500, True), (None, False), (1, False)])
...
class TestTrackTaskRun:
    ...
    async def test_track_task_run_outside_ctx(self, monkeypatch):
        mock = Mock()
        monkeypatch.setattr('prefect.utilities.visualization.TaskVizTracker.add_task', mock)
        _track_viz_task('my_task', {'a': 1})
    ...
    async def test_track_task_run_returns_viz_return_value(self):
        s = 'my_return_value'
        with TaskVizTracker():
            res = _track_viz_task('upstream_task_with_value', {'a': 1}, viz_return_value=s)
            assert res == s
            assert id(res) == id(s)
