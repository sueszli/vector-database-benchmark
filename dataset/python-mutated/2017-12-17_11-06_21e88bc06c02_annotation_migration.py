...
def upgrade():
    ...
    for slc in session.query(Slice).filter(or_(Slice.viz_type.like('line'), Slice.viz_type.like('bar'))):
        ...
        if layers:
            new_layers = []
            for layer in layers:
                new_layers.append({'annotationType': 'INTERVAL', 'style': 'solid', 'name': f'Layer {layer}', 'show': True, 'overrides': {'since': None, 'until': None}, 'value': layer, 'width': 1, 'sourceType': 'NATIVE'})
