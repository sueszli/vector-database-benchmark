...
polar_text('<––– hue –––', family=family, angle=np.pi / 4, radius=radius, scale=2 * scale)
ax.text(3 * np.pi / 4, 1.5, 'value = 1.00', family=family, size=10, bbox={'pad': 1.5, 'linewidth': 0.5, 'boxstyle': 'round,pad=.2', 'edgecolor': 'black', 'facecolor': 'white'})
...
(ax.set_xticks([]), ax.set_yticks([]))
polar_text('<––– hue –––', family=family, angle=np.pi / 4, radius=radius, scale=2 * scale)
ax.text(3 * np.pi / 4, 1.5, 'value = 0.75', family=family, size=10, bbox={'pad': 1.5, 'linewidth': 0.5, 'boxstyle': 'round,pad=.2', 'edgecolor': 'None', 'facecolor': '0.75'})
ax.text(np.pi, 0.0, '––– saturation –––> ', size=8, family=family)
polar_imshow(ax, Z)
...
(ax.set_xticks([]), ax.set_yticks([]))
polar_text('<––– hue –––', family=family, angle=np.pi / 4, radius=radius, scale=2 * scale)
ax.text(3 * np.pi / 4, 1.5, 'value = 0.50', family=family, size=10, color='white', bbox={'pad': 1.5, 'linewidth': 0.5, 'boxstyle': 'round,pad=.2', 'edgecolor': 'None', 'facecolor': '0.5'})
...
(ax.set_xticks([]), ax.set_yticks([]))
polar_text('<––– hue –––', family=family, angle=np.pi / 4, radius=radius, scale=2 * scale)
ax.text(3 * np.pi / 4, 1.5, 'value = 0.25', family=family, size=10, color='white', bbox={'pad': 1.5, 'linewidth': 0.5, 'boxstyle': 'round,pad=.2', 'edgecolor': 'None', 'facecolor': '0.25'})
ax.text(np.pi, 0.0, '-–– saturation –––> ', size=8, family=family, color='1.0')
polar_imshow(ax, Z)
...
labels = ['red', 'orange', 'yellow', 'lime', 'green', 'turquoise', 'cyan', 'skyblue', 'blue', 'violet', 'purple', 'magenta']
for (label, x) in zip(labels, np.linspace(0.5, 12.5, 12)):
    label = label.upper()
    angle = x / 13 * 2 * np.pi
    polar_text(label.upper(), family=family, angle=angle, radius=radius, scale=1.25 * scale)
