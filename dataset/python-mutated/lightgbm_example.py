...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-cv', action='store_true', help='Use `lgb.cv` instead of `lgb.train`.')
    (args, _) = parser.parse_known_args()
    config = {'objective': 'binary', 'metric': ['binary_error', 'binary_logloss'], 'verbose': -1, 'boosting_type': tune.grid_search(['gbdt', 'dart']), 'num_leaves': tune.randint(10, 1000), 'learning_rate': tune.loguniform(1e-08, 0.1)}
    tuner = tune.Tuner(train_breast_cancer if not args.use_cv else train_breast_cancer_cv, tune_config=tune.TuneConfig(metric='binary_error', mode='min', num_samples=2, scheduler=ASHAScheduler()), param_space=config)
    results = tuner.fit()
