...
import torch
from torch.fx.passes.utils.source_matcher_utils import check_subgraphs_connected, get_source_partitions, SourcePartition
__all__ = ['find_sequential_partitions', 'get_equivalent_types', 'update_equivalent_types_dict']
_EQUIVALENT_TYPES: List[Set] = [{torch.nn.Conv1d, torch.nn.functional.conv1d}, {torch.nn.Conv2d, torch.nn.functional.conv2d}, {torch.nn.AdaptiveAvgPool2d, torch.nn.functional.adaptive_avg_pool2d}, {torch.nn.ReLU, torch.nn.functional.relu, torch.nn.functional.relu_}, {torch.nn.BatchNorm2d, torch.nn.functional.batch_norm}, {torch.nn.Hardtanh, torch.nn.functional.hardtanh, torch.nn.functional.hardtanh_}, {torch.add, operator.add, operator.iadd, 'add', 'add_'}, {torch.mul, operator.mul, operator.imul, 'mul', 'mul_'}]
