...
class ProductMediaCreateInput(BaseInputObjectType):
    ...
    image = Upload(required=False, description='Represents an image file in a multipart request.')
    product = graphene.ID(required=True, description='ID of an product.', name='product')
    media_url = graphene.String(required=False, description='Represents an URL to an external media.')
...
class ProductMediaCreate(BaseMutation):
    ...
    class Arguments:
        input = ProductMediaCreateInput(required=True, description='Fields required to create a product media.')
