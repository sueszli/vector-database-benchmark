...
class GRPCClient:
    ...
    async def run_code(self, code: str, sdk: SdkEnum, pipeline_options: str, datasets: List[api_pb2.Dataset], files: List[api_pb2.SnippetFile]) -> str:
        """
        ...
        if sdk not in api_pb2.Sdk.values():
            ...
            sdks.remove(api_pb2.Sdk.Name(0))
            raise Exception(f"Incorrect sdk: must be from this pool: {', '.join(sdks)}")
        request = api_pb2.RunCodeRequest(code=code, sdk=sdk, pipeline_options=pipeline_options, datasets=datasets, files=files)
