...
class LibTorrentEndpoint(RESTEndpoint):
    ...
    @docs(tags=['Libtorrent'], summary='Return Libtorrent session settings.', parameters=[{'in': 'query', 'name': 'hop', 'description': 'The hop count of the session for which to return settings', 'type': 'string', 'required': False}], responses={200: {'description': 'Return a dictonary with key-value pairs from the Libtorrent session settings', 'schema': schema(LibtorrentSessionResponse={'hop': Integer, 'settings': schema(LibtorrentSettings={})})}})
    ...
    @docs(tags=['Libtorrent'], summary='Return Libtorrent session information.', parameters=[{'in': 'query', 'name': 'hop', 'description': 'The hop count of the session for which to return information', 'type': 'string', 'required': False}], responses={200: {'description': 'Return a dictonary with key-value pairs from the Libtorrent session information', 'schema': schema(LibtorrentinfoResponse={'hop': Integer, 'settings': schema(LibtorrentInfo={})})}})
    async def get_libtorrent_session_info(self, request):
        ...
        if 'hop' in args and args['hop']:
            hop = int(args['hop'])
        if hop not in self.download_manager.ltsessions or not hasattr(self.download_manager.ltsessions[hop], 'post_session_stats'):
