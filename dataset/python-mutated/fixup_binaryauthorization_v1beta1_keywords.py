...
class binaryauthorizationCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_attestor': ('parent', 'attestor_id', 'attestor'), 'delete_attestor': ('name',), 'get_attestor': ('name',), 'get_policy': ('name',), 'get_system_policy': ('name',), 'list_attestors': ('parent', 'page_size', 'page_token'), 'update_attestor': ('attestor',), 'update_policy': ('policy',)}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=binaryauthorizationCallTransformer()):
    if False:
        return 10
