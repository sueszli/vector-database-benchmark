...
import numpy as np
from fairseq.data import ConcatSentencesDataset, Dictionary, IdDataset, NestedDictionaryDataset, NumelDataset, NumSamplesDataset, OffsetTokensDataset, PrependTokenDataset, RawLabelDataset, RightPadDataset, RightPaddingMaskDataset, RollDataset, SortDataset, StripTokenDataset, data_utils
...
class SentencePredictionConfig(FairseqDataclass):
    ...
    num_classes: int = field(default=-1, metadata={'help': 'number of classes or regression targets'})
    init_token: Optional[int] = field(default=None, metadata={'help': 'add token at the beginning of each batch item'})
    separator_token: Optional[int] = field(default=None, metadata={'help': 'add separator token between inputs'})
    no_shuffle: bool = field(default=False)
    shorten_method: SHORTEN_METHOD_CHOICES = field(default='none', metadata={'help': 'if not none, shorten sequences that exceed tokens_per_sample'})
    ...
    add_prev_output_tokens: bool = field(default=False, metadata={'help': 'add prev_output_tokens to sample, used for encoder-decoder arch'})
    max_positions: int = field(default=512, metadata={'help': 'max tokens per example'})
    ...
    d2v2_multi: bool = field(default=False, metadata={'help': 'prepare dataset for data2vec_multi'})
