...
class TestLaunch:
    ...
    async def test_terminate_on_nursery_exit(self, caplog: pytest.LogCaptureFixture, webbrowser_launch):
        ...
        assert [(record.name, record.levelname, record.msg) for record in caplog.records] == [('streamlink.webbrowser.webbrowser', 'debug', 'Waiting for web browser process to terminate')]
    ...
    async def test_terminate_on_nursery_timeout(self, caplog: pytest.LogCaptureFixture, mock_clock, webbrowser_launch):
        ...
        async with webbrowser_launch(timeout=10) as (_nursery, process):
            ...
            await trio.sleep(0)
        assert process.poll() == (1 if is_win32 else -SIGTERM), 'Process has been terminated'
        assert [(record.name, record.levelname, record.msg) for record in caplog.records] == [('streamlink.webbrowser.webbrowser', 'warning', 'Web browser task group has timed out'), ('streamlink.webbrowser.webbrowser', 'debug', 'Waiting for web browser process to terminate')]
    ...
    async def test_terminate_on_nursery_baseexception(self, caplog: pytest.LogCaptureFixture, webbrowser_launch):
        ...
        class FakeBaseException(BaseException):
            pass
        process: trio.Process
        with pytest.raises(FakeBaseException):
            async with webbrowser_launch() as (_nursery, process):
                assert process.poll() is None, 'process is still running'
    ...
    async def test_process_ended_early(self, caplog: pytest.LogCaptureFixture, webbrowser_launch, exit_code):
        ...
        assert [(record.name, record.levelname, record.msg) for record in caplog.records] == [('streamlink.webbrowser.webbrowser', 'warning', 'Web browser process ended early')]
