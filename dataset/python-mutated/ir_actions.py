...
class IrActions(models.Model):
    ...
    help = fields.Html(string='Action Description', help='Optional help text for the users with a description of the target view, such as its usage and purpose.', translate=True)
...
class IrActionsReportXml(models.Model):
    ...
    report_type = fields.Selection([('qweb-pdf', 'PDF'), ('qweb-html', 'HTML'), ('controller', 'Controller'), ('pdf', 'RML pdf (deprecated)'), ('sxw', 'RML sxw (deprecated)'), ('webkit', 'Webkit (deprecated)')], required=True, default='pdf', help='HTML will open the report directly in your browser, PDF will use wkhtmltopdf to render the HTML into a PDF file and let you download it, Controller allows you to define the url of a custom controller outputting any kind of report.')
    report_name = fields.Char(string='Template Name', required=True, help="For QWeb reports, name of the template used in the rendering. The method 'render_html' of the model 'report.template_name' will be called (if any) to give the html. For RML reports, this is the LocalService name.")
    groups_id = fields.Many2many('res.groups', 'res_groups_report_rel', 'uid', 'gid', string='Groups')
    ir_values_id = fields.Many2one('ir.values', string='More Menu entry', readonly=True, help='More menu entry.', copy=False)
    ...
    attachment = fields.Char(string='Save as Attachment Prefix', help='This is the filename of the attachment used to store the printing result. Keep empty to not save the printed reports. You can use a python expression with the object and time variables.')
    ...
    report_file = fields.Char(related='report_rml', string='Report File', required=False, readonly=False, store=True, help='The path to the main report file (depending on Report Type) or empty if the content is in another field')
...
class IrActionsActWindow(models.Model):
    ...
    domain = fields.Char(string='Domain Value', help='Optional domain filtering of the destination data, as a Python expression')
    ...
    res_model = fields.Char(string='Destination Model', required=True, help='Model name of the object to open in the view window')
    ...
    view_mode = fields.Char(required=True, default='tree,form', help="Comma-separated list of allowed view modes, such as 'form', 'tree', 'calendar', etc. (Default: tree,form)")
    view_type = fields.Selection([('tree', 'Tree'), ('form', 'Form')], default='form', string='View Type', required=True, help="View type: Tree type to use for the tree view, set to 'tree' for a hierarchical tree view, or 'form' for a regular list view")
    usage = fields.Char(string='Action Usage', help='Used to filter menu and home actions from the user form.')
    view_ids = fields.One2many('ir.actions.act_window.view', 'act_window_id', string='Views')
    views = fields.Binary(compute='_compute_views', help='This function field computes the ordered list of views that should be enabled when displaying the result of an action, federating view mode, views and reference view. The result is returned as an ordered list of pairs (view_id,view_mode).')
...
VIEW_TYPES = [('tree', 'Tree'), ('form', 'Form'), ('graph', 'Graph'), ('pivot', 'Pivot'), ('calendar', 'Calendar'), ('gantt', 'Gantt'), ('kanban', 'Kanban')]
...
class IrActionsActUrl(models.Model):
    ...
    target = fields.Selection([('new', 'New Window'), ('self', 'This Window')], string='Action Target', default='new', required=True)
...
class IrActionsServer(models.Model):
    ...
    condition = fields.Char(default='True', help="Condition verified before executing the server action. If it is not verified, the action will not be executed. The condition is a Python expression, like 'record.list_price > 5000'. A void condition is considered as always True. Help about python expression is given in the help tab.")
    state = fields.Selection(selection=lambda self: self._get_states(), string='Action To Do', default='code', required=True, help="Type of server action. The following values are available:\n- 'Execute Python Code': a block of python code that will be executed\n- 'Trigger a Workflow Signal': send a signal to a workflow\n- 'Run a Client Action': choose a client action to launch\n- 'Create or Copy a new Record': create a new record with new values, or copy an existing record in your database\n- 'Write on a Record': update the values of a record\n- 'Execute several actions': define an action that triggers several other server actions\n- 'Send Email': automatically send an email (available in email_template)")
    sequence = fields.Integer(default=5, help='When dealing with multiple actions, the execution order is based on the sequence. Low number means high priority.')
    model_id = fields.Many2one('ir.model', string='Base Model', required=True, ondelete='cascade', help='Base model on which the server action runs.')
    model_name = fields.Char(related='model_id.model', readonly=True)
    menu_ir_values_id = fields.Many2one('ir.values', string='More Menu entry', readonly=True, help='More menu entry.', copy=False)
    action_id = fields.Many2one('ir.actions.actions', string='Client Action', help='Select the client action that has to be executed.')
    code = fields.Text(string='Python Code', default=DEFAULT_PYTHON_CODE, help='Write Python code that the action will execute. Some variables are available for use; help about pyhon expression is given in the help tab.')
    use_relational_model = fields.Selection([('base', 'Use the base model of the action'), ('relational', 'Use a relation field on the base model')], string='Relational Target Model', default='base', required=True)
    wkf_transition_id = fields.Many2one('workflow.transition', string='Signal to Trigger', help='Select the workflow signal to trigger.')
    wkf_model_id = fields.Many2one('ir.model', string='Target Model', help='The model that will receive the workflow signal. Note that it should have a workflow associated with it.')
    wkf_model_name = fields.Char(string='Target Model Name', related='wkf_model_id.model', store=True, readonly=True)
    wkf_field_id = fields.Many2one('ir.model.fields', string='Relation Field', oldname='trigger_obj_id', help='The field on the current object that links to the target object record (must be a many2one, or an integer field with the record ID)')
    child_ids = fields.Many2many('ir.actions.server', 'rel_server_actions', 'server_id', 'action_id', string='Child Actions', help='Child server actions that will be executed. Note that the last return returned action value will be used as global return value.')
    use_create = fields.Selection([('new', 'Create a new record in the Base Model'), ('new_other', 'Create a new record in another model'), ('copy_current', 'Copy the current record'), ('copy_other', 'Choose and copy a record in the database')], string='Creation Policy', default='new', required=True)
    crud_model_id = fields.Many2one('ir.model', string='Create/Write Target Model', oldname='srcmodel_id', help='Model for record creation / update. Set this field only to specify a different model than the base model.')
    ...
    link_new_record = fields.Boolean(string='Attach the new record', help='Check this if you want to link the newly-created record to the current record on which the server action runs.')
    link_field_id = fields.Many2one('ir.model.fields', string='Link using field', oldname='record_id', help='Provide the field where the record id is stored after the operations.')
    use_write = fields.Selection([('current', 'Update the current record'), ('expression', 'Update a record linked to the current record using python'), ('other', 'Choose and Update a record in the database')], string='Update Policy', default='current', required=True)
    write_expression = fields.Char(string='Expression', oldname='write_id', help='Provide an expression that, applied on the current record, gives the field to update.')
...
class IrActionsTodo(models.Model):
    ...
    name = fields.Char()
    type = fields.Selection([('manual', 'Launch Manually'), ('once', 'Launch Manually Once'), ('automatic', 'Launch Automatically')], default='manual', required=True, help='Manual: Launched manually.\n                                    Automatic: Runs whenever the system is reconfigured.\n                                    Launch Manually Once: after having been launched manually, it sets automatically to Done.')
...
class IrActionsActClient(models.Model):
    ...
    tag = fields.Char(string='Client action tag', required=True, help='An arbitrary string, interpreted by the client according to its own needs and wishes. There is no central tag repository across clients.')
    ...
    context = fields.Char(string='Context Value', default='{}', required=True, help='Context dictionary as Python expression, empty by default (Default: {})')
    params = fields.Binary(compute='_compute_params', inverse='_inverse_params', string='Supplementary arguments', help='Arguments sent to the client along withthe view tag')
