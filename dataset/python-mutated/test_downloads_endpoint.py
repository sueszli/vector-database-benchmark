...
ExtendedStatusConfig = collections.namedtuple('ExtendedStatusConfig', ['hops', 'candidates', 'has_status'], defaults=[0, 0, True])
...
async def test_get_downloads(mock_dlmgr, test_download, rest_api):
    ...
    assert 'peers' in downloads['downloads'][0]
    assert 'pieces' not in downloads['downloads'][0]
    assert 'files' in downloads['downloads'][0]
...
async def test_get_downloads_circuit_peer(mock_dlmgr, test_download, rest_api, endpoint):
    ...
    test_download.get_peerlist = lambda : [{'have': b'\x00' * 200, 'ip': '127.0.0.1', 'port': CIRCUIT_ID_PORT, 'circuit': 42}]
    downloads = await do_request(rest_api, 'downloads?get_peers=1', expected_code=200)
    assert len(downloads['downloads']) == 1
    assert 'peers' in downloads['downloads'][0]
    assert 'pieces' not in downloads['downloads'][0]
    assert 'files' not in downloads['downloads'][0]
    assert downloads['checkpoints'] == {'total': 1, 'loaded': 1, 'all_loaded': True}
...
async def test_get_downloads_with_passing_filter(mock_dlmgr, test_download, rest_api):
    ...
    mock_dlmgr.checkpoints_loaded = 1
    mock_dlmgr.all_checkpoints_are_loaded = True
    downloads = await do_request(rest_api, 'downloads?get_peers=1&get_pieces=1&infohash=' + test_download.infohash, expected_code=200)
    assert len(downloads['downloads']) == 1
    assert 'peers' in downloads['downloads'][0]
    assert 'pieces' in downloads['downloads'][0]
    assert 'files' not in downloads['downloads'][0]
    assert downloads['checkpoints'] == {'total': 1, 'loaded': 1, 'all_loaded': True}
...
async def test_get_downloads_with_unpassing_filter(mock_dlmgr, test_download, rest_api):
    ...
    mock_dlmgr.checkpoints_loaded = 1
    mock_dlmgr.all_checkpoints_are_loaded = True
    downloads = await do_request(rest_api, 'downloads?get_peers=1&get_pieces=1&infohash=' + '00' * 20, expected_code=200)
    assert len(downloads['downloads']) == 1
    assert 'peers' not in downloads['downloads'][0]
    assert 'pieces' not in downloads['downloads'][0]
    assert 'files' not in downloads['downloads'][0]
    assert downloads['checkpoints'] == {'total': 1, 'loaded': 1, 'all_loaded': True}
...
async def test_start_download_from_file(test_download, mock_dlmgr, rest_api):
    ...
    await do_request(rest_api, 'downloads', expected_code=200, request_type='PUT', post_data={'uri': uri}, expected_json=expected_json)
...
async def test_remove_wrong_infohash(mock_dlmgr, rest_api):
    ...
    await do_request(rest_api, 'downloads/abcd', post_data={'remove_data': True}, expected_code=404, request_type='DELETE')
...
async def test_remove(mock_dlmgr, test_download, rest_api):
    ...
    await do_request(rest_api, f'downloads/{test_download.infohash}', post_data={'remove_data': False}, expected_code=200, request_type='DELETE', expected_json={'removed': True, 'infohash': test_download.infohash})
...
async def test_stop_download(mock_dlmgr, test_download, rest_api):
    ...
    await do_request(rest_api, f'downloads/{test_download.infohash}', post_data={'state': 'stop'}, expected_code=200, request_type='PATCH', expected_json={'modified': True, 'infohash': test_download.infohash})
...
async def test_select_download_file_range(mock_dlmgr, test_download, rest_api):
    ...
    """
    mock_dlmgr.get_download = lambda _: test_download
    await do_request(rest_api, f'downloads/{test_download.infohash}', expected_code=400, post_data={'selected_files': [1234]}, request_type='PATCH')
...
async def test_select_download_file(mock_dlmgr, test_download, rest_api):
    ...
    await do_request(rest_api, f'downloads/{test_download.infohash}', post_data={'selected_files': [0]}, expected_code=200, request_type='PATCH', expected_json={'modified': True, 'infohash': test_download.infohash})
...
async def test_resume_download(mock_dlmgr, test_download, rest_api):
    ...
    await do_request(rest_api, f'downloads/{test_download.infohash}', post_data={'state': 'resume'}, expected_code=200, request_type='PATCH', expected_json={'modified': True, 'infohash': test_download.infohash})
...
async def test_recheck_download(mock_dlmgr, test_download, rest_api):
    ...
    await do_request(rest_api, f'downloads/{test_download.infohash}', post_data={'state': 'recheck'}, expected_code=200, request_type='PATCH', expected_json={'modified': True, 'infohash': test_download.infohash})
...
async def test_download_unknown_state(mock_dlmgr, test_download, rest_api):
    ...
    """
    mock_dlmgr.get_download = lambda _: test_download
    await do_request(rest_api, f'downloads/{test_download.infohash}', expected_code=400, post_data={'state': 'abc'}, request_type='PATCH', expected_json={'error': 'unknown state parameter'})
...
async def test_change_hops_error(mock_dlmgr, test_download, rest_api):
    ...
    await do_request(rest_api, f'downloads/{test_download.infohash}', post_data={'state': 'resume', 'anon_hops': 1}, expected_code=400, request_type='PATCH')
...
async def test_move_to_non_existing_dir(mock_dlmgr, test_download, rest_api, tmp_path):
    ...
    response_dict = await do_request(rest_api, f'downloads/{test_download.infohash}', expected_code=200, post_data=data, request_type='PATCH')
...
async def test_export_download(mock_dlmgr, mock_handle, test_download, rest_api):
    ...
    mock_dlmgr.get_download = lambda _: test_download
    await do_request(rest_api, f'downloads/{test_download.infohash}/torrent', expected_code=200, request_type='GET', json_response=False)
...
async def test_get_download_files(mock_dlmgr, test_download, rest_api):
    ...
    """
    mock_dlmgr.get_download = lambda _: test_download
    response_dict = await do_request(rest_api, f'downloads/{test_download.infohash}/files', expected_code=200, request_type='GET')
...
async def test_stream_unknown_download(mock_dlmgr, rest_api):
    ...
    """
    mock_dlmgr.get_download = lambda _: None
    await do_request(rest_api, 'downloads/abcd/stream/0', headers={'range': 'bytes=0-'}, expected_code=404, request_type='GET')
...
async def test_stream_download_out_of_bounds_file(mock_dlmgr, mock_handle, test_download, rest_api):
    ...
    """
    mock_dlmgr.get_download = lambda _: test_download
    await do_request(rest_api, f'downloads/{test_download.infohash}/stream/100', headers={'range': 'bytes=0-'}, expected_code=500, request_type='GET')
...
async def test_stream_download(mock_dlmgr, mock_handle, test_download, rest_api, tmp_path):
    ...
    await do_request(rest_api, f'downloads/{test_download.infohash}/stream/0', headers={'range': 'bytes=0-'}, expected_code=206, json_response=False)
...
async def test_change_hops(mock_dlmgr, test_download, rest_api):
    ...
    await do_request(rest_api, f'downloads/{test_download.infohash}', post_data={'anon_hops': 1}, expected_code=200, request_type='PATCH', expected_json={'modified': True, 'infohash': test_download.infohash})
...
async def test_change_hops_fail(mock_dlmgr, test_download, rest_api):
    ...
    await do_request(rest_api, f'downloads/{test_download.infohash}', post_data={'anon_hops': 1}, expected_code=500, request_type='PATCH', expected_json={'error': {'message': '', 'code': 'RuntimeError', 'handled': True}})