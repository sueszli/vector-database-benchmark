...
TRANSLATED_ATTRS_RE = re.compile('@(%s)\\b' % '|'.join(TRANSLATED_ATTRS))
...
class View(models.Model):
    ...
    type = fields.Selection([('tree', 'Tree'), ('form', 'Form'), ('graph', 'Graph'), ('pivot', 'Pivot'), ('calendar', 'Calendar'), ('diagram', 'Diagram'), ('gantt', 'Gantt'), ('kanban', 'Kanban'), ('search', 'Search'), ('qweb', 'QWeb')], string='View Type')
    ...
    xml_id = fields.Char(string='External ID', compute='_compute_xml_id', help='ID of the view defined in xml file')
    ...
    mode = fields.Selection([('primary', 'Base view'), ('extension', 'Extension View')], string='View inheritance mode', default='primary', required=True, help="Only applies if this view inherits from an other one (inherit_id is not False/Null).\n\n* if extension (default), if this view is requested the closest primary view\nis looked up (via inherit_id), then all views inheriting from it with this\nview's model are applied\n* if primary, the closest primary view is fully resolved (even if it uses a\ndifferent model than this one), then this view's inheritance specs\n(<xpath/>) are applied, and the result is used as if it were this view's\nactual arch.\n")
    active = fields.Boolean(default=True, help='If this view is inherited,\n* if True, the view always extends its parent\n* if False, the view currently does not extend its parent but can be enabled\n         ')
    ...
    _sql_constraints = [('inheritance_mode', "CHECK (mode != 'extension' OR inherit_id IS NOT NULL)", "Invalid inheritance mode: if the mode is 'extension', the view must extend an other view")]
