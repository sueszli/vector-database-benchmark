...
class BuildAssetsCommand(BaseBuildCommand):
    user_options = BaseBuildCommand.user_options + [('asset-json-path=', None, 'Relative path for JSON manifest. Defaults to {dist_name}/assets.json'), ('inplace', 'i', 'ignore build-lib and put compiled javascript files into the source ' + 'directory alongside your pure Python modules'), ('force', 'f', 'Force rebuilding of static content. Defaults to rebuilding on version change detection.')]
