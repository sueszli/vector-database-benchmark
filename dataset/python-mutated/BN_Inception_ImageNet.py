...
def create_image_mb_source(map_file, mean_file, is_training, total_number_of_samples):
    ...
    transforms = []
    if is_training:
        transforms += [xforms.crop(crop_type='randomside', side_ratio=(0.46666, 0.875), jitter_type='uniratio')]
    else:
        transforms += [xforms.crop(crop_type='center', side_ratio=0.875)]
...
def bn_inception_train_and_eval(train_data, test_data, mean_data, minibatch_size=32, epoch_size=1281167, max_epochs=300, restore=True, log_to_file=None, num_mbs_per_log=100, gen_heartbeat=False, profiler_dir=None, testing_parameters=(5000, 32)):
    if False:
        for i in range(10):
            print('nop')
    _cntk_py.set_computation_network_trace_level(1)
    progress_printer = ProgressPrinter(freq=num_mbs_per_log, tag='Training', log_to_file=log_to_file, gen_heartbeat=gen_heartbeat, num_epochs=max_epochs)
