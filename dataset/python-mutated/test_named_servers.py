...
async def test_default_server(app, named_servers):
    ...
    assert user_model == fill_user({'name': username, 'roles': ['user'], 'auth_state': None, 'server': user.url, 'servers': {'': {'name': '', 'started': TIMESTAMP, 'last_activity': TIMESTAMP, 'url': user.url, 'pending': None, 'ready': True, 'stopped': False, 'progress_url': 'PREFIX/hub/api/users/{}/server/progress'.format(username), 'state': {'pid': 0}, 'user_options': {}}}})
    ...
    r.raise_for_status()
    user_model = normalize_user(r.json())
    assert user_model == fill_user({'name': username, 'roles': ['user'], 'auth_state': None})

@pytest.mark.parametrize('servername,escapedname,caller_escape', [('trevor', 'trevor', False), ('$p~c|a! ch@rs', '%24p~c%7Ca%21%20ch@rs', False), ('$p~c|a! ch@rs', '%24p~c%7Ca%21%20ch@rs', True), ('hash#?question', 'hash%23%3Fquestion', True)])
async def test_create_named_server(app, named_servers, servername, escapedname, caller_escape):
    username = 'walnut'
    user = add_user(app.db, app, name=username)
    ...
    r = await api_request(app, 'users', username, 'servers', request_servername, method='post')
    r.raise_for_status()
    assert r.status_code == 201
    assert r.text == ''
    ...
    r = await api_request(app, 'users', username)
    r.raise_for_status()
    db_server_names = set(app.db.query(orm.User).filter_by(name=username).first().orm_spawners.keys())
    assert db_server_names == {'', servername}
    user_model = normalize_user(r.json())
    assert user_model == fill_user({'name': username, 'roles': ['user'], 'auth_state': None, 'servers': {servername: {'name': name, 'started': TIMESTAMP, 'last_activity': TIMESTAMP, 'url': url_path_join(user.url, escapedname, '/'), 'pending': None, 'ready': True, 'stopped': False, 'progress_url': 'PREFIX/hub/api/users/{}/servers/{}/progress'.format(username, escapedname), 'state': {'pid': 0}, 'user_options': {}} for name in [servername]}})
...
async def test_create_invalid_named_server(app, named_servers):
    ...
    with pytest.raises(HTTPError) as exc:
        r.raise_for_status()
    assert exc.value.response.json() == {'status': 400, 'message': "Invalid server_name (may not contain '/'): a$/b"}
...
async def test_delete_named_server(app, named_servers):
    ...
    r = await api_request(app, 'users', username, 'servers', servername, method='delete', data=json.dumps({'remove': True}))
    r.raise_for_status()
    assert r.status_code == 204
...
async def test_named_server_limit(app, named_servers):
    ...
    assert r.status_code == 201
    assert r.text == ''
    r = await api_request(app, 'users', username, 'servers', servername1, method='delete', data=json.dumps({'remove': True}))
    r.raise_for_status()
    assert r.status_code == 204
    r = await api_request(app, 'users', username, 'servers', servername3, method='post')
    r.raise_for_status()
...
@pytest.mark.parametrize('username,admin', [('nonsuperfoo', False), ('superfoo', True)])
async def test_named_server_limit_as_callable(app, named_servers_with_callable_limit, username, admin):
    ...
    cookies = await app.login_user(username)
    servername1 = 'bar-1'
    r = await api_request(app, 'users', username, 'servers', servername1, method='post', cookies=cookies)
    r.raise_for_status()
    assert r.status_code == 201
    assert r.text == ''
    servername2 = 'bar-2'
    r = await api_request(app, 'users', username, 'servers', servername2, method='post', cookies=cookies)
    r.raise_for_status()
    assert r.status_code == 201
    assert r.text == ''
    servername3 = 'bar-3'
    r = await api_request(app, 'users', username, 'servers', servername3, method='post', cookies=cookies)
    if admin:
        r.raise_for_status()
        assert r.status_code == 201
        assert r.text == ''
        ...
        assert r.status_code == 400
        assert r.json() == {'status': 400, 'message': f'User {username} already has the maximum of 2 named servers.  One must be deleted before a new server can be created'}
...
async def test_named_server_spawn_form(app, username, named_servers):
    ...
    with mock.patch.dict(app.users.settings, {'spawner_class': FormSpawner}):
        ...
        next_url = url_path_join(app.base_url, 'hub/spawn-pending', username, server_name)
        r = await async_requests.post(url_concat(action_url, {'next': next_url}), cookies=cookies, data={'bounds': ['-10', '10'], 'energy': '938MeV'})
        r.raise_for_status()
        assert r.history
...
async def test_user_redirect_default_server_name(app, username, named_servers, default_server_name):
    name = username
    ...
    assert query == urlencode({'next': url_path_join(app.hub.base_url, '/user-redirect/tree/top/')})
    ...
    while '/spawn-pending/' in path:
        ...
        path = urlparse(r.url).path
    assert path == url_path_join(app.base_url, f'/user/{name}/{server_name}/notebooks/test.ipynb')

async def test_user_redirect_hook_default_server_name(app, username, named_servers, default_server_name):
    """
    ...
    async def dummy_redirect(path, request, user, base_url):
        ...
        assert request.uri == url_path_join(base_url, 'hub', 'user-redirect', 'redirect-to-terminal')
    ...
    assert path == url_path_join(app.base_url, '/hub/login')
    query = urlparse(r.url).query
    assert query == urlencode({'next': url_path_join(app.hub.base_url, '/user-redirect/redirect-to-terminal')})
    ...
    assert redirected_url.path == url_path_join(app.base_url, 'user', username, 'terminals/1')
...
async def test_named_server_stop_server(app, username, named_servers):
    ...
    with mock.patch.object(app.proxy, 'add_user', side_effect=Exception('mock exception')):
        r = await api_request(app, 'users', username, 'servers', server_name, method='post')
        r.raise_for_status()
        assert r.status_code == 201
        assert r.text == ''
...
@pytest.mark.parametrize('include_stopped_servers', [True, False])
