...
class glue_database_connections_ssl_enabled(Check):
    ...
    def execute(self):
        ...
        for conn in glue_client.connections:
            ...
            report.status = 'FAIL'
            report.status_extended = f'Glue connection {conn.name} has SSL connection disabled.'
            if conn.properties.get('JDBC_ENFORCE_SSL') == 'true':
                report.status = 'PASS'
                report.status_extended = f'Glue connection {conn.name} has SSL connection enabled.'
