...
from sentry.snuba.outcomes import COLUMN_MAP, GROUPBY_MAP, QueryDefinition, massage_outcomes_result, run_outcomes_query_timeseries, run_outcomes_query_totals
...
class OrgStatsQueryParamsSerializer(serializers.Serializer):
    statsPeriod = serializers.CharField(help_text='This defines the range of the time series, relative to now. The range is given in a `<number><unit>` format. For example `1d` for a one day range. Possible units are `m` for minutes, `h` for hours, `d` for days and `w` for weeks.You must either provide a `statsPeriod`, or a `start` and `end`.', required=False)
    interval = serializers.CharField(help_text='This is the resolution of the time series, given in the same format as `statsPeriod`. The default resolution is `1h` and the minimum resolution is currently restricted to `1h` as well. Intervals larger than `1d` are not supported, and the interval has to cleanly divide one day.', required=False)
    start = serializers.DateTimeField(help_text='This defines the start of the time series range as an explicit datetime, either in UTC ISO8601 or epoch seconds.Use along with `end` instead of `statsPeriod`.', required=False)
    end = serializers.DateTimeField(help_text='This defines the inclusive end of the time series range as an explicit datetime, either in UTC ISO8601 or epoch seconds.Use along with `start` instead of `statsPeriod`.', required=False)
    groupBy = serializers.MultipleChoiceField(choices=list(GROUPBY_MAP.keys()), required=True, help_text='can pass multiple groupBy parameters to group by multiple, e.g.\xa0`groupBy=project&groupBy=outcome`\xa0to group by multiple dimensions. Note that grouping by project can cause missing rows if the number of projects / interval is large. If you have a large number of projects, we recommend filtering and querying by them individually.Also note that grouping by projects does not currently support timeseries interval responses and will instead be a sum of the projectover the entire period specified.')
    field = serializers.ChoiceField(list(COLUMN_MAP.keys()), help_text="the `sum(quantity)` field is bytes for attachments, and all others the 'event' count for those types of events.\n\n`sum(times_seen)` sums the number of times an event has been seen. For 'normal' event types, this will be equal to\xa0`sum(quantity)`\xa0for now. For sessions, quantity will sum the total number of events seen in a session, while `times_seen` will be the unique number of sessions. and for attachments, `times_seen` will be the total number of attachments, while quantity will be the total sum of attachment bytes.")
    project = serializers.ListField(required=False, help_text='The ID of the projects to filter by.\n\nUse `-1` to include all accessible projects.')
    category = serializers.ChoiceField(('error', 'transaction', 'attachment', 'replays', 'profiles'), required=False, help_text='If filtering by attachments, you cannot filter by any other category due to quantity values becoming nonsensical (combining bytes and event counts).\n\nIf filtering by\xa0`error`, it will automatically add\xa0`default`\xa0and\xa0`security`\xa0as we currently roll those two categories into\xa0`error`\xa0for displaying.')
    outcome = serializers.ChoiceField([o.name.lower() for o in Outcome], required=False, help_text='See https://docs.sentry.io/product/stats/ for more information on outcome statuses.')
    reason = serializers.CharField(required=False, help_text='The reason field will contain why an event was filtered/dropped.')

class _StatsGroup(TypedDict):
    by: Dict[str, Any]
    totals: Dict[str, Any]
    series: Dict[str, Any]
...
class OrganizationStatsEndpointV2(OrganizationEventsEndpointBase):
    ...
    rate_limits = {'GET': {RateLimitCategory.IP: RateLimit(20, 1), RateLimitCategory.USER: RateLimit(20, 1), RateLimitCategory.ORGANIZATION: RateLimit(20, 1)}}
