...
class MailMailStats(models.Model):
    ...
    mail_mail_id_int = fields.Integer(string='Mail ID (tech)', help='ID of the related mail_mail. This field is an integer field because the related mail_mail can be deleted separately from its statistics. However the ID is needed for several action and controllers.')
    ...
    mass_mailing_campaign_id = fields.Many2one(related='mass_mailing_id.mass_mailing_campaign_id', string='Mass Mailing Campaign', store=True, readonly=True)
    ...
    state = fields.Selection(compute='_compute_state', selection=[('outgoing', 'Outgoing'), ('exception', 'Exception'), ('sent', 'Sent'), ('opened', 'Opened'), ('replied', 'Replied'), ('bounced', 'Bounced')], store=True)
    state_update = fields.Datetime(compute='_compute_state', string='State Update', help='Last state update of the mail', store=True)
