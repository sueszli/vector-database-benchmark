...
@sync_compatible
async def git_clone(repository: str, branch: Optional[str]=None, include_submodules: bool=False, access_token: Optional[str]=None, credentials: Optional[Block]=None) -> dict:
    """
    ...
    if access_token and credentials:
        raise ValueError('Please provide either an access token or credentials but not both.')
    credentials = {'access_token': access_token} if access_token else credentials
    storage = GitRepository(url=repository, credentials=credentials, branch=branch, include_submodules=include_submodules)
...
async def pull_with_block(block_document_name: str, block_type_slug: str):
    ...
    try:
        ...
        deployment_logger.exception("Unable to create storage adapter for block '%s'", full_slug)
        raise
    await storage.pull_code()
    directory = str(storage.destination.relative_to(Path.cwd()))
    deployment_logger.info("Pulled code using block '%s' into '%s'", full_slug, directory)
    return {'directory': directory}

@deprecated_callable(start_date='Jun 2023', help="Use 'git clone' instead.")
@sync_compatible
async def git_clone_project(repository: str, branch: Optional[str]=None, include_submodules: bool=False, access_token: Optional[str]=None) -> dict:
    """Deprecated. Use `git_clone` instead."""
    return await git_clone(repository=repository, branch=branch, include_submodules=include_submodules, access_token=access_token)