...
from .....payment.interface import PaymentGatewayData, TransactionProcessActionData, TransactionSessionData
from .....webhook.event_types import WebhookEventSyncType
from .....webhook.models import Webhook
from .....webhook.transport.asynchronous.transport import create_deliveries_for_subscriptions
TRANSACTION_INITIALIZE_SESSION = '\nsubscription {\n  event{\n    ...on TransactionInitializeSession{\n      merchantReference\n      action{\n        amount\n        currency\n        actionType\n      }\n      data\n      customerIpAddress\n      sourceObject{\n        __typename\n        ... on Checkout{\n          id\n          user{\n            id\n          }\n          totalPrice{\n            gross{\n              amount\n            }\n          }\n        }\n        ... on Order{\n          id\n          user{\n            id\n          }\n        }\n      }\n    }\n  }\n}\n'

def test_transaction_initialize_session_checkout_with_data(checkout, webhook_app, permission_manage_payments, transaction_item_generator, customer_user):
    if False:
        print('Hello World!')
    ...
    assert json.loads(delivery.payload.payload) == {'merchantReference': graphene.Node.to_global_id('TransactionItem', transaction.token), 'action': {'amount': amount, 'currency': 'USD', 'actionType': action_type.upper()}, 'data': payload_data, 'customerIpAddress': '127.0.0.1', 'sourceObject': {'__typename': 'Checkout', 'id': checkout_id, 'totalPrice': {'gross': {'amount': 0.0}}, 'user': {'id': graphene.Node.to_global_id('User', customer_user.pk)}}}

def test_transaction_initialize_session_checkout_without_data(checkout, webhook_app, permission_manage_payments, transaction_item_generator):
    if False:
        while True:
            i = 10
    webhook_app.permissions.add(permission_manage_payments)
...
def test_transaction_initialize_session_order_with_data(order, webhook_app, permission_manage_payments, transaction_item_generator):
    if False:
        for i in range(10):
            print('nop')
    webhook_app.permissions.add(permission_manage_payments)
...
def test_transaction_initialize_session_order_without_data(order, webhook_app, permission_manage_payments, transaction_item_generator):
    if False:
        i = 10
        return i + 15
    webhook_app.permissions.add(permission_manage_payments)
...
def test_transaction_initialize_session_empty_customer_ip_addess(order, webhook_app, permission_manage_payments, transaction_item_generator):
    if False:
        i = 10
        return i + 15
    webhook_app.permissions.add(permission_manage_payments)
