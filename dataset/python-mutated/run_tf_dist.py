...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.getLevelName('INFO'), handlers=[logging.StreamHandler(sys.stdout)], format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    if SDP_ENABLED:
        sdp.init()
    ...
    (tf_train_dataset, tf_test_dataset) = get_datasets(tokenizer=tokenizer, train_batch_size=args.per_device_train_batch_size, eval_batch_size=args.per_device_eval_batch_size)
    ...
    if args.do_train:
        ...
        train_results = fit(model, loss, optimizer, tf_train_dataset, args.epochs, args.per_device_train_batch_size, max_steps=args.max_steps)
