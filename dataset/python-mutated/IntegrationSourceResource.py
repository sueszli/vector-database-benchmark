...
class IntegrationSourceResource(GenericResource):
    ...
    async def collection(self, query, meta, user, **kwargs):
        ...
        return self.build_result_set(collection, user, **kwargs)
    ...
    def create(self, payload, user, **kwargs):
        ...
        if 'test_connection' == action_type:
            ...
            pipeline_uuid = payload['pipeline_uuid']
            pipeline = IntegrationPipeline.get(pipeline_uuid)
            streams_updated = pipeline.preview_data(BlockType.DATA_LOADER, streams=payload.get('streams'))
            streams = list(streams_updated)
            success = True
        return self(dict(error_message=error_message, streams=streams, success=success), user, **kwargs)
