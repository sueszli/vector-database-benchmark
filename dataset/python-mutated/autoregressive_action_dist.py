...
from ray.rllib.examples.models.autoregressive_action_model import AutoregressiveActionModel, TorchAutoregressiveActionModel
from ray.rllib.examples.models.autoregressive_action_dist import BinaryAutoregressiveDistribution, TorchBinaryAutoregressiveDistribution
...
def get_cli_args():
    ...
    parser.add_argument('--no-autoreg', action='store_true', help='Do NOT use an autoregressive action distribution but normal,independently distributed actions.')
    ...
    parser.add_argument('--stop-reward', type=float, default=200.0, help='Reward at which we stop training.')
...
if __name__ == '__main__':
    ...
    ModelCatalog.register_custom_model('autoregressive_model', TorchAutoregressiveActionModel if args.framework == 'torch' else AutoregressiveActionModel)
    ModelCatalog.register_custom_action_dist('binary_autoreg_dist', TorchBinaryAutoregressiveDistribution if args.framework == 'torch' else BinaryAutoregressiveDistribution)
    config = get_trainable_cls(args.run).get_default_config().experimental(_enable_new_api_stack=False).environment(CorrelatedActionsEnv).framework(args.framework).training(gamma=0.5).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    if not args.no_autoreg:
        config.model.update({'custom_model': 'autoregressive_model', 'custom_action_dist': 'binary_autoreg_dist'})
    ...
    if args.no_tune:
        ...
        tuner = tune.Tuner(args.run, run_config=air.RunConfig(stop=stop, verbose=2), param_space=config)
        results = tuner.fit()
        if args.as_test:
