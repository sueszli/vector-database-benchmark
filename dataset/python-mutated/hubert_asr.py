...
class HubertAsrConfig(FairseqDataclass):
    ...
    dropout: float = field(default=0.0, metadata={'help': 'dropout probability inside hubert model'})
    attention_dropout: float = field(default=0.0, metadata={'help': 'dropout probability for attention weights inside hubert model'})
    activation_dropout: float = field(default=0.0, metadata={'help': 'dropout probability after activation in FFN inside hubert model'})
    encoder_embed_dim: Optional[int] = field(default=768, metadata={'help': 'encoder embedding dimension'})
    ...
    mask_channel_other: float = field(default=0, metadata={'help': 'secondary mask argument (used for more complex distributions), see help in compute_mask_indices'})
    no_mask_channel_overlap: bool = field(default=False, metadata={'help': 'whether to allow channel masks to overlap'})
    freeze_finetune_updates: int = field(default=0, metadata={'help': 'dont finetune hubert for this many updates'})
    feature_grad_mult: float = field(default=0.0, metadata={'help': 'reset feature grad mult in hubert to this'})
    layerdrop: float = field(default=0.0, metadata={'help': 'probability of dropping a layer in hubert'})
...
class HubertSeq2SeqConfig(HubertAsrConfig):
    ...
    seq2seq_path: str = field(default='', metadata={'help': 'reset_dict'})
    reset_dict: bool = field(default=False, metadata={'help': 'reset_dict'})
