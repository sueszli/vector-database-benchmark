...
class SendConfirmationEmail(BaseMutation):
    ...
    class Arguments:
        redirect_url = graphene.String(required=True, description='Base of frontend URL that will be needed to create confirmation URL.')
        channel = graphene.String(required=True, description='Slug of a channel which will be used for notify user.')

    class Meta:
        description = 'Sends a notification confirmation.' + ADDED_IN_315 + PREVIEW_FEATURE
        doc_category = DOC_CATEGORY_USERS
        error_type_class = SendConfirmationEmailError
        permissions = (AuthorizationFilters.AUTHENTICATED_USER,)
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.NOTIFY_USER, description='A notification for account confirmation.'), WebhookEventInfo(type=WebhookEventAsyncType.ACCOUNT_CONFIRMATION_REQUESTED, description='An account confirmation was requested. This event is always sent regardless of settings.')]
