...
from tribler.core.components.restapi.rest.rest_endpoint import HTTP_INTERNAL_SERVER_ERROR, HTTP_NOT_FOUND, HTTP_UNAUTHORIZED, HTTP_REQUEST_ENTITY_TOO_LARGE, MAX_REQUEST_SIZE, RESTResponse
...
async def error_middleware(request, handler):
    try:
        ...
        return RESTResponse({'error': {'handled': True, 'message': f'Could not find {request.path}'}}, status=HTTP_NOT_FOUND)
    except HTTPRequestEntityTooLarge as http_error:
        return RESTResponse({'error': {'handled': True, 'message': http_error.text}}, status=HTTP_REQUEST_ENTITY_TOO_LARGE)
    except Exception as e:
        logger.exception(e)
        full_exception = traceback.format_exc()
        default_core_exception_handler.unhandled_error_observer(None, {'exception': e, 'should_stop': False})
        return RESTResponse({'error': {'handled': False, 'code': e.__class__.__name__, 'message': str(full_exception)}}, status=HTTP_INTERNAL_SERVER_ERROR)
    return response
...
class RESTManager:
    ...
    async def start(self):
        ...
        aiohttp_apispec = AiohttpApiSpec(url='/docs/swagger.json', app=self.root_endpoint.app, title='Tribler REST API documentation', version=version_id, swagger_path='/docs')
        if self.config.key:
    ...
    async def start_http_site(self):
        api_port = max(self.config.http_port, 0)
