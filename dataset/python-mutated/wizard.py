...
class l10n_eu_service(models.TransientModel):
    ...
    fiscal_position_id = fields.Many2one('account.fiscal.position', string='Fiscal Position', default=_default_fiscal_position_id, help='Optional fiscal position to use as template for general account mapping. Should usually be your current Intra-EU B2B fiscal position. If not set, no general account mapping will be configured for EU fiscal positions.')
    tax_id = fields.Many2one('account.tax', string='Service VAT', required=True, default=_default_tax_id, help='Select your current VAT tax for services. This is the tax that will be mapped to the corresponding VAT tax in each EU country selected below.')
    account_collected_id = fields.Many2one('account.account', string='Tax Collection Account', help='Optional account to use for collecting tax amounts when selling services in each EU country selected below. If not set, the current collecting account of your Service VAT will be used.')
    done_country_ids = fields.Many2many('res.country', 'l10n_eu_service_country_rel_done', default=_default_done_country_ids, string='Already Supported')
    todo_country_ids = fields.Many2many('res.country', 'l10n_eu_service_country_rel_todo', default=_default_todo_country_ids, string='EU Customers From', required=True)
