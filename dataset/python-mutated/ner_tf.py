...
for i in range(epochs):
    ...
    for j in range(n_batches):
        ...
        (c, p, _) = sess.run((cost_op, predict_op, train_op), feed_dict={inputs: x, targets: y})
        cost += c
        for (yi, pi) in zip(y, p):
            ...
            n_total += len(yii)
        if j % 10 == 0:
            sys.stdout.write('j/N: %d/%d correct rate so far: %f, cost so far: %f\r' % (j, n_batches, float(n_correct) / n_total, cost))
            sys.stdout.flush()
    ...
    print('i:', i, 'cost:', '%.4f' % cost, 'train acc:', '%.4f' % (float(n_correct) / n_total), 'test acc:', '%.4f' % test_acc, 'time for epoch:', datetime.now() - t0)
    costs.append(cost)
plt.plot(costs)
plt.show()