...
def start_local_trainers_cpu(trainer_endpoints, training_script, training_script_args, log_dir=None):
    ...
    for (rank_id, endpoint) in enumerate(trainer_endpoints):
        proc_env = {'PADDLE_DISTRI_BACKEND': 'gloo', 'PADDLE_TRAINER_ID': '%d' % rank_id, 'PADDLE_CURRENT_ENDPOINT': '%s' % endpoint, 'PADDLE_TRAINERS_NUM': '%d' % n_rank, 'PADDLE_TRAINER_ENDPOINTS': ','.join(trainer_endpoints)}
...
def start_local_trainers(cluster, pod, training_script, training_script_args, allocator_strategy='auto_growth', log_dir=None, need_envs={}):
    if False:
        while True:
            i = 10
    current_env = copy.copy(os.environ.copy())
    ...
    for t in pod.trainers:
        proc_env = {'FLAGS_selected_gpus': '%s' % ','.join([str(g) for g in t.gpus]), 'PADDLE_TRAINER_ID': '%d' % t.rank, 'PADDLE_CURRENT_ENDPOINT': '%s' % t.endpoint, 'PADDLE_TRAINERS_NUM': '%d' % cluster.trainers_nranks(), 'PADDLE_TRAINER_ENDPOINTS': ','.join(cluster.trainers_endpoints()), 'FLAGS_dynamic_static_unified_comm': '0'}
...
class TestMultipleGpus(unittest.TestCase):
    ...
    def run_mnist_2gpu(self, target_file_name, allocator_strategy='auto_growth', need_envs={}):
        if False:
            for i in range(10):
                print('nop')
        ...
        procs = start_local_trainers(cluster, pod, allocator_strategy=allocator_strategy, training_script=target_file_name, training_script_args=[], need_envs=need_envs)
        while True:
...
class TestMultipleWithGloo(unittest.TestCase):
    ...
    def run_mnist_2cpu(self, target_file_name):
        if False:
            for i in range(10):
                print('nop')
        (cluster, pod) = get_cluster_from_args([0, 1])
        procs = start_local_trainers_cpu(cluster.trainers_endpoints(), training_script=target_file_name, training_script_args=[])
        while True:
...
class TestDataParallelWithPyLayer(TestMultipleGpus):
    ...
    def test_parallel_dygraph_dataparallel_with_pylayer(self):
        ...
        self.run_mnist_2gpu('parallel_dygraph_dataparallel_with_pylayer.py', allocator_strategy='naive_best_fit')
