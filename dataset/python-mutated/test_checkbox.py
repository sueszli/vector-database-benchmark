...
class CheckboxApp(App[None]):
    ...
    def on_checkbox_changed(self, event: Checkbox.Changed) -> None:
        if False:
            for i in range(10):
                print('nop')
        self.events_received.append((event.checkbox.id, event.checkbox.value, event.checkbox == event.control))
...
async def test_checkbox_initial_state() -> None:
    ...
    async with CheckboxApp().run_test() as pilot:
        ...
        assert [box.has_class('-on') for box in pilot.app.query(Checkbox)] == [False, False, True]
...
async def test_checkbox_toggle() -> None:
    ...
    async with CheckboxApp().run_test() as pilot:
        ...
        assert [box.has_class('-on') for box in pilot.app.query(Checkbox)] == [True, True, False]
        await pilot.pause()
        assert pilot.app.events_received == [('cb1', True, True), ('cb2', True, True), ('cb3', False, True)]