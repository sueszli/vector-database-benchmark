...
def passthrough(x):
    if False:
        while True:
            i = 10
    return x
topics_to_send_to_gui = [notifications.tunnel_removed, notifications.watch_folder_corrupt_file, notifications.tribler_new_version, notifications.channel_discovered, notifications.torrent_finished, notifications.channel_entity_updated, notifications.tribler_shutdown_state, notifications.remote_query_results, notifications.low_space, notifications.report_config_error]
...
class EventsEndpoint(RESTEndpoint):
    ...
    @docs(tags=['General'], summary='Open an EventStream for receiving Tribler events.', responses={200: {'schema': schema(EventsResponse={'type': marshmallow.fields.String, 'event': marshmallow.fields.Dict})}})
    async def get_events(self, request):
        ...
        """
        response = RESTStreamResponse(status=200, reason='OK', headers={'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', 'Connection': 'keep-alive'})
