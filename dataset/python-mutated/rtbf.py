...
class RTBF(Plugin):
    ...
    _stream_size_re = re.compile('https?://.+-(?P<size>\\d+p?)\\..+?$')
    _video_player_re = re.compile('<iframe\\s+class="embed-responsive-item\\s+js-embed-iframe".*src="(?P<player_url>.+?)".*?</iframe>', re.DOTALL)
    _video_stream_data_re = re.compile('<div\\s+id="js-embed-player"\\s+class="js-embed-player\\s+embed-player"\\s+data-media="(.+?)"')
    _radio_id_re = re.compile('var currentStationKey = "(?P<radio_id>.+?)"')
    _geo_schema = validate.Schema({'country': str, 'zone': str})
    _video_stream_schema = validate.Schema(validate.transform(_video_stream_data_re.search), validate.any(None, validate.all(validate.get(1), validate.transform(html_unescape), validate.parse_json(), {'geoLocRestriction': str, validate.optional('isLive'): bool, validate.optional('startDate'): str, validate.optional('endDate'): str, 'sources': validate.any([], validate.Schema({str: validate.any(None, '', validate.url())})), validate.optional('urlHls'): validate.any(None, '', validate.url()), validate.optional('urlDash'): validate.any(None, '', validate.url()), validate.optional('streamUrlHls'): validate.any(None, '', validate.url()), validate.optional('streamUrlDash'): validate.any(None, '', validate.url()), validate.optional('drm'): bool})))
    _radio_stream_schema = validate.Schema({'audioUrls': validate.all([{'url': validate.url(), 'mimeType': str}])})
