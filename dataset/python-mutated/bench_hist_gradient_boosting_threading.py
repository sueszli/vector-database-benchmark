...
parser.add_argument('--n-samples', type=int, default=int(1000000.0))
...
sklearn_est = Estimator(learning_rate=lr, max_iter=n_trees, max_bins=max_bins, max_leaf_nodes=n_leaf_nodes, early_stopping=False, random_state=0, verbose=0)
loss = args.loss
if args.problem == 'classification':
    if loss == 'default':
        loss = 'log_loss'
...
for n_threads in n_threads_list:
    ...
    (sklearn_score, sklearn_fit_duration, sklearn_score_duration, lightgbm_score, lightgbm_fit_duration, lightgbm_score_duration, xgb_score, xgb_fit_duration, xgb_score_duration, cat_score, cat_fit_duration, cat_score_duration) = one_run(n_threads, n_samples)
