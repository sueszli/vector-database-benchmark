...
if __name__ == '__main__':
    ...
    parser.add_argument('--address', required=False, type=str, help='The address to use for Redis.')
    ...
    parser.add_argument('--data-dir', required=False, type=str, default='~/data', help='Root directory for storing downloaded dataset.')
    parser.add_argument('--synch', action='store_true', default=False, help='Use synchronous PBT.')
    (args, _) = parser.parse_known_args()
    if args.smoke_test:
        ray.init(num_cpus=4)
    else:
        ray.init(address=args.address)
    trainer = TorchTrainer(train_func, scaling_config=ScalingConfig(num_workers=args.num_workers, use_gpu=args.use_gpu))
    pbt_scheduler = PopulationBasedTraining(time_attr='training_iteration', perturbation_interval=1, hyperparam_mutations={'train_loop_config': {'lr': tune.loguniform(0.001, 0.1), 'momentum': [0.8, 0.9, 0.99]}}, synch=args.synch)
    tuner = Tuner(trainer, param_space={'train_loop_config': {'lr': tune.grid_search([0.001, 0.01, 0.05, 0.1]), 'momentum': 0.8, 'batch_size': 128 * args.num_workers, 'test_mode': args.smoke_test, 'data_dir': args.data_dir, 'epochs': args.num_epochs}}, tune_config=TuneConfig(num_samples=1, metric='loss', mode='min', scheduler=pbt_scheduler), run_config=RunConfig(stop={'training_iteration': 3 if args.smoke_test else args.num_epochs}, failure_config=FailureConfig(max_failures=3)))
    results = tuner.fit()
