...
class webriskCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'compute_threat_list_diff': ('threat_type', 'constraints', 'version_token'), 'create_submission': ('parent', 'submission'), 'search_hashes': ('threat_types', 'hash_prefix'), 'search_uris': ('uri', 'threat_types'), 'submit_uri': ('parent', 'submission', 'threat_info', 'threat_discovery')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=webriskCallTransformer()):
    if False:
        i = 10
        return i + 15
