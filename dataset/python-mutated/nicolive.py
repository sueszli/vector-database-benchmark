...
@pluginmatcher(re.compile('https?://(?P<domain>live\\d*\\.nicovideo\\.jp)/watch/(lv|co)\\d+'))
@pluginargument('email', sensitive=True, argument_name='niconico-email', metavar='EMAIL', help='The email or phone number associated with your Niconico account')
@pluginargument('password', sensitive=True, argument_name='niconico-password', metavar='PASSWORD', help='The password of your Niconico account')
@pluginargument('user-session', sensitive=True, argument_name='niconico-user-session', metavar='VALUE', help='\n        Value of the user-session token.\n\n        Can be used as an alternative to providing a password.\n    ')
@pluginargument('purge-credentials', argument_name='niconico-purge-credentials', action='store_true', help='Purge cached Niconico credentials to initiate a new session and reauthenticate.')
@pluginargument('timeshift-offset', type=hours_minutes_seconds, argument_name='niconico-timeshift-offset', metavar='[[XX:]XX:]XX | [XXh][XXm][XXs]', help='\n        Amount of time to skip from the beginning of a stream.\n\n        Default is 0.\n    ')
class NicoLive(Plugin):
    STREAM_READY_TIMEOUT = 6
    LOGIN_URL = 'https://account.nicovideo.jp/login/redirector'
    LOGIN_URL_PARAMS = {'show_button_twitter': 1, 'show_button_facebook': 1, 'next_url': '/'}
    wsclient: NicoLiveWsClient
