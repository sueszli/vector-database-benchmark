...
class EbookpointStore(BasicStoreConfig, StorePlugin):
    ...
    def search(self, query, max_results=25, timeout=60):
        if False:
            print('Hello World!')
        url = 'http://ebookpoint.pl/search?qa=&szukaj=' + quote_plus(query.decode('utf-8').encode('iso-8859-2')) + '&serwisyall=0&wprzyg=0&wsprzed=1&wyczerp=0&formaty=em-p'
        br = browser()
        counter = max_results
