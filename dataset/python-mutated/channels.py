...
from strawberry.channels import GraphQLHTTPConsumer, GraphQLWSConsumer, SyncGraphQLHTTPConsumer
...
class DebuggableGraphQLHTTPConsumer(GraphQLHTTPConsumer):
    ...
    async def process_result(self, request: ChannelsConsumer, result: Any) -> GraphQLHTTPResponse:
...
class ChannelsHttpClient(HttpClient):
    ...
    async def _graphql_request(self, method: Literal['get', 'post'], query: Optional[str]=None, variables: Optional[Dict[str, object]]=None, files: Optional[Dict[str, BytesIO]]=None, headers: Optional[Dict[str, str]]=None, **kwargs: Any) -> Response:
        ...
        return await self.request(url=endpoint_url, method=method, body=body, headers=headers)

    async def request(self, url: str, method: Literal['get', 'post', 'patch', 'put', 'delete'], body: bytes=b'', headers: Optional[Dict[str, str]]=None) -> Response:
        if headers:
            headers = [(k.encode(), v.encode()) for (k, v) in headers.items()]
        communicator = HttpCommunicator(self.http_app, method.upper(), url, body=body, headers=headers)
        response = await communicator.get_response()
        return Response(status_code=response['status'], data=response['body'], headers={k.decode(): v.decode() for (k, v) in response['headers']})
