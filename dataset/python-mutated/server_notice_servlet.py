...
class SendServerNoticeServlet(RestServlet):
    ...
    async def _do(self, request: SynapseRequest, requester: Requester, txn_id: Optional[str]) -> Tuple[int, JsonDict]:
        ...
        if not self.server_notices_manager.is_enabled():
            raise SynapseError(HTTPStatus.BAD_REQUEST, 'Server notices are not enabled on this server')
        target_user = UserID.from_string(body['user_id'])
        if not self.is_mine(target_user):
            raise SynapseError(HTTPStatus.BAD_REQUEST, 'Server notices can only be sent to local users')
        if not await self.admin_handler.get_user(target_user):
            raise NotFoundError('User not found')
        event = await self.server_notices_manager.send_notice(user_id=target_user.to_string(), type=event_type, state_key=state_key, event_content=body['content'], txn_id=txn_id)
    ...
    async def on_PUT(self, request: SynapseRequest, txn_id: str) -> Tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)
        set_tag('txn_id', txn_id)
        return await self.txns.fetch_or_execute_request(request, requester, self._do, request, requester, txn_id)