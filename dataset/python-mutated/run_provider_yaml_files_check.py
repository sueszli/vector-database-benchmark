...
DEPRECATED_MODULES = ['airflow.providers.apache.hdfs.sensors.hdfs', 'airflow.providers.apache.hdfs.hooks.hdfs', 'airflow.providers.cncf.kubernetes.triggers.kubernetes_pod', 'airflow.providers.cncf.kubernetes.operators.kubernetes_pod']
KNOWN_DEPRECATED_CLASSES = ['airflow.providers.google.cloud.links.dataproc.DataprocLink']
try:
    from yaml import CSafeLoader as SafeLoader
except ImportError:
    from yaml import SafeLoader
if __name__ != '__main__':
    raise Exception('This file is intended to be executed as an executable program. You cannot use it as a module.')
ROOT_DIR = pathlib.Path(__file__).resolve().parents[2]
DOCS_DIR = ROOT_DIR.joinpath('docs')
PROVIDER_DATA_SCHEMA_PATH = ROOT_DIR.joinpath('airflow', 'provider.yaml.schema.json')
PROVIDER_ISSUE_TEMPLATE_PATH = ROOT_DIR.joinpath('.github', 'ISSUE_TEMPLATE', 'airflow_providers_bug_report.yml')
CORE_INTEGRATIONS = ['SQL', 'Local']
errors: list[str] = []
console = Console(width=400, color_system='standard')
if os.environ.get('PYTHONWARNINGS') != 'default':
    console.print('[red]Error: PYTHONWARNINGS not set[/]\nYou must set `PYTHONWARNINGS=default` environment variable to run this script')
    sys.exit(1)
