...
class FulfillmentUpdateTracking(BaseMutation):
    fulfillment = graphene.Field(Fulfillment, description='A fulfillment with updated tracking.')
    order = graphene.Field(Order, description='Order for which fulfillment was updated.')

    class Arguments:
        id = graphene.ID(required=True, description='ID of a fulfillment to update.')
        input = FulfillmentUpdateTrackingInput(required=True, description='Fields required to update a fulfillment.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.FULFILLMENT_TRACKING_NUMBER_UPDATED, description='Fulfillment tracking number is updated.')]
