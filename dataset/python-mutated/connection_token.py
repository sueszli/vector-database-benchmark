...
__all__ = ['ConnectionTokenSerializer', 'SuperConnectionTokenSerializer', 'ConnectionTokenReusableSerializer']

class ConnectionTokenSerializer(CommonModelSerializer):
    expire_time = serializers.IntegerField(read_only=True, label=_('Expired time'))
    input_secret = EncryptedField(label=_('Input secret'), max_length=40960, required=False, allow_blank=True)
    ...
    class Meta:
        ...
        fields_small = fields_mini + ['user', 'asset', 'account', 'input_username', 'input_secret', 'connect_method', 'connect_options', 'protocol', 'actions', 'is_active', 'is_reusable', 'from_ticket', 'from_ticket_info', 'date_expired', 'date_created', 'date_updated', 'created_by', 'updated_by', 'org_id', 'org_name']
        read_only_fields = ['user', 'expire_time', 'is_expired', 'date_expired', 'user_display', 'asset_display']
        fields = fields_small + read_only_fields
        extra_kwargs = {'from_ticket': {'read_only': True}, 'value': {'read_only': True}, 'is_expired': {'read_only': True, 'label': _('Is expired')}}
