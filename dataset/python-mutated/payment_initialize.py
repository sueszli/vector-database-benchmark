...
class PaymentInitialize(BaseMutation):
    initialized_payment = graphene.Field(PaymentInitialized, required=False, description='Payment that was initialized.')

    class Arguments:
        gateway = graphene.String(description='A gateway name used to initialize the payment.', required=True)
        channel = graphene.String(description='Slug of a channel for which the data should be returned.')
        payment_data = JSONString(required=False, description='Client-side generated data required to initialize the payment.')
