...
def finetune_poetry(work_dir='./gpt3_poetry'):
    ...
    train_dataset = dataset_dict['train'].remap_columns({'text1': 'src_txt'}).select(range(32))
    eval_dataset = dataset_dict['test'].remap_columns({'text1': 'src_txt'}).select(range(32))
    max_epochs = 2
    tmp_dir = './gpt3_poetry'
    ...
    def cfg_modify_fn(cfg):
        ...
        return cfg
    kwargs = dict(model='damo/nlp_gpt3_text-generation_chinese-base', train_dataset=train_dataset, eval_dataset=eval_dataset, max_epochs=max_epochs, work_dir=tmp_dir, cfg_modify_fn=cfg_modify_fn, launcher='pytorch')
...
def evaluate_poetry(work_dir='./gpt3_poetry'):
    ...
    tmp_dir = './gpt3_poetry'
    kwargs = dict(model=f'{tmp_dir}/output', eval_dataset=eval_dataset, work_dir=tmp_dir, launcher='pytorch')
...
def predict_poetry(work_dir='./gpt3_poetry'):
    ...
    eval_dataset = dataset_dict['test'].remap_columns({'text1': 'src_txt'}).select(range(20))
    tmp_dir = './gpt3_poetry'
    kwargs = dict(model=f'{tmp_dir}/output', predict_datasets=eval_dataset, work_dir=tmp_dir, launcher='pytorch')
    trainer = build_trainer(default_args=kwargs)
    trainer.predict()
