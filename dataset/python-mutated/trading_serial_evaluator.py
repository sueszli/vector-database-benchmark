...
class TradingSerialEvaluator(InteractionSerialEvaluator):
    ...
    Property:
        env, policy
    """
    config = dict(eval_freq=1000, render=dict(render_freq=-1, mode='train_iter'), type='trading_interaction')
...
class TradingEvalMonitor(VectorEvalMonitor):
    ...
    Interfaces:
        Besides (__init__, is_finished, update_info, update_reward, get_episode_return,            get_latest_reward, get_current_episode, get_episode_info), there are                (update_max_profit, get_max_episode_profit).
    """
