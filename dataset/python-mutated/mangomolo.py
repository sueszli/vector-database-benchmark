...
class MangomoloBaseIE(InfoExtractor):
    _BASE_REGEX = '(?:https?:)?//(?:admin\\.mangomolo\\.com/analytics/index\\.php/customers/embed/|player\\.mangomolo\\.com/v1/)'
    _SLUG = None
...
class MangomoloVideoIE(MangomoloBaseIE):
    ...
    _SLUG = 'video\\?.*?\\bid=(?P<id>\\d+)'
    _IS_LIVE = False

class MangomoloLiveIE(MangomoloBaseIE):
    _TYPE = 'live'
    IE_NAME = 'mangomolo:' + _TYPE
    _SLUG = '(?:live|index)\\?.*?\\bchannelid=(?P<id>(?:[A-Za-z0-9+/=]|%2B|%2F|%3D)+)'
    _IS_LIVE = True
