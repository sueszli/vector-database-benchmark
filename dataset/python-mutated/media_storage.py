...
from typing import IO, TYPE_CHECKING, Any, Awaitable, BinaryIO, Callable, Generator, Optional, Sequence, Tuple, Type
import attr
...
class MediaStorage:
    ...
    async def fetch_media(self, file_info: FileInfo) -> Optional[Responder]:
        ...
        if file_info.thumbnail and file_info.server_name:
            paths.append(self.filepaths.remote_media_thumbnail_rel_legacy(server_name=file_info.server_name, file_id=file_info.file_id, width=file_info.thumbnail.width, height=file_info.thumbnail.height, content_type=file_info.thumbnail.type))
        for path in paths:
    ...
    async def ensure_media_is_in_local_cache(self, file_info: FileInfo) -> str:
        ...
        if os.path.exists(local_path):
            return local_path
        if file_info.thumbnail and file_info.server_name:
            legacy_path = self.filepaths.remote_media_thumbnail_rel_legacy(server_name=file_info.server_name, file_id=file_info.file_id, width=file_info.thumbnail.width, height=file_info.thumbnail.height, content_type=file_info.thumbnail.type)
        ...
        for provider in self.storage_providers:
            ...
            if res:
                with res:
                    consumer = BackgroundFileConsumer(open(local_path, 'wb'), self.reactor)
