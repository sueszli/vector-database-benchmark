...
def set_order_voucher_code(apps, schema_editor):
    ...
    orders = Order.objects.filter(voucher__isnull=False, voucher_code__isnull=True).order_by('pk')
...
class Migration(migrations.Migration):
    dependencies = [('discount', '0056_voucher_code_indexes'), ('order', '0176_order_voucher_code_add_index')]
    run_before = [('discount', '0066_clear_voucher_and_vouchercustomer')]
    operations = [migrations.RunPython(set_order_voucher_code, migrations.RunPython.noop)]