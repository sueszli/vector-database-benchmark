...
class PyCmaSampler(BaseSampler):
    ...
    .. _cma.CMAEvolutionStrategy: https://cma-es.github.io/apidocs-pycma/    cma.evolution_strategy.CMAEvolutionStrategy.html
    .. _cma_default_options: https://cma-es.github.io/apidocs-pycma/    cma.evolution_strategy.html#cma_default_options_
    """

    def __init__(self, x0: Optional[Dict[str, Any]]=None, sigma0: Optional[float]=None, cma_stds: Optional[Dict[str, float]]=None, seed: Optional[int]=None, cma_opts: Optional[Dict[str, Any]]=None, n_startup_trials: int=1, independent_sampler: Optional[BaseSampler]=None, warn_independent_sampling: bool=True) -> None:
        if False:
            for i in range(10):
                print('nop')
        _imports.check()
    ...
    def sample_relative(self, study: Study, trial: FrozenTrial, search_space: Dict[str, BaseDistribution]) -> Dict[str, float]:
        ...
        if len(search_space) == 1:
            _logger.info('`PyCmaSampler` does not support optimization of 1-D search space. `{}` is used instead of `PyCmaSampler`.'.format(self._independent_sampler.__class__.__name__))
...
class _Optimizer:
    ...
    def __init__(self, search_space: Dict[str, BaseDistribution], x0: Dict[str, Any], sigma0: float, cma_stds: Optional[Dict[str, float]], cma_opts: Dict[str, Any]) -> None:
        if False:
            while True:
                i = 10
        ...
        initial_cma_params = []
        for param_name in self._param_names:
            initial_cma_params.append(self._to_cma_params(self._search_space, param_name, x0[param_name]))
        cma_option = {'BoundaryHandler': cma.BoundTransform, 'bounds': [lows, highs]}
        if cma_stds:
    ...
    def tell(self, trials: List[FrozenTrial], study_direction: StudyDirection) -> int:
        ...
        for i in range(generation):
            ...
            for t in complete_trials[i * popsize:(i + 1) * popsize]:
                x = [self._to_cma_params(self._search_space, name, t.params[name]) for name in self._param_names]
                xs.append(x)
                ys.append(t.value)
    ...
    def _collect_target_trials(self, trials: List[FrozenTrial], last_told: int=-1, target_states: Optional[Container[TrialState]]=None) -> List[FrozenTrial]:
        if False:
            while True:
                i = 10
...
class CmaEsSampler(PyCmaSampler):
    ...
    def __init__(self, x0: Optional[Dict[str, Any]]=None, sigma0: Optional[float]=None, cma_stds: Optional[Dict[str, float]]=None, seed: Optional[int]=None, cma_opts: Optional[Dict[str, Any]]=None, n_startup_trials: int=1, independent_sampler: Optional[BaseSampler]=None, warn_independent_sampling: bool=True) -> None:
        if False:
            for i in range(10):
                print('nop')
        super().__init__(x0=x0, sigma0=sigma0, cma_stds=cma_stds, seed=seed, cma_opts=cma_opts, n_startup_trials=n_startup_trials, independent_sampler=independent_sampler, warn_independent_sampling=warn_independent_sampling)