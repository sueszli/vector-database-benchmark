...
from ..utils import ExtractorError, InAdvancePagedList, int_or_none, remove_start, traverse_obj, update_url_query, url_or_none

class VideoKenBaseIE(InfoExtractor):
    _ORGANIZATIONS = {'videos.icts.res.in': 'icts', 'videos.cncf.io': 'cncf', 'videos.neurips.cc': 'neurips'}
    _BASE_URL_RE = f"https?://(?P<host>{'|'.join(map(re.escape, _ORGANIZATIONS))})/"
    _PAGE_SIZE = 12
...
class VideoKenPlayerIE(VideoKenBaseIE):
    ...
    _TESTS = [{'url': 'https://player.videoken.com/embed/slideslive-38968434', 'info_dict': {'id': '38968434', 'ext': 'mp4', 'title': 'Deep Learning with Label Differential Privacy', 'timestamp': 1643377020, 'upload_date': '20220128', 'thumbnail': 're:^https?://.*\\.(?:jpg|png)', 'thumbnails': 'count:30', 'chapters': 'count:29'}, 'params': {'skip_download': 'm3u8'}}]
...
class VideoKenPlaylistIE(VideoKenBaseIE):
    _VALID_URL = VideoKenBaseIE._BASE_URL_RE + '(?:category/\\d+/)?playlist/(?P<id>\\d+)'
    _TESTS = [{'url': 'https://videos.icts.res.in/category/1822/playlist/381', 'playlist_mincount': 117, 'info_dict': {'id': '381', 'title': 'Cosmology - The Next Decade'}}]
...
class VideoKenCategoryIE(VideoKenBaseIE):
    _VALID_URL = VideoKenBaseIE._BASE_URL_RE + 'category/(?P<id>\\d+)/?(?:$|[?#])'
    _TESTS = [{'url': 'https://videos.icts.res.in/category/1822/', 'playlist_mincount': 500, 'info_dict': {'id': '1822', 'title': 'Programs'}}, {'url': 'https://videos.neurips.cc/category/350/', 'playlist_mincount': 34, 'info_dict': {'id': '350', 'title': 'NeurIPS 2018'}}, {'url': 'https://videos.cncf.io/category/479/', 'playlist_mincount': 328, 'info_dict': {'id': '479', 'title': "KubeCon + CloudNativeCon Europe'19"}}]
...
class VideoKenTopicIE(VideoKenBaseIE):
    _VALID_URL = VideoKenBaseIE._BASE_URL_RE + 'topic/(?P<id>[^/#?]+)/?(?:$|[?#])'
    _TESTS = [{'url': 'https://videos.neurips.cc/topic/machine%20learning/', 'playlist_mincount': 500, 'info_dict': {'id': 'machine_learning', 'title': 'machine learning'}}, {'url': 'https://videos.icts.res.in/topic/gravitational%20waves/', 'playlist_mincount': 77, 'info_dict': {'id': 'gravitational_waves', 'title': 'gravitational waves'}}, {'url': 'https://videos.cncf.io/topic/prometheus/', 'playlist_mincount': 134, 'info_dict': {'id': 'prometheus', 'title': 'prometheus'}}]
