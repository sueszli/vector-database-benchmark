...
from typing import Any, Iterator, List, Mapping, MutableMapping, NamedTuple, Optional, Set, cast
import opentracing
...
class Recovery(Service):
    ...
    async def on_rebalance(self, assigned: Set[TP], revoked: Set[TP], newly_assigned: Set[TP]) -> None:
        ...
        for tp in assigned_actives:
            ...
            await asyncio.sleep(0)
        await asyncio.sleep(0)
        active_offsets = {tp: offset for (tp, offset) in self.active_offsets.items() if tp in self.active_tps}
        ...
        if app.tracer and rebalancing_span:
            self._recovery_span = app.tracer.get_tracer('_faust').start_span('recovery', child_of=rebalancing_span)
    ...
    async def _restart_recovery(self) -> None:
        ...
        while not self.should_stop:
            ...
            if tracer is not None and self._recovery_span:
                span = tracer.start_span('recovery-thread', child_of=self._recovery_span)
            ...
            try:
                ...
                await self._wait(T(self._build_highwaters)(consumer, assigned_active_tps, active_highwaters, 'active'))
                self.log.dev('Build offsets for active partitions')
                await self._wait(T(self._build_offsets)(consumer, assigned_active_tps, active_offsets, 'active'))
                for tp in assigned_active_tps:
                    if active_offsets[tp] > active_highwaters[tp]:
                        raise ConsistencyError(E_PERSISTED_OFFSET.format(tp, active_offsets[tp], active_highwaters[tp]))
                self.log.dev('Build offsets for standby partitions')
                await self._wait(T(self._build_offsets)(consumer, assigned_standby_tps, standby_offsets, 'standby'))
                self.log.dev('Seek offsets for active partitions')
                await self._wait(T(self._seek_offsets)(consumer, assigned_active_tps, active_offsets, 'active'))
                if self.need_recovery():
                    ...
                    if tracer is not None and span:
                        self._actives_span = tracer.start_span('recovery-actives', child_of=span, tags={'Active-Stats': self.active_stats()})
                        self.app._span_add_default_tags(span)
                    try:
                ...
                if standby_tps:
                    ...
                    await self._wait(T(self._seek_offsets)(consumer, standby_tps, standby_offsets, 'standby'))
                    self.log.dev('Build standby highwaters')
                    await self._wait(T(self._build_highwaters)(consumer, standby_tps, standby_highwaters, 'standby'))
                    for tp in standby_tps:
                        if standby_offsets[tp] > standby_highwaters[tp]:
                            raise ConsistencyError(E_PERSISTED_OFFSET.format(tp, standby_offsets[tp], standby_highwaters[tp]))
                    if tracer is not None and span:
                        self._standbys_span = tracer.start_span('recovery-standbys', child_of=span, tags={'Standby-Stats': self.standby_stats()})
                        self.app._span_add_default_tags(span)
                    self.log.dev('Resume standby partitions')
                    T(consumer.resume_partitions)(standby_tps)
                await self._wait(asyncio.sleep(0.1))
    ...
    def _estimated_active_remaining_secs(self, remaining: float) -> Optional[float]:
        ...
        if len(processing_times) >= self.num_samples_required_for_estimate:
            ...
            return mean_time * remaining * 1.1
        else:
            return None

    async def _wait(self, coro: WaitArgT) -> None:
        wait_result = await self.wait_first(coro, self.signal_recovery_reset, self.signal_recovery_start)
    ...
    async def on_recovery_completed(self) -> None:
        ...
        callback_coros = [table.on_recovery_completed(self.actives_for_table[table], self.standbys_for_table[table]) for table in self.tables.values()]
        if callback_coros:
        ...
        self.completed.set()
        self.log.dev('Resume stream partitions')
        consumer.resume_partitions({tp for tp in assignment if not self._is_changelog_tp(tp)})
    ...
    async def _build_highwaters(self, consumer: ConsumerT, tps: Set[TP], destination: Counter[TP], title: str) -> None:
        highwaters = await consumer.highwaters(*tps)
        highwaters = {tp: value - 1 if value is not None else -1 for (tp, value) in highwaters.items()}
        self.log.info('Highwater for %s changelog partitions:\n%s', title, self._highwater_logtable(highwaters, title=title))
        destination.clear()
    ...
    def _highwater_logtable(self, highwaters: Mapping[TP, int], *, title: str) -> str:
        if False:
            for i in range(10):
                print('nop')
        table_data = [[k.topic, str(k.partition), str(v)] for (k, v) in sorted(highwaters.items())]
        return terminal.logtable(list(self._consolidate_table_keys(table_data)), title=f'Highwater - {title.capitalize()}', headers=['topic', 'partition', 'highwater'])
    ...
    async def _build_offsets(self, consumer: ConsumerT, tps: Set[TP], destination: Counter[TP], title: str) -> None:
        ...
        if destination:
            self.log.info('%s offsets at start of reading:\n%s', title, self._start_offsets_logtable(destination, title=title))

    def _start_offsets_logtable(self, offsets: Mapping[TP, int], *, title: str) -> str:
        if False:
            print('Hello World!')
        table_data = [[k.topic, str(k.partition), str(v)] for (k, v) in sorted(offsets.items())]
        return terminal.logtable(list(self._consolidate_table_keys(table_data)), title=f'Reading Starts At - {title.capitalize()}', headers=['topic', 'partition', 'offset'])

    async def _seek_offsets(self, consumer: ConsumerT, tps: Set[TP], offsets: Counter[TP], title: str) -> None:
        new_offsets = {}
        for tp in tps:
            offset = offsets[tp]
            if offset == -1:
    ...
    async def _slurp_changelogs(self) -> None:
        ...
        while not self.should_stop:
            try:
                event: EventT = await asyncio.wait_for(changelog_queue.get(), timeout=5.0)
    ...
    def active_remaining(self) -> Counter[TP]:
        ...
        return Counter({tp: highwater - offsets[tp] for (tp, highwater) in highwaters.items() if highwater is not None and offsets[tp] is not None})
    ...
    def standby_remaining(self) -> Counter[TP]:
        ...
        return Counter({tp: highwater - offsets[tp] for (tp, highwater) in highwaters.items() if highwater >= 0 and offsets[tp] >= 0})
    ...
    def active_stats(self) -> RecoveryStatsMapping:
        ...
        offsets = self.active_offsets
        return {tp: RecoveryStats(highwater, offsets[tp], highwater - offsets[tp]) for (tp, highwater) in self.active_highwaters.items() if offsets[tp] is not None and highwater - offsets[tp] != 0}
    ...
    def standby_stats(self) -> RecoveryStatsMapping:
        ...
        offsets = self.standby_offsets
        return {tp: RecoveryStats(highwater, offsets[tp], highwater - offsets[tp]) for (tp, highwater) in self.standby_highwaters.items() if offsets[tp] is not None and highwater - offsets[tp] != 0}

    def _stats_to_logtable(self, title: str, stats: RecoveryStatsMapping) -> str:
        if False:
            print('Hello World!')
        table_data = [list(map(str, [tp.topic, tp.partition, s.highwater, s.offset, s.remaining])) for (tp, s) in sorted(stats.items())]
        return terminal.logtable(list(self._consolidate_table_keys(table_data)), title=title, headers=['topic', 'partition', 'need offset', 'have offset', 'remaining'])
        ...
        async for sleep_time in self.itertimer(interval, name='Recovery.stats'):
            if self.in_recovery:
                ...
                if stats and num_samples >= self.num_samples_required_for_estimate:
                    ...
                    self.log.info('Still fetching changelog topics for recovery, estimated time remaining %s (total remaining=%r):\n%s', self.active_remaining_seconds(remaining_total), remaining_total, self._stats_to_logtable('Remaining for active recovery', stats))
                elif stats:
                    await self._verify_remaining(now, stats)
                else:
                    recovery_started_at = self._recovery_started_at
                    if recovery_started_at is None:
                        self.log.error('POSSIBLE INTERNAL ERROR: Recovery marked as started but missing self._recovery_started_at timestamp.')
                    else:
                        secs_since_started = now - recovery_started_at
                        if secs_since_started >= 30.0:
                            self.log.error('POSSIBLE INTERNAL ERROR: Recovery has no remaining offsets to fetch, but we have spent %s waiting for the worker to transition out of recovery state...', humanize_seconds(secs_since_started))
    ...
    async def _verify_remaining(self, now: float, stats: RecoveryStatsMapping) -> None:
        ...
        if last_flush_at is None:
            if secs_since_started >= self.flush_timeout_secs:
                self.log.warning('Recovery has not flushed buffers since recovery startted (started %s). Current total buffer size: %r', humanize_seconds_ago(secs_since_started), self._current_total_buffer_size())
        else:
            secs_since_last_flush = now - last_flush_at
            if secs_since_last_flush >= self.flush_timeout_secs:
                self.log.warning('Recovery has not flushed buffers in the last %r seconds (last flush was %s). Current total buffer size: %r', self.flush_timeout_secs, humanize_seconds_ago(secs_since_last_flush), self._current_total_buffer_size())
        for tp in stats:
            await self.sleep(0)
            if self.should_stop:
                break
            ...
            if last_event_received is None:
                if secs_since_started >= self.event_timeout_secs:
                    self.log.warning('No event received for active tp %r since recovery start (started %s)', tp, humanize_seconds_ago(secs_since_started))
                continue
            secs_since_received = now - last_event_received
            if secs_since_received >= self.event_timeout_secs:
                self.log.warning('No event received for active tp %r in the last %r seconds (last event received %s)', tp, self.event_timeout_secs, humanize_seconds_ago(secs_since_received))
