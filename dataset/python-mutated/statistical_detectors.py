...
from snuba_sdk import And, BooleanCondition, BooleanOp, Column, Condition, CurriedFunction, Direction, Entity, Function, Granularity, Limit, LimitBy, Op, Or, OrderBy, Query, Request
...
from sentry.statistical_detectors.algorithm import MovingAverageDetectorState, MovingAverageRelativeChangeDetector, MovingAverageRelativeChangeDetectorConfig
...
def all_function_timeseries(functions_list: List[Tuple[Project, int]], start: datetime, trend_function: str) -> Generator[Tuple[int, int, Any], None, None]:
    ...
    for functions_chunk in chunked(functions_list, 25):
        try:
            ...
            sentry_sdk.capture_exception(e)
            continue
BACKEND_TRANSACTION_OPS = ['function.aws', 'function.aws.lambda', 'http.server', 'serverless.function', 'asgi.server', 'rails.request']
