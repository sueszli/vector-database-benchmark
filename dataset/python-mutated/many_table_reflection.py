...
if __name__ == '__main__':
    ...
    parser.add_argument('--db', help='Database url', default='sqlite:///many-table.db')
    parser.add_argument('--schema-name', help='optional schema name', type=str, default=None)
    parser.add_argument('--table-number', help='Number of table to generate.', type=int, default=250)
    parser.add_argument('--min-cols', help='Min number of column per table.', type=int, default=15)
    parser.add_argument('--max-cols', help='Max number of column per table.', type=int, default=250)
    parser.add_argument('--no-create', help='Do not run create tables', action='store_true')
    parser.add_argument('--no-drop', help='Do not run drop tables', action='store_true')
    parser.add_argument('--reflect', help='Run metadata reflect', action='store_true')
    parser.add_argument('--test', help="Run these tests. 'all' runs all tests", nargs='+', choices=tuple(tests) + ('all', 'none'), default=['all'])
    parser.add_argument('--sqlstats', help='count and time individual queries', action='store_true')
    parser.add_argument('--multi-only', help='Only run multi table tests', action='store_true')
    parser.add_argument('--single-only', help='Only run single table tests', action='store_true')
    parser.add_argument('--echo', action='store_true', help='Enable echo on the engine')
    parser.add_argument('--ignore-diff', action='store_true', help='Ignores differences in the single/multi reflections')
    parser.add_argument('--single-inspect-conn', action='store_true', help='Uses inspect on a connection instead of on the engine when using single reflections. Mainly for sqlite.')
    parser.add_argument('--pool-class', help='The pool class to use')
    parser.add_argument('--drop-all', action='store_true', help='Drop all tables, do nothing else')
    ...
    main(args.db, args.schema_name, args.table_number, min_cols, max_cols, args)