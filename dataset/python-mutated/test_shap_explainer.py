...
from darts.models import CatBoostModel, ExponentialSmoothing, LightGBMModel, LinearRegressionModel, NotImportedModule, RegressionModel, XGBModel
...
class TestShapExplainer:
    ...
    add_encoders = {'cyclic': {'past': ['month', 'day']}, 'datetime_attribute': {'future': ['hour', 'dayofweek']}, 'position': {'past': ['relative'], 'future': ['relative']}, 'custom': {'past': [extract_year]}, 'transformer': Scaler(scaler)}
    ...
    target_ts = TimeSeries.from_times_and_values(days, np.concatenate([x_1.reshape(-1, 1), x_2.reshape(-1, 1)], axis=1)).with_columns_renamed(['0', '1'], ['price', 'power'])
    target_ts_with_static_covs = TimeSeries.from_times_and_values(days, x_1.reshape(-1, 1), static_covariates=pd.DataFrame({'type': [0], 'state': [1]})).with_columns_renamed(['0'], ['price'])
    target_ts_with_multi_component_static_covs = TimeSeries.from_times_and_values(days, np.concatenate([x_1.reshape(-1, 1), x_2.reshape(-1, 1)], axis=1), static_covariates=pd.DataFrame({'type': [0, 1], 'state': [2, 3]})).with_columns_renamed(['0', '1'], ['price', 'power'])
    target_ts_multiple_series_with_different_static_covs = [TimeSeries.from_times_and_values(days, x_1.reshape(-1, 1), static_covariates=pd.DataFrame({'type': [0]})).with_columns_renamed(['0'], ['price']), TimeSeries.from_times_and_values(days, x_2.reshape(-1, 1), static_covariates=pd.DataFrame({'state': [1]})).with_columns_renamed(['0'], ['price'])]
    past_cov_ts = TimeSeries.from_times_and_values(days_past_cov, np.concatenate([past_cov_1.reshape(-1, 1), past_cov_2.reshape(-1, 1), past_cov_3.reshape(-1, 1)], axis=1))
    fut_cov_ts = TimeSeries.from_times_and_values(days, np.concatenate([fut_cov_1.reshape(-1, 1), fut_cov_2.reshape(-1, 1)], axis=1))
