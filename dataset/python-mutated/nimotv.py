...
@pluginmatcher(re.compile('https?://(?:www\\.|m\\.)?nimo\\.tv/(?P<username>.*)'))
class NimoTV(Plugin):
    data_url = 'https://m.nimo.tv/{0}'
    video_qualities = {250: '240p', 500: '360p', 1000: '480p', 2500: '720p', 6000: '1080p'}
    _re_appid = re.compile(b'appid=(\\d+)')
    _re_domain = re.compile(b'(https?:\\/\\/[A-Za-z]{2,3}.hls[A-Za-z\\.\\/]+)(?:V|&)')
    _re_id = re.compile(b'id=([^|\\\\]+)')
    _re_tp = re.compile(b'tp=(\\d+)')
    _re_wsSecret = re.compile(b'wsSecret=(\\w+)')
    _re_wsTime = re.compile(b'wsTime=(\\w+)')
