...
class LimeSDR(Device):
    ...
    DEVICE_METHODS.update({Device.Command.SET_FREQUENCY.name: 'set_center_frequency', Device.Command.SET_BANDWIDTH.name: 'set_lpf_bandwidth', Device.Command.SET_RF_GAIN.name: 'set_normalized_gain', Device.Command.SET_CHANNEL_INDEX.name: 'set_channel', Device.Command.SET_ANTENNA_INDEX.name: 'set_antenna'})
    ...
    def device_parameters(self):
        if False:
            i = 10
            return i + 15
        return OrderedDict([(self.Command.SET_CHANNEL_INDEX.name, self.channel_index), (self.Command.SET_ANTENNA_INDEX.name, self.antenna_index), (self.Command.SET_FREQUENCY.name, self.frequency), (self.Command.SET_SAMPLE_RATE.name, self.sample_rate), (self.Command.SET_BANDWIDTH.name, self.bandwidth), (self.Command.SET_RF_GAIN.name, self.gain * 0.01), ('identifier', self.device_serial)])
