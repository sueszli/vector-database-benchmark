...
if __name__ == '__main__':
    ...
    for update in range(1, args.num_updates + 1):
        ...
        (agent_state, episode_stats, next_obs, next_done, storage, key, handle) = rollout(agent_state, episode_stats, next_obs, next_done, key, handle)
    ...
    if args.save_model:
        ...
        with open(model_path, 'wb') as f:
            f.write(flax.serialization.to_bytes([vars(args), [agent_state.params.network_params, agent_state.params.actor_params, agent_state.params.critic_params]]))
        print(f'model saved to {model_path}')
        from cleanrl_utils.evals.ppo_envpool_jax_eval import evaluate
        episodic_returns = evaluate(model_path, make_env, args.env_id, eval_episodes=10, run_name=f'{run_name}-eval', Model=(Network, Actor, Critic))
