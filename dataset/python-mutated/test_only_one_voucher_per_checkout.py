...
from ...utils import checkout_add_promo_code, checkout_complete, checkout_create, checkout_delivery_method_update, checkout_dummy_payment_create, checkout_shipping_address_update

def prepare_free_shipping_voucher(e2e_staff_api_client, channel_id, free_shipping_voucher_code, voucher_discount_type, voucher_discount_value, voucher_type):
    if False:
        return 10
    input = {'code': free_shipping_voucher_code, 'discountValueType': voucher_discount_type, 'type': voucher_type}
    ...
    create_voucher_channel_listing(e2e_staff_api_client, free_shipping_voucher_code_id, channel_listing)
    return (free_shipping_voucher_code, free_shipping_voucher_code_id)

def prepare_fixed_voucher(e2e_staff_api_client, channel_id, fixed_voucher_code, voucher_discount_type, voucher_discount_value, voucher_type):
    if False:
        for i in range(10):
            print('nop')
    input = {'code': fixed_voucher_code, 'discountValueType': voucher_discount_type, 'type': voucher_type}
    voucher_data = create_voucher(e2e_staff_api_client, input)
...
@pytest.mark.e2e
def test_checkout_use_fixed_voucher_core_0901(e2e_not_logged_api_client, e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_shipping, permission_manage_product_types_and_attributes, permission_manage_discounts, permission_manage_checkouts):
    if False:
        i = 10
        return i + 15
    ...
    (_product_id, product_variant_id, product_variant_price) = prepare_product(e2e_staff_api_client, warehouse_id, channel_id, variant_price=15.55)
    (free_shipping_voucher_code, free_shipping_voucher_code_id) = prepare_free_shipping_voucher(e2e_staff_api_client, channel_id, free_shipping_voucher_code='FREESHIPPING', voucher_discount_type='PERCENTAGE', voucher_discount_value=100, voucher_type='SHIPPING')
    fixed_voucher_code = prepare_fixed_voucher(e2e_staff_api_client, channel_id, fixed_voucher_code='FIXED_VOUCHER', voucher_discount_type='FIXED', voucher_discount_value=50, voucher_type='ENTIRE_ORDER')
    ...
    data = checkout_add_promo_code(e2e_not_logged_api_client, checkout_id, free_shipping_voucher_code)
    ...
    data = checkout_add_promo_code(e2e_not_logged_api_client, checkout_id, fixed_voucher_code)
