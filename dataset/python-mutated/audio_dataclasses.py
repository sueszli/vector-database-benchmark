...
from typing import AsyncIterator, Callable, Final, Iterator, MutableMapping, Optional, Pattern, Tuple, Union
...
_RE_REMOVE_START: Final[Pattern] = re.compile('^(sc|list) ')
_RE_YOUTUBE_TIMESTAMP: Final[Pattern] = re.compile('[&|?]t=(\\d+)s?')
_RE_YOUTUBE_INDEX: Final[Pattern] = re.compile('&index=(\\d+)')
_RE_SPOTIFY_URL: Final[Pattern] = re.compile('(http[s]?://)?(open\\.spotify\\.com)/')
_RE_SPOTIFY_TIMESTAMP: Final[Pattern] = re.compile('#(\\d+):(\\d+)')
_RE_SOUNDCLOUD_TIMESTAMP: Final[Pattern] = re.compile('#t=(\\d+):(\\d+)s?')
_RE_TWITCH_TIMESTAMP: Final[Pattern] = re.compile('\\?t=(\\d+)h(\\d+)m(\\d+)s')
_PATH_SEPS: Final[Tuple[str, str]] = (posixpath.sep, ntpath.sep)
_FULLY_SUPPORTED_MUSIC_EXT: Final[Tuple[str, ...]] = ('.mp3', '.flac', '.ogg')
_PARTIALLY_SUPPORTED_MUSIC_EXT: Tuple[str, ...] = ('.m3u', '.m4a', '.aac', '.ra', '.wav', '.opus', '.wma', '.ts', '.au')
_PARTIALLY_SUPPORTED_VIDEO_EXT: Tuple[str, ...] = ('.mp4', '.mov', '.flv', '.webm', '.mkv', '.wmv', '.3gp', '.m4v', '.mk3d', '.mka', '.mks')
...
class LocalPath:
    ...
    async def _multiglob(self, pattern: str, folder: bool, method: Callable):
        async for rp in AsyncIter(method(pattern)):
            ...
            if folder and rp_local.is_dir() and rp_local.exists() or ((not folder and rp_local.suffix in self._all_music_ext and rp_local.is_file()) and rp_local.exists()):
                yield rp_local
    ...
    async def subfolders_in_tree(self):
        ...
        async for f in self.multirglob('', folder=True):
            with contextlib.suppress(ValueError):
                if f not in return_folders and f.is_dir() and (f.path != self.localtrack_folder) and f.path.relative_to(self.path):
    ...
    async def subfolders(self):
        ...
        async for f in self.multiglob('', folder=True):
            with contextlib.suppress(ValueError):
                if f not in return_folders and f.path != self.localtrack_folder and f.path.relative_to(self.path):
                    return_folders.append(f)
        return sorted(return_folders, key=lambda x: x.to_string_user().lower())
