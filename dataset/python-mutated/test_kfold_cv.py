...
from tests.integration_tests.utils import binary_feature, category_feature, create_data_set_to_use, generate_data, number_feature, sequence_feature, text_feature
logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)
logging.getLogger('ludwig').setLevel(logging.INFO)
FeaturesToUse = namedtuple('FeaturesToUse', 'input_features output_features')
FEATURES_TO_TEST = [FeaturesToUse([number_feature(normalization='zscore'), number_feature(normalization='zscore')], [number_feature()]), FeaturesToUse([number_feature(normalization='zscore'), number_feature(normalization='zscore')], [binary_feature()]), FeaturesToUse([number_feature(normalization='zscore'), number_feature(normalization='zscore')], [category_feature(decoder={'vocab_size': 4}, reduce_input='sum', output_feature=True)]), FeaturesToUse([number_feature(normalization='zscore'), number_feature(normalization='zscore')], [sequence_feature(decoder={'min_len': 5, 'max_len': 10, 'type': 'generator', 'cell_type': 'lstm', 'attention': 'bahdanau'}, reduce_input=None, output_feature=True)]), FeaturesToUse([sequence_feature(encoder={'min_len': 5, 'max_len': 10, 'type': 'rnn', 'cell_type': 'lstm', 'reduce_output': None})], [sequence_feature(decoder={'max_len': 10, 'type': 'tagger'}, reduce_input=None, output_feature=True)]), FeaturesToUse([number_feature(normalization='zscore'), number_feature(normalization='zscore')], [text_feature(output_feature=True)])]
...
def test_kfold_cv_api_in_memory(tmpdir):
    ...
    for key in ['fold_' + str(i + 1) for i in range(num_folds)]:
        assert key in kfold_split_indices
DATA_FORMATS_FOR_KFOLDS = ['csv', 'df', 'dict', 'excel', 'feather', 'fwf', 'html', 'json', 'jsonl', 'parquet', 'pickle', 'stata', 'tsv']
