...
def test_get_tools_success(mocks):
    ...
    with patch('superagi.helper.auth.get_user_organisation') as mock_get_user_org, patch('superagi.controllers.tool.db') as mock_db, patch('superagi.helper.auth.db') as mock_auth_db:
        mock_db.session.query.return_value.filter.return_value.all.side_effect = [user_toolkits, [tool_1, tool_2], [tool_3]]
        response = client.get('/tools/list')
        assert response.status_code == 200
        assert response.json() == [{'id': 1, 'name': 'tool_1', 'description': 'Test Tool', 'folder_name': 'test folder', 'file_name': 'test file', 'toolkit_id': 1}, {'id': 1, 'name': 'tool_2', 'description': 'Test Tool', 'folder_name': 'test folder', 'file_name': 'test file', 'toolkit_id': 1}, {'id': 1, 'name': 'tool_3', 'description': 'Test Tool', 'folder_name': 'test folder', 'file_name': 'test file', 'toolkit_id': 2}]