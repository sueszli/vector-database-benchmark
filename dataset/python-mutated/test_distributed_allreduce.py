...
def test_allreduce_different_name(ray_start_distributed_2_nodes_4_gpus, group_name, world_size):
    if False:
        print('Hello World!')
...
def test_allreduce_different_array_size(ray_start_distributed_2_nodes_4_gpus, array_size):
    if False:
        for i in range(10):
            print('nop')
    world_size = 4
...
def test_allreduce_destroy(ray_start_distributed_2_nodes_4_gpus, backend='nccl', group_name='default'):
    if False:
        print('Hello World!')
    world_size = 4
    (actors, _) = create_collective_workers(world_size)
    ...
    results = ray.get([a.do_allreduce.remote() for a in actors])
    assert (results[0] == cp.ones((10,), dtype=cp.float32) * world_size * world_size).all()
    assert (results[1] == cp.ones((10,), dtype=cp.float32) * world_size * world_size).all()

def test_allreduce_multiple_group(ray_start_distributed_2_nodes_4_gpus, backend='nccl', num_groups=5):
    if False:
        while True:
            i = 10
    world_size = 4
