...
class Test_vpc_subnet_separate_private_public:
    ...
    def test_vpc_subnet_only_private(self):
        ...
        vpc = ec2_client.create_vpc(CidrBlock='172.28.7.0/24', InstanceTenancy='default', TagSpecifications=[{'ResourceType': 'vpc', 'Tags': [{'Key': 'Name', 'Value': 'vpc_name'}]}])
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info):
            with mock.patch('prowler.providers.aws.services.vpc.vpc_subnet_separate_private_public.vpc_subnet_separate_private_public.vpc_client', new=VPC(current_audit_info)):
                from prowler.providers.aws.services.vpc.vpc_subnet_separate_private_public.vpc_subnet_separate_private_public import vpc_subnet_separate_private_public
                ...
                for result in results:
                    if result.resource_id == vpc['Vpc']['VpcId']:
                        ...
                        assert result.status_extended == 'VPC vpc_name has only private subnets.'
                        assert result.resource_id == vpc['Vpc']['VpcId']
                        assert result.resource_tags == [{'Key': 'Name', 'Value': 'vpc_name'}]
            ...
            with mock.patch('prowler.providers.aws.services.vpc.vpc_subnet_separate_private_public.vpc_subnet_separate_private_public.vpc_client', new=VPC(current_audit_info)):
                ...
                for result in results:
                    if result.resource_id == vpc['Vpc']['VpcId']:
                        found = True
                        assert result.status == 'FAIL'
                        assert result.status_extended == f"VPC {vpc['Vpc']['VpcId']} has only public subnets."
                        assert result.resource_id == vpc['Vpc']['VpcId']
    ...
    def test_vpc_subnet_private_and_public(self):
        ...
        ec2_client.create_route(DestinationCidrBlock='10.10.10.0', RouteTableId=route_table_private['RouteTable']['RouteTableId'])
        ec2_client.associate_route_table(RouteTableId=route_table_private['RouteTable']['RouteTableId'], SubnetId=subnet_private['Subnet']['SubnetId'])
        subnet_public = ec2_client.create_subnet(VpcId=vpc['Vpc']['VpcId'], CidrBlock='172.28.7.0/26', AvailabilityZone=f'{AWS_REGION}a')
        route_table_public = ec2_client.create_route_table(VpcId=vpc['Vpc']['VpcId'])
        igw = ec2_client.create_internet_gateway()
        ec2_client.create_route(DestinationCidrBlock='0.0.0.0', RouteTableId=route_table_public['RouteTable']['RouteTableId'], GatewayId=igw['InternetGateway']['InternetGatewayId'])
        ec2_client.associate_route_table(RouteTableId=route_table_public['RouteTable']['RouteTableId'], SubnetId=subnet_public['Subnet']['SubnetId'])
            ...
            with mock.patch('prowler.providers.aws.services.vpc.vpc_subnet_separate_private_public.vpc_subnet_separate_private_public.vpc_client', new=VPC(current_audit_info)):
                ...
                for result in results:
                    if result.resource_id == vpc['Vpc']['VpcId']:
                        ...
                        assert result.status_extended == f"VPC {vpc['Vpc']['VpcId']} has private and public subnets."
                        assert result.resource_id == vpc['Vpc']['VpcId']
                        assert result.resource_tags == []
                        assert result.region == AWS_REGION
                if not found:
