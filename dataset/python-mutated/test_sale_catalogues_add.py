...
def test_sale_add_catalogues(updated_webhook_mock, update_products_discounted_prices_for_promotion_task_mock, staff_api_client, promotion_converted_from_sale_with_empty_predicate, category, product, collection, product_variant_list, permission_manage_discounts):
    if False:
        for i in range(10):
            print('nop')
    ...
    variables = {'id': graphene.Node.to_global_id('Sale', promotion.old_sale_id), 'input': {'products': [product_id], 'collections': [collection_id], 'categories': [category_id], 'variants': variant_ids}}
...
@patch('saleor.product.tasks.update_products_discounted_prices_for_promotion_task.delay')
@patch('saleor.plugins.manager.PluginsManager.sale_updated')
def test_sale_add_catalogues_no_changes_in_catalogue(updated_webhook_mock, update_products_discounted_prices_for_promotion_task_mock, staff_api_client, promotion_converted_from_sale, catalogue_predicate, collection, category, product, variant, permission_manage_discounts):
    if False:
        for i in range(10):
            print('nop')
    query = SALE_CATALOGUES_ADD_MUTATION
...
@patch('saleor.product.tasks.update_products_discounted_prices_for_promotion_task.delay')
@patch('saleor.plugins.manager.PluginsManager.sale_updated')
def test_sale_add_empty_catalogues(updated_webhook_mock, update_products_discounted_prices_for_promotion_task_mock, staff_api_client, promotion_converted_from_sale, catalogue_predicate, collection, category, product, variant, permission_manage_discounts):
    if False:
        return 10
    query = SALE_CATALOGUES_ADD_MUTATION
    promotion = promotion_converted_from_sale
    ...
    product_id = graphene.Node.to_global_id('Product', product.id)
    variant_id = graphene.Node.to_global_id('ProductVariant', variant.id)
    variables = {'id': graphene.Node.to_global_id('Sale', promotion.old_sale_id), 'input': {'products': [], 'collections': [], 'categories': [], 'variants': []}}
    response = staff_api_client.post_graphql(query, variables, permissions=[permission_manage_discounts])
    content = get_graphql_content(response)
    assert not content['data']['saleCataloguesAdd']['errors']
    promotion.refresh_from_db()
...
@patch('saleor.product.tasks.update_products_discounted_prices_for_promotion_task.delay')
@patch('saleor.plugins.manager.PluginsManager.sale_updated')
def test_sale_add_empty_catalogues_to_sale_with_empty_catalogues(updated_webhook_mock, update_products_discounted_prices_for_promotion_task_mock, staff_api_client, promotion_converted_from_sale_with_empty_predicate, permission_manage_discounts):
    if False:
        while True:
            i = 10
    query = SALE_CATALOGUES_ADD_MUTATION
...
@patch('saleor.product.tasks.update_products_discounted_prices_for_promotion_task.delay')
@patch('saleor.plugins.manager.PluginsManager.sale_updated')
def test_sale_add_catalogues_no_product_ids_change(updated_webhook_mock, update_products_discounted_prices_for_promotion_task_mock, staff_api_client, promotion_converted_from_sale, catalogue_predicate, product, product_variant_list, permission_manage_discounts):
    if False:
        for i in range(10):
            print('nop')
    query = SALE_CATALOGUES_ADD_MUTATION
    ...
    variables = {'id': graphene.Node.to_global_id('Sale', promotion.old_sale_id), 'input': {'variants': variant_ids}}
    response = staff_api_client.post_graphql(query, variables, permissions=[permission_manage_discounts])
    content = get_graphql_content(response)
    assert not content['data']['saleCataloguesAdd']['errors']
    promotion.refresh_from_db()
...
@patch('saleor.product.tasks.update_products_discounted_prices_for_promotion_task.delay')
def test_sale_add_catalogues_with_product_without_variants(update_products_discounted_prices_for_promotion_task_mock, staff_api_client, promotion_converted_from_sale, catalogue_predicate, category, product, collection, permission_manage_discounts):
    if False:
        while True:
            i = 10
    query = SALE_CATALOGUES_ADD_MUTATION
    ...
    collection_id = graphene.Node.to_global_id('Collection', collection.id)
    category_id = graphene.Node.to_global_id('Category', category.id)
    variables = {'id': graphene.Node.to_global_id('Sale', promotion.old_sale_id), 'input': {'products': [product_id], 'collections': [collection_id], 'categories': [category_id]}}
    response = staff_api_client.post_graphql(query, variables, permissions=[permission_manage_discounts])
    content = get_graphql_content(response)
...
def test_sale_add_catalogues_with_promotion_id(staff_api_client, promotion_converted_from_sale, product, permission_manage_discounts):
    if False:
        while True:
            i = 10
    query = SALE_CATALOGUES_ADD_MUTATION
    ...
    assert error['message'] == "Provided ID refers to Promotion model. Please use 'promotionRuleCreate' mutation instead."

def test_sale_add_catalogues_not_found_error(staff_api_client, permission_manage_discounts):
    if False:
        for i in range(10):
            print('nop')
    query = SALE_CATALOGUES_ADD_MUTATION
