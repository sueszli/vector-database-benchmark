...
def test_gam_knots_key():
    ...
    h2o_model2 = H2OGeneralizedAdditiveEstimator(family='multinomial', gam_columns=['C6', 'C7', 'C8'], scale=[1, 1, 1], num_knots=numKnots)
    ...
    for kNames in keyNames:
        assert abs(h2oCoeffs['coefficients'][kNames] - h2oCoeffs2['coefficients'][kNames]) < 1e-06, 'expected coefficients: {0}.  actual coefficients: {1}'.format(h2oCoeffs['coefficients'][kNames], h2oCoeffs2['coefficients'][kNames])
