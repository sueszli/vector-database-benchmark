"""Script for training, evaluation and sampling for Real NVP.
...
from real_nvp_utils import batch_norm, batch_norm_log_diff, conv_layer, squeeze_2x2, squeeze_2x2_ordered, standard_normal_ll, standard_normal_sample, unsqueeze_2x2, variable_on_cpu
tf.flags.DEFINE_string('master', 'local', 'BNS name of the TensorFlow master, or local.')
tf.flags.DEFINE_string('logdir', '/tmp/real_nvp_multiscale', 'Directory to which writes logs.')
tf.flags.DEFINE_string('traindir', '/tmp/real_nvp_multiscale', 'Directory to which writes logs.')
tf.flags.DEFINE_integer('train_steps', 1000000000000000000, 'Number of steps to train for.')
tf.flags.DEFINE_string('data_path', '', 'Path to the data.')
tf.flags.DEFINE_string('mode', 'train', "Mode of execution. Must be 'train', 'sample' or 'eval'.")
tf.flags.DEFINE_string('dataset', 'imnet', "Dataset used. Must be 'imnet', 'celeba' or 'lsun'.")
tf.flags.DEFINE_integer('recursion_type', 2, 'Type of the recursion.')
tf.flags.DEFINE_integer('image_size', 64, 'Size of the input image.')
tf.flags.DEFINE_integer('eval_set_size', 0, 'Size of evaluation dataset.')
tf.flags.DEFINE_string('hpconfig', '', 'A comma separated list of hyperparameters for the model. Format is hp1=value1,hp2=value2,etc. If this FLAG is set, the model will be trained with the specified hyperparameters, filling in missing hyperparameters from the default_values in |hyper_params|.')
