...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--repository', type=str, default=REPOSITORY, help="Specify the repository (default: '%(default)s')")
    parser.add_argument('-t', '--token', dest='token', type=str, metavar='TOKEN', default=GITHUB_TOKEN, help='Specify the Github token to use (requires `repo` scope)')
    parser.add_argument('-o', '--output-file', type=argparse.FileType('w', encoding='UTF-8'), default=open(str(OUTPUT_FILE.absolute()), 'w'), metavar='/path/to/output_file.md', help=f"Specify the output file (default: '{OUTPUT_FILE}')")
    parser.add_argument('--version', type=str, default=datetime.date.today().strftime('%Y.%m'), help="Specify the version number (default: '%(default)s')")
    parser.add_argument('--codename', type=str, default=shell('random-word').title(), help="Specify the version codename (default: '%(default)s')")
    parser.add_argument('--push-release', action='store_true', default=False, help='Create the new tag and publish the release on Github')
