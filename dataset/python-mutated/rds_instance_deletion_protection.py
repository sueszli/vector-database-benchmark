    ...
    def execute(self):
        ...
        for db_instance in rds_client.db_instances:
            ...
            if db_instance.cluster_id:
                if db_instance.cluster_arn in rds_client.db_clusters and rds_client.db_clusters[db_instance.cluster_arn].deletion_protection:
                    report.status = 'PASS'
            ...
            elif db_instance.deletion_protection:
                report.status = 'PASS'
                report.status_extended = f'RDS Instance {db_instance.id} deletion protection is enabled.'
            else:
                report.status = 'FAIL'
