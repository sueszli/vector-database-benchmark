...
def call_graphviz(program, arguments, working_dir, **kwargs):
    ...
    if arguments is None:
        arguments = []
    env = {'PATH': os.environ.get('PATH', ''), 'LD_LIBRARY_PATH': os.environ.get('LD_LIBRARY_PATH', '')}
...
class Edge(Common):
    ...
    def __eq__(self, edge):
        ...
        if self.get_parent_graph().get_top_graph_type() == 'graph':
            if self.get_source() == edge.get_source() and self.get_destination() == edge.get_destination() or (edge.get_source() == self.get_destination() and edge.get_destination() == self.get_source()):
                return True
        elif self.get_source() == edge.get_source() and self.get_destination() == edge.get_destination():
            return True
        return False
...
class Graph(Common):
    ...
    def __init__(self, graph_name='G', obj_dict=None, graph_type='digraph', strict=False, suppress_disconnected=False, simplify=False, **attrs):
        if False:
            return 10
        if obj_dict is not None:
            self.obj_dict = obj_dict
            ...
            if graph_type not in ['graph', 'digraph']:
                raise Error('Invalid type "{t}". Accepted graph types are: graph, digraph'.format(t=graph_type))
...
class Subgraph(Graph):
    ...
    def __init__(self, graph_name='', obj_dict=None, suppress_disconnected=False, simplify=False, **attrs):
        if False:
            print('Hello World!')
        Graph.__init__(self, graph_name=graph_name, obj_dict=obj_dict, suppress_disconnected=suppress_disconnected, simplify=simplify, **attrs)
        if obj_dict is None:
...
class Cluster(Graph):
    ...
    def __init__(self, graph_name='subG', obj_dict=None, suppress_disconnected=False, simplify=False, **attrs):
        if False:
            return 10
        Graph.__init__(self, graph_name=graph_name, obj_dict=obj_dict, suppress_disconnected=suppress_disconnected, simplify=simplify, **attrs)
        if obj_dict is None:
...
class Dot(Graph):
    ...
    def __init__(self, *argsl, **argsd):
        ...
        self.formats = ['canon', 'cmap', 'cmapx', 'cmapx_np', 'dia', 'dot', 'fig', 'gd', 'gd2', 'gif', 'hpgl', 'imap', 'imap_np', 'ismap', 'jpe', 'jpeg', 'jpg', 'mif', 'mp', 'pcl', 'pdf', 'pic', 'plain', 'plain-ext', 'png', 'ps', 'ps2', 'svg', 'svgz', 'vml', 'vmlz', 'vrml', 'vtx', 'wbmp', 'xdot', 'xlib']
        self.prog = 'dot'
    ...
    def create(self, prog=None, format='ps', encoding=None):
        ...
        try:
            (stdout_data, stderr_data, process) = call_graphviz(program=prog, arguments=arguments, working_dir=tmp_dir)
        except OSError as e:
        ...
        if process.returncode != 0:
            message = '"{prog}" with args {arguments} returned code: {code}\n\nstdout, stderr:\n {out}\n{err}\n'.format(prog=prog, arguments=arguments, code=process.returncode, out=stdout_data, err=stderr_data)
            print(message)
