...
sqlalchemy_config = SQLAlchemyAsyncConfig(connection_string='sqlite+aiosqlite:///test.sqlite', session_config=session_config)
...
@get(path='/')
async def get_blogs(blog_post_repo: BlogPostRepository) -> list[BlogPostDTO]:
...
async def get_blog_details(post_slug: str, blog_post_repo: BlogPostRepository) -> BlogPostDTO:
    """Interact with SQLAlchemy engine and session."""
    obj = await blog_post_repo.get_one(slug=post_slug)
    return BlogPostDTO.model_validate(obj)

@post(path='/')
async def create_blog(blog_post_repo: BlogPostRepository, data: BlogPostCreate) -> BlogPostDTO:
    ...
    return BlogPostDTO.model_validate(obj)
app = Litestar(route_handlers=[create_blog, get_blogs, get_blog_details], dependencies={'blog_post_repo': Provide(provide_blog_post_repo, sync_to_thread=False)}, on_startup=[on_startup], plugins=[SQLAlchemyInitPlugin(config=sqlalchemy_config)])