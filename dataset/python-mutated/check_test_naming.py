...
def is_misnamed_test_func(node: ast.expr | ast.stmt, names: Sequence[str], line: str) -> bool:
    if False:
        return 10
    return isinstance(node, ast.FunctionDef) and (not node.name.startswith('test')) and (names.count(node.name) == 0) and (not any((_is_fixture(decorator) for decorator in node.decorator_list))) and (PRAGMA not in line) and (node.name not in ('teardown_method', 'setup_method', 'teardown_class', 'setup_class'))

def is_misnamed_test_class(node: ast.expr | ast.stmt, names: Sequence[str], line: str) -> bool:
    if False:
        while True:
            i = 10
    return isinstance(node, ast.ClassDef) and (not node.name.startswith('Test')) and (names.count(node.name) == 0) and (not any((_is_register_dtype(decorator) for decorator in node.decorator_list))) and (PRAGMA not in line)
...
def main(content: str, file: str) -> int:
    ...
    for node in tree.body:
        if is_misnamed_test_func(node, names, lines[node.lineno - 1]):
            print(f"{file}:{node.lineno}:{node.col_offset} found test function which does not start with 'test'")
            ret = 1
        elif is_misnamed_test_class(node, names, lines[node.lineno - 1]):
            print(f"{file}:{node.lineno}:{node.col_offset} found test class which does not start with 'Test'")
            ret = 1
        if isinstance(node, ast.ClassDef) and names.count(node.name) == 0 and (not any((_is_register_dtype(decorator) for decorator in node.decorator_list))) and (PRAGMA not in lines[node.lineno - 1]):
            for _node in node.body:
                if is_misnamed_test_func(_node, names, lines[_node.lineno - 1]):
                    ...
                    if should_continue:
                        continue
                    print(f"{file}:{_node.lineno}:{_node.col_offset} found test function which does not start with 'test'")
                    ret = 1
    return ret
