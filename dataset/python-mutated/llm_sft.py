...
from utils import DATASET_MAPPING, DEFAULT_PROMPT, MODEL_MAPPING, data_collate_fn, get_dataset, get_model_tokenizer, get_T_max, get_work_dir, parse_args, plot_images, print_example, print_model_info, process_dataset, seed_everything, show_freeze_layers, stat_dataset, tokenize_function
from modelscope import get_logger
from modelscope.trainers import EpochBasedTrainer
from modelscope.utils.config import Config
warnings.warn('This directory has been migrated to https://github.com/modelscope/swift/tree/main/examples/pytorch/llm, and the files in this directory are no longer maintained.', DeprecationWarning)
logger = get_logger()

@dataclass
class SftArguments:
    seed: int = 42
    model_type: str = field(default='qwen-7b', metadata={'choices': list(MODEL_MAPPING.keys())})
    sft_type: str = field(default='lora', metadata={'choices': ['lora', 'full']})
    output_dir: Optional[str] = None
    ignore_args_error: bool = False
    dataset: str = field(default='alpaca-en,alpaca-zh', metadata={'help': f'dataset choices: {list(DATASET_MAPPING.keys())}'})
    dataset_seed: int = 42
    dataset_sample: int = 20000
    ...
    tb_interval: int = 5
    use_flash_attn: Optional[bool] = field(default=None, metadata={'help': "This parameter is used only when model_type='qwen-7b'"})
