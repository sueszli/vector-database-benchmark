...
INIT_HEADERS_FILTER = {hdrs.CONTENT_LENGTH, hdrs.CONTENT_ENCODING, hdrs.TRANSFER_ENCODING, hdrs.ACCEPT_ENCODING, hdrs.SEC_WEBSOCKET_EXTENSIONS, hdrs.SEC_WEBSOCKET_PROTOCOL, hdrs.SEC_WEBSOCKET_VERSION, hdrs.SEC_WEBSOCKET_KEY}
RESPONSE_HEADERS_FILTER = {hdrs.TRANSFER_ENCODING, hdrs.CONTENT_LENGTH, hdrs.CONTENT_TYPE, hdrs.CONTENT_ENCODING}
...
class HassIOIngress(HomeAssistantView):
    ...
    async def _handle(self, request: web.Request, token: str, path: str) -> web.Response | web.StreamResponse | web.WebSocketResponse:
    ...
    async def _handle_websocket(self, request: web.Request, token: str, path: str) -> web.WebSocketResponse:
        """Ingress route for websocket."""
        req_protocols: Iterable[str]
        if hdrs.SEC_WEBSOCKET_PROTOCOL in request.headers:
            req_protocols = [str(proto.strip()) for proto in request.headers[hdrs.SEC_WEBSOCKET_PROTOCOL].split(',')]
        else:
            req_protocols = ()
        ...
        async with self._websession.ws_connect(url, headers=source_header, protocols=req_protocols, autoclose=False, autoping=False) as ws_client:
            await asyncio.wait([asyncio.create_task(_websocket_forward(ws_server, ws_client)), asyncio.create_task(_websocket_forward(ws_client, ws_server))], return_when=asyncio.FIRST_COMPLETED)
        return ws_server

    async def _handle_request(self, request: web.Request, token: str, path: str) -> web.Response | web.StreamResponse:
        """Ingress route for request."""
        url = self._create_url(token, path)
        source_header = _init_header(request, token)
        async with self._websession.request(request.method, url, headers=source_header, params=request.query, allow_redirects=False, data=request.content if request.method != 'GET' else None, timeout=ClientTimeout(total=None), skip_auto_headers={hdrs.CONTENT_TYPE}) as result:
            ...
            if result.status in (204, 304) or (content_length is not UNDEFINED and (content_length_int := int(content_length or 0)) <= MAX_SIMPLE_RESPONSE_SIZE):
                body = await result.read()
                simple_response = web.Response(headers=headers, status=result.status, content_type=content_type, body=body)
                if content_length_int > MIN_COMPRESSED_SIZE and should_compress(content_type or simple_response.content_type):
