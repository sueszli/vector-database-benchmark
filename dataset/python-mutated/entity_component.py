...
from homeassistant.const import CONF_ENTITY_NAMESPACE, CONF_SCAN_INTERVAL, EVENT_HOMEASSISTANT_STOP
from homeassistant.core import EntityServiceResponse, Event, HomeAssistant, ServiceCall, ServiceResponse, SupportsResponse, callback
...
async def async_update_entity(hass: HomeAssistant, entity_id: str) -> None:
    ...
    if entity_comp is None:
        logging.getLogger(__name__).warning('Forced update failed. Component for %s not loaded.', entity_id)
        return
    if (entity_obj := entity_comp.get_entity(entity_id)) is None:
        logging.getLogger(__name__).warning('Forced update failed. Entity %s not found.', entity_id)
        return
...
class EntityComponent(Generic[_EntityT]):
    ...
    async def async_setup(self, config: ConfigType) -> None:
        ...
        for (p_type, p_config) in config_per_platform(config, self.domain):
            if p_type is not None:
                self.hass.async_create_task(self.async_setup_platform(p_type, p_config), f'EntityComponent setup platform {p_type} {self.domain}')

        async def component_platform_discovered(platform: str, info: dict[str, Any] | None) -> None:
            """Handle the loading of a platform."""
            await self.async_setup_platform(platform, {}, info)
        discovery.async_listen_platform(self.hass, self.domain, component_platform_discovered)

    async def async_setup_entry(self, config_entry: ConfigEntry) -> bool:
        """Set up a config entry."""
        platform_type = config_entry.domain
        platform = await async_prepare_setup_platform(self.hass, self.config or {}, self.domain, platform_type)
        if platform is None:
            return False
        key = config_entry.entry_id
        if key in self._platforms:
            raise ValueError('Config entry has already been setup!')
        self._platforms[key] = self._async_init_entity_platform(platform_type, platform, scan_interval=getattr(platform, 'SCAN_INTERVAL', None))
    ...
    async def async_extract_from_service(self, service_call: ServiceCall, expand_group: bool=True) -> list[_EntityT]:
        ...
        This method must be run in the event loop.
        """
        return await service.async_extract_entities(self.hass, self.entities, service_call, expand_group)
    ...
    async def async_setup_platform(self, platform_type: str, platform_config: ConfigType, discovery_info: DiscoveryInfoType | None=None) -> None:
        ...
        platform = await async_prepare_setup_platform(self.hass, self.config, self.domain, platform_type)
        if platform is None:
            return
        scan_interval = platform_config.get(CONF_SCAN_INTERVAL, getattr(platform, 'SCAN_INTERVAL', None))
        entity_namespace = platform_config.get(CONF_ENTITY_NAMESPACE)
        key = (platform_type, scan_interval, entity_namespace)
        if key not in self._platforms:
            self._platforms[key] = self._async_init_entity_platform(platform_type, platform, scan_interval, entity_namespace)
    ...
    async def async_prepare_reload(self, *, skip_reset: bool=False) -> ConfigType | None:
        ...
        processed_conf = await conf_util.async_process_component_config(self.hass, conf, integration)
    ...
    async def _async_shutdown(self, event: Event) -> None:
        ...
        await asyncio.gather(*(platform.async_shutdown() for platform in chain(self._platforms.values())))