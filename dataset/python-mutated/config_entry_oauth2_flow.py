...
class LocalOAuth2Implementation(AbstractOAuth2Implementation):
    ...
    async def _token_request(self, data: dict) -> dict:
        ...
        if resp.status >= 400:
            ...
            _LOGGER.error('Token request failed (%s): %s', error_code, error_description)
...
async def async_get_implementations(hass: HomeAssistant, domain: str) -> dict[str, AbstractOAuth2Implementation]:
    ...
    registered = cast(dict[str, AbstractOAuth2Implementation], hass.data.setdefault(DATA_IMPLEMENTATIONS, {}).get(domain, {}))
