...
class redis_clusterCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_cluster': ('parent', 'cluster_id', 'cluster', 'request_id'), 'delete_cluster': ('name', 'request_id'), 'get_cluster': ('name',), 'list_clusters': ('parent', 'page_size', 'page_token'), 'update_cluster': ('update_mask', 'cluster', 'request_id')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=redis_clusterCallTransformer()):
    if False:
        while True:
            i = 10
