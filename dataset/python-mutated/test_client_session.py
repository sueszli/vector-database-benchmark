...
async def test_init_headers_list_of_tuples(create_session: Any) -> None:
    session = await create_session(headers=[('h1', 'header1'), ('h2', 'header2'), ('h3', 'header3')])
    assert session.headers == CIMultiDict([('h1', 'header1'), ('h2', 'header2'), ('h3', 'header3')])

async def test_init_headers_MultiDict(create_session: Any) -> None:
    session = await create_session(headers=MultiDict([('h1', 'header1'), ('h2', 'header2'), ('h3', 'header3')]))
    assert session.headers == CIMultiDict([('H1', 'header1'), ('H2', 'header2'), ('H3', 'header3')])

async def test_init_headers_list_of_tuples_with_duplicates(create_session: Any) -> None:
    session = await create_session(headers=[('h1', 'header11'), ('h2', 'header21'), ('h1', 'header12')])
    assert session.headers == CIMultiDict([('H1', 'header11'), ('H2', 'header21'), ('H1', 'header12')])
...
async def test_merge_headers_with_list_of_tuples_duplicated_names(create_session: Any) -> None:
    session = await create_session(headers={'h1': 'header1', 'h2': 'header2'})
    headers = session._prepare_headers([('h1', 'v1'), ('h1', 'v2')])
    assert isinstance(headers, CIMultiDict)
    assert list(sorted(headers.items())) == [('h1', 'v1'), ('h1', 'v2'), ('h2', 'header2')]
...
async def test_del_debug(connector: Any, loop: Any) -> None:
    ...
    with pytest.warns(ResourceWarning):
        ...
        gc.collect()
    assert len(logs) == 1
    expected = {'client_session': mock.ANY, 'message': 'Unclosed client session', 'source_traceback': mock.ANY}
    assert logs[0] == expected

async def test_borrow_connector_loop(connector: Any, create_session: Any, loop: Any) -> None:
    session = ClientSession(connector=connector)
    try:
...
async def test_close_conn_on_error(create_session: Any, create_mocked_conn: Any) -> None:
...
async def test_cookie_jar_usage(loop: Any, aiohttp_client: Any) -> None:
    ...
    session = await aiohttp_client(app, cookies={'request': 'req_value'}, cookie_jar=jar)
...
async def test_request_tracing(loop: Any, aiohttp_client: Any) -> None:
    ...
    with io.BytesIO() as gathered_req_body, io.BytesIO() as gathered_res_body:
        ...
        trace_config = aiohttp.TraceConfig(trace_config_ctx_factory=mock.Mock(return_value=trace_config_ctx))
        ...
        async with session.post('/', data=body, trace_request_ctx=trace_request_ctx) as resp:
            await resp.json()
            on_request_start.assert_called_once_with(session.session, trace_config_ctx, aiohttp.TraceRequestStartParams(hdrs.METH_POST, session.make_url('/'), headers))
            on_request_end.assert_called_once_with(session.session, trace_config_ctx, aiohttp.TraceRequestEndParams(hdrs.METH_POST, session.make_url('/'), headers, resp))
...
async def test_request_tracing_url_params(loop: Any, aiohttp_client: Any) -> None:
    ...
    (on_request_start, on_request_redirect, on_request_end, on_request_exception, on_request_chunk_sent, on_response_chunk_received, on_request_headers_sent) = mocks
    trace_config = aiohttp.TraceConfig(trace_config_ctx_factory=mock.Mock(return_value=mock.Mock()))
    trace_config.on_request_start.append(on_request_start)
    trace_config.on_request_redirect.append(on_request_redirect)
    trace_config.on_request_end.append(on_request_end)
    ...
    for req in [lambda : session.get('/?x=0'), lambda : session.get('/', params=dict(x=0))]:
        reset_mocks()
    ...
    for req in [lambda : session.get('/redirect?x=0'), lambda : session.get('/redirect', params=dict(x=0))]:
        reset_mocks()
        async with req() as resp:
            await resp.text()
            ...
            assert to_trace_urls(on_request_exception) == []
            assert to_trace_urls(on_request_chunk_sent) == [to_url('/redirect?x=0'), to_url('/')]
            assert to_trace_urls(on_response_chunk_received) == [to_url('/')]
            assert to_trace_urls(on_request_headers_sent) == [to_url('/redirect?x=0'), to_url('/')]
...
async def test_request_tracing_exception() -> None:
    ...
    with mock.patch('aiohttp.client.TCPConnector.connect') as connect_patched:
        ...
        try:
            ...
            pass
        on_request_exception.assert_called_once_with(session, mock.ANY, aiohttp.TraceRequestExceptionParams(hdrs.METH_GET, URL('http://example.com'), CIMultiDict(), error))
        assert not on_request_end.called
    await session.close()

async def test_request_tracing_interpose_headers(loop: Any, aiohttp_client: Any) -> None:
    ...
    session = await aiohttp_client(app, request_class=MyClientRequest, trace_configs=[trace_config])
    await session.get('/')
...
async def test_build_url_returns_expected_url(create_session, base_url, url, expected_url) -> None:
