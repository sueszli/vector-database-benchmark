...
parser.add_argument('--run', type=str, default='CQL', help='The RLlib-registered algorithm to use.')
parser.add_argument('--framework', choices=['tf', 'tf2', 'torch'], default='torch', help='The DL framework specifier.')
parser.add_argument('--stop-iters', type=int, default=100)
parser.add_argument('--input-files', type=str, default=os.path.join(os.path.dirname(os.path.abspath(__file__)), '../tests/data/pendulum/small.json'))
...
if __name__ == '__main__':
    ...
    config = default_config.environment('Pendulum-v1', clip_actions=True).framework(args.framework).offline_data(input_='custom_input', input_config={'input_files': args.input_files}, actions_in_input_normalized=True).training(train_batch_size=2000).evaluation(evaluation_interval=1, evaluation_num_workers=2, evaluation_duration=10, evaluation_parallel_to_training=True, evaluation_config=default_config.overrides(input_='sampler', explore=False)).reporting(metrics_num_episodes_for_smoothing=5)
    if args.run == 'CQL':
        config.training(twin_q=True, num_steps_sampled_before_learning_starts=0, bc_iters=100)
    stop = {'training_iteration': args.stop_iters, 'evaluation/episode_reward_mean': -600}
    tuner = tune.Tuner(args.run, param_space=config, run_config=air.RunConfig(stop=stop, verbose=1))
    tuner.fit()