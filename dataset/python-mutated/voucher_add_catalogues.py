...
class CatalogueInput(BaseInputObjectType):
    products = NonNullList(graphene.ID, description='Products related to the discount.', name='products')
    categories = NonNullList(graphene.ID, description='Categories related to the discount.', name='categories')
    collections = NonNullList(graphene.ID, description='Collections related to the discount.', name='collections')
    variants = NonNullList(graphene.ID, description='Product variant related to the discount.' + ADDED_IN_31, name='variants')

    class Meta:
        doc_category = DOC_CATEGORY_DISCOUNTS

class VoucherBaseCatalogueMutation(BaseMutation):
    voucher = graphene.Field(Voucher, description='Voucher of which catalogue IDs will be modified.')

    class Arguments:
        id = graphene.ID(required=True, description='ID of a voucher.')
        input = CatalogueInput(required=True, description='Fields required to modify catalogue IDs of voucher.')
...
class VoucherAddCatalogues(VoucherBaseCatalogueMutation):
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.VOUCHER_UPDATED, description='A voucher was updated.')]
