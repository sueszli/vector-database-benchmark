...
from prefect.server.utilities.database import JSON, UUID, GenerateUUID, Pydantic, Timestamp, camel_to_snake, date_diff, interval_add, now
...
class ORMBase:
    ...
    id = sa.Column(UUID(), primary_key=True, server_default=GenerateUUID(), default=uuid.uuid4)
    created = sa.Column(Timestamp(), nullable=False, server_default=now(), default=lambda : pendulum.now('UTC'))
    updated = sa.Column(Timestamp(), nullable=False, index=True, server_default=now(), default=lambda : pendulum.now('UTC'), onupdate=now(), server_onupdate=FetchedValue())
...
class ORMFlowRunState:
    ...
    type = sa.Column(sa.Enum(schemas.states.StateType, name='state_type'), nullable=False, index=True)
    timestamp = sa.Column(Timestamp(), nullable=False, server_default=now(), default=lambda : pendulum.now('UTC'))
    name = sa.Column(sa.String, nullable=False, index=True)
    message = sa.Column(sa.String)
    state_details = sa.Column(Pydantic(schemas.states.StateDetails), server_default='{}', default=schemas.states.StateDetails, nullable=False)
...
class ORMArtifact:
    ...
    """
    key = sa.Column(sa.String, nullable=True, index=True)
...
class ORMArtifactCollection:
    ...
    task_run_id = sa.Column(UUID(), nullable=True)
    flow_run_id = sa.Column(UUID(), nullable=True)
    type = sa.Column(sa.String)
    data = sa.Column(sa.JSON, nullable=True)
    description = sa.Column(sa.String, nullable=True)
    metadata_ = sa.Column(sa.JSON, nullable=True)
...
class ORMTaskRunStateCache:
    ...
    cache_expiration = sa.Column(Timestamp(), nullable=True)
...
class ORMRun:
    ...
    """
    name = sa.Column(sa.String, default=lambda : generate_slug(2), nullable=False, index=True)
    ...
    total_run_time = sa.Column(sa.Interval(), server_default='0', default=datetime.timedelta(0), nullable=False)
...
class ORMFlowRun(ORMRun):
    ...
    empirical_policy = sa.Column(Pydantic(schemas.core.FlowRunPolicy), server_default='{}', default=schemas.core.FlowRunPolicy, nullable=False)
    tags = sa.Column(JSON, server_default='[]', default=list, nullable=False)
    created_by = sa.Column(Pydantic(schemas.core.CreatedBy), server_default=None, default=None, nullable=True)
    ...
    auto_scheduled = sa.Column(sa.Boolean, server_default='0', default=False, nullable=False)
...
class ORMTaskRun(ORMRun):
    ...
    flow_run_run_count = sa.Column(sa.Integer, server_default='0', default=0, nullable=False)
    empirical_policy = sa.Column(Pydantic(schemas.core.TaskRunPolicy), server_default='{}', default=schemas.core.TaskRunPolicy, nullable=False)
    task_inputs = sa.Column(Pydantic(Dict[str, List[Union[schemas.core.TaskRunResult, schemas.core.Parameter, schemas.core.Constant]]]), server_default='{}', default=dict, nullable=False)
    tags = sa.Column(JSON, server_default='[]', default=list, nullable=False)
...
class ORMDeployment:
    ...
    is_schedule_active = sa.Column(sa.Boolean, nullable=False, server_default='1', default=True)
    tags = sa.Column(JSON, server_default='[]', default=list, nullable=False)
    parameters = sa.Column(JSON, server_default='{}', default=dict, nullable=False)
    pull_steps = sa.Column(JSON, default=list, nullable=True)
    parameter_openapi_schema = sa.Column(JSON, default=dict, nullable=True)
    enforce_parameter_schema = sa.Column(sa.Boolean, default=False, server_default='0', nullable=False)
    created_by = sa.Column(Pydantic(schemas.core.CreatedBy), server_default=None, default=None, nullable=True)
    updated_by = sa.Column(Pydantic(schemas.core.UpdatedBy), server_default=None, default=None, nullable=True)
...
class ORMBlockType:
    ...
    is_protected = sa.Column(sa.Boolean, nullable=False, server_default='0', default=False)
...
class ORMBlockSchema:
    ...
    version = sa.Column(sa.String, server_default=schemas.core.DEFAULT_BLOCK_SCHEMA_VERSION, nullable=False)
...
class ORMSavedSearch:
    ...
    name = sa.Column(sa.String, nullable=False)
    filters = sa.Column(JSON, server_default='[]', default=list, nullable=False)
...
class ORMWorkQueue:
    ...
    name = sa.Column(sa.String, nullable=False)
    filter = sa.Column(Pydantic(schemas.core.QueueFilter), server_default=None, default=None, nullable=True)
...
class ORMWorker:
    ...
    def work_pool_id(cls):
        ...
        return sa.Column(UUID, sa.ForeignKey('work_pool.id', ondelete='cascade'), nullable=False, index=True)
    name = sa.Column(sa.String, nullable=False)
    last_heartbeat_time = sa.Column(Timestamp(), nullable=False, server_default=now(), default=lambda : pendulum.now('UTC'), index=True)
...
class ORMAgent:
    ...
    last_activity_time = sa.Column(Timestamp(), nullable=False, server_default=now(), default=lambda : pendulum.now('UTC'))
