...
class RoomKeysServlet(RestServlet):
    PATTERNS = client_patterns('/room_keys/keys(/(?P<room_id>[^/]+))?(/(?P<session_id>[^/]+))?$')
    ...
    async def on_PUT(self, request: SynapseRequest, room_id: Optional[str], session_id: Optional[str]) -> Tuple[int, JsonDict]:
        """
    ...
    async def on_GET(self, request: SynapseRequest, room_id: Optional[str], session_id: Optional[str]) -> Tuple[int, JsonDict]:
        """
        ...
        user_id = requester.user.to_string()
        version = parse_string(request, 'version', required=True)
        room_keys = cast(JsonDict, await self.e2e_room_keys_handler.get_room_keys(user_id, version, room_id, session_id))
        if session_id:
            if room_keys['rooms'] == {}:
    ...
    async def on_DELETE(self, request: SynapseRequest, room_id: Optional[str], session_id: Optional[str]) -> Tuple[int, JsonDict]:
        """
        ...
        user_id = requester.user.to_string()
        version = parse_string(request, 'version', required=True)
        ret = await self.e2e_room_keys_handler.delete_room_keys(user_id, version, room_id, session_id)
        return (200, ret)
...
class RoomKeysVersionServlet(RestServlet):
    ...
    async def on_GET(self, request: SynapseRequest, version: str) -> Tuple[int, JsonDict]:
        """
    ...
    async def on_DELETE(self, request: SynapseRequest, version: str) -> Tuple[int, JsonDict]:
        """
    ...
    async def on_PUT(self, request: SynapseRequest, version: str) -> Tuple[int, JsonDict]:
        """
