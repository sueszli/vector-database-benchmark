...
if __name__ == '__main__':
    ...
    parser.add_argument('--trigger-type-refs', required=False, help='Comma delimited string of trigger type references')
    parser.add_argument('--poll-interval', type=int, default=None, required=False, help='Sensor poll interval')
    parser.add_argument('--parent-args', required=False, help='Command line arguments passed to the parent process')
    ...
    if not isinstance(parent_args, list):
        raise TypeError(f'Command line arguments passed to the parent process must be a list (was {type(parent_args)}).')
    obj = SensorWrapper(pack=args.pack, file_path=args.file_path, class_name=args.class_name, trigger_types=trigger_types, poll_interval=args.poll_interval, parent_args=parent_args)
    obj.run()