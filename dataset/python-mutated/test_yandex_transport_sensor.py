...
NAME = 'test_name'
TEST_BUS_CONFIG = {'sensor': {'platform': 'yandex_transport', 'stop_id': 'stop__9639579', 'routes': ROUTES, 'name': NAME}}
TEST_SUBURBAN_CONFIG = {'sensor': {'platform': 'yandex_transport', 'stop_id': 'station__lh_9876336', 'name': NAME}}
FILTERED_ATTRS = {'т36': ['18:25', '18:42', '18:46'], 'т47': ['18:35', '18:37', '18:40', '18:42'], 'м10': ['18:20', '18:27', '18:29', '18:41', '18:43'], 'stop_name': '7-й автобусный парк', 'attribution': 'Data provided by maps.yandex.ru'}
BUS_RESULT_STATE = dt_util.utc_from_timestamp(1583421540).isoformat(timespec='seconds')
SUBURBAN_RESULT_STATE = dt_util.utc_from_timestamp(1634984640).isoformat(timespec='seconds')
...
async def test_setup_platform_valid_config(hass: HomeAssistant, mock_requester_bus) -> None:
    """Test that sensor is set up properly with valid config."""
    await assert_setup_sensor(hass, TEST_BUS_CONFIG)

async def test_setup_platform_invalid_config(hass: HomeAssistant, mock_requester_bus) -> None:
    """Check an invalid configuration."""
    await assert_setup_sensor(hass, {'sensor': {'platform': 'yandex_transport', 'stopid': 1234}}, count=0)
...
async def test_suburban_trains(hass: HomeAssistant, mock_requester_suburban_train) -> None:
