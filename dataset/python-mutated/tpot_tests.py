...
(training_features, testing_features, training_target, testing_target) = train_test_split(digits_data.data.astype(np.float64), digits_data.target.astype(np.float64), random_state=42)
features_with_nan = np.copy(training_features)
features_with_nan[0][0] = float('nan')
boston_data = load_boston()
(training_features_r, testing_features_r, training_target_r, testing_target_r) = train_test_split(boston_data.data, boston_data.target, random_state=42)
...
custom_cvs = [model_selection.LeaveOneGroupOut().split(X=pretest_X, y=pretest_y, groups=groups), model_selection.LeavePGroupsOut(2).split(X=pretest_X, y=pretest_y, groups=groups), model_selection.RepeatedStratifiedKFold(), 5, 10]
input_data = pd.read_csv('tests/tests.csv', sep=',', dtype=np.float64)
...
(TPOTSelectPercentile, TPOTSelectPercentile_args) = TPOTOperatorClassFactory(test_operator_key, classifier_config_dict[test_operator_key])
