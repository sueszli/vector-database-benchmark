...
from release_test_util import train_ray, FailureState, FailureInjection, TrackingCallback
if __name__ == '__main__':
    ...
    ray_params = RayParams(elastic_training=True, max_failed_actors=2, max_actor_restarts=3, num_actors=4, cpus_per_actor=4, gpus_per_actor=0)
    world_sizes = []
    start_actors = []
    ...
    with patch('xgboost_ray.main._train') as mocked:
        ...
        (_, additional_results, _) = train_ray(path='/data/classification.parquet', num_workers=4, num_boost_rounds=100, num_files=200, regression=False, use_gpu=False, ray_params=ray_params, xgboost_params=None, callbacks=[TrackingCallback(), FailureInjection(id='first_fail', state=failure_state, ranks=[2], iteration=14), FailureInjection(id='second_fail', state=failure_state, ranks=[0], iteration=34)])
    ...
    if 3 not in actor_1_world_size and 3 not in world_sizes and (1 not in world_sizes):
        warnings.warn('No training with only 3 actors observed, but this was elastic training. Please check the output to see if data loading was too fast so that the training actors were re-integrated directly after restarting.')
    print('PASSED.')