...
def set_initial_value_for_bot_creation_policy(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    if False:
        while True:
            i = 10
...
class Migration(migrations.Migration):
    dependencies = [('zerver', '0142_userprofile_translate_emoticons')]
    operations = [migrations.AddField(model_name='realm', name='bot_creation_policy', field=models.PositiveSmallIntegerField(default=BOT_CREATION_EVERYONE)), migrations.RunPython(set_initial_value_for_bot_creation_policy, reverse_code=reverse_code, elidable=True)]