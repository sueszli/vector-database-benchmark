...
from .. import command_arguments, configuration as configuration_module, frontend_configuration, json_rpc, log, log_lsp_event
from ..language_server import connections, protocol as lsp
from . import commands, start
from .initialization import InitializationExit, InitializationFailure, InitializationSuccess
...
async def try_initialize(input_channel: connections.AsyncTextReader, output_channel: connections.AsyncTextWriter) -> Union[InitializationSuccess, InitializationFailure, InitializationExit]:
    """
...
class PysaServer:
    ...
    async def show_message_to_client(self, message: str, level: lsp.MessageType=lsp.MessageType.INFO) -> None:
        await lsp.write_json_rpc(self.output_channel, json_rpc.Request(method='window/showMessage', parameters=json_rpc.ByNameParameters({'type': int(level), 'message': message})))

    async def log_and_show_message_to_client(self, message: str, level: lsp.MessageType=lsp.MessageType.INFO) -> None:
    ...
    async def process_open_request(self, parameters: lsp.DidOpenTextDocumentParameters) -> None:
    ...
    async def process_did_save_request(self, parameters: lsp.DidSaveTextDocumentParameters) -> None:
        document_path = parameters.text_document.document_uri().to_file_path()
        if document_path is None:
            raise json_rpc.InvalidRequestError(f'Document URI is not a file: {parameters.text_document.uri}')
        ...
        while True:
            ...
            try:
                if request.method == 'exit':
                    ...
                    await lsp.write_json_rpc(self.output_channel, json_rpc.SuccessResponse(id=request.id, result=None))
                    return await self.wait_for_exit()
                elif request.method == 'textDocument/didOpen':
                    parameters = request.parameters
                    if parameters is None:
                        raise json_rpc.InvalidRequestError('Missing parameters for didOpen method')
                    await self.process_open_request(lsp.DidOpenTextDocumentParameters.from_json_rpc_parameters(parameters))
                elif request.method == 'textDocument/didClose':
                    parameters = request.parameters
                    if parameters is None:
                        raise json_rpc.InvalidRequestError('Missing parameters for didClose method')
                    await self.process_close_request(lsp.DidCloseTextDocumentParameters.from_json_rpc_parameters(parameters))
                elif request.method == 'textDocument/didSave':
                    parameters = request.parameters
                    if parameters is None:
                        raise json_rpc.InvalidRequestError('Missing parameters for didSave method')
                    await self.process_did_save_request(lsp.DidSaveTextDocumentParameters.from_json_rpc_parameters(parameters))
                elif request.id is not None:
                    raise lsp.RequestCancelledError('Request not supported yet')
            except json_rpc.JSONRPCException as json_rpc_error:
                LOG.debug(f'Exception occurred while processing request: {json_rpc_error}')
...
async def run_persistent(binary_location: str, project_identifier: str, pysa_arguments: start.Arguments) -> int:
    ...
    while True:
        ...
        if isinstance(initialize_result, InitializationExit):
            ...
            log_lsp_event.log(remote_logging=pysa_arguments.base_arguments.remote_logging, event=log_lsp_event.LSPEvent.INITIALIZED, normals={} if client_info is None else {'lsp client name': client_info.name, 'lsp client version': client_info.version})
            client_capabilities = initialize_result.client_capabilities
            LOG.debug(f'Client capabilities: {client_capabilities}')
            server = PysaServer(input_channel=stdin, output_channel=stdout, client_capabilities=client_capabilities, binary_location=binary_location, project_identifier=project_identifier, pyre_arguments=pysa_arguments)
            return await server.run()
        elif isinstance(initialize_result, InitializationFailure):
            exception = initialize_result.exception
            message = str(exception) if exception is not None else 'ignoring notification'
