...
from azure.core.serialization import _datetime_as_isostr
...
job_requested_worker_selectors = [RouterWorkerSelector(key='FakeKey1', label_operator=LabelOperator.EQUAL, value=True), RouterWorkerSelector(key='FakeKey2', label_operator=LabelOperator.NOT_EQUAL, value=False), RouterWorkerSelector(key='FakeKey3', label_operator=LabelOperator.LESS_THAN, value=10), RouterWorkerSelector(key='FakeKey4', label_operator=LabelOperator.LESS_THAN_OR_EQUAL, value=10.01), RouterWorkerSelector(key='FakeKey5', label_operator=LabelOperator.GREATER_THAN, value=10), RouterWorkerSelector(key='FakeKey6', label_operator=LabelOperator.GREATER_THAN_OR_EQUAL, value=10)]
prioritization_rules = [StaticRouterRule(value=10)]
cp_worker_selectors = [StaticWorkerSelectorAttachment(worker_selector=RouterWorkerSelector(key='FakeKeyFromCp', label_operator=LabelOperator.EQUAL, value='FakeValue'))]
expected_attached_worker_selectors = [RouterWorkerSelector(key='FakeKeyFromCp', label_operator=LabelOperator.EQUAL, value='FakeValue')]
...
class TestRouterJobAsync(AsyncRouterRecordedTestCase):
    ...
    async def setup_job_queue(self):
        ...
        async with client:
            ...
            job_queue: RouterQueue = RouterQueue(name=job_queue_id, labels=job_labels, distribution_policy_id=self.get_distribution_policy_id())
    ...
    def get_fallback_queue_id(self):
        if False:
            i = 10
            return i + 15
        return '_' + self._testMethodName + '_tst_flbk_q_async'
    ...
    async def test_create_job_direct_q(self):
        ...
        router_client: JobRouterClient = self.create_client()
        async with router_client:
            router_job: RouterJob = RouterJob(channel_reference=job_channel_references[0], channel_id=job_channel_ids[0], queue_id=self.get_job_queue_id(), priority=job_priority, requested_worker_selectors=job_requested_worker_selectors, labels=job_labels, tags=job_tags, notes=job_notes)
    ...
    async def test_create_job_w_cp(self):
        ...
        router_client: JobRouterClient = self.create_client()
        async with router_client:
            router_job: RouterJob = RouterJob(channel_reference=job_channel_references[0], channel_id=job_channel_ids[0], classification_policy_id=self.get_classification_policy_id(), requested_worker_selectors=job_requested_worker_selectors, labels=job_labels, tags=job_tags, notes=job_notes)
            router_job = await router_client.upsert_job(job_identifier, router_job)
            self.job_ids[self._testMethodName] = [job_identifier]
            assert router_job is not None
    ...
    async def test_list_jobs(self):
        ...
        async with router_client:
            ...
            router_jobs = router_client.list_jobs(results_per_page=3, status=RouterJobStatus.QUEUED, queue_id=self.get_job_queue_id(), channel_id=job_channel_ids[0])
    ...
    async def test_list_sch_jobs(self, **kwargs):
        ...
        async with router_client:
            for identifier in job_identifiers:
                router_job: RouterJob = RouterJob(channel_reference=job_channel_references[0], channel_id=job_channel_ids[0], queue_id=self.get_job_queue_id(), priority=job_priority, requested_worker_selectors=job_requested_worker_selectors, labels=job_labels, tags=job_tags, notes=job_notes, matching_mode=matching_mode)
                router_job = await router_client.upsert_job(identifier, router_job)
                self.job_ids[self._testMethodName].append(identifier)
                assert router_job is not None
                ...
                created_job_response[router_job.id] = router_job
            router_jobs = router_client.list_jobs(results_per_page=2, status=RouterJobStatus.SCHEDULED, queue_id=self.get_job_queue_id(), scheduled_before=recorded_variables['scheduled_time_utc'], channel_id=job_channel_ids[0])
            async for router_job_page in router_jobs.by_page():
                list_of_jobs = [i async for i in router_job_page]
