...
if is_tf_available():
    ...
    from transformers.models.openai.modeling_tf_openai import TF_OPENAI_GPT_PRETRAINED_MODEL_ARCHIVE_LIST, TFOpenAIGPTDoubleHeadsModel, TFOpenAIGPTForSequenceClassification, TFOpenAIGPTLMHeadModel, TFOpenAIGPTModel
...
class TFOpenAIGPTModelTester:
    ...
    def create_and_check_openai_gpt_for_sequence_classification(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        if False:
            i = 10
            return i + 15
...
class TFOpenAIGPTModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TFOpenAIGPTModel, TFOpenAIGPTLMHeadModel, TFOpenAIGPTDoubleHeadsModel, TFOpenAIGPTForSequenceClassification) if is_tf_available() else ()
    all_generative_model_classes = (TFOpenAIGPTLMHeadModel,) if is_tf_available() else ()
    pipeline_model_mapping = {'feature-extraction': TFOpenAIGPTModel, 'text-classification': TFOpenAIGPTForSequenceClassification, 'text-generation': TFOpenAIGPTLMHeadModel, 'zero-shot': TFOpenAIGPTForSequenceClassification} if is_tf_available() else {}
...
class TFOPENAIGPTModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_lm_generate_openai_gpt(self):
        ...
        input_ids = tf.convert_to_tensor([[481, 4735, 544]], dtype=tf.int32)
