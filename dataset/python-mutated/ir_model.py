...
class IrModel(models.Model):
    ...
    inherited_model_ids = fields.Many2many('ir.model', compute='_inherited_models', string='Inherited models', help='The list of models that extends the current model.')
    ...
    _sql_constraints = [('obj_name_uniq', 'unique (model)', 'Each model must be unique!')]
...
class IrModelFields(models.Model):
    ...
    model = fields.Char(string='Object Name', required=True, index=True, help='The technical name of the model this field belongs to')
    relation = fields.Char(string='Object Relation', help='For relationship fields, the technical name of the target model')
    ...
    model_id = fields.Many2one('ir.model', string='Model', required=True, index=True, ondelete='cascade', help='The model this field belongs to')
    ...
    selection = fields.Char(string='Selection Options', default='', help="List of options for a selection field, specified as a Python expression defining a list of (key, label) pairs. For example: [('blue','Blue'),('yellow','Yellow')]")
    ...
    on_delete = fields.Selection([('cascade', 'Cascade'), ('set null', 'Set NULL'), ('restrict', 'Restrict')], string='On Delete', default='set null', help='On delete property for many2one fields')
    domain = fields.Char(default='[]', help="The optional domain to restrict possible values for relationship fields, specified as a Python expression defining a list of triplets. For example: [('color','=','red')]")
    ...
    serialization_field_id = fields.Many2one('ir.model.fields', 'Serialization Field', domain="[('ttype','=','serialized')]", ondelete='cascade', help='If set, this field will be stored in the sparse structure of the serialization field, instead of having its own database column. This cannot be changed after creation.')
    ...
    depends = fields.Char(string='Dependencies', help='Dependencies of compute method; a list of comma-separated field names, like\n\n    name, partner_id.name')
    ...
    _sql_constraints = [('size_gt_zero', 'CHECK (size>=0)', 'Size of the field cannot be negative.')]
...
class IrModelConstraint(models.Model):
    ...
    name = fields.Char(string='Constraint', required=True, index=True, help='PostgreSQL constraint or foreign key name.')
    ...
    type = fields.Char(string='Constraint Type', required=True, size=1, index=True, help='Type of the constraint: `f` for a foreign key, `u` for other constraints.')
    date_update = fields.Datetime(string='Update Date')
    date_init = fields.Datetime(string='Initialization Date')
    _sql_constraints = [('module_name_uniq', 'unique(name, module)', 'Constraints with the same name are unique per module.')]
...
class IrModelRelation(models.Model):
    ...
    name = fields.Char(string='Relation Name', required=True, index=True, help='PostgreSQL table name implementing a many2many relation.')
    model = fields.Many2one('ir.model', required=True, index=True)
    module = fields.Many2one('ir.module.module', required=True, index=True)
    date_update = fields.Datetime(string='Update Date')
    date_init = fields.Datetime(string='Initialization Date')
...
class IrModelData(models.Model):
    ...
    name = fields.Char(string='External Identifier', required=True, help='External Key/Identifier that can be used for data integration with third-party systems')
