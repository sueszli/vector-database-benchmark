...
from typing import Any, Union, List, Optional, Iterable, Dict, Tuple, Set, Callable, cast
...
class ServiceEnumerator(AutomotiveTestCase, metaclass=abc.ABCMeta):
    ...
    _supported_kwargs.update({'timeout': ((int, float), lambda x: x > 0), 'count': (int, lambda x: x >= 0), 'execution_time': (int, None), 'state_allow_list': ((list, EcuState), None), 'state_block_list': ((list, EcuState), None), 'retry_if_none_received': (bool, None), 'exit_if_no_answer_received': (bool, None), 'exit_if_service_not_supported': (bool, None), 'exit_scan_on_first_negative_response': (bool, None), 'retry_if_busy_returncode': (bool, None), 'stop_event': (threading.Event, None), 'debug': (bool, None), 'scan_range': ((list, tuple, range), None), 'unittest': (bool, None)})
...
class StateGeneratingServiceEnumerator(ServiceEnumerator, StateGenerator, metaclass=abc.ABCMeta):
