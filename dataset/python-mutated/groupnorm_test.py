...
groupnorm_configs_short = op_bench.cross_product_configs(dims=((32, 8, 16), (32, 8, 56, 56)), num_groups=(2, 4), tags=['short'])
...
class GroupNormBenchmark(op_bench.TorchBenchmarkBase):
    ...
    def init(self, dims, num_groups):
        if False:
            ...
            return i + 15
        num_channels = dims[1]
        self.inputs = {'input': (torch.rand(*dims) - 0.5) * 256, 'num_groups': num_groups, 'weight': torch.rand(num_channels, dtype=torch.float), 'bias': torch.rand(num_channels, dtype=torch.float), 'eps': 1e-05}
