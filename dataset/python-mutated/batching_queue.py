import logging
from typing import Awaitable, Callable, Dict, Generic, Hashable, List, Set, Tuple, TypeVar
...
V = TypeVar('V')
R = TypeVar('R')
number_queued = Gauge('synapse_util_batching_queue_number_queued', 'The number of items waiting in the queue across all keys', labelnames=('name',))
number_in_flight = Gauge('synapse_util_batching_queue_number_pending', 'The number of items across all keys either being processed or waiting in a queue', labelnames=('name',))
number_of_keys = Gauge('synapse_util_batching_queue_number_of_keys', 'The number of distinct keys that have items queued', labelnames=('name',))
