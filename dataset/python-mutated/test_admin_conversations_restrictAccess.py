...
class TestWebClient(unittest.TestCase):
    ...
    async def test_async(self):
        ...
        add_group = await client.admin_conversations_restrictAccess_addGroup(channel_id=self.channel_id, group_id=self.idp_group_id, team_id=self.team_id)
        self.assertIsNotNone(add_group)
        await asyncio.sleep(10)
        list_groups = await client.admin_conversations_restrictAccess_listGroups(team_id=self.team_id, channel_id=self.channel_id)
        self.assertIsNotNone(list_groups)
        await asyncio.sleep(10)
        remove_group = await client.admin_conversations_restrictAccess_removeGroup(channel_id=self.channel_id, group_id=self.idp_group_id, team_id=self.team_id)
        self.assertIsNotNone(remove_group)
