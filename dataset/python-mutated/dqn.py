...
Transition = collections.namedtuple('Transition', 'info_state action reward next_info_state is_final_step legal_actions_mask')
ILLEGAL_ACTION_LOGITS_PENALTY = -1000000000.0
...
class DQN(rl_agent.AbstractAgent):
  ...
  """
    ...
    def __init__(self, session, player_id, state_representation_size, num_actions, hidden_layers_sizes=128, replay_buffer_capacity=10000, batch_size=128, replay_buffer_class=ReplayBuffer, learning_rate=0.01, update_target_network_every=1000, learn_every=10, discount_factor=1.0, min_buffer_size_to_learn=1000, epsilon_start=1.0, epsilon_end=0.1, epsilon_decay_duration=int(1000000.0), optimizer_str='sgd', loss_str='mse'):
        if False:
            print('Hello World!')
        ...
        self._reward_ph = tf.placeholder(shape=[None], dtype=tf.float32, name='reward_ph')
        self._is_final_step_ph = tf.placeholder(shape=[None], dtype=tf.float32, name='is_final_step_ph')
        self._next_info_state_ph = tf.placeholder(shape=[None, state_representation_size], dtype=tf.float32, name='next_info_state_ph')
        ...
        self._loss = tf.reduce_mean(loss_class(labels=target, predictions=predictions))
    ...
    def add_transition(self, prev_time_step, prev_action, time_step):
        ...
        transition = Transition(info_state=prev_time_step.observations['info_state'][self.player_id][:], action=prev_action, reward=time_step.rewards[self.player_id], next_info_state=time_step.observations['info_state'][self.player_id][:], is_final_step=float(time_step.last()), legal_actions_mask=legal_actions_mask)
    ...
    def _epsilon_greedy(self, info_state, legal_actions, epsilon):
        ...
        if np.random.rand() < epsilon:
            ...
            q_values = self._session.run(self._q_values, feed_dict={self._info_state_ph: info_state})[0]
    ...
    def _get_epsilon(self, is_evaluation, power=1.0):
        ...
        decayed_epsilon = self._epsilon_end + (self._epsilon_start - self._epsilon_end) * (1 - decay_steps / self._epsilon_decay_duration) ** power
    ...
    def learn(self):
        ...
        (loss, _) = self._session.run([self._loss, self._learn_step], feed_dict={self._info_state_ph: info_states, self._action_ph: actions, self._reward_ph: rewards, self._is_final_step_ph: are_final_steps, self._next_info_state_ph: next_info_states, self._legal_actions_mask_ph: legal_actions_mask})
        return loss
    ...
    def _initialize(self):
        if False:
            for i in range(10):
                print('nop')
        initialization_weights = tf.group(*[var.initializer for var in self._variables])
        initialization_target_weights = tf.group(*[var.initializer for var in self._target_variables])
    ...
    def copy_with_noise(self, sigma=0.0, copy_weights=True):
        ...
        if copy_weights:
            copy_weights = tf.group(*[va.assign(vb * (1 + sigma * tf.random.normal(vb.shape))) for (va, vb) in zip(q_network.variables, self._q_network.variables)])
            self._session.run(copy_weights)
            copy_target_weights = tf.group(*[va.assign(vb * (1 + sigma * tf.random.normal(vb.shape))) for (va, vb) in zip(target_q_network.variables, self._target_q_network.variables)])
