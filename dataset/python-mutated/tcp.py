...
class TCPPubClient(salt.transport.base.PublishClient):
    ...
    """
    ttype = 'tcp'
    async_methods = ['connect', 'connect_uri', 'recv']
    close_methods = ['close']
        ...
        while stream is None and (not self._closed and (not self._closing)):
            try:
                if self.host and self.port:
                    log.debug('PubClient connecting to %r %r:%r', self, self.host, self.port)
                    self._tcp_client = TCPClientKeepAlive(self.opts, resolver=self.resolver)
                    stream = await asyncio.wait_for(self._tcp_client.connect(ip_bracket(self.host, strip=True), self.port, ssl_options=self.opts.get('ssl'), **kwargs), 1)
                    self.unpacker = salt.utils.msgpack.Unpacker()
                    log.debug('PubClient conencted to %r %r:%r', self, self.host, self.port)
                else:
                    log.debug('PubClient connecting to %r %r', self, self.path)
                    sock_type = socket.AF_UNIX
                    stream = tornado.iostream.IOStream(socket.socket(sock_type, socket.SOCK_STREAM))
                ...
                log.warning('TCP Publish Client encountered an exception while connecting to %s: %r, will reconnect in %d seconds', _connect_to, exc, self.backoff)
    ...
    async def connect(self, port=None, connect_callback=None, disconnect_callback=None, timeout=None):
        if port is not None:
            self.port = port
        if connect_callback:
        ...
        while True:
            ...
            if msg:
                try:
                    ...
                    log.error('Unhandled exception while running callback %r', self, exc_info=True)
...
class SaltMessageServer(tornado.tcpserver.TCPServer):
    ...
    async def handle_stream(self, stream, address, _StreamClosedError=tornado.iostream.StreamClosedError):
        """
        ...
        try:
            while True:
                ...
                for framed_msg in unpacker:
                    ...
                    self.io_loop.spawn_callback(self.message_handler, stream, framed_msg['body'], header)
...
class PubServer(tornado.tcpserver.TCPServer):
    ...
    async def publish_payload(self, package, topic_list=None):
        log.trace('TCP PubServer sending payload: topic_list=%r %r', topic_list, package)
    ...
    async def handle_stream(self, stream):
        ...
        while not stream.closed():
            try:
                ...
                if exc.errno == 0:
                    log.trace('Exception occurred with error number 0, spurious exception: %s', exc)
                else:
...
class TCPPublishServer(salt.transport.base.DaemonizedPublishServer):
    ...
    async def publisher(self, publish_payload, presence_callback=None, remove_presence_callback=None, io_loop=None):
        if io_loop is None:
            io_loop = tornado.ioloop.IOLoop.current()
        self.pub_server = pub_server = PubServer(self.opts, io_loop=io_loop, presence_callback=presence_callback, remove_presence_callback=remove_presence_callback)
        if self.pub_path:
            log.debug('Publish server binding pub to %s', self.pub_path)
            sock = tornado.netutil.bind_unix_socket(self.pub_path)
        else:
            log.debug('Publish server binding pub to %s:%s', self.pub_host, self.pub_port)
            sock = _get_socket(self.opts)
        ...
        if self.pull_path:
            ...
            log.debug('Publish server binding pull to %s:%s', self.pull_host, self.pull_port)
            pull_host = self.pull_host
            pull_port = self.pull_port
        self.pull_sock = TCPPuller(host=self.pull_host, port=self.pull_port, path=self.pull_path, io_loop=io_loop, payload_handler=publish_payload)
...
class _TCPPubServerPublisher:
    ...
    """
    async_methods = ['send', 'connect', '_connect']
    close_methods = ['close']
    ...
    async def _connect(self, timeout=None):
        ...
        while True:
            ...
            if self.stream is None:
                self.stream = tornado.iostream.IOStream(socket.socket(sock_type, socket.SOCK_STREAM))
            try:
...
class TCPReqClient(salt.transport.base.RequestClient):
    ...
    async def getstream(self, **kwargs):
        ...
        while stream is None and (not self._closed and (not self._closing)):
            try:
                stream = await self._tcp_client.connect(ip_bracket(self.host, strip=True), self.port, ssl_options=self.opts.get('ssl'), **kwargs)
            except Exception as exc:
                log.warning('TCP Message Client encountered an exception while connecting to %s:%s: %r, will reconnect in %d seconds', self.host, self.port, exc, self.backoff)
                await asyncio.sleep(self.backoff)
        return stream
    ...
    async def _stream_return(self):
        ...
        while not self._closing:
            try:
                ...
                log.error('tcp stream to %s:%s closed, unable to recv', self.host, self.port)
                for future in self.send_future_map.values():
                    future.set_exception(e)
                self.send_future_map = {}
                if self._closing or self._closed:
