...
class CenterCrop(GeometricAugmentationBase2D):
    ...
    def __init__(self, size: Union[int, Tuple[int, int]], align_corners: bool=True, resample: Union[str, int, Resample]=Resample.BILINEAR.name, p: float=1.0, keepdim: bool=False, cropping_mode: str='slice') -> None:
        if False:
            while True:
                i = 10
        ...
        self.flags = {'resample': Resample.get(resample), 'cropping_mode': cropping_mode, 'align_corners': align_corners, 'size': self.size, 'padding_mode': 'zeros'}
    ...
    def apply_transform(self, input: Tensor, params: Dict[str, Tensor], flags: Dict[str, Any], transform: Optional[Tensor]=None) -> Tensor:
        ...
        if flags['cropping_mode'] == 'resample':
            ...
            return crop_by_transform_mat(input, transform[:, :2, :], self.size, flags['resample'].name.lower(), 'zeros', flags['align_corners'])
