...
pytestmark = [pytest.mark.filterwarnings('ignore:unclosed <socket.socket fd=.*:ResourceWarning'), pytest.mark.filterwarnings('ignore:unclosed transport <_SelectorSocketTransport closing fd=.*:ResourceWarning')]
secure_proxy_xfail = functools.partial(pytest.mark.xfail, PY_310 and platform.system() != 'Darwin' or platform.system() == 'Windows', reason='The secure proxy fixture does not seem to work under Python 3.10 on Linux and any Python on Windows. See https://github.com/abhinavsingh/proxy.py/issues/622.')
ASYNCIO_SUPPORTS_TLS_IN_TLS = hasattr(asyncio.sslproto._SSLProtocolTransport, '_start_tls_compatible')
...
async def web_server_endpoint_url(aiohttp_server, ssl_ctx, web_server_endpoint_payload, web_server_endpoint_type):
    server_kwargs = {'ssl': ssl_ctx} if web_server_endpoint_type == 'https' else {}
...
async def test_secure_https_proxy_absolute_path(client_ssl_ctx, secure_proxy_url, web_server_endpoint_url, web_server_endpoint_payload) -> None:
    """Ensure HTTP(S) sites are accessible through a secure proxy."""
    conn = aiohttp.TCPConnector()
    sess = aiohttp.ClientSession(connector=conn)
    response = await sess.get(web_server_endpoint_url, proxy=secure_proxy_url, ssl=client_ssl_ctx)
...
async def test_https_proxy_unsupported_tls_in_tls(client_ssl_ctx, secure_proxy_url, web_server_endpoint_type) -> None:
...
async def test_proxy_http_absolute_path(proxy_test_server: Any, get_request: Any) -> None:
...
async def test_proxy_http_idna_support(proxy_test_server: Any, get_request: Any) -> None:
    url = 'http://éé.com/'
    proxy = await proxy_test_server()
    await get_request(url=url, proxy=proxy.url)
...
async def test_proxy_http_bad_response(proxy_test_server: Any, get_request: Any) -> None:
    url = 'http://aiohttp.io/path'
    proxy = await proxy_test_server()
...
async def test_proxy_http_auth_from_url(proxy_test_server: Any, get_request: Any) -> None:
    url = 'http://aiohttp.io/path'
    proxy = await proxy_test_server()
...
async def test_proxy_http_acquired_cleanup_force(proxy_test_server: Any, loop: Any) -> None:
    url = 'http://aiohttp.io/path'
...
@pytest.mark.xfail
async def xtest_proxy_https_connect_with_port(proxy_test_server: Any, get_request: Any) -> None:
    proxy = await proxy_test_server()
...
@pytest.mark.xfail
async def xtest_proxy_https_idna_support(proxy_test_server: Any, get_request: Any) -> None:
    url = 'https://éé.com/'
    proxy = await proxy_test_server()
    await get_request(url=url, proxy=proxy.url)
    connect = proxy.requests_list[0]
...
async def test_proxy_https_bad_response(proxy_test_server: Any, get_request: Any) -> None:
    url = 'https://secure.aiohttp.io/path'
    proxy = await proxy_test_server()
    proxy.return_value = dict(status=502, headers={'Proxy-Agent': 'TestProxy'})
...
@pytest.mark.xfail
async def xtest_proxy_https_acquired_cleanup_force(proxy_test_server: Any, loop: Any) -> None:
    url = 'https://secure.aiohttp.io/path'
    conn = aiohttp.TCPConnector(force_close=True)
    sess = aiohttp.ClientSession(connector=conn)
    proxy = await proxy_test_server()
...
async def test_proxy_from_env_http(proxy_test_server: Any, get_request: Any, mocker: Any) -> None:
    url = 'http://aiohttp.io/path'
    proxy = await proxy_test_server()
...
async def test_proxy_from_env_http_with_auth(proxy_test_server: Any, get_request: Any, mocker: Any) -> None:
    url = 'http://aiohttp.io/path'
    proxy = await proxy_test_server()
    auth = aiohttp.BasicAuth('user', 'pass')
...
async def test_proxy_from_env_http_with_auth_from_netrc(proxy_test_server: Any, get_request: Any, tmp_path: Any, mocker: Any) -> None:
    url = 'http://aiohttp.io/path'
    proxy = await proxy_test_server()
    auth = aiohttp.BasicAuth('user', 'pass')
...
async def test_proxy_from_env_http_without_auth_from_netrc(proxy_test_server: Any, get_request: Any, tmp_path: Any, mocker: Any) -> None:
    url = 'http://aiohttp.io/path'
    proxy = await proxy_test_server()
    auth = aiohttp.BasicAuth('user', 'pass')
    netrc_file = tmp_path / 'test_netrc'
...
async def test_proxy_from_env_http_without_auth_from_wrong_netrc(proxy_test_server: Any, get_request: Any, tmp_path: Any, mocker: Any) -> None:
    url = 'http://aiohttp.io/path'
    proxy = await proxy_test_server()
    auth = aiohttp.BasicAuth('user', 'pass')
    netrc_file = tmp_path / 'test_netrc'
...
@pytest.mark.xfail
async def xtest_proxy_from_env_https(proxy_test_server: Any, get_request: Any, mocker: Any) -> None:
...
@pytest.mark.xfail
async def xtest_proxy_from_env_https_with_auth(proxy_test_server: Any, get_request: Any, mocker: Any) -> None:
    url = 'https://aiohttp.io/path'
    proxy = await proxy_test_server()
    auth = aiohttp.BasicAuth('user', 'pass')
...
async def test_proxy_auth() -> None:
    async with aiohttp.ClientSession() as session:
        with pytest.raises(ValueError, match='proxy_auth must be None or BasicAuth\\(\\) tuple'):
            await session.get('http://python.org', proxy='http://proxy.example.com', proxy_auth=('user', 'pass'))