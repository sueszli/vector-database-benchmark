...
def upgrade():
    if False:
        for i in range(10):
            print('nop')
    op.create_table('cloudtrail', sa.Column('id', sa.Integer(), nullable=False), sa.Column('event_id', sa.String(length=36), nullable=True), sa.Column('request_id', sa.String(length=36), nullable=True), sa.Column('event_source', sa.String(length=64), nullable=False), sa.Column('event_name', sa.String(length=64), nullable=False), sa.Column('event_time', sa.DateTime(), nullable=False), sa.Column('request_parameters', postgresql.JSON(), nullable=True), sa.Column('responseElements', postgresql.JSON(), nullable=True), sa.Column('source_ip', sa.String(length=45), nullable=True), sa.Column('user_agent', sa.String(length=300), nullable=True), sa.Column('full_entry', postgresql.JSON(), nullable=True), sa.Column('user_identity', postgresql.JSON(), nullable=True), sa.Column('user_identity_arn', sa.String(length=300), nullable=True), sa.Column('revision_id', sa.Integer(), nullable=False), sa.Column('item_id', sa.Integer(), nullable=False), sa.ForeignKeyConstraint(['item_id'], ['item.id']), sa.ForeignKeyConstraint(['revision_id'], ['itemrevision.id']), sa.PrimaryKeyConstraint('id'))
