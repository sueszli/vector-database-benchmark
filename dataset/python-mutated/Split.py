...
parser.add_argument('-f', '--filename', dest='inputfile', required=True, help='The input file to split tool.')
parser.add_argument('-s', '--split', dest='position', required=True, help='The number of bytes in the first file. The valid format are HEX, Decimal and Decimal[KMG].')
parser.add_argument('-p', '--prefix', dest='output', help='The output folder.')
parser.add_argument('-o', '--firstfile', help='The first file name')
parser.add_argument('-t', '--secondfile', help='The second file name')
parser.add_argument('--version', action='version', version='%(prog)s Version 2.0', help='Print debug information.')
group = parser.add_mutually_exclusive_group()
group.add_argument('-v', '--verbose', action='store_true', help='Print debug information.')
group.add_argument('-q', '--quiet', action='store_true', help='Disable all messages except fatal errors')
SizeDict = {'K': 1024, 'M': 1024 * 1024, 'G': 1024 * 1024 * 1024}
