...
class LavalinkTasks(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def lavalink_attempt_connect(self, timeout: int=50, manual: bool=False) -> None:
        ...
        while retry_count < max_retries:
            ...
            if external is False:
                ...
                try:
                    ...
                    log.exception('Unhandled exception whilst starting managed Lavalink node, aborting...', exc_info=exc)
                    self.lavalink_connection_aborted = True
                    if self.managed_node_controller is not None:
                        await self.managed_node_controller.shutdown()
                    return
            ...
            log.critical('Setting up the managed Lavalink node failed after multiple attempts. See above logs for details.')
            self.lavalink_connection_aborted = True
            if self.managed_node_controller is not None:
                await self.managed_node_controller.shutdown()
            return
        ...
        while retry_count < max_retries:
            try:
                await lavalink.initialize(bot=self.bot, host=host, password=password, port=port, timeout=timeout, resume_key=f'Red-Core-Audio-{self.bot.user.id}-{data_manager.instance_name()}', secured=secured)
                ...
                log.exception('Unhandled exception whilst connecting to Lavalink node, aborting...', exc_info=exc)
                await lavalink.close(self.bot)
                self.lavalink_connection_aborted = True
                return
            else:
            ...
            self.lavalink_connection_aborted = True
            log.critical('Connecting to the Lavalink node failed after multiple attempts. See above tracebacks for details.')
            await lavalink.close(self.bot)
            return
        if external:
            await asyncio.sleep(5)
