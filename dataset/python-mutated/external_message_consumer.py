...
from freqtrade.rpc.api_server.ws_schemas import WSAnalyzedDFMessage, WSAnalyzedDFRequest, WSMessageSchema, WSRequestSchema, WSSubscribeRequest, WSWhitelistMessage, WSWhitelistRequest
if TYPE_CHECKING:
...
class ExternalMessageConsumer:
    ...
    async def _main(self):
        ...
        lock = asyncio.Lock()
        try:
            self._sub_tasks = [self._loop.create_task(self._handle_producer_connection(producer, lock)) for producer in self.producers]
    ...
    async def _create_connection(self, producer: Producer, lock: asyncio.Lock):
        ...
        while self._running:
            try:
                ...
                async with websockets.connect(ws_url, max_size=self.message_size_limit, ping_interval=None) as ws:
                    async with create_channel(ws, channel_id=name, send_throttle=0.5) as channel:
                        self._channel_streams[name] = MessageStream()
                        await channel.run_channel_tasks(self._receive_messages(channel, producer, lock), self._send_requests(channel, self._channel_streams[name]))
    ...
    async def _receive_messages(self, channel: WebSocketChannel, producer: Producer, lock: asyncio.Lock):
        """
