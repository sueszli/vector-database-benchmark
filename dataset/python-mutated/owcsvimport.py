...
import typing
from typing import List, Tuple, Dict, Optional, Any, Callable, Iterable, Union, AnyStr, BinaryIO, Set, Type, Mapping, Sequence, NamedTuple
from AnyQt.QtCore import Qt, QFileInfo, QTimer, QSettings, QObject, QSize, QMimeDatabase, QMimeType
from AnyQt.QtGui import QStandardItem, QStandardItemModel, QPalette, QColor, QIcon
from AnyQt.QtWidgets import QLabel, QComboBox, QPushButton, QDialog, QDialogButtonBox, QGridLayout, QVBoxLayout, QSizePolicy, QStyle, QFileIconProvider, QFileDialog, QApplication, QMessageBox, QTextBrowser, QMenu
...
from Orange.widgets.utils import textimport, concurrent as qconcurrent, unique_everseen, enum_get, qname
from Orange.widgets.utils.combobox import ItemStyledComboBox
from Orange.widgets.utils.pathutils import PathItem, VarPath, AbsPath, samepath, prettyfypath, isprefixed
from Orange.widgets.utils.overlay import OverlayWidget
from Orange.widgets.utils.settings import QSettings_readArray, QSettings_writeArray
...
class FileFormat(NamedTuple):
    ...
    name: str
    globs: Sequence[str]
FileFormats = [FileFormat('text/csv', 'Text - comma separated', ('*.csv', '*')), FileFormat('text/tab-separated-values', 'Text - tab separated', ('*.tsv', '*')), FileFormat('text/plain', 'Text - all files', ('*.txt', '*'))]
...
class OWCSVFileImport(widget.OWWidget):
    ...
    class Outputs:
        data = widget.Output(name='Data', type=Orange.data.Table, doc='Loaded data set.')
        data_frame = widget.Output(name='Data Frame', type=pd.DataFrame, doc='', auto_summary=False)

    class Error(widget.OWWidget.Error):
        error = widget.Msg('Unexpected error')
        encoding_error = widget.Msg('Encoding error\nThe file might be encoded in an unsupported encoding or it might be binary')
    _session_items = settings.Setting([], schema_only=True)
    _session_items_v2 = settings.Setting([], schema_only=True)
    dialog_state = settings.Setting({'directory': '', 'filter': ''})
    settings_version = 3
...
compression_types = ['application/gzip', 'application/zip', 'application/x-xz', 'application/x-bzip']
...
NA_VALUES = {ColumnType.Numeric: NA_DEFAULT, ColumnType.Categorical: NA_DEFAULT, ColumnType.Time: NA_DEFAULT + ['NaT', 'NAT'], ColumnType.Text: [], ColumnType.Auto: NA_DEFAULT}
