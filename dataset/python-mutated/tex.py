...
re_bibunit = re.compile('\\\\(?P<type>putbib)\\[(?P<file>[^\\[\\]]*)\\]', re.M)
...
exts_tex = ['.ltx', '.tex']
re_tex = re.compile('\\\\(?P<type>usepackage|RequirePackage|include|bibliography([^\\[\\]{}]*)|putbib|includegraphics|input|import|bringin|lstinputlisting)(\\[[^\\[\\]]*\\])?{(?P<file>[^{}]*)}', re.M)
