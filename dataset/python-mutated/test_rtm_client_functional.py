...
class TestRTMClientFunctional(unittest.TestCase):
    ...
    async def test_run_async_valid(self):
        client = slack.RTMClient(token='xoxb-valid', base_url='http://localhost:8765', run_async=True)
        client._web_client = slack.WebClient(token='xoxb-valid', base_url='http://localhost:8888', run_async=True)
        self.called = False
