...
bool_cols = ['engaged_with_user_is_verified', 'enaging_user_is_verified']
count_cols = ['engaged_with_user_follower_count', 'engaged_with_user_following_count', 'enaging_user_follower_count', 'enaging_user_following_count']
timestamp_cols = ['reply_timestamp', 'retweet_timestamp', 'retweet_with_comment_timestamp', 'like_timestamp']
cat_cols = ['engaged_with_user_id', 'enaging_user_id', 'present_media', 'tweet_type', 'language']
list_cols = ['hashtags', 'present_domains', 'present_links']
len_cols = ['len_hashtags', 'len_domains', 'len_links']
media_map = {'': 0, 'GIF': 1, 'GIF_GIF': 2, 'GIF_Photo': 3, 'GIF_Video': 4, 'Photo': 5, 'Photo_GIF': 6, 'Photo_Photo': 7, 'Photo_Video': 8, 'Video': 9, 'Video_GIF': 10, 'Video_Photo': 11, 'Video_Video': 12}
type_map = {'Quote': 0, 'Retweet': 1, 'TopLevel': 2}
cross_cols = [['present_media', 'language']]
enaging_features = ['enaging_user_follower_count', 'enaging_user_following_count', 'enaging_user_id', 'enaging_user_is_verified']
engaged_features = ['engaged_with_user_follower_count', 'engaged_with_user_following_count', 'engaged_with_user_id', 'engaged_with_user_is_verified']
user_features = ['follower_count', 'following_count', 'user_id', 'is_verified']
item_features = ['present_media', 'language', 'tweet_id', 'tweet_type', 'engaged_with_user_follower_count', 'engaged_with_user_following_count', 'len_hashtags', 'len_domains', 'len_links', 'present_media_language', 'engaged_with_user_is_verified', 'engaged_with_user_id', 'hashtags', 'present_domains']
conf = {'spark.network.timeout': '10000000', 'spark.sql.broadcastTimeout': '7200', 'spark.sql.shuffle.partitions': '2000', 'spark.locality.wait': '0s', 'spark.sql.crossJoin.enabled': 'true', 'spark.task.cpus': '1', 'spark.executor.heartbeatInterval': '200s', 'spark.driver.maxResultSize': '40G', 'spark.eventLog.enabled': 'true', 'spark.eventLog.dir': 'hdfs://172.16.0.105:8020/sparkHistoryLogs', 'spark.app.name': 'recsys-preprocess', 'spark.executor.memoryOverhead': '20g'}
...
if __name__ == '__main__':
    ...
    if args.cluster_mode == 'local':
        ...
        init_orca_context('standalone', master=args.master, cores=args.executor_cores, num_nodes=args.num_executor, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, conf=conf)
    elif args.cluster_mode == 'yarn':
        init_orca_context('yarn-client', cores=args.executor_cores, num_nodes=args.num_executor, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, conf=conf)
    else:
        invalidInputError(False, "cluster_mode should be one of 'local', 'yarn' and 'standalone', but got " + args.cluster_mode)
    start = time()
    ...
    if args.train_files != 'all':
        train_paths = [os.path.join(input_train_folder, 'part-%05d*.parquet' % i) for i in args.train_files]
    ...
    user_index = full_tbl.gen_string_idx({'src_cols': ['engaged_with_user_id', 'enaging_user_id'], 'col_name': 'user_id'})
    ...
    train_tbl = train_tbl.encode_string(list_cols, indexes, do_split=True, sep='\t', keep_most_frequent=True)
    test_tbl = test_tbl.encode_string(list_cols, indexes, do_split=True, sep='\t', keep_most_frequent=True)
    train_tbl = train_tbl.fillna(0, list_cols)
    test_tbl = test_tbl.fillna(0, list_cols)
    train_tbl.cache()
    test_tbl.cache()
    bins = [1, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 10000000.0]
    ...
    full_tbl = train_tbl.concat(test_tbl)
    enaging_user = full_tbl.select(enaging_features).rename(dict(zip(enaging_features, user_features)))
    engaged_user = full_tbl.select(engaged_features).rename(dict(zip(engaged_features, user_features)))
    enaging_user.concat(engaged_user).drop_duplicates().write_parquet(os.path.join(output_folder, 'wnd_user.parquet'))
    full_tbl.select(item_features).drop_duplicates().write_parquet(os.path.join(output_folder, 'wnd_item.parquet'))
    cat_sizes_dict = {}
    ...
    cross_sizes_dict = dict(zip(['_'.join(cross_names) for cross_names in cross_cols], args.cross_sizes))
    ...
    with tempfile.TemporaryDirectory() as local_path:
        ...
        put_local_file_to_remote(os.path.join(local_path, 'categorical_sizes.pkl'), os.path.join(output_folder, 'meta/categorical_sizes.pkl'), over_write=True)
    end = time()
