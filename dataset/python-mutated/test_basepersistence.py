...
from telegram.ext import Application, ApplicationBuilder, ApplicationHandlerStop, BaseHandler, BasePersistence, CallbackContext, ConversationHandler, ExtBot, MessageHandler, PersistenceInput, filters
...
class TrackingPersistence(BasePersistence):
    ...
    def __init__(self, store_data: Optional[PersistenceInput]=None, update_interval: float=60, fill_data: bool=False):
        ...
        if fill_data:
            self.fill()
    CALLBACK_DATA = ([('uuid', time.time(), {'uuid4': 'callback_data'})], {'query_id': 'keyboard_id'})
...
papp_store_all_or_none = pytest.mark.parametrize('papp', [PappInput(), PappInput(False, False, False, False)], ids=('all_data', 'no_data'), indirect=True)
...
class TestBasePersistence:
    ...
    @pytest.mark.parametrize('papp', [PappInput(fill_data=True), PappInput(False, False, False, False, False, fill_data=True)], indirect=True)
    async def test_initialization_basic(self, papp: Application):
        ...
        assert not papp.handlers[0][0].check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_1, chat_id=1))
        assert not papp.handlers[0][0].check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_2, chat_id=2))
        assert not papp.handlers[0][1].check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_3, chat_id=3))
        assert not papp.handlers[0][1].check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_4, chat_id=4))
        async with papp:
            if papp.persistence.store_data.bot_data:
                ...
                assert papp.bot.callback_data_cache.persistence_data == TrackingPersistence.CALLBACK_DATA
                assert papp.handlers[0][0].check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_1, chat_id=1))
                assert papp.handlers[0][0].check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_2, chat_id=2))
                assert papp.handlers[0][1].check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_3, chat_id=3))
                assert papp.handlers[0][1].check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_4, chat_id=4))
            else:
                assert not papp.chat_data
                assert not papp.user_data
                assert not papp.bot_data
    ...
    @pytest.mark.parametrize('papp', [PappInput(fill_data=True)], indirect=True)
    ...
    async def test_initialization_invalid_callback_data(self, papp: Application, callback_data, monkeypatch):
    ...
    async def test_add_conversation_handler_after_init(self, papp: Application, recwarn):
        ...
        await conversation.handle_update(update=update, check_result=check, application=papp, context=context)
        assert conversation.check_update(TrackingConversationHandler.build_update(state=HandlerStates.STATE_1, chat_id=2))
        ...
        check = conversation.check_update(update=update)
        await conversation.handle_update(update=update, check_result=check, application=papp, context=context)
        assert conversation.check_update(TrackingConversationHandler.build_update(state=HandlerStates.STATE_2, chat_id=1))
        async with papp:
            ...
            assert conversation.check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_1, chat_id=2))
            assert not conversation.check_update(TrackingConversationHandler.build_update(state=HandlerStates.STATE_2, chat_id=1))
            assert conversation.check_update(TrackingConversationHandler.build_update(state=HandlerStates.STATE_1, chat_id=1))
    ...
    @pytest.mark.parametrize('papp', [PappInput(update_interval=0.0)], indirect=True)
    ...
    @pytest.mark.parametrize('papp', [PappInput(update_interval=1.5)], indirect=True)
    async def test_update_interval(self, papp: Application, monkeypatch):
        ...
        async with papp:
            ...
            assert sum(diffs) / len(diffs) == pytest.approx(papp.persistence.update_interval, rel=0.1)

    @papp_store_all_or_none
    async def test_update_persistence_loop_call_count_update_handling(self, papp: Application, caplog):
        async with papp:
            for _ in range(5):
                await papp.process_update(TrackingConversationHandler.build_update(HandlerStates.END, chat_id=1))
            ...
            assert papp.persistence.updated_callback_data == papp.persistence.store_data.callback_data
            ...
            if papp.persistence.store_data.chat_data:
                ...
                assert not papp.persistence.updated_chat_ids
            assert papp.persistence.updated_conversations == {'conv_1': {(1, 1): 1}, 'conv_2': {(1, 1): 1}}
    ...
    async def test_calls_on_shutdown(self, papp, chat_id):
        papp.add_handler(MessageHandler(filters.ALL, callback=self.handler_callback(chat_id=chat_id)), group=-1)
        async with papp:
            await papp.process_update(TrackingConversationHandler.build_update(HandlerStates.STATE_1, chat_id=1))
            assert not papp.persistence.updated_bot_data
            assert not papp.persistence.updated_callback_data
            assert not papp.persistence.updated_user_ids
            assert not papp.persistence.updated_chat_ids
    ...
    @papp_store_all_or_none
    async def test_update_persistence_loop_saved_data_update_handling(self, papp: Application, chat_id):
        papp.add_handler(MessageHandler(filters.ALL, callback=self.handler_callback(chat_id=chat_id)), group=-1)
        async with papp:
            await papp.process_update(TrackingConversationHandler.build_update(HandlerStates.STATE_1, chat_id=1))
            ...
            assert papp.persistence.callback_data is not papp.bot.callback_data_cache.persistence_data
            assert not papp.persistence.conversations
            await papp.update_persistence()
            assert papp.persistence.bot_data is not papp.bot_data
            if papp.persistence.store_data.bot_data:
    ...
    async def test_update_persistence_loop_saved_data_job(self, papp: Application, chat_id):
        ...
        async with papp:
            await papp.job_queue.start()
            papp.job_queue.run_once(self.job_callback(chat_id=chat_id), when=1.5, chat_id=1, user_id=1)
            await asyncio.sleep(2.5)
            assert not papp.persistence.bot_data
            assert papp.persistence.bot_data is not papp.bot_data
            assert not papp.persistence.chat_data
    ...
    async def test_update_persistence_loop_async_logic(self, papp: Application, delay_type: str, chat_id):
        ...
        async with papp:
            if delay_type == 'job':
                ...
                papp.add_handler(MessageHandler(filters.ALL, self.handler_callback(sleep=sleep), block=False))
    ...
    @papp_store_all_or_none
    async def test_update_persistence_loop_manual_mark_for_update_persistence(self, papp: Application, chat_id):
        async with papp:
    ...
    async def test_errors_while_persisting(self, bot_info, caplog):
        ...
        app = ApplicationBuilder().bot(make_bot(bot_info, arbitrary_callback_data=True)).persistence(ErrorPersistence()).build()
        async with app:
            app.add_error_handler(error)
            for _ in range(5):
                await app.process_update(TrackingConversationHandler.build_update(HandlerStates.END, chat_id=1))
    ...
    @default_papp
    @pytest.mark.parametrize('delay_type', ['job', 'blocking_handler', 'nonblocking_handler', 'task'])
    async def test_update_persistence_after_exception(self, papp: Application, delay_type: str, chat_id):
        ...
        async with papp:
            ...
            if delay_type == 'job':
                ...
                papp.add_handler(MessageHandler(filters.ALL, raise_error, block=delay_type.startswith('blocking')))
                await papp.process_update(update)
            else:
    ...
    async def test_non_blocking_conversations(self, bot, caplog):
        ...
        conversation = ConversationHandler(entry_points=[TrackingConversationHandler.build_handler(HandlerStates.END, callback=callback)], states={}, fallbacks=[], persistent=True, name='conv', block=False)
        papp.add_handler(conversation)
        async with papp:
            await papp.start()
            ...
            with caplog.at_level(logging.DEBUG):
                await papp.update_persistence()
            assert not any((record.getMessage().startswith('A ConversationHandlers state was not yet') for record in caplog.records))
            assert papp.persistence.updated_conversations == {'conv': {(1, 1): 1}}
    ...
    async def test_non_blocking_conversations_raises_Exception(self, bot):
        ...
        conversation = ConversationHandler(entry_points=[TrackingConversationHandler.build_handler(HandlerStates.END, callback=callback_1)], states={HandlerStates.STATE_1: [TrackingConversationHandler.build_handler(HandlerStates.STATE_1, callback=callback_2)]}, fallbacks=[], persistent=True, name='conv', block=False)
        papp.add_handler(conversation)
        async with papp:
            assert papp.persistence.updated_conversations == {}
            await papp.process_update(TrackingConversationHandler.build_update(HandlerStates.END, 1))
            ...
            assert papp.persistence.updated_conversations == {'conv': {(1, 1): 1}}
            assert papp.persistence.conversations == {'conv': {(1, 1): None}}
            await papp.process_update(TrackingConversationHandler.build_update(HandlerStates.STATE_1, 1))
            papp.persistence.reset_tracking()
            await asyncio.sleep(0.01)
            await papp.update_persistence()
            assert papp.persistence.updated_conversations == {'conv': {(1, 1): 1}}
    ...
    async def test_non_blocking_conversation_ends(self, bot):
        ...
        async with papp:
            assert papp.persistence.conversations == {'conv': {(1, 1): HandlerStates.END}}
            assert conversation.check_update(TrackingConversationHandler.build_update(HandlerStates.END, 1))
            await papp.update_persistence()
            assert papp.persistence.conversations == {'conv': {(1, 1): None}}
    ...
    async def test_conversation_timeout(self, bot):
        ...
        async def callback(_, __):
            return HandlerStates.STATE_1
        conversation = ConversationHandler(entry_points=[TrackingConversationHandler.build_handler(HandlerStates.END, callback=callback)], states={HandlerStates.STATE_1: []}, fallbacks=[], persistent=True, name='conv', conversation_timeout=3)
        papp.add_handler(conversation)
        async with papp:
            await papp.start()
            assert papp.persistence.updated_conversations == {}
    ...
    async def test_persistent_nested_conversations(self, bot):
        ...
        parent = ConversationHandler(entry_points=[TrackingConversationHandler.build_handler(HandlerStates.END)], states={HandlerStates.STATE_1: [child], HandlerStates.STATE_2: [TrackingConversationHandler.build_handler(HandlerStates.STATE_2, callback=build_callback(HandlerStates.END))]}, fallbacks=[], persistent=True, name='parent')
        ...
        await papp.initialize()
        assert papp.persistence.updated_conversations == {}
        assert not parent.check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_2, 1))
        assert not parent.check_update(TrackingConversationHandler.build_update(HandlerStates.END, 1))
        assert parent.check_update(TrackingConversationHandler.build_update(HandlerStates.STATE_1, 1))
        await papp.process_update(TrackingConversationHandler.build_update(HandlerStates.STATE_1, 1))
        assert papp.persistence.updated_conversations == {}
        await papp.update_persistence()
        assert papp.persistence.updated_conversations == {'grand_child': {(1, 1): 1}, 'child': {(1, 1): 1}}
        assert papp.persistence.conversations == {'grand_child': {(1, 1): None}, 'child': {(1, 1): HandlerStates.STATE_2}, 'parent': {(1, 1): HandlerStates.STATE_1}}
        papp.persistence.reset_tracking()
        await papp.process_update(TrackingConversationHandler.build_update(HandlerStates.STATE_2, 1))
        await papp.update_persistence()
        assert papp.persistence.updated_conversations == {'parent': {(1, 1): 1}, 'child': {(1, 1): 1}}
        assert papp.persistence.conversations == {'child': {(1, 1): None}, 'parent': {(1, 1): HandlerStates.STATE_2}}
        papp.persistence.reset_tracking()
        await papp.process_update(TrackingConversationHandler.build_update(HandlerStates.STATE_2, 1))
        await papp.update_persistence()
        assert papp.persistence.updated_conversations == {'parent': {(1, 1): 1}}
        assert papp.persistence.conversations == {'parent': {(1, 1): None}}
        await papp.shutdown()