...
async def test_run_history(recorder_mock: Recorder, hass: HomeAssistant) -> None:
    ...
    assert process_timestamp(instance.recorder_runs_manager.get(three_days_ago + timedelta(microseconds=1)).start) == three_days_ago
    assert process_timestamp(instance.recorder_runs_manager.get(two_days_ago + timedelta(microseconds=1)).start) == two_days_ago
    assert process_timestamp(instance.recorder_runs_manager.get(one_day_ago + timedelta(microseconds=1)).start) == one_day_ago
    assert process_timestamp(instance.recorder_runs_manager.get(now).start) == instance.recorder_runs_manager.recording_start

async def test_run_history_while_recorder_is_not_yet_started(async_setup_recorder_instance: RecorderInstanceGenerator, hass: HomeAssistant, recorder_db_url: str) -> None:
    ...
    """
    with patch('homeassistant.components.recorder.table_managers.recorder_runs.RecorderRunsManager.start'):
