...
airtable_regex = '^https://airtable.com/(app[A-Za-z0-9]+)/(tbl[A-Za-z0-9]+)/?(viw[A-z0-9]+)?'
...
class AirtableSheet(Sheet):
    ...
    rowtype = 'records'
    columns = [ItemColumn('id', 'id', type=str, width=0), ItemColumn('createdTime', 'createdTime', type=date, width=0)]
