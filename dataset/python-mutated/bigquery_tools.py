...
_PROJECT_PATTERN = '([a-z0-9.-]+:)?[a-z][a-z0-9-]*[a-z0-9]'
_DATASET_PATTERN = '\\w{1,1024}'
_TABLE_PATTERN = '[\\p{L}\\p{M}\\p{N}\\p{Pc}\\p{Pd}\\p{Zs}$]{1,1024}'
BIGQUERY_TYPE_TO_PYTHON_TYPE = {'STRING': str, 'BOOL': bool, 'BOOLEAN': bool, 'BYTES': bytes, 'INT64': np.int64, 'INTEGER': np.int64, 'FLOAT64': np.float64, 'FLOAT': np.float64, 'NUMERIC': decimal.Decimal, 'TIMESTAMP': apache_beam.utils.timestamp.Timestamp}
...
class RetryStrategy(object):
    ...
    _NON_TRANSIENT_ERRORS = {'invalid', 'invalidQuery', 'notImplemented', 'Bad Request', 'Unauthorized', 'Forbidden', 'Not Found', 'Not Implemented'}
