...
class PushRulesHandler:
    ...
    async def set_rule_attr(self, user_id: str, spec: RuleSpec, val: Union[bool, JsonDict]) -> None:
        ...
        if spec.attr == 'enabled':
            ...
            await self._main_store.set_push_rule_enabled(user_id, namespaced_rule_id, val, is_default_rule)
            ...
            if is_default_rule:
                if namespaced_rule_id not in BASE_RULE_IDS:
                    raise RuleNotFoundException('Unknown rule %r' % (namespaced_rule_id,))
            await self._main_store.set_push_rule_actions(user_id, namespaced_rule_id, actions, is_default_rule)
        else:
            raise UnrecognizedRequestError()
    ...
    async def push_rules_for_user(self, user: UserID) -> Dict[str, Dict[str, List[Dict[str, Any]]]]:
        """
