...
from ray.rllib.algorithms.ppo import PPOConfig, PPOTF1Policy, PPOTF2Policy, PPOTorchPolicy
...
parser.add_argument('--stop-reward', type=float, default=50.0, help='Reward at which we stop training.')
if __name__ == '__main__':
    ...
    ppo_config = PPOConfig().experimental(_enable_new_api_stack=False).environment('multi_agent_cartpole').framework(args.framework).rollouts(observation_filter='MeanStdFilter').training(model={'vf_share_layers': True}, vf_loss_coeff=0.01, num_sgd_iter=6).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    dqn_config = DQNConfig().environment('multi_agent_cartpole').framework(args.framework).rollouts(observation_filter='MeanStdFilter').training(model={'vf_share_layers': True}, n_step=3, gamma=0.95).resources(num_gpus=int(os.environ.get('RLLIB_NUM_GPUS', '0')))
    policies = {'ppo_policy': (select_policy('PPO', args.framework), obs_space, act_space, ppo_config), 'dqn_policy': (select_policy('DQN', args.framework), obs_space, act_space, dqn_config)}
    ...
    def policy_mapping_fn(agent_id, episode, worker, **kwargs):
        ...
        if agent_id % 2 == 0:
            ...
            return 'dqn_policy'
    ppo_config.multi_agent(policies=policies, policy_mapping_fn=policy_mapping_fn, policies_to_train=['ppo_policy'])
    ppo = ppo_config.build()
    dqn_config.multi_agent(policies=policies, policy_mapping_fn=policy_mapping_fn, policies_to_train=['dqn_policy'])
    ...
    for i in range(args.stop_iters):
        ...
        if args.as_test and result_dqn['episode_reward_mean'] > args.stop_reward and (result_ppo['episode_reward_mean'] > args.stop_reward):
