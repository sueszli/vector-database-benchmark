...
def train():
    ...
    for step in range(10):
        ...
        if loss.name in distributed_main_program.global_block().vars:
            (loss_print,) = exe.run(distributed_main_program, feed={'tokens': tokens, 'position_ids': position_ids, 'attention_mask': attention_mask, 'labels': labels, 'loss_mask': loss_mask}, fetch_list=[loss])
            print(f'step: {step}, loss: {loss_print:f}')
        else:
            exe.run(distributed_main_program, feed={'tokens': tokens, 'position_ids': position_ids, 'attention_mask': attention_mask, 'labels': labels, 'loss_mask': loss_mask})
