...
from ...core.types import BaseInputObjectType, GiftCardSettingsError, TimePeriodInputType
...
class GiftCardSettingsUpdateInput(BaseInputObjectType):
    expiry_type = GiftCardSettingsExpiryTypeEnum(description='Defines gift card default expiry settings.')
...
class GiftCardSettingsUpdate(BaseMutation):
    gift_card_settings = graphene.Field(GiftCardSettings, description='Gift card settings.')

    class Arguments:
        input = GiftCardSettingsUpdateInput(required=True, description='Fields required to update gift card settings.')
