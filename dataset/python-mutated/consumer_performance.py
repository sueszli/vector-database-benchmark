    ...
    def run(args):
        ...
        try:
            ...
            if args.brokers:
                ...
                producer = KafkaProducer(compression_type=args.fixture_compression, **props)
            ...
            timer = StatsReporter(args.stats_interval, consumer, event=timer_stop, raw_metrics=args.raw_metrics)
            timer.start()
            print('-> OK!')
            print()
            records = 0
...
class StatsReporter(threading.Thread):
    ...
    def print_stats(self):
        ...
        if self.raw_metrics:
            ...
            print('{records-consumed-rate} records/sec ({bytes-consumed-rate} B/sec), {fetch-latency-avg} latency, {fetch-rate} fetch/s, {fetch-size-avg} fetch size, {records-lag-max} max record lag, {records-per-request-avg} records/req'.format(**metrics['consumer-fetch-manager-metrics']))
...
def get_args_parser():
    if False:
        i = 10
        return i + 15
    parser = argparse.ArgumentParser(description='This tool is used to verify the consumer performance.')
    parser.add_argument('--topic', type=str, help='Topic for consumer test', default='kafka-python-benchmark-test')
    parser.add_argument('--num-records', type=int, help='number of messages to consume', default=1000000)
    parser.add_argument('--record-size', type=int, help='message size in bytes', default=100)
    parser.add_argument('--consumer-config', type=str, nargs='+', default=(), help='kafka consumer related configuration properties like bootstrap_servers,client_id etc..')
    parser.add_argument('--fixture-compression', type=str, help='specify a compression type for use with broker fixtures / producer')
