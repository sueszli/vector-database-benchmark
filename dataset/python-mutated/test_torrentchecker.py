...
from tribler.core.components.torrent_checker.torrent_checker.dataclasses import HealthInfo, TOLERABLE_TIME_DRIFT, TrackerResponse
from tribler.core.components.torrent_checker.torrent_checker.torrent_checker import TorrentChecker
from tribler.core.components.torrent_checker.torrent_checker.torrentchecker_session import HttpTrackerSession, UdpSocketManager
from tribler.core.components.torrent_checker.torrent_checker.tracker_manager import TrackerManager
from tribler.core.components.torrent_checker.torrent_checker.utils import aggregate_responses_for_infohash, filter_non_exceptions
...
async def torrent_checker_fixture(tribler_config, tracker_manager, metadata_store):
    torrent_checker = TorrentChecker(config=tribler_config, download_manager=MagicMock(), notifier=MagicMock(), metadata_store=metadata_store, tracker_manager=tracker_manager)
...
async def test_health_check_blacklisted_trackers(torrent_checker):
    ...
    with db_session:
        ...
        torrent_checker.mds.TorrentState(infohash=b'a' * 20, seeders=5, leechers=10, trackers={tracker}, last_check=int(time.time()))
...
async def test_check_local_torrents(torrent_checker):
    ...
    selection_range = stale_infohashes[0:torrent_checker_module.TORRENT_SELECTION_POOL_SIZE] + stale_infohashes[-torrent_checker_module.TORRENT_SELECTION_POOL_SIZE:]
...
async def test_get_tracker_response_cancelled_error(torrent_checker: TorrentChecker, caplog):
    ...
    assert caplog.record_tuples == [('TorrentChecker', logging.INFO, 'Tracker session is being cancelled: <tracker_url>')]
...
async def test_get_tracker_response_other_error(torrent_checker: TorrentChecker, caplog):
    ...
    assert caplog.record_tuples == [('TorrentChecker', logging.WARNING, 'Got session error for the tracker: <tracker_url>\nerror text')]
...
async def test_update_torrent_health_no_replace(torrent_checker, caplog):
    ...
    with db_session:
        torrent_state = torrent_checker.mds.TorrentState(infohash=unhexlify('abcd0123'), seeders=2, leechers=1, last_check=now, self_checked=True)
