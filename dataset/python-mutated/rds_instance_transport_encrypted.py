...
class rds_instance_transport_encrypted(Check):
    ...
    def execute(self):
        ...
        for db_instance in rds_client.db_instances:
            ...
            report.status = 'FAIL'
            report.status_extended = f'RDS Instance {db_instance.id} connections are not encrypted.'
            if any((engine in db_instance.engine for engine in supported_engines)) and 'aurora' not in db_instance.engine:
                for parameter in db_instance.parameters:
                    if parameter['ParameterName'] == 'rds.force_ssl' and parameter['ParameterValue'] == '1':
                        report.status = 'PASS'
