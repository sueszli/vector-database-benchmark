...
from electrum.lnonion import OnionHopsDataSingle, new_onion_packet, process_onion_packet, _decode_onion_error, decode_onion_error, OnionFailureCode, OnionPacket
...
class Test_LNRouter(ElectrumTestCase):
    ...
    def prepare_graph(self):
        ...
        self.cdb.add_channel_announcements({'node_id_1': node('b'), 'node_id_2': node('c'), 'bitcoin_key_1': node('b'), 'bitcoin_key_2': node('c'), 'short_channel_id': channel(1), 'chain_hash': BitcoinTestnet.rev_genesis_bytes(), 'len': 0, 'features': b''}, trusted=True)
        self.assertEqual(self.cdb.num_channels, 1)
        self.cdb.add_channel_announcements({'node_id_1': node('b'), 'node_id_2': node('e'), 'bitcoin_key_1': node('b'), 'bitcoin_key_2': node('e'), 'short_channel_id': channel(2), 'chain_hash': BitcoinTestnet.rev_genesis_bytes(), 'len': 0, 'features': b''}, trusted=True)
        self.cdb.add_channel_announcements({'node_id_1': node('a'), 'node_id_2': node('b'), 'bitcoin_key_1': node('a'), 'bitcoin_key_2': node('b'), 'short_channel_id': channel(3), 'chain_hash': BitcoinTestnet.rev_genesis_bytes(), 'len': 0, 'features': b''}, trusted=True)
        self.cdb.add_channel_announcements({'node_id_1': node('c'), 'node_id_2': node('d'), 'bitcoin_key_1': node('c'), 'bitcoin_key_2': node('d'), 'short_channel_id': channel(4), 'chain_hash': BitcoinTestnet.rev_genesis_bytes(), 'len': 0, 'features': b''}, trusted=True)
        self.cdb.add_channel_announcements({'node_id_1': node('d'), 'node_id_2': node('e'), 'bitcoin_key_1': node('d'), 'bitcoin_key_2': node('e'), 'short_channel_id': channel(5), 'chain_hash': BitcoinTestnet.rev_genesis_bytes(), 'len': 0, 'features': b''}, trusted=True)
        self.cdb.add_channel_announcements({'node_id_1': node('a'), 'node_id_2': node('d'), 'bitcoin_key_1': node('a'), 'bitcoin_key_2': node('d'), 'short_channel_id': channel(6), 'chain_hash': BitcoinTestnet.rev_genesis_bytes(), 'len': 0, 'features': b''}, trusted=True)
        self.cdb.add_channel_announcements({'node_id_1': node('c'), 'node_id_2': node('e'), 'bitcoin_key_1': node('c'), 'bitcoin_key_2': node('e'), 'short_channel_id': channel(7), 'chain_hash': BitcoinTestnet.rev_genesis_bytes(), 'len': 0, 'features': b''}, trusted=True)
    ...
    async def test_find_path_for_payment(self):
        ...
        path = self.path_finder.find_path_for_payment(nodeA=node('a'), nodeB=node('e'), invoice_amount_msat=amount_to_send)
        self.assertEqual([PathEdge(start_node=node('a'), end_node=node('b'), short_channel_id=channel(3)), PathEdge(start_node=node('b'), end_node=node('e'), short_channel_id=channel(2))], path)
    ...
    def test_liquidity_hints(self):
        ...
        self.assertEqual(fee_for_edge_msat(amount_to_send, DEFAULT_PENALTY_BASE_MSAT, DEFAULT_PENALTY_PROPORTIONAL_MILLIONTH), liquidity_hints.penalty(node_from, node_to, channel_id, amount_to_send))
        liquidity_hints.update_can_send(node_from, node_to, channel_id, 1000000)
        liquidity_hints.update_cannot_send(node_from, node_to, channel_id, 2000000)
        hint = liquidity_hints.get_hint(channel_id)
        self.assertEqual(1000000, hint.can_send(node_from < node_to))
        self.assertEqual(None, hint.cannot_send(node_to < node_from))
        self.assertEqual(2000000, hint.cannot_send(node_from < node_to))
        self.assertEqual(2000000, hint.can_send(node_to < node_from))
        self.assertEqual(0.0, liquidity_hints.penalty(node_from, node_to, channel_id, 1000000))
        self.assertEqual(650, liquidity_hints.penalty(node_from, node_to, channel_id, 1500000))
        self.assertEqual(inf, liquidity_hints.penalty(node_from, node_to, channel_id, 2000000))
        liquidity_hints.update_can_send(node_from, node_to, channel_id, 500000)
        hint = liquidity_hints.get_hint(channel_id)
        self.assertEqual(1000000, hint.can_send(node_from < node_to))
        liquidity_hints.update_can_send(node_from, node_to, channel_id, 3000000)
        hint = liquidity_hints.get_hint(channel_id)
        self.assertEqual(3000000, hint.can_send(node_from < node_to))
        ...
        self.assertEqual(1200, liquidity_hints.penalty(node_from, node_to, channel_id, 1000000))

    @needs_test_with_all_chacha20_implementations
    def test_new_onion_packet(self):
        if False:
            return 10
        payment_path_pubkeys = [bfh('02eec7245d6b7d2ccb30380bfbe2a3648cd7a942653f5aa340edcea1f283686619'), bfh('0324653eac434488002cc06bbfb7f10fe18991e35f9fe4302dbea6d2353dc0ab1c'), bfh('027f31ebc5462c1fdce1b737ecff52d37d75dea43ce11c74d25aa297165faa2007'), bfh('032c0b7cf95324a07d05398b240174dc0c2be444d96b159aa6c7f7b1e668680991'), bfh('02edabbd16b41c8371b92ef2f04c1185b4f03b6dcd52ba9b78d9d7c89c8f221145')]
        ...
        hops_data = [OnionHopsDataSingle(), OnionHopsDataSingle(), OnionHopsDataSingle(), OnionHopsDataSingle(), OnionHopsDataSingle()]
