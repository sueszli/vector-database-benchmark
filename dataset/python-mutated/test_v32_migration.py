...
async def test_migrate_times(caplog: pytest.LogCaptureFixture, tmp_path: Path) -> None:
    ...
    mock_state = State('sensor.test', 'old', {'last_reset': now.isoformat()}, last_changed=one_second_past, last_updated=now)
    state_changed_event = Event(EVENT_STATE_CHANGED, {'entity_id': 'sensor.test', 'old_state': None, 'new_state': mock_state}, EventOrigin.local, time_fired=now)
    custom_event = Event('custom_event', {'entity_id': 'sensor.custom'}, EventOrigin.local, time_fired=now)
    ...
    with patch.object(recorder, 'db_schema', old_db_schema), patch.object(recorder.migration, 'SCHEMA_VERSION', old_db_schema.SCHEMA_VERSION), patch.object(core, 'StatesMeta', old_db_schema.StatesMeta), patch.object(core, 'EventTypes', old_db_schema.EventTypes), patch.object(core, 'EventData', old_db_schema.EventData), patch.object(core, 'States', old_db_schema.States), patch.object(core, 'Events', old_db_schema.Events), patch(CREATE_ENGINE_TARGET, new=_create_engine_test), patch('homeassistant.components.recorder.Recorder._migrate_events_context_ids'), patch('homeassistant.components.recorder.Recorder._migrate_states_context_ids'), patch('homeassistant.components.recorder.Recorder._migrate_event_type_ids'), patch('homeassistant.components.recorder.Recorder._migrate_entity_ids'), patch('homeassistant.components.recorder.Recorder._post_migrate_entity_ids'), patch('homeassistant.components.recorder.Recorder._cleanup_legacy_states_event_ids'):
        hass = await async_test_home_assistant(asyncio.get_running_loop())
        recorder_helper.async_initialize_recorder(hass)
        assert await async_setup_component(hass, 'recorder', {'recorder': {'db_url': dburl}})
        ...
        states_indexes = await recorder.get_instance(hass).async_add_executor_job(_get_states_index_names)
    ...
    events_indexes = await recorder.get_instance(hass).async_add_executor_job(_get_events_index_names)
...
async def test_migrate_can_resume_entity_id_post_migration(caplog: pytest.LogCaptureFixture, tmp_path: Path) -> None:
