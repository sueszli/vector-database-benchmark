...
def geometric_median(x, eps=1e-05):
    if False:
        print('Hello World!')
...
def euclidean(v1, v2, c=False):
    ...
    if c:
        res = []
        for _v in v1:
            res.append([np.sqrt((_v[0] - v2[0]) ** 2 + (_v[1] - v2[1]) ** 2 + (_v[2] - v2[2]) ** 2)])
...
def rod_3D(x, gm=None, median=None, scaler1=None, scaler2=None):
    ...
    (gammas, scaler1, scaler2) = scale_angles(np.arccos(np.clip(np.dot(_x, gm) / (v_norm * norm_), -1, 1)), scaler1=scaler1, scaler2=scaler2)
...
def process_sub(subspace, gm, median, scaler1, scaler2):
    ...
    (mad_subspace, gm, median, scaler1, scaler2) = rod_3D(subspace, gm=gm, median=median, scaler1=scaler1, scaler2=scaler2)
...
def rod_nD(X, parallel, gm=None, median=None, data_scaler=None, angles_scalers1=None, angles_scalers2=None):
    ...
    if parallel:
        ...
        args = [[a, b, c, d, e] for (a, b, c, d, e) in zip(all_subspaces, all_gms, all_meds, all_angles_scalers1, all_angles_scalers2)]
    ...
    (subspaces_scores, gm, median, angles_scalers1, angles_scalers2) = ([], [], [], [], [])
    for (subspace, _gm, med, ang_s1, ang_s2) in zip(all_subspaces, all_gms, all_meds, all_angles_scalers1, all_angles_scalers2):
        (scores_, gm_, med_, ang_s1_, ang_s2_) = process_sub(subspace=subspace, gm=_gm, median=med, scaler1=ang_s1, scaler2=ang_s2)
...
class ROD(BaseDetector):
    ...
    def __init__(self, contamination=0.1, parallel_execution=False):
        ...
        if not isinstance(parallel_execution, bool):
            raise TypeError('parallel_execution should be bool. Got {}'.format(type(parallel_execution)))
    ...
    def decision_function(self, X):
        ...
        if X.shape[1] == 3:
            (scores, self.gm_, self.median_, self.angles_scaler1_, self.angles_scaler2_) = rod_3D(x=X, gm=self.gm_, median=self.median_, scaler1=self.angles_scaler1_, scaler2=self.angles_scaler2_)
            return scores
        (scores, self.gm_, self.median_, self.data_scaler_, self.angles_scaler1_, self.angles_scaler2_) = rod_nD(X=X, parallel=self.parallel_execution, gm=self.gm_, median=self.median_, data_scaler=self.data_scaler_, angles_scalers1=self.angles_scaler1_, angles_scalers2=self.angles_scaler2_)
        return scores