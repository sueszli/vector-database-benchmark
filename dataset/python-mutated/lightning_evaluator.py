...
class MyModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler_dict = {'scheduler': nni.trace(torch.optim.lr_scheduler.LambdaLR)(optimizer, lr_lambda=lambda epoch: 1 / epoch), 'interval': 'epoch'}
        return {'optimizer': optimizer, 'lr_scheduler': scheduler_dict}

class MyDataModule(pl.LightningDataModule):
    pass
from nni.compression import LightningEvaluator
pl_trainer = nni.trace(pl.Trainer)(accelerator='auto', devices=1, max_epochs=3, logger=TensorBoardLogger('./lightning_logs', name='vgg'))
