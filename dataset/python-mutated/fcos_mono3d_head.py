...
from mmdet3d.core import box3d_multiclass_nms, limit_period, points_img2cam, xywhr2xyxyr
...
class FCOSMono3DHead(AnchorFreeMono3DHead):
    ...
    def __init__(self, regress_ranges=((-1, 48), (48, 96), (96, 192), (192, 384), (384, INF)), center_sampling=True, center_sample_radius=1.5, norm_on_bbox=True, centerness_on_reg=True, centerness_alpha=2.5, loss_cls=dict(type='FocalLoss', use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=1.0), loss_bbox=dict(type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0), loss_dir=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0), loss_attr=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0), loss_centerness=dict(type='CrossEntropyLoss', use_sigmoid=True, loss_weight=1.0), bbox_coder=dict(type='FCOS3DBBoxCoder', code_size=9), norm_cfg=dict(type='GN', num_groups=32, requires_grad=True), centerness_branch=(64,), init_cfg=None, **kwargs):
        if False:
            while True:
                i = 10
        ...
        super().__init__(loss_cls=loss_cls, loss_bbox=loss_bbox, loss_dir=loss_dir, loss_attr=loss_attr, norm_cfg=norm_cfg, init_cfg=init_cfg, **kwargs)
    ...
    def _init_layers(self):
        ...
        super()._init_layers()
        self.conv_centerness_prev = self._init_branch(conv_channels=self.centerness_branch, conv_strides=(1,) * len(self.centerness_branch))
    ...
    def forward(self, feats):
        ...
        return multi_apply(self.forward_single, feats, self.scales, self.strides)[:5]
    ...
    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'dir_cls_preds', 'attr_preds', 'centernesses'))
    def loss(self, cls_scores, bbox_preds, dir_cls_preds, attr_preds, centernesses, gt_bboxes, gt_labels, gt_bboxes_3d, gt_labels_3d, centers2d, depths, attr_labels, img_metas, gt_bboxes_ignore=None):
        ...
        loss_cls = self.loss_cls(flatten_cls_scores, flatten_labels_3d, avg_factor=num_pos + num_imgs)
    ...
    def get_bboxes(self, cls_scores, bbox_preds, dir_cls_preds, attr_preds, centernesses, img_metas, cfg=None, rescale=None):
        ...
        for img_id in range(len(img_metas)):
            ...
            det_bboxes = self._get_bboxes_single(cls_score_list, bbox_pred_list, dir_cls_pred_list, attr_pred_list, centerness_pred_list, mlvl_points, input_meta, cfg, rescale)
    ...
    def pts2Dto3D(points, view):
        ...
        warning.warn('DeprecationWarning: This static method has been moved out of this class to mmdet3d/core. The function pts2Dto3D will be deprecated.')
        ...
        homo_points2D = torch.cat([unnorm_points2D, points2D.new_ones((nbr_points, 1))], dim=1)
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, gt_bboxes_3d, gt_labels_3d, centers2d, depths, attr_labels, points, regress_ranges, num_points_per_lvl):
        ...
        if num_gts == 0:
            return (gt_labels.new_full((num_points,), self.background_label), gt_bboxes.new_zeros((num_points, 4)), gt_labels_3d.new_full((num_points,), self.background_label), gt_bboxes_3d.new_zeros((num_points, self.bbox_code_size)), gt_bboxes_3d.new_zeros((num_points,)), attr_labels.new_full((num_points,), self.attr_background_label))
        gt_bboxes_3d[..., 6] = -torch.atan2(gt_bboxes_3d[..., 0], gt_bboxes_3d[..., 2]) + gt_bboxes_3d[..., 6]
        ...
        gt_bboxes_3d = gt_bboxes_3d[None].expand(num_points, num_gts, self.bbox_code_size)
        ...
        bbox_targets_3d = torch.cat((delta_xs, delta_ys, depths, gt_bboxes_3d[..., 3:]), dim=-1)
        left = xs - gt_bboxes[..., 0]
