        ...
        for trail in cloudtrail_client.trails:
            if trail.name:
                ...
                if trail.is_multiregion:
                    report.status_extended = f'Multiregion trail {trail.name} has encryption disabled.'
                else:
                    report.status_extended = f'Single region trail {trail.name} has encryption disabled.'
                if trail.kms_key:
                    report.status = 'PASS'
                    if trail.is_multiregion:
