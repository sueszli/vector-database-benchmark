...
D_NUMBERS = {b'integer': 1, b'float': 1.3, b'decimal': Decimal('1.3'), b'long': 4, b'complex': complex(13.3)}
...
D_TEXT = {b'foo': QUICK_BROWN_FOX, b'bar': B_QUICK_BROWN_FOX, b'baz': B_QUICK_BROWN_FOX, b'xuzzy': B_QUICK_BROWN_FOX}
L_NUMBERS = list(D_NUMBERS.values())
D_TEXT_LARGE = {b'bazxuzzyfoobarlongverylonglong': QUICK_BROWN_FOX * 30}
D_ALL = {b'numbers': D_NUMBERS, b'intkeys': D_INT_KEYS, b'text': D_TEXT, b'largetext': D_TEXT_LARGE}
D_D_TEXT = {b'rest': D_TEXT}
RE_OLD_SET_REPR = re.compile('(?<!frozen)set\\([\\[|\\{](.+?)[\\}\\]]\\)')
RE_OLD_SET_REPR_REPLACE = '{\\1}'
RE_OLD_SET_CUSTOM_REPR = re.compile('((?:frozen)?set\\d?\\()\\[(.+?)\\](\\))')
RE_OLD_SET_CUSTOM_REPR_REPLACE = '\\1{\\2}\\3'
RE_EMPTY_SET_REPR = re.compile('((?:frozen)?set\\d?)\\(\\[\\]\\)')
RE_EMPTY_SET_REPR_REPLACE = '\\1()'
RE_LONG_SUFFIX = re.compile('(\\d)+L')
