...
def trilinear_interp_test(x, OutSize=None, SizeTensor=None, Scale=None, data_layout='NCHW', out_d=-1, out_h=-1, out_w=-1, scale=[], interp_method='trilinear', align_corners=True, align_mode=0):
    if False:
        i = 10
        return i + 15
    ...
    return paddle._C_ops.trilinear_interp(x, OutSize, SizeTensor, Scale, data_layout, out_d, out_h, out_w, scale, interp_method, align_corners, align_mode)

def trilinear_interp_np(input, out_d, out_h, out_w, scale_d=0, scale_h=0, scale_w=0, out_size=None, actual_shape=None, align_corners=True, align_mode=0, data_layout='NCDHW'):
    if False:
        for i in range(10):
            print('nop')
...
class TestTrilinearInterpOp(OpTest):
    ...
    def setUp(self):
        ...
        if self.scale:
            ...
            out_h = self.out_h
            out_w = self.out_w
        output_np = trilinear_interp_np(input_np, out_d, out_h, out_w, scale_d, scale_h, scale_w, self.out_size, self.actual_shape, self.align_corners, self.align_mode, self.data_layout)
        ...
        if self.data_layout == 'NCDHW':
            ...
            data_layout = 'NHWC'
        self.attrs = {'out_d': self.out_d, 'out_h': self.out_h, 'out_w': self.out_w, 'interp_method': self.interp_method, 'align_corners': self.align_corners, 'align_mode': self.align_mode, 'data_layout': data_layout}
        if self.scale:
            if isinstance(self.scale, (float, int)):
                if self.scale > 0:
...
class TestTrilinearInterpOpUint8(OpTest):
    ...
    def setUp(self):
        ...
        if self.scale:
            ...
            out_h = self.out_h
            out_w = self.out_w
        output_np = trilinear_interp_np(input_np, out_d, out_h, out_w, 0, 0, 0, self.out_size, self.actual_shape, self.align_corners, self.align_mode)
        self.inputs = {'X': input_np}
        if self.out_size is not None:
            self.inputs['OutSize'] = self.out_size
        self.attrs = {'out_d': self.out_d, 'out_h': self.out_h, 'out_w': self.out_w, 'interp_method': self.interp_method, 'align_corners': self.align_corners, 'align_mode': self.align_mode}
        if self.scale:
            if isinstance(self.scale, (float, int)):
                if self.scale > 0:
                    self.scale = [self.scale]
...
class TestTrilinearInterpOpForFloat16(unittest.TestCase):
    ...
    def check_main(self, x_np, dtype):
        ...
        y = interpolate(x, size=self.out_size.tolist(), mode=self.interp_method, align_corners=self.align_corners, align_mode=self.align_mode, data_format=self.data_layout)
        x_g = paddle.grad(y, x)
