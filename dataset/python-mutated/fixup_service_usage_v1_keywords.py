...
class service_usageCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'batch_enable_services': ('parent', 'service_ids'), 'batch_get_services': ('parent', 'names'), 'disable_service': ('name', 'disable_dependent_services', 'check_if_service_has_usage'), 'enable_service': ('name',), 'get_service': ('name',), 'list_services': ('parent', 'page_size', 'page_token', 'filter')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=service_usageCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
