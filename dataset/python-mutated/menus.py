...
__all__ = ('menu', 'next_page', 'prev_page', 'close_menu', 'start_adding_reactions', 'DEFAULT_CONTROLS')
_T = TypeVar('_T')
...
class _GenericButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        emoji = str(self.emoji) if self.emoji.is_unicode_emoji() else ctx.bot.get_emoji(self.emoji.id) or self.emoji
        try:
            await self.func(ctx, pages, controls, message, page, timeout, emoji)
        except Exception:
            pass

async def menu(ctx: commands.Context, pages: _PageList, controls: Optional[Mapping[str, _ControlCallable]]=None, message: discord.Message=None, page: int=0, timeout: float=30.0) -> _T:
    """
    An emoji-based menu
    ...
    try:
        ...
        tasks = [asyncio.create_task(ctx.bot.wait_for('reaction_add', check=predicates)), asyncio.create_task(ctx.bot.wait_for('reaction_remove', check=predicates))]
        ...
        if not ctx.me:
            return
        try:
            if isinstance(message.channel, discord.PartialMessageable) or message.channel.permissions_for(ctx.me).manage_messages:
...
async def next_page(ctx: commands.Context, pages: list, controls: Mapping[str, _ControlCallable], message: discord.Message, page: int, timeout: float, emoji: str) -> _T:
    """
...
async def prev_page(ctx: commands.Context, pages: list, controls: Mapping[str, _ControlCallable], message: discord.Message, page: int, timeout: float, emoji: str) -> _T:
    """
...
async def close_menu(ctx: commands.Context, pages: list, controls: Mapping[str, _ControlCallable], message: discord.Message, page: int, timeout: float, emoji: str) -> None:
    """
...
DEFAULT_CONTROLS: Mapping[str, _ControlCallable] = MappingProxyType({'⬅️': prev_page, '❌': close_menu, '➡️': next_page})