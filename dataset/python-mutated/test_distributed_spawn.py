...
if 'BACKEND' not in os.environ or 'WORLD_SIZE' not in os.environ or 'TEMP_DIR' not in os.environ or (not path.exists(path.join(os.environ['TEMP_DIR'], 'barrier'))):
    raise RuntimeError(f"Missing expected env vars for `test_distributed_spawn.py`.  Please ensure to specify the following:\n'BACKEND' = one of {_allowed_backends}\n'WORLD_SIZE' = int >= 2\n'TEMP_DIR' specifying a directory containing a barrier file named 'barrier'.\n\ne.g.\ntouch /tmp/barrier && TEMP_DIR=/tmp BACKEND='nccl' WORLD_SIZE=2 python {__file__}")
