...
ignore_paths = (re.compile('changelog/unreleased_\\d{2}'),)
...
format_directive = re.compile('^\\.\\.\\s*format\\s*:\\s*(on|off)\\s*$')
doctest_code_start = re.compile('^(\\s+)({(?:opensql|execsql|printsql|sql|stop)})?>>>\\s?(.+)')
doctest_code_continue = re.compile('^\\s+\\.\\.\\.\\s?(\\s*.*)')
sql_code_start = re.compile('^(\\s+)({(?:open|print|exec)?sql})')
sql_code_stop = re.compile('^(\\s+){stop}')
start_code_section = re.compile('^(((?!\\.\\.).+::)|(\\.\\.\\s*sourcecode::(.*py.*)?)|(::))$')
...
if __name__ == '__main__':
    parser = ArgumentParser(description='Formats code inside docs using black. Supports doctest code blocks and plain code block identified as indented sections that are preceded by ``::`` or ``.. sourcecode:: py``.\n\nTo disable formatting on a file section the comment ``.. format: off`` disables formatting until ``.. format: on`` is encountered or the file ends.\n\nUse --report-doctest to ignore errors on plain code blocks.\n', formatter_class=RawDescriptionHelpFormatter)
    parser.add_argument('-f', '--file', help='Format only this file instead of all docs', nargs='+')
    parser.add_argument('-d', '--directory', help='Find documents in this directory and its sub dirs', default='doc/build')
    parser.add_argument('-c', '--check', help="Don't write the files back, just return the status. Return code 0 means nothing would change. Return code 1 means some files would be reformatted", action='store_true')
    parser.add_argument('-e', '--exit-on-error', help='Exit in case of black format error instead of ignoring it', action='store_true')
    parser.add_argument('-l', '--project-line-length', help='Configure the line length to the project value instead of using the black default of 88', action='store_true')
    parser.add_argument('-rd', '--report-doctest', help='Report errors only when running doctest blocks. When active exit-on-error will be valid only on doctest blocks', action='store_true')
    args = parser.parse_args()
    config = parse_pyproject_toml(home / 'pyproject.toml')
    BLACK_MODE = Mode(target_versions={TargetVersion[val.upper()] for val in config.get('target_version', []) if val != 'py27'}, line_length=config.get('line_length', DEFAULT_LINE_LENGTH) if args.project_line_length else DEFAULT_LINE_LENGTH)
