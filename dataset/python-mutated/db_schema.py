...
from sqlalchemy import CHAR, JSON, BigInteger, Boolean, ColumnElement, DateTime, Float, ForeignKey, Identity, Index, Integer, LargeBinary, SmallInteger, String, Text, case, type_coerce
...
from homeassistant.const import MAX_LENGTH_EVENT_EVENT_TYPE, MAX_LENGTH_STATE_ENTITY_ID, MAX_LENGTH_STATE_STATE
...
from homeassistant.util.json import JSON_DECODE_EXCEPTIONS, json_loads, json_loads_object
from .const import ALL_DOMAIN_EXCLUDE_ATTRS, SupportedDialect
from .models import StatisticData, StatisticDataTimestamp, StatisticMetaData, bytes_to_ulid_or_none, bytes_to_uuid_hex_or_none, datetime_to_timestamp_or_none, process_timestamp, ulid_to_bytes_or_none, uuid_hex_to_bytes_or_none
...
ALL_TABLES = [TABLE_STATES, TABLE_STATE_ATTRIBUTES, TABLE_EVENTS, TABLE_EVENT_DATA, TABLE_EVENT_TYPES, TABLE_RECORDER_RUNS, TABLE_SCHEMA_CHANGES, TABLE_STATES_META, TABLE_STATISTICS, TABLE_STATISTICS_META, TABLE_STATISTICS_RUNS, TABLE_STATISTICS_SHORT_TERM]
...
MYSQL_ENGINE = 'InnoDB'
_DEFAULT_TABLE_ARGS = {'mysql_default_charset': MYSQL_DEFAULT_CHARSET, 'mysql_collate': MYSQL_COLLATE, 'mysql_engine': MYSQL_ENGINE, 'mariadb_default_charset': MYSQL_DEFAULT_CHARSET, 'mariadb_collate': MYSQL_COLLATE, 'mariadb_engine': MYSQL_ENGINE}
...
class NativeLargeBinary(LargeBinary):
    ...
    def result_processor(self, dialect, coltype):
        ...
        return None
UINT_32_TYPE = BigInteger().with_variant(mysql.INTEGER(unsigned=True), 'mysql', 'mariadb')
...
DATETIME_TYPE = DateTime(timezone=True).with_variant(mysql.DATETIME(timezone=True, fsp=6), 'mysql', 'mariadb').with_variant(FAST_PYSQLITE_DATETIME(), 'sqlite')
DOUBLE_TYPE = Float().with_variant(mysql.DOUBLE(asdecimal=False), 'mysql', 'mariadb').with_variant(oracle.DOUBLE_PRECISION(), 'oracle').with_variant(postgresql.DOUBLE_PRECISION(), 'postgresql')
...
CONTEXT_BINARY_TYPE = LargeBinary(CONTEXT_ID_BIN_MAX_LENGTH).with_variant(NativeLargeBinary(CONTEXT_ID_BIN_MAX_LENGTH), 'mysql', 'mariadb', 'sqlite')
...
class Events(Base):
    """Event history data."""
    __table_args__ = (Index('ix_events_event_type_id_time_fired_ts', 'event_type_id', 'time_fired_ts'), Index(EVENTS_CONTEXT_ID_BIN_INDEX, 'context_id_bin', mysql_length=CONTEXT_ID_BIN_MAX_LENGTH, mariadb_length=CONTEXT_ID_BIN_MAX_LENGTH), _DEFAULT_TABLE_ARGS)
    ...
    data_id: Mapped[int | None] = mapped_column(Integer, ForeignKey('event_data.data_id'), index=True)
    ...
    event_type_id: Mapped[int | None] = mapped_column(Integer, ForeignKey('event_types.event_type_id'))
...
class EventData(Base):
    ...
    shared_data: Mapped[str | None] = mapped_column(Text().with_variant(mysql.LONGTEXT, 'mysql', 'mariadb'))
...
class EventTypes(Base):
    ...
    event_type: Mapped[str | None] = mapped_column(String(MAX_LENGTH_EVENT_EVENT_TYPE), index=True, unique=True)
...
class States(Base):
    ...
    __table_args__ = (Index(METADATA_ID_LAST_UPDATED_INDEX_TS, 'metadata_id', 'last_updated_ts'), Index(STATES_CONTEXT_ID_BIN_INDEX, 'context_id_bin', mysql_length=CONTEXT_ID_BIN_MAX_LENGTH, mariadb_length=CONTEXT_ID_BIN_MAX_LENGTH), _DEFAULT_TABLE_ARGS)
    ...
    last_updated_ts: Mapped[float | None] = mapped_column(TIMESTAMP_TYPE, default=time.time, index=True)
    old_state_id: Mapped[int | None] = mapped_column(Integer, ForeignKey('states.state_id'), index=True)
    attributes_id: Mapped[int | None] = mapped_column(Integer, ForeignKey('state_attributes.attributes_id'), index=True)
    context_id: Mapped[str | None] = mapped_column(UNUSED_LEGACY_COLUMN)
    context_user_id: Mapped[str | None] = mapped_column(UNUSED_LEGACY_COLUMN)
    context_parent_id: Mapped[str | None] = mapped_column(UNUSED_LEGACY_COLUMN)
    origin_idx: Mapped[int | None] = mapped_column(SmallInteger)
    ...
    context_user_id_bin: Mapped[bytes | None] = mapped_column(CONTEXT_BINARY_TYPE)
    context_parent_id_bin: Mapped[bytes | None] = mapped_column(CONTEXT_BINARY_TYPE)
    metadata_id: Mapped[int | None] = mapped_column(Integer, ForeignKey('states_meta.metadata_id'))
...
class StateAttributes(Base):
    ...
    hash: Mapped[int | None] = mapped_column(UINT_32_TYPE, index=True)
    shared_attrs: Mapped[str | None] = mapped_column(Text().with_variant(mysql.LONGTEXT, 'mysql', 'mariadb'))
...
class StatesMeta(Base):
    ...
    entity_id: Mapped[str | None] = mapped_column(String(MAX_LENGTH_STATE_ENTITY_ID), index=True, unique=True)
...
class StatisticsBase:
    ...
    metadata_id: Mapped[int | None] = mapped_column(Integer, ForeignKey(f'{TABLE_STATISTICS_META}.id', ondelete='CASCADE'))
...
class Statistics(Base, StatisticsBase):
    ...
    __table_args__ = (Index('ix_statistics_statistic_id_start_ts', 'metadata_id', 'start_ts', unique=True),)
    __tablename__ = TABLE_STATISTICS

class StatisticsShortTerm(Base, StatisticsBase):
    """Short term statistics."""
    duration = timedelta(minutes=5)
    __table_args__ = (Index('ix_statistics_short_term_statistic_id_start_ts', 'metadata_id', 'start_ts', unique=True),)
...
class StatisticsMeta(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, Identity(), primary_key=True)
    statistic_id: Mapped[str | None] = mapped_column(String(255), index=True, unique=True)
...
SHARED_ATTR_OR_LEGACY_ATTRIBUTES = case((StateAttributes.shared_attrs.is_(None), States.attributes), else_=StateAttributes.shared_attrs).label('attributes')
SHARED_DATA_OR_LEGACY_EVENT_DATA = case((EventData.shared_data.is_(None), Events.event_data), else_=EventData.shared_data).label('event_data')