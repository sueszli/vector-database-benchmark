...
class _BaseFlowManagerView(HomeAssistantView):
    ...
    def _prepare_result_json(self, result: data_entry_flow.FlowResult) -> data_entry_flow.FlowResult:
        ...
        if (schema := data['data_schema']) is None:
            ...
            data['data_schema'] = voluptuous_serialize.convert(schema, custom_serializer=cv.custom_serializer)
        return data

class FlowManagerIndexView(_BaseFlowManagerView):
    """View to create config flows."""

    @RequestDataValidator(vol.Schema({vol.Required('handler'): vol.Any(str, list), vol.Optional('show_advanced_options', default=False): cv.boolean}, extra=vol.ALLOW_EXTRA))
    async def post(self, request: web.Request, data: dict[str, Any]) -> web.Response:
        ...
        try:
            result = await self._flow_mgr.async_init(handler, context={'source': config_entries.SOURCE_USER, 'show_advanced_options': data['show_advanced_options']})
        except data_entry_flow.UnknownHandler:
            return self.json_message('Invalid handler specified', HTTPStatus.NOT_FOUND)
        except data_entry_flow.UnknownStep:
            return self.json_message('Handler does not support user', HTTPStatus.BAD_REQUEST)
...
class FlowManagerResourceView(_BaseFlowManagerView):
    ...
    async def post(self, request: web.Request, data: dict[str, Any], flow_id: str) -> web.Response:
        ...
        try:
            ...
            return self.json_message(f'User input malformed: {ex}', HTTPStatus.BAD_REQUEST)
        result = self._prepare_result_json(result)
        return self.json(result)
