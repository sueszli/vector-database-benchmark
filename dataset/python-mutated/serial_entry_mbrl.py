...
from ding.worker import BaseLearner, InteractionSerialEvaluator, BaseSerialCommander, create_buffer, get_buffer_cls, create_serial_collector
...
def mbrl_entry_setup(input_cfg: Union[str, Tuple[dict, dict]], seed: int=0, env_setting: Optional[List[Any]]=None, model: Optional[torch.nn.Module]=None) -> Tuple:
    if False:
        for i in range(10):
            print('nop')
    ...
    commander = BaseSerialCommander(cfg.policy.other.commander, learner, collector, evaluator, env_buffer, policy.command_mode)
    return (cfg, policy, world_model, env_buffer, learner, collector, collector_env, evaluator, commander, tb_logger)

def create_img_buffer(cfg: dict, input_cfg: Union[str, Tuple[dict, dict]], world_model: WorldModel, tb_logger: 'SummaryWriter') -> IBuffer:
    if False:
        return 10
    if isinstance(input_cfg, str):
...
def serial_pipeline_dyna(input_cfg: Union[str, Tuple[dict, dict]], seed: int=0, env_setting: Optional[List[Any]]=None, model: Optional[torch.nn.Module]=None, max_train_iter: Optional[int]=int(10000000000.0), max_env_step: Optional[int]=int(10000000000.0)) -> 'Policy':
    if False:
        return 10
    ...
    (cfg, policy, world_model, env_buffer, learner, collector, collector_env, evaluator, commander, tb_logger) = mbrl_entry_setup(input_cfg, seed, env_setting, model)
    ...
    while True:
        ...
        if world_model.should_train(collector.envstep):
            ...
            world_model.fill_img_buffer(policy.collect_mode, env_buffer, img_buffer, collector.envstep, learner.train_iter)
...
def serial_pipeline_dream(input_cfg: Union[str, Tuple[dict, dict]], seed: int=0, env_setting: Optional[List[Any]]=None, model: Optional[torch.nn.Module]=None, max_train_iter: Optional[int]=int(10000000000.0), max_env_step: Optional[int]=int(10000000000.0)) -> 'Policy':
    if False:
        while True:
            i = 10
    ...
    while True:
        ...
        for i in range(update_per_collect):
            ...
            learner.train(train_data, collector.envstep, policy_kwargs=dict(world_model=world_model, envstep=collector.envstep))
        if collector.envstep >= max_env_step or learner.train_iter >= max_train_iter:
            break
    learner.call_hook('after_run')
    return policy

def serial_pipeline_dreamer(input_cfg: Union[str, Tuple[dict, dict]], seed: int=0, env_setting: Optional[List[Any]]=None, model: Optional[torch.nn.Module]=None, max_train_iter: Optional[int]=int(10000000000.0), max_env_step: Optional[int]=int(10000000000.0)) -> 'Policy':
    if False:
        return 10
    ...
    while True:
        ...
        for _ in range(steps):
            ...
            start = post
            learner.train(start, collector.envstep, policy_kwargs=dict(world_model=world_model, envstep=collector.envstep))
