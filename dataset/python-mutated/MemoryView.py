...
ERR_UNINITIALIZED = 'Cannot check if memoryview %s is initialized without the GIL, consider using initializedcheck(False)'
...
_spec_to_const = {'direct': MEMVIEW_DIRECT, 'ptr': MEMVIEW_PTR, 'full': MEMVIEW_FULL, 'contig': MEMVIEW_CONTIG, 'strided': MEMVIEW_STRIDED, 'follow': MEMVIEW_FOLLOW}
_spec_to_abbrev = {'direct': 'd', 'ptr': 'p', 'full': 'f', 'contig': 'c', 'strided': 's', 'follow': '_'}
...
def get_mode(specs):
    ...
    for (access, packing) in specs:
        if access in ('ptr', 'full'):
            return 'full'
    return 'strided'
view_constant_to_access_packing = {'generic': ('full', 'strided'), 'strided': ('direct', 'strided'), 'indirect': ('ptr', 'strided'), 'generic_contiguous': ('full', 'contig'), 'contiguous': ('direct', 'contig'), 'indirect_contiguous': ('ptr', 'contig')}
...
context = {'memview_struct_name': memview_objstruct_cname, 'max_dims': Options.buffer_max_dims, 'memviewslice_name': memviewslice_cname, 'memslice_init': PyrexTypes.MemoryViewSliceType.default_value, 'THREAD_LOCKS_PREALLOCATED': 8}
...
memviewslice_init_code = load_memview_c_utility('MemviewSliceInit', context=dict(context, BUF_MAX_NDIMS=Options.buffer_max_dims), requires=[memviewslice_declare_code, atomic_utility])
memviewslice_index_helpers = load_memview_c_utility('MemviewSliceIndex')
typeinfo_to_format_code = load_memview_cy_utility('BufferFormatFromTypeInfo', requires=[Buffer._typeinfo_to_format_code])
...
copy_contents_new_utility = load_memview_c_utility('MemviewSliceCopyTemplate', context, requires=[])
view_utility_code = load_memview_cy_utility('View.MemoryView', context=context, requires=[Buffer.buffer_struct_declare_code, Buffer.buffer_formats_declare_code, memviewslice_init_code, is_contig_utility, overlapping_utility, copy_contents_new_utility])
view_utility_allowlist = ('array', 'memoryview', 'array_cwrapper', 'generic', 'strided', 'indirect', 'contiguous', 'indirect_contiguous')
