...
class TestAlgorithmRLModuleRestore(unittest.TestCase):
    ...
    def get_ppo_config(num_agents=NUM_AGENTS):
        ...
        def policy_mapping_fn(agent_id, episode, worker, **kwargs):
            ...
            return pol_id
        scaling_config = {'num_learner_workers': 0, 'num_gpus_per_learner_worker': 0}
        policies = {f'policy_{i}' for i in range(num_agents)}
        config = PPOConfig().experimental(_enable_new_api_stack=True).rollouts(rollout_fragment_length=4).environment(MultiAgentCartPole, env_config={'num_agents': num_agents}).training(num_sgd_iter=1, train_batch_size=8, sgd_minibatch_size=8).multi_agent(policies=policies, policy_mapping_fn=policy_mapping_fn).resources(**scaling_config)
        return config
    ...
    def test_e2e_load_simple_marl_module(self):
        ...
        for fw in framework_iterator(config, frameworks=['tf2', 'torch']):
            ...
            config = config.experimental(_enable_new_api_stack=True).rl_module(rl_module_spec=marl_module_spec_from_checkpoint)
            algo = config.build()
    ...
    def test_e2e_load_rl_module(self):
        ...
        scaling_config = {'num_learner_workers': 0, 'num_gpus_per_learner_worker': 0}
        config = PPOConfig().experimental(_enable_new_api_stack=True).rollouts(rollout_fragment_length=4).environment('CartPole-v1').training(num_sgd_iter=1, train_batch_size=8, sgd_minibatch_size=8).resources(**scaling_config)
        ...
        for fw in framework_iterator(config, frameworks=['tf2', 'torch']):
            ...
            config = config.experimental(_enable_new_api_stack=True).rl_module(rl_module_spec=module_to_load_spec)
            algo = config.build()
            algo_module_weights = algo.learner_group.get_weights()
