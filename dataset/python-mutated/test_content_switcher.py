...
async def test_no_initial_display() -> None:
    ...
    async with SwitcherApp().run_test() as pilot:
        ...
        assert all((not child.display for child in pilot.app.query_one(ContentSwitcher).children))
...
async def test_initial_display() -> None:
    ...
    async with SwitcherApp('w3').run_test() as pilot:
        ...
        assert pilot.app.query_one(ContentSwitcher).visible_content is pilot.app.query_one('#w3')
...
async def test_initial_display_then_change() -> None:
    ...
    async with SwitcherApp('w3').run_test() as pilot:
        ...
        assert pilot.app.query_one(ContentSwitcher).visible_content is pilot.app.query_one('#w2')
...
@pytest.mark.xfail(reason="The expected exception doesn't appear to make it to pytest -- perhaps related to https://github.com/Textualize/textual/issues/1972")
