...
def get_config():
    ...
    examples_private_info['ooo_days'] = [scenario_schedule_meeting.OOO_A, scenario_schedule_meeting.OOO_B]
    examples_private_info['day_prefs'] = [scenario_schedule_meeting.DAY_PREFS_A, scenario_schedule_meeting.DAY_PREFS_B]
    scenario_a = env_schedule_meeting_with_dow_info.Scenario(scenario_schedule_meeting.SCENARIO_A, 'Bob', 'Suzy', scenario_schedule_meeting.OOO_A, scenario_schedule_meeting.DAY_PREFS_A, 'Thursday')
    scenario_b = env_schedule_meeting_with_dow_info.Scenario(scenario_schedule_meeting.SCENARIO_B, 'Jill', 'George', scenario_schedule_meeting.OOO_B, scenario_schedule_meeting.DAY_PREFS_B, 'Friday')
    examples_scenarios = [scenario_a, scenario_b]
    llm_termination_prompt = scenario_schedule_meeting.LLM_TERMINATION_PROMPT
    params = {'num_distinct_actions': num_players * num_days, 'num_llm_seeds': 1, 'num_players': num_players, 'min_utility': min([float(p.min) for p in payoffs]), 'max_utility': max([float(p.max) for p in payoffs]), 'num_max_replies': 3}
