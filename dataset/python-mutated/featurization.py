...
def atom14_to_atom37(atom14, batch):
    if False:
        i = 10
        return i + 15
    atom37_data = batched_gather(atom14, batch['residx_atom37_to_atom14'], dim=-2, num_batch_dims=len(atom14.shape[:-2]))
...
def build_template_angle_feat(template_feats, v2_feature=False):
    ...
    if not v2_feature:
        ...
        template_angle_feat = torch.cat([one_hot(template_aatype, 22), torsion_angles_sin_cos.reshape(*torsion_angles_sin_cos.shape[:-2], 14), alt_torsion_angles_sin_cos.reshape(*alt_torsion_angles_sin_cos.shape[:-2], 14), torsion_angles_mask], dim=-1)
        ...
        template_angle_feat = torch.cat([one_hot(template_aatype, 22), chi_angles_sin, chi_angles_cos, chi_mask], dim=-1)
        template_angle_mask = chi_mask[..., 0]
    return (template_angle_feat, template_angle_mask)

def build_template_pair_feat(batch, min_bin, max_bin, num_bins, eps=1e-20, inf=100000000.0):
    if False:
        return 10
...
def build_template_pair_feat_v2(batch, min_bin, max_bin, num_bins, multichain_mask_2d=None, eps=1e-20, inf=100000000.0):
    if False:
        return 10
    template_mask = batch['template_pseudo_beta_mask']
    template_mask_2d = template_mask[..., None] * template_mask[..., None, :]
    ...
    rigids = Frame.make_transform_from_reference(n_xyz=batch['template_all_atom_positions'][..., n, :], ca_xyz=batch['template_all_atom_positions'][..., ca, :], c_xyz=batch['template_all_atom_positions'][..., c, :], eps=eps)
...
def build_extra_msa_feat(batch):
    ...
    msa_feat = [msa_1hot, batch['extra_msa_has_deletion'].unsqueeze(-1), batch['extra_msa_deletion_value'].unsqueeze(-1)]
