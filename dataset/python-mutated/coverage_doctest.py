...
if __name__ == '__main__':
    bintest_dir = os.path.abspath(os.path.dirname(__file__))
    sympy_top = os.path.split(bintest_dir)[0]
    sympy_dir = os.path.join(sympy_top, 'sympy')
    if os.path.isdir(sympy_dir):
        sys.path.insert(0, sympy_top)
    usage = 'usage: ./bin/doctest_coverage.py PATHS'
    parser = ArgumentParser(description=__doc__, usage=usage, formatter_class=RawDescriptionHelpFormatter)
    parser.add_argument('path', nargs='*', default=[os.path.join(sympy_top, 'sympy')])
    parser.add_argument('-v', '--verbose', action='store_true', dest='verbose', default=False)
    parser.add_argument('--no-colors', action='store_true', dest='no_color', help='use no colors', default=False)
    parser.add_argument('--no-sphinx', action='store_false', dest='sphinx', help="don't report Sphinx coverage", default=True)
    ...
    for file in args.path:
        ...
        print('=' * 70)
        print()
        (doctests, total_sphinx, num_functions) = go(sympy_top, file, verbose=args.verbose, no_color=args.no_color, sphinx=args.sphinx)
        if num_functions == 0:
            score = 100
            sphinx_score = 100
        else:
        ...
        print('=' * 70)
        if args.no_color:
            print('TOTAL DOCTEST SCORE for %s: %s%% (%s of %s)' % (get_mod_name(file, sympy_top), score, doctests, num_functions))
        elif score < 100:
            print('TOTAL DOCTEST SCORE for %s: %s%s%% (%s of %s)%s' % (get_mod_name(file, sympy_top), c_color % colors['Red'], score, doctests, num_functions, c_normal))
        else:
            print('TOTAL DOCTEST SCORE for %s: %s%s%% (%s of %s)%s' % (get_mod_name(file, sympy_top), c_color % colors['Green'], score, doctests, num_functions, c_normal))
        if args.sphinx:
            if args.no_color:
                print('TOTAL SPHINX SCORE for %s: %s%% (%s of %s)' % (get_mod_name(file, sympy_top), sphinx_score, num_functions - total_sphinx, num_functions))
            elif sphinx_score < 100:
                print('TOTAL SPHINX SCORE for %s: %s%s%% (%s of %s)%s' % (get_mod_name(file, sympy_top), c_color % colors['Red'], sphinx_score, num_functions - total_sphinx, num_functions, c_normal))
            else:
                print('TOTAL SPHINX SCORE for %s: %s%s%% (%s of %s)%s' % (get_mod_name(file, sympy_top), c_color % colors['Green'], sphinx_score, num_functions - total_sphinx, num_functions, c_normal))
        print()
