...
class TradingModelFXTrend_Example(TradingModel):
    ...
    def load_assets(self, br=None):
        ...
        market_data_request = MarketDataRequest(start_date=br.start_date, finish_date=br.finish_date, freq='daily', data_source='quandl', tickers=full_bkt, fields=['close'], vendor_tickers=vendor_tickers, vendor_fields=['close'], cache_algo='cache_algo_return')
    ...
    def construct_strategy_benchmark(self):
        if False:
            return 10
        tsr_indices = MarketDataRequest(start_date=self.br.start_date, finish_date=self.br.finish_date, freq='daily', data_source='quandl', tickers=['EURUSD'], vendor_tickers=['FRED/DEXUSEU'], fields=['close'], vendor_fields=['close'], cache_algo='cache_algo_return')
