...
class SkipTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    def testBasic(self, count):
        ...
        self.assertDatasetProduces(dataset, [tuple(components[0][i:i + 1]) for i in range(start_range, 10)])
...
class SkipDatasetCheckpointTest(checkpoint_test_base.CheckpointTestBase, parameterized.TestCase):
...
class SkipRandomAccessTest(test_base.DatasetTestBase, parameterized.TestCase):
    ...
    @combinations.generate(combinations.times(test_base.default_test_combinations(), combinations.combine(skip=[-2, -1])))
    ...
    @combinations.generate(combinations.times(test_base.default_test_combinations(), combinations.combine(skip=[5, 8])))
    ...
    @combinations.generate(combinations.times(test_base.default_test_combinations(), combinations.combine(elements=[0, 5, 10], skip=[-1, 0, 5, 15])))
    def testMultipleCombinations(self, elements, skip):
        ...
        for i in range(self.evaluate(dataset.cardinality())):
            self.assertEqual(self.evaluate(random_access.at(dataset, index=i)), i + skip)
