...
def opts_userpass():
    if False:
        for i in range(10):
            print('nop')
    return {'proxy': {'proxytype': 'esxcluster', 'vcenter': 'fake_vcenter', 'datacenter': 'fake_dc', 'cluster': 'fake_cluster', 'mechanism': 'userpass', 'username': 'fake_username', 'passwords': ['fake_password'], 'protocol': 'fake_protocol', 'port': 100}}

@pytest.fixture
def opts_sspi():
    if False:
        print('Hello World!')
    return {'proxy': {'proxytype': 'esxcluster', 'vcenter': 'fake_vcenter', 'datacenter': 'fake_dc', 'cluster': 'fake_cluster', 'mechanism': 'sspi', 'domain': 'fake_domain', 'principal': 'fake_principal', 'protocol': 'fake_protocol', 'port': 100}}
...
def configure_loader_modules(opts_sspi):
    ...
    with patch.dict(esxcluster.DETAILS):
        with patch('salt.proxy.esxcluster.merge', MagicMock(return_value=opts_sspi['proxy'])):
...
def test_invalid_proxy_input_error(opts_userpass):
    if False:
        for i in range(10):
            print('nop')
    with patch('salt.proxy.esxcluster.jsonschema.validate', MagicMock(side_effect=jsonschema.exceptions.ValidationError('Validation Error'))):
...
def test_find_credentials(opts_userpass):
    ...
    with patch('salt.proxy.esxcluster.merge', MagicMock(return_value=opts_userpass['proxy'])):
...
def test_details_userpass(opts_userpass):
    ...
    with patch('salt.proxy.esxcluster.merge', MagicMock(return_value=opts_userpass['proxy'])):
        with patch('salt.proxy.esxcluster.find_credentials', mock_find_credentials):
            esxcluster.init(opts_userpass)
    assert esxcluster.DETAILS == {'vcenter': 'fake_vcenter', 'datacenter': 'fake_dc', 'cluster': 'fake_cluster', 'mechanism': 'userpass', 'username': 'fake_username', 'password': 'fake_password', 'passwords': ['fake_password'], 'protocol': 'fake_protocol', 'port': 100}

def test_details_sspi(opts_sspi):
    if False:
        return 10
    esxcluster.init(opts_sspi)
    assert esxcluster.DETAILS == {'vcenter': 'fake_vcenter', 'datacenter': 'fake_dc', 'cluster': 'fake_cluster', 'mechanism': 'sspi', 'domain': 'fake_domain', 'principal': 'fake_principal', 'protocol': 'fake_protocol', 'port': 100}