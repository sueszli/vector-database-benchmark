...
def test_infinite():
    ...
    scheduler.start()
    assert results1.messages == [on_next(300, 4), on_next(340, 5), on_next(410, 6), on_next(520, 7)]
...
def test_finite():
    if False:
        for i in range(10):
            print('nop')
    scheduler = TestScheduler()
    xs = scheduler.create_hot_observable(on_next(70, 1), on_next(110, 2), on_next(220, 3), on_next(270, 4), on_next(340, 5), on_next(410, 6), on_next(520, 7), on_completed(630), on_next(640, 9), on_completed(650), on_error(660, RxException()))
    subject = [None]
    subscription = [None]
    subscription1 = [None]
    subscription2 = [None]
...
def test_error():
    ...
    scheduler = TestScheduler()
    ex = RxException()
    xs = scheduler.create_hot_observable(on_next(70, 1), on_next(110, 2), on_next(220, 3), on_next(270, 4), on_next(340, 5), on_next(410, 6), on_next(520, 7), on_error(630, ex), on_next(640, 9), on_completed(650), on_error(660, RxException()))
    subject = [None]
    subscription = [None]
    subscription1 = [None]
    subscription2 = [None]
...
def test_subject_disposed():
    ...
    assert results2.messages == [on_next(300, 2), on_next(350, 3), on_next(450, 4), on_next(550, 5)]
