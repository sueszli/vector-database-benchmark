...
class OrderLineDiscountUpdate(OrderDiscountCommon):
    order_line = graphene.Field(OrderLine, description='Order line which has been discounted.')
    order = graphene.Field(Order, description='Order which is related to the discounted line.')

    class Arguments:
        order_line_id = graphene.ID(description='ID of a order line to update price', required=True)
        input = OrderDiscountCommonInput(required=True, description='Fields required to update price for the order line.')
