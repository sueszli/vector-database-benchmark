...
from tests.models import DataclassPerson, DataclassPersonFactory, DataclassPetFactory, MsgSpecStructPerson
...
async def test_request_factory_create_with_data(data_cls: DataContainerType) -> None:
    ...
    request = RequestFactory()._create_request_with_data(HttpMethod.POST, '/', data=data_cls(**person_data))
...
async def test_request_factory_create_with_data_with_custom_encoder() -> None:
    ...
    class Foo:
        bar: str = 'baz'
    request = RequestFactory(app=Litestar(type_encoders={Foo: lambda f: {'bar': f.bar}}))._create_request_with_data(HttpMethod.POST, '/', data=Foo())
    body = await request.body()
    assert json.loads(body) == {'bar': 'baz'}

@pytest.mark.parametrize('request_media_type, verify_data', [[RequestEncodingType.JSON, lambda data: json.loads(data) == msgspec.to_builtins(pet)], [RequestEncodingType.MULTI_PART, lambda data: 'Content-Disposition' in data], [RequestEncodingType.URL_ENCODED, lambda data: data == f'name={pet.name}&age={pet.age}&species={pet.species.value}']])
async def test_request_factory_create_with_content_type(request_media_type: RequestEncodingType, verify_data: Callable[[str], bool]) -> None:
    request = RequestFactory()._create_request_with_data(HttpMethod.POST, '/', data=msgspec.to_builtins(pet), request_media_type=request_media_type)
...
@pytest.mark.parametrize('factory, method', [(RequestFactory().post, HttpMethod.POST), (RequestFactory().put, HttpMethod.PUT), (RequestFactory().patch, HttpMethod.PATCH)])
