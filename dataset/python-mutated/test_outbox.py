...
from sentry.models.outbox import ControlOutbox, OutboxCategory, OutboxFlushError, OutboxScope, RegionOutbox, WebhookProviderIdentifier, outbox_context
...
class ControlOutboxTest(TestCase):
    webhook_request = RequestFactory().post('/extensions/github/webhook/?query=test', data={'installation': {'id': 'github:1'}}, content_type='application/json', HTTP_X_GITHUB_EMOTICON='>:^]')
