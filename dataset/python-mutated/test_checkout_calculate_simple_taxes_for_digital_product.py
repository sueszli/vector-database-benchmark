...
from ..utils import checkout_billing_address_update, checkout_complete, checkout_create, checkout_dummy_payment_create, get_checkout
...
def prepare_shop(e2e_staff_api_client, shipping_price):
    ...
    channel_data = create_channel(e2e_staff_api_client, slug=channel_slug, warehouse_ids=warehouse_ids, currency='CZK', country='CZ')
    channel_id = channel_data['id']
    channel_ids = [channel_id]
    shipping_zone_data = create_shipping_zone(e2e_staff_api_client, countries=['CZ', 'US'], warehouse_ids=warehouse_ids, channel_ids=channel_ids)
...
@pytest.mark.e2e
def test_digital_checkout_calculate_simple_tax_based_on_billing_country_CORE_2007(e2e_staff_api_client, e2e_not_logged_api_client, permission_manage_products, permission_manage_channels, permission_manage_product_types_and_attributes, permission_manage_shipping, permission_manage_taxes):
    if False:
        i = 10
        return i + 15
    ...
    billing_country_tax_rate = prepare_tax_configuration(e2e_staff_api_client, channel_slug, shipping_country_code='US', shipping_country_tax_rate=9, billing_country_code='CZ', billing_country_tax_rate=21, prices_entered_with_tax=False)
    variant_price = 88.89
    (_product_id, product_variant_id, product_variant_price) = prepare_digital_product(e2e_staff_api_client, channel_id, warehouse_id, variant_price)
    ...
    calculated_tax = round(product_variant_price * (billing_country_tax_rate / 100), 2)
    ...
    assert order_data['total']['gross']['amount'] == product_variant_price + calculated_tax