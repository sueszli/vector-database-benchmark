...
class SimpleDownloader(BaseDownloader):
    ...
    NAME_PATTERN = None
    SIZE_PATTERN = None
    HASHSUM_PATTERN = '[^\\w](?P<H>(CRC|crc)(-?32)?|(MD|md)-?5|(SHA|sha)-?(1|224|256|384|512)).*(:|=|>)[ ]*(?P<D>(?:[a-z0-9]|[A-Z0-9]){8,})'
    ...
    ERROR_PATTERN = None
    FILE_ERRORS = [('Html error', b'\\A(?:\\s*<.+>)?((?:[\\w\\s]*(?:[Ee]rror|ERROR)\\s*\\:?)?\\s*\\d{3})(?:\\Z|\\s+)'), ('Request error', b'([Aa]n error occurred while processing your request)'), ('Html file', b'\\A\\s*<!DOCTYPE html')]
