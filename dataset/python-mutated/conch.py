...
class ClientOptions(ConchOptions):
    ...
    longdesc = 'conch is a SSHv2 client that allows logging into a remote machine and executing commands.'
    optParameters = [['escape', 'e', '~'], ['localforward', 'L', None, 'listen-port:host:port   Forward local port to remote address'], ['remoteforward', 'R', None, 'listen-port:host:port   Forward remote port to local address']]
    optFlags = [['null', 'n', 'Redirect input from /dev/null.'], ['fork', 'f', 'Fork to background after authentication.'], ['tty', 't', 'Tty; allocate a tty even if command is given.'], ['notty', 'T', 'Do not allocate a tty.'], ['noshell', 'N', 'Do not execute a shell or command.'], ['subsystem', 's', 'Invoke command (mandatory) as SSH2 subsystem.']]
    compData = usage.Completions(mutuallyExclusive=[('tty', 'notty')], optActions={'localforward': usage.Completer(descr='listen-port:host:port'), 'remoteforward': usage.Completer(descr='listen-port:host:port')}, extraActions=[usage.CompleteUserAtHost(), usage.Completer(descr='command'), usage.Completer(descr='argument', repeat=True)])
