...
class vpcaccessCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_connector': ('parent', 'connector_id', 'connector'), 'delete_connector': ('name',), 'get_connector': ('name',), 'list_connectors': ('parent', 'page_size', 'page_token')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=vpcaccessCallTransformer()):
    if False:
        return 10
