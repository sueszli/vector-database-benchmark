...
def get_mock_provider(valid_api_key):
    if False:
        print('Hello World!')
    return Provider(id='provider_id', tenant_id='tenant_id', provider_name='tongyi', provider_type=ProviderType.CUSTOM.value, encrypted_config=json.dumps({'dashscope_api_key': valid_api_key}), is_valid=True)
...
def get_mock_model(model_name):
    ...
    return TongyiModel(model_provider=model_provider, name=model_name, model_kwargs=model_kwargs)
...
def test_run(mock_decrypt, mocker):
    ...
    model = get_mock_model('qwen-turbo')
    rst = model.run([PromptMessage(content='Human: Are you Human? you MUST only answer `y` or `n`? \nAssistant: ')], stop=['\nHuman:'])
