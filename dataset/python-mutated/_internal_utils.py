...
from typing import AsyncIterable, AsyncIterator, Awaitable, Callable, Generator, Iterable, Iterator, List, Optional, Union, TypeVar, TYPE_CHECKING, Tuple, cast
import aiohttp
import discord
...
__all__ = ('safe_delete', 'fuzzy_command_search', 'format_fuzzy_results', 'create_backup', 'send_to_owners_with_preprocessor', 'send_to_owners_with_prefix_replaced', 'expected_version', 'fetch_latest_red_version_info', 'deprecated_removed', 'RichIndefiniteBarColumn', 'cli_level_to_log_level')
_T = TypeVar('_T')
...
async def fuzzy_command_search(ctx: Context, term: Optional[str]=None, *, commands: Optional[Union[AsyncIterator[Command], Iterator[Command]]]=None, min_score: int=80) -> Optional[List[Command]]:
    ...
    extracted = rapidfuzz.process.extract(term, choices, limit=5, scorer=rapidfuzz.fuzz.QRatio, processor=rapidfuzz.utils.default_process)
    if not extracted:
        return None
    matched_commands = []
...
async def create_backup(dest: Path=Path.home()) -> Optional[Path]:
    ...
    to_backup = []
    exclusions = ['__pycache__', 'Lavalink.jar', os.path.join('Downloader', 'lib'), os.path.join('CogManager', 'cogs'), os.path.join('RepoManager', 'repos'), os.path.join('Audio', 'logs')]
...
async def send_to_owners_with_preprocessor(bot: Red, content: str, *, content_preprocessor: Optional[Callable[[Red, discord.abc.Messageable, str], Awaitable[str]]]=None, **kwargs):
    """
    ...
    async def wrapped_send(bot, location, content=None, preprocessor=None, **kwargs):
        try:
            ...
            main_log.error('I could not send an owner notification to %s (%s)', location, location.id, exc_info=_exc)
...
async def send_to_owners_with_prefix_replaced(bot: Red, content: str, **kwargs):
    ...
    async def preprocessor(bot: Red, destination: discord.abc.Messageable, content: str) -> str:
        ...
        prefix = re.sub(f'<@!?{bot.user.id}>', f'@{bot.user.name}'.replace('\\', '\\\\'), prefixes[0])
