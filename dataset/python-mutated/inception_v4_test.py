...
class InceptionTest(tf.test.TestCase):
    ...
    def testBuildLogits(self):
        ...
        self.assertTrue(predictions.op.name.startswith('InceptionV4/Logits/Predictions'))
    ...
    def testBuildWithoutAuxLogits(self):
        ...
        num_classes = 1000
        inputs = tf.random_uniform((batch_size, height, width, 3))
        (logits, endpoints) = inception.inception_v4(inputs, num_classes, create_aux_logits=False)
    ...
    def testBuildBaseNetwork(self):
        ...
        expected_endpoints = ['Conv2d_1a_3x3', 'Conv2d_2a_3x3', 'Conv2d_2b_3x3', 'Mixed_3a', 'Mixed_4a', 'Mixed_5a', 'Mixed_5b', 'Mixed_5c', 'Mixed_5d', 'Mixed_5e', 'Mixed_6a', 'Mixed_6b', 'Mixed_6c', 'Mixed_6d', 'Mixed_6e', 'Mixed_6f', 'Mixed_6g', 'Mixed_6h', 'Mixed_7a', 'Mixed_7b', 'Mixed_7c', 'Mixed_7d']
    ...
    def testBuildOnlyUpToFinalEndpoint(self):
        ...
        batch_size = 5
        (height, width) = (299, 299)
        all_endpoints = ['Conv2d_1a_3x3', 'Conv2d_2a_3x3', 'Conv2d_2b_3x3', 'Mixed_3a', 'Mixed_4a', 'Mixed_5a', 'Mixed_5b', 'Mixed_5c', 'Mixed_5d', 'Mixed_5e', 'Mixed_6a', 'Mixed_6b', 'Mixed_6c', 'Mixed_6d', 'Mixed_6e', 'Mixed_6f', 'Mixed_6g', 'Mixed_6h', 'Mixed_7a', 'Mixed_7b', 'Mixed_7c', 'Mixed_7d']
        for (index, endpoint) in enumerate(all_endpoints):
            with tf.Graph().as_default():
                inputs = tf.random_uniform((batch_size, height, width, 3))
                (out_tensor, end_points) = inception.inception_v4_base(inputs, final_endpoint=endpoint)
                self.assertTrue(out_tensor.op.name.startswith('InceptionV4/' + endpoint))
    ...
    def testGlobalPoolUnknownImageShape(self):
        ...
        with self.test_session() as sess:
            ...
            (logits, end_points) = inception.inception_v4(inputs, num_classes, create_aux_logits=False)
            self.assertTrue(logits.op.name.startswith('InceptionV4/Logits'))
            self.assertListEqual(logits.get_shape().as_list(), [batch_size, num_classes])
            pre_pool = end_points['Mixed_7d']
    ...
    def testEvaluation(self):
        ...
        with self.test_session() as sess:
            ...
            (logits, _) = inception.inception_v4(eval_inputs, num_classes, is_training=False)
    ...
    def testTrainEvalWithReuse(self):
        ...
        with self.test_session() as sess:
            ...
            (logits, _) = inception.inception_v4(eval_inputs, num_classes, is_training=False, reuse=True)
            predictions = tf.argmax(logits, 1)
            sess.run(tf.global_variables_initializer())
            output = sess.run(predictions)
    ...
    def testBatchNormScale(self):
        ...
        num_classes = 1000
        inputs = tf.placeholder(tf.float32, (1, height, width, 3))
        with contrib_slim.arg_scope(inception.inception_v4_arg_scope(batch_norm_scale=True)):
            inception.inception_v4(inputs, num_classes, is_training=False)
