...
async def handle_analytics_event(cursor, event, server):
    ...
    for (command, stats) in event['api'].items():
        data = {'server': server, 'command': command, 'event_time': datetime.now()}
        for (key, value) in stats.items():
            ...
            if isinstance(value, list):
                data.update({key + '_avg': value[0], key + '_min': value[1], key + '_five': value[2], key + '_twenty_five': value[3], key + '_fifty': value[4], key + '_seventy_five': value[5], key + '_ninety_five': value[6], key + '_max': value[7]})
            else:
                data[key] = value
...
async def boris_says(what_boris_says):
    if SLACKCLIENT:
        await SLACKCLIENT.chat_postMessage(username='boris the wallet monitor', icon_emoji=':boris:', channel='#tech-sdk', text=what_boris_says)
    else:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(10)) as session:
                try:
                    ...
                    if first_attempt:
                        ...
                        await boris_says(random.choice([f'{server} is not responding, probably dead, will not connect again.']))
                        return
                    raise
                if first_attempt:
                ...
                async for msg in ws:
                    ...
                    if height is None:
                        ...
                        if height_change_reported:
                            await boris_says(f'Server {server} received new block after {height_change_time / 60:.1f} minutes.')
                            height_change_reported = False
                    elif height_change_time > 30 * 60:
                        if not height_change_reported or height_change_time % (2 * 60) == 0:
                            await boris_says(f"It's been {height_change_time / 60:.1f} minutes since {server} received a new block.")
                            height_change_reported = True
                    await handle_analytics_event(c, event, server)
                    db.commit()
        except (aiohttp.ClientConnectionError, asyncio.TimeoutError):
            await boris_says(random.choice([f'<!channel> Guys, we have a problem! Nobody home at {server}. Will check on it again in {delay} seconds.', f'<!channel> Something wrong with {server}. I think dead. Will poke it again in {delay} seconds.', f"<!channel> Don't hear anything from {server}, maybe dead. Will try it again in {delay} seconds."]))
            await asyncio.sleep(delay)
            delay += 30

async def main(dsn, servers):
    db = ensure_database(dsn)
    await boris_says(random.choice(['No fear, Boris is here! I will monitor the servers now and will try not to fall asleep again.', 'Comrad the Cat and Boris are here now, monitoring wallet servers.']))
    await asyncio.gather(*(asyncio.create_task(monitor(db, server)) for server in servers))
...
if __name__ == '__main__':
    ...
    try:
        ...
        loop.run_until_complete(boris_says(random.choice(["Wallet servers will have to watch themselves, I'm leaving now.", "I'm going to go take a nap, hopefully nothing blows up while I'm gone.", "Babushka is calling, I'll be back later, someone else watch the servers while I'm gone."])))