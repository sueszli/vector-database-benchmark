...
if lightning_clf is not None and (not fit_intercept):
    ...
    clfs.append(('Lightning-SVRG', lightning_clf.SVRGClassifier(alpha=alpha, eta=step_size, tol=tol, loss='log'), sag_iter_range, [], [], [], []))
    clfs.append(('Lightning-SAG', lightning_clf.SAGClassifier(alpha=alpha, eta=step_size, tol=tol, loss='log'), sag_iter_range, [], [], [], []))
    X_csc = X.tocsc()
    nnz_in_each_features = X_csc.indptr[1:] - X_csc.indptr[:-1]
    X = X_csc[:, np.argsort(nnz_in_each_features)[-200:]]
    X = X.toarray()
    print('dataset: %.3f MB' % (X.nbytes / 1000000.0))
n = 23149
X_test = X[:n, :]
y_test = y[:n]
X = X[n:, :]
