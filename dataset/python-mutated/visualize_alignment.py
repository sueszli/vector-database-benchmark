...
if __name__ == '__main__':
    ...
    parser.add_argument('--list_posegraphs', help='list number of node and edges of all pose graphs', action='store_true')
    ...
    with open(args.config) as json_file:
        ...
        if args.list_posegraphs:
            list_posegraph_files(join(config['path_dataset'], config['folder_fragment']))
            list_posegraph_files(join(config['path_dataset'], config['folder_scene']))
        if args.before_optimized:
            global_pose_graph_name = join(config['path_dataset'], config['template_global_posegraph'])
        else:
            global_pose_graph_name = join(config['path_dataset'], config['template_refined_posegraph_optimized'])
        print('Reading posegraph')
        print(global_pose_graph_name)
        pose_graph = o3d.io.read_pose_graph(global_pose_graph_name)
        n_nodes = len(pose_graph.nodes)
        n_edges = len(pose_graph.edges)
        print('Global o3d.pipelines.registration.PoseGraph having %d nodes and %d edges' % (n_nodes, n_edges))
        for edge in pose_graph.edges:
            print('o3d.pipelines.registration.PoseGraphEdge %d-%d' % (edge.source_node_id, edge.target_node_id))
            if args.adjacent and edge.target_node_id - edge.source_node_id == 1 or (not args.adjacent and (args.source_id == edge.source_node_id and args.target_id == edge.target_node_id)) or args.all:
                ...
                draw_registration_result(source_down, target_down, edge.transformation)
                print('optimized registration')
                source_down.transform(pose_graph.nodes[edge.source_node_id].pose)
                target_down.transform(pose_graph.nodes[edge.target_node_id].pose)
