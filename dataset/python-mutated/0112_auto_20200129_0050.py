...
def create_unique_slugs_for_producttypes(apps, schema_editor):
    ...
    product_types = ProductType.objects.filter(slug__isnull=True).order_by(Lower('name')).iterator()
    previous_char = ''
    slug_values = []
    for product_type in product_types:
        ...
        if first_char != previous_char:
            ...
            slug_values = list(ProductType.objects.filter(slug__istartswith=first_char).values_list('slug', flat=True))
...
def update_non_unique_slugs_for_models(apps, schema_editor):
    ...
    for model in models_to_update:
        ...
        duplicated_slugs = Model.objects.all().values('slug').annotate(duplicated_slug_num=models.Count('slug')).filter(duplicated_slug_num__gt=1)
...
class Migration(migrations.Migration):
    dependencies = [('product', '0111_auto_20191209_0437')]
    operations = [migrations.RunPython(update_non_unique_slugs_for_models, migrations.RunPython.noop), migrations.AddField(model_name='producttype', name='slug', field=models.SlugField(null=True, max_length=128, unique=True), preserve_default=False), migrations.AlterField(model_name='category', name='slug', field=models.SlugField(max_length=128, unique=True)), migrations.AlterField(model_name='collection', name='slug', field=models.SlugField(max_length=128, unique=True)), migrations.RunPython(create_unique_slugs_for_producttypes, migrations.RunPython.noop), migrations.AlterField(model_name='producttype', name='slug', field=models.SlugField(max_length=128, unique=True))]