...
class OrderUpdateInput(BaseInputObjectType):
    ...
    external_reference = graphene.String(description='External ID of this order.' + ADDED_IN_310, required=False)
...
class OrderUpdate(DraftOrderCreate, ModelWithExtRefMutation):
    ...
    class Arguments:
        ...
        external_reference = graphene.String(required=False, description=f'External ID of an order to update. {ADDED_IN_310}')
        input = OrderUpdateInput(required=True, description='Fields required to update an order.')
