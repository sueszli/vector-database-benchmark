...
class PtbModel(paddle.nn.Layer):
    ...
    def __init__(self, hidden_size, vocab_size, num_layers=2, num_steps=20, init_scale=0.1, dropout=None):
        ...
        self.embedding = Embedding(vocab_size, hidden_size, sparse=False, weight_attr=base.ParamAttr(name='embedding_para', initializer=paddle.nn.initializer.Uniform(low=-init_scale, high=init_scale)))
...
class TestDygraphPtbRnn(unittest.TestCase):
    ...
    def func_setUp(self):
        ...
        with base.dygraph.guard():
            ...
            ptb_model = PtbModel(hidden_size=hidden_size, vocab_size=vocab_size, num_layers=num_layers, num_steps=num_steps, init_scale=init_scale)
            bd = []
            lr_arr = [1.0]
            for i in range(1, 10):
                bd.append(100 * i)
            ...
            scheduler = paddle.optimizer.lr.PiecewiseDecay(boundaries=bd, values=lr_arr)
            adam = Adam(learning_rate=scheduler, parameters=ptb_model.parameters())
            dy_param_updated = {}
            dy_param_init = {}
            dy_loss = None
            last_hidden = None
            ...
            for (k, v) in self.opti_dict.items():
                if isinstance(v, core.eager.Tensor):
                    ...
                    self.base_opti[k] = v
            paddle.save(self.opti_dict, os.path.join(self.temp_dir.name, 'test_dy_v2.pdopt'))
            self.state_dict = ptb_model.state_dict()
            self.model_base = {}
            for (k, v) in self.state_dict.items():
                np_t = v.numpy()
                self.model_base[k] = np_t
            paddle.save(self.state_dict, os.path.join(self.temp_dir.name, 'test_dy_v2.pdparams'))
        ...
        with base.dygraph.guard():
            ...
            for (k, v) in opti_dict.items():
                if isinstance(v, core.eager.Tensor):
                    ...
                    self.assertTrue(np.sum(np.abs(v.numpy())) == 0)
            para_state_dict = paddle.load(os.path.join(self.temp_dir.name, 'test_dy_v2.pdparams'))
            opti_state_dict = paddle.load(os.path.join(self.temp_dir.name, 'test_dy_v2.pdopt'))
            adam.set_state_dict(opti_state_dict)
            opti_dict = adam.state_dict()
            for (k, v) in opti_dict.items():
                if isinstance(v, core.eager.Tensor):
                    np.testing.assert_array_equal(v.numpy(), self.base_opti[v.name])
                else:
    ...
    def func_testSetVariableBeforeTrain(self):
        ...
        with base.dygraph.guard():
            ...
            ptb_model = PtbModel(hidden_size=hidden_size, vocab_size=vocab_size, num_layers=num_layers, num_steps=num_steps, init_scale=init_scale)
            place = base.CPUPlace() if not core.is_compiled_with_cuda() else base.CUDAPlace(0)
            adam = Adam(learning_rate=0.0, beta1=0.8, beta2=0.6, parameters=ptb_model.parameters())
            dy_param_updated = {}
            dy_param_init = {}
            dy_loss = None
            last_hidden = None
            ...
            for (k, v) in opti_dict.items():
                if k == 'global_step':
                    np.testing.assert_array_equal(v.numpy(), self.base_opti[v.name] + 1)
                if k.find('beta1_pow_acc_0') > 0:
                    np.testing.assert_array_equal(v.numpy(), self.base_opti[v.name] * adam._beta1)
                if k.find('beta2_pow_acc_0') > 0:
                    np.testing.assert_array_equal(v.numpy(), self.base_opti[v.name] * adam._beta2)
            state_dict = ptb_model.state_dict()
            for (k, v) in state_dict.items():
                new_t = v.numpy()
                base_t = self.model_base[k]
    ...
    def func_testSetNumpyBeforeTrain(self):
        ...
        with base.dygraph.guard():
            ...
            place = base.CPUPlace() if not core.is_compiled_with_cuda() else base.CUDAPlace(0)
            scheduler = paddle.optimizer.lr.PiecewiseDecay(boundaries=bd, values=lr_arr)
            adam = Adam(learning_rate=scheduler, beta1=0.8, beta2=0.6, parameters=ptb_model.parameters())
            dy_param_updated = {}
            dy_param_init = {}
            dy_loss = None
            last_hidden = None
            ...
            for (k, v) in opti_dict.items():
                if k == 'LR_Scheduler':
                    np.testing.assert_array_equal(v['last_epoch'], self.base_opti[k]['last_epoch'] + 1)
                if k.find('beta1_pow_acc_0') > 0:
                    np.testing.assert_array_equal(v.numpy(), self.base_opti[v.name] * adam._beta1)
                if k.find('beta2_pow_acc_0') > 0:
                    np.testing.assert_array_equal(v.numpy(), self.base_opti[v.name] * adam._beta2)
    ...
    def func_testOnlyLoadParams(self):
        ...
        with base.dygraph.guard():
            ...
            paddle.save(state_dict, os.path.join(self.temp_dir.name, 'saved_dy', 'emb_dy.pdparams'))
            para_state_dict = paddle.load(os.path.join(self.temp_dir.name, 'saved_dy', 'emb_dy.pdparams'))
    ...
    def func_test_state_shape_mismatch(self):
        ...
        with base.dygraph.guard():
            ...
            state_dict = emb.state_dict()
            paddle.save(state_dict, os.path.join(self.temp_dir.name, 'saved_dy', 'emb_dy.pdparams'))
            para_state_dict = paddle.load(os.path.join(self.temp_dir.name, 'saved_dy', 'emb_dy.pdparams'), return_numpy=True)
            para_state_dict['weight'] = np.expand_dims(para_state_dict['weight'], axis=-1)
            emb.set_state_dict(para_state_dict)
