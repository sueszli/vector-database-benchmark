...
async def main() -> None:
    ...
    @quart_app.post('/telegram')
    async def telegram() -> Response:
        """Handle incoming Telegram updates by putting them into the `update_queue`"""
        await application.update_queue.put(Update.de_json(data=await request.get_json(), bot=application.bot))
        return Response(status=HTTPStatus.OK)

    @quart_app.route('/submitpayload', methods=['GET', 'POST'])
    ...
    @quart_app.get('/healthcheck')
    async def health() -> Response:
        """For the health endpoint, reply with a simple plain text message."""
        response = await make_response('The bot is still running fine :)', HTTPStatus.OK)
        response.mimetype = 'text/plain'
        return response
    webserver = uvicorn.Server(config=uvicorn.Config(app=quart_app, port=PORT, use_colors=False, host='127.0.0.1'))
