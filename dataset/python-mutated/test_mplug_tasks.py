...
class MplugTasksTest(unittest.TestCase):
    ...
    def test_run_with_image_captioning_with_model(self):
        if False:
            while True:
                i = 10
        model = Model.from_pretrained('damo/mplug_image-captioning_coco_base_en')
        pipeline_caption = pipeline(task=Tasks.image_captioning, model=model)
    ...
    def test_run_with_image_captioning_with_name(self):
        if False:
            while True:
                i = 10
        pipeline_caption = pipeline(Tasks.image_captioning, model='damo/mplug_image-captioning_coco_base_en')
        image = Image.open('data/test/images/image_mplug_vqa.jpg')
        result = pipeline_caption(image)
        print(result[OutputKeys.CAPTION])
    ...
    def test_run_with_image_text_retrieval_with_model(self):
        if False:
            i = 10
            return i + 15
        model = Model.from_pretrained('damo/mplug_image-text-retrieval_flickr30k_large_en')
    ...
    def test_run_with_image_captioning_zh_base_with_name(self):
        if False:
            i = 10
            return i + 15
        pipeline_caption = pipeline(Tasks.image_captioning, model='damo/mplug_image-captioning_coco_base_zh')
        image = Image.open('data/test/images/image_mplug_vqa.jpg')
        result = pipeline_caption(image)
        print(result[OutputKeys.CAPTION])
