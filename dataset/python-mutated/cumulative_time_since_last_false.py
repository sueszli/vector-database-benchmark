...
class CumulativeTimeSinceLastFalse(TransformPrimitive):
    ...
    input_types = [ColumnSchema(logical_type=Datetime, semantic_tags={'time_index'}), ColumnSchema(logical_type=Boolean)]
    ...
    def get_function(self):
        ...
        def time_since_previous_false(datetime_col, bool_col):
            ...
            df = pd.DataFrame({'datetime': datetime_col, 'last_false_datetime': datetime_col, 'bool': bool_col})
            ...
            total_seconds = (df['datetime'] - df['last_false_datetime']).dt.total_seconds()
