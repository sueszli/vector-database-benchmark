...
class TestCRUDAsync:
    ...
    async def _set_up(cls):
        ...
        cls.database_for_test = await cls.client.create_database_if_not_exists(test_config._test_config.TEST_DATABASE_ID)
    ...
    async def test_database_crud_async(self):
        ...
        databases = [database async for database in self.client.query_databases(query='SELECT * FROM root r WHERE r.id=@id', parameters=[{'name': '@id', 'value': database_id}])]
    ...
    async def test_sql_query_crud_async(self):
        ...
        databases = [database async for database in self.client.query_databases(query='SELECT * FROM root r WHERE r.id=@id', parameters=[{'name': '@id', 'value': db1.id}])]
        assert 1 == len(databases)
        databases = [database async for database in self.client.query_databases(query='SELECT * FROM root r WHERE r.id="database non-existing"')]
    ...
    async def test_collection_crud_async(self):
        ...
        created_collection = await created_db.create_container(id=collection_id, indexing_policy=collection_indexing_policy, partition_key=PartitionKey(path='/pk', kind='Hash'))
        ...
        collections = [collection async for collection in created_db.query_containers(query='SELECT * FROM root r WHERE r.id=@id', parameters=[{'name': '@id', 'value': collection_id}])]
        ...
        container_proxy = await created_db.create_container_if_not_exists(id=created_collection.id, partition_key=PartitionKey(path='/id', kind='Hash'))
        ...
        container_proxy = await created_db.create_container_if_not_exists(id=created_collection.id, partition_key=created_properties['partitionKey'])
    ...
    async def test_partitioned_collection_async(self):
        await self._set_up()
        created_db = self.database_for_test
        collection_definition = {'id': 'test_partitioned_collection ' + str(uuid.uuid4()), 'partitionKey': {'paths': ['/id'], 'kind': documents.PartitionKind.Hash}}
        offer_throughput = 10100
        created_collection = await created_db.create_container(id=collection_definition['id'], partition_key=collection_definition['partitionKey'], offer_throughput=offer_throughput)
        assert collection_definition.get('id') == created_collection.id
        created_collection_properties = await created_collection.read()
        assert collection_definition.get('partitionKey').get('paths')[0] == created_collection_properties['partitionKey']['paths'][0]
    ...
    async def test_partitioned_collection_quota_async(self):
        ...
        retrieved_collection = created_db.get_container_client(container=created_collection.id)
        retrieved_collection_properties = await retrieved_collection.read(populate_partition_key_range_statistics=True, populate_quota_info=True)
    ...
    async def test_partitioned_collection_partition_key_extraction_async(self):
        ...
        created_collection = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path='/address/state', kind=documents.PartitionKind.Hash))
        ...
        created_collection1 = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path='/address', kind=documents.PartitionKind.Hash))
        self.OriginalExecuteFunction = _retry_utility_async.ExecuteFunctionAsync
        _retry_utility_async.ExecuteFunctionAsync = self._mock_execute_function
        ...
        created_collection2 = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path='/address/state/city', kind=documents.PartitionKind.Hash))
        self.OriginalExecuteFunction = _retry_utility_async.ExecuteFunctionAsync
        _retry_utility_async.ExecuteFunctionAsync = self._mock_execute_function
    ...
    async def test_partitioned_collection_partition_key_extraction_special_chars_async(self):
        ...
        created_collection1 = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path='/"level\' 1*()"/"le/vel2"', kind=documents.PartitionKind.Hash))
        ...
        created_collection2 = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path='/\'level" 1*()\'/\'le/vel2\'', kind=documents.PartitionKind.Hash))
    ...
    async def test_partitioned_collection_document_crud_and_query_async(self):
        ...
        read_document = await created_collection.read_item(item=created_document.get('id'), partition_key=created_document.get('id'))
        ...
        document_list = [document async for document in created_collection.query_items(query="SELECT * FROM root r WHERE r.id='" + replaced_document.get('id') + "'")]
        assert len(document_list) == 1
        document_list = [document async for document in created_collection.query_items(query="SELECT * FROM root r WHERE r.key='" + replaced_document.get('key') + "'")]
        assert len(document_list) == 1
        document_list = [document async for document in created_collection.query_items(query="SELECT * FROM root r WHERE r.key='" + replaced_document.get('key') + "'", partition_key=replaced_document.get('id'))]
        assert len(document_list) == 1
        await self._clear()
    ...
    async def test_partitioned_collection_permissions_async(self):
        ...
        all_collection = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path='/key', kind=documents.PartitionKind.Hash))
        collection_id = 'test_partitioned_collection_permissions read collection' + str(uuid.uuid4())
        read_collection = await created_db.create_container(id=collection_id, partition_key=PartitionKey(path='/key', kind=documents.PartitionKind.Hash))
        user = await created_db.create_user(body={'id': 'user' + str(uuid.uuid4())})
        permission_definition = {'id': 'all permission', 'permissionMode': documents.PermissionMode.All, 'resource': all_collection.container_link, 'resourcePartitionKey': [1]}
        all_permission = await user.create_permission(body=permission_definition)
        permission_definition = {'id': 'read permission', 'permissionMode': documents.PermissionMode.Read, 'resource': read_collection.container_link, 'resourcePartitionKey': [1]}
        ...
        async with CosmosClient(TestCRUDAsync.host, resource_tokens) as restricted_client:
            ...
            await self.__assert_http_failure_with_status(StatusCodes.FORBIDDEN, read_collection.create_item, document_definition)
            document_definition['key'] = 2
            await self.__assert_http_failure_with_status(StatusCodes.FORBIDDEN, all_collection.create_item, document_definition)
            document_definition['key'] = 1
            await all_collection.delete_item(item=created_document['id'], partition_key=document_definition['key'])
            await self.__assert_http_failure_with_status(StatusCodes.FORBIDDEN, read_collection.delete_item, document_definition['id'], document_definition['id'])
            await self._clear()

    @pytest.mark.asyncio
    async def test_partitioned_collection_execute_stored_procedure_async(self):
        await self._set_up()
        created_collection = await self.database_for_test.create_container(test_config._test_config.TEST_COLLECTION_MULTI_PARTITION_WITH_CUSTOM_PK_PARTITION_KEY, PartitionKey(path='/pk'))
        sproc = {'id': 'storedProcedure' + str(uuid.uuid4()), 'body': 'function () {' + '   var client = getContext().getCollection();' + "   client.createDocument(client.getSelfLink(), { id: 'testDoc', pk : 2}, {}, function(err, docCreated, options) { " + "   if(err) throw new Error('Error while creating document: ' + err.message);" + '   else {' + '   getContext().getResponse().setBody(1);' + '        }' + '   });}'}
        ...
        assert result == 1
        await self.__assert_http_failure_with_status(StatusCodes.BAD_REQUEST, created_collection.scripts.execute_stored_procedure, created_sproc['id'])
        await self._clear()
    ...
    async def test_partitioned_collection_partition_key_value_types_async(self):
        await self._set_up()
        created_db = self.database_for_test
        created_collection = await created_db.create_container(id='test_partitioned_collection_partition_key_value_types ' + str(uuid.uuid4()), partition_key=PartitionKey(path='/pk', kind='Hash'))
        ...
        await created_collection.create_item(body=document_definition)
        document_definition = {'id': 'document1' + str(uuid.uuid4()), 'pk': 10.5, 'spam': 'eggs'}
        await created_collection.create_item(body=document_definition)
        document_definition = {'name': 'sample document', 'spam': 'eggs', 'pk': 'value'}
        await self.__assert_http_failure_with_status(StatusCodes.BAD_REQUEST, created_collection.create_item, document_definition)
        await self._clear()
    ...
    async def test_partitioned_collection_conflict_crud_and_query_async(self):
        ...
        conflict_definition = {'id': 'new conflict', 'resourceId': 'doc1', 'operationType': 'create', 'resourceType': 'document'}
        await self.__assert_http_failure_with_status(StatusCodes.NOT_FOUND, created_collection.get_conflict, conflict_definition['id'], conflict_definition['id'])
        conflict_list = [conflict async for conflict in created_collection.list_conflicts()]
        assert len(conflict_list) == 0
        await self.__assert_http_failure_with_status(StatusCodes.NOT_FOUND, created_collection.delete_conflict, conflict_definition['id'], conflict_definition['id'])
        conflict_list = [conflict async for conflict in created_collection.query_conflicts(query="SELECT * FROM root r WHERE r.resourceType='" + conflict_definition.get('resourceType') + "'")]
        assert len(conflict_list) == 0
        options = {'partitionKey': conflict_definition.get('id')}
        conflict_list = [conflict async for conflict in created_collection.query_conflicts(query="SELECT * FROM root r WHERE r.resourceType='" + conflict_definition.get('resourceType') + "'", partition_key=conflict_definition['id'])]
        assert len(conflict_list) == 0
        await self._clear()
    ...
    async def test_document_crud_async(self):
        ...
        document_definition = {'name': 'sample document', 'spam': 'eggs', 'key': 'value'}
        ...
        document_definition = {'name': 'sample document', 'spam': 'eggs', 'key': 'value', 'id': str(uuid.uuid4())}
        created_document = await created_collection.create_item(body=document_definition)
        assert created_document.get('name') == document_definition['name']
        assert created_document.get('id') == document_definition['id']
        duplicated_definition_with_id = document_definition.copy()
        await self.__assert_http_failure_with_status(StatusCodes.CONFLICT, created_collection.create_item, duplicated_definition_with_id)
        document_list = [document async for document in created_collection.read_all_items()]
        assert len(document_list) == before_create_documents_count + 2
        document_list = [document async for document in created_collection.query_items(query='SELECT * FROM root r WHERE r.name=@name', parameters=[{'name': '@name', 'value': document_definition['name']}])]
        assert document_list is not None
        document_list = [document async for document in created_collection.query_items(query='SELECT * FROM root r WHERE r.name=@name', parameters=[{'name': '@name', 'value': document_definition['name']}], enable_scan_in_query=True)]
        assert document_list is not None
        ...
        await self.__assert_http_failure_with_status(StatusCodes.PRECONDITION_FAILED, created_collection.replace_item, replaced_document['id'], replaced_document, if_match=old_etag)
        try:
            await created_collection.replace_item(etag=replaced_document['_etag'], item=replaced_document['id'], body=replaced_document)
            pytest.fail('should fail if only etag specified')
        except ValueError:
            pass
        try:
            await created_collection.replace_item(match_condition=MatchConditions.IfNotModified, item=replaced_document['id'], body=replaced_document)
            pytest.fail('should fail if only match condition specified')
        except ValueError:
            pass
        try:
            await created_collection.replace_item(match_condition=MatchConditions.IfModified, item=replaced_document['id'], body=replaced_document)
            pytest.fail('should fail if only match condition specified')
        except ValueError:
            pass
        try:
            await created_collection.replace_item(match_condition=replaced_document['_etag'], item=replaced_document['id'], body=replaced_document)
            pytest.fail('should fail if invalid match condition specified')
        except TypeError:
            pass
        replaced_document_conditional = await created_collection.replace_item(match_condition=MatchConditions.IfNotModified, etag=replaced_document['_etag'], item=replaced_document['id'], body=replaced_document)
        ...
        one_document_from_read = await created_collection.read_item(item=replaced_document['id'], partition_key=replaced_document['id'])
        assert replaced_document['id'] == one_document_from_read['id']
        await created_collection.delete_item(item=replaced_document, partition_key=replaced_document['id'])
        await self.__assert_http_failure_with_status(StatusCodes.NOT_FOUND, created_collection.read_item, replaced_document['id'], replaced_document['id'])
        await self._clear()
    ...
    async def test_document_upsert_async(self):
        ...
        document_list = [document async for document in created_collection.read_all_items()]
        before_create_documents_count = len(document_list)
        document_definition = {'id': 'doc', 'name': 'sample document', 'spam': 'eggs', 'key': 'value'}
        ...
        with pytest.raises(exceptions.CosmosHttpResponseError):
            await created_collection.upsert_item(body=created_document, match_condition=MatchConditions.IfNotModified, etag=new_document['_etag'])
    ...
    async def _test_spatial_index(self):
        ...
        collection = await db.create_container(id='collection with spatial index ' + str(uuid.uuid4()), indexing_policy={'includedPaths': [{'path': '/"Location"/?', 'indexes': [{'kind': 'Spatial', 'dataType': 'Point'}]}, {'path': '/'}]}, partition_key=PartitionKey(path='/id', kind='Hash'))
        ...
        results = [result async for result in collection.query_items(query="SELECT * FROM root WHERE (ST_DISTANCE(root.Location, {type: 'Point', coordinates: [20.1, 20]}) < 20000)")]
    ...
    async def test_user_crud_async(self):
        ...
        results = [user async for user in db.query_users(query='SELECT * FROM root r WHERE r.id=@id', parameters=[{'name': '@id', 'value': user_id}])]
    ...
    async def test_permission_crud_async(self):
        ...
        permission = {'id': 'new permission', 'permissionMode': documents.PermissionMode.Read, 'resource': 'dbs/AQAAAA==/colls/AQAAAJ0fgTc='}
        ...
        results = [permission async for permission in user.query_permissions(query='SELECT * FROM root r WHERE r.id=@id', parameters=[{'name': '@id', 'value': permission.id}])]
        assert results is not None
    ...
    async def test_permission_upsert_async(self):
        ...
        permissions = [permission async for permission in user.list_permissions()]
        before_create_count = len(permissions)
        permission_definition = {'id': 'permission', 'permissionMode': documents.PermissionMode.Read, 'resource': 'dbs/AQAAAA==/colls/AQAAAJ0fgTc='}
    ...
    async def test_authorization_async(self):
        ...
        async def __setup_entities():
            ...
            """
            db = self.database_for_test
            collection = await db.create_container(id='test_authorization' + str(uuid.uuid4()), partition_key=PartitionKey(path='/id', kind='Hash'))
            document = await collection.create_item(body={'id': 'doc1', 'spam': 'eggs', 'key': 'value'})
            user = await db.create_user(body={'id': 'user' + str(uuid.uuid4())})
            permission = {'id': 'permission On Coll', 'permissionMode': documents.PermissionMode.Read, 'resource': 'dbs/' + db.id + '/colls/' + collection.id}
            permission_on_coll = await user.create_permission(body=permission)
            assert permission_on_coll.properties['_token'] is not None
            permission = {'id': 'permission On Doc', 'permissionMode': documents.PermissionMode.All, 'resource': 'dbs/' + db.id + '/colls/' + collection.id + '/docs/' + document['id']}
            permission_on_doc = await user.create_permission(body=permission)
            assert permission_on_doc.properties['_token'] is not None
            entities = {'db': db, 'coll': collection, 'doc': document, 'user': user, 'permissionOnColl': permission_on_coll, 'permissionOnDoc': permission_on_doc}
            return entities
        try:
        ...
        async with CosmosClient(TestCRUDAsync.host, TestCRUDAsync.masterKey) as client:
            entities = await __setup_entities()
            resource_tokens = {'dbs/' + entities['db'].id + '/colls/' + entities['coll'].id: entities['permissionOnColl'].properties['_token']}
        async with CosmosClient(TestCRUDAsync.host, resource_tokens) as col_client:
            db = entities['db']
            ...
            await self.__assert_http_failure_with_status(StatusCodes.FORBIDDEN, success_coll.delete_item, doc_id, doc_id)
            resource_tokens = {'dbs/' + entities['db'].id + '/colls/' + entities['coll'].id + '/docs/' + doc_id: entities['permissionOnDoc'].properties['_token']}
        async with CosmosClient(TestCRUDAsync.host, resource_tokens) as doc_client:
            read_doc = await doc_client.get_database_client(db.id).get_container_client(success_coll.id).read_item(doc_id, doc_id)
            assert read_doc['id'] == doc_id
            await doc_client.get_database_client(db.id).get_container_client(success_coll.id).delete_item(doc_id, doc_id)
            assert read_doc['id'] == doc_id
    ...
    async def test_trigger_crud_async(self):
        ...
        trigger_definition = {'id': 'sample trigger', 'serverScript': 'function() {var x = 10;}', 'triggerType': documents.TriggerType.Pre, 'triggerOperation': documents.TriggerOperation.All}
        ...
        triggers = [trigger async for trigger in collection.scripts.query_triggers(query='SELECT * FROM root r WHERE r.id=@id', parameters=[{'name': '@id', 'value': trigger_definition['id']}])]
        ...
        await self.__assert_http_failure_with_status(StatusCodes.NOT_FOUND, collection.scripts.delete_trigger, replaced_trigger['id'])
        await self._clear()
    ...
    async def test_udf_crud_async(self):
        ...
        results = [udf async for udf in collection.scripts.query_user_defined_functions(query='SELECT * FROM root r WHERE r.id=@id', parameters=[{'name': '@id', 'value': udf_definition['id']}])]
        assert results is not None
        ...
        await self.__assert_http_failure_with_status(StatusCodes.NOT_FOUND, collection.scripts.get_user_defined_function, replaced_udf['id'])
        await self._clear()
    ...
    async def test_sproc_crud_async(self):
        ...
        sprocs = [sproc async for sproc in collection.scripts.query_stored_procedures(query='SELECT * FROM root r WHERE r.id=@id', parameters=[{'name': '@id', 'value': sproc_definition['id']}])]
        ...
        await self.__assert_http_failure_with_status(StatusCodes.NOT_FOUND, collection.scripts.get_stored_procedure, replaced_sproc['id'])
        await self._clear()
    ...
    async def test_script_logging_execute_stored_procedure_async(self):
        await self._set_up()
        created_collection = await self.database_for_test.create_container(str(uuid.uuid4()), PartitionKey(path='/id'))
        sproc = {'id': 'storedProcedure' + str(uuid.uuid4()), 'body': 'function () {' + "   var mytext = 'x';" + '   var myval = 1;' + '   try {' + "       console.log('The value of %s is %s.', mytext, myval);" + "       getContext().getResponse().setBody('Success!');" + '   }' + '   catch (err) {' + "       getContext().getResponse().setBody('inline err: [' + err.number + '] ' + err);" + '   }}'}
        created_sproc = await created_collection.scripts.create_stored_procedure(body=sproc)
        ...
        result = await created_collection.scripts.execute_stored_procedure(sproc=created_sproc['id'], enable_script_logging=True, partition_key=1)
        assert result == 'Success!'
        assert urllib.quote('The value of x is 1.') == created_collection.scripts.client_connection.last_response_headers.get(HttpHeaders.ScriptLogResults)
        result = await created_collection.scripts.execute_stored_procedure(sproc=created_sproc['id'], enable_script_logging=False, partition_key=1)
        assert result == 'Success!'
        assert HttpHeaders.ScriptLogResults not in created_collection.scripts.client_connection.last_response_headers
        await self._clear()
    ...
    async def test_collection_indexing_policy_async(self):
        await self._set_up()
        db = self.database_for_test
        collection = await db.create_container(id='test_collection_indexing_policy default policy' + str(uuid.uuid4()), partition_key=PartitionKey(path='/id', kind='Hash'))
        ...
        consistent_collection = await db.create_container(id='test_collection_indexing_policy consistent collection ' + str(uuid.uuid4()), indexing_policy={'indexingMode': documents.IndexingMode.Consistent}, partition_key=PartitionKey(path='/id', kind='Hash'))
        ...
        collection_with_indexing_policy = await db.create_container(id='CollectionWithIndexingPolicy ' + str(uuid.uuid4()), indexing_policy={'automatic': True, 'indexingMode': documents.IndexingMode.Consistent, 'includedPaths': [{'path': '/', 'indexes': [{'kind': documents.IndexKind.Hash, 'dataType': documents.DataType.Number, 'precision': 2}]}], 'excludedPaths': [{'path': '/"systemMetadata"/*'}]}, partition_key=PartitionKey(path='/id', kind='Hash'))
    ...
    async def test_create_default_indexing_policy_async(self):
        await self._set_up()
        db = self.database_for_test
        collection = await db.create_container(id='test_create_default_indexing_policy TestCreateDefaultPolicy01' + str(uuid.uuid4()), partition_key=PartitionKey(path='/id', kind='Hash'))
        collection_properties = await collection.read()
        await self._check_default_indexing_policy_paths(collection_properties['indexingPolicy'])
        await db.delete_container(container=collection)
        collection = await db.create_container(id='test_create_default_indexing_policy TestCreateDefaultPolicy01' + str(uuid.uuid4()), indexing_policy={'indexingMode': documents.IndexingMode.Consistent, 'automatic': True}, partition_key=PartitionKey(path='/id', kind='Hash'))
        collection_properties = await collection.read()
        await self._check_default_indexing_policy_paths(collection_properties['indexingPolicy'])
        await db.delete_container(container=collection)
        collection = await db.create_container(id='test_create_default_indexing_policy TestCreateDefaultPolicy03' + str(uuid.uuid4()), indexing_policy={}, partition_key=PartitionKey(path='/id', kind='Hash'))
        collection_properties = await collection.read()
        await self._check_default_indexing_policy_paths(collection_properties['indexingPolicy'])
        await db.delete_container(container=collection)
        collection = await db.create_container(id='test_create_default_indexing_policy TestCreateDefaultPolicy04' + str(uuid.uuid4()), indexing_policy={'includedPaths': [{'path': '/*'}]}, partition_key=PartitionKey(path='/id', kind='Hash'))
        collection_properties = await collection.read()
        await self._check_default_indexing_policy_paths(collection_properties['indexingPolicy'])
        await db.delete_container(container=collection)
        collection = await db.create_container(id='test_create_default_indexing_policy TestCreateDefaultPolicy05' + str(uuid.uuid4()), indexing_policy={'includedPaths': [{'path': '/*', 'indexes': [{'kind': documents.IndexKind.Hash, 'dataType': documents.DataType.String}, {'kind': documents.IndexKind.Range, 'dataType': documents.DataType.Number}]}]}, partition_key=PartitionKey(path='/id', kind='Hash'))
        collection_properties = await collection.read()
        await self._check_default_indexing_policy_paths(collection_properties['indexingPolicy'])
        await self._clear()
    ...
    async def test_create_indexing_policy_with_composite_and_spatial_indexes_async(self):
        await self._set_up()
        db = self.database_for_test
        indexing_policy = {'spatialIndexes': [{'path': '/path0/*', 'types': ['Point', 'LineString', 'Polygon', 'MultiPolygon']}, {'path': '/path1/*', 'types': ['Point', 'LineString', 'Polygon', 'MultiPolygon']}], 'compositeIndexes': [[{'path': '/path1', 'order': 'ascending'}, {'path': '/path2', 'order': 'descending'}, {'path': '/path3', 'order': 'ascending'}], [{'path': '/path4', 'order': 'ascending'}, {'path': '/path5', 'order': 'descending'}, {'path': '/path6', 'order': 'ascending'}]]}
        custom_logger = logging.getLogger('CustomLogger')
        created_container = await db.create_container(id='composite_index_spatial_index' + str(uuid.uuid4()), indexing_policy=indexing_policy, partition_key=PartitionKey(path='/id', kind='Hash'), headers={'Foo': 'bar'}, user_agent='blah', user_agent_overwrite=True, logging_enable=True, logger=custom_logger)
    ...
    async def _check_default_indexing_policy_paths(self, indexing_policy):
        ...
        root_included_path = __get_first([included_path for included_path in indexing_policy['includedPaths'] if included_path['path'] == '/*'])
    ...
    async def test_client_request_timeout_async(self):
        if not ('localhost' in self.host or '127.0.0.1' in self.host):
            ...
            connection_policy.RequestTimeout = 1e-12
            with pytest.raises(Exception):
                async with CosmosClient(TestCRUDAsync.host, TestCRUDAsync.masterKey, connection_policy=connection_policy) as client:
                    print('Async initialization')
    ...
    async def test_client_request_timeout_when_connection_retry_configuration_specified_async(self):
        ...
        with pytest.raises(AzureError):
            async with CosmosClient(TestCRUDAsync.host, TestCRUDAsync.masterKey, connection_policy=connection_policy, retry_total=3, retry_connect=3, retry_read=3, retry_backoff_max=0.3, retry_on_status_codes=[500, 502, 504]) as client:
                print('Async Initialization')
    ...
    async def initialize_client_with_connection_urllib_retry_config(self, retries):
        ...
        try:
            async with CosmosClient('https://localhost:9999', TestCRUDAsync.masterKey, retry_total=retries, retry_connect=retries, retry_read=retries, retry_backoff_max=0.3, retry_on_status_codes=[500, 502, 504]) as client:
                print('Async initialization')
            pytest.fail()
    ...
    async def initialize_client_with_connection_core_retry_config(self, retries):
        start_time = time.time()
        try:
            async with CosmosClient('https://localhost:9999', TestCRUDAsync.masterKey, retry_total=retries, retry_read=retries, retry_connect=retries, retry_status=retries) as client:
                print('Async initialization')
            pytest.fail()
        except AzureError as e:
            end_time = time.time()
    ...
    async def test_absolute_client_timeout_async(self):
        with pytest.raises(exceptions.CosmosClientTimeoutError):
            async with CosmosClient('https://localhost:9999', TestCRUDAsync.masterKey, retry_total=3, timeout=1) as client:
                print('Async initialization')
        error_response = ServiceResponseError('Read timeout')
        timeout_transport = TimeoutTransport(error_response)
        async with CosmosClient(self.host, self.masterKey, transport=timeout_transport, passthrough=True) as client:
            print('Async initialization')
            with pytest.raises(exceptions.CosmosClientTimeoutError):
    ...
    async def test_query_iterable_functionality_async(self):
        ...
        async def __create_resources():
            ...
            """
            collection = await self.database_for_test.create_container_if_not_exists(str(uuid.uuid4()), PartitionKey(path='/pk'))
    ...
    async def test_trigger_functionality_async(self):
        await self._set_up()
        triggers_in_collection1 = [{'id': 't1', 'body': 'function() {' + '    var item = getContext().getRequest().getBody();' + "    item.id = item.id.toUpperCase() + 't1';" + '    getContext().getRequest().setBody(item);' + '}', 'triggerType': documents.TriggerType.Pre, 'triggerOperation': documents.TriggerOperation.All}, {'id': 'response1', 'body': 'function() {' + '    var prebody = getContext().getRequest().getBody();' + "    if (prebody.id != 'TESTING POST TRIGGERt1')        throw 'id mismatch';" + '    var postbody = getContext().getResponse().getBody();' + "    if (postbody.id != 'TESTING POST TRIGGERt1')        throw 'id mismatch';}", 'triggerType': documents.TriggerType.Post, 'triggerOperation': documents.TriggerOperation.All}, {'id': 'response2', 'body': 'function() {' + '    var predoc = getContext().getRequest().getBody();' + '    var postdoc = getContext().getResponse().getBody();' + '    getContext().getResponse().setValue(' + "        'predocname', predoc.id + 'response2');" + '    getContext().getResponse().setValue(' + "        'postdocname', postdoc.id + 'response2');" + '}', 'triggerType': documents.TriggerType.Post, 'triggerOperation': documents.TriggerOperation.All}]
        triggers_in_collection2 = [{'id': 't2', 'body': 'function() { }', 'triggerType': documents.TriggerType.Pre, 'triggerOperation': documents.TriggerOperation.All}, {'id': 't3', 'body': 'function() {' + '    var item = getContext().getRequest().getBody();' + "    item.id = item.id.toLowerCase() + 't3';" + '    getContext().getRequest().setBody(item);' + '}', 'triggerType': documents.TriggerType.Pre, 'triggerOperation': documents.TriggerOperation.All}]
        triggers_in_collection3 = [{'id': 'triggerOpType', 'body': 'function() { }', 'triggerType': documents.TriggerType.Post, 'triggerOperation': documents.TriggerOperation.Delete}]
        ...
        db = self.database_for_test
        collection1 = await db.create_container(id='test_trigger_functionality 1 ' + str(uuid.uuid4()), partition_key=PartitionKey(path='/key', kind='Hash'))
        collection2 = await db.create_container(id='test_trigger_functionality 2 ' + str(uuid.uuid4()), partition_key=PartitionKey(path='/key', kind='Hash'))
        collection3 = await db.create_container(id='test_trigger_functionality 3 ' + str(uuid.uuid4()), partition_key=PartitionKey(path='/key', kind='Hash'))
        ...
        document_1_1 = await collection1.create_item(body={'id': 'doc1', 'key': 'value'}, pre_trigger_include='t1')
        assert document_1_1['id'] == 'DOC1t1'
        document_1_2 = await collection1.create_item(body={'id': 'testing post trigger', 'key': 'value'}, pre_trigger_include='t1', post_trigger_include='response1')
        assert document_1_2['id'] == 'TESTING POST TRIGGERt1'
        document_1_3 = await collection1.create_item(body={'id': 'responseheaders', 'key': 'value'}, pre_trigger_include='t1')
        ...
        document_2_1 = await collection2.create_item(body={'id': 'doc2', 'key': 'value2'}, pre_trigger_include='t2')
        assert document_2_1['id'] == 'doc2'
        document_2_2 = await collection2.create_item(body={'id': 'Doc3', 'prop': 'empty', 'key': 'value2'}, pre_trigger_include='t3')
        ...
        with pytest.raises(Exception):
            await collection3.create_item(body={'id': 'Docoptype', 'key': 'value2'}, post_trigger_include='triggerOpType')
        await self._clear()
    ...
    async def test_stored_procedure_functionality_async(self):
        await self._set_up()
        collection = await self.database_for_test.create_container(str(uuid.uuid4()), PartitionKey(path='/id'))
        sproc1 = {'id': 'storedProcedure1' + str(uuid.uuid4()), 'body': 'function () {' + '  for (var i = 0; i < 1000; i++) {' + '    var item = getContext().getResponse().getBody();' + "    if (i > 0 && item != i - 1) throw 'body mismatch';" + '    getContext().getResponse().setBody(i);' + '  }' + '}'}
        ...
        assert result == 999
        sproc2 = {'id': 'storedProcedure2' + str(uuid.uuid4()), 'body': 'function () {' + '  for (var i = 0; i < 10; i++) {' + "    getContext().getResponse().appendValue('Body', i);" + '  }' + '}'}
        ...
        sproc3 = {'id': 'storedProcedure3' + str(uuid.uuid4()), 'body': 'function (input) {' + '  getContext().getResponse().setBody(' + "      'a' + input.temp);" + '}'}
        retrieved_sproc3 = await collection.scripts.create_stored_procedure(body=sproc3)
        result = await collection.scripts.execute_stored_procedure(sproc=retrieved_sproc3['id'], parameters={'temp': 'so'}, partition_key=1)
        assert result == 'aso'
        await self._clear()
    ...
    async def test_offer_read_and_query_async(self):
        await self._set_up()
        db = self.database_for_test
        collection = await db.create_container(id='test_offer_read_and_query ' + str(uuid.uuid4()), partition_key=PartitionKey(path='/id', kind='Hash'))
    ...
    async def test_index_progress_headers_async(self):
        await self._set_up()
        created_db = self.database_for_test
        consistent_coll = await created_db.create_container(id='test_index_progress_headers consistent_coll ' + str(uuid.uuid4()), partition_key=PartitionKey(path='/id', kind='Hash'))
        ...
        none_coll = await created_db.create_container(id='test_index_progress_headers none_coll ' + str(uuid.uuid4()), indexing_policy={'indexingMode': documents.IndexingMode.NoIndex, 'automatic': False}, partition_key=PartitionKey(path='/id', kind='Hash'))
    ...
    async def test_get_resource_with_dictionary_and_object_async(self):
        ...
        assert read_item['id'], created_item['id']
        created_sproc = await created_container.scripts.create_stored_procedure({'id': 'storedProcedure' + str(uuid.uuid4()), 'body': 'function () { }'})
        ...
        assert read_sproc['id'] == created_sproc['id']
        created_trigger = await created_container.scripts.create_trigger({'id': 'sample trigger' + str(uuid.uuid4()), 'serverScript': 'function() {var x = 10;}', 'triggerType': documents.TriggerType.Pre, 'triggerOperation': documents.TriggerOperation.All})
        ...
        assert read_trigger['id'] == created_trigger['id']
        created_udf = await created_container.scripts.create_user_defined_function({'id': 'sample udf' + str(uuid.uuid4()), 'body': 'function() {var x = 10;}'})
        ...
        assert read_user.id == created_user.id
        created_permission = await created_user.create_permission({'id': 'all permission' + str(uuid.uuid4()), 'permissionMode': documents.PermissionMode.All, 'resource': created_container.container_link, 'resourcePartitionKey': [1]})
    ...
    async def test_patch_operations_async(self):
        await self._set_up()
        created_container = await self.database_for_test.create_container_if_not_exists(id='patch_container', partition_key=PartitionKey(path='/pk'))
        item_id = 'patch_item_' + str(uuid.uuid4())
        item = {'id': item_id, 'pk': 'patch_item_pk', 'prop': 'prop1', 'address': {'city': 'Redmond'}, 'company': 'Microsoft', 'number': 3}
        await created_container.create_item(item)
        operations = [{'op': 'add', 'path': '/color', 'value': 'yellow'}, {'op': 'remove', 'path': '/prop'}, {'op': 'replace', 'path': '/company', 'value': 'CosmosDB'}, {'op': 'set', 'path': '/address/new_city', 'value': 'Atlanta'}, {'op': 'incr', 'path': '/number', 'value': 7}, {'op': 'move', 'from': '/color', 'path': '/favorite_color'}]
        patched_item = await created_container.patch_item(item=item_id, partition_key='patch_item_pk', patch_operations=operations)
    ...
    async def test_conditional_patching_async(self):
        await self._set_up()
        created_container = await self.database_for_test.create_container_if_not_exists(id='patch_filter_container', partition_key=PartitionKey(path='/pk'))
        ...
        try:
            await created_container.patch_item(item=item_id, partition_key='patch_item_pk', patch_operations=operations, filter_predicate=filter_predicate)
        except exceptions.CosmosHttpResponseError as e:
            assert e.status_code == StatusCodes.PRECONDITION_FAILED
        filter_predicate = 'from root where root.number = ' + str(item.get('number'))
        patched_item = await created_container.patch_item(item=item_id, partition_key='patch_item_pk', patch_operations=operations, filter_predicate=filter_predicate)
        assert patched_item.get('color') is None
        assert patched_item.get('prop') is None
        assert patched_item.get('company') == 'CosmosDB'
        assert patched_item.get('address').get('new_city') == 'Atlanta'
    ...
    async def test_priority_level_async(self):
        await self._set_up()
        created_container = await self.database_for_test.create_container_if_not_exists(id='priority_level_container_async', partition_key=PartitionKey(path='/pk'))
        ...
        async def priority_mock_execute_function(function, *args, **kwargs):
            if args:
                priority_level_headers.append(args[4].headers[HttpHeaders.PriorityLevel] if HttpHeaders.PriorityLevel in args[4].headers else '')
        ...
        assert priority_level_headers[-1] == 'Low'
        query = [doc async for doc in created_container.query_items('Select * from c', partition_key='pk1', priority_level='High')]
        assert priority_level_headers[-1] == 'High'
