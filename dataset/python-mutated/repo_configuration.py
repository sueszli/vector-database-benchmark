...
REDIS_CLUSTER_CONFIG = {'type': 'redis', 'redis_type': 'redis_cluster', 'connection_string': '127.0.0.1:6001,127.0.0.1:6002,127.0.0.1:6003'}
SNOWFLAKE_CONFIG = {'type': 'snowflake.online', 'account': os.getenv('SNOWFLAKE_CI_DEPLOYMENT', ''), 'user': os.getenv('SNOWFLAKE_CI_USER', ''), 'password': os.getenv('SNOWFLAKE_CI_PASSWORD', ''), 'role': os.getenv('SNOWFLAKE_CI_ROLE', ''), 'warehouse': os.getenv('SNOWFLAKE_CI_WAREHOUSE', ''), 'database': 'FEAST', 'schema': 'ONLINE'}
BIGTABLE_CONFIG = {'type': 'bigtable', 'project_id': os.getenv('GCLOUD_PROJECT', 'kf-feast'), 'instance': os.getenv('BIGTABLE_INSTANCE_ID', 'feast-integration-tests')}
ROCKSET_CONFIG = {'type': 'rockset', 'api_key': os.getenv('ROCKSET_APIKEY', ''), 'host': os.getenv('ROCKSET_APISERVER', 'api.rs2.usw2.rockset.com')}
OFFLINE_STORE_TO_PROVIDER_CONFIG: Dict[str, DataSourceCreator] = {'file': ('local', FileDataSourceCreator), 'bigquery': ('gcp', BigQueryDataSourceCreator), 'redshift': ('aws', RedshiftDataSourceCreator), 'snowflake': ('aws', SnowflakeDataSourceCreator)}
AVAILABLE_OFFLINE_STORES: List[Tuple[str, Type[DataSourceCreator]]] = [('local', FileDataSourceCreator)]
AVAILABLE_ONLINE_STORES: Dict[str, Tuple[Union[str, Dict[str, str]], Optional[Type[OnlineStoreCreator]]]] = {'sqlite': ({'type': 'sqlite'}, None)}
if os.getenv('FEAST_IS_LOCAL_TEST', 'False') != 'True':
    AVAILABLE_OFFLINE_STORES.extend([('gcp', BigQueryDataSourceCreator), ('aws', RedshiftDataSourceCreator), ('aws', SnowflakeDataSourceCreator)])
...
if full_repo_configs_module is not None:
    try:
        ...
        raise FeastModuleImportError(full_repo_configs_module, 'FULL_REPO_CONFIGS') from e
    try:
        ...
        try:
            ...
            raise FeastModuleImportError(full_repo_configs_module, 'FULL_REPO_CONFIGS') from e
        AVAILABLE_OFFLINE_STORES = [(config.provider, config.offline_store_creator) for config in FULL_REPO_CONFIGS]
        AVAILABLE_OFFLINE_STORES = list(set(AVAILABLE_OFFLINE_STORES))
        AVAILABLE_ONLINE_STORES = {c.online_store['type'] if isinstance(c.online_store, dict) else c.online_store: (c.online_store, c.online_store_creator) for c in FULL_REPO_CONFIGS}
if os.getenv('FEAST_LOCAL_ONLINE_CONTAINER', 'False').lower() == 'true':
    replacements: Dict[str, Tuple[Union[str, Dict[str, str]], Optional[Type[OnlineStoreCreator]]]] = {'redis': (REDIS_CONFIG, RedisOnlineStoreCreator), 'dynamodb': (DYNAMO_CONFIG, DynamoDBOnlineStoreCreator), 'datastore': ('datastore', DatastoreOnlineStoreCreator), 'bigtable': ('bigtable', BigtableOnlineStoreCreator)}
