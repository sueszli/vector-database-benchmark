...
class HelpSheet(MetaSheet):
    ...
    columns = [ColumnAttr('sheet'), ColumnAttr('module'), ColumnAttr('longname'), Column('keystrokes', getter=lambda col, row: col.sheet.revbinds.get(row.longname, [None])[0]), Column('all_bindings', width=0, getter=lambda col, row: list(set(col.sheet.revbinds.get(row.longname, [])))), Column('description', width=40, getter=lambda col, row: col.sheet.cmddict[row.sheet, row.longname].helpstr), ColumnAttr('execstr', width=0), Column('logged', width=0, getter=lambda col, row: vd.isLoggableCommand(row.longname))]
    nKeys = 2
