import unittest
import numpy as np
from op_test import OpTest, convert_float_to_uint16, get_numeric_gradient, paddle_static_guard
...
def create_test_cudnn_bf16_class(parent):
    ...
    class TestConv3DCUDNNBF16(parent):
        ...
        def test_check_output(self):
            ...
            self.check_output_with_place(place, check_dygraph=not self.use_mkldnn, check_pir=True)
        ...
        def test_check_grad_no_filter(self):
            ...
            self.check_grad_with_place(place, ['Input'], 'Output', no_grad_set={'Filter'}, check_dygraph=not self.use_mkldnn, user_defined_grads=[numeric_grads], check_pir=True)
        ...
        def test_check_grad_no_input(self):
            ...
            self.check_grad_with_place(place, ['Filter'], 'Output', no_grad_set={'Input'}, check_dygraph=not self.use_mkldnn, user_defined_grads=[numeric_grads], check_pir=True)
        ...
        def test_check_grad(self):
            ...
            self.check_grad_with_place(place, ['Input', 'Filter'], 'Output', user_defined_grads=[numeric_input_grads, numeric_fliter_grads], check_dygraph=not self.use_mkldnn, check_pir=True)
...
def conv3d_wrapper(x, weight, stride=1, padding=0, padding_algorithm='EXPLICIT', groups=1, dilation=1, data_format='NCDHW'):
    if False:
        while True:
            i = 10
    ...
    return paddle._C_ops.conv3d(x, weight, stride, padding, padding_algorithm, groups, dilation, data_format)
    ...
    def test_check_grad(self):
        if False:
            while True:
                i = 10
        place = core.CUDAPlace(0) if self.has_cudnn() else core.CPUPlace()
        self.check_grad_with_place(place, {'Input', 'Filter'}, 'Output', max_relative_error=0.03, check_dygraph=not self.use_mkldnn, check_pir=True)
    ...
    def test_check_grad_no_filter(self):
        if False:
            for i in range(10):
                print('nop')
        place = core.CUDAPlace(0) if self.has_cudnn() else core.CPUPlace()
        self.check_grad_with_place(place, ['Input'], 'Output', max_relative_error=0.03, no_grad_set={'Filter'}, check_dygraph=not self.use_mkldnn, check_pir=True)
    ...
    def test_check_grad_no_input(self):
        if False:
            for i in range(10):
                print('nop')
        place = core.CUDAPlace(0) if self.has_cudnn() else core.CPUPlace()
        self.check_grad_with_place(place, ['Filter'], 'Output', max_relative_error=0.03, no_grad_set={'Input'}, check_dygraph=not self.use_mkldnn, check_pir=True)
    ...
    def test_check_output(self):
        ...
        if core.is_compiled_with_cuda():
            ...
            if core.is_float16_supported(place):
                self.check_output_with_place(place, atol=0.02, check_pir=True)
...
class TestConv3DOp_2(OpTest):
    ...
    def test_check_output(self):
        if False:
            return 10
        place = core.CUDAPlace(0) if self.has_cudnn() else core.CPUPlace()
        self.check_output_with_place(place, atol=1e-05, check_pir=True)
    ...
    def test_check_grad(self):
        ...
        if self.dtype == np.float16:
            return
        place = core.CUDAPlace(0) if self.has_cudnn() else core.CPUPlace()
        self.check_grad_with_place(place, {'Input', 'Filter'}, 'Output', max_relative_error=0.03, check_pir=True)
    ...
    def test_check_grad_no_filter(self):
        ...
        if self.dtype == np.float16:
            return
        place = core.CUDAPlace(0) if self.has_cudnn() else core.CPUPlace()
        self.check_grad_with_place(place, ['Input'], 'Output', max_relative_error=0.03, no_grad_set={'Filter'}, check_pir=True)
    ...
    def test_check_grad_no_input(self):
        ...
        if self.dtype == np.float16:
            return
        place = core.CUDAPlace(0) if self.has_cudnn() else core.CPUPlace()
        self.check_grad_with_place(place, ['Filter'], 'Output', max_relative_error=0.03, no_grad_set={'Input'}, check_pir=True)
