...
from tribler.core.components.restapi.rest.rest_endpoint import HTTP_BAD_REQUEST, HTTP_INTERNAL_SERVER_ERROR, HTTP_NOT_FOUND, RESTEndpoint, RESTResponse, RESTStreamResponse
...
class DownloadsEndpoint(RESTEndpoint):
    ...
    async def get_downloads(self, request):
        ...
        checkpoints = {TOTAL: self.download_manager.checkpoints_count, LOADED: self.download_manager.checkpoints_loaded, ALL_LOADED: self.download_manager.all_checkpoints_are_loaded}
        ...
        for download in downloads:
            ...
            if download.config.get_channel_download():
                download_name = self.mds.ChannelMetadata.get_channel_name_cached(tdef.get_name_utf8(), tdef.get_infohash())
            elif self.mds is None:
                download_name = tdef.get_name_utf8()
            else:
                download_name = self.mds.TorrentMetadata.get_torrent_title(tdef.get_infohash()) or tdef.get_name_utf8()
    ...
    @docs(tags=['Libtorrent'], summary='Start a download from a provided URI.', parameters=[{'in': 'query', 'name': 'get_peers', 'description': 'Flag indicating whether or not to include peers', 'type': 'boolean', 'required': False}, {'in': 'query', 'name': 'get_pieces', 'description': 'Flag indicating whether or not to include pieces', 'type': 'boolean', 'required': False}, {'in': 'query', 'name': 'get_files', 'description': 'Flag indicating whether or not to include files', 'type': 'boolean', 'required': False}], responses={200: {'schema': schema(AddDownloadResponse={'started': Boolean, 'infohash': String}), 'examples': {'started': True, 'infohash': '4344503b7e797ebf31582327a5baae35b11bda01'}}})
    @json_schema(schema(AddDownloadRequest={'anon_hops': (Integer, 'Number of hops for the anonymous download. No hops is equivalent to a plain download'), 'safe_seeding': (Boolean, 'Whether the seeding of the download should be anonymous or not'), 'destination': (String, 'the download destination path of the torrent'), 'uri*': (String, 'The URI of the torrent file that should be downloaded. This URI can either represent a file location, a magnet link or a HTTP(S) url.')}))
    ...
    @docs(tags=['Libtorrent'], summary='Remove a specific download.', parameters=[{'in': 'path', 'name': 'infohash', 'description': 'Infohash of the download to remove', 'type': 'string', 'required': True}], responses={200: {'schema': schema(DeleteDownloadResponse={'removed': Boolean, 'infohash': String}), 'examples': {'removed': True, 'infohash': '4344503b7e797ebf31582327a5baae35b11bda01'}}})
    @json_schema(schema(RemoveDownloadRequest={'remove_data': (Boolean, 'Whether or not to remove the associated data')}))
    ...
    async def vod_response(self, download, parameters, request, vod_mode):
        ...
        if vod_mode:
            ...
            if file_index is None:
                return RESTResponse({'error': 'fileindex is necessary to enable vod_mode'}, status=HTTP_BAD_REQUEST)
            ...
            modified = True
        return RESTResponse({'vod_prebuffering_progress': download.stream.prebuffprogress, 'vod_prebuffering_progress_consec': download.stream.prebuffprogress_consec, 'vod_header_progress': download.stream.headerprogress, 'vod_footer_progress': download.stream.footerprogress, 'vod_mode': download.stream.enabled, 'infohash': hexlify(download.get_def().get_infohash()), 'modified': modified})

    @docs(tags=['Libtorrent'], summary='Update a specific download.', parameters=[{'in': 'path', 'name': 'infohash', 'description': 'Infohash of the download to update', 'type': 'string', 'required': True}], responses={200: {'schema': schema(UpdateDownloadResponse={'modified': Boolean, 'infohash': String}), 'examples': {'modified': True, 'infohash': '4344503b7e797ebf31582327a5baae35b11bda01'}}})
    @json_schema(schema(UpdateDownloadRequest={'state': (String, 'State parameter to be passed to modify the state of the download (resume/stop/recheck)'), 'selected_files': (List(Integer), 'File indexes to be included in the download'), 'anon_hops': (Integer, 'The anonymity of a download can be changed at runtime by passing the anon_hops parameter, however, this must be the only parameter in this request.')}))
    async def update_download(self, request):
        ...
        if vod_mode is not None:
            if not isinstance(vod_mode, bool):
                return RESTResponse({'error': 'vod_mode must be bool flag'}, status=HTTP_BAD_REQUEST)
            return await self.vod_response(download, parameters, request, vod_mode)
        if len(parameters) > 1 and 'anon_hops' in parameters:
            return RESTResponse({'error': 'anon_hops must be the only parameter in this request'}, status=HTTP_BAD_REQUEST)
    ...
    @docs(tags=['Libtorrent'], summary='Return the .torrent file associated with the specified download.', parameters=[{'in': 'path', 'name': 'infohash', 'description': 'Infohash of the download from which to get the .torrent file', 'type': 'string', 'required': True}], responses={200: {'description': 'The torrent'}})
    async def get_torrent(self, request):
        ...
        if not torrent:
            return DownloadsEndpoint.return_404(request)
        return RESTResponse(lt.bencode(torrent), headers={'content-type': 'application/x-bittorrent', 'Content-Disposition': 'attachment; filename=%s.torrent' % hexlify(infohash).encode('utf-8')})

    @docs(tags=['Libtorrent'], summary='Return file information of a specific download.', parameters=[{'in': 'path', 'name': 'infohash', 'description': 'Infohash of the download to from which to get file information', 'type': 'string', 'required': True}], responses={200: {'schema': schema(GetFilesResponse={'files': [schema(File={'index': Integer, 'name': String, 'size': Integer, 'included': Boolean, 'progress': Float})]})}})
    ...
    @docs(tags=['Libtorrent'], summary='Stream the contents of a file that is being downloaded.', parameters=[{'in': 'path', 'name': 'infohash', 'description': 'Infohash of the download to stream', 'type': 'string', 'required': True}, {'in': 'path', 'name': 'fileindex', 'description': 'The fileindex to stream', 'type': 'string', 'required': True}], responses={206: {'description': 'Contents of the stream'}})
    async def stream(self, request):
        ...
        response = RESTStreamResponse(status=206, reason='OK', headers={'Accept-Ranges': 'bytes', 'Content-Type': 'application/octet-stream', 'Content-Length': f'{stop - start}', 'Content-Range': f'{start}-{stop}/{download.stream.filesize}'})
