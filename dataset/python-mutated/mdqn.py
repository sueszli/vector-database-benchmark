...
class MDQNPolicy(DQNPolicy):
    ...
    Config:
        ...
        == ==================== ======== ============== ======================================== =======================
    """
    config = dict(type='mdqn', cuda=False, on_policy=False, priority=False, priority_IS_weight=False, discount_factor=0.97, entropy_tau=0.03, m_alpha=0.9, nstep=1, learn=dict(update_per_collect=3, batch_size=64, learning_rate=0.001, target_update_freq=100, ignore_done=False), collect=dict(n_sample=4, unroll_len=1), eval=dict(), other=dict(eps=dict(type='exp', start=0.95, end=0.1, decay=10000), replay_buffer=dict(replay_buffer_size=10000)))
