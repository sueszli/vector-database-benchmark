...
async def main() -> None:
    ...
    async def custom_updates(request: Request) -> PlainTextResponse:
        ...
        try:
            ...
            return PlainTextResponse(status_code=HTTPStatus.BAD_REQUEST, content='Please pass both `user_id` and `payload` as query parameters.')
        except ValueError:
            return PlainTextResponse(status_code=HTTPStatus.BAD_REQUEST, content='The `user_id` must be a string!')
    ...
    starlette_app = Starlette(routes=[Route('/telegram', telegram, methods=['POST']), Route('/healthcheck', health, methods=['GET']), Route('/submitpayload', custom_updates, methods=['POST', 'GET'])])
    webserver = uvicorn.Server(config=uvicorn.Config(app=starlette_app, port=PORT, use_colors=False, host='127.0.0.1'))
