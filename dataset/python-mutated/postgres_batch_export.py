...
async def postgres_connection(inputs) -> typing.AsyncIterator[psycopg.AsyncConnection]:
    ...
    connection = await psycopg.AsyncConnection.connect(user=inputs.user, password=inputs.password, dbname=inputs.database, host=inputs.host, port=inputs.port, sslmode='disable' if inputs.has_self_signed_cert is True else 'prefer')
    try:
        yield connection
    except Exception:
...
async def copy_tsv_to_postgres(tsv_file, postgres_connection: psycopg.AsyncConnection, schema: str, table_name: str, schema_columns: list[str]):
    ...
    async with postgres_connection.cursor() as cursor:
        if schema:
            ...
            async with cursor.copy(sql.SQL("COPY {table_name} ({fields}) FROM STDIN WITH DELIMITER AS '\t'").format(table_name=sql.Identifier(table_name), fields=sql.SQL(',').join((sql.Identifier(column) for column in schema_columns)))) as copy:
                while (data := tsv_file.read()):
                    await copy.write(data)
Field = tuple[str, str]
Fields = collections.abc.Iterable[Field]

async def create_table_in_postgres(postgres_connection: psycopg.AsyncConnection, schema: str | None, table_name: str, fields: Fields) -> None:
    ...
    async with postgres_connection.cursor() as cursor:
        await cursor.execute(sql.SQL('\n                CREATE TABLE IF NOT EXISTS {table} (\n                    {fields}\n                )\n                ').format(table=table_identifier, fields=sql.SQL(',').join((sql.SQL('{field} {type}').format(field=sql.Identifier(field), type=sql.SQL(field_type)) for (field, field_type) in fields))))
        ...
        with BatchExportTemporaryFile() as pg_file:
            async with postgres_connection(inputs) as connection:
                for result in results_iterator:
                    row = {key: json.dumps(result[key]) if key in json_columns else result[key] for key in schema_columns}
                    pg_file.write_records_to_tsv([row], fieldnames=schema_columns)

                async def flush_to_postgres():
                    logger.debug('Copying %s records of size %s bytes', pg_file.records_since_last_reset, pg_file.bytes_since_last_reset)
                    await copy_tsv_to_postgres(pg_file, connection, inputs.schema, inputs.table_name, schema_columns)
                    rows_exported.add(pg_file.records_since_last_reset)
                    bytes_exported.add(pg_file.bytes_since_last_reset)
                for result in results_iterator:
                    row = {key: json.dumps(result[key]) if key in json_columns and result[key] is not None else result[key] for key in schema_columns}
                    pg_file.write_records_to_tsv([row], fieldnames=schema_columns)
...
class PostgresBatchExportWorkflow(PostHogWorkflow):
    ...
    async def run(self, inputs: PostgresBatchExportInputs):
        ...
        insert_inputs = PostgresInsertInputs(team_id=inputs.team_id, user=inputs.user, password=inputs.password, host=inputs.host, port=inputs.port, database=inputs.database, schema=inputs.schema, table_name=inputs.table_name, has_self_signed_cert=inputs.has_self_signed_cert, data_interval_start=data_interval_start.isoformat(), data_interval_end=data_interval_end.isoformat(), exclude_events=inputs.exclude_events, include_events=inputs.include_events)
        await execute_batch_export_insert_activity(insert_into_postgres_activity, insert_inputs, non_retryable_error_types=['OperationalErrorInvalidSchemaNameInsufficientPrivilege'], update_inputs=update_inputs, heartbeat_timeout_seconds=None)