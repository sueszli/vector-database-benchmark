...
class OptionsSheet(Sheet):
    ...
    rowtype = 'options'
    precious = False
    columns = (Column('option', getter=lambda col, row: row.name), Column('module', getter=lambda col, row: row.module, max_help=1), Column('value', getter=lambda col, row: col.sheet.diffOption(row.name), setter=lambda col, row, val: col.sheet.source.options.set(row.name, val)), Column('default', getter=lambda col, row: vd.options.getdefault(row.name)), Column('description', width=40, getter=lambda col, row: vd.options._get(row.name, 'default').helpstr), AttrColumn('replayable', max_help=1))
    colorizers = [CellColorizer(3, None, lambda s, c, r, v: v.value if r and c in s.columns[2:4] and r.name.startswith('color_') else None)]
    nKeys = 2
