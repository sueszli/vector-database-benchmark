...
async def test_dns(caplog_async) -> None:
    ...
    with taddons.context(ps, DummyResolver()) as tctx:
        tctx.configure(ps, mode=['dns@127.0.0.1:0'])
        assert await ps.setup_servers()
        ps.running()
...
async def test_udp(caplog_async) -> None:
    ...
    with taddons.context(ps, nl) as tctx:
        async with udp_server(server_handler) as server_addr:
            ...
            assert await ps.setup_servers()
            ps.running()
            await caplog_async.await_log(f'reverse proxy to udp://{server_addr[0]}:{server_addr[1]} listening')
            assert ps.servers
...
async def quic_server(create_protocol, alpn: list[str]) -> AsyncGenerator[Address, None]:
    ...
    configuration.load_cert_chain(certfile=tlsdata.path('../net/data/verificationcerts/trusted-leaf.crt'), keyfile=tlsdata.path('../net/data/verificationcerts/trusted-leaf.key'))
    loop = asyncio.get_running_loop()
    (transport, server) = await loop.create_datagram_endpoint(lambda : QuicServer(configuration=configuration, create_protocol=create_protocol), local_addr=('127.0.0.1', 0))
    try:
...
@asynccontextmanager
async def quic_connect(cls: type[T], alpn: list[str], address: Address) -> AsyncGenerator[T, None]:
    configuration = QuicConfiguration(is_client=True, alpn_protocols=alpn, server_name='example.mitmproxy.org', verify_mode=ssl.CERT_NONE, max_datagram_frame_size=65536)
    loop = asyncio.get_running_loop()
    (transport, protocol) = await loop.create_datagram_endpoint(lambda : cls(QuicConnection(configuration=configuration)), local_addr=('127.0.0.1', 0))
...
async def _test_echo(client: H3Client, strict: bool) -> None:
    ...
    headers = [(b':scheme', b'https'), (b':authority', b'example.mitmproxy.org'), (b':method', b'GET'), (b':path', b'/test')]
    r1 = await client.request(headers=headers + [(b'x-request', b'justheaders')], data=None, trailers=None).wait_result()
    assert r1.headers == [(b':status', b'200'), (b'x-response', b'justheaders')]
    assert_no_data(r1)
    assert r1.trailers is None
    r2 = await client.request(headers=headers + [(b'x-request', b'hasdata')], data=b'echo', trailers=None).wait_result()
    assert r2.headers == [(b':status', b'200'), (b'x-response', b'hasdata')]
    assert r2.data == b'echo'
    assert r2.trailers is None
    r3 = await client.request(headers=headers + [(b'x-request', b'nodata')], data=None, trailers=[(b'x-request', b'buttrailers')]).wait_result()
    assert r3.headers == [(b':status', b'200'), (b'x-response', b'nodata')]
    assert_no_data(r3)
    assert r3.trailers == [(b'x-response', b'buttrailers')]
    r4 = await client.request(headers=headers + [(b'x-request', b'this')], data=b'has', trailers=[(b'x-request', b'everything')]).wait_result()
    assert r4.headers == [(b':status', b'200'), (b'x-response', b'this')]
    assert r4.data == b'has'
    assert r4.trailers == [(b'x-response', b'everything')]
    r5 = client.request(headers=headers + [(b'x-request', b'this')], data=b'has', trailers=[(b'x-request', b'everything but end_stream')], end_stream=False)
    if not strict:
        ...
        await asyncio.sleep(0)
    client._quic.send_stream_data(stream_id=r5.stream_id, data=b'', end_stream=True)
    client.transmit()
    await r5.wait_result()
    assert r5.headers == [(b':status', b'200'), (b'x-response', b'this')]
...
async def test_reverse_http3_and_quic_stream(caplog_async, scheme: str, connection_strategy: str) -> None:
    caplog_async.set_level('INFO')
    ps = Proxyserver()
    nl = NextLayer()
    ta = TlsConfig()
    with taddons.context(ps, nl, ta) as tctx:
        ...
        async with quic_server(H3EchoServer, alpn=['h3']) as server_addr:
            ...
            tctx.configure(ta, ssl_verify_upstream_trusted_ca=tlsdata.path('../net/data/verificationcerts/trusted-root.crt'))
            tctx.configure(ps, mode=[mode])
            assert await ps.setup_servers()
            ps.running()
