...
from ignite.contrib.handlers.visdom_logger import global_step_from_engine, GradsScalarHandler, VisdomLogger, WeightsScalarHandler
...
def run(train_batch_size, val_batch_size, epochs, lr, momentum, log_dir):
    ...
    vd_logger.attach_output_handler(trainer, event_name=Events.ITERATION_COMPLETED(every=100), tag='training', output_transform=lambda loss: {'batchloss': loss})
    for (tag, evaluator) in [('training', train_evaluator), ('validation', validation_evaluator)]:
        vd_logger.attach_output_handler(evaluator, event_name=Events.EPOCH_COMPLETED, tag=tag, metric_names=['loss', 'accuracy'], global_step_transform=global_step_from_engine(trainer))
