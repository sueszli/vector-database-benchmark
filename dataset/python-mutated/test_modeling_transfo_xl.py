...
class TransfoXLModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TransfoXLModel, TransfoXLLMHeadModel, TransfoXLForSequenceClassification) if is_torch_available() else ()
    all_generative_model_classes = (TransfoXLLMHeadModel,) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': TransfoXLModel, 'text-classification': TransfoXLForSequenceClassification, 'text-generation': TransfoXLLMHeadModel, 'zero-shot': TransfoXLForSequenceClassification} if is_torch_available() else {}
    test_pruning = False
