...
def glm_alpha_array_with_lambda_search_cv():
    ...
    cv_model = glm(family='multinomial', alpha=[0.1], lambda_search=True, nfolds=3, nlambdas=5, fold_assignment='modulo')
    cv_model.train(training_frame=training_data, x=myX, y=myY)
    cv_r = glm.getGLMRegularizationPath(cv_model)
    cv_model_valid = glm(family='multinomial', alpha=[0.1], lambda_search=True, nfolds=3, nlambdas=5, fold_assignment='modulo')
    ...
    for l in range(len(cv_r['lambdas'])):
        ...
        pyunit_utils.assertEqualCoeffDicts(cv_r['coefficients'][l], cv_r_valid['coefficients'][l], tol=1e-06)
        pyunit_utils.assertEqualCoeffDicts(cv_r['coefficients_std'][l], cv_r_valid['coefficients_std'][l], tol=1e-06)
