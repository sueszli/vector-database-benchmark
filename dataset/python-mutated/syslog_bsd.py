            ...
            if syslog_match:
                ...
                if hostname:
                    if hostname.endswith(':'):
                        ...
                        tag = None
                        hostname = hostname[:-1]
                syslog_dict = {'priority': priority, 'date': syslog_match.group('date'), 'hostname': hostname, 'tag': tag, 'content': content.lstrip(' :').rstrip()}
            else:
                syslog_dict = {'unparsable': line}
                if not quiet:
                    jc.utils.warning_message([f'Unparsable line found: {line}'])
            if syslog_dict:
