...
from mage_ai.data_preparation.models.errors import FileExistsError, FileNotInProjectError
...
class FileResource(GenericResource):
    ...
    async def create(self, payload: Dict, user, **kwargs) -> 'FileResource':
        ...
        try:
            ...
            file = File.create(filename, dir_path, repo_path=repo_path, content=content, overwrite=payload.get('overwrite', False))
            ...
            error.update(dict(message=f'File at path: {file_path} is not in the project directory.'))
            raise ApiError(error)
    ...
    async def update(self, payload, **kwargs):
        ...
        if block_type:
            cache_block_action_object = await BlockActionObjectCache.initialize_cache()
            cache_block_action_object.update_block(block_file_absolute_path=self.model.file_path, remove=True)
        new_path = os.path.join(self.model.repo_path, payload['dir_path'], payload['name'])
        try:
            ensure_file_is_in_project(new_path)
        except FileNotInProjectError:
            error = ApiError.RESOURCE_INVALID.copy()
            error.update(dict(message=f'File cannot be moved to path: {new_path} because it is not in the project directory.'))
            raise ApiError(error)
