...
async def serve(q: Q):
    if not q.client.initialized:
        q.page['form'] = ui.form_card(box='1 1 -1 -1', items=[ui.table(name='table', columns=[ui.table_column(name='text', label='Text', searchable=True, link=False)], rows=[ui.table_row(name=i.text, cells=[i.text]) for i in issues[0:rows_per_page]], pagination=ui.table_pagination(total_rows=len(issues), rows_per_page=rows_per_page), height='660px', events=['page_change', 'search'])])
