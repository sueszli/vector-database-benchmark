...
def testGLMBinomialScoringHistoryLambdaSearch():
    if False:
        while True:
            i = 10
    col_list_compare = ['iteration', 'training_logloss', 'validation_logloss', 'training_classification_error', 'validation_classification_error', 'training_rmse', 'validation_rmse', 'training_auc', 'validation_auc', 'training_pr_auc', 'validation_pr_auc', 'training_lift', 'validation_lift', 'deviance_train', 'deviance_test']
    ...
    Y = 'C21'
    X = list(range(0, 20))
    print('Building model with score_interval=1 and lambda search on.  Should generate same model as score_each_iteration turned on.  However, in this case, no scoring history is generated at every iteration due to speed constraint.')
    ...
    h2o_model_score_each_cv = glm(family='binomial', score_each_iteration=True, lambda_search=True, nlambdas=10, nfolds=2, fold_assignment='modulo')
    ...
    h2o_model_cv = glm(family='binomial', score_iteration_interval=1, lambda_search=True, nlambdas=10, nfolds=2, fold_assignment='modulo')
    ...
    h2o_model_4th_cv = glm(family='binomial', score_iteration_interval=4, lambda_search=True, nlambdas=10, nfolds=2, fold_assignment='modulo')
