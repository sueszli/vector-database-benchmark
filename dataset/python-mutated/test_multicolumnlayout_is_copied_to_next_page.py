...
class TestMultiColumnLayoutIsCopiedToNextPage(TestCase):
    ...
    def test_multicolumnlayout_is_copied_to_next_page_001(self):
        ...
        random.seed(0)
        for s in [Lipsum.generate_arthur_conan_doyle_text(random.choice([5, 6, 7])) for _ in range(0, 16)]:
    ...
    def test_multicolumnlayout_is_copied_to_next_page_002(self):
        ...
        layout.add(self.get_test_header(test_description='This test sets a MultiColumnLayout to a PDF with 2 column', font_size=Decimal(8)))
        random.seed(0)
        for s in [Lipsum.generate_arthur_conan_doyle_text(random.choice([5, 6, 7])) for _ in range(0, 16)]:
    ...
    def test_multicolumnlayout_is_copied_to_next_page_003(self):
        ...
        layout.add(self.get_test_header(test_description='This test sets a MultiColumnLayout to a PDF with 3 columns', font_size=Decimal(6)))
        random.seed(0)
        for s in [Lipsum.generate_arthur_conan_doyle_text(random.choice([5, 6, 7])) for _ in range(0, 16)]:
