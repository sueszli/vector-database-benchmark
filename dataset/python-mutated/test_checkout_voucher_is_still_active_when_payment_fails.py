...
from ...utils import checkout_add_promo_code, checkout_create, checkout_delivery_method_update, raw_checkout_dummy_payment_create
...
def test_checkout_voucher_is_still_active_when_payment_fails_core_0919(e2e_not_logged_api_client, e2e_staff_api_client, permission_manage_products, permission_manage_channels, permission_manage_shipping, permission_manage_product_types_and_attributes, permission_manage_discounts, permission_manage_checkouts):
    ...
    data = raw_checkout_dummy_payment_create(e2e_not_logged_api_client, checkout_id, discounted_total_gross, token=None)
    assert data['errors'] != []
    assert data['errors'][0]['field'] == 'token'
    assert data['errors'][0]['code'] == 'REQUIRED'
    assert data['errors'][0]['message'] == 'Token is required for mirumee.payments.dummy.'
