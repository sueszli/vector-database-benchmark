...
from azure.security.attestation import AttestationType, AttestationPolicyToken, PolicyModification, CertificateModification
...
class TestAsyncPolicyGetSet(AzureRecordedTestCase):
    ...
    async def test_aad_set_policy_unsecured(self, attestation_aad_url, **kwargs):
        ...
        (policy_set_response, _) = await attest_client.set_policy(attestation_type, attestation_policy)
    ...
    async def test_aad_reset_policy_unsecured(self, attestation_aad_url, **kwargs):
        ...
        (policy_set_response, _) = await attest_client.reset_policy(kwargs.pop('attestation_type'))
    ...
    @AttestationPreparer()
    @AllAttestationTypes
    async def test_aad_reset_policy_secured(self, attestation_aad_url, attestation_policy_signing_key0, attestation_policy_signing_certificate0, **kwargs):
        ...
        attest_client = self.create_admin_client(attestation_aad_url)
        (policy_set_response, _) = await attest_client.reset_policy(attestation_type, signing_key=key, signing_certificate=signing_certificate)
        assert None == policy_set_response.policy_token_hash
        assert policy_set_response.policy_resolution == PolicyModification.REMOVED
    ...
    @AttestationPreparer()
    @AllAttestationTypes
    async def test_aad_set_policy_secured(self, attestation_aad_url, attestation_policy_signing_key0, attestation_policy_signing_certificate0, **kwargs):
        attestation_policy = u'version=1.0; authorizationrules{=> permit();}; issuancerules{};'
        signing_certificate = pem_from_base64(attestation_policy_signing_certificate0, 'CERTIFICATE')
        key = pem_from_base64(attestation_policy_signing_key0, 'PRIVATE KEY')
        attest_client = self.create_admin_client(attestation_aad_url)
        attestation_type = kwargs.pop('attestation_type')
        (policy_set_response, _) = await attest_client.set_policy(attestation_type, attestation_policy, signing_key=key, signing_certificate=signing_certificate)
    ...
    @AttestationPreparer()
    @AllAttestationTypes
    async def test_isolated_set_policy_secured(self, attestation_isolated_url, attestation_isolated_signing_key, attestation_isolated_signing_certificate, **kwargs):
        attestation_policy = u'version=1.0; authorizationrules{=> permit();}; issuancerules{};'
        decoded_cert = pem_from_base64(attestation_isolated_signing_certificate, 'CERTIFICATE')
        ...
        (policy_set_response, _) = await attest_client.set_policy(attestation_type, attestation_policy, signing_key=key, signing_certificate=decoded_cert)
        (policy, _) = await attest_client.get_policy(attestation_type)
        assert policy == attestation_policy
        expected_policy = AttestationPolicyToken(attestation_policy, signing_key=key, signing_certificate=decoded_cert)
        hasher = hashes.Hash(hashes.SHA256(), backend=default_backend())
        hasher.update(expected_policy.to_jwt_string().encode('utf-8'))
        expected_hash = hasher.finalize()
        assert expected_hash == policy_set_response.policy_token_hash
    ...
    @AttestationPreparer()
    @AllAttestationTypes
    async def test_isolated_reset_policy_secured(self, attestation_aad_url, attestation_isolated_signing_key, attestation_isolated_signing_certificate, **kwargs):
        signing_certificate = pem_from_base64(attestation_isolated_signing_certificate, 'CERTIFICATE')
        key = pem_from_base64(attestation_isolated_signing_key, 'PRIVATE KEY')
        attest_client = self.create_admin_client(attestation_aad_url)
        (policy_set_response, _) = await attest_client.reset_policy(kwargs.pop('attestation_type'), signing_key=key, signing_certificate=signing_certificate)
        assert None == policy_set_response.policy_token_hash
        assert policy_set_response.policy_resolution == PolicyModification.REMOVED
    ...
    async def test_add_remove_policy_certificate(self, **kwargs):
        ...
        admin_client = self.create_admin_client(attestation_isolated_url, signing_key=key, signing_certificate=signing_certificate)
        ...
        (result, _) = await admin_client.add_policy_management_certificate(pem_certificate_to_add)
        assert result.certificate_resolution == CertificateModification.IS_PRESENT
        (result, _) = await admin_client.add_policy_management_certificate(pem_certificate_to_add)
        assert result.certificate_resolution == CertificateModification.IS_PRESENT
        await self._test_get_policy_management_certificates(attestation_isolated_url, pem_certificate_to_add)
        (result, _) = await admin_client.remove_policy_management_certificate(pem_certificate_to_add, signing_key=key, signing_certificate=signing_certificate)
