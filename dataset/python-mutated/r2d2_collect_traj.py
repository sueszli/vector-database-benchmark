    ...
    def _forward_learn(self, data: dict) -> Dict[str, Any]:
        ...
        if len(data['burnin_nstep_obs']) != 0:
            with torch.no_grad():
                ...
                burnin_output = self._learn_model.forward(inputs, saved_state_timesteps=[self._burnin_step, self._burnin_step + self._nstep])
                burnin_output_target = self._target_model.forward(inputs, saved_state_timesteps=[self._burnin_step, self._burnin_step + self._nstep])
