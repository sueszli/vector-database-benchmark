...
class KernelDensity(BaseEstimator):
    ...
    """
    _parameter_constraints: dict = {'bandwidth': [Interval(Real, 0, None, closed='neither'), StrOptions({'scott', 'silverman'})], 'algorithm': [StrOptions(set(TREE_DICT.keys()) | {'auto'})], 'kernel': [StrOptions(set(VALID_KERNELS))], 'metric': [StrOptions(set(itertools.chain(*[VALID_METRICS[alg] for alg in TREE_DICT.keys()])))], 'atol': [Interval(Real, 0, None, closed='left')], 'rtol': [Interval(Real, 0, None, closed='left')], 'breadth_first': ['boolean'], 'leaf_size': [Interval(Integral, 1, None, closed='left')], 'metric_params': [None, dict]}
