...
from trio.testing import MemoryReceiveStream, MemorySendStream, MockClock, memory_stream_pair, wait_all_tasks_blocked
if TYPE_CHECKING:
...
class MemoryListener(trio.abc.Listener[StapledMemoryStream]):
    ...
    queued_streams: tuple[MemorySendChannel[StapledMemoryStream], MemoryReceiveChannel[StapledMemoryStream]] = attr.ib(factory=lambda : trio.open_memory_channel[StapledMemoryStream](1))
...
async def test_serve_listeners_accept_capacity_error(autojump_clock: MockClock, caplog: pytest.LogCaptureFixture) -> None:
    listener = MemoryListener()

    async def raise_EMFILE() -> NoReturn:
        raise OSError(errno.EMFILE, 'out of file descriptors')
    listener.accept_hook = raise_EMFILE
    with trio.move_on_after(0.95):
        await trio.serve_listeners(None, [listener])
...
async def test_serve_listeners_connection_nursery(autojump_clock: MockClock) -> None:
    ...
    async def connection_watcher(*, task_status: TaskStatus[Nursery]=trio.TASK_STATUS_IGNORED) -> NoReturn:
        async with trio.open_nursery() as nursery:
    ...
    with pytest.raises(Done):
        async with trio.open_nursery() as nursery:
            ...
            await nursery.start(partial(trio.serve_listeners, handler, [listener], handler_nursery=handler_nursery))
            for _ in range(10):
