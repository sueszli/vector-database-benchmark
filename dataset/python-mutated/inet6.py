...
from scapy.data import DLT_IPV6, DLT_RAW, DLT_RAW_ALT, ETHER_ANY, ETH_P_ALL, ETH_P_IPV6, MTU
from scapy.error import log_runtime, warning
from scapy.fields import BitEnumField, BitField, ByteEnumField, ByteField, DestIP6Field, FieldLenField, FlagsField, IntField, IP6Field, LongField, MACField, MayEnd, PacketLenField, PacketListField, ShortEnumField, ShortField, SourceIP6Field, StrField, StrFixedLenField, StrLenField, X3BytesField, XBitField, XIntField, XShortField
...
def getmacbyip6(ip6, chainCC=0):
    ...
    if res is not None:
        ...
        return mac
    return None
ipv6nh = {0: 'Hop-by-Hop Option Header', 4: 'IP', 6: 'TCP', 17: 'UDP', 41: 'IPv6', 43: 'Routing Header', 44: 'Fragment Header', 47: 'GRE', 50: 'ESP Header', 51: 'AH Header', 58: 'ICMPv6', 59: 'No Next Header', 60: 'Destination Option Header', 112: 'VRRP', 132: 'SCTP', 135: 'Mobility Header'}
...
class IPv6(_IPv6GuessPayload, Packet, IPTools):
    name = 'IPv6'
    fields_desc = [BitField('version', 6, 4), BitField('tc', 0, 8), BitField('fl', 0, 20), ShortField('plen', None), ByteEnumField('nh', 59, ipv6nh), ByteField('hlim', 64), SourceIP6Field('src', 'dst'), DestIP6Field('dst', '::1')]
...
class _IPv6ExtHdr(_IPv6GuessPayload, Packet):
    ...
    aliastypes = [IPv6, IPerror6]
_hbhopts = {0: 'Pad1', 1: 'PadN', 4: 'Tunnel Encapsulation Limit', 5: 'Router Alert', 6: 'Quick-Start', 194: 'Jumbo Payload', 201: 'Home Address Option'}
...
class _OTypeField(ByteEnumField):
    ...
    """
    pol = {0: '00: skip', 64: '01: discard', 128: '10: discard+ICMP', 192: '11: discard+ICMP not mcast'}
    enroutechange = {0: "0: Don't change en-route", 32: '1: May change en-route'}
...
class HBHOptUnknown(Packet):
    ...
    fields_desc = [_OTypeField('otype', 1, _hbhopts), FieldLenField('optlen', None, length_of='optdata', fmt='B'), StrLenField('optdata', '', length_from=lambda pkt: pkt.optlen)]
...
class Pad1(Packet):
    name = 'Pad1'
    fields_desc = [_OTypeField('otype', 0, _hbhopts)]
...
class RouterAlert(Packet):
    name = 'Router Alert'
    fields_desc = [_OTypeField('otype', 5, _hbhopts), ByteField('optlen', 2), ShortEnumField('value', None, {0: 'Datagram contains a MLD message', 1: 'Datagram contains RSVP message', 2: 'Datagram contains an Active Network message', 68: 'NSIS NATFW NSLP', 69: 'MPLS OAM', 65535: 'Reserved'})]
...
class Jumbo(Packet):
    name = 'Jumbo Payload'
    fields_desc = [_OTypeField('otype', 194, _hbhopts), ByteField('optlen', 4), IntField('jumboplen', None)]
...
class HAO(Packet):
    ...
    fields_desc = [_OTypeField('otype', 201, _hbhopts), ByteField('optlen', 16), IP6Field('hoa', '::')]
    ...
    def extract_padding(self, p):
        if False:
            for i in range(10):
                print('nop')
        return (b'', p)
_hbhoptcls = {0: Pad1, 1: PadN, 5: RouterAlert, 194: Jumbo, 201: HAO}
...
class IPv6ExtHdrHopByHop(_IPv6ExtHdr):
    ...
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), FieldLenField('len', None, length_of='options', fmt='B', adjust=lambda pkt, x: (x + 2 + 7) // 8 - 1), _PhantomAutoPadField('autopad', 1), _OptionsField('options', [], HBHOptUnknown, 2, length_from=lambda pkt: 8 * (pkt.len + 1) - 2)]
...
class IPv6ExtHdrRouting(_IPv6ExtHdr):
    ...
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), FieldLenField('len', None, count_of='addresses', fmt='B', adjust=lambda pkt, x: 2 * x), ByteField('type', 0), ByteField('segleft', None), BitField('reserved', 0, 32), IP6ListField('addresses', [], length_from=lambda pkt: 8 * pkt.len)]
...
_segment_routing_header_tlvs = {0: 'Pad1 TLV', 1: 'Ingress Node TLV', 2: 'Egress Node TLV', 4: 'PadN TLV', 5: 'HMAC TLV'}
...
class IPv6ExtHdrSegmentRoutingTLVPad1(IPv6ExtHdrSegmentRoutingTLV):
    ...
    fields_desc = [ByteEnumField('type', 0, _segment_routing_header_tlvs), FieldLenField('len', None, length_of='padding', fmt='B'), StrLenField('padding', b'\x00', length_from=lambda pkt: pkt.len)]

class IPv6ExtHdrSegmentRoutingTLVPadN(IPv6ExtHdrSegmentRoutingTLV):
    name = 'IPv6 Option Header Segment Routing - PadN TLV'
    fields_desc = [ByteEnumField('type', 4, _segment_routing_header_tlvs), FieldLenField('len', None, length_of='padding', fmt='B'), StrLenField('padding', b'\x00', length_from=lambda pkt: pkt.len)]

class IPv6ExtHdrSegmentRoutingTLVHMAC(IPv6ExtHdrSegmentRoutingTLV):
    name = 'IPv6 Option Header Segment Routing - HMAC TLV'
    fields_desc = [ByteEnumField('type', 5, _segment_routing_header_tlvs), FieldLenField('len', None, length_of='hmac', adjust=lambda _, x: x + 48), BitField('D', 0, 1), BitField('reserved', 0, 15), IntField('hmackeyid', 0), StrLenField('hmac', '', length_from=lambda pkt: pkt.len - 48)]

class IPv6ExtHdrSegmentRouting(_IPv6ExtHdr):
    name = 'IPv6 Option Header Segment Routing'
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), ByteField('len', None), ByteField('type', 4), ByteField('segleft', None), ByteField('lastentry', None), BitField('unused1', 0, 1), BitField('protected', 0, 1), BitField('oam', 0, 1), BitField('alert', 0, 1), BitField('hmac', 0, 1), BitField('unused2', 0, 3), ShortField('tag', 0), IP6ListField('addresses', ['::1'], count_from=lambda pkt: pkt.lastentry + 1), PacketListField('tlv_objects', [], IPv6ExtHdrSegmentRoutingTLV, length_from=lambda pkt: 8 * pkt.len - 16 * (pkt.lastentry + 1))]
    overload_fields = {IPv6: {'nh': 43}}
...
def fragment6(pkt, fragSize):
    ...
    while True:
        if len(remain) > lastFragSize:
            ...
            break
    return res
icmp6typescls = {1: 'ICMPv6DestUnreach', 2: 'ICMPv6PacketTooBig', 3: 'ICMPv6TimeExceeded', 4: 'ICMPv6ParamProblem', 128: 'ICMPv6EchoRequest', 129: 'ICMPv6EchoReply', 130: 'ICMPv6MLQuery', 131: 'ICMPv6MLReport', 132: 'ICMPv6MLDone', 133: 'ICMPv6ND_RS', 134: 'ICMPv6ND_RA', 135: 'ICMPv6ND_NS', 136: 'ICMPv6ND_NA', 137: 'ICMPv6ND_Redirect', 139: 'ICMPv6NIQuery', 140: 'ICMPv6NIReply', 141: 'ICMPv6ND_INDSol', 142: 'ICMPv6ND_INDAdv', 143: 'ICMPv6MLReport2', 144: 'ICMPv6HAADRequest', 145: 'ICMPv6HAADReply', 146: 'ICMPv6MPSol', 147: 'ICMPv6MPAdv', 151: 'ICMPv6MRD_Advertisement', 152: 'ICMPv6MRD_Solicitation', 153: 'ICMPv6MRD_Termination', 155: 'ICMPv6RPL'}
icmp6typesminhdrlen = {1: 8, 2: 8, 3: 8, 4: 8, 128: 8, 129: 8, 130: 24, 131: 24, 132: 24, 133: 8, 134: 16, 135: 24, 136: 24, 137: 40, 141: 8, 142: 8, 143: 8, 144: 8, 145: 8, 146: 8, 147: 8, 151: 8, 152: 4, 153: 4, 155: 4}
icmp6types = {1: 'Destination unreachable', 2: 'Packet too big', 3: 'Time exceeded', 4: 'Parameter problem', 100: 'Private Experimentation', 101: 'Private Experimentation', 128: 'Echo Request', 129: 'Echo Reply', 130: 'MLD Query', 131: 'MLD Report', 132: 'MLD Done', 133: 'Router Solicitation', 134: 'Router Advertisement', 135: 'Neighbor Solicitation', 136: 'Neighbor Advertisement', 137: 'Redirect Message', 138: 'Router Renumbering', 139: 'ICMP Node Information Query', 140: 'ICMP Node Information Response', 141: 'Inverse Neighbor Discovery Solicitation Message', 142: 'Inverse Neighbor Discovery Advertisement Message', 143: 'MLD Report Version 2', 144: 'Home Agent Address Discovery Request Message', 145: 'Home Agent Address Discovery Reply Message', 146: 'Mobile Prefix Solicitation', 147: 'Mobile Prefix Advertisement', 148: 'Certification Path Solicitation', 149: 'Certification Path Advertisement', 151: 'Multicast Router Advertisement', 152: 'Multicast Router Solicitation', 153: 'Multicast Router Termination', 155: 'RPL Control Message', 200: 'Private Experimentation', 201: 'Private Experimentation'}
...
class ICMPv6DestUnreach(_ICMPv6Error):
    ...
    fields_desc = [ByteEnumField('type', 1, icmp6types), ByteEnumField('code', 0, {0: 'No route to destination', 1: 'Communication with destination administratively prohibited', 2: 'Beyond scope of source address', 3: 'Address unreachable', 4: 'Port unreachable'}), XShortField('cksum', None), ByteField('length', 0), X3BytesField('unused', 0)]
...
class ICMPv6TimeExceeded(_ICMPv6Error):
    ...
    fields_desc = [ByteEnumField('type', 3, icmp6types), ByteEnumField('code', 0, {0: 'hop limit exceeded in transit', 1: 'fragment reassembly time exceeded'}), XShortField('cksum', None), ByteField('length', 0), X3BytesField('unused', 0)]

class ICMPv6ParamProblem(_ICMPv6Error):
    name = 'ICMPv6 Parameter Problem'
    fields_desc = [ByteEnumField('type', 4, icmp6types), ByteEnumField('code', 0, {0: 'erroneous header field encountered', 1: 'unrecognized Next Header type encountered', 2: 'unrecognized IPv6 option encountered', 3: 'first fragment has incomplete header chain'}), XShortField('cksum', None), IntField('ptr', 6)]
...
class ICMPv6MLQuery2(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 130, icmp6types), ByteField('code', 0), XShortField('cksum', None), ShortField('mrd', 10000), ShortField('reserved', 0), IP6Field('mladdr', '::'), BitField('Resv', 0, 4), BitField('S', 0, 1), BitField('QRV', 0, 3), ByteField('QQIC', 0), ShortField('sources_number', None), IP6ListField('sources', [], count_from=lambda pkt: pkt.sources_number)]
    overload_fields = {IPv6: {'dst': 'ff02::1', 'hlim': 1, 'nh': 58}}
...
class ICMPv6MLDMultAddrRec(Packet):
    ...
    fields_desc = [ByteField('rtype', 4), FieldLenField('auxdata_len', None, length_of='auxdata', fmt='B'), FieldLenField('sources_number', None, length_of='sources', adjust=lambda p, num: num // 16), IP6Field('dst', '::'), IP6ListField('sources', [], length_from=lambda p: 16 * p.sources_number), StrLenField('auxdata', '', length_from=lambda p: p.auxdata_len)]
...
class ICMPv6MLReport2(_ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 143, icmp6types), ByteField('res', 0), XShortField('cksum', None), ShortField('reserved', 0), ShortField('records_number', None), PacketListField('records', [], ICMPv6MLDMultAddrRec, count_from=lambda p: p.records_number)]
...
class ICMPv6MRD_Termination(_ICMPv6):
    ...
    def extract_padding(self, s):
        if False:
            return 10
        return (s[:4], s[4:])
icmp6ndopts = {1: 'Source Link-Layer Address', 2: 'Target Link-Layer Address', 3: 'Prefix Information', 4: 'Redirected Header', 5: 'MTU', 6: 'NBMA Shortcut Limit Option', 7: 'Advertisement Interval Option', 8: 'Home Agent Information Option', 9: 'Source Address List', 10: 'Target Address List', 11: 'CGA Option', 12: 'RSA Signature Option', 13: 'Timestamp Option', 14: 'Nonce option', 15: 'Trust Anchor Option', 16: 'Certificate Option', 17: 'IP Address Option', 18: 'New Router Prefix Information Option', 19: 'Link-layer Address Option', 20: 'Neighbor Advertisement Acknowledgement Option', 21: 'CARD Request Option', 22: 'CARD Reply Option', 23: 'MAP Option', 24: 'Route Information Option', 25: 'Recursive DNS Server Option', 26: 'IPv6 Router Advertisement Flags Option'}
icmp6ndoptscls = {1: 'ICMPv6NDOptSrcLLAddr', 2: 'ICMPv6NDOptDstLLAddr', 3: 'ICMPv6NDOptPrefixInfo', 4: 'ICMPv6NDOptRedirectedHdr', 5: 'ICMPv6NDOptMTU', 6: 'ICMPv6NDOptShortcutLimit', 7: 'ICMPv6NDOptAdvInterval', 8: 'ICMPv6NDOptHAInfo', 9: 'ICMPv6NDOptSrcAddrList', 10: 'ICMPv6NDOptTgtAddrList', 17: 'ICMPv6NDOptIPAddr', 18: 'ICMPv6NDOptNewRtrPrefix', 19: 'ICMPv6NDOptLLA', 23: 'ICMPv6NDOptMAP', 24: 'ICMPv6NDOptRouteInfo', 25: 'ICMPv6NDOptRDNSS', 26: 'ICMPv6NDOptEFA', 31: 'ICMPv6NDOptDNSSL', 37: 'ICMPv6NDOptCaptivePortal', 38: 'ICMPv6NDOptPREF64'}
icmp6ndraprefs = {0: 'Medium (default)', 1: 'High', 2: 'Reserved', 3: 'Low'}
...
class ICMPv6NDOptPrefixInfo(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 3), ByteField('len', 4), ByteField('prefixlen', 64), BitField('L', 1, 1), BitField('A', 1, 1), BitField('R', 0, 1), BitField('res1', 0, 5), XIntField('validlifetime', 4294967295), XIntField('preferredlifetime', 4294967295), XIntField('res2', 0), IP6Field('prefix', '::')]
...
class ICMPv6NDOptRedirectedHdr(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 4), FieldLenField('len', None, length_of='pkt', fmt='B', adjust=lambda pkt, x: (x + 8) // 8), MayEnd(StrFixedLenField('res', b'\x00' * 6, 6)), TruncPktLenField('pkt', b'', IPv6, length_from=lambda pkt: 8 * pkt.len - 8)]
...
class ICMPv6NDOptShortcutLimit(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 6), ByteField('len', 1), ByteField('shortcutlim', 40), ByteField('res1', 0), IntField('res2', 0)]
...
class ICMPv6NDOptLLA(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 19), ByteField('len', 1), ByteEnumField('optcode', 0, _rfc4068_lla_optcode), MACField('lla', ETHER_ANY)]

class ICMPv6NDOptMAP(_ICMPv6NDGuessPayload, Packet):
    name = 'ICMPv6 Neighbor Discovery - MAP Option'
    fields_desc = [ByteField('type', 23), ByteField('len', 3), BitField('dist', 1, 4), BitField('pref', 15, 4), BitField('R', 1, 1), BitField('res', 0, 7), IntField('validlifetime', 4294967295), IP6Field('addr', '::')]
...
class ICMPv6NDOptRouteInfo(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 24), FieldLenField('len', None, length_of='prefix', fmt='B', adjust=lambda pkt, x: x // 8 + 1), ByteField('plen', None), BitField('res1', 0, 3), BitEnumField('prf', 0, 2, icmp6ndraprefs), BitField('res2', 0, 3), IntField('rtlifetime', 4294967295), _IP6PrefixField('prefix', None)]
...
class ICMPv6NDOptRDNSS(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 25), FieldLenField('len', None, count_of='dns', fmt='B', adjust=lambda pkt, x: 2 * x + 1), ShortField('res', None), IntField('lifetime', 4294967295), IP6ListField('dns', [], length_from=lambda pkt: 8 * (pkt.len - 1))]
...
class ICMPv6NDOptDNSSL(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 31), FieldLenField('len', None, length_of='searchlist', fmt='B', adjust=lambda pkt, x: 1 + x // 8), ShortField('res', None), IntField('lifetime', 4294967295), DomainNameListField('searchlist', [], length_from=lambda pkt: 8 * pkt.len - 8, padded=True)]
...
class ICMPv6NDOptCaptivePortal(_ICMPv6NDGuessPayload, Packet):
    name = 'ICMPv6 Neighbor Discovery Option - Captive-Portal Option'
    fields_desc = [ByteField('type', 37), FieldLenField('len', None, length_of='URI', fmt='B', adjust=lambda pkt, x: (2 + x) // 8), CaptivePortalURI('URI', '', length_from=lambda pkt: 8 * max(pkt.len, 1) - 2)]
...
class ICMPv6NDOptPREF64(_ICMPv6NDGuessPayload, Packet):
    name = 'ICMPv6 Neighbor Discovery Option - PREF64 Option'
    fields_desc = [ByteField('type', 38), ByteField('len', 2), BitField('scaledlifetime', 0, 13), BitEnumField('plc', 0, 3, ['/96', '/64', '/56', '/48', '/40', '/32']), _PREF64('prefix', '::')]
...
class ICMPv6ND_RA(_ICMPv6NDGuessPayload, _ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 134, icmp6types), ByteField('code', 0), XShortField('cksum', None), ByteField('chlim', 0), BitField('M', 0, 1), BitField('O', 0, 1), BitField('H', 0, 1), BitEnumField('prf', 1, 2, icmp6ndraprefs), BitField('P', 0, 1), BitField('res', 0, 2), ShortField('routerlifetime', 1800), IntField('reachabletime', 0), IntField('retranstimer', 0)]
...
class ICMPv6NDOptSrcAddrList(_ICMPv6NDGuessPayload, Packet):
    ...
    fields_desc = [ByteField('type', 9), FieldLenField('len', None, count_of='addrlist', fmt='B', adjust=lambda pkt, x: 2 * x + 1), StrFixedLenField('res', b'\x00' * 6, 6), IP6ListField('addrlist', [], length_from=lambda pkt: 8 * (pkt.len - 1))]
...
class ICMPv6NIQueryNOOP(_ICMPv6NIHashret, _ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 139, icmp6types), NIQueryCodeField('code', None, _niquery_code), XShortField('cksum', None), ShortEnumField('qtype', 0, icmp6_niqtypes), BitField('unused', 0, 10), FlagsField('flags', 0, 6, 'TACLSG'), NonceField('nonce', None), NIQueryDataField('data', None)]
...
class ICMPv6NIReplyNOOP(_ICMPv6NIAnswers, _ICMPv6NIHashret, _ICMPv6):
    ...
    fields_desc = [ByteEnumField('type', 140, icmp6types), ByteEnumField('code', 0, _nireply_code), XShortField('cksum', None), ShortEnumField('qtype', 0, icmp6_niqtypes), BitField('unused', 0, 10), FlagsField('flags', 0, 6, 'TACLSG'), NonceField('nonce', None), NIReplyDataField('data', None)]
...
def _niquery_guesser(p):
    ...
    return cls
rplcodes = {0: 'DIS', 1: 'DIO', 2: 'DAO', 3: 'DAO-ACK', 7: 'DCO', 8: 'DCO-ACK'}
...
_mobopttypes = {2: 'Binding Refresh Advice', 3: 'Alternate Care-of Address', 4: 'Nonce Indices', 5: 'Binding Authorization Data', 6: 'Mobile Network Prefix (RFC3963)', 7: 'Link-Layer Address (RFC4068)', 8: 'Mobile Node Identifier (RFC4283)', 9: 'Mobility Message Authentication (RFC4285)', 10: 'Replay Protection (RFC4285)', 11: 'CGA Parameters Request (RFC4866)', 12: 'CGA Parameters (RFC4866)', 13: 'Signature (RFC4866)', 14: 'Home Keygen Token (RFC4866)', 15: 'Care-of Test Init (RFC4866)', 16: 'Care-of Test (RFC4866)'}
...
class MIP6OptLLAddr(_MIP6OptAlign):
    ...
    fields_desc = [ByteEnumField('otype', 7, _mobopttypes), ByteField('olen', 7), ByteEnumField('ocode', 2, _rfc4068_lla_optcode), ByteField('pad', 0), MACField('lla', ETHER_ANY)]
    x = 0
    y = 0

class MIP6OptMNID(_MIP6OptAlign):
    name = 'MIPv6 Option - Mobile Node Identifier'
    fields_desc = [ByteEnumField('otype', 8, _mobopttypes), FieldLenField('olen', None, length_of='id', fmt='B', adjust=lambda pkt, x: x + 1), ByteEnumField('subtype', 1, {1: 'NAI'}), StrLenField('id', '', length_from=lambda pkt: pkt.olen - 1)]
    x = 0
    y = 0

class MIP6OptMsgAuth(_MIP6OptAlign):
    name = 'MIPv6 Option - Mobility Message Authentication'
    fields_desc = [ByteEnumField('otype', 9, _mobopttypes), FieldLenField('olen', None, length_of='authdata', fmt='B', adjust=lambda pkt, x: x + 5), ByteEnumField('subtype', 1, {1: 'MN-HA authentication mobility option', 2: 'MN-AAA authentication mobility option'}), IntField('mspi', None), StrLenField('authdata', 'A' * 12, length_from=lambda pkt: pkt.olen - 5)]
    x = 4
    y = 1
...
class MIP6MH_Generic(_MobilityHeader):
    ...
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), ByteField('len', None), ByteEnumField('mhtype', None, mhtypes), ByteField('res', None), XShortField('cksum', None), StrLenField('msg', b'\x00' * 2, length_from=lambda pkt: 8 * pkt.len - 6)]

class MIP6MH_BRR(_MobilityHeader):
    name = 'IPv6 Mobility Header - Binding Refresh Request'
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), ByteField('len', None), ByteEnumField('mhtype', 0, mhtypes), ByteField('res', None), XShortField('cksum', None), ShortField('res2', None), _PhantomAutoPadField('autopad', 1), _OptionsField('options', [], MIP6OptUnknown, 8, length_from=lambda pkt: 8 * pkt.len)]
    overload_fields = {IPv6: {'nh': 135}}
...
class MIP6MH_HoTI(_MobilityHeader):
    ...
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), ByteField('len', None), ByteEnumField('mhtype', 1, mhtypes), ByteField('res', None), XShortField('cksum', None), StrFixedLenField('reserved', b'\x00' * 2, 2), StrFixedLenField('cookie', b'\x00' * 8, 8), _PhantomAutoPadField('autopad', 1), _OptionsField('options', [], MIP6OptUnknown, 16, length_from=lambda pkt: 8 * (pkt.len - 1))]
    overload_fields = {IPv6: {'nh': 135}}
...
class MIP6MH_HoT(_MobilityHeader):
    ...
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), ByteField('len', None), ByteEnumField('mhtype', 3, mhtypes), ByteField('res', None), XShortField('cksum', None), ShortField('index', None), StrFixedLenField('cookie', b'\x00' * 8, 8), StrFixedLenField('token', b'\x00' * 8, 8), _PhantomAutoPadField('autopad', 1), _OptionsField('options', [], MIP6OptUnknown, 24, length_from=lambda pkt: 8 * (pkt.len - 2))]
    overload_fields = {IPv6: {'nh': 135}}
...
class MIP6MH_BU(_MobilityHeader):
    ...
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), ByteField('len', None), ByteEnumField('mhtype', 5, mhtypes), ByteField('res', None), XShortField('cksum', None), XShortField('seq', None), FlagsField('flags', 'KHA', 7, 'PRMKLHA'), XBitField('reserved', 0, 9), LifetimeField('mhtime', 3), _PhantomAutoPadField('autopad', 1), _OptionsField('options', [], MIP6OptUnknown, 12, length_from=lambda pkt: 8 * pkt.len - 4)]
    overload_fields = {IPv6: {'nh': 135}}
...
class MIP6MH_BA(_MobilityHeader):
    ...
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), ByteField('len', None), ByteEnumField('mhtype', 6, mhtypes), ByteField('res', None), XShortField('cksum', None), ByteEnumField('status', 0, bastatus), FlagsField('flags', 'K', 3, 'PRK'), XBitField('res2', None, 5), XShortField('seq', None), XShortField('mhtime', 0), _PhantomAutoPadField('autopad', 1), _OptionsField('options', [], MIP6OptUnknown, 12, length_from=lambda pkt: 8 * pkt.len - 4)]
    overload_fields = {IPv6: {'nh': 135}}
...
class MIP6MH_BE(_MobilityHeader):
    ...
    fields_desc = [ByteEnumField('nh', 59, ipv6nh), ByteField('len', None), ByteEnumField('mhtype', 7, mhtypes), ByteField('res', 0), XShortField('cksum', None), ByteEnumField('status', 0, _bestatus), ByteField('reserved', 0), IP6Field('ha', '::'), _OptionsField('options', [], MIP6OptUnknown, 24, length_from=lambda pkt: 8 * (pkt.len - 2))]
    overload_fields = {IPv6: {'nh': 135}}
...
bind_layers(Ether, IPv6, type=34525)
bind_layers(CookedLinux, IPv6, proto=34525)
bind_layers(GRE, IPv6, proto=34525)
bind_layers(SNAP, IPv6, code=34525)
if WINDOWS:
    bind_layers(Loopback, IPv6, type=24)
else:
