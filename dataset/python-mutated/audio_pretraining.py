...
class AudioPretrainingConfig(FairseqDataclass):
    ...
    multi_corpus_keys: Optional[str] = field(default=None, metadata={'help': 'Comma separated names for loading multi corpus datasets'})
    multi_corpus_sampling_weights: Optional[str] = field(default=None, metadata={'help': 'Comma separated string of sampling weights corresponding to the multi_corpus_keys'})
    binarized_dataset: bool = field(default=False, metadata={'help': 'if true, loads binarized dataset (useful for very large datasets). See examples/wav2vec/scripts/binarize_manifest.sh'})
    ...
    min_sample_size: Optional[int] = field(default=None, metadata={'help': 'min sample size to skip small examples'})
    num_batch_buckets: int = field(default=0, metadata={'help': 'number of buckets'})
