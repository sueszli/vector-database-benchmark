...
@testing.parameterize(*testing.product({'x_dtype': [numpy.float16, numpy.float32, numpy.float64], 'W_dtype': [numpy.float16, numpy.float32, numpy.float64]}))
@testing.inject_backend_tests(None, [{}] + testing.product({'use_cuda': [True], 'use_cudnn': ['never', 'always'], 'cuda_device': [0, 1]}) + [{'use_chainerx': True, 'chainerx_device': 'native:0'}, {'use_chainerx': True, 'chainerx_device': 'cuda:0'}, {'use_chainerx': True, 'chainerx_device': 'cuda:1'}])
...
@testing.parameterize(*testing.product({'conv_args': [((None, 2, 3, 2, 1), {}), ((2, 3), {'stride': 2, 'pad': 1})]}))
@testing.inject_backend_tests(None, [{}] + testing.product({'use_cuda': [True], 'use_cudnn': ['never', 'always'], 'cuda_device': [0, 1]}) + [{'use_chainerx': True, 'chainerx_device': 'native:0'}, {'use_chainerx': True, 'chainerx_device': 'cuda:0'}, {'use_chainerx': True, 'chainerx_device': 'cuda:1'}])
...
@testing.inject_backend_tests(['test_param_layout_to_device', 'test_forward'], [{'use_cuda': True, 'use_cudnn': 'always'}])
class TestConvolution2DMemoryLayouts(unittest.TestCase, Convolution2DMemoryLayoutsTestBase):
...
@testing.inject_backend_tests(['test_forward'], [{}, {'use_cuda': True, 'use_cudnn': 'never'}])
class TestConvolution2DInvalidComputeMode(unittest.TestCase, Convolution2DMemoryLayoutsTestBase):
