...
import pytest
import yaml
from ray.autoscaler._private.kuberay.autoscaling_config import _derive_autoscaling_config_from_ray_cr, AutoscalingConfigProducer, _round_up_k8s_quantity
...
PARAM_ARGS = ','.join(['ray_cr_in', 'expected_config_out', 'expected_error', 'expected_error_message', 'expected_log_warning'])
TEST_DATA = [] if platform.system() == 'Windows' else [pytest.param(get_basic_ray_cr(), _get_basic_autoscaling_config(), None, None, None, id='basic'), pytest.param(_get_ray_cr_no_cpu_error(), None, ValueError, _get_no_cpu_error(), None, id='no-cpu-error'), pytest.param(_get_ray_cr_with_overrides(), _get_autoscaling_config_with_overrides(), None, None, None, id='overrides'), pytest.param(_get_ray_cr_with_autoscaler_options(), _get_autoscaling_config_with_options(), None, None, None, id='autoscaler-options')]
