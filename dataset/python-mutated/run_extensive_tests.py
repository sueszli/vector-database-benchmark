...
if __name__ == '__main__':
    ...
    datasets = list(map(lambda x: x['Prefix'][:-1], s3_client.list_objects(Bucket=bucket_name, Prefix='', Delimiter='/')['CommonPrefixes']))
    displayed_results = {}
    run_time = {}
    error_log = {}
    for dataset in datasets:
        ...
        train_ds = Dataset(train_df, label_name=metadata['label_name'], features=metadata['features'], cat_features=metadata['cat_features'])
        test_ds = Dataset(test_df, label_name=metadata['label_name'], features=metadata['features'], cat_features=metadata['cat_features'])
        ...
        for model_obj in models:
            ...
            for (name, check) in OverallCheckSuite.checks.items():
                ...
                try:
                    displayed_results[dataset][model_name][check_name] = suite_of_check_to_run.run(model, train_ds, test_ds, 'both')
