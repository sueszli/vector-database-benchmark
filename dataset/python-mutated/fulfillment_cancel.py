...
class FulfillmentCancelInput(BaseInputObjectType):
    warehouse_id = graphene.ID(description='ID of a warehouse where items will be restocked. Optional when fulfillment is in WAITING_FOR_APPROVAL state.', required=False)
...
class FulfillmentCancel(BaseMutation):
    ...
    class Arguments:
        ...
        input = FulfillmentCancelInput(required=False, description='Fields required to cancel a fulfillment.')
