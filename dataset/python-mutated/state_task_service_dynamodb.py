...
from localstack.services.stepfunctions.asl.component.state.state_execution.state_task.service.resource import ResourceRuntimePart
...
class StateTaskServiceDynamoDB(StateTaskService):
    ...
    def _error_cause_from_client_error(client_error: ClientError) -> tuple[str, str]:
        ...
        response_details = '; '.join(['Service: AmazonDynamoDBv2', f"Status Code: {client_error.response['ResponseMetadata']['HTTPStatusCode']}", f'Error Code: {error_code}', f"Request ID: {client_error.response['ResponseMetadata']['RequestId']}", 'Proxy: null'])
    ...
    def _from_error(self, env: Environment, ex: Exception) -> FailureEvent:
        ...
        if isinstance(ex, ClientError):
            ...
            return FailureEvent(error_name=CustomErrorName(self._ERROR_NAME_AWS), event_type=HistoryEventType.TaskFailed, event_details=EventDetails(taskFailedEventDetails=TaskFailedEventDetails(error=self._ERROR_NAME_AWS, cause=str(ex), resource=self._get_sfn_resource(), resourceType=self._get_sfn_resource_type())))
    ...
    def _eval_service_task(self, env: Environment, resource_runtime_part: ResourceRuntimePart, normalised_parameters: dict):
        ...
        dynamodb_client = boto_client_for(region=resource_runtime_part.region, account=resource_runtime_part.account, service='dynamodb')
