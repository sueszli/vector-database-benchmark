import abc
from typing import TYPE_CHECKING, Any, Collection, Dict, Iterable, List, Mapping, Optional, Sequence, Set, Tuple, Union, cast, overload
import attr
from canonicaljson import encode_canonical_json
from typing_extensions import Literal
from synapse.api.constants import DeviceKeyAlgorithms
from synapse.appservice import TransactionOneTimeKeysCount, TransactionUnusedFallbackKeys
from synapse.logging.opentracing import log_kv, set_tag, trace
from synapse.replication.tcp.streams._base import DeviceListsStream
from synapse.storage._base import SQLBaseStore, db_to_json
from synapse.storage.database import DatabasePool, LoggingDatabaseConnection, LoggingTransaction, make_in_list_sql_clause, make_tuple_in_list_sql_clause
...
class EndToEndKeyWorkerStore(EndToEndKeyBackgroundStore, CacheInvalidationWorkerStore):
    ...
    async def get_e2e_device_keys_for_federation_query(self, user_id: str) -> Tuple[int, Sequence[JsonMapping]]:
        ...
        cached_results = self._get_e2e_device_keys_for_federation_query_inner.cache.get_immediate(user_id, None)
        if cached_results is not None:
            sql = '\n                SELECT user_id FROM device_lists_stream\n                WHERE stream_id >= ? AND user_id = ?\n            '
            rows = await self.db_pool.execute('get_e2e_device_keys_for_federation_query_check', sql, now_stream_id, user_id)
            if not rows:
    ...
    async def _get_e2e_device_keys_for_federation_query_inner(self, user_id: str) -> Sequence[JsonMapping]:
        """Get all devices (with any device keys) for a user"""
    ...
    @trace
    @cancellable
    async def get_e2e_device_keys_for_cs_api(self, query_list: Collection[Tuple[str, Optional[str]]], include_displaynames: bool=True) -> Dict[str, Dict[str, JsonDict]]:
    ...
    @overload
    async def get_e2e_device_keys_and_signatures(self, query_list: Collection[Tuple[str, Optional[str]]], include_all_devices: Literal[False]=False) -> Dict[str, Dict[str, DeviceKeyLookupResult]]:
        ...

    @overload
    async def get_e2e_device_keys_and_signatures(self, query_list: Collection[Tuple[str, Optional[str]]], include_all_devices: bool=False, include_deleted_devices: Literal[False]=False) -> Dict[str, Dict[str, DeviceKeyLookupResult]]:
        ...

    @overload
    async def get_e2e_device_keys_and_signatures(self, query_list: Collection[Tuple[str, Optional[str]]], include_all_devices: Literal[True], include_deleted_devices: Literal[True]) -> Dict[str, Dict[str, Optional[DeviceKeyLookupResult]]]:
        ...

    @trace
    @cancellable
    async def get_e2e_device_keys_and_signatures(self, query_list: Collection[Tuple[str, Optional[str]]], include_all_devices: bool=False, include_deleted_devices: bool=False) -> Union[Dict[str, Dict[str, DeviceKeyLookupResult]], Dict[str, Dict[str, Optional[DeviceKeyLookupResult]]]]:
        ...
        result = await self._get_e2e_device_keys(query_list, include_all_devices, include_deleted_devices)
        signature_query = ((user_id, device_id) for (user_id, dev) in result.items() for (device_id, d) in dev.items() if d is not None and d.keys is not None)
        for batch in batch_iter(signature_query, 50):
            cross_sigs_result = await self.db_pool.runInteraction('get_e2e_cross_signing_signatures_for_devices', self._get_e2e_cross_signing_signatures_for_devices_txn, batch)
            for (user_id, key_id, device_id, signature) in cross_sigs_result:
                target_device_result = result[user_id][device_id]
                assert target_device_result is not None
                assert target_device_result.keys is not None
                target_device_signatures = target_device_result.keys.setdefault('signatures', {})
                signing_user_signatures = target_device_signatures.setdefault(user_id, {})
                signing_user_signatures[key_id] = signature
        log_kv(result)
        return result

    async def _get_e2e_device_keys(self, query_list: Collection[Tuple[str, Optional[str]]], include_all_devices: bool=False, include_deleted_devices: bool=False) -> Dict[str, Dict[str, Optional[DeviceKeyLookupResult]]]:
        ...
        if user_list:
            (user_id_in_list_clause, user_args) = make_in_list_sql_clause(self.database_engine, 'user_id', user_list)
        ...
        if user_device_list:
            for user_device_batch in batch_iter(user_device_list, 1024):
                (user_device_id_in_list_clause, user_device_args) = make_tuple_in_list_sql_clause(self.database_engine, ('user_id', 'device_id'), user_device_batch)
        ...
        for (query_clause, query_params) in zip(query_clauses, query_params_list):
            await self.db_pool.runInteraction('_get_e2e_device_keys', get_e2e_device_keys_txn, query_clause, query_params)
        if include_deleted_devices:
    ...
    async def get_e2e_one_time_keys(self, user_id: str, device_id: str, key_ids: List[str]) -> Dict[Tuple[str, str], str]:
        ...
        """
        rows = cast(List[Tuple[str, str, str]], await self.db_pool.simple_select_many_batch(table='e2e_one_time_keys_json', column='key_id', iterable=key_ids, retcols=('algorithm', 'key_id', 'key_json'), keyvalues={'user_id': user_id, 'device_id': device_id}, desc='add_e2e_one_time_keys_check'))
        result = {(algorithm, key_id): key_json for (algorithm, key_id, key_json) in rows}
        log_kv({'message': 'Fetched one time keys for user', 'one_time_keys': result})
        return result

    async def add_e2e_one_time_keys(self, user_id: str, device_id: str, time_now: int, new_keys: Iterable[Tuple[str, str, str]]) -> None:
        ...
        """
        await self.db_pool.runInteraction('add_e2e_one_time_keys_insert', self._add_e2e_one_time_keys_txn, user_id, device_id, time_now, new_keys)
    ...
    async def count_e2e_one_time_keys(self, user_id: str, device_id: str) -> Mapping[str, int]:
        ...
        def _count_e2e_one_time_keys(txn: LoggingTransaction) -> Dict[str, int]:
            ...
            return result
        return await self.db_pool.runInteraction('count_e2e_one_time_keys', _count_e2e_one_time_keys)

    async def count_bulk_e2e_one_time_keys_for_as(self, user_ids: Collection[str]) -> TransactionOneTimeKeysCount:
        """
        ...
        def _count_bulk_e2e_one_time_keys_txn(txn: LoggingTransaction) -> TransactionOneTimeKeysCount:
            ...
            return result
        return await self.db_pool.runInteraction('count_bulk_e2e_one_time_keys', _count_bulk_e2e_one_time_keys_txn)

    async def get_e2e_bulk_unused_fallback_key_types(self, user_ids: Collection[str]) -> TransactionUnusedFallbackKeys:
        """
        ...
        def _get_bulk_e2e_unused_fallback_keys_txn(txn: LoggingTransaction) -> TransactionUnusedFallbackKeys:
            ...
            return result
        return await self.db_pool.runInteraction('_get_bulk_e2e_unused_fallback_keys', _get_bulk_e2e_unused_fallback_keys_txn)

    async def set_e2e_fallback_keys(self, user_id: str, device_id: str, fallback_keys: JsonDict) -> None:
        ...
        """
        await self.db_pool.runInteraction('set_e2e_fallback_keys_txn', self._set_e2e_fallback_keys_txn, user_id, device_id, fallback_keys)
    ...
    @cached(max_entries=10000)
    async def get_e2e_unused_fallback_key_types(self, user_id: str, device_id: str) -> Sequence[str]:
        ...
        Returns:
            a list of key types
        """
        return await self.db_pool.simple_select_onecol('e2e_fallback_keys_json', keyvalues={'user_id': user_id, 'device_id': device_id, 'used': False}, retcol='algorithm', desc='get_e2e_unused_fallback_key_types')

    async def get_e2e_cross_signing_key(self, user_id: str, key_type: str, from_user_id: Optional[str]=None) -> Optional[JsonMapping]:
    ...
    @cachedList(cached_method_name='_get_bare_e2e_cross_signing_keys', list_name='user_ids', num_args=1)
    async def _get_bare_e2e_cross_signing_keys_bulk(self, user_ids: Iterable[str]) -> Mapping[str, Optional[Mapping[str, JsonMapping]]]:
        ...
        """
        return await self.db_pool.runInteraction('get_bare_e2e_cross_signing_keys_bulk', self._get_bare_e2e_cross_signing_keys_bulk_txn, user_ids)
    ...
    @cancellable
    async def get_e2e_cross_signing_keys_bulk(self, user_ids: List[str], from_user_id: Optional[str]=None) -> Mapping[str, Optional[Mapping[str, JsonMapping]]]:
        ...
        if from_user_id:
            result = cast(Dict[str, Optional[Mapping[str, JsonMapping]]], await self.db_pool.runInteraction('get_e2e_cross_signing_signatures', self._get_e2e_cross_signing_signatures_txn, result, from_user_id))
        return result

    async def get_all_user_signature_changes_for_remotes(self, instance_name: str, last_id: int, current_id: int, limit: int) -> Tuple[List[Tuple[int, tuple]], int, bool]:
        ...
        return await self.db_pool.runInteraction('get_all_user_signature_changes_for_remotes', _get_all_user_signature_changes_for_remotes_txn)
    ...
    async def claim_e2e_one_time_keys(self, query_list: Collection[Tuple[str, str, str, int]]) -> Tuple[Dict[str, Dict[str, Dict[str, JsonDict]]], List[Tuple[str, str, str, int]]]:
        ...
        if isinstance(self.database_engine, PostgresEngine):
            ...
            bulk_claims = await self.db_pool.runInteraction('claim_e2e_one_time_keys', self._claim_e2e_one_time_keys_bulk, query_list, db_autocommit=True)
            ...
            missing = [(user, device, alg, count) for ((user, device, alg), count) in unfulfilled_claim_counts.items() if count > 0]
        else:
            for (user_id, device_id, algorithm, count) in query_list:
                claim_rows = await self.db_pool.runInteraction('claim_e2e_one_time_keys', self._claim_e2e_one_time_key_simple, user_id, device_id, algorithm, count, db_autocommit=False)
                if claim_rows:
                    device_results = results.setdefault(user_id, {}).setdefault(device_id, {})
    ...
    async def claim_e2e_fallback_keys(self, query_list: Iterable[Tuple[str, str, str, bool]]) -> Dict[str, Dict[str, Dict[str, JsonDict]]]:
        ...
        """
        if isinstance(self.database_engine, PostgresEngine):
            return await self.db_pool.runInteraction('_claim_e2e_fallback_keys_bulk', self._claim_e2e_fallback_keys_bulk_txn, query_list, db_autocommit=True)
        else:
    ...
    async def _claim_e2e_fallback_keys_simple(self, query_list: Iterable[Tuple[str, str, str, bool]]) -> Dict[str, Dict[str, Dict[str, JsonDict]]]:
        ...
        for (user_id, device_id, algorithm, mark_as_used) in query_list:
            row = await self.db_pool.simple_select_one(table='e2e_fallback_keys_json', keyvalues={'user_id': user_id, 'device_id': device_id, 'algorithm': algorithm}, retcols=('key_id', 'key_json', 'used'), desc='_get_fallback_key', allow_none=True)
            if row is None:
                continue
            (key_id, key_json, used) = row
            if not used and mark_as_used:
                await self.db_pool.simple_update_one(table='e2e_fallback_keys_json', keyvalues={'user_id': user_id, 'device_id': device_id, 'algorithm': algorithm, 'key_id': key_id}, updatevalues={'used': True}, desc='_get_fallback_key_set_used')
                await self.invalidate_cache_and_stream('get_e2e_unused_fallback_key_types', (user_id, device_id))
            device_results = results.setdefault(user_id, {}).setdefault(device_id, {})
            device_results[f'{algorithm}:{key_id}'] = json_decoder.decode(key_json)
        return results
    ...
    async def get_master_cross_signing_key_updatable_before(self, user_id: str) -> Tuple[bool, Optional[int]]:
        ...
        def impl(txn: LoggingTransaction) -> Tuple[bool, Optional[int]]:
            ...
            if row is None:
                return (False, None)
            return (True, row[0])
        return await self.db_pool.runInteraction('e2e_cross_signing_keys', impl)
...
class EndToEndKeyStore(EndToEndKeyWorkerStore, SQLBaseStore):
    ...
    async def set_e2e_device_keys(self, user_id: str, device_id: str, time_now: int, device_keys: JsonDict) -> bool:
        ...
        """
        return await self.db_pool.runInteraction('set_e2e_device_keys', self._set_e2e_device_keys_txn, user_id, device_id, time_now, device_keys)
    ...
    async def set_e2e_cross_signing_key(self, user_id: str, key_type: str, key: JsonDict) -> None:
        """Set a user's cross-signing key.
        ...
        async with self._cross_signing_id_gen.get_next() as stream_id:
            return await self.db_pool.runInteraction('add_e2e_cross_signing_key', self._set_e2e_cross_signing_key_txn, user_id, key_type, key, stream_id)

    async def store_e2e_cross_signing_signatures(self, user_id: str, signatures: 'Iterable[SignatureListItem]') -> None:
        ...
        """
        await self.db_pool.simple_insert_many('e2e_cross_signing_signatures', keys=('user_id', 'key_id', 'target_user_id', 'target_device_id', 'signature'), values=[(user_id, item.signing_key_id, item.target_user_id, item.target_device_id, item.signature) for item in signatures], desc='add_e2e_signing_key')

    async def allow_master_cross_signing_key_replacement_without_uia(self, user_id: str, duration_ms: int) -> Optional[int]:
        ...
        def impl(txn: LoggingTransaction) -> Optional[int]:
            ...
            if txn.rowcount == 0:
                return None
            return timestamp
        return await self.db_pool.runInteraction('allow_master_cross_signing_key_replacement_without_uia', impl)