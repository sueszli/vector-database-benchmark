...
def test_publish_template(mocks):
    if False:
        for i in range(10):
            print('nop')
    with patch('superagi.helper.auth.get_user_organisation') as mock_get_user_org, patch('superagi.helper.auth.get_current_user') as mock_get_user, patch('superagi.helper.auth.db') as mock_auth_db, patch('superagi.controllers.agent_template.db') as mock_db:
