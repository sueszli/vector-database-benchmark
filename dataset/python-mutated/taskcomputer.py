...
class NewTaskComputer:
    ...
    async def _create_client_and_compute(self) -> Path:
        ...
        task_api_service = EnvironmentTaskApiService(env=env, payload_builder=payload_builder, prereq=prereq, shared_dir=shared_dir)
        self._app_client = await ProviderAppClient.create(task_api_service)
        return await self._app_client.compute(task_id=assigned_task.task_id, subtask_id=assigned_task.subtask_id, subtask_params=assigned_task.subtask_params)
...
class TaskComputation:
    ...
    non-single\xa0core computation or multiple single-core computations.
    """
