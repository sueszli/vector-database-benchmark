...
class TestLSTMForecaster(TestCase):
    ...
    def setUp(self):
        ...
        self.forecaster = LSTMForecaster(past_seq_len=10, input_feature_num=10, output_feature_num=2)
    ...
    def test_lstm_forecaster_evaluate(self):
        ...
        forecaster = LSTMForecaster.from_tsdataset(train_tsdata, past_seq_len=24, hidden_dim=16, layer_num=2)
    ...
    def test_lstm_customized_loss_metric(self):
        ...
        from bigdl.chronos.forecaster.tf.lstm_forecaster import LSTMForecaster
        self.forecaster = LSTMForecaster(past_seq_len=10, input_feature_num=10, output_feature_num=2, loss=loss, metrics=[customized_metric], lr=0.01)
        self.forecaster.fit(train_data, epochs=2, batch_size=32)
        yhat = self.forecaster.predict(test_data)
        with tempfile.TemporaryDirectory() as tmp_dir_file:
            tmp_dir_file = os.path.join(tmp_dir_file, 'lstm.ckpt')
    ...
    def test_lstm_from_tsdataset(self):
        ...
        test.roll(lookback=lstm.model_config['past_seq_len'], horizon=lstm.model_config['future_seq_len'])
    ...
    def test_lstm_forecaster_distributed(self):
        ...
        forecaster = LSTMForecaster(past_seq_len=10, input_feature_num=10, output_feature_num=2, loss='mae', distributed=True)
    ...
    def test_lstm_forecaster_distributed_illegal_input(self):
        ...
        from bigdl.orca import init_orca_context, stop_orca_context
        init_orca_context(cores=2, memory='2g')
        forecaster = LSTMForecaster(past_seq_len=10, input_feature_num=2, output_feature_num=2, loss='mae', distributed=True)
        (train_data, _, test_data) = create_data(tf_data=True)
