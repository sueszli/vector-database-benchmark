...
from ray.serve.http_adapters import NdArray, image_to_ndarray, json_to_multi_ndarray, json_to_ndarray, pandas_read_json
...
async def test_pandas_dataframe():
    ...
    parsed_df = await pandas_read_json(MockRequest(_body=raw_json, query_params={'orient': 'index'}))
    assert parsed_df.equals(df)
    raw_json = df.to_json(orient='records')
    parsed_df = await pandas_read_json(MockRequest(_body=raw_json, query_params={'orient': 'records'}))
    assert parsed_df.equals(df)
