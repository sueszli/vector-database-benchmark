...
class CryptoTicker(GenPollUrl):
    ...
    """
    QUERY_URL_DICT = {'coinbase': ('https://api.coinbase.com/v2/prices/{}-{}/spot', lambda x: float(x['data']['amount'])), 'binance': ('https://api.binance.com/api/v3/ticker/price?symbol={}{}', lambda x: float(x['price']))}
    defaults = [('currency', _DEFAULT_CURRENCY.strip(), 'The baseline currency that the value of the crypto is displayed in.'), ('symbol', _DEFAULT_SYMBOL, 'The symbol for the baseline currency.'), ('crypto', 'BTC', 'The cryptocurrency to display.'), ('format', '{crypto}: {symbol}{amount:.2f}', 'Display string formatting.'), ('api', 'coinbase', 'API that provides the data.')]
