...
CURRENT_PLAIN_VERSION = AwesomeVersion(CURRENT_VERSION.string.removesuffix(f'{CURRENT_VERSION.modifier}'))
...
async def async_provide_implementation(hass: HomeAssistant, domain: str) -> list[config_entry_oauth2_flow.AbstractOAuth2Implementation]:
    ...
    for service in services:
        if service['service'] == domain and service['min_version'] <= CURRENT_PLAIN_VERSION and (service.get('accepts_new_authorizations', True) or ((entries := hass.config_entries.async_entries(domain)) and any((entry.data.get('auth_implementation') == DOMAIN for entry in entries)))):
...
class CloudOAuth2Implementation(config_entry_oauth2_flow.AbstractOAuth2Implementation):
    ...
    async def async_generate_authorize_url(self, flow_id: str) -> str:
        ...
        helper = account_link.AuthorizeAccountHelper(self.hass.data[DOMAIN], self.service)
        ...
        async def await_tokens() -> None:
            ...
            try:
                ...
                _LOGGER.info('Failed to fetch tokens for flow %s: %s', flow_id, err.code)
            else:
                await self.hass.config_entries.flow.async_configure(flow_id=flow_id, user_input=tokens)
    ...
    async def _async_refresh_token(self, token: dict) -> dict:
        """Refresh a token."""
        new_token = await account_link.async_fetch_access_token(self.hass.data[DOMAIN], self.service, token['refresh_token'])
