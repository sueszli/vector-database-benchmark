            ...
            if queue.policy:
                for statement in queue.policy['Statement']:
                    if statement['Effect'] == 'Allow':
                        if 'Principal' in statement and ('*' in statement['Principal'] or ('AWS' in statement['Principal'] and '*' in statement['Principal']['AWS']) or ('CanonicalUser' in statement['Principal'] and '*' in statement['Principal']['CanonicalUser'])):
                            if 'Condition' in statement:
                                if is_account_only_allowed_in_condition(statement['Condition'], sqs_client.audited_account):
