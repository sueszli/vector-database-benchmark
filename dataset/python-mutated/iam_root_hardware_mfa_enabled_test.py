...
class Test_iam_root_hardware_mfa_enabled_test:
    ...
    def test_root_hardware_virtual_mfa_enabled(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_root_hardware_mfa_enabled.iam_root_hardware_mfa_enabled.iam_client', new=IAM(current_audit_info)) as service_client:
            from prowler.providers.aws.services.iam.iam_root_hardware_mfa_enabled.iam_root_hardware_mfa_enabled import iam_root_hardware_mfa_enabled
            ...
            assert search('Root account has a virtual MFA instead of a hardware MFA device enabled.', result[0].status_extended)
    ...
    def test_root_hardware_virtual_hardware_mfa_enabled(self):
        ...
        with mock.patch('prowler.providers.aws.lib.audit_info.audit_info.current_audit_info', new=current_audit_info), mock.patch('prowler.providers.aws.services.iam.iam_root_hardware_mfa_enabled.iam_root_hardware_mfa_enabled.iam_client', new=IAM(current_audit_info)) as service_client:
            ...
            assert search('Root account has a hardware MFA device enabled.', result[0].status_extended)
            assert result[0].resource_id == '<root_account>'
            assert result[0].resource_arn == f'arn:aws:iam::{service_client.audited_account}:root'