...
class KeyboardButton(MutableTelegramObject):
    ...
    **Note:** *request_user* and *request_chat* options will only work in Telegram versions released after 3 February, 2023. Older clients will display *unsupported message*.
    ...
    if TYPE_CHECKING:
        ...
        def __init__(__pydantic__self__, *, text: str, request_user: Optional[KeyboardButtonRequestUser]=None, request_chat: Optional[KeyboardButtonRequestChat]=None, request_contact: Optional[bool]=None, request_location: Optional[bool]=None, request_poll: Optional[KeyboardButtonPollType]=None, web_app: Optional[WebAppInfo]=None, **__pydantic_kwargs: Any) -> None:
            if False:
                return 10
            super().__init__(text=text, request_user=request_user, request_chat=request_chat, request_contact=request_contact, request_location=request_location, request_poll=request_poll, web_app=web_app, **__pydantic_kwargs)