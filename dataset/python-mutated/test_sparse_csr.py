...
from torch.testing._internal.common_utils import TEST_WITH_TORCHINDUCTOR, TEST_WITH_ROCM, TEST_SCIPY, TEST_NUMPY, TEST_MKL, IS_WINDOWS, TestCase, run_tests, load_tests, coalescedonoff, parametrize, subtest, skipIfTorchDynamo, skipIfRocm, IS_FBCODE, IS_REMOTE_GPU
from torch.testing._internal.common_device_type import ops, instantiate_device_type_tests, dtypes, OpDTypes, dtypesIfCUDA, onlyCPU, onlyCUDA, skipCUDAIfNoSparseGeneric, precisionOverride, skipMeta, skipCUDAIf, skipCUDAIfRocm, skipCPUIfNoMklSparse, skipCUDAIfRocmVersionLessThan, largeTensorTest
from torch.testing._internal.common_methods_invocations import op_db, sparse_csr_unary_ufuncs, ReductionOpInfo
from torch.testing._internal.common_cuda import _get_torch_cuda_version, TEST_CUDA
from torch.testing._internal.common_dtype import floating_types, all_types_and_complex_and, floating_and_complex_types, floating_types_and, all_types_and_complex, floating_and_complex_types_and
...
_sparse_compressed_ops = list(filter(lambda op: op.supports_sparse_csr or op.supports_sparse_csc or op.supports_sparse_bsr or op.supports_sparse_bsc, op_db))
...
UNARY_EWISE_CSR_ALLOW_AUTOGRAD = ['abs', 'conj_physical', 'deg2rad', 'neg', 'positive', 'frac', 'nn.functional.relu', 'log1p', 'rad2deg']
