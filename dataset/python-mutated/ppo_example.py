...
FLAGS = flags.FLAGS
flags.DEFINE_string('exp_name', os.path.basename(__file__).rstrip('.py'), 'the name of this experiment')
flags.DEFINE_string('game_name', 'atari', 'the id of the OpenSpiel game')
flags.DEFINE_float('learning_rate', 0.00025, 'the learning rate of the optimizer')
flags.DEFINE_integer('seed', 1, 'seed of the experiment')
flags.DEFINE_integer('total_timesteps', 10000000, 'total timesteps of the experiments')
flags.DEFINE_integer('eval_every', 10, 'evaluate the policy every N updates')
flags.DEFINE_bool('torch_deterministic', True, 'if toggled, `torch.backends.cudnn.deterministic=False`')
flags.DEFINE_bool('cuda', True, 'if toggled, cuda will be enabled by default')
flags.DEFINE_string('gym_id', 'BreakoutNoFrameskip-v4', 'the id of the environment')
flags.DEFINE_bool('capture_video', False, 'whether to capture videos of the agent performances (check out `videos` folder)')
flags.DEFINE_integer('num_envs', 8, 'the number of parallel game environments')
flags.DEFINE_integer('num_steps', 128, 'the number of steps to run in each environment per policy rollout')
flags.DEFINE_bool('anneal_lr', True, 'Toggle learning rate annealing for policy and value networks')
flags.DEFINE_bool('gae', True, 'Use GAE for advantage computation')
flags.DEFINE_float('gamma', 0.99, 'the discount factor gamma')
flags.DEFINE_float('gae_lambda', 0.95, 'the lambda for the general advantage estimation')
...
flags.DEFINE_bool('clip_vloss', True, 'Toggles whether or not to use a clipped loss for the value function, as per the paper')
...
flags.DEFINE_float('max_grad_norm', 0.5, 'the maximum norm for the gradient clipping')
