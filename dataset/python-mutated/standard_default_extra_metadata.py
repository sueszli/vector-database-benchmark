...
def get_files(**kwargs):
    ...
    for f in get_template_files(**kwargs):
        ...
        if f.path.parts[0] != kwargs['package_name']:
            continue
        files.append(f)
    files.extend((File(Path(metadata_directory, 'extra_metadata', 'foo.txt'), ''), File(Path(metadata_directory, 'extra_metadata', 'nested', 'bar.txt'), ''), File(Path(metadata_directory, 'WHEEL'), f'Wheel-Version: 1.0\nGenerator: hatchling {__version__}\nRoot-Is-Purelib: true\nTag: py3-none-any\n'), File(Path(metadata_directory, 'METADATA'), f"Metadata-Version: {DEFAULT_METADATA_VERSION}\nName: {kwargs['project_name']}\nVersion: 0.0.1\nLicense-File: LICENSE.txt\nRequires-Python: >3\n")))
