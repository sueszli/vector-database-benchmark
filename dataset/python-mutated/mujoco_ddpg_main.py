...
def main(cfg, seed=0, max_iterations=int(10000000000.0)):
    if False:
        print('Hello World!')
    cfg = compile_config(cfg, BaseEnvManager, DDPGPolicy, BaseLearner, SampleSerialCollector, InteractionSerialEvaluator, AdvancedReplayBuffer, MujocoEnv, save_cfg=True)
    (collector_env_num, evaluator_env_num) = (cfg.env.collector_env_num, cfg.env.evaluator_env_num)
    collector_env = BaseEnvManager(env_fn=[lambda : MujocoEnv(cfg.env) for _ in range(collector_env_num)], cfg=cfg.env.manager)
    evaluator_env = BaseEnvManager(env_fn=[lambda : MujocoEnv(cfg.env) for _ in range(evaluator_env_num)], cfg=cfg.env.manager)
