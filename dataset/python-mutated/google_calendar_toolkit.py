...
class GoogleCalendarToolKit(BaseToolkit, ABC):
    ...
    def get_env_keys(self) -> List[ToolConfiguration]:
        if False:
            return 10
        return [ToolConfiguration(key='GOOGLE_CLIENT_ID', key_type=ToolConfigKeyType.STRING, is_required=True, is_secret=False), ToolConfiguration(key='GOOGLE_CLIENT_SECRET', key_type=ToolConfigKeyType.STRING, is_required=True, is_secret=True)]