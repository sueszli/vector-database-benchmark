...
def _load_mapping_arg_effect(oparg: int, _jmp: int=0) -> int:
    ...
    if oparg == 2:
        ...
        return -2
    return 1
opcode.stack_effects.update(INVOKE_METHOD=lambda oparg, jmp: -oparg[1], LOAD_FIELD=0, STORE_FIELD=-2, CAST=0, LOAD_LOCAL=1, STORE_LOCAL=-1, PRIMITIVE_BOX=0, POP_JUMP_IF_ZERO=-1, POP_JUMP_IF_NONZERO=-1, PRIMITIVE_UNBOX=0, PRIMITIVE_BINARY_OP=lambda oparg, jmp: -1, PRIMITIVE_UNARY_OP=lambda oparg, jmp: 0, PRIMITIVE_COMPARE_OP=lambda oparg, jmp: -1, LOAD_ITERABLE_ARG=1, LOAD_MAPPING_ARG=_load_mapping_arg_effect, INVOKE_FUNCTION=lambda oparg, jmp=0: -oparg[1] + 1, INVOKE_NATIVE=lambda oparg, jmp=0: -len(oparg[1]) + 2, JUMP_IF_ZERO_OR_POP=lambda oparg, jmp=0: 0 if jmp else -1, JUMP_IF_NONZERO_OR_POP=lambda oparg, jmp=0: 0 if jmp else -1, FAST_LEN=0, CONVERT_PRIMITIVE=0, LOAD_CLASS=1, BUILD_CHECKED_MAP=lambda oparg, jmp: 1 - 2 * oparg[1], SEQUENCE_GET=-1, SEQUENCE_SET=-3, LIST_DEL=-2, REFINE_TYPE=0, PRIMITIVE_LOAD_CONST=1, RETURN_PRIMITIVE=-1, TP_ALLOC=1, BUILD_CHECKED_LIST=lambda oparg, jmp: 1 - oparg[1], LOAD_TYPE=0)