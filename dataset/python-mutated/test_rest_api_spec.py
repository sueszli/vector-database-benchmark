...
from ...test_server.test_rest_api_spec import API_PARAMS_RENAMES, IMPLEMENTED_FEATURES, PARAMS_RENAMES, RUN_ASYNC_REST_API_TESTS, YAML_TEST_SPECS, YamlRunner
...
class AsyncYamlRunner(YamlRunner):
    ...
    async def run_do(self, action):
        ...
        assert len(action) == 1
        if headers and headers.get('Authorization', None) == 'Basic eF9wYWNrX3Jlc3RfdXNlcjp4LXBhY2stdGVzdC1wYXNzd29yZA==':
                ...
                if duplicate_args:
                    for arg in list(duplicate_args):
                        if args['body'][arg] == args[arg]:
                            ...
                            pytest.skip('Contains a duplicate parameter with a different value')
        ...
        with warnings.catch_warnings(record=True) as caught_warnings:
            try:
                ...
                if catch:
                    raise AssertionError(f'Failed to catch {catch!r} in {self.last_response!r}.')
        caught_warnings = [str(w.message) for w in caught_warnings if w.category == ElasticsearchWarning and str(w.message) not in allowed_warnings]
        if warn and sorted(warn) != sorted(caught_warnings):
            raise AssertionError('Expected warnings not equal to actual warnings: expected=%r actual=%r' % (warn, caught_warnings))
    ...
    async def _feature_enabled(self, name):
        ...
        if XPACK_FEATURES is None:
            try:
                ...
                XPACK_FEATURES = set((f for f in xinfo['features'] if xinfo['features'][f]['enabled']))
