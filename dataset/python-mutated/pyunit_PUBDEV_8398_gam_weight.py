...
def link_functions_tweedie_vpow():
    ...
    test = train.drop('W')
    print(train)
    h2o_model = H2OGeneralizedAdditiveEstimator(family='tweedie', gam_columns=['X3'], weights_column='W', lambda_=0, tweedie_variance_power=1.5, bs=[2], tweedie_link_power=0)
