...
class HotlinkCc(SimpleDownloader):
    ...
    __version__ = '0.01'
    __status__ = 'testing'
    __pattern__ = 'https?://hotlink\\.cc/\\w+'
    ...
    NAME_PATTERN = '<i class="glyphicon glyphicon-download"></i>\\s*(?P<N>.+?)<'
    SIZE_PATTERN = 'Filesize <b>(?P<S>[\\d.,]+) (?P<U>[\\w^_]+)<'
    WAIT_PATTERN = '<span class="seconds yellow"><b>(\\d+)<'
    DL_LIMIT_PATTERN = 'You have to wait (.+?) untill the next download'
    OFFLINE_PATTERN = '>File Not Found</'
    LINK_FREE_PATTERN = '<a href="(https?://fs\\d+.hotlink.cc/.+?)"'
