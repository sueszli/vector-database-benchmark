...
class dataqnaCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_question': ('parent', 'question'), 'execute_question': ('name', 'interpretation_index'), 'get_question': ('name', 'read_mask'), 'get_user_feedback': ('name',), 'suggest_queries': ('parent', 'scopes', 'query', 'suggestion_types'), 'update_user_feedback': ('user_feedback', 'update_mask')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=dataqnaCallTransformer()):
    if False:
        while True:
            i = 10
