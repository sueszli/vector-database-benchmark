...
if __name__ == '__main__':
    ...
    session.add(loris)
    q = session.query(Animal).filter(Animal.facts.any(and_(AnimalFact.key == 'color', AnimalFact.value == 'reddish')))
    print('reddish animals', q.all())
    q = session.query(Animal).filter(Animal.with_characteristic('color', 'brown'))
    print('brown animals', q.all())
    q = session.query(Animal).filter(~Animal.with_characteristic('poisonous-part', 'elbows'))
