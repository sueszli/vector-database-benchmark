...
EventSpan = namedtuple('EventSpan', ['span_id', 'is_segment', 'parent_span_id', 'group', 'group_raw', 'description', 'op', 'start_timestamp', 'start_ms', 'duration', 'exclusive_time'])

class EventSpans(TypedDict):
    transaction_id: str
    spans: List[EventSpan]
AggregateSpanRow = TypedDict('AggregateSpanRow', {'node_fingerprint': str, 'parent_node_fingerprint': str, 'group': str, 'description': str, 'op': str, 'start_timestamp': int, 'start_ms': int, 'avg(exclusive_time)': float, 'avg(duration)': float, 'is_segment': int, 'avg(absolute_offset)': float, 'avg(relative_offset)': float, 'count()': int, 'samples': Set[Tuple[Optional[str], str]]})
NULL_GROUP = '00'
