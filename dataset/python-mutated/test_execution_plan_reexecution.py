...
from dagster._core.errors import DagsterExecutionStepNotFoundError, DagsterInvariantViolationError, DagsterRunNotFoundError
from dagster._core.events import get_step_output_event
from dagster._core.execution.api import ReexecutionOptions, create_execution_plan, execute_job, execute_plan
...
def add_three(num):
    if False:
        for i in range(10):
            print('nop')
    return num + 3
addy_graph = GraphDefinition(name='execution_plan_reexecution', node_defs=[add_one, add_two, add_three], dependencies={'add_two': {'num': DependencyDefinition('add_one')}, 'add_three': {'num': DependencyDefinition('add_two')}})
