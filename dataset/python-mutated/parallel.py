...
from airflow_breeze.utils.shared_options import get_dry_run, get_forced_answer, get_verbose, set_dry_run, set_forced_answer, set_verbose
MAX_LINE_LENGTH = 155
...
ANSI_COLOUR_MATCHER = re.compile('(?:\\x1B[@-_]|[\\x80-\\x9F])[0-?]*[ -/]*[@-~]')
...
class DockerBuildxProgressMatcher(AbstractProgressInfoMatcher):
    DOCKER_BUILDX_PROGRESS_MATCHER = re.compile('\\s*#(\\d*) ')
...
class GenericRegexpProgressMatcher(AbstractProgressInfoMatcher):
    ...
    def get_best_matching_lines(self, output: Output) -> list[str] | None:
        ...
        if last_match is None:
            return None
        return [last_match]
DOCKER_PULL_PROGRESS_REGEXP = '^[0-9a-f]+: .*|.*\\[[ \\d%]*\\].*|^Waiting'
...
def get_multi_tuple_array(title: str, tuples: list[tuple[NamedTuple, ...]]) -> Table:
    ...
    for t in tuples:
        ...
        table.add_row(*row, style='special')
    return table
IGNORED_FSTYPES = ['autofs', 'bps', 'cgroup', 'cgroup2', 'configfs', 'debugfs', 'devpts', 'fusectl', 'mqueue', 'nsfs', 'overlay', 'proc', 'pstore', 'squashfs', 'tracefs']
