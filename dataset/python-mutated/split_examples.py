...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_filepath', help='Path to the label file', required=True)
    parser.add_argument('--train_proportion', help='Proportion of examples to go in training set', type=float, required=True)
    parser.add_argument('--num_negatives_set', help='Number of negative samples to include in a negative condidate set for hard negative mining', type=float, default=0)
    parser.add_argument('--num_negatives_test', help='Number of negative samples to reserve for negative test set for hard negative mining', type=float, default=0)
    parser.add_argument('--negative_label_id', help='Label of the negative class if applicable')
    args = parser.parse_args()
    main(label_filepath=args.label_filepath, train_proportion=args.train_proportion, num_negatives_set=args.num_negatives_set, num_negatives_test=args.num_negatives_test, negative_label_id=args.negative_label_id)