...
class TestAsyncWebhook_HttpRetries(unittest.TestCase):
    ...
    async def test_send(self):
        ...
        client = AsyncWebhookClient('http://localhost:8888/remote_disconnected', retry_handlers=[retry_handler])
        try:
    ...
    async def test_ratelimited(self):
        client = AsyncWebhookClient('http://localhost:8888/ratelimited', retry_handlers=[AsyncRateLimitErrorRetryHandler()])
