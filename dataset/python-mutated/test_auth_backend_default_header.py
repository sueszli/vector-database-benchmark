...
from ..jwt import JWT_ACCESS_TYPE, create_access_token, create_access_token_for_app, create_refresh_token, jwt_encode, jwt_user_payload
...
def test_use_default_header_as_a_fallback(rf, staff_user, customer_user):
    ...
    request = rf.request(HTTP_AUTHORIZATION_BEARER='', HTTP_AUTHORIZATION=f'JWT {customer_access_token}')
...
def test_owner_field_is_missing(prefix, rf, staff_user, settings):
    if False:
        for i in range(10):
            print('nop')
    payload = jwt_user_payload(staff_user, JWT_ACCESS_TYPE, settings.JWT_TTL_ACCESS, token_owner=None)
    token = jwt_encode(payload)
    request = rf.request(HTTP_AUTHORIZATION=f'{prefix} {token}')
    backend = JSONWebTokenBackend()
    assert backend.authenticate(request) is None
