...
from tests.testing_utils import IS_WINDOWS, RUNNING_ON_CI, RUNNING_TEST_FOR_MASTER_ON_CI, RUN_BY_CANARY, CI_OVERRIDE, run_command, SKIP_DOCKER_TESTS, SKIP_DOCKER_BUILD, SKIP_DOCKER_MESSAGE, run_command_with_input, UpdatableSARTemplate
from .build_integ_base import BuildIntegBase, DedupBuildIntegBase, CachedBuildIntegBase, BuildIntegRubyBase, NestedBuildIntegBase, IntrinsicIntegBase, BuildIntegNodeBase, BuildIntegGoBase, BuildIntegProvidedBase, BuildIntegPythonBase, BuildIntegJavaBase, BuildIntegEsbuildBase
...
@skipIf(not RUN_BY_CANARY and (not CI_OVERRIDE), 'Skip build tests on windows when running in CI unless overridden')
@pytest.mark.python
...
@skipIf(not RUN_BY_CANARY or ((IS_WINDOWS and RUNNING_ON_CI) and (not CI_OVERRIDE)), 'Skip build tests on windows when running in CI unless overridden')
...
@parameterized_class(('template', 'FUNCTION_LOGICAL_ID', 'overrides', 'runtime', 'codeuri', 'check_function_only', 'prop'), [('template.yaml', 'Function', True, 'python3.7', 'Python', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.8', 'Python', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.9', 'Python', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.10', 'Python', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.11', 'Python', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.7', 'PythonPEP600', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.8', 'PythonPEP600', False, 'CodeUri')])
...
@parameterized_class(('template', 'FUNCTION_LOGICAL_ID', 'overrides', 'runtime', 'codeuri', 'check_function_only', 'prop'), [('template.yaml', 'Function', True, 'python3.7', 'Python', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.8', 'Python', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.9', 'Python', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.10', 'Python', False, 'CodeUri'), ('template.yaml', 'Function', True, 'python3.11', 'Python', False, 'CodeUri')])
...
@skipIf(SKIP_DOCKER_TESTS, 'Skip build tests that requires Docker in CI environment')
@parameterized_class(('template', 'FUNCTION_LOGICAL_ID', 'overrides', 'runtime', 'codeuri', 'use_container', 'check_function_only', 'prop'), [('cdk_v1_synthesized_template_zip_image_functions.json', 'RandomCitiesFunction5C47A2B8', False, None, None, False, True, 'Code')])
...
@skipIf(SKIP_DOCKER_TESTS, 'Skip build tests that requires Docker in CI environment')
@parameterized_class(('template', 'FUNCTION_LOGICAL_ID', 'overrides', 'use_container', 'prop'), [('cdk_v2_synthesized_template_image_function_shared_code.json', 'TestLambdaFunctionC089708A', False, False, 'Code.ImageUri')])
...
@skipIf((IS_WINDOWS and RUNNING_ON_CI) and (not CI_OVERRIDE), 'Skip build tests on windows when running in CI unless overridden')
@parameterized_class(('template',), [('esbuild_templates/template_with_metadata_node_options.yaml',), ('esbuild_templates/template_with_metadata_global_node_options.yaml',)])
...
class TestBuildCommand_Java(BuildIntegJavaBase):
    ...
    EXPECTED_MAVEN_DEPENDENCIES = {'software.amazon.awssdk.annotations-2.1.0.jar', 'com.amazonaws.aws-lambda-java-core-1.1.0.jar'}
...
class TestBuildCommand_Dotnet_cli_package(BuildIntegBase):
    FUNCTION_LOGICAL_ID = 'Function'
    EXPECTED_FILES_PROJECT_MANIFEST = {'Amazon.Lambda.APIGatewayEvents.dll', 'Amazon.Lambda.Core.dll', 'HelloWorld.runtimeconfig.json', 'Amazon.Lambda.Serialization.Json.dll', 'Newtonsoft.Json.dll', 'HelloWorld.deps.json', 'HelloWorld.dll'}
    EXPECTED_FILES_PROJECT_MANIFEST_PROVIDED = {'bootstrap'}
...
class TestBuildCommand_SingleFunctionBuilds(BuildIntegBase):
    ...
    EXPECTED_FILES_PROJECT_MANIFEST = {'__init__.py', 'main.py', 'numpy', 'requirements.txt'}
...
@parameterized_class(('template', 'is_nested_parent'), [(os.path.join('nested-parent', 'template-parent.yaml'), 'is_nested_parent'), ('template.yaml', False)])
...
@skipIf(SKIP_DOCKER_TESTS or SKIP_DOCKER_BUILD, 'Skip build tests on windows when running in CI unless overridden')
...
@parameterized_class(('template', 'use_base_dir'), [(os.path.join('deep-nested', 'template.yaml'), False), (os.path.join('base-dir', 'template', 'template.yaml'), 'use_base_dir')])
...
@skipIf(SKIP_DOCKER_TESTS or SKIP_DOCKER_BUILD, SKIP_DOCKER_MESSAGE)
@parameterized_class(('template', 'use_base_dir'), [(os.path.join('nested-parent', 'nested-root-template-image.yaml'), False), (os.path.join('base-dir-image', 'template', 'template.yaml'), 'use_base_dir')])
...
@parameterized_class(('template', 'stack_paths', 'layer_full_path', 'function_full_paths', 'invoke_error_message'), [(os.path.join('nested-with-intrinsic-functions', 'template-pass-down.yaml'), ['', 'AppUsingRef', 'AppUsingJoin'], 'MyLayerVersion', ['AppUsingRef/FunctionInChild', 'AppUsingJoin/FunctionInChild'], 'Invalid Layer Arn'), (os.path.join('nested-with-intrinsic-functions', 'template-pass-up.yaml'), ['', 'ChildApp'], 'ChildApp/MyLayerVersion', ['FunctionInRoot'], None)])
