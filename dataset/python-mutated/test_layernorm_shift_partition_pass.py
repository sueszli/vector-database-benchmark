...
class TestLayernormShiftPartitionPass(PassAutoScanTest):
    ...
    def sample_predictor_configs(self, program_config):
        ...
        config.set_trt_dynamic_shape_info({'input_data': [1, 9, 96]}, {'input_data': [4, 3136, 768]}, {'input_data': [1, 784, 384]})
        yield (config, ['layernorm_shift_partition'], (1e-05, 1e-05))
        config = self.create_trt_inference_config()
        ...
        config.set_trt_dynamic_shape_info({'input_data': [1, 9, 96]}, {'input_data': [4, 3136, 768]}, {'input_data': [1, 784, 384]})
        yield (config, ['layernorm_shift_partition'], (0.001, 0.001))
    ...
    def sample_program_config(self, draw):
        ...
        program_config = ProgramConfig(ops=[layer_norm_op, reshape_op2, reshape_op3, transpose_op4, reshape_op5, reshape_op6], weights={'layer_norm_bias': TensorConfig(data_gen=partial(generate_weight, attrs)), 'layer_norm_scale': TensorConfig(data_gen=partial(generate_weight, attrs))}, inputs={'input_data': TensorConfig(data_gen=partial(generate_input, attrs))}, outputs=['reshape_output6'])
    ...
    def test(self):
        if False:
            while True:
                i = 10
        self.run_and_statis(quant=False, max_examples=50, passes=['layernorm_shift_partition_fuse_pass'], max_duration=250, min_success_num=50)
...
class TestLayernormShiftPartition2Pass(PassAutoScanTest):
    ...
    def sample_program_config(self, draw):
        ...
        layer_norm_op = OpConfig(type='layer_norm', inputs={'X': ['input_data'], 'Bias': ['layer_norm_bias'], 'Scale': ['layer_norm_scale']}, outputs={'Y': ['layer_norm_output1'], 'Mean': ['layer_norm_output2'], 'Variance': ['layer_norm_output3']}, attrs={'begin_norm_axis': attrs[0]['begin_norm_axis'], 'epsilon': attrs[0]['epsilon']})
        reshape_op2 = OpConfig(type='reshape2', inputs={'X': ['layer_norm_output1']}, outputs={'Out': ['reshape_output2'], 'XShape': ['reshape_output2_xshape']}, attrs={'shape': [-1, attrs[2]['input_resolution'], attrs[2]['input_resolution'], attrs[1]['input_dim'][-1]]})
        roll_op1 = OpConfig(type='roll', inputs={'X': ['reshape_output2']}, outputs={'Out': ['roll_output1']}, attrs={'axis': [1, 2], 'shifts': [-math.floor(attrs[2]['window_size'] / 2.0), -math.floor(attrs[2]['window_size'] / 2.0)]})
        reshape_op3 = OpConfig(type='reshape2', inputs={'X': ['roll_output1']}, outputs={'Out': ['reshape_output3'], 'XShape': ['reshape_output3_xshape']}, attrs={'shape': [-1, attrs[2]['move_shape'], attrs[2]['window_size'], attrs[2]['move_shape'], attrs[2]['window_size'], attrs[1]['input_dim'][-1]]})
        transpose_op4 = OpConfig(type='transpose2', inputs={'X': ['reshape_output3']}, outputs={'Out': ['transpose_output4']}, attrs={'axis': attrs[2]['axis']})
        reshape_op5 = OpConfig(type='reshape2', inputs={'X': ['transpose_output4']}, outputs={'Out': ['reshape_output5'], 'XShape': ['reshape_output5_xshape']}, attrs={'shape': [-1, attrs[2]['window_size'], attrs[2]['window_size'], attrs[1]['input_dim'][-1]]})
        reshape_op6 = OpConfig(type='reshape2', inputs={'X': ['reshape_output5']}, outputs={'Out': ['reshape_output6'], 'XShape': ['reshape_output6_xshape']}, attrs={'shape': [-1, attrs[2]['window_size'] ** 2, attrs[1]['input_dim'][-1]]})
        program_config = ProgramConfig(ops=[layer_norm_op, reshape_op2, roll_op1, reshape_op3, transpose_op4, reshape_op5, reshape_op6], weights={'layer_norm_bias': TensorConfig(data_gen=partial(generate_weight, attrs)), 'layer_norm_scale': TensorConfig(data_gen=partial(generate_weight, attrs))}, inputs={'input_data': TensorConfig(data_gen=partial(generate_input, attrs))}, outputs=['reshape_output6'])
        return program_config
