...
if __name__ == '__main__':
    ...
    for sys_platform in ['darwin', 'linux', 'win32']:
        for py_version in ray_constants.RUNTIME_ENV_CONDA_PY_VERSIONS:
            if 'dev' in ray.__version__:
                url = get_master_wheel_url(ray_commit=ray.__commit__, sys_platform=sys_platform, ray_version=ray.__version__, py_version=py_version)
            else:
                url = get_release_wheel_url(ray_commit=ray.__commit__, sys_platform=sys_platform, ray_version=ray.__version__, py_version=py_version)
    ...
    while retry and num_retries < MAX_NUM_RETRIES:
        print(f'There are {len(retry)} URLs to retry. Sleeping 10 minutes to give some time for wheels to be built. Trial {num_retries + 1}/{MAX_NUM_RETRIES}.')
