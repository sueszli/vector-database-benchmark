...
class FastshareCz(SimpleDownloader):
    ...
    __version__ = '0.45'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?fastshare\\.cz/\\d+/.+'
    ...
    __license__ = 'GPLv3'
    __authors__ = [('Walter Purcaro', 'vuolter@gmail.com'), ('GammaC0de', 'nitzo2001[AT]yahoo[DOT]com'), ('ondrej', 'git@ondrej.it')]
    URL_REPLACEMENTS = [('#.*', '')]
    COOKIES = [('fastshare.cz', 'lang', 'en')]
    NAME_PATTERN = '<h2 title="(.+?)" class="section_title'
    SIZE_PATTERN = '<i class="fa fa-bars"></i> (?P<S>\\d+)&nbsp;(?P<U>[\\w^_]+)'
    TEMP_OFFLINE_PATTERN = '[^\\w](503\\s|[Mm]aint(e|ai)nance|[Tt]emp([.-]|orarily))'
    OFFLINE_PATTERN = '>(The file has been deleted|Requested page not found|This file is no longer available)'
    LINK_FREE_PATTERN = 'form .*target="iframe_dwn" .*action=([^>]+)'
    LINK_PREMIUM_PATTERN = '(https?://\\w+\\.fastshare\\.cz/download\\.php\\?id=\\d+&)'
