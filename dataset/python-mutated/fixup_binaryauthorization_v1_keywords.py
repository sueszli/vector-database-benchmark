...
class binaryauthorizationCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_attestor': ('parent', 'attestor_id', 'attestor'), 'delete_attestor': ('name',), 'get_attestor': ('name',), 'get_policy': ('name',), 'get_system_policy': ('name',), 'list_attestors': ('parent', 'page_size', 'page_token'), 'update_attestor': ('attestor',), 'update_policy': ('policy',), 'validate_attestation_occurrence': ('attestor', 'attestation', 'occurrence_note', 'occurrence_resource_uri')}
