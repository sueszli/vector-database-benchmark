...
def h2o_H2OFrame_sum():
    ...
    assert pyunit_utils.equal_two_arrays(numpsum, h2omean.values.tolist()[0], 1e-12, 1e-06), 'h2o.H2OFrame.meansum() command is not working.'
    h2oSum = h2oframe.sum(skipna=False, axis=1)
    assert_is_type(h2oSum, H2OFrame)
    numpsum = list(np.sum(python_lists, axis=1))
    h2omean = h2oSum.as_data_frame(use_pandas=True, header=False)
    assert pyunit_utils.equal_two_arrays(numpsum, h2omean.values.T.tolist()[0], 1e-12, 1e-06), 'h2o.H2OFrame.meansum() command is not working.'
