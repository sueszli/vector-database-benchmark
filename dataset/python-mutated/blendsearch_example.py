...
def run_blendsearch_tune_w_budget(time_budget_s=10):
    ...
    algo = BlendSearch(metric='mean_loss', mode='min', space={'width': tune.uniform(0, 20), 'height': tune.uniform(-100, 100), 'activation': tune.choice(['relu', 'tanh'])})
    algo.set_search_properties(config={'time_budget_s': time_budget_s})
    algo = ConcurrencyLimiter(algo, max_concurrent=4)
    scheduler = AsyncHyperBandScheduler()
    tuner = tune.Tuner(easy_objective, tune_config=tune.TuneConfig(metric='mean_loss', mode='min', search_alg=algo, scheduler=scheduler, time_budget_s=time_budget_s, num_samples=-1), param_space={'steps': 100})
    results = tuner.fit()
    print('Best hyperparameters found were: ', results.get_best_result().config)
