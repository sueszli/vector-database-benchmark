...
class SuppressionCollector(VisitorWithPositionData):
    suppression_regexes: Dict[SuppressionKind, str] = {SuppressionKind.PYRE_FIXME: '.*# *pyre-fixme(\\[(\\d* *,? *)*\\])?', SuppressionKind.PYRE_IGNORE: '.*# *pyre-ignore(\\[(\\d* *,? *)*\\])?', SuppressionKind.TYPE_IGNORE: '.*# *type: ignore'}
...
class ModuleModeCollector(VisitorWithPositionData):
    unsafe_regex: Pattern[str] = compile(' ?#+ *pyre-unsafe')
    strict_regex: Pattern[str] = compile(' ?#+ *pyre-strict')
    ignore_all_regex: Pattern[str] = compile(' ?#+ *pyre-ignore-all-errors')
    ignore_all_by_code_regex: Pattern[str] = compile(' ?#+ *pyre-ignore-all-errors\\[[0-9]+[0-9, ]*\\]')
