...
NEW_TYPES_OUTPUT_SCHEMA = '{"fields": [{"name": "bytes","type": "BYTES"},{"name": "date","type": "DATE"},{"name": "time","type": "TIME"}]}'
NEW_TYPES_OUTPUT_VERIFY_QUERY = 'SELECT bytes, date, time FROM `%s`;'
NEW_TYPES_OUTPUT_EXPECTED = [(b'xyw', datetime.date(2011, 1, 1), datetime.time(23, 59, 59, 999999)), (b'abc', datetime.date(2000, 1, 1), datetime.time(0, 0)), (b'\xe4\xbd\xa0\xe5\xa5\xbd', datetime.date(3000, 12, 31), datetime.time(23, 59, 59, 990000)), (b'\xab\xac\xad', datetime.date(2000, 1, 1), datetime.time(0, 0))]
LEGACY_QUERY = 'SELECT * FROM (SELECT "apple" as fruit), (SELECT "orange" as fruit),'
STANDARD_QUERY = 'SELECT * FROM (SELECT "apple" as fruit) UNION ALL (SELECT "orange" as fruit)'
