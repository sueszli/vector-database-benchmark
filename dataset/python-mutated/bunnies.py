...
ax = plt.subplot(2, 2, 1, xlim=[-1, +1], ylim=[-1, +1], xticks=[], yticks=[], aspect=1, frameon=False)
model = xrotate(20) @ yrotate(45)
view = translate(0, 0, -2.5)
proj = perspective(25, 1, 1, 100)
ax.add_collection(mesh(proj @ view @ model, V, F, 'magma', False))
ax = plt.subplot(2, 2, 2, xlim=[-1, +1], ylim=[-1, +1], xticks=[], yticks=[], aspect=1, frameon=False)
model = xrotate(90)
view = np.eye(4)
proj = ortho(-1, 1, -1, 1, 0, 100)
ax.add_collection(mesh(proj @ view @ model, 2 * V, F))
ax = plt.subplot(2, 2, 3, xlim=[-1, +1], ylim=[-1, +1], xticks=[], yticks=[], aspect=1, frameon=False)
model = yrotate(90)
view = np.eye(4)
proj = ortho(-1, 1, -1, 1, 0, 100)
ax.add_collection(mesh(proj @ view @ model, 2 * V, F))
ax = plt.subplot(2, 2, 4, xlim=[-1, +1], ylim=[-1, +1], xticks=[], yticks=[], aspect=1, frameon=False)
model = view = np.eye(4)
proj = ortho(-1, 1, -1, 1, 0, 100)
ax.add_collection(mesh(proj @ view @ model, 2 * V, F))
plt.tight_layout()
