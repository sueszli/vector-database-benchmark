...
class SearchEndpoint(MetadataEndpointBase):
    ...
    @docs(tags=['Metadata'], summary='Perform a search for a given query.', responses={200: {'schema': schema(SearchResponse={'results': [MetadataSchema], 'first': Integer(), 'last': Integer(), 'sort_by': String(), 'sort_desc': Integer(), 'total': Integer()})}})
    ...
    async def search(self, request):
        ...
        try:
            with db_session:
                if tags:
                    infohash_set = self.tribler_db.knowledge.get_subjects_intersection(subjects_type=ResourceType.TORRENT, objects=set(tags), predicate=ResourceType.TAG, case_sensitive=False)
                    if infohash_set:
        ...
        if sanitized['first'] == 1:
            search_results = self.build_snippets(search_results)
        response_dict = {'results': search_results, 'first': sanitized['first'], 'last': sanitized['last'], 'sort_by': sanitized['sort_by'], 'sort_desc': sanitized['sort_desc']}
        if include_total:
            response_dict.update(total=total, max_rowid=max_rowid)
        return RESTResponse(response_dict)

    @docs(tags=['Metadata'], summary='Return auto-completion suggestions for a given query.', parameters=[{'in': 'query', 'name': 'q', 'description': 'Search query', 'type': 'string', 'required': True}], responses={200: {'schema': schema(CompletionsResponse={'completions': [String]}), 'examples': {'completions': ['pioneer one', 'pioneer movie']}}})
