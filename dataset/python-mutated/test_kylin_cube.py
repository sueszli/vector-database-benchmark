...
class TestKylinCubeOperator:
    _config = {'kylin_conn_id': 'kylin_default', 'project': 'learn_kylin', 'cube': 'kylin_sales_cube', 'command': 'build', 'start_time': str(int(datetime(2012, 1, 2, 0, 0).timestamp() * 1000)), 'end_time': str(int(datetime(2012, 1, 3, 0, 0).timestamp() * 1000))}
    cube_command = ['fullbuild', 'build', 'merge', 'refresh', 'delete', 'build_streaming', 'merge_streaming', 'refresh_streaming', 'disable', 'enable', 'purge', 'clone', 'drop']
