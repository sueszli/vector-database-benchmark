...
class HaveIBeenPwned(classes.ObservableAnalyzer):
    ...
    def run(self):
        if False:
            print('Hello World!')
        params = {'truncateResponse': self.truncate_response, 'includeUnverified': self.include_unverified}
        if self.domain:
            params['domain'] = self.domain
        headers = {'hibp-api-key': self._api_key_name}
        try:
            response = requests.get(self.base_url + self.observable_name, params=params, headers=headers)
