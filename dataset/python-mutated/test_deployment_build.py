...
async def ensure_default_agent_pool_exists(session):
    ...
    if default_work_pool is None:
        default_work_pool = await models.workers.create_work_pool(session=session, work_pool=schemas.actions.WorkPoolCreate(name=models.workers.DEFAULT_AGENT_WORK_POOL_NAME, type='prefect-agent'))
...
class TestWorkPool:
    ...
    async def test_creates_work_queue_in_work_pool(self, patch_import, tmp_path, work_pool, session):
        await run_sync_in_worker_thread(invoke_and_assert, ['deployment', 'build', 'fake-path.py:fn', '-n', 'TEST', '-p', work_pool.name, '-q', 'new-queue', '-o', str(tmp_path / 'test.yaml'), '--apply'], expected_code=0, temp_dir=tmp_path)
        ...
        work_queue = await models.workers.read_work_queue_by_name(session=session, work_pool_name=work_pool.name, work_queue_name='new-queue')
...
class TestWorkQueueConcurrency:
    ...
    async def test_setting_work_queue_concurrency_limits_with_build(self, built_deployment_with_queue_and_limit_overrides, prefect_client):
        queue = await prefect_client.read_work_queue_by_name('the-queue-to-end-all-queues')
        assert queue.concurrency_limit == 424242

    async def test_setting_work_queue_concurrency_limits_with_apply(self, applied_deployment_with_queue_and_limit_overrides, prefect_client):
