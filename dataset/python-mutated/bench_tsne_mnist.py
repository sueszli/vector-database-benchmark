...
if __name__ == '__main__':
    ...
    parser.add_argument('--bhtsne', action='store_true', help='if set and the reference bhtsne code is correctly installed, run it in the benchmark.')
    parser.add_argument('--all', action='store_true', help='if set, run the benchmark with the whole MNIST.dataset. Note that it will take up to 1 hour.')
    parser.add_argument('--profile', action='store_true', help='if set, run the benchmark with a memory profiler.')
    parser.add_argument('--verbose', type=int, default=0)
    parser.add_argument('--pca-components', type=int, default=50, help='Number of principal components for preprocessing.')
    ...
    if args.profile:
        try:
            ...
            raise ImportError('To run the benchmark with `--profile`, you need to install `memory_profiler`. Please run `pip install memory_profiler`.') from e
    ...
    for n in data_size:
        ...
        for (name, method) in methods:
            ...
            print('Fitting {} on {} samples took {:.3f}s in {:d} iterations, nn accuracy: {:0.3f}'.format(name, n, duration, n_iter, precision_5))
