...
async def animation(bot, chat_id):
    with data_file('game.gif').open('rb') as f, data_file('thumb.jpg').open('rb') as thumb:
        return (await bot.send_animation(chat_id, animation=f, read_timeout=50, thumbnail=thumb)).animation
    ...
    async def test_send_animation_local_files(self, monkeypatch, bot, chat_id, local_mode):
        try:
            ...
            async def make_assertion(_, data, *args, **kwargs):
                nonlocal test_flag
                if local_mode:
                    test_flag = data.get('animation') == expected and data.get('thumbnail') == expected
                else:
                    test_flag = isinstance(data.get('animation'), InputFile) and isinstance(data.get('thumbnail'), InputFile)
...
class TestAnimationWithRequest(TestAnimationBase):
    ...
    async def test_send_all_args(self, bot, chat_id, animation_file, animation, thumb_file):
        message = await bot.send_animation(chat_id, animation_file, duration=self.duration, width=self.width, height=self.height, caption=self.caption, parse_mode='Markdown', disable_notification=False, protect_content=True, thumbnail=thumb_file, has_spoiler=True)
    ...
    async def test_send_animation_url_file(self, bot, chat_id, animation):
        message = await bot.send_animation(chat_id=chat_id, animation=self.animation_file_url, caption=self.caption)
        ...
        assert message.animation.file_name.startswith('game.gif') == animation.file_name.startswith('game.gif')
        assert message.animation.mime_type == animation.mime_type
    ...
    async def test_send_animation_caption_entities(self, bot, chat_id, animation):
        ...
        message = await bot.send_animation(chat_id, animation, caption=test_string, caption_entities=entities)
    ...
    async def test_send_animation_default_parse_mode_1(self, default_bot, chat_id, animation_file):
        ...
        message = await default_bot.send_animation(chat_id, animation_file, caption=test_markdown_string)
    ...
    async def test_send_animation_default_parse_mode_2(self, default_bot, chat_id, animation_file):
        test_markdown_string = '_Italic_ *Bold* `Code`'
        message = await default_bot.send_animation(chat_id, animation_file, caption=test_markdown_string, parse_mode=None)
    ...
    async def test_send_animation_default_parse_mode_3(self, default_bot, chat_id, animation_file):
        test_markdown_string = '_Italic_ *Bold* `Code`'
        message = await default_bot.send_animation(chat_id, animation_file, caption=test_markdown_string, parse_mode='HTML')
        assert message.caption == test_markdown_string
        assert message.caption_markdown == escape_markdown(test_markdown_string)
    ...
    async def test_send_animation_default_allow_sending_without_reply(self, default_bot, chat_id, animation, custom):
        reply_to_message = await default_bot.send_message(chat_id, 'test')
        await reply_to_message.delete()
        if custom is not None:
            message = await default_bot.send_animation(chat_id, animation, allow_sending_without_reply=custom, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        elif default_bot.defaults.allow_sending_without_reply:
            message = await default_bot.send_animation(chat_id, animation, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        else:
            with pytest.raises(BadRequest, match='Message to reply not found'):
                await default_bot.send_animation(chat_id, animation, reply_to_message_id=reply_to_message.message_id)

    @pytest.mark.parametrize('default_bot', [{'protect_content': True}], indirect=True)
    async def test_send_animation_default_protect_content(self, default_bot, chat_id, animation):
        tasks = asyncio.gather(default_bot.send_animation(chat_id, animation), default_bot.send_animation(chat_id, animation, protect_content=False))
