...
class TestContactWithoutRequest(TestContactBase):
    ...
    async def test_send_mutually_exclusive(self, bot, chat_id, contact):
        with pytest.raises(ValueError, match='Not both'):
            await bot.send_contact(chat_id=chat_id, contact=contact, phone_number=contact.phone_number, first_name=contact.first_name)
...
class TestContactWithRequest(TestContactBase):
    ...
    async def test_send_contact_default_allow_sending_without_reply(self, default_bot, chat_id, contact, custom):
        reply_to_message = await default_bot.send_message(chat_id, 'test')
        await reply_to_message.delete()
        if custom is not None:
            message = await default_bot.send_contact(chat_id, contact=contact, allow_sending_without_reply=custom, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        elif default_bot.defaults.allow_sending_without_reply:
            message = await default_bot.send_contact(chat_id, contact=contact, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        else:
            with pytest.raises(BadRequest, match='Message to reply not found'):
                await default_bot.send_contact(chat_id, contact=contact, reply_to_message_id=reply_to_message.message_id)

    @pytest.mark.parametrize('default_bot', [{'protect_content': True}], indirect=True)
    async def test_send_contact_default_protect_content(self, chat_id, default_bot, contact):
        tasks = asyncio.gather(default_bot.send_contact(chat_id, contact=contact), default_bot.send_contact(chat_id, contact=contact, protect_content=False))
