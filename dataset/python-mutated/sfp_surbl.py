...
class sfp_surbl(SpiderFootPlugin):
    meta = {'name': 'SURBL', 'summary': 'Check if a netblock, IP address or domain is in the SURBL blacklist.', 'flags': [], 'useCases': ['Investigate', 'Passive'], 'categories': ['Reputation Systems'], 'dataSource': {'website': 'http://www.surbl.org/', 'model': 'FREE_NOAUTH_UNLIMITED', 'references': ['http://www.surbl.org/lists', 'http://www.surbl.org/guidelines'], 'logo': 'http://www.surbl.org/images/logo.png', 'description': 'SURBLs are lists of web sites that have appeared in unsolicited messages. Unlike most lists, SURBLs are not lists of message senders.'}}
    opts = {'checkaffiliates': True, 'checkcohosts': True, 'netblocklookup': True, 'maxnetblock': 24, 'subnetlookup': True, 'maxsubnet': 24}
    optdescs = {'checkaffiliates': 'Apply checks to affiliates?', 'checkcohosts': "Apply checks to sites found to be co-hosted on the target's IP?", 'netblocklookup': 'Look up all IPs on netblocks deemed to be owned by your target for possible blacklisted hosts on the same target subdomain/domain?', 'maxnetblock': 'If looking up owned netblocks, the maximum netblock size to look up all IPs within (CIDR value, 24 = /24, 16 = /16, etc.)', 'subnetlookup': 'Look up all IPs on subnets which your target is a part of for blacklisting?', 'maxsubnet': 'If looking up subnets, the maximum subnet size to look up all the IPs within (CIDR value, 24 = /24, 16 = /16, etc.)'}
    results = None
    errorState = False
    ...
    def watchedEvents(self):
        if False:
            print('Hello World!')
        return ['IP_ADDRESS', 'AFFILIATE_IPADDR', 'NETBLOCK_OWNER', 'NETBLOCK_MEMBER', 'INTERNET_NAME', 'AFFILIATE_INTERNET_NAME', 'CO_HOSTED_SITE']
