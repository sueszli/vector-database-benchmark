...
@pluginmatcher(re.compile('https?://network\\.wwe\\.com/(?:video|live)/(?P<stream_id>\\d+)'))
@pluginargument('email', required=True, requires=['password'], metavar='EMAIL', help='The email associated with your WWE Network account, required to access any WWE Network stream.')
@pluginargument('password', required=True, sensitive=True, metavar='PASSWORD', help='A WWE Network account password to use with --wwenetwork-email.')
class WWENetwork(Plugin):
    site_config_re = re.compile('">window.__data = (\\{.*?\\})</script>')
