...
class AppInstallInput(BaseInputObjectType):
    ...
    activate_after_installation = graphene.Boolean(default_value=True, required=False, description='Determine if app will be set active or not.')
    permissions = NonNullList(PermissionEnum, description='List of permission code names to assign to this app.')

    class Meta:
        doc_category = DOC_CATEGORY_APPS
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.APP_INSTALLED, description='An app was installed.')]

class AppInstall(ModelMutation):

    class Arguments:
        input = AppInstallInput(required=True, description='Fields required to install a new app.')
