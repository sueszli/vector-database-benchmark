...
def training_model(model: torch.nn.Module, optimizer: Optimizer, training_step: Callable, scheduler: SCHEDULER | None=None, max_steps: int | None=None, max_epochs: int | None=None):
    if False:
        while True:
            i = 10
    model.train()
...
optimizer = nni.trace(SGD)(model.parameters(), lr=0.01, momentum=0.9, weight_decay=0.0005)
evaluator = TorchEvaluator(training_model, optimizer, training_step)
config_list = [{'op_names': ['conv1', 'conv2', 'fc1', 'fc2'], 'target_names': ['_input_', 'weight', '_output_'], 'quant_dtype': 'int8', 'quant_scheme': 'affine', 'granularity': 'default'}, {'op_names': ['relu1', 'relu2', 'relu3'], 'target_names': ['_output_'], 'quant_dtype': 'int8', 'quant_scheme': 'affine', 'granularity': 'default'}]
