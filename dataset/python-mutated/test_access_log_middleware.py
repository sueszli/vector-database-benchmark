...
from sentry.testutils.silo import all_silo_test, assume_test_silo_mode, control_silo_test, region_silo_test
...
class RateLimitedEndpoint(Endpoint):
    ...
    rate_limits = RateLimitConfig(group='foo', limit_overrides={'GET': {RateLimitCategory.IP: RateLimit(0, 1), RateLimitCategory.USER: RateLimit(0, 1), RateLimitCategory.ORGANIZATION: RateLimit(0, 1)}})
...
class ConcurrentRateLimitedEndpoint(Endpoint):
    permission_classes = (AllowAny,)
    enforce_rate_limit = True
    rate_limits = RateLimitConfig(group='foo', limit_overrides={'GET': {RateLimitCategory.IP: RateLimit(20, 1, 1), RateLimitCategory.USER: RateLimit(20, 1, 1), RateLimitCategory.ORGANIZATION: RateLimit(20, 1, 1)}})
    ...
    def get(self, request, organization_context, organization):
        if False:
            for i in range(10):
                print('nop')
        return Response({'ok': True})
urlpatterns = [re_path('^/dummy$', DummyEndpoint.as_view(), name='dummy-endpoint'), re_path('^/dummyfail$', DummyFailEndpoint.as_view(), name='dummy-fail-endpoint'), re_path('^/dummyratelimit$', RateLimitedEndpoint.as_view(), name='ratelimit-endpoint'), re_path('^/dummyratelimitconcurrent$', ConcurrentRateLimitedEndpoint.as_view(), name='concurrent-ratelimit-endpoint'), re_path('^(?P<organization_slug>[^\\/]+)/stats_v2/$', MyOrganizationEndpoint.as_view(), name='sentry-api-0-organization-stats-v2'), re_path('^(?P<organization_slug>[^\\/]+)/members/$', MyControlOrganizationEndpoint.as_view(), name='sentry-api-0-organization-members'), re_path('^rpc/(?P<service_name>\\w+)/(?P<method_name>\\w+)/$', RpcServiceEndpoint.as_view(), name='sentry-api-0-rpc-service')]
access_log_fields = ('method', 'view', 'response', 'user_id', 'is_app', 'token_type', 'organization_id', 'auth_id', 'path', 'caller_ip', 'user_agent', 'rate_limited', 'rate_limit_category', 'request_duration_seconds', 'group', 'rate_limit_type', 'concurrent_limit', 'concurrent_requests', 'reset_time', 'limit', 'remaining')
