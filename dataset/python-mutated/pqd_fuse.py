...
import pickle
import torch
from examples.compression.models import build_resnet18, prepare_dataloader, prepare_optimizer, train, training_step, evaluate, device
...
if __name__ == '__main__':
    ...
    p_config_list = [{'op_types': ['Conv2d'], 'sparse_ratio': 0.5}, *[{'op_names': [name], 'target_names': ['_output_'], 'target_settings': {'_output_': {'align': {'module_name': name.replace('bn', 'conv') if 'bn' in name else name.replace('downsample.1', 'downsample.0'), 'target_name': 'weight', 'dims': [0]}, 'granularity': 'per_channel'}}} for name in bn_list]]
    ...
    q_config_list = [{'op_types': ['Conv2d'], 'quant_dtype': 'int8', 'target_names': ['_input_'], 'granularity': 'per_channel'}, {'op_types': ['Conv2d'], 'quant_dtype': 'int8', 'target_names': ['weight'], 'granularity': 'out_channel'}, {'op_types': ['BatchNorm2d'], 'quant_dtype': 'int8', 'target_names': ['_output_'], 'granularity': 'per_channel'}]
    ...
    d_config_list = [{'op_types': ['Conv2d'], 'lambda': 0.1, 'apply_method': 'mse'}]
