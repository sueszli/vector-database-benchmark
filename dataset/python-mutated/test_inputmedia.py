...
import pytest
from telegram import InputFile, InputMediaAnimation, InputMediaAudio, InputMediaDocument, InputMediaPhoto, InputMediaVideo, Message, MessageEntity
from telegram.constants import ParseMode
from telegram.error import BadRequest
from telegram.request import RequestData
from tests._files.test_animation import animation, animation_file
from tests.auxil.files import data_file
from tests.auxil.networking import expect_bad_request
from tests.auxil.slots import mro_slots
from tests.test_forum import emoji_id, real_topic
from .test_audio import audio, audio_file
from .test_document import document, document_file
from .test_photo import photo, photo_file, photolist, thumb
...
class TestSendMediaGroupWithRequest:
    ...
    async def test_send_media_group_photo(self, bot, chat_id, media_group):
        ...
        assert all((mes.caption_entities == (MessageEntity(MessageEntity.BOLD, 0, 5),) for mes in messages))

    async def test_send_media_group_new_files(self, bot, chat_id, video_file, photo_file):

        async def func():
            return await bot.send_media_group(chat_id, [InputMediaVideo(video_file), InputMediaPhoto(photo_file), InputMediaPhoto(data_file('telegram.jpg').read_bytes())])
        messages = await expect_bad_request(func, 'Type of file mismatch', 'Telegram did not accept the file.')
        assert isinstance(messages, tuple)
        assert len(messages) == 3
        assert all((isinstance(mes, Message) for mes in messages))
        assert all((mes.media_group_id == messages[0].media_group_id for mes in messages))
    ...
    async def test_send_media_group_different_sequences(self, bot, chat_id, media_group, sequence_type, bot_class, raw_bot):
    ...
    async def test_send_media_group_with_message_thread_id(self, bot, real_topic, forum_group_id, media_group):
        messages = await bot.send_media_group(forum_group_id, media_group, message_thread_id=real_topic.message_thread_id)
        assert isinstance(messages, tuple)
        assert len(messages) == 3
        assert all((isinstance(mes, Message) for mes in messages))
        assert all((i.message_thread_id == real_topic.message_thread_id for i in messages))

    @pytest.mark.parametrize(('caption', 'parse_mode', 'caption_entities'), [('*photo* 1', 'Markdown', None), ('<b>photo</b> 1', 'HTML', None), ('photo 1', None, [MessageEntity(MessageEntity.BOLD, 0, 5)])])
    async def test_send_media_group_with_group_caption(self, bot, chat_id, media_group_no_caption_args, caption, parse_mode, caption_entities):
        copied_media_group = media_group_no_caption_args.copy()
        messages = await bot.send_media_group(chat_id, media_group_no_caption_args, caption=caption, parse_mode=parse_mode, caption_entities=caption_entities)
    ...
    async def test_send_media_group_all_args(self, bot, raw_bot, chat_id, media_group):
        ...
        for msg_task in asyncio.as_completed(aws):
            ...
            messages = await m1.get_bot().send_media_group(chat_id, media_group, disable_notification=True, reply_to_message_id=m1.message_id, protect_content=True)
            assert media_group == copied_media_group
            assert all((a.parse_mode == b.parse_mode for (a, b) in zip(media_group, copied_media_group)))
            assert isinstance(messages, tuple)
            assert len(messages) == 3
            assert all((isinstance(mes, Message) for mes in messages))
            assert all((mes.media_group_id == messages[0].media_group_id for mes in messages))
    ...
    async def test_send_media_group_with_spoiler(self, bot, chat_id, photo_file, video_file):
        media = [InputMediaPhoto(photo_file, has_spoiler=True), InputMediaVideo(video_file, has_spoiler=True)]
    ...
    async def test_edit_message_media(self, bot, raw_bot, chat_id, media_group):
        ...
        for msg_task in asyncio.as_completed(aws):
            ...
            new_message = await messages[-1].get_bot().edit_message_media(chat_id=cid, message_id=mid, media=media_group[0])
    ...
    async def test_edit_message_media_new_file(self, bot, chat_id, media_group, thumb_file):
        ...
        cid = messages[-1].chat.id
        mid = messages[-1].message_id
        new_message = await bot.edit_message_media(chat_id=cid, message_id=mid, media=InputMediaPhoto(thumb_file))
        assert isinstance(new_message, Message)
    ...
    async def test_send_media_group_default_allow_sending_without_reply(self, default_bot, chat_id, media_group, custom):
        reply_to_message = await default_bot.send_message(chat_id, 'test')
        await reply_to_message.delete()
        if custom is not None:
            messages = await default_bot.send_media_group(chat_id, media_group, allow_sending_without_reply=custom, reply_to_message_id=reply_to_message.message_id)
            assert [m.reply_to_message is None for m in messages]
        elif default_bot.defaults.allow_sending_without_reply:
            messages = await default_bot.send_media_group(chat_id, media_group, reply_to_message_id=reply_to_message.message_id)
            assert [m.reply_to_message is None for m in messages]
        else:
            with pytest.raises(BadRequest, match='Message to reply not found'):
                await default_bot.send_media_group(chat_id, media_group, reply_to_message_id=reply_to_message.message_id)

    @pytest.mark.parametrize('default_bot', [{'protect_content': True}], indirect=True)
    async def test_send_media_group_default_protect_content(self, chat_id, media_group, default_bot):
        tasks = asyncio.gather(default_bot.send_media_group(chat_id, media_group), default_bot.send_media_group(chat_id, media_group, protect_content=False))
    ...
    async def test_send_media_group_default_parse_mode(self, chat_id, media_group_no_caption_args, default_bot):
        default = await default_bot.send_media_group(chat_id, media_group_no_caption_args, caption='<b>photo</b> 1')
        assert not any((item.parse_mode for item in media_group_no_caption_args))
        tasks = asyncio.gather(default_bot.send_media_group(chat_id, media_group_no_caption_args.copy(), caption='*photo* 1', parse_mode=ParseMode.MARKDOWN_V2), default_bot.send_media_group(chat_id, media_group_no_caption_args.copy(), caption='<b>photo</b> 1', parse_mode=None))
    ...
    @pytest.mark.parametrize('default_bot', [{'parse_mode': ParseMode.HTML}], indirect=True, ids=['HTML-Bot'])
    @pytest.mark.parametrize('media_type', ['animation', 'document', 'audio', 'photo', 'video'])
    async def test_edit_message_media_default_parse_mode(self, chat_id, default_bot, media_type, animation, document, audio, photo, video):
        html_caption = '<b>bold</b> <i>italic</i> <code>code</code>'
        markdown_caption = '*bold* _italic_ `code`'
        test_caption = 'bold italic code'
        test_entities = [MessageEntity(MessageEntity.BOLD, 0, 4), MessageEntity(MessageEntity.ITALIC, 5, 6), MessageEntity(MessageEntity.CODE, 12, 4)]
        ...
        message = await default_bot.edit_message_media(media, message.chat_id, message.message_id)
