...
from scapy.fields import XShortField, ByteEnumField, XByteField, StrFixedLenField, StrLenField, LEShortField, LEFieldLenField, XLE3BytesField, XLEShortField
from scapy.volatile import RandShort
from scapy.compat import bytes_encode, orb
_protocol_modes = {101: 'ascii', 102: 'binary'}
_ascii_command_codes = {'ID': 'Send Identification Command', 'CCR': 'Send Start Command', 'CCS': 'Send Stop Command', 'CCE': 'Send Reset Command', 'CCI': 'Send Init Command', 'CC': 'Reply of Admin Commands (CC*)', 'UG': 'Get UnitID', 'US': 'Set UnitID', 'RC': 'Get RTC', 'SC': 'Set RTC', 'RE': 'Read Inputs', 'RA': 'Read Outputs', 'GS': 'Read System Bits', 'GF': 'Read System Integers', 'RNH': 'Read System Longs', 'RNJ': 'Read System Double Words', 'RB': 'Read Memory Bits', 'RW': 'Read Memory Integers', 'RNL': 'Read Memory Longs', 'RND': 'Read Memory Double Words', 'RN': 'Read Longs / Double Words', 'SA': 'Write Outputs', 'SS': 'Write System Bits', 'SF': 'Write System Integers', 'SNH': 'Write System Longs', 'SNJ': 'Write System Double Words', 'SB': 'Write Memory Bits', 'SW': 'Write Memory Integers', 'SNL': 'Write Memory Longs', 'SND': 'Write Memory Double Words', 'SN': 'Write Longs / Double Words'}
_binary_command_codes = {12: 'Get PLC Name Request', 140: 'Get PLC Name Reply', 77: 'Read Operands Request', 205: 'Read Operands Reply', 4: 'Read Data Table Request', 132: 'Read Data Table Reply', 68: 'Write Data Table Request', 196: 'Write Data Table Reply'}

class PCOM(Packet):
    fields_desc = [XShortField('transId', RandShort()), ByteEnumField('mode', 101, _protocol_modes), XByteField('reserved', 0), LEShortField('len', None)]
...
class PCOMAsciiRequest(PCOMAscii):
    ...
    fields_desc = [StrFixedLenField('stx', '/', 1), StrFixedLenField('unitId', '00', 2), PCOMAsciiCommandField('command', '', length_from=lambda pkt: pkt.underlayer.len - 6), XShortField('chksum', None), XByteField('etx', 13)]
...
class PCOMAsciiResponse(PCOMAscii):
    ...
    fields_desc = [StrFixedLenField('stx', '/A', 2), StrFixedLenField('unitId', '00', 2), PCOMAsciiCommandField('command', '', length_from=lambda pkt: pkt.underlayer.len - 7), XShortField('chksum', None), XByteField('etx', 13)]
...
class PCOMBinaryRequest(PCOMBinary):
    ...
    fields_desc = [StrFixedLenField('stx', '/_OPLC', 6), XByteField('id', 0), XByteField('reserved1', 254), XByteField('reserved2', 1), XLE3BytesField('reserved3', 0), PCOMBinaryCommandField('command', None), XByteField('reserved4', 0), StrFixedLenField('commandSpecific', '', 6), LEFieldLenField('len', 0, length_of='data'), XLEShortField('headerChksum', None), StrLenField('data', '', length_from=lambda pkt: pkt.len), XLEShortField('footerChksum', None), XByteField('etx', 92)]

class PCOMBinaryResponse(PCOMBinary):
    name = 'PCOM/Binary Response'
    fields_desc = [StrFixedLenField('stx', '/_OPLC', 6), XByteField('reserved1', 254), XByteField('id', 0), XByteField('reserved2', 1), XLE3BytesField('reserved3', 0), PCOMBinaryCommandField('command', None), XByteField('reserved4', 0), StrFixedLenField('commandSpecific', '', 6), LEFieldLenField('len', 0, length_of='data'), XLEShortField('headerChksum', None), StrLenField('data', '', length_from=lambda pkt: pkt.len), XLEShortField('footerChksum', None), XByteField('etx', 92)]
bind_layers(TCP, PCOMRequest, dport=20256)
bind_layers(TCP, PCOMResponse, sport=20256)
bind_layers(PCOMRequest, PCOMAsciiRequest, mode=101)
bind_layers(PCOMRequest, PCOMBinaryRequest, mode=102)
bind_layers(PCOMResponse, PCOMAsciiResponse, mode=101)
bind_layers(PCOMResponse, PCOMBinaryResponse, mode=102)