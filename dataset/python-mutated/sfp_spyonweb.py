...
class sfp_spyonweb(SpiderFootPlugin):
    meta = {'name': 'SpyOnWeb', 'summary': 'Search SpyOnWeb for hosts sharing the same IP address, Google Analytics code, or Google Adsense code.', 'flags': ['apikey'], 'useCases': ['Footprint', 'Investigate', 'Passive'], 'categories': ['Passive DNS'], 'dataSource': {'website': 'http://spyonweb.com/', 'model': 'FREE_AUTH_LIMITED', 'references': ['https://api.spyonweb.com/v1/docs', 'https://api.spyonweb.com/'], 'apiKeyInstructions': ['Visit https://api.spyonweb.com', 'Sign up for a free account', "Click on 'Dashboard'", "The API key is listed under 'Access Token'"], 'favIcon': 'http://spyonweb.com/favicon.ico', 'logo': 'http://spyonweb.com/favicon.ico', 'description': 'We take the information from public sources, then structure it for your quick and convenient search for the websites that probably belong to the same owner.'}}
    opts = {'api_key': '', 'limit': 100, 'timeout': 30, 'maxage': 1095, 'verify': True, 'cohostsamedomain': False, 'maxcohost': 100}
    optdescs = {'api_key': 'SpyOnWeb API key.', 'limit': 'Maximum number of results to fetch.', 'timeout': 'Query timeout, in seconds.', 'maxage': 'The maximum age of the data returned, in days, in order to be considered valid.', 'verify': 'Verify co-hosts are valid by checking if they still resolve to the shared IP.', 'cohostsamedomain': 'Treat co-hosted sites on the same target domain as co-hosting?', 'maxcohost': 'Stop reporting co-hosted sites after this many are found, as it would likely indicate web hosting.'}
    cohostcount = 0
    results = None
    errorState = False
