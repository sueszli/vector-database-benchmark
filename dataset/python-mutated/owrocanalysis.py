...
from AnyQt.QtGui import QColor, QPen, QBrush, QPainter, QPalette, QFont, QCursor, QFontMetrics
...
from Orange.widgets.evaluate.utils import check_results_adequacy, check_can_calibrate
...
ROCPoints = namedtuple('ROCPoints', ['fpr', 'tpr', 'thresholds'])
ROCPoints.is_valid = property(lambda self: self.fpr.size > 0)
ROCCurve = namedtuple('ROCCurve', ['points', 'hull'])
ROCCurve.is_valid = property(lambda self: self.points.is_valid)
ROCAveragedVert = namedtuple('ROCAveragedVert', ['points', 'hull', 'tpr_std'])
ROCAveragedVert.is_valid = property(lambda self: self.points.is_valid)
ROCAveragedThresh = namedtuple('ROCAveragedThresh', ['points', 'hull', 'tpr_std', 'fpr_std'])
ROCAveragedThresh.is_valid = property(lambda self: self.points.is_valid)
ROCData = namedtuple('ROCData', ['merged', 'folds', 'avg_vertical', 'avg_threshold'])
...
PlotAvgCurve = namedtuple('PlotAvgCurve', ['curve', 'curve_item', 'hull_item', 'confint_item'])
...
def once(f):
    ...
    def wraped():
        ...
        return cached.val
    return wraped
PlotCurves = namedtuple('PlotCurves', ['merge', 'folds', 'avg_vertical', 'avg_threshold'])
...
class OWROCAnalysis(widget.OWWidget):
    name = 'ROC Analysis'
    description = 'Display the Receiver Operating Characteristics curve based on the evaluation of classifiers.'
