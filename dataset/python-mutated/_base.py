...
from elastic_transport import ApiResponse, AsyncTransport, BinaryApiResponse, HeadApiResponse, HttpHeaders, ListApiResponse, NodeConfig, ObjectApiResponse, SniffOptions, TextApiResponse
from elastic_transport.client_utils import DEFAULT, DefaultType
from ..._version import __versionstr__
from ...compat import warn_stacklevel
from ...exceptions import HTTP_EXCEPTIONS, ApiError, ConnectionError, ElasticsearchWarning, SerializationError, UnsupportedProductError
...
_COMPAT_MIMETYPE_RE = re.compile('application/(json|x-ndjson|vnd\\.mapbox-vector-tile)')
_COMPAT_MIMETYPE_SUB = _COMPAT_MIMETYPE_TEMPLATE % ('\\g<1>',)
...
class BaseClient:
    ...
    async def perform_request(self, method: str, path: str, *, params: Optional[Mapping[str, Any]]=None, headers: Optional[Mapping[str, str]]=None, body: Optional[Any]=None) -> ApiResponse[Any]:
        if headers:
        ...
        if params:
            ...
            target = path
        (meta, resp_body) = await self.transport.perform_request(method, target, headers=request_headers, body=body, request_timeout=self._request_timeout, max_retries=self._max_retries, retry_on_status=self._retry_on_status, retry_on_timeout=self._retry_on_timeout, client_meta=self._client_meta)
        if not (method == 'HEAD' and meta.status == 404) and (not 200 <= meta.status < 299 and (self._ignore_status is DEFAULT or self._ignore_status is None or meta.status not in self._ignore_status)):
        ...
        if not self._verified_elasticsearch:
            if meta.headers.get('x-elastic-product', '') == 'Elasticsearch':
                ...
                raise UnsupportedProductError(message='The client noticed that the server is not Elasticsearch and we do not support this unknown product', meta=meta, body=resp_body)
        if 'warning' in meta.headers:
            ...
            for warning_message in warning_messages:
                warnings.warn(warning_message, category=ElasticsearchWarning, stacklevel=stacklevel)
        if method == 'HEAD':
            response = HeadApiResponse(meta=meta)
        elif isinstance(resp_body, dict):
            response = ObjectApiResponse(body=resp_body, meta=meta)
        elif isinstance(resp_body, list):
            response = ListApiResponse(body=resp_body, meta=meta)
        elif isinstance(resp_body, str):
            response = TextApiResponse(body=resp_body, meta=meta)
        elif isinstance(resp_body, bytes):
            response = BinaryApiResponse(body=resp_body, meta=meta)
        else:
            response = ApiResponse(body=resp_body, meta=meta)
        return response
...
class NamespacedClient(BaseClient):
    ...
    async def perform_request(self, method: str, path: str, *, params: Optional[Mapping[str, Any]]=None, headers: Optional[Mapping[str, str]]=None, body: Optional[Any]=None) -> ApiResponse[Any]:
        return await self._client.perform_request(method, path, params=params, headers=headers, body=body)