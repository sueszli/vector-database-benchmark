    ...
    def execute(self):
        ...
        for vpc in vpc_client.vpcs.values():
            if not vpc_client.audit_info.ignore_unused_services or vpc.in_use:
                ...
                report.status = 'FAIL'
                report.status_extended = f'VPC {(vpc.name if vpc.name else vpc.id)} Flow logs are disabled.'
                if vpc.flow_log:
                    report.status = 'PASS'
                    report.status_extended = f'VPC {(vpc.name if vpc.name else vpc.id)} Flow logs are enabled.'
