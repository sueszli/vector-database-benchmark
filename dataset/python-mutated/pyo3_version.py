...
def replace_section_content(section):
    ...
    if not isinstance(section, dict) or 'Chapter' not in section:
        return
    section['Chapter']['content'] = section['Chapter']['content'].replace('{{#PYO3_VERSION_TAG}}', PYO3_VERSION_TAG).replace('{{#PYO3_DOCS_URL}}', PYO3_DOCS_URL).replace('{{#PYO3_DOCS_VERSION}}', PYO3_DOCS_VERSION).replace('{{#PYO3_CRATE_VERSION}}', PYO3_CRATE_VERSION)
