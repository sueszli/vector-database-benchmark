...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--number', type=int, default=50, help='number of images to download (default: 50)')
    parser.add_argument('-p', '--path', type=str, default='data', help='path to download the images (created if needed)')
    parser.add_argument('--query-in-path', '-i', dest='query_in_path', action='store_true')
    parser.add_argument('--no-query-in-path', dest='query_in_path', action='store_false')
    parser.set_defaults(query_in_path=True)
    args = parser.parse_args()
    query_flickr(key=args.key, secret=args.secret, query=args.query, number=args.number, path=args.path, query_in_path=args.query_in_path)