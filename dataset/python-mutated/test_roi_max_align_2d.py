...
@testing.parameterize(*testing.product({'sampling_ratio': [None, 1, 2, (None, 3), (1, 2), (numpy.int32(1), numpy.int32(2))], 'outsize': [5, 7, (5, 7), (numpy.int32(5), numpy.int32(7))], 'spatial_scale': [0.6, 1.0, 2.0, numpy.float32(0.6), numpy.int32(2)]}))
class TestROIMaxAlign2D(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rois = numpy.array([[1, 1, 6, 6], [6, 2, 7, 11], [3, 1, 5, 10], [3, 3, 3, 3], [1.1, 2.2, 3.3, 4.4]], dtype=numpy.float32)
    ...
    def check_forward(self, x_data, roi_data, roi_index_data):
        ...
        y = functions.roi_max_align_2d(x, rois, roi_indices, outsize=self.outsize, spatial_scale=self.spatial_scale, sampling_ratio=self.sampling_ratio)
    ...
    def test_forward_cpu_gpu_equal(self):
        ...
        y_cpu = functions.roi_max_align_2d(x_cpu, rois_cpu, roi_index_cpu, outsize=self.outsize, spatial_scale=self.spatial_scale, sampling_ratio=self.sampling_ratio)
        x_gpu = chainer.Variable(cuda.to_gpu(self.x))
        rois_gpu = chainer.Variable(cuda.to_gpu(self.rois))
        roi_index_gpu = chainer.Variable(cuda.to_gpu(self.roi_indices))
        y_gpu = functions.roi_max_align_2d(x_gpu, rois_gpu, roi_index_gpu, outsize=self.outsize, spatial_scale=self.spatial_scale, sampling_ratio=self.sampling_ratio)
