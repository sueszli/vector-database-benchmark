...
if __name__ == '__main__':
    ...
    loaders = {'train': DataLoader(train_dataset, batch_size=32, collate_fn=collate_fn_train), 'valid': DataLoader(test_dataset, batch_size=32, collate_fn=collate_fn_valid)}
    item_num = len(train_dataset[0])
    model = MultiDAE([200, 600, item_num], dropout=0.5)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    engine = dl.Engine()
    callbacks = [dl.NDCGCallback('logits', 'targets', [20, 50, 100]), dl.MAPCallback('logits', 'targets', [20, 50, 100]), dl.MRRCallback('logits', 'targets', [20, 50, 100]), dl.HitrateCallback('logits', 'targets', [20, 50, 100]), dl.BackwardCallback('loss'), dl.OptimizerCallback('loss', accumulation_steps=1)]
    runner = dl.SupervisedRunner(input_key='inputs', output_key='logits', target_key='targets', loss_key='loss')
    runner.train(model=model, optimizer=optimizer, criterion=criterion, engine=engine, loaders=loaders, num_epochs=100, verbose=True, timeit=False, callbacks=callbacks, logdir='./logs_multidae')