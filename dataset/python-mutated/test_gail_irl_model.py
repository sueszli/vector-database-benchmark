...
device = 'cpu'
action_space = 3
cfg1 = (dict(input_size=obs_space_1d + 1, hidden_size=64, batch_size=5, learning_rate=0.001, update_per_collect=2, data_path=expert_data_path_1d),)
cfg2 = (dict(input_size=obs_space_3d, hidden_size=64, batch_size=5, learning_rate=0.001, update_per_collect=2, data_path=expert_data_path_3d, action_size=action_space),)
data_1d = []
for i in range(20):
    d = {}
    d['obs'] = torch.zeros(obs_space_1d)
    d['action'] = torch.Tensor([1.0])
    data_1d.append(d)
data_3d = []
for i in range(20):
    d = {}
