...
class TestBaseRateLimiter:
    ...
    async def test_argument_passing(self, bot_info, monkeypatch, bot):
        ...
        rl_bot = ExtBot(token=bot.token, defaults=defaults, request=test_request, rate_limiter=TestRateLimiter())
        async with standard_bot:
            await standard_bot.set_my_commands(commands=[BotCommand('test', 'test')], language_code='en', api_kwargs={'api': 'kwargs'})
        async with rl_bot:
            await rl_bot.set_my_commands(commands=[BotCommand('test', 'test')], language_code='en', rate_limit_args=(43, 'test-1'), api_kwargs={'api': 'kwargs'})
        assert len(self.rl_received) == 2
        assert self.rl_received[0] == ('getMe', {}, None)
        assert self.rl_received[1] == ('setMyCommands', {'commands': [BotCommand('test', 'test')], 'language_code': 'en', 'api': 'kwargs'}, (43, 'test-1'))
...
@pytest.mark.skipif(bool(GITHUB_ACTION and platform.system() == 'Darwin'), reason='The timings are apparently rather inaccurate on MacOS.')
...
class TestAIORateLimiter:
    ...
    class CountRequest(BaseRequest):
        ...
        async def do_request(self, *args, **kwargs):
            ...
            if url.endswith('getme'):
                return (HTTPStatus.OK, json.dumps({'ok': True, 'result': User(id=1, first_name='bot', is_bot=True).to_dict()}).encode())
            if url.endswith('sendmessage'):
                return (HTTPStatus.OK, json.dumps({'ok': True, 'result': Message(message_id=1, date=datetime.now(), chat=Chat(1, 'chat')).to_dict()}).encode())
            return None
    ...
    async def test_max_retries(self, bot, max_retries):
        bot = ExtBot(token=bot.token, request=self.CountRequest(retry_after=1), rate_limiter=AIORateLimiter(max_retries=max_retries, overall_max_rate=0, group_max_rate=0))
    ...
    async def test_delay_all_pending_on_retry(self, bot):
        bot = ExtBot(token=bot.token, request=self.CountRequest(retry_after=1), rate_limiter=AIORateLimiter(max_retries=1, overall_max_rate=0, group_max_rate=0))
    ...
    async def test_basic_rate_limiting(self, bot, group_id, chat_id):
        try:
            rl_bot = ExtBot(token=bot.token, request=self.CountRequest(retry_after=None), rate_limiter=AIORateLimiter(overall_max_rate=1, overall_time_period=1 / 4, group_max_rate=1, group_time_period=1 / 2))
            async with rl_bot:
                non_group_tasks = {}
                group_tasks = {}
                for i in range(4):
                    group_tasks[i] = asyncio.create_task(rl_bot.send_message(chat_id=group_id, text='test'))
                for i in range(8):
                    non_group_tasks[i] = asyncio.create_task(rl_bot.send_message(chat_id=chat_id, text='test'))
    ...
    async def test_rate_limiting_no_chat_id(self, bot):
        try:
            rl_bot = ExtBot(token=bot.token, request=self.CountRequest(retry_after=None), rate_limiter=AIORateLimiter(overall_max_rate=1, overall_time_period=1 / 2))
            async with rl_bot:
                non_chat_tasks = {}
                chat_tasks = {}
                for i in range(4):
    ...
    async def test_group_caching(self, bot, intermediate):
        try:
            max_rate = 1000
            rl_bot = ExtBot(token=bot.token, request=self.CountRequest(retry_after=None), rate_limiter=AIORateLimiter(overall_max_rate=max_rate, overall_time_period=1, group_max_rate=max_rate, group_time_period=1))
            assert len(rl_bot.rate_limiter._group_limiters) == 0
            await asyncio.gather(*(rl_bot.send_message(chat_id=-(i + 1), text=f'{i}') for i in range(513)))
            if intermediate:
