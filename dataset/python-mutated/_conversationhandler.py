...
from typing import TYPE_CHECKING, Any, Dict, Final, Generic, List, NoReturn, Optional, Set, Tuple, Union, cast
...
class ConversationHandler(BaseHandler[Update, CCT]):
    ...
    """
    __slots__ = ('_allow_reentry', '_block', '_child_conversations', '_conversation_timeout', '_conversations', '_entry_points', '_fallbacks', '_map_to_parent', '_name', '_per_chat', '_per_message', '_per_user', '_persistent', '_states', '_timeout_jobs_lock', 'timeout_jobs')
    END: Final[int] = -1
    ...
    async def _initialize_persistence(self, application: 'Application') -> Dict[str, TrackingDict[ConversationKey, object]]:
        ...
        if not (self.persistent and self.name and application.persistence):
            raise RuntimeError('This handler is not persistent, has no name or the application has no persistence!')
        current_conversations = self._conversations
        self._conversations = cast(TrackingDict[ConversationKey, object], TrackingDict())
        ...
        for handler in self._child_conversations:
            out.update(await handler._initialize_persistence(application=application))
        return out
    ...
    async def _schedule_job_delayed(self, new_state: asyncio.Task, application: 'Application[Any, CCT, Any, Any, Any, JobQueue]', update: Update, context: CCT, conversation_key: ConversationKey) -> None:
        try:
            effective_new_state = await new_state
        except Exception as exc:
            _LOGGER.debug('Non-blocking handler callback raised exception. Not scheduling conversation timeout.', exc_info=exc)
            return None
        return self._schedule_job(new_state=effective_new_state, application=application, update=update, context=context, conversation_key=conversation_key)
    ...
    async def handle_update(self, update: Update, application: 'Application[Any, CCT, Any, Any, Any, Any]', check_result: _CheckUpdateType[CCT], context: CCT) -> Optional[object]:
        ...
        try:
            if block:
                new_state: object = await handler.handle_update(update, application, handler_check_result, context)
            else:
                new_state = application.create_task(coroutine=handler.handle_update(update, application, handler_check_result, context), update=update, name=f'ConversationHandler:{update.update_id}:handle_update:non_blocking_cb')
        ...
        async with self._timeout_jobs_lock:
            if self.conversation_timeout:
                if application.job_queue is None:
                    warn('Ignoring `conversation_timeout` because the Application has no JobQueue.', stacklevel=1)
                elif not application.job_queue.scheduler.running:
                    warn('Ignoring `conversation_timeout` because the Applications JobQueue is not running.', stacklevel=1)
                elif isinstance(new_state, asyncio.Task):
                    application.create_task(self._schedule_job_delayed(new_state, application, update, context, conversation_key), update=update, name=f'ConversationHandler:{update.update_id}:handle_update:timeout_job')
                else:
    ...
    async def _trigger_timeout(self, context: CCT) -> None:
        ...
        _LOGGER.debug('Conversation timeout was triggered for conversation %s!', ctxt.conversation_key)
        ...
        for handler in handlers:
            ...
            if check is not None and check is not False:
                try:
                    await handler.handle_update(ctxt.update, ctxt.application, check, callback_context)
                except ApplicationHandlerStop:
                    warn('ApplicationHandlerStop in TIMEOUT state of ConversationHandler has no effect. Ignoring.', stacklevel=2)
