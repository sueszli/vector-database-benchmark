...
from .common import assert_dict_of_states_equal_without_context_and_last_changed, assert_multiple_states_equal_without_context, assert_multiple_states_equal_without_context_and_last_changed, assert_states_equal_without_context, old_db_schema, wait_recording_done
...
def test_get_significant_states_with_initial(hass_recorder: Callable[..., HomeAssistant]) -> None:
    if False:
        print('Hello World!')
    'Test that only significant states are returned.\n\n    We should get back every thermostat change that\n    includes an attribute change, but only the state updates for\n    media player (attribute changes are not significant and not returned).\n    '
    hass = hass_recorder()
