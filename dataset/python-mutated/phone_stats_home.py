...
current_mau_by_service_gauge = Gauge('synapse_admin_mau_current_mau_by_service', 'Current MAU by service', ['app_service'])
max_mau_gauge = Gauge('synapse_admin_mau_max', 'MAU Limit')
registered_reserved_users_mau_gauge = Gauge('synapse_admin_mau_registered_reserved_users', 'Registered users with reserved threepids')

@wrap_as_background_process('phone_stats_home')
async def phone_stats_home(hs: 'HomeServer', stats: JsonDict, stats_process: List[Tuple[int, 'resource.struct_rusage']]=_stats_process) -> None:
    ...
    used_cpu_time = new[1].ru_utime + new[1].ru_stime - (old[1].ru_utime + old[1].ru_stime)
    ...
    logger.info('Reporting stats to %s: %s' % (hs.config.metrics.report_stats_endpoint, stats))
    try:
        await hs.get_proxied_http_client().put_json(hs.config.metrics.report_stats_endpoint, stats)
