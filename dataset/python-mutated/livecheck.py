...
class Order(faust.Record):
    ...
    price: float
    fake: bool = False
app = faust.App('orders', cache='redis://localhost:6379', origin='examples.livecheck', autodiscover=True)
...
class OrderView(web.View):
    ...
    async def get(self, request: web.Request, side: str) -> web.Response:
        ...
        async with test_order.maybe_trigger(order_id, side=side) as test:
            ...
            data = {'order_id': order_id, 'user_id': user_id, 'side': side, 'did_execute_test': bool(test), 'fake': fake}
...
class test_order(Case):
    ...
    async def run(self, side: str) -> None:
        ...
        assert await livecheck.cache.client.sismember(f'order.{order.user_id}.orders', order.id)
...
@app.command(cli.option('--side', default='sell', help='Order side: buy, sell'), cli.option('--base-url', default='http://localhost:6066'))
