...
class GrapheneScheduleDryRunResult(graphene.Union):
    ...
    class Meta:
        types = (GrapheneDryRunInstigationTick, GraphenePythonError, GrapheneScheduleNotFoundError)
...
class GrapheneScheduleDryRunMutation(graphene.Mutation):
    ...
    @capture_error
    def mutate(self, graphene_info: 'ResolveInfo', selector_data: Mapping[str, Any], timestamp: float):
        if False:
            i = 10
            return i + 15
        return GrapheneDryRunInstigationTick(selector=ScheduleSelector.from_graphql_input(selector_data), timestamp=timestamp)
types = [GrapheneScheduleDryRunMutation, GrapheneScheduleDryRunResult]