...
class SoundAndroidPlayer(Sound):
    ...
    def extensions():
        if False:
            print('Hello World!')
        return ('mp3', 'mp4', 'aac', '3gp', 'flac', 'mkv', 'wav', 'ogg', 'm4a', 'gsm', 'mid', 'xmf', 'mxmf', 'rtttl', 'rtx', 'ota', 'imy')
    ...
    def load(self):
        ...
        if api_version >= 21:
            self._mediaplayer.setAudioAttributes(AudioAttributesBuilder().setLegacyStreamType(AudioManager.STREAM_MUSIC).build())
        else:
