...
class ElasticsearchDocumentStore(_ElasticsearchDocumentStore):
    ...
    def __init__(self, host: Union[str, List[str]]='localhost', port: Union[int, List[int]]=9200, username: str='', password: str='', api_key_id: Optional[str]=None, api_key: Optional[str]=None, aws4auth=None, index: str='document', label_index: str='label', search_fields: Union[str, list]='content', content_field: str='content', name_field: str='name', embedding_field: str='embedding', embedding_dim: int=768, custom_mapping: Optional[dict]=None, excluded_meta_data: Optional[list]=None, analyzer: str='standard', scheme: str='http', ca_certs: Optional[str]=None, verify_certs: bool=True, recreate_index: bool=False, create_index: bool=True, refresh_type: str='wait_for', similarity: str='dot_product', timeout: int=300, return_embedding: bool=False, duplicate_documents: str='overwrite', scroll: str='1d', skip_missing_embeddings: bool=True, synonyms: Optional[List]=None, synonym_type: str='synonym', use_system_proxy: bool=False, batch_size: int=10000):
        ...
        client = ElasticsearchDocumentStore._init_elastic_client(host=host, port=port, username=username, password=password, api_key=api_key, api_key_id=api_key_id, scheme=scheme, ca_certs=ca_certs, verify_certs=verify_certs, timeout=timeout, use_system_proxy=use_system_proxy)
    ...
    @staticmethod
    def _init_elastic_client(host: Union[str, List[str]], port: Union[int, List[int]], username: str, password: str, api_key_id: Optional[str], api_key: Optional[str], scheme: str, ca_certs: Optional[str], verify_certs: bool, timeout: int, use_system_proxy: bool, aws4auth: Optional[str]='') -> Elasticsearch:
        if False:
            print('Hello World!')
        ...
        if api_key_id and api_key:
            if ca_certs is not None:
                client = Elasticsearch(hosts=hosts, api_key=(api_key_id, api_key), ca_certs=ca_certs, verify_certs=verify_certs, request_timeout=timeout, node_class=node_class)
            else:
                client = Elasticsearch(hosts=hosts, api_key=(api_key_id, api_key), verify_certs=verify_certs, request_timeout=timeout, node_class=node_class)
        elif username:
            if ca_certs is not None:
                client = Elasticsearch(hosts=hosts, basic_auth=(username, password), ca_certs=ca_certs, verify_certs=verify_certs, request_timeout=timeout, node_class=node_class)
            else:
                client = Elasticsearch(hosts=hosts, basic_auth=(username, password), verify_certs=verify_certs, request_timeout=timeout, node_class=node_class)
        elif ca_certs is not None:
            client = Elasticsearch(hosts=hosts, ca_certs=ca_certs, verify_certs=verify_certs, request_timeout=timeout, node_class=node_class)
        else:
            client = Elasticsearch(hosts=hosts, basic_auth=(username, password), verify_certs=verify_certs, request_timeout=timeout, node_class=node_class)
        try:
            if username in ['', 'elastic']:
                status = client.ping()
                if not status:
                    raise ConnectionError(f'Initial connection to Elasticsearch failed. Make sure you run an Elasticsearch instance at `{hosts}` and that it has finished the initial ramp up (can take > 30s). Also, make sure you are using the correct credentials if you are using a secured Elasticsearch instance.')
        except Exception:
            raise ConnectionError(f'Initial connection to Elasticsearch failed. Make sure you run an Elasticsearch instance at `{hosts}` and that it has finished the initial ramp up (can take > 30s). Also, make sure you are using the correct credentials if you are using a secured Elasticsearch instance.')
        return client
    ...
    def _index_exists(self, index_name: str, headers: Optional[Dict[str, str]]=None) -> bool:
        if False:
            for i in range(10):
                print('nop')
        if logger.isEnabledFor(logging.DEBUG) and self.client.options(headers=headers).indices.exists_alias(name=index_name):
    ...
    def _delete_by_query(self, *args, **kwargs):
        ...
        body = kwargs.pop('body', {})
        return self.client.options(headers=headers, ignore_status=ignore_status).delete_by_query(*args, **kwargs, **body)
