...
from scapy.fields import BitField, BitFieldLenField, BoundStrLenField, ByteEnumField, ByteField, ConditionalField, Field, FieldLenField, FieldListField, FlagsField, IEEEFloatField, IP6PrefixField, IPField, IPPrefixField, IntField, LongField, MACField, PacketField, PacketListField, ShortField, ThreeBytesField, XIntField, XShortField
...
class _ISIS_GenericTlv_Base(Packet):
    fields_desc = [ByteField('type', 0), FieldLenField('len', None, length_of='val', fmt='B'), BoundStrLenField('val', '', length_from=lambda pkt: pkt.len)]
...
_isis_subtlv_classes_1 = {3: 'ISIS_AdministrativeGroupSubTlv', 4: 'ISIS_LinkLocalRemoteIdentifiersSubTlv', 6: 'ISIS_IPv4InterfaceAddressSubTlv', 8: 'ISIS_IPv4NeighborAddressSubTlv', 9: 'ISIS_MaximumLinkBandwidthSubTlv', 10: 'ISIS_MaximumReservableLinkBandwidthSubTlv', 11: 'ISIS_UnreservedBandwidthSubTlv', 12: 'ISIS_IPv6InterfaceAddressSubTlv', 13: 'ISIS_IPv6NeighborAddressSubTlv', 18: 'ISIS_TEDefaultMetricSubTlv'}
_isis_subtlv_names_1 = {3: 'Administrative Group (Color)', 4: 'Link Local/Remote Identifiers', 6: 'IPv4 Interface Address', 8: 'IPv4 Neighbor Address', 9: 'Maximum Link Bandwidth', 10: 'Maximum Reservable Link Bandwidth', 11: 'Unreserved Bandwidth', 12: 'IPv6 Interface Address', 13: 'IPv6 Neighbor Address', 18: 'TE Default Metric'}
...
class ISIS_MaximumLinkBandwidthSubTlv(ISIS_GenericSubTlv):
    ...
    fields_desc = [ByteEnumField('type', 9, _isis_subtlv_names_1), FieldLenField('len', None, length_of='maxbw', fmt='B'), IEEEFloatField('maxbw', 1000)]

class ISIS_MaximumReservableLinkBandwidthSubTlv(ISIS_GenericSubTlv):
    name = 'Maximum Reservable Link Bandwidth SubTLV'
    fields_desc = [ByteEnumField('type', 10, _isis_subtlv_names_1), FieldLenField('len', None, length_of='maxrsvbw', fmt='B'), IEEEFloatField('maxrsvbw', 1000)]

class ISIS_UnreservedBandwidthSubTlv(ISIS_GenericSubTlv):
    name = 'Unreserved Bandwidth SubTLV'
    fields_desc = [ByteEnumField('type', 11, _isis_subtlv_names_1), FieldLenField('len', None, length_of='unrsvbw', fmt='B'), FieldListField('unrsvbw', [1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000], IEEEFloatField('', 1000), count_from=lambda pkt: pkt.len / 4)]

class ISIS_TEDefaultMetricSubTlv(ISIS_GenericSubTlv):
    name = 'TE Default Metric SubTLV'
    fields_desc = [ByteEnumField('type', 18, _isis_subtlv_names_1), FieldLenField('len', None, length_of='temetric', adjust=lambda pkt, x: x - 1, fmt='B'), ThreeBytesField('temetric', 1000)]
...
class ISIS_32bitAdministrativeTagSubTlv(ISIS_GenericSubTlv):
    ...
    fields_desc = [ByteEnumField('type', 1, _isis_subtlv_names_2), FieldLenField('len', None, length_of='tags', fmt='B'), FieldListField('tags', [], IntField('', 0), count_from=lambda pkt: pkt.len // 4)]

class ISIS_64bitAdministrativeTagSubTlv(ISIS_GenericSubTlv):
    name = 'ISIS 64-bit Administrative Tag (S)'
    fields_desc = [ByteEnumField('type', 2, _isis_subtlv_names_2), FieldLenField('len', None, length_of='tags', fmt='B'), FieldListField('tags', [], LongField('', 0), count_from=lambda pkt: pkt.len // 8)]

class ISIS_PrefixSegmentIdentifierSubTlv(ISIS_GenericSubTlv):
    name = 'ISIS Prefix SID sub TLV'
    fields_desc = [ByteEnumField('type', 3, _isis_subtlv_names_2), ByteField('len', 5), FlagsField('flags', 0, 8, ['res1', 'res2', 'L', 'V', 'E', 'P', 'N', 'R']), ByteField('algorithm', 0), ConditionalField(ThreeBytesField('sid', 0), lambda pkt: pkt.len == 5), ConditionalField(IntField('idx', 0), lambda pkt: pkt.len == 6)]
...
_isis_subtlv_classes_4 = {2: 'ISIS_SRCapabilitiesSubTLV', 19: 'ISIS_SRAlgorithmSubTLV'}
_isis_subtlv_names_4 = {2: 'Segment Routing Capability sub TLV', 19: 'Segment Routing Algorithm'}
...
class ISIS_SRCapabilitiesSubTLV(ISIS_GenericSubTlv):
    ...
    fields_desc = [ByteEnumField('type', 2, _isis_subtlv_names_3), FieldLenField('len', None, length_of='srgb_ranges', adjust=lambda pkt, x: x + 1, fmt='B'), FlagsField('flags', 0, 8, ['res1', 'res2', 'res3', 'res4', 'res5', 'res6', 'V', 'I']), PacketListField('srgb_ranges', [], ISIS_SRGBDescriptorEntry, length_from=lambda pkt: pkt.len - 1)]
...
class ISIS_AreaEntry(Packet):
    ...
    fields_desc = [FieldLenField('arealen', None, length_of='areaid', fmt='B'), ISIS_AreaIdField('areaid', '49', length_from=lambda pkt: pkt.arealen)]
...
class ISIS_AreaTlv(ISIS_GenericTlv):
    name = 'ISIS Area TLV'
    fields_desc = [ByteEnumField('type', 1, _isis_tlv_names), FieldLenField('len', None, length_of='areas', fmt='B'), PacketListField('areas', [], ISIS_AreaEntry, length_from=lambda x: x.len)]

class ISIS_AuthenticationTlv(ISIS_GenericTlv):
    name = 'ISIS Authentication TLV'
    fields_desc = [ByteEnumField('type', 10, _isis_tlv_names), FieldLenField('len', None, length_of='password', adjust=lambda pkt, x: x + 1, fmt='B'), ByteEnumField('authtype', 1, {1: 'Plain', 17: 'HMAC-MD5'}), BoundStrLenField('password', '', maxlen=254, length_from=lambda pkt: pkt.len - 1)]
...
class ISIS_DynamicHostnameTlv(ISIS_GenericTlv):
    ...
    fields_desc = [ByteEnumField('type', 137, _isis_tlv_names), FieldLenField('len', None, length_of='hostname', fmt='B'), BoundStrLenField('hostname', '', length_from=lambda pkt: pkt.len)]

class ISIS_ExtendedIpPrefix(Packet):
    name = 'ISIS Extended IP Prefix'
    fields_desc = [IntField('metric', 1), BitField('updown', 0, 1), BitField('subtlvindicator', 0, 1), BitFieldLenField('pfxlen', None, 6, length_of='pfx'), IPPrefixField('pfx', None, wordbytes=1, length_from=lambda x: x.pfxlen), ConditionalField(FieldLenField('subtlvslen', None, length_of='subtlvs', fmt='B'), lambda pkt: pkt.subtlvindicator == 1), ConditionalField(PacketListField('subtlvs', [], _ISIS_GuessSubTlvClass_2, length_from=lambda x: x.subtlvslen), lambda pkt: pkt.subtlvindicator == 1)]
...
class ISIS_ExtendedIpReachabilityTlv(ISIS_GenericTlv):
    ...
    fields_desc = [ByteEnumField('type', 135, _isis_tlv_names), FieldLenField('len', None, length_of='pfxs', fmt='B'), PacketListField('pfxs', [], ISIS_ExtendedIpPrefix, length_from=lambda pkt: pkt.len)]

class ISIS_ExtendedIsNeighbourEntry(Packet):
    name = 'ISIS Extended IS Neighbour Entry'
    fields_desc = [ISIS_NodeIdField('neighbourid', '0102.0304.0506.07'), ThreeBytesField('metric', 1), FieldLenField('subtlvslen', None, length_of='subtlvs', fmt='B'), PacketListField('subtlvs', [], _ISIS_GuessSubTlvClass_1, length_from=lambda x: x.subtlvslen)]
...
class ISIS_ExtendedIsReachabilityTlv(ISIS_GenericTlv):
    ...
    fields_desc = [ByteEnumField('type', 22, _isis_tlv_names), FieldLenField('len', None, length_of='neighbours', fmt='B'), PacketListField('neighbours', [], ISIS_ExtendedIsNeighbourEntry, length_from=lambda x: x.len)]

class ISIS_IpInterfaceAddressTlv(ISIS_GenericTlv):
    name = 'ISIS IP Interface Address TLV'
    fields_desc = [ByteEnumField('type', 132, _isis_tlv_names), FieldLenField('len', None, length_of='addresses', fmt='B'), FieldListField('addresses', [], IPField('', '0.0.0.0'), count_from=lambda pkt: pkt.len // 4)]
...
class ISIS_Ipv6Prefix(Packet):
    ...
    fields_desc = [IntField('metric', 1), BitField('updown', 0, 1), BitField('external', 0, 1), BitField('subtlvindicator', 0, 1), BitField('reserved', 0, 5), FieldLenField('pfxlen', None, length_of='pfx', fmt='B'), IP6PrefixField('pfx', None, wordbytes=1, length_from=lambda x: x.pfxlen), ConditionalField(FieldLenField('subtlvslen', None, length_of='subtlvs', fmt='B'), lambda pkt: pkt.subtlvindicator == 1), ConditionalField(PacketListField('subtlvs', [], _ISIS_GuessSubTlvClass_2, length_from=lambda x: x.subtlvslen), lambda pkt: pkt.subtlvindicator == 1)]
...
class ISIS_Ipv6ReachabilityTlv(ISIS_GenericTlv):
    ...
    fields_desc = [ByteEnumField('type', 236, _isis_tlv_names), FieldLenField('len', None, length_of='pfxs', fmt='B'), PacketListField('pfxs', [], ISIS_Ipv6Prefix, length_from=lambda pkt: pkt.len)]

class ISIS_IsNeighbourTlv(ISIS_GenericTlv):
    name = 'ISIS IS Neighbour TLV'
    fields_desc = [ByteEnumField('type', 6, _isis_tlv_names), FieldLenField('len', None, length_of='neighbours', fmt='B'), FieldListField('neighbours', [], MACField('', '00.00.00.00.00.00'), count_from=lambda pkt: pkt.len // 6)]

class ISIS_LspEntry(Packet):
    name = 'ISIS LSP Entry'
    fields_desc = [ShortField('lifetime', 1200), ISIS_LspIdField('lspid', '0102.0304.0506.07-08'), XIntField('seqnum', 1), XShortField('checksum', None)]
...
class ISIS_LspEntryTlv(ISIS_GenericTlv):
    ...
    fields_desc = [ByteEnumField('type', 9, _isis_tlv_names), FieldLenField('len', None, length_of='entries', fmt='B'), PacketListField('entries', [], ISIS_LspEntry, count_from=lambda pkt: pkt.len // 16)]
...
class ISIS_P2PAdjacencyStateTlv(ISIS_GenericTlv):
    ...
    fields_desc = [ByteEnumField('type', 240, _isis_tlv_names), _AdjacencyStateTlvLenField('len', None, fmt='B'), ByteEnumField('state', 'Down', {2: 'Down', 1: 'Initialising', 0: 'Up'}), ConditionalField(IntField('extlocalcircuitid', None), lambda pkt: pkt.len >= 5), ConditionalField(ISIS_SystemIdField('neighboursystemid', None), lambda pkt: pkt.len >= 11), ConditionalField(IntField('neighbourextlocalcircuitid', None), lambda pkt: pkt.len == 15)]
...
class ISIS_ProtocolsSupportedTlv(ISIS_GenericTlv):
    ...
    fields_desc = [ByteEnumField('type', 129, _isis_tlv_names), FieldLenField('len', None, count_of='nlpids', fmt='B'), FieldListField('nlpids', [], ByteEnumField('', 'IPv4', network_layer_protocol_ids), count_from=lambda pkt: pkt.len)]

class ISIS_RouterCapabilityTlv(ISIS_GenericTlv):
    name = 'ISIS Router Capability TLV'
    fields_desc = [ByteEnumField('type', 242, _isis_tlv_names), FieldLenField('len', None, length_of='subtlvs', adjust=lambda pkt, x: x + 5, fmt='B'), IPField('routerid', '0.0.0.0'), FlagsField('flags', 0, 8, ['S', 'D', 'res1', 'res2', 'res3', 'res4', 'res5', 'res6']), PacketListField('subtlvs', [], _ISIS_GuessSubTlvClass_4, length_from=lambda pkt: pkt.len - 5)]

class ISIS_IpReachabilityEntry(Packet):
    name = 'ISIS IP Reachability'
    fields_desc = [ByteField('defmetric', 1), ByteField('delmetric', 128), ByteField('expmetric', 128), ByteField('errmetric', 128), IPField('ipaddress', '0.0.0.0'), IPField('subnetmask', '255.255.255.255')]
...
class ISIS_InternalIpReachabilityTlv(ISIS_GenericTlv):
    ...
    fields_desc = [ByteEnumField('type', 128, _isis_tlv_names), FieldLenField('len', None, length_of='entries', fmt='B'), PacketListField('entries', [], ISIS_IpReachabilityEntry, count_from=lambda x: x.len // 12)]

class ISIS_ExternalIpReachabilityTlv(ISIS_GenericTlv):
    name = 'ISIS External IP Reachability TLV'
    fields_desc = [ByteEnumField('type', 130, _isis_tlv_names), FieldLenField('len', None, length_of='entries', fmt='B'), PacketListField('entries', [], ISIS_IpReachabilityEntry, count_from=lambda x: x.len // 12)]

class ISIS_IsReachabilityEntry(Packet):
    name = 'ISIS IS Reachability'
    fields_desc = [ByteField('defmetric', 1), ByteField('delmetric', 128), ByteField('expmetric', 128), ByteField('errmetric', 128), ISIS_NodeIdField('neighbourid', '0102.0304.0506.07')]
...
class ISIS_IsReachabilityTlv(ISIS_GenericTlv):
    ...
    fields_desc = [ByteEnumField('type', 2, _isis_tlv_names), FieldLenField('len', None, fmt='B', length_of='neighbours', adjust=lambda pkt, x: x + 1), ByteField('virtual', 0), PacketListField('neighbours', [], ISIS_IsReachabilityEntry, count_from=lambda x: (x.len - 1) // 11)]
...
class ISIS_CommonHdr(Packet):
    ...
    fields_desc = [ByteEnumField('nlpid', 131, network_layer_protocol_ids), ByteField('hdrlen', None), ByteField('version', 1), ByteField('idlen', 0), ByteEnumField('pdutype', None, _isis_pdu_names), ByteField('pduversion', 1), ByteField('hdrreserved', 0), ByteField('maxareaaddr', 0)]
...
class _ISIS_LAN_HelloBase(_ISIS_PduBase):
    fields_desc = [ISIS_CircuitTypeField(), ISIS_SystemIdField('sourceid', '0102.0304.0506'), ShortField('holdingtime', 30), _ISIS_PduLengthField(), ByteField('priority', 1), ISIS_NodeIdField('lanid', '0000.0000.0000.00'), _ISIS_TlvListField()]
...
class _ISIS_LSP_Base(_ISIS_PduBase):
    fields_desc = [_ISIS_PduLengthField(), ShortField('lifetime', 1199), ISIS_LspIdField('lspid', '0102.0304.0506.00-00'), XIntField('seqnum', 1), XShortField('checksum', None), FlagsField('typeblock', 3, 8, ['L1', 'L2', 'OL', 'ADef', 'ADel', 'AExp', 'AErr', 'P']), _ISIS_TlvListField()]
    ...
    def answers(self, other):
        if False:
            for i in range(10):
                print('nop')
        return _snp_answers(self, other, 'ISIS_L2_LSP')
register_cln_protocol(131, ISIS_CommonHdr)
