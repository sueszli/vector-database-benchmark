...
class ZeroShotClassificationTest(unittest.TestCase):
    ...
    def test_run_with_direct_file_download(self):
        ...
        pipeline1 = SiameseUiePipeline(model, preprocessor=tokenizer, model_revision='v1.1')
        pipeline2 = pipeline(Tasks.siamese_uie, model=model, preprocessor=tokenizer, model_revision='v1.1')
        print(f'sentence: {self.sentence}\npipeline1:{pipeline1(input=self.sentence, schema=json.dumps(self.schema, ensure_ascii=False))}')
        print(f'sentence: {self.sentence}\npipeline2: {pipeline2(self.sentence, schema=self.schema)}')
    ...
    def test_run_with_model_from_modelhub(self):
        ...
        pipeline_ins = pipeline(task=Tasks.siamese_uie, model=model, preprocessor=tokenizer, model_revision='v1.1')
    ...
    def test_run_with_model_name(self):
        if False:
            print('Hello World!')
        pipeline_ins = pipeline(task=Tasks.siamese_uie, model=self.model_id, model_revision='v1.1')
        print(pipeline_ins(input=self.sentence, schema=self.schema))
