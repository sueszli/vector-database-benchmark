...
from .....payment.error_codes import TransactionCreateErrorCode, TransactionUpdateErrorCode
from .....payment.transaction_item_calculations import calculate_transaction_amount_based_on_events, recalculate_transaction_amounts
...
from .transaction_create import TransactionCreate, TransactionCreateInput, TransactionEventInput
...
class TransactionUpdate(TransactionCreate):
    ...
    class Arguments:
        ...
        transaction = TransactionUpdateInput(description='Input data required to create a new transaction object.')
        transaction_event = TransactionEventInput(description='Data that defines a transaction transaction.')

    class Meta:
        auto_permission_message = False
        description = 'Update transaction.' + ADDED_IN_34 + PREVIEW_FEATURE + '\n\nRequires the following permissions: ' + f'{AuthorizationFilters.OWNER.name} ' + f'and {PaymentPermissions.HANDLE_PAYMENTS.name} for apps, {PaymentPermissions.HANDLE_PAYMENTS.name} for staff users. Staff user cannot update a transaction that is owned by the app.'
        doc_category = DOC_CATEGORY_PAYMENTS
