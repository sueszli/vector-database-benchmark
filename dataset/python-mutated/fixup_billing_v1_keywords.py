...
class billingCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_billing_account': ('billing_account',), 'get_billing_account': ('name',), 'get_iam_policy': ('resource', 'options'), 'get_project_billing_info': ('name',), 'list_billing_accounts': ('page_size', 'page_token', 'filter'), 'list_project_billing_info': ('name', 'page_size', 'page_token'), 'list_services': ('page_size', 'page_token'), 'list_skus': ('parent', 'start_time', 'end_time', 'currency_code', 'page_size', 'page_token'), 'set_iam_policy': ('resource', 'policy', 'update_mask'), 'test_iam_permissions': ('resource', 'permissions'), 'update_billing_account': ('name', 'account', 'update_mask'), 'update_project_billing_info': ('name', 'project_billing_info')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=billingCallTransformer()):
    if False:
        while True:
            i = 10
