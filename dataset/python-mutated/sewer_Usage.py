...
if __name__ == '__main__':
    ...
    ACME_AUTH_STATUS_WAIT_PERIOD = int(data['dnssleep'])
    ...
    if os.path.isfile(os.path.join(ssl_home_path, 'account_key.key')):
        with open(os.path.join(ssl_home_path, 'account_key.key'), 'r') as account_key_file:
            account_key = account_key_file.read()
    else:
        account_key = None
    client = ACMEclient(account_key=account_key, Manual=Manual, contact_email=contact_email, dns_class=dns_class, domain_alt_names=domain_alt_names, ACME_AUTH_STATUS_WAIT_PERIOD=ACME_AUTH_STATUS_WAIT_PERIOD, ACME_AUTH_STATUS_MAX_CHECKS=2, ACME_DIRECTORY_URL='https://acme-v02.api.letsencrypt.org/directory', ACME_REQUEST_TIMEOUT=30)
