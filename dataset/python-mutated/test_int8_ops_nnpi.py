...
import datetime
core.GlobalInit(['caffe2', '--caffe2_log_level=-3', '--glow_global_fp16=1', '--glow_clip_quant_range_to_fp16=1', '--glow_global_fp16_constants=1'])
...
class Int8OpsTest(serial.SerializedTestCase):
    ...
    @given(n=st.integers(2, 1024), rand_seed=st.integers(0, 65534), non_zero_offset=st.booleans())
    ...
    def test_int8_quantize(self, n, rand_seed, non_zero_offset):
        ...
        if non_zero_offset:
            X_fp32 = np.random.uniform(-1, 1, size=(n, n)).astype(np.float16).astype(np.float32)
        else:
        ...
        ref_net.Int8FCFakeAcc32NNPI(['X_int8', 'W_int8', 'b'], ['Y_int8'], Y_scale=X_scale, Y_zero_point=X_zero_point)
        ...
        if not np.allclose(Y_glow, Y_fbgemm):
            ...
            print_test_debug_info('int8_fc', {'seed': rand_seed, 'n': n, 'X': X_fp32, 'W': W_fp32, 'b': b_fp32, 'Y_fbgemm': Y_fbgemm, 'Y_glow': Y_glow, 'diff': diff_Y, 'maxdiff': diff_Y.max(axis=1)})
            assert 0

    @given(n=st.integers(1, 1024), m=st.integers(1, 1024), k=st.integers(1, 1024), f=st.integers(1, 1), rand_seed=st.integers(0, 65534), quantize_bias=st.sampled_from([False]))
    @settings(deadline=datetime.timedelta(seconds=50))
    def test_int8_fc(self, n, m, k, rand_seed, quantize_bias, f):
        if False:
            print('Hello World!')
        print(f'n={n}, m={m}, k={k}, rand_seed={rand_seed}, quantize_bias={quantize_bias}')
        np.random.seed(rand_seed)
        workspace.ResetWorkspace()
        ff = float(f)
        ...
        workspace.FeedBlob('W', W_fp32)
        workspace.FeedBlob('b', b_fp32)
        workspace.RunOperatorOnce(core.CreateOperator('Int8FCPackWeight', ['W', 'b'] if quantize_bias else ['W'], ['W_int8', 'b_int32'] if quantize_bias else ['W_int8'], engine='DNNLOWP', save_unpacked_weights=True, in_scale=X_scale))
        ref_net = core.Net('net')
        ref_net.Int8QuantizeNNPI(['X'], ['X_int8'], Y_scale=X_scale, Y_zero_point=X_zero_point)
        ref_net.Int8FCFakeAcc32NNPI(['X_int8', 'W_int8', 'b_int32' if quantize_bias else 'b'], ['Y_int8'], Y_scale=Y_scale, Y_zero_point=Y_zero_point)
        ref_net.Int8DequantizeNNPI(['Y_int8'], ['Y'])
        ref_net.Proto().external_output.append('Y')
        workspace.RunNetOnce(ref_net)
        Y_fbgemm = workspace.FetchBlob('Y')
        ...
        ref_net.Proto().op[1].type = 'Int8FC'
        ref_net.Proto().op[2].type = 'Int8Dequantize'
        net_onnxified = onnxifi_caffe2_net(ref_net.Proto(), {}, debug=True, adjust_batch=False, use_onnx=False, weight_names=['W_int8', 'b_int32'] if quantize_bias else ['W_int8', 'b'])
        num_onnxified_ops = sum((1 if o.type == 'Onnxifi' else 0 for o in net_onnxified.op))
        np.testing.assert_equal(num_onnxified_ops, 1)
        workspace.CreateNet(net_onnxified)
        workspace.RunNet(net_onnxified.name)
        ...
        if not np.allclose(Y_glow, Y_fbgemm):
            diff_Y = np.abs(Y_glow - Y_fbgemm)
            print_test_debug_info('int8_fc', {'seed': rand_seed, 'n': n, 'm': m, 'k': k, 'X': X_fp32, 'W': W_fp32, 'b': b_fp32, 'Y_fbgemm': Y_fbgemm, 'Y_glow': Y_glow, 'diff': diff_Y, 'maxdiff': diff_Y.max(axis=1)})
            assert 0
