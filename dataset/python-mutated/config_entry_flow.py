    ...
    async def async_step_confirm(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if self.source == config_entries.SOURCE_USER:
            ...
            if not (has_devices := bool(in_progress)):
                has_devices = await cast('asyncio.Future[bool]', self.hass.async_add_job(self._discovery_function, self.hass))
            if not has_devices:
...
class WebhookFlowHandler(config_entries.ConfigFlow):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if 'cloud' in self.hass.config.components and self.hass.components.cloud.async_active_subscription():
            if not self.hass.components.cloud.async_is_connected():
                return self.async_abort(reason='cloud_not_connected')
            webhook_url = await self.hass.components.cloud.async_create_cloudhook(webhook_id)
            cloudhook = True
        else:
        ...
        return self.async_create_entry(title=self._title, data={'webhook_id': webhook_id, 'cloudhook': cloudhook}, description_placeholders=self._description_placeholder)
