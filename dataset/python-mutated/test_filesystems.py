...
class TestLocalFileSystem:
    ...
    async def test_dir_contents_copied_correctly_with_get_directory_relative_from_path(self, tmp_path):
        sub_dir_name = 'puppy'
    ...
    async def test_dir_contents_copied_correctly_with_put_directory(self, tmp_path):
        ...
        with TemporaryDirectory() as tmp_dst:
            ...
            await f.put_directory(local_path=tmp_path, to_path=tmp_dst)
            assert set(os.listdir(tmp_dst)) == set(parent_contents)
            assert set(os.listdir(Path(tmp_dst) / sub_dir_name)) == set(child_contents)
    ...
    async def test_to_path_modifies_base_path_correctly(self, tmp_path):
        ...
        with TemporaryDirectory() as tmp_dst:
            f = LocalFileSystem(basepath=Path(tmp_dst).parent)
            await f.put_directory(local_path=tmp_path, to_path=Path(tmp_dst).name)
            assert set(os.listdir(tmp_dst)) == set(parent_contents)
            assert set(os.listdir(Path(tmp_dst) / sub_dir_name)) == set(child_contents)
    ...
    async def test_to_path_raises_error_when_not_in_basepath(self, tmp_path):
        ...
        outside_path = '~/puppy'
        with pytest.raises(ValueError, match='Provided path .* is outside of the base path.*'):
            await f.put_directory(to_path=outside_path)

    async def test_dir_contents_copied_correctly_with_put_directory_and_file_pattern(self, tmp_path):
        ...
        with TemporaryDirectory() as tmp_dst:
            f = LocalFileSystem(basepath=Path(tmp_dst).parent)
            await f.put_directory(local_path=tmp_path, to_path=tmp_dst, ignore_file=ignore_fpath)
            assert set(os.listdir(tmp_dst)) == set(expected_contents)
            assert set(os.listdir(Path(tmp_dst) / sub_dir_name)) == set(child_contents)

    async def test_dir_contents_copied_correctly_with_put_directory_and_directory_pattern(self, tmp_path):
        """Make sure that ignore file behaves properly."""
        sub_dir_name = 'puppy'
        skip_sub_dir = 'kitty'
        (parent_contents, child_contents) = setup_test_directory(tmp_path, sub_dir_name)
...
class TestRemoteFileSystem:
    ...
    async def test_write_to_different_scheme(self):
        fs = RemoteFileSystem(basepath='memory://foo')
        with pytest.raises(ValueError, match="with scheme 'file' must use the same scheme as the base path 'memory'"):
    ...
    async def test_put_directory_flat(self):
        ...
        await fs.put_directory(os.path.join(TEST_PROJECTS_DIR, 'flat-project'), ignore_file=os.path.join(TEST_PROJECTS_DIR, 'flat-project', '.prefectignore'))
        copied_files = set(fs.filesystem.glob('/flat/**'))
        copied_files.discard('/flat')
        assert copied_files == {'/flat/explicit_relative.py', '/flat/implicit_relative.py', '/flat/shared_libs.py'}

    async def test_put_directory_tree(self):
        fs = RemoteFileSystem(basepath='memory://tree')
        await fs.put_directory(os.path.join(TEST_PROJECTS_DIR, 'tree-project'), ignore_file=os.path.join(TEST_PROJECTS_DIR, 'tree-project', '.prefectignore'))
        copied_files = set(fs.filesystem.glob('/tree/**'))
        copied_files.discard('/tree')
        assert copied_files == {'/tree/imports', '/tree/imports/explicit_relative.py', '/tree/imports/implicit_relative.py', '/tree/shared_libs', '/tree/shared_libs/bar.py', '/tree/shared_libs/foo.py', '/tree/.hidden'}

    async def test_put_directory_put_file_count(self):
        ignore_file = os.path.join(TEST_PROJECTS_DIR, 'tree-project', '.prefectignore')
        fs = RemoteFileSystem(basepath='memory://tree')
        num_files_put = await fs.put_directory(os.path.join(TEST_PROJECTS_DIR, 'tree-project'), ignore_file=ignore_file)
        ignore_patterns = Path(ignore_file).read_text().splitlines(keepends=False)
        included_files = prefect.utilities.filesystem.filter_files(os.path.join(TEST_PROJECTS_DIR, 'tree-project'), ignore_patterns, include_dirs=False)
        num_files_expected = len(included_files)
        assert num_files_put == num_files_expected

    @pytest.mark.parametrize('null_value', {None, ''})
    async def test_get_directory_empty_local_path_uses_cwd(self, tmp_path: Path, null_value):
    ...
    @pytest.mark.parametrize('null_value', {None, ''})
    async def test_get_directory_empty_from_path_uses_basepath(self, tmp_path: Path, null_value):
    ...
    @pytest.mark.parametrize('null_value', {None, ''})
    async def test_put_directory_empty_local_path_uses_cwd(self, tmp_path: Path, null_value):
    ...
    @pytest.mark.parametrize('null_value', {None, ''})
    async def test_put_directory_empty_from_path_uses_basepath(self, tmp_path: Path, null_value):
...
class TestGitHub:
    ...
    async def test_subprocess_errors_are_surfaced(self):
        ...
        with pytest.raises(OSError, match="fatal: repository 'incorrect-url-scheme' does not exist"):
    ...
    async def test_token_added_correctly_from_credential(self, monkeypatch):
        ...
        g = GitHub(repository=repo, access_token=credential)
        await g.get_directory()
        assert mock.await_count == 1
        expected_cmd = ['git', 'clone', f'https://{credential}@github.com/PrefectHQ/prefect.git', '--depth', '1']
        assert mock.await_args[0][0][:len(expected_cmd)] == expected_cmd
    ...
    async def test_ssh_fails_with_credential(self, monkeypatch):
        ...
        monkeypatch.setattr(prefect.filesystems, 'run_process', mock)
        credential = 'XYZ'
        error_msg = "Crendentials can only be used with GitHub repositories using the 'HTTPS' format"
        with pytest.raises(InvalidRepositoryURLError, match=error_msg):
            GitHub(repository='git@github.com:PrefectHQ/prefect.git', access_token=credential)

    async def test_dir_contents_copied_correctly_with_get_directory(self, monkeypatch, tmp_path):
        ...
        with TemporaryDirectory() as tmp_dst:
            monkeypatch.setattr(prefect.filesystems, 'TemporaryDirectory', self.MockTmpDir)
            g = GitHub(repository='https://github.com/PrefectHQ/prefect.git')
    ...
    async def test_dir_contents_copied_correctly_with_get_directory_and_from_path(self, monkeypatch, tmp_path):
        """Check that `get_directory` is able to correctly copy contents from src->dst
    ...
    @pytest.mark.parametrize('include_git_objects, expect_git_objects', [(True, True), (False, False), (None, True)])
    async def test_dir_contents_copied_correctly_with_include_git_object(self, monkeypatch, tmp_path, include_git_objects, expect_git_objects):
        """Check that `get_directory` is able to correctly copy contents from src->dst
        ...
        with TemporaryDirectory() as tmp_dst:
            ...
            if include_git_objects is None:
                ...
                g = GitHub(repository='https://github.com/PrefectHQ/prefect.git', include_git_objects=include_git_objects)
            await g.get_directory(local_path=tmp_dst)
