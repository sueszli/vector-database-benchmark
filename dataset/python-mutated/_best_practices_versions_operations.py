...
def build_get_request(best_practice_name: str, version_name: str, **kwargs: Any) -> HttpRequest:
    ...
    _url = kwargs.pop('template_url', '/providers/Microsoft.Automanage/bestPractices/{bestPracticeName}/versions/{versionName}')
    path_format_arguments = {'bestPracticeName': _SERIALIZER.url('best_practice_name', best_practice_name, 'str'), 'versionName': _SERIALIZER.url('version_name', version_name, 'str')}
...
def build_list_by_tenant_request(best_practice_name: str, **kwargs: Any) -> HttpRequest:
    ...
    path_format_arguments = {'bestPracticeName': _SERIALIZER.url('best_practice_name', best_practice_name, 'str')}
    _url: str = _format_url_section(_url, **path_format_arguments)
    _params['api-version'] = _SERIALIZER.query('api_version', api_version, 'str')
    _headers['Accept'] = _SERIALIZER.header('accept', accept, 'str')
    return HttpRequest(method='GET', url=_url, params=_params, headers=_headers, **kwargs)
...
class BestPracticesVersionsOperations:
    ...
    def get(self, best_practice_name: str, version_name: str, **kwargs: Any) -> _models.BestPractice:
        ...
        request = build_get_request(best_practice_name=best_practice_name, version_name=version_name, api_version=api_version, template_url=self.get.metadata['url'], headers=_headers, params=_params)
        ...
        def prepare_request(next_link=None):
            if False:
                while True:
                    i = 10
            if not next_link:
                request = build_list_by_tenant_request(best_practice_name=best_practice_name, api_version=api_version, template_url=self.list_by_tenant.metadata['url'], headers=_headers, params=_params)
                request = _convert_request(request)
                request.url = self._client.format_url(request.url)
            else:
