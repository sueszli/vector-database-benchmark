...
TZSP_PORT_DEFAULT = 37008

class TZSP(Packet):
    TYPE_RX_PACKET = 0
    TYPE_TX_PACKET = 1
    TYPE_CONFIG = 3
    TYPE_KEEPALIVE = TYPE_NULL = 4
    TYPE_PORT = 5
    TYPES = {TYPE_RX_PACKET: 'RX_PACKET', TYPE_TX_PACKET: 'TX_PACKET', TYPE_CONFIG: 'CONFIG', TYPE_NULL: 'KEEPALIVE/NULL', TYPE_PORT: 'PORT'}
    ...
    fields_desc = [ByteField('version', 1), ByteEnumField('type', TYPE_RX_PACKET, TYPES), ShortEnumField('encapsulated_protocol', ENCAPSULATED_ETHERNET, ENCAPSULATED_PROTOCOLS)]
...
class _TZSPTag(Packet):
    TAG_TYPE_PADDING = 0
    TAG_TYPE_END = 1
    TAG_TYPE_RAW_RSSI = 10
    TAG_TYPE_SNR = 11
    TAG_TYPE_DATA_RATE = 12
    TAG_TYPE_TIMESTAMP = 13
    TAG_TYPE_CONTENTION_FREE = 15
    TAG_TYPE_DECRYPTED = 16
    TAG_TYPE_FCS_ERROR = 17
    TAG_TYPE_RX_CHANNEL = 18
    TAG_TYPE_PACKET_COUNT = 40
    TAG_TYPE_RX_FRAME_LENGTH = 41
    TAG_TYPE_WLAN_RADIO_HDR_SERIAL = 60
    TAG_TYPES = {TAG_TYPE_PADDING: 'PADDING', TAG_TYPE_END: 'END', TAG_TYPE_RAW_RSSI: 'RAW_RSSI', TAG_TYPE_SNR: 'SNR', TAG_TYPE_DATA_RATE: 'DATA_RATE', TAG_TYPE_TIMESTAMP: 'TIMESTAMP', TAG_TYPE_CONTENTION_FREE: 'CONTENTION_FREE', TAG_TYPE_DECRYPTED: 'DECRYPTED', TAG_TYPE_FCS_ERROR: 'FCS_ERROR', TAG_TYPE_RX_CHANNEL: 'RX_CHANNEL', TAG_TYPE_PACKET_COUNT: 'PACKET_COUNT', TAG_TYPE_RX_FRAME_LENGTH: 'RX_FRAME_LENGTH', TAG_TYPE_WLAN_RADIO_HDR_SERIAL: 'WLAN_RADIO_HDR_SERIAL'}
...
class TZSPTagPadding(_TZSPTag):
    ...
    """
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_PADDING, _TZSPTag.TAG_TYPES)]

class TZSPTagEnd(Packet):
    """
    last tag
    """
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_END, _TZSPTag.TAG_TYPES)]
...
class TZSPTagContentionFree(_TZSPTag):
    ...
    NO = 0
    YES = 1
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_CONTENTION_FREE, _TZSPTag.TAG_TYPES), ByteField('len', 1), YesNoByteField('contention_free', NO)]
...
class TZSPTagDecrypted(_TZSPTag):
    ...
    YES = 0
    NO = 1
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_DECRYPTED, _TZSPTag.TAG_TYPES), ByteField('len', 1), YesNoByteField('decrypted', NO, config={'yes': YES, 'no': (NO, 255)})]
...
class TZSPTagError(_TZSPTag):
    ...
    NO = 0
    YES = 1
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_FCS_ERROR, _TZSPTag.TAG_TYPES), ByteField('len', 1), YesNoByteField('fcs_error', NO, config={'no': NO, 'yes': YES, 'reserved': (YES + 1, 255)})]

class TZSPTagRXChannel(_TZSPTag):
    """
    channel the sensor was on while receiving the frame
    """
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_RX_CHANNEL, _TZSPTag.TAG_TYPES), ByteField('len', 1), ByteField('rx_channel', 0)]

class TZSPTagPacketCount(_TZSPTag):
    """
    packet counter
    """
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_PACKET_COUNT, _TZSPTag.TAG_TYPES), ByteField('len', 4), IntField('packet_count', 0)]

class TZSPTagRXFrameLength(_TZSPTag):
    """
    received packet length
    """
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_RX_FRAME_LENGTH, _TZSPTag.TAG_TYPES), ByteField('len', 2), ShortField('rx_frame_length', 0)]

class TZSPTagWlanRadioHdrSerial(_TZSPTag):
    """
    (vendor specific) unique capture device (sensor/AP) identifier
    """
    fields_desc = [ByteEnumField('type', _TZSPTag.TAG_TYPE_WLAN_RADIO_HDR_SERIAL, _TZSPTag.TAG_TYPES), FieldLenField('len', None, length_of='sensor_id', fmt='b'), StrLenField('sensor_id', '', length_from=lambda pkt: pkt.len)]

class TZSPTagUnknown(_TZSPTag):
    """
    unknown tag type dummy
    """
    fields_desc = [ByteField('type', 255), FieldLenField('len', None, length_of='data', fmt='b'), XStrLenField('data', '', length_from=lambda pkt: pkt.len)]
_TZSP_TAG_CLASSES = {_TZSPTag.TAG_TYPE_PADDING: TZSPTagPadding, _TZSPTag.TAG_TYPE_END: TZSPTagEnd, _TZSPTag.TAG_TYPE_RAW_RSSI: {1: TZSPTagRawRSSIByte, 2: TZSPTagRawRSSIShort}, _TZSPTag.TAG_TYPE_SNR: {1: TZSPTagSNRByte, 2: TZSPTagSNRShort}, _TZSPTag.TAG_TYPE_DATA_RATE: TZSPTagDataRate, _TZSPTag.TAG_TYPE_TIMESTAMP: TZSPTagTimestamp, _TZSPTag.TAG_TYPE_CONTENTION_FREE: TZSPTagContentionFree, _TZSPTag.TAG_TYPE_DECRYPTED: TZSPTagDecrypted, _TZSPTag.TAG_TYPE_FCS_ERROR: TZSPTagError, _TZSPTag.TAG_TYPE_RX_CHANNEL: TZSPTagRXChannel, _TZSPTag.TAG_TYPE_PACKET_COUNT: TZSPTagPacketCount, _TZSPTag.TAG_TYPE_RX_FRAME_LENGTH: TZSPTagRXFrameLength, _TZSPTag.TAG_TYPE_WLAN_RADIO_HDR_SERIAL: TZSPTagWlanRadioHdrSerial}