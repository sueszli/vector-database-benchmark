...
from ray.rllib.offline.estimators.tests.utils import get_cliff_walking_wall_policy_and_data, check_estimate
SEED = 0
...
class TestDMLearning(unittest.TestCase):
    ...
    def test_dm_random_policy_random_data(self):
        ...
        check_estimate(estimator_cls=DirectMethod, gamma=self.gamma, q_model_config=self.q_model_config, policy=self.random_policy, batch=self.random_batch, mean_ret=self.random_reward, std_ret=self.random_std, seed=SEED)
    ...
    def test_dm_random_policy_mixed_data(self):
        ...
        check_estimate(estimator_cls=DirectMethod, gamma=self.gamma, q_model_config=self.q_model_config, policy=self.random_policy, batch=self.mixed_batch, mean_ret=self.random_reward, std_ret=self.random_std, seed=SEED)

    def test_dm_mixed_policy_random_data(self):
        if False:
            return 10
        print('Test DirectMethod on mixed policy on random dataset')
        check_estimate(estimator_cls=DirectMethod, gamma=self.gamma, q_model_config=self.q_model_config, policy=self.mixed_policy, batch=self.random_batch, mean_ret=self.mixed_reward, std_ret=self.mixed_std, seed=SEED)
    ...
    def test_dm_mixed_policy_mixed_data(self):
        ...
        check_estimate(estimator_cls=DirectMethod, gamma=self.gamma, q_model_config=self.q_model_config, policy=self.mixed_policy, batch=self.mixed_batch, mean_ret=self.mixed_reward, std_ret=self.mixed_std, seed=SEED)

    def test_dm_mixed_policy_expert_data(self):
        if False:
            return 10
        print('Test DirectMethod on mixed policy on expert dataset')
        check_estimate(estimator_cls=DirectMethod, gamma=self.gamma, q_model_config=self.q_model_config, policy=self.mixed_policy, batch=self.expert_batch, mean_ret=self.mixed_reward, std_ret=self.mixed_std, seed=SEED)

    def test_dm_expert_policy_random_data(self):
        if False:
            return 10
        print('Test DirectMethod on expert policy on random dataset')
        check_estimate(estimator_cls=DirectMethod, gamma=self.gamma, q_model_config=self.q_model_config, policy=self.expert_policy, batch=self.random_batch, mean_ret=self.expert_reward, std_ret=self.expert_std, seed=SEED)

    def test_dm_expert_policy_mixed_data(self):
        if False:
            print('Hello World!')
        print('Test DirectMethod on expert policy on mixed dataset')
        check_estimate(estimator_cls=DirectMethod, gamma=self.gamma, q_model_config=self.q_model_config, policy=self.expert_policy, batch=self.mixed_batch, mean_ret=self.expert_reward, std_ret=self.expert_std, seed=SEED)
    ...
    def test_dm_expert_policy_expert_data(self):
        ...
        check_estimate(estimator_cls=DirectMethod, gamma=self.gamma, q_model_config=self.q_model_config, policy=self.expert_policy, batch=self.expert_batch, mean_ret=self.expert_reward, std_ret=self.expert_std, seed=SEED)
