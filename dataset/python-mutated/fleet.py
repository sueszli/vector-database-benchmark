...
class FleetVehicleCost(models.Model):
    ...
    cost_type = fields.Selection([('contract', 'Contract'), ('services', 'Services'), ('fuel', 'Fuel'), ('other', 'Other')], 'Category of the cost', default='other', help='For internal purpose only', required=True)
...
class FleetVehicleModelBrand(models.Model):
    ...
    image = fields.Binary('Logo', attachment=True, help='This field holds the image used as logo for the brand, limited to 1024x1024px.')
    image_medium = fields.Binary('Medium-sized image', attachment=True, help='Medium-sized logo of the brand. It is automatically resized as a 128x128px image, with aspect ratio preserved. Use this field in form views or some kanban views.')
    image_small = fields.Binary('Small-sized image', attachment=True, help='Small-sized logo of the brand. It is automatically resized as a 64x64px image, with aspect ratio preserved. Use this field anywhere a small image is required.')
...
class FleetVehicle(models.Model):
    ...
    odometer_unit = fields.Selection([('kilometers', 'Kilometers'), ('miles', 'Miles')], 'Odometer Unit', default='kilometers', help='Unit of the odometer ', required=True)
...
class FleetVehicleLogContract(models.Model):
    ...
    expiration_date = fields.Date('Contract Expiration Date', default=lambda self: self.compute_next_year_date(fields.Date.context_today(self)), help='Date when the coverage of the contract expirates (by default, one year after begin date)')
    ...
    state = fields.Selection([('open', 'In Progress'), ('toclose', 'To Close'), ('closed', 'Terminated')], 'Status', default='open', readonly=True, help='Choose wheter the contract is still valid or not', copy=False)
    ...
    cost_frequency = fields.Selection([('no', 'No'), ('daily', 'Daily'), ('weekly', 'Weekly'), ('monthly', 'Monthly'), ('yearly', 'Yearly')], 'Recurring Cost Frequency', default='no', help='Frequency of the recuring cost', required=True)
