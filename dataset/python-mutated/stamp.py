...
class ErrorEstimate(Packet):
    ...
    """
    name = 'Error Estimate'
    fields_desc = [BitEnumField('S', 0, 1, _sync_types), BitEnumField('Z', 0, 1, _timestamp_types), BitField('scale', 0, 6), ByteField('multiplier', 1)]
...
class STAMPTestTLV(Packet):
    ...
    fields_desc = [FlagsField('flags', 0, 8, 'UMIRRRRR'), ByteEnumField('type', None, _stamp_tlvs), ShortField('len', 0), StrLenField('value', '', length_from=lambda pkt: pkt.len)]
...
class STAMPSessionSenderTestUnauthenticated(Packet):
    ...
    fields_desc = [IntField('seq', 0), MultipleTypeField([(TimeStampField('ts', 0), lambda pkt: pkt.err_estimate.Z == 0)], UTCTimeField('ts', 0, fmt='Q')), PacketField('err_estimate', ErrorEstimate(), ErrorEstimate), ShortField('ssid', 1), NBytesField('mbz', 0, 28), PacketListField('tlv_objects', [], STAMPTestTLV)]
