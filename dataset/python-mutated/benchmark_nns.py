...
if __name__ == '__main__':
    ...
    parser.add_argument('--search_type', type=str, default='knn', choices=['knn', 'radius', 'hybrid', 'all'])
    ...
    if args.search_type == 'all':
        methods = [NNS(o3d_cuda_dev, 'knn', 'int'), NNS(o3d_cuda_dev, 'knn', 'long'), NNS(o3d_cuda_dev, 'radius', 'int'), NNS(o3d_cuda_dev, 'radius', 'long'), NNS(o3d_cuda_dev, 'hybrid', 'int'), NNS(o3d_cuda_dev, 'hybrid', 'long')]
    else:
        methods = [NNS(o3d_cuda_dev, args.search_type, 'int'), NNS(o3d_cuda_dev, args.search_type, 'long')]
    ...
    for method in methods:
        ...
        for (example_name, example) in datasets.items():
            ...
            for (knn, radius) in zip(neighbors, radii):
                ...
                memory = measure_memory(lambda : method.search(index, queries, dict(knn=knn, radius=radius)), handle)
    ...
    for (idx, result) in enumerate(results):
        ...
        for value in result.values():
            if value['num_points'] == int(np.power(10, 4)):
                ...
                raise ValueError
        ax1.plot(ks[0], ms[0], marker=lines[idx], color=colors[idx], label=dtypes[idx])
        ax2.plot(ks[1], ms[1], marker=lines[idx], color=colors[idx], label=dtypes[idx])
        ax3.plot(ks[2], ms[2], marker=lines[idx], color=colors[idx], label=dtypes[idx])
        ax4.plot(ks[3], ms[3], marker=lines[idx], color=colors[idx], label=dtypes[idx])
