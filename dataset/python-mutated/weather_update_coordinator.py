...
class WeatherUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _get_owm_weather(self):
        ...
        if self.forecast_mode in (FORECAST_MODE_ONECALL_HOURLY, FORECAST_MODE_ONECALL_DAILY):
            weather = await self.hass.async_add_executor_job(self._owm_client.one_call, self._latitude, self._longitude)
        else:
            weather = await self.hass.async_add_executor_job(self._get_legacy_weather_and_forecast)
        return weather
