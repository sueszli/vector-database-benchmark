...
class ChannelReorderWarehouses(BaseMutation):
    channel = graphene.Field(Channel, description='Channel within the warehouses are reordered.')

    class Arguments:
        channel_id = graphene.ID(description='ID of a channel.', required=True)
        moves = NonNullList(ReorderInput, required=True, description='The list of reordering operations for the given channel warehouses.')
