...
VALIDATE_CREDENTIAL = {'minimax_group_id': 'fake-group-id', 'minimax_api_key': 'valid_key'}
...
def test_is_provider_credentials_valid_or_raise_valid(mocker):
    if False:
        for i in range(10):
            print('nop')
    mocker.patch('core.third_party.langchain.llms.minimax_llm.MinimaxChatLLM._generate', return_value=ChatResult(generations=[ChatGeneration(message=AIMessage(content='abc'))]))
...
def test_get_credentials_custom(mock_decrypt):
    ...
    provider = Provider(id='provider_id', tenant_id='tenant_id', provider_name=PROVIDER_NAME, provider_type=ProviderType.CUSTOM.value, encrypted_config=json.dumps(encrypted_credential), is_valid=True)
