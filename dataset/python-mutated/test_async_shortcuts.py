...
class GetListObjectOr404Test(TestCase):
    ...
    async def test_aget_object_or_404(self):
        ...
        self.assertEqual(await aget_object_or_404(SimpleModel.objects.all(), field=1), self.s2)
        self.assertEqual(await aget_object_or_404(self.s1.relatedmodel_set, pk=self.r1.pk), self.r1)
    ...
    async def test_get_list_or_404(self):
        ...
        self.assertEqual(await aget_list_or_404(SimpleModel.objects.all(), field=1), [self.s2])
        self.assertEqual(await aget_list_or_404(self.s1.relatedmodel_set, pk=self.r1.pk), [self.r1])
        msg = 'No SimpleModel matches the given query.'
        with self.assertRaisesMessage(Http404, msg):
            await aget_list_or_404(SimpleModel, field=2)

    async def test_get_object_or_404_bad_class(self):
        msg = "First argument to aget_object_or_404() must be a Model, Manager, or QuerySet, not 'str'."
        with self.assertRaisesMessage(ValueError, msg):
            await aget_object_or_404('SimpleModel', field=0)

    async def test_get_list_or_404_bad_class(self):
        msg = "First argument to aget_list_or_404() must be a Model, Manager, or QuerySet, not 'list'."
        with self.assertRaisesMessage(ValueError, msg):
