...
re_reference_remove = re.compile('\\[\\d+(?:,\\s*\\d+)*?\\]')
re_single_reference_remove = re.compile('\\[\\s?\\d+\\s?\\]')
re_whitespace_newline_match = re.compile('^[\\s\\n]*$')
LINKING_CHARS = ['\n', '\n\n', ' ']
...
class QADataset(Dataset):
    ...
    """
    DATASET_FORMAT_MAPPING = {'squad_v2': {'index_fn': index_squad_v2}, 'ua_squad': {'index_fn': index_uasquad, 'name': 'FIdo-AI/ua-squad', 'params': {'field': 'data'}, 'no_val': True}, 'trivia_qa_nocontext': {'index_fn': index_trivia_qa_nocontext, 'name': 'trivia_qa', 'params': {'name': 'rc.nocontext'}}, 'trivia_qa_context': {'index_fn': index_trivia_qa_context, 'name': 'trivia_qa', 'params': {'name': 'rc'}}, 'adversarial_qa': {'index_fn': index_adversarial_qa, 'params': {'name': 'adversarialQA'}}, 'gsm8k_hard': {'index_fn': index_gsm_hard, 'name': 'reasoning-machines/gsm-hard', 'no_val': True}, 'gsm8k': {'index_fn': index_gsm8k, 'params': {'name': 'main'}, 'validation': 'test'}, 'wikihow': {'name': 'b-mc2/wikihow_lists', 'index_fn': index_wikihow, 'no_val': True}, 'essay_instruction': {'name': 'ChristophSchuhmann/essays-with-instructions', 'index_fn': index_essay_instruction, 'no_val': True}, 'math_qa': {'index_fn': index_math_qa}, 'reddit_eli5': {'name': 'eli5', 'index_fn': index_eli5, 'split_postfix': '_eli5'}, 'reddit_askh': {'name': 'eli5', 'index_fn': index_eli5, 'split_postfix': '_askh'}, 'reddit_asks': {'name': 'eli5', 'index_fn': index_eli5, 'split_postfix': '_asks'}}
