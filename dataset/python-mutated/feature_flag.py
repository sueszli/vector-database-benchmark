...
from posthog.models.feature_flag import FeatureFlagDashboards, can_user_edit_feature_flag, get_all_feature_flags, get_user_blast_radius
...
DATABASE_FOR_LOCAL_EVALUATION = 'default' if 'local_evaluation' not in settings.READ_REPLICA_OPT_IN or 'replica' not in settings.DATABASES else 'replica'
...
class FeatureFlagSerializer(TaggedItemSerializerMixin, serializers.HyperlinkedModelSerializer):
    ...
    analytics_dashboards = serializers.PrimaryKeyRelatedField(many=True, required=False, queryset=Dashboard.objects.all())
    name = serializers.CharField(required=False, allow_blank=True, help_text='contains the description for the flag (field name `name` is kept for backwards-compatibility)')
    can_edit = serializers.SerializerMethodField()

    class Meta:
        model = FeatureFlag
        fields = ['id', 'name', 'key', 'filters', 'deleted', 'active', 'created_by', 'created_at', 'is_simple_flag', 'rollout_percentage', 'ensure_experience_continuity', 'experiment_set', 'surveys', 'features', 'rollback_conditions', 'performed_rollback', 'can_edit', 'tags', 'usage_dashboard', 'analytics_dashboards', 'has_enriched_analytics']
...
class MinimalFeatureFlagSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = FeatureFlag
        fields = ['id', 'team_id', 'name', 'key', 'filters', 'deleted', 'active', 'ensure_experience_continuity']
...
class FeatureFlagViewSet(TaggedItemViewSetMixin, StructuredViewSetMixin, ForbidDestroyModel, viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, ProjectMembershipNecessaryPermissions, TeamMemberAccessPermission, CanEditFeatureFlag]
    authentication_classes = [PersonalAPIKeyAuthentication, TemporaryTokenAuthentication, authentication.SessionAuthentication, authentication.BasicAuthentication]
