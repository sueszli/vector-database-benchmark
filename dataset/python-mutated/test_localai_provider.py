...
VALIDATE_CREDENTIAL = {'server_url': 'http://127.0.0.1:8080/'}
...
def test_is_credentials_valid_or_raise_valid(mocker):
    if False:
        for i in range(10):
            print('nop')
    mocker.patch('langchain.embeddings.localai.LocalAIEmbeddings.embed_query', return_value='abc')
    MODEL_PROVIDER_CLASS.is_model_credentials_valid_or_raise(model_name='username/test_model_name', model_type=ModelType.EMBEDDINGS, credentials=VALIDATE_CREDENTIAL.copy())
...
def test_is_credentials_valid_or_raise_invalid():
    ...
    with pytest.raises(CredentialsValidateFailedError):
        MODEL_PROVIDER_CLASS.is_model_credentials_valid_or_raise(model_name='test_model_name', model_type=ModelType.EMBEDDINGS, credentials={})
...
def test_encrypt_model_credentials(mock_encrypt, mocker):
    ...
    result = MODEL_PROVIDER_CLASS.encrypt_model_credentials(tenant_id='tenant_id', model_name='test_model_name', model_type=ModelType.EMBEDDINGS, credentials=VALIDATE_CREDENTIAL.copy())
...
def test_get_model_credentials_custom(mock_decrypt, mocker):
    ...
    result = model_provider.get_model_credentials(model_name='test_model_name', model_type=ModelType.EMBEDDINGS)
...
def test_get_model_credentials_obfuscated(mock_decrypt, mocker):
    ...
    mocker.patch('extensions.ext_database.db.session.query', return_value=mock_query)
    model_provider = MODEL_PROVIDER_CLASS(provider=provider)
    result = model_provider.get_model_credentials(model_name='test_model_name', model_type=ModelType.EMBEDDINGS, obfuscated=True)
