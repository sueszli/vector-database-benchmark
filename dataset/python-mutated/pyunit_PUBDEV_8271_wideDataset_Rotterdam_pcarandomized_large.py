...
def pca_wideDataset_rotterdam_pcarandomized():
    ...
    randomizedPCA = H2OPCA(k=8, impute_missing=True, transform='NONE', pca_method='Randomized', seed=12345, max_iterations=5)
    randomizedPCA.train(x=x, training_frame=rotterdamH2O)
    print('@@@@@@  Comparing eigenvalues between GramSVD and Randomized...\n')
    pyunit_utils.assert_H2OTwoDimTable_equal(gramSVD._model_json['output']['importance'], randomizedPCA._model_json['output']['importance'], ['Standard deviation', 'Cumulative Proportion', 'Cumulative Proportion'], tolerance=0.1, check_all=False)
    print('@@@@@@  Comparing eigenvectors between GramSVD and Randomized...\n')
    pyunit_utils.assert_H2OTwoDimTable_equal(gramSVD._model_json['output']['eigenvectors'], randomizedPCA._model_json['output']['eigenvectors'], randomizedPCA._model_json['output']['names'], tolerance=tol, check_sign=True, check_all=False)
