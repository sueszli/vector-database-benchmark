...
from video_annotation_utils import parse_video_file_name, read_classes_file, create_clip_file_name, get_clip_action_label, extract_clip, extract_contiguous_negative_clips, extract_sampled_negative_clips
...
if __name__ == '__main__':
    ...
    parser.add_argument('--annotation_filepath', help='CSV filepath from the annotator', required=True)
    parser.add_argument('--video_dir', help='Input video dirictory', required=True)
    parser.add_argument('--clip_dir', help='Output directory where the extracted clips will be stored', required=True)
    parser.add_argument('--classes_filepath', help='Path to file defining classes and class IDs', required=True)
    parser.add_argument('--label_filepath', help='Path where the label file will be stored', required=True)
    parser.add_argument('--clip_format', default='mp4')
    parser.add_argument('--no_action_class', help='Label for the no action class. Provide this argument to create negative examples.')
    parser.add_argument('--contiguous', help='Set to true to extract all non-overlapping negative samples. Otherwise extract num_negative_samples randomly sampled negative clips.', action='store_true', default=False)
    parser.add_argument('--negative_clip_length', type=float, help='The length of negative samples to extract', default=2.0)
    parser.add_argument('--negative_clip_margin', type=float, help='The length around the positive samples to be ignored for negative sampling', default=3.0)
    parser.add_argument('--sample_annotated_only', help='Source negative clips only from videos that have at least one positive action (only for non-contiguous samples)', action='store_true', default=True)
    parser.add_argument('--num_negative_samples', type=float, help='The number of negative clips to sample. This only applies to non-contiguous sampling.', default=0.0)
    args = parser.parse_args()
    main(annotation_filepath=args.annotation_filepath, video_dir=args.video_dir, clip_dir=args.clip_dir, classes_filepath=args.classes_filepath, label_filepath=args.label_filepath, clip_format=args.clip_format, no_action_class=args.no_action_class, contiguous=args.contiguous, negative_clip_length=args.negative_clip_length, negative_clip_margin=args.negative_clip_margin, sample_annotated_only=args.sample_annotated_only, num_negative_samples=args.num_negative_samples)