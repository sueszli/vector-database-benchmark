...
class DinatModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (DinatModel, DinatForImageClassification, DinatBackbone) if is_torch_available() else ()
    pipeline_model_mapping = {'feature-extraction': DinatModel, 'image-classification': DinatForImageClassification} if is_torch_available() else {}
    fx_compatible = False
    ...
    def check_hidden_states_output(self, inputs_dict, config, model_class, image_size):
        ...
        self.assertListEqual(list(hidden_states[0].shape[-3:]), [height, width, self.model_tester.embed_dim])
        if model_class.__name__ != 'DinatBackbone':
            reshaped_hidden_states = outputs.reshaped_hidden_states
            self.assertEqual(len(reshaped_hidden_states), expected_num_layers)
            (batch_size, num_channels, height, width) = reshaped_hidden_states[0].shape
            reshaped_hidden_states = reshaped_hidden_states[0].view(batch_size, num_channels, height, width).permute(0, 2, 3, 1)
            self.assertListEqual(list(reshaped_hidden_states.shape[-3:]), [height, width, self.model_tester.embed_dim])
