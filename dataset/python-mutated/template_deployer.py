...
from localstack.services.cloudformation.deployment_utils import PLACEHOLDER_AWS_NO_VALUE, get_action_name_for_resource_change, remove_none_values
...
from localstack.services.cloudformation.engine.template_utils import AWS_URL_SUFFIX, fn_equals_type_conversion, get_deps_for_resource
from localstack.services.cloudformation.resource_provider import Credentials, OperationStatus, ResourceProviderExecutor, ResourceProviderPayload, get_resource_type
from localstack.services.cloudformation.service_models import DependencyNotYetSatisfied, GenericBaseModel
...
REGEX_OUTPUT_APIGATEWAY = re.compile(f'^(https?://.+\\.execute-api\\.)(?:[^-]+-){{2,3}}\\d\\.(amazonaws\\.com|{AWS_URL_SUFFIX})/?(.*)$')
REGEX_DYNAMIC_REF = re.compile('{{resolve:([^:]+):(.+)}}')
LOG = logging.getLogger(__name__)
STATIC_REFS = ['AWS::Region', 'AWS::Partition', 'AWS::StackName', 'AWS::AccountId']
RESOURCE_MODELS: dict[str, Type[GenericBaseModel]] = {model.cloudformation_type(): model for model in get_all_subclasses(GenericBaseModel)}
