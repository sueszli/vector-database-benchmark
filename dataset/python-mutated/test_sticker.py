...
import pytest
from telegram import Audio, Bot, File, InputFile, InputSticker, MaskPosition, PhotoSize, Sticker, StickerSet
...
class TestStickerBase:
    sticker_file_url = 'https://github.com/python-telegram-bot/python-telegram-bot/blob/master/tests/data/telegram.webp?raw=true'
    emoji = '💪'
    width = 510
    height = 512
    is_animated = False
...
class TestStickerWithRequest(TestStickerBase):
    ...
    async def test_send_all_args(self, bot, chat_id, sticker_file, sticker):
        message = await bot.send_sticker(chat_id, sticker=sticker_file, disable_notification=False, protect_content=True)
    ...
    async def test_send_with_emoji(self, bot, chat_id):
        message = await bot.send_sticker(chat_id=chat_id, sticker=data_file('telegram.jpg'), emoji=self.emoji)
    ...
    async def test_send_sticker_default_allow_sending_without_reply(self, default_bot, chat_id, sticker, custom):
        reply_to_message = await default_bot.send_message(chat_id, 'test')
        await reply_to_message.delete()
        if custom is not None:
            message = await default_bot.send_sticker(chat_id, sticker, allow_sending_without_reply=custom, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        elif default_bot.defaults.allow_sending_without_reply:
            message = await default_bot.send_sticker(chat_id, sticker, reply_to_message_id=reply_to_message.message_id)
            assert message.reply_to_message is None
        else:
            with pytest.raises(BadRequest, match='Message to reply not found'):
                await default_bot.send_sticker(chat_id, sticker, reply_to_message_id=reply_to_message.message_id)

    @pytest.mark.parametrize('default_bot', [{'protect_content': True}], indirect=True)
    async def test_send_sticker_default_protect_content(self, chat_id, sticker, default_bot):
        tasks = asyncio.gather(default_bot.send_sticker(chat_id, sticker), default_bot.send_sticker(chat_id, sticker, protect_content=False))
    ...
    async def test_premium_animation(self, bot):
        ...
        premium_sticker_dict = {'file_unique_id': 'AQADOBwAAifPOElr', 'file_id': premium_sticker.premium_animation.file_id, 'file_size': premium_sticker.premium_animation.file_size}
...
class TestStickerSetWithRequest:
    ...
    async def test_create_sticker_set(self, bot, chat_id, sticker_file, animated_sticker_file, video_sticker_file):
        ...
        for sticker_set in [test_by, f'animated_{test_by}', f'video_{test_by}']:
            try:
                ...
                if sticker_set.startswith(test_by):
                    s = await bot.create_new_sticker_set(chat_id, name=sticker_set, title='Sticker Test', stickers=[InputSticker(sticker_file, emoji_list=['😄'])], sticker_format=StickerFormat.STATIC)
                    assert s
                elif sticker_set.startswith('animated'):
                    a = await bot.create_new_sticker_set(chat_id, name=sticker_set, title='Animated Test', stickers=[InputSticker(animated_sticker_file, emoji_list=['😄'])], sticker_format=StickerFormat.ANIMATED)
                    assert a
                elif sticker_set.startswith('video'):
                    v = await bot.create_new_sticker_set(chat_id, name=sticker_set, title='Video Test', stickers=[InputSticker(video_sticker_file, emoji_list=['😄'])], sticker_format=StickerFormat.VIDEO)
                    assert v
    ...
    async def test_delete_sticker_set(self, bot, chat_id, sticker_file):
        ...
        assert await bot.create_new_sticker_set(chat_id, name=name, title='Stickerset delete Test', stickers=[InputSticker(sticker_file, emoji_list=['😄'])], sticker_format=StickerFormat.STATIC)
        await asyncio.sleep(1)
        assert await bot.delete_sticker_set(name)

    async def test_set_custom_emoji_sticker_set_thumbnail(self, bot, chat_id, animated_sticker_file):
        ...
        try:
            ...
            assert await bot.create_new_sticker_set(chat_id, name=ss_name, title='Custom Emoji Sticker Set', stickers=[InputSticker(animated_sticker_file, emoji_list=['😄'])], sticker_format=StickerFormat.ANIMATED, sticker_type=Sticker.CUSTOM_EMOJI)
    ...
    async def test_bot_methods_1_png(self, bot, chat_id, sticker_file):
        with data_file('telegram_sticker.png').open('rb') as f:
            file = await bot.upload_sticker_file(chat_id, sticker=f, sticker_format=StickerFormat.STATIC)
        assert file
        await asyncio.sleep(1)
        tasks = asyncio.gather(bot.add_sticker_to_set(chat_id, f'test_by_{bot.username}', sticker=InputSticker(sticker=file.file_id, emoji_list=['😄'])), bot.add_sticker_to_set(chat_id, f'test_by_{bot.username}', sticker=InputSticker(sticker=sticker_file, emoji_list=['😄'], mask_position=MaskPosition(MaskPosition.EYES, -1, 1, 2))))
        assert all(await tasks)

    async def test_bot_methods_1_tgs(self, bot, chat_id):
        await asyncio.sleep(1)
        assert await bot.add_sticker_to_set(chat_id, f'animated_test_by_{bot.username}', sticker=InputSticker(sticker=data_file('telegram_animated_sticker.tgs').open('rb'), emoji_list=['😄']))

    async def test_bot_methods_1_webm(self, bot, chat_id):
        await asyncio.sleep(1)
        with data_file('telegram_video_sticker.webm').open('rb') as f:
            assert await bot.add_sticker_to_set(chat_id, f'video_test_by_{bot.username}', sticker=InputSticker(sticker=f, emoji_list=['🤔']))
    ...
    async def test_bot_methods_3_png(self, bot, chat_id, sticker_set_thumb_file):
        await asyncio.sleep(1)
        assert await bot.set_sticker_set_thumbnail(f'test_by_{bot.username}', chat_id, sticker_set_thumb_file)

    async def test_bot_methods_3_tgs(self, bot, chat_id, animated_sticker_file, animated_sticker_set):
        await asyncio.sleep(1)
        animated_test = f'animated_test_by_{bot.username}'
        file_id = animated_sticker_set.stickers[-1].file_id
        tasks = asyncio.gather(bot.set_sticker_set_thumbnail(animated_test, chat_id, animated_sticker_file), bot.set_sticker_set_thumbnail(animated_test, chat_id, file_id))
        assert all(await tasks)
...
class TestMaskPositionWithRequest(TestMaskPositionBase):
    ...
    async def test_create_new_mask_sticker_set(self, bot, chat_id, sticker_file, mask_position):
        ...
        try:
            ...
            if not e.message == 'Stickerset_invalid':
                raise e
            sticker_set = await bot.create_new_sticker_set(chat_id, name, 'Mask Stickers', stickers=[InputSticker(sticker=sticker_file, emoji_list=['😔'], mask_position=mask_position, keywords=['sad'])], sticker_format=StickerFormat.STATIC, sticker_type=Sticker.MASK)
            assert sticker_set
