...
from homeassistant.components.hassio import AddonError, AddonInfo, AddonManager, AddonState, hostname_from_addon_slug, is_hassio
...
DEFAULT_CHANNEL = 15
DEFAULT_CHANNEL_CHANGE_DELAY = 5 * 60
...
class WaitingAddonManager(AddonManager):
    ...
    async def async_install_addon_waiting(self) -> None:
        ...
        await self.async_wait_until_addon_state(AddonState.RUNNING, AddonState.NOT_RUNNING)
...
class MultiprotocolAddonManager(WaitingAddonManager):
    ...
    async def async_setup(self) -> None:
        """Set up the manager."""
        await async_process_integration_platforms(self._hass, 'silabs_multiprotocol', self._register_multipan_platform)
    ...
    async def _register_multipan_platform(self, hass: HomeAssistant, integration_domain: str, platform: MultipanProtocol) -> None:
        ...
        _LOGGER.info("Registering new multipan platform '%s', using multipan: %s, channel: %s", integration_domain, using_multipan, channel)
        if self._channel is not None or not using_multipan:
            return
        if channel is None:
            return
        _LOGGER.info("Setting multipan channel to %s (source: '%s')", channel, integration_domain)
...
class OptionsFlowHandler(config_entries.OptionsFlow, ABC):
    ...
    async def _resume_flow_when_done(self, awaitable: Awaitable) -> None:
        try:
            ...
            self.hass.async_create_task(self.flow_manager.async_configure(flow_id=self.flow_id))
    ...
    async def _async_get_addon_info(self, addon_manager: AddonManager) -> AddonInfo:
        ...
        try:
            ...
            _LOGGER.error(err)
            raise AbortFlow('addon_info_failed', description_placeholders={'addon_name': addon_manager.addon_name}) from err
        return addon_info
    ...
    async def async_step_addon_not_installed(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if user_input is None:
            return self.async_show_form(step_id='addon_not_installed', data_schema=vol.Schema({vol.Required(CONF_ENABLE_MULTI_PAN, default=False): bool}), description_placeholders={'hardware_name': self._hardware_name()})
    ...
    async def async_step_install_addon(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if not self.install_task:
            multipan_manager = await get_multiprotocol_addon_manager(self.hass)
            self.install_task = self.hass.async_create_task(self._resume_flow_when_done(multipan_manager.async_install_addon_waiting()), 'SiLabs Multiprotocol addon install')
            return self.async_show_progress(step_id='install_addon', progress_action='install_addon', description_placeholders={'addon_name': multipan_manager.addon_name})
        try:
    ...
    async def async_step_install_failed(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        multipan_manager = await get_multiprotocol_addon_manager(self.hass)
        return self.async_abort(reason='addon_install_failed', description_placeholders={'addon_name': multipan_manager.addon_name})
    ...
    async def async_step_configure_addon(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        from homeassistant.components.zha.radio_manager import ZhaMultiPANMigrationHelper
        from homeassistant.components.zha.silabs_multiprotocol import async_get_channel as async_get_zha_channel
        multipan_manager = await get_multiprotocol_addon_manager(self.hass)
        addon_info = await self._async_get_addon_info(multipan_manager)
        addon_config = addon_info.options
        serial_port_settings = await self._async_serial_port_settings()
        new_addon_config = {**addon_config, CONF_ADDON_AUTOFLASH_FW: True, **dataclasses.asdict(serial_port_settings)}
        ...
        if zha_entries:
            ...
            migration_data = {'new_discovery_info': {'name': self._zha_name(), 'port': {'path': get_zigbee_socket()}, 'radio_type': 'ezsp'}, 'old_discovery_info': await self._async_zha_physical_discovery()}
    ...
    async def async_step_start_addon(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if not self.start_task:
            multipan_manager = await get_multiprotocol_addon_manager(self.hass)
            self.start_task = self.hass.async_create_task(self._resume_flow_when_done(multipan_manager.async_start_addon_waiting()))
            return self.async_show_progress(step_id='start_addon', progress_action='start_addon', description_placeholders={'addon_name': multipan_manager.addon_name})
        try:
    ...
    async def async_step_start_failed(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        """Add-on start failed."""
        multipan_manager = await get_multiprotocol_addon_manager(self.hass)
        return self.async_abort(reason='addon_start_failed', description_placeholders={'addon_name': multipan_manager.addon_name})
    ...
    async def async_step_addon_menu(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        return self.async_show_menu(step_id='addon_menu', menu_options=['reconfigure_addon', 'uninstall_addon'])
    ...
    async def async_step_notify_unknown_multipan_user(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if user_input is None:
            return self.async_show_form(step_id='notify_unknown_multipan_user')
        return await self.async_step_change_channel()
    ...
    async def async_step_change_channel(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if user_input is None:
            ...
            data_schema = vol.Schema({vol.Required('channel', description={'suggested_value': str(suggested_channel)}): SelectSelector(SelectSelectorConfig(options=channels, mode=SelectSelectorMode.DROPDOWN))})
            return self.async_show_form(step_id='change_channel', data_schema=data_schema)
        await multipan_manager.async_change_channel(int(user_input['channel']), DEFAULT_CHANNEL_CHANGE_DELAY)
    ...
    async def async_step_notify_channel_change(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if user_input is None:
            return self.async_show_form(step_id='notify_channel_change', description_placeholders={'delay_minutes': str(DEFAULT_CHANNEL_CHANGE_DELAY // 60)})
        return self.async_create_entry(title='', data={})
    ...
    async def async_step_uninstall_addon(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if user_input is None:
            return self.async_show_form(step_id='uninstall_addon', data_schema=vol.Schema({vol.Required(CONF_DISABLE_MULTI_PAN, default=False): bool}), description_placeholders={'hardware_name': self._hardware_name()})
    ...
    async def async_step_firmware_revert(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        return self.async_abort(reason='addon_already_running', description_placeholders={'addon_name': flasher_manager.addon_name})
    ...
    async def async_step_install_flasher_addon(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if not self.install_task:
            self.install_task = self.hass.async_create_task(self._resume_flow_when_done(flasher_manager.async_install_addon_waiting()), 'SiLabs Flasher addon install')
            return self.async_show_progress(step_id='install_flasher_addon', progress_action='install_addon', description_placeholders={'addon_name': flasher_manager.addon_name})
        try:
            await self.install_task
        except AddonError as err:
            _LOGGER.error(err)
    ...
    async def async_step_configure_flasher_addon(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if zha_entries:
            zha_migration_mgr = ZhaMultiPANMigrationHelper(self.hass, zha_entries[0])
            migration_data = {'new_discovery_info': {'name': self._hardware_name(), 'port': {'path': new_settings.device, 'baudrate': int(new_settings.baudrate), 'flow_control': 'hardware' if new_settings.flow_control else None}, 'radio_type': 'ezsp'}, 'old_discovery_info': {'hw': {'name': self._zha_name(), 'port': {'path': get_zigbee_socket()}, 'radio_type': 'ezsp'}}}
            _LOGGER.debug('Starting ZHA migration with: %s', migration_data)
            try:
                if await zha_migration_mgr.async_initiate_migration(migration_data):
                    self._zha_migration_mgr = zha_migration_mgr
        ...
        flasher_manager = get_flasher_addon_manager(self.hass)
        addon_info = await self._async_get_addon_info(flasher_manager)
        new_addon_config = {**addon_info.options, 'device': new_settings.device, 'flow_control': new_settings.flow_control}
    ...
    async def async_step_uninstall_multiprotocol_addon(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if not self.stop_task:
            multipan_manager = await get_multiprotocol_addon_manager(self.hass)
            self.stop_task = self.hass.async_create_task(self._resume_flow_when_done(multipan_manager.async_uninstall_addon_waiting()), 'SiLabs Multiprotocol addon uninstall')
            return self.async_show_progress(step_id='uninstall_multiprotocol_addon', progress_action='uninstall_multiprotocol_addon', description_placeholders={'addon_name': multipan_manager.addon_name})
        try:
    ...
    async def async_step_start_flasher_addon(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        if not self.start_task:
            ...
            async def start_and_wait_until_done() -> None:
                ...
                await flasher_manager.async_wait_until_addon_state(AddonState.NOT_RUNNING)
            self.start_task = self.hass.async_create_task(self._resume_flow_when_done(start_and_wait_until_done()))
            return self.async_show_progress(step_id='start_flasher_addon', progress_action='start_flasher_addon', description_placeholders={'addon_name': flasher_manager.addon_name})
        try:
            await self.start_task
        except (AddonError, AbortFlow) as err:
            _LOGGER.error(err)
    ...
    async def async_step_flasher_failed(self, user_input: dict[str, Any] | None=None) -> FlowResult:
        ...
        flasher_manager = get_flasher_addon_manager(self.hass)
        return self.async_abort(reason='addon_start_failed', description_placeholders={'addon_name': flasher_manager.addon_name})
