...
async def get_modified_files(git_branch: str, git_revision: str, diffed_branch: str, is_local: bool, ci_context: CIContext, pull_request: PullRequest) -> List[str]:
...
async def get_modified_files_str(ctx: click.Context):
    modified_files = await get_modified_files(ctx.obj['git_branch'], ctx.obj['git_revision'], ctx.obj['diffed_branch'], ctx.obj['is_local'], ctx.obj['ci_context'], ctx.obj['pull_request'])
    return transform_strs_to_paths(modified_files)

@click.group(cls=LazyGroup, help='Airbyte CI top-level command group.', lazy_subcommands={'connectors': 'pipelines.airbyte_ci.connectors.commands.connectors', 'format': 'pipelines.airbyte_ci.format.commands.format_code', 'metadata': 'pipelines.airbyte_ci.metadata.commands.metadata', 'test': 'pipelines.airbyte_ci.test.commands.test'})
...
@click.option('--diffed-branch', help='Branch to which the git diff will happen to detect new or modified connectors', default='origin/master', type=str)
...
@click.option('--ci-gcs-credentials', help='The service account to use during CI.', type=click.STRING, required=False, envvar='GCP_GSM_CREDENTIALS')
...
async def airbyte_ci(ctx: click.Context):
