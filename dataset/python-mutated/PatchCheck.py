...
class EmailAddressCheck:
    ...
    def error(self, *err):
        ...
        for line in err:
            ...
            print(prefix, line)
            count += 1
    email_re1 = re.compile('(?:\\s*)(.*?)(\\s*)<(.+)>\\s*$', re.MULTILINE | re.IGNORECASE)
...
class CommitMessageCheck:
    ...
    def error(self, *err):
        ...
        for line in err:
            ...
            print(prefix, line)
            count += 1
    contributed_under_re = re.compile('^\\s*contributed-under\\s*:', re.MULTILINE | re.IGNORECASE)
    ...
    def check_signed_off_by(self):
        ...
        if len(sobs) == 0:
            ...
            return
    sig_types = ('Reviewed', 'Reported', 'Tested', 'Suggested', 'Acked', 'Cc')
...
class GitDiffCheck:
    ...
    def run(self):
        ...
        if self.state == START:
            ...
            self.line_num += 1
    pre_patch_prefixes = ('--- ', '+++ ', 'index ', 'new file ', 'deleted file ', 'old mode ', 'new mode ', 'similarity index ', 'copy from ', 'copy to ', 'rename ')
    line_endings = ('\r\n', '\n\r', '\n', '\r')
    newfile_prefix_re = re.compile('^\n                       index\\ 0+\\.\\.\n                   ', re.VERBOSE)
