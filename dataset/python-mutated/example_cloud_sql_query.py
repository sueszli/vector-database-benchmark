...
GCSQL_POSTGRES_INSTANCE_NAME_QUERY = os.environ.get('GCSQL_POSTGRES_INSTANCE_NAME_QUERY', 'test-postgres-query')
...
postgres_kwargs = {'user': quote_plus(GCSQL_POSTGRES_USER), 'password': quote_plus(GCSQL_POSTGRES_PASSWORD), 'public_port': GCSQL_POSTGRES_PUBLIC_PORT, 'public_ip': quote_plus(GCSQL_POSTGRES_PUBLIC_IP), 'project_id': quote_plus(GCP_PROJECT_ID), 'location': quote_plus(GCP_REGION), 'instance': quote_plus(GCSQL_POSTGRES_INSTANCE_NAME_QUERY), 'database': quote_plus(GCSQL_POSTGRES_DATABASE_NAME), 'client_cert_file': quote_plus(get_absolute_path(GCSQL_POSTGRES_CLIENT_CERT_FILE)), 'client_key_file': quote_plus(get_absolute_path(GCSQL_POSTGRES_CLIENT_KEY_FILE)), 'server_ca_file': quote_plus(get_absolute_path(GCSQL_POSTGRES_SERVER_CA_FILE))}
...
os.environ['AIRFLOW_CONN_PROXY_MYSQL_SOCKET'] = 'gcpcloudsql://{user}:{password}@{public_ip}:{public_port}/{database}?database_type=mysql&project_id={project_id}&location={location}&instance={instance}&use_proxy=True&sql_proxy_use_tcp=False'.format(**mysql_kwargs)
...
connection_names = ['proxy_postgres_tcp', 'proxy_postgres_socket', 'public_postgres_tcp', 'public_postgres_tcp_ssl', 'proxy_mysql_tcp', 'proxy_mysql_socket', 'public_mysql_tcp', 'public_mysql_tcp_ssl', 'public_mysql_tcp_ssl_no_project_id']
tasks = []
with DAG(dag_id='example_gcp_sql_query', start_date=datetime(2021, 1, 1), catchup=False, tags=['example']) as dag:
    prev_task = None
    for connection_name in connection_names:
        task = CloudSQLExecuteQueryOperator(gcp_cloudsql_conn_id=connection_name, task_id='example_gcp_sql_task_' + connection_name, sql=SQL, sql_proxy_binary_path=sql_proxy_binary_path)
        tasks.append(task)
        if prev_task:
            prev_task >> task
        prev_task = task