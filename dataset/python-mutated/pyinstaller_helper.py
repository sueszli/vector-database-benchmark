...
import shutil
import sys
HIDDEN_IMPORTS = ['packaging.specifiers', 'packaging.requirements', 'pkg_resources.py2_warn', 'numpy.core._methods', 'numpy.core._dtype_ctypes', 'numpy.random.common', 'numpy.random.entropy', 'numpy.random.bounded_integers']
...
if __name__ == '__main__':
    ...
    urh_cmd = cmd + ['--name=urh', '--windowed', '--workpath', './urh_build', os.path.join(urh_path, 'src/urh/main.py')]
    urh_debug_cmd = cmd + ['--name=urh_debug', '--workpath', './urh_debug_build', os.path.join(urh_path, 'src/urh/main.py')]
    cli_cmd = cmd + ['--workpath', './urh_cli_build', os.path.join(urh_path, 'src/urh/cli/urh_cli.py')]
