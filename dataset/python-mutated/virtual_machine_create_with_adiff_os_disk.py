...
def main():
    ...
    response = client.virtual_machines.begin_create_or_update(resource_group_name='myResourceGroup', vm_name='myVM', parameters={'location': 'westus', 'plan': {'name': 'windows2016', 'product': 'windows-data-science-vm', 'publisher': 'microsoft-ads'}, 'properties': {'hardwareProfile': {'vmSize': 'Standard_DS1_v2'}, 'networkProfile': {'networkInterfaces': [{'id': '/subscriptions/{subscription-id}/resourceGroups/myResourceGroup/providers/Microsoft.Network/networkInterfaces/{existing-nic-name}', 'properties': {'primary': True}}]}, 'osProfile': {'adminPassword': '{your-password}', 'adminUsername': '{your-username}', 'computerName': 'myVM'}, 'storageProfile': {'imageReference': {'offer': 'windows-data-science-vm', 'publisher': 'microsoft-ads', 'sku': 'windows2016', 'version': 'latest'}, 'osDisk': {'caching': 'ReadOnly', 'createOption': 'FromImage', 'diffDiskSettings': {'option': 'Local'}, 'managedDisk': {'storageAccountType': 'Standard_LRS'}, 'name': 'myVMosdisk'}}}}).result()
    print(response)
