...
async def get_mock_wallet(sd_hash, storage, wallet_dir, balance=10.0, fee=None):
    ...
    tx.height = 514081
    txo = tx.outputs[0]
    txo.meta.update({'permanent_url': '33rpm#c49566d631226492317d06ad7fdbe1ed32925124'})
    ...
    wallet = Wallet()
    ledger = Ledger({'db': Database(os.path.join(wallet_dir, 'blockchain.db')), 'headers': FakeHeaders(514082), 'tx_cache_size': 10000})
    ...
    manager.ledger.network.client = ClientSession(network=manager.ledger.network, server=('fakespv.lbry.com', 50001))
...
class TestStreamManager(BlobExchangeTestBase):
    ...
    async def setup_stream_manager(self, balance=10.0, fee=None, old_sort=False):
        ...
        descriptor = await StreamDescriptor.create_stream(self.loop, self.server_blob_manager.blob_dir, file_path, old_sort=old_sort)
        self.sd_hash = descriptor.sd_hash
        (self.mock_wallet, self.uri) = await get_mock_wallet(self.sd_hash, self.client_storage, self.client_wallet_dir, balance, fee)
        analytics_manager = AnalyticsManager(self.client_config, binascii.hexlify(generate_id()).decode(), binascii.hexlify(generate_id()).decode())
        self.stream_manager = StreamManager(self.loop, self.client_config, self.client_blob_manager, self.mock_wallet, self.client_storage, get_mock_node(self.server_from_client), analytics_manager)
        self.file_manager = FileManager(self.loop, self.client_config, self.mock_wallet, self.client_storage, analytics_manager)
    ...
    async def test_download_stop_resume_delete(self):
        ...
        stored_status = await self.client_storage.run_and_return_one_or_none('select status from file where stream_hash=?', stream_hash)
    ...
    async def test_insufficient_funds(self):
        fee = {'currency': 'LBC', 'amount': 11.0, 'address': 'bYFeMtSL7ARuG1iMpjFyrnTe4oJHSAVNXF', 'version': '_0_0_1'}
    ...
    async def test_fee_above_max_allowed(self):
        fee = {'currency': 'USD', 'amount': 51.0, 'address': 'bYFeMtSL7ARuG1iMpjFyrnTe4oJHSAVNXF', 'version': '_0_0_1'}
    ...
    async def test_download_sd_timeout(self):
        self.server.stop_server()
        await self.setup_stream_manager()
        await self._test_download_error_analytics_on_start(DownloadSDTimeoutError, f'Failed to download sd blob {self.sd_hash} within timeout.', timeout=1)
    ...
    async def test_download_data_timeout(self):
        ...
        await self._test_download_error_analytics_on_start(DownloadDataTimeoutError, f'Failed to download data blobs for sd hash {self.sd_hash} within timeout.', timeout=1)

    async def test_unexpected_error(self):
        await self.setup_stream_manager()
        err_msg = f"invalid blob directory '{self.client_dir}'"
        shutil.rmtree(self.client_dir)
        await self._test_download_error_analytics_on_start(OSError, err_msg, timeout=1)
    ...
    async def test_download_then_recover_stream_on_startup(self, old_sort=False):
        expected_analytics_events = ['Time To First Bytes', 'Download Finished']
        received_events = []
