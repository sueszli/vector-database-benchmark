...
class Conv1d(Layer):
    ...
    def __init__(self, n_filter=32, filter_size=5, stride=1, act=None, padding='SAME', data_format='channels_last', dilation_rate=1, W_init=tl.initializers.truncated_normal(stddev=0.02), b_init=tl.initializers.constant(value=0.0), in_channels=None, name=None):
        if False:
            return 10
        ...
        if self.in_channels:
            self.build(None)
            self._built = True
        logging.info('Conv1d %s: n_filter: %d filter_size: %s stride: %d pad: %s act: %s' % (self.name, n_filter, filter_size, stride, padding, self.act.__name__ if self.act is not None else 'No Activation'))
    ...
    def forward(self, inputs):
        if False:
            for i in range(10):
                print('nop')
        outputs = tf.nn.conv1d(input=inputs, filters=self.W, stride=self.stride, padding=self.padding, data_format=self.data_format, dilations=self.dilation_rate, name=self.name)
        if self.b_init:
...
class Conv2d(Layer):
    ...
    def __init__(self, n_filter=32, filter_size=(3, 3), strides=(1, 1), act=None, padding='SAME', data_format='channels_last', dilation_rate=(1, 1), W_init=tl.initializers.truncated_normal(stddev=0.02), b_init=tl.initializers.constant(value=0.0), in_channels=None, name=None):
        if False:
            print('Hello World!')
        super().__init__(name, act=act)
        self.n_filter = n_filter
        ...
        if self.in_channels:
            self.build(None)
            self._built = True
        logging.info('Conv2d %s: n_filter: %d filter_size: %s strides: %s pad: %s act: %s' % (self.name, n_filter, str(filter_size), str(strides), padding, self.act.__name__ if self.act is not None else 'No Activation'))
    ...
    def forward(self, inputs):
        if False:
            while True:
                i = 10
        outputs = tf.nn.conv2d(input=inputs, filters=self.W, strides=self._strides, padding=self.padding, data_format=self.data_format, dilations=self._dilation_rate, name=self.name)
        if self.b_init:
            outputs = tf.nn.bias_add(outputs, self.b, data_format=self.data_format, name='bias_add')
        if self.act:
            outputs = self.act(outputs)
...
class Conv3d(Layer):
    ...
    def __init__(self, n_filter=32, filter_size=(3, 3, 3), strides=(1, 1, 1), act=None, padding='SAME', data_format='channels_last', dilation_rate=(1, 1, 1), W_init=tl.initializers.truncated_normal(stddev=0.02), b_init=tl.initializers.constant(value=0.0), in_channels=None, name=None):
        if False:
            i = 10
            return i + 15
        super().__init__(name, act=act)
        ...
        if self.in_channels:
            self.build(None)
            self._built = True
        logging.info('Conv3d %s: n_filter: %d filter_size: %s strides: %s pad: %s act: %s' % (self.name, n_filter, str(filter_size), str(strides), padding, self.act.__name__ if self.act is not None else 'No Activation'))
    ...
    def build(self, inputs_shape):
        ...
        if self.data_format == 'channels_last':
            ...
            raise Exception('data_format should be either channels_last or channels_first')
        self.filter_shape = (self.filter_size[0], self.filter_size[1], self.filter_size[2], self.in_channels, self.n_filter)
        self.W = self._get_weights('filters', shape=self.filter_shape, init=self.W_init)
        if self.b_init:
            self.b = self._get_weights('biases', shape=(self.n_filter,), init=self.b_init)
    ...
    def forward(self, inputs):
        if False:
            while True:
                i = 10
        outputs = tf.nn.conv3d(input=inputs, filters=self.W, strides=self._strides, padding=self.padding, data_format=self.data_format, dilations=self._dilation_rate, name=self.name)
        if self.b_init:
            outputs = tf.nn.bias_add(outputs, self.b, data_format=self.data_format, name='bias_add')
        if self.act:
            outputs = self.act(outputs)
