...
parser.add_argument('-c', '--changed', action='store_true', help='Only run on changed files')
parser.add_argument('--print-slowest', action='store_true', help='Print the slowest checks')
...
files.sort()
file_types = ('.h', '.c', '.cpp', '.tcc', '.yaml', '.yml', '.ini', '.txt', '.ico', '.svg', '.png', '.py', '.html', '.js', '.md', '.sh', '.css', '.proto', '.conf', '.cfg', '.woff', '.woff2', '')
...
CONF_RE = '^(CONF_[a-zA-Z0-9_]+)\\s*=\\s*[\\\'"].*?[\\\'"]\\s*?$'
...
RAW_PIN_ACCESS_RE = '^\\s(pinMode|digitalWrite|digitalRead)\\((.*)->get_pin\\(\\),\\s*([^)]+).*\\)'
...
ARDUINO_FORBIDDEN = ['digitalWrite', 'digitalRead', 'pinMode', 'shiftOut', 'shiftIn', 'radians', 'degrees', 'interrupts', 'noInterrupts', 'lowByte', 'highByte', 'bitRead', 'bitSet', 'bitClear', 'bitWrite', 'bit', 'analogRead', 'analogWrite', 'pulseIn', 'pulseInLong', 'tone']
ARDUINO_FORBIDDEN_RE = '[^\\w\\d](' + '|'.join(ARDUINO_FORBIDDEN) + ')\\(.*'
...
IDF_CONVERSION_FORBIDDEN = {'ARDUINO_ARCH_ESP32': 'USE_ESP32', 'ARDUINO_ARCH_ESP8266': 'USE_ESP8266', 'pgm_read_byte': 'progmem_read_byte', 'ICACHE_RAM_ATTR': 'IRAM_ATTR', 'esphome/core/esphal.h': 'esphome/core/hal.h'}
IDF_CONVERSION_FORBIDDEN_RE = '(' + '|'.join(IDF_CONVERSION_FORBIDDEN) + ').*'
