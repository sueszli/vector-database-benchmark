...
class MultiUpOrg(SimpleDecrypter):
    ...
    __version__ = '0.18'
    __status__ = 'testing'
    __pattern__ = 'https?://(?:www\\.)?multiup\\.(?:org|eu)/(?:en/|fr/)?(?:(?P<TYPE>project|download|mirror)/)?\\w+(?:/\\w+)?'
    __config__ = [('enabled', 'bool', 'Activated', True), ('use_premium', 'bool', 'Use premium account if available', True), ('folder_per_package', 'Default;Yes;No', 'Create folder for each package', 'Default'), ('max_wait', 'int', 'Reconnect if waiting time is greater than minutes', 10), ('hosts_priority', 'str', 'Prefered hosts priority (bar-separated)', ''), ('ignored_hosts', 'str', 'Ignored hosts list (bar-separated)', ''), ('grab_all', 'bool', 'Grab all URLs (default only first match)', False)]
    ...
    NAME_PATTERN = '<title>.*(?:Project|Projet|Download|Télécharger) (?P<N>.+?) (\\(|- )'
    OFFLINE_PATTERN = 'The requested file could not be found'
    TEMP_OFFLINE_PATTERN = '^unmatchable$'
    DIRECT_LINK = False
    URL_REPLACEMENTS = [('https?://(?:www\\.)?multiup\\.(?:org|eu)/', 'https://www.multiup.org/'), ('/fr/', '/en/')]
