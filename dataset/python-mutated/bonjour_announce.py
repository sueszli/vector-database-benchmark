...
def validate(config):
    ...
    if not isinstance(config, list):
        ...
        return (False, 'Configuration for bonjour_announce beacon must contain servicetype, port and txt items.')
...
def beacon(config):
    ...
    if changes:
        ...
        if not LAST_GRAINS:
            ...
            changes['ipv4'] = __grains__.get('ipv4', [])
            changes['ipv6'] = __grains__.get('ipv6', [])
            SD_REF = pybonjour.DNSServiceRegister(name=servicename, regtype=config['servicetype'], port=config['port'], txtRecord=txt_record, callBack=_register_callback)
            ...
            pybonjour.DNSServiceUpdateRecord(SD_REF, RecordRef=None, flags=0, rdata=txt_record_raw)
