...
class ReadableDeploymentStorage(Block, abc.ABC):
    ...
    @abc.abstractmethod
    async def get_directory(self, from_path: str=None, local_path: str=None) -> None:
        pass
...
class WritableDeploymentStorage(Block, abc.ABC):
    ...
    @abc.abstractmethod
    async def put_directory(self, local_path: str=None, to_path: str=None, ignore_file: str=None) -> None:
        pass
...
class LocalFileSystem(WritableFileSystem, WritableDeploymentStorage):
    ...
    _documentation_url = 'https://docs.prefect.io/concepts/filesystems/#local-filesystem'
    basepath: Optional[str] = Field(default=None, description='Default local path for this block to write to.')
    ...
    async def get_directory(self, from_path: str=None, local_path: str=None) -> None:
        ...
        if (from_path / Path('.prefectignore')).exists():
            ignore_func = await self._get_ignore_func(local_path=from_path, ignore_file=from_path / Path('.prefectignore'))
        else:
            ignore_func = None
    ...
    async def put_directory(self, local_path: str=None, to_path: str=None, ignore_file: str=None) -> None:
        ...
        if ignore_file:
            ignore_func = await self._get_ignore_func(local_path=local_path, ignore_file=ignore_file)
        else:
            ignore_func = None
        if local_path == destination_path:
            pass
        else:
            copytree(src=local_path, dst=destination_path, ignore=ignore_func, dirs_exist_ok=True)
...
class RemoteFileSystem(WritableFileSystem, WritableDeploymentStorage):
    ...
    _documentation_url = 'https://docs.prefect.io/concepts/filesystems/#remote-file-system'
    basepath: str = Field(default=..., description='Default path for this block to write to.', example='s3://my-bucket/my-folder/')
    settings: Dict[str, Any] = Field(default_factory=dict, description='Additional settings to pass through to fsspec.')
    ...
    @sync_compatible
    async def put_directory(self, local_path: Optional[str]=None, to_path: Optional[str]=None, ignore_file: Optional[str]=None, overwrite: bool=True) -> int:
        """
        ...
        if ignore_file:
            with open(ignore_file, 'r') as f:
                ignore_patterns = f.readlines()
            included_files = filter_files(local_path, ignore_patterns, include_dirs=True)
        counter = 0
...
class S3(WritableFileSystem, WritableDeploymentStorage):
    ...
    bucket_path: str = Field(default=..., description='An S3 bucket path.', example='my-bucket/a-directory-within')
    aws_access_key_id: Optional[SecretStr] = Field(default=None, title='AWS Access Key ID', description='Equivalent to the AWS_ACCESS_KEY_ID environment variable.', example='AKIAIOSFODNN7EXAMPLE')
    aws_secret_access_key: Optional[SecretStr] = Field(default=None, title='AWS Secret Access Key', description='Equivalent to the AWS_SECRET_ACCESS_KEY environment variable.', example='wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY')
    ...
    @sync_compatible
    async def get_directory(self, from_path: Optional[str]=None, local_path: Optional[str]=None) -> bytes:
        """
        Downloads a directory from a given remote path to a local directory.

        Defaults to downloading the entire contents of the block's basepath to the current working directory.
        """
        return await self.filesystem.get_directory(from_path=from_path, local_path=local_path)
    ...
    async def put_directory(self, local_path: Optional[str]=None, to_path: Optional[str]=None, ignore_file: Optional[str]=None) -> int:
        ...
        Defaults to uploading the entire contents of the current working directory to the block's basepath.
        """
        return await self.filesystem.put_directory(local_path=local_path, to_path=to_path, ignore_file=ignore_file)
...
class GCS(WritableFileSystem, WritableDeploymentStorage):
    ...
    bucket_path: str = Field(default=..., description='A GCS bucket path.', example='my-bucket/a-directory-within')
    service_account_info: Optional[SecretStr] = Field(default=None, description='The contents of a service account keyfile as a JSON string.')
    project: Optional[str] = Field(default=None, description='The project the GCS bucket resides in. If not provided, the project will be inferred from the credentials or environment.')
...
class Azure(WritableFileSystem, WritableDeploymentStorage):
    ...
    bucket_path: str = Field(default=..., description='An Azure storage bucket path.', example='my-bucket/a-directory-within')
    azure_storage_connection_string: Optional[SecretStr] = Field(default=None, title='Azure storage connection string', description='Equivalent to the AZURE_STORAGE_CONNECTION_STRING environment variable.')
    azure_storage_account_name: Optional[SecretStr] = Field(default=None, title='Azure storage account name', description='Equivalent to the AZURE_STORAGE_ACCOUNT_NAME environment variable.')
    azure_storage_account_key: Optional[SecretStr] = Field(default=None, title='Azure storage account key', description='Equivalent to the AZURE_STORAGE_ACCOUNT_KEY environment variable.')
    ...
    azure_storage_client_secret: Optional[SecretStr] = Field(None, title='Azure storage client secret', description='Equivalent to the AZURE_CLIENT_SECRET environment variable.')
...
class SMB(WritableFileSystem, WritableDeploymentStorage):
    ...
    share_path: str = Field(default=..., description='SMB target (requires <SHARE>, followed by <PATH>).', example='/SHARE/dir/subdir')
    smb_username: Optional[SecretStr] = Field(default=None, title='SMB Username', description='Username with access to the target SMB SHARE.')
    smb_password: Optional[SecretStr] = Field(default=None, title='SMB Password', description='Password for SMB access.')
    smb_host: str = Field(default=..., tile='SMB server/hostname', description='SMB server/hostname.')
    smb_port: Optional[int] = Field(default=None, title='SMB port', description='SMB port (default: 445).')
    _remote_file_system: RemoteFileSystem = None
    ...
    async def put_directory(self, local_path: Optional[str]=None, to_path: Optional[str]=None, ignore_file: Optional[str]=None) -> int:
        ...
        Defaults to uploading the entire contents of the current working directory to the block's basepath.
        """
        return await self.filesystem.put_directory(local_path=local_path, to_path=to_path, ignore_file=ignore_file, overwrite=False)
...
class GitHub(ReadableDeploymentStorage):
    ...
    repository: str = Field(default=..., description='The URL of a GitHub repository to read from, in either HTTPS or SSH format.')
    reference: Optional[str] = Field(default=None, description='An optional reference to pin to; can be a branch name or tag.')
    access_token: Optional[SecretStr] = Field(name='Personal Access Token', default=None, description="A GitHub Personal Access Token (PAT) with repo scope. To use a fine-grained PAT, provide '{username}:{PAT}' as the value.")
    include_git_objects: bool = Field(default=True, description='Whether to include git objects when copying the repo contents to a directory.')
    ...
    async def get_directory(self, from_path: Optional[str]=None, local_path: Optional[str]=None) -> None:
        ...
        with TemporaryDirectory(suffix='prefect') as tmp_dir:
            ...
            copytree(src=content_source, dst=content_destination, dirs_exist_ok=True, ignore=ignore_func)