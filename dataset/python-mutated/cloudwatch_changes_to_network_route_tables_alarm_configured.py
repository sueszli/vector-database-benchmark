...
class cloudwatch_changes_to_network_route_tables_alarm_configured(Check):
    ...
    def execute(self):
        if False:
            i = 10
            return i + 15
        pattern = '\\$\\.eventName\\s*=\\s*.?CreateRoute.+\\$\\.eventName\\s*=\\s*.?CreateRouteTable.+\\$\\.eventName\\s*=\\s*.?ReplaceRoute.+\\$\\.eventName\\s*=\\s*.?ReplaceRouteTableAssociation.+\\$\\.eventName\\s*=\\s*.?DeleteRouteTable.+\\$\\.eventName\\s*=\\s*.?DeleteRoute.+\\$\\.eventName\\s*=\\s*.?DisassociateRouteTable.?'
        findings = []
