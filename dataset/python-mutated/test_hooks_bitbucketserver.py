...
class TestChangeHookConfiguredWithGitChange(unittest.TestCase, TestReactorMixin):
    ...
    def setUp(self):
        ...
        self.change_hook = change_hook.ChangeHookResource(dialects={'bitbucketserver': {'bitbucket_property_whitelist': ['bitbucket.*']}}, master=fakeMasterForHooks(self))
    ...
    def _checkPush(self, change):
        ...
        self.assertEqual(change['comments'], 'Bitbucket Server commit 793d4754230023d85532f9a38dba3290f959beb4')
    ...
    def _checkPullRequest(self, change):
        ...
        self.assertEqual(change['author'], 'John Smith <John>')
        self.assertEqual(change['project'], 'Continuous Integration')
        self.assertEqual(change['comments'], 'Bitbucket Server Pull Request #21')
    ...
    def _checkCodebase(self, event_type, expected_codebase):
        if False:
            return 10
        payloads = {'repo:refs_changed': pushJsonPayload, 'pullrequest:updated': pullRequestUpdatedJsonPayload}
    ...
    def testHookWithCodebaseFunctionOnPushEvent(self):
        if False:
            while True:
                i = 10
        self.change_hook.dialects = {'bitbucketserver': {'codebase': lambda payload: payload['repository']['project']['key']}}
    ...
    def testHookWithChangeOnDeleteTag(self):
        if False:
            print('Hello World!')
        request = _prepare_request(deleteTagJsonPayload, headers={_HEADER_EVENT: 'repo:refs_changed'})
        yield request.test_render(self.change_hook)
        self.assertEqual(len(self.change_hook.master.data.updates.changesAdded), 1)
        change = self.change_hook.master.data.updates.changesAdded[0]
        self._checkPush(change)
    ...
    def testHookWithChangeOnDeleteBranch(self):
        if False:
            print('Hello World!')
        request = _prepare_request(deleteBranchJsonPayload, headers={_HEADER_EVENT: 'repo:refs_changed'})
        yield request.test_render(self.change_hook)
        self.assertEqual(len(self.change_hook.master.data.updates.changesAdded), 1)
        change = self.change_hook.master.data.updates.changesAdded[0]
        self._checkPush(change)
