...
class NGramCounter(Transformer):
    ...
    * **list** : Each element of the list must be a string, which is tokenized
    ...
    def __init__(self, features=None, excluded_features=None, n=2, method='word', to_lower=True, ignore_punct=True, ignore_space=True, delimiters=['\r', '\x0b', '\n', '\x0c', '\t', ' ', '!', '#', '$', '%', '&', "'", '(', ')', '*', '+', ',', '-', '.', '/', ':', ';', '<', '=', '>', '?', '@', '[', '\\', ']', '^', '_', '`', '{', '|', '}', '~'], output_column_prefix=None):
        if False:
            print('Hello World!')
        ...
        opts = {'n': n, 'features': features, 'ngram_type': method, 'to_lower': to_lower, 'ignore_punct': ignore_punct, 'ignore_space': ignore_space, 'delimiters': delimiters, 'output_column_prefix': output_column_prefix}
        if _exclude:
            opts['exclude'] = True
    ...
    def _get_summary_struct(self):
        ...
        fields = [('NGram length', 'n'), ('NGram type (word or character)', 'ngram_type'), ('Convert strings to lower case', 'to_lower'), ('Ignore punctuation in character ngram', 'ignore_punct'), ('Ignore space in character ngram', 'ignore_space'), ('Delimiters', 'delimiters'), ('Features', _features), ('Output column prefix', 'output_column_prefix')]
