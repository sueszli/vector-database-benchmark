...
_room_id_schema = validate.Schema({'data': validate.any(None, {'room_id': int, 'live_status': int})}, validate.get('data'))
_room_stream_list_schema = validate.Schema({'data': validate.any(None, {'durl': [{'url': validate.url()}]})}, validate.get('data'))

@pluginmatcher(re.compile('https?://live\\.bilibili\\.com/(?P<channel>[^/]+)'))
