...
class HTTPEventProvider:
    ...
    async def send_event(self, workflow_id: str, req: Request) -> JSONResponse:
        ...
        try:
            ...
            return JSONResponse(status_code=404, content={'error': {'code': 404, 'message': f'{e} field is not found in the request JSON'}})
        try:
            self.event_key_payload[workflow_id][event_key].set_result((event_key, event_payload))
        except KeyError:
            return JSONResponse(status_code=404, content={'error': {'code': 404, 'message': 'workflow_id and event_key need to be registered to receive event. Please make sure they are registered before resending.'}})
        ...
        return JSONResponse(status_code=500, content={'error': {'code': 500, 'message': 'event processing failed'}})
    ...
    async def report_checkpointed(self, workflow_id: str, event_key: str, confirmation: bool) -> str:
        ...
        try:
            ...
            logger.error(f'{event_key} cannot be found to acknowledge request. The event provider may have been restarted.')
            raise WorkflowEventHandleError(workflow_id, f'{event_key} cannot be found to acknowledge request.')
        return 'OK'
...
class HTTPListener(EventListener):
    ...
    async def poll_for_event(self, event_key: str=None) -> Event:
        ...
        event_key_payload = await self.handle.get_event_payload.remote(workflow_id, event_key)
    ...
    async def event_checkpointed(self, event: Event) -> None:
        ...
        (event_key, _) = event
        await self.handle.report_checkpointed.remote(workflow_context.get_current_workflow_id(), event_key, True)