...
from sentry.api.serializers.models.group import BaseGroupSerializerResponse, GroupSerializer, GroupSerializerSnuba, SeenStats, snuba_tsdb
...
class GroupStatsMixin:
    STATS_PERIOD_CHOICES = {'14d': StatsPeriod(14, timedelta(hours=24)), '24h': StatsPeriod(24, timedelta(hours=1))}
    CUSTOM_ROLLUP_CHOICES = {'1h': timedelta(hours=1).total_seconds(), '2h': timedelta(hours=2).total_seconds(), '3h': timedelta(hours=3).total_seconds(), '6h': timedelta(hours=6).total_seconds(), '12h': timedelta(hours=12).total_seconds(), '24h': timedelta(hours=24).total_seconds()}
    CUSTOM_SEGMENTS = 29
    CUSTOM_SEGMENTS_12H = 35
    CUSTOM_ROLLUP_6H = timedelta(hours=6).total_seconds()
