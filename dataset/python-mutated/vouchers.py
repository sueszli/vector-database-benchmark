...
from ...channel.types import Channel, ChannelContext, ChannelContextType, ChannelContextTypeWithMetadata
from ...core import ResolveInfo, types
from ...core.connection import CountableConnection, create_connection_slice
from ...core.context import get_database_connection_name
from ...core.descriptions import ADDED_IN_31, ADDED_IN_318, DEPRECATED_IN_3X_FIELD, PREVIEW_FEATURE
from ...core.doc_category import DOC_CATEGORY_DISCOUNTS
from ...core.fields import ConnectionField, PermissionsField
from ...core.types import ModelObjectType, Money, NonNullList
from ...meta.types import ObjectWithMetadata
from ...product.types import CategoryCountableConnection, CollectionCountableConnection, ProductCountableConnection, ProductVariantCountableConnection
from ...translations.fields import TranslationField
from ...translations.types import VoucherTranslation
from ..dataloaders import CodeByVoucherIDLoader, UsedByVoucherIDLoader, VoucherChannelListingByVoucherIdAndChanneSlugLoader, VoucherChannelListingByVoucherIdLoader
...
class VoucherChannelListing(ModelObjectType[models.VoucherChannelListing]):
    ...
    channel = graphene.Field(Channel, required=True, description='The channel in which voucher can be applied.')
    discount_value = graphene.Float(required=True, description='The value of the discount on voucher in a channel.')
    currency = graphene.String(required=True, description='Currency code for voucher in a channel.')
    min_spent = graphene.Field(Money, description='Minimum order value for voucher to apply in channel.')
...
class VoucherCode(ModelObjectType[models.VoucherCode]):
    ...
    created_at = graphene.DateTime(required=True, description='Date time of code creation.')
...
class Voucher(ChannelContextTypeWithMetadata[models.Voucher]):
    ...
    codes = ConnectionField(VoucherCodeCountableConnection, description='List of codes available for this voucher.' + ADDED_IN_318)
    code = graphene.String(description='The code of the voucher.' + DEPRECATED_IN_3X_FIELD)
    ...
    start_date = graphene.DateTime(required=True, description='The start date and time of voucher.')
    end_date = graphene.DateTime(description='The end date and time of voucher.')
    apply_once_per_order = graphene.Boolean(required=True, description='Determine if the voucher should be applied once per order. If set to True, the voucher is applied to a single cheapest eligible product in checkout.')
    apply_once_per_customer = graphene.Boolean(required=True, description='Determine if the voucher usage should be limited to one use per customer.')
    single_use = graphene.Boolean(required=True, description='Determine if the voucher codes can be used once or multiple times.' + ADDED_IN_318 + PREVIEW_FEATURE)
    only_for_staff = graphene.Boolean(required=True, description='Determine if the voucher is available only for staff members.')
    min_checkout_items_quantity = graphene.Int(description='Determine minimum quantity of items for checkout.')
    categories = ConnectionField(CategoryCountableConnection, description='List of categories this voucher applies to.')
    collections = ConnectionField(CollectionCountableConnection, description='List of collections this voucher applies to.', permissions=[DiscountPermissions.MANAGE_DISCOUNTS])
    products = ConnectionField(ProductCountableConnection, description='List of products this voucher applies to.', permissions=[DiscountPermissions.MANAGE_DISCOUNTS])
    variants = ConnectionField(ProductVariantCountableConnection, description='List of product variants this voucher applies to.' + ADDED_IN_31, permissions=[DiscountPermissions.MANAGE_DISCOUNTS])
    countries = NonNullList(types.CountryDisplay, description='List of countries available for the shipping voucher.')
    translation = TranslationField(VoucherTranslation, type_name='voucher', resolver=ChannelContextType.resolve_translation)
    discount_value_type = DiscountValueTypeEnum(description='Determines a type of discount for voucher - value or percentage', required=True)
    discount_value = graphene.Float(description='Voucher value.')
    currency = graphene.String(description='Currency code for voucher.')
    min_spent = graphene.Field(Money, description='Minimum order value to apply voucher.')
    type = VoucherTypeEnum(description='Determines a type of voucher.', required=True)
    channel_listings = PermissionsField(NonNullList(VoucherChannelListing), description='List of availability in channels for the voucher.', permissions=[DiscountPermissions.MANAGE_DISCOUNTS])
