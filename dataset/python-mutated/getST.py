...
from impacket.krb5.asn1 import AP_REQ, AS_REP, TGS_REQ, Authenticator, TGS_REP, seq_set, seq_set_iter, PA_FOR_USER_ENC, Ticket as TicketAsn1, EncTGSRepPart, PA_PAC_OPTIONS, EncTicketPart
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(add_help=True, description='Given a password, hash or aesKey, it will request a Service Ticket and save it as ccache')
    parser.add_argument('identity', action='store', help='[domain/]username[:password]')
    parser.add_argument('-spn', action='store', required=True, help='SPN (service/server) of the target service the service ticket will be generated for')
    parser.add_argument('-impersonate', action='store', help='target username that will be impersonated (thru S4U2Self) for quering the ST. Keep in mind this will only work if the identity provided in this scripts is allowed for delegation to the SPN specified')
    ...
    parser.add_argument('-force-forwardable', action='store_true', help='Force the service ticket obtained through S4U2Self to be forwardable. For best results, the -hashes and -aesKey values for the specified -identity should be provided. This allows impresonation of protected users and bypass of "Kerberos-only" constrained delegation restrictions. See CVE-2020-17049')
