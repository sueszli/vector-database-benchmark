...
async def prepare_dictionary(inputs: QueryInputs) -> str:
    ...
    sync_execute(CREATE_DICTIONARY_QUERY.format(database=settings.CLICKHOUSE_DATABASE, dictionary_name=inputs.dictionary_name, user=settings.CLICKHOUSE_USER, password=settings.CLICKHOUSE_PASSWORD, cluster_name=settings.CLICKHOUSE_CLUSTER))
...
async def drop_dictionary(inputs: QueryInputs) -> None:
    ...
    sync_execute(DROP_DICTIONARY_QUERY.format(database=settings.CLICKHOUSE_DATABASE, dictionary_name=inputs.dictionary_name))
...
async def select_persons_to_delete(inputs: QueryInputs) -> list[SerializablePersonOverrideToDelete]:
    ...
    to_delete_rows = sync_execute(SELECT_PERSONS_TO_DELETE_QUERY.format(database=settings.CLICKHOUSE_DATABASE), {'latest_created_at': latest_created_at})
    ...
    for row in to_delete_rows:
        ...
        try:
            absolute_oldest_event_at = sync_execute(SELECT_CREATED_AT_FOR_PERSON_EVENT_QUERY.format(database=settings.CLICKHOUSE_DATABASE), {'team_id': person_to_delete.team_id, 'old_person_id': person_to_delete.old_person_id, 'oldest_event_at': person_oldest_event_at})[0][0]
        except IndexError:
            absolute_oldest_event_at = EPOCH
        if absolute_oldest_event_at != EPOCH:
            min_oldest_event_at = min(person_oldest_event_at, absolute_oldest_event_at)
        else:
    ...
    persons_to_delete.extend((older_person._make_serializable() for older_person in older_persons_to_delete if older_person.old_person_id in persons_to_delete_ids))
...
async def squash_events_partition(inputs: QueryInputs) -> None:
    ...
    for partition_id in inputs.partition_ids:
        ...
        parameters = {'partition_id': partition_id, 'team_ids': inputs.team_ids, 'latest_created_at': latest_created_at}
        if inputs.dry_run is True:
            ...
            continue
        sync_execute(query.format(database=settings.CLICKHOUSE_DATABASE, dictionary_name=inputs.dictionary_name, team_id_filter='AND team_id in %(team_ids)s' if inputs.team_ids else ''), parameters)
...
async def delete_squashed_person_overrides_from_clickhouse(inputs: QueryInputs) -> None:
    ...
    latest_created_at = inputs.latest_created_at.timestamp() if inputs.latest_created_at else inputs.latest_created_at
    parameters = {'old_person_ids': old_person_ids_to_delete, 'latest_created_at': latest_created_at}
    if inputs.dry_run is True:
...
async def delete_squashed_person_overrides_from_postgres(inputs: QueryInputs) -> None:
    ...
    with psycopg2.connect(dbname=settings.DATABASES['default']['NAME'], user=settings.DATABASES['default']['USER'], password=settings.DATABASES['default']['PASSWORD'], host=settings.DATABASES['default']['HOST'], port=settings.DATABASES['default']['PORT'], **settings.DATABASES['default'].get('SSL_OPTIONS', {})) as connection:
        with connection.cursor() as cursor:
            for person_override_to_delete in inputs.iter_person_overides_to_delete():
                ...
                cursor.execute(SELECT_ID_FROM_OVERRIDE_UUID, {'team_id': person_override_to_delete.team_id, 'uuid': person_override_to_delete.old_person_id})
                row = cursor.fetchone()
                if not row:
                    continue
                old_person_id = row[0]
                cursor.execute(SELECT_ID_FROM_OVERRIDE_UUID, {'team_id': person_override_to_delete.team_id, 'uuid': person_override_to_delete.override_person_id})
                row = cursor.fetchone()
                if not row:
                    continue
                override_person_id = row[0]
                parameters = {'team_id': person_override_to_delete.team_id, 'old_person_id': old_person_id, 'override_person_id': override_person_id, 'latest_version': person_override_to_delete.latest_version}
                if inputs.dry_run is True:
                    activity.logger.info('This is a DRY RUN so nothing will be deleted.')
                    activity.logger.info('Would have run query: %s with parameters %s', DELETE_FROM_PERSON_OVERRIDES, parameters)
                    continue
                ...
                if not row:
                    continue
                deleted_id = row[0]
                cursor.execute(DELETE_FROM_PERSON_OVERRIDE_MAPPINGS, {'id': deleted_id})

@contextlib.asynccontextmanager
async def person_overrides_dictionary(workflow, query_inputs: QueryInputs, retry_policy: RetryPolicy) -> AsyncIterator[str]:
    ...
    """
    await workflow.execute_activity(prepare_person_overrides, query_inputs, start_to_close_timeout=timedelta(seconds=60), retry_policy=retry_policy)
    latest_created_at = await workflow.execute_activity(prepare_dictionary, query_inputs, start_to_close_timeout=timedelta(seconds=60), retry_policy=retry_policy)
    try:
        yield latest_created_at
    finally:
        await workflow.execute_activity(drop_dictionary, query_inputs, start_to_close_timeout=timedelta(seconds=60), retry_policy=retry_policy)
...
class SquashPersonOverridesWorkflow(PostHogWorkflow):
    ...
    async def run(self, inputs: SquashPersonOverridesInputs):
        ...
        query_inputs = QueryInputs(dictionary_name=inputs.dictionary_name, team_ids=inputs.team_ids, dry_run=inputs.dry_run)
        async with person_overrides_dictionary(workflow, query_inputs, retry_policy=RetryPolicy(maximum_attempts=3, initial_interval=timedelta(seconds=10))) as latest_created_at:
            query_inputs._latest_created_at = latest_created_at
            query_inputs.partition_ids = list(inputs.iter_partition_ids())
            persons_to_delete = await workflow.execute_activity(select_persons_to_delete, query_inputs, start_to_close_timeout=timedelta(seconds=60), retry_policy=retry_policy)
            query_inputs.person_overrides_to_delete = persons_to_delete
            await workflow.execute_activity(squash_events_partition, query_inputs, start_to_close_timeout=timedelta(seconds=300), retry_policy=retry_policy)
            ...
            if not persons_to_delete:
                ...
                return
            await workflow.execute_activity(delete_squashed_person_overrides_from_clickhouse, query_inputs, start_to_close_timeout=timedelta(seconds=300), retry_policy=retry_policy)
            await workflow.execute_activity(delete_squashed_person_overrides_from_postgres, query_inputs, start_to_close_timeout=timedelta(seconds=300), retry_policy=retry_policy)
