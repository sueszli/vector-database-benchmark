...
from fairseq.data import AppendTokenDataset, ConcatDataset, DenoisingDataset, Dictionary, PrependTokenDataset, ResamplingDataset, SortDataset, TokenBlockDataset, data_utils
...
class MultilingualDenoisingConfig(DenoisingConfig):
    multilang_sampling_alpha: float = field(default=1.0, metadata={'help': 'smoothing alpha for sample ratios across multiple datasets'})
    add_lang_token: bool = field(default=False, metadata={'help': ''})
    langs: Optional[str] = field(default=None, metadata={'help': 'language ids we are considering'})
    no_whole_word_mask_langs: str = field(default='', metadata={'help': "languages without spacing between words don't support whole word masking"})
