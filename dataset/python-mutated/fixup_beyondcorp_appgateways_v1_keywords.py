...
class beyondcorp_appgatewaysCallTransformer(cst.CSTTransformer):
    CTRL_PARAMS: Tuple[str] = ('retry', 'timeout', 'metadata')
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_app_gateway': ('parent', 'app_gateway', 'app_gateway_id', 'request_id', 'validate_only'), 'delete_app_gateway': ('name', 'request_id', 'validate_only'), 'get_app_gateway': ('name',), 'list_app_gateways': ('parent', 'page_size', 'page_token', 'filter', 'order_by')}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=beyondcorp_appgatewaysCallTransformer()):
    if False:
        for i in range(10):
            print('nop')
