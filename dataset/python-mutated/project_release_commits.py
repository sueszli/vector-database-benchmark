...
class ProjectReleaseCommitsEndpoint(ProjectEndpoint):
    ...
    def get(self, request: Request, project, version) -> Response:
        ...
        try:
            release = Release.objects.get(organization_id=organization_id, projects=project, version=version)
        ...
        if repo_id:
            try:
                repo = Repository.objects.get(organization_id=organization_id, external_id=repo_id, status=ObjectStatus.ACTIVE)
                queryset = queryset.filter(commit__repository_id=repo.id)
            except Repository.DoesNotExist:
                raise ResourceDoesNotExist
        elif repo_name:
            try:
                repo = Repository.objects.get(organization_id=organization_id, name=repo_name, status=ObjectStatus.ACTIVE)
                queryset = queryset.filter(commit__repository_id=repo.id)
            except Repository.DoesNotExist:
                raise ResourceDoesNotExist
