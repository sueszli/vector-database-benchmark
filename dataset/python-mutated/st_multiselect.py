...
i5 = st.multiselect('multiselect 5', list(map(lambda x: f'{x} I am a ridiculously long string to have in a multiselect, so perhaps I should just not wrap and go to the next line.', range(5))))
...
set_multiselect_9 = st.checkbox('set_multiselect_9', on_change=set_multiselect_9_to_have_bad_state)
...
with st.form('my_max_selections_ms_in_form'):
    i10 = st.multiselect('multiselect 10', options, max_selections=1, key='multiselect 10')
