...
class TestAsyncWebClient_HttpRetries(unittest.TestCase):
    ...
    async def test_remote_disconnected(self):
        ...
        client = AsyncWebClient(base_url='http://localhost:8888', token='xoxb-remote_disconnected', team_id='T111', retry_handlers=[retry_handler])
        try:
    ...
    async def test_ratelimited_no_retry(self):
        client = AsyncWebClient(base_url='http://localhost:8888', token='xoxb-ratelimited', team_id='T111')
        try:
            await client.auth_test()
            self.fail('An exception is expected')
    ...
    async def test_ratelimited(self):
        client = AsyncWebClient(base_url='http://localhost:8888', token='xoxb-ratelimited_only_once', team_id='T111')
        client.retry_handlers.append(AsyncRateLimitErrorRetryHandler())
        await client.auth_test()

    @async_test
    async def test_fatal_error_no_retry(self):
        client = AsyncWebClient(base_url='http://localhost:8888', token='xoxb-fatal_error', team_id='T111')
        try:
            await client.auth_test()
            self.fail('An exception is expected')
        except err.SlackApiError as e:
    ...
    async def test_fatal_error(self):
        client = AsyncWebClient(base_url='http://localhost:8888', token='xoxb-fatal_error_only_once', team_id='T111')
    ...
    async def test_retries(self):
        retry_handler = MyRetryHandler(max_retry_count=2)
        client = AsyncWebClient(token='xoxp-remote_disconnected', base_url='http://localhost:8888', retry_handlers=[retry_handler])
        try:
            await client.auth_test()
            self.fail('An exception is expected')
        except Exception as _:
    ...
    async def test_server_error(self):
        client = AsyncWebClient(base_url='http://localhost:8888', token='xoxb-server_error_only_once', team_id='T111')
