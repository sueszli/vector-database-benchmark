...
HYPERLINK_TAG_WITH_URL_PATTERN = '(.*?<a\\s+href=\\\\?\\")(\\w+://\\S+[^\\\\])(\\\\?\\">)'
...
def propagate_names_for_rich_text_attribute_value_translations(apps, schema_editor):
    ...
    queryset = AttributeValueTranslation.objects.exclude(rich_text=None).filter(name='').order_by('pk')
    for batch_pks in queryset_in_batches(queryset):
        batch = AttributeValueTranslation.objects.filter(pk__in=batch_pks)
        instances = []
        for instance in batch:
            instance.name = truncatechars(clean_editor_js(instance.rich_text, to_string=True), 100)
            instances.append(instance)
        AttributeValueTranslation.objects.bulk_update(instances, ['name'])

class Migration(migrations.Migration):
    dependencies = [('attribute', '0021_auto_20220406_1713')]
    operations = [migrations.RunPython(propagate_names_for_rich_text_attribute_value_translations, migrations.RunPython.noop)]