...
if __name__ == '__main__':
    ...
    parser.add_argument('--host', default='127.0.0.1', help='Remote host name (domain name or IP address')
    parser.add_argument('--port', type=int, default=65500, help='port number to connect to')
    parser.add_argument('--packet-size', type=int, default=1472, help='packet size.')
    parser.add_argument('-r', '--sample-rate', type=int, default=32000, help='audio signal sample rate [default=%(default)r]')
    parser.add_argument('--no-eof', action='store_true', default=False, help="don't send EOF on disconnect")
    args = parser.parse_args()
    top_block = audio_source(args.host, args.port, args.packet_size, args.sample_rate, not args.no_eof)
    try:
        top_block.run()
