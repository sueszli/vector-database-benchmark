...
from litestar.middleware.rate_limit import DURATION_VALUES, CacheObject, DurationUnit, RateLimitConfig
...
async def test_non_default_store(memory_store: Store) -> None:
    ...
    def handler() -> None:
        if False:
            ...
            return i + 15
        return None
    app = Litestar([handler], middleware=[RateLimitConfig(('second', 10)).middleware], stores={'rate_limit': memory_store})
...
async def test_set_store_name(memory_store: Store) -> None:
    ...
    def handler() -> None:
        if False:
            print('Hello World!')
        return None
    app = Litestar([handler], middleware=[RateLimitConfig(('second', 10), store='some_store').middleware], stores={'some_store': memory_store})
    with TestClient(app) as client:
        res = client.get('/')
        assert res.status_code == 200
    assert await memory_store.exists('RateLimitMiddleware::testclient')
...
async def test_rate_limiting_works_with_mounted_apps(tmpdir: 'Path') -> None:
    ...
    static_files_config = StaticFilesConfig(directories=[tmpdir], path='/src/static')
    rate_limit_config = RateLimitConfig(rate_limit=('minute', 1), exclude=['^/src.*$'])
    with create_test_client([handler], static_files_config=[static_files_config], middleware=[rate_limit_config.middleware]) as client:
        response = client.get('/not-excluded')
        assert response.status_code == HTTP_200_OK
        response = client.get('/not-excluded')
        assert response.status_code == HTTP_429_TOO_MANY_REQUESTS
