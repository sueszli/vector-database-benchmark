...
@testing.parameterize(*testing.product({'shape,dim_in,dim_out': [((), 1, 1), ((1,), 1, 1), ((4, 3), 3, 4), ((6, 2, 3), 6, 6), ((3, 4, 5), 20, 3)], 'scale,dtype': [(2.0, numpy.float16), (None, numpy.float32), (None, numpy.float64), (7.3, numpy.float32), (7.3, numpy.float64)], 'rng_class': [None, numpy.random.RandomState]}))
...
class OrthogonalBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self.dtype == numpy.float16:
            self.check_options = {'atol': 0.005, 'rtol': 0.05}
    ...
    def check_orthogonality(self, w):
        ...
        testing.assert_allclose(dots, numpy.identity(n) * expected_scale ** 2, **self.check_options)
    ...
    def check_initializer_statistics(self, backend_config, n):
        ...
        for samples in sampless:
            if larger_dim == 1:
                ...
                (_, p) = stats.kstest(samples, stats.beta(ab, ab, loc=-expected_scale, scale=2 * expected_scale).cdf)
            assert p >= alpha
...
@testing.parameterize(*testing.product({'shape': [(3,), (4, 3), (21, 4, 5)], 'mode': ['projection', 'basis']}) + testing.product({'shape': [(0,), (3, 4, 5)], 'mode': ['embedding', 'basis']}))
