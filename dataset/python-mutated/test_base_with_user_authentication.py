...
for method_name in dir(Base):
    ...
    setattr(BaseOperationsWithUserAuthenticationTest, method_name.replace('run_mixin_', ''), _test)
    if method_name.startswith('run_mixin_test_execute_'):
        ...
        setattr(BaseOperationsWithUserAuthenticationTest, method_name.replace('run_mixin_', '') + '_with_parent', _test_with_parent)