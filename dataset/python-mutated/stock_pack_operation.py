...
class PackOperation(models.Model):
    ...
    def _get_default_to_loc(self):
        ...
        if default_loc:
            return self.env['stock.location'].browse(default_loc).name
    picking_id = fields.Many2one('stock.picking', 'Stock Picking', required=True, help='The stock operation where the packing has been made')
    ...
    result_package_id = fields.Many2one('stock.quant.package', 'Destination Package', ondelete='cascade', required=False, help='If set, the operations are packed into this package')
    ...
    remaining_qty = fields.Float(compute='_get_remaining_qty', string='Remaining Qty', digits=0, help='Remaining quantity in default UoM according to moves matched with this operation.')
    ...
    state = fields.Selection(selection=[('draft', 'Draft'), ('cancel', 'Cancelled'), ('waiting', 'Waiting Another Operation'), ('confirmed', 'Waiting Availability'), ('partially_available', 'Partially Available'), ('assigned', 'Available'), ('done', 'Done')], related='picking_id.state')
...
class PackOperationLot(models.Model):
    ...
    _sql_constraints = [('qty', 'CHECK(qty >= 0.0)', 'Quantity must be greater than or equal to 0.0!'), ('uniq_lot_id', 'unique(operation_id, lot_id)', 'You have already mentioned this lot in another line'), ('uniq_lot_name', 'unique(operation_id, lot_name)', 'You have already mentioned this lot name in another line')]
...
class OperationLink(models.Model):
    ...
    operation_id = fields.Many2one('stock.pack.operation', 'Operation', ondelete='cascade', required=True)
    move_id = fields.Many2one('stock.move', 'Move', ondelete='cascade', required=True)
