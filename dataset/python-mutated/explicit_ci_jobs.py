...
WORKFLOWS_TO_CHECK = ['binary_builds', 'build', 'master_build']
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='make .circleci/config.yml only have a specific set of jobs and delete GitHub actions')
    parser.add_argument('--job', action='append', help='job name', default=[])
    parser.add_argument('--filter-gha', help='keep only these github actions (glob match)', default='')
    parser.add_argument('--make-commit', action='store_true', help='add change to git with to a do-not-merge commit')
