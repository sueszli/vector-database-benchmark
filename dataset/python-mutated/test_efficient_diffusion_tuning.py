...
class EfficientDiffusionTuningTest(unittest.TestCase):
    ...
    def test_efficient_diffusion_tuning_control_lora_run_pipeline(self):
        ...
        model_revision = 'v1.0.2'
        inputs = {'prompt': 'pale golden rod circle with old lace background', 'cond': 'data/test/images/efficient_diffusion_tuning_sd_control_lora_source.png'}
        edt_pipeline = pipeline(self.task, model_id, model_revision=model_revision)
        result = edt_pipeline(inputs)
        print(f'Efficient-diffusion-tuning-control-lora output: {result}.')

    @unittest.skipUnless(test_level() >= 0, 'skip test in current test level')
    def test_efficient_diffusion_tuning_control_lora_load_model_from_pretrained(self):
        if False:
            return 10
        model_id = 'damo/multi-modal_efficient-diffusion-tuning-control-lora'
        model_revision = 'v1.0.2'
