...
class DockerServiceRegistry:
    ...
    async def start(self, name: str, *, check: Callable[..., Awaitable], timeout: float=30, pause: float=0.1, **kwargs: Any) -> None:
        if name not in self._running_services:
            self.run_command('up', '-d', name)
            self._running_services.add(name)
            await wait_until_responsive(check=ensure_async_callable(check), timeout=timeout, pause=pause, host=self.docker_ip, **kwargs)
...
async def postgres_responsive(host: str) -> bool:
    try:
        conn = await asyncpg.connect(host=host, port=5423, user='postgres', database='postgres', password='super-secret')
