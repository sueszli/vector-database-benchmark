...
class TrainingArgumentsForAuto(TrainingArguments):
    ...
    model_path: str = field(default='facebook/muppet-roberta-base', metadata={'help': 'model path for HPO natural language understanding tasks, default is set to facebook/muppet-roberta-base'})
    ...
    label_all_tokens: bool = field(default=False, metadata={'help': 'For NER task, whether to set the extra tokenized labels to the same label (instead of -100)'})
    pad_to_max_length: bool = field(default=False, metadata={'help': 'Whether to pad all samples to model maximum sentence length. If False, will pad the samples dynamically when batching to the maximum length in the batch. '})
    per_device_eval_batch_size: int = field(default=1, metadata={'help': 'per gpu evaluation batch size'})
...
class Seq2SeqTrainingArgumentsForAuto(TrainingArgumentsForAuto):
    model_path: str = field(default='t5-small', metadata={'help': 'model path for HPO natural language generation tasks, default is set to t5-small'})
