...
async def test_get_result_correctness(graph1):
    ...
    values = await asyncio.gather(*[visualizer._get_result(node.get_stable_uuid()) for node in visualizer.node_to_block])
    ...
    def get_fn_index_for_fn(api_info, fn_name):
        ...
        for (fn_index, fn_api_info) in api_info.items():
            if len(fn_api_info['returns']) == 1 and fn_api_info['returns'][0]['label'] == fn_name:
                return int(fn_index)
    api_info = client.view_api(return_format='dict', print_info=False)['unnamed_endpoints']
