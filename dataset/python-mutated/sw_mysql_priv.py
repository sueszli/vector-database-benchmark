...
_title = '数据库备份权限检测'
_version = 1.0
_ps = '检测MySQL root用户是否具备数据库备份权限'
_level = 3
_date = '2020-09-19'
_ignore = os.path.exists('data/warning/ignore/sw_database_priv.pl')
_tips = ['临时以无授权方式进入数据库，建议恢复root用户所有权限。']
_help = ''
...
def check_run():
    ...
    select_sql = "Select {} FROM mysql.user WHERE user='root' and host=SUBSTRING_INDEX((select current_user()),'@', -1);".format(','.join(base_backup_privs))
