...
class DoorLockClusterHandler(ClusterHandler):
    ...
    _value_attribute = 0
    REPORT_CONFIG = (AttrReportConfig(attr='lock_state', config=REPORT_CONFIG_IMMEDIATE),)
    ...
    async def async_set_user_code(self, code_slot: int, user_code: str) -> None:
        ...
        await self.set_pin_code(code_slot - 1, closures.DoorLock.UserStatus.Enabled, closures.DoorLock.UserType.Unrestricted, user_code)
...
class WindowCovering(ClusterHandler):
    ...
    _value_attribute_lift = closures.WindowCovering.AttributeDefs.current_position_lift_percentage.id
    _value_attribute_tilt = closures.WindowCovering.AttributeDefs.current_position_tilt_percentage.id
    REPORT_CONFIG = (AttrReportConfig(attr='current_position_lift_percentage', config=REPORT_CONFIG_IMMEDIATE), AttrReportConfig(attr='current_position_tilt_percentage', config=REPORT_CONFIG_IMMEDIATE))
    ...
    async def async_update(self):
        ...
        if result is not None:
            self.async_send_signal(f'{self.unique_id}_{SIGNAL_ATTR_UPDATED}', self._value_attribute_lift, 'current_position_lift_percentage', result)
        result = await self.get_attribute_value('current_position_tilt_percentage', from_cache=False)
        self.debug('read current tilt position: %s', result)
        if result is not None:
            self.async_send_signal(f'{self.unique_id}_{SIGNAL_ATTR_UPDATED}', self._value_attribute_tilt, 'current_position_tilt_percentage', result)
