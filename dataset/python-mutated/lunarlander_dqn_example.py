...
from ding.framework.middleware import OffPolicyLearner, StepCollector, interaction_evaluator, data_pusher, eps_greedy_handler, CkptSaver, ContextExchanger, ModelExchanger, online_logger, termination_checker, nstep_reward_enhancer
...
def main():
    ...
    with task.start(async_mode=False, ctx=OnlineRLContext()):
        collector_env = SubprocessEnvManagerV2(env_fn=[lambda : DingEnvWrapper(gym.make('LunarLander-v2')) for _ in range(cfg.env.collector_env_num)], cfg=cfg.env.manager)
        evaluator_env = SubprocessEnvManagerV2(env_fn=[lambda : DingEnvWrapper(gym.make('LunarLander-v2')) for _ in range(cfg.env.evaluator_env_num)], cfg=cfg.env.manager)
        ...
        task.use(termination_checker(max_env_step=int(3000000.0)))
        task.run()
