...
class TestTaskSessionPep8(testutils.PEP8MixIn, TestCase):
    PEP8_FILES = ['golem/task/tasksession.py', 'tests/golem/task/test_tasksession.py']
...
@patch('golem.ranking.manager.database_manager.get_provider_efficacy', Mock(return_value=_fake_get_efficacy()))
...
class TaskSessionTestBase(ConcentMessageMixin, LogTestCase, testutils.TempDirFixture):
...
class ForceReportComputedTaskTestCase(testutils.DatabaseFixture, testutils.TempDirFixture):
...
class ReportComputedTaskTest(ConcentMessageMixin, LogTestCase, testutils.TempDirFixture):
