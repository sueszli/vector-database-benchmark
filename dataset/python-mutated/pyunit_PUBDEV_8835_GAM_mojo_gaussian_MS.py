...
def gam_gaussian_mojo_MS():
    ...
    gamGaussianModel = pyunit_utils.build_save_model_generic(params, x, train, 'response', 'gam', TMPDIR)
    MOJONAME = pyunit_utils.getMojoName(gamGaussianModel._id)
    h2o.download_csv(test, os.path.join(TMPDIR, 'in.csv'))
    (pred_h2o, pred_mojo) = pyunit_utils.mojo_predict(gamGaussianModel, TMPDIR, MOJONAME)
