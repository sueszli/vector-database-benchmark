...
from os_win.constants import HOST_SHUTDOWN_ACTION_SAVE, VM_SNAPSHOT_TYPE_DISABLED, HYPERV_VM_STATE_SUSPENDED, HYPERV_VM_STATE_ENABLED, HOST_SHUTDOWN_ACTION_SHUTDOWN, HYPERV_VM_STATE_DISABLED
...
class Events(Enum):
    ...
    MEM = 'lowered_memory'
    DISK = 'low_diskspace'
MESSAGES = {Events.SMB: 'Port {SMB_PORT} unreachable. Please check firewall settings.', Events.MEM: 'Not enough free RAM to start the VM, lowering memory to {mem_mb} MB', Events.DISK: 'Not enough disk space. Creating VM with min memory'}
EVENTS: Dict[Events, Dict[str, Union[str, Optional[Dict]]]] = {Events.SMB: {'component': Component.hypervisor, 'method': 'setup', 'stage': Stage.exception, 'data': None}, Events.MEM: {'component': Component.hypervisor, 'method': 'start_vm', 'stage': Stage.warning, 'data': None}, Events.DISK: {'component': Component.hypervisor, 'method': 'start_vm', 'stage': Stage.warning, 'data': None}}
...
class HyperVHypervisor(DockerMachineHypervisor):
    ...
    OPTIONS = dict(mem='--hyperv-memory', cpu='--hyperv-cpu-count', disk='--hyperv-disk-size', boot2docker_url='--hyperv-boot2docker-url', no_virt_mem='--hyperv-disable-dynamic-memory', virtual_switch='--hyperv-virtual-switch')
    BOOT2DOCKER_URL = 'https://golem-bootdocker.cdn.golem.network/boot2docker/v18.09.7-golem/boot2docker-v18.09.7-golem.iso'
    ...
    GET_VSWITCH_SCRIPT_PATH = os.path.join(SCRIPTS_PATH, 'docker', 'get-default-vswitch.ps1')
    GET_HYPERV_SCRIPT_PATH = os.path.join(SCRIPTS_PATH, 'virtualization', 'get-hyperv-state.ps1')
    START_VM_RETRIES = 2
