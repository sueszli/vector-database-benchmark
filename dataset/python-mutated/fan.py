...
ORDERED_NAMED_FAN_SPEEDS = [tasmota_const.FAN_SPEED_LOW, tasmota_const.FAN_SPEED_MEDIUM, tasmota_const.FAN_SPEED_HIGH]
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    hass.data[DATA_REMOVE_DISCOVER_COMPONENT.format(FAN_DOMAIN)] = async_dispatcher_connect(hass, TASMOTA_DISCOVERY_ENTITY_NEW.format(FAN_DOMAIN), async_discover)

class TasmotaFan(TasmotaAvailability, TasmotaDiscoveryUpdate, FanEntity):
    ...
    async def async_set_percentage(self, percentage: int) -> None:
        ...
        if percentage == 0:
            ...
            tasmota_speed = percentage_to_ordered_list_item(ORDERED_NAMED_FAN_SPEEDS, percentage)
            await self._tasmota_entity.set_speed(tasmota_speed)

    async def async_turn_on(self, percentage: int | None=None, preset_mode: str | None=None, **kwargs: Any) -> None:
        """Turn the fan on."""
        await self.async_set_percentage(percentage or ordered_list_item_to_percentage(ORDERED_NAMED_FAN_SPEEDS, self._fan_speed))
