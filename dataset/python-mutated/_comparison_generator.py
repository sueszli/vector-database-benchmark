...
_np_matrix = 'Use of :class:`numpy.matrix` is discouraged in NumPy and thus we have no plan to add it to CuPy.'
_np_poly1d = 'Use of :class:`numpy.poly1d` is discouraged in NumPy and thus we have stopped adding functions with the interface.'
_dtype_na = '`object` and string dtypes are not supported in GPU and thus left unimplemented in CuPy.'
_dtype_time = '`datetime64` and `timedelta64` dtypes are currently unsupported.'
_dtype_rec = 'Structured arrays and record arrays are currently unsupported.'
_float_error_state = 'Floating point error handling depends on CPU-specific features which is not available in GPU.'
