...
from ..pattern_matcher import Arg, CallFunction, filter_nodes, get_arg_value, KeywordArg, MULTIPLE
from ..virtualized import ops
from .freezing_patterns import register_freezing_graph_pattern
from .post_grad import register_lowering_pattern
from .quantization import _register_quantization_lowerings, _register_quantization_weight_pack_pass
if torch._C._has_mkldnn:
    ...
    def _register_hardtanh_fusion_lowering(pattern, computation_op, is_bf16=False):
        ...
        def fn(match, *args, **kwargs):
            ...
            if matched:
                ...
                return out
        return fn
    _binary_attr = {aten.add: 'add', ops.add: 'add', aten.sub: 'sub', ops.sub: 'sub'}
    ...
    def _register_binary_unary_maybe_inplace_fusion_lowering(pattern, computation_op, binary_op, inplace_fusion_op, outplace_fusion_op, unary_attr=None, other_index=None):
        ...
        return fn
    computation_ops = [mkldnn._convolution_pointwise.default, mkldnn._linear_pointwise.default, mkldnn._convolution_transpose_pointwise.default]
    ...
    def _is_packable_linear(match):
        ...
        if input_meta_value.dtype == torch.bfloat16 or weight_meta_value.dtype == torch.bfloat16:
            if not mkldnn._is_mkldnn_bf16_supported():
                return False
        return True
    _aten_conv_args = (Arg(), Arg(), Arg(), Arg(), Arg(), Arg(), KeywordArg('is_transposed'), Arg(), Arg())
    _aten_mkldnn_rnn_layer_args = (Arg(), Arg(), Arg(), Arg(), Arg(), Arg(), Arg(), KeywordArg('reverse'), Arg(), Arg(), Arg(), Arg(), Arg(), Arg(), Arg(), Arg())
