...
class DeleteMetadata(BaseMetadataMutation):
    ...
    class Arguments:
        id = graphene.ID(description='ID or token (for Order and Checkout) of an object to update.', required=True)
        keys = NonNullList(graphene.String, description='Metadata keys to delete.', required=True)

    @classmethod
    def perform_mutation(cls, _root, info: ResolveInfo, /, *, id: str, keys: list[str]):
        if False:
            while True:
                i = 10
