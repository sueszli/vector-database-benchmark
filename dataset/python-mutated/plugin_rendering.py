...
class RenderedPlaceholder:
    __slots__ = ('language', 'site_id', 'cached', 'editable', 'placeholder', 'has_content')
...
class ContentRenderer(BaseRenderer):
    plugin_edit_template = '<template class="cms-plugin cms-plugin-start cms-plugin-{pk}"></template>{content}<template class="cms-plugin cms-plugin-end cms-plugin-{pk}"></template>'
    placeholder_edit_template = '{content} <div class="cms-placeholder cms-placeholder-{placeholder_id}"></div> <script data-cms>{plugin_js}\n{placeholder_js}</script>'
