...
torch.set_printoptions(precision=2, threshold=None, edgeitems=16, linewidth=480, profile=None, sci_mode=False)
...
if __name__ == '__main__':
    dtype_lookup = {'int8': torch.int8, 'fp16': torch.float16, 'bf16': torch.bfloat16, 'fp32': torch.float32}
    parser = argparse.ArgumentParser(description='Semi-Structured Sparsity Benchmarks')
    parser.add_argument('--mode', type=str, choices=['nvidia-bert', 'nvidia-fixed-k', 'nvidia-fixed-mn'])
    parser.add_argument('--dtype', type=str, choices=dtype_lookup.keys(), default='fp16')
    ...
    if args.mode == 'nvidia-bert':
        bert_shapes = [(3072, 1024, 16384), (4096, 1024, 16384), (1024, 1024, 16384), (1024, 4096, 16384)]
        results = (eval_fn(m, k, n, dtype, args.contiguous, args.backend) for (m, k, n) in tqdm(bert_shapes))
    elif args.mode == 'nvidia-fixed-k':
        mn_vals = [3072, 4096, 5120, 6144, 7168, 8192, 9216, 10240, 11264, 12288, 13312, 14336, 15360, 16384, 17408, 18432, 19456, 20480]
        results = (eval_fn(mn, 10240, mn, dtype, args.contiguous, args.backend) for mn in tqdm(mn_vals))
    elif args.mode == 'nvidia-fixed-mn':
        k_vals = [2560, 3840, 5120, 6400, 7680, 8960, 10240, 11520, 12800, 14080, 15360, 16640, 17920, 19200, 20480]
        results = (eval_fn(10240, k, 10240, dtype, args.contiguous, args.backend) for k in tqdm(k_vals))
