...
import numpy as np
import pandas as pd
from transformers import MODEL_FOR_CAUSAL_LM_MAPPING, MODEL_FOR_MASKED_LM_MAPPING, MODEL_FOR_MULTIPLE_CHOICE_MAPPING, MODEL_FOR_NEXT_SENTENCE_PREDICTION_MAPPING, MODEL_FOR_SEQ_TO_SEQ_CAUSAL_LM_MAPPING, MODEL_FOR_SEQUENCE_CLASSIFICATION_MAPPING, MODEL_FOR_TABLE_QUESTION_ANSWERING_MAPPING, MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING, TapasConfig, is_torch_available
...
if is_torch_available():
    import torch
    from transformers import TapasForMaskedLM, TapasForQuestionAnswering, TapasForSequenceClassification, TapasModel, TapasTokenizer
    from transformers.models.tapas.modeling_tapas import IndexMap, ProductIndexMap, flatten, gather, range_index_map, reduce_max, reduce_mean, reduce_sum
...
class TapasModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TapasModel, TapasForMaskedLM, TapasForQuestionAnswering, TapasForSequenceClassification) if is_torch_available() else None
    pipeline_model_mapping = {'feature-extraction': TapasModel, 'fill-mask': TapasForMaskedLM, 'table-question-answering': TapasForQuestionAnswering, 'text-classification': TapasForSequenceClassification, 'zero-shot': TapasForSequenceClassification} if is_torch_available() else {}
    test_pruning = False
