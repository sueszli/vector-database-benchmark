...
class Tab:
    ...
    async def evaluate_js(self, js: str, run_async: bool | None=False, manage_socket: bool | None=True, get_result: bool=True):
        try:
            ...
            res = await self._send_devtools_cmd({'method': 'Runtime.evaluate', 'params': {'expression': js, 'userGesture': True, 'awaitPromise': run_async}}, get_result)
        finally:
            if manage_socket:
                await self.close_websocket()
        return res
    ...
    async def close(self, manage_socket: bool=True):
        try:
            if manage_socket:
                await self.open_websocket()
            res = await self._send_devtools_cmd({'method': 'Page.close'}, False)
        finally:
            if manage_socket:
                await self.close_websocket()
        return res
    ...
    async def enable(self):
        ...
        """
        await self._send_devtools_cmd({'method': 'Page.enable'}, False)

    async def disable(self):
        """
        Disables page domain notifications.
        """
        await self._send_devtools_cmd({'method': 'Page.disable'}, False)

    async def refresh(self, manage_socket: bool=True):
        try:
            if manage_socket:
                await self.open_websocket()
            await self._send_devtools_cmd({'method': 'Page.reload'}, False)
        finally:
            if manage_socket:
                await self.close_websocket()
        return
    ...
    async def reload_and_evaluate(self, js: str, manage_socket: bool=True):
        ...
        try:
            ...
            await self._send_devtools_cmd({'method': 'Runtime.evaluate', 'params': {'expression': 'location.reload();', 'userGesture': True, 'awaitPromise': False}}, False)
            breakpoint_res = await self._send_devtools_cmd({'method': 'Debugger.setInstrumentationBreakpoint', 'params': {'instrumentation': 'beforeScriptExecution'}}, True)
            assert breakpoint_res is not None
            logger.info(breakpoint_res)
            for _ in range(20):
                await self._send_devtools_cmd({'method': 'Runtime.evaluate', 'params': {'expression': js, 'userGesture': True, 'awaitPromise': False}}, False)
            await self._send_devtools_cmd({'method': 'Debugger.removeBreakpoint', 'params': {'breakpointId': breakpoint_res['result']['breakpointId']}}, False)
            for _ in range(4):
    ...
    async def add_script_to_evaluate_on_new_document(self, js: str, add_dom_wrapper: bool=True, manage_socket: bool=True, get_result: bool=True):
        ...
        * whenever the child frame is attached or navigated. In this case, the
        ...
        try:
            ...
            if manage_socket:
                await self.open_websocket()
            res = await self._send_devtools_cmd({'method': 'Page.addScriptToEvaluateOnNewDocument', 'params': {'source': wrappedjs}}, get_result)
        finally:
            if manage_socket:
                await self.close_websocket()
        return res
    ...
    async def remove_script_to_evaluate_on_new_document(self, script_id: str, manage_socket: bool=True):
        ...
        try:
            if manage_socket:
                await self.open_websocket()
            await self._send_devtools_cmd({'method': 'Page.removeScriptToEvaluateOnNewDocument', 'params': {'identifier': script_id}}, False)
        finally:
            if manage_socket:
                await self.close_websocket()
...
CLOSEABLE_URLS = ['about:blank', 'data:text/html,%3Cbody%3E%3C%2Fbody%3E']
DO_NOT_CLOSE_URL = 'Valve Steam Gamepad/default'
