...
def print_descr(annot):
    ...
    annot.parent.insert_text(annot.rect.br + (10, -5), '%s annotation' % annot.type[1], color=red)
doc = fitz.open()
page = doc.new_page()
page.set_rotation(0)
...
print_descr(annot)
r = r + displ
annot = page.add_freetext_annot(r, t1, fontsize=10, rotate=90, text_color=blue, fill_color=gold, align=fitz.TEXT_ALIGN_CENTER)
...
page.insert_text(pos, highlight, morph=(pos, fitz.Matrix(-5)))
...
print_descr(annot)
r += displ
annot = page.add_file_annot(r.tl, b'just anything for testing', 'testdata.txt')
print_descr(annot)
r += displ
...
print_descr(annot)
r += displ + (0, 0, 50, 10)
rc = page.insert_textbox(r, 'This content will be removed upon applying the redaction.', color=blue, align=fitz.TEXT_ALIGN_CENTER)
