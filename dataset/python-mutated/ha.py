...
class InstanceLink(BaseModel):
    ...
    link_state = models.CharField(choices=States.choices, default=States.ADDING, max_length=16, help_text=_('Indicates the current life cycle stage of this peer link.'))
...
class Instance(HasPolicyEditsMixin, BaseModel):
    ...
    class Meta:
        app_label = 'main'
        ordering = ('hostname',)
        constraints = [models.UniqueConstraint(fields=['ip_address'], condition=~Q(ip_address=''), name='unique_ip_address_not_empty', violation_error_message=_('Field ip_address must be unique.'))]
    ...
    cpu = models.DecimalField(default=Decimal(0.0), max_digits=4, decimal_places=1, editable=False)
    memory = models.BigIntegerField(default=0, editable=False, help_text=_('Total system memory of this instance in bytes.'))
    errors = models.TextField(default='', blank=True, editable=False, help_text=_('Any error details from the last health check.'))
    last_seen = models.DateTimeField(null=True, editable=False, help_text=_('Last time instance ran its heartbeat task for main cluster nodes. Last known connection to receptor mesh for execution nodes.'))
    health_check_started = models.DateTimeField(null=True, editable=False, help_text=_('The last time a health check was initiated on this instance.'))
    last_health_check = models.DateTimeField(null=True, editable=False, help_text=_('Last time a health check was ran on this instance to refresh cpu, memory, and capacity.'))
    ...
    node_state = models.CharField(choices=States.choices, default=States.READY, max_length=16, help_text=_('Indicates the current life cycle stage of this instance.'))
    listener_port = models.PositiveIntegerField(blank=True, null=True, default=None, validators=[MinValueValidator(1024), MaxValueValidator(65535)], help_text=_('Port that Receptor will listen for incoming connections on.'))
...
class InstanceGroup(HasPolicyEditsMixin, BaseModel, RelatedJobsMixin, ResourceMixin):
    ...
    created = models.DateTimeField(auto_now_add=True)
    modified = models.DateTimeField(auto_now=True)
    instances = models.ManyToManyField('Instance', related_name='rampart_groups', editable=False, help_text=_('Instances that are members of this InstanceGroup'))
    is_container_group = models.BooleanField(default=False)
    credential = models.ForeignKey('Credential', related_name='%(class)ss', blank=True, null=True, default=None, on_delete=models.SET_NULL)
    pod_spec_override = prevent_search(models.TextField(blank=True, default=''))
    ...
    read_role = ImplicitRoleField(parent_role=['singleton:' + ROLE_SINGLETON_SYSTEM_AUDITOR, 'use_role', 'admin_role'])
    ...
    policy_instance_list = models.JSONField(default=list, blank=True, help_text=_('List of exact-match Instances that will always be automatically assigned to this group'))
...
class UnifiedJobTemplateInstanceGroupMembership(models.Model):
    ...
    position = models.PositiveIntegerField(null=True, default=None, db_index=True)
