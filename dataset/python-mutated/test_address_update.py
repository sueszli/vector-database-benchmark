...
def test_address_update_mutation(staff_api_client, customer_user, permission_manage_users, graphql_address_data):
    if False:
        return 10
    ...
    assert generate_address_search_document_value(address_obj) in customer_user.search_document

@freeze_time('2022-05-12 12:00:00')
@patch('saleor.plugins.webhook.plugin.get_webhooks_for_event')
@patch('saleor.plugins.webhook.plugin.trigger_webhooks_async')
def test_address_update_mutation_trigger_webhook(mocked_webhook_trigger, mocked_get_webhooks_for_event, any_webhook, staff_api_client, customer_user, permission_manage_users, graphql_address_data, settings):
    if False:
        return 10
    ...
    response = staff_api_client.post_graphql(ADDRESS_UPDATE_MUTATION, variables, permissions=[permission_manage_users])
    content = get_graphql_content(response)
    address.refresh_from_db()
    assert content['data']['addressUpdate']
    mocked_webhook_trigger.assert_called_with(*generate_address_webhook_call_args(address, WebhookEventAsyncType.ADDRESS_UPDATED, staff_api_client.user, any_webhook))

@patch('saleor.graphql.account.mutations.base.prepare_user_search_document_value')
def test_address_update_mutation_no_user_assigned(prepare_user_search_document_value_mock, staff_api_client, address, permission_manage_users, graphql_address_data):
    if False:
        while True:
            i = 10
    query = ADDRESS_UPDATE_MUTATION
