...
if __name__ == '__main__':
    ...
    parser.add_argument('--cores', type=int, default=1, help='The number of cpu cores you want to use on each node.You can change it depending on your own cluster setting.')
    parser.add_argument('--epoch', type=int, default=25, help='Max number of epochs to train in each trial.')
    parser.add_argument('--datadir', type=str, help='Use local npz file by default.')
    parser.add_argument('--plot_figures', type=bool, default=True, help='Plot Figure 1, 6, 19 in the http://arxiv.org/abs/1909.13403')
    parser.add_argument('--batch_size', type=int, default=100, help='Training batch size.')
    parser.add_argument('--checkpoint_path', type=str, default='./checkpoint', help='The checkpoint root dir.')
    parser.add_argument('--checkpoint_every_n_epoch', type=int, default=0, help='Checkpoint per n epoch.')
    ...
    doppelganger = DPGANSimulator(L_max=550, sample_len=10, feature_dim=1, num_real_attribute=3, num_threads=args.cores, ckpt_dir=args.checkpoint_path, checkpoint_every_n_epoch=args.checkpoint_every_n_epoch)
    doppelganger.fit(data_feature=real_data['data_feature'], data_attribute=real_data['data_attribute'], data_gen_flag=real_data['data_gen_flag'], feature_outputs=feature_outputs, attribute_outputs=attribute_outputs, epoch=args.epoch, batch_size=args.batch_size)
    print('saving to', args.checkpoint_path)
    doppelganger.save(args.checkpoint_path)
    print('saved to', args.checkpoint_path)
    print('generating data')
    (features, attributes, gen_flags, lengths) = doppelganger.generate(sample_num=real_data['data_feature'].shape[0])
