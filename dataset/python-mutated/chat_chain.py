...
import utils
import websocket
from chat_chain_prompts import ASSISTANT_PREFIX, CUSTOM_INSTRUCTIONS_PREFIX, HUMAN_PREFIX, JSON_FORMAT_NO_PAYLOAD, JSON_FORMAT_PAYLOAD, OBSERVATION_SEQ, PREFIX, SUFFIX, THOUGHT_SEQ
...
llm = HFInference(inference_server_url=settings.inference_server_url, max_new_tokens=512, stop_sequences=[], top_k=50, temperature=0.2, seed=43, repetition_penalty=1 / 0.92)
...
if __name__ == '__main__':
    plugin = inference.PluginEntry(enabled=True, url='http://localhost:8082/ai-plugin.json', plugin_config=inference.PluginConfig(name_for_human='Local dev plugin', name_for_model='Local dev plugin', description_for_model='Local dev plugin', description_for_human='Local dev plugin', schema_version='0.0.1', api={'type': 'openapi', 'url': 'http://localhost:8082/openapi.json', 'has_user_authentication': False}, auth={'type': 'none'}))
    model_config = ModelConfig(model_id='decapoda-research/llama-30b-hf', max_input_length=1024, max_total_length=2048)
    ...
    worker_config = inference.WorkerConfig(model_config=model_config, model_id=model_config.model_id, max_input_length=model_config.max_input_length, max_total_length=model_config.max_total_length, do_sample=True, seed=42)
    while True:
        input_ = input('Enter your input: ')
        if input == 'exit':
            break
        work_request = inference.WorkRequest(request_type='work', parameters=work_parameters, thread=inference.Thread(messages=[inference.MessageRead(id='1', chat_id='1', parent_id=None, content='Hi, what is your name?', created_at=datetime.datetime.now(), role='prompter', state=inference.MessageState.complete, score=0, work_parameters=work_parameters, reports=[]), inference.MessageRead(id='1', chat_id='1', parent_id=None, content='Hello, my name is Open Assistant, how i can help you today?', created_at=datetime.datetime.now(), role='assistant', state=inference.MessageState.complete, score=0, work_parameters=work_parameters, reports=[]), inference.MessageRead(id='1', chat_id='1', parent_id=None, content=input_, created_at=datetime.datetime.now(), role='prompter', state=inference.MessageState.in_progress, score=0, work_parameters=work_parameters, reports=[])]))
