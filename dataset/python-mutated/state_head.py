...
from ray.dashboard.consts import RAY_STATE_SERVER_MAX_HTTP_REQUEST, RAY_STATE_SERVER_MAX_HTTP_REQUEST_ALLOWED, RAY_STATE_SERVER_MAX_HTTP_REQUEST_ENV_NAME
from ray.dashboard.datacenter import DataSource
from ray.dashboard.modules.log.log_manager import LogsManager
from ray.dashboard.optional_utils import rest_response
from ray.dashboard.state_aggregator import StateAPIManager
from ray.dashboard.utils import Change
from ray.util.state.common import RAY_MAX_LIMIT_FROM_API_SERVER, ListApiOptions, GetLogOptions, PredicateType, SupportedFilterType, SummaryApiOptions, SummaryApiResponse, DEFAULT_RPC_TIMEOUT, DEFAULT_LIMIT, DEFAULT_LOG_LIMIT
...
class StateHead(dashboard_utils.DashboardHeadModule, RateLimitedModule):
    ...
    async def limit_handler_(self):
        return self._reply(success=False, error_message=f"Max number of in-progress requests={self.max_num_call_} reached. To set a higher limit, set environment variable: export {RAY_STATE_SERVER_MAX_HTTP_REQUEST_ENV_NAME}='xxx'. Max allowed = {RAY_STATE_SERVER_MAX_HTTP_REQUEST_ALLOWED}", result=None)
    ...
    async def _update_raylet_stubs(self, change: Change):
        ...
        if change.new:
            ...
            self._state_api_data_source_client.register_raylet_client(node_id, node_info['nodeManagerAddress'], int(node_info['nodeManagerPort']), int(node_info['runtimeEnvAgentPort']))
    ...
    async def _update_agent_stubs(self, change: Change):
        ...
        if change.new:
            ...
            self._state_api_data_source_client.register_agent_client(node_id, ip, int(ports[1]))

    async def _handle_list_api(self, list_api_fn: Callable[[ListApiOptions], dict], req: aiohttp.web.Request):
        try:
            result = await list_api_fn(option=self._options_from_req(req))
            return self._reply(success=True, error_message='', result=asdict(result))
    ...
    @RateLimitedModule.enforce_max_concurrent_calls
    async def list_cluster_events(self, req: aiohttp.web.Request) -> aiohttp.web.Response:
    ...
    async def list_logs(self, req: aiohttp.web.Request) -> aiohttp.web.Response:
        ...
        if not node_id and (not node_ip):
            return self._reply(success=False, error_message='Both node id and node ip are not provided. Please provide at least one of them.', result=None)
        node_id = node_id or self._log_api.ip_to_node_id(node_ip)
        if not node_id:
            return self._reply(success=False, error_message=f'Cannot find matching node_id for a given node ip {node_ip}', result=None)
        try:
            result = await self._log_api.list_logs(node_id, timeout, glob_filter=glob_filter)
        except DataSourceUnavailable as e:
            return self._reply(success=False, error_message=str(e), result=None, reason=str(e))
    ...
    async def get_logs(self, req: aiohttp.web.Request):
        ...
        try:
            ...
            error_msg.extend(f'Closing HTTP stream due to internal server error.\n{e}'.encode())
            await response.write(bytes(error_msg))
            await response.write_eof()
            return response

    async def _handle_summary_api(self, summary_fn: Callable[[SummaryApiOptions], SummaryApiResponse], req: aiohttp.web.Request):
    ...
    async def tasks_timeline(self, req: aiohttp.web.Request) -> aiohttp.web.Response:
        ...
        if download == '1':
            ...
            content_disposition = f'attachment; filename="timeline-{job_id}-{now_str}.json"'
    ...
    async def delayed_response(self, req: aiohttp.web.Request):
        ...
        return self._reply(success=True, error_message='', result={}, partial_failure_warning=None)
