...
from dagster import DagsterInstance, Definitions, _check as check, asset, define_asset_job, observable_source_asset, schedule, sensor
...
with environ({'MY_STRING': 'bar', 'MY_OTHER_STRING': 'foo'}):
    defs = Definitions(assets=[my_asset, my_observable_source_asset], resources={'foo': 'a_string', 'my_resource': MyResource(a_string='foo'), 'my_resource_env_vars': MyResource(a_string=EnvVar('MY_STRING')), 'my_resource_two_env_vars': MyResource(a_string=EnvVar('MY_STRING'), an_unset_string=EnvVar('MY_OTHER_STRING')), 'my_outer_resource': MyOuterResource(inner=MyInnerResource(a_str='wrapped'))}, jobs=[my_asset_job], sensors=[my_sensor, my_sensor_two], schedules=[my_schedule])
