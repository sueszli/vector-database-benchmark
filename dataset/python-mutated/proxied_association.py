...
if __name__ == '__main__':
    ...
    (tshirt, mug, hat, crowbar) = (Item('SA T-Shirt', 10.99), Item('SA Mug', 6.5), Item('SA Hat', 8.99), Item('MySQL Crowbar', 16.99))
    ...
    print([(assoc.item.description, assoc.price, assoc.item.price) for assoc in order.order_items])
    print([(item.description, item.price) for item in order.items])
    orders = session.query(Order).join('order_items', 'item').filter(Item.description == 'MySQL Crowbar').filter(Item.price > OrderItem.price)
