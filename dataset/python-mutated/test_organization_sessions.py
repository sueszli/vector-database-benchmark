...
RECEIVED = TIMESTAMP
SESSION_STARTED = TIMESTAMP // 3600 * 3600
TEMPLATE = {'distinct_id': '00000000-0000-0000-0000-000000000000', 'status': 'exited', 'seq': 0, 'release': 'foo@1.0.0', 'environment': 'production', 'retention_days': 90, 'duration': 123.4, 'errors': 0, 'started': SESSION_STARTED, 'received': RECEIVED}
...
class OrganizationSessionsEndpointTest(APITestCase, SnubaTestCase):
    ...
    expected_duration_values = {'avg(session.duration)': 42375.0, 'max(session.duration)': 80000.0, 'p50(session.duration)': 33500.0, 'p75(session.duration)': 53750.0, 'p90(session.duration)': 71600.0, 'p95(session.duration)': 75800.0, 'p99(session.duration)': 79159.99999999999}
...
class OrganizationSessionsEndpointMetricsTest(BaseMetricsTestCase, OrganizationSessionsEndpointTest):
