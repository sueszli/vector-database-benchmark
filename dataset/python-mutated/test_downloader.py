...
from redbot.cogs.downloader.errors import AmbiguousRevision, ExistingGitRepo, GitException, UnknownRevision
...
@pytest.mark.parametrize('maybe_ancestor_rev,descendant_rev,returncode,expected', [(ancestor_rev, descendant_rev, 0, True), (descendant_rev, ancestor_rev, 1, False)])
async def test_is_ancestor(mocker, repo, maybe_ancestor_rev, descendant_rev, returncode, expected):
    ...
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_IS_ANCESTOR, path=repo.folder_path, maybe_ancestor_rev=maybe_ancestor_rev, descendant_rev=descendant_rev), valid_exit_codes=(0, 1), debug_only=True)
...
async def test_is_ancestor_object_raise(mocker, repo):
    ...
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_IS_ANCESTOR, path=repo.folder_path, maybe_ancestor_rev='invalid1', descendant_rev='invalid2'), valid_exit_codes=(0, 1), debug_only=True)

async def test_is_ancestor_commit_raise(mocker, repo):
    m = _mock_run(mocker, repo, 128, b'', b'fatal: Not a valid commit name 0123456789abcde0123456789abcde0123456789')
    with pytest.raises(UnknownRevision):
        await repo.is_ancestor('0123456789abcde0123456789abcde0123456789', 'c950fc05a540dd76b944719c2a3302da2e2f3090')
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_IS_ANCESTOR, path=repo.folder_path, maybe_ancestor_rev='0123456789abcde0123456789abcde0123456789', descendant_rev='c950fc05a540dd76b944719c2a3302da2e2f3090'), valid_exit_codes=(0, 1), debug_only=True)
...
async def test_get_file_update_statuses(mocker, repo):
    ...
    m = _mock_run(mocker, repo, 0, b'A\x00added_file.txt\x00\tM\x00mycog/__init__.py\x00\tD\x00sample_file1.txt\x00\tD\x00sample_file2.txt\x00\tA\x00sample_file3.txt')
    ret = await repo._get_file_update_statuses(old_rev, new_rev)
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_DIFF_FILE_STATUS, path=repo.folder_path, old_rev=old_rev, new_rev=new_rev))
    assert ret == {'added_file.txt': 'A', 'mycog/__init__.py': 'M', 'sample_file1.txt': 'D', 'sample_file2.txt': 'D', 'sample_file3.txt': 'A'}
...
async def test_is_module_modified(mocker, repo):
    ...
    m = mocker.patch.object(repo, '_get_file_update_statuses', autospec=True, return_value={'added_file.txt': 'A', 'mycog/__init__.py': 'M', 'sample_file1.txt': 'D', 'sample_file2.txt': 'D', 'sample_file3.txt': 'A'})
...
async def test_get_full_sha1_success(mocker, repo):
    ...
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_GET_FULL_SHA1, path=repo.folder_path, rev=commit))
    assert ret == commit
...
async def test_get_full_sha1_notfound(mocker, repo):
    ...
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_GET_FULL_SHA1, path=repo.folder_path, rev='invalid'))

async def test_get_full_sha1_ambiguous(mocker, repo):
    m = _mock_run(mocker, repo, 128, b'', b'error: short SHA1 c6f0 is ambiguous\nhint: The candidates are:\nhint:   c6f028f tag ambiguous_tag_66387\nhint:   c6f0e5e commit 2019-10-24 - Commit ambiguous with tag.\nfatal: Needed a single revision')
    with pytest.raises(AmbiguousRevision) as exc_info:
        await repo.get_full_sha1('c6f0')
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_GET_FULL_SHA1, path=repo.folder_path, rev='c6f0'))
    assert exc_info.value.candidates == [Candidate('c6f028f', 'tag', 'ambiguous_tag_66387'), Candidate('c6f0e5e', 'commit', '2019-10-24 - Commit ambiguous with tag.')]
...
async def test_checkout(mocker, repo):
    ...
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_CHECKOUT, path=repo.folder_path, rev=commit))
...
async def test_clone_with_branch(mocker, repo):
    ...
    await repo.clone()
    assert repo.commit == commit
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_CLONE, branch=branch, url=repo.url, folder=repo.folder_path))
...
async def test_clone_without_branch(mocker, repo):
    ...
    await repo.clone()
    assert repo.commit == commit
    m.assert_called_once_with(ProcessFormatter().format(repo.GIT_CLONE_NO_BRANCH, url=repo.url, folder=repo.folder_path))
...
async def test_add_repo(monkeypatch, repo_manager):
    ...
    monkeypatch.setattr('redbot.cogs.downloader.repo_manager.Repo.current_commit', fake_current_commit)
    squid = await repo_manager.add_repo(url='https://github.com/tekulvw/Squid-Plugins', name='squid', branch='rewrite_cogs')
    assert squid.available_modules == ()

async def test_lib_install_requirements(monkeypatch, library_installable, repo, tmpdir):
    monkeypatch.setattr('redbot.cogs.downloader.repo_manager.Repo._run', fake_run_noprint)
    monkeypatch.setattr('redbot.cogs.downloader.repo_manager.Repo.available_libraries', (library_installable,))
    ...
    (installed, failed) = await repo.install_libraries(target_dir=sharedlib_path, req_target_dir=lib_path)
...
async def test_remove_repo(monkeypatch, repo_manager):
    monkeypatch.setattr('redbot.cogs.downloader.repo_manager.Repo._run', fake_run_noprint)
    monkeypatch.setattr('redbot.cogs.downloader.repo_manager.Repo.current_commit', fake_current_commit)
    await repo_manager.add_repo(url='https://github.com/tekulvw/Squid-Plugins', name='squid', branch='rewrite_cogs')
