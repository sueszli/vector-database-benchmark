...
class AysncServerViewTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_disjoint_groups(self):
        (dataset, first, second) = make_disjoint_groups_dataset()
        first_samples = await paginate_samples(dataset.name, [], {}, first=1, sample_filter=fosv.SampleFilter(group=fosv.GroupElementFilter(slice='first', id=first.group.id, slices=['first'])), pagination_data=True)
        self.assertEqual(len(first_samples.edges), 1)
        self.assertEqual(first_samples.edges[0].node.id, first._id)
        second_samples = await paginate_samples(dataset.name, [], {}, first=1, sample_filter=fosv.SampleFilter(group=fosv.GroupElementFilter(slice='second', id=second.group.id, slices=['second'])), pagination_data=True)
