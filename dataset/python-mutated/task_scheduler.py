...
class TaskScheduler:
    ...
    """
    SCHEDULE_INTERVAL_MS = 1 * 60 * 1000
    CLEANUP_INTERVAL_MS = 30 * 60 * 1000
    KEEP_TASKS_FOR_MS = 7 * 24 * 60 * 60 * 1000
    MAX_CONCURRENT_RUNNING_TASKS = 10
    LAST_UPDATE_BEFORE_WARNING_MS = 24 * 60 * 60 * 1000
    ...
    async def schedule_task(self, action: str, *, resource_id: Optional[str]=None, timestamp: Optional[int]=None, params: Optional[JsonMapping]=None) -> str:
        ...
        task = ScheduledTask(random_string(16), action, status, timestamp, resource_id, params, result=None, error=None)
    ...
    async def update_task(self, id: str, *, timestamp: Optional[int]=None, status: Optional[TaskStatus]=None, result: Optional[JsonMapping]=None, error: Optional[str]=None) -> bool:
        ...
        if status == TaskStatus.COMPLETE or status == TaskStatus.FAILED:
            raise Exception("update_task can't be called with a FAILED or COMPLETE status")
        if timestamp is None:
            timestamp = self._clock.time_msec()
        return await self._store.update_scheduled_task(id, timestamp, status=status, result=result, error=error)
    ...
    async def get_tasks(self, *, actions: Optional[List[str]]=None, resource_id: Optional[str]=None, statuses: Optional[List[TaskStatus]]=None, max_timestamp: Optional[int]=None, limit: Optional[int]=None) -> List[ScheduledTask]:
        ...
        """
        return await self._store.get_scheduled_tasks(actions=actions, resource_id=resource_id, statuses=statuses, max_timestamp=max_timestamp, limit=limit)
    ...
    async def _launch_scheduled_tasks(self) -> None:
        ...
        try:
            for task in await self.get_tasks(statuses=[TaskStatus.ACTIVE], limit=self.MAX_CONCURRENT_RUNNING_TASKS):
                await self._launch_task(task)
            for task in await self.get_tasks(statuses=[TaskStatus.SCHEDULED], max_timestamp=self._clock.time_msec(), limit=self.MAX_CONCURRENT_RUNNING_TASKS):
                await self._launch_task(task)
        finally:
            self._launching_new_tasks = False
    ...
    async def _clean_scheduled_tasks(self) -> None:
        ...
        for task in await self._store.get_scheduled_tasks(statuses=[TaskStatus.FAILED, TaskStatus.COMPLETE], max_timestamp=now - TaskScheduler.KEEP_TASKS_FOR_MS):
    ...
    async def _launch_task(self, task: ScheduledTask) -> None:
        ...
        if task.action not in self._actions:
            raise Exception(f'No function associated with action {task.action} of the scheduled task {task.id}')
        ...
        async def wrapper() -> None:
            with nested_logging_context(task.id):
                try:
                    ...
                    f = Failure()
                    logger.error(f'scheduled task {task.id} failed', exc_info=(f.type, f.value, f.getTracebackObject()))
                    status = TaskStatus.FAILED
                    result = None
                    error = f.getErrorMessage()
                await self._store.update_scheduled_task(task.id, self._clock.time_msec(), status=status, result=result, error=error)
                self._running_tasks.remove(task.id)
            self._clock.call_later(1, self._launch_scheduled_tasks)
        if len(self._running_tasks) >= TaskScheduler.MAX_CONCURRENT_RUNNING_TASKS:
            return
        if self._clock.time_msec() > task.timestamp + TaskScheduler.LAST_UPDATE_BEFORE_WARNING_MS:
            logger.warn(f'Task {task.id} (action {task.action}) has seen no update for more than 24h and may be stuck')
