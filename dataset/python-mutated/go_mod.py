...
specs: Dict[str, 'Parser[Optional[Tuple[str,...]]]'] = {'module': comment.result(None) | consume_line, 'go': comment.result(None) | consume_line, 'toolchain': comment.result(None) | consume_line, 'require': dep_spec, 'exclude': dep_spec, 'replace': comment.result(None) | consume_line, 'retract': comment.result(None) | consume_line}
directive = alt(*(make_directive(string(dir), mark_line(spec)) for (dir, spec) in specs.items()))
go_mod = (comment.optional() >> string('\n')).many() >> directive.sep_by((comment.optional() >> string('\n')).at_least(1)) << (comment.optional() >> string('\n')).many()
