...
class DaggerPipelineCommand(click.Command):
    ...
    async def invoke(self, ctx: click.Context) -> Any:
        ...
        main_logger.info("If you're running this command for the first time the Dagger engine image will be pulled, it can take a short minute...")
        ...
        try:
            ...
            if ctx.obj.get('dagger_logs_path'):
                ...
                if ctx.obj['is_ci']:
                    (gcs_uri, public_url) = upload_to_gcs(ctx.obj['dagger_logs_path'], ctx.obj['ci_report_bucket_name'], dagger_logs_gcs_key, ctx.obj['ci_gcs_credentials'])
