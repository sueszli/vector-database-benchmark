            ...
            if instance.state != 'terminated':
                ...
                if instance.user_data:
                    ...
                    if detect_secrets_output:
                        secrets_string = ', '.join([f"{secret['type']} on line {secret['line_number']}" for secret in detect_secrets_output[temp_user_data_file.name]])
                        report.status = 'FAIL'
                        report.status_extended = f'Potential secret found in EC2 instance {instance.id} User Data -> {secrets_string}.'
                    else:
                        report.status = 'PASS'
                        report.status_extended = f'No secrets found in EC2 instance {instance.id} User Data.'
