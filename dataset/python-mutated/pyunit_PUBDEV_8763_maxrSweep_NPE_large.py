...
def test_maxrsweep_NPE():
    ...
    npred = 100
    maxrsweep_model = H2OModelSelectionEstimator(mode='maxrsweep', max_predictor_number=npred, intercept=True, build_glm_model=True)
    ...
    for ind in range(0, len(maxrsweep_coeffs)):
        pyunit_utils.assertCoefDictEqual(maxrsweep_coeffs[ind], maxrsweepGLM_coeffs[ind], 1e-06)
        pyunit_utils.assertCoefDictEqual(maxrsweep_coeffs[ind], maxrsweep_coeffs_MM[ind], 1e-06)
        pyunit_utils.assertCoefDictEqual(maxrsweep_coeffs_norm[ind], maxrsweepGLM_coeffs_norm[ind], 1e-06)
        pyunit_utils.assertCoefDictEqual(maxrsweep_coeffs_norm[ind], maxrsweep_coeffs_norm_MM[ind], 1e-06)
        ...
        assert maxrsweep_best_model_predictors[ind] == maxrsweepGLM_best_model_predictors[ind], 'Expected predictor subset: {0}, actual predictor subset: {1}'.format(maxrsweep_best_model_predictors[ind], maxrsweepGLM_best_model_predictors[ind])
        assert maxrsweep_best_model_predictors[ind] == maxrsweep_best_model_predictors_MM[ind], 'Expected predictor subset: {0}, actual predictor subset: {1}'.format(maxrsweep_best_model_predictors[ind], maxrsweep_best_model_predictors_MM[ind])
