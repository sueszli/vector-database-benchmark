...
if TYPE_CHECKING:
    ...
    from strawberry.subscriptions.protocols.graphql_ws.types import ConnectionInitPayload, OperationMessage, OperationMessagePayload, StartPayload

class BaseGraphQLWSHandler(ABC):

    def __init__(self, schema: BaseSchema, debug: bool, keep_alive: bool, keep_alive_interval: float):
        if False:
            while True:
                i = 10
        self.schema = schema
    ...
    async def handle_start(self, message: OperationMessage) -> None:
        ...
        try:
            result_source = await self.schema.subscribe(query=query, variable_values=variables, operation_name=operation_name, context_value=context, root_value=root_value)
    ...
    async def handle_async_results(self, result_source: AsyncGenerator, operation_id: str) -> None:
        try:
            ...
            await self.send_message(GQL_DATA, operation_id, {'data': None, 'errors': [error.formatted]})
            self.schema.process_errors([error])
    ...
    async def send_message(self, type_: str, operation_id: str, payload: Optional[OperationMessagePayload]=None) -> None:
