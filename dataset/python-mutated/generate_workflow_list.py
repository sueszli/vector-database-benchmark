...
match_zip = re.compile('alfred-workflow-.*\\.zip').search
is_github_url = re.compile('^https?://github.com/.+$').match
parse_github_url = re.compile('https?://github.com/(.+?)/(.+)').match
...
MAX_CACHE_AGE = 7
logging.basicConfig(level=logging.DEBUG, format='%(filename)s:%(lineno)d %(levelname)-8s %(message)s', datefmt='%H:%M:%S')
...
class Cache(object):
    ...
    find_github_url = re.compile('field-github-url\n                                 .*?\n                                 href="(.+?)"\n                                 .*?\n                                 </div>', re.DOTALL | re.VERBOSE).search
