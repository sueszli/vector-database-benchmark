...
parser.add_argument('template_files', metavar='TEMPLATE_FILE', type=argparse.FileType('r'), nargs='+', help='template filename: <something>.template.ld')
...
def remove_UL(s):
    if False:
        i = 10
        return i + 15
    return REMOVE_UL_RE.sub('\\1', s)
VALUE_LINE_RE = re.compile('^/\\*\\s*(\\w+)\\s*=\\*/\\s*(.*);\\s*$')
