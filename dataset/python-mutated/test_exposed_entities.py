...
from homeassistant.components.homeassistant.exposed_entities import DATA_EXPOSED_ENTITIES, ExposedEntities, ExposedEntity, async_expose_entity, async_get_assistant_settings, async_get_entity_settings, async_listen_entity_updates, async_should_expose
...
async def test_expose_entity(hass: HomeAssistant, entity_registry: er.EntityRegistry, hass_ws_client: WebSocketGenerator) -> None:
    """Test expose entity."""
    ...
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_entity', 'assistants': ['cloud.alexa'], 'entity_ids': [entry1.entity_id], 'should_expose': True})
    ...
    assert len(exposed_entities.entities) == 0
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_entity', 'assistants': ['cloud.alexa', 'cloud.google_assistant'], 'entity_ids': [entry1.entity_id, entry2.entity_id], 'should_expose': False})
    response = await ws_client.receive_json()
    assert response['success']
    entry1 = entity_registry.async_get(entry1.entity_id)
    assert entry1.options == {'cloud.alexa': {'should_expose': False}, 'cloud.google_assistant': {'should_expose': False}}
    entry2 = entity_registry.async_get(entry2.entity_id)
    assert entry2.options == {'cloud.alexa': {'should_expose': False}, 'cloud.google_assistant': {'should_expose': False}}
    assert len(exposed_entities.entities) == 0

async def test_expose_entity_unknown(hass: HomeAssistant, hass_ws_client: WebSocketGenerator) -> None:
    ...
    exposed_entities: ExposedEntities = hass.data[DATA_EXPOSED_ENTITIES]
    assert len(exposed_entities.entities) == 0
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_entity', 'assistants': ['cloud.alexa'], 'entity_ids': ['test.test'], 'should_expose': True})
    response = await ws_client.receive_json()
    assert response['success']
    assert len(exposed_entities.entities) == 1
    assert exposed_entities.entities == {'test.test': ExposedEntity({'cloud.alexa': {'should_expose': True}})}
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_entity', 'assistants': ['cloud.alexa', 'cloud.google_assistant'], 'entity_ids': ['test.test', 'test.test2'], 'should_expose': False})
    response = await ws_client.receive_json()
    assert response['success']
    assert len(exposed_entities.entities) == 2
    assert exposed_entities.entities == {'test.test': ExposedEntity({'cloud.alexa': {'should_expose': False}, 'cloud.google_assistant': {'should_expose': False}}), 'test.test2': ExposedEntity({'cloud.alexa': {'should_expose': False}, 'cloud.google_assistant': {'should_expose': False}})}

async def test_expose_entity_blocked(hass: HomeAssistant, hass_ws_client: WebSocketGenerator) -> None:
    """Test behavior when exposing a blocked entity."""
    ws_client = await hass_ws_client(hass)
    assert await async_setup_component(hass, 'homeassistant', {})
    await hass.async_block_till_done()
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_entity', 'assistants': ['cloud.alexa'], 'entity_ids': ['group.all_locks'], 'should_expose': True})
    response = await ws_client.receive_json()
    assert not response['success']
    assert response['error'] == {'code': 'not_allowed', 'message': "can't expose 'group.all_locks'"}

@pytest.mark.parametrize('expose_new', [True, False])
async def test_expose_new_entities(hass: HomeAssistant, entity_registry: er.EntityRegistry, hass_ws_client: WebSocketGenerator, expose_new) -> None:
    """Test expose entity."""
    ws_client = await hass_ws_client(hass)
    assert await async_setup_component(hass, 'homeassistant', {})
    await hass.async_block_till_done()
    ...
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_new_entities/get', 'assistant': 'cloud.alexa'})
    response = await ws_client.receive_json()
    assert response['success']
    assert response['result'] == {'expose_new': False}
    assert async_should_expose(hass, 'cloud.alexa', entry1.entity_id) is False
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_new_entities/set', 'assistant': 'cloud.alexa', 'expose_new': expose_new})
    response = await ws_client.receive_json()
    assert response['success']
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_new_entities/get', 'assistant': 'cloud.alexa'})
    response = await ws_client.receive_json()
...
async def test_listen_updates(hass: HomeAssistant, entity_registry: er.EntityRegistry) -> None:
...
async def test_get_assistant_settings(hass: HomeAssistant, entity_registry: er.EntityRegistry, snapshot: SnapshotAssertion) -> None:
...
@pytest.mark.parametrize('entities', ['entities_unique_id', 'entities_no_unique_id'], indirect=True)
async def test_should_expose(hass: HomeAssistant, entity_registry: er.EntityRegistry, hass_ws_client: WebSocketGenerator, entities: dict[str, str]) -> None:
    """Test expose entity."""
    ws_client = await hass_ws_client(hass)
    assert await async_setup_component(hass, 'homeassistant', {})
    await hass.async_block_till_done()
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_new_entities/set', 'assistant': 'cloud.alexa', 'expose_new': True})
    response = await ws_client.receive_json()
    assert response['success']
    ...
    assert async_should_expose(hass, 'cloud.alexa', entities['temperature_sensor']) is True
    assert async_should_expose(hass, 'cloud.alexa', entities['temperature_sensor']) is True
    exposed_entities: ExposedEntities = hass.data[DATA_EXPOSED_ENTITIES]
    exposed_entities.async_set_expose_new_entities('cloud.no_default_expose', False)
    assert async_should_expose(hass, 'cloud.no_default_expose', entities['temperature_sensor']) is False

async def test_should_expose_hidden_categorized(hass: HomeAssistant, entity_registry: er.EntityRegistry, hass_ws_client: WebSocketGenerator) -> None:
    """Test expose entity."""
    ws_client = await hass_ws_client(hass)
    assert await async_setup_component(hass, 'homeassistant', {})
    await hass.async_block_till_done()
    ...
    response = await ws_client.receive_json()
    assert response['success']
    entity_registry.async_get_or_create('lock', 'test', 'unique2', hidden_by=er.RegistryEntryHider.USER)
    assert async_should_expose(hass, 'cloud.alexa', 'lock.test_unique2') is False
    entity_registry.async_get_or_create('lock', 'test', 'unique3', entity_category=EntityCategory.CONFIG)
    assert async_should_expose(hass, 'cloud.alexa', 'lock.test_unique3') is False

async def test_list_exposed_entities(hass: HomeAssistant, entity_registry: er.EntityRegistry, hass_ws_client: WebSocketGenerator) -> None:
    ...
    entry1 = entity_registry.async_get_or_create('test', 'test', 'unique1')
    entry2 = entity_registry.async_get_or_create('test', 'test', 'unique2')
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_entity', 'assistants': ['cloud.alexa', 'cloud.google_assistant'], 'entity_ids': [entry1.entity_id, entry2.entity_id], 'should_expose': True})
    response = await ws_client.receive_json()
    assert response['success']
    await ws_client.send_json_auto_id({'type': 'homeassistant/expose_entity', 'assistants': ['cloud.alexa', 'cloud.google_assistant'], 'entity_ids': ['test.test', 'test.test2'], 'should_expose': False})
    response = await ws_client.receive_json()
    ...
    response = await ws_client.receive_json()
    assert response['success']
    assert response['result'] == {'exposed_entities': {'test.test': {'cloud.alexa': False, 'cloud.google_assistant': False}, 'test.test2': {'cloud.alexa': False, 'cloud.google_assistant': False}, 'test.test_unique1': {'cloud.alexa': True, 'cloud.google_assistant': True}, 'test.test_unique2': {'cloud.alexa': True, 'cloud.google_assistant': True}}}

async def test_listeners(hass: HomeAssistant, entity_registry: er.EntityRegistry) -> None:
