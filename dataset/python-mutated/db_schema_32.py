...
class Events(Base):
    ...
    def from_event(event: Event) -> Events:
        ...
        return Events(event_type=event.event_type, event_data=None, origin_idx=EVENT_ORIGIN_TO_IDX.get(event.origin), time_fired=None, time_fired_ts=dt_util.utc_to_timestamp(event.time_fired), context_id=event.context.id, context_user_id=event.context.user_id, context_parent_id=event.context.parent_id)
    ...
    def to_native(self, validate_entity_id: bool=True) -> Event | None:
        ...
        try:
            return Event(self.event_type or '', json_loads(self.event_data) if self.event_data else {}, EventOrigin(self.origin) if self.origin else EVENT_ORIGIN_ORDER[self.origin_idx or 0], dt_util.utc_from_timestamp(self.time_fired_ts or 0), context=context)
...
class States(Base):
    ...
    __table_args__ = (Index(ENTITY_ID_LAST_UPDATED_TS_INDEX, 'entity_id', 'last_updated_ts'), Index(STATES_CONTEXT_ID_BIN_INDEX, 'context_id_bin', mysql_length=CONTEXT_ID_BIN_MAX_LENGTH, mariadb_length=CONTEXT_ID_BIN_MAX_LENGTH), {'mysql_default_charset': 'utf8mb4', 'mysql_collate': 'utf8mb4_unicode_ci'})
    ...
    def from_event(event: Event) -> States:
        ...
        dbstate = States(entity_id=entity_id, attributes=None, context_id=event.context.id, context_user_id=event.context.user_id, context_parent_id=event.context.parent_id, origin_idx=EVENT_ORIGIN_TO_IDX.get(event.origin), last_updated=None, last_changed=None)
        if state is None:
            dbstate.state = ''
