...
import sys
import tarfile
manifest = {'collection_info': {'namespace': 'suspicious', 'name': 'test', 'version': '1.0.0', 'dependencies': {}}, 'file_manifest_file': {'name': 'FILES.json', 'ftype': 'file', 'chksum_type': 'sha256', 'chksum_sha256': None, 'format': 1}, 'format': 1}
files = {'files': [{'name': '.', 'ftype': 'dir', 'chksum_type': None, 'chksum_sha256': None, 'format': 1}], 'format': 1}
...
def add_file(tar_file, filename, b_content, update_files=True):
    ...
    if update_files:
        ...
        files['files'].append({'name': filename, 'ftype': 'file', 'chksum_type': 'sha256', 'chksum_sha256': sha256.hexdigest(), 'format': 1})
