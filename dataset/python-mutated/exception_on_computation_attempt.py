...
def wait_failure(self, timeout=None):
    if False:
        for i in range(10):
            print('nop')
    return -1
with mock.patch('golem.docker.job.DockerJob.wait', wait_failure), mock.patch('golem.task.benchmarkmanager.BenchmarkManager.run_benchmark', run_benchmark_error_performance_0):
    main()