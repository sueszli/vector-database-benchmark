...
from deeplake.constants import PYTEST_S3_PROVIDER_BASE_ROOT, PYTEST_GCS_PROVIDER_BASE_ROOT, PYTEST_AZURE_PROVIDER_BASE_ROOT, S3_OPT, GCS_OPT, AZURE_OPT
from deeplake.tests.common import is_opt_true
import pytest
enabled_storages = pytest.mark.parametrize('storage', ['memory_storage', 'local_storage', 's3_storage', 'gcs_storage', 'azure_storage', 'gdrive_storage'], indirect=True)
enabled_persistent_storages = pytest.mark.parametrize('storage', ['local_storage', 's3_storage', 'gcs_storage', 'azure_storage', 'gdrive_storage'], indirect=True)
enabled_remote_storages = pytest.mark.parametrize('storage', ['s3_storage', 'gcs_storage', 'gdrive_storage', 'gcs_root_storage', 'azure_root_storage', 's3_root_storage'], indirect=True)
