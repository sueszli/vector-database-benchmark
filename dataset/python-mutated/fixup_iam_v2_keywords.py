...
class iamCallTransformer(cst.CSTTransformer):
    ...
    METHOD_TO_PARAMS: Dict[str, Tuple[str]] = {'create_policy': ('parent', 'policy', 'policy_id'), 'delete_policy': ('name', 'etag'), 'get_policy': ('name',), 'list_policies': ('parent', 'page_size', 'page_token'), 'update_policy': ('policy',)}
...
def fix_files(in_dir: pathlib.Path, out_dir: pathlib.Path, *, transformer=iamCallTransformer()):
    if False:
        i = 10
        return i + 15
