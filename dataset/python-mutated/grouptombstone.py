...
from sentry.db.models import BoundedBigIntegerField, BoundedPositiveIntegerField, FlexibleForeignKey, GzippedDictField, Model, region_silo_only_model
...
class GroupTombstone(Model):
    ...
    level = BoundedPositiveIntegerField(choices=[(key, str(val)) for (key, val) in sorted(LOG_LEVELS.items())], default=logging.ERROR, blank=True)
