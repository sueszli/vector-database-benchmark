...
async def test_get_torrentinfo(tmp_path, rest_api, download_manager: DownloadManager):
    ...
    path = quote_plus(f'magnet:?xt=urn:btih:{hexlify(UBUNTU_1504_INFOHASH)}&dn=test torrent&tr=http://ubuntu.org/ann')
    hops_list = []
    ...
    path = 'magnet:?xt=urn:ed2k:354B15E68FB8F36D7CD88FF94116CDC1'
    ...
    await do_request(rest_api, f'torrentinfo?uri={path}', expected_code=500)
    mock_download = MagicMock(stop=AsyncMock(), shutdown=AsyncMock())
...
async def test_on_got_invalid_metainfo(rest_api):
    ...
    assert 'error' in res
caught_exceptions = [ServerConnectionError(), ClientResponseError(Mock(), Mock()), SSLError(), ClientConnectorError(Mock(), Mock()), AsyncTimeoutError()]
