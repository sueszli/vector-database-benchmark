...
class ImageColorizationTrainerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        dataset_val = MsDataset.load('imagenet-val5k-image', namespace='damo', subset_name='default', split='validation', download_mode=DownloadMode.REUSE_DATASET_IF_EXISTS)._hf_ds
        self.dataset_train = ImageColorizationDataset(dataset_train, self.config.dataset, is_train=True)
        self.dataset_val = ImageColorizationDataset(dataset_val, self.config.dataset, is_train=False)
    ...
    def test_trainer(self):
        ...
        pipeline_colorization = pipeline(task=Tasks.image_colorization, model=f'{self.tmp_dir}/output')
