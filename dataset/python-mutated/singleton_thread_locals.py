...
class Container(containers.DeclarativeContainer):
    ...
    put_in_queue = providers.Callable(put_in_queue, example_object=thread_local_object, queue_object=queue_provider)
    thread_factory = providers.Factory(threading.Thread, target=put_in_queue.provider)
if __name__ == '__main__':
    container = Container()
    n = 10
    threads = []
    for thread_number in range(n):
        threads.append(container.thread_factory(name='Thread{0}'.format(thread_number)))
