...
class ClapAudioModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_hidden_states_output(self):
        ...
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            self.assertListEqual(list(hidden_states[0].shape[-2:]), [2 * self.model_tester.patch_embeds_hidden_size, 2 * self.model_tester.patch_embeds_hidden_size])
...
class ClapTextModelTester:
    ...
    def get_config(self):
        if False:
            print('Hello World!')
        return ClapTextConfig(vocab_size=self.vocab_size, hidden_size=self.hidden_size, projection_dim=self.projection_dim, num_hidden_layers=self.num_hidden_layers, num_attention_heads=self.num_attention_heads, intermediate_size=self.intermediate_size, dropout=self.dropout, attention_dropout=self.attention_dropout, max_position_embeddings=self.max_position_embeddings, initializer_range=self.initializer_range, projection_hidden_act=self.projection_hidden_act)
...
class ClapModelTester:
    ...
    def get_config(self):
        if False:
            for i in range(10):
                print('nop')
        return ClapConfig.from_text_audio_configs(self.text_model_tester.get_config(), self.audio_model_tester.get_config(), projection_dim=64)
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        inputs_dict = {'input_ids': input_ids, 'attention_mask': attention_mask, 'input_features': input_features, 'return_loss': True}
        return (config, inputs_dict)
...
class ClapModelIntegrationTest(unittest.TestCase):
    ...
    def test_integration_unfused(self):
        if False:
            for i in range(10):
                print('nop')
        EXPECTED_MEANS_UNFUSED = {'repeatpad': 0.0024, 'pad': 0.002, 'repeat': 0.0023}
        ...
        for padding in self.paddings:
            inputs = processor(audios=audio_sample['audio']['array'], return_tensors='pt', padding=padding).to(torch_device)
            audio_embed = model.get_audio_features(**inputs)
            expected_mean = EXPECTED_MEANS_UNFUSED[padding]
            self.assertTrue(torch.allclose(audio_embed.cpu().mean(), torch.tensor([expected_mean]), atol=0.001, rtol=0.001))

    def test_integration_fused(self):
        if False:
            print('Hello World!')
        EXPECTED_MEANS_FUSED = {'repeatpad': 0.00069, 'repeat': 0.00196, 'pad': -0.000379}
        librispeech_dummy = load_dataset('hf-internal-testing/librispeech_asr_dummy', 'clean', split='validation')
        audio_sample = librispeech_dummy[-1]
        ...
        processor = ClapProcessor.from_pretrained(model_id)
        for padding in self.paddings:
            inputs = processor(audios=audio_sample['audio']['array'], return_tensors='pt', padding=padding, truncation='fusion').to(torch_device)
            audio_embed = model.get_audio_features(**inputs)
    ...
    def test_batched_fused(self):
        if False:
            for i in range(10):
                print('nop')
        EXPECTED_MEANS_FUSED = {'repeatpad': 0.001, 'repeat': 0.002, 'pad': 0.0006}
        librispeech_dummy = load_dataset('hf-internal-testing/librispeech_asr_dummy', 'clean', split='validation')
        ...
        processor = ClapProcessor.from_pretrained(model_id)
        for padding in self.paddings:
            inputs = processor(audios=audio_samples, return_tensors='pt', padding=padding, truncation='fusion').to(torch_device)
            audio_embed = model.get_audio_features(**inputs)
            expected_mean = EXPECTED_MEANS_FUSED[padding]
            self.assertTrue(torch.allclose(audio_embed.cpu().mean(), torch.tensor([expected_mean]), atol=0.001, rtol=0.001))
    ...
    def test_batched_unfused(self):
        if False:
            return 10
        EXPECTED_MEANS_FUSED = {'repeatpad': 0.0016, 'repeat': 0.0019, 'pad': 0.0019}
        librispeech_dummy = load_dataset('hf-internal-testing/librispeech_asr_dummy', 'clean', split='validation')
        audio_samples = [sample['array'] for sample in librispeech_dummy[0:4]['audio']]
        model_id = 'laion/clap-htsat-unfused'
        model = ClapModel.from_pretrained(model_id).to(torch_device)
