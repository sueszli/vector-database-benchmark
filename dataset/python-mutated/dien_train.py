...
if __name__ == '__main__':
    ...
    parser.add_argument('--executor_cores', type=int, default=2, help='The executor core number.')
    ...
    parser.add_argument('--model_type', default='DIN-V2-gru-vec-attGru', type=str, help='model type: DIEN (default)')
    ...
    if args.cluster_mode == 'local':
        ...
        init_orca_context('standalone', master=args.master, cores=args.executor_cores, num_nodes=args.num_executors, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, init_ray_on_spark=False)
    elif args.cluster_mode == 'yarn':
        init_orca_context('yarn-client', cores=args.executor_cores, num_nodes=args.num_executors, memory=args.executor_memory, driver_cores=args.driver_cores, driver_memory=args.driver_memory, init_ray_on_spark=False)
    ...
    estimator = Estimator.from_graph(inputs=inputs, outputs=[model.y_hat], labels=[model.target_ph], loss=model.loss, optimizer=model.optim, model_dir=args.model_dir, metrics={'loss': model.loss, 'accuracy': model.accuracy})
    estimator.fit(train_data.df, epochs=args.epochs, batch_size=args.batch_size, feature_cols=feature_cols, label_cols=['label'], validation_data=test_data.df)
    ...
    result = estimator.evaluate(test_data.df, args.batch_size, feature_cols=feature_cols, label_cols=['label'])
    evaluator = BinaryClassificationEvaluator(rawPredictionCol='prediction', labelCol='label_t', metricName='areaUnderROC')
