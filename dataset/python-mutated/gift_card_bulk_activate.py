...
class GiftCardBulkActivate(BaseBulkMutation):
    ...
    class Arguments:
        ids = NonNullList(graphene.ID, required=True, description='List of gift card IDs to activate.')
    ...
    class Meta:
        ...
        webhook_events_info = [WebhookEventInfo(type=WebhookEventAsyncType.GIFT_CARD_STATUS_CHANGED, description='A gift card was activated.')]
    ...
    def clean_instance(cls, _info: ResolveInfo, instance):
        ...
        if is_gift_card_expired(instance):
            raise ValidationError('Cannot activate expired card.', code=GiftCardErrorCode.EXPIRED_GIFT_CARD.value)
    ...
    def bulk_action(cls, info: ResolveInfo, queryset, /):
        ...
        events.gift_cards_activated_event(gift_card_ids, user=info.context.user, app=app)
