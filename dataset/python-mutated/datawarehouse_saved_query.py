...
class DataWarehouseSavedQuery(CreatedMetaFields, UUIDModel, DeletedMetaFields):
    ...
    columns: models.JSONField = models.JSONField(default=dict, null=True, blank=True, help_text='Dict of all columns with ClickHouse type (including Nullable())')
    external_tables: models.JSONField = models.JSONField(default=list, null=True, blank=True, help_text='List of all external tables')
    query: models.JSONField = models.JSONField(default=dict, null=True, blank=True, help_text='HogQL query')

    class Meta:
        constraints = [models.UniqueConstraint(fields=['team', 'name'], name='posthog_datawarehouse_saved_query_unique_name')]
