...
from scapy.fields import ByteEnumField, ByteField, FieldLenField, FieldListField, FlagsField, IPField, IntEnumField, IntField, MultipleTypeField, PacketLenField, ShortEnumField, ShortField, StrLenEnumField, StrLenField, XByteField, XStrFixedLenField, XStrLenField
...
IPSECAttributeTypes = {'LifeType': (1, {'Reserved': 0, 'seconds': 1, 'kilobytes': 2}, 0), 'LifeDuration': (2, {}, 1), 'GroupDesc': (3, ISAKMPAttributeTypes['GroupDesc'][1], 0), 'EncapsulationMode': (4, {'Reserved': 0, 'Tunnel': 1, 'Transport': 2, 'UDP-Encapsulated-Tunnel': 3, 'UDP-Encapsulated-Transport': 4}, 0), 'AuthenticationAlgorithm': (5, {'HMAC-MD5': 1, 'HMAC-SHA': 2, 'DES-MAC': 3, 'KPDK': 4, 'HMAC-SHA2-256': 5, 'HMAC-SHA2-384': 6, 'HMAC-SHA2-512': 7, 'HMAC-RIPEMD': 8, 'AES-XCBC-MAC': 9, 'SIG-RSA': 10, 'AES-128-GMAC': 11, 'AES-192-GMAC': 12, 'AES-256-GMAC': 13}, 0), 'KeyLength': (6, {}, 0), 'KeyRounds': (7, {}, 0), 'CompressDictionarySize': (8, {}, 0), 'CompressPrivateAlgorithm': (9, {}, 1)}
_rev = lambda x: {v[0]: (k, {vv: kk for (kk, vv) in v[1].items()}, v[2]) for (k, v) in x.items()}
...
class ISAKMPTransformSetField(StrLenField):
    ...
    def m2i(self, pkt, m):
        ...
        return lst
ISAKMP_payload_type = {0: 'None', 1: 'SA', 2: 'Proposal', 3: 'Transform', 4: 'KE', 5: 'ID', 6: 'CERT', 7: 'CR', 8: 'Hash', 9: 'SIG', 10: 'Nonce', 11: 'Notification', 12: 'Delete', 13: 'VendorID'}
ISAKMP_exchange_type = {0: 'None', 1: 'base', 2: 'identity protection', 3: 'authentication only', 4: 'aggressive', 5: 'informational', 32: 'quick mode'}
ISAKMP_protos = {1: 'ISAKMP', 2: 'IPSEC_AH', 3: 'IPSEC_ESP', 4: 'IPCOMP', 5: 'GIGABEAM_RADIO'}
ISAKMP_doi = {0: 'ISAKMP', 1: 'IPSEC'}
...
class ISAKMP(_ISAKMP_class):
    name = 'ISAKMP'
    fields_desc = [XStrFixedLenField('init_cookie', '', 8), XStrFixedLenField('resp_cookie', '', 8), ByteEnumField('next_payload', 0, ISAKMP_payload_type), XByteField('version', 16), ByteEnumField('exch_type', 0, ISAKMP_exchange_type), FlagsField('flags', 0, 8, ['encryption', 'commit', 'auth_only']), IntField('id', 0), IntField('length', None)]
...
class ISAKMP_payload(_ISAKMP_class):
    name = 'ISAKMP payload'
    show_indent = 0
    fields_desc = [ByteEnumField('next_payload', None, ISAKMP_payload_type), ByteField('res', 0), ShortField('length', None), XStrLenField('load', '', length_from=lambda x: x.length - 4)]
...
class ISAKMP_payload_Transform(ISAKMP_payload):
    name = 'IKE Transform'
    deprecated_fields = {'num': ('transform_count', '2.5.0'), 'id': ('transform_id', '2.5.0')}
    fields_desc = ISAKMP_payload.fields_desc[:3] + [ByteField('transform_count', None), ByteEnumField('transform_id', 1, {1: 'KEY_IKE'}), ShortField('res2', 0), ISAKMPTransformSetField('transforms', None, length_from=lambda x: x.length - 8)]

class ISAKMP_payload_Proposal(ISAKMP_payload):
    name = 'IKE proposal'
    fields_desc = ISAKMP_payload.fields_desc[:3] + [ByteField('proposal', 1), ByteEnumField('proto', 1, ISAKMP_protos), FieldLenField('SPIsize', None, 'SPI', 'B'), ByteField('trans_nb', None), StrLenField('SPI', '', length_from=lambda x: x.SPIsize), PacketLenField('trans', conf.raw_layer(), ISAKMP_payload_Transform, length_from=lambda x: x.length - 8)]
ISAKMP_VENDOR_IDS = {b'\t\x00&\x89\xdf\xd6\xb7\x12': 'XAUTH', b'\xaf\xca\xd7\x13h\xa1\xf1\xc9k\x86\x96\xfcwW\x01\x00': 'RFC 3706 DPD', b'@H\xb7\xd5n\xbc\xe8\x85%\xe7\xde\x7f\x00\xd6\xc2\xd3\x80': 'Cisco Fragmentation', b'J\x13\x1c\x81\x07\x03XE\\W(\xf2\x0e\x95E/': 'RFC 3947 Negotiation of NAT-Transversal', b'\x90\xcb\x80\x91>\xbbin\x08c\x81\xb5\xecB{\x1f': 'draft-ietf-ipsec-nat-t-ike-02'}

class ISAKMP_payload_VendorID(ISAKMP_payload):
    name = 'ISAKMP Vendor ID'
    fields_desc = ISAKMP_payload.fields_desc[:3] + [StrLenEnumField('VendorID', b'', ISAKMP_VENDOR_IDS, length_from=lambda x: x.length - 4)]

class ISAKMP_payload_SA(ISAKMP_payload):
    name = 'ISAKMP SA'
    fields_desc = ISAKMP_payload.fields_desc[:3] + [IntEnumField('doi', 1, ISAKMP_doi), IntEnumField('situation', 1, {1: 'identity'}), PacketLenField('prop', conf.raw_layer(), ISAKMP_payload_Proposal, length_from=lambda x: x.length - 12)]
...
class ISAKMP_payload_ID(ISAKMP_payload):
    ...
    fields_desc = ISAKMP_payload.fields_desc[:3] + [ByteEnumField('IDtype', 1, {1: 'IPv4_addr', 11: 'Key'}), ByteEnumField('ProtoID', 0, {0: 'Unused'}), ShortEnumField('Port', 0, {0: 'Unused'}), MultipleTypeField([(IPField('IdentData', '127.0.0.1'), lambda pkt: pkt.IDtype == 1)], StrLenField('IdentData', '', length_from=lambda x: x.length - 8))]

class ISAKMP_payload_Hash(ISAKMP_payload):
    name = 'ISAKMP Hash'
NotifyMessageType = {1: 'INVALID-PAYLOAD-TYPE', 2: 'DOI-NOT-SUPPORTED', 3: 'SITUATION-NOT-SUPPORTED', 4: 'INVALID-COOKIE', 5: 'INVALID-MAJOR-VERSION', 6: 'INVALID-MINOR-VERSION', 7: 'INVALID-EXCHANGE-TYPE', 8: 'INVALID-FLAGS', 9: 'INVALID-MESSAGE-ID', 10: 'INVALID-PROTOCOL-ID', 11: 'INVALID-SPI', 12: 'INVALID-TRANSFORM-ID', 13: 'ATTRIBUTES-NOT-SUPPORTED', 14: 'NO-PROPOSAL-CHOSEN', 15: 'BAD-PROPOSAL-SYNTAX', 16: 'PAYLOAD-MALFORMED', 17: 'INVALID-KEY-INFORMATION', 18: 'INVALID-ID-INFORMATION', 19: 'INVALID-CERT-ENCODING', 20: 'INVALID-CERTIFICATE', 21: 'CERT-TYPE-UNSUPPORTED', 22: 'INVALID-CERT-AUTHORITY', 23: 'INVALID-HASH-INFORMATION', 24: 'AUTHENTICATION-FAILED', 25: 'INVALID-SIGNATURE', 26: 'ADDRESS-NOTIFICATION', 27: 'NOTIFY-SA-LIFETIME', 28: 'CERTIFICATE-UNAVAILABLE', 29: 'UNSUPPORTED-EXCHANGE-TYPE', 36136: 'R-U-THERE', 36137: 'R-U-THERE-ACK'}

class ISAKMP_payload_Notify(ISAKMP_payload):
    name = 'ISAKMP Notify (Notification)'
    fields_desc = ISAKMP_payload.fields_desc[:3] + [IntEnumField('doi', 0, ISAKMP_doi), ByteEnumField('proto', 1, ISAKMP_protos), FieldLenField('SPIsize', None, 'SPI', 'B'), ShortEnumField('notify_msg_type', None, NotifyMessageType), StrLenField('SPI', '', length_from=lambda x: x.SPIsize), StrLenField('notify_data', '', length_from=lambda x: x.length - x.SPIsize - 12)]
