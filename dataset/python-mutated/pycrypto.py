...
def secure_password(length=20, use_random=True, chars=None, lowercase=True, uppercase=True, digits=True, punctuation=True, whitespace=False, printable=False):
    if False:
        for i in range(10):
            print('nop')
    ...
    try:
        ...
        while len(pw) < length:
            if HAS_RANDOM and use_random:
                ...
                while True:
                    try:
                        char = salt.utils.stringutils.to_str(get_random_bytes(1), encoding=encoding)
                        break
                    except UnicodeDecodeError:
                        continue
                pw += re.sub(salt.utils.stringutils.to_str('[^{}]'.format(re.escape(chars)), encoding=encoding), '', char)
            else:
...
def gen_hash(crypt_salt=None, password=None, algorithm=None):
    ...
    if HAS_CRYPT and algorithm in methods:
        return _gen_hash_crypt(crypt_salt=crypt_salt, password=password, algorithm=algorithm)
    elif HAS_PASSLIB and algorithm in known_methods:
        return _gen_hash_passlib(crypt_salt=crypt_salt, password=password, algorithm=algorithm)
    else:
        raise SaltInvocationError("Cannot hash using '{}' hash algorithm. Natively supported algorithms are: {}. If passlib is installed ({}), the supported algorithms are: {}.".format(algorithm, list(methods), HAS_PASSLIB, known_methods))