...
class AssertAction(ActionSequence):
    ...
    def __init__(self, test_condition):
        if False:
            print('Hello World!')
        super().__init__()
        self.add_action(ConditionalAction(condition=test_condition, else_action=ActionSequence(actions=[ScreenshotAction(), CustomAction(f"raise AssertionError('Assertion fails: %s' % {test_condition!r})")])))