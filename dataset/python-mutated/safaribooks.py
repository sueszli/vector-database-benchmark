...
class Display:
    ...
    SH_DEFAULT = '\x1b[0m' if 'win' not in sys.platform else ''
    SH_YELLOW = '\x1b[33m' if 'win' not in sys.platform else ''
    SH_BG_RED = '\x1b[41m' if 'win' not in sys.platform else ''
    SH_BG_YELLOW = '\x1b[43m' if 'win' not in sys.platform else ''
...
class SafariBooks:
    ...
    BASE_01_HTML = '<!DOCTYPE html>\n<html lang="en" xml:lang="en" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.w3.org/2002/06/xhtml2/ http://www.w3.org/MarkUp/SCHEMA/xhtml2.xsd" xmlns:epub="http://www.idpf.org/2007/ops">\n<head>\n{0}\n<style type="text/css">body{{margin:1em;background-color:transparent!important;}}#sbo-rt-content *{{text-indent:0pt!important;}}#sbo-rt-content .bq{{margin-right:1em!important;}}'
    KINDLE_HTML = '#sbo-rt-content *{{word-wrap:break-word!important;word-break:break-word!important;}}#sbo-rt-content table,#sbo-rt-content pre{{overflow-x:unset!important;overflow:unset!important;overflow-y:unset!important;white-space:pre-wrap!important;}}'
    ...
    HEADERS = {'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8', 'Accept-Encoding': 'gzip, deflate', 'Referer': LOGIN_ENTRY_URL, 'Upgrade-Insecure-Requests': '1', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36'}
    COOKIE_FLOAT_MAX_AGE_PATTERN = re.compile('(max-age=\\d*\\.\\d*)', re.IGNORECASE)
...
if __name__ == '__main__':
    arguments = argparse.ArgumentParser(prog='safaribooks.py', description='Download and generate an EPUB of your favorite books from Safari Books Online.', add_help=False, allow_abbrev=False)
    login_arg_group = arguments.add_mutually_exclusive_group()
    login_arg_group.add_argument('--cred', metavar='<EMAIL:PASS>', default=False, help='Credentials used to perform the auth login on Safari Books Online. Es. ` --cred "account_mail@mail.com:password01" `.')
    ...
    arguments.add_argument('--kindle', dest='kindle', action='store_true', help="Add some CSS rules that block overflow on `table` and `pre` elements. Use this option if you're going to export the EPUB to E-Readers like Amazon Kindle.")
    arguments.add_argument('--preserve-log', dest='log', action='store_true', help="Leave the `info_XXXXXXXXXXXXX.log` file even if there isn't any error.")
    arguments.add_argument('--help', action='help', default=argparse.SUPPRESS, help='Show this help message.')
    arguments.add_argument('bookid', metavar='<BOOK ID>', help='Book digits ID that you want to download. You can find it in the URL (X-es): `' + SAFARI_BASE_URL + '/library/view/book-name/XXXXXXXXXXXXX/`')
