...
from ..const import ATTR_COMMAND_CLASS, ATTR_COMMAND_CLASS_NAME, ATTR_CURRENT_VALUE, ATTR_CURRENT_VALUE_RAW, ATTR_ENDPOINT, ATTR_NODE_ID, ATTR_PREVIOUS_VALUE, ATTR_PREVIOUS_VALUE_RAW, ATTR_PROPERTY, ATTR_PROPERTY_KEY, ATTR_PROPERTY_KEY_NAME, ATTR_PROPERTY_NAME, DOMAIN
...
async def async_validate_trigger_config(hass: HomeAssistant, config: ConfigType) -> ConfigType:
    ...
    if not async_get_nodes_from_targets(hass, config):
        raise vol.Invalid(f'No nodes found for given {ATTR_DEVICE_ID}s or {ATTR_ENTITY_ID}s.')
    return config

async def async_attach_trigger(hass: HomeAssistant, config: ConfigType, action: TriggerActionType, trigger_info: TriggerInfo, *, platform_type: str=PLATFORM_TYPE) -> CALLBACK_TYPE:
    """Listen for state changes based on configuration."""
    dev_reg = dr.async_get(hass)
    if not async_get_nodes_from_targets(hass, config, dev_reg=dev_reg):
        raise ValueError(f'No nodes found for given {ATTR_DEVICE_ID}s or {ATTR_ENTITY_ID}s.')
