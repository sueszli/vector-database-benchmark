...
def __virtual__():
    ...
    if not HAS_ELEMENT_TREE:
        return (False, 'Cannot load {} module: ElementTree library unavailable'.format(__virtualname__))
    else:
        return True
...
def get_latest_snapshot(nexus_url, repository, group_id, artifact_id, packaging, target_dir='/tmp', target_file=None, classifier=None, username=None, password=None):
    ...
    log.debug('======================== MODULE FUNCTION: nexus.get_latest_snapshot, nexus_url=%s, repository=%s, group_id=%s, artifact_id=%s, packaging=%s, target_dir=%s, classifier=%s)', nexus_url, repository, group_id, artifact_id, packaging, target_dir, classifier)
    headers = {}
    if username and password:
        headers['Authorization'] = 'Basic {}'.format(base64.encodestring('{}:{}'.format(username, password)).replace('\n', ''))
...
def get_snapshot_version_string(nexus_url, repository, group_id, artifact_id, packaging, version, classifier=None, username=None, password=None):
    ...
    log.debug('======================== MODULE FUNCTION: nexus.get_snapshot_version_string(nexus_url=%s, repository=%s, group_id=%s, artifact_id=%s, packaging=%s, version=%s, classifier=%s)', nexus_url, repository, group_id, artifact_id, packaging, version, classifier)
    headers = {}
    if username and password:
        headers['Authorization'] = 'Basic {}'.format(base64.encodestring('{}:{}'.format(username, password)).replace('\n', ''))
...
def get_latest_release(nexus_url, repository, group_id, artifact_id, packaging, target_dir='/tmp', target_file=None, classifier=None, username=None, password=None):
    ...
    log.debug('======================== MODULE FUNCTION: nexus.get_latest_release(nexus_url=%s, repository=%s, group_id=%s, artifact_id=%s, packaging=%s, target_dir=%s, classifier=%s)', nexus_url, repository, group_id, artifact_id, packaging, target_dir, classifier)
    headers = {}
    if username and password:
        headers['Authorization'] = 'Basic {}'.format(base64.encodestring('{}:{}'.format(username, password)).replace('\n', ''))
    artifact_metadata = _get_artifact_metadata(nexus_url=nexus_url, repository=repository, group_id=group_id, artifact_id=artifact_id, headers=headers)
...
def _get_snapshot_url(nexus_url, repository, group_id, artifact_id, version, packaging, snapshot_version=None, classifier=None, headers=None, just_get_version_string=None):
    ...
    if snapshot_version is None:
        ...
        if packaging not in snapshot_version_metadata['snapshot_versions']:
            error_message = "Cannot find requested packaging '{packaging}' in the snapshot version metadata.\n                      nexus_url: {nexus_url}\n                      repository: {repository}\n                      group_id: {group_id}\n                      artifact_id: {artifact_id}\n                      packaging: {packaging}\n                      classifier: {classifier}\n                      version: {version}".format(nexus_url=nexus_url, repository=repository, group_id=group_id, artifact_id=artifact_id, packaging=packaging, classifier=classifier, version=version)
    ...
    snapshot_url = '{nexus_url}/{repository}/{group_url}/{artifact_id}/{version}/{file_name}'.format(nexus_url=nexus_url, repository=repository, group_url=group_url, artifact_id=artifact_id, version=version, file_name=file_name)
...
def _get_release_url(repository, group_id, artifact_id, packaging, version, nexus_url, classifier=None):
    ...
    release_url = '{nexus_url}/{repository}/{group_url}/{artifact_id}/{version}/{file_name}'.format(nexus_url=nexus_url, repository=repository, group_url=group_url, artifact_id=artifact_id, version=version, file_name=file_name)
...
def _get_artifact_metadata_url(nexus_url, repository, group_id, artifact_id):
    if False:
        for i in range(10):
            print('nop')
    group_url = __get_group_id_subpath(group_id)
    artifact_metadata_url = '{nexus_url}/{repository}/{group_url}/{artifact_id}/maven-metadata.xml'.format(nexus_url=nexus_url, repository=repository, group_url=group_url, artifact_id=artifact_id)
...
def _get_artifact_metadata_xml(nexus_url, repository, group_id, artifact_id, headers):
    if False:
        print('Hello World!')
    artifact_metadata_url = _get_artifact_metadata_url(nexus_url=nexus_url, repository=repository, group_id=group_id, artifact_id=artifact_id)
    try:
...
def _get_snapshot_version_metadata_url(nexus_url, repository, group_id, artifact_id, version):
    if False:
        return 10
    group_url = __get_group_id_subpath(group_id)
    snapshot_version_metadata_url = '{nexus_url}/{repository}/{group_url}/{artifact_id}/{version}/maven-metadata.xml'.format(nexus_url=nexus_url, repository=repository, group_url=group_url, artifact_id=artifact_id, version=version)
...
def _get_snapshot_version_metadata_xml(nexus_url, repository, group_id, artifact_id, version, headers):
    if False:
        while True:
            i = 10
    snapshot_version_metadata_url = _get_snapshot_version_metadata_url(nexus_url=nexus_url, repository=repository, group_id=group_id, artifact_id=artifact_id, version=version)
    try:
    ...
    if os.path.isfile(target_file):
        ...
        if checksum_success:
            ...
            if artifact_sum == file_sum:
                ...
                result['comment'] = 'File {} already exists, checksum matches with nexus.\nChecksum URL: {}'.format(target_file, checksum_url)
                return result
            else:
                result['comment'] = 'File {} already exists, checksum does not match with nexus!\nChecksum URL: {}'.format(target_file, checksum_url)
        else:
            result['status'] = False
