...
from paddle.distributed.auto_parallel.static.dist_context import DistributedContext, set_default_distributed_context
from paddle.distributed.auto_parallel.static.planner_v2 import Planner
from paddle.distributed.auto_parallel.static.tuner.parallel_tuner import ParallelTuner
...
hidden_size = 1024
sequence_len = 512
_g_process_mesh = [ProcessMesh([0, 1], dim_names=['x']), ProcessMesh([2, 3], dim_names=['x'])]
...
def get_program_v3():
    ...
    modeling.DPMPPP_MESH_LIST = [ProcessMesh([[0, 1], [2, 3]], dim_names=['x', 'y']), ProcessMesh([[4, 5], [6, 7]], dim_names=['x', 'y'])]
...
class TestParallelTunerFull(unittest.TestCase):
    ...
    def test_tune_with_planner(self):
        ...
        dist_context = DistributedContext(train_program, start_program, optimizer, loss, feed_vars, fetch_vars, cluster, strategy)
        dist_context.initialize()
        planner = Planner('train', dist_context)
        planner._parallel_tuner = ParallelTuner(planner._dist_context, mode=planner._mode, max_trials=3)
        planner.plan()
        flag = True
