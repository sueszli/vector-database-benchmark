...
class Wav2Vec2AsrConfig(FairseqDataclass):
    w2v_path: str = field(default=MISSING, metadata={'help': 'path to wav2vec 2.0 model'})
    no_pretrained_weights: bool = field(default=False, metadata={'help': 'if true, does not load pretrained weights'})
    dropout_input: float = field(default=0.0, metadata={'help': 'dropout to apply to the input (after feat extr)'})
    final_dropout: float = field(default=0.0, metadata={'help': 'dropout after transformer and before final projection'})
    dropout: float = field(default=0.0, metadata={'help': 'dropout probability inside wav2vec 2.0 model'})
    attention_dropout: float = field(default=0.0, metadata={'help': 'dropout probability for attention weights inside wav2vec 2.0 model'})
    activation_dropout: float = field(default=0.0, metadata={'help': 'dropout probability after activation in FFN inside wav2vec 2.0 model'})
    apply_mask: bool = field(default=False, metadata={'help': 'apply masking during fine-tuning'})
    mask_length: int = field(default=10, metadata={'help': 'repeat the mask indices multiple times'})
    mask_prob: float = field(default=0.5, metadata={'help': 'probability of replacing a token with mask (normalized by length)'})
    mask_selection: MASKING_DISTRIBUTION_CHOICES = field(default='static', metadata={'help': 'how to choose masks'})
    ...
    mask_min_space: Optional[int] = field(default=1, metadata={'help': 'min space between spans (if no overlap is enabled)'})
    ...
    freeze_finetune_updates: int = field(default=0, metadata={'help': 'dont finetune wav2vec for this many updates'})
    feature_grad_mult: float = field(default=0.0, metadata={'help': 'reset feature grad mult in wav2vec 2.0 to this'})
    layerdrop: float = field(default=0.0, metadata={'help': 'probability of dropping a layer in wav2vec 2.0'})
    drop_path: float = 0
    mask_channel_min_space: Optional[int] = field(default=1, metadata={'help': 'min space between spans (if no overlap is enabled)'})
    mask_channel_before: bool = False
    normalize: bool = II('task.normalize')
    update_alibi: bool = True
    data: str = II('task.data')
    w2v_args: Any = None
    offload_activations: bool = field(default=False, metadata={'help': 'offload_activations'})
    min_params_to_wrap: int = field(default=int(100000000.0), metadata={'help': 'minimum number of params for a layer to be wrapped with FSDP() when training with --ddp-backend=fully_sharded. Smaller values will improve memory efficiency, but may make torch.distributed communication less efficient due to smaller input sizes. This option is set to 0 (i.e., always wrap) when --checkpoint-activations or --offload-activations are passed.'})
    ...
    freeze_regex: Optional[str] = field(default=None)
...
class Wav2Vec2Seq2SeqConfig(Wav2Vec2AsrConfig):
    ...
    decoder_dropout: float = field(default=0.0, metadata={'help': 'dropout probability in the decoder'})
    decoder_attention_dropout: float = field(default=0.0, metadata={'help': 'dropout probability for attention weights inside the decoder'})
