...
@pluginmatcher(re.compile('https?://(?:www\\.)?telemadrid\\.es/'))
class Telemadrid(Plugin):
    ...
    def _get_streams(self):
        if False:
            while True:
                i = 10
        data = self.session.http.get(self.url, schema=validate.Schema(validate.parse_html(), validate.xml_find('.//video-js[@data-video-id][@data-account][@data-player][1]'), validate.union_get('data-video-id', 'data-account', 'data-player')))
